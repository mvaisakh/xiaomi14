			      ...
			      struct kgsl_device_getproperty *param)
{
	...
	case KGSL_PROP_DEVICE_INFO: {
		...
		devinfo.gmem_sizebytes = adreno_dev->gpucore->gmem_size;
	...
	case KGSL_PROP_DEVICE_SHADOW: {
		...
		if (device->memstore->hostptr) {
			...
			shadowprop.gpuaddr =
				(unsigned int)KGSL_MEMSTORE_TOKEN_ADDRESS;
			shadowprop.size = (unsigned int)device->memstore->size;
			/*
...
static long
adreno_ioctl_perfcounter_query_compat(struct kgsl_device_private *dev_priv,
				      unsigned int cmd, void *data)
{
	...
	result = adreno_perfcounter_query_group(adreno_dev, query.groupid,
						query.countables, query.count,
						&query.max_counters);
...
static long
adreno_ioctl_perfcounter_read_compat(struct kgsl_device_private *dev_priv,
				     unsigned int cmd, void *data)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(dev_priv->device);
   ...
   */
	...
	read.reads = (struct kgsl_perfcounter_read_group __user *)(uintptr_t)
			     read32->reads;
...
long adreno_compat_ioctl(struct kgsl_device_private *dev_priv, unsigned int cmd,
			 unsigned long arg)
{
