 ...
 */
static u64
a6xx_counter_read_norestore(struct adreno_device *adreno_dev,
...
static int
a6xx_hwsched_counter_enable(struct adreno_device *adreno_dev,
			    const struct adreno_perfcount_group *group,
			    u32 counter, u32 countable)
{
	...
	return a6xx_hwsched_counter_inline_enable(adreno_dev, group, counter,
						  countable);
}

static int
a6xx_counter_inline_enable(struct adreno_device *adreno_dev,
			   const struct adreno_perfcount_group *group,
			   unsigned int counter, unsigned int countable)
{
	...
	ret = a6xx_ringbuffer_addcmds(adreno_dev, rb, NULL, F_NOTPROTECTED,
				      cmds, 3, 0, NULL);
	if (ret)
		return ret;
				    ...
				    unsigned int countable)
{
   ...
   */
	...
	kgsl_regrmw(device, reg->select, GBIF_PERF_SEL_RMW_MASK << shift,
		    countable << shift);
	/* enable counter */
...
static int
a630_counter_vbif_pwr_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static int
a6xx_counter_gbif_pwr_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
   ...
   * clearing the counter
   */
	kgsl_regrmw(device, reg->select + GBIF_PWR_CLR_REG_EN_OFF, perfctr_mask,
		    perfctr_mask);
	kgsl_regrmw(device, reg->select + GBIF_PWR_CLR_REG_EN_OFF, perfctr_mask,
		    0);
	/* select the desired countable */
...
static int
a6xx_counter_alwayson_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	return 0;
...
static u64
a6xx_counter_alwayson_read(struct adreno_device *adreno_dev,
			   const struct adreno_perfcount_group *group,
			   unsigned int counter)
{
	struct adreno_perfcount_register *reg = &group->regs[counter];
...
static int
a6xx_counter_gmu_xoclk_enable(struct adreno_device *adreno_dev,
			      const struct adreno_perfcount_group *group,
			      unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static int
a6xx_counter_gmu_gmuclk_enable(struct adreno_device *adreno_dev,
			       const struct adreno_perfcount_group *group,
			       unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
   ...
   */
	a6xx_write_gmu_counter_enable(device, reg, 16 + (counter * 8),
				      countable);
...
static int
a6xx_counter_gmu_perf_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static struct adreno_perfcount_register a6xx_perfcounters_uche[] = {
	...
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A6XX_RBBM_PERFCTR_UCHE_10_LO,
	  A6XX_RBBM_PERFCTR_UCHE_10_HI, 69, A6XX_UCHE_PERFCTR_UCHE_SEL_10 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A6XX_RBBM_PERFCTR_UCHE_11_LO,
	  A6XX_RBBM_PERFCTR_UCHE_11_HI, 70, A6XX_UCHE_PERFCTR_UCHE_SEL_11 },
};
...
#define A6XX_REGULAR_PERFCOUNTER_GROUP(offset, name)              \
	A6XX_PERFCOUNTER_GROUP(offset, name, a6xx_counter_enable, \
			       a6xx_counter_read, a6xx_counter_load)

static const struct adreno_perfcount_group
	a630_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		A6XX_REGULAR_PERFCOUNTER_GROUP(CP, cp),
		A6XX_PERFCOUNTER_GROUP_FLAGS(RBBM, rbbm, 0, a6xx_counter_enable,
					     a6xx_counter_read,
					     a6xx_counter_load),
...
static const struct adreno_perfcount_group
	a6xx_legacy_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		...
		A6XX_PERFCOUNTER_GROUP_FLAGS(VBIF, gbif, 0,
					     a6xx_counter_gbif_enable,
					     a6xx_counter_read_norestore, NULL),
		A6XX_PERFCOUNTER_GROUP_FLAGS(VBIF_PWR, gbif_pwr,
					     ADRENO_PERFCOUNTER_GROUP_FIXED,
					     a6xx_counter_gbif_pwr_enable,
					     a6xx_counter_read_norestore, NULL),
		A6XX_PERFCOUNTER_GROUP_FLAGS(ALWAYSON, alwayson,
					     ADRENO_PERFCOUNTER_GROUP_FIXED,
					     a6xx_counter_alwayson_enable,
...
static const struct adreno_perfcount_group
	a6xx_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		A6XX_REGULAR_PERFCOUNTER_GROUP(CP, cp),
		A6XX_PERFCOUNTER_GROUP_FLAGS(RBBM, rbbm, 0, a6xx_counter_enable,
					     a6xx_counter_read,
					     a6xx_counter_load),
		...
		A6XX_PERFCOUNTER_GROUP_FLAGS(GMU_XOCLK, gmu_xoclk, 0,
					     a6xx_counter_gmu_xoclk_enable,
					     a6xx_counter_read_norestore, NULL),
		A6XX_PERFCOUNTER_GROUP_FLAGS(GMU_GMUCLK, gmu_gmuclk, 0,
					     a6xx_counter_gmu_gmuclk_enable,
					     a6xx_counter_read_norestore, NULL),
		A6XX_PERFCOUNTER_GROUP_FLAGS(GMU_PERF, gmu_perf, 0,
					     a6xx_counter_gmu_perf_enable,
					     a6xx_counter_read_norestore, NULL),
	};

static const struct adreno_perfcount_group
	a6xx_hwsched_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		A6XX_REGULAR_PERFCOUNTER_GROUP(CP, cp),
		A6XX_PERFCOUNTER_GROUP_FLAGS(RBBM, rbbm, 0, a6xx_counter_enable,
					     a6xx_counter_read,
					     a6xx_counter_load),
