...
#define PREEMPT_SMMU_RECORD(_field) offsetof(struct a5xx_cp_smmu_info, _field)
...
a5xx_next_ringbuffer(struct adreno_device *adreno_dev)
{
				 ...
				 struct adreno_context *drawctxt, u32 *cmds)
{
     ...
     */
		if (drawctxt->base.flags & KGSL_CONTEXT_SECURE)
			...
			preempt_style =
				FIELD_GET(KGSL_CONTEXT_PREEMPT_STYLE_MASK,
					  drawctxt->base.flags);
	}
   ...
   */
	...
	*cmds++ = ((preempt_style == KGSL_CONTEXT_PREEMPT_STYLE_FINEGRAIN) ? 2 :
									     0);
					   ...
					   uint64_t counteraddr)
{
	...
	if (IS_ERR_OR_NULL(rb->preemption_desc))
		rb->preemption_desc = kgsl_allocate_global(
			device, A5XX_CP_CTXRECORD_SIZE_IN_BYTES, SZ_16K, 0,
	...
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(info), 0);
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(data), 0);
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(cntl),
			      A5XX_CP_RB_CNTL_DEFAULT);
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(rptr), 0);
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(wptr), 0);
	...
	kgsl_sharedmem_writeq(rb->preemption_desc, PREEMPT_RECORD(counter),
			      counteraddr);
...
int a5xx_preemption_init(struct adreno_device *adreno_dev)
{
	...
	if (IS_ERR_OR_NULL(preempt->scratch))
		preempt->scratch = kgsl_allocate_global(
			device,
			adreno_dev->num_ringbuffers *
				A5XX_CP_CTXRECORD_PREEMPTION_COUNTER_SIZE,
			0, 0, 0, "preemption_counters");
	...
	if (kgsl_mmu_is_perprocess(&device->mmu) &&
	    IS_ERR_OR_NULL(iommu->smmu_info))
		iommu->smmu_info = kgsl_allocate_global(
			device, PAGE_SIZE, 0, KGSL_MEMFLAGS_GPUREADONLY,
			KGSL_MEMDESC_PRIVILEGED, "smmu_info");
