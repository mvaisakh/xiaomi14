...
#define PREEMPT_SMMU_RECORD(_field) offsetof(struct a6xx_cp_smmu_info, _field)
			 ...
			 bool atomic)
{
     ...
     */
		if (rb->skip_inline_wptr) {
			ret = a6xx_fenced_write(
				adreno_dev, A6XX_CP_RB_WPTR, rb->wptr,
	...
	if (val) {
		if (adreno_is_a660(adreno_dev) || adreno_is_a663(adreno_dev))
			gmu_core_regwrite(device,
					  A6XX_GMU_PWR_COL_PREEMPT_KEEPALIVE,
					  0x1);
		else
			gmu_core_regrmw(device, A6XX_GMU_AO_SPARE_CNTL, 0x0,
					0x2);
	} else {
		if (adreno_is_a660(adreno_dev) || adreno_is_a663(adreno_dev))
			gmu_core_regwrite(device,
			...
			gmu_core_regrmw(device, A6XX_GMU_AO_SPARE_CNTL, 0x2,
					0x0);
	}
}
...
a6xx_next_ringbuffer(struct adreno_device *adreno_dev)
{
...
void a6xx_preemption_trigger(struct adreno_device *adreno_dev, bool atomic)
{
   ...
   */
	...
	if (a6xx_fenced_write(
		    adreno_dev, A6XX_CP_CONTEXT_SWITCH_NON_PRIV_RESTORE_ADDR_LO,
		    lower_32_bits(gpuaddr), FENCE_STATUS_WRITEDROPPED1_MASK))
		goto err;

	if (a6xx_fenced_write(
		    adreno_dev, A6XX_CP_CONTEXT_SWITCH_NON_PRIV_RESTORE_ADDR_HI,
...
int a6xx_preemption_init(struct adreno_device *adreno_dev)
{
   ...
   * data. Reserve 11 dwords in the device scratch buffer from
   ...
   */
	if (!adreno_dev->perfcounter) {
		u32 *postamble =
			device->scratch->hostptr + SCRATCH_POSTAMBLE_OFFSET;
		u32 count = 0;
...
int a6xx_preemption_context_init(struct kgsl_context *context)
{
   ...
   */
	context->user_ctxt_record = gpumem_alloc_entry(
		context->dev_priv, A6XX_CP_CTXRECORD_USER_RESTORE_SIZE, flags);
