...
static int _rb_readtimestamp(struct kgsl_device *device, void *priv,
			    ...
			    struct adreno_ringbuffer *rb, int id)
{
	...
	adreno_allocate_global(device, &rb->profile_desc, PAGE_SIZE, 0,
	...
	ret = adreno_allocate_global(device, &rb->buffer_desc, KGSL_RB_SIZE,
				     SZ_4K, KGSL_MEMFLAGS_GPUREADONLY, priv,
				     "ringbuffer");
	if (ret)
		return ret;
   ...
   */
	...
	if (context->l3_pwr_constraint.type &&
		...
		if (!device->num_l3_pwrlevels) {
			dev_err_once(device->dev, "l3 voting not available\n");
			return;
		}
       ...
       */
			...
			ret = qcom_dcvs_update_votes(KGSL_L3_DEVICE, &freq, 1,
						     DCVS_SLOW_PATH);
			if (!ret) {
				trace_kgsl_constraint(
					device, KGSL_CONSTRAINT_L3_PWRLEVEL,
					new_l3, 1);
				...
				struct adreno_submit_time *time)
{
   ...
   */
	...
	ret = gpudev->ringbuffer_submitcmd(adreno_dev, cmdobj, flags, time);
...
static inline int
adreno_ringbuffer_check_timestamp(struct adreno_ringbuffer *rb,
				    ...
				    unsigned int timestamp, unsigned int msecs)
{
	...
	if (msecs == 0)
		return adreno_ringbuffer_check_timestamp(
			       rb, timestamp, KGSL_TIMESTAMP_RETIRED) ?
			       0 :
			       -EBUSY;
   ...
   */
	if (!ret && !adreno_ringbuffer_check_timestamp(
			    rb, timestamp, KGSL_TIMESTAMP_RETIRED)) {
		ret = -EAGAIN;
	}
