...
static int globals_show(struct seq_file *s, void *unused)
{
	...
	list_for_each_entry(md, &device->globals, node) {
		...
		seq_printf(s, "0x%pK-0x%pK %16llu %5s %s\n",
			   ...
			   (u64 *)(uintptr_t)(memdesc->gpuaddr + memdesc->size -
					      1),
			   memdesc->size, flags, md->name);
	}
...
DEFINE_DEBUGFS_ATTRIBUTE(_reserved_fops, kgsl_pool_reserved_get, NULL,
			 "%llu\n");
DEFINE_DEBUGFS_ATTRIBUTE(_page_count_fops, kgsl_pool_page_count_get, NULL,
			 "%llu\n");

void kgsl_pool_init_debugfs(struct dentry *pool_debugfs, char *name, void *pool)
{
	...
	dentry = debugfs_create_file("reserved", 0444, pool_debugfs, pool,
				     &_reserved_fops);

	WARN((IS_ERR_OR_NULL(dentry)),
	     "Unable to create 'reserved' file for %s\n", name);

	dentry = debugfs_create_file("count", 0444, pool_debugfs, pool,
				     &_page_count_fops);

	WARN((IS_ERR_OR_NULL(dentry)), "Unable to create 'count' file for %s\n",
	     name);
}
...
static int print_mem_entry(void *data, void *ptr)
{
	...
	seq_printf(s,
		   ...
		   /*
              ...
              */
		   NULL, m->size, entry->id, flags, memtype_str(usermem_type),
						   ...
						   void *ptr, loff_t pos)
{
	...
	for (entry = idr_get_next(&private->mem_idr, &id); entry;
	     id++, entry = idr_get_next(&private->mem_idr, &id), temp_pos++) {
...
static void *process_mem_seq_next(struct seq_file *s, void *ptr, loff_t *pos)
{
	++*pos;
...
static int process_mem_seq_show(struct seq_file *s, void *ptr)
{
	if (ptr == SEQ_START_TOKEN) {
		seq_printf(
			...
			"usage", "sglen", "mapcnt", "eglsrf", "eglimg",
			"inode");
		return 0;
	} else
...
void kgsl_core_debugfs_init(void)
{
	...
	dentry = debugfs_create_file("pool_size", 0444, mempools_debugfs, NULL,
				     &_pool_size_fops);
