...
static ssize_t cur_ab_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct devfreq *df = to_devfreq(dev);
	struct msm_busmon_extended_profile *bus_profile = container_of(
		(df->profile), struct msm_busmon_extended_profile, profile);
...
static const struct device_attribute *gpubw_attr_list[] = {
	&dev_attr_sampling_interval, &dev_attr_cur_ab, NULL
};
...
static int devfreq_gpubw_get_target(struct devfreq *df, unsigned long *freq)
{
   ...
   */
	...
	*freq = stats->current_frequency;
	...
	bus_profile->flag =
		generate_hint(priv, b.buslevel, *freq, b.gpu_minfreq);
...
static int gpubw_start(struct devfreq *devfreq)
{
	...
	struct msm_busmon_extended_profile *bus_profile =
		container_of((devfreq->profile),
			     struct msm_busmon_extended_profile, profile);
