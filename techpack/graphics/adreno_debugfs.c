...
static int _lm_limit_set(void *data, u64 val)
{
	...
	if (adreno_dev->lm_enabled)
		return adreno_power_cycle_u32(adreno_dev, &adreno_dev->lm_limit,
					      val);
...
DEFINE_DEBUGFS_ATTRIBUTE(_lm_limit_fops, _lm_limit_get, _lm_limit_set,
			 "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(_lm_threshold_fops, _lm_threshold_count_get, NULL,
			 "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(_coop_reset_fops, _coop_reset_get, _coop_reset_set,
			 "%llu\n");
...
static int _gpu_client_pf_set(void *data, u64 val)
{
	...
	return adreno_power_cycle(ADRENO_DEVICE(device), set_gpu_client_pf,
				  &val);
}
			     ...
			     struct kgsl_drawobj_sync_event *sync_event)
{
	...
	case KGSL_CMD_SYNCPOINT_TYPE_TIMESTAMP: {
		seq_printf(s, "sync: ctx: %u ts: %u", sync_event->context->id,
	...
	case KGSL_CMD_SYNCPOINT_TYPE_FENCE: {
		...
		for (i = 0; info && i < info->num_fences; i++)
			seq_printf(s, "sync: %s", info->fences[i].name);
		break;
	}
...
#define print_flags(_s, _flag, _array...)                                     \
	({                                                                    \
		const struct flag_entry symbols[] = { _array, { -1, NULL } }; \
		_print_flags(_s, symbols, _flag);                             \
	})

static void syncobj_print(struct seq_file *s, struct kgsl_drawobj_sync *syncobj)
{
...
static void cmdobj_print(struct seq_file *s, struct kgsl_drawobj_cmd *cmdobj)
{
	...
	seq_puts(s, " priv: ");
	print_flags(s, cmdobj->priv, { BIT(CMDOBJ_SKIP), "skip" },
...
static int ctx_print(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "id: %u type: %s priority: %d process: %s (%d) tid: %d\n",
		   drawctxt->base.id, kgsl_context_type(drawctxt->type),
		   drawctxt->base.priority, drawctxt->base.proc_priv->comm,
		   pid_nr(drawctxt->base.proc_priv->pid), drawctxt->base.tid);
	...
	seq_printf(s, "queued: %u consumed: %u retired: %u global:%u\n", queued,
		   consumed, retired, drawctxt->internal_timestamp);
				 ...
				 struct adreno_context *ctx)
{
   ...
   */
	...
	ctx->debug_root = debugfs_create_file(
		name, 0444, adreno_dev->ctx_d_debugfs, ctx, &ctx_fops);
}
...
DEFINE_DEBUGFS_ATTRIBUTE(_bcl_throttle_fops, _bcl_throttle_time_us_get, NULL,
			 "%llu\n");
...
static int _skipsaverestore_store(void *data, u64 val)
{
	...
	if (adreno_dev->hwsched_enabled)
		return adreno_power_cycle_bool(
			adreno_dev, &adreno_dev->preempt.skipsaverestore, val);
...
DEFINE_DEBUGFS_ATTRIBUTE(skipsaverestore_fops, _skipsaverestore_show,
			 _skipsaverestore_store, "%llu\n");
...
static int _usesgmem_store(void *data, u64 val)
{
	...
	if (adreno_dev->hwsched_enabled)
		return adreno_power_cycle_bool(
			adreno_dev, &adreno_dev->preempt.usesgmem, val);
...
DEFINE_DEBUGFS_ATTRIBUTE(usesgmem_fops, _usesgmem_show, _usesgmem_store,
			 "%llu\n");
...
static int _preempt_level_store(void *data, u64 val)
{
	...
	if (adreno_dev->hwsched_enabled)
		return adreno_power_cycle_u32(
			adreno_dev, &adreno_dev->preempt.preempt_level,
			min_t(u64, val, 2));
...
DEFINE_DEBUGFS_ATTRIBUTE(preempt_level_fops, _preempt_level_show,
			 _preempt_level_store, "%llu\n");
...
static int _ifpc_hyst_store(void *data, u64 val)
{
	...
	return adreno_power_cycle_u32(adreno_dev, &adreno_dev->ifpc_hyst, hyst);
}
...
DEFINE_DEBUGFS_ATTRIBUTE(ifpc_hyst_fops, _ifpc_hyst_show, _ifpc_hyst_store,
			 "%llu\n");
...
void adreno_debugfs_init(struct adreno_device *adreno_dev)
{
	...
	adreno_dev->ctx_d_debugfs =
		debugfs_create_dir("ctx", device->d_debugfs);
	...
	if (ADRENO_FEATURE(adreno_dev, ADRENO_LM)) {
		...
		debugfs_create_file("lm_threshold_count", 0444,
				    device->d_debugfs, device,
				    &_lm_threshold_fops);
	}

	if (adreno_is_a5xx(adreno_dev))
		debugfs_create_file("isdb", 0644, device->d_debugfs, device,
				    &_isdb_fops);
	...
	debugfs_create_file("ctxt_record_size", 0644, snapshot_dir, device,
			    &_ctxt_record_size_fops);
	debugfs_create_file("gpu_client_pf", 0644, snapshot_dir, device,
			    &_gpu_client_pf_fops);
	debugfs_create_bool("dump_all_ibs", 0644, snapshot_dir,
			    &device->dump_all_ibs);

	adreno_dev->bcl_debugfs_dir =
		debugfs_create_dir("bcl", device->d_debugfs);
	if (!IS_ERR_OR_NULL(adreno_dev->bcl_debugfs_dir)) {
		debugfs_create_file("sid0", 0644, adreno_dev->bcl_debugfs_dir,
				    device, &_sid0_fops);
		debugfs_create_file("sid1", 0644, adreno_dev->bcl_debugfs_dir,
				    device, &_sid1_fops);
		debugfs_create_file("sid2", 0644, adreno_dev->bcl_debugfs_dir,
				    device, &_sid2_fops);
		debugfs_create_file("bcl_throttle_time_us", 0444,
				    adreno_dev->bcl_debugfs_dir, device,
				    &_bcl_throttle_fops);
	}

	adreno_dev->preemption_debugfs_dir =
		debugfs_create_dir("preemption", device->d_debugfs);
	if (!IS_ERR_OR_NULL(adreno_dev->preemption_debugfs_dir)) {
		debugfs_create_file("preempt_level", 0644,
				    adreno_dev->preemption_debugfs_dir, device,
				    &preempt_level_fops);
		debugfs_create_file("usesgmem", 0644,
				    adreno_dev->preemption_debugfs_dir, device,
				    &usesgmem_fops);
		debugfs_create_file("skipsaverestore", 0644,
				    adreno_dev->preemption_debugfs_dir, device,
				    &skipsaverestore_fops);
	}
}
