...
static struct gmu_vma_entry a6xx_gmu_vma_legacy[] = {
	...
	[GMU_ICACHE] = { .start = 0x04000,
			 .size = (SZ_256K - SZ_16K),
			 .next_va = 0x4000 },
	[GMU_DTCM] = { .start = 0x40000, .size = SZ_16K },
	[GMU_DCACHE] = { .start = 0x44000,
			 .size = (SZ_256K - SZ_16K),
			 .next_va = 0x44000 },
	[GMU_NONCACHED_KERNEL] = { .start = 0x60000000,
				   .size = SZ_512M,
				   .next_va = 0x60000000 },
};
				       ...
				       const char *buf, size_t count)
{
	struct a6xx_gmu_device *gmu =
		container_of(kobj, struct a6xx_gmu_device, log_kobj);
	bool val;
	int ret;
...
{
	struct a6xx_gmu_device *gmu =
		container_of(kobj, struct a6xx_gmu_device, stats_kobj);
	bool val;
	int ret;
...
struct a6xx_gmu_device *to_a6xx_gmu(struct adreno_device *adreno_dev)
{
	struct a6xx_device *a6xx_dev =
		container_of(adreno_dev, struct a6xx_device, adreno_dev);
...
void a6xx_load_rsc_ucode(struct adreno_device *adreno_dev)
{
	...
	_regwrite(rscc, A6XX_RSCC_HIDDEN_TCS_CMD0_DATA + RSC_CMD_OFFSET * 2,
		  0x80000000);
	_regwrite(rscc, A6XX_RSCC_HIDDEN_TCS_CMD0_ADDR + RSC_CMD_OFFSET * 2, 0);
...
int a6xx_load_pdc_ucode(struct adreno_device *adreno_dev)
{
   ...
   */
	if (gmu->pdc_cfg_base == NULL) {
		if (res_cfg)
			gmu->pdc_cfg_base =
				devm_ioremap(&gmu->pdev->dev, res_cfg->start,
					     resource_size(res_cfg));
		else if (res_pdc)
			gmu->pdc_cfg_base = devm_ioremap(
				&gmu->pdev->dev, res_pdc->start + cfg_offset,
				0x10000);
   ...
   */
	if (gmu->pdc_seq_base == NULL) {
		res_seq = platform_get_resource_byname(
			gmu->pdev, IORESOURCE_MEM, "kgsl_gmu_pdc_seq");

		if (res_seq)
			gmu->pdc_seq_base =
				devm_ioremap(&gmu->pdev->dev, res_seq->start,
					     resource_size(res_seq));
		else if (res_pdc)
			gmu->pdc_seq_base = devm_ioremap(
				&gmu->pdev->dev, res_pdc->start + seq_offset,
				0x10000);
...
static void a6xx_gmu_power_config(struct adreno_device *adreno_dev)
{
	...
	gmu_core_regwrite(device, A6XX_GMU_PWR_COL_INTER_FRAME_CTRL, 0x9C40400);

	if (gmu->idle_level == GPU_HW_IFPC) {
		gmu_core_regwrite(device, A6XX_GMU_PWR_COL_INTER_FRAME_HYST,
				  A6X_GMU_SHORT_IFPC_HYST |
					  adreno_dev->ifpc_hyst);
...
int a6xx_gmu_device_start(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device, A6XX_GMU_CM3_FW_INIT_RESULT, val,
				      GMU_START_TIMEOUT, mask)) {
...
int a6xx_gmu_hfi_start(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device, A6XX_GMU_HFI_CTRL_STATUS, BIT(0),
				      GMU_START_TIMEOUT, BIT(0))) {
...
int a6xx_rscc_wakeup_sequence(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device, A6XX_GMU_RSCC_CONTROL_ACK, BIT(1),
				      GPU_START_TIMEOUT, BIT(1))) {
		dev_err(dev, "Failed to do GPU RSC power on\n");
		return -ETIMEDOUT;
	}

	if (timed_poll_check_rscc(device, A6XX_RSCC_SEQ_BUSY_DRV0, 0,
				  GPU_START_TIMEOUT, 0xFFFFFFFF))
		goto error_rsc;
...
int a6xx_rscc_sleep_sequence(struct adreno_device *adreno_dev)
{
	...
	ret = timed_poll_check_rscc(device, A6XX_GPU_RSCC_RSC_STATUS0_DRV0,
				    BIT(16), GPU_START_TIMEOUT, BIT(16));
			       ...
			       struct a6xx_gmu_device *gmu)
{
	...
	gmu_core_blkwrite(device, A6XX_GMU_CM3_ITCM_START, fw->data, fw->size);
...
int a6xx_gmu_load_fw(struct adreno_device *adreno_dev)
{
	...
	while (fw < gmu->fw_image->data + gmu->fw_image->size) {
		...
		if (id == GMU_ITCM) {
			load_tcm(adreno_dev, fw, A6XX_GMU_CM3_ITCM_START,
				 gmu->vma[GMU_ITCM].start, blk);
		} else if (id == GMU_DTCM) {
			load_tcm(adreno_dev, fw, A6XX_GMU_CM3_DTCM_START,
...
int a6xx_gmu_oob_set(struct kgsl_device *device, enum oob_request req)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct a6xx_gmu_device *gmu = to_a6xx_gmu(adreno_dev);
	int ret = 0;
...
void a6xx_gmu_oob_clear(struct kgsl_device *device, enum oob_request req)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct a6xx_gmu_device *gmu = to_a6xx_gmu(adreno_dev);
	int clear;
...
static u32 a6xx_rscc_tcsm_drv0_status_reglist[] = {
	A6XX_RSCC_TCS0_DRV0_STATUS, A6XX_RSCC_TCS1_DRV0_STATUS,
	A6XX_RSCC_TCS2_DRV0_STATUS, A6XX_RSCC_TCS3_DRV0_STATUS,
	A6XX_RSCC_TCS4_DRV0_STATUS, A6XX_RSCC_TCS5_DRV0_STATUS,
	A6XX_RSCC_TCS6_DRV0_STATUS, A6XX_RSCC_TCS7_DRV0_STATUS,
	A6XX_RSCC_TCS8_DRV0_STATUS, A6XX_RSCC_TCS9_DRV0_STATUS,
};
				    ...
				    unsigned int timeout)
{
	...
	int count = adreno_is_a650_family(adreno_dev) ?
			    ARRAY_SIZE(a6xx_rscc_tcsm_drv0_status_reglist) :
			    4;
	int i, ret = 0;

	for (i = 0; i < count; i++)
		ret |= timed_poll_check_rscc(
			device, a6xx_rscc_tcsm_drv0_status_reglist[i], BIT(0),
			timeout, BIT(0));
...
int a6xx_gmu_sptprac_enable(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device, A6XX_GMU_SPTPRAC_PWR_CLK_STATUS,
...
bool a6xx_gmu_sptprac_is_on(struct adreno_device *adreno_dev)
{
	...
	if (adreno_is_a619_holi(adreno_dev))
		kgsl_regread(device, A6XX_GMU_SPTPRAC_PWR_CLK_STATUS, &val);
	else
...
int a6xx_gmu_wait_for_lowest_idle(struct adreno_device *adreno_dev)
{
	...
	if (adreno_is_a660(adreno_dev)) {
		...
		gmu_core_regread(device, A6XX_GMU_PWR_COL_PREEMPT_KEEPALIVE,
				 &val);
...
int a6xx_gmu_wait_for_idle(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device,
				      A6XX_GPU_GMU_AO_GPU_CX_BUSY_STATUS, 0,
				      GMU_START_TIMEOUT, CXGXCPUBUSYIGNAHB)) {
		gmu_core_regread(device, A6XX_GPU_GMU_AO_GPU_CX_BUSY_STATUS2,
				 &status2);
		dev_err(&gmu->pdev->dev,
...
static void a6xx_gmu_enable_throttle_counters(struct adreno_device *adreno_dev)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
	u32 val;
...
void a6xx_gmu_register_config(struct adreno_device *adreno_dev)
{
   ...
   */
	if (adreno_is_a660_shima(adreno_dev))
		chipid |=
			((ADRENO_CHIPID_PATCH(adreno_dev->chipid) >> 4) << 28);
...
int a6xx_gmu_memory_init(struct adreno_device *adreno_dev)
{
	...
	if (adreno_is_a630(adreno_dev) || adreno_is_a615_family(adreno_dev)) {
		if (IS_ERR_OR_NULL(gmu->dump_mem))
			gmu->dump_mem = reserve_gmu_kernel_block(
				gmu, 0, SZ_16K, GMU_NONCACHED_KERNEL, 0);
	...
	if (IS_ERR_OR_NULL(gmu->gmu_log))
		gmu->gmu_log = reserve_gmu_kernel_block(
...
static void a6xx_gmu_pwrctrl_suspend(struct adreno_device *adreno_dev)
{
   ...
   */
	if (gmu->gx_gdsc) {
       ...
       */
			if (adreno_is_a660(adreno_dev)) {
				gmu_core_regrmw(device, A6XX_GPU_CPR_FSM_CTL, 1,
						0);
				ndelay(520);
			}
...
static int a6xx_gmu_dcvs_set(struct adreno_device *adreno_dev, int gpu_pwrlevel,
			     int bus_level)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
	struct kgsl_pwrctrl *pwr = &device->pwrctrl;
...
static int a6xx_gmu_ifpc_store(struct kgsl_device *device, unsigned int val)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct a6xx_gmu_device *gmu = to_a6xx_gmu(adreno_dev);
...
nmi:
	...
	wmb();
	if (ADRENO_QUIRK(adreno_dev, ADRENO_QUIRK_HFI_USE_REG))
		gmu_core_regwrite(device, A6XX_GMU_NMI_CONTROL_STATUS, 0);
...
static int a6xx_gmu_wait_for_active_transition(struct kgsl_device *device)
{
	unsigned int reg;
	struct a6xx_gmu_device *gmu = to_a6xx_gmu(ADRENO_DEVICE(device));
...
void a6xx_gmu_handle_watchdog(struct adreno_device *adreno_dev)
{
	...
	gmu_core_regread(device, A6XX_GMU_AO_HOST_INTERRUPT_MASK, &mask);
	gmu_core_regwrite(device, A6XX_GMU_AO_HOST_INTERRUPT_MASK,
		       ...
		       struct kgsl_snapshot *snapshot)
{
	...
	gmu_core_regwrite(device, A6XX_GMU_GMU2HOST_INTR_CLR, 0xffffffff);
	gmu_core_regwrite(device, A6XX_GMU_GMU2HOST_INTR_MASK, HFI_IRQ_MASK);
}
...
static void a6xx_gmu_force_first_boot(struct kgsl_device *device)
{
	...
	if (gmu->pdc_cfg_base) {
		...
		val = __raw_readl(gmu->pdc_cfg_base +
				  (PDC_GPU_ENABLE_PDC << 2));
...
static int a6xx_gmu_reg_probe(struct adreno_device *adreno_dev)
{
	...
	ret = kgsl_regmap_add_region(&device->regmap, gmu->pdev, "kgsl_gmu_reg",
				     NULL, NULL);
	if (ret)
...
{
	struct a6xx_gmu_device *gmu =
		container_of(nb, struct a6xx_gmu_device, gdsc_nb);
...
int a6xx_gmu_probe(struct kgsl_device *device, struct platform_device *pdev)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct a6xx_gmu_device *gmu = to_a6xx_gmu(adreno_dev);
...
int a6xx_halt_gbif(struct adreno_device *adreno_dev)
{
	...
	ret = adreno_wait_for_halt_ack(device, A6XX_GBIF_HALT_ACK,
...
static int a630_vbif_halt(struct adreno_device *adreno_dev)
{
	...
	ret = adreno_wait_for_halt_ack(device, A6XX_VBIF_XIN_HALT_CTRL1,
				       A6XX_VBIF_XIN_HALT_CTRL0_MASK);
...
int a6xx_gmu_device_probe(struct platform_device *pdev, u32 chipid,
			  const struct adreno_gpu_core *gpucore)
{
	...
	a6xx_dev = devm_kzalloc(&pdev->dev, sizeof(*a6xx_dev), GFP_KERNEL);
	if (!a6xx_dev)
		return -ENOMEM;
