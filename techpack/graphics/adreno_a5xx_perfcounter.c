...
static u64
a5xx_counter_read_norestore(struct adreno_device *adreno_dev,
...
static int
a5xx_counter_inline_enable(struct adreno_device *adreno_dev,
			   const struct adreno_perfcount_group *group,
			   unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
	...
	ret = a5xx_ringbuffer_addcmds(adreno_dev, rb, NULL, F_NOTPROTECTED,
				      cmds, 3, 0, NULL);
...
static int
a5xx_counter_vbif_pwr_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static int
a5xx_counter_alwayson_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	return 0;
...
static u64
a5xx_counter_alwayson_read(struct adreno_device *adreno_dev,
			   const struct adreno_perfcount_group *group,
			   unsigned int counter)
{
	struct adreno_perfcount_register *reg = &group->regs[counter];
...
static int
a5xx_counter_pwr_gpmu_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static int
a5xx_counter_pwr_alwayson_enable(struct adreno_device *adreno_dev,
				 const struct adreno_perfcount_group *group,
				 unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static struct adreno_perfcount_register a5xx_pwrcounters_uche[] = {
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_UCHE_POWER_COUNTER_0_LO,
	  A5XX_UCHE_POWER_COUNTER_0_HI, -1, A5XX_UCHE_POWERCTR_UCHE_SEL_0 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_UCHE_POWER_COUNTER_1_LO,
	  A5XX_UCHE_POWER_COUNTER_1_HI, -1, A5XX_UCHE_POWERCTR_UCHE_SEL_1 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_UCHE_POWER_COUNTER_2_LO,
	  A5XX_UCHE_POWER_COUNTER_2_HI, -1, A5XX_UCHE_POWERCTR_UCHE_SEL_2 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_UCHE_POWER_COUNTER_3_LO,
	  A5XX_UCHE_POWER_COUNTER_3_HI, -1, A5XX_UCHE_POWERCTR_UCHE_SEL_3 },
};
...
static struct adreno_perfcount_register a5xx_pwrcounters_gpmu[] = {
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_GPMU_POWER_COUNTER_0_LO,
	  A5XX_GPMU_POWER_COUNTER_0_HI, -1, A5XX_GPMU_POWER_COUNTER_SELECT_0 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_GPMU_POWER_COUNTER_1_LO,
	  A5XX_GPMU_POWER_COUNTER_1_HI, -1, A5XX_GPMU_POWER_COUNTER_SELECT_0 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_GPMU_POWER_COUNTER_2_LO,
	  A5XX_GPMU_POWER_COUNTER_2_HI, -1, A5XX_GPMU_POWER_COUNTER_SELECT_0 },
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_GPMU_POWER_COUNTER_3_LO,
	  A5XX_GPMU_POWER_COUNTER_3_HI, -1, A5XX_GPMU_POWER_COUNTER_SELECT_0 },
	...
	{ KGSL_PERFCOUNTER_NOT_USED, 0, 0, A5XX_GPMU_POWER_COUNTER_5_LO,
	  A5XX_GPMU_POWER_COUNTER_5_HI, -1, A5XX_GPMU_POWER_COUNTER_SELECT_1 },
};
...
#define A5XX_POWER_COUNTER_GROUP(offset, name, enable, read)   \
	[KGSL_PERFCOUNTER_GROUP_##offset##                     \
		_PWR] = { a5xx_pwrcounters_##name,             \
			  ARRAY_SIZE(a5xx_pwrcounters_##name), \
			  __stringify(name##_pwr),             \
			  0,                                   \
			  enable,                              \
			  read,                                \
			  NULL }
...
static struct adreno_perfcount_group
	a5xx_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		A5XX_REGULAR_PERFCOUNTER_GROUP(CP, cp),
		A5XX_PERFCOUNTER_GROUP(RBBM, rbbm, a5xx_counter_rbbm_enable,
				       a5xx_counter_read, a5xx_counter_load),
		...
		A5XX_PERFCOUNTER_GROUP(VBIF, vbif, a5xx_counter_vbif_enable,
		...
		A5XX_PERFCOUNTER_GROUP_FLAGS(ALWAYSON, alwayson,
					     ADRENO_PERFCOUNTER_GROUP_FIXED,
					     a5xx_counter_alwayson_enable,
					     a5xx_counter_alwayson_read, NULL),
		A5XX_POWER_COUNTER_GROUP(SP, sp, a5xx_counter_pwr_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(TP, tp, a5xx_counter_pwr_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(RB, rb, a5xx_counter_pwr_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(CCU, ccu, a5xx_counter_pwr_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(UCHE, uche, a5xx_counter_pwr_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(CP, cp, a5xx_counter_pwr_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(GPMU, gpmu,
					 a5xx_counter_pwr_gpmu_enable,
					 a5xx_counter_read_norestore),
		A5XX_POWER_COUNTER_GROUP(ALWAYSON, alwayson,
					 a5xx_counter_pwr_alwayson_enable,
					 a5xx_counter_read_norestore),
	};
