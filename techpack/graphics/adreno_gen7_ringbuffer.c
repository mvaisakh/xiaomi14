	...
	if (adreno_drawctxt_get_pagetable(rb->drawctxt_active) != pagetable) {
		...
		if (!adreno_dev->perfcounter) {
			cmds[count++] = cp_type4_packet(
				GEN7_RBBM_PERFCTR_SRAM_INIT_CMD, 1);
			cmds[count++] = 0x1;
		}
	...
	if (adreno_in_preempt_state(adreno_dev, ADRENO_PREEMPT_NONE)) {
		if (adreno_dev->cur_rb == rb) {
			...
			ret = gen7_fenced_write(
				adreno_dev, GEN7_CP_RB_WPTR, rb->_wptr,
				      ...
				      u32 target_offset, u32 *cmds)
{
	...
	dwords = gen7_get_alwayson_counter(
			      ...
			      struct adreno_submit_time *time)
{
	...
	if (IS_KERNEL_PROFILE(flags)) {
		index += GEN7_KERNEL_PROFILE(adreno_dev, cmdobj, &cmds[index],
					     started);
		index += GEN7_KERNEL_PROFILE_CONTEXT(adreno_dev, cmdobj,
   ...
   */
	...
	ret = gen7_ringbuffer_addcmds(adreno_dev, drawctxt->rb, drawctxt, flags,
