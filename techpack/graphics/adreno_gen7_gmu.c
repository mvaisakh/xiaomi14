				       ...
				       const char *buf, size_t count)
{
	struct gen7_gmu_device *gmu =
		container_of(kobj, struct gen7_gmu_device, log_kobj);
	bool val;
	int ret;
...
static ssize_t log_stream_enable_show(struct kobject *kobj,
...
static ssize_t log_group_mask_show(struct kobject *kobj,
				   struct kobj_attribute *attr, char *buf)
{
	struct gen7_gmu_device *gmu =
		container_of(kobj, struct gen7_gmu_device, log_kobj);
...
static struct kobj_attribute log_stream_enable_attr =
	__ATTR(log_stream_enable, 0644, log_stream_enable_show,
	       log_stream_enable_store);
...
{
	struct gen7_gmu_device *gmu =
		container_of(kobj, struct gen7_gmu_device, stats_kobj);
	bool val;
	int ret;
...
static ssize_t stats_enable_show(struct kobject *kobj,
				 struct kobj_attribute *attr, char *buf)
{
	struct gen7_gmu_device *gmu =
		container_of(kobj, struct gen7_gmu_device, stats_kobj);
...
static ssize_t stats_mask_show(struct kobject *kobj,
			       struct kobj_attribute *attr, char *buf)
{
	struct gen7_gmu_device *gmu =
		container_of(kobj, struct gen7_gmu_device, stats_kobj);
...
static ssize_t stats_interval_show(struct kobject *kobj,
				   struct kobj_attribute *attr, char *buf)
{
	struct gen7_gmu_device *gmu =
		container_of(kobj, struct gen7_gmu_device, stats_kobj);
...
static void _regwrite(void __iomem *regbase, unsigned int offsetwords,
		      unsigned int value)
{
	void __iomem *reg;
...
void gen7_load_rsc_ucode(struct adreno_device *adreno_dev)
{
	...
	_regwrite(rscc, GEN7_RSCC_HIDDEN_TCS_CMD0_DATA + RSC_CMD_OFFSET * 2,
		  adreno_is_gen7_2_x_family(adreno_dev) ? 0x80000021 :
							  0x80000000);
...
int gen7_gmu_enable_gdsc(struct adreno_device *adreno_dev)
{
	...
	ret = wait_for_completion_timeout(&gmu->gdsc_gate,
					  msecs_to_jiffies(5000));
	if (!ret) {
		dev_err(device->dev,
			"GPU CX wait timeout. Dumping CX votes:\n");
...
int gen7_gmu_hfi_start(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device, GEN7_GMU_HFI_CTRL_STATUS, BIT(0),
				      100, BIT(0))) {
...
int gen7_rscc_wakeup_sequence(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device, GEN7_GMU_RSCC_CONTROL_ACK, BIT(1),
				      100, BIT(1))) {
		dev_err(dev, "Failed to do GPU RSC power on\n");
		return -ETIMEDOUT;
	}

	if (gen7_timed_poll_check_rscc(gmu, GEN7_RSCC_SEQ_BUSY_DRV0, 0x0, 100,
				       UINT_MAX)) {
...
int gen7_gmu_load_fw(struct adreno_device *adreno_dev)
{
	...
	while (fw < gmu->fw_image->data + gmu->fw_image->size) {
		...
		if (id == GMU_ITCM) {
			load_tcm(adreno_dev, fw, GEN7_GMU_CM3_ITCM_START,
				 gmu->vma[GMU_ITCM].start, blk);
		} else if (id == GMU_DTCM) {
			load_tcm(adreno_dev, fw, GEN7_GMU_CM3_DTCM_START,
...
int gen7_gmu_oob_set(struct kgsl_device *device, enum oob_request req)
{
	...
	if (gmu_core_timed_poll_check(device, GEN7_GMU_GMU2HOST_INTR_INFO,
				      check, 100, check)) {
		if (req == oob_perfcntr)
...
void gen7_gmu_oob_clear(struct kgsl_device *device, enum oob_request req)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct gen7_gmu_device *gmu = to_gen7_gmu(adreno_dev);
...
static u32 gen7_rscc_tcsm_drv0_status_reglist[] = {
	GEN7_RSCC_TCS0_DRV0_STATUS, GEN7_RSCC_TCS1_DRV0_STATUS,
	GEN7_RSCC_TCS2_DRV0_STATUS, GEN7_RSCC_TCS3_DRV0_STATUS,
	GEN7_RSCC_TCS4_DRV0_STATUS, GEN7_RSCC_TCS5_DRV0_STATUS,
	GEN7_RSCC_TCS6_DRV0_STATUS, GEN7_RSCC_TCS7_DRV0_STATUS,
	GEN7_RSCC_TCS8_DRV0_STATUS, GEN7_RSCC_TCS9_DRV0_STATUS,
};

static u32 gen7_2_0_rscc_tcsm_drv0_status_reglist[] = {
	GEN7_2_0_RSCC_TCS0_DRV0_STATUS, GEN7_2_0_RSCC_TCS1_DRV0_STATUS,
	GEN7_2_0_RSCC_TCS2_DRV0_STATUS, GEN7_2_0_RSCC_TCS3_DRV0_STATUS,
	GEN7_2_0_RSCC_TCS4_DRV0_STATUS, GEN7_2_0_RSCC_TCS5_DRV0_STATUS,
	GEN7_2_0_RSCC_TCS6_DRV0_STATUS, GEN7_2_0_RSCC_TCS7_DRV0_STATUS,
	GEN7_2_0_RSCC_TCS8_DRV0_STATUS, GEN7_2_0_RSCC_TCS9_DRV0_STATUS,
};

static int gen7_complete_rpmh_votes(struct gen7_gmu_device *gmu, u32 timeout)
{
	...
	if (adreno_is_gen7_2_x_family(adreno_dev)) {
		for (i = 0;
		     i < ARRAY_SIZE(gen7_2_0_rscc_tcsm_drv0_status_reglist);
		     i++)
			ret |= gen7_timed_poll_check_rscc(
				gmu, gen7_2_0_rscc_tcsm_drv0_status_reglist[i],
				BIT(0), timeout, BIT(0));
	} else {
		for (i = 0; i < ARRAY_SIZE(gen7_rscc_tcsm_drv0_status_reglist);
		     i++)
			ret |= gen7_timed_poll_check_rscc(
				gmu, gen7_rscc_tcsm_drv0_status_reglist[i],
				BIT(0), timeout, BIT(0));
	}
...
bool gen7_gmu_gx_is_on(struct adreno_device *adreno_dev)
{
	...
	gmu_core_regread(KGSL_DEVICE(adreno_dev), GEN7_GMU_GFX_PWR_CLK_STATUS,
			 &val);
	return is_on(val);
}
...
int gen7_gmu_wait_for_lowest_idle(struct adreno_device *adreno_dev)
{
	...
	do {
		gmu_core_regread(device, GEN7_GPU_GMU_CX_GMU_RPMH_POWER_STATE,
				 &reg);
...
int gen7_gmu_wait_for_idle(struct adreno_device *adreno_dev)
{
	...
	if (gmu_core_timed_poll_check(device,
				      ...
				      100, CXGXCPUBUSYIGNAHB)) {
		gmu_core_regread(device, GEN7_GPU_GMU_AO_GPU_CX_BUSY_STATUS2,
				 &status2);
		dev_err(&gmu->pdev->dev,
...
void gen7_gmu_register_config(struct adreno_device *adreno_dev)
{
	...
	gmu_core_regwrite(device, GEN7_GMU_AHB_FENCE_RANGE_0,
			  BIT(31) | FIELD_PREP(GENMASK(30, 18), 0x32) |
   ...
   */
	...
	gmu_core_regwrite(device, GEN7_GMU_GENERAL_9, gmu->log_wptr_retention);
...
static struct gmu_vma_node *find_va(struct gmu_vma_entry *vma, u32 addr,
				    u32 size)
{
	struct rb_node *node = vma->vma_root.rb_node;

	while (node != NULL) {
		struct gmu_vma_node *data =
			rb_entry(node, struct gmu_vma_node, node);
...
static int _map_gmu_dynamic(struct gen7_gmu_device *gmu,
			    struct kgsl_memdesc *md, u32 addr, u32 vma_id,
			    int attrs, u32 align)
{
	int ret;
	...
	if (!addr) {
		/*
     * We will end up with a hole (GMU VA range not backed by physical mapping)
     * if the aligned size is greater than the size of the physical mapping
     */
		addr = find_unmapped_va(vma, size,
					hfi_get_gmu_va_alignment(align));
		if (addr == 0) {
	...
	spin_unlock(&vma->lock);
	if (ret < 0) {
		dev_err(&gmu->pdev->dev, "Could not insert va: %x size %x\n",
			addr, size);
		return ret;
	}
...
static int _map_gmu_static(struct gen7_gmu_device *gmu, struct kgsl_memdesc *md,
...
static int _map_gmu(struct gen7_gmu_device *gmu, struct kgsl_memdesc *md,
		    u32 addr, u32 vma_id, int attrs, u32 align)
{
...
struct kgsl_memdesc *
gen7_reserve_gmu_kernel_block_fixed(struct gen7_gmu_device *gmu, u32 addr,
				    u32 size, u32 vma_id, const char *resource,
				    int attrs, u32 align)
{
	int ret;
	struct kgsl_memdesc *md;
...
int gen7_alloc_gmu_kernel_block(struct gen7_gmu_device *gmu,
				struct kgsl_memdesc *md, u32 size, u32 vma_id,
				int attrs)
{
	int ret;
	struct kgsl_device *device = KGSL_DEVICE(gen7_gmu_to_adreno(gmu));
...
void gen7_free_gmu_block(struct gen7_gmu_device *gmu, struct kgsl_memdesc *md)
{
	...
	/*
   * Do not remove the vma node if we failed to unmap the entire buffer. This is
   * because the iommu driver considers remapping an already mapped iova as
   * fatal.
   */
...
int gen7_gmu_memory_init(struct adreno_device *adreno_dev)
{
	...
	if (IS_ERR_OR_NULL(gmu->gmu_log))
		gmu->gmu_log = gen7_reserve_gmu_kernel_block(
			gmu, 0, GMU_LOG_SIZE, GMU_NONCACHED_KERNEL, 0);
...
static void gen7_gmu_pwrctrl_suspend(struct adreno_device *adreno_dev)
{
	...
	if (gen7_gmu_gx_is_on(adreno_dev))
		_do_gbif_halt(device, GEN7_RBBM_GBIF_HALT,
			      GEN7_RBBM_GBIF_HALT_ACK, GEN7_GBIF_GX_HALT_MASK,
			      "GX");
...
static int gen7_gmu_notify_slumber(struct adreno_device *adreno_dev)
{
	...
	int perf_idx =
		gmu->hfi.dcvs_table.gpu_level_num - pwr->default_pwrlevel - 1;
...
static int gen7_gmu_dcvs_set(struct adreno_device *adreno_dev, int gpu_pwrlevel,
			     int bus_level, u32 ab)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
	struct kgsl_pwrctrl *pwr = &device->pwrctrl;
	struct gen7_gmu_device *gmu = to_gen7_gmu(adreno_dev);
...
static int gen7_gmu_ifpc_store(struct kgsl_device *device, unsigned int val)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct gen7_gmu_device *gmu = to_gen7_gmu(adreno_dev);
...
static void gen7_gmu_cooperative_reset(struct kgsl_device *device)
{
   ...
   */
	...
	gmu_core_regread(device, GEN7_GMU_CM3_FW_INIT_RESULT, &result);
	dev_err(&gmu->pdev->dev, "GMU cooperative reset timed out 0x%x\n",
		result);
	/*
...
void gen7_gmu_aop_send_acd_state(struct gen7_gmu_device *gmu, bool flag)
{
	...
	if (ret < 0)
		dev_err(&gmu->pdev->dev, "AOP mbox send message failed: %d\n",
			ret);
}
...
static int gen7_gmu_first_boot(struct adreno_device *adreno_dev)
{
	...
	if (gen7_hfi_send_get_value(adreno_dev, HFI_VALUE_GMU_AB_VOTE, 0) ==
	    1) {
...
static int gen7_bcl_sid_set(struct kgsl_device *device, u32 sid_id, u64 sid_val)
{
	...
	case 0:
		...
		bcl_data = adreno_dev->bcl_data |
			   FIELD_PREP(BCL_SID0_MASK, val);
		break;
	case 1:
		adreno_dev->bcl_data &= ~BCL_SID1_MASK;
		bcl_data = adreno_dev->bcl_data |
			   FIELD_PREP(BCL_SID1_MASK, val);
		break;
	case 2:
		adreno_dev->bcl_data &= ~BCL_SID2_MASK;
		bcl_data = adreno_dev->bcl_data |
			   FIELD_PREP(BCL_SID2_MASK, val);
		break;
	default:
		return -EINVAL;
	}

	return adreno_power_cycle_u32(adreno_dev, &adreno_dev->bcl_data,
				      bcl_data);
}
			    ...
			    u32 ab)
{
	...
	ret = gen7_gmu_dcvs_set(adreno_dev, INVALID_DCVS_IDX, buslevel, ab);
	if (ret)
		return ret;
...
u32 gen7_bus_ab_quantize(struct adreno_device *adreno_dev, u32 ab)
{
     ...
     *
     * max ddr bandwidth (kbps) = (Max bw in kbps per channel * number of
     * channel) max ab (Mbps) = max ddr bandwidth (kbps) / 1000
     */
		u32 max_bw =
			pwr->ddr_table[pwr->ddr_table_count - 1] * NUM_CHANNELS;
		u32 max_ab = max_bw / 1000;

		ab = min_t(u32, ab, max_ab);

		/*
     * Power FW supports a 16 bit AB BW level. We can quantize the entire
     * vote-able BW range to a 16 bit space and the quantized value can be used
     * to vote for AB though GMU. Quantization can be performed as below.
     *
   ...
   * and with this return we want to set the upper 16 bits and EN field
   * specifies if the AB vote is valid or not.
   */
	return (FIELD_PREP(GENMASK(31, 16), vote) |
		FIELD_PREP(GENMASK(15, 8), 1));
}
...
static void gen7_free_gmu_globals(struct gen7_gmu_device *gmu)
{
	...
	for (i = 0; i < gmu->global_entries && i < ARRAY_SIZE(gmu->gmu_globals);
	     i++) {
		struct kgsl_memdesc *md = &gmu->gmu_globals[i];
			       ...
			       struct device_node *node)
{
   ...
   */
	...
	if (ret)
		dev_err(&gmu->pdev->dev, "AOP mailbox init failed: %d\n", ret);
}
...
static int gen7_gmu_reg_probe(struct adreno_device *adreno_dev)
{
	...
	ret = kgsl_regmap_add_region(&device->regmap, gmu->pdev, "gmu", NULL,
				     NULL);

	if (ret)
		dev_err(&gmu->pdev->dev, "Unable to map the GMU registers\n");
	/*
   * gmu_ao_blk_dec1 and gmu_ao_blk_dec2 are contiguous and contained within the
   * gmu region mapped above. gmu_ao_blk_dec0 is not within the gmu region and
   * is mapped separately.
   */
	kgsl_regmap_add_region(&device->regmap, gmu->pdev, "gmu_ao_blk_dec0",
			       NULL, NULL);
...
static int gen7_gmu_clk_probe(struct adreno_device *adreno_dev)
{
   ...
   */
	...
	if (of_get_property(gmu->pdev->dev.of_node, "qcom,gmu-freq-table",
			    &tbl_size) == NULL)
	...
	for (i = 0; i < num_freqs; i++) {
		...
		ret = of_property_read_u32_index(gmu->pdev->dev.of_node,
						 "qcom,gmu-freq-table", offset,
						 &gmu->freqs[i]);
		if (ret)
			goto default_gmu_freq;
		ret = of_property_read_u32_index(gmu->pdev->dev.of_node,
						 "qcom,gmu-freq-table",
						 offset + 1, &gmu->vlvls[i]);
		if (ret)
			goto default_gmu_freq;
	}
	return 0;
				...
				struct kgsl_device *device)
{
	...
	res = platform_get_resource_byname(device->pdev, IORESOURCE_MEM,
					   "rdpm_cx");
	if (res)
		gmu->rdpm_cx_virt = devm_ioremap(&device->pdev->dev, res->start,
						 resource_size(res));

	res = platform_get_resource_byname(device->pdev, IORESOURCE_MEM,
					   "rdpm_mx");
	if (res)
		gmu->rdpm_mx_virt = devm_ioremap(&device->pdev->dev, res->start,
						 resource_size(res));
}

static int gmu_cx_gdsc_event(struct notifier_block *nb, unsigned long event,
			     void *data)
{
	struct gen7_gmu_device *gmu =
		container_of(nb, struct gen7_gmu_device, gdsc_nb);
	struct adreno_device *adreno_dev = gen7_gmu_to_adreno(gmu);
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
	u32 val;
				     ...
				     struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev,
			"Failed to register gmu cx gdsc notifier: %d\n", ret);
		return ret;
	}
...
static int gen7_gmu_iommu_fault_handler(struct iommu_domain *domain,
					struct device *dev, unsigned long addr,
					int flags, void *token)
{
	...
	dev_err(dev, "GMU fault addr = %lX, context=kernel (%s %s fault)\n",
		addr, (flags & IOMMU_FAULT_WRITE) ? "write" : "read",
		fault_type);
...
static int gen7_gmu_iommu_init(struct gen7_gmu_device *gmu)
{
   ...
   */
	qcom_iommu_set_fault_model(gmu->domain,
...
int gen7_gmu_probe(struct kgsl_device *device, struct platform_device *pdev)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct gen7_gmu_device *gmu = to_gen7_gmu(adreno_dev);
	...
	(void)kobject_init_and_add(&gmu->log_kobj, &log_kobj_type, &dev->kobj,
				   "log");
	(void)kobject_init_and_add(&gmu->stats_kobj, &stats_kobj_type,
				   &dev->kobj, "stats");

	of_property_read_u32(gmu->pdev->dev.of_node, "qcom,gmu-perf-ddr-bw",
			     &gmu->perf_ddr_bw);

	gmu->irq = kgsl_request_irq(gmu->pdev, "gmu", gen7_gmu_irq_handler,
				    device);
...
int gen7_halt_gbif(struct adreno_device *adreno_dev)
{
	...
	ret = adreno_wait_for_halt_ack(device, GEN7_GBIF_HALT_ACK,
				       GEN7_GBIF_CLIENT_HALT_MASK);

	/* Halt all AXI requests */
	kgsl_regwrite(device, GEN7_GBIF_HALT, GEN7_GBIF_ARB_HALT_MASK);
	ret = adreno_wait_for_halt_ack(device, GEN7_GBIF_HALT_ACK,
				       GEN7_GBIF_ARB_HALT_MASK);
...
{
	struct kgsl_device *device =
		container_of(t, struct kgsl_device, idle_timer);
...
int gen7_gmu_device_probe(struct platform_device *pdev, u32 chipid,
			  const struct adreno_gpu_core *gpucore)
{
	...
	gen7_dev = devm_kzalloc(&pdev->dev, sizeof(*gen7_dev), GFP_KERNEL);
	if (!gen7_dev)
		return -ENOMEM;
...
int gen7_gmu_hfi_probe(struct adreno_device *adreno_dev)
{
	...
	hfi->irq = kgsl_request_irq(gmu->pdev, "hfi", gen7_hfi_irq_handler,
