...
static ssize_t msm_mmrm_debugfs_info_read(struct file *file, char __user *buf,
...
struct dentry *msm_mmrm_debugfs_init(void)
{
	...
	/* basic info */
	if (!debugfs_create_file("info", 0444, dir, &file_val,
				 &msm_mmrm_debugfs_info_fops)) {
	...
	debugfs_create_u8("allow_multiple_register", 0644, dir,
			  &msm_mmrm_allow_multiple_register);
	debugfs_create_u8("enable_throttle_feature", 0644, dir,
