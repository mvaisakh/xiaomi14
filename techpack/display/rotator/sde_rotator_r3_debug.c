				  ...
				  struct dentry *debugfs_root)
{
	...
	if (!debugfs_create_bool("dbgmem", 0644, debugfs_root,
				 &hw_data->dbgmem)) {
		SDEROT_ERR("fail create dbgmem\n");
		return -EINVAL;
	}

	debugfs_create_u32("koff_timeout", 0644, debugfs_root,
			   &hw_data->koff_timeout);

	debugfs_create_u32("vid_trigger", 0644, debugfs_root,
			   &hw_data->vid_trigger);

	debugfs_create_u32("cmd_trigger", 0644, debugfs_root,
			   &hw_data->cmd_trigger);

	debugfs_create_u32("sbuf_headroom", 0644, debugfs_root,
			   &hw_data->sbuf_headroom);

	debugfs_create_u32("solid_fill", 0644, debugfs_root,
			   &hw_data->solid_fill);
