...
void sde_reg_dump(void __iomem *base, u32 length, const char *prefix, u32 debug)
{
	if (debug)
		...
		for (i = 0; i < num_vreg; i++) {
			...
			if (type == SDE_REG_LDO) {
				rc = regulator_set_voltage(
					curr_vreg->vreg, curr_vreg->min_voltage,
		...
		if (clk_arry[i].clk) {
			if (clk_arry[i].type != SDE_CLK_AHB) {
				DEV_DBG("%pS->%s: '%s' rate %ld\n",
					__builtin_return_address(0), __func__,
					clk_arry[i].clk_name, clk_arry[i].rate);
...
int sde_rot_enable_clk(struct sde_clk *clk_arry, int num_clk, int enable)
{
	...
	if (enable) {
		for (i = 0; i < num_clk; i++) {
			...
			if (rc) {
				sde_rot_enable_clk(&clk_arry[i], i, false);
				break;
			}
		}
	} else {
