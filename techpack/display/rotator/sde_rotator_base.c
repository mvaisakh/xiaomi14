...
static inline u64 apply_fudge_factor(u64 val, struct sde_mult_factor *factor)
{
...
u32 sde_apply_comp_ratio_factor(u32 quota, struct sde_mdp_format_params *fmt,
				struct sde_mult_factor *factor)
{
	struct sde_rot_data_type *mdata = sde_rot_get_mdata();

	if (!mdata || !test_bit(SDE_QOS_OVERHEAD_FACTOR, mdata->sde_qos_map))
		return quota;

	/* apply compression ratio, only for compressed formats */
	if (sde_mdp_is_ubwc_format(fmt) && validate_comp_ratio(factor))
...
static int sde_mdp_wait_for_xin_halt(u32 xin_id)
{
	...
	rc = readl_poll_timeout(vbif_base + MMSS_VBIF_XIN_HALT_CTRL1, status,
				(status & idle_mask), 1000,
				XIN_HALT_TIMEOUT_US);
	if (rc == -ETIMEDOUT) {
...
u32 sde_mdp_get_ot_limit(u32 width, u32 height, u32 pixfmt, u32 fps, u32 is_rd)
{
   ...
   */
	if (IS_SDE_MAJOR_MINOR_SAME(mdata->mdss_version, SDE_MDP_HW_REV_540)) {
		if (is_yuv) {
			if (res <= (RES_1080p * 30))
				ot_lim = 2;
	...
	} else if (IS_SDE_MAJOR_SAME(mdata->mdss_version, SDE_MDP_HW_REV_600) ||
		   is_yuv) {
		if (res <= (RES_1080p * 30))
			ot_lim = 2;
		else if (res <= (RES_1080p * 60))
			...
			struct sde_mdp_set_ot_params *params)
{
	...
	ot_lim = sde_mdp_get_ot_limit(
		params->width, params->height, params->fmt, params->fps,
...
void sde_mdp_set_ot_limit(struct sde_mdp_set_ot_params *params)
{
	...
	ot_lim = get_ot_limit(reg_off_vbif_lim_conf, bit_off_vbif_lim_conf,
			      params) &
		 0xFF;
				    ...
				    char *prop_name, u32 *offsets, int len)
{
	...
	rc = of_property_read_u32_array(pdev->dev.of_node, prop_name, offsets,
					len);
	if (rc) {
				       ...
				       struct sde_rot_data_type *mdata)
{
	...
	mdata->vbif_memtype_count =
		sde_mdp_parse_dt_prop_len(pdev, "qcom,mdss-rot-vbif-memtype");
	mdata->vbif_memtype =
		kcalloc(mdata->vbif_memtype_count, sizeof(u32), GFP_KERNEL);
	if (!mdata->vbif_memtype || !mdata->vbif_memtype_count) {
		mdata->vbif_memtype_count = 0;
		return;
	}

	rc = sde_mdp_parse_dt_handler(pdev, "qcom,mdss-rot-vbif-memtype",
				      mdata->vbif_memtype,
				      mdata->vbif_memtype_count);
	if (rc) {
				   ...
				   struct sde_rot_data_type *mdata)
{
	...
	mdata->npriority_lvl = sde_mdp_parse_dt_prop_len(
		pdev, "qcom,mdss-rot-vbif-qos-setting");
	mdata->vbif_nrt_qos =
		kcalloc(mdata->npriority_lvl, sizeof(u32), GFP_KERNEL);
	if (!mdata->vbif_nrt_qos || !mdata->npriority_lvl) {
		mdata->npriority_lvl = 0;
		return;
	}

	rc = sde_mdp_parse_dt_handler(pdev, "qcom,mdss-rot-vbif-qos-setting",
				      mdata->vbif_nrt_qos,
				      mdata->npriority_lvl);
	if (rc) {
				      ...
				      struct sde_rot_data_type *mdata)
{
	...
	len = sde_mdp_parse_dt_prop_len(pdev, "qcom,mdss-rot-cdp-setting");
	if (len == SDE_ROT_OP_MAX) {
		rc = sde_mdp_parse_dt_handler(pdev, "qcom,mdss-rot-cdp-setting",
					      data, len);
		if (rc) {
					  ...
					  struct sde_rot_data_type *mdata)
{
	...
	if (len == 4) {
		rc = sde_mdp_parse_dt_handler(pdev, "qcom,mdss-rot-qos-lut",
					      data, len);
		if (!rc) {
	...
	if (len == SDE_ROT_OP_MAX) {
		rc = sde_mdp_parse_dt_handler(pdev, "qcom,mdss-rot-danger-lut",
					      data, len);
		if (!rc) {
	...
	if (len == SDE_ROT_OP_MAX) {
		rc = sde_mdp_parse_dt_handler(pdev, "qcom,mdss-rot-safe-lut",
					      data, len);
		if (!rc) {
...
static void
sde_mdp_parse_inline_rot_lut_setting(struct platform_device *pdev,
				     struct sde_rot_data_type *mdata)
{
	int rc;
	u32 len, data[4];
	...
	if (len == 4) {
		rc = sde_mdp_parse_dt_handler(
			pdev, "qcom,mdss-inline-rot-qos-lut", data, len);
		if (!rc) {
	...
	if (len == SDE_ROT_OP_MAX) {
		rc = sde_mdp_parse_dt_handler(
			pdev, "qcom,mdss-inline-rot-danger-lut", data, len);
		if (!rc) {
	...
	if (len == SDE_ROT_OP_MAX) {
		rc = sde_mdp_parse_dt_handler(
			pdev, "qcom,mdss-inline-rot-safe-lut", data, len);
		if (!rc) {
...
static void sde_mdp_parse_rt_rotator(struct device_node *np)
{
	...
	rc = of_parse_phandle_with_args(np, "qcom,mdss-rot-parent",
					"#list-cells", 0, &phargs);
...
int sde_rotator_base_init(struct sde_rot_data_type **pmdata,
			  struct platform_device *pdev, const void *drvdata)
{
	int rc;
	struct sde_rot_data_type *mdata;
	...
	SDEROT_DBG("SDE ROT HW Base addr=0x%x len=0x%x\n",
		   (int)(unsigned long)mdata->sde_io.base, mdata->sde_io.len);
