...
static inline bool dsi_pll_5nm_is_hw_revision(struct dsi_pll_resource *rsc)
{
	return (rsc->pll_revision == DSI_PLL_5NM) ? true : false;
}
				       ...
				       struct dsi_pll_resource *rsc)
{
	...
	default:
		if (vco_rate < 3100000000ULL)
			DSI_PLL_REG_W(pll_base, PLL_ANALOG_CONTROLS_FIVE_1,
				      0x01);
		else
			DSI_PLL_REG_W(pll_base, PLL_ANALOG_CONTROLS_FIVE_1,
				      0x03);
...
static int dsi_pll_calc_dphy_pclk_div(struct dsi_pll_resource *pll)
{
	...
	DSI_PLL_DBG(pll,
		    "bpp: %d, lanes: %d, m_val: %u, n_val: %u, pclk_div: %u\n",
...
static int dsi_pll_calc_cphy_pclk_div(struct dsi_pll_resource *pll)
{
	...
	DSI_PLL_DBG(
		pll,
		"bpp: %d, lanes: %d, m_val: %u, n_val: %u, phy_post_div: %u "
		"pclk_div: %u\n",
