...
static int _rsc_hw_seq_memory_init_v3(struct sde_rsc_priv *rsc)
{
	...
	/* Mode - 0 sequence */
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x0, 0xff399ebe,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x4, 0x20209ebe,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x8, 0x20202020,
		  rsc->debug_mode);

	/* Mode - 1 sequence */
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0xc, 0xe0389ebe,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x10, 0x9ebeff39,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x14, 0x20202020,
		  rsc->debug_mode);

	/* Mode - 2 sequence */
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x18, 0xf9b9baa0,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x1c, 0x999afebd,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x20, 0x81e1a138,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x24, 0xe2a2e0ac,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x28, 0xfd9d3982,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x2c, 0x2020208c,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x30, 0x20202020,
		  rsc->debug_mode);

	/* tcs sleep & wake sequence */
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x34, 0x01a6fcbc,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x38, 0x20209ce6,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x3c, 0x01a7fcbc,
		  rsc->debug_mode);
	dss_reg_w(&rsc->drv_io, seq_mem_offset + 0x40, 0x00209ce7,
		  rsc->debug_mode);
...
static int _rsc_hw_solver_init(struct sde_rsc_priv *rsc)
{
	...
	dss_reg_w(&rsc->drv_io, SDE_RSC_SOLVER_MODE_PARM1_DRV0_MODE0, param_1,
		  rsc->debug_mode);
	...
	dss_reg_w(&rsc->drv_io, SDE_RSC_SOLVER_MODE_PARM1_DRV0_MODE1, param_1,
		  rsc->debug_mode);
	...
	dss_reg_w(&rsc->drv_io, SDE_RSC_SOLVER_MODE_PARM1_DRV0_MODE2, param_1,
		  rsc->debug_mode);
...
static int sde_rsc_mode2_entry_trigger(struct sde_rsc_priv *rsc)
{
	...
	ctrl2_status = dss_reg_r(&rsc->wrapper_io,
				 SDE_RSCC_WRAPPER_OVERRIDE_CTRL2,
				 rsc->debug_mode);
	ctrl2_status &= ~BIT(3);
				   ...
				   enum sde_rsc_state state)
{
	...
	case SDE_RSC_CMD_STATE:
		...
		dss_reg_w(&rsc->wrapper_io, SDE_RSCC_WRAPPER_OVERRIDE_CTRL, 0x0,
			  rsc->debug_mode);
...
int rsc_hw_init_v3(struct sde_rsc_priv *rsc)
{
	...
	rsc->hw_drv_ver =
		dss_reg_r(&rsc->drv_io, SDE_RSCC_RSC_ID_DRV0, rsc->debug_mode);
...
static int rsc_hw_profiling_counter_ctrl(struct sde_rsc_priv *rsc, bool enable)
{
	...
	for (i = 0; i < NUM_RSC_PROFILING_COUNTERS; ++i) {
		dss_reg_w(&rsc->drv_io,
			  SDE_RSCC_LPM_PROFILING_COUNTER0_EN_DRV0 + (0x20 * i),
			  enable ? 1 : 0, rsc->debug_mode);
		dss_reg_w(&rsc->drv_io,
			  SDE_RSCC_LPM_PROFILING_COUNTER0_CLR_DRV0 + (0x20 * i),
			  1, rsc->debug_mode);
	}
