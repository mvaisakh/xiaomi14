...
int mi_sde_connector_debugfs_esd_sw_trigger(void *display)
{
	...
	c_conn->panel_dead = true;
	DISP_ERROR(
		"[esd-test]esd irq check failed report PANEL_DEAD conn_id: %d "
		"enc_id: %d\n",
...
int mi_sde_connector_check_layer_flags(struct drm_connector *connector)
{
	...
	if (connector->connector_type == DRM_MODE_CONNECTOR_DSI) {
		c_conn = to_sde_connector(connector);
		value = sde_connector_get_property(
			connector->state, CONNECTOR_PROP_MI_LAYER_INFO);
		...
		if (flags.gxzw_anim_changed) {
			...
			if (display &&
			    mi_disp_lhbm_fod_enabled(display->panel)) {
				ret = mi_disp_lhbm_aod_to_normal_optimize(
					display, flags.gxzw_anim_present);
				if (ret == -EAGAIN) {
					/*Doze brightness queue work schedule delay
           * And trigger once in next frame*/
					display->panel->mi_cfg
						.aod_to_normal_pending = true;
				} else {
					display->panel->mi_cfg
						.aod_to_normal_pending = false;
				}
				display->panel->mi_cfg.lhbm_gxzw =
					flags.gxzw_anim_present;
				mi_disp_update_0size_lhbm_layer(
					display, flags.gxzw_anim_present);
			}
		}
	...
	if (mi_cfg->flat_sync_te) {
		...
		if (flat_mode_val != mi_cfg->flat_cfg.cur_flat_state) {
			if (flat_mode_val == FEATURE_ON) {
				if (mi_get_panel_id_by_dsi_panel(
					    dsi_display->panel) ==
					    N3_PANEL_PA &&
				    dsi_display->panel->id_config.build_id >=
					    N3_PANEL_PA_P11)
