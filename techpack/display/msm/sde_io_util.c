...
void dss_reg_w(struct dss_io_data *io, u32 offset, u32 value, u32 debug)
{
	...
	if (debug) {
		...
		DEV_DBG("[%08x] => %08x [%08x]\n",
			(u32)(unsigned long)(io->base + offset), value, in_val);
	}
...
void dss_reg_dump(void __iomem *base, u32 length, const char *prefix, u32 debug)
{
	if (debug)
...
int msm_dss_get_io_mem(struct platform_device *pdev, struct list_head *mem_list)
{
	...
	num_entry = of_property_count_strings(pdev->dev.of_node, "reg-names");
	if (num_entry < 0)
		num_entry = 0;
   ...
   */
	num_exclude_entry = of_property_count_strings(
		pdev->dev.of_node, "qcom,sde-vm-exclude-reg-names");
	...
	for (i = 0; i < num_entry; i++) {
		...
		of_property_read_string_index(pdev->dev.of_node, "reg-names", i,
					      &reg_name);
...
int msm_dss_get_vreg(struct device *dev, struct dss_vreg *in_vreg, int num_vreg,
		     int enable)
{
	int i = 0, rc = 0;
	...
	if (enable) {
		for (i = 0; i < num_vreg; i++) {
			curr_vreg = &in_vreg[i];
			curr_vreg->vreg =
				regulator_get(dev, curr_vreg->vreg_name);
...
int msm_dss_mmrm_register(struct device *dev, struct dss_module_power *mp,
			  int (*cb_fnc)(struct mmrm_client_notifier_data *data),
			  void *phandle, bool *mmrm_enable)
{
	int i, rc = 0;
	...
	*mmrm_enable = false;
	...
	for (i = 0; i < num_clk; i++) {
		...
		desc.client_info.desc.client_id = clk_array[i].mmrm.clk_id;
		...
		if (!clk_array[i].mmrm.mmrm_client) {
			...
			*mmrm_enable = true;
			DEV_DBG("mmrm register id:%d name=%s prio:%d\n",
				desc.client_info.desc.client_id,
				desc.client_info.desc.name, desc.priority);
		}
	}
...
void msm_dss_mmrm_deregister(struct device *dev, struct dss_module_power *mp)
{
	...
	for (i = 0; i < num_clk; i++) {
		...
		ret = mmrm_client_deregister(clk_array[i].mmrm.mmrm_client);
		if (ret) {
			DEV_DBG("fail mmrm deregister ret:%d clk:%s\n", ret,
				clk_array[i].clk_name);
			continue;
		}
...
int msm_dss_single_clk_set_rate(struct dss_clk *clk)
{
   ...
   */
	...
	if (clk->type != DSS_CLK_AHB && clk->type != DSS_CLK_MMRM &&
	    !clk->mmrm.flags) {
		...
		rc = mmrm_client_set_value(clk->mmrm.mmrm_client, &client_data,
					   clk->rate);
		if (rc) {
