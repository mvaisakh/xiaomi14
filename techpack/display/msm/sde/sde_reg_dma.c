...
static void set_default_dma_ops(struct sde_hw_reg_dma *reg_dma)
{
	const static struct sde_hw_reg_dma_ops ops = {
		...
		default_kick_off,	   default_reset,
		default_alloc_reg_dma_buf, default_dealloc_reg_dma,
		default_buf_reset_reg_dma, default_last_command,
		default_last_command_sb,   default_dump_reg
	};
...
static int sde_reg_dma_reset(void *ctl_data, void *priv_data)
{
	...
	if (ops && ops->reset) {
		SDE_EVT32(sde_hw_ctl ? sde_hw_ctl->idx : 0xff,
		     ...
		     struct drm_device *dev)
{
	...
	client_entry_handle = msm_register_fence_error_event(
		dev, &sde_reg_dma_event_ops, NULL);
