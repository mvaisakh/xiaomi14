...
#define SDE_DEBUG_CONN(c, fmt, ...) \
	SDE_DEBUG("conn%d " fmt, (c) ? (c)->base.base.id : -1, ##__VA_ARGS__)

#define SDE_ERROR_CONN(c, fmt, ...) \
	SDE_ERROR("conn%d " fmt, (c) ? (c)->base.base.id : -1, ##__VA_ARGS__)
...
static void sde_dimming_bl_notify(struct sde_connector *conn,
				  struct dsi_backlight_config *config)
{
	...
	if (!conn->dimming_bl_notify_enabled ||
	    config->user_disable_notification)
		return;
...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if (c_conn->ops.set_backlight) {
		...
		mi_disp_feature_event_notify_by_type(
			...
			MI_DISP_EVENT_BACKLIGHT, sizeof(brightness),
			brightness);
		...
		if (!rc) {
			sde_dimming_bl_notify(c_conn,
					      &display->panel->bl_config);
			if (c_conn->base.state && c_conn->base.state->crtc) {
				sde_crtc_backlight_notify(
					c_conn->base.state->crtc, brightness,
					display->panel->bl_config
			       ...
			       struct drm_device *dev)
{
	...
	if (bl_config->type != DSI_BACKLIGHT_DCS && sde_in_trusted_vm(sde_kms))
		return 0;
	...
	snprintf(bl_node_name, BL_NODE_NAME_SIZE, "panel%u-backlight",
		 display_count);
	c_conn->bl_device =
		backlight_device_register(bl_node_name, dev->dev, c_conn,
...
int sde_connector_trigger_event(void *drm_connector, uint32_t event_idx,
				uint32_t instance_idx, uint32_t data0,
				uint32_t data1, uint32_t data2, uint32_t data3)
{
	struct sde_connector *c_conn;
	unsigned long irq_flags;
	int (*cb_func)(uint32_t event_idx, uint32_t instance_idx, void *usr,
   ...
   */
	...
	if (cb_func)
		rc = cb_func(event_idx, instance_idx, usr, data0, data1, data2,
			     data3);
	else
		rc = -EAGAIN;
...
int sde_connector_register_event(
	struct drm_connector *connector, uint32_t event_idx,
	int (*cb_func)(uint32_t event_idx, uint32_t instance_idx, void *usr,
		       uint32_t data0, uint32_t data1, uint32_t data2,
		       uint32_t data3),
	void *usr)
						   ...
						   struct sde_connector *c_conn)
{
	...
	snprintf(prop_name, ARRAY_SIZE(prop_name), "%s%d", "SDE_PP_DITHER_V",
		 version);
	switch (version) {
	case 1:
	case 2:
...
static void
sde_connector_get_avail_res_info(struct drm_connector *conn,
...
int sde_connector_get_lm_cnt_from_topology(
	struct drm_connector *conn, const struct drm_display_mode *drm_mode)
{
	struct sde_connector *c_conn;
				...
				struct msm_mode_info *mode_info)
{
	...
	return sde_conn->ops.get_mode_info(conn, drm_mode, sub_mode, mode_info,
					 ...
					 uint32_t data2, uint32_t data3)
{
	...
	/* update the panel id */
	msm_property_set_blob(&c_conn->property_info, &c_conn->blob_panel_id,
			      &msb_arr, sizeof(msb_arr),
...
static int
_sde_connector_update_dimming_bl_lut(struct sde_connector *c_conn,
				     struct sde_connector_state *c_state)
{
	bool is_dirty;
	size_t sz = 0;
	...
	bl_config->dimming_bl_lut = msm_property_get_blob(
		&c_conn->property_info, &c_state->property_state, &sz,
		CONNECTOR_PROP_DIMMING_BL_LUT);
...
static int
_sde_connector_update_dimming_ctrl(struct sde_connector *c_conn,
				   struct sde_connector_state *c_state,
				   uint64_t val)
{
	struct dsi_display *dsi_display;
...
static int
_sde_connector_update_dimming_min_bl(struct sde_connector *c_conn,
				     struct sde_connector_state *c_state,
				     uint64_t val)
{
	struct dsi_display *dsi_display;
...
void sde_connector_set_colorspace(struct sde_connector *c_conn)
{
	...
	if (c_conn->ops.set_colorspace)
		rc = c_conn->ops.set_colorspace(&c_conn->base, c_conn->display);
...
void sde_connector_set_qsync_params(struct drm_connector *connector)
{
	...
	if (prop_dirty) {
		qsync_propval = sde_connector_get_property(
			c_conn->base.state, CONNECTOR_PROP_QSYNC_MODE);
	...
	if (prop_dirty) {
		ept_fps = sde_connector_get_property(c_conn->base.state,
						     CONNECTOR_PROP_EPT_FPS);
...
static int
_sde_connector_update_hdr_metadata(struct sde_connector *c_conn,
				   struct sde_connector_state *c_state)
{
	int rc = 0;
...
static int
_sde_connector_update_dirty_properties(struct drm_connector *connector)
{
	struct sde_connector *c_conn;
	struct sde_connector_state *c_state;
	int idx;
...
sde_connector_get_dyn_hdr_meta(struct drm_connector *connector)
{
	struct sde_connector_state *c_state;
...
int sde_connector_prepare_commit(struct drm_connector *connector)
{
	...
	SDE_EVT32(connector->base.id, params.qsync_mode, params.qsync_update,
		  rc);
   ...
   */
	...
	if (!sde_in_trusted_vm(sde_kms) && c_conn->bl_device &&
		...
		if (display->panel->cur_mode->dsi_mode_flags &
		    DSI_MODE_FLAG_DMS) {
			DISP_DEBUG(
				"dynamic mode switch, skip backlight update\n");
		} else {
...
int sde_connector_clk_ctrl(struct drm_connector *connector, bool enable)
{
	...
	if (display && c_conn->ops.clk_ctrl)
		rc = c_conn->ops.clk_ctrl(display->mdp_clk_handle, DSI_ALL_CLKS,
					  state);
...
static void
sde_connector_atomic_destroy_state(struct drm_connector *connector,
...
sde_connector_atomic_duplicate_state(struct drm_connector *connector)
{
	...
	msm_property_duplicate_state(&c_conn->property_info, c_oldstate,
				     c_state, &c_state->property_state,
				     c_state->property_values);
...
int sde_connector_roi_v1_check_roi(struct drm_connector_state *conn_state)
{
	...
	for (i = 0; i < c_state->rois.num_rects; ++i) {
		...
		SDE_EVT32_VERBOSE(DRMID(&c_conn->base), roi_conn->x1,
				  roi_conn->y1, roi_conn->x2, roi_conn->y2);
...
static int _sde_connector_set_roi_v1(struct sde_connector *c_conn,
				     struct sde_connector_state *c_state,
				     void __user *usr_ptr)
{
	...
	for (i = 0; i < roi_v1.num_rects; ++i) {
		...
		SDE_DEBUG_CONN(c_conn, "roi%d: roi (%d,%d) (%d,%d)\n", i,
			       c_state->rois.roi[i].x1, c_state->rois.roi[i].y1,
...
static int _sde_connector_set_ext_hdr_info(struct sde_connector *c_conn,
					   struct sde_connector_state *c_state,
					   void __user *usr_ptr)
{
	int rc = 0;
	...
	if (copy_from_user(&c_state->hdr_meta, (void __user *)usr_ptr,
			   sizeof(*hdr_meta))) {
					  ...
					  uint64_t val)
{
	...
	c_state->out_fb = drm_framebuffer_lookup(connector->dev, NULL, val);
...
static int
_sde_connector_set_prop_retire_fence(struct drm_connector *connector,
				     struct drm_connector_state *state,
				     uint64_t val)
{
	int rc = 0;
   ...
   */
	if (prev_user_fd == -1) {
		...
		offset = sde_connector_get_property(
			state, CONN_PROP_RETIRE_FENCE_OFFSET);
		/*
     ...
     */
		...
		if (c_conn->connector_type == DRM_MODE_CONNECTOR_VIRTUAL) {
			struct drm_encoder *drm_enc =
				sde_connector_best_encoder(connector);
...
static int
_sde_connector_set_prop_dyn_transfer_time(struct sde_connector *c_conn,
					  uint64_t val)
{
	int rc = 0;
	...
	if (rc)
		SDE_ERROR_CONN(
			c_conn,
			"updating transfer time failed, val: %u, rc %d\n", val,
			rc);
					     ...
					     uint64_t val)
{
	...
	case CONNECTOR_PROP_RETIRE_FENCE:
		...
		rc = _sde_connector_set_prop_retire_fence(connector, state,
							  val);
		break;
   ...
   */
	...
	case CONNECTOR_PROP_DIMMING_MIN_BL:
		...
		break;
	case CONNECTOR_PROP_HDR_METADATA:
		rc = _sde_connector_set_ext_hdr_info(c_conn, c_state,
						     (void *)(uintptr_t)val);
		if (rc)
	...
	case CONNECTOR_PROP_SET_PANEL_MODE:
		if (val == DRM_MODE_FLAG_VID_MODE_PANEL)
			c_conn->expected_panel_mode = MSM_DISPLAY_VIDEO_MODE;
		else if (val == DRM_MODE_FLAG_CMD_MODE_PANEL)
			c_conn->expected_panel_mode = MSM_DISPLAY_CMD_MODE;
		break;
	case CONNECTOR_PROP_DYN_BIT_CLK:
		...
		if (rc)
			SDE_ERROR_CONN(c_conn,
				       "dynamic bit clock set failed, rc: %d",
				       rc);
	...
	case CONNECTOR_PROP_LP:
		...
		if (val == SDE_MODE_DPMS_OFF) {
			memset(&c_conn->previous_misr_sign, 0,
			       sizeof(struct sde_misr_sign));
	...
	if (!rc && c_conn->ops.set_property) {
		rc = c_conn->ops.set_property(connector, state, idx, val,
					      c_conn->display);
...
static int
sde_connector_atomic_get_property(struct drm_connector *connector,
	...
	if (idx == CONNECTOR_PROP_RETIRE_FENCE) {
		*val = ~0;
		...
		rc = msm_property_atomic_get(&c_conn->property_info,
					     &c_state->property_state, property,
					     val);
	}

	/* allow for custom override */
	if (c_conn->ops.get_property)
		rc = c_conn->ops.get_property(
			connector, (struct drm_connector_state *)state, idx,
			val, c_conn->display);
	return rc;
}
...
void sde_connector_fence_error_ctx_signal(struct drm_connector *conn,
					  int input_fence_status, bool is_vid)
{
	struct sde_connector *sde_conn;
				   ...
				   enum sde_fence_event fence_event)
{
	...
	sde_fence_signal(to_sde_connector(connector)->retire_fence, ts,
			 fence_event, NULL);
}
...
void sde_connector_commit_reset(struct drm_connector *connector, ktime_t ts)
{
	...
	/* signal connector's retire fence */
	sde_fence_signal(c_conn->retire_fence, ts, SDE_FENCE_RESET_TIMELINE,
			 hw_ctl);
}
...
static void sde_connector_update_colorspace(struct drm_connector *connector)
{
	...
	ret = msm_property_set_property(
		...
		CONNECTOR_PROP_SUPPORTED_COLORSPACES, c_conn->color_enc_fmt);

	if (ret)
		SDE_ERROR("failed to set colorspace property for connector\n");
}

static int sde_connector_detect_ctx(struct drm_connector *connector,
				    ...
				    bool force)
{
	...
	if (c_conn->ops.detect_ctx)
		status = c_conn->ops.detect_ctx(connector, ctx, force,
						c_conn->display);
					  ...
					  uint32_t property_idx, uint64_t value)
{
	...
	c_conn = to_sde_connector(connector);
	property = msm_property_index_to_drm_property(&c_conn->property_info,
						      property_idx);
	if (!property) {
	...
	return sde_connector_atomic_set_property(connector, state, property,
						 value);
}
	...
	struct drm_connector_state *connector_state)
{
	...
	for (prop_idx = 0; prop_idx < CONNECTOR_PROP_COUNT; prop_idx++) {
		...
		ret = sde_connector_atomic_set_property(
			connector, connector_state, drm_prop, def);
		if (ret) {
			SDE_ERROR_CONN(c_conn,
				...
				struct drm_display_mode *mode)
{
	...
	vfp = c_conn->ops.get_panel_vfp(c_conn->display, mode->hdisplay,
					mode->vdisplay);
	if (vfp <= 0)
						 ...
						 loff_t *ppos)
{
	...
	mutex_lock(&c_conn->lock);
	blen = snprintf(buffer, MAX_CMD_PAYLOAD_SIZE, "last_cmd_tx_sts:0x%x",
...
static int sde_connector_init_debugfs(struct drm_connector *connector)
{
	...
	if (sde_connector->ops.cmd_transfer) {
		if (!debugfs_create_file("tx_cmd", 0600,
					 connector->debugfs_entry, connector,
					 &conn_cmd_tx_fops)) {
	...
	if (sde_connector->connector_type == DRM_MODE_CONNECTOR_VIRTUAL &&
	    sde_kms->catalog->hw_fence_rev)
		debugfs_create_bool(
			"wb_hw_fence_enable", 0600, connector->debugfs_entry,
				    ...
				    uint32_t max_width, uint32_t max_height)
{
	...
	mode_count = drm_helper_probe_single_connector_modes(
	...
	rc = sde_connector_set_blob_data(connector, connector->state,
...
static int sde_connector_get_modes(struct drm_connector *connector)
{
	...
	mode_count =
		c_conn->ops.get_modes(connector, c_conn->display, &avail_res);
	if (!mode_count) {
				  ...
				  struct drm_atomic_state *state)
{
	...
	if (c_conn->ops.atomic_best_encoder)
		encoder = c_conn->ops.atomic_best_encoder(
			connector, c_conn->display, connector_state);
				      ...
				      struct drm_atomic_state *state)
{
	...
	if (c_conn->ops.atomic_check)
		return c_conn->ops.atomic_check(connector, c_conn->display,
						state);
				       ...
				       struct msm_display_topology topology)
{
	...
	topology_idx = (int)sde_rm_get_topology_name(&sde_kms->rm, topology);
...
static void sde_connector_check_status_work(struct work_struct *work)
{
	...
	conn = container_of(to_delayed_work(work), struct sde_connector,
			    status_work);
	if (!conn) {
	...
	if (!conn->ops.check_status || dev->power.is_suspended ||
	    ...
	    (conn->lp_mode == SDE_MODE_DPMS_LP2)) {
					    ...
					    struct sde_kms_info *info)
{
	...
	list_for_each_entry(mode, &conn->modes, head) {
		...
		if (c_conn->ops.set_submode_info) {
			c_conn->ops.set_submode_info(conn, info,
						     c_conn->display, mode);
		} else {
			topo_name = sde_conn_get_topology_name(
				conn, mode_info.topology);
			if (topo_name)
				sde_kms_info_add_keystr(info, "topology",
							topo_name);
		}

		sde_kms_info_add_keyint(info, "qsync_min_fps",
					mode_info.qsync_min_fps);
		sde_kms_info_add_keyint(info, "avr_step_fps",
					mode_info.avr_step_fps);
		sde_kms_info_add_keyint(info, "has_cwb_crop",
					test_bit(SDE_FEATURE_CWB_CROP,
						 sde_kms->catalog->features));
		sde_kms_info_add_keyint(info, "has_dedicated_cwb_support",
					test_bit(SDE_FEATURE_DEDICATED_CWB,
						 sde_kms->catalog->features));
		...
		if (mode_info.mdp_transfer_time_us_min &&
		    mode_info.mdp_transfer_time_us_max) {
			sde_kms_info_add_keyint(
				info, "mdp_transfer_time_us_min",
		...
		sde_kms_info_add_keyint(info, "partial_update_num_roi",
					mode_info.roi_caps.num_roi);
		sde_kms_info_add_keyint(
			info, "partial_update_xstart",
			mode_info.roi_caps.align.xstart_pix_align);
		sde_kms_info_add_keyint(
			info, "partial_update_walign",
	...
	case CONNECTOR_PROP_SDE_INFO:
       ...
       */
		...
		if (c_conn->ops.set_info_blob) {
			rc = c_conn->ops.set_info_blob(
				conn, info, c_conn->display, &mode_info);
			if (rc) {
				SDE_ERROR_CONN(c_conn,
...
static void _sde_connector_install_qsync_properties(
	struct sde_kms *sde_kms, struct sde_connector *c_conn,
	struct dsi_display *dsi_display, struct msm_display_info *display_info)
{
	...
	if (test_bit(SDE_FEATURE_QSYNC, sde_kms->catalog->features) &&
		...
		if (test_bit(SDE_FEATURE_AVR_STEP,
			     sde_kms->catalog->features) &&
		    (display_info->capabilities & MSM_DISPLAY_CAP_VID_MODE))
			msm_property_install_enum(
				&c_conn->property_info, "avr_step_state", 0, 0,
				e_avr_step_state, ARRAY_SIZE(e_avr_step_state),
				0, CONNECTOR_PROP_AVR_STEP_STATE);
		...
		if (test_bit(SDE_FEATURE_EPT_FPS, sde_kms->catalog->features) &&
			...
			msm_property_install_range(&c_conn->property_info,
						   "EPT_FPS", 0x0, 0, U32_MAX,
						   0, CONNECTOR_PROP_EPT_FPS);
	}
}

static int _sde_connector_install_properties(
	struct drm_device *dev, struct sde_kms *sde_kms,
	struct sde_connector *c_conn, int connector_type, void *display,
	struct msm_display_info *display_info)
{
	...
	rc = sde_connector_set_blob_data(&c_conn->base, NULL,
					 CONNECTOR_PROP_SDE_INFO);
	if (rc) {
		SDE_ERROR_CONN(c_conn,
	...
	if (connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		if (dsi_display && dsi_display->panel) {
			msm_property_install_blob(
				&c_conn->property_info, "dimming_bl_lut",
				DRM_MODE_PROP_BLOB,
				CONNECTOR_PROP_DIMMING_BL_LUT);
			msm_property_install_range(&c_conn->property_info,
						   "dimming_dyn_ctrl", 0x0, 0,
						   ~0, 0,
						   CONNECTOR_PROP_DIMMING_CTRL);
			msm_property_install_range(
				&c_conn->property_info, "dimming_min_bl", 0x0,
				0,
				dsi_display->panel->bl_config
					.brightness_max_level,
				0, CONNECTOR_PROP_DIMMING_MIN_BL);
		}
		...
		if (dsi_display && dsi_display->panel &&
			...
			msm_property_set_blob(
				&c_conn->property_info, &c_conn->blob_hdr,
		...
		if (dsi_display && dsi_display->panel &&
			...
			msm_property_install_range(&c_conn->property_info,
						   "dyn_bit_clk", 0x0, 0, ~0, 0,
						   CONNECTOR_PROP_DYN_BIT_CLK);

		msm_property_install_range(&c_conn->property_info,
					   "dyn_transfer_time", 0x0, 0, 1000000,
					   0, CONNECTOR_PROP_DYN_TRANSFER_TIME);
	...
	if (connector_type == DRM_MODE_CONNECTOR_DisplayPort) {
		...
		msm_property_set_blob(&c_conn->property_info,
				      &c_conn->blob_ext_hdr, &hdr, sizeof(hdr),
				      CONNECTOR_PROP_EXT_HDR_INFO);
	...
	msm_property_install_volatile_range(&c_conn->property_info,
					    "RETIRE_FENCE_OFFSET", 0x0, 0, ~0,
					    0, CONN_PROP_RETIRE_FENCE_OFFSET);

	msm_property_install_range(&c_conn->property_info, "autorefresh", 0x0,
				   0, AUTOREFRESH_MAX_FRAME_CNT, 0,
				   CONNECTOR_PROP_AUTOREFRESH);

	if (connector_type == DRM_MODE_CONNECTOR_DSI) {
		_sde_connector_install_qsync_properties(
			sde_kms, c_conn, dsi_display, display_info);
		...
		msm_property_install_enum(&c_conn->property_info, "dsc_mode", 0,
					  0, e_dsc_mode, ARRAY_SIZE(e_dsc_mode),
					  0, CONNECTOR_PROP_DSC_MODE);

		if (dsi_display && dsi_display->panel &&
		    display_info->capabilities & MSM_DISPLAY_CAP_CMD_MODE &&
		    display_info->capabilities & MSM_DISPLAY_CAP_VID_MODE)
			msm_property_install_enum(
				&c_conn->property_info, "panel_mode", 0, 0,
				e_panel_mode, ARRAY_SIZE(e_panel_mode),
				(dsi_display->panel->panel_mode ==
				 DSI_OP_VIDEO_MODE) ?
					0 :
					1,
		...
		msm_property_install_enum(&c_conn->property_info, "bpp_mode", 0,
					  0, e_bpp_mode, ARRAY_SIZE(e_bpp_mode),
					  0, CONNECTOR_PROP_BPP_MODE);

		if (test_bit(SDE_FEATURE_DEMURA, sde_kms->catalog->features)) {
			msm_property_install_blob(
				&c_conn->property_info, "DEMURA_PANEL_ID",
				DRM_MODE_PROP_IMMUTABLE,
				CONNECTOR_PROP_DEMURA_PANEL_ID);
			msm_property_set_blob(&c_conn->property_info,
					      &c_conn->blob_panel_id, &panel_id,
					      sizeof(panel_id),
					      CONNECTOR_PROP_DEMURA_PANEL_ID);
		}
	}
	...
	if ((display_info->capabilities & MSM_DISPLAY_CAP_CMD_MODE) ||
		...
		msm_property_install_enum(
			&c_conn->property_info, "frame_trigger_mode", 0, 0,
			e_frame_trigger_mode, ARRAY_SIZE(e_frame_trigger_mode),
			0, CONNECTOR_PROP_CMD_FRAME_TRIGGER_MODE);

	msm_property_install_range(&c_conn->property_info, "bl_scale", 0x0, 0,
				   MAX_BL_SCALE_LEVEL, MAX_BL_SCALE_LEVEL,
				   CONNECTOR_PROP_BL_SCALE);

	msm_property_install_range(&c_conn->property_info, "sv_bl_scale", 0x0,
				   0, U32_MAX, MAX_SV_BL_SCALE_LEVEL,
	...
	if (connector_type == DRM_MODE_CONNECTOR_DisplayPort)
		msm_property_install_range(
			&c_conn->property_info, "supported_colorspaces",
	...
	msm_property_install_enum(&c_conn->property_info, "topology_control", 0,
				  1, e_topology_control,
				  ARRAY_SIZE(e_topology_control), 0,
				  CONNECTOR_PROP_TOPOLOGY_CONTROL);
	msm_property_install_enum(&c_conn->property_info, "LP", 0, 0,
				  e_power_mode, ARRAY_SIZE(e_power_mode), 0,
				  CONNECTOR_PROP_LP);

#ifdef MI_DISPLAY_MODIFY
	msm_property_install_range(&c_conn->property_info, "mi_layer_info", 0x0,
				   0, U32_MAX, 0, CONNECTOR_PROP_MI_LAYER_INFO);
#endif
	return 0;
}
...
struct drm_connector *sde_connector_init(struct drm_device *dev,
					 ...
					 struct drm_panel *panel, void *display,
					 ...
					 int connector_poll, int connector_type)
{
	...
	rc = drm_connector_init(dev, &c_conn->base, &sde_connector_ops,
				connector_type);
	if (rc)
	...
	snprintf(c_conn->name, SDE_CONNECTOR_NAME_SIZE, "conn%u",
		 c_conn->base.base.id);

	c_conn->retire_fence =
		sde_fence_init(c_conn->name, c_conn->base.base.id);
	...
	rc = _sde_connector_install_properties(
		dev, sde_kms, c_conn, connector_type, display, &display_info);
	if (rc)
		goto error_cleanup_fence;
...
#ifdef MI_DISPLAY_MODIFY
	dsi_display = (struct dsi_display *)(display);
	if (connector_type == DRM_MODE_CONNECTOR_DSI && dsi_display &&
	    dsi_display->panel &&
