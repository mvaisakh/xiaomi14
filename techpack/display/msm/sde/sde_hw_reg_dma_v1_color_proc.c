...
#define GAMUT_LUT_MEM_SIZE \
	((sizeof(struct drm_msm_3d_gamut)) + REG_DMA_HEADERS_BUFFER_SZ)
...
#define GC_LUT_MEM_SIZE \
	((sizeof(struct drm_msm_pgc_lut)) + REG_DMA_HEADERS_BUFFER_SZ)

#define IGC_LUT_MEM_SIZE \
	((sizeof(struct drm_msm_igc_lut)) + REG_DMA_HEADERS_BUFFER_SZ)

#define PCC_LUT_ENTRIES (PCC_NUM_PLANES * PCC_NUM_COEFF)
#define PCC_LEN (PCC_LUT_ENTRIES * sizeof(u32))
#define PCC_MEM_SIZE (PCC_LEN + REG_DMA_HEADERS_BUFFER_SZ)

#define HSIC_MEM_SIZE \
	((sizeof(struct drm_msm_pa_hsic)) + REG_DMA_HEADERS_BUFFER_SZ)
#define SIXZONE_MEM_SIZE \
	((sizeof(struct drm_msm_sixzone)) + REG_DMA_HEADERS_BUFFER_SZ)
#define MEMCOLOR_MEM_SIZE \
	((sizeof(struct drm_msm_memcol)) + REG_DMA_HEADERS_BUFFER_SZ)

#define RC_MASK_CFG_SIZE \
	(sizeof(struct drm_msm_rc_mask_cfg) + REG_DMA_HEADERS_BUFFER_SZ)
// TBD: exact size calculations
#define RC_PU_CFG_SIZE ((64 * sizeof(u32)) + REG_DMA_HEADERS_BUFFER_SZ)

#define QSEED3_MEM_SIZE                                            \
	(sizeof(struct sde_hw_scaler3_cfg) + (450 * sizeof(u32)) + \
	 REG_DMA_HEADERS_BUFFER_SZ)
#define LTM_INIT_MEM_SIZE \
	((sizeof(struct drm_msm_ltm_init_param)) + REG_DMA_HEADERS_BUFFER_SZ)
#define LTM_ROI_MEM_SIZE \
	((sizeof(struct drm_msm_ltm_cfg_param)) + REG_DMA_HEADERS_BUFFER_SZ)
#define LTM_VLUT_MEM_SIZE \
	((sizeof(struct drm_msm_ltm_data)) + REG_DMA_HEADERS_BUFFER_SZ)
#define SPR_INIT_MEM_SIZE \
	((sizeof(struct drm_msm_spr_init_cfg)) + REG_DMA_HEADERS_BUFFER_SZ)
#define SPR_UDC_MEM_SIZE \
	((sizeof(struct drm_msm_spr_udc_cfg)) + REG_DMA_HEADERS_BUFFER_SZ)
#define DEMURA_MEM_SIZE \
	((sizeof(struct drm_msm_dem_cfg)) + REG_DMA_HEADERS_BUFFER_SZ)
#define DEMURA_CFG0_PARAM2_MEM_SIZE \
	((sizeof(struct drm_msm_dem_cfg0_param2)) + REG_DMA_HEADERS_BUFFER_SZ)
...
static u32 sspp_feature_map[SDE_SSPP_MAX] = {
	[SDE_SSPP_VIG_IGC] = IGC,	  [SDE_SSPP_VIG_GAMUT] = GAMUT,
	[SDE_SSPP_DMA_IGC] = IGC,	  [SDE_SSPP_DMA_GC] = GC,
	[SDE_SSPP_SCALER_QSEED3] = QSEED, [SDE_SSPP_SCALER_QSEED3LITE] = QSEED,
};
...
static u32 sspp_mapping[SSPP_MAX] = {
	[SSPP_VIG0] = VIG0, [SSPP_VIG1] = VIG1, [SSPP_VIG2] = VIG2,
	[SSPP_VIG3] = VIG3, [SSPP_DMA0] = DMA0, [SSPP_DMA1] = DMA1,
	[SSPP_DMA2] = DMA2, [SSPP_DMA3] = DMA3, [SSPP_DMA4] = DMA4,
	[SSPP_DMA5] = DMA5,
};
...
#define REG_DMA_SETUP_OPS(cfg, block_off, data_ptr, data_len, op, wrap_sz, \
			  wrap_inc, reg_mask)                              \
	do {                                                               \
		(cfg).ops = op;                                            \
...
#define REG_DMA_SETUP_KICKOFF(cfg, hw_ctl, feature_dma_buf, ops, ctl_q, mode, \
			      reg_dma_feature)                                \
	do {                                                                  \
		memset(&cfg, 0, sizeof(cfg));                                 \
		(cfg).ctl = hw_ctl;                                           \
...
static void _perform_sbdma_kickoff(struct sde_hw_dspp *ctx,
				   ...
				   struct sde_hw_reg_dma_ops *dma_ops, u32 blk,
				   enum sde_reg_dma_features feature);

static inline int _reg_dmav1_rc_write(struct sde_hw_dspp *ctx, u32 reg_offset,
				      u32 val,
				      ...
				      enum sde_reg_dma_features feature)
{
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, abs_offset, &val, sizeof(__u32),
...
static int _reg_dmav1_rc_program_enable_bits(
	...
	struct sde_hw_reg_dma_ops *dma_ops, enum sde_reg_dma_features feature)
{
	int rc = 0;
	...
	SDE_DEBUG(
		"idx:%d w:%d h:%d flags:%x, R1:%d, R2:%d, PU R1:%d, PU R2:%d, "
		"Y_START:%d\n",
		RC_IDX(hw_dspp), mask_w, mask_h, flags, r1_valid, r2_valid,
		pu_in_r1, pu_in_r2, ystart);
	SDE_EVT32(RC_IDX(hw_dspp), mask_w, mask_h, flags, r1_valid, r2_valid,
		  pu_in_r1, pu_in_r2, ystart);

	val |= param_c;
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG1, val, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG13, ystart, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG9, rc_merge_mode,
				 dma_ops, feature);
				     ...
				     enum sde_reg_dma_features feature)
{
	...
	rc = _reg_dmav1_rc_program_enable_bits(hw_dspp, rc_mask_cfg, param_a,
					       param_b, param_r, merge_mode,
					       rc_roi, dma_ops, feature);
	if (rc) {
	...
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG2, val2, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG3, val3, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG4, val4, dma_ops,
				 feature);
	...
	struct sde_hw_reg_dma_ops *dma_ops, enum sde_reg_dma_features feature)
{
	...
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG5, val5, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG6, val6, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG7, val7, dma_ops,
				 feature);
	rc = _reg_dmav1_rc_write(hw_dspp, SDE_HW_RC_REG8, val8, dma_ops,
				 feature);
...
static int _reg_dma_init_dspp_feature_buf(int feature, enum sde_dspp idx)
{
	...
	if (feature == SDE_DSPP_MEMCOLOR) {
		rc = reg_dma_buf_init(&dspp_buf[MEMC_SKIN][idx],
				      feature_reg_dma_sz[feature]);
		if (rc)
			return rc;

		rc = reg_dma_buf_init(&dspp_buf[MEMC_SKY][idx],
				      feature_reg_dma_sz[feature]);
		if (rc)
			return rc;

		rc = reg_dma_buf_init(&dspp_buf[MEMC_FOLIAGE][idx],
				      feature_reg_dma_sz[feature]);
		if (rc)
			return rc;

		rc = reg_dma_buf_init(&dspp_buf[MEMC_PROT][idx],
				      feature_reg_dma_sz[feature]);
	} else if (feature == SDE_DSPP_RC) {
		rc = reg_dma_buf_init(&dspp_buf[RC_MASK_CFG][idx],
				      feature_reg_dma_sz[feature]);
		if (rc)
			return rc;

		rc = reg_dma_buf_init(&dspp_buf[RC_PU_CFG][idx],
	...
	if (!hw_cfg->payload) {
		...
		for (index = 0; index < num_of_mixers; index++) {
			...
			SDE_REG_WRITE(&dspp->hw,
				      dspp->cap->sblk->hist.base +
					      PA_LUTV_DSPP_CTRL_OFF,
				      0);
		}
		goto exit;
	}
...
static int sde_gamut_get_mode_info(u32 pipe, struct drm_msm_3d_gamut *payload,
				   u32 *tbl_len, u32 *tbl_off, u32 *opcode,
				   u32 *scale_off)
{
	int rc = 0;
	...
	case GAMUT_3D_MODE_13:
		...
		*tbl_off = (*opcode == gamut_mode_13a) ? 0 : GAMUT_MODE_13B_OFF;
...
static void dspp_3d_gamutv4_off(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->gamut.base, &op_mode,
			  sizeof(op_mode), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("opmode write single reg failed ret %d\n", rc);
		return;
...
void reg_dmav1_setup_dspp_gcv18(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	reg = BIT(0);
	REG_DMA_SETUP_OPS(dma_write_cfg,
			  ctx->cap->sblk->gc.base + GC_LUT_SWAP_OFF, &reg,
...
static void _dspp_igcv31_off(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	reg = IGC_DIS;
	REG_DMA_SETUP_OPS(dma_write_cfg,
			  ctx->cap->sblk->igc.base + IGC_OPMODE_OFF, &reg,
			  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
...
void reg_dmav1_setup_dspp_igcv31(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	for (i = 0; i < IGC_TBL_NUM; i++) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg, offset, addr[i],
				  IGC_TBL_LEN * sizeof(u32), REG_BLK_WRITE_INC,
				  0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
			DRM_ERROR("lut write failed ret %d\n", rc);
...
int reg_dmav1_setup_rc_pu_configv1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	SDE_EVT32(RC_IDX(ctx), roi_list, rc_mask_cfg,
	...
	param_a = rc_mask_cfg->cfg_param_03;
	rc = _reg_dmav1_rc_program_enable_bits(ctx, rc_mask_cfg, param_a,
					       param_b, param_r, merge_mode,
					       &rc_roi, dma_ops, RC_PU_CFG);
	if (rc) {
		SDE_ERROR("failed to program enable bits, rc:%d\n", rc);
		return rc;
	}
	...
	memcpy(ctx->rc_state.last_roi_list, roi_list,
...
int reg_dmav1_setup_rc_mask_configv1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if ((hw_cfg->len == 0 && hw_cfg->payload == NULL)) {
		...
		rc = _reg_dmav1_rc_write(ctx, SDE_HW_RC_REG1, 0, dma_ops,
					 RC_MASK_CFG);
	...
	rc = _reg_dmav1_rc_program_roi(ctx, rc_mask_cfg, merge_mode, &rc_roi,
				       dma_ops, RC_MASK_CFG);
	if (rc) {
	...
	rc = _reg_dmav1_rc_program_data_offset(ctx, rc_mask_cfg, dma_ops,
					       RC_MASK_CFG);
	if (rc) {
...
static void _dspp_pcc_common_off(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	reg = PCC_DIS;
	REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->pcc.base, &reg,
			  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("setting opcode failed ret %d\n", rc);
...
void reg_dmav1_setup_dspp_pcc_common(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->pcc.base + PCC_C_OFF,
			  data, PCC_LEN, REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
...
void reg_dmav1_setup_dspp_pa_hsicv17(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if (hsic_cfg->flags & PA_HSIC_HUE_ENABLE) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg,
				  ctx->cap->sblk->hsic.base + PA_HUE_OFF, &reg,
				  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
	...
	if (hsic_cfg->flags & PA_HSIC_SAT_ENABLE) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg,
				  ctx->cap->sblk->hsic.base + PA_SAT_OFF, &reg,
				  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
	...
	if (hsic_cfg->flags & PA_HSIC_VAL_ENABLE) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg,
				  ctx->cap->sblk->hsic.base + PA_VAL_OFF, &reg,
				  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
	...
	if (hsic_cfg->flags & PA_HSIC_CONT_ENABLE) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg,
				  ctx->cap->sblk->hsic.base + PA_CONT_OFF, &reg,
				  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
	...
	for (i = 0; i < num_of_mixers; i++) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->hsic.base,
				  &local_opcode, sizeof(local_opcode),
...
static int reg_dma_validate_sixzone_config(struct sde_hw_dspp *ctx, void *cfg,
					   u32 *num_of_mixers, u32 *blk,
					   struct sde_hw_dspp *dspp_list[])
{
	struct sde_hw_cp_cfg *hw_cfg = cfg;
	u32 opcode = 0;
	int rc;
...
void reg_dmav1_setup_dspp_sixzonev17(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	rc = reg_dma_validate_sixzone_config(ctx, cfg, &num_of_mixers, &blk,
					     dspp_list);
	if (rc) {
		return;
	}
	...
	reg = BIT(26);
	REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->sixzone.base, &reg,
			  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl,
			      dspp_buf[SIX_ZONE][ctx->idx], REG_DMA_WRITE,
			      DMA_CTL_QUEUE0, WRITE_IMMEDIATE, SIX_ZONE);
	LOG_FEATURE_ON;
	rc = dma_ops->kick_off(&kick_off);
	if (rc)
		DRM_ERROR("failed to kick off ret %d\n", rc);
...
void reg_dmav2_setup_dspp_sixzonev2(struct sde_hw_dspp *ctx, void *cfg)
{
   ...
   * 16 bit per LUT entry */
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, 0, (u32 *)data, len, REG_BLK_LUT_WRITE,
			  0, 0, 0);
	...
	REG_DMA_SETUP_OPS(dma_write_cfg,
			  ctx->cap->sblk->sixzone.base + SIXZONE_THRESHOLDS_OFF,
			  &sixzone->threshold, sizeof(u32), REG_SINGLE_WRITE, 0,
			  0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("write sixzone threshold failed ret %d\n", rc);
	...
	REG_DMA_SETUP_OPS(dma_write_cfg,
			  ...
			  &sixzone->adjust_p0, sizeof(u32), REG_SINGLE_WRITE, 0,
			  0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(dma_write_cfg,
			  ...
			  &sixzone->adjust_p1, sizeof(u32), REG_SINGLE_WRITE, 0,
			  0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
	...
	for (i = 0; i < num_of_mixers; i++) {
		...
		if (rc) {
			DRM_ERROR(
				"write decode select failed for sixzone ret %d\n",
				rc);
			goto exit;
		}
		...
		if (rc) {
			DRM_ERROR(
				"setting local_hold failed for sixzone ret %d\n",
				rc);
			goto exit;
		}
		...
		if (rc) {
			DRM_ERROR(
				"setting local_opcode failed for sixzone ret %d\n",
				rc);
			goto exit;
		}
	}
				...
				struct sde_hw_cp_cfg *hw_cfg)
{
	...
	for (i = 0; i < num_of_mixers; i++) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->hsic.base,
				  &opcode, sizeof(opcode), REG_SINGLE_MODIFY, 0,
				  0, opcode_mask);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
			DRM_ERROR("setting opcode failed ret %d\n", rc);
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl, dspp_buf[type][ctx->idx],
			      REG_DMA_WRITE, DMA_CTL_QUEUE0, WRITE_IMMEDIATE,
			      type);
	rc = dma_ops->kick_off(&kick_off);
	if (rc)
...
void reg_dmav1_setup_dspp_memcol_protv17(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	REG_DMA_INIT_OPS(dma_write_cfg, dspp_mapping[ctx->idx], MEMC_PROT,
			 dspp_buf[MEMC_PROT][ctx->idx]);
			      ...
			      enum sde_sspp_multirect_index idx)
{
	...
	if (!hw_cfg->ctl || !SDE_SSPP_VALID(ctx->idx) ||
	    feature >= REG_DMA_FEATURES_MAX) {
...
static void vig_gamutv5_off(struct sde_hw_pipe *ctx, void *cfg)
{
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, gamut_base, &op_mode, sizeof(op_mode),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
...
void reg_dmav1_setup_vig_gamutv5(struct sde_hw_pipe *ctx, void *cfg)
{
	...
	for (i = 0; i < GAMUT_3D_TBL_NUM; i++) {
		...
		if (use_2nd_memory)
			reg |= BIT(11);
		REG_DMA_SETUP_OPS(dma_write_cfg,
				  gamut_base + GAMUT_TABLE_SEL_OFF, &reg,
				  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
			DRM_ERROR("write tbl sel reg failed ret %d\n", rc);
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, gamut_base, &op_mode, sizeof(op_mode),
			  REG_SINGLE_MODIFY, 0, 0, REG_DMA_VIG_GAMUT_OP_MASK);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("opmode write single reg failed ret %d\n", rc);
		return;
...
static int
reg_dmav1_setup_vig_igc_common(struct sde_hw_reg_dma_ops *dma_ops,
			       struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
			       ...
			       struct drm_msm_igc_lut *igc_lut)
{
	...
	for (i = 0; i < IGC_TBL_NUM; i++) {
		...
		index = 0;
		REG_DMA_SETUP_OPS(*dma_write_cfg, igc_base + 0x1B0, &index,
				  sizeof(index), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(dma_write_cfg);
		if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, igc_base + 0x1C0, &reg, sizeof(reg),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
		DRM_ERROR("dither strength failed ret %d\n", rc);
...
void reg_dmav1_setup_vig_igcv5(struct sde_hw_pipe *ctx, void *cfg)
{
	...
	rc = reg_dmav1_setup_vig_igc_common(dma_ops, &dma_write_cfg, ctx, cfg,
					    VIG_IGC_DATA_MASK, igc_lut);
	if (rc) {
			       ...
			       enum sde_sspp_multirect_index idx)
{
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, igc_dither_off, &reg, sizeof(reg),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
			      ...
			      enum sde_sspp_multirect_index idx)
{
   ...
   */
	REG_DMA_SETUP_OPS(dma_write_cfg, gc_base, gc_lut->c2,
			  DMA_1D_LUT_GC_LEN * sizeof(u32), REG_BLK_WRITE_SINGLE,
			  0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("lut write failed ret %d\n", rc);
	...
	reg = BIT(2);
	REG_DMA_SETUP_OPS(dma_write_cfg, gc_opmode_off, &reg, sizeof(reg),
			  REG_SINGLE_MODIFY, 0, 0, REG_DMA_DMA_GC_OP_MASK);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("setting opcode failed ret %d\n", rc);
		return;
				     ...
				     u32 offset)
{
	...
	for (filter = 0; filter < QSEED3LITE_FILTERS && config_lut; filter++) {
		if (!lut[filter])
			continue;
		lut_offset = 0;
		lut_addr = QSEED3L_COEF_LUT_OFF + offset + off_tbl[filter];
		REG_DMA_SETUP_OPS(*buf, lut_addr, &lut[filter][0],
				  QSEED3L_LUT_SIZE * sizeof(u32),
				  REG_BLK_WRITE_SINGLE, 0, 0, 0);
		rc = dma_ops->setup_payload(buf);
		if (rc) {
			DRM_ERROR("lut write failed ret %d\n", rc);
				      ...
				      u32 offset, bool de_lpf)
{
	...
	REG_DMA_SETUP_OPS(*buf, offset + QSEED3_DE_OFFSET, de_config,
			  sizeof(de_config), REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(buf);
	if (rc) {
	...
	if (de_lpf) {
		...
		REG_DMA_SETUP_OPS(*buf, offset + QSEED5_DE_LPF_OFFSET,
				  &de_lpf_config, sizeof(u32), REG_SINGLE_WRITE,
				  0, 0, 0);
		rc = dma_ops->setup_payload(buf);
		if (rc) {
...
void reg_dmav1_setup_vig_qseed3(struct sde_hw_pipe *ctx,
				...
				struct sde_hw_pixel_ext *pe, void *scaler_cfg)
{
	...
	if (!ctx || !pe || !scaler_cfg) {
		DRM_ERROR("invalid params ctx %pK pe %pK scaler_cfg %pK", ctx,
			  pe, scaler_cfg);
		return;
	}
	...
	if (scaler3_cfg->de.enable) {
		if (test_bit(SDE_SSPP_SCALER_DE_LPF_BLEND, &ctx->cap->features))
			de_lpf_cap = true;
		rc = reg_dmav1_setup_scaler3_de(&dma_write_cfg, scaler3_cfg,
						offset, de_lpf_cap);
		if (!rc)
			op_mode |= BIT(8);
	}
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, offset + 0x10, cache, sizeof(cache),
			  REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("setting phase failed ret %d\n", rc);
	...
	if (is_qseed3_rev_qseed3lite(ctx->catalog)) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg, offset + 0x60, &dir_weight,
				  sizeof(u32), REG_SINGLE_WRITE, 0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
			DRM_ERROR("lut write failed ret %d\n", rc);
			return;
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, offset + 0x4, &op_mode,
			  sizeof(op_mode), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
...
static int reg_dmav1_get_ltm_blk(struct sde_hw_cp_cfg *hw_cfg, enum sde_ltm idx,
				 enum sde_ltm *dspp_idx, u32 *blk)
{
...
void reg_dmav1_setup_ltm_roiv1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if (cfg_param->cfg_param_01 + cfg_param->cfg_param_03 >
		...
		DRM_DEBUG_DRIVER("invalid input = [%u,%u], displayh = %u\n",
				 cfg_param->cfg_param_01,
				 cfg_param->cfg_param_03, hw_cfg->displayh);
	...
	if (cfg_param->cfg_param_02 + cfg_param->cfg_param_04 >
		...
		DRM_DEBUG_DRIVER("invalid input = [%u,%u], displayv = %u\n",
				 cfg_param->cfg_param_02,
				 cfg_param->cfg_param_04, hw_cfg->displayv);
...
static int reg_dmav1_setup_ltm_vlutv1_common(
	struct sde_hw_dspp *ctx, void *cfg, struct sde_hw_reg_dma_ops *dma_ops,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg, u32 *opmode,
	enum sde_ltm *dspp_idx)
{
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, 0x3c, &payload->data[0][0], len,
			  REG_BLK_WRITE_INC, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
...
void reg_dmav1_setup_ltm_vlutv1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	rc = reg_dmav1_setup_ltm_vlutv1_common(
		ctx, cfg, dma_ops, &dma_write_cfg, opmode, dspp_idx);
	if (rc)
		goto vlut_exit;
...
void reg_dmav1_setup_ltm_vlutv1_2(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	for (i = 0; i < num_mixers; i++) {
		...
		REG_DMA_SETUP_OPS(dma_write_cfg, 0x18, &merge_mode, sizeof(u32),
				  REG_SINGLE_MODIFY, 0, 0, 0xFFFFFFFC);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
				   ...
				   enum sde_reg_dma_features feature)
{
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl,
			      ...
			      DMA_CTL_QUEUE1, WRITE_IMMEDIATE, feature);
...
static void _dspp_igcv4_off(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	reg = 0;
	REG_DMA_SETUP_OPS(dma_write_cfg, ctx->cap->sblk->igc.base + 0x4, &reg,
			  sizeof(reg), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(&dma_write_cfg);
	if (rc) {
		DRM_ERROR("setting opcode failed ret %d\n", rc);
...
void reg_dmav2_setup_dspp_3d_gamutv43(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	for (j = 0; j < GAMUT_3D_MODE17_TBL_SZ; j++) {
       ...
       */
			...
			data[k++] =
				(u16)((payload->col[i][j].c2_c1 >> 16) << 4);
	...
	if (payload && (payload->flags & GAMUT_3D_MAP_EN)) {
		for (i = 0; i < GAMUT_3D_SCALE_OFF_TBL_NUM; i++) {
			for (j = 0; j < GAMUT_3D_SCALE_OFF_SZ; j++) {
				...
				scale_off[i][j] &= 0xfff;
				tmp = tmp << 3;
				scale_off[i][j] = tmp | scale_off[i][j];
			}
		}
	}
	...
	if (op_mode & GAMUT_MAP_EN) {
		for (i = 0; i < GAMUT_3D_SCALE_OFF_TBL_NUM; i++) {
			...
			REG_DMA_SETUP_OPS(dma_write_cfg, scale_tbl_offset,
					  &scale_off[i][0], GAMUT_SCALE_OFF_LEN,
					  REG_BLK_WRITE_SINGLE, 0, 0, 0);
			rc = dma_ops->setup_payload(&dma_write_cfg);
			if (rc) {
				DRM_ERROR("write scale/off reg failed ret %d\n",
...
void reg_dmav2_setup_vig_gamutv61(struct sde_hw_pipe *ctx, void *cfg)
{
	...
	for (j = 0; j < GAMUT_3D_MODE17_TBL_SZ; j++) {
       ...
       */
			...
			data[k++] =
				(u16)((payload->col[i][j].c2_c1 >> 16) << 6);
...
int reg_dmav2_init_spr_op_v1(int feature, enum sde_dspp dspp_idx)
{
	...
	for (i = 0; (i < dma_feature_cnt) && !rc; i++) {
		rc = dma_ops->check_support(dma_features[i], blk,
					    &is_supported);
		if (!rc) {
			if (is_supported)
				rc = reg_dma_buf_init(
					&dspp_buf[dma_features[i]][dspp_idx],
					buffer_size);
			else
				rc = -EOPNOTSUPP;
		}
...
int reg_dmav1_setup_spr_cfg3_params(
	struct sde_hw_dspp *ctx, struct drm_msm_spr_init_cfg *payload,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops)
{
	...
	for (i = 0; i < reg_cnt; i++) {
		index = 2 * i;
		reg[i] =
			APPLY_MASK_AND_SHIFT(payload->cfg15[index], 12, 0) |
...
int reg_dmav1_setup_spr_cfg4_params(
	struct sde_hw_dspp *ctx, struct drm_msm_spr_init_cfg *payload,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops)
{
...
int reg_dmav1_setup_spr_cfg5_params(
	struct sde_hw_dspp *ctx, struct drm_msm_spr_init_cfg *payload,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops)
{
...
void reg_dmav1_disable_spr(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl,
			      dspp_buf[SPR_INIT][ctx->idx], REG_DMA_WRITE,
			      DMA_CTL_QUEUE0, WRITE_IMMEDIATE, SPR_INIT);
	rc = dma_ops->kick_off(&kick_off);
	if (rc) {
		DRM_ERROR("failed to kick off ret %d\n", rc);
		return;
...
int reg_dmav1_setup_spr_init_common(
	struct sde_hw_dspp *ctx, struct drm_msm_spr_init_cfg *payload,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops)
{
	...
	for (i = 0; i < reg_cnt; i++) {
		index = 6 * i;
		reg[i] =
			APPLY_MASK_AND_SHIFT(payload->cfg17[index], 5, 0) |
	...
	for (i = 0; i < reg_cnt; i++) {
		index = 2 * i;
		reg[i] =
			APPLY_MASK_AND_SHIFT(payload->cfg16[index], 11, 0) |
	...
	rc = reg_dmav1_setup_spr_cfg3_params(ctx, payload, dma_write_cfg,
					     dma_ops);
	if (rc)
		return rc;

	rc = reg_dmav1_setup_spr_cfg4_params(ctx, payload, dma_write_cfg,
					     dma_ops);
...
int reg_dmav1_setup_spr_init_kickoff(
	uint32_t version, uint32_t base_off, struct sde_hw_reg_dma_ops *dma_ops,
	struct sde_hw_cp_cfg *hw_cfg, struct drm_msm_spr_init_cfg *payload,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg)
{
	struct sde_reg_dma_kickoff_cfg kick_off;
	uint32_t reg[2];
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl, dma_write_cfg->dma_buf,
			      REG_DMA_WRITE, DMA_CTL_QUEUE0, WRITE_IMMEDIATE,
			      SPR_INIT);
	rc = dma_ops->kick_off(&kick_off);
	if (rc) {
...
void reg_dmav1_setup_spr_init_cfgv1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if (reg_dmav1_get_spr_target(ctx, cfg, &dma_ops, &base_off, &buffer,
				     &disable))
		return;
	...
	if (!(payload->flags & SPR_FLAG_BYPASS)) {
		rc = reg_dmav1_setup_spr_init_common(ctx, payload,
...
void reg_dmav1_setup_spr_init_cfgv2(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if (!(payload->flags & SPR_FLAG_BYPASS)) {
		...
		rc = reg_dmav1_setup_spr_cfg5_params(ctx, payload,
						     &dma_write_cfg, dma_ops);
		if (rc)
			return;
	} else {
...
void reg_dmav1_setup_spr_udc_cfgv2(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if (reg == NULL) {
		DRM_ERROR(
			"Unable to allocate memory for UDC mask programming\n");
		return;
	}
	...
	if (disable) {
		...
		for (i = 0; i < 3; i++) {
			...
			reg[index++] =
				APPLY_MASK_AND_SHIFT(payload->cfg1[j], 16, 0) |
				APPLY_MASK_AND_SHIFT(payload->cfg1[j + 1], 16,
						     16);
			j += 2;

			lines[i] = APPLY_MASK_AND_SHIFT(payload->cfg1[j + 1], 9,
							0);
			reg[index++] =
				APPLY_MASK_AND_SHIFT(payload->cfg1[j], 10, 0) |
				lines[i] << 16;
			j += 2;

			reg[index++] =
				APPLY_MASK_AND_SHIFT(payload->cfg1[j], 4, 0) |
				APPLY_MASK_AND_SHIFT(payload->cfg1[j + 1], 4,
						     8) |
				APPLY_MASK_AND_SHIFT(payload->cfg1[j + 2], 4,
						     16) |
				APPLY_MASK_AND_SHIFT(payload->cfg1[j + 3], 4,
						     24);
			j += 4;
			reg[index++] =
				APPLY_MASK_AND_SHIFT(payload->cfg1[j++], 11, 0);
		}
		...
		REG_DMA_SETUP_OPS(dma_write_cfg, reg_off, reg,
				  reg_cnt * sizeof(u32), REG_BLK_WRITE_SINGLE,
				  0, 0, 0);
		rc = dma_ops->setup_payload(&dma_write_cfg);
		if (rc) {
		...
		for (i = 0; i < 3; i++) {
			...
			for (line = 0; line < line_cnt; line++) {
				reg[j++] = (payload->cfg2[index] & 0xff) |
					   ((payload->cfg2[index + 1] & 0xff)
					    << 8) |
					   ((payload->cfg2[index + 2] & 0xff)
					    << 16) |
					   ((payload->cfg2[index + 3] & 0xff)
					    << 24);
				index += 4;
			}
		}
		...
		if (rc) {
			DRM_ERROR(
				"write spr udc cfg p3 failed - L%d\t ret %d\n",
				i, rc);
			goto cleanup;
		}
...
int reg_dmav1_setup_spr_pu_common(struct sde_hw_dspp *ctx,
				  struct sde_hw_cp_cfg *hw_cfg,
				  struct msm_roi_list *roi_list,
				  struct sde_hw_reg_dma_ops *dma_ops,
				  struct sde_reg_dma_buffer *buffer)
{
	...
	if (!roi_list) {
		...
		if (roi_list->num_rects > 1) {
			DRM_ERROR(
				"multiple pu regions not supported with spr\n");
			return -EINVAL;
		}

		if ((roi_list->spr_roi[0].x2 - roi_list->spr_roi[0].x1) !=
		    hw_cfg->panel_width) {
			DRM_ERROR("pu region not full width %d\n",
				  (roi_list->spr_roi[0].x2 -
				   roi_list->spr_roi[0].x1));
			return -EINVAL;
		}
	...
	REG_DMA_SETUP_OPS(dma_write_cfg, reg_off, &reg, sizeof(__u32),
			  REG_SINGLE_WRITE, 0, 0, 0);
...
void reg_dmav1_setup_spr_pu_cfgv1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl, buffer, REG_DMA_WRITE,
			      DMA_CTL_QUEUE0, WRITE_IMMEDIATE, SPR_PU_CFG);
	rc = dma_ops->kick_off(&kick_off);
	if (rc) {
		DRM_ERROR("failed to kick off ret %d\n", rc);
		return;
			       ...
			       struct sde_hw_cp_cfg *hw_cfg)
{
	...
	SDE_EVT32(SDE_EVTLOG_FUNC_ENTRY);
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl,
			      dspp_buf[DEMURA_CFG][ctx->idx], REG_DMA_WRITE,
			      DMA_CTL_QUEUE0, WRITE_IMMEDIATE, DEMURA_CFG);
...
static int __reg_dmav1_setup_demurav1_cfg0_c_params_cmn(
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, u64 *p, u32 len, u32 *temp,
	u32 comp_index, u32 demura_base)
{
	u32 i;
	int rc;

	i = ((comp_index & 0x3) << 28) | BIT(31);
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x68, &i, sizeof(i),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x6c, temp,
			  sizeof(u64) * len, REG_BLK_WRITE_INC, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
...
static int __reg_dmav1_setup_demurav1_cfg0_c_params(
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct drm_msm_dem_cfg *dcfg, struct sde_hw_reg_dma_ops *dma_ops,
	u32 *temp, u32 temp_sz, u32 comp_index, u32 demura_base)
{
	u32 len;
	u64 *p;
	int rc;
	...
	if (temp_sz < ARRAY_SIZE(dcfg->cfg0_param2_c0) * 8 || comp_index > 2) {
		DRM_ERROR("exp sz %zd act sz %d comp index %d\n",
			  ARRAY_SIZE(dcfg->cfg0_param2_c0) * 8, temp_sz,
			  comp_index);
		return -EINVAL;
	}
	...
	rc = __reg_dmav1_setup_demurav1_cfg0_c_params_cmn(
		dma_write_cfg, dma_ops, p, len, temp, comp_index, demura_base);
	return rc;
}
...
static u32 __get_offset_idx(u32 idx, u32 depth)
{
	...
	if (depth > 8 || idx > 3 || (depth == 8 && idx > 0) ||
	    (depth == 7 && idx > 1)) {
...
static int __reg_dmav1_setup_demurav1_cfg0(
	struct sde_hw_dspp *ctx, struct drm_msm_dem_cfg *dcfg,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, struct sde_hw_cp_cfg *hw_cfg)
{
	...
	for (i = 0; i < ARRAY_SIZE(dcfg->cfg01_param0); i += 2) {
		temp[i >> 1] =
			(dcfg->cfg01_param0[i] & REG_MASK(12)) |
			((dcfg->cfg01_param0[i + 1] & REG_MASK(12)) << 16);
		DRM_DEBUG_DRIVER("0x1c: index %d value %x\n", i >> 1,
				 temp[i >> 1]);
	}
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x1c, temp,
			  sizeof(u32) * 4, REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
		DRM_ERROR("0x1c: write err %d len %zd buffer index %d\n", rc,
			  sizeof(u32) * 4, dma_write_cfg->dma_buf->index);
		goto quit;
	}
	...
	for (i = 0; i < ARRAY_SIZE(dcfg->cfg0_param1); i++) {
		...
		*p |= (((dcfg->cfg0_param1[i] & 0x3f) & REG_MASK(6)) << shift);
		DRM_DEBUG_DRIVER("0xc: index %d value %x val %x shift %d\n", i,
				 *p, (dcfg->cfg0_param1[i] & 0x3f), shift);
	}
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0xc, temp,
			  sizeof(u32) * 2, REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
		DRM_ERROR("0xc: write err %d len %zd buffer index %d\n", rc,
			  sizeof(u32) * 2, dma_write_cfg->dma_buf->index);
		goto quit;
	}
	...
	for (i = 0; i < ARRAY_SIZE(dcfg->cfg0_param0); i++)
		DRM_DEBUG_DRIVER("0x2c: index %d value %x\n", i,
				 dcfg->cfg0_param0[i]);
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x2c, dcfg->cfg0_param0,
			  ARRAY_SIZE(dcfg->cfg0_param0) * sizeof(u32),
			  REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0xb0, temp,
	...
	for (i = 0; i < 3 && !(dcfg->flags & DEMURA_SKIP_CFG0_PARAM2); i++) {
		rc = __reg_dmav1_setup_demurav1_cfg0_c_params(
			dma_write_cfg, dcfg, dma_ops, temp,
			sizeof(struct drm_msm_dem_cfg), i, demura_base);
		if (rc)
			goto quit;
	}
	...
	codebook_offset =
		__get_offset_idx(dcfg->cfg0_param2_idx, dcfg->c0_depth);

	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x70, &codebook_offset,
			  sizeof(codebook_offset), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
		DRM_ERROR("0x70: REG_SINGLE_WRITE err %d len %zd buf idx %d\n",
			  rc, sizeof(codebook_offset),
			  dma_write_cfg->dma_buf->index);
		goto quit;
	}
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x80, &width,
			  sizeof(width), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	i = 0x400;
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0xe0, &i, sizeof(i),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
...
static int __reg_dmav1_setup_demurav1_cfg1(
	struct sde_hw_dspp *ctx, struct drm_msm_dem_cfg *dcfg,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, struct sde_hw_cp_cfg *hw_cfg)
{
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x14, &i, sizeof(i),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x54, &temp[0],
			  sizeof(temp[0]), REG_SINGLE_WRITE, 0, 0, 0);
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x5c, &temp[1],
			  sizeof(temp[1]), REG_SINGLE_WRITE, 0, 0, 0);
	...
	if (dcfg->cfg1_en) {
		cfg1_data[0] =
			(dcfg->cfg1_param0_c0[0] & REG_MASK(10)) |
			((dcfg->cfg1_param0_c1[0] & REG_MASK(10)) << 10) |
			((dcfg->cfg1_param0_c2[0] & REG_MASK(10)) << 20) |
			BIT(31);
		DRM_DEBUG_DRIVER("0x64: value %x\n", cfg1_data[0]);
		for (i = 1; i < len; i++) {
			cfg1_data[i] =
				(dcfg->cfg1_param0_c0[i] & REG_MASK(10)) |
				((dcfg->cfg1_param0_c1[i] & REG_MASK(10))
				 << 10) |
				((dcfg->cfg1_param0_c2[i] & REG_MASK(10))
				 << 20);
			DRM_DEBUG_DRIVER("0x64 index %d value %x\n", i,
					 cfg1_data[i]);
		}
		REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x64, cfg1_data,
				  len * sizeof(u32), REG_BLK_WRITE_INC, 0, 0,
				  0);
		rc = dma_ops->setup_payload(dma_write_cfg);
		if (rc) {
			DRM_ERROR("lut write failed ret %d\n", rc);
...
static int
__reg_dmav1_setup_demurav1_cfg3(struct sde_hw_dspp *ctx,
				...
				struct sde_hw_reg_dma_ops *dma_ops)
{
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0xd0, temp,
			  ARRAY_SIZE(temp) * sizeof(u32), REG_BLK_WRITE_SINGLE,
			  0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc)
...
static int
__reg_dmav1_setup_demurav1_cfg5(struct sde_hw_dspp *ctx,
				struct drm_msm_dem_cfg *dcfg,
				struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
				struct sde_hw_reg_dma_ops *dma_ops)
{
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0xa0, temp,
			  ARRAY_SIZE(temp) * sizeof(u32), REG_BLK_WRITE_SINGLE,
			  0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc)
					 ...
					 struct sde_hw_cp_cfg *hw_cfg)
{
	...
	temp = hw_cfg->panel_width /
	       (2 * ((dcfg->flags & DEMURA_FLAG_1) ? 2 : 1));
	if (dcfg->pentile) {
		w = dcfg->c0_depth * (temp / 2) + dcfg->c1_depth * temp +
		    dcfg->c2_depth * (temp / 2);
	} else {
		w = dcfg->c0_depth * temp + dcfg->c1_depth * temp +
		    dcfg->c2_depth * temp;
	}
	if (w % 32)
		w = 32 - (w % 32) + w;
	...
	if (h != hw_cfg->skip_blend_plane_h ||
	    w != hw_cfg->skip_blend_plane_w) {
		DRM_ERROR(
			"invalid hfc cfg exp h %d exp w %d act h %d act w %d\n",
			h, w, hw_cfg->skip_blend_plane_h,
			hw_cfg->skip_blend_plane_w);
		DRM_ERROR(
			"c0_depth %d c1_depth %d c2 depth %d hw_cfg->panel_width %d\n",
			dcfg->c0_depth, dcfg->c1_depth, dcfg->c2_depth,
			hw_cfg->panel_width);
		return false;
	}
	...
	DRM_ERROR(
		"invalid HFC plane dcfg->src_id %d hw_cfg->skip_blend_plane %d\n",
		dcfg->src_id, hw_cfg->skip_blend_plane);
	return false;
}

static void __reg_dmav1_setup_demura_common_en(
	struct sde_hw_dspp *ctx, struct drm_msm_dem_cfg *dcfg,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, struct sde_hw_cp_cfg *hw_cfg,
	u32 *en)
...
static int __reg_dmav1_setup_demurav1_en(
	struct sde_hw_dspp *ctx, struct drm_msm_dem_cfg *dcfg,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, struct sde_hw_cp_cfg *hw_cfg)
{
	...
	__reg_dmav1_setup_demura_common_en(ctx, dcfg, dma_write_cfg, dma_ops,
					   hw_cfg, &en);

	DRM_DEBUG_DRIVER("demura v1 en 0x%x\n", en);
	SDE_EVT32(en);
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x4, &en, sizeof(en),
			  REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc)
...
static int __reg_dmav1_setup_demurav2_en(
	struct sde_hw_dspp *ctx, struct drm_msm_dem_cfg *dcfg,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, struct sde_hw_cp_cfg *hw_cfg)
{
...
static int __reg_dmav1_setup_demurav1_dual_pipe(
	...
	struct sde_hw_reg_dma_ops *dma_ops)
{
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg, demura_base + 0x58, &temp,
			  sizeof(temp), REG_SINGLE_WRITE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc)
...
static int __reg_dmav1_setup_demura_cfg_common(
	struct sde_hw_dspp *ctx, struct drm_msm_dem_cfg *dcfg,
	struct sde_reg_dma_setup_ops_cfg *dma_write_cfg,
	struct sde_hw_reg_dma_ops *dma_ops, struct sde_hw_cp_cfg *hw_cfg)
{
	...
	rc = __reg_dmav1_setup_demurav1_cfg0(ctx, dcfg, dma_write_cfg, dma_ops,
					     hw_cfg);
	if (rc) {
		DRM_ERROR("failed setup_demurav1_cfg0 rc %d", rc);
		return rc;
	}
	rc = __reg_dmav1_setup_demurav1_cfg1(ctx, dcfg, dma_write_cfg, dma_ops,
					     hw_cfg);
	if (rc) {
	...
	rc = __reg_dmav1_setup_demurav1_cfg3(ctx, dcfg, dma_write_cfg, dma_ops);
	if (rc) {
		DRM_ERROR("failed setup_demurav1_cfg3 rc %d", rc);
		return rc;
	}

	rc = __reg_dmav1_setup_demurav1_cfg5(ctx, dcfg, dma_write_cfg, dma_ops);
	if (rc) {
...
void reg_dmav1_setup_demurav1(struct sde_hw_dspp *ctx, void *cfx)
{
	...
	rc = __reg_dmav1_setup_demura_cfg_common(ctx, dcfg, &dma_write_cfg,
						 dma_ops, hw_cfg);
	if (rc) {
	...
	rc = __reg_dmav1_setup_demurav1_en(ctx, dcfg, &dma_write_cfg, dma_ops,
					   hw_cfg);
	if (rc) {
...
void reg_dmav1_setup_demurav2(struct sde_hw_dspp *ctx, void *cfx)
{
	...
	rc = __reg_dmav1_setup_demurav2_en(ctx, dcfg, &dma_write_cfg, dma_ops,
					   hw_cfg);
	if (rc) {
...
void reg_dmav1_setup_demura_cfg0_param2(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	for (i = 0; i < 3; i++) {
		if (!i)
			...
			p = dcfg->cfg0_param2_c2;
		__reg_dmav1_setup_demurav1_cfg0_c_params_cmn(
			&dma_write_cfg, dma_ops, p, len, temp, i, demura_base);
	}
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl,
			      dspp_buf[DEMURA_CFG0_PARAM2][ctx->idx],
			      REG_DMA_WRITE, DMA_CTL_QUEUE0, WRITE_IMMEDIATE,
