...
#define RM_IS_TOPOLOGY_MATCH(t, r)                                      \
	((t).num_lm == (r).num_lm && (t).num_comp_enc == (r).num_enc && \
	 (t).num_intf == (r).num_intf && (t).comp_type == (r).comp_type)
...
char sde_hw_blk_str[SDE_HW_BLK_MAX][SDE_HW_BLK_NAME_LEN] = {
	"top", "sspp", "lm",	   "dspp", "ds",
	"ctl", "cdm",  "pingpong", "intf", "wb",
	"dsc", "vdc",  "merge_3d", "qdss", "dnsc_blur"
};
...
static void
_sde_rm_inc_resource_info_lm(struct sde_rm *rm,
			     ...
			     struct sde_rm_hw_blk *blk)
{
     ...
     */
		if ((!blk2->rsvp ||
		     (blk->rsvp && blk2->rsvp->enc_id == blk->rsvp->enc_id)) &&
...
static void
_sde_rm_dec_resource_info_lm(struct sde_rm *rm,
			     struct msm_resource_caps_info *avail_res,
			     struct sde_rm_hw_blk *blk)
{
			      ...
			      struct msm_resource_caps_info *avail_res)
{
	...
	u32 lm_pref =
		(BIT(SDE_DISP_PRIMARY_PREF) | BIT(SDE_DISP_SECONDARY_PREF));

	mutex_lock(&rm->rm_lock);

	/* Get all currently available resources */
	memcpy(avail_res, &rm->avail_res, sizeof(rm->avail_res));

	/**
   * When the encoder is null, assume display is external in order to return the
   * count of availalbe non-preferred LMs
   */
	if (!drm_enc)
		is_built_in = false;
	else
	...
	for (type = 0; type < SDE_HW_BLK_MAX; type++) {
		list_for_each_entry(blk, &rm->hw_blks[type], list) {
			...
			if (blk->rsvp && drm_enc &&
			    blk->rsvp->enc_id == drm_enc->base.id) {
       ...
       */
			if (type == SDE_HW_BLK_LM) {
				...
				if (!blk->rsvp && !blk->rsvp_nxt &&
				    !is_built_in && is_pref)
					_sde_rm_dec_resource_info(rm, avail_res,
								  blk);
			}
		}
	}
...
static void _sde_rm_print_rsvps(struct sde_rm *rm,
				enum sde_rm_dbg_rsvp_stage stage)
{
	...
	list_for_each_entry(rsvp, &rm->rsvps, list) {
		SDE_DEBUG("%d rsvp%s[s%ue%u] topology %d\n", stage,
			  rsvp->pending ? "_nxt" : "", rsvp->seq, rsvp->enc_id,
			  rsvp->topology);
	...
	for (type = 0; type < SDE_HW_BLK_MAX; type++) {
		list_for_each_entry(blk, &rm->hw_blks[type], list) {
			...
			SDE_EVT32(stage, (blk->rsvp) ? blk->rsvp->seq : 0,
				  (blk->rsvp) ? blk->rsvp->enc_id : 0,
				  (blk->rsvp_nxt) ? blk->rsvp_nxt->seq : 0,
				  (blk->rsvp_nxt) ? blk->rsvp_nxt->enc_id : 0,
				  blk->type, blk->id);
...
static void _sde_rm_print_rsvps_by_type(struct sde_rm *rm,
					enum sde_hw_blk_type type)
{
	struct sde_rm_hw_blk *blk;
...
void sde_rm_init_hw_iter(struct sde_rm_hw_iter *iter, uint32_t enc_id,
			 enum sde_hw_blk_type type)
{
...
sde_rm_get_topology_name(struct sde_rm *rm,
	...
	if (list_forward) {
		list_for_each_entry_continue(i->blk, blk_list, list) {
			...
			if (i->blk->type != i->type) {
				SDE_ERROR(
					"found incorrect block type %d on %d list\n",
					i->blk->type, i->type);
				return false;
			}

			if ((i->enc_id == 0) ||
			    (rsvp && rsvp->enc_id == i->enc_id)) {
				i->hw = i->blk->hw;
...
static void _sde_rm_hw_destroy(enum sde_hw_blk_type type,
			       struct sde_hw_blk_reg_map *hw)
{
	switch (type) {
	case SDE_HW_BLK_LM:
...
static int _sde_rm_hw_blk_create(struct sde_rm *rm, struct sde_mdss_cfg *cat,
				 void __iomem *mmio, enum sde_hw_blk_type type,
				 uint32_t id, void *hw_catalog_info)
{
	int rc;
	struct sde_rm_hw_blk *blk;
			   ...
			   struct sde_kms *sde_kms)
{
	...
	while (_sde_rm_get_hw_locked(rm, &iter, true)) {
		...
		if (sde_kms->aspace[MSM_SMMU_DOMAIN_UNSECURE] &&
			...
			if (sde_hw_fence_init(
				    ctl, sde_kms, use_ipcc,
				     ...
				     struct sde_kms *sde_kms)
{
	...
	if (cat->hw_fence_rev) {
		if (_init_hw_fences(rm,
				    test_bit(SDE_FEATURE_HW_FENCE_IPCC,
					     cat->features),
				    sde_kms)) {
			SDE_INFO(
				"failed to init hw-fences, disabling hw-fences\n");
			cat->hw_fence_rev = 0;
		}
	}
...
static int _sde_rm_debugfs_status_open(struct inode *inode, struct file *file)
{
...
void sde_rm_debugfs_init(struct sde_rm *sde_rm, struct dentry *parent)
{
	...
	debugfs_create_file("rm_status", 0400, parent, sde_rm,
...
static bool _sde_rm_check_lm(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
...
static bool _sde_rm_reserve_dspp(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
				 const struct sde_lm_cfg *lm_cfg,
				 struct sde_rm_hw_blk *lm,
				 struct sde_rm_hw_blk **dspp)
{
...
static bool _sde_rm_reserve_ds(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
			       const struct sde_lm_cfg *lm_cfg,
			       struct sde_rm_hw_blk *lm,
			       struct sde_rm_hw_blk **ds)
{
...
static bool _sde_rm_reserve_pp(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
			       struct sde_rm_requirements *reqs,
			       const struct sde_lm_cfg *lm_cfg,
...
static bool _sde_rm_check_lm_and_get_connected_blks(
	struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
	struct sde_rm_requirements *reqs, struct sde_rm_hw_blk *lm,
	struct sde_rm_hw_blk **dspp, struct sde_rm_hw_blk **ds,
	struct sde_rm_hw_blk **pp, struct sde_rm_hw_blk *primary_lm,
	u32 conn_lm_mask)
{
	...
	*pp = NULL;
	...
	is_conn_primary = (reqs->hw_res.display_type == SDE_CONNECTOR_PRIMARY) ?
				  true :
				  false;
	is_conn_secondary =
		(reqs->hw_res.display_type == SDE_CONNECTOR_SECONDARY) ? true :
									 false;

	SDE_DEBUG(
		"check lm %d: dspp %d ds %d pp %d features %ld disp type %d\n",
	...
	if (!lm_primary_pref && !lm_secondary_pref) {
		...
		ret = _sde_rm_check_lm(rm, rsvp, reqs, lm_cfg, lm, dspp, ds,
				       pp);
		if (!ret)
			return ret;
     ...
     */
		if ((RM_RQ_CWB(reqs) && !cwb_pref) ||
			...
			SDE_DEBUG(
				"fail: cwb/dcwb supported lm not allocated\n");
			return false;
		} else if (!RM_RQ_DCWB(reqs) && dcwb_pref) {
			SDE_DEBUG(
				"fail: dcwb supported dummy lm incorrectly allocated\n");
			return false;
	...
	/* Reserve pp */
	ret = _sde_rm_reserve_pp(rm, rsvp, reqs, lm_cfg, pp_cfg, lm, dspp, ds,
				 pp);
	if (!ret)
		return ret;
...
static int _sde_rm_reserve_lms(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
			       struct sde_rm_requirements *reqs, u8 *_lm_ids)

{
	...
	while (lm_count != reqs->topology->num_lm &&
		...
		SDE_DEBUG("blk id = %d, _lm_ids[%d] = %d\n", iter_i.blk->id,
			  lm_count, _lm_ids ? _lm_ids[lm_count] : -1);

		if (_lm_ids && (lm[lm_count])->id != _lm_ids[lm_count])
			continue;

		if (!_sde_rm_check_lm_and_get_connected_blks(
			    rm, rsvp, reqs, lm[lm_count], &dspp[lm_count],
			    &ds[lm_count], &pp[lm_count], NULL, conn_lm_mask))
			continue;
		...
		if (RM_RQ_DCWB(reqs))
			conn_lm_mask = conn_lm_mask &
				       ~BIT(ffs(conn_lm_mask) - 1);
		...
		while (_sde_rm_get_hw_locked(rm, &iter_j, true)) {
			...
			if (!_sde_rm_check_lm_and_get_connected_blks(
				    rm, rsvp, reqs, iter_j.blk, &dspp[lm_count],
				    &ds[lm_count], &pp[lm_count], iter_i.blk,
				    conn_lm_mask))
				continue;

			SDE_DEBUG("blk id = %d, _lm_ids[%d] = %d\n",
				  iter_j.blk->id, lm_count,
	...
	for (i = 0; i < lm_count; i++) {
		...
		SDE_EVT32(lm[i]->type, rsvp->enc_id, lm[i]->id, pp[i]->id,
			  dspp[i] ? dspp[i]->id : 0, ds[i] ? ds[i]->id : 0);
	}
...
static int _sde_rm_reserve_ctls(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
				struct sde_rm_requirements *reqs,
				...
				u8 *_ctl_ids)
{
     ...
     */
		...
		if (_sde_rm_get_hw_locked(rm, &curr, true) &&
		    (curr.blk->id != iter.blk->id)) {
			SDE_EVT32(curr.blk->id, iter.blk->id,
...
static bool _sde_rm_check_vdc(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
...
static int _sde_rm_reserve_dsc(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
			       struct sde_rm_requirements *reqs, u8 *_dsc_ids)
{
	struct sde_rm_hw_iter iter_i, iter_j;
	...
	while (alloc_count != num_dsc_enc &&
		...
		bool has_422_420_support = BIT(SDE_DSC_NATIVE_422_EN) &
					   features;
		...
		SDE_DEBUG("blk id = %d, _dsc_ids[%d] = %d\n", iter_i.blk->id,
			  alloc_count, _dsc_ids ? _dsc_ids[alloc_count] : -1);

		/* reset and restart from current block if allocated dsc blocks are not
     * contiguous */
		if (alloc_count >= 1 &&
		    (abs(dsc[alloc_count - 1]->id - iter_i.blk->id) != 1)) {
			reserve_mask = 0;
			alloc_count = 0;
		}
		...
		while (_sde_rm_get_hw_locked(rm, &iter_j, list_forward)) {
			...
			if (_dsc_ids &&
			    (iter_j.blk->id != _dsc_ids[alloc_count]))
				continue;
...
static int _sde_rm_reserve_vdc(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
			       struct sde_rm_requirements *reqs,
			       const struct sde_rm_topology_def *top,
			       u8 *_vdc_ids)
{
	...
	while (alloc_count != num_vdc_enc &&
		...
		SDE_DEBUG("blk id = %d, _vdc_ids[%d] = %d\n", iter_i.blk->id,
			  alloc_count, _vdc_ids ? _vdc_ids[alloc_count] : -1);
...
static int _sde_rm_reserve_qdss(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
				const struct sde_rm_topology_def *top,
				u8 *_qdss_ids)
{
	struct sde_rm_hw_iter iter;
...
static int _sde_rm_reserve_dnsc_blur(struct sde_rm *rm,
				     struct sde_rm_rsvp *rsvp, uint32_t id,
				     enum sde_hw_blk_type type)
{
	struct sde_rm_hw_iter iter;
	...
	while (_sde_rm_get_hw_locked(rm, &iter, true)) {
		struct sde_hw_dnsc_blur *dnsc_blur =
			to_sde_hw_dnsc_blur(iter.blk->hw);
		bool match = false;
		...
		SDE_DEBUG("type %d id %d, dnsc_blur wbs %lu match %d\n", type,
			  id, dnsc_blur->caps->wb_connect, match);
...
static int _sde_rm_reserve_cdm(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
					   ...
					   struct sde_rm_requirements *reqs)
{
	...
	for (i = 0; i < ARRAY_SIZE(hw_res->intfs); i++) {
		if (hw_res->intfs[i] == INTF_MODE_NONE)
			continue;
		id = i + INTF_0;
		ret = _sde_rm_reserve_intf_or_wb(rm, rsvp, id, SDE_HW_BLK_INTF,
						 reqs);
		if (ret)
			return ret;
	}
	...
	for (i = 0; i < ARRAY_SIZE(hw_res->wbs); i++) {
		if (hw_res->wbs[i] == INTF_MODE_NONE)
			continue;
		id = i + WB_0;
		ret = _sde_rm_reserve_intf_or_wb(rm, rsvp, id, SDE_HW_BLK_WB,
						 reqs);
		if (ret)
			return ret;
	}
				 ...
				 struct sde_splash_display *splash_display)
{
	int i;
	u8 *hw_ids = NULL;
	u8 prev_dsc[MAX_DATA_PATH_PER_DSIPLAY] = {
		0,
	};
	...
	if (splash_display) {
		...
		for (i = 0; i < splash_display->dsc_cnt; i++)
			SDE_DEBUG("splash_data.dsc_ids[%d] = %d\n", i,
				  splash_display->dsc_ids[i]);
	}

	/*
   * find if this encoder has previously allocated dsc hw blocks, use same dsc
   * blocks if found to avoid switching dsc encoders during each modeset, as
   * currently we dont have feasible way of decoupling previously owned dsc
   * blocks by resetting respective dsc encoders mux control and flush them from
   * commit path
   */
...
static int
_sde_rm_update_active_only_pipes(struct sde_splash_display *splash_display,
				 u32 active_pipes_mask)
{
	...
	for (i = SSPP_VIG0; i < SSPP_MAX; i++) {
		...
		if (test_bit(i, pipe_info->pipes) ||
		    test_bit(i, pipe_info->virt_pipes))
			continue;
 ...
 */
static int
_sde_rm_get_hw_blk_for_cont_splash(struct sde_rm *rm, struct sde_hw_ctl *ctl,
				   struct sde_splash_display *splash_display)
{
	...
	while (_sde_rm_get_hw_locked(rm, &iter_lm, true)) {
		...
		if (ctl->ops.get_staged_sspp) {
			...
			if (pipes_per_lm ||
				...
				splash_display
					->lm_ids[splash_display->lm_cnt++] =
					iter_lm.blk->id;
				SDE_DEBUG("lm_cnt=%d lm_id %d pipe_cnt%d\n",
					  splash_display->lm_cnt,
					  iter_lm.blk->id - LM_0, pipes_per_lm);
			}
		}
	}
	...
	while (_sde_rm_get_hw_locked(rm, &iter_dsc, true)) {
		if (ctl->ops.read_active_status &&
		    !(ctl->ops.read_active_status(ctl, SDE_HW_BLK_DSC,
						  iter_dsc.blk->id)))
			continue;
...
int sde_rm_cont_splash_res_init(struct msm_drm_private *priv, struct sde_rm *rm,
				struct sde_splash_data *splash_data,
				struct sde_mdss_cfg *cat)
{
	...
	SDE_DEBUG("mixer_count=%d, ctl_count=%d, dsc_count=%d\n",
		  cat->mixer_count, cat->ctl_count, cat->dsc_count);
	...
	while (_sde_rm_get_hw_locked(rm, &iter_c, true) &&
		...
		if (intf_sel) {
			...
			_sde_rm_get_hw_blk_for_cont_splash(rm, ctl,
							   splash_display);
...
static struct drm_connector *_sde_rm_get_connector(struct drm_encoder *enc)
{
...
static int _sde_rm_populate_requirements(struct sde_rm *rm,
					 struct drm_encoder *enc,
					 struct drm_crtc_state *crtc_state,
					 struct drm_connector_state *conn_state,
					 ...
					 struct sde_rm_requirements *reqs)
{
	...
	reqs->top_ctrl = sde_connector_get_property(
		conn_state, CONNECTOR_PROP_TOPOLOGY_CONTROL);
   ...
   */
     ...
     */
		...
		num_lm = sde_crtc_get_num_datapath(
			crtc_state->crtc, conn_state->connector, crtc_state);
	...
	if (RM_RQ_DCWB(reqs)) {
		drm_for_each_encoder_mask(encoder_iter, enc->dev,
			...
			if (drm_encoder_mask(encoder_iter) ==
			    drm_encoder_mask(enc))
				continue;

			conn = _sde_rm_get_connector(encoder_iter);
			if (conn)
				reqs->conn_lm_mask =
					to_sde_connector(conn)->lm_mask;
			break;
		}
	}
	...
	SDE_DEBUG(
		"num_lm: %d num_ctl: %d topology: %d split_display: %d mask: 0x%llX\n",
...
bool sde_rm_topology_is_group(struct sde_rm *rm, struct drm_crtc_state *state,
			      enum sde_rm_topology_group group)
{
	...
	for (i = 0; i < cstate->num_connectors; i++) {
		...
		conn_state =
			drm_atomic_get_new_connector_state(state->state, conn);
		if (!conn_state) {
 ...
 */
static void _sde_rm_release_rsvp(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
				 struct drm_connector *conn)
{
		...
		list_for_each_entry(blk, &rm->hw_blks[type], list) {
			if (blk->rsvp == rsvp) {
				...
				SDE_DEBUG("rel rsvp %d enc %d %d %d\n",
					  rsvp->seq, rsvp->enc_id, blk->type,
					  blk->id);
				_sde_rm_inc_resource_info(rm, &rm->avail_res,
							  blk);
			}
				...
				struct drm_connector_state *conn_state)
{
	...
	for (type = 0; type < SDE_HW_BLK_MAX; type++) {
		list_for_each_entry(blk, &rm->hw_blks[type], list) {
			if (blk->rsvp_nxt &&
			    conn_state->best_encoder->base.id ==
				    blk->rsvp_nxt->enc_id) {
				blk->rsvp = blk->rsvp_nxt;
				blk->rsvp_nxt = NULL;
				_sde_rm_dec_resource_info(rm, &rm->avail_res,
							  blk);
			}
		}
	}
...
int sde_rm_reserve(struct sde_rm *rm, struct drm_encoder *enc,
		   struct drm_crtc_state *crtc_state,
		   struct drm_connector_state *conn_state, bool test_only)
{
	struct sde_rm_rsvp *rsvp_cur, *rsvp_nxt;
	struct sde_rm_requirements reqs = {
		0,
   ...
   */
	if (test_only && rsvp_nxt) {
		...
		if (rsvp_nxt) {
			pr_err("poll timeout cur %d nxt %d enc %d\n",
			       (rsvp_cur) ? rsvp_cur->seq : -1, rsvp_nxt->seq,
			       enc->base.id);
	...
	ret = _sde_rm_populate_requirements(rm, enc, crtc_state, conn_state,
					    sde_kms->catalog, &reqs);
	if (ret) {
   ...
   */
	...
	ret = _sde_rm_make_next_rsvp(rm, enc, crtc_state, conn_state, rsvp_nxt,
				     &reqs);
