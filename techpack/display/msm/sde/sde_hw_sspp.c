...
static inline int _sspp_subblk_offset(struct sde_hw_pipe *ctx, int s_id,
				      u32 *idx)
{
	int rc = 0;
...
static void sde_hw_sspp_update_multirect(struct sde_hw_pipe *ctx, bool enable,
...
static void _sspp_setup_opmode(struct sde_hw_pipe *ctx, u32 mask, u8 en)
{
	u32 idx;
	u32 opmode;
...
static void _sspp_setup_csc10_opmode(struct sde_hw_pipe *ctx, u32 mask, u8 en)
{
	u32 idx;
	u32 opmode;
...
static void
sde_hw_sspp_set_src_split_order(struct sde_hw_pipe *ctx,
...
static void sde_hw_sspp_setup_ubwc(struct sde_hw_pipe *ctx,
				   struct sde_hw_blk_reg_map *c,
				   const struct sde_format *fmt,
				   bool const_alpha_en, bool const_color_en,
				   enum sde_sspp_multirect_index rect_mode)
{
	u32 alpha_en_mask = 0, color_en_mask = 0, ubwc_ctrl_off;

	SDE_REG_WRITE(c, SSPP_FETCH_CONFIG,
		      SDE_FETCH_CONFIG_RESET_VALUE | ctx->mdp->highest_bank_bit
							     << 18);
				     ...
				     enum sde_sspp_multirect_index rect_mode)
{
	...
	src_format |= ((fmt->unpack_count - 1) << 12) |
		      (fmt->unpack_tight << 17) | (fmt->unpack_align_msb << 18);
	...
	if ((flags & SDE_SSPP_ROT_90) &&
	    test_bit(SDE_SSPP_INLINE_CONST_CLR, &ctx->cap->features))
	...
	if (fmt->fetch_mode != SDE_FETCH_LINEAR) {
		...
		sde_hw_sspp_setup_ubwc(ctx, c, fmt, const_alpha_en,
				       const_color_en, rect_mode);
	}
...
static void
sde_hw_sspp_clear_ubwc_error(struct sde_hw_pipe *ctx,
...
static u32
sde_hw_sspp_get_ubwc_error(struct sde_hw_pipe *ctx,
			   enum sde_sspp_multirect_index multirect_index)
{
	struct sde_hw_blk_reg_map *c;
	u32 reg_code;
...
static void
sde_hw_sspp_clear_ubwc_error_v1(struct sde_hw_pipe *ctx,
				enum sde_sspp_multirect_index multirect_index)
{
	struct sde_hw_blk_reg_map *c;
...
static u32
sde_hw_sspp_get_ubwc_error_v1(struct sde_hw_pipe *ctx,
			      enum sde_sspp_multirect_index multirect_index)
{
	struct sde_hw_blk_reg_map *c;
	u32 reg_code;
...
static void
sde_hw_sspp_clear_meta_error(struct sde_hw_pipe *ctx,
			     enum sde_sspp_multirect_index multirect_index)
{
	struct sde_hw_blk_reg_map *c;
...
static u32
sde_hw_sspp_get_meta_error(struct sde_hw_pipe *ctx,
			   enum sde_sspp_multirect_index multirect_index)
{
	struct sde_hw_blk_reg_map *c;
	u32 reg_code;
...
static void
sde_hw_sspp_ubwc_stats_set_roi(struct sde_hw_pipe *ctx,
			       enum sde_sspp_multirect_index multirect_index,
			       struct sde_drm_ubwc_stats_roi *roi)
{
	...
	if (multirect_index == SDE_SSPP_RECT_SOLO ||
	    multirect_index == SDE_SSPP_RECT_0) {
...
static void
sde_hw_sspp_ubwc_stats_get_data(struct sde_hw_pipe *ctx,
				enum sde_sspp_multirect_index multirect_index,
					...
					struct sde_hw_pixel_ext *pe_ext)
{
	...
	for (color = 0; color < SDE_MAX_PLANES; color++) {
		...
		tot_req_pixels[color] = (((pe_ext->roi_h[color] +
					   ...
					   pe_ext->num_ext_pxls_btm[color]) &
					  shortmask)
					 << 16) |
					((pe_ext->roi_w[color] +
					  pe_ext->num_ext_pxls_left[color] +
					  pe_ext->num_ext_pxls_right[color]) &
					 shortmask);
	}
				       ...
				       void *scaler_cfg)
{
	...
	sde_hw_setup_scaler3(&ctx->hw, scaler3_cfg,
			     ctx->cap->sblk->scaler_blk.version, idx,
			     sspp->layout.format, de_lpf_en);
}

static void sde_hw_sspp_setup_pre_downscale(
 ...
 */
static void
_sde_hw_sspp_setup_excl_rect(struct sde_hw_pipe *ctx,
...
static void
sde_hw_sspp_setup_sourceaddress(struct sde_hw_pipe *ctx,
				struct sde_hw_pipe_cfg *cfg,
				enum sde_sspp_multirect_index rect_mode)
{
	int i;
...
static void
sde_hw_sspp_setup_solidfill(struct sde_hw_pipe *ctx, u32 color,
			    enum sde_sspp_multirect_index rect_index)
{
	u32 idx;
				      ...
				      struct sde_hw_pipe_qos_cfg *cfg)
{
	...
	if (ctx->cap &&
	    test_bit(SDE_PERF_SSPP_QOS_8LVL, &ctx->cap->perf_features)) {
				       ...
				       struct sde_hw_pipe_qos_cfg *cfg)
{
	...
	if (cfg->vblank_en) {
		qos_ctrl |= ((cfg->creq_vblank & SSPP_QOS_CTRL_CREQ_VBLANK_MASK)
			     << SSPP_QOS_CTRL_CREQ_VBLANK_OFF);
		qos_ctrl |=
			((cfg->danger_vblank & SSPP_QOS_CTRL_DANGER_VBLANK_MASK)
					 ...
					 enum sde_sspp_multirect_index index)
{
	...
	if ((index == SDE_SSPP_RECT_SOLO || index == SDE_SSPP_RECT_0) &&
	    test_bit(SDE_PERF_SSPP_TS_PREFILL, &cap->perf_features)) {
...
static void
sde_hw_sspp_setup_uidle_fill_scale(struct sde_hw_pipe *ctx,
				       ...
				       bool is_virtual_pipe)
{
	...
	if (is_virtual_pipe) {
		features &= ~(BIT(SDE_SSPP_VIG_IGC) | BIT(SDE_SSPP_VIG_GAMUT));
	...
	if (test_bit(SDE_SSPP_DMA_GC, &features)) {
		if (c->cap->sblk->gc_blk[0].version ==
			...
			ret = reg_dmav1_init_sspp_op_v4(SDE_SSPP_DMA_GC,
							c->idx);
			if (!ret)
				c->ops.setup_dma_gc = reg_dmav1_setup_dma_gcv5;
			else
...
static void
sde_hw_sspp_setup_dgm_inverse_pma(struct sde_hw_pipe *ctx,
				  enum sde_sspp_multirect_index index,
				  u32 enable)
{
	u32 offset;
...
static void
sde_hw_sspp_setup_line_insertion(struct sde_hw_pipe *ctx,
				 enum sde_sspp_multirect_index rect_index,
				 struct sde_hw_pipe_line_insertion_cfg *cfg)
{
	...
	if (cfg->enable)
		ctl_val = BIT(31) | (cfg->dummy_lines << 16) |
			  (cfg->first_active_lines << 8) | (cfg->active_lines);

	SDE_REG_WRITE(c, ctl_off, ctl_val);
	SDE_REG_WRITE(c, size_off, cfg->dst_h << 16);
}

static void _setup_layer_ops(struct sde_hw_pipe *c, unsigned long features,
			     unsigned long perf_features, bool is_virtual_pipe)
{
	int ret;
	...
	if (test_bit(SDE_PERF_SSPP_QOS, &features)) {
		c->ops.setup_qos_lut = sde_hw_sspp_setup_qos_lut;
	...
	if (test_bit(SDE_SSPP_SCALER_QSEED3, &features) ||
		...
		c->ops.setup_scaler_lut =
			is_qseed3_rev_qseed3lite(c->catalog) ?
				reg_dmav1_setup_scaler3lite_lut :
				reg_dmav1_setup_scaler3_lut;
		ret = reg_dmav1_init_sspp_op_v4(
			is_qseed3_rev_qseed3lite(c->catalog) ?
	...
	if (test_bit(SDE_PERF_SSPP_UIDLE, &perf_features)) {
		...
		if (test_bit(SDE_PERF_SSPP_UIDLE_FILL_LVL_SCALE,
			     &perf_features))
			c->ops.setup_uidle_fill_scale =
...
static struct sde_sspp_cfg *_sspp_offset(enum sde_sspp sspp, void __iomem *addr,
...
struct sde_hw_pipe *sde_hw_sspp_init(enum sde_sspp idx, void __iomem *addr,
				     struct sde_mdss_cfg *catalog,
				     bool is_virtual_pipe,
				     struct sde_vbif_clk_client *clk_client)
{
	...
	if (!is_virtual_pipe) {
		sde_dbg_reg_register_dump_range(
			SDE_DBG_NAME, cfg->name, hw_pipe->hw.blk_off,
		...
		if (test_bit(SDE_SSPP_VIG_GAMUT, &hw_pipe->cap->features)) {
			sde_dbg_reg_register_dump_range(
				SDE_DBG_NAME, cfg->sblk->gamut_blk.name,
				hw_pipe->hw.blk_off + cfg->sblk->gamut_blk.base,
				hw_pipe->hw.blk_off +
					cfg->sblk->gamut_blk.base +
					VIG_GAMUT_SIZE,
				hw_pipe->hw.xin_id);
		}
	...
	if (cfg->sblk->scaler_blk.len && !is_virtual_pipe)
		sde_dbg_reg_register_dump_range(
			SDE_DBG_NAME, cfg->sblk->scaler_blk.name,
