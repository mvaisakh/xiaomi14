...
{
	i_vpr_h(inst,
		"%s: width %d, height %d, crop offsets[0] %#x, crop offsets[1] %#x, "
		"bit depth %#x, coded frames %d "
		"fw min count %d, poc %d, color info %d, profile %d, level %d, tier "
		"%d, fg present %d, sb enabled %d max_num_reorder_frames %d\n",
		str,
		...
		subsc_params.color_info, subsc_params.profile,
		subsc_params.level, subsc_params.tier,
...
bool is_valid_hfi_port(struct msm_vidc_inst *inst, u32 port, u32 buffer_type,
		       const char *func)
{
	if (port == HFI_PORT_NONE && buffer_type != HFI_BUFFER_ARP &&
...
bool is_valid_hfi_buffer_type(struct msm_vidc_inst *inst, u32 buffer_type,
			      const char *func)
{
	if (buffer_type != HFI_BUFFER_BITSTREAM &&
	    buffer_type != HFI_BUFFER_RAW &&
	    buffer_type != HFI_BUFFER_METADATA &&
	    buffer_type != HFI_BUFFER_BIN && buffer_type != HFI_BUFFER_ARP &&
	    buffer_type != HFI_BUFFER_COMV &&
	    buffer_type != HFI_BUFFER_NON_COMV &&
	    buffer_type != HFI_BUFFER_LINE && buffer_type != HFI_BUFFER_DPB &&
	    buffer_type != HFI_BUFFER_PERSIST &&
			       ...
			       struct hfi_header *hdr, const char *function)
{
	...
	for (i = 0; i < hdr->num_packets; i++) {
		...
		rc = validate_packet(pkt, core->response_packet,
				     core->packet_size, function);
		if (rc)
			return rc;
...
void fw_coredump(struct msm_vidc_core *core)
{
	...
	data += res_size;
	memcpy(data, (char *)core->iface_q_table.align_virtual_addr,
	       TOTAL_QSIZE);
...
int handle_system_error(struct msm_vidc_core *core, struct hfi_packet *pkt)
{
	bool bug_on = false;
	...
	if (pkt) {
		...
		if (pkt->type == HFI_SYS_ERROR_FATAL) {
			bug_on = !!(msm_vidc_enable_bugon &
				    MSM_VIDC_BUG_ON_FATAL);
		} else if (pkt->type == HFI_SYS_ERROR_NOC) {
			bug_on =
				!!(msm_vidc_enable_bugon & MSM_VIDC_BUG_ON_NOC);
			venus_hfi_noc_error_info(core);
		} else if (pkt->type == HFI_SYS_ERROR_WD_TIMEOUT) {
			bug_on = !!(msm_vidc_enable_bugon &
				    MSM_VIDC_BUG_ON_WD_TIMEOUT);
		}
		if (bug_on) {
				   ...
				   struct msm_vidc_buffer *buf)
{
   ...
   */
	if (!found) {
		...
		print_vidc_buffer(VIDC_LOW, "low ", "ro buf added", inst,
				  ro_buf);
	} else {
		print_vidc_buffer(VIDC_LOW, "low ", "ro buf found", inst,
				  ro_buf);
	}
			       ...
			       struct hfi_buffer *buffer)
{
	...
	if (!found) {
		i_vpr_e(inst,
			"%s: invalid buffer idx %d addr %#llx data_offset %d\n",
	...
	if (msm_vidc_is_super_buffer(inst)) {
		frame_size = call_session_op(core, buffer_size, inst,
					     MSM_VIDC_BUF_INPUT);
		...
		if (buffer->addr_offset / frame_size < batch_size - 1) {
			i_vpr_l(inst,
				"%s: superframe last buffer not reached: %u, %u, %u\n",
				__func__, buffer->addr_offset, frame_size,
				batch_size);
	...
	if (is_ts_reorder_allowed(inst) && inst->hfi_frame_info.no_output) {
		i_vpr_h(inst,
			"%s: received no_output buffer. remove timestamp %lld\n",
				...
				struct hfi_buffer *buffer)
{
	...
	if (is_decode_session(inst)) {
		...
		if (buffer->flags & HFI_BUF_FW_FLAG_PSC_LAST) {
			...
			if (rc)
				msm_vidc_change_state(inst, MSM_VIDC_ERROR,
						      __func__);
		}
	...
	if (!found) {
		i_vpr_l(inst, "%s: invalid idx %d daddr %#llx\n", __func__,
			buffer->index, buffer->base_address);
		return 0;
	}
	...
	if (is_encode_session(inst)) {
		...
		if (inst->hfi_frame_info.overflow) {
			...
			if (is_image_session(inst)) {
				i_vpr_e(inst,
					"%s: overflow detected for an image session\n",
					__func__);
				fatal = true;
			}
			...
			if (!buffer->data_size &&
				...
				i_vpr_e(inst,
					"%s: overflow detected for cbr_cfr session\n",
					__func__);
				fatal = true;
			}
		}
   ...
   */
	if ((buffer->flags & HFI_BUF_FW_FLAG_LAST) ||
		...
		if (buffer->data_size) {
			i_vpr_e(inst,
				"%s: reset data size to zero for last flag buffer\n",
				__func__);
			buf->data_size = 0;
		}
		if (buffer->flags & HFI_BUF_FW_FLAG_READONLY) {
			i_vpr_e(inst,
				"%s: reset RO flag for last flag buffer\n",
				__func__);
	...
	if (is_decode_session(inst)) {
		...
		if (inst->buffers.dpb.size &&
		    buffer->flags & HFI_BUF_FW_FLAG_READONLY) {
			print_vidc_buffer(VIDC_ERR, "err ",
					  "unexpected RO flag", inst, buf);
			msm_vidc_change_state(inst, MSM_VIDC_ERROR, __func__);
		}
					 ...
					 struct hfi_buffer *buffer)
{
	...
	buffers =
		msm_vidc_get_buffers(inst, MSM_VIDC_BUF_OUTPUT_META, __func__);
	if (!buffers)
		return -EINVAL;
	...
	for (i = 0; i < ARRAY_SIZE(buffer_type); i++) {
         ...
         */
				if (buf->attr & MSM_VIDC_ATTR_BUFFER_DONE) {
					...
					rc = msm_vidc_vb2_buffer_done(inst,
								      buf);
					if (rc) {
						print_vidc_buffer(
							VIDC_HIGH, "err ",
							"vb2 done failed", inst,
							buf);
						/* ignore the error */
						rc = 0;
					}
				 ...
				 struct hfi_packet *pkt)
{
	...
	if (pkt->payload_info == HFI_PAYLOAD_NONE) {
		i_vpr_h(inst,
			"%s: received hfi buffer packet without payload\n",
			__func__);
		return 0;
	}
...
static int handle_input_port_settings_change(struct msm_vidc_inst *inst)
{
	...
	if (allow == MSM_VIDC_DISALLOW) {
		...
		if (rc)
			return rc;
		print_psc_properties("INPUT_PSC", inst,
...
static int handle_output_port_settings_change(struct msm_vidc_inst *inst)
{
	...
	print_psc_properties("OUTPUT_PSC", inst,
				    ...
				    struct hfi_packet *pkt)
{
	...
	if (!is_decode_session(inst)) {
		i_vpr_e(inst, "%s: unsupported for non-decode session\n",
			__func__);
		return -EINVAL;
	}
   ...
   */
	for (i = 0; (i + 3) < num_words_in_payload; i = i + 4) {
		i_vpr_l(inst,
			...
			__func__, inst->dpb_list_payload[i],
			inst->dpb_list_payload[i + 1],
			inst->dpb_list_payload[i + 2],
					...
					struct hfi_packet *pkt, u32 port)
{
	...
	case HFI_PROP_MAX_NUM_REORDER_FRAMES:
		inst->subcr_params[port].max_num_reorder_frames =
			payload_ptr[0];
		break;
	...
	case HFI_PROP_SUBFRAME_INPUT:
		if (port != INPUT_PORT) {
			i_vpr_e(inst, "%s: invalid port: %d for property %#x\n",
	...
	case HFI_PROP_AV1_TILE_ROWS_COLUMNS:
		inst->hfi_frame_info.av1_tile_rows_columns = payload_ptr[0];
		break;
	case HFI_PROP_AV1_UNIFORM_TILE_SPACING:
		if (!payload_ptr[0])
			inst->hfi_frame_info.av1_non_uniform_tile_spacing =
				true;
		break;
	...
	for (i = 0; i < ARRAY_SIZE(be); i++) {
		...
		for (j = 0; j < hdr->num_packets; j++) {
			...
			if (in_range(be[i], packet->type)) {
				...
				if (rc)
					msm_vidc_change_state(
						inst, MSM_VIDC_ERROR, __func__);
			}
			pkt += packet->size;
		}
	}
