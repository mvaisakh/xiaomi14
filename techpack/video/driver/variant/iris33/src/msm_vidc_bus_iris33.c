...
{
	u32 mbs_width = (width % lcu_size) ? (width / lcu_size + 1) :
					     (width / lcu_size);
	u32 mbs_height = (height % lcu_size) ? (height / lcu_size + 1) :
					       (height / lcu_size);

	return mbs_width * mbs_height;
}

u32 calculate_number_ubwctiles_pineapple(u32 width, u32 height, u32 tile_w,
					 u32 tile_h)
{
	u32 tiles_width = (width % tile_w) ? (width / tile_w + 1) :
					     (width / tile_w);
	u32 tiles_height = (height % tile_h) ? (height / tile_h + 1) :
					       (height / tile_h);
			    ...
			    struct api_calculation_input codec_input)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		compression_factor->dpb_cf_y =
			dpbopb_ubwc30_cr_table_cratio_pineapple[cr_index_entry]
							       [cr_index_y];
		...
		if ((codec_input.regression_mode == 3) &&
			...
			compression_factor->dpb_cf_y =
				(u32)(codec_input.cr_dpb * 100);
			compression_factor->dpb_cf_cbcr =
				(u32)(codec_input.cr_dpb * 100);
			compression_factor->opb_cf_ycbcr =
				(u32)(codec_input.cr_opb * 100);
		}
	} else { /* encoder */
		/*
     * IPB CR Table Choice; static sheet (if framewidth<3840, use lossless
     * table) (else, use lossy table) stick to this choice for SW purpose (no
     * change for SW)
     */
		if (frame_width < 3840) {
			compression_factor->ipb_cr =
				ipblossless_ubwc30_cr_table_cratio_pineapple
			...
			compression_factor->ipb_cr =
				ipblossy_ubwc30_cr_table_cratio_pineapple
					[cr_index_entry][cr_index_uni];
			compression_factor->ipb_cr_y =
				ipblossy_ubwc30_cr_table_cratio_pineapple
					[cr_index_entry][cr_index_y];
		...
		compression_factor->dpb_cf_y =
			rpb_ubwc30_cr_table_cratio_pineapple[cr_index_entry]
							    [cr_index_y];
		...
		if ((codec_input.regression_mode == 3) &&
			...
			compression_factor->dpb_cf_y =
				(u32)(codec_input.cr_rpb * 100);
			compression_factor->dpb_cf_cbcr =
				(u32)(codec_input.cr_rpb * 100);
			compression_factor->ipb_cr_y =
				(u32)(codec_input.cr_ipb * 100);
		}
	}
	...
	struct api_calculation_bw_output *codec_output)
{
	...
	lcu_per_frame = calculate_number_lcus_pineapple(
		frame_width, frame_height, frame_lcu_size);
	...
	frame420_y_bw_no_ubwc_tile_10bpp =
		((calculate_number_ubwctiles_pineapple(frame_width,
		 ...
		 999) /
		1000;
	frame420_y_bw_linear_10bpp =
		((frame_width * frame_height * codec_input.frame_rate * 2 +
		  999) / 1000 +
		 999) /
		1000;
	...
	decoder_frame_complexity_factor =
		(codec_input.complexity_setting == 0) ?
			400 :
			((codec_input.complexity_setting == 1) ? 266 : 100);

	reconstructed_write_bw_factor_rd =
		(codec_input.complexity_setting == 0) ? 105 : 100;
	...
	if (codec_input.codec == CODEC_AV1) {
		...
		av1_tile_numbers = av1_num_tiles_pineapple[av1tile_index_entry]
							  [av1tile_complexity];
		...
		av1_fe_left_line_buffer_rdwr =
			(((av1d_fe_leftlinebuffer_perlcu_tileboudary *
				   frame_height *
				   (av1_tile_numbers > 1 ? av1_tile_numbers / 2 :
							   0) +
			   999) / 1000 +
			  999) / 1000 +
			 (frame_lcu_size - 1)) /
	...
	large_bw_calculation_fp =
		((target_bitrate * decoder_vsp_read_factor + 7) / 8);
	...
	large_bw_calculation_fp =
		((target_bitrate * bins_to_bits_factor + 7) / 8);
	...
	codec_output->collocated_rd_wr_total_noc =
		(u32)(codec_output->collocated_rd_noc +
		      codec_output->collocated_wr_noc);
	...
	codec_output->dpb_rd_y_ddr =
		(llc_enabled_ref_y_rd) ? ((codec_output->dpb_rd_y_noc * 100 +
					   reference_y_read_bw_factor - 1) /
					  reference_y_read_bw_factor) :
					 codec_output->dpb_rd_y_noc;

	codec_output->dpb_rd_crcb_ddr =
		(llc_enable_ref_crcb_rd) ?
			((codec_output->dpb_rd_crcb_noc * 100 +
			  reference_cbcr_read_bw_factor - 1) /
			 reference_cbcr_read_bw_factor) :
	...
	if (codec_input.linear_opb || codec_input.split_opb) {
		if (codec_input.linear_opb) {
			if (codec_input.bitdepth == CODEC_BITDEPTH_8) {
				large_bw_calculation_fp =
					((frame420_y_bw_linear_8bpp) * 3 / 2 /
					 dpb_to_opb_ratios_ds);

				codec_output->opb_write_total_noc =
					large_bw_calculation_fp;
			} else {
				large_bw_calculation_fp =
					((frame420_y_bw_linear_10bpp) * 3 / 2 /
					 dpb_to_opb_ratios_ds);
			...
			if (codec_input.bitdepth == CODEC_BITDEPTH_8) {
				...
				large_bw_calculation_fp =
					(frame420_y_bw_no_ubwc_tile_10bpp * 3 /
						 2 / dpb_to_opb_ratios_ds *
						 100 +
					 opb_compression_factor_ycbcr - 1) /
					opb_compression_factor_ycbcr;
	...
	if (codec_input.codec == CODEC_AV1) {
		...
		if (llc_enable_probtable_av1d_21pipe) {
			...
			codec_output->statistics_rd_ddr =
				codec_output->statistics_rd_noc -
				av1_probability_table_rdwr_bytesperframe *
					av1_tile_numbers / 1000 / 1000;

			codec_output->statistics_wr_ddr =
				codec_output->statistics_wr_noc -
				av1_probability_table_rdwr_bytesperframe *
					av1_tile_numbers / 1000 / 1000;
		} else {
	...
	struct api_calculation_bw_output *codec_output)
{
	...
	if ((codec_input.codec == CODEC_H264) ||
		...
		frame_lcu_size = 16;
		collocated_bytes_per_lcu =
			16; /* TODO Fixes Uninitialized compilation error. */
	}
	...
	if (codec_input.ipb_yuvrgb == 0) {
		frame420_y_bw_linear_8bpp =
			((calculate_number_ubwctiles_pineapple(
				  frame_width, frame_height, 32, 8) *
				  256 * codec_input.frame_rate +
			  999) / 1000 +
			 999) /
			1000;
	...
	reconstructed_write_bw_factor_rd =
		((en_tile_number - 100) * 2 *
			 ((codec_input.lcu_size + ubwc_tile_w - 1) /
			  ubwc_tile_w) *
			 ubwc_tile_w +
		 (frame_width - 1)) /
			(frame_width) +
	...
	reference_y_read_bw_factor =
		((en_tile_number - 100) * 2 *
			 ((en_search_windows_size_horizontal + ubwc_tile_w - 1) /
			  ubwc_tile_w) *
			 ubwc_tile_w +
		 (frame_width - 1)) /
			frame_width +
	...
	large_bw_calculation_fp =
		(target_bitrate * en_bins_to_bits_factor + 7) / 8;
	codec_output->vsp_read_noc = large_bw_calculation_fp;
	codec_output->vsp_read_ddr = codec_output->vsp_read_noc;
	...
	large_bw_calculation_fp = ((codec_input.bitdepth == CODEC_BITDEPTH_8) ?
					   frame420_y_bw_linear_8bpp :
					   frame420_y_bw_no_ubwc_tile_10bpp) *
				  reference_crcb_read_bw_factor / 2;
   ...
   */
	...
	large_bw_calculation_fp = (large_bw_calculation_fp +
				   reconstructed_write_bw_factor_rd - 1) /
				  reconstructed_write_bw_factor_rd;

	codec_output->dpb_rdwr_duetooverlap_noc = large_bw_calculation_fp;

	codec_output->dpb_rd_y_ddr = (en_llc_enable_ref_rd_y_overlap) ?
					     (codec_output->dpb_rd_y_noc * 100 +
					      reference_y_read_bw_factor -
					      1) / reference_y_read_bw_factor :
					     codec_output->dpb_rd_y_noc;

	codec_output->dpb_rd_crcb_ddr =
		(en_llc_enable_ref_rd_crcb) ?
			(codec_output->dpb_rd_crcb_noc * 100 +
			 reference_crcb_read_bw_factor - 1) /
				reference_crcb_read_bw_factor :
			codec_output->dpb_rd_crcb_noc;

	codec_output->dpb_rdwr_duetooverlap_ddr =
		(en_llc_enable_rec_wr_uncompleted) ?
			0 :
			codec_output->dpb_rdwr_duetooverlap_noc;

	codec_output->dpb_wr_ddr = (en_llc_enable_rec_wr_uncompleted) ?
					   0 :
					   codec_output->dpb_wr_noc;
	...
	if (codec_input.bitdepth == CODEC_BITDEPTH_8) {
		if (codec_input.ipb_yuvrgb == 0) { /* yuv */
			...
			codec_output->ipb_rd_total_noc =
				large_bw_calculation_fp;
			if (codec_input.linear_ipb == 0) {
				if (codec_input.complexity_setting ==
				    0) /* pwc */
					codec_output->ipb_rd_total_noc =
						(large_bw_calculation_fp * 100 +
		...
		if (codec_input.linear_ipb == 1) {
			...
			large_bw_calculation_fp =
				(frame420_y_bw_no_ubwc_tile_10bpp * 300 / 2 +
				 ipb_compression_factor - 1) /
				ipb_compression_factor;
			codec_output->ipb_rd_total_noc =
				large_bw_calculation_fp;
	...
	if (en_rotation_90_270) {
		if (codec_input.codec == CODEC_HEVC) {
			...
			codec_output->ipb_rd_total_noc =
				codec_output->ipb_rd_total_noc * 2;
		}
	}
	...
	codec_output->bse_tlb_rd_noc = ((bse_tlb_byte_per_lcu * lcu_per_frame *
						 codec_input.frame_rate +
					 999) / 1000 +
					999) /
				       1000;
				 ...
				 struct api_calculation_bw_output *codec_output)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		rc = calculate_bandwidth_decoder_iris33(codec_input,
							codec_output);
	} else if (codec_input.decoder_or_encoder == CODEC_ENCODER) {
		rc = calculate_bandwidth_encoder_iris33(codec_input,
							codec_output);
	} else {
