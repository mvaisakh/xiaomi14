...
#define WRAPPER_DEBUG_BRIDGE_LPI_CONTROL_IRIS33 \
	(WRAPPER_BASE_OFFS_IRIS33 + 0x54)
...
#define NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_MAINCTL_LOW \
	(NOC_BASE_OFFS + 0xA008)
#define NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRCLR_LOW (NOC_BASE_OFFS + 0xA018)
#define NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRLOG2_LOW \
	(NOC_BASE_OFFS + 0xA030)
#define NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRLOG3_LOW \
	(NOC_BASE_OFFS + 0xA038)
#define NOC_SIDEBANDMANAGER_MAIN_SIDEBANDMANAGER_FAULTINEN0_LOW \
	(NOC_BASE_OFFS + 0x7040)
...
static int __program_bootup_registers_iris33(struct msm_vidc_core *core)
{
	...
	if (min_dev_reg_addr && dev_reg_size) {
		rc = __write_register(core, HFI_DEVICE_REGION_ADDR_IRIS33,
				      min_dev_reg_addr);
		if (rc)
			return rc;

		rc = __write_register(core, HFI_DEVICE_REGION_SIZE_IRIS33,
				      dev_reg_size);
		if (rc)
			return rc;
	} else {
...
static int __power_off_iris33_hardware(struct msm_vidc_core *core)
{
   ...
   */
	...
	if (value) {
		d_vpr_e("%s: core clock config not enabled, enabling it to read vcodec "
			"registers\n",
			__func__);
		rc = __write_register(core, WRAPPER_CORE_CLOCK_CONFIG_IRIS33,
				      0);
		if (rc)
			return rc;
	}
   ...
   */
	for (i = 0; i < core->capabilities[NUM_VPP_PIPE].value; i++) {
		rc = __read_register_with_poll_timeout(
			core, VCODEC_SS_IDLE_STATUSn + 4 * i, 0x400000,
			0x400000, 2000, 20000);
		if (rc)
	...
	rc = __write_register_masked(core, AON_WRAPPER_MVP_NOC_LPI_CONTROL, 0x1,
				     BIT(0));
	if (rc)
		return rc;

	rc = __read_register_with_poll_timeout(
		core, AON_WRAPPER_MVP_NOC_LPI_STATUS, 0x1, 0x1, 200, 2000);
	if (rc)
...
static int __power_off_iris33_controller(struct msm_vidc_core *core)
{
   ...
   */
	...
	rc = __read_register_with_poll_timeout(
		core, WRAPPER_IRIS_CPU_NOC_LPI_STATUS, 0x1, 0x1, 200, 2000);
	if (rc)
		d_vpr_e("%s: WRAPPER_IRIS_CPU_NOC_LPI_CONTROL failed\n",
			__func__);

	/* Debug bridge LPI release */
	rc = __write_register(core, WRAPPER_DEBUG_BRIDGE_LPI_CONTROL_IRIS33,
			      0x0);
	if (rc)
		return rc;

	rc = __read_register_with_poll_timeout(
		core, WRAPPER_DEBUG_BRIDGE_LPI_STATUS_IRIS33, 0xffffffff, 0x0,
		200, 2000);
	if (rc)
	...
	rc = call_res_op(core, clk_set_flag, core, "video_cc_mvs0c_clk",
   ...
   */
	count = 0;
	do {
		rc = call_res_op(core, reset_control_acquire, core,
				 "video_xo_reset");
		if (!rc) {
			break;
		} else {
	...
	rc = __read_register_with_poll_timeout(core, AON_WRAPPER_SPARE, 0x1,
					       0x0, 1000, 50 * 1000);
	if (rc)
...
static int __power_on_iris33(struct msm_vidc_core *core)
{
	...
	freq = core->power.clk_freq ? core->power.clk_freq : freq_tbl[0].freq;
   ...
   */
	rc = __write_register(
		core, NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRCLR_LOW, 0x1);
	if (rc) {
		d_vpr_e("%s: error clearing NOC_MAIN_ERRORLOGGER_ERRCLR_LOW\n",
			__func__);
		goto fail_program_noc_regs;
	}

	rc = __write_register(
		core, NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_MAINCTL_LOW, 0x3);
	if (rc) {
		d_vpr_e("%s: failed to set NOC_ERL_MAIN_ERRORLOGGER_MAINCTL_LOW\n",
			__func__);
		goto fail_program_noc_regs;
	}
	rc = __write_register(
		core, NOC_SIDEBANDMANAGER_MAIN_SIDEBANDMANAGER_FAULTINEN0_LOW,
		0x1);
	if (rc) {
		d_vpr_e("%s: failed to set NOC_SIDEBANDMANAGER_FAULTINEN0_LOW\n",
			__func__);
		goto fail_program_noc_regs;
	}
...
skip_power_off:
	...
	d_vpr_e("Skip PC, wfi=%#x, idle=%#x, pcr=%#x, ctrl=%#x)\n", wfi_status,
		idle_status, pc_ready, ctrl_status);
	return -EAGAIN;
}
...
static int __raise_interrupt_iris33(struct msm_vidc_core *core)
{
	...
	rc = __write_register(core, CPU_IC_SOFTINT_IRIS33,
			      1 << CPU_IC_SOFTINT_H2A_SHFT_IRIS33);
	if (rc)
		return rc;
...
static int __noc_error_info_iris33(struct msm_vidc_core *core)
{
  ...
  */
   ...
   */
	...
	rc = __read_register(
		core, NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRLOG2_LOW, &value);
	if (!rc)
		d_vpr_e("%s: NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRLOG2_LOW:  %#x\n",
			__func__, value);

	rc = __read_register(
		core, NOC_ERL_ERRORLOGGER_MAIN_ERRORLOGGER_ERRLOG3_LOW, &value);
	if (!rc)
...
static int __clear_interrupt_iris33(struct msm_vidc_core *core)
{
	...
	mask = (WRAPPER_INTR_STATUS_A2H_BMSK_IRIS33 |
		WRAPPER_INTR_STATUS_A2HWD_BMSK_IRIS33 | HFI_CTRL_VCODEC_IDLE);
...
static int __boot_firmware_iris33(struct msm_vidc_core *core)
{
	...
	while (count < max_tries) {
		rc = __read_register(core, HFI_CTRL_STATUS_IRIS33,
				     &ctrl_status);
		if (rc)
			return rc;
...
int msm_vidc_decide_work_mode_iris33(struct msm_vidc_inst *inst)
{
	...
	if (is_decode_session(inst)) {
		...
		if (inst->capabilities[CODED_FRAMES].value ==
		    ...
		    inst->capabilities[LOWLATENCY_MODE].value || res_ok) {
			work_mode = MSM_VIDC_STAGE_1;
		}
		...
		if (res_ok && (inst->capabilities[LOWLATENCY_MODE].value)) {
			work_mode = MSM_VIDC_STAGE_1;
		}
...
exit:
	i_vpr_h(inst,
		"Configuring work mode = %u low latency = %u, gop size = %u\n",
...
int msm_vidc_decide_quality_mode_iris33(struct msm_vidc_inst *inst)
{
	...
	if (!is_realtime_session(inst)) {
		if (((inst->capabilities[COMPLEXITY].flags &
		      CAP_FLAG_CLIENT_SET) &&
		     (inst->capabilities[COMPLEXITY].value >=
		      DEFAULT_COMPLEXITY)) ||
		    mbpf <= max_hq_mbpf) {
			mode = MSM_VIDC_MAX_QUALITY_MODE;
			goto decision_done;
...
int msm_vidc_adjust_bitrate_boost_iris33(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, BITRATE_BOOST, BITRATE_MODE,
				      &rc_type, __func__))
		return -EINVAL;
