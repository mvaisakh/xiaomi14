...
static u64 __calculate(struct msm_vidc_inst *inst,
		       struct vidc_bus_vote_data *d);
static u64 msm_vidc_calc_freq_iris33_legacy(struct msm_vidc_inst *inst,
					    u32 data_size);
...
static int
msm_vidc_init_codec_input_freq(struct msm_vidc_inst *inst, u32 data_size,
			       struct api_calculation_input *codec_input)
{
	...
	if (inst->codec == MSM_VIDC_H264) {
		...
		if (inst->capabilities[ENTROPY_MODE].value ==
			...
			codec_input->entropy_coding_mode =
				CODEC_ENTROPY_CODING_CABAC;
			codec_input->codec = CODEC_H264;
		} else {
			codec_input->entropy_coding_mode =
				CODEC_ENTROPY_CODING_CAVLC;
	...
	codec_input->hierachical_layer = msm_vidc_get_hier_layer_val(inst);
	...
	if (inst->codec == MSM_VIDC_AV1 && codec_input->lcu_size == 128) {
		...
		if (res_is_less_than_or_equal_to(1920, 1088,
						 codec_input->frame_width,
						 codec_input->frame_height)) {
			if (tile_rows_columns <= 2)
				codec_input->av1d_commer_tile_enable = 0;
		} else if (res_is_less_than_or_equal_to(
				   4096, 2172, codec_input->frame_width,
				   codec_input->frame_height)) {
			if (tile_rows_columns <= 4)
				codec_input->av1d_commer_tile_enable = 0;
		} else if (res_is_less_than_or_equal_to(
				   8192, 4320, codec_input->frame_width,
				   codec_input->frame_height)) {
	...
	/* set as sanity mode, this regression mode has no effect on power
   * calculations */
...
static int
msm_vidc_init_codec_input_bus(struct msm_vidc_inst *inst,
			      struct vidc_bus_vote_data *d,
			      struct api_calculation_input *codec_input)
{
   ...
   * complexity_setting and refframe_complexity to be pwc(performance worst
   * case) values. If the motion_vector_complexity is < 2 then set the
   * complexity_setting and refframe_complexity to be average case values.
   */
	...
	if (d->domain == MSM_VIDC_DECODER) {
		...
		codec_input->cr_ipb =
			((Q16_INT(d->input_cr) * 100) + Q16_FRAC(d->input_cr));
	...
	if (msm_vidc_debug & VIDC_BUS) {
		struct dump dump[] = {
			{ "complexity_factor_int", "%d",
			  complexity_factor_int },
			{ "complexity_factor_frac", "%d",
			  complexity_factor_frac },
			{ "refframe_complexity", "%d",
			  codec_input->refframe_complexity },
			{ "complexity_setting", "%d",
			  codec_input->complexity_setting },
			...
			{ "entropy_coding_mode", "%d",
			  codec_input->entropy_coding_mode },
			{ "hierachical_layer", "%d",
			  codec_input->hierachical_layer },
			{ "status_llc_onoff", "%d",
			  codec_input->status_llc_onoff },
			...
			{ "encoder_multiref", "%d",
			  codec_input->encoder_multiref },
			{ "bitrate_mbps", "%d", codec_input->bitrate_mbps },
			{ "lumaonly_decode", "%d",
			  codec_input->lumaonly_decode },
			{ "av1d_commer_tile_enable", "%d",
			  codec_input->av1d_commer_tile_enable },
			{ "regression_mode", "%d",
			  codec_input->regression_mode },
		};
					       ...
					       u64 vpp_min_freq)
{
	...
	closest_freq_upper_corner = core->resource->freq_set.freq_tbl[0].freq;
	...
	for (i = 0; i < core->resource->freq_set.count; i++) {
		if (vpp_min_freq <= core->resource->freq_set.freq_tbl[i].freq) {
	...
	margin_percent =
		div_u64((margin_freq * 100), closest_freq_upper_corner);
...
static u64 msm_vidc_calc_freq_iris33_new(struct msm_vidc_inst *inst,
					 u32 data_size)
{
	u64 freq = 0;
	...
	if (inst->domain == MSM_VIDC_ENCODER) {
		if (!inst->capabilities[ENC_RING_BUFFER_COUNT].value &&
		    is_vpp_cycles_close_to_freq_corner(
			    core, codec_output.vpp_min_freq)) {
			/*
       ...
       */
			codec_output.vpp_min_freq +=
				div_u64(codec_output.vpp_min_freq *
						VPP_MIN_FREQ_MARGIN_PERCENT,
					100);
			codec_output.hw_min_freq =
				max(codec_output.hw_min_freq,
				    codec_output.vpp_min_freq);
		}
	}
	...
	i_vpr_p(inst,
		"%s: filled len %d, required freq %llu, vpp %u, vsp %u, tensilica "
		"%u, hw_freq %u, fps %u, mbpf %u\n",
	...
	if (!is_realtime_session(inst) || inst->codec == MSM_VIDC_AV1 ||
				       ...
				       struct vidc_bus_vote_data *vidc_data)
{
	...
	i_vpr_l(inst, "%s: calc_bw_ddr %lu calc_bw_llcc %lu", __func__,
		vidc_data->calc_bw_ddr, vidc_data->calc_bw_llcc);
   ...
   */
     ...
     */
		...
		if (operating_rate >
			...
			vsp_factor_den = inst->capabilities[FRAME_RATE].value >>
					 16;
		}
         ...
         */
				if (inst->capabilities[SUPER_BLOCK].value)
					vpp_cycles += div_u64(vpp_cycles * 1464,
							      1000);
				else
					vpp_cycles +=
						div_u64(vpp_cycles * 410, 1000);
			} else {
       ...
       */
			...
			input_bitrate_mbps =
				fps * data_size * 8 / (1024 * 1024);
			vsp_hw_min_frequency =
				freq_tbl_value * 1000 * input_bitrate_mbps;

			if (inst->capabilities[STAGE].value ==
			    MSM_VIDC_STAGE_2) {
				vsp_hw_min_frequency +=
					(bitrate_2stage[bitrate_entry] *
						 fw_sw_vsp_offset -
					 1);
				vsp_hw_min_frequency =
					div_u64(vsp_hw_min_frequency,
						(bitrate_2stage[bitrate_entry] *
						 fw_sw_vsp_offset));
				/* VSP fw overhead 1.05 */
				vsp_hw_min_frequency = div_u64(
					vsp_hw_min_frequency *
				...
				vsp_hw_min_frequency +=
					(bitrate_1stage * fw_sw_vsp_offset - 1);
				vsp_hw_min_frequency = div_u64(
					vsp_hw_min_frequency,
			...
			base_cycles =
				inst->has_bframe ?
					80 :
					inst->capabilities[MB_CYCLES_VSP].value;
			...
			if (inst->iframe &&
			    is_hevc_10bit_decode_session(inst)) {
	...
	i_vpr_p(inst,
		"%s: filled len %d, required freq %llu, vpp %llu, vsp %llu, "
		"fw_cycles %llu, fps %u, mbpf %u\n",
		__func__, data_size, freq, vpp_cycles, vsp_cycles, fw_cycles,
		fps, mbpf);
...
static u64 __calculate_decoder(struct vidc_bus_vote_data *d)
{
   ...
   */
	...
	fp_t bins_to_bit_factor, vsp_read_factor, vsp_write_factor, dpb_factor,
		dpb_write_factor, y_bw_no_ubwc_8bpp;
	...
	dpb_opb_scaling_ratio =
		fp_div(FP_INT(d->input_width * d->input_height),
	...
	opb_write_compression_factor =
		opb_compression_enabled ? dpb_write_compression_factor : FP_ONE;
	...
	lcu_per_frame =
		DIV_ROUND_UP(width, lcu_size) * DIV_ROUND_UP(height, lcu_size);
	...
	tnbr_per_lcu = lcu_size == 16 ? 128 : lcu_size == 32 ? 64 : 128;

	/* .... For DDR & LLC  ...... */
	ddr.vsp_read =
		fp_div(fp_mult(FP_INT(bitrate), vsp_read_factor), FP_INT(8));
	ddr.vsp_write =
		fp_div(fp_mult(FP_INT(bitrate), vsp_write_factor), FP_INT(8));

	ddr.collocated_read =
		fp_div(FP_INT(lcu_per_frame * collocated_bytes_per_lcu * fps),
		       FP_INT(bps(1)));
	ddr.collocated_write = ddr.collocated_read;

	y_bw_no_ubwc_8bpp =
		fp_div(FP_INT(width * height * fps), FP_INT(1000 * 1000));

	if (dpb_bpp != 8) {
		y_bw_no_ubwc_10bpp = fp_div(
			fp_mult(y_bw_no_ubwc_8bpp, FP_INT(256)), FP_INT(192));
		y_bw_10bpp_p010 = y_bw_no_ubwc_8bpp * 2;
	}

	ddr.dpb_read = dpb_bpp == 8 ? y_bw_no_ubwc_8bpp : y_bw_no_ubwc_10bpp;
	ddr.dpb_read =
		fp_div(fp_mult(ddr.dpb_read,
	...
	ddr.opb_write = unified_dpb_opb ?
				FP_ZERO :
				(dpb_bpp == 8 ? y_bw_no_ubwc_8bpp :
						(opb_compression_enabled ?
							 y_bw_no_ubwc_10bpp :
							 y_bw_10bpp_p010));
	ddr.opb_write = fp_div(fp_mult(dpb_factor, ddr.opb_write),
			       fp_mult(dpb_opb_scaling_ratio,
				       opb_write_compression_factor));

	ddr.line_buffer_read = fp_div(
		FP_INT(tnbr_per_lcu * lcu_per_frame * fps), FP_INT(bps(1)));
	...
	ddr.total = ddr.vsp_read + ddr.vsp_write + ddr.collocated_read +
		    ddr.collocated_write + ddr.dpb_read + ddr.dpb_write +
		    ddr.opb_read + ddr.opb_write + ddr.line_buffer_read +
		    ddr.line_buffer_write;
	...
	if (msm_vidc_debug & VIDC_BUS) {
		struct dump dump[] = {
			...
			{ "bins to bit factor", DUMP_FP_FMT,
			  bins_to_bit_factor },
			...
			{ "collocated write", DUMP_FP_FMT,
			  ddr.collocated_write },
			{ "line buffer read", DUMP_FP_FMT,
			  ddr.line_buffer_read },
			{ "line buffer write", DUMP_FP_FMT,
			  ddr.line_buffer_write },
			...
			{ "llc line buffer read", DUMP_FP_FMT,
			  llc.line_buffer_read },
			{ "llc line buffer write", DUMP_FP_FMT,
			  llc.line_buffer_write },
...
static u64 __calculate_encoder(struct vidc_bus_vote_data *d)
{
   ...
   */
	...
	bool work_mode_1, original_compression_enabled, low_power,
		cropping_or_scaling, b_frames_enabled = false,
	...
	fp_t dpb_compression_factor, original_compression_factor,
		original_compression_factor_y, y_bw_no_ubwc_8bpp,
		y_bw_no_ubwc_10bpp = 0, y_bw_10bpp_p010 = 0,
		input_compression_factor, downscaling_ratio,
	...
	struct {
		fp_t vsp_read, vsp_write, collocated_read, collocated_write,
			ref_read_y, ref_read_crcb, ref_write, ref_write_overlap,
			orig_read, line_buffer_read, line_buffer_write, total;
	} ddr = { 0 };
   ...
   */
	...
	original_color_format = d->num_formats >= 1 ? d->color_formats[0] :
						      MSM_VIDC_FMT_NV12C;
     ...
     */
		original_compression_factor_y =
			input_compression_factor > FP(1, 8, 100) ?
				fp_div(input_compression_factor,
				       FP(1, 8, 100)) :
	...
	ddr.ref_read_y = dpb_bpp == 8 ? y_bw_no_ubwc_8bpp : y_bw_no_ubwc_10bpp;
	if (b_frames_enabled)
		ddr.ref_read_y = ddr.ref_read_y * 2;
	ddr.ref_read_y = fp_div(ddr.ref_read_y, dpb_compression_factor);

	ddr.ref_read_crcb =
		fp_mult((ddr.ref_read_y / 2), ref_cbcr_read_bw_factor);

	if (width > vertical_tile_width) {
		ddr.ref_read_y = fp_mult(ddr.ref_read_y, ref_y_read_bw_factor);
	}

	if (llc_ref_chroma_cache_enabled) {
		total_ref_read_crcb = ddr.ref_read_crcb;
		ddr.ref_read_crcb =
			fp_div(ddr.ref_read_crcb, ref_cbcr_read_bw_factor);
	...
	if (width > vertical_tile_width) {
		ddr.ref_write_overlap = fp_mult(
			ddr.ref_write, (recon_write_bw_factor - FP_ONE));
	...
	ddr.total = ddr.vsp_read + ddr.vsp_write + ddr.collocated_read +
		    ddr.collocated_write + ddr.ref_read_y + ddr.ref_read_crcb +
		    ddr.ref_write + ddr.ref_write_overlap + ddr.orig_read +
	...
	if (msm_vidc_debug & VIDC_BUS) {
		struct dump dump[] = {
			...
			{ "fps", "%d", fps },
			{ "dpb bitdepth", "%d", dpb_bpp },
			{ "input downscaling ratio", DUMP_FP_FMT,
			  downscaling_ratio },
			{ "rotation", "%d", rotation },
			...
			{ "original frame format", "%#x",
			  original_color_format },
			{ "VPSS preprocessing", "%d",
			  vpss_preprocessing_enabled },
			...
			{ "ref write overlap", DUMP_FP_FMT,
			  ddr.ref_write_overlap },
...
int msm_vidc_ring_buf_count_iris33(struct msm_vidc_inst *inst, u32 data_size)
{
	...
	if (is_vpp_cycles_close_to_freq_corner(core,
					       codec_output.vpp_min_freq)) {
		/* enable ring buffer */
		i_vpr_h(inst, "%s: vpp_min_freq %d, ring_buffer_count %d\n",
			__func__, codec_output.vpp_min_freq,
