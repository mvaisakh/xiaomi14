...
static int __power_off_iris3_hardware(struct msm_vidc_core *core)
{
   ...
   */
	...
	if (value) {
		d_vpr_h("%s: core clock config not enabled, enabling it to read vcodec "
			"registers\n",
			__func__);
   ...
   */
	...
	rc = __read_register_with_poll_timeout(
		core, AON_WRAPPER_MVP_NOC_RESET_ACK, 0x3, 0x3, 200, 2000);
	if (rc)
	...
	rc = __read_register_with_poll_timeout(
		core, AON_WRAPPER_MVP_NOC_RESET_ACK, 0x3, 0x0, 200, 2000);
	if (rc)
...
static int __power_off_iris3_controller(struct msm_vidc_core *core)
{
   ...
   */
	...
	rc = __write_register(core, WRAPPER_DEBUG_BRIDGE_LPI_CONTROL_IRIS3,
			      0x0);
	if (rc)
		return rc;

	rc = __read_register_with_poll_timeout(
		core, WRAPPER_DEBUG_BRIDGE_LPI_STATUS_IRIS3, 0xffffffff, 0x0,
		200, 2000);
	if (rc)
...
static int __prepare_pc_iris3(struct msm_vidc_core *core)
{
	...
	rc = __read_register_with_poll_timeout(core, CTRL_STATUS_IRIS3,
					       ...
					       CTRL_STATUS_PC_READY_IRIS3, 250,
					       2500);
	if (rc) {
...
static int __raise_interrupt_iris3(struct msm_vidc_core *core)
{
	...
	rc = __write_register(core, CPU_IC_SOFTINT_IRIS3,
			      1 << CPU_IC_SOFTINT_H2A_SHFT_IRIS3);
	if (rc)
		return rc;
