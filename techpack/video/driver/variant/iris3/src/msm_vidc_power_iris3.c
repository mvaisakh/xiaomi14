...
static u64 msm_vidc_calc_freq_iris3_legacy(struct msm_vidc_inst *inst,
					   u32 data_size);
			      ...
			      struct api_calculation_input *codec_input)
{
   ...
   */
	...
	codec_input->bitrate_mbps =
		(d->bitrate / 1000000); /* bps 10;	set as 10mbps */
	...
	codec_input->cr_opb = opb_compression_enabled ? codec_input->cr_dpb :
							FP_ONE;
...
static u64 msm_vidc_calc_freq_iris3_new(struct msm_vidc_inst *inst,
					u32 data_size)
{
	u64 freq = 0;
	...
	i_vpr_p(inst,
		"%s: filled len %d, required freq %llu, fps %u, mbpf %u\n",
