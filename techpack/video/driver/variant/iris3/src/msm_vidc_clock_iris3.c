		       ...
		       struct api_calculation_freq_output *codec_output)
{
   ...
   */
	...
	vsp_hw_min_frequency =
		frequency_table_kalama[0][1] * input_bitrate_fp * 1000;
     ...
     */
		...
		if ((codec_input.codec == CODEC_H264) ||
			...
			vsp_hw_min_frequency = DIV_ROUND_UP(
				frequency_table_kalama[0][1], fw_sw_vsp_offset);
		} else if (((codec_input.codec == CODEC_HEVC) &&
			    (codec_input.vsp_vpp_mode ==
			...
			if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
				vsp_hw_min_frequency = DIV_ROUND_UP(
					vsp_hw_min_frequency,
					(bitrate_table_kalama_2stage_fp[codec]
								       [0] *
					 fw_sw_vsp_offset));
			} else {
				vsp_hw_min_frequency = DIV_ROUND_UP(
					vsp_hw_min_frequency,
					(bitrate_table_kalama_1stage_fp[codec]
								       [0] *
					 fw_sw_vsp_offset));
			}
		}
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		...
		if (codec_input.codec == CODEC_AV1) {
			...
			if (pixel_count <= 1920 * 1080)
				pipe_penalty_codec = pipe_penalty_kalama
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) { /* decoder */
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			decoder_vpp_fw_overhead =
				DIV_ROUND_UP((DECODER_VPP_FW_OVERHEAD_KALAMA *
					      10 * codec_input.frame_rate),
					     15);

			decoder_vpp_fw_overhead = DIV_ROUND_UP(
				(decoder_vpp_fw_overhead * 1000),
				(codec_mbspersession_kalama *
				 decoder_vpp_target_clk_per_mb_kalama /
				 codec_input.pipe_num));
       ...
       */
		...
		tensilica_min_frequency =
			(DECODER_SW_OVERHEAD_KALAMA * 10 + 14) / 15;
		tensilica_min_frequency =
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			encoder_vpp_fw_overhead =
				DIV_ROUND_UP((ENCODER_VPP_FW_OVERHEAD_KALAMA *
					      10 * codec_input.frame_rate),
					     15);
		...
		tensilica_min_frequency =
			(ENCODER_SW_OVERHEAD_KALAMA * 10 + 14) / 15;
		tensilica_min_frequency =
			(tensilica_min_frequency + 999) / 1000;
