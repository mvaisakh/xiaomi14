			  ...
			  enum msm_vidc_buffer_type buffer_type)
{
	...
	static const struct msm_vidc_buf_type_handle dec_buf_type_handle[] = {
		...
		{ MSM_VIDC_BUF_PARTIAL_DATA,
		  msm_vidc_decoder_partial_data_size_iris3 },
	};
...
static int
msm_buffer_delivery_mode_based_min_count_iris3(struct msm_vidc_inst *inst,
					       uint32_t count)
{
	struct v4l2_format *f;
			       ...
			       enum msm_vidc_buffer_type buffer_type)
{
	...
	case MSM_VIDC_BUF_OUTPUT_META:
		...
		count = msm_buffer_delivery_mode_based_min_count_iris3(inst,
								       count);
		break;
