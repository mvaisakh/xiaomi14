...
int synx_columns = NAME_COLUMN | ID_COLUMN | STATE_COLUMN | GLOBAL_COLUMN;
EXPORT_SYMBOL(synx_columns);

int synx_debug = SYNX_ERR | SYNX_WARN | SYNX_INFO;
EXPORT_SYMBOL(synx_debug);

void populate_bound_rows(struct synx_coredata *row, char *cur, char *end)
{
	int j;
...
static ssize_t synx_table_read(struct file *file, char *buf, size_t count,
			       loff_t *ppos)
{
	...
	if (synx_columns & ERROR_CODES && !list_empty(&dev->error_list)) {
		...
		list_for_each_entry_safe(err_node, err_node_tmp,
					 &dev->error_list, node) {
			cur += scnprintf(cur, end - cur,
					 "\n\tTime: %s - ID: %d - Code: %d",
					 err_node->timestamp, err_node->h_synx,
					 err_node->error_code);
...
static ssize_t synx_table_write(struct file *file, const char __user *buf,
...
struct dentry *synx_init_debugfs_dir(struct synx_device *dev)
{
	...
	if (!debugfs_create_file("synx_table", 0644, dir, dev,
				 &synx_table_fops)) {
