...
void cam_cci_cmds_dump(struct cci_device *cci_dev, enum cci_i2c_master_t master,
		       enum cci_i2c_queue_t queue)
{
	uint32_t read_val, reg_offset;
	uint32_t start, end, idx;

	start = cci_dev->cci_master_info[master].cci_write_cmds_pos_start;
	end = (cci_dev->cci_master_info[master].cci_write_cmds_pos_current +
	       CCI_I2C_CMDS_SNAPSHOT_MAX_COUNT - 1) %
	      CCI_I2C_CMDS_SNAPSHOT_MAX_COUNT;
	end = (start < end) ? end : (end + CCI_I2C_CMDS_SNAPSHOT_MAX_COUNT);
...
static void cam_cci_cmds_snapshot(struct cci_device *cci_dev, uint32_t cmd,
				  enum cci_i2c_master_t master)
{
	uint32_t *current_write_pos =
		&(cci_dev->cci_master_info[master].cci_write_cmds_pos_current);

	CAM_DBG(CAM_CCI, "current_write_pos = %d, current val = 0x%x",
		*current_write_pos, cmd);

	cci_dev->cci_master_info[master].cci_write_cmds[*current_write_pos] =
		cmd;
	*current_write_pos =
		(*current_write_pos + 1) % CCI_I2C_CMDS_SNAPSHOT_MAX_COUNT;
}
				...
				enum cci_i2c_master_t master)
{
	...
	if (!rc)
		CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d Success: Reset complete",
			cci_dev->soc_info.index, master);
}
				      ...
				      enum cci_i2c_queue_t queue)
{
	...
	read_val = cam_io_r_mb(base + CCI_I2C_M0_Q0_CUR_WORD_CNT_ADDR +
			       reg_offset);
	CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d_CUR_WORD_CNT_ADDR %d len %d max %d",
	...
	if ((read_val + len + 1) >
		...
		uint32_t report_val = CCI_I2C_REPORT_CMD | (1 << 8) | (1 << 9) |
				      (report_id << 4);
		...
		read_val++;
		cci_dev->cci_i2c_queue_info[master][queue].report_id++;
		if (cci_dev->cci_i2c_queue_info[master][queue].report_id ==
		    REPORT_IDSIZE)
			cci_dev->cci_i2c_queue_info[master][queue].report_id =
				0;
		...
		CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d_START_ADDR",
			cci_dev->soc_info.index, master, queue);
		spin_lock_irqsave(
			     ...
			     enum cci_i2c_queue_t queue)
{
	...
	if (!cam_common_wait_for_completion_timeout(
		...
		CAM_ERR(CAM_CCI, "CCI%d_I2C_M%d_Q%d wait timeout, rc: %d",
			cci_dev->soc_info.index, master, queue, rc);
		rc = -ETIMEDOUT;
		...
		cam_cci_flush_queue(cci_dev, master);
		CAM_INFO(CAM_CCI, "CCI%d_I2C_M%d_Q%d dump register after reset",
			 cci_dev->soc_info.index, master, queue);
		cam_cci_dump_registers(cci_dev, master, queue);
		return rc;
	}
				   ...
				   enum cci_i2c_queue_t queue)
{
	...
	CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d ENTER", cci_dev->soc_info.index,
		master, queue);

	spin_lock_irqsave(&cci_dev->cci_master_info[master].lock_q[queue],
			  flags);
	if (atomic_read(&cci_dev->cci_master_info[master].q_free[queue]) == 0) {
		CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d is free",
			cci_dev->soc_info.index, master, queue);
		cam_cci_load_report_cmd(cci_dev, master, queue);
		atomic_set(&cci_dev->cci_master_info[master].q_free[queue], 1);
		cam_io_w_mb(reg_val, base + CCI_QUEUE_START_ADDR);
				      ...
				      enum cci_i2c_queue_t queue)
{
	...
	if (atomic_read(&cci_dev->cci_master_info[master].q_free[queue]) == 1) {
		...
		spin_unlock_irqrestore(
			&cci_dev->cci_master_info[master].lock_q[queue], flags);
		CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d is set to 1",
			cci_dev->soc_info.index, master, queue);
		rc = cam_cci_wait(cci_dev, master, queue);
		if (rc < 0) {
			CAM_ERR(CAM_CCI,
	...
	CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d EXIT", cci_dev->soc_info.index,
		master, queue);
				    ...
				    uint32_t *pack)
{
	...
	*pack = 0;
	...
	if (len > cci_dev->payload_size) {
		CAM_ERR(CAM_CCI, "Len error: len: %u expected_len: %u", len,
			cci_dev->payload_size);
		return -EINVAL;
	}
				     ...
				     struct cam_cci_ctrl *c_ctrl)
{
   ...
   */
	...
	if (i2c_freq_mode == cci_dev->i2c_freq_mode[master]) {
		CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d, curr_freq: %d",
			cci_dev->soc_info.index, master, i2c_freq_mode);
	...
	CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d, curr_freq: %d, req_freq: %d",
		cci_dev->soc_info.index, master, cci_dev->i2c_freq_mode[master],
		i2c_freq_mode);
	down(&cci_master->master_sem);
     ...
     */
		...
		if (en_seq_write == 0) {
			for (j = 0; j < i2c_msg->addr_type; j++) {
				write_data[i2c_msg->addr_type - j] =
					(reg_addr >> (j * 8)) & 0xFF;
				i++;
			}
		}
				  ...
				  struct cam_cci_ctrl *c_ctrl)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_CCI,
			"CCI%d_I2C_M%d_Q%d cam_cci_set_clk_param failed rc: %d",
			cci_dev->soc_info.index, master, queue, rc);
		return rc;
	}
   ...
   */
	...
	if (c_ctrl->cci_info->retries > CCI_I2C_READ_MAX_RETRIES) {
		CAM_ERR(CAM_CCI,
			"CCI%d_I2C_M%d_Q%d Invalid read retries info retries from slave: "
			"%d, max retries: %d",
			cci_dev->soc_info.index, master, queue,
	...
	if (read_cfg->addr_type >= CAMERA_SENSOR_I2C_TYPE_MAX) {
		CAM_ERR(CAM_CCI,
			"CCI%d_I2C_M%d_Q%d failed : Invalid addr type: %u",
			cci_dev->soc_info.index, master, queue,
			read_cfg->addr_type);
		rc = -EINVAL;
		goto rel_mutex_q;
	...
	while (total_read_words != exp_words) {
		...
		if (!rem_jiffies) {
			...
			val = cam_io_r_mb(base +
					  ...
					  master * 0x100);
			CAM_ERR(CAM_CCI,
				"CCI%d_I2C_M%d_Q%d wait timeout for th_complete, FIFO "
				"buf_lvl:0x%x, rc: %d",
				cci_dev->soc_info.index, master, queue, val,
				rc);
			/* xiaomi add for cci cmds dump start */
...
read_again:
		...
		CAM_DBG(CAM_CCI,
			...
			cci_dev->soc_info.index, master, queue, j,
			total_read_words);
...
enable_irq:
       ...
       */
			...
			if (!rem_jiffies) {
				...
				val = cam_io_r_mb(
					...
					master * 0x100);
				CAM_ERR(CAM_CCI,
					"CCI%d_I2C_M%d_Q%d wait timeout for RD_DONE irq for rc = %d "
					"FIFO buf_lvl:0x%x, rc: %d",
					cci_dev->soc_info.index, master, queue,
					val, rc);
				/* xiaomi add for cci cmds dump start */
				cam_cci_cmds_dump(cci_dev, master, queue);
	...
	CAM_DBG(CAM_CCI,
		"CCI%d_I2C_M%d_Q%d Burst read successful words_read %d",
...
static int32_t cam_cci_read(struct v4l2_subdev *sd, struct cam_cci_ctrl *c_ctrl)
{
   ...
   */
	...
	read_data_byte[0] = CCI_I2C_WRITE_DISABLE_P_CMD |
			    (read_cfg->addr_type << 4);
	for (i = 0; i < read_cfg->addr_type; i++) {
		read_data_byte[read_cfg->addr_type - i] =
			(read_cfg->addr >> (i * 8)) & 0xFF;
	}
	...
	if (read_words != exp_words) {
		CAM_ERR(CAM_CCI,
			"CCI%d_I2C_M%d_Q%d read_words: %d, exp words: %d",
			cci_dev->soc_info.index, master, queue, read_words,
			exp_words);
	...
	while (read_words > 0) {
		...
		for (i = 0; (i < 4) && (index < read_cfg->num_byte); i++) {
			CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d_Q%d i: %d, index: %d",
				cci_dev->soc_info.index, master, queue, i,
				index);
			if (!first_byte) {
...
static void cam_cci_write_async_helper(struct work_struct *work)
{
	...
	if (rc < 0)
		CAM_ERR(CAM_CCI, "CCI%d_I2C_M%d_Q%d Failed rc: %d",
			cci_dev->soc_info.index, master, write_async->queue,
			rc);
				       ...
				       enum cci_i2c_sync sync_en)
{
	...
	if (!write_async) {
		CAM_ERR(CAM_CCI,
			"CCI%d_I2C_M%d_Q%d Memory allocation failed for write_async",
			cci_dev->soc_info.index,
			c_ctrl->cci_info->cci_i2c_master, queue);
		return -ENOMEM;
	}
	...
	if (!cci_i2c_write_cfg_w->reg_setting) {
		CAM_ERR(CAM_CCI,
			"CCI%d_I2C_M%d_Q%d Couldn't allocate memory for reg_setting",
			cci_dev->soc_info.index,
			c_ctrl->cci_info->cci_i2c_master, queue);
		kfree(write_async);
		return -ENOMEM;
			     ...
			     struct cam_cci_ctrl *c_ctrl)
{
	...
	CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d ctrl_cmd = %d", cci_dev->soc_info.index,
		master, c_ctrl->cmd);
...
int32_t cam_cci_core_cfg(struct v4l2_subdev *sd, struct cam_cci_ctrl *cci_ctrl)
{
	...
	CAM_DBG(CAM_CCI, "CCI%d_I2C_M%d cmd = %d", cci_dev->soc_info.index,
		master, cci_ctrl->cmd);
	...
	case MSM_CCI_I2C_WRITE_SYNC_BLOCK:
		...
		if (rc < 0) {
			CAM_ERR(CAM_CCI,
				"cam cci rc %d slav 0x%x on dev/master %d/%d",
