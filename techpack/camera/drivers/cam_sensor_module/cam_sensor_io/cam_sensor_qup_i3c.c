...
static int cam_qup_i3c_rxdata(struct i3c_device *dev_client,
			      ...
			      int data_length)
{
	...
	rc = i3c_device_do_priv_xfers(dev_client, read_buf,
				      ARRAY_SIZE(read_buf));
	if (rc == -ENOTCONN) {
		...
		if (rc)
			CAM_ERR(CAM_SENSOR,
				"Retry Failed i3c_read: rc = %d, us = %d", rc,
				us);
	} else if (rc)
...
static int cam_qup_i3c_txdata(struct camera_io_master *dev_client,
			      unsigned char *txdata, int length)
{
	int rc;
	uint32_t us = 0;
	...
	if (rc == -ENOTCONN) {
		while (us < CAM_I3C_DEV_PROBE_TIMEOUT_US) {
			usleep_range(1000, 1005);
			rc = i3c_device_do_priv_xfers(dev_client->i3c_client,
						      &write_buf, 1);
			if (rc != -ENOTCONN)
				break;
			us += 1000;
		...
		if (rc)
			CAM_ERR(CAM_SENSOR,
				"Retry Failed i3c_write: rc = %d, us = %d", rc,
				us);
	} else if (rc)
...
int cam_qup_i3c_read_seq(struct i3c_device *client, uint32_t addr,
...
static int cam_qup_i3c_compare(struct i3c_device *client, uint32_t addr,
			       ...
			       enum camera_sensor_i2c_type addr_type)
{
	...
	rc = cam_qup_i3c_read(client, addr, &reg_data, addr_type, data_type);
	if (rc < 0)
		return rc;
...
int cam_qup_i3c_poll(struct i3c_device *client, uint32_t addr, uint16_t data,
		     uint16_t data_mask, enum camera_sensor_i2c_type addr_type,
		     enum camera_sensor_i2c_type data_type, uint32_t delay_ms)
{
	int rc;
	int i;
	...
	for (i = 0; i < delay_ms; i++) {
		rc = cam_qup_i3c_compare(client, addr, data, data_mask,
					 data_type, addr_type);
...
cam_qup_i3c_write_optimized(struct camera_io_master *client,
			    struct cam_sensor_i2c_reg_setting *write_setting,
			    struct i3c_priv_xfer *msgs, unsigned char *buf)
{
	int32_t rc = 0;
	uint8_t len = 0;
	...
	while (i < write_setting->size) {
		...
		do {
			...
			if (data_type == CAMERA_SENSOR_I2C_TYPE_BYTE) {
				...
				CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len,
					buf[offset]);
				offset += 1;
				len += 1;
			} else if (data_type == CAMERA_SENSOR_I2C_TYPE_WORD) {
			...
			if (i < write_setting->size) {
				reg_setting++;
				isLookAhead = ((reg_setting_previous->reg_addr +
						1) == reg_setting->reg_addr) ?
						      true :
						      false;
			} else {
				break;
		...
		CAM_DBG(CAM_SENSOR, "offset: %d len: %d curr_mindx: %d", offset,
			len, curr_mindx);
		cam_qup_i3c_txdata_fill(client, &buf[offset - len], len, msgs,
					curr_mindx);
			    ...
			    struct cam_sensor_i2c_reg_setting *write_setting)
{
	...
	msgs = kcalloc(write_setting->size, sizeof(struct i3c_priv_xfer),
		       GFP_KERNEL);
	if (!msgs) {
	...
	buf = kzalloc(write_setting->size * I3C_REG_MAX_BUF_SIZE,
		      GFP_KERNEL | GFP_DMA);
	if (!buf) {
	...
	i3c_msg_size =
		cam_qup_i3c_write_optimized(client, write_setting, msgs, buf);
	if (i3c_msg_size < 0) {
		rc = i3c_msg_size;
		goto deallocate_buffer;
	}
	...
	if (rc == -ENOTCONN) {
		while (us < CAM_I3C_DEV_PROBE_TIMEOUT_US) {
			usleep_range(1000, 1005);
			rc = i3c_device_do_priv_xfers(client->i3c_client, msgs,
						      write_setting->size);
			if (rc != -ENOTCONN)
				break;
			us += 1000;
...
static int
cam_qup_i3c_write_burst(struct camera_io_master *client,
			struct cam_sensor_i2c_reg_setting *write_setting)
{
	int i;
	int rc;
