...
int32_t cam_cci_i2c_read(struct cam_sensor_cci_client *cci_client,
			 ...
			 enum camera_sensor_i2c_type data_type, bool is_probing)
{
	int32_t rc = -EINVAL;
...
int32_t cam_cci_i2c_read_with_little_endian(
	struct cam_sensor_cci_client *cci_client, uint32_t addr, uint32_t *data,
	enum camera_sensor_i2c_type addr_type,
	enum camera_sensor_i2c_type data_type, bool is_probing)
{
	int32_t rc = -EINVAL;
...
static int32_t
cam_cci_i2c_compare_with_32(struct cam_sensor_cci_client *client, uint32_t addr,
				 ...
				 uint32_t delay_ms)
{
	...
	for (i = 0; i < delay_ms; i++) {
		rc = cam_cci_i2c_compare_with_32(client, addr, data, data_mask,
						 data_type, addr_type);
		if (!rc)
			return rc;
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"[S10]cam_cci_i2c_compare_with_32 rc = %d", rc);
			break;
		}
	...
	/* If rc is 1 then read is successful but poll is failure */
	if (rc == 1)
		CAM_ERR(CAM_OIS, "[S10]poll_with_32 failed rc=%d(non-fatal)",
			rc);
