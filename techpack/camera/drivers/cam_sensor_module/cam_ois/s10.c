...
static int32_t convert_data_endian(uint32_t *reg_data)
{
	...
	temp = (*reg_data);
	(*reg_data) = ((((temp) & 0xff) << 24) | (((temp) & 0xff00) << 8) |
		       (((temp) & 0xff0000) >> 8) | (((temp) >> 24) & 0xff));

	return rc;
}

static int32_t
write_s10_one_register(struct camera_io_master *io_master_info,
		       struct cam_sensor_i2c_reg_setting *write_setting,
		       uint32_t replace_data)
{
	int32_t rc = 0;
	...
	if (0 == rc) {
		...
		if (rc) {
			CAM_ERR(CAM_OIS,
				"[S10] write_s10_one_register fail rc = %d",
				rc);
		}
	} else {
	...
	CAM_DBG(CAM_OIS,
		"[S10] address 0x%x, data 0x%x, replace_data 0x%x, delay %d",
...
static bool cal_fw_checksum(struct cam_ois_ctrl_t *o_ctrl, uint16_t *fw_data,
			    uint32_t fw_size)
{
	...
	// set checksum
	write_s10_one_register(&(o_ctrl->io_master_info), &write_CHECKSUM_DATA,
			       (uint32_t)checksum);

	// request check checksum
	SETTING_CHECKSUM_CTRL[0].data_mask = 0;
	SETTING_CHECKSUM_CTRL[0].delay = 0;
	write_s10_one_register(&(o_ctrl->io_master_info), &write_CHECKSUM_CTRL,
			       0x00000001);
	...
	rc = cam_cci_i2c_poll_with_32(ois_cci_client,
				      SETTING_CHECKSUM_CTRL[0].reg_addr,
				      SETTING_CHECKSUM_CTRL[0].reg_data,
				      SETTING_CHECKSUM_CTRL[0].data_mask,
				      CAMERA_SENSOR_I2C_TYPE_DWORD,
				      CAMERA_SENSOR_I2C_TYPE_WORD,
				      SETTING_CHECKSUM_CTRL[0].delay);
	if (rc == 0) {
	...
	// Check the checksum
	rc = cam_cci_i2c_poll_with_32(ois_cci_client, SETTING_OIS_ERR.reg_addr,
				      SETTING_OIS_ERR.reg_data,
				      SETTING_OIS_ERR.data_mask,
				      CAMERA_SENSOR_I2C_TYPE_DWORD,
				      CAMERA_SENSOR_I2C_TYPE_WORD,
				      SETTING_OIS_ERR.delay);
	if (rc == 0) {
...
static int32_t apply_fw_setting(struct cam_ois_ctrl_t *o_ctrl,
				uint16_t *fw_data, uint32_t fw_size)
{
	struct cam_sensor_cci_client *ois_cci_client = NULL;
	int32_t rc = 0;
	...
	// execute NOP
	write_s10_one_register(&(o_ctrl->io_master_info), &write_FLCCMD,
			       0x00000000);
	// set boot address
	write_s10_one_register(&(o_ctrl->io_master_info), &write_FLCBTADR,
			       0x000000C1);
	// restart cpu
	write_s10_one_register(&(o_ctrl->io_master_info), &write_CPUSTP,
			       0x00000000);

	usleep_range(10000, 10010);
	// poll ois status is IDLE
	rc = cam_cci_i2c_poll_with_32(ois_cci_client, SETTING_OIS_STS.reg_addr,
				      SETTING_OIS_STS.reg_data,
				      SETTING_OIS_STS.data_mask,
				      CAMERA_SENSOR_I2C_TYPE_DWORD,
				      CAMERA_SENSOR_I2C_TYPE_WORD,
				      SETTING_OIS_STS.delay);
	if (rc == 0) {
	...
	if ((0 == rc) && (result_checksum)) {
		CAM_INFO(CAM_OIS,
			 "[S10]apply kernel firmware setting success.");
	} else {
		CAM_ERR(CAM_OIS, "[S10]apply kernel firmware setting failed.");
	}

	// set update part data
	write_s10_one_register(&(o_ctrl->io_master_info),
			       &write_UPDATEPART_DATA, 0x00000041);
					...
					struct i2c_settings_array *i2c_set)
{
	...
	struct cam_sensor_cci_client *ois_cci_client =
	...
	cam_cci_i2c_read_with_little_endian(
		ois_cci_client, SETTING_GYRO_ORIENT.reg_addr,
		&(SETTING_GYRO_ORIENT.reg_data), CAMERA_SENSOR_I2C_TYPE_WORD,
		CAMERA_SENSOR_I2C_TYPE_DWORD, FALSE);

	CAM_INFO(CAM_OIS,
		 "[S10] GYRO_ORIENT is %d flash gyrogainx 0x%x, gyrogainy 0x%x",
		 (SETTING_GYRO_ORIENT.reg_data & SETTING_GYRO_ORIENT.data_mask),
		 gyrogainx, gyrogainy);
	...
	list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
		...
		case CAM_SENSOR_I2C_WRITE_RANDOM: {
			for (j = 0; j < i2c_list->i2c_settings.size; j++) {
				if (i2c_list->i2c_settings.reg_setting[j]
					    .reg_addr == GYRO_X) {
					buff[i] = (i2c_list->i2c_settings
							   .reg_setting[j]
							   .reg_data);
					i++;
				} else if (i2c_list->i2c_settings.reg_setting[j]
						   .reg_addr == GYRO_Y) {
					buff[i] = (i2c_list->i2c_settings
							   .reg_setting[j]
							   .reg_data);
	...
	write_s10_one_register(&(o_ctrl->io_master_info), &write_GYROX_CTRL,
			       buff[0]);
	write_s10_one_register(&(o_ctrl->io_master_info), &write_GYROY_CTRL,
			       buff[1]);
	if ((buff[2] != gyrogainx) || (buff[3] != gyrogainy)) {
		write_s10_one_register(&(o_ctrl->io_master_info),
				       &write_GYROGAINX_CTRL, buff[2]);
		write_s10_one_register(&(o_ctrl->io_master_info),
				       &write_GYROGAINY_CTRL, buff[3]);
		CAM_INFO(CAM_OIS,
			 "[S10] using eeprom gyro gainx=0x%x gyroy=0x%x",
			 SETTING_GYROGAINX[0].reg_data,
			 SETTING_GYROGAINY[0].reg_data);
	}

	CAM_INFO(CAM_OIS, "[S10] reg_data x=0x%x y=0x%x",
		 SETTING_GYROX_CTRL[0].reg_data,
		 SETTING_GYROY_CTRL[0].reg_data);

	if (rc < 0) {
		CAM_ERR(CAM_OIS, "[S10] Failed in Applying i2c wrt settings");
	}
...
static int32_t
write_firmware_to_flcdata(struct cam_ois_ctrl_t *o_ctrl,
			  struct cam_sensor_i2c_reg_setting *i2c_reg_setting)
{
	...
	// poll flash busy
	rc1 = cam_cci_i2c_poll_with_32(
		ois_cci_client, SETTING_FLCST.reg_addr, SETTING_FLCST.reg_data,
	...
	list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
			...
			for (j = 0; j < i2c_list->i2c_settings.size; j++) {
				convert_data_endian(
					&(i2c_list->i2c_settings.reg_setting[j]
						  .reg_data));
				CAM_DBG(CAM_OIS,
					"[S10] ois name  %s, request id %d, j=[%d], reg_addr 0x%x, "
					"0x%x delay %d",
		...
		if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
			...
			for (i = 0; i < size; i++) {
				...
				if (rc < 0) {
					CAM_ERR(CAM_OIS,
						"[S10] i2c poll apply setting Fail read_addr 0x%x",
						i2c_list->i2c_settings
							.reg_setting[i]
							.reg_addr);
					return rc;
					...
					rc = 0;
					CAM_ERR(CAM_OIS,
						"i2c poll mismatch read_addr 0x%x",
						i2c_list->i2c_settings
							.reg_setting[i]
							.reg_addr);
				} else {
...
static int32_t cam_s10_read_flash(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	for (addr = START_DUMP_ADDR; addr < END_DUMP_ADDR;
	     addr = addr + DL_BYTES) {
		rc = cam_cci_i2c_read_with_little_endian(
			ois_cci_client, addr, &data,
			CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_DWORD, FALSE);
		if (rc) {
			CAM_WARN(CAM_OIS, "[S10] cam_s10_read_flash failed");
			return rc;
		}

		if (((THERMISTOR == addr) || (RESISTANCE0 == addr) ||
		     (RESISTANCE2 == addr)) &&
		    (REG_ZERO == data)) {
			CAM_ERR(CAM_OIS,
				"[S10]Thermistor or resistance error addr:0x%04x data: 0x%08x",
				addr, data);
			NG_judge = true;
		}
		...
		if ((WIRESTATUS == addr) && (REG_ZERO != data)) {
			CAM_ERR(CAM_OIS,
				"[S10]OpenShort error addr:0x%04x data: 0x%08x",
				addr, data);
			NG_judge = true;
		}
...
static int32_t cam_s10_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	rc = cam_cci_i2c_read_with_little_endian(
		ois_cci_client, SETTING_FW_REV.reg_addr, &curr_fw_version,
		CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_DWORD,
		TRUE);
	CAM_GET_TIMESTAMP(ts2);
	CAM_GET_TIMESTAMP_DIFF_IN_MICRO(ts1, ts2, microsec1);

	if ((false == result_checksum) ||
	    (FIRMWARE_FLASH_DUMP_SWITCH == s10_ois_fw_check)) {
		rc = request_firmware(&fw, fw_name_prog, dev);
		if (rc) {
			CAM_ERR(CAM_OIS, "[S10]Failed to locate %s",
				fw_name_prog);
			release_firmware(fw);
			return rc;
		}
		result_checksum = cal_fw_checksum(o_ctrl, (uint16_t *)fw->data,
						  (uint32_t)fw->size);
		CAM_INFO(CAM_OIS,
			 "[S10] need do cal_fw_checksum, result_checksum "
			 "%d,s10_ois_fw_check %d ",
			 result_checksum, s10_ois_fw_check);
		// set ois on
		write_s10_one_register(&(o_ctrl->io_master_info), &write_CTRL,
				       0x00000001);

		if (cam_s10_read_flash(o_ctrl)) {
			CAM_WARN(CAM_OIS, "[S10] read flash failed! rc:%d", rc);
		} else {
	...
	CAM_DBG(CAM_OIS, "[S10]microsec %d -- %d -- %d", microsec0, microsec1,
		microsec2);

	CAM_INFO(
		CAM_OIS,
		"[S10]read FW_ver ID:0x%x, current FW_ves ID:0x%x, result_checksum %d",
		curr_fw_version, o_ctrl->opcode.fw_version, result_checksum);

	if ((((curr_fw_version != o_ctrl->opcode.fw_version) &&
	      (false == result_checksum)) &&
		...
		if (o_ctrl->i2c_init_data.is_settings_valid == 1) {
			// poll ois status is IDLE
			rc = cam_cci_i2c_poll_with_32(
				ois_cci_client, SETTING_OIS_STS.reg_addr,
				SETTING_OIS_STS.reg_data,
				SETTING_OIS_STS.data_mask,
				CAMERA_SENSOR_I2C_TYPE_DWORD,
				CAMERA_SENSOR_I2C_TYPE_WORD,
			...
			if (rc == 0) {
				...
				CAM_ERR(CAM_OIS,
					"[S10] poll 0x5010 failed rc = %d", rc);
			}
			// apply init settings
			...
			if (rc == -EAGAIN) {
				...
				rc = s10_ois_apply_settings(
					o_ctrl, &o_ctrl->i2c_init_data);
			}
			if (rc) {
				CAM_ERR(CAM_OIS,
					"[S10] Cannot apply i2c_init_data data %d flag %d",
					rc, o_ctrl->opcode.customized_ois_flag);
				return rc;
			} else {
				CAM_INFO(
					CAM_OIS,
					"[S10] apply i2c_init_data data success");
			}
		}
		...
		fw_size = (sizeof(struct cam_sensor_i2c_reg_array) *
			   each_download_size);
		vaddr = vmalloc(fw_size);
		if (!vaddr) {
			CAM_ERR(CAM_OIS,
				"[S10]Failed in allocating i2c_array: fw_size: %u",
				fw_size);
			release_firmware(fw);
			return -ENOMEM;
		}
		...
		for (i = 0, ptr = (uint8_t *)fw->data; i < total_dwords;) {
			for (cnt = 0;
			     cnt < each_download_size && i < total_dwords;
			     cnt++, i++, ptr += 4) {
				...
				CAM_DBG(CAM_OIS,
					"[S10]addr:0x%04x data: 0x%02x 0x%02x 0x%02x 0x%02x reg_data 0x%x",
					i2c_reg_setting.reg_setting[cnt]
						.reg_addr,
			...
			rc = write_firmware_to_flcdata(o_ctrl,
						       &i2c_reg_setting);
			if (rc < 0) {
				CAM_ERR(CAM_OIS,
					"[S10]OIS FW(prog) size(%d) download failed. %d",
					total_dwords, rc);
				goto release_firmware;
			} else {
				CAM_DBG(CAM_OIS,
					"[S10] write_firmware_to_flcdata success rc = %d",
					rc);
			}
		}

		/* 4. Check whether the download is successful*/
		apply_fw_setting(o_ctrl, (uint16_t *)fw->data,
				 (uint32_t)fw->size);
release_firmware:
		vfree(vaddr);
		vaddr = NULL;
		...
		// read fw version
		cam_cci_i2c_read_with_little_endian(
			ois_cci_client, SETTING_FW_REV.reg_addr,
			&curr_fw_version, CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_DWORD, FALSE);

		CAM_INFO(CAM_OIS,
			 "[S10]after DL, read program ID:0x%x, current ID:0x%x",
...
int s10_ois_pkt_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	for (i = 0; i < FIRMWARE_UPDATE_RETRY_TIMES; i++) {
		...
		if (rc) {
			CAM_WARN(CAM_OIS, "[S10] fw update failed retry:%d/%d",
				 i, FIRMWARE_UPDATE_RETRY_TIMES);
		} else {
	...
	if (rc) {
		...
		if (o_ctrl->is_ois_calib) {
			rc = s10_ois_apply_calib_settings(
			...
			if ((rc == -EAGAIN) &&
				...
				CAM_WARN(
					CAM_OIS,
					"[S10] CCI HW is restting: Reapplying calib settings");
				usleep_range(1000, 1010);
				rc = s10_ois_apply_calib_settings(
					o_ctrl, &o_ctrl->i2c_calib_data);
			}
			if (rc) {
				CAM_ERR(CAM_OIS,
					"[S10] Cannot apply calib data %d flag %d",
					rc, o_ctrl->opcode.customized_ois_flag);
				goto pwr_dwn;
			} else {
				CAM_INFO(
					CAM_OIS,
					"[S10] apply calib data settings success");
			}
		}
		...
		if (o_ctrl->i2c_fwinit_data.is_settings_valid == 1) {
			...
			if (rc == -EAGAIN) {
				...
				usleep_range(1000, 1010);
				rc = s10_ois_apply_settings(
					o_ctrl, &o_ctrl->i2c_fwinit_data);
			}
			if (rc) {
				CAM_ERR(CAM_OIS,
					"Cannot apply i2c_fwinit_data data %d, flag %d",
					rc, o_ctrl->opcode.customized_ois_flag);
				goto pwr_dwn;
			} else {
				CAM_INFO(CAM_OIS,
					 "[S10] apply i2c_fwinit_data success");
			}
		}
	}
