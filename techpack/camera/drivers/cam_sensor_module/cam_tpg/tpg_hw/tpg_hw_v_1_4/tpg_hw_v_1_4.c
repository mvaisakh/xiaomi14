...
static int
get_tpg_vc_dt_pattern_id(enum tpg_interleaving_format_t vc_dt_pattern)
{
...
static int configure_xcfa_array(struct tpg_hw *hw, int config)
{
	...
	case 1:
		cam_io_w_mb(RGGB_IR_0,
			    soc_info->reg_map[0].mem_base +
				    tpg_reg->tpg_vc0_color_bar_cfa_color0);
		cam_io_w_mb(RGGB_IR_1,
			    soc_info->reg_map[0].mem_base +
				    tpg_reg->tpg_vc0_color_bar_cfa_color1);
		break;
	case 2:
	...
	case 3:
		...
		cam_io_w_mb(RGGB_3x3_2,
			    soc_info->reg_map[0].mem_base +
				    tpg_reg->tpg_vc0_color_bar_cfa_color2);
		break;
	case 4:
		...
		cam_io_w_mb(RGGB_4x4_3,
			    soc_info->reg_map[0].mem_base +
				    tpg_reg->tpg_vc0_color_bar_cfa_color3);
		break;
		break;
	}
	return 0;
...
static int configure_global_configs(struct tpg_hw *hw, int num_vcs,
				    struct tpg_global_config_t *configs)
{
	...
	val = (1 << tpg_reg->rup_done_mask_vec_shift) |
	      (1 << tpg_reg->tpg_done_mask_vec_shift);
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->top_irq_mask);

	val = (num_vcs - 1) << (tpg_reg->num_active_vc_shift) |
	...
	if (hw->settings_update) {
		cam_io_w_mb(TPG_DISABLE, soc_info->reg_map[0].mem_base +
		...
		if (rem_jiffies) {
			...
			CAM_ERR(CAM_TPG, "TPG[%d] hw timeout %llu", hw->hw_idx,
				rem_jiffies);
			rc = -EBUSY;
		}
	}
...
static int get_pixel_coordinate(int cfa_pattern_start_index,
				int cfa_pattern_end_index, uint32_t *val,
	...
	*val = 0;
	for (i = cfa_pattern_start_index; i < cfa_pattern_end_index; i++) {
		for (j = 0; j < configs->cfa_info.pattern_width; j++) {
			...
			*val |= (configs->cfa_info.pixel_coordinate[idx]
					 .pixel_type)
				<< shift;
		}
	}
...
static int configure_xcfa_array_v3(struct tpg_hw *hw,
				   struct tpg_stream_config_v3_t *configs)
{
	struct cam_hw_soc_info *soc_info = NULL;
	struct cam_tpg_ver_1_4_reg_offset *tpg_reg = NULL;
	...
	case XCFA_TYPE_RGBIR:
		...
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc0_color_bar_cfa_color0);
		get_pixel_coordinate(2, configs->cfa_info.pattern_height, &val,
				     configs);
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc0_color_bar_cfa_color1);
		break;
	...
	case XCFA_TYPE_THREEXTHREECFA:
		...
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc0_color_bar_cfa_color1);
		get_pixel_coordinate(4, configs->cfa_info.pattern_height, &val,
				     configs);
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc0_color_bar_cfa_color2);
		break;
	case XCFA_TYPE_FOURXFOURCFA:
		...
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc0_color_bar_cfa_color2);
		get_pixel_coordinate(6, configs->cfa_info.pattern_height, &val,
				     configs);
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc0_color_bar_cfa_color3);
		break;
...
static int configure_dt_v3(struct tpg_hw *hw, uint32_t vc_slot,
			   uint32_t dt_slot,
			   struct tpg_stream_config_v3_t *stream)
{
	...
	CAM_DBG(CAM_TPG, "TPG[%d] slot(%d,%d) <= dt:%d", hw->hw_idx, vc_slot,
		dt_slot, stream->dt);

	val = (((stream->stream_dimension.width & 0xFFFF)
		<< tpg_reg->frame_width_shift) |
	       (stream->stream_dimension.height &
		0xFFFF << tpg_reg->frame_height_shift));
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				 tpg_reg->tpg_vc0_dt_0_cfg_0 +
				 (0x60 * vc_slot) + (dt_slot * 0x0c));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_dt%d_cfg_0=0x%x", hw->hw_idx, vc_slot,
		dt_slot, val);

	cam_io_w_mb(stream->dt, soc_info->reg_map[0].mem_base +
					tpg_reg->tpg_vc0_dt_0_cfg_1 +
					(0x60 * vc_slot) + (dt_slot * 0x0c));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_dt%d_cfg_1=0x%x", hw->hw_idx, vc_slot,
		dt_slot, stream->dt);

	val = ((get_tpg_encode_format(stream->pixel_depth) & 0xF)
	       << tpg_reg->encode_format_shift) |
	      (get_tpg_payload_mode(stream->pattern_type)
	       << tpg_reg->pattern_shift);
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				 tpg_reg->tpg_vc0_dt_0_cfg_2 +
				 (0x60 * vc_slot) + (dt_slot * 0x0c));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_dt%d_cfg_2=0x%x", hw->hw_idx, vc_slot,
		dt_slot, val);

	return 0;
}

static int configure_dt(struct tpg_hw *hw, uint32_t vc_slot, uint32_t dt_slot,
...
static int configure_vc_v3(struct tpg_hw *hw, uint32_t vc_slot, int num_dts,
			   struct tpg_stream_config_v3_t *stream)
{
	uint32_t val = 0;
	struct cam_hw_soc_info *soc_info = NULL;
	...
	if (stream->cfa_info_exist != 0) {
		val |= ((stream->cfa_info.pattern_height - 1)
			<< tpg_reg->size_y_shift);
		val |= ((stream->cfa_info.pattern_width - 1)
			<< tpg_reg->size_x_shift);
		val |= (1 << tpg_reg->xcfa_en_shift);
		configure_xcfa_array_v3(hw, stream);
	}

	CAM_DBG(CAM_TPG, "TPG[%d] period: %d", hw->hw_idx,
		stream->rotate_period);
	val |= ((stream->rotate_period & 0x3F) << tpg_reg->rotate_period_shift);
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				 tpg_reg->tpg_vc0_color_bars_cfg +
				 (0x60 * vc_slot));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_color_bar_cfg=0x%x", hw->hw_idx, vc_slot,
		val);

	val = stream->hbi;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				 tpg_reg->tpg_vc0_hbi_cfg + (0x60 * vc_slot));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_hbi_cfg=0x%x", hw->hw_idx, vc_slot, val);

	val = stream->vbi;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				 tpg_reg->tpg_vc0_vbi_cfg + (0x60 * vc_slot));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_vbi_cgf=0x%x", hw->hw_idx, vc_slot, val);
	...
	cam_io_w_mb(0x12345678, soc_info->reg_map[0].mem_base +
					tpg_reg->tpg_vc0_lfsr_seed +
					(0x60 * vc_slot));

	val = ((stream->frame_count << tpg_reg->num_frames_shift) |
	       ((num_dts - 1) << tpg_reg->num_active_dt_shift) | stream->vc);
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->tpg_vc0_cfg0 +
				 (0x60 * vc_slot));
	CAM_DBG(CAM_TPG, "TPG[%d] vc%d_cfg0=0x%x", hw->hw_idx, vc_slot, val);
	if (hw->hw_info->shdr_overlap == 1) {
		cam_io_w_mb(hw->hw_info->shdr_overlap
				    << tpg_reg->overlap_shdr_en_shift,
	...
	if (hw->hw_info->shdr_offset_num_batch >= 0 && vc_slot > 0) {
		...
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					 tpg_reg->tpg_vc1_shdr_cfg +
					 (0x60 * (vc_slot - 1)));
		CAM_DBG(CAM_TPG, "TPG[%d] vc%d_cfg0=0x%x shdr", hw->hw_idx,
			vc_slot, val);
	}
...
static int configure_vc(struct tpg_hw *hw, uint32_t vc_slot, int num_dts,
			struct tpg_stream_config_t *stream)
{
	uint32_t val = 0;
	struct cam_hw_soc_info *soc_info = NULL;
...
static int tpg_hw_v_1_4_reset(struct tpg_hw *hw, void *data)
{
	struct cam_hw_soc_info *soc_info = NULL;
	uint32_t val;
	struct cam_tpg_ver_1_4_reg_offset *tpg_reg = NULL;
	...
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base + tpg_reg->top_irq_mask);

	cam_io_w_mb(1, soc_info->reg_map[0].mem_base + tpg_reg->top_irq_clear);

	cam_io_w_mb(1, soc_info->reg_map[0].mem_base + tpg_reg->irq_cmd);

	cam_io_w_mb(1, soc_info->reg_map[0].mem_base + tpg_reg->tpg_ctrl_cmd);

	/* Read the version */
	val = cam_io_r_mb(soc_info->reg_map[0].mem_base + tpg_reg->hw_version);
	CAM_INFO(CAM_TPG, "TPG[%d] TPG HW version: 0x%x started", hw->hw_idx,
		 val);

	return 0;
}

int tpg_hw_v_1_4_process_cmd(struct tpg_hw *hw, uint32_t cmd, void *arg)
{
	int rc = 0;
	...
	case TPG_CONFIG_VC: {
		if (hw->stream_version == 1) {
			...
			rc = configure_vc(hw, vc_config->vc_slot,
					  vc_config->num_dts,
					  vc_config->stream);
			...
			rc = configure_vc_v3(hw, vc_config_v3->vc_slot,
	...
	case TPG_CONFIG_DT: {
		if (hw->stream_version == 1) {
			...
			rc = configure_dt(hw, dt_config->vc_slot,
					  dt_config->dt_slot,
					  dt_config->stream);
		} else if (hw->stream_version == 3) {
			...
			rc = configure_dt_v3(hw, dt_config_v3->vc_slot,
	...
	case TPG_CONFIG_CTRL: {
		...
		rc = configure_global_configs(hw, global_args->num_vcs,
...
irqreturn_t tpg_hw_v_1_4_handle_irq(struct tpg_hw *hw)
{
	...
	if (val & ((1 << tpg_reg->rup_done_status_shift) |
		...
		cam_io_w_mb(val,
			    soc_info->reg_map[0].mem_base + tpg_reg->irq_cmd);
...
static int tpg_1_4_get_shdr_offset_num_batch(void *data, u64 *val)
{
	...
	CAM_INFO(CAM_TPG, "get shdr_num_batch : %d",
		 hw->hw_info->shdr_offset_num_batch);
	*val = hw->hw_info->shdr_offset_num_batch;
	return 0;
}
static int tpg_1_4_get_shdr_line_offset0(void *data, u64 *val)
{
	...
	CAM_INFO(CAM_TPG, "get shdr_offset0 : %d",
		 hw->hw_info->shdr_line_offset0);
...
static int tpg_1_4_get_shdr_line_offset1(void *data, u64 *val)
{
	...
	CAM_INFO(CAM_TPG, "get shdr_offset1 : %d",
		 hw->hw_info->shdr_line_offset1);
...
static int tpg_1_4_set_shdr_offset_num_batch(void *data, u64 val)
{
	...
	CAM_INFO(CAM_TPG, "set shdr_num_batch : %d",
		 hw->hw_info->shdr_offset_num_batch);
...
static int tpg_1_4_set_shdr_line_offset0(void *data, u64 val)
{
	...
	CAM_INFO(CAM_TPG, "set shdr_offset0 : %d",
		 hw->hw_info->shdr_line_offset0);
...
int tpg_1_4_layer_init(struct tpg_hw *hw)
{
	...
	snprintf(dir_name, sizeof(dir_name), "tpg%d", hw->hw_idx);
	...
	debugfs_create_file("tpg_xcfa_test", 0644, dbgfileptr_parent, hw,
			    &tpg_1_4_xcfa_test);
	debugfs_create_file("tpg_shdr_overlap_test", 0644, dbgfileptr_parent,
			    hw, &tpg_1_4_shdr_overlap_test);
	debugfs_create_file("tpg_shdr_offset_num_batch", 0644,
			    dbgfileptr_parent, hw,
			    &tpg_1_4_shdr_offset_num_batch);
	debugfs_create_file("tpg_shdr_line_offset0", 0644, dbgfileptr_parent,
			    hw, &tpg_1_4_shdr_line_offset0);
	debugfs_create_file("tpg_shdr_line_offset1", 0644, dbgfileptr_parent,
			    hw, &tpg_1_4_shdr_line_offset1);
