...
static int cam_io_tpg_dump(void __iomem *base_addr, uint32_t start_offset,
			   int size)
{
	char line_str[128];
	char *p_str;
	...
	CAM_DBG(CAM_TPG, "addr=%pK offset=0x%x size=%d", base_addr,
		start_offset, size);
...
static const char *const tpg_phy_type_strings[] = { "TPG_PHY_TYPE_INVALID",
						    "TPG_PHY_TYPE_DPHY",
						    "TPG_PHY_TYPE_CPHY",
						    "TPG_PHY_TYPE_MAX" };

static const char *const tpg_interleaving_format_string[] = {
	"TPG_INTERLEAVING_FORMAT_INVALID",  "TPG_INTERLEAVING_FORMAT_FRAME",
	"TPG_INTERLEAVING_FORMAT_LINE",	    "TPG_INTERLEAVING_FORMAT_SHDR",
	"TPG_INTERLEAVING_FORMAT_SparsePD", "TPG_INTERLEAVING_FORMAT_MAX"
};

static const char *const tpg_shutter_type_strings[] = {
	"TPG_SHUTTER_TYPE_INVALID", "TPG_SHUTTER_TYPE_ROLLING",
	"TPG_SHUTTER_TYPE_GLOBAL", "TPG_SHUTTER_TYPE_MAX"
};
...
static const char *const tpg_color_bar_mode_strings[] = {
	"TPG_COLOR_BAR_MODE_INVALID", "TPG_COLOR_BAR_MODE_NORMAL",
	"TPG_COLOR_BAR_MODE_SPLIT", "TPG_COLOR_BAR_MODE_ROTATING",
	"TPG_COLOR_BAR_MODE_MAX"
};

static const char *const tpg_stream_type_strings[] = {
	"TPG_STREAM_TYPE_INVALID", "TPG_STREAM_TYPE_IMAGE",
	"TPG_STREAM_TYPE_PDAF", "TPG_STREAM_TYPE_META", "TPG_STREAM_TYPE_MAX"
};

static const char *const tpg_image_format_type_strings[] = {
	"TPG_IMAGE_FORMAT_INVALID", "TPG_IMAGE_FORMAT_BAYER",
	"TPG_IMAGE_FORMAT_QCFA",    "TPG_IMAGE_FORMAT_YUV",
	"TPG_IMAGE_FORMAT_JPEG",    "TPG_IMAGE_FORMAT_MAX"
};
#endif
...
{
#ifdef __TPG_DEBUG_DUMP__
	CAM_DBG(CAM_TPG, "TPG[%d] phy_type            : %s", idx,
		tpg_phy_type_strings[global->phy_type]);
	CAM_DBG(CAM_TPG, "TPG[%d] lane_count          : %d", idx,
		global->lane_count);
	CAM_DBG(CAM_TPG, "TPG[%d] interleaving_format : %s", idx,
		tpg_interleaving_format_string[global->interleaving_format]);
	CAM_DBG(CAM_TPG, "TPG[%d] phy_mode            : %d", idx,
		global->phy_mode);
	CAM_DBG(CAM_TPG, "TPG[%d] shutter_type        : %s", idx,
		tpg_shutter_type_strings[global->shutter_type]);
	CAM_DBG(CAM_TPG, "TPG[%d] skip pattern        : 0x%x", idx,
		global->skip_pattern);
	CAM_DBG(CAM_TPG, "TPG[%d] tpg clock           : %d", idx,
		global->tpg_clock);
#endif
	return 0;
}
...
{
#ifdef __TPG_DEBUG_DUMP__
	CAM_DBG(CAM_TPG, "TPG[%d][%d] pattern_type    : %s", hw_idx, stream_idx,
		tpg_pattern_type_strings[stream->pattern_type]);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] cb_mode         : %s", hw_idx, stream_idx,
		tpg_color_bar_mode_strings[stream->cb_mode]);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] frame_count     : %d", hw_idx, stream_idx,
		stream->frame_count);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] stream_type     : %s", hw_idx, stream_idx,
		tpg_stream_type_strings[stream->stream_type]);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] left            : %d", hw_idx, stream_idx,
		stream->stream_dimension.left);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] top             : %d", hw_idx, stream_idx,
		stream->stream_dimension.top);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] width           : %d", hw_idx, stream_idx,
		stream->stream_dimension.width);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] height          : %d", hw_idx, stream_idx,
		stream->stream_dimension.height);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] pixel_depth     : %d", hw_idx, stream_idx,
		stream->pixel_depth);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] cfa_arrangement : %d", hw_idx, stream_idx,
		stream->cfa_arrangement);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] output_format   : %s", hw_idx, stream_idx,
		tpg_image_format_type_strings[stream->output_format]);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] vc              : 0x%x", hw_idx,
		stream_idx, stream->vc);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] dt              : 0x%x", hw_idx,
		stream_idx, stream->dt);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] hbi             : %d", hw_idx, stream_idx,
		stream->hbi);
	CAM_DBG(CAM_TPG, "TPG[%d][%d] vbi             : %d", hw_idx, stream_idx,
		stream->vbi);
#endif
	return 0;
}
	...
	for (i = 0; i < hw->hw_info->max_vc_channels; i++) {
		...
		if (hw->stream_version == 1) {
			list_for_each_safe(pos, pos_next,
					   &req->vc_slots[i].head) {
				entry = list_entry(pos, struct tpg_hw_stream,
						   list);
				list_del(pos);
				kfree(entry);
			}
			...
			list_for_each_safe(pos, pos_next,
					   &req->vc_slots[i].head) {
				entry_v3 = list_entry(
					pos, struct tpg_hw_stream_v3, list);
				list_del(pos);
				kfree(entry_v3);
			}
		}
...
static int tpg_hw_free_waiting_requests_locked(struct tpg_hw *hw)
{
	...
	CAM_DBG(CAM_TPG, "TPG[%d]  freeing all waiting requests", hw->hw_idx);
	list_for_each_safe(pos, pos_next, &hw->waiting_request_q) {
		req = list_entry(pos, struct tpg_hw_request, list);
		CAM_DBG(CAM_TPG, "TPG[%d] freeing request[%lld] ", hw->hw_idx,
			req->request_id);
		list_del(pos);
...
static int tpg_hw_free_active_requests_locked(struct tpg_hw *hw)
{
	...
	CAM_DBG(CAM_TPG, "TPG[%d]  freeing all active requests", hw->hw_idx);
...
static int tpg_hw_soc_disable(struct tpg_hw *hw)
{
	...
	rc = cam_soc_util_disable_platform_resource(
		hw->soc_info, CAM_CLK_SW_CLIENT_IDX, true, true);
...
static int tpg_hw_soc_enable(struct tpg_hw *hw, enum cam_vote_level clk_level,
			     uint32_t enable_irq)
{
	int rc = 0;
	...
	CAM_DBG(CAM_TPG,
		...
		hw->hw_idx, axi_vote.axi_path[0].camnoc_bw,
		axi_vote.axi_path[0].mnoc_ab_bw,
		axi_vote.axi_path[0].mnoc_ib_bw);

	rc = cam_cpas_start(hw->cpas_handle, &ahb_vote, &axi_vote);
	if (rc) {
		CAM_ERR(CAM_TPG, "TPG[%d] CPAS start failed", hw->hw_idx);
		rc = -EFAULT;
		goto end;
	}

	rc = cam_soc_util_enable_platform_resource(hw->soc_info,
						   CAM_CLK_SW_CLIENT_IDX, true,
						   clk_level, enable_irq);

	if (rc) {
		CAM_ERR(CAM_TPG, "TPG[%d] enable platform failed", hw->hw_idx);
		goto stop_cpas;
	}
					      ...
					      struct tpg_hw_request *req)
{
	...
	if (!hw || !hw->hw_info || !hw->hw_info->ops ||
	    !hw->hw_info->ops->process_cmd || !hw->soc_info || !req) {
	...
	if (hw->stream_version == 1) {
		for (i = 0; i < hw->hw_info->max_vc_channels; i++) {
			...
			vc_stream_entry =
				list_first_entry(&req->vc_slots[i].head,
						 struct tpg_hw_stream, list);
			vc_config.stream = &vc_stream_entry->stream;
			hw->hw_info->ops->process_cmd(hw, TPG_CONFIG_VC,
						      &vc_config);
			...
			list_for_each_safe(pos, pos_next,
				...
				entry = list_entry(pos, struct tpg_hw_stream,
						   list);
				dump_stream_configs(hw->hw_idx, stream_idx++,
						    &entry->stream);
				dt_config.vc_slot = i;
				dt_config.dt_slot = dt_slot++;
				dt_config.stream = &entry->stream;
				hw->hw_info->ops->process_cmd(hw, TPG_CONFIG_DT,
							      &dt_config);
			}
		}
	} else if (hw->stream_version == 3) {
		for (i = 0; i < hw->hw_info->max_vc_channels; i++) {
			...
			struct list_head *pos = NULL, *pos_next = NULL;
			struct tpg_hw_stream_v3 *entry = NULL,
						*vc_stream_entry = NULL;
			...
			list_for_each_safe(pos, pos_next,
				...
				entry = list_entry(pos, struct tpg_hw_stream_v3,
						   list);
				dump_stream_configs_v3(hw->hw_idx, stream_idx++,
						       &entry->stream);
				dt_config.vc_slot = i;
				dt_config.dt_slot = dt_slot++;
				dt_config.stream = &entry->stream;
	...
	hw->hw_info->ops->process_cmd(hw, TPG_CONFIG_CTRL, &globalargs);
...
static int
tpg_hw_check_hw_state_and_apply_settings_locked(struct tpg_hw *hw,
						struct tpg_hw_request *req)
{
	int rc = 0;
     ...
     */
		if (hw->state == TPG_HW_STATE_BUSY) {
			wait_jiffies =
				CAM_TPG_HW_WAIT_TIMEOUT * timeout_multiplier;
			reinit_completion(&hw->complete_rup);
			rem_jiffies = cam_common_wait_for_completion_timeout(
				&hw->complete_rup, wait_jiffies);
			if (!rem_jiffies) {
...
static int tpg_hw_reapply_from_active_queue_locked(struct tpg_hw *hw,
						   uint64_t request_id)
{
	int rc = 0;
	...
	if (reapply_req) {
		CAM_DBG(CAM_TPG, "TPG[%d] got reapply request %d", hw->hw_idx,
			request_id);
		rc = tpg_hw_check_hw_state_and_apply_settings_locked(
			hw, reapply_req);
	} else {
		CAM_ERR(CAM_TPG,
...
static int tpg_hw_lookup_queues_and_apply_req_locked(struct tpg_hw *hw,
						     uint64_t request_id)
{
	int rc = 0;
	...
	if (!list_empty(&hw->waiting_request_q)) {
		...
		if (req->request_id == request_id) {
			CAM_DBG(CAM_TPG, "TPG[%d] request %d got matched",
			...
			if (rc == 0) {
				...
				if (!list_empty(&hw->active_request_q) &&
				    (hw->active_request_q_depth >=
				     MAX_ACTIVE_QUEUE_DEPTH)) {
					active_req = list_first_entry(
						&hw->active_request_q,
						struct tpg_hw_request, list);
	...
	if (rc == 0) {
		CAM_DBG(CAM_TPG, "TPG[%d] Hw Apply done for req %lld",
			hw->hw_idx, request_id);
	} else {
		CAM_ERR(CAM_TPG, "TPG[%d] Hw Apply Failed for req %lld",
			hw->hw_idx, request_id);
		rc = -EAGAIN;
	}
...
static int tpg_hw_apply_req_from_waiting_queue_locked(struct tpg_hw *hw)
{
	int rc = 0;
	struct list_head *pos = NULL, *pos_next = NULL;
	struct tpg_hw_request *req = NULL;
...
int tpg_hw_start(struct tpg_hw *hw)
{
	...
	case TPG_HW_VERSION_1_4:
		...
		break;
	default:
		CAM_ERR(CAM_TPG, "TPG[%d] Unsupported HW Version", hw->hw_idx);
		rc = -EINVAL;
		break;
	}
...
int tpg_hw_acquire(struct tpg_hw *hw, struct tpg_hw_acquire_args *acquire)
{
	int rc = 0;
...
enum cam_vote_level get_tpg_clk_level(struct tpg_hw *hw)
{
	...
	for (cam_vote_level = 0; cam_vote_level < CAM_MAX_VOTE;
		...
		if (soc_info->clk_rate[cam_vote_level][soc_info->src_clk_idx] >=
		    clk) {
			CAM_INFO(CAM_TPG,
				 "match detected %s : %llu:%d level : %d",
				 soc_info->clk_name[soc_info->src_clk_idx], clk,
...
static int tpg_hw_configure_init_settings(struct tpg_hw *hw,
...
static int
tpg_hw_configure_init_settings_v3(struct tpg_hw *hw,
...
int tpg_hw_config(struct tpg_hw *hw, enum tpg_hw_cmd_t config_cmd,
		  void *config_args)
{
	int rc = 0;
		...
		if (hw->settings_config.active_count != 0) {
			...
			if (hw->stream_version == 1) {
				tpg_hw_configure_init_settings(
					hw, (struct tpg_hw_initsettings *)
						    config_args);
			} else if (hw->stream_version == 3) {
				tpg_hw_configure_init_settings_v3(
					hw, (struct tpg_hw_initsettings_v3 *)
						    config_args);
			}
		}
		break;
...
int tpg_hw_copy_settings_config(struct tpg_hw *hw,
				struct tpg_settings_config_t *settings)
{
	...
	hw->register_settings = kzalloc(sizeof(struct tpg_reg_settings) *
						settings->settings_array_size,
					GFP_KERNEL);
	...
	reg_settings =
		(struct tpg_reg_settings *)((uint8_t *)settings +
					    settings->settings_array_offset);

	mutex_lock(&hw->mutex);
	memcpy(&hw->settings_config, settings,
	       sizeof(struct tpg_settings_config_t));
	memcpy(hw->register_settings, reg_settings,
...
static int assign_vc_slot(struct tpg_hw *hw, int vc, struct tpg_hw_request *req,
	...
	for (i = 0; i < hw->hw_info->max_vc_channels; i++) {
		...
		if (req->vc_slots[i].vc == vc) {
			...
			if (req->vc_slots[i].stream_count <
				...
				list_add_tail(&stream->list,
					      &req->vc_slots[i].head);
				req->vc_slots[i].stream_count++;
				req->vc_slots[i].vc = vc;
				rc = 0;
				CAM_DBG(CAM_TPG, "vc[%d]dt[%d]=>slot[%d]", vc,
					stream->stream.dt, i);
				break;
			} else {
				/**
...
static int assign_vc_slot_v3(struct tpg_hw *hw, int vc,
			     struct tpg_hw_request *req,
...
int tpg_hw_free_request(struct tpg_hw *hw, struct tpg_hw_request *req)
{
	int rc = 0;
...
int tpg_hw_reset(struct tpg_hw *hw)
{
	...
	if (rc)
		CAM_ERR(CAM_TPG,
			"TPG[%d] unable to free up the pending requests",
			hw->hw_idx);

	if (hw->state != TPG_HW_STATE_HW_DISABLED) {
		rc = cam_soc_util_disable_platform_resource(
			hw->soc_info, CAM_CLK_SW_CLIENT_IDX, true, false);
		if (rc) {
			CAM_ERR(CAM_TPG, "TPG[%d] Disable platform failed %d",
				hw->hw_idx, rc);
		}
	...
	if (rc) {
		CAM_ERR(CAM_TPG,
			"TPG[%d] unable to free up the active requests",
			hw->hw_idx);
	}
	mutex_unlock(&hw->mutex);
...
int tpg_hw_add_stream(struct tpg_hw *hw, struct tpg_hw_request *req,
		      struct tpg_stream_config_t *cmd)
{
	...
	memcpy(&stream->stream, cmd, sizeof(struct tpg_stream_config_t));
...
int tpg_hw_add_request(struct tpg_hw *hw, struct tpg_hw_request *req)
{
	int rc = 0;
...
struct tpg_hw_request *tpg_hw_create_request(struct tpg_hw *hw,
					     uint64_t request_id)
{
	struct tpg_hw_request *req = NULL;
	...
	/* Allocate request */
	req = kzalloc(sizeof(struct tpg_hw_request), GFP_KERNEL);
	if (!req) {
	...
	req->vc_slots = kcalloc(num_vc_channels,
				sizeof(struct tpg_vc_slot_info), GFP_KERNEL);
	...
	CAM_DBG(CAM_TPG, "TPG[%d] request(%lld) allocated success", hw->hw_idx,
		request_id);
	return req;
err_exit_1:
	kfree(req);
...
int tpg_hw_request_set_opcode(struct tpg_hw_request *req, uint32_t opcode)
{
	int rc = 0;
	...
	CAM_INFO(CAM_TPG, "req[%d] type = %d", req->request_id,
		 req->request_type);
	return rc;
}

int tpg_hw_apply(struct tpg_hw *hw, uint64_t request_id)
{
	int rc = 0;
...
int tpg_hw_add_stream_v3(struct tpg_hw *hw, struct tpg_hw_request *req,
			 struct tpg_stream_config_v3_t *cmd)
{
	...
	memcpy(&stream->stream, cmd, sizeof(struct tpg_stream_config_v3_t));
