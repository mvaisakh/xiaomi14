				    ...
				    struct tpg_global_config_t *configs)
{
	...
	CAM_DBG(CAM_TPG, "TPG[%d] cfg3=0x%x", hw->hw_idx, val);
	...
	cam_io_w_mb(val, soc_info->reg_map[1].mem_base +
				 tpg_reg->top_mux_reg_offset);
	CAM_INFO(CAM_TPG, "TPG[%d] Set CPAS top mux: 0x%x", hw->hw_idx, val);

	val = (1 << tpg_reg->tpg_en_shift);
	cam_io_w_mb(val,
		    soc_info->reg_map[0].mem_base + tpg_reg->tpg_module_cfg);
			...
			struct tpg_stream_config_t *stream)
{
	...
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->tpg_cfg0);
	CAM_DBG(CAM_TPG, "TPG[%d] cfg0=0x%x", hw->hw_idx, val);

	val = stream->dt << tpg_reg->tpg_dt_shift;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->tpg_cfg2);
	CAM_DBG(CAM_TPG, "TPG[%d] cfg2=0x%x", hw->hw_idx, val);
			...
			struct tpg_stream_config_t *stream)
{
	...
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->tpg_cfg1);
	CAM_DBG(CAM_TPG, "TPG[%d] cfg1=0x%x", hw->hw_idx, val);

	return 0;
}

static int tpg_hw_v_1_2_reset(struct tpg_hw *hw, void *data)
{
	struct cam_hw_soc_info *soc_info = NULL;
	uint32_t val;
	struct cam_tpg_ver_1_2_reg_offset *tpg_reg = &cam_tpg102_reg;
...
int tpg_hw_v_1_2_process_cmd(struct tpg_hw *hw, uint32_t cmd, void *arg)
{
	int rc = 0;
	if (hw == NULL) {
	...
	switch (cmd) {
	case TPG_CONFIG_VC: {
		struct vc_config_args *vc_config = (struct vc_config_args *)arg;

		if (vc_config == NULL) {
			CAM_ERR(CAM_TPG, "invalid argument");
			return -EINVAL;
		}
		rc = configure_vc(hw, vc_config->vc_slot, vc_config->num_dts,
				  vc_config->stream);
	} break;
	case TPG_CONFIG_DT: {
		struct dt_config_args *dt_config = (struct dt_config_args *)arg;

		if (dt_config == NULL) {
			CAM_ERR(CAM_TPG, "invalid argument");
			return -EINVAL;
		}
		rc = configure_dt(hw, dt_config->vc_slot, dt_config->dt_slot,
				  dt_config->stream);
	} break;
