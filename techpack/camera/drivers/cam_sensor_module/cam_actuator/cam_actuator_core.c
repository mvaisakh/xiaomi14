...
static int32_t cam_actuator_power_up(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->io_master_info.master_type == I3C_MASTER)
		i3c_probe_completion =
			cam_actuator_get_i3c_completion(a_ctrl->soc_info.index);
	...
	if (rc) {
		...
		cam_hw_notify_v4l2_error_event(
			CAM_ACTUATOR_NAME, (void *)a_ctrl,
			V4L_EVENT_HW_ISSUE_POWER_ERROR,
			HW_ISSUE_ERROR_TYPE_ACTUATOR_POWER,
					  ...
					  uint32_t *cmd_buf, size_t len)
{
	...
	if (a_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		a_ctrl->io_master_info.cci_client->sid = i2c_info->slave_addr >>
							 1;
	...
	list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
		...
		if (rc < 0) {
			...
			cam_hw_notify_v4l2_error_event(
				...
				HW_ISSUE_ERROR_TYPE_ACTUATOR_CCI,
...
int32_t cam_actuator_apply_request(struct cam_req_mgr_apply_request *apply)
{
	...
	trace_cam_apply_req("Actuator", a_ctrl->soc_info.index,
...
static int cam_actuator_update_req_mgr(struct cam_actuator_ctrl_t *a_ctrl,
				   ...
				   void *arg)
{
			...
			case CAMERA_SENSOR_CMD_TYPE_PWR_DOWN:
				...
				if (PARKLENS_INVALID !=
				    parklens_atomic_read(
					    &(a_ctrl->parklens_ctrl
						      .parklens_state))) {
					parklens_thread_stop(
						a_ctrl,
	...
	case CAM_ACTUATOR_PACKET_OPCODE_READ: {
		...
		rc = cam_sensor_i2c_read_data(&i2c_read_settings,
					      &a_ctrl->io_master_info);
		if (rc < 0) {
	...
	case CAM_ACTUATOR_PACKET_OPCODE_PARKLENS: {
		int32_t exit_result = parklens_atomic_read(
			&(a_ctrl->parklens_ctrl.exit_result));
		...
		if ((parklens_state != PARKLENS_INVALID) ||
			...
			CAM_WARN(
				MI_PARKLENS,
				"[ParklensLog] Not in right state to do parklens: %d/%d, exit "
				"result %d release power control right",
				a_ctrl->cam_act_state, parklens_state,
				exit_result);
...
void cam_actuator_shutdown(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->cam_act_state >= CAM_ACTUATOR_CONFIG) {
		if (false == is_parklens_power_down(a_ctrl)) {
			...
			CAM_INFO(MI_PARKLENS,
				 "[ParklensLog] parklens isn't powerdown");
		} else {
			CAM_INFO(MI_PARKLENS,
				 "[ParklensLog] parklens has been powerdown");
		}
...
int32_t cam_actuator_driver_cmd(struct cam_actuator_ctrl_t *a_ctrl, void *arg)
{
	...
	case CAM_RELEASE_DEV: {
		...
		CAM_INFO(
			MI_PARKLENS,
			"[ParklensLog] CAM_RELEASE_DEV is_parklens_power_down status %d/%d",
			parklens_power_down, a_ctrl->cam_act_state);
		if (a_ctrl->cam_act_state == CAM_ACTUATOR_CONFIG) {
...
static int32_t parklens_thread_func(void *arg)
{
	...
	if (!arg) {
		...
		CAM_ERR(CAM_ACTUATOR,
			"[ParklensLog] parklens_thread_func arg is NULL");
	} else {
		a_ctrl = (struct cam_actuator_ctrl_t *)arg;
		parklens_ctrl = &(a_ctrl->parklens_ctrl);
		soc_private = (struct cam_actuator_soc_private *)
				      a_ctrl->soc_info.soc_private;
		i2c_set = (struct i2c_settings_array *)(&(
			a_ctrl->i2c_data.parklens_settings));
		power_info = &soc_private->power_info;

		rc = parklens_wake_lock_create(&parklens_wakelock,
					       "parklens_wakeup");
		if (rc < 0) {
			parklens_atomic_set(
				...
				PARKLENS_EXIT_CREATE_WAKELOCK_FAILED);
			CAM_ERR(MI_PARKLENS,
				"[ParklensLog] acquire wakelock at parklens thread failed");
		} else {
			parklens_wake_lock_acquire(&parklens_wakelock);
			CAM_DBG(MI_PARKLENS,
				"[ParklensLog] acquire wakelock at parklens thread");
		}
	}
	...
	while (parklens_step > 0) {
		if (i2c_set == NULL) {
			...
			if ((i2c_list_last != NULL) &&
			    list_entry_is_head(i2c_list, &(i2c_set->list_head),
					       list)) {
				CAM_DBG(MI_PARKLENS,
				...
				if (i2c_list->i2c_settings.delay > 10 &&
				...
				rc = cam_actuator_i2c_modes_util(
					&(a_ctrl->io_master_info), i2c_list);
				if (rc < 0) {
					CAM_ERR(MI_PARKLENS,
						"[ParklensLog] parklens step failed, %d",
						rc);
					parklens_atomic_set(
						&(parklens_ctrl->exit_result),
				...
				if (1 == parklens_step) {
					CAM_INFO(
						MI_PARKLENS,
						"[ParklensLog] parklens step exceed! power down");
					parklens_atomic_set(
						&(parklens_ctrl->exit_result),
						PARKLENS_EXIT_WITH_POWERDOWN);
				}
		...
		if ((wait_result > 0) || ((-ERESTARTSYS) == wait_result)) {
			...
			parklens_opcode = parklens_atomic_read(
				&(parklens_ctrl->parklens_opcode));
			if (parklens_opcode ==
			    EXIT_PARKLENS_WITHOUT_POWERDOWN) {
				CAM_DBG(MI_PARKLENS,
				...
				CAM_ERR(MI_PARKLENS,
					"[ParklensLog] Invalid opcode for parklens %d",
					parklens_opcode);
				continue;
			}
		} else {
			CAM_DBG(MI_PARKLENS,
				"[ParklensLog] Parklens time out %d",
				wait_result);
		}
		parklens_step--;
		...
		CAM_DBG(MI_PARKLENS,
			...
			parklens_step, parklens_ctrl->parklens_state);
	}
...
exit_with_powerdown:
	...
	lock_power_sync_mutex(a_ctrl->io_master_info.cci_client,
			      a_ctrl->cci_i2c_master);
	rc = cam_actuator_power_down(a_ctrl);
	unlock_power_sync_mutex(a_ctrl->io_master_info.cci_client,
				a_ctrl->cci_i2c_master);
	if (rc < 0) {
		CAM_DBG(MI_PARKLENS,
...
exit_without_powerdown:
	...
	if (rc < 0)
		CAM_ERR(CAM_SENSOR,
			"[ParklensLog] delete parklens request failed rc: %d",
			rc);
...
int32_t init_parklens_info(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (!a_ctrl) {
		CAM_ERR(MI_PARKLENS, "[ParklensLog] failed: a_ctrl %pK",
			a_ctrl);
		return -EINVAL;
	}
	...
	CAM_DBG(MI_PARKLENS, "[ParklensLog] init parklens: %s",
		a_ctrl->device_name);
...
int32_t deinit_parklens_info(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	CAM_DBG(MI_PARKLENS, "[ParklensLog] deinit parklens: %s",
		a_ctrl->device_name);
	...
	if (PARKLENS_STOP !=
		...
		CAM_ERR(MI_PARKLENS,
			"[ParklensLog] deinit parklens in wrong state");
		return -EINVAL;
	}
...
bool is_parklens_power_down(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	case PARKLENS_RUNNING: {
		CAM_DBG(MI_PARKLENS,
			"[ParklensLog] parklens is running, no need powerdown");
		is_power_down = true;
	} break;
	...
	default:
		CAM_ERR(MI_PARKLENS, "Invalid parklens_state %d",
			parklens_state);
	}
...
int32_t parklens_thread_trigger(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (rc < 0) {
		CAM_ERR(MI_PARKLENS,
			"[ParklensLog] failed to init parklens info %d", rc);
		return rc;
	}
	...
	parklens_ctrl->parklens_thread = parklens_thread_run(
		parklens_thread_func, a_ctrl, "parklens-thread");

	if (!(parklens_ctrl->parklens_thread)) {
		CAM_ERR(MI_PARKLENS,
			"[ParklensLog] parklens_thread create failed");
		rc = -ENOMEM;
		...
		if (rc > PARKLENS_ENTER) {
			CAM_DBG(MI_PARKLENS,
				"[ParklensLog] parklens thread execute failed %d",
				rc);
			goto clear_thread;
		} else
			     ...
			     enum parklens_opcodes opcode)
{
	...
	if (parklens_state != PARKLENS_RUNNING) {
		CAM_DBG(MI_PARKLENS,
			"[ParklensLog] parklens thread is in other state: %d",
			parklens_state);
		return -EINVAL;
	} else {
		if (exit_result != PARKLENS_ENTER) {
			CAM_DBG(MI_PARKLENS,
				"[ParklensLog] parklens thread is already end: %d",
				exit_result);
		} else {
			...
			CAM_DBG(MI_PARKLENS,
				"[ParklensLog] wait for parklens thread start");
			parklens_wake_up_interruptible(
				&(parklens_ctrl->parklens_wait_queue));
			CAM_DBG(MI_PARKLENS,
				"[ParklensLog] wake up parklens thread to stop it");
		}
		...
		if (exit_result) {
			CAM_INFO(
				MI_PARKLENS,
				"[ParklensLog] parklens thread exit status: %d",
				exit_result);
		} else
			CAM_ERR(MI_PARKLENS,
				"[ParklensLog] parklens thread exit failed ! ! !");
	}
