					   ...
					   bool skip_dump)
{
	...
	rc = icp_dev_intf->hw_ops.process_cmd(icp_dev_intf->hw_priv,
					      ...
					      &dump_type, sizeof(dump_type));
	if (rc)
		CAM_ERR(CAM_ICP, "[%s] Fail to dump debug info",
			hw_mgr->hw_mgr_name);
}
...
static int cam_icp_send_ubwc_cfg(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	rc = icp_dev_intf->hw_ops.process_cmd(icp_dev_intf->hw_priv,
					      CAM_ICP_CMD_UBWC_CFG, &ubwc_cmd,
					      sizeof(ubwc_cmd));
	if (rc)
		CAM_ERR(CAM_ICP, "[%s] Fail to submit UBWC config",
			hw_mgr->hw_mgr_name);
				      ...
				      uint32_t base_clk)
{
	...
	CAM_DBG(CAM_PERF, "%s: Already clk at lower level",
		ctx_data->ctx_id_string);
	return base_clk;
}
				     ...
				     uint32_t base_clk)
{
	...
	CAM_DBG(CAM_PERF, "%s: Already clk at higher level",
		ctx_data->ctx_id_string);
...
static int
cam_icp_get_frame_process_idx_from_req_id(struct cam_icp_hw_ctx_data *ctx_data,
					  uint64_t req_id)
{
...
static int32_t cam_icp_deinit_idle_clk(void *priv, void *data)
{
	...
	for (i = 0; i < CAM_ICP_CTX_MAX; i++) {
		...
		if (ctx_data->state == CAM_ICP_CTX_STATE_ACQUIRED) {
			if (ctx_data->device_info->hw_dev_type ==
			    dev_info->hw_dev_type) {
	...
	if (!ctx_data) {
		CAM_ERR(CAM_ICP, "[%s] No acquired ctx data found",
			hw_mgr->hw_mgr_name);
		rc = -EFAULT;
		goto done;
	}
	...
	CAM_DBG(CAM_PERF, "[%s] Disable %d", hw_mgr->hw_mgr_name,
		dev_info->dev_name);
...
static inline bool cam_icp_validate_bw_path_idx(int path_idx,
						uint32_t path_data_type)
{
	if (path_idx < 0) {
		...
		CAM_WARN(
			...
			path_data_type, CAM_AXI_PATH_DATA_IPE_START_OFFSET,
...
static inline int
cam_icp_get_axi_path_index(struct cam_cpas_axi_per_path_bw_vote *axi_path,
			   enum cam_icp_hw_type hw_dev_type)
{
	...
	case CAM_ICP_DEV_IPE:
		return axi_path->path_data_type -
...
static inline int
cam_icp_get_bw_device_share_ratio(struct cam_icp_hw_mgr *hw_mgr,
				  struct cam_icp_hw_device_info *dev_info)
{
   ...
   */
	...
	if (num_dev > 1) {
		...
		if (num_dev > 2) {
			CAM_ERR(CAM_ICP,
				"[%s] Number of devices %u not supported for geting bw device "
				"share ratio",
				 ...
				 struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	if (!ctx_data->icp_dev_acquire_info) {
		CAM_WARN(CAM_ICP, "%s: NULL acquire info",
			 ctx_data->ctx_id_string);
		return -EINVAL;
	}

	CAM_DBG(CAM_PERF, "%s: ubw = %lld cbw = %lld curr_fc = %u bc = %u",
	...
	device_share_ratio =
		cam_icp_get_bw_device_share_ratio(hw_mgr, dev_info);
     ...
     */
		for (i = 0; i < ctx_data->clk_info.num_paths; i++) {
			path_index = cam_icp_get_axi_path_index(
				&ctx_data->clk_info.axi_path[i], hw_dev_type);

			if (cam_icp_validate_bw_path_idx(
			...
			CAM_DBG(CAM_PERF,
				"%s: Removing ctx bw from path_type: %s, transac_type: %s, "
				"camnoc_bw = %lld mnoc_ab_bw = %lld, mnoc_ib_bw = %lld",
				ctx_data->ctx_id_string,
			...
			CAM_DBG(CAM_PERF,
				"%s: Final HW bw for path_type: %s, transac_type: %s, camnoc_bw "
				"= %lld mnoc_ab_bw = %lld, mnoc_ib_bw = %lld",
				ctx_data->ctx_id_string,
				cam_cpas_axi_util_path_type_to_string(
   ...
   */
	...
	CAM_DBG(CAM_PERF, "%s: X :curr_fc = %u bc = %u",
		ctx_data->ctx_id_string, ctx_data->clk_info.curr_fc,
		ctx_data->clk_info.base_clk);
...
static void cam_icp_ctx_timer_cb(struct timer_list *timer_data)
{
	...
	cam_req_mgr_workq_enqueue_task(task, hw_mgr, CRM_TASK_PRIORITY_0);
	spin_unlock_irqrestore(&hw_mgr->hw_mgr_lock, flags);
}
...
static int cam_icp_get_svs_clk_info(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (CAM_ICP_IS_DEV_HW_EXIST(hw_mgr->hw_cap_mask, CAM_ICP_DEV_IPE)) {
		dev_intf =
			hw_mgr->dev_info[hw_mgr->dev_info_idx[CAM_ICP_DEV_IPE]]
				.dev_intf[0];
		if (!dev_intf) {
			CAM_ERR(CAM_ICP, "[%s] IPE dev intf is invalid",
				hw_mgr->hw_mgr_name);
			return -EINVAL;
		}
	} else if (CAM_ICP_IS_DEV_HW_EXIST(hw_mgr->hw_cap_mask,
					   CAM_ICP_DEV_OFE)) {
		dev_intf =
			hw_mgr->dev_info[hw_mgr->dev_info_idx[CAM_ICP_DEV_OFE]]
				.dev_intf[0];
		if (!dev_intf) {
			CAM_ERR(CAM_ICP, "[%s] OFE dev inf is invalid",
				hw_mgr->hw_mgr_name);
			return -EINVAL;
		}
	} else {
	...
	CAM_DBG(CAM_PERF, "[%s] icp_svs_clk = %lld", hw_mgr->hw_mgr_name,
		hw_mgr->icp_svs_clk);
	return 0;
}
...
static int cam_icp_ctx_timer_start(struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	if (rc)
		CAM_ERR(CAM_ICP, "%s: Failed to start timer",
			ctx_data->ctx_id_string);
...
static bool cam_icp_update_clk_busy(
	...
	if (request_clk_info->frame_cycles > ctx_data->clk_info.curr_fc) {
		...
		if (dev_clk_info->base_clk > actual_clk) {
			...
			next_clk_level = cam_icp_get_next_clk_rate(
				hw_mgr, ctx_data, dev_clk_info->curr_clk);
	...
	struct cam_icp_clk_bw_request *request_clk_info, uint32_t base_clk)
{
   ...
   */
	if (dev_clk_info->over_clked < dev_clk_info->threshold) {
		...
		dev_clk_info->curr_clk = cam_icp_get_lower_clk_rate(
			hw_mgr, ctx_data, dev_clk_info->curr_clk);
		if (dev_clk_info->curr_clk > dev_clk_info->base_clk) {
			if (cam_icp_is_over_clk(hw_mgr, ctx_data, dev_clk_info))
				dev_clk_info->over_clked = 0;
		} else if (dev_clk_info->curr_clk < dev_clk_info->base_clk) {
			dev_clk_info->curr_clk = cam_icp_get_next_clk_rate(
				hw_mgr, ctx_data, dev_clk_info->curr_clk);
			dev_clk_info->over_clked = 0;
		} else if (dev_clk_info->curr_clk == dev_clk_info->base_clk) {
			dev_clk_info->over_clked = 0;
				     ...
				     struct cam_icp_clk_info *hw_mgr_clk_info)
{
	if (hw_mgr->icp_debug_clk &&
		...
		CAM_DBG(CAM_PERF, "[%s] bc = %d cc = %d", hw_mgr->hw_mgr_name,
			hw_mgr_clk_info->base_clk, hw_mgr_clk_info->curr_clk);
		return true;
	}
   ...
   */
	for (i = 0; i < ctx_data->clk_info.num_paths; i++) {
		...
		CAM_DBG(CAM_PERF,
			...
			ctx_data->ctx_id_string, i, path_index,
			cam_cpas_axi_util_trans_type_to_string(
				     ...
				     int idx)
{
	...
	if (!ctx_data->clk_info.rt_flag &&
		...
		base_clk = cam_icp_mgr_calc_base_clk(
			request_clk_info->frame_cycles,
			request_clk_info->budget_ns);

	if (busy)
		rc = cam_icp_update_clk_busy(hw_mgr, ctx_data, dev_clk_info,
					     request_clk_info, base_clk);
	else
		rc = cam_icp_update_clk_free(hw_mgr, ctx_data, dev_clk_info,
					     request_clk_info, base_clk);

	CAM_DBG(CAM_PERF, "%s: bc = %d cc = %d busy = %d overclk = %d uc = %d",
		ctx_data->ctx_id_string, dev_clk_info->base_clk,
		dev_clk_info->curr_clk, busy, dev_clk_info->over_clked, rc);
				    ...
				    int idx)
{
	...
	if (ctx_data->bw_config_version == CAM_ICP_BW_CONFIG_V1) {
		...
		CAM_DBG(CAM_PERF, "%s: Req[%lld] Current camno=%lld, mnoc=%lld",
			...
			dev_clk_info->uncompressed_bw,
			dev_clk_info->compressed_bw);

		bw_updated = cam_icp_update_bw(hw_mgr, ctx_data, dev_clk_info,
					       request_clk_info, busy);
	} else if (ctx_data->bw_config_version == CAM_ICP_BW_CONFIG_V2) {
		request_clk_info_v2 =
			&ctx_data->hfi_frame_process.clk_info_v2[idx];

		CAM_DBG(CAM_PERF, "%s: index=%d, num_paths=%d, ctx_data=%pK",
			ctx_data->ctx_id_string, idx,
			request_clk_info_v2->num_paths, ctx_data);

		bw_updated = cam_icp_update_bw_v2(hw_mgr, ctx_data,
						  dev_clk_info,
						  request_clk_info_v2, busy);

		for (i = 0; i < dev_clk_info->num_paths; i++) {
			CAM_DBG(CAM_PERF,
				"%s: Final path_type: %s, transac_type: %s, camnoc_bw = %lld "
				"mnoc_ab_bw = %lld, mnoc_ib_bw = %lld",
				ctx_data->ctx_id_string,
				cam_cpas_axi_util_path_type_to_string(
					dev_clk_info->axi_path[i]
						.path_data_type),
				cam_cpas_axi_util_trans_type_to_string(
				   ...
				   struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	scnprintf(tmp_buff, sizeof(tmp_buff), "%s Before clk update rate=%u",
		  ctx_data->ctx_id_string, dev_clk_info->prev_clk);
	...
	CAM_DBG(CAM_PERF, "%s: clk_rate %u", ctx_data->ctx_id_string,
		curr_clk_rate);
				    ...
				    struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	device_share_ratio = cam_icp_get_bw_device_share_ratio(
		hw_mgr, ctx_data->device_info);
	if (device_share_ratio < 0) {
		CAM_ERR(CAM_ICP, "%s: Fail to get device share ratio",
			ctx_data->ctx_id_string);
		return -EINVAL;
	...
	for (i = 0; i < ctx_data->device_info->hw_dev_cnt; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_PERF,
				"%s: Failed in updating cpas vote for %s cell idx: %u, rc=%d",
				ctx_data->ctx_id_string,
				ctx_data->device_info->dev_name, i, rc);
		}
	}
...
static inline int
cam_icp_mgr_get_core_info_mask(enum cam_icp_hw_type hw_dev_type,
			       uint32_t dev_num, uint32_t *core_info_mask)
{
	if (dev_num > 2) {
		CAM_ERR(CAM_ICP, "%u devices core info mask is not supported",
			dev_num);
		return -EINVAL;
	}
	...
	case CAM_ICP_DEV_OFE:
		*core_info_mask = ICP_PWR_CLP_OFE;
		break;
	default:
		CAM_ERR(CAM_ICP, "Invalid hw device type: %u", hw_dev_type);
		return -EINVAL;
	}
				     ...
				     struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	for (i = 0; i < dev_info->hw_dev_cnt; i++) {
		...
		if (hw_mgr->dev_pc_flag) {
			dev_intf->hw_ops.process_cmd(
				dev_intf->hw_priv, CAM_ICP_DEV_CMD_POWER_RESUME,
				NULL, 0);
		}
	}

	rc = cam_icp_mgr_get_core_info_mask(hw_dev_type, dev_info->hw_dev_cnt,
					    &core_info_mask);
	if (rc) {
		CAM_ERR(CAM_ICP,
	...
	CAM_DBG(CAM_PERF, "%s core_info 0x%x", ctx_data->ctx_id_string,
		core_info_mask);
					  ...
					  int dev_type)
{
	...
	CAM_DBG(CAM_PERF, "%s: device %s ctx cnt: %u", ctx_data->ctx_id_string,
		dev_info->dev_name, dev_info->dev_ctx_info.dev_ctxt_cnt);
...
static int cam_icp_mgr_dev_get_gdsc_control(struct cam_icp_hw_mgr *hw_mgr)
{
	int rc = 0, i, j;
...
DEFINE_DEBUGFS_ATTRIBUTE(cam_icp_debug_fw_dump,
			 cam_icp_get_icp_fw_hang_dump_lvl,
...
DEFINE_DEBUGFS_ATTRIBUTE(cam_icp_debug_fw_ramdump,
			 cam_icp_get_icp_fw_ramdump_lvl,
			 cam_icp_set_icp_fw_ramdump_lvl, "%08llu");

#ifdef CONFIG_CAM_TEST_ICP_FW_DOWNLOAD
static ssize_t cam_icp_hw_mgr_fw_load_unload(struct file *file,
					     ...
					     size_t size, loff_t *loff_t)
{
	...
	if (strcmp(input_buf, "load\n") == 0) {
		...
		CAM_WARN(CAM_ICP, "[%s] Invalid input: %s", hw_mgr->hw_mgr_name,
			 input_buf);
	}
...
static int cam_icp_test_irq_line(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (rc)
		CAM_ERR(CAM_ICP, "[%s] failed to verify IRQ line",
			hw_mgr->hw_mgr_name);
...
static int cam_icp_test_irq_line(struct cam_icp_hw_mgr *hw_mgr)
{
	CAM_ERR(CAM_ICP, "[%s] IRQ line verification disabled",
		hw_mgr->hw_mgr_name);
	return -EPERM;
}
...
static int cam_icp_hw_mgr_create_debugfs_entry(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "[%s] DebugFS could not create directory",
			hw_mgr->hw_mgr_name);
		rc = -ENOENT;
		goto end;
	}
	...
	debugfs_create_file("icp_debug_clk", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_debug_default_clk);

	debugfs_create_bool("icp_jtag_debug", 0644, hw_mgr->dentry,
			    &hw_mgr->icp_jtag_debug);

	debugfs_create_file("icp_debug_type", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_debug_type_fs);

	debugfs_create_file("icp_debug_lvl", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_debug_fs);

	debugfs_create_file("icp_fw_dump_lvl", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_debug_fw_dump);

	debugfs_create_file("icp_fw_ramdump_lvl", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_debug_fw_ramdump);

	debugfs_create_bool("disable_ubwc_comp", 0644, hw_mgr->dentry,
			    &hw_mgr->disable_ubwc_comp);

#ifdef CONFIG_CAM_TEST_ICP_FW_DOWNLOAD
	debugfs_create_file("icp_fw_load_unload", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_hw_mgr_fw_load_options);
#endif
	debugfs_create_file("test_irq_line", 0644, hw_mgr->dentry, hw_mgr,
			    &cam_icp_irq_line_test);
...
static const char *cam_icp_error_handle_id_to_type(uint32_t error_handle)
{
...
static inline void
cam_icp_mgr_apply_evt_injection(struct cam_hw_done_event_data *buf_done_data,
				struct cam_icp_hw_ctx_data *ctx_data,
...
static void cam_icp_mgr_dump_active_req_info(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	CAM_INFO(CAM_ICP,
		 ...
		 hw_mgr->hw_mgr_name, total_active_streams,
		 total_active_requests);
}

static void
cam_icp_mgr_compute_fw_avg_response_time(struct cam_icp_hw_ctx_data *ctx_data,
					 uint32_t request_idx)
{
	struct cam_icp_ctx_perf_stats *perf_stats;
	uint64_t delta;

	delta = ktime_ms_delta(
		ktime_get(),
	...
	CAM_DBG(CAM_PERF,
		"%s: Avg response time: current_req: %llu total_processed_requests: "
		"%llu avg_time: %llums",
		ctx_data->ctx_id_string,
		ctx_data->hfi_frame_process.request_id[request_idx],
		perf_stats->total_requests,
...
static int cam_icp_mgr_handle_frame_process(uint32_t *msg_ptr, int flag)
{
	...
	cam_icp_ctx_timer_reset(ctx_data);
	if (ctx_data->state != CAM_ICP_CTX_STATE_ACQUIRED) {
		CAM_DBG(CAM_ICP, "%s: is in %d state", ctx_data->ctx_id_string,
			ctx_data->state);
		mutex_unlock(&ctx_data->ctx_mutex);
		return 0;
	}

	CAM_DBG(CAM_REQ, "%s: request_id: %lld", ctx_data->ctx_id_string,
		request_id);
	...
	if (flag == ICP_FRAME_PROCESS_FAILURE) {
		...
		buf_data.evt_param = cam_icp_handle_err_type_to_evt_param(
			ioconfig_ack->err_type);
	} else {
	...
	CAM_TRACE(CAM_ICP, "%s: BufDone Req %llu event_id %d",
		  ctx_data->ctx_id_string, hfi_frame_process->request_id[idx],
		  event_id);
	...
	if ((ctx_data->evt_inject_params.is_valid) &&
		...
		cam_icp_mgr_apply_evt_injection(&buf_data, ctx_data,
						&signal_fence_buffer);
	...
	if (cam_presil_mode_enabled()) {
		if (!atomic_read(&hw_mgr->frame_in_process)) {
			CAM_ERR(CAM_PRESIL,
				"%s: presil: frame_in_process not set",
				ctx_data->ctx_id_string);
		} else {
			...
			CAM_DBG(CAM_PRESIL,
				"%s: presil: unlocked frame_in_process",
				ctx_data->ctx_id_string);
		}
	}
...
static int cam_icp_mgr_process_msg_config_io(uint32_t *msg_ptr)
{
	...
	if (ioconfig_ack->opcode == HFI_IPEBPS_CMD_OPCODE_IPE_CONFIG_IO) {
		...
		if (!ctx_data) {
			CAM_ERR(CAM_ICP,
				"wrong ctx data from BPS config io response");
			return -EINVAL;
		}
...
static int cam_icp_mgr_process_msg_create_handle(uint32_t *msg_ptr)
{
	...
	if (ctx_data->state == CAM_ICP_CTX_STATE_IN_USE) {
		...
		CAM_DBG(CAM_ICP, "%s: fw_handle = %x", ctx_data->ctx_id_string,
			ctx_data->fw_handle);
	} else {
		CAM_WARN(CAM_ICP,
...
static int cam_icp_mgr_process_ofe_indirect_ack_msg(uint32_t *msg_ptr)
{
	...
	case HFI_OFE_CMD_OPCODE_CONFIG_IO: {
		...
		if (!ctx_data) {
			CAM_ERR(CAM_ICP,
				"wrong ctx data from OFE config io response");
			return -EINVAL;
		}
	...
	case HFI_OFE_CMD_OPCODE_ABORT: {
		...
		if (cam_presil_mode_enabled()) {
			hw_mgr = ctx_data->hw_mgr_priv;
			if (atomic_read(&hw_mgr->frame_in_process)) {
				if (hw_mgr->frame_in_process_ctx_id ==
				    ctx_data->ctx_id) {
					CAM_DBG(CAM_PRESIL,
						"presil: frame process abort ctx %d",
						hw_mgr->frame_in_process_ctx_id);
					hw_mgr->frame_in_process_ctx_id = -1;
					atomic_set(&hw_mgr->frame_in_process,
						   0);
					up_write(&frame_in_process_sem);
				} else {
					CAM_WARN(
...
static int cam_icp_mgr_trigger_recovery(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	for (i = 0; i < CAM_ICP_CTX_MAX; i++) {
		...
		ctx_data->ctxt_event_cb(ctx_data->context_priv,
					CAM_ICP_EVT_ID_ERROR, &icp_err_evt);
		mutex_unlock(&ctx_data->ctx_mutex);
		break;
	}
	mutex_unlock(&hw_mgr->hw_mgr_mutex);
					   ...
					   uint32_t *msg_ptr)
{
	...
	if (!event_notify) {
		CAM_ERR(CAM_ICP, "[%s] Empty event message",
			hw_mgr->hw_mgr_name);
		return -EINVAL;
	}

	CAM_DBG(CAM_ICP, "[%s] evt_id: %u evt_data1: %u evt_data2: %u",
		hw_mgr->hw_mgr_name, event_notify->event_id,
		event_notify->event_data1, event_notify->event_data2);
					...
					uint32_t *msg_processed_len)
{
	...
	case HFI_MSG_SYS_INIT_DONE:
		CAM_DBG(CAM_ICP, "[%s] received SYS_INIT_DONE",
			hw_mgr->hw_mgr_name);
	...
	case HFI_MSG_SYS_PING_ACK:
		CAM_DBG(CAM_ICP, "[%s] received SYS_PING_ACK",
			hw_mgr->hw_mgr_name);
	...
	case HFI_MSG_IPEBPS_ASYNC_COMMAND_DIRECT_ACK:
		CAM_DBG(CAM_ICP, "[%s] received ASYNC_DIRECT_ACK",
			hw_mgr->hw_mgr_name);
	...
	case HFI_MSG_EVENT_NOTIFY:
		CAM_DBG(CAM_ICP, "[%s] received EVENT_NOTIFY",
			hw_mgr->hw_mgr_name);
	...
	case HFI_MSG_DBG_SYNX_TEST:
		...
		break;
	default:
		CAM_ERR(CAM_ICP, "[%s] invalid msg : %u", hw_mgr->hw_mgr_name,
...
static int32_t cam_icp_mgr_process_msg(void *priv, void *data)
{
	...
	if (!rc) {
		...
		temp_read_buf =
			kzalloc(sizeof(uint32_t) * temp_read_size, GFP_KERNEL);
		if (!temp_read_buf) {
			rc = -ENOMEM;
			goto end;
		}
	...
	while (true) {
		...
		if (read_len > 0) {
			msg_ptr += (msg_processed_len >> BYTE_WORD_SHIFT);
			msg_processed_len = 0;
		} else {
			break;
		}
...
static void cam_icp_free_hfi_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (hw_mgr->hfi_mem.fw_uncached_region) {
		...
		if (rc)
			CAM_ERR(CAM_ICP,
				"[%s] failed to unreserve fwuncached region",
				hw_mgr->hw_mgr_name);
		hw_mgr->hfi_mem.fw_uncached_region = false;
	} else {
		rc = cam_mem_mgr_free_memory_region(&hw_mgr->hfi_mem.sec_heap);
		if (rc)
			CAM_ERR(CAM_ICP, "[%s] failed to unreserve sec heap",
				hw_mgr->hw_mgr_name);
	...
	cam_smmu_unmap_phy_mem_region(hw_mgr->iommu_hdl, CAM_SMMU_REGION_QDSS,
				      0);

	/* Skip freeing if not mapped */
	if (hw_mgr->synx_signaling_en) {
		cam_smmu_unmap_phy_mem_region(
			hw_mgr->iommu_hdl, CAM_SMMU_REGION_FWUNCACHED,
				     ...
				     struct cam_mem_mgr_memory_desc *secheap)
{
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "[%s] Unable to get secheap memory info",
			hw_mgr->hw_mgr_name);
		return rc;
	}
	...
	rc = cam_mem_mgr_reserve_memory_region(&alloc, CAM_SMMU_REGION_SECHEAP,
					       &out);
	if (rc) {
		CAM_ERR(CAM_ICP, "[%s] Unable to reserve secheap memory",
			hw_mgr->hw_mgr_name);
		return rc;
	}
	...
	*secheap = out;
	CAM_DBG(CAM_ICP, "[%s] kva: %llX, iova: %x, hdl: %x, len: %lld",
		hw_mgr->hw_mgr_name, out.kva, out.iova, out.mem_handle,
		out.len);
...
static int cam_icp_allocate_fw_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	rc = cam_smmu_alloc_firmware(hw_mgr->iommu_hdl, &iova, &kvaddr, &len);
	if (rc) {
	...
	CAM_DBG(CAM_ICP, "[%s], kva: %zX, iova: %llx, len: %zu",
		hw_mgr->hw_mgr_name, kvaddr, iova, len);
...
static int cam_icp_allocate_qdss_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	CAM_DBG(CAM_ICP, "[%s] iova: %llx, len: %zu", hw_mgr->hw_mgr_name, iova,
		len);
...
static int
cam_icp_allocate_device_synx_hwmutex_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	rc = cam_smmu_map_phy_mem_region(hw_mgr->iommu_hdl,
					 CAM_SMMU_REGION_DEVICE,
					 CAM_SMMU_SUBREGION_SYNX_HWMUTEX, &iova,
					 &len);
	if (rc) {
		CAM_ERR(CAM_ICP, "Failed in allocating hwmutex mem rc %d", rc);
		return rc;
	}
...
static int cam_icp_allocate_device_global_cnt_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	rc = cam_smmu_map_phy_mem_region(hw_mgr->iommu_hdl,
					 CAM_SMMU_REGION_DEVICE,
					 CAM_SMMU_SUBREGION_GLOBAL_CNTR, &iova,
					 &len);
	if (rc) {
		CAM_ERR(CAM_ICP, "Failed in allocating global cntr mem rc %d",
			rc);
		return rc;
	}
...
static int
cam_icp_allocate_device_ipc_hwmutex_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	rc = cam_smmu_map_phy_mem_region(hw_mgr->iommu_hdl,
					 CAM_SMMU_REGION_DEVICE,
					 CAM_SMMU_SUBREGION_IPC_HWMUTEX, &iova,
					 &len);
	if (rc) {
		CAM_ERR(CAM_ICP, "Failed in allocating hwmutex mem rc %d", rc);
		return rc;
...
static int
cam_icp_allocate_mem_for_fence_signaling(struct cam_icp_hw_mgr *hw_mgr)
{
	int rc;

	rc = cam_smmu_get_region_info(hw_mgr->iommu_hdl, CAM_SMMU_REGION_DEVICE,
				      &hw_mgr->hfi_mem.device);
	if (rc) {
...
static int cam_icp_get_io_mem_info(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	rc = cam_smmu_get_io_region_info(hw_mgr->iommu_hdl, &iova, &len,
					 &discard_iova_start,
					 &discard_iova_len);
	if (rc)
		return rc;
	...
	CAM_DBG(CAM_ICP, "[%s] iova: %llx, len: %zu discard iova %llx len %llx",
		hw_mgr->hw_mgr_name, iova, len, discard_iova_start,
		discard_iova_len);
...
static int cam_icp_allocate_hfi_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	size_t qtbl_size, cmdq_size, msgq_size, dbgq_size, sfr_size,
		sec_heap_size;

	rc = cam_smmu_get_region_info(hw_mgr->iommu_hdl, CAM_SMMU_REGION_SHARED,
				      &hw_mgr->hfi_mem.shmem);
	if (rc) {
   ...
   */
	...
	sec_heap_size =
		ALIGN(ICP_SEC_HEAP_SIZE_IN_BYTES, PAGE_SIZE) + PAGE_SIZE;
     ...
     */
		...
		alloc.smmu_hdl = hw_mgr->iommu_hdl;
		rc = cam_mem_mgr_reserve_memory_region(
			&alloc, CAM_SMMU_REGION_FWUNCACHED, &out);
		if (rc) {
			CAM_ERR(CAM_ICP,
				"[%s] Unable to reserve fw uncached memory rc %d",
				hw_mgr->hw_mgr_name, rc);
     ...
     */
		...
		rc = cam_icp_alloc_shared_mem(hw_mgr, qtbl_size,
					      &hw_mgr->hfi_mem.qtbl);
		if (rc) {
			CAM_ERR(CAM_ICP,
				"[%s] Unable to allocate qtbl memory, rc %d",
				hw_mgr->hw_mgr_name, rc);
			goto qtbl_alloc_failed;
		}

		rc = cam_icp_alloc_shared_mem(hw_mgr, cmdq_size,
					      &hw_mgr->hfi_mem.cmd_q);
		if (rc) {
			CAM_ERR(CAM_ICP,
		...
		rc = cam_icp_alloc_shared_mem(hw_mgr, msgq_size,
					      &hw_mgr->hfi_mem.msg_q);
		if (rc) {
			CAM_ERR(CAM_ICP,
		...
		rc = cam_icp_alloc_shared_mem(hw_mgr, dbgq_size,
					      &hw_mgr->hfi_mem.dbg_q);
		if (rc) {
			CAM_ERR(CAM_ICP,
		...
		rc = cam_icp_alloc_shared_mem(hw_mgr, sfr_size,
					      &hw_mgr->hfi_mem.sfr_buf);
		if (rc) {
			CAM_ERR(CAM_ICP,
		...
		rc = cam_icp_alloc_secheap_mem(hw_mgr,
	...
	CAM_DBG(CAM_ICP,
		...
		hw_mgr->hw_mgr_name, hw_mgr->hfi_mem.shmem.iova_start,
		hw_mgr->hfi_mem.shmem.iova_len,
		fwuncached_region_info.iova_start,
		fwuncached_region_info.iova_len);

	CAM_DBG(CAM_ICP,
		"[%s] FwUncached[0x%x %lld] FwUncached_Generic[0x%x %p %lld] "
		"QTbl[0x%x %p %lld] CmdQ[0x%x %p %lld] MsgQ[0x%x %p %lld]",
		hw_mgr->hw_mgr_name, hw_mgr->hfi_mem.fw_uncached.iova_start,
		...
		hw_mgr->hfi_mem.qtbl.iova, hw_mgr->hfi_mem.qtbl.kva,
		hw_mgr->hfi_mem.qtbl.len, hw_mgr->hfi_mem.cmd_q.iova,
		hw_mgr->hfi_mem.cmd_q.kva, hw_mgr->hfi_mem.cmd_q.len,
		hw_mgr->hfi_mem.msg_q.iova, hw_mgr->hfi_mem.msg_q.kva,
		hw_mgr->hfi_mem.msg_q.len);

	CAM_DBG(CAM_ICP,
		"[%s] DbgQ[0x%x %p %lld] SFR[0x%x %p %lld] SecHeap[0x%x %p %lld]",
		hw_mgr->hw_mgr_name, hw_mgr->hfi_mem.dbg_q.iova,
		hw_mgr->hfi_mem.dbg_q.kva, hw_mgr->hfi_mem.dbg_q.len,
		hw_mgr->hfi_mem.sfr_buf.iova, hw_mgr->hfi_mem.sfr_buf.kva,
		hw_mgr->hfi_mem.sfr_buf.len, hw_mgr->hfi_mem.sec_heap.iova,
		hw_mgr->hfi_mem.sec_heap.kva, hw_mgr->hfi_mem.sec_heap.len);
...
static int cam_icp_mgr_send_pc_prep(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	rem_jiffies = CAM_COMMON_WAIT_FOR_COMPLETION_TIMEOUT_ERRMSG(
		...
		"[%s] FW response timeout for PC PREP handle command",
		hw_mgr->hw_mgr_name);
	if (!rem_jiffies)
		rc = -ETIMEDOUT;
	...
	for (i = 0; i < hw_mgr->num_dev_info; i++) {
		...
		for (j = 0; j < dev_info->hw_dev_cnt; j++) {
			if (dev_info->dev_ctx_info.dev_clk_state) {
				...
				rc = dev_intf->hw_ops.deinit(dev_intf->hw_priv,
							     NULL, 0);
				if (rc) {
					CAM_ERR(CAM_ICP,
						"[%s] %s[%u] failed to deinit rc: %d",
						hw_mgr->hw_mgr_name,
						dev_info->dev_name, j, rc);
				}
				dev_info->dev_ctx_info.dev_clk_state = false;
			}
		}
...
static void cam_icp_mgr_proc_suspend(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	rc = icp_dev_intf->hw_ops.process_cmd(
		icp_dev_intf->hw_priv, CAM_ICP_CMD_POWER_COLLAPSE, NULL, 0);
	if (rc)
...
static int cam_icp_mgr_icp_power_collapse(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (rc)
		CAM_ERR(CAM_ICP, "[%s] Fail to deinit ICP",
			hw_mgr->hw_mgr_name);
...
static int cam_icp_mgr_proc_boot(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (!icp_dev_intf) {
		CAM_ERR(CAM_ICP, "[%s] invalid device interface",
			hw_mgr->hw_mgr_name);
		return -EINVAL;
	}
					      ...
					      struct hfi_mem_info *hfi_mem)
{
	...
	CAM_DBG(CAM_ICP, "[%s] qtbl kva = %llx IOVA = %x length = %lld\n",
		hw_mgr->hw_mgr_name, hfi_mem->qtbl.kva, hfi_mem->qtbl.iova,
		hfi_mem->qtbl.len);
	...
	CAM_DBG(CAM_ICP, "[%s] cmd_q kva = %llx IOVA = %x length = %lld\n",
		hw_mgr->hw_mgr_name, hfi_mem->cmd_q.kva, hfi_mem->cmd_q.iova,
		hfi_mem->cmd_q.len);
	...
	CAM_DBG(CAM_ICP, "[%s] msg_q kva = %llx IOVA = %x length = %lld\n",
		hw_mgr->hw_mgr_name, hfi_mem->msg_q.kva, hfi_mem->msg_q.iova,
		hfi_mem->msg_q.len);
	...
	CAM_DBG(CAM_ICP, "[%s] dbg_q kva = %llx IOVA = %x length = %lld\n",
		hw_mgr->hw_mgr_name, hfi_mem->dbg_q.kva, hfi_mem->dbg_q.iova,
		hfi_mem->dbg_q.len);
	...
	CAM_DBG(CAM_ICP, "[%s] sfr kva = %llx IOVA = %x length = %lld\n",
		hw_mgr->hw_mgr_name, hfi_mem->sfr_buf.kva,
	...
	if (hw_mgr->synx_signaling_en) {
		...
		CAM_DBG(CAM_ICP, "device memory [iova = 0x%llx len = 0x%llx]",
	...
	if (hw_mgr->hfi_mem.io_mem.discard_iova_start &&
		...
		hfi_mem->io_mem2.iova =
			hw_mgr->hfi_mem.io_mem.discard_iova_start +
			hw_mgr->hfi_mem.io_mem.discard_iova_len;
		hfi_mem->io_mem2.len = hw_mgr->hfi_mem.io_mem.iova_start +
	...
	CAM_DBG(CAM_ICP,
		"[%s] IO region1 IOVA = %X length = %lld, IO region2 IOVA = %X "
		"length = %lld",
		hw_mgr->hw_mgr_name, hfi_mem->io_mem.iova, hfi_mem->io_mem.len,
...
static int
cam_icp_mgr_populate_abort_cmd(struct cam_icp_hw_ctx_data *ctx_data,
			       struct hfi_cmd_dev_async **abort_cmd_ptr)
{
	...
	case CAM_ICP_DEV_BPS:
		...
		packet_size =
			packet_size + sizeof(struct hfi_cmd_abort) -
	...
	case CAM_ICP_DEV_OFE:
		...
		break;
	default:
		CAM_ERR(CAM_ICP, "%s: Invalid device type not supported: %u",
			ctx_data->ctx_id_string,
			ctx_data->device_info->hw_dev_type);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ICP, "%s: abort pkt size = %d", ctx_data->ctx_id_string,
		(int)packet_size);
...
static int cam_icp_mgr_abort_handle(struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	CAM_DBG(CAM_ICP, "%s: fw_handle = 0x%x ctx_data = %pK",
		ctx_data->ctx_id_string, ctx_data->fw_handle, ctx_data);
	rem_jiffies = CAM_COMMON_WAIT_FOR_COMPLETION_TIMEOUT_ERRMSG(
		&ctx_data->wait_complete, msecs_to_jiffies(timeout), CAM_ICP,
...
static int cam_icp_mgr_destroy_handle(struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	case CAM_ICP_DEV_BPS:
		pkt_type = HFI_CMD_IPEBPS_ASYNC_COMMAND_DIRECT;
		packet_size =
			packet_size + sizeof(struct hfi_cmd_destroy) -
			sizeof(((struct hfi_cmd_dev_async *)0)->payload.direct);
...
static int cam_icp_mgr_release_ctx(struct cam_icp_hw_mgr *hw_mgr, int ctx_id)
{
	...
	CAM_DBG(CAM_PERF,
		"Avg response time on %s: total_processed_requests: %llu avg_time: "
		"%llums",
		hw_mgr->ctx_data[ctx_id].ctx_id_string,
		perf_stats->total_requests,
		perf_stats->total_requests ? (perf_stats->total_resp_time /
					      perf_stats->total_requests) :
					     0);
	...
	if (hw_mgr->ctx_data[ctx_id].state != CAM_ICP_CTX_STATE_ACQUIRED) {
		...
		CAM_DBG(CAM_ICP, "%s: Not in right state to release: %d",
			hw_mgr->ctx_data[ctx_id].ctx_id_string,
			hw_mgr->ctx_data[ctx_id].state);
		return 0;
	}
	cam_icp_mgr_dev_power_collapse(hw_mgr, &hw_mgr->ctx_data[ctx_id], 0);
						 ...
						 void *priv_data)
{
	...
	if (!hw_mgr) {
		CAM_ERR(CAM_ICP, "Uninitialized hw mgr for subdev: %u",
			hw_mgr_idx);
		return -EINVAL;
	}
	...
	for (i = 0; i < CAM_ICP_CTX_MAX; i++) {
		...
		md->num_context++;
		ctx_md = (struct cam_icp_hw_ctx_mini_dump *)((uint8_t *)dst +
							     dumped_len);
		md->ctx[i] = ctx_md;
		...
		memcpy(ctx_md->hfi_frame.request_id,
		       ctx->hfi_frame_process.request_id,
		       sizeof(ctx->hfi_frame_process.request_id));
		memcpy(ctx_md->hfi_frame.in_resource,
		       ctx->hfi_frame_process.in_resource,
		...
		memcpy(ctx_md->hfi_frame.num_out_res,
		       ctx->hfi_frame_process.num_out_resources,
		       sizeof(ctx->hfi_frame_process.num_out_resources));
		memcpy(ctx_md->hfi_frame.out_res,
		       ctx->hfi_frame_process.out_resource,
	...
	/* Dump fw image */
	if (!hw_mgr->icp_use_pil) {
		icp_dump_args.cpu_addr =
			(uintptr_t)((uint8_t *)dst + dumped_len);
		icp_dump_args.offset = 0;
		icp_dump_args.buf_len = remain_len;
		icp_dev_intf = hw_mgr->icp_dev_intf;
		rc = icp_dev_intf->hw_ops.process_cmd(
			icp_dev_intf->hw_priv, CAM_ICP_CMD_HW_MINI_DUMP,
			&icp_dump_args, sizeof(struct cam_icp_hw_dump_args));
		if (rc)
			goto end;
...
static void cam_icp_mgr_device_deinit(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	dev_intf->hw_ops.deinit(dev_intf->hw_priv, &send_freq_info,
				sizeof(send_freq_info));
}
...
static int cam_icp_mgr_hw_close(void *hw_priv, void *hw_close_args)
{
	...
	if (!hw_mgr->icp_booted) {
		CAM_DBG(CAM_ICP, "[%s] hw mgr is already closed",
			hw_mgr->hw_mgr_name);
		return 0;
	}

	if (!hw_mgr->icp_dev_intf) {
		CAM_DBG(CAM_ICP, "[%s] ICP device interface is NULL",
			hw_mgr->hw_mgr_name);
		return -EINVAL;
	}
...
static int cam_icp_mgr_device_init(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	rc = dev_intf->hw_ops.init(dev_intf->hw_priv, &send_freq_info,
				   sizeof(send_freq_info));
	if (rc) {
...
static int cam_icp_mgr_hfi_init(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (hw_mgr->synx_signaling_en) {
		...
		if (hw_mgr->hfi_mem.fw_uncached_global_sync.iova >=
		    hw_mgr->hfi_mem.fw_uncached_generic.iova) {
			CAM_ERR(CAM_ICP,
				"global sync memory [start: 0x%x] expected to be at the start of "
				"FW uncached [uncached_generic start: 0x%x]",
		...
		/*
     * Global sync memory is part of FW uncached region, but to FW remove this
     * entry from FW uncached to avoid it being mapped with FW uncached. Global
     * sync mem will be mapped with sharable attributes for IPC access, and
     * hence an independent mapping of it's own.
     */
		hfi_mem.fw_uncached.iova +=
			hw_mgr->hfi_mem.fw_uncached_global_sync.len;
		hfi_mem.fw_uncached.len -=
			hw_mgr->hfi_mem.fw_uncached_global_sync.len;
	}
...
static int cam_icp_mgr_send_memory_region_info(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	region_info =
		(struct hfi_cmd_config_mem_regions *)&set_prop->prop_data[1];
	if (hw_mgr->synx_signaling_en) {
		/* Update synx global mem */
		region_info->region_info[region_info->num_valid_regions]
			.region_id = HFI_MEM_REGION_ID_IPCLITE_SHARED_MEM;
		region_info->region_info[region_info->num_valid_regions]
			.region_type = HFI_MEM_REGION_TYPE_UNCACHED;
		region_info->region_info[region_info->num_valid_regions]
			.start_addr =
			hw_mgr->hfi_mem.fw_uncached_global_sync.iova;
		...
		region_info->region_info[region_info->num_valid_regions]
			.region_id = HFI_MEM_REGION_ID_SYNX_HW_MUTEX;
		region_info->region_info[region_info->num_valid_regions]
			.region_type = HFI_MEM_REGION_TYPE_DEVICE;
		region_info->region_info[region_info->num_valid_regions]
			.start_addr = hw_mgr->hfi_mem.synx_hwmutex.iova;
		region_info->region_info[region_info->num_valid_regions].size =
		...
		region_info->region_info[region_info->num_valid_regions]
			.region_type = HFI_MEM_REGION_TYPE_DEVICE;
		region_info->region_info[region_info->num_valid_regions]
			.start_addr = hw_mgr->hfi_mem.ipc_hwmutex.iova;
		region_info->region_info[region_info->num_valid_regions].size =
		...
		region_info->region_info[region_info->num_valid_regions]
			.region_id = HFI_MEM_REGION_ID_GLOBAL_CNTR;
		region_info->region_info[region_info->num_valid_regions]
			.region_type = HFI_MEM_REGION_TYPE_DEVICE;
		region_info->region_info[region_info->num_valid_regions]
			.start_addr = hw_mgr->hfi_mem.global_cntr.iova;
		region_info->region_info[region_info->num_valid_regions].size =
			hw_mgr->hfi_mem.global_cntr.len;

		region_info->num_valid_regions++;
		CAM_DBG(CAM_ICP,
			"Synx mem regions global_sync[0x%x:0x%x] synx_hw_mutex[0x%x:0x%x] "
			"ipc_hw_mutex[0x%x:0x%x] global_cntr[0x%x:0x%x]",
			hw_mgr->hfi_mem.fw_uncached_global_sync.iova,
			hw_mgr->hfi_mem.fw_uncached_global_sync.len,
...
static int cam_icp_mgr_hw_open_k(void *hw_mgr_priv, void *download_fw_args)
{
	...
	rc = cam_icp_mgr_hw_open(hw_mgr, download_fw_args);
	CAM_DBG(CAM_ICP, "[%s] hw_open from kmd done %d", hw_mgr->hw_mgr_name,
		rc);
...
static int cam_icp_mgr_icp_resume(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (!hw_mgr->icp_booted) {
		CAM_DBG(CAM_ICP, "[%s] booting ICP processor",
			hw_mgr->hw_mgr_name);
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "[%s] Failed to resume proc rc: %d",
			hw_mgr->hw_mgr_name, rc);
		goto hw_deinit;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "[%s] Failed to resume HFI rc: %d",
			hw_mgr->hw_mgr_name, rc);
		goto power_collapse;
	}
...
static int cam_icp_mgr_hw_open(void *hw_mgr_priv, void *download_fw_args)
{
	...
	if (hw_mgr->icp_booted) {
		CAM_DBG(CAM_ICP, "[%s] ICP already booted",
			hw_mgr->hw_mgr_name);
		return rc;
	}
	...
	if (rc) {
		...
		hw_mgr->icp_dev_intf->hw_ops.process_cmd(
			hw_mgr->icp_dev_intf->hw_priv, CAM_ICP_CMD_HW_REG_DUMP,
			&dump_type, sizeof(dump_type));
	...
	if (icp_pc && hw_mgr->icp_pc_flag) {
		...
		if (rc)
			CAM_ERR(CAM_ICP,
				"[%s] Failed in ipe bps deinit with icp_pc rc %d",
				hw_mgr->hw_mgr_name, rc);
	...
	CAM_DBG(CAM_ICP, "[%s] deinit all clocks at boot up",
		hw_mgr->hw_mgr_name);
				      ...
				      struct cam_hw_config_args *config_args)
{
	...
	CAM_DBG(CAM_ICP, "[%s] req_id = %lld %pK", hw_mgr->hw_mgr_name,
				      ...
				      uint32_t io_buf_addr)
{
	...
	task = cam_req_mgr_workq_get_task(hw_mgr->cmd_work);
	if (!task) {
		CAM_ERR_RATE_LIMIT(CAM_ICP, "%s: No free cmd task",
				   ctx_data->ctx_id_string);
		return -ENOMEM;
	}
	...
	CAM_DBG(CAM_ICP, "%s: size_in_words %u", ctx_data->ctx_id_string,
		size_in_words);
	rc = cam_req_mgr_workq_enqueue_task(task, hw_mgr, CRM_TASK_PRIORITY_0);
	if (rc) {
...
static int cam_icp_mgr_config_hw(void *hw_mgr_priv, void *config_hw_args)
{
	...
	if (cam_presil_mode_enabled()) {
		CAM_DBG(CAM_PRESIL,
			"%s: presil: locking frame_in_process %d req id %u",
			ctx_data->ctx_id_string,
			atomic_read(&hw_mgr->frame_in_process),
		...
		CAM_DBG(CAM_PRESIL,
			"%s: presil: locked frame_in_process req id %u ctx_id %d",
	...
	if (cam_presil_mode_enabled()) {
		...
		cam_presil_send_buffers_from_packet(
			frame_info->pkt, hw_mgr->iommu_hdl, hw_mgr->iommu_hdl);
	}
...
static int cam_icp_mgr_prepare_frame_process_cmd(
	struct cam_icp_hw_ctx_data *ctx_data, struct hfi_cmd_dev_async *hfi_cmd,
	uint64_t request_id, uint32_t fw_cmd_buf_iova_addr)
{
	switch (ctx_data->device_info->hw_dev_type) {
	case CAM_ICP_DEV_BPS:
	...
	CAM_DBG(CAM_ICP, "%s: ctx_data : %pK, request_id :%lld cmd_buf %x",
		ctx_data->ctx_id_string, (void *)ctx_data->context_priv,
...
static bool cam_icp_mgr_is_valid_inconfig(struct cam_packet *packet)
{
	...
	CAM_DBG(CAM_ICP, "number of io config: %u in config: %u max in res: %u",
		packet->num_io_configs, num_in_map_entries, CAM_MAX_IN_RES);
...
static bool cam_icp_mgr_is_valid_outconfig(struct cam_packet *packet)
{
	...
	CAM_DBG(CAM_ICP,
		...
		packet->num_io_configs, num_out_map_entries, CAM_MAX_OUT_RES);
				      ...
				      struct cam_packet *packet)
{
	...
	if (((op_code != CAM_ICP_OPCODE_IPE_UPDATE) &&
		...
		CAM_ERR(CAM_ICP, "%s: Invalid Opcode in pkt: %d device: %s",
			ctx_data->ctx_id_string, op_code,
			ctx_data->device_info->dev_name);
		return -EINVAL;
	}
	...
	if (!packet->num_cmd_buf ||
		...
		CAM_ERR(CAM_ICP,
			"%s: Invalid number of cmd buffers: %d max cmd buf: %d",
	...
	CAM_DBG(CAM_ICP, "%s: number of cmd/patch info: %u %u %u",
		ctx_data->ctx_id_string, packet->num_cmd_buf,
		packet->num_io_configs, packet->num_patches);
					...
					struct list_head *buf_tracker)
{
	...
	*fw_cmd_buf_iova_addr = 0;
	...
	if (!cpu_addr) {
		CAM_ERR(CAM_ICP, "%s: invalid number of cmd buf",
			ctx_data->ctx_id_string);
		return -EINVAL;
	}
...
static int cam_icp_process_presil_hangdump_info(
	struct cam_icp_hw_ctx_data *ctx_data,
	struct cam_cmd_mem_regions *cmd_mem_regions, uint32_t index)
{
	int i = 0;
	struct cam_hangdump_mem_regions *mem_regions = NULL;
	...
	CAM_INFO(CAM_ICP,
		 ...
		 ctx_data->ctx_id_string, cmd_mem_regions->num_regions, index,
		 mem_regions);
					       ...
					       uint8_t *blob_data)
{
	...
	case CAM_ICP_CMD_GENERIC_BLOB_CLK:
		...
		*clk_info = *soc_req;
		CAM_DBG(CAM_PERF, "%s: budget:%llu fc: %llu %d BW %lld %lld",
			ctx_data->ctx_id_string, clk_info->budget_ns,
			clk_info->frame_cycles, clk_info->rt_flag,
			clk_info->uncompressed_bw, clk_info->compressed_bw);
		break;
	...
	case CAM_ICP_CMD_GENERIC_BLOB_CLK_V2:
		...
		for (i = 0; i < soc_req_v2->num_paths; i++) {
			clk_info_v2->axi_path[i].usage_data =
				soc_req_v2->axi_path[i].usage_data;
			...
			clk_info_v2->axi_path[i].camnoc_bw =
				soc_req_v2->axi_path[i].camnoc_bw;
			clk_info_v2->axi_path[i].mnoc_ab_bw =
				soc_req_v2->axi_path[i].mnoc_ab_bw;
			clk_info_v2->axi_path[i].mnoc_ib_bw =
				soc_req_v2->axi_path[i].mnoc_ib_bw;
		}
		...
		CAM_DBG(CAM_PERF,
			"%s: budget=%llu, frame_cycle=%llu, rt_flag=%d, num_paths=%d, "
			"index=%d, ctx_data=%pK",
			ctx_data->ctx_id_string, clk_info_v2->budget_ns,
			clk_info_v2->frame_cycles, clk_info_v2->rt_flag,
			clk_info_v2->num_paths, index, ctx_data);

		for (i = 0; i < clk_info_v2->num_paths; i++) {
			CAM_DBG(CAM_PERF,
				"%s: [%d] : path_type=%d, trans_type=%d, camnoc=%lld, "
				"mnoc_ab=%lld, mnoc_ib=%lld",
				ctx_data->ctx_id_string, i,
				clk_info_v2->axi_path[i].path_data_type,
				clk_info_v2->axi_path[i].transac_type,
				clk_info_v2->axi_path[i].camnoc_bw,
	...
	case CAM_ICP_CMD_GENERIC_BLOB_CFG_IO:
		CAM_DBG(CAM_ICP, "%s: CAM_ICP_CMD_GENERIC_BLOB_CFG_IO",
			ctx_data->ctx_id_string);
		pResource = *((uint32_t *)blob_data);
		if (copy_from_user(&dev_io_info, (void __user *)pResource,
				   sizeof(struct cam_icp_acquire_dev_info))) {
			CAM_ERR(CAM_ICP, "%s: Failed in copy from user",
				ctx_data->ctx_id_string);
			return -EFAULT;
		}
		CAM_DBG(CAM_ICP, "%s: buf handle %d", ctx_data->ctx_id_string,
			dev_io_info.io_config_cmd_handle);
		rc = cam_mem_get_io_buf(dev_io_info.io_config_cmd_handle,
					hw_mgr->iommu_hdl, blob->io_buf_addr,
					&io_buf_size, NULL, NULL);
		if (rc)
			CAM_ERR(CAM_ICP, "%s: Failed in blob update",
				ctx_data->ctx_id_string);
		else
	...
	case CAM_ICP_CMD_GENERIC_BLOB_PRESIL_HANGDUMP:
		if (cam_presil_mode_enabled()) {
			...
			if (cmd_mem_regions->num_regions <= 0) {
				...
				rc = cam_icp_process_presil_hangdump_info(
					ctx_data, cmd_mem_regions, index);
			}
		}
		break;

	default:
		CAM_WARN(CAM_ICP, "%s: Invalid blob type %d",
			 ctx_data->ctx_id_string, blob_type);
		break;
	}
	return rc;
}
	...
	struct cam_hw_prepare_update_args *prepare_args, int32_t *idx)
{
	...
	if (ctx_data->hfi_frame_process.frame_info[index].io_config) {
		hfi_cmd =
			(struct hfi_cmd_dev_async *)&ctx_data->hfi_frame_process
				.frame_info[index]
				.hfi_cfg_io_cmd;
					 ...
					 void *prepare_hw_update_args)
{
	...
	hfi_cmd = (struct hfi_cmd_dev_async *)&ctx_data->hfi_frame_process
			  .hfi_frame_cmd[idx];
...
static int cam_icp_mgr_enqueue_abort(struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	CAM_DBG(CAM_ICP, "%s: Abort after flush is success",
		ctx_data->ctx_id_string);
	return 0;
}
...
static int cam_icp_mgr_hw_dump(void *hw_priv, void *hw_dump_args)
{
	...
	if ((!hw_priv) || (!hw_dump_args)) {
		CAM_ERR(CAM_ICP, "Invalid params %pK %pK", hw_priv,
			hw_dump_args);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ICP, "[%s] Req %lld", hw_mgr->hw_mgr_name,
		dump_args->request_id);
	frm_process = &ctx_data->hfi_frame_process;
	for (i = 0; i < CAM_FRAME_CMD_MAX; i++) {
		if ((frm_process->request_id[i] == dump_args->request_id) &&
		    frm_process->fw_process_flag[i]) {
			frm_idx = i;
			break;
		}
	...
	if (frm_idx >= 0) {
		req_ts = ktime_to_timespec64(
			frm_process->submit_timestamp[frm_idx]);
	}

	CAM_INFO(CAM_ICP,
		 "[%s] Error req %lld req timestamp:[%lld.%06lld] curr "
		 "timestamp:[%lld.%06lld]",
		 hw_mgr->hw_mgr_name, dump_args->request_id, req_ts.tv_sec,
		 req_ts.tv_nsec / NSEC_PER_USEC, cur_ts.tv_sec,
		 cur_ts.tv_nsec / NSEC_PER_USEC);
	rc = cam_mem_get_cpu_buf(dump_args->buf_handle, &icp_dump_args.cpu_addr,
				 &icp_dump_args.buf_len);
	if (rc) {
	...
	if (icp_dump_args.buf_len <= dump_args->offset) {
		CAM_WARN(CAM_ICP,
			 "[%s] dump buffer overshoot len %zu offset %zu",
			 hw_mgr->hw_mgr_name, icp_dump_args.buf_len,
			 dump_args->offset);
		rc = -ENOSPC;
		goto put_cpu_buf;
	}
	...
	*addr++ = cur_ts.tv_nsec / NSEC_PER_USEC;
	...
	rc = icp_dev_intf->hw_ops.process_cmd(
		icp_dev_intf->hw_priv, CAM_ICP_CMD_HW_DUMP, &icp_dump_args,
...
static int
cam_icp_mgr_synx_core_control(struct cam_icp_hw_mgr *hw_mgr,
...
static int
cam_icp_mgr_synx_send_test_cmd(struct cam_icp_hw_mgr *hw_mgr,
			       struct cam_synx_test_cmd *synx_test_params)
{
	...
	CAM_DBG(CAM_ICP,
		"Input (hdl: 0x%x iova: 0x%x size: 0x%x) output (hdl: 0x%x iova: "
		"0x%x size: 0x%x)",
		synx_test_params->ip_mem_hdl, synx_test_cmd.input_iova,
		synx_test_cmd.input_size, synx_test_params->op_mem_hdl,
		synx_test_cmd.output_iova, synx_test_cmd.output_size);
...
static int cam_icp_mgr_service_synx_test_cmds(void *hw_priv, void *synx_args)
{
	...
	case CAM_SYNX_TEST_CMD_TYPE_CORE_CTRL: {
		rc = cam_icp_mgr_synx_core_control(hw_mgr,
						   &synx_params->u.core_ctrl);
	} break;
	case CAM_SYNX_TEST_CMD_TYPE_SYNX_CMD: {
		rc = cam_icp_mgr_synx_send_test_cmd(hw_mgr,
						    &synx_params->u.test_cmd);
	} break;
	default:
		rc = -EINVAL;
		goto end;
...
static int cam_icp_mgr_release_hw(void *hw_mgr_priv, void *release_hw_args)
{
	...
	CAM_DBG(CAM_ICP, "%s: Enter recovery set %d", ctx_data->ctx_id_string,
		atomic_read(&hw_mgr->recovery));
				     ...
				     struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	if (ctx_data->device_info->hw_dev_type == CAM_ICP_DEV_OFE) {
		...
		case CAM_ICP_RES_TYPE_OFE_SEMI_RT:
			...
			break;
		default:
			CAM_ERR(CAM_ICP, "Invalid OFE stream type: %u",
				dev_type);
			return -EINVAL;
		}
	} else {
		...
		case CAM_ICP_RES_TYPE_BPS_SEMI_RT:
			...
			break;
		default:
			CAM_ERR(CAM_ICP, "Invalid IPE/BPS stream type: %u",
				dev_type);
			return -EINVAL;
		}
	}
	...
	if (ctx_data->fw_handle == 0) {
		CAM_ERR(CAM_ICP, "%s: Invalid handle created",
			ctx_data->ctx_id_string);
		rc = -EINVAL;
	}
				    ...
				    struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	if (copy_from_user(&icp_dev_acquire_info,
			   ...
			   sizeof(struct cam_icp_acquire_dev_info))) {
		CAM_ERR(CAM_ICP, "%s: Failed in acquire",
			ctx_data->ctx_id_string);
		return -EFAULT;
	}
	...
	CAM_DBG(CAM_ICP, "%s: %x %x %x %x %x %x", ctx_data->ctx_id_string,
	...
	for (i = 0; i < icp_dev_acquire_info.num_out_res; i++)
		CAM_DBG(CAM_ICP, "%s: out[i] %x %x %x %x",
			ctx_data->ctx_id_string, p_icp_out[i].format,
...
static int cam_icp_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	if (!CAM_ICP_IS_VALID_HW_DEV_TYPE(hw_dev_type)) {
		CAM_ERR(CAM_ICP,
			"[%s] Fail to get hw device type from dev type: %u",
	...
	ctx_data->device_info =
		&hw_mgr->dev_info[hw_mgr->dev_info_idx[hw_dev_type]];

	scnprintf(ctx_data->ctx_id_string, sizeof(ctx_data->ctx_id_string),
		  "[%s]_%s_ctx[%d]_hwmgr_ctx[%d]", hw_mgr->hw_mgr_name,
		  cam_icp_dev_type_to_name(
			  ctx_data->icp_dev_acquire_info->dev_type),
	...
	if (!hw_mgr->ctxt_cnt) {
		...
		hfi_set_fw_dump_levels(hw_mgr->hfi_handle,
				       hw_mgr->icp_fw_dump_lvl,
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "%s: ping ack not received",
			ctx_data->ctx_id_string);
	...
	rc = cam_icp_mgr_create_handle(hw_mgr, icp_dev_acquire_info->dev_type,
				       ctx_data);
	if (rc) {
		CAM_ERR(CAM_ICP, "%s: create handle failed",
			ctx_data->ctx_id_string);
		goto create_handle_failed;
	}

	CAM_DBG(CAM_ICP, "%s: created stream handle", ctx_data->ctx_id_string);
	...
	if (copy_to_user((void __user *)args->acquire_info,
			 ...
			 sizeof(struct cam_icp_acquire_dev_info))) {
		CAM_ERR_RATE_LIMIT(CAM_ICP, "%s: copy from user failed",
				   ctx_data->ctx_id_string);
...
static int cam_icp_mgr_get_hw_caps(void *hw_mgr_priv, void *hw_caps_args)
{
	...
	if (sizeof(struct cam_icp_query_cap_cmd) != query_cap->size) {
		CAM_ERR(CAM_ICP,
			"[%s] Input query cap size:%u does not match expected query cap "
			"size: %u",
	...
	mutex_lock(&hw_mgr->hw_mgr_mutex);
	if (copy_from_user(&icp_caps, u64_to_user_ptr(query_cap->caps_handle),
			   sizeof(struct cam_icp_query_cap_cmd))) {
		CAM_ERR(CAM_ICP, "[%s] copy from_user failed",
			hw_mgr->hw_mgr_name);
		rc = -EFAULT;
		goto end;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "[%s] Fail to get hfi caps",
			hw_mgr->hw_mgr_name);
		goto end;
	}
	...
	if (copy_to_user(u64_to_user_ptr(query_cap->caps_handle), &icp_caps,
			 sizeof(struct cam_icp_query_cap_cmd))) {
		CAM_ERR(CAM_ICP, "[%s] copy_to_user failed",
			hw_mgr->hw_mgr_name);
		rc = -EFAULT;
	}
end:
...
static int cam_icp_mgr_get_hw_caps_v2(void *hw_mgr_priv, void *hw_caps_args)
{
	...
	if (copy_from_user(&query_cmd, u64_to_user_ptr(query_cap->caps_handle),
			   sizeof(struct cam_icp_query_cap_cmd_v2))) {
		CAM_ERR(CAM_ICP, "[%s] copy from user failed",
			hw_mgr->hw_mgr_name);
		rc = -EFAULT;
		goto end;
	}
	...
	memset(&query_cmd.dev_info, 0,
		...
		sizeof(struct cam_icp_device_info)));

	for (i = 0; i < CAM_ICP_MAX_ICP_HW_TYPE; i++) {
		if (!CAM_ICP_IS_DEV_HW_EXIST(hw_mgr->hw_cap_mask, i))
			continue;

		query_cmd.dev_info[num_supported_device].dev_type =
			CAM_ICP_DEV_TYPE_ICP;
		query_cmd.dev_info[num_supported_device].num_devices =
			CAM_ICP_MAX_ICP_PROC_PER_DEV;
	...
	for (i = 0; i < hw_mgr->num_dev_info; i++) {
		...
		if (num_supported_device >= CAM_ICP_MAX_NUM_OF_DEV_TYPES) {
			CAM_ERR(CAM_ICP,
				"[%s] number of supported hw devices:%u exceeds maximum number "
				"of devices supported by query cap struct: %u",
	...
	if (copy_to_user(u64_to_user_ptr(query_cap->caps_handle), &query_cmd,
			 sizeof(struct cam_icp_query_cap_cmd_v2))) {
		CAM_ERR(CAM_ICP, "[%s] copy_to_user failed",
			hw_mgr->hw_mgr_name);
		rc = -EFAULT;
...
static int cam_icp_mgr_verify_hw_caps(struct cam_icp_hw_mgr *hw_mgr,
				      uint32_t *icp_dev_mask,
				      uint32_t num_icp_dev_mask)
{
	...
	if (num_icp_dev_mask > num_cpas_cap_mask) {
		CAM_ERR(CAM_ICP,
			...
			hw_mgr->hw_mgr_name, num_icp_dev_mask,
			num_cpas_cap_mask);
		return -EINVAL;
	}
				  ...
				  uint32_t *hw_dev_cnt)
{
	...
	uint32_t num = 0, num_cpas_mask = 0,
		 cpas_hw_mask[MAX_HW_CAPS_MASK] = { 0 };

	rc = cam_icp_alloc_processor_devs(np, &icp_hw_type, &alloc_devices,
					  hw_dev_cnt);
	if (rc) {
	...
	if (hw_dev_cnt[icp_hw_type] > CAM_ICP_MAX_ICP_PROC_PER_DEV) {
		CAM_ERR(CAM_ICP,
			"Invalid number of ICP device: %u allocated exceeds ICP device "
			"supported per hw mgr: %u",
	...
	if (!rc) {
		alloc_devices =
			kcalloc(num, sizeof(*alloc_devices), GFP_KERNEL);
		if (!alloc_devices) {
		...
		num_cpas_mask =
			max(num_cpas_mask, (uint32_t)(IPE_CAPS_MASK_IDX + 1));
	...
	if (!rc) {
		...
		num_cpas_mask =
			max(num_cpas_mask, (uint32_t)(BPS_CAPS_MASK_IDX + 1));
	...
	if (!rc) {
		...
		num_cpas_mask =
			max(num_cpas_mask, (uint32_t)(OFE_CAPS_MASK_IDX + 1));
	...
	if (rc) {
		CAM_ERR(CAM_ICP,
			"CPAS ICP HW capability verification fails rc=%d", rc);
		goto free_devs;
	}
	...
	hw_mgr->synx_signaling_en =
		of_property_read_bool(np, "synx_signaling_en");
				       ...
				       uint32_t *hw_dev_cnt)
{
	...
	hw_mgr->dev_info = kcalloc(hw_mgr->num_dev_info,
				 ...
				 struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (count < 0) {
		CAM_ERR(CAM_ICP, "[%s] Invalid compat-hw-name count=%d",
			hw_mgr->hw_mgr_name, count);
		rc = count;
		goto free_devices;
	}
	...
	for (i = 0; i < count; i++) {
		...
		rc = of_property_read_string_index(np, "compat-hw-name", i,
						   &name);
		if (rc) {
			CAM_ERR(CAM_ICP,
...
static int cam_icp_mgr_create_wq(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	scnprintf(q_name, sizeof(q_name), "%s_command_queue",
		  hw_mgr->hw_mgr_name);
	rc = cam_req_mgr_workq_create(
		"icp_command_queue", ICP_WORKQ_NUM_TASK, &hw_mgr->cmd_work,
	...
	scnprintf(q_name, sizeof(q_name), "%s_message_queue",
		  hw_mgr->hw_mgr_name);
	rc = cam_req_mgr_workq_create(
		q_name, ICP_WORKQ_NUM_TASK, &hw_mgr->msg_work,
	...
	scnprintf(q_name, sizeof(q_name), "%s_timer_queue",
		  hw_mgr->hw_mgr_name);
	rc = cam_req_mgr_workq_create(
		q_name, ICP_WORKQ_NUM_TASK, &hw_mgr->timer_work,
		CRM_WORKQ_USAGE_IRQ, 0,
				     ...
				     struct cam_hw_cmd_pf_args *pf_cmd_args)
{
	...
	rc = cam_packet_util_get_packet_addr(
		&packet, pf_cmd_args->pf_req_info->packet_handle,
   ...
   */
	...
	cam_packet_util_put_packet_addr(
...
static void cam_icp_mgr_unregister_hfi_client(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (!icp_dev_intf) {
		CAM_ERR(CAM_ICP, "[%s] ICP dev intf is NULL",
			hw_mgr->hw_mgr_name);
		return;
	}
...
static int cam_icp_mgr_get_hw_mgr_name(uint32_t device_idx, char *hw_mgr_name)
{
	...
	if (device_idx)
		scnprintf(hw_mgr_name, CAM_ICP_HW_MGR_NAME_SIZE, "icp%u",
			  device_idx);
	else
			...
			int device_idx)
{
	...
	if (rc) {
		CAM_ERR(CAM_ICP,
			"Fail to get hw mgr name rc: %d for icp dev[%u]", rc,
			device_idx);
		goto free_hw_mgr;
	}

	CAM_DBG(CAM_ICP, "Initailize hw mgr[%u] with name: %s", device_idx,
		hw_mgr->hw_mgr_name);
	...
	for (i = 0; i < CAM_ICP_CTX_MAX; i++) {
		...
		if (cam_presil_mode_enabled()) {
			size = CAM_FRAME_CMD_MAX *
			       sizeof(struct cam_hangdump_mem_regions);
			hw_mgr->ctx_data[i]
				.hfi_frame_process.hangdump_mem_regions =
	...
	cam_common_register_mini_dump_cb(cam_icp_hw_mgr_mini_dump_cb,
					 hw_mgr->hw_mgr_name,
					 &hw_mgr->hw_mgr_id);
	...
	CAM_DBG(CAM_ICP, "Done hw mgr[%u] init: icp name:%s", device_idx,
		hw_mgr->hw_mgr_name);
...
void cam_icp_hw_mgr_deinit(int device_idx)
{
	...
	CAM_DBG(CAM_ICP, "hw mgr deinit: %u icp name: %s", device_idx,
		hw_mgr->hw_mgr_name);
