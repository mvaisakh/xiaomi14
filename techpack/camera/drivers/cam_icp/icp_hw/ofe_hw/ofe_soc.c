...
static int cam_ofe_request_platform_resource(struct cam_hw_soc_info *soc_info,
					     irq_handler_t ofe_irq_handler,
					     void *data)
{
	int rc = 0, i;
	...
	rc = cam_soc_util_request_platform_resource(soc_info, ofe_irq_handler,
						    &(irq_data[0]));
			       ...
			       irq_handler_t ofe_irq_handler, void *irq_data)
{
	...
	rc = cam_ofe_request_platform_resource(soc_info, ofe_irq_handler,
					       irq_data);
	if (rc < 0)
		return rc;
...
static inline int cam_ofe_set_regulators_mode(struct cam_hw_soc_info *soc_info,
					      uint32_t mode)
{
	int i, rc;
...
int cam_ofe_update_clk_rate(struct cam_hw_soc_info *soc_info, uint32_t clk_rate)
{
	int32_t src_clk_idx;
