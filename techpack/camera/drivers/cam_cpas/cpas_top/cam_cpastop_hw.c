...
static const uint32_t
	cam_cpas_hw_version_map[CAM_CPAS_CAMERA_VERSION_ID_MAX]
...
static int cam_cpas_translate_camera_cpas_version_id(uint32_t cam_version,
						     ...
						     uint32_t *cpas_version_id)
{
	...
	case CAM_CPAS_CAMERA_VERSION_980:
		*cam_version_id = CAM_CPAS_CAMERA_VERSION_ID_980;
		break;
	default:
		CAM_ERR(CAM_CPAS, "Invalid cam version %u", cam_version);
		return -EINVAL;
	}
	...
	default:
		CAM_ERR(CAM_CPAS, "Invalid cpas version %u", cpas_version);
		return -EINVAL;
	}
	return 0;
}
				   ...
				   struct cam_cpas_hw_caps *hw_caps)
{
	...
	rc = cam_cpas_translate_camera_cpas_version_id(
		cam_version, cpas_version, &cam_version_id, &cpas_version_id);
	if (rc) {
					     ...
					     int32_t num_reg_map)
{
	...
	/*
   * camnoc reg base represents reg base for legacy camnoc, this becomes
   * optional since some targets have camnoc split into RT/NRT
   */
	rc = cam_common_util_get_string_index(soc_info->mem_block_name,
					      soc_info->num_mem_block,
					      "cam_camnoc", &index);
	if ((rc == 0) && (index < num_reg_map)) {
		...
		CAM_DBG(CAM_CPAS, "regbase found for CAMNOC, rc=%d, %d %d", rc,
			index, num_reg_map);
	} else {
	...
	/* optional - reg base for a target where camnoc reg is split into two
   * domains: rt/nrt */
	rc = cam_common_util_get_string_index(soc_info->mem_block_name,
					      soc_info->num_mem_block,
					      "cam_camnoc_rt", &index);
	if ((rc == 0) && (index < num_reg_map)) {
		...
		CAM_DBG(CAM_CPAS,
			"regbase not found for CAMNOC RT, rc=%d, %d %d", rc,
			index, num_reg_map);
   ...
   * domains: rt/nrt */
	...
	if ((rc == 0) && (index < num_reg_map)) {
		...
		CAM_DBG(CAM_CPAS,
			"regbase not found for CAMNOC NRT, rc=%d, %d %d", rc,
			index, num_reg_map);
	...
	if ((rc == 0) && (index < num_reg_map)) {
		...
		CAM_DBG(CAM_CPAS, "regbase found for RPMH, rc=%d, %d %d", rc,
			index, num_reg_map);
	} else {
	...
	if ((rc == 0) && (index < num_reg_map)) {
		...
		CAM_DBG(CAM_CPAS, "regbase found for cesta, rc=%d, %d %d", rc,
			index, num_reg_map);
	} else {
...
static int
cam_cpastop_handle_errlogger(int camnoc_idx, struct cam_cpas *cpas_core,
			     struct cam_hw_soc_info *soc_info,
			     struct cam_camnoc_irq_slave_err_data *slave_err)
{
	int regbase_idx =
		cpas_core->regbase_index[camnoc_info[camnoc_idx]->reg_base];
	...
	CAM_ERR_RATE_LIMIT(
		CAM_CPAS,
		"[%s] Possible memory configuration issue, fault at SMMU "
		"raised as CAMNOC SLAVE_IRQ err_code=%d(%s)",
...
static int
cam_cpastop_handle_ubwc_enc_err(int camnoc_idx, struct cam_cpas *cpas_core,
				...
				struct cam_camnoc_irq_ubwc_enc_data *enc_err)
{
	...
	enc_err->encerr_status.value = cam_io_r_mb(
	...
	CAM_DBG(CAM_CPAS, "[%s] ubwc enc err [%d]: offset[0x%x] value[0x%x]",
...
static int
cam_cpastop_handle_ubwc_dec_err(int camnoc_idx, struct cam_cpas *cpas_core,
				struct cam_hw_soc_info *soc_info, int i,
				struct cam_camnoc_irq_ubwc_dec_data *dec_err)
{
	...
	dec_err->decerr_status.value = cam_io_r_mb(
		soc_info->reg_map[regbase_idx].mem_base +
		camnoc_info[camnoc_idx]->irq_err[i].err_status.offset);

	/* Let clients handle the UBWC errors */
	CAM_DBG(CAM_CPAS,
		"[%s] ubwc dec err status [%d]: offset[0x%x] value[0x%x] thr_err=%d, "
		"fcl_err=%d, len_md_err=%d, format_err=%d",
		camnoc_info[camnoc_idx]->camnoc_name, i,
		camnoc_info[camnoc_idx]->irq_err[i].err_status.offset,
...
static int cam_cpastop_handle_ahb_timeout_err(
	int camnoc_idx, struct cam_hw_info *cpas_hw,
...
static void cam_cpastop_enable_camnoc_irqs(struct cam_hw_info *cpas_hw,
					   int camnoc_idx)
{
	int i;
	struct cam_cpas *cpas_core = (struct cam_cpas *)cpas_hw->core_info;
	...
	for (i = 0; i < camnoc_info[camnoc_idx]->irq_err_size; i++) {
		if (camnoc_info[camnoc_idx]->irq_err[i].enable) {
			cam_cpas_util_reg_update(
				cpas_hw, camnoc_info[camnoc_idx]->reg_base,
					   ...
					   int camnoc_idx)
{
	...
	/* Disable all serviced irqs, all disabled irqs are enabled only when CPAS
   * restarts */
	cam_io_w(((~updated_sbm_mask) &
		  (camnoc_info[camnoc_idx]->irq_sbm->sbm_enable.value)),
		 soc_info->reg_map[regbase_idx].mem_base +
				 ...
				 struct cam_hw_info *cpas_hw, int camnoc_idx)
{
	...
	if (wait_for_irq) {
		if (!cam_common_wait_for_completion_timeout(
			    &test_irq_hw_complete[camnoc_idx],
			    msecs_to_jiffies(2000)))
			CAM_ERR(CAM_CPAS,
				"[%s] CAMNOC Test IRQ line verification timed out",
				camnoc_info[camnoc_idx]->camnoc_name);
		...
		if ((payload->irq_status &
			...
			case CAM_CAMNOC_HW_IRQ_TFE_UBWC_ENCODE_ERROR:
				cam_cpastop_handle_ubwc_enc_err(
					camnoc_idx, cpas_core, soc_info, i,
					&irq_data.u.enc_err);
				break;
			...
			case CAM_CAMNOC_HW_IRQ_CAMNOC_TEST:
				...
				break;
			default:
				CAM_ERR(CAM_CPAS, "Invalid IRQ type: %u",
					irq_type);
				break;
			}
...
static irqreturn_t cam_cpastop_handle_irq(int irq_num, void *data)
{
	...
	if ((cpas_core->slave_err_irq_en[camnoc_idx]) &&
	    (payload->irq_status &
		...
		irq_data.irq_type =
			(enum cam_camnoc_irq_type)camnoc_info[camnoc_idx]
				->irq_err[slave_err_irq_idx]
				.irq_type;
		...
		if (slave_err->errlog0_low.err_code ==
...
static int cam_cpastop_print_poweron_settings(struct cam_hw_info *cpas_hw)
{
	...
	for (i = 0; i < cpas_core->num_valid_camnoc; i++) {
		CAM_INFO(CAM_CPAS,
			 "QOS settings for %s :", camnoc_info[i]->camnoc_name);
		...
		for (j = 0; j < camnoc_info[i]->specific_size; j++) {
			if (camnoc_info[i]->specific[j].enable) {
				CAM_DBG(CAM_CPAS,
					...
					camnoc_info[i]->specific[j].port_name);
				cam_cpas_util_reg_update(
					cpas_hw, camnoc_info[i]->reg_base,
					&camnoc_info[i]
						 ->specific[j]
						 .priority_lut_low);
				cam_cpas_util_reg_update(
...
static int cam_cpastop_poweroff(struct cam_hw_info *cpas_hw)
{
	...
	if (errata_wa_list->camnoc_flush_slave_pending_trans.enable) {
		...
		camnoc_index =
			cpas_core->regbase_index[camnoc_info[0]->reg_base];
					  ...
					  bool power_on, bool force_on)
{
	...
	for (i = 0; i < cpas_info->num_qchannel; i++) {
		...
		if (!icp_clk_enabled) {
			errata_wa_list = camnoc_info[i]->errata_wa_list;
			if (errata_wa_list &&
			    errata_wa_list->enable_icp_clk_for_qchannel.enable) {
				CAM_DBG(CAM_CPAS,
					"Enabling ICP clk for qchannel handshake");

				if (soc_private->icp_clk_index == -1) {
					CAM_ERR(CAM_CPAS,
						"ICP clock not added as optional clk, qchannel "
						"handshake will fail");
				} else {
					...
					if (rc)
						CAM_ERR(CAM_CPAS,
							"Error enable icp clk failed rc=%d",
							rc);
					else
		...
		qchannel_status =
			cam_io_r_mb(soc_info->reg_map[reg_indx].mem_base +
				    qchannel_info->qchannel_status);
		CAM_DBG(CAM_CPAS, "CPAS_%s %s : qchannel status 0x%x",
			power_on ? "START" : "STOP",
	...
	if (icp_clk_enabled) {
		rc = cam_soc_util_clk_disable(soc_info, CAM_CLK_SW_CLIENT_IDX,
					      true, soc_private->icp_clk_index);
		if (rc)
			CAM_ERR(CAM_CPAS, "Error disable icp clk failed rc=%d",
				rc);
	}
					  ...
					  struct cam_camnoc_info **alloc_camnoc)
{
	...
	for (i = 0; i < CAM_CAMNOC_HW_TYPE_MAX; i++) {
		if (alloc_camnoc[i]) {
			alloc_camnoc[i]->camnoc_name =
				cam_cpastop_get_camnoc_name(i);
			...
			if (cpas_core->regbase_index[camnoc_info[i]->reg_base] ==
			    -1) {
	...
	if (cpas_core->camnoc_info_idx[CAM_CAMNOC_HW_COMBINED] >= 0)
		cpas_core->camnoc_rt_idx =
			cpas_core->camnoc_info_idx[CAM_CAMNOC_HW_COMBINED];
	else if (cpas_core->camnoc_info_idx[CAM_CAMNOC_HW_RT] >= 0)
		cpas_core->camnoc_rt_idx =
			cpas_core->camnoc_info_idx[CAM_CAMNOC_HW_RT];
	else {
				       ...
				       struct cam_cpas_hw_caps *hw_caps)
{
	...
	struct cam_camnoc_info *alloc_camnoc_info[CAM_CAMNOC_HW_TYPE_MAX] = {
		0
	};
	...
	if (rc) {
		CAM_ERR(CAM_CPAS, "Failed to get titan hw capability rc=%d",
			rc);
		return rc;
	}
					  ...
					  uint32_t selection_mask)
{
	...
	CAM_DBG(CAM_CPAS, "QoS selection : hw_version=0x%x selection_mask 0x%x",
		soc_info->hw_version, selection_mask);

	switch (soc_info->hw_version) {
	case CAM_CPAS_TITAN_480_V100:
		if (selection_mask & CAM_CPAS_QOS_CUSTOM_SETTINGS_MASK)
			camnoc_info[CAM_CAMNOC_HW_COMBINED] =
