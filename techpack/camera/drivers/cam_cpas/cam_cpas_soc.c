...
void cam_cpas_dump_tree_vote_info(struct cam_hw_info *cpas_hw,
				  ...
				  const char *identifier, int ddr_drv_idx,
				  int cesta_drv_idx)
{
	...
	if (soc_private->enable_cam_ddr_drv) {
		...
		CAM_INFO(
			CAM_PERF,
			"%s node:%s lvl:%d drv_idx:%d cesta_drv_idx:%d ==== printing full "
			"node state",
			identifier, tree_node->node_name, tree_node->level_idx,
...
void cam_cpas_dump_full_tree_state(struct cam_hw_info *cpas_hw,
				   const char *identifier)
{
	...
	for (j = 0; j < CAM_CPAS_MAX_TREE_NODES; j++) {
		...
		if (soc_private->enable_cam_ddr_drv) {
			...
			CAM_INFO(
				CAM_PERF,
				"Identifier[%s] node:[%s] cell:%d lvl:%d PortIdx mnoc[%d %d %d "
				"%d] camnoc[%d] camnoc_max %d, bus_width:%d, drv_idx:%d",
				identifier, curr_node->node_name,
				curr_node->cell_idx, curr_node->level_idx,
				curr_node->axi_port_idx_arr
			...
			for (i = 0; i < CAM_CPAS_MAX_DRV_PORTS; i++) {
				if (i == CAM_CPAS_PORT_HLOS_DRV)
					...
					CAM_INFO(
						CAM_PERF,
						"    idx[%d] DRV camnoc[%llu %llu], DDR ab[%llu %llu] "
						"ib[%llu %llu]",
						i,
						curr_node->bw_info[i]
							.drv_vote.high.camnoc,
			...
			CAM_INFO(
				CAM_CPAS,
				"[%s] Cell[%d] level[%d] PortIdx[%d][%d] camnoc_bw[%d %d %lld "
				"%lld] mnoc_bw[%lld %lld]",
				...
				curr_node->camnoc_max_needed,
				curr_node->bus_width_factor,
				curr_node->bw_info[CAM_CPAS_PORT_HLOS_DRV]
					.hlos_vote.camnoc,
				curr_node->bw_info[CAM_CPAS_PORT_HLOS_DRV]
						.hlos_vote.camnoc *
					curr_node->bus_width_factor,
	...
	CAM_INFO(CAM_CPAS, "Dumping cpas tree full state end ============== %s",
		 identifier);
}
				 ...
				 struct cam_axi_vote *axi_vote)
{
	...
	for (i = 0; i < axi_vote->num_paths; i++) {
		CAM_INFO(
			CAM_PERF,
			"Client [%s][%d] : [%s], Path=[%d] [%d], [%s], camnoc[%llu], "
			"mnoc_ab[%llu], mnoc_ib[%llu]",
...
void cam_cpas_util_debug_parse_data(struct cam_cpas_private_soc *soc_private)
{
	...
	for (i = 0; i < CAM_CPAS_MAX_TREE_NODES; i++) {
		...
		CAM_INFO(
			CAM_CPAS,
			"NODE cell_idx: %d, level: %d, name: %s, axi_port_idx: %d, "
			"merge_type: %d, parent_name: %s camnoc_max_needed: %d",
			curr_node->cell_idx, curr_node->level_idx,
			curr_node->node_name,
...

{
	...
	camnoc_node = of_find_node_by_name(curr_node, "qcom,axi-port-camnoc");
	if (camnoc_node) {
		...
		if (rc) {
			CAM_ERR(CAM_CPAS, "fail to read camnoc-port-name rc=%d",
				rc);
			return rc;
		}
...
static int cam_cpas_parse_mnoc_node(struct cam_cpas *cpas_core,
				    struct cam_cpas_private_soc *soc_private,
				    struct cam_cpas_tree_node *curr_node_ptr,
				    ...
				    int *mnoc_idx)
{
	...
	ib_voting_needed = of_property_read_bool(curr_node_ptr->tree_dev_node,
						 "ib-bw-voting-needed");
	is_rt_port = of_property_read_bool(curr_node_ptr->tree_dev_node,
					   "rt-axi-port");
	...
	if (soc_private->bus_icc_based) {
		...
		for (i = 0; i < count; i++) {
			...
			if (*mnoc_idx >= CAM_CPAS_MAX_AXI_PORTS) {
				CAM_ERR(CAM_CPAS, "Invalid mnoc index: %d",
					*mnoc_idx);
				return -EINVAL;
			}
			...
			rc = of_property_read_string_index(
				mnoc_node, "interconnect-names", i,
			...
			rc = of_parse_phandle_with_args(mnoc_node,
							"interconnects",
							"#interconnect-cells",
							(2 * i), &src_args);
			if (rc) {
			...
			cpas_core->axi_port[*mnoc_idx]
				...
				dst_args.args[0];
			cpas_core->axi_port[*mnoc_idx]
				.bus_client.common_data.num_usecases = 2;
       ...
       */
			...
			cpas_core->axi_port[*mnoc_idx].ib_bw_voting_needed =
				ib_voting_needed;
		...
		rc = of_property_read_string(
			curr_node_ptr->tree_dev_node, "qcom,axi-port-name",
			&cpas_core->axi_port[*mnoc_idx]
				 .bus_client.common_data.name);
		if (rc) {
			CAM_ERR(CAM_CPAS, "failed to read mnoc-port-name rc=%d",
				rc);
			return rc;
		}
	...
	for (level_idx = (CAM_CPAS_MAX_TREE_LEVELS - 1); level_idx >= 0;
		...
		for_each_available_child_of_node(level_node, curr_node) {
			...
			curr_node_ptr->bw_info =
				kzalloc((sizeof(struct cam_cpas_axi_bw_info) *
					 num_drv_ports),
					GFP_KERNEL);
			if (!curr_node_ptr->bw_info) {
				CAM_ERR(CAM_CPAS,
					"Failed in allocating memory for bw info");
				return -ENOMEM;
			}

			curr_node_ptr->axi_port_idx_arr = kzalloc(
				(sizeof(int) * num_drv_ports), GFP_KERNEL);
			if (!curr_node_ptr->axi_port_idx_arr) {
				CAM_ERR(CAM_CPAS,
					"Failed in allocating memory for port indices");
				return -ENOMEM;
			}
			...
			if (soc_private->enable_smart_qos && (level_idx == 1) &&
				...
				rc = of_property_read_u32(
					curr_node, "priority-lut-low-offset",
					&curr_node_ptr->pri_lut_low_offset);
				if (rc) {
					CAM_ERR(CAM_CPAS,
						"Invalid priority low offset rc %d",
						rc);
					return rc;
				}

				rc = of_property_read_u32(
					curr_node, "priority-lut-high-offset",
					&curr_node_ptr->pri_lut_high_offset);
				if (rc) {
					CAM_ERR(CAM_CPAS,
						"Invalid priority high offset rc %d",
						rc);
					return rc;
				}
				...
				niu_idx = soc_private->smart_qos_info
						  ->num_rt_wr_nius;
			...
			mnoc_node = of_get_child_by_name(curr_node,
							 "qcom,axi-port-mnoc");
			if (mnoc_node) {
				rc = cam_cpas_parse_mnoc_node(
					cpas_core, soc_private, curr_node_ptr,
					mnoc_node, &mnoc_idx);
				if (rc) {
					CAM_ERR(CAM_CPAS,
						"failed to parse mnoc node info rc=%d",
						rc);
					return rc;
				}
			}
			...
			if (!soc_private->control_camnoc_axi_clk) {
				rc = cam_cpas_update_camnoc_node(cpas_core,
								 curr_node,
								 curr_node_ptr,
								 &camnoc_idx);
				if (rc) {
					CAM_ERR(CAM_CPAS,
						"failed to parse camnoc node info rc=%d",
						rc);
					return rc;
				}
			}
			...
			if (!rc) {
				...
				if (soc_private->enable_cam_ddr_drv) {
					rc = of_property_read_u32(
						curr_node, "drv-voting-index",
						&curr_node_ptr->drv_voting_idx);
					if (rc)
						curr_node_ptr->merge_type =
							CAM_CPAS_PORT_HLOS_DRV;

					if (curr_node_ptr->drv_voting_idx ==
			...
			if (soc_private->enable_cam_ddr_drv)
				for (j = CAM_CPAS_PORT_DRV_0; j < num_drv_ports;
				     j++)
			     ...
			     struct cam_cpas_private_soc *soc_private)
{
	...
	for (i = 0; (i + 5) <= count; i = i + 5) {
		of_property_read_u32_index(of_node, "cam_hw_fuse", i, &feature);
		...
		soc_private->feature_info[num_feature].feature = feature;
         ...
         */
			...
			CAM_INFO(CAM_CPAS, "feature 0x%x enable=%d hw_map=0x%x",
				 feature_info->feature, feature_info->enable,
				 feature_info->hw_map);
		} else {
			feature_info->value = (fuse_val & fuse_mask) >>
					      fuse_shift;
			CAM_INFO(CAM_CPAS,
...
static inline enum cam_sys_cache_config_types
cam_cpas_find_type_from_string(const char *cache_name)
{
...
static int
cam_cpas_parse_sys_cache_uids(struct device_node *of_node,
			      struct cam_cpas_private_soc *soc_private)
{
	...
	if (num_caches > CAM_LLCC_MAX) {
		CAM_ERR(CAM_CPAS, "invalid number of cache-names found: 0x%x",
			num_caches);
		return -EINVAL;
	}

	soc_private->llcc_info = kcalloc(
		num_caches, sizeof(struct cam_sys_cache_info), GFP_KERNEL);
	if (!soc_private->llcc_info)
		return -ENOMEM;

	for (i = 0; i < num_caches; i++) {
		rc = of_property_read_string_index(
			of_node, "sys-cache-names", i,
			&soc_private->llcc_info[i].name);
		if (rc) {
			CAM_ERR(CAM_CPAS, "failed to read cache-names at %d",
				i);
			goto end;
		}
		...
		rc = of_property_read_u32_index(of_node, "sys-cache-uids", i,
		...
		if (IS_ERR_OR_NULL(soc_private->llcc_info[i].slic_desc)) {
			CAM_ERR(CAM_CPAS, "Failed to get slice desc for uid %u",
				soc_private->llcc_info[i].uid);
			rc = -EINVAL;
			goto end;
		}
...
static int
cam_cpas_parse_domain_id_mapping(struct device_node *of_node,
				 struct cam_cpas_private_soc *soc_private)
{
	int count, i, rc = 0;
	...
	if (count <= 0) {
		CAM_DBG(CAM_CPAS, "No domain-id mapping found, count: %d",
			count);
		return rc;
	}
	...
	if (count % 2) {
		CAM_ERR(CAM_CPAS,
			"Mismatch in domain-id mapping, found %d number of entries",
			count);
		rc = -EINVAL;
		goto err;
	}
	...
	if (soc_private->domain_id_info.num_domain_ids >
	    CAM_CPAS_DOMAIN_ID_MAX) {
		CAM_ERR(CAM_CPAS,
	...
	for (i = 0; i < soc_private->domain_id_info.num_domain_ids; i++) {
		...
		rc = of_property_read_u32_index(of_node, "domain-id", (i * 2),
						&domain_id_entry->domain_type);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Error reading domain-id type entry at pos %d",
				i);
			rc = -EINVAL;
			goto err;
		}
		...
		rc = of_property_read_u32_index(of_node, "domain-id",
						((i * 2) + 1),
						&domain_id_entry->mapping_id);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Error reading domain-id mapping id at pos %d",
				i);
			rc = -EINVAL;
			goto err;
		}
...
static int
cam_cpas_get_domain_id_support_clks(struct device_node *of_node,
				    ...
				    struct cam_cpas_private_soc *soc_private)
{
	...
	soc_private->domain_id_clks = kzalloc(
		sizeof(struct cam_cpas_domain_id_support_clks), GFP_KERNEL);
	...
	for (i = 0; i < count; i++) {
		rc = of_property_read_string_index(
			of_node, "domain-id-support-clks", i,
			&domain_id_clks->clk_names[i]);
		if (rc) {
			CAM_ERR(CAM_CPAS,
		...
		rc = cam_soc_util_get_option_clk_by_name(
			soc_info, domain_id_clks->clk_names[i],
				...
				struct cam_cpas_private_soc *soc_private)
{
	...
	if (soc_private->domain_id_info.domain_id_supported == false) {
		CAM_ERR(CAM_CPAS,
			"Domain ID configuration is expected for this target");
		return -EINVAL;
	}
#endif
	...
	soc_private->bus_icc_based =
		of_property_read_bool(of_node, "interconnects");

	if (soc_private->bus_icc_based) {
		rc = of_property_read_string(
			of_node, "interconnect-names",
		...
		num_bw_values =
			of_property_count_u32_elems(of_node, "cam-ahb-bw-KBps");
		if (num_bw_values <= 0) {
		...
		for (i = 0; i < num_levels; i++) {
			...
			if (rc) {
				CAM_ERR(CAM_UTIL,
					"Error reading ab bw value, rc=%d", rc);
				return rc;
			}
			cpas_core->ahb_bus_client.common_data.bw_pair[i].ab =
				ahb_bus_client_ab;
			...
			if (rc) {
				CAM_ERR(CAM_UTIL,
					"Error reading ib bw value, rc=%d", rc);
				return rc;
			}
			...
			CAM_DBG(CAM_CPAS,
				...
				i,
				cpas_core->ahb_bus_client.common_data.bw_pair[i]
					.ab,
				cpas_core->ahb_bus_client.common_data.bw_pair[i]
					.ib);
		}
	...
	soc_private->enable_smart_qos =
		of_property_read_bool(of_node, "enable-smart-qos");

	if (soc_private->enable_smart_qos) {
		uint32_t value;

		soc_private->smart_qos_info = kzalloc(
			sizeof(struct cam_cpas_smart_qos_info), GFP_KERNEL);
     ...
     */
		...
		soc_private->smart_qos_info->rt_wr_priority_min =
			(uint8_t)value;
		...
		soc_private->smart_qos_info->rt_wr_priority_max =
			(uint8_t)value;

		rc = of_property_read_u32(of_node, "rt-wr-priority-clamp",
					  &value);
		if (rc) {
			CAM_ERR(CAM_CPAS, "Invalid clamp Qos priority rc %d",
				rc);
			goto cleanup_clients;
		}
		soc_private->smart_qos_info->rt_wr_priority_clamp =
			(uint8_t)value;
		...
		if (value > CAM_CPAS_MAX_SLOPE_FACTOR) {
			CAM_ERR(CAM_UTIL, "Invalid slope factor value %d",
				value);
			rc = -EINVAL;
			goto cleanup_clients;
		} else
		...
		CAM_DBG(CAM_CPAS,
			...
			(uint32_t)
				soc_private->smart_qos_info->rt_wr_priority_max,
			(uint32_t)soc_private->smart_qos_info
				->rt_wr_priority_clamp,
			(uint32_t)soc_private->smart_qos_info
				->rt_wr_slope_factor);

		rc = of_property_read_u32(
			of_node, "rt-wr-leaststressed-clamp-threshold", &value);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Invalid leaststressed clamp threshold rc %d",
				rc);
			goto cleanup_clients;
		}
		...
		rc = of_property_read_u32(
			of_node, "rt-wr-moststressed-clamp-threshold", &value);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Invalid moststressed clamp threshold rc %d",
				rc);
			goto cleanup_clients;
		}
		...
		rc = of_property_read_u32(
			of_node, "rt-wr-highstress-indicator-threshold",
			&value);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Invalid highstress indicator threshold rc %d",
				rc);
			goto cleanup_clients;
		}
		...
		if (value > CAM_CPAS_MAX_STRESS_INDICATOR) {
			CAM_ERR(CAM_UTIL,
				"Invalid highstress indicator threshold value %d",
				value);
			rc = -EINVAL;
			goto cleanup_clients;
		} else
		...
		rc = of_property_read_u32(
			of_node, "rt-wr-lowstress-indicator-threshold", &value);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Invalid lowstress indicator threshold rc %d",
				rc);
			goto cleanup_clients;
		}
		...
		if (value > CAM_CPAS_MAX_STRESS_INDICATOR) {
			CAM_ERR(CAM_UTIL,
				"Invalid lowstress indicator threshold value %d",
				value);
			rc = -EINVAL;
			goto cleanup_clients;
		} else
		...
		rc = of_property_read_u32(
			of_node, "rt-wr-bw-ratio-scale-factor", &value);
		if (rc) {
			CAM_ERR(CAM_CPAS, "Invalid bw ratio scale factor rc %d",
				rc);
			goto cleanup_clients;
		}
		soc_private->smart_qos_info->bw_ratio_scale_factor =
			(uint8_t)value;

		CAM_DBG(CAM_CPAS,
			"highstress_indicator_th=%u, lowstress_indicator_th=%u, scale "
			"factor=%u",
			(uint32_t)soc_private->smart_qos_info
	...
	rc = of_property_read_u32(of_node, "enable-cam-drv",
				  &cam_drv_en_mask_val);
	...
	if (!rc) {
		...
		if (cam_drv_en_mask_val & CAM_CLK_DRV) {
			...
			if (rc) {
				CAM_ERR(CAM_CPAS,
					"Failed to populate cam cesta crm device rc %d",
					rc);
				goto cleanup_clients;
			}
		}
	...
	CAM_DBG(CAM_CPAS,
		...
		soc_private->enable_cam_clk_drv, cam_drv_en_mask_val);
	...
	if (soc_private->enable_smart_qos) {
		...
		for (port_idx = 0; port_idx < cpas_core->num_axi_ports;
		     port_idx++) {
			CAM_DBG(CAM_CPAS, "[%d] : Port[%s] is_rt=%d", port_idx,
				cpas_core->axi_port[port_idx].axi_port_name,
   ...
   */
	if (count == CAM_RPMH_BCM_INFO_MAX) {
		for (i = 0; i < count; i++) {
			rc = of_property_read_u32_index(
				of_node, "rpmh-bcm-info", i,
				&soc_private->rpmh_info[i]);
			if (rc) {
				CAM_ERR(CAM_CPAS,
			...
			CAM_DBG(CAM_CPAS, "RPMH BCM Info [%d]=0x%x", i,
				soc_private->rpmh_info[i]);
		}
				      ...
				      void **irq_data)
{
	...
	for (i = 0; i < soc_info->irq_count; i++) {
		...
		if (!strcmp(soc_info->irq_name[i], "cpas_camnoc"))
			soc_private->irq_data[i].camnoc_type =
				...
				struct cam_hw_info *cpas_hw)
{
	...
	soc_private->irq_data = kcalloc(soc_info->irq_count,
	...
	if (soc_private->domain_id_info.domain_id_supported) {
		rc = cam_cpas_get_domain_id_support_clks(
			soc_info->pdev->dev.of_node, soc_info, soc_private);
		if (rc)
			goto release_res;
	}
...
int cam_cpas_soc_deinit_resources(struct cam_hw_soc_info *soc_info)
{
	...
	if (soc_private->icp_clk_index != -1) {
		rc = cam_soc_util_put_optional_clk(soc_info,
						   soc_private->icp_clk_index);
		if (rc)
			CAM_ERR(CAM_CPAS, "Error Put optional clk failed rc=%d",
				rc);
	}
				  ...
				  enum cam_vote_level default_level)
{
	...
	/* set this everytime in order to support debugfs to disable clk drv between
   * runs */
	soc_info->is_clk_drv_en = soc_private->enable_cam_clk_drv;

	rc = cam_soc_util_enable_platform_resource(
		soc_info, CAM_CLK_SW_CLIENT_IDX, true, default_level, true);
	if (rc)
				   ...
				   bool disable_clocks, bool disable_irq)
{
	...
	rc = cam_soc_util_disable_platform_resource(
		soc_info, CAM_CLK_SW_CLIENT_IDX, disable_clocks, disable_irq);
	if (rc)
