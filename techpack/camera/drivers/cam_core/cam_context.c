				     ...
				     struct cam_req_mgr_flush_request *flush)
{
	...
	if (ctx->state != CAM_CTX_FLUSHED) {
		if (ctx->state_machine[ctx->state].crm_ops.flush_req) {
			rc = ctx->state_machine[ctx->state].crm_ops.flush_req(
				ctx, flush);
		} else {
			CAM_INFO(
				CAM_CORE,
				    ...
				    struct cam_req_mgr_dump_info *dump)
{
	...
	if (ctx->state_machine[ctx->state].crm_ops.dump_req)
		rc = ctx->state_machine[ctx->state].crm_ops.dump_req(ctx, dump);
	else
...
int cam_context_mini_dump_from_hw(struct cam_context *ctx, void *args)
{
	int rc = 0;

	if (!ctx->state_machine) {
		CAM_ERR(CAM_CORE, "Context [id %d name:%s] is not ready",
			ctx->ctx_id, ctx->dev_name);
		return -EINVAL;
	}
	...
	if ((ctx->state >= CAM_CTX_AVAILABLE) &&
		...
		if (ctx->state_machine[ctx->state].mini_dump_ops)
			rc = ctx->state_machine[ctx->state].mini_dump_ops(ctx,
									  args);
		else
				...
				struct cam_dump_req_cmd *cmd)
{
	...
	if (ctx->state_machine[ctx->state].ioctl_ops.dump_dev)
		rc = ctx->state_machine[ctx->state].ioctl_ops.dump_dev(ctx,
								       cmd);
	else
...
int cam_context_handle_hw_recovery(void *priv, void *data)
{
	...
	if (ctx->state != CAM_CTX_ACTIVATED) {
		CAM_DBG(CAM_CORE,
			...
			ctx->ctx_id, ctx->dev_name, ctx->state);
		goto end;
	}
	CAM_DBG(CAM_CORE, "try hw recovery for ctx:%d dev:%s", ctx->ctx_id,
		ctx->dev_name);
