...
static inline struct icc_path *
cam_wrapper_icc_get(struct device *dev, const int src_id, const int dst_id)
{
...
static inline int cam_wrapper_icc_set_bw(struct icc_path *path, u32 avg_bw,
					 u32 peak_bw)
{
	if (debug_bypass_drivers & CAM_BYPASS_ICC) {
...
static inline void cam_wrapper_icc_set_tag(struct icc_path *path, u32 tag)
{
	if (debug_bypass_drivers & CAM_BYPASS_ICC) {
...
const char *
cam_soc_bus_path_data_to_str(enum cam_soc_bus_path_data bus_path_data)
{
...
int cam_soc_bus_client_update_request(void *client, unsigned int idx)
{
	...
	rc = cam_wrapper_icc_set_bw(
		...
		Bps_to_icc(ab), Bps_to_icc(ib));
	if (rc) {
		CAM_ERR(CAM_UTIL, "Update request failed, client[%s], idx: %d",
				 ...
				 enum cam_soc_bus_path_data bus_path_data)
{
	...
	rc = cam_wrapper_icc_set_bw(bus_client_data->icc_data[bus_path_data],
				    Bps_to_icc(ab), Bps_to_icc(ib));
	if (rc) {
	...
	void **client, struct cam_soc_bus_client_common_data *common_data)
{
	...
	*client = bus_client;
	...
	bus_client_data =
		kzalloc(sizeof(struct cam_soc_bus_client_data), GFP_KERNEL);
	if (!bus_client_data) {
		kfree(bus_client);
		*client = NULL;
		rc = -ENOMEM;
	...
	if (bus_client->common_data->is_drv_port) {
		...
		if (IS_ERR_OR_NULL(
			    bus_client_data
				    ->icc_data[CAM_SOC_BUS_PATH_DATA_DRV_HIGH])) {
			CAM_ERR(CAM_UTIL,
				"Failed to register DRV bus client Bus Client=[%s] : src=%d, "
				"dst=%d bus_path:%d",
				bus_client->common_data->src_id,
				bus_client->common_data->dst_id,
		...
		if (IS_ERR_OR_NULL(
			    bus_client_data
				    ->icc_data[CAM_SOC_BUS_PATH_DATA_DRV_LOW])) {
			CAM_ERR(CAM_UTIL,
				"Failed to register DRV bus client Bus Client=[%s] : src=%d, "
				"dst=%d bus_path:%d",
				bus_client->common_data->src_id,
