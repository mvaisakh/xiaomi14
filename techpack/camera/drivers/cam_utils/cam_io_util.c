			       ...
			       uint32_t len)
{
	...
	for (i = 0; i < len; i++) {
		CAM_DBG(CAM_IO_ACCESS, "i= %d len =%d val=%x addr =%pK", i, len,
			data[i], addr);
			      ...
			      uint32_t len)
{
	...
	for (i = 0; i < len; i++) {
		CAM_DBG(CAM_IO_ACCESS,
			"i= %d len =%d val=%x addr_base =%pK reg=%x", i, len,
			__VAL(i), addr_base, __OFFSET(i));
...
int cam_io_dump(void __iomem *base_addr, uint32_t start_offset, int size)
{
	...
	CAM_DBG(CAM_IO_ACCESS, "addr=%pK offset=0x%x size=%d", base_addr,
		start_offset, size);
