...
int cam_cpas_drv_channel_switch_for_dev(const struct device *dev)
{
	...
	if (rc) {
		CAM_ERR(CAM_CPAS, "[%s] Failed in DRV channel switch",
			dev_name(dev));
		return rc;
	}
...
int cam_ife_notify_safe_lut_scm(bool safe_trigger)
{
	...
	if (!rc) {
		...
		case CAM_CPAS_TITAN_175_V100:
			if (qcom_scm_smmu_notify_secure_lut(smmu_se_ife,
							    safe_trigger)) {
				CAM_ERR(CAM_ISP,
				  ...
				  bool is_shutdown)
{
	...
	if (!is_shutdown) {
		...
		if (rc) {
			CAM_ERR(CAM_CSIPHY,
				"Failed getting mink env object, rc: %d", rc);
			return rc;
		}

		rc = IClientEnv_open(client_env, CTrustedCameraDriver_UID,
				     &sc_object);
		if (rc) {
			CAM_ERR(CAM_CSIPHY,
				"Failed getting mink sc_object, rc: %d", rc);
			return rc;
		}
		...
		rc = ITrustedCameraDriver_dynamicProtectSensor(sc_object,
							       &params);
		if (rc) {
			CAM_ERR(CAM_CSIPHY, "Mink secure call failed, rc: %d",
				rc);
			return rc;
		}
		...
		if (rc) {
			CAM_ERR(CAM_CSIPHY,
				"Failed releasing secure camera object, rc: %d",
				rc);
			return rc;
		}
		...
		if (rc) {
			CAM_ERR(CAM_CSIPHY,
				"Failed releasing mink env object, rc: %d", rc);
			return rc;
		}
	} else {
...
int cam_csiphy_notify_secure_mode(struct csiphy_device *csiphy_dev,
				  bool protect, int32_t offset,
				  bool __always_unused is_shutdown)
{
	int rc = 0;
   ...
   */
	...
	if (offset >= CSIPHY_MAX_INSTANCES_PER_PHY) {
		CAM_ERR(CAM_CSIPHY, "Invalid CSIPHY offset");
		rc = -EINVAL;
	} else if (qcom_scm_camera_protect_phy_lanes(
			   protect, csiphy_dev->csiphy_info[offset]
...
#endif
		start_dev = NULL;
		while ((match_dev = bus_find_device(
				&platform_bus_type, start_dev, drv_ptr,
				&camera_platform_compare_dev))) {
...
#endif
		start_dev = NULL;
		while ((match_dev = bus_find_device(&i2c_bus_type, start_dev,
						    drv_ptr,
						    &camera_i2c_compare_dev))) {
			put_device(start_dev);
			    ...
			    struct cam_smmu_pf_info *pf_info)
{
	...
	CAM_ERR(CAM_SMMU, "smmu fault ids bid:%d pid:%d mid:%d", fault_ids.bid,
		fault_ids.pid, fault_ids.mid);
...
static int inline cam_subdev_list_cmp(struct cam_subdev *entry_1,
...
int cam_compat_util_get_dmabuf_va(struct dma_buf *dmabuf, uintptr_t *vaddr)
{
	...
	if (error_code) {
		...
		*vaddr = (mapping.is_iomem) ? (uintptr_t)mapping.vaddr_iomem :
...
void cam_smmu_util_iommu_custom(struct device *dev, dma_addr_t discard_start,
				size_t discard_length)
{
}

int cam_req_mgr_ordered_list_cmp(void *priv, const struct list_head *head_1,
				 const struct list_head *head_2)
{
...
int cam_req_mgr_ordered_list_cmp(void *priv, struct list_head *head_1,
...
int cam_compat_util_get_irq(struct cam_hw_soc_info *soc_info)
{
	...
	for (i = 0; i < soc_info->irq_count; i++) {
		soc_info->irq_line[i] = platform_get_resource_byname(
			soc_info->pdev, IORESOURCE_IRQ, soc_info->irq_name[i]);
		if (!soc_info->irq_line[i]) {
			CAM_ERR(CAM_UTIL,
				"Failed to get IRQ line for irq: %s of %s",
