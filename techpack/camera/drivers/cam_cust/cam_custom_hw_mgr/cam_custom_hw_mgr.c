...
static int cam_custom_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	custom_args = (struct cam_custom_start_args *)start_hw_args;

	hw_config = (struct cam_hw_config_args *)&custom_args->hw_config;
...
static int cam_custom_hw_mgr_notify(uint32_t evt_id,
...
static int cam_custom_hw_mgr_evt_handler(void *priv, uint32_t evt_id,
					 void *hw_evt_info)
{
	int rc = 0;
	...
	CAM_DBG(CAM_CUSTOM, "Invoked for HW Event ID 0x%x", evt_id);
	...
	struct cam_isp_in_port_generic_info **gen_port_info)
{
	...
	in_port_length =
		...
		(in->num_out_res - 1) * sizeof(struct cam_custom_out_port_info);
	...
	for (i = 0; i < prepare->packet->num_io_configs; i++) {
		...
		if (io_cfg[i].direction == CAM_BUF_OUTPUT) {
			...
			if (num_out_buf < prepare->max_out_map_entries) {
				...
				out_map_entries->sync_id = io_cfg[i].fence;
				num_out_buf++;
			} else {
...
static int
cam_custom_hw_mgr_reset_csid_res(struct cam_custom_hw_mgr_res *hw_mgr_res)
{
	int rc = -1;
...
err:
	CAM_ERR(CAM_CUSTOM, "RESET HW failed for res_id:%u",
		hw_mgr_res->res_id);
	return rc;
}

static int cam_custom_hw_mgr_reset(void *hw_mgr_priv, void *hw_reset_args)
{
	...
	ctx = (struct cam_custom_hw_mgr_ctx *)reset_args->ctxt_to_hw_map;
	if (!ctx || !ctx->ctx_in_use) {
		CAM_ERR(CAM_CUSTOM, "Invalid context is used");
		return -EPERM;
	}
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_custom_csid, list) {
		...
		if (rc) {
			CAM_ERR(CAM_CUSTOM, "Failed to reset CSID:%d rc: %d",
...
static int cam_custom_mgr_config_hw(void *hw_mgr_priv, void *hw_config_args)
{
	...
	prepare_hw_data = (struct cam_custom_prepare_hw_update_data *)cfg->priv;
	for (i = 0; i < prepare_hw_data->num_cfg; i++) {
		CAM_DBG(CAM_CUSTOM, "plane %d io_addr %pK", i,
	...
	for (i = 0; i < prepare_hw_data->num_cfg; i++) {
		CAM_DBG(CAM_CUSTOM, "plane %d io_addr %p cfg %u", i,
			prepare_hw_data->io_addr[i], prepare_hw_data->num_cfg);
...
static int cam_custom_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	custom_ctx =
		(struct cam_custom_hw_mgr_ctx *)hw_cmd_args->ctxt_to_hw_map;
			   ...
			   struct cam_hw_mgr_intf *hw_mgr_intf, int *iommu_hdl)
{
	...
	for (i = 0; i < CAM_CUSTOM_CSID_HW_MAX; i++) {
		...
		rc = cam_custom_csid_hw_init(&g_custom_hw_mgr.csid_devices[i],
					     i);
		CAM_DBG(CAM_CUSTOM, "csid custom module initialized. rc:%d",
			rc);
	}
   ...
   */
	...
	rc = cam_cpas_prepare_subpart_info(
		CAM_CUSTOM_HW_IDX, num_custom_available, num_custom_functional);
	if (rc)
		CAM_ERR(CAM_CUSTOM, "Failed to populate num_custom, rc: %d",
			rc);
