...
static void
__cam_req_mgr_reset_state_monitor_array(struct cam_req_mgr_core_link *link)
{
	int i = 0;
	...
	for (i = 0; i < MAX_REQ_STATE_MONITOR_NUM; i++) {
		...
		memset(&state_monitor->time_stamp, 0,
 ...
 */
static int32_t
__cam_req_mgr_name_to_dev_handle(char *dev_name,
				 struct cam_req_mgr_core_link *link)
{
	struct cam_req_mgr_connected_device *dev = NULL;
	int i = 0;
	...
	for (i = 0; i < link->num_devs; i++) {
		dev = &link->l_dev[i];
		CAM_DBG(CAM_CRM, "dev name %s, iterate dev name %s", dev_name,
			dev->dev_info.name);
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev->dev_info.p_delay == pd) {
			...
			if (link->wq_congestion)
				CAM_INFO_RATE_LIMIT(
					CAM_CRM,
					"WQ congestion, Skip Frame: req: %lld not ready on "
					"link: 0x%x for pd: %d dev: %s open_req count: %u",
					req_id, link->link_hdl, pd,
					dev->dev_info.name, link->open_req_cnt);
			else
				CAM_INFO(
					CAM_CRM,
					"Skip Frame: req: %lld not ready on link: 0x%x for pd: %d "
					"dev: %s open_req count: %u",
					req_id, link->link_hdl, pd,
					dev->dev_info.name, link->open_req_cnt);
		}
	}
...
static const char *
__cam_req_mgr_operation_type_to_str(enum cam_req_mgr_req_state type)
{
	switch (type) {
	...
	struct cam_req_mgr_state_monitor *state)
{
	...
	CAM_DBG(CAM_REQ,
		"Update: link_hdl %x dev %x dev_name %s req_id %lld frame_id %lld "
		"set to State: %s",
		link->link_hdl, state->dev_hdl, dev_name, state->req_id,
		state->frame_id,
	...
	scnprintf(state_monitor->name, sizeof(state_monitor->name), "%s",
		  dev_name);
	ktime_get_clocktai_ts64(&state_monitor->time_stamp);

	__cam_req_mgr_inc_idx(&link->req.next_state_idx, 1,
 ...
 */
static void
__cam_req_mgr_dump_state_monitor_array(struct cam_req_mgr_core_link *link)
{
	int i;
	int idx = link->req.next_state_idx;
	struct tm ts;
	...
	/*
   * Get current time and print it immediately, wen need this log to cacluate
   * the difference between log time and the time we get using
   * ktime_get_clocktai_ts64, we need do like this because the time we get
   * always has a 20-30ms difference from log time and do not have a good
   * solution at present.
   */
	...
	CAM_INFO(CAM_CRM, "%16s  %6s  %10s  %8s  %10s", "state", "req id",
		 "dev hdl", "frame id", "time stamp");
	spin_lock_bh(&link->req.monitor_slock);
	for (i = 0; i < MAX_REQ_STATE_MONITOR_NUM; i++) {
		if (link->req.state_monitor[idx].req_state !=
		    CAM_CRM_STATE_INVALID) {
			time64_to_tm(
				link->req.state_monitor[idx].time_stamp.tv_sec,
				0, &ts);
			CAM_INFO(
				CAM_CRM,
				...
				ts.tm_min, ts.tm_sec,
				link->req.state_monitor[idx].time_stamp.tv_nsec /
					1000000);
		}
 ...
 *
 */
static int __cam_req_mgr_notify_frame_skip(struct cam_req_mgr_core_link *link,
					   uint32_t trigger)
{
	int rc = 0, i, j, pd, idx;
	bool found = false;
     ...
     */
		...
		frame_skip.request_id = apply_data[pd].req_id;
 ...
 *
 */
static int __cam_req_mgr_send_evt(uint64_t req_id,
				   ...
				   struct cam_req_mgr_connected_device *dev)
{
	...
	__cam_req_mgr_send_evt(link->req.apply_data[link->min_delay].req_id,
			       CAM_REQ_MGR_LINK_EVT_STALLED, CRM_KMD_ERR_FATAL,
			       link);
 ...
 */
static void
__cam_req_mgr_disconnect_req_on_sync_link(struct cam_req_mgr_core_link *link,
					  struct cam_req_mgr_slot *slot)
{
	int i, j;
	int sync_idx;
	...
	for (i = 0; i < slot->num_sync_links; i++) {
		...
		CAM_DBG(CAM_CRM,
			...
			slot->req_id, link->link_hdl, sync_link->link_hdl,
			sync_idx);
 ...
 */
static int
__cam_req_mgr_move_to_next_req_slot(struct cam_req_mgr_core_link *link)
{
	int rc = 0;
	struct cam_req_mgr_req_queue *in_q = link->req.in_q;
   ...
   */
	if (slot->status == CRM_SLOT_STATUS_REQ_APPLIED) {
		CAM_WARN(
			CAM_CRM,
			"slot [idx: %d req: %lld last_applied_idx: %d] was not reset, "
			"reset it now",
			idx, in_q->slot[idx].req_id, in_q->last_applied_idx);
		if (in_q->last_applied_idx == idx) {
			CAM_WARN(CAM_CRM, "last_applied_idx: %d",
	...
	if (slot->status == CRM_SLOT_STATUS_NO_REQ) {
		...
		if (link->cont_empty_slots++ >= link->max_delay) {
			CAM_DBG(CAM_CRM,
				"There are %d continuous empty slots on link 0x%x",
		...
		if (in_q->wr_idx != idx)
			...
			__cam_req_mgr_inc_idx(&in_q->wr_idx, 1,
					      in_q->num_slots);
	} else
		link->cont_empty_slots = 0;
		       ...
		       struct cam_req_mgr_connected_device **failed_dev)
{
   ...
   */
	for (i = 0; i < link->num_devs; i++) {
		...
		if (!slot->ops.num_dev ||
		    (slot->ops.num_dev > MAX_DEV_FOR_SPECIAL_OPS)) {
			CAM_DBG(CAM_CRM,
		...
		if (!found) {
			CAM_DBG(CAM_CRM, "Dev_hdl : %d Not matched",
				dev->dev_hdl);
			continue;
		}
		...
		if (apply_data[pd].skip_idx || (apply_data[pd].req_id < 0)) {
			CAM_DBG(CAM_CRM, "dev %s skip %d req_id %lld",
				dev->dev_info.name, apply_data[pd].skip_idx,
			...
			apply_req.last_applied_max_pd_req =
				link->req.prev_apply_data[link->max_delay]
					.req_id;
			if ((dev->ops) && (dev->ops->notify_frame_skip))
		...
		apply_req.dev_hdl = dev->dev_hdl;
		apply_req.request_id = apply_data[pd].req_id;
		apply_req.trigger_point = trigger;
		if ((dev->ops) && (dev->ops->apply_req)) {
			...
			CAM_DBG(CAM_REQ,
				...
				dev->ops,
				(dev->ops ? dev->ops->apply_req : NULL));
			break;
		}
		...
		CAM_DBG(CAM_REQ, "SEND: link_hdl %x dev %s pd %d req_id %lld",
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev) {
			...
			if (apply_data[pd].skip_idx ||
				...
				apply_req.last_applied_max_pd_req =
					link->req
						.prev_apply_data[link->max_delay]
						.req_id;
				if ((dev->ops) && (dev->ops->notify_frame_skip))
					dev->ops->notify_frame_skip(&apply_req);
				continue;
				    ...
				    struct cam_req_mgr_slot *slot)
{
	...
	CAM_DBG(CAM_CRM,
		"link_hdl %x req %lld frame_skip_flag %d open_req_cnt:%u "
		"initial_sync_req [%lld,%lld] is_master:%d",
				      ...
				      uint32_t trigger)
{
	...
	CAM_DBG(CAM_REQ, "link_hdl %x sync link_hdl %x req %lld",
		link->link_hdl, sync_link->link_hdl, req_id);

	if ((sync_link->initial_skip) && (req_id >= sync_req_id)) {
		link->initial_skip = false;
	...
	slot_idx_diff =
		(sync_slot_idx - sync_rd_idx + sync_num_slots) % sync_num_slots;
	if ((sync_link->req.in_q->slot[sync_slot_idx].status !=
	     CRM_SLOT_STATUS_REQ_APPLIED) &&
	    ((slot_idx_diff > 1) ||
	     ((slot_idx_diff == 1) &&
	...
	if (sync_link->req.in_q) {
		rc = __cam_req_mgr_check_link_is_ready(sync_link, sync_slot_idx,
						       true);
...
static int __cam_req_mgr_check_multi_sync_link_ready(
	struct cam_req_mgr_core_link *link,
	struct cam_req_mgr_core_link **sync_link, struct cam_req_mgr_slot *slot,
	int32_t num_sync_links, uint32_t trigger)
{
	int i, rc = 0;
	...
	for (i = 0; i < num_sync_links; i++) {
		if (sync_link[i]) {
			if (slot->req_id <= sync_link[i]->last_flush_id) {
				CAM_DBG(CAM_CRM,
					"link:0x%x req:%lld has been flushed in sync link:0x%x, "
					"last_flush_id:%lld",
			...
			if (link->max_delay == sync_link[i]->max_delay) {
				rc = __cam_req_mgr_check_sync_req_is_ready(
					link, sync_link[i], slot, trigger);
				if (rc < 0) {
					CAM_DBG(CAM_CRM, "link %x not ready",
						link->link_hdl);
					return rc;
				}
			} else if (link->max_delay > sync_link[i]->max_delay) {
				link->is_master = true;
	...
	if (rc < 0) {
		CAM_DBG(CAM_CRM, "Req: %lld needs to inject delay at %s",
			slot->req_id,
			  ...
			  struct cam_req_mgr_trigger_notify *trigger_data)
{
	...
	struct cam_req_mgr_apply *apply_data = NULL;
	struct cam_req_mgr_core_link *sync_link[MAXIMUM_LINKS_PER_SESSION - 1];
   ...
   */
	CAM_DBG(CAM_REQ,
		"%s Req[%lld] idx %d req_status %d link_hdl %x wd_timeout %d ms "
		"trigger:%d",
		((trigger == CAM_TRIGGER_POINT_SOF) ? "SOF" : "EOF"),
     ...
     */
		...
		wq_sched_timeout = CAM_REQ_MGR_HALF_FRAME_DURATION(
					   link->sof_timestamp -
					   link->prev_sof_timestamp) /
				   CAM_COMMON_NS_PER_MS;

		/* Check for WQ congestion */
		if (jiffies_to_msecs(jiffies - link->last_sof_trigger_jiffies) <
		    wq_sched_timeout)
	...
	if (slot->status != CRM_SLOT_STATUS_REQ_READY) {
       ...
       */
			if (slot->num_sync_links == 0) {
				...
				for (i = 0; i < num_sync_links; i++)
					sync_link[i] = cam_get_link_priv(
						slot->sync_link_hdls[i]);
			}
       ...
       */
			...
			if (!rc) {
				rc = __cam_req_mgr_inject_delay(
					link->req.l_tbl, slot->idx, trigger);
				if (rc < 0)
					CAM_DBG(CAM_CRM,
       ...
       */
		...
		if (rc < 0) {
			CAM_DBG(CAM_CRM,
				...
				link->link_hdl, slot->req_id,
				(trigger == CAM_TRIGGER_POINT_SOF) ? "SOF" :
								     "EOF");
			__cam_req_mgr_notify_frame_skip(link, trigger);
			__cam_req_mgr_validate_crm_wd_timer(link);
			goto end;
	...
	if (rc < 0) {
		...
		if (!link->wq_congestion && dev) {
			...
			if (link->retry_cnt >= MAXIMUM_RETRY_ATTEMPTS) {
				CAM_DBG(CAM_CRM,
					"Max retry attempts (count %d) reached on link[0x%x] for req "
					"[%lld]",
				...
				trace_cam_delay_detect(
					"CRM", "Max retry attempts reached",
					in_q->slot[in_q->rd_idx].req_id,
					CAM_DEFAULT_VALUE, link->link_hdl,
         ...
         */
			...
			CAM_WARN_RATE_LIMIT(
				...
				in_q->last_applied_idx, in_q->rd_idx);
	} else {
		if (link->retry_cnt)
			link->retry_cnt = 0;
...
static int
__cam_req_mgr_flush_dev_with_max_pd(struct cam_req_mgr_core_link *link,
 ...
 */
static int
__cam_req_mgr_try_cancel_req(struct cam_req_mgr_core_link *link,
			     struct cam_req_mgr_flush_info *flush_info)
{
	struct cam_req_mgr_slot *slot = NULL;
	struct cam_req_mgr_req_queue *in_q = link->req.in_q;
	...
	case CRM_SLOT_STATUS_REQ_APPLIED:
		...
		while (pd + 1 <= link->max_delay) {
			if (flush_info->req_id <=
			    link->req.prev_apply_data[pd + 1].req_id)
				break;
			pd++;
		}
		fallthrough;
...
int cam_req_mgr_process_flush_req(void *priv, void *data)
{
	...
	case CAM_REQ_MGR_FLUSH_TYPE_CANCEL_REQ:
		...
		break;
	default:
		CAM_ERR(CAM_CRM, "Invalid flush type %u",
			flush_info->flush_type);
		rc = -EINVAL;
		break;
	}
...
int cam_req_mgr_process_sched_req(void *priv, void *data)
{
	...
	if (sched_req->additional_timeout < 0) {
		...
		CAM_WARN(
			CAM_CRM,
			"Requested timeout [%dms] max supported timeout [%dms] resetting "
			"to max",
			sched_req->additional_timeout,
			CAM_REQ_MGR_WATCHDOG_TIMEOUT_MAX);
	...
	for (i = 0; i < sched_req->num_links; i++) {
		if (link->link_hdl != sched_req->link_hdls[i]) {
			...
			CAM_DBG(CAM_REQ,
				"link:0x%x req:%lld sync_link[%d]:0x%x",
	...
	CAM_DBG(CAM_REQ, "Open_req_cnt: %u after scheduling req: %d",
		link->open_req_cnt, sched_req->req_id);
 ...
 * @return   : 0 for success, negative for failure.
 */
int cam_req_mgr_sync_info_for_sync_mode(struct cam_req_mgr_core_link *link,
					...
					uint32_t type)
{
	...
	for (i = 0; i < link->num_sync_links; i++) {
		...
		dev_name = __cam_req_mgr_dev_handle_to_name(add_req->dev_hdl,
							    link);
		...
		case CAM_SYNC_TYPE_APPLY_AT_EOF:
			...
			if (sync_slot->ops.num_dev == MAX_DEV_FOR_SPECIAL_OPS)
				CAM_WARN(
					...
					link->sync_link[i]->link_hdl, idx,
					sync_slot->ops.num_dev);
			else
				sync_slot->ops
					.dev_hdl[sync_slot->ops.num_dev++] =
					sync_dev_hdl;
			break;
		default:
   ...
   */
	...
	if ((add_req->skip_at_sof & 0xFF) > slot->inject_delay_at_sof) {
		...
		CAM_DBG(CAM_CRM,
			...
			add_req->req_id, slot->inject_delay_at_sof,
		...
		if (link_slot->sync_mode == CAM_REQ_MGR_SYNC_MODE_SYNC) {
			...
			if (rc < 0) {
				CAM_ERR(CAM_CRM,
					"Req_id %llu injecting delay %llu failed frame at SOF by %s "
					"for sync link",
					add_req->req_id,
					slot->inject_delay_at_sof,
					device->dev_info.name);
	...
	if ((add_req->skip_at_eof & 0xFF) > slot->inject_delay_at_eof) {
		...
		CAM_DBG(CAM_CRM,
			...
			add_req->req_id, slot->inject_delay_at_eof,
			device->dev_info.name);
		...
		if (link_slot->sync_mode == CAM_REQ_MGR_SYNC_MODE_SYNC) {
			...
			if (rc < 0) {
				CAM_ERR(CAM_CRM,
					"Req_id %llu injecting delay %llu failed frame at EOF by %s "
					"for sync link",
					add_req->req_id,
					slot->inject_delay_at_eof,
					device->dev_info.name);
	...
	if (add_req->trigger_eof) {
		...
		if (link_slot->sync_mode == CAM_REQ_MGR_SYNC_MODE_SYNC) {
			...
			if (rc < 0) {
				CAM_ERR(CAM_REQ,
					"Req_id %llu slot:%d added failed for EOF tigger for Device: "
					"%s for sync link",
					add_req->req_id, idx,
					device->dev_info.name);
				...
				CAM_DBG(CAM_REQ,
					"Req_id %llu slot:%d added for EOF tigger for Device: %s for "
					"sync link",
					add_req->req_id, idx,
					device->dev_info.name);
			}
 ...
 *
 */
void __cam_req_mgr_apply_on_bubble(struct cam_req_mgr_core_link *link,
				   struct cam_req_mgr_error_notify *err_info)
{
	...
	trigger_data.sof_timestamp_val = err_info->sof_timestamp_val;
	trigger_data.trigger = err_info->trigger;

	rc = __cam_req_mgr_process_req(link, &trigger_data);
	if (rc)
		CAM_ERR(CAM_CRM, "Failed to apply request on bubbled frame");
}
...
int cam_req_mgr_process_error(void *priv, void *data)
{
	...
	case CRM_KMD_WARN_INTERNAL_RECOVERY:
       ...
       * it again.
       */
			if ((in_q->slot[idx].bubble_times >=
			     REQ_MAXIMUM_BUBBLE_TIMES) &&
       ...
       */
			...
			for (i = 0; i < slot_diff; i++) {
				...
				CAM_DBG(CAM_CRM,
					...
					in_q->rd_idx, idx,
					in_q->slot[idx].status);
				if (in_q->slot[idx].status ==
				    CRM_SLOT_STATUS_REQ_APPLIED) {
					in_q->slot[idx].status =
...
static void cam_req_mgr_notify_eof_event(struct cam_req_mgr_core_link *link)
{
	...
	CAM_DBG(CAM_CRM, "Notify EOF event done on link:0x%x", link->link_hdl);
}
...
static int cam_req_mgr_cb_add_req(struct cam_req_mgr_add_request *add_req)
{
	...
	if (idx < 0) {
		if (((uint32_t)add_req->req_id) <= (link->last_flush_id)) {
			CAM_INFO(
				CAM_CRM,
				"req %lld not found in in_q; it has been flushed "
				"[last_flush_req %lld] link 0x%x",
	...
	if (dev_req->trigger_eof) {
		...
		for (i = 0; i < link->num_sync_links; i++) {
			mutex_lock(&link->sync_link[i]->req.lock);
			atomic_set(&link->sync_link[i]->eof_event_cnt,
				   link->eof_event_cnt.counter);
			CAM_DBG(CAM_REQ,
				     ...
				     uint32_t trigger)
{
	...
	if ((link->trigger_cnt[0][trigger] &&
	    ...
	    (link->trigger_cnt[1][trigger] &&
	     (link->trigger_cnt[1][trigger] - link->trigger_cnt[0][trigger] >
	      1))) {
		CAM_WARN(CAM_CRM,
...
cam_req_mgr_cb_notify_timer(struct cam_req_mgr_timer_notify *timer_data)
{
	...
	if (link->watchdog) {
		...
		crm_timer_reset(link->watchdog);
		CAM_DBG(CAM_CRM, "link %x pause_timer %d", link->link_hdl,
			link->watchdog->pause_timer);
	}
...
cam_req_mgr_cb_notify_trigger(struct cam_req_mgr_trigger_notify *trigger_data)
{
	...
	in_q = link->req.in_q;
	if (!in_q) {
		CAM_DBG(CAM_CRM, "in_q ptr NULL, link_hdl %x",
   ...
   */
	...
	spin_lock_bh(&link->link_state_spin_lock);
	if (link->state < CAM_CRM_LINK_STATE_READY) {
		CAM_WARN_RATE_LIMIT(CAM_CRM, "invalid link state:%d",
				    link->state);
		spin_unlock_bh(&link->link_state_spin_lock);
		rc = -EPERM;
		goto end;
					 ...
					 struct cam_req_mgr_ver_info *link_info)
{
	...
	for (i = 0; i < num_devices; i++) {
		...
		if (rc < 0 || dev->dev_info.p_delay >= CAM_PIPELINE_DELAY_MAX ||
			...
			goto error;
		} else if (dev->dev_info.m_delay >= CAM_MODESWITCH_DELAY_MAX ||
			   dev->dev_info.m_delay < CAM_MODESWITCH_DELAY_0) {
	...
	if (num_trigger_devices > CAM_REQ_MGR_MAX_TRIGGERS) {
		CAM_ERR(CAM_CRM, "Unsupported number of trigger devices %u",
			num_trigger_devices);
		rc = -EINVAL;
		goto error;
	}
 ...
 *
 */
static int __cam_req_mgr_unlink(struct cam_req_mgr_core_session *session,
				struct cam_req_mgr_core_link *link)
{
	int rc;
	...
	if (sched_req->sync_mode == CAM_REQ_MGR_SYNC_MODE_SYNC) {
		if ((sched_req->num_links <= 0) &&
			...
			CAM_ERR(CAM_CRM,
				...
				link->link_hdl, sched_req->req_id,
				sched_req->num_links);
			rc = -EINVAL;
			goto end;
		}
		...
		for (i = 0; i < sched_req->num_links; i++) {
			if (!sched_req->link_hdls[i]) {
				CAM_ERR(CAM_CRM,
					"link handle %d in sched_req is null",
					i);
				rc = -EINVAL;
				goto end;
			}
			...
			sync_links[i] =
				cam_get_link_priv(sched_req->link_hdls[i]);
			if (!sync_links[i] || (sync_links[i]->link_hdl !=
					       sched_req->link_hdls[i])) {
				CAM_ERR(CAM_CRM,
					"Invalid sync link, sync link[%d]: %s sched_req->link_hdl: %x "
					"sync_links->link_hdl: 0x%x",
					i, CAM_IS_NULL_TO_STR(sync_links[i]),
					sched_req->link_hdls[i],
					((!sync_links[i]) ?
						 CAM_REQ_MGR_DEFAULT_HDL_VAL :
...
int cam_req_mgr_sync_config(struct cam_req_mgr_sync_mode *sync_info)
{
	...
	for (i = 0; i < sync_info->num_links; i++) {
		...
		if (!link[i] ||
		    (link[i]->link_hdl != sync_info->link_hdls[i])) {
			CAM_ERR(CAM_CRM,
				"link: %s, sync_info->link_hdl:%x, link->link_hdl:%x",
				CAM_IS_NULL_TO_STR(link),
				sync_info->link_hdls[i],
				(!link[i]) ? CAM_REQ_MGR_DEFAULT_HDL_VAL :
					     link[i]->link_hdl);
			rc = -EINVAL;
			goto done;
		}
	...
	if (sync_info->sync_mode == CAM_REQ_MGR_SYNC_MODE_SYNC) {
		for (i = 0; i < sync_info->num_links; i++) {
			...
			sync_idx = 0;
			CAM_DBG(CAM_REQ,
				"link %x adds sync link:", link[i]->link_hdl);
	...
	CAM_DBG(CAM_REQ, "Sync config completed on %d links with sync_mode %d",
...
int cam_req_mgr_flush_requests(struct cam_req_mgr_flush_info *flush_info)
{
	...
	if (!session || (session->session_hdl != flush_info->session_hdl)) {
		CAM_ERR(CAM_CRM,
			"session: %s, flush_info->ses_hdl:%x, session->ses_hdl:%x",
	...
	if (!link || (link->link_hdl != flush_info->link_hdl)) {
		CAM_ERR(CAM_CRM,
			"link: %s, flush_info->link_hdl:%x, link->link_hdl:%x",
...
int cam_req_mgr_link_control(struct cam_req_mgr_link_control *control)
{
	...
	for (i = 0; i < control->num_links; i++) {
		...
		if (!link || (link->link_hdl != control->link_hdls[i])) {
			CAM_ERR(CAM_CRM,
				"link: %s, control->link_hdl:%x, link->link_hdl:%x",
				CAM_IS_NULL_TO_STR(link), control->link_hdls[i],
				(!link) ? CAM_REQ_MGR_DEFAULT_HDL_VAL :
					  link->link_hdl);
			rc = -EINVAL;
			break;
		}
		...
		if (control->ops == CAM_REQ_MGR_LINK_ACTIVATE) {
			...
			crm_timer_exit(&link->watchdog);
			spin_unlock_bh(&link->link_state_spin_lock);
			CAM_DBG(CAM_CRM, "De-activate link: 0x%x",
				link->link_hdl);
		} else {
...
int cam_req_mgr_link_properties(struct cam_req_mgr_link_properties *properties)
{
	...
	CAM_DBG(CAM_CRM,
		"link 0x%x set properties successfully, properties mask:0x%x",
...
static void *cam_req_mgr_user_dump_state_monitor_array_info(void *dump_struct,
							    uint8_t *addr_ptr)
{
	...
	*addr++ = evt->req_id;
	...
	scnprintf(hdr->tag, CAM_CRM_DUMP_TAG_MAX_LEN, "%s", evt->name);
	hdr->word_size = sizeof(uint64_t);
	addr = (uint64_t *)(dst + sizeof(struct cam_common_hw_dump_header));

	dst = (uint8_t *)addr;
	hdr = (struct cam_common_hw_dump_header *)dst;
	scnprintf(hdr->tag, CAM_CRM_DUMP_TAG_MAX_LEN, "%s",
		  __cam_req_mgr_operation_type_to_str(evt->req_state));
	hdr->word_size = sizeof(uint64_t);
	addr = (uint64_t *)(dst + sizeof(struct cam_common_hw_dump_header));
 ...
 */
static int
cam_req_mgr_dump_state_monitor_info(struct cam_req_mgr_core_link *link,
				    struct cam_req_mgr_dump_info *dump_info)
{
	...
	min_len = sizeof(struct cam_common_hw_dump_header) +
		  (MAX_REQ_STATE_MONITOR_NUM *
		   (CAM_CRM_DUMP_EVENT_NUM_WORDS * sizeof(uint64_t) +
		    sizeof(struct cam_common_hw_dump_header) *
			    CAM_CRM_DUMP_EVENT_NUM_HEADERS));
	...
	for (i = 0; i < MAX_REQ_STATE_MONITOR_NUM; i++) {
		if (link->req.state_monitor[idx].req_state !=
			...
			rc = cam_common_user_dump_helper(
				...
				&link->req.state_monitor[idx], sizeof(uint64_t),
				"CRM_STATE_MONITOR.%x:", link->link_hdl);
			if (rc) {
				CAM_ERR(CAM_CRM,
					"Dump state info failed, rc: %d", rc);
				cam_mem_put_cpu_buf(dump_info->buf_handle);
				return rc;
			}
		}
...
int cam_req_mgr_dump_request(struct cam_dump_req_cmd *dump_req)
{
	...
	if (!session || (session->session_hdl != dump_req->session_handle)) {
		CAM_ERR(CAM_CRM,
			"session: %s, dump_req->ses_hdl:%x, session->ses_hdl:%x",
	...
	if (!link || (link->link_hdl != dump_req->link_hdl)) {
		CAM_ERR(CAM_CRM,
			"link: %s, dump_req->link_hdl:%x, link->link_hdl:%x",
   ...
   */
	...
	if (rc)
		CAM_ERR(CAM_REQ, "Fail to dump mem mgr rc %d", rc);
   ...
   */
	...
	if (rc)
		CAM_ERR(CAM_REQ, "Fail to dump state monitor req %llu rc %d",
			info.req_id, rc);
	...
	CAM_INFO(CAM_REQ, "req %llu, offset %zu", dump_req->issue_req_id,
		 dump_req->offset);
end:
	mutex_unlock(&g_crm_core_dev->crm_lock);
	return rc;
						   ...
						   void *priv_data)
{
	...
	if (!dst || len < sizeof(*md)) {
		CAM_ERR(CAM_CRM,
			"Insufficient received length %lu dumped len: %u", len,
			dumped_len);
		return 0;
	}
