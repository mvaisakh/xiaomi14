	...
	struct cam_fd_hw_cmd_prestart_args *prestart_args)
{
	...
	if (num_regval_pairs) {
		...
		ctx_hw_private->cdm_ops->cdm_write_regrandom(
					     ...
					     void *args)
{
	...
	if (!fd_hw || !args) {
		CAM_ERR(CAM_FD, "Invalid args %pK %pK", fd_hw, args);
		return -EINVAL;
	}
	...
	scnprintf(hdr->tag, CAM_FD_HW_DUMP_TAG_MAX_LEN, "FD_REG:");
			  ...
			  uint32_t arg_size)
{
	...
	case CAM_FD_HW_CMD_HW_DUMP: {
		rc = cam_fd_hw_util_processcmd_hw_dump(fd_hw, cmd_args);
		break;
	}
	default:
		break;
