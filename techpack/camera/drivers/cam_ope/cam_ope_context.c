...
static int cam_ope_context_dump_active_request(void *data, void *args)
{
	...
	if (!ctx || !pf_args) {
		CAM_ERR(CAM_OPE, "Invalid ctx %pK or pf args %pK", ctx,
			pf_args);
		return -EINVAL;
	}

	CAM_INFO(CAM_OPE, "iommu fault for ope ctx %d state %d", ctx->ctx_id,
		 ctx->state);
...
static int __cam_ope_handle_buf_done_in_ready(void *ctx, uint32_t evt_id,
					      void *done)
{
	return cam_context_buf_done_from_hw(ctx, done, evt_id);
}

static struct cam_ctx_ops cam_ope_ctx_state_machine[CAM_CTX_STATE_MAX] = {
    /* Uninit */
    {
        .ioctl_ops = {},
        .crm_ops = {},
			 ...
			 int img_iommu_hdl)
{
	...
	rc = cam_context_init(ctx->base, ope_dev_name, CAM_OPE, ctx_id, NULL,
