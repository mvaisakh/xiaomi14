...
int cam_ope_get_hw_caps(void *hw_priv, void *get_hw_cap_args, uint32_t arg_size)
{
	...
	if ((!soc_info) || (!core_info)) {
		CAM_ERR(CAM_OPE, "soc_info = %x core_info = %x", soc_info,
			core_info);
		return -EINVAL;
	}
...
static int cam_ope_dev_process_init(struct ope_hw *ope_hw, void *cmd_args)
{
	int rc = 0;
...
bus_wr_init_fail:
	rc = cam_ope_bus_rd_process(ope_hw, 0, OPE_HW_DEINIT, NULL);
bus_rd_init_fail:
	rc = cam_ope_top_process(ope_hw, 0, OPE_HW_DEINIT, NULL);
top_init_fail:
	return rc;
}

static int cam_ope_process_init(struct ope_hw *ope_hw, void *cmd_args,
				bool hfi_en)
{
	if (!hfi_en)
...
int cam_ope_init_hw(void *device_priv, void *init_hw_args, uint32_t arg_size)
{
	...
	core_info = (struct cam_ope_device_core_info *)ope_dev->core_info;
	if ((!soc_info) || (!core_info)) {
		CAM_ERR(CAM_OPE, "soc_info = %pK core_info = %pK", soc_info,
			core_info);
		rc = -EINVAL;
		goto end;
	}
...
int cam_ope_deinit_hw(void *device_priv, void *init_hw_args, uint32_t arg_size)
{
	struct cam_hw_info *ope_dev = device_priv;
	struct cam_hw_soc_info *soc_info = NULL;
	struct cam_ope_device_core_info *core_info = NULL;
...
static int cam_ope_dev_process_dump_debug_reg(struct ope_hw *ope_hw)
{
	...
	rc = cam_ope_top_process(ope_hw, -1, OPE_HW_DUMP_DEBUG, NULL);
...
static int cam_ope_dev_process_reset(struct ope_hw *ope_hw, void *cmd_args)
{
	...
	rc = cam_ope_top_process(ope_hw, -1, OPE_HW_RESET, NULL);
...
static int cam_ope_dev_process_acquire(struct ope_hw *ope_hw, void *cmd_args)
{
	...
	if (!cmd_args || !ope_hw) {
		CAM_ERR(CAM_OPE, "Invalid arguments: %pK %pK", cmd_args,
			ope_hw);
		return -EINVAL;
	}
...
bus_wr_acquire_fail:
	cam_ope_bus_rd_process(ope_hw, ope_dev_acquire->ctx_id, OPE_HW_RELEASE,
			       ope_dev_acquire->ope_acquire);
bus_rd_acquire_fail:
	cam_ope_top_process(ope_hw, ope_dev_acquire->ctx_id, OPE_HW_RELEASE,
			    ope_dev_acquire->ope_acquire);
top_acquire_fail:
	return rc;
}
...
static int cam_ope_dev_prepare_cdm_request(
	...
	struct cam_ope_ctx *ctx_data, uint32_t req_idx, uint32_t kmd_buf_offset,
	struct cam_ope_dev_prepare_req *ope_dev_prepare_req, uint32_t len,
	bool arbitrate)
{
	int i;
	...
	kmd_buf =
		(uint32_t *)ope_request->ope_kmd_buf.cpu_addr + kmd_buf_offset;
	...
	cdm_cmd->cmd[i].offset =
		kmd_buf_offset + ope_request->ope_kmd_buf.offset;
...
static int dump_dmi_cmd(uint32_t print_idx, uint32_t *print_ptr,
			struct cdm_dmi_cmd *dmi_cmd, uint32_t *temp)
{
	CAM_DBG(CAM_OPE, "%d:dma_ptr:%x l:%d", print_idx, print_ptr,
		dmi_cmd->length);
	CAM_DBG(CAM_OPE, "%d:cmd:%hhx addr:%x", print_ptr, dmi_cmd->cmd,
		dmi_cmd->addr);
	CAM_DBG(CAM_OPE, "%d: dmiadr:%x sel:%d", print_idx, dmi_cmd->DMIAddr,
		dmi_cmd->DMISel);
	CAM_DBG(CAM_OPE, "%d: %x %x %x", print_idx, temp[0], temp[1], temp[2]);

	return 0;
}

static int dump_direct_cmd(uint32_t print_idx, uint32_t *print_ptr,
			   struct ope_frame_process *frm_proc, int batch_idx,
			   int cmd_buf_idx)
{
	int len;

	if (cmd_buf_idx >= OPE_MAX_CMD_BUFS || batch_idx >= OPE_MAX_BATCH_SIZE)
		return 0;
...
static int dump_frame_cmd(struct ope_frame_process *frm_proc, int i, int j,
			  uint64_t iova_addr, uint32_t *kmd_buf,
			  uint32_t buf_len)
{
	...
	CAM_DBG(CAM_OPE, "kmdbuf:%x memhdl:%x iova:%x %pK", kmd_buf,
		frm_proc->cmd_buf[i][j].mem_handle, iova_addr, iova_addr);
	CAM_DBG(CAM_OPE, "buflen:%d len:%d offset:%d", buf_len,
		frm_proc->cmd_buf[i][j].length, frm_proc->cmd_buf[i][j].offset);

	return 0;
}

static int dump_stripe_cmd(struct ope_frame_process *frm_proc,
			   uint32_t stripe_idx, int i, int k,
			   uint64_t iova_addr, uint32_t *kmd_buf,
			   uint32_t buf_len)
{
	...
	CAM_DBG(CAM_OPE, "iova:%x %pK buflen:%d len:%d", iova_addr, iova_addr,
		buf_len, frm_proc->cmd_buf[i][k].length);
	CAM_DBG(CAM_OPE, "offset:%d", frm_proc->cmd_buf[i][k].offset);
	return 0;
}

static uint32_t *
ope_create_frame_cmd_batch(struct cam_ope_hw_mgr *hw_mgr,
			   ...
			   struct cam_ope_dev_prepare_req *ope_dev_prepare_req)
{
	...
	wr_cdm_info = &ope_dev_prepare_req->wr_cdm_batch->io_port_cdm[0];
	rd_cdm_info = &ope_dev_prepare_req->rd_cdm_batch->io_port_cdm[0];
	...
	for (j = 0; j < frm_proc->num_cmd_bufs[i]; j++) {
		...
		if (frm_proc->cmd_buf[i][j].cmd_buf_usage == OPE_CMD_BUF_KMD ||
		    frm_proc->cmd_buf[i][j].cmd_buf_usage == OPE_CMD_BUF_DEBUG)
			continue;

		if (frm_proc->cmd_buf[i][j].cmd_buf_buffered != buffered)
			continue;

		if (!frm_proc->cmd_buf[i][j].mem_handle)
			continue;

		rc = cam_mem_get_io_buf(frm_proc->cmd_buf[i][j].mem_handle,
					hw_mgr->iommu_cdm_hdl, &iova_addr,
					&buf_len, NULL, NULL);
		if (rc) {
		...
		rc = cam_mem_get_cpu_buf(frm_proc->cmd_buf[i][j].mem_handle,
					 &cpu_addr, &buf_len);
		if (rc || !cpu_addr) {
			CAM_ERR(CAM_OPE, "get cmd buf failed %x",
				hw_mgr->iommu_hdl);
		...
		if (frm_proc->cmd_buf[i][j].type == OPE_CMD_BUF_TYPE_DIRECT) {
			kmd_buf = cdm_ops->cdm_write_indirect(
				kmd_buf, iova_addr,
				frm_proc->cmd_buf[i][j].length);
			print_ptr = (uint32_t *)cpu_addr;
			dump_direct_cmd(print_idx, print_ptr, frm_proc, i, j);
		} else {
		...
		if (hw_mgr->frame_dump_enable)
			dump_frame_cmd(frm_proc, i, j, iova_addr, kmd_buf,
				       buf_len);
...
static uint32_t *
ope_create_frame_wr(struct cam_ope_ctx *ctx_data,
		    struct ope_bus_wr_io_port_cdm_info *wr_cdm_info,
		    uint32_t *kmd_buf, struct cam_ope_request *ope_request)
{
	...
	for (i = 0; i < wr_cdm_info->num_frames_cmds; i++) {
		...
		CAM_DBG(CAM_OPE, "FrameWR:i:%d kmdbuf:%x len:%d iova:%x %pK", i,
			kmd_buf, wr_cdm_info->f_cdm_info[i].len,
...
static uint32_t *
ope_create_frame_rd(struct cam_ope_ctx *ctx_data,
		    struct ope_bus_rd_io_port_cdm_info *rd_cdm_info,
		    uint32_t *kmd_buf, struct cam_ope_request *ope_request)
{
	struct cam_cdm_utils_ops *cdm_ops;
	...
	for (i = 0; i < rd_cdm_info->num_frames_cmds; i++) {
		...
		CAM_DBG(CAM_OPE, "FrameRD:i:%d kmdbuf:%x len:%d iova:%x %pK", i,
			kmd_buf, rd_cdm_info->f_cdm_info[i].len,
			ope_request->ope_kmd_buf.iova_cdm_addr,
			ope_request->ope_kmd_buf.iova_cdm_addr);
	}
	return kmd_buf;
...
static uint32_t *
ope_create_frame_cmd(struct cam_ope_hw_mgr *hw_mgr,
		     struct cam_ope_ctx *ctx_data, uint32_t req_idx,
	...
	for (i = 0; i < frm_proc->batch_size; i++) {
		for (j = 0; j < frm_proc->num_cmd_bufs[i]; j++) {
			...
			rc = cam_mem_get_cpu_buf(
				frm_proc->cmd_buf[i][j].mem_handle, &cpu_addr,
				&buf_len);
			if (rc || !cpu_addr) {
				CAM_ERR(CAM_OPE, "get cmd buf failed %x",
					hw_mgr->iommu_hdl);
			...
			if (frm_proc->cmd_buf[i][j].type ==
				...
				CAM_DBG(CAM_OPE, "Frame DB : In direct: E");
				print_ptr = (uint32_t *)cpu_addr;
				for (print_idx = 0; print_idx < num_dmi;
				     print_idx++) {
					memcpy(temp, (const void *)print_ptr,
					       sizeof(struct cdm_dmi_cmd));
					dmi_cmd = (struct cdm_dmi_cmd *)temp;
			...
			if (hw_mgr->frame_dump_enable)
				dump_frame_cmd(frm_proc, i, j, iova_addr,
					       kmd_buf, buf_len);
				       ...
				       struct ope_frame_process *frm_proc)
{
	...
	if (s_idx >= OPE_MAX_CMD_BUFS || batch_idx >= OPE_MAX_BATCH_SIZE) {
		CAM_ERR(CAM_OPE, "Invalid inputs: %d %d", batch_idx, s_idx);
		return NULL;
	}
	...
	for (k = 0; k < frm_proc->num_cmd_bufs[i]; k++) {
		...
		if (frm_proc->cmd_buf[i][k].stripe_idx != stripe_idx)
			continue;
		...
		if (frm_proc->cmd_buf[i][k].type == OPE_CMD_BUF_TYPE_DIRECT) {
			...
			print_ptr = (uint32_t *)cpu_addr;
			CAM_DBG(CAM_OPE, "Stripe:%d direct:E", stripe_idx);
			if (hw_mgr->frame_dump_enable)
				dump_direct_cmd(print_idx, print_ptr, frm_proc,
						i, k);
		...
		if (hw_mgr->frame_dump_enable)
			dump_stripe_cmd(frm_proc, stripe_idx, i, k, iova_addr,
					kmd_buf, buf_len);
...
static uint32_t *
ope_create_stripe_wr(struct cam_ope_ctx *ctx_data, uint32_t stripe_idx,
		     struct ope_bus_wr_io_port_cdm_info *wr_cdm_info,
		     struct cam_ope_request *ope_request, uint32_t *kmd_buf)
{
	...
	for (k = 0; k < wr_cdm_info->num_s_cmd_bufs[stripe_idx]; k++) {
		...
		CAM_DBG(CAM_OPE, "WR stripe:%d %d kmdbuf:%x", stripe_idx, k,
			kmd_buf);
...
static uint32_t *
ope_create_stripe_rd(struct cam_ope_ctx *ctx_data, uint32_t stripe_idx,
		     struct ope_bus_rd_io_port_cdm_info *rd_cdm_info,
		     struct cam_ope_request *ope_request, uint32_t *kmd_buf)
{
	struct cam_cdm_utils_ops *cdm_ops;
...
static uint32_t *
ope_create_stripes_batch(struct cam_ope_hw_mgr *hw_mgr,
			 struct cam_ope_ctx *ctx_data, uint32_t req_idx,
			 uint32_t *kmd_buf, int batch_idx,
			 struct cam_ope_dev_prepare_req *ope_dev_prepare_req)
{
	...
	wr_cdm_info = &ope_dev_prepare_req->wr_cdm_batch->io_port_cdm[i];
	rd_cdm_info = &ope_dev_prepare_req->rd_cdm_batch->io_port_cdm[i];
	for (j = 0; j < ope_request->num_stripes[i]; j++) {
		/* cmd buffer stripes */
		kmd_buf = ope_create_stripe_cmd(hw_mgr, ctx_data, kmd_buf, i, j,
						stripe_idx, frm_proc);
		if (!kmd_buf)
			goto end;
		...
		CAM_DBG(CAM_OPE, "Go cmd for stripe:%d kmd_buf:%x", stripe_idx,
			kmd_buf);
...
static uint32_t *
ope_create_stripes(struct cam_ope_hw_mgr *hw_mgr, struct cam_ope_ctx *ctx_data,
		   uint32_t req_idx, uint32_t *kmd_buf,
		   struct cam_ope_dev_prepare_req *ope_dev_prepare_req)
{
	int i, j;
	struct cam_ope_request *ope_request;
...
static uint32_t *
ope_create_stripes_nrt(struct cam_ope_hw_mgr *hw_mgr,
		       struct cam_ope_ctx *ctx_data, uint32_t req_idx,
		       uint32_t *kmd_buf,
		       struct cam_ope_dev_prepare_req *ope_dev_prepare_req,
		       uint32_t *kmd_buf_offset, uint32_t **cdm_kmd_start_addr)
{
	...
	for (i = 0; i < frm_proc->batch_size; i++) {
		...
		for (j = 0; j < ope_request->num_stripes[i]; j++) {
			...
			if (!num_nrt_stripes) {
				kmd_buf = cdm_ops->cdm_write_wait_comp_event(
					kmd_buf, OPE_WAIT_COMP_IDLE, 0x0);
			...
			if (!num_nrt_stripes) {
				...
				/* Frame 0 DB */
				kmd_buf = ope_create_frame_cmd(
					hw_mgr, ctx_data, req_idx, kmd_buf,
					OPE_CMD_BUF_DOUBLE_BUFFERED,
					ope_dev_prepare_req);
				if (!kmd_buf)
					goto end;
				...
				if (!kmd_buf)
					goto end;
				num_arb--;
				num_nrt_stripes = ctx_data->ope_acquire
							  .nrt_stripes_for_arb;
			}
			// add go command
			kmd_buf = cdm_ops->cdm_write_indirect(
...
static int cam_ope_dev_create_kmd_buf_nrt(
	struct cam_ope_hw_mgr *hw_mgr,
	struct cam_hw_prepare_update_args *prepare_args,
	struct cam_ope_ctx *ctx_data, uint32_t req_idx,
	struct cam_ope_dev_prepare_req *ope_dev_prepare_req)
{
	...
	kmd_buf = cdm_ops->cdm_write_clear_comp_event(
		kmd_buf, OPE_WAIT_COMP_IDLE | OPE_WAIT_COMP_RUP, 0x0);

	/* Frame 0 DB */
	kmd_buf = ope_create_frame_cmd(hw_mgr, ctx_data, req_idx, kmd_buf,
				       OPE_CMD_BUF_DOUBLE_BUFFERED,
				       ope_dev_prepare_req);
	if (!kmd_buf) {
		rc = -EINVAL;
	...
	/* Frame 0 WR */
	kmd_buf = ope_create_frame_wr(ctx_data, wr_cdm_info, kmd_buf,
				      ope_request);
	if (!kmd_buf) {
		rc = -EINVAL;
		goto end;
	...
	/* Frame 0 RD */
	kmd_buf = ope_create_frame_rd(ctx_data, rd_cdm_info, kmd_buf,
				      ope_request);
	if (!kmd_buf) {
		rc = -EINVAL;
		goto end;
	...
	kmd_buf = ope_create_stripes_nrt(hw_mgr, ctx_data, req_idx, kmd_buf,
					 ope_dev_prepare_req, &kmd_buff_offset,
					 &cdm_kmd_start_addr);
	if (!kmd_buf) {
		rc = -EINVAL;
		goto end;
...
static int cam_ope_dev_create_kmd_buf_batch(
	struct cam_ope_hw_mgr *hw_mgr,
	struct cam_hw_prepare_update_args *prepare_args,
	struct cam_ope_ctx *ctx_data, uint32_t req_idx,
	struct cam_ope_dev_prepare_req *ope_dev_prepare_req)
{
	...
	for (i = 0; i < frm_proc->batch_size; i++) {
		...
		if (i) {
			kmd_buf = cdm_ops->cdm_write_wait_prefetch_disable(
				kmd_buf, 0x0, OPE_WAIT_COMP_IDLE, 0x0);
		}

		/* Frame i DB */
		kmd_buf = ope_create_frame_cmd_batch(
			hw_mgr, ctx_data, req_idx, kmd_buf,
			OPE_CMD_BUF_DOUBLE_BUFFERED, i, ope_dev_prepare_req);
		if (!kmd_buf) {
			rc = -EINVAL;
			goto end;
		}
		...
		kmd_buf = ope_create_frame_cmd_batch(
			hw_mgr, ctx_data, req_idx, kmd_buf,
			OPE_CMD_BUF_SINGLE_BUFFERED, i, ope_dev_prepare_req);
		if (!kmd_buf) {
			rc = -EINVAL;
			goto end;
		}
...
static int
cam_ope_dev_create_kmd_buf(struct cam_ope_hw_mgr *hw_mgr,
			   struct cam_hw_prepare_update_args *prepare_args,
			   struct cam_ope_ctx *ctx_data, uint32_t req_idx,
			   struct cam_ope_dev_prepare_req *ope_dev_prepare_req)
{
	...
	if (ctx_data->ope_acquire.dev_type == OPE_DEV_TYPE_OPE_NRT) {
		return cam_ope_dev_create_kmd_buf_nrt(
			...
			ope_dev_prepare_req->prepare_args,
			ope_dev_prepare_req->ctx_data,
			ope_dev_prepare_req->req_idx, ope_dev_prepare_req);
	}
	...
	CAM_DBG(CAM_OPE, "kmd_buf:%x req_idx:%d req_id:%lld offset:%d", kmd_buf,
...
static int cam_ope_dev_process_probe(struct ope_hw *ope_hw, void *cmd_args)
{
...
static int cam_ope_process_probe(struct ope_hw *ope_hw, void *cmd_args,
				 bool hfi_en)
{
...
static int cam_ope_process_dump_debug_reg(struct ope_hw *ope_hw, bool hfi_en)
{
	if (!hfi_en)
...
static int cam_ope_process_reset(struct ope_hw *ope_hw, void *cmd_args,
				 bool hfi_en)
{
	if (!hfi_en)
...
static int cam_ope_process_release(struct ope_hw *ope_hw, void *cmd_args,
				   bool hfi_en)
{
	if (!hfi_en)
...
static int cam_ope_process_acquire(struct ope_hw *ope_hw, void *cmd_args,
				   bool hfi_en)
{
	if (!hfi_en)
...
static int cam_ope_process_prepare(struct ope_hw *ope_hw, void *cmd_args,
				   bool hfi_en)
{
	if (!hfi_en)
...
int cam_ope_process_cmd(void *device_priv, uint32_t cmd_type, void *cmd_args,
			uint32_t arg_size)
{
	int rc = 0;
	struct cam_hw_info *ope_dev = device_priv;
	...
	if (!device_priv) {
		CAM_ERR(CAM_OPE, "Invalid args %x for cmd %u", device_priv,
			cmd_type);
		return -EINVAL;
	}
