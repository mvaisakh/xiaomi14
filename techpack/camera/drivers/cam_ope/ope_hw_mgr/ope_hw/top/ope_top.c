...
static int cam_ope_top_dump_debug_reg(struct ope_hw *ope_hw_info)
{
	...
	for (i = 0; i < top_reg->num_debug_registers; i = i + 3) {
		...
		val[1] =
			((i + 1) < top_reg->num_debug_registers) ?
				cam_io_r_mb(top_reg->base +
					    top_reg->debug_regs[i + 1].offset) :
				0;
		val[2] =
			((i + 2) < top_reg->num_debug_registers) ?
				cam_io_r_mb(top_reg->base +
					    top_reg->debug_regs[i + 2].offset) :
				0;

		CAM_INFO(CAM_OPE, "status[%d-%d] : 0x%x 0x%x 0x%x", i, i + 2,
			 val[0], val[1], val[2]);
	}
	...
	for (i = 0; i < pp_reg->num_clients; i++) {
		pp_hw_status = 0;
		pp_hw_status = cam_io_r_mb(pp_reg->base +
					   pp_reg->pp_clients[i].hw_status);

		if (pp_hw_status)
			CAM_ERR(CAM_OPE,
				"ope pp hw_status offset 0x%x val 0x%x",
				pp_reg->pp_clients[i].hw_status, pp_hw_status);
	}
	return 0;
}

static int cam_ope_top_reset(struct ope_hw *ope_hw_info, int32_t ctx_id,
			     void *data)
{
	int rc = 0;
	struct cam_ope_top_reg *top_reg;
	...
	rc = cam_common_wait_for_completion_timeout(
		&ope_top_info.reset_complete, msecs_to_jiffies(60));
...
static int cam_ope_top_release(struct ope_hw *ope_hw_info, int32_t ctx_id,
			       void *data)
{
	int rc = 0;
...
static int cam_ope_top_acquire(struct ope_hw *ope_hw_info, int32_t ctx_id,
			       void *data)
{
	int rc = 0;
...
static int cam_ope_top_init(struct ope_hw *ope_hw_info, int32_t ctx_id,
			    void *data)
{
	int rc = 0;
	struct cam_ope_top_reg *top_reg;
...
static int cam_ope_top_probe(struct ope_hw *ope_hw_info, int32_t ctx_id,
			     void *data)
{
	int rc = 0;
...
static int cam_ope_top_isr(struct ope_hw *ope_hw_info, int32_t ctx_id,
			   void *data)
{
	int rc = 0;
	uint32_t irq_status;
	...
	if (irq_status & top_reg_val->ope_violation) {
		violation_status =
			cam_io_r_mb(top_reg->base + top_reg->violation_status);
		irq_data->error = 1;
		CAM_ERR(CAM_OPE, "ope violation: %x", violation_status);

		for (i = 0; i < pp_reg->num_clients; i++) {
			pp_hw_status = 0;
			pp_hw_status = cam_io_r_mb(
				pp_reg->base + pp_reg->pp_clients[i].hw_status);
...
int cam_ope_top_process(struct ope_hw *ope_hw_info, int32_t ctx_id,
