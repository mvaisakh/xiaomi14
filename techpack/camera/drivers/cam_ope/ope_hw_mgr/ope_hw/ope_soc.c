...
static int cam_ope_request_platform_resource(struct cam_hw_soc_info *soc_info,
					     irq_handler_t ope_irq_handler,
					     void *data)
{
	int rc = 0, i;
				  ...
				  bool disable_clk)
{
	...
	rc = cam_soc_util_disable_platform_resource(
		soc_info, CAM_CLK_SW_CLIENT_IDX, disable_clk, true);
	if (rc)
		CAM_ERR(CAM_OPE, "enable platform failed");
...
int cam_ope_update_clk_rate(struct cam_hw_soc_info *soc_info, uint32_t clk_rate)
{
	int32_t src_clk_idx;
	...
	CAM_DBG(CAM_OPE, "clk_rate = %u src_clk_index = %d", clk_rate,
		src_clk_idx);
