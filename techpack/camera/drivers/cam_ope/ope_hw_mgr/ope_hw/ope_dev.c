				   ...
				   struct cam_ope_device_core_info *core_info)
{
	...
	CAM_DBG(CAM_OPE, "soc_info = %x core_info = %x", soc_info, core_info);
	...
	CAM_DBG(CAM_OPE, "core: %x", core_info->ope_hw_info->ope_cdm_base);
	...
	core_info->hw_version =
		cam_io_r_mb(core_info->ope_hw_info->ope_top_base +
...
static int cam_ope_component_bind(struct device *dev, struct device *master_dev,
				  void *data)
{
	...
	CAM_DBG(CAM_OPE, "type %d index %d", ope_dev_intf->hw_type,
		ope_dev_intf->hw_idx);

	if (ope_dev_intf->hw_idx < OPE_DEV_MAX)
		cam_ope_dev_list[ope_dev_intf->hw_idx].hw_intf = ope_dev_intf;

	platform_set_drvdata(pdev, ope_dev_intf);

	ope_dev->core_info =
		kzalloc(sizeof(struct cam_ope_device_core_info), GFP_KERNEL);
	...
	rc = cam_ope_register_cpas(&ope_dev->soc_info, core_info,
				   ope_dev_intf->hw_idx);
	if (rc < 0)
	...
	cam_ope_process_cmd(ope_dev, OPE_HW_PROBE, &ope_probe,
			    sizeof(ope_probe));
