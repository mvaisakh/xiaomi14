...
#define update_cre_reg_set(cre_reg_buf, off, val)                             \
	do {                                                                  \
		cre_reg_buf->wr_reg_set[cre_reg_buf->num_wr_reg_set].offset = \
			(off);                                                \
		cre_reg_buf->wr_reg_set[cre_reg_buf->num_wr_reg_set].value =  \
			(val);                                                \
		cre_reg_buf->num_wr_reg_set++;                                \
	} while (0)

static int cam_cre_translate_write_format(
				 ...
				 struct cre_reg_buffer *cre_reg_buf)
{
	...
	out_port_idx = cam_cre_bus_wr_out_port_idx(io_buf->resource_type);
	if (out_port_idx < 0) {
	...
	for (k = 0; k < io_buf->num_planes; k++) {
		...
		val = 0;
		val |= ((wr_client_reg_val->mode & wr_client_reg_val->mode_mask)
     ...
     */
		...
		update_cre_reg_set(
			cre_reg_buf,
			wr_reg->offset + wr_reg_client->debug_status_cfg, val);
	}
				  ...
				  int32_t ctx_id, void *data)
{
	...
	CAM_DBG(CAM_CRE, "req_idx = %d req_id = %lld num_io_bufs = %d", req_idx,
		cre_request->request_id, cre_request->num_io_bufs[0]);
	...
	for (i = 0; i < cre_request->num_batch; i++) {
		...
		for (j = 0; j < cre_request->num_io_bufs[i]; j++) {
			...
			CAM_DBG(CAM_CRE, "batch = %d io buf num = %d", i, j);
			if (io_buf->direction != CAM_BUF_OUTPUT)
				continue;

			rc = cam_cre_bus_wr_update(cam_cre_hw_info, ctx_id,
						   prepare, i, j, cre_reg_buf);
			if (rc)
				goto end;
		}
	}
				...
				int32_t ctx_id, void *data)
{
	...
	for (i = 0; i < bus_wr_reg_val->num_clients; i++) {
		output_port_idx = bus_wr_reg_val->wr_clients[i].wm_port_id;
	...
	for (i = 0; i < CRE_MAX_OUT_RES; i++) {
		...
		CAM_DBG(CAM_CRE, "output port id = %d",
			out_port_to_wm->output_port_id);
		CAM_DBG(CAM_CRE, "num_wms = %d", out_port_to_wm->num_wm);
			      ...
			      int32_t ctx_id, void *data)
{
	...
	if ((irq_status_0 & bus_wr_reg_val->violation) ||
		...
		CAM_ERR(CAM_CRE,
			...
			violation_status, img_violation_status, debug_status_0,
			debug_status_1);
	}
	...
	if (irq_status_0 & bus_wr_reg_val->comp_buf_done) {
		for (i = 0; i < bus_wr_reg_val->num_clients; i++) {
			if (irq_status_1 &
			    bus_wr_reg_val->wr_clients[i].client_buf_done)
...
int cam_cre_bus_wr_process(struct cam_cre_hw *cam_cre_hw_info, int32_t ctx_id,
