...
int cam_tfe_get_hw_caps(void *hw_priv, void *get_hw_cap_args, uint32_t arg_size)
{
	return -EPERM;
}
...
int cam_tfe_irq_config(void *tfe_core_data, uint32_t *irq_mask,
		       uint32_t num_reg, bool enable)
{
	...
	for (i = 0; i < num_reg; i++) {
		...
		if (enable)
			...
			val &= ~irq_mask[i];
		cam_io_w_mb(val,
			    mem_base + core_info->tfe_hw_info->top_irq_mask[i]);
	}
	if (need_lock)
...
static void cam_tfe_log_tfe_in_debug_status(struct cam_tfe_top_priv *top_priv)
{
	void __iomem *mem_base;
	...
	for (i = 0; i < CAM_TFE_TOP_IN_PORT_MAX; i++) {
		...
		if (top_priv->in_rsrc[i].res_id == CAM_ISP_HW_TFE_IN_CAMIF) {
			camif_data = (struct cam_tfe_camif_data *)top_priv
					     ->in_rsrc[i]
					     .res_priv;
			...
			CAM_INFO(CAM_ISP,
				 ...
				 camif_data->first_line, camif_data->last_line);
			CAM_INFO(CAM_ISP, "vbi_value:0x%x hbi_value:0x%x",
				 camif_data->vbi_value, camif_data->hbi_value);
			...
			rdi_data =
				(struct cam_tfe_rdi_data *)top_priv->in_rsrc[i]
					.res_priv;
			val_0 = cam_io_r(mem_base +
			...
			CAM_INFO(
				...
				top_priv->in_rsrc[i].res_id, val_1,
				((val_0 >> 16) & 0x1FFF), (val_0 & 0x1FFF));
			CAM_INFO(
				CAM_ISP,
				"sync mode:%d left start pxl:0x%x end_pixel:0x%x",
				rdi_data->sync_mode, rdi_data->left_first_pixel,
				rdi_data->left_last_pixel);
			CAM_INFO(CAM_ISP,
				 "sync mode:%d line start:0x%x line end:0x%x",
				 rdi_data->sync_mode, rdi_data->first_line,
				 rdi_data->last_line);
		}
	}
}
static void
cam_tfe_log_error_irq_status(struct cam_tfe_hw_core_info *core_info,
			     struct cam_tfe_top_priv *top_priv,
			     struct cam_tfe_irq_evt_payload *evt_payload)
{
	...
	CAM_INFO(
		CAM_ISP,
		"ERROR timestamp:[%lld.%06lld] SOF timestamp:[%lld.%06lld] EPOCH "
		"timestamp:[%lld.%06lld] EOF timestamp:[%lld.%06lld]",
		top_priv->error_ts.tv_sec, top_priv->error_ts.tv_nsec / 1000,
		top_priv->sof_ts.tv_sec, top_priv->sof_ts.tv_nsec / 1000,
		top_priv->epoch_ts.tv_sec, top_priv->epoch_ts.tv_nsec / 1000,
		top_priv->eof_ts.tv_sec, top_priv->eof_ts.tv_nsec / 1000);
	...
	for (i = 0; i < top_priv->common_data.common_reg->num_perf_cfg; i++) {
		val_0 = cam_io_r(mem_base +
				 top_priv->common_data.common_reg->perf_cfg[i]
					 .perf_pixel_count);
	...
	for (i = 0; i < hw_info->num_clc; i++) {
		val_0 = cam_io_r(mem_base + clc_hw_status[i].hw_status_reg);
		if (val_0)
			CAM_INFO(
				CAM_ISP,
				"CLC HW status :name:%s offset:0x%x value:0x%x",
				clc_hw_status[i].name,
				clc_hw_status[i].hw_status_reg, val_0);
	}
	...
	core_info->tfe_bus->bottom_half_handler(core_info->tfe_bus->bus_priv,
						false, evt_payload, true);

	CAM_INFO(CAM_ISP, "TFE clock rate:%d TFE total bw applied:%lld",
		 top_priv->hw_clk_rate, top_priv->total_bw_applied);
}

static int
cam_tfe_error_irq_bottom_half(struct cam_tfe_hw_core_info *core_info,
			      struct cam_tfe_top_priv *top_priv,
			      struct cam_tfe_irq_evt_payload *evt_payload,
			      cam_hw_mgr_event_cb_func event_cb,
			      void *event_cb_priv)
{
	...
	if (error_detected) {
		...
		top_priv->error_ts.tv_sec = evt_payload->ts.mono_time.tv_sec;
		top_priv->error_ts.tv_nsec = evt_payload->ts.mono_time.tv_nsec;
...
static int
cam_tfe_rdi_irq_bottom_half(struct cam_tfe_top_priv *top_priv,
			    ...
			    struct cam_tfe_irq_evt_payload *evt_payload)
{
	...
	if ((!epoch_process) &&
	    (evt_payload->irq_reg_val[1] & rdi_priv->reg_data->eof_irq_mask)) {
		CAM_DBG(CAM_ISP, "Received EOF");
		top_priv->eof_ts.tv_sec = evt_payload->ts.mono_time.tv_sec;
		top_priv->eof_ts.tv_nsec = evt_payload->ts.mono_time.tv_nsec;

		if (rdi_priv->event_cb)
			rdi_priv->event_cb(rdi_priv->priv, CAM_ISP_HW_EVENT_EOF,
					   (void *)&evt_info);
	}

	if ((!epoch_process) &&
	    (evt_payload->irq_reg_val[1] & rdi_priv->reg_data->sof_irq_mask)) {
		CAM_DBG(CAM_ISP, "Received SOF");
		top_priv->sof_ts.tv_sec = evt_payload->ts.mono_time.tv_sec;
		top_priv->sof_ts.tv_nsec = evt_payload->ts.mono_time.tv_nsec;
		...
		if (top_priv->top_debug &
			...
			CAM_INFO(
				...
				(val & common_reg->diag_sensor_hbi_mask), val2);
		}
	}
	...
	if (epoch_process && (evt_payload->irq_reg_val[1] &
		...
		top_priv->epoch_ts.tv_sec = evt_payload->ts.mono_time.tv_sec;
		top_priv->epoch_ts.tv_nsec = evt_payload->ts.mono_time.tv_nsec;
...
static int
cam_tfe_camif_irq_bottom_half(struct cam_tfe_top_priv *top_priv,
...
static int cam_tfe_irq_bottom_half(void *handler_priv, void *evt_payload_priv)
{
	struct cam_tfe_hw_core_info *core_info;
	struct cam_tfe_top_priv *top_priv;
	struct cam_tfe_irq_evt_payload *evt_payload;
	...
	for (i = 0; i < CAM_TFE_TOP_IN_PORT_MAX; i++) {
		if ((top_priv->in_rsrc[i].res_id == CAM_ISP_HW_TFE_IN_CAMIF) &&
		    (top_priv->in_rsrc[i].res_state ==
		     CAM_ISP_RESOURCE_STATE_STREAMING)) {
			camif_priv = (struct cam_tfe_camif_data *)top_priv
					     ->in_rsrc[i]
					     .res_priv;
			...
			if (camif_priv->reg_data->subscribe_irq_mask[1] &
				...
				cam_tfe_camif_irq_bottom_half(
					top_priv, &top_priv->in_rsrc[i], false,
					evt_payload);
	...
	for (i = 0; i < CAM_TFE_TOP_IN_PORT_MAX; i++) {
		if ((top_priv->in_rsrc[i].res_id == CAM_ISP_HW_TFE_IN_CAMIF) &&
			...
			if (camif_priv->reg_data->subscribe_irq_mask[1] &
			    evt_payload->irq_reg_val[1])
				cam_tfe_camif_irq_bottom_half(
					top_priv, &top_priv->in_rsrc[i], true,
					evt_payload);
		} else if ((top_priv->in_rsrc[i].res_id >=
			    CAM_ISP_HW_TFE_IN_RDI0) &&
			   (top_priv->in_rsrc[i].res_id <=
...
static int cam_tfe_irq_err_top_half(struct cam_tfe_hw_core_info *core_info,
				    void __iomem *mem_base,
				    ...
				    uint32_t *bus_irq_status)
{
	...
	if ((top_irq_status[0] & core_info->tfe_hw_info->error_irq_mask[0]) ||
	    (top_irq_status[2] & core_info->tfe_hw_info->error_irq_mask[2]) ||
	    (bus_irq_status[0] &
		...
		for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++)
			cam_io_w(0, mem_base + core_info->tfe_hw_info
						       ->top_irq_mask[i]);
...
irqreturn_t cam_tfe_irq(int irq_num, void *data)
{
	...
	spin_lock(&core_info->spin_lock);
	for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++)
		top_irq_status[i] = cam_io_r(
			mem_base + core_info->tfe_hw_info->top_irq_status[i]);

	for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++)
		cam_io_w(top_irq_status[i],
			 mem_base + core_info->tfe_hw_info->top_irq_clear[i]);

	cam_io_w_mb(core_info->tfe_hw_info->global_clear_bitmask,
		    mem_base + core_info->tfe_hw_info->top_irq_cmd);

	CAM_DBG(CAM_ISP, "TFE:%d IRQ status_0:0x%x status_1:0x%x status_2:0x%x",
		core_info->core_index, top_irq_status[0], top_irq_status[1],
		top_irq_status[2]);

	if (top_irq_status[0] & core_info->tfe_hw_info->bus_reg_irq_mask[0]) {
		for (i = 0; i < CAM_TFE_BUS_MAX_IRQ_REGISTERS; i++)
			bus_irq_status[i] = cam_io_r(
				mem_base +
				core_info->tfe_hw_info->bus_irq_status[i]);

		for (i = 0; i < CAM_TFE_BUS_MAX_IRQ_REGISTERS; i++)
			cam_io_w(bus_irq_status[i],
				 mem_base + core_info->tfe_hw_info
						    ->bus_irq_clear[i]);

		ccif_violation = cam_io_r(
			mem_base + core_info->tfe_hw_info->bus_violation_reg);
		overflow_status = cam_io_r(
			mem_base + core_info->tfe_hw_info->bus_overflow_reg);
		image_sz_violation = cam_io_r(
			mem_base +
	...
	if ((top_irq_status[0] & core_info->tfe_hw_info->reset_irq_mask[0]) ||
	    (top_irq_status[1] & core_info->tfe_hw_info->reset_irq_mask[1]) ||
	    (top_irq_status[2] & core_info->tfe_hw_info->reset_irq_mask[2])) {
		/* Reset ack */
	...
	rc = tasklet_bh_api.get_bh_payload_func(top_priv->tasklet_info,
						&bh_cmd);
	if (rc || !bh_cmd) {
		CAM_ERR_RATE_LIMIT(CAM_ISP, "No payload, IRQ handling frozen");
		cam_tfe_put_evt_payload(core_info, &evt_payload);
		goto end;
	}

	tasklet_bh_api.bottom_half_enqueue_func(top_priv->tasklet_info, bh_cmd,
						core_info, evt_payload,
...
static int cam_tfe_top_set_hw_clk_rate(struct cam_tfe_top_priv *top_priv)
{
	...
	if (!rc)
		...
		CAM_ERR(CAM_ISP, "TFE:%d set src clock rate:%lld failed, rc=%d",
			top_priv->common_data.soc_info->index, max_clk_rate,
			rc);
...
cam_tfe_top_delay_bw_reduction(struct cam_tfe_top_priv *top_priv,
...
static int cam_tfe_top_set_axi_bw_vote(struct cam_tfe_top_priv *top_priv,
				       bool start_stop)
{
	...
	CAM_DBG(CAM_PERF,
		"tfe[%d] : applied_total=%lld, new_total=%lld unchanged=%d, "
		"start_stop=%d",
		top_priv->common_data.hw_intf->hw_idx,
     ...
     */
		to_be_applied_axi_vote = cam_tfe_top_delay_bw_reduction(
	...
	CAM_DBG(CAM_PERF,
		"tfe[%d] : Delayed update: applied_total=%lld, new_total=%lld "
		"apply_bw_update=%d, start_stop=%d",
		top_priv->common_data.hw_intf->hw_idx,
...
static int cam_tfe_top_get_reg_update(struct cam_tfe_top_priv *top_priv,
				      void *cmd_args, uint32_t arg_size)
{
	uint32_t size = 0;
...
static int cam_tfe_top_clock_update(struct cam_tfe_top_priv *top_priv,
				    void *cmd_args, uint32_t arg_size)
{
	...
	clk_update = (struct cam_tfe_clock_update_args *)cmd_args;
	...
	if (hw_info->hw_state != CAM_HW_STATE_POWER_UP) {
		CAM_DBG(CAM_ISP, "TFE:%d Not ready to set clocks yet :%d",
...
static int cam_tfe_top_bw_update(struct cam_tfe_top_priv *top_priv,
				 void *cmd_args, uint32_t arg_size)
{
...
static int cam_tfe_top_bw_control(struct cam_tfe_top_priv *top_priv,
				  void *cmd_args, uint32_t arg_size)
{
...
static int cam_tfe_top_get_reg_dump(struct cam_tfe_top_priv *top_priv,
				    void *cmd_args, uint32_t arg_size)
{
	...
	if ((reg_dump_cmd->res->res_state == CAM_ISP_RESOURCE_STATE_RESERVED) ||
	    (reg_dump_cmd->res->res_state == CAM_ISP_RESOURCE_STATE_AVAILABLE))
		return 0;
	...
	for (i = 0; i < num_reg_dump_entries; i++) {
		...
		for (j = 0; j < lut_size; j++) {
			...
			CAM_INFO(CAM_ISP, "Bank%d:0x%x LO: 0x%x", lut_bank_sel,
				 j, val_0);
		}
	}
	/* No mem selected */
	...
	CAM_INFO(CAM_ISP, "bus start addr:0x%x end_offset:0x%x", start_offset,
		 end_offset);
	...
	for (i = start_offset; (i + 0xc) <= end_offset; i += 0x10) {
		...
		CAM_INFO(CAM_ISP, "0x%04x=0x%08x 0x%08x 0x%08x 0x%08x", i,
			 val_0, val_1, val_2, val_3);
	}

	wm_offset = reg_dump_data->bus_client_start_addr;

	CAM_INFO(CAM_ISP, "bus wm offset:0x%x", wm_offset);
	...
	CAM_INFO(CAM_ISP, "TFE:%d clk=%ld",
		 top_priv->common_data.hw_intf->hw_idx, top_priv->hw_clk_rate);

	return 0;
}

static int cam_tfe_hw_dump(struct cam_tfe_hw_core_info *core_info,
			   void *cmd_args, uint32_t arg_size)
{
	int i, j;
	uint8_t *dst;
		...
		for (i = 0; i < reg_dump_data->num_lut_dump_entries; i++)
			lut_dump_size +=
				((reg_dump_data->lut_entry[i].lut_addr_size) *
				 (reg_dump_data->lut_entry[i].lut_word_size /
				  8));
	...
	*addr++ = soc_info->index;
	...
	for (i = 0; i < reg_dump_data->num_lut_dump_entries; i++) {
		...
		*addr++ = lut_bank_sel;
		...
		hdr->size = hdr->word_size * (addr - start);
		dump_args->offset +=
			hdr->size + sizeof(struct cam_isp_hw_dump_header);
	}
	cam_io_w_mb(0, mem_base + 0xC24);
	cam_io_w_mb(0, mem_base + 0xC28);
...
dump_bw:
	...
	clk_waddr = (uint64_t *)(dst + sizeof(struct cam_isp_hw_dump_header));
...
static int cam_tfe_camif_irq_reg_dump(struct cam_tfe_hw_core_info *core_info,
				      void *cmd_args, uint32_t arg_size)
{
	struct cam_tfe_top_priv *top_priv;
	...
	for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++) {
		CAM_INFO(
			CAM_ISP, "Core Id =%d TOP IRQ status[%d ] val 0x%x",
	...
	for (i = 0; i < CAM_TFE_BUS_MAX_IRQ_REGISTERS; i++) {
		CAM_INFO(
			CAM_ISP, "Core Id =%d BUS IRQ status[%d ] val:0x%x",
			core_info->core_index, i,
			cam_io_r_mb(mem_base +
				    core_info->tfe_hw_info->bus_irq_status[i]));
	}
			  ...
			  void *cmd_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "TFE:%d top debug set:%d", core_info->core_index,
		top_priv->top_debug);

	return 0;
}

int cam_tfe_top_reserve(void *device_priv, void *reserve_args,
			uint32_t arg_size)
{
	struct cam_tfe_top_priv *top_priv;
	...
	for (i = 0; i < CAM_TFE_TOP_IN_PORT_MAX; i++) {
		...
		if ((top_priv->in_rsrc[i].res_id == acquire_args->res_id) &&
			...
			if (acquire_args->res_id == CAM_ISP_HW_TFE_IN_CAMIF) {
				camif_data =
					(struct cam_tfe_camif_data *)top_priv
						->in_rsrc[i]
						.res_priv;
				...
				if (acquire_args->in_port->res_id ==
				    CAM_ISP_TFE_IN_RES_TPG)
					camif_data->vbi_value = 0;
				else
					camif_data->vbi_value =
						acquire_args->in_port
							->sensor_vbi;
				...
				rdi_data = (struct cam_tfe_rdi_data *)top_priv
						   ->in_rsrc[i]
						   .res_priv;
...
int cam_tfe_top_release(void *device_priv, void *release_args,
			uint32_t arg_size)
{
	struct cam_tfe_top_priv *top_priv;
	struct cam_isp_resource_node *in_res;
...
static int cam_tfe_camif_resource_start(struct cam_tfe_hw_core_info *core_info,
					struct cam_isp_resource_node *camif_res)
{
	...
	cam_io_w_mb(computed_epoch_line_cfg,
		    rsrc_data->mem_base + rsrc_data->camif_reg->epoch_irq_cfg);
	CAM_DBG(CAM_ISP,
		"TFE:%d first_line: %u\n"
		"last_line: %u\n"
		"epoch_line_cfg: 0x%x",
		core_info->core_index, rsrc_data->first_line,
		rsrc_data->last_line, computed_epoch_line_cfg);
	...
	if (rsrc_data->sync_mode != CAM_ISP_HW_SYNC_SLAVE)
		cam_tfe_irq_config(core_info,
				   rsrc_data->reg_data->subscribe_irq_mask,
	...
	CAM_DBG(CAM_ISP, "TFE:%d perf_cfg val:%d", core_info->core_index, val);
...
int cam_tfe_top_start(struct cam_tfe_hw_core_info *core_info, void *start_args,
		      uint32_t arg_size)
{
	struct cam_tfe_top_priv *top_priv;
	struct cam_isp_resource_node *in_res;
...
int cam_tfe_top_stop(struct cam_tfe_hw_core_info *core_info, void *stop_args,
		     uint32_t arg_size)
{
	struct cam_tfe_top_priv *top_priv;
	struct cam_isp_resource_node *in_res;
...
int cam_tfe_top_init(struct cam_hw_soc_info *soc_info,
		     ...
		     struct cam_tfe_hw_core_info *core_info)
{
	...
	top_priv = kzalloc(sizeof(struct cam_tfe_top_priv), GFP_KERNEL);
	if (!top_priv) {
	...
	for (i = 0, j = 0; i < CAM_TFE_TOP_IN_PORT_MAX; i++) {
		...
		memset(&top_priv->req_axi_vote[i], 0,
		       sizeof(struct cam_axi_vote));
		top_priv->axi_vote_control[i] = CAM_TFE_BW_CONTROL_EXCLUDE;

		if (hw_info->in_port[i] == CAM_TFE_CAMIF_VER_1_0) {
			top_priv->in_rsrc[i].res_id = CAM_ISP_HW_TFE_IN_CAMIF;
			...
			rdi_priv->rdi_reg = hw_info->rdi_hw_info[j].rdi_reg;
			rdi_priv->reg_data = hw_info->rdi_hw_info[j++].reg_data;
		} else {
...
int cam_tfe_reset(void *hw_priv, void *reset_core_args, uint32_t arg_size)
{
	...
	for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++)
		irq_status[i] = cam_io_r(
			mem_base + core_info->tfe_hw_info->top_irq_status[i]);

	for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++)
		cam_io_w(irq_status[i],
			 mem_base + core_info->tfe_hw_info->top_irq_clear[i]);
	...
	for (i = 0; i < CAM_TFE_TOP_IRQ_REG_NUM; i++)
		cam_io_w(0, mem_base + core_info->tfe_hw_info->top_irq_mask[i]);
	...
	rc = cam_common_wait_for_completion_timeout(&core_info->reset_complete,
...
int cam_tfe_init_hw(void *hw_priv, void *init_hw_args, uint32_t arg_size)
{
	...
	int i, rc = 0;
	unsigned long max_clk_rate = 0;
	uint32_t reset_core_args = CAM_TFE_HW_RESET_HW_AND_REG;
	...
	rc = core_info->tfe_bus->hw_ops.init(core_info->tfe_bus->bus_priv, NULL,
					     0);
	if (rc) {
...
int cam_tfe_reserve(void *hw_priv, void *reserve_args, uint32_t arg_size)
{
	...
	if (!hw_priv || !reserve_args ||
	    (arg_size != sizeof(struct cam_tfe_acquire_args))) {
		CAM_ERR(CAM_ISP, "Invalid input arguments");
		return -EINVAL;
	}
	core_info = (struct cam_tfe_hw_core_info *)tfe_hw->core_info;
	...
	CAM_DBG(CAM_ISP, "TFE:%d acquire res type: %d", core_info->core_index,
		acquire->rsrc_type);
	mutex_lock(&tfe_hw->hw_mutex);
	if (acquire->rsrc_type == CAM_ISP_RESOURCE_TFE_IN) {
		rc = cam_tfe_top_reserve(core_info->top_priv, reserve_args,
					 arg_size);
...
int cam_tfe_start(void *hw_priv, void *start_args, uint32_t arg_size)
{
	...
	if (start_res->res_type == CAM_ISP_RESOURCE_TFE_IN) {
		rc = cam_tfe_top_start(core_info, start_args, arg_size);
		if (rc)
...
int cam_tfe_stop(void *hw_priv, void *stop_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "TFE:%d stopped res type:%d res id:%d res_state:%d ",
		core_info->core_index, isp_res->res_type, isp_res->res_id,
		isp_res->res_state);
...
int cam_tfe_process_cmd(void *hw_priv, uint32_t cmd_type, void *cmd_args,
			uint32_t arg_size)
{
	struct cam_hw_info *tfe_hw = hw_priv;
	struct cam_hw_soc_info *soc_info = NULL;
	...
	case CAM_ISP_HW_CMD_GET_IRQ_REGISTER_DUMP:
		rc = cam_tfe_camif_irq_reg_dump(core_info, cmd_args, arg_size);
		break;
	case CAM_ISP_HW_CMD_QUERY_REGSPACE_DATA:
		*((struct cam_hw_soc_info **)cmd_args) = soc_info;
		break;
	case CAM_ISP_HW_CMD_DUMP_HW:
		rc = cam_tfe_hw_dump(core_info, cmd_args, arg_size);
		break;
	case CAM_ISP_HW_CMD_SET_CAMIF_DEBUG:
		rc = cam_tfe_set_top_debug(core_info, cmd_args, arg_size);
		break;
		      ...
		      struct cam_tfe_hw_info *tfe_hw_info)
{
	...
	rc = cam_tfe_bus_init(soc_info, hw_intf, tfe_hw_info->bus_hw_info,
			      core_info, &core_info->tfe_bus);
	if (rc) {
