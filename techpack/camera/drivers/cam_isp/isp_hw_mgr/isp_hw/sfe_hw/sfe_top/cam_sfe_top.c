...
struct cam_sfe_top_priv {
	...
	struct cam_sfe_top_perf_counter_cfg
		perf_counters[CAM_SFE_PERF_COUNTER_MAX];
	...
	struct cam_sfe_top_debug_info (
		*clc_dbg_mod_info)[CAM_SFE_TOP_DBG_REG_MAX][8];
...
static int
cam_sfe_top_apply_clock_start_stop(struct cam_sfe_top_priv *top_priv);

static int cam_sfe_top_apply_bw_start_stop(struct cam_sfe_top_priv *top_priv);

static void
cam_sfe_top_print_ipp_violation_info(struct cam_sfe_top_priv *top_priv,
...
static int cam_sfe_top_set_axi_bw_vote(struct cam_sfe_top_priv *top_priv,
				       struct cam_axi_vote *final_bw_vote,
				       uint64_t total_bw_new_vote,
				       bool start_stop, uint64_t request_id)
{
	int rc = 0;
	...
	if (!rc) {
		memcpy(&top_priv->applied_axi_vote, final_bw_vote,
		...
		for (i = 0; i < CAM_DELAY_CLK_BW_REDUCTION_NUM_REQ; i++) {
			for (j = 0; j < top_priv->last_bw_vote[i].num_paths;
			     j++) {
				CAM_INFO(
					CAM_PERF,
				       ...
				       bool is_drv_config_en)
{
	...
	CAM_DBG(CAM_PERF,
		"Applying SFE:%d Clock name=%s idx=%d cesta_client_idx:%d req "
		"clk[high low]=[%lu %lu] req_id=%ld",
		top_priv->common_data.hw_intf->hw_idx,
		soc_info->clk_name[soc_info->src_clk_idx],
		soc_info->src_clk_idx, cesta_client_idx, cesta_clk_rate_high,
		cesta_clk_rate_low, (start_stop ? -1 : request_id));
	...
	if (!rc) {
		...
		CAM_ERR(CAM_ISP,
			"SFE:%d cesta_client_idx:%d Failed to set the req clk rate[high "
			"low]: [%llu %llu] rc:%d",
	...
	struct cam_sfe_top_debug_info (*status_list)[CAM_SFE_TOP_DBG_REG_MAX][8])
{
	...
	for (i = 0; i < num_reg; i++) {
		...
		for (j = 0; j < 8; j++) {
			...
			CAM_INFO_BUF(CAM_SFE, log_buf, 1024, &len,
				     "%s [I:%u V:%u R:%u]",
...
static void cam_sfe_top_print_cc_test_bus(struct cam_sfe_top_priv *top_priv)
{
	struct cam_sfe_top_common_data *common_data = &top_priv->common_data;
	struct cam_hw_soc_info *soc_info = common_data->soc_info;
	void __iomem *mem_base = soc_info->reg_map[SFE_CORE_BASE_IDX].mem_base;
	struct cam_sfe_top_cc_testbus_info *testbus_info;
	uint32_t dbg_testbus_reg =
	...
	for (i = 0; i < size; i++) {
		...
		if (val)
			CAM_INFO_BUF(CAM_SFE, log_buf, 1024, &len,
				     "%s [val:%u]", testbus_info[i].clc_name,
				     val);
	}

	CAM_INFO(CAM_SFE, "SFE_TOP_DEBUG_%d val %d  config %s",
		 common_data->common_reg->top_debug_testbus_reg, reg_val,
		 log_buf);
}

static void cam_sfe_top_dump_perf_counters(const char *event,
					   const char *res_name,
					   struct cam_sfe_top_priv *top_priv)
{
	int i;
	...
	for (i = 0; i < top_priv->common_data.common_reg->num_perf_counters;
		...
		if (top_priv->perf_counters[i].dump_counter) {
			CAM_INFO(
				CAM_SFE,
				"SFE [%u] on %s %s counter: %d pixel_cnt: %d line_cnt: %d stall_cnt: "
				"%d always_cnt: %d status: 0x%x",
				common_data->hw_intf->hw_idx, res_name, event,
				(i + 1),
...
static void cam_sfe_top_print_debug_reg_info(struct cam_sfe_top_priv *top_priv)
{
	void __iomem *mem_base;
	struct cam_sfe_top_common_data *common_data;
	struct cam_hw_soc_info *soc_info;
	...
	cam_sfe_top_check_module_status(top_priv->num_clc_module, reg_val,
					top_priv->clc_dbg_mod_info);
...
cam_sfe_top_delay_bw_reduction(struct cam_sfe_top_priv *top_priv,
				 ...
				 uint64_t request_id)
{
	...
	for (i = 0; i < top_priv->num_in_ports; i++) {
		if (top_priv->axi_vote_control[i] ==
			...
			if (num_paths + top_priv->req_axi_vote[i].num_paths >
			    CAM_CPAS_MAX_PATHS_PER_CLIENT) {
				CAM_ERR(CAM_PERF,
					"Required paths(%d) more than max(%d)",
					num_paths + top_priv->req_axi_vote[i]
							    .num_paths,
					CAM_CPAS_MAX_PATHS_PER_CLIENT);
				rc = -EINVAL;
				goto end;
	...
	for (i = 0; i < top_priv->agg_incoming_vote.num_paths; i++) {
		CAM_DBG(CAM_PERF,
			...
			cam_cpas_axi_util_path_type_to_string(
				top_priv->agg_incoming_vote.axi_path[i]
					.path_data_type),
			cam_cpas_axi_util_trans_type_to_string(
				top_priv->agg_incoming_vote.axi_path[i]
					.transac_type),
	...
	memcpy(&top_priv->last_bw_vote[top_priv->last_bw_counter],
	       &top_priv->agg_incoming_vote, sizeof(struct cam_axi_vote));
	top_priv->last_total_bw_vote[top_priv->last_bw_counter] =
		*total_bw_new_vote;
	...
	CAM_DBG(CAM_PERF,
		...
		"req_id=%ld",
		top_priv->total_bw_applied, *total_bw_new_vote, bw_unchanged,
		start_stop, (start_stop ? -1 : request_id));
	...
	if (start_stop) {
		...
		memset(top_priv->last_bw_vote, 0,
		       sizeof(struct cam_axi_vote) *
			       CAM_DELAY_CLK_BW_REDUCTION_NUM_REQ);
		memset(top_priv->last_total_bw_vote, 0,
		       sizeof(uint64_t) * CAM_DELAY_CLK_BW_REDUCTION_NUM_REQ);
		top_priv->last_bw_counter = 0;
		top_priv->last_bw_vote[top_priv->last_bw_counter] =
			top_priv->agg_incoming_vote;
		top_priv->last_total_bw_vote[top_priv->last_bw_counter] =
			*total_bw_new_vote;
		top_priv->last_bw_counter = (top_priv->last_bw_counter + 1) %
					    CAM_DELAY_CLK_BW_REDUCTION_NUM_REQ;
     ...
     */
		final_bw_vote = cam_sfe_top_delay_bw_reduction(
			top_priv, total_bw_new_vote);
	...
	CAM_DBG(CAM_PERF,
		"sfe[%d] : Delayed update: applied_total=%lld new_total=%lld, "
		"start_stop=%d bw_state=%s req_id=%ld",
		top_priv->common_data.hw_intf->hw_idx,
		top_priv->total_bw_applied, *total_bw_new_vote, start_stop,
		cam_sfe_top_clk_bw_state_to_string(top_priv->bw_state),
			  ...
			  uint32_t arg_size)
{
	...
	for (i = 0; i < top_priv->num_in_ports; i++) {
		if (top_priv->in_rsrc[i].res_id == res->res_id) {
			memcpy(&top_priv->req_axi_vote[i], &bw_update->sfe_vote,
			       sizeof(struct cam_axi_vote));
...
static int cam_sfe_top_core_cfg(struct cam_sfe_top_priv *top_priv,
				void *cmd_args, uint32_t arg_size)
{
	...
	top_priv->core_cfg.mode_sel = sfe_core_cfg->core_config.mode_sel;
	top_priv->core_cfg.fs_mode_cfg = sfe_core_cfg->core_config.fs_mode_cfg;
...
static inline void
cam_sfe_top_delay_clk_reduction(struct cam_sfe_top_priv *top_priv,
...
int cam_sfe_top_calc_hw_clk_rate(struct cam_sfe_top_priv *top_priv,
				 bool start_stop, unsigned long *final_clk_rate,
				 uint64_t request_id)
{
	int i, rc = 0;
	...
	if (start_stop && !top_priv->skip_clk_data_rst) {
		...
		*final_clk_rate = max_req_clk_rate;
		/* Reset everything, we can start afresh */
		memset(top_priv->last_clk_vote, 0,
		       sizeof(uint64_t) * CAM_DELAY_CLK_BW_REDUCTION_NUM_REQ);
...
static int cam_sfe_top_get_base(struct cam_sfe_top_priv *top_priv,
				void *cmd_args, uint32_t arg_size)
{
	uint32_t size = 0;
	uint32_t mem_base = 0;
...
static int cam_sfe_top_clock_update(struct cam_sfe_top_priv *top_priv,
				    void *cmd_args, uint32_t arg_size)
{
	...
	clk_update = (struct cam_sfe_clock_update_args *)cmd_args;
	...
	if (res->res_type != CAM_ISP_RESOURCE_SFE_IN ||
	    res->res_id >= CAM_ISP_HW_SFE_IN_MAX) {
		CAM_ERR(CAM_PERF, "SFE: %d Invalid res_type: %d res_id: %d",
			res->hw_intf->hw_idx, res->res_type, res->res_id);
		return -EINVAL;
	}
...
static int cam_sfe_set_top_debug(struct cam_sfe_top_priv *top_priv,
				 void *cmd_args)
{
	int i;
	...
	if (!debug_cfg->cache_config) {
		...
		top_priv->sensor_sel_diag_cfg =
			debug_cfg->u.dbg_cfg.sfe_sensor_sel;

		if (debug_cfg->u.dbg_cfg.num_counters <= max_counters)
			for (i = 0; i < max_counters; i++)
				top_priv->perf_counters[i].perf_counter_val =
					debug_cfg->u.dbg_cfg
						.sfe_perf_counter_val[i];
	}

	return 0;
}

static int cam_sfe_top_handle_overflow(struct cam_sfe_top_priv *top_priv,
				       void *cmd_args)
{
	struct cam_sfe_top_common_data *common_data;
	struct cam_hw_soc_info *soc_info;
	...
	bus_overflow_status =
		cam_io_r(soc_info->reg_map[SFE_CORE_BASE_IDX].mem_base +
			 top_priv->common_data.common_reg->bus_overflow_status);
	violation_status = cam_io_r(
		soc_info->reg_map[SFE_CORE_BASE_IDX].mem_base +
		top_priv->common_data.common_reg->ipp_violation_status);

	CAM_ERR(CAM_ISP,
		"SFE%d src_clk_rate sw_client:%lu hw_client:[%lu %lu] "
		"bus_overflow_status:%s violation: %s",
	...
	if (violation_status)
		cam_sfe_top_print_ipp_violation_info(top_priv,
						     violation_status);
					   ...
					   void *cmd_args, uint32_t arg_size)
{
	...
	if (hw_info->hw_state != CAM_HW_STATE_POWER_UP) {
		CAM_DBG(CAM_PERF, "SFE:%d Not ready to set clocks yet :%d",
	...
	rc = cam_sfe_top_calc_hw_clk_rate(top_priv, false, &final_clk_rate,
					  request_id);
	if (rc) {
		CAM_ERR(CAM_SFE,
	...
	if ((!to_be_applied_axi_vote) &&
	    (top_priv->bw_state != CAM_CLK_BW_STATE_UNCHANGED)) {
		CAM_ERR(CAM_PERF, "SFE:%d Invalid BW vote for state:%s",
			hw_intf->hw_idx,
			cam_sfe_top_clk_bw_state_to_string(top_priv->bw_state));
		rc = -EINVAL;
		goto end;
	...
	CAM_DBG(CAM_PERF,
		"SFE:%d APPLY CLK/BW req_id:%ld clk_state:%s bw_state:%s "
		"is_drv_config_en:%s",
	...
	if ((top_priv->clk_state == CAM_CLK_BW_STATE_UNCHANGED) &&
		...
		rc = cam_sfe_top_set_axi_bw_vote(top_priv,
						 to_be_applied_axi_vote,
						 total_bw_new_vote, false,
						 request_id);
		if (rc) {
			CAM_ERR(CAM_SFE,
		...
		rc = cam_sfe_top_set_hw_clk_rate(top_priv, final_clk_rate,
						 false, request_id,
						 clk_bw_args->is_drv_config_en);
		if (rc) {
			CAM_ERR(CAM_SFE,
				"SFE:%d Failed in voting final clk:%lu clk_state:%s bw_state:%s "
				"is_drv_config_en:%s",
		...
		CAM_ERR(CAM_SFE,
			"Invalid state to apply CLK/BW clk_state:%s bw_state:%s "
			"is_drv_config_en:%s",
			cam_sfe_top_clk_bw_state_to_string(top_priv->clk_state),
			cam_sfe_top_clk_bw_state_to_string(top_priv->bw_state),
			CAM_BOOL_TO_YESNO(clk_bw_args->is_drv_config_en));
...
static int cam_sfe_top_apply_bw_start_stop(struct cam_sfe_top_priv *top_priv)
{
	...
	rc = cam_sfe_top_calc_axi_bw_vote(
		top_priv, true, &to_be_applied_axi_vote, &total_bw_new_vote, 0);
	if (rc) {
		CAM_ERR(CAM_SFE, "SFE:%d Failed in calculating bw vote rc=%d",
			top_priv->common_data.hw_intf->hw_idx, rc);
		goto end;
...
int cam_sfe_top_process_cmd(void *priv, uint32_t cmd_type, void *cmd_args,
			    uint32_t arg_size)
{
	int rc = 0;
	struct cam_sfe_top_priv *top_priv;
	...
	case CAM_ISP_HW_CMD_GET_CHANGE_BASE:
		rc = cam_sfe_top_get_base(top_priv, cmd_args, arg_size);
		break;
	case CAM_ISP_HW_CMD_CLOCK_UPDATE:
		rc = cam_sfe_top_clock_update(top_priv, cmd_args, arg_size);
		break;
	case CAM_ISP_HW_CMD_BW_UPDATE_V2:
		rc = cam_sfe_top_bw_update(soc_private, top_priv, cmd_args,
					   arg_size);
		break;
	case CAM_ISP_HW_CMD_BW_CONTROL:
		rc = cam_sfe_top_bw_control(soc_private, top_priv, cmd_args,
					    arg_size);
		break;
	case CAM_ISP_HW_CMD_CORE_CONFIG:
		rc = cam_sfe_top_core_cfg(top_priv, cmd_args, arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_APPLY_CLK_BW_UPDATE:
		rc = cam_sfe_top_apply_clk_bw_update(top_priv, cmd_args,
						     arg_size);
		break;
...
int cam_sfe_top_reserve(void *device_priv, void *reserve_args,
			uint32_t arg_size)
{
	struct cam_sfe_top_priv *top_priv;
	...
	if (!device_priv || !reserve_args) {
		CAM_ERR(CAM_SFE, "Error invalid input arguments");
		return rc;
	}
	...
	for (i = 0; i < CAM_SFE_TOP_IN_PORT_MAX; i++) {
		...
		if ((top_priv->in_rsrc[i].res_id == acquire_args->res_id) &&
			...
			CAM_DBG(CAM_SFE, "SFE [%u] for rsrc: %u acquired",
			...
			acquire_args->rsrc_node = &top_priv->in_rsrc[i];
			rc = 0;
			break;
		}
	}
...
int cam_sfe_top_release(void *device_priv, void *release_args,
			uint32_t arg_size)
{
	struct cam_sfe_top_priv *top_priv;
	...
	CAM_DBG(CAM_SFE, "Release for SFE [%u] resource id: %u in state: %d",
		in_res->hw_intf->hw_idx, in_res->res_id, in_res->res_state);
...
static int
cam_sfe_top_get_evt_payload(struct cam_sfe_top_priv *top_priv,
			    struct cam_sfe_top_irq_evt_payload **evt_payload)
{
	...
	*evt_payload =
		list_first_entry(&top_priv->common_data.free_payload_list,
...
static int
cam_sfe_top_put_evt_payload(struct cam_sfe_top_priv *top_priv,
			    struct cam_sfe_top_irq_evt_payload **evt_payload)
{
	unsigned long flags;
...
static int
cam_sfe_top_handle_err_irq_top_half(uint32_t evt_id,
				    struct cam_irq_th_payload *th_payload)
{
	...
	base = top_priv->common_data.soc_info->reg_map[SFE_CORE_BASE_IDX]
		       .mem_base;
	if (irq_status &
	...
	evt_payload->violation_status = cam_io_r(
		base + top_priv->common_data.common_reg->ipp_violation_status);
...
static int
cam_sfe_top_handle_irq_top_half(uint32_t evt_id,
				struct cam_irq_th_payload *th_payload)
{
	int rc = 0, i;
	uint32_t irq_status = 0;
	...
	irq_status = th_payload->evt_status_arr[0];
	CAM_DBG(CAM_SFE, "SFE top irq status: 0x%x", irq_status);
	for (i = 0; i < th_payload->num_registers; i++)
		evt_payload->irq_reg_val[i] = th_payload->evt_status_arr[i];
...
void cam_sfe_top_sel_frame_counter(uint32_t res_id, uint32_t *val,
				   bool read_counter,
				   struct cam_sfe_path_data *path_data)
{
				     ...
				     uint32_t violation_status)
{
	...
	CAM_INFO(CAM_SFE, "SFE[%u] IPP Violation status 0x%x", soc_info->index,
		 val);
...
static void cam_sfe_top_print_top_irq_error(struct cam_sfe_top_priv *top_priv,
					    uint32_t irq_status,
					    uint32_t violation_status)
{
	uint32_t i = 0;
...
static int cam_sfe_top_handle_err_irq_bottom_half(void *handler_priv,
						  void *evt_payload_priv)
{
	...
	if (irq_status[0] &
		...
		cam_sfe_top_print_top_irq_error(top_priv, irq_status[0],
						viol_sts);
...
static int cam_sfe_top_handle_irq_bottom_half(void *handler_priv,
					      void *evt_payload_priv)
{
	int i;
	...
	if (irq_status[0] & path_data->path_reg_data->subscribe_irq_mask) {
		if (irq_status[0] & path_data->path_reg_data->sof_irq_mask) {
			CAM_DBG(CAM_SFE, "SFE:%d Received %s SOF",
				res->hw_intf->hw_idx, res->res_name);
			...
			if (top_priv->sfe_debug_cfg &
				...
				val0 = cam_io_r(path_data->mem_base + offset0);
				val1 = cam_io_r(path_data->mem_base + offset1);
				CAM_INFO(
					CAM_SFE,
					"SFE:%d HBI: 0x%x VBI: 0x%x NEQ_HBI: %s HBI_MIN_ERR: %s",
					res->hw_intf->hw_idx, (val0 & 0x3FFF),
					val1,
			...
			if (top_priv->sfe_debug_cfg &
				...
				cam_sfe_top_sel_frame_counter(res->res_id,
							      &frame_cnt, true,
							      path_data);

			cam_sfe_top_dump_perf_counters("SOF", res->res_name,
						       top_priv);
		}

		if (irq_status[0] & path_data->path_reg_data->eof_irq_mask) {
			CAM_DBG(CAM_SFE, "SFE:%d Received %s EOF",
				res->hw_intf->hw_idx, res->res_name);

			cam_sfe_top_dump_perf_counters("EOF", res->res_name,
						       top_priv);
		}
...
int cam_sfe_top_start(void *priv, void *start_args, uint32_t arg_size)
{
	int rc = -EINVAL;
	...
	CAM_DBG(CAM_SFE, "SFE HW [%u] core_cfg: 0x%x", hw_info->soc_info.index,
		core_cfg);

	for (i = 0; i < path_data->common_reg->num_sfe_mode; i++) {
		if (path_data->common_reg->sfe_mode[i].value == core_cfg) {
	...
	if (!top_priv->start_stop_cnt) {
		...
		if (path_data->common_reg->top_cc_test_bus_supported) {
			for (i = 0; i < top_priv->hw_info->num_of_testbus;
			     i++) {
				if ((top_priv->sfe_debug_cfg &
				     top_priv->hw_info->test_bus_info[i]
					     .debugfs_val) ||
			...
			if (top_priv->cc_testbus_sel_cfg)
				cam_io_w(
					top_priv->cc_testbus_sel_cfg,
					path_data->mem_base +
						path_data->common_reg
							->top_cc_test_bus_ctrl);
		}
	...
	if (top_priv->sfe_debug_cfg & SFE_DEBUG_ENABLE_SENSOR_DIAG_INFO) {
		if ((top_priv->sensor_sel_diag_cfg) &&
		    (top_priv->sensor_sel_diag_cfg < CAM_SFE_TOP_IN_PORT_MAX))
	...
	if (top_priv->sfe_debug_cfg & SFE_DEBUG_ENABLE_FRAME_COUNTER) {
		cam_sfe_top_sel_frame_counter(sfe_res->res_id, &val, false,
					      path_data);
		debug_cfg_enabled = true;
	}
	...
	if (debug_cfg_enabled) {
		...
		cam_io_w(diag_cfg, path_data->mem_base +
					   path_data->common_reg->diag_config);
	}
	...
	if (!top_priv->error_irq_handle) {
		top_priv->error_irq_handle = cam_irq_controller_subscribe_irq(
			top_priv->common_data.sfe_irq_controller,
			CAM_IRQ_PRIORITY_0, error_mask, top_priv,
			cam_sfe_top_handle_err_irq_top_half,
			cam_sfe_top_handle_err_irq_bottom_half,
			sfe_res->tasklet_info, &tasklet_bh_api,
			CAM_IRQ_EVT_GROUP_0);
		...
		if (!path_data->sof_eof_handle) {
			...
			path_data->sof_eof_handle =
				cam_irq_controller_subscribe_irq(
					top_priv->common_data.sfe_irq_controller,
					CAM_IRQ_PRIORITY_1, sof_eof_mask,
					sfe_res,
...
int cam_sfe_top_stop(void *priv, void *stop_args, uint32_t arg_size)
{
	int i;
	...
	if (!top_priv->start_stop_cnt &&
	    ((top_priv->sfe_debug_cfg & SFE_DEBUG_ENABLE_FRAME_COUNTER) ||
	     (top_priv->sfe_debug_cfg & SFE_DEBUG_ENABLE_SENSOR_DIAG_INFO))) {
...
int cam_sfe_top_init_hw(void *priv, void *init_hw_args, uint32_t arg_size)
{
	...
	mem_base = top_priv->common_data.soc_info->reg_map[SFE_CORE_BASE_IDX]
			   .mem_base;

	CAM_DBG(CAM_SFE, "SFE:%d hw-version:0x%x",
		top_priv->common_data.hw_intf->hw_idx,
		cam_io_r_mb(mem_base +
			    top_priv->hw_info->common_reg->hw_version));

	return 0;
}

int cam_sfe_top_init(uint32_t hw_version, struct cam_hw_soc_info *soc_info,
		     struct cam_hw_intf *hw_intf, void *top_hw_info,
		     void *sfe_irq_controller, struct cam_sfe_top **sfe_top_ptr)
{
	int i, j, rc = 0;
	...
	top_priv = kzalloc(sizeof(struct cam_sfe_top_priv), GFP_KERNEL);
	if (!top_priv) {
		rc = -ENOMEM;
		goto free_sfe_top;
	}
	...
	if (sfe_top_hw_info->num_inputs > CAM_SFE_TOP_IN_PORT_MAX) {
		CAM_ERR(CAM_SFE,
			...
			sfe_top_hw_info->num_inputs, CAM_SFE_TOP_IN_PORT_MAX);
		rc = -EINVAL;
		goto free_top_priv;
	}
	...
	CAM_DBG(CAM_SFE, "Initializing SFE [%u] top with hw_version: 0x%x",
		hw_intf->hw_idx, hw_version);
	for (i = 0, j = 0; i < top_priv->num_in_ports && j < CAM_SFE_RDI_MAX;
	     i++) {
		...
		if (sfe_top_hw_info->input_type[i] == CAM_SFE_PIX_VER_1_0) {
	...
	top_priv->common_data.common_reg = sfe_top_hw_info->common_reg;
...
int cam_sfe_top_deinit(uint32_t hw_version, struct cam_sfe_top **sfe_top_ptr)
{
	int i, rc = 0;
	unsigned long flags;
	struct cam_sfe_top *sfe_top;
	...
	CAM_DBG(CAM_SFE, "Deinit SFE [%u] top with hw_version 0x%x",
		top_priv->common_data.hw_intf->hw_idx, hw_version);
