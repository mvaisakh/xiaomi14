...
int cam_sfe_get_hw_caps(void *device_priv, void *get_hw_cap_args,
			uint32_t arg_size)
{
	return -EPERM;
}

int cam_sfe_reset(void *device_priv, void *reset_core_args, uint32_t arg_size)
{
	return -EPERM;
}

int cam_sfe_read(void *device_priv, void *read_args, uint32_t arg_size)
{
	return -EPERM;
}

int cam_sfe_write(void *device_priv, void *write_args, uint32_t arg_size)
{
	return -EPERM;
}
...
int cam_sfe_init_hw(void *hw_priv, void *init_hw_args, uint32_t arg_size)
{
	...
	if (core_info->sfe_top->hw_ops.init) {
		rc = core_info->sfe_top->hw_ops.init(
			core_info->sfe_top->top_priv, NULL, 0);
		if (rc) {
...
int cam_sfe_deinit_hw(void *hw_priv, void *deinit_hw_args, uint32_t arg_size)
{
	...
	if (core_info->sfe_top->hw_ops.deinit) {
		rc = core_info->sfe_top->hw_ops.deinit(
			core_info->sfe_top->top_priv, NULL, 0);
		if (rc)
...
int cam_sfe_test_irq_line(void *hw_priv)
{
	...
	if (rc) {
		CAM_ERR(CAM_SFE, "SFE:%d failed to init hw",
			sfe_hw->soc_info.index);
		return rc;
	}

	rc = cam_irq_controller_test_irq_line(core_info->sfe_irq_controller,
					      "SFE:%d", sfe_hw->soc_info.index);
	if (rc)
		CAM_ERR(CAM_SFE, "failed to test SFE:%d",
			sfe_hw->soc_info.index);

	rc = cam_sfe_deinit_hw(sfe_hw, NULL, 0);
	if (rc)
		CAM_ERR(CAM_SFE, "SFE:%d failed to deinit hw",
			sfe_hw->soc_info.index);
...
int cam_sfe_reserve(void *hw_priv, void *reserve_args, uint32_t arg_size)
{
	...
	if (!hw_priv || !reserve_args ||
	    (arg_size != sizeof(struct cam_sfe_acquire_args))) {
	...
	CAM_DBG(CAM_SFE, "SFE acquire for res type: %d", acquire->rsrc_type);

	mutex_lock(&sfe_hw->hw_mutex);
	if (acquire->rsrc_type == CAM_ISP_RESOURCE_SFE_IN)
		rc = core_info->sfe_top->hw_ops.reserve(
			core_info->sfe_top->top_priv, reserve_args, arg_size);
...
int cam_sfe_release(void *hw_priv, void *release_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_SFE, "SFE release for res type: %d", sfe_res->res_type);
...
int cam_sfe_start(void *hw_priv, void *start_args, uint32_t arg_size)
{
	...
	if (sfe_res->res_type == CAM_ISP_RESOURCE_SFE_IN) {
		...
		rc = core_info->sfe_bus_rd->hw_ops.start(sfe_res, NULL, 0);
		if (rc)
	...
	CAM_DBG(CAM_SFE,
		...
		sfe_res->res_type, sfe_res->res_id, sfe_res->res_state, rc);
	return rc;
}
...
int cam_sfe_process_cmd(void *hw_priv, uint32_t cmd_type, void *cmd_args,
			uint32_t arg_size)
{
	struct cam_hw_info *sfe_hw = hw_priv;
	struct cam_hw_soc_info *soc_info = NULL;
	...
	case CAM_ISP_HW_CMD_APPLY_CLK_BW_UPDATE:
		rc = core_info->sfe_top->hw_ops.process_cmd(
			core_info->sfe_top->top_priv, cmd_type, cmd_args,
			arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_GET_LAST_CONSUMED_ADDR:
		rc = core_info->sfe_bus_wr->hw_ops.process_cmd(
			core_info->sfe_bus_wr->bus_priv, cmd_type, cmd_args,
			arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_GET_RM_SECURE_MODE:
		rc = core_info->sfe_bus_rd->hw_ops.process_cmd(
			core_info->sfe_bus_rd->bus_priv, cmd_type, cmd_args,
			arg_size);
		break;
...
irqreturn_t cam_sfe_irq(int irq_num, void *data)
{
	...
	return cam_irq_controller_handle_irq(
		irq_num, core_info->sfe_irq_controller, CAM_IRQ_EVT_GROUP_0);
}
		      ...
		      struct cam_sfe_hw_info *sfe_hw_info)
{
	...
	CAM_DBG(CAM_SFE, "SFE device [%u] INIT success", hw_intf->hw_idx);
	return rc;

deinit_bus_wr:
	cam_sfe_bus_deinit(sfe_hw_info->bus_wr_version, BUS_TYPE_SFE_WR,
			   &core_info->sfe_bus_wr);
deinit_top:
	cam_sfe_top_deinit(sfe_hw_info->top_version, &core_info->sfe_top);
deinit_controller:
	if (cam_irq_controller_deinit(&core_info->sfe_irq_controller))
		CAM_ERR(CAM_SFE, "Error cam_irq_controller_deinit failed rc=%d",
			rc);
			...
			struct cam_sfe_hw_info *sfe_hw_info)
{
	...
	rc = cam_sfe_bus_deinit(sfe_hw_info->bus_rd_version, BUS_TYPE_SFE_RD,
				&core_info->sfe_bus_rd);
	if (rc)
		CAM_ERR(CAM_SFE, "SFE bus rd deinit failed rc: %d", rc);

	rc = cam_sfe_bus_deinit(sfe_hw_info->bus_wr_version, BUS_TYPE_SFE_WR,
				&core_info->sfe_bus_wr);
	if (rc)
		CAM_ERR(CAM_SFE, "SFE bus wr deinit failed rc: %d", rc);

	rc = cam_sfe_top_deinit(sfe_hw_info->top_version, &core_info->sfe_top);
	if (rc)
		CAM_ERR(CAM_SFE, "SFE top deinit failed rc: %d", rc);
