...
struct cam_sfe_bus_wr_common_data {
	...
	struct cam_sfe_bus_wr_irq_evt_payload
		evt_payload[CAM_SFE_BUS_WR_PAYLOAD_MAX];
...
static int
cam_sfe_bus_subscribe_error_irq(struct cam_sfe_bus_wr_priv *bus_priv);

static inline int32_t
cam_sfe_bus_wr_get_out_type(struct cam_sfe_bus_wr_priv *bus_priv, int32_t index)
{
	int32_t sfe_out_type = CAM_SFE_BUS_SFE_OUT_MAX;

	if (bus_priv && bus_priv->sfe_out_hw_info)
		sfe_out_type = bus_priv->sfe_out_hw_info[index].sfe_out_type;
...
bool cam_sfe_is_mipi_pcking_needed(struct cam_sfe_bus_wr_priv *bus_priv,
				   int wm_index)
{
	int i;
...
cam_sfe_bus_get_packer_fmt(struct cam_sfe_bus_wr_priv *bus_priv,
...
static void
cam_sfe_bus_wr_print_constraint_errors(struct cam_sfe_bus_wr_priv *bus_priv,
				       uint8_t *wm_name,
	...
	for (i = 0; i < constraint_err_info->num_cons_err; i++) {
		if (constraint_err_info->constraint_error_list[i].bitmask &
			...
			CAM_ERR(CAM_SFE, "WM: %s Error_desc: %s", wm_name,
				constraint_err_info->constraint_error_list[i]
					.error_description);
		}
	}
}
...
static void
cam_sfe_bus_wr_get_constraint_errors(bool *skip_error_notify,
				     struct cam_sfe_bus_wr_priv *bus_priv,
				     uint32_t *cons_err)
{
	...
	for (i = 0; i < bus_priv->num_out; i++) {
       ...
       */
			...
			if ((*cons_err &
			     BIT(cons_err_info->img_width_unalign_shift)) &&
			    (wm_data->en_cfg & BIT(16))) {
				*skip_error_notify = true;
				CAM_DBG(CAM_SFE,
					"Ignoring Image Width Unalign error on SFE[%u] out rsrc: %u "
					"WM: %s in frame based mode",
					common_data->core_index,
					out_rsrc_data->out_type, wm_name);
				continue;
			}
...
static int
cam_sfe_bus_config_rdi_wm(struct cam_sfe_bus_wr_wm_resource_data *rsrc_data)
{
	...
	case CAM_FORMAT_MIPI_RAW_10:
		...
		if (rsrc_data->use_wm_pack) {
			...
			if (rsrc_data->wm_mode == CAM_SFE_WM_LINE_BASED_MODE)
				rsrc_data->width =
					ALIGNUP((rsrc_data->acquired_width), 8);
		}
		break;
	case CAM_FORMAT_MIPI_RAW_6:
		if (rsrc_data->wm_mode == CAM_SFE_WM_LINE_BASED_MODE) {
	...
	case CAM_FORMAT_MIPI_RAW_8:
		if (rsrc_data->wm_mode == CAM_SFE_WM_LINE_BASED_MODE) {
			rsrc_data->en_cfg = 0x1;
			rsrc_data->width = ALIGNUP(rsrc_data->width, 16) / 16;
		} else if (rsrc_data->wm_mode == CAM_SFE_WM_FRAME_BASED_MODE) {
			cam_sfe_bus_config_rdi_wm_frame_based_mode(rsrc_data);
		} else {
			CAM_WARN(CAM_SFE,
...
static int cam_sfe_bus_acquire_wm(
	struct cam_sfe_bus_wr_priv *bus_priv,
	struct cam_sfe_hw_sfe_out_acquire_args *out_acq_args, void *tasklet,
	enum cam_sfe_bus_sfe_out_type sfe_out_res_id,
	enum cam_sfe_bus_plane_type plane, struct cam_isp_resource_node *wm_res,
	uint32_t *comp_done_mask,
	enum cam_sfe_bus_wr_comp_grp_type *comp_grp_id)
{
	...
	if (wm_res->res_state != CAM_ISP_RESOURCE_STATE_AVAILABLE) {
		CAM_ERR(CAM_SFE, "WM:%d not available state:%d", wm_idx,
			wm_res->res_state);
		return -EALREADY;
	}
	...
	rsrc_data->pack_fmt =
		cam_sfe_bus_get_packer_fmt(bus_priv, rsrc_data->format, wm_idx);
	...
	*comp_done_mask |= (1 << sfe_out_res_id);
	...
	CAM_DBG(CAM_SFE,
		"SFE:%d WM:%d %s processed width:%d height:%d format:0x%X pack_fmt "
		"0x%x %s",
...
static int cam_sfe_bus_release_wm(void *bus_priv,
				  struct cam_isp_resource_node *wm_res)
{
	struct cam_sfe_bus_wr_wm_resource_data *rsrc_data = wm_res->res_priv;
...
static int cam_sfe_bus_start_wm(struct cam_isp_resource_node *wm_res)
{
	...
	struct cam_sfe_bus_wr_wm_resource_data *rsrc_data = wm_res->res_priv;
	struct cam_sfe_bus_wr_common_data *common_data = rsrc_data->common_data;
	...
	CAM_DBG(CAM_SFE, "WM:%d pk_fmt:%d stride:%d", rsrc_data->index,
		rsrc_data->pack_fmt & PACKER_FMT_MAX, rsrc_data->stride);
...
static int
cam_sfe_bus_handle_wm_done_top_half(uint32_t evt_id,
...
static int cam_sfe_bus_init_wm_resource(uint32_t index,
					...
					struct cam_isp_resource_node *wm_res,
					struct cam_isp_resource_node **comp_grp,
					uint8_t *wm_name, bool en_line_done)
{
	struct cam_sfe_bus_wr_wm_resource_data *rsrc_data;
	...
	*comp_grp = &bus_priv->comp_grp[(&hw_info->bus_client_reg[index])
						->comp_group];
	...
	wm_res->bottom_half_handler = cam_sfe_bus_handle_wm_done_bottom_half;
...
static int cam_sfe_bus_deinit_wm_resource(struct cam_isp_resource_node *wm_res)
{
	struct cam_sfe_bus_wr_wm_resource_data *rsrc_data;
...
static void
cam_sfe_bus_add_wm_to_comp_grp(struct cam_isp_resource_node *comp_grp,
			       uint32_t composite_mask)
{
	...
	if (comp_grp->res_state == CAM_ISP_RESOURCE_STATE_AVAILABLE) {
		...
		if (comp_grp->res_state == CAM_ISP_RESOURCE_STATE_STREAMING) {
			CAM_ERR(CAM_SFE, "Invalid State %d comp_grp:%u",
				comp_grp->res_state, rsrc_data->comp_grp_type);
			return -EBUSY;
		}
	}
...
static int
cam_sfe_bus_release_comp_grp(struct cam_sfe_bus_wr_priv *bus_priv,
			     struct cam_isp_resource_node *in_comp_grp)
{
	...
	if (in_comp_grp->res_state == CAM_ISP_RESOURCE_STATE_STREAMING) {
		CAM_ERR(CAM_SFE, "Invalid State %d", in_comp_grp->res_state);
		return -EBUSY;
	}

	in_rsrc_data = in_comp_grp->res_priv;
	CAM_DBG(CAM_SFE, "Release SFE:%d comp_grp:%u",
		bus_priv->common_data.core_index, in_rsrc_data->comp_grp_type);
...
static int cam_sfe_bus_start_comp_grp(struct cam_isp_resource_node *comp_grp,
...
static inline int
cam_sfe_bus_stop_comp_grp(struct cam_isp_resource_node *comp_grp)
{
	comp_grp->res_state = CAM_ISP_RESOURCE_STATE_RESERVED;
...
static int cam_sfe_bus_deinit_comp_grp(struct cam_isp_resource_node *comp_grp)
{
	struct cam_sfe_bus_wr_comp_grp_data *rsrc_data = comp_grp->res_priv;
					  ...
					  uint32_t arg_size)
{
	...
	rsrc_data =
		(struct cam_sfe_bus_wr_out_data *)secure_mode->res->res_priv;
				       ...
				       uint32_t args_size)
{
	...
	if (rsrc_node->res_state != CAM_ISP_RESOURCE_STATE_AVAILABLE) {
		CAM_ERR(CAM_SFE,
			...
			bus_priv->common_data.core_index, sfe_out_res_id,
			rsrc_node->res_state);
		return -EBUSY;
	}
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		rc = cam_sfe_bus_acquire_wm(bus_priv, out_acquire_args,
					    acq_args->tasklet, sfe_out_res_id,
					    i, &rsrc_data->wm_res[i],
					    &client_done_mask, &comp_grp_id);
		if (rc) {
			CAM_ERR(CAM_SFE,
	...
	rc = cam_sfe_bus_acquire_comp_grp(
		bus_priv, out_acquire_args->out_port_info, acq_args->tasklet,
	...
	if (rc) {
		CAM_ERR(CAM_SFE,
			...
			rsrc_data->common_data->core_index, sfe_out_res_id, rc);
		goto release_wm;
	}
	...
	cam_sfe_bus_add_wm_to_comp_grp(rsrc_data->comp_grp, client_done_mask);
...
static int cam_sfe_bus_start_sfe_out(struct cam_isp_resource_node *sfe_out)
{
	int rc = 0, i;
	struct cam_sfe_bus_wr_out_data *rsrc_data = NULL;
	struct cam_sfe_bus_wr_priv *bus_priv;
	...
	rc = cam_sfe_bus_start_comp_grp(rsrc_data->comp_grp, bus_irq_reg_mask);

	if (rsrc_data->is_dual && !rsrc_data->is_master)
		goto end;

	sfe_out->irq_handle = cam_irq_controller_subscribe_irq(
		common_data->buf_done_controller, CAM_IRQ_PRIORITY_1,
		bus_irq_reg_mask, sfe_out, sfe_out->top_half_handler,
		sfe_out->bottom_half_handler, sfe_out->tasklet_info,
...
static int cam_sfe_bus_stop_sfe_out(struct cam_isp_resource_node *sfe_out)
{
	int rc = 0, i;
	struct cam_sfe_bus_wr_out_data *rsrc_data = NULL;
	struct cam_sfe_bus_wr_priv *bus_priv;
	...
	if (sfe_out->irq_handle) {
		cam_irq_controller_unsubscribe_irq(
			common_data->buf_done_controller, sfe_out->irq_handle);
		sfe_out->irq_handle = 0;
	}
...
static int
cam_sfe_bus_handle_sfe_out_done_top_half(uint32_t evt_id,
					 struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	int i;
	...
	if (status_0 & resource_data->comp_done_mask) {
		...
		evt_payload->last_consumed_addr =
			cam_io_r_mb(wm_rsrc_data->common_data->mem_base +
				    wm_rsrc_data->hw_regs->addr_status_0);
		trace_cam_log_event("bufdone", "bufdone_IRQ", status_0,
				    resource_data->comp_grp_type);
	}

	return rc;
}

static int cam_sfe_bus_handle_comp_done_bottom_half(void *handler_priv,
						    void *evt_payload_priv,
...
static int cam_sfe_bus_handle_sfe_out_done_bottom_half(void *handler_priv,
						       void *evt_payload_priv)
{
	int rc = -EINVAL;
	uint32_t evt_id = 0;
...
static int
cam_sfe_bus_init_sfe_out_resource(uint32_t index,
				  struct cam_sfe_bus_wr_priv *bus_priv,
				  struct cam_sfe_bus_wr_hw_info *hw_info)
{
	struct cam_isp_resource_node *sfe_out = NULL;
	struct cam_sfe_bus_wr_out_data *rsrc_data = NULL;
	int rc = 0, i = 0;
	int32_t sfe_out_type = cam_sfe_bus_wr_get_out_type(bus_priv, index);

	if (sfe_out_type < 0 || sfe_out_type >= CAM_SFE_BUS_SFE_OUT_MAX) {
	...
	rsrc_data = kzalloc(sizeof(struct cam_sfe_bus_wr_out_data), GFP_KERNEL);
	if (!rsrc_data) {
		rc = -ENOMEM;
		return rc;
	}
	...
	rsrc_data->source_group = hw_info->sfe_out_hw_info[index].source_group;
	rsrc_data->out_type = hw_info->sfe_out_hw_info[index].sfe_out_type;
	rsrc_data->common_data = &bus_priv->common_data;
	rsrc_data->max_width = hw_info->sfe_out_hw_info[index].max_width;
	rsrc_data->max_height = hw_info->sfe_out_hw_info[index].max_height;
	rsrc_data->secure_mode = CAM_SECURE_MODE_NON_SECURE;
	rsrc_data->num_wm = hw_info->sfe_out_hw_info[index].num_wm;

	rsrc_data->wm_res = kzalloc(
		(sizeof(struct cam_isp_resource_node) * rsrc_data->num_wm),
		GFP_KERNEL);
	if (!rsrc_data->wm_res) {
	...
	rc = cam_sfe_bus_init_wm_resource(
		hw_info->sfe_out_hw_info[index].wm_idx, bus_priv, hw_info,
		&rsrc_data->wm_res[i], &rsrc_data->comp_grp,
	...
	sfe_out->top_half_handler = cam_sfe_bus_handle_sfe_out_done_top_half;
...
static int
cam_sfe_bus_deinit_sfe_out_resource(struct cam_isp_resource_node *sfe_out)
{
	struct cam_sfe_bus_wr_out_data *rsrc_data = sfe_out->res_priv;
	int rc = 0, i = 0;
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		...
		if (rc < 0)
			CAM_ERR(CAM_SFE, "SFE:%d deinit WM:%d failed rc:%d",
...
static inline void
__cam_sfe_bus_wr_print_wm_info(struct cam_sfe_bus_wr_wm_resource_data *wm_data)
{
	...
	CAM_INFO(
		CAM_SFE,
		"SFE:%u WM:%u width:%u height:%u stride:%u x_init:%u en_cfg:%u "
		"acquired width:%u height:%u pack_cfg: 0x%x",
		wm_data->common_data->core_index, wm_data->index,
		wm_data->width, wm_data->height, wm_data->stride,
		wm_data->h_init, wm_data->en_cfg, wm_data->acquired_width,
		wm_data->acquired_height, wm_data->pack_fmt);

	CAM_INFO(
		CAM_SFE,
		"SFE:%u WM:%u last_consumed_image_addr:0x%x "
		"last_consumed_frame_header:0x%x fifo_word_cnt:0x%x [FH + Image] "
		"current_image_addr:0x%x",
		wm_data->common_data->hw_intf->hw_idx, wm_data->index,
		addr_status0, addr_status1, addr_status2, addr_status3);
}

static int
cam_sfe_bus_wr_print_dimensions(enum cam_sfe_bus_sfe_out_type sfe_out_res_id,
				struct cam_sfe_bus_wr_priv *bus_priv)
{
	struct cam_isp_resource_node *rsrc_node = NULL;
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		...
		if (wm_idx < 0 || wm_idx >= bus_priv->num_client) {
			CAM_WARN(CAM_SFE, "Unsupported SFE out %d",
				 sfe_out_res_id);
			continue;
		}
...
static int cam_sfe_bus_mini_dump(struct cam_sfe_bus_wr_priv *bus_priv,
				 void *cmd_args)
{
	struct cam_isp_resource_node *rsrc_node = NULL;
	struct cam_sfe_bus_wr_out_data *rsrc_data = NULL;
	...
	md->clk_rate =
		cam_soc_util_get_applied_src_clk(&hw_info->soc_info, true);
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		...
		for (j = 0; j < rsrc_data->num_wm; j++) {
			...
			scnprintf(md_wm->name, CAM_ISP_RES_NAME_LEN, "%s",
...
static void *cam_sfe_bus_wr_user_dump_info(void *dump_struct, uint8_t *addr_ptr)
{
	struct cam_sfe_bus_wr_wm_resource_data *wm = NULL;
	uint32_t *addr;
...
static int cam_sfe_bus_wr_user_dump(struct cam_sfe_bus_wr_priv *bus_priv,
				    void *cmd_args)
{
	struct cam_isp_resource_node *rsrc_node = NULL;
	struct cam_sfe_bus_wr_out_data *rsrc_data = NULL;
	...
	if (hw_info->hw_state == CAM_HW_STATE_POWER_DOWN) {
		CAM_WARN(CAM_ISP, "SFE BUS powered down, continuing");
		return -EINVAL;
	}

	rc = cam_common_user_dump_helper(dump_args, cam_common_user_dump_clock,
					 hw_info, sizeof(uint64_t),
					 "CLK_RATE_PRINT:");
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		...
		for (j = 0; j < rsrc_data->num_wm; j++) {
			...
			rc = cam_common_user_dump_helper(
				dump_args, cam_sfe_bus_wr_user_dump_info, wm,
				sizeof(uint32_t), "SFE_BUS_CLIENT.%s.%d:",
				rsrc_data->wm_res[j].res_name,
				rsrc_data->common_data->core_index);

			if (rc) {
				CAM_ERR(CAM_ISP,
					"SFE BUS WR: Info dump failed, rc:%d",
					rc);
				return rc;
			}
		}
...
static int
cam_sfe_bus_wr_handle_err_irq_top_half(uint32_t evt_id,
				       struct cam_irq_th_payload *th_payload)
{
	int i = 0, rc = 0;
	struct cam_sfe_bus_wr_priv *bus_priv = th_payload->handler_priv;
	struct cam_sfe_bus_wr_irq_evt_payload *evt_payload;
...
static int
cam_sfe_bus_wr_get_err_port_info(struct cam_sfe_bus_wr_priv *bus_priv,
				 uint32_t err_status, void **rsrc_data_priv,
				 uint64_t *out_port)
{
...
static int cam_sfe_bus_wr_handle_err_irq_bottom_half(void *handler_priv,
						     void *evt_payload_priv)
{
	int idx = -1;
	int rc = 0;
...
static int cam_sfe_bus_subscribe_error_irq(struct cam_sfe_bus_wr_priv *bus_priv)
{
	struct cam_sfe_bus_wr_common_data *common_data;
	...
	bus_priv->bus_irq_handle = cam_irq_controller_subscribe_irq(
		common_data->sfe_irq_controller, CAM_IRQ_PRIORITY_0,
		top_irq_reg_mask, bus_priv, cam_sfe_bus_wr_handle_bus_irq, NULL,
	...
	if (bus_priv->tasklet_info != NULL) {
		...
		bus_priv->error_irq_handle = cam_irq_controller_subscribe_irq(
			common_data->bus_irq_controller, CAM_IRQ_PRIORITY_0,
			bus_wr_error_irq_mask, bus_priv,
				    ...
				    uint32_t arg_size)
{
	...
	sfe_out_data =
		(struct cam_sfe_bus_wr_out_data *)update_buf->res->res_priv;
	...
	for (i = 0, j = 0; i < sfe_out_data->num_wm; i++) {
		...
		wm_data = (struct cam_sfe_bus_wr_wm_resource_data *)sfe_out_data
				  ->wm_res[i]
				  .res_priv;

		CAM_SFE_ADD_REG_VAL_PAIR(reg_val_pair, j, wm_data->hw_regs->cfg,
					 wm_data->en_cfg);
		CAM_DBG(CAM_SFE, "WM:%d %s en_cfg 0x%X", wm_data->index,
			sfe_out_data->wm_res[i].res_name, reg_val_pair[j - 1]);
		CAM_SFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
					 wm_data->hw_regs->packer_cfg,
					 wm_data->pack_fmt);
		CAM_DBG(CAM_SFE, "WM:%d %s packer_fmt 0x%X", wm_data->index,
			sfe_out_data->wm_res[i].res_name, reg_val_pair[j - 1]);

		curr_cache_cfg = wm_data->cache_cfg;
		wm_data->cache_cfg =
			bus_priv->common_data.sys_cache_default_cfg;
		if (wm_data->enable_caching) {
			...
			wm_data->cache_cfg = wm_data->current_scid << 8;
			wm_data->cache_cfg |= 3 << 4;
			if ((update_buf->use_scratch_cfg) &&
			    (cache_dbg_cfg->scratch_dbg_cfg))
				wm_data->cache_cfg |=
					cache_dbg_cfg->scratch_alloc;
...
skip_cache_cfg:
		...
		CAM_SFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
					 ...
					 wm_data->cache_cfg);
		CAM_DBG(CAM_SFE, "WM:%d cache_cfg:0x%x", wm_data->index,
			reg_val_pair[j - 1]);
		...
		if (!(wm_data->en_cfg & (0x3 << 16))) {
			CAM_SFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
						 ...
						 wm_data->h_init);
			CAM_DBG(CAM_SFE, "WM:%d h_init 0x%X", wm_data->index,
				reg_val_pair[j - 1]);
		}

		/* WM Image address */
		iova = update_buf->wm_update->image_buf[i] + wm_data->offset;
		update_buf->wm_update->image_buf_offset[i] = wm_data->offset;
		...
		CAM_DBG(CAM_SFE, "WM:%d image address 0x%x", wm_data->index,
			img_addr);
	...
	if (num_regval_pairs) {
		...
		cdm_util_ops->cdm_write_regrandom(update_buf->cmd.cmd_buf_addr,
						  num_regval_pairs,
						  reg_val_pair);
		...
		CAM_DBG(CAM_SFE, "No reg val pairs. num_wms: %u",
			sfe_out_data->num_wm);
				    ...
				    uint32_t arg_size)
{
	...
	for (i = 0; i < sfe_out_data->num_wm; i++) {
		...
		val = (wm_data->height << 16) | wm_data->width;
		cam_io_w_mb(val, wm_data->common_data->mem_base +
		...
		CAM_DBG(CAM_SFE, "before stride %d", val);
		val = ALIGNUP(val, 16);
		if (val != stride && val != wm_data->stride)
		...
		cam_io_w_mb(img_addr, wm_data->common_data->mem_base +
					      wm_data->hw_regs->image_addr);
		cam_io_w_mb(frame_inc, wm_data->common_data->mem_base +
					       wm_data->hw_regs->frame_incr);
		...
		cam_io_w_mb(wm_data->pack_fmt,
			    wm_data->common_data->mem_base +
				    wm_data->hw_regs->packer_cfg);
		CAM_DBG(CAM_SFE, "WM:%d packer_cfg 0x%X", wm_data->index,
			wm_data->pack_fmt);

		/* enable the WM */
		cam_io_w_mb(wm_data->en_cfg, wm_data->common_data->mem_base +
						     wm_data->hw_regs->cfg);
		CAM_DBG(CAM_SFE, "WM:%d en_cfg 0x%X", wm_data->index,
			wm_data->en_cfg);
				     ...
				     uint32_t arg_size)
{
	...
	sfe_out_data =
		(struct cam_sfe_bus_wr_out_data *)update_hfr->res->res_priv;
	...
	if (num_regval_pairs) {
		...
		cdm_util_ops->cdm_write_regrandom(update_hfr->cmd.cmd_buf_addr,
						  num_regval_pairs,
						  reg_val_pair);
					    ...
					    uint32_t arg_size)
{
	...
	sfe_out_data =
		(struct cam_sfe_bus_wr_out_data *)stripe_args->res->res_priv;
...
static int cam_sfe_bus_wr_update_wm_config(void *cmd_args)
{
	int i;
	...
	wm_config = (struct cam_isp_vfe_wm_config *)wm_config_update->data;
	...
	for (i = 0; i < sfe_out_data->num_wm; i++) {
		...
		if (wm_config->packer_format) {
			packer_fmt = cam_sfe_bus_get_packer_fmt(
				sfe_out_data->bus_priv,
...
static int cam_sfe_bus_wr_update_bw_limiter(void *priv, void *cmd_args,
					    uint32_t arg_size)
{
	struct cam_isp_hw_get_cmd_update *wm_config_update;
	struct cam_sfe_bus_wr_out_data *sfe_out_data = NULL;
	...
	wm_bw_limit_cfg =
		(struct cam_isp_wm_bw_limiter_config *)wm_config_update->data;
	...
	for (i = 0, j = 0; i < sfe_out_data->num_wm; i++) {
		if (j >=
		    (MAX_REG_VAL_PAIR_SIZE - (MAX_BUF_UPDATE_REG_NUM * 2))) {
			CAM_ERR(CAM_SFE,
		...
		if (i >= CAM_PACKET_MAX_PLANES) {
			CAM_WARN(CAM_SFE, "Num of WMs: %d exceeded max planes",
				 i);
			goto add_reg_pair;
		}
		...
		if (!wm_data->hw_regs->bw_limiter_addr) {
			CAM_ERR(CAM_SFE,
				...
				wm_data->index,
				sfe_out_data->wm_res[i].res_name);
			return -EINVAL;
		}
		...
		CAM_SFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
					 ...
					 reg_val);
		CAM_DBG(CAM_SFE, "WM: %d %s bw_limter: 0x%x", wm_data->index,
			sfe_out_data->wm_res[i].res_name, reg_val_pair[j - 1]);
	}
...
static int cam_sfe_bus_wr_get_res_for_mid(struct cam_sfe_bus_wr_priv *bus_priv,
					  void *cmd_args, uint32_t arg_size)
{
	...
	if (!get_res) {
		CAM_ERR(CAM_SFE, "invalid get resource for mid paramas");
		return -EINVAL;
	}
   ...
   */
	if (i == bus_priv->num_out) {
		CAM_INFO(CAM_SFE, "mid:%d does not match with any out resource",
			 get_res->mid);
		return 0;
	}
...
static int cam_sfe_bus_wr_irq_inject(void *priv, void *cmd_args,
				     uint32_t arg_size)
{
	...
	bus_wr_hw_info =
		(struct cam_sfe_bus_wr_hw_info *)bus_priv->bus_wr_hw_info;
...
static int cam_sfe_bus_wr_dump_irq_desc(void *priv, void *cmd_args,
					uint32_t arg_size)
{
	int i, offset = 0;
	struct cam_sfe_bus_wr_priv *bus_priv = NULL;
...
static int cam_sfe_bus_wr_start_hw(void *hw_priv, void *start_hw_args,
				   uint32_t arg_size)
{
	return cam_sfe_bus_start_sfe_out(hw_priv);
}

static int cam_sfe_bus_wr_stop_hw(void *hw_priv, void *stop_hw_args,
				  uint32_t arg_size)
{
	return cam_sfe_bus_stop_sfe_out(hw_priv);
}

static int cam_sfe_bus_wr_init_hw(void *hw_priv, void *init_hw_args,
				  uint32_t arg_size)
{
...
static int cam_sfe_bus_wr_deinit_hw(void *hw_priv, void *deinit_hw_args,
				    uint32_t arg_size)
{
	return 0;
}

static int cam_sfe_bus_wr_cache_config(void *priv, void *cmd_args,
				       uint32_t arg_size)
{
	int i;
	struct cam_sfe_bus_wr_priv *bus_priv;
	...
	sfe_out_data =
		(struct cam_sfe_bus_wr_out_data *)cache_cfg->res->res_priv;
...
static int cam_sfe_bus_wr_set_debug_cfg(void *priv, void *cmd_args)
{
	struct cam_sfe_bus_wr_priv *bus_priv =
	...
	if (debug_cfg->cache_config)
		cam_sfe_bus_parse_cache_cfg(
			false, debug_cfg->u.cache_cfg.sfe_cache_dbg,
...
static uint32_t
cam_sfe_bus_get_last_consumed_addr(struct cam_sfe_bus_wr_priv *bus_priv,
				   uint32_t res_type)
{
	...
	/* All SFE out ports have single WM */
	wm_rsrc_data = rsrc_data->wm_res->res_priv;
	last_consumed_addr = cam_io_r_mb(wm_rsrc_data->common_data->mem_base +
					 wm_rsrc_data->hw_regs->addr_status_0);

	return last_consumed_addr;
}

static int cam_sfe_bus_wr_process_cmd(void *priv, uint32_t cmd_type,
				      void *cmd_args, uint32_t arg_size)
{
	int rc = -EINVAL;
	struct cam_sfe_bus_wr_priv *bus_priv;
	...
	case CAM_ISP_HW_CMD_STRIPE_UPDATE:
		rc = cam_sfe_bus_wr_update_stripe_cfg(priv, cmd_args, arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_DUMP_BUS_INFO: {
		...
		if (sfe_out_res_id == CAM_SFE_BUS_SFE_OUT_MAX) {
			CAM_DBG(CAM_SFE, "No a SFE write res: 0x%x - skip dump",
				event_info->res_id);
			rc = 0;
			break;
		}
...
int cam_sfe_bus_wr_init(struct cam_hw_soc_info *soc_info,
			...
			void *sfe_irq_controller, struct cam_sfe_bus **sfe_bus)
{
	...
	bus_priv = kzalloc(sizeof(struct cam_sfe_bus_wr_priv), GFP_KERNEL);
	if (!bus_priv) {
	...
	bus_priv->comp_grp = kzalloc(
		(sizeof(struct cam_isp_resource_node) * bus_priv->num_comp_grp),
		GFP_KERNEL);
	...
	for (i = 0; i < bus_priv->num_comp_grp; i++) {
		rc = cam_sfe_bus_wr_init_comp_grp(i, soc_info, bus_priv,
						  bus_hw_info,
						  &bus_priv->comp_grp[i]);
		if (rc < 0) {
...
int cam_sfe_bus_wr_deinit(struct cam_sfe_bus **sfe_bus)
{
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		rc = cam_sfe_bus_deinit_sfe_out_resource(&bus_priv->sfe_out[i]);
		if (rc < 0)
			CAM_ERR(CAM_SFE,
