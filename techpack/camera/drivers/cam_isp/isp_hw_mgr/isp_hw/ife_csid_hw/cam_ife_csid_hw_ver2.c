...
static void count_ife_error(uint32_t index)
{
	...
	if (index < ife_mqs_node1_cnt && index >= 0) {
		...
		CAM_ERR(CAM_ISP,
			"set ife error node1: phy_id: %d, timestamp: sec: %ld, nsec: %ld",
...
static void
cam_ife_csid_ver2_print_debug_reg_status(struct cam_ife_csid_ver2_hw *csid_hw,
					 struct cam_isp_resource_node *res);

static int cam_ife_csid_ver2_print_hbi_vbi(struct cam_ife_csid_ver2_hw *csid_hw,
					   struct cam_isp_resource_node *res);

static bool cam_ife_csid_ver2_cpas_cb(uint32_t handle, void *user_data,
				      struct cam_cpas_irq_data *irq_data)
{
	bool handled = false;
	struct cam_ife_csid_ver2_hw *csid_hw =
		(struct cam_ife_csid_ver2_hw *)user_data;
	...
	case CAM_CAMNOC_IRQ_SLAVE_ERROR:
		if (irq_data->u.slave_err.errlog0_low.err_code ==
...
static bool
cam_ife_csid_ver2_disable_sof_retime(struct cam_ife_csid_ver2_hw *csid_hw,
				     struct cam_isp_resource_node *res)
{
						   ...
						   uint32_t ts_comb_vcdt_mask)
{
	...
	return mul_u64_u32_div(timestamp_val, CAM_IFE_CSID_QTIMER_MUL_FACTOR,
			       CAM_IFE_CSID_QTIMER_DIV_FACTOR);
}

static noinline void
cam_ife_csid_ver2_print_camif_timestamps(struct cam_ife_csid_ver2_hw *csid_hw)
{
	struct cam_ife_csid_ver2_path_cfg *path_cfg;
	struct cam_isp_resource_node *res;
	bool found = false;
	...
	if (found && path_cfg) {
		...
		if (found && path_cfg->handle_camif_irq)
			CAM_INFO(CAM_ISP, "CSID[%u] %s EPOCH[%lld:%lld]",
				 csid_hw->hw_intf->hw_idx, res->res_name,
				 path_cfg->epoch_ts.tv_sec,
				 path_cfg->epoch_ts.tv_nsec);
		...
		CAM_INFO(
			CAM_ISP,
			"qtimer current [SOF: 0x%llx EOF: 0x%llx] prev [SOF: 0x%llx EOF: "
			"0x%llx]",
...
static void cam_ife_csid_ver2_update_event_ts(struct timespec64 *dst_ts,
...
static int
cam_ife_csid_ver2_set_debug(struct cam_ife_csid_ver2_hw *csid_hw,
			    struct cam_ife_csid_debug_cfg_args *debug_args)
{
	int bit_pos = 0;
	uint32_t val, debug_val;

	memset(&csid_hw->debug_info, 0, sizeof(struct cam_ife_csid_debug_info));
   ...
   */
	csid_hw->debug_info.rst_capture_strobes =
		(debug_args->csid_rx_capture_debug &
		 CAM_IFE_CSID_DEBUGFS_RST_STROBE_MASK);
	csid_hw->debug_info.rx_capture_vc =
		((debug_args->csid_rx_capture_debug >>
		  CAM_IFE_CSID_DEBUGFS_VC_SHIFT_MASK) &
		 CAM_IFE_CSID_DEBUGFS_VC_DT_MASK);
	csid_hw->debug_info.rx_capture_dt =
		((debug_args->csid_rx_capture_debug >>
		  CAM_IFE_CSID_DEBUGFS_DT_SHIFT_MASK) &
		 CAM_IFE_CSID_DEBUGFS_VC_DT_MASK);
	csid_hw->debug_info.rx_capture_debug_set =
		debug_args->rx_capture_debug_set;
...
static int cam_ife_csid_ver2_sof_irq_debug(struct cam_ife_csid_ver2_hw *csid_hw,
					   void *cmd_args)
{
	int i = 0;
	int data_idx = 0;
	...
	if (csid_hw->hw_info->hw_state == CAM_HW_STATE_POWER_DOWN) {
		CAM_WARN(CAM_ISP, "CSID:%u powered down unable to %s sof irq",
			 csid_hw->hw_intf->hw_idx,
	...
	for (i = CAM_IFE_PIX_PATH_RES_RDI_0; i < CAM_IFE_PIX_PATH_RES_MAX;
		...
		cam_irq_controller_update_irq(
			...
			path_cfg->irq_handle, sof_irq_enable, &irq_mask);
	}
	...
	data_idx = (int)(csid_hw->rx_cfg.phy_sel -
			 csid_reg->cmn_reg->phy_sel_base_idx);
	if (sof_irq_enable) {
		if (data_idx < 0)
			CAM_WARN(
				CAM_ISP,
			...
			CAM_INFO(CAM_ISP, "CSID:%u Notify CSIPHY: %d",
				 csid_hw->hw_intf->hw_idx, data_idx);
...
static int cam_ife_csid_ver2_get_evt_payload(
	...
	struct list_head *payload_list, spinlock_t *lock)
{
	spin_lock(lock);
	...
	*evt_payload = list_first_entry(
		payload_list, struct cam_ife_csid_ver2_evt_payload, list);
...
static int
cam_ife_csid_ver2_top_info_irq_top_half(uint32_t evt_id,
					struct cam_irq_th_payload *th_payload)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%u TOP info status: 0x%x",
...
static int
cam_ife_csid_ver2_top_err_irq_top_half(uint32_t evt_id,
				       struct cam_irq_th_payload *th_payload)
{
	int rc = 0;
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
...
static int
cam_ife_csid_ver2_handle_buf_done_irq(uint32_t evt_id,
				      struct cam_irq_th_payload *th_payload)
{
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	int rc = 0;
	...
	rc = cam_irq_controller_handle_irq(
		evt_id, csid_hw->buf_done_irq_controller, CAM_IRQ_EVT_GROUP_0);
...
static int
cam_ife_csid_ver2_handle_rx_irq(uint32_t evt_id,
				struct cam_irq_th_payload *th_payload)
{
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	int rc = 0;
...
static int
cam_ife_csid_ver2_handle_path_irq(uint32_t evt_id,
				  struct cam_irq_th_payload *th_payload)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%u %s Enter", csid_hw->hw_intf->hw_idx,
		res->res_name);
	rc = cam_irq_controller_handle_irq(
		evt_id, csid_hw->path_irq_controller[res->res_id],
		CAM_IRQ_EVT_GROUP_0);
	CAM_DBG(CAM_ISP, "CSID:%u %s Exit (rc=%d)", csid_hw->hw_intf->hw_idx,
		res->res_name, rc);
...
static int
cam_ife_csid_ver2_path_err_top_half(uint32_t evt_id,
				    struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
...
static int
cam_ife_csid_ver2_path_top_half(uint32_t evt_id,
				struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	struct cam_hw_info *hw_info;
	...
	if (evt_payload->irq_reg_val & path_reg->sof_irq_mask) {
		evt_payload->sof_ts_reg_val =
			cam_io_r_mb(soc_info->reg_map[0].mem_base +
				    path_reg->timestamp_curr1_sof_addr);
		evt_payload->sof_ts_reg_val =
			(evt_payload->sof_ts_reg_val << 32) |
			cam_io_r_mb(soc_info->reg_map[0].mem_base +
				    path_reg->timestamp_curr0_sof_addr);
	}
	ktime_get_boottime_ts64(&evt_payload->timestamp);
...
static int cam_ife_csid_ver2_mc_top_half(uint32_t evt_id,
					 struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	struct cam_ife_csid_ver2_evt_payload *evt_payload;
	...
	evt_payload = (struct cam_ife_csid_ver2_evt_payload *)
...
static inline void cam_ife_csid_ver2_reset_discard_frame_cfg(
	struct cam_isp_resource_node *res, struct cam_ife_csid_ver2_hw *csid_hw,
	struct cam_ife_csid_ver2_path_cfg *path_cfg)
{
	int rc;
...
static int
cam_ife_csid_ver2_discard_sof_top_half(uint32_t evt_id,
				       struct cam_irq_th_payload *th_payload)
{
	struct cam_hw_info *hw_info;
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
...
static int cam_ife_csid_ver2_discard_sof_pix_bottom_half(void *handler_priv,
							 void *evt_payload_priv)
{
	...
	if (path_cfg->sof_cnt == path_cfg->num_frames_discard) {
		...
		cam_ife_csid_ver2_reset_discard_frame_cfg(res, csid_hw,
							  path_cfg);
	}
end:
...
static int cam_ife_csid_ver2_discard_sof_rdi_bottom_half(void *handler_priv,
							 void *evt_payload_priv)
{
	struct cam_hw_info *hw_info;
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
...
static int
cam_ife_csid_ver2_stop_csi2_in_err(struct cam_ife_csid_ver2_hw *csid_hw)
{
	CAM_DBG(CAM_ISP, "CSID:%u Stop csi2 rx", csid_hw->hw_intf->hw_idx);
	...
	if (csid_hw->rx_cfg.err_irq_handle[CAM_IFE_CSID_RX_IRQ_STATUS_REG0])
		cam_irq_controller_disable_irq(
			csid_hw->rx_irq_controller
				[CAM_IFE_CSID_RX_IRQ_STATUS_REG0],
			csid_hw->rx_cfg
				.err_irq_handle[CAM_IFE_CSID_RX_IRQ_STATUS_REG0]);

	return 0;
}

static inline void
cam_ife_csid_ver2_maskout_rx_irqs(struct cam_ife_csid_ver2_hw *csid_hw)
{
	int rc;
...
static inline void
cam_ife_csid_ver2_disable_rx_evts(struct cam_ife_csid_ver2_hw *csid_hw)
{
	int rc;
...
static int cam_ife_csid_ver2_disable_csi2(bool maskout_irqs,
					  struct cam_ife_csid_ver2_hw *csid_hw)
{
	CAM_DBG(CAM_ISP, "CSID:%u Disable csi2 rx", csid_hw->hw_intf->hw_idx);
...
static int
cam_ife_csid_ver2_rx_err_top_half(uint32_t evt_id,
				  struct cam_irq_th_payload *th_payload)
{
	int32_t rc = 0;
	uint32_t status = 0;
	...
	if (csid_hw->rx_cfg.epd_supported) {
		if (status == IFE_CSID_VER2_RX_CPHY_EOT_RECEPTION) {
			CAM_DBG(CAM_ISP,
				"CSID[%u] Rcvd Only ERROR_EOT for EPD sensor",
				csid_hw->hw_intf->hw_idx);
			return -ENODEV;
		}
...
static int
cam_ife_csid_ver2_handle_rx_debug_event(struct cam_ife_csid_ver2_hw *csid_hw,
					...
					uint32_t *rst_strobe_val)
{
	...
	case IFE_CSID_VER2_RX_LONG_PKT_CAPTURED:
		...
		CAM_INFO(CAM_ISP, "CSID :%u Long pkt VC: %u DT: %u WC: %u",
			 csid_hw->hw_intf->hw_idx,
		...
		CAM_INFO(CAM_ISP, "CSID :%u Long pkt ECC: %u",
		...
		CAM_INFO(CAM_ISP,
			 ...
			 csid_hw->hw_intf->hw_idx,
			 ((val >> csi2_reg->calc_crc_shift) &
			  csi2_reg->calc_crc_mask),
	...
	case IFE_CSID_VER2_RX_SHORT_PKT_CAPTURED:
		...
		CAM_INFO(CAM_ISP, "CSID :%u Short pkt VC: %u DT: %u LC: %u",
			 csid_hw->hw_intf->hw_idx,
			 ((val & csi2_reg->vc_mask) >> csi2_reg->vc_shift),
			 ((val & csi2_reg->dt_mask) >> csi2_reg->dt_shift),
			 ((val & csi2_reg->wc_mask) >> csi2_reg->wc_shift));
		...
		CAM_INFO(CAM_ISP, "CSID :%u Short pkt ECC: %u",
			 csid_hw->hw_intf->hw_idx, val);
	...
	case IFE_CSID_VER2_RX_CPHY_PKT_HDR_CAPTURED:
		...
		CAM_INFO(CAM_ISP, "CSID :%u CPHY pkt VC: %u DT: %u WC: %u",
			 csid_hw->hw_intf->hw_idx,
			 ((val & csi2_reg->vc_mask) >> csi2_reg->vc_shift),
			 ((val & csi2_reg->dt_mask) >> csi2_reg->dt_shift),
			 ((val & csi2_reg->wc_mask) >> csi2_reg->wc_shift));
	...
	case IFE_CSID_VER2_RX_UNMAPPED_VC_DT:
		...
		CAM_ERR(CAM_ISP,
			"CSID:%u UNMAPPED_VC_DT: VC: %u DT: %u WC: %u not mapped to any "
			"csid paths",
			csid_hw->hw_intf->hw_idx,
			((val & csi2_reg->vc_mask) >> csi2_reg->vc_shift),
			((val & csi2_reg->dt_mask) >> csi2_reg->dt_shift),
		...
		*rst_strobe_val |=
			(1 << csi2_reg->unmapped_pkt_strobe_rst_shift);
		break;
	default:
		CAM_DBG(CAM_ISP, "CSID[%u] RX_IRQ: %s",
			csid_hw->hw_intf->hw_idx,
...
static int cam_ife_csid_ver2_rx_top_half(uint32_t evt_id,
					 struct cam_irq_th_payload *th_payload)
{
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	const struct cam_ife_csid_ver2_csi2_rx_reg_info *csi2_reg;
	...
	while (irq_status) {
		...
		if ((bit_set) && (BIT(bit_pos) & csid_hw->debug_info.rx_mask))
			cam_ife_csid_ver2_handle_rx_debug_event(
				csid_hw, CAM_IFE_CSID_RX_IRQ_STATUS_REG0,
...
{
	switch (core_sel) {
	case CAM_IFE_CSID_INPUT_CORE_SEL_SFE_0:
		return 0;
	case CAM_IFE_CSID_INPUT_CORE_SEL_SFE_1:
		return 1;
	case CAM_IFE_CSID_INPUT_CORE_SEL_SFE_2:
		return 2;
	/**
...
static int cam_ife_csid_ver2_handle_event_err(
	struct cam_ife_csid_ver2_hw *csid_hw, uint32_t irq_status,
	uint32_t err_type, bool is_secondary, struct cam_isp_resource_node *res)
{
	...
	if (!is_secondary) {
		if (res) {
			...
			CAM_ERR(CAM_ISP,
				...
				csid_hw->hw_intf->hw_idx, res->res_name,
				err_type, irq_status, path_cfg->error_ts.tv_sec,
				path_cfg->error_ts.tv_nsec);
		} else {
			CAM_ERR(CAM_ISP, "csid[%u] Rx Err: 0x%x status 0x%x",
				csid_hw->hw_intf->hw_idx, err_type, irq_status);
		}
	}

	evt.in_core_idx = cam_ife_csid_ver2_input_core_to_hw_idx(
...
static int cam_ife_csid_ver2_rx_err_bottom_half(void *handler_priv,
						void *evt_payload_priv)
{
	const struct cam_ife_csid_ver2_csi2_rx_reg_info *csi2_reg;
	...
	phy_idx = (int)(csid_hw->rx_cfg.phy_sel -
			csid_reg->cmn_reg->phy_sel_base_idx);
	...
	if (irq_status) {
		...
		for (i = 0; i < 4; i++) {
			...
			if (irq_status &
			    (IFE_CSID_VER2_RX_LANE0_FIFO_OVERFLOW << i)) {
				tmp_len += scnprintf(tmp_buf + tmp_len,
						     10 - tmp_len, " %d", i);
				lane_overflow = true;
			}
		}
		...
		if (lane_overflow) {
			...
			CAM_ERR_BUF(
				CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				&len,
				"INPUT_FIFO_OVERFLOW [Lanes:%s]: Skew/Less Data on lanes/ "
				"Slow csid clock:%luHz",
				tmp_buf,
				cam_soc_util_get_applied_src_clk(soc_info,
								 true));
		}
		...
		if (irq_status & IFE_CSID_VER2_RX_ERROR_CPHY_PH_CRC) {
			...
			CAM_ERR_BUF(CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				    &len, "CPHY_PH_CRC: Pkt Hdr CRC mismatch");
		}
		...
		if (irq_status & IFE_CSID_VER2_RX_ERROR_CRC) {
			...
			if (csid_hw->rx_cfg.lane_type ==
				...
				CAM_ERR_BUF(
					...
					"PHY_CRC_ERROR: Long pkt payload CRC mismatch. Totl CRC Errs: %u, "
					"Rcvd CRC: 0x%x Caltd CRC: 0x%x, VC:%d DT:%d WC:%d",
				...
				CAM_ERR_BUF(
					CAM_ISP, log_buf,
					CAM_IFE_CSID_LOG_BUF_LEN, &len,
					"PHY_CRC_ERROR: Long pkt payload CRC mismatch. Totl CRC "
					"Errs: %u, Rcvd CRC: 0x%x Caltd CRC: 0x%x",
					total_crc, long_pkt_ftr_val & 0xffff,
					long_pkt_ftr_val >> 16);
			}
		}
	...
	CAM_ERR_RATE_LIMIT(CAM_ISP, "CSID[%u] Rx Status 0x%x",
			   csid_hw->hw_intf->hw_idx, payload->irq_reg_val);
	...
	if ((!csid_hw->flags.reset_awaited) &&
		...
		cam_ife_csid_ver2_handle_event_err(csid_hw, rx_irq_status,
						   event_type, false, NULL);
...
void cam_ife_csid_hw_ver2_rdi_line_buffer_conflict_handler(void *csid)
{
	...
	for (i = CAM_IFE_PIX_PATH_RES_RDI_0; i < CAM_IFE_PIX_PATH_RES_RDI_4;
		...
		if (rdi_cfg & path_reg->pix_store_en_shift_val)
			CAM_ERR_BUF(CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				    &len, "LINE BUFFER ENABLED for RDI%d",
				    (i - CAM_IFE_PIX_PATH_RES_RDI_0));
	}

	if (len)
		CAM_ERR(CAM_ISP, "CSID[%u] %s", csid_hw->hw_intf->hw_idx,
			log_buf);
}
...
void cam_ife_csid_hw_ver2_drv_err_handler(void *csid)
{
	...
	void __iomem *base =
		soc_info->reg_map[CAM_IFE_CSID_CLC_MEM_BASE_ID].mem_base;
	uint32_t cfg0_val = 0, cfg1_val = 0, cfg2_val = 0, debug_drv_0_val = 0,
		 debug_drv_1_val = 0;
	...
	debug_drv_0_val =
		cam_io_r_mb(base + csid_reg->cmn_reg->debug_drv_0_addr);
	debug_drv_1_val =
		cam_io_r_mb(base + csid_reg->cmn_reg->debug_drv_1_addr);

	CAM_INFO(
		CAM_ISP,
		"CSID[%u] DRV cfg0:0x%x cfg1:0x%x cfg2:0x%x qtimer_val [start:end] "
		"[0x%x : 0x%x]",
		csid_hw->hw_intf->hw_idx, cfg0_val, cfg1_val, cfg2_val,
		debug_drv_0_val, debug_drv_1_val);
}

void cam_ife_csid_hw_ver2_mup_mismatch_handler(void *csid, void *resource)
{
	uint32_t idx = 0;
	...
	CAM_INFO(
		CAM_ISP,
		"CSID:%u Last MUP value 0x%x programmed for res [id: %d name: %s]",
		csid_hw->hw_intf->hw_idx, csid_hw->rx_cfg.mup, res->res_id,
		res->res_name);
	...
	if (cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_1].valid) {
		...
		if (path_cfg->ts_comb_vcdt_en) {
			...
			if (idx < CAM_IFE_CSID_MULTI_VC_DT_GRP_MAX)
				CAM_INFO(
					CAM_ISP,
					"CSID:%d Received frame with vc:%d on [id: %d name: %s] "
					"timestamp: %lld.%09lld",
					csid_hw->hw_intf->hw_idx,
					cid_data->vc_dt[idx].vc, res->res_id,
					res->res_name, current_ts.tv_sec,
					current_ts.tv_nsec);
			else
				CAM_ERR(CAM_ISP,
					"CSID:%d Get invalid vc index: %d on [id: %d name: %s] "
					"timestamp: %lld.%09lld",
...
void cam_ife_csid_ver2_print_illegal_programming_irq_status(void *csid,
							    void *resource)
{
	struct cam_ife_csid_ver2_hw *csid_hw = csid;
	struct cam_isp_resource_node *res = resource;
	...
	if (!path_cfg || (path_cfg->cid >= CAM_IFE_CSID_CID_MAX)) {
		CAM_ERR(CAM_ISP,
			"CSID:%u Invalid params: path_cfg: %pK, num_cids: %d",
			csid_hw->hw_intf->hw_idx, path_cfg,
			(path_cfg ? (path_cfg->cid) : -1));
		return;
	}
	...
	CAM_INFO(CAM_ISP, "cfg0 = %x cfg1 = %x vcdt_cfg0 = %x ", cfg0, cfg1,
		 vcdt_cfg0);

	if (cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_1].valid) {
		decode_fmt =
			((cfg0 >> csid_reg->cmn_reg->decode_format_shift_val) &
		...
		vc1 = ((vcdt_cfg0 >>
			csid_reg->cmn_reg->multi_vcdt_vc1_shift_val) &
		       csid_reg->cmn_reg->vc_mask);
		dt1 = ((vcdt_cfg0 >>
			csid_reg->cmn_reg->multi_vcdt_dt1_shift_val) &
		       csid_reg->cmn_reg->dt_mask);
	...
	CAM_INFO(CAM_ISP,
		 "CSID:%u Illegal Programming for res [id: %d name: %s]",
					 ...
					 struct cam_isp_resource_node *res)
{
	...
	val0 = cam_io_r_mb(mem_base + path_reg->debug_camif_0_addr);
	val1 = cam_io_r_mb(mem_base + path_reg->debug_camif_1_addr);
	val2 = cam_io_r_mb(mem_base + path_reg->debug_halt_status_addr);
	...
	CAM_INFO(
		CAM_ISP,
		"CSID:%u debug_camif_0: 0x%x debug_camif_1: 0x%x halt_status: 0x%x "
		"test_bus: %s test_bus_val: 0x%x for res: %s ",
		csid_hw->hw_intf->hw_idx, val0, val1, val2,
		CAM_BOOL_TO_YESNO(csid_hw->debug_info.test_bus_enabled), val3,
		res->res_name);
}

static int cam_ife_csid_ver2_parse_path_irq_status(
	struct cam_ife_csid_ver2_hw *csid_hw, struct cam_isp_resource_node *res,
	uint32_t index, uint32_t err_mask, uint32_t irq_status,
	struct cam_ife_csid_ver2_evt_payload *evt_payload)
{
	const uint8_t **irq_reg_tag;
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	while (status) {
		if ((csid_reg->path_irq_desc[bit_pos].bitmask != 0) &&
			...
			if (csid_reg->path_irq_desc[bit_pos].err_type)
				err_type |= csid_reg->path_irq_desc[bit_pos]
						    .err_type;
			if (csid_reg->path_irq_desc[bit_pos].err_handler)
				csid_reg->path_irq_desc[bit_pos].err_handler(
					csid_hw, res);
		}
		bit_pos++;
		status >>= 1;
...
static int cam_ife_csid_ver2_top_info_irq_bottom_half(void *handler_priv,
						      void *evt_payload_priv)
{
	struct cam_ife_csid_ver2_evt_payload *payload;
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	...
	if (!irq_status) {
		CAM_ERR(CAM_ISP, "CSID:%u Unexpected Scenario",
			csid_hw->hw_intf->hw_idx);
		return 0;
	}
	...
	if (irq_status & IFE_CSID_VER2_TOP_INFO_VOTE_UP) {
		...
		CAM_INFO(CAM_ISP,
			 "CSID:%u INFO_VOTE_UP timestamp:[%lld.%09lld]",
	...
	if (irq_status & IFE_CSID_VER2_TOP_INFO_VOTE_DN) {
		cam_cpas_log_votes(true);
		CAM_INFO(CAM_ISP,
			 "CSID:%u INFO_VOTE_DN timestamp:[%lld.%09lld]",
			 csid_hw->hw_intf->hw_idx, payload->timestamp.tv_sec,
			 payload->timestamp.tv_nsec);
	}
	...
	if (irq_status & IFE_CSID_VER2_TOP_ERR_NO_VOTE_DN) {
		CAM_INFO(CAM_ISP,
			 "CSID:%u ERR_NO_VOTE_DN timestamp:[%lld.%09lld]",
			 csid_hw->hw_intf->hw_idx, payload->timestamp.tv_sec,
			 payload->timestamp.tv_nsec);
...
static int cam_ife_csid_ver2_top_err_irq_bottom_half(void *handler_priv,
						     void *evt_payload_priv)
{
	struct cam_ife_csid_ver2_evt_payload *payload;
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	...
	irq_status =
		payload->irq_reg_val &
		csid_reg->cmn_reg
			->top_err_irq_mask[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0];
	...
	if (!csid_reg->num_top_err_irqs) {
		...
		cam_ife_csid_ver2_put_evt_payload(
			csid_hw, &payload, &csid_hw->path_free_payload_list,
			&csid_hw->path_payload_lock);
		return 0;
	}

	for (i = 0;
	     i < csid_reg->num_top_err_irqs[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0];
	     i++) {
		...
		if ((*csid_reg->top_irq_desc)[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0]
					     [i]
						     .bitmask &
		    irq_status) {
			CAM_ERR(CAM_ISP, "%s %s",
				(*csid_reg->top_irq_desc)
					[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0][i]
						.err_name,
				(*csid_reg->top_irq_desc)
					[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0][i]
						.desc);
	...
	if (event_type)
		cam_ife_csid_ver2_handle_event_err(csid_hw, irq_status,
						   event_type, false, NULL);
...
void cam_ife_csid_ver2_print_format_measure_info(void *csid, void *resource)
{
	struct cam_ife_csid_ver2_hw *csid_hw = csid;
	struct cam_isp_resource_node *res = resource;
	struct cam_ife_csid_ver2_reg_info *csid_reg =
	...
	format_measure_cfg0 =
		cam_io_r_mb(base + path_reg->format_measure_cfg0_addr);
...
static int cam_ife_csid_ver2_ipp_bottom_half(void *handler_priv,
					     void *evt_payload_priv)
{
	struct cam_ife_csid_ver2_evt_payload *payload;
	const struct cam_ife_csid_ver2_path_reg_info *path_reg;
	...
	CAM_DBG(CAM_ISP, "CSID[%u] multi_ctxt_en %d IPP status:0x%x",
		csid_hw->hw_intf->hw_idx, payload->is_mc, irq_status_ipp);
	...
	if (irq_status_ipp & eof_irq_mask) {
		cam_ife_csid_ver2_update_event_ts(&path_cfg->eof_ts,
						  &payload->timestamp);
		csid_hw->event_cb(csid_hw->token, CAM_ISP_HW_EVENT_EOF,
				  (void *)&evt_info);
	}

	if (irq_status_ipp & sof_irq_mask) {
		cam_ife_csid_ver2_update_event_ts(&path_cfg->sof_ts,
						  &payload->timestamp);
	...
	if (irq_status_ipp & epoch0_irq_mask) {
		cam_ife_csid_ver2_update_event_ts(&path_cfg->epoch_ts,
						  &payload->timestamp);
	...
	err_type = cam_ife_csid_ver2_parse_path_irq_status(
		csid_hw, res, CAM_IFE_CSID_IRQ_REG_IPP, err_mask,
...
static int cam_ife_csid_ver2_ppp_bottom_half(void *handler_priv,
					     void *evt_payload_priv)
{
	const struct cam_ife_csid_ver2_path_reg_info *path_reg = NULL;
	struct cam_ife_csid_ver2_evt_payload *payload;
	...
	err_type = cam_ife_csid_ver2_parse_path_irq_status(
		csid_hw, res, CAM_IFE_CSID_IRQ_REG_PPP, err_mask,
		irq_status_ppp, payload);

	if (err_type)
		cam_ife_csid_ver2_handle_event_err(csid_hw, irq_status_ppp,
						   err_type, false, res);
unlock:
	spin_unlock(&csid_hw->lock_state);
end:
...
static int cam_ife_csid_ver2_rdi_bottom_half(void *handler_priv,
					     void *evt_payload_priv)
{
	struct cam_ife_csid_ver2_evt_payload *payload;
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	...
	CAM_DBG(CAM_ISP, "CSID[%u] RDI:%d status:0x%x",
		csid_hw->hw_intf->hw_idx, res->res_id, irq_status_rdi);
	err_mask = rdi_reg->non_fatal_err_mask | rdi_reg->fatal_err_mask;
	...
	err_type = cam_ife_csid_ver2_parse_path_irq_status(
		csid_hw, res, path_cfg->irq_reg_idx, err_mask, irq_status_rdi,
		payload);
...
int cam_ife_csid_ver2_get_hw_caps(void *hw_priv, void *get_hw_cap_args,
				  uint32_t arg_size)
{
	int rc = 0;
	...
	hw_caps->sfe_ipp_input_rdi_res =
		csid_reg->cmn_reg->sfe_ipp_input_rdi_res;
...
static int
cam_ife_csid_ver2_wait_for_reset(struct cam_ife_csid_ver2_hw *csid_hw)
{
	...
	if (rem_jiffies == 0) {
		...
		if (val & csid_reg->cmn_reg->top_reset_irq_mask[0]) {
			...
			top_irq_val[0] = cam_io_r_mb(
				mem_base +
				csid_reg->cmn_reg->top_irq_status_addr[0]);
			top_irq_val[1] = cam_io_r_mb(
				mem_base +
				csid_reg->cmn_reg->top_irq_mask_addr[0]);
			top_irq_val[2] = cam_io_r_mb(
				mem_base +
				csid_reg->cmn_reg->top_irq_clear_addr[0]);
			top_irq_val[3] = cam_io_r_mb(
				mem_base +
				csid_reg->cmn_reg->top_irq_set_addr[0]);
			csid_cfg0 = cam_io_r_mb(mem_base +
						csid_reg->cmn_reg->cfg0_addr);

			reset_cfg = cam_io_r_mb(
				mem_base + csid_reg->cmn_reg->reset_cfg_addr);
			rx_cfg[0] = cam_io_r_mb(mem_base +
						csid_reg->csi2_reg->cfg0_addr);
			rx_cfg[1] = cam_io_r_mb(mem_base +
			...
			CAM_INFO(
				...
				csid_hw->hw_intf->hw_idx, csid_cfg0, reset_cfg,
				rx_cfg[0], rx_cfg[1]);
			...
			CAM_INFO(
				CAM_ISP,
				"CSID[%d] clk lvl %u received clk_rate %u applied clk_rate "
				"sw_client:%lu hw_client:[%lu %lu]",
				csid_hw->hw_intf->hw_idx, clk_lvl,
				csid_hw->clk_rate,
				soc_info->applied_src_clk_rates.sw_client,
				soc_info->applied_src_clk_rates
					.hw_client[csid_hw->hw_intf->hw_idx]
					.high,
				soc_info->applied_src_clk_rates
					.hw_client[csid_hw->hw_intf->hw_idx]
					.low);
		...
		CAM_DBG(CAM_ISP, "CSID[%u], sync-mode[%d] reset success",
			csid_hw->hw_intf->hw_idx, csid_hw->sync_mode);
	}
...
static int
cam_ife_csid_ver2_reset_irq_top_half(uint32_t evt_id,
				     struct cam_irq_th_payload *th_payload)
{
	struct cam_ife_csid_ver2_hw *csid_hw;
	...
	CAM_DBG(CAM_ISP, "CSID[%u] TOP_IRQ_STATUS_0 = 0x%x",
		csid_hw->hw_intf->hw_idx, th_payload->evt_status_arr[0]);
...
static int
cam_ife_csid_ver2_internal_reset(struct cam_ife_csid_ver2_hw *csid_hw,
				 bool power_on_rst, uint32_t rst_cmd,
				 uint32_t rst_location, uint32_t rst_mode)
{
	uint32_t val = 0;
	struct cam_ife_csid_ver2_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
...
wait_only:
	...
	if (rc)
		CAM_ERR(CAM_ISP, "CSID[%u] Reset failed mode %d cmd %d loc %d",
			csid_hw->hw_intf->hw_idx, rst_mode, rst_cmd,
			rst_location);
	reinit_completion(&csid_hw->hw_info->hw_complete);
	return rc;
}

int cam_ife_csid_ver2_reset(void *hw_priv, void *reset_args, uint32_t arg_size)
{
	struct cam_hw_info *hw_info;
	struct cam_ife_csid_ver2_hw *csid_hw;
...
static inline void
cam_ife_csid_ver2_maskout_path_irqs(int32_t res_id,
				    struct cam_ife_csid_ver2_hw *csid_hw,
				    struct cam_ife_csid_ver2_path_cfg *path_cfg)
{
	int rc;
...
static inline void cam_ife_csid_ver2_disable_path_irqs_evts(
	int32_t res_id, struct cam_ife_csid_ver2_hw *csid_hw,
	struct cam_ife_csid_ver2_path_cfg *path_cfg)
{
	int rc;
...
static int cam_ife_csid_ver2_disable_path(bool maskout_irqs,
					  struct cam_ife_csid_ver2_hw *csid_hw,
					  struct cam_isp_resource_node *res)
{
	struct cam_ife_csid_ver2_path_cfg *path_cfg;
	...
	if (maskout_irqs)
		cam_ife_csid_ver2_maskout_path_irqs(res->res_id, csid_hw,
						    path_cfg);
	else
		cam_ife_csid_ver2_disable_path_irqs_evts(res->res_id, csid_hw,
							 path_cfg);
...
static int
cam_ife_csid_ver2_decode_format1_validate(struct cam_ife_csid_ver2_hw *csid_hw,
					  struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver2_reg_info *csid_reg =
	...
	if ((path_cfg->path_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0].decode_fmt ==
	    ...
	    (path_cfg->path_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_1].decode_fmt ==
	     csid_reg->cmn_reg->decode_format_payload_only)) {
		if (path_cfg->path_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
			    .decode_fmt !=
		...
		if (path_cfg->path_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
			...
			CAM_ERR(CAM_ISP,
				...
				path_cfg->path_format
					[CAM_IFE_CSID_MULTI_VC_DT_GRP_1]
						.decode_fmt,
				cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
					.vc,
				cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_1]
					.vc,
				cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
					.dt,
				cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_1]
...
err:
	CAM_ERR(CAM_ISP,
		"Invalid decode fmt1 cfg CSID[%u] res [id %d name %s] rc %d",
...
static bool cam_ife_csid_hw_ver2_need_unpack_mipi(
	...
	const struct cam_ife_csid_ver2_path_reg_info *path_reg, uint32_t format)
{
	...
	case CAM_FORMAT_MIPI_RAW_14:
		/*
     * CAM_FORMAT_PLAIN16_16 : can be removed? double check why
     * default_out_format has it. default_out_format is used in xCFA usecases
     * without real RDI0 out buffer. We still need to set need_unpack here so
     * that we unpack incoming data (say MIPI10) into MSB. If default_out_format
     * can be set to 16_10/16_12/16_14 - then we can remove
     */
	case CAM_FORMAT_PLAIN16_16:
		need_unpack = (bool)(path_reg->capabilities &
...
static int cam_ife_csid_hw_ver2_config_path_data(
	struct cam_ife_csid_ver2_hw *csid_hw,
	struct cam_ife_csid_ver2_path_cfg *path_cfg,
	struct cam_csid_hw_reserve_resource_args *reserve, uint32_t cid)
{
	int rc = 0, i = 0;
	const struct cam_ife_csid_ver2_reg_info *csid_reg =
		(struct cam_ife_csid_ver2_reg_info *)
	...
	if (reserve->sync_mode == CAM_ISP_HW_SYNC_MASTER) {
		...
		if (reserve->res_id >= CAM_IFE_PIX_PATH_RES_RDI_0 &&
			...
			path_cfg->width =
				path_cfg->end_pixel - path_cfg->start_pixel + 1;
		}
		CAM_DBG(CAM_ISP,
	...
	if (csid_reg->cmn_reg->decode_format1_supported &&
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"CSID[%u] res %d decode fmt1 validation failed",
	...
	struct cam_csid_hw_reserve_resource_args *reserve)
{
   ...
   */
	...
	csid_hw->rx_cfg.lane_cfg = reserve->in_port->lane_cfg;
	csid_hw->rx_cfg.lane_type = reserve->in_port->lane_type;
	csid_hw->rx_cfg.lane_num = reserve->in_port->lane_num;
	...
	default:
		csid_hw->rx_cfg.tpg_mux_sel = 0;
		csid_hw->rx_cfg.phy_sel = (reserve->in_port->res_type & 0xFF);
		break;
	}

	csid_hw->counters.csi2_reserve_cnt++;
	CAM_DBG(CAM_ISP, "CSID:%u Rx lane param: cfg:%u type:%u num:%u res:%u",
		csid_hw->hw_intf->hw_idx, reserve->in_port->lane_cfg,
		reserve->in_port->lane_type, reserve->in_port->lane_num,
...
static int
cam_ife_csid_ver_config_camif(struct cam_ife_csid_ver2_hw *csid_hw,
			      struct cam_csid_hw_reserve_resource_args *reserve,
			      struct cam_ife_csid_ver2_path_cfg *path_cfg)
{
	struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	struct cam_csid_hw_reserve_resource_args *reserve, uint32_t cid)
{
	...
	rc = cam_ife_csid_hw_ver2_config_path_data(csid_hw, path_cfg, reserve,
						   cid);
	if (rc) {
	...
	if (csid_hw->counters.csi2_reserve_cnt) {
		...
		if (csid_hw->res_type != reserve->in_port->res_type) {
			CAM_ERR(CAM_ISP,
				...
				csid_hw->hw_intf->hw_idx, csid_hw->res_type,
				reserve->in_port->res_type);
			rc = -EINVAL;
			goto err;
		}

		if (csid_hw->rx_cfg.lane_cfg != reserve->in_port->lane_cfg ||
		    csid_hw->rx_cfg.lane_type != reserve->in_port->lane_type ||
		    csid_hw->rx_cfg.lane_num != reserve->in_port->lane_num) {
			CAM_ERR(CAM_ISP,
...
int cam_ife_csid_ver2_reserve(void *hw_priv, void *reserve_args,
			      uint32_t arg_size)
{
	struct cam_ife_csid_ver2_hw *csid_hw;
	struct cam_hw_info *hw_info;
	...
	if (res->res_state != CAM_ISP_RESOURCE_STATE_AVAILABLE) {
		/**
     * intentionally set as DBG log to since this log gets printed when hw
     * manager checks if resource is available
     */
	...
	path_cfg = (struct cam_ife_csid_ver2_path_cfg *)res->res_priv;
	if (!path_cfg) {
		CAM_ERR(CAM_ISP, "CSID %d Unallocated Res_id %d state %d",
			csid_hw->hw_intf->hw_idx, reserve->res_id,
			res->res_state);
		return -EINVAL;
	}
	...
	if (!reserve->is_offline) {
		rc = cam_ife_csid_hw_ver2_hw_cfg(csid_hw, path_cfg, reserve,
						 cid);
		if (rc) {
...
int cam_ife_csid_ver2_release(void *hw_priv, void *release_args,
			      uint32_t arg_size)
{
	struct cam_ife_csid_ver2_hw *csid_hw;
	struct cam_hw_info *hw_info;
	...
	if ((res->res_state <= CAM_ISP_RESOURCE_STATE_AVAILABLE) ||
		...
		CAM_WARN(CAM_ISP, "CSID:%u res type:%d Res %d in state %d",
			 csid_hw->hw_intf->hw_idx, res->res_type, res->res_id,
			 res->res_state);
		goto end;
	}
...
static int
cam_ife_csid_ver2_res_master_slave_cfg(struct cam_ife_csid_ver2_hw *csid_hw,
				       uint32_t res_id)
{
	struct cam_ife_csid_ver2_reg_info *csid_reg;
	uint32_t val;
	...
	cam_io_w_mb(val,
		    mem_base + csid_reg->cmn_reg->shdr_master_slave_cfg_addr);
...
static int
cam_ife_csid_ver2_init_config_rdi_path(struct cam_ife_csid_ver2_hw *csid_hw,
				       struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	if (csid_hw->flags.offline_mode) {
		val = (1 << path_reg->offline_mode_en_shift_val);
   ...
   */
	...
	CAM_DBG(CAM_ISP, "CSID[%u] %s cfg0_addr 0x%x", csid_hw->hw_intf->hw_idx,
		res->res_name, cfg0);
   ...
   */
	...
	if (path_reg->mipi_pack_supported)
		cfg1 |= path_cfg->path_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
				.packing_fmt
	...
	if ((path_reg->capabilities & CAM_IFE_CSID_CAP_LINE_SMOOTHING_IN_RDI) &&
	    (res->res_id == CAM_IFE_PIX_PATH_RES_RDI_0))
		cfg1 |= 1 << path_reg->pix_store_en_shift_val;

	cam_io_w_mb(cfg1, mem_base + path_reg->cfg1_addr);

	CAM_DBG(CAM_ISP, "CSID:%u RDI:%u cfg1:0x%x", csid_hw->hw_intf->hw_idx,
		res->res_id, cfg1);
	...
	if (path_cfg->sfe_shdr || (csid_hw->flags.rdi_lcr_en &&
				   res->res_id == CAM_IFE_PIX_PATH_RES_RDI_0))
	...
	if (csid_hw->debug_info.debug_val &
		...
		cam_io_w_mb(val, mem_base + path_reg->format_measure_cfg0_addr);
	}
...
static int
cam_ife_csid_ver2_init_config_pxl_path(struct cam_ife_csid_ver2_hw *csid_hw,
				       struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	CAM_IRQ_HANDLER_BOTTOM_HALF bottom_half_handler)
{
	...
	path_cfg->discard_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->path_irq_controller[res->res_id], CAM_IRQ_PRIORITY_0,
		&irq_mask, res, top_half_handler, bottom_half_handler,
		csid_hw->tasklet, &tasklet_bh_api, CAM_IRQ_EVT_GROUP_0);
...
static int
cam_ife_csid_ver2_mc_irq_subscribe(struct cam_ife_csid_ver2_hw *csid_hw,
				   struct cam_isp_resource_node *res,
				   uint32_t top_index)
{
	uint32_t tmp_mask;
	...
	csid_hw->top_mc_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->top_irq_controller[top_index], CAM_IRQ_PRIORITY_0,
		&tmp_mask, res, cam_ife_csid_ver2_mc_top_half,
		cam_ife_csid_ver2_get_path_bh(res->res_id), csid_hw->tasklet,
...
static int
cam_ife_csid_ver2_path_irq_subscribe(struct cam_ife_csid_ver2_hw *csid_hw,
				     struct cam_isp_resource_node *res,
				     uint32_t irq_mask, uint32_t err_irq_mask)
{
	...
	for (i = CAM_IFE_CSID_TOP_IRQ_STATUS_REG0; i < csid_reg->num_top_regs;
	     i++) {
	...
	if (csid_reg->path_reg[res->res_id]->capabilities &
		...
		rc = cam_ife_csid_ver2_mc_irq_subscribe(csid_hw, res,
							top_index);
	...
	path_cfg->top_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->top_irq_controller[top_index], CAM_IRQ_PRIORITY_0,
		&top_irq_mask, res, cam_ife_csid_ver2_handle_path_irq, NULL,
	...
	path_cfg->irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->path_irq_controller[res->res_id], CAM_IRQ_PRIORITY_1,
		&irq_mask, res, cam_ife_csid_ver2_path_top_half,
		cam_ife_csid_ver2_get_path_bh(res->res_id), csid_hw->tasklet,
		&tasklet_bh_api, CAM_IRQ_EVT_GROUP_0);
	...
	path_cfg->err_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->path_irq_controller[res->res_id], CAM_IRQ_PRIORITY_0,
		&err_irq_mask, res, cam_ife_csid_ver2_path_err_top_half,
		cam_ife_csid_ver2_get_path_bh(res->res_id), csid_hw->tasklet,
		&tasklet_bh_api, CAM_IRQ_EVT_GROUP_0);
	...
	struct cam_ife_csid_ver2_rup_aup_mask *rup_aup_mask)
{
	...
	rc = cam_ife_csid_ver2_init_config_rdi_path(csid_hw, res);

	if (rc) {
		CAM_ERR(CAM_ISP, "CSID:%u %s path res type:%d res_id:%d %d",
			csid_hw->hw_intf->hw_idx, res->res_type, res->res_id,
			res->res_state);
		return rc;
	}
	...
	if (path_cfg->sec_evt_config.en_secondary_evt) {
		...
		CAM_DBG(CAM_ISP, "CSID:%u Enable camif: %d evt irq for res: %s",
			csid_hw->hw_intf->hw_idx,
	...
	struct cam_ife_csid_ver2_rup_aup_mask *rup_aup_mask)
{
	...
	rc = cam_ife_csid_ver2_init_config_pxl_path(csid_hw, res);
	...
	CAM_DBG(CAM_ISP, "CSID[%u] frame_cfg 0x%x epoch_cfg 0x%x",
		csid_hw->hw_intf->hw_idx, val, path_cfg->epoch_cfg);

	path_cfg->irq_reg_idx = cam_ife_csid_get_rt_irq_idx(
		CAM_IFE_CSID_IRQ_REG_IPP, csid_reg->cmn_reg->num_pix,
	...
	if (path_cfg->sync_mode == CAM_ISP_HW_SYNC_MASTER) {
		...
		val |= path_reg->start_mode_slave << path_reg->start_mode_shift;
	} else {
...
static int cam_ife_csid_ver2_enable_path(struct cam_ife_csid_ver2_hw *csid_hw,
					 struct cam_isp_resource_node *res)
{
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
	...
	if (csid_hw->flags.rdi_lcr_en && !csid_hw->top_cfg.dual_en)
		val |= path_reg->start_mode_global
		       << path_reg->start_mode_shift;
	else
	...
	struct cam_ife_csid_ver2_rup_aup_mask *rup_aup_mask)
{
	...
	path_cfg->irq_reg_idx = cam_ife_csid_get_rt_irq_idx(
		CAM_IFE_CSID_IRQ_REG_PPP, csid_reg->cmn_reg->num_pix,
		csid_reg->cmn_reg->num_ppp, csid_reg->cmn_reg->num_rdis);
...
static int
cam_ife_csid_ver2_rx_capture_config(struct cam_ife_csid_ver2_hw *csid_hw)
{
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
	...
	if (csid_hw->debug_info.rx_capture_debug_set) {
		...
		vc = csid_hw->cid_data[i]
			     .vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
			     .vc;
		dt = csid_hw->cid_data[i]
			     .vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]
			     .dt;
	}
	...
	CAM_DBG(CAM_ISP, "CSID[%u] rx capture_ctrl: 0x%x",
		csid_hw->hw_intf->hw_idx, val);
...
static int
cam_ife_csid_ver2_csi2_irq_subscribe(struct cam_ife_csid_ver2_hw *csid_hw,
				     uint32_t irq_mask, uint32_t err_irq_mask)
{
	struct cam_ife_csid_ver2_reg_info *csid_reg =
		csid_hw->core_info->csid_reg;
	...
	if (top_index < 0) {
		CAM_ERR(CAM_ISP,
			"CSID[%d] RX Subscribe Top Irq fail due to invalid mask",
			csid_hw->hw_intf->hw_idx);
		rc = -EINVAL;
		return rc;
	}
	...
	csid_hw->rx_cfg.top_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->top_irq_controller[top_index], CAM_IRQ_PRIORITY_0,
		&top_irq_mask, csid_hw, cam_ife_csid_ver2_handle_rx_irq, NULL,
		NULL, NULL, CAM_IRQ_EVT_GROUP_0);
	...
	if (irq_mask) {
		csid_hw->rx_cfg.irq_handle = cam_irq_controller_subscribe_irq(
			csid_hw->rx_irq_controller
				[CAM_IFE_CSID_RX_IRQ_STATUS_REG0],
			CAM_IRQ_PRIORITY_4, &irq_mask, csid_hw,
			cam_ife_csid_ver2_rx_top_half, NULL, NULL, NULL,
			CAM_IRQ_EVT_GROUP_0);
	...
	csid_hw->rx_cfg.err_irq_handle[CAM_IFE_CSID_RX_IRQ_STATUS_REG0] =
		cam_irq_controller_subscribe_irq(
			csid_hw->rx_irq_controller
				[CAM_IFE_CSID_RX_IRQ_STATUS_REG0],
			CAM_IRQ_PRIORITY_0, &err_irq_mask, csid_hw,
			cam_ife_csid_ver2_rx_err_top_half,
			cam_ife_csid_ver2_rx_err_bottom_half, csid_hw->tasklet,
			&tasklet_bh_api, CAM_IRQ_EVT_GROUP_0);
...
static int cam_ife_csid_ver2_enable_csi2(struct cam_ife_csid_ver2_hw *csid_hw)
{
	...
	CAM_DBG(CAM_ISP, "CSID[%u] rx_cfg0: 0x%x", csid_hw->hw_intf->hw_idx,
		val);
	...
	if (vc_full_width == 1) {
		...
		CAM_ERR(CAM_ISP, "CSID[%u] Error VC DT",
			csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}

	cam_io_w_mb(val, mem_base + csi2_reg->cfg1_addr);
	CAM_DBG(CAM_ISP, "CSID[%u] rx_cfg1: 0x%x", csid_hw->hw_intf->hw_idx,
		val);
	...
	rc = cam_ife_csid_ver2_csi2_irq_subscribe(
		csid_hw, csid_hw->debug_info.rx_mask, val);
	if (rc)
		return rc;
...
static int cam_ife_csid_ver2_program_top(struct cam_ife_csid_ver2_hw *csid_hw)
{
	...
	input_core_sel =
		csid_reg->input_core_sel[csid_hw->hw_intf->hw_idx]
					[csid_hw->top_cfg.input_core_type];

	CAM_DBG(CAM_ISP, "CSID[%u] input_core_sel %d", csid_hw->hw_intf->hw_idx,
		input_core_sel);
	...
	val |= csid_hw->top_cfg.out_ife_en << top_reg->out_ife_en_shift_val;
	...
	val |= csid_hw->top_cfg.dual_en << top_reg->dual_en_shift_val;
...
static int
cam_ife_csid_ver2_set_cesta_clk_rate(struct cam_ife_csid_ver2_hw *csid_hw,
				     bool force_channel_switch,
				     const char *identifier)
{
	...
	CAM_DBG(CAM_ISP,
		...
		csid_hw->hw_intf->hw_idx, csid_hw->clk_rate, channel_switch,
		identifier);

	if (csid_hw->clk_rate) {
		rc = cam_soc_util_set_src_clk_rate(
			soc_info, csid_hw->hw_intf->hw_idx, csid_hw->clk_rate,
			soc_info->clk_rate[lowest_clk_lvl]
					  [soc_info->src_clk_idx]);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Failed in setting cesta clk rates[high low]:[%ld %ld] "
				"client_idx:%d rc:%d",
				csid_hw->clk_rate,
				soc_info->clk_rate[lowest_clk_lvl]
						  [soc_info->src_clk_idx],
	...
	if (channel_switch) {
		rc = cam_soc_util_cesta_channel_switch(csid_hw->hw_intf->hw_idx,
						       identifier);
		if (rc) {
			CAM_ERR(CAM_CSIPHY,
...
static int cam_ife_csid_ver2_enable_core(struct cam_ife_csid_ver2_hw *csid_hw)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "CSID[%u] get clk level fail rate %u",
			csid_hw->clk_rate);
	}
	...
	CAM_DBG(CAM_ISP, "csid %d is_clk_drv_en %d is_drv_config_en %d",
		csid_hw->hw_intf->hw_idx, soc_info->is_clk_drv_en,
		csid_hw->is_drv_config_en);

	/*
   * cam_ife_csid_enable_soc_resources sets clk_lvl for high, low values, if the
   * usecase enables drv config, i.e enabling csid DRV events, overwrite low
   * value with LowSVS to get power benefit.
   */
	if (soc_info->is_clk_drv_en && csid_hw->is_drv_config_en) {
		rc = cam_ife_csid_ver2_set_cesta_clk_rate(csid_hw, false,
							  "csid_init");
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Failed in setting cesta clk rates, client_idx:%d rc:%d",
				csid_hw->hw_intf->hw_idx, rc);
			return rc;
		}
	}
	...
	irq_mask =
		csid_reg->cmn_reg
			->top_reset_irq_mask[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0];

	csid_hw->reset_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->top_irq_controller[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0],
		CAM_IRQ_PRIORITY_0, &irq_mask, csid_hw,
		cam_ife_csid_ver2_reset_irq_top_half, NULL, NULL, NULL,
		CAM_IRQ_EVT_GROUP_0);
...
static int cam_ife_csid_ver2_enable_hw(struct cam_ife_csid_ver2_hw *csid_hw)
{
	struct cam_hw_soc_info *soc_info;
	...
	csid_hw->buf_done_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->top_irq_controller[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0],
		CAM_IRQ_PRIORITY_4, &buf_done_irq_mask, csid_hw,
		cam_ife_csid_ver2_handle_buf_done_irq, NULL, NULL, NULL,
		CAM_IRQ_EVT_GROUP_0);
	...
	csid_hw->top_err_irq_handle[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0] =
		cam_irq_controller_subscribe_irq(
			csid_hw->top_irq_controller
				[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0],
			CAM_IRQ_PRIORITY_0, &top_err_irq_mask, csid_hw,
	...
	if (csid_hw->debug_info.top_mask) {
		...
		csid_hw->top_info_irq_handle[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0] =
			cam_irq_controller_subscribe_irq(
				csid_hw->top_irq_controller
					[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0],
				CAM_IRQ_PRIORITY_1, &top_info_irq_mask, csid_hw,
				cam_ife_csid_ver2_top_info_irq_top_half,
				cam_ife_csid_ver2_top_info_irq_bottom_half,
				csid_hw->tasklet, &tasklet_bh_api,
				CAM_IRQ_EVT_GROUP_0);

		if (csid_hw->top_info_irq_handle
			    [CAM_IFE_CSID_TOP_IRQ_STATUS_REG0] < 1) {
...
int cam_ife_csid_ver2_init_hw(void *hw_priv, void *init_args, uint32_t arg_size)
{
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	struct cam_hw_info *hw_info;
	int rc = 0;
	...
	CAM_DBG(CAM_ISP, "CSID[%u] init hw", csid_hw->hw_intf->hw_idx);
end:
	mutex_unlock(&csid_hw->hw_info->hw_mutex);
	return rc;
}

static int cam_ife_csid_ver2_disable_core(struct cam_ife_csid_ver2_hw *csid_hw)
{
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	if (!csid_hw->hw_info->open_count) {
		CAM_WARN(CAM_ISP, "CSID[%u] Unbalanced disable_hw",
			 csid_hw->hw_intf->hw_idx);
		return rc;
	}
   ...
   */
	if ((top_reg) && (!soc_private->is_ife_csid_lite)) {
		cam_io_w_mb(
			top_reg->io_path_cfg_rst_val,
			soc_info->reg_map[CAM_IFE_CSID_TOP_MEM_BASE_ID].mem_base +
				top_reg->io_path_cfg0_addr[csid_hw->hw_intf
								   ->hw_idx]);

		cam_io_w_mb(
			top_reg->dual_cfg_rst_val,
			soc_info->reg_map[CAM_IFE_CSID_TOP_MEM_BASE_ID].mem_base +
				top_reg->dual_csid_cfg0_addr[csid_hw->hw_intf
								     ->hw_idx]);
	}
...
int cam_ife_csid_ver2_deinit_hw(void *hw_priv, void *deinit_args,
				uint32_t arg_size)
{
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	struct cam_isp_resource_node *res;
...
disable_hw:
	...
	mutex_unlock(&csid_hw->hw_info->hw_mutex);
	CAM_DBG(CAM_ISP, "De-Init CSID %d Path: %d", csid_hw->hw_intf->hw_idx,
		res->res_id);
...
static void
cam_ife_csid_ver2_testbus_config(struct cam_ife_csid_ver2_hw *csid_hw,
				 uint32_t val)
{
	struct cam_hw_soc_info *soc_info = &csid_hw->hw_info->soc_info;
	struct cam_ife_csid_ver2_reg_info *csid_reg =
	...
	CAM_DBG(CAM_ISP, "CSID [%u] test_bus_ctrl: 0x%x",
		csid_hw->hw_intf->hw_idx, val);
}

static void
cam_ife_csid_ver2_send_secure_info(struct cam_csid_hw_start_args *start_args,
				   struct cam_ife_csid_ver2_hw *csid_hw)
{
	...
	CAM_DBG(CAM_ISP,
		"PHY secure info for CSID[%u], lane_cfg: 0x%x, ife: 0x%x, cdm: 0x%x, "
		"vc_mask: 0x%llx",
		csid_hw->hw_intf->hw_idx, secure_info.lane_cfg,
		secure_info.csid_hw_idx_mask, secure_info.cdm_hw_idx_mask,
		secure_info.vc_mask);

	/* Issue one call to PHY for dual IFE cases */
	phy_sel = (int)(csid_hw->rx_cfg.phy_sel -
			csid_reg->cmn_reg->phy_sel_base_idx);
	if (phy_sel < 0) {
		CAM_WARN(CAM_ISP,
			 "Can't notify csiphy, incorrect phy selected=%d",
...
int cam_ife_csid_ver2_start(void *hw_priv, void *args, uint32_t arg_size)
{
	struct cam_ife_csid_ver2_hw *csid_hw = NULL;
	struct cam_isp_resource_node *res;
	...
	CAM_DBG(CAM_ISP,
		"csid %d is_drv_config_en %d start_only %d is_internal_start %d, "
		"clk_rate=%lld",
		csid_hw->hw_intf->hw_idx, csid_hw->is_drv_config_en,
		start_args->start_only, start_args->is_internal_start,
		csid_hw->clk_rate);
	...
	if (start_args->start_only) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"CSID:%u Failed to process resume rc: %d",
				csid_hw->hw_intf->hw_idx, rc);
			goto end;
		}
	}
     ...
     *    csid layer, so we set high, low here will overwrite previously set
     * high, low
     * 2. We trigger a channel switch so that we go to high clocks for all
     * including csiphy before calling CSIPHY RESUME, so that we remove sw
     * client vote from csiphy device
     */
		rc = cam_ife_csid_ver2_set_cesta_clk_rate(csid_hw, true,
							  "csid_start");
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Failed in setting cesta clk rates, client_idx:%d rc:%d",
   ...
   */
	if ((csid_hw->sync_mode != CAM_ISP_HW_SYNC_SLAVE) &&
	    start_args->is_secure && csid_hw->flags.domain_id_security)
   ...
   */
	if (csid_reg->cmn_reg->capabilities & CAM_IFE_CSID_CAP_SPLIT_RUP_AUP) {
		...
		rup_aup_mask.rup_mask |= (csid_hw->rx_cfg.mup
					  << csid_reg->cmn_reg->mup_shift_val);
	}

	if (!start_args->is_internal_start) {
		if (csid_reg->cmn_reg->capabilities &
		    CAM_IFE_CSID_CAP_SPLIT_RUP_AUP) {
			cam_io_w_mb(rup_aup_mask.rup_mask,
				    csid_clc_membase +
					    csid_reg->cmn_reg->rup_cmd_addr);
			cam_io_w_mb(rup_aup_mask.aup_mask,
				    csid_clc_membase +
					    csid_reg->cmn_reg->aup_cmd_addr);
			cam_io_w_mb(
				rup_aup_mask.rup_aup_set_mask,
				csid_clc_membase +
	...
	CAM_DBG(CAM_ISP,
		"CSID:%u RUP:0x%x AUP: 0x%x MUP:0x%x at start updated: %s",
		csid_hw->hw_intf->hw_idx, rup_aup_mask.rup_mask,
		rup_aup_mask.aup_mask, rup_aup_mask.rup_aup_set_mask,
	...
	if (csid_hw->sync_mode != CAM_ISP_HW_SYNC_SLAVE) {
		...
		/*
     * send this even when cesta is not enabled - to support debugfs to switch
     * between cesta and non-cesta between runs. CSIPHY might have stale
     * information if we do not send this info everytime
     */
		cam_subdev_notify_message(CAM_CSIPHY_DEVICE_TYPE,
	...
	for (i = 0; i < start_args->num_res; i++) {
		...
		CAM_DBG(CAM_ISP, "CSID[%u] res_type :%d res_id:%d enabled",
			csid_hw->hw_intf->hw_idx, res->res_type, res->res_id);
	}
	...
	if (delay_rdi0_enable) {
		...
		cam_ife_csid_ver2_enable_path(csid_hw, res);
		CAM_DBG(CAM_ISP,
			"CSID[%u] Enabling RDI0 after all paths for LCR-PD cases",
			csid_hw->hw_intf->hw_idx);
	}

	if (csid_hw->debug_info.test_bus_val) {
		cam_ife_csid_ver2_testbus_config(
			csid_hw, csid_hw->debug_info.test_bus_val);
	...
	if (csid_hw->flags.rdi_lcr_en && !csid_hw->top_cfg.dual_en) {
		cam_io_w_mb(1, soc_info->reg_map[CAM_IFE_CSID_CLC_MEM_BASE_ID]
					       .mem_base +
...
static void
cam_ife_csid_ver2_maskout_all_irqs(struct cam_ife_csid_ver2_hw *csid_hw,
				   struct cam_csid_hw_stop_args *csid_stop)
{
	...
	/* Disable buf done */
	cam_io_w_mb(0x0, mem_base + csid_reg->cmn_reg->buf_done_irq_mask_addr);
...
int cam_ife_csid_ver2_stop(void *hw_priv, void *stop_args, uint32_t arg_size)
{
	...
	if (!csid_stop->num_res) {
		CAM_ERR(CAM_ISP, "CSID:%u Invalid args",
			csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}
	...
	if (csid_hw->hw_info->soc_info.is_clk_drv_en &&
	    csid_hw->is_drv_config_en) {
...
int cam_ife_csid_ver2_read(void *hw_priv, void *read_args, uint32_t arg_size)
{
...
int cam_ife_csid_ver2_write(void *hw_priv, void *write_args, uint32_t arg_size)
{
	CAM_ERR(CAM_ISP, "CSID: un supported");
	return -EINVAL;
}

static int cam_ife_csid_ver2_top_cfg(struct cam_ife_csid_ver2_hw *csid_hw,
				     void *cmd_args)
{
	...
	case CAM_IFE_CSID_INPUT_CORE_SFE_IFE:
		...
		if (top_args->core_idx == 0) {
			...
			rc = -EINVAL;
			CAM_ERR(CAM_ISP, "CSID: %u Invalid SFE node %d", hw_idx,
				top_args->core_idx);
		}
	...
	case CAM_IFE_CSID_INPUT_CORE_CUST_IFE:
		...
		if (top_args->core_idx == 0) {
			...
			rc = -EINVAL;
			CAM_ERR(CAM_ISP, "CSID: %u Invalid Cust node %d",
				hw_idx, top_args->core_idx);
		}
		break;
	default:
	...
	CAM_DBG(CAM_ISP,
		...
		csid_hw->top_cfg.out_ife_en, csid_hw->top_cfg.offline_sfe_en,
...
static int cam_ife_csid_ver2_get_mc_reg_val_pair(
	struct cam_ife_csid_ver2_hw *csid_hw, uint32_t *reg_val_pair,
	struct cam_isp_csid_reg_update_args *rup_args)
{
   ...
   */
	...
	CAM_DBG(CAM_ISP,
		"CSID[%d] configure rup: 0x%x, offset: 0x%x, aup: 0x%x, offset: 0x%x",
...
static int cam_ife_csid_ver2_reg_update(struct cam_ife_csid_ver2_hw *csid_hw,
					void *cmd_args, uint32_t arg_size)
{
	...
	if (!cdm_util_ops) {
		CAM_ERR(CAM_ISP, "CSID[%u] Invalid CDM ops",
			csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}

	num_reg_val_pairs = (csid_reg->cmn_reg->capabilities &
			     CAM_IFE_CSID_CAP_SPLIT_RUP_AUP) ?
				    3 :
				    1;
	...
	if ((!rup_args->reg_write) && ((size * 4) > rup_args->cmd.size)) {
		CAM_ERR(CAM_ISP,
			...
			csid_hw->hw_intf->hw_idx, rup_args->cmd.size,
			(size * 4));
		return -EINVAL;
	}
	...
	if (rup_args->mup_en) {
		...
		CAM_DBG(CAM_ISP, "CSID[%u] MUP %u", csid_hw->hw_intf->hw_idx,
			csid_hw->rx_cfg.mup);
	}

	if (csid_reg->cmn_reg->capabilities & CAM_IFE_CSID_CAP_SPLIT_RUP_AUP)
		cam_ife_csid_ver2_get_mc_reg_val_pair(csid_hw, reg_val_pair,
						      rup_args);
	else
		cam_ife_csid_ver2_get_sc_reg_val_pair(csid_hw, reg_val_pair,
						      rup_args);

	if (rup_args->reg_write) {
		for (i = 0; i < (2 * num_reg_val_pairs); i = i + 2)
			cam_io_w_mb(reg_val_pair[i + 1],
				    csid_clc_membase + reg_val_pair[i]);
	} else {
...
static int
cam_ife_csid_ver2_program_offline_go_cmd(struct cam_ife_csid_ver2_hw *csid_hw,
					 void *cmd_args, uint32_t arg_size)
{
	...
	if (arg_size != sizeof(struct cam_ife_csid_offline_cmd_update_args)) {
		CAM_ERR(CAM_ISP, "CSID[%u] Invalid arg size: %d expected:%ld",
			csid_hw->hw_intf->hw_idx, arg_size,
	...
	if ((size * 4) > go_args->cmd.size) {
		CAM_ERR(CAM_ISP,
			"CSID[%u] buf size:%d is not sufficient, expected: %d",
			csid_hw->hw_intf->hw_idx, go_args->cmd.size,
			(size * 4));
		return -EINVAL;
	}
	...
	cdm_util_ops->cdm_write_regrandom(go_args->cmd.cmd_buf_addr, 1,
					  reg_val_pair);
...
static int
cam_ife_csid_ver2_get_time_stamp(struct cam_ife_csid_ver2_hw *csid_hw,
				 void *cmd_args)
{
	const struct cam_ife_csid_ver2_path_reg_info *path_reg;
	struct cam_isp_resource_node *res = NULL;
	...
	if (timestamp_args->get_curr_timestamp) {
		...
		timestamp_args->time_stamp_val =
			mul_u64_u32_div(timestamp_args->time_stamp_val,
	...
	if (qtime_to_boottime == 0) {
		...
		qtime_to_boottime =
			...
			(int64_t)timestamp_args->time_stamp_val;
	}

	timestamp_args->boot_timestamp =
		timestamp_args->time_stamp_val + qtime_to_boottime;
					   ...
					   struct cam_isp_resource_node *res)
{
	...
	CAM_INFO_RATE_LIMIT(
		CAM_ISP, "CSID[%u] Resource[id:%d name:%s hbi 0x%x vbi 0x%x]",
	...
	struct cam_ife_csid_ver2_hw *csid_hw, void *cmd_args)
{
	...
	switch_update = (struct cam_ife_csid_mode_switch_update_args *)cmd_args;
   ...
   */
	if (switch_update->exp_update_args.reset_discard_cfg) {
		...
		for (i = CAM_IFE_PIX_PATH_RES_RDI_0;
		     i <= CAM_IFE_PIX_PATH_RES_RDI_2; i++) {
			res = &csid_hw->path_res[i];
			path_cfg = (struct cam_ife_csid_ver2_path_cfg *)
					   res->res_priv;
				...
				if ((i - CAM_IFE_PIX_PATH_RES_RDI_0) >=
					...
					if (path_cfg->discard_init_frames) {
						...
						path_cfg->num_frames_discard =
							0;
						atomic_dec(
							&csid_hw->discard_frame_per_path);
						CAM_DBG(CAM_ISP,
...
static int
cam_ife_csid_ver2_set_csid_clock(struct cam_ife_csid_ver2_hw *csid_hw,
				 void *cmd_args)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d, clk_rate=%lld", csid_hw->hw_intf->hw_idx,
		csid_hw->clk_rate);

	return 0;
}

static int cam_ife_csid_ver2_mini_dump(struct cam_ife_csid_ver2_hw *csid_hw,
				       void *cmd_args)
{
	...
	md = (struct cam_ife_csid_ver2_mini_dump_data *)((uint8_t *)md_args
								 ->start_addr);
	...
	for (i = 0; i < CAM_IFE_PIX_PATH_RES_MAX; i++) {
		...
		scnprintf(md_res->res_name, CAM_ISP_RES_NAME_LEN,
			  res->res_name);
		memcpy(&md_res->path_cfg, path_cfg, sizeof(*path_cfg));
	}

	memcpy(&md->rx_cfg, &csid_hw->rx_cfg,
	       sizeof(struct cam_ife_csid_ver2_rx_cfg));
	memcpy(&md->flags, &csid_hw->flags,
	       sizeof(struct cam_ife_csid_hw_flags));
...
static void *cam_ife_csid_ver2_user_dump_info(void *dump_struct,
					      uint8_t *addr_ptr)
{
	struct cam_isp_resource_node *res = NULL;
	...
	*addr++ =
		((frame >> csid_reg->cmn_reg->format_measure_height_shift_val) &
		 csid_reg->cmn_reg->format_measure_height_mask_val);
...
static int cam_ife_csid_ver2_user_dump(struct cam_ife_csid_ver2_hw *csid_hw,
				       void *cmd_args)
{
	uint32_t i = 0;
	struct cam_ife_csid_ver2_path_cfg *path_cfg;
	...
	if (!csid_hw || !cmd_args) {
		...
		CAM_WARN(CAM_ISP, "CSID:%u powered down",
			 csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}
	...
	rc = cam_common_user_dump_helper(dump_args, cam_common_user_dump_clock,
					 csid_hw->hw_info, sizeof(uint64_t),
					 "CLK_RATE_PRINT:");
	...
	for (i = 0; i < CAM_IFE_PIX_PATH_RES_MAX; i++) {
		...
		rc = cam_common_user_dump_helper(
			dump_args, cam_ife_csid_ver2_user_dump_info, res,
			sizeof(uint32_t), "CSID2_PATH.%s:", res->res_name);

		if (rc) {
			CAM_ERR(CAM_ISP,
				"CSID:%u VER2: Info dump failed, rc:%d",
				csid_hw->hw_intf->hw_idx, rc);
			return rc;
		}
	}
...
static int cam_ife_csid_ver2_dual_sync_cfg(struct cam_ife_csid_ver2_hw *csid_hw,
					   void *cmd_args)
{
	struct cam_ife_csid_dual_sync_args *dual_sync_args;

	if (!csid_hw || !cmd_args) {
		CAM_ERR(CAM_ISP, "Invalid args %pK  %pK", csid_hw, cmd_args);
		return -EINVAL;
	}
...
static int cam_ife_csid_ver2_dump_crop_reg(struct cam_hw_info *hw_info,
					   uint32_t *path_id)
{
	struct cam_ife_csid_ver2_hw *csid_hw;
	const struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	if (path_res->res_state == CAM_ISP_RESOURCE_STATE_STREAMING &&
	    path_cfg) {
		...
		CAM_INFO(
			...
			csid_hw->hw_intf->hw_idx, path_res->res_name, hcrop,
			vcrop, cfg0, cfg1);
		CAM_INFO(
			CAM_ISP,
			"CSID[%u] %s width=%d height=%d start line:%d end line:%d",
			csid_hw->hw_intf->hw_idx, path_res->res_name,
			path_cfg->width, path_cfg->height, path_cfg->start_line,
			path_cfg->end_line);
	}
...
static int
cam_ife_csid_ver2_set_discard_frame_cfg(struct cam_ife_csid_ver2_hw *csid_hw,
					void *cmd_args)
{
	struct cam_isp_resource_node *res;
	struct cam_ife_csid_ver2_path_cfg *path_cfg;
	...
	if ((res->res_state == CAM_ISP_RESOURCE_STATE_RESERVED) ||
		...
		path_cfg = (struct cam_ife_csid_ver2_path_cfg *)res->res_priv;
		if (path_cfg->skip_discard_frame_cfg ||
		    path_cfg->discard_init_frames)
			goto end;
...
static int cam_ife_csid_ver2_rdi_lcr_cfg(struct cam_ife_csid_ver2_hw *csid_hw,
					 void *cmd_args)
{
	const struct cam_ife_csid_ver2_path_reg_info *path_reg;
	struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	if (!path_cfg->sfe_shdr &&
	    (res->res_id != CAM_IFE_PIX_PATH_RES_RDI_0)) {
		CAM_ERR(CAM_ISP,
			"CSID[%u] Invalid res: %s, capabilities 0x%x sfe_shdr: %u",
			csid_hw->hw_intf->hw_idx, res->res_name,
   ...
   */
	if (path_cfg->path_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0].packing_fmt) {
		CAM_ERR(CAM_ISP,
			"CSID[%u] [%s] LCR not supported in_format %d out_format %d",
			csid_hw->hw_intf->hw_idx, res->res_name,
			path_cfg->in_format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0],
			path_cfg->out_format);
		return -EINVAL;
	...
	/* if CSID unpacked data is not in MSB, we loose few bits going into PDAF,
   * warn for now */
	if (!path_cfg->csid_out_unpack_msb)
		CAM_WARN(
			CAM_ISP,
	...
	CAM_DBG(CAM_ISP, "CSID[%u] %s top_cfg %u", csid_hw->hw_intf->hw_idx,
		res->res_name, csid_hw->top_cfg.rdi_lcr);

	return 0;
}

static int cam_ife_csid_init_config_update(void *cmd_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "Init Update for res_name: %s epoch_factor: %x",
		res->res_name, path_cfg->epoch_cfg);
end:
	return 0;
}

static int cam_ife_csid_ver2_dump_irq_desc(struct cam_ife_csid_ver2_hw *csid_hw,
					   void *args)
{
	int i, offset = 0;
	...
	offset += scnprintf(
		inject_params->line_buf + offset, LINE_BUFFER_LEN - offset,
	...
	case CAM_ISP_CSID_TOP_REG:
		for (i = 0;
		     i <
		     csid_reg->num_top_err_irqs[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0];
		     i++) {
			if (!(*csid_reg->top_irq_desc)
				     [CAM_IFE_CSID_TOP_IRQ_STATUS_REG0][i]
					     .bitmask)
	...
	case CAM_ISP_CSID_RX_REG:
		for (i = 0;
		     i <
		     csid_reg->num_rx_err_irqs[CAM_IFE_CSID_RX_IRQ_STATUS_REG0];
		     i++) {
			if (!(*csid_reg->rx_irq_desc)
				     [CAM_IFE_CSID_RX_IRQ_STATUS_REG0][i]
					     .bitmask)
				break;
...
static int cam_ife_csid_ver2_irq_inject(struct cam_ife_csid_ver2_hw *csid_hw,
					void *args)
{
	...
	switch (inject_params->reg_unit) {
	case CAM_ISP_CSID_TOP_REG: {
		irq_set_addr = csid_reg->cmn_reg->top_irq_set_addr
				       [CAM_IFE_CSID_TOP_IRQ_STATUS_REG0];
		break;
	}
	case CAM_ISP_CSID_RX_REG: {
		irq_set_addr =
			csid_reg->csi2_reg
				->irq_set_addr[CAM_IFE_CSID_RX_IRQ_STATUS_REG0];
		break;
	}
	case CAM_ISP_CSID_PATH_IPP_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_IPP]
				       ->irq_set_addr;
		break;
	}
	case CAM_ISP_CSID_PATH_PPP_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_PPP]
				       ->irq_set_addr;
		break;
	}
	case CAM_ISP_CSID_PATH_RDI0_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_RDI_0]
				       ->irq_set_addr;
		break;
	}
	case CAM_ISP_CSID_PATH_RDI1_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_RDI_1]
				       ->irq_set_addr;
		break;
	}
	case CAM_ISP_CSID_PATH_RDI2_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_RDI_2]
				       ->irq_set_addr;
		break;
	}
	case CAM_ISP_CSID_PATH_RDI3_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_RDI_3]
				       ->irq_set_addr;
		break;
	}
	case CAM_ISP_CSID_PATH_RDI4_REG: {
		irq_set_addr = csid_reg->path_reg[CAM_IFE_PIX_PATH_RES_RDI_4]
				       ->irq_set_addr;
		break;
	}
	default:
...
static int
cam_ife_csid_ver2_reset_out_of_sync_cnt(struct cam_ife_csid_ver2_hw *csid_hw,
					void *args)
{
	struct cam_ife_csid_ver2_path_cfg *path_cfg = NULL;
	struct cam_isp_resource_node *res = NULL;
	...
	CAM_DBG(CAM_ISP, "Reset out of sync cnt for res:%s", res->res_name);

	return 0;
}

static int cam_ife_csid_ver2_drv_config(struct cam_ife_csid_ver2_hw *csid_hw,
					void *cmd_args)
{
	struct cam_hw_soc_info *soc_info;
	struct cam_ife_csid_ver2_reg_info *csid_reg;
	...
	if (drv_config->is_init_config) {
		if (csid_hw->path_res[CAM_IFE_PIX_PATH_RES_IPP].res_state >=
		    CAM_ISP_RESOURCE_STATE_RESERVED) {
			cfg1_val = csid_reg->cmn_reg->drv_rup_en_val_map
					   [CAM_IFE_PIX_PATH_RES_IPP];
		} else if (csid_hw->path_res[CAM_IFE_PIX_PATH_RES_RDI_0]
				   .res_state >=
			   CAM_ISP_RESOURCE_STATE_RESERVED) {
			cfg1_val = csid_reg->cmn_reg->drv_rup_en_val_map
					   [CAM_IFE_PIX_PATH_RES_RDI_0];
		} else if (csid_hw->path_res[CAM_IFE_PIX_PATH_RES_PPP]
				   .res_state >=
			   CAM_ISP_RESOURCE_STATE_RESERVED) {
			cfg1_val = csid_reg->cmn_reg->drv_rup_en_val_map
			...
			CAM_ERR(CAM_ISP,
				"CSID:%u Failed to configure rup_en for drv",
				csid_hw->hw_intf->hw_idx);
			return -EINVAL;
		}

		cam_io_w_mb(cfg1_val,
			    mem_base + csid_reg->cmn_reg->drv_cfg1_addr);
		csid_hw->drv_init_done = true;
	}

	if (!csid_hw->drv_init_done) {
		CAM_ERR(CAM_ISP,
			"CSID:%u Failed to update drv config, init config not done",
			csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}
	...
	for (i = 0; i < CAM_ISP_MAX_PATHS; i++)
		cfg0_val |=
			((drv_config->path_idle_en & BIT(i)) ?
				 csid_reg->cmn_reg->drv_path_idle_en_val_map[i] :
				 0);

	cam_io_w_mb(cfg0_val, mem_base + csid_reg->cmn_reg->drv_cfg0_addr);

	cam_io_w_mb(drv_config->timeout_val,
		    mem_base + csid_reg->cmn_reg->drv_cfg2_addr);

	if (debug_drv)
		CAM_INFO(
			CAM_ISP,
			"CSID[%u] sfe_en:%s DRV config init_req:%s cfg0_val:0x%x "
			"cfg1_val:0x%x cfg2_val:0x%x",
			csid_hw->hw_intf->hw_idx,
			CAM_BOOL_TO_YESNO(csid_hw->flags.sfe_en),
			CAM_BOOL_TO_YESNO(drv_config->is_init_config), cfg0_val,
			cfg1_val, drv_config->timeout_val);
...
static int cam_ife_csid_ver2_irq_comp_cfg(struct cam_ife_csid_ver2_hw *csid_hw,
					  void *cmd_args, uint32_t arg_size)
{
	...
	if (arg_size != sizeof(struct cam_isp_hw_get_cmd_update)) {
		CAM_ERR(CAM_ISP,
			...
			csid_hw->hw_intf->hw_idx,
			sizeof(struct cam_isp_hw_get_cmd_update), arg_size);
		return -EINVAL;
	}

	cdm_args = (struct cam_isp_hw_get_cmd_update *)cmd_args;
	if (!cdm_args) {
		CAM_ERR(CAM_ISP, "CSID:%u Error, Invalid args",
			csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}
	...
	comp_cfg_mask = (comp_cfg->ipp_src_ctxt_mask
			 << reg_info->ipp_src_ctxt_mask_shift) |
			(comp_cfg->ipp_dst_comp_mask
			 << reg_info->ipp_dst_ctxt_mask_shift);
	...
	if ((size * 4) > cdm_args->cmd.size) {
		CAM_ERR(CAM_ISP,
			"CSID:%u buf size:%d is not sufficient, expected: %d",
			csid_hw->hw_intf->hw_idx, cdm_args->cmd.size,
			(size * 4));
		return -EINVAL;
	}
...
static int cam_ife_csid_ver2_process_cmd(void *hw_priv, uint32_t cmd_type,
					 void *cmd_args, uint32_t arg_size)
{
	int rc = 0;
	struct cam_ife_csid_ver2_hw *csid_hw;
	...
	case CAM_IFE_CSID_SET_CSID_DEBUG:
		rc = cam_ife_csid_ver2_set_debug(
			csid_hw,
	...
	case CAM_ISP_HW_CMD_GET_REG_UPDATE:
		rc = cam_ife_csid_ver2_reg_update(csid_hw, cmd_args, arg_size);
		break;
	case CAM_IFE_CSID_SET_DUAL_SYNC_CONFIG:
		rc = cam_ife_csid_ver2_dual_sync_cfg(csid_hw, cmd_args);
		break;
	...
	case CAM_IFE_CSID_PROGRAM_OFFLINE_CMD:
		rc = cam_ife_csid_ver2_program_offline_go_cmd(csid_hw, cmd_args,
							      arg_size);
		break;
	case CAM_ISP_HW_CMD_CSID_DYNAMIC_SWITCH_UPDATE:
		rc = cam_ife_csid_ver2_set_dynamic_switch_config(csid_hw,
								 cmd_args);
		break;
	...
	case CAM_ISP_HW_CMD_CSID_DUMP_CROP_REG:
		rc = cam_ife_csid_ver2_dump_crop_reg(hw_info,
						     (uint32_t *)cmd_args);
		break;
	case CAM_ISP_HW_CMD_IRQ_COMP_CFG:
		rc = cam_ife_csid_ver2_irq_comp_cfg(csid_hw, cmd_args,
						    arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_GET_SET_PRIM_SOF_TS_ADDR: {
		...
		if (!sof_addr_args->get_addr) {
			CAM_ERR(CAM_ISP,
				"CSID does not support set of primary SOF ts addr");
			rc = -EINVAL;
		} else
			rc = cam_ife_csid_ver2_get_primary_sof_timer_reg_addr(
				csid_hw, sof_addr_args);
	} break;
	default:
...
static irqreturn_t cam_ife_csid_irq(int irq_num, void *data)
{
	...
	return cam_irq_controller_handle_irq(
		irq_num,
		csid_hw->top_irq_controller[CAM_IFE_CSID_TOP_IRQ_STATUS_REG0],
		CAM_IRQ_EVT_GROUP_0);
}
...
static int cam_ife_ver2_hw_alloc_res(struct cam_isp_resource_node *res,
				     uint32_t res_type,
...
static int
cam_ife_csid_ver2_hw_init_path_res(struct cam_ife_csid_ver2_hw *csid_hw)
{
	int rc = 0;
	int i;
	struct cam_ife_csid_ver2_reg_info *csid_reg;
...
static void cam_ife_csid_hw_deinit_irq(struct cam_ife_csid_ver2_hw *csid_hw)
{
	unsigned long flags;
	int i;
...
static int cam_ife_csid_hw_init_irq(struct cam_ife_csid_ver2_hw *csid_hw)
{
	int rc = 0;
	int i;
	struct cam_hw_soc_info *soc_info;
	...
	snprintf(name, CAM_CSID_IRQ_CTRL_NAME_LEN, "csid%d_top",
		 csid_hw->hw_intf->hw_idx);
	...
	snprintf(name, CAM_CSID_IRQ_CTRL_NAME_LEN, "csid%d_rx",
		 csid_hw->hw_intf->hw_idx);
	rc = cam_irq_controller_init(
		name, mem_base,
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"CSID:%u Failed to init CSID rx irq controller rc = %d",
			csid_hw->hw_intf->hw_idx, rc);
		goto deinit_controller;
	}

	snprintf(name, CAM_CSID_IRQ_CTRL_NAME_LEN, "csid%d_buf_done",
		 csid_hw->hw_intf->hw_idx);
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"CSID:%u Failed to init CSID buf_done irq controller rc = %d",
			csid_hw->hw_intf->hw_idx, rc);
		goto deinit_controller;
	}

	if (csid_reg->cmn_reg->num_pix) {
		snprintf(name, CAM_CSID_IRQ_CTRL_NAME_LEN, "csid%d_ipp",
			 csid_hw->hw_intf->hw_idx);
		rc = cam_irq_controller_init(
			name, mem_base,
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"CSID:%u Failed to init CSID ipp irq controller rc = %d",
				csid_hw->hw_intf->hw_idx, rc);
			goto deinit_controller;
		}
	}
	...
	if (csid_reg->cmn_reg->num_ppp) {
		snprintf(name, CAM_CSID_IRQ_CTRL_NAME_LEN, "csid%d_ppp",
			 csid_hw->hw_intf->hw_idx);
		rc = cam_irq_controller_init(
			name, mem_base,
	...
	for (i = 0; i < csid_reg->cmn_reg->num_rdis; i++) {
		...
		rc = cam_irq_controller_init(
			name, mem_base,
			csid_reg->path_irq_reg_info[CAM_IFE_PIX_PATH_RES_RDI_0 +
						    i],
			&csid_hw->path_irq_controller[CAM_IFE_PIX_PATH_RES_RDI_0 +
						      i]);
		if (rc) {
			CAM_ERR(CAM_ISP,
...
int cam_ife_csid_ver2_irq_line_test(void *hw_priv)
{
	...
	mem_base =
		csid_hw->hw_info->soc_info.reg_map[CAM_IFE_CSID_CLC_MEM_BASE_ID]
			.mem_base;
	csid_reg = csid_hw->core_info->csid_reg;
	rc = cam_ife_csid_enable_soc_resources(soc_info,
					       soc_info->lowest_clk_level);
	if (rc) {
		CAM_ERR(CAM_ISP, "CSID[%u] Enable soc failed",
			csid_hw->hw_intf->hw_idx);
		return rc;
	}

	CAM_DBG(CAM_ISP, "CSID[%u] hw-version:0x%x", csid_hw->hw_intf->hw_idx,
			      ...
			      bool is_custom)
{
	...
	CAM_DBG(CAM_ISP, "type %d index %d", hw_intf->hw_type, hw_intf->hw_idx);
	...
	rc = cam_ife_csid_init_soc_resources(&csid_hw->hw_info->soc_info,
					     ...
					     is_custom);
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "CSID:%u Failed to init_soc", hw_intf->hw_idx);
		return rc;
	}
	...
	csid_hw->hw_intf->hw_ops.test_irq_line =
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "CSID[%u] Probe Init failed", hw_intf->hw_idx);
		return rc;
	}
