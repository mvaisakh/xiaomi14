...
static int
cam_vfe_bus_get_free_dual_comp_grp(struct cam_vfe_bus_ver2_priv *ver2_bus_priv,
				   struct cam_isp_resource_node **comp_grp,
				   uint32_t comp_grp_local_idx)
{
	struct cam_vfe_bus_ver2_comp_grp_data *rsrc_data = NULL;
...
int cam_vfe_bus_dump_wm_data(void *priv, void *cmd_args, uint32_t arg_size)
{
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		...
		CAM_INFO(
			...
			"acquired width:%u height:%u",
			wm_data->common_data->core_index, wm_idx,
			wm_data->width, wm_data->height, wm_data->stride,
			wm_data->h_init, wm_data->en_cfg,
			wm_data->acquired_width, wm_data->acquired_height);
	}
	return 0;
}

static int cam_vfe_bus_get_res_for_mid(struct cam_vfe_bus_ver2_priv *bus_priv,
				       void *cmd_args, uint32_t arg_size)
{
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		out_data = (struct cam_vfe_bus_ver2_vfe_out_data *)bus_priv
				   ->vfe_out[i]
				   .res_priv;
