...
static int cam_vfe_bus_rd_process_cmd(struct cam_isp_resource_node *priv,
...
static int
cam_vfe_bus_rd_handle_irq_top_half(uint32_t evt_id,
...
static int cam_vfe_bus_rd_handle_irq_bottom_half(void *handler_priv,
...
static int cam_vfe_bus_acquire_rm(
	struct cam_vfe_bus_rd_ver1_priv *ver1_bus_rd_priv, void *tasklet,
	...
	uint32_t is_dual, uint32_t unpacker_fmt)
{
	uint32_t rm_idx = 0;
...
static int cam_vfe_bus_start_rm(struct cam_isp_resource_node *rm_res)
{
	...
	cam_io_w_mb(buf_size,
		    common_data->mem_base + rm_data->hw_regs->buf_size);
					  ...
					  uint32_t args_size)
{
	...
	for (i = 0; i < num_rm; i++) {
		rc = cam_vfe_bus_acquire_rm(ver1_bus_rd_priv, acq_args->tasklet,
		...
		if (rc) {
			CAM_ERR(CAM_ISP, "VFE:%d RM:%d acquire failed rc:%d",
				rsrc_data->common_data->core_index,
				bus_rd_res_id, rc);
			goto release_rm;
		}
...
static int
cam_vfe_bus_start_vfe_bus_rd(struct cam_isp_resource_node *vfe_bus_rd)
{
	int rc = 0, i;
	struct cam_vfe_bus_rd_ver1_vfe_bus_rd_data *rsrc_data = NULL;
	...
	rsrc_data->irq_handle = cam_irq_controller_subscribe_irq(
		...
		irq_reg_mask, vfe_bus_rd, cam_vfe_bus_rd_handle_irq_top_half,
		cam_vfe_bus_rd_handle_irq_bottom_half, vfe_bus_rd->tasklet_info,
				    ...
				    uint32_t arg_size)
{
	...
	if (num_regval_pairs) {
		...
		CAM_DBG(CAM_ISP, "No reg val pairs. num_rms: %u",
			vfe_bus_rd_data->num_rm);
	}
					...
					uint32_t arg_size)
{
	...
	for (i = 0; i < vfe_bus_rd_data->num_rm; i++) {
		...
		CAM_DBG(CAM_ISP, "VFE:%d RM:%d format:0x%x unpacker_cfg:0x%x",
...
static int __cam_vfe_bus_rd_process_cmd(void *priv, uint32_t cmd_type,
			     ...
			     struct cam_vfe_bus **vfe_bus)
{
	...
	for (i = 0; i < bus_priv->num_bus_rd_resc; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				"Init VFE Out failed for client:%d, rc=%d", i,
				rc);
