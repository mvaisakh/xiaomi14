...
static void
cam_vfe_bus_ver3_unsubscribe_init_irq(struct cam_vfe_bus_ver3_priv *bus_priv);

static int
cam_vfe_bus_ver3_subscribe_init_irq(struct cam_vfe_bus_ver3_priv *bus_priv);
...
cam_vfe_bus_ver3_get_out_res_id_and_index(struct cam_vfe_bus_ver3_priv *bus_priv,
				     ...
				     struct cam_irq_th_payload *th_payload)
{
	...
	if (rc) {
		CAM_INFO_RATE_LIMIT(CAM_ISP, "VFE:%u Bus IRQ status_0: 0x%X",
				    rsrc_data->common_data->core_index,
				    th_payload->evt_status_arr[0]);
		return rc;
	}
...
static void
cam_vfe_bus_ver3_print_constraint_errors(struct cam_vfe_bus_ver3_priv *bus_priv,
					 uint8_t *wm_name,
	...
	for (i = 0; i < bus_priv->num_cons_err; i++) {
		if (bus_priv->constraint_error_list[i].bitmask &
			...
			CAM_INFO(CAM_ISP, "VFE:%u WM:%s %s",
				 ...
				 bus_priv->constraint_error_list[i]
					 .error_description);
		}
	}
}
...
static void
cam_vfe_bus_ver3_get_constraint_errors(struct cam_vfe_bus_ver3_priv *bus_priv)
{
						   ...
						   void *evt_payload_priv)
{
	...
	if (!rsrc_data->common_data->event_cb) {
		CAM_ERR(CAM_ISP, "VFE:%u Callback to HW MGR not found",
			vfe_out->hw_intf->hw_idx);
		return ret;
	}
	...
	CAM_DBG(CAM_ISP, "VFE:%u Bus RUP IRQ status_0:0x%X rc:%d",
	...
	struct cam_vfe_bus_ver3_wm_resource_data *rsrc_data)
{
	...
	case CAM_FORMAT_PLAIN16_14:
	case CAM_FORMAT_PLAIN16_16:
		rsrc_data->width = ALIGNUP(rsrc_data->width * 2, 16) / 16;
		rsrc_data->en_cfg = 0x1;

		if (rsrc_data->use_wm_pack) {
			rsrc_data->pack_fmt = cam_vfe_bus_ver3_get_packer_fmt(
				rsrc_data->format, rsrc_data->index);
			/* LSB aligned */
			rsrc_data->pack_fmt |=
		...
		break;
	case CAM_FORMAT_PLAIN64:
		rsrc_data->width = ALIGNUP(rsrc_data->width * 8, 16) / 16;
		rsrc_data->en_cfg = 0x1;
		break;
...
static int cam_vfe_bus_ver3_acquire_wm(
	...
	struct cam_vfe_hw_vfe_out_acquire_args *out_acq_args, void *tasklet,
	enum cam_vfe_bus_ver3_vfe_out_type vfe_out_res_id,
	enum cam_vfe_bus_plane_type plane, struct cam_isp_resource_node *wm_res,
	enum cam_vfe_bus_ver3_comp_grp_type *comp_grp_id)
{
	int32_t wm_idx = 0, rc;
	...
	CAM_DBG(CAM_ISP, "VFE:%u WM:%d width %d height %d",
		wm_res->hw_intf->hw_idx, rsrc_data->index, rsrc_data->width,
		rsrc_data->height);
	...
	if ((vfe_out_res_id >= CAM_VFE_BUS_VER3_VFE_OUT_RDI0) &&
		...
		default:
			CAM_ERR(CAM_ISP, "VFE:%u Invalid format %d out_type:%d",
				wm_res->hw_intf->hw_idx, rsrc_data->format,
				vfe_out_res_id);
			return -EINVAL;
		}
		rsrc_data->en_cfg = 0x1;
	...
	*comp_grp_id = rsrc_data->hw_regs->comp_group;
	...
	CAM_DBG(CAM_ISP,
		"VFE:%u WM:%d %s processed width:%d height:%d stride:%d format:0x%X "
		"en_ubwc:%d %s",
...
static int cam_vfe_bus_ver3_start_wm(struct cam_isp_resource_node *wm_res)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u WM:%d pk_fmt:%d stride:%d burst len:%d",
		rsrc_data->common_data->core_index, rsrc_data->index,
		rsrc_data->pack_fmt, rsrc_data->stride, 0xF);
...
cam_vfe_bus_ver3_init_wm_resource(uint32_t index,
				  ...
				  struct cam_isp_resource_node *wm_res,
				  struct cam_isp_resource_node **comp_grp,
				  uint8_t *wm_name, uint32_t line_based_config)
{
	struct cam_vfe_bus_ver3_wm_resource_data *rsrc_data;
	...
	*comp_grp =
		&ver3_bus_priv->comp_grp[(&ver3_hw_info->bus_client_reg[index])
						 ->comp_group];
...
static int cam_vfe_bus_ver3_acquire_comp_grp(
	struct cam_vfe_bus_ver3_priv *ver3_bus_priv, void *tasklet,
					    ...
					    uint32_t args_size)
{
	...
	CAM_DBG(CAM_ISP,
		"VFE:%u Acquire out_type:0x%X use_hw_ctxt:%s hw_ctx_id:0x%x",
		ver3_bus_priv->common_data.core_index,
	...
	vfe_out_res_id = cam_vfe_bus_ver3_get_out_res_id_and_index(
		ver3_bus_priv, out_acquire_args->out_port_info->res_type,
		&outmap_index);
	...
	if (rsrc_node->res_state != CAM_ISP_RESOURCE_STATE_AVAILABLE) {
		if ((rsrc_data->mc_based || rsrc_data->cntxt_cfg_except) &&
			...
			CAM_ERR(CAM_ISP,
				"VFE:%u out_type:0x%X mc_cap:%s cntxt_except:%s resource not "
				"available state:%d dst_hw_ctxt_id_mask:0x%x req_ctxt_id:0x%x",
				ver3_bus_priv->common_data.core_index,
				vfe_out_res_id,
   ...
   */
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		rc = cam_vfe_bus_ver3_acquire_wm(
			ver3_bus_priv, out_acquire_args, acq_args->tasklet,
			vfe_out_res_id, i, &rsrc_data->wm_res[i],
	...
	rc = cam_vfe_bus_ver3_acquire_comp_grp(ver3_bus_priv, acq_args->tasklet,
	...
	CAM_DBG(CAM_ISP, "VFE:%u Acquire successful",
		rsrc_data->common_data->core_index);
	return rc;
					    ...
					    uint32_t args_size)
{
	...
	if (secure_caps) {
       ...
       */
			CAM_ERR(CAM_ISP,
				"VFE:%u driver[%d],resource[%d] mismatch",
				rsrc_data->common_data->core_index,
				rsrc_data->common_data->secure_mode,
				rsrc_data->secure_mode);
		}
	...
	struct cam_vfe_bus_ver3_hw_info *ver3_hw_info)
{
	...
	rsrc_data->cntxt_cfg_except =
		ver3_hw_info->vfe_out_hw_info[index].cntxt_cfg_except;
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		rc = cam_vfe_bus_ver3_init_wm_resource(
			...
			ver3_bus_priv, ver3_hw_info, &rsrc_data->wm_res[i],
			&rsrc_data->comp_grp,
...
cam_vfe_bus_ver3_deinit_vfe_out_resource(struct cam_isp_resource_node *vfe_out)
{
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		...
		if (rc < 0)
			CAM_ERR(CAM_ISP, "VFE:%u deinit WM:%d failed rc:%d",
...
static void cam_vfe_bus_ver3_print_wm_info(
	struct cam_vfe_bus_ver3_wm_resource_data *wm_data,
	struct cam_vfe_bus_ver3_common_data *common_data, uint8_t *wm_name)
{
	...
	CAM_INFO(
		CAM_ISP,
		"VFE:%u WM:%d wm_name:%s width:%u height:%u stride:%u x_init:%u "
		"en_cfg:%u acquired width:%u height:%u",
	...
	CAM_INFO(
		CAM_ISP,
		"VFE:%u WM:%u last consumed address:0x%x last frame addr:0x%x fifo "
		"cnt:0x%x current client address:0x%x limiter: 0x%x",
		common_data->hw_intf->hw_idx, wm_data->index, addr_status0,
		addr_status1, addr_status2, addr_status3, limiter);
}

static int
cam_vfe_bus_ver3_print_dimensions(uint32_t res_id,
				  struct cam_vfe_bus_ver3_priv *bus_priv)
{
	struct cam_isp_resource_node *rsrc_node = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data = NULL;
	...
	if ((vfe_out_res_id == CAM_VFE_BUS_VER3_VFE_OUT_MAX) ||
		...
		CAM_WARN_RATE_LIMIT(
			...
			res_id, vfe_out_res_id, outmap_index);
		return -EINVAL;
	}
...
static int cam_vfe_bus_ver3_mini_dump(struct cam_vfe_bus_ver3_priv *bus_priv,
				      void *cmd_args)
{
	struct cam_isp_resource_node *rsrc_node = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data = NULL;
...
static void *cam_vfe_bus_ver3_user_dump_info(void *dump_struct,
					     uint8_t *addr_ptr)
{
	struct cam_vfe_bus_ver3_wm_resource_data *wm = NULL;
	uint32_t *addr;
...
static int cam_vfe_bus_ver3_user_dump(struct cam_vfe_bus_ver3_priv *bus_priv,
				      void *cmd_args)
{
	struct cam_isp_resource_node *rsrc_node = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data = NULL;
	...
	if (hw_info->hw_state == CAM_HW_STATE_POWER_DOWN) {
		CAM_WARN(CAM_ISP, "VFE:%u  BUS powered down",
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		...
		for (j = 0; j < rsrc_data->num_wm; j++) {
			...
			rc = cam_common_user_dump_helper(
				dump_args, cam_vfe_bus_ver3_user_dump_info, wm,
				sizeof(uint32_t), "VFE_BUS_CLIENT.%s.%d:",
				rsrc_data->wm_res[j].res_name,
				rsrc_data->common_data->core_index);

			if (rc) {
				CAM_ERR(CAM_ISP,
					"VFE:%u BUS VER3: Info dump failed, rc: %d",
					bus_priv->common_data.core_index, rc);
				return rc;
			}
		}
...
static int
cam_vfe_bus_ver3_handle_err_irq_top_half(uint32_t evt_id,
					 struct cam_irq_th_payload *th_payload)
{
	int i = 0, rc = 0;
...
static void
cam_vfe_check_violations(char *error_type, uint32_t status,
			 struct cam_vfe_bus_ver3_priv *bus_priv,
			 uint64_t *out_port,
...
static int cam_vfe_bus_ver3_handle_err_irq_bottom_half(void *handler_priv,
						       void *evt_payload_priv)
{
	struct cam_vfe_bus_irq_evt_payload *evt_payload = evt_payload_priv;
	...
	CAM_ERR(CAM_ISP,
		"VFE:%u BUS error image size violation %d CCIF violation %d "
		"constraint violation %d",
...
cam_vfe_bus_ver3_unsubscribe_init_irq(struct cam_vfe_bus_ver3_priv *bus_priv)
{
	...
	CAM_DBG(CAM_ISP, "BUS init irq unsubscribed, VFE:%u",
		bus_priv->common_data.core_index);
}
...
cam_vfe_bus_ver3_subscribe_init_irq(struct cam_vfe_bus_ver3_priv *bus_priv)
{
	...
	if (bus_priv->common_data.supported_irq & CAM_VFE_HW_IRQ_CAP_RUP) {
		bus_priv->rup_irq_handle = cam_irq_controller_subscribe_irq(
			bus_priv->common_data.vfe_irq_controller,
			CAM_IRQ_PRIORITY_2, top_irq_reg_mask, bus_priv,
			cam_vfe_bus_ver3_handle_rup_irq, NULL, NULL, NULL,
			CAM_IRQ_EVT_GROUP_0);
	...
	CAM_DBG(CAM_ISP, "VFE:%u BUS irq subscribed",
		bus_priv->common_data.core_index);
	return 0;
}
						   ...
						   dma_addr_t image_buf)
{
	...
	uint32_t temp = cam_smmu_is_expanded_memory() ?
				CAM_36BIT_INTF_GET_IOVA_BASE(image_buf) :
				image_buf;
				      ...
				      uint32_t arg_size)
{
	...
	if (!vfe_out_data->limiter_enabled)
		CAM_WARN(
			CAM_ISP,
			"Configuring scratch for VFE:%u out_type: %u, with no BW limiter "
			"enabled",
			bus_priv->common_data.core_index,
				      ...
				      uint32_t arg_size)
{
	...
	if (update_buf->use_scratch_cfg) {
		...
		if (!vfe_out_data->limiter_enabled)
			CAM_WARN(
				CAM_ISP,
				"Configuring scratch for VFE:%u out_type: %u, with no BW "
				"limiter enabled",
				bus_priv->common_data.core_index,
				vfe_out_data->out_type);
	} else {
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		if (j >= (MAX_REG_VAL_PAIR_SIZE - MAX_BUF_UPDATE_REG_NUM * 2)) {
			CAM_ERR(CAM_ISP,
				...
				bus_priv->common_data.core_index, j,
     ...
     */
		...
		/* WM Image address */
		iova = update_buf->wm_update->image_buf[i] + image_buf_offset;
		...
		if (cam_smmu_is_expanded_memory()) {
			...
			CAM_DBG(CAM_ISP,
				...
				bus_priv->common_data.core_index,
				wm_data->index, reg_val_pair[j - 3],
				reg_val_pair[j - 1]);
		} else {
			iova_addr = iova;
				       ...
				       uint32_t arg_size)
{
	...
	if (num_regval_pairs) {
		...
		CAM_DBG(CAM_ISP, "VFE:%u No reg val pairs. num_wms: %u",
...
static int cam_vfe_bus_ver3_update_ubwc_config_v2(void *cmd_args)
{
	...
	ubwc_generic_cfg =
		(struct cam_vfe_generic_ubwc_config *)update_ubwc->data;
     ...
     */
		...
		if ((wm_config->packer_format != CAM_FORMAT_BASE) &&
			...
			CAM_DBG(CAM_ISP,
				...
				vfe_out_data->common_data->core_index,
				wm_data->index, wm_config->packer_format,
				wm_data->pack_fmt);
		}
		...
		CAM_DBG(CAM_ISP,
			...
			wm_data->en_cfg, wm_data->height, wm_data->width,
			wm_data->stride);
	}
...
static int cam_vfe_bus_update_bw_limiter(void *priv, void *cmd_args,
					 uint32_t arg_size)
{
	struct cam_isp_hw_get_cmd_update *wm_config_update;
	struct cam_vfe_bus_ver3_vfe_out_data *vfe_out_data = NULL;
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		...
		wm_data =
			(struct cam_vfe_bus_ver3_wm_resource_data *)vfe_out_data
				->wm_res[i]
				.res_priv;
...
static int cam_vfe_bus_ver3_mc_ctxt_sel(void *priv, void *cmd_args,
					uint32_t arg_size)

{
	...
	if (!priv || !cmd_args) {
		CAM_ERR(CAM_ISP, "Invalid args priv %x cmd_args %x", priv,
			cmd_args);
		return -EINVAL;
	}
	...
	cdm_util_ops->cdm_write_regrandom(mc_config->cmd.cmd_buf_addr, 1,
					  reg_val);
...
static int cam_vfe_bus_ver3_irq_inject(void *priv, void *cmd_args,
				       uint32_t arg_size)
{
	...
	if (inject_params->reg_unit ==
		...
		inject_reg =
			&bus_hw_info->common_reg.irq_reg_info.irq_reg_set[0];
	else if (inject_params->reg_unit ==
		 CAM_ISP_IFE_0_BUS_WR_INPUT_IF_IRQ_SET_1_REG)
		inject_reg =
			&bus_hw_info->common_reg.irq_reg_info.irq_reg_set[1];
	else
		return -EINVAL;
	...
	cam_io_w_mb(0x10, soc_info->reg_map[VFE_CORE_BASE_IDX].mem_base +
				  bus_hw_info->common_reg.irq_reg_info
					  .global_irq_cmd_offset);
...
static int cam_vfe_bus_ver3_dump_irq_desc(void *priv, void *cmd_args,
					  uint32_t arg_size)
{
	int i, offset = 0;
	int num_irq_desc = 0;
...
static int cam_vfe_bus_get_res_for_mid(struct cam_vfe_bus_ver3_priv *bus_priv,
				       void *cmd_args, uint32_t arg_size)
{
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		out_data = (struct cam_vfe_bus_ver3_vfe_out_data *)bus_priv
				   ->vfe_out[i]
				   .res_priv;
...
static uint32_t
cam_vfe_bus_ver3_get_last_consumed_addr(struct cam_vfe_bus_ver3_priv *bus_priv,
					uint32_t res_type)
{
	...
	res_id = cam_vfe_bus_ver3_get_out_res_id_and_index(bus_priv, res_type,
							   &outmap_index);
					...
					uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_DUMP_BUS_INFO: {
		...
		event_info = (struct cam_isp_hw_event_info *)cmd_args;
		bus_priv = (struct cam_vfe_bus_ver3_priv *)priv;

		rc = cam_vfe_bus_ver3_print_dimensions(event_info->res_id,
						       bus_priv);
		break;
	}
	...
	default:
		CAM_ERR_RATE_LIMIT(CAM_ISP,
				   "VFE:%u Invalid camif process command:%d",
			  ...
			  struct cam_vfe_bus **vfe_bus)
{
	...
	bus_priv->common_data.pack_align_shift = ver3_hw_info->pack_align_shift;
	...
	if (bus_priv->num_out >= CAM_VFE_BUS_VER3_VFE_OUT_MAX) {
		CAM_ERR(CAM_ISP,
			"VFE:%u number of vfe out:%d more than max value:%d ",
	...
	bus_priv->vfe_out = kzalloc(
		(sizeof(struct cam_isp_resource_node) * bus_priv->num_out),
		GFP_KERNEL);
	if (!bus_priv->vfe_out) {
