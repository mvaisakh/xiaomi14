   ...
   */
	if (th_payload->evt_status_arr[1]) {
		...
		CAM_ERR(CAM_ISP, "Stopping further IRQ processing from vfe=%d",
			rdi_node->hw_intf->hw_idx);
		cam_irq_controller_disable_all(rdi_priv->vfe_irq_controller);
		error_flag = true;
	}
	...
	if (error_flag) {
		rdi_priv->error_ts.tv_sec = evt_payload->ts.mono_time.tv_sec;
		rdi_priv->error_ts.tv_nsec = evt_payload->ts.mono_time.tv_nsec;
	}
...
static int cam_vfe_rdi_resource_start(struct cam_isp_resource_node *rdi_res)
{
	...
	rdi_irq_mask[0] = (rsrc_data->reg_data->reg_update_irq_mask |
			   rsrc_data->reg_data->sof_irq_mask);

	CAM_DBG(CAM_ISP, "RDI%d irq_mask 0x%x",
		rdi_res->res_id - CAM_ISP_HW_VFE_IN_RDI0, rdi_irq_mask[0]);
	...
	if (!rsrc_data->irq_handle) {
		rsrc_data->irq_handle = cam_irq_controller_subscribe_irq(
			...
			rdi_irq_mask, rdi_res, rdi_res->top_half_handler,
			rdi_res->bottom_half_handler, rdi_res->tasklet_info,
			&tasklet_bh_api, CAM_IRQ_EVT_GROUP_0);
					      ...
					      void *evt_payload_priv)
{
	...
	if (irq_status0 & rdi_priv->reg_data->sof_irq_mask) {
		...
		rdi_priv->sof_ts.tv_sec = payload->ts.mono_time.tv_sec;
		rdi_priv->sof_ts.tv_nsec = payload->ts.mono_time.tv_nsec;
	...
	irq_rdi_status =
		(irq_status1 & rdi_priv->rdi_irq_status->rdi_overflow_mask);
	if (irq_rdi_status) {
		...
		CAM_INFO(
			...
			rdi_priv->error_ts.tv_sec, rdi_priv->error_ts.tv_nsec,
			rdi_priv->sof_ts.tv_sec, rdi_priv->sof_ts.tv_nsec);
