...
static int cam_vfe_top_dump_info(struct cam_vfe_top_ver2_priv *top_priv,
				 uint32_t cmd_type)
{
...
static int cam_vfe_top_blanking_update(uint32_t cmd_type, void *cmd_args,
				       uint32_t arg_size)
{
	struct cam_isp_blanking_config *blanking_config = NULL;
	struct cam_isp_resource_node *node_res = NULL;

	blanking_config = (struct cam_isp_blanking_config *)cmd_args;
	...
	return node_res->process_cmd(node_res, cmd_type, cmd_args, arg_size);
}
				       ...
				       void *cmd_args, uint32_t arg_size)
{
	...
	cdm_util_ops->cdm_write_wait_comp_event(cdm_args->cmd.cmd_buf_addr, 0,
						0x2);
	cdm_args->cmd.used_bytes = (size * 4);
...
static int cam_vfe_top_get_data(struct cam_vfe_top_ver2_priv *top_priv,
				void *cmd_args, uint32_t arg_size)
{
...
int cam_vfe_top_get_hw_caps(void *device_priv, void *args, uint32_t arg_size)
{
...
static int cam_vfe_hw_dump(struct cam_vfe_top_ver2_priv *top_priv,
			   void *cmd_args, uint32_t arg_size)
{
	int i, j;
	uint8_t *dst;
	...
	*addr++ = soc_info->index;
	...
	for (i = 0; i < dump_data->num_lut_dump_entries; i++) {
		...
		*addr++ = dump_data->lut_entry[i].lut_bank_sel;
		...
		cam_io_w_mb(val,
			    soc_info->reg_map[0].mem_base + dump_data->dmi_cfg);
		cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
				       dump_data->dmi_addr);
		for (j = 0; j < dump_data->lut_entry[i].lut_addr_size; j++) {
			if (dump_data->lut_entry[i].lut_word_size == 64) {
				addr[0] =
					cam_io_r(soc_info->reg_map[0].mem_base +
				...
				*addr = cam_io_r(soc_info->reg_map[0].mem_base +
						 dump_data->dmi_data_path_lo);
				addr++;
			}
		}
			    ...
			    void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_CAMIF_DATA:
		rc = cam_vfe_top_get_data(top_priv, cmd_args, arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_DUMP_HW:
		rc = cam_vfe_hw_dump(top_priv, cmd_args, arg_size);
		break;
