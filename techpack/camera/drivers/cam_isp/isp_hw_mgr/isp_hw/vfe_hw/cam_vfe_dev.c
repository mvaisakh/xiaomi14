...
static int cam_vfe_component_bind(struct device *dev, struct device *master_dev,
				  void *data)
{
	...
	rc = of_property_read_u32(pdev->dev.of_node, "cell-index",
				  &vfe_dev_idx);
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed to read cell-index of IFE HW, rc: %d",
			rc);
		goto end;
	}

	if (!cam_cpas_is_feature_supported(CAM_CPAS_ISP_FUSE, BIT(vfe_dev_idx),
					   NULL) ||
...
void cam_vfe_get_num_ifes(uint32_t *num_ifes)
{
	if (num_ifes)
		*num_ifes = g_num_ife_hws;
		...
		CAM_ERR(CAM_ISP, "Invalid argument, g_num_ife_hws: %u",
			g_num_ife_hws);
}
...
void cam_vfe_get_num_ife_lites(uint32_t *num_ife_lites)
{
	if (num_ife_lites)
		*num_ife_lites = g_num_ife_lite_hws;
		...
		CAM_ERR(CAM_ISP, "Invalid argument, g_num_ife_lite_hws: %u",
			g_num_ife_lite_hws);
}
...
int cam_vfe_probe(struct platform_device *pdev)
{
	...
	if (strnstr(compatible_name, "lite", strlen(compatible_name)) != NULL)
		g_num_ife_lite_hws++;
	else if (strnstr(compatible_name, "vfe", strlen(compatible_name)) !=
		 NULL)
		g_num_ife_hws++;
	else
		CAM_ERR(CAM_ISP,
...
int cam_vfe_hw_init(struct cam_isp_hw_intf_data **vfe_hw_intf, uint32_t hw_idx)
{
	int rc = 0;
