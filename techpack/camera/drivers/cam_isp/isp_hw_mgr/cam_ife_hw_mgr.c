...
typedef int (*cam_isp_irq_inject_cmd_parse_handler)(
	struct cam_isp_irq_inject_param *irq_inject_param, uint32_t param_index,
	char *token, bool *is_query);
...
#define IRQ_INJECT_USAGE_STRING                                                     \
	...
	"    Also we need to cat the node to get output info after echo params to " \
	"node.\n\n"                                                                 \
	"  - echo ?:?:?:? > $INJECT_NODE\n"                                         \
	"    print query info, entering '?' to any param besides req_id to "        \
	"query.\n\n"                                                                \
	...
	"    req_id   : Req to trigger the IRQ, entering 'now' to this param will " \
	"trigger "                                                                  \
	"irq immediately\n\n"                                                       \
...
#define CAM_ISP_NON_RECOVERABLE_CSID_ERRORS                                    \
	(CAM_ISP_HW_ERROR_CSID_LANE_FIFO_OVERFLOW |                            \
	 ...
	 CAM_ISP_HW_ERROR_CSID_FATAL | CAM_ISP_HW_ERROR_CSID_UNBOUNDED_FRAME | \
...
static int cam_ife_mgr_find_core_idx(int split_id,
				     struct cam_ife_hw_mgr_ctx *ctx,
				     enum cam_isp_hw_type hw_type,
				     uint32_t *core_idx);

static int
cam_isp_blob_ife_clock_update(struct cam_isp_clock_config *clock_config,
			      struct cam_ife_hw_mgr_ctx *ctx);

static int
cam_isp_blob_sfe_clock_update(struct cam_isp_clock_config *clock_config,
			      struct cam_ife_hw_mgr_ctx *ctx);

static int cam_ife_hw_mgr_event_handler(void *priv, uint32_t evt_id,
					void *evt_info);

static int cam_ife_mgr_prog_default_settings(bool is_streamon,
					     struct cam_ife_hw_mgr_ctx *ctx);
...
static void *cam_ife_hw_mgr_get_hw_intf(struct cam_isp_ctx_base_info *base)
{
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			rc = cam_ife_mgr_find_core_idx(
				i, ctx, CAM_ISP_HW_TYPE_CSID, &csid_core_idx);
			if (rc)
				return rc;

			rc = cam_ife_mgr_find_core_idx(
				i, ctx, CAM_ISP_HW_TYPE_SFE, &sfe_core_idx);
			if (rc)
				return rc;

			rc = cam_cpas_csid_input_core_info_update(
				csid_core_idx, sfe_core_idx, set_port);
			if (rc) {
				CAM_ERR(CAM_PERF,
...
static int
cam_isp_blob_drv_config(struct cam_ife_hw_mgr_ctx *ctx, uint64_t request_id,
			struct cam_isp_prepare_hw_update_data *prepare_hw_data)
{
	...
	if (debug_drv)
		CAM_INFO(
			CAM_PERF,
			"DRV config blob opcode:%u req_id:%llu disable_drv_override:%s "
			"ctx_idx:%u drv_en:%s path_idle_en:0x%x timeout_val:%u",
	...
	CAM_DBG(CAM_PERF,
		"DRV config blob opcode:%u req_id:%llu disable_drv_override:%s "
		"ctx_idx:%u drv_en:%u path_idle_en:0x%x timeout_val:%u",
		prepare_hw_data->packet_opcode_type, request_id,
	...
	if (!g_ife_hw_mgr.cam_ddr_drv_support ||
	    g_ife_hw_mgr.debug_cfg.disable_isp_drv)
		return rc;
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if (hw_intf && hw_intf->hw_ops.process_cmd) {
			rc = hw_intf->hw_ops.process_cmd(
				hw_intf->hw_priv, CAM_ISP_HW_CMD_DRV_CONFIG,
				&drv_config_args,
...
static int cam_ife_mgr_finish_clk_bw_update(struct cam_ife_hw_mgr_ctx *ctx,
					    uint64_t request_id,
					    bool skip_clk_data_rst)
{
	...
	clk_bw_args.is_drv_config_en =
		(bool)(ctx->drv_path_idle_en & CAM_ISP_PXL_PATH);

	for (i = 0; i < ctx->num_base; i++) {
		clk_bw_args.hw_intf = NULL;
		clk_bw_args.clock_updated = false;
		CAM_DBG(CAM_PERF,
			"Clock/BW Update for ctx:%u req:%d i:%d num_vfe_out:%d "
			"num_sfe_out:%d in_rd:%d",
			ctx->ctx_index, request_id, i, ctx->num_acq_vfe_out,
			ctx->num_acq_sfe_out,
		...
		if ((ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) &&
		    (ctx->num_acq_vfe_out ||
		     (!list_empty(&ctx->res_list_ife_in_rd))))
			clk_bw_args.hw_intf =
				g_ife_hw_mgr.ife_devices[ctx->base[i].idx]
					->hw_intf;
		else if ((ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE) &&
			 (ctx->num_acq_sfe_out ||
			  (!list_empty(&ctx->res_list_ife_in_rd))))
			clk_bw_args.hw_intf =
				g_ife_hw_mgr.sfe_devices[ctx->base[i].idx]
					->hw_intf;
		else
			continue;

		CAM_DBG(CAM_PERF,
			"Apply Clock/BW for ctx:%u req:%d i:%d hw_idx=%d hw_type:%d "
			"inline:%s num_vfe_out:%d num_sfe_out:%d in_rd:%d",
			ctx->ctx_index, request_id, i,
			clk_bw_args.hw_intf->hw_idx,
			clk_bw_args.hw_intf->hw_type,
			CAM_BOOL_TO_YESNO(clk_bw_args.is_drv_config_en),
			ctx->num_acq_vfe_out, ctx->num_acq_sfe_out,
			!list_empty(&ctx->res_list_ife_in_rd));
		rc = clk_bw_args.hw_intf->hw_ops.process_cmd(
			clk_bw_args.hw_intf->hw_priv,
			CAM_ISP_HW_CMD_APPLY_CLK_BW_UPDATE, &clk_bw_args,
			sizeof(struct cam_isp_apply_clk_bw_args));
		if (rc) {
			CAM_ERR(CAM_PERF,
				"Finish Clock/BW Update failed ctx:%u req:%d i:%d hw_id=%d "
				"hw_type:%d rc:%d",
	...
	if (g_ife_hw_mgr.cam_clk_drv_support) {
		...
		for (i = 0; i < CAM_CESTA_MAX_CLIENTS; i++) {
			...
			rc = cam_soc_util_cesta_channel_switch(
				i, "ife_hw_mgr_update");
			if (rc) {
				CAM_ERR(CAM_CSIPHY,
...
static inline int __cam_ife_mgr_get_hw_soc_info(
	struct list_head *res_list, enum cam_isp_hw_split_id split_id,
	enum cam_isp_hw_type hw_type, struct cam_hw_soc_info **soc_info_ptr)
{
	int rc = -EINVAL;
	...
	list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp, res_list, list) {
		...
		if (hw_intf && hw_intf->hw_ops.process_cmd) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					...
					hw_type, hw_mgr_res->res_id, split_id,
					rc);
				return rc;
			}
					...
					uint32_t *reg_base_idx)
{
	...
	*soc_info_ptr = NULL;
	...
	case CAM_REG_DUMP_BASE_TYPE_ISP_LEFT:
		rc = __cam_ife_mgr_get_hw_soc_info(&ctx->res_list_ife_src,
	...
	case CAM_REG_DUMP_BASE_TYPE_CSID_LEFT:
		rc = __cam_ife_mgr_get_hw_soc_info(&ctx->res_list_ife_csid,
						   CAM_ISP_HW_SPLIT_LEFT,
	...
	case CAM_REG_DUMP_BASE_TYPE_SFE_LEFT:
		rc = __cam_ife_mgr_get_hw_soc_info(&ctx->res_list_sfe_src,
						   CAM_ISP_HW_SPLIT_LEFT,
						   CAM_ISP_HW_TYPE_SFE,
						   soc_info_ptr);
		if (rc)
	...
	case CAM_REG_DUMP_BASE_TYPE_SFE_RIGHT:
		...
		*reg_base_idx = 0;
		break;
	default:
		CAM_ERR(CAM_ISP, "Unrecognized reg base type: %u, ctx_idx: %u",
...
cam_ife_mgr_handle_reg_dump(struct cam_ife_hw_mgr_ctx *ctx,
			    ...
			    void *soc_dump_args, bool user_triggered_dump)
{
	int rc = 0, i;
	...
	if (cam_presil_mode_enabled()) {
		if (g_ife_hw_mgr.debug_cfg.enable_presil_reg_dump) {
			...
			CAM_ERR(CAM_ISP,
				"regdump disabled by default for presil mode, ctx: %u",
				ctx->ctx_index);
			return 0;
		}
	}
	...
	if (!num_reg_dump_buf || !reg_dump_buf_desc) {
		CAM_DBG(CAM_ISP,
			"Invalid args for reg dump req_id: [%llu] ctx idx: [%u] meta_type: "
			"[%u] num_reg_dump_buf: [%u] reg_dump_buf_desc: [%pK]",
	...
	for (i = 0; i < num_reg_dump_buf; i++) {
		CAM_DBG(CAM_ISP,
			"Reg dump cmd meta data: %u req_type: %u ctx_idx: %u",
		...
		if (reg_dump_buf_desc[i].meta_data == meta_type) {
			rc = cam_soc_util_reg_dump_to_cmd_buf(
				...
				cam_ife_mgr_regspace_data_cb, soc_dump_args,
				user_triggered_dump);
			if (rc) {
				CAM_ERR(CAM_ISP,
...
static inline void cam_ife_mgr_update_hw_entries_util(
	enum cam_isp_cdm_bl_type cdm_bl_type, uint32_t total_used_bytes,
	struct cam_kmd_buf_info *kmd_buf_info,
	struct cam_hw_prepare_update_args *prepare, bool precheck_combine)
{
	bool combine;
	uint32_t num_ent;
   ...
   */
	...
	if (combine) {
		...
		prev_hw_update_entry =
			&(prepare->hw_update_entries[num_ent - 1]);
		if (prev_hw_update_entry->flags != cdm_bl_type)
			combine = false;
	}
	cam_isp_update_hw_entry(cdm_bl_type, prepare, kmd_buf_info,
				total_used_bytes, combine);
}

static inline int
cam_ife_mgr_allocate_cdm_cmd(bool is_sfe_en,
			     struct cam_cdm_bl_request **cdm_cmd)
{
	...
	*cdm_cmd = kzalloc(((sizeof(struct cam_cdm_bl_request)) +
			    ((cfg_max - 1) * sizeof(struct cam_cdm_bl_cmd))),
			   GFP_KERNEL);
...
static inline void cam_ife_mgr_free_cdm_cmd(struct cam_cdm_bl_request **cdm_cmd)
{
	kfree(*cdm_cmd);
	*cdm_cmd = NULL;
}
...
static int cam_ife_mgr_get_hw_caps(void *hw_mgr_priv, void *hw_caps_args)
{
	...
	for (i = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		...
		if (hw_mgr->ife_dev_caps[i].is_lite) {
			...
			if (ife_full_hw_info == NULL) {
				...
				ife_full_hw_info->hw_type =
					(hw_mgr->isp_device_type ==
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		ife_csid_caps =
			(struct cam_ife_csid_hw_caps *)&hw_mgr->csid_hw_caps[i];
...
static inline int cam_ife_hw_mgr_is_sfe_rdi_for_fetch(uint32_t res_id)
{
	int rc = 0;
...
static inline int cam_ife_hw_mgr_is_shdr_fs_rdi_res(uint32_t res_id,
						    bool is_sfe_shdr,
						    bool is_sfe_fs)
{
...
static inline bool cam_ife_hw_mgr_is_sfe_out_port(uint32_t res_id)
{
	...
	if ((res_id >= CAM_ISP_SFE_OUT_RES_BASE) &&
	    (res_id < (CAM_ISP_SFE_OUT_RES_BASE + max_sfe_out_res)))
		is_sfe_out = true;
...
static int
cam_ife_hw_mgr_check_and_notify_overflow(struct cam_isp_hw_event_info *evt,
					 void *ctx, bool *is_bus_overflow)
{
	int i;
	int res_id;
	int ife_res_id = -1;
	...
	for (i = 0; i < hw_mgr_ctx->num_base; i++) {
		...
		if (hw_if->hw_ops.process_cmd) {
			...
			hw_if->hw_ops.process_cmd(
				hw_if->hw_priv, CAM_ISP_HW_NOTIFY_OVERFLOW,
				&overflow_info,
...
static enum cam_ife_pix_path_res_id
cam_ife_hw_mgr_get_csid_rdi_type_for_offline(uint32_t rd_res_type)
{
	...
	default:
		...
		CAM_ERR(CAM_ISP, "maximum rdi output type exceeded 0x%x",
			rd_res_type);
		break;
	}

	CAM_DBG(CAM_ISP, "out_port: %x path_id: %d", rd_res_type, path_id);

	return path_id;
}

static bool cam_ife_hw_mgr_is_sfe_rd_res(uint32_t sfe_in_path_type)
{
...
static int cam_ife_hw_mgr_reset_csid(struct cam_ife_hw_mgr_ctx *ctx,
...
static int cam_ife_hw_mgr_init_hw_res(struct cam_isp_hw_mgr_res *isp_hw_res)
{
	int i;
	int rc = -1;
	struct cam_hw_intf *hw_intf;
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		...
		CAM_DBG(CAM_ISP, "enabled vfe hardware %u", hw_intf->hw_idx);
...
static int cam_ife_mgr_csid_start_hw(struct cam_ife_hw_mgr_ctx *ctx,
				     ...
				     bool is_internal_start, bool start_only)
{
	...
	for (j = ctx->num_base - 1; j >= 0; j--) {
		...
		list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
			...
			if (!isp_res ||
			    ctx->base[j].idx != isp_res->hw_intf->hw_idx)
				continue;
		...
		if (cnt) {
			...
			start_args.is_drv_config_en =
				(bool)ctx->drv_path_idle_en;
...
static int cam_ife_hw_mgr_start_hw_res(struct cam_isp_hw_mgr_res *isp_hw_res,
				       struct cam_ife_hw_mgr_ctx *ctx)
{
	int i;
	int rc = -1;
...
static void cam_ife_hw_mgr_stop_hw_res(struct cam_isp_hw_mgr_res *isp_hw_res)
{
	int i;
	struct cam_hw_intf *hw_intf;
	uint32_t dummy_args;
...
static void cam_ife_hw_mgr_deinit_hw_res(struct cam_isp_hw_mgr_res *isp_hw_res)
{
	int i;
	struct cam_hw_intf *hw_intf;
...
static void cam_ife_hw_mgr_deinit_hw(struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (!ctx->flags.init_done) {
		CAM_WARN(CAM_ISP, "ctx is not in init state, ctx_idx: %u",
			 ctx->ctx_index);
		return;
	}
	...
	/*
   * Do not reset the curr_idx, it can be only be changed only it new SCID is
   * reserved for a particular exposure. Check if any cache needs to be
   * de-activated
   */
	for (i = CAM_LLCC_SMALL_1; i < CAM_LLCC_MAX; i++) {
		if (ctx->flags.sys_cache_usage[i]) {
			if (cam_cpas_is_notif_staling_supported() &&
				...
				rc = cam_cpas_notif_increment_staling_counter(
					i);
				if (rc)
					CAM_ERR(CAM_ISP,
	...
	for (i = 0; i < ctx->num_base; i++) {
		if (ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE)
			for (j = 0; j < CAM_ISP_EXPOSURE_MAX; j++)
				hw_mgr->sfe_cache_info[ctx->base[i].idx]
					.activated[j] = false;
	}
...
static int cam_ife_hw_mgr_init_hw(struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	/* INIT IFE SRC */
	CAM_DBG(CAM_ISP, "INIT IFE SRC in ctx id:%u", ctx->ctx_index);
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		rc = cam_ife_hw_mgr_init_hw_res(hw_mgr_res);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not INIT IFE SRC (%d), ctx_idx:%u",
				hw_mgr_res->res_id, ctx->ctx_index);
			goto deinit;
		}
	}
	...
	if (ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		for (i = 0; i < ctx->num_acq_sfe_out; i++) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"Can not INIT SFE OUT (%d), ctx_idx:%u",
	...
	CAM_DBG(CAM_ISP, "INIT IFE csid ... in ctx id:%u", ctx->ctx_index);

	/* INIT IFE BUS RD */
	CAM_DBG(CAM_ISP, "INIT IFE BUS RD in ctx id:%u", ctx->ctx_index);
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		rc = cam_ife_hw_mgr_init_hw_res(hw_mgr_res);
		if (rc) {
	...
	/* INIT IFE OUT */
	CAM_DBG(CAM_ISP, "INIT IFE OUT RESOURCES in ctx id:%u", ctx->ctx_index);

	for (i = 0; i < ctx->num_acq_vfe_out; i++) {
		rc = cam_ife_hw_mgr_init_hw_res(&ctx->res_list_ife_out[i]);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not INIT IFE OUT (%d), ctx_idx:%u",
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not INIT IFE CSID(id :%d), ctx_idx:%u",
				hw_mgr_res->res_id, ctx->ctx_index);
			goto deinit;
		}
	}
	...
	if (hw_mgr->csid_global_reset_en) {
		rc = cam_ife_hw_mgr_reset_csid(ctx, CAM_IFE_CSID_RESET_GLOBAL,
					       true);
		if (rc) {
			CAM_ERR(CAM_ISP, "CSID reset failed, ctx_idx:%u",
				ctx->ctx_index);
			goto deinit;
		}
	}
	...
	for (i = CAM_LLCC_SMALL_1; i < CAM_LLCC_MAX; i++) {
		if (ctx->flags.sys_cache_usage[i]) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"Failed to activate cache: %d, ctx_idx:%u",
					i, ctx->ctx_index);
				goto deinit;
			}
		}
				  ...
				  struct cam_isp_hw_mgr_res **res)
{
	...
	if (!list_empty(src_list)) {
		res_ptr = list_first_entry(src_list, struct cam_isp_hw_mgr_res,
					   list);
...
static int cam_ife_hw_mgr_free_hw_res(struct cam_isp_hw_mgr_res *isp_hw_res,
				      bool del_list)
{
	int rc = 0;
	int i;
...
static inline bool cam_ife_hw_mgr_check_path_port_compat(uint32_t in_type,
							 uint32_t out_type)
{
	int i;
	const struct cam_isp_hw_path_port_map *map =
		&g_ife_hw_mgr.path_port_map;
...
static void
cam_ife_hw_mgr_dump_acquire_resources(struct cam_ife_hw_mgr_ctx *hwr_mgr_ctx)
{
	...
	CAM_INFO(
		CAM_ISP,
		"**** %llu:%llu:%llu.%llu ctx_idx: %u rdi_only: %s is_dual: %s "
		"acquired ****",
	...
	list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(
					CAM_ISP,
					"CSID split_id:%d ctx_idx:%u hw_idx:%u res:%s type:%d "
					"res_id:%d state:%s",
					i, hwr_mgr_ctx->ctx_index,
					hw_res->hw_intf->hw_idx,
					hw_res->res_name, hw_res->res_type,
					hw_res->res_id,
	...
	list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(
					CAM_ISP,
					"IFE src split_id:%d ctx_idx:%u hw_idx:%u res:%s type:%d "
					"res_id:%d state:%s",
					i, hwr_mgr_ctx->ctx_index,
					hw_res->hw_intf->hw_idx,
					hw_res->res_name, hw_res->res_type,
	...
	list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(
					CAM_ISP,
					"SFE src split_id:%d ctx_idx:%u hw_idx:%u res:%s type:%d "
					"res_id:%d state:%s",
					i, hwr_mgr_ctx->ctx_index,
					hw_res->hw_intf->hw_idx,
					hw_res->res_name, hw_res->res_type,
	...
	list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(
					CAM_ISP,
					"src_rd split_id:%d ctx_idx:%u hw_idx:%u res:%s type:%d "
					"res_id:%d state:%s",
					i, hwr_mgr_ctx->ctx_index,
					hw_res->hw_intf->hw_idx,
					hw_res->res_name, hw_res->res_type,
	...
	for (i = 0; i < hwr_mgr_ctx->num_acq_vfe_out; i++) {
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(
					CAM_ISP,
					"IFE out split_id:%d ctx_idx:%u hw_idx:%u res:%s type:%d "
					"res_id:%d state:%s",
	...
	for (i = 0; i < hwr_mgr_ctx->num_acq_sfe_out; i++) {
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(
					CAM_ISP,
					"SFE out split_id:%d ctx_idx:%u hw_idx:%u res:%s type:%d "
					"res_id:%d state:%s",
					j, hwr_mgr_ctx->ctx_index,
					hw_res->hw_intf->hw_idx,
					hw_res->res_name, hw_res->res_type,
...
static void cam_ife_hw_mgr_dump_acq_rsrc_for_all_ctx(void)
{
	...
	list_for_each_entry_safe(ctx, ctx_temp, &g_ife_hw_mgr.used_ctx_list,
				 list) {
		CAM_INFO_RATE_LIMIT(
			CAM_ISP, "ctx id:%u is_dual:%d num_base:%d rdi only:%d",
			ctx->ctx_index, ctx->flags.is_dual, ctx->num_base,
...
static void
cam_ife_hw_mgr_print_acquire_info(struct cam_ife_hw_mgr_ctx *hw_mgr_ctx,
				  uint32_t num_pix_port, uint32_t num_pd_port,
				  uint32_t num_rdi_port, int acquire_failed)
{
	struct cam_isp_hw_mgr_res *hw_mgr_res = NULL;
	struct cam_isp_resource_node *hw_res = NULL;
	char log_info[128];
	...
	if (hw_mgr_ctx->flags.is_dual) {
		len += scnprintf(log_info + len, (128 - len),
				 "Dual IFE[%d: %d]",
	...
	if (hw_mgr_ctx->is_hw_ctx_acq) {
		...
		for (i = 0; i < CAM_ISP_MULTI_CTXT_MAX; i++) {
			...
			len += scnprintf(
				log_info + len, (128 - len), " [%d:0x%x]", i,
				hw_mgr_ctx->acq_hw_ctxt_src_dst_map[i]);
		}
	}
	...
	CAM_INFO(
		CAM_ISP,
		"%llu:%llu:%llu.%llu Acquired %s with [%u pix] [%u pd] [%u rdi] "
		"ports for ctx:%u",
		hrs, min, sec, ms, log_info, num_pix_port, num_pd_port,
		num_rdi_port, hw_mgr_ctx->ctx_index);

	return;

fail:
	CAM_ERR(CAM_ISP,
		"Failed to acquire %s-IFE/SFE with [%u pix] [%u pd] [%u rdi] ports "
		"for ctx_idx:%u",
		(hw_mgr_ctx->flags.is_dual) ? "dual" : "single", num_pix_port,
		num_pd_port, num_rdi_port, hw_mgr_ctx->ctx_index);
	CAM_INFO(CAM_ISP,
		 "Previously acquired IFEs[%d %d] SFEs[%d %d], ctx_idx: %u",
		 hw_idx[CAM_ISP_HW_SPLIT_LEFT], hw_idx[CAM_ISP_HW_SPLIT_RIGHT],
		 sfe_hw_idx[CAM_ISP_HW_SPLIT_LEFT],
		 sfe_hw_idx[CAM_ISP_HW_SPLIT_RIGHT], hw_mgr_ctx->ctx_index);
	...
	if (hw_mgr_ctx->is_hw_ctx_acq)
		CAM_INFO(
			...
			hw_mgr_ctx
				->acq_hw_ctxt_src_dst_map[CAM_ISP_MULTI_CTXT_0],
			CAM_ISP_MULTI_CTXT_1,
			hw_mgr_ctx
				->acq_hw_ctxt_src_dst_map[CAM_ISP_MULTI_CTXT_1],
			CAM_ISP_MULTI_CTXT_2,
			hw_mgr_ctx
				->acq_hw_ctxt_src_dst_map[CAM_ISP_MULTI_CTXT_2]);

	cam_ife_hw_mgr_dump_acq_rsrc_for_all_ctx();
}

static int
cam_ife_mgr_csid_change_halt_mode(struct cam_ife_hw_mgr_ctx *ctx,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if ((isp_res->res_type == CAM_ISP_RESOURCE_PIX_PATH) &&
				...
				rc = hw_intf->hw_ops.process_cmd(
					...
					&halt,
					sizeof(struct cam_ife_csid_hw_halt_args));
				if (rc)
					CAM_ERR(CAM_ISP,
				      ...
				      enum cam_isp_hw_type hw_type)
{
	...
	if (!ctx->num_base) {
		...
		ctx->num_base++;
		CAM_DBG(CAM_ISP,
			"Add split id = %d ctx_idx: %u for base idx = %d num_base=%d "
			"hw_type=%d",
			split_id, ctx->ctx_index, base_idx, ctx->num_base,
			hw_type);
	} else {
		...
		if (i == ctx->num_base) {
			...
			CAM_DBG(CAM_ISP,
				...
				split_id, ctx->ctx_index, base_idx,
				ctx->num_base, hw_type);
		}
	}
}
...
static int cam_ife_mgr_process_base_info(struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	CAM_DBG(CAM_ISP, "ctx base num = %d, ctx_idx: %u", ctx->num_base,
		ctx->ctx_index);
...
static int cam_ife_mgr_share_sof_qtimer_addr(struct cam_ife_hw_mgr_ctx *ctx)
{
   ...
   */
	...
	if (cam_isp_is_ctx_primary_rdi(ctx)) {
		for (i = 0; i < ctx->num_acq_vfe_out && !res_rdi_context_set;
		     i++) {
	...
	rc = hw_intf->hw_ops.process_cmd(
		hw_intf->hw_priv, CAM_ISP_HW_CMD_GET_SET_PRIM_SOF_TS_ADDR,
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		CAM_DBG(CAM_ISP,
			"i = %d, ctx: %d out_res_id = %d, out_port: %d", i,
			ife_ctx->ctx_index, res_id, out_port->res_type);
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		if ((in_port->major_ver == 3) &&
			...
			ife_out_res = &ife_ctx->res_list_ife_out
					       [ife_ctx->vfe_out_map[k]];
			if (ife_out_res) {
		...
		ife_out_res =
			&ife_ctx->res_list_ife_out[ife_ctx->num_acq_vfe_out];
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	CAM_DBG(CAM_ISP, "sfe_in_res_id: 0x%x sfe_out_res_id: 0x%x ctx_idx: %u",
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		sfe_out_res =
			&ife_ctx->res_list_sfe_out[ife_ctx->num_acq_sfe_out];
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			sfe_out_res->hw_res[j] = sfe_acquire.sfe_out.rsrc_node;
			if (j == CAM_ISP_HW_SPLIT_LEFT) {
				index = sfe_acquire.sfe_out.comp_grp_id;
				comp_grp = &ife_ctx->sfe_bus_comp_grp[index];
				comp_grp->res_id[comp_grp->num_res] =
...
static int
cam_ife_hw_mgr_acquire_res_sfe_out(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_generic_info *in_port)
{
	int rc = -EINVAL;
	...
	list_for_each_entry(sfe_res_iterator, &ife_ctx->res_list_sfe_src,
			    list) {
		...
		case CAM_ISP_HW_SFE_IN_RDI2:
			...
			rc = cam_ife_hw_mgr_acquire_res_sfe_out_rdi(
				ife_ctx, sfe_res_iterator, in_port);
			if (rc)
				goto err;
				   ...
				   struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (list_empty(&ife_ctx->res_list_ife_src)) {
		CAM_WARN(CAM_ISP, "IFE src list empty, ctx_idx: %u",
			 ife_ctx->ctx_index);
		return 0;
	}
...
static inline void cam_ife_mgr_count_functional_ife(void)
{
	...
	CAM_DBG(CAM_ISP, "counted functional %d IFE and %d IFE lite",
		g_num_ife_functional, g_num_ife_lite_functional);
}
...
static int cam_convert_hw_idx_to_sfe_hw_num(int hw_idx)
{
	if (hw_idx < g_num_sfe_available) {
		switch (hw_idx) {
		case 0:
			return CAM_ISP_SFE0_HW;
		case 1:
			return CAM_ISP_SFE1_HW;
		}
	} else {
...
static int cam_convert_sfe_res_to_path(int res_id)
{
	...
	default:
		CAM_ERR(CAM_ISP, "SFE res path invalid res_id: 0x%x", res_id);
	}
...
static int
cam_ife_hw_mgr_acquire_sfe_hw(bool use_lower_idx,
			      struct cam_ife_hw_mgr_ctx *ife_ctx,
			      struct cam_sfe_acquire_args *sfe_acquire)
{
	int i, rc = -EINVAL;
...
static int cam_ife_hw_mgr_acquire_res_sfe_src_util(
	...
	struct cam_isp_in_port_generic_info *in_port,
	struct cam_isp_hw_mgr_res *csid_res,
	struct cam_sfe_acquire_args *sfe_acquire, bool is_rdi,
	uint32_t *acquired_hw_id, uint32_t *acquired_hw_path)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Ctx : %u failed to get res",
			ife_ctx->ctx_index);
		return rc;
	}
	...
	for (i = sfe_src_res->is_dual_isp; i >= 0; i--) {
		...
		*acquired_hw_id |= cam_convert_hw_idx_to_sfe_hw_num(
...
static bool cam_ife_hw_mgr_is_csid_rdi_sfe_rdi_out(
	struct cam_isp_in_port_generic_info *in_port, uint32_t res_id)
{
	struct cam_isp_out_port_generic_info *out_port;
	uint32_t i = 0;
...
static int
cam_ife_hw_mgr_acquire_res_sfe_src(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_generic_info *in_port,
				   ...
				   uint32_t *acquired_hw_path)
{
     ...
     */
		if (cam_ife_hw_mgr_is_csid_rdi_sfe_rdi_out(in_port,
							   csid_res->res_id)) {
     ...
     */
		if ((!(sfe_required_res & BIT(CAM_ISP_HW_SFE_IN_PIX))) &&
		    (!in_port->ife_rd_count ||
		     in_port->dynamic_hdr_switch_en) &&
		    (BIT(csid_res->res_id) ==
		     hw_mgr->csid_hw_caps[0].sfe_ipp_input_rdi_res)) {
			sfe_required_res |= BIT(CAM_ISP_HW_SFE_IN_PIX);
			csid_res_map[CAM_ISP_HW_SFE_IN_PIX] = csid_res;
		}
	}

	CAM_DBG(CAM_ISP, "ctx: %u Required SFE in resources: 0x%x",
		ife_ctx->ctx_index, sfe_required_res);
	...
	while (sfe_required_res) {
		...
		default:
			CAM_ERR(CAM_ISP,
				"Wrong CSID Path Resource ID: %u ctx_idx: %u",
				sfe_res_id, ife_ctx->ctx_index);
			goto err;
		}

		sfe_acquire.sfe_in.res_id = sfe_res_id;
		rc = cam_ife_hw_mgr_acquire_res_sfe_src_util(
			ife_ctx, in_port, csid_res_map[sfe_res_id],
			&sfe_acquire, is_rdi, acquired_hw_id, acquired_hw_path);
		if (rc)
			goto err;
		sfe_acquired_res |= sfe_res_id;
	}

	CAM_DBG(CAM_ISP, "ctx: %u Acquired SFE in resources: 0x%x",
		ife_ctx->ctx_index, sfe_acquired_res);
	return 0;
	...
	struct cam_ife_hw_mgr_ctx *ife_ctx)
{
	...
	CAM_DBG(CAM_ISP,
		...
		"ctx_idx: %u",
		csid_acquire->in_port->can_use_lite,
		ife_ctx->flags.is_rdi_only_context, can_use_lite,
		csid_acquire->res_id, ife_ctx->ctx_index);
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (ife_ctx->flags.is_dual) {
		...
		if (j == CAM_IFE_HW_NUM_MAX ||
		    !vfe_acquire.vfe_bus_rd.rsrc_node) {
			CAM_ERR(CAM_ISP,
				"Failed to acquire VFE:%d BUS RD for RIGHT, ctx: %u",
				j, ife_ctx->ctx_index);
			goto end;
		}
	}
...
static int
cam_ife_hw_mgr_acquire_sfe_bus_rd(struct cam_ife_hw_mgr_ctx *ife_ctx,
				  struct cam_isp_in_port_generic_info *in_port)
{
	int rc = -1, i;
   ...
   */
	...
	if (!sfe_acquire.sfe_rd.rsrc_node || rc) {
		CAM_ERR(CAM_ISP, "Failed to acquire SFE RD: 0x%x, ctx_idx: %u",
			in_port->sfe_in_path_type, ife_ctx->ctx_index);
		goto put_res;
	}
...
acquire_successful:
	CAM_DBG(CAM_ISP,
		"SFE RD left [%u] acquired success for path: %u is_dual: %d res: %s "
		"res_id: 0x%x ctx_idx: %u",
		sfe_rd_res->hw_res[0]->hw_intf->hw_idx,
		in_port->sfe_in_path_type, in_port->usage_type,
		sfe_rd_res->hw_res[0]->res_name, sfe_rd_res->hw_res[0]->res_id,
		ife_ctx->ctx_index);
	...
	if (in_port->usage_type && acquired_cnt == 1) {
		...
		CAM_DBG(CAM_ISP,
			...
			sfe_rd_res->hw_res[1]->hw_intf->hw_idx,
			in_port->sfe_in_path_type, ife_ctx->ctx_index);
	}
	...
	uint32_t *acquired_hw_id, uint32_t *acquired_hw_path)
{
	...
	if (ife_ctx->flags.is_dual) {
		...
		vfe_acquire.vfe_in.dual_hw_idx = ife_ctx->right_hw_idx;
	} else
		vfe_acquire.vfe_in.sync_mode = CAM_ISP_HW_SYNC_NONE;

	vfe_acquire.vfe_in.is_dual = (uint32_t)ife_ctx->flags.is_dual;
	...
	if (i == CAM_IFE_HW_NUM_MAX || rc || !vfe_acquire.vfe_in.rsrc_node) {
		CAM_ERR(CAM_ISP,
	...
	*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(hw_intf->hw_idx);
	...
	CAM_DBG(CAM_ISP,
		...
		"ctx_idx: %u",
		hw_intf->hw_idx, ife_src_res->hw_res[0]->res_type,
	...
	if (ife_ctx->flags.is_dual) {
		...
		*acquired_hw_id |=
		...
		CAM_DBG(CAM_ISP,
			...
			"ctx_idx: %u",
			hw_intf->hw_idx, ife_src_res->hw_res[1]->res_type,
...
static int cam_ife_hw_mgr_acquire_res_ife_src(
	struct cam_ife_hw_mgr_ctx *ife_ctx,
	struct cam_isp_in_port_generic_info *in_port, bool acquire_lcr,
	bool acquire_ppp, uint32_t *acquired_hw_id, uint32_t *acquired_hw_path)
{
	int rc = -1;
	int i;
	struct cam_isp_hw_mgr_res *csid_res;
...
skip_get_ife_src_res:
		...
		case CAM_IFE_PIX_PATH_RES_IPP_2:
			if (!acquire_lcr) {
				vfe_acquire.vfe_in.res_id =
					CAM_ISP_HW_VFE_IN_CAMIF;
				vfe_acquire.vfe_in.hw_ctxt_mask =
					in_port->ipp_dst_hw_ctxt_mask;
			} else {
				vfe_acquire.vfe_in.res_id =
		...
		default:
			CAM_ERR(CAM_ISP,
				"Wrong IFE CSID Path Resource ID : %d, ctx_idx: %u",
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(
			...
			if (i >= CAM_MAX_HW_SPLIT) {
				CAM_ERR(CAM_ISP,
					"HW split is invalid: %d, ctx_idx: %u",
					i, ife_ctx->ctx_index);
				return -EINVAL;
			}
			...
			CAM_DBG(CAM_ISP,
				"acquire success IFE:%d ctx_idx: %u res type :0x%x res: %s res "
				"id:0x%x",
				hw_intf->hw_idx, ife_ctx->ctx_index,
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (!ife_ctx || !csid_acquire) {
		CAM_ERR(CAM_ISP, "Invalid args ife hw ctx %pK csid_acquire %pK",
	...
	if (ife_ctx->ctx_type != CAM_IFE_CTX_TYPE_SFE)
		can_use_lite =
			cam_ife_mgr_check_can_use_lite(csid_acquire, ife_ctx);
	...
	list_for_each_entry(csid_res_iterator, &ife_ctx->res_list_ife_csid,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			csid_caps = &ife_hw_mgr->csid_hw_caps[hw_intf->hw_idx];

			if (csid_caps->is_lite && !can_use_lite) {
				CAM_DBG(CAM_ISP,
					"CSID[%u] cannot use lite, ctx_idx: %u",
					hw_intf->hw_idx, ife_ctx->ctx_index);
				continue;
			}
	...
	if (compat_count == busy_count)
		CAM_ERR(CAM_ISP, "all compatible CSIDs are busy, ctx_idx: %u",
			ife_ctx->ctx_index);
...
static bool
cam_ife_hw_mgr_is_need_csid_ipp(struct cam_ife_hw_mgr_ctx *ife_ctx,
				struct cam_isp_in_port_generic_info *in_port)
{
	struct cam_ife_hw_mgr *hw_mgr;
	bool need = true;
	...
	CAM_DBG(CAM_ISP, "Need CSID PIX %u, Ctx_type: %u, ctx_idx: %u", need,
		ife_ctx->ctx_type, ife_ctx->ctx_index);
	return need;
}
	...
	enum cam_ife_pix_path_res_id *path_res_id)
{
	...
	case CAM_ISP_PXL2_PATH:
		*path_res_id = CAM_IFE_PIX_PATH_RES_IPP_2;
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid csid path ID: 0x%x",
			in_port->path_id);
		return -EINVAL;
	}
	...
	bool crop_enable)
{
	...
	CAM_DBG(CAM_ISP, "CSID Acquire: Enter, ctx_idx: %u",
		ife_ctx->ctx_index);
	...
	for (i = csid_res->is_dual_isp; i >= 0; i--) {
		...
		rc = cam_ife_hw_mgr_acquire_csid_hw(ife_ctx, &csid_acquire,
						    in_port);
		...
		CAM_DBG(CAM_ISP,
			"acquired csid(%s)=%d ctx_idx: %u pxl path rsrc %s successfully, "
			"is_secure: %s",
			(i == 0) ? "left" : "right", hw_intf->hw_idx,
			ife_ctx->ctx_index, (is_ipp) ? "IPP" : "PPP",
	...
	if (csid_res->is_dual_isp && ife_ctx->flags.need_csid_top_cfg) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			rc = hw_intf->hw_ops.process_cmd(
				...
				&dual_sync_args,
				sizeof(struct cam_ife_csid_dual_sync_args));
		}
	}
...
cam_ife_hw_mgr_get_ife_csid_rdi_res_type(uint32_t out_port_type)
{
	...
	CAM_DBG(CAM_ISP, "out_port: 0x%x path_id: 0x%x", out_port_type,
		path_id);
...
static int cam_ife_hw_mgr_acquire_csid_rdi_util(
	struct cam_ife_hw_mgr_ctx *ife_ctx,
	struct cam_isp_in_port_generic_info *in_port, uint32_t path_res_id,
	struct cam_isp_out_port_generic_info *out_port)
{
	struct cam_isp_hw_mgr_res *csid_res;
	struct cam_csid_hw_reserve_resource_args csid_acquire;
	...
	if (out_port) {
		if (cam_ife_hw_mgr_is_shdr_fs_rdi_res(
			    out_port->res_type, ife_ctx->flags.is_sfe_shdr,
   ...
   */
	...
	if (csid_acquire.node_res == NULL) {
		CAM_ERR(CAM_ISP, "Acquire CSID RDI rsrc failed, ctx_idx: %u",
			ife_ctx->ctx_index);
	...
	uint32_t *acquired_rdi_res)
{
	...
	if (hw_mgr->csid_hw_caps[0].sfe_ipp_input_rdi_res &&
	    !in_port->usage_type)
		res_id = ffs(hw_mgr->csid_hw_caps[0].sfe_ipp_input_rdi_res) - 1;

	if ((res_id != CAM_IFE_PIX_PATH_RES_MAX) &&
	    (!(BIT(res_id) & (*acquired_rdi_res)))) {
		rc = cam_ife_hw_mgr_acquire_csid_rdi_util(ife_ctx, in_port,
							  res_id, NULL);
		if (rc) {
	...
	CAM_DBG(CAM_ISP, "Ctx: %u rdi_res:%d ctx_type %d rc %d",
		ife_ctx->ctx_index, res_id, ife_ctx->ctx_type, rc);
end:
	return rc;
}
	...
	uint32_t acquire_info_size)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		if (((uint8_t *)in_port + sizeof(struct cam_isp_in_port_info)) >
			...
			CAM_ERR(CAM_ISP, "Invalid size, ctx_idx: %u",
				ife_ctx->ctx_index);
			return -EINVAL;
		}
...
static bool cam_ife_mgr_check_for_sfe_rd(uint32_t sfe_in_path_type)
{
	...
	uint32_t acquire_info_size)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		if ((in_port->num_out_res >
		     (max_ife_out_res + max_sfe_out_res)) ||
		    (in_port->num_out_res <= 0)) {
			CAM_ERR(CAM_ISP,
				"Invalid num output res %u, ctx_idx: %u",
				in_port->num_out_res, ife_ctx->ctx_index);
		...
		CAM_DBG(CAM_ISP, "in_port%d res_type 0x%x ctx_idx: %u", i,
			in_port->res_type, ife_ctx->ctx_index);
		is_sfe_rd =
			cam_ife_mgr_check_for_sfe_rd(in_port->sfe_in_path_type);
		if (is_sfe_rd)
			ife_ctx->scratch_buf_info.num_fetches++;

		if ((!fetch_cfg) &&
		    ((in_port->res_type == CAM_ISP_IFE_IN_RES_RD) ||
		     (is_sfe_rd))) {
			ife_ctx->flags.is_fe_enabled = true;
			...
			if ((is_sfe_rd) && (!ife_ctx->flags.is_offline)) {
				...
				if (in_port->feature_flag &
				    CAM_ISP_SFE_FS_MODE_EN)
	...
	CAM_DBG(CAM_ISP,
		"is_fe_enabled %d is_offline %d sfe_fs %d sfe_shdr: %d "
		"num_sfe_fetches: %u ctx_idx: %u",
		ife_ctx->flags.is_fe_enabled, ife_ctx->flags.is_offline,
		ife_ctx->flags.is_sfe_fs, ife_ctx->flags.is_sfe_shdr,
...
static int cam_ife_hw_mgr_convert_out_port_to_csid_path(uint32_t port_id)
{
	...
	if (path_id >= CAM_IFE_PIX_PATH_RES_RDI_0 &&
	    path_id <= CAM_IFE_PIX_PATH_RES_RDI_4)
		return path_id;
			       ...
			       struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		if (cam_ife_hw_mgr_is_rdi_res(out_port->res_type)) {
			...
			in_port->rdi_count++;
		else if (cam_ife_hw_mgr_check_path_port_compat(
				 CAM_ISP_HW_VFE_IN_PDLIB, out_port->res_type))
			in_port->ppp_count++;
		else if (cam_ife_hw_mgr_check_path_port_compat(
				 CAM_ISP_HW_VFE_IN_LCR, out_port->res_type))
			in_port->lcr_count++;
		else {
			...
			if ((in_port->major_ver == 3) &&
			    (in_port->path_id &
			     (CAM_ISP_PXL_PATH | CAM_ISP_PXL1_PATH |
			      CAM_ISP_PXL2_PATH))) {
				CAM_DBG(CAM_ISP,
					"preprocess csid path resource: 0x%x, ipp_dst_hw_ctxt_mask: "
					"0x%x, outport ctxt_id: %d",
					in_port->path_id,
					in_port->ipp_dst_hw_ctxt_mask,
	...
	CAM_DBG(CAM_ISP,
		...
		in_port->ppp_count, in_port->ife_rd_count, in_port->lcr_count,
		in_port->sfe_port_count);
	...
	uint32_t *acquired_hw_id, uint32_t *acquired_hw_path)
{
	...
	rc = cam_ife_hw_mgr_get_res(&ife_ctx->free_res_list, &ife_src_res);
	if (rc) {
		CAM_ERR(CAM_ISP, "No free resource, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto end;
	}
	...
	if (i == CAM_IFE_HW_NUM_MAX || rc || !vfe_acquire.vfe_in.rsrc_node) {
		CAM_ERR(CAM_ISP,
			"Failed to acquire IFE LEFT rc: %d, ctx_idx: %u", rc,
			ife_ctx->ctx_index);
		goto put_res;
	}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	hw_intf =
		ife_hw_mgr
			->sfe_devices[sfe_bus_rd_res->hw_res[i]->hw_intf->hw_idx]
			->hw_intf;
	...
	if (ife_ctx->flags.is_dual) {
		...
		hw_intf = ife_hw_mgr
				  ->sfe_devices[sfe_bus_rd_res->hw_res[++i]
							->hw_intf->hw_idx]
				  ->hw_intf;
...
static int
cam_ife_mgr_acquire_hw_sfe_offline(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_generic_info *in_port,
				   uint32_t *acquired_hw_id,
				   uint32_t *acquired_hw_path)
{
	...
	rc = cam_ife_hw_mgr_acquire_sfe_bus_rd(ife_ctx, in_port);
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Acquire SFE BUS RD resource Failed, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto err;
	}

	rc = cam_ife_hw_mgr_acquire_offline_res_csid(ife_ctx, in_port);
	if (rc) {
		CAM_ERR(CAM_ISP,
	...
	rc = cam_ife_hw_mgr_acquire_offline_res_sfe(ife_ctx, in_port);
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Acquire SFE PIX SRC resource Failed, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto err;
	}
	...
	if (in_port->sfe_ife_enable) {
		if (in_port->ipp_count) {
			rc = cam_ife_hw_mgr_acquire_offline_res_ife_camif(
				ife_ctx, in_port, false, acquired_hw_id,
				acquired_hw_path);
			if (rc) {
				CAM_ERR(CAM_ISP,
		...
		if (in_port->lcr_count) {
			rc = cam_ife_hw_mgr_acquire_offline_res_ife_camif(
				ife_ctx, in_port, true, acquired_hw_id,
				acquired_hw_path);
			if (rc) {
				CAM_ERR(CAM_ISP,
...
static int
cam_ife_mgr_acquire_hw_ife_offline(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_generic_info *in_port,
				   uint32_t *acquired_hw_id,
				   uint32_t *acquired_hw_path)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Acquire IFE BUS RD resource Failed, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto err;
	}
	...
	uint32_t *acquired_hw_path)
{
	...
	if ((!in_port->ipp_count && !in_port->lcr_count) ||
		...
		CAM_ERR(CAM_ISP,
			...
			in_port->ife_rd_count, in_port->ipp_count,
			in_port->lcr_count, ife_ctx->ctx_index);
		return -EINVAL;
	}
...
static int cam_ife_mgr_acquire_hw_for_ctx(
	struct cam_ife_hw_mgr_ctx *ife_ctx,
	struct cam_isp_in_port_generic_info *in_port, uint32_t *acquired_hw_id,
	uint32_t *acquired_hw_path, uint32_t *acquired_rdi_res)
{
	int rc = -1;
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Can not acquire root resource, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto err;
	}

	if (!in_port->ipp_count && !in_port->rdi_count && !in_port->ppp_count &&
	    !in_port->lcr_count) {
		CAM_ERR(CAM_ISP,
	...
	if (in_port->ife_rd_count) {
		if (ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE)
			rc = cam_ife_hw_mgr_acquire_sfe_bus_rd(ife_ctx,
							       in_port);
		else
			rc = cam_ife_hw_mgr_acquire_res_ife_bus_rd(ife_ctx,
								   in_port);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	if (cam_ife_hw_mgr_is_need_csid_ipp(ife_ctx, in_port)) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Acquire IFE CSID IPP/LCR resource Failed, ctx_idx: %u",
				ife_ctx->ctx_index);
			goto err;
		}
	}
   ...
   */
	if (ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		rc = cam_ife_hw_mgr_get_csid_rdi_for_sfe_ipp_input(
			ife_ctx, in_port, acquired_rdi_res);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	if ((ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) &&
		...
		rc = cam_ife_hw_mgr_acquire_res_sfe_src(
			ife_ctx, in_port, acquired_hw_id, acquired_hw_path);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Acquire SFE SRC resource failed, ctx_idx: %u",
				ife_ctx->ctx_index);
			goto err;
		}
	}
	...
	if (in_port->ipp_count) {
		if (ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
			...
			rc = cam_ife_hw_mgr_acquire_res_ife_src(
				ife_ctx, in_port, false, false, acquired_hw_id,
				acquired_hw_path);
		}
	...
	if (in_port->ppp_count) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Acquire IFE PPP SRC resource Failed, ctx_idx: %u",
				ife_ctx->ctx_index);
			goto err;
		}
	}
	...
	if (ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Acquire SFE OUT resource Failed, ctx_idx: %u",
				ife_ctx->ctx_index);
			goto err;
		}
	}
			      ...
			      enum cam_cdm_cb_status status, void *cookie)
{
	...
	if (status == CAM_CDM_CB_STATUS_BL_SUCCESS) {
		...
		CAM_DBG(CAM_ISP,
			"CDM hdl=0x%x, udata=%pK, status=%d, cookie=%u ctx_index=%u "
			"cdm_req=%llu",
	...
	uint32_t *input_size, struct cam_isp_in_port_generic_info *in_port)
{
	...
	*input_size += in_port_length;
	...
	in_port->major_ver = (acquire_hw_info->input_info_version >> 16) &
			     0xFFFF;
...
static inline int cam_ife_mgr_hw_check_in_res_type(u32 res_type)
{
	switch (res_type) {
...
static inline void cam_ife_mgr_acquire_get_feature_flag_params_v3(
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	in_port->dynamic_sensor_switch_en = in->feature_mask &
	...
	in_port->dynamic_hdr_switch_en = in->feature_mask &
					 CAM_ISP_HDR_MODE_DYNAMIC_SWITCH_EN;
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	in_port->dynamic_hdr_switch_en = in->feature_flag &
					 CAM_ISP_HDR_MODE_DYNAMIC_SWITCH_EN;
}
	...
	uint32_t *input_size, struct cam_isp_in_port_generic_info *in_port)
{
	...
	in_port_length =
		sizeof(struct cam_isp_in_port_info_v3) +
		(in->num_out_res - 1) * sizeof(struct cam_isp_out_port_info_v3);
	...
	*input_size += in_port_length;
	...
	in_port->major_ver = (acquire_hw_info->input_info_version >> 8) &
			     0xFFFF;
	in_port->minor_ver = acquire_hw_info->input_info_version & 0xFFFF;
	...
	for (i = 0; i < in_port->num_valid_vc_dt; i++) {
		in_port->format[i] = (in->csid_info.format >>
				      (i * CAM_IFE_DECODE_FORMAT_SHIFT_VAL)) &
	...
	uint32_t *input_size, struct cam_isp_in_port_generic_info *in_port)
{
	...
	*input_size += in_port_length;
	...
	for (i = 0; i < in_port->num_valid_vc_dt; i++) {
		in_port->format[i] =
			(in->format >> (i * CAM_IFE_DECODE_FORMAT_SHIFT_VAL)) &
			CAM_IFE_DECODE_FORMAT_MASK;
	}
...
static inline void
cam_ife_mgr_reset_streamon_scratch_cfg(struct cam_ife_hw_mgr_ctx *ctx)
{
	ctx->scratch_buf_info.ife_scratch_config->skip_scratch_cfg_streamon =
		false;
	ctx->scratch_buf_info.sfe_scratch_config->skip_scratch_cfg_streamon =
		false;
...
static void
cam_ife_mgr_populate_hw_ctxt_map(struct cam_ife_hw_mgr_ctx *ife_ctx,
				 struct cam_isp_in_port_generic_info *in_port)
{
	int src_hw_ctxt_id;

	src_hw_ctxt_id =
		cam_ife_mgr_get_src_hw_ctxt_from_csid_path(in_port->path_id);
	if (src_hw_ctxt_id == -1)
		return;

	ife_ctx->acq_hw_ctxt_src_dst_map[src_hw_ctxt_id] =
		in_port->ipp_dst_hw_ctxt_mask;
}
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	if (!in_port) {
		CAM_ERR(CAM_ISP, "No memory available, ctx_idx: %u",
			ife_ctx->ctx_index);
		rc = -ENOMEM;
		goto free_ctx;
	}

	ife_ctx->vfe_bus_comp_grp =
		kcalloc(CAM_IFE_BUS_COMP_NUM_MAX,
	...
	ife_ctx->sfe_bus_comp_grp =
		kcalloc(CAM_SFE_BUS_COMP_NUM_MAX,
			sizeof(struct cam_isp_context_comp_record), GFP_KERNEL);
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		rc = cam_ife_mgr_acquire_get_unified_structure(
			acquire_hw_info, i, &input_size, &in_port[i]);
		...
		if (ife_ctx->flags.is_fe_enabled) {
			...
			for (j = 1; j < in_port[i].num_valid_vc_dt; j++) {
				if (in_port[i].format[j] !=
				    input_format_checker) {
					CAM_ERR(CAM_ISP,
						"Different input formats for FE use-cases not supported - "
						"formats vc0: %u vc%d: %u ctx_idx: %u",
	...
	ife_ctx->res_list_ife_out = kcalloc(
		total_ports, sizeof(struct cam_isp_hw_mgr_res), GFP_KERNEL);
	...
	if (!total_pix_port && !total_pd_port) {
		...
		CAM_DBG(CAM_ISP, "RDI only context, ctx_idx: %u",
			ife_ctx->ctx_index);
	}

	/* Check whether context has only RDI and PD resource */
	if (!total_pix_port && (total_pd_port && total_rdi_port)) {
	...
	if (total_sfe_ports) {
		ife_ctx->res_list_sfe_out =
			kcalloc(total_sfe_ports,
				sizeof(struct cam_isp_hw_mgr_res), GFP_KERNEL);
		memset(ife_ctx->sfe_out_map, 0xff,
		       sizeof(uint8_t) * max_sfe_out_res);
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		CAM_DBG(CAM_ISP,
			"in_res_type: 0x%x ctx_idx: %u sfe_in_path_type: 0x%x "
			"sfe_ife_enable: 0x%x",
			in_port[i].res_type, ife_ctx->ctx_index,
			in_port[i].sfe_in_path_type, in_port[i].sfe_ife_enable);
		...
		if (rc) {
			cam_ife_hw_mgr_print_acquire_info(
				...
				(in_port[i].ipp_count +
				 in_port[i].ife_rd_count +
				 in_port[i].lcr_count),
				in_port[i].ppp_count, in_port[i].rdi_count, rc);
			goto free_res;
		}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Process base info failed, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto free_res;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed to acquire the CDM HW, ctx_idx: %u",
			ife_ctx->ctx_index);
		goto free_cdm_cmd;
	}
	...
	CAM_DBG(CAM_ISP,
		...
		ife_ctx->ctx_index, cdm_acquire.id, cdm_acquire.handle,
		ife_ctx->flags.is_dual);
	...
	if (g_ife_hw_mgr.isp_caps.support_consumed_addr)
		acquire_args->op_flags |= CAM_IFE_CTX_CONSUME_ADDR_EN;

	if ((ife_ctx->flags.is_sfe_shdr) || (ife_ctx->flags.is_sfe_fs)) {
		acquire_args->op_flags |= CAM_IFE_CTX_APPLY_DEFAULT_CFG;
		ife_ctx->scratch_buf_info.sfe_scratch_config = kzalloc(
			sizeof(struct cam_sfe_scratch_buf_cfg), GFP_KERNEL);
		...
		ife_ctx->scratch_buf_info.ife_scratch_config = kzalloc(
			sizeof(struct cam_ife_scratch_buf_cfg), GFP_KERNEL);
	...
	if (ife_ctx->ctx_config & CAM_IFE_CTX_CFG_FRAME_HEADER_TS)
		acquire_args->op_flags |= CAM_IFE_CTX_FRAME_HEADER_EN;

	if (ife_ctx->ctx_config & CAM_IFE_CTX_CFG_DYNAMIC_SWITCH_ON)
		acquire_args->op_flags |= CAM_IFE_CTX_DYNAMIC_SWITCH_EN;

	if (ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE)
		acquire_args->op_flags |= CAM_IFE_CTX_SFE_EN;
...
static int cam_ife_mgr_acquire_dev(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		CAM_DBG(CAM_ISP, "ctx_idx: %u acquire no = %d total = %d",
			ife_ctx->ctx_index, i, acquire_args->num_acq);
		CAM_DBG(CAM_ISP,
		...
		if (!IS_ERR(in_port)) {
			...
			cam_ife_mgr_acquire_get_unified_dev_str(
				in_port, &gen_port_info[i]);
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		rc = cam_ife_mgr_acquire_hw_for_ctx(
			ife_ctx, &gen_port_info[i],
			&acquire_args->acquired_hw_id[i],
			acquire_args->acquired_hw_path[i], &acquired_rdi_res);
	...
	rc = cam_ife_mgr_allocate_cdm_cmd(false, &ife_ctx->cdm_cmd);
	if (rc)
		goto free_res;
	...
	struct cam_isp_prepare_hw_update_data *hw_update_data)
{
	...
	for (i = 0; i < bw_config->num_paths; i++) {
		CAM_INFO(
			CAM_PERF,
			"ctx_idx: %u ISP_BLOB usage_type=%u [%s] [%s] [%s] [%llu] [%llu] "
			"[%llu]",
...
ife_clk:
	...
	CAM_INFO(
		...
		ctx->ctx_index, ife_clock_config->usage_type,
		ife_clock_config->left_pix_hz, ife_clock_config->right_pix_hz);
...
sfe_clk:
	...
	CAM_INFO(
		...
		ctx->ctx_index, sfe_clock_config->usage_type,
		sfe_clock_config->left_pix_hz, sfe_clock_config->right_pix_hz);
...
static int
cam_isp_classify_vote_info(struct cam_isp_hw_mgr_res *hw_mgr_res,
			   struct cam_isp_bw_config_internal_v2 *bw_config,
			   struct cam_axi_vote *isp_vote, uint32_t hw_type,
			   uint32_t split_idx, bool *nrdi_l_bw_updated,
			   bool *nrdi_r_bw_updated, bool is_sfe_shdr)
{
	int rc = 0, i, j = 0;
	...
	if (hw_type == CAM_ISP_HW_TYPE_VFE) {
		...
		if ((hw_mgr_res->res_id >= CAM_ISP_HW_SFE_IN_RDI0) &&
		    (hw_mgr_res->res_id <= CAM_ISP_HW_SFE_IN_RDI4)) {
			for (i = 0; i < bw_config->num_paths; i++) {
				if ((bw_config->axi_path[i].usage_data ==
...
static int
cam_isp_blob_bw_update_v2(struct cam_isp_bw_config_internal_v2 *bw_config,
			  struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_isp_hw_mgr_res *hw_mgr_res;
	struct cam_hw_intf *hw_intf;
	...
	for (i = 0; i < bw_config->num_paths; i++) {
		CAM_DBG(CAM_PERF,
			"ctx_idx: %u ISP_BLOB usage_type=%u [%s] [%s] [%s] [%s] [%llu] [%llu] "
			"[%llu]",
			ctx->ctx_index, bw_config->usage_type,
			cam_isp_util_usage_data_to_string(
				bw_config->axi_path[i].usage_data),
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		for (split_idx = 0; split_idx < CAM_ISP_HW_SPLIT_MAX;
			...
			rc = cam_isp_classify_vote_info(
				hw_mgr_res, bw_config, &bw_upd_args.isp_vote,
				CAM_ISP_HW_TYPE_VFE, split_idx,
				&nrdi_l_bw_updated, &nrdi_r_bw_updated, false);
			if (rc)
				return rc;
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_sfe_src, list) {
		for (split_idx = 0; split_idx < CAM_ISP_HW_SPLIT_MAX;
			...
			memset(&sfe_bw_update_args.sfe_vote, 0,
			       sizeof(struct cam_axi_vote));
			rc = cam_isp_classify_vote_info(
				hw_mgr_res, bw_config,
				&sfe_bw_update_args.sfe_vote,
				CAM_ISP_HW_TYPE_SFE, split_idx,
         ...
         */
				rc = hw_intf->hw_ops.process_cmd(
					...
					CAM_ISP_HW_CMD_BW_UPDATE_V2,
					&sfe_bw_update_args,
					sizeof(struct cam_sfe_bw_update_args));
				if (rc)
					CAM_ERR(CAM_PERF,
						"BW Update failed rc: %d, ctx_idx: %u",
						rc, ctx->ctx_index);
				  ...
				  struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	CAM_DBG(CAM_PERF,
		"ctx_idx: %u ISP_BLOB usage=%u left cam_bw_bps=%llu ext_bw_bps=%llu, "
...
static void cam_ife_mgr_send_frame_event(uint64_t request_id,
					 uint32_t ctx_index)
{
	if (cam_presil_mode_enabled()) {
		CAM_DBG(CAM_PRESIL, "PRESIL FRAME req_id=%llu ctx_index %u",
			request_id, ctx_index);
		cam_presil_send_event(CAM_PRESIL_EVENT_IFE_FRAME_RUN,
				      request_id);
	}
}

static void
cam_isp_irq_inject_clear_params(struct cam_isp_irq_inject_param *param)
{
	param->hw_type = -1;
	param->hw_idx = -1;
	param->reg_unit = -1;
...
static int cam_ife_hw_mgr_sfe_irq_inject_or_dump_desc(
	struct cam_ife_hw_mgr *hw_mgr, struct cam_isp_irq_inject_param *params,
	bool dump_irq_desc)
{
	int i, rc = 0;
	char *line_buf = NULL;
	...
	for (i = 0; i < CAM_SFE_HW_NUM_MAX; i++) {
		...
		rc = hw_intf->hw_ops.process_cmd(
			hw_intf->hw_priv, CAM_ISP_HW_CMD_IRQ_INJECTION, params,
			sizeof(struct cam_isp_irq_inject_param));
		if (rc)
			scnprintf(
				line_buf, LINE_BUFFER_LEN,
...
clear_param:
	strlcat(irq_inject_display_buf, params->line_buf,
...
static int cam_ife_mgr_config_hw(void *hw_mgr_priv, void *config_hw_args)
{
   ...
   */
	...
	if (cfg->reapply_type && cfg->cdm_reset_before_apply) {
		if (ctx->last_cdm_done_req < cfg->request_id) {
			...
			CAM_ERR_RATE_LIMIT(
				CAM_ISP,
				"ctx_idx: %u CDM callback not received for req: %lld, "
				"last_cdm_done_req: %lld, is_cdm_hung: %d",
				ctx->ctx_index, cfg->request_id,
				ctx->last_cdm_done_req, is_cdm_hung);
			...
			CAM_ERR_RATE_LIMIT(
				CAM_ISP,
				"CDM callback received, should wait for buf done for "
				"req: %lld, ctx_idx: %u",
	...
	CAM_DBG(CAM_PERF,
		"ctx_idx=%u, bw_config_version=%d config_valid[BW VFE_CLK "
		"SFE_CLK]:[%d %d %d]",
	...
	if (hw_update_data->drv_config_valid) {
		rc = cam_isp_blob_drv_config(ctx, cfg->request_id,
					     hw_update_data);
		if (rc) {
			CAM_ERR(CAM_ISP,
   ...
   */
	if (hw_update_data->bw_clk_config.ife_clock_config_valid) {
		rc = cam_isp_blob_ife_clock_update(
			(struct cam_isp_clock_config *)&hw_update_data
				->bw_clk_config.ife_clock_config,
			ctx);
		if (rc) {
			CAM_ERR(CAM_PERF,
				"Clock Update Failed, rc=%d, ctx_idx=%u", rc,
				ctx->ctx_index);
			return rc;
		}
	}
	...
	if (hw_update_data->bw_clk_config.sfe_clock_config_valid) {
		rc = cam_isp_blob_sfe_clock_update(
			(struct cam_isp_clock_config *)&hw_update_data
				->bw_clk_config.sfe_clock_config,
			ctx);
		if (rc) {
			CAM_ERR(CAM_PERF,
				"Clock Update Failed, rc=%d, ctx_idx=%u", rc,
		...
		if (ctx->bw_config_version == CAM_ISP_BW_CONFIG_V1) {
			rc = cam_isp_blob_bw_update(
				(struct cam_isp_bw_config *)&hw_update_data
					->bw_clk_config.bw_config,
				ctx);
			if (rc) {
				CAM_ERR(CAM_PERF,
					"Bandwidth Update Failed rc: %d, ctx_idx=%u",
					rc, ctx->ctx_index);
				return rc;
			}
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			...
			rc, ctx->ctx_index);
		cam_ife_mgr_print_blob_info(ctx, cfg->request_id,
					    hw_update_data);
		return rc;
	}
...
skip_bw_clk_update:
	...
	if (cfg->num_hw_update_entries > 0) {
		...
		if (cam_presil_mode_enabled() || cfg->init_packet ||
		    hw_update_data->mup_en ||
			...
			if (rem_jiffies == 0) {
				...
				if (hw_update_data->mup_en) {
					ctx->current_mup =
						hw_update_data->mup_val;
					ctx->curr_num_exp =
						hw_update_data->num_exp;
				}
		...
		CAM_ERR(CAM_ISP, "No commands to config, ctx_index %u",
			ctx->ctx_index);
	}
...
static int cam_ife_mgr_stop_hw_in_overflow(void *stop_hw_args)
{
	...
	if (!ctx->num_base) {
		CAM_ERR(CAM_ISP, "Number of bases are zero, ctx_index %u",
			ctx->ctx_index);
		return -EINVAL;
	}
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				if (rc)
					CAM_ERR(CAM_ISP,
						"BW Update failed, ctx id:%u",
						ctx->ctx_index);
			} else
				CAM_WARN(CAM_ISP, "NULL hw_intf!, ctx id:%u",
					 ctx->ctx_index);
		}
	}
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	if (!ctx->num_base) {
		CAM_ERR(CAM_ISP, "number of bases are zero, ctx id:%u",
			ctx->ctx_index);
		return -EINVAL;
	}
   ...
   */
	...
	if (ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		CAM_DBG(CAM_ISP, "Going to stop SFE Out, ctx id:%u",
			ctx->ctx_index);
		...
		CAM_DBG(CAM_ISP, "Going to stop SFE SRC resources, ctx id:%u",
			ctx->ctx_index);
	...
	CAM_DBG(CAM_ISP, "Going to stop IFE out resources, ctx id:%u",
		ctx->ctx_index);
	...
	rem_jiffies = cam_common_wait_for_completion_timeout(
		&ctx->config_done_complete, msecs_to_jiffies(10));
	if (rem_jiffies == 0)
		CAM_WARN(
			CAM_ISP,
			"config done completion timeout for last applied req_id=%llu "
			"ctx_index %u",
	...
	cam_ife_hw_mgr_deinit_hw(ctx);
	CAM_DBG(CAM_ISP, "Stop success for ctx id:%u rc :%d", ctx->ctx_index,
		rc);
					 ...
					 uint32_t hw_idx)
{
	...
	for (i = 0; i < CAM_VFE_HW_NUM_MAX; i++) {
		...
		vfe_hw_intf->hw_ops.process_cmd(
			vfe_hw_intf->hw_priv, CAM_ISP_HW_CMD_UNMASK_BUS_WR_IRQ,
...
static int cam_ife_mgr_restart_hw(void *start_hw_args)
{
	...
	for (i = 0; i < ctx->num_acq_vfe_out; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not start IFE OUT (%d), ctx_idx: %u", i,
				ctx->ctx_index);
			goto err;
		}
	}
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not start IFE BUS RD (%d), ctx_idx: %u",
				hw_mgr_res->res_id, ctx->ctx_index);
			goto err;
		}
	}
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not start IFE MUX (%d), ctx_idx: %u",
				hw_mgr_res->res_id, ctx->ctx_index);
			goto err;
		}
	}
	...
	rc = cam_ife_mgr_csid_start_hw(ctx, CAM_IFE_PIX_PATH_RES_MAX, false,
				       false);
	if (rc) {
		CAM_ERR(CAM_ISP, "Error in starting CSID HW in ctx id:%u",
			ctx->ctx_index);
		goto err;
	}
...
static void cam_ife_hw_mgr_set_hw_debug_config(struct cam_ife_hw_mgr_ctx *ctx)
{
	int i, rc;
	uint32_t hw_idx = 0;
	...
	csid_debug_args.csid_rx_capture_debug =
		hw_mgr->debug_cfg.rx_capture_debug;
	csid_debug_args.rx_capture_debug_set =
		hw_mgr->debug_cfg.rx_capture_debug_set;
	...
	sfe_debug_args.u.dbg_cfg.sfe_sensor_sel =
		hw_mgr->debug_cfg.sfe_sensor_diag_cfg;
	sfe_debug_args.u.dbg_cfg.num_counters =
		hw_mgr->isp_caps.num_sfe_perf_counters;
	...
	vfe_debug_args.disable_ife_mmu_prefetch =
		hw_mgr->debug_cfg.disable_ife_mmu_prefetch;
	vfe_debug_args.enable_ife_frame_irqs =
		hw_mgr->debug_cfg.enable_ife_frame_irqs;
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		case CAM_ISP_HW_TYPE_VFE:
			if (hw_mgr->ife_devices[hw_idx]) {
				rc = hw_mgr->ife_devices[hw_idx]
					     ->hw_intf->hw_ops.process_cmd(
						     hw_mgr->ife_devices[hw_idx]
							     ->hw_intf->hw_priv,
						     CAM_ISP_HW_CMD_IFE_DEBUG_CFG,
						     &vfe_debug_args,
						     sizeof(vfe_debug_args));
				if (rc)
					CAM_DBG(CAM_ISP,
		...
		case CAM_ISP_HW_TYPE_SFE:
			if (hw_mgr->sfe_devices[hw_idx]) {
				rc = hw_mgr->sfe_devices[hw_idx]
					     ->hw_intf->hw_ops.process_cmd(
						     hw_mgr->sfe_devices[hw_idx]
							     ->hw_intf->hw_priv,
						     CAM_ISP_HW_CMD_SET_SFE_DEBUG_CFG,
						     &sfe_debug_args,
						     sizeof(sfe_debug_args));
				if (rc)
					CAM_DBG(CAM_ISP,
		...
		case CAM_ISP_HW_TYPE_CSID:
			if (hw_mgr->csid_devices[hw_idx]) {
				rc = hw_mgr->csid_devices[hw_idx]
					     ->hw_ops.process_cmd(
						     hw_mgr->csid_devices[hw_idx]
							     ->hw_priv,
	...
	if (ctx->flags.need_csid_top_cfg) {
		list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
			for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
				...
				rc = hw_intf->hw_ops.process_cmd(
					hw_intf->hw_priv,
					CAM_IFE_CSID_TOP_CONFIG, &csid_top_args,
				...
				CAM_DBG(CAM_ISP,
					"CSID: %u split_id: %d core_idx: %u core_type: %u "
					"is_sfe_offline: %d ctx_idx: %u",
					hw_intf->hw_idx, i,
					csid_top_args.core_idx,
					csid_top_args.input_core_type,
...
start_only:
	...
	if (ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		for (i = 0; i < ctx->num_acq_sfe_out; i++) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"Can not start SFE OUT (%d), ctx_idx: %u",
					i, ctx->ctx_index);
				goto err;
			}
		}
		...
		list_for_each_entry(hw_mgr_res, &ctx->res_list_sfe_src, list) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"Can not start SFE SRC (%d), ctx_idx: %u",
					hw_mgr_res->res_id, ctx->ctx_index);
				goto err;
			}
		}
	...
	CAM_DBG(CAM_ISP, "START BUS RD ... in ctx id:%u", ctx->ctx_index);
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		rc = cam_ife_hw_mgr_start_hw_res(hw_mgr_res, ctx);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not start BUS RD (%d), ctx_idx: %u",
				hw_mgr_res->res_id, ctx->ctx_index);
			goto err;
		}
	}
...
static int cam_ife_mgr_reset(void *hw_mgr_priv, void *hw_reset_args)
{
	...
	if (hw_mgr->csid_global_reset_en) {
		CAM_DBG(CAM_ISP, "Path reset not supported, ctx_idx: %u",
			ctx->ctx_index);
		return 0;
	}
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Failed to reset VFE:%d rc: %d ctx_idx: %u",
				ctx->base[i].idx, rc, ctx->ctx_index);
			goto end;
		}
	}
				    ...
				    struct cam_hw_prepare_update_args *prepare)
{
	...
	if ((prepare->num_hw_update_entries + 1) >=
		...
		CAM_ERR(CAM_ISP,
			"Insufficient HW entries :%d max:%d ctx_idx: %u",
	...
	case CAM_UBWC_CFG_VERSION_1:
		CAM_DBG(CAM_ISP, "ctx_idx: %u num_ports= %d", ctx->ctx_index,
		...
		for (i = 0; i < ubwc_config->num_ports; i++) {
			...
			CAM_DBG(CAM_ISP,
				...
				i, ubwc_plane_cfg->port_type, ctx->ctx_index);

			if (res_id_out >= max_ife_out_res) {
				CAM_ERR(CAM_ISP,
					"Invalid port type:%x, ctx_idx: %u",
			...
			hw_intf = cam_ife_hw_mgr_get_hw_intf(
				blob_info->base_info);

			if (!hw_intf || blob_info->base_info->split_id >=
						CAM_ISP_HW_SPLIT_MAX) {
				CAM_ERR(CAM_ISP, "Invalid base %u type %u",
			...
			rc = cam_isp_add_cmd_buf_update(
				hw_mgr_res
					->hw_res[blob_info->base_info->split_id],
				hw_intf, blob_type,
				blob_type_hw_cmd_map[blob_type],
				(void *)cmd_buf_addr, kmd_buf_remain_size,
				(void *)ubwc_plane_cfg, &bytes_used);
			if (rc < 0) {
				CAM_ERR(CAM_ISP,
					"Failed cmd_update, base_idx=%d, bytes_used=%u, "
					"res_id_out=0x%X ctx_idx: %u",
					blob_info->base_info->idx, bytes_used,
					ubwc_plane_cfg->port_type,
					ctx->ctx_index);
				goto end;
		...
		if (total_used_bytes) {
			cam_ife_mgr_update_hw_entries_util(
				CAM_ISP_IQ_BL, total_used_bytes, kmd_buf_info,
			    ...
			    struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < ubwc_config->num_ports; i++) {
		...
		CAM_DBG(CAM_ISP, "UBWC config idx %d, port_type=%d ctx_idx: %u",
			i, ubwc_plane_cfg->port_type, ctx->ctx_index);
		...
		hw_mgr_res =
			&ctx->res_list_ife_out[ctx->vfe_out_map[res_id_out]];
		...
		if (!hw_intf ||
		    blob_info->base_info->split_id >= CAM_ISP_HW_SPLIT_MAX) {
			CAM_ERR(CAM_ISP, "Invalid base %u type %u",
				blob_info->base_info->idx,
				blob_info->base_info->hw_type);
			return rc;
		...
		rc = cam_isp_add_cmd_buf_update(
			...
			hw_intf, blob_type, blob_type_hw_cmd_map[blob_type],
			(void *)cmd_buf_addr, kmd_buf_remain_size,
			(void *)&generic_ubwc_cfg, &bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
	...
	struct cam_ife_sfe_scratch_buf_info *port_info)
{
	...
	rc = cam_mem_get_io_buf(buffer_info->mem_handle, mmu_hdl, &io_addr,
				&size, NULL, NULL);
	if (rc) {
		CAM_ERR(CAM_ISP, "no scratch buf addr for res: 0x%x",
	...
	CAM_DBG(CAM_ISP,
		...
		port_info->res_id, port_info->width, port_info->height,
		port_info->stride, port_info->slice_height, port_info->io_addr);
	...
	for (i = 0; i < scratch_config->num_ports; i++) {
		...
		if (ife_scratch_config->num_config >= CAM_IFE_SCRATCH_NUM_MAX) {
			CAM_ERR(CAM_ISP,
				...
				ife_scratch_config->num_config,
				CAM_IFE_SCRATCH_NUM_MAX, ctx->ctx_index);
			return -EINVAL;
		}

		port_info =
			&ife_scratch_config
				 ->buf_info[ife_scratch_config->num_config++];
	...
	struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < scratch_config->num_ports; i++) {
		...
		CAM_DBG(CAM_ISP,
			"scratch config idx: %d res: 0x%x, ctx_idx: %u", i,
			buffer_info->resource_type, ctx->ctx_index);

		if (res_id_out >= CAM_SFE_FE_RDI_NUM_MAX) {
			CAM_ERR(CAM_ISP,
				"invalid out res type: 0x%x, ctx_idx: %u",
				buffer_info->resource_type, ctx->ctx_index);
			return -EINVAL;
		}

		sfe_out_res =
			&ctx->res_list_sfe_out[ctx->sfe_out_map[res_id_out]];
		...
		port_info = &ctx->scratch_buf_info.sfe_scratch_config
				     ->buf_info[res_id_out];
		rc = cam_isp_scratch_buf_update_util(buffer_info, port_info);
		if (rc)
			goto end;
	...
	struct cam_isp_sfe_bus_sys_cache_config *wm_rm_cache_cfg)
{
	...
	rc = hw_res->hw_intf->hw_ops.process_cmd(
		hw_res->hw_intf->hw_priv, cmd_type, wm_rm_cache_cfg,
		sizeof(struct cam_isp_sfe_bus_sys_cache_config));
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed in sending cache config for: %u",
			hw_res->res_id);
	}
...
static uint32_t cam_ife_hw_mgr_get_sfe_sys_cache_id(
	uint32_t exp_type, struct cam_ife_hw_mgr_ctx *ctx, uint32_t hw_idx)
{
	...
	if (use_large) {
       ...
       * available SCID.
       */
			if (hw_mgr->sfe_cache_info[hw_idx].curr_idx[exp_type] !=
			    scid_idx)
				break;
		}
	}
...
static int
cam_isp_blob_sfe_exp_order_update(uint32_t base_idx,
				  ...
				  struct cam_hw_prepare_update_args *prepare)
{
	...
	ctx = prepare->ctxt_to_hw_map;
	hw_mgr = ctx->hw_mgr;
	memset(ctx->flags.sys_cache_usage, false,
	       sizeof(ctx->flags.sys_cache_usage));

	if (!hw_mgr->num_caches_found) {
		CAM_DBG(CAM_ISP, "No caches found during probe, ctx_idx: %u",
			ctx->ctx_index);
		return 0;
	}
   ...
   */
	...
	for (i = 0; i < exp_config->num_ports; i++) {
		...
		rc = cam_ife_hw_mgr_is_sfe_rdi_for_fetch(order_cfg->res_type);
		if (!rc) {
			CAM_ERR(CAM_ISP,
		...
		if ((order_cfg->res_type - CAM_ISP_SFE_OUT_RES_RDI_0) >=
		    ctx->scratch_buf_info.num_fetches) {
			CAM_DBG(CAM_ISP,
				"Skip cache config for resource: 0x%x, active fetches: %u "
				"[exp_order: %d %d] in %u ctx",
				order_cfg->res_type,
				ctx->scratch_buf_info.num_fetches, i,
				exp_order_max, ctx->ctx_index);
			continue;
		}
		...
		sc_idx = cam_ife_hw_mgr_get_sfe_sys_cache_id(exp_type, ctx,
							     base_idx);
		...
		if (wm_rm_cache_cfg.use_cache) {
			wm_rm_cache_cfg.scid =
				hw_mgr->sys_cache_info[sc_idx].scid;
		...
		if (rc || !send_config) {
			CAM_ERR(CAM_ISP,
				"Failed to send cache config for WR res: 0x%x base_idx: %u "
				"send_config: %d rc: %d ctx_idx: %u",
		...
		list_for_each_entry_safe(hw_mgr_res, tmp,
					 &ctx->res_list_ife_in_rd, list) {
			for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
				if (!hw_mgr_res->hw_res[j])
					continue;
				...
				if (hw_mgr_res->hw_res[j]->hw_intf->hw_idx !=
				    base_idx)
					continue;
		...
		if (rc || !send_config) {
			CAM_ERR(CAM_ISP,
				"Failed to send cache config for RD res: 0x%x base_idx: %u "
				"send_config: %d rc: %d ctx_idx: %u",
		...
		if (!wm_rm_cache_cfg.rd_cfg_done &&
		    !wm_rm_cache_cfg.wr_cfg_done) {
			wm_rm_cache_cfg.use_cache = false;
			if (sc_idx < CAM_LLCC_MAX)
...
static int cam_isp_blob_sfe_update_fetch_core_cfg(
	uint32_t blob_type, struct cam_isp_generic_blob_info *blob_info,
	struct cam_hw_prepare_update_args *prepare)
{
	int rc;
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		...
		if ((ctx->ctx_config & CAM_IFE_CTX_CFG_DYNAMIC_SWITCH_ON) &&
		...
		cpu_addr = kmd_buf_info->cpu_addr +
			   kmd_buf_info->used_bytes / 4 + total_used_bytes / 4;

		CAM_DBG(CAM_ISP,
			"SFE:%u RM: %u res_id: 0x%x enable: %u num_exp: %u ctx_idx: %u",
			blob_info->base_info->idx,
			(res_id - CAM_ISP_SFE_IN_RD_0), res_id, enable,
			ctx->scratch_buf_info.sfe_scratch_config
				->updated_num_exp,
			ctx->ctx_index);

		rc = cam_isp_add_cmd_buf_update(
			hw_mgr_res->hw_res[blob_info->base_info->split_id],
			hw_intf, blob_type, CAM_ISP_HW_CMD_RM_ENABLE_DISABLE,
			(void *)cpu_addr, remain_size, (void *)&enable,
			&used_bytes);
		...
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				...
				blob_info->base_info->idx, res_id, used_bytes,
				rc, ctx->ctx_index);
			return rc;
		}
...
cam_isp_blob_hfr_update(uint32_t blob_type,
			...
			struct cam_hw_prepare_update_args *prepare,
			uint32_t out_max, enum cam_isp_hw_type hw_type)
{
	...
	for (i = 0; i < hfr_config->num_ports; i++) {
		...
		if (hw_type == CAM_ISP_HW_TYPE_SFE)
			hw_mgr_res = &ctx->res_list_sfe_out
					      [ctx->sfe_out_map[res_id_out]];
		else
			hw_mgr_res = &ctx->res_list_ife_out
					      [ctx->vfe_out_map[res_id_out]];
		...
		rc = cam_isp_add_cmd_buf_update(
			hw_mgr_res->hw_res[blob_info->base_info->split_id],
			hw_intf, blob_type, CAM_ISP_HW_CMD_GET_HFR_UPDATE,
			(void *)cmd_buf_addr, kmd_buf_remain_size,
			(void *)port_hfr_config, &bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
	...
	struct cam_hw_prepare_update_args *prepare)
{
	...
	CAM_DBG(CAM_ISP, "csid mup value=%u, ctx_idx: %u", mup_config->mup,
		ctx->ctx_index);
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if ((prepare_hw_data->packet_opcode_type ==
			...
			csid_mup_upd_args.exp_update_args.reset_discard_cfg =
				true;
			       ...
			       struct cam_hw_prepare_update_args *prepare)
{
	...
	CAM_DBG(CAM_ISP, "csid clk=%llu, ctx_idx: %u", clock_config->csid_clock,
		ctx->ctx_index);
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				if (rc)
					CAM_ERR(CAM_ISP,
						"Clock Update failed, ctx_idx: %u",
						ctx->ctx_index);
			} else
				CAM_ERR(CAM_ISP, "NULL hw_intf! ctx_idx: %u",
					ctx->ctx_index);
		}
	}
			      ...
			      struct cam_hw_prepare_update_args *prepare)
{
	...
	CAM_DBG(CAM_ISP, "csid binning=%d, ctx_idx: %u",
		qcfa_config->csid_binning, ctx->ctx_index);
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				csid_qcfa_upd_args.res = hw_mgr_res->hw_res[i];
...
static int
cam_isp_blob_sfe_core_cfg_update(uint32_t blob_type,
				 struct cam_isp_generic_blob_info *blob_info,
				 ...
				 struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		idx = ctx->base[i].idx;
		if (idx >= CAM_SFE_HW_NUM_MAX || !ife_hw_mgr->sfe_devices[idx])
			continue;

		hw_intf = ife_hw_mgr->sfe_devices[idx]->hw_intf;
		if (hw_intf && hw_intf->hw_ops.process_cmd) {
			memcpy(&sfe_core_config.core_config, core_config,
			       sizeof(struct cam_isp_sfe_core_config));
			rc = hw_intf->hw_ops.process_cmd(
				hw_intf->hw_priv, CAM_ISP_HW_CMD_CORE_CONFIG,
				&sfe_core_config,
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_sfe_src, list) {
         ...
         */
				rc = hw_intf->hw_ops.process_cmd(
					...
					CAM_ISP_HW_CMD_CLOCK_UPDATE,
					&clock_upd_args,
					sizeof(struct cam_sfe_clock_update_args));
				if (rc)
					CAM_ERR(CAM_PERF,
...
static int
cam_isp_blob_sfe_rd_update(uint32_t blob_type, uint32_t kmd_buf_remain_size,
			   uint32_t *cmd_buf_addr, uint32_t *total_used_bytes,
			   struct cam_ife_hw_mgr_ctx *ctx,
			   struct cam_isp_generic_blob_info *blob_info,
			   struct cam_isp_vfe_wm_config *wm_config)
{
	...
	list_for_each_entry(sfe_rd_res, &ctx->res_list_ife_in_rd, list) {
	...
	rc = cam_isp_add_cmd_buf_update(
		...
		blob_type, CAM_ISP_HW_CMD_FE_UPDATE_BUS_RD,
		(void *)cmd_buf_addr, kmd_buf_remain_size, (void *)wm_config,
		&bytes_used);
	if (rc < 0) {
		CAM_ERR(CAM_ISP,
			"Failed to update SFE RM config out_type:0x%X base_idx:%d "
			"bytes_used:%u rc:%d ctx_idx: %u",
...
static int
cam_ife_hw_mgr_update_scratch_offset(struct cam_ife_hw_mgr_ctx *ctx,
				     struct cam_isp_vfe_wm_config *wm_config)
{
	uint32_t res_id;
	struct cam_ife_sfe_scratch_buf_info *port_info;
	...
	if (res_id >= CAM_SFE_FE_RDI_NUM_MAX) {
		CAM_ERR_RATE_LIMIT(
			CAM_ISP, "res_id: %d exceeds max size: %d ctx_idx: %u",
			res_id, CAM_SFE_FE_RDI_NUM_MAX, ctx->ctx_index);
		return -EINVAL;
	}

	if (!ctx->scratch_buf_info.sfe_scratch_config->buf_info[res_id]
		     .config_done) {
		CAM_ERR(CAM_ISP,
...
cam_isp_blob_vfe_out_update(uint32_t blob_type,
			    ...
			    struct cam_hw_prepare_update_args *prepare,
			    uint32_t size_isp_out, enum cam_isp_hw_type hw_type)
{
	struct cam_isp_vfe_wm_config *wm_config;
	struct cam_kmd_buf_info *kmd_buf_info;
	struct cam_ife_hw_mgr_ctx *ctx = NULL;
	...
	for (i = 0; i < vfe_out_config->num_ports; i++) {
		...
		if (hw_type == CAM_ISP_HW_TYPE_SFE) {
			is_sfe_rd = cam_ife_hw_mgr_is_sfe_rd_res(
				wm_config->port_type);
			if ((!cam_ife_hw_mgr_is_sfe_out_port(
				    wm_config->port_type)) &&
			    (!is_sfe_rd))
				continue;
		}
		...
		if (is_sfe_rd) {
			rc = cam_isp_blob_sfe_rd_update(
				blob_type, kmd_buf_remain_size, cmd_buf_addr,
		...
		CAM_DBG(CAM_ISP,
			...
			(hw_type == CAM_ISP_HW_TYPE_SFE ? "SFE" : "VFE"), i,
			wm_config->port_type, ctx->ctx_index);
		...
		if (hw_type == CAM_ISP_HW_TYPE_SFE) {
			...
			if ((ctx->flags.is_sfe_shdr) &&
			    (cam_ife_hw_mgr_is_sfe_rdi_for_fetch(
				    wm_config->port_type))) {
				rc = cam_ife_hw_mgr_update_scratch_offset(
					ctx, wm_config);
				if (rc)
					return rc;
			}

			isp_out_res = &ctx->res_list_sfe_out
					       [ctx->sfe_out_map[res_id_out]];
		} else {
			isp_out_res = &ctx->res_list_ife_out
					       [ctx->vfe_out_map[res_id_out]];
		}
		...
		rc = cam_isp_add_cmd_buf_update(
			...
			hw_intf, blob_type, CAM_ISP_HW_CMD_WM_CONFIG_UPDATE,
			(void *)cmd_buf_addr, kmd_buf_remain_size,
			(void *)wm_config, &bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				"Failed to update %s Out out_type:0x%X base_idx:%d bytes_used:%u "
				"rc:%d ctx_idx: %u",
				((hw_type == CAM_ISP_HW_TYPE_SFE) ? "SFE" :
								    "VFE"),
				wm_config->port_type, blob_info->base_info->idx,
				bytes_used, rc, ctx->ctx_index);
			return rc;
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				rc = hw_intf->hw_ops.process_cmd(
					...
					CAM_ISP_HW_CMD_BLANKING_UPDATE,
					&blanking_config,
					sizeof(struct cam_isp_blanking_config));
				if (rc)
					CAM_ERR(CAM_ISP,
	...
	enum cam_isp_hw_type hw_type)
{
	...
	for (i = 0; i < bw_limit_cfg->num_ports; i++) {
		...
		if ((hw_type == CAM_ISP_HW_TYPE_SFE) &&
		    !((wm_bw_limit_cfg->res_type >= CAM_ISP_SFE_OUT_RES_BASE) &&
		      (wm_bw_limit_cfg->res_type <
		       (CAM_ISP_SFE_OUT_RES_BASE + max_sfe_out_res))))
			continue;

		if ((hw_type == CAM_ISP_HW_TYPE_VFE) &&
		    !((wm_bw_limit_cfg->res_type >= CAM_ISP_IFE_OUT_RES_BASE) &&
		      (wm_bw_limit_cfg->res_type <
		...
		rc = cam_isp_add_cmd_buf_update(
			isp_out_res->hw_res[blob_info->base_info->split_id],
			hw_intf, blob_type, CAM_ISP_HW_CMD_WM_BW_LIMIT_CONFIG,
			(void *)cmd_buf_addr, kmd_buf_remain_size,
			(void *)wm_bw_limit_cfg, &bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				"Failed to update %s BW limiter config for res:0x%x enable:%d "
				"[0x%x:0x%x] base_idx:%d bytes_used:%u rc:%d ctx_idx: %u",
				((hw_type == CAM_ISP_HW_TYPE_SFE) ? "SFE" :
								    "VFE"),
				wm_bw_limit_cfg->res_type,
				wm_bw_limit_cfg->enable_limiter,
...
static int
cam_isp_hw_mgr_add_cmd_buf_util(struct cam_isp_hw_mgr_res *hw_mgr_res,
				struct cam_hw_prepare_update_args *prepare,
				struct cam_isp_generic_blob_info *blob_info,
				void *data, uint32_t hw_cmd_type,
				uint32_t blob_type)
{
	uint32_t total_used_bytes = 0;
	uint32_t kmd_buf_remain_size;
	...
	if (kmd_buf_info->used_bytes < kmd_buf_info->size) {
		...
		CAM_ERR(CAM_ISP,
			...
			blob_info->base_info->idx, kmd_buf_info->used_bytes,
			kmd_buf_info->size);
		return -ENOMEM;
	}
	...
	rc = cam_isp_add_cmd_buf_update(
		...
		blob_type, hw_cmd_type, (void *)cmd_buf_addr,
...
static int
cam_isp_update_ife_pdaf_cfg(struct cam_ife_hw_mgr_ctx *ctx,
			    struct cam_hw_prepare_update_args *prepare,
			    struct cam_isp_generic_blob_info *blob_info,
			    ...
			    uint32_t blob_type)
{
   ...
   * vfe, since input config is done in vfe.
   */
	csid_path_id = cam_ife_hw_mgr_get_ife_csid_rdi_res_type(
	...
	CAM_DBG(CAM_ISP, "Ctx %d res: %u lcr %u id %u ctx_type %u",
		ctx->ctx_index, ife_res_id, isp_lcr_cfg->rdi_lcr_cfg->res_id,
		blob_info->base_info->idx, ctx->ctx_type);
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		if (hw_mgr_res->res_type == CAM_ISP_RESOURCE_UNINT)
			continue;
       ...
       */
			if ((ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE &&
				...
				rc = cam_isp_hw_mgr_add_cmd_buf_util(
					hw_mgr_res, prepare, blob_info,
					(void *)isp_lcr_cfg,
...
static int cam_isp_config_rdi_lcr_csid_init_params(
	...
	struct cam_hw_prepare_update_args *prepare,
	struct cam_isp_generic_blob_info *blob_info,
	struct cam_isp_lcr_rdi_config *rdi_lcr_cfg, uint32_t blob_type)
{
	struct cam_isp_hw_mgr_res *hw_mgr_res;
	struct cam_isp_resource_node *res;
	int rc = -EINVAL;
	...
	csid_res_id =
		cam_ife_hw_mgr_get_ife_csid_rdi_res_type(rdi_lcr_cfg->res_id);
	CAM_DBG(CAM_ISP,
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
		...
		if (!ctx->flags.is_sfe_shdr &&
		    hw_mgr_res->res_id != csid_res_id)
			continue;

		res = hw_mgr_res->hw_res[0];
		rc = res->hw_intf->hw_ops.process_cmd(
			res->hw_intf->hw_priv, CAM_ISP_HW_CMD_RDI_LCR_CFG, res,
			sizeof(*res));
		acquired_res_id_mask |= BIT(res->res_id);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Ctx: %u csid_res_id: %u rdi_lcr: %u sfe_shdr %u ctx_ctype %u",
				ctx->ctx_index, csid_res_id,
				rdi_lcr_cfg->res_id, ctx->flags.is_sfe_shdr,
				ctx->ctx_type);
			break;
		}
	}
	...
	if (!(acquired_res_id_mask & BIT(csid_res_id))) {
		CAM_ERR(CAM_ISP,
			"Ctx: %u Unacquired csid_res_id: %u rdi_lcr: %u sfe_shdr %u "
			"ctx_ctype %u",
			ctx->ctx_index, csid_res_id, rdi_lcr_cfg->res_id,
			ctx->flags.is_sfe_shdr, ctx->ctx_type);
		rc = -EINVAL;
...
static int
cam_isp_blob_ife_rdi_lcr_config(struct cam_ife_hw_mgr_ctx *ctx,
				struct cam_hw_prepare_update_args *prepare,
				struct cam_isp_generic_blob_info *blob_info,
				...
				uint32_t blob_type)
{
	...
	CAM_DBG(CAM_ISP,
		...
		prepare_hw_data->packet_opcode_type, rdi_lcr_cfg->res_id,
		ctx->ctx_type, ctx->flags.is_sfe_shdr, ctx->flags.rdi_lcr_en,
		ctx->ctx_index);

	if (prepare_hw_data->packet_opcode_type == CAM_ISP_PACKET_INIT_DEV) {
		rc = cam_isp_config_rdi_lcr_csid_init_params(
			ctx, prepare, blob_info, rdi_lcr_cfg, blob_type);
		if (rc) {
			CAM_ERR(CAM_ISP,
				"CSID param failed Ctx: %d rdi_lcr: %u ctx_type: %u ctx_idx: %u",
				ctx->ctx_index, rdi_lcr_cfg->res_id,
				ctx->ctx_type, ctx->ctx_index);
			return rc;
		}
     ...
     */
		CAM_ERR(CAM_ISP,
			...
			rdi_lcr_cfg->res_id, ctx->ctx_type,
	...
	rc = cam_isp_update_ife_pdaf_cfg(ctx, prepare, blob_info, &isp_cfg_args,
					 blob_type);
	if (rc) {
		CAM_ERR(CAM_ISP,
...
{
	if ((bw_limit_config->num_ports >
	     (max_ife_out_res + max_sfe_out_res)) ||
	    (bw_limit_config->num_ports == 0)) {
		CAM_ERR(CAM_ISP, "Invalid num_ports:%u in bw limit config",
	...
	if (bw_limit_config->num_ports != 1) {
		if (sizeof(struct cam_isp_wm_bw_limiter_config) >
		    ((UINT_MAX -
	...
	if (blob_size < (sizeof(struct cam_isp_out_rsrc_bw_limiter_config) +
			 (bw_limit_config->num_ports - 1) *
				 sizeof(struct cam_isp_wm_bw_limiter_config))) {
		CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu", blob_size,
			sizeof(struct cam_isp_out_rsrc_bw_limiter_config) +
				(bw_limit_config->num_ports -
				 1) * sizeof(struct cam_isp_wm_bw_limiter_config));
		return -EINVAL;
	}
...
static int
cam_isp_blob_csid_init_config_update(struct cam_hw_prepare_update_args *prepare,
...
static int
cam_isp_blob_ife_init_config_update(struct cam_hw_prepare_update_args *prepare,
				    struct cam_isp_init_config *init_config)
{
	int i, rc = -EINVAL;
	struct cam_hw_intf *hw_intf;
...
static int cam_isp_validate_scratch_buffer_blob(
	uint32_t blob_size, struct cam_ife_hw_mgr_ctx *ife_mgr_ctx,
	struct cam_isp_sfe_init_scratch_buf_config *scratch_config)
{
	if (!(ife_mgr_ctx->flags.is_sfe_fs || ife_mgr_ctx->flags.is_sfe_shdr)) {
		CAM_ERR(CAM_ISP,
	...
	if (blob_size < sizeof(struct cam_isp_sfe_init_scratch_buf_config)) {
	...
	if (scratch_config->num_ports != 1) {
		if (sizeof(struct cam_isp_sfe_scratch_buf_info) >
			...
			CAM_ERR(CAM_ISP,
				"Max size exceeded in scratch config num_ports: %u size per "
				"port: %lu ctx: %u",
	...
	if (blob_size < (sizeof(struct cam_isp_sfe_init_scratch_buf_config) +
		...
		CAM_ERR(CAM_ISP,
			...
			sizeof(struct cam_isp_sfe_init_scratch_buf_config) +
				(scratch_config->num_ports -
				 1) * sizeof(struct cam_isp_sfe_scratch_buf_info),
			ife_mgr_ctx->ctx_index);
		return -EINVAL;
	}
					       ...
					       uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_HFR_CONFIG: {
		...
		if (hfr_config->num_ports >
			    g_ife_hw_mgr.isp_caps.max_vfe_out_res_type ||
		    hfr_config->num_ports == 0) {
			CAM_ERR(CAM_ISP,
				"Invalid num_ports %u in HFR config, ctx_idx: %u",
		...
		if (hfr_config->num_ports != 1) {
			if (sizeof(struct cam_isp_port_hfr_config) >
				...
				CAM_ERR(CAM_ISP,
					"Max size exceeded in hfr config num_ports:%u size per "
					"port:%lu ctx_idx: %u",
					hfr_config->num_ports,
		...
		rc = cam_isp_blob_hfr_update(blob_type, blob_info, hfr_config,
					     prepare, max_ife_out_res,
					     CAM_ISP_HW_TYPE_VFE);
		if (rc)
			CAM_ERR(CAM_ISP, "HFR Update Failed, ctx_idx: %u",
				ife_mgr_ctx->ctx_index);
	} break;
	case CAM_ISP_GENERIC_BLOB_TYPE_CLOCK_CONFIG: {
		...
		if (clock_config->num_rdi > CAM_IFE_RDI_NUM_MAX) {
			CAM_ERR(CAM_ISP,
				"Invalid num_rdi %u in clock config, ctx_idx: %u",
		...
		if (clock_config->num_rdi > 1) {
			if (sizeof(uint64_t) >
				...
				CAM_ERR(CAM_ISP,
					"Max size exceeded in clock config num_rdi:%u size per "
					"port:%lu ctx_idx: %u",
					clock_config->num_rdi, sizeof(uint64_t),
					ife_mgr_ctx->ctx_index);
				return -EINVAL;
			}
		}
		...
		memcpy(&prepare_hw_data->bw_clk_config.ife_clock_config,
		       clock_config, clock_config_size);
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG: {
		...
		if (bw_config->num_rdi > CAM_IFE_RDI_NUM_MAX) {
			CAM_ERR(CAM_ISP,
				"Invalid num_rdi %u in bw config, ctx_idx: %u",
		...
		if (!prepare || !prepare->priv ||
			...
			CAM_ERR(CAM_ISP,
				"Invalid inputs usage type %d, ctx_idx: %u",
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V2: {
		    ...
		    (blob_size <
		     (sizeof(struct cam_isp_bw_config_v2) +
		      (bw_config->num_paths - 1) *
			      sizeof(struct cam_axi_per_path_bw_vote)))) {
			CAM_ERR(CAM_ISP,
				"Invalid blob size: %u, num_paths: %u, bw_config size: %lu, "
				"per_path_vote size: %lu, ctx_idx: %u",
		...
		prepare_hw_data->bw_clk_config.bw_config_v2.usage_type =
			bw_config->usage_type;
		prepare_hw_data->bw_clk_config.bw_config_v2.num_paths =
			bw_config->num_paths;

		for (i = 0; i < bw_config->num_paths; i++) {
			path_vote = &prepare_hw_data->bw_clk_config.bw_config_v2
					     .axi_path[i];
			path_vote->usage_data =
				bw_config->axi_path[i].usage_data;
			path_vote->transac_type =
				bw_config->axi_path[i].transac_type;
			path_vote->path_data_type =
				bw_config->axi_path[i].path_data_type;
			path_vote->vote_level = 0;
			path_vote->camnoc_bw = bw_config->axi_path[i].camnoc_bw;
			path_vote->mnoc_ab_bw =
				bw_config->axi_path[i].mnoc_ab_bw;
			path_vote->mnoc_ib_bw =
				bw_config->axi_path[i].mnoc_ib_bw;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V3: {
		...
		/* Check for integer overflow */
		if (bw_config->num_paths > 1) {
			if (sizeof(struct cam_axi_per_path_bw_vote_v2) >
			    ((UINT_MAX - sizeof(struct cam_isp_bw_config_v3)) /
			     (bw_config->num_paths - 1))) {
				CAM_ERR(CAM_ISP,
					"Size exceeds limit paths:%u size per path:%lu ctx_idx: %u",
					bw_config->num_paths - 1,
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_UBWC_CONFIG: {
		...
		if (ubwc_config->num_ports > CAM_VFE_MAX_UBWC_PORTS ||
			...
			CAM_ERR(CAM_ISP,
				"Invalid num_ports %u in ubwc config, ctx_idx: %u",
		...
		if (ubwc_config->num_ports != 1) {
			if (sizeof(struct cam_ubwc_plane_cfg_v1) >
				...
				CAM_ERR(CAM_ISP,
					"Max size exceeded in ubwc config num_ports:%u size per "
					"port:%lu ctx_idx: %u",
					ubwc_config->num_ports,
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP,
				"Invalid blob_size %u expected %lu ctx_idx: %u",
				blob_size,
				sizeof(struct cam_ubwc_config) +
					(ubwc_config->num_ports -
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"UBWC Update Failed rc: %d, ctx_idx: %u", rc,
				ife_mgr_ctx->ctx_index);
	} break;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_CSID_QCFA_CONFIG: {
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"QCFA Update Failed rc: %d, ctx_idx: %u", rc,
				ife_mgr_ctx->ctx_index);
	} break;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_IFE_CORE_CONFIG: {
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"Core cfg update fail: %d, ctx_idx: %u", rc,
				ife_mgr_ctx->ctx_index);
	} break;
	case CAM_ISP_GENERIC_BLOB_TYPE_VFE_OUT_CONFIG: {
		...
		if (vfe_out_config->num_ports != 1) {
			if (sizeof(struct cam_isp_vfe_wm_config) >
				...
				CAM_ERR(CAM_ISP,
					"Max size exceeded in vfe out config num_ports:%u size per "
					"port:%lu, ctx_idx: %u",
					vfe_out_config->num_ports,
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"VFE out update failed rc: %d, ctx_idx: %u", rc,
				ife_mgr_ctx->ctx_index);
	} break;
	case CAM_ISP_GENERIC_BLOB_TYPE_SENSOR_BLANKING_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_sensor_blanking_config)) {
			CAM_ERR(CAM_ISP,
				"Invalid blob size %zu expected %zu ctx_idx: %u",
				blob_size,
		...
		rc = cam_isp_blob_sensor_blanking_config(
			blob_type, blob_info, sensor_blanking_config, prepare);
		if (rc)
			CAM_ERR(CAM_ISP,
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_DISCARD_INITIAL_FRAMES: {
		...
		discard_config =
			(struct cam_isp_discard_initial_frames *)blob_data;

		rc = cam_isp_blob_csid_discard_init_frame_update(
			blob_info, discard_config, prepare);
		if (rc)
			CAM_ERR(CAM_ISP,
				"Discard initial frames update failed rc: %d ctx: %u",
				rc, ife_mgr_ctx->ctx_index);
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_SCRATCH_BUF_CFG: {
		...
		rc = cam_isp_validate_scratch_buffer_blob(
			blob_size, ife_mgr_ctx, scratch_config);
		if (rc)
			return rc;

		rc = cam_isp_blob_ife_scratch_buf_update(scratch_config,
							 prepare);
		if (rc)
			CAM_ERR(CAM_ISP,
				"IFE scratch buffer update failed rc: %d, ctx_idx: %u",
				rc, ife_mgr_ctx->ctx_index);
	} break;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_DYNAMIC_MODE_SWITCH: {
		...
		if (rc)
			CAM_ERR(CAM_ISP, "MUP Update Failed, ctx_idx: %u",
				ife_mgr_ctx->ctx_index);
	} break;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_RDI_LCR_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_lcr_rdi_config)) {
			CAM_ERR(CAM_ISP,
				"Invalid lcr blob size %u expected %u ctx_idx: %u",
				blob_size,
				sizeof(struct cam_isp_lcr_rdi_config),
				ife_mgr_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_DRV_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_drv_config)) {
			CAM_ERR(CAM_ISP,
				"Invalid DRV blob size %u expected %u ctx_idx: %u",
		...
		CAM_DBG(CAM_ISP,
			...
			drv_config->drv_en, drv_config->timeout_val,
			drv_config->path_idle_en, ife_mgr_ctx->ctx_index);
	...
	struct list_head *buf_tracker)
{
	...
	mmu_hdl = cam_mem_is_secure_buf(kmd_buf->handle) ?
			  hw_mgr->mgr_common.img_iommu_hdl_secure :
			  hw_mgr->mgr_common.img_iommu_hdl;

	rc = cam_mem_get_io_buf(kmd_buf->handle, mmu_hdl, &iova_addr, &len,
				NULL, buf_tracker);
	if (rc) {
		CAM_ERR(CAM_ISP,
	...
	if (frame_header_iova % CAM_FRAME_HEADER_ADDR_ALIGNMENT) {
		padded_bytes = (uint32_t)(CAM_FRAME_HEADER_ADDR_ALIGNMENT -
					  (frame_header_iova %
					   CAM_FRAME_HEADER_ADDR_ALIGNMENT));
...
static int
cam_isp_blob_csid_irq_comp_cfg(struct cam_ife_hw_mgr_ctx *ctx,
			       struct cam_hw_prepare_update_args *prepare,
			       struct cam_isp_generic_blob_info *blob_info,
...

{
	...
	rc = cam_isp_hw_mgr_add_cmd_buf_util(hw_mgr_res, prepare, blob_info,
					     (void *)comp_cfg,
					     CAM_ISP_HW_CMD_IRQ_COMP_CFG,
					     blob_type);

	CAM_DBG(CAM_ISP, "Ctx:%d IPP SRC mask 0x%x IPP DST mask 0x%x",
		ctx->ctx_index, comp_cfg->ipp_src_ctxt_mask,
		comp_cfg->ipp_dst_comp_mask);
						...
						uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_IRQ_COMP_CFG: {
		...
		rc = cam_isp_blob_csid_irq_comp_cfg(ife_mgr_ctx, prepare,
						    blob_info, irq_comp_cfg,
						    blob_type);

		CAM_DBG(CAM_ISP,
			"IRQ comp cfg blob, ipp_src_ctxt_mask: 0x%x, ipp_dest_ctxt_mask: 0x%x",
			irq_comp_cfg->ipp_src_ctxt_mask,
			irq_comp_cfg->ipp_dst_comp_mask);
	} break;
	default:
		CAM_WARN(CAM_ISP, "Invalid blob type %d, ctx_idx: %u",
			 blob_type, ife_mgr_ctx->ctx_index);
					       ...
					       uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_CLOCK_CONFIG: {
		...
		clock_config_size = sizeof(struct cam_isp_clock_config) +
				    ((clock_config->num_rdi - 1) *
				     sizeof(clock_config->rdi_hz));
		memcpy(&prepare_hw_data->bw_clk_config.sfe_clock_config,
		       clock_config, clock_config_size);
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_OUT_CONFIG: {
		...
		if (vfe_out_config->num_ports > max_sfe_out_res ||
			...
			CAM_ERR(CAM_ISP,
				...
				vfe_out_config->num_ports, max_sfe_out_res,
				ife_mgr_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (vfe_out_config->num_ports != 1) {
			if (sizeof(struct cam_isp_vfe_wm_config) >
			    ((UINT_MAX - sizeof(struct cam_isp_vfe_out_config)) /
			     (vfe_out_config->num_ports - 1))) {
				CAM_ERR(CAM_ISP,
					"Max size exceeded in sfe out config num_ports:%u size per "
					"port:%lu ctx_idx: %u",
					vfe_out_config->num_ports,
					sizeof(struct cam_isp_vfe_wm_config),
					ife_mgr_ctx->ctx_index);
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"SFE out update failed rc: %d, ctx_idx: %u", rc,
				ife_mgr_ctx->ctx_index);
	} break;
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_HFR_CONFIG: {
		struct cam_isp_resource_hfr_config *hfr_config;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_CORE_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_sfe_core_config)) {
			CAM_ERR(CAM_ISP,
				"Invalid blob size: %u expected: %u ctx_idx: %u",
				blob_size,
				sizeof(struct cam_isp_sfe_core_config),
				ife_mgr_ctx->ctx_index);
			return -EINVAL;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_SCRATCH_BUF_CFG: {
		...
		rc = cam_isp_blob_sfe_scratch_buf_update(scratch_config,
							 prepare);
		if (rc)
			CAM_ERR(CAM_ISP,
				"SFE scratch buffer update failed rc: %d ctx_idx: %u",
				rc, ife_mgr_ctx->ctx_index);
	} break;
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_FE_CONFIG: {
		struct cam_fe_config *fe_config;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_DYNAMIC_MODE_SWITCH: {
		...
		if (ife_mgr_ctx->flags.is_sfe_shdr) {
			ife_mgr_ctx->scratch_buf_info.sfe_scratch_config
				->updated_num_exp = mup_config->num_expoures;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_EXP_ORDER_CFG: {
		...
		if (blob_size < sizeof(struct cam_isp_sfe_exp_config)) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u, ctx_idx: %u",
				blob_size, ife_mgr_ctx->ctx_index);
			return -EINVAL;
		}

		exp_config = (struct cam_isp_sfe_exp_config *)blob_data;
		...
		if (exp_config->num_ports != 1) {
			if (sizeof(struct cam_isp_sfe_wm_exp_order_config) >
			    ((UINT_MAX - sizeof(struct cam_isp_sfe_exp_config)) /
			     (exp_config->num_ports - 1))) {
				CAM_ERR(CAM_ISP,
					"Max size exceeded in exp order config num_ports: %u size per "
					"port: %lu ctx_idx: %u",
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP,
				...
				sizeof(struct cam_isp_sfe_exp_config) +
					(exp_config->num_ports -
					 1) * sizeof(struct cam_isp_sfe_wm_exp_order_config),
				ife_mgr_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"SFE exp order update failed, ctx_idx: %u",
				ife_mgr_ctx->ctx_index);
	} break;
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_LIMITER_CFG: {
		struct cam_isp_out_rsrc_bw_limiter_config *bw_limit_config;
...
static inline bool
cam_ife_mgr_validate_for_io_buffers(uint32_t port_id, uint32_t scratch_cfg_mask)
{
...
static inline bool cam_isp_sfe_validate_for_scratch_buf_config(
	uint32_t res_idx, struct cam_ife_hw_mgr_ctx *ctx, bool default_settings)
{
	...
	if (default_settings)
		...
		curr_num_exp = ctx->scratch_buf_info.sfe_scratch_config
				       ->updated_num_exp;
				 ...
				 uint32_t *cpu_addr, uint32_t *used_bytes)
{
   ...
   */
	...
	rc = hw_res->hw_intf->hw_ops.process_cmd(
		hw_res->hw_intf->hw_priv, cmd_type, &update_buf,
...
static int
cam_isp_sfe_add_scratch_buffer_cfg(uint32_t base_idx, uint32_t sfe_rdi_cfg_mask,
				   struct cam_hw_prepare_update_args *prepare,
				   struct cam_kmd_buf_info *kmd_buf_info,
				   ...
				   struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	io_cfg_used_bytes = 0;
	CAM_DBG(CAM_ISP, "ctx_idx: %u num_ports: %u", ctx->ctx_index,
		ctx->scratch_buf_info.sfe_scratch_config->num_config);
	...
	for (i = 0; i < CAM_SFE_FE_RDI_NUM_MAX; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			rc = cam_isp_sfe_send_scratch_buf_upd(
				remain_size, CAM_ISP_HW_CMD_GET_BUF_UPDATE,
				hw_mgr_res->hw_res[j], buf_info, cpu_addr,
				&used_bytes);
			if (rc)
				return rc;
	...
	list_for_each_entry(hw_mgr_res, res_list_in_rd, list) {
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			cpu_addr = kmd_buf_info->cpu_addr +
				   kmd_buf_info->used_bytes / 4 +
				   io_cfg_used_bytes / 4;
			buf_info = &ctx->scratch_buf_info.sfe_scratch_config
					    ->buf_info[res_id -
						       CAM_ISP_SFE_IN_RD_0];
			...
			rc = cam_isp_sfe_send_scratch_buf_upd(
				remain_size, CAM_ISP_HW_CMD_GET_BUF_UPDATE_RM,
				hw_mgr_res->hw_res[j], buf_info, cpu_addr,
				&used_bytes);
			if (rc)
				return rc;
	...
	if (io_cfg_used_bytes)
		cam_ife_mgr_update_hw_entries_util(CAM_ISP_IOCFG_BL,
						   io_cfg_used_bytes,
						   kmd_buf_info, prepare,
						   false);
...
static int
cam_isp_ife_add_scratch_buffer_cfg(uint32_t base_idx, uint32_t scratch_cfg_mask,
				   struct cam_hw_prepare_update_args *prepare,
				   struct cam_kmd_buf_info *kmd_buf_info,
				   struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	for (i = 0; i < ctx->scratch_buf_info.ife_scratch_config->num_config;
	     i++) {
		/*
     ...
     */
		...
		res_id = ctx->scratch_buf_info.ife_scratch_config->buf_info[i]
				 .res_id &
			 0xFF;
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			cpu_addr = kmd_buf_info->cpu_addr +
				   kmd_buf_info->used_bytes / 4 +
				   io_cfg_used_bytes / 4;
			buf_info = &ctx->scratch_buf_info.ife_scratch_config
					    ->buf_info[i];
			CAM_DBG(CAM_ISP,
...
static int
cam_ife_mgr_csid_add_reg_update(struct cam_ife_hw_mgr_ctx *ctx,
				struct cam_hw_prepare_update_args *prepare,
				struct cam_kmd_buf_info *kmd_buf)
{
	int i;
	...
	struct cam_isp_csid_reg_update_args rup_args[CAM_IFE_CSID_HW_NUM_MAX] = {
		0
	};
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		rc = cam_isp_add_change_base(prepare, &ctx->res_list_ife_csid,
		...
		rc = cam_isp_add_csid_reg_update(prepare, kmd_buf, &rup_args[i],
						 true);
...
static int
cam_ife_mgr_isp_add_reg_update(struct cam_ife_hw_mgr_ctx *ctx,
			       struct cam_hw_prepare_update_args *prepare,
			       struct cam_kmd_buf_info *kmd_buf)
{
	int i;
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		CAM_DBG(CAM_ISP,
			"Add Reg Update cmd i=%d, idx=%d, rc=%d ctx_idx: %u", i,
			ctx->base[i].idx, rc, ctx->ctx_index);
	}
	return rc;
}
...
static int
cam_ife_hw_mgr_add_csid_go_cmd(struct cam_ife_hw_mgr_ctx *ctx,
			       struct cam_hw_prepare_update_args *prepare,
			       struct cam_kmd_buf_info *kmd_buf_info)
{
	int rc = -EINVAL, i;
...
add_cmds:
	...
	if (is_found) {
		...
		rc = cam_isp_add_csid_offline_cmd(
			prepare, hw_mgr_res->hw_res[i], kmd_buf_info, true);
		if (rc)
			return rc;
...
static int
cam_ife_hw_mgr_add_vfe_go_cmd(struct cam_ife_hw_mgr_ctx *ctx,
			      struct cam_hw_prepare_update_args *prepare,
			      struct cam_kmd_buf_info *kmd_buf_info)
{
	int rc = -EINVAL, i;
...
add_cmds:
	...
	if (is_found) {
		...
		rc = cam_isp_add_go_cmd(prepare, hw_mgr_res->hw_res[i],
					kmd_buf_info, true);
		if (rc)
			return rc;
...
static int
cam_ife_hw_mgr_update_cmd_buffer(struct cam_ife_hw_mgr_ctx *ctx,
				 struct cam_hw_prepare_update_args *prepare,
				 struct cam_kmd_buf_info *kmd_buf,
				 ...
				 uint32_t base_idx)
{
	...
	if (ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_SFE) {
		...
		if (!cmd_buf_count->csid_cnt &&
		    !prepare_hw_data->irq_comp_cfg_valid)
			return rc;
		res_list = &ctx->res_list_ife_csid;
	} else {
		CAM_ERR(CAM_ISP, "Invalid hw_type=%d, ctx_idx: %u",
	...
	if (!ctx->flags.internal_cdm) {
		...
		change_base_info.cdm_id = ctx->cdm_id;
		rc = cam_isp_add_change_base(prepare, res_list,
					     &change_base_info, kmd_buf);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	if (ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_SFE)
		rc = cam_sfe_add_command_buffers(
			...
			ctx->res_list_sfe_out, ctx->sfe_out_map,
		...
		rc = cam_isp_add_command_buffers(
			...
			ctx->res_list_ife_out, ctx->vfe_out_map,
			CAM_ISP_IFE_OUT_RES_BASE,
			(CAM_ISP_IFE_OUT_RES_BASE + max_ife_out_res));
	else if (ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_CSID)
		rc = cam_isp_add_csid_command_buffers(
			prepare, kmd_buf, cam_csid_packet_generic_blob_handler,
			&ctx->base[base_idx]);
	...
	struct cam_isp_check_io_cfg_for_scratch *check_for_scratch)
{
	...
	if (ctx->scratch_buf_info.ife_scratch_config->num_config) {
		...
		for (i = 0;
		     i < ctx->scratch_buf_info.ife_scratch_config->num_config;
		     i++) {
			check_for_scratch->ife_scratch_res_info
				.ife_scratch_resources[i] =
...
static int cam_ife_hw_mgr_sfe_scratch_buf_update(
	int32_t opcode_type, uint32_t base_idx,
	struct cam_kmd_buf_info *kmd_buf,
	struct cam_hw_prepare_update_args *prepare,
	struct cam_ife_hw_mgr_ctx *ctx,
	...
	if ((sfe_res_info->sfe_rdi_cfg_mask) !=
		...
		if (opcode_type == CAM_ISP_PACKET_UPDATE_DEV) {
			...
			rc = cam_isp_sfe_add_scratch_buffer_cfg(
				...
				sfe_res_info->sfe_rdi_cfg_mask, prepare,
		...
		if (opcode_type == CAM_ISP_PACKET_INIT_DEV)
			ctx->scratch_buf_info.sfe_scratch_config
				->skip_scratch_cfg_streamon = true;
	}
	...
	struct cam_isp_prepare_hw_update_data *prepare_hw_data)
{
	...
	if ((!prepare->num_reg_dump_buf) ||
	    (prepare->num_reg_dump_buf > CAM_REG_DUMP_MAX_BUF_ENTRIES)) {
		CAM_DBG(CAM_ISP,
					 ...
					 void *prepare_hw_update_args)
{
	...
	CAM_DBG(CAM_REQ, "ctx[%pK][%u] Enter for req_id %lld", ctx,
	...
	rc = cam_packet_util_get_kmd_buffer(
		prepare->packet, &prepare_hw_data->kmd_cmd_buff_info);
	if (rc)
		return rc;

	if (ctx->ctx_config & CAM_IFE_CTX_CFG_FRAME_HEADER_TS) {
		rc = cam_ife_mgr_util_insert_frame_header(
			&prepare_hw_data->kmd_cmd_buff_info, prepare_hw_data,
			prepare->buf_tracker);
		if (rc)
			return rc;
	...
	if (ctx->flags.internal_cdm)
		rc = cam_packet_util_process_patches(
			prepare->packet, prepare->buf_tracker,
			hw_mgr->mgr_common.img_iommu_hdl,
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Patch ISP packet failed. ctx_idx: %u",
			ctx->ctx_index);
		return rc;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Invalid cmd buffers, ctx_idx: %u",
			ctx->ctx_index);
		return rc;
	}
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		rc = cam_ife_hw_mgr_update_cmd_buffer(
			ctx, prepare, &prepare_hw_data->kmd_cmd_buff_info,
			&cmd_buf_count, i);
     ...
     */
		if ((check_for_scratch.validate_for_sfe) &&
			...
			rc = cam_ife_hw_mgr_sfe_scratch_buf_update(
				prepare_hw_data->packet_opcode_type, i,
	...
	if ((ctx->ctx_config & CAM_IFE_CTX_CFG_FRAME_HEADER_TS) &&
	    ...
	    (!prepare_hw_data->frame_header_res_id)) {
		CAM_ERR(CAM_ISP,
			"Failed to configure frame header, ctx_idx: %u",
			ctx->ctx_index);
		goto end;
	}
   ...
   */
	if (prepare_hw_data->packet_opcode_type == CAM_ISP_PACKET_INIT_DEV) {
		...
		/* Per req reg dump */
		cam_ife_mgr_check_for_per_request_reg_dump(prepare,
							   prepare_hw_data);
		goto end;
	} else {
		cam_ife_mgr_check_for_per_request_reg_dump(prepare,
	...
	if (hw_mgr->csid_rup_en)
		rc = cam_ife_mgr_csid_add_reg_update(
			ctx, prepare, &prepare_hw_data->kmd_cmd_buff_info);

	else
		rc = cam_ife_mgr_isp_add_reg_update(
			ctx, prepare, &prepare_hw_data->kmd_cmd_buff_info);
	...
	if (prepare->num_out_map_entries && prepare->num_in_map_entries &&
		...
		if (ctx->ctx_type != CAM_IFE_CTX_TYPE_SFE)
			rc = cam_ife_hw_mgr_add_vfe_go_cmd(
				ctx, prepare,
				&prepare_hw_data->kmd_cmd_buff_info);
		else
			rc = cam_ife_hw_mgr_add_csid_go_cmd(
				ctx, prepare,
				&prepare_hw_data->kmd_cmd_buff_info);
		if (rc)
			CAM_ERR(CAM_ISP,
	...
	if (prepare_hw_data->kmd_cmd_buff_info.size <=
		...
		CAM_ERR(CAM_ISP,
			"No Sufficient memory for the Gen IRQ command, ctx_idx: %u",
			ctx->ctx_index);
		rc = -ENOMEM;
	}
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if (hw_mgr->csid_devices[hw_idx]) {
			rc |= hw_mgr->csid_devices[hw_idx]->hw_ops.process_cmd(
				...
				CAM_IFE_CSID_SOF_IRQ_DEBUG, &sof_irq_enable,
...
static inline void
cam_ife_hw_mgr_stop_bus_rd_for_res(struct cam_ife_hw_mgr_ctx *ctx,
				   uint32_t res_id)
{
	...
	list_for_each_entry(isp_hw_res, &ctx->res_list_ife_in_rd, list) {
		if (isp_hw_res->res_id == res_id) {
			CAM_ERR(CAM_ISP,
				"Stopping IFE/SFE bus rd res id 0x%x, ctx_idx: %u",
				res_id, ctx->ctx_index);
					  ...
					  uint32_t res_id, uint32_t hw_type)
{
	...
	if (hw_type == CAM_ISP_HW_TYPE_VFE) {
		if (cam_ife_hw_mgr_is_ife_out_port(res_id)) {
			isp_hw_res =
				&ctx->res_list_ife_out[ctx->vfe_out_map[res_id &
									0xFF]];
			CAM_ERR(CAM_ISP,
				"Stopping IFE out resource res id 0x%x ctx_idx: %u",
				res_id, ctx->ctx_index);
			cam_ife_hw_mgr_stop_hw_res(isp_hw_res);
		} else
		...
		if (cam_ife_hw_mgr_is_sfe_out_port(res_id)) {
			isp_hw_res =
				&ctx->res_list_sfe_out[ctx->sfe_out_map[res_id &
									0xFF]];
			CAM_ERR(CAM_ISP,
				"Stopping SFE out resource res id 0x%x ctx_idx: %u",
				res_id, ctx->ctx_index);
			cam_ife_hw_mgr_stop_hw_res(isp_hw_res);
		} else
			cam_ife_hw_mgr_stop_bus_rd_for_res(ctx, res_id);
...
static int
cam_ife_hw_mgr_dump_bus_info(uint32_t res_id,
			     struct cam_isp_hw_intf_data *hw_intf_data)
{
	...
	return hw_intf_data->hw_intf->hw_ops.process_cmd(
		hw_intf_data->hw_intf->hw_priv, CAM_ISP_HW_CMD_DUMP_BUS_INFO,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				rc = hw_intf->hw_ops.process_cmd(
					...
					hw_mgr_res->hw_res[i], sizeof(void *));
				if (rc)
					CAM_ERR(CAM_ISP,
...
static void cam_ife_mgr_pf_dump_mid_info(
	struct cam_ife_hw_mgr_ctx *ctx, struct cam_hw_cmd_args *hw_cmd_args,
	struct cam_isp_hw_intf_data *hw_intf_data, struct cam_packet *packet)
{
	...
	rc = hw_intf_data->hw_intf->hw_ops.process_cmd(
		hw_intf_data->hw_intf->hw_priv, cmd_update.cmd_type,
		&cmd_update, sizeof(struct cam_isp_hw_get_cmd_update));
	...
	hw_cmd_args->u.pf_cmd_args->pf_args->pf_context_info.resource_type =
		get_res.out_res_id;
...
static void cam_ife_mgr_dump_pf_data(struct cam_ife_hw_mgr *hw_mgr,
		...
		if (!g_ife_hw_mgr.hw_pid_support) {
			if (ctx->base[i].split_id == CAM_ISP_HW_SPLIT_LEFT)
				cam_ife_mgr_pf_dump_mid_info(
					ctx, hw_cmd_args, hw_intf_data, packet);
			continue;
		}

		for (j = 0; j < hw_intf_data->num_hw_pid; j++) {
			if (hw_intf_data->hw_pid[j] ==
			    pf_args->pf_smmu_info->pid) {
				*ctx_found = true;
				CAM_ERR(CAM_ISP,
					"PF found for %s%d pid: %u ctx_idx: %u",
					ctx->base[i].hw_type ==
							CAM_ISP_HW_TYPE_VFE ?
						"VFE" :
						"SFE",
				...
				if (ctx->flags.pf_mid_found) {
					cam_ife_hw_mgr_stop_pf_hw_res(
						ctx, ctx->pf_info.out_port_id,
						ctx->base[i].hw_type);
					cam_ife_hw_mgr_dump_bus_info(
						ctx->pf_info.out_port_id,
						hw_intf_data);
				}
				break;
			}
...
int cam_isp_config_csid_rup_aup(struct cam_ife_hw_mgr_ctx *ctx)
{
	int rc = 0, i, j, hw_idx;
	struct cam_isp_hw_mgr_res *hw_mgr_res;
	...
	for (j = 0; j < ctx->num_base; j++) {
		...
		list_for_each_entry(hw_mgr_res, res_list, list) {
			...
			for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
				...
				hw_idx = res->hw_intf->hw_idx;
				rup_args[hw_idx].res[rup_args[hw_idx].num_res] =
					res;
				rup_args[hw_idx].num_res++;
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		rc = res->hw_intf->hw_ops.process_cmd(
			res->hw_intf->hw_priv, CAM_ISP_HW_CMD_GET_REG_UPDATE,
			&rup_args[i],
			sizeof(struct cam_isp_csid_reg_update_args));
		if (rc)
			return rc;

		CAM_DBG(CAM_ISP, "Reg update for CSID: %u mup: %u ctx_idx: %u",
...
static int cam_ife_mgr_configure_scratch_for_ife(bool is_streamon,
						 struct cam_ife_hw_mgr_ctx *ctx)
{
	int i, j, rc = 0;
	uint32_t res_id;
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if ((is_streamon) &&
			    cam_ife_mgr_validate_for_io_buffers(
				    i, ctx->scratch_buf_info.ife_scratch_config
					       ->streamon_buf_mask))
				continue;

			CAM_DBG(CAM_ISP,
				"Configure scratch for IFE res: 0x%x io_addr %pK ctx_idx: %u",
				ife_buf_info->buf_info[i].res_id,
				port_info->io_addr, ctx->ctx_index);

			rc = cam_isp_sfe_send_scratch_buf_upd(
				0x0, CAM_ISP_HW_CMD_BUF_UPDATE,
				hw_mgr_res->hw_res[j], port_info, NULL, NULL);
			if (rc)
				goto end;
		}
	}
...
static int cam_ife_mgr_configure_scratch_for_sfe(bool is_streamon,
						 struct cam_ife_hw_mgr_ctx *ctx)
{
	int i, j, res_id, rc = 0;
	struct cam_isp_hw_mgr_res *hw_mgr_res;
	...
	for (i = 0; i < CAM_SFE_FE_RDI_NUM_MAX; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			CAM_DBG(CAM_ISP,
				...
				i, hw_mgr_res->hw_res[j]->res_id,
				(res_id - CAM_ISP_SFE_OUT_RES_RDI_0),
				buf_info->io_addr, ctx->ctx_index);

			rc = cam_isp_sfe_send_scratch_buf_upd(
				0x0, CAM_ISP_HW_CMD_BUF_UPDATE,
				hw_mgr_res->hw_res[j], buf_info, NULL, NULL);
			if (rc)
				goto end;
		}
	}
...
static int cam_hw_mgr_reset_out_of_sync_cnt(struct cam_ife_hw_mgr_ctx *ife_ctx)
{
	int rc = -EINVAL;
	uint32_t i;
	struct cam_isp_hw_mgr_res *hw_mgr_res;
	...
	list_for_each_entry(hw_mgr_res, &ife_ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
			if (hw_intf->hw_ops.process_cmd) {
				args.node_res = hw_mgr_res->hw_res[i];
...
static int
cam_ife_mgr_cmd_get_last_consumed_addr(struct cam_ife_hw_mgr_ctx *ife_ctx,
				       struct cam_isp_hw_done_event_data *done)
{
	...
	list_for_each_entry(hw_mgr_res, res_list_isp_src, list) {
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			rc = res->hw_intf->hw_ops.process_cmd(
				res->hw_intf->hw_priv,
				CAM_ISP_HW_CMD_GET_LAST_CONSUMED_ADDR, done,
				sizeof(struct cam_isp_hw_done_event_data));
...
static void *cam_ife_mgr_user_dump_stream_info(void *dump_struct,
					       uint8_t *addr_ptr)
{
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		if (!hw_cmd_args->u.internal_args) {
			CAM_ERR(CAM_ISP, "Invalid cmd arguments, ctx_idx: %u",
				ctx->ctx_index);
			return -EINVAL;
		}
		...
		case CAM_ISP_HW_MGR_CMD_CTX_TYPE:
			if (ctx->flags.is_fe_enabled && ctx->flags.is_offline)
				isp_hw_cmd_args->u.ctx_type =
					CAM_ISP_CTX_OFFLINE;
			else if (ctx->flags.is_fe_enabled &&
				 !ctx->flags.is_offline &&
				 ctx->ctx_type != CAM_IFE_CTX_TYPE_SFE)
				isp_hw_cmd_args->u.ctx_type = CAM_ISP_CTX_FS2;
			else if (ctx->flags.is_rdi_only_context ||
				 ctx->flags.is_lite_context ||
		...
		case CAM_ISP_HW_MGR_GET_PACKET_OPCODE:
			packet = (struct cam_packet *)isp_hw_cmd_args->cmd_data;
		...
		case CAM_ISP_HW_MGR_GET_LAST_CDM_DONE:
			isp_hw_cmd_args->cdm_done_ts = ctx->cdm_done_ts;
		...
		case CAM_ISP_HW_MGR_CMD_PROG_DEFAULT_CFG: {
			...
			rc = cam_ife_mgr_prog_default_settings(
				(false || skip_rup_aup), ctx);
		} break;
		case CAM_ISP_HW_MGR_GET_SOF_TS:
			rc = cam_ife_mgr_cmd_get_sof_timestamp(
				ctx, &isp_hw_cmd_args->u.sof_ts.curr,
		...
		case CAM_ISP_HW_MGR_DUMP_STREAM_INFO:
			rc = cam_common_user_dump_helper(
				...
				sizeof(int32_t),
				"ISP_STREAM_INFO_FROM_IFE_HW_MGR:");
			break;
	...
	case CAM_HW_MGR_CMD_REG_DUMP_ON_FLUSH:
		...
		if (rem_jiffies == 0)
			CAM_ERR(CAM_ISP,
				"config done completion timeout, Reg dump will be unreliable "
				"rc=%d ctx_index %u",
				rc, ctx->ctx_index);
...
static int cam_ife_mgr_user_dump_hw(struct cam_ife_hw_mgr_ctx *ife_ctx,
				    struct cam_hw_dump_args *dump_args)
{
	int rc = 0;

	if (!ife_ctx || !dump_args) {
		CAM_ERR(CAM_ISP, "Invalid parameters %pK %pK", ife_ctx,
			dump_args);
		rc = -EINVAL;
		goto end;
	}

	rc = cam_ife_mgr_handle_reg_dump(ife_ctx, ife_ctx->reg_dump_buf_desc,
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			...
			dump_args->request_id, dump_args->buf_handle,
...
static int cam_ife_mgr_dump(void *hw_mgr_priv, void *args)
{
	...
	struct cam_hw_intf *hw_intf;
	struct cam_ife_hw_mgr_ctx *ife_ctx =
		(struct cam_ife_hw_mgr_ctx *)dump_args->ctxt_to_hw_map;
	int i;
	int rc = 0;
	uint32_t hw_idx = 0;
	...
	if (!ife_ctx) {
		...
		CAM_ERR(CAM_ISP, "ISP CTX num_base null, ctx_idx: %u",
			ife_ctx->ctx_index);
		return -EINVAL;
	}
   ...
   */
	...
	for (i = 0; i < ife_ctx->num_base; i++) {
		...
		case CAM_ISP_HW_TYPE_CSID:
			...
			if (!hw_intf) {
				CAM_ERR(CAM_ISP,
					"hw_intf null, returning rc...ctx_idx: %u",
					ife_ctx->ctx_index);
				rc = -EINVAL;
				goto put_cpu_buf;
			}
			rc = hw_intf->hw_ops.process_cmd(
				hw_intf->hw_priv, CAM_ISP_HW_USER_DUMP,
				&isp_hw_dump_args,
...
static inline void cam_ife_hw_mgr_get_offline_sof_timestamp(uint64_t *timestamp,
							    uint64_t *boot_time)
{
					     ...
					     bool get_curr_timestamp)
{
     ...
     */
       ...
       */
			...
			csid_get_time.get_prev_timestamp =
...
static int cam_ife_mgr_recover_hw(void *priv, void *data)
{
	...
	case CAM_ISP_HW_ERROR_VIOLATION:
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"CTX stop failed(%d) ctx_idx: %u", rc,
					ctx->ctx_index);
				return rc;
			}
		}
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			...
			rc = cam_ife_hw_mgr_reset_csid(
				ctx, CAM_IFE_CSID_RESET_PATH, false);

			if (rc) {
				CAM_ERR(CAM_ISP, "Failed RESET, ctx_idx: %u",
					ctx->ctx_index);
				return rc;
			}
		}
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"CTX start failed(%d) ctx_idx: %u", rc,
					ctx->ctx_index);
				return rc;
			}
			CAM_DBG(CAM_ISP,
				"Started resources rc (%d) ctx_idx: %u", rc,
				ctx->ctx_index);
		}
...
static void
cam_ife_hw_mgr_trigger_crop_reg_dump(struct cam_hw_intf *hw_intf,
				     struct cam_isp_hw_event_info *event_info)
{
	int rc = 0, path_id = 0;
	...
	if (hw_intf->hw_ops.process_cmd) {
		rc = hw_intf->hw_ops.process_cmd(
			hw_intf->hw_priv, CAM_ISP_HW_CMD_CSID_DUMP_CROP_REG,
			&path_id, sizeof(path_id));
		if (rc)
	...
	struct cam_ife_hw_event_recovery_data *ife_mgr_recovery_data)
{
	...
	rc = cam_req_mgr_workq_enqueue_task(
		task, recovery_data->affected_ctx[0]->common.cb_priv,
		CRM_TASK_PRIORITY_0);
	return rc;
}
			       ...
			       uint32_t *affected_core, uint32_t size)
{
	...
	CAM_DBG(CAM_ISP, "Enter:max_idx = %d, ctx_idx: %u", max_idx,
...
static int cam_ife_hw_mgr_handle_csid_secondary_err_evts(
	uint32_t err_type, struct cam_isp_hw_event_info *event_info,
	struct cam_ife_hw_mgr_ctx *ctx)
{
	int rc = -EINVAL;
...
static int
cam_ife_hw_mgr_handle_csid_error(struct cam_ife_hw_mgr_ctx *ctx,
				 struct cam_isp_hw_event_info *event_info)
{
	int rc = -EINVAL;
	...
	err_evt_info =
		(struct cam_isp_hw_error_event_info *)event_info->event_data;
	...
	if (event_info->is_secondary_evt) {
		rc = cam_ife_hw_mgr_handle_csid_secondary_err_evts(
			err_type, event_info, ctx);
		if (rc)
			CAM_ERR(CAM_ISP,
				"Failed to handle CSID[%u] sec event for res: %d err: 0x%x on "
				"ctx: %u",
				event_info->hw_idx, event_info->res_id,
				err_type, ctx->ctx_index);
	...
	if (err_type & (CAM_ISP_HW_ERROR_CSID_OUTPUT_FIFO_OVERFLOW |
		...
		cam_ife_hw_mgr_check_and_notify_overflow(event_info, ctx,
							 &is_bus_overflow);
	...
	if (recoverable &&
	    (is_bus_overflow || (err_type & CAM_ISP_RECOVERABLE_CSID_ERRORS))) {
	...
	rc = cam_ife_hw_mgr_find_affected_ctx(
		&error_event_data, event_info->hw_idx, &recovery_data);
...
static int
cam_ife_hw_mgr_handle_csid_rup(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
			       struct cam_isp_hw_event_info *event_info)
{
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		...
		ife_hwr_irq_rup_cb(ife_hw_mgr_ctx->common.cb_priv,
				   ...
				   &rup_event_data);
		CAM_DBG(CAM_ISP, "RUP done for CSID:%d source %d ctx_idx: %u",
			event_info->hw_idx, event_info->res_id,
...
static int
cam_ife_hw_mgr_handle_csid_eof(struct cam_ife_hw_mgr_ctx *ctx,
			       struct cam_isp_hw_event_info *event_info)
{
	cam_hw_event_cb_func ife_hwr_irq_rup_cb;
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		if (atomic_read(&ctx->overflow_pending))
			break;
		ife_hwr_irq_rup_cb(ctx->common.cb_priv, CAM_ISP_HW_EVENT_EOF,
				   &eof_done_event_data);
		CAM_DBG(CAM_ISP,
...
static int
cam_ife_hw_mgr_handle_csid_camif_sof(struct cam_ife_hw_mgr_ctx *ctx,
				     struct cam_isp_hw_event_info *event_info)
{
	int rc = 0;
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		...
		ife_hw_irq_sof_cb(ctx->common.cb_priv, CAM_ISP_HW_EVENT_SOF,
				  (void *)&sof_done_event_data);
...
static int
cam_ife_hw_mgr_handle_csid_camif_epoch(struct cam_ife_hw_mgr_ctx *ctx,
				       struct cam_isp_hw_event_info *event_info)
{
	int rc = 0;
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		...
		ife_hw_irq_epoch_cb(ctx->common.cb_priv, CAM_ISP_HW_EVENT_EPOCH,
...
static int cam_ife_hw_mgr_handle_sfe_hw_dump_info(void *ctx, void *evt_info)
{
	...
	/* SFE in rd resources */
	list_for_each_entry(hw_mgr_res, &ife_hw_mgr_ctx->res_list_ife_in_rd,
			    list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
	...
	if (event_info->res_type == CAM_ISP_RESOURCE_SFE_OUT) {
		...
		hw_mgr_res = &ife_hw_mgr_ctx->res_list_sfe_out
				      [ife_hw_mgr_ctx->sfe_out_map[out_port]];
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
...
static int cam_ife_hw_mgr_handle_hw_dump_info(void *ctx, void *evt_info)
{
	struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx =
		(struct cam_ife_hw_mgr_ctx *)ctx;
	struct cam_isp_hw_event_info *event_info =
	...
	list_for_each_entry(hw_mgr_res, &ife_hw_mgr_ctx->res_list_ife_src,
			    list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
	...
	list_for_each_entry(hw_mgr_res, &ife_hw_mgr_ctx->res_list_ife_csid,
			    list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
	...
	if (event_info->res_type == CAM_ISP_RESOURCE_VFE_OUT) {
		out_port = event_info->res_id & 0xFF;
		hw_mgr_res = &ife_hw_mgr_ctx->res_list_ife_out
				      [ife_hw_mgr_ctx->vfe_out_map[out_port]];
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
...
static int
cam_ife_hw_mgr_handle_sfe_hw_err(struct cam_ife_hw_mgr_ctx *ctx,
				 struct cam_isp_hw_event_info *event_info)
{
	struct cam_isp_hw_error_event_info *err_evt_info;
	struct cam_isp_hw_error_event_data error_event_data = { 0 };
	...
	if (err_evt_info->err_type & CAM_SFE_IRQ_STATUS_VIOLATION) {
		...
		CAM_DBG(CAM_ISP, "Notify context for SFE error, ctx_idx: %u",
			ctx->ctx_index);
...
static int
cam_ife_hw_mgr_handle_hw_err(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
			     struct cam_isp_hw_event_info *event_info)
{
	...
	if (err_type == CAM_VFE_IRQ_STATUS_VIOLATION) {
		...
		if (err_evt_info->err_mask &
		    CAM_VFE_IRQ_ERR_MASK_HWPD_VIOLATION)
			error_event_data.error_type |=
				CAM_ISP_HW_ERROR_HWPD_VIOLATION;
...
static int
cam_ife_hw_mgr_handle_hw_rup(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
			     struct cam_isp_hw_event_info *event_info)
{
	cam_hw_event_cb_func ife_hwr_irq_rup_cb;
	struct cam_isp_hw_reg_update_event_data rup_event_data;
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		if ((ife_hw_mgr_ctx->flags.is_dual) &&
		    (event_info->hw_idx != ife_hw_mgr_ctx->left_hw_idx))
			break;
	...
	CAM_DBG(CAM_ISP, "RUP done for VFE:%d source %d, ctx_idx: %u",
		event_info->hw_idx, event_info->res_id,
		ife_hw_mgr_ctx->ctx_index);
...
static int
cam_ife_hw_mgr_handle_hw_epoch(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
			       struct cam_isp_hw_event_info *event_info)
{
	...
	CAM_DBG(CAM_ISP, "Epoch for VFE:%d source %d ctx_idx: %u",
		event_info->hw_idx, event_info->res_id,
		ife_hw_mgr_ctx->ctx_index);
...
static int
cam_ife_hw_mgr_handle_hw_sof(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
			     struct cam_isp_hw_event_info *event_info)
{
	...
	CAM_DBG(CAM_ISP, "SOF for VFE:%d source %d ctx_idx: %u",
		event_info->hw_idx, event_info->res_id,
		ife_hw_mgr_ctx->ctx_index);
...
static int
cam_ife_hw_mgr_handle_hw_eof(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
			     struct cam_isp_hw_event_info *event_info)
{
	...
	CAM_DBG(CAM_ISP, "EOF for VFE:%d source %d ctx_idx: %u",
		event_info->hw_idx, event_info->res_id,
		ife_hw_mgr_ctx->ctx_index);
	...
	struct cam_ife_sfe_scratch_buf_info *buf_info)
{
	...
	cmp_addr = cam_smmu_is_expanded_memory() ?
			   CAM_36BIT_INTF_GET_IOVA_BASE(final_addr) :
			   final_addr;
...
static int cam_ife_hw_mgr_check_ife_scratch_buf_done(
	struct cam_ife_scratch_buf_cfg *scratch_cfg, uint32_t res_id,
	uint32_t last_consumed_addr)
{
	int rc = 0, i;
	struct cam_ife_sfe_scratch_buf_info *buf_info;
...
static int cam_ife_hw_mgr_check_sfe_scratch_buf_done(
	struct cam_sfe_scratch_buf_cfg *scratch_cfg, uint32_t res_id,
	uint32_t last_consumed_addr)
{
	int rc = 0;
	struct cam_ife_sfe_scratch_buf_info *buf_info;
	...
	case CAM_ISP_SFE_OUT_RES_RDI_2:
		buf_info =
			&scratch_cfg
				 ->buf_info[res_id - CAM_ISP_SFE_OUT_RES_RDI_0];
		if (!buf_info->config_done)
			return 0;

		if (cam_ife_hw_mgr_last_consumed_addr_check(last_consumed_addr,
							    buf_info)) {
			CAM_DBG(CAM_ISP,
...
static int cam_ife_hw_mgr_check_for_scratch_buf_done(
	struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx, enum cam_isp_hw_type hw_type,
	uint32_t res_id, uint32_t last_consumed_addr)
{
	...
	switch (hw_type) {
	case CAM_ISP_HW_TYPE_VFE:
		if (ife_hw_mgr_ctx->scratch_buf_info.ife_scratch_config
			    ->num_config)
			rc = cam_ife_hw_mgr_check_ife_scratch_buf_done(
				ife_hw_mgr_ctx->scratch_buf_info
					.ife_scratch_config,
...
static int
cam_ife_hw_mgr_handle_hw_buf_done(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
				  struct cam_isp_hw_event_info *event_info)
{
	...
	bufdone_evt_info =
		(struct cam_isp_hw_bufdone_event_info *)event_info->event_data;
	...
	if (ife_hw_mgr_ctx->flags.is_sfe_fs ||
		...
		rc = cam_ife_hw_mgr_check_for_scratch_buf_done(
			ife_hw_mgr_ctx, event_info->hw_type, event_info->res_id,
			bufdone_evt_info->last_consumed_addr);
		if (rc)
			return 0;
	}
	...
	buf_done_event_data.last_consumed_addr =
		bufdone_evt_info->last_consumed_addr;
	...
	if (buf_done_event_data.resource_handle > 0 && ife_hwr_irq_wm_done_cb) {
		CAM_DBG(CAM_ISP, "Notify ISP context for %u handles in ctx: %u",
			buf_done_event_data.resource_handle,
			ife_hw_mgr_ctx->ctx_index);
		ife_hwr_irq_wm_done_cb(ife_hw_mgr_ctx->common.cb_priv,
				       CAM_ISP_HW_EVENT_DONE,
				       (void *)&buf_done_event_data);
	}
...
static int
cam_ife_hw_mgr_handle_ife_event(struct cam_ife_hw_mgr_ctx *ctx, uint32_t evt_id,
				struct cam_isp_hw_event_info *event_info)
{
	int rc = 0;

	CAM_DBG(CAM_ISP, "Handle IFE[%u] %s event in ctx: %u",
		event_info->hw_idx, cam_isp_hw_evt_type_to_string(evt_id),
		ctx->ctx_index);
...
static int
cam_ife_hw_mgr_handle_csid_event(struct cam_ife_hw_mgr_ctx *ctx,
				 uint32_t evt_id,
				 struct cam_isp_hw_event_info *event_info)
{
	int rc = 0;
...
static int
cam_ife_hw_mgr_handle_sfe_event(struct cam_ife_hw_mgr_ctx *ctx, uint32_t evt_id,
				struct cam_isp_hw_event_info *event_info)
{
	int rc = 0;
					...
					void *evt_info)
{
	...
	if (!evt_info || !priv) {
		CAM_ERR(CAM_ISP, "Invalid data evt_info: %pK priv: %pK",
			evt_info, priv);
		return rc;
	}
	...
	if (rc)
		CAM_ERR(CAM_ISP,
			...
			cam_isp_hw_evt_type_to_string(evt_id), evt_id,
			event_info->hw_type, ctx->ctx_index, rc);
...
static int cam_ife_hw_mgr_sort_dev_with_caps(struct cam_ife_hw_mgr *ife_hw_mgr)
{
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		ife_hw_mgr->csid_rup_en = ife_hw_mgr->csid_hw_caps[i].rup_en;
		ife_hw_mgr->csid_camif_irq_support =
...
static int cam_ife_set_sfe_cache_debug(void *data, u64 val)
{
	...
	for (i = 0; i < CAM_SFE_HW_NUM_MAX; i++) {
		if ((g_ife_hw_mgr.sfe_devices[i]) && (i == hw_idx)) {
			...
			rc = hw_intf->hw_ops.process_cmd(
				hw_intf->hw_priv,
				CAM_ISP_HW_CMD_SET_SFE_DEBUG_CFG, &debug_cfg,
...
DEFINE_DEBUGFS_ATTRIBUTE(cam_ife_sfe_cache_debug, cam_ife_get_sfe_cache_debug,
...
static int __cam_ife_mgr_test_irq_line(struct cam_hw_intf *hw_intf, int *n_intf,
				       int *n_success, const char *hw_name,
				       int idx)
{
	int rc = -EINVAL;
	...
	if (!rc) {
		(*n_success)++;
		CAM_INFO(CAM_ISP, "%s:%u IRQ line verified successfully",
			 hw_name, hw_intf->hw_idx);
	} else {
		CAM_ERR(CAM_ISP, "%s:%u failed to verify IRQ line", hw_name,
			hw_intf->hw_idx);
	}
...
static int cam_ife_mgr_test_irq_lines(struct cam_ife_hw_mgr *hw_mgr)
{
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		if (hw_mgr->csid_devices[i]) {
			rc = __cam_ife_mgr_test_irq_line(
				hw_mgr->csid_devices[i], &n_intf, &n_success,
				"CSID", i);
		}
	}
	...
	for (i = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		if (hw_mgr->ife_devices[i] && hw_mgr->ife_devices[i]->hw_intf) {
			rc = __cam_ife_mgr_test_irq_line(
				hw_mgr->ife_devices[i]->hw_intf, &n_intf,
				&n_success, "IFE", i);
		}
	}
	...
	for (i = 0; i < CAM_SFE_HW_NUM_MAX; i++) {
		if (hw_mgr->sfe_devices[i] && hw_mgr->sfe_devices[i]->hw_intf) {
			rc = __cam_ife_mgr_test_irq_line(
				hw_mgr->sfe_devices[i]->hw_intf, &n_intf,
				&n_success, "SFE", i);
		}
	}
	...
	if (n_intf) {
		if (n_intf == n_success)
			CAM_INFO(CAM_ISP,
				 "verified IRQ lines for all %d hw-intf",
				 n_intf);
		else
			CAM_ERR(CAM_ISP, "verified %d/%d IRQ lines", n_success,
				n_intf);
	} else {
...
static ssize_t cam_ife_hw_mgr_perfcnt_write(struct file *file,
					    ...
					    size_t size, loff_t *loff_t)
{
	...
	*delimiter2 = '\0';
	...
	CAM_INFO(
		CAM_ISP,
		"Failed to set perf counter debug setting - invalid input format "
		"[input: %s counter: %u counter_val: %u]",
		input_buf, counter_idx, counter_val);
	return -EINVAL;
}

static ssize_t cam_ife_hw_mgr_perfcnt_read(struct file *file, char __user *ubuf,
					   size_t size, loff_t *loff_t)
{
	...
	if ((!g_ife_hw_mgr.isp_caps.num_ife_perf_counters) &&
		...
		len += scnprintf(display_string + len, (256 - len),
				 ...
				 g_ife_hw_mgr.isp_caps.num_sfe_perf_counters);
		len += scnprintf(
			display_string + len, (256 - len),
			"To choose counter write to same file - "
			"\"<hw>_<counter_index>_<reg_val>\"\nEx. \"ife_1_6619140\"\n\n");
	}
					...
					struct cam_isp_irq_inject_param *param)
{
	...
	case CAM_ISP_HW_TYPE_CSID:
		rc = cam_ife_hw_mgr_csid_irq_inject_or_dump_desc(hw_mgr, param,
								 true);
		break;
	case CAM_ISP_HW_TYPE_VFE:
		rc = cam_ife_hw_mgr_vfe_irq_inject_or_dump_desc(hw_mgr, param,
								true);
		break;
	case CAM_ISP_HW_TYPE_SFE:
		rc = cam_ife_hw_mgr_sfe_irq_inject_or_dump_desc(hw_mgr, param,
								true);
		break;
	default:
		strlcat(irq_inject_display_buf, "No matched HW_TYPE\n",
...
static inline int cam_isp_irq_inject_get_hw_type(int32_t *hw_type, char *token)
{
	...
	char *token, bool *is_query)
{
	...
	case HW_TYPE:
		if (strnstr(token, "?", 1)) {
			*is_query = true;
			offset += scnprintf(
				line_buf + offset, LINE_BUFFER_LEN - offset,
				"Interruptable HW : CSID | IFE | SFE\n");
			break;
		}
		rc = cam_isp_irq_inject_get_hw_type(&irq_inject_param->hw_type,
						    token);
		if (rc) {
		...
		if (strnstr(token, "?", 1)) {
			*is_query = true;
			...
			for (i = 0; i < CAM_ISP_REG_UNIT_MAX; i++) {
				if ((irq_inject_param->hw_type ==
				     CAM_ISP_HW_TYPE_CSID) &&
				    i > CAM_ISP_CSID_PATH_RDI4_REG)
					continue;
				else if ((irq_inject_param->hw_type ==
					  CAM_ISP_HW_TYPE_VFE) &&
					 ((i <
...
static int cam_isp_irq_inject_command_parser(
	struct cam_isp_irq_inject_param *irq_inject_param, char **msg,
	uint32_t max_params, cam_isp_irq_inject_cmd_parse_handler cmd_parse_cb,
	bool *is_query)
{
	char *token = NULL;
	char *line_buf = NULL;
	...
	token = strsep(msg, ":");
	while (token != NULL) {
		rc = cmd_parse_cb(irq_inject_param, param_index, token,
				  is_query);
		if (rc) {
			offset += scnprintf(line_buf + offset,
					    LINE_BUFFER_LEN - offset,
...
static ssize_t cam_isp_irq_injection_read(struct file *file, char __user *ubuf,
					  size_t size, loff_t *ppos)
{
	...
	strlcat(irq_inject_display_buf, IRQ_INJECT_USAGE_STRING,
		IRQ_INJECT_DISPLAY_BUF_LEN);
	...
	for (i = 0; i < MAX_INJECT_SET; i++) {
		...
		offset += scnprintf(
			line_buf + offset, LINE_BUFFER_LEN - offset,
			"injected param[%d] : hw_type:%s hw_idx:%d reg_unit:%d "
			"irq_mask:%#x req_id:%d\n",
					   ...
					   loff_t *ppos)
{
	...
	for (i = 0; i < MAX_INJECT_SET; i++) {
		...
		if ((rc != MAX_PARAMS_FOR_IRQ_INJECT) || is_query) {
			cam_isp_irq_inject_clear_params(
				&g_ife_hw_mgr.irq_inject_param[i]);
			if (!is_query)
				offset += scnprintf(
					line_buf + offset,
					LINE_BUFFER_LEN - offset,
			...
			offset += scnprintf(
				line_buf + offset, LINE_BUFFER_LEN - offset,
				"Setting param[%d] : hw_type:%s hw_idx:%d "
				"reg_unit:%d irq_mask:%#x req_id:%d\n",
				i,
				__cam_isp_irq_inject_hw_type_to_name(hw_type),
				g_ife_hw_mgr.irq_inject_param[i].hw_idx,
				g_ife_hw_mgr.irq_inject_param[i].reg_unit,
				g_ife_hw_mgr.irq_inject_param[i].irq_mask,
				g_ife_hw_mgr.irq_inject_param[i].req_id);
...
static int cam_ife_hw_mgr_debug_register(void)
{
	...
	debugfs_create_bool("disable_ubwc_comp", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry,
			    &g_ife_hw_mgr.debug_cfg.disable_ubwc_comp);
	debugfs_create_file("sfe_debug", 0644, g_ife_hw_mgr.debug_cfg.dentry,
			    NULL, &cam_ife_sfe_debug);
	debugfs_create_file("sfe_sensor_diag_sel", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry, NULL,
			    &cam_ife_sfe_sensor_diag_debug);
	...
	debugfs_create_file("sfe_cache_debug", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry, NULL,
			    &cam_ife_sfe_cache_debug);
	debugfs_create_file("test_irq_line", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry, NULL,
			    &cam_isp_test_irq_line);
	debugfs_create_file("isp_perf_counters", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry, NULL,
			    &cam_ife_hw_mgr_perfcnter_debug);
	debugfs_create_file("ife_csid_testbus", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry, NULL,
			    &cam_ife_csid_testbus_debug);
	debugfs_create_bool("disable_isp_drv", 0644,
			    g_ife_hw_mgr.debug_cfg.dentry,
			    &g_ife_hw_mgr.debug_cfg.disable_isp_drv);
						 ...
						 void *priv_data)
{
	...
	list_for_each_entry_safe(ctx, ctx_temp, &hw_mgr->used_ctx_list, list) {
		...
		ctx_md = (struct cam_ife_hw_mini_dump_ctx *)((uint8_t *)dst +
							     dumped_len);
		mgr_md->ctx[i] = ctx_md;
		...
		ctx_md->overflow_pending = atomic_read(&ctx->overflow_pending);
		...
		for (j = 0; j < ctx->num_base; j++) {
			...
			if (ctx->base[j].hw_type == CAM_ISP_HW_TYPE_CSID) {
				...
				ctx_md->csid_md[hw_idx] =
					(void *)((uint8_t *)dst + dumped_len);
				memset(&hw_dump_args, 0, sizeof(hw_dump_args));
				hw_dump_args.start_addr =
					ctx_md->csid_md[hw_idx];
				hw_dump_args.len = remain_len;
				hw_intf->hw_ops.process_cmd(
					hw_intf->hw_priv,
					CAM_ISP_HW_CSID_MINI_DUMP,
					&hw_dump_args, sizeof(hw_dump_args));
		...
		if (ctx->common.mini_dump_cb) {
			...
			rc = ctx->common.mini_dump_cb(ctx->common.cb_priv,
						      &hw_dump_args);
			if (rc ||
			    (hw_dump_args.bytes_written + dumped_len > len))
				goto end;
...
static void cam_ife_hw_mgr_attach_sfe_sys_cache_id(bool shared, uint32_t type,
						   uint32_t *hw_id,
						   uint32_t num_sfe)
{
	int i;
	...
	if (shared) {
		...
		g_ife_hw_mgr.sfe_cache_info[*hw_id].supported_scid_idx |=
			BIT(type);
		CAM_DBG(CAM_ISP, "SFE[%u] non-shared cache_type %d, type %u",
			*hw_id, g_ife_hw_mgr.sys_cache_info[type].scid, type);
...
static int cam_ife_mgr_populate_sys_cache_id(void)
{
	...
	for (i = 0; i < CAM_SFE_HW_NUM_MAX && g_ife_hw_mgr.sfe_devices[i];
	     i++) {
	...
	for (i = 0; i < num_small_scid; i++) {
		...
		cam_ife_hw_mgr_attach_sfe_sys_cache_id(
			shared, g_ife_hw_mgr.sys_cache_info[i].type, &hw_id,
			num_sfe);
	}
	...
	for (i = CAM_LLCC_LARGE_1; i < CAM_LLCC_MAX; i++) {
		...
		rc = cam_cpas_configure_staling_llcc(
			i, CAM_LLCC_STALING_MODE_NOTIFY,
			CAM_LLCC_NOTIFY_STALING_EVICT, 1);
		if ((num_large_scid == 1) && (num_large_scid < num_sfe) &&
		    (rc == -EOPNOTSUPP)) {
			CAM_ERR(CAM_ISP,
				"Fatal error llcc staling feature is not supported cache: %d",
				i);
			rc = -EFAULT;
		} else if (!rc && num_large_scid > 1) {
			CAM_ERR(CAM_ISP,
				"Fatal error llcc staling feature is supported more large cache %d",
				i);
			rc = -EFAULT;
			...
			CAM_INFO(CAM_ISP,
				 ...
				 i, rc);
			g_ife_hw_mgr.sys_cache_info[i].llcc_staling_support =
				true;
		}
	}

	CAM_DBG(CAM_ISP, "Num SCIDs Small:%u Large: %u", num_small_scid,
		num_large_scid);
	for (i = 0; i < num_sfe; i++)
			...
			uint32_t isp_device_type)
{
	...
	g_ife_hw_mgr.isp_caps.max_vfe_out_res_type = isp_cap.max_out_res_type;
	g_ife_hw_mgr.isp_caps.num_ife_perf_counters = isp_cap.num_perf_counters;
	max_ife_out_res = g_ife_hw_mgr.isp_caps.max_vfe_out_res_type & 0xFF;
	memset(&isp_cap, 0x0, sizeof(struct cam_isp_hw_cap));

	for (i = 0; i < path_port_map.num_entries; i++) {
		g_ife_hw_mgr.path_port_map.entry[i][0] =
			path_port_map.entry[i][0];
		g_ife_hw_mgr.path_port_map.entry[i][1] =
			path_port_map.entry[i][1];
	}
		...
		if (!rc) {
			if (j == 0) {
				...
				rc = sfe_device->hw_ops.process_cmd(
					sfe_hw, CAM_ISP_HW_CMD_QUERY_CAP,
					&isp_cap,
					sizeof(struct cam_isp_hw_cap));
				CAM_DBG(CAM_ISP,
					...
					isp_cap.num_perf_counters);
				if (!rc) {
					g_ife_hw_mgr.isp_caps
						.max_sfe_out_res_type =
						isp_cap.max_out_res_type;
					g_ife_hw_mgr.isp_caps
						.num_sfe_perf_counters =
						isp_cap.num_perf_counters;
					max_sfe_out_res =
						g_ife_hw_mgr.isp_caps
							.max_sfe_out_res_type &
   ...
   */
	...
	for (i = 0; i < CAM_IFE_CTX_MAX; i++) {
		...
		ctx_pool->vfe_out_map = kzalloc(
			(max_ife_out_res * sizeof(uint8_t)), GFP_KERNEL);
		if (!ctx_pool->vfe_out_map) {
			rc = -ENOMEM;
			CAM_ERR(CAM_ISP,
				"Alloc failed for ife out res list, ctx_idx: %u",
				ctx_pool->ctx_index);
			goto end;
		}

		if (max_sfe_out_res) {
			ctx_pool->sfe_out_map =
				kzalloc((max_sfe_out_res * sizeof(uint8_t)),
					GFP_KERNEL);
			if (!ctx_pool->sfe_out_map) {
				rc = -ENOMEM;
				CAM_ERR(CAM_ISP,
					"Alloc failed for SFE out res list, ctx_idx: %u",
					ctx_pool->ctx_index);
				goto end;
			}
			memset(ctx_pool->sfe_out_map, 0xff,
			       max_sfe_out_res * sizeof(uint8_t));
		}
	...
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "Unable to create worker, ctx_idx: %u",
			ctx_pool->ctx_index);
		goto end;
	}
	...
	rc = cam_cpas_prepare_subpart_info(CAM_IFE_LITE_HW_IDX,
					   g_num_ife_lite_available,
					   g_num_ife_lite_functional);
	if (rc)
		CAM_ERR(CAM_ISP, "Failed to populate num_ife_lites, rc: %d",
			rc);
	...
	cam_common_register_mini_dump_cb(cam_ife_hw_mgr_mini_dump_cb, "CAM_ISP",
					 NULL);
	cam_ife_mgr_test_irq_lines_at_probe(&g_ife_hw_mgr);

	/* Allocate memory for perf counters */
	if (g_ife_hw_mgr.isp_caps.num_ife_perf_counters) {
		g_ife_hw_mgr.debug_cfg.ife_perf_counter_val =
			kcalloc(g_ife_hw_mgr.isp_caps.num_ife_perf_counters,
				sizeof(uint32_t), GFP_KERNEL);
	}

	if (g_ife_hw_mgr.isp_caps.num_sfe_perf_counters) {
		g_ife_hw_mgr.debug_cfg.sfe_perf_counter_val =
			kcalloc(g_ife_hw_mgr.isp_caps.num_sfe_perf_counters,
				sizeof(uint32_t), GFP_KERNEL);
	}
