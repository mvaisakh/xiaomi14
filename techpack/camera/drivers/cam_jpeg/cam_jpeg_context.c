...
static int cam_jpeg_context_dump_active_request(void *data, void *args)
{
	...
	if (!ctx || !pf_args) {
		CAM_ERR(CAM_JPEG, "Invalid ctx %pK or pf args %pK", ctx,
			pf_args);
		return -EINVAL;
	}
...
static int __cam_jpeg_ctx_handle_hw_event(void *ctx, uint32_t evt_id,
					  void *evt_data)
{
	int rc;
...
static int cam_jpeg_context_validate_event_notify_injection(
	struct cam_context *ctx, struct cam_hw_inject_evt_param *evt_params)
{
	...
	case V4L_EVENT_CAM_REQ_MGR_NODE_EVENT: {
		...
		default:
			CAM_ERR(CAM_JPEG,
				"Invalid event type %u for node event injection event cause: %u "
				"req id: %llu ctx id: %u dev hdl: %d",
				node_evt_params->event_type,
				node_evt_params->event_cause, req_id,
				ctx->ctx_id, ctx->dev_hdl);
			return -EINVAL;
		}

		CAM_INFO(
			CAM_JPEG,
			"Inject Node evt: event type: %u event cause: %u req id: %llu ctx "
			"id: %u dev hdl: %d",
			node_evt_params->event_type,
			node_evt_params->event_cause, req_id, ctx->ctx_id,
			ctx->dev_hdl);
		break;
	}
	case V4L_EVENT_CAM_REQ_MGR_PF_ERROR: {
		...
		if (!non_fatal_en) {
			CAM_ERR(CAM_JPEG,
				"Fail to inject page fault event notification. Page "
				"fault is fatal for JPEG");
			return -EINVAL;
		}
	...
	default:
		CAM_ERR(CAM_JPEG, "Event notification type not supported: %u",
			evt_type);
		rc = -EINVAL;
	}
...
static int cam_jpeg_context_inject_evt(void *context, void *evt_args)
{
	...
	if (!ctx || !evt_args) {
		CAM_ERR(CAM_JPEG, "invalid params ctx %s event args %s",
	...
	if (evt_params->inject_id == CAM_COMMON_EVT_INJECT_BUFFER_ERROR_TYPE) {
		...
		if (buf_err_params->sync_error > CAM_SYNC_JPEG_EVENT_START ||
			...
			CAM_INFO(
				CAM_JPEG,
				"Inject buffer sync error %u ctx id: %u req id %llu",
				buf_err_params->sync_error, ctx->ctx_id,
				evt_params->req_id);
		} else {
...
/* top state machine */
static struct cam_ctx_ops cam_jpeg_ctx_state_machine[CAM_CTX_STATE_MAX] = {
    /* Uninit */
    {
        .ioctl_ops = {},
        .crm_ops = {},
