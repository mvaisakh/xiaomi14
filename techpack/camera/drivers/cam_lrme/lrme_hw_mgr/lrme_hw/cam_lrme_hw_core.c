...
static int cam_lrme_dump_regs_to_buf(struct cam_lrme_frame_request *req,
				     ...
				     struct cam_lrme_hw_dump_args *dump_args)
{
	...
	if (!lrme_hw || !req || !dump_args) {
		CAM_ERR(CAM_LRME, "Invalid params %pK, %pK, %pK", lrme_hw, req,
			dump_args);
		return -EINVAL;
	}
	...
	scnprintf(hdr->tag, CAM_LRME_HW_DUMP_TAG_MAX_LEN, "LRME_REG:");
	...
	*addr++ = soc_info->index;
	...
	dump_args->offset += hdr->size + sizeof(struct cam_lrme_hw_dump_header);
			    ...
			    struct cam_lrme_hw_dump_args *dump_args)
{
	...
	dst = (uint8_t *)dump_args->cpu_addr + dump_args->offset;
	hdr = (struct cam_lrme_hw_dump_header *)dst;
	scnprintf(hdr->tag, CAM_LRME_HW_DUMP_TAG_MAX_LEN, "LRME_REQ:");
	...
	struct cam_lrme_hw_cmd_config_args *config_args)
{
	...
	if (num_regval_pairs) {
		...
		hw_cdm_info->cdm_ops->cdm_write_regrandom(
			cmd_buf_addr, num_regval_pairs, reg_val_pair);
		cmd_buf_addr += size;
		available_size -= (size * 4);
