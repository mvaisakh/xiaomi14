...
#define IPATEST_DBG(fmt, args...)                                             \
	do {                                                                  \
		pr_debug(IPA_TEST_DRV_NAME " %s:%d " fmt, __func__, __LINE__, \
			 ##args);                                             \
	} while (0)

#define IPATEST_ERR(fmt, args...)                                           \
	do {                                                                \
		pr_err(IPA_TEST_DRV_NAME " %s:%d " fmt, __func__, __LINE__, \
		       ##args);                                             \
	} while (0)
...
int ipa_sys_setup(struct ipa_sys_connect_params *sys_in,
		  unsigned long *ipa_gsi_hdl, u32 *ipa_pipe_num, u32 *clnt_hdl,
		  bool en_status);
...
static void test_alloc_mem(struct ipa_mem_buffer *mem)
{
	...
	if (pdev != NULL) {
		mem->base = dma_alloc_coherent(pdev, mem->size, &dma_addr,
					       GFP_KERNEL);
...
void print_buff(void *data, size_t size)
{
	...
	for (i = 0; i < num_lines; i++) {
		str[0] = '\0';
		for (j = 0;
		     (j < bytes_in_line) && ((i * bytes_in_line + j) < size);
		     j++) {
			snprintf(
				tmp, sizeof(tmp), "%02x ",
	...
	for (i = 0; i < ipa_test->num_rx_channels; i++) {
		...
		if (!rx_channel->rx_pool_inited) {
			...
			for (j = 0; j < RX_NUM_BUFFS; j++) {
				...
				gsi_xfer.xfer_user_data =
					(void *)(rx_channel->mem.phys_base +
						 j * count);

				IPATEST_DBG("submitting credit to gsi\n");
				res |= gsi_queue_xfer(
					rx_channel->ep.gsi_chan_hdl, 1,
					&gsi_xfer, true);
				if (res) {
					IPATEST_ERR(
						"gsi_queue_xfer failed %d\n",
						res);
					return -EFAULT;
				}
			}
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	channel_dev->mem_buff_index =
		(channel_dev->mem_buff_index + 1) % TX_NUM_BUFFS;
	return count;
}
				...
				size_t count, loff_t *f_pos)
{
	...
	offset = (phys_addr_t)xfer_notify.xfer_user_data -
		 channel_dev->mem.phys_base;
	res = copy_to_user(buf, channel_dev->mem.base + offset,
			   xfer_notify.bytes_xfered);
	if (res < 0) {
...
static ssize_t channel_read(struct file *filp, char __user *buf, size_t count,
			    loff_t *f_pos)
{
 ...
 */
int create_channel_device_by_type(const int index, const char *dev_name,
				  struct channel_dev **channel_dev_ptr,
				  size_t mem_size, enum fops_type type)
{
	int ret;
	char name[MAX_CHANNEL_NAME];
	...
	*channel_dev_ptr = kzalloc(sizeof(struct channel_dev), GFP_KERNEL);
	...
	ret = alloc_chrdev_region(&channel_dev->dev_num, 0, 1,
				  channel_dev->name);
	if (ret) {
	...
	if (!ret)
		IPATEST_DBG(
			"Channel device:%d, name:%s created, address:0x%px.\n",
...
int create_channel_device(const int index, const char *dev_name,
			  struct channel_dev **channel_dev_ptr, size_t mem_size)
{
	return create_channel_device_by_type(index, dev_name, channel_dev_ptr,
					     mem_size, IPA_TEST_REG_CHANNEL);
}
...
static int datapath_read_data(void *element, int size)
{
	...
	if (kfifo_is_empty(&p_data_path_ctx->fifo)) {
		...
		if (!res) {
			IPATEST_ERR(
				"Error in wait_for_ipa_receive_completion\n");
			return -EINVAL;
		}
...
static int datapath_write_fifo(void *element, int size)
{
	int res;
				...
				size_t size, loff_t *p_pos)
{
	...
	if (datapath_read_data((void *)&p_skb, sizeof(struct sk_buff *)) < 0) {
...
static void notify_ipa_write_done(void *priv, enum ipa_dp_evt_type evt,
				  unsigned long data)
{
	...
	if (evt == IPA_WRITE_DONE) {
		...
		print_buff(((struct sk_buff *)data)->data,
...
static void notify_ipa_received(void *priv, enum ipa_dp_evt_type evt,
				unsigned long data)
{
...
static void ipa_test_gsi_evt_ring_err_cb(struct gsi_evt_err_notify *notify)
{
	if (notify) {
		...
		case GSI_EVT_UNSUPPORTED_INTER_EE_OP_ERR:
			IPATEST_ERR(
				"Received GSI_EVT_UNSUPPORTED_INTER_EE_OP_ERR\n");
			break;
...
static void ipa_test_gsi_chan_err_cb(struct gsi_chan_err_notify *notify)
{
	if (notify) {
		...
		case GSI_CHAN_NON_ALLOCATED_EVT_ACCESS_ERR:
			IPATEST_ERR(
				"Received GSI_CHAN_NON_ALLOCATED_EVT_ACCESS_ERR\n");
			break;
		...
		case GSI_CHAN_UNSUPPORTED_INTER_EE_OP_ERR:
			IPATEST_ERR(
				"Received GSI_CHAN_UNSUPPORTED_INTER_EE_OP_ERR\n");
			break;
...
int connect_ipa_to_apps(struct test_endpoint_sys *rx_ep,
			enum ipa_client_type client, u32 pipe_index,
			unsigned long ipa_gsi_hdl)
{
	...
	rx_ep->gsi_channel_props.ch_id = gsi_ep_config->ipa_gsi_chan_num;
	...
	rx_ep->gsi_channel_props.ring_base_vaddr =
		dma_alloc_coherent(pdev, GSI_CHANNEL_RING_LEN, &dma_addr, 0);
	...
	res = ipa3_sys_update_gsi_hdls(pipe_index, rx_ep->gsi_chan_hdl,
				       rx_ep->gsi_evt_ring_hdl);
	if (res) {
			...
			unsigned long ipa_gsi_hdl)
{
	...
	memset(&tx_ep->gsi_evt_ring_props, 0,
	       sizeof(tx_ep->gsi_evt_ring_props));
	tx_ep->gsi_evt_ring_props.intf = GSI_EVT_CHTYPE_GPI_EV;
	tx_ep->gsi_evt_ring_props.intr = GSI_INTR_IRQ;
	tx_ep->gsi_evt_ring_props.re_size = GSI_EVT_RING_RE_SIZE_16B;
	...
	tx_ep->gsi_channel_props.ch_id = gsi_ep_config->ipa_gsi_chan_num;
	...
	res = ipa3_sys_update_gsi_hdls(pipe_index, tx_ep->gsi_chan_hdl,
				       tx_ep->gsi_evt_ring_hdl);
	if (res) {
		IPATEST_ERR("ipa_sys_update_gsi_hdls failed %d\n", res);
		return -EFAULT;
			   ...
			   enum ipa_mode_type mode)
{
	...
	strlcpy(rt_rule->rt_tbl_name, DEFAULT_ROUTING_TABLE_NAME,
   ...
   * any equations will cause a default rule
	...
	strlcpy(rt_lookup.name, DEFAULT_ROUTING_TABLE_NAME,
		IPA_RESOURCE_NAME_MAX);
...
int configure_system_1(void)
{
	...
	res = connect_ipa_to_apps(&from_ipa_devs[0]->ep, IPA_CLIENT_TEST_CONS,
	...
	res = connect_apps_to_ipa(&to_ipa_devs[0]->ep, IPA_CLIENT_TEST_PROD,
				  ipa_pipe_num, &to_ipa_devs[0]->mem,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
fail:
...
int configure_system_2(void)
{
	...
	res = connect_ipa_to_apps(&from_ipa_devs[0]->ep, IPA_CLIENT_TEST2_CONS,
				  ipa_pipe_num, ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	res = connect_ipa_to_apps(&from_ipa_devs[1]->ep, IPA_CLIENT_TEST3_CONS,
				  ipa_pipe_num, ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	res = connect_ipa_to_apps(&from_ipa_devs[2]->ep, IPA_CLIENT_TEST4_CONS,
				  ipa_pipe_num, ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	/* Connect APPS MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[1]->ep, IPA_CLIENT_TEST2_PROD,
				  ipa_pipe_num, &to_ipa_devs[1]->mem,
				  ipa_gsi_hdl);
...
int configure_system_5(void)
{
   ...
   !< Header length in bytes to be added/removed.
	...
	memset(&ipa_ep_cfg, 0, sizeof(ipa_ep_cfg));
	ipa_ep_cfg.hdr.hdr_len = IPA_TEST_HDI_RMNET_HEADER_LENGTH;
	/* Length of Header to add / to remove */
   ...
   * to the payload length
   ...
   1: Hdr_Ofst_Pkt_Size  value is valid, i.e.
    a packet length field resides within the header*/
	ipa_ep_cfg.hdr.hdr_ofst_pkt_size = IPA_TEST_HDI_802_LENGTH_FIELD_OFFSET;
	/* Offset within header in which packet size reside.
     Upon Header Insertion, IPA will update this field
     within the header with the packet length.
...
int configure_system_6(void)
{
	...
	if (ipa3_sys_setup(&sys_in, &ipa_gsi_hdl, &ipa_pipe_num,
		...
		goto fail;
	/* Connect APPS MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[0]->ep, IPA_CLIENT_TEST2_PROD,
				  ipa_pipe_num, &to_ipa_devs[0]->mem,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
...
int configure_system_8(void)
{
	...
	res = connect_ipa_to_apps(&from_ipa_devs[2]->ep, IPA_CLIENT_TEST2_CONS,
				  ipa_pipe_num, ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	/* Connect APPS MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[0]->ep, IPA_CLIENT_TEST3_PROD,
				  ipa_pipe_num, &to_ipa_devs[0]->mem,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	/* Connect APPS MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[1]->ep, IPA_CLIENT_TEST_PROD,
				  ipa_pipe_num, &to_ipa_devs[1]->mem,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	/* Connect APPS MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[2]->ep, IPA_CLIENT_TEST2_PROD,
				  ipa_pipe_num, &to_ipa_devs[2]->mem,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	/* Connect APPS MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[3]->ep, IPA_CLIENT_TEST4_PROD,
				  ipa_pipe_num, &to_ipa_devs[3]->mem,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
...
int configure_system_12(void)
{
	...
	res = connect_ipa_to_apps(&from_ipa_devs[2]->ep, IPA_CLIENT_TEST_CONS,
				  ipa_pipe_num, ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	res = connect_ipa_to_apps(&from_ipa_devs[3]->ep, IPA_CLIENT_TEST4_CONS,
				  ipa_pipe_num, ipa_gsi_hdl);
	if (res)
		goto fail;
...
void suspend_handler(enum ipa_irq_type interrupt, void *private_data,
		     void *interrupt_data)
{
	u32 *suspend_data;
	u32 clnt_hdl;
	...
	gsi_chan_hdl = ((struct ipa_tx_suspend_private_data *)private_data)
			       ->gsi_chan_hdl;

	IPATEST_DBG(
		"in suspend handler: interrupt=%d, clnt_hdl=%d, private_data=%d, "
		"interrupt_data=%d",
...
int configure_system_19(void)
{
	...
	IPATEST_DBG(
		"end-point ctrl register configured successfully (ipa_ep_suspend "
		"= true)\n");
 ...
 *
 */
ssize_t exception_kfifo_read(struct file *filp, char __user *buf, size_t count,
			     loff_t *p_pos)
{
	int ret = 0;
	size_t data_len = 0;
	...
	if (kfifo_is_empty(
		...
		if (kfifo_is_empty(&(p_exception_hdl_data->notify_cb_data
					     .exception_kfifo))) {
	...
	ret = kfifo_to_user(
		&(p_exception_hdl_data->notify_cb_data.exception_kfifo), buf,
		data_len, &copied);
#if (EXCEPTION_KFIFO_DEBUG_VERBOSE)
	{
		...
		IPATEST_DBG(
			"Exception packet's length=%zu, Packet's content:\n",
			data_len);
		if (data_len - 3 > 0) {
...
void notify_upon_exception(void *priv, enum ipa_dp_evt_type evt,
			   unsigned long data)
{
	int i = 0;
	size_t data_len;
	...
	IPATEST_DBG("was called, evt=%s(%d)",
		    (evt == IPA_RECEIVE) ? "IPA_RECEIVE" : "IPA_WRITE_DONE",
		    evt);
	if (IPA_RECEIVE != evt) {
	...
	IPATEST_DBG("Exception packet length = %zu,Packet content:\n",
		    data_len);
	for (i = 0; i < data_len - 4; i += 4) {
		IPATEST_DUMP("%02x %02x %02x %02x", (p_data)[i],
			     (p_data)[i + 1], (p_data)[i + 2], (p_data)[i + 3]);
	}
#endif
	res = kfifo_in(&p_notify_cb_data->exception_kfifo, p_data, data_len);
	if (res != data_len) {
...
int exception_hdl_init(void)
{
	...
	res = kfifo_alloc(&(p_notify_cb_data->exception_kfifo),
			  EXCEPTION_KFIFO_SIZE *
				  (sizeof(char) * RX_DESCRIPTOR_SIZE),
			  GFP_KERNEL);
	if (0 != res) {
		IPATEST_ERR("kfifo_alloc returned error (%d)\n", res);
		return res;
	}
	res = alloc_chrdev_region(&p_exception_hdl_data->dev_num, 0, 1,
				  EXCEPTION_DRV_NAME);
	if (0 != res) {
	...
	p_exception_hdl_data->dev = device_create(p_exception_hdl_data->class,
						  NULL,
						  p_exception_hdl_data->dev_num,
...
{
	unregister_chrdev_region(p_exception_hdl_data->dev_num, 1);
	kfifo_free(&(p_exception_hdl_data->notify_cb_data.exception_kfifo));
	/* freeing kfifo */
	memset(&(p_exception_hdl_data->notify_cb_data.exception_kfifo), 0,
	       sizeof(p_exception_hdl_data->notify_cb_data.exception_kfifo));
...
void destroy_channel_device(struct channel_dev *channel_dev)
{
	...
	if (channel_dev->ep.gsi_valid && pdev != NULL) {
		...
		dma_free_coherent(
			pdev, channel_dev->ep.gsi_channel_props.ring_len,
			channel_dev->ep.gsi_channel_props.ring_base_vaddr,
			channel_dev->ep.gsi_channel_props.ring_base_addr);
		...
		dma_free_coherent(
			pdev, channel_dev->ep.gsi_evt_ring_props.ring_len,
			channel_dev->ep.gsi_evt_ring_props.ring_base_vaddr,
			channel_dev->ep.gsi_evt_ring_props.ring_base_addr);
...
#define IPA_TO_WLAN_HEADER_LEN 34
	...
	/* Copy header name */
	memcpy(ipa_to_wlan_header.name, IPA_TO_WLAN_HEADER_NAME,
	...
	memcpy(&ipa_to_wlan_header_partial->hdr[0], &ipa_to_wlan_header,
...
int configure_system_20(void)
{
	...
	res = connect_ipa_to_apps(&from_ipa_devs[index]->ep,
				  IPA_CLIENT_TEST2_CONS, ipa_pipe_num,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	res = connect_ipa_to_apps(&from_ipa_devs[index]->ep,
				  IPA_CLIENT_TEST3_CONS, ipa_pipe_num,
				  ipa_gsi_hdl);
	if (res)
		goto fail;
	...
	/* Connect AP MEM --> Tx IPA */
	res = connect_apps_to_ipa(&to_ipa_devs[2]->ep, IPA_CLIENT_TEST3_PROD,
				  ipa_pipe_num, &to_ipa_devs[2]->mem,
				  ipa_gsi_hdl);
 ...
 */
ssize_t ipa_test_write(struct file *filp, const char __user *buf, size_t size,
		       loff_t *f_pos)
{
	int ret = 0;
	int index;
	...
	case 18:
		...
		index++;
		ret = create_channel_device_by_type(
			index, "to_ipa", &to_ipa_devs[index], RX_SZ,
			IPA_TEST_DATA_PATH_TEST_CHANNEL);
		if (ret) {
			IPATEST_ERR("Channel device creation error.\n");
			return -ENODEV;
	...
	ipa_test->current_configuration_idx = ipa_test->configuration_idx;
...
static ssize_t ipa_test_read(struct file *filp, char __user *buf, size_t count,
			     loff_t *f_pos)
{
	int res, len;
	char str[10];
	...
	len = snprintf(str, 10, "%d", ipa_test->current_configuration_idx);
...
static int
configure_app_to_ipa_path(struct ipa_channel_config __user *to_ipa_user,
			  bool isUlso)
{
	int retval;
	...
	retval = copy_from_user(&to_ipa_channel_config, to_ipa_user,
				sizeof(struct ipa_channel_config));
	if (retval) {
	...
	if (to_ipa_channel_config.config_size != sizeof(struct ipa_ep_cfg)) {
		IPATEST_ERR(
			"bad config size (%zu.vs.%zu) update test struct?\n",
			to_ipa_channel_config.config_size,
	...
	if (isUlso) {
		retval = create_channel_device_by_type(
			index, "to_ipa", &to_ipa_devs[index], TX_SZ,
			IPA_TEST_ULSO_DATA_PATH_TEST_CHANNEL);
	} else {
	...
	retval = copy_from_user(&sys_in.ipa_ep_cfg, to_ipa_channel_config.cfg,
				to_ipa_channel_config.config_size);
	if (retval) {
	...
	retval = connect_apps_to_ipa(&to_ipa_devs[index]->ep,
				     to_ipa_channel_config.client, ipa_pipe_num,
...
static int
configure_app_from_ipa_path(struct ipa_channel_config __user *from_ipa_user,
			    bool isUlso)
{
	int retval;
	...
	IPATEST_DBG("copying from 0x%px %zu bytes\n", from_ipa_user,
		    sizeof(struct ipa_channel_config));
	retval = copy_from_user(&from_ipa_channel_config, from_ipa_user,
				sizeof(struct ipa_channel_config));
	if (retval) {
		IPATEST_ERR(
			"fail to copy from user - from_ipa_user (%d.vs.%zu)\n",
	...
	/* Channel from which the userspace shall communicate to this pipe */
	retval = create_channel_device(index, "from_ipa", &from_ipa_devs[index],
				       rx_size);
	if (retval) {
		IPATEST_ERR("channel device creation error\n");
		return -1;
	...
	retval = copy_from_user(&sys_in.ipa_ep_cfg, from_ipa_channel_config.cfg,
				from_ipa_channel_config.config_size);
	if (retval) {
		IPATEST_ERR("fail to copy cfg - from_ipa_user\n");
		return -1;
	...
	if (ipa3_sys_setup(&sys_in, &ipa_gsi_hdl, &ipa_pipe_num,
			   ...
			   from_ipa_channel_config.en_status)) {
		IPATEST_ERR("setup sys pipe failed\n");
		return -1;
	}
...
static int configure_test_scenario(
	...
	struct ipa_channel_config **to_ipa_channel_config_array, bool isUlso)
{
	int retval;
	int i;
	...
	if (ipa_test->num_tx_channels > 0 || ipa_test->num_rx_channels > 0) {
		IPATEST_DBG(
			"cleanning previous configuration before new one\n");
		destroy_channel_devices();
	} else {
	...
	for (i = 0; i < ipa_test_config_header->from_ipa_channels_num; i++) {
		...
		retval = configure_app_from_ipa_path(
			from_ipa_channel_config_array[i], isUlso);
		if (retval) {
	...
	for (i = 0; i < ipa_test_config_header->to_ipa_channels_num; i++) {
		retval = configure_app_to_ipa_path(
			to_ipa_channel_config_array[i], isUlso);
		if (retval) {
...
static int handle_configuration_ioctl(unsigned long ioctl_arg, bool isUlso)
{
	int retval;
	int needed_bytes;
	...
	/* header copy */
	IPATEST_DBG("copying from 0x%px\n", (u8 *)ioctl_arg);
	retval = copy_from_user(&test_header, (u8 *)ioctl_arg,
				sizeof(test_header));
	if (retval) {
		IPATEST_ERR("failing copying header from user\n");
		return retval;
	...
	retval = configure_test_scenario(&test_header,
					 ...
					 to_ipa_channel_config_array, isUlso);
	if (retval)
...
static int handle_ep_ctrl_ioctl(unsigned long ioctl_arg)
{
	...
	retval = copy_from_user(&test_ep_ctrl, (u8 *)ioctl_arg,
				sizeof(test_ep_ctrl));
	if (retval) {
	...
	if (ipa_get_hw_type() >= IPA_HW_v4_0) {
		if (ep_ctrl.ipa_ep_suspend)
			retval = ipa_stop_gsi_channel(
				from_ipa_devs[test_ep_ctrl.from_dev_num]
					->ipa_client_hdl);
		else
			retval = gsi_start_channel(
				from_ipa_devs[test_ep_ctrl.from_dev_num]
					->ep.gsi_chan_hdl);
...
static int handle_reg_suspend_handler(unsigned long ioctl_arg)
{
	...
	res = copy_from_user(&reg_data, (u8 *)ioctl_arg, sizeof(reg_data));
	if (res) {
		IPATEST_ERR("failed copying ep_ctrl data from user\n");
		return res;
	}
	...
	if (reg_data.reg) {
		...
		suspend_priv_data->clnt_hdl =
			from_ipa_devs[reg_data.DevNum]->ipa_client_hdl;
		suspend_priv_data->gsi_chan_hdl =
			from_ipa_devs[reg_data.DevNum]->ep.gsi_chan_hdl;
		IPATEST_DBG("registering interrupt handle to clnt_hdl %d\n",
			    suspend_priv_data->clnt_hdl);
		res = ipa_add_interrupt_handler(IPA_TX_SUSPEND_IRQ,
						suspend_handler,
						reg_data.deferred_flag,
						(void *)suspend_priv_data);
		if (res) {
			IPATEST_ERR(
				"register handler for suspend interrupt failed\n");
			goto fail_allocated;
		}
...
static int handle_holb_config_ioctl(unsigned long ioctl_arg)
{
	...
	retval = copy_from_user(&test_holb_config, (u8 *)ioctl_arg,
...
static int ipa_test_get_mem_part(unsigned long ioctl_arg)
{
	...
	BUILD_BUG_ON(sizeof(struct ipa_test_mem_partition) !=
		     sizeof(struct ipa3_mem_partition));

	result = copy_to_user((u8 *)ioctl_arg, ipa3_ctx->ctrl->mem_partition,
			      sizeof(struct ipa3_mem_partition));
...
static long ipa_test_ioctl(struct file *filp, unsigned int cmd,
			   unsigned long arg)
{
	int retval;
...
static int __init ipa_test_init(void)
{
	...
	ipa_test->dev = device_create(ipa_test_class, NULL, ipa_test->dev_num,
