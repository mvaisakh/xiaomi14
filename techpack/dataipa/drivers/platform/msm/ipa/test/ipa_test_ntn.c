...
static inline void
ipa_test_ntn_set_client_params(enum ipa_client_type cons_type,
...
static void ipa_test_ntn_free_dma_buff(struct ipa_mem_buffer *mem)
{
	...
	dma_free_coherent(cb->dev, mem->size, mem->base, mem->phys_base);
}
...
static int ipa_test_ntn_alloc_mmio(void)
{
	...
	test_ntn_ctx->tx_transfer_ring_addr.base = dma_alloc_coherent(
		cb->dev, size, &test_ntn_ctx->tx_transfer_ring_addr.phys_base,
		GFP_KERNEL);
	...
	test_ntn_ctx->tx_transfer_ring_sgt =
		kzalloc(sizeof(test_ntn_ctx->tx_transfer_ring_sgt), GFP_KERNEL);
	...
	ret = dma_get_sgtable(cb->dev, test_ntn_ctx->tx_transfer_ring_sgt,
	...
	test_ntn_ctx->rx_transfer_ring_addr.base = dma_alloc_coherent(
		cb->dev, size, &test_ntn_ctx->rx_transfer_ring_addr.phys_base,
		GFP_KERNEL);
	...
	test_ntn_ctx->rx_transfer_ring_sgt =
		kzalloc(sizeof(test_ntn_ctx->rx_transfer_ring_sgt), GFP_KERNEL);
	...
	ret = dma_get_sgtable(cb->dev, test_ntn_ctx->rx_transfer_ring_sgt,
	...
	test_ntn_ctx->tx_buf.base = dma_alloc_coherent(
		cb->dev, size, &test_ntn_ctx->tx_buf.phys_base, GFP_KERNEL);
	...
	test_ntn_ctx->tx_buff_sgt =
		kzalloc(sizeof(test_ntn_ctx->tx_buff_sgt), GFP_KERNEL);
	...
	ret = dma_get_sgtable(cb->dev, test_ntn_ctx->tx_buff_sgt,
			      test_ntn_ctx->tx_buf.base,
			      test_ntn_ctx->tx_buf.phys_base, size);
	if (ret) {
		IPA_UT_ERR("failed to get sgtable\n");
		ret = -ENOMEM;
	...
	test_ntn_ctx->rx_buf.base = dma_alloc_coherent(
		cb->dev, size, &test_ntn_ctx->rx_buf.phys_base, GFP_KERNEL);
	...
	test_ntn_ctx->rx_buff_sgt =
		kzalloc(sizeof(test_ntn_ctx->rx_buff_sgt), GFP_KERNEL);
	...
	ret = dma_get_sgtable(cb->dev, test_ntn_ctx->rx_buff_sgt,
			      test_ntn_ctx->rx_buf.base,
			      test_ntn_ctx->rx_buf.phys_base, size);
	if (ret) {
		IPA_UT_ERR("failed to get sgtable\n");
		ret = -ENOMEM;
   ...
   */
	...
	test_ntn_ctx->bar_addr.base = dma_alloc_coherent(
		ipa3_ctx->pdev, test_ntn_ctx->bar_addr.size,
		&test_ntn_ctx->bar_addr.phys_base, GFP_KERNEL);
...
static struct ipa_eth_ready eth_ready = { .notify = ipa_test_ntn_ready_cb,
					  .userdata = NULL };
...
static int ipa_test_ntn_suite_setup(void **priv)
{
	...
	test_ntn_ctx = kzalloc(sizeof(struct ipa_test_ntn_context), GFP_KERNEL);
	if (!test_ntn_ctx) {
		IPA_UT_ERR("failed to allocate ctx\n");
		return -ENOMEM;
	}

	ipa_test_ntn_set_client_params(IPA_CLIENT_ETHERNET_CONS,
				       IPA_CLIENT_ETHERNET_PROD, 0);
...
static void ipa_ntn_test_print_stats()
{
	...
	IPA_UT_INFO("NTN_tx_ringFull=%u\n"
		    ...
		    stats.u.ring[1].ringUsageHigh, stats.u.ring[1].ringUsageLow,
		    stats.u.ring[1].RingUtilCount);

	IPA_UT_INFO("NTN_rx_ringFull=%u\n"
		    "NTN_rx_ringEmpty=%u\n"
		    "NTN_rx_ringUsageHigh=%u\n"
		    "NTN_rx_ringUsageLow=%u\n"
		    "NTN_rx_RingUtilCount=%u\n",
		    stats.u.ring[0].ringFull, stats.u.ring[0].ringEmpty,
		    stats.u.ring[0].ringUsageHigh, stats.u.ring[0].ringUsageLow,
	...
	IPA_UT_INFO("NTN_TX_RP=0x%x\n"
		    "NTN_TX_WP=0x%x\n"
		    "NTN_TX_err=%u\n",
		    tx_stats.rp, tx_stats.wp, tx_stats.err);

	IPA_UT_INFO("NTN_RX_RP=0x%x\n"
		    "NTN_RX_WP=0x%x\n"
		    "NTN_RX_err:%u\n",
		    rx_stats.rp, rx_stats.wp, rx_stats.err);
}
...
static int ipa_ntn_test_ready_cb(void *priv)
{
	...
	ret = wait_for_completion_timeout(&test_ntn_ctx->init_completion_obj,
...
static int ipa_ntn_test_setup_pipes(void)
{
	...
	IPA_UT_DBG(
		"rx buff phys 0x%X, cpu 0x%X, size %d, fix size %d sgt 0x%X\n",
		test_ntn_ctx->rx_buf.phys_base, test_ntn_ctx->rx_buf.base,
   ...
   */
	...
	for (i = 0; i < NUM_TX_BUFS; i++) {
		...
		IPA_UT_DBG(
			"tx_pipe_info.info.data_buff_list[%d].iova = 0x%lx", i,
			test_ntn_ctx->tx_pipe_info.info.data_buff_list[i].iova);
		IPA_UT_DBG(
			"tx_pipe_info.info.data_buff_list[%d].pa = 0x%lx", i,
			test_ntn_ctx->tx_pipe_info.info.data_buff_list[i].pa);
	}
	test_ntn_ctx->tx_pipe_info.info.data_buff_list_size = NUM_TX_BUFS;

	IPA_UT_DBG(
		"tx buff phys 0x%X, cpu 0x%X, size %d, fix size %d sgt 0x%X\n",
		test_ntn_ctx->tx_buf.phys_base, test_ntn_ctx->tx_buf.base,
...
static int ipa_ntn_test_reg_intf(void)
{
	...
	intf.pipe_hdl_list = kcalloc(IPA_TEST_NTN_NUM_PIPES,
				     sizeof(*intf.pipe_hdl_list), GFP_KERNEL);
...
static void ipa_ntn_test_advance_db(u32 *db, int steps, int num_words,
				    int ring_size)
{
...
static int ipa_ntn_send_one_packet(void)
{
	...
	rx_db = ioremap(test_ntn_ctx->rx_pipe_info.info.db_pa,
			DB_REGISTER_SIZE);
	if (!rx_db) {
	...
	tx_db = ioremap(test_ntn_ctx->tx_pipe_info.info.db_pa,
			DB_REGISTER_SIZE);
	if (!tx_db) {
		IPA_UT_ERR("ioremap failed");
		return ret;
...
static int ipa_ntn_test_prepare_test(void)
{
	...
	IPA_UT_DBG(
		"writing to RX tail ptr in 0x%X le value of 0x%X",
		(u32 *)((char *)test_ntn_ctx->bar_addr.base + RX_TAIL_PTR_OFF),
	...
	*((u32 *)((char *)test_ntn_ctx->bar_addr.base + RX_TAIL_PTR_OFF)) =
	...
	IPA_UT_DBG(
		"writing to TX tail ptr in 0x%X le value of 0x%X",
		(u32 *)((char *)test_ntn_ctx->bar_addr.base + TX_TAIL_PTR_OFF),
   ...
   */
	...
	for (i = 0; i < NUM_RX_TR_ELE - 1; i++, initial_val++) {
		...
		*packet_recv = 0;
		...
		/* recycle TRE */
		/* TX */
		memset((struct tx_transfer_ring_ele *)test_ntn_ctx
...
static int ipa_ntn_test_clients2_multi_transfer_burst(void *priv)
{
	...
	ipa_test_ntn_set_client_params(IPA_CLIENT_ETHERNET2_CONS,
				       IPA_CLIENT_ETHERNET2_PROD, 1);
...
IPA_UT_DEFINE_SUITE_START(ntn, "NTN3 tests", ipa_test_ntn_suite_setup,
			  ipa_test_ntn_suite_teardown){
	IPA_UT_ADD_TEST(ready_cb, "ready callback test", ipa_ntn_test_ready_cb,
			true, IPA_HW_v5_0, IPA_HW_MAX),

	IPA_UT_ADD_TEST(single_transfer, "single data transfer",
			ipa_ntn_test_single_transfer, true, IPA_HW_v5_0,
			IPA_HW_MAX),

	IPA_UT_ADD_TEST(multi_transfer,
			"multi data transfer without wrap around",
			ipa_ntn_test_multi_transfer, true, IPA_HW_v5_0,
			IPA_HW_MAX),

	IPA_UT_ADD_TEST(multi_transfer_w_wrap,
			"multi data transfer with wrap around",
			ipa_ntn_test_multi_transfer_wrap_around, true,
			IPA_HW_v5_0, IPA_HW_MAX),

	IPA_UT_ADD_TEST(multi_transfer_burst, "send entire ring in one shot",
			ipa_ntn_test_multi_transfer_burst, true, IPA_HW_v5_0,
			IPA_HW_MAX),

	IPA_UT_ADD_TEST(clients2_multi_transfer_burst,
			"Clients pair 2 send entire ring in one shot",
			ipa_ntn_test_clients2_multi_transfer_burst, true,
			IPA_HW_v5_0, IPA_HW_MAX),
