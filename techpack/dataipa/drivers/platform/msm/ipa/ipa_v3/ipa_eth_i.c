...
static void ipa3_eth_save_client_mapping(struct ipa_eth_client_pipe_info *pipe,
...
static void
ipa3_eth_release_client_mapping(struct ipa_eth_client_pipe_info *pipe, int id)
{
	struct ipa_eth_client *client_info;
	enum ipa_eth_client_type client_type;
	u8 inst_id, pipe_hdl;
...
static int ipa3_eth_uc_init_peripheral(bool init, u8 protocol, u64 per_base)
{
	...
	if (init) {
		...
		cmd_data->protocol = protocol;
		if (protocol == IPA_HW_PROTOCOL_AQC) {
			cmd_data->PeripheralDeinit_params.AqcDeinit_params
				.reserved = 0;
		}
...
static int ipa3_eth_config_uc(bool init, u8 protocol, u8 dir, u8 gsi_ch,
			      u8 peripheral_ch)
{
	struct ipa_mem_buffer cmd;
	enum ipa_cpu_2_hw_offload_commands command;
	...
	if (init) {
		...
		case IPA_HW_PROTOCOL_AQC:
			...
			cmd_data->SetupCh_params.aqc_params.aqc_ch =
				peripheral_ch;
			break;
...
static int ipa_eth_setup_rtk_gsi_channel(struct ipa_eth_client_pipe_info *pipe,
					 struct ipa3_ep_context *ep)
{
	...
	/* setup event ring */
	bar_addr = IPA_ETH_PCIE_SET(pipe->info.client_info.rtk.bar_addr);
	...
	gsi_evt_ring_props.msi_addr =
		bar_addr + pipe->info.client_info.rtk.dest_tail_ptr_offs;
	len = pipe->info.transfer_ring_size;
	gsi_evt_ring_props.ring_len = len;
	gsi_evt_ring_props.ring_base_addr = (u64)pipe->info.transfer_ring_base;
	...
	gsi_channel_props.ring_base_addr = (u64)pipe->info.transfer_ring_base;
	...
	ch_scratch.rtk.rtk_bar_low = (u32)bar_addr;
	ch_scratch.rtk.rtk_bar_high = (u32)((u64)(bar_addr) >> 32);
	/*
   ...
   */
	ch_scratch.rtk.queue_number =
		...
		(queue_number == 0) ? 16 :
				      18;
	ch_scratch.rtk.fix_buff_size = ilog2(pipe->info.fix_buffer_size);
...
ipa_eth_get_smmu_domain(enum ipa_client_type client_type)
{
...
static bool ipa_eth_is_smmu_buff_cb_bypass(enum ipa_client_type client_type)
{
	if (ipa3_ctx->ipa_hw_type < IPA_HW_v5_0)
...
static enum ipa_smmu_cb_type
ipa_eth_get_cb_type(enum ipa_client_type client_type)
{
	if (IPA_CLIENT_IS_SMMU_ETH_INSTANCE(client_type))
				   ...
				   enum ipa_client_type client_type, bool map)
{
	...
	result = ipa3_smmu_map_peer_buff((u64)pipe->info.transfer_ring_base,
					 pipe->info.transfer_ring_size, map,
...
map_buffer:
	...
	if ((ipa3_get_smmu_ctx(cb_type))->shared) {
		...
		IPADBG("SMMU cb %d is not shared, continue to map buffers\n",
		       cb_type);
	}
	...
	if (pipe->info.is_buffer_pool_valid) {
		...
		result = ipa3_smmu_map_peer_buff(
			...
			pipe->info.fix_buffer_size, map,
			pipe->info.buffer_pool_base_sgt, cb_type);
		if (result) {
		...
		for (i = 0; i < pipe->info.data_buff_list_size; i++) {
			...
			IPA_SMMU_ROUND_TO_PAGE(iova, pa,
					       pipe->info.fix_buffer_size,
					       iova_p, pa_p, size_p);
			/* Add check on every 2nd buffer for AQC smmu-dup issue */
			if (prev_iova_p == iova_p) {
				IPADBG_LOW(
					"current buffer and previous are on the same page, skip "
					"page mapping\n");
				continue;
			}
			prev_iova_p = iova_p;
			IPADBG_LOW("%s 0x%llx to 0x%pa size %d\n",
				   map ? "mapping" : "unmapping", iova_p, &pa_p,
				   size_p);
			if (map) {
				result = ipa3_iommu_map(
					smmu_domain, iova_p, pa_p, size_p,
					IOMMU_READ | IOMMU_WRITE);
				if (result)
					IPAERR("Fail to map 0x%llx\n", iova);
			} else {
				result = iommu_unmap(smmu_domain, iova_p,
						     size_p);
				if (result != size_p) {
...
fail_map_buffer_smmu_enabled:
	ipa3_smmu_map_peer_buff((u64)pipe->info.transfer_ring_base,
				pipe->info.transfer_ring_size, !map,
				pipe->info.transfer_ring_sgt, IPA_SMMU_CB_AP);
	return result;
}

static int ipa_eth_setup_aqc_gsi_channel(struct ipa_eth_client_pipe_info *pipe,
					 struct ipa3_ep_context *ep)
{
	struct gsi_evt_ring_props gsi_evt_ring_props;
	struct gsi_chan_props gsi_channel_props;
	...
	/* setup event ring */
	bar_addr = IPA_ETH_PCIE_SET(pipe->info.client_info.aqc.bar_addr);
	memset(&gsi_evt_ring_props, 0, sizeof(gsi_evt_ring_props));
	...
	gsi_evt_ring_props.user_data = NULL;
	gsi_evt_ring_props.msi_addr =
		bar_addr + pipe->info.client_info.aqc.dest_tail_ptr_offs;
	gsi_evt_ring_props.ring_len = len;
	gsi_evt_ring_props.ring_base_addr = (u64)pipe->info.transfer_ring_base;
	result = gsi_alloc_evt_ring(&gsi_evt_ring_props, ipa3_ctx->gsi_dev_hdl,
				    &ep->gsi_evt_ring_hdl);
	...
	if (pipe->dir == IPA_ETH_PIPE_DIR_TX)
		...
		evt_scratch.aqc.head_ptr_wrb_mod_threshold =
			len * AQC_WRB_MODC_FACTOR /
			(100 * GSI_EVT_RING_RE_SIZE_16B);

	/* write ch scratch */
	memset(&ch_scratch, 0, sizeof(ch_scratch));
	ch_scratch.aqc.fix_buff_size = ilog2(pipe->info.fix_buffer_size);
	...
	if (pipe->dir == IPA_ETH_PIPE_DIR_RX) {
		...
		ch_scratch.aqc.head_ptr_msb =
			(u32)((bar_addr + head_ptr) >> 32);
	}
...
static int ipa_eth_setup_ntn_gsi_channel(struct ipa_eth_client_pipe_info *pipe,
					 struct ipa3_ep_context *ep)
{
	struct gsi_evt_ring_props gsi_evt_ring_props;
	struct gsi_chan_props gsi_channel_props;
   ...
   * on PICE address space */
	...
	gsi_evt_ring_props.user_data = NULL;
	gsi_evt_ring_props.msi_addr =
		bar_addr + pipe->info.client_info.ntn.tail_ptr_offs;
	gsi_evt_ring_props.ring_len = len;
	gsi_evt_ring_props.ring_base_addr = (u64)pipe->info.transfer_ring_base;
	result = gsi_alloc_evt_ring(&gsi_evt_ring_props, ipa3_ctx->gsi_dev_hdl,
				    &ep->gsi_evt_ring_hdl);
	...
	/* write ch scratch */
	memset(&ch_scratch, 0, sizeof(ch_scratch));
	ch_scratch.ntn.fix_buff_size = ilog2(pipe->info.fix_buffer_size);
	if (pipe->info.is_buffer_pool_valid) {
...
int ipa3_eth_connect(struct ipa_eth_client_pipe_info *pipe,
		     enum ipa_client_type client_type)
{
	...
	} else if (strnstr(net_dev->name, STR_ETH1_IFACE,
			   strlen(net_dev->name))) {
	...
	ep->cfg.nat.nat_en = IPA_CLIENT_IS_PROD(client_type) ? IPA_SRC_NAT :
							       IPA_BYPASS_NAT;
	...
	if (gsi_query_channel_db_addr(ep->gsi_chan_hdl, &gsi_db_addr_low,
				      &gsi_db_addr_high)) {
	...
	gsi_query_evt_ring_db_addr(ep->gsi_evt_ring_hdl, &evt_ring_db_addr_low,
   ...
   */
	...
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v4_5) {
		ipa3_ctx->gsi_info[prot].ch_id_info[id].ch_id =
			ep->gsi_chan_hdl;
		ipa3_ctx->gsi_info[prot].ch_id_info[id].dir = pipe->dir;
		ipa3_uc_debug_stats_alloc(ipa3_ctx->gsi_info[prot]);
	}
	...
	if ((ipa3_ctx->ipa_hw_type == IPA_HW_v4_5) &&
		...
		if (!eth_info->num_ch) {
			bar_addr = IPA_ETH_PCIE_SET(
				pipe->info.client_info.aqc.bar_addr);
			result = ipa3_eth_uc_init_peripheral(
				true, IPA_HW_PROTOCOL_AQC, bar_addr);
			if (result) {
	...
	ipa3_eth_save_client_mapping(pipe, client_type, id, ep_idx,
				     ep->gsi_chan_hdl);
	if ((ipa3_ctx->ipa_hw_type == IPA_HW_v4_5) ||
	    (prot == IPA_HW_PROTOCOL_RTK)) {
...
config_uc_fail:
	...
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v4_5) {
		ipa3_ctx->gsi_info[prot].ch_id_info[id].ch_id = 0xff;
		ipa3_ctx->gsi_info[prot].ch_id_info[id].dir = pipe->dir;
		ipa3_uc_debug_stats_alloc(ipa3_ctx->gsi_info[prot]);
	}
...
int ipa3_eth_disconnect(struct ipa_eth_client_pipe_info *pipe,
			enum ipa_client_type client_type)
{
	int result = 0;
	struct ipa3_ep_context *ep;
