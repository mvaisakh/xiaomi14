...
#define IPA_RMNET_LL_PIPE_READY_ALL   \
	(IPA_RMNET_LL_PIPE_TX_READY | \
...
static DECLARE_DELAYED_WORK(rmnet_ll_wakeup_work, rmnet_ll_wakeup_ipa);
static void apps_rmnet_ll_tx_complete_notify(void *priv,
					     enum ipa_dp_evt_type evt,
					     unsigned long data);
static void apps_rmnet_ll_receive_notify(void *priv, enum ipa_dp_evt_type evt,
					 unsigned long data);
					...
					size_t count, loff_t *ppos)
{
	...
	nbytes = scnprintf(
		...
		atomic_read(&rmnet_ll_ipa3_ctx->stats.outstanding_pkts),
...
static ssize_t rmnet_ll_ipa3_read_free_credit_threshld(struct file *file,
						       char __user *buf,
						       size_t count,
						       loff_t *ppos)
{
...
static ssize_t rmnet_ll_ipa3_write_free_credit_threshld(struct file *file,
...
static const struct rmnet_ll_ipa3_debugfs_file debugfs_files[] = {
	{ "stats", READ_ONLY_MODE, NULL, { .read = rmnet_ll_ipa3_read_stats } },
	{ "free_credit_threshld",
	  READ_WRITE_MODE,
	  NULL,
	  {
...
static void rmnet_ll_ipa3_debugfs_init(void)
{
	...
	const size_t debugfs_files_num =
		...
		sizeof(struct rmnet_ll_ipa3_debugfs_file);
	size_t i;
	...
	for (i = 0; i < debugfs_files_num; ++i) {
		const struct rmnet_ll_ipa3_debugfs_file *curr =
			&debugfs_files[i];
...
int ipa3_rmnet_ll_init(void)
{
	...
	rmnet_ll_ipa3_ctx = kzalloc(sizeof(*rmnet_ll_ipa3_ctx), GFP_KERNEL);

	if (!rmnet_ll_ipa3_ctx)
		return -ENOMEM;

	snprintf(buff, IPA_RESOURCE_NAME_MAX, "rmnet_llwq");
	rmnet_ll_ipa3_ctx->wq = alloc_workqueue(
	...
	rmnet_ll_ipa3_ctx->free_credit_thrshld =
		IPA_RMNET_LL_FREE_CREDIT_THRSHLD;
	rmnet_ll_ipa3_debugfs_init();
	return 0;
}

int ipa_register_rmnet_ll_cb(
	void (*ipa_rmnet_ll_ready_cb)(void *user_data1), void *user_data1,
	void (*ipa_rmnet_ll_stop_cb)(void *user_data2), void *user_data2,
...
int ipa3_setup_apps_low_lat_data_cons_pipe(
	struct rmnet_ingress_param *ingress_param, struct net_device *dev)
{
   ...
   */
	...
	if (ingress_param) {
		...
		ipa_low_lat_data_ep_cfg->ipa_ep_cfg.cfg.cs_offload_en =
			IPA_ENABLE_CS_DL_QMAP;
		ipa_low_lat_data_ep_cfg->ipa_ep_cfg.aggr.aggr_byte_limit = 0;
		ipa_low_lat_data_ep_cfg->ipa_ep_cfg.aggr.aggr_pkt_limit = 0;
	}

	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr.hdr_len = 8;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr.hdr_ofst_metadata_valid = 1;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr.hdr_ofst_metadata = 1;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr.hdr_ofst_pkt_size_valid = 1;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr.hdr_ofst_pkt_size = 2;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr_ext.hdr_total_len_or_pad_valid =
		true;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr_ext.hdr_total_len_or_pad = 0;
	...
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr_ext.hdr_total_len_or_pad_offset =
		0;
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr_ext.hdr_little_endian = 0;
...
int ipa3_setup_apps_low_lat_data_prod_pipe(
	struct rmnet_egress_param *egress_param, struct net_device *dev)
{
	struct ipa_sys_connect_params *ipa_low_lat_data_ep_cfg;
	int ret = 0;
	int ep_idx;
	...
	if (egress_param) {
		...
		if (egress_param->cs_offload_en) {
			...
			ipa_low_lat_data_ep_cfg->ipa_ep_cfg.cfg
				.cs_metadata_hdr_offset = 1;
			ipa_low_lat_data_ep_cfg->ipa_ep_cfg.hdr
		...
		if (egress_param->aggr_en) {
			...
			ipa_low_lat_data_ep_cfg->ipa_ep_cfg.aggr.aggr_en =
				IPA_ENABLE_DEAGGR;
			ipa_low_lat_data_ep_cfg->ipa_ep_cfg.aggr.aggr =
				IPA_QCMAP;
			ipa_low_lat_data_ep_cfg->ipa_ep_cfg.deaggr
	...
	ipa_low_lat_data_ep_cfg->ipa_ep_cfg.metadata_mask.metadata_mask = 0;
	/* modem want offset at 0! */
	...
	ipa_low_lat_data_ep_cfg->client = IPA_CLIENT_APPS_WAN_LOW_LAT_DATA_PROD;
	ipa_low_lat_data_ep_cfg->notify = apps_rmnet_ll_tx_complete_notify;
	ipa_low_lat_data_ep_cfg->priv = dev;
	ipa_low_lat_data_ep_cfg->desc_fifo_sz = IPA_SYS_TX_DATA_DESC_FIFO_SZ_8K;
...
int ipa_rmnet_ll_xmit(struct sk_buff *skb)
{
	...
	if (rmnet_ll_ipa3_ctx->state != IPA_RMNET_LL_START) {
		...
		rmnet_ll_ipa3_ctx->stats.tx_pkt_dropped++;
		rmnet_ll_ipa3_ctx->stats.tx_byte_dropped += skb->len;
	...
	if (!atomic_read(&rmnet_ll_ipa3_ctx->under_flow_controlled_state) &&
	    (atomic_read(&rmnet_ll_ipa3_ctx->stats.outstanding_pkts) >= 0) &&
	    ((atomic_read(&rmnet_ll_ipa3_ctx->stats.outstanding_pkts) +
	      skb_queue_len(&rmnet_ll_ipa3_ctx->tx_queue)) >=
	     RMNET_LL_QUEUE_MAX)) {
     ...
     */
		queue_delayed_work(rmnet_ll_ipa3_ctx->wq, &rmnet_ll_wakeup_work,
				   0);
		free_desc =
			(RMNET_LL_QUEUE_MAX -
			 (atomic_read(
...
out:
	if (atomic_read(&rmnet_ll_ipa3_ctx->stats.outstanding_pkts) == 0)
...
static void rmnet_ll_wakeup_ipa(struct work_struct *work)
{
     ...
     * IPA_CLIENT_Q6_WAN_CONS based on DMA settings
     */
		ret = ipa_tx_dp(IPA_CLIENT_APPS_WAN_LOW_LAT_DATA_PROD, skb,
				NULL);
		if (ret) {
			if (ret == -EPIPE) {
				/* try to drain skb from queue if pipe teardown */
				IPAERR_RL(
					"Low lat data fatal: pipe is not valid\n");
	...
	if (atomic_read(&rmnet_ll_ipa3_ctx->under_flow_controlled_state)) {
		...
		if ((RMNET_LL_QUEUE_MAX >= pending_credits) &&
			...
			IPADBG("IPA LL flow control lifted, %d + %d, %d\n",
			       atomic_read(&rmnet_ll_ipa3_ctx->stats
						    .outstanding_pkts),
			       skb_queue_len(&rmnet_ll_ipa3_ctx->tx_queue),
			       atomic_read(
				       &rmnet_ll_ipa3_ctx
						->under_flow_controlled_state));
			spin_unlock_irqrestore(&rmnet_ll_ipa3_ctx->tx_lock,
					       flags);
					 ...
					 unsigned long data)
{
	...
	if (evt == IPA_RECEIVE) {
		...
		if (rmnet_ll_ipa3_ctx->cb_info.rx_notify_cb) {
			...
			goto fail;
		rmnet_ll_ipa3_ctx->stats.rx_pkt_rcvd++;
		rmnet_ll_ipa3_ctx->stats.rx_byte_rcvd += len;
	} else {
