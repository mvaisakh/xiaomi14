 ...
 * Thus, software allocates up to 512 Kbytes (32 bytes *16 K QEs) contiguously
 * in memory.
 */
...
static void __ipa_tsp_get_supported_constrains(void)
{
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v5_5) {
		...
		IPAERR("TSP not supported. ingr_tc_max=%d egr_ep_max=%d egr_tc_max=%d\n",
		       ipa3_ctx->tsp.ingr_tc_max, ipa3_ctx->tsp.egr_ep_max,
		       ipa3_ctx->tsp.egr_tc_max);
	}
}
...
int ipa_tsp_init(void)
{
   ...
   */
	...
	if (!ipa3_ctx->tsp.ingr_tc_max || !ipa3_ctx->tsp.egr_ep_max ||
	    !ipa3_ctx->tsp.egr_tc_max) {
		IPAERR("TSP not supported. ingr_tc_max=%d egr_ep_max=%d egr_tc_max=%d\n",
		       ipa3_ctx->tsp.ingr_tc_max, ipa3_ctx->tsp.egr_ep_max,
		       ipa3_ctx->tsp.egr_tc_max);
   ...
   */
	ipa3_ctx->tsp.ingr_tc_tbl.size =
		ipa3_ctx->tsp.ingr_tc_max * IPA_TSP_INGR_TC_SIZE;
	ipa3_ctx->tsp.egr_ep_tbl.size =
		ipa3_ctx->tsp.egr_ep_max * IPA_TSP_EGR_EP_SIZE;
	ipa3_ctx->tsp.egr_tc_tbl.size =
		ipa3_ctx->tsp.egr_tc_max * IPA_TSP_EGR_TC_SIZE;

	if (!ipa3_ctx->tsp.ingr_tc_tbl.size || !ipa3_ctx->tsp.egr_ep_tbl.size ||
	    !ipa3_ctx->tsp.egr_tc_tbl.size) {
	...
	ipa3_ctx->tsp.ingr_tc_tbl.base = dma_alloc_coherent(
		ipa3_ctx->pdev, ipa3_ctx->tsp.ingr_tc_tbl.size,
		&(ipa3_ctx->tsp.ingr_tc_tbl.phys_base), GFP_KERNEL);
	if (!ipa3_ctx->tsp.ingr_tc_tbl.base) {
		IPAERR("Failed to allocate cache memory for ingress TC TSP table.\n");
		return -ENOMEM;
	}

	ipa3_ctx->tsp.egr_ep_tbl.base = dma_alloc_coherent(
		ipa3_ctx->pdev, ipa3_ctx->tsp.egr_ep_tbl.size,
		&(ipa3_ctx->tsp.egr_ep_tbl.phys_base), GFP_KERNEL);
	...
	ipa3_ctx->tsp.egr_tc_tbl.base = dma_alloc_coherent(
		ipa3_ctx->pdev, ipa3_ctx->tsp.egr_tc_tbl.size,
		&(ipa3_ctx->tsp.egr_tc_tbl.phys_base), GFP_KERNEL);
   ...
   */
	ipa3_ctx->tsp.egr_ep_config = (enum ipa_client_type *)kzalloc(
		ipa3_ctx->tsp.egr_ep_max * sizeof(enum ipa_client_type),
		GFP_KERNEL);
   ...
   * Every Producer can hold up to 16 K Queue elements and 8 Queues.
   * Thus, software allocates up to 512 Kbytes (32 bytes *16 K QEs) contiguously
   * in memory. Note: All queues of all PRODs have to be together in contiguous
   * memory! Must be 128B aligned.
   */
	qm_tlv_size = IPA_TSP_QM_DRAM_BYTESIZE;
	ipa3_ctx->tsp.qm_tlv_mem.size =
		qm_tlv_size + 128; // + 128 bytes to fit alignment
alloc:
	ipa3_ctx->tsp.qm_tlv_mem.base = dma_alloc_coherent(
		ipa3_ctx->pdev, ipa3_ctx->tsp.qm_tlv_mem.size,
		&(ipa3_ctx->tsp.qm_tlv_mem.phys_base), flag);
	...
	ipahal_write_reg(IPA_TSP_QM_EXTERNAL_BADDR_LSB,
			 qm_tlv_base & 0xFFFFFFFF);
	ipahal_write_reg(IPA_TSP_QM_EXTERNAL_BADDR_MSB, qm_tlv_base >> 32);
	ipahal_write_reg(IPA_TSP_QM_EXTERNAL_SIZE,
			 qm_tlv_size >> 12); // Size in 4kB resolution
...
free_egr:
	dma_free_coherent(ipa3_ctx->pdev, ipa3_ctx->tsp.egr_tc_tbl.size,
			  ipa3_ctx->tsp.egr_tc_tbl.base,
			  ipa3_ctx->tsp.egr_tc_tbl.phys_base);
	ipa3_ctx->tsp.egr_tc_tbl.base = NULL;
free_ep:
	dma_free_coherent(ipa3_ctx->pdev, ipa3_ctx->tsp.egr_ep_tbl.size,
			  ipa3_ctx->tsp.egr_ep_tbl.base,
			  ipa3_ctx->tsp.egr_ep_tbl.phys_base);
	ipa3_ctx->tsp.egr_ep_tbl.base = NULL;
free_ingr:
	dma_free_coherent(ipa3_ctx->pdev, ipa3_ctx->tsp.ingr_tc_tbl.size,
			  ipa3_ctx->tsp.ingr_tc_tbl.base,
			  ipa3_ctx->tsp.ingr_tc_tbl.phys_base);
	ipa3_ctx->tsp.ingr_tc_tbl.base = NULL;
done:
	return ret;
}

int ipa_tsp_get_ingr_tc(u8 index,
			struct ipa_ioc_tsp_ingress_class_params *output)
{
	/* The function is internal only, assuming valid params */

	ipahal_tsp_parse_hw_ingr_tc(ipa3_ctx->tsp.ingr_tc_tbl.base, index,
				    output);
	output->include_l2_len = !!(
		ipahal_read_reg(IPA_TSP_INGRESS_POLICING_CFG) & (0x1 << index));
...
int ipa_tsp_get_egr_ep(u8 index, struct ipa_ioc_tsp_egress_prod_params *output)
{
	...
	ipahal_tsp_parse_hw_egr_ep(ipa3_ctx->tsp.egr_ep_tbl.base, index,
				   output);

	output->client = ipa3_ctx->tsp.egr_ep_config[index];

	regval = ipahal_read_reg_n_fields(IPA_ENDP_INIT_PROD_CFG_n,
					  ipa_get_ep_mapping(output->client),
					  (void *)&prod_cfg);

	output->max_out_bytes = prod_cfg.max_output_size
				<< 6; // max_output_size*64
...
int ipa_tsp_get_egr_tc(u8 index, struct ipa_ioc_tsp_egress_class_params *output)
{
	...
	ipahal_tsp_parse_hw_egr_tc(ipa3_ctx->tsp.egr_tc_tbl.base, index,
				   output);

	return 0;
}

int ipa_tsp_set_ingr_tc(u8 index,
			const struct ipa_ioc_tsp_ingress_class_params *input)
{
	/* The function is internal only, assuming valid params */

	ipahal_tsp_fill_hw_ingr_tc(input, ipa3_ctx->tsp.ingr_tc_tbl.base,
				   index);
...
int ipa_tsp_set_egr_ep(u8 index,
		       const struct ipa_ioc_tsp_egress_prod_params *input)
{
	...
	regval = ipahal_read_reg_n_fields(IPA_ENDP_INIT_PROD_CFG_n, ep_index,
					  (void *)&prod_cfg);

	if (ipa3_ctx->tsp.egr_ep_config[index] != IPA_CLIENT_MAX &&
	    ipa3_ctx->tsp.egr_ep_config[index] != input->client) {
		cleanup = true;
		new_tc_range_mask &= !GENMASK(prod_cfg.egress_tc_highest,
					      prod_cfg.egress_tc_lowest);
	}
	...
	prod_cfg.max_output_size = input->max_out_bytes >>
				   6; // max_out_bytes/64
	...
	if (ipa3_cfg_ep_prod_cfg(ipa_get_ep_mapping(input->client),
				 &prod_cfg) != 0) {
		IPAERR("Failed configuring the producer EP.\n");
		return -EFAULT;
	}
...
int ipa_tsp_set_egr_tc(u8 index,
		       const struct ipa_ioc_tsp_egress_class_params *input)
{
	/* The function is internal only, assuming valid params */
   ...
   */
	if (input->guaranteed_rate || input->guaranteed_burst)
		ipahal_write_reg_mask(IPA_TSP_EGRESS_POLICING_CFG, 0x1 << index,
				      0x1 << index);
	else
		ipahal_write_reg_mask(IPA_TSP_EGRESS_POLICING_CFG, 0x0,
				      0x1 << index);
...
int ipa_tsp_commit(void)
{
	...
	/*
   * The tables are located in the HW SRAM area, and we can't issue the DMA IMM,
   * because the offset field is restricted to 16 addresses in it. Therefore, we
   * do memcopy instead, but the infrastructure will be ready for DMA in future
   * IPA versions.
   */
	memcpy_toio(ingr_tc_mmio, ipa3_ctx->tsp.ingr_tc_tbl.base,
		    ipa3_ctx->tsp.ingr_tc_tbl.size);
	memcpy_toio(egr_tc_mmio, ipa3_ctx->tsp.egr_tc_tbl.base,
		    ipa3_ctx->tsp.egr_tc_tbl.size);
	memcpy_toio(prod_mmio, ipa3_ctx->tsp.egr_ep_tbl.base,
		    ipa3_ctx->tsp.egr_ep_tbl.size);
...
int ipa_tsp_reset(void)
{
	...
	for (i = 0; i < ipa3_ctx->tsp.egr_ep_max &&
		    ipa3_ctx->tsp.egr_ep_config[i] < IPA_CLIENT_MAX;
	     i++)
		ipa3_cfg_ep_prod_cfg(
			ipa_get_ep_mapping(ipa3_ctx->tsp.egr_ep_config[i]),
			&prod_cfg);

	if (ipa3_ctx->tsp.ingr_tc_tbl.base)
		memset(ipa3_ctx->tsp.ingr_tc_tbl.base, 0,
		       ipa3_ctx->tsp.ingr_tc_tbl.size);
	if (ipa3_ctx->tsp.egr_tc_tbl.base)
		memset(ipa3_ctx->tsp.egr_tc_tbl.base, 0,
		       ipa3_ctx->tsp.egr_tc_tbl.size);
	if (ipa3_ctx->tsp.egr_ep_tbl.base)
		memset(ipa3_ctx->tsp.egr_ep_tbl.base, 0,
		       ipa3_ctx->tsp.egr_ep_tbl.size);
