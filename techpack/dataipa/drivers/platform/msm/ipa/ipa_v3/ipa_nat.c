...
#define IPA_VALID_TBL_INDEX(ti) ((ti) == 0)
...
static inline const char *ipa3_nat_mem_in_as_str(enum ipa3_nat_mem_in nmi)
{
	switch (nmi) {
...
static inline char *ipa_ioc_v4_nat_init_as_str(struct ipa_ioc_v4_nat_init *ptr,
					       char *buf, uint32_t buf_sz)
{
	if (ptr && buf && buf_sz) {
		snprintf(
			buf, buf_sz,
			"V4 NAT INIT: tbl_index(0x%02X) ipv4_rules_offset(0x%08X) "
			"expn_rules_offset(0x%08X) index_offset(0x%08X) "
			"index_expn_offset(0x%08X) table_entries(0x%04X) "
			"expn_table_entries(0x%04X) ip_addr(0x%08X)",
			ptr->tbl_index, ptr->ipv4_rules_offset,
			ptr->expn_rules_offset, ptr->index_offset,
			ptr->index_expn_offset, ptr->table_entries,
			ptr->expn_table_entries, ptr->ip_addr);
	}
	return buf;
}
...
static int ipa3_nat_ipv6ct_open(struct inode *inode, struct file *filp)
{
	...
	IPADBG("\n");
	dev = container_of(inode->i_cdev, struct ipa3_nat_ipv6ct_common_mem,
			   cdev);
	...
	if (dev->is_nat_mem) {
		...
		if (nmi == IPA_NAT_MEM_IN_DDR) {
			IPADBG("map sz=0x%zx -> vma size=0x%08x\n",
			       mld_ptr->table_alloc_size, vsize);

			result = dma_mmap_coherent(ipa3_ctx->pdev, vma,
						   mld_ptr->vaddr,
						   mld_ptr->dma_handle,
			...
			result = vm_iomap_memory(vma, mld_ptr->phys_addr,
						 dev->phys_mem_size);
		...
		IPADBG("map sz=0x%zx -> vma size=0x%08x\n",
		       dev->table_alloc_size, vsize);

		result = dma_mmap_coherent(ipa3_ctx->pdev, vma, dev->vaddr,
					   dev->dma_handle,
...
static struct ipa3_nat_ipv6ct_tmp_mem *ipa3_nat_ipv6ct_allocate_tmp_memory(void)
{
	...
	tmp_mem->vaddr = dma_alloc_coherent(ipa3_ctx->pdev,
...
static int ipa3_nat_ipv6ct_init_device(struct ipa3_nat_ipv6ct_common_mem *dev,
				       const char *name, u32 phys_mem_size,
				       u32 phys_mem_ofst,
				       struct ipa3_nat_ipv6ct_tmp_mem *tmp_mem)
{
	int result = 0;
...
static void
ipa3_nat_ipv6ct_destroy_device(struct ipa3_nat_ipv6ct_common_mem *dev)
{
	IPADBG("In\n");
...
int ipa3_nat_ipv6ct_init_devices(void)
{
   ...
   */
	...
	if (ipa3_nat_ipv6ct_init_device(&ipa3_ctx->nat_mem.dev,
					IPA_NAT_DEV_NAME, IPA_NAT_PHYS_MEM_SIZE,
					IPA_NAT_PHYS_MEM_OFFSET, tmp_mem)) {
	...
	if ((ipa3_ctx->ipa_hw_type >= IPA_HW_v4_0) &&
	    ipa3_nat_ipv6ct_init_device(&ipa3_ctx->ipv6ct_mem.dev,
					IPA_IPV6CT_DEV_NAME,
					IPA_IPV6CT_PHYS_MEM_SIZE,
					IPA_IPV6CT_PHYS_MEM_OFFSET, tmp_mem)) {
...
static int
ipa3_nat_ipv6ct_allocate_mem(struct ipa3_nat_ipv6ct_common_mem *dev,
			     ...
			     enum ipahal_nat_type nat_type)
{
	...
	if (table_alloc->size > MAX_ALLOC_NAT_SIZE(nat_entry_size)) {
		IPAERR("Trying allocate more size = %zu, Max allowed = %zu\n",
		       table_alloc->size, MAX_ALLOC_NAT_SIZE(nat_entry_size));
		result = -EPERM;
		goto bail;
	}
	...
	if (nat_type == IPAHAL_NAT_IPV4) {
		...
		if (sram_compatible &&
		    table_alloc->size <= IPA_NAT_PHYS_MEM_SIZE) {
			/*
       ...
       */
			...
			mld_ptr->io_vaddr = ioremap(mld_ptr->phys_addr,
       ...
       */
			...
			mld_ptr->vaddr = dma_alloc_coherent(
				ipa3_ctx->pdev, mld_ptr->table_alloc_size,
				&mld_ptr->dma_handle, gfp_flags);
		...
		if (nat_type == IPAHAL_NAT_IPV6CT) {
			...
			dev->vaddr = dma_alloc_coherent(ipa3_ctx->pdev,
							dev->table_alloc_size,
							&dev->dma_handle,
							gfp_flags);
...
int ipa3_allocate_nat_table(struct ipa_ioc_nat_ipv6ct_table_alloc *table_alloc)
{
	...
	result = ipa3_nat_ipv6ct_allocate_mem(&nm_ptr->dev, table_alloc,
					      IPAHAL_NAT_IPV4);
	...
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v4_0 &&
		...
		if (IPA_MEM_PART(pdn_config_size) < pdn_mem_ptr->size) {
			IPAERR("number of PDN entries exceeds SRAM available space\n");
			result = -ENOMEM;
			goto fail_alloc_pdn;
		}

		pdn_mem_ptr->base =
			dma_alloc_coherent(ipa3_ctx->pdev, pdn_mem_ptr->size,
					   &pdn_mem_ptr->phys_base, gfp_flags);
...
fail_alloc_pdn:
	...
	if (nm_ptr->last_alloc_loc == IPA_NAT_MEM_IN_DDR) {
		if (mld_ptr->vaddr) {
			dma_free_coherent(ipa3_ctx->pdev,
					  mld_ptr->table_alloc_size,
					  mld_ptr->vaddr, mld_ptr->dma_handle);
			mld_ptr->vaddr = NULL;
		}
	}
	...
	struct ipa_ioc_nat_ipv6ct_table_alloc *table_alloc)
{
	...
	result = ipa3_nat_ipv6ct_allocate_mem(&ipa3_ctx->ipv6ct_mem.dev,
					      table_alloc, IPAHAL_NAT_IPV6CT);
...
static int ipa3_nat_ipv6ct_check_table_params(
	struct ipa3_nat_ipv6ct_common_mem *dev, enum ipa3_nat_mem_in nmi,
	uint32_t offset, uint16_t entries_num, enum ipahal_nat_type nat_type)
{
	...
	IPADBG("v4(%u) v6(%u) nmi(%s) ofst(%u) ents(%u) nt(%s)\n",
	       dev->is_nat_mem, dev->is_ipv6ct_mem, ipa3_nat_mem_in_as_str(nmi),
	       offset, entries_num, ipahal_nat_type_str(nat_type));
...
static inline void ipa3_nat_ipv6ct_create_init_cmd(
	...
	bool is_shared, dma_addr_t base_addr, uint8_t tbl_index,
	uint32_t base_table_offset, uint32_t expn_table_offset,
	uint16_t table_entries, uint16_t expn_table_entries,
...
static inline bool chk_sram_offset_alignment(uintptr_t addr, u32 mask)
{
...
static inline int ipa3_nat_ipv6ct_init_device_structure(
	...
	uint32_t base_table_offset, uint32_t expn_table_offset,
	uint16_t table_entries, uint16_t expn_table_entries,
	uint32_t index_offset, uint32_t index_expn_offset, uint8_t focus_change)
{
	int ret = 0;

	IPADBG("In\n");

	IPADBG("v4(%u) v6(%u) nmi(%s) bto(%u) eto(%u) t_ents(%u) et_ents(%u) io(%u) "
	       "ieo(%u)\n",
	       dev->is_nat_mem, dev->is_ipv6ct_mem, ipa3_nat_mem_in_as_str(nmi),
	       base_table_offset, expn_table_offset, table_entries,
	       expn_table_entries, index_offset, index_expn_offset);
	...
	if (dev->is_ipv6ct_mem) {
		...
		struct ipa3_nat_mem_loc_data *mld_p = &nm_ptr->mem_loc[nmi];
       ...
       */
			if (!chk_sram_offset_alignment(
				    (uintptr_t)mld_p->base_table_addr, 31) ||
			    !chk_sram_offset_alignment(
				    (uintptr_t)mld_p->expansion_table_addr,
				    31) ||
			    !chk_sram_offset_alignment(
				    (uintptr_t)mld_p->index_table_addr, 3) ||
			    !chk_sram_offset_alignment(
...
static void ipa3_nat_create_init_cmd(struct ipa_ioc_v4_nat_init *init,
				     bool is_shared, dma_addr_t base_addr,
				     struct ipahal_imm_cmd_ip_v4_nat_init *cmd)
{
	IPADBG("\n");

	ipa3_nat_ipv6ct_create_init_cmd(
		&cmd->table_init, is_shared, base_addr, init->tbl_index,
		init->ipv4_rules_offset, init->expn_rules_offset,
		init->table_entries, init->expn_table_entries,
		ipa3_ctx->nat_mem.dev.name);

	cmd->index_table_addr_shared = is_shared;
	cmd->index_table_expansion_addr_shared = is_shared;

	cmd->index_table_addr = base_addr + init->index_offset;
	IPADBG("index_offset:0x%x\n", init->index_offset);

	cmd->index_table_expansion_addr = base_addr + init->index_expn_offset;
...
static int
ipa3_nat_create_modify_pdn_cmd(struct ipahal_imm_cmd_dma_shared_mem *mem_cmd,
			       bool zero_mem)
{
	...
	mem_cmd->local_addr =
		ipa3_ctx->smem_restricted_bytes + IPA_MEM_PART(pdn_config_ofst);
				  ...
				  bool zero_pdn_table)
{
	...
	cmd_pyld[num_cmd] = ipahal_construct_imm_cmd(IPA_IMM_CMD_IP_V4_NAT_INIT,
						     cmd, false);
	if (!cmd_pyld[num_cmd]) {
	...
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v4_0) {
		...
		if (!cmd_pyld[num_cmd]) {
			IPAERR("fail construct dma_shared_mem cmd: for pdn table");
			result = -ENOMEM;
			goto destroy_imm_cmd;
		}
		ipa3_init_imm_cmd_desc(&desc[num_cmd], cmd_pyld[num_cmd]);
...
static int ipa3_ipv6ct_send_init_cmd(struct ipahal_imm_cmd_ip_v6_ct_init *cmd)
{
	...
	cmd_pyld[num_cmd] =
		ipahal_construct_imm_cmd(IPA_IMM_CMD_IP_V6_CT_INIT, cmd, false);
	if (!cmd_pyld[num_cmd]) {
...
int ipa3_nat_init_cmd(struct ipa_ioc_v4_nat_init *init)
{
	...
	if (init->table_entries == 0 || init->table_entries == U16_MAX) {
	...
	result = ipa3_nat_ipv6ct_check_table_params(dev, nmi,
	...
	if (nmi == IPA_NAT_MEM_IN_DDR) {
		ipa3_nat_create_init_cmd(init, false, mld_ptr->dma_handle,
					 &cmd);
	} else {
		ipa3_nat_create_init_cmd(init, true, IPA_RAM_NAT_OFST, &cmd);
	}

	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v4_0 && nm_ptr->pdn_mem.base &&
	    !init->focus_change) {
		...
		result = ipahal_nat_construct_entry(
			IPAHAL_NAT_IPV4_PDN, &pdn_entry, nm_ptr->pdn_mem.base);
	...
	result = ipa3_nat_ipv6ct_init_device_structure(
		dev, nmi, init->ipv4_rules_offset, init->expn_rules_offset,
		init->table_entries, init->expn_table_entries,
		init->index_offset, init->index_expn_offset,
		init->focus_change);
...
int ipa3_ipv6ct_init_cmd(struct ipa_ioc_ipv6ct_init *init)
{
	...
	result = ipa3_nat_ipv6ct_check_table_params(dev, IPA_NAT_MEM_IN_DDR,
	...
	ipa3_nat_ipv6ct_create_init_cmd(
		&cmd.table_init, false, dev->dma_handle, init->tbl_index,
		init->base_table_offset, init->expn_table_offset,
		init->table_entries, init->expn_table_entries, dev->name);
	...
	ipa3_nat_ipv6ct_init_device_structure(
		dev, IPA_NAT_MEM_IN_DDR, init->base_table_offset,
		init->expn_table_offset, init->table_entries,
		init->expn_table_entries, 0, 0, 0);
...
int ipa3_nat_mdfy_pdn(struct ipa_ioc_nat_pdn_entry *mdfy_pdn)
{
	...
	if (pdn_mem_ptr->base == NULL) {
		IPAERR_RL(
			"Attempt to modify a PDN entry before the PDN table memory "
			"allocation\n");
		result = -EPERM;
		goto bail;
	}
   ...
   */
	result = ipahal_nat_entry_size(IPAHAL_NAT_IPV4_PDN, &entry_size);
	...
	result = ipahal_nat_construct_entry(
		IPAHAL_NAT_IPV4_PDN, &pdn_fields,
	...
	IPADBG("Modify PDN in index: %d Public ip address:%pI4h\n",
	       mdfy_pdn->pdn_index, &pdn_fields.public_ip);
...
static uint32_t ipa3_nat_ipv6ct_calculate_table_size(enum ipa3_nat_mem_in nmi,
						     uint8_t base_addr)
{
	size_t entry_size;
	u32 num_entries;
...
static int ipa3_table_validate_table_dma_one(enum ipa3_nat_mem_in nmi,
					     struct ipa_ioc_nat_dma_one *param)
{
	...
	table_size =
		ipa3_nat_ipv6ct_calculate_table_size(nmi, param->base_addr);
...
int ipa3_table_dma_cmd(struct ipa_ioc_nat_dma_cmd *dma)
{
   ...
   */
	...
	for (cnt = 0; cnt < dma->entries; ++cnt) {
		result = ipa3_table_validate_table_dma_one(dma->mem_type,
							   &dma->dma[cnt]);
...
static void ipa3_nat_ipv6ct_free_mem(struct ipa3_nat_ipv6ct_common_mem *dev)
{
	...
	if (dev->is_ipv6ct_mem) {
		...
		if (dev->vaddr) {
			...
			dma_free_coherent(ipa3_ctx->pdev, dev->table_alloc_size,
...
static int ipa3_nat_ipv6ct_create_del_table_cmd(
	uint8_t tbl_index, u32 base_addr,
	struct ipa3_nat_ipv6ct_common_mem *dev,
	struct ipahal_imm_cmd_nat_ipv6ct_init_common *table_init_cmd)
{
	...
	IPADBG("In: tbl_index(%u) base_addr(%u) v4(%u) v6(%u)\n", tbl_index,
	       base_addr, dev->is_nat_mem, dev->is_ipv6ct_mem);
...
static int ipa3_nat_send_del_table_cmd(uint8_t tbl_index)
{
	...
	result = ipa3_nat_ipv6ct_create_del_table_cmd(tbl_index,
						      IPA_NAT_PHYS_MEM_OFFSET,
						      &ipa3_ctx->nat_mem.dev,
						      &cmd.table_init);

	if (result) {
		IPAERR("Fail to create immediate command to delete NAT table\n");
		goto bail;
	}
...
static int ipa3_ipv6ct_send_del_table_cmd(uint8_t tbl_index)
{
	...
	result = ipa3_nat_ipv6ct_create_del_table_cmd(
		tbl_index, IPA_IPV6CT_PHYS_MEM_OFFSET,
		&ipa3_ctx->ipv6ct_mem.dev, &cmd.table_init);
	if (result) {
		IPAERR("Fail to create immediate command to delete IPv6CT table\n");
		return result;
	}
...
int ipa3_del_nat_table(struct ipa_ioc_nat_ipv6ct_table_del *del)
{
	...
	if (dev->is_hw_init) {
		...
		if (result) {
			IPAERR("Fail to send immediate command to delete NAT table\n");
			goto unlock;
		}
	}
	...
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v4_0 && nm_ptr->pdn_mem.base) {
		...
		dma_free_coherent(ipa3_ctx->pdev, pdn_mem_ptr->size,
				  pdn_mem_ptr->base, pdn_mem_ptr->phys_base);
...
int ipa3_nat_get_sram_info(struct ipa_nat_in_sram_info *info_ptr)
{
	...
	memset(info_ptr, 0, sizeof(struct ipa_nat_in_sram_info));
   ...
   */
	info_ptr->best_nat_in_sram_size_rqst =
		roundup(info_ptr->nat_table_offset_into_mmap + IPA_RAM_NAT_SIZE,
			PAGE_SIZE);
