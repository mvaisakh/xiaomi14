...
wcd_clsh_set_flyback_mode(struct snd_soc_component *component, int mode)
{
	if (mode == CLS_H_HIFI || mode == CLS_H_LOHIFI || mode == CLS_AB_HIFI ||
		...
		snd_soc_component_update_bits(
			component, WCD9XXX_ANA_RX_SUPPLIES, 0x04, 0x04);
#if 0
		snd_soc_component_update_bits(component,
					 ...
					 int mode, bool enable)
{
	if (enable) {
		snd_soc_component_update_bits(
			component, WCD9XXX_FLYBACK_VNEGDAC_CTRL_2, 0xE0, 0xA0);
		/* 100usec delay is needed as per HW requirement */
		usleep_range(100, 110);
		snd_soc_component_update_bits(component, WCD9XXX_CLASSH_MODE_3,
					      0x02, 0x02);
		snd_soc_component_update_bits(component, WCD9XXX_CLASSH_MODE_2,
					      0xFF, 0x1C);
		if (mode == CLS_H_LOHIFI || mode == CLS_AB_LOHIFI) {
			snd_soc_component_update_bits(
				component, WCD9XXX_HPH_NEW_INT_PA_MISC2, 0x20,
				0x20);
			snd_soc_component_update_bits(
				component, WCD9XXX_RX_BIAS_HPH_LOWPOWER, 0xF0,
				0xC0);
			snd_soc_component_update_bits(
				component, WCD9XXX_HPH_PA_CTL1, 0x0E, 0x02);
		}
	} else {
		snd_soc_component_update_bits(
			component, WCD9XXX_HPH_NEW_INT_PA_MISC2, 0x20, 0x00);
		snd_soc_component_update_bits(
...
static void wcd_clsh_buck_ctrl(struct snd_soc_component *component,
			       struct wcd_clsh_cdc_info *clsh_d, int mode,
			       bool enable)
{
	/* enable/disable buck */
     ...
     */
		...
		if (mode == CLS_H_LOHIFI || mode == CLS_H_ULP ||
			...
			snd_soc_component_update_bits(
				component, WCD9XXX_CLASSH_MODE_3, 0x02, 0x00);
				  ...
				  bool enable)
{
	...
	if ((enable && (++clsh_d->flyback_users == 1)) ||
		...
		snd_soc_component_update_bits(
			component, WCD9XXX_FLYBACK_VNEG_CTRL_1, 0xE0, 0xE0);
		snd_soc_component_update_bits(component,
					      WCD9XXX_ANA_RX_SUPPLIES, (1 << 6),
					      (enable << 6));
		/*
     ...
     */
		usleep_range(100, 110);
		snd_soc_component_update_bits(
			component, WCD9XXX_FLYBACK_VNEGDAC_CTRL_2, 0xE0, 0xE0);
		/* 500usec delay is needed as per HW requirement */
		usleep_range(500, 500 + WCD_USLEEP_RANGE);
	}
 ...
 * the input mode.
 */
void wcd_clsh_set_hph_mode(struct snd_soc_component *component, int mode)
{
	u8 val = 0;
...
{
	snd_soc_component_update_bits(component, WCD9XXX_ANA_RX_SUPPLIES, 0x02,
				      0x00);
}
...
static void wcd_clsh_state_hph_r(struct snd_soc_component *component,
				 struct wcd_clsh_cdc_info *clsh_d, u8 req_state,
...
void wcd_cls_h_fsm(struct snd_soc_component *component,
		   struct wcd_clsh_cdc_info *cdc_clsh_d, u8 clsh_event,
...
void wcd_cls_h_init(struct wcd_clsh_cdc_info *clsh)
{
	...
	clsh_state_fp[WCD_CLSH_STATE_HPH_ST_AUX] = wcd_clsh_state_hph_aux;
