...
static const struct comp_coeff_val
	comp_coeff_table[HPH_MODE_MAX][COMP_MAX_COEFF] = {
		{
			{ 0x40, 0x00 }, { 0x4C, 0x00 }, { 0x5A, 0x00 },
			{ 0x6B, 0x00 }, { 0x7F, 0x00 }, { 0x97, 0x00 },
			{ 0xB3, 0x00 }, { 0xD5, 0x00 }, { 0xFD, 0x00 },
			{ 0x2D, 0x01 }, { 0x66, 0x01 }, { 0xA7, 0x01 },
			{ 0xF8, 0x01 }, { 0x57, 0x02 }, { 0xC7, 0x02 },
			{ 0x4B, 0x03 }, { 0xE9, 0x03 }, { 0xA3, 0x04 },
			{ 0x7D, 0x05 }, { 0x90, 0x06 }, { 0xD1, 0x07 },
			{ 0x49, 0x09 }, { 0x00, 0x0B }, { 0x01, 0x0D },
			{ 0x59, 0x0F },
		},
		{
			{ 0x40, 0x00 }, { 0x4C, 0x00 }, { 0x5A, 0x00 },
			{ 0x6B, 0x00 }, { 0x80, 0x00 }, { 0x98, 0x00 },
			{ 0xB4, 0x00 }, { 0xD5, 0x00 }, { 0xFE, 0x00 },
			{ 0x2E, 0x01 }, { 0x66, 0x01 }, { 0xA9, 0x01 },
			{ 0xF8, 0x01 }, { 0x56, 0x02 }, { 0xC4, 0x02 },
			{ 0x4F, 0x03 }, { 0xF0, 0x03 }, { 0xAE, 0x04 },
			{ 0x8B, 0x05 }, { 0x8E, 0x06 }, { 0xBC, 0x07 },
			{ 0x56, 0x09 }, { 0x0F, 0x0B }, { 0x13, 0x0D },
			{ 0x6F, 0x0F },
		},
	};
...
struct rx_macro_priv {
	...
	struct platform_device *pdev_child_devices[RX_MACRO_CHILD_DEVICES_MAX];
	int child_count;
	int is_softclip_on;
	int is_aux_hpf_on;
...
static const char *const rx_macro_mux_text[] = { "ZERO", "AIF1_PB", "AIF2_PB",
						 "AIF3_PB", "AIF4_PB" };
...
static const struct soc_enum rx_mix_tx2_mux_enum = SOC_ENUM_SINGLE(
	BOLERO_CDC_RX_INP_MUX_RX_MIX_CFG5, 0, 4, rx_echo_mux_text);
...
static const struct soc_enum rx_mix_tx1_mux_enum = SOC_ENUM_SINGLE(
	BOLERO_CDC_RX_INP_MUX_RX_MIX_CFG4, 0, 4, rx_echo_mux_text);
...
static const struct soc_enum rx_mix_tx0_mux_enum = SOC_ENUM_SINGLE(
	BOLERO_CDC_RX_INP_MUX_RX_MIX_CFG4, 4, 4, rx_echo_mux_text);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int val = 0;
	unsigned short look_ahead_dly_reg = BOLERO_CDC_RX_RX0_RX_PATH_CFG0;
					  ...
					  u32 sample_rate)
{
	...
	ret = rx_macro_set_prim_interpolator_rate(dai, (u8)rate_val,
						  sample_rate);
	if (ret)
		return ret;
	ret = rx_macro_set_mix_interpolator_rate(dai, (u8)rate_val,
						 sample_rate);
	if (ret)
		return ret;
...
static int rx_macro_mute_stream(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	case RX_MACRO_AIF4_PB:
		for (j = 0; j < INTERP_MAX; j++) {
			...
			int_mux_cfg0 =
				BOLERO_CDC_RX_INP_MUX_RX_INT0_CFG0 + j * 8;
			int_mux_cfg1 = int_mux_cfg0 + 4;
			int_mux_cfg0_val =
				snd_soc_component_read(component, int_mux_cfg0);
			int_mux_cfg1_val =
			...
			if (snd_soc_component_read(component, dsm_reg) & 0x01) {
				if (int_mux_cfg0_val ||
				    (int_mux_cfg1_val & 0xF0))
					snd_soc_component_update_bits(
						component, reg, 0x20, 0x20);
				if (int_mux_cfg1_val & 0x0F) {
					snd_soc_component_update_bits(
						component, reg, 0x20, 0x20);
					snd_soc_component_update_bits(
						component, mix_reg, 0x20, 0x20);
				}
			}
		}
...
static int rx_macro_mclk_enable(struct rx_macro_priv *rx_priv, bool mclk_enable,
				bool dapm)
{
	...
	if (mclk_enable) {
		if (rx_priv->rx_mclk_users == 0) {
			...
			bolero_clk_rsc_fs_gen_request(rx_priv->dev, true);
			regcache_mark_dirty(regmap);
			regcache_sync_region(regmap, RX_START_OFFSET,
					     RX_MAX_OFFSET);
			regmap_update_bits(
				regmap, BOLERO_CDC_RX_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x01);
			regmap_update_bits(
				regmap, BOLERO_CDC_RX_CLK_RST_CTRL_MCLK_CONTROL,
				0x02, 0x02);
			regmap_update_bits(
				regmap,
				BOLERO_CDC_RX_CLK_RST_CTRL_FS_CNT_CONTROL, 0x02,
				0x00);
			regmap_update_bits(
				regmap,
				BOLERO_CDC_RX_CLK_RST_CTRL_FS_CNT_CONTROL, 0x01,
				0x01);
		}
		...
		if (rx_priv->rx_mclk_users == 0) {
			...
			regmap_update_bits(
				regmap, BOLERO_CDC_RX_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x00);
			bolero_clk_rsc_fs_gen_request(rx_priv->dev, false);
			rx_macro_core_vote(rx_priv, true);
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		break;
	default:
		dev_err(rx_priv->dev, "%s: invalid DAPM event %d\n", __func__,
			event);
		ret = -EINVAL;
	}
	return ret;
				  ...
				  u16 event, u32 data)
{
	...
	case BOLERO_MACRO_EVT_RX_MUTE:
		...
		val = mute ? 0x10 : 0x00;
		reg = BOLERO_CDC_RX_RX0_RX_PATH_CTL +
		      (rx_idx * RX_MACRO_RX_PATH_OFFSET);
		reg_mix = BOLERO_CDC_RX_RX0_RX_PATH_MIX_CTL +
			  (rx_idx * RX_MACRO_RX_PATH_OFFSET);
	...
	case BOLERO_MACRO_EVT_RX_PA_GAIN_UPDATE:
		...
		if (data) {
			...
			if ((rx_priv->rx0_gain_val >=
			    ...
			    (rx_priv->rx0_gain_val <=
			     (RX_MACRO_GAIN_MAX_VAL - RX_MACRO_MOD_GAIN)))
				snd_soc_component_update_bits(
					component, BOLERO_CDC_RX_RX0_RX_VOL_CTL,
					0xFF,
					(rx_priv->rx0_gain_val +
	...
	case BOLERO_MACRO_EVT_HPHL_HD2_ENABLE:
		...
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX0_RX_PATH_CFG0, 0x04, data);
		break;
	case BOLERO_MACRO_EVT_HPHR_HD2_ENABLE:
		/* Enable hd2 config for hphr*/
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX1_RX_PATH_CFG0, 0x04, data);
		break;
	}
done:
	return ret;
					...
					int interp, int path_type)
{
   ...
   */
	if (path_type == INTERP_MIX_PATH) {
		mux_reg = BOLERO_CDC_RX_INP_MUX_RX_INT0_CFG1 + 2 * interp;
	...
	if (path_type == INTERP_MAIN_PATH) {
		mux_reg = BOLERO_CDC_RX_INP_MUX_RX_INT1_CFG0 + 2 * (interp - 1);
		mux_reg_val = snd_soc_component_read(component, mux_reg) & 0x0f;
	...
	if ((rx_priv->idle_det_cfg.hph_idle_thr == 0) ||
		...
		snd_soc_component_write(
			component, BOLERO_CDC_RX_IDLE_DETECT_CFG3, idle_thr);
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	reg = BOLERO_CDC_RX_RX0_RX_PATH_CTL +
	      (w->shift * RX_MACRO_RX_PATH_OFFSET);
	gain_reg = BOLERO_CDC_RX_RX0_RX_VOL_CTL +
		   (w->shift * RX_MACRO_RX_PATH_OFFSET);
				     ...
				     int interp_n, int event)
{
	...
	comp_ctl0_reg =
		BOLERO_CDC_RX_COMPANDER0_CTL0 + (comp * RX_MACRO_COMP_OFFSET);
					 ...
					 bool enable)
{
	if (enable) {
		if (rx_priv->softclip_clk_users == 0)
			snd_soc_component_update_bits(
				component, BOLERO_CDC_RX_SOFTCLIP_CRC, 0x01,
				0x01);
...
static int rx_macro_config_softclip(struct snd_soc_component *component,
				    struct rx_macro_priv *rx_priv, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_SOFTCLIP_SOFTCLIP_CTRL, 0x01,
			0x01);
	}
				   ...
				   struct rx_macro_priv *rx_priv, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		if (!rx_priv->is_aux_hpf_on)
			snd_soc_component_update_bits(
				component, BOLERO_CDC_RX_RX2_RX_PATH_CFG1, 0x04,
				0x00);
	}
	...
	if (SND_SOC_DAPM_EVENT_OFF(event)) {
		...
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG1, 0x04, 0x04);
	}

	return 0;
}

static inline void rx_macro_enable_clsh_block(struct rx_macro_priv *rx_priv,
					      bool enable)
{
...
static int rx_macro_config_classh(struct snd_soc_component *component,
				  struct rx_macro_priv *rx_priv, int interp_n,
				  int event)
{
	if (SND_SOC_DAPM_EVENT_OFF(event)) {
     ...
     */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_CLSH_K1_LSB, 0xFF, 0xC0);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_CLSH_K1_MSB, 0x0F, 0x00);
	}
	switch (interp_n) {
	case INTERP_HPHL:
		if (rx_priv->is_ear_mode_on)
			snd_soc_component_update_bits(
				component, BOLERO_CDC_RX_CLSH_HPH_V_PA, 0x3F,
				0x39);
		else
			snd_soc_component_update_bits(
				component, BOLERO_CDC_RX_CLSH_HPH_V_PA, 0x3F,
				0x1C);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_CLSH_DECAY_CTRL, 0x07, 0x00);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX0_RX_PATH_CFG0, 0x40, 0x40);
		break;
	case INTERP_HPHR:
		if (rx_priv->is_ear_mode_on)
			snd_soc_component_update_bits(
		...
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_CLSH_DECAY_CTRL, 0x07, 0x00);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX1_RX_PATH_CFG0, 0x40, 0x40);
		break;
	case INTERP_AUX:
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG0, 0x08, 0x08);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG0, 0x10, 0x10);
		break;
	}
...
static int
rx_macro_vbat_bcl_gsm_mode_func_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
rx_macro_vbat_bcl_gsm_mode_func_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (ucontrol->value.integer.value[0])
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_CFG, 0x04, 0x04);
	else
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_CFG, 0x04, 0x00);
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_PATH_CTL, 0x10, 0x10);
		/* Enable VBAT block */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_CFG, 0x01, 0x01);
		/* Update interpolator with 384K path */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG1, 0x80, 0x80);
		/* Update DSM FS rate */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_SEC7, 0x02, 0x02);
		/* Use attenuation mode */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_CFG, 0x02, 0x00);
		/* BCL block needs softclip clock to be enabled */
		rx_macro_enable_softclip_clk(component, rx_priv, true);
		/* Enable VBAT at channel level */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG1, 0x02, 0x02);
		/* Set the ATTK1 gain */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD1, 0xFF,
			0xFF);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD2, 0xFF,
			0x03);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD3, 0xFF,
			0x00);
		/* Set the ATTK2 gain */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD4, 0xFF,
			0xFF);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD5, 0xFF,
			0x03);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD6, 0xFF,
			0x00);
		/* Set the ATTK3 gain */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD7, 0xFF,
			0xFF);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD8, 0xFF,
			0x03);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD9, 0xFF,
			0x00);
		break;

	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG1, 0x80, 0x00);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_SEC7, 0x02, 0x00);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_CFG, 0x02, 0x02);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX2_RX_PATH_CFG1, 0x02, 0x00);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_BCL_GAIN_UPD1, 0xFF,
			0x00);
		snd_soc_component_update_bits(
		...
		rx_macro_enable_softclip_clk(component, rx_priv, false);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_CFG, 0x01, 0x00);
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_PATH_CTL, 0x10, 0x00);
		break;
	default:
					...
					u16 interp_idx, int event)
{
	...
	if (hph_lut_bypass_reg && SND_SOC_DAPM_EVENT_ON(event)) {
		if (interp_idx == INTERP_HPHL) {
			if (rx_priv->is_ear_mode_on)
				snd_soc_component_update_bits(
					component,
					BOLERO_CDC_RX_RX0_RX_PATH_CFG1, 0x02,
					0x02);
			else
				snd_soc_component_update_bits(
	...
	if (hph_lut_bypass_reg && SND_SOC_DAPM_EVENT_OFF(event)) {
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_RX0_RX_PATH_CFG1, 0x02, 0x00);
				      ...
				      int event, int interp_idx)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (rx_priv->main_clk_users[interp_idx] == 0) {
			...
			rx_macro_config_classh(component, rx_priv, interp_idx,
					       event);
		}
...
static void rx_macro_restore_iir_coeff(struct rx_macro_priv *rx_priv,
...
static int
rx_macro_iir_enable_audio_mixer_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
rx_macro_iir_enable_audio_mixer_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
rx_macro_iir_band_audio_mixer_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
rx_macro_iir_band_audio_mixer_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static const struct snd_kcontrol_new rx_macro_snd_controls[] = {
	SOC_SINGLE_S8_TLV("RX_RX0 Digital Volume", BOLERO_CDC_RX_RX0_RX_VOL_CTL,
			  -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX1 Digital Volume", BOLERO_CDC_RX_RX1_RX_VOL_CTL,
			  -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX2 Digital Volume", BOLERO_CDC_RX_RX2_RX_VOL_CTL,
			  -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX0 Mix Digital Volume",
			  BOLERO_CDC_RX_RX0_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX1 Mix Digital Volume",
			  BOLERO_CDC_RX_RX1_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX2 Mix Digital Volume",
			  BOLERO_CDC_RX_RX2_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	...
	SOC_ENUM_EXT("HPH Idle Detect", hph_idle_detect_enum,
		     rx_macro_hph_idle_detect_get,
		     rx_macro_hph_idle_detect_put),
	...
	SOC_SINGLE_EXT("AUX_HPF Enable", SND_SOC_NOPM, 0, 1, 0,
		       rx_macro_aux_hpf_mode_get, rx_macro_aux_hpf_mode_put),
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	ec_hq_reg = BOLERO_CDC_RX_EC_REF_HQ0_EC_REF_HQ_PATH_CTL + 0x40 * ec_tx;
	snd_soc_component_update_bits(component, ec_hq_reg, 0x01, 0x01);
	ec_hq_reg = BOLERO_CDC_RX_EC_REF_HQ0_EC_REF_HQ_CFG0 + 0x40 * ec_tx;
	/* default set to 48k */
...
static const struct snd_soc_dapm_widget rx_macro_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX_E("RX MIX TX0 MUX", SND_SOC_NOPM, RX_MACRO_EC0_MUX, 0,
			   &rx_mix_tx0_mux, rx_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX MIX TX1 MUX", SND_SOC_NOPM, RX_MACRO_EC1_MUX, 0,
			   &rx_mix_tx1_mux, rx_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX MIX TX2 MUX", SND_SOC_NOPM, RX_MACRO_EC2_MUX, 0,
	...
	SND_SOC_DAPM_MUX_E("RX INT0 MIX2 INP", SND_SOC_NOPM, INTERP_HPHL, 0,
			   &rx_int0_mix2_inp_mux, rx_macro_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX INT1 MIX2 INP", SND_SOC_NOPM, INTERP_HPHR, 0,
			   &rx_int1_mix2_inp_mux, rx_macro_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX INT2 MIX2 INP", SND_SOC_NOPM, INTERP_AUX, 0,
			   &rx_int2_mix2_inp_mux, rx_macro_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MIXER_E(
		"RX INT2_1 VBAT", SND_SOC_NOPM, 0, 0, rx_int2_1_vbat_mix_switch,
		ARRAY_SIZE(rx_int2_1_vbat_mix_switch), rx_macro_enable_vbat,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
		...
		if (rx_priv->swr_clk_users == 0) {
			...
			if (rx_priv->reset_swr)
				regmap_update_bits(
					...
					0x02, 0x02);
			regmap_update_bits(
				regmap, BOLERO_CDC_RX_CLK_RST_CTRL_SWR_CONTROL,
				0x01, 0x01);
			if (rx_priv->reset_swr)
				regmap_update_bits(
					regmap,
...
static void rx_macro_init_bcl_pmic_reg(struct snd_soc_component *component)
{
	...
	case 0:
		...
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_DECODE_CTL1, 0x02,
			0x02);
		/* Update MC_SID0 */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_DECODE_CFG1, 0x0F,
			rx_priv->bcl_pmic_params.sid);
		/* Update MC_PPID0 */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_DECODE_CFG2, 0xFF,
			rx_priv->bcl_pmic_params.ppid);
		break;
	case 1:
		/* Enable ID1 to listen to respective PMIC group interrupts */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_DECODE_CTL1, 0x01,
			0x01);
		/* Update MC_SID1 */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_DECODE_CFG3, 0x0F,
			rx_priv->bcl_pmic_params.sid);
		/* Update MC_PPID1 */
		snd_soc_component_update_bits(
			component, BOLERO_CDC_RX_BCL_VBAT_DECODE_CFG1, 0xFF,
			rx_priv->bcl_pmic_params.ppid);
		break;
	default:
		dev_err(rx_dev, "%s: PMIC ID is invalid %d\n", __func__,
			rx_priv->bcl_pmic_params.id);
		break;
	}
}
...
static int rx_macro_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "qcom,rx-wsa-enable",
				   &val);
	if (ret == 0) {
	...
	rx_io_base =
		devm_ioremap(&pdev->dev, rx_base_addr, RX_MACRO_MAX_OFFSET);
	if (!rx_io_base) {
...
static int rx_macro_remove(struct platform_device *pdev)
{
	...
	for (count = 0;
	     count < rx_priv->child_count && count < RX_MACRO_CHILD_DEVICES_MAX;
	     count++)
...
static const struct dev_pm_ops bolero_dev_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
		...
		SET_RUNTIME_PM_OPS(bolero_runtime_suspend,
				   bolero_runtime_resume, NULL)
};
