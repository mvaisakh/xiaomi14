...
#define CPE_ERR_IRQ_CB(core) (core->cpe_cdc_cb->cpe_err_irq_control)
...
static struct cpe_lsm_session *lsm_sessions[WCD_CPE_LSM_MAX_SESSIONS + 1];
...
static int wcd_cpe_afe_svc_cmd_mode(void *core_handle, u8 mode);
...
#define to_wcd_cpe_attr(a) container_of((a), struct wcd_cpe_attribute, attr)
...
static int wcd_cpe_get_sfr_dump(struct wcd_cpe_core *core)
{
	...
	dev_dbg(core->dev, "%s: reading SFR from CPE, size = %zu\n", __func__,
		core->sfr_buf_size);
...
static int wcd_cpe_collect_ramdump(struct wcd_cpe_core *core)
{
	...
	dev_dbg(core->dev, "%s: Reading ramdump from CPE\n", __func__);

	rc = cpe_svc_ramdump(core->cpe_handle, &dump_seg);
	if (rc < 0) {
		dev_err(core->dev, "%s: Failed to read CPE ramdump, err = %d\n",
			__func__, rc);
		return rc;
	}

	dev_dbg(core->dev, "%s: completed reading ramdump from CPE\n",
		__func__);
	...
	rc = do_ramdump(core->cpe_ramdump_dev, &core->cpe_ramdump_seg, 1);
	if (rc)
		dev_err(core->dev,
 ...
 */
static int wcd_cpe_load_each_segment(struct wcd_cpe_core *core, int file_idx,
				     const struct elf32_phdr *phdr)
{
	...
	snprintf(split_fname, sizeof(split_fname), "%s.b%02d", core->fname,
		 file_idx);
	...
	dev_dbg(core->dev, "%s: cpe segment type %s read from firmware\n",
		__func__,
		(segment->type == CPE_SVC_INSTRUCTION_MEM) ? "INSTRUCTION" :
							     "DATA");

	ret = cpe_svc_download_segment(core->cpe_handle, segment);
	if (ret) {
		dev_err(core->dev, "%s: Failed to download %s, error = %d\n",
...
static int wcd_cpe_enable_cpe_clks(struct wcd_cpe_core *core, bool enable)
{
	...
	if (!core || !core->cpe_cdc_cb || !core->cpe_cdc_cb->cpe_clk_en) {
 ...
 */
static int wcd_cpe_bus_vote_max_bw(struct wcd_cpe_core *core, bool vote)
{
 ...
 */
static int wcd_cpe_load_fw(struct wcd_cpe_core *core, unsigned int load_type)
{
	int ret, phdr_idx;
	...
	dev_dbg(core->dev,
		"%s: start image dload, name = %s, load_type = 0x%x\n",
	...
	for (phdr_idx = 0; phdr_idx < ehdr->e_phnum; phdr_idx++) {
		...
		dev_dbg(core->dev,
			"index = %d, vaddr = 0x%x, paddr = 0x%x, filesz = 0x%x, memsz = "
			"0x%x, flags = 0x%x\n",
			phdr_idx, phdr->p_vaddr, phdr->p_paddr, phdr->p_filesz,
 ...
 *
 */
static void wcd_cpe_change_online_state(struct wcd_cpe_core *core, int online)
{
	...
	pr_debug("%s: change state 0x%x offline_change 0x%x\n"
		 ...
		 __func__, online, ssr_entry->offline_change,
...
void *wcd_cpe_get_core_handle(struct snd_soc_component *component)
{
	...
	if (!wcd_get_cpe_core) {
		dev_err(component->dev, "%s: codec callback not available\n",
			__func__);
		goto done;
	}
	...
	if (!core)
		dev_err(component->dev, "%s: handle to core not available\n",
			__func__);
done:
	return core;
}
				  ...
				  char __user *buf, size_t count, loff_t pos)
{
	...
	rmb();
	dev_dbg(core->dev, "%s: Offline 0x%x\n", __func__, ssr_entry->offline);
...
static int wcd_cpe_check_new_image(struct wcd_cpe_core *core)
{
   ...
   */
	strlcpy(temp_img_name, core->fname, WCD_CPE_IMAGE_FNAME_MAX);
	strlcpy(core->fname, core->dyn_fname, WCD_CPE_IMAGE_FNAME_MAX);
	...
	if (rc) {
		...
		strlcpy(core->fname, temp_img_name, WCD_CPE_IMAGE_FNAME_MAX);
		rc = wcd_cpe_load_fw(core, ELF_FLAG_EXECUTE);
		if (rc)
			dev_err(core->dev,
...
static int wcd_cpe_enable(struct wcd_cpe_core *core, bool enable)
{
	int ret = 0;
	...
	if (enable) {
		...
		ret = cpe_svc_reset(core->cpe_handle);
		if (ret < 0) {
			dev_err(core->dev, "%s: CPE Reset failed, error = %d\n",
				__func__, ret);
			goto done;
		}
		...
		if (ret < 0) {
			dev_err(core->dev, "%s: Failed to boot CPE\n",
				__func__);
			goto fail_boot;
		}

		/* wait for CPE to be online */
		dev_dbg(core->dev, "%s: waiting for CPE bootup\n", __func__);

		wait_for_completion(&core->online_compl);

		dev_dbg(core->dev, "%s: CPE bootup done\n", __func__);
...
static int wcd_cpe_boot_ssr(struct wcd_cpe_core *core)
{
	...
	rc = wcd_cpe_load_fw(core, ELF_FLAG_EXECUTE);
	if (rc) {
		dev_err(core->dev, "%s: Failed to load instruction, err = %d\n",
			__func__, rc);
		goto fail;
	} else {
		wcd_cpe_change_online_state(core, 1);
 ...
 */
static void wcd_cpe_clr_ready_status(struct wcd_cpe_core *core, u8 value)
{
	WCD_CPE_GRAB_LOCK(&core->ssr_lock, "SSR");
	core->ready_status &= ~(value);
	dev_dbg(core->dev, "%s: ready_status = 0x%x\n", __func__,
		core->ready_status);
	WCD_CPE_REL_LOCK(&core->ssr_lock, "SSR");
}
 ...
 */
static void wcd_cpe_set_and_complete(struct wcd_cpe_core *core, u8 value)
{
	WCD_CPE_GRAB_LOCK(&core->ssr_lock, "SSR");
	core->ready_status |= value;
	if ((core->ready_status & WCD_CPE_READY_TO_DLOAD) ==
	    WCD_CPE_READY_TO_DLOAD) {
		dev_dbg(core->dev, "%s: marking ready, status = 0x%x\n",
...
static void wcd_cpe_ssr_work(struct work_struct *work)
{
	...
	if (core->cpe_users > 0) {
       ...
       */
		...
		wcd_cpe_set_and_complete(core, WCD_CPE_BLK_READY);
	}
...
err_ret:
	...
	pm_qos_update_request(&core->pm_qos_req, PM_QOS_DEFAULT_VALUE);
 ...
 */
int wcd_cpe_ssr_event(void *core_handle, enum wcd_cpe_ssr_state_event event)
{
	struct wcd_cpe_core *core = core_handle;
   ...
   */
	...
	dev_dbg(core->dev, "%s: Schedule ssr work, event = %d\n", __func__,
		core->ssr_type);
	...
	default:
		dev_err(core->dev, "%s: unhandled SSR event %d\n", __func__,
			event);
		return -EINVAL;
	}
...
static irqreturn_t svass_exception_irq(int irq, void *data)
{
     ...
     */
		core->cpe_cdc_cb->cpe_err_irq_control(
			core->component, CPE_ERR_IRQ_MASK, &status);

		/* Clear only the interrupt that was raised */
		core->cpe_cdc_cb->cpe_err_irq_control(
			core->component, CPE_ERR_IRQ_CLEAR, &status);
		dev_err(core->dev, "%s: err_interrupt status = 0x%x\n",
			__func__, status);
...
static void wcd_cpe_svc_event_cb(const struct cpe_svc_notification *param)
{
	...
	dev_dbg(core->dev, "%s: event = 0x%x, ssr_type = 0x%x\n", __func__,
		param->event, core->ssr_type);

	switch (param->event) {
	case CPE_SVC_BOOT:
		boot_data = (struct cpe_svc_boot_event *)param->payload;
		...
		dev_dbg(core->dev,
			...
			__func__, core->sfr_buf_addr, core->sfr_buf_size);
		break;
	case CPE_SVC_ONLINE:
     ...
     */
		...
		break;
	default:
		dev_err(core->dev, "%s: unhandled notification\n", __func__);
		break;
	}
}
...
static void wcd_cpe_cleanup_irqs(struct wcd_cpe_core *core)
{
	...
	wcd9xxx_free_irq(core_res, core->irq_info.cpe_engine_irq, core);
	wcd9xxx_free_irq(core_res, core->irq_info.cpe_err_irq, core);
}
...
static int wcd_cpe_setup_irqs(struct wcd_cpe_core *core)
{
	...
	ret = wcd9xxx_request_irq(core_res, core->irq_info.cpe_engine_irq,
				  svass_engine_irq, "SVASS_Engine", core);
	if (ret) {
		dev_err(core->dev, "%s: Failed to request svass engine irq\n",
			__func__);
	...
	if (CPE_ERR_IRQ_CB(core))
		core->cpe_cdc_cb->cpe_err_irq_control(core->component,
						      CPE_ERR_IRQ_UNMASK, NULL);

	ret = wcd9xxx_request_irq(core_res, core->irq_info.cpe_err_irq,
				  svass_exception_irq, "SVASS_Exception", core);
	if (ret) {
		dev_err(core->dev, "%s: Failed to request svass err irq\n",
			__func__);
...
static int wcd_cpe_alloc_cal(int32_t cal_type, size_t data_size, void *data)
{
	...
	ret = cal_utils_alloc_cal(data_size, data, core_d->cal_data[cal_index],
				  0, NULL);
	if (ret < 0)
...
static int wcd_cpe_dealloc_cal(int32_t cal_type, size_t data_size, void *data)
{
	int ret = 0;
	int cal_index;
...
static int wcd_cpe_set_cal(int32_t cal_type, size_t data_size, void *data)
{
	...
	ret = cal_utils_set_cal(data_size, data, core_d->cal_data[cal_index], 0,
				NULL);
	if (ret < 0)
...
static int wcd_cpe_cal_init(struct wcd_cpe_core *core)
{
	...
	ret = cal_utils_create_cal_types(WCD_CPE_LSM_CAL_MAX, core->cal_data,
					 cal_type_info);
	if (ret < 0)
 ...
 */
static int wcd_cpe_vote(struct wcd_cpe_core *core, bool enable)
{
	int ret = 0;
	...
	dev_dbg(core->dev, "%s: enter, enable = %s, cpe_users = %u\n", __func__,
		(enable ? "true" : "false"), core->cpe_users);
	...
	if (enable) {
		...
		if (core->cpe_users == 0) {
			...
			dev_dbg(core->dev, "%s: %u valid users on cpe\n",
				__func__, core->cpe_users);
			goto done;
		}
	}

	dev_dbg(core->dev, "%s: leave, enable = %s, cpe_users = %u\n", __func__,
		(enable ? "true" : "false"), core->cpe_users);
...
static int wcd_cpe_debugfs_init(struct wcd_cpe_core *core)
{
	...
	if (!debugfs_create_u32("ramdump_enable", 0644, dir, &ramdump_enable)) {
	...
	if (!debugfs_create_file("cpe_ftm_test_trigger", 0200, dir, core,
	...
	if (!debugfs_create_u32("cpe_ftm_test_status", 0444, dir,
				&cpe_ftm_test_status)) {
		dev_err(core->dev, "%s: Failed to create debugfs node %s\n",
...
{
	return snprintf(buf, WCD_CPE_IMAGE_FNAME_MAX, "%s", core->dyn_fname);
}

static ssize_t fw_name_store(struct wcd_cpe_core *core, const char *buf,
			     ssize_t count)
{
	...
	if (copy_count > (WCD_CPE_IMAGE_FNAME_MAX - 1)) {
		dev_err(core->dev, "%s: Invalid length %d, max allowed %d\n",
...
static ssize_t wcd_cpe_sysfs_show(struct kobject *kobj, struct attribute *attr,
				  char *buf)
{
...
static ssize_t wcd_cpe_sysfs_store(struct kobject *kobj, struct attribute *attr,
...
static int wcd_cpe_sysfs_init(struct wcd_cpe_core *core, int id)
{
	...
	snprintf(sysfs_dir_name, WCD_CPE_SYSFS_DIR_MAX_LENGTH, "%s%d",
		 "wcd_cpe", id);

	rc = kobject_init_and_add(&core->cpe_kobj, &wcd_cpe_ktype, kernel_kobj,
				  sysfs_dir_name);
	if (unlikely(rc)) {
		dev_err(core->dev, "%s: Failed to add kobject %s, err = %d\n",
	...
	if (rc) {
		dev_err(core->dev, "%s: Failed to fw_name sysfs entry to %s\n",
				    ...
				    loff_t *ppos)
{
	...
	ret = wcd_cpe_enable_cpe_clks(core, true);
	if (ret < 0) {
		dev_err(core->dev, "%s: CPE clk enable failed, err = %d\n",
			__func__, ret);
		goto done;
	}
	...
	if (ret < 0) {
		dev_err(core->dev, "%s: CPE FTM test failed, err = %d\n",
			__func__, ret);
		if (ret == CPE_SVC_BUSY) {
	...
	if (ret < 0) {
		dev_err(core->dev, "%s: CPE clk disable failed, err = %d\n",
			__func__, ret);
	}
...
static int wcd_cpe_validate_params(struct snd_soc_component *component,
				   struct wcd_cpe_params *params)
{
	...
	if (!params) {
		dev_err(component->dev, "%s: No params supplied for codec %s\n",
			__func__, component->name);
		return -EINVAL;
	}

	if (!params->component || !params->get_cpe_core || !params->cdc_cb) {
		dev_err(component->dev, "%s: Invalid params for codec %s\n",
			__func__, component->name);
		return -EINVAL;
	}
				  ...
				  struct wcd_cpe_params *params)
{
	...
	memcpy(&core->irq_info, &params->cdc_irq_info, sizeof(core->irq_info));
   ...
   */
	...
	if (!core->cpe_handle) {
		dev_err(core->dev, "%s: failed to initialize cpe services\n",
			__func__);
		goto fail_cpe_initialize;
	}

	core->cpe_reg_handle = cpe_svc_register(
		core->cpe_handle, wcd_cpe_svc_event_cb,
		CPE_SVC_ONLINE | CPE_SVC_OFFLINE | CPE_SVC_BOOT |
			CPE_SVC_CMI_CLIENTS_DEREG,
		"codec cpe handler");
	if (!core->cpe_reg_handle) {
		dev_err(core->dev, "%s: failed to register cpe service\n",
			__func__);
		goto fail_cpe_register;
	}

	card = component->card->snd_card;
	snprintf(proc_name, (sizeof("cpe") + sizeof("_state") + sizeof(id) - 2),
	...
	if (!hw_info) {
		dev_err(core->dev, "%s: hw info not available\n", __func__);
	...
	core->cpe_ramdump_dev = create_ramdump_device("cpe", core->dev);
	if (!core->cpe_ramdump_dev) {
		dev_err(core->dev, "%s: Failed to create ramdump device\n",
			__func__);
		goto schedule_dload_work;
	}

	arch_setup_dma_ops(core->dev, 0, 0, NULL, 0);
	core->cpe_dump_v_addr =
		dma_alloc_coherent(core->dev, core->hw_info.dram_size,
				   &core->cpe_dump_addr, GFP_KERNEL);
...
static void wcd_cpe_cmi_lsm_callback(const struct cmi_api_notification *param)
{
	...
	if (hdr->opcode == CPE_CMI_BASIC_RSP_OPCODE) {
		...
		if (lsm_session->event_cb)
			lsm_session->event_cb(lsm_session->priv_d,
 ...
 */
static int wcd_cpe_cmi_send_lsm_msg(struct wcd_cpe_core *core,
				    ...
				    void *message)
{
	...
	if (unlikely(!wcd_cpe_is_online_state(core))) {
		dev_err(core->dev, "%s: MSG not sent, CPE offline\n", __func__);
		goto done;
	}
	...
	ret = wait_for_completion_timeout(&session->cmd_comp, CMI_CMD_TIMEOUT);
	if (ret > 0) {
		pr_debug("%s: command 0x%x, received response 0x%x\n", __func__,
			 hdr->opcode, session->cmd_err_code);
		...
		if (session->cmd_err_code > 0)
			pr_err("%s: CPE returned error[%s]\n", __func__,
			       cpe_err_get_err_str(session->cmd_err_code));
 ...
 *
 */
static int fill_cmi_header(struct cmi_hdr *hdr, u8 session_id, u8 service_id,
			   bool version, u8 payload_size, u16 opcode,
			   bool obm_flag)
{
	/* sanitize the data */
 ...
 */
static int fill_lsm_cmd_header_v0_inband(struct cmi_hdr *hdr, u8 session_id,
					 u8 payload_size, u16 opcode)
{
	return fill_cmi_header(hdr, session_id, CMI_CPE_LSM_SERVICE_ID, false,
...
static int wcd_cpe_cmd_lsm_open_tx_v2(struct wcd_cpe_core *core,
				      struct cpe_lsm_session *session)
{
	...
	if (core->cal_data[WCD_CPE_LSM_CAL_TOPOLOGY_ID] == NULL) {
		dev_err(core->dev, "%s: LSM_TOPOLOGY cal not allocated!\n",
			__func__);
		return -EINVAL;
	}
	...
	if (!top_cal) {
		dev_err(core->dev, "%s: Failed to get LSM TOPOLOGY cal block\n",
			__func__);
		ret = -EINVAL;
		goto unlock_cal_mutex;
	}

	lsm_top = (struct audio_cal_info_lsm_top *)top_cal->cal_info;

	if (!lsm_top) {
		dev_err(core->dev, "%s: cal_info for LSM_TOPOLOGY not found\n",
			__func__);
		ret = -EINVAL;
		goto unlock_cal_mutex;
	}
	...
	if (fill_lsm_cmd_header_v0_inband(&cmd_open_tx_v2.hdr, session->id,
					  OPEN_V2_CMD_PAYLOAD_SIZE,
				   ...
				   u16 sample_rate)
{
	...
	if (fill_lsm_cmd_header_v0_inband(&cmd_open_tx.hdr, session->id,
					  OPEN_CMD_PAYLOAD_SIZE,
	...
	if (ret)
		dev_err(core->dev, "%s: failed to send open_tx cmd, err = %d\n",
			__func__, ret);
end_ret:
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
done:
				    ...
				    struct cpe_lsm_session *session)
{
	...
	if (fill_lsm_cmd_header_v0_inband(&cmd_close_tx, session->id, 0,
					  CPE_LSM_SESSION_CMD_CLOSE_TX)) {
		ret = -EINVAL;
		goto end_ret;
	}
	...
	if (ret)
		dev_err(core->dev, "%s: lsm close_tx cmd failed, err = %d\n",
			__func__, ret);
	else
				       ...
				       u32 size)
{
	...
	if (fill_lsm_cmd_header_v0_inband(
		    &cmd_shmem_alloc.hdr, session->id, SHMEM_ALLOC_CMD_PLD_SIZE,
	...
	if (ret)
		dev_err(core->dev, "%s: lsm_shmem_alloc cmd send fail, %d\n",
			__func__, ret);
end_ret:
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
	return ret;
					 ...
					 struct cpe_lsm_session *session)
{
	...
	if (fill_lsm_cmd_header_v0_inband(
		    &cmd_dealloc.hdr, session->id, SHMEM_DEALLOC_CMD_PLD_SIZE,
	...
	if (ret) {
		dev_err(core->dev, "%s: lsm_shmem_dealloc cmd failed, rc %d\n",
			__func__, ret);
		goto end_ret;
	}

	memset(&session->lsm_mem_handle, 0, sizeof(session->lsm_mem_handle));
 ...
 */
static int wcd_cpe_send_lsm_cal(struct wcd_cpe_core *core,
				struct cpe_lsm_session *session)
{
	u8 *msg_pld;
	struct cmi_hdr *hdr;
	...
	memcpy(msg_pld, lsm_cal->cal_data.kvaddr, lsm_cal->cal_data.size);
				   ...
				   u32 set_param_cmd)
{
	...
	default:
		param_d->p_size.sr.param_size = (u16)p_size;
					...
					void *data, struct cpe_lsm_ids *ids)
{
	...
	struct cpe_param_data *param_d = &epd_cmd.param;
	int rc;

	memset(&epd_cmd, 0, sizeof(epd_cmd));
	ep_det_data = (struct snd_lsm_ep_det_thres *)data;
	if (fill_lsm_cmd_header_v0_inband(msg_hdr, session->id,
	...
	wcd_cpe_set_param_data(param_d, ids, CPE_EPD_THRES_PARAM_SIZE,
			       CPE_LSM_SESSION_CMD_SET_PARAMS_V2);
	...
	if (unlikely(rc))
		dev_err(core->dev,
			"%s: set_param(EPD Threshold) failed, rc %dn", __func__,
			rc);
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
err_ret:
	return rc;
				     ...
				     void *data, struct cpe_lsm_ids *ids)
{
	...
	struct cpe_param_data *param_d = &opmode_cmd.param;
	int rc;
	...
	wcd_cpe_set_param_data(param_d, ids, CPE_OPMODE_PARAM_SIZE,
			       CPE_LSM_SESSION_CMD_SET_PARAMS_V2);
				   ...
				   struct cpe_lsm_ids *ids)
{
	...
	struct cpe_param_data *param_d = &gain_cmd.param;
	int rc;
	...
	wcd_cpe_set_param_data(param_d, ids, CPE_GAIN_PARAM_SIZE,
			       CPE_LSM_SESSION_CMD_SET_PARAMS_V2);
	...
	if (unlikely(rc))
		dev_err(core->dev, "%s: set_param(lsm_gain) failed, rc %dn",
			__func__, rc);
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
err_ret:
	return rc;
					  ...
					  u16 port_id)
{
	...
	struct cpe_param_data *param_d = &con_port_cmd.param;
	int rc;
	...
	wcd_cpe_set_param_data(param_d, ids, CPE_CONNECTPORT_PARAM_SIZE,
			       CPE_LSM_SESSION_CMD_SET_PARAMS_V2);
	...
	if (unlikely(rc))
		dev_err(core->dev, "%s: set_param(connect_port) failed, rc %dn",
			__func__, rc);
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
err_ret:
	return rc;
...
static int wcd_cpe_send_param_conf_levels(struct wcd_cpe_core *core,
					  struct cpe_lsm_session *session,
					  struct cpe_lsm_ids *ids)
{
	...
	fill_cmi_header(hdr, session->id, CMI_CPE_LSM_SERVICE_ID, false,
			pld_size, CPE_LSM_SESSION_CMD_SET_PARAMS_V2, false);
	...
	memcpy(message, &conf_level_data, sizeof(struct cpe_lsm_conf_level));
					...
					struct cpe_lsm_ids *ids)
{
	...
	ret = fill_cmi_header(&obm_msg.hdr, session->id, CMI_CPE_LSM_SERVICE_ID,
			      0, 20, CPE_LSM_SESSION_CMD_SET_PARAMS_V2, true);
	if (ret) {
		dev_err(core->dev, "%s: Invalid parameters, rc = %d\n",
			__func__, ret);
		goto err_ret;
	}
	...
	if (ret)
		dev_err(core->dev, "%s: snd_model_register failed, %d\n",
			__func__, ret);
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
...
static int wcd_cpe_send_param_dereg_model(struct wcd_cpe_core *core,
					  struct cpe_lsm_session *session,
					  struct cpe_lsm_ids *ids)
{
	struct cmi_hdr *hdr;
	...
	param_d = (struct cpe_param_data *)(((u8 *)message) + sizeof(*hdr));

	if (fill_lsm_cmd_header_v0_inband(hdr, session->id, sizeof(*param_d),
					  CPE_LSM_SESSION_CMD_SET_PARAMS_V2)) {
		rc = -EINVAL;
		goto err_ret;
	}
	...
	if (rc)
		dev_err(core->dev, "%s: snd_model_deregister failed, %d\n",
			__func__, rc);
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
err_ret:
	kfree(message);
...
static int wcd_cpe_send_custom_param(struct wcd_cpe_core *core,
				     struct cpe_lsm_session *session,
				     void *data, u32 msg_size)
{
	...
	if (fill_lsm_cmd_header_v0_inband(hdr, session->id, msg_size,
					  CPE_LSM_SESSION_CMD_SET_PARAMS_V2)) {
		rc = -EINVAL;
		goto err_ret;
	}
	...
	if (rc)
		dev_err(core->dev, "%s: custom params send failed, err = %d\n",
			__func__, rc);
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
err_ret:
	kfree(msg);
				 ...
				 uint32_t param_type)
{
	...
	case LSM_ENDPOINT_DETECT_THRESHOLD:
		rc = wcd_cpe_send_param_epd_thres(core, session, data, &ids);
		break;
	...
	case LSM_CUSTOM_PARAMS:
		rc = wcd_cpe_send_custom_param(core, session, data,
					       p_info->param_size);
		break;
	default:
	...
	if (rc)
		dev_err(core->dev, "%s: send_param(%d) failed, err %d\n",
				  ...
				  bool detect_failure)
{
	...
	if (ret) {
		pr_err("%s: fail to sent acdb cal, err = %d", __func__, ret);
		goto err_ret;
	}
	...
	if (ret)
		dev_err(core->dev, "%s: Failed to set opmode, err=%d\n",
			__func__, ret);
				...
				bool detect_failure)
{
	...
	if (session->num_confidence_levels > 0) {
		...
		dev_dbg(core->dev, "%s: no conf levels to set\n", __func__);
	}
				     ...
				     bool detect_failure)
{
	...
	ret = wcd_cpe_lsm_set_data(core_handle, session, detect_mode,
				   detect_failure);
	if (ret) {
		dev_err(core->dev, "%s: fail to set lsm data, err = %d\n",
			__func__, ret);
		return ret;
	}
	...
	ret = fill_cmi_header(&obm_msg.hdr, session->id, CMI_CPE_LSM_SERVICE_ID,
			      0, 20, CPE_LSM_SESSION_CMD_REGISTER_SOUND_MODEL,
			      true);
	if (ret) {
		dev_err(core->dev, "%s: Invalid parameters, rc = %d\n",
			__func__, ret);
				       ...
				       struct cpe_lsm_session *session)
{
	...
	if (fill_lsm_cmd_header_v0_inband(
		    &cmd_dereg_snd_model, session->id, 0,
		    CPE_LSM_SESSION_CMD_DEREGISTER_SOUND_MODEL)) {
		ret = -EINVAL;
		goto end_ret;
	}
	...
	if (ret)
		dev_err(core->dev, "%s: failed to send dereg_snd_model cmd\n",
			__func__);
end_ret:
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
	return ret;
					   ...
					   struct cpe_lsm_session *session)
{
	...
	if (!core->cpe_cdc_cb || !core->cpe_cdc_cb->get_afe_out_port_id) {
	...
	if (rc) {
		dev_err(core->dev, "%s: failed to get port id, err = %d\n",
			__func__, rc);
		goto done;
	}
				...
				struct cpe_lsm_session *session)
{
	...
	if (ret)
		dev_err(core->dev, "%s: failed to send lsm_stop cmd\n",
			__func__);
end_ret:
	WCD_CPE_REL_LOCK(&session->lsm_lock, "lsm");
	return ret;
...
wcd_cpe_alloc_lsm_session(void *core_handle, void *client_data,
			  void (*event_cb)(void *, u8, u8, u8 *))
{
   ...
   */
	...
	if (ret) {
		dev_err(core->dev, "%s: Failed to enable cpe, err = %d\n",
			__func__, ret);
		WCD_CPE_REL_LOCK(&core->session_lock, "session_lock");
		return NULL;
	}
	...
	session->cmi_reg_handle =
		cmi_register(wcd_cpe_cmi_lsm_callback, CMI_CPE_LSM_SERVICE_ID);
 ...
 */
static int wcd_cpe_lsm_config_lab_latency(struct wcd_cpe_core *core,
					  struct cpe_lsm_session *session,
					  u32 latency)
{
	...
	wcd_cpe_set_param_data(param_d, &ids, PARAM_SIZE_LSM_LATENCY_SIZE,
			       CPE_LSM_SESSION_CMD_SET_PARAMS_V2);

	pr_debug("%s: Module 0x%x Param 0x%x size %zu pld_size 0x%x\n",
		 __func__, lab_lat->param.module_id, lab_lat->param.param_id,
		 PARAM_SIZE_LSM_LATENCY_SIZE, pld_size);
 ...
 */
static int wcd_cpe_lsm_lab_control(void *core_handle,
				   struct cpe_lsm_session *session, bool enable)
{
	struct wcd_cpe_core *core = core_handle;
	...
	wcd_cpe_set_param_data(param_d, &ids, PARAM_SIZE_LSM_CONTROL_SIZE,
			       CPE_LSM_SESSION_CMD_SET_PARAMS_V2);
 ...
 */
static int wcd_cpe_lsm_eob(struct wcd_cpe_core *core,
			   struct cpe_lsm_session *session)
{
	int ret = 0;
	struct cmi_hdr lab_eob;

	if (fill_lsm_cmd_header_v0_inband(&lab_eob, session->id, 0,
					  CPE_LSM_SESSION_CMD_EOB)) {
		return -EINVAL;
	}
				       ...
				       struct cpe_lsm_session *session)
{
	...
	WCD_CPE_GRAB_LOCK(&core->session_lock, "session_lock");
	if (!session) {
		dev_err(core->dev, "%s: Invalid lsm session\n", __func__);
		WCD_CPE_REL_LOCK(&core->session_lock, "session_lock");
		return -EINVAL;
	}
	...
	dev_dbg(core->dev, "%s: session %d being deallocated\n", __func__,
		session->id);
	if (session->id > WCD_CPE_LSM_MAX_SESSIONS) {
		dev_err(core->dev, "%s: Wrong session id %d max allowed = %d\n",
			__func__, session->id, WCD_CPE_LSM_MAX_SESSIONS);
		WCD_CPE_REL_LOCK(&core->session_lock, "session_lock");
		return -EINVAL;
	}
	...
	if (ret)
		dev_dbg(core->dev, "%s: Failed to un-vote cpe, err = %d\n",
			__func__, ret);
				...
				enum wcd_cpe_event event)
{
	...
	if (!core->cpe_cdc_cb || !core->cpe_cdc_cb->cdc_ext_clk ||
	    !core->cpe_cdc_cb->lab_cdc_ch_ctl) {
		dev_err(core->dev, "%s: Invalid codec callbacks\n", __func__);
		rc = -EINVAL;
		goto done;
	}

	component = core->component;
	dev_dbg(core->dev, "%s: event = 0x%x\n", __func__, event);
	...
	case WCD_CPE_PRE_ENABLE:
		...
		rc = core->cpe_cdc_cb->lab_cdc_ch_ctl(component, true);
		if (rc) {
			dev_err(core->dev,
				"%s: failed to enable cdc port, err = %d\n",
				__func__, rc);
			rc = core->cpe_cdc_cb->cdc_ext_clk(component, false,
							   false);
			goto done;
		}
	...
	case WCD_CPE_POST_ENABLE:
		...
		if (rc)
			dev_err(core->dev, "%s: Failed to enable lab\n",
				__func__);
		break;
     ...
     */
		...
		rc = core->cpe_cdc_cb->lab_cdc_ch_ctl(component, false);
		if (rc)
			dev_err(core->dev,
	...
	case WCD_CPE_POST_DISABLE:
		...
		if (rc)
			dev_err(core->dev, "%s: eob send failed, err = %d\n",
				__func__, rc);

		/* Continue teardown even if eob failed */
		rc = cpe_svc_toggle_lab(core->cpe_handle, false);
		if (rc)
			dev_err(core->dev, "%s: Failed to disable lab\n",
				__func__);
		...
		if (CPE_ERR_IRQ_CB(core))
			core->cpe_cdc_cb->cpe_err_irq_control(
				core->component, CPE_ERR_IRQ_UNMASK,
				&cpe_intr_bits);
		break;

	default:
		dev_err(core->dev, "%s: Invalid event 0x%x\n", __func__, event);
		rc = -EINVAL;
		break;
	}
				   ...
				   struct cpe_lsm_session *session)
{
	...
	if (fill_lsm_cmd_header_v0_inband(
		    &out_fmt_cfg.hdr, session->id, OUT_FMT_CFG_CMD_PAYLOAD_SIZE,
				...
				struct cpe_lsm_session *session, void *data)
{
	...
	ret = wcd_cpe_send_param_connectport(core, session, NULL, &ids,
					     port_id);
	if (ret)
		dev_err(core->dev,
...
static int fill_afe_cmd_header(struct cmi_hdr *hdr, u8 port_id, u16 opcode,
			       u8 pld_size, bool obm_flag)
{
 ...
 */
static int wcd_cpe_cmi_send_afe_msg(struct wcd_cpe_core *core,
 ...
 */
static int wcd_cpe_afe_shmem_alloc(struct wcd_cpe_core *core,
				   struct wcd_cmi_afe_port_data *port_d,
				   u32 size)
{
	struct cpe_cmd_shmem_alloc cmd_shmem_alloc;
	...
	pr_debug("%s: completed %s, mem_handle = 0x%x\n", __func__,
		 "CPE_AFE_CMD_SHARED_MEM_ALLOC", port_d->mem_handle);
 ...
 */
static int wcd_cpe_afe_shmem_dealloc(struct wcd_cpe_core *core,
				     struct wcd_cmi_afe_port_data *port_d)
{
	struct cpe_cmd_shmem_dealloc cmd_dealloc;
	int ret = 0;
	...
	memset(&port_d->mem_handle, 0, sizeof(port_d->mem_handle));
				...
				struct wcd_cmi_afe_port_data *port_d)
{
	...
	is_obm_msg = (afe_cal->cal_data.size > CMI_INBAND_MESSAGE_SIZE) ? true :
									  false;
	...
	if (is_obm_msg) {
		...
		if (rc) {
			dev_err(core->dev, "%s: AFE shmem alloc fail %d\n",
				__func__, rc);
			goto rel_cal_mutex;
		}
		...
		rc = fill_afe_cmd_header(hdr, port_d->port_id,
					 ...
					 CPE_AFE_PARAM_PAYLOAD_SIZE, true);
		if (rc) {
			dev_err(core->dev,
		...
		inb_msg =
			kzalloc(sizeof(struct cmi_hdr) + afe_cal->cal_data.size,
				GFP_KERNEL);
		if (!inb_msg) {
			dev_err(core->dev, "%s: no memory for afe cal inband\n",
				__func__);
			rc = -ENOMEM;
			goto rel_cal_mutex;
		}
		...
		rc = fill_afe_cmd_header(hdr, port_d->port_id,
					 CPE_AFE_CMD_SET_PARAM,
					 CPE_AFE_PARAM_PAYLOAD_SIZE, false);
		if (rc) {
			dev_err(core->dev,
				"%s: invalid params for header, err = %d\n",
				__func__, rc);
				 ...
				 const char *func)
{
	...
	if (afe_cfg->port_id > WCD_CPE_AFE_MAX_PORTS) {
		dev_err(core->dev, "%s: invalid afe port (%u)\n", func,
			afe_cfg->port_id);
		return -EINVAL;
	}

	dev_dbg(core->dev, "%s: port_id = %u\n", func, afe_cfg->port_id);
...
static int wcd_cpe_afe_svc_cmd_mode(void *core_handle, u8 mode)
{
   ...
   */
	...
	if (fill_afe_cmd_header(&afe_mode.hdr, afe_port_d->port_id,
				...
				CPE_AFE_CMD_MODE_PAYLOAD_SIZE, false)) {
		ret = -EINVAL;
		goto err_ret;
	}
	...
	if (ret)
		dev_err(core->dev, "%s: afe_svc_mode cmd failed, err = %d\n",
			__func__, ret);
				    ...
				    struct wcd_cpe_afe_port_cfg *afe_cfg)
{
	...
	if (fill_afe_cmd_header(&port_cfg_cmd.hdr, afe_cfg->port_id,
				CPE_AFE_PORT_CMD_GENERIC_CONFIG,
				CPE_AFE_CMD_PORT_CFG_PAYLOAD_SIZE, false)) {
		ret = -EINVAL;
		goto err_ret;
	}
	...
	if (afe_port_d->port_id == CPE_AFE_PORT_3_TX)
		...
		port_cfg_cmd.buffer_size = AFE_OUT_BUF_SIZE(
			afe_cfg->bit_width, afe_cfg->sample_rate);

	ret = wcd_cpe_cmi_send_afe_msg(core, afe_port_d, &port_cfg_cmd);
	if (ret)
		dev_err(core->dev, "%s: afe_port_config failed, err = %d\n",
			__func__, ret);
				  ...
				  bool afe_mad_ctl)
{
	...
	if (ret) {
		dev_err(core->dev, "%s: afe acdb cal send failed, err = %d\n",
			__func__, ret);
		goto err_ret;
	}
	...
	if (fill_afe_cmd_header(&afe_params.hdr, afe_cfg->port_id,
				CPE_AFE_CMD_SET_PARAM, (u8)pld_size, false)) {
		ret = -EINVAL;
		goto err_ret;
	}
				  ...
				  struct wcd_cpe_afe_port_cfg *port_cfg)
{
	...
	fill_afe_cmd_header(&hdr, port_cfg->port_id, CPE_AFE_PORT_CMD_START, 0,
			    false);
	ret = wcd_cpe_cmi_send_afe_msg(core, afe_port_d, &hdr);
	if (ret)
		dev_err(core->dev, "%s: afe_port_start cmd failed, err = %d\n",
			__func__, ret);
	WCD_CPE_REL_LOCK(&afe_port_d->afe_lock, "afe");
	return ret;
}
				 ...
				 struct wcd_cpe_afe_port_cfg *port_cfg)
{
	...
	fill_afe_cmd_header(&hdr, port_cfg->port_id, CPE_AFE_PORT_CMD_STOP, 0,
			    false);
	ret = wcd_cpe_cmi_send_afe_msg(core, afe_port_d, &hdr);
	if (ret)
		dev_err(core->dev, "%s: afe_stop cmd failed, err = %d\n",
			__func__, ret);
				    ...
				    struct wcd_cpe_afe_port_cfg *port_cfg)
{
	...
	memset(&hdr, 0, sizeof(struct cmi_hdr));
	fill_afe_cmd_header(&hdr, port_cfg->port_id, CPE_AFE_PORT_CMD_SUSPEND,
			    0, false);
	ret = wcd_cpe_cmi_send_afe_msg(core, afe_port_d, &hdr);
	if (ret)
		dev_err(core->dev, "%s: afe_suspend cmd failed, err = %d\n",
			__func__, ret);
	WCD_CPE_REL_LOCK(&afe_port_d->afe_lock, "afe");
	return ret;
}
				   ...
				   struct wcd_cpe_afe_port_cfg *port_cfg)
{
	...
	memset(&hdr, 0, sizeof(hdr));
	fill_afe_cmd_header(&hdr, port_cfg->port_id, CPE_AFE_PORT_CMD_RESUME, 0,
			    false);
	ret = wcd_cpe_cmi_send_afe_msg(core, afe_port_d, &hdr);
	if (ret)
		dev_err(core->dev, "%s: afe_resume cmd failed, err = %d\n",
			__func__, ret);
	WCD_CPE_REL_LOCK(&afe_port_d->afe_lock, "afe");
	return ret;
}
