...
extern const u8 rouleur_reg_access_analog[ROULEUR_REG(
	ROULEUR_ANALOG_REGISTERS_MAX_SIZE)];
extern const u8 rouleur_reg_access_digital[ROULEUR_REG(
...
{
	if (reg > ROULEUR_ANA_BASE_ADDR &&
	    reg < ROULEUR_ANALOG_REGISTERS_MAX_SIZE)
		return rouleur_reg_access_analog[ROULEUR_REG(reg)] & RD_REG;
	if (reg > ROULEUR_DIG_BASE_ADDR &&
	    reg < ROULEUR_DIGITAL_REGISTERS_MAX_SIZE)
...
struct regmap_config rouleur_regmap_config = {
	...
	.max_register =
		ROULEUR_ANALOG_MAX_REGISTER + ROULEUR_DIGITAL_MAX_REGISTER,
