				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rc >= 0) {
		...
		dev_dbg(component->dev,
			"%s: data_blk_size:%d, spk_alloc_data_blk_size:%d\n",
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!codec_data->ext_disp_ops.get_audio_edid_blk ||
		...
		dev_err_ratelimited(
			component->dev,
			"%s: get_audio_edid_blk() or get_intf_id is NULL\n",
			__func__);
		rc = -EINVAL;
		goto cable_err;
	}
	...
	if (cable_state < 0) {
		dev_err_ratelimited(
			component->dev,
			"%s: Error retrieving cable state from ext_disp, err:%d\n",
	...
	if (disp_type >= 0) {
		...
		dev_err_ratelimited(
			component->dev,
			"%s: Error retrieving disp_type from ext_disp, err:%d\n",
			__func__, disp_type);
		rc = disp_type;
	}
	return rc;
					 ...
					 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct msm_ext_disp_audio_codec_rx_data *codec_data;
	int rc = 0;
	int dai_id = ((struct soc_multi_mixer_control *)kcontrol->private_value)
			     ->shift;

	if (dai_id < 0 || dai_id > DP_DAI2) {
		dev_err_ratelimited(component->dev, "%s: invalid dai id: %d\n",
				    __func__, dai_id);
		rc = -EINVAL;
		goto done;
	}
...
static const struct snd_kcontrol_new msm_ext_disp_codec_rx_controls[] = {
	...
	SOC_ENUM_EXT("External Display Type", ext_disp_audio_type1,
		     msm_ext_disp_audio_type_get, NULL),
	SOC_ENUM_EXT("External Display1 Type", ext_disp_audio_type2,
		     msm_ext_disp_audio_type_get, NULL),
	SOC_ENUM_EXT("External HDMI Type", ext_disp_audio_type3,
		     msm_ext_disp_audio_type_get, NULL),
	SOC_ENUM_EXT("External Display Audio Ack", ext_disp_audio_ack_state1,
		     NULL, msm_ext_disp_audio_ack_set),
	SOC_ENUM_EXT("External Display1 Audio Ack", ext_disp_audio_ack_state2,
		     NULL, msm_ext_disp_audio_ack_set),
	SOC_ENUM_EXT("External HDMI Audio Ack", ext_disp_audio_ack_state3, NULL,
		     msm_ext_disp_audio_ack_set),

	SOC_SINGLE_MULTI_EXT("External Display Audio Device", SND_SOC_NOPM,
			     DP_DAI1, DP_STREAM_MAX - 1, 0, 2,
			     msm_ext_disp_audio_device_get,
			     msm_ext_disp_audio_device_set),
	SOC_SINGLE_MULTI_EXT("External Display1 Audio Device", SND_SOC_NOPM,
			     DP_DAI2, DP_STREAM_MAX - 1, 0, 2,
			     msm_ext_disp_audio_device_get,
			     msm_ext_disp_audio_device_set),
	SOC_SINGLE_MULTI_EXT("External HDMI Device", SND_SOC_NOPM, HDMI_MS_DAI,
			     DP_STREAM_MAX - 1, 0, 2,
			     msm_ext_disp_audio_device_get,
			     msm_ext_disp_audio_device_set),
...
static int
msm_ext_disp_audio_codec_rx_dai_startup(struct snd_pcm_substream *substream,
					struct snd_soc_dai *dai)
{
	...
	dev_dbg(dai->component->dev, "%s: DP ctl id %d Stream id %d\n",
		__func__, codec_data->ctl[dai->id],
		codec_data->stream[dai->id]);
...
static int
msm_ext_disp_audio_codec_rx_dai_hw_params(struct snd_pcm_substream *substream,
...
static void
msm_ext_disp_audio_codec_rx_dai_shutdown(struct snd_pcm_substream *substream,
					 struct snd_soc_dai *dai)
{
	int rc = 0;
	struct msm_ext_disp_codec_id codec_info;
	...
	if (!codec_data->ext_disp_ops.teardown_done ||
		...
		dev_err_ratelimited(
			dai->dev, "%s: teardown_done or cable_status is null\n",
			__func__);
		mutex_unlock(&codec_data->dp_ops_lock);
		return;
	}
	...
	codec_data->ext_disp_ops.teardown_done(codec_data->ext_disp_core_pdev);
	mutex_unlock(&codec_data->dp_ops_lock);
}

static int
msm_ext_disp_audio_codec_rx_probe(struct snd_soc_component *component)
{
	struct msm_ext_disp_audio_codec_rx_data *codec_data;
	...
	if (!of_node_parent) {
		dev_err(component->dev,
			"%s(): Parent device tree node not found\n", __func__);
		kfree(codec_data);
		return -ENODEV;
	}
	...
	if (msm_ext_disp_register_audio_codec(codec_data->ext_disp_core_pdev,
		...
		dev_err(component->dev,
			"%s(): can't register with ext disp core", __func__);
		kfree(codec_data);
		return -ENODEV;
	}
...
static void
msm_ext_disp_audio_codec_rx_remove(struct snd_soc_component *component)
{
	struct msm_ext_disp_audio_codec_rx_data *codec_data;
...
static int msm_ext_disp_audio_codec_rx_plat_probe(struct platform_device *pdev)
{
	dev_dbg(&pdev->dev, "%s(): dev name %s\n", __func__,
		dev_name(&pdev->dev));

	return snd_soc_register_component(
		&pdev->dev, &msm_ext_disp_codec_rx_driver,
		msm_ext_disp_audio_codec_rx_dais,
		ARRAY_SIZE(msm_ext_disp_audio_codec_rx_dais));
}

static int msm_ext_disp_audio_codec_rx_plat_remove(struct platform_device *pdev)
{
