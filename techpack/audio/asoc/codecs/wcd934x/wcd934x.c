...
#define WCD934X_FORMATS_S16_S24_S32_LE                       \
	(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | \
...
static int dig_core_collapse_timer =
	(WCD934X_DIG_CORE_COLLAPSE_TIMER_MS / 1000);
...
#define TAVIL_HPH_REG_RANGE_2 \
	(WCD934X_HPH_NEW_ANA_HPH3 - WCD934X_HPH_NEW_ANA_HPH2 + 1)
#define TAVIL_HPH_REG_RANGE_3                \
	(WCD934X_HPH_NEW_INT_PA_RDAC_MISC3 - \
	 WCD934X_HPH_NEW_INT_RDAC_GAIN_CTL + 1)
#define TAVIL_HPH_TOTAL_REG \
	(TAVIL_HPH_REG_RANGE_1 + TAVIL_HPH_REG_RANGE_2 + TAVIL_HPH_REG_RANGE_3)
...
static struct interp_sample_rate sr_val_tbl[] = {
	{ 8000, 0x0 },	{ 16000, 0x1 },	 { 32000, 0x3 },  { 48000, 0x4 },
	{ 96000, 0x5 }, { 192000, 0x6 }, { 384000, 0x7 }, { 44100, 0x9 },
	{ 88200, 0xA }, { 176400, 0xB }, { 352800, 0xC },
};
...
static struct afe_param_cdc_reg_cfg audio_reg_cfg[] = {
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SOC_MAD_MAIN_CTL_1),
	  HW_MAD_AUDIO_ENABLE, 0x1, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SOC_MAD_AUDIO_CTL_3),
	  HW_MAD_AUDIO_SLEEP_TIME, 0xF, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SOC_MAD_AUDIO_CTL_4),
	  HW_MAD_TX_AUDIO_SWITCH_OFF, 0x1, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_INTR_CFG),
	  MAD_AUDIO_INT_DEST_SELECT_REG, 0x2, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_INTR_PIN2_MASK3),
	  MAD_AUDIO_INT_MASK_REG, 0x1, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_INTR_PIN2_STATUS3),
	  MAD_AUDIO_INT_STATUS_REG, 0x1, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_INTR_PIN2_CLEAR3),
	  MAD_AUDIO_INT_CLEAR_REG, 0x1, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SB_PGD_PORT_TX_BASE),
	  SB_PGD_PORT_TX_WATERMARK_N, 0x1E, WCD934X_REG_BITS, 0x1 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SB_PGD_PORT_TX_BASE),
	  SB_PGD_PORT_TX_ENABLE_N, 0x1, WCD934X_REG_BITS, 0x1 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SB_PGD_PORT_RX_BASE),
	  SB_PGD_PORT_RX_WATERMARK_N, 0x1E, WCD934X_REG_BITS, 0x1 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_SB_PGD_PORT_RX_BASE),
	  SB_PGD_PORT_RX_ENABLE_N, 0x1, WCD934X_REG_BITS, 0x1 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_CDC_ANC0_IIR_ADAPT_CTL),
	  AANC_FF_GAIN_ADAPTIVE, 0x4, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_CDC_ANC0_IIR_ADAPT_CTL),
	  AANC_FFGAIN_ADAPTIVE_EN, 0x8, WCD934X_REG_BITS, 0 },
	{ 1, (WCD934X_REGISTER_START_OFFSET + WCD934X_CDC_ANC0_FF_A_GAIN_CTL),
	  AANC_GAIN_CONTROL, 0xFF, WCD934X_REG_BITS, 0 },
	{ 1,
	  (WCD934X_REGISTER_START_OFFSET + SB_PGD_TX_PORT_MULTI_CHANNEL_0(0)),
	  SB_PGD_TX_PORTn_MULTI_CHNL_0, 0xFF, WCD934X_REG_BITS, 0x4 },
	{ 1,
	  (WCD934X_REGISTER_START_OFFSET + SB_PGD_TX_PORT_MULTI_CHANNEL_1(0)),
...
struct tavil_priv {
	...
	struct platform_device *pdev_child_devices[WCD934X_CHILD_DEVICES_MAX];
	int child_count;
...
{
	if ((dai_id == AIF1_PB) || (dai_id == AIF2_PB) || (dai_id == AIF3_PB) ||
	    (dai_id == AIF4_PB))
		return true;
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (hwdep_cal) {
			...
			dev_dbg(component->dev,
				"%s: using hwdep calibration, cal_size %zd",
				__func__, cal_size);
		} else {
			...
			if (ret < 0) {
				dev_err(component->dev,
					"%s: Failed to acquire ANC data: %d\n",
					__func__, ret);
				return ret;
			}
			if (!fw) {
				dev_err(component->dev,
					"%s: Failed to get anc fw\n", __func__);
				return -ENODEV;
			}
			data = fw->data;
			cal_size = fw->size;
		...
		for (i = 0; i < num_anc_slots; i++) {
			...
			anc_size_remaining -=
				(anc_writes_size * WCD934X_PACKED_REG_SIZE);
			anc_ptr += anc_writes_size;
		}
		if (i == num_anc_slots) {
			dev_err(component->dev,
				"%s: Selected ANC slot not present\n",
				__func__);
			ret = -EINVAL;
			goto err;
		}
		...
		if (!strcmp(w->name, "RX INT0 DAC") ||
			...
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_RC_COMMON_CTL, 0x05,
				0x05);
			if (!strcmp(w->name, "RX INT1 DAC")) {
				snd_soc_component_update_bits(
					component,
					WCD934X_CDC_ANC0_FIFO_COMMON_CTL, 0x66,
					0x66);
			}
		} else if (!strcmp(w->name, "RX INT2 DAC")) {
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_RC_COMMON_CTL, 0x05,
				0x05);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_FIFO_COMMON_CTL,
				0x66, 0x66);
		}
		if (!strcmp(w->name, "RX INT1 DAC"))
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_CLK_RESET_CTL, 0x08,
				0x08);
		else if (!strcmp(w->name, "RX INT2 DAC"))
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_CLK_RESET_CTL, 0x08,
				0x08);
	...
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(
			component, WCD934X_CDC_ANC0_RC_COMMON_CTL, 0x05, 0x00);
		if (!strcmp(w->name, "ANC EAR PA") ||
		    !strcmp(w->name, "ANC SPK1 PA") ||
		    !strcmp(w->name, "ANC HPHL PA")) {
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_MODE_1_CTL, 0x30,
				0x00);
			msleep(50);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_MODE_1_CTL, 0x01,
				0x00);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_CLK_RESET_CTL, 0x38,
				0x38);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_CLK_RESET_CTL, 0x07,
				0x00);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC0_CLK_RESET_CTL, 0x38,
				0x00);
			...
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_MODE_1_CTL, 0x30,
				0x00);
			msleep(50);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_MODE_1_CTL, 0x01,
				0x00);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_CLK_RESET_CTL, 0x38,
				0x38);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_CLK_RESET_CTL, 0x07,
				0x00);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_ANC1_CLK_RESET_CTL, 0x38,
				0x00);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (enable) {
		if (port_id == WCD934X_TX14 &&
		    !test_bit(VI_SENSE_1, &tavil_p->status_mask)) {
			list_add_tail(&core->tx_chs[WCD934X_TX14].list,
				      &tavil_p->dai[dai_id].wcd9xxx_ch_list);
			set_bit(VI_SENSE_1, &tavil_p->status_mask);
		}
		if (port_id == WCD934X_TX15 &&
		    !test_bit(VI_SENSE_2, &tavil_p->status_mask)) {
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	case AIF3_CAP:
		...
		if (enable && !(tavil_p->tx_port_value & 1 << port_id)) {
			...
			list_add_tail(&core->tx_chs[port_id].list,
				      &tavil_p->dai[dai_id].wcd9xxx_ch_list);
		} else if (!enable && (tavil_p->tx_port_value & 1 << port_id)) {
			tavil_p->tx_port_value &= ~(1 << port_id);
	...
	dev_dbg(component->dev,
		"%s: name %s sname %s updated value %u shift %d\n", __func__,
		widget->name, widget->sname, tavil_p->tx_port_value,
		widget->shift);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	case 1:
		if (wcd9xxx_rx_vport_validation(
			    port_id + WCD934X_RX_PORT_START_NUMBER,
...
static void
tavil_codec_enable_slim_port_intr(struct wcd9xxx_codec_dai_data *dai,
				  struct snd_soc_component *component)
{
	struct wcd9xxx_ch *ch;
	int port_num = 0;
					  ...
					  bool up)
{
	...
	if (up) {
		...
		ret = wait_event_timeout(
			...
			msecs_to_jiffies(WCD934X_SLIM_CLOSE_TIMEOUT));
		if (!ret) {
				 ...
				 struct list_head *ch_list)
{
	...
	dsd0_in = (snd_soc_component_read32(component, WCD934X_CDC_DSD0_CFG0) &
		   0x3C) >>
		  2;
	dsd1_in = (snd_soc_component_read32(component, WCD934X_CDC_DSD1_CFG0) &
		   0x3C) >>
		  2;
...
static int tavil_codec_set_i2s_rx_ch(struct snd_soc_dapm_widget *w, u32 i2s_reg,
				     bool up)
{
	...
	if (up) {
		...
		snd_soc_component_update_bits(component, i2s_reg, 0x40,
					      i2s_bit_mode << 6);
		snd_soc_component_update_bits(component, i2s_reg, 0x3c,
					      (rx_fs_rate << 2));
	} else {
...
static int tavil_codec_set_i2s_tx_ch(struct snd_soc_dapm_widget *w, u32 i2s_reg,
				     bool up)
{
	...
	if (up) {
		...
		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX0_CFG, 0x03, 0x01);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX0_CFG, 0x0C, 0x01);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX1_0_CFG, 0x03, 0x01);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX1_1_CFG, 0x05, 0x05);
	} else {
		snd_soc_component_update_bits(component, i2s_reg, 0x40, 0x00);
		snd_soc_component_update_bits(component, i2s_reg, 0x3c, 0x00);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX0_CFG, 0x03, 0x00);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX0_CFG, 0x0C, 0x00);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX1_0_CFG, 0x03, 0x00);

		snd_soc_component_update_bits(
			component, WCD934X_DATA_HUB_I2S_TX1_1_CFG, 0x05, 0x00);
	}
	return 0;
}
				 ...
				 struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(component->dev,
		"%s: event called! codec name %s num_dai %d\n"
					      ...
					      int event)
{
	...
	dev_dbg(component->dev,
		...
		__func__, component->num_dai, w->sname, w->name, event,
		w->shift);
	...
	case SND_SOC_DAPM_POST_PMU:
		if (test_bit(VI_SENSE_1, &tavil_p->status_mask)) {
			...
			snd_soc_component_update_bits(
				...
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_TX10_SPKR_PROT_PATH_CTL,
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_TX9_SPKR_PROT_PATH_CTL,
				0x0F, 0x00);
		...
		if (test_bit(VI_SENSE_2, &tavil_p->status_mask)) {
			...
			snd_soc_component_update_bits(
				...
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_TX12_SPKR_PROT_PATH_CTL,
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_TX11_SPKR_PROT_PATH_CTL,
				0x0F, 0x00);
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (tavil->rx_bias_count == 1) {
			snd_soc_component_update_bits(
				component, WCD934X_ANA_RX_SUPPLIES, 0x01, 0x01);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		tavil->rx_bias_count--;
		if (!tavil->rx_bias_count)
			snd_soc_component_update_bits(
				component, WCD934X_ANA_RX_SUPPLIES, 0x01, 0x00);
		break;
	};
				       ...
				       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX7_RX_PATH_CFG0, 0x10, 0x00);
		ret = tavil_codec_enable_anc(w, kcontrol, event);
		break;
	}
	return ret;
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
     ...
     */
		...
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX0_RX_PATH_CTL, 0x10, 0x00);
		/* Remove mix path mute if it is enabled */
		if ((snd_soc_component_read32(
			    component, WCD934X_CDC_RX0_RX_PATH_MIX_CTL)) &
		    0x10)
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX0_RX_PATH_MIX_CTL,
				0x10, 0x00);
     ...
     */
		...
		if (!(strcmp(w->name, "ANC EAR PA"))) {
			ret = tavil_codec_enable_anc(w, kcontrol, event);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX0_RX_PATH_CFG0, 0x10,
				0x00);
		}
		break;
	};
...
static void tavil_codec_clear_anc_tx_hold(struct tavil_priv *tavil)
{
	if (test_and_clear_bit(ANC_MIC_AMIC1, &tavil->status_mask))
		tavil_codec_set_tx_hold(tavil->component, WCD934X_ANA_AMIC1,
					false);
	if (test_and_clear_bit(ANC_MIC_AMIC2, &tavil->status_mask))
		tavil_codec_set_tx_hold(tavil->component, WCD934X_ANA_AMIC2,
					false);
	if (test_and_clear_bit(ANC_MIC_AMIC3, &tavil->status_mask))
		tavil_codec_set_tx_hold(tavil->component, WCD934X_ANA_AMIC3,
					false);
	if (test_and_clear_bit(ANC_MIC_AMIC4, &tavil->status_mask))
		tavil_codec_set_tx_hold(tavil->component, WCD934X_ANA_AMIC4,
					false);
}
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (TAVIL_IS_1_0(tavil->wcd9xxx))
			snd_soc_component_update_bits(
				component, WCD934X_HPH_REFBUFF_LP_CTL, 0x06,
				(0x03 << 1));
		...
		if (dsd_conf && (snd_soc_component_read32(
					 component, WCD934X_CDC_DSD1_PATH_CTL) &
				 0x01)) {
     ...
     */
		...
		/* Remove mute */
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX2_RX_PATH_CTL, 0x10, 0x00);
		/* Enable GM3 boost */
		snd_soc_component_update_bits(component, WCD934X_HPH_CNP_WG_CTL,
					      0x80, 0x80);
		/* Enable AutoChop timer at the end of power up */
		snd_soc_component_update_bits(
			component, WCD934X_HPH_NEW_INT_HPH_TIMER1, 0x02, 0x02);
		/* Remove mix path mute if it is enabled */
		if ((snd_soc_component_read32(
			    component, WCD934X_CDC_RX2_RX_PATH_MIX_CTL)) &
		    0x10)
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX2_RX_PATH_MIX_CTL,
				0x10, 0x00);
		if (dsd_conf && (snd_soc_component_read32(
				 ...
				 0x01))
			snd_soc_component_update_bits(
				component, WCD934X_CDC_DSD1_CFG2, 0x04, 0x00);
		if (!(strcmp(w->name, "ANC HPHR PA"))) {
			pr_debug("%s:Do everything needed for left channel\n",
				 __func__);
			/* Do everything needed for left channel */
			snd_soc_component_update_bits(
				component, WCD934X_HPH_L_TEST, 0x01, 0x01);

			/* Remove mute */
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX1_RX_PATH_CTL, 0x10,
				0x00);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		snd_soc_component_update_bits(component, WCD934X_HPH_R_TEST,
					      0x01, 0x00);
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX2_RX_PATH_CTL, 0x10, 0x10);
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX2_RX_PATH_MIX_CTL, 0x10, 0x10);
		if (!(strcmp(w->name, "ANC HPHR PA")))
			snd_soc_component_update_bits(
				component, WCD934X_ANA_HPH, 0x40, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/*
     ...
     */
		...
		if (!(strcmp(w->name, "ANC HPHR PA"))) {
			ret = tavil_codec_enable_anc(w, kcontrol, event);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX2_RX_PATH_CFG0, 0x10,
				0x00);
		}
		tavil_ocp_control(component, true);
		break;
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
     ...
     */
		...
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX1_RX_PATH_CTL, 0x10, 0x00);
		/* Enable GM3 boost */
		snd_soc_component_update_bits(component, WCD934X_HPH_CNP_WG_CTL,
					      0x80, 0x80);
		/* Enable AutoChop timer at the end of power up */
		...
		/* Remove mix path mute if it is enabled */
		if ((snd_soc_component_read32(
			    component, WCD934X_CDC_RX1_RX_PATH_MIX_CTL)) &
		    0x10)
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX1_RX_PATH_MIX_CTL,
				0x10, 0x00);
		if (dsd_conf && (snd_soc_component_read32(
				 ...
				 0x01))
			snd_soc_component_update_bits(
				component, WCD934X_CDC_DSD0_CFG2, 0x04, 0x00);
		if (!(strcmp(w->name, "ANC HPHL PA"))) {
			...
			snd_soc_component_update_bits(
				component, WCD934X_HPH_R_TEST, 0x01, 0x01);

			/* Remove mute */
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX2_RX_PATH_CTL, 0x10,
				0x00);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		snd_soc_component_update_bits(component, WCD934X_HPH_L_TEST,
					      0x01, 0x00);
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX1_RX_PATH_CTL, 0x10, 0x10);
		snd_soc_component_update_bits(
			component, WCD934X_CDC_RX1_RX_PATH_MIX_CTL, 0x10, 0x10);
		if (!(strcmp(w->name, "ANC HPHL PA")))
			snd_soc_component_update_bits(
				component, WCD934X_ANA_HPH, 0x80, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/*
     * 5ms sleep is required after PA disable. If compander is
     ...
     */
		...
		if (!(strcmp(w->name, "ANC HPHL PA"))) {
			ret = tavil_codec_enable_anc(w, kcontrol, event);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX1_RX_PATH_CFG0, 0x10,
				0x00);
		}
		tavil_ocp_control(component, true);
		break;
					 ...
					 int event)
{
     ...
     */
		...
		if (dsd_conf &&
		    (snd_soc_component_read32(component, dsd_clk_reg) & 0x01))
				       ...
				       struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		ret = snd_soc_component_update_bits(component, i2s_reg, 0x01,
						    0x01);
		break;
	case SND_SOC_DAPM_POST_PMD:
		ret = snd_soc_component_update_bits(component, i2s_reg, 0x01,
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, WCD934X_HPH_NEW_INT_HPH_TIMER1, 0x02, 0x00);

		if (tavil->anc_func)
			ret = tavil_codec_enable_anc(w, kcontrol, event);

		wcd_clsh_fsm(component, &tavil->clsh_d, WCD_CLSH_EVENT_PRE_DAC,
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		wcd_clsh_fsm(component, &tavil->clsh_d, WCD_CLSH_EVENT_POST_PA,
			     WCD_CLSH_STATE_EAR, CLS_H_NORMAL);
		break;
	default:
		break;
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		dem_inp = snd_soc_component_read32(
				  component, WCD934X_CDC_RX2_RX_PATH_SEC0) &
			  0x03;
		...
		if ((hph_mode != CLS_H_LP) && (hph_mode != CLS_H_ULP))
			...
			snd_soc_component_update_bits(
				component, WCD934X_SIDO_NEW_VOUT_D_FREQ2, 0x01,
				0x01);
		/* Disable AutoChop timer during power up */
		snd_soc_component_update_bits(
			component, WCD934X_HPH_NEW_INT_HPH_TIMER1, 0x02, 0x00);
		...
		if (TAVIL_IS_1_0(tavil->wcd9xxx))
			snd_soc_component_update_bits(
				component, WCD934X_HPH_NEW_INT_RDAC_GAIN_CTL,
				0xF0, 0x40);
		if (dsd_conf && (snd_soc_component_read32(
					 component, WCD934X_CDC_DSD1_PATH_CTL) &
				 0x01))
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* Read DEM INP Select */
		dem_inp = snd_soc_component_read32(
				  component, WCD934X_CDC_RX1_RX_PATH_SEC0) &
			  0x03;
		if (((hph_mode == CLS_H_HIFI) || (hph_mode == CLS_H_LOHIFI) ||
		     (hph_mode == CLS_H_LP)) &&
		    (dem_inp != 0x01)) {
		...
		ret = tavil_mbhc_get_impedance(tavil->mbhc, &impedl, &impedr);
		if (!ret) {
...
static int __tavil_codec_enable_swr(struct snd_soc_dapm_widget *w, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (((strnstr(w->name, "INT7_", sizeof("RX INT7_"))) ||
		     (strnstr(w->name, "INT7 MIX2", sizeof("RX INT7 MIX2")))))
		...
		if (wcd9xxx_get_current_power_state(
			    tavil->wcd9xxx, WCD9XXX_DIG_CORE_REGION_1) !=
		    WCD_REGION_POWER_COLLAPSE_REMOVE)
			goto done;

		swrm_wcd_notify(tavil->swr.ctrl_data[0].swr_pdev, SWR_DEVICE_UP,
				NULL);
...
static int tavil_codec_config_mad(struct snd_soc_component *component)
{
	...
	snd_soc_component_update_bits(
		component, WCD934X_SOC_MAD_MAIN_CTL_1, 0xFF << 3,
	...
	snd_soc_component_update_bits(
		component, WCD934X_SOC_MAD_AUDIO_CTL_2, 0x03 << 2,
	...
	for (idx = 0; idx < ARRAY_SIZE(mad_cal->audio_info.iir_coefficients);
		...
		snd_soc_component_write(
			component, WCD934X_SOC_MAD_AUDIO_IIR_CTL_VAL,
	...
	snd_soc_component_update_bits(
		component, WCD934X_SOC_MAD_BEACON_CTL_2, 0x03 << 2,
	...
	for (idx = 0; idx < ARRAY_SIZE(mad_cal->beacon_info.iir_coefficients);
	     idx++) {
		snd_soc_component_update_bits(
			component, WCD934X_SOC_MAD_BEACON_IIR_CTL_PTR, 0x3F,
			idx);
		snd_soc_component_write(
			component, WCD934X_SOC_MAD_BEACON_IIR_CTL_VAL,
	...
	/* Ultrasound */
	snd_soc_component_update_bits(
		component, WCD934X_SOC_MAD_ULTR_CTL_1, 0x07 << 4,
		mad_cal->ultrasound_info.rms_comp_time << 4);
	snd_soc_component_update_bits(
		component, WCD934X_SOC_MAD_ULTR_CTL_2, 0x03 << 2,
				    ...
				    bool enable)
{
	...
	if (snd_soc_component_read32(component, WCD934X_CPE_SS_SVA_CFG) &
	    0x04) {
	...
	if (enable) {
		snd_soc_component_update_bits(
			component, WCD934X_SOC_MAD_AUDIO_CTL_2, 0x03, 0x03);
		rc = tavil_codec_config_mad(component);
		if (rc < 0) {
			snd_soc_component_update_bits(
				component, WCD934X_SOC_MAD_AUDIO_CTL_2, 0x03,
				0x00);
			goto done;
		}

		/* Turn on MAD clk */
		snd_soc_component_update_bits(component, WCD934X_CPE_SS_MAD_CTL,
					      0x01, 0x01);
		...
		snd_soc_component_update_bits(
			component, WCD934X_SOC_MAD_AUDIO_CTL_2, 0x03, 0x00);
		/* Reset the MAD block */
		snd_soc_component_update_bits(component, WCD934X_CPE_SS_MAD_CTL,
					      0x02, 0x02);
		snd_soc_component_update_bits(component, WCD934X_CPE_SS_MAD_CTL,
				   ...
				   struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(tavil->dev, "%s: event = %d, mad_switch_cnt = %d\n", __func__,
		event, tavil->mad_switch_cnt);
	return rc;
}

static int tavil_get_asrc_mode(struct tavil_priv *tavil, int asrc, u8 main_sr,
			       u8 mix_sr)
{
	u8 asrc_output_mode;
				   ...
				   int asrc_in, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (tavil->asrc_users[asrc] == 0) {
			if ((snd_soc_component_read32(component, clk_reg) &
			     0x02) ||
			    (snd_soc_component_read32(component, paired_reg) &
			     0x02)) {
				snd_soc_component_update_bits(
			...
			snd_soc_component_update_bits(component, cfg_reg, 0x80,
						      0x80);
			snd_soc_component_update_bits(component, clk_reg, 0x01,
						      0x01);
			main_sr = snd_soc_component_read32(component, ctl_reg) &
				  0x0F;
			mix_ctl_reg = ctl_reg + 5;
			mix_sr = snd_soc_component_read32(component,
							  mix_ctl_reg) &
				 0x0F;
			asrc_mode = tavil_get_asrc_mode(tavil, asrc, main_sr,
							mix_sr);
			dev_dbg(component->dev,
				"%s: main_sr:%d mix_sr:%d asrc_mode %d\n",
				__func__, main_sr, mix_sr, asrc_mode);
			snd_soc_component_update_bits(component, asrc_ctl, 0x07,
						      asrc_mode);
		}
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (tavil->asrc_users[asrc] <= 0) {
			...
			snd_soc_component_update_bits(component, cfg_reg, 0x80,
						      0x00);
			snd_soc_component_update_bits(component, clk_reg, 0x03,
						      0x02);
		}
		break;
	};
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (++tavil->native_clk_users == 1) {
			snd_soc_component_update_bits(
				component, WCD934X_CLK_SYS_PLL_ENABLES, 0x01,
				0x01);
			usleep_range(100, 120);
			snd_soc_component_update_bits(
				component, WCD934X_CLK_SYS_MCLK2_PRG1, 0x06,
				0x02);
			snd_soc_component_update_bits(
				component, WCD934X_CLK_SYS_MCLK2_PRG1, 0x01,
				0x01);
			snd_soc_component_update_bits(
				component, WCD934X_CODEC_RPM_CLK_GATE, 0x04,
				0x00);
			usleep_range(30, 50);
			snd_soc_component_update_bits(
				component,
				WCD934X_CDC_CLK_RST_CTRL_MCLK_CONTROL, 0x02,
				0x02);
			snd_soc_component_update_bits(
				component,
				WCD934X_CDC_CLK_RST_CTRL_FS_CNT_CONTROL, 0x10,
				0x10);
		}
		break;
	case SND_SOC_DAPM_PRE_PMD:
					   ...
					   u16 interp_idx, int event)
{
	...
	if (hph_lut_bypass_reg && SND_SOC_DAPM_EVENT_OFF(event)) {
		...
		snd_soc_component_update_bits(component, hph_lut_bypass_reg,
					      0x80, 0x00);
		snd_soc_component_update_bits(component, hph_comp_ctrl7, 0x20,
					      0x0);
	}
}

static void tavil_codec_hd2_control(struct tavil_priv *priv, u16 interp_idx,
				    int event)
{
	u16 hd2_scale_reg;
				  ...
				  int event, int interp_idx)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (tavil->main_clk_users[interp_idx] == 0) {
			...
			snd_soc_component_update_bits(component, main_reg, 0x10,
						      0x10);
			/* Clk enable */
			snd_soc_component_update_bits(component, main_reg, 0x20,
						      0x20);
	...
	if (SND_SOC_DAPM_EVENT_OFF(event)) {
		...
		if (tavil->main_clk_users[interp_idx] <= 0) {
			...
			snd_soc_component_update_bits(component, main_reg, 0x40,
						      0x40);
			snd_soc_component_update_bits(component, main_reg, 0x40,
						      0x00);
			/* Reset rate to 48K*/
			snd_soc_component_update_bits(component, main_reg, 0x0F,
						      0x04);
		}
	}

	dev_dbg(component->dev, "%s event %d main_clk_users %d\n", __func__,
		event, tavil->main_clk_users[interp_idx]);
					   ...
					   int interp, int path_type)
{
   ...
   */
	if (path_type == INTERP_MIX_PATH) {
		mux_reg =
			WCD934X_CDC_RX_INP_MUX_RX_INT1_CFG1 + 2 * (interp - 1);
		...
		if ((mux_reg_val >= INTn_2_INP_SEL_RX0) &&
			...
			*port_ptr++ =
				mux_reg_val + WCD934X_RX_PORT_START_NUMBER - 1;
			num_ports++;
		}
	}
	...
	if (path_type == INTERP_MAIN_PATH) {
		mux_reg =
			WCD934X_CDC_RX_INP_MUX_RX_INT1_CFG0 + 2 * (interp - 1);
		mux_reg_val = snd_soc_component_read32(component, mux_reg) &
			      0x0f;
	...
	dev_dbg(component->dev, "%s: num_ports: %d, ports[%d %d %d %d]\n",
		__func__, num_ports, port_id[0], port_id[1], port_id[2],
		port_id[3]);
	...
	while (num_ports) {
		...
		if ((dai_id >= 0) && (dai_id < NUM_CODEC_DAIS)) {
			dev_dbg(component->dev,
				"%s: dai_id: %d bit_width: %d\n", __func__,
				dai_id, tavil->dai[dai_id].bit_width);
				       ...
				       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		if ((tavil->swr.spkr_gain_offset ==
			...
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX7_RX_PATH_SEC1, 0x01,
				0x01);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX7_RX_PATH_MIX_SEC0,
				0x01, 0x01);
			snd_soc_component_update_bits(
				component, WCD934X_CDC_RX8_RX_PATH_SEC1, 0x01,
				0x01);
			snd_soc_component_update_bits(
...
tavil_get_dsd_config(struct snd_soc_component *component)
{
	struct tavil_priv *tavil;
				       ...
				       int adc_mux_n)
{
	...
	if (adc_mux_n < 3) {
		adc_mux_in_reg =
			WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG1 + 2 * adc_mux_n;
		mask = 0x03;
		shift = 0;
		amic_mux_sel_reg =
			WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG0 + 2 * adc_mux_n;
	} else if (adc_mux_n < 4) {
		...
		shift = 2;
		amic_mux_sel_reg =
			WCD934X_CDC_TX_INP_MUX_ADC_MUX4_CFG0 + adc_mux_n - 4;
	} else if (adc_mux_n < 8) {
		adc_mux_in_reg = WCD934X_CDC_TX_INP_MUX_ADC_MUX3_CFG1;
		mask = 0x0C;
		shift = 2;
		...
		amic_mux_sel_reg =
			...
			((adc_mux_n == 8) ? (adc_mux_n - 8) : (adc_mux_n - 9));
	} else if (adc_mux_n < 13) {
		adc_mux_in_reg = WCD934X_CDC_TX_INP_MUX_ADC_MUX3_CFG1;
		mask = 0x30;
		shift = 4;
		amic_mux_sel_reg =
			WCD934X_CDC_TX_INP_MUX_ADC_MUX4_CFG0 + adc_mux_n - 5;
	} else {
		adc_mux_in_reg = WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG1;
		mask = 0xC0;
		shift = 6;
				    ...
				    u16 amic_reg, bool set)
{
	...
	if (amic_reg == WCD934X_ANA_AMIC1 || amic_reg == WCD934X_ANA_AMIC3)
		mask = 0x40;
...
static void tavil_tx_mute_update_callback(struct work_struct *work)
{
	...
	tx_vol_ctl_reg =
		WCD934X_CDC_TX0_TX_PATH_CTL + 16 * tx_mute_dwork->decimator;
	hpf_gate_reg =
		WCD934X_CDC_TX0_TX_PATH_SEC2 + 16 * tx_mute_dwork->decimator;
					  ...
					  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (!strcmp(w->name, "RX INT7 MIX2 INP"))
			__tavil_codec_enable_swr(w, event);
		tavil_codec_enable_interp_clk(component, event, w->shift);
		snd_soc_component_update_bits(component, sidetone_reg, 0x10,
					      0x10);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, sidetone_reg, 0x10,
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_component_update_bits(component, dec_cfg_reg,
					      TX_HPF_CUT_OFF_FREQ_MASK,
					      CF_MIN_3DB_150HZ << 5);
		snd_soc_component_update_bits(component, hpf_gate_reg, 0x02,
					      0x02);
		/*
		...
		if (cancel_delayed_work_sync(
			...
			if (hpf_cut_off_freq != CF_MIN_3DB_150HZ) {
				snd_soc_component_update_bits(
					...
					hpf_cut_off_freq << 5);
				snd_soc_component_update_bits(
					component, hpf_gate_reg, 0x02, 0x02);
				/*
         * Minimum 1 clk cycle delay is required as per
         * HW spec.
         */
				usleep_range(1000, 1010);
				snd_soc_component_update_bits(
					component, hpf_gate_reg, 0x02, 0x00);
			}
		}
				      ...
				      struct wcd9xxx_pdata *pdata)
{
	...
	while (dec_found == 0 && adc_mux_index < WCD934X_MAX_VALID_ADC_MUX) {
		...
		adc_mux_sel =
			((snd_soc_component_read32(component, adc_mux_ctl_reg) &
			  0xF8) >>
			 3) -
			1;
				   ...
				   struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		dmic_sample_rate =
			tavil_get_dmic_sample_rate(component, dmic, pdata);
	...
	case SND_SOC_DAPM_POST_PMD:
		dmic_rate_val =
			tavil_get_dmic_clk_val(component, pdata->mclk_rate,
 ...
 */
int tavil_micbias_control(struct snd_soc_component *component, int micb_num,
			  int req, bool is_dapm)
{
		...
		if (tavil->micb_ref[micb_index] == 1) {
			...
			if (post_on_event && tavil->mbhc)
				blocking_notifier_call_chain(
					&tavil->mbhc->notifier, post_on_event,
					&tavil->mbhc->wcd_mbhc);
		}
	...
	case MICB_DISABLE:
		...
		if ((tavil->micb_ref[micb_index] == 0) &&
			...
			if (pre_off_event && tavil->mbhc)
				blocking_notifier_call_chain(
					&tavil->mbhc->notifier, pre_off_event,
					&tavil->mbhc->wcd_mbhc);
			snd_soc_component_update_bits(component, micb_reg, 0xC0,
						      0x00);
			if (post_off_event && tavil->mbhc)
				blocking_notifier_call_chain(
					&tavil->mbhc->notifier, post_off_event,
					&tavil->mbhc->wcd_mbhc);
			if (tavil->micb_load)
				regulator_set_load(tavil->micb_load,
						   tavil->micb_load_low);
	...
	dev_dbg(component->dev,
		...
		micb_num, tavil->micb_ref[micb_index],
		tavil->pullup_ref[micb_index]);
	...
	if (tavil->hph_mode == CLS_H_LOHIFI) {
		for (i = 0; i < ARRAY_SIZE(tavil_pre_pa_en_lohifi); i++) {
			...
			if ((TAVIL_IS_1_1(tavil->wcd9xxx)) &&
				...
				continue;
			regmap_write_bits(map, tavil_pre_pa_en_lohifi[i].reg,
					   ...
					   int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (ret) {
			dev_dbg(component->dev,
				"%s: cache and hw reg are not same\n",
				__func__);
			tavil_codec_hph_reg_recover(tavil, wcd9xxx->regmap,
						    pa_status);
		} else {
			dev_dbg(component->dev,
				"%s: cache and hw reg are same\n", __func__);
			if (tavil->mbhc)
				    ...
				    int band_idx)
{
   ...
   */
	...
	wcd9xxx_slim_write_repeat(
		tavil->wcd9xxx, reg_add, no_of_reg,
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	reg_val = snd_soc_component_read32(
		component, WCD934X_TLMM_DMIC1_CLK_PINCFG + dmic_pin - 1);
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (amic_reg)
		snd_soc_component_update_bits(
			component, amic_reg, WCD934X_AMIC_PWR_LVL_MASK,
...
static const struct soc_enum tavil_conn_mad_enum = SOC_ENUM_SINGLE_EXT(
	ARRAY_SIZE(tavil_conn_mad_text), tavil_conn_mad_text);
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	tavil_mad_input =
		snd_soc_component_read32(component, WCD934X_SOC_MAD_INP_SEL) &
		0x0F;
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tavil_mad_input >=
	    sizeof(tavil_conn_mad_text) / sizeof(tavil_conn_mad_text[0])) {
		dev_err(component->dev,
	...
	if (strnstr(tavil_conn_mad_text[tavil_mad_input], "ADC",
		    sizeof("ADC"))) {
		mad_input =
			strpbrk(tavil_conn_mad_text[tavil_mad_input], "1234");
		if (!mad_input) {
	...
	dev_dbg(component->dev, "%s: tavil input widget = %s, adc_input = %s\n",
		__func__, mad_input_widget, is_adc_input ? "true" : "false");
	...
	snd_soc_component_update_bits(component, WCD934X_SOC_MAD_INP_SEL, 0x0F,
				      tavil_mad_input);
	snd_soc_component_update_bits(component, WCD934X_ANA_MAD_SETUP, 0x07,
				      mic_bias_found);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_component_update_bits(component, WCD934X_ANA_EAR, 0x70,
				      ear_pa_gain);
	return 0;
}
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mode_val == 0) {
		dev_warn(component->dev,
			 "%s:Invalid HPH Mode, default to Cls-H LOHiFi\n",
			 __func__);
...
static const char *const asrc_mode_text[] = { "INT", "FRAC" };

static const char *const tavil_ear_pa_gain_text[] = { "G_6_DB",	   "G_4P5_DB",
						      "G_3_DB",	   "G_1P5_DB",
						      "G_0_DB",	   "G_M2P5_DB",
						      "UNDEFINED", "G_M12_DB" };
...
static const char *const tavil_speaker_boost_stage_text[] = { "NO_MAX_STATE",
							      "MAX_STATE_1",
							      "MAX_STATE_2" };
...
static const struct snd_kcontrol_new tavil_snd_controls[] = {
	...
	SOC_SINGLE_TLV("LINEOUT1 Volume", WCD934X_DIFF_LO_LO1_COMPANDER, 3, 16,
		       1, line_gain),
	SOC_SINGLE_TLV("LINEOUT2 Volume", WCD934X_DIFF_LO_LO2_COMPANDER, 3, 16,
		       1, line_gain),
	...
	SOC_SINGLE_S8_TLV("RX0 Digital Volume", WCD934X_CDC_RX0_RX_VOL_CTL, -84,
			  40, digital_gain), /* -84dB min - 40dB max */
	SOC_SINGLE_S8_TLV("RX1 Digital Volume", WCD934X_CDC_RX1_RX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("RX2 Digital Volume", WCD934X_CDC_RX2_RX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("RX3 Digital Volume", WCD934X_CDC_RX3_RX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("RX4 Digital Volume", WCD934X_CDC_RX4_RX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("RX7 Digital Volume", WCD934X_CDC_RX7_RX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("RX8 Digital Volume", WCD934X_CDC_RX8_RX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("RX0 Mix Digital Volume",
			  WCD934X_CDC_RX0_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX1 Mix Digital Volume",
			  WCD934X_CDC_RX1_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX2 Mix Digital Volume",
			  WCD934X_CDC_RX2_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX3 Mix Digital Volume",
			  WCD934X_CDC_RX3_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX4 Mix Digital Volume",
			  WCD934X_CDC_RX4_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX7 Mix Digital Volume",
			  WCD934X_CDC_RX7_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX8 Mix Digital Volume",
			  WCD934X_CDC_RX8_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),

	SOC_SINGLE_S8_TLV("DEC0 Volume", WCD934X_CDC_TX0_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC1 Volume", WCD934X_CDC_TX1_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC2 Volume", WCD934X_CDC_TX2_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC3 Volume", WCD934X_CDC_TX3_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC4 Volume", WCD934X_CDC_TX4_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC5 Volume", WCD934X_CDC_TX5_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC6 Volume", WCD934X_CDC_TX6_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC7 Volume", WCD934X_CDC_TX7_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("DEC8 Volume", WCD934X_CDC_TX8_TX_VOL_CTL, -84, 40,
			  digital_gain),
	...
	SOC_ENUM_EXT("RX HPH Mode", rx_hph_mode_mux_enum, tavil_rx_hph_mode_get,
		     tavil_rx_hph_mode_put),
	...
	SOC_SINGLE_MULTI_EXT("IIR0 Band1", IIR0, BAND1, 255, 0, 5,
			     tavil_iir_band_audio_mixer_get,
			     tavil_iir_band_audio_mixer_put),
	...
	SOC_ENUM_EXT("MAD Input", tavil_conn_mad_enum, tavil_mad_input_get,
		     tavil_mad_input_put),
...
static const char *const cdc_if_tx0_mux_text[] = { "ZERO", "RX_MIX_TX0", "DEC0",
						   "DEC0_192" };
static const char *const cdc_if_tx1_mux_text[] = { "ZERO", "RX_MIX_TX1", "DEC1",
						   "DEC1_192" };
static const char *const cdc_if_tx2_mux_text[] = { "ZERO", "RX_MIX_TX2", "DEC2",
						   "DEC2_192" };
static const char *const cdc_if_tx3_mux_text[] = { "ZERO", "RX_MIX_TX3", "DEC3",
						   "DEC3_192" };
static const char *const cdc_if_tx4_mux_text[] = { "ZERO", "RX_MIX_TX4", "DEC4",
						   "DEC4_192" };
static const char *const cdc_if_tx5_mux_text[] = { "ZERO", "RX_MIX_TX5", "DEC5",
						   "DEC5_192" };
static const char *const cdc_if_tx6_mux_text[] = { "ZERO", "RX_MIX_TX6", "DEC6",
						   "DEC6_192" };
static const char *const cdc_if_tx7_mux_text[] = { "ZERO", "RX_MIX_TX7", "DEC7",
						   "DEC7_192" };
static const char *const cdc_if_tx8_mux_text[] = { "ZERO", "RX_MIX_TX8", "DEC8",
						   "DEC8_192" };
static const char *const cdc_if_tx9_mux_text[] = { "ZERO", "DEC7", "DEC7_192" };
static const char *const cdc_if_tx10_mux_text[] = { "ZERO", "DEC6",
						    "DEC6_192" };
static const char *const cdc_if_tx11_mux_text[] = { "DEC_0_5", "DEC_9_12",
...
static const char *const cdc_if_tx13_mux_text[] = { "CDC_DEC_5", "MAD_BRDCST" };
static const char *const cdc_if_tx13_inp1_mux_text[] = { "ZERO", "DEC5",
							 "DEC5_192" };
...
static const char *const mad_sel_txt[] = { "SPE", "MSM" };

static const char *const mad_inp_mux_txt[] = { "MAD", "DEC1" };
...
static const char *const amic4_5_sel_text[] = { "AMIC4", "AMIC5" };
...
static const char *const rx_echo_mux_text[] = { "ZERO",	   "RX_MIX0", "RX_MIX1",
						"RX_MIX2", "RX_MIX3", "RX_MIX4",
						"RX_MIX5", "RX_MIX6", "RX_MIX7",
						"RX_MIX8" };

static const char *const slim_rx_mux_text[] = { "ZERO", "AIF1_PB", "AIF2_PB",
						"AIF3_PB", "AIF4_PB" };

static const char *const i2s_rx01_mux_text[] = { "ZERO", "AIF1_PB", "AIF2_PB",
						 "AIF3_PB" };

static const char *const i2s_rx23_mux_text[] = { "ZERO", "AIF1_PB", "AIF2_PB",
						 "AIF3_PB" };

static const char *const i2s_rx45_mux_text[] = { "ZERO", "AIF1_PB", "AIF2_PB",
						 "AIF3_PB" };

static const char *const i2s_rx67_mux_text[] = { "ZERO", "AIF1_PB", "AIF2_PB",
						 "AIF3_PB" };

static const char *const cdc_if_rx0_mux_text[] = { "SLIM RX0", "I2S RX0" };
static const char *const cdc_if_rx1_mux_text[] = { "SLIM RX1", "I2S RX1" };
static const char *const cdc_if_rx2_mux_text[] = { "SLIM RX2", "I2S RX2" };
static const char *const cdc_if_rx3_mux_text[] = { "SLIM RX3", "I2S RX3" };
static const char *const cdc_if_rx4_mux_text[] = { "SLIM RX4", "I2S RX4" };
static const char *const cdc_if_rx5_mux_text[] = { "SLIM RX5", "I2S RX5" };
static const char *const cdc_if_rx6_mux_text[] = { "SLIM RX6", "I2S RX6" };
static const char *const cdc_if_rx7_mux_text[] = { "SLIM RX7", "I2S RX7" };
...
static const char *const wdma3_port0_text[] = { "RX_MIX_TX0", "DEC0" };

static const char *const wdma3_port1_text[] = { "RX_MIX_TX1", "DEC1" };

static const char *const wdma3_port2_text[] = { "RX_MIX_TX2", "DEC2" };

static const char *const wdma3_port3_text[] = { "RX_MIX_TX3", "DEC3" };

static const char *const wdma3_port4_text[] = { "RX_MIX_TX4", "DEC4" };

static const char *const wdma3_port5_text[] = { "RX_MIX_TX5", "DEC5" };

static const char *const wdma3_port6_text[] = { "RX_MIX_TX6", "DEC6" };
...
WCD_DAPM_ENUM_EXT(slim_rx0, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx1, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx2, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx3, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx4, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx5, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx6, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
WCD_DAPM_ENUM_EXT(slim_rx7, SND_SOC_NOPM, 0, slim_rx_mux_text, slim_rx_mux_get,
		  slim_rx_mux_put);
...
WCD_DAPM_ENUM(mad_sel, WCD934X_CPE_SS_SVA_CFG, 0, mad_sel_txt);

WCD_DAPM_ENUM(mad_inp_mux, WCD934X_CPE_SS_SVA_CFG, 2, mad_inp_mux_txt);
...
WCD_DAPM_ENUM_EXT(tx_adc_mux0, WCD934X_CDC_TX_INP_MUX_ADC_MUX0_CFG1, 0,
		  adc_mux_text, snd_soc_dapm_get_enum_double,
		  tavil_dec_enum_put);
...
WCD_DAPM_ENUM_EXT(i2s_rx0, SND_SOC_NOPM, 0, i2s_rx01_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx1, SND_SOC_NOPM, 0, i2s_rx01_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx2, SND_SOC_NOPM, 0, i2s_rx23_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx3, SND_SOC_NOPM, 0, i2s_rx23_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx4, SND_SOC_NOPM, 0, i2s_rx45_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx5, SND_SOC_NOPM, 0, i2s_rx45_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx6, SND_SOC_NOPM, 0, i2s_rx67_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
WCD_DAPM_ENUM_EXT(i2s_rx7, SND_SOC_NOPM, 0, i2s_rx67_mux_text, i2s_rx_mux_get,
		  i2s_rx_mux_put);
...
static const struct snd_soc_dapm_widget tavil_dapm_slim_widgets[] = {
	...
	SND_SOC_DAPM_AIF_OUT("AIF4 MAD", "AIF4 MAD TX", 0, SND_SOC_NOPM, 0, 0),
...
static const struct snd_soc_dapm_widget tavil_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX_E("RX INT0 MIX2 INP", SND_SOC_NOPM, INTERP_EAR, 0,
			   ...
			   tavil_codec_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
     ...
     */
	SND_SOC_DAPM_SUPPLY("RESET_HPH_REGISTERS", SND_SOC_NOPM, 0, 0,
			    tavil_codec_reset_hph_registers,
			    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_MIXER_E("IIR0", WCD934X_CDC_SIDETONE_IIR0_IIR_PATH_CTL, 4,
			     0, NULL, 0, tavil_codec_set_iir_gain,
			     SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_MIXER_E("IIR1", WCD934X_CDC_SIDETONE_IIR1_IIR_PATH_CTL, 4,
			     0, NULL, 0, tavil_codec_set_iir_gain,
			     SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_MIXER("SRC0", WCD934X_CDC_SIDETONE_SRC0_ST_SRC_PATH_CTL, 4,
			   0, NULL, 0),
	SND_SOC_DAPM_MIXER("SRC1", WCD934X_CDC_SIDETONE_SRC1_ST_SRC_PATH_CTL, 4,
			   0, NULL, 0),
	...
	SND_SOC_DAPM_SWITCH_E("MAD_CPE1", SND_SOC_NOPM, 0, 0, &mad_cpe1_switch,
			      tavil_codec_cpe_mad_ctl,
			      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_SWITCH_E("MAD_CPE2", SND_SOC_NOPM, 0, 0, &mad_cpe2_switch,
			      tavil_codec_cpe_mad_ctl,
			      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	...
	SND_SOC_DAPM_DAC_E("RX INT0 DAC", NULL, SND_SOC_NOPM, 0, 0,
			   tavil_codec_ear_dac_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_DAC_E("RX INT1 DAC", NULL, WCD934X_ANA_HPH, 5, 0,
			   tavil_codec_hphl_dac_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_DAC_E("RX INT2 DAC", NULL, WCD934X_ANA_HPH, 4, 0,
			   tavil_codec_hphr_dac_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_DAC_E("RX INT3 DAC", NULL, SND_SOC_NOPM, 0, 0,
			   tavil_codec_lineout_dac_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SUPPLY("RX INT4 NATIVE SUPPLY", SND_SOC_NOPM, INTERP_LO2,
			    0, tavil_enable_native_supply,
			    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_SUPPLY("RX INT7 NATIVE SUPPLY", SND_SOC_NOPM, INTERP_SPKR1,
			    0, tavil_enable_native_supply,
			    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_SUPPLY("RX INT8 NATIVE SUPPLY", SND_SOC_NOPM, INTERP_SPKR2,
			    0, tavil_enable_native_supply,
			    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	...
	SND_SOC_DAPM_MUX_E("ASRC0 MUX", SND_SOC_NOPM, ASRC0, 0, &asrc0_mux,
			   tavil_codec_enable_asrc_resampler,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("ASRC1 MUX", SND_SOC_NOPM, ASRC1, 0, &asrc1_mux,
			   tavil_codec_enable_asrc_resampler,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("ASRC2 MUX", SND_SOC_NOPM, ASRC2, 0, &asrc2_mux,
			   tavil_codec_enable_asrc_resampler,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("ASRC3 MUX", SND_SOC_NOPM, ASRC3, 0, &asrc3_mux,
			   tavil_codec_enable_asrc_resampler,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static int tavil_get_channel_map(struct snd_soc_dai *dai, unsigned int *tx_num,
				 ...
				 unsigned int *rx_slot)
{
	...
	case AIF4_PB:
		if (!rx_slot || !rx_num) {
			dev_err(tavil->dev,
				"%s: Invalid rx_slot 0x%pK or rx_num 0x%pK\n",
		...
		*rx_num = i;
		dev_dbg(tavil->dev,
			"%s: dai_name = %s dai_id = %x  rx_num = %d\n",
			__func__, dai->name, dai->id, i);
		if (*rx_num == 0) {
			dev_err(tavil->dev,
				"%s: Channel list empty for dai_name = %s dai_id = %x\n",
	...
	case AIF4_VIFEED:
		if (!tx_slot || !tx_num) {
			dev_err(tavil->dev,
				"%s: Invalid tx_slot 0x%pK or tx_num 0x%pK\n",
		...
		*tx_num = i;
		dev_dbg(tavil->dev,
			"%s: dai_name = %s dai_id = %x  tx_num = %d\n",
			__func__, dai->name, dai->id, i);
		if (*tx_num == 0) {
			dev_err(tavil->dev,
				"%s: Channel list empty for dai_name = %s dai_id = %x\n",
...
static int tavil_set_channel_map(struct snd_soc_dai *dai, unsigned int tx_num,
				 unsigned int *tx_slot, unsigned int rx_num,
				 unsigned int *rx_slot)
{
	struct tavil_priv *tavil;
	...
	if (!tx_slot || !rx_slot) {
		dev_err(tavil->dev,
			"%s: Invalid tx_slot 0x%pK, rx_slot 0x%pK\n", __func__,
			tx_slot, rx_slot);
		return -EINVAL;
	}
...
static int tavil_set_decimator_rate(struct snd_soc_dai *dai, u32 sample_rate)
{
	...
	list_for_each_entry(ch, &tavil->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		if (decimator >= 0) {
			tx_fs_reg =
				WCD934X_CDC_TX0_TX_PATH_CTL + 16 * decimator;
			dev_dbg(component->dev,
			...
			dev_dbg(component->dev,
				"%s: RX_MIX_TX%u going to CDC_IF TX%u\n",
...
static int tavil_set_mix_interpolator_rate(struct snd_soc_dai *dai,
					   u8 rate_reg_val, u32 sample_rate)
{
	u8 int_2_inp;
	u32 j;
	...
	list_for_each_entry(ch, &tavil->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		if ((int_2_inp < INTn_2_INP_SEL_RX0) ||
			...
			dev_err(component->dev,
				"%s: Invalid RX%u port, Dai ID is %d\n",
				__func__,
         ...
         * 384KHz only
         */
				if ((j == INTERP_EAR) && (rate_reg_val < 0x4 ||
							  rate_reg_val > 0x7)) {
					dev_err_ratelimited(
						component->dev,
					    ...
					    u8 rate_reg_val, u32 sample_rate)
{
     ...
     */
         ...
         */
				...
				int_fs_reg =
					WCD934X_CDC_RX0_RX_PATH_CTL + 20 * j;
				dev_dbg(component->dev,
...
static int tavil_set_interpolator_rate(struct snd_soc_dai *dai, u32 sample_rate)
{
	struct snd_soc_component *component = dai->component;
	int rate_val = 0;
	int i, ret;
...
static int tavil_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		/* CPU is master */
		snd_soc_component_update_bits(dai->component, i2s_reg, 0x2,
					      0x0);
		break;
...
static void tavil_codec_power_gate_digital_core(struct tavil_priv *tavil)
{
	...
	wcd9xxx_set_power_state(tavil->wcd9xxx, WCD_REGION_POWER_COLLAPSE_BEGIN,
...
static int tavil_dig_core_remove_power_collapse(struct tavil_priv *tavil)
{
	...
	regmap_update_bits(tavil->wcd9xxx->regmap, WCD934X_CODEC_RPM_RST_CTL,
			   0x02, 0x00);
	regmap_update_bits(tavil->wcd9xxx->regmap, WCD934X_CODEC_RPM_RST_CTL,
			   0x02, 0x02);
	regmap_write(tavil->wcd9xxx->regmap,
	...
	regcache_sync_region(tavil->wcd9xxx->regmap, WCD934X_DIG_CORE_REG_MIN,
...
static int tavil_cdc_req_mclk_enable(struct tavil_priv *tavil, bool enable)
{
	int ret = 0;
...
static int __tavil_cdc_mclk_enable_locked(struct tavil_priv *tavil, bool enable)
{
	int ret = 0;
...
static int __tavil_cdc_mclk_enable(struct tavil_priv *tavil, bool enable)
{
	int ret;
					...
					struct snd_soc_component *component)
{
	...
	tavil = snd_soc_component_get_drvdata(component);
	card = component->card;
	tavil->entry = snd_info_create_module_entry(codec_root->module, "tavil",
						    codec_root);
	if (!tavil->entry) {
	...
	if (!version_entry) {
		dev_dbg(component->dev,
			"%s: failed to create wcd934x version entry\n",
			__func__);
		return -ENOMEM;
	}
					   ...
					   bool enable)
{
	...
	if (enable) {
		if (wcd_resmgr_get_clk_type(tavil->resmgr) == WCD_CLK_RCO) {
		...
		ret = wcd_resmgr_disable_clk_block(tavil->resmgr, WCD_CLK_RCO);
	}
...
int tavil_cdc_mclk_tx_enable(struct snd_soc_component *component, bool enable)
{
	...
	dev_dbg(component->dev,
		"%s: clkmode: %d, enable: %d, clk_internal: %d\n", __func__,
		clk_mode, enable, clk_internal);
...
static const struct tavil_cpr_reg_defaults cpr_defaults[] = {
	{ 0x00000820, 0x00000094 }, { 0x00000fC0, 0x00000048 },
	{ 0x0000f000, 0x00000044 }, { 0x0000bb80, 0xC0000178 },
	{ 0x00000000, 0x00000160 }, { 0x10854522, 0x00000060 },
	{ 0x10854509, 0x00000064 }, { 0x108544dd, 0x00000068 },
	{ 0x108544ad, 0x0000006C }, { 0x0000077E, 0x00000070 },
	{ 0x000007da, 0x00000074 }, { 0x00000000, 0x00000078 },
	{ 0x00000000, 0x0000007C }, { 0x00042029, 0x00000080 },
	{ 0x4002002A, 0x00000090 }, { 0x4002002B, 0x00000090 },
};
...
static void tavil_codec_init_reg(struct tavil_priv *priv)
{
	...
	for (i = 0; i < ARRAY_SIZE(tavil_codec_reg_init_common_val); i++)
		snd_soc_component_update_bits(
			component, tavil_codec_reg_init_common_val[i].reg,
	...
	if (TAVIL_IS_1_1(priv->wcd9xxx)) {
		for (i = 0; i < ARRAY_SIZE(tavil_codec_reg_init_1_1_val); i++)
			snd_soc_component_update_bits(
				component, tavil_codec_reg_init_1_1_val[i].reg,
...
static void tavil_update_cpr_defaults(struct tavil_priv *tavil)
{
	...
	regmap_update_bits(wcd9xxx->regmap, WCD934X_CODEC_RPM_CLK_GATE, 0x10,
			   0x00);

	for (i = 0; i < ARRAY_SIZE(cpr_defaults); i++) {
		regmap_bulk_write(wcd9xxx->regmap, WCD934X_CODEC_CPR_WR_DATA_0,
				  (u8 *)&cpr_defaults[i].wr_data, 4);
		regmap_bulk_write(wcd9xxx->regmap, WCD934X_CODEC_CPR_WR_ADDR_0,
...
static void tavil_slim_interface_init_reg(struct snd_soc_component *component)
{
	...
	for (i = 0; i < WCD9XXX_SLIM_NUM_PORT_REG; i++)
		wcd9xxx_interface_reg_write(
			priv->wcd9xxx, WCD934X_SLIM_PGD_PORT_INT_RX_EN0 + i,
			0xFF);
}
...
static irqreturn_t tavil_slimbus_irq(int irq, void *data)
{
	...
	for_each_set_bit(j, &status, 32) {
		...
		if (val) {
			if (!tx)
				...
				reg = WCD934X_SLIM_PGD_PORT_INT_TX_EN0 +
				      (port_id / 8);
			int_val =
				wcd9xxx_interface_reg_read(tavil->wcd9xxx, reg);
			/*
       ...
       */
		...
		if ((val & WCD934X_SLIM_IRQ_OVERFLOW) ||
			...
			if (int_val & (1 << (port_id % 8))) {
				...
				wcd9xxx_interface_reg_write(tavil->wcd9xxx, reg,
							    int_val);
			}
		}
       ...
       */
			bit = (tx ? j - 16 : j + 16);
			dev_dbg(tavil->dev,
				"%s: %s port %d closed value %x, bit %u\n",
				__func__, (tx ? "TX" : "RX"), port_id, val,
				bit);
			for (k = 0, cleared = false; k < NUM_CODEC_DAIS; k++) {
				dev_dbg(tavil->dev,
					"%s: tavil->dai[%d].ch_mask = 0x%lx\n",
					__func__, k, tavil->dai[k].ch_mask);
				if (test_and_clear_bit(
					    bit, &tavil->dai[k].ch_mask)) {
					cleared = true;
					if (!tavil->dai[k].ch_mask)
						wake_up(&tavil->dai[k].dai_wait);
					/*
...
static int tavil_setup_irqs(struct tavil_priv *tavil)
{
	...
	ret = wcd9xxx_request_irq(core_res, WCD934X_IRQ_MISC, tavil_misc_irq,
				  "CDC MISC Irq", tavil);
	if (ret)
			      ...
			      struct wcd9xxx_pdata *pdata)
{
	...
	snd_soc_component_update_bits(component, WCD934X_ANA_MICB1, 0x3F,
				      vout_ctl_1);
	snd_soc_component_update_bits(component, WCD934X_ANA_MICB2, 0x3F,
				      vout_ctl_2);
	snd_soc_component_update_bits(component, WCD934X_ANA_MICB3, 0x3F,
				      vout_ctl_3);
	snd_soc_component_update_bits(component, WCD934X_ANA_MICB4, 0x3F,
				      vout_ctl_4);
	...
	snd_soc_component_update_bits(component,
				      WCD934X_TEST_DEBUG_PAD_DRVCTL_0, 0x0C,
				      dmic_clk_drv << 2);
   ...
   */
	mad_dmic_ctl_val = tavil_get_dmic_clk_val(tavil->component,
						  pdata->mclk_rate,
						  pdata->mad_dmic_sample_rate);
	snd_soc_component_update_bits(component, WCD934X_CPE_SS_DMIC0_CTL, 0x0E,
				      mad_dmic_ctl_val << 1);
	snd_soc_component_update_bits(component, WCD934X_CPE_SS_DMIC1_CTL, 0x0E,
				      mad_dmic_ctl_val << 1);
	snd_soc_component_update_bits(component, WCD934X_CPE_SS_DMIC2_CTL, 0x0E,
				      mad_dmic_ctl_val << 1);
...
static int tavil_device_down(struct wcd9xxx *wcd9xxx)
{
	...
	for (decimator = 0; decimator < WCD934X_NUM_DECIMATORS; decimator++) {
		if (delayed_work_pending(&priv->tx_mute_dwork[decimator].dwork))
			cancel_delayed_work(
				&priv->tx_mute_dwork[decimator].dwork);
		if (delayed_work_pending(&priv->tx_hpf_work[decimator].dwork))
			cancel_delayed_work(
...
static int tavil_post_reset_cb(struct wcd9xxx *wcd9xxx)
{
	...
	tavil_slimbus_slave_port_cfg.slave_dev_pgd_la = control->slim->laddr;
	...
	if (control->mclk_rate == WCD934X_MCLK_CLK_12P288MHZ)
		snd_soc_component_update_bits(
			component, WCD934X_CODEC_RPM_CLK_MCLK_CFG, 0x03, 0x00);
	else if (control->mclk_rate == WCD934X_MCLK_CLK_9P6MHZ)
		snd_soc_component_update_bits(
			component, WCD934X_CODEC_RPM_CLK_MCLK_CFG, 0x03, 0x01);
...
static int tavil_soc_codec_probe(struct snd_soc_component *component)
{
	...
	ret = wcd_cal_create_hwdep(tavil->fw_data, WCD9XXX_CODEC_HWDEP_NODE,
				   component);
	if (ret < 0) {
	...
	ptr = devm_kzalloc(component->dev,
			   (sizeof(tavil_rx_chs) + sizeof(tavil_tx_chs)),
			   GFP_KERNEL);
	if (!ptr) {
		ret = -ENOMEM;
		goto err_hwdep;
				 ...
				 void *swrm_handle, int action)
{
	...
	if (action) {
		ret = wcd9xxx_request_irq(&wcd9xxx->core_res,
					  ...
					  swrm_irq_handler, "Tavil SWR Master",
					  swrm_handle);
		if (ret)
				       ...
				       struct device_node *node)
{
	...
	rc = of_property_read_u32(node, "qcom,master-bus-num", &prop_value);
	if (rc < 0) {
	...
	if (of_modalias_node(node, spi->modalias, sizeof(spi->modalias)) < 0) {
		dev_err(tavil->dev, "%s: cannot find modalias for %s\n",
			__func__, node->full_name);
		goto err_dt_parse;
	}

	rc = of_property_read_u32(node, "qcom,chip-select", &prop_value);
	if (rc < 0) {
		dev_err(tavil->dev, "%s: prop %s not found in node %s",
	...
	rc = of_property_read_u32(node, "qcom,max-frequency", &prop_value);
	if (rc < 0) {
		dev_err(tavil->dev, "%s: prop %s not found in node %s",
...
static void tavil_add_child_devices(struct work_struct *work)
{
	...
	if (!wcd9xxx->dev->of_node) {
		dev_err(wcd9xxx->dev,
			"%s: DT node for wcd9xxx does not exist\n", __func__);
		return;
	}
	...
	for_each_child_of_node(wcd9xxx->dev->of_node, node) {
		...
		if (strcmp(node->name, "swr_master") == 0) {
			temp = krealloc(
				swr_ctrl_data,
				(ctrl_num + 1) *
					sizeof(struct tavil_swr_ctrl_data),
				GFP_KERNEL);
			if (!temp) {
...
static int __tavil_enable_efuse_sensing(struct tavil_priv *tavil)
{
   ...
   */
	usleep_range(5000, 5500);
	wcd_resmgr_set_sido_input_src(tavil->resmgr, SIDO_SOURCE_RCO_BG);
...
static int tavil_probe(struct platform_device *pdev)
{
	...
	tavil = devm_kzalloc(&pdev->dev, sizeof(struct tavil_priv), GFP_KERNEL);
	if (!tavil)
		return -ENOMEM;
   ...
   */
	...
	if (tavil->wcd9xxx->mclk_rate == WCD934X_MCLK_CLK_12P288MHZ)
		...
		regmap_update_bits(tavil->wcd9xxx->regmap,
				   WCD934X_CODEC_RPM_CLK_MCLK_CFG, 0x03, 0x01);
	tavil_update_reg_defaults(tavil);
	__tavil_enable_efuse_sensing(tavil);
	...
	ret = snd_event_client_register(pdev->dev.parent, &wcd934x_ssr_ops,
					NULL);
	if (!ret) {
...
static int tavil_remove(struct platform_device *pdev)
{
	...
	for (count = 0;
	     count < tavil->child_count && count < WCD934X_CHILD_DEVICES_MAX;
	     count++)
