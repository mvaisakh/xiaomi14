...
static int wcd937x_slave_bind(struct device *dev, struct device *master,
			      void *data)
{
	int ret = 0;
	struct wcd937x_slave_priv *wcd937x_slave = NULL;
	...
	wcd937x_slave = devm_kzalloc(
		&pdev->dev, sizeof(struct wcd937x_slave_priv), GFP_KERNEL);
	if (!wcd937x_slave)
		return -ENOMEM;
	...
	if (!wcd937x_slave->debugfs_wcd937x_dent) {
		...
		if (!IS_ERR(wcd937x_slave->debugfs_wcd937x_dent)) {
			wcd937x_slave->debugfs_peek = debugfs_create_file(
				"swrslave_peek", S_IFREG | 0444,
				wcd937x_slave->debugfs_wcd937x_dent,
				(void *)pdev, &codec_debug_read_ops);

			wcd937x_slave->debugfs_poke = debugfs_create_file(
				"swrslave_poke", S_IFREG | 0444,
				wcd937x_slave->debugfs_wcd937x_dent,
				(void *)pdev, &codec_debug_write_ops);

			wcd937x_slave->debugfs_reg_dump = debugfs_create_file(
...
static void wcd937x_slave_unbind(struct device *dev, struct device *master,
				 void *data)
{
	struct wcd937x_slave_priv *wcd937x_slave = NULL;
	struct swr_device *pdev = to_swr_device(dev);
...
static const struct swr_device_id wcd937x_swr_id[] = { { "wcd937x-slave", 0 },
						       {} };
