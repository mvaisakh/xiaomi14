...
int lpass_cdc_update_compander_setting(
	struct snd_soc_component *component, u16 start_addr,
	struct lpass_cdc_comp_setting *comp_setting)
{
	...
	/* apply zone settings */
	snd_soc_component_write(component, start_addr + 8, zone1_rms);
	...
	snd_soc_component_write(component, start_addr + 0xC, zone2_rms);
	...
	snd_soc_component_write(component, start_addr + 0x10, zone3_rms);
	...
	snd_soc_component_write(component, start_addr + 0x14, zone4_rms);
	...
	snd_soc_component_write(component, start_addr + 0x18, zone5_rms);

	zone6_rms = upper_gain_dig_int;
	snd_soc_component_write(component, start_addr + 0x1C, zone6_rms);

	if (lower_gain_int < 0)
		max_attn = 256 + lower_gain_int;
	else
		max_attn = lower_gain_int;
	snd_soc_component_write(component, start_addr + 0x20, max_attn);

	path_gain = zone6_rms - abs(lower_gain_int);
	snd_soc_component_write(component, start_addr + 0x24, path_gain);

	snd_soc_component_write(component, start_addr + 0x28, ana_addr_map);
