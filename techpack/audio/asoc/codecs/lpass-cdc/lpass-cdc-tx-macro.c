...
struct lpass_cdc_tx_macro_priv {
	...
	struct platform_device
		*pdev_child_devices[LPASS_CDC_TX_MACRO_CHILD_DEVICES_MAX];
	int child_count;
	bool bcs_enable;
...
static int
lpass_cdc_tx_macro_wake_enable(struct lpass_cdc_tx_macro_priv *tx_priv,
			       bool wake_enable)
{
	int ret = 0;
...
static bool lpass_cdc_tx_macro_get_data(
	struct snd_soc_component *component, struct device **tx_dev,
	struct lpass_cdc_tx_macro_priv **tx_priv, const char *func_name)
{
...
static int
lpass_cdc_tx_macro_mclk_enable(struct lpass_cdc_tx_macro_priv *tx_priv,
			       bool mclk_enable)
{
	...
	if (mclk_enable) {
		ret = lpass_cdc_clk_rsc_request_clock(tx_priv->dev, TX_CORE_CLK,
						      TX_CORE_CLK, true);
		if (ret < 0) {
		...
		lpass_cdc_clk_rsc_fs_gen_request(tx_priv->dev, true);
		regcache_mark_dirty(regmap);
		regcache_sync_region(regmap, TX_START_OFFSET, TX_MAX_OFFSET);
		if (tx_priv->tx_mclk_users == 0) {
			regmap_update_bits(
				regmap, LPASS_CDC_TX_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x01);
			regmap_update_bits(
				regmap,
				LPASS_CDC_TX_CLK_RST_CTRL_FS_CNT_CONTROL, 0x01,
				0x01);
		}
		...
		lpass_cdc_clk_rsc_fs_gen_request(tx_priv->dev, false);
		lpass_cdc_clk_rsc_request_clock(tx_priv->dev, TX_CORE_CLK,
						TX_CORE_CLK, false);
	}
exit:
					 ...
					 int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		break;
	default:
		dev_err_ratelimited(tx_priv->dev, "%s: invalid DAPM event %d\n",
				    __func__, event);
		ret = -EINVAL;
	}
	return ret;
	...
	case LPASS_CDC_MACRO_EVT_BCS_CLK_OFF:
		if (tx_priv->bcs_clk_en)
			snd_soc_component_update_bits(
				component, LPASS_CDC_TX0_TX_PATH_SEC7, 0x40,
				data << 6);
		if (data)
...
static void
lpass_cdc_tx_macro_tx_hpf_corner_freq_callback(struct work_struct *work)
{
	...
	if (is_amic_enabled(component, hpf_work->decimator)) {
		adc_reg = LPASS_CDC_TX_INP_MUX_ADC_MUX0_CFG0 +
			  LPASS_CDC_TX_MACRO_ADC_MUX_CFG_OFFSET *
				  hpf_work->decimator;
...
static void tx_macro_hs_unmute_dwork(struct work_struct *work)
{
	...
	delayed_work = to_delayed_work(work);
	lpass_cdc_tx_priv = container_of(delayed_work,
					 struct lpass_cdc_tx_macro_priv,
					 tx_hs_unmute_dwork);
...
void lpass_cdc_tx_macro_mute_hs(void)
{
	...
	if (g_lpass_cdc_tx_priv->save_reg) {
		g_lpass_cdc_tx_priv->reg_before_mute = snd_soc_component_read(
			component, LPASS_CDC_TX0_TX_VOL_CTL);
		g_lpass_cdc_tx_priv->save_reg = false;
		dev_info(component->dev,
			 "%s: the reg value before mute is: %#x \n", __func__,
			 g_lpass_cdc_tx_priv->reg_before_mute);
	} else {
		dev_info(
			component->dev,
			"%s: Quick plug and unplug headset has been detected, the reg "
			"value before mute is: %#x \n",
...
static int
lpass_cdc_tx_macro_get_bcs_ch_sel(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_tx_macro_put_bcs_ch_sel(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	snd_soc_component_update_bits(component, LPASS_CDC_VA_TOP_CSR_SWR_CTRL,
				      0x0F, value);
					 ...
					 int event)
{
	...
	if (!is_amic_enabled(component, decimator))
		lpass_cdc_tx_macro_enable_dmic(w, kcontrol, event,
					       adc_mux0_reg);
	...
	case SND_SOC_DAPM_POST_PMU:
       ...
       */
		...
		if (tx_priv->bcs_enable) {
			snd_soc_component_update_bits(
				component, LPASS_CDC_VA_TOP_CSR_SWR_CTRL, 0x0F,
				tx_priv->bcs_ch);
			...
			if (tx_priv->hs_slow_insert_complete)
				snd_soc_component_update_bits(
					component, LPASS_CDC_TX0_TX_PATH_SEC7,
					0x40, 0x40);
		}
		break;
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (snd_soc_component_read(component, adc_mux_reg) & SWR_MIC)
			snd_soc_component_update_bits(
				component, LPASS_CDC_TX_TOP_CSR_SWR_CTRL, 0x01,
				0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x20,
		...
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x10,
					      0x00);
		snd_soc_component_update_bits(component, tx_fs_reg, 0x0F, 0x04);
		if (tx_priv->bcs_enable) {
			snd_soc_component_update_bits(component, dec_cfg_reg,
						      0x01, 0x00);
			snd_soc_component_update_bits(
...
#define LPASS_CDC_TX_MACRO_DAPM_ENUM_EXT(name, reg, offset, text, getname, \
					 putname)                          \
	static SOC_ENUM_SINGLE_DECL(name##_enum, reg, offset, text);       \
	static const struct snd_kcontrol_new name##_mux =                  \
...
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec0, LPASS_CDC_TX_INP_MUX_ADC_MUX0_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec1, LPASS_CDC_TX_INP_MUX_ADC_MUX1_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec2, LPASS_CDC_TX_INP_MUX_ADC_MUX2_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec3, LPASS_CDC_TX_INP_MUX_ADC_MUX3_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec4, LPASS_CDC_TX_INP_MUX_ADC_MUX4_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec5, LPASS_CDC_TX_INP_MUX_ADC_MUX5_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec6, LPASS_CDC_TX_INP_MUX_ADC_MUX6_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_TX_MACRO_DAPM_ENUM(tx_dec7, LPASS_CDC_TX_INP_MUX_ADC_MUX7_CFG1, 0,
			     adc_mux_text);
...
static const struct snd_kcontrol_new tx_aif1_cap_mixer[] = {
	SOC_SINGLE_EXT("DEC0", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC0, 1, 0,
		       lpass_cdc_tx_macro_tx_mixer_get,
...
static const struct snd_soc_dapm_widget lpass_cdc_tx_macro_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("TX_AIF1_CAP Mixer", SND_SOC_NOPM,
			   LPASS_CDC_TX_MACRO_AIF1_CAP, 0, tx_aif1_cap_mixer,
			   ARRAY_SIZE(tx_aif1_cap_mixer)),

	SND_SOC_DAPM_MIXER("TX_AIF2_CAP Mixer", SND_SOC_NOPM,
			   LPASS_CDC_TX_MACRO_AIF2_CAP, 0, tx_aif2_cap_mixer,
			   ARRAY_SIZE(tx_aif2_cap_mixer)),

	SND_SOC_DAPM_MIXER("TX_AIF3_CAP Mixer", SND_SOC_NOPM,
			   LPASS_CDC_TX_MACRO_AIF3_CAP, 0, tx_aif3_cap_mixer,
			   ARRAY_SIZE(tx_aif3_cap_mixer)),
	...
	SND_SOC_DAPM_MUX_E("TX DEC0 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC0,
			   0, &tx_dec0_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC1 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC1,
			   0, &tx_dec1_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC2 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC2,
			   0, &tx_dec2_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC3 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC3,
			   0, &tx_dec3_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC4 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC4,
			   0, &tx_dec4_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC5 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC5,
			   0, &tx_dec5_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC6 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC6,
			   0, &tx_dec6_mux, lpass_cdc_tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC7 MUX", SND_SOC_NOPM, LPASS_CDC_TX_MACRO_DEC7,
...
static const struct snd_kcontrol_new lpass_cdc_tx_macro_snd_controls[] = {
	SOC_SINGLE_S8_TLV("TX_DEC0 Volume", LPASS_CDC_TX0_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC1 Volume", LPASS_CDC_TX1_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC2 Volume", LPASS_CDC_TX2_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC3 Volume", LPASS_CDC_TX3_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC4 Volume", LPASS_CDC_TX4_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC5 Volume", LPASS_CDC_TX5_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC6 Volume", LPASS_CDC_TX6_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC7 Volume", LPASS_CDC_TX7_TX_VOL_CTL, -84, 40,
			  digital_gain),

	SOC_ENUM_EXT("DEC0 MODE", dec_mode_mux_enum,
		     lpass_cdc_tx_macro_dec_mode_get,
		     lpass_cdc_tx_macro_dec_mode_put),
	...
	SOC_SINGLE_EXT("TX_SWR_DMIC Enable", SND_SOC_NOPM, 0, 1, 0,
		       lpass_cdc_tx_macro_swr_dmic_get,
		       lpass_cdc_tx_macro_swr_dmic_put),

	SOC_ENUM_EXT("BCS Channel", bcs_ch_enum, lpass_cdc_tx_macro_bcs_ch_get,
		     lpass_cdc_tx_macro_bcs_ch_put),

	SOC_ENUM_EXT("BCS CH_SEL", bcs_ch_sel_mux_enum,
		     lpass_cdc_tx_macro_get_bcs_ch_sel,
		     lpass_cdc_tx_macro_put_bcs_ch_sel),
};
...
static int lpass_cdc_tx_macro_init(struct snd_soc_component *component)
{
	...
	ret = snd_soc_dapm_new_controls(
		dapm, lpass_cdc_tx_macro_dapm_widgets,
	...
	ret = snd_soc_add_component_controls(
		component, lpass_cdc_tx_macro_snd_controls,
	...
	for (i = 0; i < ARRAY_SIZE(lpass_cdc_tx_macro_reg_init); i++)
		snd_soc_component_update_bits(
			component, lpass_cdc_tx_macro_reg_init[i].reg,
...
static int lpass_cdc_tx_macro_probe(struct platform_device *pdev)
{
	...
	tx_priv = devm_kzalloc(
		&pdev->dev, sizeof(struct lpass_cdc_tx_macro_priv), GFP_KERNEL);
	if (!tx_priv)
		return -ENOMEM;
	...
	tx_io_base = devm_ioremap(&pdev->dev, tx_base_addr,
				  LPASS_CDC_TX_MACRO_MAX_OFFSET);
	if (!tx_io_base) {
	...
	if (ret) {
		...
		if (lpass_cdc_tx_macro_validate_dmic_sample_rate(sample_rate,
								 tx_priv) ==
		    LPASS_CDC_TX_MACRO_DMIC_SAMPLE_RATE_UNDEFINED)
			return -EINVAL;
	}
