...
const u16 macro_id_base_offset[MAX_MACRO] = {
	TX_START_OFFSET, RX_START_OFFSET,   WSA_START_OFFSET,
	VA_START_OFFSET, WSA2_START_OFFSET,
};
...
int lpass_cdc_get_macro_id(bool va_no_dec_flag, u16 reg)
{
	...
	if (reg >= WSA_START_OFFSET && reg <= WSA_MAX_OFFSET)
		return WSA_MACRO;
	if (reg >= WSA2_START_OFFSET && reg <= WSA2_MAX_OFFSET)
		return WSA2_MACRO;
	if (reg >= VA_START_OFFSET && reg <= VA_MAX_OFFSET)
		return VA_MACRO;
			   ...
			   void *val, size_t val_size)
{
	...
	reg_p = (u16 *)reg;
	macro_id =
		lpass_cdc_get_macro_id(priv->va_without_decimation, reg_p[0]);
...
static int regmap_bus_write(void *context, const void *data, size_t count)
{
	...
	if (count < REG_BYTES) {
		dev_err_ratelimited(dev,
				    "%s: count %zd bytes < %d, not supported\n",
