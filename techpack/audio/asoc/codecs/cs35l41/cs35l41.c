...
static int cs35l41_do_fast_switch(struct cs35l41_private *cs35l41)
{
	...
	wm_adsp_write_ctl(&cs35l41->dsp, "CSPL_UPDATE_PARAMS_CONFIG",
			  WMFW_ADSP2_YM, 0xcd, data_ctl_buf,
	...
	/* Verify CSPL COMMAND */
	for (i = 0; i < 5; i++) {
		wm_adsp_read_ctl(&cs35l41->dsp, "CSPL_STATE", WMFW_ADSP2_XM,
				 0xcd, &st_ctl, sizeof(s32));
		...
		if (otp_map[i].reg != 0) {
			...
			if (ret < 0) {
				dev_info(
					...
					i, bit_offset, word_offset,
					bit_sum % 32, otp_map[i].reg,
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (val & CS35L41_GLOBAL_EN_MASK) {
			...
			regmap_multi_reg_write_bypassed(
				cs35l41->regmap, cs35l41_pdn_patch,
				ARRAY_SIZE(cs35l41_pdn_patch));

			pdn = false;
			for (i = 0; i < 100; i++) {
				regmap_read(cs35l41->regmap,
					    CS35L41_IRQ1_STATUS1, &val);
					 ...
					 int num_dai)
{
	...
	if ((dai_drv != NULL) && (num_dai > 0)) {
		for (i = 0; i < num_dai; i++) {
			...
			dai_drv[i].name =
				cs35lxx_devm_kstrdup(cs35l41->dev, buf);
		}
	}
				  ...
				  struct cs35l41_private *cs35l41)
{
	...
	cs35l41->dt_name =
		devm_kstrdup_const(cs35l41->dev, np->name, GFP_KERNEL);
	...
	if (classh_config->classh_algo_enable) {
		classh_config->classh_bst_override = of_property_read_bool(
			sub_node, "cirrus,classh-bst-override");
		  ...
		  struct cs35l41_platform_data *pdata)
{
   ...
   */
	...
	cs35l41->dai_driver =
		devm_kzalloc(cs35l41->dev, sizeof(cs35l41_dai), GFP_KERNEL);
	...
	cs35lxx_dai_drv_append_suffix(cs35l41, cs35l41->dai_driver,
	...
	if (strcmp(dev_name(cs35l41->dev), "2-0040") == 0) { // B
		ret = regmap_write(cs35l41->regmap, CS35L41_VPBR_CFG,
				   0x02005304);
	} else {
		ret = regmap_write(cs35l41->regmap, CS35L41_VPBR_CFG,
				   0x02005305);
