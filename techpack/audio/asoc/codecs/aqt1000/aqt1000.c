				       ...
				       int band_idx, int coeff_idx)
{
	...
	snd_soc_component_write(
		component, AQT1000_CDC_SIDETONE_IIR0_IIR_COEF_B1_CTL,
		((band_idx * BAND_MAX + coeff_idx) * sizeof(uint32_t)) & 0x7F);

	value |= snd_soc_component_read32(
		component, AQT1000_CDC_SIDETONE_IIR0_IIR_COEF_B2_CTL);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(component->dev,
		"%s: IIR band #%d b0 = 0x%x\n"
		"%s: IIR band #%d b1 = 0x%x\n"
		"%s: IIR band #%d b2 = 0x%x\n"
		"%s: IIR band #%d a1 = 0x%x\n"
		"%s: IIR band #%d a2 = 0x%x\n",
		__func__, band_idx, (uint32_t)ucontrol->value.integer.value[0],
		__func__, band_idx, (uint32_t)ucontrol->value.integer.value[1],
		__func__, band_idx, (uint32_t)ucontrol->value.integer.value[2],
		__func__, band_idx, (uint32_t)ucontrol->value.integer.value[3],
		__func__, band_idx, (uint32_t)ucontrol->value.integer.value[4]);
					...
					struct snd_ctl_elem_value *ucontrol)
{
   ...
   */
	...
	dev_dbg(component->dev,
		...
		"%s: IIR band #%d a2 = 0x%x\n",
		__func__, band_idx,
		aqt_get_iir_band_coeff(component, band_idx, 0), __func__,
		band_idx, aqt_get_iir_band_coeff(component, band_idx, 1),
		__func__, band_idx,
		aqt_get_iir_band_coeff(component, band_idx, 2), __func__,
		band_idx, aqt_get_iir_band_coeff(component, band_idx, 3),
		__func__, band_idx,
		aqt_get_iir_band_coeff(component, band_idx, 4));
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	case COMPANDER_1:
		...
		snd_soc_component_update_bits(component, AQT1000_HPH_L_EN, 0x20,
					      (value ? 0x00 : 0x20));
		break;
	case COMPANDER_2:
		snd_soc_component_update_bits(component, AQT1000_HPH_R_EN, 0x20,
					      (value ? 0x00 : 0x20));
		break;
	default:
		/*
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aqt && (index >= 0) && (index < ASRC_MAX))
		aqt->asrc_output_mode[index] = ucontrol->value.integer.value[0];
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (amic_reg)
		snd_soc_component_update_bits(
			component, amic_reg, AQT1000_AMIC_PWR_LVL_MASK,
...
static const struct snd_kcontrol_new aqt_snd_controls[] = {
	...
	SOC_SINGLE_SX_TLV("AQT DEC0 Volume", AQT1000_CDC_TX0_TX_VOL_CTL, 0, -84,
			  40, digital_gain),
	SOC_SINGLE_SX_TLV("AQT DEC1 Volume", AQT1000_CDC_TX1_TX_VOL_CTL, 0, -84,
			  40, digital_gain),
	SOC_SINGLE_SX_TLV("AQT DEC2 Volume", AQT1000_CDC_TX2_TX_VOL_CTL, 0, -84,
			  40, digital_gain),
	...
	SOC_SINGLE_MULTI_EXT("AQT IIR0 Band1", IIR0, BAND1, 255, 0, 5,
			     aqt_iir_band_audio_mixer_get,
			     aqt_iir_band_audio_mixer_put),
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (aqt->rx_bias_count == 1) {
			snd_soc_component_update_bits(
				component, AQT1000_ANA_RX_SUPPLIES, 0x01, 0x01);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		...
		if (!aqt->rx_bias_count)
			snd_soc_component_update_bits(
				component, AQT1000_ANA_RX_SUPPLIES, 0x01, 0x00);
		break;
	};
 ...
 */
int aqt_micbias_control(struct snd_soc_component *component, int micb_num,
			int req, bool is_dapm)
{
	...
	case MICB_PULLUP_ENABLE:
		aqt->pullup_ref++;
		if ((aqt->pullup_ref == 1) && (aqt->micb_ref == 0))
	...
	case MICB_PULLUP_DISABLE:
		if (aqt->pullup_ref > 0)
			aqt->pullup_ref--;
		if ((aqt->pullup_ref == 0) && (aqt->micb_ref == 0))
			snd_soc_component_update_bits(component, micb_reg, 0xC0,
						      0x00);
		break;
	case MICB_ENABLE:
		...
		if (aqt->micb_ref == 1) {
			...
			if (post_on_event && aqt->mbhc)
				blocking_notifier_call_chain(
					&aqt->mbhc->notifier, post_on_event,
					&aqt->mbhc->wcd_mbhc);
		}
	...
	case MICB_DISABLE:
		if (aqt->micb_ref > 0)
			aqt->micb_ref--;
		if ((aqt->micb_ref == 0) && (aqt->pullup_ref > 0))
			snd_soc_component_update_bits(component, micb_reg, 0xC0,
						      0x80);
		else if ((aqt->micb_ref == 0) && (aqt->pullup_ref == 0)) {
			if (pre_off_event && aqt->mbhc)
				blocking_notifier_call_chain(
					&aqt->mbhc->notifier, pre_off_event,
					&aqt->mbhc->wcd_mbhc);
			snd_soc_component_update_bits(component, micb_reg, 0xC0,
						      0x00);
			if (post_off_event && aqt->mbhc)
				blocking_notifier_call_chain(
					&aqt->mbhc->notifier, post_off_event,
					&aqt->mbhc->wcd_mbhc);
		}
...
static int __aqt_codec_enable_micbias(struct snd_soc_dapm_widget *w, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	int micb_num;
			       ...
			       int adc_mux_n)
{
	...
	if (adc_mux_n < 3) {
		adc_mux_in_reg =
			AQT1000_CDC_TX_INP_MUX_ADC_MUX0_CFG1 + adc_mux_n;
		mask = 0x03;
		amic_mux_sel_reg =
			AQT1000_CDC_TX_INP_MUX_ADC_MUX0_CFG0 + 2 * adc_mux_n;
	}
...
static u16 aqt_codec_get_amic_pwlvl_reg(struct snd_soc_component *component,
					int amic)
{
	u16 pwr_level_reg = 0;
...
static void aqt_tx_mute_update_callback(struct work_struct *work)
{
	...
	tx_vol_ctl_reg =
		AQT1000_CDC_TX0_TX_PATH_CTL + 16 * tx_mute_dwork->decimator;
	hpf_gate_reg =
		AQT1000_CDC_TX0_TX_PATH_SEC2 + 16 * tx_mute_dwork->decimator;
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (amic_n)
			pwr_level_reg =
				aqt_codec_get_amic_pwlvl_reg(component, amic_n);
		if (pwr_level_reg) {
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		aqt->tx_hpf_work[decimator].hpf_cut_off_freq = hpf_cut_off_freq;
	...
	case SND_SOC_DAPM_PRE_PMD:
		hpf_cut_off_freq = aqt->tx_hpf_work[decimator].hpf_cut_off_freq;
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x10,
					      0x10);
		...
		cancel_delayed_work_sync(&aqt->tx_mute_dwork[decimator].dwork);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x10,
					      0x00);
				       ...
				       int event, int interp_idx)
{
	...
	dev_dbg(component->dev, "%s event %d main_clk_users %d\n", __func__,
		event, aqt->main_clk_users[interp_idx]);
...
static const char *const rx_int1_1_mux_text[] = {
	"ZERO",	 "MAIN_DMA_L", "I2S0_L",	"I2S0_R",    "DEC_L",
	"DEC_R", "DEC_V",      "SHADOW_I2S0_L", "MAIN_DMA_R"
};

static const char *const rx_int1_2_mux_text[] = {
	"ZERO",	 "MIX_DMA_L", "I2S0_L", "I2S0_R",   "DEC_L",
	"DEC_R", "DEC_V",     "IIR0",	"MIX_DMA_R"
};

static const char *const rx_int2_1_mux_text[] = {
	"ZERO",	 "MAIN_DMA_R", "I2S0_L",	"I2S0_R",    "DEC_L",
	"DEC_R", "DEC_V",      "SHADOW_I2S0_R", "MAIN_DMA_L"
};

static const char *const rx_int2_2_mux_text[] = {
	"ZERO",	 "MIX_DMA_R", "I2S0_L", "I2S0_R",   "DEC_L",
	"DEC_R", "DEC_V",     "IIR0",	"MIX_DMA_L"
};
					 ...
					 int interp, int path_type)
{
	...
	if (mux_reg_val == 0x02 || mux_reg_val == 0x03)
		bit_width = ((snd_soc_component_read32(component,
						       AQT1000_I2S_I2S_0_CTL) &
			      0x40) >>
			     6);
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	gain_reg = AQT1000_CDC_RX1_RX_VOL_CTL +
...
static int aqt_get_asrc_mode(struct aqt1000 *aqt, int asrc, u8 main_sr,
			     u8 mix_sr)
{
	u8 asrc_output_mode;
					   ...
					   int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if ((snd_soc_component_read32(component, clk_reg) & 0x02) ||
			...
			snd_soc_component_update_bits(component, clk_reg, 0x02,
						      0x00);
		...
		mix_sr = snd_soc_component_read32(component, mix_ctl_reg) &
			 0x0F;
		asrc_mode = aqt_get_asrc_mode(aqt, asrc, main_sr, mix_sr);
		dev_dbg(component->dev,
			"%s: main_sr:%d mix_sr:%d asrc_mode %d\n", __func__,
			main_sr, mix_sr, asrc_mode);
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		anc_size_remaining =
			cal_size - sizeof(struct aqt1000_anc_header);
		...
		for (i = 0; i < num_anc_slots; i++) {
			...
			anc_size_remaining -=
				(anc_writes_size * AQT1000_PACKED_REG_SIZE);
		...
		if (!strcmp(w->name, "AQT RX INT1 DAC")) {
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_RC_COMMON_CTL, 0x05,
				0x05);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_FIFO_COMMON_CTL,
				0x66, 0x66);
			anc_writes_size = anc_cal_size / 2;
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_CLK_RESET_CTL, 0x39,
				0x39);
		} else if (!strcmp(w->name, "AQT RX INT2 DAC")) {
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_RC_COMMON_CTL, 0x05,
				0x05);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_FIFO_COMMON_CTL,
				0x66, 0x66);
			i = anc_cal_size / 2;
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_CLK_RESET_CTL, 0x39,
				0x39);
		}
		...
		if (!strcmp(w->name, "AQT RX INT1 DAC"))
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_CLK_RESET_CTL, 0x08,
				0x08);
		else if (!strcmp(w->name, "AQT RX INT2 DAC"))
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_CLK_RESET_CTL, 0x08,
				0x08);
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_component_update_bits(
			component, AQT1000_CDC_ANC0_CLK_RESET_CTL, 0x08, 0x00);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_ANC1_CLK_RESET_CTL, 0x08, 0x00);
		break;

	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(
			component, AQT1000_CDC_ANC0_RC_COMMON_CTL, 0x05, 0x00);
		if (!strcmp(w->name, "AQT ANC HPHL PA")) {
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_MODE_1_CTL, 0x30,
				0x00);
       ...
       */
			msleep(50);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_MODE_1_CTL, 0x01,
				0x00);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_CLK_RESET_CTL, 0x38,
				0x38);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_CLK_RESET_CTL, 0x07,
				0x00);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC0_CLK_RESET_CTL, 0x38,
				0x00);
			...
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_MODE_1_CTL, 0x30,
				0x00);
			/* 50 msec sleep is needed to avoid click and pop as
       * per HW requirement
       */
			msleep(50);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_MODE_1_CTL, 0x01,
				0x00);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_CLK_RESET_CTL, 0x38,
				0x38);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_CLK_RESET_CTL, 0x07,
				0x00);
			snd_soc_component_update_bits(
				component, AQT1000_CDC_ANC1_CLK_RESET_CTL, 0x38,
				0x00);
			       ...
			       int event)
{
	if (mode == CLS_AB || mode == CLS_AB_HIFI) {
		...
		case SND_SOC_DAPM_POST_PMU:
			snd_soc_component_update_bits(
				component, AQT1000_ANA_RX_SUPPLIES, 0x02, 0x02);
			break;
		case SND_SOC_DAPM_POST_PMD:
			snd_soc_component_update_bits(
				component, AQT1000_ANA_RX_SUPPLIES, 0x02, 0x00);
			break;
		}
	}
}
				  ...
				  u16 amic_reg, bool set)
{
	...
	if (amic_reg == AQT1000_ANA_AMIC1 || amic_reg == AQT1000_ANA_AMIC3)
		mask = 0x40;
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, AQT1000_HPH_NEW_INT_HPH_TIMER1, 0x02, 0x00);

		aqt_clsh_fsm(component, &aqt->clsh_d, AQT_CLSH_EVENT_PRE_DAC,
			     AQT_CLSH_STATE_HPHL, hph_mode);

		if (aqt->anc_func)
			snd_soc_component_update_bits(
				component, AQT1000_CDC_RX1_RX_PATH_CFG0, 0x10,
				0x10);

		ret = aqt_mbhc_get_impedance(aqt->mbhc, &impedl, &impedr);
		if (!ret) {
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		usleep_range(1000, 1100);
		aqt_clsh_fsm(component, &aqt->clsh_d, AQT_CLSH_EVENT_POST_PA,
			     AQT_CLSH_STATE_HPHL, hph_mode);
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, AQT1000_HPH_NEW_INT_HPH_TIMER1, 0x02, 0x00);
		aqt_clsh_fsm(component, &aqt->clsh_d, AQT_CLSH_EVENT_PRE_DAC,
			     AQT_CLSH_STATE_HPHR, hph_mode);
		if (aqt->anc_func)
			snd_soc_component_update_bits(
				component, AQT1000_CDC_RX2_RX_PATH_CFG0, 0x10,
				0x10);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* 1000us required as per HW requirement */
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if ((!(strcmp(w->name, "AQT ANC HPHR PA"))) &&
			...
			snd_soc_component_update_bits(
				component, AQT1000_ANA_HPH, 0xC0, 0xC0);
     ...
     */
		...
		if (aqt->anc_func) {
			...
			if ((snd_soc_component_read32(aqt->component,
						      AQT1000_ANA_HPH) &
			     0xC0) == 0xC0)
				aqt_codec_clear_anc_tx_hold(aqt);
		}

		snd_soc_component_update_bits(component, AQT1000_HPH_R_TEST,
					      0x01, 0x01);

		/* Remove mute */
		snd_soc_component_update_bits(
			component, AQT1000_CDC_RX2_RX_PATH_CTL, 0x10, 0x00);
		/* Enable GM3 boost */
		snd_soc_component_update_bits(component, AQT1000_HPH_CNP_WG_CTL,
					      0x80, 0x80);
		/* Enable AutoChop timer at the end of power up */
		snd_soc_component_update_bits(
			component, AQT1000_HPH_NEW_INT_HPH_TIMER1, 0x02, 0x02);
		...
		if (!(strcmp(w->name, "AQT ANC HPHR PA"))) {
			...
			snd_soc_component_update_bits(
				component, AQT1000_HPH_L_TEST, 0x01, 0x01);

			/* Remove mute */
			snd_soc_component_update_bits(
				component, AQT1000_CDC_RX1_RX_PATH_CTL, 0x10,
				0x00);
			...
			if ((snd_soc_component_read32(
				...
				snd_soc_component_update_bits(
					component,
					AQT1000_CDC_RX1_RX_PATH_MIX_CTL, 0x10,
					0x00);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		snd_soc_component_update_bits(component, AQT1000_HPH_R_TEST,
					      0x01, 0x00);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_RX2_RX_PATH_CTL, 0x10, 0x10);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_RX2_RX_PATH_MIX_CTL, 0x10, 0x10);
		if (!(strcmp(w->name, "AQT ANC HPHR PA")))
			snd_soc_component_update_bits(
				component, AQT1000_ANA_HPH, 0x40, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/*
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
     ...
     */
		...
		snd_soc_component_update_bits(component, AQT1000_HPH_L_TEST,
					      0x01, 0x01);
		/* Remove Mute on primary path */
		snd_soc_component_update_bits(
			component, AQT1000_CDC_RX1_RX_PATH_CTL, 0x10, 0x00);
		/* Enable GM3 boost */
		snd_soc_component_update_bits(component, AQT1000_HPH_CNP_WG_CTL,
					      0x80, 0x80);
		/* Enable AutoChop timer at the end of power up */
		...
		/* Remove mix path mute if it is enabled */
		if ((snd_soc_component_read32(
			    component, AQT1000_CDC_RX1_RX_PATH_MIX_CTL)) &
		    0x10)
			snd_soc_component_update_bits(
				component, AQT1000_CDC_RX1_RX_PATH_MIX_CTL,
				0x10, 0x00);
		if (!(strcmp(w->name, "AQT ANC HPHL PA"))) {
			...
			/* Remove mute */
			snd_soc_component_update_bits(
				component, AQT1000_CDC_RX2_RX_PATH_CTL, 0x10,
				0x00);
			...
			if ((snd_soc_component_read32(
				...
				snd_soc_component_update_bits(
					component,
					AQT1000_CDC_RX2_RX_PATH_MIX_CTL, 0x10,
					0x00);
			/* Remove ANC Rx from reset */
			ret = aqt_codec_enable_anc(w, kcontrol, event);
		}
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		snd_soc_component_update_bits(component, AQT1000_HPH_L_TEST,
					      0x01, 0x00);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_RX1_RX_PATH_CTL, 0x10, 0x10);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_RX1_RX_PATH_MIX_CTL, 0x10, 0x10);
		if (!(strcmp(w->name, "AQT ANC HPHL PA")))
			snd_soc_component_update_bits(
				component, AQT1000_ANA_HPH, 0x80, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/*
     * 5ms sleep is required after PA disable. If compander is
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (++aqt->native_clk_users == 1) {
			snd_soc_component_update_bits(
				component, AQT1000_CLK_SYS_PLL_ENABLES, 0x01,
				0x01);
			/* 100usec is needed as per HW requirement */
			usleep_range(100, 120);
			snd_soc_component_update_bits(
				component,
				AQT1000_CDC_CLK_RST_CTRL_MCLK_CONTROL, 0x02,
				0x02);
			snd_soc_component_update_bits(
				component,
				AQT1000_CDC_CLK_RST_CTRL_FS_CNT_CONTROL, 0x10,
				0x10);
		}
		break;
	case SND_SOC_DAPM_PRE_PMD:
		if (aqt->native_clk_users && (--aqt->native_clk_users == 0)) {
			snd_soc_component_update_bits(
				component,
				AQT1000_CDC_CLK_RST_CTRL_FS_CNT_CONTROL, 0x10,
				0x00);
...
static const struct snd_kcontrol_new aif1_cap_mixer[] = {
	SOC_SINGLE_EXT("TX0", SND_SOC_NOPM, AQT_TX0, 1, 0, aif_cap_mixer_get,
		       aif_cap_mixer_put),
	SOC_SINGLE_EXT("TX1", SND_SOC_NOPM, AQT_TX1, 1, 0, aif_cap_mixer_get,
		       aif_cap_mixer_put),
};
...
static const struct snd_soc_dapm_widget aqt_dapm_widgets[] = {
	...
	SND_SOC_DAPM_AIF_OUT_E("AQT AIF1 CAP", "AQT AIF1 Capture", 0,
			       SND_SOC_NOPM, AIF1_CAP, 0,
			       aqt_codec_enable_i2s_tx,
			       SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SWITCH_E("AQT ANC OUT HPHL Enable", SND_SOC_NOPM,
			      INTERP_HPHL, 0, &anc_hphl_pa_switch,
			      aqt_anc_out_switch_cb,
			      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_SWITCH_E("AQT ANC OUT HPHR Enable", SND_SOC_NOPM,
			      INTERP_HPHR, 0, &anc_hphr_pa_switch,
			      aqt_anc_out_switch_cb,
			      SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
	...
	SND_SOC_DAPM_MUX_E("AQT ASRC0 MUX", SND_SOC_NOPM, ASRC0, 0, &asrc0_mux,
			   aqt_codec_enable_asrc_resampler,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("AQT ASRC1 MUX", SND_SOC_NOPM, ASRC1, 0, &asrc1_mux,
			   aqt_codec_enable_asrc_resampler,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_DAC_E("AQT RX INT1 DAC", NULL, AQT1000_ANA_HPH, 5, 0,
			   aqt_codec_hphl_dac_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_DAC_E("AQT RX INT2 DAC", NULL, AQT1000_ANA_HPH, 4, 0,
			   aqt_codec_hphr_dac_event,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_MIXER("AQT SRC0",
			   AQT1000_CDC_SIDETONE_SRC0_ST_SRC_PATH_CTL, 4, 0,
			   NULL, 0),

	SND_SOC_DAPM_MICBIAS_E("AQT MIC BIAS1", SND_SOC_NOPM, 0, 0,
			       aqt_codec_enable_micbias,
			       SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				       SND_SOC_DAPM_POST_PMD),
...
static int aqt_set_decimator_rate(struct snd_soc_dai *dai, u32 sample_rate)
{
	...
	tx_mux_sel = snd_soc_component_read32(
		component, AQT1000_CDC_IF_ROUTER_TX_MUX_CFG0);
	tx0_mux_sel = (tx_mux_sel & 0x03);
	tx1_mux_sel = (tx_mux_sel & 0xC0);

	if (tx0_mux_sel) {
		tx_path_ctl_reg =
			AQT1000_CDC_TX0_TX_PATH_CTL + ((tx0_mux_sel - 1) * 16);
		snd_soc_component_update_bits(component, tx_path_ctl_reg, 0x0F,
					      tx_fs_rate);
	}

	if (tx1_mux_sel) {
		tx_path_ctl_reg =
			AQT1000_CDC_TX0_TX_PATH_CTL + ((tx1_mux_sel - 1) * 16);
		snd_soc_component_update_bits(component, tx_path_ctl_reg, 0x0F,
					      tx_fs_rate);
	}
...
static int aqt_set_interpolator_rate(struct snd_soc_dai *dai, u32 sample_rate)
{
	struct snd_soc_component *component = dai->component;
	int rate_val = 0;
	int i;
	...
	snd_soc_component_update_bits(
		component, AQT1000_CDC_RX1_RX_PATH_MIX_CTL, 0x0F, (u8)rate_val);
	snd_soc_component_update_bits(
		component, AQT1000_CDC_RX2_RX_PATH_MIX_CTL, 0x0F, (u8)rate_val);
...
static int aqt_enable_mclk(struct aqt1000 *aqt)
{
	...
	if (++aqt->mclk_users == 1) {
		/* Set clock div 2 */
		snd_soc_component_update_bits(
			component, AQT1000_CLK_SYS_MCLK1_PRG, 0x0C, 0x04);
		snd_soc_component_update_bits(
			component, AQT1000_CLK_SYS_MCLK1_PRG, 0x10, 0x10);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_CLK_RST_CTRL_FS_CNT_CONTROL,
			0x01, 0x01);
		snd_soc_component_update_bits(
			component, AQT1000_CDC_CLK_RST_CTRL_MCLK_CONTROL, 0x01,
			0x01);
		/*
...
static int aqt_disable_mclk(struct aqt1000 *aqt)
{
	...
	if (--aqt->mclk_users == 0) {
		...
		snd_soc_component_update_bits(
			...
			0x01, 0x00);
		snd_soc_component_update_bits(
			component, AQT1000_CLK_SYS_MCLK1_PRG, 0x10, 0x00);
	}
...
static int aqt_enable_master_bias(struct aqt1000 *aqt)
{
	...
	if (aqt->master_bias_users == 1) {
		snd_soc_component_update_bits(component, AQT1000_ANA_BIAS, 0x80,
					      0x80);
		snd_soc_component_update_bits(component, AQT1000_ANA_BIAS, 0x40,
					      0x40);
		/*
...
static int aqt_cdc_req_mclk_enable(struct aqt1000 *aqt, bool enable)
{
	int ret = 0;
...
static int __aqt_cdc_mclk_enable_locked(struct aqt1000 *aqt, bool enable)
{
	int ret = 0;
...
static int __aqt_cdc_mclk_enable(struct aqt1000 *aqt, bool enable)
{
	int ret;
...
static int aqt_set_micbias(struct aqt1000 *aqt, struct aqt1000_pdata *pdata)
{
	struct snd_soc_component *component = aqt->component;
	int vout_ctl_1;
	...
	snd_soc_component_update_bits(component, AQT1000_ANA_MICB1, 0x3F,
				      vout_ctl_1);
				      ...
				      struct snd_soc_component *component)
{
	...
	card = component->card;
	aqt->entry = snd_info_create_subdir(codec_root->module, "aqt1000",
					    codec_root);
	if (!aqt->entry) {
	...
	if (!version_entry) {
		dev_dbg(component->dev,
			"%s: failed to create aqt1000 version entry\n",
			__func__);
		return -ENOMEM;
	}
...
static int aqt_soc_codec_probe(struct snd_soc_component *component)
{
	...
	ret = wcd_cal_create_hwdep(aqt->fw_data, AQT1000_CODEC_HWDEP_NODE,
				   component);
	if (ret < 0) {
	...
	for (i = 0; i < pdata->num_supplies; i++) {
		if (!strcmp(pdata->regulator->name, "aqt_vdd1p8")) {
			snd_soc_component_update_bits(
				component, AQT1000_BUCK_5V_EN_CTL, 0x03, 0x00);
			dev_dbg(component->dev,
				"%s: Disabled internal supply\n", __func__);
			break;
		}
	}
	...
	snd_soc_dapm_add_routes(dapm, aqt_audio_map, ARRAY_SIZE(aqt_audio_map));
