...
/* Copyright (c) 2012-2018, 2020-2021, The Linux Foundation. All rights
 * reserved.
 */
...
static int wcd9xxx_alloc_slim_sh_ch(struct wcd9xxx *wcd9xxx, u8 wcd9xxx_pgd_la,
				    u32 cnt, struct wcd9xxx_ch *channels,
				    u32 path);

static int wcd9xxx_dealloc_slim_sh_ch(struct slim_device *slim, u32 cnt,
				      struct wcd9xxx_ch *channels);
...
int wcd9xxx_deinit_slimslave(struct wcd9xxx *wcd9xxx)
{
	if (wcd9xxx->num_rx_port) {
		wcd9xxx_dealloc_slim_sh_ch(wcd9xxx->slim, wcd9xxx->num_rx_port,
					   wcd9xxx->rx_chs);
		wcd9xxx->num_rx_port = 0;
	}
	if (wcd9xxx->num_tx_port) {
		wcd9xxx_dealloc_slim_sh_ch(wcd9xxx->slim, wcd9xxx->num_tx_port,
					   wcd9xxx->tx_chs);
		wcd9xxx->num_tx_port = 0;
	}
	return 0;
				    ...
				    u32 path)
{
   ...
   */
	...
	for (ch_idx = 0; ch_idx < cnt; ch_idx++) {
		...
		ret = slim_query_ch(wcd9xxx->slim, channels[ch_idx].ch_num,
			    ...
			    u16 *grph)
{
	...
	list_for_each_entry(rx, wcd9xxx_ch_list, list) {
		...
		pr_debug("%s: codec_port %d rx 0x%p, payload %d\n"
			 ...
			 sh_ch.rx_port_ch_reg_base, sh_ch.port_rx_cfg_reg_base);
     ...
     */
		...
		ret = wcd9xxx_interface_reg_write(
			wcd9xxx,
			SB_PGD_PORT_CFG_BYTE_ADDR(sh_ch.port_rx_cfg_reg_base,
						  codec_port),
			WATER_MARK_VAL);
		if (ret < 0) {
			    ...
			    u16 *grph)
{
	...
	list_for_each_entry(tx, wcd9xxx_ch_list, list) {
		...
		pr_debug("%s: codec_port %d tx 0x%p, payload 0x%x\n", __func__,
			 codec_port, tx, payload);
		/* write to interface device */
		ret = wcd9xxx_interface_reg_write(
			wcd9xxx, SB_PGD_TX_PORT_MULTI_CHANNEL_0(codec_port),
			payload & 0x00FF);
		if (ret < 0) {
			pr_err("%s:Intf-dev fail reg[%d] payload[%d] ret[%d]\n",
			       __func__,
		...
		/* ports 8,9 */
		ret = wcd9xxx_interface_reg_write(
			wcd9xxx, SB_PGD_TX_PORT_MULTI_CHANNEL_1(codec_port),
			(payload & 0xFF00) >> 8);
		if (ret < 0) {
			pr_err("%s:Intf-dev fail reg[%d] payload[%d] ret[%d]\n",
			       __func__,
...
int wcd9xxx_rx_vport_validation(u32 port_id, struct list_head *codec_dai_list)
{
	...
	list_for_each_entry(ch, codec_dai_list, list) {
				...
				u32 num_codec_dais)
{
	...
	for_each_set_bit(index, &vtable, size) {
		if (index < num_codec_dais) {
			list_for_each_entry(
				ch, &codec_dai[index].wcd9xxx_ch_list, list) {
				pr_debug(
					"%s: index %u ch->port %u vtable 0x%lx\n",
			...
			pr_err("%s: Invalid index %d of codec dai", __func__,
			       index);
			ret = -EINVAL;
		}
		if (ret)
