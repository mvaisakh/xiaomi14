...
static int wcd938x_slave_bind(struct device *dev, struct device *master,
			      void *data)
{
	int ret = 0;
	uint8_t devnum = 0;
...
static void wcd938x_slave_unbind(struct device *dev, struct device *master,
				 void *data)
{
	struct wcd938x_slave_priv *wcd938x_slave = NULL;
	struct swr_device *pdev = to_swr_device(dev);
...
static const struct swr_device_id wcd938x_swr_id[] = { { "wcd938x-slave", 0 },
						       {} };
...
static int wcd938x_swr_probe(struct swr_device *pdev)
{
	...
	wcd938x_slave = devm_kzalloc(
		&pdev->dev, sizeof(struct wcd938x_slave_priv), GFP_KERNEL);
	if (!wcd938x_slave)
		return -ENOMEM;
	...
	if (!wcd938x_slave->debugfs_wcd938x_dent) {
		...
		if (!IS_ERR(wcd938x_slave->debugfs_wcd938x_dent)) {
			wcd938x_slave->debugfs_peek = debugfs_create_file(
				"swrslave_peek", S_IFREG | 0444,
				wcd938x_slave->debugfs_wcd938x_dent,
				(void *)pdev, &codec_debug_read_ops);

			wcd938x_slave->debugfs_poke = debugfs_create_file(
				"swrslave_poke", S_IFREG | 0444,
				wcd938x_slave->debugfs_wcd938x_dent,
				(void *)pdev, &codec_debug_write_ops);

			wcd938x_slave->debugfs_reg_dump = debugfs_create_file(
