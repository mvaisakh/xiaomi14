...
static const struct of_device_id kalama_asoc_machine_of_match[] = {
	{ .compatible = "qcom,kalama-asoc-snd", .data = "codec" },
	{ .compatible = "qcom,kalama-asoc-snd-stub", .data = "stub_codec" },
	{},
};
...
static int msm_rx_tx_codec_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (codec_variant == WCD9385)
		ret = lpass_cdc_rx_set_fir_capability(lpass_cdc_component,
						      true);
	else
		ret = lpass_cdc_rx_set_fir_capability(lpass_cdc_component,
						      false);
...
static int msm_audio_ssr_register(struct device *dev)
{
	...
	ret = snd_event_master_register(dev, &kalama_ssr_ops, ssr_clients,
					NULL);
	if (!ret)
