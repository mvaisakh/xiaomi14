					...
					struct msm_asoc_mach_data *pdata)
{
	...
	ret = of_property_read_u32_array(pdev->dev.of_node,
					 "qcom,upd_lpass_reg_addr", dt_values,
					 MAX_EARPA_REG);
	if (ret) {
		pr_debug("%s: could not find %s entry in dt\n", __func__,
		...
		pdata->upd_config.ear_pa_hw_reg_cfg
			.lpass_cdc_rx0_rx_path_ctl_phy_addr = dt_values[0];
...
static void msm_set_upd_config(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!pdata->upd_config.ear_pa_hw_reg_cfg
		     .lpass_cdc_rx0_rx_path_ctl_phy_addr ||
	    !pdata->upd_config.ear_pa_hw_reg_cfg.lpass_wr_fifo_reg_phy_addr ||
	    !pdata->upd_config.ear_pa_pkd_reg_addr) {
	...
	if (!strcmp(pdata->upd_config.backend_used, "wsa")) {
		...
		if (pdata->wcd_used == WCD937X_DEV_INDEX)
			strscpy(wcd_name, WCD937X_DRV_NAME,
				sizeof(WCD937X_DRV_NAME));
		else
			strscpy(wcd_name, WCD939X_DRV_NAME,
				sizeof(WCD939X_DRV_NAME));
   ...
   */
	...
	if (ret < 0) {
		pr_err_ratelimited(
			"%s: upd cdc duty cycling registration failed\n",
			__func__);
		return;
	}
...
static int msm_wcn_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = snd_soc_dai_set_channel_map(codec_dai, ARRAY_SIZE(tx_ch), tx_ch,
...
msm_snd_info_create_subdir(struct module *mod, const char *name,
...
static struct snd_soc_dai_link msm_pineapple_dai_links
	[ARRAY_SIZE(msm_wsa_cdc_dma_be_dai_links) +
	 ...
	 ARRAY_SIZE(msm_wcn_be_dai_links) + ARRAY_SIZE(msm_mi2s_dai_links) +
...
static struct snd_soc_dai_link
	msm_stub_dai_links[ARRAY_SIZE(msm_stub_be_dai_links)];

static const struct of_device_id pineapple_asoc_machine_of_match[] = {
	{ .compatible = "qcom,pineapple-asoc-snd", .data = "codec" },
	{ .compatible = "qcom,pineapple-asoc-snd-stub", .data = "stub_codec" },
	{},
};
						       ...
						       int wsa_max_devs)
{
	...
	if (!match) {
		dev_err_ratelimited(dev,
				    "%s: No DT match found for sound card\n",
				    __func__);
		return NULL;
	}
	...
	if (!strcmp(match->data, "codec")) {
		...
		if (!rc && val) {
			memcpy(msm_pineapple_dai_links + total_links,
			       msm_mi2s_dai_links, sizeof(msm_mi2s_dai_links));
		...
		if (!rc && val) {
			memcpy(msm_pineapple_dai_links + total_links,
			       msm_tdm_dai_links, sizeof(msm_tdm_dai_links));
		...
		memcpy(msm_stub_dai_links, msm_stub_be_dai_links,
...
static int msm_int_wsa884x_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	u8 spkleft_port_types[WSA884X_MAX_SWR_PORTS] = {
		SPKR_L, SPKR_L_COMP, SPKR_L_BOOST, PBR, SPKR_L_VI, CPS
	};
	u8 spkright_port_types[WSA884X_MAX_SWR_PORTS] = {
		SPKR_R, SPKR_R_COMP, SPKR_R_BOOST, PBR, SPKR_R_VI, CPS
	};
	unsigned int ch_rate[WSA884X_MAX_SWR_PORTS] = {
		SWR_CLK_RATE_2P4MHZ, SWR_CLK_RATE_0P6MHZ, SWR_CLK_RATE_0P3MHZ,
		SWR_CLK_RATE_48KHZ,  SWR_CLK_RATE_1P2MHZ, SWR_CLK_RATE_24KHZ
	};
	unsigned int ch_mask[WSA884X_MAX_SWR_PORTS] = { 0x1, 0xF, 0x3,
							0x1, 0x3, 0x3 };
...
static int msm_rx_tx_codec_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	lpass_cdc_set_port_map(lpass_cdc_component, ARRAY_SIZE(sm_port_map),
			       sm_port_map);
	...
	lpass_cdc_info_create_codec_entry(pdata->codec_root,
					  lpass_cdc_component);
	...
	if (pdata->wcd_used == WCD937X_DEV_INDEX) {
		...
		if (codec_variant == WCD9395)
			ret = lpass_cdc_rx_set_fir_capability(
				lpass_cdc_component, true);
		else
			ret = lpass_cdc_rx_set_fir_capability(
				lpass_cdc_component, false);
	}

	if (ret < 0) {
		dev_err_ratelimited(component->dev,
				    "%s: set fir capability failed: %d\n",
				    __func__, ret);
		return ret;
	}
done:
...
static int pineapple_ssr_enable(struct device *dev, void *data)
{
	...
	struct snd_soc_pcm_runtime *rtd = NULL, *rtd_wcd = NULL,
				   *rtd_wsa = NULL;
	struct msm_asoc_mach_data *pdata = NULL;
	int ret = 0;
	...
	if (!rtd_wcd) {
		dev_dbg(dev, "%s: snd_soc_get_pcm_runtime for %s failed!\n",
			__func__, card->dai_link[0]);
	}

	if (pdata->wsa_max_devs > 0) {
		rtd_wsa = snd_soc_get_pcm_runtime(
			card, &card->dai_link[ARRAY_SIZE(
				      msm_rx_tx_cdc_dma_be_dai_links)]);
		if (!rtd_wsa) {
			dev_dbg(dev,
				"%s: snd_soc_get_pcm_runtime for %s failed!\n",
				__func__,
				card->dai_link[ARRAY_SIZE(
					msm_rx_tx_cdc_dma_be_dai_links)]);
		}
	}
	/* set UPD configuration */
	if (!pdata->upd_config.backend_used) {
		dev_dbg(dev, "%s: upd- backend_used is NULL\n", __func__);
		goto err;
	}
	if (!strcmp(pdata->upd_config.backend_used, "wsa")) {
		if (!rtd_wsa)
		...
		dev_err_ratelimited(card->dev,
				    "%s: Invalid backend to set UPD config\n",
				    __func__);
		goto err;
	}
...
static int msm_audio_ssr_register(struct device *dev)
{
	...
	for (i = 0;; i++) {
		...
		if (!node)
			break;
		snd_event_mstr_add_client(&ssr_clients, msm_audio_ssr_compare,
					  node);
	}

	ret = snd_event_master_register(dev, &pineapple_ssr_ops, ssr_clients,
					NULL);
	if (!ret)
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "qcom,wcd-disabled",
			     &pdata->wcd_disabled);
	...
	match = of_match_node(pineapple_asoc_machine_of_match,
			      pdev->dev.of_node);
	if (!match) {
	...
	if (!strcmp(match->data, "codec")) {
		...
		if (ret) {
			dev_err(&pdev->dev,
				"%s: parse audio routing failed, err:%d\n",
				__func__, ret);
			goto err;
		}
	}
	...
	pdata->wcd_usbss_handle =
		of_parse_phandle(pdev->dev.of_node, "wcd939x-i2c-handle", 0);
	if (!pdata->wcd_usbss_handle)
