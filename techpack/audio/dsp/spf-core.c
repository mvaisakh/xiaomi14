...
static int spf_core_callback(struct gpr_device *adev, void *data)
{
	...
	dev_info_ratelimited(&adev->dev, "%s: Payload %x", __func__,
			     hdr->opcode);
	switch (hdr->opcode) {
	case GPR_IBASIC_RSP_RESULT:
		basic_rsp = GPR_PKT_GET_PAYLOAD(struct spf_cmd_basic_rsp, data);
		dev_info_ratelimited(&adev->dev, "%s: op %x status %d",
				     __func__, basic_rsp->opcode,
				     basic_rsp->status);
		if (basic_rsp->opcode == APM_CMD_CLOSE_ALL) {
			core->status = basic_rsp->status;
		} else {
			dev_err_ratelimited(&adev->dev,
					    "%s: Failed response received",
					    __func__);
		}
		core->resp_received = true;
		break;
	case APM_CMD_RSP_GET_SPF_STATE:
		spf_status_rsp = GPR_PKT_GET_PAYLOAD(
			struct apm_cmd_rsp_get_spf_status_t, data);
		dev_info_ratelimited(&adev->dev, "%s: sucess response received",
				     __func__);
...
static bool __spf_core_is_apm_ready(struct spf_core *core)
{
	...
	if (rc > 0 && core->resp_received) {
		...
		dev_err_ratelimited(spf_core_priv->dev,
				    "%s: command timedout, ret\n", __func__);
	}
done:
...
static void spf_core_add_child_devices(struct work_struct *work)
{
	...
	ret = of_platform_populate(spf_core_priv->dev->of_node, NULL, NULL,
				   spf_core_priv->dev);
	if (ret)
		dev_err(spf_core_priv->dev,
...
static int spf_core_platform_driver_probe(struct platform_device *pdev)
{
	...
	spf_core_priv = devm_kzalloc(
		&pdev->dev, sizeof(struct spf_core_private), GFP_KERNEL);
	if (!spf_core_priv)
		return -ENOMEM;
...
static struct platform_driver
	spf_core_driver = { .probe = spf_core_platform_driver_probe,
