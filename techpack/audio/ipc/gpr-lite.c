...
int gpr_send_pkt(struct gpr_device *adev, struct gpr_pkt *pkt)
{
	...
	if (!gpr) {
		pr_err_ratelimited(
			"%s: Failed to get gpr dev pointer : gpr[%pK] \n",
			__func__, gpr);
		return -EINVAL;
	}
				   ...
				   unsigned long opcode, void *data)
{
	...
	if (cb_data == NULL) {
		dev_err_ratelimited(gpr_priv->dev,
				    "%s: Callback data is NULL!\n", __func__);
		goto done;
	}
...
static int gpr_callback(struct rpmsg_device *rpdev, void *buf, int len,
			void *priv, u32 addr)
{
	...
	if (len <= GPR_HDR_SIZE) {
		dev_err_ratelimited(gpr->dev,
				    "GPR: Improper gpr pkt received:%p %d\n",
				    buf, len);
		return -EINVAL;
	}
	...
	dev_dbg(gpr->dev, "%s: dst_port %x hdr_size %d pkt_size %d\n", __func__,
		hdr->dst_port, hdr_size, pkt_size);
			  ...
			  const struct gpr_device_id *id)
{
	...
	dev_set_name(&adev->dev, "gprsvc:%s:%x:%x", adev->name, id->domain_id,
		     id->svc_id);
	...
	spin_lock(&gpr->svcs_lock);
	idr_alloc(&gpr->svcs_idr, adev, id->svc_id, id->svc_id + 1, GFP_ATOMIC);
	spin_unlock(&gpr->svcs_lock);
...
static const struct of_device_id gpr_of_match[] = { { .compatible =
							      "qcom,gpr" },
						    {} };
