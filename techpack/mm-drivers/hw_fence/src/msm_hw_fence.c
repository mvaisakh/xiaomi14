			    ...
			    struct msm_hw_fence_mem_addr *mem_descriptor)
{
	...
	if (IS_ERR_OR_NULL(hw_fence_drv_data) ||
	    !hw_fence_drv_data->resources_ready) {
	...
	client_id = hw_fence_utils_get_client_id_priv(hw_fence_drv_data,
						      client_id_ext);
   ...
   */
	...
	if (hw_fence_client->ipc_client_vid <= 0 ||
	    hw_fence_client->ipc_client_pid <= 0) {
		HWFNC_ERR("Failed to find client:%d ipc vid:%d pid:%d\n",
			  client_id, hw_fence_client->ipc_client_vid,
			  hw_fence_client->ipc_client_pid);
		ret = -EINVAL;
		goto error;
	}
	...
	hw_fence_client->ipc_signal_id =
		hw_fence_ipcc_get_signal_id(hw_fence_drv_data, client_id);
	...
	hw_fence_client->update_rxq =
		hw_fence_ipcc_needs_rxq_update(hw_fence_drv_data, client_id);
	hw_fence_client->send_ipc =
		hw_fence_ipcc_needs_ipc_irq(hw_fence_drv_data, client_id);

	hw_fence_client->queues_num =
		hw_fence_utils_get_queues_num(hw_fence_drv_data, client_id);
	...
	ret = hw_fence_init_controller_signal(hw_fence_drv_data,
					      hw_fence_client);
	if (ret)
		goto error;
   ...
   */
	...
	HWFNC_DBG_INIT(
		"Initialized ptr:0x%p client_id:%d q_num:%d ipc signal:%d "
		"vid:%d pid:%d\n",
		hw_fence_client, hw_fence_client->client_id,
		hw_fence_client->queues_num, hw_fence_client->ipc_signal_id,
		hw_fence_client->ipc_client_vid,
		hw_fence_client->ipc_client_pid);
			...
			struct msm_hw_fence_create_params *params)
{
	...
	if (IS_ERR_OR_NULL(client_handle) || !params || !params->fence ||
	    !params->handle) {
...
int msm_hw_fence_destroy(void *client_handle, struct dma_fence *fence)
{
	struct msm_hw_fence_client *hw_fence_client;
	struct dma_fence_array *array;
	int ret;
...
int msm_hw_fence_destroy_with_handle(void *client_handle, u64 handle)
{
	...
	/* Destroy the HW Fence, i.e. remove entry in the Global Table for the Fence
   */
	ret = hw_fence_destroy_with_hash(hw_fence_drv_data, hw_fence_client,
					 handle);
	if (ret) {
		HWFNC_ERR(
			"Error destroying the HW fence handle:%llu client_id:%d\n",
			handle, hw_fence_client->client_id);
		return ret;
	}
...
int msm_hw_fence_wait_update_v2(void *client_handle,
				struct dma_fence **fence_list, u64 *handles,
				u64 *client_data_list, u32 num_fences,
				bool create)
{
	struct msm_hw_fence_client *hw_fence_client;
	struct dma_fence_array *array;
	...
	if (client_data_list && data_id >= HW_FENCE_MAX_CLIENTS_WITH_DATA) {
		HWFNC_ERR(
			"Populating non-NULL client_data_list with invalid client_id_ext:%d\n",
	...
	if (hw_fence_client->client_id > hw_fence_drv_data->rxq_clients_num) {
		HWFNC_ERR(
			"Transmit-only client client_id:%d client_id_ext:%d register for "
			"wait\n",
			hw_fence_client->client_id,
			hw_fence_client->client_id_ext);
		return -EINVAL;
	...
	for (i = 0; i < num_fences; i++) {
		...
		array = to_dma_fence_array(fence);
		if (array) {
			ret = hw_fence_process_fence_array(hw_fence_drv_data,
							   hw_fence_client,
							   array, &hash,
							   client_data);
			if (ret) {
			...
			ret = hw_fence_process_fence(hw_fence_drv_data,
						     hw_fence_client, fence,
						     &hash, client_data);
			if (ret) {
...
int msm_hw_fence_wait_update(void *client_handle, struct dma_fence **fence_list,
			     u32 num_fences, bool create)
{
	return msm_hw_fence_wait_update_v2(client_handle, fence_list, NULL,
					   NULL, num_fences, create);
}
...
int msm_hw_fence_reset_client_by_id(enum hw_fence_client_id client_id_ext,
				    u32 reset_flags)
{
	enum hw_fence_client_id client_id;
...
int msm_hw_fence_update_txq(void *client_handle, u64 handle, u64 flags,
			    u32 error)
{
	struct msm_hw_fence_client *hw_fence_client;
	...
	if (IS_ERR_OR_NULL(hw_fence_drv_data) ||
		...
		HWFNC_ERR("Invalid handle:%d or client handle:%d max:%d\n",
			  handle, IS_ERR_OR_NULL(client_handle),
	...
	hw_fence_update_queue(hw_fence_drv_data, hw_fence_client,
			      ...
			      hw_fence_drv_data->hw_fences_tbl[handle].seq_id,
			      handle, flags, 0, error, HW_FENCE_TX_QUEUE - 1);

	return 0;
}
EXPORT_SYMBOL(msm_hw_fence_update_txq);

int msm_hw_fence_update_txq_error(void *client_handle, u64 handle, u32 error,
				  u32 update_flags)
{
	struct msm_hw_fence_client *hw_fence_client;
	...
	if (IS_ERR_OR_NULL(hw_fence_drv_data) ||
		...
		HWFNC_ERR(
			"Invalid client_handle:0x%pK or fence handle:%d max:%d or error:%d\n",
			client_handle, handle,
			hw_fence_drv_data->hw_fences_tbl_cnt, error);
		return -EINVAL;
	} else if (update_flags != MSM_HW_FENCE_UPDATE_ERROR_WITH_MOVE) {
		HWFNC_ERR(
			"invalid flags:0x%x expected:0x%x no support of in-place error "
			"update\n",
...
int msm_hw_fence_trigger_signal(void *client_handle, u32 tx_client_pid,
				u32 rx_client_vid, u32 signal_id)
{
	struct msm_hw_fence_client *hw_fence_client;
...
int msm_hw_fence_register_error_cb(void *client_handle,
				   msm_hw_fence_error_cb_t cb, void *data)
{
	struct msm_hw_fence_client *hw_fence_client;
	...
	if (IS_ERR_OR_NULL(hw_fence_drv_data) ||
		...
		HWFNC_ERR("hw fence driver not ready\n");
		return -EAGAIN;
	} else if (IS_ERR_OR_NULL(client_handle) || IS_ERR_OR_NULL(cb) ||
		   IS_ERR_OR_NULL(data)) {
		HWFNC_ERR(
			"Invalid params client:0x%pK cb_func:0x%pK data:0x%pK\n",
			client_handle, cb, data);
		return -EINVAL;
	}
	...
	if (hw_fence_client->fence_error_cb) {
		HWFNC_ERR(
			"client_id:%d client_id_ext:%d already registered cb_func:%pK "
			"data:%pK\n",
			hw_fence_client->client_id,
			hw_fence_client->client_id_ext,
...
int msm_hw_fence_deregister_error_cb(void *client_handle)
{
	...
	if (!mutex_trylock(&hw_fence_client->error_cb_lock)) {
		HWFNC_ERR(
			"client_id:%d is modifying or using fence_error_cb:0x%pK data:0x%pK\n",
			hw_fence_client->client_id,
			hw_fence_client->fence_error_cb,
			hw_fence_client->fence_error_cb_userdata);
		return -EAGAIN;
	...
	if (!hw_fence_client->fence_error_cb) {
		HWFNC_ERR(
			"client_id:%d client_id_ext:%d did not register cb:%pK data:%pK\n",
			hw_fence_client->client_id,
			hw_fence_client->client_id_ext,
			hw_fence_client->fence_error_cb,
			hw_fence_client->fence_error_cb_userdata);
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
int msm_hw_fence_dump_debug_data(void *client_handle, u32 dump_flags,
				 u32 dump_clients_mask)
{
	...
	if (dump_flags & MSM_HW_FENCE_DBG_DUMP_QUEUES) {
		...
		if (dump_clients_mask)
			for (client_id = 0; client_id < HW_FENCE_CLIENT_MAX;
			     client_id++)
				if ((dump_clients_mask & (1 << client_id)) &&
				    hw_fence_drv_data->clients[client_id])
					hw_fence_debug_dump_queues(
						HW_FENCE_PRINTK,
						hw_fence_drv_data
							->clients[client_id]);
	}
...
int msm_hw_fence_dump_fence(void *client_handle, struct dma_fence *fence)
{
	...
	if (IS_ERR_OR_NULL(hw_fence_drv_data) ||
		...
		HWFNC_ERR(
			"DMA Fence is not a HW Fence ctx:%llu seqno:%llu flags:0x%llx\n",
			fence->context, fence->seqno, fence->flags);
		return -EINVAL;
	}
	hw_fence_client = (struct msm_hw_fence_client *)client_handle;

	hw_fence = msm_hw_fence_find(hw_fence_drv_data, hw_fence_client,
				     fence->context, fence->seqno, &hash);
	if (!hw_fence) {
		HWFNC_ERR(
			"failed to find hw-fence client_id:%d fence:0x%pK ctx:%llu "
			"seqno:%llu\n",
			hw_fence_client->client_id, fence, fence->context,
			fence->seqno);
		return -EINVAL;
	}
...
int msm_hw_fence_driver_doorbell_sim(u64 db_mask)
{
	...
	HWFNC_DBG_IRQ("db callback sim-mode flags:0x%llx qtime:%llu\n", db_mask,
		      hw_fence_get_qtime(hw_fence_drv_data));
