...
static enum hw_fence_client_id
_get_hw_fence_client_id(enum synx_client_id synx_client_id)
{
...
struct synx_session *
synx_hwfence_initialize(struct synx_initialization_params *params)
{
	...
	client_handle = msm_hw_fence_register(
		client_id, (struct msm_hw_fence_mem_addr *)params->ptr);
	if (IS_ERR_OR_NULL(client_handle)) {
		kfree(session);
		HWFNC_ERR("failed to initialize synx_id:%d ret:%d\n",
			  params->id, PTR_ERR(client_handle));
...
int synx_hwfence_create(struct synx_session *session,
			struct synx_create_params *params)
{
	...
	if (IS_ERR_OR_NULL(params->h_synx) ||
		...
		HWFNC_ERR(
			"synx_id:%d invalid create params h_synx:0x%pK flags:0x%x "
			"fence:0x%pK\n",
	...
	if (ret) {
		HWFNC_ERR(
			"synx_id:%d failed create fence:0x%pK flags:0x%x ret:%d\n",
			session->type, params->fence, params->flags, ret);
		return to_synx_status(ret);
	}
	if (handle > U32_MAX) {
		HWFNC_ERR(
			"synx_id:%d fence handle:%llu would overflow h_synx\n",
			session->type, handle);
...
int synx_hwfence_release(struct synx_session *session, u32 h_synx)
{
	...
	if (ret)
		HWFNC_ERR(
			"synx_id:%d failed to destroy fence h_synx:%u ret:%d\n",
			session->type, h_synx, ret);

	return to_synx_status(ret);
}
EXPORT_SYMBOL(synx_hwfence_release);

int synx_hwfence_signal(struct synx_session *session, u32 h_synx,
			enum synx_signal_status status)
{
	...
	if (ret)
		HWFNC_ERR(
			"synx_id:%d failed to signal fence h_synx:%u status:%d ret:%d\n",
...
int synx_hwfence_recover(enum synx_client_id id)
{
	...
	ret = msm_hw_fence_reset_client_by_id(
		_get_hw_fence_client_id(id),
				    ...
				    struct synx_import_indv_params *params)
{
	...
	if (IS_ERR_OR_NULL(client) || IS_ERR_OR_NULL(params) ||
	    ...
	    IS_ERR_OR_NULL(params->fence)) {
		HWFNC_ERR(
			"invalid client:0x%pK params:0x%pK h_synx:0x%pK flags:0x%x "
			"fence:0x%pK\n",
			client, params,
			IS_ERR_OR_NULL(params) ? NULL : params->new_h_synx,
			IS_ERR_OR_NULL(params) ? 0 : params->flags,
			IS_ERR_OR_NULL(params) ? NULL : params->fence);
		return -SYNX_INVALID;
	}

	ret = msm_hw_fence_wait_update_v2(client,
					  (struct dma_fence **)&params->fence,
					  &handle, NULL, 1, true);
	if (ret) {
	...
	if (handle > U32_MAX) {
		HWFNC_ERR("fence handle:%llu would overflow new_h_synx\n",
			  handle);
		msm_hw_fence_wait_update_v2(client,
					    (struct dma_fence **)&params->fence,
					    &handle, NULL, 1, false);
		return -SYNX_INVALID;
	}
...
static int synx_hwfence_import_arr(void *client,
				   struct synx_import_arr_params *params)
{
	...
	if (IS_ERR_OR_NULL(client) || IS_ERR_OR_NULL(params) ||
	    !params->num_fences) {
		HWFNC_ERR(
			"invalid import arr client:0x%pK params:0x%pK num_fences:%u\n",
			client, params,
			IS_ERR_OR_NULL(params) ? -1 : params->num_fences);
		return -SYNX_INVALID;
	}
...
int synx_hwfence_import(struct synx_session *session,
			struct synx_import_params *params)
{
	...
	if (ret)
		HWFNC_ERR("synx_id:%d failed to import type:%s fences ret:%d\n",
			  session->type,
			  (params->type == SYNX_IMPORT_ARR_PARAMS) ? "arr" :
								     "indv",
			  ret);
