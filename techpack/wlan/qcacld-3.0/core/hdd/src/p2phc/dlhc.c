 ...
 */
static struct dlhc *__dlhc_init(struct dlhc *comp, int rslots, int tslots,
				unsigned char flags)
{
	short i;
	struct cstate *ts;
...
static int __dlhc_compress_tcp(struct dlhc *comp, unsigned char *icp, int isize,
			       unsigned char *ocp, unsigned char **cpp,
			       int compress_cid)
{
   ...
   */
	if (ip->version != cs->cs_ip.version || ip->ihl != cs->cs_ip.ihl ||
	    ...
	    (ip->frag_off & htons(0x4000)) !=
		    (cs->cs_ip.frag_off & htons(0x4000)) ||
		  ...
		  unsigned char *ocp, unsigned char **cpp, int compress_cid)
{
   ...
   */
	...
	if (ip->protocol == IPPROTO_TCP) {
		return __dlhc_compress_tcp(comp, icp, isize, ocp, cpp,
					   compress_cid);
	} else if (ip->protocol == IPPROTO_UDP) {
		return __dlhc_compress_udp(comp + 1, icp, isize, ocp, cpp,
					   compress_cid);
	}
...
static int __dlhc_uncompress_tcp(struct dlhc *comp, unsigned char *icp,
				 int isize)
{
...
static int __dlhc_uncompress_udp(struct dlhc *comp, unsigned char *icp,
				 int isize)
{
	unsigned char changes;
	long x;
...
{
	printk(KERN_DEBUG
	       "Called IP function on non IP-system: dlhc_uncompress");
	return -EINVAL;
}
int dlhc_compress(struct dlhc *comp, unsigned char *icp, int isize,
		  unsigned char *ocp, unsigned char **cpp, int compress_cid)
