...
static ssize_t __hdd_sysfs_stats_show(struct net_device *net_dev, char *buf)
{
...
static ssize_t hdd_sysfs_stats_show(struct device *dev,
...
static DEVICE_ATTR(stats, 0440, hdd_sysfs_stats_show, NULL);

static ssize_t __wlan_hdd_sysfs_dump_stats_store(struct net_device *net_dev,
...
static ssize_t __wlan_hdd_sysfs_dump_stats_show(struct net_device *net_dev,
						char *buf)
{
	struct hdd_adapter *adapter = netdev_priv(net_dev);
	struct hdd_context *hdd_ctx;
...
static ssize_t __wlan_hdd_sysfs_clear_stats_store(struct net_device *net_dev,
						  char const *buf, size_t count)
{
	struct hdd_adapter *adapter = netdev_priv(net_dev);
	struct hdd_context *hdd_ctx;
...
static DEVICE_ATTR(clear_stats, 0220, NULL, wlan_hdd_sysfs_clear_stats_store);
