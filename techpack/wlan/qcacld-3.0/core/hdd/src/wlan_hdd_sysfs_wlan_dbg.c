...
static ssize_t __hdd_sysfs_wlan_dbg_store(struct hdd_context *hdd_ctx,
					  ...
					  const char *buf, size_t count)
{
	...
	sptr = buf_local;
	hdd_debug("set_wlan_dbg: count %zu buf_local:(%s)", count, buf_local);
...
static ssize_t hdd_sysfs_wlan_dbg_store(struct kobject *kobj,
					struct kobj_attribute *attr,
					const char *buf, size_t count)
{
	...
	errno_size = __hdd_sysfs_wlan_dbg_store(hdd_ctx, attr, buf, count);
...
static struct kobj_attribute set_wlan_dbg_attribute =
	__ATTR(wlan_dbg, 0220, NULL, hdd_sysfs_wlan_dbg_store);
...
int hdd_sysfs_wlan_dbg_create(struct kobject *driver_kobject)
{
	...
	error = sysfs_create_file(driver_kobject, &set_wlan_dbg_attribute.attr);
	if (error)
...
void hdd_sysfs_wlan_dbg_destroy(struct kobject *driver_kobject)
{
	if (!driver_kobject) {
		hdd_err("could not get driver kobject!");
		return;
