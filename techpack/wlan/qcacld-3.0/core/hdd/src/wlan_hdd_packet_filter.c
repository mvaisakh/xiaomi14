...
int hdd_enable_default_pkt_filters(struct hdd_context *hdd_ctx, uint8_t vdev_id)
{
	uint8_t filters = 0, i = 0, filter_id = 1;

	if (hdd_ctx->user_configured_pkt_filter_rules) {
		hdd_info(
			"user has defined pkt filter run hence skipping default packet "
			"filter rule");
		return 0;
	}
	...
	while (filters != 0) {
		if (filters & 0x1) {
			hdd_err("setting filter[%d], of id = %d", i + 1,
				filter_id);
...
int hdd_disable_default_pkt_filters(struct hdd_context *hdd_ctx,
				    uint8_t vdev_id)
{
	uint8_t filters = 0, i = 0, filter_id = 1;
...
int wlan_hdd_set_filter(struct hdd_context *hdd_ctx,
			struct pkt_filter_cfg *request, uint8_t vdev_id)
{
		...
		for (i = 0; i < request->num_params; i++) {
			...
			if (request->params_data[i].data_offset >
				...
				hdd_err("Invalid data offset %u for param %d (max = %d)",
					request->params_data[i].data_offset, i,
					SIR_MAX_FILTER_TEST_DATA_OFFSET);
				status = QDF_STATUS_E_INVAL;
				goto out;
			}
			...
			if (sizeof(pmo_set_pkt_fltr_req->params_data[i]
					   .compare_data) <
			    (request->params_data[i].data_length)) {
				hdd_err("Error invalid data length %d",
					request->params_data[i].data_length);
			...
			memcpy(&pmo_set_pkt_fltr_req->params_data[i]
					.compare_data,
			...
			hdd_debug(
				"CData %d CData %d CData %d CData %d CData %d CData %d",
			...
			hdd_debug(
				"MData %d MData %d MData %d MData %d MData %d MData %d",
	...
	case HDD_RCV_FILTER_CLEAR:
		...
		pmo_clr_pkt_fltr_param =
			qdf_mem_malloc(sizeof(*pmo_clr_pkt_fltr_param));
		if (!pmo_clr_pkt_fltr_param)
			return QDF_STATUS_E_NOMEM;

		pmo_clr_pkt_fltr_param->filter_id = request->filter_id;
		status = ucfg_pmo_clear_pkt_filter(
			hdd_ctx->psoc, pmo_clr_pkt_fltr_param, vdev_id);
		if (QDF_IS_STATUS_ERROR(status)) {
