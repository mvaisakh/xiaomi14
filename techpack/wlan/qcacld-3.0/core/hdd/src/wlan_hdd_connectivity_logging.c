...
#define GET_ATTR_OFFSET(member) qdf_offsetof(struct wlan_log_record, member)
...
static const struct connectivity_log_attr
    connectivity_attr_table[QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_MAX +
                            1][WLAN_CONNECTIVITY_ATTR_END] = {
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_CONNECTING] =
            {
                {QCA_WLAN_VENDOR_ATTR_DIAG_EVENT_TYPE, NLA_U32,
                 sizeof(uint32_t), GET_ATTR_OFFSET(log_subtype)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_HOST_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(timestamp_us)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_KERNEL_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(ktime_us)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_IFINDEX, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(vdev_id)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_SSID, NLA_BINARY,
                 GET_ATTR_OFFSET(conn_info.ssid_len),
                 GET_ATTR_OFFSET(conn_info.ssid)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BSSID, NLA_BINARY, QDF_MAC_ADDR_SIZE,
                 GET_ATTR_OFFSET(bssid)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BSSID_HINT, NLA_BINARY,
                 QDF_MAC_ADDR_SIZE, GET_ATTR_OFFSET(conn_info.bssid_hint)},
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_PAIRWISE_SUITE, NLA_U32,
                 sizeof(uint32_t), GET_ATTR_OFFSET(conn_info.pairwise)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_AKM, NLA_U32, sizeof(uint32_t),
                 GET_ATTR_OFFSET(conn_info.akm)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_GROUP_SUITE, NLA_U32,
                 sizeof(uint32_t), GET_ATTR_OFFSET(conn_info.group)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_GROUP_MGMT_SUITE, NLA_U32,
                 sizeof(uint32_t), GET_ATTR_OFFSET(conn_info.group_mgmt)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_AUTH_ALGO, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(conn_info.auth_type)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BT_COEX_ACTIVE, NLA_FLAG,
                 sizeof(uint8_t), GET_ATTR_OFFSET(conn_info.is_bt_coex_active)},
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_CONNECTING_FAIL] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_FREQ, NLA_U16, sizeof(uint16_t),
                 GET_ATTR_OFFSET(conn_info.freq)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_REASON_CODE, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(conn_info.conn_status)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_AUTH_REQ] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_SAE_AUTH_FRAME_TYPE, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(pkt_info.auth_type)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_SEQUENCE_NUMBER, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(pkt_info.seq_num)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_STATUS_CODE, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(pkt_info.frame_status_code)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FRAME_TX_STATUS, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(pkt_info.tx_status)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_IS_RETRY_FRAME, NLA_FLAG,
                 sizeof(uint8_t), GET_ATTR_OFFSET(pkt_info.is_retry_frame)},
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_DEAUTH_RX] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_SEQUENCE_NUMBER, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(pkt_info.seq_num)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_REASON_CODE, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(pkt_info.frame_status_code)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_ROAM_SCAN_START] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_TRIGGER_SUB_REASON, NLA_U8,
                 sizeof(uint8_t),
                 GET_ATTR_OFFSET(roam_trig.trigger_sub_reason)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_RSSI, NLA_S32, sizeof(int32_t),
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_SCAN_TYPE, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(roam_trig.is_full_scan)},
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_ROAM_SCAN_DONE] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_IFINDEX, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(vdev_id)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_AP_COUNT, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(roam_scan.cand_ap_count)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_FREQ_LIST, NLA_NESTED,
                 sizeof(uint16_t), GET_ATTR_OFFSET(roam_scan.scan_freq)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BT_COEX_ACTIVE, NLA_FLAG,
                 sizeof(uint8_t), GET_ATTR_OFFSET(roam_scan.is_btcoex_active)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_ROAM_SCORE_CURR_AP] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_SCORE, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(ap.total_score)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_ROAM_SCORE_CAND_AP] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_IFINDEX, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(vdev_id)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_AP_RANK, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(ap.idx)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_CU, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(ap.cu_load)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BSSID, NLA_BINARY, QDF_MAC_ADDR_SIZE,
                 GET_ATTR_OFFSET(ap.cand_bssid)},
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_SCORE, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(ap.total_score)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_AP_ETP, NLA_U32,
                 sizeof(uint32_t), GET_ATTR_OFFSET(ap.etp)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_ROAM_RESULT] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_SUCCESSFUL, NLA_FLAG,
                 sizeof(uint8_t),
                 GET_ATTR_OFFSET(roam_result.is_roam_successful)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_BTM_QUERY] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_QUERY_REASON, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(btm_info.reason)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_BTM_REQ] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_CANDIDATE_LIST_COUNT, NLA_U8,
                 sizeof(uint8_t),
                 GET_ATTR_OFFSET(btm_info.candidate_list_count)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_BTM_WTC] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_IFINDEX, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(vdev_id)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_REASON_CODE, NLA_U16,
                 sizeof(uint16_t), GET_ATTR_OFFSET(btm_info.reason)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_WTC_SUB_REASON_CODE, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(btm_info.sub_reason)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_WTC_DURATION, NLA_U32,
                 sizeof(uint32_t), GET_ATTR_OFFSET(btm_info.wtc_duration)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_BTM_REQ_CANDI] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_IFINDEX, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(vdev_id)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_AP_RANK, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(btm_cand.idx)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BSSID, NLA_BINARY, QDF_MAC_ADDR_SIZE,
                 GET_ATTR_OFFSET(btm_cand.bssid)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_PREFERENCE, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(btm_cand.preference)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_FIRMWARE_TIMESTAMP, NLA_U64,
                 sizeof(uint64_t), GET_ATTR_OFFSET(fw_timestamp_us)},
                {WLAN_CONNECTIVITY_ATTR_END, 0, 0, 0},
            },
        ...
        [QCA_WLAN_VENDOR_DIAG_EVENT_TYPE_BTM_RESP] =
            {
                ...
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_TOKEN, NLA_U8, sizeof(uint8_t),
                 GET_ATTR_OFFSET(btm_info.token)},
                {QCA_WLAN_VENDOR_ATTR_DIAG_BTM_STATUS_CODE, NLA_U8,
                 sizeof(uint8_t), GET_ATTR_OFFSET(btm_info.btm_status_code)},
...
wlan_hdd_get_connectivity_log_tlv_len(struct wlan_log_record *rec)
{
	...
	for (j = 0; j < WLAN_CONNECTIVITY_ATTR_END; j++) {
		attr_id = connectivity_attr_table[log_evt_type][j].attribute_id;
		...
		if (log_evt_type ==
			...
			len += nla_total_size(
				connectivity_attr_table[log_evt_type][j]
					.attribute_length);
		}
	}
				       ...
				       struct connectivity_log_attr *attribute)
{
	...
	enum qca_wlan_vendor_attr_diag attribute_id = attribute->attribute_id;
				    ...
				    struct wlan_log_record *rec)
{
	...
	attr = nla_nest_start(skb, QCA_WLAN_VENDOR_ATTR_DIAG_ROAM_FREQ_LIST);
	if (!attr)
		return QDF_STATUS_E_FAILURE;
					...
					struct wlan_log_record *rec)
{
   ...
   */
	for (i = 1; i < WLAN_CONNECTIVITY_ATTR_END; i++) {
		...
		case NLA_U8:
			...
			if (attr.attribute_id ==
				...
				attr_val8 =
					wlan_hdd_get_converted_roam_sub_reason(
						val8);
			else if (attr.attribute_id ==
				 QCA_WLAN_VENDOR_ATTR_DIAG_FRAME_TX_STATUS)
				attr_val8 =
					wlan_hdd_get_converted_tx_status(val8);
			else if (attr.attribute_id ==
				...
				attr_val8 =
					wlan_hdd_convert_roam_scan_type(val8);
	...
	if (errno) {
		hdd_err_rl("NLA put failed for attr:%d", attr.attribute_id);
		return QDF_STATUS_E_FAILURE;
	}
...
wlan_hdd_send_connectivity_log_to_user(struct wlan_log_record *rec,
				       void *hdd_context, uint8_t num_records)
{
	...
	vendor_event = wlan_cfg80211_vendor_event_alloc(
		...
		QCA_NL80211_VENDOR_SUBCMD_DIAG_EVENT_INDEX, GFP_ATOMIC);
	if (!vendor_event) {
		hdd_err("wlan_cfg80211_vendor_event_alloc failed");
		return QDF_STATUS_E_NOMEM;
	}

	attr = nla_nest_start(vendor_event, QCA_WLAN_VENDOR_ATTR_DIAG_EVENT);
	if (!attr)
		goto failure;
	...
	for (i = 0; i < num_records; i++) {
		...
		status = wlan_hdd_fill_connectivity_logging_data(vendor_event,
								 &rec[i]);
		if (QDF_IS_STATUS_ERROR(status)) {
			hdd_err_rl("Failed to fill fat idx:%d subtype:%d", i,
				   rec[i].log_subtype);
			goto failure;
		}
					    ...
					    uint8_t vdev_id)
{
	...
	qdf_mem_copy(wlan_diag_event.ssid, req->ssid, wlan_diag_event.ssid_len);
