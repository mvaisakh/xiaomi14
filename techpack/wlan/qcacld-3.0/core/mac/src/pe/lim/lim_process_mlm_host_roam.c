				 ...
				 tLimMlmReassocReq *reassoc_req)
{
	...
	session =
		pe_find_session_by_session_id(mac_ctx, reassoc_req->sessionId);
	if (!session) {
	...
	pe_debug(
		"ReAssoc Req on session: %d role: %d mlm: %d " QDF_MAC_ADDR_FMT,
     ...
     */
		...
		pe_warn("unexpected msg state: %X role: %d MAC " QDF_MAC_ADDR_FMT,
   ...
   */
	...
	if (!auth_node && (qdf_mem_cmp(reassoc_req->peerMacAddr, session->bssId,
				       sizeof(tSirMacAddr)))) {
		/*
   ...
   */
	...
	if (lim_mlm_reassoc_cnf->resultCode == eSIR_SME_SUCCESS) {
		...
		lim_send_sme_join_reassoc_rsp(
			...
			lim_mlm_reassoc_cnf->protStatusCode, session,
...
QDF_STATUS lim_sta_reassoc_error_handler(struct reassoc_params *param)
{
	...
	if (param->result_code == eSIR_SME_REASSOC_REFUSED) {
		/*
     ...
     */
		...
		lim_send_sme_join_reassoc_rsp(mac_ctx, eWNI_SME_REASSOC_RSP,
					      param->result_code,
					      param->prot_status_code, session,
					      session->smeSessionId);
	} else {
		...
		MTRACE(mac_trace(mac_ctx, TRACE_CODE_SME_STATE,
				 session->peSessionId, session->limSmeState));
		/* Need to send Reassoc rsp with Assoc failure to Host. */
		lim_handle_sme_reaasoc_result(mac_ctx, param->result_code,
					      param->prot_status_code, session);
	}
					...
					struct pe_session *pe_session)
{
	...
	pe_debug("Set the mlm state: %d session: %d", pe_session->limMlmState,
		 pe_session->peSessionId);
	...
	qdf_mem_copy((uint8_t *)pAddStaParams->staMac,
		     (uint8_t *)pe_session->self_mac_addr, sizeof(tSirMacAddr));
	...
	if (pe_session->htCapability) {
		...
		pAddStaParams->fShortGI20Mhz = lim_get_ht_capability(
			mac, eHT_SHORT_GI_20MHZ, pe_session);
		pAddStaParams->fShortGI40Mhz = lim_get_ht_capability(
			mac, eHT_SHORT_GI_40MHZ, pe_session);
	}
				    ...
				    tLimMlmReassocReq *reassoc_req)
{
	...
	lim_diag_event_report(mac, WLAN_PE_DIAG_REASSOCIATING, session, 0, 0);
#endif
	...
	qdf_mem_copy(reassoc_req->peerMacAddr, session->bssId,
		     sizeof(tSirMacAddr));
