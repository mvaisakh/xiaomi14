				 ...
				 struct pe_session *pe_session)
{
	...
	qdf_mem_copy(pe_session->bssId, pe_session->limReAssocbssId,
		     sizeof(tSirMacAddr));
	...
	pe_session->limCurrentBssQosCaps = pe_session->limReassocBssQosCaps;
   ...
   */
     ...
     */
		...
		if (mac->lim.gLimProtectionControl !=
			...
			lim_decide_sta_protection_on_assoc(mac, beacon_struct,
							   pe_session);
     ...
     */
		if (QDF_STATUS_SUCCESS !=
		    lim_sta_send_add_bss(mac, assocRsp, beacon_struct, bss_desc,
					 false, pe_session)) {
					    ...
					    struct pe_session *pe_session)
{
     ...
     */
		...
		lim_extract_ap_capabilities(
			mac,
			(uint8_t *)pe_session->pLimReAssocReq->bssDescription
				.ieFields,
		...
		if (QDF_STATUS_SUCCESS !=
		    lim_sta_send_add_bss(
			    mac, assocRsp, pBeaconStruct,
			    &pe_session->pLimReAssocReq->bssDescription, true,
			    pe_session)) {
 ...
 *         else false
 */
bool lim_is_reassoc_in_progress(struct mac_context *mac,
				struct pe_session *pe_session)
{
	if (!pe_session)
		return false;
...
void lim_restore_pre_reassoc_state(struct mac_context *mac,
