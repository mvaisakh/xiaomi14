 ...
 *
 ***LOGIC:
 * This function add the WPS PBC probe request in the WPS PBC probe request link
 *list The link list is in decreased time order of probe request that is
 *received. The entry that is more than 120 second is removed.
 *
 ***ASSUMPTIONS:
 *
 *
...
static void lim_update_pbc_session_entry(struct mac_context *mac, uint8_t *addr,
					 uint8_t *uuid_e,
					 struct pe_session *pe_session)
{
	...
	curTime = (uint32_t)(qdf_mc_timer_get_system_ticks() /
			     QDF_TICKS_PER_SECOND);

	pe_debug("Receive WPS probe request curTime: %d", curTime);
	QDF_TRACE_HEX_DUMP(QDF_MODULE_ID_PE, QDF_TRACE_LEVEL_DEBUG, addr,
			   QDF_MAC_ADDR_SIZE);
	QDF_TRACE_HEX_DUMP(QDF_MODULE_ID_PE, QDF_TRACE_LEVEL_DEBUG, uuid_e,
			   SIR_WPS_UUID_LEN);

	pbc = pe_session->pAPWPSPBCSession;

	while (pbc) {
		if ((!qdf_mem_cmp((uint8_t *)pbc->addr.bytes, (uint8_t *)addr,
				  QDF_MAC_ADDR_SIZE)) &&
		    (!qdf_mem_cmp((uint8_t *)pbc->uuid_e, (uint8_t *)uuid_e,
				  SIR_WPS_UUID_LEN))) {
			if (prev)
				prev->next = pbc->next;
			else
	...
	if (!pbc) {
		...
		if (uuid_e)
			qdf_mem_copy((uint8_t *)pbc->uuid_e, (uint8_t *)uuid_e,
				     SIR_WPS_UUID_LEN);
	}
...
{
	if ((0x02 == (rate)) || (0x04 == (rate)) || (0x0b == (rate)) ||
	    (0x16 == (rate))) {
		return true;
	}
	return false;
...
void lim_process_probe_req_frame(struct mac_context *mac_ctx,
				 uint8_t *rx_pkt_info,
				 struct pe_session *session)
{
	...
	if (LIM_IS_AP_ROLE(session)) {
		...
		pe_debug(
			"Received Probe Request: %d bytes from " QDF_MAC_ADDR_FMT,
		...
		if ((session->access_policy_vendor_ie) &&
			...
			if (!wlan_get_vendor_ie_ptr_from_oui(
				    ...
				    body_ptr, frame_len)) {
				pe_warn("Vendor IE is not present and access policy is: %x dropping "
					"probe request",
					session->access_policy);
				return;
			}
		...
		if (sir_convert_probe_req_frame2_struct(mac_ctx, body_ptr,
			...
			pe_err("Parse error ProbeReq, length: %d, SA is: " QDF_MAC_ADDR_FMT,
			       frame_len, QDF_MAC_ADDR_REF(mac_hdr->sa));
			goto free_and_exit;
		}
		if (session->opmode == QDF_P2P_GO_MODE) {
			...
			if ((rate_11b > 0) && (other_rates == 0)) {
				pe_debug(
					"Received a probe req frame with only 11b rates, "
					"SA: " QDF_MAC_ADDR_FMT,
		...
		if (LIM_IS_AP_ROLE(session) &&
			...
			if (session->fwdWPSPBCProbeReq) {
				...
				lim_update_pbc_session_entry(
					mac_ctx, mac_hdr->sa, uuid, session);
			}
		}
		...
		if (probe_req->ssId.length) {
			...
			if (!qdf_mem_cmp((uint8_t *)&ssid,
				...
				lim_send_probe_rsp_mgmt_frame(
					mac_ctx, dst_mac, &ssid, session,
					probe_req->p2pIePresent);
				goto free_and_exit;
			} else if (session->opmode == QDF_P2P_GO_MODE) {
				...
				pe_debug(
					"Ignore ProbeReq frm with unmatch SSID received from "
					"SA: " QDF_MAC_ADDR_FMT,
					QDF_MAC_ADDR_REF(mac_hdr->sa));
			}
		} else {
...
multipleSSIDcheck:
		...
		pe_debug(
			"Ignoring Probe Request frame received from SA: " QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(mac_hdr->sa));
	}
...
static void lim_indicate_probe_req_to_hdd(struct mac_context *mac, uint8_t *pBd,
					  struct pe_session *pe_session)
{
	...
	lim_send_sme_mgmt_frame_ind(mac, pHdr->fc.subType, (uint8_t *)pHdr,
				    ...
				    pe_session->vdev_id, WMA_GET_RX_FREQ(pBd),
 ...
 * @return None
 */
void lim_process_probe_req_frame_multiple_bss(struct mac_context *mac_ctx,
					      uint8_t *buf_descr,
					      struct pe_session *session)
{
	uint8_t i;
	...
	if (session) {
		if (LIM_IS_AP_ROLE(session)) {
			lim_indicate_probe_req_to_hdd(mac_ctx, buf_descr,
						      session);
		}
     ...
     **/
		if (!chan || chan->ch_freq != probe_req_freq) {
			pe_debug(
				"do not send probe resp to requested probe freq %d",
				probe_req_freq);
			continue;
		}

		lim_indicate_probe_req_to_hdd(mac_ctx, buf_descr, session);
		lim_process_probe_req_frame(mac_ctx, buf_descr, session);
	}
}
 ...
 * @return None
 */
void lim_send_sme_probe_req_ind(struct mac_context *mac,
				tSirMacAddr peerMacAddr, uint8_t *pProbeReqIE,
				uint32_t ProbeReqIELen,
				struct pe_session *pe_session)
{
	...
	if (ProbeReqIELen >
	    sizeof(pSirSmeProbeReqInd->WPSPBCProbeReq.probeReqIE)) {
		ProbeReqIELen =
			sizeof(pSirSmeProbeReqInd->WPSPBCProbeReq.probeReqIE);
	}
