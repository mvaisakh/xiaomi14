...
static void lim_process_mlm_deauth_ind(struct mac_context *,
				       tLimMlmDeauthInd *);
 ...
 * @return None
 */
void lim_process_mlm_rsp_messages(struct mac_context *mac, uint32_t msgType,
				  uint32_t *msg_buf)
{
	if (!msg_buf) {
...
void lim_process_mlm_start_cnf(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session =
		pe_find_session_by_session_id(mac, pLimMlmStartCnf->sessionId);
	if (!pe_session) {
     ...
     */
		...
		if (pe_session->ch_center_freq_seg1)
			ch_cfreq1 = wlan_reg_chan_band_to_freq(
				mac->pdev, pe_session->ch_center_freq_seg1,
				BIT(band));
       ...
       */
		...
		if (send_bcon_ind) {
			...
			pe_debug("Start Beacon with ssid " QDF_SSID_FMT
				 " Ch freq %d",
 ...
 * Return: None
 */
void lim_process_mlm_join_cnf(struct mac_context *mac_ctx, uint32_t *msg)
{
	...
	session_entry =
		pe_find_session_by_session_id(mac_ctx, join_cnf->sessionId);
	if (!session_entry) {
	...
	if (result_code == eSIR_SME_SUCCESS) {
		...
		if (lim_sta_send_add_bss_pre_assoc(mac_ctx, session_entry) ==
		    QDF_STATUS_SUCCESS)
			return;
		else
	...
	lim_handle_sme_join_result(mac_ctx, result_code,
				   ((tLimMlmJoinCnf *)msg)->protStatusCode,
				   session_entry);
	return;
}
				   ...
				   struct pe_session *session_entry)
{
   ...
   */
	...
	MTRACE(mac_trace(mac_ctx, TRACE_CODE_SME_STATE,
			 session_entry->peSessionId,
			 session_entry->limSmeState));
	lim_post_mlm_message(mac_ctx, LIM_MLM_ASSOC_REQ, (uint32_t *)assoc_req);
}
...
void lim_pmf_comeback_timer_callback(void *context)
{
	...
	if (session->limMlmState != eLIM_MLM_WT_ASSOC_RSP_STATE) {
		pe_debug(
			"Don't send assoc req, timer expire when limMlmState %d vdev id %d",
			session->limMlmState, session->vdev_id);
		return;
	}

	pe_info("comeback later timer expired. sending MLM ASSOC req for vdev %d, "
		"session limMlmState %d, info lim_mlm_state %d",
...
void lim_process_mlm_auth_cnf(struct mac_context *mac_ctx, uint32_t *msg)
{
	...
	session_entry =
		pe_find_session_by_session_id(mac_ctx, auth_cnf->sessionId);
	if (!session_entry) {
       ...
       */
			if (auth_type == eSIR_AUTH_TYPE_SAE &&
				...
				lim_send_deauth_mgmt_frame(
					mac_ctx, REASON_TIMEDOUT,
					auth_cnf->peerMacAddr, session_entry,
					false);
			}
 ...
 * Return: None
 */
void lim_process_mlm_assoc_cnf(struct mac_context *mac_ctx, uint32_t *msg)
{
	struct pe_session *session_entry;
	...
	session_entry =
		pe_find_session_by_session_id(mac_ctx, assoc_cnf->sessionId);
	if (!session_entry) {
     ...
     */
		lim_handle_sme_join_result(
			mac_ctx, ((tLimMlmAssocCnf *)msg)->resultCode,
...
void lim_fill_sme_assoc_ind_params(struct mac_context *mac_ctx,
				   tpLimMlmAssocInd assoc_ind,
				   struct assoc_ind *sme_assoc_ind,
				   struct pe_session *session_entry,
				   bool assoc_req_alloc)
{
	...
	if (assoc_req_alloc && assoc_ind->assocReqLength) {
		sme_assoc_ind->assocReqPtr =
			qdf_mem_malloc(assoc_ind->assocReqLength);
	...
	if (assoc_ind->spectrumMgtIndicator == true) {
		...
		qdf_mem_copy(
			(uint8_t *)&sme_assoc_ind->supportedChannels.channelList,
...
/**
 * lim_process_mlm_assoc_ind() - This function is called to processes
 * MLM_ASSOC_IND message from MLM State machine.
...
void lim_process_mlm_assoc_ind(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session = pe_find_session_by_session_id(
		mac, ((tpLimMlmAssocInd)msg_buf)->sessionId);
	if (!pe_session) {
	...
	lim_fill_sme_assoc_ind_params(mac, (tpLimMlmAssocInd)msg_buf,
				      pSirSmeAssocInd, pe_session, false);
	msg.type = eWNI_SME_ASSOC_IND;
	msg.bodyptr = pSirSmeAssocInd;
	msg.bodyval = 0;
	sta = dph_get_hash_entry(mac, ((tpLimMlmAssocInd)msg_buf)->aid,
  ...
  **/
	lim_activate_cnf_timer(mac, (uint16_t)((tpLimMlmAssocInd)msg_buf)->aid,
			       pe_session);
...
void lim_process_mlm_disassoc_ind(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session =
		pe_find_session_by_session_id(mac, pMlmDisassocInd->sessionId);
	if (!pe_session) {
		pe_err("Session Does not exist for given sessionID");
		return;
	}
       ...
       */
		...
		if (disassoc_cnf->disassocTrigger ==
			...
			lim_send_sme_disassoc_ntf(
				mac_ctx, disassoc_cnf->peerMacAddr, result_code,
		...
		lim_send_sme_disassoc_ntf(
			mac_ctx, disassoc_cnf->peerMacAddr, result_code,
			disassoc_cnf->disassocTrigger, disassoc_cnf->aid,
			session_entry->smeSessionId, session_entry);
	}
}
				       ...
				       tLimMlmDeauthInd *deauth_ind)
{
	...
	session = pe_find_session_by_bssid(mac_ctx, deauth_ind->peerMacAddr,
					   &session_id);
	if (!session) {
...
void lim_process_mlm_deauth_cnf(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session =
		pe_find_session_by_session_id(mac, pMlmDeauthCnf->sessionId);
	if (!pe_session) {
		pe_err("session does not exist for given session Id");
		return;
	}

	resultCode = (tSirResultCodes)(pMlmDeauthCnf->deauthTrigger ==
	...
	if (LIM_IS_STA_ROLE(pe_session)) {
       ...
       */
		...
		if (pMlmDeauthCnf->resultCode == eSIR_SME_SUCCESS) {
			...
			pe_session->limSmeState = pe_session->limPrevSmeState;
		MTRACE(mac_trace(mac, TRACE_CODE_SME_STATE,
				 pe_session->peSessionId,
				 pe_session->limSmeState));
	...
	lim_send_sme_deauth_ntf(mac, pMlmDeauthCnf->peer_macaddr.bytes,
				resultCode, pMlmDeauthCnf->deauthTrigger, aid,
				pe_session->smeSessionId);
...
void lim_process_mlm_purge_sta_ind(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session =
		pe_find_session_by_session_id(mac, pMlmPurgeStaInd->sessionId);
	if (!pe_session) {
       ...
       */
		...
		if (pMlmPurgeStaInd->purgeTrigger == eLIM_PEER_ENTITY_DEAUTH) {
			lim_send_sme_deauth_ntf(
				mac, pMlmPurgeStaInd->peerMacAddr, resultCode,
				pMlmPurgeStaInd->purgeTrigger,
				pMlmPurgeStaInd->aid, pe_session->smeSessionId);
		} else
			lim_send_sme_disassoc_ntf(
				mac, pMlmPurgeStaInd->peerMacAddr, resultCode,
				pMlmPurgeStaInd->purgeTrigger,
...
void lim_process_mlm_set_keys_cnf(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session =
		pe_find_session_by_session_id(mac, pMlmSetKeysCnf->sessionId);
	if (!pe_session) {
   ...
   */
	if (eSIR_SME_SUCCESS == pMlmSetKeysCnf->resultCode) {
		sta_ds = dph_lookup_hash_entry(
			mac, pMlmSetKeysCnf->peer_macaddr.bytes, &aid,
			&pe_session->dph.dphHashTable);
		if (sta_ds && pMlmSetKeysCnf->key_len_nonzero)
			sta_ds->is_key_installed = 1;
	}
	pe_debug("vdev %d: " QDF_MAC_ADDR_FMT
		 " Status %d key_len_nonzero %d key installed %d",
		 pe_session->vdev_id,
		 QDF_MAC_ADDR_REF(pMlmSetKeysCnf->peer_macaddr.bytes),
		 pMlmSetKeysCnf->resultCode, pMlmSetKeysCnf->key_len_nonzero,
		 sta_ds ? sta_ds->is_key_installed : 0);

	lim_send_sme_set_context_rsp(
		mac, pMlmSetKeysCnf->peer_macaddr, 1,
		(tSirResultCodes)pMlmSetKeysCnf->resultCode, pe_session,
		pe_session->smeSessionId);
...
void lim_join_result_callback(struct mac_context *mac, uint8_t vdev_id)
{
	struct pe_session *session;
	...
	lim_send_sme_join_reassoc_rsp(mac, eWNI_SME_JOIN_RSP,
				      session->result_code,
				      session->prot_status_code, session,
				      vdev_id);
...
QDF_STATUS lim_sta_handle_connect_fail(join_params *param)
{
	...
	if (sta_ds) {
		sta_ds->mlmStaContext.disassocReason = REASON_UNSPEC_FAILURE;
		sta_ds->mlmStaContext.cleanupTrigger = eLIM_JOIN_FAILURE;
				...
				struct pe_session *session)
{
	...
	if (result_code == eSIR_SME_SUCCESS) {
		...
		wlan_vdev_mlme_sm_deliver_evt(
			session->vdev, WLAN_VDEV_SM_EV_START_SUCCESS, 0, NULL);
	...
	if (wlan_vdev_mlme_get_substate(session->vdev) ==
		...
		status = wlan_vdev_mlme_sm_deliver_evt(
			session->vdev, WLAN_VDEV_SM_EV_START_REQ_FAIL,
			sizeof(param), &param);
	else
		status = wlan_vdev_mlme_sm_deliver_evt(
			session->vdev, WLAN_VDEV_SM_EV_CONNECTION_FAIL,
			sizeof(param), &param);
...
void lim_process_mlm_add_sta_rsp(struct mac_context *mac,
				 ...
				 struct pe_session *pe_session)
{
	/* we need to process the deferred message since the initiating req. there
   * might be nested request. */
	/* in the case of nested request the new request initiated from the response
...
#ifdef WLAN_FEATURE_11BE_MLO
static void lim_process_add_sta_rsp_mlo(struct mac_context *mac_ctx,
...
#else /* WLAN_FEATURE_11BE_MLO */
static inline void lim_process_add_sta_rsp_mlo(struct mac_context *mac_ctx,
					       tLimMlmAssocCnf *mlm_assoc_cnf,
					       struct pe_session *session_entry)
{
}
				     ...
				     struct pe_session *session_entry)
{
	...
	if (true == session_entry->fDeauthReceived) {
		...
		if (QDF_STATUS_SUCCESS == add_sta_params->status) {
			pe_err("ADD_STA success, send update result code with "
			       "eSIR_SME_JOIN_DEAUTH_FROM_AP_DURING_ADD_STA limMlmState: %d "
     ...
     */
		...
		lim_send_obss_color_collision_cfg(
			mac_ctx, session_entry, OBSS_COLOR_COLLISION_DETECTION);
		if (lim_is_session_he_capable(session_entry) && sta_ds) {
			if (mac_ctx->usr_cfg_mu_edca_params) {
				...
				lim_send_edca_params(
					mac_ctx, mac_ctx->usr_mu_edca_params,
...
void lim_process_sta_mlm_del_bss_rsp(struct mac_context *mac,
				     ...
				     struct pe_session *pe_session)
{
	tpDphHashNode sta = dph_get_hash_entry(mac, DPH_STA_HASH_INDEX_PEER,
					       &pe_session->dph.dphHashTable);
	...
	if ((LIM_IS_STA_ROLE(pe_session)) &&
	    (pe_session->limSmeState != eLIM_SME_WT_DISASSOC_STATE &&
	     pe_session->limSmeState != eLIM_SME_WT_DEAUTH_STATE) &&
	    sta->mlmStaContext.cleanupTrigger != eLIM_JOIN_FAILURE) {
				    ...
				    struct pe_session *pe_session)
{
	...
	/** Softmac may send all the buffered packets right after resuming the
   * transmission hence to occupy the medium during non channel occupancy
   * period. So resume the transmission after HAL gives back the response.
   */
				 ...
				 struct scheduler_msg *msg)
{
   ...
   */
	...
	session_entry = pe_find_session_by_session_id(
		mac_ctx, del_sta_params->sessionId);
	if (!session_entry) {
		pe_err("Session Doesn't exist: %d", del_sta_params->sessionId);
	...
	if (LIM_IS_AP_ROLE(session_entry)) {
		lim_process_ap_mlm_del_sta_rsp(mac_ctx, msg, session_entry);
		return;
	}
				    ...
				    struct pe_session *session_entry)
{
	...
	sta_ds = dph_get_hash_entry(mac_ctx, del_sta_params->assocId,
				    &session_entry->dph.dphHashTable);
	if (!sta_ds) {
		pe_err("DPH Entry for STA %X missing", del_sta_params->assocId);
		status_code = eSIR_SME_REFUSED;
		qdf_mem_free(del_sta_params);
		msg->bodyptr = NULL;
		return;
	...
	pe_debug(
		"AP received the DEL_STA_RSP for assocID: %X sta mac " QDF_MAC_ADDR_FMT,
		del_sta_params->assocId, QDF_MAC_ADDR_REF(sta_ds->staAddr));
				    ...
				    struct pe_session *pe_session)
{
	...
	sta = dph_get_hash_entry(mac, pAddStaParams->assocId,
				 &pe_session->dph.dphHashTable);
	if (!sta) {
	...
	if (QDF_STATUS_SUCCESS != pAddStaParams->status) {
		...
		lim_reject_association(mac, sta->staAddr,
				       sta->mlmStaContext.subType, true,
				       sta->mlmStaContext.authType,
				       sta->assocId, true,
				       STATUS_UNSPECIFIED_FAILURE, pe_session);
		add_sta_rsp_status = false;
		goto end;
	}
	sta->nss = pAddStaParams->nss;
	/* if the AssocRsp frame is not acknowledged, then keep alive timer will take
   * care of the state */
	sta->valid = 1;
					   ...
					   struct add_bss_rsp *add_bss_rsp)
{
	...
	/* TBD: free the memory before returning, do it for all places where lookup
   * fails. */
					    ...
					    struct pe_session *session_entry)
{
	...
	mlm_assoc_cnf.resultCode = eSIR_SME_SUCCESS;
	if (eLIM_MLM_WT_ADD_BSS_RSP_REASSOC_STATE ==
		    session_entry->limMlmState ||
	...
	if (QDF_IS_STATUS_SUCCESS(add_bss_rsp->status)) {
		if (eLIM_MLM_WT_ADD_BSS_RSP_FT_REASSOC_STATE ==
			...
			lim_process_sta_mlm_add_bss_rsp_ft(mac_ctx, add_bss_rsp,
							   session_entry);
			return;
		}
		...
		if (!sta_ds) {
			...
			lim_set_active_edca_params(
				mac_ctx, session_entry->gLimEdcaParams,
				session_entry);
			lim_send_edca_params(
				mac_ctx, session_entry->gLimEdcaParamsActive,
			    ...
			    struct add_bss_rsp *add_bss_rsp)
{
   ...
   */
	...
	session_entry =
		pe_find_session_by_vdev_id(mac_ctx, add_bss_rsp->vdev_id);
	if (!session_entry) {
		...
		if (wlan_reg_is_ext_tpc_supported(mac_ctx->psoc)) {
			...
			if (wlan_get_tpc_update_required_for_sta(
				...
				sta_session = lim_get_concurrent_session(
					mac_ctx, session_entry->vdev_id,
					session_entry->opmode);
				if (!sta_session) {
					pe_err("TPC update required is set, but concurrent session doesn't "
					       "exist");
					wlan_set_tpc_update_required_for_sta(
						session_entry->vdev, false);
				} else {
	...
	if (eSIR_NDI_MODE == session_entry->bssType) {
		...
		if (eLIM_SME_WT_START_BSS_STATE == session_entry->limSmeState) {
			if (eLIM_MLM_WT_ADD_BSS_RSP_STATE !=
				...
				lim_send_start_bss_confirm(mac_ctx,
							   &mlm_start_cnf);
			}
			lim_process_ap_mlm_add_bss_rsp(mac_ctx, add_bss_rsp);
		} else {
					    ...
					    uint8_t vdev_id)
{
	...
	session_entry = pe_find_session_by_vdev_id(mac_ctx, vdev_id);
	if (!session_entry) {
		pe_err("vdev_id:%d Session Doesn't exist", vdev_id);
		return;
	}
	/* Update beacon */
				     ...
				     struct scheduler_msg *msg)
{
	...
	if (wlan_get_opmode_from_vdev_id(mac_ctx->pdev, vdev_id) !=
		...
		if (!session_entry) {
			...
			msg->bodyptr = NULL;
			lim_send_sme_set_context_rsp(
				mac_ctx, mlm_set_key_cnf.peer_macaddr, 0,
				eSIR_SME_INVALID_SESSION, NULL, vdev_id);
			return;
		}
				     ...
				     struct scheduler_msg *msg)
{
	...
	pe_debug("vdev %d (pe %d) limMlmState %d status %d key_len %d", vdev_id,
		 session_id, session_entry->limMlmState, result_status,
		 key_len);
 ...
 */
static void
lim_process_switch_channel_re_assoc_req(struct mac_context *mac,
					struct pe_session *pe_session,
					QDF_STATUS status)
{
	...
	pMlmReassocReq = (tLimMlmReassocReq *)(pe_session->pLimMlmReassocReq);
	if (!pMlmReassocReq) {
				    ...
				    struct mac_context *mac_ctx)
{
	...
	if (assoc_rsp.len) {
		...
		status = util_gen_link_assoc_rsp(
			assoc_rsp.ptr, assoc_rsp.len - 24, false, link_id,
			sta_link_addr, link_assoc_rsp.ptr, assoc_rsp.len,
			(qdf_size_t *)&link_assoc_rsp.len);
		if (QDF_IS_STATUS_SUCCESS(status)) {
			pe_debug("MLO: process assoc rsp for link vdev");
			lim_process_assoc_rsp_frame(
				mac_ctx, link_assoc_rsp.ptr,
				(link_assoc_rsp.len - SIR_MAC_HDR_LEN_3A),
				LIM_ASSOC, session_entry);
		} else {
					 ...
					 struct mac_context *mac_ctx)
{
	...
	if (assoc_rsp.len) {
		...
		mlo_get_link_mac_addr_from_reassoc_rsp(session_entry->vdev,
						       &bssid);
		sir_copy_mac_addr(session_entry->limReAssocbssId, bssid.bytes);
		link_id = wlan_vdev_get_link_id(session_entry->vdev);
		pe_debug(
			"MLO ROAM: Generate and process assoc rsp for link vdev %d",
			link_id);

		status = util_gen_link_assoc_rsp(
			assoc_rsp.ptr, assoc_rsp.len, true, link_id,
			sta_link_addr, link_assoc_rsp.ptr, assoc_rsp.len,
			(qdf_size_t *)&link_assoc_rsp.len);
		...
		if (QDF_IS_STATUS_SUCCESS(status)) {
			...
			pe_debug(
				"MLO_ROAM: link vdev assoc rsp generation failed");
			assoc_cnf.resultCode = eSIR_SME_INVALID_PARAMETERS;
			assoc_cnf.protStatusCode = STATUS_UNSPECIFIED_FAILURE;
			/* Update PE sessionId */
			assoc_cnf.sessionId = session_entry->peSessionId;
 ...
 */
static void
lim_process_switch_channel_join_req(struct mac_context *mac_ctx,
				    struct pe_session *session_entry,
				    QDF_STATUS status)
{
	tSirMacSSid ssId;
	...
	if (wlan_vdev_mlme_is_mlo_link_vdev(session_entry->vdev)) {
		if (mlo_roam_is_auth_status_connected(mac_ctx->psoc,
						      session_entry->vdev_id))
			mlo_status = lim_process_switch_channel_join_mlo_roam(
				session_entry, mac_ctx);
		else
			mlo_status = lim_process_switch_channel_join_mlo(
				session_entry, mac_ctx);
   ...
   */
	...
	qdf_mem_copy(ssId.ssId, session_entry->ssId.ssId,
	...
	mac_ctx->lim.lim_timers.gLimPeriodicJoinProbeReqTimer.sessionId =
		session_entry->peSessionId;
	pe_debug("vdev %d Send Probe req on freq %d " QDF_SSID_FMT
		 " " QDF_MAC_ADDR_FMT,
   ...
   */
	...
	sap_session = lim_get_concurrent_session(
		mac_ctx, session_entry->vdev_id, session_entry->opmode);
   ...
   */
	if (sap_session && lim_is_power_change_required_for_sta(
				   mac_ctx, session_entry, sap_session))
	...
	lim_send_probe_req_mgmt_frame(
		mac_ctx, &ssId,
		session_entry->pLimMlmJoinReq->bssDescription.bssId,
		session_entry->curr_op_freq, session_entry->self_mac_addr,
					      ...
					      QDF_STATUS status)
{
	...
	if (QDF_STATUS_SUCCESS !=
	    scheduler_post_message(QDF_MODULE_ID_PE, QDF_MODULE_ID_SME,
				   QDF_MODULE_ID_SME, &message)) {
				    ...
				    struct vdev_start_response *rsp)
{
   ...
   * will take care of resetting */
	...
	case LIM_SWITCH_CHANNEL_REASSOC:
		lim_process_switch_channel_re_assoc_req(mac, pe_session,
							status);
		break;
     ...
     */
		if (mac->lim.gpchangeChannelCallback)
			mac->lim.gpchangeChannelCallback(
				mac, status, mac->lim.gpchangeChannelData,
				pe_session);
     ...
     */
		policy_mgr_update_connection_info(mac->psoc,
						  pe_session->smeSessionId);
		wlan_cm_handle_sta_sta_roaming_enablement(
			mac->psoc, pe_session->smeSessionId);
		if (pe_session->opmode == QDF_P2P_CLIENT_MODE) {
			pe_debug(
				"Send p2p operating channel change conf action frame once first "
				"beacon is received on new channel");
