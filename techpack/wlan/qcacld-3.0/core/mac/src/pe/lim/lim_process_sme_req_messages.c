...
static void lim_process_sme_channel_change_request(struct mac_context *mac,
						   uint32_t *pMsg);
static void lim_process_sme_start_beacon_req(struct mac_context *mac,
					     uint32_t *pMsg);
static void lim_process_sme_dfs_csa_ie_request(struct mac_context *mac,
					       uint32_t *pMsg);
static void lim_process_nss_update_request(struct mac_context *mac,
					   uint32_t *pMsg);
 ...
 */
static QDF_STATUS lim_process_set_hw_mode(struct mac_context *mac,
					  uint32_t *msg)
{
						   ...
						   uint32_t *msg)
{
	...
	pe_debug("Post SIR_HAL_SOC_ANTENNA_MODE_REQ to WMA: %d %d",
		 req_msg->num_rx_chains, req_msg->num_tx_chains);
	status = scheduler_post_message(QDF_MODULE_ID_PE, QDF_MODULE_ID_WMA,
					QDF_MODULE_ID_WMA, &message);
	if (!QDF_IS_STATUS_SUCCESS(status)) {
		pe_err("scheduler_post_msg failed!(err=%d)", status);
 ...
 */
static bool __lim_is_deferred_msg_for_radar(struct mac_context *mac_ctx,
				   ...
				   struct start_bss_config *sme_start_bss_req)
{
	...
	qdf_mem_copy((void *)&session->cfgProtection, (void *)&ht_cap,
		     sizeof(uint16_t));
...
static void lim_set_privacy(struct mac_context *mac_ctx, int32_t ucast_cipher,
			    int32_t auth_mode, int32_t akm, bool ap_privacy)
{
	...
	pe_debug(
		"rsn_enabled %d privacy %d ucast_cipher %x auth_mode %x akm %x "
		"auth_type %d",
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS lim_send_start_vdev_req(struct pe_session *session,
				    ...
				    struct pe_session *session)
{
	...
	status = lim_strip_ie(mac_ctx, add_ie->probeRespBCNData_buff,
			      ...
			      (uint8_t)EHT_OP_OUI_SIZE, eht_op_buff,
			      DOT11F_IE_EHT_OP_MAX_LEN);
	if (status != QDF_STATUS_SUCCESS)
				   ...
				   struct pe_session *session)
{
	...
	status = lim_strip_ie(mac_ctx, add_ie->probeRespBCNData_buff,
			      ...
			      (uint8_t)HE_CAP_OUI_SIZE, he_cap_buff,
				     ...
				     struct pe_session *session)
{
	...
	status = lim_strip_ie(mac_ctx, add_ie->probeRespBCNData_buff,
			      ...
			      DOT11F_EID_WAPIOPAQUE, ONE_BYTE, NULL, 0,
 ...
 * Return: void
 */
static void lim_save_max_mcs_idx(struct mac_context *mac_ctx,
				 struct pe_session *session)
{
	...
	if (IS_DOT11_MODE_HE(session->dot11mode)) {
		...
		session_max_mcs_idx =
			lim_get_he_max_mcs_idx(session->ch_width, &he_cap);
	}
	...
	if (session_max_mcs_idx == INVALID_MCS_NSS_INDEX &&
		...
		session_max_mcs_idx = lim_get_max_rate_idx(&session->rateSet);
 ...
 * Return: None
 */
static void __lim_handle_sme_start_bss_request(struct mac_context *mac_ctx,
					       uint32_t *msg_buf)
{
	uint16_t size;
	uint32_t val = 0;
...
/* FEATURE_WLAN_DIAG_SUPPORT */
   ...
   */
	lim_diag_event_report(mac_ctx, WLAN_PE_DIAG_START_BSS_REQ_EVENT, NULL,
			      0, 0);
	...
	if ((mac_ctx->lim.gLimSmeState == eLIM_SME_OFFLINE_STATE) ||
		...
		if (!lim_is_sme_start_bss_req_valid(mac_ctx, sme_start_bss_req,
						    bss_type)) {
			pe_warn("Received invalid eWNI_SME_START_BSS_REQ");
			ret_code = eSIR_SME_INVALID_PARAMETERS;
			goto free;
		}
		channel_number = wlan_reg_freq_to_chan(
			mac_ctx->pdev, sme_start_bss_req->oper_ch_freq);
		/*
     ...
     */
		...
		if (session) {
			...
			session = pe_create_session(
				mac_ctx, bssid.bytes, &session_id,
				mac_ctx->lim.max_sta_of_pe_session, bss_type,
		...
		ucast_cipher = wlan_crypto_get_param(
			session->vdev, WLAN_CRYPTO_PARAM_UCAST_CIPHER);
     ...
     */
		if (bss_type == eSIR_INFRA_AP_MODE) {
			session->parsedAssocReq =
				qdf_mem_malloc(session->dph.dphHashTable.size *
     ...
     */
		lim_set_rs_nie_wp_aiefrom_sme_start_bss_req_message(
			mac_ctx, &sme_start_bss_req->rsnIE, session);
		...
		mlm_start_req->dfs_regdomain = sme_start_bss_req->dfs_regdomain;
		if (LIM_IS_AP_ROLE(session)) {
			...
			qdf_status = wlan_mlme_is_ap_obss_prot_enabled(
				mac_ctx->psoc, &cfg_value);
     ...
     */
		qdf_mem_copy((void *)&mlm_start_req->rateSet,
			     (void *)&session->rateSet, sizeof(tSirMacRateSet));
		...
		session->limRFBand =
			lim_get_rf_band(sme_start_bss_req->oper_ch_freq);
		...
		if (CHAN_HOP_ALL_BANDS_ENABLE ||
			...
			if (session->lim11hEnable &&
			    (eSIR_INFRA_AP_MODE == mlm_start_req->bssType)) {
				session->lim11hEnable =
					mac_ctx->mlme_cfg->dfs_cfg
						.dfs_master_capable;
			}
		}
		...
		pe_debug(
			"Freq %d width %d freq0 %d freq1 %d, dot11mode %d nss %d vendor vht %d",
			session->curr_op_freq, session->ch_width,
			session->ch_center_freq_seg0,
			session->ch_center_freq_seg1, session->dot11mode,
			session->vdev_nss, session->vendor_vht_sap);
...
free:
	if ((session) && (session->pLimStartBssReq == sme_start_bss_req)) {
 ...
 *
 *  FUNCTION:This function is called to process generate the random number for
 * bssid This function is called to process SME_SCAN_REQ message from HDD or
					  ...
					  tLimMlmReassocReq *reassoc_req)
{
	...
	return wlan_vdev_mlme_sm_deliver_evt(session->vdev,
					     WLAN_VDEV_SM_EV_START,
					     sizeof(*reassoc_req), reassoc_req);
}
#endif
					    ...
					    tDot11fBeaconIEs *bcn_ie)
{
   ...
   */
	...
	if (session->opmode == QDF_STA_MODE) {
		pe_debug(
			"AP capability shortGI20MHz %d shortGI40MHz %d ini capability "
			"short_gi_20_mhz %d short_gi_40_mhz %d",
	...
	lim_revise_req_vht_cap_per_band(session);
	pe_debug("HT capability 0x%x VHT capability 0x%x", ht_caps.caps,
		 vht_config.caps);
}
			      ...
			      struct pe_session *session)
{
	...
	pe_debug("vdev %d peer_rmf_capable %d rsn_caps 0x%x", session->vdev_id,
		 peer_rmf_capable, rsn_caps);
 ...
 */
static uint8_t lim_get_nss_supported_by_sta_and_ap(
	tDot11fIEVHTCaps *vht_caps, tDot11fIEHTCaps *ht_caps,
	tDot11fIEhe_cap *he_cap, enum mlme_dot11_mode dot11_mode)
{
						 ...
						 tDot11fBeaconIEs *ies)
{
	...
	if (ies->he_cap.present)
		if ((int_mcs == 0xFFFF) &&
		    (ies->vendor_vht_ie.present || ies->VHTCaps.present)) {
 ...
 * Return: void
 */
static void lim_update_he_caps_htc(struct pe_session *session, bool val)
{
	struct wlan_objmgr_vdev *vdev = session->vdev;
	struct mlme_legacy_priv *mlme_priv;
			     ...
			     struct pe_session *session)
{
	...
	if (!tx_nss || tx_nss > 2 || !rx_nss || rx_nss > 2) {
		pe_err("invalid Nss values tx_nss: %u rx_nss: %u", tx_nss,
		       rx_nss);
		return;
	}
					     ...
					     tDot11fBeaconIEs *ie_struct)
{
	...
	ap_nss = lim_get_nss_supported_by_sta_and_ap(&ie_struct->VHTCaps,
						     &ie_struct->HTCaps,
						     &ie_struct->he_cap,
						     session->dot11mode);
   ...
   */
	is_vendor_ap_present = wlan_action_oui_search(
		mac_ctx->psoc, &vendor_ap_search_attr, ACTION_OUI_CCKM_1X1);
   ...
   */
	is_vendor_ap_present =
		wlan_action_oui_search(mac_ctx->psoc, &vendor_ap_search_attr,
				       ACTION_OUI_SWITCH_TO_11N_MODE);
	if (mac_ctx->roam.configParam.is_force_1x1 &&
	    mac_ctx->mlme_cfg->gen.as_enabled && is_vendor_ap_present &&
	...
	is_vendor_ap_present = wlan_action_oui_search(
		mac_ctx->psoc, &vendor_ap_search_attr, ACTION_OUI_CONNECT_1X1);

	if (is_vendor_ap_present) {
		is_vendor_ap_present = lim_check_vendor_ap_3_present(
			mac_ctx, vendor_ap_search_attr.ie_data, ie_len);
	}
   ...
   */
	...
	if (is_vendor_ap_present &&
		...
		pe_debug("For special ap, NSS: %d force 1x1 %d", session->nss,
			 mac_ctx->roam.configParam.is_force_1x1);
	}
...
static enum mlme_dot11_mode lim_get_self_dot11_mode(struct mac_context *mac_ctx,
						    enum QDF_OPMODE opmode,
						    uint8_t vdev_id)
{
	struct wlan_objmgr_vdev *vdev;
					  ...
					  tDot11fBeaconIEs *ie_struct)
{
	...
	return mlme_get_bss_11be_allowed(
		mac_ctx->psoc, (struct qdf_mac_addr *)&bss_desc->bssId,
...
static QDF_STATUS lim_get_intersected_dot11_mode_sta_ap(
	struct mac_context *mac_ctx, enum mlme_dot11_mode self_dot11_mode,
	enum mlme_dot11_mode bss_dot11_mode,
	enum mlme_dot11_mode *intersected_mode, tDot11fBeaconIEs *ie_struct,
	struct bss_description *bss_desc)
{
	...
	case MLME_DOT11_MODE_11G:
		return lim_handle_11g_dot11_mode(bss_dot11_mode,
						 intersected_mode, bss_desc);
	case MLME_DOT11_MODE_11N:
		return lim_handle_11n_dot11_mode(
			bss_dot11_mode, intersected_mode, ie_struct, bss_desc);
	case MLME_DOT11_MODE_11G_ONLY:
		return lim_handle_11g_only_dot11_mode(
			bss_dot11_mode, intersected_mode, bss_desc);
	case MLME_DOT11_MODE_11N_ONLY:
		return lim_handle_11n_only_dot11_mode(
			bss_dot11_mode, intersected_mode, ie_struct, bss_desc);
	case MLME_DOT11_MODE_11AC:
		return lim_handle_11ac_dot11_mode(
			bss_dot11_mode, intersected_mode, ie_struct, bss_desc);
	case MLME_DOT11_MODE_11AC_ONLY:
		return lim_handle_11ac_only_dot11_mode(
			bss_dot11_mode, intersected_mode, ie_struct, bss_desc);
	case MLME_DOT11_MODE_11AX:
		return lim_handle_11ax_dot11_mode(
			bss_dot11_mode, intersected_mode, ie_struct, bss_desc);
	case MLME_DOT11_MODE_11AX_ONLY:
		return lim_handle_11ax_only_dot11_mode(
			bss_dot11_mode, intersected_mode, ie_struct);
	case MLME_DOT11_MODE_11BE:
...
static void lim_verify_dot11_mode_with_crypto(struct pe_session *session)
{
	struct bss_description *bss_desc =
		&session->lim_join_req->bssDescription;
	int32_t ucast_cipher;
				      ...
				      tDot11fBeaconIEs *ie_struct)
{
	...
	self_dot11_mode = lim_intersect_user_dot11_mode(
		mac_ctx, session->opmode, session->vdev_id, self_dot11_mode);
...
static bool lim_enable_twt(struct mac_context *mac_ctx, tDot11fBeaconIEs *ie)
{
	...
	if (twt_request && (ie->qcn_ie.present || ie->he_cap.twt_responder)) {
		pe_debug(
			"TWT is supported, hence disable UAPSD; twt req supp: %d,twt "
			"respon supp: %d, QCN_IE: %d",
			mac_ctx->mlme_cfg->he_caps.dot11_he_cap.twt_request,
			ie->he_cap.twt_responder, ie->qcn_ie.present);
		return true;
	}

	wlan_twt_cfg_get_support_in_11n(mac_ctx->psoc, &twt_support_in_11n);
	ext_cap = (struct s_ext_cap *)ie->ExtCap.bytes;
	if (twt_support_in_11n && ie->ExtCap.present &&
	    ext_cap->twt_responder_support) {
		pe_debug(
			"TWT is supported for 11n, twt_support_in_11n %d, ext_cap %d, "
			"twt_responder support %d",
...
#else
static inline bool lim_enable_twt(struct mac_context *mac_ctx,
				  tDot11fBeaconIEs *ie)
{
	return false;
}
 ...
 */
static void lim_update_sae_single_pmk_ap_cap(struct mac_context *mac,
					     struct pe_session *session)
{
	int32_t akm;
...
#else
static inline void lim_update_sae_single_pmk_ap_cap(struct mac_context *mac,
						    struct pe_session *session)
{
}
#endif
				  ...
				  struct pe_session *session)
{
	...
	qdf_mem_copy(bssid.bytes, session->bssId, QDF_MAC_ADDR_SIZE);
...
static void lim_fill_11r_params(struct mac_context *mac_ctx,
				struct pe_session *session,
				bool ese_version_present)
{
	...
	if (!mlme_obj)
		return;
	if (wlan_cm_is_auth_type_11r(
		    mlme_obj, session->vdev,
...
#ifdef FEATURE_WLAN_ESE
static void lim_fill_ese_params(struct mac_context *mac_ctx,
				struct pe_session *session,
				bool ese_version_present)
{
...
#else
static inline void lim_fill_ese_params(struct mac_context *mac_ctx,
				       struct pe_session *session,
				       bool ese_version_present)
{
}
			   ...
			   tDot11fBeaconIEs *ie_struct)
{
	...
	pe_debug("qos_type %d QOS %d WMM %d", qos_type, session->limQosEnabled,
...
static void lim_reset_self_ocv_caps(struct pe_session *session)
{
	...
	self_rsn_cap =
		wlan_crypto_get_param(session->vdev, WLAN_CRYPTO_PARAM_RSN_CAP);
	if (self_rsn_cap == -1)
		return;
		    ...
		    struct bss_description *bss_desc)
{
   ...
   */
	lim_update_rrm_capability(mac_ctx);
	bss_chan_id = wlan_reg_freq_to_chan(mac_ctx->pdev, bss_desc->chan_freq);

	lim_update_sae_config(mac_ctx, session);
	lim_update_sae_single_pmk_ap_cap(mac_ctx, session);

	/* Update the beacon/probe filter in mac_ctx */
	lim_set_bcn_probe_filter(mac_ctx, session, bss_chan_id);
	session->max_amsdu_num = mac_ctx->mlme_cfg->ht_caps.max_num_amsdu;
   ...
   */
	...
	mac_ctx->mlme_cfg->timeouts.join_failure_timeout = QDF_MIN(
		timeout, mac_ctx->mlme_cfg->timeouts.join_failure_timeout_ori);
	/*
   ...
   */
	...
	lim_join_req_update_ht_vht_caps(mac_ctx, session, bss_desc, ie_struct);
   ...
   */
	...
	session->enableAmpduPs = mac_ctx->mlme_cfg->ht_caps.enable_ampdu_ps;
	session->send_smps_action = mac_ctx->roam.configParam.send_smps_action;
	session->vhtCapability = IS_DOT11_MODE_VHT(session->dot11mode);
	...
	session->limRmfEnabled = lim_get_vdev_rmf_capable(mac_ctx, session);
	...
	if (wlan_reg_is_6ghz_chan_freq(bss_desc->chan_freq)) {
		...
		status = wlan_reg_get_best_6g_power_type(
			mac_ctx->psoc, mac_ctx->pdev, &power_type_6g,
			session->ap_defined_power_type_6g, bss_desc->chan_freq);
		if (QDF_IS_STATUS_ERROR(status)) {
   ...
   */
	...
	if (session->limRFBand != REG_BAND_2G)
		session->lim11hEnable = mac_ctx->mlme_cfg->gen.enabled_11h;
	else
		session->lim11hEnable = 0;
	...
	if (session->country_info_present &&
	    lim_isconnected_on_dfs_freq(mac_ctx, session->curr_op_freq))
   ...
   */
	if (mac_ctx->mlme_cfg->gen.enabled_11h && ap_cap_info->spectrumMgt &&
   ...
   */
	session->max_11h_pwr =
		QDF_MIN(lim_get_cfg_max_tx_power(mac_ctx, bss_desc->chan_freq),
			MAX_TX_PWR_CAP);
...
static QDF_STATUS lim_send_connect_req_to_mlm(struct pe_session *session)
{
	tLimMlmJoinReq *mlm_join_req;
	uint32_t len;
	QDF_STATUS status;
	...
	qdf_mem_copy((uint8_t *)&mlm_join_req->bssDescription.bssId,
		     (uint8_t *)&session->lim_join_req->bssDescription.bssId,
		     session->lim_join_req->bssDescription.length + 2);
...
static struct pe_session *lim_cm_create_session(struct mac_context *mac_ctx,
						struct cm_vdev_join_req *req)
{
	...
	if (pe_session) {
		pe_err("vdev_id: %d cm_id 0x%x :pe-session(%d (vdev %d)) already exists "
		       "for BSSID: " QDF_MAC_ADDR_FMT " in lim_sme_state = %X",
	...
	pe_session = pe_create_session(mac_ctx, req->entry->bssid.bytes,
				       ...
				       eSIR_INFRASTRUCTURE_MODE, req->vdev_id);
	if (!pe_session)
		pe_err("vdev_id: %d cm_id 0x%x : pe_session create failed "
...
static bool lim_is_wpa_profile(struct pe_session *session)
{
	int32_t ucast_cipher;
	int32_t auth_mode;
...
static bool lim_is_wapi_profile(struct pe_session *session)
{
	int32_t ucast_cipher;
	int32_t auth_mode;
...
static bool lim_is_rsn_profile(struct pe_session *session)
{
	int32_t ucast_cipher;
	int32_t auth_mode;
	bool is_rsn = false;
...
static enum ani_akm_type lim_get_wpa_akm(uint32_t akm)
{
...
static enum ani_akm_type lim_get_rsn_akm(uint32_t akm)
{
...
enum ani_akm_type lim_get_connected_akm(struct pe_session *session,
					int32_t ucast_cipher, int32_t auth_mode,
					int32_t akm)
{
					       ...
					       struct wlan_crypto_pmksa *pmksa)
{
	...
	qdf_mem_copy(mlme_priv->connect_info.fils_con_info->pmk, pmksa->pmk,
		     pmksa->pmk_len);
	qdf_mem_copy(mlme_priv->connect_info.fils_con_info->pmkid, pmksa->pmkid,
		     PMKID_LEN);
}
#else
static inline void lim_update_pmksa_to_profile(struct wlan_objmgr_vdev *vdev,
				 ...
				 struct cm_vdev_join_req *req)
{
	...
	rsnxe = wlan_get_ie_ptr_from_eid(WLAN_ELEMID_RSNXE, req->assoc_ie.ptr,
					 req->assoc_ie.len);
	if (!rsnxe)
		return false;
	...
	if (!rsnxe_cap || (rsnxe[SIR_MAC_IE_LEN_OFFSET] > (cap_len + 1))) {
		mlme_err(
			"RSNXE caps not present/unknown caps present. Cap len %d",
			cap_len);
		return true;
	}
...
static inline uint8_t *lim_rebuild_rsnxe_cap(uint8_t *rsnx_ie, uint8_t length)
{
	...
	QDF_TRACE_HEX_DUMP(QDF_MODULE_ID_PE, QDF_TRACE_LEVEL_DEBUG, new_rsnxe,
			   length + SIR_MAC_IE_TYPE_LEN_SIZE);
	return new_rsnxe;
}
...
lim_append_rsnxe_to_assoc_ie(struct cm_vdev_join_req *req, uint8_t *new_rsnxe)
{
...
static inline QDF_STATUS lim_strip_rsnx_ie(struct mac_context *mac_ctx,
					   struct pe_session *session,
					   struct cm_vdev_join_req *req)
{
	int32_t akm;
   ...
   */
	...
	lim_strip_ie(mac_ctx, req->assoc_ie.ptr, (uint16_t *)&req->assoc_ie.len,
		     WLAN_ELEMID_RSNXE, ONE_BYTE, NULL, 0, rsnxe,
		     WLAN_MAX_IE_LEN);
	...
	pe_err("Error in handling RSNXE. Length AP: %d SELF: %d", ap_rsnxe_len,
...
void lim_update_connect_rsn_ie(struct pe_session *session, uint8_t *rsn_ie_buf,
			       struct wlan_crypto_pmksa *pmksa)
{
	uint8_t *rsn_ie_end;
	...
	qdf_mem_copy(session->lim_join_req->rsnIE.rsnIEdata, rsn_ie_buf,
		     rsn_ie_len);
}

static QDF_STATUS lim_fill_rsn_ie(struct mac_context *mac_ctx,
				  struct pe_session *session,
				  struct cm_vdev_join_req *req)
{
	QDF_STATUS status;
	...
	status = lim_strip_ie(mac_ctx, req->assoc_ie.ptr,
			      (uint16_t *)&req->assoc_ie.len, WLAN_ELEMID_RSN,
			      ONE_BYTE, NULL, 0, rsn_ie, DOT11F_IE_RSN_MAX_LEN);
	...
	if (req->force_rsne_override && QDF_IS_STATUS_SUCCESS(status)) {
		...
		session->lim_join_req->rsnIE.length = rsn_ie_len;
		qdf_mem_copy(session->lim_join_req->rsnIE.rsnIEdata, rsn_ie,
			     rsn_ie_len);
	...
	if (bss_desc->fils_info_element.is_cache_id_present) {
		...
		qdf_mem_copy(pmksa.cache_id,
			     ...
			     CACHE_ID_LEN);
		pe_debug(
			"FILS: vdev %d Cache id =0x%x 0x%x ssid: " QDF_SSID_FMT,
...
static QDF_STATUS lim_fill_wpa_ie(struct mac_context *mac_ctx,
				  struct pe_session *session,
				  struct cm_vdev_join_req *req)
{
	QDF_STATUS status;
	...
	status = lim_strip_ie(mac_ctx, req->assoc_ie.ptr,
			      (uint16_t *)&req->assoc_ie.len, DOT11F_EID_WPA,
			      ONE_BYTE, "\x00\x50\xf2", 3, NULL, 0);
	...
	qdf_mem_copy(session->lim_join_req->rsnIE.rsnIEdata, wpa_ie, ie_len);
...
#ifdef FEATURE_WLAN_WAPI
static QDF_STATUS lim_fill_wapi_ie(struct mac_context *mac_ctx,
				   struct pe_session *session,
				   struct cm_vdev_join_req *req)
{
	QDF_STATUS status;
	...
	status = lim_strip_ie(mac_ctx, req->assoc_ie.ptr,
			      (uint16_t *)&req->assoc_ie.len, WLAN_ELEMID_WAPI,
			      ONE_BYTE, NULL, 0, NULL, 0);
	...
	session->lim_join_req->rsnIE.length = ie_len;
	qdf_mem_copy(session->lim_join_req->rsnIE.rsnIEdata, wapi_ie, ie_len);
...
#else
static inline QDF_STATUS lim_fill_wapi_ie(struct mac_context *mac_ctx,
					  struct pe_session *session,
					  struct cm_vdev_join_req *req)
{
	return QDF_STATUS_SUCCESS;
					 ...
					 struct cm_vdev_join_req *req)
{
	...
	auth_mode = wlan_crypto_get_param(session->vdev,
					  WLAN_CRYPTO_PARAM_AUTH_MODE);
	akm = wlan_crypto_get_param(session->vdev, WLAN_CRYPTO_PARAM_KEY_MGMT);
	ap_cap_info = (tSirMacCapabilityInfo *)&session->lim_join_req
			      ->bssDescription.capabilityInfo;
...
static QDF_STATUS lim_fill_session_params(struct mac_context *mac_ctx,
					  struct pe_session *session,
					  struct cm_vdev_join_req *req)
{
	QDF_STATUS status;
	...
	bss_len = (uint16_t)(offsetof(struct bss_description, ieFields[0]) +
			     ie_len);

	session->lim_join_req =
		qdf_mem_malloc(sizeof(*session->lim_join_req) + bss_len);
	...
	if (session->connected_akm != ANI_AKM_TYPE_SAE) {
		...
		wlan_mlme_clear_sae_single_pmk_info(session->vdev, NULL);
	}
	...
	qdf_mem_copy(pe_join_req->addIEAssoc.addIEdata, req->assoc_ie.ptr,
		     req->assoc_ie.len);
	...
	qdf_mem_copy(pe_join_req->addIEScan.addIEdata, req->scan_ie.ptr,
		     req->scan_ie.len);
	...
	if (session->lim_join_req->addIEAssoc.length) {
		...
		status = lim_strip_ie(mac_ctx, add_ie, &add_ie_len,
				      DOT11F_EID_EXTCAP, ONE_BYTE, NULL, 0,
...
static QDF_STATUS lim_cm_handle_join_req(struct cm_vdev_join_req *req)
{
	...
	pe_debug(
		"Freq %d width %d freq0 %d freq1 %d, Smps %d: mode %d action %d, "
		"nss 1x1 %d vdev_nss %d nss %d cbMode %d dot11mode %d subfer %d "
		"subfee %d csn %d is_cisco %d WPS %d OSEN %d fils %d akm %d",
		...
		mac_ctx->mlme_cfg->ht_caps.smps, pe_session->send_smps_action,
		pe_session->supported_nss_1x1, pe_session->vdev_nss,
		pe_session->nss, pe_session->htSupportedChannelWidthSet,
		pe_session->dot11mode, pe_session->vht_config.su_beam_former,
		...
		lim_is_fils_connection(pe_session), pe_session->connected_akm);
	...
	if (!wlan_vdev_mlme_is_mlo_link_vdev(pe_session->vdev))
		lim_send_mlo_caps_ie(mac_ctx, pe_session->vdev, QDF_STA_MODE,
				     pe_session->vdev_id);
...
static void lim_process_disconnect_sta(struct pe_session *session,
				       struct scheduler_msg *msg)
{
	if (QDF_IS_STATUS_SUCCESS(wlan_vdev_is_restart_progress(session->vdev)))
		wlan_vdev_mlme_sm_deliver_evt(session->vdev,
					      WLAN_VDEV_SM_EV_RESTART_REQ_FAIL,
					      sizeof(*msg), msg);
	else
		wlan_vdev_mlme_sm_deliver_evt(
			session->vdev, WLAN_VDEV_SM_EV_DOWN, sizeof(*msg), msg);
}
					  ...
					  struct wlan_cm_vdev_discon_req *req)
{
	...
	case REASON_DEAUTH_NETWORK_LEAVING:
		wlan_mlme_get_enable_deauth_to_disassoc_map(
			mac_ctx->psoc, &enable_deauth_to_disassoc_map);
...
lim_get_disconnect_session(struct mac_context *mac_ctx,
			   struct wlan_cm_vdev_discon_req *req)
{
	struct pe_session *session;
   ...
   */
	if (!session) {
		...
		if (session)
			pe_info("vdev_id %d cm_id 0x%x: using vdev id, session (%d) found for "
				"bssid " QDF_MAC_ADDR_FMT
				" [bssid in req " QDF_MAC_ADDR_FMT
				"] sme state %d mlm state %d",
   ...
   */
	if (session && QDF_IS_STATUS_SUCCESS(
			       wlan_vdev_mlme_is_init_state(session->vdev))) {
		pe_err("vdev_id %d cm_id 0x%x: sme state %d mlm state %d: vdev is in INIT "
...
lim_cm_handle_disconnect_req(struct wlan_cm_vdev_discon_req *req)
{
	...
	if (!pe_session) {
		pe_err("vdev_id %d cm_id 0x%x: bssid " QDF_MAC_ADDR_FMT
		       " : no session found",
		       req->req.vdev_id, req->cm_id,
		       QDF_MAC_ADDR_REF(req->req.bssid.bytes));
...
static void wma_get_mld_info_sta(struct cm_peer_create_req *req,
				 uint8_t **peer_mld_addr, bool *is_assoc_peer)
{
...
static void lim_handle_reassoc_req(struct cm_vdev_join_req *req)
{
	...
	session_entry = pe_find_session_by_bssid(
		mac_ctx, req->entry->bssid.bytes, &session_id);
	if (!session_entry) {
	...
	reassoc_req =
		qdf_mem_malloc(sizeof(*session_entry->lim_join_req) + bss_len);
	if (!reassoc_req) {
		ret_code = eSIR_SME_RESOURCES_UNAVAILABLE;
		goto end;
	}
	...
	status = wlan_get_parsed_bss_description_ies(mac_ctx, bss_desc,
						     &ie_struct);
	if (QDF_IS_STATUS_ERROR(status)) {
		pe_err("IE parsing failed vdev id %d", session_entry->vdev_id);
		session_entry->lim_join_req = NULL;
		ret_code = eSIR_SME_RESOURCES_UNAVAILABLE;
		goto end;
	}
	...
	if (req->assoc_ie.len)
		QDF_TRACE_HEX_DUMP(QDF_MODULE_ID_PE, QDF_TRACE_LEVEL_DEBUG,
				   req->assoc_ie.ptr, req->assoc_ie.len);
	qdf_mem_copy(reassoc_req->addIEAssoc.addIEdata, req->assoc_ie.ptr,
		     req->assoc_ie.len);
	...
	pe_debug(
		"enableHtSmps: %d htSmps: %d send action: %d supported nss 1x1: %d",
		mac_ctx->mlme_cfg->ht_caps.enable_smps,
	...
	if (session_entry->limSmeState != eLIM_SME_LINK_EST_STATE) {
       ...
       */
			...
			if (qdf_mem_cmp(req->entry->bssid.bytes,
					...
					QDF_MAC_ADDR_SIZE)) {
				pe_err("Requested BSSID: " QDF_MAC_ADDR_FMT
				       " but bssId in reassoc state" QDF_MAC_ADDR_FMT,
				       QDF_MAC_ADDR_REF(
	...
	pe_info("Reg max = %d, local pwr constraint = %d, max tx = %d", reg_max,
		local_pwr_constraint, session_entry->maxTxPower);
	/* Copy the SSID from session entry to local variable */
	session_entry->limReassocSSID.length = req->entry->ssid.length;
	qdf_mem_copy(session_entry->limReassocSSID.ssId, req->entry->ssid.ssid,
   ...
   */
	...
	if (session_entry->country_info_present &&
	    lim_isconnected_on_dfs_freq(mac_ctx, session_entry->curr_op_freq))
		session_entry->spectrumMgtEnabled = true;
   ...
   */
	lim_send_sme_join_reassoc_rsp(mac_ctx, eWNI_SME_REASSOC_RSP, ret_code,
						  ...
						  uint8_t vdev_id)
{
	...
	self_dot11_mode =
		lim_get_self_dot11_mode(mac_ctx, QDF_STA_MODE, vdev_id);
	/* if user set dot11 mode by cmd, need to do intersect first */
	self_dot11_mode = lim_intersect_user_dot11_mode(
		mac_ctx, QDF_STA_MODE, vdev_id, self_dot11_mode);
...
static QDF_STATUS lim_cm_handle_preauth_req(struct wlan_preauth_req *req)
{
	...
	qdf_mem_copy(&preauth_req->preAuthbssId, scan_entry->bssid.bytes,
		     QDF_MAC_ADDR_SIZE);
		      ...
		      tDot11fIEhe_op *he_op, bool *has_tpe_updated)
{
   ...
   */
	...
	if (session->sta_follows_sap_power) {
		pe_debug_rl(
			"STA operates in 6 GHz power of SAP, do not update STA power");
		return;
	}
	...
	*has_tpe_updated = false;
	...
	if (psd_set) {
		...
		if (!num_octets) {
			...
			num_octets =
				lim_get_num_pwr_levels(true, session->ch_width);
			vdev_mlme->reg_tpc_obj.num_pwr_levels = num_octets;
...
void lim_calculate_tpc(struct mac_context *mac, struct pe_session *session)
{
	...
	wlan_reg_set_channel_params_for_pwrmode(
		mac->pdev, oper_freq, 0, &ch_params, REG_CURRENT_PWR_MODE);
	if (ch_params.mhz_freq_seg1)
     ...
     */
		...
		num_pwr_levels =
			lim_get_num_pwr_levels(is_psd_power, session->ch_width);
	}
			...
			if (is_6ghz_freq) {
				if (LIM_IS_STA_ROLE(session)) {
					...
					if (wlan_reg_decide_6ghz_power_within_bw_for_freq(
					    ...
					    QDF_STATUS_SUCCESS) {
						pe_debug(
							"get pwr attr from secondary list");
	...
	pe_debug("num_pwr_levels: %d, is_psd_power: %d, total eirp_power: %d, "
		 "ap_pwr_type: %d",
					   ...
					   uint32_t *msg_buf)
{
	...
	if (!lim_is_sme_disassoc_req_valid(mac, &smeDisassocReq, pe_session)) {
	...
	pe_session = pe_find_session_by_bssid_and_vdev_id(
		mac, smeDisassocReq.bssid.bytes, smeDisassocReq.sessionId,
		&sessionId);
	if (!pe_session) {
	...
	pe_debug(
		"vdev %d (%d) opmode %d Reason: %u SmeState: %d limMlmState %d ho "
		"fail %d send OTA %d to: " QDF_MAC_ADDR_FMT
		" bssid " QDF_MAC_ADDR_FMT,
	...
	case eLIM_STA_ROLE:
		...
		case eLIM_SME_LINK_EST_STATE:
			pe_session->limPrevSmeState = pe_session->limSmeState;
       ...
       * for disassoc frame.
       *
       * It will send a disassoc, which is ok. However, we can use the global
       * flag sendDisassoc to not send disassoc frame.
       */
			break;
	...
	lim_post_mlm_message(mac, LIM_MLM_DISASSOC_REQ,
			     (uint32_t *)pMlmDisassocReq);
	return;
...
void __lim_process_sme_disassoc_cnf(struct mac_context *mac, uint32_t *msg_buf)
{
	...
	pe_session = pe_find_session_by_bssid_and_vdev_id(
		mac, smeDisassocCnf.bssid.bytes, smeDisassocCnf.vdev_id,
		&sessionId);
	if (!pe_session) {
		pe_err("session does not exist for bssid " QDF_MAC_ADDR_FMT,
		       QDF_MAC_ADDR_REF(smeDisassocCnf.bssid.bytes));
		status = lim_prepare_disconnect_done_ind(
			mac, &msg, smeDisassocCnf.vdev_id,
			eSIR_SME_INVALID_SESSION, NULL);
		if (QDF_IS_STATUS_SUCCESS(status))
			lim_send_sme_disassoc_deauth_ntf(
				mac, QDF_STATUS_SUCCESS, (uint32_t *)msg);
		return;
	}
	...
	if (!lim_is_sme_disassoc_cnf_valid(mac, &smeDisassocCnf, pe_session)) {
		...
		status = lim_prepare_disconnect_done_ind(
			mac, &msg, pe_session->smeSessionId,
			eSIR_SME_INVALID_PARAMETERS,
	...
	if (smeDisassocCnf.messageType == eWNI_SME_DISASSOC_CNF)
		...
		lim_diag_event_report(mac, WLAN_PE_DIAG_DEAUTH_CNF_EVENT,
				      ...
				      (uint16_t)smeDisassocCnf.status_code, 0);
#endif /* FEATURE_WLAN_DIAG_SUPPORT */
	pe_debug("vdev %d (%d) opmode %d SmeState: %d limMlmState %d "
		 "from: " QDF_MAC_ADDR_FMT " bssid " QDF_MAC_ADDR_FMT,
		 pe_session->vdev_id, pe_session->peSessionId,
	...
	if ((pe_session->limSmeState == eLIM_SME_WT_DISASSOC_STATE) ||
		...
		sta = dph_lookup_hash_entry(mac,
					    smeDisassocCnf.peer_macaddr.bytes,
					    &aid,
					    &pe_session->dph.dphHashTable);
		if (!sta) {
			pe_err("DISASSOC_CNF for a STA with no context, addr= " QDF_MAC_ADDR_FMT,
			       QDF_MAC_ADDR_REF(
				       smeDisassocCnf.peer_macaddr.bytes));
			status = lim_prepare_disconnect_done_ind(
				mac, &msg, pe_session->smeSessionId,
		...
		if ((sta->mlmStaContext.mlmState ==
			...
			pe_err("No need of cleanup for addr:" QDF_MAC_ADDR_FMT
			       "as MLM state is %d",
			       QDF_MAC_ADDR_REF(
				       smeDisassocCnf.peer_macaddr.bytes),
			       sta->mlmStaContext.mlmState);
		...
		lim_clean_up_disassoc_deauth_req(
			mac, (char *)&smeDisassocCnf.peer_macaddr, 0);
	}
					 ...
					 uint32_t *msg_buf)
{
   ...
   */
	session_entry = pe_find_session_by_bssid_and_vdev_id(
		mac_ctx, sme_deauth_req.bssid.bytes, sme_deauth_req.vdev_id,
		&session_id);
	if (!session_entry) {
		pe_err("session does not exist for bssid " QDF_MAC_ADDR_FMT,
	...
	pe_debug(
		"vdev %d (%d) opmode %d reasoncode %u limSmestate %d limMlmState %d "
		"to " QDF_MAC_ADDR_FMT " bssid " QDF_MAC_ADDR_FMT,
		vdev_id, session_entry->peSessionId, session_entry->opmode,
		sme_deauth_req.reasonCode, session_entry->limSmeState,
...
static void __lim_counter_measures(struct mac_context *mac,
				   struct pe_session *pe_session)
{
	tSirMacAddr mac_addr = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };

	if (LIM_IS_AP_ROLE(pe_session))
		lim_send_disassoc_mgmt_frame(mac, REASON_MIC_FAILURE, mac_addr,
					     pe_session, false);
};
...
static void
lim_flush_all_peer_from_serialization_queue(struct mac_context *mac_ctx,
					    struct pe_session *session)
{
	...
	if (!LIM_IS_NDI_ROLE(session)) {
		...
		if (session->stop_bss_reason == eSIR_SME_MIC_COUNTER_MEASURES)
			...
			lim_send_disassoc_mgmt_frame(
				mac_ctx, REASON_DEAUTH_NETWORK_LEAVING, bc_addr,
				session, false);
	}
...
static void lim_delete_peers_and_send_vdev_stop(struct pe_session *session)
{
	...
	if (QDF_IS_STATUS_SUCCESS(wlan_vdev_is_restart_progress(session->vdev)))
		status = wlan_vdev_mlme_sm_deliver_evt(
			session->vdev, WLAN_VDEV_SM_EV_RESTART_REQ_FAIL,
...
static void __lim_handle_sme_stop_bss_request(struct mac_context *mac,
					      uint32_t *msg_buf)
{
	...
	pe_session = pe_find_session_by_bssid(mac, bssid.bytes, &session_id);
	if (!pe_session) {
		pe_err("session does not exist for bssid " QDF_MAC_ADDR_FMT,
		       QDF_MAC_ADDR_REF(bssid.bytes));
	...
	if (pe_session->limSmeState !=
		    eLIM_SME_NORMAL_STATE || /* Added For BT -AMP Support */
	...
	MTRACE(mac_trace(mac, TRACE_CODE_SME_STATE, pe_session->peSessionId,
			 pe_session->limSmeState));
				 ...
				 struct pe_session *pe_session)
{
	...
	lim_send_stop_bss_response(mac, pe_session->vdev_id, eSIR_SME_SUCCESS);
	return;
}
...
void __lim_process_sme_assoc_cnf_new(struct mac_context *mac_ctx,
				     uint32_t msg_type, uint32_t *msg_buf)
{
	...
	if (qdf_mem_cmp((uint8_t *)sta_ds->staAddr,
			...
			QDF_MAC_ADDR_SIZE)) {
		pe_debug(
			"peerMacAddr mismatched for aid %d, peer " QDF_MAC_ADDR_FMT,
			assoc_cnf.aid,
			QDF_MAC_ADDR_REF(assoc_cnf.peer_macaddr.bytes));
		goto end;
	}
	...
	if ((sta_ds->mlmStaContext.mlmState != eLIM_MLM_WT_ASSOC_CNF_STATE) ||
	    ...
	    ((sta_ds->mlmStaContext.subType == LIM_REASSOC) &&
	     (msg_type != eWNI_SME_ASSOC_CNF))) {
		pe_debug(
			"peer " QDF_MAC_ADDR_FMT
			" not in WT_ASSOC_CNF_STATE, for aid %d, sta mlmstate %d",
			QDF_MAC_ADDR_REF(assoc_cnf.peer_macaddr.bytes),
   ...
   */
     ...
     */
		...
		pe_debug(
			"Recv Assoc Cnf, status Code : %d(assoc id=%d) Reason code: %d",
		...
		if (assoc_cnf.mac_status_code == STATUS_INVALID_PMKID ||
		    assoc_cnf.mac_status_code == STATUS_NOT_SUPPORTED_AUTH_ALG)
			add_pre_auth_context = false;

		lim_reject_association(
			mac_ctx, sta_ds->staAddr, sta_ds->mlmStaContext.subType,
			add_pre_auth_context, sta_ds->mlmStaContext.authType,
			sta_ds->assocId, true, mac_status_code, session_entry);
	}
end:
...
static void __lim_process_sme_addts_req(struct mac_context *mac,
					uint32_t *msg_buf)
{
	tpDphHashNode sta;
	tSirMacAddr peerMac;
	...
	if (!pe_session) {
		...
		lim_send_sme_addts_rsp(mac, pSirAddts->rspReqd,
				       QDF_STATUS_E_FAILURE, NULL,
				       pSirAddts->req.tspec, smesessionId);
		return;
	}
#ifdef FEATURE_WLAN_DIAG_SUPPORT_LIM /* FEATURE_WLAN_DIAG_SUPPORT */
   ...
   */
	...
	if (mac->lim.gLimAddtsSent) {
		pe_err("Addts (token %d, tsid %d, up %d) is still pending",
		       ...
		       mac->lim.gLimAddtsReq.req.tspec.tsinfo.traffic.userPrio);
		goto send_failure_addts_rsp;
	}
	...
	qdf_mem_copy((uint8_t *)&mac->lim.gLimAddtsReq, (uint8_t *)pSirAddts,
		     sizeof(tSirAddtsReq));
	...
	if (tx_timer_change(&mac->lim.lim_timers.gLimAddtsRspTimer, timeout,
			    0) != TX_SUCCESS) {
...
send_failure_addts_rsp:
	lim_send_sme_addts_rsp(mac, pSirAddts->rspReqd, QDF_STATUS_E_FAILURE,
			       pe_session, pSirAddts->req.tspec, smesessionId);
}

#ifdef WLAN_FEATURE_MSCS
static void __lim_process_sme_mscs_req(struct mac_context *mac,
				       uint32_t *msg_buf)
{
	...
	/* ship out the message now */
	lim_send_mscs_req_action_frame(mac, peer_mac, mscs_req, pe_session);
}
#else
static inline void __lim_process_sme_mscs_req(struct mac_context *mac,
					      uint32_t *msg_buf)
{
	return;
}
...
static void __lim_process_sme_delts_req(struct mac_context *mac,
					uint32_t *msg_buf)
{
	...
	pe_session = pe_find_session_by_bssid(mac, pDeltsReq->bssid.bytes,
					      &sessionId);
	if (!pe_session) {
		pe_err("Session Does not exist for given bssId");
		status = QDF_STATUS_E_FAILURE;
	...
	pTsinfo = pDeltsReq->req.wmeTspecPresent ?
			  &pDeltsReq->req.tspec.tsinfo :
			  &pDeltsReq->req.tsinfo;
   ...
   */
	lim_set_tspec_uapsd_mask_per_session(mac, pe_session, pTsinfo,
					     CLEAR_UAPSD_MASK);
...
void lim_process_sme_addts_rsp_timeout(struct mac_context *mac, uint32_t param)
{
	...
	pe_session = pe_find_session_by_session_id(
		mac, mac->lim.lim_timers.gLimAddtsRspTimer.sessionId);
	if (!pe_session) {
					       ...
					       uint32_t vdev_id)
{
	...
	sta_ds_ptr = dph_get_hash_entry(mac_ctx, DPH_STA_HASH_INDEX_PEER,
					&pe_session->dph.dphHashTable);
	if (sta_ds_ptr)
		lim_send_edca_params(mac_ctx, pe_session->gLimEdcaParamsActive,
				     pe_session->vdev_id, false);
	else
		pe_err("Self entry missing in Hash Table");
}
					       ...
					       uint32_t *msg_buf)
{
	...
	pe_session->gLimEdcaParams[ac].cw.min = msg->txq_edca_params.cw.min;
	pe_session->gLimEdcaParams[ac].cw.max = msg->txq_edca_params.cw.max;
	pe_session->gLimEdcaParams[ac].aci.aci = msg->txq_edca_params.aci.aci;
	...
	lim_send_edca_params(mac_ctx, pe_session->gLimEdcaParams,
			     pe_session->vdev_id, false);
}
...
static void lim_process_sme_cfg_action_frm_in_tb_ppdu(
	struct mac_context *mac_ctx, struct sir_cfg_action_frm_tb_ppdu *msg)
{
	if (!msg) {
...
static void lim_process_sme_send_vdev_pause(struct mac_context *mac_ctx,
					    struct sme_vdev_pause *msg)
{
	...
	vdev_pause_dur_ms =
		session->beaconParams.beaconInterval * msg->vdev_pause_duration;
	wlan_mlo_send_vdev_pause(mac_ctx->psoc, session->vdev, msg->session_id,
				 vdev_pause_dur_ms);
}
...
void lim_send_vdev_restart(struct mac_context *mac,
					     ...
					     uint32_t *msg_buf)
{
	...
	pe_debug("received SME Session update for %d val %d", msg->param_type,
		 msg->param_val);
					...
					uint32_t *msg_buf)
{
	...
	pe_session = pe_find_session_by_bssid(mac, pChangeBIParams->bssid.bytes,
					      &sessionId);
	if (!pe_session) {
		pe_err("Session does not exist for given BSSID");
		return;
	...
	if (mac->sch.beacon_interval != pChangeBIParams->beacon_interval) {
		mac->sch.beacon_interval = pChangeBIParams->beacon_interval;
					      ...
					      uint32_t *msg_buf)
{
	...
	pe_session = pe_find_session_by_bssid(mac, pSetHT2040Mode->bssid.bytes,
					      &sessionId);
	if (!pe_session) {
		pe_debug(
			"Session does not exist for given BSSID: " QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(pSetHT2040Mode->bssid.bytes));
		return;
	}

	pe_debug("Update session entry for cbMod=%d", pSetHT2040Mode->cbMode);
	...
	for (staId = 0; staId < pe_session->dph.dphHashTable.size; staId++) {
		...
		if (sta->valid && sta->htSupportedChannelWidthSet) {
			...
			if (!QDF_IS_STATUS_SUCCESS(scheduler_post_message(
				    QDF_MODULE_ID_PE, QDF_MODULE_ID_WMA,
 ...
 *
 * FUNCTION:  Send SIR_HAL_SET_MAX_TX_POWER_REQ message to change the max tx
 * power.
 *
 * LOGIC:
 *
 ...
 */
static void
__lim_process_sme_register_mgmt_frame_req(struct mac_context *mac_ctx,
					  uint32_t *msg_buf)
{
...
skip_match:
	...
	if (sme_req->registerFrame) {
		lim_mgmt_regn = qdf_mem_malloc(
			sizeof(struct mgmt_frm_reg_info) + sme_req->matchLen);
		if (lim_mgmt_regn) {
					      ...
					      void *msg_buf)
{
   ...
   */
	...
	pe_debug("msg_type %d len %d vdev_id %d mac: " QDF_MAC_ADDR_FMT
		 " reason %d wait_for_ack %d",
		 req->msg_type, req->length, req->vdev_id,
		 QDF_MAC_ADDR_REF(req->peer_mac), req->reason,
		 req->wait_for_ack);
	...
	for (i = 1; i <= nss; i++) {
		...
		*(ie_params->ie_ptr + 1) = ie_params->ie_len - 2;
		...
		if (NSS_1x1_MODE == i) {
			...
			vht_cap->txSTBC = 0;
			vht_mcs = (tSirVhtMcsInfo *)&p_ie
				[2 + sizeof(tSirMacVHTCapabilityInfo)];
 ...
 * Return: None
 */
static void lim_process_set_pdev_IEs(struct mac_context *mac_ctx,
				     uint32_t *msg_buf)
{
	...
	pe_debug("rcvd set pdev ht vht ie req with nss = %d", ht_vht_cfg->nss);
 ...
 */
static void
lim_process_sme_update_access_policy_vendor_ie(struct mac_context *mac_ctx,
					       uint32_t *msg)
{
	...
	pe_session_entry =
		pe_find_session_by_vdev_id(mac_ctx, update_vendor_ie->vdev_id);
					 ...
					 struct scheduler_msg *msg)
{
	...
	session = pe_find_session_by_bssid(
		mac_ctx, sme_disassoc_cnf.bssid.bytes, &session_id);
	if (!session) {
		pe_err("session not found for bssid:" QDF_MAC_ADDR_FMT,
		       QDF_MAC_ADDR_REF(sme_disassoc_cnf.bssid.bytes));
		status = lim_prepare_disconnect_done_ind(
			mac_ctx, &err_msg, sme_disassoc_cnf.vdev_id,
			eSIR_SME_INVALID_SESSION, NULL);

		if (QDF_IS_STATUS_SUCCESS(status))
			lim_send_sme_disassoc_deauth_ntf(
				mac_ctx, QDF_STATUS_SUCCESS, err_msg);
		return;
	}
					 ...
					 struct scheduler_msg *msg)
{
	...
	session = pe_find_session_by_bssid(mac_ctx, disassoc_req.bssid.bytes,
					   &session_id);
	if (!session) {
		pe_err("session not found for bssid:" QDF_MAC_ADDR_FMT,
				       ...
				       struct scheduler_msg *msg)
{
	...
	session = pe_find_session_by_bssid(mac_ctx, sme_deauth_req.bssid.bytes,
					   &session_id);
	if (!session) {
		pe_err("session not found for bssid:" QDF_MAC_ADDR_FMT,
		       QDF_MAC_ADDR_REF(sme_deauth_req.bssid.bytes));
				  ...
				  struct scheduler_msg *pMsg)
{
   ...
   */
	...
	pe_nofl_debug("LIM handle SME Msg %s(%d)", lim_msg_str(pMsg->type),
		      pMsg->type);
	...
	case eWNI_SME_SET_ADDBA_ACCEPT:
		lim_process_sme_set_addba_accept(
			mac, (struct sme_addba_accept *)msg_buf);
		break;
	...
	case WNI_SME_CFG_ACTION_FRM_HE_TB_PPDU:
		lim_process_sme_cfg_action_frm_in_tb_ppdu(
			mac, (struct sir_cfg_action_frm_tb_ppdu *)msg_buf);
		break;
	case eWNI_SME_VDEV_PAUSE_IND:
		lim_process_sme_send_vdev_pause(
			mac, (struct sme_vdev_pause *)msg_buf);
		break;
	default:
					     ...
					     uint32_t *pMsg)
{
	...
	pe_session = pe_find_session_by_bssid(mac, pBeaconStartInd->bssid,
					      &sessionId);
	if (!pe_session) {
		pe_err("Session does not exist for given bssId: " QDF_MAC_ADDR_FMT,
     ...
     */
		...
		lim_send_obss_color_collision_cfg(
			mac, pe_session, OBSS_COLOR_COLLISION_DETECTION);
	} else {
...
static void lim_mon_change_channel(struct mac_context *mac_ctx,
				   struct pe_session *session_entry)
{
...
static void lim_change_channel(struct mac_context *mac_ctx,
			       struct pe_session *session_entry)
{
						   ...
						   uint32_t *msg_buf)
{
	...
	max_tx_pwr = wlan_reg_get_channel_reg_power_for_freq(mac_ctx->pdev,
							     target_freq);
	if (max_tx_pwr == WMA_MAX_TXPOWER_INVALID) {
		pe_err("Invalid max tx power");
		return;
	}
	wlan_mlme_get_mac_vdev_id(mac_ctx->pdev, ch_change_req->vdev_id,
				  &bssid);
	session_entry =
		pe_find_session_by_bssid(mac_ctx, bssid.bytes, &session_id);
	if (!session_entry) {
		pe_err("Session does not exist for bssid " QDF_MAC_ADDR_FMT,
		       QDF_MAC_ADDR_REF(bssid.bytes));
		return;
	...
	if ((session_entry->curr_op_freq == target_freq &&
		...
		pe_err("Target channel and mode is same as current channel and mode "
		       "channel freq %d and mode %d",
	...
	session_entry->ch_width = ch_change_req->ch_width;
	session_entry->ch_center_freq_seg0 = ch_change_req->center_freq_seg0;
	session_entry->ch_center_freq_seg1 = ch_change_req->center_freq_seg1;
	...
	session_entry->htRecommendedTxWidthSet =
		session_entry->htSupportedChannelWidthSet;
	session_entry->curr_op_freq = target_freq;
	session_entry->limRFBand = lim_get_rf_band(session_entry->curr_op_freq);
	...
	qdf_mem_copy(&session_entry->rateSet, &ch_change_req->opr_rates,
		     sizeof(session_entry->rateSet));
	qdf_mem_copy(&session_entry->extRateSet, &ch_change_req->ext_rates,
				       ...
				       uint32_t *msg_buf)
{
	...
	session_entry = pe_find_session_by_bssid(
		mac_ctx, modify_add_ies->modifyIE.bssid.bytes, &session_id);
	...
	if ((0 == modify_add_ies->modifyIE.ieBufferlength) ||
		...
		pe_err("Invalid request pIEBuffer %pK ieBufferlength %d ieIDLen %d ieID "
		       "%d. update Type %d",
		       modify_add_ies->modifyIE.pIEBuffer,
	...
	case eUPDATE_IE_PROBE_BCN:
		...
		break;
	default:
		pe_err("unhandled buffer type %d", modify_add_ies->updateType);
		break;
	}
end:
				       ...
				       uint32_t *msg_buf)
{
	...
	/* incoming message has smeSession, use BSSID to find PE session */
	session_entry = pe_find_session_by_bssid(
		mac_ctx, update_ie->bssid.bytes, &session_id);

	if (!session_entry) {
		pe_debug("Session not found for given bssid" QDF_MAC_ADDR_FMT,
		...
		case eUPDATE_IE_PROBE_BCN:
			...
			if (update_ie->notify)
				lim_handle_param_update(
					mac_ctx, update_add_ies->updateType);
			break;
		default:
			break;
		}
					    ...
					    struct pe_session *session_entry)
{
	...
	op_class = lim_op_class_from_bandwidth(
		mac_ctx, new_channel_freq, ch_bandwidth,
	...
	if (LIM_IS_AP_ROLE(session_entry)) {
		for (i = 0; i <= mac_ctx->lim.max_sta_of_pe_session; i++) {
			psta = session_entry->dph.dphHashTable.pDphNodeArray +
			       i;
			if (psta && psta->added)
					       ...
					       uint32_t *msg_buf)
{
	...
	session_entry = pe_find_session_by_bssid(mac_ctx, dfs_csa_ie_req->bssid,
						 &session_id);
	if (!session_entry) {
		pe_err("Session not found for given BSSID" QDF_MAC_ADDR_FMT,
	...
	/* target channel */
	session_entry->gLimChannelSwitch.primaryChannel = wlan_reg_freq_to_chan(
		mac_ctx->pdev, dfs_csa_ie_req->target_chan_freq);
	...
	wlan_reg_set_channel_params_for_pwrmode(
		mac_ctx->pdev, dfs_csa_ie_req->target_chan_freq, 0,
		&dfs_csa_ie_req->ch_params, REG_CURRENT_PWR_MODE);
...
skip_vht:
   ...
   */
	...
	if (CHAN_HOP_ALL_BANDS_ENABLE &&
		...
		send_extended_chan_switch_action_frame(mac_ctx, target_ch_freq,
						       ch_width, session_entry);
}
					   ...
					   uint32_t *msg)
{
	...
	send_extended_chan_switch_action_frame(
		mac_ctx, ext_chng_channel->new_ch_freq, 0, session_entry);
}
 ...
 * Return: None
 */
static void lim_nss_update_rsp(struct mac_context *mac_ctx, uint8_t vdev_id,
			       QDF_STATUS status)
{
	struct scheduler_msg msg = { 0 };
...
void lim_send_bcn_rsp(struct mac_context *mac_ctx, tpSendbeaconParams rsp)
{
	...
	pe_debug("Send beacon resp status %d for reason %d", rsp->status,
		 rsp->reason);
 ...
 * Return: None
 */
static void lim_process_set_ie_req(struct mac_context *mac_ctx,
				   uint32_t *msg_buf)
{
 ...
 */
static void
obss_color_collision_process_color_disable(struct mac_context *mac_ctx,
					   struct pe_session *session)
{
	...
	if (session->valid && !LIM_IS_AP_ROLE(session)) {
		pe_err("Invalid SystemRole %d", GET_LIM_SYSTEM_ROLE(session));
		return;
	}
	...
	if (session->bss_color_changing == 1) {
		...
		lim_send_obss_color_collision_cfg(
			mac_ctx, session, OBSS_COLOR_COLLISION_DETECTION);
		return;
	}
	...
	if (session->he_op.bss_col_disabled == 1) {
		...
		lim_send_obss_color_collision_cfg(
			mac_ctx, session, OBSS_COLOR_FREE_SLOT_AVAILABLE);
		return;
	}
 ...
 * Return: None
 */
static void obss_color_collision_process_color_change(
		...
		if (session->he_op.bss_color < 32)
			...
			is_color_collision =
				(obss_color_info->obss_color_bitmap_bit32to63 >>
				 (session->he_op.bss_color - 32)) &
				0x01;
		if (!is_color_collision) {
			pe_err("%d: color collision not found, curr_color: %d",
			       session->smeSessionId, session->he_op.bss_color);
			return;
		}
	}
...
void lim_process_set_he_bss_color(struct mac_context *mac_ctx,
				  uint32_t *msg_buf)
{
	...
	lim_send_obss_color_collision_cfg(
		mac_ctx, session_entry, OBSS_COLOR_COLLISION_DETECTION_DISABLE);
}
				  ...
				  uint32_t *msg_buf)
{
	...
	if (!session->he_capable ||
		...
		pe_debug(
			"%d: obss color det not enabled, he_cap:%d, sup:%d:%d event_type:%d",
...
void lim_send_obss_color_collision_cfg(
	struct mac_context *mac_ctx, struct pe_session *session,
					   ...
					   uint32_t *msg_buf)
{
	...
	case OBSS_COLOR_FREE_SLOT_TIMER_EXPIRY:
		...
		break;
	default:
		pe_err("%d: Invalid event type %d", obss_color_info->vdev_id,
		       obss_color_info->evt_type);
		return;
	}
}
