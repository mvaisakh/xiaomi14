...
static bool lim_create_non_ap_timers(struct mac_context *mac)
{
	...
	cfgValue =
		SYS_MS_TO_TICKS(mac->mlme_cfg->timeouts.join_failure_timeout);
	/* Create Join failure timer and activate it later */
	if (tx_timer_create(mac, &mac->lim.lim_timers.gLimJoinFailureTimer,
			    "JOIN FAILURE TIMEOUT", lim_timer_handler,
			    SIR_LIM_JOIN_FAIL_TIMEOUT, cfgValue, 0,
	...
	if (tx_timer_create(
		    mac, &mac->lim.lim_timers.gLimPeriodicJoinProbeReqTimer,
		    "Periodic Join Probe Request Timer", lim_timer_handler,
		    SIR_LIM_PERIODIC_JOIN_PROBE_REQ_TIMEOUT, cfgValue, 0,
		    TX_NO_ACTIVATE) != TX_SUCCESS) {
	...
	if ((tx_timer_create(
		    mac, &mac->lim.lim_timers.g_lim_periodic_auth_retry_timer,
		    "Periodic AUTH Timer", lim_timer_handler,
	...
	cfgValue =
		SYS_MS_TO_TICKS(mac->mlme_cfg->timeouts.assoc_failure_timeout);
	...
	if (tx_timer_create(mac, &mac->lim.lim_timers.gLimAddtsRspTimer,
			    ...
			    SIR_LIM_ADDTS_RSP_TIMEOUT, cfgValue, 0,
			    TX_NO_ACTIVATE) != TX_SUCCESS) {
	...
	cfgValue =
		SYS_MS_TO_TICKS(mac->mlme_cfg->timeouts.auth_failure_timeout);
	/* Create Auth failure timer and activate it later */
	if (tx_timer_create(mac, &mac->lim.lim_timers.gLimAuthFailureTimer,
			    "AUTH FAILURE TIMEOUT", lim_timer_handler,
			    SIR_LIM_AUTH_FAIL_TIMEOUT, cfgValue, 0,
			    TX_NO_ACTIVATE) != TX_SUCCESS) {
   ...
   * authentication.
   */
	if ((tx_timer_create(mac, &mac->lim.lim_timers.sae_auth_timer,
			     "SAE AUTH Timer", lim_timer_handler,
			     SIR_LIM_AUTH_SAE_TIMEOUT,
...
uint32_t lim_create_timers(struct mac_context *mac)
{
	...
	for (i = 0; i < (mac->lim.maxStation + 1); i++) {
		if (tx_timer_create(
			    mac, &mac->lim.lim_timers.gpLimCnfWaitTimer[i],
			    "CNF_MISS_TIMEOUT", lim_cnf_wait_tmer_handler,
			    (uint32_t)i, cfgValue, 0,
			    TX_NO_ACTIVATE) != TX_SUCCESS) {
	...
	cfgValue = SYS_MS_TO_TICKS(mac->mlme_cfg->timeouts.olbc_detect_timeout);
	...
	if (tx_timer_create(mac, &mac->lim.lim_timers.gLimDisassocAckTimer,
			    "DISASSOC ACK TIMEOUT", lim_timer_handler,
			    SIR_LIM_DISASSOC_ACK_TIMEOUT, cfgValue, 0,
			    TX_NO_ACTIVATE) != TX_SUCCESS) {
	...
	if (tx_timer_create(mac, &mac->lim.lim_timers.gLimDeauthAckTimer,
			    ...
			    WLAN_INVALID_VDEV_ID, cfgValue, 0,
			    TX_NO_ACTIVATE) != TX_SUCCESS) {
...
void lim_assoc_failure_timer_handler(void *mac_global, uint32_t param)
{
	...
	if (LIM_REASSOC == param && session &&
		...
		if (mac_ctx->lim.reAssocRetryAttempt <
			...
			lim_send_retry_reassoc_req_frame(
				mac_ctx, session->pLimMlmReassocRetryReq,
				session);
...
void lim_deactivate_and_change_timer(struct mac_context *mac, uint32_t timerId)
{
	...
	case eLIM_AUTH_SAE_TIMER:
		if (tx_timer_deactivate(&mac->lim.lim_timers.sae_auth_timer) !=
		    TX_SUCCESS)
		...
		if (tx_timer_change(&mac->lim.lim_timers.sae_auth_timer, val,
				    0) != TX_SUCCESS)
...
void lim_deactivate_and_change_per_sta_id_timer(struct mac_context *mac,
						uint32_t timerId,
						uint16_t staId)
{
	uint32_t val;
	...
	case eLIM_AUTH_RSP_TIMER: {
		...
		pAuthNode = lim_get_pre_auth_node_from_index(
			mac, &mac->lim.gLimPreAuthTimerTable, staId);

		if (!pAuthNode) {
			pe_err("Invalid Pre Auth Index passed :%d", staId);
			break;
		}
		...
		/* Change timer to reactivate it in future */
		val = SYS_MS_TO_TICKS(mac->mlme_cfg->timeouts.auth_rsp_timeout);
...
void lim_activate_auth_rsp_timer(struct mac_context *mac,
				 tLimPreAuthNode *pAuthNode)
{
