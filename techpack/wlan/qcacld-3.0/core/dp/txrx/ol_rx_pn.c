/*
 * Copyright (c) 2011, 2013-2017, 2019-2021 The Linux Foundation. All rights
 * reserved.
 *
...
#define ADD_MPDU_TO_LIST(head, tail, mpdu, mpdu_tail)  \
	do {                                           \
		if (!head) {                           \
			head = mpdu;                   \
		} else {                               \
...
int ol_rx_pn_cmp24(union htt_rx_pn_t *new_pn, union htt_rx_pn_t *old_pn,
...
int ol_rx_pn_cmp48(union htt_rx_pn_t *new_pn, union htt_rx_pn_t *old_pn,
		   int is_unicast, int opmode, bool strict_chk)
{
	if (strict_chk)
...
int ol_rx_pn_wapi_cmp(union htt_rx_pn_t *new_pn, union htt_rx_pn_t *old_pn,
		      int is_unicast, int opmode, bool strict_chk)
{
	int pn_is_replay = 0;
...
qdf_nbuf_t ol_rx_pn_check_base(struct ol_txrx_vdev_t *vdev,
			       struct ol_txrx_peer_t *peer, unsigned int tid,
			       qdf_nbuf_t msdu_list, bool strict_chk)
{
     ...
     */
		...
		if (last_pn_valid) {
			pn_is_replay =
				pdev->rx_pn[peer->security[index].sec_type].cmp(
					&new_pn, last_pn,
					index == txrx_sec_ucast, vdev->opmode,
					strict_chk);
		} else {
...
void ol_rx_pn_check(struct ol_txrx_vdev_t *vdev, struct ol_txrx_peer_t *peer,
...
void ol_rx_pn_check_only(struct ol_txrx_vdev_t *vdev,
			 struct ol_txrx_peer_t *peer, unsigned int tid,
...
void ol_rx_pn_trace_add(struct ol_txrx_pdev_t *pdev,
			struct ol_txrx_peer_t *peer, uint16_t tid,
			void *rx_desc)
{
