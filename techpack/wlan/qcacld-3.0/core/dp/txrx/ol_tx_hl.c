...
u_int16_t ol_tx_desc_pool_size_hl(struct cdp_cfg *ctrl_pdev)
{
   ...
   */
	...
	desc_pool_size = ol_cfg_max_thruput_mbps(ctrl_pdev) *
			 1000 /* 1e6 bps/mbps / 1e3 ms per sec = 1000 */ /
			 (8 * OL_TX_AVG_FRM_BYTES) *
			 steady_state_tx_lifetime_ms * safety_factor;
...
ol_tx_hl_desc_alloc(struct ol_txrx_pdev_t *pdev, struct ol_txrx_vdev_t *vdev,
		    ...
		    qdf_nbuf_t msdu, struct ol_txrx_msdu_info_t *msdu_info)
{
     ...
     */
		pdev->pause_cb(vdev->vdev_id, WLAN_STOP_NON_PRIORITY_QUEUE,
		...
		pdev->pause_cb(vdev->vdev_id, WLAN_NETIF_PRIORITY_QUEUE_OFF,
...
static inline uint16_t ol_txrx_rsrc_threshold_lo(int desc_pool_size)
{
	int threshold_low;

	/* always maintain a 5% margin of unallocated descriptors */
	threshold_low = ((TX_RESOURCE_LOW_TH_IN_PER)*desc_pool_size) / 100;

	return threshold_low;
}

static inline uint16_t ol_txrx_rsrc_threshold_hi(int desc_pool_size)
{
	int threshold_high;
	/* when freeing up descriptors, keep going until
   * there's a 15% margin
   */
	threshold_high = ((TX_RESOURCE_HIGH_TH_IN_PER)*desc_pool_size) / 100;
...
ol_txrx_mgmt_tx_desc_alloc(struct ol_txrx_pdev_t *pdev,
			   struct ol_txrx_vdev_t *vdev, qdf_nbuf_t tx_mgmt_frm,
			   struct ol_txrx_msdu_info_t *tx_msdu_info)
{
	struct ol_tx_desc_t *tx_desc;
			    ...
			    uint16_t chanfreq)
{
   ...
   */
	...
	if (qdf_unlikely(htt_tx_desc_init(pdev->htt_pdev, tx_desc->htt_tx_desc,
					  ...
					  tx_mgmt_frm, &tx_msdu_info->htt,
					  &tx_msdu_info->tso_info, NULL, 0)))
		goto out;
 ...
 */
static inline qdf_nbuf_t ol_tx_hl_base(ol_txrx_vdev_handle vdev,
				       enum ol_tx_spec tx_spec,
				       qdf_nbuf_t msdu_list, int tx_comp_req,
				       bool call_sched)
{
	struct ol_txrx_pdev_t *pdev = vdev->pdev;
   ...
   */
     ...
     */
       ...
       */
		...
		if (QDF_NBUF_CB_TX_EXTRA_FRAG_FLAGS_NOTIFY_COMP(msdu) == 1) {
			tx_msdu_info.htt.action.tx_comp_req = 1;
			tx_desc->pkt_type = OL_TX_FRM_NO_FREE;
		} else {
     ...
     */
		...
		htt_tx_desc_init(pdev->htt_pdev, tx_desc->htt_tx_desc,
				 tx_desc->htt_tx_desc_paddr,
				 ol_tx_desc_id(pdev, tx_desc), msdu,
				 &tx_msdu_info.htt, &tx_msdu_info.tso_info,
				 &tx_ctrl, vdev->opmode == wlan_op_mode_ocb);
...
void ol_tx_pdev_reset_driver_del_ack(struct cdp_soc_t *soc_hdl, uint8_t pdev_id)
{
	...
	TAILQ_FOREACH(vdev, &pdev->vdev_list, vdev_list_elem)
	{
		...
		dp_debug("vdev_id %d driver_del_ack_enabled %d", vdev->vdev_id,
					  ...
					  uint32_t low_th)
{
	...
	if (old_driver_del_ack_enabled != vdev->driver_del_ack_enabled) {
		dp_debug(
			"vdev_id %d driver_del_ack_enabled %d rx_packets %ld time_in_ms "
			"%d high_th %d low_th %d",
			vdev->vdev_id, vdev->driver_del_ack_enabled, rx_packets,
...
{
	return ((info->dst_port + info->dst_ip + info->src_port +
		 info->src_ip) &
 ...
 */
static bool ol_tx_is_tcp_ack(qdf_nbuf_t msdu)
{
	uint16_t ether_type;
	uint8_t protocol;
	...
	if (skb_len <
	    (QDF_NBUF_TRAC_IPV4_OFFSET + QDF_NBUF_TRAC_IPV4_HEADER_SIZE +
	     QDF_NBUF_TRAC_TCP_FLAGS_OFFSET))
		goto exit;

	ether_type = (uint16_t)(*(uint16_t *)(skb_data +
					      QDF_NBUF_TRAC_ETH_TYPE_OFFSET));
	protocol = (uint16_t)(*(
		uint16_t *)(skb_data + QDF_NBUF_TRAC_IPV4_PROTO_TYPE_OFFSET));

	if ((QDF_SWAP_U16(QDF_NBUF_TRAC_IPV4_ETH_TYPE) == ether_type) &&
	    (protocol == QDF_NBUF_TRAC_TCP_TYPE)) {
		ip_header_len =
			((uint8_t)(*(uint8_t *)(skb_data +
...
void ol_tx_get_packet_info(qdf_nbuf_t msdu, struct packet_info *info)
{
	...
	if ((QDF_SWAP_U16(QDF_NBUF_TRAC_IPV4_ETH_TYPE) == ether_type) &&
		...
		info->src_ip = QDF_SWAP_U32((uint32_t)(*(
			uint32_t *)(skb_data +
				    QDF_NBUF_TRAC_IPV4_SRC_ADDR_OFFSET)));
		info->dst_ip = QDF_SWAP_U32((uint32_t)(*(
			uint32_t *)(skb_data +
				    QDF_NBUF_TRAC_IPV4_DEST_ADDR_OFFSET)));
		info->src_port = QDF_SWAP_U16((uint16_t)(*(
			uint16_t *)(skb_data + tcp_header_off +
				    QDF_NBUF_TRAC_TCP_SPORT_OFFSET)));
		info->dst_port = QDF_SWAP_U16((uint16_t)(*(
			uint16_t *)(skb_data + tcp_header_off +
		...
		if ((flag == QDF_NBUF_TRAC_TCP_ACK_MASK) && (seg_len == 0)) {
			...
			info->ack_number = (uint32_t)(*(
				uint32_t *)(skb_data + tcp_header_off +
				       ...
				       struct packet_info *info)
{
	...
	qdf_spin_lock_bh(
		&vdev->tcp_ack_hash.node[info->stream_id].hash_node_lock);
	...
	if (node_to_be_remove) {
		...
		msdu_list = ol_tx_hl_base(vdev, OL_TX_SPEC_STD,
					  node_to_be_remove->head, tx_comp_req,
					  true);
		if (msdu_list)
			qdf_nbuf_tx_free(msdu_list, 1 /*error*/);
...
static struct tcp_stream_node *ol_tx_hl_rep_tcp_ack(struct ol_txrx_vdev_t *vdev,
						    qdf_nbuf_t msdu,
						    struct packet_info *info,
						    bool *is_found,
						    bool *start_timer)
{
	...
	if ((head->dst_ip == info->dst_ip) && (head->src_ip == info->src_ip) &&
	    (head->src_port == info->src_port) &&
	    (head->dst_port == info->dst_port)) {
		*is_found = true;
				       ...
				       struct packet_info *info)
{
	...
	no_of_entries = vdev->tcp_ack_hash.node[info->stream_id].no_of_entries;
	if (no_of_entries > 0) {
		node_to_be_remove = ol_tx_hl_rep_tcp_ack(
	...
	if (start_timer &&
		...
		qdf_hrtimer_start(
			&vdev->tcp_ack_hash.timer,
			qdf_ns_to_ktime((ol_cfg_get_del_ack_timer_value(
						 vdev->pdev->ctrl_pdev) *
					 1000000)),
...
{
	struct ol_txrx_vdev_t *vdev = qdf_container_of(
		timer, struct ol_txrx_vdev_t, tcp_ack_hash.timer);
...
void ol_txrx_vdev_init_tcp_del_ack(struct ol_txrx_vdev_t *vdev)
{
	...
	dp_debug("vdev-id=%u, driver_del_ack_enabled=%d", vdev->vdev_id,
		 vdev->driver_del_ack_enabled);
	...
	qdf_hrtimer_init(&vdev->tcp_ack_hash.timer,
			 ol_tx_hl_vdev_tcp_del_ack_timer, __QDF_CLOCK_MONOTONIC,
			 __QDF_HRTIMER_MODE_REL, QDF_CONTEXT_HARDWARE);
	qdf_create_bh(&vdev->tcp_ack_hash.tcp_del_ack_tq, tcp_del_ack_tasklet,
		      vdev);
...
qdf_nbuf_t ol_tx_hl(ol_txrx_vdev_handle vdev, qdf_nbuf_t msdu_list)
{
	struct ol_txrx_pdev_t *pdev = vdev->pdev;
	int tx_comp_req = pdev->cfg.default_tx_comp_req ||
			  pdev->cfg.request_tx_comp;
	...
	temp = ol_tx_hl_base(vdev, OL_TX_SPEC_STD, msdu_list, tx_comp_req,
			     true);
	return temp;
}
#else
...
void ol_tx_pdev_reset_bundle_require(struct cdp_soc_t *soc_hdl, uint8_t pdev_id)
{
	...
	TAILQ_FOREACH(vdev, &pdev->vdev_list, vdev_list_elem)
	{
		...
		ol_txrx_info("vdev_id %d bundle_require %d", vdev->vdev_id,
			     vdev->bundling_required);
	}
}
				   ...
				   uint32_t low_th)
{
	...
	if (old_bundle_required != vdev->bundling_required)
		ol_txrx_info(
			"vdev_id %d bundle_require %d tx_bytes %ld time_in_ms %d "
			"high_th %d low_th %d",
 ...
 * Return: none
 */
static void ol_tx_hl_vdev_queue_append(struct ol_txrx_vdev_t *vdev,
				       qdf_nbuf_t msdu_list)
{
	qdf_spin_lock_bh(&vdev->bundle_queue.mutex);
 ...
 */
static qdf_nbuf_t ol_tx_hl_vdev_queue_send_all(struct ol_txrx_vdev_t *vdev,
					       bool call_sched,
					       bool in_timer_context)
{
	...
	if (likely((qdf_atomic_read(&vdev->tx_desc_count) +
		    vdev->bundle_queue.txq.depth) < vdev->queue_stop_th)) {
		qdf_timer_stop(&vdev->bundle_queue.timer);
		vdev->pdev->total_bundle_queue_length -=
			vdev->bundle_queue.txq.depth;
		msdu_list = ol_tx_hl_base(vdev, OL_TX_SPEC_STD,
		...
		if (pkt_to_sent) {
			...
			while (pkt_to_sent) {
				skb_list_tail = vdev->bundle_queue.txq.head;
				vdev->bundle_queue.txq.head = qdf_nbuf_next(
					vdev->bundle_queue.txq.head);
				vdev->pdev->total_bundle_queue_length--;
				vdev->bundle_queue.txq.depth--;
				pkt_to_sent--;
		...
		if (in_timer_context && vdev->bundle_queue.txq.head) {
			qdf_timer_start(&vdev->bundle_queue.timer,
 ...
 * Return: NULL for success
 */
qdf_nbuf_t ol_tx_hl_pdev_queue_send_all(struct ol_txrx_pdev_t *pdev)
{
	struct ol_txrx_vdev_t *vdev;
	qdf_nbuf_t msdu_list;
 ...
 * Return: none
 */
void ol_tx_hl_vdev_bundle_timer(void *ctx)
{
	qdf_nbuf_t msdu_list;
...
qdf_nbuf_t ol_tx_hl(struct ol_txrx_vdev_t *vdev, qdf_nbuf_t msdu_list)
{
	struct ol_txrx_pdev_t *pdev = vdev->pdev;
	int tx_comp_req = pdev->cfg.default_tx_comp_req ||
			  pdev->cfg.request_tx_comp;
			       ...
			       uint8_t *bss_addr)
{
	...
	if (bss_addr && vdev->last_real_peer &&
	    !qdf_mem_cmp((u8 *)bss_addr, vdev->last_real_peer->mac_addr.raw,
			 QDF_MAC_ADDR_SIZE))
...
void ol_txrx_add_last_real_peer(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
				uint8_t vdev_id)
{
	struct ol_txrx_soc_t *soc = cdp_soc_t_to_ol_txrx_soc_t(soc_hdl);
	ol_txrx_pdev_handle pdev = ol_txrx_get_pdev_from_pdev_id(soc, pdev_id);
	...
	if (!vdev->last_real_peer && peer &&
		...
		qdf_mem_zero(vdev->hl_tdls_ap_mac_addr.raw, QDF_MAC_ADDR_SIZE);
	}
...
void ol_txrx_set_tdls_offchan_enabled(struct cdp_soc_t *soc_hdl,
 ...
 * Return: None
 */
void ol_txrx_hl_tdls_flag_reset(struct cdp_soc_t *soc_hdl, uint8_t vdev_id,
				bool flag)
{
	struct ol_txrx_vdev_t *vdev =
		(struct ol_txrx_vdev_t *)ol_txrx_get_vdev_from_vdev_id(vdev_id);
 ...
 */
void ol_txrx_update_group_credit(struct ol_tx_queue_group_t *group,
 ...
 * Return: None
 */
void ol_txrx_update_tx_queue_groups(ol_txrx_pdev_handle pdev, u_int8_t group_id,
				    int32_t credit, u_int8_t absolute,
				    u_int32_t vdev_id_mask, u_int32_t ac_mask)
{
   ...
   */
	...
	credit +=
		ol_txrx_distribute_group_credits(pdev, group_id, vdev_id_mask);
	/*
   * membership (vdev id mask and ac mask) is not matching
   * TODO: ignoring ac mask for now
   */
	qdf_assert(ac_mask == 0xffff);
	group_vdev_id_mask = OL_TXQ_GROUP_VDEV_ID_MASK_GET(group->membership);

	TAILQ_FOREACH(vdev, &pdev->vdev_list, vdev_list_elem)
	{
		group_vdev_bit_mask = OL_TXQ_GROUP_VDEV_ID_BIT_MASK_GET(
			group_vdev_id_mask, vdev->vdev_id);
		vdev_bit_mask = OL_TXQ_GROUP_VDEV_ID_BIT_MASK_GET(
       ...
       */
			if (!vdev_bit_mask) {
				...
				ol_txrx_info(
					"Group membership removed for vdev_id %d from group_id %d",
					vdev->vdev_id, group_id);
				ol_tx_set_vdev_group_ptr(pdev, vdev->vdev_id,
							 NULL);
			} else {
				/* Set Group Pointer (vdev and peer) */
				ol_txrx_info(
					"Group membership updated for vdev_id %d to group_id %d",
					vdev->vdev_id, group_id);
				ol_tx_set_vdev_group_ptr(pdev, vdev->vdev_id,
							 group);
			}
       ...
       */
		...
		if (vdevid_mask_othgrp)
			...
			ol_txrx_update_group_credit(
				grp_nxt, qdf_atomic_read(&grp->credit), 0);
...
#ifdef QCA_HL_NETDEV_FLOW_CONTROL
int ol_txrx_register_hl_flow_control(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
...
void ol_tx_dump_flow_pool_info_compact(struct ol_txrx_pdev_t *pdev)
{
	...
	bytes_written = snprintf(&comb_log_str[bytes_written], free_size, "%d ",
				 qdf_atomic_read(&pdev->target_tx_credit));
	for (i = 0; i < OL_TX_MAX_TXQ_GROUPS; i++) {
		bytes_written +=
			snprintf(&comb_log_str[bytes_written], free_size,
...
void ol_tx_dump_flow_pool_info(struct cdp_soc_t *soc_hdl)
{
	...
	TAILQ_FOREACH(vdev, &pdev->vdev_list, vdev_list_elem)
	{
		...
		txrx_nofl_info(
			"limit %d available %d stop_threshold %d restart_threshold %d",
			vdev->tx_desc_limit,
			qdf_atomic_read(&vdev->tx_desc_count),
