...
static char
	g_log2ceil[] = {
		...
		32, 32, 32, 32, 32, /* 17-32 ->
                                                                       32 */
		64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64,
		64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64,
		64, 64, 64, 64, 64, 64, 64, 64, 64, 64, /* 33-64 -> 64
                                                                 */
	};
...
ol_rx_reorder_seq_num_check(struct ol_txrx_pdev_t *pdev,
 ...
 */
enum htt_rx_status ol_rx_seq_num_check(struct ol_txrx_pdev_t *pdev,
				       struct ol_txrx_peer_t *peer, uint8_t tid,
				       void *rx_mpdu_desc)
{
...
void ol_rx_reorder_store(struct ol_txrx_pdev_t *pdev,
			 struct ol_txrx_peer_t *peer, unsigned int tid,
...
void ol_rx_reorder_release(struct ol_txrx_vdev_t *vdev,
			   struct ol_txrx_peer_t *peer, unsigned int tid,
...
void ol_rx_reorder_flush(struct ol_txrx_vdev_t *vdev,
			 struct ol_txrx_peer_t *peer, unsigned int tid,
			 unsigned int idx_start, unsigned int idx_end,
...
void ol_rx_reorder_first_hole(struct ol_txrx_peer_t *peer, unsigned int tid,
 ...
 * Return: void
 */
static void ol_rx_reorder_detect_hole(struct ol_txrx_peer_t *peer, uint32_t tid,
				      uint32_t idx_start)
{
...
void ol_rx_reorder_peer_cleanup(struct ol_txrx_vdev_t *vdev,
...
void ol_rx_addba_handler(ol_txrx_pdev_handle pdev, uint16_t peer_id,
			 uint8_t tid, uint8_t win_sz, uint16_t start_seq_num,
			 uint8_t failed)
{
...
void ol_rx_delba_handler(ol_txrx_pdev_handle pdev, uint16_t peer_id,
			 uint8_t tid)
{
	struct ol_txrx_peer_t *peer;
	struct ol_rx_reorder_t *rx_reorder;
   ...
   */
	if (rx_reorder->array != &rx_reorder->base) {
		ol_txrx_dbg("delete reorder array, tid:%d", tid);
		qdf_mem_free(rx_reorder->array);
	}
...
void ol_rx_flush_handler(ol_txrx_pdev_handle pdev, uint16_t peer_id,
			 uint8_t tid, uint16_t idx_start, uint16_t idx_end,
			 enum htt_rx_flush_action action)
{
	...
	rx_reorder_array_elem = &peer->tids_rx_reorder[tid].array[idx];
	if (rx_reorder_array_elem->head) {
		rx_desc = htt_rx_msdu_desc_retrieve(
...
void ol_rx_pn_ind_handler(ol_txrx_pdev_handle pdev, uint16_t peer_id,
			  uint8_t tid, uint16_t seq_num_start,
			  uint16_t seq_num_end, uint8_t pn_ie_cnt,
			  uint8_t *pn_ie)
{
	struct ol_txrx_vdev_t *vdev = NULL;
	void *rx_desc;
			...
			if (pn_ie_cnt && seq_num == (int)(pn_ie[i])) {
				...
				if (TXRX_PN_CHECK_FAILURE_PRINT_PERIOD_MS <
					...
					ol_txrx_warn(
						...
						QDF_MAC_ADDR_REF(
							peer->mac_addr.raw),
						pn.pn128[1], pn.pn128[0],
						pn.pn128[0] & 0xffffffffffffULL,
						htt_rx_mpdu_desc_seq_num(
							htt_pdev, rx_desc,
							false));
				} else {
					ol_txrx_dbg(
...
void ol_rx_reorder_trace_add(ol_txrx_pdev_handle pdev, uint8_t tid,
			     uint16_t reorder_idx, uint16_t seq_num,
			     int num_mpdus)
{
...
void ol_rx_reorder_trace_display(ol_txrx_pdev_handle pdev, int just_once,
				 int limit)
{
	...
	do {
		...
		if (seq_num < (1 << 14)) {
			QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_INFO,
				  "  %6lld  %4d  %3d  %4d  %4d (%d)", cnt, i,
				  pdev->rx_reorder_trace.data[i].tid,
				  reorder_idx, seq_num, seq_num & 63);
		} else {
			int err = TXRX_SEQ_NUM_ERR(seq_num);

			QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_INFO,
				  "  %6lld  %4d err %d (%d MPDUs)", cnt, i, err,
