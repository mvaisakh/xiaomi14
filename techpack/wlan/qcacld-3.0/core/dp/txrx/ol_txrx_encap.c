...
static inline A_STATUS ol_tx_copy_native_wifi_header(qdf_nbuf_t msdu,
						     uint8_t *hdsize,
						     uint8_t *localbuf)
{
			     ...
			     struct ol_txrx_msdu_info_t *tx_msdu_info)
{
   ...
   */
	if (tx_msdu_info->htt.info.is_unicast && peer->qos_capable) {
		...
		qos_cntl->i_qos[0] = tx_msdu_info->htt.info.ext_tid &
				     IEEE80211_QOS_TID;
		...
		/* count for qos field */
		new_hdsize = hdsize + sizeof(struct ieee80211_qosframe) -
			     sizeof(struct ieee80211_frame);

		/*add ht control field if needed */

		/* copy new hd to bd */
		qdf_mem_copy((void *)htt_tx_desc_mpdu_header(
				     tx_desc->htt_tx_desc, new_hdsize),
			     localbuf, new_hdsize);
	...
	if (vdev->pdev->sw_pf_proc_enable &&
		...
		if (tx_desc->orig_l2_hdr_bytes) {
			wh = (struct ieee80211_frame *)htt_tx_desc_mpdu_header(
				tx_desc->htt_tx_desc,
				tx_msdu_info->htt.info.l3_hdr_offset);
		} else {
			if (A_OK != ol_tx_copy_native_wifi_header(msdu, &hdsize,
								  localbuf))
				return A_ERROR;
			wh = (struct ieee80211_frame *)htt_tx_desc_mpdu_header(
				tx_desc->htt_tx_desc, hdsize);
...
static inline A_STATUS
ol_tx_encap_from_8023(struct ol_txrx_vdev_t *vdev, struct ol_tx_desc_t *tx_desc,
   ...
   */
	...
	*(uint16_t *)wh->i_dur = 0;
	...
	case wlan_op_mode_ibss:
		...
		qdf_mem_copy(wh->i_addr1, eth_hdr->dest_addr,
			     QDF_MAC_ADDR_SIZE);
		qdf_mem_copy(wh->i_addr2, eth_hdr->src_addr, QDF_MAC_ADDR_SIZE);
	...
	if (vdev->pdev->sw_tx_llc_proc_enable) {
		...
		if (ether_type >= ETH_P_802_3_MIN) {
			qdf_mem_copy(
				llc_hdr, ethernet_II_llc_snap_header_prefix,
	...
	qdf_mem_copy((void *)htt_tx_desc_mpdu_header(tx_desc->htt_tx_desc,
						     new_l2_hdsize),
		     localbuf, new_hdsize);
	qdf_nbuf_pull_head(msdu, hdsize);
...
A_STATUS
ol_tx_encap(struct ol_txrx_vdev_t *vdev, struct ol_tx_desc_t *tx_desc,
...
static inline void ol_rx_decap_to_native_wifi(struct ol_txrx_vdev_t *vdev,
					      qdf_nbuf_t msdu,
...
static inline void ol_rx_decap_to_8023(struct ol_txrx_vdev_t *vdev,
				       qdf_nbuf_t msdu,
				       struct ol_rx_decap_info_t *info,
				       struct ethernet_hdr_t *ethr_hdr)
{
...
static inline A_STATUS ol_rx_decap_subfrm_amsdu(struct ol_txrx_vdev_t *vdev,
						qdf_nbuf_t msdu,
						struct ol_rx_decap_info_t *info)
{
	struct ol_txrx_pdev_t *pdev = vdev->pdev;
...
static inline A_STATUS ol_rx_decap_msdu(struct ol_txrx_vdev_t *vdev,
					qdf_nbuf_t msdu,
					struct ol_rx_decap_info_t *info)
{
	struct ol_txrx_pdev_t *pdev = vdev->pdev;
...
A_STATUS
ol_rx_decap(struct ol_txrx_vdev_t *vdev, struct ol_txrx_peer_t *peer,
