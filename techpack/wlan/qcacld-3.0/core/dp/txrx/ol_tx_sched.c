...
static void ol_tx_sched_log(struct ol_txrx_pdev_t *pdev);
...
typedef TAILQ_HEAD(ol_tx_frms_queue_list_s,
		   ol_tx_frms_queue_t) ol_tx_frms_queue_list;
 ...
 */
static int ol_tx_sched_select_batch_rr(struct ol_txrx_pdev_t *pdev,
				       ...
				       u_int32_t credit)
{
	...
	tx_limit =
		ol_tx_bad_peer_dequeue_check(next_tq, frames, &tx_limit_flag);
	frames = ol_tx_dequeue(pdev, next_tq, &sctx->head, tx_limit, &credit,
			       &bytes);
	...
	if (next_tq->frms > 0) {
		...
		TAILQ_INSERT_TAIL(&scheduler->tx_active_tids_list, txq_queue,
				  list_elem);
...
static inline void ol_tx_sched_txq_enqueue_rr(struct ol_txrx_pdev_t *pdev,
...
static inline void ol_tx_sched_txq_deactivate_rr(struct ol_txrx_pdev_t *pdev,
						 struct ol_tx_frms_queue_t *txq,
						 int tid)
{
	struct ol_tx_sched_rr_t *scheduler = pdev->tx_sched.scheduler;
...
int ol_tx_sched_discard_select_category_rr(struct ol_txrx_pdev_t *pdev)
{
   ...
   */
	for (i = 0; i < (OL_TX_NUM_TIDS + OL_TX_VDEV_NUM_QUEUES); i++) {
		...
		score = scheduler->tx_active_queues_in_tid_array[i].frms *
...
void ol_tx_sched_txq_discard_rr(struct ol_txrx_pdev_t *pdev,
				struct ol_tx_frms_queue_t *txq, int tid,
				int frames, int bytes)
{
	struct ol_tx_sched_rr_t *scheduler = pdev->tx_sched.scheduler;
	struct ol_tx_active_queues_in_tid_t *txq_queue;
...
void ol_tx_sched_category_info_rr(struct ol_txrx_pdev_t *pdev, int cat,
				  int *active, int *frms, int *bytes)
{
	struct ol_tx_sched_rr_t *scheduler = pdev->tx_sched.scheduler;
	struct ol_tx_active_queues_in_tid_t *txq_queue;
...
void *ol_tx_sched_init_rr(struct ol_txrx_pdev_t *pdev)
{
	struct ol_tx_sched_rr_t *scheduler;
	int i;
...
void ol_txrx_set_wmm_param(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
...
#define OL_TX_SCHED_WRR_ADV_CAT_CFG_SPEC(cat, wrr_skip_weight,                \
					 credit_threshold, send_limit,        \
					 credit_reserve, discard_weights)     \
	enum {                                                                \
		OL_TX_SCHED_WRR_ADV_##cat##_WRR_SKIP_WEIGHT =                 \
			(wrr_skip_weight)                                     \
	};                                                                    \
	enum {                                                                \
		OL_TX_SCHED_WRR_ADV_##cat##_CREDIT_THRESHOLD =                \
			(credit_threshold)                                    \
	};                                                                    \
	enum { OL_TX_SCHED_WRR_ADV_##cat##_SEND_LIMIT = (send_limit) };       \
	enum {                                                                \
		OL_TX_SCHED_WRR_ADV_##cat##_CREDIT_RESERVE = (credit_reserve) \
	};                                                                    \
	enum { OL_TX_SCHED_WRR_ADV_##cat##_DISCARD_WEIGHT = (discard_weights) };
/* Rome:
...
#ifdef DEBUG_HL_LOGGING
static void
ol_tx_sched_wrr_adv_cat_stat_dump(struct ol_tx_sched_wrr_adv_t *scheduler)
{
	int i;
...
static void
ol_tx_sched_wrr_adv_cat_cur_state_dump(struct ol_tx_sched_wrr_adv_t *scheduler)
{
	int i;
...
static void
ol_tx_sched_wrr_adv_cat_stat_clear(struct ol_tx_sched_wrr_adv_t *scheduler)
{
	int i;
...
static void ol_tx_sched_select_init_wrr_adv(struct ol_txrx_pdev_t *pdev)
{
...
static void ol_tx_sched_wrr_adv_rotate_order_list_tail(
...
static void ol_tx_sched_wrr_adv_credit_sanity_check(struct ol_txrx_pdev_t *pdev,
						    u_int32_t credit)
{
	struct ol_tx_sched_wrr_adv_t *scheduler = pdev->tx_sched.scheduler;
	int i;
	...
	for (i = 0; i < OL_TX_SCHED_WRR_ADV_NUM_CATEGORIES; i++) {
		if (scheduler->categories[i].specs.credit_threshold > credit) {
			QDF_TRACE(
				...
				"*** Config error: credit (%d) not enough to support category "
				"%d threshold (%d)\n",
				credit, i,
				scheduler->categories[i].specs.credit_threshold);
			okay = 0;
 ...
 * so there is no need to worry about mutex within this function.
 */
static int ol_tx_sched_select_batch_wrr_adv(struct ol_txrx_pdev_t *pdev,
					    struct ol_tx_sched_ctx *sctx,
					    u_int32_t credit)
{
	static int first = 1;
   ...
   */
	...
	while (txq) {
		...
		if (credit > category->specs.credit_reserve) {
			...
			tx_limit = ol_tx_bad_peer_dequeue_check(
				txq, category->specs.send_limit,
				&tx_limit_flag);
			frames = ol_tx_dequeue(pdev, txq, &sctx->head, tx_limit,
					       &credit, &bytes);
			ol_tx_bad_peer_update_tx_limit(pdev, txq, frames,
						       tx_limit_flag);
			...
			used_credits = credit;
			credit = ol_tx_txq_update_borrowed_group_credits(
				pdev, txq, credit);
...
static inline void
ol_tx_sched_txq_enqueue_wrr_adv(struct ol_txrx_pdev_t *pdev,
				struct ol_tx_frms_queue_t *txq, int tid,
				int frms, int bytes)
{
	struct ol_tx_sched_wrr_adv_t *scheduler = pdev->tx_sched.scheduler;
...
static inline void
ol_tx_sched_txq_deactivate_wrr_adv(struct ol_txrx_pdev_t *pdev,
				   struct ol_tx_frms_queue_t *txq, int tid)
{
	struct ol_tx_sched_wrr_adv_t *scheduler = pdev->tx_sched.scheduler;
	struct ol_tx_sched_wrr_adv_category_info_t *category;
...
ol_tx_sched_discard_select_category_wrr_adv(struct ol_txrx_pdev_t *pdev)
{
   ...
   */
	for (i = 0; i < OL_TX_SCHED_WRR_ADV_NUM_CATEGORIES; i++) {
		...
		score = scheduler->categories[i].state.frms *
...
static void ol_tx_sched_txq_discard_wrr_adv(struct ol_txrx_pdev_t *pdev,
					    struct ol_tx_frms_queue_t *txq,
...
static void ol_tx_sched_category_info_wrr_adv(struct ol_txrx_pdev_t *pdev,
					      int cat, int *active, int *frms,
					      int *bytes)
{
	struct ol_tx_sched_wrr_adv_t *scheduler = pdev->tx_sched.scheduler;
	struct ol_tx_sched_wrr_adv_category_info_t *category;
 ...
 */
static void
ol_tx_sched_wrr_param_update(struct ol_txrx_pdev_t *pdev,
			     struct ol_tx_sched_wrr_adv_t *scheduler)
{
	int i;
	static const char *const tx_sched_wrr_name[4] = { "BE", "BK", "VI",
							  "VO" };
	...
	for (i = OL_TX_SCHED_WRR_ADV_CAT_BE; i <= OL_TX_SCHED_WRR_ADV_CAT_VO;
	     i++) {
...
static void *ol_tx_sched_init_wrr_adv(struct ol_txrx_pdev_t *pdev)
{
	struct ol_tx_sched_wrr_adv_t *scheduler;
	int i;

	scheduler = qdf_mem_malloc(sizeof(struct ol_tx_sched_wrr_adv_t));
	if (!scheduler)
		return scheduler;
			   ...
			   struct ol_tx_wmm_param_t wmm_param)
{
	...
	weight[OL_TX_SCHED_WRR_ADV_CAT_VO] = wmm_param.ac[QCA_WLAN_AC_VO].aifs +
					     wmm_param.ac[QCA_WLAN_AC_VO].cwmin;
	weight[OL_TX_SCHED_WRR_ADV_CAT_VI] = wmm_param.ac[QCA_WLAN_AC_VI].aifs +
					     wmm_param.ac[QCA_WLAN_AC_VI].cwmin;
	weight[OL_TX_SCHED_WRR_ADV_CAT_BK] = wmm_param.ac[QCA_WLAN_AC_BK].aifs +
					     wmm_param.ac[QCA_WLAN_AC_BK].cwmin;
	weight[OL_TX_SCHED_WRR_ADV_CAT_BE] = wmm_param.ac[QCA_WLAN_AC_BE].aifs +
...
ol_tx_sched_discard_select_txq(struct ol_txrx_pdev_t *pdev,
...
u_int16_t ol_tx_sched_discard_select(struct ol_txrx_pdev_t *pdev,
				     u_int16_t frms, ol_tx_desc_list *tx_descs,
				     bool force)
{
	int cat;
	struct ol_tx_frms_queue_t *txq;
 ...
 */
void ol_tx_sched_notify(struct ol_txrx_pdev_t *pdev,
			struct ol_tx_sched_notify_ctx_t *ctx)
{
	...
	case OL_TX_UNPAUSE_QUEUE:
		tid = ctx->info.ext_tid;
		if (txq->frms != 0)
			ol_tx_sched_txq_enqueue(pdev, txq, tid, txq->frms,
						txq->bytes);
	...
	case OL_TX_DISCARD_FRAMES:
		...
		tid = ctx->info.ext_tid;
		ol_tx_sched_txq_discard(pdev, txq, tid, ctx->frames,
					ctx->bytes);
		break;
	default:
...
static void ol_tx_sched_dispatch(struct ol_txrx_pdev_t *pdev,
				 struct ol_tx_sched_ctx *sctx)
{
	...
	if (cur_tx_pad_credit < MIN_TX_PAD_CREDIT_THRESH) {
		...
		if (replenish_credit < 0) {
			QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_FATAL,
				  ...
				  cur_tx_pad_credit, avail_targ_tx_credit);
			qdf_assert(0);
		}
...
void ol_tx_sched(struct ol_txrx_pdev_t *pdev)
{
	struct ol_tx_sched_ctx sctx;
	u_int32_t credit;
	...
	while (qdf_atomic_read(&pdev->target_tx_credit) > 0) {
		...
		if (num_credits > 0) {
			...
			DPTRACE(qdf_dp_trace_credit_record(
				QDF_TX_SCHED, QDF_CREDIT_DEC, num_credits,
				qdf_atomic_read(&pdev->target_tx_credit) -
					num_credits,
...
void *ol_tx_sched_attach(struct ol_txrx_pdev_t *pdev)
{
	pdev->tx_sched.tx_sched_status = ol_tx_scheduler_idle;
	return ol_tx_sched_init(pdev);
}

void ol_tx_sched_detach(struct ol_txrx_pdev_t *pdev)
{
	if (pdev->tx_sched.scheduler) {
...
static void ol_tx_sched_log(struct ol_txrx_pdev_t *pdev)
{
   ...
   */
	...
	ol_tx_queue_log_sched(pdev, credit, &num_cats_active, &active_bitmap,
			      &buf);
	...
	*active_bitmap = 0;
	for (i = 0, j = 0;
	     i < OL_TX_SCHED_NUM_CATEGORIES && j < num_cats_active; i++) {
		u_int8_t *p;
