 ...
 */
static inline bool htt_rx_msdu_first_msdu_flag_hl(htt_pdev_handle pdev,
						  void *msdu_desc)
{
	return ((u_int8_t *)msdu_desc -
		sizeof(struct hl_htt_rx_ind_base))[HTT_ENDIAN_BYTE_IDX_SWAP(
...
u_int16_t htt_rx_msdu_rx_desc_size_hl(htt_pdev_handle pdev, void *msdu_desc)
{
...
static void htt_set_checksum_result_hl(qdf_nbuf_t msdu,
				       struct htt_host_rx_desc_base *rx_desc)
{
	...
	qdf_nbuf_rx_cksum_t cksum = { QDF_NBUF_RX_CKSUM_NONE,
 ...
 */
static inline void *htt_rx_mpdu_desc_list_next_hl(htt_pdev_handle pdev,
 ...
 */
static inline void *htt_rx_msdu_desc_retrieve_hl(htt_pdev_handle pdev,
						 qdf_nbuf_t msdu)
{
	return qdf_nbuf_data(msdu);
}

static bool htt_rx_mpdu_is_encrypted_hl(htt_pdev_handle pdev, void *mpdu_desc)
{
	if (htt_rx_msdu_first_msdu_flag_hl(pdev, mpdu_desc) == true) {
		...
		qdf_print("Error: get encrypted from a not-first msdu.\n");
		qdf_assert(0);
		return false;
	}
}

static inline bool htt_rx_msdu_chan_info_present_hl(htt_pdev_handle pdev,
						    void *mpdu_desc)
{
	if (htt_rx_msdu_first_msdu_flag_hl(pdev, mpdu_desc) == true &&
...
static bool htt_rx_msdu_center_freq_hl(htt_pdev_handle pdev,
				       ...
				       uint8_t *phy_mode)
{
	...
	index = htt_rx_msdu_is_wlan_mcast(pdev, mpdu_desc) ? txrx_sec_mcast :
							     txrx_sec_ucast;

	pn_len = (peer ? pdev->txrx_pdev->rx_pn[peer->security[index].sec_type]
				  .len :
			 0);
	...
	if (htt_rx_msdu_chan_info_present_hl(pdev, mpdu_desc)) {
		if (primary_chan_center_freq_mhz)
			*primary_chan_center_freq_mhz = HTT_WORD_GET(
				*chan_info,
				HTT_CHAN_INFO_PRIMARY_CHAN_CENTER_FREQ);
		if (contig_chan1_center_freq_mhz)
			*contig_chan1_center_freq_mhz = HTT_WORD_GET(
				*chan_info,
		...
		if (phy_mode)
			*phy_mode = HTT_WORD_GET(*chan_info,
...
static bool htt_rx_msdu_desc_key_id_hl(htt_pdev_handle htt_pdev,
 ...
 */
static inline bool htt_rx_mpdu_desc_retry_hl(htt_pdev_handle pdev,
					     void *mpdu_desc)
{
	return false;
}

static int htt_rx_amsdu_pop_hl(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg,
			       qdf_nbuf_t *head_msdu, qdf_nbuf_t *tail_msdu,
			       uint32_t *msdu_count)
{
	pdev->rx_desc_size_hl =
		(qdf_nbuf_data(rx_ind_msg))[HTT_ENDIAN_BYTE_IDX_SWAP(
			HTT_RX_IND_HL_RX_DESC_LEN_OFFSET)];

	/* point to the rx desc */
	qdf_nbuf_pull_head(rx_ind_msg, sizeof(struct hl_htt_rx_ind_base));
	*head_msdu = *tail_msdu = rx_ind_msg;

	htt_set_checksum_result_hl(
		rx_ind_msg,
		(struct htt_host_rx_desc_base *)(qdf_nbuf_data(rx_ind_msg)));

	qdf_nbuf_set_next(*tail_msdu, NULL);
	return 0;
}

static int htt_rx_frag_pop_hl(htt_pdev_handle pdev, qdf_nbuf_t frag_msg,
			      qdf_nbuf_t *head_msdu, qdf_nbuf_t *tail_msdu,
			      uint32_t *msdu_count)
{
	qdf_nbuf_pull_head(frag_msg, HTT_RX_FRAG_IND_BYTES);
	pdev->rx_desc_size_hl =
		(qdf_nbuf_data(frag_msg))[HTT_ENDIAN_BYTE_IDX_SWAP(
			HTT_RX_IND_HL_RX_DESC_LEN_OFFSET)];

	/* point to the rx desc */
	qdf_nbuf_pull_head(frag_msg, sizeof(struct hl_htt_rx_ind_base));
...
static inline int htt_rx_offload_msdu_cnt_hl(htt_pdev_handle pdev)
{
	return 1;
}

static inline int htt_rx_offload_msdu_pop_hl(htt_pdev_handle pdev,
					     qdf_nbuf_t offload_deliver_msg,
					     int *vdev_id, int *peer_id,
					     int *tid, u_int8_t *fw_desc,
					     qdf_nbuf_t *head_buf,
					     qdf_nbuf_t *tail_buf)
{
	qdf_nbuf_t buf;
...
static uint16_t htt_rx_mpdu_desc_seq_num_hl(htt_pdev_handle pdev,
					    void *mpdu_desc,
					    bool update_seq_num)
{
	if (pdev->rx_desc_size_hl) {
		if (update_seq_num)
			return pdev->cur_seq_num_hl = (u_int16_t)(HTT_WORD_GET(
				       *(u_int32_t *)mpdu_desc,
				       HTT_HL_RX_DESC_MPDU_SEQ_NUM));
		else
			return (u_int16_t)(HTT_WORD_GET(
				*(u_int32_t *)mpdu_desc,
				HTT_HL_RX_DESC_MPDU_SEQ_NUM));
	} else {
...
static void htt_rx_mpdu_desc_pn_hl(htt_pdev_handle pdev, void *mpdu_desc,
 ...
 */
static inline uint8_t htt_rx_mpdu_desc_tid_hl(htt_pdev_handle pdev,
					      void *mpdu_desc)
{
	return 0xff; /* Invalid TID */
}

static inline bool htt_rx_msdu_desc_completes_mpdu_hl(htt_pdev_handle pdev,
						      void *msdu_desc)
{
	return (((u_int8_t *)(msdu_desc) -
		 sizeof(struct hl_htt_rx_ind_base))[HTT_ENDIAN_BYTE_IDX_SWAP(
...
static inline int htt_rx_msdu_has_wlan_mcast_flag_hl(htt_pdev_handle pdev,
						     void *msdu_desc)
{
...
static inline bool htt_rx_msdu_is_wlan_mcast_hl(htt_pdev_handle pdev,
						void *msdu_desc)
{
	struct hl_htt_rx_desc_base *rx_desc =
		(struct hl_htt_rx_desc_base *)msdu_desc;

	return HTT_WORD_GET(*(u_int32_t *)rx_desc, HTT_HL_RX_DESC_MCAST_BCAST);
}

static inline int htt_rx_msdu_is_frag_hl(htt_pdev_handle pdev, void *msdu_desc)
{
	struct hl_htt_rx_desc_base *rx_desc =
		(struct hl_htt_rx_desc_base *)msdu_desc;
