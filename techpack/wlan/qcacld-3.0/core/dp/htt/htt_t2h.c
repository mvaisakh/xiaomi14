 ...
 * Return: None
 */
static inline void htt_rx_frag_set_last_msdu(struct htt_pdev_t *pdev,
					     qdf_nbuf_t msg)
{
}
#else
...
static void htt_ipa_op_response(struct htt_pdev_t *pdev, uint32_t *msg_word)
{
	...
	op_msg_buffer =
		qdf_mem_malloc(sizeof(struct htt_wdi_ipa_op_response_t) + len);
	if (!op_msg_buffer)
		return;

	qdf_mem_copy(op_msg_buffer, msg_start_ptr,
		     sizeof(struct htt_wdi_ipa_op_response_t) + len);
	cdp_ipa_op_response(cds_get_context(QDF_MODULE_ID_SOC), OL_TXRX_PDEV_ID,
			    op_msg_buffer);
}
#else
static void htt_ipa_op_response(struct htt_pdev_t *pdev, uint32_t *msg_word)
				   ...
				   bool free_msg_buf)
{
	...
	case HTT_T2H_MSG_TYPE_RX_FLUSH: {
		...
		action = HTT_RX_FLUSH_MPDU_STATUS_GET(*(msg_word + 1)) == 1 ?
	...
	case HTT_T2H_MSG_TYPE_RX_OFFLOAD_DELIVER_IND: {
		...
		if (!pdev->cfg.is_high_latency &&
			...
			qdf_print(
				"HTT_T2H_MSG_TYPE_RX_OFFLOAD_DELIVER_IND not ");
		...
		ol_rx_offload_deliver_ind_handler(pdev->txrx_pdev, htt_t2h_msg,
						  msdu_cnt);
		if (pdev->cfg.is_high_latency) {
			/*
	...
	case HTT_T2H_MSG_TYPE_PEER_MAP: {
		...
		peer_mac_addr = htt_t2h_mac_addr_deswizzle(
			(uint8_t *)(msg_word + 1), &mac_addr_deswizzle_buf[0]);
		...
		ol_rx_peer_map_handler(pdev->txrx_pdev, peer_id, vdev_id,
				       peer_mac_addr, 1 /*can tx */);
		break;
	}
	...
	case HTT_T2H_MSG_TYPE_MGMT_TX_COMPL_IND: {
		...
		int msg_len = qdf_nbuf_len(htt_t2h_msg);
		if (msg_len < (sizeof(struct htt_mgmt_tx_compl_ind) +
			       sizeof(*msg_word))) {
			QDF_TRACE(
		...
		ol_tx_desc_update_group_credit(pdev->txrx_pdev,
					       compl_msg->desc_id, 1, 0,
					       compl_msg->status);

		DPTRACE(qdf_dp_trace_credit_record(
			QDF_TX_COMP, QDF_CREDIT_INC, 1,
	...
	case HTT_T2H_MSG_TYPE_TX_CREDIT_UPDATE_IND: {
		...
		DPTRACE(qdf_dp_trace_credit_record(
			QDF_TX_CREDIT_UPDATE, QDF_CREDIT_INC, htt_credit_delta,
			qdf_atomic_read(&pdev->txrx_pdev->target_tx_credit) +
				htt_credit_delta,
			qdf_atomic_read(&pdev->txrx_pdev->txq_grps[0].credit),
			qdf_atomic_read(&pdev->txrx_pdev->txq_grps[1].credit)));
	...
	case HTT_T2H_MSG_TYPE_FLOW_POOL_MAP: {
		...
		while (num_flows) {
			pool_map_payoad =
				(struct htt_flow_pool_map_payload_t *)msg_word;
			ol_tx_flow_pool_map_handler(
				pool_map_payoad->flow_id,
	...
	case HTT_T2H_MSG_TYPE_RX_OFLD_PKT_ERR: {
		...
		default: {
			qdf_print("unhandled error type %d",
				  HTT_RX_OFLD_PKT_ERR_MSG_SUB_TYPE_GET(
					  *msg_word));
			break;
		}
		}
...
void htt_t2h_msg_handler(void *context, HTC_PACKET *pkt)
{
	...
	case HTT_T2H_MSG_TYPE_RX_IND: {
		...
		if (msg_len <
		    (2 + HTT_RX_PPDU_DESC_SIZE32 + 1) * sizeof(uint32_t)) {
     ...
     */
		...
		if (qdf_unlikely(num_msdu_bytes > rx_mpdu_range_offset_bytes)) {
			qdf_print("HTT_T2H_MSG_TYPE_RX_IND, invalid %s %u\n",
				  "num_msdu_bytes", num_msdu_bytes);
			WARN_ON(1);
			break;
		}
		pdev->rx_mpdu_range_offset_words = rx_mpdu_range_offset_bytes >>
						   2;
		num_mpdu_ranges =
			HTT_RX_IND_NUM_MPDU_RANGES_GET(*(msg_word + 1));
		pdev->rx_ind_msdu_byte_idx = 0;
		if (qdf_unlikely(rx_mpdu_range_offset_bytes > msg_len)) {
     ...
     */
		...
		ol_rx_indication_handler(pdev->txrx_pdev, htt_t2h_msg, peer_id,
					 tid, num_mpdu_ranges);
     ...
     */
		if ((num_msdus * HTT_TX_COMPL_BYTES_PER_MSDU_ID +
			...
			qdf_print("%s: num_msdus(%d) is invalid,"
				  "adf_nbuf_len = %d\n",
				  __FUNCTION__, num_msdus, msg_len);
			break;
		}
		...
		if (pdev->cfg.is_high_latency &&
			...
			if (((old_credit + num_msdus) > MAX_TARGET_TX_CREDIT) ||
				...
				if (!pdev->cfg.default_tx_comp_req) {
					...
					HTT_TX_MUTEX_ACQUIRE(
						&pdev->credit_mutex);
					qdf_atomic_add(num_msdus,
						       &pdev->htt_tx_credit
								.target_delta);
					credit_delta =
						htt_tx_credit_update(pdev);
					...
					ol_tx_target_credit_update(
						pdev->txrx_pdev, num_msdus);
				}
			}
		}

		ol_tx_completion_handler(pdev->txrx_pdev, num_msdus, status,
					 msg_word);
		HTT_TX_SCHED(pdev);
		break;
	}
	case HTT_T2H_MSG_TYPE_RX_PN_IND: {
		...
		ol_rx_pn_ind_handler(pdev->txrx_pdev, peer_id, tid,
				     seq_num_start, seq_num_end, pn_ie_cnt,
				     pn_ie);
...
#if defined(HELIUMPLUS_DEBUG)
		qdf_print("peerid %d tid %d offloadind %d fragind %d", peer_id,
			  tid, offload_ind, frag_ind);
#endif
			      ...
			      uint32_t num_cmpls)
{
     ...
     */
		...
		/* confirm alignment */
		HTT_ASSERT3((((unsigned long)qdf_nbuf_data(htt_t2h_msg)) &
			     0x3) == 0);
		...
		case HTT_T2H_MSG_TYPE_RX_IND: {
			...
			if (tid >= OL_TXRX_NUM_EXT_TIDS) {
				qdf_print(
					"HTT_T2H_MSG_TYPE_RX_IND, invalid tid %d\n",
					tid);
				WARN_ON(1);
				break;
			}
       ...
       */
		...
		case HTT_T2H_MSG_TYPE_TX_OFFLOAD_DELIVER_IND: {
			...
			offload_deliver_msg =
				(struct htt_tx_offload_deliver_ind_hdr_t *)
					msg_word;
			is_pkt_during_roam = (offload_deliver_msg->reserved_2 ?
						      true :
						      false);
       ...
       */
		...
		case HTT_T2H_MSG_TYPE_RX_IN_ORD_PADDR_IND: {
			...
			if (qdf_unlikely(!pdev->cfg.is_full_reorder_offload)) {
				qdf_print(
					"HTT_T2H_MSG_TYPE_RX_IN_ORD_PADDR_IND not supported when "
					"full reorder offload is disabled\n");
				break;
			}
			...
			if (qdf_unlikely(
				...
				qdf_print(
					"HTT_T2H_MSG_TYPE_RX_IN_ORD_PADDR_IND not supported on high "
					"latency\n");
				break;
			}

			peer_id =
				HTT_RX_IN_ORD_PADDR_IND_PEER_ID_GET(*msg_word);
			tid = HTT_RX_IN_ORD_PADDR_IND_EXT_TID_GET(*msg_word);
			offload_ind =
				HTT_RX_IN_ORD_PADDR_IND_OFFLOAD_GET(*msg_word);
			frag_ind = HTT_RX_IN_ORD_PADDR_IND_FRAG_GET(*msg_word);
...
void htt_rx_ind_flush_seq_num_range(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg,
...
void htt_rx_ind_release_seq_num_range(htt_pdev_handle pdev,
...
void htt_rx_ind_mpdu_range_info(struct htt_pdev_t *pdev, qdf_nbuf_t rx_ind_msg,
				int mpdu_range_num, enum htt_rx_status *status,
				int *mpdu_count)
{
	uint32_t *msg_word;
...
int16_t htt_rx_ind_rssi_dbm(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg)
{
	...
	return (HTT_TGT_RSSI_INVALID == rssi) ? HTT_RSSI_INVALID : rssi;
}
 ...
 */
int16_t htt_rx_ind_rssi_dbm_chain(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg,
				  int8_t chain)
{
	int8_t rssi;
	uint32_t *msg_word;
 ...
 */
void htt_rx_ind_legacy_rate(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg,
 ...
 */
void htt_rx_ind_timestamp(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg,
 ...
 */
uint32_t htt_rx_ind_tsf32(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg)
{
	uint32_t *msg_word;
 ...
 * Return: Extended TID
 */
uint8_t htt_rx_ind_ext_tid(htt_pdev_handle pdev, qdf_nbuf_t rx_ind_msg)
{
	uint32_t *msg_word;
...
void htt_t2h_dbg_stats_hdr_parse(uint8_t *stats_info_list,
				 enum htt_dbg_stats_type *type,
				 enum htt_dbg_stats_status *status, int *length,
				 uint8_t **stats_data)
{
...
void htt_rx_frag_ind_flush_seq_num_range(htt_pdev_handle pdev,
