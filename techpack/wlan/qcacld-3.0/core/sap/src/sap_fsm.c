...
static QDF_STATUS sap_get_freq_list(struct sap_context *sap_ctx,
				    uint32_t **freq_list, uint8_t *num_ch);
#endif
 ...
 */
static bool sap_is_chan_change_needed(struct sap_context *sap_ctx)
{
	uint8_t ch_wd;
	...
	if (ch_params->ch_width == sap_ctx->ch_params.ch_width) {
		status = wlan_reg_extract_puncture_by_bw(
			ch_params->ch_width, ch_params->reg_punc_bitmap,
			sap_ctx->chan_freq, ch_params->mhz_freq_seg1,
			CH_WIDTH_20MHZ, &pri_freq_puncture);
...
#else
static inline bool sap_is_chan_change_needed(struct sap_context *sap_ctx)
{
	return true;
}
#endif
...
static qdf_freq_t sap_random_channel_sel(struct sap_context *sap_ctx)
{
	...
	if (mac_ctx->sap.SapDfsInfo.sap_operating_chan_preferred_location == 1)
		flag |= DFS_RANDOM_CH_FLAG_NO_UPEER_5G_CH;
	else if (mac_ctx->sap.SapDfsInfo.sap_operating_chan_preferred_location ==
		 2)
   ...
   */
	...
	if (sap_ctx->candidate_freq &&
		...
		wlan_reg_set_channel_params_for_pwrmode(
			pdev, chan_freq, 0, ch_params, REG_CURRENT_PWR_MODE);
					    ...
					    struct ch_params *ch_params)
{
	...
	if (IS_CH_BONDING_WITH_WEATHER_CH(
		    wlan_reg_freq_to_chan(pdev, chan_freq)) &&
 ...
 */
static uint8_t sap_ch_params_to_bonding_channels(struct ch_params *ch_params,
 ...
 */
static bool sap_operating_on_dfs(struct mac_context *mac_ctx,
				 struct sap_context *sap_ctx)
{
	...
	if (chan->ch_flagext & (IEEE80211_CHAN_DFS | IEEE80211_CHAN_DFS_CFREQ2))
		return true;

	return false;
}

bool sap_plus_sap_cac_skip(struct mac_context *mac, struct sap_context *sap_ctx,
			   qdf_freq_t chan_freq)
{
	uint8_t intf;
	...
	for (intf = 0; intf < SAP_MAX_NUM_SESSION; intf++) {
		...
		if (sap_context->isCacEndNotified &&
			...
			sap_debug(
				"SAP vid %d CAC can skip due to CAC completed on other SAP vid %d",
 ...
 */
static bool is_wlansap_cac_required_for_chan(struct mac_context *mac_ctx,
					     struct sap_context *sap_ctx,
					     qdf_freq_t chan_freq,
					     struct ch_params *ch_params)
{
	...
	if (ch_params->ch_width == CH_WIDTH_160MHZ) {
		...
		if (wlan_reg_get_5g_bonded_channel_state_for_pwrmode(
			    mac_ctx->pdev, chan_freq, ch_params,
		...
		if (wlan_reg_get_channel_state_for_pwrmode(
		    ...
		    wlan_reg_get_channel_state_for_pwrmode(
			    mac_ctx->pdev, ch_params->mhz_freq_seg1,
			    REG_CURRENT_PWR_MODE) == CHANNEL_STATE_DFS)
			is_ch_dfs = true;
	} else {
	...
	sap_debug(
		"vdev id %d chan %d is_ch_dfs %d pre_cac_complete %d ignore_cac %d "
		"cac_state %d",
		sap_ctx->sessionId, chan_freq, is_ch_dfs,
		wlan_pre_cac_complete_get(sap_ctx->vdev),
		mac_ctx->sap.SapDfsInfo.ignore_cac, cac_state);
	...
	if (cac_required) {
		sta_cnt = policy_mgr_get_mode_specific_conn_info(
			mac_ctx->psoc, freq_list, vdev_id_list, PM_STA_MODE);
...
void sap_get_cac_dur_dfs_region(struct sap_context *sap_ctx,
				uint32_t *cac_duration_ms, uint32_t *dfs_region,
				qdf_freq_t chan_freq,
				struct ch_params *ch_params)
{
	int i;
	...
	*cac_duration_ms = 0;
	...
	cac_required = is_wlansap_cac_required_for_chan(mac, sap_ctx, chan_freq,
							ch_params);
...
void sap_dfs_set_current_channel(void *ctx)
{
	...
	is_dfs = wlan_mlme_check_chan_param_has_dfs(pdev, &sap_ctx->ch_params,
						    sap_ctx->chan_freq);

	sap_debug("freq=%d, dfs %d seg0=%d, seg1=%d, bw %d", sap_ctx->chan_freq,
		  is_dfs, vht_seg0, vht_seg1, sap_ctx->ch_params.ch_width);
	...
	if (is_dfs) {
		if (policy_mgr_concurrent_beaconing_sessions_running(
			...
			if (!con_ch_freq ||
			    !wlan_reg_is_dfs_for_freq(pdev, con_ch_freq))
 ...
 */
bool sap_dfs_is_channel_in_nol_list(struct sap_context *sap_context,
				    ...
				    ePhyChanBondState chan_bondState)
{
	...
	sap_context->ch_params.mhz_freq_seg0 = wlan_reg_legacy_chan_to_freq(
		pdev, sap_context->ch_params.center_freq_seg0);
	sap_context->ch_params.mhz_freq_seg1 = wlan_reg_legacy_chan_to_freq(
		pdev, sap_context->ch_params.center_freq_seg1);
	...
	if (channel_freq == sap_context->chan_freq &&
		...
		num_ch_freq = sap_get_bonding_channels(sap_context,
						       channel_freq, freq_list,
						       MAX_BONDED_CHANNELS,
						       chan_bondState);
...
bool sap_chan_bond_dfs_sub_chan(struct sap_context *sap_context,
				qdf_freq_t channel_freq,
				ePhyChanBondState bond_state)
{
	...
	if (channel_freq == sap_context->chan_freq &&
		...
		num_freq = sap_get_bonding_channels(sap_context, channel_freq,
						    freq_list,
						    MAX_BONDED_CHANNELS,
						    bond_state);
	...
	for (i = 0; i < num_freq; i++) {
		if (wlan_reg_chan_has_dfs_attribute_for_freq(pdev,
							     freq_list[i])) {
			sap_debug("sub ch freq=%d is dfs in %d", freq_list[i],
				  channel_freq);
			return true;
		}
	}
			     ...
			     uint32_t con_ch_freq)
{
   ...
   */
	if ((con_ch_freq >= 2467) && (con_ch_freq <= 2484) &&
		...
		sap_debug(
			"conc ch freq %d & sap acs ch list is 1-11, prefer mcc",
			con_ch_freq);
		return true;
	}
 ...
 */
static bool sap_process_force_scc_with_go_start(struct wlan_objmgr_psoc *psoc,
						struct sap_context *sap_context)
{
	...
	existing_vdev_id = policy_mgr_fetch_existing_con_info(
		psoc, sap_context->sessionId, sap_context->chan_freq,
		&existing_vdev_mode, &con_freq, &ch_width);
	if (existing_vdev_id < WLAN_UMAC_VDEV_ID_MAX &&
	    existing_vdev_mode == PM_SAP_MODE) {
		sap_debug(
			"concurrent sap vdev: %d on freq %d, skip GO force scc",
...
static bool sap_process_liberal_scc_for_go(struct sap_context *sap_context)
{
	...
	existing_vdev_id = policy_mgr_fetch_existing_con_info(
		mac_ctx->psoc, sap_context->sessionId, sap_context->chan_freq,
		&existing_vdev_mode, &con_freq, &ch_width);
	...
	if (existing_vdev_id < WLAN_UMAC_VDEV_ID_MAX &&
		...
		sap_debug("set forcescc flag for go vdev: %d",
			  existing_vdev_id);
		sap_set_forcescc_required(existing_vdev_id);
		return true;
	}
	if (existing_vdev_id < WLAN_UMAC_VDEV_ID_MAX &&
...
QDF_STATUS
sap_validate_chan(struct sap_context *sap_context, bool pre_start_bss,
	...
	if (policy_mgr_concurrent_beaconing_sessions_running(mac_ctx->psoc) ||
		...
		if (sap_context->cc_switch_mode !=
			...
			con_ch_freq = sme_check_concurrent_channel_overlap(
				mac_handle, sap_context->chan_freq,
				sap_context->phyMode,
				sap_context->cc_switch_mode,
				sap_context->sessionId);
			sap_debug(
				"After check overlap: sap freq %d con freq:%d",
       ...
       */
			...
			if (con_ch_freq &&
			    ...
			    (!wlan_mlme_check_chan_param_has_dfs(
				     mac_ctx->pdev, &ch_params, con_ch_freq) ||
			     sta_sap_scc_on_dfs_chan)) {
				if (is_mcc_preferred(sap_context, con_ch_freq))
					goto validation_done;

				sap_debug(
					"Override ch freq %d (bw %d) to %d (bw %d) due to CC Intf",
					sap_context->chan_freq,
...
validation_done:
   ...
   */
	...
	if (pre_start_bss) {
		...
		sap_context->acs_cfg->ch_width = sap_context->ch_width_orig;
...
static void sap_sort_freq_list(struct chan_list *list, uint8_t num_ch)
{
	int i, j, temp;
					    ...
					    uint8_t *ch_count)
{
	...
	while (loop_count < *ch_count) {
		...
		if (qdf_system_time_before(
			...
			sap_info(
				"ACS chan %d skipped from scan as last scan ts %lu\n",
	...
	if (*ch_count == 0)
		sap_info(
			"All ACS freq channels are scanned recently, skip ACS scan\n");
	else
 ...
 * Return: void
 */
static void sap_reset_clean_freq_array(struct sap_context *sap_context)
{
	memset(sap_context->clean_channel_array, 0, NUM_CHANNELS);
}
#else
static inline void sap_reset_clean_freq_array(struct sap_context *sap_context)
{
}
#endif
...
QDF_STATUS sap_channel_sel(struct sap_context *sap_context)
{
	...
	if (sap_context->acs_cfg->skip_scan_status == eSAP_SKIP_ACS_SCAN) {
		...
		wlansap_pre_start_bss_acs_scan_callback(mac_handle, sap_context,
							sap_context->sessionId,
							0, eCSR_SCAN_SUCCESS);
	}
#endif
...
QDF_STATUS sap_acquire_vdev_ref(struct wlan_objmgr_psoc *psoc,
				struct sap_context *sap_ctx, uint8_t session_id)
{
				 ...
				 uint32_t session_id)
{
	...
	sap_debug("Initializing sap_ctx = %pK with session = %d", sapctx,
		  session_id);
...
{
	if ((ch_width == CH_WIDTH_320MHZ) || (ch_width == CH_WIDTH_160MHZ) ||
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS sap_update_mcs_rate(struct sap_context *sap_ctx,
				      bool is_start)
{
	...
	sap_debug(
		"session id %d, disable mcs13 support %d, he_mcs_12_13 %d, start "
		"%d, disabled_mcs13 %d, ch width %d",
		sap_ctx->sessionId, disable_mcs13_support, he_mcs_12_13_supp,
		is_start, sap_ctx->disabled_mcs13, sap_ctx->ch_params.ch_width);

	if (!disable_mcs13_support || !he_mcs_12_13_supp)
		return status;
	...
	if (is_start) {
		status = sme_send_unit_test_cmd(sap_ctx->sessionId, 10, 2,
						fixed_mcs);
				      ...
				      eSapStatus status)
{
	...
	sap_event->sapevt.sap_acs_scan_comp.freq_list = sap_context->freq_list;
}
#else
static void sap_handle_acs_scan_event(struct sap_context *sap_context,
				      struct sap_event *sap_event,
					 ...
					 bool reassoc)
{
	...
	if (reassoc) {
		...
		assoc_req_ie =
			assoc_ind->assocReqPtr + WLAN_REASSOC_REQ_IES_OFFSET;
		assoc_req_ie_len =
			assoc_ind->assocReqLength - WLAN_REASSOC_REQ_IES_OFFSET;
	} else {
		...
		assoc_req_ie =
			assoc_ind->assocReqPtr + WLAN_ASSOC_REQ_IES_OFFSET;
		assoc_req_ie_len =
			assoc_ind->assocReqLength - WLAN_ASSOC_REQ_IES_OFFSET;
	}
	rsn_ie = wlan_get_ie_ptr_from_eid(DOT11F_EID_RSN, assoc_req_ie,
					  assoc_req_ie_len);
	if (!rsn_ie) {
 ...
 * Return: void
 */
static void sap_update_cac_history(struct mac_context *mac_ctx,
				   struct sap_context *sap_ctx,
				   eSapHddEvent event_id)
{
	...
	case eSAP_DFS_RADAR_DETECT:
		qdf_mem_zero(cac_result, sizeof(struct prev_cac_result));
		break;
	...
	case eSAP_STOP_BSS_EVENT:
		if (cac_result->ap_start_time) {
			...
			cac_result->ap_end_time = qdf_get_monotonic_boottime();
			...
			sap_debug("ap cac saved %llu ms %llu (%d, %d) bw %d",
				  diff_ms, cac_result->ap_end_time,
				  cac_result->cac_ch_param.mhz_freq_seg0,
				  cac_result->cac_ch_param.mhz_freq_seg1,
				  cac_result->cac_ch_param.ch_width);
			if (event_id == eSAP_STOP_BSS_EVENT)
 ...
 * Return: bool
 */
static bool find_ch_freq_in_radar_hist(struct dfs_radar_history *radar_result,
 ...
 * Return: void
 */
static void sap_append_cac_history(struct mac_context *mac_ctx,
	...
	if (ch_param.ch_width <= CH_WIDTH_20MHZ) {
		...
		while (chan_cfreq <= bonded_chan_ptr->end_freq) {
			...
			if (state == CHANNEL_STATE_INVALID) {
				sap_debug("invalid ch freq %d", chan_cfreq);
			...
			if (wlan_reg_is_dfs_for_freq(mac_ctx->pdev,
			    ...
			    *idx < max_elems) {
				...
				radar_result[*idx].radar_found = false;
				sap_debug(
					"radar hist[%d] freq %d time %llu no radar",
					*idx, chan_cfreq,
					cac_result->ap_end_time);
				(*idx)++;
			}
			    ...
			    uint32_t *count)
{
	...
	radar_result =
		qdf_mem_malloc(sizeof(struct dfs_radar_history) * hist_count);
	if (!radar_result) {
	...
	for (i = 0; i < nol_info->dfs_ch_nchans && i < DFS_CHAN_MAX; i++) {
		...
		sap_debug("radar hist[%d] freq %d time %llu radar", i,
			  nol_info->dfs_nol[i].nol_freq,
...
#else
static inline void sap_update_cac_history(struct mac_context *mac_ctx,
					  struct sap_context *sap_ctx,
					  eSapHddEvent event_id)
{
}
...
static inline bool
sap_check_peer_for_peer_null_mldaddr(struct wlan_objmgr_peer *peer)
{
...
static QDF_STATUS
sap_populate_peer_assoc_info(struct mac_context *mac_ctx,
			     ...
			     struct sap_event *sap_ap_event)
{
	...
	peer = wlan_objmgr_get_peer_by_mac(
		mac_ctx->psoc, csr_roaminfo->peerMac.bytes, WLAN_LEGACY_MAC_ID);
	if (!peer) {
	...
	if (sap_check_peer_for_peer_null_mldaddr(peer) ||
		...
		reassoc_complete->ies_len =
			(csr_roaminfo->assocReqLength - ASSOC_REQ_IE_OFFSET);
		reassoc_complete->ies =
			(csr_roaminfo->assocReqPtr + ASSOC_REQ_IE_OFFSET);
...
sap_reassoc_mld_copy(struct csr_roam_info *csr_roaminfo,
		     tSap_StationAssocReassocCompleteEvent *reassoc_complete)
{
	qdf_copy_macaddr(&reassoc_complete->sta_mld, &csr_roaminfo->peer_mld);
...
QDF_STATUS sap_signal_hdd_event(struct sap_context *sap_ctx,
				struct csr_roam_info *csr_roaminfo,
				eSapHddEvent sap_hddevent, void *context)
{
	QDF_STATUS qdf_status = QDF_STATUS_SUCCESS;
	struct sap_event *sap_ap_event;
	struct mac_context *mac_ctx;
	...
	case eSAP_STA_ASSOC_IND:
		...
		if (csr_roaminfo->owe_pending_assoc_ind) {
			...
			if (!sap_save_owe_pending_assoc_ind(
				    sap_ctx,
				    csr_roaminfo->owe_pending_assoc_ind)) {
		...
		if (csr_roaminfo->ft_pending_assoc_ind) {
			if (!sap_save_ft_pending_assoc_ind(
				    sap_ctx,
	...
	case eSAP_START_BSS_EVENT:
		...
		if (QDF_IS_STATUS_SUCCESS(bss_complete->status)) {
			sap_update_cac_history(mac_ctx, sap_ctx, sap_hddevent);
	...
	case eSAP_STA_REASSOC_EVENT:
		...
		chaninfo->reg_info_1 = csr_roaminfo->chan_info.reg_info_1;
		chaninfo->reg_info_2 = csr_roaminfo->chan_info.reg_info_2;
	...
	case eSAP_WPS_PBC_PROBE_REQ_EVENT:
		...
		qdf_mem_copy(
			&sap_ap_event->sapevt.sapPBCProbeReqEvent.WPSPBCProbeReq,
	...
	case eSAP_UNKNOWN_STA_JOIN:
		...
		qdf_mem_copy((void *)sap_ap_event->sapevt.sapUnknownSTAJoin
				     .macaddr.bytes,
			     (void *)context, QDF_MAC_ADDR_SIZE);
		break;
	...
	case eSAP_MAX_ASSOC_EXCEEDED:
		...
		qdf_copy_macaddr(
			&sap_ap_event->sapevt.sapMaxAssocExceeded.macaddr,
			&csr_roaminfo->peerMac);
		break;
	...
	case eSAP_STOP_BSS_DUE_TO_NO_CHNL:
		...
		sap_debug("stopping session_id:%d, bssid:" QDF_MAC_ADDR_FMT
			  ", chan_freq:%d",
			  sap_ctx->sessionId,
	...
	case eSAP_CHANNEL_CHANGE_RESP:
		...
		acs_selected->pri_ch_freq = sap_ctx->chan_freq;
		acs_selected->ht_sec_ch_freq = sap_ctx->sec_ch_freq;
		acs_selected->ch_width = sap_ctx->ch_params.ch_width;
		acs_selected->vht_seg0_center_ch_freq =
	...
	qdf_status =
		(*sap_ctx->sap_event_cb)(sap_ap_event, sap_ctx->user_context);
...
bool sap_is_dfs_cac_wait_state(struct sap_context *sap_ctx)
{
	...
	vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		mac_ctx->psoc, sap_ctx->sessionId, WLAN_DFS_ID);
	if (!vdev) {
...
void sap_cac_reset_notify(mac_handle_t mac_handle)
{
	...
	for (intf = 0; intf < SAP_MAX_NUM_SESSION; intf++) {
		...
		if (((QDF_SAP_MODE == mac->sap.sapCtxList[intf].sapPersona) ||
		     (QDF_P2P_GO_MODE ==
		      mac->sap.sapCtxList[intf].sapPersona)) &&
		    mac->sap.sapCtxList[intf].sap_context) {
	...
	for (intf = 0; intf < SAP_MAX_NUM_SESSION; intf++) {
		...
		if (((QDF_SAP_MODE == mac->sap.sapCtxList[intf].sapPersona) ||
			...
			if (!sap_operating_on_dfs(
				    mac, mac->sap.sapCtxList[intf].sap_context))
				continue;
			sap_debug(
				"sapdfs: Signaling eSAP_DFS_CAC_START to HDD for sapctx[%pK]",
				sap_context);

			qdf_status = sap_signal_hdd_event(
				sap_context, NULL, eSAP_DFS_CAC_START,
				(void *)eSAP_STATUS_SUCCESS);
	...
	for (intf = 0; intf < SAP_MAX_NUM_SESSION; intf++) {
       ...
       */
			if (wlan_pre_cac_get_status(mac->psoc)) {
				wlansap_pre_cac_end_notify(sap_context, mac,
							   intf);
         ...
         */
			...
			qdf_status = sap_signal_hdd_event(
				sap_context, NULL, eSAP_DFS_CAC_END,
				(void *)eSAP_STATUS_SUCCESS);
			if (QDF_STATUS_SUCCESS != qdf_status) {
       ...
       */
			sap_context->fsm_state = SAP_STARTED;
			sap_debug(
				"sap_fsm: vdev %d: SAP_STARTING => SAP_STARTED, freq %d",
				sap_context->vdev_id, sap_context->chan_freq);

			/*Action code for transition */
			qdf_status = sap_signal_hdd_event(
				sap_context, roamInfo, eSAP_START_BSS_EVENT,
				(void *)eSAP_STATUS_SUCCESS);
			if (QDF_STATUS_SUCCESS != qdf_status) {
				sap_err("Failed setting isCacEndNotified on interface[%d]",
					intf);
				       ...
				       struct mac_context *mac_ctx)
{
	...
	utils_dfs_mark_leaking_chan_for_freq(
		mac_ctx->pdev, sap_ctx->ch_params.ch_width, 1, &temp_freq);
	...
	if (is_chan_nol || b_leak_chan) {
		...
		if (sap_phymode_is_eht(sap_ctx->phyMode))
			wlan_reg_set_create_punc_bitmap(&sap_ctx->ch_params,
							true);
		wlan_reg_set_channel_params_for_pwrmode(
			mac_ctx->pdev, sap_ctx->chan_freq, sap_ctx->sec_ch_freq,
					      ...
					      struct sap_context *sap_ctx)
{
	...
	if (sap_ctx->ch_params.ch_width > CH_WIDTH_20MHZ &&
		...
		wlan_reg_set_channel_params_for_pwrmode(
			mac_ctx->pdev, sap_ctx->chan_freq,
			sap_ctx->ch_params.sec_ch_offset, &sap_ctx->ch_params,
			REG_CURRENT_PWR_MODE);
	...
	if (orig_ch_width != sap_ctx->ch_params.ch_width ||
		...
		sap_info("Freq %d Updated BW %d -> %d , phymode %d -> %d",
			 ...
			 sap_ctx->ch_params.ch_width, orig_phymode,
			 sap_ctx->phyMode);
}

static bool wlansap_is_power_change_required(struct mac_context *mac_ctx,
					     qdf_freq_t sap_freq)
{
	...
	sta_vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		mac_ctx->psoc, sta_vdev_id, WLAN_LEGACY_SAP_ID);
	if (!sta_vdev)
		return false;
	...
	if (ap_pwr_type_6g == REG_INDOOR_AP && indoor_ch_support) {
		sap_debug(
			"STA is connected to Indoor AP and indoor concurrency is supported");
		return false;
	}
	...
	state = wlan_reg_get_channel_state_for_pwrmode(mac_ctx->pdev, sap_freq,
						       REG_AP_VLP);
				    ...
				    mac_handle_t mac_handle)
{
   ...
   */
	...
	sap_debug(
		"sap_fsm: vdev %d: SAP_INIT => SAP_STARTING, phyMode %d bw %d",
	...
	qdf_mem_copy(dot11_cfg.opr_rates.rate, opr_rates->rate,
		     opr_rates->numRates);
	dot11_cfg.opr_rates.numRates = opr_rates->numRates;

	qdf_mem_copy(dot11_cfg.ext_rates.rate, ext_rates->rate,
		     ext_rates->numRates);
	...
	if (dot11_cfg.opr_rates.numRates) {
		qdf_mem_copy(opr_rates->rate, dot11_cfg.opr_rates.rate,
	...
	if (dot11_cfg.ext_rates.numRates) {
		qdf_mem_copy(ext_rates->rate, dot11_cfg.ext_rates.rate,
				     ...
				     mac_handle_t mac_handle)
{
     ...
     */
		...
		qdf_status = sap_goto_starting(sap_ctx, sap_event, mac_ctx,
					       mac_handle);
		if (QDF_IS_STATUS_ERROR(qdf_status))
	...
	if (mac_ctx->sap.SapDfsInfo.target_chan_freq) {
		if (sap_phymode_is_eht(sap_ctx->phyMode))
			wlan_reg_set_create_punc_bitmap(&sap_ctx->ch_params,
							true);
		wlan_reg_set_channel_params_for_pwrmode(
			mac_ctx->pdev, mac_ctx->sap.SapDfsInfo.target_chan_freq,
					       ...
					       mac_handle_t mac_handle)
{
     ...
     */
		...
		sap_debug(
			"sap_fsm: vdev %d: SAP_STARTING => SAP_STOPPING, start is in progress",
			sap_ctx->vdev_id);
...
static void sap_propagate_cac_events(struct sap_context *sap_ctx)
{
	...
	qdf_status = sap_signal_hdd_event(sap_ctx, NULL, eSAP_DFS_CAC_START,
					  (void *)eSAP_STATUS_SUCCESS);
	...
	qdf_status = sap_signal_hdd_event(sap_ctx, NULL, eSAP_DFS_CAC_END,
					  (void *)eSAP_STATUS_SUCCESS);
	if (qdf_status != QDF_STATUS_SUCCESS) {
...
static qdf_freq_t sap_get_safe_channel_freq(struct sap_context *sap_ctx)
{
	...
	if (!freq)
		freq = wlansap_get_safe_channel_from_pcl_and_acs_range(sap_ctx,
								       NULL);

	sap_debug(
		"new selected freq %d as target chan as current freq unsafe %d",
						...
						struct sap_context *sap_ctx)
{
   ...
   */
	...
	sap_debug("sap vdev %d change to safe ch freq %d bw %d from unsafe %d",
		  ...
		  sap_ctx->chan_freq);
	status = wlansap_set_channel_change_with_csa(sap_ctx, target_chan_freq,
						     target_bw, false);
	if (QDF_IS_STATUS_ERROR(status))
					 ...
					 mac_handle_t mac_handle)
{
     ...
     */
		...
		sap_debug(
			"sap_fsm: vdev %d: SAP_STARTING => SAP_STARTED, freq %d ch_width %d",
		...
		if (sap_ctx->is_chan_change_inprogress) {
			...
			qdf_status = sap_signal_hdd_event(
				sap_ctx, roam_info, eSAP_CHANNEL_CHANGE_EVENT,
				(void *)eSAP_STATUS_SUCCESS);
			sap_ctx->is_chan_change_inprogress = false;
		} else {
			sap_debug(
				"vdev %d notify hostapd about chan freq selection: %d",
				sap_ctx->vdev_id, sap_ctx->chan_freq);
			qdf_status = sap_signal_hdd_event(
				sap_ctx, roam_info, eSAP_CHANNEL_CHANGE_EVENT,
				(void *)eSAP_STATUS_SUCCESS);
			/* Action code for transition */
			qdf_status = sap_signal_hdd_event(
				sap_ctx, roam_info, eSAP_START_BSS_EVENT,
				(void *)eSAP_STATUS_SUCCESS);
		}
		...
		if (sap_ctx->ch_params.center_freq_seg1)
			ch_cfreq1 = wlan_reg_chan_band_to_freq(
				mac_ctx->pdev,
				sap_ctx->ch_params.center_freq_seg1, BIT(band));
       ...
       */
		...
		if (is_dfs) {
			...
			if ((false == sap_dfs_info->ignore_cac) &&
				...
				sap_ctx->fsm_state = SAP_STARTING;
				sap_debug(
					"sap_fsm: vdev %d: SAP_STARTED => SAP_STARTING to start cac timer",
					sap_ctx->vdev_id);
     ...
     */
		...
		qdf_status =
			sap_fsm_handle_start_failure(sap_ctx, msg, mac_handle);
		...
		for (intf = 0; intf < SAP_MAX_NUM_SESSION; intf++) {
			...
			if (((QDF_SAP_MODE ==
			     ...
			     (QDF_P2P_GO_MODE ==
			      mac_ctx->sap.sapCtxList[intf].sapPersona)) &&
			    mac_ctx->sap.sapCtxList[intf].sap_context) {
				temp_sap_ctx = mac_ctx->sap.sapCtxList[intf]
						       .sap_context;
				/*
         * Radar won't come on non-dfs channel, so
         * no need to move them
         */
				if (!sap_operating_on_dfs(mac_ctx,
							  temp_sap_ctx)) {
					sap_debug(
						"vdev %d freq %d (state %d) is not DFS or disabled so continue",
				...
				sap_debug("vdev %d switch freq %d -> %d",
					  ...
					  mac_ctx->sap.SapDfsInfo
						  .target_chan_freq);
				qdf_status = sap_fsm_send_csa_restart_req(
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS sap_fsm_state_stopping(struct sap_context *sap_ctx,
					 struct sap_sm_event *sap_event,
					 struct mac_context *mac_ctx,
					 mac_handle_t mac_handle)
{
...
QDF_STATUS sap_fsm(struct sap_context *sap_ctx, struct sap_sm_event *sap_event)
{
   ...
   */
	...
	switch (state_var) {
	case SAP_INIT:
		qdf_status = sap_fsm_state_init(sap_ctx, sap_event, mac_ctx,
						mac_handle);
		break;

	case SAP_STARTING:
		qdf_status = sap_fsm_state_starting(sap_ctx, sap_event, mac_ctx,
						    mac_handle);
		break;
	...
	case SAP_STOPPING:
		qdf_status = sap_fsm_state_stopping(sap_ctx, sap_event, mac_ctx,
						    mac_handle);
		break;
	}
	return qdf_status;
...
void sap_sort_mac_list(struct qdf_mac_addr *macList, uint16_t size)
{
	...
	for (outer = 0; outer < size; outer++) {
		for (inner = 0; inner < size - 1; inner++) {
			nRes = qdf_mem_cmp((macList + inner)->bytes,
...
bool sap_search_mac_list(struct qdf_mac_addr *macList, uint16_t num_mac,
			 uint8_t *peerMac, uint16_t *index)
{
	...
	while (nStart <= nEnd) {
		...
		nRes = qdf_mem_cmp(&macList[nMiddle], peerMac,
				   QDF_MAC_ADDR_SIZE);
...
void sap_add_mac_to_acl(struct qdf_mac_addr *macList, uint16_t *size,
			uint8_t *peerMac)
{
	int32_t nRes = -1;
	int i;
	...
	for (i = ((*size) - 1); i >= 0; i--) {
		nRes = qdf_mem_cmp(&macList[i], peerMac, QDF_MAC_ADDR_SIZE);
		if (nRes > 0) {
			/* Move alphabetically greater mac addresses one index down to allow for
         insertion of new mac in sorted order */
	...
	/* This should also take care of if the element is the first to be added in
   * the list */
...
void sap_remove_mac_from_acl(struct qdf_mac_addr *macList, uint16_t *size,
			     uint16_t index)
{
	int i;
   ...
   */
	...
	for (i = index; i < ((*size) - 1); i++) {
		/* Move mac addresses starting from "index" passed one index up to delete
       the void created by deletion of a mac address in ACL */
		qdf_mem_copy((macList + i)->bytes, (macList + i + 1)->bytes,
			     QDF_MAC_ADDR_SIZE);
	}
	/* The last space should be made empty since all mac addresses moved one step
   * up */
				   ...
				   uint8_t *peerMac)
{
	...
	if (sap_search_mac_list(sap_ctx->acceptMacList, sap_ctx->nAcceptMac,
				peerMac, NULL))
		return QDF_STATUS_SUCCESS;

	if (sap_search_mac_list(sap_ctx->denyMacList, sap_ctx->nDenyMac,
				peerMac, NULL)) {
   ...
   */
	...
	/* The new STA is neither in accept list nor in deny list. In this case, deny
   * the association but send a wifi event notification indicating the mac
   * address being denied
   */
...
void sap_dump_acs_channel(struct sap_acs_cfg *acs_cfg)
{
   ...
   */
	...
	for (i = 0; i < acs_cfg->ch_list_count; i++)
		len += qdf_scnprintf(chan_buff + len, buf_len - len, " %d",
				     acs_cfg->freq_list[i]);

	sap_nofl_debug("ACS freq list[%d]:%s", acs_cfg->ch_list_count,
		       chan_buff);
				    ...
				    uint32_t **freq_list, uint8_t *num_ch)
{
	...
	pm_conc_connection_list =
		policy_mgr_get_conn_info(&max_num_of_conc_connections);
	if (mac_ctx->psoc) {
		...
		is_sap_only_allow_sta_dfs_indoor_chan =
			policy_mgr_is_sap_only_allow_sta_dfs_indoor_chan(
				mac_ctx->psoc);
	}
	...
	wlansap_extend_to_acs_range(MAC_HANDLE(mac_ctx), &start_ch_freq,
				    &end_ch_freq, &band_start_ch, &band_end_ch);

	sap_debug(
		"expanded startChannel %d,EndChannel %d band_start_ch %d, band_end_ch %d",
     ...
     */
		if (!wlansap_is_channel_present_in_acs_list(
			    chan_freq, sap_ctx->acs_cfg->freq_list,
     ...
     */
		if (wlan_reg_is_dfs_in_secondary_list_for_freq(
			...
			if (wlansap_dcs_is_wlan_interference_mitigation_enabled(
				    sap_ctx))
				sap_debug(
					"dfs chan_freq %d added when dcs enabled",
					WLAN_REG_CH_TO_FREQ(loop_count));
			else if (policy_mgr_disallow_mcc(
					 mac_ctx->psoc,
					 WLAN_REG_CH_TO_FREQ(loop_count)))
				continue;
			normalize_factor = MLME_GET_DFS_CHAN_WEIGHT(
		...
		wlan_mlme_get_srd_master_mode_for_vdev(
			mac_ctx->psoc, vdev_opmode, &srd_chan_enabled);
		...
		for (i = 0; i < mac_ctx->mlme_cfg->acs.num_weight_range; i++) {
			if (chan_freq >= range_list[i].start_freq &&
				...
				sap_debug(
					"Range list, freq %d normalize weight factor %d",
...
qdf_freq_t sap_indicate_radar(struct sap_context *sap_ctx)
{
   ...
   */
	...
	if (mac->mlme_cfg->dfs_cfg.dfs_disable_channel_switch) {
		mac->sap.SapDfsInfo.new_chanWidth = sap_ctx->ch_params.ch_width;
		sap_debug(
			"DFS channel switch disabled, CSA to same ch %d wd %d",
	...
	if (chan_freq) {
		sap_info(
			"sapdfs: set chan freq before pre cac %d as target chan",
			chan_freq);
		return chan_freq;
	}
...
void sap_dfs_cac_timer_callback(void *data)
{
   ...
   */
	sap_debug(
		"sapdfs: Sending eSAP_DFS_CHANNEL_CAC_END for target_chan_freq = "
		"%d on sapctx[%pK]",
...
static int sap_start_dfs_cac_timer(struct sap_context *sap_ctx)
{
	...
	qdf_mc_timer_init(&mac->sap.SapDfsInfo.sap_dfs_cac_timer,
			  QDF_TIMER_TYPE_SW, sap_dfs_cac_timer_callback,
			  MAC_HANDLE(mac));
 ...
 * Return: None
 */
void sap_build_start_bss_config(struct start_bss_config *sap_bss_cfg,
				struct sap_config *config)
{
	...
	if (config->RSNWPAReqIELength)
		qdf_mem_copy(sap_bss_cfg->rsnIE.rsnIEdata, config->RSNWPAReqIE,
			     config->RSNWPAReqIELength);

	/* Probe response IE */
	if (config->probeRespIEsBufferLen > 0 && config->pProbeRespIEsBuffer) {
	...
	if (config->probeRespBcnIEsLen > 0 && config->pProbeRespBcnIEsBuffer) {
