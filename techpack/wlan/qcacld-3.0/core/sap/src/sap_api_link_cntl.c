					...
					struct ch_params *ch_params)
{
	sap_debug("ccfs0 %d ch_width %d, punct 0x%x",
		  ch_params->center_freq_seg0, ch_params->ch_width,
...
void sap_config_acs_result(mac_handle_t mac_handle, struct sap_context *sap_ctx,
			   uint32_t sec_ch_freq)
{
	...
	new_ch_width = wlan_sap_get_concurrent_bw(mac_ctx->pdev, mac_ctx->psoc,
...
static QDF_STATUS sap_hdd_signal_event_handler(void *ctx)
{
	...
	status = sap_signal_hdd_event(sap_ctx, NULL, sap_ctx->sap_state,
					   ...
					   uint32_t scan_id)
{
	if (scan_id) {
		...
		sap_signal_hdd_event(sap_ctx, NULL, eSAP_ACS_SCAN_SUCCESS_EVENT,
...
static uint32_t wlansap_calculate_chan_from_scan_result(
	mac_handle_t mac_handle, struct sap_context *sap_ctx, uint32_t scan_id)
{
	struct mac_context *mac_ctx = MAC_CONTEXT(mac_handle);
...
static void wlansap_filter_unsafe_ch(struct wlan_objmgr_psoc *psoc,
						   ...
						   eCsrScanStatus scan_status)
{
	...
	host_log_acs_scan_done(acs_scan_done_status_str(scan_status), sessionid,
			       scanid);

	if (sap_ctx->optimize_acs_chan_selected) {
		sap_debug(
			"SAP channel selected using first clean channel, ignore scan "
			"complete event");
	...
	if (!sap_ctx->acs_cfg->ch_list_count) {
		oper_channel =
			sap_select_default_oper_chan(mac_ctx, sap_ctx->acs_cfg);
	...
	if (eCSR_SCAN_SUCCESS != scan_status) {
		sap_err("CSR scan_status = eCSR_SCAN_ABORT/FAILURE (%d), choose default "
			"channel",
			scan_status);
		oper_channel =
			sap_select_default_oper_chan(mac_ctx, sap_ctx->acs_cfg);
 ...
 * Return: void
 */
static void wlansap_roam_process_ch_change_success(
	struct mac_context *mac_ctx, struct sap_context *sap_ctx,
	struct csr_roam_info *csr_roam_info, QDF_STATUS *ret_status)
{
   ...
   */
	...
	if (sap_ctx->fsm_state == SAP_INIT ||
		...
		sap_info(
			"sap_fsm: vdev %d: sapdfs: state %d, not starting SAP after "
			"channel change",
	...
	if (sap_ctx->ch_params.ch_width == CH_WIDTH_160MHZ) {
		...
		if (wlan_reg_get_5g_bonded_channel_state_for_pwrmode(
			    mac_ctx->pdev, target_chan_freq, &ch_params,
		...
		if (wlan_reg_get_channel_state_for_pwrmode(
		    ...
		    wlan_reg_get_channel_state_for_pwrmode(
			    mac_ctx->pdev, sap_ctx->ch_params.mhz_freq_seg1,
			    REG_CURRENT_PWR_MODE) == CHANNEL_STATE_DFS)
			is_ch_dfs = true;
	} else {
	...
	if (is_ch_dfs && wlan_pre_cac_complete_get(sap_ctx->vdev)) {
		...
		if (sap_plus_sap_cac_skip(mac_ctx, sap_ctx, sap_ctx->chan_freq))
 ...
 * Return: void
 */
static void wlansap_roam_process_dfs_chansw_update(mac_handle_t mac_handle,
						   struct sap_context *sap_ctx,
						   QDF_STATUS *ret_status)
{
     ...
     */
		...
		*ret_status = sme_roam_start_beacon_req(
			mac_handle, sap_ctx->bssid, dfs_beacon_start_req);
		return;
	}
	/*
   ...
   */
	if (sap_ctx->fsm_state != SAP_STARTED) {
		...
		sap_warn(
			"eCSR_ROAM_RESULT_DFS_RADAR_FOUND_IND received in (%d) state",
			sap_ctx->fsm_state);
		return;
	}
   ...
   */
     ...
     */
		qdf_status = wlansap_channel_change_request(
			sap_context, mac_ctx->sap.SapDfsInfo.target_chan_freq);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
 ...
 */
static void wlansap_roam_process_dfs_radar_found(struct mac_context *mac_ctx,
						 struct sap_context *sap_ctx,
						 QDF_STATUS *ret_status)
{
	QDF_STATUS qdf_status;
 ...
 * Return: result of operation
 */
static void wlansap_roam_process_infra_assoc_ind(
	struct sap_context *sap_ctx, eCsrRoamResult roam_result,
	struct csr_roam_info *csr_roam_info, QDF_STATUS *ret_status)
{
	QDF_STATUS qdf_status;

	sap_debug(
		"CSR roam_result = eCSR_ROAM_RESULT_INFRA_ASSOCIATION_IND (%d)",
		roam_result);
	...
	/* MAC filtering */
	qdf_status = sap_is_peer_mac_allowed(
		sap_ctx, (uint8_t *)csr_roam_info->peerMac.bytes);
	...
	if (QDF_STATUS_SUCCESS == qdf_status) {
		qdf_status = sap_signal_hdd_event(sap_ctx, csr_roam_info,
						  ...
						  (void *)eSAP_STATUS_SUCCESS);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			sap_err("CSR roam_result = (%d) MAC (" QDF_MAC_ADDR_FMT
				") fail",
				roam_result,
		...
		sap_warn("CSR roam_result = (%d) MAC (" QDF_MAC_ADDR_FMT
			 ") not allowed",
			 roam_result,
			 QDF_MAC_ADDR_REF(csr_roam_info->peerMac.bytes));
		*ret_status = QDF_STATUS_E_FAILURE;
					   ...
					   struct sap_context *sap_ctx)
{
	...
	mac_ctx->sap.SapDfsInfo.target_chan_freq = wlan_reg_legacy_chan_to_freq(
		mac_ctx->pdev, sap_ctx->dfs_vendor_channel);

	mac_ctx->sap.SapDfsInfo.new_chanWidth = sap_ctx->dfs_vendor_chan_bw;
 ...
 */
static void
sap_check_and_process_forcescc_for_go_plus_go(struct sap_context *cur_sap_ctx)
{
	struct sap_context *sap_ctx;
	...
	for (i = 0; i < SAP_MAX_NUM_SESSION; i++) {
		...
		if (sap_ctx &&
			...
			sap_debug(
				"update chan_freq %d of sessionId %d with chan_freq %d",
 ...
 * Return: None
 */
static void sap_check_and_process_go_force_scc(struct sap_context *sap_ctx)
{
	struct mac_context *mac_ctx;
	uint32_t con_freq;
				     ...
				     struct sap_context *sap_ctx)
{
	...
	vdev = wlan_objmgr_get_vdev_by_id_from_psoc(psoc, sap_ctx->sessionId,
						    WLAN_DFS_ID);
	if (!vdev) {
...
#else
static inline void wlan_sap_pre_cac_radar_ind(struct sap_context *sap_ctx,
					      struct mac_context *mac_ctx)
{
}
#endif

QDF_STATUS wlansap_roam_callback(void *ctx, struct csr_roam_info *csr_roam_info,
				 ...
				 eCsrRoamResult roam_result)
{
	...
	case eCSR_ROAM_DFS_RADAR_IND:
		...
		if (sap_ctx->fsm_state != SAP_STARTED &&
			...
			sap_debug(
				"Ignore Radar event in sap state %d cac wait state %d",
				sap_ctx->fsm_state,
				sap_is_dfs_cac_wait_state(sap_ctx));
			goto EXIT;
		}
		...
		if (!sap_chan_bond_dfs_sub_chan(sap_ctx, sap_ctx->chan_freq,
		...
		if (0 <
			...
			mac_ctx->sap.SapDfsInfo.target_chan_freq =
				mac_ctx->sap.SapDfsInfo
					.user_provided_target_chan_freq;
			mac_ctx->sap.SapDfsInfo.user_provided_target_chan_freq =
				0;
		}
		...
		for (intf = 0; intf < SAP_MAX_NUM_SESSION; intf++) {
			...
			if (((QDF_SAP_MODE ==
			     ...
			     (QDF_P2P_GO_MODE ==
			      mac_ctx->sap.sapCtxList[intf].sapPersona)) &&
			    mac_ctx->sap.sapCtxList[intf].sap_context != NULL) {
				sap_context = mac_ctx->sap.sapCtxList[intf]
						      .sap_context;
				if (!wlan_reg_is_passive_or_disable_for_pwrmode(
					...
					continue;
				sap_debug(
					"Vdev %d no channel available , stop bss",
	...
	case eCSR_ROAM_SET_CHANNEL_RSP:
		...
		ucfg_if_mgr_deliver_event(sap_ctx->vdev,
					  WLAN_IF_MGR_EV_AP_CSA_COMPLETE, NULL);
		break;
	...
	case eCSR_ROAM_RESULT_MAX_ASSOC_EXCEEDED:
		...
		qdf_status = sap_signal_hdd_event(
			sap_ctx, csr_roam_info, eSAP_MAX_ASSOC_EXCEEDED, NULL);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status))
			qdf_ret_status = QDF_STATUS_E_FAILURE;
	...
	case eCSR_ROAM_RESULT_CSA_RESTART_RSP:
		...
		if (!QDF_IS_STATUS_SUCCESS(qdf_ret_status))
			sap_debug(
				"CSR roam_result = eCSR_ROAM_RESULT_CSA_RESTART_RSP %d",
				roam_result);
		break;
	...
	case eCSR_ROAM_RESULT_CHANNEL_CHANGE_SUCCESS:
		wlansap_roam_process_ch_change_success(
			mac_ctx, sap_ctx, csr_roam_info, &qdf_ret_status);
     ...
     */
		...
		qdf_ret_status = sap_signal_hdd_event(
			sap_ctx, csr_roam_info, eSAP_CHANNEL_CHANGE_RESP,
			     ...
			     struct scan_event *event, void *arg)
{
   ...
   */
	...
	wlansap_pre_start_bss_acs_scan_callback(mac_handle, arg, session_id,
						scan_id, scan_status);
}
 ...
 * Return: void
 */
static void sap_mark_freq_as_clean(bool *clean_channel_array, qdf_freq_t freq)
{
	uint32_t ch_index;
 ...
 */
static bool sap_is_prev_n_freqs_free(bool *clean_channel_array,
				     uint32_t curr_index,
				     ...
				     const struct bonded_channel_freq *range)
{
	...
	if (max_index >= INVALID_CHANNEL || min_index >= INVALID_CHANNEL)
		return false;
   ...
   */
	for (index = min_index; index > 0 && index <= max_index; index++) {
 ...
 */
static bool is_freq_allowed_for_sap(struct wlan_objmgr_pdev *pdev,
				    bool *clean_channel_array, qdf_freq_t freq,
				    enum phy_ch_width ch_width,
				    struct wlan_objmgr_vdev *vdev)
{
	uint16_t min_bw = 0;
   ...
   */
	...
	status = wlan_reg_get_min_max_bw_for_chan_index(pdev, curr_index,
							&min_bw, &max_bw);
	...
	switch (ch_width) {
	case CH_WIDTH_40MHZ:
		return sap_is_prev_n_freqs_free(clean_channel_array, curr_index,
						40 / 20, range);
	case CH_WIDTH_80MHZ:
		return sap_is_prev_n_freqs_free(clean_channel_array, curr_index,
						80 / 20, range);
				     ...
				     struct csr_roam_info *roam_info)
{
	...
	state = wlan_reg_get_channel_state_for_pwrmode(
		mac->pdev, roam_info->chan_info_freq, REG_CURRENT_PWR_MODE);
   ...
   */
	if (sap_ctx->acs_cfg->ch_list_count &&
	    !wlan_reg_is_same_band_freqs(
		    sap_ctx->acs_cfg
			    ->freq_list[sap_ctx->acs_cfg->ch_list_count - 1],
		    roam_info->chan_info_freq))
		return;
	/* Confirm the freq is in ACS list. */
	if (!wlansap_is_channel_present_in_acs_list(
		    roam_info->chan_info_freq, sap_ctx->acs_cfg->freq_list,
		    sap_ctx->acs_cfg->ch_list_count))
		return;
	/* For 6 GHz, do not select non PSC channel */
	if (wlan_reg_is_6ghz_chan_freq(roam_info->chan_info_freq) &&
	    !wlan_reg_is_6ghz_psc_chan_freq(roam_info->chan_info_freq))
		return;
	...
	filter->age_threshold =
		qdf_get_time_of_the_day_ms() - sap_ctx->acs_req_timestamp;
	...
	if (sap_ctx->acs_cfg->ch_width > CH_WIDTH_20MHZ) {
		sap_mark_freq_as_clean(sap_ctx->clean_channel_array,
				       roam_info->chan_info_freq);
		if (!is_freq_allowed_for_sap(
			    mac->pdev, sap_ctx->clean_channel_array,
			    roam_info->chan_info_freq,
			    sap_ctx->acs_cfg->ch_width, sap_ctx->vdev)) {
			goto exit;
		}
	}
	...
	wlan_abort_scan(mac->pdev, WLAN_INVALID_PDEV_ID, sap_ctx->sessionId,
