 ...
 */
static inline bool osif_dp_mark_pkt_type_by_priority(struct sk_buff *skb)
{
	bool type_marked = false;
	uint32_t pkt_type = qdf_nbuf_get_priority_pkt_type(skb);
...
#define DP_IS_EXTRA_GRO_FLUSH_NECESSARY(_gro_ret) ((_gro_ret) != GRO_DROP)
#else
 ...
 */
static inline void
osif_dp_rx_thread_napi_gro_flush(struct napi_struct *napi,
...
static QDF_STATUS osif_dp_rx_napi_gro_flush(qdf_napi_struct *napi_to_use,
					    qdf_nbuf_t nbuf,
					    uint8_t *low_tput_force_flush)
{
	...
	if (DP_IS_EXTRA_GRO_FLUSH_NECESSARY(gro_ret)) {
		*low_tput_force_flush = 1;
		osif_dp_rx_thread_napi_gro_flush(
 ...
 *	   QDF error code.
 */
static QDF_STATUS osif_dp_rx_napi_gro_receive(qdf_napi_struct *napi_to_use,
					      qdf_nbuf_t nbuf)
{
	QDF_STATUS status = QDF_STATUS_SUCCESS;
	gro_result_t gro_ret;
 ...
 * Return: None
 */
static QDF_STATUS osif_dp_rx_check_qdisc_configured(qdf_netdev_t ndev,
						    uint32_t prio)
{
...
{
	return cfg80211_is_gratuitous_arp_unsolicited_na(
		(struct sk_buff *)nbuf);
}
...
static bool osif_dp_cfg80211_rx_control_port(qdf_netdev_t dev, u8 *ta_addr,
					     qdf_nbuf_t nbuf, bool unencrypted)
{
	return cfg80211_rx_control_port((struct net_device *)dev, ta_addr,
					(struct sk_buff *)nbuf, unencrypted);
}
...
#ifdef RECEIVE_OFFLOAD
static void
osif_dp_register_rx_offld_flush_cb(enum dp_rx_offld_flush_cb cb_type)
{
	...
	if (cb_type == DP_RX_FLUSH_LRO)
		...
		cdp_register_rx_offld_flush_cb(
			soc, osif_dp_rxthread_napi_normal_gro_flush);
	else if (cb_type == DP_RX_FLUSH_NAPI)
		cdp_register_rx_offld_flush_cb(soc, osif_dp_hif_napi_gro_flush);
}
#else
...
static QDF_STATUS osif_dp_rx_pkt_to_nw(qdf_nbuf_t nbuf,
