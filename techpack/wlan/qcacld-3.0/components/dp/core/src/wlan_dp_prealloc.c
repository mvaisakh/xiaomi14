...
static struct dp_prealloc_context g_dp_context_allocs[] = {
	...
	{ DP_RX_ERR_RING_HIST_TYPE, sizeof(struct dp_rx_err_history), false,
	  false, NULL },
	...
	{ DP_TX_TCL_HIST_TYPE,
	  DP_TX_TCL_HIST_PER_SLOT_MAX * sizeof(struct dp_tx_desc_event), false,
	  false, NULL },
	{ DP_TX_TCL_HIST_TYPE,
	  DP_TX_TCL_HIST_PER_SLOT_MAX * sizeof(struct dp_tx_desc_event), false,
	  false, NULL },
	...
	{ DP_TX_COMP_HIST_TYPE,
	  DP_TX_COMP_HIST_PER_SLOT_MAX * sizeof(struct dp_tx_desc_event), false,
	  false, NULL },
	{ DP_TX_COMP_HIST_TYPE,
	  DP_TX_COMP_HIST_PER_SLOT_MAX * sizeof(struct dp_tx_desc_event), false,
	  false, NULL },
	...
	{ DP_MON_STATUS_BUF_HIST_TYPE,
	  sizeof(struct dp_mon_status_ring_history), false, false, NULL },
#endif
#ifdef WIFI_MONITOR_SUPPORT
	{ DP_MON_PDEV_TYPE, sizeof(struct dp_mon_pdev), false, false, NULL },
#endif
#ifdef WLAN_FEATURE_DP_CFG_EVENT_HISTORY
	{ DP_CFG_EVENT_HIST_TYPE,
	  DP_CFG_EVT_HIST_PER_SLOT_MAX * sizeof(struct dp_cfg_event), false,
	  false, NULL },
	{ DP_CFG_EVENT_HIST_TYPE,
	  DP_CFG_EVT_HIST_PER_SLOT_MAX * sizeof(struct dp_cfg_event), false,
	  false, NULL },
...
static struct dp_consistent_prealloc g_dp_consistent_allocs[] = {
	...
	{ WBM_IDLE_LINK,
	  (sizeof(struct wbm_link_descriptor_ring)) * WBM_IDLE_LINK_RING_SIZE,
	  0, NULL, NULL, 0, 0 },
...
/* no op */
	...
	{ QDF_DP_RX_DESC_STATUS_TYPE,
	  sizeof(union dp_rx_desc_list_elem_t),
	  WLAN_CFG_RXDMA_MONITOR_STATUS_RING_SIZE + 1,
	  0,
	  CACHEABLE,
	  { 0 } },
	{ QDF_DP_RX_DESC_STATUS_TYPE,
	...
	{ QDF_DP_TX_DIRECT_LINK_CE_BUF_TYPE,
	  DIRECT_LINK_DEFAULT_BUF_SZ,
	  TX_DIRECT_LINK_CE_BUF_NUM,
	  0,
	  NON_CACHEABLE,
	  { 0 } },
	{ QDF_DP_TX_DIRECT_LINK_BUF_TYPE,
	  DIRECT_LINK_DEFAULT_BUF_SZ,
	  TX_DIRECT_LINK_BUF_NUM,
	  0,
	  NON_CACHEABLE,
	  { 0 } },
	{ QDF_DP_RX_DIRECT_LINK_CE_BUF_TYPE,
	  DIRECT_LINK_CE_RX_BUF_SIZE,
	  RX_DIRECT_LINK_CE_BUF_NUM,
	  0,
	  NON_CACHEABLE,
	  { 0 } },
#endif
...
static struct dp_consistent_prealloc_unaligned g_dp_consistent_unaligned_allocs[] = {
	/* CE-0 */
	...
	{ CE_DST, (sizeof(struct ce_srng_dest_desc) * 512 + CE_DESC_RING_ALIGN),
	  false, NULL, 0 },
	{ CE_DST_STATUS,
	  (sizeof(struct ce_srng_dest_status_desc) * 512 + CE_DESC_RING_ALIGN),
	  false, NULL, 0 },
	/* CE-2 */
	{ CE_DST, (sizeof(struct ce_srng_dest_desc) * 32 + CE_DESC_RING_ALIGN),
	  false, NULL, 0 },
	{ CE_DST_STATUS,
	  (sizeof(struct ce_srng_dest_status_desc) * 32 + CE_DESC_RING_ALIGN),
	  false, NULL, 0 },
	/* CE-3 */
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_consistent_allocs); i++) {
		...
		if (p->va_aligned) {
			dp_debug("i %d: va aligned %pK pa aligned %pK size %d",
				 ...
				 p->size);
			qdf_mem_free_consistent(qdf_ctx, qdf_ctx->dev, p->size,
						p->va_unaligned,
						p->pa_unaligned, 0);
			p->in_use = false;
			p->va_unaligned = NULL;
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_multi_page_allocs); i++) {
		...
		if (mp->pages.num_pages) {
			dp_info("i %d: type %d cacheable_pages %pK dma_pages %pK num_pages %d",
				i, mp->desc_type, mp->pages.cacheable_pages,
				mp->pages.dma_pages, mp->pages.num_pages);
			qdf_mem_multi_pages_free(qdf_ctx, &mp->pages, 0,
						 mp->cacheable);
			mp->in_use = false;
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_consistent_unaligned_allocs); i++) {
		...
		if (up->va_unaligned) {
			dp_info("i %d: va unalign %pK pa unalign %pK size %d",
				i, up->va_unaligned, (void *)up->pa_unaligned,
				up->size);
			qdf_mem_free_consistent(qdf_ctx, qdf_ctx->dev, up->size,
						up->va_unaligned,
						up->pa_unaligned, 0);
			up->in_use = false;
 ...
 */
static inline uint32_t dp_get_tx_mon_mem_size(struct wlan_dp_prealloc_cfg *cfg,
					      enum hal_ring_type ring_type)
{
	uint32_t mem_size = 0;
...
dp_get_tx_mon_desc_pool_mem_size(struct wlan_dp_prealloc_cfg *cfg)
{
 ...
 * Return: none
 */
static void dp_update_mem_size_by_ctx_type(struct wlan_dp_prealloc_cfg *cfg,
 ...
 * Return: None
 */
static void dp_update_mem_size_by_ring_type(struct wlan_dp_prealloc_cfg *cfg,
					    enum hal_ring_type ring_type,
					    uint32_t *mem_size)
{
	switch (ring_type) {
				    ...
				    uint16_t *num_elements)
{
	...
	case QDF_DP_RX_DESC_BUF_TYPE:
		*num_elements =
			cfg->num_rx_sw_desc * WLAN_CFG_RX_SW_DESC_WEIGHT_SIZE;
		return;
	default:
		return;
	}
...
QDF_STATUS dp_prealloc_init(struct cdp_ctrl_objmgr_psoc *ctrl_psoc)
{
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_context_allocs); i++) {
		cp = &g_dp_context_allocs[i];
		dp_update_mem_size_by_ctx_type(&cfg, cp->ctxt_type, &cp->size);
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_consistent_allocs); i++) {
		...
		p->va_aligned = qdf_aligned_mem_alloc_consistent(
			qdf_ctx, &p->size, &p->va_unaligned, &p->pa_unaligned,
			&p->pa_aligned, DP_RING_BASE_ALIGN);
		if (qdf_unlikely(!p->va_unaligned)) {
			dp_warn("i %d: unable to preallocate %d bytes memory!",
				i, p->size);
			break;
		}
		dp_debug("i %d: va aligned %pK pa aligned %pK size %d", i,
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_multi_page_allocs); i++) {
		...
		qdf_mem_multi_pages_alloc(qdf_ctx, &mp->pages, mp->element_size,
					  mp->element_num, 0, mp->cacheable);
		...
		dp_info("i %d: cacheable_pages %pK dma_pages %pK num_pages %d",
			i, mp->pages.cacheable_pages, mp->pages.dma_pages,
			mp->pages.num_pages);
	}
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_consistent_unaligned_allocs); i++) {
		up = &g_dp_consistent_unaligned_allocs[i];
		up->in_use = 0;
		up->va_unaligned = qdf_mem_alloc_consistent(
			qdf_ctx, qdf_ctx->dev, up->size, &up->pa_unaligned);
		if (qdf_unlikely(!up->va_unaligned)) {
			dp_warn("i %d: fail to prealloc unaligned %d bytes!", i,
				up->size);
			break;
		}
		dp_info("i %d: va unalign %pK pa unalign %pK size %d", i,
			up->va_unaligned, (void *)up->pa_unaligned, up->size);
	}
...
void *dp_prealloc_get_context_memory(uint32_t ctxt_type, qdf_size_t ctxt_size)
{
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_context_allocs); i++) {
		...
		if ((ctxt_type == cp->ctxt_type) && !cp->in_use && cp->addr &&
		    ctxt_size <= cp->size) {
			cp->in_use = true;
			return cp->addr;
		}
...
void dp_prealloc_get_multi_pages(uint32_t desc_type, qdf_size_t element_size,
				 uint16_t element_num,
				 struct qdf_mem_multi_page_t *pages,
				 bool cacheable)
{
		...
		if (desc_type == mp->desc_type && !mp->in_use &&
			...
			*pages = mp->pages;
			...
			dp_info("i %d: desc_type %d cacheable_pages %pK dma_pages %pK num_pages %d",
				i, desc_type, mp->pages.cacheable_pages,
				mp->pages.dma_pages, mp->pages.num_pages);
			break;
		}
	}
				 ...
				 struct qdf_mem_multi_page_t *pages)
{
	...
	for (i = 0; i < QDF_ARRAY_SIZE(g_dp_multi_page_allocs); i++) {
		...
		if (desc_type == mp->desc_type) {
			...
			mp_found = mp->cacheable ? (mp->pages.cacheable_pages ==
						    pages->cacheable_pages) :
						   (mp->pages.dma_pages ==
						    pages->dma_pages);
			/* find it, put back to prealloc pool */
			if (mp_found) {
				dp_info("i %d: desc_type %d returned", i,
					desc_type);
				mp->in_use = false;
	...
	if (qdf_unlikely(!mp_found))
		dp_warn("Not prealloc pages %pK desc_type %d cacheable_pages %pK dma_pages %pK",
			pages, desc_type, pages->cacheable_pages,
			pages->dma_pages);
}
