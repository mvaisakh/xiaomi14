 ...
 */
static int32_t
tdls_peer_reset_discovery_processed(struct tdls_vdev_priv_obj *tdls_vdev)
{
	int i;
	qdf_list_t *head;
...
void tdls_discovery_timeout_peer_cb(void *user_data)
{
	...
	if (wlan_vdev_mlme_is_mlo_vdev(vdev) &&
		...
		if (select_vdev) {
			tdls_vdev = wlan_objmgr_vdev_get_comp_private_obj(
				select_vdev, WLAN_UMAC_COMP_TDLS);
	...
	for (i = 0; i < WLAN_TDLS_PEER_LIST_SIZE; i++) {
		...
		while (QDF_IS_STATUS_SUCCESS(status)) {
			...
			tdls_set_peer_link_status(peer, TDLS_LINK_IDLE,
				   ...
				   struct tdls_soc_priv_obj *tdls_soc)
{
	...
	mac_entries =
		QDF_MIN(tdls_vdev->valid_mac_entries, WLAN_TDLS_CT_TABLE_SIZE);

	qdf_mem_copy(mac_table, tdls_vdev->ct_peer_table,
		     (sizeof(struct tdls_conn_tracker_mac_table)) *
			     mac_entries);
			    ...
			    struct qdf_mac_addr *dest_mac_addr)
{
	...
	if (QDF_STATUS_SUCCESS !=
	    tdls_get_vdev_objects(vdev, &tdls_vdev_obj, &tdls_soc_obj))
		return;
   ...
   */
	...
	for (mac_cnt = 0; mac_cnt < valid_mac_entries; mac_cnt++) {
		if (qdf_mem_cmp(mac_table[mac_cnt].mac_address.bytes, mac_addr,
				QDF_MAC_ADDR_SIZE) == 0) {
	...
	if (!qdf_mem_cmp(vdev->vdev_mlme.macaddr, mac_addr, QDF_MAC_ADDR_SIZE))
		return;
   ...
   */
	if (mac_cnt < WLAN_TDLS_CT_TABLE_SIZE) {
		qdf_mem_copy(mac_table[mac_cnt].mac_address.bytes, mac_addr,
			     QDF_MAC_ADDR_SIZE);
			    ...
			    struct tdls_peer *curr_peer, uint16_t reason)
{
	...
	if (!tdls_vdev || !curr_peer) {
		tdls_err("tdls_vdev: %pK, curr_peer: %pK", tdls_vdev,
			 curr_peer);
		return;
	}
	...
	if (tdls_soc->tdls_dp_vdev_update)
		tdls_soc->tdls_dp_vdev_update(
			&tdls_soc->soc, wlan_vdev_get_id(tdls_vdev->vdev),
			tdls_soc->tdls_update_dp_vdev_flags, false);
	...
	for (sta_idx = 0; sta_idx < WLAN_TDLS_STA_MAX_NUM; sta_idx++) {
		if (!qdf_mem_cmp(
			...
			tdls_debug("tdls peer exists idx %d " QDF_MAC_ADDR_FMT,
				   sta_idx, QDF_MAC_ADDR_REF(peer_mac->bytes));
...
static void tdls_ct_process_idle_handler(struct wlan_objmgr_vdev *vdev,
   ...
   */
	if ((curr_peer->tx_pkt >=
		...
		tdls_notice("trigger tdls link to " QDF_MAC_ADDR_FMT " down",
			    QDF_MAC_ADDR_REF(curr_peer->peer_mac.bytes));
		tdls_indicate_teardown(tdls_vdev_obj, curr_peer,
 ...
 * Return: None
 */
static void tdls_ct_process_connected_link(struct tdls_peer *curr_peer,
					   struct tdls_vdev_priv_obj *tdls_vdev,
					   struct tdls_soc_priv_obj *tdls_soc)
{
	...
	if ((0 != tdls_vdev->threshold_config.idle_packet_n) &&
		...
		if (QDF_TIMER_STATE_RUNNING !=
			...
			tdls_warn("Tx/Rx Idle timer start: " QDF_MAC_ADDR_FMT
				  ...
				  QDF_MAC_ADDR_REF(curr_peer->peer_mac.bytes));
			tdls_timer_restart(
				tdls_vdev->vdev, &curr_peer->peer_idle_timer,
			      ...
			      struct tdls_soc_priv_obj *tdls_soc_obj)
{
	if (curr_peer->rx_pkt || curr_peer->tx_pkt)
		tdls_debug(QDF_MAC_ADDR_FMT
			   "link_status %d tdls_support %d tx %d rx %d rssi %d",
			   QDF_MAC_ADDR_REF(curr_peer->peer_mac.bytes),
					...
					struct tdls_soc_priv_obj *tdls_soc)
{
	...
	if (curr_peer->rx_pkt || curr_peer->tx_pkt)
		tdls_debug(QDF_MAC_ADDR_FMT
			   "link_status %d tdls_support %d tx %d rx %d",
			   QDF_MAC_ADDR_REF(curr_peer->peer_mac.bytes),
			   curr_peer->link_status, curr_peer->tdls_support,
			   curr_peer->tx_pkt, curr_peer->rx_pkt);
...
static void tdls_ct_process_handler(struct wlan_objmgr_vdev *vdev)
{
	...
	for (i = 0; i < WLAN_TDLS_PEER_LIST_SIZE; i++) {
		...
		while (QDF_IS_STATUS_SUCCESS(status)) {
			...
			curr_peer->rx_pkt = 0;
			status =
				qdf_list_peek_next(head, list_node, &list_node);
		}
	}
...
int tdls_set_tdls_offchannel(struct tdls_soc_priv_obj *tdls_soc, int offchannel)
{
		    ...
		    struct tdls_channel_switch_params *params)
{
	...
	if (params->tdls_off_ch_bw_offset == BW40_HIGH_PRIMARY)
		params->oper_class = tdls_find_opclass(
			psoc, params->tdls_off_ch, BW40_LOW_PRIMARY);
	else if (params->tdls_off_ch_bw_offset == BW40_LOW_PRIMARY)
		params->oper_class = tdls_find_opclass(
			psoc, params->tdls_off_ch, BW40_HIGH_PRIMARY);
}

#ifdef WLAN_FEATURE_TDLS_CONCURRENCIES
static inline QDF_STATUS tdls_update_peer_off_channel_list(
	struct wlan_objmgr_pdev *pdev, struct tdls_soc_priv_obj *tdls_soc,
	struct wlan_objmgr_vdev *vdev, struct tdls_peer *peer,
	struct tdls_channel_switch_params *params)
{
   ...
   */
     ...
     */
		if (tdls_soc->tdls_off_channel &&
			...
			params->oper_class = tdls_get_opclass_from_bandwidth(
...
int tdls_set_tdls_offchannelmode(struct wlan_objmgr_vdev *vdev, int offchanmode)
{
	...
	tdls_notice(
		"TDLS Channel Switch in off_chan_mode=%d tdls_off_channel %d "
		"offchanoffset %d",
	...
	case ENABLE_CHANSWITCH:
		if (tdls_soc->tdls_off_channel &&
			...
			chan_switch_params->tdls_off_ch = wlan_reg_freq_to_chan(
				pdev, conn_peer->pref_off_chan_freq);
			chan_switch_params
				->oper_class = tdls_get_opclass_from_bandwidth(
     ...
     */
		status = tdls_update_peer_off_channel_list(
			pdev, tdls_soc, vdev, conn_peer, chan_switch_params);
	...
	tdls_notice("Peer " QDF_MAC_ADDR_FMT
		    " vdevId: %d, off channel: %d, offset: %d, "
		    "num_allowed_off_chan:%d mode: %d, is_responder: %d",
				      ...
				      struct tdls_soc_priv_obj *tdls_soc)
{
	...
	qdf_mem_copy(del_msg->bssid.bytes, wlan_peer_get_macaddr(peer),
		     QDF_MAC_ADDR_SIZE);
...
void tdls_disable_offchan_and_teardown_links(struct wlan_objmgr_vdev *vdev)
{
	...
	if (!(connected_tdls_peers || tdls_in_progress)) {
		...
		tdls_debug(
			"No TDLS connected/progress peers to delete Disable tdls for "
			"vdev id %d, "
			"FW as second interface is coming up",
			vdev_id);
