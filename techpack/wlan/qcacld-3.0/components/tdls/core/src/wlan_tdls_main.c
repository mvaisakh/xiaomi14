					      ...
					      void *arg_list)
{
	...
	status = wlan_objmgr_psoc_component_obj_detach(
		psoc, WLAN_UMAC_COMP_TDLS, tdls_soc_obj);
					     ...
					     void *arg)
{
	...
	status = ucfg_scan_register_event_handler(
		pdev, tdls_scan_complete_event_handler, tdls_soc_obj);
...
err_vdev_init:
	wlan_objmgr_vdev_component_obj_detach(vdev, WLAN_UMAC_COMP_TDLS,
					      (void *)tdls_vdev_obj);
err_attach:
					      ...
					      void *arg)
{
	...
	tdls_vdev_obj = wlan_objmgr_vdev_get_comp_private_obj(
	...
	status = wlan_objmgr_vdev_component_obj_detach(
		vdev, WLAN_UMAC_COMP_TDLS, tdls_vdev_obj);
	if (QDF_IS_STATUS_ERROR(status))
	...
	for (i = 0; i < WLAN_TDLS_PEER_LIST_SIZE; i++) {
		...
		while (QDF_IS_STATUS_SUCCESS(status)) {
			...
			if (buf_len < 32 + 1)
				break;
			len = qdf_scnprintf(
				buf, buf_len, QDF_MAC_ADDR_FMT "%4s%3s%5d\n",
 ...
 */
static void
tdls_get_all_peers_from_list(struct tdls_get_all_peers *get_tdls_peers)
{
	int32_t len;
	struct tdls_soc_priv_obj *tdls_soc_obj;
 ...
 */
static QDF_STATUS
tdls_reset_all_peers(struct tdls_delete_all_peers_params *delete_all_peers_ind)
{
	QDF_STATUS status;
...
void tdls_timer_restart(struct wlan_objmgr_vdev *vdev, qdf_mc_timer_t *timer,
			uint32_t expiration_time)
{
	if (QDF_TIMER_STATE_RUNNING != qdf_mc_timer_get_current_state(timer))
				 ...
				 struct tdls_soc_priv_obj **tdls_soc_obj)
{
	...
	*tdls_soc_obj = wlan_vdev_get_tdls_soc_obj(vdev);
	...
	if (device_mode != QDF_STA_MODE && device_mode != QDF_P2P_CLIENT_MODE)
		return QDF_STATUS_E_FAILURE;
					...
					enum supported_6g_pwr_types pwr_typ)
{
	...
	num_chan = wlan_reg_get_band_channel_list_for_pwrmode(
		pdev, band_mask, chan, REG_CLI_DEF_VLP);
...
bool tdls_check_is_tdls_allowed(struct wlan_objmgr_vdev *vdev)
{
	...
	sta_count = policy_mgr_mode_specific_connection_count(
		tdls_soc_obj->soc, PM_STA_MODE, NULL);
	p2p_cli_count = policy_mgr_mode_specific_connection_count(
		tdls_soc_obj->soc, PM_P2P_CLIENT_MODE, NULL);
...
bool tdls_is_concurrency_allowed(struct wlan_objmgr_psoc *psoc)
{
	...
	if (policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE, NULL) >
	    1) {
   ...
   */
	if (policy_mgr_get_connection_count_with_mlo(psoc) > 1 &&
	    !policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE, NULL))
		return false;
		      ...
		      struct wlan_objmgr_vdev *vdev)
{
	...
	sta_count = policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
							      NULL);
	p2p_count = policy_mgr_mode_specific_connection_count(
...
set_state:
	...
	tdls_debug("vdev:%d enable_tdls_connection_tracker %d current_mode:%d "
		   "feature_flags:0x%x",
...
tdls_process_decrement_active_session(struct wlan_objmgr_psoc *psoc)
{
	...
	if (!policy_mgr_is_hw_dbs_2x2_capable(psoc) &&
		...
		tdls_debug(
			"Current HW mode is 1*1 DBS. Wait for Opportunistic timer to "
			"expire to enable TDLS in FW");
		return QDF_STATUS_SUCCESS;
	}
   ...
   */
	tdls_debug(
		"Enable TDLS in FW and host as active sta/p2p_cli interface is present");
	...
	tdls_send_update_to_fw(
		tdls_priv_vdev, tdls_priv_soc,
		mlme_get_tdls_prohibited(tdls_obj_vdev),
		mlme_get_tdls_chan_switch_prohibited(tdls_obj_vdev), true,
		vdev_id);
						...
						wlan_objmgr_ref_dbgid dbg_id)
{
	...
	if (mlo_vdev && wlan_objmgr_vdev_try_get_ref(mlo_vdev, dbg_id) ==
				QDF_STATUS_SUCCESS)
		return mlo_vdev;
...
void tdls_send_update_to_fw(struct tdls_vdev_priv_obj *tdls_vdev_obj,
			    ...
			    bool sta_connect_event, uint8_t session_id)
{
	...
	tdls_info_to_fw->tx_discovery_threshold = threshold_params->tx_packet_n;
...
void tdls_process_enable_for_vdev(struct wlan_objmgr_vdev *vdev)
{
	...
	if (opmode == QDF_P2P_CLIENT_MODE && sta_count) {
		tdls_debug(
			"STA + P2P concurrency. Don't allow TDLS on P2P vdev");
		return;
	}
	...
	tdls_send_update_to_fw(tdls_vdev_obj, tdls_soc_obj,
			       ...
			       mlme_get_tdls_chan_switch_prohibited(vdev), true,
			       wlan_vdev_get_id(vdev));
...
tdls_process_sta_disconnect(struct tdls_sta_notify_params *notify)
{
   ...
   */
	...
	tdls_send_update_to_fw(tdls_vdev_obj, tdls_soc_obj, false, false, false,
			       notify->session_id);
   ...
   */
	...
	status =
		tdls_get_vdev_objects(temp_vdev, &tdls_vdev_obj, &tdls_soc_obj);
	if (QDF_IS_STATUS_ERROR(status)) {
...
void tdls_notify_reset_adapter(struct wlan_objmgr_vdev *vdev)
{
	...
	if (QDF_STATUS_SUCCESS !=
	    wlan_objmgr_vdev_try_get_ref(vdev, WLAN_TDLS_NB_ID))
		return;
					   ...
					   uint8_t vdev_id)
{
	...
	if (!vdev) {
		tdls_err("vdev not exist for the vdev id %d", vdev_id);
		qdf_mem_free(notify);
		return QDF_STATUS_E_INVAL;
	}
...
static QDF_STATUS
tdls_delete_all_peers_indication(struct wlan_objmgr_psoc *psoc, uint8_t vdev_id)
{
	struct scheduler_msg msg = {
		0,
	};
					 ...
					 uint8_t vdev_id)
{
	...
	uint32_t pdev_id;
	enum QDF_OPMODE opmode;
	uint8_t sta_count = policy_mgr_mode_specific_connection_count(
				  ...
				  enum tdls_disable_sources source)
{
     ...
     */
		...
		set_bit((unsigned long)source, &tdls_soc->tdls_source_bitmap);
				  ...
				  enum tdls_disable_sources source)
{
	...
	vdev = wlan_objmgr_get_vdev_by_opmode_from_psoc(
		tdls_soc->soc, QDF_STA_MODE, WLAN_TDLS_NB_ID);
	if (vdev) {
		tdls_debug("set mode in tdls vdev ");
		tdls_vdev = wlan_vdev_get_tdls_vdev_obj(vdev);
		if (tdls_vdev)
			tdls_set_mode_in_vdev(tdls_vdev, tdls_soc, tdls_mode,
					      source);
		wlan_objmgr_vdev_release_ref(vdev, WLAN_TDLS_NB_ID);
	}
	...
	/* get p2p client vdev */
	vdev = wlan_objmgr_get_vdev_by_opmode_from_psoc(
		tdls_soc->soc, QDF_P2P_CLIENT_MODE, WLAN_TDLS_NB_ID);
	if (vdev) {
		tdls_debug("set mode in tdls vdev ");
		tdls_vdev = wlan_vdev_get_tdls_vdev_obj(vdev);
		if (tdls_vdev)
...
QDF_STATUS tdls_set_operation_mode(struct tdls_set_mode_params *tdls_set_mode)
{
	...
	status = tdls_get_vdev_objects(tdls_set_mode->vdev, &tdls_vdev,
				       &tdls_soc);

	if (QDF_IS_STATUS_ERROR(status))
		goto release_mode_ref;

	tdls_set_current_mode(tdls_soc, tdls_set_mode->tdls_mode,
...
void tdls_scan_done_callback(struct tdls_soc_priv_obj *tdls_soc)
{
	...
	if (TDLS_SUPPORT_IMP_MODE == tdls_soc->tdls_last_mode ||
		...
		tdls_set_current_mode(tdls_soc, tdls_soc->tdls_last_mode, false,
...
static QDF_STATUS tdls_post_scan_done_msg(struct tdls_soc_priv_obj *tdls_soc)
{
	...
	return scheduler_post_message(QDF_MODULE_ID_TDLS, QDF_MODULE_ID_TDLS,
				      QDF_MODULE_ID_OS_IF, &msg);
}
...
QDF_STATUS tdls_scan_callback(struct tdls_soc_priv_obj *tdls_soc)
{
	...
	if (tdls_is_progress(tdls_vdev, NULL, 0)) {
		if (tdls_soc->scan_reject_count++ >= TDLS_SCAN_REJECT_MAX) {
			tdls_notice(
				"Allow this scan req. as already max no of scan's are rejected");
	...
	if (TDLS_IS_SCAN_ENABLED(feature)) {
		tdls_debug(
			"TDLS Scan enabled, keep tdls link and allow scan, connected "
			"tdls peers: %d",
			tdls_peer_count);
		goto disable_tdls;
	}
	...
	if (TDLS_IS_BUFFER_STA_ENABLED(feature) &&
		...
		if (peer_buf_capable) {
			tdls_debug(
				"All peers (num %d) bufSTAs, we can be sleep sta, so allow "
				"scan, tdls mode changed to %d",
				tdls_peer_count, tdls_soc->tdls_current_mode);
			goto disable_tdls;
		}
	}
...
disable_tdls:
	tdls_set_current_mode(tdls_soc, TDLS_SUPPORT_DISABLED, false,
			      TDLS_SET_MODE_SOURCE_SCAN);
					...
					uint8_t *reg_bw_offset)
{
	...
	if (bw_offset & (1 << BW_160_OFFSET_BIT)) {
		opclass = tdls_find_opclass_frm_freq(vdev, freq, BW_160_MHZ,
						     BEHAV_NONE);
		*reg_bw_offset = BWALL;
	} else if (bw_offset & (1 << BW_80_OFFSET_BIT)) {
		opclass = tdls_find_opclass_frm_freq(vdev, freq, BW_80_MHZ,
						     BEHAV_NONE);
		*reg_bw_offset = BW80;
	} else if (bw_offset & (1 << BW_40_OFFSET_BIT)) {
		opclass = tdls_find_opclass_frm_freq(vdev, freq, BW_40_MHZ,
						     BEHAV_BW40_LOW_PRIMARY);
		*reg_bw_offset = BW40_LOW_PRIMARY;
		if (!opclass) {
			opclass = tdls_find_opclass_frm_freq(
				vdev, freq, BW_40_MHZ, BEHAV_BW40_HIGH_PRIMARY);
			*reg_bw_offset = BW40_HIGH_PRIMARY;
		}
	} else if (bw_offset & (1 << BW_20_OFFSET_BIT)) {
		opclass = tdls_find_opclass_frm_freq(vdev, freq, BW_20_MHZ,
						     BEHAV_NONE);
		*reg_bw_offset = BW20;
	} else {
		opclass = tdls_find_opclass_frm_freq(vdev, freq, BW_160_MHZ,
