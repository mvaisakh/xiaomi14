				  ...
				  struct pmo_mc_addr_list_params *ip)
{
	...
	int i, j = 0;
	static const uint8_t ipv6_rs[] = { 0x33, 0x33, 0x00, 0x00, 0x00, 0x02 };
     ...
     */
		...
		qdf_spin_lock_bh(&temp_ctx->pmo_vdev_lock);
		qdf_mem_zero(&op_list->mc_addr[j].bytes, QDF_MAC_ADDR_SIZE);
		qdf_mem_copy(&op_list->mc_addr[j].bytes, ip->mc_addr[i].bytes,
			     QDF_MAC_ADDR_SIZE);
		qdf_spin_unlock_bh(&temp_ctx->pmo_vdev_lock);
...
static QDF_STATUS
pmo_core_flush_mc_addr_list_from_vdev_priv(struct wlan_objmgr_vdev *vdev)
{
	struct pmo_vdev_priv_obj *vdev_ctx;
...
static QDF_STATUS
pmo_core_do_enable_mc_addr_list(struct wlan_objmgr_vdev *vdev,
				struct pmo_vdev_priv_obj *vdev_ctx,
...
static QDF_STATUS
pmo_core_do_disable_mc_addr_list(struct wlan_objmgr_vdev *vdev,
				 struct pmo_vdev_priv_obj *vdev_ctx,
				 struct pmo_mc_addr_list *op_mc_list_req)
{
	QDF_STATUS status;
...
static QDF_STATUS
pmo_core_mc_addr_flitering_sanity(struct wlan_objmgr_vdev *vdev)
{
	struct pmo_vdev_priv_obj *vdev_ctx;
...
QDF_STATUS
pmo_core_cache_mc_addr_list(struct pmo_mc_addr_list_params *mc_list_config)
{
	struct wlan_objmgr_vdev *vdev;
	QDF_STATUS status;
	...
	vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		mc_list_config->psoc, mc_list_config->vdev_id, WLAN_PMO_ID);
	if (!vdev) {
		pmo_err("vdev is NULL");
		status = QDF_STATUS_E_NULL_VALUE;
		goto out;
				       ...
				       uint8_t vdev_id)
{
	...
	pmo_debug("Flush mc addr list for vdev id: %d psoc: %pK", vdev_id,
		  psoc);
...
static QDF_STATUS
pmo_core_handle_enable_mc_list_trigger(struct wlan_objmgr_vdev *vdev,
...
QDF_STATUS
pmo_core_enable_mc_addr_filtering_in_fwr(struct wlan_objmgr_psoc *psoc,
					 uint8_t vdev_id,
					 enum pmo_offload_trigger trigger)
{
	QDF_STATUS status;
...
static QDF_STATUS
pmo_core_handle_disable_mc_list_trigger(struct wlan_objmgr_vdev *vdev,
					enum pmo_offload_trigger trigger)
{
	QDF_STATUS status = QDF_STATUS_SUCCESS;
	struct pmo_vdev_priv_obj *vdev_ctx;
...
QDF_STATUS
pmo_core_disable_mc_addr_filtering_in_fwr(struct wlan_objmgr_psoc *psoc,
					  uint8_t vdev_id,
					  enum pmo_offload_trigger trigger)
{
	QDF_STATUS status;
...
QDF_STATUS
pmo_core_get_mc_addr_list(struct wlan_objmgr_psoc *psoc, uint8_t vdev_id,
