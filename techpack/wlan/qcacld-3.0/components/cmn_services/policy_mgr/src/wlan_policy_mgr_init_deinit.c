						...
						void *data)
{
	...
	policy_mgr_ctx =
		qdf_mem_malloc(sizeof(struct policy_mgr_psoc_priv_obj));
	if (!policy_mgr_ctx)
	...
	wlan_objmgr_psoc_component_obj_attach(psoc, WLAN_UMAC_COMP_POLICY_MGR,
					      policy_mgr_ctx,
					      QDF_STATUS_SUCCESS);
						 ...
						 void *data)
{
	...
	wlan_objmgr_psoc_component_obj_detach(psoc, WLAN_UMAC_COMP_POLICY_MGR,
					      policy_mgr_ctx);
						...
						void *data)
{
	...
	wlan_reg_register_chan_change_callback(
		psoc, policy_mgr_reg_chan_change_callback, NULL);
						 ...
						 void *data)
{
	...
	wlan_reg_unregister_chan_change_callback(
		psoc, policy_mgr_reg_chan_change_callback);
...
static QDF_STATUS policy_mgr_register_link_switch_notifier(void)
{
	...
	status = mlo_mgr_register_link_switch_notifier(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_link_switch_notifier_cb);
	if (status == QDF_STATUS_E_NOSUPPORT) {
		status = QDF_STATUS_SUCCESS;
		policy_mgr_debug("Link switch not supported");
	} else if (status != QDF_STATUS_SUCCESS) {
		policy_mgr_err(
			"Failed to register link switch notifier for policy mgr!");
	}
...
static QDF_STATUS policy_mgr_unregister_link_switch_notifier(void)
{
	...
	if (status == QDF_STATUS_E_NOSUPPORT)
		...
		policy_mgr_err(
			"Failed to unregister link switch notifier for policy mgr!");
...
QDF_STATUS policy_mgr_init(void)
{
	...
	status = wlan_objmgr_register_psoc_create_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_psoc_obj_create_cb, NULL);
	...
	status = wlan_objmgr_register_psoc_destroy_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_psoc_obj_destroy_cb,
		NULL);
	if (status != QDF_STATUS_SUCCESS) {
	...
	status = wlan_objmgr_register_psoc_status_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_psoc_obj_status_cb, NULL);
	if (status != QDF_STATUS_SUCCESS) {
	...
	status = wlan_objmgr_register_pdev_create_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_pdev_obj_create_cb, NULL);
	if (status != QDF_STATUS_SUCCESS) {
	...
	status = wlan_objmgr_register_pdev_destroy_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_pdev_obj_destroy_cb,
		NULL);
	if (status != QDF_STATUS_SUCCESS) {
	...
	status = wlan_objmgr_register_vdev_create_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_vdev_obj_create_cb, NULL);
	if (status != QDF_STATUS_SUCCESS) {
	...
	status = wlan_objmgr_register_vdev_destroy_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_vdev_obj_destroy_cb,
		NULL);
	if (status != QDF_STATUS_SUCCESS) {
	...
	status = wlan_objmgr_register_vdev_status_handler(
		WLAN_UMAC_COMP_POLICY_MGR, policy_mgr_vdev_obj_status_cb, NULL);
	if (status != QDF_STATUS_SUCCESS) {
...
QDF_STATUS policy_mgr_psoc_open(struct wlan_objmgr_psoc *psoc)
{
	...
	if (!QDF_IS_STATUS_SUCCESS(
		    qdf_mutex_create(&pm_ctx->qdf_conc_list_lock))) {
	...
	pm_ctx->sta_ap_intf_check_work_info =
		qdf_mem_malloc(sizeof(struct sta_ap_intf_check_work_ctx));
	...
	if (QDF_IS_STATUS_ERROR(qdf_delayed_work_create(
		    ...
		    policy_mgr_check_sta_ap_concurrent_ch_intf, pm_ctx))) {
...
static void policy_mgr_init_non_dbs_pcl(struct wlan_objmgr_psoc *psoc)
{
	second_connection_pcl_non_dbs_table =
		&second_connection_pcl_nodbs_table;
	third_connection_pcl_non_dbs_table = &third_connection_pcl_nodbs_table;
}
#endif
...
policy_mgr_init_ml_link_update(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	...
	if (QDF_IS_STATUS_ERROR(qdf_status)) {
		policy_mgr_err(
			"init event failed for for set_link_update_done_evt");
		return QDF_STATUS_E_FAILURE;
	}
...
policy_mgr_deinit_ml_link_update(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	...
	if (QDF_IS_STATUS_ERROR(qdf_status)) {
		policy_mgr_err(
			"deinit event failed for set_link_update_done_evt");
		return QDF_STATUS_E_FAILURE;
	}
...
QDF_STATUS policy_mgr_psoc_enable(struct wlan_objmgr_psoc *psoc)
{
	...
	policy_mgr_debug("is DBS Capable %d, is SBS Capable %d",
			 ...
			 policy_mgr_is_hw_sbs_capable(psoc));
	policy_mgr_debug(
		"is2x2 %d, 2g-on-dbs %d is2x2+1x1 %d, is2x2_5g+1x1_2g %d, "
		"is2x2_2g+1x1_5g %d",
...
QDF_STATUS policy_mgr_psoc_disable(struct wlan_objmgr_psoc *psoc)
{
	...
	if (!QDF_IS_STATUS_SUCCESS(
		    qdf_event_destroy(&pm_ctx->connection_update_done_evt))) {
	...
	if (!QDF_IS_STATUS_SUCCESS(qdf_event_destroy(
		...
		policy_mgr_err(
			"Failed to destroy opportunistic_update_done_evt");
		status = QDF_STATUS_E_FAILURE;
		QDF_ASSERT(0);
	}
	/* destroy channel_switch_complete_evt */
	if (!QDF_IS_STATUS_SUCCESS(
		    qdf_event_destroy(&pm_ctx->channel_switch_complete_evt))) {
	...
	if (QDF_TIMER_STATE_RUNNING ==
	    qdf_mc_timer_get_current_state(&pm_ctx->dbs_opportunistic_timer)) {
		qdf_mc_timer_stop(&pm_ctx->dbs_opportunistic_timer);
	}

	if (!QDF_IS_STATUS_SUCCESS(
		    qdf_mc_timer_destroy(&pm_ctx->dbs_opportunistic_timer))) {
...
QDF_STATUS
policy_mgr_register_conc_cb(struct wlan_objmgr_psoc *psoc,
...
QDF_STATUS
policy_mgr_register_sme_cb(struct wlan_objmgr_psoc *psoc,
			   struct policy_mgr_sme_cbacks *sme_cbacks)
{
	...
	pm_ctx->sme_cbacks.sme_rso_start_cb = sme_cbacks->sme_rso_start_cb;
	pm_ctx->sme_cbacks.sme_rso_stop_cb = sme_cbacks->sme_rso_stop_cb;
 ...
 */
QDF_STATUS
policy_mgr_register_hdd_cb(struct wlan_objmgr_psoc *psoc,
...
QDF_STATUS
policy_mgr_register_wma_cb(struct wlan_objmgr_psoc *psoc,
...
QDF_STATUS
policy_mgr_register_cdp_cb(struct wlan_objmgr_psoc *psoc,
			   struct policy_mgr_cdp_cbacks *cdp_cbacks)
{
	...
	pm_ctx->cdp_cbacks.cdp_update_mac_id = cdp_cbacks->cdp_update_mac_id;
...
QDF_STATUS
policy_mgr_register_tdls_cb(struct wlan_objmgr_psoc *psoc,
...
QDF_STATUS
policy_mgr_register_mode_change_cb(struct wlan_objmgr_psoc *psoc,
