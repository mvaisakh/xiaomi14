 ...
 */
const enum policy_mgr_pcl_type first_connection_pcl_table
...
pm_dbs_pcl_second_connection_table_type *second_connection_pcl_dbs_table;

enum policy_mgr_pcl_type const (
	*second_connection_pcl_non_dbs_table)[PM_MAX_ONE_CONNECTION_MODE]
					     [PM_MAX_NUM_OF_MODE]
					     [PM_MAX_CONC_PRIORITY_MODE];
pm_dbs_pcl_third_connection_table_type *third_connection_pcl_dbs_table;
enum policy_mgr_pcl_type const (
...
QDF_STATUS policy_mgr_get_pcl_for_existing_conn(
	...
	uint32_t *pcl_ch, uint32_t *len, uint8_t *pcl_weight,
	uint32_t weight_len, bool all_matching_cxn_to_del, uint8_t vdev_id)
{
	...
	*len = 0;
	...
	if (policy_mgr_mode_specific_connection_count(psoc, mode, NULL) > 0) {
		...
		policy_mgr_store_and_del_conn_info(psoc, mode,
						   ...
						   info, &num_cxn_del);
		/* Get the PCL */
		status = policy_mgr_get_pcl(psoc, mode, pcl_ch, len, pcl_weight,
					    weight_len, vdev_id);
 ...
 */
static uint32_t policy_mgr_get_pcl_concurrent_connetions(
	struct wlan_objmgr_psoc *psoc, enum policy_mgr_con_mode mode,
	uint8_t vdev_id, uint8_t *vdev_ids, uint32_t vdev_ids_size)
{
	...
	for (i = 0; i < num_non_ml + num_ml && num_related < vdev_ids_size;
	     i++) {
	...
	for (i = 0; i < num_ml && num_related < vdev_ids_size; i++) {
		...
		if (is_ml_sta) {
			policy_mgr_debug("vdev_ids[%d]: %d", num_related,
					 ml_vdev_id);
...
#else
static inline uint32_t policy_mgr_get_pcl_concurrent_connetions(
	struct wlan_objmgr_psoc *psoc, enum policy_mgr_con_mode mode,
	uint8_t vdev_id, uint8_t *vdev_ids, uint32_t vdev_ids_size)
{
	if (!vdev_ids || !vdev_ids_size) {
...
QDF_STATUS policy_mgr_get_pcl_for_vdev_id(struct wlan_objmgr_psoc *psoc,
					  ...
					  uint8_t *pcl_weight,
					  uint32_t weight_len, uint8_t vdev_id)
{
	struct policy_mgr_conc_connection_info
		info[MAX_NUMBER_OF_CONC_CONNECTIONS] = { { 0 } };
	QDF_STATUS status = QDF_STATUS_SUCCESS;
	...
	policy_mgr_debug("get pcl for existing conn:%d vdev id %d", mode,
			 vdev_id);
	pm_ctx = policy_mgr_get_context(psoc);
	if (!pm_ctx) {
		policy_mgr_err("Invalid Context");
	...
	qdf_mutex_acquire(&pm_ctx->qdf_conc_list_lock);
	id_num = policy_mgr_get_pcl_concurrent_connetions(
		psoc, mode, vdev_id, ids, QDF_ARRAY_SIZE(ids));
					...
					uint32_t weight_len, uint8_t vdev_id)
{
	...
	policy_mgr_store_and_del_conn_info_by_chan_and_mode(
		psoc, vdev_freq, mode, info, &num_del);
	status = policy_mgr_get_pcl(psoc, mode, pcl_ch, len, pcl_weight,
				    weight_len, vdev_id);
	if (num_del > 0)
...
void polic_mgr_send_pcl_to_fw(struct wlan_objmgr_psoc *psoc,
			      enum QDF_OPMODE mode)
{
	...
	for (conn_idx = 0; conn_idx < MAX_NUMBER_OF_CONC_CONNECTIONS;
	     conn_idx++) {
		qdf_mutex_acquire(&pm_ctx->qdf_conc_list_lock);
		if (!(pm_conc_connection_list[conn_idx].mode == PM_STA_MODE &&
     ...
     */
		if (mode == QDF_SAP_MODE &&
		    wlan_cm_roaming_in_progress(pm_ctx->pdev, vdev_id)) {
			policy_mgr_debug(
				"Roaming is in progress, don't stop RSO for vdev_id: %d",
				vdev_id);
			continue;
		}

		pm_ctx->sme_cbacks.sme_rso_stop_cb(mac_handle, vdev_id,
						   REASON_DRIVER_DISABLED,
						   RSO_SET_PCL);

		policy_mgr_set_pcl_for_existing_combo(pm_ctx->psoc, PM_STA_MODE,
						      vdev_id);
		pm_ctx->sme_cbacks.sme_rso_start_cb(mac_handle, vdev_id,
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		...
		if (is_client)
			state = wlan_reg_get_channel_state_for_pwrmode(
				pm_ctx->pdev, ch_freq, REG_CURRENT_PWR_MODE);
		else
			state = wlan_reg_get_channel_state_from_secondary_list_for_freq(
				pm_ctx->pdev, ch_freq);
...
void policy_mgr_set_freq_restriction_mask(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
...
void policy_mgr_reg_chan_change_callback(
	struct wlan_objmgr_psoc *psoc, struct wlan_objmgr_pdev *pdev,
	struct regulatory_channel *chan_list,
	struct avoid_freq_ind_data *avoid_freq_ind, void *arg)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t i;
	struct ch_avoid_ind_type *freq_list;
   ...
   */
	pm_ctx->unsafe_channel_count =
		avoid_freq_ind->chan_list.chan_cnt >= NUM_CHANNELS ?
			NUM_CHANNELS :
			avoid_freq_ind->chan_list.chan_cnt;
					  ...
					  uint16_t chan_cnt)
{
	...
	pm_ctx->unsafe_channel_count = chan_cnt >= NUM_CHANNELS ? NUM_CHANNELS :
								  chan_cnt;
					      ...
					      uint32_t weight_len)
{
	...
	nan_2g_freq = policy_mgr_mode_specific_get_channel(pm_ctx->psoc,
							   PM_NAN_DISC_MODE);
	...
	for (i = 0; i < current_channel_count; i++) {
		...
		if (is_unsafe && scc_on_lte_coex &&
			...
			policy_mgr_debug(
				"CH %d unsafe ignored when STA present on it",
				current_channel_list[i]);
			is_unsafe = 0;
		} else if (is_unsafe &&
...
static QDF_STATUS policy_mgr_modify_go_pcl_based_on_indoor(
	...
	uint8_t *weight_list_org, uint32_t *pcl_len_org)
{
	...
	if (include_indoor_channel &&
	    ...
	    wlan_reg_is_freq_indoor_in_secondary_list(
		    pm_ctx->pdev, pm_conc_connection_list[0].freq)) {
				...
				uint32_t *vdev_id)
{
	...
	for (idx = 0; idx < MAX_NUMBER_OF_CONC_CONNECTIONS; idx++) {
		...
		if ((pm_conc_connection_list[idx].mode == mode) &&
		    (!vdev_id ||
		     (*vdev_id == pm_conc_connection_list[idx].vdev_id)) &&
	...
	*skip_dfs_channel = false;
	...
	if (policy_mgr_is_hw_dbs_capable(psoc)) {
		...
		if ((policy_mgr_mode_specific_connection_count(
		    ...
		    !sta_sap_scc_on_dfs_chan) {
			policy_mgr_debug(
				"skip DFS ch from pcl for non-DBS SAP/Go");
			*skip_dfs_channel = true;
		}
	}
...
static QDF_STATUS policy_mgr_modify_pcl_based_on_srd(
	struct wlan_objmgr_psoc *psoc, uint32_t *pcl_list_org,
	uint8_t *weight_list_org, uint32_t *pcl_len_org)
{
	uint32_t i, pcl_len = 0;
	...
	for (i = 0; i < *pcl_len_org; i++) {
		if (wlan_reg_is_etsi13_srd_chan_for_freq(pm_ctx->pdev,
							 pcl_list_org[i]))
			continue;
		pcl_list[pcl_len] = pcl_list_org[i];
		weight_list[pcl_len++] = weight_list_org[i];
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS policy_mgr_modify_pcl_based_on_indoor(
	struct wlan_objmgr_psoc *psoc, uint32_t *pcl_list_org,
	uint8_t *weight_list_org, uint32_t *pcl_len_org)
{
	uint32_t i, pcl_len = 0;
   ...
   */
	...
	if (include_indoor_channel) {
		policy_mgr_debug(
			"Indoor channels allowed. PCL not modified for indoor channels");
		return QDF_STATUS_SUCCESS;
	}
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS policy_mgr_modify_sap_pcl_for_6G_channels(
	struct wlan_objmgr_psoc *psoc, uint32_t *pcl_list_org,
	uint8_t *weight_list_org, uint32_t *pcl_len_org)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
static QDF_STATUS policy_mgr_modify_sap_go_4th_conc_disallow(
	struct wlan_objmgr_psoc *psoc, enum policy_mgr_con_mode mode,
	uint32_t *pcl_list_org, uint8_t *weight_list_org, uint32_t *pcl_len_org)
{
	size_t i, pcl_len = 0;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	for (i = 0; i < *pcl_len_org; i++) {
		if (policy_mgr_allow_4th_new_freq(psoc, pcl_list_org[i], mode,
						  0)) {
			pcl_list_org[pcl_len] = pcl_list_org[i];
			weight_list_org[pcl_len++] = weight_list_org[i];
		}
...
static QDF_STATUS
policy_mgr_pcl_modification_for_sap(struct wlan_objmgr_psoc *psoc,
				    ...
				    uint32_t *len, uint32_t weight_len)
{
	...
	status = policy_mgr_modify_sap_pcl_based_on_nol(psoc, pcl_channels,
							pcl_weight, len);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = policy_mgr_modify_sap_pcl_based_on_dfs(psoc, pcl_channels,
							pcl_weight, len);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	if (!srd_chan_enabled) {
		status = policy_mgr_modify_pcl_based_on_srd(psoc, pcl_channels,
							    pcl_weight, len);
		if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = policy_mgr_filter_passive_ch(pm_ctx->pdev, pcl_channels, len);
	...
	status = policy_mgr_modify_sap_pcl_for_6G_channels(psoc, pcl_channels,
							   pcl_weight, len);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = policy_mgr_modify_sap_go_4th_conc_disallow(
	...
	policy_mgr_debug("%d %d %d %d %d %d %d", mandatory_modified_pcl,
			 nol_modified_pcl, dfs_modified_pcl,
			 indoor_modified_pcl, passive_modified_pcl,
			 modified_final_pcl, band_6ghz_modified_pcl);
...
static QDF_STATUS policy_mgr_pcl_modification_for_p2p_go(
	...
	uint8_t *pcl_weight, uint32_t *len, uint32_t weight_len)
{
	QDF_STATUS status = QDF_STATUS_E_FAILURE;
	bool srd_chan_enabled;
	...
	uint8_t *pcl_weight, uint32_t *len, uint32_t weight_len)
{
	...
	policy_mgr_pcl_modification_for_sap(psoc, pcl_channels, pcl_weight, len,
					    weight_len);

	for (i = 0; i < *len; i++) {
		/* Remove passive/dfs/6G invalid channel for LL_LT_SAP */
		if (wlan_reg_is_passive_for_freq(pm_ctx->pdev,
						 pcl_channels[i]) ||
...
#else
static inline QDF_STATUS policy_mgr_pcl_modification_for_ll_lt_sap(
	struct wlan_objmgr_psoc *psoc, uint32_t *pcl_channels,
	uint8_t *pcl_weight, uint32_t *len, uint32_t weight_len)
{
	return QDF_STATUS_SUCCESS;
...
policy_mgr_get_pcl_4_port(struct wlan_objmgr_psoc *psoc,
			  enum policy_mgr_con_mode mode,
			  enum policy_mgr_conc_priority_mode pref)
{
   ...
   */
	if (!policy_mgr_is_hw_dbs_capable(psoc)) {
		policy_mgr_err(
			"Can't find index for 4th port pcl table for non dbs capable");
	...
	if (mode != PM_STA_MODE && mode != PM_SAP_MODE && mode != PM_NDI_MODE) {
			      ...
			      uint8_t vdev_id)
{
	...
	case 1:
		...
		if (PM_MAX_ONE_CONNECTION_MODE == second_index) {
			policy_mgr_err(
				"couldn't find index for 2nd connection pcl table");
			return status;
		}
		...
		if (policy_mgr_is_hw_dbs_capable(psoc) == true &&
		    policy_mgr_is_dbs_allowed_for_concurrency(psoc, qdf_mode)) {
...
policy_mgr_get_first_connection_pcl_table_index(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
policy_mgr_get_second_connection_pcl_table_index(struct wlan_objmgr_psoc *psoc)
{
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_P2P_CLI_SAP_SCC_24_1x1, PM_P2P_CLI_SAP_SCC_24_2x2,
		PM_P2P_CLI_SAP_SCC_5_1x1, PM_P2P_CLI_SAP_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_P2P_CLI_SAP_MCC_24_1x1, PM_P2P_CLI_SAP_MCC_24_2x2,
		PM_P2P_CLI_SAP_MCC_5_1x1, PM_P2P_CLI_SAP_MCC_5_2x2,
		PM_P2P_CLI_SAP_MCC_24_5_1x1, PM_P2P_CLI_SAP_MCC_24_5_2x2);

	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_P2P_CLI_SAP_SBS_5_1x1, PM_P2P_CLI_SAP_SBS_5_2x2,
		PM_P2P_CLI_SAP_DBS_1x1, PM_P2P_CLI_SAP_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_STA_SAP_SCC_24_1x1, PM_STA_SAP_SCC_24_2x2,
		PM_STA_SAP_SCC_5_1x1, PM_STA_SAP_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_STA_SAP_MCC_24_1x1, PM_STA_SAP_MCC_24_2x2,
		PM_STA_SAP_MCC_5_1x1, PM_STA_SAP_MCC_5_2x2,
		PM_STA_SAP_MCC_24_5_1x1, PM_STA_SAP_MCC_24_5_2x2);

	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_STA_SAP_SBS_5_1x1, PM_STA_SAP_SBS_5_2x2,
		PM_STA_SAP_DBS_1x1, PM_STA_SAP_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_SAP_SAP_SCC_24_1x1, PM_SAP_SAP_SCC_24_2x2,
		PM_SAP_SAP_SCC_5_1x1, PM_SAP_SAP_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_SAP_SAP_MCC_24_1x1, PM_SAP_SAP_MCC_24_2x2,
		PM_SAP_SAP_MCC_5_1x1, PM_SAP_SAP_MCC_5_2x2,
		PM_SAP_SAP_MCC_24_5_1x1, PM_SAP_SAP_MCC_24_5_2x2);

	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_SAP_SAP_SBS_5_1x1, PM_SAP_SAP_SBS_5_2x2,
		PM_SAP_SAP_DBS_1x1, PM_SAP_SAP_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_STA_P2P_GO_SCC_24_1x1, PM_STA_P2P_GO_SCC_24_2x2,
		PM_STA_P2P_GO_SCC_5_1x1, PM_STA_P2P_GO_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_STA_P2P_GO_MCC_24_1x1, PM_STA_P2P_GO_MCC_24_2x2,
		PM_STA_P2P_GO_MCC_5_1x1, PM_STA_P2P_GO_MCC_5_2x2,
		PM_STA_P2P_GO_MCC_24_5_1x1, PM_STA_P2P_GO_MCC_24_5_2x2);

	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_STA_P2P_GO_SBS_5_1x1, PM_STA_P2P_GO_SBS_5_2x2,
		PM_STA_P2P_GO_DBS_1x1, PM_STA_P2P_GO_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_STA_P2P_CLI_SCC_24_1x1, PM_STA_P2P_CLI_SCC_24_2x2,
		PM_STA_P2P_CLI_SCC_5_1x1, PM_STA_P2P_CLI_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_STA_P2P_CLI_MCC_24_1x1, PM_STA_P2P_CLI_MCC_24_2x2,
		PM_STA_P2P_CLI_MCC_5_1x1, PM_STA_P2P_CLI_MCC_5_2x2,
		PM_STA_P2P_CLI_MCC_24_5_1x1, PM_STA_P2P_CLI_MCC_24_5_2x2);

	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_STA_P2P_CLI_SBS_5_1x1, PM_STA_P2P_CLI_SBS_5_2x2,
		PM_STA_P2P_CLI_DBS_1x1, PM_STA_P2P_CLI_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_P2P_GO_P2P_CLI_SCC_24_1x1, PM_P2P_GO_P2P_CLI_SCC_24_2x2,
		PM_P2P_GO_P2P_CLI_SCC_5_1x1, PM_P2P_GO_P2P_CLI_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_P2P_GO_P2P_CLI_MCC_24_1x1,
		PM_P2P_GO_P2P_CLI_MCC_24_2x2, PM_P2P_GO_P2P_CLI_MCC_5_1x1,
		PM_P2P_GO_P2P_CLI_MCC_5_2x2, PM_P2P_GO_P2P_CLI_MCC_24_5_1x1,
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_P2P_GO_P2P_CLI_SBS_5_1x1, PM_P2P_GO_P2P_CLI_SBS_5_2x2,
		PM_P2P_GO_P2P_CLI_DBS_1x1, PM_P2P_GO_P2P_CLI_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_P2P_GO_SAP_SCC_24_1x1, PM_P2P_GO_SAP_SCC_24_2x2,
		PM_P2P_GO_SAP_SCC_5_1x1, PM_P2P_GO_SAP_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_P2P_GO_SAP_MCC_24_1x1, PM_P2P_GO_SAP_MCC_24_2x2,
		PM_P2P_GO_SAP_MCC_5_1x1, PM_P2P_GO_SAP_MCC_5_2x2,
		PM_P2P_GO_SAP_MCC_24_5_1x1, PM_P2P_GO_SAP_MCC_24_5_2x2);

	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_P2P_GO_SAP_SBS_5_1x1, PM_P2P_GO_SAP_SBS_5_2x2,
		PM_P2P_GO_SAP_DBS_1x1, PM_P2P_GO_SAP_DBS_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	if (policy_mgr_is_ml_vdev_id(psoc,
		...
		index = policy_mgr_get_3rd_pcl_table_index_for_dbs_with_ml_sta(
			psoc, PM_STA_STA_SBS_5_1x1, PM_STA_STA_SBS_5_2x2,
			PM_STA_STA_DBS_1x1, PM_STA_STA_DBS_2x2);
		if (index != PM_MAX_TWO_CONNECTION_MODE)
			return index;
	}
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_STA_STA_SCC_24_1x1, PM_STA_STA_SCC_24_2x2,
		PM_STA_STA_SCC_5_1x1, PM_STA_STA_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_STA_STA_MCC_24_1x1, PM_STA_STA_MCC_24_2x2,
		PM_STA_STA_MCC_5_1x1, PM_STA_STA_MCC_5_2x2,
		PM_STA_STA_MCC_24_5_1x1, PM_STA_STA_MCC_24_5_2x2);
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_P2P_CLI_P2P_CLI_SCC_24_1x1, PM_P2P_CLI_P2P_CLI_SCC_24_2x2,
		PM_P2P_CLI_P2P_CLI_SCC_5_1x1, PM_P2P_CLI_P2P_CLI_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_P2P_CLI_P2P_CLI_MCC_24_1x1,
		PM_P2P_CLI_P2P_CLI_MCC_24_2x2, PM_P2P_CLI_P2P_CLI_MCC_5_1x1,
		PM_P2P_CLI_P2P_CLI_MCC_5_2x2, PM_P2P_CLI_P2P_CLI_MCC_24_5_1x1,
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_P2P_CLI_P2P_CLI_SBS_5_1x1,
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_scc(
		PM_P2P_GO_P2P_GO_SCC_24_1x1, PM_P2P_GO_P2P_GO_SCC_24_2x2,
		PM_P2P_GO_P2P_GO_SCC_5_1x1, PM_P2P_GO_P2P_GO_SCC_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_mcc(
		psoc, PM_P2P_GO_P2P_GO_MCC_24_1x1, PM_P2P_GO_P2P_GO_MCC_24_2x2,
		PM_P2P_GO_P2P_GO_MCC_5_1x1, PM_P2P_GO_P2P_GO_MCC_5_2x2,
		PM_P2P_GO_P2P_GO_MCC_24_5_1x1, PM_P2P_GO_P2P_GO_MCC_24_5_2x2);
	if (index != PM_MAX_TWO_CONNECTION_MODE)
		return index;

	index = policy_mgr_check_and_get_third_connection_pcl_table_index_for_dbs(
		psoc, PM_P2P_GO_P2P_GO_SBS_5_1x1, PM_P2P_GO_P2P_GO_SBS_5_2x2,
		PM_P2P_GO_P2P_GO_DBS_1x1, PM_P2P_GO_P2P_GO_DBS_2x2);
	return index;
}
...
policy_mgr_get_third_connection_pcl_table_index(struct wlan_objmgr_psoc *psoc)
{
	enum policy_mgr_two_connection_mode index = PM_MAX_TWO_CONNECTION_MODE;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	if (((PM_P2P_CLIENT_MODE == pm_conc_connection_list[0].mode) &&
		...
		index = policy_mgr_get_third_connection_pcl_table_index_cli_sap(
			psoc);
	else if (((PM_STA_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_SAP_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_SAP_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_STA_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_sta_sap(
			psoc);
	else if ((PM_SAP_MODE == pm_conc_connection_list[0].mode) &&
		 (PM_SAP_MODE == pm_conc_connection_list[1].mode))
		index = policy_mgr_get_third_connection_pcl_table_index_sap_sap(
			psoc);
	else if (((PM_STA_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_P2P_GO_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_P2P_GO_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_STA_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_sta_go(
			psoc);
	else if (((PM_STA_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_P2P_CLIENT_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_P2P_CLIENT_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_STA_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_sta_cli(
			psoc);
	else if (((PM_P2P_GO_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_P2P_CLIENT_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_P2P_CLIENT_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_P2P_GO_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_go_cli(
			psoc);
	else if (((PM_SAP_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_P2P_GO_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_P2P_GO_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_SAP_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_go_sap(
			psoc);
	else if (((PM_STA_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_STA_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_STA_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_STA_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_sta_sta(
			psoc);
	else if (((PM_NAN_DISC_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_STA_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_STA_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_NAN_DISC_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_sta_nan(
			psoc);
	else if (((PM_NAN_DISC_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_NDI_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_NDI_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_NAN_DISC_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_nan_ndi(
			psoc);
	else if (((PM_SAP_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_NAN_DISC_MODE == pm_conc_connection_list[1].mode)) ||
		 ((PM_NAN_DISC_MODE == pm_conc_connection_list[0].mode) &&
		  (PM_SAP_MODE == pm_conc_connection_list[1].mode)))
		index = policy_mgr_get_third_connection_pcl_table_index_sap_nan(
			psoc);
	else if ((pm_conc_connection_list[0].mode == PM_P2P_GO_MODE) &&
		 (pm_conc_connection_list[1].mode == PM_P2P_GO_MODE))
		index = policy_mgr_get_third_connection_pcl_table_index_go_go(
			psoc);

	else if ((pm_conc_connection_list[0].mode == PM_P2P_CLIENT_MODE) &&
		 (pm_conc_connection_list[1].mode == PM_P2P_CLIENT_MODE))
		index = policy_mgr_get_third_connection_pcl_table_index_cli_cli(
			psoc);

	policy_mgr_debug(
		"mode0:%d mode1:%d freq0:%d freq1:%d chain:%d index:%d",
		pm_conc_connection_list[0].mode,
 ...
 * Return: none
 */
static void policy_mgr_get_index_for_3_given_freq_dbs(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	enum policy_mgr_three_connection_mode *index, qdf_freq_t freq1,
	qdf_freq_t freq2, qdf_freq_t freq3)
{
	...
	if ((WLAN_REG_IS_24GHZ_CH_FREQ(freq1) ==
		...
		policy_mgr_err(
			"Invalid mode for all freq %d, %d and %d on same band",
			freq1, freq2, freq3);
		return;
	}
	...
	qdf_freq_t freq2, qdf_freq_t freq3)
{
   ...
   */
	if (!policy_mgr_are_sbs_chan(pm_ctx->psoc, freq1, freq2) &&
		...
		policy_mgr_err("freq1 %d, freq2 %d and freq3 %d, none of the 2 "
			       "connections/3 vdevs are leading to SBS",
			       freq1, freq2, freq3);
		return;
	}
	...
	qdf_freq_t *sta_freq_list, uint8_t *ml_sta_idx)
{
	...
	if ((WLAN_REG_IS_24GHZ_CH_FREQ(sta_freq_list[ml_sta_idx[0]]) ==
		...
		policy_mgr_err(
			"Invalid mode for ML STA %d and %d are on same band as SAP %d",
   ...
   */
	if (WLAN_REG_IS_24GHZ_CH_FREQ(sta_freq_list[ml_sta_idx[0]]) ==
       ...
       */
			policy_mgr_err(
				"Invalid mode for ML STA %d and %d are on 2.4 GHZ, sap freq %d",
				sta_freq_list[ml_sta_idx[0]],
				sta_freq_list[ml_sta_idx[1]], sap_freq);
	...
	qdf_freq_t *sta_freq_list, uint8_t *ml_sta_idx)
{
   ...
   */
	...
	if (!sbs_24_shared_low_support && !sbs_24_shared_high_support) {
		policy_mgr_err(
			"unexpected sbs mode: low share %d high share %d",
			sbs_24_shared_low_support, sbs_24_shared_high_support);
		return;
	}
	sbs_cut_off_freq = policy_mgr_get_sbs_cut_off_freq(pm_ctx->psoc);
	if (!sbs_cut_off_freq) {
	...
	policy_mgr_debug(
		"4th index %d sap freq %d ml sta 5g %d sbs_cut_off_freq %d "
		"support high share %d low share %d",
		*index, sap_freq, ml_sta_5g_freq, sbs_cut_off_freq,
		sbs_24_shared_high_support, sbs_24_shared_low_support);
}
	...
	qdf_freq_t *sta_freq_list, uint8_t *ml_sta_idx)
{
   ...
   */
	if (!policy_mgr_are_sbs_chan(pm_ctx->psoc, sta_freq_list[ml_sta_idx[0]],
		...
		policy_mgr_err(
			"SAP freq (%d) and ML STA freq %d and %d, none of the 2 "
			"connections/3 vdevs are leading to SBS",
     ...
     */
		if (policy_mgr_2_freq_same_mac_in_sbs(
			    pm_ctx, sap_freq, sta_freq_list[ml_sta_idx[0]])) {
			/*
   ...
   */
	if (policy_mgr_2_freq_same_mac_in_sbs(pm_ctx,
					      sta_freq_list[ml_sta_idx[0]],
					      sta_freq_list[ml_sta_idx[1]])) {
		if (sap_freq < sbs_cut_off_freq)
...
static void policy_mgr_get_index_for_ml_sta_sap(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	enum policy_mgr_three_connection_mode *index, qdf_freq_t sap_freq,
	qdf_freq_t *sta_freq_list, uint8_t *ml_sta_idx)
{
   ...
   */
	policy_mgr_debug(
		"channel: sap0: %d, ML STA link0: %d, ML STA link1: %d",
		sap_freq, sta_freq_list[ml_sta_idx[0]],
		sta_freq_list[ml_sta_idx[1]]);
	if (policy_mgr_is_current_hwmode_sbs(pm_ctx->psoc) ||
		...
		policy_mgr_get_index_for_ml_sta_sap_sbs(
			pm_ctx, index, sap_freq, sta_freq_list, ml_sta_idx);
		return;
	}
	/*
...
static void policy_mgr_get_index_for_ml_sta_sap_sap(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	enum policy_mgr_three_connection_mode *index, qdf_freq_t ml_sta_freq,
	qdf_freq_t sap_freq_1, qdf_freq_t sap_freq_2)
{
	/*
   * P2P GO/P2P CLI are treated as SAP to optimize as pcl
   * table is same for all three.
   */
	...
	if (policy_mgr_is_current_hwmode_sbs(pm_ctx->psoc))
		...
		return policy_mgr_get_index_for_3_given_freq_sbs(
			pm_ctx, index, ml_sta_freq, sap_freq_1, sap_freq_2);
...
static inline void policy_mgr_get_index_for_ml_sta_sap(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	enum policy_mgr_three_connection_mode *index, qdf_freq_t sap_freq,
	qdf_freq_t *sta_freq_list, uint8_t *ml_sta_idx)
{
...
static inline void policy_mgr_get_index_for_ml_sta_sap_sap(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	enum policy_mgr_three_connection_mode *index, qdf_freq_t ml_sta_freq,
	qdf_freq_t sap_freq_1, qdf_freq_t sap_freq_2)
{
...
policy_mgr_get_fourth_connection_pcl_table_index(struct wlan_objmgr_psoc *psoc)
{
   ...
   */
	...
	count_sta = policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
							      list_sta);
	policy_mgr_get_ml_and_non_ml_sta_count(psoc, &num_ml_sta, ml_sta_idx,
					       &num_non_ml_sta, non_ml_sta_idx,
					       freq_list, vdev_id_list);

	count_ndi = policy_mgr_mode_specific_connection_count(psoc, PM_NDI_MODE,
							      list_ndi);
	...
	if (count_sap == 2 && num_ml_sta == 1) {
		...
		policy_mgr_debug("channel: sap0: %d, sta0: %d, sta1: %d",
				 pm_conc_connection_list[list_sap[0]].freq,
				 pm_conc_connection_list[list_sta[0]].freq,
...
uint32_t policy_mgr_get_nondfs_preferred_channel(struct wlan_objmgr_psoc *psoc,
						 enum policy_mgr_con_mode mode,
						 ...
						 uint8_t vdev_id)
{
   ...
   */
     ...
     */
		if (true ==
		    policy_mgr_is_any_nondfs_chnl_present(psoc, &non_dfs_freq))
			return non_dfs_freq;

		if (QDF_STATUS_SUCCESS !=
		    policy_mgr_get_pcl_for_existing_conn(
			    psoc, mode, pcl_channels, &pcl_len, pcl_weight,
			    QDF_ARRAY_SIZE(pcl_weight), false, vdev_id))
			return freq;
	} else {
		if (QDF_STATUS_SUCCESS !=
		    policy_mgr_get_pcl(psoc, mode, pcl_channels, &pcl_len,
...
QDF_STATUS
policy_mgr_get_valid_chans_from_range(struct wlan_objmgr_psoc *psoc,
				      uint32_t *ch_freq_list, uint32_t *ch_cnt,
				      enum policy_mgr_con_mode mode)
{
	...
	/* check the channel avoidance list for beaconing entities */
	if (mode == PM_SAP_MODE || mode == PM_P2P_GO_MODE)
		policy_mgr_update_with_safe_channel_list(psoc, ch_freq_list,
							 ch_cnt, ch_weight_list,
							 ch_weight_len);

	status = policy_mgr_mode_specific_modification_on_pcl(
		psoc, ch_freq_list, ch_weight_list, ch_cnt, ch_weight_len,
		mode);
...
policy_mgr_is_sta_on_indoor_channel(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t conn_index;
	...
	uint8_t *weight_list_org, uint32_t *pcl_len_org)
{
	...
	if (scc_on_dfs_channel)
		policy_mgr_is_sta_present_on_dfs_channel(
			psoc, &sta_vdev_id, &dfs_sta_freq, &sta_ch_width);
...
policy_mgr_get_sap_mandatory_channel(struct wlan_objmgr_psoc *psoc,
				     ...
				     uint32_t *intf_ch_freq, uint8_t vdev_id)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	QDF_STATUS status;
	...
	if (!sta_count ||
	    mcc_to_scc_switch != QDF_MCC_TO_SCC_SWITCH_WITH_FAVORITE_CHANNEL)
		return QDF_STATUS_E_FAILURE;
	...
	status = policy_mgr_get_pcl_for_existing_conn(
		psoc, PM_SAP_MODE, pcl.pcl_list, &pcl.pcl_len, pcl.weight_list,
		QDF_ARRAY_SIZE(pcl.weight_list), false, vdev_id);
	if (QDF_IS_STATUS_ERROR(status)) {
   ...
   */
	if (!pcl.pcl_len && !policy_mgr_mode_specific_connection_count(
				    psoc, PM_SAP_MODE, NULL)) {
		policy_mgr_debug(
			"policy_mgr_get_pcl_for_existing_conn returned no pcl");
		status = policy_mgr_get_pcl(psoc, PM_SAP_MODE, pcl.pcl_list,
					    &pcl.pcl_len, pcl.weight_list,
					    QDF_ARRAY_SIZE(pcl.weight_list),
					    vdev_id);
		if (QDF_IS_STATUS_ERROR(status)) {
			policy_mgr_err(
				"Unable to get PCL for SAP: policy_mgr_get_pcl");
			return status;
		}
	}

	status = policy_mgr_modify_sap_pcl_based_on_mandatory_channel(
		psoc, pcl.pcl_list, pcl.weight_list, &pcl.pcl_len);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	if (!pcl.pcl_len) {
		policy_mgr_err(
			"No common channel between mandatory list & PCL");
		return QDF_STATUS_E_FAILURE;
	}
	...
	if (policy_mgr_are_sbs_chan(psoc, sap_ch_freq, *intf_ch_freq)) {
		for (i = 0; i < pcl.pcl_len; i++) {
			if (pcl.pcl_list[i] == sap_ch_freq) {
				policy_mgr_debug(
					"As both freq, %d and %d are SBS, allow sap on mandatory freq %d",
   ...
   */
     ...
     */
		if (wlan_reg_is_freq_indoor(pm_ctx->pdev, pcl.pcl_list[i]) &&
			...
			sap_new_freq = pcl.pcl_list[i];
			policy_mgr_debug(
				"Choose Indoor channel from PCL list %d sap_new_freq %d",
		...
		if (user_config_freq && (pcl.pcl_list[i] == user_config_freq)) {
			sap_new_freq = pcl.pcl_list[i];
			policy_mgr_debug(
				"Prefer starting SAP on user configured channel:%d",
				sap_ch_freq);
			goto update_freq;
		}
	}
...
QDF_STATUS
policy_mgr_get_valid_chan_weights(struct wlan_objmgr_psoc *psoc,
				  ...
				  struct wlan_objmgr_vdev *vdev)
{
	...
	if ((mode == PM_P2P_GO_MODE || mode == PM_P2P_CLIENT_MODE) ||
	    (mode == PM_STA_MODE && policy_mgr_mode_specific_connection_count(
     ...
     */
		if (mode == PM_STA_MODE) {
			...
			if (vdev)
				policy_mgr_store_and_del_conn_info_by_vdev_id(
					psoc, wlan_vdev_get_id(vdev), info,
					&num_del);
			else
				policy_mgr_store_and_del_conn_info(
					psoc, mode, true, info, &num_del);
		}
		/*
...
uint32_t policy_mgr_get_alternate_channel_for_sap(struct wlan_objmgr_psoc *psoc,
						  uint8_t sap_vdev_id,
						  uint32_t sap_ch_freq,
						  enum reg_wifi_band pref_band)
{
   ...
   */
	qdf_mutex_acquire(&pm_ctx->qdf_conc_list_lock);
	policy_mgr_store_and_del_conn_info_by_vdev_id(psoc, sap_vdev_id, &info,
						      &num_cxn_del);
	if (QDF_STATUS_SUCCESS ==
	    policy_mgr_get_pcl(psoc, con_mode, pcl_channels, &pcl_len,
			       pcl_weight, QDF_ARRAY_SIZE(pcl_weight),
			       sap_vdev_id)) {
		for (i = 0; i < pcl_len; i++) {
			/*
       ...
       */
			...
			if (policy_mgr_get_connection_count(psoc) &&
			    policy_mgr_are_2_freq_on_same_mac(psoc, sap_ch_freq,
							      pcl_channels[i]))
				continue;
						 ...
						 qdf_freq_t ch_freq)
{
	...
	if (pm_conc_connection_list[0].freq != ch_freq ||
		...
		policy_mgr_debug("No MCC support in 3vif in same mac: %d %d %d",
				 pm_conc_connection_list[0].freq,
				 pm_conc_connection_list[1].freq, ch_freq);
		return false;
	}
	...
	if (policy_mgr_is_hw_dbs_capable(psoc) == true) {
		pcl = (*third_connection_pcl_dbs_table)[third_index][mode]
						       [conc_system_pref];
	} else {
		pcl = (*third_connection_pcl_non_dbs_table)[third_index][mode]
							   [conc_system_pref];
	}
	...
	struct wlan_objmgr_pdev *pdev, qdf_freq_t freq)
{
	...
	skip_6g_and_indoor_freq = wlan_scan_cfg_skip_6g_and_indoor_freq(psoc);
 ...
 */
static bool _policy_mgr_is_vdev_ll_sap(struct wlan_objmgr_psoc *psoc,
				       uint32_t vdev_id,
...
bool policy_mgr_is_vdev_ll_sap(struct wlan_objmgr_psoc *psoc, uint32_t vdev_id)
{
	return _policy_mgr_is_vdev_ll_sap(psoc, vdev_id, LL_AP_TYPE_ANY);
}

bool policy_mgr_is_vdev_ll_ht_sap(struct wlan_objmgr_psoc *psoc,
				  uint32_t vdev_id)
{
	return _policy_mgr_is_vdev_ll_sap(psoc, vdev_id, LL_AP_TYPE_HT);
}

bool policy_mgr_is_vdev_ll_lt_sap(struct wlan_objmgr_psoc *psoc,
				  uint32_t vdev_id)
{
...
policy_mgr_get_pcl_ch_for_sap_go_with_ll_sap_present(
	struct wlan_objmgr_psoc *psoc, uint32_t *len, uint32_t *pcl_channels,
	uint8_t *pcl_weight)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t pcl_len = 0, i, conn_idx = 0;
...
QDF_STATUS
policy_mgr_get_pcl_channel_for_ll_sap_concurrency(struct wlan_objmgr_psoc *psoc,
						  uint32_t vdev_id,
						  uint32_t *pcl_channels,
						  uint8_t *pcl_weight,
						  uint32_t *len)
{
     ...
     */
		policy_mgr_get_pcl_ch_for_sap_go_with_ll_sap_present(
			psoc, len, pcl_channels, pcl_weight);
	}
