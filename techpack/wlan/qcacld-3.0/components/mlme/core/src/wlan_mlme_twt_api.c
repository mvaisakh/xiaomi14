...
QDF_STATUS
mlme_init_all_peers_twt_context(struct wlan_objmgr_psoc *psoc, uint8_t vdev_id,
				uint8_t dialog_id)
{
	...
	while (peer) {
		...
		if (peer_priv) {
			...
			peer_priv->twt_ctx.num_twt_sessions = num_twt_sessions;
			for (i = 0; i < num_twt_sessions; i++) {
				uint8_t existing_dialog_id =
					peer_priv->twt_ctx.session_info[i]
						.dialog_id;
		...
		peer_next = wlan_peer_get_next_active_peer_of_vdev(
			vdev, peer_list, peer, WLAN_MLME_NB_ID);
...
void mlme_set_twt_peer_capabilities(struct wlan_objmgr_psoc *psoc,
				    struct qdf_mac_addr *peer_mac, uint8_t caps)
{
	struct wlan_objmgr_peer *peer;
	struct peer_mlme_priv_obj *peer_priv;
...
bool mlme_twt_any_peer_cmd_in_progress(struct wlan_objmgr_psoc *psoc,
				       uint8_t vdev_id, uint8_t dialog_id,
				       enum wlan_twt_commands cmd)
{
	qdf_list_t *peer_list;
	...
	while (peer) {
		...
		if (peer_priv) {
			...
			for (i = 0; i < num_twt_sessions; i++) {
				...
				active_cmd = peer_priv->twt_ctx.session_info[i]
						     .active_cmd;
				existing_dialog_id =
					peer_priv->twt_ctx.session_info[i]
						.dialog_id;
				...
				if (existing_dialog_id == dialog_id ||
					...
					if (dialog_id !=
						...
						wlan_objmgr_peer_release_ref(
							peer, WLAN_MLME_NB_ID);
						wlan_objmgr_vdev_release_ref(
							vdev, WLAN_MLME_NB_ID);
