...
enum uapsd_psave { PS_OFF = 0, PS_ON = 1 };

typedef int (*uapsd_tx_state_func_ptr)(struct sigma_stream *, u32,
				       enum uapsd_psave, u32);
...
static int uapsd_tx_start(struct sigma_stream *s, u32 usr_priority,
			  enum uapsd_psave ps, u32 sleep_duration);
static int uapsd_tx_confirm(struct sigma_stream *s, u32 usr_priority,
			    enum uapsd_psave ps, u32 sleep_duration);
static int uapsd_tx_data(struct sigma_stream *s, u32 usr_priority,
			 enum uapsd_psave ps, u32 sleep_duration);
static int uapsd_tx_stop(struct sigma_stream *s, u32 usr_priority,
			 enum uapsd_psave ps, u32 sleep_duration);
static int uapsd_tx_cyclic(struct sigma_stream *s, u32 usr_priority,
			   enum uapsd_psave ps, u32 sleep_duration);
static int uapsd_tx_data_twice(struct sigma_stream *s, u32 usr_priority,
			       enum uapsd_psave ps, u32 sleep_duration);
...
typedef int (*uapsd_recv_state_func_ptr)(struct sigma_stream *s, unsigned int *,
					 int);
...
static int uapsd_rx_start(struct sigma_stream *s, unsigned int *rxpkt,
			  int rxpkt_len);
static int uapsd_rx_data(struct sigma_stream *s, unsigned int *rxpkt,
			 int rxpkt_len);
static int uapsd_rx_stop(struct sigma_stream *s, unsigned int *rxpkt,
			 int rxpkt_len);
static int uapsd_rx_cyclic_vo(struct sigma_stream *s, unsigned int *rxpkt,
			      int rxpkt_len);
...
struct uapsd_console_state_table uapsd_console_state_tbl[LAST_TC + 1][10] = {
	...
	/* B.Z */
	{ { console_rx_hello },
	  { console_rx_confirm_tx_vi },
	  { console_rx_vo_tx_bcst_tx_stop },
	  { console_rx_tx_stop },
	  { NULL },
	  { NULL },
	  { NULL },
			    ...
			    u32 uapsd_dscp, struct sigma_stream *s)
{
	...
	txbuf[9] = s->sta_id;
	txbuf[10] = t->cmd;
	strlcpy((char *)&txbuf[11], t->name, txbuf_len - sizeof(txbuf[0]) * 11);
}
			    ...
			    enum uapsd_psave ps, u32 sleep_duration)
{
	...
	if (send(s->sock, tpkt, pktlen, 0) > 0) {
		...
		sigma_dut_print(dut, DUT_MSG_ERROR, "send_uapsd: Send failed");
	}
	free(tpkt);
	sigma_dut_print(
		dut, DUT_MSG_INFO,
			 ...
			 int rxpkt_len)
{
	...
	if ((rxpkt[10] == msgid) &&
	    ((rxpkt[1] == TOS_BE) || (rxpkt[1] == TOS_BK) ||
	     (rxpkt[1] == TOS_VI) || (rxpkt[1] == TOS_VO) ||
	     (rxpkt[1] == TOS_VO7) || (rxpkt[1] == TOS_VO6))) {
		s->rx_cookie = rxpkt[0];
		(s->uapsd_rx_state)++;
		sigma_dut_print(
			dut, DUT_MSG_INFO,
	...
	sigma_dut_print(dut, DUT_MSG_INFO, "receive_uapsd: Exit uapsd_rx_data");
			      ...
			      int rxpkt_len)
{
   ...
   * APTS DEFAULT to 38 */
	...
	if (rxpkt[10] != msgid) {
		if ((rxpkt[10] == msgid2) &&
		    ((rxpkt[1] == TOS_VO) || (rxpkt[1] == TOS_VO7) ||
		     (rxpkt[1] == TOS_VO6))) {
			/* ; 5.7 */
			s->rx_cookie = rxpkt[0];
		} else {
...
static void *send_uapsd(void *data)
{
	...
	while (!s->stop) {
		...
		tx_state_func =
			sta_uapsd_tx_tbl[s->uapsd_sta_tc][s->uapsd_tx_state]
				.state_func;
		usr_priority =
			sta_uapsd_tx_tbl[s->uapsd_sta_tc][s->uapsd_tx_state]
				.usr_priority;
		...
		if (tx_state_func) {
			...
			sigma_dut_print(
				dut, DUT_MSG_INFO,
				"send_uapsd: Null Function Detected for TC : %d in "
				"uapsd_tx_state : %d",
				s->uapsd_sta_tc, s->uapsd_tx_state);
		}
	}
...
void receive_uapsd(struct sigma_stream *s)
{
	...
	while (!s->stop) {
		...
		sigma_dut_print(
			...
			res, rxpkt[0], rxpkt[1], rxpkt[10], rxpkt[9]);
		...
		if (rxpkt_len > 0) {
			...
			if (recv_state_func) {
				...
				sigma_dut_print(
					dut, DUT_MSG_INFO,
					"receive_uapsd: Null Function Detected for TC : %d in "
					"uapsd_rx_state : %d",
					s->uapsd_sta_tc, s->uapsd_rx_state);
			}
		} else if (res < 0) {
			   ...
			   unsigned int type, u32 tos)
{
	...
	if (!res) {
		sigma_dut_print(
			dut, DUT_MSG_ERROR,
			"packet_expected: No match: received pkt_type %u expected "
			"type %u, received dscp 0x%x expected dscp 0x%x",
...
void send_uapsd_console(struct sigma_stream *s)
{
	...
	while (!s->stop) {
		...
		if (res < 0) {
			...
			sigma_dut_print(
				dut, DUT_MSG_INFO,
				"send_uapsd_console: running res %d cookie %d dscp %d "
				"apts-pkt %d sta-id %d",
				res, rpkt[0], rpkt[1], rpkt[10], rpkt[9]);
			if (res == 0)
				continue;
       ...
       */
			...
			if (console_state_func) {
				...
				sigma_dut_print(
					dut, DUT_MSG_INFO,
					"send_uapsd_console: Null function detected for TC: %d "
					"in uapsd_rx_state: %d",
					s->uapsd_sta_tc, s->uapsd_rx_state);
			}
		}
