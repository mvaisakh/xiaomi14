...
static inline struct dbglog_info *
handle2info(struct common_dbglog_handle *dbg_handle)
{
...
int fwdbg_parse_debug_logs(struct common_dbglog_handle *dbg_handle,
			   ol_scn_t soc, uint8_t *datap, uint16_t len,
			   void *context)
{
	struct dbglog_info *dbg_info = handle2info(dbg_handle);

	if (dbg_info->ops->dbglog_parse_debug_logs)
		return dbg_info->ops->dbglog_parse_debug_logs(soc, datap, len,
							      context);
			  ...
			  uint16_t vap_id, bool isenable)
{
	...
	if (dbg_info->ops->dbglog_vap_log_enable)
		dbg_info->ops->dbglog_vap_log_enable(scn, vap_id, isenable);
}
			     ...
			     ol_scn_t scn, uint32_t mod_id, bool isenable)
{
	...
	if (dbg_info->ops->dbglog_module_log_enable)
		dbg_info->ops->dbglog_module_log_enable(scn, mod_id, isenable);
}
