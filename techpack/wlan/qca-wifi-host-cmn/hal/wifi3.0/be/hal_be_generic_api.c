 ...
 *
 */
uint32_t reo_dest_ring_remap[] = { REO_REMAP_SW1, REO_REMAP_SW2, REO_REMAP_SW3,
				   REO_REMAP_SW4, REO_REMAP_SW5, REO_REMAP_SW6,
...
void hal_setup_reo_swap(struct hal_soc *soc)
{
	...
	reg_val = HAL_REG_READ(
		soc, HWIO_REO_R0_CACHE_CTL_CONFIG_ADDR(REO_REG_REG_BASE));
	...
	HAL_REG_WRITE(soc, HWIO_REO_R0_CACHE_CTL_CONFIG_ADDR(REO_REG_REG_BASE),
		      reg_val);
}
#else
void hal_setup_reo_swap(struct hal_soc *soc)
 ...
 * Return: none
 */
static void hal_tx_init_data_ring_be(hal_soc_handle_t hal_soc_hdl,
 ...
 * Return: void
 */
static void hal_rx_buf_cookie_rbm_get_be(uint32_t *buf_addr_info_hdl,
					 hal_buf_info_t buf_info_hdl)
{
   ...
   */
	buf_info->rbm =
		hal_rx_ret_buf_manager_get_be((hal_ring_desc_t)buf_addr_info);
}
 ...
 */
static inline bool hal_rx_en_mcast_fp_data_filter_generic_be(void)
{
	return true;
}
 ...
 *
 */
static inline void hal_rxdma_buff_addr_info_set_be(void *rxdma_entry,
...
static inline void hal_msdu_desc_info_set_be(hal_soc_handle_t hal_soc_hdl,
...
static inline void hal_mpdu_desc_info_set_be(hal_soc_handle_t hal_soc_hdl,
					     ...
					     uint32_t seq_no)
{
	...
	HAL_RX_FLD_SET(desc, REO_ENTRANCE_RING, MPDU_SEQUENCE_NUMBER, seq_no);

	HAL_RX_MPDU_DESC_INFO_SET(mpdu_desc_info, MSDU_COUNT, 0x1);
	HAL_RX_MPDU_DESC_INFO_SET(mpdu_desc_info, FRAGMENT_FLAG, 0x1);
 ...
 */
static inline uint32_t
hal_rx_msdu_reo_dst_ind_get_be(hal_soc_handle_t hal_soc_hdl,
			       void *msdu_link_desc)
{
	struct hal_soc *hal_soc = (struct hal_soc *)hal_soc_hdl;
	...
	msdu_desc_info =
		hal_rx_msdu_ext_desc_info_get_ptr(&msdu_details[0], hal_soc);
	dst_ind = HAL_RX_MSDU_REO_DST_IND_GET(msdu_desc_info);
	return dst_ind;
}

uint32_t hal_reo_ix_remap_value_get_be(hal_soc_handle_t hal_soc_hdl,
	...
	if (hal_rx_wbm_get_cookie_convert_done(rx_desc))
		buf_info->paddr =
			(HAL_RX_WBM_COMP_BUF_ADDR_31_0_GET(rx_desc) |
			 ((uint64_t)(HAL_RX_WBM_COMP_BUF_ADDR_39_32_GET(rx_desc))
			  << 32));
	else
		buf_info->paddr =
			(HAL_RX_WBM_BUF_ADDR_31_0_GET(rx_desc) |
			 ((uint64_t)(HAL_RX_WBM_BUF_ADDR_39_32_GET(rx_desc))
			  << 32));
}
#else
static inline void
 ...
 * Return: None
 */
static void hal_unregister_reo_send_cmd_be(struct hal_soc *hal_soc)
{
 ...
 * Return: None
 */
static void hal_register_reo_send_cmd_be(struct hal_soc *hal_soc)
{
 ...
 * Return: None
 */
static void hal_reset_rx_reo_tid_q_be(struct hal_soc *hal_soc,
...
void hal_hw_txrx_default_ops_attach_be(struct hal_soc *hal_soc)
{
	...
	hal_soc->ops->hal_gen_reo_remap_val = hal_gen_reo_remap_val_generic_be;
	...
	hal_soc->ops->hal_rx_mpdu_desc_info_get = hal_rx_mpdu_desc_info_get_be;
