...
static void wlan_ipa_uc_rt_debug_host_fill(void *ctext)
{
	...
	dump_info = &ipa_ctx->rt_bug_buffer[ipa_ctx->rt_buf_fill_index %
...
void wlan_ipa_uc_rt_debug_host_dump(struct wlan_ipa_priv *ipa_ctx)
{
	...
	qdf_mutex_acquire(&ipa_ctx->rt_debug_lock);
	for (dump_count = 0; dump_count < WLAN_IPA_UC_RT_DEBUG_BUF_COUNT;
	     dump_count++) {
...
static void wlan_ipa_dump_ipa_ctx(struct wlan_ipa_priv *ipa_ctx)
{
	...
	ipa_info("\n==== IPA IPA ====\n"
		 ...
		 "pm_lock: %pK\n"
		 "suspended: %d\n",
		 ipa_ctx->num_iface, ipa_ctx->rm_state, &ipa_ctx->rm_lock,
		 &ipa_ctx->uc_rm_work, &ipa_ctx->uc_op_work,
		 &ipa_ctx->wake_lock, &ipa_ctx->wake_lock_work,
		 ipa_ctx->wake_lock_released, ipa_ctx->tx_ref_cnt.counter,
		 ipa_ctx->pm_queue_head.head, ipa_ctx->pm_queue_head.tail,
		 ipa_ctx->pm_queue_head.qlen, &ipa_ctx->pm_work,
		 &ipa_ctx->pm_lock, ipa_ctx->suspended);
	...
	ipa_info("\nq_lock: %pK\n"
		 ...
		 "sta_connected: %d\n",
		 &ipa_ctx->q_lock, ipa_ctx->pend_desc_head.next,
		 ipa_ctx->pend_desc_head.prev, &ipa_ctx->stats,
		 ipa_ctx->curr_prod_bw, ipa_ctx->curr_cons_bw,
		 ipa_ctx->activated_fw_pipe, ipa_ctx->sap_num_connected_sta,
	...
	ipa_info("\ntx_pipe_handle: 0x%x\n"
		 ...
		 ipa_ctx->pending_cons_req, ipa_ctx->pending_event.anchor.next,
		 ipa_ctx->pending_event.anchor.prev,
		 ipa_ctx->pending_event.count, ipa_ctx->pending_event.max_size,
		 &ipa_ctx->event_lock, ipa_ctx->ipa_tx_packets_diff,
		 ipa_ctx->ipa_rx_packets_diff, ipa_ctx->ipa_p_tx_packets,
		 ipa_ctx->ipa_p_rx_packets, ipa_ctx->stat_req_reason);
	...
	ipa_info("\ncons_pipe_in----\n"
		 ...
		 ipa_ctx->uc_loaded, ipa_ctx->wdi_enabled,
		 &ipa_ctx->rt_debug_fill_timer, &ipa_ctx->rt_debug_lock,
		 &ipa_ctx->ipa_lock);
...
static void wlan_ipa_dump_sys_pipe(struct wlan_ipa_priv *ipa_ctx)
{
	...
	for (i = 0; i < WLAN_IPA_MAX_SYSBAM_PIPE; i++) {
		...
		ipa_info("\nsys_pipe[%d]----\n"
			 ...
			 "\tskip_ep_cfg: %d\n"
			 "\tkeep_ipa_awake: %d\n",
			 i, sys_pipe->conn_hdl, sys_pipe->conn_hdl_valid,
...
static void wlan_ipa_dump_iface_context(struct wlan_ipa_priv *ipa_ctx)
{
	...
	for (i = 0; i < WLAN_IPA_MAX_IFACE; i++) {
		...
		ipa_info("\niface_context[%d]----\n"
			 ...
			 "\tinterface_lock: %pK\n"
			 "\tifa_address: 0x%x\n",
			 i, iface_context->ipa_ctx, iface_context->session_id,
			 iface_context->cons_client, iface_context->prod_client,
...
static void wlan_ipa_print_session_info(struct wlan_ipa_priv *ipa_ctx)
{
	...
	ipa_info("\n==== IPA SESSION INFO ====\n"
		 ...
		 "NUM SEND MSG: %d\n"
		 "NUM FREE MSG: %d\n",
		 ipa_ctx->num_iface, ipa_ctx->rm_state,
		 ipa_ctx->activated_fw_pipe, ipa_ctx->sap_num_connected_sta,
		 ipa_ctx->sta_connected, (ipa_ctx->mcc_mode ? "MCC" : "SCC"),
		 ipa_ctx->resource_loading, ipa_ctx->resource_unloading,
		 ipa_ctx->pending_cons_req, ipa_ctx->ipa_pipes_down,
		 ipa_ctx->uc_loaded, ipa_ctx->wdi_enabled,
	...
	for (i = 0; i < WLAN_IPA_MAX_IFACE; i++) {
		...
		ipa_info(
			"\nIFACE[%d]: mode:%d, offload:%d", i,
			iface_context->device_mode,
			ipa_ctx->vdev_offload_enabled[iface_context->session_id]);
	}

	for (i = 0; i < QDF_IPA_WLAN_EVENT_MAX; i++)
		ipa_info("\nEVENT[%d]=%d", i, ipa_ctx->stats.event[i]);

	i = 0;
	qdf_list_peek_front(&ipa_ctx->pending_event,
			    (qdf_list_node_t **)&event);
	while (event) {
		ipa_info("PENDING EVENT[%d]: EVT:%s, MAC:" QDF_MAC_ADDR_FMT, i,
			 wlan_ipa_wlan_event_to_str(event->type),
...
static void wlan_ipa_print_txrx_stats(struct wlan_ipa_priv *ipa_ctx)
{
	...
	ipa_info("\n==== IPA IPA TX/RX STATS ====\n"
		 ...
		 "PEND DESC HEAD: %pK\n"
		 "TX DESC LIST: %pK\n",
		 ipa_ctx->stats.num_rm_grant, ipa_ctx->stats.num_rm_release,
		 ipa_ctx->stats.num_rm_grant_imm,
		 ipa_ctx->stats.num_cons_perf_req,
		 ipa_ctx->stats.num_prod_perf_req, ipa_ctx->stats.num_rx_drop,
		 ipa_ctx->stats.num_rx_excep, ipa_ctx->stats.num_tx_fwd_ok,
		 ipa_ctx->stats.num_tx_fwd_err,
		 ipa_ctx->stats.num_tx_desc_q_cnt,
		 ipa_ctx->stats.num_tx_desc_error,
		 ipa_ctx->stats.num_tx_comp_cnt, ipa_ctx->stats.num_tx_queued,
		 ipa_ctx->stats.num_tx_dequeued,
		 ipa_ctx->stats.num_max_pm_queue, ipa_ctx->tx_ref_cnt.counter,
		 ipa_ctx->suspended, &ipa_ctx->pend_desc_head,
				 ...
				 struct ipa_uc_fw_stats *uc_fw_stat)
{
	ipa_info("\n==== WLAN FW WDI TX STATS ====\n"
		 ...
		 uc_fw_stat->tx_pkts_enqueued, uc_fw_stat->tx_pkts_completed,
	...
	ipa_info("\n==== WLAN FW WDI RX STATS ====\n"
		 ...
		 uc_fw_stat->rx_refill_idx, uc_fw_stat->rx_num_pkts_indicated,
		 uc_fw_stat->rx_buf_refilled,
		 uc_fw_stat->rx_num_ind_drop_no_space,
		 uc_fw_stat->rx_num_ind_drop_no_buf, uc_fw_stat->rx_is_suspend);
}
...
static void wlan_ipa_uc_set_quota(struct wlan_ipa_priv *ipa_ctx,
				  uint8_t set_quota, uint64_t quota_bytes)
{
	ipa_info("SET_QUOTA: set_quota=%d, quota_bytes=%llu", set_quota,
		 quota_bytes);
					     ...
					     void *data)
{
     ...
     */
		...
		wlan_ipa_uc_sharing_stats_request(
			ipa_ctx,
		...
		if (!QDF_IS_STATUS_SUCCESS(status)) {
			...
			QDF_IPA_SET_WIFI_QUOTA_BYTES(ipa_set_quota) =
				((uint64_t)(ipa_ctx->ipa_quota_rsp.quota_hi)
				 << 32) |
				ipa_ctx->ipa_quota_rsp.quota_lo;
				   ...
				   struct op_msg_type *op_msg)
{
	...
	if (msg->op_code == WLAN_IPA_UC_OPCODE_SHARING_STATS) {
		...
		uc_quota_rsp =
			(struct ipa_uc_quota_rsp *)((uint8_t *)op_msg +
						    sizeof(struct op_msg_type));
		...
		/* hit quota limit */
		uc_quota_ind =
			(struct ipa_uc_quota_ind *)((uint8_t *)op_msg +
						    sizeof(struct op_msg_type));

		ipa_ctx->ipa_quota_ind.quota_bytes = uc_quota_ind->quota_bytes;
		...
		if (iface_ctx)
			qdf_ipa_broadcast_wdi_quota_reach_ind(
				iface_ctx->dev->ifindex, quota_bytes);
		else
