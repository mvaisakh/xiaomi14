...
#ifdef WLAN_MLO_MULTI_CHIP
static inline bool mlo_psoc_get_index_id(struct wlan_objmgr_psoc *psoc,
					 uint8_t grp_id, uint8_t *index,
					 bool teardown)
{
	...
	if (grp_id >= mlo_ctx->total_grp) {
		mlo_err("Invalid grp id %d, total no of groups %d", grp_id,
			mlo_ctx->total_grp);
		return false;
	}
...
void mlo_get_soc_list(struct wlan_objmgr_psoc **soc_list, uint8_t grp_id,
		      uint8_t total_socs, enum MLO_SOC_LIST curr)
{
	struct mlo_mgr_context *mlo_ctx = wlan_objmgr_get_mlo_ctx();
	uint8_t chip_idx;
	...
	if (curr == WLAN_MLO_GROUP_CURRENT_SOC_LIST) {
		for (chip_idx = 0; chip_idx < total_socs; chip_idx++)
			soc_list[chip_idx] = mlo_ctx->setup_info[grp_id]
						     .curr_soc_list[chip_idx];
	} else {
		for (chip_idx = 0; chip_idx < total_socs; chip_idx++)
			soc_list[chip_idx] =
...
void mlo_setup_update_total_socs(uint8_t grp_id, uint8_t tot_socs)
{
	...
	qdf_info("Grp_id %d Total MLO socs = %d links = %d", grp_id,
		 mlo_ctx->setup_info[grp_id].tot_socs,
...
static void mlo_check_state(struct wlan_objmgr_psoc *psoc, void *obj,
			    void *args)
{
	struct wlan_objmgr_pdev *pdev;
	uint8_t link_idx;
...
QDF_STATUS mlo_check_all_pdev_state(struct wlan_objmgr_psoc *psoc,
				    uint8_t grp_id, enum MLO_LINK_STATE state)
{
	...
	wlan_objmgr_iterate_obj_list(psoc, WLAN_PDEV_OP, mlo_check_state,
				     &params, 0, WLAN_MLME_NB_ID);
...
void mlo_setup_init(uint8_t total_grp)
{
	...
	if (!total_grp && total_grp > WLAN_MAX_MLO_GROUPS) {
		mlo_err("Total number of groups (%d) is greater than MAX (%d), MLD Setup "
			"failed!!",
	...
	setup_info = qdf_mem_malloc(sizeof(struct mlo_setup_info) * total_grp);
...
void mlo_setup_update_num_links(struct wlan_objmgr_psoc *psoc, uint8_t grp_id,
				uint8_t num_links)
{
	struct mlo_mgr_context *mlo_ctx = wlan_objmgr_get_mlo_ctx();
	...
	qdf_info("Grp_id %d Total MLO links = %d", grp_id,
		 mlo_ctx->setup_info[grp_id].tot_links);
}
...
void mlo_setup_update_soc_ready(struct wlan_objmgr_psoc *psoc, uint8_t grp_id)
{
	...
	if (!mlo_psoc_get_index_id(psoc, grp_id, &chip_idx, 0)) {
		mlo_err("Unable to fetch chip idx for psoc id %d grp id %d",
			psoc->soc_objmgr.psoc_id, grp_id);
		return;
	}
	...
	mlo_debug("SoC updated to mld grp %d , chip idx %d num soc %d", grp_id,
		  chip_idx, setup_info->num_soc);
	...
	if (!dp_mlo_ctxt) {
		dp_mlo_ctxt =
			cdp_mlo_ctxt_attach(wlan_psoc_get_dp_handle(psoc),
...
void mlo_setup_link_ready(struct wlan_objmgr_pdev *pdev, uint8_t grp_id)
{
	...
	qdf_info(
		"Pdev updated to Grp id %d mld link %d num_links %d  hw link id %d "
		"Valid link bitmap %d",
		grp_id, link_idx, setup_info->num_links, link_id,
	...
	if (setup_info->num_links == setup_info->tot_links &&
		...
		if (tx_ops && tx_ops->mops.target_if_mlo_setup_req) {
			tx_ops->mops.target_if_mlo_setup_req(
				setup_info->pdev_list, setup_info->num_links,
				grp_id);
		}
	}
}
...
void mlo_link_setup_complete(struct wlan_objmgr_pdev *pdev, uint8_t grp_id)
{
	...
	for (link_idx = 0; link_idx < setup_info->tot_links; link_idx++)
		if (setup_info->pdev_list[link_idx] == pdev) {
			setup_info->state[link_idx] = MLO_LINK_SETUP_DONE;
			break;
		}
	...
	if (link_idx == setup_info->tot_links) {
		...
		if (tx_ops && tx_ops->mops.target_if_mlo_ready) {
			tx_ops->mops.target_if_mlo_ready(setup_info->pdev_list,
							 setup_info->num_links);
		}
	}
}
...
static void mlo_setup_link_down(struct wlan_objmgr_psoc *psoc, void *obj,
				void *args)
{
	struct wlan_objmgr_pdev *pdev;
	uint8_t link_idx;
	...
	mlo_debug("Pdev link down grp_id %d link_idx %d num_links %d", grp_id,
		  link_idx, setup_info->num_links);
}

static void mlo_dp_ctxt_detach(struct wlan_objmgr_psoc *psoc, uint8_t grp_id,
...
void mlo_setup_update_soc_down(struct wlan_objmgr_psoc *psoc, uint8_t grp_id)
{
	...
	mlo_debug("Soc down, mlo group %d num soc %d num links %d", grp_id,
		  setup_info->num_soc, setup_info->num_links);
}
...
static void mlo_send_teardown_req(struct wlan_objmgr_psoc *psoc, uint8_t grp_id,
				  uint32_t reason)
{
	struct mlo_mgr_context *mlo_ctx = wlan_objmgr_get_mlo_ctx();
	struct wlan_lmac_if_tx_ops *tx_ops;
	...
	if (reason == WMI_MLO_TEARDOWN_REASON_MODE1_SSR) {
		for (link_idx = 0; link_idx < tot_links; link_idx++) {
			...
			if (tx_ops && tx_ops->mops.target_if_mlo_teardown_req) {
				mlo_info(
					"Trigger Teardown  with Pdev %d Psoc id %d link idx %d Umac "
					"reset %d for Mode 1",
					wlan_objmgr_pdev_get_pdev_id(temp_pdev),
					wlan_psoc_get_id(
						wlan_pdev_get_psoc(temp_pdev)),
					link_idx, umac_reset);
				tx_ops->mops.target_if_mlo_teardown_req(
					setup_info->pdev_list[link_idx], reason,
					umac_reset);
			}
		}
	} else {
...
QDF_STATUS mlo_link_teardown_link(struct wlan_objmgr_psoc *psoc, uint8_t grp_id,
				  uint32_t reason)
{
	QDF_STATUS status;
	struct mlo_setup_info *setup_info;
	...
	mlo_debug("Teardown req with grp_id %d num_soc %d num_link %d", grp_id,
		  setup_info->num_soc, setup_info->num_links);
	...
	if (mlo_grp_in_teardown(grp_id)) {
		mlo_debug(
			"Skip teardown: as teardown sent already, grp_id %d num_soc %d "
			"num_link %d",
	...
	status = qdf_wait_for_event_completion(&setup_info->event,
				       ...
				       struct wlan_objmgr_pdev **pdev_list)
{
   ...
   */
	for (idx = 0; idx < tot_grp_socs; idx++) {
		...
		qdf_info("Checking adjacency of soc %d and %d", psoc_id,
			 tmp_psoc_id);
