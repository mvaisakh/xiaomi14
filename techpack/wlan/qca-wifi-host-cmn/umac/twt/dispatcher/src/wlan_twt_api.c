...
#ifdef WLAN_POWER_MANAGEMENT_OFFLOAD
static QDF_STATUS wlan_twt_suspend_handler(struct wlan_objmgr_psoc *psoc,
					   void *arg)
{
...
static QDF_STATUS wlan_twt_resume_handler(struct wlan_objmgr_psoc *psoc,
					  void *arg)
{
...
static void wlan_twt_register_pmo_handler(void)
{
	pmo_register_suspend_handler(WLAN_UMAC_COMP_TWT,
				     wlan_twt_suspend_handler, NULL);
	pmo_register_resume_handler(WLAN_UMAC_COMP_TWT, wlan_twt_resume_handler,
				    NULL);
}

static inline void wlan_twt_unregister_pmo_handler(void)
{
...
QDF_STATUS wlan_twt_init(void)
{
	...
	status = wlan_objmgr_register_psoc_create_handler(
		WLAN_UMAC_COMP_TWT, wlan_twt_psoc_obj_create_handler, NULL);
	...
	status = wlan_objmgr_register_psoc_destroy_handler(
		WLAN_UMAC_COMP_TWT, wlan_twt_psoc_obj_destroy_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = wlan_objmgr_register_vdev_create_handler(
		WLAN_UMAC_COMP_TWT, wlan_twt_vdev_obj_create_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = wlan_objmgr_register_vdev_destroy_handler(
		WLAN_UMAC_COMP_TWT, wlan_twt_vdev_obj_destroy_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = wlan_objmgr_register_peer_create_handler(
		WLAN_UMAC_COMP_TWT, wlan_twt_peer_obj_create_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = wlan_objmgr_register_peer_destroy_handler(
		WLAN_UMAC_COMP_TWT, wlan_twt_peer_obj_destroy_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
...
QDF_STATUS twt_psoc_disable(struct wlan_objmgr_psoc *psoc)
{
	...
	if (QDF_IS_STATUS_ERROR(status))
		twt_err("twt_deregister_events failed (status=%d)", status);
