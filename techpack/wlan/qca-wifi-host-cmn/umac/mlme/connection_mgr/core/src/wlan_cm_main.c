 ...
 * Return: void
 */
static inline void cm_req_lock_create(struct cnx_mgr *cm_ctx)
{
 ...
 * Return: void
 */
static inline void cm_req_lock_destroy(struct cnx_mgr *cm_ctx)
{
...
QDF_STATUS wlan_cm_init(struct vdev_mlme_obj *vdev_mlme)
{
	...
	vdev_mlme->cnx_mgr_ctx->max_connect_attempts = CM_MAX_CONNECT_ATTEMPTS;
	...
	vdev_mlme->cnx_mgr_ctx->scan_requester_id =
		wlan_scan_register_requester(psoc, "CM", wlan_cm_scan_cb,
	...
	for (link_info_iter = 0; link_info_iter < 3; link_info_iter++) {
		...
		if (link_info->vdev_id == WLAN_INVALID_VDEV_ID) {
			...
			wlan_scan_update_mlme_by_bssinfo(
				wlan_vdev_get_pdev(vdev), &bss_info,
				&mlme_info);
		}
