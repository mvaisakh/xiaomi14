...
#define TWO_IN_DB 3
static int32_t SNR_DB_TO_BIT_PER_TONE_LUT[DB_NUM] = {
	0,    171,  212,  262,	323,  396,  484,  586,	706,  844,
	1000, 1176, 1370, 1583, 1812, 2058, 2317, 2588, 2870, 3161
};
#endif

/* MLO link types */
enum MLO_TYPE { SLO, MLSR, MLMR, MLO_TYPE_MAX };
 ...
 */
static inline int8_t cm_get_rssi_pcnt_for_slot(int32_t high_rssi_threshold,
					       ...
					       int8_t bss_rssi)
{
	...
	num_slot =
		((high_rssi_threshold - low_rssi_threshold) / bucket_size) + 1;
	slot_size = ((high_rssi_pcnt - low_rssi_pcnt) + (num_slot / 2)) /
		    (num_slot);
	...
	mlme_debug(
		"Window %d -> %d pcnt range %d -> %d bucket_size %d bss_rssi %d "
		"num_slot %d slot_size %d rssi_diff %d slot_index %d rssi_pcnt %d",
				       ...
				       int32_t rssi, uint8_t rssi_weightage)
{
   ...
   */
	...
	if (rssi > good_rssi_threshold)
		...
		rssi_pcnt = cm_get_rssi_pcnt_for_slot(
			good_rssi_threshold, bad_rssi_threshold, good_rssi_pcnt,
			bad_rssi_pcnt, bad_bucket_size, rssi);
 ...
 */
static int8_t cm_get_rssi_prorate_pct(struct rssi_config_score *score_param,
				      int32_t rssi, uint8_t rssi_weightage)
{
	int32_t good_rssi_threshold;
	int32_t bad_rssi_threshold;
	...
	return cm_get_rssi_pcnt_for_slot(
		good_rssi_threshold, bad_rssi_threshold,
		score_param->good_rssi_pcnt, score_param->bad_rssi_pcnt,
		score_param->bad_rssi_bucket_size, rssi);
}
 ...
 */
static int32_t cm_get_score_for_index(uint8_t index, uint8_t weightage,
				      struct per_slot_score *score)
{
	if (index <= CM_SCORE_INDEX_3)
		return weightage *
		       CM_GET_SCORE_PERCENTAGE(score->score_pcnt3_to_0, index);
...
static int32_t cm_get_congestion_pct(struct scan_cache_entry *entry)
{
	...
	if (entry->air_time_fraction) {
		...
		est_air_time_percentage =
			entry->air_time_fraction * CM_MAX_CHANNEL_WEIGHT;
		est_air_time_percentage =
     ...
     */
		congestion =
			CM_MAX_CHANNEL_UTILIZATION - est_air_time_percentage;
		if (!congestion)
			congestion = 1;
					     ...
					     bool rssi_bad_zone)
{
	...
	*congestion_pct = cm_get_congestion_pct(entry);
	...
	if (score_params->esp_qbss_scoring.num_slot > CM_SCORE_MAX_INDEX)
		score_params->esp_qbss_scoring.num_slot = CM_SCORE_MAX_INDEX;
	...
	window_size =
		CM_MAX_PCT_SCORE / score_params->esp_qbss_scoring.num_slot;
	...
	return cm_get_score_for_index(index, chan_congestion_weight,
				      &score_params->esp_qbss_scoring);
}
...
static uint32_t cm_get_sta_nss(struct wlan_objmgr_psoc *psoc,
			       qdf_freq_t bss_channel_freq, uint8_t vdev_nss_2g,
			       uint8_t vdev_nss_5g)
{
	/*
   ...
   */
	...
	if (policy_mgr_is_chnl_in_diff_band(psoc, bss_channel_freq) &&
	    policy_mgr_is_hw_dbs_capable(psoc) &&
	    !(policy_mgr_is_hw_dbs_2x2_capable(psoc)))
		return 1;

	return (WLAN_REG_IS_24GHZ_CH_FREQ(bss_channel_freq) ? vdev_nss_2g :
							      vdev_nss_5g);
}
#else
static uint32_t cm_get_sta_nss(struct wlan_objmgr_psoc *psoc,
...
#ifdef CONN_MGR_ADV_FEATURE
static bool cm_get_pcl_weight_of_channel(uint32_t chan_freq,
   ...
   */
	...
	if (pcl_chan_weight) {
		...
		temp_pcl_chan_weight = qdf_do_div(
			temp_pcl_chan_weight, CM_PCL_GROUPS_WEIGHT_DIFFERENCE);
					  ...
					  struct scoring_cfg *score_params)
{
	...
	if (score_params->oce_wan_scoring.num_slot > CM_SCORE_MAX_INDEX)
		score_params->oce_wan_scoring.num_slot = CM_SCORE_MAX_INDEX;

	window_size =
		CM_SCORE_MAX_INDEX / score_params->oce_wan_scoring.num_slot;
	...
	if (wlan_parse_oce_reduced_wan_metrics_ie(mbo_oce_ie, &wan_metrics)) {
		...
		index = qdf_do_div(wan_metrics.downlink_av_cap, window_size);
	} else {
	...
	return cm_get_score_for_index(
		index, score_params->weight_config.oce_wan_weightage,
				     ...
				     int8_t *ap_tx_pwr_dbm)
{
	...
	if (normalized_ap_tx_pwr > best_rssi_threshold) {
		...
		ap_tx_pwr_factor = cm_get_rssi_pcnt_for_slot(
			best_rssi_threshold, good_rssi_threshold, 100,
			good_rssi_pcnt, good_bucket_size, normalized_ap_tx_pwr);
	} else {
		/* Uplink RSSI lies between good to best rssi threshold */
		ap_tx_pwr_factor = cm_get_rssi_pcnt_for_slot(
			good_rssi_threshold, bad_rssi_threshold, good_rssi_pcnt,
			bad_rssi_pcnt, bad_bucket_size, normalized_ap_tx_pwr);
	}
				...
				struct scan_cache_entry *entry)
{
	...
	if (check_assoc_disallowed &&
		...
		mlme_nofl_debug(
			"Candidate(" QDF_MAC_ADDR_FMT
			" freq %d): rssi %d, assoc disallowed set in MBO/OCE IE reason %d",
			QDF_MAC_ADDR_REF(entry->bssid.bytes),
			entry->channel.chan_freq, entry->rssi_raw, reason);
		return false;
	}
		       ...
		       struct psoc_phy_config *phy_config)
{
	...
	if (!phy_config->ht_cap && ch_width >= CH_WIDTH_20MHZ)
		ch_width = CH_WIDTH_20MHZ;
...
static uint32_t cm_get_etp_max_bits_per_sc_1000x_for_nss(
	struct wlan_objmgr_psoc *psoc, struct scan_cache_entry *entry,
	uint8_t nss, struct psoc_phy_config *phy_config)
{
	...
	if (nss == 4) {
		...
		mcs_map =
			QDF_MIN(mcs_map, (bss_vht_cap->rx_mcs_map & 0xC0) >> 6);
	} else if (nss == 3) {
		mcs_map = (self_rx_mcs_map & 0x30) >> 4;
		mcs_map =
			QDF_MIN(mcs_map, (bss_vht_cap->rx_mcs_map & 0x30) >> 4);
	} else if (nss == 2) {
		mcs_map = (self_rx_mcs_map & 0x0C) >> 2;
		mcs_map =
			QDF_MIN(mcs_map, (bss_vht_cap->rx_mcs_map & 0x0C) >> 2);
	} else {
...
static uint32_t calculate_bit_per_tone(int32_t rssi, enum phy_ch_width ch_width)
{
...
static uint32_t cm_calculate_etp(struct wlan_objmgr_psoc *psoc,
				 struct scan_cache_entry *entry,
				 struct etp_params *etp_param, uint8_t max_nss,
				 enum phy_ch_width ch_width, bool is_ht,
				 bool is_vht, bool is_he, int8_t rssi,
				 struct psoc_phy_config *phy_config)
{
	uint16_t ntone;
	...
	max_bits_per_sc_1000x = cm_get_etp_max_bits_per_sc_1000x_for_nss(
		psoc, entry, max_nss, phy_config);
	...
	/* compute MPDU_p_PPDU */
	if (is_ht) {
		min_mpdu_ss_us_100x = cm_etp_get_min_mpdu_ss_us_100x(htcap);
		max_amsdu_len = cm_etp_get_max_amsdu_len(psoc, htcap);
		ppdu_payload_dur_us =
			etp_param->data_ppdu_dur_target_us - phy_hdr_dur_us;
		mpdu_per_ampdu = QDF_MIN(
			qdf_ceil(ppdu_payload_dur_us * 100,
				 min_mpdu_ss_us_100x),
			qdf_ceil(ppdu_payload_dur_us * (data_rate_kbps / 1000),
	...
	estimated_throughput_mbps =
		(estimated_throughput_mbps * etp_param->airtime_fraction) /
		CM_MAX_ESTIMATED_AIR_TIME_FRACTION;
	...
	mlme_nofl_debug(
		"Candidate(" QDF_MAC_ADDR_FMT
		" freq %d): rssi %d HT %d VHT %d HE %d ATF: %d NSS %d, ch_width: %d",
		QDF_MAC_ADDR_REF(entry->bssid.bytes), entry->channel.chan_freq,
		entry->rssi_raw, is_ht, is_vht, is_he,
		etp_param->airtime_fraction, entry->nss, ch_width);
	if (is_ht)
		mlme_nofl_debug(
			"min_mpdu_ss_us_100x = %d, max_amsdu_len = %d, ppdu_payload_dur_us = "
			"%d, mpdu_per_ampdu = %d, mpdu_per_ppdu = %d, ba_window_size = %d",
			min_mpdu_ss_us_100x, max_amsdu_len, ppdu_payload_dur_us,
			mpdu_per_ampdu, mpdu_per_ppdu,
			etp_param->ba_window_size);
	mlme_nofl_debug(
		"ETP score params: ntone: %d, phy_hdr_dur_us: %d, max_bits_per_sc_1000x: "
		"%d, log_2_snr_tone_1000x: %d mpdu_p_ppdu = %d, max_amsdu_len = %d, "
		"ppdu_dur_us = %d, total score = %d",
...
static uint32_t cm_calculate_etp_score(struct wlan_objmgr_psoc *psoc,
				       struct scan_cache_entry *entry,
				       struct psoc_phy_config *phy_config,
				       enum MLO_TYPE bss_mlo_type)
{
	...
	nss = cm_get_sta_nss(psoc, entry->channel.chan_freq,
			     phy_config->vdev_nss_24g, phy_config->vdev_nss_5g);
	...
	if (entry->air_time_fraction) {
		...
		esp_ie = (struct wlan_esp_ie *)util_scan_entry_esp_info(entry);
		if (esp_ie) {
			esp = &esp_ie->esp_info_AC_BE;
			etp_param.ba_window_size =
				cm_etp_get_ba_win_size_from_esp(
					esp->ba_window_size);
   ...
   * range: [1, 20000]
   */
	score = cm_calculate_etp(psoc, entry, &etp_param, nss, ch_width,
				 is_ht_intersect, is_vht_intersect,
				 is_he_intersect, entry->rssi_raw, phy_config);
	if (bss_mlo_type == SLO)
		return score;
	...
	if (mlo_prefer_percentage) {
		mlo_score = score;
		mlo_score =
			mlo_score + (mlo_score * mlo_prefer_percentage) / 100;
		score = mlo_score;
	}
	return score;
}
 ...
 */
static int cm_get_band_score(uint32_t freq, struct scoring_cfg *score_config)
{
	uint8_t band_index;
...
uint16_t link_bw_score[CM_BAND_WIDTH_NUM] = { 9,  18, 27, 35, 44, 53, 56, 67,
					      74, 80, 86, 90, 93, 96, 98, 100 };
				...
				uint8_t prorated_pcnt)
{
	...
	score = bw_weightage * link_bw_score[index] * prorated_pcnt /
		CM_MAX_PCT_SCORE;
...
static struct mlo_rssi_pct mlo_rssi_pcnt[CM_RSSI_BUCKET_NUM] = {
	{ 80, 100, 100 }, { 60, 87, 100 }, { 44, 74, 100 }, { 30, 61, 100 },
	{ 20, 48, 54 },	  { 10, 35, 28 },  { 0, 22, 1 }
};
				      ...
				      int8_t link2_rssi, uint16_t *prorate_pcnt)
{
	...
	joint_factor = QDF_MIN(
		(link1_factor * link1_factor + link2_factor * link2_factor) *
			100 / (2 * 45 * 45),
		100);
					     ...
					     struct qdf_mac_addr *link_addr)
{
	...
	while (cur_node) {
		curr_entry = qdf_container_of(cur_node, struct scan_cache_node,
					      node);
		if (!qdf_mem_cmp(&curr_entry->entry->mac_addr, link_addr,
				 QDF_MAC_ADDR_SIZE))
...
static bool is_freq_dbs_or_sbs(struct wlan_objmgr_psoc *psoc, qdf_freq_t freq_1,
			       qdf_freq_t freq_2)
{
...
#else
static inline uint8_t cm_get_sta_mlo_conn_max_num(struct wlan_objmgr_psoc *psoc)
{
				     ...
				     qdf_list_t *scan_list)
{
	...
	for (i = 0; i < entry->ml_info.num_links; i++) {
		...
		entry_partner[i] = cm_get_entry(
			scan_list, &entry->ml_info.link_info[i].link_addr);
		if (entry_partner[i])
 ...
 */
static uint32_t cm_get_mlo_congestion_score(uint16_t bw1, uint16_t bw2,
 ...
 */
static int cm_calculate_mlo_bss_score(
	struct wlan_objmgr_psoc *psoc, struct scan_cache_entry *entry,
	struct scoring_cfg *score_params, struct psoc_phy_config *phy_config,
	qdf_list_t *scan_list, uint8_t *rssi_prorated_pct, int pcl_chan_weight)
{
	struct scan_cache_entry *entry_partner[MLD_MAX_LINKS - 1];
	...
	cong_score = cm_calculate_congestion_score(entry, score_params,
						   &cong_pct, false);

	assoc_score = cm_calculate_bss_score(psoc, entry, pcl_chan_weight, NULL,
					     scan_list, true);
	entry->ml_info.link_score = assoc_score;

	assoc_band_score =
		cm_get_band_score(entry->channel.chan_freq, score_params);
	...
	for (i = 0; i < entry->ml_info.num_links; i++) {
		...
		if (entry_partner[i])
			...
			freq[i] = link[i].freq;
		if (!is_freq_dbs_or_sbs(psoc, freq[i], freq_entry)) {
			mlme_nofl_debug("freq %d and %d can't be MLMR", freq[i],
					freq_entry);
			continue;
		}

		if (entry_partner[i]) {
			link_score[i] = cm_calculate_bss_score(
				psoc, entry_partner[i], pcl_chan_weight, NULL,
				scan_list, true);
			entry_partner[i]->ml_info.link_score = link_score[i];

			rssi[i] = entry_partner[i]->rssi_raw;
			ch_width[i] =
				cm_get_ch_width(entry_partner[i], phy_config);
		} else {
			rssi[i] = cm_estimate_rssi(entry->rssi_raw, freq_entry,
						   freq[i]);
			pdev = psoc->soc_objmgr.wlan_pdev_list[0];
			ch_width[i] = wlan_reg_get_op_class_width(
				pdev, link[i].op_class, true);
			mlme_nofl_debug(
				"No entry for partner, estimate with rnr");
		}
		rssi_score[i] = cm_get_mlo_rssi_score(
			weight_config->rssi_weightage, entry->rssi_raw, rssi[i],
			&prorated_pct[i]);

		bandwidth_score[i] = cm_get_bw_score(
			weight_config->chan_width_weightage,
			chan_width + ch_width[i], prorated_pct[i]);

		rssi_bad_zone = prorated_pct[i] < CM_MAX_PCT_SCORE;
		congestion_score[i] = cm_calculate_congestion_score(
			entry_partner[i], score_params, &congestion_pct[i],
			rssi_bad_zone);
		cong_total_score[i] = cm_get_mlo_congestion_score(
			chan_width, ch_width[i], cong_score,
			congestion_score[i], score_params);
		...
		mlme_nofl_debug(
			"ML score: link index %u rssi %d %d rssi score %u pror %u "
			"freq %u %u bw %u %u, bw score %u congest score %u %u %u, "
			"band score: %u %u, total %u",
			i, entry->rssi_raw, rssi[i], rssi_score[i],
			prorated_pct[i], freq_entry, freq[i], chan_width,
			ch_width[i], bandwidth_score[i], cong_score,
			congestion_score[i], cong_total_score[i],
			assoc_band_score, link_band_score[i], total_score[i]);
	}
	...
	*rssi_prorated_pct = prorated_pct[best_partner_index];
	...
	best_total_score += weight_config->mlo_weightage * mlo_boost_pct[MLMR];
...
static void cm_sort_vendor_algo_mlo_bss_entry(
	struct wlan_objmgr_psoc *psoc, struct scan_cache_entry *entry,
	struct psoc_phy_config *phy_config, qdf_list_t *scan_list,
	enum MLO_TYPE bss_mlo_type)
{
	struct scan_cache_entry *entry_partner[MLD_MAX_LINKS - 1];
	uint32_t freq[MLD_MAX_LINKS - 1];
	...
	for (i = 0; i < entry->ml_info.num_links; i++) {
		mlme_debug(
			"freq: %d, link id: %d is valid %d " QDF_MAC_ADDR_FMT,
			link[i].freq, link[i].link_id, link[i].is_valid_link,
				  ...
				  qdf_list_t *scan_list, bool is_link_score)
{
	...
	if (score_config->is_bssid_hint_priority && bssid_hint &&
		...
		mlme_nofl_debug(
			"Candidate(" QDF_MAC_ADDR_FMT
			" freq %d): rssi %d BSSID hint given, give max score %d",
			QDF_MAC_ADDR_REF(entry->bssid.bytes),
	...
	if (score_config->vendor_roam_score_algorithm) {
		...
		mlme_nofl_debug("Candidate(" QDF_MAC_ADDR_FMT
				" freq %d): rssi %d score %d",
				QDF_MAC_ADDR_REF(entry->bssid.bytes),
				entry->channel.chan_freq, entry->rssi_raw,
				entry->bss_score);
		return score;
	...
	if (is_link_score || bss_mlo_type == SLO || bss_mlo_type == MLSR) {
		rssi_score = cm_calculate_rssi_score(
			&score_config->rssi_score, entry->rssi_raw,
			weight_config->rssi_weightage);
		prorated_pcnt = cm_get_rssi_prorate_pct(
			&score_config->rssi_score, entry->rssi_raw,
			weight_config->rssi_weightage);
		score += rssi_score;
		bandwidth_score = cm_get_bw_score(
			weight_config->chan_width_weightage,
			cm_get_ch_width(entry, phy_config), prorated_pcnt);
		score += bandwidth_score;

		congestion_score = cm_calculate_congestion_score(
			entry, score_config, &congestion_pct, 0);
   ...
   */
	...
	if (entry->rssi_raw < good_rssi_threshold)
		same_bucket = cm_rssi_is_same_bucket(
			good_rssi_threshold, entry->rssi_raw,
			rssi_pref_5g_rssi_thresh,
	...
	if (vht_cap && vht_cap->su_beam_former) {
		ap_su_beam_former = true;
					    ...
					    WLAN_HE_PHYCAP_SU_BFER_OFFSET),
					  WLAN_HE_PHYCAP_SU_BFER_IDX,
	...
	if (phy_config->beamformee_cap && is_vht && ap_su_beam_former &&
   ...
   */
	...
	oce_ap_tx_pwr_score = cm_calculate_oce_ap_tx_pwr_weightage(
		entry, score_config, &ap_tx_pwr_dbm);
	score += oce_ap_tx_pwr_score;

	oce_subnet_id_score = cm_calculate_oce_subnet_id_weightage(
		entry, score_config, &oce_subnet_id_present);
   ...
   */
	security_score =
		cm_calculate_security_score(score_config, entry->neg_sec_info);
	...
	mlme_nofl_debug(
		"Candidate(" QDF_MAC_ADDR_FMT
		" freq %d): rssi %d HT %d VHT %d HE %d EHT %d su bfer %d phy %d  air "
		"time frac %d qbss %d cong_pct %d NSS %d ap_tx_pwr_dbm %d "
		"oce_subnet_id_present %d sae_pk_cap_present %d prorated_pcnt %d keymgmt "
		"0x%x mlo type %d",
		QDF_MAC_ADDR_REF(entry->bssid.bytes), entry->channel.chan_freq,
		entry->rssi_raw, util_scan_entry_htcap(entry) ? 1 : 0,
		util_scan_entry_vhtcap(entry) ? 1 : 0,
		util_scan_entry_hecap(entry) ? 1 : 0,
		util_scan_entry_ehtcap(entry) ? 1 : 0, ap_su_beam_former,
		entry->phy_mode, entry->air_time_fraction,
		entry->qbss_chan_load, congestion_pct, entry->nss,
		ap_tx_pwr_dbm, oce_subnet_id_present, sae_pk_cap_present,
		prorated_pcnt, entry->neg_sec_info.key_mgmt, bss_mlo_type);

	mlme_nofl_debug(
		"%s score: rssi %d pcl %d ht %d vht %d he %d bfee %d bw %d "
		"band %d congestion %d nss %d oce wan %d oce ap tx pwr %d "
		"subnet %d sae_pk %d eht %d security %d TOTAL %d",
		is_link_score ? "Link" : "AP", rssi_score, pcl_score, ht_score,
		vht_score, he_score, beamformee_score, bandwidth_score,
		band_score, congestion_score, nss_score, oce_wan_score,
		oce_ap_tx_pwr_score, oce_subnet_id_score, sae_pk_score,
		eht_score, security_score, score);
 ...
 */
static bool cm_is_bad_rssi_entry(struct scan_cache_entry *scan_entry,
				 struct scoring_cfg *score_config,
				 struct qdf_mac_addr *bssid_hint)
{
	...
	if (score_config->is_bssid_hint_priority &&
		...
		mlme_nofl_debug(
			"Candidate(  " QDF_MAC_ADDR_FMT
			"  freq %d): remove entry, rssi %d lower than rssi_threshold %d",
			QDF_MAC_ADDR_REF(scan_entry->bssid.bytes),
			scan_entry->channel.chan_freq, scan_entry->rssi_raw,
			rssi_threshold);
		return true;
	}
					  ...
					  struct qdf_mac_addr *self_mac)
{
	...
	if (QDF_IS_LAST_3_BYTES_OF_MAC_SAME(self_mac, scan_entry_bssid)) {
		mlme_nofl_debug(
			"Candidate(" QDF_MAC_ADDR_FMT
			" freq %d): boost bss score due to same last 3 byte match",
			QDF_MAC_ADDR_REF(scan_entry_bssid->bytes),
			scan_entry->entry->channel.chan_freq);
		scan_entry->entry->bss_score = CM_BEST_CANDIDATE_MAX_BSS_SCORE;
	}
}
#else
static inline bool cm_is_bad_rssi_entry(struct scan_cache_entry *scan_entry,
					struct scoring_cfg *score_config,
					struct qdf_mac_addr *bssid_hint)
				 ...
				 struct qdf_mac_addr *self_mac)
{
	...
	mlme_nofl_debug(
		"Self caps: HT %d VHT %d HE %d EHT %d VHT_24Ghz %d BF cap %d "
		"bw_above_20_24ghz %d bw_above_20_5ghz %d 2.4G NSS %d 5G NSS %d",
		config->ht_cap, config->vht_cap, config->he_cap,
		config->eht_cap, config->vht_24G_cap, config->beamformee_cap,
		config->bw_above_20_24ghz, config->bw_above_20_5ghz,
		config->vdev_nss_24g, config->vdev_nss_5g);
	...
	while (cur_node) {
		...
		assoc_allowed =
			cm_is_assoc_allowed(mlme_psoc_obj, scan_entry->entry);
		...
		if (denylist_action == CM_DLM_NO_ACTION && pcl_lst &&
		    pcl_lst->num_of_pcl_channels &&
		...
		if (denylist_action == CM_DLM_NO_ACTION ||
			...
			mlme_nofl_debug(
				"Candidate(" QDF_MAC_ADDR_FMT
				" freq %d): rssi %d, is in Avoidlist, give min score %d",
				QDF_MAC_ADDR_REF(
     ...
     */
		if (denylist_action == CM_DLM_REMOVE &&
			...
			if (!force_connect_candidate) {
				force_connect_candidate = qdf_mem_malloc(
		...
		if (QDF_IS_STATUS_ERROR(status)) {
			mlme_err(
				"failed to remove node for BSS " QDF_MAC_ADDR_FMT
				" from scan list",
				QDF_MAC_ADDR_REF(
					scan_entry->entry->bssid.bytes));
			return;
     ...
     */
		if (denylist_action == CM_DLM_REMOVE ||
			...
			if (assoc_allowed && !is_rssi_bad)
				mlme_nofl_debug(
					"Candidate( " QDF_MAC_ADDR_FMT
					" freq %d): rssi %d, dlm action %d is in Denylist, remove entry",
					QDF_MAC_ADDR_REF(
						scan_entry->entry->bssid.bytes),
					scan_entry->entry->channel.chan_freq,
					scan_entry->entry->rssi_raw,
	...
	if (are_all_candidate_denylisted && force_connect_candidate) {
		mlme_nofl_debug(
			"All candidates in denylist, Candidate( " QDF_MAC_ADDR_FMT
			" freq %d): rssi %d, selected for connection",
			QDF_MAC_ADDR_REF(
				force_connect_candidate->entry->bssid.bytes),
   ...
   */
     ...
     */
		if (!(config->key_mgmt_mask_6ghz & key_mgmt)) {
			mlme_debug(
				"user configured mask %x didn't match AKM %x",
...
void wlan_cm_set_check_6ghz_security(struct wlan_objmgr_psoc *psoc, bool value)
{
	struct psoc_mlme_obj *mlme_psoc_obj;
					       ...
					       bool value)
{
	...
	mlme_psoc_obj->psoc_cfg.score_config.disable_vlp_sta_conn_to_sp_ap =
		value;
}
...
bool wlan_cm_get_disable_vlp_sta_conn_to_sp_ap(struct wlan_objmgr_psoc *psoc)
{
	...
	return mlme_psoc_obj->psoc_cfg.score_config
...
static uint32_t cm_limit_max_per_index_score(uint32_t per_index_score)
{
	uint8_t i, score;
...
static void cm_init_bw_weight_per_index(struct wlan_objmgr_psoc *psoc,
					struct scoring_cfg *score_cfg)
{
	score_cfg->bandwidth_weight_per_index[0] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX));

	score_cfg->bandwidth_weight_per_index[1] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_4_TO_7));

	score_cfg->bandwidth_weight_per_index[2] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_8_TO_11));

	score_cfg->bandwidth_weight_per_index[3] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_12_TO_15));

	score_cfg->bandwidth_weight_per_index[4] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_16_TO_19));

	score_cfg->bandwidth_weight_per_index[5] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_20_TO_23));

	score_cfg->bandwidth_weight_per_index[6] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_24_TO_27));

	score_cfg->bandwidth_weight_per_index[7] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_BW_WEIGHT_PER_IDX_28_TO_31));

	score_cfg->bandwidth_weight_per_index[8] = cm_limit_max_per_index_score(
...
static void cm_init_nss_weight_per_index(struct wlan_objmgr_psoc *psoc,
					 struct scoring_cfg *score_cfg)
{
	score_cfg->nss_weight_per_index[0] = cm_limit_max_per_index_score(
		cfg_get(psoc, CFG_SCORING_NSS_WEIGHT_PER_IDX));

	score_cfg->nss_weight_per_index[1] = cm_limit_max_per_index_score(
			       ...
			       struct scoring_cfg *score_cfg)
{
   ...
   */
	...
	score_cfg->band_weight_per_index = cm_limit_max_per_index_score(
