...
{
	if (rcc_param->m_directed_ftm || rcc_param->m_all_ftm_ack ||
	    rcc_param->m_ndpa_ndp_directed || rcc_param->m_ndpa_ndp_all ||
	    rcc_param->m_ta_ra_filter || rcc_param->m_all_packet)
		return true;
	else
		return false;
}
				     ...
				     struct cfr_capture_params *params)
{
     ...
     */
		if (qdf_mem_cmp(&pa->unassoc_pool[idx].mac, unassoc_mac,
			...
			qdf_mem_copy(&pa->unassoc_pool[idx].cfr_params, params,
   ...
   */
	qdf_mem_copy(&pa->unassoc_pool[idx_to_insert].mac, unassoc_mac,
		     sizeof(struct qdf_mac_addr));
	qdf_mem_copy(&pa->unassoc_pool[idx_to_insert].cfr_params, params,
		     sizeof(struct cfr_capture_params));
...
static void cfr_iter_peer_handler(struct wlan_objmgr_pdev *pdev, void *object,
				  void *arg)
{
	...
	if (pe->period && (pe->request == PEER_CFR_CAPTURE_ENABLE)) {
		*cfr_capt_status = pe->request;
		cfr_debug("CFR capture running for peer " QDF_MAC_ADDR_FMT,
				 ...
				 enum cfr_capt_status *status)
{
	*status = PEER_CFR_CAPTURE_DISABLE;
	...
	wlan_objmgr_pdev_iterate_obj_list(pdev, WLAN_PEER_OP,
					  cfr_iter_peer_handler, status, 1,
					  WLAN_CFR_ID);
}
			   ...
			   struct csi_cfr_header *hdr, uint32_t mem_index)
{
	...
	vdev = wlan_objmgr_get_vdev_by_id_from_psoc(psoc, vdev_id, WLAN_CFR_ID);
	if (!vdev) {
   ...
   */
	...
	tgt_cfr_info_send(pdev, hdr, sizeof(struct csi_cfr_header), payload,
			  payload_len, &end_magic_num, sizeof(uint32_t));
...
static QDF_STATUS dev_sanity_check(struct wlan_objmgr_vdev *vdev,
				     ...
				     struct cfr_wlanconfig_param *params)
{
	...
	tgt_cfr_default_ta_ra_cfg(pdev, &pcfr->rcc_param, true,
				  params->reset_cfg);
				...
				struct cfr_wlanconfig_param *params)
{
	...
	if (params->grp_id >= MAX_TA_RA_ENTRIES) {
		cfr_err("err parameter grp_id, value=%u, max=%u\n",
			params->grp_id, MAX_TA_RA_ENTRIES);
		return QDF_STATUS_E_INVAL;
	}
	...
	qdf_set_bit(params->grp_id, &pcfr->rcc_param.modified_in_curr_session);
				    ...
				    struct cfr_wlanconfig_param *params)
{
	...
	qdf_mem_copy(curr_cfg->tx_addr_mask, params->ta_mask,
		     QDF_MAC_ADDR_SIZE);
	qdf_mem_copy(curr_cfg->rx_addr_mask, params->ra_mask,
		     QDF_MAC_ADDR_SIZE);
...
QDF_STATUS ucfg_cfr_get_cfg(struct wlan_objmgr_vdev *vdev)
{
	...
	cfr_err("m_directed_ftm is : %s\n",
		pcfr->rcc_param.m_directed_ftm ? "enabled" : "disabled");
	cfr_err("m_all_ftm_ack is : %s\n",
		pcfr->rcc_param.m_all_ftm_ack ? "enabled" : "disabled");
	cfr_err("m_ndpa_ndp_directed is: %s\n",
		pcfr->rcc_param.m_ndpa_ndp_directed ? "enabled" : "disabled");
	cfr_err("m_ndpa_ndp_all is : %s\n",
		pcfr->rcc_param.m_ndpa_ndp_all ? "enabled" : "disabled");
	cfr_err("m_ta_ra_filter is : %s\n",
		pcfr->rcc_param.m_ta_ra_filter ? "enabled" : "disabled");
	cfr_err("m_all_packet is : %s\n",
		pcfr->rcc_param.m_all_packet ? "enabled" : "disabled");
	...
	cfr_err("Freeze TLV delay count is : %s\n",
		pcfr->rcc_param.freeze_tlv_delay_cnt_en ? "enabled" :
							  "disabled");
...
static const char *mac_freeze_reason_to_str(enum mac_freeze_capture_reason type)
{
	switch (type) {
			   ...
			   bool cfr_enable_monitor_mode)
{
	...
	cdp_cfr_filter(wlan_psoc_get_dp_handle(psoc),
		       wlan_objmgr_pdev_get_pdev_id(pdev), enable, filter_val,
   ...
   */
	...
	if (cfr_is_filter_enabled(&pcfr->rcc_param)) {
		...
		if (pcfr->rcc_param.m_all_ftm_ack) {
			filter_val.mode |= MON_FILTER_PASS | MON_FILTER_OTHER;
	...
	if (status == QDF_STATUS_SUCCESS) {
		...
		tgt_cfr_default_ta_ra_cfg(pdev, &pcfr->rcc_param, false,
					  MAX_RESET_CFG_ENTRY);
	} else {
...
bool ucfg_cfr_get_rcc_enabled(struct wlan_objmgr_vdev *vdev)
{
	...
	if ((pcfr->rcc_param.vdev_id != CFR_INVALID_VDEV_ID) &&
		...
		cfr_debug("vdev id mismatch, input %d, pcfr %d",
			  wlan_vdev_get_id(vdev), pcfr->rcc_param.vdev_id);
		return false;
	}
