...
{
	struct wlan_dfs *dfs;
	dfs = wlan_objmgr_pdev_get_comp_private_obj(pdev, WLAN_UMAC_COMP_DFS);
...
register_dfs_postnol_csa_callback(struct dfs_to_mlme *mlme_callback)
{
	...
	mlme_callback->mlme_postnol_chan_switch = mlme_dfs_postnol_chan_switch;
}
#else
static inline void
register_dfs_postnol_csa_callback(struct dfs_to_mlme *mlme_callback)
...
register_dfs_unpunc_chan_switch_callback(struct dfs_to_mlme *mlme_callback)
{
	...
	mlme_callback->mlme_unpunc_chan_switch = mlme_dfs_unpunc_chan_switch;
}
#else
static inline void
register_dfs_unpunc_chan_switch_callback(struct dfs_to_mlme *mlme_callback)
	...
	struct dfs_to_mlme *tmp_dfs_to_mlme)
{
	...
	tmp_dfs_to_mlme->mlme_proc_spoof_success = mlme_dfs_proc_spoof_success;
}
#else
...
void register_dfs_callbacks(void)
{
	...
	tmp_dfs_to_mlme->mlme_is_opmode_sta = mlme_dfs_is_opmode_sta;
	...
	tmp_dfs_to_mlme->mlme_bringdown_vaps = mlme_dfs_bringdown_vaps;
	tmp_dfs_to_mlme->mlme_dfs_deliver_event = mlme_dfs_deliver_event;
 ...
 */
static QDF_STATUS
dfs_psoc_obj_create_notification(struct wlan_objmgr_psoc *psoc, void *arg_list)
{
	QDF_STATUS status;
	...
	status = wlan_objmgr_psoc_component_obj_attach(psoc, WLAN_UMAC_COMP_DFS,
						       (void *)dfs_soc_obj,
						       QDF_STATUS_SUCCESS);
	...
	dfs_debug(NULL, WLAN_DEBUG_DFS1, "DFS obj attach to psoc successfully");
 ...
 */
static QDF_STATUS
dfs_psoc_obj_destroy_notification(struct wlan_objmgr_psoc *psoc, void *arg_list)
{
	QDF_STATUS status;
	struct dfs_soc_priv_obj *dfs_soc_obj;
	...
	status = wlan_objmgr_psoc_component_obj_detach(psoc, WLAN_UMAC_COMP_DFS,
						       dfs_soc_obj);
...
QDF_STATUS dfs_init(void)
{
	...
	status = wlan_objmgr_register_psoc_create_handler(
		WLAN_UMAC_COMP_DFS, dfs_psoc_obj_create_notification, NULL);
	...
	status = wlan_objmgr_register_psoc_destroy_handler(
		WLAN_UMAC_COMP_DFS, dfs_psoc_obj_destroy_notification, NULL);
	...
	status = wlan_objmgr_register_pdev_destroy_handler(
		WLAN_UMAC_COMP_DFS, wlan_dfs_pdev_obj_destroy_notification,
		NULL);
	...
	status = qdf_print_set_category_verbose(
		qdf_get_pidx(), QDF_MODULE_ID_DFS, QDF_TRACE_LEVEL_DEBUG, true);
						 ...
						 void *arg)
{
	...
	status = global_dfs_to_mlme.pdev_component_obj_attach(
		pdev, WLAN_UMAC_COMP_DFS, (void *)dfs, QDF_STATUS_SUCCESS);
	if (QDF_IS_STATUS_ERROR(status)) {
						  ...
						  void *arg)
{
	...
	if (dfs) {
		...
		dfs_detach(dfs);
		global_dfs_to_mlme.pdev_component_obj_detach(
			pdev, WLAN_UMAC_COMP_DFS, (void *)dfs);
...
QDF_STATUS wifi_dfs_psoc_enable(struct wlan_objmgr_psoc *psoc)
{
	...
	if (status != QDF_STATUS_SUCCESS) {
		dfs_err(NULL, WLAN_DEBUG_DFS_ALWAYS,
			"tgt_dfs_reg_ev_handler failed");
		return QDF_STATUS_E_FAILURE;
	}

	status = wlan_serialization_register_comp_info_cb(
		psoc, WLAN_UMAC_COMP_DFS, WLAN_SER_CMD_SCAN,
		dfs_scan_serialization_comp_info_cb);
	if (status != QDF_STATUS_SUCCESS) {
		dfs_err(NULL, WLAN_DEBUG_DFS_ALWAYS,
			"Serialize scan cmd register failed");
		return status;
	}
...
QDF_STATUS wifi_dfs_psoc_disable(struct wlan_objmgr_psoc *psoc)
{
	...
	if (status != QDF_STATUS_SUCCESS) {
		dfs_err(NULL, WLAN_DEBUG_DFS_ALWAYS,
			"Serialize scan cmd deregister failed");
		return status;
	}
