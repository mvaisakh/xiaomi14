					   ...
					   uint8_t *num_channels)
{
	...
	if (*num_channels > MAX_20MHZ_SUBCHANS) {
		dfs_err(dfs, WLAN_DEBUG_DFS, "Invalid num channels: %d",
			*num_channels);
		return QDF_STATUS_E_FAILURE;
	}

	for (i = 0; i < *num_channels; i++) {
		if (freq_list[i] == 0 || freq_list[i] == last_chan_freq)
			continue;
		...
		DFS_NOL_ADD_CHAN_LOCKED(dfs, freq_list[i],
					dfs->wlan_dfs_nol_timeout);
		nol_freq_list[num_ch++] = last_chan_freq;
		utils_dfs_deliver_event(dfs->dfs_pdev_obj, freq_list[i],
					WLAN_EV_NOL_STARTED);
	...
	*num_channels = num_ch;
	...
	dfs_update_cac_elements(dfs, nol_freq_list, num_ch, NULL,
				WLAN_EV_NOL_STARTED);

	if (!dfs_get_disable_radar_marking(dfs)) {
		utils_dfs_reg_update_nol_chan_for_freq(
			dfs->dfs_pdev_obj, nol_freq_list, num_ch, DFS_NOL_SET);
...
dfs_find_20mhz_subchans_from_offsets(struct freqs_offsets *freq_offset,
				     uint32_t center_freq, uint16_t ch_width)
{
...
#ifdef CONFIG_CHAN_FREQ_API
void dfs_compute_radar_found_cfreq(struct wlan_dfs *dfs,
 ...
 */
#ifdef CONFIG_CHAN_FREQ_API
static uint8_t dfs_find_radar_affected_subchans_for_freq(
	struct wlan_dfs *dfs, struct radar_found_info *radar_found,
	uint16_t *freq_list, uint32_t freq_center)
{
	int i, j;
	...
	dfs_info(dfs, WLAN_DEBUG_DFS,
		 ...
		 radar_found->freq_offset, radar_found->is_chirp, flag,
		 freq_center);

	ch_width = dfs_chan_to_ch_width(curchan);
	if (ch_width == BW_INVALID) {
		dfs_err(dfs, WLAN_DEBUG_DFS, "channel flag=%d is invalid",
			flag);
		return 0;
	}
	...
	n_cur_subchans = dfs_get_bonding_channels_for_freq(
		dfs, curchan, radar_found->segment_id, radar_found->detector_id,
		cur_subchans);
 ...
 * Return: void
 */
static void dfs_calc_bonding_freqs(qdf_freq_t center_freq, uint16_t ch_width,
				   uint16_t *freq_list)
{
#define CHAN_SPACING_MHZ_5G 20
#define SUB20CHAN_BW_MHZ_5G 20
	uint8_t nchans = ch_width / CHAN_SPACING_MHZ_5G;
	qdf_freq_t first_subchan_cfreq =
		center_freq - (ch_width / 2) + (SUB20CHAN_BW_MHZ_5G / 2);
	uint8_t i;
 ...
 * Return: void
 */
static void dfs_get_20mhz_bonding_channels(uint16_t center_freq,
					   uint16_t *freq_list)
{
 ...
 * Return: void
 */
static void dfs_get_40mhz_bonding_channels(uint16_t center_freq,
					   uint16_t *freq_list)
{
 ...
 * Return: void
 */
static void dfs_get_80mhz_bonding_channels(uint16_t center_freq,
					   uint16_t *freq_list)
{
 ...
 * Return: void
 */
static void dfs_get_160mhz_bonding_channels(uint16_t center_freq,
					    uint16_t *freq_list)
{
...
static void dfs_get_320mhz_bonding_channels(uint16_t center_freq,
					    uint16_t *freq_list,
					    uint8_t *nchannels)
{
						  ...
						  uint16_t *freq_list)
{
	...
	if (WLAN_IS_CHAN_MODE_20(chan)) {
		...
		dfs_get_80mhz_bonding_channels(center_freq, freq_list + 4);
 ...
 */
static void dfs_get_agile_subchans_for_curchan_160(struct wlan_dfs *dfs,
						   uint16_t center_freq,
						   uint32_t segment_id,
						   uint16_t *freq_list,
						   uint8_t *nchannels)
					  ...
					  uint16_t *freq_list)
{
   ...
   */
	...
	if (WLAN_IS_CHAN_MODE_20(curchan)) {
		...
		nchannels = 8;
		if (detector_id == dfs_get_agile_detector_id(dfs))
			dfs_get_agile_subchans_for_curchan_160(dfs, center_freq,
							       segment_id,
							       freq_list,
							       &nchannels);
		else
 ...
 * return true.
 */
static bool dfs_radar_found_event_basic_sanity(struct wlan_dfs *dfs,
				...
				struct radar_found_info *radar_found)
{
	...
	if (dfs->dfs_is_radar_found_chan_freq_eq_center_freq) {
		dfs_debug(
			dfs, WLAN_DEBUG_DFS,
			"There is no radar translation required for chips where HALPHY "
			"reports the exact radar found chan's center freq\n");
		return;
	}
...
#ifdef CONFIG_CHAN_FREQ_API
uint8_t dfs_find_radar_affected_channels(struct wlan_dfs *dfs,
					 struct radar_found_info *radar_found,
					 uint16_t *freq_list,
					 uint32_t freq_center)
{
	...
	/* BW reduction is dependent on subchannel marking */
		...
		num_channels = dfs_find_radar_affected_subchans_for_freq(
			dfs, radar_found, freq_list, freq_center);
	else
		num_channels = dfs_get_bonding_channels_for_freq(
			dfs, dfs->dfs_curchan, radar_found->segment_id,
 ...
 * Return: QDF_STATUS
 */
static bool dfs_is_radarsource_agile(struct wlan_dfs *dfs,
				     struct radar_found_info *radar_found)
{
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS_AGILE, "radar on PreCAC segment: ADFS:%d",
				   ...
				   uint8_t num_radar_channels)
{
	...
	n_cur_channels = dfs_get_bonding_channel_without_seg_info_for_freq(
		dfs->dfs_curchan, cur_freq_list);
				   ...
				   struct radar_found_info *radar_found)
{
   ...
   */
	...
	if (radar_found->segment_id == SEG_ID_SECONDARY)
		dfs_debug(
			...
			"Radar found on second segment.Radarfound Freq=%d MHz.Secondary "
			"Chan cfreq=%d MHz.",
			radarfound_freq, freq_center);
	else
		dfs_debug(
			NULL, WLAN_DEBUG_DFS_ALWAYS,
			"Radar found on channel=%d, freq=%d MHz. Primary beaconning "
			"chan:%d, freq=%d MHz.",
			utils_dfs_freq_to_chan(radarfound_freq),
			radarfound_freq, dfs_curchan->dfs_ch_ieee,
			dfs_curchan->dfs_ch_freq);

	num_channels = dfs_find_radar_affected_channels(dfs, radar_found,
							freq_list, freq_center);
	...
	if (dfs->dfs_use_puncture && !dfs->dfs_is_stadfs_enabled) {
		...
		dfs_handle_radar_puncturing(dfs, &dfs_radar_bitmap, freq_list,
					    num_channels,
	...
	status = dfs_radar_add_channel_list_to_nol_for_freq(
		dfs, freq_list, nol_freq_list, &num_channels);
   ...
   * channel.
   */
	dfs_debug(dfs, WLAN_DEBUG_DFS, "found_on_second=%d is_pre=%d",
   ...
   */
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS, "Radar found on dfs detector: %d",
		  radar_found->detector_id);
	dfs_mark_precac_nol_for_freq(dfs, dfs->is_radar_found_on_secondary_seg,
				     radar_found->detector_id, nol_freq_list,
				     num_channels);

	dfs_send_nol_ie_and_rcsa(dfs, radar_found, nol_freq_list, num_channels,
				 &wait_for_csa);
   ...
   */
	...
	dfs_mlme_mark_dfs(dfs->dfs_pdev_obj, dfs->dfs_curchan->dfs_ch_ieee,
			  dfs->dfs_curchan->dfs_ch_freq,
			  dfs->dfs_curchan->dfs_ch_mhz_freq_seg2,
			  dfs->dfs_curchan->dfs_ch_flags, dfs_radar_bitmap);
