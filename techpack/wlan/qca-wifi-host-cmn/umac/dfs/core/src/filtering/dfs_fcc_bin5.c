			 ...
			 struct dfs_bin5radars *br)
{
	...
	if (DFS_EVENT_NOTCHIRP(re)) {
		dfs_debug(
			dfs, WLAN_DEBUG_DFS_BIN5,
			"rejecting chirp: ts=%llu, dur=%d, rssi=%d checkchirp=%d, "
			"hwchirp=%d, swchirp=%d",
			(unsigned long long)re->re_full_ts, (int)re->re_dur,
			(int)re->re_rssi,
			!!(re->re_flags & DFS_EVENT_CHECKCHIRP),
			!!(re->re_flags & DFS_EVENT_HW_CHIRP),
			!!(re->re_flags & DFS_EVENT_SW_CHIRP));
	...
	if ((re->re_dur >= br->br_pulse.b5_mindur) &&
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5, "dur=%d, rssi=%d - adding!",
			  (int)re->re_dur, (int)re->re_rssi);
		return 1;
	}
...
int dfs_bin5_addpulse(struct wlan_dfs *dfs, struct dfs_bin5radars *br,
   ...
   */
	...
	while ((!stop) && (br->br_numelems - 1) > 0) {
		if ((thists - br->br_elems[index].be_ts) >
			...
			br->br_numelems--;
			br->br_firstelem = (br->br_firstelem + 1) &
					   DFS_MAX_B5_MASK;
			index = br->br_firstelem;
		} else {
			stop = 1;
...
static inline void dfs_calculate_bursts_for_same_rssi(
	struct wlan_dfs *dfs, struct dfs_bin5radars *br, uint32_t *bursts,
	uint32_t *numevents, uint32_t prev, uint32_t this, int *index)
{
	uint32_t rssi_diff;
     ...
     */
       ...
       */
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5, "Bin5 rssi_diff=%d",
			  rssi_diff);
	}
}

void bin5_rules_check_internal(struct wlan_dfs *dfs, struct dfs_bin5radars *br,
			       uint32_t *bursts, uint32_t *numevents,
			       uint32_t prev, uint32_t i, uint32_t this,
			       int *index)
{
	uint64_t pri = 0;
	...
	dfs_debug(
		...
		(uint64_t)pri, (uint64_t)br->br_elems[this].be_ts,
		(int)br->br_elems[this].be_dur, (int)br->br_elems[this].be_rssi,
     ...
     */
       ...
       * burst should be same (+/- margin)
       */
			dfs_calculate_bursts_for_same_rssi(
				dfs, br, bursts, numevents, prev, this, index);
		else
			dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5,
...
int dfs_bin5_check(struct wlan_dfs *dfs)
{
	...
	for (n = 0; n < dfs->dfs_rinfo.rn_numbin5radars; n++) {
       ...
       */
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5, "bursts=%u numevents=%u",
			  bursts, numevents);
		if (bursts >= br->br_pulse.b5_threshold) {
			...
			dfs_debug(
				dfs, WLAN_DEBUG_DFS_BIN5,
				"bursts=%u numevents=%u total_width=%d average_width=%d "
				"total_diff=%d average_diff=%d",
				bursts, numevents, total_width, average_width,
				total_diff, average_diff);
			dfs_debug(dfs, WLAN_DEBUG_DFS_ALWAYS,
				  "bin 5 radar detected, bursts=%d", bursts);
			return 1;
		}
	}
 ...
 */
static int dfs_check_chirping_sowl(struct wlan_dfs *dfs, void *buf,
				   uint16_t datalen, int is_ctl, int is_ext,
				   int *slope, int *is_dc)
{
#define FFT_LEN 70
...
#define MIN_CHIRPING_SLOPE 4
	...
	*is_dc = 0;
	...
	for (p = 0; p < num_fft_packets; p++) {
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5_FFT, "fft_data_ptr=0x%pK\t",
			  fft_data_ptr);
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5_FFT, "[66]=%d [69]=%d",
			  *(fft_data_ptr + FFT_LOWER_BIN_MAX_INDEX_BYTE) >> 2,
			  *(fft_data_ptr + FFT_UPPER_BIN_MAX_INDEX_BYTE) >> 2);
		fft_data_ptr += FFT_LEN;
	}

	dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5_FFT, "datalen=%d num_fft_packets=%d",
   ...
   */
	...
	if ((ctl_slope >= MIN_CHIRPING_SLOPE) ||
		...
		is_chirp = 1;
		dfs_debug(dfs,
			  WLAN_DEBUG_DFS_BIN5 | WLAN_DEBUG_DFS_BIN5_FFT |
				  WLAN_DEBUG_DFS_PHYERR_SUM,
			  "is_chirp=%d is_dc=%d", is_chirp, *is_dc);
	}
 ...
 * @is_dc: DC found
 */
static int dfs_check_chirping_merlin(struct wlan_dfs *dfs, void *buf,
				     uint16_t datalen, int is_ctl, int is_ext,
				     int *slope, int *is_dc)
{
#define ABS_DIFF(_x, _y) \
	((int)_x > (int)_y ? (int)_x - (int)_y : (int)_y - (int)_x)
...
#define UPPER_MAG_BYTE_HT20 29
   ...
   */
	for (i = 0; i < NUM_DIFFS; i++) {
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5_FFT,
			  "i=%d, delta_peak[i]=%d, delta_diff=%d", i,
			  delta_peak[i], delta_diff);
	}
   ...
   */
     ...
     */
       ...
       */
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5_FFT,
			  "i=%d, computed bin_count=%d", i, bin_count);
	}

	if (chirp_found) {
		dfs_debug(dfs,
			  WLAN_DEBUG_DFS_BIN5_FFT | WLAN_DEBUG_DFS_PHYERR_SUM,
			  "CHIRPING_YES");
	} else {
		dfs_debug(dfs,
			  WLAN_DEBUG_DFS_BIN5_FFT | WLAN_DEBUG_DFS_PHYERR_SUM,
...
int dfs_check_chirping(struct wlan_dfs *dfs, void *buf, uint16_t datalen,
...
uint8_t dfs_retain_bin5_burst_pattern(struct wlan_dfs *dfs, uint32_t diff_ts,
				      uint8_t old_dur)
{
	/*
   ...
   */
	...
	if (diff_ts < 50) {
		dfs_debug(dfs, WLAN_DEBUG_DFS_BIN5,
			  "SPLIT pulse diffTs=%u dur=%d (old_dur=%d)", diff_ts,
   ...
   */
     ...
     */
		dfs_debug(
			dfs, WLAN_DEBUG_DFS_BIN5,
			"this pulse belongs to the same burst as before, give it same "
			"dur=%d (old_dur=%d)",
			dfs->dfs_rinfo.dfs_last_bin5_dur, old_dur);
...
int dfs_get_random_bin5_dur(struct wlan_dfs *dfs, uint64_t tstamp)
{
