...
{
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    pulsedur=%d",
		  rsu->pulse_duration);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    rssi=%d", rsu->rssi);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    ischirp=%d", rsu->is_chirp);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    sidx=%d", rsu->sidx);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    raw tsf=%d", rsu->raw_tsf);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    tsf_offset=%d",
		  rsu->tsf_offset);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    cooked tsf=%d",
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    agc_total_gain=%d",
		  rsu->agc_total_gain);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "    agc_mb_gain=%d",
		  rsu->agc_mb_gain);
}
 ...
 */
static void dfs_radar_summary_parse(struct wlan_dfs *dfs, const char *buf,
				    size_t len, struct rx_radar_status *rsu)
{
	uint32_t rs[3];
	...
	if (len < sizeof(rs)) {
		dfs_debug(dfs,
			  WLAN_DEBUG_DFS_PHYERR | WLAN_DEBUG_DFS_PHYERR_SUM,
   ...
   */
	...
	rsu->sidx = dfs_sign_extend_32(
		MS(rs[RADAR_REPORT_PULSE_REG_1], RADAR_REPORT_PULSE_SIDX), 10);
	rsu->freq_offset =
					      ...
					      struct rx_search_fft_report *rsfr)
{
   ...
   */
	...
	rsfr->total_gain_db = MS(rs[SEARCH_FFT_REPORT_REG_1],
	...
	rsfr->num_str_bins_ib = MS(rs[SEARCH_FFT_REPORT_REG_2],
				   SEARCH_FFT_REPORT_NUM_STR_BINS_IB);

	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR,
		  "two 32 bit values are: %08x %08x", rs[0], rs[1]);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->total_gain_db = %d",
		  rsfr->total_gain_db);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->base_pwr_db = %d",
		  rsfr->base_pwr_db);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->fft_chn_idx = %d",
		  rsfr->fft_chn_idx);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->peak_sidx = %d",
		  rsfr->peak_sidx);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->relpwr_db = %d",
		  rsfr->relpwr_db);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->avgpwr_db = %d",
		  rsfr->avgpwr_db);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->peak_mag = %d",
		  rsfr->peak_mag);
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->num_str_bins_ib = %d",
		  rsfr->num_str_bins_ib);

	if (dfs->dfs_caps.wlan_chip_is_ht160) {
		rsfr->seg_id = MS(rs[SEARCH_FFT_REPORT_REG_3],
				  SEARCH_FFT_REPORT_SEG_ID);
		dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "rsfr->seg_id = %d",
			  rsfr->seg_id);
	}
}
 ...
 * @rssi: RSSI.
 */
static inline void dfs_check_for_false_detection(struct wlan_dfs *dfs,
						 ...
						 uint8_t rssi)
{
	...
	if (*false_detect)
		dfs_debug(
			dfs, WLAN_DEBUG_DFS_PHYERR,
			"setting false_detect to TRUE because of mb/total_gain/rssi, "
			"agc_mb_gain=%d, agc_total_gain=%d, rssi=%d",
 ...
 */
static int dfs_tlv_parse_frame(struct wlan_dfs *dfs, struct rx_radar_status *rs,
			       struct rx_search_fft_report *rsfr,
			       const char *buf, size_t len, uint8_t rssi,
			       ...
			       int16_t *psidx_diff)
{
	...
	*psidx_diff = 0;
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR, "total length = %zu bytes", len);
     ...
     */
		...
		case TAG_ID_SEARCH_FFT_REPORT:
			sfr_count++;
			dfs_radar_fft_search_report_parse(
				dfs, buf + i, MS(tlv_hdr[TLV_REG], TLV_LEN),
				rsfr);
       ...
       * saturation event.
       */
			if (PERE_IS_OVERSAMPLING(dfs) && (sfr_count == 1) &&
...
static int dfs_tlv_calc_event_freq_pulse(struct wlan_dfs *dfs,
					 ...
					 uint32_t *freq_lo, uint32_t *freq_hi)
{
	int chan_width;
	int chan_centre;
					 ...
					 uint32_t *freq_lo, uint32_t *freq_hi)
{
   ...
   */
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS_PHYERR | WLAN_DEBUG_DFS_PHYERR_SUM,
		  "delta_peak=%d, pulse_duration=%d, bin_resolution=%d.%dKHz, "
		  ...
		  total_bw / HUNDRED, (long)abs(total_bw % HUNDRED));
...
int dfs_process_phyerr_bb_tlv(struct wlan_dfs *dfs, void *buf, uint16_t datalen,
			      uint8_t rssi, uint8_t ext_rssi,
			      ...
			      struct dfs_phy_err *e)
{
   ...
   */
	...
	dfs_debug(
		dfs, WLAN_DEBUG_DFS_PHYERR_SUM,
		"fbin=%d, freq=%d.%d MHz, raw tsf=%u, offset=%d, cooked tsf=%u, "
		"rssi=%d, dur=%d, is_chirp=%d, fulltsf=%llu, freq=%d.%d MHz, "
		"freq_lo=%d.%dMHz, freq_hi=%d.%d MHz",
	...
	dfs_debug(
		dfs, WLAN_DEBUG_DFS_FALSE_DET,
		"ts=%u, dur=%d, rssi=%d, freq_offset=%d.%dMHz, is_chirp=%d, "
		"seg_id=%d, peak_mag=%d, total_gain=%d, mb_gain=%d, relpwr_db=%d, "
		"delta_peak=%d, delta_diff=%d, psidx_diff=%d",
		...
		rsfr.relpwr_db, rs.delta_peak, rs.delta_diff, psidx_diff);
