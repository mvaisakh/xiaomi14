 ...
 * return: Void
 */
static inline void dfs_dur_check(struct wlan_dfs *dfs, struct dfs_channel *chan,
				 struct dfs_event *re, uint32_t diff_ts)
{
	if ((dfs->dfsdomain == DFS_FCC_DOMAIN ||
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS_ALWAYS,
			  ...
			  chan->dfs_ch_flags, chan->dfs_ch_freq,
...
static void dfs_print_radar_events(struct wlan_dfs *dfs)
{
	...
	for (i = 0; (i < DFS_EVENT_LOG_SIZE) && (i < dfs->dfs_event_log_count);
		...
		dfs_debug(
			dfs, WLAN_DEBUG_DFS,
			"ts=%llu diff_ts=%u rssi=%u dur=%u, is_chirp=%d, seg_id=%d, sidx=%d, "
			"freq_offset=%d.%dMHz, peak_mag=%d, total_gain=%d, mb_gain=%d, "
			"relpwr_db=%d, delta_diff=%d, delta_peak=%d, psidx_diff=%d",
			...
			dfs->radar_log[i].total_gain, dfs->radar_log[i].mb_gain,
 ...
 */
static bool dfs_is_real_radar(struct wlan_dfs *dfs, struct dfs_filter *rf,
			      int ext_chan_flag)
{
	int i = 0;
	int index;
   ...
   */
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS2,
		  "target_ts=%llu, dl_seq_num_start=%d, dl_seq_num_second=%d, "
		  "dl_seq_num_stop=%d",
		  target_ts, dl->dl_seq_num_start, dl->dl_seq_num_second,
		  dl->dl_seq_num_stop);
	...
	if (max_score_index != 0) {
		dfs_debug(
			dfs, WLAN_DEBUG_DFS_ALWAYS,
			"Rejecting Radar since Fractional PRI detected: searchpri=%d, "
			"threshold=%d, fractional PRI=%d, Fractional PRI score=%d",
	...
	if ((dl->dl_max_sidx - dl->dl_min_sidx) > rf->rf_sidx_spread) {
		dfs_debug(
			dfs, WLAN_DEBUG_DFS_ALWAYS,
			"Rejecting Radar since frequency spread is too large : "
			"min_sidx=%d, max_sidx=%d, rf_sidx_spread=%d",
			dl->dl_min_sidx, dl->dl_max_sidx, rf->rf_sidx_spread);
		return 0;
	}

	if ((rf->rf_check_delta_peak) &&
	    ((dl->dl_delta_peak_match_count + dl->dl_psidx_diff_match_count -
	      1) < rf->rf_threshold)) {
		dfs_debug(
			dfs, WLAN_DEBUG_DFS_ALWAYS,
			"Rejecting Radar since delta peak values are invalid : "
			"dl_delta_peak_match_count=%d, dl_psidx_diff_match_count=%d, "
			"rf_threshold=%d",
			dl->dl_delta_peak_match_count,
			dl->dl_psidx_diff_match_count, rf->rf_threshold);
		return 0;
	}
	dfs_debug(dfs, WLAN_DEBUG_DFS_FALSE_DET,
		  "dl->dl_min_sidx: %d, dl->dl_max_sidx: %d", dl->dl_min_sidx,
		  dl->dl_max_sidx);
 ...
 * @this_ts: Timestamp.
 */
static inline bool dfs_reject_on_pri(struct wlan_dfs *dfs,
				     struct dfs_filter *rf, uint64_t deltaT,
				     uint64_t this_ts)
{
	if ((deltaT < rf->rf_minpri) && (deltaT != 0)) {
		...
		dfs_debug(
			dfs, WLAN_DEBUG_DFS2,
			"filterID %d : Rejecting on individual filter min PRI "
			"deltaT=%lld rf->rf_minpri=%u",
			rf->rf_pulseid, (uint64_t)deltaT, rf->rf_minpri);
		return 1;
	}
	...
	if (rf->rf_ignore_pri_window > 0) {
		if (deltaT < rf->rf_minpri) {
			dfs_debug(
				dfs, WLAN_DEBUG_DFS2,
				"filterID %d : Rejecting on individual filter max PRI "
				"deltaT=%lld rf->rf_minpri=%u",
...
static inline void dfs_confirm_radar_check(struct wlan_dfs *dfs,
					   struct dfs_filter *rf,
					   int ext_chan_event_flag, int *found,
...
static void __dfs_process_radarevent(struct wlan_dfs *dfs,
				     ...
				     struct dfs_event *re, uint64_t this_ts,
				     int *found, int *false_radar_found)
{
	int p;
	uint64_t deltaT = 0;
	...
	for (p = 0, *found = 0;
	     (p < ft->ft_numfilters) && (!(*found)) && !(*false_radar_found);
	     p++) {
		rf = ft->ft_filters[p];
       ...
       */
			...
			if (rf->rf_patterntype == 2) {
				*found = dfs_staggered_check(
					dfs, rf, (uint32_t)deltaT, re->re_dur);
			} else {
           ...
           */
					...
					dfs_confirm_radar_check(
						dfs, rf, ext_chan_event_flag,
						found, false_radar_found);
					rf->rf_check_delta_peak =
						ori_rf_check_delta_peak;
				}
			}
	...
	if (*found) {
		dfs_debug(dfs, WLAN_DEBUG_DFS_ALWAYS,
			  ...
			  ft->ft_mindur, rf ? rf->rf_pulseid : -1);
	}
   ...
   */
	...
	if (count > 0) {
		...
		dfs_debug(dfs, WLAN_DEBUG_DFS2,
			  ...
			  dfs->dfs_average_pri, dfs->dfs_average_duration,
 ...
 * @seg_id: Segment id.
 */
static inline void dfs_radarfound_reset_vars(struct wlan_dfs *dfs,
					     struct dfs_state *rs,
					     ...
					     uint8_t seg_id)
{
   ...
   */
	...
	thischan = &rs->rs_chan;
	if ((seg_id == SEG_ID_SECONDARY) && (dfs_is_precac_timer_running(dfs)))
   ...
   */
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS1, "Primary channel freq = %u flags=0x%x",
	...
	if (chan->dfs_ch_freq != thischan->dfs_ch_freq)
		dfs_debug(dfs, WLAN_DEBUG_DFS1,
			  ...
			  thischan->dfs_ch_freq, thischan->dfs_ch_flagext);
...
{
	dfs_debug(dfs, WLAN_DEBUG_DFS, "bangradar on 2nd segment cfreq = %u",
 ...
 */
static inline int dfs_handle_bangradar(struct wlan_dfs *dfs,
				       struct dfs_channel *chan,
				       struct dfs_state **rs, uint8_t *seg_id,
				       bool *retval)
{
 ...
 */
static inline void dfs_process_w53_pulses(struct wlan_dfs *dfs)
{
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS1,
		  ...
		  dfs->dfs_phyerr_w53_counter, dfs->dfs_phyerr_freq_max,
		  dfs->dfs_phyerr_freq_min, dfs->dfs_pri_multiplier);
 ...
 */
static inline int dfs_handle_missing_pulses(struct wlan_dfs *dfs,
					    struct dfs_channel *chan)
{
 ...
 */
static inline void dfs_is_radarq_empty(struct wlan_dfs *dfs, int *empty)
{
	WLAN_DFSQ_LOCK(dfs);
 ...
 */
static inline void dfs_remove_event_from_radarq(struct wlan_dfs *dfs,
 ...
 */
static inline void dfs_return_event_to_eventq(struct wlan_dfs *dfs,
					      struct dfs_event *event)
{
 ...
 * @index: Index value.
 */
static inline void dfs_log_event(struct wlan_dfs *dfs, struct dfs_event *re,
				 uint64_t this_ts, uint32_t diff_ts,
				 uint32_t index)
{
	uint8_t i;
 ...
 */
static inline void dfs_check_if_nonbin5(struct wlan_dfs *dfs,
					struct dfs_event *re,
					struct dfs_state **rs, uint64_t this_ts,
					uint32_t diff_ts, int *found,
					bool *retval, int *false_radar_found)
{
	...
	dfs_debug(dfs, WLAN_DEBUG_DFS1,
		  ...
		  (*rs)->rs_chan.dfs_ch_freq, (uint64_t)this_ts, (*re).re_dur,
		  (*re).re_rssi);

	while ((tabledepth < DFS_MAX_RADAR_OVERLAP) &&
	       ((dfs->dfs_ftindextable[(*re).re_dur])[tabledepth] != -1) &&
	       (!*retval) && !(*false_radar_found)) {
		ft = dfs->dfs_radarf[(
			(dfs->dfs_ftindextable[(*re).re_dur])[tabledepth])];
		dfs_debug(dfs, WLAN_DEBUG_DFS2,
			  "  ** RD (%d): ts %x dur %u rssi %u",
			  (*rs)->rs_chan.dfs_ch_freq, (*re).re_ts, (*re).re_dur,
			  (*re).re_rssi);
 ...
 * @found: Pointer to found. If radar found or not.
 */
static inline void dfs_check_each_b5radar(struct wlan_dfs *dfs,
					  struct dfs_event *re,
 ...
 * @found: Pointer to found. If radar found or not.
 */
static inline void dfs_check_if_bin5(struct wlan_dfs *dfs, struct dfs_event *re,
				     uint64_t this_ts, uint32_t diff_ts,
				     int *found)
{
	int p;
 ...
 * @rs: Pointer to dfs_state.
 */
static inline bool dfs_skip_the_event(struct wlan_dfs *dfs,
				      struct dfs_event *re,
				      struct dfs_state **rs)
{
 ...
 * Return: Deltafull ts.
 */
static inline uint64_t dfs_check_ts_wrap(struct wlan_dfs *dfs,
					 struct dfs_event *re,
					 uint64_t deltafull_ts)
{
	if (deltafull_ts >
	    ((uint64_t)((DFS_TSMASK - dfs->dfs_rinfo.rn_last_ts) + 1 +
			(*re).re_ts)))
		deltafull_ts -= (DFS_TSMASK - dfs->dfs_rinfo.rn_last_ts) + 1 +
				(*re).re_ts;
 ...
 * @re: Pointer to re(radar event).
 */
static inline void dfs_calculate_ts_prefix(struct wlan_dfs *dfs,
					   struct dfs_event *re)
{
	uint64_t deltafull_ts;
...
static inline void dfs_calculate_timestamps(struct wlan_dfs *dfs,
					    struct dfs_event *re,
					    uint64_t *this_ts)
{
 ...
 */
static inline void dfs_add_to_pulseline(struct wlan_dfs *dfs,
					struct dfs_event *re, uint64_t *this_ts,
					uint32_t *test_ts, uint32_t *diff_ts,
					uint32_t *index)
{
	struct dfs_pulseline *pl;
 ...
 * @re: Pointer to dfs_event structure.
 */
static inline void dfs_conditional_clear_delaylines(struct wlan_dfs *dfs,
						    uint32_t diff_ts,
						    uint64_t this_ts,
						    struct dfs_event re)
{
 ...
 */
static inline int dfs_process_each_radarevent(struct wlan_dfs *dfs,
					      struct dfs_channel *chan,
					      struct dfs_state **rs,
					      uint8_t *seg_id, bool *retval,
					      int *false_radar_found)
{
 ...
 *
 */
static void dfs_fill_radar_found_info(struct wlan_dfs *dfs,
...
void dfs_radarfound_action_generic(struct wlan_dfs *dfs, uint8_t seg_id)
{
	...
	radar_found->pdev_id = wlan_objmgr_pdev_get_pdev_id(dfs->dfs_pdev_obj);
 ...
 * @seg_id: Segment id.
 */
static void dfs_radar_found_action(struct wlan_dfs *dfs, bool bangradar,
				   uint8_t seg_id)
{
 ...
 */
static bool dfs_radar_pulse_event_basic_sanity(struct wlan_dfs *dfs,
					       struct dfs_channel *chan)
{
	if (!chan) {
		dfs_err(dfs, WLAN_DEBUG_DFS1, "dfs->dfs_curchan is NULL");
		return false;
	}
 ...
 *
 */
static void dfs_find_radar(struct wlan_dfs *dfs, struct dfs_channel *chan,
			   struct dfs_state *rs, uint8_t *seg_id,
			   bool *is_bangradar, bool *is_radar_found)
{
...
void dfs_process_radarevent(struct wlan_dfs *dfs, struct dfs_channel *chan)
{
