			   ...
			   struct security_info *security)
{
	...
	if (filter->ucastcipherset &&
		...
		scm_debug(QDF_MAC_ADDR_FMT
			  " : Filter doesn't have CIPHER none in uc %x",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
	...
	if (filter->mcastcipherset &&
		...
		scm_debug(QDF_MAC_ADDR_FMT
			  " : Filter doesn't have CIPHER none in mc %x",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
			  ...
			  struct security_info *security)
{
	...
	if (!filter->ucastcipherset || !filter->mcastcipherset) {
		scm_debug(QDF_MAC_ADDR_FMT
			  " : Filter uc %x or mc %x cipher are 0",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
			  filter->ucastcipherset, filter->mcastcipherset);
		return false;
	}
	...
	if (!(QDF_HAS_PARAM(filter->ucastcipherset, WLAN_CRYPTO_CIPHER_WEP) ||
		...
		scm_debug(QDF_MAC_ADDR_FMT
			  " : Filter doesn't have WEP cipher in uc %x",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
			  filter->ucastcipherset);
		return false;
	}
	...
	if (!(QDF_HAS_PARAM(filter->mcastcipherset, WLAN_CRYPTO_CIPHER_WEP) ||
	      ...
	      QDF_HAS_PARAM(filter->mcastcipherset,
			    WLAN_CRYPTO_CIPHER_WEP_104))) {
		scm_debug(QDF_MAC_ADDR_FMT
			  " : Filter doesn't have WEP cipher in mc %x",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
			  filter->mcastcipherset);
		return false;
	}
				  ...
				  struct security_info *security)
{
	if (!scm_chk_if_cipher_n_akm_match(filter, ap_crypto)) {
		scm_debug(
			QDF_MAC_ADDR_FMT
			": fail. adaptive 11r %d Self: AKM %x CIPHER: mc %x uc %x mgmt "
			"%x pmf %d AP: AKM %x CIPHER: mc %x uc %x mgmt %x, RSN caps %x",
			QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
			is_adaptive_11r, filter->key_mgmt,
			  ...
			  struct security_info *security)
{
	...
	match = scm_chk_crypto_params(filter, ap_crypto, false, db_entry,
				      security);
	qdf_mem_free(ap_crypto);
			   ...
			   struct security_info *security)
{
	...
	if (!scm_chk_if_cipher_n_akm_match(filter, ap_crypto)) {
		scm_debug(
			QDF_MAC_ADDR_FMT
			": fail. Self: AKM %x CIPHER: mc %x uc %x mgmt %x pmf %d AP: AKM "
			"%x CIPHER: mc %x uc %x mgmt %x, RSN caps %x",
			QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
				   ...
				   struct security_info *security)
{
	...
	if (util_scan_entry_rsn(db_entry)) {
		status = wlan_crypto_rsnie_check(ap_crypto,
						 util_scan_entry_rsn(db_entry));
		if (QDF_IS_STATUS_ERROR(status)) {
			scm_err(QDF_MAC_ADDR_FMT
				": failed to parse RSN IE, status %d",
				QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
				status);
			goto free;
		}
	...
	if (util_scan_entry_wpa(db_entry)) {
		...
		if (QDF_IS_STATUS_ERROR(status)) {
			scm_err(QDF_MAC_ADDR_FMT
				": failed to parse WPA IE, status %d",
				QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
				status);
			goto free;
		}
	...
	if (util_scan_entry_wapi(db_entry)) {
		status = wlan_crypto_wapiie_check(
			ap_crypto, util_scan_entry_wapi(db_entry));
		if (QDF_IS_STATUS_ERROR(status)) {
			scm_err(QDF_MAC_ADDR_FMT
				": failed to parse WPA IE, status %d",
				QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
					  ...
					  struct scan_cache_entry *db_entry)
{
	...
	if (is_hidden && !qdf_is_macaddr_zero(&filter->bssid_hint))
		scm_debug(QDF_MAC_ADDR_FMT
			  ": Ignore hidden AP as key_mgmt 0x%x is not OWE "
			  "or bssid hint: " QDF_MAC_ADDR_FMT " does not match",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
			  filter->key_mgmt,
				     ...
				     struct scan_cache_entry *db_entry)
{
   ...
   */
	...
	if (indication_ie->is_hessid_present && (data + HESSID_LEN) <= end_ptr)
		data += HESSID_LEN;

	for (i = 1; i <= indication_ie->realm_identifiers_cnt &&
		    (data + REALM_HASH_LEN) <= end_ptr;
	     i++) {
		if (!qdf_mem_cmp(filter->fils_scan_filter.fils_realm, data,
				 REALM_HASH_LEN))
			return true;
				  ...
				  struct scan_cache_entry *db_entry)
{
	...
	if ((assoc_band_bitmap &&
	     !(band_bitmap & BIT(band) & assoc_band_bitmap)) ||
	...
	for (i = 0; i < db_entry->ml_info.num_links; i++) {
		...
		is_disabled = wlan_reg_is_disable_for_pwrmode(
			pdev, partner_link->freq, REG_BEST_PWR_MODE);
		if (is_disabled) {
			scm_debug(
				"partner link id %d freq %d disabled : " QDF_MAC_ADDR_FMT,
				partner_link->link_id, partner_link->freq,
				QDF_MAC_ADDR_REF(
		...
		if (band_bitmap & BIT(band)) {
			scm_debug(
				"partner link id %d freq %d match band bitmap: "
				"0x%x " QDF_MAC_ADDR_FMT,
				partner_link->link_id, partner_link->freq,
				filter->band_bitmap,
				QDF_MAC_ADDR_REF(
					partner_link->link_addr.bytes));
				    ...
				    struct scan_cache_entry *db_entry)
{
	...
	if (QDF_GET_BITS(eht_ops->ehtop_param,
		...
		orig_puncture_bitmap = QDF_GET_BITS(
	...
	orig_width = QDF_GET_BITS(eht_ops->control, EHTOP_INFO_CHAN_WIDTH_IDX,
	...
	status = wlan_reg_extract_puncture_by_bw(
		width, orig_puncture_bitmap, db_entry->channel.chan_freq,
		center_freq_320m, CH_WIDTH_20MHZ, &new_puncture_bitmap);
	if (QDF_IS_STATUS_ERROR(status) || new_puncture_bitmap) {
		scm_debug(
			QDF_MAC_ADDR_FMT
			"freq %d width %d 320m center %d puncture: orig %d new %d status %d",
			QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
   ...
   */
     ...
     */
		if (filter->num_of_ssid)
			scm_debug(
				QDF_MAC_ADDR_FMT
				": Ignore as BSSID not in list (no. of BSSID in list %d)",
				QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
				filter->num_of_bssid);
		return false;
	}
     ...
     */
		if (filter->num_of_bssid || filter->num_of_ssid)
			scm_debug(
				QDF_MAC_ADDR_FMT
				": Ignore as age %lu ms is greater than threshold %lu ms",
				QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
	...
	if (filter->dot11mode && !scm_check_dot11mode(db_entry, filter)) {
		scm_debug(QDF_MAC_ADDR_FMT
			  ": Ignore as dot11mode %d didn't match phymode %d",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
	...
	for (i = 0; i < filter->num_of_channels; i++) {
		if (!filter->chan_freq_list[i] ||
		    filter->chan_freq_list[i] == db_entry->channel.chan_freq) {
			match = true;
			break;
		}
	}
     ...
     */
		if (filter->num_of_bssid || filter->num_of_ssid)
			scm_debug(
				QDF_MAC_ADDR_FMT
				": Ignore as AP's freq %d is not in freq list",
				QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
				db_entry->channel.chan_freq);
		return false;
	}
	...
	if (!filter->ignore_auth_enc_type && !filter->match_security_func &&
		...
		scm_debug(QDF_MAC_ADDR_FMT
			  ": Ignore as security profile didn't match",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes));
		return false;
	}
	...
	if (filter->match_security_func &&
	    !filter->match_security_func(filter->match_security_func_arg,
					 db_entry)) {
		scm_debug(QDF_MAC_ADDR_FMT
			  ": Ignore as custom security match failed",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes));
		return false;
	}

	if (filter->ccx_validate_bss &&
	    !filter->ccx_validate_bss(filter->ccx_validate_bss_arg, db_entry,
				      0)) {
	...
	if (!util_is_bss_type_match(filter->bss_type, db_entry->cap_info)) {
		scm_debug(
			QDF_MAC_ADDR_FMT
			": Ignore as bss type didn't match cap_info %x bss_type %d",
			QDF_MAC_ADDR_REF(db_entry->bssid.bytes),
	...
	if (!scm_is_fils_config_match(filter, db_entry)) {
		scm_debug(QDF_MAC_ADDR_FMT
			  ":Ignore as fils config didn't match",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes));
		return false;
	}
	...
	if (!util_mlo_filter_match(pdev, filter, db_entry)) {
		scm_debug(QDF_MAC_ADDR_FMT
			  ": Ignore as mlo filter didn't match",
			  QDF_MAC_ADDR_REF(db_entry->bssid.bytes));
		return false;
	}
