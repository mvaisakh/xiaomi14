...
wlan_serialization_get_pdev_queue_obj(struct wlan_ser_pdev_obj *pdev_obj,
...
wlan_serialization_get_vdev_queue_obj(struct wlan_ser_vdev_obj *vdev_obj,
				      enum wlan_serialization_cmd_type cmd_type)
{
...
qdf_list_t *wlan_serialization_get_list_from_pdev_queue(
	...
	enum wlan_serialization_cmd_type cmd_type, uint8_t is_active_cmd)
{
	...
	uint8_t for_active_queue)
{
	...
	status = wlan_serialization_add_cmd_to_queue(queue, cmd_list, pdev_obj,
						     for_active_queue,
						     WLAN_SER_PDEV_NODE);
	...
	uint8_t for_active_queue)
{
	...
	vdev_queue_obj =
		wlan_serialization_get_vdev_queue_obj(vdev_obj, cmd->cmd_type);

	queue = wlan_serialization_get_list_from_vdev_queue(
		vdev_obj, cmd->cmd_type, for_active_queue);
	...
	struct wlan_serialization_command *cmd, uint8_t is_active_queue)
{
	...
	status = wlan_serialization_remove_cmd_from_queue(
		queue, cmd, pcmd_list, pdev_obj, WLAN_SER_PDEV_NODE);
	...
	struct wlan_serialization_command *cmd, uint8_t is_active_queue)
{
	...
	queue = wlan_serialization_get_list_from_vdev_queue(
		vdev_obj, cmd->cmd_type, is_active_queue);

	status = wlan_serialization_remove_cmd_from_queue(
		queue, cmd, pcmd_list, pdev_obj, WLAN_SER_VDEV_NODE);
