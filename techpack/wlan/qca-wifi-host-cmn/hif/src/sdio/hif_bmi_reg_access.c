 ...
 */
static QDF_STATUS hif_bmi_buffer_send(struct hif_sdio_softc *scn,
				      struct hif_sdio_dev *device, char *buffer,
				      uint32_t length)
{
	QDF_STATUS status;
	uint32_t timeout;
	...
	*p_bmi_cmd_credits = 0;
	...
	while (timeout-- && !(*p_bmi_cmd_credits)) {
		...
		address = COUNT_DEC_ADDRESS +
			  (HTC_MAILBOX_NUM_MAX + ENDPOINT1) * 4;
     ...
     * 4-byte aligned
     */
		status = hif_read_write(device, address,
...
static QDF_STATUS hif_bmi_read_write(struct hif_sdio_dev *device, char *buffer,
				     uint32_t length)
{
	QDF_STATUS status;

	status = hif_read_write(device, HOST_INT_STATUS_ADDRESS, buffer, length,
				HIF_RD_SYNC_BYTE_INC, NULL);
	if (status != QDF_STATUS_SUCCESS) {
				     ...
				     uint32_t length)
{
	...
	status = hif_read_write(device, RX_LOOKAHEAD_VALID_ADDRESS, buffer,
				length, HIF_RD_SYNC_BYTE_INC, NULL);
	if (status != QDF_STATUS_SUCCESS) {
		AR_DEBUG_PRINTF(ATH_DEBUG_ERR,
 ...
 * Return: QDF_STATUS_SUCCESS for success.
 */
static QDF_STATUS hif_bmi_buffer_receive(struct hif_sdio_dev *device,
     ...
     */
		...
		while ((!want_timeout || timeout--) && !word_available) {
			if (get_pending_events_func) {
				status = get_pending_events_func(
					device, &hif_pending_events, NULL);
				if (status != QDF_STATUS_SUCCESS) {
					AR_DEBUG_PRINTF(
						ATH_DEBUG_ERR,
			      ...
			      struct bmi_target_info *targ_info)
{
	...
	status = hif_bmi_buffer_receive(device, (char *)&targ_info->target_ver,
	...
	if (targ_info->target_ver == TARGET_VERSION_SENTINAL) {
		...
		status = hif_bmi_buffer_receive(
			device, (char *)&targ_info->target_info_byte_count,
...
QDF_STATUS hif_exchange_bmi_msg(struct hif_opaque_softc *hif_ctx,
				...
				uint8_t *send_message, uint32_t length,
					  ...
					  uint32_t buffer)
{
	...
	status = hif_read_write(device, SDIO_SCRATCH_1_ADDRESS,
				(uint8_t *)&buffer, 4, HIF_WR_SYNC_BYTE_INC,
				NULL);
	if (status != QDF_STATUS_SUCCESS) {
		AR_DEBUG_PRINTF(ATH_DEBUG_ERR,
				("%s: Unable to write to 0x%x\n", __func__,
				 SDIO_SCRATCH_1_ADDRESS));
		return QDF_STATUS_E_FAILURE;
	}
...
QDF_STATUS hif_bmi_read_scratch_register(struct hif_sdio_dev *device)
{
	...
	status = hif_read_write(device, SDIO_SCRATCH_1_ADDRESS,
				(uint8_t *)&buffer, 4, HIF_RD_SYNC_BYTE_INC,
				NULL);
	if (status != QDF_STATUS_SUCCESS) {
		AR_DEBUG_PRINTF(ATH_DEBUG_ERR,
