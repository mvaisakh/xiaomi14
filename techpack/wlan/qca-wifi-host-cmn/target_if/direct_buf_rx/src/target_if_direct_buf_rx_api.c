...
static inline void target_if_direct_buf_rx_debugfs_init(void)
{
	dbr_debugfs_entry = qdf_debugfs_create_dir("dbr_ring_debug", NULL);

	if (!dbr_debugfs_entry)
		direct_buf_rx_err(
			"error while creating direct_buf rx debugfs dir");
}

static inline void target_if_direct_buf_rx_debugfs_deinit(void)
{
	if (dbr_debugfs_entry) {
...
QDF_STATUS direct_buf_rx_init(void)
{
	...
	status = wlan_objmgr_register_psoc_create_handler(
		...
		target_if_direct_buf_rx_psoc_create_handler, NULL);
	...
	status = wlan_objmgr_register_psoc_destroy_handler(
		WLAN_TARGET_IF_COMP_DIRECT_BUF_RX,
		target_if_direct_buf_rx_psoc_destroy_handler, NULL);
	...
	status = wlan_objmgr_register_pdev_create_handler(
		WLAN_TARGET_IF_COMP_DIRECT_BUF_RX,
		target_if_direct_buf_rx_pdev_create_handler, NULL);
	...
	status = wlan_objmgr_register_pdev_destroy_handler(
		WLAN_TARGET_IF_COMP_DIRECT_BUF_RX,
		target_if_direct_buf_rx_pdev_destroy_handler, NULL);
	...
	direct_buf_rx_info(
		"Direct Buffer RX pdev,psoc create and destroy handlers registered");
...
QDF_STATUS direct_buf_rx_deinit(void)
{
	...
	direct_buf_rx_debug(
		"Direct Buffer RX pdev,psoc create and destroy handlers unregistered");
...
QDF_STATUS
target_if_dbr_update_pdev_for_hw_mode_change(struct wlan_objmgr_pdev *pdev,
					     int phy_idx)
{
...
QDF_STATUS
target_if_dbr_set_event_handler_ctx(struct wlan_objmgr_psoc *psoc,
