...
#define DP_RX_REO_QDESC_GET_MAC(freedesc, peer)                  \
	qdf_mem_copy((freedesc)->peer_mac, (peer)->mac_addr.raw, \
		     QDF_MAC_ADDR_SIZE)
...
static inline void dp_set_ssn_valid_flag(struct hal_reo_cmd_params *params,
					 uint8_t valid)
{
	params->u.upd_queue_params.update_svld = 1;
	params->u.upd_queue_params.svld = valid;
	dp_peer_debug("Setting SSN valid bit to %d", valid);
}
				       ...
				       union hal_reo_status *reo_status)
{
	...
	if (queue_status->header.status != HAL_REO_CMD_SUCCESS) {
		dp_err("REO stats failure %d", queue_status->header.status);
		return;
	}
			...
			union hal_reo_status *reo_status)
{
	...
	DP_PRINT_STATS(
		...
		rx_tid->tid, queue_status->ssn, queue_status->curr_idx,
		queue_status->pn_31_0, queue_status->pn_63_32,
		queue_status->pn_95_64, queue_status->pn_127_96,
		queue_status->last_rx_enq_tstamp,
		queue_status->last_rx_deq_tstamp, queue_status->rx_bitmap_31_0,
		queue_status->rx_bitmap_63_32, queue_status->rx_bitmap_95_64,
		queue_status->rx_bitmap_127_96, queue_status->rx_bitmap_159_128,
		queue_status->rx_bitmap_191_160,
		queue_status->rx_bitmap_223_192,
		queue_status->rx_bitmap_255_224);

	DP_PRINT_STATS("curr_mpdu_cnt      : %d\n"
		       "curr_msdu_cnt      : %d\n"
		       "fwd_timeout_cnt    : %d\n"
		       "fwd_bar_cnt        : %d\n"
		       "dup_cnt            : %d\n"
		       ...
		       queue_status->win_jump_2k, queue_status->hole_cnt);
	...
	DP_PRINT_STATS("Addba Req          : %d\n"
		       ...
		       "BA window size     : %d\n"
		       "Pn size            : %d\n",
		       rx_tid->num_of_addba_req, rx_tid->num_of_addba_resp,
		       rx_tid->num_addba_rsp_success,
		       rx_tid->num_addba_rsp_failed, rx_tid->num_of_delba_req,
		       rx_tid->delba_tx_success_cnt, rx_tid->delba_tx_fail_cnt,
		       rx_tid->ba_win_size, rx_tid->pn_size);
}
				...
				union hal_reo_status *reo_status)
{
	...
	if ((reo_status->rx_queue_status.header.status !=
	     HAL_REO_CMD_SUCCESS) &&
	    (reo_status->rx_queue_status.header.status != HAL_REO_CMD_DRAIN)) {
 ...
 */
static inline bool dp_rx_tid_setup_allow(struct dp_peer *peer)
{
 ...
 * Return: true - allow, false - disallow
 */
static inline bool dp_rx_tid_update_allow(struct dp_peer *peer)
{
		       ...
		       uint32_t start_seq, bool bar_update)
{
	...
	if (!bar_update)
		dp_peer_rx_reorder_queue_setup(soc, peer, tid, ba_window_size);
...
void check_free_list_for_invalid_flush(struct dp_soc *soc)
{
	...
	for (i = 0; i < soc->free_addr_list_idx; i++) {
		addr_deref_val = (uint32_t *)soc->list_qdesc_addr_free[i]
					 .hw_qdesc_vaddr_unalign;
			     ...
			     union hal_reo_status *reo_status)
{
	...
	if ((reo_status->fl_cache_status.header.status !=
		...
		dp_peer_err("%pK: Rx tid HW desc flush failed(%d): tid %d", soc,
			    reo_status->rx_queue_status.header.status,
			    freedesc->rx_tid.tid);
	}
	dp_peer_info("%pK: %lu hw_qdesc_paddr: %pK, tid:%d", soc, curr_ts,
				 ...
				 uint32_t ba_window_size, uint32_t start_seq)
{
   ...
   */
	hw_qdesc_size =
		hal_get_reo_qdesc_size(soc->hal_soc, ba_window_size, tid);
...
try_desc_alloc:
	...
	if (!rx_tid->hw_qdesc_vaddr_unaligned) {
		dp_peer_err("%pK: Rx tid HW desc alloc failed: tid %d", soc,
			    tid);
     ...
     */
		qdf_mem_free(rx_tid->hw_qdesc_vaddr_unaligned);
		rx_tid->hw_qdesc_vaddr_unaligned = qdf_mem_malloc(
			rx_tid->hw_qdesc_alloc_size + hw_qdesc_align - 1);
		...
		hw_qdesc_vaddr = (void *)qdf_align(
			(unsigned long)rx_tid->hw_qdesc_vaddr_unaligned,
			hw_qdesc_align);

		dp_peer_debug("%pK: Total Size %d Aligned Addr %pK", soc,
			      rx_tid->hw_qdesc_alloc_size, hw_qdesc_vaddr);
   ...
   */
	...
	if (dp_reo_desc_addr_chk(rx_tid->hw_qdesc_paddr) !=
		...
		if (alloc_tries++ < 10) {
			...
			dp_peer_err(
				"%pK: Rx tid HW desc alloc failed (lowmem): tid %d",
				soc, tid);
	...
	if (rx_tid->hw_qdesc_vaddr_unaligned) {
		if (dp_reo_desc_addr_chk(rx_tid->hw_qdesc_paddr) ==
			...
			qdf_mem_unmap_nbytes_single(
				soc->osdev, rx_tid->hw_qdesc_paddr,
				QDF_DMA_BIDIRECTIONAL,
				rx_tid->hw_qdesc_alloc_size);
		qdf_mem_free(rx_tid->hw_qdesc_vaddr_unaligned);
		rx_tid->hw_qdesc_vaddr_unaligned = NULL;
...
static void dp_peer_rst_tids(struct dp_soc *soc, struct dp_peer *peer,
			     void *arg)
{
	int tid;
				 ...
				 union hal_reo_status *reo_status)
{
	desc->free_ts = qdf_get_system_timestamp();
	DP_STATS_INC(soc, rx.err.reo_cmd_send_fail, 1);
	qdf_list_insert_back(&soc->reo_desc_freelist, (qdf_list_node_t *)desc);
}
...
static inline void dp_reo_limit_clean_batch_sz(uint32_t *list_size)
{
	...
	if ((*list_size) > REO_DESC_FREELIST_SIZE) {
		dp_err_log("%lu:freedesc number %d in freelist", curr_ts,
			   *list_size);
 ...
 * Return: QDF status
 */
static QDF_STATUS dp_resend_update_reo_cmd(struct dp_soc *soc,
					   struct reo_desc_list_node *desc,
			 ...
			 union hal_reo_status *reo_status)
{
	...
	dp_peer_info("%pK: rx_tid: %d status: %d", soc, freedesc->rx_tid.tid,
		     reo_status->rx_queue_status.header.status);
   ...
   */
     ...
     */
		...
		desc_size =
			hal_get_reo_qdesc_size(soc->hal_soc, 0, DP_NON_QOS_TID);
		...
		while ((tot_desc_size -= desc_size) > 0) {
			...
			if (QDF_STATUS_SUCCESS !=
				...
				desc->pending_ext_desc_size =
					tot_desc_size + desc_size;
		...
		qdf_mem_zero(&params, sizeof(params));
		params.std.need_status = 1;
		params.std.addr_lo = (uint64_t)(rx_tid->hw_qdesc_paddr) &
				     0xffffffff;
		params.std.addr_hi = (uint64_t)(rx_tid->hw_qdesc_paddr) >> 32;
       ...
       */
			dp_info_rl(
				"fail to send REO cmd to flush cache: tid %d",
				rx_tid->tid);
			dp_reo_desc_clean_up(soc, desc, &reo_status);
			DP_STATS_INC(soc, rx.err.reo_cmd_send_fail, 1);
			break;
...
static int dp_rx_tid_delete_wifi3(struct dp_peer *peer, int tid)
{
	...
	union hal_reo_status reo_status;
	struct hal_reo_cmd_params params;
	struct reo_desc_list_node *freedesc = qdf_mem_malloc(sizeof(*freedesc));

	if (!freedesc) {
		dp_peer_err("%pK: malloc failed for freedesc: tid %d", soc,
			    tid);
		qdf_assert(0);
		return -ENOMEM;
	}
...
static void dp_peer_setup_remaining_tids(struct dp_peer *peer)
{
	...
	for (tid = 1; tid < DP_MAX_TIDS - 1; tid++) {
		...
		dp_peer_debug(
			"Setting up TID %d for peer %pK peer->local_id %d", tid,
			peer, peer->local_id);
	}
}
#else
   ...
   */
	if (qdf_unlikely(vdev->mesh_vdev) ||
		...
		dp_rx_tid_setup_wifi3(
			peer, 0, hal_get_rx_max_ba_window(soc->hal_soc, 0), 0);
	else
		...
		if (rx_tid->ba_win_size <= 64) {
			...
			if (rx_tid->ba_status == DP_RX_BA_ACTIVE ||
				...
				if (!rx_tid->delba_tx_status) {
					...
					qdf_spin_unlock_bh(&rx_tid->tid_lock);
					if (peer->vdev->pdev->soc->cdp_soc
						    .ol_ops->send_delba)
						peer->vdev->pdev->soc->cdp_soc
							.ol_ops->send_delba(
								peer->vdev->pdev
									->soc
									->ctrl_psoc,
				      ...
				      uint8_t tid, int status)
{
	...
	if (!qdf_atomic_read(&peer->is_default_route_set)) {
		qdf_spin_unlock_bh(&rx_tid->tid_lock);
		dp_peer_debug(
			"%pK: default route is not set for peer: " QDF_MAC_ADDR_FMT,
			cdp_soc, QDF_MAC_ADDR_REF(peer->mac_addr.raw));
		goto fail;
	}

	if (dp_rx_tid_update_wifi3(peer, tid, rx_tid->ba_win_size,
				   rx_tid->startseqnum, false)) {
		dp_err("Failed update REO SSN");
	}

	dp_info("tid %u window_size %u start_seq_num %u", tid,
		rx_tid->ba_win_size, rx_tid->startseqnum);
 ...
 * Return: void
 */
static void dp_check_ba_buffersize(struct dp_peer *peer, uint16_t tid,
				   uint16_t buffersize)
{
	struct dp_rx_tid *rx_tid = NULL;
	struct dp_soc *soc = peer->vdev->pdev->soc;
	...
	dp_info("Input buffersize %d, max dp allowed %d", buffersize,
		max_ba_window);
	/* Adjust BA window size, restrict it to max DP allowed */
	buffersize = QDF_MIN(buffersize, max_ba_window);

	dp_info(QDF_MAC_ADDR_FMT
		" per_tid_basize_max_tid %d tid %d buffersize %d hw_buffer_size %d",
		QDF_MAC_ADDR_REF(peer->mac_addr.raw),
		soc->per_tid_basize_max_tid, tid, buffersize,
		peer->hw_buffer_size);

	rx_tid = &peer->rx_tid[tid];
	if (soc->per_tid_basize_max_tid && tid < soc->per_tid_basize_max_tid) {
...
out:
	dp_info("rx_tid->ba_win_size %d peer->hw_buffer_size %d "
		"peer->kill_256_sessions %d",
					...
					uint8_t tid, uint16_t buffersize)
{
	...
	peer = dp_peer_get_tgt_peer_hash_find(
		(struct dp_soc *)cdp_soc, peer_mac, 0, vdev_id, DP_MOD_ID_CDP);
	if (!peer) {
		dp_peer_debug("%pK: Peer is NULL!", cdp_soc);
		return QDF_STATUS_E_FAILURE;
...
int dp_addba_requestprocess_wifi3(struct cdp_soc_t *cdp_soc, uint8_t *peer_mac,
				  ...
				  uint16_t buffersize, uint16_t startseqnum)
{
	...
	if (rx_tid->rx_ba_win_size_override == DP_RX_BA_SESSION_DISABLE) {
		dp_peer_info("%pK: disable BA session", cdp_soc);
		...
		dp_peer_info("%pK: restore BA win %d based on addba req",
			     cdp_soc, buffersize);
	}

	dp_check_ba_buffersize(peer, tid, buffersize);

	if (dp_rx_tid_setup_wifi3(peer, tid, rx_tid->ba_win_size,
				  startseqnum)) {
		rx_tid->ba_status = DP_RX_BA_INACTIVE;
		qdf_spin_unlock_bh(&rx_tid->tid_lock);
		status = QDF_STATUS_E_FAILURE;
...
int dp_delba_tx_completion_wifi3(struct cdp_soc_t *cdp_soc, uint8_t *peer_mac,
				 uint16_t vdev_id, uint8_t tid, int status)
{
		      ...
		      uint32_t *rx_pn)
{
	...
	for (i = 0; i < DP_MAX_TIDS; i++) {
		...
		qdf_spin_lock_bh(&rx_tid->tid_lock);
		if (rx_tid->hw_qdesc_vaddr_unaligned) {
			params.std.addr_lo = rx_tid->hw_qdesc_paddr &
					     0xffffffff;
			params.std.addr_hi =
				(uint64_t)(rx_tid->hw_qdesc_paddr) >> 32;
			...
			if (pn_size) {
				dp_peer_info(
					"%pK: PN set for TID:%d pn:%x:%x:%x:%x",
					soc, i, rx_pn[3], rx_pn[2], rx_pn[1],
					rx_pn[0]);
				params.u.upd_queue_params.update_pn_valid = 1;
			...
			if (dp_reo_send_cmd(soc, CMD_UPDATE_RX_REO_QUEUE,
					    ...
					    rx_tid)) {
				dp_err_log(
					"fail to send CMD_UPDATE_RX_REO_QUEUE"
					"tid %d desc %pK",
					rx_tid->tid,
					(void *)(rx_tid->hw_qdesc_paddr));
				DP_STATS_INC(soc, rx.err.reo_cmd_send_fail, 1);
			...
			dp_peer_info("%pK: PN Check not setup for TID :%d ",
				     soc, i);
		}
		qdf_spin_unlock_bh(&rx_tid->tid_lock);
	}
...
QDF_STATUS
dp_rx_delba_ind_handler(void *soc_handle, uint16_t peer_id, uint8_t tid,
			uint16_t win_sz)
{
	...
	if (!peer) {
		dp_peer_err("%pK: Couldn't find peer from ID %d", soc, peer_id);
		return QDF_STATUS_E_FAILURE;
	}
	...
	if (rx_tid->hw_qdesc_vaddr_unaligned) {
		...
		dp_peer_err("%pK: BA session is not setup for TID:%d ", soc,
			    tid);
		status = QDF_STATUS_E_FAILURE;
	}
				...
				dp_rxtid_stats_cmd_cb dp_stats_cmd_cb)
{
	...
	for (i = 0; i < DP_MAX_TIDS; i++) {
		...
		if (rx_tid->hw_qdesc_vaddr_unaligned) {
			...
			comb_peer_id_tid =
				((i << DP_PEER_REO_STATS_TID_SHIFT) | peer_id);
...
int dp_peer_rxtid_stats(struct dp_peer *peer,
			dp_rxtid_stats_cmd_cb dp_stats_cmd_cb, void *cb_ctxt)
{
	struct dp_soc *soc = peer->vdev->pdev->soc;
	struct hal_reo_cmd_params params;
	int i;
		...
		if (rx_tid->hw_qdesc_vaddr_unaligned) {
			...
			if (cb_ctxt) {
				status = dp_reo_send_cmd(
					soc, CMD_GET_QUEUE_STATS, &params,
					dp_stats_cmd_cb, cb_ctxt);
			} else {
				status = dp_reo_send_cmd(
					soc, CMD_GET_QUEUE_STATS, &params,
...
QDF_STATUS dp_peer_rx_tids_create(struct dp_peer *peer)
{
	...
	peer->rx_tid = qdf_mem_malloc(DP_MAX_TIDS * sizeof(struct dp_rx_tid));
			       ...
			       union hal_reo_status *reo_status)
{
	...
	if (reo_status->fl_cache_status.header.status != HAL_REO_CMD_SUCCESS) {
