 ...
 * Return: None
 */
static inline void dp_wdi_event_del_subs(wdi_event_subscribe *wdi_sub,
					 int event_index)
{
 ...
 * Return: None
 */
static inline void dp_wdi_event_iter_sub(struct dp_pdev *pdev,
...
void dp_wdi_event_handler(enum WDI_EVENT event, struct dp_soc *soc, void *data,
			  uint16_t peer_id, int status, uint8_t pdev_id)
{
	uint32_t event_index;
	wdi_event_subscribe *wdi_sub;
   ...
   */
	...
	/* Find the subscriber */
	dp_wdi_event_iter_sub(txrx_pdev, event_index, wdi_sub, data, peer_id,
			      status);
}
...
int dp_wdi_event_sub(struct cdp_soc_t *soc, uint8_t pdev_id,
		     wdi_event_subscribe *event_cb_sub_handle, uint32_t event)
{
	uint32_t event_index;
	wdi_event_subscribe *wdi_sub;
	wdi_event_subscribe *wdi_sub_itr;
	struct dp_pdev *txrx_pdev = dp_get_pdev_from_soc_pdev_id_wifi3(
