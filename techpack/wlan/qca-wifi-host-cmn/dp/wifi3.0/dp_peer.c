 ...
 */
static int dp_add_wds_entry_wrapper(struct dp_soc *soc, struct dp_peer *peer,
				    const uint8_t *dest_macaddr, uint32_t flags,
				    uint8_t type)
{
	QDF_STATUS status;

	status = target_if_add_wds_entry(soc->ctrl_psoc, peer->vdev->vdev_id,
					 peer->mac_addr.raw, dest_macaddr,
					 WMI_HOST_WDS_FLAG_STATIC, type);
 ...
 * Return: 0 on success, error code otherwise.
 */
static int dp_update_wds_entry_wrapper(struct dp_soc *soc, struct dp_peer *peer,
				       uint8_t *dest_macaddr, uint32_t flags)
{
	QDF_STATUS status;

	status = target_if_update_wds_entry(soc->ctrl_psoc, peer->vdev->vdev_id,
					    dest_macaddr, peer->mac_addr.raw,
...
{
	target_if_del_wds_entry(soc->ctrl_psoc, vdev_id, wds_macaddr, type,
				delete_in_fw);
}
#else
static int dp_add_wds_entry_wrapper(struct dp_soc *soc, struct dp_peer *peer,
				    ...
				    uint8_t type)
{
	...
	status = soc->cdp_soc.ol_ops->peer_add_wds_entry(
		...
		peer->peer_id, dest_macaddr, peer->mac_addr.raw, flags, type);
				       ...
				       uint8_t *dest_macaddr, uint32_t flags)
{
	...
	status = soc->cdp_soc.ol_ops->peer_update_wds_entry(
		soc->ctrl_psoc, peer->vdev->vdev_id, dest_macaddr,
		peer->mac_addr.raw, flags);
			      ...
			      uint8_t delete_in_fw)
{
	soc->cdp_soc.ol_ops->peer_del_wds_entry(
		soc->ctrl_psoc, vdev_id, wds_macaddr, type, delete_in_fw);
}
#endif /* BYPASS_OL_OPS */
#else
void dp_del_wds_entry_wrapper(struct dp_soc *soc, uint8_t vdev_id,
...
QDF_STATUS dp_peer_ast_table_attach(struct dp_soc *soc)
{
	...
	soc->ast_table =
		qdf_mem_malloc(max_ast_index * sizeof(struct dp_ast_entry *));
	if (!soc->ast_table) {
					...
					uint8_t vdev_id, enum dp_mod_id mod_id)
{
	bool ast_ind_disable =
		wlan_cfg_get_ast_indication_disable(soc->wlan_cfg_ctx);
...
dp_peer_find_hash_index(struct dp_soc *soc, union dp_align_mac_addr *mac_addr)
{
	uint32_t index;
				       ...
				       enum dp_mod_id mod_id)
{
	...
	if (mac_addr_is_aligned) {
		...
		qdf_mem_copy(&local_mac_addr_aligned.raw[0], peer_mac_addr,
			     QDF_MAC_ADDR_SIZE);
...
void dp_peer_find_hash_add(struct dp_soc *soc, struct dp_peer *peer)
{
	...
	if (peer->peer_type == CDP_LINK_PEER_TYPE) {
		...
		if (QDF_IS_STATUS_ERROR(
			    dp_peer_get_ref(soc, peer, DP_MOD_ID_CONFIG))) {
			      ...
			      struct dp_peer *peer)
{
	...
	if (found) {
		...
		dp_peer_debug(
			"%pK: peer:%pK not found in vdev:%pK peerlist:%pK", soc,
			peer, vdev, &peer->vdev->peer_list);
	}
	qdf_spin_unlock_bh(&vdev->peer_list_lock);
}

void dp_txrx_peer_attach_add(struct dp_soc *soc, struct dp_peer *peer,
			     struct dp_txrx_peer *txrx_peer)
{
...
void dp_peer_find_id_to_obj_add(struct dp_soc *soc, struct dp_peer *peer,
				uint16_t peer_id)
{
     ...
     */
		dp_err("peer %pK(" QDF_MAC_ADDR_FMT
		       ")map failed, id %d mapped to peer %pK",
...
void dp_peer_find_id_to_obj_remove(struct dp_soc *soc, uint16_t peer_id)
{
...
QDF_STATUS dp_peer_mec_hash_attach(struct dp_soc *soc)
{
	...
	dp_peer_info("%pK: max mec index: %d", soc, DP_PEER_MAX_MEC_IDX);

	/* allocate an array of TAILQ mec object lists */
	soc->mec_hash.bins =
		qdf_mem_malloc(hash_elems * sizeof(TAILQ_HEAD(anonymous_tail_q,
							      dp_mec_entry)));
						     ...
						     uint8_t *mec_mac_addr)
{
	...
	qdf_mem_copy(&local_mac_addr_aligned.raw[0], mec_mac_addr,
		     QDF_MAC_ADDR_SIZE);
	mac_addr = &local_mac_addr_aligned;
...
QDF_STATUS dp_peer_mec_add_entry(struct dp_soc *soc, struct dp_vdev *vdev,
				 uint8_t *mac_addr)
{
	...
	if (qdf_unlikely(qdf_atomic_read(&soc->mec_cnt) >=
		...
		dp_peer_warn(
			"%pK: max MEC entry limit reached mac_addr: " QDF_MAC_ADDR_FMT,
	...
	qdf_spin_lock_bh(&soc->mec_lock);
	mecentry =
		dp_peer_mec_hash_find_by_pdevid(soc, pdev->pdev_id, mac_addr);
	...
	dp_peer_debug(
		"%pK: pdevid: %u vdev: %u type: MEC mac_addr: " QDF_MAC_ADDR_FMT,
		soc, pdev->pdev_id, vdev->vdev_id, QDF_MAC_ADDR_REF(mac_addr));

	mecentry = (struct dp_mec_entry *)qdf_mem_malloc(
		sizeof(struct dp_mec_entry));
			      ...
			      void *ptr)
{
	...
	TAILQ_REMOVE(&soc->mec_hash.bins[index], mecentry, hash_list_elem);
...
void dp_peer_mec_free_list(struct dp_soc *soc, void *ptr)
{
	...
	TAILQ_FOREACH_SAFE(mecentry, free_list, hash_list_elem, mecentry_next)
	{
 ...
 */
static bool dp_peer_exist_on_pdev(struct dp_soc *soc, uint8_t *peer_mac_addr,
				  int mac_addr_is_aligned, struct dp_pdev *pdev)
{
	union dp_align_mac_addr local_mac_addr_aligned, *mac_addr;
	unsigned int index;
	struct dp_peer *peer;
...
QDF_STATUS dp_peer_ast_hash_attach(struct dp_soc *soc)
{
	...
	dp_peer_info("%pK: ast hash_elems: %d, max_ast_idx: %d", soc,
...
void dp_peer_ast_hash_remove(struct dp_soc *soc, struct dp_ast_entry *ase)
{
	unsigned index;
						     ...
						     uint8_t vdev_id)
{
	...
	qdf_mem_copy(&local_mac_addr_aligned.raw[0], ast_mac_addr,
		     QDF_MAC_ADDR_SIZE);
	mac_addr = &local_mac_addr_aligned;
...
static inline void dp_peer_map_ipa_evt(struct dp_soc *soc, struct dp_peer *peer,
				       struct dp_ast_entry *ast_entry,
				       uint8_t *mac_addr)
{
	if (ast_entry || (peer->vdev && peer->vdev->proxysta_vdev)) {
		if (soc->cdp_soc.ol_ops->peer_map_event) {
			soc->cdp_soc.ol_ops->peer_map_event(
				...
				mac_addr, ast_entry->type,
				ast_entry->ast_hash_value);
		}
	} else {
 ...
 * Return: None
 */
static inline void dp_peer_unmap_ipa_evt(struct dp_soc *soc, uint16_t peer_id,
					 uint8_t vdev_id, uint8_t *mac_addr)
{
	if (soc->cdp_soc.ol_ops->peer_unmap_event) {
		soc->cdp_soc.ol_ops->peer_unmap_event(soc->ctrl_psoc, peer_id,
						      vdev_id, mac_addr);
	}
}
#else
				    ...
				    uint8_t is_wds)
{
	...
	if (!peer) {
		dp_peer_info(
			"Peer not found soc:%pK: peer_id %d, peer_mac " QDF_MAC_ADDR_FMT
			", vdev_id %d",
			soc, peer_id, QDF_MAC_ADDR_REF(mac_addr), vdev_id);
		return QDF_STATUS_E_INVAL;
	}
	...
	dp_peer_debug(
		"%pK: vdev: %u  ast_entry->type: %d peer_mac: " QDF_MAC_ADDR_FMT
		" peer: %pK mac " QDF_MAC_ADDR_FMT,
		soc, vdev->vdev_id, type, QDF_MAC_ADDR_REF(peer->mac_addr.raw),
		peer, QDF_MAC_ADDR_REF(mac_addr));
   ...
   */
	...
	if (ast_entry && type != CDP_TXRX_AST_TYPE_MLD) {
		dp_peer_debug("AST present ID %d vid %d mac " QDF_MAC_ADDR_FMT,
			      hw_peer_id, vdev_id, QDF_MAC_ADDR_REF(mac_addr));
	...
	ast_entry = (struct dp_ast_entry *)qdf_mem_malloc(
					 ...
					 uint8_t is_wds)
{
	...
	dp_peer_err("%pK: peer %pK ID %d vid %d mac " QDF_MAC_ADDR_FMT, soc,
		    peer, hw_peer_id, vdev_id, QDF_MAC_ADDR_REF(mac_addr));
     ...
     */
		if (hw_peer_id == HTT_INVALID_PEER) {
			...
			if (ast_entry) {
				...
				if (cb) {
					cb(soc->ctrl_psoc,
					   dp_soc_to_cdp_soc(soc), cookie,
					   CDP_TXRX_AST_DELETED);
				}
			} else {
				qdf_spin_unlock_bh(&soc->ast_lock);
				dp_peer_alert(
					"AST entry not found with peer %pK peer_id %u "
					"peer_mac " QDF_MAC_ADDR_FMT
					" mac_addr " QDF_MAC_ADDR_FMT
					" vdev_id %u next_hop %u",
					peer, peer->peer_id,
					QDF_MAC_ADDR_REF(peer->mac_addr.raw),
					QDF_MAC_ADDR_REF(mac_addr), vdev_id,
					is_wds);
			...
			dp_hmwds_ast_add_notify(peer, mac_addr, peer_type, err,
						true);
	...
	if (ast_entry || (peer->vdev && peer->vdev->proxysta_vdev)) {
		if (soc->cdp_soc.ol_ops->peer_map_event) {
			soc->cdp_soc.ol_ops->peer_map_event(
				soc->ctrl_psoc, peer->peer_id, hw_peer_id,
				vdev_id, mac_addr, peer_type, ast_hash);
		}
	} else {
	...
	dp_hmwds_ast_add_notify(peer, mac_addr, peer_type, err, true);

	return err;
}

void dp_peer_free_hmwds_cb(struct cdp_ctrl_objmgr_psoc *ctrl_psoc,
			   struct cdp_soc *dp_soc, void *cookie,
			   enum cdp_ast_free_status status)
{
	...
	peer = dp_peer_find_hash_find(soc, &param->peer_mac_addr.raw[0], 0,
				      param->vdev_id, DP_MOD_ID_AST);
	if (peer) {
		err = dp_peer_add_ast(soc, peer, &param->mac_addr.raw[0],
				      param->type, param->flags);
			   ...
			   uint32_t flags)
{
	...
	dp_peer_debug(
		"%pK: pdevid: %u vdev: %u  ast_entry->type: %d flags: 0x%x "
		"peer_mac: " QDF_MAC_ADDR_FMT
		" peer: %pK mac " QDF_MAC_ADDR_FMT,
...
add_ast_entry:
	...
	case CDP_TXRX_AST_TYPE_DA:
		vap_bss_peer =
			dp_vdev_bss_peer_ref_n_get(soc, vdev, DP_MOD_ID_AST);
		if (!vap_bss_peer) {
			qdf_spin_unlock_bh(&soc->ast_lock);
	...
	if ((ast_entry->type != CDP_TXRX_AST_TYPE_STATIC) &&
	    ...
	    (ast_entry->type != CDP_TXRX_AST_TYPE_STA_BSS) &&
	    (ast_entry->type != CDP_TXRX_AST_TYPE_WDS_HM_SEC))
		status = dp_add_wds_entry_wrapper(soc, peer, mac_addr, flags,
						  ast_entry->type);
...
void dp_peer_free_ast_entry(struct dp_soc *soc, struct dp_ast_entry *ast_entry)
{
	/*
			      ...
			      struct dp_peer *peer)
{
	...
	if (ast_entry->peer_id == HTT_INVALID_PEER) {
		dp_info_rl(
			"Invalid peer id in AST entry mac addr:" QDF_MAC_ADDR_FMT
			" type:%d",
			QDF_MAC_ADDR_REF(ast_entry->mac_addr.raw),
			ast_entry->type);
		return;
...
void dp_peer_del_ast(struct dp_soc *soc, struct dp_ast_entry *ast_entry)
{
	...
	if (ast_entry->delete_in_progress) {
		dp_info_rl(
			"AST entry deletion in progress mac addr:" QDF_MAC_ADDR_FMT
			" type:%d",
			QDF_MAC_ADDR_REF(ast_entry->mac_addr.raw),
			ast_entry->type);
		return;
   ...
   * state
   */
	peer = __dp_peer_get_ref_by_id(soc, ast_entry->peer_id, DP_MOD_ID_AST);
		       ...
		       struct dp_ast_entry *ast_entry, uint32_t flags)
{
	...
	dp_peer_debug(
		"%pK: ast_entry->type: %d pdevid: %u vdevid: %u flags: 0x%x "
		"mac_addr: " QDF_MAC_ADDR_FMT " peer_mac: " QDF_MAC_ADDR_FMT
		"\n",
   ...
   */
	...
	old_peer =
		dp_peer_get_ref_by_id(soc, ast_entry->peer_id, DP_MOD_ID_AST);
	if (!old_peer)
		return 0;
	...
	ret = dp_update_wds_entry_wrapper(soc, peer, ast_entry->mac_addr.raw,
					  flags);
...
void dp_peer_ast_set_type(struct dp_soc *soc, struct dp_ast_entry *ast_entry,
			  enum cdp_txrx_ast_entry_type type)
{
	ast_entry->type = type;
}
			      ...
			      struct dp_peer *peer)
{
	...
	QDF_TRACE(
		QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_TRACE,
		"%s: ast_entry->type: %d pdevid: %u vdev: %u mac_addr: " QDF_MAC_ADDR_FMT
		" next_hop: %u peer_id: %uM\n",
		__func__, ast_entry->type, ast_entry->pdev_id,
		ast_entry->vdev_id, QDF_MAC_ADDR_REF(ast_entry->mac_addr.raw),
   ...
   */
	if (ast_entry->next_hop) {
		if (peer &&
		    dp_peer_state_cmp(peer, DP_PEER_STATE_LOGICAL_DELETE))
			delete_in_fw = false;
		else
			delete_in_fw = true;

		dp_del_wds_entry_wrapper(soc, ast_entry->vdev_id,
					 ast_entry->mac_addr.raw,
					 ast_entry->type, delete_in_fw);
	}
}
#else
void dp_peer_free_ast_entry(struct dp_soc *soc, struct dp_ast_entry *ast_entry)
...
dp_peer_host_add_map_ast(struct dp_soc *soc, uint16_t peer_id,
			 uint8_t *mac_addr, uint16_t hw_peer_id,
			 uint8_t vdev_id, uint16_t ast_hash, uint8_t is_wds)
{
	return QDF_STATUS_SUCCESS;
}
...
void dp_peer_ast_send_multi_wds_del(struct dp_soc *soc, uint8_t vdev_id,
					     ...
					     struct dp_peer *peer)
{
	...
	DP_PEER_ITERATE_ASE_LIST(peer, ast_entry, temp_ast_entry)
	{
		...
		DP_STATS_INC(soc, ast.deleted, 1);
		dp_peer_ast_hash_remove(soc, ast_entry);
		TAILQ_INSERT_TAIL(&ast_local_list, ast_entry, ase_list_elem);
		soc->num_ast_entries--;
	}
 ...
 * Return: Number of wds ast entries freed
 */
static void dp_peer_clean_wds_entries(struct dp_soc *soc, struct dp_peer *peer,
				      uint32_t free_wds_count)
{
	...
	if ((DP_PEER_WDS_COUNT_INVALID != free_wds_count) &&
		...
		dp_alert(
			"For peer %pK (mac: " QDF_MAC_ADDR_FMT
			")number of wds entries deleted by fw = %d during peer delete is "
			"not same as the numbers deleted by host = %d",
			peer, peer->mac_addr.raw, free_wds_count, wds_deleted);
	}
}
...
static int dp_peer_ast_free_entry_by_mac(struct dp_soc *soc,
					 struct dp_peer *peer, uint8_t vdev_id,
					 uint8_t *mac_addr)
{
	struct dp_ast_entry *ast_entry;
	void *cookie = NULL;
					...
					struct dp_peer *peer)
{
	...
	if (hal_reo_shared_qaddr_is_enable(soc->hal_soc)) {
		for (tid = 0; tid < DP_MAX_TIDS; tid++) {
			hal_reo_shared_qaddr_write(soc->hal_soc, peer_id, tid,
						   0);
		}
	}
}
...
dp_peer_find_add_id(struct dp_soc *soc, uint8_t *peer_mac_addr,
		    uint16_t peer_id, uint16_t hw_peer_id, uint8_t vdev_id,
		    enum cdp_peer_type peer_type)
{
	...
	dp_peer_debug("%pK: peer %pK ID %d vid %d mac " QDF_MAC_ADDR_FMT, soc,
		      peer, peer_id, vdev_id, QDF_MAC_ADDR_REF(peer_mac_addr));
     ...
     */
		if (dp_peer_state_cmp(peer, DP_PEER_STATE_LOGICAL_DELETE)) {
			dp_peer_unref_delete(peer, DP_MOD_ID_CONFIG);
			dp_alert("Peer %pK[" QDF_MAC_ADDR_FMT
				 "] logical delete state vid %d",
			   ...
			   struct dp_mlo_link_info *mlo_link_info)
{
	...
	dp_cfg_event_record_peer_map_unmap_evt(soc, DP_CFG_EVENT_MLO_PEER_MAP,
					       ...
					       ml_peer_id, 0, vdev_id);

	dp_info("mlo_peer_map_event (soc:%pK): peer_id %d ml_peer_id %d, "
		"peer_mac " QDF_MAC_ADDR_FMT,
		soc, peer_id, ml_peer_id, QDF_MAC_ADDR_REF(peer_mac_addr));
   ...
   */
	...
	peer = dp_peer_find_add_id(soc, peer_mac_addr, ml_peer_id, hw_peer_id,
				   vdev_id, CDP_MLD_PEER_TYPE);
	if (peer) {
     ...
     */
		if (!peer->self_ast_entry) {
			...
			dp_peer_add_ast(soc, peer, peer_mac_addr, type, 0);
		}
     ...
     */
		...
		if (hal_reo_shared_qaddr_is_enable(primary_soc->hal_soc) &&
			...
			hal_reo_shared_qaddr_write(
				primary_soc->hal_soc, ml_peer_id, 0,
				peer->rx_tid[0].hw_qdesc_paddr);
			hal_reo_shared_qaddr_write(
				primary_soc->hal_soc, ml_peer_id,
				DP_NON_QOS_TID,
	...
	err = dp_peer_map_ast(soc, peer, peer_mac_addr, hw_peer_id, vdev_id,
			      ast_hash, is_wds);
...
static void dp_tx_ppeds_cfg_astidx_cache_mapping(struct dp_soc *soc,
						 struct dp_vdev *vdev,
						 bool peer_map)
{
		       ...
		       uint8_t is_wds)
{
	...
	dp_cfg_event_record_peer_map_unmap_evt(soc, DP_CFG_EVENT_PEER_MAP, NULL,
					       peer_mac_addr, 1, peer_id, 0, 0,
					       vdev_id);
	dp_info("peer_map_event (soc:%pK): peer_id %d, hw_peer_id %d, "
     ...
     */
		if ((!soc->ast_offload_support) &&
		    ((hw_peer_id < 0) ||
		     (hw_peer_id >=
		      wlan_cfg_get_max_ast_idx(soc->wlan_cfg_ctx)))) {
			dp_peer_err("%pK: invalid hw_peer_id: %d", soc,
				    hw_peer_id);
			qdf_assert_always(0);
		}
       ...
       */
			if (!soc->ast_offload_support &&
				...
				dp_info("Add self ast from map " QDF_MAC_ADDR_FMT,
					QDF_MAC_ADDR_REF(peer_mac_addr));
				dp_peer_add_ast(soc, peer, peer_mac_addr, type,
						0);
			}
       ...
       */
			if (hal_reo_shared_qaddr_is_enable(soc->hal_soc) &&
				...
				hal_reo_shared_qaddr_write(
					soc->hal_soc, peer_id, 0,
					peer->rx_tid[0].hw_qdesc_paddr);
				add_entry_write_list(soc, peer, DP_NON_QOS_TID);
				hal_reo_shared_qaddr_write(
					soc->hal_soc, peer_id, DP_NON_QOS_TID,
					peer->rx_tid[DP_NON_QOS_TID]
						.hw_qdesc_paddr);
			}
		}
			      ...
			      uint8_t is_wds, uint32_t free_wds_count)
{
   ...
   */
	...
	if (peer->txrx_peer) {
		...
		dp_wdi_event_handler(WDI_EVENT_PEER_UNMAP, soc, (void *)&params,
				     peer_id, WDI_NO_VAL, vdev->pdev->pdev_id);
	}
   ...
   */
	if (is_wds) {
		...
		dp_alert("AST entry not found with peer %pK peer_id %u "
			 "peer_mac " QDF_MAC_ADDR_FMT
			 " mac_addr " QDF_MAC_ADDR_FMT
			 " vdev_id %u next_hop %u",
			 peer, peer->peer_id,
			 QDF_MAC_ADDR_REF(peer->mac_addr.raw),
			 QDF_MAC_ADDR_REF(mac_addr), vdev_id, is_wds);
	...
	dp_cfg_event_record_peer_map_unmap_evt(soc, DP_CFG_EVENT_PEER_UNMAP,
					       peer, mac_addr, 0, peer_id, 0, 0,
					       vdev_id);
	dp_info("peer_unmap_event (soc:%pK) peer_id %d peer %pK", soc, peer_id,
		peer);
...
void dp_rx_mlo_peer_unmap_handler(struct dp_soc *soc, uint16_t peer_id)
{
	...
	dp_cfg_event_record_peer_map_unmap_evt(soc, DP_CFG_EVENT_MLO_PEER_UNMAP,
					       NULL, mac_addr, 0, peer_id, 0, 0,
					       vdev_id);
	dp_info("MLO peer_unmap_event (soc:%pK) peer_id %d", soc, peer_id);

	dp_rx_peer_unmap_handler(soc, ml_peer_id, vdev_id, mac_addr, is_wds,
...
void dp_peer_cleanup(struct dp_vdev *vdev, struct dp_peer *peer)
{
	...
	qdf_mem_copy(vdev_mac_addr, vdev->mac_addr.raw, QDF_MAC_ADDR_SIZE);

	if (soc->cdp_soc.ol_ops->peer_unref_delete)
		soc->cdp_soc.ol_ops->peer_unref_delete(
			soc->ctrl_psoc, vdev->pdev->pdev_id, peer->mac_addr.raw,
			vdev_mac_addr, vdev_opmode);
}
			  ...
			  bool is_unicast)
{
	...
	dp_peer_info("%pK: key sec spec for peer %pK " QDF_MAC_ADDR_FMT
		     ": %s key of type %d",
			   ...
			   u_int32_t *michael_key, u_int32_t *rx_pn)
{
	...
	if (!txrx_peer) {
		dp_peer_err(
			"Couldn't find txrx peer from ID %d - skipping security inits",
			peer_id);
		return;
	}

	dp_peer_info("%pK: sec spec for peer %pK " QDF_MAC_ADDR_FMT
		     ": %s key of type %d",
		     soc, peer, QDF_MAC_ADDR_REF(peer->mac_addr.raw),
		     is_unicast ? "ucast" : "mcast", sec_type);
	sec_index = is_unicast ? dp_sec_ucast : dp_sec_mcast;
   ...
   */
	...
	if (sec_type != cdp_sec_type_wapi) {
       ...
       */
			...
			qdf_mem_copy((u_int8_t *)&peer->tids_last_pn[i],
				     (u_int8_t *)rx_pn,
					  ...
					  struct dp_txrx_peer *txrx_peer)
{
     ...
     */
		txrx_peer->jitter_stats = qdf_mem_malloc(
...
void dp_peer_jitter_stats_ctx_clr(struct dp_txrx_peer *txrx_peer)
{
	...
	if (!wlan_cfg_is_peer_jitter_stats_enabled(
		    txrx_peer->vdev->pdev->soc->wlan_cfg_ctx))
		return;
	...
	if (wlan_cfg_get_dp_pdev_nss_enabled(
		    txrx_peer->vdev->pdev->wlan_cfg_ctx))
		qdf_mem_zero(jitter_stats,
			     sizeof(struct cdp_peer_tid_stats) * DP_MAX_TIDS);

	else
		qdf_mem_zero(jitter_stats, sizeof(struct cdp_peer_tid_stats) *
						   DP_MAX_TIDS *
						   CDP_MAX_TXRX_CTX);
}
#endif
			    ...
			    struct ol_txrx_desc_type *sta_desc)
{
	...
	peer = dp_peer_find_hash_find(soc, sta_desc->peer_addr.bytes, 0,
				...
				enum ol_txrx_peer_state state)
{
	...
	dp_peer_info("peer" QDF_MAC_ADDR_FMT "state %d",
		     QDF_MAC_ADDR_REF(peer->mac_addr.raw), peer->state);
	      ...
	      struct qdf_mac_addr peer_addr)
{
	...
	peer = dp_peer_find_hash_find(soc, peer_addr.bytes, 0, DP_VDEV_ALL,
				      DP_MOD_ID_CDP);
			 ...
			 uint8_t *vdev_id)
{
	...
	struct dp_peer *peer = dp_peer_find_hash_find(
		soc, peer_mac, 0, DP_VDEV_ALL, DP_MOD_ID_CDP);

	if (!peer)
		return QDF_STATUS_E_FAILURE;

	dp_info("peer %pK vdev %pK vdev id %d", peer, peer->vdev,
		peer->vdev->vdev_id);
...
uint8_t *dp_peer_get_peer_mac_addr(void *peer_handle)
{
	...
	dp_info("peer %pK mac 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", peer, mac[0],
				      ...
				      uint16_t max_bssid)
{
	...
	for (i = 0; i < max_bssid; i++) {
		...
		peer = dp_peer_find_hash_find(soc, peer_addr, 0, i,
					      DP_MOD_ID_CDP);
		if (peer) {
			dp_err("Duplicate peer " QDF_MAC_ADDR_FMT
			      ...
			      uint8_t *peer_mac, bool val)
{
	...
	dp_info("Set tdls flag %d for peer:" QDF_MAC_ADDR_FMT, val,
		QDF_MAC_ADDR_REF(peer_mac));
...
QDF_STATUS
dp_set_michael_key(struct cdp_soc_t *soc, uint8_t vdev_id, uint8_t *peer_mac,
		   bool is_unicast, uint32_t *key)
{
	...
	qdf_mem_copy(&peer->txrx_peer->security[sec_index].michael_key[0], key,
...
{
	struct dp_peer *peer =
		dp_peer_get_ref_by_id(soc, peer_id, DP_MOD_ID_HTT);
...
{
	struct dp_peer *peer = dp_peer_get_tgt_peer_hash_find(
		soc, peer_mac, 0, vdev_id, DP_MOD_ID_TX_MULTIPASS);
   ...
   */
	...
	TAILQ_INSERT_HEAD(&peer->txrx_peer->vdev->mpass_peer_list,
			  peer->txrx_peer, mpass_peer_list_elem);
...
void dp_peer_set_vlan_id(struct cdp_soc_t *cdp_soc, uint8_t vdev_id,
			 uint8_t *peer_mac, uint16_t vlan_id)
{
	struct dp_soc *soc = (struct dp_soc *)cdp_soc;
	struct dp_vdev *vdev = dp_vdev_get_ref_by_id(
		(struct dp_soc *)soc, vdev_id, DP_MOD_ID_TX_MULTIPASS);

	dp_info("vdev_id %d, vdev %pK, multipass_en %d, peer_mac " QDF_MAC_ADDR_FMT
		" vlan %d",
