...
QDF_STATUS dp_mon_filter_alloc_2_0(struct dp_pdev *pdev)
{
	...
	qdf_mem_zero(mon_filter, sizeof(struct dp_mon_filter_be *) *
					 DP_MON_FILTER_MAX_MODE);

	for (mode = 0; mode < DP_MON_FILTER_MAX_MODE; mode++) {
		mon_filter[mode] =
			qdf_mem_malloc(sizeof(struct dp_mon_filter_be) *
				 ...
				 struct htt_rx_ring_tlv_filter *tlv_filter)
{
	...
	HTT_RX_RING_SELECTION_CFG_CONFIG_LENGTH_MGMT_SET(
		*msg_word, tlv_filter->mgmt_dma_length);
	HTT_RX_RING_SELECTION_CFG_CONFIG_LENGTH_CTRL_SET(
		*msg_word, tlv_filter->ctrl_dma_length);
	HTT_RX_RING_SELECTION_CFG_CONFIG_LENGTH_DATA_SET(
		*msg_word, tlv_filter->data_dma_length);
}
				   ...
				   struct htt_rx_ring_tlv_filter *tlv_filter)
{
	...
	if (tlv_filter->mgmt_dma_length) {
		HTT_RX_RING_SELECTION_CFG_PKT_TYPE_ENABLE_MSDU_MPDU_LOGGING_SET(
			*msg_word, 1);
		HTT_RX_RING_SELECTION_CFG_DMA_MPDU_MGMT_SET(
			*msg_word, tlv_filter->mgmt_mpdu_log);
	}

	if (tlv_filter->ctrl_dma_length) {
		HTT_RX_RING_SELECTION_CFG_PKT_TYPE_ENABLE_MSDU_MPDU_LOGGING_SET(
			*msg_word, 2);
		HTT_RX_RING_SELECTION_CFG_DMA_MPDU_CTRL_SET(
			*msg_word, tlv_filter->ctrl_mpdu_log);
	}
	...
	if (tlv_filter->data_dma_length) {
		...
		HTT_RX_RING_SELECTION_CFG_DMA_MPDU_DATA_SET(
			*msg_word, tlv_filter->data_mpdu_log);
	}
}

void dp_rx_mon_word_mask_subscribe(uint32_t *msg_word,
				   struct htt_rx_ring_tlv_filter *tlv_filter)
{
	if (!msg_word || !tlv_filter)
		return;
	...
	*msg_word = 0;
	...
	HTT_RX_RING_SELECTION_CFG_RX_MPDU_START_WORD_MASK_SET(
		*msg_word, RX_MON_MPDU_START_WMASK);

	/* word 15 */
	msg_word++;
	*msg_word = 0;
	HTT_RX_RING_SELECTION_CFG_RX_MSDU_END_WORD_MASK_SET(
		*msg_word, RX_MON_MSDU_END_WMASK);
	...
	*msg_word = 0;
	HTT_RX_RING_SELECTION_CFG_RX_PPDU_END_USR_STATS_WORD_MASK_V2_SET(
		*msg_word, RX_MON_PPDU_END_USR_STATS_WMASK);
}

void dp_rx_mon_pkt_tlv_offset_subscribe(
	uint32_t *msg_word, struct htt_rx_ring_tlv_filter *tlv_filter)
{
	if (!msg_word || !tlv_filter)
		return;
	...
	*msg_word = 0;
	if (tlv_filter->rx_pkt_tlv_offset) {
		HTT_RX_RING_SELECTION_CFG_ENABLE_RX_PKT_TLV_OFFSET_SET(
			*msg_word, 1);
		HTT_RX_RING_SELECTION_CFG_RX_PKT_TLV_OFFSET_SET(
			*msg_word, tlv_filter->rx_pkt_tlv_offset);
	}
}

void dp_rx_mon_enable_fpmo(uint32_t *msg_word,
			   struct htt_rx_ring_tlv_filter *tlv_filter)
{
	if (!msg_word || !tlv_filter)
		return;
	...
	if (tlv_filter->enable_fpmo) {
		...
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0000,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_ASSOC_REQ) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0001,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_ASSOC_RES) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0010,
			(tlv_filter->fpmo_mgmt_filter &
			 FILTER_MGMT_REASSOC_REQ) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0011,
			(tlv_filter->fpmo_mgmt_filter &
			 FILTER_MGMT_REASSOC_RES) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0100,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_PROBE_REQ) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0101,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_PROBE_RES) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0110,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_TIM_ADVT) ?
				1 :
				0);
		/* reserved */
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 0111,
			(tlv_filter->fpmo_mgmt_filter &
			 FILTER_MGMT_RESERVED_7) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1000,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_BEACON) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1001,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_ATIM) ? 1 :
									    0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1010,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_DISASSOC) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1011,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_AUTH) ? 1 :
									    0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1100,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_DEAUTH) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1101,
			(tlv_filter->fpmo_mgmt_filter & FILTER_MGMT_ACTION) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, MGMT, 1110,
			(tlv_filter->fpmo_mgmt_filter &
		...
		/* reserved */
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0001,
			(tlv_filter->fpmo_ctrl_filter &
			 FILTER_CTRL_RESERVED_2) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0010,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_TRIGGER) ?
				1 :
				0);
		/* reserved */
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0011,
			(tlv_filter->fpmo_ctrl_filter &
			 FILTER_CTRL_RESERVED_4) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0100,
			(tlv_filter->fpmo_ctrl_filter &
			 FILTER_CTRL_BF_REP_POLL) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0101,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_VHT_NDP) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0110,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_FRAME_EXT) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 0111,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_CTRLWRAP) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1000,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_BA_REQ) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1001,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_BA) ? 1 :
									  0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1010,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_PSPOLL) ?
				1 :
				0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1011,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_RTS) ? 1 :
									   0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1100,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_CTS) ? 1 :
									   0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1101,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_ACK) ? 1 :
									   0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1110,
			(tlv_filter->fpmo_ctrl_filter & FILTER_CTRL_CFEND) ? 1 :
									     0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG0, FPMO, CTRL, 1111,
			(tlv_filter->fpmo_ctrl_filter &
		...
		*msg_word = 0;
		...
		/* TYPE: DATA */
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG1, FPMO, DATA, MCAST,
			(tlv_filter->fpmo_data_filter & FILTER_DATA_MCAST) ? 1 :
									     0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG1, FPMO, DATA, UCAST,
			(tlv_filter->fpmo_data_filter & FILTER_DATA_UCAST) ? 1 :
									     0);
		htt_rx_ring_pkt_enable_subtype_set(
			*msg_word, FLAG1, FPMO, DATA, NULL,
			(tlv_filter->fpmo_data_filter & FILTER_DATA_NULL) ? 1 :
									    0);
			       ...
			       struct htt_tx_ring_tlv_filter *htt_tlv_filter)
{
	...
	if (tlv->tx_fes_setup)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_FES_SETUP, tlv->tx_fes_setup);

	if (tlv->tx_peer_entry)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_PEER_ENTRY, tlv->tx_peer_entry);

	if (tlv->tx_queue_extension)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_QUEUE_EXTENSION, tlv->tx_queue_extension);

	if (tlv->tx_last_mpdu_end)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_LAST_MPDU_END, tlv->tx_last_mpdu_end);

	if (tlv->tx_last_mpdu_fetched)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_LAST_MPDU_FETCHED,
			tlv->tx_last_mpdu_fetched);

	if (tlv->tx_data_sync)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_DATA_SYNC, tlv->tx_data_sync);

	if (tlv->pcu_ppdu_setup_init)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, PCU_PPDU_SETUP_INIT,
			tlv->pcu_ppdu_setup_init);

	if (tlv->fw2s_mon)
		htt_tx_monitor_tlv_filter_in0_enable_set(*msg_word, FW2SW_MON,
							 tlv->fw2s_mon);

	if (tlv->tx_loopback_setup)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_LOOPBACK_SETUP, tlv->tx_loopback_setup);

	if (tlv->sch_critical_tlv_ref)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, SCH_CRITICAL_TLV_REFERENCE,
			tlv->sch_critical_tlv_ref);

	if (tlv->ndp_preamble_done)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, NDP_PREAMBLE_DONE, tlv->ndp_preamble_done);

	if (tlv->tx_raw_frame_setup)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_RAW_OR_NATIVE_FRAME_SETUP,
			tlv->tx_raw_frame_setup);

	if (tlv->txpcu_user_setup)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TXPCU_USER_SETUP, tlv->txpcu_user_setup);

	if (tlv->rxpcu_setup)
		htt_tx_monitor_tlv_filter_in0_enable_set(*msg_word, RXPCU_SETUP,
							 tlv->rxpcu_setup);

	if (tlv->rxpcu_setup_complete)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, RXPCU_SETUP_COMPLETE,
			tlv->rxpcu_setup_complete);

	if (tlv->coex_tx_req)
		htt_tx_monitor_tlv_filter_in0_enable_set(*msg_word, COEX_TX_REQ,
							 tlv->coex_tx_req);

	if (tlv->rxpcu_user_setup)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, RXPCU_USER_SETUP, tlv->rxpcu_user_setup);

	if (tlv->rxpcu_user_setup_ext)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, RXPCU_USER_SETUP_EXT,
			tlv->rxpcu_user_setup_ext);

	if (tlv->wur_data)
		htt_tx_monitor_tlv_filter_in0_enable_set(*msg_word, TX_WUR_DATA,
							 tlv->wur_data);

	if (tlv->tqm_mpdu_global_start)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TQM_MPDU_GLOBAL_START,
			tlv->tqm_mpdu_global_start);

	if (tlv->tx_fes_setup_complete)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, TX_FES_SETUP_COMPLETE,
			tlv->tx_fes_setup_complete);

	if (tlv->scheduler_end)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, SCHEDULER_END, tlv->scheduler_end);

	if (tlv->sch_wait_instr_tx_path)
		htt_tx_monitor_tlv_filter_in0_enable_set(
			*msg_word, SCH_WAIT_INSTR_TX_PATH,
			       ...
			       struct htt_tx_ring_tlv_filter *htt_tlv_filter)
{
	...
	if (tlv->rx_response_required_info)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RX_RESPONSE_REQUIRED_INFO,
			tlv->rx_response_required_info);

	if (tlv->response_start_status)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RESPONSE_START_STATUS,
			tlv->response_start_status);

	if (tlv->response_end_status)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RESPONSE_END_STATUS,
			tlv->response_end_status);

	if (tlv->tx_fes_status_start)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_START,
			tlv->tx_fes_status_start);

	if (tlv->tx_fes_status_end)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_END, tlv->tx_fes_status_end);

	if (tlv->tx_fes_status_start_ppdu)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_START_PPDU,
			tlv->tx_fes_status_start_ppdu);

	if (tlv->tx_fes_status_user_ppdu)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_USER_PPDU,
			tlv->tx_fes_status_user_ppdu);

	if (tlv->tx_fes_status_ack_or_ba)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_ACK_OR_BA,
			tlv->tx_fes_status_ack_or_ba);

	if (tlv->tx_fes_status_1k_ba)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_1K_BA,
			tlv->tx_fes_status_1k_ba);

	if (tlv->tx_fes_status_start_prot)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_START_PROT,
			tlv->tx_fes_status_start_prot);

	if (tlv->tx_fes_status_prot)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_PROT, tlv->tx_fes_status_prot);

	if (tlv->tx_fes_status_user_response)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TX_FES_STATUS_USER_RESPONSE,
			tlv->tx_fes_status_user_response);

	if (tlv->rx_frame_bitmap_ack)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RX_FRAME_BITMAP_ACK,
			tlv->rx_frame_bitmap_ack);

	if (tlv->rx_frame_1k_bitmap_ack)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RX_FRAME_1K_BITMAP_ACK,
			tlv->rx_frame_1k_bitmap_ack);

	if (tlv->coex_tx_status)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, COEX_TX_STATUS, tlv->coex_tx_status);

	if (tlv->received_response_info)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RECEIVED_RESPONSE_INFO,
			tlv->received_response_info);

	if (tlv->received_response_info_p2)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RECEIVED_RESPONSE_INFO_PART2,
			tlv->received_response_info_p2);

	if (tlv->ofdma_trigger_details)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, OFDMA_TRIGGER_DETAILS,
			tlv->ofdma_trigger_details);

	if (tlv->received_trigger_info)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, RECEIVED_TRIGGER_INFO,
			tlv->received_trigger_info);

	if (tlv->pdg_tx_request)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, PDG_TX_REQUEST, tlv->pdg_tx_request);

	if (tlv->pdg_response)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, PDG_RESPONSE, tlv->pdg_response);

	if (tlv->pdg_trig_response)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, PDG_TRIG_RESPONSE, tlv->pdg_trig_response);

	if (tlv->trigger_response_tx_done)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, TRIGGER_RESPONSE_TX_DONE,
			tlv->trigger_response_tx_done);

	if (tlv->prot_tx_end)
		htt_tx_monitor_tlv_filter_in1_enable_set(*msg_word, PROT_TX_END,
							 tlv->prot_tx_end);

	if (tlv->ppdu_tx_end)
		htt_tx_monitor_tlv_filter_in1_enable_set(*msg_word, PPDU_TX_END,
							 tlv->ppdu_tx_end);

	if (tlv->r2r_status_end)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, R2R_STATUS_END, tlv->r2r_status_end);

	if (tlv->flush_req)
		htt_tx_monitor_tlv_filter_in1_enable_set(*msg_word, FLUSH_REQ,
							 tlv->flush_req);

	if (tlv->mactx_phy_desc)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, MACTX_PHY_DESC, tlv->mactx_phy_desc);

	if (tlv->mactx_user_desc_cmn)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, MACTX_USER_DESC_COMMON,
			tlv->mactx_user_desc_cmn);

	if (tlv->mactx_user_desc_per_usr)
		htt_tx_monitor_tlv_filter_in1_enable_set(
			*msg_word, MACTX_USER_DESC_PER_USER,
			tlv->mactx_user_desc_per_usr);

	if (tlv->l_sig_a)
		htt_tx_monitor_tlv_filter_in1_enable_set(*msg_word, L_SIG_A,
							 tlv->l_sig_a);

	if (tlv->l_sig_b)
		htt_tx_monitor_tlv_filter_in1_enable_set(*msg_word, L_SIG_B,
							 tlv->l_sig_b);
}
			       ...
			       struct htt_tx_ring_tlv_filter *htt_tlv_filter)
{
	...
	if (tlv->ht_sig)
		htt_tx_monitor_tlv_filter_in2_enable_set(*msg_word, HT_SIG,
							 tlv->ht_sig);

	if (tlv->vht_sig_a)
		htt_tx_monitor_tlv_filter_in2_enable_set(*msg_word, VHT_SIG_A,
							 tlv->vht_sig_a);

	if (tlv->vht_sig_b_su20)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_SU20, tlv->vht_sig_b_su20);

	if (tlv->vht_sig_b_su40)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_SU40, tlv->vht_sig_b_su40);

	if (tlv->vht_sig_b_su80)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_SU80, tlv->vht_sig_b_su80);

	if (tlv->vht_sig_b_su160)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_SU160, tlv->vht_sig_b_su160);

	if (tlv->vht_sig_b_mu20)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_MU20, tlv->vht_sig_b_mu20);

	if (tlv->vht_sig_b_mu40)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_MU40, tlv->vht_sig_b_mu40);

	if (tlv->vht_sig_b_mu80)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_MU80, tlv->vht_sig_b_mu80);

	if (tlv->vht_sig_b_mu160)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, VHT_SIG_B_MU160, tlv->vht_sig_b_mu160);

	if (tlv->tx_service)
		htt_tx_monitor_tlv_filter_in2_enable_set(*msg_word, TX_SERVICE,
							 tlv->tx_service);

	if (tlv->he_sig_a_su)
		htt_tx_monitor_tlv_filter_in2_enable_set(*msg_word, HE_SIG_A_SU,
							 tlv->he_sig_a_su);

	if (tlv->he_sig_a_mu_dl)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, HE_SIG_A_MU_DL, tlv->he_sig_a_mu_dl);

	if (tlv->he_sig_a_mu_ul)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, HE_SIG_A_MU_UL, tlv->he_sig_a_mu_ul);

	if (tlv->he_sig_b1_mu)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, HE_SIG_B1_MU, tlv->he_sig_b1_mu);

	if (tlv->he_sig_b2_mu)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, HE_SIG_B2_MU, tlv->he_sig_b2_mu);

	if (tlv->he_sig_b2_ofdma)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, HE_SIG_B2_OFDMA, tlv->he_sig_b2_ofdma);

	if (tlv->u_sig_eht_su_mu)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, U_SIG_EHT_SU_MU, tlv->u_sig_eht_su_mu);

	if (tlv->u_sig_eht_su)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, U_SIG_EHT_SU, tlv->u_sig_eht_su);

	if (tlv->u_sig_eht_tb)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, U_SIG_EHT_TB, tlv->u_sig_eht_tb);

	if (tlv->eht_sig_usr_su)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, EHT_SIG_USR_SU, tlv->eht_sig_usr_su);

	if (tlv->eht_sig_usr_mu_mimo)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, EHT_SIG_USR_MU_MIMO,
			tlv->eht_sig_usr_mu_mimo);

	if (tlv->eht_sig_usr_ofdma)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, EHT_SIG_USR_OFDMA, tlv->eht_sig_usr_ofdma);

	if (tlv->phytx_ppdu_header_info_request)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, PHYTX_PPDU_HEADER_INFO_REQUEST,
			tlv->phytx_ppdu_header_info_request);

	if (tlv->tqm_update_tx_mpdu_count)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TQM_UPDATE_TX_MPDU_COUNT,
			tlv->tqm_update_tx_mpdu_count);

	if (tlv->tqm_acked_mpdu)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TQM_ACKED_MPDU, tlv->tqm_acked_mpdu);

	if (tlv->tqm_acked_1k_mpdu)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TQM_ACKED_1K_MPDU, tlv->tqm_acked_1k_mpdu);

	if (tlv->txpcu_buf_status)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TXPCU_BUFFER_STATUS, tlv->txpcu_buf_status);

	if (tlv->txpcu_user_buf_status)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TXPCU_USER_BUFFER_STATUS,
			tlv->txpcu_user_buf_status);

	if (tlv->txdma_stop_request)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TXDMA_STOP_REQUEST, tlv->txdma_stop_request);

	if (tlv->expected_response)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, EXPECTED_RESPONSE, tlv->expected_response);

	if (tlv->tx_mpdu_count_transfer_end)
		htt_tx_monitor_tlv_filter_in2_enable_set(
			*msg_word, TX_MPDU_COUNT_TRANSFER_END,
			       ...
			       struct htt_tx_ring_tlv_filter *htt_tlv_filter)
{
	...
	if (tlv->rx_trig_info)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RX_TRIG_INFO, tlv->rx_trig_info);

	if (tlv->rxpcu_tx_setup_clear)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RXPCU_TX_SETUP_CLEAR,
			tlv->rxpcu_tx_setup_clear);

	if (tlv->rx_frame_bitmap_req)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RX_FRAME_BITMAP_REQ,
			tlv->rx_frame_bitmap_req);

	if (tlv->rx_phy_sleep)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RX_PHY_SLEEP, tlv->rx_phy_sleep);

	if (tlv->txpcu_preamble_done)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, TXPCU_PREAMBLE_DONE,
			tlv->txpcu_preamble_done);

	if (tlv->txpcu_phytx_debug32)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, TXPCU_PHYTX_DEBUG32,
			tlv->txpcu_phytx_debug32);

	if (tlv->txpcu_phytx_other_transmit_info32)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, TXPCU_PHYTX_OTHER_TRANSMIT_INFO32,
			tlv->txpcu_phytx_other_transmit_info32);

	if (tlv->rx_ppdu_noack_report)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RX_PPDU_NO_ACK_REPORT,
			tlv->rx_ppdu_noack_report);

	if (tlv->rx_ppdu_ack_report)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RX_PPDU_ACK_REPORT, tlv->rx_ppdu_ack_report);

	if (tlv->coex_rx_status)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, COEX_RX_STATUS, tlv->coex_rx_status);

	if (tlv->rx_start_param)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RX_START_PARAM, tlv->rx_start_param);

	if (tlv->tx_cbf_info)
		htt_tx_monitor_tlv_filter_in3_enable_set(*msg_word, TX_CBF_INFO,
							 tlv->tx_cbf_info);

	if (tlv->rxpcu_early_rx_indication)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RXPCU_EARLY_RX_INDICATION,
			tlv->rxpcu_early_rx_indication);

	if (tlv->received_response_user_7_0)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RECEIVED_RESPONSE_USER_7_0,
			tlv->received_response_user_7_0);

	if (tlv->received_response_user_15_8)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RECEIVED_RESPONSE_USER_15_8,
			tlv->received_response_user_15_8);

	if (tlv->received_response_user_23_16)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RECEIVED_RESPONSE_USER_23_16,
			tlv->received_response_user_23_16);

	if (tlv->received_response_user_31_24)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RECEIVED_RESPONSE_USER_31_24,
			tlv->received_response_user_31_24);

	if (tlv->received_response_user_36_32)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, RECEIVED_RESPONSE_USER_36_32,
			tlv->received_response_user_36_32);

	if (tlv->rx_pm_info)
		htt_tx_monitor_tlv_filter_in3_enable_set(*msg_word, RX_PM_INFO,
							 tlv->rx_pm_info);

	if (tlv->rx_preamble)
		htt_tx_monitor_tlv_filter_in3_enable_set(*msg_word, RX_PREAMBLE,
							 tlv->rx_preamble);

	if (tlv->others)
		htt_tx_monitor_tlv_filter_in3_enable_set(*msg_word, OTHERS,
							 tlv->others);

	if (tlv->mactx_pre_phy_desc)
		htt_tx_monitor_tlv_filter_in3_enable_set(
			*msg_word, MACTX_PRE_PHY_DESC, tlv->mactx_pre_phy_desc);
}
 ...
 */
static int htt_h2t_tx_ring_cfg(struct htt_soc *htt_soc, int pdev_id,
			       ...
			       struct htt_tx_ring_tlv_filter *htt_tlv_filter)
{
   ...
   */
	...
	HTT_TX_MONITOR_CFG_PDEV_ID_SET(*msg_word, target_pdev_id);

	HTT_TX_MONITOR_CFG_RING_ID_SET(*msg_word, htt_ring_id);

	HTT_TX_MONITOR_CFG_STATUS_TLV_SET(*msg_word, !!(srng_params.flags &
							HAL_SRNG_MSI_SWAP));
	...
	msg_word++;
	*msg_word = 0;
	HTT_TX_MONITOR_CFG_RING_BUFFER_SIZE_SET(*msg_word, ring_buf_size);

	if (htt_tlv_filter->mgmt_filter)
		htt_tx_ring_pkt_type_set(*msg_word, ENABLE_FLAGS, MGMT, 1);

	if (htt_tlv_filter->ctrl_filter)
		htt_tx_ring_pkt_type_set(*msg_word, ENABLE_FLAGS, CTRL, 1);

	if (htt_tlv_filter->data_filter)
		htt_tx_ring_pkt_type_set(*msg_word, ENABLE_FLAGS, DATA, 1);

	if (htt_tlv_filter->mgmt_dma_length)
		HTT_TX_MONITOR_CFG_CONFIG_LENGTH_MGMT_SET(
			*msg_word, htt_tlv_filter->mgmt_dma_length);

	if (htt_tlv_filter->ctrl_dma_length)
		HTT_TX_MONITOR_CFG_CONFIG_LENGTH_CTRL_SET(
			*msg_word, htt_tlv_filter->ctrl_dma_length);

	if (htt_tlv_filter->data_dma_length)
		HTT_TX_MONITOR_CFG_CONFIG_LENGTH_DATA_SET(
			*msg_word, htt_tlv_filter->data_dma_length);
	...
	*msg_word = 0;
	...
	if (htt_tlv_filter->mgmt_mpdu_start)
		HTT_TX_MONITOR_CFG_FILTER_IN_TX_MPDU_START_MGMT_SET(*msg_word,
								    1);

	if (htt_tlv_filter->ctrl_mpdu_start)
		HTT_TX_MONITOR_CFG_FILTER_IN_TX_MPDU_START_CTRL_SET(*msg_word,
								    1);

	if (htt_tlv_filter->data_mpdu_start)
		HTT_TX_MONITOR_CFG_FILTER_IN_TX_MPDU_START_DATA_SET(*msg_word,
								    1);

	if (htt_tlv_filter->mgmt_msdu_start)
		HTT_TX_MONITOR_CFG_FILTER_IN_TX_MSDU_START_MGMT_SET(*msg_word,
								    1);

	if (htt_tlv_filter->ctrl_msdu_start)
		HTT_TX_MONITOR_CFG_FILTER_IN_TX_MSDU_START_CTRL_SET(*msg_word,
								    1);

	if (htt_tlv_filter->data_msdu_start)
		HTT_TX_MONITOR_CFG_FILTER_IN_TX_MSDU_START_DATA_SET(*msg_word,
								    1);
	...
	*msg_word = 0;
	if (htt_tlv_filter->wmask.tx_fes_setup)
		HTT_TX_MONITOR_CFG_TX_FES_SETUP_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_fes_setup);

	if (htt_tlv_filter->wmask.tx_peer_entry)
		HTT_TX_MONITOR_CFG_TX_PEER_ENTRY_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_peer_entry);

	if (htt_tlv_filter->wmask.tx_queue_ext)
		HTT_TX_MONITOR_CFG_TX_QUEUE_EXT_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_queue_ext);

	if (htt_tlv_filter->wmask.tx_msdu_start)
		HTT_TX_MONITOR_CFG_TX_MSDU_START_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_msdu_start);

	/* word 8 */
	msg_word++;
	*msg_word = 0;
	if (htt_tlv_filter->wmask.pcu_ppdu_setup_init)
		HTT_TX_MONITOR_CFG_PCU_PPDU_SETUP_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.pcu_ppdu_setup_init);
	...
	*msg_word = 0;
	...
	if (htt_tlv_filter->wmask.tx_mpdu_start)
		HTT_TX_MONITOR_CFG_TX_MPDU_START_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_mpdu_start);

	if (htt_tlv_filter->wmask.rxpcu_user_setup)
		HTT_TX_MONITOR_CFG_RXPCU_USER_SETUP_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.rxpcu_user_setup);

	htt_tx_ring_pkt_type_set(*msg_word, ENABLE_MSDU_OR_MPDU_LOGGING, MGMT,
				 htt_tlv_filter->mgmt_mpdu_log);

	htt_tx_ring_pkt_type_set(*msg_word, ENABLE_MSDU_OR_MPDU_LOGGING, CTRL,
				 htt_tlv_filter->ctrl_mpdu_log);

	htt_tx_ring_pkt_type_set(*msg_word, ENABLE_MSDU_OR_MPDU_LOGGING, DATA,
	...
	*msg_word = 0;
	...
	if (htt_tlv_filter->wmask.tx_queue_ext)
		HTT_TX_MONITOR_CFG_TX_QUEUE_EXT_V2_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_queue_ext);

	if (htt_tlv_filter->wmask.tx_peer_entry)
		HTT_TX_MONITOR_CFG_TX_PEER_ENTRY_V2_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_peer_entry);
	...
	*msg_word = 0;
	...
	if (htt_tlv_filter->wmask.tx_fes_status_end)
		HTT_TX_MONITOR_CFG_FES_STATUS_END_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_fes_status_end);

	if (htt_tlv_filter->wmask.response_end_status)
		HTT_TX_MONITOR_CFG_RESPONSE_END_STATUS_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.response_end_status);
	...
	*msg_word = 0;
	...
	if (htt_tlv_filter->wmask.tx_fes_status_prot)
		HTT_TX_MONITOR_CFG_FES_STATUS_PROT_WORD_MASK_SET(
			*msg_word, htt_tlv_filter->wmask.tx_fes_status_prot);
	...
	status = DP_HTT_SEND_HTC_PKT(soc, pkt, HTT_H2T_MSG_TYPE_TX_MONITOR_CFG,
				     htt_logger_bufp);
...
static void
dp_tx_mon_filter_set_downstream_tlvs(struct htt_tx_ring_tlv_filter *filter)
{
...
static void dp_tx_mon_filter_set_all(struct dp_mon_pdev_be *mon_pdev_be,
				     struct htt_tx_ring_tlv_filter *filter)
{
	qdf_mem_zero(&filter->dtlvs, sizeof(filter->dtlvs));
	qdf_mem_zero(&filter->utlvs, sizeof(filter->utlvs));
	qdf_mem_zero(&filter->wmask, sizeof(filter->wmask));
...
static void
dp_tx_mon_filter_set_word_mask(struct dp_pdev *pdev,
			       struct htt_tx_ring_tlv_filter *filter)
{
...
static void dp_rx_mon_filter_show_filter(struct dp_mon_filter_be *filter)
{
	...
	DP_MON_FILTER_PRINT("packet_header: %d", rx_tlv_filter->packet_header);
	...
	DP_MON_FILTER_PRINT("rx_hdr_length: %d", rx_tlv_filter->rx_hdr_length);
...
static void dp_tx_mon_filter_show_filter(struct dp_mon_filter_be *filter)
{
	...
	DP_MON_FILTER_PRINT("rx_trig_info: %d", tlv_filter->utlvs.rx_trig_info);
	...
	DP_MON_FILTER_PRINT("rx_phy_sleep: %d", tlv_filter->utlvs.rx_phy_sleep);
	...
	DP_MON_FILTER_PRINT(
		"txpcu_phytx_other_transmit_info32: %d",
	...
	DP_MON_FILTER_PRINT("tx_cbf_info: %d", tlv_filter->utlvs.tx_cbf_info);
	...
	DP_MON_FILTER_PRINT("rx_pm_info: %d", tlv_filter->utlvs.rx_pm_info);
	DP_MON_FILTER_PRINT("rx_preamble: %d", tlv_filter->utlvs.rx_preamble);
	DP_MON_FILTER_PRINT("others: %d", tlv_filter->utlvs.others);
				     ...
				     struct dp_mon_filter_be *filter)
{
	...
	DP_MON_FILTER_PRINT("[Mode %d]: Valid: %d", mode,
			    filter->rx_tlv_filter.valid);
 ...
 * @filter: tlv filter
 */
static void dp_rx_mon_filter_h2t_setup(struct dp_soc *soc, struct dp_pdev *pdev,
				       enum dp_mon_filter_srng_type srng_type,
				       struct dp_mon_filter *filter)
{
   ...
   */
	for (current_mode = 0; current_mode < DP_MON_FILTER_MAX_MODE;
	     current_mode++) {
		mon_filter = &mon_pdev_be->filter_be[current_mode][srng_type];
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_TLV);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_FP_MGMT);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_MO_MGMT);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_FP_CTRL);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_MO_CTRL);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_FP_DATA);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_MO_DATA);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_MD_DATA);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_MD_MGMT);
     ...
     */
		src_filter = DP_MON_FILTER_GET(
			&mon_filter->rx_tlv_filter.tlv_filter, FILTER_MD_CTRL);
     ...
     */
		if (src_tlv_filter->mgmt_mpdu_log && !tlv_filter->mgmt_mpdu_log)
     ...
     */
		if (src_tlv_filter->ctrl_mpdu_log && !tlv_filter->ctrl_mpdu_log)
     ...
     */
		if (src_tlv_filter->data_mpdu_log && !tlv_filter->data_mpdu_log)
     ...
     * set hdr tlv length
     */
		if (src_tlv_filter->rx_hdr_length && !tlv_filter->rx_hdr_length)
     ...
     */
		if (src_tlv_filter->enable_fpmo && !tlv_filter->enable_fpmo) {
			tlv_filter->enable_fpmo = src_tlv_filter->enable_fpmo;
...
static void
dp_tx_mon_downstream_tlv_set(struct htt_tx_ring_tlv_filter *dst_filter,
			     struct htt_tx_ring_tlv_filter *src_filter)
{
	dst_filter->dtlvs.tx_fes_setup |= src_filter->dtlvs.tx_fes_setup;
	dst_filter->dtlvs.tx_peer_entry |= src_filter->dtlvs.tx_peer_entry;
	...
	dst_filter->dtlvs.tx_data_sync |= src_filter->dtlvs.tx_data_sync;
	dst_filter->dtlvs.pcu_ppdu_setup_init |=
		src_filter->dtlvs.pcu_ppdu_setup_init;
	dst_filter->dtlvs.fw2s_mon |= src_filter->dtlvs.fw2s_mon;
	...
	dst_filter->dtlvs.rxpcu_setup |= src_filter->dtlvs.rxpcu_setup;
	dst_filter->dtlvs.rxpcu_setup_complete |=
		src_filter->dtlvs.rxpcu_setup_complete;
	dst_filter->dtlvs.coex_tx_req |= src_filter->dtlvs.coex_tx_req;
...
static void
dp_tx_mon_upstream_tlv_set(struct htt_tx_ring_tlv_filter *dst_filter,
			   struct htt_tx_ring_tlv_filter *src_filter)
{
	...
	dst_filter->utlvs.coex_tx_status |= src_filter->utlvs.coex_tx_status;
	...
	dst_filter->utlvs.pdg_tx_request |= src_filter->utlvs.pdg_tx_request;
	dst_filter->utlvs.pdg_response |= src_filter->utlvs.pdg_response;
	...
	dst_filter->utlvs.prot_tx_end |= src_filter->utlvs.prot_tx_end;
	dst_filter->utlvs.ppdu_tx_end |= src_filter->utlvs.ppdu_tx_end;
	dst_filter->utlvs.r2r_status_end |= src_filter->utlvs.r2r_status_end;
	dst_filter->utlvs.flush_req |= src_filter->utlvs.flush_req;
	dst_filter->utlvs.mactx_phy_desc |= src_filter->utlvs.mactx_phy_desc;
	...
	dst_filter->utlvs.tqm_acked_mpdu |= src_filter->utlvs.tqm_acked_mpdu;
	...
	dst_filter->utlvs.u_sig_eht_su_mu |= src_filter->utlvs.u_sig_eht_su_mu;
	dst_filter->utlvs.u_sig_eht_su |= src_filter->utlvs.u_sig_eht_su;
	dst_filter->utlvs.u_sig_eht_tb |= src_filter->utlvs.u_sig_eht_tb;
	dst_filter->utlvs.eht_sig_usr_su |= src_filter->utlvs.eht_sig_usr_su;
	...
	dst_filter->utlvs.he_sig_a_su |= src_filter->utlvs.he_sig_a_su;
	dst_filter->utlvs.he_sig_a_mu_dl |= src_filter->utlvs.he_sig_a_mu_dl;
	dst_filter->utlvs.he_sig_a_mu_ul |= src_filter->utlvs.he_sig_a_mu_ul;
	dst_filter->utlvs.he_sig_b1_mu |= src_filter->utlvs.he_sig_b1_mu;
	dst_filter->utlvs.he_sig_b2_mu |= src_filter->utlvs.he_sig_b2_mu;
	dst_filter->utlvs.he_sig_b2_ofdma |= src_filter->utlvs.he_sig_b2_ofdma;
	dst_filter->utlvs.vht_sig_b_mu160 |= src_filter->utlvs.vht_sig_b_mu160;
	dst_filter->utlvs.vht_sig_b_mu80 |= src_filter->utlvs.vht_sig_b_mu80;
	dst_filter->utlvs.vht_sig_b_mu40 |= src_filter->utlvs.vht_sig_b_mu40;
	dst_filter->utlvs.vht_sig_b_mu20 |= src_filter->utlvs.vht_sig_b_mu20;
	dst_filter->utlvs.vht_sig_b_su160 |= src_filter->utlvs.vht_sig_b_su160;
	dst_filter->utlvs.vht_sig_b_su80 |= src_filter->utlvs.vht_sig_b_su80;
	dst_filter->utlvs.vht_sig_b_su40 |= src_filter->utlvs.vht_sig_b_su40;
	dst_filter->utlvs.vht_sig_b_su20 |= src_filter->utlvs.vht_sig_b_su20;
	dst_filter->utlvs.vht_sig_a |= src_filter->utlvs.vht_sig_a;
	dst_filter->utlvs.ht_sig |= src_filter->utlvs.ht_sig;
	dst_filter->utlvs.l_sig_b |= src_filter->utlvs.l_sig_b;
	dst_filter->utlvs.l_sig_a |= src_filter->utlvs.l_sig_a;
	dst_filter->utlvs.tx_service |= src_filter->utlvs.tx_service;
	...
	dst_filter->utlvs.rx_trig_info |= src_filter->utlvs.rx_trig_info;
	...
	dst_filter->utlvs.rx_phy_sleep |= src_filter->utlvs.rx_phy_sleep;
	...
	dst_filter->utlvs.coex_rx_status |= src_filter->utlvs.coex_rx_status;
	dst_filter->utlvs.rx_start_param |= src_filter->utlvs.rx_start_param;
	dst_filter->utlvs.tx_cbf_info |= src_filter->utlvs.tx_cbf_info;
	...
	dst_filter->utlvs.rx_pm_info |= src_filter->utlvs.rx_pm_info;
	dst_filter->utlvs.rx_preamble |= src_filter->utlvs.rx_preamble;
	dst_filter->utlvs.others |= src_filter->utlvs.others;
	dst_filter->utlvs.mactx_pre_phy_desc |=
		src_filter->utlvs.mactx_pre_phy_desc;
}

static void
dp_tx_mon_wordmask_config_set(struct htt_tx_ring_tlv_filter *dst_filter,
			      struct htt_tx_ring_tlv_filter *src_filter)
{
	dst_filter->wmask.pcu_ppdu_setup_init |=
		src_filter->wmask.pcu_ppdu_setup_init;
	dst_filter->wmask.tx_peer_entry |= src_filter->wmask.tx_peer_entry;
	dst_filter->wmask.tx_queue_ext |= src_filter->wmask.tx_queue_ext;
	...
	dst_filter->wmask.tx_fes_setup |= src_filter->wmask.tx_fes_setup;
	dst_filter->wmask.tx_msdu_start |= src_filter->wmask.tx_msdu_start;
	dst_filter->wmask.tx_mpdu_start |= src_filter->wmask.tx_mpdu_start;
	dst_filter->wmask.rxpcu_user_setup |=
		src_filter->wmask.rxpcu_user_setup;
	dst_filter->compaction_enable |= src_filter->compaction_enable;
}
 ...
 * @filter: tlv filter
 */
static void dp_tx_mon_filter_h2t_setup(struct dp_soc *soc, struct dp_pdev *pdev,
				       enum dp_mon_filter_srng_type srng_type,
				       struct dp_mon_filter_be *filter)
{
	int32_t current_mode = 0;
...
static QDF_STATUS
dp_tx_mon_ht2_ring_cfg(struct dp_soc *soc, struct dp_pdev *pdev,
		       enum dp_mon_filter_srng_type srng_type,
		       struct htt_tx_ring_tlv_filter *tlv_filter)
{
	...
	for (mac_id = 0; mac_id < max_mac_rings; mac_id++) {
		...
		hal_ring_hdl = mon_soc_be->tx_mon_dst_ring[lmac_id].hal_srng;
...
QDF_STATUS dp_tx_mon_filter_update_2_0(struct dp_pdev *pdev)
{
	...
	dp_tx_mon_ht2_ring_cfg(soc, pdev, srng_type, &filter.tx_tlv_filter);
...
{
	struct dp_mon_filter_be filter = { 0 };
	enum dp_mon_filter_mode filter_mode = DP_MON_FILTER_LITE_MON_MODE;
	enum dp_mon_filter_srng_type srng_type =
		DP_MON_FILTER_SRNG_TYPE_RXMON_DEST;
...
void dp_mon_filter_setup_rx_lite_mon(struct dp_mon_pdev_be *be_mon_pdev)
{
	...
	if (config->rx_config.fpmo_enabled) {
		...
		rx_tlv_filter->tlv_filter.fpmo_mgmt_filter =
			config->rx_config
				.mgmt_filter[DP_MON_FRM_FILTER_MODE_FP_MO];
		rx_tlv_filter->tlv_filter.fpmo_ctrl_filter =
			config->rx_config
				.ctrl_filter[DP_MON_FRM_FILTER_MODE_FP_MO];
		rx_tlv_filter->tlv_filter.fpmo_data_filter =
			config->rx_config
				.data_filter[DP_MON_FRM_FILTER_MODE_FP_MO];
	}
   ...
   */
	if (dp_lite_mon_is_full_len_configured(mgmt_len, ctrl_len, data_len)) {
	...
	max_custom_len =
		dp_lite_mon_get_max_custom_len(mgmt_len, ctrl_len, data_len);
...
uint8_t tx_lite_mon_set_len(uint16_t len)
{
	...
	default:
		dp_mon_filter_err(
			"Invalid length %d, Using minimal length of 64B", len);
		return DMA_LENGTH_64B;
	}
}
...
void dp_mon_filter_setup_tx_lite_mon(struct dp_pdev *pdev)
{
	...
	if (config->tx_config.md_enabled || config->tx_config.mo_enabled ||
		...
		dp_mon_filter_err(
			"md mo and fpmo are invalid filter configuration for Tx");
		return;
	}
	...
	if (config->tx_config.mgmt_filter[DP_MON_FRM_FILTER_MODE_FP]) {
		...
		tx_tlv_filter->mgmt_dma_length = tx_lite_mon_set_len(
			config->tx_config.len[WLAN_FC0_TYPE_MGMT]);
	...
	if (config->tx_config.ctrl_filter[DP_MON_FRM_FILTER_MODE_FP]) {
		...
		tx_tlv_filter->ctrl_dma_length = tx_lite_mon_set_len(
			config->tx_config.len[WLAN_FC0_TYPE_CTRL]);
		if ((config->tx_config.level == CDP_LITE_MON_LEVEL_MPDU) ||
		    (config->tx_config.level == CDP_LITE_MON_LEVEL_PPDU))
   ...
   */
	...
	if (config->tx_config.data_filter[DP_MON_FRM_FILTER_MODE_FP]) {
		...
		tx_tlv_filter->data_dma_length = tx_lite_mon_set_len(
			config->tx_config.len[WLAN_FC0_TYPE_DATA]);
		if ((config->tx_config.level == CDP_LITE_MON_LEVEL_MPDU) ||
		    (config->tx_config.level == CDP_LITE_MON_LEVEL_PPDU))
 ...
 * Return: void
 */
static void dp_cfr_filter_2_0(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
			      bool enable,
