		       ...
		       uint32_t work_done)
{
	...
	dp_mon_debug_rl(
		"tx_ppdu_info[%u :D %u] STATUS[R %llu: F %llu] PKT_BUF[R %llu: F %llu : "
		"P %llu : S %llu]",
		tx_mon_be->tx_ppdu_info_list_depth,
		tx_mon_be->defer_ppdu_info_list_depth,
		tx_mon_be->stats.status_buf_recv,
		tx_mon_be->stats.status_buf_free, tx_mon_be->stats.pkt_buf_recv,
...
static inline uint32_t dp_tx_mon_srng_process_2_0(struct dp_soc *soc,
						  ...
						  uint32_t quota)
{
	...
	while (qdf_likely((tx_mon_dst_ring_desc = (void *)hal_srng_dst_peek(
		...
		hal_be_get_mon_dest_status(soc->hal_soc, tx_mon_dst_ring_desc,
					   &hal_mon_tx_desc);

		if (hal_mon_tx_desc.empty_descriptor) {
			/* update stats counter */
			dp_mon_debug(
				"P_ID:%d INIT:%d E_DESC:%d R_ID:%d L_CNT:%d  DROP[PPDU:%d MPDU:%d "
				"TLV:%d] E_O_PPDU:%d",
		...
		dp_mon_debug(
			"P_ID:%d INIT:%d E_DESC:%d R_ID:%d L_CNT:%d BUF_ADDR: 0x%llx "
			"E_OFF: %d E_REA: %d",
			hal_mon_tx_desc.ppdu_id, hal_mon_tx_desc.initiator,
			hal_mon_tx_desc.empty_descriptor,
			hal_mon_tx_desc.ring_id, hal_mon_tx_desc.looping_count,
			hal_mon_tx_desc.buf_addr, hal_mon_tx_desc.end_offset,
			hal_mon_tx_desc.end_reason);

		mon_desc = (struct dp_mon_desc *)(uintptr_t)(hal_mon_tx_desc
								     .buf_addr);
		...
		if (qdf_unlikely(!status_frag)) {
			dp_mon_debug(
				"P_ID:%d INIT:%d E_DESC:%d R_ID:%d L_CNT:%d BUF_ADDR: "
				"0x%llx E_OFF: %d E_REA: %d",
				hal_mon_tx_desc.ppdu_id,
				hal_mon_tx_desc.initiator,
				hal_mon_tx_desc.empty_descriptor,
				hal_mon_tx_desc.ring_id,
		...
		if ((hal_mon_tx_desc.end_reason == HAL_MON_FLUSH_DETECTED) ||
			...
			dp_tx_mon_status_free_packet_buf(
				pdev, status_frag, end_offset, &mon_desc_list);
		...
		dp_tx_process_pktlog_be(soc, pdev, status_frag, end_offset);

		dp_tx_mon_process_status_tlv(soc, pdev, &hal_mon_tx_desc,
	...
	if (mon_desc_list.tx_mon_reap_cnt) {
		dp_mon_buffers_replenish(soc, &mon_soc_be->tx_mon_buf_ring,
					 ...
					 &mon_desc_list.tail, &replenish_cnt);
	}
	qdf_spin_unlock_bh(&mon_pdev->mon_lock);
	dp_mon_debug("mac_id: %d, work_done:%d tx_monitor_reap_cnt:%d", mac_id,
		     work_done, mon_desc_list.tx_mon_reap_cnt);
...
uint32_t dp_tx_mon_process_2_0(struct dp_soc *soc, struct dp_intr *int_ctx,
...
void dp_tx_mon_print_ring_stat_2_0(struct dp_pdev *pdev)
{
...
void dp_tx_mon_buf_desc_pool_deinit(struct dp_soc *soc)
{
	struct dp_mon_soc *mon_soc = soc->monitor_soc;
	struct dp_mon_soc_be *mon_soc_be =
		dp_get_be_mon_soc_from_dp_mon_soc(mon_soc);
...
QDF_STATUS dp_tx_mon_soc_init_2_0(struct dp_soc *soc)
{
	...
	if (dp_srng_init(soc, &mon_soc_be->tx_mon_buf_ring, TX_MONITOR_BUF, 0,
			 0)) {
...
dp_tx_mon_buf_desc_pool_alloc(struct dp_soc *soc)
{
	...
	qdf_print("%s:%d tx mon buf desc pool entries: %d", __func__, __LINE__,
		  entries);
	return dp_mon_desc_pool_alloc(soc, DP_MON_TX_DESC_POOL_TYPE, entries,
				      tx_mon_desc_pool);
}

void dp_tx_mon_buffers_free(struct dp_soc *soc)
{
	struct dp_mon_desc_pool *tx_mon_desc_pool;
	struct dp_mon_soc *mon_soc = soc->monitor_soc;
	struct dp_mon_soc_be *mon_soc_be =
...
dp_tx_mon_buffers_alloc(struct dp_soc *soc, uint32_t size)
{
	...
	return dp_mon_buffers_replenish(soc, mon_buf_ring, tx_mon_desc_pool,
 ...
 */
static inline uint32_t dp_tx_mon_nbuf_get_num_frag(qdf_nbuf_t nbuf)
{
	uint32_t num_frag = 0;
						...
						uint32_t ppdu_id)
{
	...
	struct dp_pdev_tx_monitor_be *tx_mon_be = &mon_pdev_be->tx_monitor_be;
	...
	if (type == TX_PROT_PPDU_INFO) {
		qdf_mem_zero(&tx_mon_be->prot_status_info,
			     sizeof(struct hal_tx_status_info));
		tx_mon_be->tx_prot_ppdu_info = tx_ppdu_info;
		TXMON_PPDU_HAL(tx_ppdu_info, is_data) = 0;
	} else {
		qdf_mem_zero(&tx_mon_be->data_status_info,
			     sizeof(struct hal_tx_status_info));
...
dp_config_enh_tx_monitor_2_0(struct dp_pdev *pdev, uint8_t val)
{
	...
	case TX_MON_BE_FULL_CAPTURE: {
		...
		status = dp_vdev_set_monitor_mode_buf_rings_tx_2_0(
			pdev, num_of_buffers);
	...
	case TX_MON_BE_PEER_FILTER: {
		status = dp_vdev_set_monitor_mode_buf_rings_tx_2_0(
			pdev, DP_MON_RING_FILL_LEVEL_DEFAULT);
		if (status != QDF_STATUS_SUCCESS) {
			dp_mon_err("Tx monitor buffer allocation failed");
			return status;
		}
 ...
 * Return: QDF_STATUS
 */
static inline QDF_STATUS dp_lite_mon_filter_ppdu(uint8_t mpdu_count,
						 uint8_t level)
{
			...
			struct ieee80211_frame_min_one *wh)
{
   ...
   */
	...
	{
		if (!qdf_mem_cmp(&peer->peer_mac.raw[0], &wh->i_addr1[0],
				 QDF_MAC_ADDR_SIZE)) {
			return QDF_STATUS_SUCCESS;
		}
	}
				...
				qdf_nbuf_t buf)
{
	...
	if (dp_tx_mon_nbuf_get_num_frag(buf)) {
		wh = (struct ieee80211_frame_min_one *)qdf_nbuf_get_frag_addr(
			buf, 0);
	} else {
		nbuf = qdf_nbuf_get_ext_list(buf);
		if (nbuf)
			wh = (struct ieee80211_frame_min_one *)qdf_nbuf_data(
				nbuf);
		else
			return QDF_STATUS_E_INVAL;
	}
...
dp_tx_lite_mon_filtering(struct dp_pdev *pdev,
			 struct dp_tx_ppdu_info *tx_ppdu_info, qdf_nbuf_t buf,
			 int mpdu_count)
{
	struct dp_mon_pdev *mon_pdev = pdev->monitor_pdev;
	struct dp_mon_pdev_be *mon_pdev_be =
			     ...
			     qdf_nbuf_t buf)
{
	...
	if (qdf_unlikely(!IS_LOCAL_PKT_CAPTURE_RUNNING(
...
static int dp_tx_handle_local_pkt_capture(struct dp_pdev *pdev, qdf_nbuf_t nbuf)
{
 ...
 * Return: void
 */
static void dp_tx_mon_send_to_stack(struct dp_pdev *pdev, qdf_nbuf_t mpdu,
				    uint32_t num_frag, uint32_t ppdu_id)
{
     ...
     */
		...
		dp_wdi_event_handler(WDI_EVENT_TX_PKT_CAPTURE, pdev->soc,
				     &tx_capture_info, HTT_INVALID_PEER,
				     WDI_NO_VAL, pdev->pdev_id);
	} else {
		dp_fill_lite_mon_vdev(&tx_capture_info, mon_pdev_be);
		dp_wdi_event_handler(WDI_EVENT_LITE_MON_TX, pdev->soc,
				     &tx_capture_info, HTT_INVALID_PEER,
				     WDI_NO_VAL, pdev->pdev_id);
	}
 ...
 * Return: void
 */
static void dp_tx_mon_send_per_usr_mpdu(struct dp_pdev *pdev,
					...
					uint8_t user_idx)
{
	...
	while ((buf = qdf_nbuf_queue_remove(usr_mpdu_q)) != NULL) {
		...
		qdf_nbuf_update_radiotap(&ppdu_info->hal_txmon.rx_status, buf,
					 qdf_nbuf_headroom(buf));
				    ...
				    struct dp_tx_ppdu_info *ppdu_info)
{
	...
	if (!tsft && ppdu_timestamp) {
		...
		uint32_t last_usec =
			(tx_mon_be->last_ppdu_timestamp / PHY_MEDIUM_MHZ);
		uint32_t diff = 0;
 ...
 * Return: void
 */
static inline void dp_tx_mon_update_channel_freq(struct dp_pdev *pdev,
						 struct dp_soc *soc,
						 uint16_t freq)
{
	if (soc && soc->cdp_soc.ol_ops->freq_to_channel) {
		...
		c_num = soc->cdp_soc.ol_ops->freq_to_channel(
 ...
 * Return: void
 */
static void dp_tx_mon_update_radiotap(struct dp_pdev *pdev,
				      struct dp_tx_ppdu_info *ppdu_info)
{
	uint32_t usr_idx = 0;
	...
	for (usr_idx = 0; usr_idx < num_users; usr_idx++) {
		...
		if (mpdu_q && (qdf_nbuf_queue_len(mpdu_q) > 1)) {
			TXMON_PPDU_COM(ppdu_info, rs_flags) |=
		...
		if (qdf_unlikely(!TXMON_PPDU_COM(ppdu_info, rate))) {
			...
			rate = dp_getrateindex(
				TXMON_PPDU_COM(ppdu_info, sgi),
				TXMON_PPDU_USR(ppdu_info, usr_idx, mcs),
				TXMON_PPDU_COM(ppdu_info, nss),
				TXMON_PPDU_COM(ppdu_info, preamble_type),
				TXMON_PPDU_COM(ppdu_info, bw), 0, &rix,
				&ratecode);
...
static void dp_tx_mon_ppdu_process(void *context)
{
			    ...
			    tx_ppdu_info_queue_elem, defer_ppdu_info_next)
	{
		...
		STAILQ_REMOVE(&tx_mon_be->defer_tx_ppdu_info_queue,
			      defer_ppdu_info, dp_tx_ppdu_info,
...
void dp_tx_ppdu_stats_attach_2_0(struct dp_pdev *pdev)
{
	...
	qdf_create_work(0, &tx_mon_be->post_ppdu_work, dp_tx_mon_ppdu_process,
			pdev);
...
void dp_tx_ppdu_stats_detach_2_0(struct dp_pdev *pdev)
{
   ...
   */
	qdf_spin_lock_bh(&tx_mon_be->tx_mon_list_lock);
	STAILQ_FOREACH_SAFE(tx_ppdu_info, &tx_mon_be->tx_ppdu_info_queue,
	...
	qdf_spin_lock_bh(&tx_mon_be->tx_mon_list_lock);
	STAILQ_FOREACH_SAFE(tx_ppdu_info, &tx_mon_be->defer_tx_ppdu_info_queue,
			    tx_ppdu_info_queue_elem, tx_ppdu_info_next)
	{
		/* remove dp_tx_ppdu_info from the list */
		STAILQ_REMOVE(&tx_mon_be->defer_tx_ppdu_info_queue,
			      tx_ppdu_info, dp_tx_ppdu_info,
			      tx_ppdu_info_queue_elem);
		/* decrement list length */
		tx_mon_be->defer_ppdu_info_list_depth--;
		/* free tx_ppdu_info */
...
QDF_STATUS dp_tx_mon_soc_htt_srng_setup_2_0(struct dp_soc *soc, int mac_id)
{
	struct dp_mon_soc *mon_soc = soc->monitor_soc;
	struct dp_mon_soc_be *mon_soc_be =
		dp_get_be_mon_soc_from_dp_mon_soc(mon_soc);
...
QDF_STATUS dp_tx_mon_pdev_rings_alloc_2_0(struct dp_pdev *pdev,
					  uint32_t lmac_id)
{
	struct dp_soc *soc = pdev->soc;
	int entries;
...
QDF_STATUS dp_tx_mon_soc_attach_2_0(struct dp_soc *soc, uint32_t lmac_id)
{
	...
	return dp_srng_alloc(soc, &mon_soc_be->tx_mon_buf_ring, TX_MONITOR_BUF,
			     entries, 0);
}
