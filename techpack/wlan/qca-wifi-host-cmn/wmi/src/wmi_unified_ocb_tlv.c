...
/**
 * send_ocb_start_timing_advert_cmd_tlv() - start sending the timing
 ...
 * Return: 0 on success
 */
static QDF_STATUS send_ocb_start_timing_advert_cmd_tlv(
	wmi_unified_t wmi_handle, struct ocb_timing_advert_param *timing_advert)
{
	...
	WMITLV_SET_HDR(buf_ptr, WMITLV_TAG_ARRAY_BYTE, len_template);
...
/**
 * send_ocb_stop_timing_advert_cmd_tlv() - stop sending the timing advertisement
 *frames on a channel
 * @wmi_handle: pointer to the wmi handle
 * @timing_advert: pointer to the timing advertisement struct
 *
 * Return: 0 on success
 */
static QDF_STATUS send_ocb_stop_timing_advert_cmd_tlv(
	wmi_unified_t wmi_handle, struct ocb_timing_advert_param *timing_advert)
{
	QDF_STATUS ret;
 ...
 */
static QDF_STATUS
send_dcc_get_stats_cmd_tlv(wmi_unified_t wmi_handle,
			   struct ocb_dcc_get_stats_param *get_stats_param)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_dcc_get_stats_cmd_fixed_param));
					       ...
					       uint32_t dcc_stats_bitmap)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_dcc_clear_stats_cmd_fixed_param));
	cmd->vdev_id = vdev_id;
 ...
 */
static QDF_STATUS
send_dcc_update_ndl_cmd_tlv(wmi_unified_t wmi_handle,
			    struct ocb_dcc_update_ndl_param *update_ndl_param)
{
	...
	/* Allocate memory for the WMI command */
	len = sizeof(*cmd) + WMI_TLV_HDR_SIZE +
	      update_ndl_param->dcc_ndl_chan_list_len + WMI_TLV_HDR_SIZE +
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_dcc_update_ndl_cmd_fixed_param));
					      ...
					      struct ocb_config *config)
{
   ...
   */
	...
	len = sizeof(*cmd) + WMI_TLV_HDR_SIZE +
	      config->channel_count * sizeof(wmi_channel) + WMI_TLV_HDR_SIZE +
	      ...
	      WMI_TLV_HDR_SIZE +
	      active_state_count * sizeof(wmi_dcc_ndl_active_state_config) +
	      WMI_TLV_HDR_SIZE +
 ...
 */
static QDF_STATUS
extract_ocb_tsf_timer_tlv(wmi_unified_t wmi_handle, void *evt_buf,
 ...
 */
static QDF_STATUS
extract_ocb_ndl_resp_tlv(wmi_unified_t wmi_handle, void *evt_buf,
 ...
 */
static QDF_STATUS
extract_ocb_dcc_stats_tlv(wmi_unified_t wmi_handle, void *evt_buf,
			  struct ocb_dcc_get_stats_response **resp)
{
	...
	if (fix_param->num_channels >
		    ((WMI_SVC_MSG_MAX_SIZE - sizeof(*fix_param)) /
	...
	response =
		qdf_mem_malloc(sizeof(*response) +
			       fix_param->num_channels *
				       sizeof(wmi_dcc_ndl_stats_per_channel));
	*resp = response;
	if (!response)
...
void wmi_ocb_attach_tlv(wmi_unified_t wmi_handle)
{
	...
	ops->extract_ocb_chan_config_resp = extract_ocb_channel_config_resp_tlv;
