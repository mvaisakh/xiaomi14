...
QDF_STATUS
wmi_unified_wow_patterns_to_fw_cmd(wmi_unified_t wmi_handle, uint8_t vdev_id,
				   ...
				   bool user, uint8_t default_patterns)
{
	if (wmi_handle->ops->send_wow_patterns_to_fw_cmd)
		return wmi_handle->ops->send_wow_patterns_to_fw_cmd(
			wmi_handle, vdev_id, ptrn_id, ptrn, ptrn_len,
			ptrn_offset, mask, mask_len, user, default_patterns);
...
QDF_STATUS
wmi_unified_add_clear_mcbc_filter_cmd(wmi_unified_t wmi_handle, uint8_t vdev_id,
					    ...
					    uint32_t gtk_offload_opcode)
{
	if (wmi_handle->ops->send_gtk_offload_cmd)
		return wmi_handle->ops->send_gtk_offload_cmd(
			wmi_handle, vdev_id, params, enable_offload,
			gtk_offload_opcode);
...
QDF_STATUS
wmi_unified_enable_enhance_multicast_offload_cmd(wmi_unified_t wmi_handle,
...
QDF_STATUS wmi_extract_gtk_rsp_event(wmi_unified_t wmi_handle, void *evt_buf,
				     struct pmo_gtk_rsp_params *gtk_rsp_param,
				     uint32_t len)
{
	if (wmi_handle->ops->extract_gtk_rsp_event)
		return wmi_handle->ops->extract_gtk_rsp_event(
			wmi_handle, evt_buf, gtk_rsp_param, len);
					     ...
					     uint16_t rate_limit_interval)
{
	if (wmi_handle->ops->send_wow_sta_ra_filter_cmd)
		return wmi_handle->ops->send_wow_sta_ra_filter_cmd(
			wmi_handle, vdev_id, default_pattern,
			rate_limit_interval);
...
QDF_STATUS
wmi_unified_lphb_config_hbenable_cmd(wmi_unified_t wmi_handle,
...
QDF_STATUS wmi_unified_config_packet_filter_cmd(
	wmi_unified_t wmi_handle, uint8_t vdev_id,
	struct pmo_rcv_pkt_fltr_cfg *rcv_filter_param, uint8_t filter_id,
	bool enable)
{
	if (wmi_handle->ops->send_config_packet_filter_cmd)
		return wmi_handle->ops->send_config_packet_filter_cmd(
			wmi_handle, vdev_id, rcv_filter_param, filter_id,
			enable);
					      ...
					      uint8_t ptrn_id, uint8_t vdev_id)
{
	if (wmi_handle->ops->send_wow_delete_pattern_cmd)
		return wmi_handle->ops->send_wow_delete_pattern_cmd(
			wmi_handle, ptrn_id, vdev_id);
						 ...
						 bool tx_pending_ind)
{
	if (wmi_handle->ops->send_host_wakeup_ind_to_fw_cmd)
		return wmi_handle->ops->send_host_wakeup_ind_to_fw_cmd(
			wmi_handle, tx_pending_ind);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_unified_wow_timer_pattern_cmd(wmi_unified_t wmi_handle,
					     uint8_t vdev_id, uint32_t cookie,
					     uint32_t time)
{
	if (wmi_handle->ops->send_wow_timer_pattern_cmd)
		return wmi_handle->ops->send_wow_timer_pattern_cmd(
			wmi_handle, vdev_id, cookie, time);
...
QDF_STATUS wmi_unified_set_app_type2_params_in_fw_cmd(
	wmi_unified_t wmi_handle, struct app_type2_params *appType2Params)
{
...
QDF_STATUS wmi_unified_app_type1_params_in_fw_cmd(
	wmi_unified_t wmi_handle, struct app_type1_params *app_type1_params)
{
...
QDF_STATUS
wmi_unified_config_icmp_offload_cmd(wmi_unified_t wmi_handle,
