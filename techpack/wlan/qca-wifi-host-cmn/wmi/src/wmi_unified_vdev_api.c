...
QDF_STATUS wmi_unified_send_multiple_vdev_set_param_cmd(
	struct wmi_unified *wmi_handle, struct multiple_vdev_set_param *param)
{
				       ...
				       struct beacon_params *param)
{
	if (wmi_handle->ops->send_beacon_send_cmd)
		return wmi_handle->ops->send_beacon_send_cmd(wmi_handle, param);
			     ...
			     struct vdev_delete_response *delete_rsp)
{
	if (wmi_handle->ops->extract_vdev_delete_resp)
		return wmi_handle->ops->extract_vdev_delete_resp(
			wmi_handle, evt_buf, delete_rsp);
...
QDF_STATUS wmi_unified_vdev_set_neighbour_rx_cmd_send(
	struct wmi_unified *wmi_handle, uint8_t macaddr[QDF_MAC_ADDR_SIZE],
...
QDF_STATUS
wmi_send_peer_vlan_config(struct wmi_unified *wmi_handle, uint8_t *macaddr,
			  struct peer_vlan_config_param param)
{
	...
	if (wmi_handle->ops->send_peer_vlan_config_cmd)
		return wmi_handle->ops->send_peer_vlan_config_cmd(
			wmi_handle, peer_mac, &param);
