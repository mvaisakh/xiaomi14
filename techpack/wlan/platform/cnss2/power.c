				...
				struct cnss_vreg_info *vreg)
{
	...
	cnss_pr_dbg("Got regulator: %s, min_uv: %u, max_uv: %u, load_ua: %u, "
...
static int cnss_vreg_on_single(struct cnss_vreg_info *vreg)
{
	...
	if (vreg->cfg.min_uv != 0 && vreg->cfg.max_uv != 0) {
		...
		if (ret) {
			cnss_pr_err(
				"Failed to set voltage for regulator %s, min_uv: %u, max_uv: "
				"%u, err = %d\n",
...
static int cnss_get_vreg(struct cnss_plat_data *plat_priv,
			 ...
			 struct cnss_vreg_cfg *vreg_cfg, u32 vreg_list_size)
{
	int ret = 0;
	int i;
	struct cnss_vreg_info *vreg;
	...
	for (i = 0; i < id_n; i++) {
		...
		if (plat_priv->dt_type == CNSS_DTT_MULTIEXCHG) {
			ret = of_property_read_string_index(
				dt_node, WLAN_VREGS_PROP, i, &vreg->cfg.name);
			if (ret) {
			  ...
			  struct list_head *vreg_list)
{
	...
	while (!list_empty(vreg_list)) {
		vreg = list_first_entry(vreg_list, struct cnss_vreg_info, list);
		list_del(&vreg->list);
		       ...
		       enum cnss_vreg_type type)
{
	...
	switch (type) {
	case CNSS_VREG_PRIM:
		ret = cnss_get_vreg(plat_priv, &plat_priv->vreg_list, vreg_cfg,
				    vreg_list_size);
		break;
	default:
		cnss_pr_err("Unsupported vreg type 0x%x\n", type);
...
cleanup:
	while (!list_empty(clk_list)) {
		clk_info =
			list_first_entry(clk_list, struct cnss_clk_info, list);
...
int cnss_get_pinctrl(struct cnss_plat_data *plat_priv)
{
	...
	if (of_find_property(dev->of_node, BOOTSTRAP_GPIO, NULL)) {
		pinctrl_info->bootstrap_active = pinctrl_lookup_state(
			pinctrl_info->pinctrl, BOOTSTRAP_ACTIVE);
		if (IS_ERR_OR_NULL(pinctrl_info->bootstrap_active)) {
			ret = PTR_ERR(pinctrl_info->bootstrap_active);
			cnss_pr_err(
				"Failed to get bootstrap active state, err = %d\n",
				ret);
			goto out;
		}
	}
	...
	if (of_find_property(dev->of_node, HOST_SOL_GPIO, NULL) &&
		...
		pinctrl_info->sol_default = pinctrl_lookup_state(
			pinctrl_info->pinctrl, SOL_DEFAULT);
	...
	if (of_find_property(dev->of_node, WLAN_EN_GPIO, NULL)) {
		pinctrl_info->wlan_en_gpio =
			of_get_named_gpio(dev->of_node, WLAN_EN_GPIO, 0);
		cnss_pr_dbg("WLAN_EN GPIO: %d\n", pinctrl_info->wlan_en_gpio);
		pinctrl_info->wlan_en_active = pinctrl_lookup_state(
			pinctrl_info->pinctrl, WLAN_EN_ACTIVE);
		if (IS_ERR_OR_NULL(pinctrl_info->wlan_en_active)) {
			ret = PTR_ERR(pinctrl_info->wlan_en_active);
			cnss_pr_err(
				"Failed to get wlan_en active state, err = %d\n",
				ret);
			goto out;
		}

		pinctrl_info->wlan_en_sleep = pinctrl_lookup_state(
			pinctrl_info->pinctrl, WLAN_EN_SLEEP);
		if (IS_ERR_OR_NULL(pinctrl_info->wlan_en_sleep)) {
			ret = PTR_ERR(pinctrl_info->wlan_en_sleep);
			cnss_pr_err(
				"Failed to get wlan_en sleep state, err = %d\n",
				ret);
			goto out;
		}
	...
	if (of_find_property(dev->of_node, BT_EN_GPIO, NULL)) {
		pinctrl_info->bt_en_gpio =
			of_get_named_gpio(dev->of_node, BT_EN_GPIO, 0);
	...
	if (of_find_property(dev->of_node, XO_CLK_GPIO, NULL)) {
		pinctrl_info->xo_clk_gpio =
			of_get_named_gpio(dev->of_node, XO_CLK_GPIO, 0);
	...
	if (of_find_property(dev->of_node, SW_CTRL_GPIO, NULL)) {
		pinctrl_info->sw_ctrl_gpio =
			of_get_named_gpio(dev->of_node, SW_CTRL_GPIO, 0);
   ...
   */
	gpio_id_n =
		of_property_count_u32_elems(dev->of_node, "mpm_wake_set_gpios");
	if (gpio_id_n > 0) {
		cnss_pr_dbg(
		...
		for (i = 0; i < gpio_id_n; i++) {
			...
			if (ret) {
				cnss_pr_err(
					"Failed to read gpio_id at index: %d\n",
					i);
				continue;
			}
			...
			if (ret < 0) {
				cnss_pr_err(
					"Failed to setup gpio_id: %d as interrupt wakeup capable, "
					"ret: %d\n",
					ret);
			} else {
				cnss_pr_dbg(
					"gpio_id: %d successfully setup for interrupt wakeup capable\n",
					gpio_id);
			}
		}
	} else {
...
int cnss_get_wlan_sw_ctrl(struct cnss_plat_data *plat_priv)
{
	...
	if (of_find_property(dev->of_node, WLAN_SW_CTRL_GPIO, NULL)) {
		pinctrl_info->wlan_sw_ctrl_gpio =
			of_get_named_gpio(dev->of_node, WLAN_SW_CTRL_GPIO, 0);
				     ...
				     bool state)
{
	...
	if (state) {
		if (!IS_ERR_OR_NULL(pinctrl_info->bootstrap_active)) {
			...
			if (ret) {
				cnss_pr_err(
					"Failed to select bootstrap active state, err = %d\n",
					ret);
				goto out;
			}
		...
		if (!IS_ERR_OR_NULL(pinctrl_info->sol_default)) {
			...
			if (ret) {
				cnss_pr_err(
					"Failed to select sol default state, err = %d\n",
					ret);
				goto out;
			}
		...
		if (!IS_ERR_OR_NULL(pinctrl_info->wlan_en_active)) {
			...
			if (ret) {
				cnss_pr_err(
					"Failed to select wlan_en active state, err = %d\n",
					ret);
				goto out;
			}
		...
		if (!IS_ERR_OR_NULL(pinctrl_info->wlan_en_sleep)) {
			...
			if (test_bit(CNSS_WLAN_HW_DISABLED,
			...
			if (ret) {
				cnss_pr_err(
					"Failed to select wlan_en sleep state, err = %d\n",
					ret);
				goto out;
			}
		} else {
...
int cnss_get_cpr_info(struct cnss_plat_data *plat_priv)
{
	...
	ret = of_property_read_string(plat_dev->dev.of_node, "qcom,cmd_db_name",
				      &cmd_db_name);
	if (ret) {
...
/**
 * cnss_aop_interface_init: Initialize AOP interface: either mbox channel or
 * direct QMP
...
/**
 * cnss_aop_send_msg: Sends json message to AOP using either mbox channel or
 * direct QMP
 * @plat_priv: Pointer to cnss platform data
				   ...
				   int val)
{
	...
	static const char *const aop_tcs_seq_str[] = {
		[CNSS_TCS_UP_SEQ] = "upval",
		[CNSS_TCS_DOWN_SEQ] = "dwnval",
	...
	snprintf(msg, CNSS_MBOX_MSG_MAX_LEN,
		 ...
		 cnss_aop_pdc_name_str(plat_priv, vreg_name), vreg_name,
		 aop_vreg_param_str[param], aop_tcs_seq_str[seq_param], val);
			      ...
			      struct wlfw_pmu_cfg_v01 *fw_pmu_cfg)
{
	...
	if (plat_priv->pmu_vreg_map_len <= 0 || !plat_priv->pmu_vreg_map ||
	...
	for (i = 0; i <= plat_vreg_param_len; i++) {
		if (plat_vreg_param[i].wake_volt > 0) {
			ret = cnss_aop_set_vreg_param(
				plat_priv, plat_vreg_param[i].vreg,
				CNSS_VREG_VOLTAGE, CNSS_TCS_UP_SEQ,
				plat_vreg_param[i].wake_volt);
		}
		if (plat_vreg_param[i].sleep_volt > 0) {
			ret = cnss_aop_set_vreg_param(
				plat_priv, plat_vreg_param[i].vreg,
				CNSS_VREG_VOLTAGE, CNSS_TCS_DOWN_SEQ,
...
void cnss_power_misc_params_init(struct cnss_plat_data *plat_priv)
{
	...
	plat_priv->pdc_init_table_len =
		of_property_count_strings(dev->of_node, "qcom,pdc_init_table");
	if (plat_priv->pdc_init_table_len > 0) {
		plat_priv->pdc_init_table =
			kcalloc(plat_priv->pdc_init_table_len, sizeof(char *),
				GFP_KERNEL);
		if (plat_priv->pdc_init_table) {
			ret = of_property_read_string_array(
				dev->of_node, "qcom,pdc_init_table",
				plat_priv->pdc_init_table,
				plat_priv->pdc_init_table_len);
			if (ret < 0)
	...
	plat_priv->vreg_pdc_map_len =
		of_property_count_strings(dev->of_node, "qcom,vreg_pdc_map");
	if (plat_priv->vreg_pdc_map_len > 0) {
		plat_priv->vreg_pdc_map = kcalloc(plat_priv->vreg_pdc_map_len,
						  sizeof(char *), GFP_KERNEL);
		if (plat_priv->vreg_pdc_map) {
			ret = of_property_read_string_array(
				dev->of_node, "qcom,vreg_pdc_map",
	...
	if (plat_priv->device_id == QCA6390_DEVICE_ID ||
		...
		ret = of_property_read_string(dev->of_node, "qcom,vreg_ol_cpr",
					      &plat_priv->vreg_ol_cpr);
		if (ret)
			cnss_pr_dbg("VReg for QCA6490 OL CPR not configured\n");

		ret = of_property_read_string(dev->of_node, "qcom,vreg_ipa",
					      &plat_priv->vreg_ipa);
		if (ret)
			cnss_pr_dbg(
	...
	if (ret > 0) {
		...
		if (cfg_arr) {
			ret = of_property_read_u32_array(
				...
				cfg_arr_size);
			if (!ret) {
				plat_priv->on_chip_pmic_devices_count =
					cfg_arr_size;
...
int cnss_update_cpr_info(struct cnss_plat_data *plat_priv)
{
	...
	if (!plat_priv->vreg_ol_cpr ||
		...
		return cnss_aop_set_vreg_param(
			plat_priv, plat_priv->vreg_ol_cpr, CNSS_VREG_VOLTAGE,
			CNSS_TCS_DOWN_SEQ, cpr_info->voltage);
	}
	...
	for (i = 0; i < MAX_TCS_NUM; i++) {
		for (j = 0; j < MAX_TCS_CMD_NUM; j++) {
			...
			tcs_cmd_addr =
				plat_priv->tcs_info.cmd_base_addr_io + offset;
			pmic_addr = readl_relaxed(tcs_cmd_addr);
			if (pmic_addr == cpr_info->cpr_pmic_addr) {
				tcs_cmd_data_addr =
					tcs_cmd_addr + TCS_CMD_DATA_ADDR_OFFSET;
...
int cnss_enable_int_pow_amp_vreg(struct cnss_plat_data *plat_priv)
{
	...
	if (!plat_priv->vreg_ipa ||
		...
		ret = cnss_aop_set_vreg_param(plat_priv, plat_priv->vreg_ipa,
					      CNSS_VREG_ENABLE, CNSS_TCS_UP_SEQ,
					      1);
		if (ret == 0)
			config_done = true;
		return ret;
	...
	ret = of_property_read_u32(plat_dev->dev.of_node,
				   "qcom,tcs_offset_int_pow_amp_vreg", &offset);
	if (ret) {
