...
int ipa_rm_resource_consumer_release_work(struct ipa_rm_resource_cons *consumer,
	...
	if (driver_result == 0) {
		if (notify_completion)
			ipa_rm_resource_consumer_handle_cb(
				consumer, IPA_RM_RESOURCE_RELEASED);
		else
					  ...
					  bool dec_client_on_err)
{
	...
	if (driver_result == 0) {
		if (notify_completion) {
			ipa_rm_resource_consumer_handle_cb(
				consumer, IPA_RM_RESOURCE_GRANTED);
		} else {
...
int ipa_rm_resource_consumer_request(struct ipa_rm_resource_cons *consumer,
				     u32 prod_needed_bw, bool inc_usage_count,
				     bool wake_client)
{
	int result = 0;
	enum ipa_rm_resource_state prev_state;
	...
	case IPA_RM_RELEASE_IN_PROGRESS:
		...
		IPA_ACTIVE_CLIENTS_PREP_RESOURCE(
			log_info, ipa_rm_resource_str(consumer->resource.name));
...
int ipa_rm_resource_consumer_release(struct ipa_rm_resource_cons *consumer,
				     u32 prod_needed_bw, bool dec_usage_count)
{
	int result = 0;
		...
		if (consumer->usage_count == 0) {
			...
			if (save_state == IPA_RM_REQUEST_IN_PROGRESS ||
				...
				ipa_rm_wq_send_suspend_cmd(
					consumer->resource.name, save_state,
					prod_needed_bw);
				result = -EINPROGRESS;
				goto bail;
			}
			result = ipa_rm_resource_consumer_release_work(
				consumer, save_state, false);
			goto bail;
...
void ipa_rm_resource_producer_notify_clients(
	struct ipa_rm_resource_prod *producer, enum ipa_rm_event event,
...
static int
ipa_rm_resource_producer_create(struct ipa_rm_resource **resource,
				...
				int *max_peers)
{
	...
	*producer = kzalloc(sizeof(**producer), GFP_ATOMIC);
	...
	result = ipa_rm_resource_producer_register(
		*producer, &(create_params->reg_params), false);
	if (result) {
...
static void
ipa_rm_resource_producer_delete(struct ipa_rm_resource_prod *producer)
{
	struct ipa_rm_notification_info *reg_info;
	...
	list_for_each_safe(pos, q, &(producer->event_listeners)) {
		reg_info =
			list_entry(pos, struct ipa_rm_notification_info, link);
		list_del(pos);
		kfree(reg_info);
	}
}
...
static int
ipa_rm_resource_consumer_create(struct ipa_rm_resource **resource,
 ...
 */
int ipa_rm_resource_create(struct ipa_rm_create_params *create_params,
			   struct ipa_rm_resource **resource)
{
	...
	if (IPA_RM_RESORCE_IS_PROD(create_params->name)) {
		result = ipa_rm_resource_producer_create(
			resource, &producer, create_params, &max_peers);
		if (result) {
			IPA_RM_ERR("ipa_rm_resource_producer_create failed\n");
			goto bail;
		}
	} else if (IPA_RM_RESORCE_IS_CONS(create_params->name)) {
		result = ipa_rm_resource_consumer_create(
			resource, &consumer, create_params, &max_peers);
		if (result) {
			IPA_RM_ERR("ipa_rm_resource_producer_create failed\n");
			goto bail;
		}
	...
	result =
		ipa_rm_peers_list_create(max_peers, &((*resource)->peers_list));
	if (result) {
	...
	if (resource->type == IPA_RM_PRODUCER) {
		if (resource->peers_list) {
			list_size = ipa_rm_peers_list_get_size(
				resource->peers_list);
			for (peers_index = 0; peers_index < list_size;
			     peers_index++) {
				consumer = ipa_rm_peers_list_get_resource(
					peers_index, resource->peers_list);
				if (consumer) {
					userspace_dep =
				      ...
				      bool explicit)
{
	...
	list_for_each(pos, &(producer->event_listeners)) {
		reg_info =
			list_entry(pos, struct ipa_rm_notification_info, link);
		if (reg_info->reg_params.notify_cb == reg_params->notify_cb) {
 ...
 *
 */
int ipa_rm_resource_producer_deregister(
	struct ipa_rm_resource_prod *producer,
	struct ipa_rm_register_params *reg_params)
{
	int result = -EINVAL;
				   ...
				   bool userspace_dep)
{
	...
	if (ipa_rm_peers_list_check_dependency(
		    resource->peers_list, resource->name,
		    depends_on->peers_list, depends_on->name,
				      ...
				      bool userspace_dep)
{
   ...
   */
	...
	case IPA_RM_GRANTED:
		...
		break;
	case IPA_RM_RELEASE_IN_PROGRESS:
		if (((struct ipa_rm_resource_prod *)resource)->pending_release >
		    0)
			((struct ipa_rm_resource_prod *)resource)
				->pending_release--;
		if (depends_on->state == IPA_RM_RELEASE_IN_PROGRESS &&
		    ((struct ipa_rm_resource_prod *)resource)->pending_release ==
			    0) {
			resource->state = IPA_RM_RELEASED;
			state_changed = true;
	...
	case IPA_RM_REQUEST_IN_PROGRESS:
		release_consumer = true;
		if (((struct ipa_rm_resource_prod *)resource)->pending_request >
		    0)
			((struct ipa_rm_resource_prod *)resource)
				->pending_request--;
	...
	if (state_changed) {
		(void)ipa_rm_wq_send_cmd(IPA_RM_WQ_NOTIFY_PROD, resource->name,
					 evt, false);
	}
	IPA_RM_DBG("%s new state: %d\n", ipa_rm_resource_str(resource->name),
		   resource->state);
	ipa_rm_peers_list_remove_peer(resource->peers_list, depends_on->name);
	ipa_rm_peers_list_remove_peer(depends_on->peers_list, resource->name);
	if (release_consumer)
		(void)ipa_rm_resource_consumer_release(
			(struct ipa_rm_resource_cons *)depends_on,
			resource->max_bw, true);
bail:
	IPA_RM_DBG("EXIT with %d\n", result);
...
int ipa_rm_resource_producer_request(struct ipa_rm_resource_prod *producer)
{
	...
	for (peers_index = 0;
	     peers_index <
	     ipa_rm_peers_list_get_size(producer->resource.peers_list);
	     peers_index++) {
		consumer = ipa_rm_peers_list_get_resource(
			peers_index, producer->resource.peers_list);
		if (consumer) {
			producer->pending_request++;
			consumer_result = ipa_rm_resource_consumer_request(
				(struct ipa_rm_resource_cons *)consumer,
				producer->resource.max_bw, true, false);
			if (consumer_result == -EINPROGRESS) {
				result = -EINPROGRESS;
			} else {
				producer->pending_request--;
...
static void
ipa_rm_resource_producer_handle_cb(struct ipa_rm_resource_prod *producer,
				   enum ipa_rm_event event)
{
	IPA_RM_DBG_LOW("%s state: %d event: %d pending_request: %d\n",
		       ipa_rm_resource_str(producer->resource.name),
		       producer->resource.state, event,
		       producer->pending_request);
		...
		if (producer->pending_request > 0) {
			...
			if (producer->pending_request == 0) {
				...
				ipa_rm_resource_producer_notify_clients(
					producer, IPA_RM_RESOURCE_GRANTED,
					false);
				goto bail;
			}
		}
		...
		if (producer->pending_release > 0) {
			...
			if (producer->pending_release == 0) {
				...
				ipa_rm_perf_profile_change(
					producer->resource.name);
				ipa_rm_resource_producer_notify_clients(
					producer, IPA_RM_RESOURCE_RELEASED,
					false);
				goto bail;
			}
		}
					...
					enum ipa_rm_event event)
{
	...
	for (peers_index = 0;
	     peers_index <
	     ipa_rm_peers_list_get_size(consumer->resource.peers_list);
	     peers_index++) {
		producer = ipa_rm_peers_list_get_resource(
			peers_index, consumer->resource.peers_list);
		if (producer)
			ipa_rm_resource_producer_handle_cb(
				(struct ipa_rm_resource_prod *)producer, event);
	}
				     ...
				     struct ipa_rm_perf_profile *profile)
{
	...
	if ((resource->type == IPA_RM_PRODUCER &&
		...
		for (peers_index = 0;
		     peers_index <
		     ipa_rm_peers_list_get_size(resource->peers_list);
		     peers_index++) {
			peer = ipa_rm_peers_list_get_resource(
				peers_index, resource->peers_list);
			if (!peer)
				continue;
 ...
 */
int ipa_rm_resource_producer_print_stat(struct ipa_rm_resource *resource,
					char *buf, int size)
{
	int i;
	int nbytes;
	...
	switch (resource->state) {
	case IPA_RM_RELEASED:
		nbytes = scnprintf(buf + cnt, size - cnt, "Released] -> ");
		cnt += nbytes;
		break;
	case IPA_RM_REQUEST_IN_PROGRESS:
		nbytes = scnprintf(buf + cnt, size - cnt,
		...
		break;
	case IPA_RM_GRANTED:
		nbytes = scnprintf(buf + cnt, size - cnt, "Granted] -> ");
		cnt += nbytes;
		break;
	case IPA_RM_RELEASE_IN_PROGRESS:
		nbytes = scnprintf(buf + cnt, size - cnt,
	...
	for (i = 0; i < resource->peers_list->max_peers; ++i) {
		consumer =
			ipa_rm_peers_list_get_resource(i, resource->peers_list);
		if (consumer) {
			nbytes = scnprintf(buf + cnt, size - cnt,
