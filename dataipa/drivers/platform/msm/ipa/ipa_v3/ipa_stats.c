...
static int ipa_get_generic_stats(unsigned long arg)
{
	...
	alloc_size =
		sizeof(struct ipa_lnx_generic_stats) +
	...
	generic_stats->excep_stats.excptn_type_pkt_len =
		ipa3_ctx->stats
			.rx_excp_pkts[IPAHAL_PKT_STATUS_EXCEPTION_PACKET_LENGTH];
	...
	generic_stats->excep_stats.excptn_type_frag_rule_miss =
		ipa3_ctx->stats
			.rx_excp_pkts[IPAHAL_PKT_STATUS_EXCEPTION_FRAG_RULE_MISS];
	generic_stats->excep_stats.excptn_type_sw_flt =
		ipa3_ctx->stats
			.rx_excp_pkts[IPAHAL_PKT_STATUS_EXCEPTION_SW_FILT];
	...
	/* ODL EP stats */
	if (ipa3_odl_ctx) {
		generic_stats->odl_stats.rx_pkt =
	...
	if (ipa_lnx_agent_ctx.alloc_info.num_holb_drop_stats_clients != 0) {
		...
		for (i = 0; i < IPA_CLIENT_MAX; i++) {
			...
			if (!(ipa3_ctx->hw_stats &&
			      (ipa3_ctx->hw_stats->drop.init
				       .enabled_bitmask[reg_idx] &
			       ipahal_get_ep_bit(ep_idx))))
				continue;

			holb_disc_stats_ptr->client_type = i;
			holb_disc_stats_ptr->num_drp_cnt =
				out->client[i].drop_packet_cnt;
			holb_disc_stats_ptr->num_drp_bytes =
				out->client[i].drop_byte_cnt;
			holb_disc_stats_ptr =
				(struct holb_discard_stats
					 *)((uint64_t)holb_disc_stats_ptr +
					    sizeof(struct holb_discard_stats));
		}
	}
	...
	if (ipa_lnx_agent_ctx.alloc_info.num_holb_mon_stats_clients != 0) {
		...
		holb_mon_stats_ptr =
			(struct holb_monitor_stats
				 *)((uint64_t)&generic_stats->holb_stats
					    .holb_disc_stats[0] +
				    (ipa_lnx_agent_ctx.alloc_info
					     .num_holb_drop_stats_clients *
				     sizeof(struct holb_discard_stats)));
		for (i = 0; i < generic_stats->holb_stats.num_holb_mon_clients;
		     i++) {
			holb_client =
				&(ipa3_ctx->uc_ctx.holb_monitor.client[i]);
			/* Get the client type from gsi_hdl */
			for (j = 0; j < IPA5_MAX_NUM_PIPES; j++) {
				if (ipa3_ctx->ep[j].gsi_chan_hdl ==
				    holb_client->gsi_chan_hdl) {
					holb_mon_stats_ptr->client_type =
						ipa3_ctx->ep[j].client;
					break;
				}
			}
			holb_mon_stats_ptr->curr_index =
				holb_client->current_idx;
			holb_mon_stats_ptr->num_en_cnt =
				holb_client->enable_cnt;
			holb_mon_stats_ptr->num_dis_cnt =
				holb_client->disable_cnt;
			holb_mon_stats_ptr =
				(struct holb_monitor_stats
					 *)((uint64_t)holb_mon_stats_ptr +
					    sizeof(struct holb_monitor_stats));
		}
	}

	if (copy_to_user((void __user *)arg, (u8 *)generic_stats, alloc_size)) {
		kfree(generic_stats);
		kfree(out);
		...
		if (ipa_get_pm_client_stats_filled(pm_stats_ptr, i)) {
			...
			pm_stats_ptr =
				(struct pm_client_stats
					 *)((uint64_t)pm_stats_ptr +
					    sizeof(struct pm_client_stats));
		}
	}

	if (copy_to_user((void __user *)arg, (u8 *)clock_stats, alloc_size)) {
		kfree(clock_stats);
		IPA_STATS_ERR("copy to user failed");
		return -EFAULT;
	}
 ...
 */
static void ipa_get_gsi_pipe_info(struct ipa_lnx_pipe_info *pipe_info_ptr_local,
				  struct ipa3_ep_context *ep)
{
	...
	pipe_info_ptr_local->gsi_chan_stop_stm = gsi_get_chan_stop_stm(
		ep->gsi_chan_hdl, gsi_get_peripheral_ee());
	...
	pipe_info_ptr_local->gsi_chan_ring_rp = gsi_read_chan_ring_rp(
		ep->gsi_chan_hdl, gsi_get_peripheral_ee());
	pipe_info_ptr_local->gsi_chan_ring_wp = gsi_read_chan_ring_wp(
		ep->gsi_chan_hdl, gsi_get_peripheral_ee());
	...
	pipe_info_ptr_local->gsi_evt_ring_wp = gsi_read_event_ring_wp(
		ep->gsi_evt_ring_hdl, gsi_get_peripheral_ee());
}
 ...
 * filled(rx). 100 value being efficient and 0 being non efficient/stall/IPA
 * idle
 */
	...
	int client_type)
{
	...
	if (IPA_CLIENT_IS_CONS(client_type) && tx_instance_ptr_local) {
		if (tx_instance_ptr_local->num_tx_ring_100_perc_with_cred >=
		    poll_pack_and_cred_info[client_type]
			    .tx_cred_info.num_tx_ring_100_perc_with_cred)
			diff_100_perc_cred =
				tx_instance_ptr_local
					->num_tx_ring_100_perc_with_cred -
				poll_pack_and_cred_info[client_type]
					.tx_cred_info
					.num_tx_ring_100_perc_with_cred;
		else
		...
		if (tx_instance_ptr_local->num_tx_ring_above_75_perc_cred >=
		    poll_pack_and_cred_info[client_type]
			    .tx_cred_info.num_tx_ring_above_75_perc_cred)
			diff_75_perc_cred =
				tx_instance_ptr_local
					->num_tx_ring_above_75_perc_cred -
				poll_pack_and_cred_info[client_type]
					.tx_cred_info
					.num_tx_ring_above_75_perc_cred;
		else
		...
		if (tx_instance_ptr_local->num_tx_ring_above_25_perc_cred >=
		    poll_pack_and_cred_info[client_type]
			    .tx_cred_info.num_tx_ring_above_25_perc_cred)
			diff_25_perc_cred =
				tx_instance_ptr_local
		...
		if (tx_instance_ptr_local->num_tx_ring_0_perc_with_cred >=
		    poll_pack_and_cred_info[client_type]
			    .tx_cred_info.num_tx_ring_0_perc_with_cred)
			diff_0_perc_cred =
				tx_instance_ptr_local
					->num_tx_ring_0_perc_with_cred -
				poll_pack_and_cred_info[client_type]
					.tx_cred_info
					.num_tx_ring_0_perc_with_cred;
		else
		...
		if (tx_instance_ptr_local->num_tx_ring_stats_polled >=
		    poll_pack_and_cred_info[client_type]
			    .tx_cred_info.num_tx_ring_stats_polled)
			diff_tx_polled =
		...
		diff_50_perc_cred = diff_tx_polled -
				    (diff_100_perc_cred + diff_75_perc_cred +
				     diff_25_perc_cred + diff_0_perc_cred);
		/**
     ...
     */
		tx_instance_ptr_local->tx_summary =
			((diff_100_perc_cred * 100) + (diff_75_perc_cred * 75) +
			 (diff_50_perc_cred * 50) + (diff_25_perc_cred * 25) +
			 (diff_0_perc_cred * 0)) /
			diff_tx_polled;
	} else if (!IPA_CLIENT_IS_CONS(client_type) && rx_instance_ptr_local) {
		if (rx_instance_ptr_local->num_rx_ring_100_perc_with_pack >=
		    poll_pack_and_cred_info[client_type]
			    .rx_pack_info.num_rx_ring_100_perc_with_pack)
			diff_100_perc_pack =
				rx_instance_ptr_local
					->num_rx_ring_100_perc_with_pack -
				poll_pack_and_cred_info[client_type]
					.rx_pack_info
					.num_rx_ring_100_perc_with_pack;
		else
		...
		if (rx_instance_ptr_local->num_rx_ring_above_75_perc_pack >=
		    poll_pack_and_cred_info[client_type]
			    .rx_pack_info.num_rx_ring_above_75_perc_pack)
			diff_75_perc_pack =
				rx_instance_ptr_local
					->num_rx_ring_above_75_perc_pack -
				poll_pack_and_cred_info[client_type]
					.rx_pack_info
					.num_rx_ring_above_75_perc_pack;
		else
		...
		if (rx_instance_ptr_local->num_rx_ring_above_25_perc_pack >=
		    poll_pack_and_cred_info[client_type]
			    .rx_pack_info.num_rx_ring_above_25_perc_pack)
			diff_25_perc_pack =
				rx_instance_ptr_local
					->num_rx_ring_above_25_perc_pack -
				poll_pack_and_cred_info[client_type]
					.rx_pack_info
					.num_rx_ring_above_25_perc_pack;
		else
		...
		if (rx_instance_ptr_local->num_rx_ring_0_perc_with_pack >=
		    poll_pack_and_cred_info[client_type]
			    .rx_pack_info.num_rx_ring_0_perc_with_pack)
			diff_0_perc_pack =
				rx_instance_ptr_local
					->num_rx_ring_0_perc_with_pack -
				poll_pack_and_cred_info[client_type]
					.rx_pack_info
					.num_rx_ring_0_perc_with_pack;
		else
		...
		if (rx_instance_ptr_local->num_rx_ring_stats_polled >=
		    poll_pack_and_cred_info[client_type]
			    .rx_pack_info.num_rx_ring_stats_polled)
			diff_rx_polled =
		...
		diff_50_perc_pack = diff_rx_polled -
				    (diff_100_perc_pack + diff_75_perc_pack +
				     diff_25_perc_pack + diff_0_perc_pack);
		/**
     ...
     */
		rx_instance_ptr_local->rx_summary =
			((diff_100_perc_pack * 0) + (diff_75_perc_pack * 25) +
			 (diff_50_perc_pack * 50) + (diff_25_perc_pack * 75) +
			 (diff_0_perc_pack * 100)) /
			diff_rx_polled;
	}
}
...
static int ipa_get_wlan_inst_stats(unsigned long arg)
{
	...
	for (i = 0; i < ipa_lnx_agent_ctx.alloc_info.num_wlan_instances; i++) {
		alloc_size = alloc_size +
			     (ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[i]
				      .num_tx_instances *
	...
	for (i = 0; i < wlan_stats->num_wlan_instance; i++) {
		...
		instance_ptr->pm_bandwidth = ipa_pm_get_pm_clnt_throughput(
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[i]
				.tx_inst_client_type[0]);

		tx_instance_ptr =
			(struct ipa_lnx_gsi_tx_debug_stats
				 *)((uint64_t)instance_ptr +
				    sizeof(struct wlan_instance_info));
		for (j = 0; j < ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[i]
					.num_tx_instances;
		     j++) {
			tx_instance_ptr_local =
				(struct ipa_lnx_gsi_tx_debug_stats
					 *)((uint64_t)tx_instance_ptr +
					    (j *
					     sizeof(struct ipa_lnx_gsi_tx_debug_stats)));

			client_type =
				ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[i]
					.tx_inst_client_type[j];
			...
			ipa_lnx_calculate_gsi_ring_summay(tx_instance_ptr_local,
							  NULL, client_type);
		...
		for (j = 0; j < instance_ptr->num_pipes; j++) {
			...
			ipa_get_gsi_pipe_info(pipe_info_ptr_local,
					      &ipa3_ctx->ep[ep_idx]);
		}
...
success:
	if (copy_to_user((void __user *)arg, (u8 *)wlan_stats, alloc_size)) {
		IPA_STATS_ERR("copy to user failed");
		kfree(wlan_stats);
		return -EFAULT;
	}
...
static int ipa_get_eth_inst_stats(unsigned long arg)
{
	...
	for (i = 0; i < ipa_lnx_agent_ctx.alloc_info.num_eth_instances; i++) {
		alloc_size = alloc_size +
			     (ipa_lnx_agent_ctx.alloc_info.eth_inst_info[i]
				      .num_tx_instances *
			      sizeof(struct ipa_lnx_gsi_tx_debug_stats)) +
			     (ipa_lnx_agent_ctx.alloc_info.eth_inst_info[i]
				      .num_rx_instances *
	...
	for (i = 0; i < eth_stats->num_eth_instance; i++) {
		...
		tx_instance_ptr = (struct ipa_lnx_gsi_tx_debug_stats
					   *)((uint64_t)instance_ptr +
					      sizeof(struct eth_instance_info));
		for (j = 0; j < ipa_lnx_agent_ctx.alloc_info.eth_inst_info[i]
					.num_tx_instances;
		     j++) {
			tx_instance_ptr_local =
				(struct ipa_lnx_gsi_tx_debug_stats
...
#if IPA_ETH_API_VER >= 2
				/* Get the client pipe info[0] from the allocation info context only if
         * it is NTN3 */
				if ((instance_ptr->eth_mode ==
				     IPA_ETH_CLIENT_NTN3)) {
					tx_instance_ptr_local->tx_client =
...
#if IPA_ETH_API_VER >= 2
				/* Get the client pipe info[1] from the allocation info context only if
         * it is NTN3 */
				if ((instance_ptr->eth_mode ==
				     IPA_ETH_CLIENT_NTN3)) {
					rx_instance_ptr_local->rx_client =
...
success:
	if (copy_to_user((void __user *)arg, (u8 *)eth_stats, alloc_size)) {
		IPA_STATS_ERR("copy to user failed");
		kfree(eth_stats);
		return -EFAULT;
	}
...
static int ipa_get_usb_inst_stats(unsigned long arg)
{
	...
	for (i = 0; i < ipa_lnx_agent_ctx.alloc_info.num_usb_instances; i++) {
		alloc_size = alloc_size +
			     (ipa_lnx_agent_ctx.alloc_info.usb_inst_info[i]
				      .num_tx_instances *
			      sizeof(struct ipa_lnx_gsi_tx_debug_stats)) +
			     (ipa_lnx_agent_ctx.alloc_info.usb_inst_info[i]
				      .num_rx_instances *
	...
	for (i = 0; i < usb_stats->num_usb_instance; i++) {
		...
		tx_instance_ptr = (struct ipa_lnx_gsi_tx_debug_stats
					   *)((uint64_t)instance_ptr +
					      sizeof(struct usb_instance_info));
		for (j = 0; j < ipa_lnx_agent_ctx.alloc_info.usb_inst_info[i]
					.num_tx_instances;
		     j++) {
			tx_instance_ptr_local =
				(struct ipa_lnx_gsi_tx_debug_stats
		...
		instance_ptr = (struct usb_instance_info
					*)((uint64_t)pipe_info_ptr +
					   (sizeof(struct ipa_lnx_pipe_info) *
					    (instance_ptr->num_pipes)));
	}
...
success:
	if (copy_to_user((void __user *)arg, (u8 *)usb_stats, alloc_size)) {
		IPA_STATS_ERR("copy to user failed");
		kfree(usb_stats);
		return -EFAULT;
	}
...
static int ipa_get_mhip_inst_stats(unsigned long arg)
{
	...
	for (i = 0; i < ipa_lnx_agent_ctx.alloc_info.num_mhip_instances; i++) {
		alloc_size = alloc_size +
			     (ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[i]
				      .num_tx_instances *
			      sizeof(struct ipa_lnx_gsi_tx_debug_stats)) +
			     (ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[i]
				      .num_rx_instances *
	...
	for (i = 0; i < mhip_stats->num_mhip_instance; i++) {
		instance_ptr->instance_id = i;
		instance_ptr->mhip_mode = ipa_lnx_agent_ctx.usb_teth_prot[i];
		instance_ptr->pm_bandwidth = ipa_pm_get_pm_clnt_throughput(
			ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[i]
				.tx_inst_client_type[0]);
		instance_ptr->num_pipes =
		...
		tx_instance_ptr =
			(struct ipa_lnx_gsi_tx_debug_stats
				 *)((uint64_t)instance_ptr +
				    sizeof(struct mhip_instance_info));
		for (j = 0; j < ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[i]
					.num_tx_instances;
		     j++) {
			tx_instance_ptr_local =
				(struct ipa_lnx_gsi_tx_debug_stats
...
success:
	if (copy_to_user((void __user *)arg, (u8 *)mhip_stats, alloc_size)) {
		IPA_STATS_ERR("copy to user failed");
		kfree(mhip_stats);
		return -EFAULT;
	}
...
static int ipa_get_page_recycle_stats(unsigned long arg)
{
	...
	page_recycle_stats =
		(struct ipa_lnx_pipe_page_recycling_stats *)memdup_user(
			(const void __user *)arg, alloc_size);
	...
	if (copy_to_user((void __user *)arg, (u8 *)page_recycle_stats,
			 alloc_size)) {
		IPA_STATS_ERR("copy to user failed");
...
static int ipa_stats_get_alloc_info(unsigned long arg)
{
	...
	if (ipa_lnx_agent_ctx.log_type_mask & TLPD_IPA_LOG_TYPE_WLAN_STATS) {
		...
		if ((ipa_ep_idx_tx == -1) || (ipa_ep_idx_rx == -1) ||
			...
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.num_pipes = 2;
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.num_tx_instances = 1;
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.num_rx_instances = 1;
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.pipes_client_type[0] = IPA_CLIENT_WLAN2_CONS;
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.pipes_client_type[1] = IPA_CLIENT_WLAN2_PROD;
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.tx_inst_client_type[0] = IPA_CLIENT_WLAN2_CONS;
			ipa_lnx_agent_ctx.alloc_info.wlan_inst_info[0]
				.rx_inst_client_type[0] = IPA_CLIENT_WLAN2_PROD;
	...
	if (ipa_lnx_agent_ctx.log_type_mask & TLPD_IPA_LOG_TYPE_MHIP_STATS) {
		...
		if (!ipa3_ctx->mhip_ctx.dbg_stats.uc_dbg_stats_mmio) {
			...
			if (ipa_usb_is_teth_prot_connected(IPA_USB_RNDIS))
				ipa_lnx_agent_ctx.usb_teth_prot[0] =
					IPA_USB_RNDIS;
			...
			ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[0]
				.num_pipes = 4;
			ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[0]
				.num_tx_instances = 2;
			ipa_lnx_agent_ctx.alloc_info.mhip_inst_info[0]
	...
	if (copy_to_user((u8 *)arg, &ipa_lnx_agent_ctx,
			 sizeof(struct ipa_lnx_stats_tlpd_ctx))) {
		IPA_STATS_ERR("copy to user failed");
		return -EFAULT;
	}
...
static long ipa_lnx_stats_ioctl(struct file *filp, unsigned int cmd,
				unsigned long arg)
{
	...
	case IPA_LNX_IOC_GET_CONSOLIDATED_STATS:
		consolidated_stats =
			(struct ipa_lnx_consolidated_stats *)memdup_user(
				(const void __user *)arg,
				sizeof(struct ipa_lnx_consolidated_stats));
		...
		if (consolidated_stats->log_type_mask &
		    TLPD_IPA_LOG_TYPE_GENERIC_STATS) {
			retval = ipa_get_generic_stats(
				(unsigned long)
					consolidated_stats->generic_stats);
			if (retval) {
				IPA_STATS_ERR("ipa get generic stats fail");
				break;
			}
		...
		if (consolidated_stats->log_type_mask &
		    TLPD_IPA_LOG_TYPE_CLOCK_STATS) {
			retval = ipa_get_clock_stats(
		...
		if (consolidated_stats->log_type_mask &
		    TLPD_IPA_LOG_TYPE_WLAN_STATS) {
			retval = ipa_get_wlan_inst_stats(
				(unsigned long)consolidated_stats->wlan_stats);
			if (retval) {
				IPA_STATS_ERR("ipa get wlan inst stats fail");
				break;
			}
		...
		if (consolidated_stats->log_type_mask &
		    TLPD_IPA_LOG_TYPE_ETH_STATS) {
			retval = ipa_get_eth_inst_stats(
				(unsigned long)consolidated_stats->eth_stats);
			if (retval) {
				IPA_STATS_ERR("ipa get eth inst stats fail");
				break;
			}
		...
		if (consolidated_stats->log_type_mask &
			...
			retval = ipa_get_usb_inst_stats(
				(unsigned long)consolidated_stats->usb_stats);
			if (retval) {
				IPA_STATS_ERR("ipa get usb inst stats fail");
				break;
			}
		...
		if (consolidated_stats->log_type_mask &
		    TLPD_IPA_LOG_TYPE_MHIP_STATS) {
#if IS_ENABLED(CONFIG_IPA3_MHI_PRIME_MANAGER)
			retval = ipa_get_mhip_inst_stats(
				(unsigned long)consolidated_stats->mhip_stats);
			if (retval) {
				IPA_STATS_ERR("ipa get mhip inst stats fail");
				break;
			}
		...
		if (consolidated_stats->log_type_mask &
		    TLPD_IPA_LOG_TYPE_RECYCLE_STATS) {
			retval = ipa_get_page_recycle_stats(
				(unsigned long)
					consolidated_stats->recycle_stats);
			if (retval) {
				IPA_STATS_ERR(
					"ipa get page recycle stats fail\n");
				break;
			}
		}
		break;
...
int ipa3_update_usb_per_stats(enum ipa_per_stats_type_e stats_type,
			      uint32_t data)
{
	...
	if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MDM) {
		peripheral_stats->mdm.usb_enum_value =
			IPA_PER_USB_ENUM_TYPE_INVALID;
		peripheral_stats->mdm.usb_prot_enum_value =
		...
		peripheral_stats->msm.usb_enum_value =
			IPA_PER_USB_ENUM_TYPE_INVALID;
		peripheral_stats->msm.usb_prot_enum_value =
			IPA_PER_USB_PROT_TYPE_INVALID;
...
int ipa3_update_pcie_per_stats(enum ipa_per_stats_type_e stats_type,
			       uint32_t data)
{
	union ipa_peripheral_stats *peripheral_stats =
		(union ipa_peripheral_stats *)ipa3_ctx->per_stats_smem_va;
...
int ipa3_update_wifi_per_stats(enum ipa_per_stats_type_e stats_type,
			       uint32_t data)
{
	union ipa_peripheral_stats *peripheral_stats =
		(union ipa_peripheral_stats *)ipa3_ctx->per_stats_smem_va;
	if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MDM) {
		peripheral_stats->mdm.wifi_enum_type_val =
			IPA_PER_WIFI_ENUM_TYPE_INVALID;
		peripheral_stats->mdm.wifi_max_speed_val = 0;
		peripheral_stats->mdm.wifi_dual_band_enabled_val = 0;
	} else if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MSM) {
		peripheral_stats->msm.wifi_enum_type_val =
			IPA_PER_WIFI_ENUM_TYPE_INVALID;
...
int ipa3_update_eth_per_stats(enum ipa_per_stats_type_e stats_type,
			      uint32_t data)
{
	union ipa_peripheral_stats *peripheral_stats =
		(union ipa_peripheral_stats *)ipa3_ctx->per_stats_smem_va;
...
int ipa3_update_apps_per_stats(enum ipa_per_stats_type_e stats_type,
			       uint32_t data)
{
	union ipa_peripheral_stats *peripheral_stats =
		(union ipa_peripheral_stats *)ipa3_ctx->per_stats_smem_va;
	if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MDM) {
		...
		peripheral_stats->mdm.periph_type_val =
			IPA_PER_TYPE_BITMASK_NONE;
	} else if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MSM) {
		peripheral_stats->msm.periph_val = 0;
		peripheral_stats->msm.periph_wwan_val = 0;
		peripheral_stats->msm.periph_type_val =
			IPA_PER_TYPE_BITMASK_NONE;
	}
	return 0;
}
...
int ipa3_update_client_holb_per_stats(enum ipa_per_stats_type_e stats_type,
				      uint32_t data)
{
	union ipa_peripheral_stats *peripheral_stats =
		(union ipa_peripheral_stats *)ipa3_ctx->per_stats_smem_va;
	if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MDM) {
		...
		peripheral_stats->mdm.wifi_holb_uc_stats_num_periph_recovered =
			0;

		peripheral_stats->mdm.eth_holb_uc_stats_num_periph_bad = 0;
		peripheral_stats->mdm.eth_holb_uc_stats_num_periph_recovered =
			0;

		peripheral_stats->mdm.usb_holb_uc_stats_num_periph_bad = 0;
		peripheral_stats->mdm.usb_holb_uc_stats_num_periph_recovered =
			0;
	} else if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MSM) {
		peripheral_stats->msm.wifi_holb_uc_stats_num_periph_bad = 0;
		peripheral_stats->msm.wifi_holb_uc_stats_num_periph_recovered =
			0;

		peripheral_stats->msm.usb_holb_uc_stats_num_periph_bad = 0;
		peripheral_stats->msm.usb_holb_uc_stats_num_periph_recovered =
			0;
	}
	return 0;
}

int ipa3_update_dma_per_stats(enum ipa_per_stats_type_e stats_type,
			      uint32_t data)
{
	union ipa_peripheral_stats *peripheral_stats =
		(union ipa_peripheral_stats *)ipa3_ctx->per_stats_smem_va;
...
{
	if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MDM) {
		peripheral_stats->mdm.num_entries =
			IPA_PERIPHERAL_STATS_MDM_NUM_ENTRIES;
		...
		peripheral_stats->mdm.periph_wwan_id =
		...
		peripheral_stats->mdm.periph_type_id =
		...
		peripheral_stats->mdm.pcie_gen_type_id =
		...
		/* TLV for Current gen info if PCIe interconnect is valid */
		/* value = IPA_PER_STATS_TYPE_PCIE_GEN */
		peripheral_stats->mdm.pcie_width_type_id =
		...
		peripheral_stats->mdm.pcie_max_speed_id =
		...
		peripheral_stats->mdm.pcie_num_lpm_trans_id =
		...
		peripheral_stats->mdm.usb_prot_enum_id =
		...
		peripheral_stats->mdm.usb_max_speed_id =
		...
		peripheral_stats->mdm.wifi_enum_type_id =
			IPA_PER_STATS_TYPE_WIFI_ENUM_TYPE;
		peripheral_stats->mdm.wifi_enum_type_len = 4;
		peripheral_stats->mdm.wifi_enum_type_val =
			IPA_PER_WIFI_ENUM_TYPE_INVALID;

		/* TLV for Theoritical Max WLAN speed in current gen in Mbps (pipe for 5GHz
     * in case of dual band) */
		/* value = IPA_PER_STATS_TYPE_WIFI_MAX_SPEED */
		peripheral_stats->mdm.wifi_max_speed_id =
			IPA_PER_STATS_TYPE_WIFI_MAX_SPEED;
		peripheral_stats->mdm.wifi_max_speed_len = 4;
		peripheral_stats->mdm.wifi_max_speed_val = 0;

		/* TLV for Theoretical Max WLAN speed on the 2.4GHz pipe, value of 0 means
     * disabled */
		/* value = IPA_PER_STATS_TYPE_WIFI_DUAL_BAND_EN */
		peripheral_stats->mdm.wifi_dual_band_enabled_id =
		...
		peripheral_stats->mdm.eth_client_id =
		...
		peripheral_stats->mdm.eth_max_speed_id =
     ...
     */
		/* For cases where GSI used for QDSS direct DMA, need to extract bytes stat
     * from GSI FW */
		/* value = IPA_PER_STATS_TYPE_IPA_DMA_BYTES */
		peripheral_stats->mdm.ipa_dma_bytes_id =
		...
		peripheral_stats->mdm.wifi_holb_uc_stats_id =
		...
		peripheral_stats->mdm.eth_holb_uc_stats_id =
		...
		peripheral_stats->mdm.usb_holb_uc_stats_id =
		...
		peripheral_stats->msm.num_entries =
			IPA_PERIPHERAL_STATS_MSM_NUM_ENTRIES;
		...
		/* TLV for number of periphers from/to traffic flowing from modem */
		/* value = IPA_PER_STATS_TYPE_NUM_PERS_WWAN */
		peripheral_stats->msm.periph_wwan_id =
			IPA_PER_STATS_TYPE_NUM_PERS_WWAN;
		...
		/* TLV for bitmask for active/connected peripherals */
		/* value = IPA_PER_STATS_TYPE_ACT_PER_TYPE */
		peripheral_stats->msm.periph_type_id =
			IPA_PER_STATS_TYPE_ACT_PER_TYPE;
		...
		/* TLV for Current USB protocol enumeration if active */
		/* value = IPA_PER_STATS_TYPE_USB_PROT */
		peripheral_stats->msm.usb_prot_enum_id =
			IPA_PER_STATS_TYPE_USB_PROT;
		...
		/* TLV for Max USB speed in current gen in Mbps */
		/* value = IPA_PER_STATS_TYPE_USB_MAX_SPEED */
		peripheral_stats->msm.usb_max_speed_id =
			IPA_PER_STATS_TYPE_USB_MAX_SPEED;
		...
		/* TLV for Wifi enumeration type*/
		/* value = IPA_PER_STATS_TYPE_WIFI_ENUM_TYPE */
		peripheral_stats->msm.wifi_enum_type_id =
			IPA_PER_STATS_TYPE_WIFI_ENUM_TYPE;
     ...
     * in case of dual band) */
		/* value = IPA_PER_STATS_TYPE_WIFI_MAX_SPEED */
		peripheral_stats->msm.wifi_max_speed_id =
			IPA_PER_STATS_TYPE_WIFI_MAX_SPEED;
     ...
     * disabled */
		/* value = IPA_PER_STATS_TYPE_WIFI_DUAL_BAND_EN */
		peripheral_stats->msm.wifi_dual_band_enabled_id =
			IPA_PER_STATS_TYPE_WIFI_DUAL_BAND_EN;
		...
		/* TLV for number of wifi peripherals connected to APROC */
		/* value = IPA_PER_STATS_TYPE_WIFI_HOLB_UC */
		peripheral_stats->msm.wifi_holb_uc_stats_id =
			IPA_PER_STATS_TYPE_WIFI_HOLB_UC;
		...
		/* TLV for number of usb peripherals connected to APROC */
		/* value = IPA_PER_STATS_TYPE_USB_HOLB_UC */
		peripheral_stats->msm.usb_holb_uc_stats_id =
			IPA_PER_STATS_TYPE_USB_HOLB_UC;
