...
#define IPA_GENERIC_RX_BUFF_SZ(X) ((X) - (IPA_REAL_GENERIC_RX_BUFF_SZ(X) - (X)))
...
static struct ipa3_rx_pkt_wrapper *
ipa3_alloc_rx_pkt_page(gfp_t flag, bool is_tmp_alloc,
...
static int ipa_gsi_setup_coal_def_channel(struct ipa_sys_connect_params *in,
					  ...
					  struct ipa3_ep_context *coal_ep);
static int ipa_gsi_setup_channel(struct ipa_sys_connect_params *in,
				 struct ipa3_ep_context *ep);
static int ipa_gsi_setup_event_ring(struct ipa3_ep_context *ep, u32 ring_size,
				    gfp_t mem_flag);
static int ipa_gsi_setup_transfer_ring(struct ipa3_ep_context *ep,
				       u32 ring_size,
				       struct ipa3_sys_context *user_data,
				       gfp_t mem_flag);
...
static void
ipa3_collect_default_coal_recycle_stats_wq(struct work_struct *work);
static DECLARE_DELAYED_WORK(ipa3_collect_default_coal_recycle_stats_wq_work,
			    ipa3_collect_default_coal_recycle_stats_wq);

static void
ipa3_collect_low_lat_data_recycle_stats_wq(struct work_struct *work);
...
static void ipa3_collect_default_coal_recycle_stats_wq(struct work_struct *work)
{
	...
	ipa3_ctx->recycle_stats.interval_time_in_ms =
		IPA_LNX_PIPE_PAGE_RECYCLING_INTERVAL_TIME;

	/* Coalescing pipe page recycling stats */
	ipa3_ctx->recycle_stats
		.rx_channel[RX_WAN_COALESCING][stat_interval_index]
		.total_cumulative =
	...
	ipa3_ctx->recycle_stats.rx_channel[RX_WAN_DEFAULT][stat_interval_index]
		.total_cumulative =
	...
	ipa3_ctx->recycle_stats.rx_channel[RX_WAN_DEFAULT][stat_interval_index]
		.total_diff =
		ipa3_ctx->recycle_stats
			.rx_channel[RX_WAN_DEFAULT][stat_interval_index]
			.total_cumulative -
...
static void ipa3_collect_low_lat_data_recycle_stats_wq(struct work_struct *work)
{
	...
	ipa3_ctx->recycle_stats
		.rx_channel[RX_WAN_LOW_LAT_DATA][stat_interval_index]
		.total_cumulative =
	...
	ipa3_ctx->recycle_stats.low_lat_stats_index =
		(ipa3_ctx->recycle_stats.low_lat_stats_index + 1) %
		IPA_LNX_PIPE_PAGE_RECYCLING_INTERVAL_COUNT;

	if (sys && atomic_read(&sys->curr_polling_state))
		queue_delayed_work(
			ipa3_ctx->collect_recycle_stats_wq,
			&ipa3_collect_low_lat_data_recycle_stats_wq_work,
			msecs_to_jiffies(10));
...
{
	struct ipa3_sys_context *sys =
		container_of(napi_tx, struct ipa3_sys_context, napi_tx);
...
{
	struct ipa3_sys_context *sys =
		container_of(work, struct ipa3_sys_context, work);
...
int ipa3_send(struct ipa3_sys_context *sys, u32 num_desc,
	      struct ipa3_desc *desc, bool in_atomic)
{
	...
	for (i = 0; i < num_desc; i++) {
		if (!list_empty(&sys->avail_tx_wrapper_list)) {
			...
			tx_pkt = kmem_cache_zalloc(
				ipa3_ctx->tx_pkt_wrapper_cache, GFP_ATOMIC);
		}
		if (!tx_pkt) {
		...
		if (desc[i].type != IPA_DATA_DESC_SKB_PAGED) {
			...
			if (!desc[i].dma_address_valid) {
				tx_pkt->mem.phys_base = dma_map_single(
					ipa3_ctx->pdev, tx_pkt->mem.base,
					tx_pkt->mem.size, DMA_TO_DEVICE);
			} else {
			...
			if (!desc[i].dma_address_valid) {
				tx_pkt->mem.phys_base = skb_frag_dma_map(
					ipa3_ctx->pdev, desc[i].frag, 0,
					tx_pkt->mem.size, DMA_TO_DEVICE);
			} else {
				tx_pkt->mem.phys_base = desc[i].dma_address;
				tx_pkt->no_unmap_dma = true;
     ...
     */
		if (desc[i].type == IPA_IMM_CMD_DESC) {
			...
			gsi_xfer[i].type = GSI_XFER_ELEM_IMME_CMD;
		} else {
		...
		if (i == (num_desc - 1)) {
			if (ipa3_ctx->tx_poll || !sys->use_comm_evt_ring ||
			...
			gsi_xfer[i].xfer_user_data = tx_pkt_first;
		} else {
	...
	result =
		gsi_queue_xfer(sys->ep->gsi_chan_hdl, num_desc, gsi_xfer, true);
...
{
	struct ipa3_sys_context *sys =
		container_of(param, struct ipa3_sys_context, db_timer);
...
static void ipa3_tasklet_find_freepage(unsigned long data)
{
	...
	list_for_each_entry_safe(
		rx_pkt, tmp, &sys->page_recycle_repl->page_repl_head, link) {
...
int ipa_setup_sys_pipe(struct ipa_sys_connect_params *sys_in, u32 *clnt_hdl)
{
	...
	if (sys_in == NULL || clnt_hdl == NULL) {
		IPAERR("NULL args: sys_in(%p) and/or clnt_hdl(%u)\n", sys_in,
		       clnt_hdl);
		goto fail_gen;
	}
	...
	*clnt_hdl = 0;
	...
	if (!ep->sys) {
		...
		snprintf(buff, IPA_RESOURCE_NAME_MAX, "iparepwq%d",
			 sys_in->client);
		ep->sys->repl_wq = alloc_workqueue(
			buff,
	...
	if (IPA_CLIENT_IS_PROD(sys_in->client) && ipa3_ctx->tx_napi_enable) {
		...
		if (!(ipa3_ctx->wan_common_page_pool &&
			...
			if (!ep->sys->page_recycle_repl) {
				ep->sys->page_recycle_repl = kzalloc(
					sizeof(*ep->sys->page_recycle_repl),
					GFP_KERNEL);
				if (!ep->sys->page_recycle_repl) {
					IPAERR("failed to alloc repl for client %d\n",
					       sys_in->client);
				...
				atomic_set(&ep->sys->page_recycle_repl->pending,
					   0);
				...
				IPADBG("Page repl capacity for client:%d, value:%d\n",
				       sys_in->client,
				       ep->sys->page_recycle_repl->capacity);
				INIT_LIST_HEAD(&ep->sys->page_recycle_repl
							->page_repl_head);
				INIT_DELAYED_WORK(&ep->sys->freepage_work,
			...
			ep->sys->repl =
				kzalloc(sizeof(*ep->sys->repl), GFP_KERNEL);
			if (!ep->sys->repl) {
				IPAERR("failed to alloc repl for client %d\n",
				       sys_in->client);
				result = -ENOMEM;
			...
			ep->sys->page_recycle_repl =
				ipa3_ctx->ep[wan_coal_ep_id]
					.sys->page_recycle_repl;
		}
	}

	if (IPA_CLIENT_IS_CONS(sys_in->client)) {
		if ((IPA_CLIENT_IS_WAN_CONS(sys_in->client) ||
		     sys_in->client == IPA_CLIENT_APPS_WAN_LOW_LAT_DATA_CONS) &&
	...
	if (IPA_CLIENT_IS_APPS_COAL_CONS(sys_in->client)) {
		...
		if (sys_in->client == IPA_CLIENT_APPS_WAN_COAL_CONS) {
			...
			if (!sys_in->ext_ioctl_v2) {
				...
				result =
					ipa_setup_sys_pipe(sys_in, &wan_handle);
			}
			...
			if (!sys_in->ext_ioctl_v2) {
				...
				result =
					ipa_setup_sys_pipe(sys_in, &lan_handle);
			}
		}

		if (result) {
			IPAERR("Failed to setup default %s coalescing pipe\n",
			       str);
			goto fail_repl;
		}
...
fail_napi:
	...
	if (ipa3_ctx->tx_napi_enable && (IPA_CLIENT_IS_PROD(sys_in->client)))
		...
		if ((ipa3_ctx->ipa_hw_type >= IPA_HW_v5_0) &&
		    ((network_header->version == 4 &&
		     ...
		     (((struct ipv6hdr *)network_header)->version == 6 &&
		      ((struct ipv6hdr *)network_header)->nexthdr ==
			      NEXTHDR_ICMP))) {
			ipa_imm_cmd_modify_ip_packet_init_ex_dest_pipe(
				ipa3_ctx->pkt_init_ex_imm[ipa3_ctx->ipa_num_pipes]
					.base,
				dst_ep_idx);
       ...
       */
		...
		if (num_frags == 0) {
			...
			for (f = 0; f < num_frags; f++) {
				...
				desc[data_idx + f + 1].len = skb_frag_size(
					desc[data_idx + f + 1].frag);
			}
			/* don't free skb till frag mappings are released */
...
begin:
	while (1) {
		...
		rx_pkt =
			kmem_cache_zalloc(ipa3_ctx->rx_pkt_wrapper_cache, flag);
		if (!rx_pkt)
			goto fail_kmem_cache_alloc;
		...
		if (dma_mapping_error(ipa3_ctx->pdev, rx_pkt->data.dma_addr)) {
			pr_err_ratelimited(
				"%s dma map fail %pK for %pK sys=%pK\n",
				__func__, (void *)rx_pkt->data.dma_addr, ptr,
				sys);
...
fail_kmem_cache_alloc:
	if (atomic_read(&sys->repl->tail_idx) ==
		...
		if (IPA_CLIENT_IS_WAN_CONS(sys->ep->client))
			...
			IPA_STATS_INC_CNT(
				ipa3_ctx->stats.low_lat_repl_rx_empty);
		else if (sys->ep->client ==
			 IPA_CLIENT_APPS_WAN_LOW_LAT_DATA_CONS)
			IPA_STATS_INC_CNT(
		       ...
		       struct ipa3_sys_context *sys)
{
	...
	rx_pkt = kmem_cache_zalloc(ipa3_ctx->rx_pkt_wrapper_cache, flag);
	if (unlikely(!rx_pkt))
		return NULL;
	...
	rx_pkt->page_data.page = ipa3_alloc_page(
		flag, &rx_pkt->page_data.page_order,
	...
	if (dma_mapping_error(ipa3_ctx->pdev, rx_pkt->page_data.dma_addr)) {
		pr_err_ratelimited("%s dma map fail %pK for %pK\n", __func__,
				   (void *)rx_pkt->page_data.dma_addr,
...
static struct ipa3_rx_pkt_wrapper *
ipa3_get_free_page(struct ipa3_sys_context *sys, u32 stats_i)
{
	struct ipa3_rx_pkt_wrapper *rx_pkt = NULL;
	struct ipa3_rx_pkt_wrapper *tmp = NULL;
	struct page *cur_page;
	...
	spin_unlock_bh(&sys->common_sys->spinlock);
	IPADBG_LOW(
		"napi_sort_page_thrshld_cnt = %d ipa_max_napi_sort_page_thrshld = %d\n",
	...
	if (sys->common_sys->napi_sort_page_thrshld_cnt >=
		...
		if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
			...
			atomic_inc(
				&ipa3_ctx->stats.num_free_page_task_scheduled);
			if (stats_i == 2) {
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					FREE_PAGE_TASK_SCHEDULED_LL,
					&sys->common_sys
						 ->napi_sort_page_thrshld_cnt);
			} else {
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					FREE_PAGE_TASK_SCHEDULED,
					&sys->common_sys
						 ->napi_sort_page_thrshld_cnt);
			}
...
int ipa_register_notifier(void *fn_ptr)
{
	...
	if (fn_ptr == NULL)
		return -EFAULT;
	spin_lock(&ipa3_ctx->notifier_lock);
	ipa_notifier_block = (struct ipa_notifier_block_data *)kzalloc(
		sizeof(struct ipa_notifier_block_data), GFP_KERNEL);
	...
	list_add(&ipa_notifier_block->entry,
		 &ipa3_ctx->notifier_block_list_head);
...
int ipa_unregister_notifier(void *fn_ptr)
{
	...
	list_for_each_entry_safe(ipa_notifier_block, temp,
				 &ipa3_ctx->notifier_block_list_head, entry) {
		if (ipa_notifier_block->ipa_rmnet_notifier.notifier_call ==
		    fn_ptr) {
...
static void ipa3_replenish_rx_page_recycle(struct ipa3_sys_context *sys)
{
	...
	while (rx_len_cached < sys->rx_pool_sz) {
		...
		if (atomic_read(&sys->common_sys->page_avilable) &&
			...
			ipa3_ctx->stats.page_recycle_stats[stats_i]
				.page_recycled++;
       ...
       */
			...
			curr_wq = (++curr_wq == sys->repl->capacity) ? 0 :
								       curr_wq;
		}
		rx_pkt->sys = sys;

		trace_ipa3_replenish_rx_page_recycle(
			stats_i, rx_pkt->page_data.page,
	...
	ret = gsi_queue_xfer(sys->ep->gsi_chan_hdl, idx, gsi_xfer_elem_array,
			     true);
	if (ret == GSI_STATUS_SUCCESS) {
	...
	if (rx_len_cached <= IPA_DEFAULT_SYS_YELLOW_WM) {
		if (sys->ep->client == IPA_CLIENT_APPS_WAN_CONS) {
			...
			if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
				...
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					BUFF_BELOW_LOW_THRESHOLD_FOR_DEFAULT_PIPE,
					&rx_len_cached);
			...
			if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
				...
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					BUFF_BELOW_LOW_THRESHOLD_FOR_COAL_PIPE,
					&rx_len_cached);
			...
			if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
				...
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					BUFF_BELOW_LOW_THRESHOLD_FOR_LL_PIPE,
					&rx_len_cached);
	...
	if (rx_len_cached >= IPA_BUFF_THRESHOLD_HIGH) {
		if (sys->ep->client == IPA_CLIENT_APPS_WAN_CONS) {
			...
			if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
				...
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					BUFF_ABOVE_HIGH_THRESHOLD_FOR_DEFAULT_PIPE,
					&rx_len_cached);
				ipa3_ctx->buff_above_thresh_for_def_pipe_notified =
					true;
				ipa3_ctx->buff_below_thresh_for_def_pipe_notified =
			...
			if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
				...
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					BUFF_ABOVE_HIGH_THRESHOLD_FOR_COAL_PIPE,
					&rx_len_cached);
				ipa3_ctx->buff_above_thresh_for_coal_pipe_notified =
					true;
				ipa3_ctx->buff_below_thresh_for_coal_pipe_notified =
			...
			if (ipa3_ctx->ipa_rmnet_notifier_enabled &&
				...
				raw_notifier_call_chain(
					ipa3_ctx->ipa_rmnet_notifier_list_internal,
					BUFF_ABOVE_HIGH_THRESHOLD_FOR_LL_PIPE,
					&rx_len_cached);
				ipa3_ctx->buff_above_thresh_for_ll_pipe_notified =
					true;
				ipa3_ctx->buff_below_thresh_for_ll_pipe_notified =
...
static void ipa3_alloc_wlan_rx_common_cache(u32 size)
{
	...
	while (rx_len_cached < size) {
		...
		rx_pkt->data.skb =
			ipa3_get_skb_ipa_rx(IPA_WLAN_RX_BUFF_SZ, flag);
		if (rx_pkt->data.skb == NULL) {
...
/**
 * ipa3_first_replenish_rx_cache() - Replenish the Rx packets cache for the
 * first time.
 *
...
static void ipa3_replenish_rx_cache_recycle(struct ipa3_sys_context *sys)
{
	...
	struct gsi_xfer_elem gsi_xfer_elem_array[IPA_REPL_XFER_MAX];
	gfp_t flag = GFP_NOWAIT | __GFP_NOWARN;
	u32 stats_i = (sys->ep->client == IPA_CLIENT_APPS_LAN_COAL_CONS) ? 0 :
		      (sys->ep->client == IPA_CLIENT_APPS_LAN_CONS)	 ? 1 :
									   2;
	...
	while (rx_len_cached < sys->rx_pool_sz) {
		if (list_empty(&sys->rcycl_list)) {
			...
			ipa3_ctx->stats.cache_recycle_stats[stats_i]
				.pkt_allocd++;
		} else {
			spin_lock_bh(&sys->spinlock);
...
{
	struct ipa3_rx_pkt_wrapper *rx_pkt =
		(struct ipa3_rx_pkt_wrapper *)xfer_user_data;
	struct ipa3_sys_context *sys =
		(struct ipa3_sys_context *)chan_user_data;

	dma_unmap_single(ipa3_ctx->pdev, rx_pkt->data.dma_addr, sys->rx_buff_sz,
			 DMA_FROM_DEVICE);
	sys->free_skb(rx_pkt->data.skb);
	kmem_cache_free(ipa3_ctx->rx_pkt_wrapper_cache, rx_pkt);
}
	...
	if (sys->len_rem <= skb->len) {
		if (sys->prev_skb) {
			...
			if (likely(skb2)) {
				...
				skb2->truesize =
					skb2->len + sizeof(struct sk_buff);
		...
		if (sys->prev_skb) {
			skb2 = ipa3_join_prev_skb(sys->prev_skb, skb, skb->len);
			sys->prev_skb = skb2;
		}
		sys->len_rem -= skb->len;
		skb_pull(skb, skb->len);
...
void ipa3_lan_rx_cb(void *priv, enum ipa_dp_evt_type evt, unsigned long data)
{
   ...
   */
	...
	*(u8 *)(rx_skb->cb + 4) = ucp;
	...
	if (likely((!atomic_read(&ep->disconnect_in_progress)) && ep->valid &&
		   ep->client_notify)) {
 ...
 */
static int _skip_ipv6_exthdr(u8 *hdr_ptr, int start, u8 *nexthdrp,
			     __be16 *fragp)
{
	u8 nexthdr = *nexthdrp;
...
static int _calc_partial_csum(struct sk_buff *skb, ipa_header_data_t *hdr_data,
			      u32 aggr_payload_size)
{
	...
	if (!skb || !hdr_data) {
		IPAERR("NULL args: skb(%p) and/or hdr_data(%p)\n", skb,
		       hdr_data);
		...
		if (ip_vers == 4) {
			...
			pseudo = ~csum_tcpudp_magic(iph->saddr, iph->daddr,
						    len_for_calc, ip_proto, 0);

		} else { /* ( ip_vers == 6 ) */

			struct ipv6hdr *iph = (struct ipv6hdr *)new_ip_hdr;

			pseudo = ~csum_ipv6_magic(&iph->saddr, &iph->daddr,
						  len_for_calc, ip_proto, 0);
		}
 ...
 */
static int _prep_and_send_skb(struct sk_buff *rx_skb,
			      struct ipa3_ep_context *ep, u32 metadata, u8 ucp,
			      ipa_header_data_t *hdr_data, ipa_pkt_data_t *pkts,
			      u32 num_pkts, u32 aggr_payload_size, u8 pkt_id,
			      bool recalc_cksum)
{
	struct ethhdr *eth_hdr;
	u32 eth_hdr_size;
	...
	if (client_notify) {
		...
		if (rx_skb) {
			...
			head_skb = alloc_skb(aggr_hdr_len + aggr_payload_size,
					     GFP_ATOMIC);
       ...
       */
			new_eth_hdr =
				skb_put_data(head_skb, eth_hdr, eth_hdr_size);
       ...
       */
			new_ip_hdr =
				skb_put_data(head_skb, ip_hdr, ip_hdr_size);
			...
			if (ip_vers == 4) {
				...
				ip4h->tot_len =
					htons(ip_hdr_size + proto_hdr_size +
					      aggr_payload_size);

			} else {
				struct ipv6hdr *ip6h = new_ip_hdr;

				ip6h->payload_len = htons(proto_hdr_size +
							  aggr_payload_size);
			}
       ...
       */
			new_proto_hdr = skb_put_data(head_skb, proto_hdr,
						     proto_hdr_size);
     ...
     */
		if (recalc_cksum) {
			_calc_partial_csum(head_skb, hdr_data,
					   aggr_payload_size);
		}
     ...
     */
		...
		if (ip_proto == IPPROTO_TCP) {
			shinfo->gso_type = (ip_vers == 4) ? SKB_GSO_TCPV4 :
							    SKB_GSO_TCPV6;
 ...
 */
void ipa3_lan_coal_rx_cb(void *priv, enum ipa_dp_evt_type evt,
			 unsigned long data)
{
	struct sk_buff *rx_skb = (struct sk_buff *)data;
   ...
   * packets as well...
   */
	for (i = tot_pkts = 0; i < MAX_COAL_PACKET_STATUS_INFO; ++i, ++cpsi) {
   ...
   */
     ...
     * hence...
     */
		hdr_size = _skip_ipv6_exthdr((u8 *)ip_hdr, sizeof(*ip6h),
					     &ip_proto, &frag_off);
     ...
     */
		if (hdr_size < 0 || frag_off) {
			IPAERR("_skip_ipv6_exthdr() failed. Errored with hdr_size(%d) "
			       "and/or frag_off(%d)\n",
			       hdr_size, ntohs(frag_off));
   ...
   */
	...
	if (ip_proto == IPPROTO_TCP) {
		...
		cksum_is_zero = (ip_vers == 4 &&
				 ((struct udphdr *)proto_hdr)->check == 0);
   ...
   */
	skb_pull(rx_skb,
		 pkt_status_sz + sizeof(union qmap_hdr_u) + cpsi_hdrs_size);
   ...
   */
	if (gro && qmap_hdr.num_nlos == 1 && qmap_hdr.chksum_valid) {
		...
		in_pkts[0].pkt_len =
			cpsi->pkt_len - (ip_hdr_size + proto_hdr_size);

		in_pkts_sub = 1;

		aggr_payload_size = rx_skb->len - hdr_data.aggr_hdr_len;

		_prep_and_send_skb(rx_skb, ep, metadata, ucp, &hdr_data,
				   in_pkts, in_pkts_sub, aggr_payload_size,
				   tot_pkts, false);
   ...
   */
	...
	for (i = tot_pkts = 0, cpsi = cpsi_orig; i < qmap_hdr.num_nlos;
	     ++i, ++cpsi) {
		aggr_payload_size = in_pkts_sub = 0;

		for (j = 0; j < cpsi->num_pkts;
         ...
         */
				if (in_pkts_sub) {
					_prep_and_send_skb(NULL, ep, metadata,
							   ucp, &hdr_data,
							   in_pkts, in_pkts_sub,
							   aggr_payload_size,
							   tot_pkts,
         ...
         */
				...
				aggr_payload_size +=
					in_pkts[in_pkts_sub].pkt_len;
				pkt_data += in_pkts[in_pkts_sub].pkt_len;

				in_pkts_sub++;

				_prep_and_send_skb(
					NULL, ep, metadata, ucp, &hdr_data,
					in_pkts, in_pkts_sub, aggr_payload_size,
					tot_pkts,
		...
		if (in_pkts_sub) {
			_prep_and_send_skb(NULL, ep, metadata, ucp, &hdr_data,
					   in_pkts, in_pkts_sub,
					   aggr_payload_size, tot_pkts,
					   !cksum_is_zero);
		}
	}
...
handle_skb_completion(struct gsi_chan_xfer_notify *notify, bool update_truesize,
		      struct ipa3_rx_pkt_wrapper **rx_pkt_ptr)
{
	...
	if ((notify->evt_id == GSI_CHAN_EVT_EOB || sys->skip_eot) &&
 ...
 */
static struct sk_buff *
handle_page_completion(struct gsi_chan_xfer_notify *notify,
		       bool update_truesize)
{
	...
	if (notify->veid >= GSI_VEID_MAX) {
		...
		if (!rx_page.is_tmp_alloc) {
			...
			spin_lock_bh(&rx_pkt->sys->common_sys->spinlock);
			/* Add the element to head. */
			list_add(&rx_pkt->link, &rx_pkt->sys->page_recycle_repl
							 ->page_repl_head);
			spin_unlock_bh(&rx_pkt->sys->common_sys->spinlock);
		} else {
	...
	if (notify->evt_id == GSI_CHAN_EVT_EOT ||
		...
		if (unlikely(!rx_skb)) {
			...
			list_for_each_entry_safe(rx_pkt, tmp, head, link) {
				...
				if (!rx_page.is_tmp_alloc) {
					init_page_count(rx_page.page);
					spin_lock_bh(&rx_pkt->sys->common_sys
							      ->spinlock);
					/* Add the element to head. */
					list_add(&rx_pkt->link,
						 &rx_pkt->sys->page_recycle_repl
							  ->page_repl_head);
					spin_unlock_bh(&rx_pkt->sys->common_sys
								->spinlock);
				} else {
					dma_unmap_page(ipa3_ctx->pdev,
						       rx_page.dma_addr,
		...
		list_for_each_entry_safe(rx_pkt, tmp, head, link) {
			...
			trace_handle_page_completion(rx_page.page, rx_skb,
						     notify->bytes_xfered,
						     rx_page.is_tmp_alloc,
						     sys->ep->client);
		}
	} else {
		return NULL;
	...
	if (sys->ep->client != IPA_CLIENT_APPS_WAN_COAL_CONS) {
		for (i = 0; i < num; i++) {
			if (!ipa3_ctx->ipa_wan_skb_page)
				rx_skb = handle_skb_completion(&notify[i],
							       false, NULL);
			else
				rx_skb = handle_page_completion(&notify[i],
								false);
			...
			if (rx_skb) {
				...
				prev_skb = rx_skb;
				trace_ipa3_rx_napi_chain(first_skb, prev_skb,
							 rx_skb);
			}
		}
		if (prev_skb) {
           ...
           */
			...
			for (i = 0; i < num; i++) {
				...
				if (rx_skb) {
					...
					prev_skb = rx_skb;
					trace_ipa3_rx_napi_chain(
						first_skb, prev_skb, rx_skb);
				}
			}
			if (prev_skb) {
				skb_shinfo(prev_skb)->frag_list = NULL;
	...
	if (ipa3_ctx->ipa_wan_skb_page) {
		...
		*aggr_byte_limit = IPA_ADJUST_AGGR_BYTE_LIMIT(*aggr_byte_limit);
		...
		IPAERR("set default rx_buff_sz %lu\n",
		       (unsigned long)IPA_GENERIC_RX_BUFF_SZ(adjusted_sz));
		sys->rx_buff_sz = IPA_GENERIC_RX_BUFF_SZ(adjusted_sz);
		*aggr_byte_limit =
			sys->rx_buff_sz < *aggr_byte_limit ?
			      ...
			      struct ipa3_sys_context *sys)
{
	...
	apps_wan_cons_agg_gro_flag = ipa3_ctx->ipa_client_apps_wan_cons_agg_gro;
	...
	if (IPA_CLIENT_IS_PROD(in->client)) {
		...
		if (IPA_CLIENT_IS_LAN_CONS(in->client) ||
			...
			sys->rx_pool_sz =
				in->desc_fifo_sz / IPA_FIFO_ELEMENT_SIZE - 1;
		...
		} else if (in->client == IPA_CLIENT_MEMCPY_DMA_ASYNC_CONS) {
			IPADBG("assigning policy to client:%d", in->client);
			...
			INIT_DELAYED_WORK(&sys->switch_to_intr_work,
					  ipa3_switch_to_intr_rx_work_func);
		} else if (in->client == IPA_CLIENT_MEMCPY_DMA_SYNC_CONS) {
			IPADBG("assigning policy to client:%d", in->client);
...
static void ipa3_tx_client_rx_notify_release(void *user1, int user2)
{
	...
	IPADBG_LOW("ep=%d fifo_desc_free_count=%d\n", ep_idx,
		   atomic_read(&ipa3_ctx->ep[ep_idx].avail_fifo_desc));
 ...
 * Returns:	0 on success, negative on failure
 */
int ipa3_tx_dp_mul(enum ipa_client_type src, struct ipa_tx_data_desc *data_desc)
{
...
#define IPA_WLAN_HDR_QMAP_ID_OFFSET 1
	...
	list_for_each_entry(entry, &data_desc->link, link) {
		...
		IPADBG_LOW("ep=%d fifo desc=%d\n", ep_idx,
			   atomic_read(&sys->ep->avail_fifo_desc));
	}
...
void ipa_free_skb(struct ipa_rx_data *data)
{
	...
	list_add_tail(&rx_pkt->link, &ipa3_ctx->wc_memb.wlan_comm_desc_list);
	ipa3_ctx->wc_memb.wlan_comm_free_cnt++;
...
int ipa3_sys_setup(struct ipa_sys_connect_params *sys_in,
		   unsigned long *ipa_transport_hdl, u32 *ipa_pipe_num,
		   u32 *clnt_hdl, bool en_status)
{
	struct ipa3_ep_context *ep;
	int ipa_ep_idx;
	int result = -EINVAL;
		...
		if (sys->tx_poll) {
			if (!atomic_read(&sys->curr_polling_state)) {
				...
				if (IPA_ACTIVE_CLIENTS_INC_EP_NO_BLOCK(
					    sys->ep->client)) {
					IPAERR_RL(
						"clk off, event likely handled in NAPI contxt");
					return;
				}
				...
				gsi_config_channel_mode(sys->ep->gsi_chan_hdl,
							GSI_CHAN_MODE_POLL);
				atomic_set(&sys->curr_polling_state, 1);
				__ipa3_update_curr_poll_state(sys->ep->client,
							      1);
				napi_schedule(&tx_pkt->sys->napi_tx);
			}
		} else if (ipa_net_initialized && sys->napi_tx_enable) {
			if (!atomic_cmpxchg(&tx_pkt->sys->in_napi_context, 0,
					    1))
				napi_schedule(&tx_pkt->sys->napi_tx);
		} else {
...
void __ipa_gsi_irq_rx_scedule_poll(struct ipa3_sys_context *sys)
{
   ...
   */
	...
	if (!clk_off && ipa_net_initialized && sys->napi_obj) {
		...
		trace_ipa3_napi_schedule(sys->ep->client);
		napi_schedule(&sys->napi_rx);
	} else if (!clk_off && IPA_CLIENT_IS_LOW_LAT_CONS(sys->ep->client)) {
		tasklet_schedule(&sys->tasklet);
	} else
		queue_work(sys->wq, &sys->work);
}
...
int ipa3_alloc_common_event_ring(void)
{
	...
	if (ipa3_ctx->ipa_gpi_event_rp_ddr) {
		gsi_evt_ring_props.rp_update_vaddr = dma_alloc_coherent(
			ipa3_ctx->pdev, IPA_GSI_EVENT_RP_SIZE, &evt_rp_dma_addr,
			GFP_KERNEL);
				 ...
				 struct ipa3_ep_context *ep)
{
   ...
   */
	...
	if (ep->sys && ep->sys->use_comm_evt_ring) {
		...
		result = ipa_gsi_setup_event_ring(
			ep, IPA_COMMON_EVENT_RING_SIZE, mem_flag);
		if (result)
			goto fail_setup_event_ring;

	} else if (in->client == IPA_CLIENT_APPS_WAN_CONS &&
		   IPA_CLIENT_IS_MAPPED_VALID(IPA_CLIENT_APPS_WAN_COAL_CONS,
					      wan_coal_ep_id)) {
		IPADBG("Wan consumer pipe configured\n");
		result = ipa_gsi_setup_coal_def_channel(
			in, ep, &ipa3_ctx->ep[wan_coal_ep_id]);
		if (result) {
		...
		if (result)
		   ...
		   IPA_CLIENT_IS_MAPPED_VALID(IPA_CLIENT_APPS_LAN_COAL_CONS,
					      lan_coal_ep_id)) {
		IPADBG("Lan consumer pipe configured\n");
		result = ipa_gsi_setup_coal_def_channel(
			in, ep, &ipa3_ctx->ep[lan_coal_ep_id]);
		if (result) {
			IPAERR("Failed to setup default coal GSI channel\n");
			goto fail_setup_event_ring;
		}
	...
	result = ipa_gsi_setup_transfer_ring(ep, ring_size, ep->sys, mem_flag);
	if (result)
				    ...
				    gfp_t mem_flag)
{
	...
	if ((ipa3_ctx->gsi_msi_addr) &&
		...
		gsi_evt_ring_props.intr =
			GSI_INTR_MSI; // intvec chosen dynamically.
	else
		gsi_evt_ring_props.intr = GSI_INTR_IRQ;
	gsi_evt_ring_props.re_size = GSI_EVT_RING_RE_SIZE_16B;
	...
	IPADBG("client=%d moderation threshold cycles=%u cnt=%u\n", ep->client,
	       gsi_evt_ring_props.int_modt, gsi_evt_ring_props.int_modc);
	if (ipa3_ctx->ipa_gpi_event_rp_ddr) {
		gsi_evt_ring_props.rp_update_vaddr = dma_alloc_coherent(
			ipa3_ctx->pdev, IPA_GSI_EVENT_RP_SIZE, &evt_rp_dma_addr,
			GFP_KERNEL);
	...
	/* copy mem info */
	ep->gsi_mem_info.evt_ring_rp_addr = gsi_evt_ring_props.rp_update_addr;
	ep->gsi_mem_info.evt_ring_rp_vaddr = gsi_evt_ring_props.rp_update_vaddr;
...
fail_write_channel_scratch:
	if (gsi_dealloc_channel(ep->gsi_chan_hdl) != GSI_STATUS_SUCCESS) {
					  ...
					  struct ipa3_ep_context *coal_ep)
{
	...
	result = ipa_gsi_setup_transfer_ring(ep, ring_size, coal_ep->sys,
					     GFP_ATOMIC);
	if (result) {
		if (ep->gsi_mem_info.evt_ring_base_vaddr)
			      ...
			      int expected_num, int *actual_num)
{
	...
	ret = gsi_poll_n_channel(sys->ep->gsi_chan_hdl, &notify[idx],
				 expected_num - idx, &poll_num);
   ...
   */
	...
	while (remain_aggr_weight > 0 &&
		...
		if (ipa3_ctx->enable_napi_chain) {
			...
			ret = ipa_poll_gsi_n_pkt(ep->sys, notify, 1, &num);
		}
		if (ret)
			break;
...
static uint64_t pointer_to_tag_wa(struct ipa3_tx_pkt_wrapper *tx_pkt)
{
	...
	if (BITS_PER_LONG == 64) {
		temp = (u16)(~((unsigned long)tx_pkt & 0xFFFF000000000000) >>
			     48);
		if (temp) {
...
int ipa_gsi_ch20_wa(void)
{
	...
	gsi_channel_props.ring_base_vaddr = dma_alloc_coherent(
		ipa3_ctx->pdev, gsi_channel_props.ring_len, &dma_addr, 0);
	gsi_channel_props.ring_base_addr = dma_addr;
	...
	for (i = 0; i < IPA_GSI_CH_20_WA_NUM_CH_TO_ALLOC; i++) {
		...
		result = gsi_alloc_channel(&gsi_channel_props,
					   ipa3_ctx->gsi_dev_hdl, &chan_hdl[i]);
		if (result != GSI_STATUS_SUCCESS) {
...
{
	struct ipa3_sys_context *sys =
		container_of(napi_rx, struct ipa3_sys_context, napi_rx);
	int remain_aggr_weight;
	int ret;
	int cnt = 0;
	int num = 0;
...
start_poll:
   ...
   */
	...
	while (remain_aggr_weight > 0 &&
	       atomic_read(&sys->curr_polling_state)) {
		atomic_set(&ipa3_ctx->transport_pm.eot_activity, 1);
		ret = ipa_poll_gsi_n_pkt(sys, notify, remain_aggr_weight, &num);
		if (ret)
			break;
   ...
   */
	if (cnt < budget && (sys->len > IPA_DEFAULT_SYS_YELLOW_WM)) {
		...
		ret = ipa3_rx_switch_to_intr_mode(sys);
		if (ret == -GSI_STATUS_PENDING_IRQ && napi_reschedule(napi_rx))
			goto start_poll;
		IPA_ACTIVE_CLIENTS_DEC_EP_NO_BLOCK(sys->ep->client);
	} else {
		cnt = budget;
