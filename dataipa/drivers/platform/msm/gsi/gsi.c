 ...
 * 4) / 4) */
#define GSI_GSI_SHRAM_n_EP_FOR_SEQ_HIGH_N_GET(ep_id) \
	(((8 * (ep_id * 10 + 9)) + 4) / 4)
...
static void __gsi_config_all_ch_irq(int ee, uint32_t mask, uint32_t val)
{
	...
	for (k = 0; k < max_k; k++) {
		curr = gsihal_read_reg_nk(GSI_EE_n_CNTXT_SRC_GSI_CH_IRQ_MSK_k,
					  ee, k);
...
static void __gsi_config_all_evt_irq(int ee, uint32_t mask, uint32_t val)
{
	...
	max_k = gsihal_get_bit_map_array_size();
	for (k = 0; k < max_k; k++) {
		curr = gsihal_read_reg_nk(GSI_EE_n_CNTXT_SRC_EV_CH_IRQ_MSK_k,
					  ee, k);
...
static void __gsi_config_all_ieob_irq(int ee, uint32_t mask, uint32_t val)
{
	...
	max_k = gsihal_get_bit_map_array_size();
	for (k = 0; k < max_k; k++) {
		curr = gsihal_read_reg_nk(GSI_EE_n_CNTXT_SRC_IEOB_IRQ_MSK_k, ee,
					  k);
...
static void __gsi_config_ieob_irq_k(int ee, uint32_t k, uint32_t mask,
				    uint32_t val)
{
	uint32_t curr;
...
static void gsi_channel_state_change_wait(unsigned long chan_hdl,
					  struct gsi_chan_ctx *ctx, uint32_t tm,
					  enum gsi_ch_cmd_opcode op)
{
   ...
   */
	...
	for (poll_cnt = 0; poll_cnt < GSI_CMD_POLL_CNT; poll_cnt++) {
		...
		if (gsi_ctx->per.ver >= GSI_VER_3_0) {
			gsi_pending_intr = gsihal_read_reg_nk(
				GSI_EE_n_CNTXT_SRC_GSI_CH_IRQ_k, ee,
				gsihal_get_ch_reg_idx(chan_hdl));
		} else {
		...
		if (gsi_ctx->per.ver == GSI_VER_1_0) {
			gsihal_read_reg_nk_fields(GSI_EE_n_GSI_CH_k_CNTXT_0, ee,
						  chan_hdl, &ch_k_cntxt_0);
     ...
     */
		if (stop_retry == GSI_STOP_CMD_POLL_CNT) {
			if (gsi_ctx->per.ver >= GSI_VER_3_0) {
				gsihal_write_reg_nk(
					GSI_EE_n_CNTXT_SRC_GSI_CH_IRQ_CLR_k, ee,
					gsihal_get_ch_reg_idx(chan_hdl),
					gsi_pending_intr);
			} else {
				gsihal_write_reg_n(
					GSI_EE_n_CNTXT_SRC_GSI_CH_IRQ_CLR, ee,
					gsi_pending_intr);
			}
			ctx->state = curr_state;
			return;
     ...
     */
		...
		GSIDBG("GSI wait on chan_hld=%lu irqtyp=%u state=%u intr=%u\n",
		       chan_hdl, type, ctx->state, gsi_pending_intr);
	}
...
static void gsi_handle_ch_ctrl(int ee)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		max_k = gsihal_get_bit_map_array_size();
		for (k = 0; k < max_k; k++) {
			ch = gsihal_read_reg_nk(GSI_EE_n_CNTXT_SRC_GSI_CH_IRQ_k,
						ee, k);
		...
		for (i = 0; i < GSI_STTS_REG_BITS; i++) {
			if ((1 << i) & ch) {
				...
				ctx = &gsi_ctx->chan[i];
				gsihal_read_reg_nk_fields(
					GSI_EE_n_GSI_CH_k_CNTXT_0, ee, i,
					&ch_k_cntxt_0);
				ctx->state = ch_k_cntxt_0.chstate;
...
static void gsi_handle_ev_ctrl(int ee)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		max_k = gsihal_get_bit_map_array_size();
		for (k = 0; k < max_k; k++) {
			ch = gsihal_read_reg_nk(GSI_EE_n_CNTXT_SRC_EV_CH_IRQ_k,
						ee, k);
			gsihal_write_reg_nk(GSI_EE_n_CNTXT_SRC_EV_CH_IRQ_CLR_k,
					    ee, k, ch);
			...
			for (i = 0; i < GSI_STTS_REG_BITS; i++) {
				if ((1 << i) & ch) {
					...
					gsihal_read_reg_nk_fields(
						GSI_EE_n_EV_CH_k_CNTXT_0, ee,
						evt_hdl, &ev_ch_k_cntxt_0);
...
static void gsi_handle_glob_ee(int ee)
{
	...
	val = gsihal_read_reg_n_fields(GSI_EE_n_CNTXT_GLOB_IRQ_STTS, ee,
				       &cntxt_glob_irq_stts);
			     ...
			     struct gsi_chan_xfer_notify *notify, bool callback)
{
	...
	if (evt->type != GSI_XFER_COMPL_TYPE_GCI) {
		...
		if (ch_ctx->ring.rp_local != rp) {
			ch_ctx->stats.completed += gsi_get_complete_num(
				&ch_ctx->ring, ch_ctx->ring.rp_local, rp);
...
static void gsi_ring_evt_doorbell(struct gsi_evt_ctx *ctx)
{
	...
	gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_DOORBELL_0, gsi_ctx->per.ee,
			    ctx->id, val);
}
...
static void gsi_ring_chan_doorbell(struct gsi_chan_ctx *ctx)
{
   ...
   */
	...
	val = GSI_LSB(ctx->ring.wp_local);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_DOORBELL_0, gsi_ctx->per.ee,
			    ctx->props.ch_id, val);
}
...
static void gsi_handle_ieob(int ee)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		max_k = gsihal_get_bit_map_array_size();
		for (k = 0; k < max_k; k++) {
			ch = gsihal_read_reg_nk(GSI_EE_n_CNTXT_SRC_IEOB_IRQ_k,
						ee, k);
			msk = gsihal_read_reg_nk(
				GSI_EE_n_CNTXT_SRC_IEOB_IRQ_MSK_k, ee, k);
...
check_again_v3_0:
		...
		gsihal_write_reg_n(GSI_EE_n_CNTXT_SRC_IEOB_IRQ_CLR, ee,
				   ch & msk);
...
static void gsi_handle_inter_ee_ch_ctrl(int ee)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		max_k = gsihal_get_bit_map_array_size();
		for (k = 0; k < max_k; k++) {
			ch = gsihal_read_reg_nk(GSI_INTER_EE_n_SRC_GSI_CH_IRQ_k,
						ee, k);
			gsihal_write_reg_nk(GSI_INTER_EE_n_SRC_GSI_CH_IRQ_k, ee,
					    k, ch);
...
static void gsi_handle_inter_ee_ev_ctrl(int ee)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		max_k = gsihal_get_bit_map_array_size();
		for (k = 0; k < max_k; k++) {
			ch = gsihal_read_reg_nk(GSI_INTER_EE_n_SRC_EV_CH_IRQ_k,
						ee, k);
			gsihal_write_reg_nk(GSI_INTER_EE_n_SRC_EV_CH_IRQ_CLR_k,
					    ee, k, ch);
...
static void gsi_handle_general(int ee)
{
	...
	val = gsihal_read_reg_n_fields(GSI_EE_n_CNTXT_GSI_IRQ_STTS, ee,
				       &gsi_irq_stts);
...
static void gsi_handle_irq(void)
{
	...
	while (1) {
		if (!gsi_ctx->per.clk_status_cb())
			break;
		type = gsihal_read_reg_n_fields(GSI_EE_n_CNTXT_TYPE_IRQ, ee,
						&ctx_type_irq);
		...
		gsi_ctx->gsi_isr_cache[index].timestamp = sched_clock();
...
static irqreturn_t gsi_isr(int irq, void *ctxt)
{
	if (gsi_ctx->per.req_clk_cb) {
		...
		if (atomic_read(&gsi_ctx->num_unclock_irq) == GSI_IRQ_STORM_THR)
...
static uint32_t gsi_get_max_channels(enum gsi_ver ver)
{
	...
	case GSI_VER_MAX:
		...
		break;
	case GSI_VER_1_0:
		gsihal_read_reg_n_fields(GSI_EE_n_GSI_HW_PARAM, gsi_ctx->per.ee,
					 &hw_param);
...
int gsi_map_base(phys_addr_t gsi_base_addr, u32 gsi_size, enum gsi_ver ver)
{
	...
	gsi_ctx->base = devm_ioremap(gsi_ctx->dev, gsi_base_addr, gsi_size);
	...
	GSIDBG("GSI base(%pa) mapped to (%pK) with len (0x%x)\n",
	       &gsi_base_addr, gsi_ctx->base, gsi_size);
...
static void __gsi_msi_write_msg(struct msi_desc *desc, struct msi_msg *msg)
{
	...
	if (IS_ERR_OR_NULL(desc) || IS_ERR_OR_NULL(msg) ||
	    IS_ERR_OR_NULL(gsi_ctx))
		BUG();
	...
	if ((msi >= gsi_ctx->msi.num) ||
	    (test_bit(msi, gsi_ctx->msi.allocated)))
		BUG();

	/* Save the message for later use */
	memcpy(&gsi_ctx->msi.msg[msi], msg, sizeof(*msg));

	dev_notice(gsi_ctx->dev, "saved msi %u msg data %u addr 0x%08x%08x\n",
		   msi, msg->data, msg->address_hi, msg->address_lo);
...
static int __gsi_allocate_msis(void)
{
	...
	/* Allocate all MSIs */
	GSIDBG("gsi_ctx->dev = %lu, gsi_ctx->msi.num = %d", gsi_ctx->dev,
	       gsi_ctx->msi.num);
...
int gsi_register_device(struct gsi_per_props *props, unsigned long *dev_hdl)
{
     ...
     */
       ...
       */
		...
		GSIDBG("succeeded to register isr for %u\n", props->irq);
   ...
   */
	...
	if (running_emulation) {
		...
		gsi_ctx->intcntrlr_mem_size = props->emulator_intcntrlr_size;
...
#endif
				gsi_ctx->dev, props->emulator_intcntrlr_addr,
				props->emulator_intcntrlr_size);
		if (!gsi_ctx->intcntrlr_base) {
			GSIERR("failed to remap emulator's interrupt controller HW\n");
			gsi_unmap_base();
		...
		GSIDBG("Emulator's interrupt controller base(%pa) mapped to (%pK) with len "
		       "(0x%lx)\n",
		       &(props->emulator_intcntrlr_addr),
		       gsi_ctx->intcntrlr_base, props->emulator_intcntrlr_size);
   ...
   */
	__gsi_config_glob_irq(props->ee, gsihal_get_glob_irq_en_gp_int1_mask(),
			      0);
	...
	gsihal_write_reg_n_fields(GSI_EE_n_CNTXT_GSI_IRQ_EN, gsi_ctx->per.ee,
				  &gen_irq);
	...
	if ((gsi_ctx->per.ver >= GSI_VER_2_0) &&
		...
		gsihal_write_reg_n(GSI_EE_n_CNTXT_MSI_BASE_LSB, gsi_ctx->per.ee,
				   0);
		gsihal_write_reg_n(GSI_EE_n_CNTXT_MSI_BASE_MSB, gsi_ctx->per.ee,
				   0);
	}

	gsihal_read_reg_n_fields(GSI_EE_n_GSI_STATUS, gsi_ctx->per.ee,
				 &gsi_status);
     ...
     */
		res = setup_emulator_cntrlr(gsi_ctx->intcntrlr_base,
			     ...
			     struct gsi_device_scratch *val)
{
	...
	if (val->max_usb_pkt_size_valid && val->max_usb_pkt_size != 1024 &&
	    val->max_usb_pkt_size != 512 && val->max_usb_pkt_size != 64) {
	...
	if (val->max_usb_pkt_size_valid) {
		...
		if (val->max_usb_pkt_size > 64)
			max_usb_pkt_size = (val->max_usb_pkt_size == 1024) ? 1 :
									     0;
		gsi_ctx->scratch.word0.s.max_usb_pkt_size = max_usb_pkt_size;
	}

	gsihal_write_reg_n(GSI_EE_n_CNTXT_SCRATCH_0, gsi_ctx->per.ee,
				     ...
				     uint8_t evt_id, unsigned int ee)
{
	...
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_0, ee, evt_id,
				   &ev_ch_k_cntxt_0);

	ev_ch_k_cntxt_1.r_length = props->ring_len;
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_1, ee, evt_id,
				   &ev_ch_k_cntxt_1);

	ev_ch_k_cntxt_2.r_base_addr_lsbs = GSI_LSB(props->ring_base_addr);
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_2, ee, evt_id,
				   &ev_ch_k_cntxt_2);

	ev_ch_k_cntxt_3.r_base_addr_msbs = GSI_MSB(props->ring_base_addr);
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_3, ee, evt_id,
				   &ev_ch_k_cntxt_3);

	ev_ch_k_cntxt_8.int_modt = props->int_modt;
	ev_ch_k_cntxt_8.int_modc = props->int_modc;
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_8, ee, evt_id,
				   &ev_ch_k_cntxt_8);

	ev_ch_k_cntxt_9.intvec = props->intvec;
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_9, ee, evt_id,
				   &ev_ch_k_cntxt_9);

	if (props->intf != GSI_EVT_CHTYPE_WDI3_V2_EV) {
		ev_ch_k_cntxt_10.msi_addr_lsb = GSI_LSB(props->msi_addr);
		gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_10, ee,
					   evt_id, &ev_ch_k_cntxt_10);

		ev_ch_k_cntxt_11.msi_addr_msb = GSI_MSB(props->msi_addr);
		gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_11, ee,
					   evt_id, &ev_ch_k_cntxt_11);

		ev_ch_k_cntxt_12.rp_update_addr_lsb =
			GSI_LSB(props->rp_update_addr);
		gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_12, ee,
					   evt_id, &ev_ch_k_cntxt_12);

		ev_ch_k_cntxt_13.rp_update_addr_msb =
			GSI_MSB(props->rp_update_addr);
		gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_CNTXT_13, ee,
					   evt_id, &ev_ch_k_cntxt_13);
	} else {
...
static void gsi_prime_evt_ring(struct gsi_evt_ctx *ctx)
{
	...
	ctx->ring.wp_local =
		ctx->ring.base + ctx->ring.max_num_elem * ctx->ring.elem_sz;

	/* write order MUST be MSB followed by LSB */
	db.write_ptr_msb = GSI_MSB(ctx->ring.wp_local);
	gsihal_write_reg_nk_fields(GSI_EE_n_EV_CH_k_DOORBELL_1, gsi_ctx->per.ee,
				   ctx->id, &db);
...
static int gsi_validate_evt_ring_props(struct gsi_evt_ring_props *props)
{
	...
	if (props->intf == GSI_EVT_CHTYPE_GPI_EV && !props->ring_base_vaddr) {
	...
	if (props->intf != GSI_EVT_CHTYPE_MHI_EV && props->evchid_valid) {
...
static int gsi_cleanup_xfer_user_data(unsigned long chan_hdl,
				      void (*cleanup_cb)(void *chan_user_data,
							 void *xfer_user_data))
{
	struct gsi_chan_ctx *ctx;
	uint64_t i;
...
gsi_read_event_ring_rp_ddr(struct gsi_evt_ring_props *props, uint8_t id, int ee)
{
 ...
 */
static inline uint64_t
gsi_read_event_ring_rp_reg(struct gsi_evt_ring_props *props, uint8_t id, int ee)
{
	uint64_t rp;

	rp = gsihal_read_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_4, ee, id);
	rp |= ((uint64_t)gsihal_read_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_5, ee, id))
	      << 32;
			  ...
			  struct gsi_evt_ring_props *props)
{
	...
	if (IS_ERR_OR_NULL(ctx) || IS_ERR_OR_NULL(props) ||
	    IS_ERR_OR_NULL(gsi_ctx))
		BUG();
		       ...
		       unsigned long *evt_ring_hdl)
{
	...
	if (props->rp_update_addr != 0) {
		GSIDBG("Using DDR to read event RP for virt evt id: %lu\n",
		       evt_id);
		props->gsi_read_event_ring_rp = gsi_read_event_ring_rp_ddr;
	} else {
		GSIDBG("Using CONTEXT reg to read event RP for virt evt id: %lu\n",
		       evt_id);
		props->gsi_read_event_ring_rp = gsi_read_event_ring_rp_reg;
	}
	...
	*evt_ring_hdl = evt_id;
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		...
		if ((props->intf != GSI_EVT_CHTYPE_GPI_EV) &&
			...
			__gsi_config_ieob_irq(gsi_ctx->per.ee, 1 << ctx->id,
					      ~0);
	}
	spin_unlock_irqrestore(&gsi_ctx->slock, flags);
...
{
	gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_SCRATCH_0, gsi_ctx->per.ee,
			    evt_ring_hdl, val.data.word1);
	gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_SCRATCH_1, gsi_ctx->per.ee,
			    evt_ring_hdl, val.data.word2);
}
...
int gsi_dealloc_evt_ring(unsigned long evt_ring_hdl)
{
	...
	ev_ch_cmd.opcode = op;
	gsihal_write_reg_n_fields(GSI_EE_n_EV_CH_CMD, gsi_ctx->per.ee,
				  &ev_ch_cmd);
	res = wait_for_completion_timeout(&ctx->compl, GSI_CMD_TIMEOUT);
	if (res == 0) {
		GSIERR("evt_id=%lu timed out\n", evt_ring_hdl);
			       ...
			       uint32_t *db_addr_wp_msb)
{
	...
	*db_addr_wp_lsb = gsi_ctx->per.phys_addr +
			  gsihal_get_reg_nk_ofst(GSI_EE_n_EV_CH_k_DOORBELL_0,
						 gsi_ctx->per.ee, evt_ring_hdl);

	*db_addr_wp_msb = gsi_ctx->per.phys_addr +
			  gsihal_get_reg_nk_ofst(GSI_EE_n_EV_CH_k_DOORBELL_1,
						 gsi_ctx->per.ee, evt_ring_hdl);
...
int gsi_ring_ch_ring_db(unsigned long chan_hdl, uint64_t value)
{
	...
	/* write MSB first */
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_DOORBELL_1, gsi_ctx->per.ee,
			    ctx->props.ch_id, GSI_MSB(ctx->ring.wp_local));
				     ...
				     unsigned int ee)
{
	...
	gsihal_write_reg_nk_fields(GSI_EE_n_GSI_CH_k_QOS, ee, props->ch_id,
				   &ch_k_qos);
}
				 ...
				 uint8_t erindex)
{
	...
	gsihal_write_reg_nk_fields(GSI_EE_n_GSI_CH_k_CNTXT_0, ee, props->ch_id,
				   &ch_k_cntxt_0);

	ch_k_cntxt_1.r_length = props->ring_len;
	gsihal_write_reg_nk_fields(GSI_EE_n_GSI_CH_k_CNTXT_1, ee, props->ch_id,
				   &ch_k_cntxt_1);

	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_2, ee, props->ch_id,
			    GSI_LSB(props->ring_base_addr));
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_3, ee, props->ch_id,
			    GSI_MSB(props->ring_base_addr));
...
static int gsi_validate_channel_props(struct gsi_chan_props *props)
{
	...
	if ((props->re_size == GSI_CHAN_RE_SIZE_4B && props->ring_len % 4) ||
	    (props->re_size == GSI_CHAN_RE_SIZE_8B && props->ring_len % 8) ||
	    (props->re_size == GSI_CHAN_RE_SIZE_16B && props->ring_len % 16) ||
	    (props->re_size == GSI_CHAN_RE_SIZE_32B && props->ring_len % 32) ||
	    (props->re_size == GSI_CHAN_RE_SIZE_64B && props->ring_len % 64)) {
		GSIERR("bad params ring_len %u not a multiple of re size %u\n",
		       props->ring_len, props->re_size);
		return -GSI_STATUS_INVALID_PARAMS;
	}
	...
	if ((props->ring_base_addr & 0xFFFFFFFF00000000ULL) !=
		...
		GSIERR("MSB is not fixed on ring base 0x%llx size 0x%x\n",
		       props->ring_base_addr, props->ring_len);
		return -GSI_STATUS_INVALID_PARAMS;
	}

	if (props->prot == GSI_CHAN_PROT_GPI && !props->ring_base_vaddr) {
		GSIERR("protocol %u requires ring base VA\n", props->prot);
		return -GSI_STATUS_INVALID_PARAMS;
	}
		      ...
		      unsigned long *chan_hdl)
{
   ...
   */
	...
	user_data = devm_kzalloc(
		gsi_ctx->dev, user_data_size * sizeof(*user_data), GFP_KERNEL);
	if (user_data == NULL) {
	...
	if (erindex < GSI_EVT_RING_MAX) {
		...
		if (ctx->evtr->num_of_chan_allocated >=
			...
			GSIERR("too many channels sharing the same event ring %u\n",
			       erindex);
			GSI_ASSERT();
		}
		if (props->prot != GSI_CHAN_PROT_GCI) {
...
{
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_0, gsi_ctx->per.ee,
			    chan_hdl, val.data.word1);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_1, gsi_ctx->per.ee,
			    chan_hdl, val.data.word2);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_2, gsi_ctx->per.ee,
			    chan_hdl, val.data.word3);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_3, gsi_ctx->per.ee,
			    chan_hdl, val.data.word4);
}

static void __gsi_write_wdi3_channel_scratch2_reg(
	unsigned long chan_hdl,
...
int gsi_write_channel_scratch3_reg(
	unsigned long chan_hdl, union __packed gsi_wdi_channel_scratch3_reg val)
{
	struct gsi_chan_ctx *ctx;
...
static void __gsi_read_wdi3_channel_scratch2_reg(
	unsigned long chan_hdl,
	union __packed gsi_wdi3_channel_scratch2_reg *val)
{
...
int gsi_write_wdi3_channel_scratch2_reg(
	unsigned long chan_hdl,
	union __packed gsi_wdi3_channel_scratch2_reg val)
{
	struct gsi_chan_ctx *ctx;
...
int gsi_read_wdi3_channel_scratch2_reg(
	unsigned long chan_hdl,
	union __packed gsi_wdi3_channel_scratch2_reg *val)
{
	struct gsi_chan_ctx *ctx;
...
int gsi_query_channel_db_addr(unsigned long chan_hdl, uint32_t *db_addr_wp_lsb,
			      uint32_t *db_addr_wp_msb)
{
	if (!gsi_ctx) {
		pr_err("%s:%d gsi context not allocated\n", __func__, __LINE__);
...
int gsi_start_channel(unsigned long chan_hdl)
{
	...
	if (ctx->evtr && ctx->evtr->props.intf == GSI_EVT_CHTYPE_GPI_EV &&
	    val != GSI_INTR_IRQ) {
	...
	ch_cmd.chid = chan_hdl;
	ch_cmd.opcode = op;
	gsihal_write_reg_n_fields(GSI_EE_n_GSI_CH_CMD, gsi_ctx->per.ee,
				  &ch_cmd);
	GSIDBG("GSI Channel Start, waiting for completion\n");
	gsi_channel_state_change_wait(chan_hdl, ctx, GSI_START_CMD_TIMEOUT_MS,
				      op);
...
void gsi_dump_ch_info(unsigned long chan_hdl)
{
	...
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_0, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX0  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_1, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX1  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_2, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX2  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_3, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX3  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_4, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX4  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_5, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX5  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_6, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d CTX6  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_7, gsi_ctx->per.ee,
				 chan_hdl);
	...
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_QOS, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d QOS   0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_0, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d SCR0  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_1, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d SCR1  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_2, gsi_ctx->per.ee,
				 chan_hdl);
	GSIERR("CH%2d SCR2  0x%x\n", chan_hdl, val);
	val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_3, gsi_ctx->per.ee,
				 chan_hdl);
...
int gsi_stop_channel(unsigned long chan_hdl)
{
	...
	gsi_channel_state_change_wait(chan_hdl, ctx, GSI_STOP_CMD_TIMEOUT_MS,
				      op);
...
int gsi_stop_db_channel(unsigned long chan_hdl)
{
	...
	res = wait_for_completion_timeout(
		&ctx->compl, msecs_to_jiffies(GSI_STOP_CMD_TIMEOUT_MS));
	if (res == 0) {
					...
					uint16_t *num_free_re)
{
	...
	if (!ctx->evtr) {
		rp = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_4, ee,
					ctx->props.ch_id);
		rp |= ctx->ring.rp & GSI_MSB_MASK;
		ctx->ring.rp = rp;
	} else {
...
int gsi_query_channel_info(unsigned long chan_hdl, struct gsi_chan_info *info)
{
	struct gsi_chan_ctx *ctx;
	spinlock_t *slock;
	unsigned long flags;
	...
	rp = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_4, ee,
				ctx->props.ch_id);
	rp |= ((uint64_t)gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_5, ee,
					    ctx->props.ch_id))
	      << 32;
	ctx->ring.rp = rp;
	info->rp = rp;

	wp = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_6, ee,
				ctx->props.ch_id);
	wp |= ((uint64_t)gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_7, ee,
					    ctx->props.ch_id))
	      << 32;
	ctx->ring.wp = wp;
	info->wp = wp;
	...
	if (info->evt_valid) {
		rp = gsihal_read_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_4, ee,
					ctx->evtr->id);
		...
		wp = gsihal_read_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_6, ee,
					ctx->evtr->id);
	...
	GSIDBG("ch=%lu RP=0x%llx WP=0x%llx ev_valid=%d ERP=0x%llx EWP=0x%llx\n",
	       chan_hdl, info->rp, info->wp, info->evt_valid, info->evt_rp,
	       info->evt_wp);
...
bool gsi_is_event_pending(unsigned long chan_hdl)
{
	...
	rp = ctx->evtr->props.gsi_read_event_ring_rp(&ctx->evtr->props,
						     ctx->evtr->id, ee);
	rp |= ctx->evtr->ring.rp & GSI_MSB_MASK;
	rp_local = ctx->evtr->ring.rp_local;
...
int __gsi_populate_tre(struct gsi_chan_ctx *ctx, struct gsi_xfer_elem *xfer)
{
	struct gsi_tre tre;
	...
	idx = gsi_find_idx_from_addr(&ctx->ring, ctx->ring.wp_local);
	tre_ptr =
		(struct gsi_tre *)(ctx->ring.base_va + idx * ctx->ring.elem_sz);
		       ...
		       int *actual_num)
{
	...
	if (chan_hdl >= gsi_ctx->max_ch || !notify || !actual_num ||
	    expected_num <= 0) {
	...
	if (ctx->evtr->ring.rp == ctx->evtr->ring.rp_local) {
		...
		if (rp == ctx->evtr->ring.rp_local) {
			...
			if (gsi_ctx->per.ver >= GSI_VER_3_0) {
				gsihal_write_reg_nk(
					GSI_EE_n_CNTXT_SRC_IEOB_IRQ_CLR_k, ee,
					gsihal_get_ch_reg_idx(ctx->evtr->id),
					gsihal_get_ch_reg_mask(ctx->evtr->id));
			} else {
				gsihal_write_reg_n(
					GSI_EE_n_CNTXT_SRC_IEOB_IRQ_CLR, ee,
					1 << ctx->evtr->id);
			}
			...
			ctx->evtr->ring.rp = rp;
			if (rp == ctx->evtr->ring.rp_local) {
				spin_unlock_irqrestore(&ctx->evtr->ring.slock,
						       flags);
	...
	*actual_num = gsi_get_complete_num(
		&ctx->evtr->ring, ctx->evtr->ring.rp_local, ctx->evtr->ring.rp);
...
int gsi_config_channel_mode(unsigned long chan_hdl, enum gsi_chan_mode mode)
{
	...
	if (curr == GSI_CHAN_MODE_CALLBACK && mode == GSI_CHAN_MODE_POLL) {
		if (gsi_ctx->per.ver >= GSI_VER_3_0) {
			...
			__gsi_config_ieob_irq(gsi_ctx->per.ee,
					      1 << ctx->evtr->id, 0);
		}
		if (gsi_ctx->per.ver >= GSI_VER_3_0) {
			gsihal_write_reg_nk(
     ...
     */
		if ((gsi_ctx->per.ver == GSI_VER_2_2 ||
			...
			u32 src = gsihal_read_reg_n(GSI_EE_n_CNTXT_SRC_IEOB_IRQ,
						    gsi_ctx->per.ee);
			if (src & (1 << ctx->evtr->id)) {
				if (gsi_ctx->per.ver >= GSI_VER_3_0) {
					__gsi_config_ieob_irq_k(
						gsi_ctx->per.ee,
						gsihal_get_ch_reg_idx(
							ctx->evtr->id),
						gsihal_get_ch_reg_mask(
							ctx->evtr->id),
						0);
					gsihal_write_reg_nk(
						GSI_EE_n_CNTXT_SRC_IEOB_IRQ_CLR_k,
				...
				GSIDBG("IEOB WA pnd cnt = %ld prvmode = %d\n",
				       ctx->stats.poll_pending_irq, chan_mode);
...
static void gsi_configure_ieps(enum gsi_ver ver)
{
	...
	if (ver >= GSI_VER_2_5)
		gsihal_write_reg(GSI_GSI_IRAM_PTR_TLV_CH_NOT_FULL, 17);

	if (ver >= GSI_VER_2_11)
		gsihal_write_reg(GSI_GSI_IRAM_PTR_MSI_DB, 18);
	if (ver >= GSI_VER_3_0)
		gsihal_write_reg(GSI_GSI_IRAM_PTR_INT_NOTIFY_MCS, 19);
}
...
static void gsi_dump_halt_debug_reg(unsigned int chan_idx, unsigned int ee)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_2_9)
		GSIERR("GSI_EE_n_GSI_CH_k_SCRATCH_4 = 0x%x\n",
		       gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_4, ee,
					  chan_idx));

	gsihal_read_reg_nk_fields(GSI_EE_n_GSI_CH_k_CNTXT_0, ee, chan_idx,
				  &ch_k_cntxt_0);
...
int gsi_halt_channel_ee(unsigned int chan_idx, unsigned int ee, int *code)
{
	...
	cmd.ee = ee;
	gsihal_write_reg_n_fields(GSI_EE_n_GSI_EE_GENERIC_CMD, gsi_ctx->per.ee,
				  &cmd);
			       ...
			       int *code)
{
	...
	gsihal_read_reg_nk_fields(GSI_EE_n_GSI_CH_k_CNTXT_0, gsi_ctx->per.ee,
				  chan_idx, &ch_k_cntxt_0);
	curr_state = ch_k_cntxt_0.chstate;
...
{
	struct gsihal_reg_gsi_ee_generic_cmd cmd;
	enum gsi_generic_ee_cmd_opcode op =
		enable ? GSI_GEN_EE_CMD_ENABLE_FLOW_CHANNEL :
...
wait_again:
	fc_pending =
		gsihal_read_reg_n(GSI_GSI_SHRAM_n,
				  (ep_id * GSI_FC_NUM_WORDS_PER_CHNL_SHRAM) +
					  GSI_FC_STATE_INDEX_SHRAM) &
		GSI_FC_PENDING_MASK;
	res = wait_for_completion_timeout(&gsi_ctx->gen_ee_cmd_compl,
	...
	if (res == 0) {
		...
		if (op == GSI_GEN_EE_CMD_ENABLE_FLOW_CHANNEL &&
		    wait_due_pending < GSI_FC_MAX_TIMEOUT && fc_pending) {
			wait_due_pending++;
			goto wait_again;
		}
		GSIERR("GSI_EE_n_CNTXT_GLOB_IRQ_EN_OFFS = 0x%x\n",
		       gsihal_read_reg_n(GSI_EE_n_CNTXT_GLOB_IRQ_EN,
					 gsi_ctx->per.ee));
		GSIERR("GSI_EE_n_CNTXT_GLOB_IRQ_STTS_OFFS IRQ type = 0x%x\n",
		       gsihal_read_reg_n(GSI_EE_n_CNTXT_GLOB_IRQ_STTS,
					 gsi_ctx->per.ee));
	}
	...
	GSIDBG("Flow control command response GENERIC_CMD_RESPONSE_CODE = %u, val = "
	       "%u\n",
	       gsi_ctx->scratch.word0.s.generic_ee_cmd_return_code,
	       gsi_ctx->scratch.word0.val);
...
int gsi_map_virtual_ch_to_per_ep(u32 ee, u32 chan_num, u32 per_ep_index)
{
	...
	gsihal_write_reg_nk(GSI_MAP_EE_n_CH_k_VP_TABLE, ee, chan_num,
			    per_ep_index);
				...
				uint32_t db_addr_low, uint32_t db_addr_high)
{
	...
	if (gsi_ctx->per.ver >= GSI_VER_2_9) {
		gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_10, gsi_ctx->per.ee,
				    evt_ring_hdl, db_addr_low);

		gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_11, gsi_ctx->per.ee,
				    evt_ring_hdl, db_addr_high);
	} else {
		gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_12, gsi_ctx->per.ee,
				    evt_ring_hdl, db_addr_low);

		gsihal_write_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_13, gsi_ctx->per.ee,
				    evt_ring_hdl, db_addr_high);
	}
}
...
int gsi_ntn3_client_stats_get(unsigned ep_id, int scratch_id, unsigned chan_hdl)
{
	...
	case -1:
		return gsihal_read_reg_n(
			GSI_GSI_SHRAM_n,
			GSI_GSI_SHRAM_n_EP_FOR_SEQ_HIGH_N_GET(ep_id));
	case 4:
		return (gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_4,
					   gsi_ctx->per.ee, chan_hdl) >>
			GSI_NTN3_PENDING_DB_AFTER_RB_MASK) &
		       GSI_NTN3_PENDING_DB_AFTER_RB_SHIFT;
		break;
	case 5:
		return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_5,
					  gsi_ctx->per.ee, chan_hdl);
		break;
	case 6:
		return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_6,
					  gsi_ctx->per.ee, chan_hdl);
		break;
	case 7:
		return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_7,
					  gsi_ctx->per.ee, chan_hdl);
		break;
	case 8:
		return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_8,
					  gsi_ctx->per.ee, chan_hdl);
		break;
	case 9:
		return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_9,
					  gsi_ctx->per.ee, chan_hdl);
		break;
	default:
...
#define GSI_AQC_RDM_ERR_SHIFT 16
	...
	if (gsi_ctx->per.ver >= GSI_VER_3_0) {
		...
		case 5:
			return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_5,
						  gsi_ctx->per.ee, chan_hdl) &
			       GSI_RTK_ERR_STATS_MASK;
			break;
		case 6:
			return gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_6,
						  gsi_ctx->per.ee, chan_hdl) &
			       GSI_NTN_ERR_STATS_MASK;
			break;
		case 7:
			val = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_7,
						 gsi_ctx->per.ee, chan_hdl);
       ...
       * required channel.
       */
			return gsihal_read_reg_n(GSI_GSI_SHRAM_n,
						 ep_id * 12 + 7) &
			       GSI_RTK_ERR_STATS_MASK;
		}
	}
...
uint64_t gsi_read_event_ring_wp(int evtr_id, int ee)
{
	...
	wp = gsihal_read_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_6, ee, evtr_id);
	wp |= ((uint64_t)gsihal_read_reg_nk(GSI_EE_n_EV_CH_k_CNTXT_7, ee,
					    evtr_id))
	      << 32;
...
uint64_t gsi_read_chan_ring_rp(int chan_id, int ee)
{
	...
	rp = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_4, ee, chan_id);
	rp |= ((uint64_t)gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_5, ee,
					    chan_id))
	      << 32;
...
uint64_t gsi_read_chan_ring_wp(int chan_id, int ee)
{
	...
	wp = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_6, ee, chan_id);
	wp |= ((uint64_t)gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_CNTXT_7, ee,
					    chan_id))
	      << 32;
...
uint64_t gsi_read_chan_ring_re_fetch_wp(int chan_id, int ee)
{
	...
	wp = gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_RE_FETCH_WRITE_PTR, ee,
				chan_id);
...
uint32_t gsi_get_chan_stop_stm(int chan_id, int ee)
{
	uint32_t ch_scratch;
	ch_scratch =
		gsihal_read_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_4, ee, chan_id);
...
void gsi_update_almst_empty_thrshold(unsigned long chan_hdl,
	...
	unsigned long chan_hdl, struct __packed gsi_mhi_channel_scratch mscr)
{
   ...
   */
	...
	/* WRITE */
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_0, gsi_ctx->per.ee,
			    chan_hdl, scr.data.word1);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_1, gsi_ctx->per.ee,
			    chan_hdl, scr.data.word2);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_2, gsi_ctx->per.ee,
			    chan_hdl, scr.data.word3);
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_3, gsi_ctx->per.ee,
			    chan_hdl, scr.data.word4);
...
int gsi_get_hw_profiling_stats(struct gsi_hw_profiling_data *stats)
{
	...
	stats->bp_cnt =
		(u64)gsihal_read_reg(GSI_GSI_MCS_PROFILING_BP_CNT_LSB) +
		((u64)gsihal_read_reg(GSI_GSI_MCS_PROFILING_BP_CNT_MSB) << 32);
	stats->bp_and_pending_cnt =
		...
		((u64)gsihal_read_reg(
		 ...
		 << 32);
	stats->mcs_busy_cnt =
		(u64)gsihal_read_reg(GSI_GSI_MCS_PROFILING_MCS_BUSY_CNT_LSB) +
		((u64)gsihal_read_reg(GSI_GSI_MCS_PROFILING_MCS_BUSY_CNT_MSB)
		 << 32);
	stats->mcs_idle_cnt =
		(u64)gsihal_read_reg(GSI_GSI_MCS_PROFILING_MCS_IDLE_CNT_LSB) +
		((u64)gsihal_read_reg(GSI_GSI_MCS_PROFILING_MCS_IDLE_CNT_MSB)
		 << 32);
...
static int msm_gsi_probe(struct platform_device *pdev)
{
	...
	gsi_ctx->ipc_logbuf =
		ipc_log_context_create(GSI_IPC_LOG_PAGES, "gsi", MINIDUMP_MASK);
