 ...
 * *****************************************************************************
 */
int setup_emulator_cntrlr(void __iomem *intcntrlr_base, u32 intcntrlr_mem_size)
{
	...
	GSIDBG("CTL_VER_CNT reg val(0x%x) intr cnt(%u) cntrlr ver(0x%x) rangeCnt(%u)\n",
   ...
   */
	if (ver == 0 || ver == 0xFFFF || ver < DEO_IC_INT_CTL_VER_MIN) {
		GSIERR("Error: invalid interrupt controller version 0x%x\n",
		       ver);
   ...
   */
	if ((intrCnt % 32) != 0) {
		GSIERR("Invalid interrupt count read from HW 0x%04x\n",
		       intrCnt);
   ...
   */
	if (rangeCnt > DEO_IC_MAX_RANGE_CNT) {
		GSIERR("SW interrupt limit(%u) passed, increase DEO_IC_MAX_RANGE_CNT(%u)\n",
		       rangeCnt, DEO_IC_MAX_RANGE_CNT);
		return -GSI_STATUS_ERROR;
	}
   ...
   */
	...
	if (val > intcntrlr_mem_size) {
		GSIERR("Interrupt controller register range (%u) exceeds dtsi provisioned "
		       "size (%u)\n",
   ...
   */
	...
	gsi_emu_writel(0x0, intcntrlr_base + GE_INT_MASTER_ENABLE);
   ...
   */
     ...
     */
		...
		gsi_emu_writel(0x0, intcntrlr_base + GE_INT_TYPE_n(range));
	}
   ...
   */
	...
	gsi_emu_writel(0x00FF, /* See <---HERE above */
   ...
   */
	GSIDBG("Writing GE_INT_MASTER_ENABLE\n");
	gsi_emu_writel(0x1, intcntrlr_base + GE_INT_MASTER_ENABLE);
 ...
 * *****************************************************************************
 */
irqreturn_t emulator_hard_irq_isr(int irq, void *ctxt)
{
   ...
   */
	gsi_emu_writel(0x0, gsi_ctx_ptr->intcntrlr_base + GE_INT_OUT_ENABLE);
 ...
 * *****************************************************************************
 */
irqreturn_t emulator_soft_irq_isr(int irq, void *ctxt)
{
	struct gsi_ctx *gsi_ctx_ptr = (struct gsi_ctx *)ctxt;
   ...
   */
	gsi_emu_writel(0x1, gsi_ctx_ptr->intcntrlr_base + GE_INT_OUT_ENABLE);
