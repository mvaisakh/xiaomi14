 ...
 * Total number of client types with and without configurable number of
 * sub-clients
 */
#define HW_FENCE_MAX_CLIENT_TYPE           \
 ...
 * Each bit in this mask represents possible doorbell flag ids for which hw
 * fence driver can receive
 */
...
/**
 * struct hw_fence_client_types - Table describing all supported client types,
 * used to parse device-tree properties related to client queue size.
 *
 * The fields name, init_id, and max_clients_num are constants. Default values
 * for clients_num, queues_num, and skip_txq_wr_idx are provided in this table,
 * and clients_num, queues_num, queue_entries, and skip_txq_wr_idx can be read
 * from device-tree.
 *
 * If a value for queue entries is not parsed for the client type, then the
 * default number of client queue entries (parsed from device-tree) is used.
 *
 * Notes:
 * 1. Client types must be in the same order as client_ids within the enum
 * 'hw_fence_client_id'.
 * 2. Each HW Fence client ID must be described by one of the client types in
 * this table.
 * 3. A new client type must set: name, init_id, max_clients_num, clients_num,
 * queues_num, and skip_txq_wr_idx.
 * 4. Either constant HW_FENCE_MAX_CLIENT_TYPE_CONFIGURABLE or
 * HW_FENCE_MAX_CLIENT_TYPE_STATIC must be incremented as appropriate for new
 * client types.
 */
struct hw_fence_client_type_desc
	hw_fence_client_types[HW_FENCE_MAX_CLIENT_TYPE] = {
		{ "gpu", HW_FENCE_CLIENT_ID_CTX0, HW_FENCE_CLIENT_TYPE_MAX_GPU,
		  HW_FENCE_CLIENT_TYPE_MAX_GPU, HW_FENCE_CLIENT_QUEUES, 0, 0, 0,
		  0, 0, 0, false },
		{ "dpu", HW_FENCE_CLIENT_ID_CTL0, HW_FENCE_CLIENT_TYPE_MAX_DPU,
		  HW_FENCE_CLIENT_TYPE_MAX_DPU, HW_FENCE_CLIENT_QUEUES, 0, 0, 0,
		  0, 0, 0, false },
		{ "val", HW_FENCE_CLIENT_ID_VAL0, HW_FENCE_CLIENT_TYPE_MAX_VAL,
		  HW_FENCE_CLIENT_TYPE_MAX_VAL, HW_FENCE_CLIENT_QUEUES, 0, 0, 0,
		  0, 0, 0, false },
		{ "ipe", HW_FENCE_CLIENT_ID_IPE, HW_FENCE_CLIENT_TYPE_MAX_IPE,
		  0, HW_FENCE_CLIENT_QUEUES, 0, 0, 0, 0, 0, 0, false },
		{ "vpu", HW_FENCE_CLIENT_ID_VPU, HW_FENCE_CLIENT_TYPE_MAX_VPU,
		  0, HW_FENCE_CLIENT_QUEUES, 0, 0, 0, 0, 0, 0, false },
		{ "ife0", HW_FENCE_CLIENT_ID_IFE0, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife1", HW_FENCE_CLIENT_ID_IFE1, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife2", HW_FENCE_CLIENT_ID_IFE2, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife3", HW_FENCE_CLIENT_ID_IFE3, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife4", HW_FENCE_CLIENT_ID_IFE4, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife5", HW_FENCE_CLIENT_ID_IFE5, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife6", HW_FENCE_CLIENT_ID_IFE6, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
		{ "ife7", HW_FENCE_CLIENT_ID_IFE7, HW_FENCE_CLIENT_TYPE_MAX_IFE,
		  0, 1, 0, 0, 0, 0, 0, 0, true },
	};
...
static void _lock(uint64_t *wait)
{
	...
	__asm__(
		...
		"STXR W5, W7, [%x[i_lock]]\n\t" // Atomic Store exclusive BIT0 (lock =
		// 0x1)
...
static void _unlock(struct hw_fence_driver_data *drv_data, uint64_t *lock)
{
     ...
     */
		...
		HWFNC_DBG_LOCK(
			"triggering ipc to unblock SVM lock_val:%d cnt:%llu\n",
			lock_val, drv_data->debugfs_data.lock_wake_cnt);
#endif
		hw_fence_ipcc_trigger_signal(drv_data,
					     drv_data->ipcc_client_pid,
					     drv_data->ipcc_client_vid,
					     30); /* Trigger APPS Signal 30 */
	}
}

void global_atomic_store(struct hw_fence_driver_data *drv_data, uint64_t *lock,
			 bool val)
{
	if (val) {
		preempt_disable();
...
int hw_fence_utils_fence_error_cb(struct msm_hw_fence_client *hw_fence_client,
				  ...
				  u32 error)
{
	...
	HWFNC_DBG_L(
		"invoking cb for client:%d ctx:%llu seq:%llu flags:%llu e:%u "
		"data:0x%pK\n",
...
static int
_process_fence_error_client_loopback(struct hw_fence_driver_data *drv_data,
				     int db_flag_id)
{
	...
	for (i = 0; read && i < HW_FENCE_MAX_ITER_READ; i++) {
		read = hw_fence_read_queue_helper(
			&drv_data->ctrl_queues[HW_FENCE_RX_QUEUE - 1],
			&payload);
		if (read < 0) {
			HWFNC_DBG_Q(
				"unable to read ctrl rxq for db_flag_id:%d\n",
				db_flag_id);
			return read;
		}
		if (payload.type != HW_FENCE_PAYLOAD_TYPE_2) {
			HWFNC_ERR(
				"unsupported payload type in ctrl rxq received:%u expected:%u\n",
		...
		if (payload.client_data < HW_FENCE_CLIENT_ID_CTX0 ||
			...
			HWFNC_ERR(
				"read invalid client_id:%llu from ctrl rxq min:%u max:%u\n",
		...
		HWFNC_DBG_Q(
			"ctrl rxq rd: it:%d h:%llu ctx:%llu seq:%llu f:%llu e:%u client:%u\n",
			i, payload.hash, payload.ctxt_id, payload.seqno,
			payload.flags, payload.error, client_id);

		hw_fence_client = drv_data->clients[client_id];
		if (!hw_fence_client) {
			HWFNC_ERR(
				"processing fence error cb for unregistered client_id:%u\n",
				client_id);
			ret = -EINVAL;
			continue;
		}

		cb_ret = hw_fence_utils_fence_error_cb(
			hw_fence_client, payload.ctxt_id, payload.seqno,
			payload.hash, payload.flags, payload.error);
		if (cb_ret) {
			HWFNC_ERR(
				"fence_error_cb failed for client:%u ctx:%llu seq:%llu err:%u\n",
...
static int _process_doorbell_id(struct hw_fence_driver_data *drv_data,
				int db_flag_id)
{
	int ret;
	...
	case HW_FENCE_CTRL_QUEUE_DOORBELL:
		ret = _process_fence_error_client_loopback(drv_data,
							   db_flag_id);
		break;
#if IS_ENABLED(CONFIG_DEBUG_FS)
...
void hw_fence_utils_process_doorbell_mask(struct hw_fence_driver_data *drv_data,
					  u64 db_flags)
{
	...
	for (; db_flag_id <= HW_FENCE_DOORBELL_FLAGS_ID_LAST; db_flag_id++) {
		...
		if (mask & db_flags) {
			...
			HWFNC_DBG_H(
				"db_flag_id:%d cleared flags:0x%llx mask:0x%llx ~mask:0x%llx\n",
...
static void _hw_fence_cb(int irq, void *data)
{
	struct hw_fence_driver_data *drv_data =
		(struct hw_fence_driver_data *)data;
	...
	HWFNC_DBG_IRQ("db callback label:%d irq:%d flags:0x%llx qtime:%llu\n",
		      drv_data->db_label, irq, clear_flags,
...
int hw_fence_utils_init_virq(struct hw_fence_driver_data *drv_data)
{
	...
	ret = of_property_read_u32(node_compat, "gunyah-label",
				   &drv_data->db_label);
	if (ret) {
	...
	drv_data->rx_dbl =
		gh_dbl_rx_register(drv_data->db_label, _hw_fence_cb, drv_data);
...
{
	struct qcom_scm_vmperm src_vmlist[] = { { self, PERM_READ | PERM_WRITE |
								PERM_EXEC } };
	struct qcom_scm_vmperm dst_vmlist[] = {
		{ self, PERM_READ | PERM_WRITE },
	...
	ret = qcom_scm_assign_mem(drv_data->res.start,
				  resource_size(&drv_data->res), &srcvmids,
				  dst_vmlist, ARRAY_SIZE(dst_vmlist));
	if (ret) {
		HWFNC_ERR(
			"%s: qcom_scm_assign_mem failed addr=%x size=%u err=%d\n",
...
#if (KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE)
	ret = ghd_rm_mem_share(GH_RM_MEM_TYPE_NORMAL, 0, drv_data->label, acl,
			       sgl, NULL, &drv_data->memparcel);
#else
	ret = gh_rm_mem_share(GH_RM_MEM_TYPE_NORMAL, 0, drv_data->label, acl,
			      sgl, NULL, &drv_data->memparcel);
#endif
	if (ret) {
		...
		qcom_scm_assign_mem(drv_data->res.start,
				    resource_size(&drv_data->res), &dstvmids,
...
static int hw_fence_rm_cb(struct notifier_block *nb, unsigned long cmd,
			  void *data)
{
	...
	case GH_RM_VM_STATUS_READY:
		...
		if (ret) {
			HWFNC_DBG_INIT("mem not shared ret:%d, attempt share\n",
				       ret);
			if (hw_fence_gunyah_share_mem(drv_data, self_vmid,
						      peer_vmid))
			...
			if (drv_data->res.start == res.start &&
				...
				drv_data->vm_ready = true;
				HWFNC_DBG_INIT(
					"mem_ready: add:0x%x size:%d ret:%d\n",
					res.start, resource_size(&res), ret);
			} else {
				HWFNC_ERR(
					"mem-shared mismatch:[0x%x,%d] expected:[0x%x,%d]\n",
...
int hw_fence_utils_alloc_mem(struct hw_fence_driver_data *drv_data)
{
	...
	if (drv_data->size < drv_data->used_mem_size) {
		HWFNC_ERR(
			"0x%x size of carved-out memory region is less than required "
			"size:0x%x\n",
	...
	HWFNC_DBG_INIT(
		"io_mem_base:0x%x start:0x%x end:0x%x size:0x%x name:%s\n",
		drv_data->io_mem_base, drv_data->res.start, drv_data->res.end,
		drv_data->size, drv_data->res.name);

	memset_io(drv_data->io_mem_base, 0x0, drv_data->size);

	/* Register memory with HYP */
	ret = of_property_read_u32(node_compat, "peer-name",
				   &drv_data->peer_name);
	if (ret)
...
int hw_fence_utils_reserve_mem(struct hw_fence_driver_data *drv_data,
			       enum hw_fence_mem_reserve type,
			       phys_addr_t *phys, void **pa, u32 *size,
			       int client_id)
{
	int ret = 0;
	u32 start_offset = 0;
	...
	case HW_FENCE_MEM_RESERVE_CLIENT_QUEUE:
		if (client_id >= drv_data->clients_num ||
			...
			HWFNC_ERR(
				"unexpected client_id:%d for clients_num:%lu\n",
				client_id, drv_data->clients_num);
			ret = -EINVAL;
			goto exit;
		}

		start_offset = drv_data->hw_fence_client_queue_size[client_id]
				       .start_offset;
		*size = drv_data->hw_fence_client_queue_size[client_id]
				.type->mem_size;
		break;
	case HW_FENCE_MEM_RESERVE_EVENTS_BUFF:
		...
		if (start_offset >= drv_data->size ||
			...
			HWFNC_DBG_INFO(
				"no space for events total_sz:%lu offset:%lu evt_sz:%lu\n",
				drv_data->size, start_offset,
				sizeof(struct msm_hw_fence_event));
			ret = -ENOMEM;
			goto exit;
		}

		total_events = remaining_size_bytes /
			       sizeof(struct msm_hw_fence_event);
	...
	HWFNC_DBG_INIT(
		"type:%s (%d) io_mem_base:0x%x start:0x%x start_offset:%lu size:0x%x\n",
		_get_mem_reserve_type(type), type, drv_data->io_mem_base,
		drv_data->res.start, start_offset, *size);
...
static int
_parse_client_queue_dt_props_extra(struct hw_fence_driver_data *drv_data,
				   struct hw_fence_client_type_desc *desc)
{
	u32 max_idx_from_zero,
		payload_size_u32 = HW_FENCE_CLIENT_QUEUE_PAYLOAD / sizeof(u32);
	char name[40];
	u32 tmp[4];
	bool idx_by_payload = false;
	int count, ret;

	snprintf(name, sizeof(name), "qcom,hw-fence-client-type-%s-extra",
		 desc->name);
	...
	ret = of_property_read_u32_array(drv_data->dev->of_node, name, tmp,
					 count);
	if (ret) {
		HWFNC_ERR(
			"Failed to read %s extra dt properties ret=%d count=%d\n",
			desc->name, ret, count);
		ret = -EINVAL;
		goto exit;
	}
	...
	if (desc->start_padding % sizeof(u32) ||
	    desc->end_padding % sizeof(u32) ||
	    (desc->start_padding + desc->end_padding) % sizeof(u64)) {
		HWFNC_ERR(
			"%s start_padding:%lu end_padding:%lu violates mem alignment\n",
			desc->name, desc->start_padding, desc->end_padding);
		ret = -EINVAL;
		goto exit;
	}

	if (desc->start_padding >=
	    U32_MAX - HW_FENCE_HFI_CLIENT_HEADERS_SIZE(desc->queues_num)) {
		HWFNC_ERR(
			"%s client queues_num:%lu start_padding:%lu will overflow mem_size\n",
	...
	if (desc->end_padding >=
	    U32_MAX - HW_FENCE_HFI_CLIENT_HEADERS_SIZE(desc->queues_num) -
		    desc->start_padding) {
		HWFNC_ERR(
			"%s client q_num:%lu start_p:%lu end_p:%lu will overflow mem_size\n",
			desc->name, desc->queues_num, desc->start_padding,
			desc->end_padding);
		ret = -EINVAL;
		goto exit;
	...
	max_idx_from_zero = idx_by_payload ?
				    desc->queue_entries :
				    desc->queue_entries * payload_size_u32;
	if (desc->txq_idx_start >= U32_MAX - max_idx_from_zero) {
		HWFNC_ERR(
			"%s txq_idx start:%lu by_payload:%s q_entries:%d will overflow "
			"txq_idx\n",
			desc->name, desc->txq_idx_start,
			idx_by_payload ? "true" : "false", desc->queue_entries);
		ret = -EINVAL;
		goto exit;
	}

	HWFNC_DBG_INIT(
		"%s: start_p=%lu end_p=%lu txq_idx_start:%lu txq_idx_by_payload:%s\n",
		desc->name, desc->start_padding, desc->end_padding,
		desc->txq_idx_start, idx_by_payload ? "true" : "false");
...
static int
_parse_client_queue_dt_props_indv(struct hw_fence_driver_data *drv_data,
				  struct hw_fence_client_type_desc *desc)
{
	char name[31];
	u32 tmp[4];
	...
	snprintf(name, sizeof(name), "qcom,hw-fence-client-type-%s",
		 desc->name);
	ret = of_property_read_u32_array(drv_data->dev->of_node, name, tmp, 4);
	if (ret) {
		HWFNC_DBG_INIT(
			"missing %s client queue entry or invalid ret:%d\n",
			desc->name, ret);
	...
	if (desc->clients_num > desc->max_clients_num || !desc->queues_num ||
		...
		HWFNC_ERR(
			"%s invalid dt: clients_num:%lu queues_num:%lu, queue_entries:%lu\n",
	...
	if (desc->queue_entries >= U32_MAX / HW_FENCE_CLIENT_QUEUE_PAYLOAD) {
		HWFNC_ERR(
			"%s client queue entries:%lu will overflow client queue size\n",
	...
	if (queue_size >= ((U32_MAX & PAGE_MASK) -
			   (HW_FENCE_HFI_CLIENT_HEADERS_SIZE(desc->queues_num) +
			    desc->start_padding + desc->end_padding)) /
				  desc->queues_num) {
		HWFNC_ERR(
			"%s client queue_sz:%lu start_p:%lu end_p:%lu will overflow mem size\n",
			desc->name, queue_size, desc->start_padding,
			desc->end_padding);
		return -EINVAL;
	}

	desc->mem_size =
		PAGE_ALIGN(HW_FENCE_HFI_CLIENT_HEADERS_SIZE(desc->queues_num) +
			   (queue_size * desc->queues_num) +
			   desc->start_padding + desc->end_padding);

	if (desc->mem_size > MAX_CLIENT_QUEUE_MEM_SIZE) {
		HWFNC_ERR(
			"%s client queue mem_size:%lu greater than max mem size:%lu\n",
			desc->name, desc->mem_size, MAX_CLIENT_QUEUE_MEM_SIZE);
		return -EINVAL;
	}

	HWFNC_DBG_INIT(
		"%s: clients=%lu q_num=%lu q_entries=%lu mem_sz=%lu skips_wr_ptr:%s\n",
		desc->name, desc->clients_num, desc->queues_num,
		desc->queue_entries, desc->mem_size,
		desc->skip_txq_wr_idx ? "true" : "false");
...
static int _parse_client_queue_dt_props(struct hw_fence_driver_data *drv_data)
{
	...
	for (i = 0; i < HW_FENCE_MAX_CLIENT_TYPE; i++) {
		...
		if (ret) {
			HWFNC_ERR(
				"failed to initialize %s client queue size properties\n",
				desc->name);
			return ret;
		}
	...
	/* clients and size desc are allocated for all static clients regardless of
   * device-tree */
	drv_data->clients_num =
		HW_FENCE_MAX_STATIC_CLIENTS_INDEX + configurable_clients_num;

	/* allocate memory for client queue size descriptors */
	size = drv_data->clients_num *
	       sizeof(struct hw_fence_client_queue_desc);
	...
	start_offset =
		PAGE_ALIGN(drv_data->hw_fence_mem_ctrl_queues_size +
			   HW_FENCE_MEM_LOCKS_SIZE(drv_data->rxq_clients_num) +
	...
	for (i = 0; i < HW_FENCE_MAX_CLIENT_TYPE; i++) {
		...
		for (j = 0; j < desc->clients_num; j++) {
			...
			enum hw_fence_client_id client_id =
				hw_fence_utils_get_client_id_priv(
					drv_data, client_id_ext);
			...
			HWFNC_DBG_INIT(
				"%s client_id_ext:%lu client_id:%lu start_offset:%lu\n",
				desc->name, client_id_ext, client_id,
				start_offset);
...
int hw_fence_utils_parse_dt_props(struct hw_fence_driver_data *drv_data)
{
	...
	ret = of_property_read_u32(drv_data->dev->of_node,
				   "qcom,hw-fence-table-entries", &val);
	if (ret || !val) {
		HWFNC_ERR(
			"missing hw fences table entry or invalid ret:%d val:%d\n",
			ret, val);
		return ret;
	}
	drv_data->hw_fence_table_entries = val;

	if (drv_data->hw_fence_table_entries >=
	    U32_MAX / sizeof(struct msm_hw_fence)) {
	...
	ret = of_property_read_u32(drv_data->dev->of_node,
				   "qcom,hw-fence-queue-entries", &val);
	if (ret || !val) {
		HWFNC_ERR(
			"missing queue entries table entry or invalid ret:%d val:%d\n",
			ret, val);
		return ret;
	}
	...
	if (drv_data->hw_fence_queue_entries >=
	    U32_MAX / HW_FENCE_CTRL_QUEUE_PAYLOAD) {
		HWFNC_ERR("queue entries:%lu will overflow ctrl queue size\n",
			  drv_data->hw_fence_queue_entries);
		return -EINVAL;
	}
	drv_data->hw_fence_ctrl_queue_size =
		HW_FENCE_CTRL_QUEUE_PAYLOAD * drv_data->hw_fence_queue_entries;

	if (drv_data->hw_fence_ctrl_queue_size >=
	    (U32_MAX - HW_FENCE_HFI_CTRL_HEADERS_SIZE) / HW_FENCE_CTRL_QUEUES) {
		HWFNC_ERR("queue size:%lu will overflow ctrl queue mem size\n",
			  drv_data->hw_fence_ctrl_queue_size);
		return -EINVAL;
	}
	drv_data->hw_fence_mem_ctrl_queues_size =
		HW_FENCE_HFI_CTRL_HEADERS_SIZE +
	...
	HWFNC_DBG_INIT("clients_num: %lu, total_mem_size:%lu\n",
		       drv_data->clients_num, drv_data->used_mem_size);
...
int hw_fence_utils_map_ipcc(struct hw_fence_driver_data *drv_data)
{
	...
	ret = of_property_read_u32_array(drv_data->dev->of_node,
					 "qcom,ipcc-reg", reg_config, 2);
	if (ret) {
	...
	/* Mmap ipcc registers */
	ptr = devm_ioremap(drv_data->dev, drv_data->ipcc_reg_base,
			   drv_data->ipcc_size);
	if (!ptr) {
...
int hw_fence_utils_map_qtime(struct hw_fence_driver_data *drv_data)
{
	...
	ptr = devm_ioremap(drv_data->dev, drv_data->qtime_reg_base,
			   drv_data->qtime_size);
	if (!ptr) {
...
hw_fence_utils_get_client_id_priv(struct hw_fence_driver_data *drv_data,
				  enum hw_fence_client_id client_id)
{
	...
	/* consolidate external 'hw_fence_client_id' enum into consecutive internal
   * client IDs */
	...
	for (i = HW_FENCE_MAX_CLIENT_TYPE_STATIC; i < client_type; i++)
		client_id_priv +=
			drv_data->hw_fence_client_types[i].clients_num;

	return client_id_priv;
}

int hw_fence_utils_get_queues_num(struct hw_fence_driver_data *drv_data,
				  int client_id)
{
