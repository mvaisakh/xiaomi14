...
#define HFENCE_TBL_MSG                                                        \
	"[%d]hfence[%d] v:%d err:%lu ctx:%llu seq:%llu wait:0x%llx alloc:%d " \
	"f:0x%llx child_cnt:%d"                                               \
...
#define HW_FENCE_MAX_PARENTS_DUMP \
	(sizeof("parent_list[] ") + HW_FENCE_MAX_PARENTS_SUBLIST_DUMP)
 ...
 * @seqno_cnt: sequence number, this is a counter to simulate the seqno for
 * debugging.
 * @client_handle: handle for the client, this is returned by the hw-fence
 * driver after a successful registration of the client.
 * @mem_descriptor: memory descriptor for the client-queues. This is populated
 * by the hw-fence driver after a successful registration of the client.
 * @list: client node.
 */
struct client_data {
	int client_id;
				     ...
				     struct hw_fence_driver_data **drv_data)
{
	...
	*drv_data = file->private_data;
	...
	if (client_id < HW_FENCE_CLIENT_ID_CTX0 ||
	    client_id >= HW_FENCE_CLIENT_MAX) {
...
static int _debugfs_ipcc_trigger(struct file *file, const char __user *user_buf,
				 size_t count, loff_t *ppos, u32 tx_client,
				 u32 rx_client)
{
	struct hw_fence_driver_data *drv_data;
	...
	HWFNC_DBG_IRQ("client_id:%d ipcc write tx_client:%d rx_client:%d "
		      "signal_id:%d qtime:%llu\n",
		      client_id, tx_client, rx_client, signal_id,
 ...
 *
 * This debugfs receives as parameter a hw-fence driver client_id, and triggers
 * an ipcc signal from apps to apps for that client id.
 */
static ssize_t hw_fence_dbg_ipcc_write(struct file *file,
				       const char __user *user_buf,
				       size_t count, loff_t *ppos)
{
	struct hw_fence_driver_data *drv_data = file->private_data;

	return _debugfs_ipcc_trigger(file, user_buf, count, ppos,
				     drv_data->ipcc_client_pid,
				     drv_data->ipcc_client_vid);
}

/**
 * hw_fence_dbg_ipcc_dpu_write() - debugfs write to trigger an ipcc irq to dpu
 * core.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @count: size of the user buffer.
 ...
 *
 * This debugfs receives as parameter a hw-fence driver client_id, and triggers
 * an ipcc signal from apps to dpu for that client id.
 */
static ssize_t hw_fence_dbg_ipcc_dpu_write(struct file *file,
					   const char __user *user_buf,
					   size_t count, loff_t *ppos)
{
	struct hw_fence_driver_data *drv_data = file->private_data;

	return _debugfs_ipcc_trigger(
		file, user_buf, count, ppos, drv_data->ipcc_client_pid,
		hw_fence_ipcc_get_client_virt_id(drv_data,
...
struct client_data *_get_client_node(struct hw_fence_driver_data *drv_data,
				     u32 client_id)
{
...
/**
 * hw_fence_dbg_reset_client_wr() - debugfs write to trigger reset in a debug
 * hw-fence client.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @count: size of the user buffer.
 ...
 *
 * This debugfs receives as parameter a hw-fence driver client_id, and triggers
 * a reset for this client. Note that this operation will only perform on
 * hw-fence clients created through the debug framework.
 */
...
/**
 * hw_fence_dbg_register_clients_wr() - debugfs write to register a client with
 * the hw-fence driver for debugging.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @count: size of the user buffer.
 * @ppos: position offset of the user buffer.
 *
 * This debugfs receives as parameter a hw-fence driver client_id to register
 * for debug. Note that if the client_id received was already registered by any
 * other driver, the registration here will fail.
 */
						...
						size_t count, loff_t *ppos)
{
	...
	client_info->client_handle =
		msm_hw_fence_register(client_id, &client_info->mem_descriptor);
...
/**
 * hw_fence_dbg_tx_and_signal_clients_wr() - debugfs write to simulate the
 * lifecycle of a hw-fence.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @count: size of the user buffer.
 * @ppos: position offset of the user buffer.
 *
 * This debugfs receives as parameter the number of iterations that the
 * simulation will run, each iteration will: create, signal, register-for-signal
 * and destroy a hw-fence. Note that this simulation relies in the user first
 * registering the clients as debug-clients through the debugfs
 * 'hw_fence_dbg_register_clients_wr'. If the clients are not previously
 * registered as debug-clients, this simulation will fail and won't run.
 */
static ssize_t
hw_fence_dbg_tx_and_signal_clients_wr(struct file *file,
				      const char __user *user_buf, size_t count,
				      loff_t *ppos)
{
	...
	if (input_data <= 0) {
		HWFNC_ERR(
			"won't do anything, write value greather than 0 to start..\n");
		return 0;
	...
	if (!client_info_src ||
	    IS_ERR_OR_NULL(client_info_src->client_handle) ||
	    !client_info_dst ||
	    IS_ERR_OR_NULL(client_info_dst->client_handle)) {
		/* Make sure we registered this client through debugfs */
		HWFNC_ERR(
			"client_id_src:%d or client_id_dst:%d not registered as debug "
			"client!\n",
	...
	hw_fence_client =
		(struct msm_hw_fence_client *)client_info_src->client_handle;
	hw_fence_client_dst =
		(struct msm_hw_fence_client *)client_info_dst->client_handle;
	...
	while (drv_data->debugfs_data.create_hw_fences && input_data > 0) {
		...
		ret = hw_fence_create(drv_data, hw_fence_client, context, seqno,
				      &hash);
		if (ret) {
		...
		/* Write to Tx queue */
		hw_fence_update_queue(
			drv_data, hw_fence_client, context, seqno, hash, 0, 0,
			0, HW_FENCE_TX_QUEUE - 1); /* no flags and no error */

		/**********************************************/
		/***** DST CLIENT - REGISTER WAIT CLIENT ******/
		/**********************************************/
		/* use same context and seqno that src client used to create fence */
		ret = hw_fence_register_wait_client(drv_data, NULL,
						    hw_fence_client_dst,
						    context, seqno, &hash, 0);
		if (ret) {
		...
		/* AFTER THIS IS WHEN SVM WILL GET CALLED AND WILL PROCESS SRC AND DST
     * CLIENTS */

		/* Trigger IPCC for SVM to read the queue */

		/* Get signal-id that hw-fence driver would trigger for this client */
		signal_id = dbg_out_clients_signal_map_no_dpu[client_id_src]
				    .ipc_signal_id;
		if (signal_id < 0)
			return -EINVAL;
		...
		HWFNC_DBG_IRQ(
			"client:%d tx_client:%d rx_client:%d signal:%d delay:%d in_data%d\n",
			client_id_src, tx_client, rx_client, signal_id,
			drv_data->debugfs_data.hw_fence_sim_release_delay,
			input_data);

		hw_fence_ipcc_trigger_signal(drv_data, tx_client, rx_client,
					     signal_id);
		...
		ret = hw_fence_destroy(drv_data, hw_fence_client, context,
				       seqno);
		if (ret) {
...
/**
 * hw_fence_dbg_create_wr() - debugfs write to simulate the creation of a
 * hw-fence.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @count: size of the user buffer.
 * @ppos: position offset of the user buffer.
 *
 * This debugfs receives as parameter the client-id, for which the hw-fence will
 * be created. Note that this simulation relies in the user first registering
 * the client as a debug-client through the debugfs
 * 'hw_fence_dbg_register_clients_wr'. If the client is not previously
				      ...
				      loff_t *ppos)
{
	...
	snprintf(dma_fence->name, HW_FENCE_NAME_SIZE,
		 "hwfence:id:%d:ctx=%lu:seqno:%lu", client_id,
	...
	if (ret) {
		HWFNC_ERR(
			"failed to create hw_fence for client:%d ctx:%llu seqno:%llu\n",
			client_id, client_info->dma_context,
			hw_fence_dbg_seqno);
...
static void _dump_fence_helper(enum hw_fence_drv_prio prio,
			       ...
			       char *parents_dump, u64 hash, u32 count)
{
	...
	if (hw_fence->parents_cnt) {
		if (hw_fence->parents_cnt > MSM_HW_FENCE_MAX_JOIN_PARENTS) {
			HWFNC_ERR(
				"hfence[%d] has invalid parents_cnt:%d greater than max:%d\n",
				hash, hw_fence->parents_cnt,
		...
		for (i = 0; i < parents_cnt; i++)
			len += scnprintf(sublist + len,
					 HW_FENCE_MAX_PARENTS_SUBLIST_DUMP -
						 len,
					 "%lu,", hw_fence->parent_list[i]);
		scnprintf(parents_dump, HW_FENCE_MAX_PARENTS_DUMP, " p:[%s]",
			  sublist);
	}

	HWFNC_DBG_DUMP(prio, HFENCE_TBL_MSG, count, hash, hw_fence->valid,
		       hw_fence->error, hw_fence->ctx_id, hw_fence->seq_id,
		       hw_fence->wait_client_mask, hw_fence->fence_allocator,
		       hw_fence->flags, hw_fence->pending_child_cnt,
		       parents_dump, hw_fence->fence_create_time,
		       hw_fence->fence_trigger_time, hw_fence->fence_wait_time);
}

void hw_fence_debug_dump_fence(enum hw_fence_drv_prio prio,
...
static inline int _dump_fence(struct msm_hw_fence *hw_fence, char *buf, int len,
			      int max_size, u32 index, u32 cnt)
{
	int ret;
	char parents_dump[HW_FENCE_MAX_PARENTS_DUMP];
	...
	ret = scnprintf(buf + len, max_size - len, HFENCE_TBL_MSG, cnt, index,
			hw_fence->valid, hw_fence->error, hw_fence->ctx_id,
			hw_fence->seq_id, hw_fence->wait_client_mask,
			hw_fence->fence_allocator, hw_fence->flags,
			hw_fence->pending_child_cnt, parents_dump,
			hw_fence->fence_create_time,
			hw_fence->fence_trigger_time,
			hw_fence->fence_wait_time);

	return ret;
}

void hw_fence_debug_dump_table(enum hw_fence_drv_prio prio,
			       struct hw_fence_driver_data *drv_data)
{
	u32 i, cnt = 0;
	struct msm_hw_fence *hw_fence;
...
static int dump_single_entry(struct hw_fence_driver_data *drv_data, char *buf,
			     u32 *index, int max_size)
{
	struct msm_hw_fence *hw_fence;
...
static int dump_full_table(struct hw_fence_driver_data *drv_data, char *buf,
			   u32 *index, u32 *cnt, int max_size, int entry_size)
{
	struct msm_hw_fence *hw_fence;
	int len = 0;

	while (((*index)++ < drv_data->hw_fences_tbl_cnt) &&
...
static void _find_earliest_event(struct hw_fence_driver_data *drv_data,
				 u32 *start_index, u64 *start_time)
{
	u32 i;
...
static void _dump_event(enum hw_fence_drv_prio prio,
			struct msm_hw_fence_event *event, char *data, u32 index)
{
	u32 data_cnt;
	int i, len = 0;
	...
	if (event->data_cnt > HW_FENCE_EVENT_MAX_DATA) {
		HWFNC_ERR(
			"event[%d] has invalid data_cnt:%lu greater than max_data_cnt:%lu\n",
	...
	for (i = 0; i < data_cnt; i++)
		len += scnprintf(data + len,
				 HW_FENCE_MAX_DATA_PER_EVENT_DUMP - len, "%lx|",
				 event->data[i]);

	HWFNC_DBG_DUMP(prio, HFENCE_EVT_MSG, index, event->cpu, event->time,
		       event->data_cnt, data);
}

void hw_fence_debug_dump_events(enum hw_fence_drv_prio prio,
				struct hw_fence_driver_data *drv_data)
{
	...
	for (i = start_index;
	     i < drv_data->total_events && drv_data->events[i].time; i++)
 ...
 * @ppos: position offset of the user buffer.
 */
static ssize_t hw_fence_dbg_dump_events_rd(struct file *file,
					   ...
					   size_t user_buf_size, loff_t *ppos)
{
	...
	if (!start_time) {
		...
		index = start_index;
		HWFNC_DBG_H(
			"events:0x%pK start_index:%d start_time:%llu total_events:%d\n",
			drv_data->events, start_index, start_time,
			drv_data->total_events);
	}
	...
	while ((!wraparound || index < start_index) &&
		...
		if (drv_data->events[index].time) {
			_dump_event(HW_FENCE_INFO, &drv_data->events[index],
				    data, index);
			len += scnprintf(buf + len, max_size - len,
					 HFENCE_EVT_MSG, index,
					 drv_data->events[index].cpu,
					 drv_data->events[index].time,
					 drv_data->events[index].data_cnt,
					 data);
		}
...
static void _dump_queue(enum hw_fence_drv_prio prio,
			...
			int queue_type)
{
	...
	if ((queue_type > hw_fence_client->queues_num) || !queue ||
	    !queue->va_header || !queue->va_queue) {
		HWFNC_ERR(
			"Cannot dump client:%d q_type:%s q_ptr:0x%pK q_header:0x%pK "
			"q_va:0x%pK\n",
			hw_fence_client->client_id,
			(queue_type == HW_FENCE_TX_QUEUE) ? "TX QUEUE" :
							    "RX QUEUE",
			queue, queue ? queue->va_header : NULL,
			queue ? queue->va_queue : NULL);
		return;
	}
	...
	HWFNC_DBG_DUMP(
		prio,
		"%s va:0x%pK rd_idx:%lu wr_idx:%lu tx_wm:%lu q_size_bytes:%lu\n",
		(queue_type == HW_FENCE_TX_QUEUE) ? "TX QUEUE" : "RX QUEUE",
		queue->va_queue, hfi_header->read_index,
		hfi_header->write_index, hfi_header->tx_wm,
		queue->q_size_bytes);
	queue_entries = queue->q_size_bytes / HW_FENCE_CLIENT_QUEUE_PAYLOAD;

	for (i = 0; i < queue_entries; i++) {
		read_ptr = ((u32 *)queue->va_queue +
			    (i * (sizeof(struct msm_hw_fence_queue_payload) /
				  sizeof(u32))));
		payload = (struct msm_hw_fence_queue_payload *)read_ptr;
		timestamp = (u64)payload->timestamp_lo |
			    ((u64)payload->timestamp_hi << 32);

		HWFNC_DBG_DUMP(
			prio,
			"%s[%d]: hash:%d ctx:%llu seqno:%llu f:%llu d:%llu err:%u time:%llu\n",
			(queue_type == HW_FENCE_TX_QUEUE) ? "tx" : "rx", i,
			payload->hash, payload->ctxt_id, payload->seqno,
			payload->flags, payload->client_data, payload->error,
			timestamp);
	}
}
 ...
 * @ppos: position offset of the user buffer.
 *
 * This debugfs dumps the hw-fence queues. Takes as input the desired client to
 * dump. Dumps to debug msgs the contents of the TX and RX queues for that
 ...
 * @ppos: position offset of the user buffer.
 *
 * This debugfs dumps the hw-fence table. By default debugfs will dump all the
 * valid entries of the whole table. However, if user only wants to dump only
 * one particular entry, user can provide the context-id and seqno of the
 * dma-fence of interest by writing to this debugfs node (see documentation for
 * the write in 'hw_fence_dbg_dump_table_wr').
 */
static ssize_t hw_fence_dbg_dump_table_rd(struct file *file,
					  char __user *user_buf,
					  size_t user_buf_size, loff_t *ppos)
{
	struct hw_fence_driver_data *drv_data;
	...
	len = drv_data->debugfs_data.entry_rd ?
		      ...
		      dump_full_table(drv_data, buf, &index, &cnt, max_size,
				      entry_size);
...
/**
 * hw_fence_dbg_dump_table_wr() - debugfs write to control the dump of the
 * hw-fences table.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @user_buf_size: size of the user buffer.
 * @ppos: position offset of the user buffer.
 *
 * This debugfs receives as parameters the settings to dump either the whole
 * hw-fences table or only one element on the table in the next read of the same
 * debugfs node. If this debugfs receives two input values, it will interpret
 * them as the 'context-id' and the 'sequence-id' to dump from the hw-fence
 * table in the subsequent reads of the debugfs. Otherwise, if the debugfs
 * receives only one input value, the next read from the debugfs, will dump the
 * whole hw-fences table.
 */
...
/**
 * hw_fence_dbg_create_join_fence() - debugfs write to simulate the lifecycle of
 * a join hw-fence.
 * @file: file handler.
 * @user_buf: user buffer content from debugfs.
 * @count: size of the user buffer.
 * @ppos: position offset of the user buffer.
 *
 * This debugfs will: create, signal, register-for-signal and destroy a join
 * hw-fence. Note that this simulation relies in the user first registering the
 * clients as debug-clients through the debugfs
 * 'hw_fence_dbg_register_clients_wr'. If the clients are not previously
 * registered as debug-clients, this simulation will fail and won't run.
 */
					      ...
					      size_t count, loff_t *ppos)
{
	...
	if (!client_info_src ||
	    ...
	    IS_ERR_OR_NULL(client_info_dst->client_handle)) {
		HWFNC_ERR(
			"client_src:%d or client:%d is not register as debug client\n",
			client_id_src, client_id_dst);
		return -EINVAL;
	}
	hw_fence_client =
	...
	for (i = 0; i < num_fences; i++) {
		...
		ret = msm_hw_fence_create(client_info_src->client_handle,
					  &params);
		if (ret) {
			HWFNC_ERR("Error creating HW fence\n");
			count = -EINVAL;
	...
	msm_hw_fence_wait_update_v2(client_info_dst->client_handle,
				    &fence_array_fence, NULL, NULL, 1, 1);

	signal_id =
		dbg_out_clients_signal_map_no_dpu[client_id_src].ipc_signal_id;
	if (signal_id < 0) {
		count = -EINVAL;
		goto error;
	}
...
error:
	...
	/*
   * free array of pointers, no need to call kfree in 'fences', since that is
   * released from the fence-array release api
   */
	kfree(fences_lock);
				       ...
				       int client_id)
{
	...
	if (client_id < HW_FENCE_CLIENT_ID_VAL0 ||
...
int hw_fence_debug_debugfs_register(struct hw_fence_driver_data *drv_data)
{
	...
	debugfs_create_file("hw_fence_reset_client", 0600, debugfs_root,
			    drv_data, &hw_fence_reset_client_fops);
	debugfs_create_file("hw_fence_register_clients", 0600, debugfs_root,
			    drv_data, &hw_fence_register_clients_fops);
	debugfs_create_file("hw_fence_tx_and_signal", 0600, debugfs_root,
			    drv_data, &hw_fence_tx_and_signal_clients_fops);
	debugfs_create_file("hw_fence_create_join_fence", 0600, debugfs_root,
			    drv_data, &hw_fence_create_join_fence_fops);
	...
	debugfs_create_u32("hw_fence_debug_level", 0600, debugfs_root,
			   &msm_hw_fence_debug_level);
	debugfs_create_file("hw_fence_dump_table", 0600, debugfs_root, drv_data,
			    &hw_fence_dump_table_fops);
	debugfs_create_file("hw_fence_dump_queues", 0600, debugfs_root,
			    drv_data, &hw_fence_dump_queues_fops);
	debugfs_create_file("hw_sync", 0600, debugfs_root, NULL,
			    &hw_sync_debugfs_fops);
	debugfs_create_u64("hw_fence_lock_wake_cnt", 0600, debugfs_root,
			   &drv_data->debugfs_data.lock_wake_cnt);
	debugfs_create_file("hw_fence_dump_events", 0600, debugfs_root,
			    drv_data, &hw_fence_dump_events_fops);
