...
#define HW_SYNC_IOC_CREATE_FENCE \
	_IOWR(HW_SYNC_IOC_MAGIC, 12, struct hw_fence_sync_create_data)
#define HW_SYNC_IOC_CREATE_FENCE_ARRAY \
	_IOWR(HW_SYNC_IOC_MAGIC, 14, struct hw_fence_array_sync_create_data)
...
#define HW_IOCTL_DEF(ioctl, _func) \
	[HW_FENCE_IOCTL_NR(        \
		ioctl)] = { .cmd = ioctl, .func = _func, .name = #ioctl }
...
/**
 * struct hw_fence_array_sync_create_data - data used in creating multiple
 * fences.
...
static bool _is_valid_client(struct hw_sync_obj *obj)
{
	...
	if (obj->client_id < HW_FENCE_CLIENT_ID_VAL0 ||
	    obj->client_id > HW_FENCE_CLIENT_ID_VAL6) {
...
static long hw_sync_ioctl_unreg_client(struct hw_sync_obj *obj,
				       unsigned long arg)
{
	int client_id = _get_client_id(obj, arg);
...
static long hw_sync_ioctl_create_fence(struct hw_sync_obj *obj,
				       unsigned long arg)
{
	...
	snprintf(fence->name, HW_FENCE_NAME_SIZE,
		 "hwfence:id:%d:ctx=%lu:seqno:%lu", obj->client_id,
		 obj->context, data.seqno);

	spin_lock_init(fence_lock);
	dma_fence_init(&fence->base, &hw_fence_dbg_ops, fence_lock,
		       obj->context, data.seqno);
...
static long hw_sync_ioctl_create_fence_array(struct hw_sync_obj *obj,
					     unsigned long arg)
{
	...
	if (num_fences > HW_FENCE_ARRAY_SIZE) {
		HWFNC_ERR(
			"Number of fences: %d is greater than allowed size: %d\n",
	...
	fence_array = dma_fence_array_create(num_fences, fences, obj->context,
					     data.seqno, 0);
	if (!fence_array) {
 ...
 */
static long hw_sync_ioctl_reg_for_wait(struct hw_sync_obj *obj,
				       unsigned long arg)
{
	...
	ret = msm_hw_fence_wait_update_v2(obj->client_handle, &fence, NULL,
					  NULL, num_fences, 1);
...
static long hw_sync_ioctl_fence_signal(struct hw_sync_obj *obj,
				       unsigned long arg)
{
	...
	ret = msm_hw_fence_update_txq(obj->client_handle, data.hash, 0,
				      data.error_flag);
	if (ret) {
	...
	signal_id =
		dbg_out_clients_signal_map_no_dpu[obj->client_id].ipc_signal_id;
	if (signal_id < 0)
		return -EINVAL;

	tx_client = hw_fence_client->ipc_client_pid;
	rx_client = hw_fence_client->ipc_client_vid;
	ret = msm_hw_fence_trigger_signal(obj->client_handle, tx_client,
					  rx_client, signal_id);
	if (ret) {
...
static long hw_sync_ioctl_fence_wait(struct hw_sync_obj *obj, unsigned long arg)
{
	...
	int fd, ret, read = 1,
		     queue_type = HW_FENCE_RX_QUEUE - 1; /* rx queue index */
	...
	do {
		ret = wait_event_timeout(
			hw_fence_client->wait_queue,
			atomic_read(&hw_fence_client->val_signal) > 0,
			msecs_to_jiffies(data.timeout_ms));
		cur_ktime = ktime_get();
	} while ((atomic_read(&hw_fence_client->val_signal) <= 0) &&
		 (ret == 0) && ktime_compare_safe(exp_ktime, cur_ktime) > 0);
	...
	while (read) {
		read = hw_fence_read_queue(obj->client_handle, &payload,
					   queue_type);
		if (read < 0) {
		...
		if (payload.ctxt_id == fence->context &&
		    payload.seqno == fence->seqno) {
			/* Decrement the refcount that hw_sync_get_fence increments */
			dma_fence_put(fence);
			return 0;
		}
	...
	HWFNC_ERR(
		"fence received: context:%d seqno:%d fence expected: context:%d "
		"seqno:%d\n",
...
static long hw_sync_ioctl_reset_client(struct hw_sync_obj *obj,
				       unsigned long arg)
{
	int ret;
...
static const struct hw_sync_ioctl_def hw_sync_debugfs_ioctls[] = {
	...
	HW_IOCTL_DEF(HW_SYNC_IOC_CREATE_FENCE_ARRAY,
		     hw_sync_ioctl_create_fence_array),
...
static long hw_sync_debugfs_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
	struct hw_sync_obj *obj = file->private_data;
