...
/* Global atomic lock */
#define GLOBAL_ATOMIC_STORE(drv_data, lock, val) \
	global_atomic_store(drv_data, lock, val)
...
#define REQUIRES_IDX_TRANSLATION(queue) \
	((queue)->rd_wr_idx_factor &&   \
	 ((queue)->rd_wr_idx_start || (queue)->rd_wr_idx_factor > 1))
...
static int init_hw_fences_queues(struct hw_fence_driver_data *drv_data,
				 ...
				 struct msm_hw_fence_queue *queues,
				 int queues_num, int client_id)
{
	...
	case HW_FENCE_MEM_RESERVE_CLIENT_QUEUE:
		...
		queue_size =
			HW_FENCE_CLIENT_QUEUE_PAYLOAD * desc->queue_entries;
		payload_size = HW_FENCE_CLIENT_QUEUE_PAYLOAD;
		txq_idx_start = desc->txq_idx_start;
		txq_idx_factor = desc->txq_idx_factor ? desc->txq_idx_factor :
							1;
	...
	ret = hw_fence_utils_reserve_mem(drv_data, mem_reserve_id, &phys, &ptr,
					 &size, client_id);
	if (ret) {
	...
	mem_descriptor->mem_data =
		NULL; /* Currently we don't need any special info */

	HWFNC_DBG_INIT(
		"Initialize headers: headers_size:%d start_padding:%d end_padding:%d\n",
	...
	hfi_table_header->qhdr0_offset =
		HW_FENCE_HFI_TABLE_HEADER_SIZE + start_padding;
   ...
   */
	...
	for (i = 0; i < queues_num; i++) {
		...
		start_queue_offset =
			headers_size + (i * queue_size); /* Bytes */
		qphys = phys +
			start_queue_offset; /* start of the PA for the queue elems */
		qptr = (char *)ptr +
		       start_queue_offset; /* start of the va for queue elems */
		...
		hfi_queue_header->type = IS_HW_FENCE_TX_QUEUE(i) ?
						 HW_FENCE_TX_QUEUE :
						 HW_FENCE_RX_QUEUE;
		...
		HWFNC_DBG_INIT(
			"init:%s client:%d q[%d] va=0x%pK pa=0x%x hd:0x%pK sz:%u pkt:%d\n",
			hfi_queue_header->type == HW_FENCE_TX_QUEUE ?
				"TX_QUEUE" :
				"RX_QUEUE",
			client_id, i, queues[i].va_queue, queues[i].pa_queue,
			queues[i].va_header, queues[i].q_size_bytes,
			payload_size);
		...
		HWFNC_DBG_INIT(
			"rd_wr_idx_start:%lu rd_wr_idx_factor:%lu skip_wr_idx:%s\n",
...
static void
_translate_queue_indexes_custom_to_default(struct msm_hw_fence_queue *queue,
					   u32 *read_idx, u32 *write_idx)
{
	if (REQUIRES_IDX_TRANSLATION(queue)) {
		...
		*write_idx = IDX_TRANSLATE_CUSTOM_TO_DEFAULT(queue, *write_idx);
		HWFNC_DBG_Q(
			"rd_idx_u32:%lu wr_idx_u32:%lu rd_wr_idx start:%lu factor:%lu\n",
			*read_idx, *write_idx, queue->rd_wr_idx_start,
			queue->rd_wr_idx_factor);
	}
}
			...
			int queue_type)
{
	...
	if (queue_type >= HW_FENCE_CLIENT_QUEUES || !hw_fence_client ||
	    !payload) {
		HWFNC_ERR(
			"Invalid queue type:%s hw_fence_client:0x%pK payload:0x%pK\n",
			queue_type, hw_fence_client, payload);
		return -EINVAL;
	}
			       ...
			       struct msm_hw_fence_queue_payload *payload)
{
	...
	payload_size_u32 =
		(sizeof(struct msm_hw_fence_queue_payload) / sizeof(u32));
   ...
   * offset */
	_translate_queue_indexes_custom_to_default(queue, &read_idx,
						   &write_idx);

	HWFNC_DBG_Q(
		"read rd_ptr:0x%pK wr_ptr:0x%pK rd_idx:%d wr_idx:%d queue:0x%pK\n",
		&hfi_header->read_index, &hfi_header->write_index, read_idx,
		write_idx, queue);
	...
	HWFNC_DBG_Q(
		"read_ptr:0x%pK queue: va=0x%pK pa=0x%pK read_ptr_payload:0x%pK\n",
		read_ptr, queue->va_queue, queue->pa_queue, read_ptr_payload);

	/* Calculate the index after the read */
	to_read_idx = read_idx + payload_size_u32;

	/*
   * wrap-around case, here we are reading the last element of the queue,
   * therefore set to_read_idx, which is the index after the read, to the
   ...
   */
	...
	if (REQUIRES_IDX_TRANSLATION(queue)) {
		...
		HWFNC_DBG_Q(
			"translated to_read_idx:%lu rd_wr_idx start:%lu factor:%lu\n",
			to_read_idx, queue->rd_wr_idx_start,
			queue->rd_wr_idx_factor);
	}
...
static int
_get_update_queue_params(struct msm_hw_fence_queue *queue,
			 struct msm_hw_fence_hfi_queue_header **hfi_header,
			 u32 *q_size_u32, u32 *payload_size,
...
int hw_fence_update_queue(struct hw_fence_driver_data *drv_data,
			  ...
			  u64 ctxt_id, u64 seqno, u64 hash, u64 flags,
			  u64 client_data, u32 error, int queue_type)
{
	...
	if (queue_type >= hw_fence_client->queues_num) {
		HWFNC_ERR("Invalid queue type:%d client_id:%d q_num:%lu\n",
			  queue_type, hw_fence_client->client_id,
			  hw_fence_client->queues_num);
		return -EINVAL;
	}

	queue = &hw_fence_client->queues[queue_type];
	if (_get_update_queue_params(queue, &hfi_header, &q_size_u32,
   ...
   */
	...
	HWFNC_DBG_Q(
		"wr client:%d r_ptr:0x%pK w_ptr:0x%pK r_idx:%d w_idx:%d q:0x%pK "
		"type:%d s:%s\n",
   ...
   * offset */
	...
	q_free_u32 = read_idx <= write_idx ?
			     (q_size_u32 - (write_idx - read_idx)) :
			     (read_idx - write_idx);
	...
	write_ptr_payload =
		(struct msm_hw_fence_queue_payload *)q_payload_write_ptr;
	HWFNC_DBG_Q("q_payload_write_ptr:0x%pK queue: va=0x%pK pa=0x%pK "
		    "write_ptr_payload:0x%pK\n",
		    q_payload_write_ptr, queue->va_queue, queue->pa_queue,
		    write_ptr_payload);
	...
	HWFNC_DBG_Q("to_write_idx:%d write_idx:%d payload_size:%u\n",
		    to_write_idx, write_idx, payload_size_u32);
	HWFNC_DBG_L("client_id:%d update %s hash:%llu ctx_id:%llu seqno:%llu "
		    "flags:%llu error:%u\n",
		    hw_fence_client->client_id, _get_queue_type(queue_type),
		    hash, ctxt_id, seqno, flags, error);

	/*
   * wrap-around case, here we are writing to the last element of the queue,
   * therefore set to_write_idx, which is the index after the write, to the
   * beginning of the queue
   */
	...
	if (REQUIRES_IDX_TRANSLATION(queue)) {
		to_write_idx =
			IDX_TRANSLATE_DEFAULT_TO_CUSTOM(queue, to_write_idx);
		HWFNC_DBG_Q(
			"translated to_write_idx:%lu rd_wr_idx start:%lu factor:%lu\n",
			to_write_idx, queue->rd_wr_idx_start,
			queue->rd_wr_idx_factor);
	}
...
int hw_fence_update_existing_txq_payload(
	struct hw_fence_driver_data *drv_data,
	struct msm_hw_fence_client *hw_fence_client, u64 hash, u32 error)
{
	u32 q_size_u32, payload_size, payload_size_u32, read_idx, write_idx,
		second_idx, *wr_ptr;
   ...
   * offset */
	...
	first_payload =
		(struct msm_hw_fence_queue_payload *)((u32 *)queue->va_queue +
						      read_idx);
	HWFNC_DBG_Q(
		"client:%d txq: va=0x%pK pa=0x%pK idx:%d ptr_payload:0x%pK\n",
		hw_fence_client->client_id, queue->va_queue, queue->pa_queue,
		read_idx, first_payload);
	...
	if (first_payload->hash == hash) {
		...
		if (second_idx == write_idx) {
			HWFNC_ERR(
				"Failed to find matching entry with hash:%llu\n",
				hash);
			return -EINVAL;
		}
		...
		HWFNC_DBG_Q(
			"client:%d txq: va=0x%pK pa=0x%pK idx:%d ptr_payload:0x%pK\n",
			hw_fence_client->client_id, queue->va_queue,
			queue->pa_queue, second_idx, second_payload);

		if (second_payload->hash != hash) {
			HWFNC_ERR(
				"hash:%llu not found in first two queue payloads:%u, %u\n",
				hash, read_idx, second_idx);
			return -EINVAL;
		}
     ...
     */
		...
		*second_payload = tmp;
		...
		HWFNC_DBG_L(
			"client_id:%d txq move from idx:%u to idx:%u hash:%llu c:%llu s:%llu\n",
			hw_fence_client->client_id, read_idx, second_idx, hash,
			tmp.ctxt_id, tmp.seqno);
	}
	...
	HWFNC_DBG_L(
		"client_id:%d update tx queue index:%u hash:%llu error:%u\n",
...
static int init_global_locks(struct hw_fence_driver_data *drv_data)
{
	...
	ret = hw_fence_utils_reserve_mem(drv_data,
	...
	drv_data->client_lock_tbl =
		(u64 *)drv_data->clients_locks_mem_desc.virtual_addr;
	drv_data->client_lock_tbl_cnt =
		drv_data->clients_locks_mem_desc.size / sizeof(u64);
...
static int init_hw_fences_table(struct hw_fence_driver_data *drv_data)
{
	...
	ret = hw_fence_utils_reserve_mem(drv_data, HW_FENCE_MEM_RESERVE_TABLE,
					 &phys, &ptr, &size, 0);
	if (ret) {
	...
	drv_data->hw_fences_tbl =
		(struct msm_hw_fence *)drv_data->hw_fences_mem_desc.virtual_addr;
	drv_data->hw_fences_tbl_cnt =
		drv_data->hw_fences_mem_desc.size / sizeof(struct msm_hw_fence);
...
static int init_hw_fences_events(struct hw_fence_driver_data *drv_data)
{
	...
	HWFNC_DBG_INIT(
		"events:0x%pK total_events:%u event_sz:%u total_size:%u\n",
		drv_data->events, drv_data->total_events,
		sizeof(struct msm_hw_fence_event), size);
				    ...
				    struct msm_hw_fence_mem_addr *mem_descriptor)
{
	...
	if (!drv_data->hw_fence_client_queue_size[hw_fence_client->client_id]
		     .type) {
		HWFNC_ERR(
			"invalid client_id:%d not reserved client queue; check dt props\n",
			hw_fence_client->client_id);
		return -EINVAL;
	}

	/* Init client queues */
	ret = init_hw_fences_queues(
		drv_data, HW_FENCE_MEM_RESERVE_CLIENT_QUEUE,
		&hw_fence_client->mem_descriptor, hw_fence_client->queues,
		drv_data->hw_fence_client_queue_size[hw_fence_client->client_id]
			.type->queues_num,
		hw_fence_client->client_id);
	if (ret) {
				    ...
				    struct msm_hw_fence_client *hw_fence_client)
{
   ...
   */
	...
	case HW_FENCE_CLIENT_ID_CTL5:
		...
		HWFNC_DBG_H(
			"init_controller_signal: DPU client_id_ext:%d initialized:%d\n",
			hw_fence_client->client_id_ext,
			drv_data->ipcc_dpu_initialized);
...
int hw_fence_init_controller_resources(
	struct msm_hw_fence_client *hw_fence_client)
{
	/*
...
static inline int _calculate_hash(u32 table_total_entries, u64 context,
				  u64 seqno, u64 step, u64 *hash)
{
     ...
     */
		*hash = (a_multiplier * seqno * b_multiplier +
			 (c_multiplier * context)) %
			m_size;
	} else {
		if (step >= m_size) {
			/*
       * If we already traversed the whole table, return failure since this
       * means there are not available spots, table is either full or
       * full-enough that we couldn't find an available spot after traverse the
       * whole table. Ideally table shouldn't be so full that we cannot find a
       * value after some iterations, so this maximum step size could be
       * optimized to fail earlier.
       */
			HWFNC_ERR(
				"Fence Table tranversed and no available space!\n");
			val = -EINVAL;
		} else {
			/*
...
static inline struct msm_hw_fence *
_get_hw_fence(u32 table_total_entries, struct msm_hw_fence *hw_fences_tbl,
	      u64 hash)
{
...
static bool _is_hw_fence_free(struct msm_hw_fence *hw_fence, u64 context,
			      u64 seqno)
{
...
static bool _hw_fence_match(struct msm_hw_fence *hw_fence, u64 context,
			    u64 seqno)
{
...
static void _reserve_hw_fence(struct hw_fence_driver_data *drv_data,
			      ...
			      u64 context, u64 seqno, u32 hash,
			      u32 pending_child_cnt)
{
	...
	hw_fence->flags =
		0; /* fence just reserved, there shouldn't be any flags set */
				...
				u32 pending_child_cnt)
{
	...
	HWFNC_DBG_LUT(
		"Reserved join fence client:%d ctx:%llu seq:%llu hash:%llu\n",
		client_id, context, seqno, hash);
}
...
{
	/*
   * Do nothing, when this find fence fn is invoked, all processing is done
   * outside. Currently just keeping this function for debugging purposes, can
   * be removed in final versions
   */
...
struct msm_hw_fence *
_hw_fence_lookup_and_process(struct hw_fence_driver_data *drv_data,
			     struct msm_hw_fence *hw_fences_tbl, u64 context,
			     u64 seqno, u32 client_id, u32 pending_child_cnt,
			     enum hw_fence_lookup_ops op_code, u64 *hash)
{
	bool (*compare_fnc)(struct msm_hw_fence *hfence, u64 context,
			    u64 seqno);
	void (*process_fnc)(struct hw_fence_driver_data *drv_data,
			    struct msm_hw_fence *hfence, u32 client_id,
			    u64 context, u64 seqno, u32 hash, u32 pending);
	...
	*hash = ~0;
	...
	while (!hw_fence_found && (step < drv_data->hw_fence_table_entries)) {
		...
		ret = _calculate_hash(drv_data->hw_fence_table_entries, context,
				      seqno, step, hash);
		if (ret) {
			HWFNC_ERR(
				"error calculating hash ctx:%llu seqno:%llu hash:%llu\n",
				context, seqno, *hash);
			break;
		}
		...
		hw_fence = _get_hw_fence(drv_data->hw_fence_table_entries,
					 hw_fences_tbl, *hash);
		HWFNC_DBG_LUT(
			"hw_fence_tbl:0x%pK hw_fence:0x%pK, hash:%llu valid:0x%x\n",
			hw_fences_tbl, hw_fence, *hash,
			hw_fence ? hw_fence->valid : 0xbad);
		if (!hw_fence) {
			HWFNC_ERR(
				"bad hw fence ctx:%llu seqno:%llu hash:%llu\n",
				context, seqno, *hash);
			break;
		}
		...
		if (compare_fnc(hw_fence, context, seqno)) {
			...
			if (process_fnc) {
				process_fnc(drv_data, hw_fence, client_id,
					    context, seqno, *hash,
					    pending_child_cnt);
			...
			HWFNC_DBG_L(
				"client_id:%lu op:%s ctx:%llu seqno:%llu hash:%llu step:%llu\n",
				client_id, _get_op_mode(op_code), context,
				seqno, *hash, step);
			...
			if ((op_code == HW_FENCE_LOOKUP_OP_CREATE ||
			    ...
			    seqno == hw_fence->seq_id &&
			    context == hw_fence->ctx_id) {
				/* ctx & seqno must be unique creating a hw-fence */
				HWFNC_ERR(
					"cannot create hw fence with same ctx:%llu seqno:%llu\n",
					context, seqno);
				GLOBAL_ATOMIC_STORE(drv_data, &hw_fence->lock,
						    0);
				break;
			}
			/* compare can fail if we have a collision, we will linearly resolve it */
			HWFNC_DBG_H(
				"compare failed for hash:%llu [ctx:%llu seqno:%llu]\n",
				*hash, context, seqno);
		}
	...
	/* If we iterated through the whole list and didn't find the fence, return
   * null */
	if (!hw_fence_found) {
	...
	HWFNC_DBG_LUT(
		"lookup:%d hw_fence:%pK ctx:%llu seqno:%llu hash:%llu flags:0x%llx\n",
		op_code, hw_fence, context, seqno, *hash,
		hw_fence ? hw_fence->flags : -1);

	return hw_fence;
}

int hw_fence_create(struct hw_fence_driver_data *drv_data,
		    struct msm_hw_fence_client *hw_fence_client, u64 context,
		    u64 seqno, u64 *hash)
{
	...
	if (!_hw_fence_lookup_and_process(drv_data, hw_fences_tbl, context,
					  seqno, client_id, 0,
					  HW_FENCE_LOOKUP_OP_CREATE, hash)) {
		HWFNC_ERR(
			"Fail to create fence client:%lu ctx:%llu seqno:%llu\n",
...
static inline int _hw_fence_cleanup(struct hw_fence_driver_data *drv_data,
				    ...
				    u32 client_id, u64 context, u64 seqno)
{
	u64 hash;

	if (!_hw_fence_lookup_and_process(drv_data, hw_fences_tbl, context,
					  seqno, client_id, 0,
					  HW_FENCE_LOOKUP_OP_DESTROY, &hash))
		return -EINVAL;
		     ...
		     u64 seqno)
{
	...
	if (_hw_fence_cleanup(drv_data, hw_fences_tbl, client_id, context,
			      seqno)) {
		HWFNC_ERR(
			"Fail destroying fence client:%lu ctx:%llu seqno:%llu\n",
			client_id, context, seqno);
		ret = -EINVAL;
	}
			       ...
			       u64 hash)
{
	...
	if (hw_fence->fence_allocator != client_id) {
		HWFNC_ERR(
			"client:%lu cannot destroy fence hash:%llu fence_allocator:%lu\n",
			client_id, hash, hw_fence->fence_allocator);
		return -EINVAL;
	}

	/* remove hw fence from table*/
	if (_hw_fence_cleanup(drv_data, hw_fences_tbl, client_id,
			      hw_fence->ctx_id, hw_fence->seq_id)) {
		HWFNC_ERR(
			"Fail destroying fence client:%lu ctx:%llu seqno:%llu hash:%llu\n",
...
_hw_fence_process_join_fence(struct hw_fence_driver_data *drv_data,
			     struct msm_hw_fence_client *hw_fence_client,
			     struct dma_fence_array *array, u64 *hash,
			     bool create)
{
	struct msm_hw_fence *hw_fences_tbl;
   ...
   * the other fences (i.e. drv_data->hw_fences_tbl), functionally this will
   * work, however, this might impact the lookup algorithm, since the
   * "join-fences" are created with the context and seqno of a fence-array, and
   * those might not be changing by the client, so this will linearly increment
   * the look-up and very likely impact the other fences if these join-fences
   * start to fill-up a particular region of the fences global table. So we
   * might have to allocate a different table altogether for these join fences.
   ...
   */
	...
	if (create) {
		/* allocate the fence */
		join_fence = _hw_fence_lookup_and_process(
			drv_data, hw_fences_tbl, context, seqno, client_id,
			pending_child_cnt, HW_FENCE_LOOKUP_OP_CREATE_JOIN,
			hash);
		if (!join_fence)
			HWFNC_ERR(
				"Fail to create join fence client:%lu ctx:%llu seqno:%llu\n",
				client_id, context, seqno);
	} else {
		/* destroy the fence */
		if (_hw_fence_cleanup(drv_data, hw_fences_tbl, client_id,
				      context, seqno))
			HWFNC_ERR(
				"Fail destroying join fence client:%lu ctx:%llu seqno:%llu\n",
				client_id, context, seqno);
	}
...
struct msm_hw_fence *
msm_hw_fence_find(struct hw_fence_driver_data *drv_data,
		  struct msm_hw_fence_client *hw_fence_client, u64 context,
		  u64 seqno, u64 *hash)
{
	struct msm_hw_fence *hw_fences_tbl = drv_data->hw_fences_tbl;
	...
	hw_fence = _hw_fence_lookup_and_process(drv_data, hw_fences_tbl,
						context, seqno, client_id, 0,
						HW_FENCE_LOOKUP_OP_FIND_FENCE,
						hash);
	if (!hw_fence)
		HWFNC_ERR(
			"Fail to find hw fence client:%lu ctx:%llu seqno:%llu\n",
			client_id, context, seqno);
...
{
	u32 tx_client_id =
		drv_data->ipcc_client_pid; /* phys id for tx client */
	u32 rx_client_id =
		hw_fence_client->ipc_client_vid; /* virt id for rx client */
	...
	if (error && hw_fence_client->fence_error_cb) {
		hw_fence_utils_fence_error_cb(hw_fence_client, hw_fence->ctx_id,
					      hw_fence->seq_id, hash, flags,
					      error);
	} else {
		/* Write to Rx queue */
		if (hw_fence_client->update_rxq)
			hw_fence_update_queue(drv_data, hw_fence_client,
					      hw_fence->ctx_id,
					      hw_fence->seq_id, hash, flags,
					      client_data, error,
					      HW_FENCE_RX_QUEUE - 1);
		...
		if (hw_fence_client->send_ipc)
			hw_fence_ipcc_trigger_signal(
				drv_data, tx_client_id, rx_client_id,
	...
	if (hw_fence_client->client_id >= HW_FENCE_CLIENT_ID_VAL0 &&
		...
		process_validation_client_loopback(drv_data,
						   hw_fence_client->client_id);
#endif /* CONFIG_DEBUG_FS */
}

static void
_cleanup_join_and_child_fences(struct hw_fence_driver_data *drv_data,
			       struct msm_hw_fence_client *hw_fence_client,
			       int iteration, struct dma_fence_array *array,
			       ...
			       u64 hash_join_fence)
{
	...
	for (idx = iteration; idx >= 0; idx--) {
		...
		hw_fence_child = msm_hw_fence_find(drv_data, hw_fence_client,
						   child_fence->context,
						   child_fence->seqno, &hash);
		if (!hw_fence_child) {
			HWFNC_ERR(
				"Cannot cleanup child fence context:%lu seqno:%lu hash:%lu\n",
       ...
       */
		...
		GLOBAL_ATOMIC_STORE(drv_data, &hw_fence_child->lock,
				    1); /* lock */
		for (j = hw_fence_child->parents_cnt; j > 0; j--) {
			if (j > MSM_HW_FENCE_MAX_JOIN_PARENTS) {
				HWFNC_ERR(
					"Invalid max parents_cnt:%d, will reset to max:%d\n",
			...
			if (hw_fence_child->parent_list[j - 1] ==
			    hash_join_fence) {
				hw_fence_child->parent_list[j - 1] =
					HW_FENCE_INVALID_PARENT_FENCE;
				 ...
				 u64 *hash_join_fence, u64 client_data)
{
	...
	if (client_data) {
		data_id = hw_fence_get_client_data_id(
			hw_fence_client->client_id_ext);
		if (data_id >= HW_FENCE_MAX_CLIENTS_WITH_DATA) {
			HWFNC_ERR(
				"Populating client_data:%llu with invalid client_id_ext:%d\n",
   ...
   */
	join_fence = _hw_fence_process_join_fence(drv_data, hw_fence_client,
						  array, hash_join_fence, true);
	if (!join_fence) {
	...
	for (i = 0; i < array->num_fences; i++) {
		...
		if (!test_bit(MSM_HW_FENCE_FLAG_ENABLED_BIT,
			      &child_fence->flags)) {
			HWFNC_ERR(
				"DMA Fence in FenceArray is not a HW Fence\n");
			ret = -EINVAL;
			goto error_array;
		}
		...
		if (!hw_fence_child) {
			HWFNC_ERR(
				"Cannot find child fence context:%lu seqno:%lu hash:%lu\n",
				child_fence->context, child_fence->seqno, hash);
			ret = -EINVAL;
			goto error_array;
		}
		...
		if (hw_fence_child->flags & MSM_HW_FENCE_FLAG_SIGNAL) {
			...
			GLOBAL_ATOMIC_STORE(drv_data, &join_fence->lock,
					    1); /* lock */
			...
			if (hw_fence_child->parents_cnt >=
				...
				HWFNC_ERR(
					"DMA Fence in FenceArray exceeds parents:%d\n",
					hw_fence_child->parents_cnt);
				hw_fence_child->parents_cnt--;
				...
				/* unlock */
				GLOBAL_ATOMIC_STORE(drv_data,
						    &hw_fence_child->lock, 0);
				ret = -EINVAL;
				goto error_array;
			}

			hw_fence_child
				->parent_list[hw_fence_child->parents_cnt - 1] =
				*hash_join_fence;
	...
	if (signal_join_fence) {
		...
		_fence_ctl_signal(drv_data, hw_fence_client, join_fence,
				  *hash_join_fence, 0, client_data,
				  join_fence->error);
     ...
     * the fence-array, then fail here, otherwise driver would create a
     * join-fence with no-childs that won't be signaled at all or an incomplete
     * join-fence
     */
		HWFNC_ERR(
			"invalid fence-array ctx:%llu seqno:%llu without fences\n",
...
error_array:
	_cleanup_join_and_child_fences(drv_data, hw_fence_client, i, array,
				       join_fence, *hash_join_fence);
				  ...
				  u64 client_data)
{
	...
	hw_fence = msm_hw_fence_find(drv_data, hw_fence_client, context, seqno,
				     hash);
	if (!hw_fence) {
	...
	if (hw_fence->flags & MSM_HW_FENCE_FLAG_SIGNAL) {
		...
		_fence_ctl_signal(drv_data, hw_fence_client, hw_fence, *hash, 0,
				  client_data, 0);
	}
			   ...
			   struct dma_fence *fence, u64 *hash, u64 client_data)
{
	...
	ret = hw_fence_register_wait_client(drv_data, fence, hw_fence_client,
					    fence->context, fence->seqno, hash,
					    client_data);
	if (ret)
				     ...
				     int error)
{
	...
	for (wait_client_id = 0; wait_client_id <= drv_data->rxq_clients_num;
	     wait_client_id++) {
		if (hw_fence->wait_client_mask & BIT(wait_client_id)) {
			hw_fence_wait_client =
				drv_data->clients[wait_client_id];

			if (!hw_fence_wait_client)
				continue;

			data_id = hw_fence_get_client_data_id(
				hw_fence_wait_client->client_id_ext);

			if (data_id < HW_FENCE_MAX_CLIENTS_WITH_DATA)
				client_data = hw_fence->client_data[data_id];

			_fence_ctl_signal(drv_data, hw_fence_wait_client,
					  hw_fence, hash, 0, client_data,
					  error);
		}
	}
}
				 ...
				 struct msm_hw_fence_client *hw_fence_client)
{
	...
	/* For the client TxQ: set the read-index same as last write that was done by
   * the client */
	...
	/* For the client RxQ: set the write-index same as last read done by the
   * client */
	if (hw_fence_client->update_rxq) {
		lock_idx = hw_fence_client->client_id - 1;
		...
		if (lock_idx >= drv_data->client_lock_tbl_cnt) {
			HWFNC_ERR(
				"cannot reset rxq, lock for client id:%d exceed max:%d\n",
				hw_fence_client->client_id,
				drv_data->client_lock_tbl_cnt);
			return;
		}
		...
		/* lock the client rx queue to update */
		GLOBAL_ATOMIC_STORE(drv_data,
				    &drv_data->client_lock_tbl[lock_idx], 1);
		...
		HWFNC_DBG_Q(
			"update rx queue write_index to match read_index:%lu\n",
			rd_idx);
	}
}
...
{
	int ret = 0;
	int error = (reset_flags & MSM_HW_FENCE_RESET_WITHOUT_ERROR) ?
			    0 :
	...
	if (hw_fence->wait_client_mask & BIT(hw_fence_client->client_id)) {
		HWFNC_DBG_H(
			"clearing client:%d wait bit for fence: ctx:%d seqno:%d\n",
	...
	if (hw_fence->fence_allocator == hw_fence_client->client_id) {
		...
		if (!(hw_fence->flags & MSM_HW_FENCE_FLAG_SIGNAL))
			_signal_all_wait_clients(drv_data, hw_fence, hash,
						 error);
		...
		if (ret) {
			HWFNC_ERR(
				"Error destroying HW fence: ctx:%d seqno:%d\n",
				hw_fence->ctx_id, hw_fence->seq_id);
		}
	}
...
hw_fence_get_client_data_id(enum hw_fence_client_id client_id)
{
	enum hw_fence_client_data_id data_id;
