...
int synx_util_init_coredata(struct synx_coredata *synx_obj,
			    ...
			    struct dma_fence_ops *ops, u64 dma_context)
{
	int rc = -SYNX_INVALID;
	...
	if (params->flags & SYNX_CREATE_GLOBAL_FENCE && *params->h_synx != 0) {
		rc = synx_global_get_ref(synx_util_global_idx(*params->h_synx));
     ...
     */
		...
		dprintk(SYNX_MEM, "allocated backing fence %pK\n", fence);
...
free:
	if (params->flags & SYNX_CREATE_GLOBAL_FENCE)
		synx_global_put_ref(synx_util_global_idx(*params->h_synx));
	else
...
int synx_util_add_callback(struct synx_coredata *synx_obj, u32 h_synx)
{
   ...
   */
	rc = dma_fence_add_callback(synx_obj->fence, &signal_cb->fence_cb,
				    synx_fence_callback);
	if (rc != 0) {
		if (rc == -ENOENT) {
				  ...
				  u32 num_objs, u64 dma_context)
{
	...
	array = dma_fence_array_create(num_objs, fences, dma_context, 1, false);
...
static void synx_util_destroy_coredata(struct kref *kref)
{
	...
	if (synx_util_is_global_object(synx_obj)) {
		rc = synx_global_clear_subscribed_core(synx_obj->global_idx,
						       SYNX_CORE_APSS);
		if (rc)
	...
	if (dma_fence_is_array(synx_obj->fence)) {
		...
		for (i = 0; i < array->num_fences; i++) {
			if (kref_read(&array->fences[i]->refcount) == 1 &&
			    __fence_state(array->fences[i], false) ==
				    SYNX_STATE_ACTIVE) {
				dma_fence_set_error(
					array->fences[i],
...
void synx_util_object_destroy(struct synx_coredata *synx_obj)
{
	...
	list_for_each_entry_safe(synx_cb, synx_cb_temp, &synx_obj->reg_cbs_list,
		...
		dprintk(SYNX_VERB, "dispatched callback for fence %pKn",
			synx_obj->fence);
	}
	...
	for (i = 0; i < synx_obj->num_bound_synxs; i++) {
		...
		rc = bind_ops->deregister_callback(synx_external_callback, data,
						   sync_id);
		if (rc < 0) {
			dprintk(SYNX_ERR,
	...
	if (!IS_ERR_OR_NULL(synx_obj->fence)) {
		...
		if (synx_util_is_merged_object(synx_obj) &&
		    synx_util_get_object_status_locked(synx_obj) ==
			    SYNX_STATE_ACTIVE)
			rc = synx_util_cleanup_merged_fence(
				synx_obj, -SYNX_STATE_SIGNALED_CANCEL);
			  ...
			  void *map_entry)
{
	...
	hash_for_each_possible(client->handle_map, curr, node, *new_h_synx) {
	...
	if (unlikely(found))
		...
		hash_add(client->handle_map, &synx_data->node, *new_h_synx);
...
static u32 synx_util_add_fence(struct synx_coredata *synx_obj,
			       struct dma_fence **fences, u32 idx)
{
	struct dma_fence_array *array = NULL;
	u32 i = 0;
...
s32 synx_util_merge_error(struct synx_client *client, u32 *h_synxs,
			  u32 num_objs)
{
	u32 i = 0;
...
int synx_util_validate_merge(struct synx_client *client, u32 *h_synxs,
			     u32 num_objs, struct dma_fence ***fence_list,
			     u32 *fence_cnt)
{
	u32 count = 0;
	u32 i = 0;
...
static u32 __fence_group_state(struct dma_fence *fence, bool locked)
{
	...
	dprintk(SYNX_DBG, "group cnt stats act:%u, sig: %u, err: %u\n",
...
synx_util_acquire_handle(struct synx_client *client, u32 h_synx)
{
	struct synx_handle_coredata *synx_data = NULL;
	struct synx_handle_coredata *synx_handle = ERR_PTR(-SYNX_NOENT);

	if (IS_ERR_OR_NULL(client))
		return ERR_PTR(-SYNX_INVALID);

	spin_lock_bh(&client->handle_map_lock);
	hash_for_each_possible(client->handle_map, synx_data, node, h_synx) {
		if (synx_data->key == h_synx && synx_data->rel_count != 0) {
					       ...
					       u32 h_synx, u32 flags)
{
	...
	if (synx_util_is_global_handle(h_synx)) {
		...
		hash_add(synx_dev->native->global_map, &map_entry->node,
			 h_synx);
		spin_unlock_bh(&synx_dev->native->global_map_lock);
		dprintk(SYNX_MEM, "added handle %u to global map %pK\n", h_synx,
			map_entry);
	} else {
		spin_lock_bh(&synx_dev->native->local_map_lock);
		hash_add(synx_dev->native->local_map, &map_entry->node, h_synx);
		spin_unlock_bh(&synx_dev->native->local_map_lock);
		dprintk(SYNX_MEM, "added handle %u to local map %pK\n", h_synx,
			map_entry);
	}
...
struct synx_map_entry *synx_util_get_map_entry(u32 h_synx)
{
	...
	if (synx_util_is_global_handle(h_synx)) {
		spin_lock_bh(&synx_dev->native->global_map_lock);
		hash_for_each_possible(synx_dev->native->global_map, curr, node,
				       h_synx) {
			if (curr->key == h_synx) {
				kref_get(&curr->refcount);
				map_entry = curr;
		...
		spin_lock_bh(&synx_dev->native->local_map_lock);
		hash_for_each_possible(synx_dev->native->local_map, curr, node,
				       h_synx) {
			if (curr->key == h_synx) {
				kref_get(&curr->refcount);
				map_entry = curr;
...
static void synx_util_cleanup_fence(struct synx_coredata *synx_obj)
{
	struct synx_signal_cb *signal_cb;
	unsigned long flags;
	u32 g_status;
	...
	if (synx_obj->map_count == 0 && (signal_cb != NULL) &&
	    (synx_obj->global_idx != 0) && (f_status == SYNX_STATE_ACTIVE)) {
		/*
     ...
     */
		...
		if (g_status > SYNX_STATE_ACTIVE) {
			dprintk(SYNX_DBG,
				"signaling fence %pK with status %u\n",
			...
			if (synx_util_get_object_status_locked(synx_obj) ==
				...
				synx_global_fetch_handle_details(
					synx_obj->global_idx, &h_synx);
				signal_cb->handle = h_synx;
...
static void synx_util_destroy_map_entry_worker(struct work_struct *dispatch)
{
	struct synx_map_entry *map_entry =
...
void synx_util_release_map_entry(struct synx_map_entry *map_entry)
{
	...
	spin_lock_bh(lock);
	kref_put(&map_entry->refcount, synx_util_destroy_map_entry);
	spin_unlock_bh(lock);
}

static void synx_util_destroy_handle_worker(struct work_struct *dispatch)
{
	struct synx_handle_coredata *synx_data =
		container_of(dispatch, struct synx_handle_coredata, dispatch);
...
{
	struct synx_handle_coredata *synx_data =
		container_of(kref, struct synx_handle_coredata, refcount);
...
void synx_util_release_handle(struct synx_handle_coredata *synx_data)
{
	...
	spin_lock_bh(&client->handle_map_lock);
	kref_put(&synx_data->refcount, synx_util_destroy_handle);
	spin_unlock_bh(&client->handle_map_lock);
}
...
int synx_util_alloc_cb_entry(struct synx_client *client,
			     struct synx_kernel_payload *data, u32 *cb_idx)
{
	long idx;
	...
	if (idx >= SYNX_MAX_OBJS) {
		dprintk(SYNX_ERR, "[sess :%llu] free cb index not available\n",
			client->id);
		return -SYNX_NOMEM;
	}
	...
	cb->client = client;
	cb->idx = idx;
	memcpy(&cb->kernel_cb, data, sizeof(cb->kernel_cb));
...
void synx_util_cb_dispatch(struct work_struct *cb_dispatch)
{
	...
	if (IS_ERR_OR_NULL(client)) {
		dprintk(SYNX_ERR, "invalid session data %pK in cb payload\n",
			synx_cb->session);
		goto free;
	}

	if (synx_cb->idx == 0 || synx_cb->idx >= SYNX_MAX_OBJS) {
		dprintk(SYNX_ERR, "[sess :%llu] invalid cb index %u\n",
...
int synx_get_child_coredata(struct synx_coredata *synx_obj,
			    struct synx_coredata ***child_synx_obj,
			    int *num_fences)
{
	int rc = SYNX_SUCCESS;
	...
	if (dma_fence_is_array(synx_obj->fence)) {
		...
		if (IS_ERR_OR_NULL(array))
			return -SYNX_INVALID;
		synx_datas = kcalloc(array->num_fences, sizeof(*synx_datas),
				     GFP_KERNEL);
		if (IS_ERR_OR_NULL(synx_datas))
			return -SYNX_NOMEM;
...
u32 synx_util_get_fence_entry(u64 key, u32 global)
{
	...
	hash_for_each_possible(synx_dev->native->fence_map, curr, node, key) {
		if (curr->key == key) {
			if (global)
...
void synx_util_release_fence_entry(u64 key)
{
	...
	if (entry) {
		hash_del(&entry->node);
		dprintk(SYNX_MEM, "released fence entry %pK for fence %pK\n",
			entry, (void *)key);
		kfree(entry);
	}
				 ...
				 u32 global)
{
	...
	spin_lock_bh(&synx_dev->native->fence_map_lock);
	hash_for_each_possible(synx_dev->native->fence_map, curr, node,
			       entry->key) {
	...
	if (rc == SYNX_SUCCESS) {
		hash_add(synx_dev->native->fence_map, &entry->node, entry->key);
		dprintk(SYNX_MEM, "added fence entry %pK for fence %pK\n",
...
static void synx_client_cleanup(struct work_struct *dispatch)
{
   ...
   */
	hash_for_each_safe(client->handle_map, i, tmp, curr, node) {
		dprintk(SYNX_WARN, "[sess :%llu] un-released handle %u\n",
			client->id, curr->key);
...
void synx_util_generate_timestamp(char *timestamp, size_t size)
{
	...
	snprintf(timestamp, size, "%02d-%02d %02d:%02d:%02d", tm.tm_mon + 1,
...
void synx_util_log_error(u32 client_id, u32 h_synx, s32 err)
{
	...
	list_add(&err_node->node, &synx_dev->error_list);
	mutex_unlock(&synx_dev->error_lock);
}

int synx_util_save_data(void *fence, u32 flags, u32 h_synx)
{
	int rc = SYNX_SUCCESS;
	...
	case SYNX_TYPE_CSL:
		...
		hash_for_each_possible(synx_dev->native->csl_fence_map, curr,
				       node, key) {
			if (curr->key == key) {
				rc = -SYNX_ALREADY;
				break;
...
u32 synx_util_map_client_id_to_core(enum synx_client_id id)
{
	...
	case SYNX_CLIENT_NATIVE:
		core_id = SYNX_CORE_APSS;
		break;
	case SYNX_CLIENT_ICP_CTX0:
		core_id = SYNX_CORE_ICP;
		break;
	case SYNX_CLIENT_EVA_CTX0:
		core_id = SYNX_CORE_EVA;
		break;
	case SYNX_CLIENT_VID_CTX0:
		core_id = SYNX_CORE_IRIS;
		break;
	case SYNX_CLIENT_NSP_CTX0:
		core_id = SYNX_CORE_NSP;
		break;
	default:
