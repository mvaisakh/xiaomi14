...
static ssize_t adsp_ssr_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static void adsp_load_fw(struct work_struct *adsp_ldr_work)
{
	...
	if (rc) {
		dev_err(&pdev->dev, "%s: ADSP state = %x\n", __func__,
			adsp_state);
		goto fail;
	}
	...
	prop = of_find_property(pdev->dev.of_node, "qcom,proc-img-to-load",
				&size);
	if (!prop) {
		dev_dbg(&pdev->dev, "%s: loading default image ADSP\n",
			__func__);
		goto load_adsp;
	}
	...
	if (!strcmp(rproc->name, "modem")) {
		if (adsp_state == SPF_SUBSYS_DOWN) {
			...
			dev_dbg(&pdev->dev, "%s: MDSP state = %x\n", __func__,
				adsp_state);
		}
...
load_adsp: {
	...
	if (adsp_state == SPF_SUBSYS_DOWN) {
		...
		dev_dbg(&pdev->dev, "%s: ADSP state = %x\n", __func__,
			adsp_state);
	}
...
static int adsp_loader_probe(struct platform_device *pdev)
{
     ...
     */
		...
		if (adsp_fuse_not_supported) {
			...
			if (ret < 0) {
				dev_dbg(&pdev->dev,
					"%s: unable to read fw-name\n",
					__func__);
				goto wqueue;
			}

			fw_name_size = strlen(adsp_fw_name) + 1;
			priv->adsp_fw_name = devm_kzalloc(
				&pdev->dev, fw_name_size, GFP_KERNEL);
			if (!priv->adsp_fw_name)
				goto wqueue;
			strlcpy(priv->adsp_fw_name, adsp_fw_name, fw_name_size);
			...
			if (ret < 0) {
				dev_dbg(&pdev->dev,
					"%s: unable to read fw-dtb-name\n",
					__func__);
				goto wqueue;
			}

			fw_name_size = strlen(adsp_dtb_name) + 1;
			priv->adsp_dtb_name = devm_kzalloc(
				&pdev->dev, fw_name_size, GFP_KERNEL);
			if (!priv->adsp_dtb_name)
				goto wqueue;
	...
	adsp_fw_cnt =
		of_property_count_strings(pdev->dev.of_node, "adsp-fw-names");
	...
	adsp_fw_bit_values =
		devm_kzalloc(&pdev->dev, adsp_fw_cnt * sizeof(u32), GFP_KERNEL);
	...
	ret = of_property_read_u32_array(pdev->dev.of_node,
					 "adsp-fw-bit-values",
					 adsp_fw_bit_values, adsp_fw_cnt);
	if (ret) {
	...
	adsp_fw_name_array = devm_kzalloc(
		&pdev->dev, adsp_fw_cnt * sizeof(char *), GFP_KERNEL);
	if (!adsp_fw_name_array)
		goto wqueue;

	/* Read ADSP firmware image names */
	ret = of_property_read_string_array(pdev->dev.of_node, "adsp-fw-names",
					    adsp_fw_name_array, adsp_fw_cnt);
	if (ret < 0) {
