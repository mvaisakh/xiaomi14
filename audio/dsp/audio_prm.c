...
static int prm_gpr_send_pkt(struct gpr_pkt *pkt, wait_queue_head_t *wait)
{
	...
	if (wait) {
		ret = wait_event_timeout(g_prm.wait, (g_prm.resp_received),
...
static int _audio_prm_set_lpass_cpu_lpr_req(uint8_t enable)
{
	...
	pkt->hdr.header =
		GPR_SET_FIELD(GPR_PKT_VERSION, GPR_PKT_VER) |
	...
	prm_lpr_request.payload_header.payload_size =
		sizeof(struct prm_cpu_lpr_request_t) - sizeof(apm_cmd_header_t);

	/** Populate the param payload */
	prm_lpr_request.module_payload_0.module_instance_id =
	...
	memcpy(&pkt->payload, &prm_lpr_request,
	       sizeof(struct prm_cpu_lpr_request_t));
...
int _audio_prm_set_lpass_hw_core_req(uint32_t hw_core_id, uint8_t enable)
{
	...
	prm_rsc_request.payload_header.payload_size =
		sizeof(prm_cmd_request_hw_core_t) - sizeof(apm_cmd_header_t);

	/** Populate the param payload */
	prm_rsc_request.module_payload_0.module_instance_id =
		PRM_MODULE_INSTANCE_ID;
	...
	memcpy(&pkt->payload, &prm_rsc_request,
	       sizeof(prm_cmd_request_hw_core_t));
...
int audio_prm_set_lpass_hw_core_req(struct clk_cfg *cfg, uint32_t hw_core_id,
				    uint8_t enable)
{
	if (hw_core_id == HW_CORE_ID_LPASS) {
		if (g_prm.prm_sleep_api_supported)
			...
			return _audio_prm_set_lpass_hw_core_req(hw_core_id,
								enable);
	}
...
static int audio_prm_set_lpass_clk_cfg_req(struct clk_cfg *cfg)
{
	...
	// sizeof(prm_cmd_request_rsc_t));

	prm_rsc_request.payload_header.payload_address_lsw = 0;
	prm_rsc_request.payload_header.payload_address_msw = 0;
	prm_rsc_request.payload_header.mem_map_handle = 0;
	prm_rsc_request.payload_header.payload_size =
		sizeof(prm_cmd_request_rsc_t) - sizeof(apm_cmd_header_t);
...
static int audio_prm_set_lpass_clk_cfg_rel(struct clk_cfg *cfg)
{
	...
	prm_rsc_release.payload_header.payload_size =
		sizeof(prm_cmd_release_rsc_t) - sizeof(apm_cmd_header_t);

	/** Populate the param payload */
	prm_rsc_release.module_payload_0.module_instance_id =
		PRM_MODULE_INSTANCE_ID;
 ...
 */
int audio_prm_set_cdc_earpa_duty_cycling_req(
	struct prm_earpa_hw_intf_config *earpa_config, uint32_t enable)
{
	struct gpr_pkt *pkt;
	...
	memset(&prm_rsc_request_reg_info, 0,
	...
	prm_rsc_request_reg_info.module_payload_0.param_size =
		sizeof(prm_cmd_request_cdc_duty_cycling_t) -
		sizeof(apm_cmd_header_t) - sizeof(apm_module_param_data_t);
	prm_rsc_request_reg_info.hw_codec_reg_info_t.num_reg_info_t =
		MAX_EARPA_REG;
	/* Setting up DIGITAL Mute register value */
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_reg_id = HW_CODEC_DIG_REG_ID_MUTE_CTRL;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_reg_addr_msw = 0;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_reg_addr_lsw =
		earpa_config->ear_pa_hw_reg_cfg
	...
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_op[0]
		.hw_codec_op_id = HW_CODEC_OP_DIG_MUTE_ENABLE;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_op[0]
		.hw_codec_op_value = DIG_MUTE_ENABLE;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_op[1]
		.hw_codec_op_id = HW_CODEC_OP_DIG_MUTE_DISABLE;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[0]
		.hw_codec_op[1]
		.hw_codec_op_value = DIG_MUTE_DISABLE;
	/* Setting up LPASS_PA_REG Values */
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_reg_id = HW_CODEC_ANALOG_REG_ID_CMD_FIFO_WRITE;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_reg_addr_msw = 0;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_reg_addr_lsw =
	...
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_op[0]
		.hw_codec_op_id = HW_CODEC_OP_ANA_PGA_ENABLE;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_op[0]
		.hw_codec_op_value =
	...
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_op[1]
		.hw_codec_op_id = HW_CODEC_OP_ANA_PGA_DISABLE;
	prm_rsc_request_reg_info.hw_codec_reg_info_t.hw_codec_reg[1]
		.hw_codec_op[1]
		.hw_codec_op_value =
		earpa_config->ear_pa_pkd_cfg.ear_pa_disable_pkd_reg_addr;

	memcpy(&pkt->payload, &prm_rsc_request_reg_info,
	       sizeof(prm_cmd_request_cdc_duty_cycling_t));
	ret = prm_gpr_send_pkt(pkt, &g_prm.wait);
	kfree(pkt);
	return ret;
...
int audio_prm_set_vote_against_sleep(uint8_t enable)
{
	if (g_prm.prm_sleep_api_supported)
		...
		return _audio_prm_set_lpass_hw_core_req(HW_CORE_ID_LPASS,
							enable);
}
...
static int audio_prm_probe(struct gpr_device *adev)
{
	...
	ret = of_property_read_u32(adev->dev.of_node,
				   "qcom,sleep-api-supported",
				   &g_prm.prm_sleep_api_supported);
	if (ret < 0)
