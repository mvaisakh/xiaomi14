...
static bool csra66x0_addr_is_in_range(unsigned int addr, unsigned int addr_min,
...
struct csra66x0_cluster_device csra_clust_dev_tbl[] = {
	...
	{ NULL, "CSRA_78" }, { NULL, "CSRA_9A" }, { NULL, "CSRA_BC" },
	{ NULL, "CSRA_DE" }, { NULL, "CSRA_F0" }
};
				      ...
				      loff_t *ppos)
{
	...
	if (!(csra66x0_addr_is_in_range(param[0], CSRA66X0_BASE,
					CSRA66X0_MAX_REGISTER_ADDR) ||
	      csra66x0_addr_is_in_range(param[0], CSRA66X0_COEFF_BASE,
					CSRA66X0_MAX_COEFF_ADDR))) {
		dev_err(component->dev,
			"%s: register address 0x%04X out of range\n", __func__,
			param[0]);
		return -EINVAL;
	}
	if ((param[1] < 0) || (param[1] > 255)) {
		dev_err(component->dev,
			"%s: register data 0x%02X out of range\n", __func__,
			param[1]);
		return -EINVAL;
	}
	if (rc == 0) {
		...
		dev_err(component->dev,
			"%s: write to register addr=0x%04X failed\n", __func__,
			param[0]);
	}
	return rc;
}
					 ...
					 loff_t *ppos)
{
	...
	for (i = ((int)*ppos + addr_min); i <= addr_max; i++) {
...
static const DECLARE_TLV_DB_RANGE(csra66x0_bass_treble_tlv, 0, 0,
				  TLV_DB_SCALE_ITEM(0, 0, 0), 1, 15,
				  TLV_DB_SCALE_ITEM(-1500, 100, 0), 16, 30,
...
static const char *const csra66x0_output_invert_text[] = { "UNCHANGED",
							   "INVERTED" };
static const char *const csra66x0_deemp_config_text[] = { "DISABLED",
							  "ENABLED" };
...
SOC_ENUM_SINGLE_DECL(csra66x0_ch2_output_invert_enum,
		     ...
		     csra66x0_output_invert_text);
SOC_ENUM_DOUBLE_DECL(csra66x0_deemp_config_enum, CSRA66X0_DEEMP_CONFIG_FA, 0, 1,
...
static const struct snd_kcontrol_new csra_mix_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_soc_dapm_widget csra66x0_dapm_widgets[] = {
	SND_SOC_DAPM_INPUT("IN"),
	SND_SOC_DAPM_MIXER("MIXER", SND_SOC_NOPM, 0, 0, csra_mix_switch,
			   ARRAY_SIZE(csra_mix_switch)),
...
static int csra66x0_allow_run(struct csra66x0_priv *csra66x0)
{
	...
	if (!csra66x0->in_cluster) {
		/* enable interrupts */
		if (csra66x0->irq) {
			snd_soc_component_write(component, CSRA66X0_PIO0_SELECT,
						0x1);
			if (csra66x0->irq_active_low)
				snd_soc_component_write(
					component, CSRA66X0_IRQ_OUTPUT_POLARITY,
					0x0);
			else
				snd_soc_component_write(
					component, CSRA66X0_IRQ_OUTPUT_POLARITY,
			...
			snd_soc_component_write(
				component, CSRA66X0_IRQ_OUTPUT_ENABLE, 0x01);
		} else {
			snd_soc_component_write(
				component, CSRA66X0_IRQ_OUTPUT_ENABLE, 0x00);
		}
		/* allow run */
	...
	if (csra66x0->num_cluster_devices == component->card->num_aux_devs) {
		...
		for (i = 0; i < component->card->num_aux_devs; i++) {
			...
			if (csra66x0->irq) {
				snd_soc_component_write(
					component, CSRA66X0_PIO0_SELECT, 0x1);
				if (csra66x0->irq_active_low)
					snd_soc_component_write(
						component,
...
static int csra66x0_init(struct csra66x0_priv *csra66x0)
{
	...
	if (ret) {
		dev_err(component->dev,
			"%s: timeout while %s is waiting for config state\n",
...
static int csra66x0_msconfig(struct csra66x0_priv *csra66x0)
{
	...
	if (csra66x0->is_master) {
		...
		snd_soc_component_write(component, CSRA66X0_PIO_PULL_EN0, 0xFF);
		snd_soc_component_write(component, CSRA66X0_PIO_PULL_DIR0,
					0x80);
		snd_soc_component_write(component, CSRA66X0_PIO_PULL_EN1, 0x01);
		snd_soc_component_write(component, CSRA66X0_PIO_PULL_DIR1,
					0x01);
	} else {
		/* Slave specific config */
		snd_soc_component_write(component, CSRA66X0_PIO_PULL_EN0, 0x7F);
		snd_soc_component_write(component, CSRA66X0_PIO_PULL_EN1, 0x00);
	}
...
static int csra66x0_soc_probe(struct snd_soc_component *component)
{
	...
	if (csra66x0->in_cluster) {
		...
		dev_dbg(component->dev,
			"%s: assign prefix %s to component device %s\n",
			__func__, component->name_prefix, component->name);
		...
		for (i = 0; i < csra66x0->max_num_cluster_devices; i++) {
			if (!strncmp(component->name_prefix,
				     ...
				     strnlen(csra_clust_dev_tbl[i]
						     .csra66x0_prefix,
					     sizeof(csra_clust_dev_tbl[i]
							    .csra66x0_prefix)))) {
...
static ssize_t
csra66x0_sysfs_write2reg_addr_value(struct device *dev,
				    ...
				    const char *buf, size_t count)
{
	...
	if (ret != count) {
		dev_err(component->dev,
			"%s: copy input from user space failed. ret=%d\n",
			__func__, ret);
		ret = -EFAULT;
		goto end;
	}
	...
	if (ret) {
		dev_err(component->dev,
			"%s: get sysfs parameter failed. ret=%d\n", __func__,
			ret);
		goto end;
	}
						...
						const char *buf, size_t count)
{
	...
	if (!(csra66x0_addr_is_in_range(reg_addr, CSRA66X0_BASE,
					CSRA66X0_MAX_REGISTER_ADDR) ||
	      csra66x0_addr_is_in_range(reg_addr, CSRA66X0_COEFF_BASE,
					CSRA66X0_MAX_COEFF_ADDR))) {
						...
						char *buf)
{
	...
	ret = snprintf(buf, CSRA66X0_SYSFS_ENTRY_MAX_LEN, "0x%04X\n", reg_addr);
	pr_debug("%s: 0x%04X\n", __func__, reg_addr);
					     ...
					     char *buf)
{
	...
	ret = snprintf(buf, CSRA66X0_SYSFS_ENTRY_MAX_LEN, "0x%04X:	0x%02X\n",
		       csra66x0->sysfs_reg_addr, reg_val);
	pr_debug("%s: 0x%04X: 0x%02X\n", __func__, csra66x0->sysfs_reg_addr,
		 reg_val);
			      ...
			      const struct i2c_device_id *id)
{
	...
	csra66x0->regmap =
		devm_regmap_init_i2c(client_i2c, &csra66x0_regmap_config);
	...
	if (client_i2c->dev.of_node) {
		...
		ret = of_property_read_u32(client_i2c->dev.of_node,
					   "qcom,csra-cluster",
					   &csra66x0->in_cluster);
		if (ret) {
			dev_info(
		...
		ret = of_property_read_u32(client_i2c->dev.of_node,
					   "qcom,csra-cluster-master",
					   &csra66x0->is_master);
		if (ret) {
			dev_info(
				&client_i2c->dev,
				"%s: qcom,csra-cluster-master property not defined in DT, slave "
				"assumed\n",
				__func__);
		...
		/* gpio setup for vreg */
		csra66x0->vreg_gpio = of_get_named_gpio(
			client_i2c->dev.of_node, "qcom,csra-vreg-en-gpio", 0);
		if (!gpio_is_valid(csra66x0->vreg_gpio)) {
			dev_err(&client_i2c->dev,
				"%s: %s property is not found %d\n", __func__,
				"qcom,csra-vreg-en-gpio", csra66x0->vreg_gpio);
			return -ENODEV;
		}
		dev_dbg(&client_i2c->dev, "%s: vreg_en gpio %d\n", __func__,
			csra66x0->vreg_gpio);
		ret = gpio_request(csra66x0->vreg_gpio,
				   dev_name(&client_i2c->dev));
		if (ret) {
			if (ret == -EBUSY) {
		...
		if (client_i2c->irq) {
			...
			ret = devm_request_threaded_irq(
				&client_i2c->dev, csra66x0->irq, NULL,
				csra66x0_irq, irq_trigger | IRQF_ONESHOT,
	...
	csra66x0->debugfs_file_wo = debugfs_create_file(
		...
		(void *)csra66x0, &debugfs_codec_ops);
	...
	if (ret != 0) {
		dev_err(&client_i2c->dev,
			"%s %d: Failed to register component: %d\n", __func__,
			__LINE__, ret);
		if (gpio_is_valid(csra66x0->vreg_gpio)) {
			gpio_set_value(csra66x0->vreg_gpio, 0);
...
static const struct i2c_device_id csra66x0_i2c_id[] = { { "csra66x0", 0 }, {} };
