				    ...
				    unsigned int reg, unsigned int val)
{
	...
	if (wsa881x->regmap_flag) {
		...
		for (i = 0; rc && i < ARRAY_SIZE(delay_array_msec); i++) {
			...
			rc = regmap_write(wsa881x->regmap[wsa881x_index], reg,
					  val);
		}
		if (rc)
				   ...
				   unsigned int reg)
{
	...
	if (wsa881x->regmap_flag) {
		...
		for (i = 0; rc && i < ARRAY_SIZE(delay_array_msec); i++) {
			pr_err_ratelimited(
				"Failed reading reg=%u - retry(%d)\n", reg, i);
			/* retry after delay of increasing order */
			msleep(delay_array_msec[i]);
			rc = regmap_read(wsa881x->regmap[wsa881x_index], reg,
					 &val);
		}
		if (rc) {
	...
	if (enable) {
		if (!WSA881X_IS_2_0(wsa881x->version)) {
			snd_soc_component_update_bits(
				component, WSA881X_ANA_CTL, 0x01, 0x01);
			snd_soc_component_update_bits(
				component, WSA881X_ANA_CTL, 0x04, 0x04);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_PS_CTL, 0x40, 0x00);
			snd_soc_component_update_bits(component,
						      WSA881X_BOOST_PRESET_OUT1,
						      0xF0, 0xB0);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_ZX_CTL, 0x20, 0x00);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_EN_CTL, 0x80, 0x80);
		} else {
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_LOOP_STABILITY, 0x03,
				0x03);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_MISC2_CTL, 0xFF, 0x14);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_START_CTL, 0x80, 0x80);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_START_CTL, 0x03, 0x00);
			snd_soc_component_update_bits(
			...
			if (snd_soc_component_read(component,
						   WSA881X_OTP_REG_0))
				snd_soc_component_update_bits(
					component, WSA881X_BOOST_PRESET_OUT1,
					0xF0, 0x70);
			else
				snd_soc_component_update_bits(
					component, WSA881X_BOOST_PRESET_OUT1,
					0xF0, 0xB0);
			snd_soc_component_update_bits(
				component, WSA881X_ANA_CTL, 0x03, 0x01);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_DRV_EN, 0x08, 0x08);
			snd_soc_component_update_bits(
				component, WSA881X_ANA_CTL, 0x04, 0x04);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_CURRENT_LIMIT, 0x0F,
				0x08);
			snd_soc_component_update_bits(
				component, WSA881X_BOOST_EN_CTL, 0x80, 0x80);
		}
				     ...
				     u8 isense2_gain, u8 vsense_gain)
{
	...
	if (enable) {
		if (WSA881X_IS_2_0(wsa881x->version)) {
			snd_soc_component_update_bits(
				component, WSA881X_OTP_REG_28, 0x3F, 0x3A);
			snd_soc_component_update_bits(component,
		...
		snd_soc_component_update_bits(
			component, WSA881X_SPKR_PROT_FE_VSENSE_VCM, 0x08, 0x00);
		if (WSA881X_IS_2_0(wsa881x->version)) {
			snd_soc_component_update_bits(component,
		...
		value = ((isense2_gain << 6) | (isense1_gain << 4) |
			 (vsense_gain << 3));
		snd_soc_component_update_bits(
			component, WSA881X_SPKR_PROT_FE_GAIN, 0xF8, value);
		snd_soc_component_update_bits(
			component, WSA881X_SPKR_PROT_FE_GAIN, 0x01, 0x01);
	} else {
		if (WSA881X_IS_2_0(wsa881x->version))
			snd_soc_component_update_bits(
     ...
     */
		...
		snd_soc_component_update_bits(
			component, WSA881X_SPKR_PROT_FE_GAIN, 0x01, 0x00);
	}
	return 0;
}
				    ...
				    bool enable)
{
	...
	if (enable) {
		if (!WSA881X_IS_2_0(wsa881x->version))
			snd_soc_component_update_bits(
				component, WSA881X_ADC_SEL_IBIAS, 0x70, 0x40);
		snd_soc_component_update_bits(
			component, WSA881X_ADC_EN_SEL_IBIAS, 0x07, 0x04);
				 ...
				 bool enable)
{
	...
	if (enable) {
		...
		if (wsa881x->bg_cnt == 1) {
			snd_soc_component_update_bits(
				component, WSA881X_TEMP_OP, 0x08, 0x08);
			/* 400usec sleep is needed as per HW requirement */
			usleep_range(400, 410);
			snd_soc_component_update_bits(
				component, WSA881X_TEMP_OP, 0x04, 0x04);
		}
	} else {
		--wsa881x->bg_cnt;
		if (wsa881x->bg_cnt <= 0) {
			wsa881x->bg_cnt = 0;
			snd_soc_component_update_bits(
				component, WSA881X_TEMP_OP, 0x04, 0x00);
			snd_soc_component_update_bits(
				component, WSA881X_TEMP_OP, 0x08, 0x00);
		}
	}
...
static void wsa881x_clk_ctrl(struct snd_soc_component *component, bool enable)
{
	...
	if (enable) {
		...
		if (wsa881x->clk_cnt == 1) {
			snd_soc_component_write(component, WSA881X_CDC_RST_CTL,
						0x02);
			snd_soc_component_write(component, WSA881X_CDC_RST_CTL,
						0x03);
			snd_soc_component_write(component, WSA881X_CLOCK_CONFIG,
						0x01);
		...
		if (wsa881x->clk_cnt <= 0) {
			...
			if (WSA881X_IS_2_0(wsa881x->version))
				snd_soc_component_update_bits(
					component, WSA881X_CDC_TOP_CLK_CTL,
					0x01, 0x00);
		}
	}
...
static int wsa881x_rdac_ctrl(struct snd_soc_component *component, bool enable)
{
	...
	pr_debug("%s: enable:%d\n", __func__, enable);
	if (enable) {
		snd_soc_component_update_bits(component, WSA881X_ANA_CTL, 0x08,
					      0x00);
		...
		if (WSA881X_IS_2_0(wsa881x->version)) {
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_BIAS_CAL, 0x01, 0x01);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_OCP_CTL, 0x30, 0x30);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_OCP_CTL, 0x0C, 0x00);
		}
		snd_soc_component_update_bits(component, WSA881X_SPKR_DRV_GAIN,
					      0xF0, 0x40);
				...
				bool enable)
{
     ...
     */
		if (!WSA881X_IS_2_0(wsa881x->version)) {
			snd_soc_component_update_bits(component,
						      ...
						      0xFF, 0x01);
			snd_soc_component_update_bits(
				component, WSA881X_ADC_EN_MODU_V, 0x02, 0x02);
			snd_soc_component_update_bits(component,
						      WSA881X_ADC_EN_DET_TEST_V,
						      0xFF, 0x10);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_PWRSTG_DBG, 0xA0, 0xA0);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_DRV_EN, 0x80, 0x80);
			usleep_range(700, 710);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_PWRSTG_DBG, 0x00, 0x00);
			snd_soc_component_update_bits(component,
						      WSA881X_ADC_EN_DET_TEST_V,
						      0xFF, 0x00);
			snd_soc_component_update_bits(
				component, WSA881X_ADC_EN_MODU_V, 0x02, 0x00);
			snd_soc_component_update_bits(component,
						      WSA881X_ADC_EN_DET_TEST_I,
						      0xFF, 0x00);
		} else
		...
		if (WSA881X_IS_2_0(wsa881x->version))
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_BIAS_CAL, 0x01, 0x00);
		usleep_range(1000, 1010);
		snd_soc_component_update_bits(component, WSA881X_SPKR_DRV_GAIN,
					      0xF0,
		...
		if (wsa881x->visense_enable) {
			...
			wsa881x_visense_txfe_ctrl(component, true, 0x00, 0x01,
						  0x00);
     ...
     */
		if (WSA881X_IS_2_0(wsa881x->version)) {
			...
			msleep(20);
			snd_soc_component_update_bits(
				component, WSA881X_ANA_CTL, 0x03, 0x00);
			usleep_range(200, 210);
		}
		snd_soc_component_update_bits(component, WSA881X_SPKR_DRV_EN,
					      0x80, 0x00);
...
static const struct snd_kcontrol_new wsa881x_snd_controls[] = {
	SOC_SINGLE_EXT("BOOST Switch", SND_SOC_NOPM, 0, 1, 0, wsa881x_get_boost,
		       wsa881x_set_boost),
...
static const struct snd_kcontrol_new rdac_mux[] = { SOC_DAPM_ENUM("RDAC",
								  rdac_enum) };
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!WSA881X_IS_2_0(wsa881x->version))
			snd_soc_component_update_bits(
				component, WSA881X_BIAS_REF_CTRL, 0x0F, 0x08);
		wsa881x_bandgap_ctrl(component, true);
		if (!WSA881X_IS_2_0(wsa881x->version))
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_BBM_CTL, 0x02, 0x02);
		snd_soc_component_update_bits(component, WSA881X_SPKR_MISC_CTL1,
					      0xC0, 0x80);
		snd_soc_component_update_bits(component, WSA881X_SPKR_MISC_CTL1,
					      0x06, 0x06);
		if (!WSA881X_IS_2_0(wsa881x->version)) {
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_MISC_CTL2, 0x04, 0x04);
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_BIAS_INT, 0x09, 0x09);
		}
		snd_soc_component_update_bits(component, WSA881X_SPKR_PA_INT,
					      0xF0, 0x20);
		if (WSA881X_IS_2_0(wsa881x->version))
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_PA_INT, 0x0E, 0x0E);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (wsa881x->visense_enable) {
			...
			wsa881x_visense_txfe_ctrl(component, false, 0x00, 0x01,
						  0x00);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (ret < 0) {
			pr_err("%s: wsa shutdown failed ret: %d", __func__,
			       ret);
			return ret;
		}
		break;
...
static const struct snd_soc_dapm_widget wsa881x_dapm_widgets[] = {
	...
	SND_SOC_DAPM_DAC_E(
		"RDAC Analog", NULL, SND_SOC_NOPM, 0, 0, wsa881x_rdac_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
			SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX("WSA_RDAC", SND_SOC_NOPM, 0, 0, rdac_mux),

	SND_SOC_DAPM_PGA_S(
		"WSA_SPKR PGA", 1, SND_SOC_NOPM, 0, 0, wsa881x_spkr_pa_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
			SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
				     ...
				     struct wsa_temp_register *wsa_temp_reg)
{
	...
	if (WSA881X_IS_2_0(wsa881x->version)) {
		...
		wsa_temp_reg->dmeas_msb =
			snd_soc_component_read(component, WSA881X_TEMP_MSB);
		wsa_temp_reg->dmeas_lsb =
			snd_soc_component_read(component, WSA881X_TEMP_LSB);
		snd_soc_component_update_bits(component, WSA881X_TADC_VALUE_CTL,
					      0x01, 0x01);
	} else {
		wsa_temp_reg->dmeas_msb = snd_soc_component_read(
			component, WSA881X_TEMP_DOUT_MSB);
		wsa_temp_reg->dmeas_lsb = snd_soc_component_read(
			component, WSA881X_TEMP_DOUT_LSB);
	}
	wsa_temp_reg->d1_msb =
		snd_soc_component_read(component, WSA881X_OTP_REG_1);
	wsa_temp_reg->d1_lsb =
		snd_soc_component_read(component, WSA881X_OTP_REG_2);
	wsa_temp_reg->d2_msb =
		snd_soc_component_read(component, WSA881X_OTP_REG_3);
	wsa_temp_reg->d2_lsb =
		snd_soc_component_read(component, WSA881X_OTP_REG_4);
...
static int wsa881x_probe(struct snd_soc_component *component)
{
	...
	if (!retry)
		dev_err(&client->dev,
			"%s: max retry expired and regmap of\n"
	...
	if (component->name_prefix) {
		widget_name =
			kcalloc(WIDGET_NAME_MAX_SIZE, sizeof(char), GFP_KERNEL);
		if (!widget_name)
			return -ENOMEM;

		snprintf(widget_name, WIDGET_NAME_MAX_SIZE, "%s WSA_SPKR",
			 component->name_prefix);
		snd_soc_dapm_ignore_suspend(dapm, widget_name);
		snprintf(widget_name, WIDGET_NAME_MAX_SIZE, "%s WSA_IN",
			 component->name_prefix);
		snd_soc_dapm_ignore_suspend(dapm, widget_name);
		kfree(widget_name);
	} else {
...
static int wsa881x_populate_dt_pdata(struct device *dev, int wsa881x_index)
{
	...
	pdata->wsa_vi_gpio_p =
		of_parse_phandle(dev->of_node, "qcom,wsa-analog-vi-gpio", 0);
	pdata->wsa_clk_gpio_p =
		of_parse_phandle(dev->of_node, "qcom,wsa-analog-clk-gpio", 0);
	pdata->wsa_reset_gpio_p =
		of_parse_phandle(dev->of_node, "qcom,wsa-analog-reset-gpio", 0);
	pinctrl_init = true;
	return ret;
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (pdata->status == WSA881X_STATUS_I2C) {
		...
		pdata->regmap[WSA881X_ANALOG_SLAVE] = devm_regmap_init_i2c(
			client,
		...
		pdata->regmap[WSA881X_DIGITAL_SLAVE] = devm_regmap_init_i2c(
			client,
		...
		pdata->version =
			wsa881x_i2c_read_device(pdata, WSA881X_CHIP_ID1);
		...
		pdata->driver = devm_kzalloc(
			&client->dev, sizeof(struct snd_soc_component_driver),
			GFP_KERNEL);
		if (!pdata->driver) {
			ret = -ENOMEM;
			goto err1;
		...
		pdata->dai_driver = devm_kzalloc(
			&client->dev, sizeof(struct snd_soc_dai_driver),
			GFP_KERNEL);
		if (!pdata->dai_driver) {
			ret = -ENOMEM;
			goto err_mem;
		...
		snprintf(buffer, sizeof(buffer), "WSA881X_AIF%d Playback",
			 wsa881x_index);
		pdata->dai_driver->playback.stream_name =
			kstrndup(buffer, strlen(buffer), GFP_KERNEL);

		/* Number of DAI's used is 1 */
		ret = snd_soc_register_component(&client->dev, pdata->driver,
						 pdata->dai_driver, 1);

		pdata->wsa881x_name_prefix =
			kstrndup(wsa881x_name_prefix_of,
				 strlen(wsa881x_name_prefix_of), GFP_KERNEL);

		component = snd_soc_lookup_component(&client->dev,
						     pdata->driver->name);
		if (!component) {
...
static const struct dev_pm_ops wsa881x_i2c_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	wsa881x_i2c_suspend, wsa881x_i2c_resume) };
