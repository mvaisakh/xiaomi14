...
enum { MSM_DMIC, SWR_MIC, ANC_FB_TUNE1 };
...
struct tx_macro_priv {
	...
	struct platform_device *pdev_child_devices[TX_MACRO_CHILD_DEVICES_MAX];
	int child_count;
	int tx_swr_clk_cnt;
	int va_swr_clk_cnt;
	int va_clk_status;
...
static int tx_macro_mclk_enable(struct tx_macro_priv *tx_priv, bool mclk_enable)
{
	...
	if (mclk_enable) {
		ret = bolero_clk_rsc_request_clock(tx_priv->dev, TX_CORE_CLK,
						   TX_CORE_CLK, true);
		if (ret < 0) {
		...
		bolero_clk_rsc_fs_gen_request(tx_priv->dev, true);
		regcache_mark_dirty(regmap);
		regcache_sync_region(regmap, TX_START_OFFSET, TX_MAX_OFFSET);
		if (tx_priv->tx_mclk_users == 0) {
			...
			regmap_update_bits(regmap,
					   ...
					   0x01, 0x01);
			regmap_update_bits(
				regmap, BOLERO_CDC_TX_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x01);
			regmap_update_bits(
				regmap,
				BOLERO_CDC_TX_CLK_RST_CTRL_FS_CNT_CONTROL, 0x01,
				0x01);
		}
		...
		bolero_clk_rsc_fs_gen_request(tx_priv->dev, false);
		bolero_clk_rsc_request_clock(tx_priv->dev, TX_CORE_CLK,
					     TX_CORE_CLK, false);
	}
exit:
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		break;
	default:
		dev_err(tx_priv->dev, "%s: invalid DAPM event %d\n", __func__,
			event);
		ret = -EINVAL;
	}
	return ret;
				  ...
				  u16 event, u32 data)
{
	...
	case BOLERO_MACRO_EVT_SSR_DOWN:
		...
		if (tx_priv->swr_ctrl_data) {
			swrm_wcd_notify(tx_priv->swr_ctrl_data[0].tx_swr_pdev,
	...
	case BOLERO_MACRO_EVT_BCS_CLK_OFF:
		if (tx_priv->bcs_clk_en)
			snd_soc_component_update_bits(
				component, BOLERO_CDC_TX0_TX_PATH_SEC7, 0x40,
				data << 6);
		if (data)
...
static int tx_macro_reg_wake_irq(struct snd_soc_component *component, u32 data)
{
	struct device *tx_dev = NULL;
	struct tx_macro_priv *tx_priv = NULL;
	u32 ipc_wakeup = data;
...
static void tx_macro_tx_hpf_corner_freq_callback(struct work_struct *work)
{
	...
	if (is_amic_enabled(component, hpf_work->decimator)) {
		...
		snd_soc_component_update_bits(component, dec_cfg_reg,
					      ...
					      hpf_cut_off_freq << 5);
		snd_soc_component_update_bits(component, hpf_gate_reg, 0x03,
					      0x02);
		...
		if (val != 0) {
			if (val < 5) {
				...
				dmic_clk_reg =
					...
					((val - 5) / 2) * 4;
				snd_soc_component_update_bits(
					component, dmic_clk_reg, 0x0E,
					tx_priv->dmic_clk_div << 0x1);
			}
		}
	} else {
		/* DMIC selected */
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_component_update_bits(component, BOLERO_CDC_TX_TOP_CSR_I2S_CLK,
				      0x1, val);
...
static inline int tx_macro_path_get(const char *wname, unsigned int *path_num)
{
	int ret = 0;
...
static const struct soc_enum bcs_ch_sel_mux_enum = SOC_ENUM_SINGLE_EXT(
	ARRAY_SIZE(bcs_ch_sel_mux_text), bcs_ch_sel_mux_text);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((tx_priv->version == BOLERO_VERSION_2_1) ||
		...
		value = (snd_soc_component_read(
				component, BOLERO_CDC_VA_TOP_CSR_SWR_CTRL)) &
			0x0F;
	else if (tx_priv->version == BOLERO_VERSION_2_0)
		value = (snd_soc_component_read(
				component, BOLERO_CDC_TX_TOP_CSR_SWR_CTRL)) &
			0x0F;
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((tx_priv->version == BOLERO_VERSION_2_1) ||
	    (tx_priv->version == BOLERO_VERSION_2_2))
		snd_soc_component_update_bits(
			component, BOLERO_CDC_VA_TOP_CSR_SWR_CTRL, 0x0F, value);
	else if (tx_priv->version == BOLERO_VERSION_2_0)
		snd_soc_component_update_bits(
			component, BOLERO_CDC_TX_TOP_CSR_SWR_CTRL, 0x0F, value);
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	dec_cfg_reg = BOLERO_CDC_TX0_TX_PATH_CFG0 +
		      TX_MACRO_TX_PATH_OFFSET * decimator;
	tx_gain_ctl_reg =
		BOLERO_CDC_TX0_TX_VOL_CTL + TX_MACRO_TX_PATH_OFFSET * decimator;
	adc_mux_reg = BOLERO_CDC_TX_INP_MUX_ADC_MUX0_CFG1 +
		      TX_MACRO_ADC_MUX_CFG_OFFSET * decimator;
	tx_fs_reg = BOLERO_CDC_TX0_TX_PATH_CTL +
		    TX_MACRO_TX_PATH_OFFSET * decimator;

	tx_priv->pcm_rate[decimator] =
		(snd_soc_component_read(component, tx_fs_reg) & 0x0F);

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(
			component, dec_cfg_reg, 0x06,
			tx_priv->dec_mode[decimator]
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x20,
					      0x20);
       ...
       */
		...
		if (tx_priv->tx_hpf_work[decimator].hpf_cut_off_freq !=
			...
			if (!is_amic_enabled(component, decimator))
				snd_soc_component_update_bits(
					component, hpf_gate_reg, 0x03, 0x00);
			snd_soc_component_update_bits(component, hpf_gate_reg,
						      0x03, 0x01);
			/*
       ...
       */
		...
		if (tx_priv->bcs_enable) {
			...
			if (tx_priv->hs_slow_insert_complete)
				snd_soc_component_update_bits(
					component, BOLERO_CDC_TX0_TX_PATH_SEC7,
					0x40, 0x40);
		}
		if (tx_priv->version == BOLERO_VERSION_2_0) {
			if (snd_soc_component_read(component, adc_mux_reg) &
				...
				snd_soc_component_update_bits(
					component,
					BOLERO_CDC_TX_TOP_CSR_SWR_CTRL, 0x01,
					0x01);
				snd_soc_component_update_bits(
					component,
	...
	case SND_SOC_DAPM_PRE_PMD:
         ...
         */
				usleep_range(1000, 1010);
				snd_soc_component_update_bits(
					component, hpf_gate_reg, 0x03, 0x01);
			}
		}
	...
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x20,
					      0x00);
		snd_soc_component_update_bits(component, dec_cfg_reg, 0x06,
					      0x00);
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x40,
					      0x40);
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x40,
					      0x00);
		snd_soc_component_update_bits(component, tx_vol_ctl_reg, 0x10,
...
static const char *const adc_mux_text[] = { "MSM_DMIC", "SWR_MIC",
					    "ANC_FB_TUNE1" };

TX_MACRO_DAPM_ENUM(tx_dec0, BOLERO_CDC_TX_INP_MUX_ADC_MUX0_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec1, BOLERO_CDC_TX_INP_MUX_ADC_MUX1_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec2, BOLERO_CDC_TX_INP_MUX_ADC_MUX2_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec3, BOLERO_CDC_TX_INP_MUX_ADC_MUX3_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec4, BOLERO_CDC_TX_INP_MUX_ADC_MUX4_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec5, BOLERO_CDC_TX_INP_MUX_ADC_MUX5_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec6, BOLERO_CDC_TX_INP_MUX_ADC_MUX6_CFG1, 0,
		   adc_mux_text);
TX_MACRO_DAPM_ENUM(tx_dec7, BOLERO_CDC_TX_INP_MUX_ADC_MUX7_CFG1, 0,
		   adc_mux_text);

static const char *const dmic_mux_text[] = { "ZERO",  "DMIC0", "DMIC1",
					     "DMIC2", "DMIC3", "DMIC4",
					     "DMIC5", "DMIC6", "DMIC7" };

TX_MACRO_DAPM_ENUM_EXT(tx_dmic0, BOLERO_CDC_TX_INP_MUX_ADC_MUX0_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic1, BOLERO_CDC_TX_INP_MUX_ADC_MUX1_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic2, BOLERO_CDC_TX_INP_MUX_ADC_MUX2_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic3, BOLERO_CDC_TX_INP_MUX_ADC_MUX3_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic4, BOLERO_CDC_TX_INP_MUX_ADC_MUX4_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic5, BOLERO_CDC_TX_INP_MUX_ADC_MUX5_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic6, BOLERO_CDC_TX_INP_MUX_ADC_MUX6_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_dmic7, BOLERO_CDC_TX_INP_MUX_ADC_MUX7_CFG0, 4,
		       dmic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

static const char *const smic_mux_text[] = {
	"ZERO",	     "ADC0",	  "ADC1",      "ADC2",	    "ADC3",
	"SWR_DMIC0", "SWR_DMIC1", "SWR_DMIC2", "SWR_DMIC3", "SWR_DMIC4",
	"SWR_DMIC5", "SWR_DMIC6", "SWR_DMIC7"
};

TX_MACRO_DAPM_ENUM_EXT(tx_smic0, BOLERO_CDC_TX_INP_MUX_ADC_MUX0_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic1, BOLERO_CDC_TX_INP_MUX_ADC_MUX1_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic2, BOLERO_CDC_TX_INP_MUX_ADC_MUX2_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic3, BOLERO_CDC_TX_INP_MUX_ADC_MUX3_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic4, BOLERO_CDC_TX_INP_MUX_ADC_MUX4_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic5, BOLERO_CDC_TX_INP_MUX_ADC_MUX5_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic6, BOLERO_CDC_TX_INP_MUX_ADC_MUX6_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic7, BOLERO_CDC_TX_INP_MUX_ADC_MUX7_CFG0, 0,
		       smic_mux_text, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

static const char *const smic_mux_text_v2[] = {
	"ZERO",	    "SWR_MIC0",	 "SWR_MIC1", "SWR_MIC2", "SWR_MIC3",
	"SWR_MIC4", "SWR_MIC5",	 "SWR_MIC6", "SWR_MIC7", "SWR_MIC8",
	"SWR_MIC9", "SWR_MIC10", "SWR_MIC11"
};

TX_MACRO_DAPM_ENUM_EXT(tx_smic0_v2, BOLERO_CDC_TX_INP_MUX_ADC_MUX0_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic1_v2, BOLERO_CDC_TX_INP_MUX_ADC_MUX1_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic2_v2, BOLERO_CDC_TX_INP_MUX_ADC_MUX2_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic3_v2, BOLERO_CDC_TX_INP_MUX_ADC_MUX3_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic4_v3, BOLERO_CDC_TX_INP_MUX_ADC_MUX4_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic5_v3, BOLERO_CDC_TX_INP_MUX_ADC_MUX5_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic6_v3, BOLERO_CDC_TX_INP_MUX_ADC_MUX6_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic7_v3, BOLERO_CDC_TX_INP_MUX_ADC_MUX7_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum);

TX_MACRO_DAPM_ENUM_EXT(tx_smic0_v4, BOLERO_CDC_TX_INP_MUX_ADC_MUX0_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum_v2);

TX_MACRO_DAPM_ENUM_EXT(tx_smic1_v4, BOLERO_CDC_TX_INP_MUX_ADC_MUX1_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum_v2);

TX_MACRO_DAPM_ENUM_EXT(tx_smic2_v4, BOLERO_CDC_TX_INP_MUX_ADC_MUX2_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum_v2);

TX_MACRO_DAPM_ENUM_EXT(tx_smic3_v4, BOLERO_CDC_TX_INP_MUX_ADC_MUX3_CFG0, 0,
		       smic_mux_text_v2, snd_soc_dapm_get_enum_double,
		       tx_macro_put_dec_enum_v2);
...
TX_MACRO_DAPM_ENUM_EXT(rx_swr_tx_pcm_in0, SND_SOC_NOPM, 0, pcm_in0_mux_text,
		       snd_soc_dapm_get_enum_double, tx_macro_put_pcm_in_enum);

TX_MACRO_DAPM_ENUM_EXT(rx_swr_tx_pcm_in1, SND_SOC_NOPM, 0, pcm_in1_mux_text,
		       snd_soc_dapm_get_enum_double, tx_macro_put_pcm_in_enum);
...
static const struct soc_enum dec_mode_mux_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(dec_mode_mux_text), dec_mode_mux_text);
...
static const struct soc_enum bcs_ch_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(bcs_ch_enum_text), bcs_ch_enum_text);
...
static const struct snd_soc_dapm_widget tx_macro_dapm_widgets_common[] = {
	SND_SOC_DAPM_AIF_OUT("TX_AIF1 CAP", "TX_AIF1 Capture", 0, SND_SOC_NOPM,
			     TX_MACRO_AIF1_CAP, 0),

	SND_SOC_DAPM_AIF_OUT("TX_AIF2 CAP", "TX_AIF2 Capture", 0, SND_SOC_NOPM,
			     TX_MACRO_AIF2_CAP, 0),

	SND_SOC_DAPM_AIF_OUT("TX_AIF3 CAP", "TX_AIF3 Capture", 0, SND_SOC_NOPM,
			     TX_MACRO_AIF3_CAP, 0),
	...
	SND_SOC_DAPM_MUX_E("TX DEC0 MUX", SND_SOC_NOPM, TX_MACRO_DEC0, 0,
			   &tx_dec0_mux, tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC1 MUX", SND_SOC_NOPM, TX_MACRO_DEC1, 0,
			   &tx_dec1_mux, tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC2 MUX", SND_SOC_NOPM, TX_MACRO_DEC2, 0,
			   &tx_dec2_mux, tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC3 MUX", SND_SOC_NOPM, TX_MACRO_DEC3, 0,
...
static const struct snd_soc_dapm_widget tx_macro_dapm_widgets_v2[] = {
	SND_SOC_DAPM_MIXER("TX_AIF1_CAP Mixer", SND_SOC_NOPM, TX_MACRO_AIF1_CAP,
			   0, tx_aif1_cap_mixer_v2,
			   ARRAY_SIZE(tx_aif1_cap_mixer_v2)),

	SND_SOC_DAPM_MIXER("TX_AIF2_CAP Mixer", SND_SOC_NOPM, TX_MACRO_AIF2_CAP,
			   0, tx_aif2_cap_mixer_v2,
			   ARRAY_SIZE(tx_aif2_cap_mixer_v2)),

	SND_SOC_DAPM_MIXER("TX_AIF3_CAP Mixer", SND_SOC_NOPM, TX_MACRO_AIF3_CAP,
			   0, tx_aif3_cap_mixer_v2,
			   ARRAY_SIZE(tx_aif3_cap_mixer_v2)),
...
static const struct snd_soc_dapm_widget tx_macro_dapm_widgets_v3[] = {
	SND_SOC_DAPM_MIXER("TX_AIF1_CAP Mixer", SND_SOC_NOPM, TX_MACRO_AIF1_CAP,
			   0, tx_aif1_cap_mixer, ARRAY_SIZE(tx_aif1_cap_mixer)),

	SND_SOC_DAPM_MIXER("TX_AIF2_CAP Mixer", SND_SOC_NOPM, TX_MACRO_AIF2_CAP,
			   0, tx_aif2_cap_mixer, ARRAY_SIZE(tx_aif2_cap_mixer)),

	SND_SOC_DAPM_MIXER("TX_AIF3_CAP Mixer", SND_SOC_NOPM, TX_MACRO_AIF3_CAP,
			   0, tx_aif3_cap_mixer, ARRAY_SIZE(tx_aif3_cap_mixer)),
	...
	SND_SOC_DAPM_MUX_E("TX DEC4 MUX", SND_SOC_NOPM, TX_MACRO_DEC4, 0,
			   &tx_dec4_mux, tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC5 MUX", SND_SOC_NOPM, TX_MACRO_DEC5, 0,
			   &tx_dec5_mux, tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC6 MUX", SND_SOC_NOPM, TX_MACRO_DEC6, 0,
			   &tx_dec6_mux, tx_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("TX DEC7 MUX", SND_SOC_NOPM, TX_MACRO_DEC7, 0,
...
static const struct snd_kcontrol_new tx_macro_snd_controls_common[] = {
	SOC_SINGLE_S8_TLV("TX_DEC0 Volume", BOLERO_CDC_TX0_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC1 Volume", BOLERO_CDC_TX1_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC2 Volume", BOLERO_CDC_TX2_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC3 Volume", BOLERO_CDC_TX3_TX_VOL_CTL, -84, 40,
			  digital_gain),

	SOC_SINGLE_EXT("TX LPI Enable", 0, 0, 1, 0, tx_macro_lpi_get,
		       tx_macro_lpi_put),

	SOC_ENUM_EXT("DEC0 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_ENUM_EXT("DEC1 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_ENUM_EXT("DEC2 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_ENUM_EXT("DEC3 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_SINGLE_EXT("DEC0_BCS Switch", SND_SOC_NOPM, 0, 1, 0,
		       tx_macro_get_bcs, tx_macro_set_bcs),

	SOC_ENUM_EXT("BCS Channel", bcs_ch_enum, tx_macro_bcs_ch_get,
		     tx_macro_bcs_ch_put),

	SOC_ENUM_EXT("BCS CH_SEL", bcs_ch_sel_mux_enum, tx_macro_get_bcs_ch_sel,
		     tx_macro_put_bcs_ch_sel),
};

static const struct snd_kcontrol_new tx_macro_snd_controls_v3[] = {
	SOC_SINGLE_S8_TLV("TX_DEC4 Volume", BOLERO_CDC_TX4_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC5 Volume", BOLERO_CDC_TX5_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC6 Volume", BOLERO_CDC_TX6_TX_VOL_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("TX_DEC7 Volume", BOLERO_CDC_TX7_TX_VOL_CTL, -84, 40,
			  digital_gain),

	SOC_ENUM_EXT("DEC4 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_ENUM_EXT("DEC5 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_ENUM_EXT("DEC6 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),

	SOC_ENUM_EXT("DEC7 MODE", dec_mode_mux_enum, tx_macro_dec_mode_get,
		     tx_macro_dec_mode_put),
};
				      ...
				      bool enable)
{
	...
	if (enable) {
		...
		clk_tx_ret = bolero_clk_rsc_request_clock(
			tx_priv->dev, TX_CORE_CLK, TX_CORE_CLK, true);
		if (clk_type == TX_MCLK) {
		...
		if (clk_type == VA_MCLK) {
			...
			ret = bolero_clk_rsc_request_clock(
				tx_priv->dev, TX_CORE_CLK, VA_CORE_CLK, true);
			if (ret < 0) {
				if (tx_priv->swr_clk_users == 0)
					msm_cdc_pinctrl_select_sleep_state(
						tx_priv->tx_swr_gpio_p);
			...
			if (tx_priv->tx_mclk_users == 0) {
				regmap_update_bits(
					regmap, BOLERO_CDC_TX_TOP_CSR_FREQ_MCLK,
					0x01, 0x01);
				regmap_update_bits(
					regmap,
		...
		if (tx_priv->swr_clk_users == 0) {
			...
			if (tx_priv->reset_swr)
				regmap_update_bits(
					...
					0x02, 0x02);
			regmap_update_bits(
				regmap, BOLERO_CDC_TX_CLK_RST_CTRL_SWR_CONTROL,
				0x01, 0x01);
			if (tx_priv->reset_swr)
				regmap_update_bits(
					regmap,
		...
		if (!clk_tx_ret)
			ret = bolero_clk_rsc_request_clock(
				tx_priv->dev, TX_CORE_CLK, TX_CORE_CLK, false);
		...
		if (clk_type == VA_MCLK) {
			...
			bolero_clk_rsc_fs_gen_request(tx_priv->dev, false);
			ret = bolero_clk_rsc_request_clock(
				tx_priv->dev, TX_CORE_CLK, VA_CORE_CLK, false);
			if (ret < 0) {
				dev_err_ratelimited(
					tx_priv->dev,
					"%s: swr request clk failed\n",
...
static int tx_macro_clk_switch(struct snd_soc_component *component, int clk_src)
{
	...
	dev_dbg(component->dev,
		...
		__func__, tx_priv->va_swr_clk_cnt, tx_priv->tx_swr_clk_cnt,
	...
	if (enable) {
		...
		if (tx_priv->va_clk_status && !tx_priv->tx_clk_status) {
			...
			if (!tx_priv->va_swr_clk_cnt &&
			    tx_priv->tx_swr_clk_cnt) {
				ret = tx_macro_tx_va_mclk_enable(
					tx_priv, regmap, VA_MCLK, enable);
				if (ret)
					goto done;
				--tx_priv->va_clk_status;
			} else {
				ret = tx_macro_tx_va_mclk_enable(
					tx_priv, regmap, TX_MCLK, enable);
				if (ret)
					goto done;
				--tx_priv->tx_clk_status;
			}
			...
			dev_dbg(tx_priv->dev, "%s: Both clocks are disabled\n",
				__func__);
		}
	}
...
static int tx_macro_init(struct snd_soc_component *component)
{
	...
	if (tx_priv->version >= BOLERO_VERSION_2_0) {
		ret = snd_soc_dapm_new_controls(
			dapm, tx_macro_dapm_widgets_common,
	...
	if (tx_priv->version >= BOLERO_VERSION_2_0) {
		ret = snd_soc_dapm_add_routes(dapm, tx_audio_map_common,
	...
	if (tx_priv->version >= BOLERO_VERSION_2_0) {
		ret = snd_soc_add_component_controls(
			component, tx_macro_snd_controls_common,
		...
		if (tx_priv->version == BOLERO_VERSION_2_0)
			ret = snd_soc_add_component_controls(
				component, tx_macro_snd_controls_v3,
		...
		ret = snd_soc_add_component_controls(
			component, tx_macro_snd_controls,
	...
	for_each_available_child_of_node(tx_priv->dev->of_node, node) {
		...
		if (tx_swr_master_node) {
			...
			temp = krealloc(
				swr_ctrl_data,
				(ctrl_num + 1) *
					sizeof(struct tx_macro_swr_ctrl_data),
				GFP_KERNEL);
			if (!temp) {
				ret = -ENOMEM;
				goto fail_pdev_add;
		...
		if (tx_priv->child_count < TX_MACRO_CHILD_DEVICES_MAX)
			tx_priv->pdev_child_devices[tx_priv->child_count++] =
				pdev;
		else
			goto err;
	}
...
static void tx_macro_init_ops(struct macro_ops *ops, char __iomem *tx_io_base)
{
...
static int tx_macro_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "reg", &tx_base_addr);
	if (ret) {
	...
	if (of_find_property(pdev->dev.of_node, is_used_tx_swr_gpio_dt, NULL)) {
	...
	tx_priv->tx_swr_gpio_p =
		of_parse_phandle(pdev->dev.of_node, "qcom,tx-swr-gpios", 0);
	...
	tx_io_base =
		devm_ioremap(&pdev->dev, tx_base_addr, TX_MACRO_MAX_OFFSET);
	if (!tx_io_base) {
	...
	if (ret) {
		...
		if (tx_macro_validate_dmic_sample_rate(sample_rate, tx_priv) ==
		    TX_MACRO_DMIC_SAMPLE_RATE_UNDEFINED)
			return -EINVAL;
	}
...
static int tx_macro_remove(struct platform_device *pdev)
{
	...
	if (tx_priv->is_used_tx_swr_gpio) {
		...
		for (count = 0; count < tx_priv->child_count &&
				count < TX_MACRO_CHILD_DEVICES_MAX;
		     count++)
