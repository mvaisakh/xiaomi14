...
static bool bolero_is_readable_register(struct device *dev, unsigned int reg)
{
	...
	macro_id = bolero_get_macro_id(priv->va_without_decimation, reg);
...
static bool bolero_is_writeable_register(struct device *dev, unsigned int reg)
{
	struct bolero_priv *priv = dev_get_drvdata(dev);
	u16 reg_offset;
	int macro_id;
...
static bool bolero_is_volatile_register(struct device *dev, unsigned int reg)
{
