...
#define WCD939X_COMP_OFFSET (WCD939X_R_BASE - WCD939X_COMPANDER_HPHL_BASE)
...
static u8 tx_mode_bit[] = {
	[ADC_MODE_INVALID] = 0x00, [ADC_MODE_HIFI] = 0x01,
	[ADC_MODE_LO_HIF] = 0x02,  [ADC_MODE_NORMAL] = 0x04,
	[ADC_MODE_LP] = 0x08,	   [ADC_MODE_ULP1] = 0x10,
	[ADC_MODE_ULP2] = 0x20,
};
...
static bool wcd939x_readable_register(struct device *dev, unsigned int reg)
{
	...
	if (reg >= WCD939X_FLYBACK_NEW_CTRL_2 &&
	    reg <= WCD939X_FLYBACK_NEW_CTRL_4) {
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		wcd939x->compander_enabled[compander];
	return 0;
}
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	int xtalk = ((struct soc_multi_mixer_control *)kcontrol->private_value)
			    ->shift;
				    ...
				    int rate, int bank)
{
	...
	snd_soc_component_update_bits(component, WCD939X_SWR_TX_CLK_RATE, mask,
				      val);
...
static int wcd939x_init_reg(struct snd_soc_component *component)
{
	...
	snd_soc_component_update_bits(
		component, REG_FIELD_VALUE(BIAS, ANALOG_BIAS_EN, 0x01));
	...
	snd_soc_component_update_bits(component,
				      REG_FIELD_VALUE(RDAC_HD2_CTL_L,
						      HD2_RES_DIV_CTL_L, 0x15));
	snd_soc_component_update_bits(component,
				      REG_FIELD_VALUE(RDAC_HD2_CTL_R,
						      HD2_RES_DIV_CTL_R, 0x15));
	snd_soc_component_update_bits(
		component, REG_FIELD_VALUE(CDC_DMIC_CTL, CLK_SCALE_EN, 0x01));

	snd_soc_component_update_bits(
		component, REG_FIELD_VALUE(TXFE_ICTRL_STG2CASC_ULP,
					   ICTRL_SCBIAS_ULP0P6M, 0x1));
	snd_soc_component_update_bits(component,
				      REG_FIELD_VALUE(TXFE_ICTRL_STG2CASC_ULP,
						      ICTRL_STG2CASC_ULP, 0x4));

	snd_soc_component_update_bits(
		component, REG_FIELD_VALUE(TXFE_ICTRL_STG2MAIN_ULP,
					   ICTRL_STG2MAIN_ULP, 0x08));
	...
	if (of_find_property(component->card->dev->of_node,
		...
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(HPHLR_SURGE_EN,
					EN_SURGE_PROTECTION_HPHL, 0x00));
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(HPHLR_SURGE_EN,
					EN_SURGE_PROTECTION_HPHR, 0x00));
	} else {
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(HPHLR_SURGE_EN,
					EN_SURGE_PROTECTION_HPHL, 0x01));
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(HPHLR_SURGE_EN,
					EN_SURGE_PROTECTION_HPHR, 0x01));
	}

	snd_soc_component_update_bits(
		component, REG_FIELD_VALUE(HPH_OCP_CTL, OCP_FSM_EN, 0x01));
	snd_soc_component_update_bits(
		component, REG_FIELD_VALUE(HPH_OCP_CTL, SCD_OP_EN, 0x01));
   ...
   */
	snd_soc_component_update_bits(component, WCD939X_ZDET_BIAS_CTL, 0x80,
				      0x80);
	return 0;
}
...
found:
	if (i > num_ports || j == MAX_CH_PER_PORT) {
		dev_err_ratelimited(
			component->dev,
			"%s Failed to find slave port for type %u\n", __func__,
			slv_prt_type);
		return -EINVAL;
	}
	*port_id = i;
...
/* qcom,swr-tx-port-params = <OFFSET1_VAL0 LANE1>, <OFFSET1_VAL5 LANE0>,
   <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>,*UC0* <OFFSET1_VAL0 LANE1>,
   <OFFSET1_VAL2 LANE0>, <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>, *UC1*
                        <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>,
   <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>; *UC2* <OFFSET1_VAL1 LANE0>,
   <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>; *UC3 */
static int wcd939x_parse_port_params(struct device *dev, char *prop, u8 path)
{
...
static int wcd939x_parse_port_mapping(struct device *dev, char *prop, u8 path)
{
...
static int wcd939x_tx_connect_port(struct snd_soc_component *component,
				   u8 slv_port_type, int clk_rate, u8 enable)
{
	struct wcd939x_priv *wcd939x = snd_soc_component_get_drvdata(component);
	...
	ret = wcd939x_set_port_params(component, slv_port_type, &port_id,
				      &num_ch, &ch_mask, &ch_rate, &ch_type,
				      CODEC_TX);
	if (ret)
		return ret;
	...
	if (enable)
		ret = swr_connect_port(wcd939x->tx_swr_dev, &port_id, num_port,
				       &ch_mask, &ch_rate, &num_ch, &ch_type);
	else
				   ...
				   u8 slv_port_type, u8 enable)
{
	...
	if (enable)
		ret = swr_connect_port(wcd939x->rx_swr_dev, &port_id, num_port,
				 ...
				 int rx_num)
{
	...
	if (wcd939x->rx_clk_cnt == 0) {
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(CDC_ANA_CLK_CTL,
						   ANA_RX_DIV2_CLK_EN, 0x01));
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(CDC_ANA_CLK_CTL,
						   ANA_RX_DIV4_CLK_EN, 0x01));
...
static int wcd939x_rx_clk_disable(struct snd_soc_component *component)
{
	...
	if (wcd939x->rx_clk_cnt == 0) {
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(RX_SUPPLIES, VNEG_EN, 0x00));
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(RX_SUPPLIES, VPOS_EN, 0x00));
		snd_soc_component_update_bits(
			component,
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(CDC_ANA_CLK_CTL,
						   ANA_RX_DIV4_CLK_EN, 0x00));
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(CDC_ANA_CLK_CTL,
						   ANA_RX_DIV2_CLK_EN, 0x00));
		snd_soc_component_update_bits(
			component,
				     ...
				     int event, int index, int mode)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (mode == CLS_H_ULP) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(REFBUFF_UHQA_CTL,
						REFBUFP_IOUT_CTL, 0x1));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(REFBUFF_UHQA_CTL,
						REFBUFN_IOUT_CTL, 0x1));

			if (wcd939x->compander_enabled[index]) {
				if (index == WCD939X_HPHL) {
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(CTL12,
								ZONE3_RMS,
								0x21));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(CTL13,
								ZONE4_RMS,
								0x30));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(CTL14,
								ZONE5_RMS,
								0x3F));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(CTL15,
								ZONE6_RMS,
								0x48));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(CTL17,
								PATH_GAIN,
								0x0C));
				} else if (index == WCD939X_HPHR) {
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(R_CTL12,
								ZONE3_RMS,
								0x21));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(R_CTL13,
								ZONE4_RMS,
								0x30));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(R_CTL14,
								ZONE5_RMS,
								0x3F));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(R_CTL15,
								ZONE6_RMS,
								0x48));
					snd_soc_component_update_bits(
						component,
						REG_FIELD_VALUE(R_CTL17,
								PATH_GAIN,
								0x0C));
				}
			}
	...
	case SND_SOC_DAPM_POST_PMD:
		if (mode == CLS_H_ULP) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(REFBUFF_UHQA_CTL,
						REFBUFN_IOUT_CTL, 0x0));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(REFBUFF_UHQA_CTL,
						REFBUFP_IOUT_CTL, 0x0));
		}
		break;
	}
	return 0;
			...
			if (wcd939x->rx_clk_config == RX_CLK_11P2896MHZ)
				...
				snd_soc_component_update_bits(
					...
					REG_FIELD_VALUE(HPHL_RX_PATH_CFG1,
							...
							0x3));
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(HPHL_RX_PATH_CFG0,
							   DLY_ZN_EN, 0x1));
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(HPHL_RX_PATH_CFG0,
							   INT_EN, 0x3));
			...
			if (wcd939x->rx_clk_config == RX_CLK_11P2896MHZ)
				...
				snd_soc_component_update_bits(
					...
					REG_FIELD_VALUE(HPHR_RX_PATH_CFG1,
							...
							0x3));
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(HPHR_RX_PATH_CFG0,
							   DLY_ZN_EN, 0x1));
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(HPHR_RX_PATH_CFG0,
							   INT_EN, 0x3));
				    ...
				    int event, int compander_indx)
{
	...
	if (!wcd939x->compander_enabled[compander_indx]) {
		...
		if (compander_indx == WCD939X_HPHL) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(L_EN, GAIN_SOURCE_SEL,
						gain_source_sel));
		} else if (compander_indx == WCD939X_HPHR) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(R_EN, GAIN_SOURCE_SEL,
						gain_source_sel));
		}
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		snd_soc_component_update_bits(component, comp_ctl7_reg, 0x1E,
					      0x00);
		...
		/* Enable compander*/
		snd_soc_component_update_bits(component, WCD939X_CDC_COMP_CTL_0,
					      comp_en_mask_val,
					      comp_en_mask_val);

	} else if (SND_SOC_DAPM_EVENT_OFF(event)) {
		snd_soc_component_update_bits(component, WCD939X_CDC_COMP_CTL_0,
					      comp_en_mask_val, 0x00);
		snd_soc_component_update_bits(component, comp_ctl0_reg, 0x01,
					      0x00);
		if (compander_indx == WCD939X_HPHL)
			snd_soc_component_update_bits(
...
static int wcd939x_config_xtalk(struct snd_soc_component *component, int event,
				int xtalk_indx)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		xtalk_sec0 = WCD939X_HPHL_RX_PATH_SEC0 +
			     (xtalk_indx * WCD939X_XTALK_OFFSET);
		xtalk_sec1 = WCD939X_HPHL_RX_PATH_SEC1 +
			     (xtalk_indx * WCD939X_XTALK_OFFSET);
		xtalk_sec2 = WCD939X_HPHL_RX_PATH_SEC2 +
			     (xtalk_indx * WCD939X_XTALK_OFFSET);
		xtalk_sec3 = WCD939X_HPHL_RX_PATH_SEC3 +
			     (xtalk_indx * WCD939X_XTALK_OFFSET);

		/* Write scale and alpha based on channel */
		if (xtalk_indx == XTALK_L_CH_NUM) {
			snd_soc_component_update_bits(
				component, xtalk_sec1, 0xFF,
				pdata->usbcss_hs.xtalk.alpha_l);
			snd_soc_component_update_bits(
				component, xtalk_sec0, 0x1F,
			...
			snd_soc_component_update_bits(component, xtalk_sec1,
						      0xFF, MIN_XTALK_ALPHA);
			snd_soc_component_update_bits(component, xtalk_sec0,
						      0x1F, MAX_XTALK_SCALE);
		}

		dev_dbg(component->dev, "%s Scale = 0x%x, Alpha = 0x%x\n",
			__func__, snd_soc_component_read(component, xtalk_sec0),
		...
		//			snd_soc_component_update_bits(component, xtalk_sec1,
		//0xFF, 0x42); 			snd_soc_component_update_bits(component, xtalk_sec0, 0x1F,
		//0x05);
		//		}
		/* for HPHR XTALK */
		//		if(xtalk_indx == 1) {
		...
		//			snd_soc_component_update_bits(component, xtalk_sec1,
		//0xFF, 0x44); 			snd_soc_component_update_bits(component, xtalk_sec0, 0x1F,
		//0x05);
		//		}
		...
		/* xtalk_sec1: WCD939X_HPHL_RX_PATH_SEC1 - 0x3522 */
		//			snd_soc_component_update_bits(component, xtalk_sec1,
		//0xFF, 0x0B); 			snd_soc_component_update_bits(component, xtalk_sec0, 0x1F,
		//0x05);
		//		}
		/* for HPHR XTALK */
		//		if(xtalk_indx == 1) {
		...
		snd_soc_component_update_bits(component, xtalk_sec3, 0xFF,
					      0x4F);
		snd_soc_component_update_bits(component, xtalk_sec2, 0x1F,
					      0x11);
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		wcd939x_rx_clk_enable(component, w->shift);
		if (wcd939x->hph_pcm_enabled)
			wcd939x_config_power_mode(component, event, w->shift,
						  hph_mode);
	...
	case SND_SOC_DAPM_POST_PMU:
		wcd939x_config_xtalk(component, event, w->shift);
		/*TBD: need to revisit , for both L & R we are updating, but in QCRG only
     * once*/
		if (wcd939x->hph_pcm_enabled) {
			if (hph_mode == CLS_H_HIFI || hph_mode == CLS_AB_HIFI)
				snd_soc_component_update_bits(
					component,
					REG_FIELD_VALUE(TOP_CFG0,
							HPH_DAC_RATE_SEL, 0x1));
			else
				snd_soc_component_update_bits(
					component,
					REG_FIELD_VALUE(TOP_CFG0,
							HPH_DAC_RATE_SEL, 0x0));
		}
				     ...
				     int mode_2vpk)
{
	...
	if (rc) {
		dev_err_ratelimited(component->dev,
				    "%s: Unable to get impedance for 2Vpk mode",
				    __func__);
		return;
	}
	...
	case SUPPLY_LEVEL_2VPK:
		...
		if (zl < HPH_IMPEDANCE_2VPK_MODE_OHMS)
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(PA_GAIN_CTL_L, EN_HPHPA_2VPK,
						0x00));
		else
			snd_soc_component_update_bits(
				component,
	...
	case REGULATOR_MODE_2VPK:
		if (zl >= HPH_IMPEDANCE_2VPK_MODE_OHMS) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(RX_SUPPLIES, REGULATOR_MODE,
						0x01));
			snd_soc_component_update_bits(component,
						      WCD939X_FLYBACK_TEST_CTL,
						      0x0F, 0x02);
		} else {
			snd_soc_component_update_bits(component,
						      WCD939X_FLYBACK_TEST_CTL,
	...
	case SET_HPH_GAIN_2VPK:
		if (zl >= HPH_IMPEDANCE_2VPK_MODE_OHMS) {
			snd_soc_component_update_bits(
				component, WCD939X_PA_GAIN_CTL_L, 0x1F, 0x02);
			snd_soc_component_update_bits(
				component, WCD939X_PA_GAIN_CTL_R, 0x1F, 0x02);
		}
					...
					int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (!wcd939x->hph_pcm_enabled)
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(RDAC_CLK_CTL1,
						OPAMP_CHOP_CLK_EN, 0x00));
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(RDAC_HD2_CTL_L,
						   HD2_RES_DIV_CTL_L, 0x1D));
		if (!wcd939x->hph_pcm_enabled) {
			if (wcd939x->comp1_enable) {
				snd_soc_component_update_bits(
					component,
					REG_FIELD_VALUE(CDC_COMP_CTL_0,
							HPHL_COMP_EN, 0x01));
				/* 5msec compander delay as per HW requirement */
				if (!wcd939x->comp2_enable ||
				    (snd_soc_component_read(
					     component,
					     WCD939X_CDC_COMP_CTL_0) &
				     0x01))
					usleep_range(5000, 5010);
				snd_soc_component_update_bits(
					component,
					REG_FIELD_VALUE(HPH_TIMER1,
							AUTOCHOP_TIMER_CTL_EN,
							0x00));
			} else {
				snd_soc_component_update_bits(
					component,
		...
		if (wcd939x->hph_pcm_enabled) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(HPH_TIMER1,
						AUTOCHOP_TIMER_CTL_EN, 0x00));
			snd_soc_component_write(component, WCD939X_VNEG_CTRL_1,
						0xEB);
			if (wcd939x->hph_mode == CLS_H_LOHIFI)
				snd_soc_component_write(
					component, WCD939X_HPH_RDAC_BIAS_LOHIFI,
					0x52);
			else
				snd_soc_component_write(
					component, WCD939X_HPH_RDAC_BIAS_LOHIFI,
					...
					int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(RDAC_HD2_CTL_R,
						   HD2_RES_DIV_CTL_R, 0x1D));
		if (!wcd939x->hph_pcm_enabled) {
			if (wcd939x->comp1_enable) {
				snd_soc_component_update_bits(
					component,
				       ...
				       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(component,
					      REG_FIELD_VALUE(CDC_EAR_GAIN_CTL,
							      EAR_EN, 0x01));
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(CDC_DIG_CLK_CTL, RXD2_CLK_EN, 0x01));
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(EAR_DAC_CON,
						   DAC_SAMPLE_EDGE_SEL, 0x00));
		/* 5 msec delay as per HW requirement */
		usleep_range(5000, 5010);
		wcd_cls_h_fsm(component, &wcd939x->clsh_info,
			      WCD_CLSH_EVENT_PRE_DAC, WCD_CLSH_STATE_EAR,
			      CLS_AB_HIFI);
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(VNEG_CTRL_4, ILIM_SEL, 0xD));
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(
			component,
					...
					int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (hph_mode == CLS_H_LP || hph_mode == CLS_H_LOHIFI ||
			...
			if (!wcd939x->hph_pcm_enabled)
				snd_soc_component_update_bits(
					component,
					REG_FIELD_VALUE(REFBUFF_LP_CTL,
							PREREF_FILT_BYPASS,
							0x01));
		}
		...
		if (wcd939x->mbhc->wcd_mbhc.mbhc_cfg->enable_usbc_analog)
			wcd_usbss_audio_config(
				NULL, WCD_USBSS_CONFIG_TYPE_POWER_MODE,
				wcd939x_get_usbss_hph_power_mode(hph_mode));
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(VNEG_CTRL_4, ILIM_SEL, 0xD));
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(HPH, HPHR_REF_ENABLE, 0x01));
		if ((snd_soc_component_read(component, WCD939X_HPH) & 0x30) ==
		    0x30)
			usleep_range(
				2500,
				2600); /* 2.5msec delay as per HW requirement */
     ...
     */
		...
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(HPH_TIMER1, AUTOCHOP_TIMER_CTL_EN,
					0x01));
     ...
     */
		if (!wcd939x->comp2_enable)
			...
			usleep_range(7000, 7100);
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(HPH, HPHR_ENABLE, 0x00));
     ...
     */
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(HPH, HPHR_REF_ENABLE, 0x00));
		snd_soc_component_update_bits(component,
					      REG_FIELD_VALUE(PDM_WD_CTL1,
							      PDM_WD_EN, 0x00));
		if (wcd939x->mbhc->wcd_mbhc.mbhc_cfg->enable_usbc_analog &&
		    !(snd_soc_component_read(component, WCD939X_HPH) & 0XC0))
			wcd_usbss_audio_config(
				NULL, WCD_USBSS_CONFIG_TYPE_POWER_MODE, 1);
		wcd_cls_h_fsm(component, &wcd939x->clsh_info,
			      WCD_CLSH_EVENT_POST_PA, WCD_CLSH_STATE_HPHR,
			      hph_mode);
		if (wcd939x->ldoh)
			snd_soc_component_update_bits(
				component,
					...
					int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = swr_slvdev_datapath_control(wcd939x->rx_swr_dev,
						  ...
						  true);
		wcd_cls_h_fsm(component, &wcd939x->clsh_info,
			      WCD_CLSH_EVENT_PRE_DAC, WCD_CLSH_STATE_HPHL,
			      hph_mode);
		wcd939x_config_2Vpk_mode(component, wcd939x,
					 REGULATOR_MODE_2VPK);
		if (hph_mode == CLS_H_LP || hph_mode == CLS_H_LOHIFI ||
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(VNEG_CTRL_4, ILIM_SEL, 0xD));
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(HPH, HPHL_REF_ENABLE, 0x01));
		if ((snd_soc_component_read(component, WCD939X_HPH) & 0x30) ==
		    0x30)
			usleep_range(
				2500,
     ...
     */
		if (!wcd939x->comp1_enable)
			...
			usleep_range(7000, 7100);
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(HPH, HPHL_ENABLE, 0x00));
		blocking_notifier_call_chain(&wcd939x->mbhc->notifier,
     ...
     */
		...
		blocking_notifier_call_chain(&wcd939x->mbhc->notifier,
					     ...
					     &wcd939x->mbhc->wcd_mbhc);
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(HPH, HPHL_REF_ENABLE, 0x00));
		snd_soc_component_update_bits(component,
					      REG_FIELD_VALUE(PDM_WD_CTL0,
							      PDM_WD_EN, 0x00));
		if (wcd939x->mbhc->wcd_mbhc.mbhc_cfg->enable_usbc_analog &&
		    !(snd_soc_component_read(component, WCD939X_HPH) & 0XC0))
			wcd_usbss_audio_config(
				NULL, WCD_USBSS_CONFIG_TYPE_POWER_MODE, 1);
		wcd_cls_h_fsm(component, &wcd939x->clsh_info,
			      WCD_CLSH_EVENT_POST_PA, WCD_CLSH_STATE_HPHL,
			      hph_mode);
		if (wcd939x->ldoh)
			snd_soc_component_update_bits(
				component,
				       ...
				       struct snd_kcontrol *kcontrol, int event)
{
     ...
     */
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(EAR_COMPANDER_CTL,
						   GAIN_OVRD_REG, 0x01));
		break;
	case SND_SOC_DAPM_POST_PMU:
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_component_update_bits(component,
					      REG_FIELD_VALUE(PDM_WD_CTL0,
							      PDM_WD_EN, 0x00));
		wcd_cls_h_fsm(component, &wcd939x->clsh_info,
			      WCD_CLSH_EVENT_POST_PA, WCD_CLSH_STATE_EAR,
			      CLS_AB_HIFI);
		break;
	};
	return ret;
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	if (mode == CLS_H_LOHIFI || mode == CLS_H_ULP || mode == CLS_H_HIFI ||
	    mode == CLS_H_LP) {
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (wcd939x->hph_pcm_enabled)
			...
			if (wcd939x->comp1_enable)
				wcd939x_rx_connect_port(component, COMP_L,
							true);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (wcd939x->hph_pcm_enabled)
			...
			if (wcd939x->comp2_enable)
				wcd939x_rx_connect_port(component, COMP_R,
							true);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(component, WCD939X_CDC_AMIC_CTL,
					      (0x01 << dmic_ctl_shift), 0x00);
		/* 250us sleep as per HW requirement */
		usleep_range(250, 260);
		if (dmic2_left_en)
			snd_soc_component_update_bits(component, dmic2_left_en,
						      0x80, 0x80);
		/* Setting DMIC clock rate to 2.4MHz */
		snd_soc_component_update_bits(component, dmic_clk_reg,
					      dmic_clk_mask,
					      (0x03 << dmic_clk_shift));
		snd_soc_component_update_bits(component, dmic_clk_en_reg, 0x08,
					      0x08);
...
int wcd939x_tx_channel_config(struct snd_soc_component *component, int channel,
			      int mode)
{
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (w->shift + ADC1 == ADC2 &&
			...
			wcd939x_tx_connect_port(component, MBHC, 0, false);
					    ...
					    bool bcs_disable)
{
	...
	if (wcd939x->update_wcd_event) {
		if (bcs_disable)
			wcd939x->update_wcd_event(
				wcd939x->handle, SLV_BOLERO_EVT_BCS_CLK_OFF, 0);
		else
			wcd939x->update_wcd_event(
				wcd939x->handle, SLV_BOLERO_EVT_BCS_CLK_OFF, 1);
	}
}
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, REG_FIELD_VALUE(CDC_ANA_CLK_CTL,
						   ANA_TX_DIV2_CLK_EN, 0x01));
		snd_soc_component_update_bits(
			component,
			REG_FIELD_VALUE(CDC_REQ_CTL, FS_RATE_4P8, 0x01));
		snd_soc_component_update_bits(component,
					      REG_FIELD_VALUE(CDC_REQ_CTL,
							      NO_NOTCH, 0x00));
		...
		case 0:
			snd_soc_component_update_bits(
				component, WCD939X_CDC_TX_ANA_MODE_0_1, 0x0F,
				mode);
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_DIG_CLK_CTL,
							   TXD0_CLK_EN, 0x01));
			break;
		case 1:
			snd_soc_component_update_bits(
				component, WCD939X_CDC_TX_ANA_MODE_0_1, 0xF0,
				mode << 4);
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_DIG_CLK_CTL,
							   TXD1_CLK_EN, 0x01));
		...
		case 2:
			snd_soc_component_update_bits(
				component, WCD939X_CDC_TX_ANA_MODE_2_3, 0x0F,
				mode);
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_DIG_CLK_CTL,
							   TXD2_CLK_EN, 0x01));
		...
		case 3:
			snd_soc_component_update_bits(
				component, WCD939X_CDC_TX_ANA_MODE_2_3, 0xF0,
				mode << 4);
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_DIG_CLK_CTL,
							   TXD3_CLK_EN, 0x01));
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		case 0:
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_TX_ANA_MODE_0_1,
							   TXD0_MODE, 0x00));
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_DIG_CLK_CTL,
							   TXD0_CLK_EN, 0x00));
		...
		case 2:
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_TX_ANA_MODE_2_3,
							   TXD2_MODE, 0x00));
			snd_soc_component_update_bits(
				component, REG_FIELD_VALUE(CDC_DIG_CLK_CTL,
							   TXD2_CLK_EN, 0x00));
		...
		if (wcd939x->adc_count == 0) {
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(CDC_ANA_CLK_CTL,
						ANA_TX_DIV2_CLK_EN, 0x00));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(CDC_ANA_CLK_CTL, ANA_TX_CLK_EN,
						0x00));
		}
		break;
	};
...
int wcd939x_micbias_control(struct snd_soc_component *component, int micb_num,
			    int req, bool is_dapm)
{
	...
	case MICB_ENABLE:
		...
		if (wcd939x->micb_ref[micb_index] == 1) {
			...
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(CDC_ANA_TX_CLK_CTL,
						ANA_TXSCBIAS_CLK_EN, 0x01));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(TEST_CTL_2, IBIAS_LDO_DRIVER,
						0x01));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(MICB2_TEST_CTL_2,
						IBIAS_LDO_DRIVER, 0x01));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(MICB3_TEST_CTL_2,
						IBIAS_LDO_DRIVER, 0x01));
			snd_soc_component_update_bits(
				component,
				REG_FIELD_VALUE(MICB4_TEST_CTL_2,
						IBIAS_LDO_DRIVER, 0x01));
			snd_soc_component_update_bits(component, micb_reg, 0xC0,
						      0x40);
			if (post_on_event)
				blocking_notifier_call_chain(
					&wcd939x->mbhc->notifier, post_on_event,
					&wcd939x->mbhc->wcd_mbhc);
		}
	...
	case MICB_DISABLE:
		...
		if ((wcd939x->micb_ref[micb_index] == 0) &&
			...
			if (pre_off_event && wcd939x->mbhc)
				blocking_notifier_call_chain(
					&wcd939x->mbhc->notifier, pre_off_event,
					&wcd939x->mbhc->wcd_mbhc);
			snd_soc_component_update_bits(component, micb_reg, 0xC0,
						      0x00);
	...
	dev_dbg(component->dev,
		...
		micb_num, wcd939x->micb_ref[micb_index],
		wcd939x->pullup_ref[micb_index]);
...
static int wcd939x_event_notify(struct notifier_block *block, unsigned long val,
				void *data)
{
	...
	case BOLERO_SLV_EVT_SSR_UP:
		...
		if (wcd939x->notify_swr_dmic)
			blocking_notifier_call_chain(&wcd939x->notifier,
						     WCD939X_EVT_SSR_UP, NULL);
		if (wcd939x->usbc_hs_status)
			mdelay(500);
		break;
	case BOLERO_SLV_EVT_CLK_NOTIFY:
		snd_soc_component_update_bits(component, WCD939X_TOP_CLK_CFG,
					      0x06, ((val >> 0x10) << 0x01));
					  ...
					  int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		wcd939x_micbias_control(component, micb_num, MICB_DISABLE,
					true);
		break;
	};
						 ...
						 int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		wcd939x_micbias_control(component, micb_num, MICB_PULLUP_ENABLE,
					true);
		break;
	case SND_SOC_DAPM_POST_PMU:
		/* 1 msec delay as per HW requirement */
...
static int wcd939x_enable_micbias(struct wcd939x_priv *wcd939x, int micb_num,
				  int req)
{
	int micb_index = micb_num - 1;
	u16 micb_reg;
	...
	case MICB_ENABLE:
		...
		if (wcd939x->micb_ref[micb_index] == 1) {
			...
			regmap_update_bits(wcd939x->regmap,
					   WCD939X_CDC_ANA_TX_CLK_CTL, 0x01,
					   0x01);
			regmap_update_bits(wcd939x->regmap, WCD939X_TEST_CTL_2,
					   0x01, 0x01);
			regmap_update_bits(wcd939x->regmap,
					   WCD939X_MICB2_TEST_CTL_2, 0x01,
					   0x01);
			regmap_update_bits(wcd939x->regmap,
					   WCD939X_MICB3_TEST_CTL_2, 0x01,
					   0x01);
			regmap_update_bits(wcd939x->regmap,
					   WCD939X_MICB4_TEST_CTL_2, 0x01,
					   0x01);
			regmap_update_bits(wcd939x->regmap, micb_reg, 0xC0,
					   0x40);
			regmap_update_bits(wcd939x->regmap, micb_reg, 0x3F,
					   0x10);
		}
		break;
	case MICB_PULLUP_ENABLE:
					  ...
					  int event, int micb_num)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		wcd939x_enable_micbias(wcd939x_priv, micb_num,
				       MICB_PULLUP_ENABLE);
...
static inline int wcd939x_tx_path_get(const char *wname, unsigned int *path_num)
{
	int ret = 0;
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	ear_pa_gain =
		snd_soc_component_read(component, WCD939X_EAR_COMPANDER_CTL);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_component_update_bits(component, WCD939X_EAR_COMPANDER_CTL,
				      0x7C, ear_pa_gain);
					 ...
					 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = msm_cdc_enable_ondemand_supply(
			wcd939x->dev, wcd939x->supplies, pdata->regulator,
			pdata->num_supplies, "cdc-vdd-buck");
		if (ret == -EINVAL) {
			dev_err_ratelimited(component->dev,
					    "%s: vdd buck is not enabled\n",
					    __func__);
			return ret;
		}
		clear_bit(ALLOW_BUCK_DISABLE, &wcd939x->status_mask);
...
const char *const tx_master_ch_text[] = {
	"ZERO",		"SWRM_PCM_OUT", "SWRM_TX1_CH1", "SWRM_TX1_CH2",
	"SWRM_TX1_CH3", "SWRM_TX1_CH4", "SWRM_TX2_CH1", "SWRM_TX2_CH2",
	"SWRM_TX2_CH3", "SWRM_TX2_CH4", "SWRM_TX3_CH1", "SWRM_TX3_CH2",
	"SWRM_TX3_CH3", "SWRM_TX3_CH4", "SWRM_PCM_IN",
};
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(component->dev,
		"%s: ucontrol->value.enumerated.item[0] = %ld\n", __func__,
	...
	wcd939x->tx_master_ch_map[slave_ch_idx] =
		wcd939x_slave_get_master_ch(idx);
	return 0;
}
...
static const struct soc_enum tx_mode_mux_enum_wcd9390 = SOC_ENUM_SINGLE_EXT(
	ARRAY_SIZE(tx_mode_mux_text_wcd9390), tx_mode_mux_text_wcd9390);

static const char *const tx_mode_mux_text[] = {
	"ADC_INVALID", "ADC_HIFI", "ADC_LO_HIF", "ADC_NORMAL",
	"ADC_LP",      "ADC_ULP1", "ADC_ULP2",
};

static const struct soc_enum tx_mode_mux_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tx_mode_mux_text), tx_mode_mux_text);

static const char *const rx_hph_mode_mux_text_wcd9390[] = {
	"CLS_H_INVALID",   "CLS_H_INVALID_1", "CLS_H_LP",
	"CLS_AB",	   "CLS_H_LOHIFI",    "CLS_H_ULP",
	"CLS_H_INVALID_2", "CLS_AB_LP",	      "CLS_AB_LOHIFI",
};

static const char *const wcd939x_ear_pa_gain_text[] = {
	"G_6_DB",     "G_4P5_DB",   "G_3_DB",	 "G_1P5_DB",   "G_0_DB",
	"G_M1P5_DB",  "G_M3_DB",    "G_M4P5_DB", "G_M6_DB",    "G_7P5_DB",
	"G_M9_DB",    "G_M10P5_DB", "G_M12_DB",	 "G_M13P5_DB", "G_M15_DB",
	"G_M16P5_DB", "G_M18_DB",
};

static const struct soc_enum rx_hph_mode_mux_enum_wcd9390 = SOC_ENUM_SINGLE_EXT(
	ARRAY_SIZE(rx_hph_mode_mux_text_wcd9390), rx_hph_mode_mux_text_wcd9390);

static SOC_ENUM_SINGLE_EXT_DECL(wcd939x_ear_pa_gain_enum,
				wcd939x_ear_pa_gain_text);

static const char *const rx_hph_mode_mux_text[] = {
	"CLS_H_INVALID", "CLS_H_HIFI",	 "CLS_H_LP",
	"CLS_AB",	 "CLS_H_LOHIFI", "CLS_H_ULP",
	"CLS_AB_HIFI",	 "CLS_AB_LP",	 "CLS_AB_LOHIFI",
};
...
static const struct snd_kcontrol_new wcd9390_snd_controls[] = {
	...
	SOC_ENUM_EXT("TX0 MODE", tx_mode_mux_enum_wcd9390, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
	SOC_ENUM_EXT("TX1 MODE", tx_mode_mux_enum_wcd9390, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
	SOC_ENUM_EXT("TX2 MODE", tx_mode_mux_enum_wcd9390, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
	SOC_ENUM_EXT("TX3 MODE", tx_mode_mux_enum_wcd9390, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
};
...
static const struct snd_kcontrol_new wcd9395_snd_controls[] = {
	...
	SOC_ENUM_EXT("TX0 MODE", tx_mode_mux_enum, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
	SOC_ENUM_EXT("TX1 MODE", tx_mode_mux_enum, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
	SOC_ENUM_EXT("TX2 MODE", tx_mode_mux_enum, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
	SOC_ENUM_EXT("TX3 MODE", tx_mode_mux_enum, wcd939x_tx_mode_get,
		     wcd939x_tx_mode_put),
};
...
static const struct snd_kcontrol_new wcd939x_snd_controls[] = {
	...
	SOC_SINGLE_EXT("HPHR_COMP Switch", SND_SOC_NOPM, 1, 1, 0,
		       wcd939x_get_compander, wcd939x_set_compander),
	SOC_SINGLE_EXT("LDOH Enable", SND_SOC_NOPM, 0, 1, 0, wcd939x_ldoh_get,
		       wcd939x_ldoh_put),

	SOC_SINGLE_EXT("ADC2_BCS Disable", SND_SOC_NOPM, 0, 1, 0,
		       wcd939x_bcs_get, wcd939x_bcs_put),

	SOC_SINGLE_TLV("HPHL Volume", WCD939X_PA_GAIN_CTL_L, 0, 0x18, 0,
		       hph_analog_gain),
	SOC_SINGLE_TLV("HPHR Volume", WCD939X_PA_GAIN_CTL_R, 0, 0x18, 0,
		       hph_analog_gain),
	SOC_SINGLE_TLV("ADC1 Volume", WCD939X_TX_CH1, 0, 20, 0, analog_gain),
	SOC_SINGLE_TLV("ADC2 Volume", WCD939X_TX_CH2, 0, 20, 0, analog_gain),
	SOC_SINGLE_TLV("ADC3 Volume", WCD939X_TX_CH3, 0, 20, 0, analog_gain),
	SOC_SINGLE_TLV("ADC4 Volume", WCD939X_TX_CH4, 0, 20, 0, analog_gain),
	...
	SOC_ENUM_EXT("ADC1 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("ADC2 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("ADC3 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("ADC4 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC0 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC1 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("MBHC ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC2 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC3 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC4 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC5 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC6 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
	SOC_ENUM_EXT("DMIC7 ChMap", tx_master_ch_enum, wcd939x_tx_master_ch_get,
		     wcd939x_tx_master_ch_put),
};
...
static const struct snd_kcontrol_new adc3_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new adc4_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new amic1_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new amic2_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new amic3_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new amic4_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new amic5_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new va_amic1_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new va_amic2_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new va_amic3_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new va_amic4_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new va_amic5_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };
...
static const struct snd_kcontrol_new dmic3_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new dmic4_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new dmic5_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new dmic6_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new dmic7_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };

static const struct snd_kcontrol_new dmic8_switch[] = { SOC_DAPM_SINGLE(
	"Switch", SND_SOC_NOPM, 0, 1, 0) };
...
static const char *const adc2_mux_text[] = { "CH2_AMIC_DISABLE", "CH2_AMIC1",
					     "CH2_AMIC2",	 "CH2_AMIC3",
					     "CH2_AMIC4",	 "CH2_AMIC5" };
...
static const char *const adc3_mux_text[] = { "CH3_AMIC_DISABLE", "CH3_AMIC1",
					     "CH3_AMIC3", "CH3_AMIC4",
					     "CH3_AMIC5" };
...
static const char *const adc4_mux_text[] = { "CH4_AMIC_DISABLE", "CH4_AMIC1",
					     "CH4_AMIC3", "CH4_AMIC4",
					     "CH4_AMIC5" };
...
static const char *const rdac3_mux_text[] = { "RX3", "RX1" };

static const struct soc_enum rdac3_enum =
	SOC_ENUM_SINGLE(WCD939X_CDC_EAR_PATH_CTL, 0, ARRAY_SIZE(rdac3_mux_text),
			rdac3_mux_text);

static const struct snd_kcontrol_new rx_rdac3_mux =
	SOC_DAPM_ENUM("RDAC3_MUX Mux", rdac3_enum);

static const char *const rx1_mux_text[] = { "ZERO", "RX1 MUX" };
...
static const char *const rx2_mux_text[] = { "ZERO", "RX2 MUX" };
...
static const char *const rx3_mux_text[] = { "ZERO", "RX3 MUX" };
...
static const struct snd_soc_dapm_widget wcd939x_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER_E("ADC4 REQ", SND_SOC_NOPM, 3, 0, NULL, 0,
			     wcd939x_enable_req,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("AMIC1_MIXER", SND_SOC_NOPM, 0, 0, amic1_switch,
			     ARRAY_SIZE(amic1_switch), NULL,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("AMIC2_MIXER", SND_SOC_NOPM, 0, 0, amic2_switch,
			     ARRAY_SIZE(amic2_switch), NULL,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("AMIC3_MIXER", SND_SOC_NOPM, 0, 0, amic3_switch,
			     ARRAY_SIZE(amic3_switch), NULL,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("AMIC4_MIXER", SND_SOC_NOPM, 0, 0, amic4_switch,
			     ARRAY_SIZE(amic4_switch), NULL,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("AMIC5_MIXER", SND_SOC_NOPM, 0, 0, amic5_switch,
			     ARRAY_SIZE(amic5_switch), NULL,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	/*tx mixers*/
	SND_SOC_DAPM_MIXER_E("ADC1_MIXER", SND_SOC_NOPM, ADC1, 0, adc1_switch,
			     ARRAY_SIZE(adc1_switch), wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("ADC2_MIXER", SND_SOC_NOPM, ADC2, 0, adc2_switch,
			     ARRAY_SIZE(adc2_switch), wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_MIXER_E("ADC4_MIXER", SND_SOC_NOPM, ADC4, 0, adc4_switch,
			     ...
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC1_MIXER", SND_SOC_NOPM, DMIC1, 0,
			     dmic1_switch, ARRAY_SIZE(dmic1_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC2_MIXER", SND_SOC_NOPM, DMIC2, 0,
			     dmic2_switch, ARRAY_SIZE(dmic2_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC3_MIXER", SND_SOC_NOPM, DMIC3, 0,
			     dmic3_switch, ARRAY_SIZE(dmic3_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC4_MIXER", SND_SOC_NOPM, DMIC4, 0,
			     dmic4_switch, ARRAY_SIZE(dmic4_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC5_MIXER", SND_SOC_NOPM, DMIC5, 0,
			     dmic5_switch, ARRAY_SIZE(dmic5_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC6_MIXER", SND_SOC_NOPM, DMIC6, 0,
			     dmic6_switch, ARRAY_SIZE(dmic6_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC7_MIXER", SND_SOC_NOPM, DMIC7, 0,
			     dmic7_switch, ARRAY_SIZE(dmic7_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("DMIC8_MIXER", SND_SOC_NOPM, DMIC8, 0,
			     dmic8_switch, ARRAY_SIZE(dmic8_switch),
			     wcd939x_tx_swr_ctrl,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	/* micbias widgets*/
	...
	SND_SOC_DAPM_DAC_E(
		"RDAC1", NULL, SND_SOC_NOPM, 0, 0, wcd939x_codec_hphl_dac_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
			SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_DAC_E(
		"RDAC2", NULL, SND_SOC_NOPM, 0, 0, wcd939x_codec_hphr_dac_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
			SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_DAC_E(
		"RDAC3", NULL, SND_SOC_NOPM, 0, 0, wcd939x_codec_ear_dac_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
			SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
...
int wcd939x_get_codec_variant(struct snd_soc_component *component)
{
	...
	if (!priv) {
		dev_err(component->dev, "%s:wcd939x not probed\n", __func__);
		return 0;
	}
				    ...
				    struct snd_soc_component *component)
{
	...
	priv = snd_soc_component_get_drvdata(component);
	if (priv->entry) {
		dev_dbg(priv->dev, "%s:wcd939x module already created\n",
			__func__);
		return 0;
	}
	card = component->card;
	...
	if (!version_entry) {
		dev_dbg(component->dev,
			"%s: failed to create wcd939x version entry\n",
			__func__);
		snd_info_free_entry(priv->entry);
		return -ENOMEM;
	}
	...
	if (!variant_entry) {
		dev_dbg(component->dev,
			"%s: failed to create wcd939x variant entry\n",
			__func__);
		snd_info_free_entry(version_entry);
		snd_info_free_entry(priv->entry);
		return -ENOMEM;
				    ...
				    struct wcd939x_pdata *pdata)
{
	...
	regmap_update_bits(wcd939x->regmap, WCD939X_MICB1, 0x3F, vout_ctl_1);
	regmap_update_bits(wcd939x->regmap, WCD939X_MICB2, 0x3F, vout_ctl_2);
	regmap_update_bits(wcd939x->regmap, WCD939X_MICB3, 0x3F, vout_ctl_3);
	regmap_update_bits(wcd939x->regmap, WCD939X_MICB4, 0x3F, vout_ctl_4);
...
static int wcd939x_soc_codec_probe(struct snd_soc_component *component)
{
	...
	devm_regmap_qti_debugfs_register(&wcd939x->tx_swr_dev->dev,
					 wcd939x->regmap);

	/*Harmonium contains only one variant i.e wcd9395*/
	wcd939x->variant = WCD9395;

	/* Check device tree to see if 2Vpk flag is enabled, this value should not be
   * changed */
	wcd939x->in_2Vpk_mode = of_find_property(wcd939x->dev->of_node,
						 "qcom,hph-2p15v-mode",
						 NULL) != NULL;

	wcd939x->fw_data = devm_kzalloc(
		component->dev, sizeof(*(wcd939x->fw_data)), GFP_KERNEL);
	if (!wcd939x->fw_data) {
	...
	ret = wcd_cal_create_hwdep(wcd939x->fw_data, WCD9XXX_CODEC_HWDEP_NODE,
				   component);
	...
	if (wcd939x->variant == WCD9390) {
		ret = snd_soc_add_component_controls(
			component, wcd9390_snd_controls,
	...
	if (wcd939x->variant == WCD9395) {
		ret = snd_soc_add_component_controls(
			component, wcd9395_snd_controls,
...
static void wcd939x_soc_codec_remove(struct snd_soc_component *component)
{
	...
	if (wcd939x->register_notifier)
		wcd939x->register_notifier(wcd939x->handle, &wcd939x->nblock,
					   false);
}
...
static int wcd939x_reset(struct device *dev)
{
	...
	if (!wcd939x->rst_np) {
		dev_err_ratelimited(
			dev, "%s: reset gpio device node not specified\n",
			__func__);
		return -EINVAL;
	}
					  ...
					  struct wcd939x_micbias_setting *mb)
{
	...
	if (of_find_property(dev->of_node, "qcom,cdc-micbias1-mv", NULL)) {
		rc = wcd939x_read_of_property_u32(dev, "qcom,cdc-micbias1-mv",
						  &prop_val);
		if (!rc)
	...
	if (of_find_property(dev->of_node, "qcom,cdc-micbias2-mv", NULL)) {
		rc = wcd939x_read_of_property_u32(dev, "qcom,cdc-micbias2-mv",
						  &prop_val);
		if (!rc)
	...
	if (of_find_property(dev->of_node, "qcom,cdc-micbias3-mv", NULL)) {
		rc = wcd939x_read_of_property_u32(dev, "qcom,cdc-micbias3-mv",
						  &prop_val);
		if (!rc)
	...
	/* MB4 */
	if (of_find_property(dev->of_node, "qcom,cdc-micbias4-mv", NULL)) {
		rc = wcd939x_read_of_property_u32(dev, "qcom,cdc-micbias4-mv",
						  &prop_val);
		if (!rc)
...
static void fill_r_common_gnd_buffer(struct wcd939x_usbcss_hs_params *usbcss_hs,
				     u32 val)
{
	size_t i;
...
static void init_usbcss_hs_params(struct wcd939x_usbcss_hs_params *usbcss_hs)
{
	...
	usbcss_hs->gnd.r_gnd_ext_fet_mohms =
		0; /* to be computed during MBHC zdet */
	...
	usbcss_hs->aud.l.r_load_eff_mohms =
		0; /* to be computed during MBHC zdet */
	usbcss_hs->aud.r.r_load_eff_mohms =
		0; /* to be computed during MBHC zdet */
...
static void parse_xtalk_param(struct device *dev, u32 default_val,
			      u32 *prop_val_p, char *prop)
{
	int rc = 0;

	if (of_find_property(dev->of_node, prop, NULL)) {
		rc = wcd939x_read_of_property_u32(dev, prop, prop_val_p);
		if ((!rc) && (*prop_val_p <= MAX_USBCSS_HS_IMPEDANCE_MOHMS) &&
		    (*prop_val_p > 0))
			return;
		...
		*prop_val_p = default_val;
		dev_dbg(dev,
			"%s: %s property not found. Default value of %d will be used.\n",
			__func__, prop, default_val);
	}
}

static void
wcd939x_dt_parse_usbcss_hs_info(struct device *dev,
				struct wcd939x_usbcss_hs_params *usbcss_hs)
{
	...
	/* xtalk_config: Determine type of crosstalk: none (0), digital (1), or analog
   * (2) */
	if (of_find_property(dev->of_node, "qcom,usbcss-hs-xtalk-config",
			     NULL)) {
		rc = wcd939x_read_of_property_u32(
			dev, "qcom,usbcss-hs-xtalk-config", &prop_val);
		if ((!rc) &&
		    (prop_val == XTALK_NONE || prop_val == XTALK_DIGITAL ||
		     prop_val == XTALK_ANALOG)) {
			usbcss_hs->xtalk.xtalk_config =
				(enum xtalk_mode)prop_val;
		} else
			dev_dbg(dev, "%s: %s OOB. Default value of %s used.\n",
				__func__, "qcom,usbcss-hs-xtalk-config",
				"XTALK_NONE");
	} else
		dev_dbg(dev,
			"%s: %s property not found. Default value of %s used.\n",
	...
	if (of_find_property(dev->of_node, "qcom,usbcss-hs-lin-k-aud", NULL)) {
		rc = wcd939x_read_of_property_s32(
			dev, "qcom,usbcss-hs-lin-k-aud", &prop_val_signed);
		if ((!rc) && (prop_val_signed <= MAX_K_TIMES_100) &&
		    (prop_val_signed >= MIN_K_TIMES_100))
			usbcss_hs->aud.k_aud_times_100 = prop_val_signed;
		else
			dev_dbg(dev,
				"%s: %s OOB. Default value of %d will be used.\n",
	...
	if (of_find_property(dev->of_node, "qcom,usbcss-hs-diff-slope", NULL)) {
		rc = wcd939x_read_of_property_u32(
			dev, "qcom,usbcss-hs-diff-slope", &prop_val);
	...
	/* R_ds(on) */
	parse_xtalk_param(dev, usbcss_hs->gnd.rdson_mohms, &prop_val,
	...
	usbcss_hs->gnd.gnd_ext_fet_delta_mohms =
		(s32)(usbcss_hs->gnd.rdson_3p6v_mohms -
	...
	if (usbcss_hs->xtalk.xtalk_config == XTALK_ANALOG) {
		...
		usbcss_hs->gnd.sbu1.r_gnd_par_route1_mohms =
			usbcss_hs->gnd.sbu1.r5 + usbcss_hs->gnd.sbu1.r4;
		usbcss_hs->gnd.sbu2.r_gnd_par_route1_mohms =
			usbcss_hs->gnd.sbu2.r5 + usbcss_hs->gnd.sbu2.r4;
		...
		usbcss_hs->gnd.sbu1.r_gnd_par_route2_mohms =
			usbcss_hs->gnd.sbu1.r6 + usbcss_hs->gnd.sbu1.r4;
		usbcss_hs->gnd.sbu2.r_gnd_par_route2_mohms =
			usbcss_hs->gnd.sbu2.r6 + usbcss_hs->gnd.sbu2.r4;
	...
	usbcss_hs->aud.l.r_aud_res_tot_mohms = get_r_aud_res_tot_mohms(
		...
		usbcss_hs->aud.l.r_aud_ext_fet_mohms, usbcss_hs->aud.l.r3);
	usbcss_hs->aud.r.r_aud_res_tot_mohms = get_r_aud_res_tot_mohms(
		usbcss_hs->aud.r.r_aud_int_fet_mohms,
		usbcss_hs->aud.r.r_aud_ext_fet_mohms, usbcss_hs->aud.r.r3);
	...
	if (usbcss_hs->gnd.rdson_3p6v_mohms < GND_EXT_FET_MARGIN_MOHMS)
		...
		usbcss_hs->gnd.gnd_ext_fet_min_mohms =
			usbcss_hs->gnd.rdson_3p6v_mohms -
			GND_EXT_FET_MARGIN_MOHMS;
	else
...
struct wcd939x_pdata *wcd939x_populate_dt_data(struct device *dev)
{
	...
	pdata = devm_kzalloc(dev, sizeof(struct wcd939x_pdata), GFP_KERNEL);
	if (!pdata)
		return NULL;

	pdata->rst_np =
		of_parse_phandle(dev->of_node, "qcom,wcd-rst-gpio-node", 0);
	...
	if (!pdata->regulator || (pdata->num_supplies <= 0)) {
		dev_err_ratelimited(dev,
				    "%s: no power supplies defined for codec\n",
				    __func__);
		return NULL;
	}
...
static void wcd939x_update_regmap_cache(struct wcd939x_priv *wcd939x)
{
	...
	if (wcd939x->version >= WCD939X_VERSION_1_1) {
		for (int i = 0; i < ARRAY_SIZE(reg_def_1_1); ++i)
			regmap_write(wcd939x->regmap, reg_def_1_1[i].reg,
				     reg_def_1_1[i].def);
	}

	if (wcd939x->version == WCD939X_VERSION_2_0) {
		for (int i = 0; i < ARRAY_SIZE(reg_def_2_0); ++i)
			regmap_write(wcd939x->regmap, reg_def_2_0[i].reg,
				     reg_def_2_0[i].def);
	}
}
...
static int wcd939x_bind(struct device *dev)
{
   ...
   */
	...
	if (!wcd939x->rx_swr_dev) {
		dev_err_ratelimited(dev,
				    "%s: Could not find RX swr slave device\n",
				    __func__);
		ret = -ENODEV;
		goto err;
	}
	...
	if (!wcd939x->tx_swr_dev) {
		dev_err_ratelimited(dev,
				    "%s: Could not find TX swr slave device\n",
				    __func__);
		ret = -ENODEV;
		goto err;
	}
	...
	if (id1 == 0)
		wcd939x->version = ((status1 & 0x3) ? WCD939X_VERSION_1_1 :
						      WCD939X_VERSION_1_0);
	else if (id1 == 1)
	...
	for (i = 0; i < wcd939x_regmap_irq_chip.num_regs; i++)
		regmap_write(wcd939x->regmap, (WCD939X_INTR_LEVEL_0 + i), 0);
					...
					struct component_match **matchptr)
{
	...
	of_node_get(rx_node);
	component_match_add_release(dev, matchptr, wcd939x_release_of,
				    wcd939x_compare_of, rx_node);
...
static int wcd939x_probe(struct platform_device *pdev)
{
	...
	wcd939x = devm_kzalloc(dev, sizeof(struct wcd939x_priv), GFP_KERNEL);
	if (!wcd939x)
		return -ENOMEM;
	...
	ret = msm_cdc_init_supplies(dev, &wcd939x->supplies, pdata->regulator,
				    pdata->num_supplies);
	if (!wcd939x->supplies) {
	...
	ret = wcd939x_parse_port_mapping(dev, "qcom,rx_swr_ch_map", CODEC_RX);
	ret |= wcd939x_parse_port_mapping(dev, "qcom,tx_swr_ch_map", CODEC_TX);
	...
	return component_master_add_with_match(dev, &wcd939x_comp_ops, match);
	...
	if (wcd939x->dapm_bias_off ||
	    (wcd939x->component &&
	     (snd_soc_component_get_bias_level(wcd939x->component) ==
	      SND_SOC_BIAS_OFF))) {
		msm_cdc_set_supplies_lpm_mode(wcd939x->dev, wcd939x->supplies,
					      pdata->regulator,
					      pdata->num_supplies, true);
		set_bit(WCD_SUPPLIES_LPM_MODE, &wcd939x->status_mask);
		if (msm_cdc_is_ondemand_supply(
			    wcd939x->dev, wcd939x->supplies, pdata->regulator,
			    pdata->num_supplies, "cdc-vdd-px")) {
			if (msm_cdc_supply_supports_retention_mode(
				    wcd939x->dev, wcd939x->supplies,
				    pdata->regulator, pdata->num_supplies,
				    "cdc-vdd-px") &&
				...
				if (ret) {
					dev_dbg(dev,
						"%s: vdd px supply suspend failed!\n",
						__func__);
				}
			}
		}
