...
static int wcd9335_bring_up(struct wcd9xxx *wcd9xxx)
{
	...
	if ((val < 0) || (byte0 < 0)) {
		dev_err(wcd9xxx->dev,
			"%s: tasha codec version detection fail!\n", __func__);
		return -EINVAL;
	}
				...
				struct wcd9xxx_codec_type *wcd_type)
{
	...
	if (id_major == TASHA_MAJOR) {
		...
		dev_err(wcd9xxx->dev,
			"%s: wcd9335 version unknown (major 0x%x, minor 0x%x)\n",
			__func__, id_major, id_minor);
	...
	wcd_type->intr_reg[WCD9XXX_INTR_CLEAR_BASE] = WCD9335_INTR_PIN1_CLEAR0;
	wcd_type->intr_reg[WCD9XXX_INTR_MASK_BASE] = WCD9335_INTR_PIN1_MASK0;
	wcd_type->intr_reg[WCD9XXX_INTR_LEVEL_BASE] = WCD9335_INTR_LEVEL0;
	wcd_type->intr_reg[WCD9XXX_INTR_CLR_COMMIT] = WCD9335_INTR_CLR_COMMIT;
...
version_unknown:
	...
	wcd_type->intr_reg[WCD9XXX_INTR_CLEAR_BASE] = WCD934X_INTR_PIN1_CLEAR0;
	wcd_type->intr_reg[WCD9XXX_INTR_MASK_BASE] = WCD934X_INTR_PIN1_MASK0;
	wcd_type->intr_reg[WCD9XXX_INTR_LEVEL_BASE] = WCD934X_INTR_LEVEL0;
	wcd_type->intr_reg[WCD9XXX_INTR_CLR_COMMIT] = WCD934X_INTR_CLR_COMMIT;
