...
static const struct cpe_svc_hw_cfg cpe_svc_tomtom_info = {
	TOMTOM_A_SVASS_SPE_DRAM_SIZE,  TOMTOM_A_SVASS_SPE_DRAM_OFFSET,
	TOMTOM_A_SVASS_SPE_IRAM_SIZE,  TOMTOM_A_SVASS_SPE_IRAM_OFFSET,
	TOMTOM_A_SVASS_SPE_INBOX_SIZE, TOMTOM_A_SVASS_SPE_OUTBOX_SIZE
};

static const struct cpe_svc_hw_cfg cpe_svc_wcd9335_info = {
	WCD9335_CPE_SS_SPE_DRAM_SIZE,  WCD9335_CPE_SS_SPE_DRAM_OFFSET,
	WCD9335_CPE_SS_SPE_IRAM_SIZE,  WCD9335_CPE_SS_SPE_IRAM_OFFSET,
	WCD9335_CPE_SS_SPE_INBOX_SIZE, WCD9335_CPE_SS_SPE_OUTBOX_SIZE
};
...
struct cpe_info {
	...
	enum cpe_svc_result (*cpe_cmd_validate)(const struct cpe_info *i,
						enum cpe_command command);
	enum cpe_svc_result (*cpe_start_notification)(struct cpe_info *i);
	u32 initialized;
...
struct cpe_svc_tgt_abstraction {
	...
	enum cpe_svc_result (*tgt_read_mailbox)(u8 *buffer, size_t size);

	enum cpe_svc_result (*tgt_write_mailbox)(u8 *buffer, size_t size);

	enum cpe_svc_result (*tgt_read_ram)(struct cpe_info *c,
					    struct cpe_svc_mem_segment *data);

	enum cpe_svc_result (*tgt_write_ram)(
		struct cpe_info *c, const struct cpe_svc_mem_segment *data);
	...
	enum cpe_svc_result (*tgt_deinit)(struct cpe_svc_tgt_abstraction *param);
	enum cpe_svc_result (*tgt_voice_tx_lab)(bool);
	u8 *inbox;
	u8 *outbox;
...
cpe_tgt_tomtom_init(struct cpe_svc_codec_info_v1 *codec_info,
		    struct cpe_svc_tgt_abstraction *param);

static enum cpe_svc_result
cpe_tgt_wcd9335_init(struct cpe_svc_codec_info_v1 *codec_info,
		     struct cpe_svc_tgt_abstraction *param);
...
struct generic_notification {
	void (*notification)(const struct cpe_svc_notification *parameter);
	void (*cmi_notification)(const struct cmi_api_notification *parameter);
};
...
struct cpe_priv {
	...
	void (*cpe_change_freq_plan_cb)(void *cdc_priv, u32 clk_freq);
	u32 cpe_msg_buffer;
	void *cpe_cmi_handle;
...
static enum cpe_svc_result cpe_is_command_valid(const struct cpe_info *t_info,
						enum cpe_command command);
...
static enum cpe_svc_result cpe_update_bits(u32 reg, u32 mask, u32 value)
{
	int ret = 0;

	ret = snd_soc_update_bits(cpe_d.cdc_priv, reg, mask, value);
	if (ret < 0)
...
static void cpe_create_worker_thread(struct cpe_info *t_info)
{
	...
	t_info->thread_handler = kthread_run(cpe_worker_thread, (void *)t_info,
					     "cpe-worker-thread");
...
static enum cpe_svc_result cpe_send_cmd_to_thread(struct cpe_info *t_info,
						  enum cpe_command command,
						  void *data, bool high_prio)
{
	...
	cmd = kzalloc(sizeof(struct cpe_command_node), GFP_ATOMIC);
	if (!cmd)
	...
	CPE_SVC_GRAB_LOCK(&t_info->msg_lock, "msg_lock");
	if (high_prio)
		list_add(&(cmd->list), &(t_info->main_queue));
	else
...
static enum cpe_svc_result cpe_change_state(struct cpe_info *t_info,
					    enum cpe_state state,
					    enum cpe_substate ss)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	...
	pr_debug("%s: current state: %d,%d, new_state: %d,%d\n", __func__,
		 t_info->state, t_info->substate, state, ss);
			      ...
			      struct cpe_svc_notification *payload)
{
	...
	if ((client->mask & CPE_SVC_CMI_MSG) && client->notif.cmi_notification)
...
static void *cpe_register_generic(
	struct cpe_info *t_info,
	void notification_callback(const struct cpe_svc_notification *parameter),
	void cmi_callback(const struct cmi_api_notification *parameter),
	u32 mask, u32 service, const char *name)
{
	struct cpe_notif_node *n = NULL;

	n = kzalloc(sizeof(struct cpe_notif_node), GFP_KERNEL);
	if (!n)
		return NULL;
	n->mask = mask;
	n->service = service;
...
static enum cpe_svc_result cpe_send_msg_to_inbox(struct cpe_info *t_info,
						 u32 opcode,
						 struct cpe_send_msg *msg)
{
	size_t bytes = 0;
	size_t inbox_size = t_info->tgt->tgt_get_cpe_info()->inbox_size;
	struct cmi_hdr *hdr;
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	...
	case CPE_CORE_SVC_CMD_SHARED_MEM_ALLOC: {
		...
		CMI_HDR_SET_OPCODE(hdr, CPE_CORE_SVC_CMD_SHARED_MEM_ALLOC);
		CMI_HDR_SET_PAYLOAD_SIZE(
			hdr, sizeof(struct cmi_core_svc_cmd_shared_mem_alloc));
		m = (struct cmi_core_svc_cmd_shared_mem_alloc *)CMI_GET_PAYLOAD(
			t_info->tgt->inbox);
		m->size = CPE_MSG_BUFFER_SIZE;
		pr_debug("send shared mem alloc msg to cpe inbox\n");
	} break;
	case CPE_CORE_SVC_CMD_DRAM_ACCESS_REQ:
		CMI_HDR_SET_OPCODE(hdr, CPE_CORE_SVC_CMD_DRAM_ACCESS_REQ);
	...
	case CPE_CMI_BASIC_RSP_OPCODE: {
		...
		CMI_HDR_SET_OPCODE(hdr, CPE_CMI_BASIC_RSP_OPCODE);
		CMI_HDR_SET_PAYLOAD_SIZE(hdr,
					 sizeof(struct cmi_basic_rsp_result));
		rsp = (struct cmi_basic_rsp_result *)CMI_GET_PAYLOAD(
			t_info->tgt->inbox);
		rsp->status = 0;
	...
	default:
		if (msg->address != 0) {
			...
			m = (struct cmi_msg_transport *)CMI_GET_PAYLOAD(
				t_info->tgt->inbox);
			m->addr = msg->address;
			m->size = msg->size;
			CMI_HDR_SET_PAYLOAD_SIZE(
				hdr, sizeof(struct cmi_msg_transport));
		} else {
			memcpy(t_info->tgt->inbox, msg->payload, msg->size);
		}
...
static bool cpe_is_cmd_clk_req(void *cmd)
{
	...
	if ((CMI_HDR_GET_SERVICE(hdr) == CMI_CPE_CORE_SERVICE_ID)) {
		if (CMI_GET_OPCODE(cmd) == CPE_CORE_SVC_CMD_CLK_FREQ_REQUEST)
			return true;
	}
...
static enum cpe_svc_result cpe_process_clk_change_req(struct cpe_info *t_info)
{
	struct cmi_core_svc_cmd_clk_freq_request *req;

	req = (struct cmi_core_svc_cmd_clk_freq_request *)CMI_GET_PAYLOAD(
		t_info->tgt->outbox);
	...
	cpe_d.cpe_change_freq_plan_cb(cpe_d.cdc_priv, req->clk_freq);

	/*send a basic response*/
	cpe_send_msg_to_inbox(t_info, CPE_CMI_BASIC_RSP_OPCODE, NULL);

	return CPE_SVC_SUCCESS;
}

static void cpe_process_irq_int(u32 irq, struct cpe_info *t_info)
{
	...
	case CPE_STATE_SENDING_MSG:
		...
		if (CMI_GET_OPCODE(t_info->tgt->outbox) ==
			...
			pr_debug(
				"%s: session_id: %u, state: %d,%d, event received\n",
		...
		case CPE_SS_MSG_REQUEST_ACCESS:
			cpe_send_cmd_to_thread(t_info, CPE_CMD_SEND_TRANS_MSG,
					       m, true);
			break;
		...
		case CPE_SS_MSG_SEND_INBOX:
			if (cpe_is_cmd_clk_req(t_info->tgt->outbox))
				...
				cpe_send_cmd_to_thread(
					t_info, CPE_CMD_SEND_MSG_COMPLETE, m,
					true);
			break;
...
static enum cpe_svc_result broadcast_boot_event(struct cpe_info *t_info)
{
	struct cpe_svc_notification payload;
						   ...
						   enum cpe_svc_result *cpe_rc)
{
	...
	p = (struct cmi_core_svc_event_system_boot *)CMI_GET_PAYLOAD(
		t_info->tgt->outbox);
	...
	/* boot was successful */
	if (p->version == CPE_CORE_VERSION_SYSTEM_BOOT_EVENT) {
		cpe_d.cpe_debug_vector.debug_address = p->sfr_buff_address;
		cpe_d.cpe_debug_vector.debug_buffer_size = p->sfr_buff_size;
	...
	cpe_change_state(t_info, CPE_STATE_BOOTING, CPE_SS_BOOT_INIT);
	(*cpe_rc) = cpe_send_msg_to_inbox(
		t_info, CPE_CORE_SVC_CMD_SHARED_MEM_ALLOC, NULL);
	rc = CPE_PROC_SUCCESS;
	return rc;
}

static void
cpe_svc_core_cmi_handler(const struct cmi_api_notification *parameter)
{
	struct cmi_hdr *hdr;
	...
	if (hdr->opcode == CPE_CMI_BASIC_RSP_OPCODE) {
		...
		result = (struct cmi_basic_rsp_result *)((u8 *)parameter
								 ->message) +
			 (sizeof(*hdr));
		if (result->status)
...
static void cpe_clk_plan_work(struct work_struct *work)
{
	...
	cpe_d.cpe_cmi_handle =
		cmi_register(cpe_svc_core_cmi_handler, CMI_CPE_CORE_SERVICE_ID);
	...
	size = sizeof(plan.current_clk_feq) + sizeof(plan.num_clk_freqs);
	size += plan.num_clk_freqs * sizeof(plan.clk_freqs[0]);
	cmi_msg = kzalloc(size + sizeof(struct cmi_hdr), GFP_KERNEL);
	if (!cmi_msg)
		return;

	hdr = (struct cmi_hdr *)cmi_msg;
	CMI_HDR_SET_OPCODE(hdr, CPE_CORE_SVC_CMD_CFG_CLK_PLAN);
	CMI_HDR_SET_SERVICE(hdr, CMI_CPE_CORE_SERVICE_ID);
	CMI_HDR_SET_SESSION(hdr, 1);
	CMI_HDR_SET_VERSION(hdr, CMI_DRIVER_SUPPORTED_VERSION);
	CMI_HDR_SET_PAYLOAD_SIZE(hdr, size);
	memcpy(CMI_GET_PAYLOAD(cmi_msg), &plan, size);
...
static enum cpe_process_result cpe_boot_complete(struct cpe_info *t_info)
{
	...
	p = (struct cmi_core_svc_cmdrsp_shared_mem_alloc *)CMI_GET_PAYLOAD(
		t_info->tgt->outbox);
...
static enum cpe_process_result
cpe_process_send_msg(struct cpe_info *t_info, enum cpe_svc_result *cpe_rc,
		     struct cpe_command_node *command_node)
{
	enum cpe_process_result rc = CPE_PROC_SUCCESS;
	struct cpe_send_msg *m = (struct cpe_send_msg *)command_node->data;
	u32 size = m->size;
	...
	if (size <= t_info->tgt->tgt_get_cpe_info()->inbox_size) {
		...
		*cpe_rc = cpe_send_msg_to_inbox(t_info, 0, m);
		...
		cpe_change_state(t_info, CPE_STATE_SENDING_MSG,
				 CPE_SS_MSG_REQUEST_ACCESS);
		*cpe_rc = cpe_send_msg_to_inbox(
			t_info, CPE_CORE_SVC_CMD_DRAM_ACCESS_REQ, m);
	} else {
...
static enum cpe_process_result cpe_process_incoming(struct cpe_info *t_info)
{
	...
	if (CMI_HDR_GET_SERVICE(hdr) == CMI_CPE_CORE_SERVICE_ID) {
		...
		pr_debug("%s: Message received, notifying client\n", __func__);
		cpe_notify_cmi_client(t_info, t_info->tgt->outbox,
				      CPE_SVC_SUCCESS);
		rc = CPE_PROC_SUCCESS;
	}
...
static enum cpe_process_result
cpe_process_kill_thread(struct cpe_info *t_info,
			struct cpe_command_node *command_node)
{
	struct cpe_svc_notification payload;
...
static enum cpe_process_result
cpe_mt_process_cmd(struct cpe_command_node *command_node)
{
	struct cpe_info *t_info = cpe_d.cpe_default_handle;
	...
	case CPE_CMD_SEND_MSG:
		rc = cpe_process_send_msg(t_info, &cpe_rc, command_node);
		break;
...
static enum cpe_svc_result cpe_mt_validate_cmd(const struct cpe_info *t_info,
					       enum cpe_command command)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	...
	if (rc != CPE_SVC_SUCCESS)
		pr_err("%s: invalid command %d, state = %d\n", __func__,
		       command, t_info->state);
	return rc;
}

void *cpe_svc_initialize(void irq_control_callback(u32 enable),
			 const void *codec_info, void *context)
{
	...
	if (!cpe_d.cpe_default_handle) {
		cpe_d.cpe_default_handle =
			kzalloc(sizeof(struct cpe_info), GFP_KERNEL);
		if (!cpe_d.cpe_default_handle)
			goto err_register;

		memset(cpe_d.cpe_default_handle, 0, sizeof(struct cpe_info));
	}
	...
	t_info->tgt =
		kzalloc(sizeof(struct cpe_svc_tgt_abstraction), GFP_KERNEL);
	if (!t_info->tgt)
		goto err_tgt_alloc;
	t_info->codec_id = ((struct cpe_svc_codec_info_v1 *)codec_info)->id;
...
enum cpe_svc_result cpe_svc_deinitialize(void *cpe_handle)
{
	...
	cpe_change_state(t_info, CPE_STATE_UNINITIALIZED, CPE_SS_IDLE);
		       ...
		       u32 mask, const char *name)
{
	...
	reg_handle = cpe_register_generic((struct cpe_info *)cpe_handle,
					  notification_callback, NULL, mask,
					  CPE_NO_SERVICE, name);
	CPE_SVC_REL_LOCK(&cpe_d.cpe_api_mutex, "cpe_api");
...
enum cpe_svc_result cpe_svc_deregister(void *cpe_handle, void *reg_handle)
{
	...
	rc = cpe_deregister_generic((struct cpe_info *)cpe_handle, reg_handle);
	CPE_SVC_REL_LOCK(&cpe_d.cpe_api_mutex, "cpe_api");
...
enum cpe_svc_result
cpe_svc_download_segment(void *cpe_handle,
...
static enum cpe_svc_result __cpe_svc_shutdown(void *cpe_handle)
{
	...
	if (t_info->pending) {
		struct cpe_send_msg *m = (struct cpe_send_msg *)t_info->pending;
		cpe_notify_cmi_client(t_info, m->payload,
				      CPE_SVC_SHUTTING_DOWN);
...
void *cmi_register(void notification_callback(
		   ...
		   u32 service)
{
	...
	CPE_SVC_GRAB_LOCK(&cpe_d.cpe_api_mutex, "cpe_api");
	reg_handle = cpe_register_generic(
		cpe_d.cpe_default_handle, NULL, notification_callback,
		(CPE_SVC_CMI_MSG | CPE_SVC_OFFLINE | CPE_SVC_ONLINE), service,
		"CMI_CLIENT");
	CPE_SVC_REL_LOCK(&cpe_d.cpe_api_mutex, "cpe_api");
...
enum cmi_api_result cmi_send_msg(void *message)
{
	...
	msg = kzalloc(sizeof(struct cpe_send_msg), GFP_ATOMIC);
	if (!msg) {
		CPE_SVC_REL_LOCK(&cpe_d.cpe_api_mutex, "cpe_api");
		return CPE_SVC_NO_MEMORY;
	}
	...
	msg->size = sizeof(struct cmi_hdr) + CMI_HDR_GET_PAYLOAD_SIZE(hdr);
	...
	rc = (enum cmi_api_result)cpe_send_cmd_to_thread(
		cpe_d.cpe_default_handle, CPE_CMD_SEND_MSG, (void *)msg, false);
...
enum cpe_svc_result cpe_svc_ftm_test(void *cpe_handle, u32 *status)
{
	...
	if (t_info && t_info->tgt) {
		backup_data =
			kzalloc(t_info->tgt->tgt_waiti_info->tgt_waiti_size,
				GFP_KERNEL);
...
static enum cpe_svc_result cpe_tgt_tomtom_read_mailbox(u8 *buffer, size_t size)
{
	return CPE_SVC_SUCCESS;
}

static enum cpe_svc_result cpe_tgt_tomtom_write_mailbox(u8 *buffer, size_t size)
{
	return CPE_SVC_SUCCESS;
}

static enum cpe_svc_result
cpe_tgt_tomtom_read_RAM(struct cpe_info *t_info,
			struct cpe_svc_mem_segment *mem_seg)
{
	return CPE_SVC_SUCCESS;
}
...
static enum cpe_svc_result
cpe_tgt_tomtom_write_RAM(struct cpe_info *t_info,
...
static enum cpe_svc_result
cpe_tgt_tomtom_route_notification(enum cpe_svc_module module,
				  enum cpe_svc_route_dest dest)
{
	return CPE_SVC_SUCCESS;
}
...
static enum cpe_svc_result
cpe_tgt_tomtom_deinit(struct cpe_svc_tgt_abstraction *param)
{
	kfree(param->inbox);
	param->inbox = NULL;
	kfree(param->outbox);
		    ...
		    struct cpe_svc_tgt_abstraction *param)
{
	...
	if (codec_info->id == CPE_SVC_CODEC_TOMTOM) {
		...
		param->inbox =
			kzalloc(TOMTOM_A_SVASS_SPE_INBOX_SIZE, GFP_KERNEL);
		if (!param->inbox)
			return CPE_SVC_NO_MEMORY;

		param->outbox =
			kzalloc(TOMTOM_A_SVASS_SPE_OUTBOX_SIZE, GFP_KERNEL);
		if (!param->outbox) {
			kfree(param->inbox);
			return CPE_SVC_NO_MEMORY;
		}
...
static enum cpe_svc_result cpe_tgt_wcd9335_boot(int debug_mode)
{
	...
	if (!debug_mode)
		rc |= cpe_update_bits(WCD9335_CPE_SS_WDOG_CFG, 0x3f, 0x31);
	else
...
static enum cpe_svc_result cpe_tgt_wcd9335_reset(void)
{
	...
	rc |= cpe_register_write(WCD9335_CODEC_RPM_PWR_CPE_IRAM_SHUTDOWN, 0x00);
	rc |= cpe_register_write(WCD9335_CODEC_RPM_PWR_CPE_DRAM1_SHUTDOWN,
				 0x00);
	rc |= cpe_register_write(WCD9335_CODEC_RPM_PWR_CPE_DRAM0_SHUTDOWN_1,
				 0x00);
	rc |= cpe_register_write(WCD9335_CODEC_RPM_PWR_CPE_DRAM0_SHUTDOWN_2,
				 0x00);
...
static enum cpe_svc_result cpe_tgt_wcd9335_read_mailbox(u8 *buffer, size_t size)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	u32 cnt = 0;
...
static enum cpe_svc_result
cpe_wcd9335_get_mem_addr(struct cpe_info *t_info,
			 const struct cpe_svc_mem_segment *mem_seg, u32 *addr,
			 u8 *mem)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
...
static enum cpe_svc_result
cpe_tgt_wcd9335_read_RAM(struct cpe_info *t_info,
			 struct cpe_svc_mem_segment *mem_seg)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	u8 temp = 0;
	...
	do {
		...
		if (!autoinc)
			rc |= cpe_register_write(WCD9335_CPE_SS_MEM_CTRL, 0);
	} while ((++cnt < mem_seg->size) || (rc != CPE_SVC_SUCCESS));
...
static enum cpe_svc_result
cpe_tgt_wcd9335_write_RAM(struct cpe_info *t_info,
			  const struct cpe_svc_mem_segment *mem_seg)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	u8 mem_reg_val = 0;
	...
	rc = cpe_update_bits(WCD9335_CPE_SS_MEM_CTRL, 0x0F, mem_reg_val);

	rc = cpe_register_write(WCD9335_CPE_SS_MEM_PTR_0, (addr & 0xFF));
	rc = cpe_register_write(WCD9335_CPE_SS_MEM_PTR_1, ((addr >> 8) & 0xFF));
	...
	while (temp_size <= mem_seg->size) {
		...
		cpe_register_write_repeat(WCD9335_CPE_SS_MEM_BANK_0, temp_ptr,
					  to_write);
...
static enum cpe_svc_result
cpe_tgt_wcd9335_route_notification(enum cpe_svc_module module,
				   enum cpe_svc_route_dest dest)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
...
static enum cpe_svc_result cpe_tgt_wcd9335_voicetx(bool enable)
{
	enum cpe_svc_result rc = CPE_SVC_SUCCESS;
	u8 val = 0;
		     ...
		     struct cpe_svc_tgt_abstraction *param)
{
	...
	if (codec_info->id == CPE_SVC_CODEC_WCD9335) {
		...
		param->inbox =
			kzalloc(WCD9335_CPE_SS_SPE_INBOX_SIZE, GFP_KERNEL);
		if (!param->inbox)
			return CPE_SVC_NO_MEMORY;

		param->outbox =
			kzalloc(WCD9335_CPE_SS_SPE_OUTBOX_SIZE, GFP_KERNEL);
		if (!param->outbox) {
			kfree(param->inbox);
			return CPE_SVC_NO_MEMORY;
		}
