...
static int phyirq_to_virq(struct wcd9xxx_core_resource *wcd9xxx_res, int irq);
static unsigned int
wcd9xxx_irq_get_upstream_irq(struct wcd9xxx_core_resource *wcd9xxx_res);
static void
wcd9xxx_irq_put_downstream_irq(struct wcd9xxx_core_resource *wcd9xxx_res);
static void
wcd9xxx_irq_put_upstream_irq(struct wcd9xxx_core_resource *wcd9xxx_res);
...
static void wcd9xxx_irq_sync_unlock(struct irq_data *data)
{
	...
	if ((ARRAY_SIZE(wcd9xxx_res->irq_masks_cur) > WCD9XXX_MAX_IRQ_REGS) ||
	    (ARRAY_SIZE(wcd9xxx_res->irq_masks_cache) > WCD9XXX_MAX_IRQ_REGS)) {
     ...
     */
		if (wcd9xxx_res->irq_masks_cur[i] !=
			...
			regmap_write(
				wcd9xxx_res->wcd_core_regmap,
				wcd9xxx_res->intr_reg[WCD9XXX_INTR_MASK_BASE] +
					i,
				wcd9xxx_res->irq_masks_cur[i]);
		}
	}
...
static void wcd9xxx_irq_disable(struct irq_data *data)
{
	...
	if ((byte < size) && (byte >= 0)) {
		wcd9xxx_res->irq_masks_cur[byte] |= BYTE_BIT_MASK(wcd9xxx_irq);
	} else {
		pr_err("%s: Array size is %d but index is %d: Out of range\n",
		       __func__, size, byte);
	}
...
bool wcd9xxx_lock_sleep(struct wcd9xxx_core_resource *wcd9xxx_res)
{
   ...
   */
	...
	if (!wait_event_timeout(
		    wcd9xxx_res->pm_wq,
		    ((os = wcd9xxx_pm_cmpxchg(wcd9xxx_res, WCD9XXX_PM_SLEEPABLE,
					      WCD9XXX_PM_AWAKE)) ==
			     WCD9XXX_PM_SLEEPABLE ||
		     (os == WCD9XXX_PM_AWAKE)),
		    msecs_to_jiffies(WCD9XXX_SYSTEM_RESUME_TIMEOUT_MS))) {
		pr_warn("%s: system didn't resume within %dms, s %d, w %d\n",
			__func__, WCD9XXX_SYSTEM_RESUME_TIMEOUT_MS,
			wcd9xxx_res->pm_state, wcd9xxx_res->wlock_holders);
...
static irqreturn_t wcd9xxx_irq_thread(int irq, void *data)
{
	...
	if (!wcd9xxx) {
		dev_err(wcd9xxx_res->dev, "%s: Codec core not supplied\n",
			__func__);
		goto err_disable_irq;
	}
...
err_disable_irq:
	dev_err(wcd9xxx_res->dev, "Disable irq %d\n", wcd9xxx_res->irq);
 ...
 *
 */
void wcd9xxx_free_irq(struct wcd9xxx_core_resource *wcd9xxx_res, int irq,
		      void *data)
{
...
static int
wcd9xxx_irq_setup_downstream_irq(struct wcd9xxx_core_resource *wcd9xxx_res)
{
	int irq, virq, ret;
...
int wcd9xxx_irq_init(struct wcd9xxx_core_resource *wcd9xxx_res)
{
	...
	for (i = 0; i < wcd9xxx_res->num_irqs; i++) {
		...
		irq_level[BIT_BYTE(i)] |= wcd9xxx_res->irq_level_high[i]
					  << (i % BITS_PER_BYTE);
	}

	if (!wcd9xxx_res->wcd_core_regmap) {
		dev_err(wcd9xxx_res->dev, "%s: Codec core regmap not defined\n",
			__func__);
		ret = -EINVAL;
		goto fail_irq_init;
	}
	...
	ret = request_threaded_irq(wcd9xxx_res->irq, NULL, wcd9xxx_irq_thread,
				   IRQF_TRIGGER_HIGH | IRQF_ONESHOT, "wcd9xxx",
				   wcd9xxx_res);
	if (ret != 0)
...
fail_irq_init:
	dev_err(wcd9xxx_res->dev, "%s: Failed to init wcd9xxx irq\n", __func__);
	kfree(irq_level);
fail_irq_level:
...
static int wcd9xxx_map_irq(struct wcd9xxx_core_resource *wcd9xxx_core_res,
			   int irq)
{
...
wcd9xxx_irq_add_domain(struct device_node *node, struct device_node *parent)
{
...
static const struct of_device_id of_match[] = { { .compatible =
							  "qcom,wcd9xxx-irq" },
						{} };
