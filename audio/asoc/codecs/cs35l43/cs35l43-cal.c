				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l43->dsp.booted) {
		...
		val = cpu_to_be32(val);
		cs35l43_wm_adsp_write_ctl(&cs35l43->dsp, "INP_CND_CH_BAL",
					  WMFW_ADSP2_XM, 0x5f20e, (void *)&val,
					  sizeof(__be32));
	}
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l43->dsp.booted) {
		cs35l43_wm_adsp_read_ctl(&cs35l43->dsp, "INP_CND_CH_BAL",
					 WMFW_ADSP2_XM, 0x5f20e, (void *)&val,
					 sizeof(__be32));
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = cs35l43_wm_adsp_read_ctl(dsp, "CAL_AMBIENT", WMFW_ADSP2_XM,
				       CS35L43_ALG_ID_PROTECT_LITE,
	...
	dev_dbg(cs35l43->dev, "%s: get cal_ambient = %d, %d\n", __func__,
		be32_to_cpu(val), cs35l43->ambient);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	val = cpu_to_be32(val);
	ret = cs35l43_wm_adsp_write_ctl(dsp, "CAL_AMBIENT", WMFW_ADSP2_XM,
					CS35L43_ALG_ID_PROTECT_LITE,
					(void *)&val, sizeof(__be32));
	if (ret)
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = cs35l43_wm_adsp_read_ctl(dsp, "CAL_R_INIT", WMFW_ADSP2_XM,
				       CS35L43_ALG_ID_PROTECT_LITE,
				       (void *)&val, sizeof(__be32));
	if (ret)
		return -EIO;
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	val = ucontrol->value.enumerated.item[0];
	val = cpu_to_be32(val);
	ret = cs35l43_wm_adsp_write_ctl(dsp, "CAL_R_SEL", WMFW_ADSP2_XM,
					CS35L43_ALG_ID_PROTECT_LITE,
					(void *)&val, sizeof(__be32));
	if (ret)
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = cs35l43_wm_adsp_read_ctl(dsp, "CAL_R", WMFW_ADSP2_XM,
				       CS35L43_ALG_ID_PROTECT_LITE,
				       (void *)&val, sizeof(__be32));
	if (ret)
		return -EIO;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = cs35l43_wm_adsp_read_ctl(dsp, "CAL_STATUS", WMFW_ADSP2_XM,
				       CS35L43_ALG_ID_PROTECT_LITE,
				       (void *)&val, sizeof(__be32));
	if (ret)
		return -EIO;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = cs35l43_wm_adsp_read_ctl(dsp, "CAL_EN", WMFW_ADSP2_XM,
				       CS35L43_ALG_ID_PROTECT_LITE,
				       (void *)&val, sizeof(__be32));
	if (ret)
		return -EIO;
	...
	dev_info(cs35l43->dev,
		 "%s: DSP is running in mode: %d [1:Play, 2:Calib]\n", __func__,
		 ucontrol->value.enumerated.item[0]);
	return 0;
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (1 == ucontrol->value.enumerated.item[0]) {
		...
		ret = cs35l43_wm_adsp_write_ctl(dsp, "PILOT_THRESH",
						WMFW_ADSP2_XM,
						CS35L43_ALG_ID_PROTECT_LITE,
						(void *)&val, sizeof(__be32));
		if (ret)
			dev_err(cs35l43->dev,
				"%s: Write PILOT_THRESH, ret=%d\n", __func__,
				ret);

		val = cpu_to_be32(0);
		ret = cs35l43_wm_adsp_write_ctl(dsp, "CAL_EN", WMFW_ADSP2_XM,
						CS35L43_ALG_ID_PROTECT_LITE,
						(void *)&val, sizeof(__be32));
		if (ret)
		...
		val = cpu_to_be32(1);
		ret = cs35l43_wm_adsp_write_ctl(dsp, "CALIB_FIRST_RUN",
						WMFW_ADSP2_XM,
						CS35L43_ALG_ID_PROTECT_LITE,
						(void *)&val, sizeof(__be32));
		if (ret)
			dev_err(cs35l43->dev,
				"%s: Write CALIB_FIRST_RUN, ret=%d\n", __func__,
				ret);
	}

	if (dsp->component->name_prefix)
		snprintf(ctl_name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, "%s %s",
			 dsp->component->name_prefix, "Reinit");
	else
	...
	if (cs35l43->cs35l43_rcv_pinctrl && cs35l43->enable_rcv_pin_control) {
		if (cs35l43->enable_rcv_switch) {
			dev_info(
				...
				__func__, cs35l43->enable_rcv_switch);
...
#endif
		dev_info(cs35l43->dev, "Linux kernel version <= 5.10.70, using "
				       "ignore_suspend with name_prefix");
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "AMP Capture");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "AMP Playback");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASPRX1");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASPRX2");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "DSP RX1 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "DSP RX2 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "DSP RX3 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASPTX1");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASPTX2");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASPTX3");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASPTX4");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASP TX1 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASP TX2 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASP TX3 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "ASP TX4 Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "TEMPMON ADC");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "IMON ADC");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "VMON ADC");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "VPMON ADC");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "VBSTMON ADC");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "PCM Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "High Rate PCM Source");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "Hibernate");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "Ultrasonic Mode");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "Main AMP");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "DSP1");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "DSP1 Preload");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "DSP1 Preloader");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "SPK");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s",
			 component->name_prefix, "AMP Enable");
		snd_soc_dapm_ignore_suspend(dapm, widget);
	} else {
		dev_info(cs35l43->dev, "Linux kernel version > 5.10.70, using "
				       "ignore_suspend WITHOUT name_prefix");
