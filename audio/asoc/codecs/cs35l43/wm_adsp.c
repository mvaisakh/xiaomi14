...
#else
static inline void
cs35l43_wm_adsp2_init_debugfs(struct wm_adsp *dsp,
			      struct snd_soc_component *component)
{
}
				   ...
				   size_t len)
{
	...
	adsp_dbg(
		dsp,
		"Wrote %zu bytes <buf[0]= 0x%08x> to addr:%x( ctl name is %s)\n",
			       ...
			       size_t len)
{
	...
	adsp_dbg(
		dsp,
		"write %s: ctl->enabled:%d, ctl->dsp->running:%d, buf = 0x%08x, "
		"ctl->cache=0x%08x\n",
		ctl->name, ctl->enabled, ctl->dsp->running, *((int *)buf),
		*((int *)ctl->cache));
					...
					struct wm_coeff_parsed_coeff *blk)
{
	...
	case 1:
		...
		*data = *data + sizeof(raw->hdr) + le32_to_cpu(raw->hdr.size);
		...
		blk->name_len = strlen(raw->name);
		blk->ctl_type =
			(__force snd_ctl_elem_type_t)le16_to_cpu(raw->ctl_type);
	...
	default:
		...
		blk->ctl_type = (__force snd_ctl_elem_type_t)wm_coeff_parse_int(
			sizeof(raw->ctl_type), &tmp);
					    ...
					    unsigned int alg)
{
	...
	list_for_each_entry(pos, &dsp->ctl_list, list) {
		...
		if (strncmp(pos->subname, name, pos->subname_len) == 0 &&
		    pos->fw_name == fw_txt && pos->alg_region.alg == alg &&
...
{
	static const int types[] = { WMFW_ADSP2_XM, WMFW_HALO_XM_PACKED,
...
int cs35l43_wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	...
	adsp_info(
		dsp,
		"after hw_id_value: 0x%x hw_version_platform：%d hw_country_ver：0x%x\n",
	...
	if (((uint32_t)CountryCN == hw_country_ver) &&
	    !((hw_id_value == 0x1) || (hw_id_value == 0x10000) ||
	      (hw_id_value == 0x10001))) {
...
int cs35l43_wm_adsp2_set_dspclk(struct snd_soc_dapm_widget *w,
				unsigned int freq)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		adsp_info(
			dsp,
			"%s, dsp->running: %d dsp->booted: %d, execution started\n",
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		adsp_info(
			dsp,
			"%s, dsp->running: %d dsp->booted: %d, execution stopped\n",
			__func__, dsp->running, dsp->booted);
		break;
...
int cs35l43_wm_adsp2_component_probe(struct wm_adsp *dsp,
				     struct snd_soc_component *component)
{
	char preload[32];
...
int cs35l43_wm_adsp2_component_remove(struct wm_adsp *dsp,
				      struct snd_soc_component *component)
{
...
int cs35l43_wm_adsp_compr_open(struct wm_adsp *dsp,
			       struct snd_compr_stream *stream)
{
...
static int wm_adsp_buffer_parse_coeff(struct wm_coeff_ctl *ctl)
{
	...
	for (i = 0; i < 5; ++i) {
		ret = regmap_raw_read(ctl->dsp->regmap, reg, &bufp,
				      sizeof(bufp));
		if (ret < 0)
			return ret;
...
static int wm_adsp_buffer_capture_block(struct wm_adsp_compr *compr, int target)
{
	...
	/* Read data from DSP */
	ret = wm_adsp_read_raw_data_block(buf->dsp, mem_type, adsp_addr, nwords,
					  (__be32 *)compr->raw_buf);
	if (ret < 0)
		return ret;
