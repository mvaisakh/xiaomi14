...
static const unsigned int ep92_samp_freq_table[8] = { 32000,  44100, 48000,
						      88200,  96000, 176400,
						      192000, 768000 };

static const unsigned int ep92_dsd_freq_table[4] = { 64, 128, 256, 0 };
...
int ep92_set_ext_mclk(struct snd_soc_codec *codec, uint32_t mclk_freq)
{
	...
	pr_debug("%s mclk:%u, in sample freq:%u, write reg:0x%02x val:0x%02x\n",
		 __func__, mclk_freq, samp_freq, EP92_GENERAL_CONTROL_2,
				      ...
				      loff_t *ppos)
{
	...
	if (rc == 0) {
		...
		dev_err(component->dev,
			"%s: write to register addr=0x%02X failed\n", __func__,
			param[0]);
	}
	return rc;
}
...
static ssize_t debugfs_codec_read_op(struct file *filp, char __user *ubuf,
...
static int ep92_send_uevent(struct ep92_pdata *ep92, char *event)
{
	...
	if (!ep92->component)
		return -ENODEV;
	return kobject_uevent_env(&ep92->component->dev->kobj, KOBJ_CHANGE,
				  env);
}
				      ...
				      struct ep92_pdata *ep92)
{
	...
	old = ep92->gi.tx_info;
	ep92->gi.tx_info =
		snd_soc_component_read32(component, EP92_BI_GENERAL_INFO_0);
	if (ep92->gi.tx_info == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_BI_GENERAL_INFO_0 read 0xff\n");
		ep92->gi.tx_info = old;
	}
	...
	if (ep92->gi.tx_info & EP92_GI_TX_HOT_PLUG_MASK) {
		...
		if (ep92->hyst_tx_plug > 0) {
			...
			if ((ep92->hyst_tx_plug == 0) &&
				...
				ep92->filt_tx_plug = 0;
				dev_dbg(component->dev,
					"ep92 out_plug changed to 0\n");
				ep92_send_uevent(
					ep92, "EP92EVT_OUT_PLUG=DISCONNECTED");
			}
		}
	}
	...
	ep92->gi.video_latency =
		snd_soc_component_read32(component, EP92_BI_GENERAL_INFO_4);
	if (ep92->gi.video_latency == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_BI_GENERAL_INFO_4 read 0xff\n");
	...
	old = ep92->gc.ctl;
	ep92->gc.ctl =
		snd_soc_component_read32(component, EP92_GENERAL_CONTROL_0);
	if (ep92->gc.ctl == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_GENERAL_CONTROL_0 read 0xff\n");
		ep92->gc.ctl = old;
	}
	...
	old = ep92->gc.rx_sel;
	ep92->gc.rx_sel =
		snd_soc_component_read32(component, EP92_GENERAL_CONTROL_1);
	if (ep92->gc.rx_sel == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_GENERAL_CONTROL_1 read 0xff\n");
		ep92->gc.rx_sel = old;
	}
	...
	ep92->gc.cec_volume =
		snd_soc_component_read32(component, EP92_GENERAL_CONTROL_3);
	if (ep92->gc.cec_volume == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_GENERAL_CONTROL_3 read 0xff\n");
	...
	old = ep92->gc.link;
	ep92->gc.link =
		snd_soc_component_read32(component, EP92_GENERAL_CONTROL_4);
	if (ep92->gc.link == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_GENERAL_CONTROL_4 read 0xff\n");
		ep92->gc.link = old;
	}
	...
	if (ep92->gc.link & EP92_GC_LINK_ON0_MASK) {
		...
		if (ep92->hyst_link_on0 > 0) {
			...
			if ((ep92->hyst_link_on0 == 0) &&
				...
				ep92->filt_link_on0 = 0;
				dev_dbg(component->dev,
					"ep92 link_on0 changed to 0\n");
				ep92_send_uevent(
					ep92, "EP92EVT_LINK_ON0=DISCONNECTED");
			}
		}
	}
	...
	if (ep92->gc.link & EP92_GC_LINK_ON1_MASK) {
		...
		if (ep92->hyst_link_on1 > 0) {
			...
			if ((ep92->hyst_link_on1 == 0) &&
				...
				ep92->filt_link_on1 = 0;
				dev_dbg(component->dev,
					"ep92 link_on1 changed to 0\n");
				ep92_send_uevent(
					ep92, "EP92EVT_LINK_ON1=DISCONNECTED");
			}
		}
	}
	...
	if (ep92->gc.link & EP92_GC_LINK_ON2_MASK) {
		...
		if (ep92->hyst_link_on2 > 0) {
			...
			if ((ep92->hyst_link_on2 == 0) &&
				...
				ep92->filt_link_on2 = 0;
				dev_dbg(component->dev,
					"ep92 link_on2 changed to 0\n");
				ep92_send_uevent(
					ep92, "EP92EVT_LINK_ON2=DISCONNECTED");
			}
		}
	}
}
				 ...
				 struct ep92_pdata *ep92)
{
	...
	ep92->ai.system_status_0 = snd_soc_component_read32(
		component, EP92_AUDIO_INFO_SYSTEM_STATUS_0);
	if (ep92->ai.system_status_0 == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_AUDIO_INFO_SYSTEM_STATUS_0 read 0xff\n");
	...
	ep92->ai.system_status_1 =
		snd_soc_read(codec, EP92_AUDIO_INFO_SYSTEM_STATUS_1);
	...
	ep92->ai.audio_status = snd_soc_component_read32(
		component, EP92_AUDIO_INFO_AUDIO_STATUS);
	if (ep92->ai.audio_status == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_AUDIO_INFO_AUDIO_STATUS read 0xff\n");
	...
	old = ep92->ai.cs[0];
	ep92->ai.cs[0] = snd_soc_component_read32(
		component, EP92_AUDIO_INFO_CHANNEL_STATUS_0);
	if (ep92->ai.cs[0] == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_AUDIO_INFO_CHANNEL_STATUS_0 read 0xff\n");
		ep92->ai.cs[0] = old;
	}
	...
	old = ep92->ai.cc;
	ep92->ai.cc = snd_soc_component_read32(
		component, EP92_AUDIO_INFO_ADO_INFO_FRAME_1);
	if (ep92->ai.cc == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_AUDIO_INFO_ADO_INFO_FRAME_1 read 0xff\n");
		ep92->ai.cc = old;
	}
	...
	if (change & EP92_AI_CH_COUNT_MASK) {
		dev_dbg(component->dev, "ep92 ch_count changed to %d (%d)\n",
			...
			(ep92->ai.cc & EP92_AI_CH_COUNT_MASK) == 0 ?
				0 :
	...
	old = ep92->ai.ca;
	ep92->ai.ca = snd_soc_component_read32(
		component, EP92_AUDIO_INFO_ADO_INFO_FRAME_4);
	if (ep92->ai.ca == 0xff) {
		dev_dbg(component->dev,
			"ep92 EP92_AUDIO_INFO_ADO_INFO_FRAME_4 read 0xff\n");
		ep92->ai.ca = old;
	}
...
static int ep92_probe(struct snd_soc_component *component)
{
	...
	mod_timer(&ep92->timer,
		  jiffies + msecs_to_jiffies(EP92_POLL_INTERVAL_OFF_MSEC));
...
{
	struct ep92_pdata *ep92 =
		container_of(work, struct ep92_pdata, read_status_worker);
	struct snd_soc_component *component = ep92->component;
	u8 val;
...
static irqreturn_t ep92_irq(int irq, void *data)
{
	...
	ep92->poll_trig = 1;
	mod_timer(&ep92->timer,
		  jiffies + msecs_to_jiffies(EP92_POLL_INTERVAL_ON_MSEC));
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	reg0 = snd_soc_component_read32(ep92->component, EP92_BI_VERSION_YEAR);
	reg1 = snd_soc_component_read32(ep92->component, EP92_BI_VERSION_MONTH);
	reg2 = snd_soc_component_read32(ep92->component, EP92_BI_VERSION_DATE);
	reg3 = snd_soc_component_read32(ep92->component, EP92_BI_VERSION_NUM);
					 ...
					 char *buf)
{
	...
	val = ep92_samp_freq_table[(ep92->ai.audio_status) & EP92_AI_RATE_MASK];
	ret = snprintf(buf, EP92_SYSFS_ENTRY_MAX_LEN, "%d\n", val);
	dev_dbg(dev, "%s: '%d'\n", __func__, val);
					    ...
					    char *buf)
{
	...
	val = (ep92->ai.cs[0] & EP92_AI_PREEMPH_MASK) >> EP92_AI_PREEMPH_SHIFT;
					  ...
					  char *buf)
{
	...
	val = (ep92->gc.ctl2 >> EP92_GC_ARC_DIS_SHIFT) & EP92_2CHOICE_MASK;
					  ...
					  const char *buf, size_t count)
{
	...
	reg = snd_soc_component_read32(ep92->component, EP92_GENERAL_CONTROL_2);
				      ...
				      const char *buf, size_t count)
{
	...
	reg = snd_soc_component_read32(ep92->component, EP92_GENERAL_CONTROL_1);
					 ...
					 const char *buf, size_t count)
{
	...
	ep92->gc.ctl |= (val << EP92_GC_ARC_EN_SHIFT) & EP92_GC_ARC_EN_MASK;
				       ...
				       const char *buf, size_t count)
{
	...
	ep92->gc.ctl |= (val << EP92_GC_CEC_MUTE_SHIFT) & EP92_GC_CEC_MUTE_MASK;
...
static struct attribute *ep92_fs_attrs[] = {
	&dev_attr_chipid.attr,	       &dev_attr_version.attr,
	&dev_attr_audio_state.attr,    &dev_attr_audio_format.attr,
	&dev_attr_audio_rate.attr,     &dev_attr_audio_layout.attr,
	&dev_attr_audio_ch_count.attr, &dev_attr_audio_ch_alloc.attr,
	&dev_attr_audio_preemph.attr,  &dev_attr_audio_avmute.attr,
	&dev_attr_link_on0.attr,       &dev_attr_link_on1.attr,
	&dev_attr_link_on2.attr,       &dev_attr_out_plug.attr,
	&dev_attr_video_latency.attr,  &dev_attr_arc_disable.attr,
	&dev_attr_power_on.attr,       &dev_attr_audio_path.attr,
	&dev_attr_src_sel.attr,	       &dev_attr_arc_enable.attr,
	&dev_attr_cec_mute.attr,       &dev_attr_cec_volume.attr,
	&dev_attr_runout.attr,	       &dev_attr_force_inactive.attr,
	&dev_attr_dsd_rate.attr,       NULL,
};
...
static int ep92_sysfs_create(struct i2c_client *client, struct ep92_pdata *ep92)
{
	int rc;
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (ep92->irq) {
		ret = devm_request_threaded_irq(
			&client->dev, ep92->irq, NULL, ep92_irq,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "ep92_irq", ep92);
		if (ret) {
			dev_err(&client->dev,
...
static const struct i2c_device_id ep92_i2c_id[] = { { "ep92-dev", 0 }, {} };
