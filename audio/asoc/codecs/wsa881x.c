...
static const char *const wsa_pa_gain_text[] = {
	"G_18_DB",   "G_16P5_DB", "G_15_DB",  "G_13P5_DB", "G_12_DB",
	"G_10P5_DB", "G_9_DB",	  "G_7P5_DB", "G_6_DB",	   "G_4P5_DB",
	"G_3_DB",    "G_1P5_DB",  "G_0_DB"
};
...
static const struct snd_kcontrol_new wsa_snd_controls[] = {
	SOC_ENUM_EXT("WSA PA Gain", wsa_pa_gain_enum, wsa_pa_gain_get,
		     wsa_pa_gain_put),
	SOC_SINGLE_EXT("WSA PA Mute", SND_SOC_NOPM, 0, 1, 0, wsa881x_get_mute,
		       wsa881x_set_mute),
					  ...
					  struct snd_soc_component *component)
{
	...
	wsa881x->entry = snd_info_create_module_entry(
	...
	if (!version_entry) {
		dev_dbg(component->dev,
			"%s: failed to create wsa881x version entry\n",
			__func__);
		snd_info_free_entry(wsa881x->entry);
		return -ENOMEM;
	}
...
static bool is_swr_slv_reg_readable(int reg)
{
	...
	if (((reg > 0x46) && (reg < 0x4A)) || ((reg > 0x4A) && (reg < 0x50)) ||
	    ((reg > 0x55) && (reg < 0xE0)) || ((reg > 0xE0) && (reg < 0xF0)) ||
	    ((reg > 0xF0) && (reg < 0x100)) ||
	    ((reg > 0x105) && (reg < 0x120)) ||
	    ((reg > 0x128) && (reg < 0x130)) ||
	    ((reg > 0x138) && (reg < 0x200)) ||
					 ...
					 loff_t *ppos)
{
	...
	for (i = (((int)*ppos / BYTES_PER_LINE) + SWR_SLV_START_REG_ADDR);
		...
		if (!is_swr_slv_reg_readable(i))
			continue;
		swr_read(dbgwsa881x->swr_slave, devnum, i, &reg_val, 1);
...
static ssize_t codec_debug_write(struct file *filp, const char __user *ubuf,
				 size_t cnt, loff_t *ppos)
{
	...
	if (!strcmp(access_str, "swrslave_poke")) {
		...
		if ((param[0] <= SWR_SLV_MAX_REG_ADDR) && (param[1] <= 0xFF) &&
		    (rc == 0))
			swr_write(dbgwsa881x->swr_slave, param[2], param[0],
				  &param[1]);
		else
			rc = -EINVAL;
		...
		if ((param[0] <= SWR_SLV_MAX_REG_ADDR) && (rc == 0))
			swr_read(dbgwsa881x->swr_slave, param[1], param[0],
				 &read_data, 1);
		else
			rc = -EINVAL;
	} else if (!strcmp(access_str, "swrslave_reg_dump")) {
				     ...
				     u8 isense2_gain, u8 vsense_gain)
{
	...
	if (enable) {
		...
		snd_soc_component_update_bits(
			component, WSA881X_SPKR_PROT_FE_VSENSE_VCM, 0x08, 0x08);
		/*
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	wsa_boost_level =
		snd_soc_component_read32(component, WSA881X_BOOST_PRESET_OUT1);
				       ...
				       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		wsa881x_set_port(component, SWR_DAC_PORT, &port_id[num_port],
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		swr_slvdev_datapath_control(wsa881x->swr_slave,
					    wsa881x->swr_slave->dev_num, false);
				 ...
				 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		swr_slvdev_datapath_control(wsa881x->swr_slave,
					    wsa881x->swr_slave->dev_num, true);
		/* Set register mode if compander is not enabled */
		if (!wsa881x->comp_enable)
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_DRV_GAIN, 0x08, 0x08);
		else
			snd_soc_component_update_bits(
				component, WSA881X_SPKR_DRV_GAIN, 0x08, 0x00);
	...
	case SND_SOC_DAPM_POST_PMU:
       ...
       */
		...
		if (wsa881x->visense_enable) {
			wsa881x_visense_txfe_ctrl(component, ENABLE, 0x00, 0x03,
						  0x01);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (wsa881x->visense_enable) {
			...
			snd_soc_component_update_bits(component,
						      WSA881X_ADC_EN_SEL_IBAIS,
						      0x07, 0x00);
			wsa881x_visense_txfe_ctrl(component, DISABLE, 0x00,
						  0x01, 0x01);
		}
...
static void wsa881x_init(struct snd_soc_component *component)
{
	...
	snd_soc_component_update_bits(component, WSA881X_SWR_RESET_EN, 0x07,
				      0x07);
	/* Bring out of analog reset */
	snd_soc_component_update_bits(component, WSA881X_CDC_RST_CTL, 0x02,
				      0x02);
	/* Bring out of digital reset */
	snd_soc_component_update_bits(component, WSA881X_CDC_RST_CTL, 0x01,
				      0x01);

	snd_soc_component_update_bits(component, WSA881X_CLOCK_CONFIG, 0x10,
				      0x10);
	snd_soc_component_update_bits(component, WSA881X_SPKR_OCP_CTL, 0x02,
				      0x02);
	snd_soc_component_update_bits(component, WSA881X_SPKR_MISC_CTL1, 0xC0,
				      0x80);
	snd_soc_component_update_bits(component, WSA881X_SPKR_MISC_CTL1, 0x06,
				      0x06);
	snd_soc_component_update_bits(component, WSA881X_SPKR_BIAS_INT, 0xFF,
				      0x00);
	snd_soc_component_update_bits(component, WSA881X_SPKR_PA_INT, 0xF0,
				      0x40);
	snd_soc_component_update_bits(component, WSA881X_SPKR_PA_INT, 0x0E,
				      0x0E);
	snd_soc_component_update_bits(component, WSA881X_BOOST_LOOP_STABILITY,
				      0x03, 0x03);
	snd_soc_component_update_bits(component, WSA881X_BOOST_MISC2_CTL, 0xFF,
				      0x14);
	snd_soc_component_update_bits(component, WSA881X_BOOST_START_CTL, 0x80,
				      0x80);
	snd_soc_component_update_bits(component, WSA881X_BOOST_START_CTL, 0x03,
				      0x00);
	snd_soc_component_update_bits(
		component, WSA881X_BOOST_SLOPE_COMP_ISENSE_FB, 0x0C, 0x04);
	snd_soc_component_update_bits(
		component, WSA881X_BOOST_SLOPE_COMP_ISENSE_FB, 0x03, 0x00);
	if (snd_soc_component_read32(component, WSA881X_OTP_REG_0))
		snd_soc_component_update_bits(
			component, WSA881X_BOOST_PRESET_OUT1, 0xF0, 0x70);
	snd_soc_component_update_bits(component, WSA881X_BOOST_PRESET_OUT2,
				      0xF0, 0x30);
	snd_soc_component_update_bits(component, WSA881X_SPKR_DRV_EN, 0x08,
				      0x08);
	snd_soc_component_update_bits(component, WSA881X_BOOST_CURRENT_LIMIT,
				      0x0F, 0x08);
	snd_soc_component_update_bits(component, WSA881X_SPKR_OCP_CTL, 0x30,
				      0x30);
	snd_soc_component_update_bits(component, WSA881X_SPKR_OCP_CTL, 0x0C,
				      0x00);
	snd_soc_component_update_bits(component, WSA881X_OTP_REG_28, 0x3F,
				      0x3A);
	snd_soc_component_update_bits(component, WSA881X_BONGO_RESRV_REG1, 0xFF,
				      0xB2);
	snd_soc_component_update_bits(component, WSA881X_BONGO_RESRV_REG2, 0xFF,
				      0x05);
}
				     ...
				     struct wsa_temp_register *wsa_temp_reg)
{
	...
	snd_soc_component_update_bits(component, WSA881X_TADC_VALUE_CTL, 0x01,
				      0x00);
	wsa_temp_reg->dmeas_msb =
		snd_soc_component_read32(component, WSA881X_TEMP_MSB);
	wsa_temp_reg->dmeas_lsb =
		snd_soc_component_read32(component, WSA881X_TEMP_LSB);
	snd_soc_component_update_bits(component, WSA881X_TADC_VALUE_CTL, 0x01,
				      0x01);
	wsa_temp_reg->d1_msb =
		snd_soc_component_read32(component, WSA881X_OTP_REG_1);
	wsa_temp_reg->d1_lsb =
		snd_soc_component_read32(component, WSA881X_OTP_REG_2);
	wsa_temp_reg->d2_msb =
		snd_soc_component_read32(component, WSA881X_OTP_REG_3);
	wsa_temp_reg->d2_lsb =
		snd_soc_component_read32(component, WSA881X_OTP_REG_4);
...
static int wsa881x_gpio_init(struct swr_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "%s: gpio %d request with name %s\n", __func__,
		wsa881x->pd_gpio, dev_name(&pdev->dev));
...
static int wsa881x_event_notify(struct notifier_block *nb, unsigned long val,
				void *ptr)
{
	u16 event = (val & 0xffff);
	struct wsa881x_priv *wsa881x =
		container_of(nb, struct wsa881x_priv, bolero_nblock);
	...
	case BOLERO_SLV_EVT_PA_OFF_PRE_SSR:
		snd_soc_component_update_bits(
			wsa881x->component, WSA881X_SPKR_DRV_GAIN, 0xF0, 0xC0);
...
static int wsa881x_swr_probe(struct swr_device *pdev)
{
	...
	if (wsa881x->wsa_rst_np)
		pin_state_current =
			msm_cdc_pinctrl_get_state(wsa881x->wsa_rst_np);
	...
	if (!debugfs_wsa881x_dent) {
		...
		debugfs_wsa881x_dent =
			debugfs_create_dir("wsa881x_swr_slave", 0);
		if (!IS_ERR(debugfs_wsa881x_dent)) {
			debugfs_peek = debugfs_create_file(
				"swrslave_peek", S_IFREG | 0444,
				debugfs_wsa881x_dent, (void *)"swrslave_peek",
				&codec_debug_ops);

			debugfs_poke = debugfs_create_file(
				"swrslave_poke", S_IFREG | 0444,
				debugfs_wsa881x_dent, (void *)"swrslave_poke",
				&codec_debug_ops);

			debugfs_reg_dump = debugfs_create_file(
				"swrslave_reg_dump", S_IFREG | 0444,
				debugfs_wsa881x_dent,
				(void *)"swrslave_reg_dump", &codec_debug_ops);
		}
	}
...
static const struct dev_pm_ops wsa881x_swr_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	wsa881x_swr_suspend, wsa881x_swr_resume) };

static const struct swr_device_id wsa881x_swr_id[] = { { "wsa881x", 0 }, {} };
