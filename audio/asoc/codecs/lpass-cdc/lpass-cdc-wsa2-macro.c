...
#define LPASS_CDC_WSA2_MACRO_RX_RATES                                        \
	(SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_32000 | \
	 SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_96000 | SNDRV_PCM_RATE_192000)
#define LPASS_CDC_WSA2_MACRO_RX_MIX_RATES \
	(SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_96000 | SNDRV_PCM_RATE_192000)
#define LPASS_CDC_WSA2_MACRO_RX_FORMATS                      \
	(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | \
	 SNDRV_PCM_FMTBIT_S24_3LE | SNDRV_PCM_FMTBIT_S32_LE)

#define LPASS_CDC_WSA2_MACRO_ECHO_RATES \
	(SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_48000)
#define LPASS_CDC_WSA2_MACRO_ECHO_FORMATS                    \
	(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | \
...
static struct lpass_cdc_comp_setting comp_setting_table[G_MAX_DB] = {
	{ 42, 0, 42 }, { 39, 0, 42 }, { 36, 0, 42 },
	{ 33, 0, 42 }, { 30, 0, 42 }, { 27, 0, 42 },
	{ 24, 0, 42 }, { 21, 0, 42 }, { 18, 0, 42 },
};
...
static int lpass_cdc_wsa2_macro_mute_stream(struct snd_soc_dai *dai, int mute,
					    int stream);
...
static int
lpass_cdc_wsa2_macro_enable_vi_decimator(struct snd_soc_component *component);

#define LPASS_CDC_WSA2_MACRO_SET_VOLUME_TLV(xname, xreg, xmin, xmax,         \
					    tlv_array)                       \
	{                                                                    \
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname),        \
		.access = SNDRV_CTL_ELEM_ACCESS_TLV_READ |                   \
			  SNDRV_CTL_ELEM_ACCESS_READWRITE,                   \
		.tlv.p = (tlv_array), .info = snd_soc_info_volsw,            \
		.get = snd_soc_get_volsw,                                    \
		.put = lpass_cdc_wsa2_macro_set_digital_volume,              \
		.private_value = (unsigned long)&(struct soc_mixer_control)  \
		{                                                            \
			.reg = xreg, .rreg = xreg, .min = xmin, .max = xmax, \
			.sign_bit = 7,                                       \
		}                                                            \
	}
...
struct lpass_cdc_wsa2_macro_priv {
	...
	struct platform_device
		*pdev_child_devices[LPASS_CDC_WSA2_MACRO_CHILD_DEVICES_MAX];
	int child_count;
	int wsa2_spkrrecv;
...
static const char *const rx_text[] = { "ZERO",	  "RX0", "RX1",	 "RX_MIX0",
				       "RX_MIX1", "RX4", "RX5",	 "RX6",
				       "RX7",	  "RX8", "DEC0", "DEC1" };

static const char *const rx_mix_text[] = { "ZERO",    "RX0", "RX1", "RX_MIX0",
					   "RX_MIX1", "RX4", "RX5", "RX6",
					   "RX7",     "RX8" };

static const char *const rx_mix_ec_text[] = { "ZERO", "RX_MIX_TX0",
					      "RX_MIX_TX1" };

static const char *const rx_mux_text[] = { "ZERO", "AIF1_PB", "AIF_MIX1_PB" };

static const char *const rx_sidetone_mix_text[] = { "ZERO", "SRC0" };
...
static const struct soc_enum rx0_prim_inp0_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA2_RX_INP_MUX_RX_INT0_CFG0, 0, 12, rx_text);

static const struct soc_enum rx0_prim_inp1_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA2_RX_INP_MUX_RX_INT0_CFG0, 3, 12, rx_text);

static const struct soc_enum rx0_prim_inp2_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA2_RX_INP_MUX_RX_INT0_CFG1, 3, 12, rx_text);

static const struct soc_enum rx0_mix_chain_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA2_RX_INP_MUX_RX_INT0_CFG1, 0, 10, rx_mix_text);
...
static const struct soc_enum rx1_prim_inp0_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA2_RX_INP_MUX_RX_INT1_CFG0, 0, 12, rx_text);

static const struct soc_enum rx1_prim_inp1_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA2_RX_INP_MUX_RX_INT1_CFG0, 3, 12, rx_text);

static const struct soc_enum rx1_prim_inp2_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA2_RX_INP_MUX_RX_INT1_CFG1, 3, 12, rx_text);

static const struct soc_enum rx1_mix_chain_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA2_RX_INP_MUX_RX_INT1_CFG1, 0, 10, rx_mix_text);
...
static const struct soc_enum rx_mix_ec0_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA2_RX_INP_MUX_RX_MIX_CFG0, 0, 3, rx_mix_ec_text);

static const struct soc_enum rx_mix_ec1_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA2_RX_INP_MUX_RX_MIX_CFG0, 3, 3, rx_mix_ec_text);
...
static bool lpass_cdc_wsa2_macro_get_data(
	struct snd_soc_component *component, struct device **wsa2_dev,
	struct lpass_cdc_wsa2_macro_priv **wsa2_priv, const char *func_name)
{
...
static int
lpass_cdc_wsa2_macro_set_port_map(struct snd_soc_component *component,
				  u32 usecase, u32 size, void *data)
{
	...
	if (!lpass_cdc_wsa2_macro_get_data(component, &wsa2_dev, &wsa2_priv,
					   __func__))
		return -EINVAL;
	...
	if (wsa2_priv->swr_ctrl_data)
		ret = swrm_wcd_notify(wsa2_priv->swr_ctrl_data[0].wsa2_swr_pdev,
				      SWR_SET_PORT_MAP, &port_cfg);

	return ret;
}

static int lpass_cdc_wsa2_macro_set_prim_interpolator_rate(
	struct snd_soc_dai *dai, u8 int_prim_fs_rate_reg_val, u32 sample_rate)
{
	u8 int_1_mix1_inp;
	u32 j, port;
...
static int lpass_cdc_wsa2_macro_set_mix_interpolator_rate(
	struct snd_soc_dai *dai, u8 int_mix_fs_rate_reg_val, u32 sample_rate)
{
	u8 int_2_inp;
	u32 j, port;
						      ...
						      u32 sample_rate)
{
	...
	if ((i == ARRAY_SIZE(int_mix_sample_rate_val)) || (rate_val < 0))
		goto prim_rate;
	ret = lpass_cdc_wsa2_macro_set_mix_interpolator_rate(dai, (u8)rate_val,
							     sample_rate);
prim_rate:
	...
	if ((i == ARRAY_SIZE(int_prim_sample_rate_val)) || (rate_val < 0))
		return -EINVAL;
	ret = lpass_cdc_wsa2_macro_set_prim_interpolator_rate(dai, (u8)rate_val,
							      sample_rate);
	return ret;
}
					  ...
					  struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_STREAM_PLAYBACK:
		ret = lpass_cdc_wsa2_macro_set_interpolator_rate(
						...
						unsigned int *rx_slot)
{
	...
	case LPASS_CDC_WSA2_MACRO_AIF_ECHO:
		val = snd_soc_component_read(
			component, LPASS_CDC_WSA2_RX_INP_MUX_RX_MIX_CFG0);
	...
	case LPASS_CDC_WSA2_MACRO_AIF_MIX1_PB:
		for (j = 0; j < NUM_INTERPOLATORS; ++j) {
			...
			snd_soc_component_update_bits(component, reg, 0x10,
						      0x00);
			snd_soc_component_update_bits(component, mix_reg, 0x10,
						      0x00);
		}
	}
}
...
static int
lpass_cdc_wsa2_macro_mclk_enable(struct lpass_cdc_wsa2_macro_priv *wsa2_priv,
				 bool mclk_enable, bool dapm)
{
	...
	dev_dbg(wsa2_priv->dev,
		"%s: mclk_enable = %u, dapm = %d clk_users= %d\n", __func__,
		mclk_enable, dapm, wsa2_priv->wsa2_mclk_users);

	mutex_lock(&wsa2_priv->mclk_lock);
	if (mclk_enable) {
		if (wsa2_priv->wsa2_mclk_users == 0) {
			ret = lpass_cdc_clk_rsc_request_clock(
				wsa2_priv->dev, wsa2_priv->default_clk_id,
				wsa2_priv->default_clk_id, true);
			if (ret < 0) {
				dev_err_ratelimited(
					wsa2_priv->dev,
			...
			lpass_cdc_clk_rsc_fs_gen_request(wsa2_priv->dev, true);
			regcache_mark_dirty(regmap);
			regcache_sync_region(regmap, WSA2_START_OFFSET,
					     WSA2_MAX_OFFSET);
			/* 9.6MHz MCLK, set value 0x00 if other frequency */
			regmap_update_bits(regmap, LPASS_CDC_WSA2_TOP_FREQ_MCLK,
					   0x01, 0x01);
			regmap_update_bits(
				regmap,
				LPASS_CDC_WSA2_CLK_RST_CTRL_MCLK_CONTROL, 0x01,
				0x01);
			/* Toggle fs_cntr_clr bit*/
			regmap_update_bits(
				regmap,
				LPASS_CDC_WSA_CLK_RST_CTRL_FS_CNT_CONTROL, 0x02,
				0x02);
			regmap_update_bits(
				regmap,
				LPASS_CDC_WSA_CLK_RST_CTRL_FS_CNT_CONTROL, 0x02,
		...
		if (wsa2_priv->wsa2_mclk_users == 0) {
			...
			regmap_update_bits(
				...
				LPASS_CDC_WSA2_CLK_RST_CTRL_MCLK_CONTROL, 0x01,
				0x00);
			lpass_cdc_clk_rsc_fs_gen_request(wsa2_priv->dev, false);

			lpass_cdc_clk_rsc_request_clock(
				wsa2_priv->dev, wsa2_priv->default_clk_id,
				wsa2_priv->default_clk_id, false);
		}
	}
exit:
...
static int
lpass_cdc_wsa2_macro_event_handler(struct snd_soc_component *component,
				   u16 event, u32 data)
{
	struct device *wsa2_dev = NULL;
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv = NULL;
...
lpass_cdc_wsa2_macro_enable_vi_decimator(struct snd_soc_component *component)
{
	...
	if (test_bit(LPASS_CDC_WSA2_MACRO_TX0,
		...
		/* Enable V&I sensing */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX1_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX1_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX1_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x20,
			0x00);
	...
	if (test_bit(LPASS_CDC_WSA2_MACRO_TX1,
		...
		/* Enable V&I sensing */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX3_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX3_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX3_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x20,
			0x00);
...
static int lpass_cdc_wsa2_macro_disable_vi_feedback(
	struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	struct device *wsa2_dev = NULL;
	...
	case SND_SOC_DAPM_POST_PMD:
		if (test_bit(LPASS_CDC_WSA2_MACRO_TX0,
			     &wsa2_priv->active_ch_mask
				      [LPASS_CDC_WSA2_MACRO_AIF_VI])) {
			/* Disable V&I sensing */
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x20,
				0x20);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX1_SPKR_PROT_PATH_CTL, 0x20,
				0x20);
			dev_dbg(wsa2_dev, "%s: spkr1 disabled\n", __func__);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x10,
				0x00);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX1_SPKR_PROT_PATH_CTL, 0x10,
				0x00);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX0_SPKR_PROT_PATH_CTL, 0x20,
		...
		if (test_bit(LPASS_CDC_WSA2_MACRO_TX1,
			...
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x20,
				0x20);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX3_SPKR_PROT_PATH_CTL, 0x20,
				0x20);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x10,
				0x00);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX3_SPKR_PROT_PATH_CTL, 0x10,
				0x00);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_TX2_SPKR_PROT_PATH_CTL, 0x20,
...
static void
lpass_cdc_wsa2_macro_hd2_control(struct snd_soc_component *component, u16 reg,
				 int event)
{
	u16 hd2_scale_reg;
					   ...
					   int event)
{
	...
	dev_dbg(wsa2_priv->dev, "%s: current swr ch cnt: %d\n", __func__,
		wsa2_priv->rx_0_count + wsa2_priv->rx_1_count);
						...
						int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, w->reg, 0x20, 0x00);
		lpass_cdc_wsa2_macro_enable_swr(w, kcontrol, event);
		break;
	}
...
static int
lpass_cdc_wsa2_macro_config_compander(struct snd_soc_component *component,
				      int comp, int event)
{
...
static void lpass_cdc_wsa2_macro_enable_softclip_clk(
	struct snd_soc_component *component,
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv, int path, bool enable)
{
	...
	if (enable) {
		if (wsa2_priv->softclip_clk_users[path] == 0) {
			snd_soc_component_update_bits(
				component, softclip_clk_reg, 0x01, 0x01);
			snd_soc_component_update_bits(
				component,
		...
		if (wsa2_priv->softclip_clk_users[path] == 0) {
			snd_soc_component_update_bits(
				component, softclip_clk_reg, 0x01, 0x00);
			snd_soc_component_update_bits(
				component,
				LPASS_CDC_WSA2_RX_INP_MUX_SOFTCLIP_CFG0,
...
static int
lpass_cdc_wsa2_macro_config_softclip(struct snd_soc_component *component,
				     int path, int event)
{
	...
	softclip_ctrl_reg =
		LPASS_CDC_WSA2_SOFTCLIP0_SOFTCLIP_CTRL +
					  ...
					  int path, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		snd_soc_component_update_bits(component, reg1, 0x08, 0x08);
		snd_soc_component_update_bits(component, reg2, 0x40, 0x40);
		snd_soc_component_update_bits(component, reg3, 0x80, 0x80);
		lpass_cdc_wsa2_macro_enable_softclip_clk(component, wsa2_priv,
							 softclip_path, true);
		if (wsa2_priv->pbr_clk_users == 0)
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA2_PBR_PATH_CTL, 0x01,
				0x01);
	...
	if (SND_SOC_DAPM_EVENT_OFF(event)) {
		...
		lpass_cdc_wsa2_macro_enable_softclip_clk(component, wsa2_priv,
							 softclip_path, false);
		snd_soc_component_update_bits(component, reg1, 0x08, 0x00);
		snd_soc_component_update_bits(component, reg2, 0x40, 0x00);
		snd_soc_component_update_bits(component, reg3, 0x80, 0x00);
		--wsa2_priv->pbr_clk_users;
		if (wsa2_priv->pbr_clk_users < 0)
...
static int lpass_cdc_wsa2_macro_enable_prim_interpolator(
	struct snd_soc_component *component, u16 reg, int event)
{
	u16 prim_int_reg;
	u16 ind = 0;
	struct device *wsa2_dev = NULL;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (wsa2_priv->prim_int_users[ind] == 1) {
			snd_soc_component_update_bits(
				component,
				prim_int_reg +
					LPASS_CDC_WSA2_MACRO_RX_PATH_CFG3_OFFSET,
				0x03, 0x03);
			snd_soc_component_update_bits(component, prim_int_reg,
						      0x10, 0x10);
			lpass_cdc_wsa2_macro_hd2_control(component,
							 prim_int_reg, event);
			snd_soc_component_update_bits(
				component,
				prim_int_reg +
		...
		if ((reg != prim_int_reg) &&
		    ((snd_soc_component_read(component, prim_int_reg)) & 0x10))
			snd_soc_component_update_bits(component, reg, 0x10,
						      0x10);
		break;
	case SND_SOC_DAPM_POST_PMD:
...
static void
lpass_cdc_macro_idle_detect_control(struct snd_soc_component *component,
				    ...
				    int interp, int event)
{
	...
	if ((wsa2_priv->noise_gate_mode == NG2 && mode >= G_13P5_DB) ||
	    ...
	    !wsa2_priv->pbr_enable || wsa2_priv->wsa2_spkrrecv) {
		snd_soc_component_update_bits(component, source_reg, 0x80,
					      0x00);
...
static int lpass_cdc_wsa2_macro_enable_interpolator(
	struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* Reset if needed */
		lpass_cdc_wsa2_macro_enable_prim_interpolator(component, reg,
							      event);
		break;
	case SND_SOC_DAPM_POST_PMU:
		if (!strcmp(w->name, "WSA2_RX INT0 INTERP")) {
			...
			if (snd_soc_component_read(
				...
				snd_soc_component_update_bits(
					wsa2_priv->component,
					LPASS_CDC_WSA2_RX0_RX_VOL_CTL, 0xFF,
					gain);
				dev_dbg(wsa2_priv->dev,
		...
		if (!strcmp(w->name, "WSA2_RX INT1 INTERP")) {
			...
			if (snd_soc_component_read(
				...
				snd_soc_component_update_bits(
					wsa2_priv->component,
					LPASS_CDC_WSA2_RX1_RX_VOL_CTL, 0xFF,
					gain);
				dev_dbg(wsa2_priv->dev,
		...
		lpass_cdc_wsa2_macro_config_compander(component, w->shift,
						      event);
		lpass_cdc_macro_idle_detect_control(component, wsa2_priv,
						    w->shift, event);
		lpass_cdc_wsa2_macro_config_softclip(component, w->shift,
						     event);
		lpass_cdc_was_macro_config_pbr(component, w->shift, event);
		if (wsa2_priv->wsa2_spkrrecv)
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA2_RX0_RX_PATH_CFG1,
				0x08, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_RX0_RX_PATH_CFG1, 0x08, 0x08);
		lpass_cdc_wsa2_macro_config_compander(component, w->shift,
						      event);
		lpass_cdc_macro_idle_detect_control(component, wsa2_priv,
						    w->shift, event);
					    ...
					    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_PATH_CTL, 0x10,
			0x10);
		/* Enable VBAT block */
		...
		snd_soc_component_update_bits(component, vbat_path_cfg, 0x80,
					      0x80);
     ...
     */
		...
		/* Set the ATTK1 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD1,
			0xFF, 0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD2,
			0xFF, 0x03);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD3,
			0xFF, 0x00);
		/* Set the ATTK2 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD4,
			0xFF, 0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD5,
			0xFF, 0x03);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD6,
			0xFF, 0x00);
		/* Set the ATTK3 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_VBAT_BCL_VBAT_BCL_GAIN_UPD7,
			0xFF, 0xFF);
		snd_soc_component_update_bits(
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_CB_DECODE_CB_DECODE_CTL1,
			0x01, 0x01);
		/* Enable BCL path */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_CB_DECODE_CB_DECODE_CTL2,
			0x01, 0x01);
		/* Request for BCL data */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_CB_DECODE_CB_DECODE_CTL3,
			0x01, 0x01);
		break;
					    ...
					    int event)
{
	...
	if (wsa2_priv->ec_hq[ec_tx]) {
		snd_soc_component_update_bits(
			...
			0x1 << ec_tx, 0x1 << ec_tx);
		ec_hq_reg =
			LPASS_CDC_WSA2_EC_HQ0_EC_REF_HQ_PATH_CTL + 0x40 * ec_tx;
		snd_soc_component_update_bits(component, ec_hq_reg, 0x01, 0x01);
		ec_hq_reg = LPASS_CDC_WSA2_EC_HQ0_EC_REF_HQ_CFG0 + 0x40 * ec_tx;
		/* default set to 48k */
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	int ec_tx = ((struct soc_multi_mixer_control *)kcontrol->private_value)
			    ->shift;
	struct device *wsa2_dev = NULL;
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv = NULL;
...
static int
lpass_cdc_wsa2_macro_get_rx_mute_status(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_set_rx_mute_status(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	case 2:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_RX0_RX_PATH_MIX_CTL, 0x10,
			value << 4);
		break;
	case 3:
		snd_soc_component_update_bits(
...
static int
lpass_cdc_wsa2_macro_set_digital_volume(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (mc->reg == LPASS_CDC_WSA2_RX0_RX_VOL_CTL) {
		wsa2_priv->rx0_origin_gain = (u8)snd_soc_component_read(
			wsa2_priv->component, mc->reg);
		gain = (u8)(wsa2_priv->rx0_origin_gain -
			    wsa2_priv->thermal_cur_state);
	} else if (mc->reg == LPASS_CDC_WSA2_RX1_RX_VOL_CTL) {
		wsa2_priv->rx1_origin_gain = (u8)snd_soc_component_read(
			wsa2_priv->component, mc->reg);
		gain = (u8)(wsa2_priv->rx1_origin_gain -
			    wsa2_priv->thermal_cur_state);
	} else {
	...
	if (wsa2_priv->dapm_mclk_enable && wsa2_priv->thermal_cur_state > 0) {
		snd_soc_component_update_bits(wsa2_priv->component, mc->reg,
					      0xFF, gain);
		dev_dbg(wsa2_priv->dev,
...
static int
lpass_cdc_wsa2_macro_get_compander(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_set_compander(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_ear_spkrrecv_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_ear_spkrrecv_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_idle_detect_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_idle_detect_put(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_comp_mode_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_comp_mode_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
					   ...
					   struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(wsa2_dev, "%s: mux input: %d, mux output: %d, bit: %d\n",
	...
	case 1:
	case 2:
		set_bit(bit_input, &wsa2_priv->active_ch_mask[rx_port_value]);
...
static int lpass_cdc_wsa2_macro_vbat_bcl_gsm_mode_func_get(
...
static int lpass_cdc_wsa2_macro_vbat_bcl_gsm_mode_func_put(
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_soft_clip_enable_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	struct device *wsa2_dev = NULL;
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv = NULL;
	int path = ((struct soc_multi_mixer_control *)kcontrol->private_value)
			   ->shift;
...
static int
lpass_cdc_wsa2_macro_soft_clip_enable_put(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_pbr_enable_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_pbr_enable_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_2ch_dma_enable_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa2_macro_2ch_dma_enable_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static const struct snd_kcontrol_new lpass_cdc_wsa2_macro_snd_controls[] = {
	SOC_ENUM_EXT("WSA2_GSM mode Enable",
		     lpass_cdc_wsa2_macro_vbat_bcl_gsm_mode_enum,
	...
	SOC_SINGLE_EXT("WSA2 Idle Detect", SND_SOC_NOPM, 0, 1, 0,
	...
	LPASS_CDC_WSA2_MACRO_SET_VOLUME_TLV("WSA2_RX0 Digital Volume",
					    LPASS_CDC_WSA2_RX0_RX_VOL_CTL, -84,
					    40, digital_gain),
	LPASS_CDC_WSA2_MACRO_SET_VOLUME_TLV("WSA2_RX1 Digital Volume",
					    LPASS_CDC_WSA2_RX1_RX_VOL_CTL, -84,
					    40, digital_gain),
	SOC_SINGLE_EXT("WSA2_RX0 Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_RX0, 1, 0,
		       lpass_cdc_wsa2_macro_get_rx_mute_status,
		       lpass_cdc_wsa2_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA2_RX1 Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_RX1, 1, 0,
		       lpass_cdc_wsa2_macro_get_rx_mute_status,
		       lpass_cdc_wsa2_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA2_RX0_MIX Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_RX_MIX0, 1, 0,
		       lpass_cdc_wsa2_macro_get_rx_mute_status,
		       lpass_cdc_wsa2_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA2_RX1_MIX Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_RX_MIX1, 1, 0,
		       lpass_cdc_wsa2_macro_get_rx_mute_status,
		       lpass_cdc_wsa2_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA2_COMP1 Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_COMP1, 1, 0,
		       lpass_cdc_wsa2_macro_get_compander,
		       lpass_cdc_wsa2_macro_set_compander),
	SOC_SINGLE_EXT("WSA2_COMP2 Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_COMP2, 1, 0,
		       lpass_cdc_wsa2_macro_get_compander,
		       lpass_cdc_wsa2_macro_set_compander),
	SOC_SINGLE_EXT("WSA2_RX0 EC_HQ Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_RX0, 1, 0,
		       lpass_cdc_wsa2_macro_get_ec_hq,
		       lpass_cdc_wsa2_macro_set_ec_hq),
	SOC_SINGLE_EXT("WSA2_RX1 EC_HQ Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_RX1, 1, 0,
		       lpass_cdc_wsa2_macro_get_ec_hq,
		       lpass_cdc_wsa2_macro_set_ec_hq),
	SOC_SINGLE_EXT("WSA2 PBR Enable", SND_SOC_NOPM, 0, 1, 0,
		       lpass_cdc_wsa2_macro_pbr_enable_get,
		       lpass_cdc_wsa2_macro_pbr_enable_put),
	SOC_SINGLE_EXT("WSA2 2CH_DMA ENABLE", SND_SOC_NOPM, 0, 1, 0,
...
static int
lpass_cdc_wsa2_macro_vi_feed_mixer_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static int
lpass_cdc_wsa2_macro_vi_feed_mixer_put(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static const struct snd_kcontrol_new aif_vi_mixer[] = {
	SOC_SINGLE_EXT("WSA2_SPKR_VI_1", SND_SOC_NOPM, LPASS_CDC_WSA2_MACRO_TX0,
		       1, 0, lpass_cdc_wsa2_macro_vi_feed_mixer_get,
		       lpass_cdc_wsa2_macro_vi_feed_mixer_put),
	SOC_SINGLE_EXT("WSA2_SPKR_VI_2", SND_SOC_NOPM, LPASS_CDC_WSA2_MACRO_TX1,
		       1, 0, lpass_cdc_wsa2_macro_vi_feed_mixer_get,
		       lpass_cdc_wsa2_macro_vi_feed_mixer_put),
};

static int
lpass_cdc_wsa2_macro_cps_feed_mixer_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static int
lpass_cdc_wsa2_macro_cps_feed_mixer_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static const struct snd_kcontrol_new aif_cps_mixer[] = {
	SOC_SINGLE_EXT("WSA2_SPKR_CPS_1", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_TX0, 1, 0,
		       lpass_cdc_wsa2_macro_cps_feed_mixer_get,
		       lpass_cdc_wsa2_macro_cps_feed_mixer_put),
	SOC_SINGLE_EXT("WSA2_SPKR_CPS_2", SND_SOC_NOPM,
		       LPASS_CDC_WSA2_MACRO_TX1, 1, 0,
		       lpass_cdc_wsa2_macro_cps_feed_mixer_get,
		       lpass_cdc_wsa2_macro_cps_feed_mixer_put),
};
...
static const struct snd_soc_dapm_widget lpass_cdc_wsa2_macro_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("WSA2_AIF_VI Mixer", SND_SOC_NOPM,
			   LPASS_CDC_WSA2_MACRO_AIF_VI, 0, aif_vi_mixer,
			   ARRAY_SIZE(aif_vi_mixer)),
	SND_SOC_DAPM_MIXER("WSA2_AIF_CPS Mixer", SND_SOC_NOPM,
			   LPASS_CDC_WSA2_MACRO_AIF_CPS, 0, aif_cps_mixer,
			   ARRAY_SIZE(aif_cps_mixer)),
	SND_SOC_DAPM_MUX_E("WSA2 RX_MIX EC0_MUX", SND_SOC_NOPM,
			   LPASS_CDC_WSA2_MACRO_EC0_MUX, 0, &rx_mix_ec0_mux,
			   lpass_cdc_wsa2_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("WSA2 RX_MIX EC1_MUX", SND_SOC_NOPM,
			   LPASS_CDC_WSA2_MACRO_EC1_MUX, 0, &rx_mix_ec1_mux,
			   lpass_cdc_wsa2_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX("WSA2 RX0 MUX", SND_SOC_NOPM, LPASS_CDC_WSA2_MACRO_RX0,
			 0, &rx_mux[LPASS_CDC_WSA2_MACRO_RX0]),
	SND_SOC_DAPM_MUX("WSA2 RX1 MUX", SND_SOC_NOPM, LPASS_CDC_WSA2_MACRO_RX1,
			 0, &rx_mux[LPASS_CDC_WSA2_MACRO_RX1]),
	SND_SOC_DAPM_MUX("WSA2 RX_MIX0 MUX", SND_SOC_NOPM,
			 LPASS_CDC_WSA2_MACRO_RX_MIX0, 0,
			 &rx_mux[LPASS_CDC_WSA2_MACRO_RX_MIX0]),
	SND_SOC_DAPM_MUX("WSA2 RX_MIX1 MUX", SND_SOC_NOPM,
			 LPASS_CDC_WSA2_MACRO_RX_MIX1, 0,
			 &rx_mux[LPASS_CDC_WSA2_MACRO_RX_MIX1]),
	SND_SOC_DAPM_MUX("WSA2 RX4 MUX", SND_SOC_NOPM, LPASS_CDC_WSA2_MACRO_RX4,
			 0, &rx_mux[LPASS_CDC_WSA2_MACRO_RX4]),
	SND_SOC_DAPM_MUX("WSA2 RX5 MUX", SND_SOC_NOPM, LPASS_CDC_WSA2_MACRO_RX5,
			 0, &rx_mux[LPASS_CDC_WSA2_MACRO_RX5]),
	...
	SND_SOC_DAPM_MUX_E("WSA2_RX0 INP2", SND_SOC_NOPM, 0, 0,
			   ...
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("WSA2_RX0 MIX INP", SND_SOC_NOPM, 0, 0, &rx0_mix_mux,
	...
	SND_SOC_DAPM_MUX_E("WSA2_RX1 INP2", SND_SOC_NOPM, 0, 0,
			   ...
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("WSA2_RX1 MIX INP", SND_SOC_NOPM, 0, 0, &rx1_mix_mux,
			   lpass_cdc_wsa2_macro_enable_mix_path,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("WSA2_RX INT0 MIX", SND_SOC_NOPM, 0, 0, NULL, 0,
			   lpass_cdc_wsa2_macro_enable_main_path,
			   SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_PGA_E("WSA2_RX INT1 MIX", SND_SOC_NOPM, 1, 0, NULL, 0,
			   lpass_cdc_wsa2_macro_enable_main_path,
			   SND_SOC_DAPM_PRE_PMU),
	...
	SND_SOC_DAPM_MUX_E(
		"WSA2_RX0 INT0 SIDETONE MIX", LPASS_CDC_WSA2_RX0_RX_PATH_CFG1,
		4, 0, &rx0_sidetone_mix_mux, lpass_cdc_wsa2_macro_enable_swr,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_MIXER_E("WSA2_RX INT0 INTERP", SND_SOC_NOPM,
			     LPASS_CDC_WSA2_MACRO_COMP1, 0, NULL, 0,
			     lpass_cdc_wsa2_macro_enable_interpolator,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				     SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("WSA2_RX INT1 INTERP", SND_SOC_NOPM,
			     LPASS_CDC_WSA2_MACRO_COMP2, 0, NULL, 0,
			     lpass_cdc_wsa2_macro_enable_interpolator,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				     SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MIXER_E("WSA2_RX INT0 CHAIN", SND_SOC_NOPM, 0, 0, NULL, 0,
			     lpass_cdc_wsa2_macro_spk_boost_event,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				     SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("WSA2_RX INT1 CHAIN", SND_SOC_NOPM, 0, 0, NULL, 0,
			     lpass_cdc_wsa2_macro_spk_boost_event,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				     SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MIXER_E("WSA2_RX INT0 VBAT", SND_SOC_NOPM, 0, 0,
			     wsa2_int0_vbat_mix_switch,
			     ARRAY_SIZE(wsa2_int0_vbat_mix_switch),
			     lpass_cdc_wsa2_macro_enable_vbat,
			     SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MIXER_E("WSA2_RX INT1 VBAT", SND_SOC_NOPM, 0, 0,
			     wsa2_int1_vbat_mix_switch,
...
static void lpass_cdc_wsa2_macro_init_pbr(struct snd_soc_component *component)
{
	...
	case WSA_6_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0, 0xE0, 0x80);
		break;
	case WSA_8_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0, 0xE0, 0xC0);
		break;
	case WSA_32_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0, 0xE0, 0xE0);
		break;
	default:
		break;
	}
	snd_soc_component_update_bits(component, LPASS_CDC_WSA2_ILIM_CFG1, 0x0F,
				      sys_gain);
	snd_soc_component_update_bits(component, LPASS_CDC_WSA2_ILIM_CFG9, 0xC0,
				      (bat_cfg - 1) << 0x6);
	/* Thesh */
	vth1 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth1_data[sys_gain][bat_cfg][rload]);
	vth2 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth2_data[sys_gain][bat_cfg][rload]);
	vth3 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth3_data[sys_gain][bat_cfg][rload]);
	vth4 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth4_data[sys_gain][bat_cfg][rload]);
	vth5 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth5_data[sys_gain][bat_cfg][rload]);
	vth6 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth6_data[sys_gain][bat_cfg][rload]);
	vth7 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth7_data[sys_gain][bat_cfg][rload]);
	vth8 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth8_data[sys_gain][bat_cfg][rload]);
	vth9 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth9_data[sys_gain][bat_cfg][rload]);
	vth10 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth10_data[sys_gain][bat_cfg][rload]);
	vth11 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth11_data[sys_gain][bat_cfg][rload]);
	vth12 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth12_data[sys_gain][bat_cfg][rload]);
	vth13 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth13_data[sys_gain][bat_cfg][rload]);
	vth14 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth14_data[sys_gain][bat_cfg][rload]);
	vth15 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth15_data[sys_gain][bat_cfg][rload]);
	...
	case WSA_4_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0_1, 0xE0, 0x40);
		break;
	case WSA_6_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0_1, 0xE0, 0x80);
		break;
	case WSA_8_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0_1, 0xE0, 0xC0);
		break;
	case WSA_32_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA2_ILIM_CFG0_1, 0xE0, 0xE0);
		break;
	default:
		break;
	}
	snd_soc_component_update_bits(component, LPASS_CDC_WSA2_ILIM_CFG1_1,
				      0x0F, sys_gain);
	snd_soc_component_update_bits(component, LPASS_CDC_WSA2_ILIM_CFG9, 0x30,
				      (bat_cfg - 1) << 0x4);
	/* Thesh */
	vth1 = LPASS_CDC_WSA2_MACRO_VTH_TO_REG(
		pbr_vth1_data[sys_gain][bat_cfg][rload]);
...
static void lpass_cdc_wsa2_macro_init_reg(struct snd_soc_component *component)
{
	...
	for (i = 0; i < ARRAY_SIZE(lpass_cdc_wsa2_macro_reg_init); i++)
		snd_soc_component_update_bits(
			component, lpass_cdc_wsa2_macro_reg_init[i].reg,
...
static int lpass_cdc_wsa2_macro_core_vote(void *handle, bool enable)
{
	int rc = 0;
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv =
		(struct lpass_cdc_wsa2_macro_priv *)handle;
...
static int wsa2_swrm_clock(void *handle, bool enable)
{
	...
	if (enable) {
		...
		if (wsa2_priv->swr_clk_users == 0) {
			...
			ret = lpass_cdc_wsa2_macro_mclk_enable(wsa2_priv, 1,
							       true);
			if (ret < 0) {
			...
			if (wsa2_priv->reset_swr)
				regmap_update_bits(
					...
					0x02, 0x02);
			regmap_update_bits(
				regmap, LPASS_CDC_WSA2_CLK_RST_CTRL_SWR_CONTROL,
				0x01, 0x01);
			if (wsa2_priv->reset_swr)
				regmap_update_bits(
					regmap,
...
/* Thermal Functions */
static int
lpass_cdc_wsa2_macro_get_max_state(struct thermal_cooling_device *cdev,
				   unsigned long *state)
{
...
static int
lpass_cdc_wsa2_macro_get_cur_state(struct thermal_cooling_device *cdev,
				   unsigned long *state)
{
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv = cdev->devdata;
...
static int
lpass_cdc_wsa2_macro_set_cur_state(struct thermal_cooling_device *cdev,
				   unsigned long state)
{
	struct lpass_cdc_wsa2_macro_priv *wsa2_priv = cdev->devdata;
	...
	dev_dbg(wsa2_priv->dev, "%s: set the thermal current state to %d\n",
		__func__, wsa2_priv->thermal_cur_state);
...
static int lpass_cdc_wsa2_macro_init(struct snd_soc_component *component)
{
	...
	ret = snd_soc_dapm_new_controls(
		dapm, lpass_cdc_wsa2_macro_dapm_widgets,
	...
	ret = snd_soc_add_component_controls(
		component, lpass_cdc_wsa2_macro_snd_controls,
	...
	for_each_available_child_of_node(wsa2_priv->dev->of_node, node) {
		...
		if (strnstr(node->name, "wsa2_swr_master",
			...
			temp = krealloc(
				swr_ctrl_data,
				(ctrl_num +
				 1) * sizeof(struct lpass_cdc_wsa2_macro_swr_ctrl_data),
				GFP_KERNEL);
			if (!temp) {
		...
		if (wsa2_priv->child_count <
		    LPASS_CDC_WSA2_MACRO_CHILD_DEVICES_MAX)
			wsa2_priv->pdev_child_devices[wsa2_priv->child_count++] =
				pdev;
		else
			goto err;
	}
...
static int lpass_cdc_wsa2_macro_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "reg", &wsa2_base_addr);
	if (ret) {
	...
	if (!wsa2_priv->wsa2_fs_reg_base && wsa2_priv->wsa2_fs_ctl_reg)
		wsa2_priv->wsa2_fs_reg_base =
			devm_ioremap(&pdev->dev, wsa2_priv->wsa2_fs_ctl_reg,
	...
	wsa2_priv->wsa2_swr_gpio_p =
		of_parse_phandle(pdev->dev.of_node, "qcom,wsa2-swr-gpios", 0);
	...
	msm_cdc_pinctrl_set_wakeup_capable(wsa2_priv->wsa2_swr_gpio_p, false);

	wsa2_io_base = devm_ioremap(&pdev->dev, wsa2_base_addr,
				    LPASS_CDC_WSA2_MACRO_MAX_OFFSET);
	if (!wsa2_io_base) {
	...
	if (of_find_property(wsa2_priv->dev->of_node, "#cooling-cells", NULL)) {
		...
		wsa2_priv->tcdev = devm_thermal_of_cooling_device_register(
			&pdev->dev, wsa2_priv->dev->of_node, "wsa2", wsa2_priv,
			&wsa2_cooling_ops);
	...
	ret = of_property_read_u32(pdev->dev.of_node, "qcom,noise-gate-mode",
				   &noise_gate_mode);
	if (ret) {
...
static int lpass_cdc_wsa2_macro_remove(struct platform_device *pdev)
{
	...
	for (count = 0; count < wsa2_priv->child_count &&
			count < LPASS_CDC_WSA2_MACRO_CHILD_DEVICES_MAX;
	     count++)
		platform_device_unregister(
			wsa2_priv->pdev_child_devices[count]);
