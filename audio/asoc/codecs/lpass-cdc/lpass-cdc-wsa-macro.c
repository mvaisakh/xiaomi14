...
#define LPASS_CDC_WSA_MACRO_RX_RATES                                         \
	(SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_32000 | \
	 SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_96000 | SNDRV_PCM_RATE_192000)
#define LPASS_CDC_WSA_MACRO_RX_MIX_RATES \
	(SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_96000 | SNDRV_PCM_RATE_192000)
#define LPASS_CDC_WSA_MACRO_RX_FORMATS                       \
	(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | \
	 SNDRV_PCM_FMTBIT_S24_3LE | SNDRV_PCM_FMTBIT_S32_LE)

#define LPASS_CDC_WSA_MACRO_ECHO_RATES \
	(SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_48000)
#define LPASS_CDC_WSA_MACRO_ECHO_FORMATS                     \
	(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | \
...
static int lpass_cdc_wsa_macro_mute_stream(struct snd_soc_dai *dai, int mute,
					   int stream);
...
static int
lpass_cdc_wsa_macro_enable_vi_decimator(struct snd_soc_component *component);
...
struct lpass_cdc_wsa_macro_priv {
	...
	struct platform_device
		*pdev_child_devices[LPASS_CDC_WSA_MACRO_CHILD_DEVICES_MAX];
	int child_count;
	int wsa_spkrrecv;
...
static const struct soc_enum rx0_prim_inp0_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA_RX_INP_MUX_RX_INT0_CFG0, 0, 12, rx_text);

static const struct soc_enum rx0_prim_inp1_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA_RX_INP_MUX_RX_INT0_CFG0, 3, 12, rx_text);

static const struct soc_enum rx0_prim_inp2_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA_RX_INP_MUX_RX_INT0_CFG1, 3, 12, rx_text);

static const struct soc_enum rx0_mix_chain_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA_RX_INP_MUX_RX_INT0_CFG1, 0, 10, rx_mix_text);
...
static const struct soc_enum rx1_prim_inp0_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA_RX_INP_MUX_RX_INT1_CFG0, 0, 12, rx_text);

static const struct soc_enum rx1_prim_inp1_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA_RX_INP_MUX_RX_INT1_CFG0, 3, 12, rx_text);

static const struct soc_enum rx1_prim_inp2_chain_enum =
	SOC_ENUM_SINGLE(LPASS_CDC_WSA_RX_INP_MUX_RX_INT1_CFG1, 3, 12, rx_text);

static const struct soc_enum rx1_mix_chain_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA_RX_INP_MUX_RX_INT1_CFG1, 0, 10, rx_mix_text);
...
static const struct soc_enum rx_mix_ec0_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA_RX_INP_MUX_RX_MIX_CFG0, 0, 3, rx_mix_ec_text);

static const struct soc_enum rx_mix_ec1_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_WSA_RX_INP_MUX_RX_MIX_CFG0, 3, 3, rx_mix_ec_text);
...
static bool lpass_cdc_wsa_macro_get_data(
	struct snd_soc_component *component, struct device **wsa_dev,
	struct lpass_cdc_wsa_macro_priv **wsa_priv, const char *func_name)
{
					    ...
					    u32 usecase, u32 size, void *data)
{
	...
	if (!lpass_cdc_wsa_macro_get_data(component, &wsa_dev, &wsa_priv,
					  __func__))
		return -EINVAL;
	...
	if (wsa_priv->swr_ctrl_data)
		ret = swrm_wcd_notify(wsa_priv->swr_ctrl_data[0].wsa_swr_pdev,
				      SWR_SET_PORT_MAP, &port_cfg);

	return ret;
}

static int lpass_cdc_wsa_macro_set_prim_interpolator_rate(
	struct snd_soc_dai *dai, u8 int_prim_fs_rate_reg_val, u32 sample_rate)
{
	...
	for_each_set_bit(port, &wsa_priv->active_ch_mask[dai->id],
		...
		if ((int_1_mix1_inp < LPASS_CDC_WSA_MACRO_RX0) ||
			...
			dev_err_ratelimited(
				wsa_dev, "%s: Invalid RX port, Dai ID is %d\n",
				__func__, dai->id);
			return -EINVAL;
		}
     ...
     */
		for (j = 0; j < NUM_INTERPOLATORS; j++) {
			int_mux_cfg1 = int_mux_cfg0 +
				       LPASS_CDC_WSA_MACRO_MUX_CFG1_OFFSET;
...
static int lpass_cdc_wsa_macro_set_mix_interpolator_rate(
						     ...
						     u32 sample_rate)
{
	...
	if ((i == ARRAY_SIZE(int_mix_sample_rate_val)) || (rate_val < 0))
		goto prim_rate;
	ret = lpass_cdc_wsa_macro_set_mix_interpolator_rate(dai, (u8)rate_val,
							    sample_rate);
prim_rate:
	...
	if ((i == ARRAY_SIZE(int_prim_sample_rate_val)) || (rate_val < 0))
		return -EINVAL;
	ret = lpass_cdc_wsa_macro_set_prim_interpolator_rate(dai, (u8)rate_val,
							     sample_rate);
	return ret;
}
					 ...
					 struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_STREAM_PLAYBACK:
		ret = lpass_cdc_wsa_macro_set_interpolator_rate(
...
static int
lpass_cdc_wsa_macro_mclk_enable(struct lpass_cdc_wsa_macro_priv *wsa_priv,
	...
	if (mclk_enable) {
		if (wsa_priv->wsa_mclk_users == 0) {
			ret = lpass_cdc_clk_rsc_request_clock(
				wsa_priv->dev, wsa_priv->default_clk_id,
				wsa_priv->default_clk_id, true);
			if (ret < 0) {
				dev_err_ratelimited(
					wsa_priv->dev,
			...
			lpass_cdc_clk_rsc_fs_gen_request(wsa_priv->dev, true);
			regcache_mark_dirty(regmap);
			regcache_sync_region(regmap, WSA_START_OFFSET,
					     WSA_MAX_OFFSET);
			/* 9.6MHz MCLK, set value 0x00 if other frequency */
			regmap_update_bits(regmap, LPASS_CDC_WSA_TOP_FREQ_MCLK,
					   0x01, 0x01);
			regmap_update_bits(
				regmap, LPASS_CDC_WSA_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x01);
			...
			regmap_update_bits(
				regmap,
				LPASS_CDC_WSA_CLK_RST_CTRL_FS_CNT_CONTROL, 0x01,
				0x01);
		}
		...
		if (wsa_priv->wsa_mclk_users == 0) {
			...
			regmap_update_bits(
				regmap, LPASS_CDC_WSA_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x00);
			lpass_cdc_clk_rsc_fs_gen_request(wsa_priv->dev, false);

			lpass_cdc_clk_rsc_request_clock(
				wsa_priv->dev, wsa_priv->default_clk_id,
				wsa_priv->default_clk_id, false);
		}
	}
exit:
...
static int
lpass_cdc_wsa_macro_event_handler(struct snd_soc_component *component,
				  u16 event, u32 data)
{
	struct device *wsa_dev = NULL;
	struct lpass_cdc_wsa_macro_priv *wsa_priv = NULL;
	...
	case LPASS_CDC_MACRO_EVT_SSR_DOWN:
		...
		if (wsa_priv->swr_ctrl_data) {
			swrm_wcd_notify(wsa_priv->swr_ctrl_data[0].wsa_swr_pdev,
...
lpass_cdc_wsa_macro_enable_vi_decimator(struct snd_soc_component *component)
{
	...
	if (test_bit(LPASS_CDC_WSA_MACRO_TX0,
		...
		/* Enable V&I sensing */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX0_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX1_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX0_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX1_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX0_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX1_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX0_SPKR_PROT_PATH_CTL, 0x20,
			0x00);
	...
	if (test_bit(LPASS_CDC_WSA_MACRO_TX1,
		...
		/* Enable V&I sensing */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX2_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX3_SPKR_PROT_PATH_CTL, 0x20,
			0x20);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX2_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX3_SPKR_PROT_PATH_CTL, 0x0F,
			val);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX2_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX3_SPKR_PROT_PATH_CTL, 0x10,
			0x10);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_TX2_SPKR_PROT_PATH_CTL, 0x20,
			0x00);
...
static int lpass_cdc_wsa_macro_disable_vi_feedback(
	struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		if (test_bit(LPASS_CDC_WSA_MACRO_TX0,
			     &wsa_priv->active_ch_mask
				      [LPASS_CDC_WSA_MACRO_AIF_VI])) {
			/* Disable V&I sensing */
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA_TX0_SPKR_PROT_PATH_CTL,
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA_TX1_SPKR_PROT_PATH_CTL,
				0x20, 0x20);
		...
		if (test_bit(LPASS_CDC_WSA_MACRO_TX1,
			...
			snd_soc_component_update_bits(
				...
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA_TX3_SPKR_PROT_PATH_CTL,
				0x20, 0x20);
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA_TX2_SPKR_PROT_PATH_CTL,
				0x10, 0x00);
					  ...
					  int event)
{
	...
	dev_dbg(wsa_priv->dev, "%s: current swr ch cnt: %d\n", __func__,
		wsa_priv->rx_0_count + wsa_priv->rx_1_count);
...
static int
lpass_cdc_wsa_macro_config_compander(struct snd_soc_component *component,
				     int comp, int event)
{
...
static void lpass_cdc_wsa_macro_enable_softclip_clk(
	struct snd_soc_component *component,
	struct lpass_cdc_wsa_macro_priv *wsa_priv, int path, bool enable)
{
...
static int
lpass_cdc_wsa_macro_config_softclip(struct snd_soc_component *component,
				    int path, int event)
{
	...
	softclip_ctrl_reg =
		LPASS_CDC_WSA_SOFTCLIP0_SOFTCLIP_CTRL +
					  ...
					  int path, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		if (wsa_priv->pbr_clk_users == 0)
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA_PBR_PATH_CTL, 0x01,
				0x01);
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (wsa_priv->prim_int_users[ind] == 1) {
			snd_soc_component_update_bits(
				component,
				prim_int_reg +
					LPASS_CDC_WSA_MACRO_RX_PATH_CFG3_OFFSET,
				0x03, 0x03);
			snd_soc_component_update_bits(component, prim_int_reg,
						      0x10, 0x10);
			lpass_cdc_wsa_macro_hd2_control(component, prim_int_reg,
							event);
			snd_soc_component_update_bits(
				component,
				prim_int_reg +
...
static int lpass_cdc_wsa_macro_enable_interpolator(
	struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component =
		snd_soc_dapm_to_component(w->dapm);
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* Reset if needed */
		lpass_cdc_wsa_macro_enable_prim_interpolator(component, reg,
							     event);
		break;
	case SND_SOC_DAPM_POST_PMU:
		if (!strcmp(w->name, "WSA_RX INT0 INTERP")) {
			...
			if (snd_soc_component_read(
				...
				snd_soc_component_update_bits(
					wsa_priv->component,
					LPASS_CDC_WSA_RX0_RX_VOL_CTL, 0xFF,
					gain);
				dev_dbg(wsa_priv->dev,
		...
		if (!strcmp(w->name, "WSA_RX INT1 INTERP")) {
			...
			if (snd_soc_component_read(
				...
				snd_soc_component_update_bits(
					wsa_priv->component,
					LPASS_CDC_WSA_RX1_RX_VOL_CTL, 0xFF,
					gain);
				dev_dbg(wsa_priv->dev,
		...
		lpass_cdc_wsa_macro_config_compander(component, w->shift,
						     event);
		...
		if (wsa_priv->wsa_spkrrecv)
			snd_soc_component_update_bits(
				component, LPASS_CDC_WSA_RX0_RX_PATH_CFG1, 0x08,
				0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_RX0_RX_PATH_CFG1, 0x08, 0x08);
		lpass_cdc_wsa_macro_config_compander(component, w->shift,
						     event);
		lpass_cdc_macro_idle_detect_control(component, wsa_priv,
						    w->shift, event);
					   ...
					   int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_PATH_CTL, 0x10,
			0x10);
		/* Enable VBAT block */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_CFG, 0x01, 0x01);
		/* Update interpolator with 384K path */
		snd_soc_component_update_bits(component, vbat_path_cfg, 0x80,
					      0x80);
		/* Use attenuation mode */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_CFG, 0x02, 0x00);
		/*
     ...
     */
		...
		/* Set the ATTK1 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_BCL_GAIN_UPD1,
			0xFF, 0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_BCL_GAIN_UPD2,
			0xFF, 0x03);
		snd_soc_component_update_bits(
		...
		snd_soc_component_update_bits(
			...
			0xFF, 0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_BCL_GAIN_UPD5,
			0xFF, 0x03);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_BCL_GAIN_UPD6,
			0xFF, 0x00);
		/* Set the ATTK3 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_BCL_GAIN_UPD7,
			0xFF, 0xFF);
		snd_soc_component_update_bits(
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_CB_DECODE_CB_DECODE_CTL1, 0x01,
			0x01);
		/* Enable BCL path */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_CB_DECODE_CB_DECODE_CTL2, 0x01,
			0x01);
		/* Request for BCL data */
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_CB_DECODE_CB_DECODE_CTL3, 0x01,
			0x01);
		break;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_component_update_bits(component, vbat_path_cfg, 0x80,
					      0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_CFG, 0x02, 0x02);
		snd_soc_component_update_bits(component, vbat_path_cfg, 0x02,
					      0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_BCL_GAIN_UPD1,
		...
		lpass_cdc_wsa_macro_enable_softclip_clk(component, wsa_priv,
							softclip_path, false);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_CFG, 0x01, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_PATH_CTL, 0x10,
			0x00);
		break;
					   ...
					   int event)
{
	...
	if (wsa_priv->ec_hq[ec_tx]) {
		snd_soc_component_update_bits(
			...
			0x1 << ec_tx, 0x1 << ec_tx);
		ec_hq_reg =
			LPASS_CDC_WSA_EC_HQ0_EC_REF_HQ_PATH_CTL + 0x40 * ec_tx;
		snd_soc_component_update_bits(component, ec_hq_reg, 0x01, 0x01);
		ec_hq_reg = LPASS_CDC_WSA_EC_HQ0_EC_REF_HQ_CFG0 + 0x40 * ec_tx;
		/* default set to 48k */
...
static int
lpass_cdc_wsa_macro_get_rx_mute_status(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_set_rx_mute_status(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_set_digital_volume(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (mc->reg == LPASS_CDC_WSA_RX0_RX_VOL_CTL) {
		wsa_priv->rx0_origin_gain = (u8)snd_soc_component_read(
			wsa_priv->component, mc->reg);
		gain = (u8)(wsa_priv->rx0_origin_gain -
			    wsa_priv->thermal_cur_state);
	} else if (mc->reg == LPASS_CDC_WSA_RX1_RX_VOL_CTL) {
		wsa_priv->rx1_origin_gain = (u8)snd_soc_component_read(
			wsa_priv->component, mc->reg);
		gain = (u8)(wsa_priv->rx1_origin_gain -
			    wsa_priv->thermal_cur_state);
	} else {
	...
	if (wsa_priv->dapm_mclk_enable && wsa_priv->thermal_cur_state > 0) {
		snd_soc_component_update_bits(wsa_priv->component, mc->reg,
					      0xFF, gain);
		dev_dbg(wsa_priv->dev,
...
static int
lpass_cdc_wsa_macro_get_compander(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_set_compander(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_ear_spkrrecv_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_ear_spkrrecv_put(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_idle_detect_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_idle_detect_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_comp_mode_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_comp_mode_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(wsa_dev, "%s: mux input: %d, mux output: %d, bit: %d\n",
	...
	case 1:
	case 2:
		set_bit(bit_input, &wsa_priv->active_ch_mask[rx_port_value]);
...
static int lpass_cdc_wsa_macro_vbat_bcl_gsm_mode_func_get(
...
static int lpass_cdc_wsa_macro_vbat_bcl_gsm_mode_func_put(
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (ucontrol->value.integer.value[0])
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_CFG, 0x04, 0x04);
	else
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_VBAT_BCL_VBAT_CFG, 0x04, 0x00);

	return 0;
}

static int
lpass_cdc_wsa_macro_soft_clip_enable_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_soft_clip_enable_put(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_pbr_enable_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_wsa_macro_pbr_enable_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static const struct snd_kcontrol_new lpass_cdc_wsa_macro_snd_controls[] = {
	SOC_ENUM_EXT("GSM mode Enable",
		     lpass_cdc_wsa_macro_vbat_bcl_gsm_mode_enum,
	...
	SOC_SINGLE_EXT("Idle Detect", SND_SOC_NOPM, 0, 1, 0,
	...
	LPASS_CDC_WSA_MACRO_SET_VOLUME_TLV("WSA_RX0 Digital Volume",
					   LPASS_CDC_WSA_RX0_RX_VOL_CTL, -84,
					   40, digital_gain),
	LPASS_CDC_WSA_MACRO_SET_VOLUME_TLV("WSA_RX1 Digital Volume",
					   LPASS_CDC_WSA_RX1_RX_VOL_CTL, -84,
					   40, digital_gain),
	SOC_SINGLE_EXT("WSA_RX0 Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_RX0, 1, 0,
		       lpass_cdc_wsa_macro_get_rx_mute_status,
		       lpass_cdc_wsa_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA_RX1 Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_RX1, 1, 0,
		       lpass_cdc_wsa_macro_get_rx_mute_status,
		       lpass_cdc_wsa_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA_RX0_MIX Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_RX_MIX0, 1, 0,
		       lpass_cdc_wsa_macro_get_rx_mute_status,
		       lpass_cdc_wsa_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA_RX1_MIX Digital Mute", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_RX_MIX1, 1, 0,
		       lpass_cdc_wsa_macro_get_rx_mute_status,
		       lpass_cdc_wsa_macro_set_rx_mute_status),
	SOC_SINGLE_EXT("WSA_COMP1 Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_COMP1, 1, 0,
		       lpass_cdc_wsa_macro_get_compander,
		       lpass_cdc_wsa_macro_set_compander),
	SOC_SINGLE_EXT("WSA_COMP2 Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_COMP2, 1, 0,
		       lpass_cdc_wsa_macro_get_compander,
		       lpass_cdc_wsa_macro_set_compander),
	SOC_SINGLE_EXT("WSA_RX0 EC_HQ Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_RX0, 1, 0,
		       lpass_cdc_wsa_macro_get_ec_hq,
		       lpass_cdc_wsa_macro_set_ec_hq),
	SOC_SINGLE_EXT("WSA_RX1 EC_HQ Switch", SND_SOC_NOPM,
		       LPASS_CDC_WSA_MACRO_RX1, 1, 0,
		       lpass_cdc_wsa_macro_get_ec_hq,
		       lpass_cdc_wsa_macro_set_ec_hq),
	SOC_SINGLE_EXT("WSA PBR Enable", SND_SOC_NOPM, 0, 1, 0,
...
static int
lpass_cdc_wsa_macro_vi_feed_mixer_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static int
lpass_cdc_wsa_macro_vi_feed_mixer_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static const struct snd_kcontrol_new aif_vi_mixer[] = {
	SOC_SINGLE_EXT("WSA_SPKR_VI_1", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_TX0,
		       1, 0, lpass_cdc_wsa_macro_vi_feed_mixer_get,
		       lpass_cdc_wsa_macro_vi_feed_mixer_put),
	SOC_SINGLE_EXT("WSA_SPKR_VI_2", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_TX1,
		       1, 0, lpass_cdc_wsa_macro_vi_feed_mixer_get,
		       lpass_cdc_wsa_macro_vi_feed_mixer_put),
};

static int
lpass_cdc_wsa_macro_cps_feed_mixer_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static int
lpass_cdc_wsa_macro_cps_feed_mixer_put(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget =
		snd_soc_dapm_kcontrol_widget(kcontrol);
...
static const struct snd_kcontrol_new aif_cps_mixer[] = {
	SOC_SINGLE_EXT("WSA_SPKR_CPS_1", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_TX0,
		       1, 0, lpass_cdc_wsa_macro_cps_feed_mixer_get,
		       lpass_cdc_wsa_macro_cps_feed_mixer_put),
	SOC_SINGLE_EXT("WSA_SPKR_CPS_2", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_TX1,
		       1, 0, lpass_cdc_wsa_macro_cps_feed_mixer_get,
		       lpass_cdc_wsa_macro_cps_feed_mixer_put),
};

static const struct snd_soc_dapm_widget lpass_cdc_wsa_macro_dapm_widgets[] = {
	SND_SOC_DAPM_AIF_IN("WSA AIF1 PB", "WSA_AIF1 Playback", 0, SND_SOC_NOPM,
			    0, 0),
	...
	SND_SOC_DAPM_MIXER("WSA_AIF_VI Mixer", SND_SOC_NOPM,
			   LPASS_CDC_WSA_MACRO_AIF_VI, 0, aif_vi_mixer,
			   ARRAY_SIZE(aif_vi_mixer)),
	SND_SOC_DAPM_MIXER("WSA_AIF_CPS Mixer", SND_SOC_NOPM,
			   LPASS_CDC_WSA_MACRO_AIF_CPS, 0, aif_cps_mixer,
			   ARRAY_SIZE(aif_cps_mixer)),
	SND_SOC_DAPM_MUX_E("WSA RX_MIX EC0_MUX", SND_SOC_NOPM,
			   LPASS_CDC_WSA_MACRO_EC0_MUX, 0, &rx_mix_ec0_mux,
			   lpass_cdc_wsa_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("WSA RX_MIX EC1_MUX", SND_SOC_NOPM,
			   LPASS_CDC_WSA_MACRO_EC1_MUX, 0, &rx_mix_ec1_mux,
			   lpass_cdc_wsa_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX("WSA RX0 MUX", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_RX0,
			 0, &rx_mux[LPASS_CDC_WSA_MACRO_RX0]),
	SND_SOC_DAPM_MUX("WSA RX1 MUX", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_RX1,
			 0, &rx_mux[LPASS_CDC_WSA_MACRO_RX1]),
	SND_SOC_DAPM_MUX("WSA RX_MIX0 MUX", SND_SOC_NOPM,
			 LPASS_CDC_WSA_MACRO_RX_MIX0, 0,
			 &rx_mux[LPASS_CDC_WSA_MACRO_RX_MIX0]),
	SND_SOC_DAPM_MUX("WSA RX_MIX1 MUX", SND_SOC_NOPM,
			 LPASS_CDC_WSA_MACRO_RX_MIX1, 0,
			 &rx_mux[LPASS_CDC_WSA_MACRO_RX_MIX1]),
	SND_SOC_DAPM_MUX("WSA RX4 MUX", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_RX4,
			 0, &rx_mux[LPASS_CDC_WSA_MACRO_RX4]),
	SND_SOC_DAPM_MUX("WSA RX5 MUX", SND_SOC_NOPM, LPASS_CDC_WSA_MACRO_RX5,
			 0, &rx_mux[LPASS_CDC_WSA_MACRO_RX5]),
	...
	SND_SOC_DAPM_MUX_E("WSA_RX0 INP2", SND_SOC_NOPM, 0, 0,
			   ...
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("WSA_RX0 MIX INP", SND_SOC_NOPM, 0, 0, &rx0_mix_mux,
	...
	SND_SOC_DAPM_MUX_E("WSA_RX1 INP2", SND_SOC_NOPM, 0, 0,
			   ...
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("WSA_RX1 MIX INP", SND_SOC_NOPM, 0, 0, &rx1_mix_mux,
			   lpass_cdc_wsa_macro_enable_mix_path,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("WSA_RX INT0 MIX", SND_SOC_NOPM, 0, 0, NULL, 0,
			   lpass_cdc_wsa_macro_enable_main_path,
			   SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_PGA_E("WSA_RX INT1 MIX", SND_SOC_NOPM, 1, 0, NULL, 0,
			   lpass_cdc_wsa_macro_enable_main_path,
			   SND_SOC_DAPM_PRE_PMU),
	...
	SND_SOC_DAPM_MUX_E(
		"WSA_RX0 INT0 SIDETONE MIX", LPASS_CDC_WSA_RX0_RX_PATH_CFG1, 4,
		0, &rx0_sidetone_mix_mux, lpass_cdc_wsa_macro_enable_swr,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static void lpass_cdc_wsa_macro_init_pbr(struct snd_soc_component *component)
{
	...
	case WSA_6_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_ILIM_CFG0, 0xE0, 0x80);
		break;
	case WSA_8_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_ILIM_CFG0, 0xE0, 0xC0);
		break;
	case WSA_32_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_ILIM_CFG0, 0xE0, 0xE0);
		break;
	default:
		break;
	}
	snd_soc_component_update_bits(component, LPASS_CDC_WSA_ILIM_CFG1, 0x0F,
				      sys_gain);
	snd_soc_component_update_bits(component, LPASS_CDC_WSA_ILIM_CFG9, 0xC0,
				      (bat_cfg - 1) << 0x6);
	/* Thesh */
	vth1 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth1_data[sys_gain][bat_cfg][rload]);
	vth2 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth2_data[sys_gain][bat_cfg][rload]);
	vth3 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth3_data[sys_gain][bat_cfg][rload]);
	vth4 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth4_data[sys_gain][bat_cfg][rload]);
	vth5 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth5_data[sys_gain][bat_cfg][rload]);
	vth6 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth6_data[sys_gain][bat_cfg][rload]);
	vth7 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth7_data[sys_gain][bat_cfg][rload]);
	vth8 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth8_data[sys_gain][bat_cfg][rload]);
	vth9 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth9_data[sys_gain][bat_cfg][rload]);
	vth10 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth10_data[sys_gain][bat_cfg][rload]);
	vth11 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth11_data[sys_gain][bat_cfg][rload]);
	vth12 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth12_data[sys_gain][bat_cfg][rload]);
	vth13 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth13_data[sys_gain][bat_cfg][rload]);
	vth14 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth14_data[sys_gain][bat_cfg][rload]);
	vth15 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
	...
	case WSA_6_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_ILIM_CFG0_1, 0xE0, 0x80);
		break;
	case WSA_8_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_ILIM_CFG0_1, 0xE0, 0xC0);
		break;
	case WSA_32_OHMS:
		snd_soc_component_update_bits(
			component, LPASS_CDC_WSA_ILIM_CFG0_1, 0xE0, 0xE0);
		break;
	default:
		break;
	}
	snd_soc_component_update_bits(component, LPASS_CDC_WSA_ILIM_CFG1_1,
				      0x0F, sys_gain);
	snd_soc_component_update_bits(component, LPASS_CDC_WSA_ILIM_CFG9, 0x30,
				      (bat_cfg - 1) << 0x4);
	/* Thesh */
	vth1 = LPASS_CDC_WSA_MACRO_VTH_TO_REG(
		pbr_vth1_data[sys_gain][bat_cfg][rload]);
...
static void lpass_cdc_wsa_macro_init_reg(struct snd_soc_component *component)
{
	...
	for (i = 0; i < ARRAY_SIZE(lpass_cdc_wsa_macro_reg_init); i++)
		snd_soc_component_update_bits(
			component, lpass_cdc_wsa_macro_reg_init[i].reg,
...
static int lpass_cdc_wsa_macro_core_vote(void *handle, bool enable)
{
	int rc = 0;
	struct lpass_cdc_wsa_macro_priv *wsa_priv =
		(struct lpass_cdc_wsa_macro_priv *)handle;
...
static int wsa_swrm_clock(void *handle, bool enable)
{
	...
	if (enable) {
		...
		if (wsa_priv->swr_clk_users == 0) {
			...
			ret = lpass_cdc_wsa_macro_mclk_enable(wsa_priv, 1,
							      true);
			if (ret < 0) {
			...
			if (wsa_priv->reset_swr)
				regmap_update_bits(
					...
					0x02, 0x02);
			regmap_update_bits(
				regmap, LPASS_CDC_WSA_CLK_RST_CTRL_SWR_CONTROL,
				0x01, 0x01);
			if (wsa_priv->reset_swr)
				regmap_update_bits(
					regmap,
...
/* Thermal Functions */
static int
lpass_cdc_wsa_macro_get_max_state(struct thermal_cooling_device *cdev,
				  unsigned long *state)
{
...
static int
lpass_cdc_wsa_macro_get_cur_state(struct thermal_cooling_device *cdev,
				  unsigned long *state)
{
	struct lpass_cdc_wsa_macro_priv *wsa_priv = cdev->devdata;
...
static int
lpass_cdc_wsa_macro_set_cur_state(struct thermal_cooling_device *cdev,
				  unsigned long state)
{
	struct lpass_cdc_wsa_macro_priv *wsa_priv = cdev->devdata;
	...
	dev_dbg(wsa_priv->dev, "%s: set the thermal current state to %d\n",
		__func__, wsa_priv->thermal_cur_state);
...
static int lpass_cdc_wsa_macro_init(struct snd_soc_component *component)
{
	...
	ret = snd_soc_dapm_new_controls(
		dapm, lpass_cdc_wsa_macro_dapm_widgets,
	...
	ret = snd_soc_add_component_controls(
		component, lpass_cdc_wsa_macro_snd_controls,
	...
	for_each_available_child_of_node(wsa_priv->dev->of_node, node) {
		...
		if (strnstr(node->name, "wsa_swr_master",
			...
			temp = krealloc(
				swr_ctrl_data,
				(ctrl_num +
				 1) * sizeof(struct lpass_cdc_wsa_macro_swr_ctrl_data),
				GFP_KERNEL);
			if (!temp) {
		...
		if (wsa_priv->child_count <
		    LPASS_CDC_WSA_MACRO_CHILD_DEVICES_MAX)
			wsa_priv->pdev_child_devices[wsa_priv->child_count++] =
				pdev;
		else
			goto err;
	}
...
static int lpass_cdc_wsa_macro_probe(struct platform_device *pdev)
{
	...
	wsa_priv = devm_kzalloc(&pdev->dev,
				sizeof(struct lpass_cdc_wsa_macro_priv),
				GFP_KERNEL);
	if (!wsa_priv)
		return -ENOMEM;
	...
	ret = of_property_read_u32(pdev->dev.of_node, "reg", &wsa_base_addr);
	if (ret) {
	...
	if (!wsa_priv->wsa_fs_reg_base && wsa_priv->wsa_fs_ctl_reg)
		wsa_priv->wsa_fs_reg_base =
			devm_ioremap(&pdev->dev, wsa_priv->wsa_fs_ctl_reg,
				     LPASS_CDC_WSA_MACRO_MAX_OFFSET);
	...
	wsa_priv->wsa_swr_gpio_p =
		of_parse_phandle(pdev->dev.of_node, "qcom,wsa-swr-gpios", 0);
	...
	msm_cdc_pinctrl_set_wakeup_capable(wsa_priv->wsa_swr_gpio_p, false);

	wsa_io_base = devm_ioremap(&pdev->dev, wsa_base_addr,
				   LPASS_CDC_WSA_MACRO_MAX_OFFSET);
	if (!wsa_io_base) {
	...
	if (of_find_property(wsa_priv->dev->of_node, "#cooling-cells", NULL)) {
		...
		wsa_priv->tcdev = devm_thermal_of_cooling_device_register(
			&pdev->dev, wsa_priv->dev->of_node, "wsa", wsa_priv,
			&wsa_cooling_ops);
...
static int lpass_cdc_wsa_macro_remove(struct platform_device *pdev)
{
	...
	for (count = 0; count < wsa_priv->child_count &&
			count < LPASS_CDC_WSA_MACRO_CHILD_DEVICES_MAX;
	     count++)
