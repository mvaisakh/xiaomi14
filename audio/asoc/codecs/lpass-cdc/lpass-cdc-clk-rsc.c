...
static char clk_src_name[MAX_CLK][LPASS_CDC_CLK_NAME_LENGTH] = {
	"tx_core_clk",	   "rx_core_clk",      "wsa_core_clk",
...
static char __iomem *
lpass_cdc_clk_rsc_get_clk_muxsel(struct lpass_cdc_clk_rsc *priv, int clk_id)
{
	switch (clk_id) {
	case RX_CORE_CLK:
	...
	if (enable) {
		if (priv->clk_cnt[clk_id] == 0) {
			if (clk_id != VA_CORE_CLK) {
				ret = lpass_cdc_clk_rsc_mux0_clk_request(
...
static int
lpass_cdc_clk_rsc_check_and_update_va_clk(struct lpass_cdc_clk_rsc *priv,
	...
	if (enable) {
       ...
       */
			if (priv->clk_cnt[TX_CORE_CLK] == 0) {
				ret = lpass_cdc_clk_rsc_mux1_clk_request(
 ...
 */
int lpass_cdc_clk_rsc_request_clock(struct device *dev, int default_clk_id,
				    int clk_id_req, bool enable)
{
	...
	if (mux_switch) {
		if (clk_id_req != VA_CORE_CLK) {
			ret = lpass_cdc_clk_rsc_mux1_clk_request(
				priv, clk_id_req, enable);
			if (ret < 0)
				goto err;
		}
	} else {
		ret = lpass_cdc_clk_rsc_mux0_clk_request(priv, clk_id_req,
							 enable);
		if (ret < 0)
			goto err;
	}
