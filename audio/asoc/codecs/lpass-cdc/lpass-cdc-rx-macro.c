...
#define LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(name, reg, offset, text, getname, \
					 putname)                          \
	static SOC_ENUM_SINGLE_DECL(name##_enum, reg, offset, text);       \
	static const struct snd_kcontrol_new name##_mux =                  \
...
static struct comp_coeff_val comp_coeff_table[HPH_MODE_MAX][COMP_MAX_COEFF] = {
	{
		{ 0x40, 0x00 }, { 0x4C, 0x00 }, { 0x5A, 0x00 }, { 0x6B, 0x00 },
		{ 0x7F, 0x00 }, { 0x97, 0x00 }, { 0xB3, 0x00 }, { 0xD5, 0x00 },
		{ 0xFD, 0x00 }, { 0x2D, 0x01 }, { 0x66, 0x01 }, { 0xA7, 0x01 },
		{ 0xF8, 0x01 }, { 0x57, 0x02 }, { 0xC7, 0x02 }, { 0x4B, 0x03 },
		{ 0xE9, 0x03 }, { 0xA3, 0x04 }, { 0x7D, 0x05 }, { 0x90, 0x06 },
		{ 0xD1, 0x07 }, { 0x49, 0x09 }, { 0x00, 0x0B }, { 0x01, 0x0D },
		{ 0x59, 0x0F },
	},
	{
		{ 0x40, 0x00 }, { 0x4C, 0x00 }, { 0x5A, 0x00 }, { 0x6B, 0x00 },
		{ 0x80, 0x00 }, { 0x98, 0x00 }, { 0xB4, 0x00 }, { 0xD5, 0x00 },
		{ 0xFE, 0x00 }, { 0x2E, 0x01 }, { 0x66, 0x01 }, { 0xA9, 0x01 },
		{ 0xF8, 0x01 }, { 0x56, 0x02 }, { 0xC4, 0x02 }, { 0x4F, 0x03 },
		{ 0xF0, 0x03 }, { 0xAE, 0x04 }, { 0x8B, 0x05 }, { 0x8E, 0x06 },
		{ 0xBC, 0x07 }, { 0x56, 0x09 }, { 0x0F, 0x0B }, { 0x13, 0x0D },
		{ 0x6F, 0x0F },
	},
};

enum { RX_MODE_ULP, RX_MODE_LOHIFI, RX_MODE_EAR, RX_MODE_MAX };
...
enum { INTERP_HPHL, INTERP_HPHR, INTERP_AUX, INTERP_MAX };
...
#define LPASS_CDC_RX_MACRO_IIR_FILTER_CTL(xname, iidx, bidx)               \
	{                                                                  \
		...
		.private_value = (unsigned long)&(                         \
			struct lpass_cdc_rx_macro_iir_filter_ctl)          \
		{                                                          \
			.iir_idx = iidx, .band_idx = bidx,                 \
			.bytes_ext = {                                     \
				.max = LPASS_CDC_RX_MACRO_IIR_FILTER_SIZE, \
			},                                                 \
		}                                                          \
	}
...
#define LPASS_CDC_RX_MACRO_FIR_FILTER_CTL(xname, pidx, gidx)                \
	{                                                                   \
		...
		.private_value = (unsigned long)&(                          \
			struct lpass_cdc_rx_macro_fir_filter_ctl)           \
		{                                                           \
			.path_idx = pidx, .grp_idx = gidx,                  \
			.bytes_ext = {                                      \
				.max = LPASS_CDC_RX_MACRO_FIR_FILTER_BYTES, \
			},                                                  \
		}                                                           \
	}
...
static int
lpass_cdc_rx_macro_int_dem_inp_mux_put(struct snd_kcontrol *kcontrol,
...
static int lpass_cdc_rx_macro_mux_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol);
static int
lpass_cdc_rx_macro_enable_interp_clk(struct snd_soc_component *component,
...
struct lpass_cdc_rx_macro_priv {
	...
	struct platform_device
		*pdev_child_devices[LPASS_CDC_RX_MACRO_CHILD_DEVICES_MAX];
	int child_count;
	int is_softclip_on;
	int is_aux_hpf_on;
...
static const char *const rx_int_mix_mux_text[] = { "ZERO", "RX0", "RX1", "RX2",
						   "RX3",  "RX4", "RX5" };
...
static const char *const iir_inp_mux_text[] = { "ZERO", "DEC0", "DEC1", "DEC2",
						"DEC3", "RX0",	"RX1",	"RX2",
						"RX3",	"RX4",	"RX5" };
...
static const char *const lpass_cdc_rx_macro_mux_text[] = { "ZERO", "AIF1_PB",
							   "AIF2_PB", "AIF3_PB",
							   "AIF4_PB" };
...
static const char *const lpass_cdc_rx_macro_hph_hd2_mode_text[] = { "OFF",
								    "ON" };
...
static const char *const lpass_cdc_rx_macro_hph_pwr_mode_text[] = { "ULP",
								    "LOHIFI" };
...
static const char *const lpass_cdc_rx_macro_vbat_bcl_gsm_mode_text[] = { "OFF",
									 "ON" };
...
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int0_1_mix_inp0,
			     LPASS_CDC_RX_INP_MUX_RX_INT0_CFG0, 0,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int0_1_mix_inp1,
			     LPASS_CDC_RX_INP_MUX_RX_INT0_CFG0, 4,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int0_1_mix_inp2,
			     LPASS_CDC_RX_INP_MUX_RX_INT0_CFG1, 4,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int1_1_mix_inp0,
			     LPASS_CDC_RX_INP_MUX_RX_INT1_CFG0, 0,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int1_1_mix_inp1,
			     LPASS_CDC_RX_INP_MUX_RX_INT1_CFG0, 4,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int1_1_mix_inp2,
			     LPASS_CDC_RX_INP_MUX_RX_INT1_CFG1, 4,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int2_1_mix_inp0,
			     LPASS_CDC_RX_INP_MUX_RX_INT2_CFG0, 0,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int2_1_mix_inp1,
			     LPASS_CDC_RX_INP_MUX_RX_INT2_CFG0, 4,
			     rx_prim_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int2_1_mix_inp2,
			     LPASS_CDC_RX_INP_MUX_RX_INT2_CFG1, 4,
			     rx_prim_mix_text);

LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int0_mix2_inp,
			     LPASS_CDC_RX_INP_MUX_SIDETONE_SRC_CFG0, 2,
			     rx_sidetone_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int1_mix2_inp,
			     LPASS_CDC_RX_INP_MUX_SIDETONE_SRC_CFG0, 4,
			     rx_sidetone_mix_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(rx_int2_mix2_inp,
			     LPASS_CDC_RX_INP_MUX_SIDETONE_SRC_CFG0, 6,
			     rx_sidetone_mix_text);

LPASS_CDC_RX_MACRO_DAPM_ENUM(iir0_inp0, LPASS_CDC_RX_IIR_INP_MUX_IIR0_MIX_CFG0,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir0_inp1, LPASS_CDC_RX_IIR_INP_MUX_IIR0_MIX_CFG1,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir0_inp2, LPASS_CDC_RX_IIR_INP_MUX_IIR0_MIX_CFG2,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir0_inp3, LPASS_CDC_RX_IIR_INP_MUX_IIR0_MIX_CFG3,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir1_inp0, LPASS_CDC_RX_IIR_INP_MUX_IIR1_MIX_CFG0,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir1_inp1, LPASS_CDC_RX_IIR_INP_MUX_IIR1_MIX_CFG1,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir1_inp2, LPASS_CDC_RX_IIR_INP_MUX_IIR1_MIX_CFG2,
			     0, iir_inp_mux_text);
LPASS_CDC_RX_MACRO_DAPM_ENUM(iir1_inp3, LPASS_CDC_RX_IIR_INP_MUX_IIR1_MIX_CFG3,
			     0, iir_inp_mux_text);
...
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(rx_int0_dem_inp, LPASS_CDC_RX_RX0_RX_PATH_CFG1,
				 0, rx_int_dem_inp_mux_text,
				 snd_soc_dapm_get_enum_double,
				 lpass_cdc_rx_macro_int_dem_inp_mux_put);
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(rx_int1_dem_inp, LPASS_CDC_RX_RX1_RX_PATH_CFG1,
				 0, rx_int_dem_inp_mux_text,
				 snd_soc_dapm_get_enum_double,
				 lpass_cdc_rx_macro_int_dem_inp_mux_put);

LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(lpass_cdc_rx_macro_rx0, SND_SOC_NOPM, 0,
				 lpass_cdc_rx_macro_mux_text,
				 lpass_cdc_rx_macro_mux_get,
				 lpass_cdc_rx_macro_mux_put);
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(lpass_cdc_rx_macro_rx1, SND_SOC_NOPM, 0,
				 lpass_cdc_rx_macro_mux_text,
				 lpass_cdc_rx_macro_mux_get,
				 lpass_cdc_rx_macro_mux_put);
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(lpass_cdc_rx_macro_rx2, SND_SOC_NOPM, 0,
				 lpass_cdc_rx_macro_mux_text,
				 lpass_cdc_rx_macro_mux_get,
				 lpass_cdc_rx_macro_mux_put);
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(lpass_cdc_rx_macro_rx3, SND_SOC_NOPM, 0,
				 lpass_cdc_rx_macro_mux_text,
				 lpass_cdc_rx_macro_mux_get,
				 lpass_cdc_rx_macro_mux_put);
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(lpass_cdc_rx_macro_rx4, SND_SOC_NOPM, 0,
				 lpass_cdc_rx_macro_mux_text,
				 lpass_cdc_rx_macro_mux_get,
				 lpass_cdc_rx_macro_mux_put);
LPASS_CDC_RX_MACRO_DAPM_ENUM_EXT(lpass_cdc_rx_macro_rx5, SND_SOC_NOPM, 0,
				 lpass_cdc_rx_macro_mux_text,
				 lpass_cdc_rx_macro_mux_get,
				 lpass_cdc_rx_macro_mux_put);
...
static const struct soc_enum rx_mix_tx2_mux_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_RX_INP_MUX_RX_MIX_CFG5, 0, 4, rx_echo_mux_text);
...
static const struct soc_enum rx_mix_tx1_mux_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_RX_INP_MUX_RX_MIX_CFG4, 0, 4, rx_echo_mux_text);
...
static const struct soc_enum rx_mix_tx0_mux_enum = SOC_ENUM_SINGLE(
	LPASS_CDC_RX_INP_MUX_RX_MIX_CFG4, 4, 4, rx_echo_mux_text);
        ...
        .playback =
            {
                ...
                .rates =
                    LPASS_CDC_RX_MACRO_RATES | LPASS_CDC_RX_MACRO_FRAC_RATES,
 ...
 */
static void
lpass_cdc_rx_macro_wcd_clsh_imped_config(struct snd_soc_component *component,
	...
	if (reset) {
		for (i = 0; i < MAX_IMPED_PARAMS; i++)
			snd_soc_component_update_bits(
				component, imped_table_ptr[index][i].reg,
...
static bool lpass_cdc_rx_macro_get_data(
	struct snd_soc_component *component, struct device **rx_dev,
	struct lpass_cdc_rx_macro_priv **rx_priv, const char *func_name)
{
					   ...
					   u32 usecase, u32 size, void *data)
{
	...
	if (!lpass_cdc_rx_macro_get_data(component, &rx_dev, &rx_priv,
					 __func__))
		return -EINVAL;
	...
	if (rx_priv->swr_ctrl_data)
		ret = swrm_wcd_notify(rx_priv->swr_ctrl_data[0].rx_swr_pdev,
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int val = 0;
	unsigned short look_ahead_dly_reg = LPASS_CDC_RX_RX0_RX_PATH_CFG0;
...
static int
lpass_cdc_rx_macro_set_prim_interpolator_rate(struct snd_soc_dai *dai,
					      u8 rate_reg_val, u32 sample_rate)
{
     ...
     */
		for (j = 0; j < INTERP_MAX; j++) {
			...
			int_mux_cfg0_val =
				snd_soc_component_read(component, int_mux_cfg0);
			int_mux_cfg1_val =
				snd_soc_component_read(component, int_mux_cfg1);
	...
	for_each_set_bit(port, &rx_priv->active_ch_mask[dai->id],
		...
		for (j = 0; j < INTERP_MAX; j++) {
			int_mux_cfg1_val = snd_soc_component_read(
					   ...
					   0x0F;
			if (int_mux_cfg1_val ==
			    int_2_inp + INTn_2_INP_SEL_RX0) {
				int_fs_reg =
					LPASS_CDC_RX_RX0_RX_PATH_MIX_CTL +
					LPASS_CDC_RX_MACRO_RX_PATH_OFFSET * j;
				pr_debug(
						    ...
						    u32 sample_rate)
{
	...
	for (i = 0; i < ARRAY_SIZE(sr_val_tbl); i++) {
		if (sample_rate == sr_val_tbl[i].sample_rate) {
			...
			if (lpass_cdc_rx_macro_is_fractional_sample_rate(
				    sample_rate))
	...
	ret = lpass_cdc_rx_macro_set_prim_interpolator_rate(dai, (u8)rate_val,
							    sample_rate);
	if (ret)
		return ret;
	ret = lpass_cdc_rx_macro_set_mix_interpolator_rate(dai, (u8)rate_val,
							   sample_rate);
	if (ret)
		return ret;
					...
					struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_STREAM_PLAYBACK:
		ret = lpass_cdc_rx_macro_set_interpolator_rate(
			dai, params_rate(params));
		if (ret) {
...
static int
lpass_cdc_rx_macro_mclk_enable(struct lpass_cdc_rx_macro_priv *rx_priv,
			       bool mclk_enable, bool dapm)
{
	...
	if (mclk_enable) {
		if (rx_priv->rx_mclk_users == 0) {
			...
			ret = lpass_cdc_clk_rsc_request_clock(
				rx_priv->dev, rx_priv->default_clk_id,
				rx_priv->clk_id, true);
			...
			lpass_cdc_clk_rsc_fs_gen_request(rx_priv->dev, true);
			regcache_mark_dirty(regmap);
			regcache_sync_region(regmap, RX_START_OFFSET,
					     RX_MAX_OFFSET);
			regmap_update_bits(
				regmap, LPASS_CDC_RX_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x01);
			regmap_update_bits(
				regmap, LPASS_CDC_RX_CLK_RST_CTRL_MCLK_CONTROL,
				0x02, 0x02);
			regmap_update_bits(
				regmap,
				LPASS_CDC_RX_CLK_RST_CTRL_FS_CNT_CONTROL, 0x02,
				0x00);
			regmap_update_bits(
				regmap,
				LPASS_CDC_RX_CLK_RST_CTRL_FS_CNT_CONTROL, 0x01,
				0x01);
		}
		...
		if (rx_priv->rx_mclk_users == 0) {
			...
			regmap_update_bits(
				regmap, LPASS_CDC_RX_CLK_RST_CTRL_MCLK_CONTROL,
				0x01, 0x00);
			lpass_cdc_clk_rsc_fs_gen_request(rx_priv->dev, false);
			ret = lpass_cdc_rx_macro_core_vote(rx_priv, true);
			if (ret < 0) {
				dev_err_ratelimited(
					rx_priv->dev,
					 ...
					 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (rx_priv->swr_ctrl_data)
			swrm_wcd_notify(rx_priv->swr_ctrl_data[0].rx_swr_pdev,
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		break;
	default:
		dev_err_ratelimited(rx_priv->dev, "%s: invalid DAPM event %d\n",
				    __func__, event);
		ret = -EINVAL;
	}
	return ret;
					    ...
					    u16 event, u32 data)
{
	...
	case LPASS_CDC_MACRO_EVT_RX_MUTE:
		...
		reg = LPASS_CDC_RX_RX0_RX_PATH_CTL +
		      (rx_idx * LPASS_CDC_RX_MACRO_RX_PATH_OFFSET);
		reg_mix = LPASS_CDC_RX_RX0_RX_PATH_MIX_CTL +
			  (rx_idx * LPASS_CDC_RX_MACRO_RX_PATH_OFFSET);
		snd_soc_component_update_bits(component, reg, 0x10, val);
		snd_soc_component_update_bits(component, reg_mix, 0x10, val);
		break;
	...
	case LPASS_CDC_MACRO_EVT_IMPED_FALSE:
		lpass_cdc_rx_macro_wcd_clsh_imped_config(component, data,
							 false);
		break;
	...
	case LPASS_CDC_MACRO_EVT_RX_PA_GAIN_UPDATE:
		rx_priv->rx0_gain_val = snd_soc_component_read(
			component, LPASS_CDC_RX_RX0_RX_VOL_CTL);
		rx_priv->rx1_gain_val = snd_soc_component_read(
			component, LPASS_CDC_RX_RX1_RX_VOL_CTL);
		if (data) {
			/* Reduce gain by half only if its greater than -6DB */
			if ((rx_priv->rx0_gain_val >=
			     LPASS_CDC_RX_MACRO_GAIN_VAL_UNITY) &&
			    (rx_priv->rx0_gain_val <=
			     LPASS_CDC_RX_MACRO_GAIN_MAX_VAL))
				snd_soc_component_update_bits(
					component, LPASS_CDC_RX_RX0_RX_VOL_CTL,
					0xFF,
					(rx_priv->rx0_gain_val -
	...
	case LPASS_CDC_MACRO_EVT_HPHL_HD2_ENABLE:
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_PATH_CFG0, 0x04, data);
		break;
	case LPASS_CDC_MACRO_EVT_HPHR_HD2_ENABLE:
		/* Enable hd2 config for hphr*/
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX1_RX_PATH_CFG0, 0x04, data);
		break;
	}
done:
	return ret;
...
static int lpass_cdc_rx_macro_find_playback_dai_id_for_port(
	int port_id, struct lpass_cdc_rx_macro_priv *rx_priv)
{
	int i = 0;
...
static int
lpass_cdc_rx_macro_set_idle_detect_thr(struct snd_soc_component *component,
				       ...
				       int interp, int path_type)
{
   ...
   */
	if (path_type == INTERP_MIX_PATH) {
		mux_reg = LPASS_CDC_RX_INP_MUX_RX_INT0_CFG1 + 2 * interp;
		mux_reg_val = snd_soc_component_read(component, mux_reg) & 0x0f;
	...
	if (path_type == INTERP_MAIN_PATH) {
		mux_reg = LPASS_CDC_RX_INP_MUX_RX_INT1_CFG0 + 2 * (interp - 1);
		mux_reg_val = snd_soc_component_read(component, mux_reg) & 0x0f;
		...
		while (i) {
			if ((mux_reg_val >= INTn_1_INP_SEL_RX0) &&
				...
				*port_ptr++ = mux_reg_val - INTn_1_INP_SEL_RX0;
				num_ports++;
			}
			mux_reg_val =
	...
	i = 0;
	while (num_ports) {
		dai_id = lpass_cdc_rx_macro_find_playback_dai_id_for_port(
			port_id[i++], rx_priv);

		if ((dai_id >= 0) && (dai_id < LPASS_CDC_RX_MACRO_MAX_DAIS)) {
			dev_dbg(component->dev,
				"%s: dai_id: %d bit_width: %d\n", __func__,
				dai_id, rx_priv->bit_width[dai_id]);
	...
	if ((rx_priv->idle_det_cfg.hph_idle_thr == 0) ||
		...
		snd_soc_component_write(
			component, LPASS_CDC_RX_IDLE_DETECT_CFG3, idle_thr);
					      ...
					      int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		lpass_cdc_rx_macro_set_idle_detect_thr(
			component, rx_priv, w->shift, INTERP_MIX_PATH);
		lpass_cdc_rx_macro_enable_interp_clk(component, event,
						     w->shift);
		/* Clk Enable */
					       ...
					       int event)
{
	...
	reg = LPASS_CDC_RX_RX0_RX_PATH_CTL +
	      (w->shift * LPASS_CDC_RX_MACRO_RX_PATH_OFFSET);
	gain_reg = LPASS_CDC_RX_RX0_RX_VOL_CTL +
		   (w->shift * LPASS_CDC_RX_MACRO_RX_PATH_OFFSET);

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		lpass_cdc_rx_macro_set_idle_detect_thr(
			component, rx_priv, w->shift, INTERP_MAIN_PATH);
		lpass_cdc_rx_macro_enable_interp_clk(component, event,
						     w->shift);
		if (lpass_cdc_rx_macro_adie_lb(component, w->shift))
			snd_soc_component_update_bits(component, reg, 0x20,
						      0x20);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_write(component, gain_reg,
...
static void
lpass_cdc_rx_macro_droop_setting(struct snd_soc_component *component,
				 struct lpass_cdc_rx_macro_priv *rx_priv,
				 int interp_n, int event)
{
	...
	rx0_path_ctl_reg = LPASS_CDC_RX_RX0_RX_PATH_CTL +
			   (interp_n * LPASS_CDC_RX_MACRO_RX_PATH_OFFSET);
	pcm_rate = (snd_soc_component_read(component, rx0_path_ctl_reg) & 0x0F);
	if (pcm_rate < 0x06)
		val = 0x03;
	...
	if (SND_SOC_DAPM_EVENT_ON(event))
		snd_soc_component_update_bits(component, rx_path_cfg3_reg, 0x03,
					      val);
...
static int
lpass_cdc_rx_macro_config_compander(struct snd_soc_component *component,
				    struct lpass_cdc_rx_macro_priv *rx_priv,
				    int interp_n, int event)
{
	int comp = 0;
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		lpass_cdc_load_compander_coeff(
			component, comp_coeff_lsb_reg, comp_coeff_msb_reg,
			comp_coeff_table[rx_priv->hph_pwr_mode],
			COMP_MAX_COEFF);

		lpass_cdc_update_compander_setting(component, comp_ctl8_reg,
...
static void
lpass_cdc_rx_macro_enable_softclip_clk(struct snd_soc_component *component,
				       struct lpass_cdc_rx_macro_priv *rx_priv,
				       bool enable)
{
	if (enable) {
...
static int
lpass_cdc_rx_macro_config_softclip(struct snd_soc_component *component,
				   struct lpass_cdc_rx_macro_priv *rx_priv,
				   int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		lpass_cdc_rx_macro_enable_softclip_clk(component, rx_priv,
						       true);
		/* Enable Softclip control */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_SOFTCLIP_SOFTCLIP_CTRL, 0x01,
			0x01);
	}
...
static int
lpass_cdc_rx_macro_config_aux_hpf(struct snd_soc_component *component,
				  struct lpass_cdc_rx_macro_priv *rx_priv,
				  int event)
{
	dev_dbg(component->dev, "%s: event %d, enabled %d\n", __func__, event,
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		if (!rx_priv->is_aux_hpf_on)
			snd_soc_component_update_bits(
				component, LPASS_CDC_RX_RX2_RX_PATH_CFG1, 0x04,
				0x00);
	}
	...
	if (SND_SOC_DAPM_EVENT_OFF(event)) {
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG1, 0x04, 0x04);
	}
...
static inline void
lpass_cdc_rx_macro_enable_clsh_block(struct lpass_cdc_rx_macro_priv *rx_priv,
				     bool enable)
{
...
static int
lpass_cdc_rx_macro_config_classh(struct snd_soc_component *component,
				 struct lpass_cdc_rx_macro_priv *rx_priv,
				 int interp_n, int event)
{
	if (interp_n == INTERP_AUX)
	...
	if (interp_n == INTERP_HPHL || interp_n == INTERP_HPHR) {
		/*
     * These K1 values depend on the Headphone Impedance
     * For now it is assumed to be 16 ohm
     */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CLSH_K1_LSB, 0xFF, 0xC0);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CLSH_K1_MSB, 0x0F, 0x00);
	}
	switch (interp_n) {
	case INTERP_HPHL:
		if (rx_priv->is_ear_mode_on)
			snd_soc_component_update_bits(
				component, LPASS_CDC_RX_CLSH_HPH_V_PA, 0x3F,
				0x39);
		else
			snd_soc_component_update_bits(
				component, LPASS_CDC_RX_CLSH_HPH_V_PA, 0x3F,
				0x1C);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CLSH_DECAY_CTRL, 0x07, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_PATH_CFG0, 0x40, 0x40);
		break;
	case INTERP_HPHR:
		if (rx_priv->is_ear_mode_on)
			snd_soc_component_update_bits(
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CLSH_DECAY_CTRL, 0x07, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX1_RX_PATH_CFG0, 0x40, 0x40);
		break;
	case INTERP_AUX:
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG0, 0x08, 0x08);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG0, 0x10, 0x10);
		break;
	}
...
static void
lpass_cdc_rx_macro_hd2_control(struct snd_soc_component *component,
			       struct lpass_cdc_rx_macro_priv *rx_priv,
...
static int
lpass_cdc_rx_macro_hph_idle_detect_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
...
static int
lpass_cdc_rx_macro_hph_idle_detect_put(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	case 3:
	case 4:
		set_bit(widget->shift, &rx_priv->active_ch_mask[rx_port_value]);
...
static int
lpass_cdc_rx_macro_get_hph_hd2_mode(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_put_hph_hd2_mode(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_get_hph_pwr_mode(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_put_hph_pwr_mode(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int lpass_cdc_rx_macro_vbat_bcl_gsm_mode_func_get(
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		((snd_soc_component_read(component, LPASS_CDC_RX_BCL_VBAT_CFG) &
		  0x04) ?
			 1 :
			 0);
...
static int lpass_cdc_rx_macro_vbat_bcl_gsm_mode_func_put(
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (ucontrol->value.integer.value[0])
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_CFG, 0x04, 0x04);
	else
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_CFG, 0x04, 0x00);

	return 0;
}

static int
lpass_cdc_rx_macro_soft_clip_enable_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_soft_clip_enable_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_aux_hpf_mode_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_aux_hpf_mode_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
					  ...
					  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_PATH_CTL, 0x10, 0x10);
		/* Enable VBAT block */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_CFG, 0x01, 0x01);
		/* Update interpolator with 384K path */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG1, 0x80, 0x80);
		/* Update DSM FS rate */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_SEC7, 0x02, 0x02);
		/* Use attenuation mode */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_CFG, 0x02, 0x00);
		/* BCL block needs softclip clock to be enabled */
		lpass_cdc_rx_macro_enable_softclip_clk(component, rx_priv,
						       true);
		/* Enable VBAT at channel level */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG1, 0x02, 0x02);
		/* Set the ATTK1 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD1, 0xFF,
			0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD2, 0xFF,
			0x03);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD3, 0xFF,
			0x00);
		/* Set the ATTK2 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD4, 0xFF,
			0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD5, 0xFF,
			0x03);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD6, 0xFF,
			0x00);
		/* Set the ATTK3 gain */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD7, 0xFF,
			0xFF);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD8, 0xFF,
			0x03);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD9, 0xFF,
			0x00);
		/* Enable CB decode block clock */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CB_DECODE_CB_DECODE_CTL1, 0x01,
			0x01);
		/* Enable BCL path */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CB_DECODE_CB_DECODE_CTL2, 0x01,
			0x01);
		/* Request for BCL data */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_CB_DECODE_CB_DECODE_CTL3, 0x01,
			0x01);
		break;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_component_update_bits(
			...
			0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG1, 0x80, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_SEC7, 0x02, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_CFG, 0x02, 0x02);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX2_RX_PATH_CFG1, 0x02, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_BCL_GAIN_UPD1, 0xFF,
			0x00);
		snd_soc_component_update_bits(
		...
		lpass_cdc_rx_macro_enable_softclip_clk(component, rx_priv,
						       false);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_CFG, 0x01, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_BCL_VBAT_PATH_CTL, 0x10, 0x00);
		break;
	default:
...
static void
lpass_cdc_rx_macro_idle_detect_control(struct snd_soc_component *component,
				       struct lpass_cdc_rx_macro_priv *rx_priv,
...
static void
lpass_cdc_rx_macro_hphdelay_lutbypass(struct snd_soc_component *component,
				      struct lpass_cdc_rx_macro_priv *rx_priv,
				      u16 interp_idx, int event)
{
	...
	if (hph_lut_bypass_reg && SND_SOC_DAPM_EVENT_ON(event)) {
		if (interp_idx == INTERP_HPHL) {
			if (rx_priv->is_ear_mode_on)
				snd_soc_component_update_bits(
					component,
					LPASS_CDC_RX_RX0_RX_PATH_CFG1, 0x02,
					0x02);
			else
				snd_soc_component_update_bits(
					component, hph_lut_bypass_reg, 0x80,
					0x80);
		} else {
			snd_soc_component_update_bits(
				component, hph_lut_bypass_reg, 0x80, 0x80);
		}
	...
	if (hph_lut_bypass_reg && SND_SOC_DAPM_EVENT_OFF(event)) {
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_PATH_CFG1, 0x02, 0x00);
				     ...
				     int event, int interp_idx)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (rx_priv->main_clk_users[interp_idx] == 0) {
			...
			snd_soc_component_update_bits(component, main_reg, 0x10,
						      0x10);
			snd_soc_component_update_bits(component, dsm_reg, 0x01,
						      0x01);
			/* Clk Enable */
			...
			snd_soc_component_update_bits(component, rx_cfg2_reg,
						      0x03, 0x03);
			lpass_cdc_rx_macro_idle_detect_control(
				component, rx_priv, interp_idx, event);
			if (rx_priv->hph_hd2_mode)
				lpass_cdc_rx_macro_hd2_control(
					component, rx_priv, interp_idx, event);
			lpass_cdc_rx_macro_hphdelay_lutbypass(
				component, rx_priv, interp_idx, event);
			lpass_cdc_rx_macro_droop_setting(component, rx_priv,
							 interp_idx, event);
			lpass_cdc_rx_macro_config_compander(component, rx_priv,
							    interp_idx, event);
			if (interp_idx == INTERP_AUX) {
				lpass_cdc_rx_macro_config_softclip(
					component, rx_priv, event);
				lpass_cdc_rx_macro_config_aux_hpf(
					component, rx_priv, event);
			}
			...
			if (rx_priv->is_pcm_enabled && !rx_priv->is_native_on &&
				...
				if (rx_priv->pcm_select_users == 0)
					snd_soc_component_update_bits(
						component,
						LPASS_CDC_RX_TOP_SWR_CTRL, 0x02,
						0x02);
				++rx_priv->pcm_select_users;
			}
			lpass_cdc_notify_wcd_rx_clk(rx_dev,
	...
	dev_dbg(component->dev, "%s event %d main_clk_users %d\n", __func__,
		event, rx_priv->main_clk_users[interp_idx]);
						 ...
						 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(component, sidetone_reg, 0x10,
					      0x10);
		snd_soc_component_update_bits(component, fs_reg, 0x20, 0x20);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, sidetone_reg, 0x10,
					      0x00);
...
static void
lpass_cdc_rx_macro_restore_iir_coeff(struct lpass_cdc_rx_macro_priv *rx_priv,
				     int iir_idx, int band_idx)
{
	...
	for (coeff_idx = 0;
	     coeff_idx < LPASS_CDC_RX_MACRO_SIDETONE_IIR_COEFF_MAX;
	     coeff_idx++) {
...
static int lpass_cdc_rx_macro_iir_enable_audio_mixer_get(
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int lpass_cdc_rx_macro_iir_enable_audio_mixer_put(
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_iir_filter_info(struct snd_kcontrol *kcontrol,
...
static int
lpass_cdc_rx_macro_iir_band_audio_mixer_get(struct snd_kcontrol *kcontrol,
					    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	for (coeff_idx = 0;
	     coeff_idx < LPASS_CDC_RX_MACRO_SIDETONE_IIR_COEFF_MAX;
	     coeff_idx++) {
		coeff[coeff_idx] = get_iir_band_coeff(component, iir_idx,
						      band_idx, coeff_idx);
	}
	...
	dev_dbg(component->dev,
		...
		__func__, iir_idx, band_idx, coeff[0], __func__, iir_idx,
		band_idx, coeff[1], __func__, iir_idx, band_idx, coeff[2],
		__func__, iir_idx, band_idx, coeff[3], __func__, iir_idx,
		band_idx, coeff[4]);
	return 0;
}
...
static int
lpass_cdc_rx_macro_iir_band_audio_mixer_put(struct snd_kcontrol *kcontrol,
					    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_fir_filter_enable_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	ucontrol->value.bytes.data[0] =
		(unsigned char)rx_priv->is_fir_filter_on;
	return 0;
}

static int
lpass_cdc_rx_macro_fir_filter_enable_put(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (rx_priv->is_fir_filter_on) {
		...
		dev_dbg(component->dev,
			"%s: HIFI FIR Path:%d total coefficients"
			" number written: %d.\n",
			__func__, RX0_PATH,
			rx_priv->fir_total_coeff_num[RX0_PATH]);
		...
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_TOP_TOP_CFG1, 0x01, 0x01);
		/* Enable FIR_CLK_EN */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_PATH_CTL, 0x80, 0x80);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX1_RX_PATH_CTL, 0x80, 0x80);
		/* Start the FIR filter */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_FIR_CTL, 0x0D, 0x05);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX1_RX_FIR_CTL, 0x0D, 0x05);
	} else {
		/* Stop the FIR filter */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_FIR_CTL, 0x0D, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX1_RX_FIR_CTL, 0x0D, 0x00);
		/* Disable FIR_CLK_EN */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX0_RX_PATH_CTL, 0x80, 0x00);
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_RX1_RX_PATH_CTL, 0x80, 0x00);
		/* Disable HIFI_FEAT_EN bit */
		snd_soc_component_update_bits(
			component, LPASS_CDC_RX_TOP_TOP_CFG1, 0x01, 0x00);
...
static int
lpass_cdc_rx_macro_fir_filter_info(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_info *ucontrol)
{
...
static int
lpass_cdc_rx_macro_fir_audio_mixer_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	memcpy(ucontrol->value.bytes.data, &readArray[0],
	       sizeof(readArray[0]) * copy_size);
				...
				unsigned int path_idx)
{
	...
	case RX1_PATH:
		...
		break;
	default:
		dev_err_ratelimited(rx_priv->dev, "%s: inavlid FIR ID: %d\n",
				    __func__, path_idx);
		ret = -EINVAL;
		goto exit;
	}
	...
	/* Enable HIFI_FEAT_EN bit */
	snd_soc_component_update_bits(component, LPASS_CDC_RX_TOP_TOP_CFG1,
				      0x01, 0x01);
	...
	for (grp_idx = 0; grp_idx < GRP_MAX; grp_idx++) {
		...
		for (coeff_idx = coeff_idx_start;
			...
			u32 value = rx_priv->fir_coeff_array[path_idx][grp_idx]
							    [array_idx++];
			dev_dbg(rx_priv->dev,
				"%s: val of coeff_idx:%d, COEFF:0x%x\n",
			...
			value = rx_priv->fir_coeff_array[path_idx][grp_idx]
							[array_idx++];
			dev_dbg(rx_priv->dev,
				"%s: val of coeff_idx:%d, COEFF:0x%x\n",
				__func__, coeff_idx, value);
			...
			snd_soc_component_write(component, coeff_addr,
						addr_offset);
			snd_soc_component_update_bits(component, fir_ctl_addr,
						      0x02, 0x02);
			usleep_range(13, 15);
			snd_soc_component_update_bits(component, fir_ctl_addr,
						      0x02, 0x00);
		...
		dev_dbg(component->dev,
			"%s: HIFI FIR Path:%d Group:%d coefficients"
			" updated.\n",
...
static int
lpass_cdc_rx_macro_fir_audio_mixer_put(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	dev_dbg(rx_priv->dev,
		"%s: bytes.data: path:%d, grp:%d, num_coeff_grp:%d\n", __func__,
		path_idx, grp_idx, num_coeff_grp);
	...
	memcpy(&coeff[1], &(ucontrol->value.bytes.data[ele_size]),
	       ele_size * num_coeff_grp);
...
static int
lpass_cdc_rx_macro_fir_coeff_num_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int
lpass_cdc_rx_macro_fir_coeff_num_put(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static const struct snd_kcontrol_new lpass_cdc_rx_macro_snd_controls[] = {
	SOC_SINGLE_S8_TLV("RX_RX0 Digital Volume", LPASS_CDC_RX_RX0_RX_VOL_CTL,
			  -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX1 Digital Volume", LPASS_CDC_RX_RX1_RX_VOL_CTL,
			  -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX2 Digital Volume", LPASS_CDC_RX_RX2_RX_VOL_CTL,
			  -84, 40, digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX0 Mix Digital Volume",
			  LPASS_CDC_RX_RX0_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX1 Mix Digital Volume",
			  LPASS_CDC_RX_RX1_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),
	SOC_SINGLE_S8_TLV("RX_RX2 Mix Digital Volume",
			  LPASS_CDC_RX_RX2_RX_VOL_MIX_CTL, -84, 40,
			  digital_gain),

	SOC_SINGLE_EXT("RX_COMP1 Switch", SND_SOC_NOPM,
		       LPASS_CDC_RX_MACRO_COMP1, 1, 0,
		       lpass_cdc_rx_macro_get_compander,
		       lpass_cdc_rx_macro_set_compander),
	SOC_SINGLE_EXT("RX_COMP2 Switch", SND_SOC_NOPM,
		       LPASS_CDC_RX_MACRO_COMP2, 1, 0,
		       lpass_cdc_rx_macro_get_compander,
		       lpass_cdc_rx_macro_set_compander),

	SOC_SINGLE_EXT("RX_HPH PCM", SND_SOC_NOPM, 0, 1, 0,
		       lpass_cdc_rx_macro_get_pcm_path,
	...
	SOC_SINGLE_EXT("RX0 FIR Coeff Num", SND_SOC_NOPM, RX0_PATH,
		       ...
		       lpass_cdc_rx_macro_fir_coeff_num_get,
	...
	SOC_ENUM_EXT("HPH Idle Detect", hph_idle_detect_enum,
		     lpass_cdc_rx_macro_hph_idle_detect_get,
		     lpass_cdc_rx_macro_hph_idle_detect_put),

	SOC_ENUM_EXT("RX_EAR Mode", lpass_cdc_rx_macro_ear_mode_enum,
		     lpass_cdc_rx_macro_get_ear_mode,
		     lpass_cdc_rx_macro_put_ear_mode),
	...
	SOC_ENUM_EXT("RX_HPH_PWR_MODE", lpass_cdc_rx_macro_hph_pwr_mode_enum,
		     lpass_cdc_rx_macro_get_hph_pwr_mode,
		     lpass_cdc_rx_macro_put_hph_pwr_mode),

	SOC_ENUM_EXT("RX_GSM mode Enable",
		     lpass_cdc_rx_macro_vbat_bcl_gsm_mode_enum,
	...
	LPASS_CDC_RX_MACRO_FIR_FILTER_CTL("RX0 FIR Coeff Group0", RX0_PATH,
					  GRP0),
	LPASS_CDC_RX_MACRO_FIR_FILTER_CTL("RX0 FIR Coeff Group1", RX0_PATH,
					  GRP1),
	LPASS_CDC_RX_MACRO_FIR_FILTER_CTL("RX1 FIR Coeff Group0", RX1_PATH,
					  GRP0),
	LPASS_CDC_RX_MACRO_FIR_FILTER_CTL("RX1 FIR Coeff Group1", RX1_PATH,
					  GRP1),
};
					  ...
					  int event)
{
	...
	if (ec_tx < 0 || (ec_tx >= LPASS_CDC_RX_MACRO_EC_MUX_MAX)) {
		dev_err_ratelimited(rx_dev,
				    "%s: EC mix control not set correctly\n",
				    __func__);
		return -EINVAL;
	}
	ec_hq_reg = LPASS_CDC_RX_EC_REF_HQ0_EC_REF_HQ_PATH_CTL + 0x40 * ec_tx;
	snd_soc_component_update_bits(component, ec_hq_reg, 0x01, 0x01);
	ec_hq_reg = LPASS_CDC_RX_EC_REF_HQ0_EC_REF_HQ_CFG0 + 0x40 * ec_tx;
	/* default set to 48k */
...
static const struct snd_soc_dapm_widget lpass_cdc_rx_macro_dapm_widgets[] = {
	...
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX_MACRO RX0 MUX", LPASS_CDC_RX_MACRO_RX0,
				    lpass_cdc_rx_macro_rx0),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX_MACRO RX1 MUX", LPASS_CDC_RX_MACRO_RX1,
				    lpass_cdc_rx_macro_rx1),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX_MACRO RX2 MUX", LPASS_CDC_RX_MACRO_RX2,
				    lpass_cdc_rx_macro_rx2),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX_MACRO RX3 MUX", LPASS_CDC_RX_MACRO_RX3,
				    lpass_cdc_rx_macro_rx3),
	...
	SND_SOC_DAPM_MUX_E("RX MIX TX0 MUX", SND_SOC_NOPM,
			   LPASS_CDC_RX_MACRO_EC0_MUX, 0, &rx_mix_tx0_mux,
			   lpass_cdc_rx_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX MIX TX1 MUX", SND_SOC_NOPM,
			   LPASS_CDC_RX_MACRO_EC1_MUX, 0, &rx_mix_tx1_mux,
			   lpass_cdc_rx_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX MIX TX2 MUX", SND_SOC_NOPM,
			   LPASS_CDC_RX_MACRO_EC2_MUX, 0, &rx_mix_tx2_mux,
			   lpass_cdc_rx_macro_enable_echo,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MIXER_E("IIR0", LPASS_CDC_RX_SIDETONE_IIR0_IIR_PATH_CTL, 4,
			     0, NULL, 0, lpass_cdc_rx_macro_set_iir_gain,
			     SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_MIXER_E("IIR1", LPASS_CDC_RX_SIDETONE_IIR1_IIR_PATH_CTL, 4,
			     0, NULL, 0, lpass_cdc_rx_macro_set_iir_gain,
			     SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	...
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT0_1 MIX1 INP0", 0,
				    rx_int0_1_mix_inp0),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT0_1 MIX1 INP1", 0,
				    rx_int0_1_mix_inp1),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT0_1 MIX1 INP2", 0,
				    rx_int0_1_mix_inp2),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT1_1 MIX1 INP0", 0,
				    rx_int1_1_mix_inp0),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT1_1 MIX1 INP1", 0,
				    rx_int1_1_mix_inp1),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT1_1 MIX1 INP2", 0,
				    rx_int1_1_mix_inp2),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT2_1 MIX1 INP0", 0,
				    rx_int2_1_mix_inp0),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT2_1 MIX1 INP1", 0,
				    rx_int2_1_mix_inp1),
	LPASS_CDC_RX_MACRO_DAPM_MUX("RX INT2_1 MIX1 INP2", 0,
				    rx_int2_1_mix_inp2),
	...
	SND_SOC_DAPM_MUX_E("RX INT0 MIX2 INP", SND_SOC_NOPM, INTERP_HPHL, 0,
			   &rx_int0_mix2_inp_mux,
			   lpass_cdc_rx_macro_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX INT1 MIX2 INP", SND_SOC_NOPM, INTERP_HPHR, 0,
			   &rx_int1_mix2_inp_mux,
			   lpass_cdc_rx_macro_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("RX INT2 MIX2 INP", SND_SOC_NOPM, INTERP_AUX, 0,
			   &rx_int2_mix2_inp_mux,
			   lpass_cdc_rx_macro_enable_rx_path_clk,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MIXER_E("RX INT2_1 VBAT", SND_SOC_NOPM, 0, 0,
			     rx_int2_1_vbat_mix_switch,
...
static int lpass_cdc_rx_macro_core_vote(void *handle, bool enable)
{
	int rc = 0;
	struct lpass_cdc_rx_macro_priv *rx_priv =
		(struct lpass_cdc_rx_macro_priv *)handle;
		...
		if (rx_priv->swr_clk_users == 0) {
			...
			if (rx_priv->reset_swr)
				regmap_update_bits(
					...
					0x02, 0x02);
			regmap_update_bits(
				regmap, LPASS_CDC_RX_CLK_RST_CTRL_SWR_CONTROL,
				0x01, 0x01);
			if (rx_priv->reset_swr)
				regmap_update_bits(
					regmap,
...
static int lpass_cdc_rx_macro_init(struct snd_soc_component *component)
{
	...
	rx_dev = lpass_cdc_get_device_ptr(component->dev, RX_MACRO);
	if (!rx_dev) {
		dev_err(component->dev, "%s: null device for macro!\n",
			__func__);
		return -EINVAL;
	}
	rx_priv = dev_get_drvdata(rx_dev);
	if (!rx_priv) {
		dev_err(component->dev, "%s: priv is null for macro!\n",
			__func__);
		return -EINVAL;
	}

	ret = snd_soc_dapm_new_controls(
		dapm, lpass_cdc_rx_macro_dapm_widgets,
	...
	ret = snd_soc_add_component_controls(
		component, lpass_cdc_rx_macro_snd_controls,
	...
	for (i = 0; i < ARRAY_SIZE(lpass_cdc_rx_macro_reg_init); i++)
		snd_soc_component_update_bits(
			component, lpass_cdc_rx_macro_reg_init[i].reg,
...
static void lpass_cdc_rx_macro_add_child_devices(struct work_struct *work)
{
	...
	if (!rx_priv->dev->of_node) {
		dev_err(rx_priv->dev, "%s: DT node for RX dev does not exist\n",
			__func__);
		return;
	}
	...
	for_each_available_child_of_node(rx_priv->dev->of_node, node) {
		...
		if (rx_swr_master_node) {
			...
			temp = krealloc(swr_ctrl_data,
					(ctrl_num + 1) *
						sizeof(struct rx_swr_ctrl_data),
					GFP_KERNEL);
			if (!temp) {
				ret = -ENOMEM;
				goto fail_pdev_add;
...
static void lpass_cdc_rx_macro_init_ops(struct macro_ops *ops,
					char __iomem *rx_io_base)
{
...
static int lpass_cdc_rx_macro_probe(struct platform_device *pdev)
{
	...
	if (!lpass_cdc_is_va_macro_registered(&pdev->dev)) {
		dev_err(&pdev->dev, "%s: va-macro not registered yet, defer\n",
			__func__);
		return -EPROBE_DEFER;
	}

	rx_priv = devm_kzalloc(
		&pdev->dev, sizeof(struct lpass_cdc_rx_macro_priv), GFP_KERNEL);
	if (!rx_priv)
		return -ENOMEM;

	rx_priv->pre_dev_up = true;
	rx_priv->dev = &pdev->dev;
	ret = of_property_read_u32(pdev->dev.of_node, "reg", &rx_base_addr);
	if (ret) {
	...
	if (of_find_property(pdev->dev.of_node, is_used_rx_swr_gpio_dt, NULL)) {
		ret = of_property_read_u32(pdev->dev.of_node,
	...
	rx_priv->rx_swr_gpio_p =
		of_parse_phandle(pdev->dev.of_node, "qcom,rx-swr-gpios", 0);
	...
	msm_cdc_pinctrl_set_wakeup_capable(rx_priv->rx_swr_gpio_p, false);
...
static int lpass_cdc_rx_macro_remove(struct platform_device *pdev)
{
	...
	for (count = 0; count < rx_priv->child_count &&
			count < LPASS_CDC_RX_MACRO_CHILD_DEVICES_MAX;
	     count++)
...
static const struct dev_pm_ops lpass_cdc_dev_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
		...
		SET_RUNTIME_PM_OPS(lpass_cdc_runtime_suspend,
				   lpass_cdc_runtime_resume, NULL)
};
