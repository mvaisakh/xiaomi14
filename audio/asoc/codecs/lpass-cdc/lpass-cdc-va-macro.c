...
struct lpass_cdc_va_macro_priv {
	...
	struct platform_device
		*pdev_child_devices[LPASS_CDC_VA_MACRO_CHILD_DEVICES_MAX];
...
static int
lpass_cdc_va_macro_wake_enable(struct lpass_cdc_va_macro_priv *va_priv,
			       bool wake_enable)
{
	int ret = 0;
...
static bool lpass_cdc_va_macro_get_data(
	struct snd_soc_component *component, struct device **va_dev,
	struct lpass_cdc_va_macro_priv **va_priv, const char *func_name)
{
...
static int lpass_cdc_va_macro_clk_div_get(struct snd_soc_component *component)
{
	...
	if (!lpass_cdc_va_macro_get_data(component, &va_dev, &va_priv,
					 __func__))
		return -EINVAL;
...
static int
lpass_cdc_va_macro_mclk_enable(struct lpass_cdc_va_macro_priv *va_priv,
			       bool mclk_enable, bool dapm)
{
	...
	if (mclk_enable) {
		...
		lpass_cdc_clk_rsc_fs_gen_request(va_priv->dev, true);
		...
		va_priv->va_mclk_users--;
		lpass_cdc_clk_rsc_fs_gen_request(va_priv->dev, false);
		ret = lpass_cdc_va_macro_core_vote(va_priv, true);
		if (ret < 0) {
			dev_err_ratelimited(va_priv->dev,
					    "%s: va request core vote failed\n",
					    ...
					    int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		break;
	default:
		dev_err_ratelimited(va_priv->dev, "%s: invalid DAPM event %d\n",
				    __func__, event);
		ret = -EINVAL;
	}
	return ret;
					 ...
					 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!va_priv->use_lpi_mixer_control) {
			...
			if (va_priv->lpi_enable)
				ret = lpass_cdc_va_macro_mclk_enable(va_priv, 1,
								     true);
			else
	...
	case SND_SOC_DAPM_POST_PMD:
		if (!va_priv->use_lpi_mixer_control) {
			...
			if (va_priv->lpi_enable)
				lpass_cdc_va_macro_mclk_enable(va_priv, 0,
							       true);
			else
...
static int
lpass_cdc_va_macro_tx_va_mclk_enable(struct lpass_cdc_va_macro_priv *va_priv,
				     ...
				     bool enable)
{
		...
		if (va_priv->swr_clk_users == 0) {
			...
			if (va_priv->reset_swr)
				regmap_update_bits(
					...
					0x02, 0x02);
			regmap_update_bits(
				regmap, LPASS_CDC_VA_CLK_RST_CTRL_SWR_CONTROL,
				0x01, 0x01);
			if (va_priv->reset_swr)
				regmap_update_bits(
					regmap,
	...
	if (!clk_tx_ret)
		lpass_cdc_clk_rsc_request_clock(va_priv->dev, TX_CORE_CLK,
						TX_CORE_CLK, false);
	return ret;
}
...
static void
lpass_cdc_va_macro_tx_hpf_corner_freq_callback(struct work_struct *work)
{
					  ...
					  int event, u16 adc_mux0_cfg)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		lpass_cdc_dmic_clk_enable(component, (u32)dmic, (u32)DMIC_VA,
					  true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		lpass_cdc_dmic_clk_enable(component, (u32)dmic, (u32)DMIC_VA,
					 ...
					 int event)
{
	...
	if (!is_amic_enabled(component, decimator))
		lpass_cdc_va_macro_enable_dmic(w, kcontrol, event,
					       adc_mux0_reg);
					...
					struct snd_soc_dai *dai)
{
	...
	for_each_set_bit(decimator, &va_priv->active_ch_mask[dai->id],
		...
		if (decimator >= 0) {
			...
			dev_err_ratelimited(
				va_dev, "%s: ERROR: Invalid decimator: %d\n",
				__func__, decimator);
			return -EINVAL;
		}
	}
...
#define LPASS_CDC_VA_MACRO_DAPM_ENUM_EXT(name, reg, offset, text, getname, \
					 putname)                          \
	static SOC_ENUM_SINGLE_DECL(name##_enum, reg, offset, text);       \
	static const struct snd_kcontrol_new name##_mux =                  \
...
LPASS_CDC_VA_MACRO_DAPM_ENUM(va_dec0, LPASS_CDC_VA_INP_MUX_ADC_MUX0_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_VA_MACRO_DAPM_ENUM(va_dec1, LPASS_CDC_VA_INP_MUX_ADC_MUX1_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_VA_MACRO_DAPM_ENUM(va_dec2, LPASS_CDC_VA_INP_MUX_ADC_MUX2_CFG1, 0,
			     adc_mux_text);
LPASS_CDC_VA_MACRO_DAPM_ENUM(va_dec3, LPASS_CDC_VA_INP_MUX_ADC_MUX3_CFG1, 0,
			     adc_mux_text);
...
static const struct snd_soc_dapm_widget lpass_cdc_va_macro_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("VA_AIF1_CAP Mixer", SND_SOC_NOPM,
			   LPASS_CDC_VA_MACRO_AIF1_CAP, 0, va_aif1_cap_mixer,
			   ARRAY_SIZE(va_aif1_cap_mixer)),

	SND_SOC_DAPM_MIXER("VA_AIF2_CAP Mixer", SND_SOC_NOPM,
			   LPASS_CDC_VA_MACRO_AIF2_CAP, 0, va_aif2_cap_mixer,
			   ARRAY_SIZE(va_aif2_cap_mixer)),

	SND_SOC_DAPM_MIXER("VA_AIF3_CAP Mixer", SND_SOC_NOPM,
			   LPASS_CDC_VA_MACRO_AIF3_CAP, 0, va_aif3_cap_mixer,
			   ARRAY_SIZE(va_aif3_cap_mixer)),
	...
	SND_SOC_DAPM_MUX_E("VA DEC0 MUX", SND_SOC_NOPM, LPASS_CDC_VA_MACRO_DEC0,
			   0, &va_dec0_mux, lpass_cdc_va_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("VA DEC1 MUX", SND_SOC_NOPM, LPASS_CDC_VA_MACRO_DEC1,
			   0, &va_dec1_mux, lpass_cdc_va_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("VA DEC2 MUX", SND_SOC_NOPM, LPASS_CDC_VA_MACRO_DEC2,
			   0, &va_dec2_mux, lpass_cdc_va_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_MUX_E("VA DEC3 MUX", SND_SOC_NOPM, LPASS_CDC_VA_MACRO_DEC3,
			   0, &va_dec3_mux, lpass_cdc_va_macro_enable_dec,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				   SND_SOC_DAPM_PRE_PMD |
				   SND_SOC_DAPM_POST_PMD),
...
static const struct snd_kcontrol_new lpass_cdc_va_macro_snd_controls[] = {
	SOC_SINGLE_S8_TLV("VA_DEC0 Volume", LPASS_CDC_VA_TX0_TX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("VA_DEC1 Volume", LPASS_CDC_VA_TX1_TX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("VA_DEC2 Volume", LPASS_CDC_VA_TX2_TX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_S8_TLV("VA_DEC3 Volume", LPASS_CDC_VA_TX3_TX_VOL_CTL, -84,
			  40, digital_gain),
	SOC_SINGLE_EXT("LPI Enable", 0, 0, 1, 0, lpass_cdc_va_macro_lpi_get,
...
static int lpass_cdc_va_macro_validate_dmic_sample_rate(
	u32 dmic_sample_rate, struct lpass_cdc_va_macro_priv *va_priv)
{
	u32 div_factor;
...
static int lpass_cdc_va_macro_init(struct snd_soc_component *component)
{
	...
	ret = snd_soc_dapm_new_controls(
		dapm, lpass_cdc_va_macro_dapm_widgets,
	...
	ret = snd_soc_add_component_controls(
		component, lpass_cdc_va_macro_snd_controls,
	...
	snd_soc_component_update_bits(
		component, LPASS_CDC_VA_TOP_CSR_SWR_MIC_CTL0, 0xEE, 0xCC);
	snd_soc_component_update_bits(
		component, LPASS_CDC_VA_TOP_CSR_SWR_MIC_CTL1, 0xEE, 0xCC);
	snd_soc_component_update_bits(
		component, LPASS_CDC_VA_TOP_CSR_SWR_MIC_CTL2, 0xEE, 0xCC);
	...
	for_each_available_child_of_node(va_priv->dev->of_node, node) {
		...
		if (va_swr_master_node) {
			...
			temp = krealloc(
				swr_ctrl_data,
				(ctrl_num +
				 1) * sizeof(struct lpass_cdc_va_macro_swr_ctrl_data),
				GFP_KERNEL);
			if (!temp) {
				ret = -ENOMEM;
				goto fail_pdev_add;
...
static int lpass_cdc_va_macro_probe(struct platform_device *pdev)
{
	...
	va_priv = devm_kzalloc(
		&pdev->dev, sizeof(struct lpass_cdc_va_macro_priv), GFP_KERNEL);
	if (!va_priv)
		return -ENOMEM;
	...
	if (ret) {
		...
		if (lpass_cdc_va_macro_validate_dmic_sample_rate(sample_rate,
								 va_priv) ==
	...
	if (of_find_property(pdev->dev.of_node, "use-lpi-control", NULL)) {
		dev_dbg(&pdev->dev,
			"%s(): Usage of LPI Enable mixer control is enabled\n",
			__func__);
	...
	if (is_used_va_swr_gpio)
		schedule_work(
			&va_priv->lpass_cdc_va_macro_add_child_devices_work);
	return ret;
...
static int lpass_cdc_va_macro_remove(struct platform_device *pdev)
{
	...
	if (va_priv->is_used_va_swr_gpio) {
		...
		for (count = 0; count < va_priv->child_count &&
				count < LPASS_CDC_VA_MACRO_CHILD_DEVICES_MAX;
		     count++)
