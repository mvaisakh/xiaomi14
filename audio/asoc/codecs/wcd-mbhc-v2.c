...
#include <sound/soc.h>
	...
	defined(CONFIG_TARGET_PRODUCT_MANET) || \
...
static const unsigned int mbhc_ext_dev_supported_table[] = {
	...
	EXTCON_MECHANICAL,	EXTCON_NONE,
};

struct mutex hphl_pa_lock;
struct mutex hphr_pa_lock;

void wcd_mbhc_jack_report(struct wcd_mbhc *mbhc, struct snd_soc_jack *jack,
			  int status, int mask)
{
		...
		case SND_JACK_BTN_1:
			if (!jackstatus) {
				...
				headset_status[1] =
					headset_status[1] |
					(headset_key_order
					 << HEADSET_EVENT_KEY_PREVIOUS_ORDER);
			} else {
				headset_status[1] =
					maxF(headset_status[1],
				   ...
				   size_t count, loff_t *ppos)
{
	...
	sprintf(buf, "0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x\n",
		headset_status[0], headset_status[1], headset_status[2],
		headset_status[3], headset_status[4], headset_status[5]);
...
static void __hphocp_off_report(struct wcd_mbhc *mbhc, u32 jack_status, int irq)
{
			    ...
			    void *data)
{
	...
	if (mbhc->mbhc_cb->micbias_enable_status) {
		micbias2 =
			mbhc->mbhc_cb->micbias_enable_status(mbhc, MIC_BIAS_2);
		micbias1 =
	...
	case WCD_EVENT_POST_MICBIAS_2_ON:
		...
		if (mbhc->mbhc_cb->mbhc_common_micb_ctrl) {
			mbhc->mbhc_cb->mbhc_common_micb_ctrl(
				component, MBHC_COMMON_MICB_PRECHARGE, true);
			mbhc->mbhc_cb->mbhc_common_micb_ctrl(
				component, MBHC_COMMON_MICB_SET_VAL, true);
			/*
       ...
       */
		...
		if (mbhc->mbhc_cb->mbhc_common_micb_ctrl)
			mbhc->mbhc_cb->mbhc_common_micb_ctrl(
				component, MBHC_COMMON_MICB_PRECHARGE, false);
out_micb_en:
	...
	case WCD_EVENT_OCP_OFF:
		mbhc->mbhc_cb->irq_control(mbhc->component,
					   mbhc->intr_ids->hph_left_ocp, false);
		break;
	case WCD_EVENT_OCP_ON:
		mbhc->mbhc_cb->irq_control(mbhc->component,
					   mbhc->intr_ids->hph_left_ocp, true);
		break;
	default:
		break;
	}
...
static void wcd_mbhc_clr_and_turnon_hph_padac(struct wcd_mbhc *mbhc)
{
	...
	if (test_and_clear_bit(WCD_MBHC_ANC0_OFF_ACK, &mbhc->hph_anc_state)) {
		usleep_range(20000, 20100);
		pr_debug("%s: HPHL ANC clear flag and enable ANC_EN\n",
			 __func__);
		if (mbhc->mbhc_cb->update_anc_state)
			mbhc->mbhc_cb->update_anc_state(mbhc->component, true,
							0);
	}

	if (test_and_clear_bit(WCD_MBHC_ANC1_OFF_ACK, &mbhc->hph_anc_state)) {
		usleep_range(20000, 20100);
...
static void wcd_mbhc_set_and_turnoff_hph_padac(struct wcd_mbhc *mbhc)
{
   ...
   */
	...
	if (mbhc->mbhc_cb->is_anc_on && mbhc->mbhc_cb->is_anc_on(mbhc)) {
		...
		if (mbhc->mbhc_cb->update_anc_state) {
			mbhc->mbhc_cb->update_anc_state(mbhc->component, false,
							0);
			mbhc->mbhc_cb->update_anc_state(mbhc->component, false,
							1);
		} else {
     ...
     */
		if (mbhc->mbhc_cfg->detect_extn_cable &&
			...
			if (!mbhc->force_linein) {
				...
				extcon_set_state_sync(mbhc->extdev, extdev_type,
						      0);
			}
				   ...
				   enum wcd_mbhc_plug_type plug_type)
{
     ...
     */
		...
		ret = extcon_set_state_sync(mbhc->extdev, EXTCON_JACK_HEADPHONE,
					    1);
	} else if (plug_type == MBHC_PLUG_TYPE_GND_MIC_SWAP) {
		if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
			wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADPHONE);
			ret = extcon_set_state_sync(mbhc->extdev,
     ...
     */
		...
		if (mbhc->mbhc_cfg->detect_extn_cable) {
			...
			wcd_mbhc_report_plug(mbhc, 1, SND_JACK_LINEOUT);
			ret = extcon_set_state_sync(mbhc->extdev,
						    EXTCON_JACK_LINE_OUT, 1);
			pr_debug(
				"%s: setup mic trigger for further detection\n",
				__func__);
...
static void wcd_mbhc_set_hsj_connect(struct wcd_mbhc *mbhc, bool connect)
{
	...
	if (connect) {
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->zdet_leakage_resistance)
			mbhc->mbhc_cb->zdet_leakage_resistance(
				mbhc, false); /* enable 1M pull-up */
...
static void wcd_btn_lpress_fn(struct work_struct *work)
{
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		pr_debug(
			"%s: Reporting long button press event, btn_result: %d\n",
			__func__, btn_result);
...
static irqreturn_t wcd_mbhc_hphr_ocp_irq(int irq, void *data)
{
	...
	if (mbhc->hphrocp_cnt < OCP_ATTEMPT) {
		...
		mbhc->mbhc_cb->irq_control(
			mbhc->component, mbhc->intr_ids->hph_right_ocp, false);
#if IS_ENABLED(CONFIG_AUDIO_QGKI)
...
static void wcd_mbhc_fw_read(struct work_struct *work)
{
	...
	while (retry < FW_READ_ATTEMPTS) {
		...
		if (mbhc->mbhc_cb->get_hwdep_fw_cal)
			fw_data = mbhc->mbhc_cb->get_hwdep_fw_cal(
				mbhc, WCD9XXX_MBHC_CAL);
		if (!fw_data)
...
static int wcd_mbhc_set_keycode(struct wcd_mbhc *mbhc)
{
	...
	for (i = 0; i < WCD_MBHC_KEYCODE_NUM; i++) {
		if (btn_key_code[i] != 0) {
			...
			ret = snd_jack_set_key(mbhc->button_jack.jack, type,
					       btn_key_code[i]);
			if (ret) {
					   ...
					   unsigned long mode, void *ptr)
{
	...
	if (mode == TYPEC_ACCESSORY_AUDIO) {
		...
		if ((mode == TYPEC_ACCESSORY_NONE) && !detection_type) {
			if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) ==
				     false))
				pr_warn("%s: failed to hold suspend\n",
					__func__);
			else {
...
int wcd_mbhc_start(struct wcd_mbhc *mbhc, struct wcd_mbhc_config *mbhc_cfg)
{
	...
	if (mbhc_cfg->enable_usbc_analog == 0 || rc != 0) {
		dev_dbg(card->dev, "%s: %s in dt node is missing or false\n",
			__func__, usb_c_dt);
		dev_dbg(card->dev, "%s: skipping USB c analog configuration\n",
			__func__);
	}
	...
	if (mbhc_cfg->enable_usbc_analog) {
		...
		if (IS_ENABLED(CONFIG_QCOM_WCD_USBSS_I2C))
			mbhc->aatc_dev_np = of_parse_phandle(
				card->dev->of_node, "wcd939x-i2c-handle", 0);
		else if (IS_ENABLED(CONFIG_QCOM_FSA4480_I2C))
			mbhc->aatc_dev_np = of_parse_phandle(
				card->dev->of_node, "fsa4480-i2c-handle", 0);
		if (!mbhc->aatc_dev_np) {
			dev_err(card->dev,
				"%s: wcd939x or fsa i2c node not found\n",
				__func__);
			rc = -EINVAL;
			goto err;
		}
	...
	if (mbhc_cfg->enable_usbc_analog) {
		mbhc->aatc_dev_nb.notifier_call =
			wcd_mbhc_usbc_ana_event_handler;
...
#endif
#if IS_ENABLED(CONFIG_QCOM_FSA4480_I2C)
		rc = fsa4480_reg_notifier(&mbhc->aatc_dev_nb,
					  mbhc->aatc_dev_np);
#endif
	}
		  ...
		  bool impedance_det_en)
{
	...
	if (ret) {
		dev_err(card->dev, "%s: missing %s in dt node\n", __func__,
			hph_switch);
		goto err;
	}
	...
	ret = of_property_read_u32(card->dev->of_node, hs_thre,
				   &(mbhc->hs_thr));
	if (ret)
		dev_dbg(card->dev, "%s: missing %s in dt node\n", __func__,
			hs_thre);
	...
	if (!mbhc_cb || !mbhc_cb->request_irq || !mbhc_cb->irq_control ||
		...
		dev_err(component->dev,
			"%s: required mbhc callbacks are not defined\n",
			__func__);
		return -EINVAL;
	}
	...
	if (mbhc->headset_jack.jack == NULL) {
		...
		ret = snd_soc_card_jack_new(component->card, "Button Jack",
		...
		ret = snd_jack_set_key(mbhc->button_jack.jack, SND_JACK_BTN_0,
				       KEY_MEDIA);
		if (ret) {
		...
		INIT_DELAYED_WORK(&mbhc->mbhc_firmware_dwork, wcd_mbhc_fw_read);
	...
	default:
		pr_err("%s: Unknown detection logic type %d\n", __func__,
		       mbhc->mbhc_detection_logic);
		break;
	}

	if (!mbhc->mbhc_fn || !mbhc->mbhc_fn->wcd_mbhc_hs_ins_irq ||
	...
	if (!mbhc->extdev) {
		mbhc->extdev = devm_extcon_dev_allocate(
			component->dev, mbhc_ext_dev_supported_table);
...
err_mbhc_hs_ins_irq:
	mbhc->mbhc_cb->free_irq(component,
				mbhc->intr_ids->mbhc_btn_release_intr, mbhc);
err_btn_release_irq:
