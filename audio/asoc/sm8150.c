...
static u32 mi2s_ebit_clk[MI2S_MAX] = { Q6AFE_LPASS_CLK_ID_PRI_MI2S_EBIT,
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("BT SampleRate RX", bt_sample_rate_rx,
		     msm_bt_sample_rate_rx_get, msm_bt_sample_rate_rx_put),
	SOC_ENUM_EXT("BT SampleRate TX", bt_sample_rate_tx,
		     msm_bt_sample_rate_tx_get, msm_bt_sample_rate_tx_put),
	...
	SOC_ENUM_EXT("Display Port RX SampleRate", ext_disp_rx_sample_rate,
		     ext_disp_rx_sample_rate_get, ext_disp_rx_sample_rate_put),
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
   ...
   * TX14, TX15, TX16
   */
	unsigned int rx_ch[WCD934X_RX_MAX] = { 144, 145, 146, 147,
					       148, 149, 150, 151 };
				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		ret = snd_soc_dai_set_channel_map(cpu_dai, 0, 0, num_rx_ch,
						  rx_ch);
		if (ret < 0) {
			pr_err("%s: failed to set cpu chan map, err:%d\n",
			       __func__, ret);
		...
		ret = snd_soc_dai_set_channel_map(cpu_dai, num_tx_ch, tx_ch, 0,
						  0);
		if (ret < 0) {
			pr_err("%s: failed to set tx cpu chan map, err:%d\n",
			       __func__, ret);
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (--mi2s_intf_conf[index].ref_cnt == 0) {
		...
		if (index == QUAT_MI2S) {
			ret_pinctrl =
				msm_set_pinctrl(pinctrl_info, STATE_DISABLE);
			if (ret_pinctrl)
				pr_err("%s: MI2S TLMM pinctrl set failed with %d\n",
				       __func__, ret_pinctrl);
		}
...
static struct snd_soc_dai_link msm_tavil_dai_links
	[ARRAY_SIZE(msm_common_dai_links) + ARRAY_SIZE(msm_tavil_fe_dai_links) +
	 ARRAY_SIZE(msm_common_misc_fe_dai_links) +
	 ARRAY_SIZE(msm_common_be_dai_links) +
	 ARRAY_SIZE(msm_tavil_be_dai_links) + ARRAY_SIZE(msm_wcn_be_dai_links) +
	 ARRAY_SIZE(ext_disp_be_dai_link) + ARRAY_SIZE(msm_mi2s_be_dai_links) +
				  ...
				  struct snd_pcm_hw_params *params)
{
	...
	int ret = 0;
	unsigned int rx_ch[] = { 144, 145, 146, 147, 148, 149, 150, 151 };
	unsigned int tx_ch[] = { 128, 129, 130, 131, 132, 133, 134, 135,
				 136, 137, 138, 139, 140, 141, 142, 143 };

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = snd_soc_dai_set_channel_map(
			cpu_dai, 0, 0, slim_rx_cfg[SLIM_RX_0].channels, rx_ch);
		if (ret < 0)
			pr_err("%s: RX failed to set cpu chan map error %d\n",
			       __func__, ret);
	} else {
		ret = snd_soc_dai_set_channel_map(
			cpu_dai, slim_tx_cfg[SLIM_TX_0].channels, tx_ch, 0, 0);
		if (ret < 0)
...
static const struct of_device_id sm8150_asoc_machine_of_match[] = {
	{ .compatible = "qcom,sm8150-asoc-snd-tavil", .data = "tavil_codec" },
	{ .compatible = "qcom,sm8150-asoc-snd-stub", .data = "stub_codec" },
	{},
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "tavil_codec")) {
		...
		memcpy(msm_tavil_dai_links, msm_common_dai_links,
		       sizeof(msm_common_dai_links));
		memcpy(msm_tavil_dai_links + len_1, msm_tavil_fe_dai_links,
		       sizeof(msm_tavil_fe_dai_links));
		memcpy(msm_tavil_dai_links + len_2,
		       msm_common_misc_fe_dai_links,
		       sizeof(msm_common_misc_fe_dai_links));
		memcpy(msm_tavil_dai_links + len_3, msm_common_be_dai_links,
		       sizeof(msm_common_be_dai_links));
		memcpy(msm_tavil_dai_links + len_4, msm_tavil_be_dai_links,
		...
		ret = of_property_read_u32(dev->of_node, "qcom,mi2s-audio-intf",
					   &val);
		if (!ret && val) {
			memcpy(msm_tavil_dai_links + total_links,
		...
		memcpy(msm_stub_dai_links, msm_stub_fe_dai_links,
		       sizeof(msm_stub_fe_dai_links));
		memcpy(msm_stub_dai_links + len_1, msm_stub_be_dai_links,
...
static int msm_wsa881x_init(struct snd_soc_component *component)
{
	...
	if (!strcmp(component->name_prefix, "SpkrLeft")) {
		...
		dev_dbg(codec->dev,
			"%s: setting right ch map to component %s\n", __func__,
			component->name);
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		pdata->hph_en1_gpio_p = of_parse_phandle(
			pdev->dev.of_node, "qcom,hph-en1-gpio", 0);
		...
		pdata->hph_en0_gpio_p = of_parse_phandle(
			pdev->dev.of_node, "qcom,hph-en0-gpio", 0);
   ...
   */
	pdata->us_euro_gpio_p =
		of_parse_phandle(pdev->dev.of_node, "qcom,us-euro-gpios", 0);
	if (!pdata->us_euro_gpio_p) {
	...
	pdata->fsa_handle =
		of_parse_phandle(pdev->dev.of_node, "fsa4480-i2c-handle", 0);
	if (!pdata->fsa_handle)
		dev_dbg(&pdev->dev, "property %s not detected in node %s\n",
