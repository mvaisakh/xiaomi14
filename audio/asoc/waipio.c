...
static void msm_set_upd_config(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!strcmp(pdata->upd_config.backend_used, "wsa")) {
		...
		if (!component) {
			component =
				snd_soc_rtdcom_lookup(rtd, WCD937X_DRV_NAME);
			if (!component) {
...
static const struct of_device_id waipio_asoc_machine_of_match[] = {
	{ .compatible = "qcom,waipio-asoc-snd", .data = "codec" },
	{ .compatible = "qcom,waipio-asoc-snd-stub", .data = "stub_codec" },
	{},
};
...
static int msm_audio_ssr_register(struct device *dev)
{
	...
	ret = snd_event_master_register(dev, &waipio_ssr_ops, ssr_clients,
					NULL);
	if (!ret)
