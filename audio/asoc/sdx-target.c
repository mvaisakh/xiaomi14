...
static struct snd_soc_dai_link
	msm_sdx_dai_links[ARRAY_SIZE(msm_rx_tx_cdc_dma_be_dai_links) +
...
static const struct of_device_id sdx_asoc_machine_of_match[] = {
	{ .compatible = "qcom,sdx-asoc-snd-tavil", .data = "codec" },
	{},
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "codec")) {
		...
		memcpy(msm_sdx_dai_links + total_links, msm_common_be_dai_links,
...
static int msm_int_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	u8 spkright_port_types[WSA881X_MAX_SWR_PORTS] = { SPKR_R, SPKR_R_COMP,
							  SPKR_R_BOOST,
							  SPKR_R_VI };
	unsigned int ch_rate[WSA881X_MAX_SWR_PORTS] = { SWR_CLK_RATE_2P4MHZ,
							SWR_CLK_RATE_0P6MHZ,
							SWR_CLK_RATE_0P3MHZ,
							SWR_CLK_RATE_1P2MHZ };
...
static int msm_audio_ssr_register(struct device *dev)
{
	...
	ret = snd_event_master_register(dev, &sdx_ssr_ops, ssr_clients, NULL);
	if (!ret)
