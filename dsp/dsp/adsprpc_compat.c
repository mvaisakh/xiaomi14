...
static int compat_get_fastrpc_ioctl_invoke(
	...
	struct fastrpc_ioctl_invoke_async *inv, unsigned int cmd,
	unsigned int sc)
{
	compat_uint_t u = 0;
	compat_size_t s;
	...
	if ((cmd == COMPAT_FASTRPC_IOCTL_INVOKE_PERF) ||
	    (cmd == FASTRPC_INVOKE2_ASYNC)) {
...
static int compat_fastrpc_ioctl_invoke(struct file *filp, unsigned int cmd,
				       unsigned long arg)
{
	struct compat_fastrpc_ioctl_invoke_async __user *inv32;
	...
	len = REMOTE_SCALARS_LENGTH(sc);
	VERIFY(err,
	       NULL != (inv = kmalloc(sizeof(*inv) +
					      len * sizeof(union remote_arg),
				      GFP_KERNEL)));
	if (err)
		return -EFAULT;
	VERIFY(err, 0 == compat_get_fastrpc_ioctl_invoke(inv32, inv, cmd, sc));
	if (err)
		return err;
	VERIFY(err, 0 == (err = fastrpc_internal_invoke(fl, fl->mode, USER_MSG,
							inv)));
	return err;
}
...
static int compat_get_fastrpc_ioctl_invoke2(
	...
	struct fastrpc_ioctl_invoke2 **inva, unsigned int cmd)
{
	int err = 0;
	...
	case FASTRPC_INVOKE2_ASYNC: {
		...
		struct compat_fastrpc_ioctl_invoke_async_no_perf __user
			*lasync32_old;
		compat_uint_t sc = 0;
		int len = 0;
		...
		len = REMOTE_SCALARS_LENGTH(sc);
		VERIFY(err,
		       NULL != (inv2_user = kmalloc(
					sizeof(*inv2_user) +
						sizeof(*asyncinv_user) +
						len * sizeof(union remote_arg),
					GFP_KERNEL)));
		if (err) {
			err = -EFAULT;
		...
		if (size < sizeof(struct compat_fastrpc_ioctl_invoke_async)) {
			...
			VERIFY(err,
			       0 == compat_get_fastrpc_ioctl_invoke(
					    lasync32, asyncinv_user,
					    COMPAT_FASTRPC_IOCTL_INVOKE_CRC,
					    sc));
			if (err)
				goto bail;
	...
	case FASTRPC_INVOKE2_STATUS_NOTIF: {
		if (!ref_size) {
			ref_size =
				sizeof(struct compat_fastrpc_ioctl_notif_rsp);
			VERIFY(err, size == ref_size);
			if (err) {
				err = -EBADE;
				goto bail;
		...
		VERIFY(err, NULL != (inv2_user = kmalloc(sizeof(*inv2_user),
							 GFP_KERNEL)));
		if (err) {
			err = -EFAULT;
			goto bail;
...
static int compat_fastrpc_ioctl_invoke2(struct file *filp, unsigned int cmd,
					unsigned long arg)
{
	struct compat_fastrpc_ioctl_invoke2 __user *inv32;
	...
	inv32 = compat_ptr(arg);
	VERIFY(err, 0 == compat_get_fastrpc_ioctl_invoke2(inv32, &inv, cmd));
	if (err)
		return err;
...
static int
compat_get_fastrpc_ioctl_mmap(struct compat_fastrpc_ioctl_mmap __user *map32,
...
static int
compat_put_fastrpc_ioctl_mmap(struct compat_fastrpc_ioctl_mmap __user *map32,
			      struct fastrpc_ioctl_mmap __user *map)
{
	compat_uptr_t p;
	int err = 0;
...
static int compat_get_fastrpc_ioctl_init(
	...
	struct fastrpc_ioctl_init_attrs *init, unsigned int cmd)
{
	compat_uint_t u;
	compat_uptr_t p;
	compat_int_t i;
...
static int compat_fastrpc_control(struct fastrpc_file *fl, unsigned long arg)
{
	int err = 0;
	struct compat_fastrpc_ioctl_control __user *ctrl32;
	struct fastrpc_ioctl_control *ctrl;
	...
	VERIFY(err, NULL != (ctrl = kmalloc(sizeof(*ctrl), GFP_KERNEL)));
	if (err)
		return -EFAULT;
	VERIFY(err, 0 == compat_get_fastrpc_ioctl_control(ctrl32, ctrl));
	if (err)
		return err;
	...
	if (p == FASTRPC_CONTROL_KALLOC) {
		memcpy(&p, &ctrl->kalloc.kalloc_support,
		       sizeof(ctrl->kalloc.kalloc_support));
				       ...
				       unsigned long arg)
{
	...
	VERIFY(err, NULL != (info = kmalloc(sizeof(*info), GFP_KERNEL)));
						    ...
						    unsigned long arg)
{
	...
	case COMPAT_FASTRPC_IOCTL_MEM_MAP: {
		...
		VERIFY(err, NULL != (map = kmalloc(sizeof(*map), GFP_KERNEL)));
		if (err)
			return -EFAULT;
		...
		VERIFY(err, 0 == (err = fastrpc_internal_mem_map(fl, map)));
		if (err)
			return err;
	...
	case COMPAT_FASTRPC_IOCTL_MEM_UNMAP: {
		...
		VERIFY(err, 0 == (err = fastrpc_internal_mem_unmap(fl, unmap)));
		return err;
	}
	...
	case COMPAT_FASTRPC_IOCTL_MUNMAP: {
		...
		unmap32 = compat_ptr(arg);
		VERIFY(err,
		       NULL != (unmap = kmalloc(sizeof(*unmap), GFP_KERNEL)));
		if (err)
			return -EFAULT;
		VERIFY(err,
		       0 == compat_get_fastrpc_ioctl_munmap(unmap32, unmap));
		if (err)
			return err;
		VERIFY(err, 0 == (err = fastrpc_internal_munmap(fl, unmap)));
		return err;
	}
	default:
		return -ENOIOCTLCMD;
...
static long compat_fastrpc_dspsignal_create(struct fastrpc_file *fl,
					    unsigned long arg)
{
...
static long compat_fastrpc_dspsignal_destroy(struct fastrpc_file *fl,
					     unsigned long arg)
{
...
static long compat_fastrpc_dspsignal_signal(struct fastrpc_file *fl,
					    unsigned long arg)
{
...
static long compat_fastrpc_dspsignal_wait(struct fastrpc_file *fl,
					  unsigned long arg)
{
...
static long compat_fastrpc_dspsignal_cancel_wait(struct fastrpc_file *fl,
						 unsigned long arg)
{
				 ...
				 unsigned long arg)
{
	...
	case COMPAT_FASTRPC_IOCTL_MUNMAP_64: {
		...
		if (err)
			return -EFAULT;
		VERIFY(err,
		       0 == compat_get_fastrpc_ioctl_munmap_64(unmap32, unmap));
		if (err)
			return err;
	...
	case COMPAT_FASTRPC_IOCTL_INIT_ATTRS: {
		...
		VERIFY(err,
		       NULL != (init = kmalloc(sizeof(*init), GFP_KERNEL)));
		if (err)
			return -EFAULT;
		VERIFY(err,
		       0 == compat_get_fastrpc_ioctl_init(init32, init, cmd));
		if (err)
			return err;
	...
	case FASTRPC_IOCTL_GETINFO: {
		...
		info32 = compat_ptr(arg);
		VERIFY(err,
		       NULL != (info = kmalloc(sizeof(*info), GFP_KERNEL)));
		if (err)
			return -EFAULT;
