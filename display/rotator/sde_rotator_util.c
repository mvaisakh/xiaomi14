 ...
 * Copyright (c) 2012, 2015-2019, 2021, The Linux Foundation. All rights
 * reserved.
 */
...
void sde_mdp_get_v_h_subsample_rate(u8 chroma_sample, u8 *v_sample,
				    u8 *h_sample)
{
...
void sde_rot_crop_rect(struct sde_rect *src_rect, struct sde_rect *dst_rect,
		       const struct sde_rect *sci_rect)
{
	struct sde_rect res;
	...
	if (res.w && res.h) {
		...
		*dst_rect = (struct sde_rect){ (res.x - sci_rect->x),
					       (res.y - sci_rect->y), res.w,
					       res.h };
	}
}
...
int sde_mdp_get_rau_strides(u32 w, u32 h, struct sde_mdp_format_params *fmt,
			    struct sde_mdp_plane_sizes *ps)
{
	...
	SDEROT_DBG("BWC rau_cnt=%d strides={%d,%d} heights={%d,%d}\n",
		   ps->rau_cnt, ps->ystride[0], ps->ystride[1], ps->rau_h[0],
		   ps->rau_h[1]);
				      ...
				      struct sde_mdp_plane_sizes *ps)
{
	...
	if (sde_mdp_is_nv12_8b_format(fmt)) {
		...
		ps->plane_size[0] =
			ALIGN(ps->ystride[0] * ALIGN(height, 32), 4096);

		/* CbCr bitstream stride and plane size */
		ps->ystride[1] = ALIGN(width, 128);
		ps->plane_size[1] =
			ALIGN(ps->ystride[1] * ALIGN(height / 2, 32), 4096);
		...
		ps->plane_size[2] = ALIGN(
			ps->ystride[2] * ALIGN(DIV_ROUND_UP(height, 8), 16),
			4096);

		/* CbCr meta data stride and plane size */
		ps->ystride[3] = ALIGN(DIV_ROUND_UP(width / 2, 16), 64);
		ps->plane_size[3] = ALIGN(
			ps->ystride[3] * ALIGN(DIV_ROUND_UP(height / 2, 8), 16),
			4096);
		...
		ps->plane_size[0] =
			ALIGN(ps->ystride[0] * ALIGN(height, 16), 4096);

		/* CbCr bitstream stride and plane size */
		ps->ystride[1] = ALIGN(width * 2, 256);
		ps->plane_size[1] =
			ALIGN(ps->ystride[1] * ALIGN(height / 2, 16), 4096);
		...
		ps->ystride[2] = ALIGN(DIV_ROUND_UP(width, 32), 64);
		ps->plane_size[2] = ALIGN(
			ps->ystride[2] * ALIGN(DIV_ROUND_UP(height, 4), 16),
			4096);

		/* CbCr meta data stride and plane size */
		ps->ystride[3] = ALIGN(DIV_ROUND_UP(width / 2, 16), 64);
		ps->plane_size[3] = ALIGN(
			ps->ystride[3] * ALIGN(DIV_ROUND_UP(height / 2, 4), 16),
			4096);
		...
		ps->plane_size[0] =
			ALIGN(ps->ystride[0] * (yHeight * TILEHEIGHT_SIZE /
						Y_TILEHEIGHT),
			      4096);
		...
		ps->plane_size[1] =
			ALIGN(ps->ystride[1] * (uvHeight * TILEHEIGHT_SIZE /
						UV_TILEHEIGHT),
			      4096);
		...
		ps->plane_size[2] = ALIGN(
			ps->ystride[2] * ALIGN((yHeight / Y_TILEHEIGHT), 16),
			4096);

		/* CbCr meta data stride and plane size */
		ps->ystride[3] = ALIGN(uvWidth / UV_TILEWIDTH, 64);
		ps->plane_size[3] = ALIGN(
			ps->ystride[3] * ALIGN((uvHeight / UV_TILEHEIGHT), 16),
			4096);
		...
		ps->plane_size[0] =
			ALIGN(bpp * aligned_bitstream_width * ALIGN(height, 16),
			      4096);
			    ...
			    bool rotation)
{
	...
	if (sde_mdp_is_tilea5x_format(fmt)) {
		...
		if (fmt->fetch_planes == SDE_MDP_PLANE_INTERLEAVED) {
			...
			ps->plane_size[0] =
				VENUS_Y_SCANLINES(cf, h) * ps->ystride[0];
			ps->plane_size[1] =
				VENUS_UV_SCANLINES(cf, h) * ps->ystride[1];
       ...
       */
			...
			ps->plane_size[0] =
				ALIGN(ps->ystride[0] * yHeight, 4096);
			ps->plane_size[1] =
				ALIGN(ps->ystride[1] * uvHeight, 4096);
		} else {
			...
			sde_mdp_get_v_h_subsample_rate(
			...
			ps->plane_size[0] =
				ps->ystride[0] * ALIGN(h, height_align);
				  ...
				  struct sde_mdp_format_params *fmt)
{
	...
	if (sde_mdp_is_yuv_format(fmt)) {
		...
		data->p[3].addr =
			base_addr + ps->plane_size[0] + ps->plane_size[2];
...
int sde_validate_offset_for_ubwc_format(struct sde_mdp_format_params *fmt,
					u16 x, u16 y)
{
	int ret;
				   ...
				   struct sde_mdp_format_params *fmt)
{
	...
	if (sde_mdp_is_nv12_8b_format(fmt)) {
		...
		offset = y * ps->ystride[0] + (x / macro_w) * 4096;
			      ...
			      struct sde_mdp_format_params *fmt)
{
	...
	if (data->num_planes == 1) {
		...
		sde_mdp_get_v_h_subsample_rate(fmt->chroma_sample, &v_subsample,
					       &h_subsample);
...
static int sde_mdp_put_img(struct sde_mdp_img_data *data, bool rotator, int dir)
{
	u32 domain;
			      ...
			      int dir)
{
     ...
     */
		...
		if (IS_ERR_OR_NULL(sgt) || IS_ERR_OR_NULL(sgt->sgl)) {
...
static int sde_mdp_data_get(struct sde_mdp_data *data,
			    struct sde_fb_data *planes, int num_planes,
			    u32 flags, struct device *dev, bool rotator,
			    int dir)
{
	int i, rc = 0;
...
int sde_mdp_data_get_and_validate_size(struct sde_mdp_data *data,
				       struct sde_fb_data *planes,
				       int num_planes, u32 flags,
				       struct device *dev, bool rotator,
				       int dir, struct sde_layer_buffer *buffer)
{
	struct sde_mdp_format_params *fmt;
	struct sde_mdp_plane_sizes ps;
	int ret, i;
	...
	ret = sde_mdp_data_get(data, planes, num_planes, flags, dev, rotator,
			       dir);
	if (ret)
		return ret;
	...
	for (i = 0; i < num_planes; i++) {
		unsigned long plane_len =
			(data->p[i].srcp_dma_buf) ?
				data->p[i].srcp_dma_buf->size :
				data->p[i].len;
...
sde_rot_dmabuf_map_tiny(struct dma_buf_attachment *attach,
