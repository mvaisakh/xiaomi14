...
static int sde_crtc_mmrm_interrupt_handler(struct drm_crtc *crtc_drm, bool en,
					   struct sde_irq_callback *idle_irq);
static int sde_crtc_pm_event_handler(struct drm_crtc *crtc, bool en,
				     struct sde_irq_callback *noirq);
static int
sde_crtc_frame_data_interrupt_handler(struct drm_crtc *crtc_drm, bool en,
				      struct sde_irq_callback *idle_irq);
static int _sde_crtc_set_noise_layer(struct sde_crtc *sde_crtc,
				     struct sde_crtc_state *cstate,
				     void __user *usr_ptr);
static int sde_crtc_vm_release_handler(struct drm_crtc *crtc_drm, bool en,
				       struct sde_irq_callback *irq);
static int sde_crtc_opr_event_handler(struct drm_crtc *crtc_drm, bool en,
				      struct sde_irq_callback *irq);
...
static void
sde_crtc_install_noise_layer_properties(struct sde_crtc *sde_crtc,
...
enum sde_wb_usage_type sde_crtc_get_wb_usage_type(struct drm_crtc *crtc)
{
	...
	drm_for_each_connector_iter(conn, &conn_iter) {
		if (conn->state && (conn->state->crtc == crtc) &&
			...
			usage_type = sde_connector_get_property(
				conn->state, CONNECTOR_PROP_WB_USAGE_TYPE);
			break;
		}
	}
...
_sde_crtc_get_virt_conn_state(struct drm_crtc *crtc,
			      struct drm_crtc_state *crtc_state)
{
	...
	if (crtc_state->state) {
		for_each_new_connector_in_state(crtc_state->state, conn,
						conn_state, i) {
...
void sde_crtc_get_mixer_resolution(struct drm_crtc *crtc,
				   struct drm_crtc_state *crtc_state,
				   struct drm_display_mode *mode, u32 *width,
				   u32 *height)
{
	...
	*height = 0;
	...
	virt_cstate = virt_conn_state ?
			      to_sde_connector_state(virt_conn_state) :
			      NULL;
...
void sde_crtc_get_resolution(struct drm_crtc *crtc,
			     struct drm_crtc_state *crtc_state,
			     struct drm_display_mode *mode, u32 *width,
			     u32 *height)
{
				 ...
				 struct device_attribute *attr, char *buf)
{
   ...
   */
	current_time_index = (sde_crtc->fps_info.next_time_index == 0) ?
				     MAX_FRAME_COUNT - 1 :
				     (sde_crtc->fps_info.next_time_index - 1);
	...
	for (i = 0; i < MAX_FRAME_COUNT; i++) {
		...
		diff_us = (u64)ktime_us_delta(
			...
			sde_crtc->fps_info.time_buf[current_time_index]);
		if (ptime > buftime &&
		    diff_us >= (u64)sde_crtc->fps_info.fps_periodic_duration) {
			/* Multiplying with 10 to get fps in floating point */
	...
	if (i == MAX_FRAME_COUNT) {
		current_time_index =
			(sde_crtc->fps_info.next_time_index == 0) ?
				MAX_FRAME_COUNT - 1 :
				(sde_crtc->fps_info.next_time_index - 1);
...
static void sde_crtc_event_notify(struct drm_crtc *crtc, uint32_t type,
				  void *payload, uint32_t len)
{
	...
	SDE_EVT32(DRMID(crtc), type, len, *data, ((uint64_t)payload) >> 32,
...
_sde_crtc_get_sde_connector_state(struct drm_crtc *crtc,
				      ...
				      struct sde_format *format)
{
	...
	SDE_DEBUG("format: %4.4s, alpha_enable %u fg alpha:0x%x bg alpha:0x%x "
		  "blend_op:0x%x\n",
		  (char *)&format->base.pixel_format, format->alpha_enable,
		  fg_alpha, bg_alpha, blend_op);
}

static void _sde_crtc_calc_split_dim_layer_yh_param(struct drm_crtc *crtc,
						    u16 *y, u16 *h)
{
	...
	for (i = 0; i < sde_crtc->num_mixers; i++) {
       ...
       */
			...
			split_dim_layer.rect.x =
				split_dim_layer.rect.x - cstate->lm_roi[i].x;
			split_dim_layer.rect.y =
				split_dim_layer.rect.y - cstate->lm_roi[i].y;
		}

		/* update dim layer rect for panel stacking crtc */
		if (cstate->line_insertion.padding_height)
			_sde_crtc_calc_split_dim_layer_yh_param(
				crtc, &split_dim_layer.rect.y,
				...
				void __user *usr_ptr)
{
	...
	memset(&cstate->cached_user_roi_list, 0,
	       sizeof(cstate->cached_user_roi_list));
	...
	for (i = 0; i < roi_v1.num_rects; ++i) {
		...
		if (cstate->user_roi_list.roi_feature_flags &
       ...
       */
		...
		SDE_DEBUG("crtc%d: roi%d: roi (%d,%d) (%d,%d)\n", DRMID(crtc),
			  i, cstate->user_roi_list.roi[i].x1,
		...
		SDE_DEBUG("crtc%d: spr roi%d: spr roi (%d,%d) (%d,%d)\n",
			  DRMID(crtc), i, cstate->user_roi_list.spr_roi[i].x1,
		...
		SDE_DEBUG(
			...
			roi_v1.spr_roi[i].x1, roi_v1.spr_roi[i].y1,
			roi_v1.spr_roi[i].x2, roi_v1.spr_roi[i].y2);
				  ...
				  struct drm_crtc_state *state)
{
	...
	for_each_new_connector_in_state(state->state, conn, conn_state, i) {
		...
		is_conn_roi_dirty = msm_property_is_dirty(
			&sde_conn->property_info,
			&sde_conn_state->property_state, CONNECTOR_PROP_ROI_V1);
     ...
     */
		if (memcmp(&sde_conn_state->rois.roi,
			   &crtc_state->user_roi_list.spr_roi,
		...
		SDE_EVT32_VERBOSE(DRMID(crtc), DRMID(conn), conn_roi.x,
				  conn_roi.y, conn_roi.w, conn_roi.h);
	}
	...
	sde_crtc_get_resolution(crtc, state, adj_mode, &crtc_width,
				&crtc_height);
	if (crtc_roi->x == 0 && crtc_roi->y == 0 && crtc_roi->w == crtc_width &&
				       ...
				       struct drm_crtc_state *state)
{
	...
	for_each_new_connector_in_state(state->state, conn, conn_state, i) {
		...
		autorefresh = sde_connector_get_property(
			conn_state, CONNECTOR_PROP_AUTOREFRESH);
		if (autorefresh) {
			SDE_ERROR(
   ...
   */
	...
	if (roi[0]->x + roi[0]->w != roi[1]->x || roi[0]->w != roi[1]->w) {
		SDE_ERROR(
			...
			sde_crtc->name, roi[0]->x, roi[0]->w, roi[1]->x,
			roi[1]->w);
		return -EINVAL;
	}
	...
	drm_atomic_crtc_state_for_each_plane_state(plane, pstate, state) {
		...
		if (!sde_kms_rect_is_equal(&plane_roi, &intersection)) {
			SDE_ERROR(
				...
				sde_crtc->name, plane->base.id, plane_roi.x,
				plane_roi.y, plane_roi.w, plane_roi.h,
				crtc_roi->x, crtc_roi->y, crtc_roi->w,
				crtc_roi->h);
			return -E2BIG;
		}
	}
				...
				struct drm_crtc_state *state)
{
	...
	sde_crtc_get_resolution(crtc, state, adj_mode, &crtc_width,
				&crtc_height);
	sde_crtc_get_mixer_resolution(crtc, state, adj_mode, &mixer_width,
				      &mixer_height);
	...
	if (sde_crtc->num_mixers &&
		...
		SDE_ERROR(
			"%s: invalid w/h crtc:%d,%d, mixer:%d,%d, num_mixers:%d\n",
			sde_crtc->name, crtc_width, crtc_height, mixer_width,
			mixer_height, sde_crtc->num_mixers);
		rc = -EINVAL;
		goto end;
	} else if (state->state) {
		for_each_new_connector_in_state(state->state, conn, conn_state,
						i) {
			if (conn_state && (conn_state->crtc == crtc) &&
			    ((sde_connector_is_dualpipe_3d_merge_enabled(
				      conn_state) &&
			      (crtc_width % 4)) ||
			     (sde_connector_is_quadpipe_3d_merge_enabled(
				      conn_state) &&
			      (crtc_width % 8)))) {
				SDE_ERROR(
					"%s: invalid 3d-merge_w - mixer_w:%d, crtc_w:%d, num_mixers:%d\n",
					sde_crtc->name, mixer_width, crtc_width,
					sde_crtc->num_mixers);
				return -EINVAL;
			}
		}
   ...
   */
	for (i = 0; i < sde_crtc_state->num_connectors; i++) {
		...
		if (sde_connector_is_3d_merge_enabled(conn->state) &&
		    (mixer_width % 2)) {
			SDE_ERROR(
				"%s: invalid width w/ 3d-merge - mixer_w:%d, crtc_w:%d, "
				"num_mixers:%d\n",
				sde_crtc->name, crtc_width, mixer_width,
				sde_crtc->num_mixers);
			rc = -EINVAL;
			goto end;
		}
					  ...
					  struct drm_crtc_state *state)
{
	...
	if (!kms->catalog->has_line_insertion || !state->mode_changed ||
		...
		SDE_DEBUG(
			"no line insertion support mode change %d enc cnt %d\n",
	...
	if (mode_info.vpadding < state->mode.vdisplay) {
		...
		sde_crtc_state->line_insertion.padding_height = 0;
		return 0;
	} else if (mode_info.vpadding ==
		   sde_crtc_state->line_insertion.padding_height) {
	...
	if (num_of_active_lines > MAX_VPADDING_RATIO_M ||
		...
		SDE_ERROR("unsupported panel stacking pattern %d:%d",
			  num_of_active_lines, num_of_dummy_lines);
		return -EINVAL;
	}
     ...
     */
		if ((right_pid < left_pid) &&
			...
			SDE_ERROR(
				...
				stage, left_rect.x, left_rect.w, right_rect.x,
				right_rect.w);
			return -EINVAL;
			...
			SDE_ERROR(
				...
				stage, left_rect.y, left_rect.h, right_rect.y,
				right_rect.h);
			return -EINVAL;
		}
	}
					       ...
					       int cnt)
{
	...
	for (i = cnt - 1; i >= 0; i--) {
		...
		for (lm_idx = 0; lm_idx < num_mixers; lm_idx++) {
			...
			_sde_crtc_setup_blend_cfg(
				mixer + lm_idx, pstates[i].sde_pstate, format);
					...
					struct sde_crtc_mixer *mixer)
{
	...
	drm_atomic_crtc_for_each_plane(plane, crtc) {
		...
		SDE_DEBUG("crtc %d stage:%d - plane %d sspp %d fb %d\n",
			  crtc->base.id, pstate->stage, plane->base.id,
		...
		if (blend_type == SDE_DRM_BLEND_OP_SKIP) {
			...
			sde_cp_set_skip_blend_plane_info(crtc,
							 &skip_blend_plane);
		}
		...
		if (blend_type != SDE_DRM_BLEND_OP_SKIP) {
			...
			SDE_EVT32(DRMID(crtc), DRMID(plane),
				  ...
				  state->crtc_h, pstate->rotation, mode);
	...
	/* blend config update */
	_sde_crtc_setup_blend_cfg_by_stage(mixer, sde_crtc->num_mixers, pstates,
					   cnt);
				  ...
				  bool add_planes)
{
	...
	if (test_bit(SDE_CRTC_DIRTY_DIM_LAYERS, &sde_crtc->revalidate_mask)) {
		...
		clear_bit(SDE_CRTC_DIRTY_DIM_LAYERS,
			  &sde_crtc->revalidate_mask);
	}
	...
	for (i = 0; i < sde_crtc->num_mixers; i++) {
		...
		SDE_DEBUG("lm %d, op_mode 0x%X, ctl %d, flush mask 0x%x\n",
			  ...
			  ctl->idx - CTL_0, cfg.pending_flush_mask);
		...
		if (sde_kms_rect_is_null(lm_roi)) {
			...
			ctl->ops.setup_blendstage(
				ctl, mixer[i].hw_lm->idx,
...
int sde_crtc_find_plane_fb_modes(struct drm_crtc *crtc, uint32_t *fb_ns,
				 uint32_t *fb_sec, uint32_t *fb_sec_dir)
{
	struct drm_plane *plane;
	struct sde_plane_state *sde_pstate;
	uint32_t mode = 0;
			  ...
			  bool old_valid_fb, int *ops)
{
	...
	if ((smmu_state->state == ATTACHED) &&
		...
		smmu_state->state = (test_bit(SDE_FEATURE_SUI_NS_ALLOWED,
					      catalog->features)) ?
					    DETACH_SEC_REQ :
					    DETACH_ALL_REQ;
...
static void _sde_drm_fb_transactions(struct sde_kms_smmu_state_data *smmu_state,
				     struct sde_mdss_cfg *catalog,
				     bool old_valid_fb, bool post_commit,
				     int *ops)
{
   ...
   */
	drm_for_each_encoder_mask(encoder, crtc->dev,
		...
		if (sde_encoder_is_dsi_display(encoder))
			post_commit |= sde_encoder_check_curr_mode(
	...
	drm_atomic_crtc_for_each_plane(plane, crtc) {
		...
		translation_mode =
			sde_plane_get_property(to_sde_plane_state(plane->state),
					       PLANE_PROP_FB_TRANSLATION_MODE);
	...
	case SDE_DRM_FB_SEC:
	case SDE_DRM_FB_NON_SEC:
		_sde_drm_fb_transactions(smmu_state, catalog, old_valid_fb,
					 post_commit, &ops);
		break;
	...
	if (ops) {
		SDE_DEBUG("crtc%d: state%d sec%d sec_lvl%d type%d ops%x\n",
			  DRMID(crtc), smmu_state->state, secure_level,
			  smmu_state->secure_level, smmu_state->transition_type,
			  ops);
		SDE_EVT32(DRMID(crtc), secure_level, translation_mode,
			  smmu_state->state, smmu_state->transition_type,
			  smmu_state->secure_level, old_valid_fb, post_commit,
			  ops, SDE_EVTLOG_FUNC_EXIT);
	}
					     ...
					     void __user *usr)
{
	...
	for (i = 0; i < ctrl.num_buffers; i++) {
		if (_sde_crtc_get_frame_data_buffer(crtc, ctrl.fds[i])) {
			SDE_ERROR("unable to set buffer for fd %d",
				  ctrl.fds[i]);
			goto exit;
		}
...
static void
_sde_crtc_frame_data_notify(struct drm_crtc *crtc,
			    struct sde_drm_frame_data_packet *frame_data_packet)
{
	...
	sde_crtc->frame_data.idx =
		++sde_crtc->frame_data.idx % sde_crtc->frame_data.cnt;
}
...
void sde_crtc_get_frame_data(struct drm_crtc *crtc)
{
	...
	drm_for_each_plane_mask(plane, crtc->dev, sde_crtc->plane_mask_old) {
		if (i < SDE_FRAME_DATA_MAX_PLANES)
			sde_plane_get_frame_data(plane,
						 &data->plane_frame_data[i++]);
	}
		...
		if (conn->state && conn->state->crtc == crtc &&
			...
			if (encoder)
				sde_encoder_register_frame_event_callback(
					encoder, sde_crtc_frame_event_cb, crtc);
...
static void sde_crtc_vblank_notify_work(struct kthread_work *work)
{
	...
	struct sde_crtc_vblank_event *vevent =
		container_of(work, struct sde_crtc_vblank_event, work);
	unsigned long flags;
...
void sde_crtc_opr_event_notify(struct drm_crtc *crtc)
{
	...
	for (i = 0; i < sde_crtc->num_mixers; i++) {
		...
		if (current_opr_value[i] ==
		    sde_crtc->previous_opr_value.opr_value[i])
			continue;
			      ...
			      struct drm_crtc_state *old_state)
{
	...
	if ((crtc->state->active_changed || cont_splash_enabled) &&
	    crtc->state->active)
		sde_crtc_event_notify(crtc, DRM_EVENT_CRTC_POWER, &power_on,
				      sizeof(u32));
				       ...
				       void __user *usr_ptr)
{
	...
	for (i = 0; i < count; i++) {
		...
		dim_layer[i].stage = test_bit(SDE_FEATURE_BASE_LAYER,
					      kms->catalog->features) ?
					     user_cfg->stage :
					     user_cfg->stage + SDE_STAGE_0;
		...
		SDE_DEBUG("dim_layer[%d] - flags:%d, stage:%d\n", i,
			  dim_layer[i].flags, dim_layer[i].stage);
				     ...
				     void __user *usr_ptr)
{
	...
	for (i = 0; i < count; i++) {
		...
		SDE_EVT32_VERBOSE(DRMID(&sde_crtc->base), scaler_v2.enable,
				  ...
				  scaler_v2.src_width[0],
				  scaler_v2.src_height[0], scaler_v2.dst_width,
				  scaler_v2.dst_height);

		SDE_DEBUG("ds cfg[%d]-ndx(%d) flags(%d) lm(%dx%d)\n", i,
			  ds_cfg_usr->index, ds_cfg_usr->flags,
...
static int _sde_crtc_check_dest_scaler_validate_ds(
	struct drm_crtc *crtc, struct sde_crtc *sde_crtc,
	struct sde_crtc_state *cstate, struct drm_display_mode *mode,
	struct sde_hw_ds *hw_ds, u32 hdisplay, u32 *num_ds_enable,
...
static void _sde_crtc_check_dest_scaler_data_disable(
	struct drm_crtc *crtc, struct sde_crtc_state *cstate, u32 num_ds_enable)
{
	struct sde_hw_ds_cfg *cfg;
	int i;
					    ...
					    struct drm_crtc_state *state)
{
   ...
   */
	...
	ret = _sde_crtc_check_dest_scaler_validate_ds(
		crtc, sde_crtc, cstate, mode, hw_ds, hdisplay, &num_ds_enable,
		max_in_width, max_out_width);
	if (ret)
		goto err;
...
static struct dma_fence *_sde_plane_get_input_hw_fence(struct drm_plane *plane)
{
	...
	if (input_fence) {
		...
		if (test_bit(SPEC_FENCE_FLAG_FENCE_ARRAY, &fence->flags)) {
			...
			array = container_of(fence, struct dma_fence_array,
					     base);
			...
			if (!test_bit(SPEC_FENCE_FLAG_FENCE_ARRAY_BOUND,
				      &fence->flags))
				if (spec_sync_wait_bind_array(
					    array, SPEC_FENCE_TIMEOUT_MS) < 0)
					goto exit;
		...
		} else if (test_bit(MSM_HW_FENCE_FLAG_ENABLED_BIT,
				    &fence->flags)) {
			input_hw_fence = fence;

			SDE_DEBUG(
				"input hwfence ctx:%llu seqno:%llu f:0x%lx timeline:%s\n",
...
int sde_crtc_sw_fence_error_handle(struct drm_crtc *crtc, int err_status)
{
	...
	drm_for_each_encoder_mask(drm_encoder, crtc->dev,
				  crtc->state->encoder_mask) {
		/* continue if copy encoder is encountered */
	...
	list_for_each_entry(entry, &priv->fence_error_client_list, list) {
		...
		rc = entry->ops.fence_error_handle_submodule(hw_ctl,
							     entry->data);
		if (rc) {
			SDE_ERROR(
				"fence_error_handle_submodule failed for device: %d\n",
				entry->dev->id);
...
/**
 * _sde_crtc_fences_wait_list - wait for input sw-fences and return any
 * hw-fences
 * @crtc: Pointer to CRTC object.
 * @use_hw_fences: Boolean to indicate if function should use hw-fences and skip
 * hw-fences sw-wait.
 * @dma_hw_fences: List of available hw-fences, this is populated by this
 * function.
 * @max_hw_fences: Max number of hw-fences that can be added to the
 * dma_hw_fences list
 *
 * This function iterates through all crtc planes, if 'use_hw_fences' is set,
 * for each fence:
 * - If the fence is a hw-fence, it will get its dma-fence object and add it to
 * the 'dma_hw_fences' list, skipping any sw-wait, since wait will happen in hw.
 * - If the fence is not a hw-fence, it will wait for the sw-fence to be
 * signaled before proceed. If 'use_hw_fences' is not set, function will wait on
 * the sw-fences for all fences regardless if they support or not hw-fence.
...
static int _sde_crtc_fences_wait_list(struct drm_crtc *crtc, bool use_hw_fences,
				      struct dma_fence **dma_hw_fences,
				      int max_hw_fences)
{
     ...
     */
		if (use_hw_fences && !mode_switch) {
			dma_hw_fences[num_hw_fences] =
				_sde_plane_get_input_hw_fence(plane);

			if (dma_hw_fences[num_hw_fences] &&
			    (num_hw_fences < max_hw_fences)) {
...
{
	struct sde_crtc *sde_crtc = to_sde_crtc(crtc);
	bool is_vid_mode = sde_encoder_check_curr_mode(
		sde_crtc->mixers[0].encoder, MSM_DISPLAY_VIDEO_MODE);

	return is_vid_mode && crtc->state->active_changed &&
	       crtc->state->active;
}

/**
 * _sde_crtc_wait_for_fences - wait for incoming framebuffer sync fences or
 * register hw-fences
 * @crtc: Pointer to CRTC object
 *
...
static bool _sde_crtc_wait_for_fences(struct drm_crtc *crtc)
{
	...
	if (test_bit(HW_FENCE_IN_FENCES_ENABLE,
		     sde_crtc->hwfence_features_mask) &&
	    !sde_fence_update_input_hw_fence_signal(
		    hw_ctl, sde_kms->debugfs_hw_fence, sde_kms->hw_mdp,
		    disable_hw_fences))
		ipcc_input_signal_wait = true;

	/* avoid hw-fences in first frame after timing engine enable */
	input_hw_fences_enable =
		(ipcc_input_signal_wait && !_is_vid_power_on_frame(crtc));

	/* wait for sw fences and get hw fences list (if any) */
	num_hw_fences = _sde_crtc_fences_wait_list(
		crtc, input_hw_fences_enable, &dma_hw_fences[0], MAX_HW_FENCES);
	...
	if (num_hw_fences > 0 && num_hw_fences <= MAX_HW_FENCES) {
		...
		if (ret) {
			SDE_ERROR(
				"failed to register for hw-fence wait, will wait in sw\n");
			...
			num_hw_fences = _sde_crtc_fences_wait_list(
				crtc, false, &dma_hw_fences[0], MAX_HW_FENCES);
		}
	}
	...
	SDE_DEBUG(
		...
		input_hw_fences_enable,
		test_bit(HW_FENCE_IN_FENCES_NO_OVERRIDE,
			 sde_crtc->hwfence_features_mask),
		hw_ctl ? hw_ctl->idx - CTL_0 : -1, ipcc_input_signal_wait,
		num_hw_fences);
	SDE_EVT32(input_hw_fences_enable,
		  test_bit(HW_FENCE_IN_FENCES_NO_OVERRIDE,
			   sde_crtc->hwfence_features_mask),
		  ipcc_input_signal_wait, num_hw_fences,
		  hw_ctl ? hw_ctl->idx - CTL_0 : -1);

	/* if hw is waiting for ipcc signal and no hw-fences, override signal */
	if (ipcc_input_signal_wait && !num_hw_fences &&
					      ...
					      struct drm_encoder *enc)
{
	...
	for (i = sde_crtc->num_mixers; i < ARRAY_SIZE(sde_crtc->mixers); i++) {
		...
		SDE_DEBUG("setup mixer %d: ctl %d\n", i,
			  mixer->hw_ctl->idx - CTL_0);
		if (mixer->hw_ds)
			SDE_DEBUG("setup mixer %d: ds %d\n", i,
				  mixer->hw_ds->idx - DS_0);
	}
}
			     ...
			     struct drm_crtc_state *old_state)
{
   ...
   */
     ...
     */
		...
		if (idle_pc_state != IDLE_PC_NONE)
			sde_encoder_control_idle_pc(encoder, (idle_pc_state ==
							      IDLE_PC_ENABLE) ?
								     true :
								     false);
   ...
   */
	...
	/*
   * for cmd and wb modes, update the txq for incoming fences before flush to
   * avoid race condition between txq update and the hw signal during ctl-done
   * for partial updates
   */
	if (test_bit(HW_FENCE_OUT_FENCES_ENABLE,
		     sde_crtc->hwfence_features_mask) &&
	    !is_vid)
 ...
 * @Return: error code
 */
static int _sde_crtc_vblank_enable(struct sde_crtc *sde_crtc, bool enable)
{
	struct drm_crtc *crtc;
	struct drm_encoder *enc;
	...
	if (enable) {
		...
		drm_for_each_encoder_mask(enc, crtc->dev,
			...
			sde_encoder_register_vblank_callback(
				enc, sde_crtc_vblank_cb, (void *)crtc);
		}
		mutex_unlock(&sde_crtc->crtc_lock);
	} else {
		mutex_lock(&sde_crtc->crtc_lock);
...
static void _sde_crtc_reserve_resource(struct drm_crtc *crtc,
				       struct drm_connector *conn)
{
	...
	mode_clock_hz = mult_frac(
		crtc->mode.htotal * crtc->mode.vtotal * updated_fps, 105, 100);
	mode_clock_hz = div_u64(mode_clock_hz, lm_count);
	SDE_DEBUG("[%s] h=%d v=%d fps=%d lm=%d mode_clk=%u\n", crtc->mode.name,
		  crtc->mode.htotal, crtc->mode.vtotal, updated_fps, lm_count,
		  mode_clock_hz);
...
static struct drm_crtc_state *sde_crtc_duplicate_state(struct drm_crtc *crtc)
{
	...
	msm_property_duplicate_state(&sde_crtc->property_info, old_cstate,
				     cstate, &cstate->property_state,
...
static void sde_crtc_mmrm_cb_notification(struct drm_crtc *crtc)
{
	...
	sde_crtc_event_notify(crtc, DRM_EVENT_MMRM_CB, &requested_clk,
...
static void sde_crtc_handle_power_event(u32 event_type, void *arg)
{
	...
	case SDE_POWER_EVENT_POST_DISABLE:
		...
		power_on = 0;
		sde_crtc_event_notify(crtc, DRM_EVENT_SDE_POWER, &power_on,
				      sizeof(u32));
		break;
   ...
   */
	if (!crtc->state->active || !crtc->state->enable) {
		...
		sde_fence_signal(sde_crtc->output_fence, ktime_get(),
				 SDE_FENCE_RESET_TIMELINE, hw_ctl);
		for (i = 0; i < cstate->num_connectors; ++i)
	...
	if (sde_crtc->opr_event_notify_enabled)
		memset(&sde_crtc->previous_opr_value, 0,
		       sizeof(struct sde_opr_value));
   ...
   */
	if (!sde_crtc->enabled) {
		...
		if ((intf_mode != INTF_MODE_WB_BLOCK) &&
			...
			if (test_bit(SDE_FEATURE_HW_VSYNC_TS,
				     kms->catalog->features))
...
static int _sde_crtc_excl_dim_layer_check(struct drm_crtc *crtc,
					  struct drm_crtc_state *state,
					  struct plane_state pstates[], int cnt)
{
	...
	for (i = 0; i < cstate->num_dim_layers; i++) {
		...
		if ((CHECK_LAYER_BOUNDS(rect->y, rect->h, crtc_height)) ||
		    ...
		    (!rect->w) || (!rect->h)) {
			SDE_ERROR(
				"crtc:%d wxh:%dx%d, invalid dim_layer:{%d,%d,%d,%d}, stage:%d\n",
	...
	for (i = 0; i < cnt; i++) {
		...
		SDE_DEBUG("p %d z %d src{%d,%d,%d,%d} excl_rect{%d,%d,%d,%d}\n",
			  ...
			  pstate->crtc_h, sde_pstate->excl_rect.x,
			  sde_pstate->excl_rect.y, sde_pstate->excl_rect.w,
			  sde_pstate->excl_rect.h);
	}
...
static int _sde_crtc_check_secure_blend_config(struct drm_crtc *crtc,
					       ...
					       struct plane_state pstates[],
					       struct sde_crtc_state *cstate,
					       struct sde_kms *sde_kms, int cnt,
					       int secure, int fb_ns,
					       int fb_sec, int fb_sec_dir)
{
		...
		for (i = 1; i < cnt; i++) {
			...
			if (!sde_plane_is_sec_ui_allowed(plane)) {
				...
				SDE_ERROR(
					...
					DRMID(crtc), i, pstates[i].stage, i - 1,
					pstates[i - 1].stage);
				return -EINVAL;
			}
		}
     ...
     */
		if (sde_kms->catalog->sui_supported_blendstage) {
			...
			if (!test_bit(SDE_FEATURE_BASE_LAYER,
...
static int _sde_crtc_check_secure_state_smmu_translation(
	struct drm_crtc *crtc, struct drm_crtc_state *state,
	struct sde_kms *sde_kms, int secure, int fb_ns, int fb_sec,
	int fb_sec_dir)
{
	...
	drm_for_each_encoder_mask(encoder, crtc->dev, state->encoder_mask) {
		if (sde_encoder_is_dsi_display(encoder))
			is_video_mode |= sde_encoder_check_curr_mode(
				encoder, MSM_DISPLAY_VIDEO_MODE);
	}
   ...
   */
	if (fb_sec_dir && secure == SDE_DRM_SEC_NON_SEC &&
	    ...
	    smmu_state->secure_level == SDE_DRM_SEC_ONLY) {
		SDE_EVT32(DRMID(crtc), fb_ns, fb_sec_dir, smmu_state->state,
			  smmu_state->secure_level, secure);
		goto sec_err;
	}
   ...
   */
	if (is_video_mode && smmu_state && state->plane_mask &&
		...
		SDE_EVT32(DRMID(crtc), fb_ns, fb_sec_dir, smmu_state->state,
			  smmu_state->secure_level, secure,
			  crtc->state->plane_mask, state->plane_mask);
		goto sec_err;
	}
					...
					struct plane_state pstates[], int cnt)
{
	...
	rc = sde_crtc_state_find_plane_fb_modes(state, &fb_ns, &fb_sec,
						&fb_sec_dir);
	if (rc)
		return rc;

	rc = _sde_crtc_check_secure_blend_config(crtc, state, pstates, cstate,
						 sde_kms, cnt, secure, fb_ns,
						 fb_sec, fb_sec_dir);
	if (rc)
		return rc;
   ...
   */
	...
	rc = _sde_crtc_check_secure_state_smmu_translation(
		crtc, state, sde_kms, secure, fb_ns, fb_sec, fb_sec_dir);
	if (rc)
		return rc;
...
static int _sde_crtc_check_get_pstates(
	struct drm_crtc *crtc, struct drm_crtc_state *state,
	struct drm_display_mode *mode, struct plane_state *pstates,
	struct drm_plane *plane,
	struct sde_multirect_plane_states *multirect_plane, int *cnt)
{
	struct sde_crtc *sde_crtc;
	struct sde_crtc_state *cstate;
	const struct drm_plane_state *pstate;
		...
		if (blend_type == SDE_DRM_BLEND_OP_SKIP) {
			if (CHECK_LAYER_BOUNDS(pstate->crtc_y, pstate->crtc_h,
			    ...
			    CHECK_LAYER_BOUNDS(pstate->crtc_x, pstate->crtc_w,
					       mode->hdisplay)) {
				SDE_ERROR(
					"invalid dest - y:%d h:%d vdisp:%d x:%d w:%d hdisp:%d\n",
					pstate->crtc_y, pstate->crtc_h,
					mode->vdisplay, pstate->crtc_x,
					pstate->crtc_w, mode->hdisplay);
				return -E2BIG;
			}
		} else if (CHECK_LAYER_BOUNDS(pstate->crtc_y, pstate->crtc_h,
					      crtc_height) ||
			   CHECK_LAYER_BOUNDS(pstate->crtc_x, pstate->crtc_w,
					      crtc_width)) {
			SDE_ERROR(
				"invalid dest - y:%d h:%d crtc_h:%d x:%d w:%d crtc_w:%d\n",
...
static int _sde_crtc_check_zpos(struct drm_crtc_state *state,
				...
				struct plane_state *pstates,
				struct sde_crtc_state *cstate,
				struct drm_display_mode *mode, int cnt)
{
	int rc = 0, i, z_pos;
	u32 zpos_cnt = 0;
	struct drm_crtc *crtc;
	...
	struct sde_multirect_plane_states *multirect_plane)
{
	...
	rc = _sde_crtc_check_get_pstates(crtc, state, mode, pstates, plane,
					 multirect_plane, &cnt);
	if (rc)
		return rc;
					...
					struct drm_crtc_state *crtc_state)
{
	...
	sde_crtc_get_resolution(crtc, crtc_state, mode, &crtc_width,
				&crtc_height);
	...
	drm_atomic_crtc_state_for_each_plane(plane, crtc_state) {
		...
		if (CHECK_LAYER_BOUNDS(plane_state->crtc_x, plane_state->crtc_w,
				       layout_split)) {
				  ...
				  struct drm_crtc_state *state)
{
	...
	drm_connector_list_iter_begin(dev, &conn_iter);
	drm_for_each_connector_iter(conn, &conn_iter)
		if ((state->connector_mask &
		     (1 << drm_connector_index(conn))) &&
		    cstate->num_connectors < MAX_CONNECTORS) {
			cstate->connectors[cstate->num_connectors++] = conn;
		}
	drm_connector_list_iter_end(&conn_iter);
...
int sde_crtc_get_num_datapath(struct drm_crtc *crtc,
			      struct drm_connector *virtual_conn,
			      struct drm_crtc_state *crtc_state)
{
	struct sde_crtc *sde_crtc = to_sde_crtc(crtc);
   ...
   */
	...
	/* if PM resume occurs with CWB enabled, retrieve num_lm from primary dsi
   * panel mode */
	if (primary_conn && !num_lm) {
		num_lm = sde_connector_get_lm_cnt_from_topology(
			primary_conn, &crtc_state->adjusted_mode);
		if (num_lm < 0) {
						   ...
						   struct sde_kms_info *info)
{
	...
	if (catalog->ds[0].features & BIT(SDE_SSPP_SCALER_QSEED3)) {
		msm_property_install_volatile_range(&sde_crtc->property_info,
						    "dest_scaler", 0x0, 0, ~0,
						    0, CRTC_PROP_DEST_SCALER);
		msm_property_install_blob(&sde_crtc->property_info, "ds_lut_ed",
					  0, CRTC_PROP_DEST_SCALER_LUT_ED);
	...
	} else if (catalog->ds[0].features & BIT(SDE_SSPP_SCALER_QSEED3LITE)) {
		msm_property_install_volatile_range(&sde_crtc->property_info,
						    "dest_scaler", 0x0, 0, ~0,
						    0, CRTC_PROP_DEST_SCALER);
	}
					     ...
					     struct sde_kms_info *info)
{
	...
	msm_property_install_range(&sde_crtc->property_info, "dram_ib", 0x0, 0,
				   U64_MAX, catalog->perf.max_bw_high * 1000ULL,
				   CRTC_PROP_DRAM_IB);
	msm_property_install_range(&sde_crtc->property_info, "rot_prefill_bw",
				   0, 0, U64_MAX,
				   catalog->perf.max_bw_high * 1000ULL,
				   CRTC_PROP_ROT_PREFILL_BW);
	msm_property_install_range(&sde_crtc->property_info, "rot_clk", 0, 0,
				   U64_MAX, sde_kms->perf.max_core_clk_rate,
				   CRTC_PROP_ROT_CLK);
					     ...
					     struct sde_mdss_cfg *catalog)
{
	...
	if (sde_is_custom_client()) {
		...
		if (catalog->smart_dma_rev == SDE_SSPP_SMART_DMA_V2)
			sde_kms_info_add_keystr(info, "smart_dma_rev",
						"smart_dma_v2");
	...
	sde_kms_info_add_keyint(info, "has_src_split",
				test_bit(SDE_FEATURE_SRC_SPLIT,
					 catalog->features));
	sde_kms_info_add_keyint(info, "has_hdr",
				test_bit(SDE_FEATURE_HDR, catalog->features));
	sde_kms_info_add_keyint(info, "has_hdr_plus",
				test_bit(SDE_FEATURE_HDR_PLUS,
					 catalog->features));
	sde_kms_info_add_keyint(info, "skip_inline_rot_threshold",
				test_bit(SDE_FEATURE_INLINE_SKIP_THRESHOLD,
					 catalog->features));

	if (catalog->allowed_dsc_reservation_switch)
		sde_kms_info_add_keyint(
			info, "allowed_dsc_reservation_switch",
			catalog->allowed_dsc_reservation_switch);

	if (catalog->uidle_cfg.uidle_rev)
		sde_kms_info_add_keyint(info, "has_uidle", true);
					...
					struct sde_mdss_cfg *catalog)
{
	...
	msm_property_install_enum(&sde_crtc->property_info,
				  "fence_error_handle_flag", 0, 0,
				  e_fence_error_handle_flag,
	...
	if (catalog->dspp_count) {
		...
		if (catalog->rc_count) {
			sde_kms_info_add_keyint(info, "rc_count",
						catalog->rc_count);
			sde_kms_info_add_keyint(
				info, "rc_mem_size",
	...
	sde_kms_info_add_keyint(info, "use_baselayer_for_stage",
				test_bit(SDE_FEATURE_BASE_LAYER,
					 catalog->features));
...
{
	enum sde_intf_mode intf_mode =
		sde_crtc_get_intf_mode(crtc, crtc->state);
				      ...
				      uint64_t *val)
{
	...
	/*
   * If hw-fence is enabled, find hw_ctl and pass it to sde_fence_create, this
   * will attempt to create a hw-fence for this ctl, whereas if hw_ctl is not
   * passed to sde_fence, this won't use hw-fences for this output-fence.
   */
	if (!is_wb &&
	    test_bit(HW_FENCE_OUT_FENCES_ENABLE,
		     sde_crtc->hwfence_features_mask) &&
					...
					uint64_t val)
{
	...
	case CRTC_PROP_FRAME_DATA_BUF:
		_sde_crtc_set_frame_data_buffers(crtc, cstate,
						 (void __user *)(uintptr_t)val);
		break;
	default:
		/* nothing to do */
	...
	if (ret) {
		if (ret != -EPERM)
			SDE_ERROR("%s: failed to set property%d %s: %d\n",
				  crtc->name, DRMID(property), property->name,
				  ret);
		else
...
static void sde_crtc_update_line_time(struct drm_crtc *crtc)
{
	...
	drm_for_each_encoder_mask(encoder, crtc->dev,
				  crtc->state->encoder_mask) {
		if (sde_encoder_check_curr_mode(encoder, MSM_DISPLAY_CMD_MODE))
			sde_encoder_get_transfer_time(encoder,
						      &min_transfer_time);
	}
	...
	if (min_transfer_time) {
		...
		sde_crtc->line_time_in_ns = DIV_ROUND_UP(
			1000000000, updated_fps * crtc->mode.vdisplay);
	} else {
	...
	SDE_EVT32(min_transfer_time, updated_fps, crtc->mode.vdisplay,
		  crtc->mode.vtotal, drm_mode_vrefresh(&crtc->mode),
		  sde_crtc->line_time_in_ns);
}
	...
	if (i == CRTC_PROP_OUTPUT_FENCE) {
		*val = ~0;
		...
		ret = msm_property_atomic_get(&sde_crtc->property_info,
					      &cstate->property_state, property,
					      val);
		if (ret)
					    ...
					    struct drm_crtc_state *crtc_state)
{
	...
	for (prop_idx = 0; prop_idx < CRTC_PROP_COUNT; prop_idx++) {
		...
		SDE_DEBUG("%s: set prop %s idx %d from %llu to %llu\n",
			  sde_crtc->name, drm_prop->name, prop_idx, val, def);
			    ...
			    struct sde_crtc_misr_info *crtc_misr_info)
{
	...
	sde_crtc = to_sde_crtc(crtc);
	crtc_misr_info->misr_enable = sde_crtc->misr_enable_debugfs ? true :
								      false;
...
static int _sde_debugfs_status_show(struct seq_file *s, void *data)
{
	...
	mutex_lock(&sde_crtc->crtc_lock);
	mode = &crtc->state->adjusted_mode;
	sde_crtc_get_mixer_resolution(crtc, crtc->state, mode, &mixer_width,
				      &mixer_height);
	...
	for (i = 0; i < cstate->num_dim_layers; i++) {
		...
		seq_printf(s, "\tdim_layer:%d] stage:%d flags:%d\n", i,
			   dim_layer->stage, dim_layer->flags);
	...
	if (sde_crtc->vblank_cb_count) {
		...
		seq_printf(
			...
			fps, sde_crtc->vblank_cb_count, ktime_to_ms(diff),
			sde_crtc->play_count);
...
static ssize_t
_sde_debugfs_hw_fence_features_mask_wr(struct file *file,
				       ...
				       size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%u %u", &bit, &enable) != 2) {
		SDE_ERROR(
			"incorrect usage: expected 2 parameters, bit and enable\n");
		return -EINVAL;
	}
						      ...
						      loff_t *ppos)
{
	...
	for (i = HW_FENCE_OUT_FENCES_ENABLE; i < HW_FENCE_FEATURES_MAX; i++) {
		len += scnprintf(buf + len, 256 - len, "bit %d: %d\n", i,
				 test_bit(i, sde_crtc->hwfence_features_mask));
	}
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (!sde_crtc->misr_enable_debugfs) {
		len += scnprintf(buf + len, MISR_BUFF_SIZE - len, "disabled\n");
		goto buff_check;
	}
...
static int _sde_crtc_init_debugfs(struct drm_crtc *crtc)
{
	...
	sde_crtc->debugfs_root = debugfs_create_dir(
		sde_crtc->name, crtc->dev->primary->debugfs_root);
	...
	debugfs_create_file("status", 0400, sde_crtc->debugfs_root, sde_crtc,
			    &debugfs_status_fops);
	debugfs_create_file("state", 0400, sde_crtc->debugfs_root,
			    &sde_crtc->base, &sde_crtc_debugfs_state_fops);
	...
	if (sde_kms->catalog->hw_fence_rev) {
		debugfs_create_file("hwfence_features_mask", 0600,
				    sde_crtc->debugfs_root, &sde_crtc->base,
				    &debugfs_hw_fence_features_fops);
		debugfs_create_u32("hwfence_out_fences_skip", 0600,
...
static void vblank_ctrl_worker(struct kthread_work *work)
{
	struct vblank_work *cur_work =
		container_of(work, struct vblank_work, work);
...
static int vblank_ctrl_queue_work(struct msm_drm_private *priv, int crtc_id,
				  bool enable)
{
	struct vblank_work *cur_work;
	struct drm_crtc *crtc;
				 ...
				 bool is_vidmode)
{
	...
	if (!test_bit(SDE_SYS_CACHE_DISP,
		      sde_kms->catalog->sde_sys_cache_type_map)) {
	...
	if (test_bit(SDE_SYS_CACHE_DISP_1,
		     sde_kms->catalog->sde_sys_cache_type_map) &&
	...
	SDE_EVT32(DRMID(crtc), state, sde_crtc->cache_state,
		  sde_crtc->cache_type);

	sde_crtc->cache_state = state;
	drm_atomic_crtc_for_each_plane(plane, crtc)
		sde_plane_static_img_control(plane, sde_crtc->cache_state,
					     sde_crtc->cache_type);
}
...
void sde_crtc_cancel_delayed_work(struct drm_crtc *crtc)
{
	...
	cache_status = kthread_cancel_delayed_work_sync(
		&sde_crtc->static_cache_read_work);
...
struct drm_crtc *sde_crtc_init(struct drm_device *dev, struct drm_plane *plane)
{
	...
	sde_crtc->fps_info.time_buf =
		kmalloc_array(MAX_FRAME_COUNT, sizeof(ktime_t), GFP_KERNEL);
	...
	if (kms->catalog->hw_fence_rev) {
		set_bit(HW_FENCE_OUT_FENCES_ENABLE,
			sde_crtc->hwfence_features_mask);
		set_bit(HW_FENCE_IN_FENCES_ENABLE,
			sde_crtc->hwfence_features_mask);
	}
	...
	SDE_DEBUG(
		...
		test_bit(HW_FENCE_OUT_FENCES_ENABLE,
			 sde_crtc->hwfence_features_mask),
		test_bit(HW_FENCE_IN_FENCES_ENABLE,
			 sde_crtc->hwfence_features_mask));
	return crtc;
}
...
void sde_crtc_update_cont_splash_settings(struct drm_crtc *crtc)
{
	...
	sde_crtc->cur_perf.core_clk_rate =
		(rate > 0) ? rate : kms->perf.max_core_clk_rate;
...
void _sde_crtc_vm_release_notify(struct drm_crtc *crtc)
{
	...
	sde_crtc_event_notify(crtc, DRM_EVENT_VM_RELEASE, &val,
			      sizeof(uint32_t));
}

void sde_crtc_calc_vpadding_param(struct drm_crtc_state *state, u32 crtc_y,
				  uint32_t crtc_h, u32 *padding_y,
				  u32 *padding_start, u32 *padding_height)
{
	struct sde_kms *kms;
	struct sde_crtc_state *cstate = to_sde_crtc_state(state);
   ...
   */
	...
	*padding_height = y_end - y_start + 1;
	SDE_EVT32(DRMID(cstate->base.crtc), y_remain, y_start, y_end,
		  *padding_y, *padding_start, *padding_height);
	SDE_DEBUG("crtc:%d padding_y:%d padding_start:%d padding_height:%d\n",
		  DRMID(cstate->base.crtc), *padding_y, *padding_start,
		  *padding_height);
}
