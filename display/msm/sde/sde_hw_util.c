...
static void
_sde_hw_setup_scaler3lite_lut(struct sde_hw_blk_reg_map *c,
			      struct sde_hw_scaler3_cfg *scaler3_cfg,
			      u32 offset)
{
	int i, filter;
	...
	if (config_lut) {
		for (filter = 0; filter < QSEED3LITE_FILTERS; filter++) {
			...
			lut_offset = 0;
			lut_addr =
				QSEED3L_COEF_LUT_OFF + offset + off_tbl[filter];
				     ...
				     u32 offset)
{
	...
	de_thr = (de_cfg->thr_low & 0x3FF) | ((de_cfg->thr_high & 0x3FF) << 16);
...
static inline scaler_lut_type
get_scaler_lut(struct sde_hw_scaler3_cfg *scaler3_cfg, u32 scaler_version)
{
...
void sde_hw_setup_scaler3(struct sde_hw_blk_reg_map *c,
			  struct sde_hw_scaler3_cfg *scaler3_cfg,
			  u32 scaler_version, u32 scaler_offset,
			  const struct sde_format *format, bool de_lpf)
{
	u32 op_mode = 0;
	...
	if (scaler_version == 0x1002) {
		phase_init = ((scaler3_cfg->init_phase_x[0] & 0x3F) << 0) |
...
void sde_hw_csc_setup(struct sde_hw_blk_reg_map *c, u32 csc_reg_off,
 ...
 */
uint32_t sde_copy_formats(struct sde_format_extended *dst_list,
			  uint32_t dst_list_size, uint32_t dst_list_pos,
