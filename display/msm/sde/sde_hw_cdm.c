...
static struct sde_csc_cfg rgb2yuv_cfg = {
	{ 0x0083, 0x0102, 0x0032, 0x1fb5, 0x1f6c, 0x00e1, 0x00e1, 0x1f45,
	  0x1fdc },
	{ 0x00, 0x00, 0x00 },
...
static struct sde_cdm_cfg *_cdm_offset(enum sde_cdm cdm, struct sde_mdss_cfg *m,
				       void __iomem *addr,
				 ...
				 struct sde_hw_cdm_cfg *cfg)
{
	...
	case CDM_CDWN_COSITE:
		...
		SDE_REG_WRITE(c, CDM_CDWN2_COEFF_COSITE_H_0, cosite_h_coeff[0]);
		SDE_REG_WRITE(c, CDM_CDWN2_COEFF_COSITE_H_1, cosite_h_coeff[1]);
		SDE_REG_WRITE(c, CDM_CDWN2_COEFF_COSITE_H_2, cosite_h_coeff[2]);
		break;
	...
	case CDM_CDWN_COSITE:
		...
		SDE_REG_WRITE(c, CDM_CDWN2_COEFF_COSITE_V, cosite_v_coeff[0]);
		break;
	case CDM_CDWN_OFFSITE:
		...
		SDE_REG_WRITE(c, CDM_CDWN2_COEFF_OFFSITE_V, offsite_v_coeff[0]);
		break;
	default:
		return -EINVAL;
	}
	...
	SDE_REG_WRITE(c, CDM_CDWN2_CLAMP_OUT, ((0x3FF << 16) | 0x0));

	return 0;
}

int sde_hw_cdm_enable(struct sde_hw_cdm *ctx, struct sde_hw_cdm_cfg *cdm)
{
	struct sde_hw_blk_reg_map *c = &ctx->hw;
...
static void sde_hw_cdm_bind_pingpong_blk(struct sde_hw_cdm *ctx, bool enable,
...
static void _setup_cdm_ops(struct sde_hw_cdm_ops *ops, unsigned long features)
{
...
struct sde_hw_blk_reg_map *sde_hw_cdm_init(enum sde_cdm idx, void __iomem *addr,
