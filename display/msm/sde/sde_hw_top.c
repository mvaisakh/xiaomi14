...
#define HW_FENCE_IPCC_PROTOCOLp_CLIENTc_SEND(ba, p, c) \
	((ba + 0xc) + (0x40000 * p) + (0x1000 * c))
#define HW_FENCE_IPCC_PROTOCOLp_CLIENTc_RECV_ID(ba, p, c) \
	((ba + 0x10) + (0x40000 * p) + (0x1000 * c))
...
#define MDP_CTL_FENCE_ISR_OP_CODE(opcode, op0, op1, op2)                     \
	(((op2 & 0xff) << 24) | ((op1 & 0xff) << 16) | ((op0 & 0xff) << 8) | \
	 (opcode & 0xff))
...
static int ppb_offset_map[PINGPONG_MAX] = {
	1, 0, 3, 2, 5, 4, 7, 7, 6, 6, -1, -1
};
				    ...
				    struct split_pipe_cfg *cfg)
{
	...
	if (test_bit(SDE_MDP_PERIPH_TOP_0_REMOVED, &mdp->caps->features) &&
	    cfg->en) {
				      ...
				      struct sde_vsync_source_cfg *cfg)
{
	struct sde_hw_blk_reg_map *c;
	u32 reg, i;
	static const u32 pp_offset[PINGPONG_MAX] = { 0xC, 0x8, 0x4, 0x13,
						     0x18 };
...
void sde_hw_reset_ubwc(struct sde_hw_mdp *mdp, struct sde_mdss_cfg *m)
{
	...
	if (IS_UBWC_40_SUPPORTED(ubwc_dec_version) ||
	    IS_UBWC_43_SUPPORTED(ubwc_dec_version)) {
...
void sde_hw_set_vm_sid_v2(struct sde_hw_sid *sid, u32 vm,
...
struct sde_hw_sid *sde_hw_sid_init(void __iomem *addr, u32 sid_len,
...
static void sde_hw_program_cwb_ppb_ctrl(struct sde_hw_mdp *mdp, bool dual,
					bool dspp_out)
{
...
static void sde_hw_set_hdr_plus_metadata(struct sde_hw_mdp *mdp, u8 *payload,
					 u32 len, u32 stream_id)
{
	...
	bool is_4k_aligned = mdp->caps->features & BIT(SDE_MDP_DHDR_MEMPOOL_4K);
...
static void sde_hw_hw_fence_timestamp_ctrl(struct sde_hw_mdp *mdp, bool enable,
					   bool clear)
{
	struct sde_hw_blk_reg_map c;
	u32 val;
...
static void sde_hw_input_hw_fence_status(struct sde_hw_mdp *mdp, u64 *s_val,
					 u64 *e_val)
{
						 ...
						 struct sde_hw_blk_reg_map *c)
{
	...
	val = (protocol_id == HW_FENCE_IPCC_FENCE_PROTOCOL_ID) ?
		      HW_FENCE_DPU_FENCE_PROTOCOL_ID :
		      protocol_id;
	...
	/* configure the start of the FENCE_IDn_ISR ops for input and output fence
   * isr's */
	val = (HW_FENCE_DPU_OUTPUT_FENCE_START_N << 16) |
	      (HW_FENCE_DPU_INPUT_FENCE_START_N & 0xFF);
	...
	val = HW_FENCE_IPCC_PROTOCOLp_CLIENTc_RECV_ID(
		ipcc_base_addr, protocol_id, client_phys_id);
	...
	offset = MDP_CTL_HW_FENCE_ID_OFFSET_n(
		MDP_CTL_HW_FENCE_IDn_ISR,
...
static void sde_hw_setup_hw_fences_config(struct sde_hw_mdp *mdp,
					  u32 protocol_id, u32 client_phys_id,
					  unsigned long ipcc_base_addr)
{
	u32 val, offset;
	struct sde_hw_blk_reg_map c;
	...
	_sde_hw_setup_hw_input_fences_config(protocol_id, client_phys_id,
					     ipcc_base_addr, &c);
	...
	val = HW_FENCE_IPCC_PROTOCOLp_CLIENTc_SEND(ipcc_base_addr, protocol_id,
						   client_phys_id);
	SDE_REG_WRITE(&c, offset, val);
	...
	/* set load_data op*/
	offset = MDP_CTL_HW_FENCE_ID_OFFSET_n(
		MDP_CTL_HW_FENCE_IDn_ISR, HW_FENCE_DPU_OUTPUT_FENCE_START_N);
...
void sde_hw_top_set_ppb_fifo_size(struct sde_hw_mdp *mdp, u32 pp, u32 sz)
{
	...
	val = (ppb_offset_map[pp_index] % 2) ? (val & 0xFFFF) :
					       (val & 0xFFFF0000);
	SDE_REG_WRITE(&c, offset, val | sz);
	spin_unlock(&mdp->slock);
}

static void sde_hw_setup_hw_fences_config_with_dir_write(
	struct sde_hw_mdp *mdp, u32 protocol_id, u32 client_phys_id,
	unsigned long ipcc_base_addr)
{
	u32 val, offset;
	struct sde_hw_blk_reg_map c;
...
static void _setup_mdp_ops(struct sde_hw_mdp_ops *ops, unsigned long cap,
			   u32 hw_fence_rev)
{
	...
	if (hw_fence_rev) {
		...
		ops->hw_fence_input_timestamp_ctrl =
...
struct sde_hw_mdp *sde_hw_mdptop_init(enum sde_mdp idx, void __iomem *addr,
				      const struct sde_mdss_cfg *m)
{
   ...
   */
	...
	if (test_bit(SDE_MDP_PERIPH_TOP_0_REMOVED, &m->mdp[0].features)) {
		...
		sde_dbg_reg_register_dump_range(
			SDE_DBG_NAME, cfg->name, mdp->hw.blk_off,
			mdp->hw.blk_off + MDP_PERIPH_TOP0, mdp->hw.xin_id);

		sde_dbg_reg_register_dump_range(
			SDE_DBG_NAME, name, mdp->hw.blk_off + MDP_SSPP_TOP2,
			mdp->hw.blk_off + mdp->hw.length, mdp->hw.xin_id);

		/* do not use blk_off, following offsets start from  mdp_phys */
		sde_dbg_reg_register_dump_range(
			SDE_DBG_NAME, "hw_fence", MDP_CTL_HW_FENCE_CTRL,
			MDP_CTL_HW_FENCE_ID_OFFSET_m(MDP_CTL_HW_FENCE_IDm_ATTR,
						     5),
			mdp->hw.xin_id);
	} else {
		sde_dbg_reg_register_dump_range(
