...
static inline int
_dnsc_blur_subblk_offset(struct sde_hw_dnsc_blur *hw_dnsc_blur, int s_id,
			 u32 *base)
{
...
static void
_sde_hw_dnsc_blur_gaus_lut_setup(struct sde_hw_dnsc_blur *hw_dnsc_blur,
				 struct sde_drm_dnsc_blur_cfg *cfg, u32 lut_sel)
{
	...
	if (_dnsc_blur_subblk_offset(hw_dnsc_blur, SDE_DNSC_BLUR_GAUS_LUT,
				     &base))
		return;
	...
	if (cfg->flags_h & DNSC_BLUR_GAUS_FILTER) {
		lut_off = lut_sel ? DNSC_BLUR_GAUS_COEF_LUT_H1 :
				    DNSC_BLUR_GAUS_COEF_LUT_H0;
		for (i = 0; i < DNSC_BLUR_COEF_NUM; i++)
			SDE_REG_WRITE(hw, lut_off + (i * 0x4) + base,
				      cfg->coef_hori[i]);
	}

	if (cfg->flags_v & DNSC_BLUR_GAUS_FILTER) {
		lut_off = lut_sel ? DNSC_BLUR_GAUS_COEF_LUT_V1 :
				    DNSC_BLUR_GAUS_COEF_LUT_V0;
		for (i = 0; i < DNSC_BLUR_COEF_NUM; i++)
			SDE_REG_WRITE(hw, lut_off + (i * 0x4) + base,
				      cfg->coef_vert[i]);
	}
...
static void
_sde_hw_dnsc_blur_filter_setup(struct sde_hw_dnsc_blur *hw_dnsc_blur,
			       struct sde_drm_dnsc_blur_cfg *cfg, u32 lut_sel)
{
	struct sde_hw_blk_reg_map *hw = &hw_dnsc_blur->hw;
	u32 val;
	...
	if (cfg->flags_h & DNSC_BLUR_PCMN_FILTER) {
		SDE_REG_WRITE(hw, DNSC_BLUR_PCMN_PHASE_INIT_H,
			      cfg->phase_init_h);
		SDE_REG_WRITE(hw, DNSC_BLUR_PCMN_PHASE_STEP_H,
			      cfg->phase_step_h);
	}

	if (cfg->flags_v & DNSC_BLUR_PCMN_FILTER) {
		SDE_REG_WRITE(hw, DNSC_BLUR_PCMN_PHASE_INIT_V,
			      cfg->phase_init_v);
		SDE_REG_WRITE(hw, DNSC_BLUR_PCMN_PHASE_STEP_V,
			      cfg->phase_step_v);
	}
				    ...
				    u32 lut_sel)
{
	...
	opmode |= (cfg->flags & DNSC_BLUR_RND_8B_EN) ?
			  DNSC_BLUR_OPMODE_OUT_RND_8B_EN :
			  0;

	if (cfg->flags_h) {
		opmode |= DNSC_BLUR_OPMODE_DWNS_H_EN;
		opmode |= (cfg->flags_h & DNSC_BLUR_PCMN_FILTER) ?
				  DNSC_BLUR_OPMODE_PCMN_H :
				  0;
	}

	if (cfg->flags_v) {
		opmode |= DNSC_BLUR_OPMODE_DWNS_V_EN;
		opmode |= (cfg->flags_v & DNSC_BLUR_PCMN_FILTER) ?
				  DNSC_BLUR_OPMODE_PCMN_V :
				  0;
	}
	...
	SDE_REG_WRITE(hw, DNSC_BLUR_OUT_IMG_SIZE,
		      (cfg->dst_height << 16) | cfg->dst_width);
	SDE_REG_WRITE(hw, DNSC_BLUR_SRC_IMG_SIZE,
		      (cfg->src_height << 16) | cfg->src_width);
}

static void
_sde_hw_dnsc_blur_dither_setup(struct sde_hw_dnsc_blur *hw_dnsc_blur,
			       struct sde_drm_dnsc_blur_cfg *cfg)
{
	struct sde_hw_blk_reg_map *hw = &hw_dnsc_blur->hw;
	int i;
...
static void
_sde_hw_dnsc_blur_bind_pingpong_blk(struct sde_hw_dnsc_blur *hw_dnsc_blur,
				    ...
				    bool cwb)
{
	...
	if (enable)
		mux_cfg = cwb ? (pp < PINGPONG_CWB_2 ? 0xd : 0xb) :
				(pp - PINGPONG_0) & 0x7;
	else
		mux_cfg = 0xF;
...
static void _setup_dnsc_blur_ops(struct sde_hw_dnsc_blur_ops *ops,
						 ...
						 struct sde_mdss_cfg *m)
{
	...
	if (cfg->sblk->gaus_lut.base && cfg->sblk->gaus_lut.len)
		sde_dbg_reg_register_dump_range(
			SDE_DBG_NAME, cfg->sblk->gaus_lut.name,
			c->hw.blk_off + cfg->sblk->gaus_lut.base,
			c->hw.blk_off + cfg->sblk->gaus_lut.base +
				cfg->sblk->gaus_lut.len,
			c->hw.xin_id);
