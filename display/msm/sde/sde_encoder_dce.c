...
#define SDE_DEBUG_DCE(e, fmt, ...) \
	SDE_DEBUG("enc%d " fmt, (e) ? (e)->base.base.id : -1, ##__VA_ARGS__)

#define SDE_ERROR_DCE(e, fmt, ...) \
				   ...
				   int frame_width, int frame_height)
{
	...
	if ((frame_width % vdc->slice_width) ||
		...
		SDE_ERROR("pic_dim=%dx%d has to be multiple of slice=%dx%d\n",
			  frame_width, frame_height, vdc->slice_width,
			  vdc->slice_height);
		return -EINVAL;
	}
				      ...
				      int enc_ip_width, int dsc_cmn_mode)
{
	...
	container_slice_width = (native_422 ? dsc->config.slice_width / 2 :
	...
	input_ssm_out_latency =
		pipeline_latency +
		3 * (max_ssm_delay + 2) * dsc->num_active_ss_per_enc;
	rtl_num_components = (native_420 || native_422) ? 4 : 3;
	ob_data_width_4comps =
		(rtl_output_data_width >= (2 * max_muxword_size)) ?
	...
	multi_hs_d = (dsc->num_active_ss_per_enc > 1) *
		     (ob_data_width > compress_bpp_group);
	multi_hs_extra_budget_bits =
		multi_hs_c ? chunk_bits :
			     (multi_hs_d ? chunk_bits :
					   output_rate_extra_budget_bits);
	multi_hs_extra_latency =
		DIV_ROUND_UP(multi_hs_extra_budget_bits, compress_bpp_group);
	dsc->initial_lines =
		DIV_ROUND_UP((base_hs_latency + multi_hs_extra_latency),
...
static void _dce_dsc_pipe_cfg(struct sde_hw_dsc *hw_dsc,
			      ...
			      struct msm_display_dsc_info *dsc, u32 common_mode,
			      bool ich_reset, struct sde_hw_pingpong *hw_dsc_pp,
...
static inline bool
_dce_check_half_panel_update(int num_lm, struct sde_encoder_virt *sde_enc)
{
	/**
...
static int _dce_dsc_setup_single(
	struct sde_encoder_virt *sde_enc, struct msm_display_dsc_info *dsc,
	unsigned long affected_displays, int index, const struct sde_rect *roi,
	int dsc_common_mode, bool merge_3d, bool disable_merge_3d, bool mode_3d,
	bool dsc_4hsmerge, bool half_panel_partial_update, int ich_res)
{
   ...
   */
	...
	SDE_EVT32(DRMID(&sde_enc->base), roi->w, roi->h, dsc_common_mode, index,
		  active, merge_3d, disable_merge_3d, dsc_4hsmerge);
	...
	SDE_DEBUG_DCE(sde_enc, "update_intf_cfg hw_ctl[%d], dsc:%d, %s %d\n",
		      hw_ctl->idx, cfg.dsc[0], active ? "enabled" : "disabled",
		      half_panel_partial_update);
				 ...
				 enum sde_rm_topology_name topology)
{
	...
	merge_3d = ((mode_3d != BLEND_3D_NONE) &&
	...
	dsc_merge = ((num_dsc > num_intf) && !dsc->half_panel_pu &&
		     !(enc_master->hw_intf->cfg.split_link_en)) ?
			    true :
			    false;
	disable_merge_3d = (merge_3d && dsc->half_panel_pu) ? false : true;
	dsc_4hsmerge = (dsc_merge && num_dsc == 4 && num_intf == 1) ? true :
								      false;
   ...
   */
	...
	for (i = 0; i < num_dsc; i++) {
		rc = _dce_dsc_setup_single(sde_enc, dsc, affected_displays, i,
					   ...
					   dsc_4hsmerge, dsc->half_panel_pu,
					   ich_res);
		if (rc)
			break;
	}
			  ...
			  struct sde_encoder_kickoff_params *params)
{
   ...
   */
	...
	half_panel_partial_update =
		_dce_check_half_panel_update(num_lm, sde_enc);
	...
	is_video_mode = sde_encoder_check_curr_mode(&sde_enc->base,
     ...
     */
		...
		if (!hw_vdc[i]) {
			...
			SDE_EVT32(DRMID(&sde_enc->base), roi->w, roi->h, i,
				  active);
			return -EINVAL;
		}

		_dce_vdc_pipe_cfg(hw_vdc[i], hw_pp[i], vdc, mode_3d,
				  disable_merge_3d, active, is_video_mode);
		...
		SDE_DEBUG_DCE(sde_enc, "update_intf_cfg hw_ctl[%d], vdc:%d, %s",
			      hw_ctl->idx, cfg.vdc[0],
		...
		if (mode_3d) {
			...
			SDE_DEBUG("mode_3d %s, on CTL_%d PP-%d merge3d:%d\n",
				  disable_merge_3d ? "disabled" : "enabled",
				  hw_ctl->idx - CTL_0,
...
static void _dce_dsc_disable(struct sde_encoder_virt *sde_enc)
{
   ...
   */
	...
	for (i = 0; i < MAX_CHANNELS_PER_ENC; i++) {
		...
		_dce_dsc_pipe_cfg(hw_dsc, hw_pp, NULL, 0, 0, hw_dsc_pp,
...
static void _dce_vdc_disable(struct sde_encoder_virt *sde_enc)
{
	...
	for (i = 0; i < MAX_CHANNELS_PER_ENC; i++) {
		...
		_dce_vdc_pipe_cfg(hw_vdc, hw_pp, NULL, BLEND_3D_NONE, false,
				  false, is_video_mode);
