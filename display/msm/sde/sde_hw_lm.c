...
static struct sde_lm_cfg *_lm_offset(enum sde_lm mixer, struct sde_mdss_cfg *m,
				     void __iomem *addr,
...
static void
sde_hw_lm_setup_blend_config_combined_alpha(struct sde_hw_mixer *ctx, u32 stage,
					    u32 fg_alpha, u32 bg_alpha,
					    u32 blend_op)
{
	struct sde_hw_blk_reg_map *c = &ctx->hw;
	int stage_off;
...
static void sde_hw_lm_setup_blend_config(struct sde_hw_mixer *ctx, u32 stage,
					 u32 fg_alpha, u32 bg_alpha,
					 u32 blend_op)
{
	struct sde_hw_blk_reg_map *c = &ctx->hw;
...
static void sde_hw_lm_gc(struct sde_hw_mixer *mixer, void *cfg)
{
}
...
static void sde_hw_lm_setup_misr(struct sde_hw_mixer *ctx, bool enable,
				 u32 frame_count)
{
	struct sde_hw_blk_reg_map *c = &ctx->hw;
	u32 config = 0;
				       ...
				       struct sde_hw_noise_layer_cfg *cfg)
{
	...
	if (cfg->noise_blend_stage == SDE_STAGE_BASE ||
		...
		SDE_ERROR(
			"invalid noise_blend_stage %d attn_blend_stage %d max stage %d\n",
			cfg->noise_blend_stage, cfg->attn_blend_stage,
			sblk->maxblendstages);
		return -EINVAL;
	}
	...
	if (stage_off < 0) {
		SDE_ERROR(
			"invalid stage_off:%d for noise layer blend stage:%d\n",
	...
	if (stage_off < 0) {
		SDE_ERROR(
			"invalid stage_off:%d for atten layer blend stage:%d\n",
...
static void _setup_mixer_ops(struct sde_mdss_cfg *m, struct sde_hw_lm_ops *ops,
...
struct sde_hw_blk_reg_map *sde_hw_lm_init(enum sde_lm idx, void __iomem *addr,
