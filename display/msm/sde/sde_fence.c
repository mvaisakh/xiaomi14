...
/**
 * hw_fence_data_dpu_client - this table maps the dpu ipcc input and output
 * signals for each display clients to communicate with the fence controller.
 * This struct must match the order of the 'sde_hw_fence_clients' enum,
 * the output signal must match with the signals that FenceCTL expects for each
 * display client. This 'hw_fence_data_dpu_client' must be used for HW that does
 * not support dpu-signal.
 */
 ...
 * This struct must match the order of the 'sde_hw_fence_clients' enum,
 * the output signal must match with the signals that FenceCTL expects for each
 * display client. This 'hw_fence_data_dpu_client' must be used for HW that
 * supports dpu-signal
 */
...
void msm_hw_fence_error_cb(u32 handle, int error, void *cb_data)
{
	...
	sde_hw_fence_error_data = (struct sde_hw_fence_error_cb_data
					   *)(msm_hw_fence_cb_data->data);
	...
	sde_encoder_handle_hw_fence_error(sde_hw_fence_error_data->ctl_idx,
					  sde_hw_fence_error_data->sde_kms,
					  handle, error);
}

int sde_hw_fence_init(struct sde_hw_ctl *hw_ctl, struct sde_kms *sde_kms,
		      bool use_dpu_ipcc, struct msm_mmu *mmu)
{
	struct msm_hw_fence_hfi_queue_header *hfi_queue_header_va,
		*hfi_queue_header_pa;
	...
	sde_hw_fence_data = use_dpu_ipcc ? hw_fence_data_dpu_client :
					   hw_fence_data_no_dpu;
	...
	memcpy(hwfence_data, &sde_hw_fence_data[ctl_id],
	       sizeof(struct sde_hw_fence_data));

	SDE_DEBUG("hwfence register ctl:%d client:%d\n", ctl_id,
		  hwfence_data->hw_fence_client_id);
	hwfence_data->hw_fence_handle =
		msm_hw_fence_register(hwfence_data->hw_fence_client_id,
	...
	if (IS_ERR_OR_NULL(hwfence_data->hw_fence_handle)) {
		...
		SDE_DEBUG(
			"error cannot register ctl_id:%d hw-fence client:%d\n",
			ctl_id, hwfence_data->hw_fence_client_id);
		return -EINVAL;
	}

	sde_hw_fence_error_cb_data =
		&(hwfence_data->sde_hw_fence_error_cb_data);
	sde_hw_fence_error_cb_data->ctl_idx = hw_ctl->idx;
	sde_hw_fence_error_cb_data->sde_kms = sde_kms;

	ret = msm_hw_fence_register_error_cb(
		hwfence_data->hw_fence_handle, msm_hw_fence_error_cb,
		(void *)sde_hw_fence_error_cb_data);
	if (ret) {
	...
	ret = mmu->funcs->one_to_one_map(
		mmu, ctl_hfi_iova, hwfence_data->mem_descriptor.device_addr,
		hwfence_data->mem_descriptor.size, IOMMU_READ | IOMMU_WRITE);
	if (ret) {
		SDE_ERROR(
			"queue one2one memory smmu map failed, ret:%d ctl_id:%d, client:%d\n",
	...
	hfi_table_header =
		(struct msm_hw_fence_hfi_queue_table_header *)queue_va;
	qhdr0_offset = hfi_table_header->qhdr0_offset;

	/* initialize tx_wm pointer */
	hfi_queue_header_va =
		(struct msm_hw_fence_hfi_queue_header *)(queue_va +
							 qhdr0_offset);
	...
	hfi_queue_header_pa =
		(struct msm_hw_fence_hfi_queue_header *)(queue_pa +
							 qhdr0_offset);
	hwfence_data->txq_wr_ptr_pa = &hfi_queue_header_pa->write_index;

	SDE_DEBUG("hwfence registered ctl:%d client:%d handle:0x%pK tx_wm:0x%x "
		  "wr_idx:0x%x\n",
		  ctl_id, hwfence_data->hw_fence_client_id,
		  hwfence_data->hw_fence_handle, *hwfence_data->txq_tx_wm_va,
		  *hwfence_data->txq_wr_ptr_pa);
...
static int sde_fence_create_hw_fence(struct sde_hw_ctl *hw_ctl,
				     struct sde_fence *sde_fence)
{
	...
	if (ret) {
		SDE_ERROR(
			...
			ctl_id, sde_fence->base.context, sde_fence->base.seqno);
	} else {
		...
		SDE_DEBUG(
			...
			sde_fence->hwfence_index, ctl_id,
...
int sde_fence_register_hw_fences_wait(struct sde_hw_ctl *hw_ctl,
				      struct dma_fence **fences, u32 num_fences)
{
	...
	SDE_DEBUG("register for wait fences:%d ctl_id:%d hw_fence_client:%s\n",
		  num_fences, ctl_id,
		  _get_client_id_name(data->hw_fence_client_id));
	...
	for (i = 0; i < num_fences; i++) {
		...
		if (dma_fence_is_array(fences[i])) {
			array_count++;
			array = container_of(fences[i], struct dma_fence_array,
					     base);
	...
	if (num_fences > 1) {
		...
		fence_list =
			kzalloc(((num_fences - array_count) + array_childs) *
		...
		for (i = 0; i < num_fences; i++) {
			if (dma_fence_is_array(fences[i])) {
					...
					SDE_EVT32(
						...
						SDE_EVTLOG_L32(
							array->fences[0]
								->context),
						SDE_EVTLOG_H32(
							array->fences[0]->seqno),
						SDE_EVTLOG_L32(
							array->fences[0]
								->seqno));
				else
					SDE_EVT32(ctl_id, num_fences,
						  array->num_fences, i,
						  SDE_EVTLOG_ERROR);
				...
				SDE_EVT32(ctl_id, num_fences, i,
					  SDE_EVTLOG_H32(fences[i]->context),
		...
		temp_array =
			dma_fence_array_create(fence_list_index, fence_list,
					       data->dma_context, seqno, 0);
		if (!temp_array) {
			SDE_ERROR(
				"unable to create fence array, cant register for wait\n");
			_cleanup_fences_refcount(fences, num_fences);
			kfree(fence_list);
			return -EINVAL;
		}
		SDE_EVT32(ctl_id, fence_list_index,
			  SDE_EVTLOG_H32(data->dma_context),
			  SDE_EVTLOG_L32(data->dma_context),
			  SDE_EVTLOG_H32(seqno), SDE_EVTLOG_L32(seqno));
		...
		tmp_array = dma_fence_is_array(fences[0]) ?
				    container_of(fences[0],
						 struct dma_fence_array, base) :
				    NULL;
		SDE_EVT32(ctl_id, num_hw_fences,
			  SDE_EVTLOG_H32(fences[0]->context),
			  SDE_EVTLOG_L32(fences[0]->context),
			  SDE_EVTLOG_H32(fences[0]->seqno),
			  SDE_EVTLOG_L32(fences[0]->seqno), fences[0]->flags,
			  tmp_array ? tmp_array->num_fences :
				      SDE_EVTLOG_FUNC_CASE2);
	}

	/* register for wait */
	ret = msm_hw_fence_wait_update(data->hw_fence_handle, hw_fences,
				       num_hw_fences, true);
	if (ret)
		SDE_ERROR(
			"failed to register wait fences for ctl_id:%d ret:%d\n",
			ctl_id, ret);

	/* fence-array put will release each individual extra refcount during array
   * release */
	if (temp_array)
...
static int _arm_output_hw_fence(struct sde_hw_ctl *hw_ctl, bool vid_mode,
				u32 line_count, u32 debugfs_hw_fence)
{
	...
	ctl_id = hw_ctl->idx - CTL_0;
	data = &hw_ctl->hwfence_data;
	if (data->ipcc_out_signal_pp_idx >=
	...
	hw_ctl->ops.hw_fence_update_output_fence(hw_ctl, data->ipcc_out_client,
						 ipcc_out_signal);
	SDE_EVT32_VERBOSE(ctl_id, ipcc_out_signal);

	/* arm dpu to trigger output fence signal once ready */
	if (line_count)
		hw_ctl->ops.hw_fence_trigger_output_fence(
			hw_ctl, HW_FENCE_TRIGGER_SEL_PROG_LINE_COUNT);
	else if (vid_mode &&
		 (hw_ctl->caps->features & BIT(SDE_CTL_HW_FENCE_TRIGGER_SEL)))
		hw_ctl->ops.hw_fence_trigger_output_fence(
			hw_ctl, HW_FENCE_TRIGGER_SEL_VID_MODE);
	else
		hw_ctl->ops.hw_fence_trigger_output_fence(
			hw_ctl, HW_FENCE_TRIGGER_SEL_CMD_MODE);

	return 0;
}

static int _sde_fence_arm_output_hw_fence(struct sde_fence_context *ctx,
					  bool vid_mode, u32 line_count,
					  u32 debugfs_hw_fence)
{
	...
	list_for_each_entry_safe(fc, next, &ctx->fence_list_head, fence_list) {
       ...
       */
			SDE_ERROR(
				"invalid hw ctl, this can cause a fence-timeout!\n");
			SDE_EVT32(SDE_EVTLOG_ERROR, SDE_EVTLOG_FUNC_CASE1,
				  fence->flags, fence->context, fence->seqno);
	...
	if (hw_ctl)
		_arm_output_hw_fence(hw_ctl, vid_mode, line_count,
				     debugfs_hw_fence);
...
{
	if (hw_ctl && hw_ctl->ops.hw_fence_output_fence_dir_write_init)
		hw_ctl->ops.hw_fence_output_fence_dir_write_init(
			hw_ctl, hw_ctl->hwfence_data.txq_wr_ptr_pa,
			HW_FENCE_DIR_WRITE_SIZE, HW_FENCE_DIR_WRITE_MASK);
}
				   ...
				   u32 line_count, u32 debugfs_hw_fence)
{
	...
	list_for_each_entry_safe(fc, next, &ctx->fence_list_head, fence_list) {
       ...
       */
			SDE_ERROR(
				"invalid hw ctl, this can cause a fence-timeout!\n");
			SDE_EVT32(SDE_EVTLOG_FUNC_CASE1, fence->flags,
		...
		if (IS_ERR_OR_NULL(data->hw_fence_handle)) {
			SDE_ERROR(
				"unexpected handle for ctl_id:%d, this can fence-timeout\n",
				ctl_id);
			SDE_EVT32(SDE_EVTLOG_FUNC_CASE2, fence->flags,
				  fence->context, fence->seqno, ctl_id,
				  SDE_EVTLOG_ERROR);
			ret = -EINVAL;
			goto exit;
		}
		...
		SDE_EVT32(ctl_id, SDE_EVTLOG_H32(fc->hwfence_index),
			  ...
			  *data->txq_tx_wm_va);
		ret = msm_hw_fence_update_txq(data->hw_fence_handle,
					      fc->hwfence_index, 0, 0);
		if (ret) {
			SDE_ERROR(
				"fail txq update index:%llu fctx:%llu seqno:%llu client:%d\n",
				fc->hwfence_index, fence->context, fence->seqno,
				data->hw_fence_client_id);
			SDE_EVT32(SDE_EVTLOG_FUNC_CASE3, fence->flags,
				  fence->context, fence->seqno, ctl_id,
				  SDE_EVTLOG_ERROR);
			goto exit;
		}
		...
		if (hw_ctl->ops.hw_fence_output_fence_dir_write_data)
			hw_ctl->ops.hw_fence_output_fence_dir_write_data(
				hw_ctl, *data->txq_tx_wm_va);

		/* avoid updating txq more than once and avoid repeating the same fence
     * twice */
		txq_updated = fc->txq_updated_fence = true;

		SDE_DEBUG(
			"update txq fence:0x%pK ctx:%llu seqno:%llu f:0x%llx ctl:%d vid:%d\n",
			fence, fence->context, fence->seqno, fence->flags,
			ctl_id, vid_mode);
...
exit:
   ...
   */
	if ((txq_updated && hw_ctl) || line_count)
		_sde_fence_arm_output_hw_fence(ctx, vid_mode, line_count,
					       debugfs_hw_fence);
...
static void _sde_hw_fence_release(struct sde_fence *f)
{
	...
	if (ret)
		SDE_ERROR(
			"failed to destroy hw_fence for ctl_id:%d ctx:%llu seqno:%llu\n",
			ctl_id, f->base.context, f->base.seqno);
}
					   ...
					   bool disable)
{
	...
	SDE_DEBUG("configure input signal:%d out client:%d ctl_id:%d\n",
		  ipcc_signal_id, ipcc_client_id, ctl_id);

	/* configure dpu hw for the client/signal pair signaling input-fence */
	hw_ctl->ops.hw_fence_update_input_fence(hw_ctl, ipcc_client_id,
						ipcc_signal_id);
...
void sde_fence_error_ctx_update(struct sde_fence_context *ctx,
				int input_fence_status,
				enum sde_fence_error_state sde_fence_error_state)
{
	if (!ctx) {
		SDE_DEBUG("invalid fence\n");
		SDE_EVT32(input_fence_status, sde_fence_error_state,
			  SDE_EVTLOG_ERROR);
		return;
	}
...
void sde_fence_dump(struct dma_fence *fence)
{
	...
	if (fence->ops->timeline_value_str)
		fence->ops->timeline_value_str(fence, timeline_str,
					       TIMELINE_VAL_LENGTH);

	SDE_ERROR("fence drv name:%s timeline name:%s seqno:0x%llx timeline:%s "
		  "signaled:0x%x status:%d flags:0x%x\n",
		  fence->ops->get_driver_name(fence),
		  fence->ops->get_timeline_name(fence), fence->seqno,
		  timeline_str,
		  fence->ops->signaled ? fence->ops->signaled(fence) :
					 0xffffffff,
...
signed long sde_sync_wait(void *fnc, long timeout_ms, int *error_status)
{
	...
	rc = dma_fence_wait_timeout(fence, true, msecs_to_jiffies(timeout_ms));
	...
	list_for_each_entry_safe(fc, next, &ctx->fence_list_head, fence_list) {
		...
		if (fence_error_state) {
			if (fence_error_state == HANDLE_OUT_OF_ORDER &&
			    fence_error_ctx->last_good_frame_fence_seqno ==
				    fc->base.seqno) {
				SDE_EVT32(fence_error_ctx
						  ->last_good_frame_fence_seqno,
					  fence_error_state,
				...
				dma_fence_set_error(
					&fc->base,
					fence_error_ctx->fence_error_status);
				dma_fence_signal_timestamp_locked(&fc->base,
								  ts);
				spin_unlock_irqrestore(&ctx->lock, flags);

				SDE_EVT32(fence_error_state,
					  fence_error_ctx->fence_error_status,
					  ktime_to_us(ts), fc->base.seqno,
					  SDE_EVTLOG_FUNC_CASE2);
	...
	if (fence_event == SDE_FENCE_RESET_TIMELINE) {
		...
		if (hw_ctl)
			_reset_hw_fence_timeline(
				hw_ctl,
				MSM_HW_FENCE_RESET_WITHOUT_ERROR |
			       ...
			       struct seq_file **s)
{
	...
	seq_printf(*s,
		   "drm obj:%s id:%d type:0x%x done_count:%d commit_count:%d\n",
		   obj_name, drm_obj->id, drm_obj->type, ctx->done_count,
		   ctx->commit_count);
