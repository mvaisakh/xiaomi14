...
mi_disp_timming_switch_delayed_work_handler(struct kthread_work *work)
{
	...
	if (dsi_panel->panel_initialized) {
		dsi_panel_tx_cmd_set(dsi_panel,
...
int mi_dsi_set_switch_cmd_before(struct dsi_panel *panel, int fps_mode)
{
	...
	if ((last_fps == 10 && (fps_mode & FPS_MODE_AUTO)) ||
	    ((fps_mode & FPS_MODE_IDLE) &&
	     ((cur_time_h_skew & FPS_VALUE_MASK) == 10) &&
	     (last_fps_mode & FPS_MODE_AUTO))) {
		DSI_DEBUG(
			"[%s] N3 switch 10hz to auto 120hz remove delay cmd, rc=%d\n",
			panel->name, rc);
		return rc;
	}

	if (last_fps == 120 && ((last_fps_mode & FPS_MODE_AUTO) ||
				(last_fps_mode & FPS_MODE_QSYNC))) {
		mi_dsi_update_switch_cmd_N3(
			panel, DSI_CMD_SET_TIMING_SWITCH_FROM_AUTO_UPDATA,
			DSI_CMD_SET_MI_TIMING_SWITCH_FROM_AUTO);
		rc = dsi_panel_tx_cmd_set(
			panel, DSI_CMD_SET_MI_TIMING_SWITCH_FROM_AUTO);
	} else if (last_fps >= 90) {
		mi_dsi_update_switch_cmd_N3(
			panel, DSI_CMD_SET_TIMING_SWITCH_FROM_NORMAL_UPDATA,
			DSI_CMD_SET_MI_TIMING_SWITCH_FROM_NORMAL);
		rc = dsi_panel_tx_cmd_set(
			panel, DSI_CMD_SET_MI_TIMING_SWITCH_FROM_NORMAL);
	} else if (last_fps >= 24) {
		mi_dsi_update_switch_cmd_N3(
			panel, DSI_CMD_SET_TIMING_SWITCH_FROM_SKIP_UPDATA,
			DSI_CMD_SET_MI_TIMING_SWITCH_FROM_SKIP);
		rc = dsi_panel_tx_cmd_set(
			panel, DSI_CMD_SET_MI_TIMING_SWITCH_FROM_SKIP);
	} else if (last_fps >= 1) {
		mi_dsi_update_switch_cmd_N3(
			panel, DSI_CMD_SET_TIMING_SWITCH_FROM_AUTO_UPDATA,
			DSI_CMD_SET_MI_TIMING_SWITCH_FROM_AUTO);
	...
	if (rc)
		DSI_INFO(
			"[%s] N3 failed to send DSI_CMD_SET_MI_TIMING_SWITCH BEFOR cmds, "
			"rc=%d\n",
			panel->name, rc);
						  ...
						  enum dsi_cmd_set_type type)
{
	...
	case DSI_CMD_SET_TIMING_SWITCH:
		...
		is_update_BA_reg = mi_dsi_update_BA_reg(
			panel, update_BA_reg_val, sizeof(update_BA_reg_val));
		break;
...
int mi_dsi_panel_set_gamma_update_reg(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (mi_cfg->nedd_auto_update_gamma) {
		...
		if (rc)
			DISP_ERROR(
				"failed to set DSI_CMD_SET_MI_AUTO_UPDATE_GAMMA\n");
		else
	...
	if (mi_cfg->in_fod_calibration) {
		...
		if (bl_lvl != 0) {
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) ==
			    ...
			    is_hbm_fod_on(panel)) {
				DISP_DEBUG(
					"[%s] skip set backlight %d due to LHBM is on",
					panel->type, bl_lvl);
				return true;
			}
				    ...
				    struct dsi_display_mode *mode)
{
	...
	rc = utils->read_u32_array(utils->data,
				   "mi,mdss-flat-status-control-gamma-26-cfg",
				   gamma_26_cfg, 2);
	if (rc) {
		DISP_DEBUG(
			"mi,mdss-flat-status-control-gamma-26-cfg not defined rc=%d\n",
			rc);
	} else {
		DISP_INFO("FPS: %d, gamma 26 cfg: 0x%02X, 0x%02X\n",
			  mode->timing.refresh_rate, gamma_26_cfg[0],
			  gamma_26_cfg[1]);
	...
	rc = utils->read_u32_array(utils->data,
				   "mi,mdss-flat-status-control-gamma-C0-cfg",
				   gamma_c0_cfg, 2);
	if (rc) {
		DISP_DEBUG(
			"mi,mdss-flat-status-control-gamma-C0-cfg not defined rc=%d\n",
			rc);
	} else {
		DISP_INFO("FPS: %d, gamma c0 cfg: 0x%02X, 0x%02X\n",
			  mode->timing.refresh_rate, gamma_c0_cfg[0],
			  gamma_c0_cfg[1]);
	...
	rc = utils->read_u32_array(utils->data,
				   "mi,mdss-flat-status-control-gamma-DF-cfg",
				   gamma_df_cfg, 2);
	if (rc) {
		DISP_DEBUG(
			"mi,mdss-flat-status-control-gamma-DF-cfg not defined rc=%d\n",
			rc);
	} else {
		DISP_INFO("FPS: %d, gamma df cfg: 0x%02X, 0x%02X\n",
			  mode->timing.refresh_rate, gamma_df_cfg[0],
			  gamma_df_cfg[1]);
					    ...
					    struct dsi_display_mode *mode)
{
	...
	rc = utils->read_u32_array(utils->data,
				   "mi,mdss-flat-status-control-gamma-2F-cfg",
				   gamma_2f_cfg, 2);
	if (rc) {
		DISP_INFO(
			"mi,mdss-flat-status-control-gamma-2F-cfg not defined rc=%d\n",
			rc);
	} else {
		DISP_INFO("FPS: %d, gamma 2F cfg: 0x%02X, 0x%02X\n",
			  mode->timing.refresh_rate, gamma_2f_cfg[0],
			  gamma_2f_cfg[1]);
...
int mi_dsi_panel_write_dsi_cmd_set(struct dsi_panel *panel, int type)
{
	...
	for (i = 0; i < cmd_count; i++) {
		...
		buf_count =
			snprintf(buffer, buf_size, "%02X", cmds->msg.tx_len);
					       ...
					       char *rdbuf, int rdlen)
{
	...
	rc = mipi_dsi_dcs_read(&panel->mipi_device, manufacturer_info_addr,
			       rdbuf, rdlen);
	if (rc < 0) {
						  ...
						  u8 size, int bl_lvl)
{
	...
	for (i = 0; i < size / 2; i++) {
		...
		DISP_DEBUG("gamma update tmp %d [%d] %d 0x%x  %x\n", i, bl_lvl,
			   dev_index, update_val[i * 2], update_val[i * 2 + 1]);
	}
}
					     ...
					     int bl_lvl, bool lhbm_is_0size)
{
	...
	DISP_DEBUG("[%s] bl_lvl = %d, alpha = 0x%x%x type=%d\n", panel->type,
		   bl_lvl, alpha_buf[0], alpha_buf[1], type);
	if (lhbm_is_0size)
		size_set = 0x01;
	else
		size_set = 0x03;
	...
	for (i = 0; i < length; i++) {
		update_rgb_reg[i] =
			mi_cfg->lhbm_rgb_param[i % 6 + lhbm_data_index];
		DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n", panel->type, i,
			   update_rgb_reg[i]);
	}
	update_rgb_reg_ptr = update_rgb_reg;
	if (is_cal_lhbm_param)
		mi_dsi_panel_cal_lhbm_param_N3(
			lhbm_data_index, update_rgb_reg_ptr, length, bl_lvl);
	...
	for (i = 0; i < cmd_update_count; i++) {
		...
		if (info && info->mipi_address == 0x63 &&
			...
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
						    alpha_buf,
						    sizeof(alpha_buf));
		} else if (info && info->mipi_address == 0xC5 &&
			   info->length == length) {
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
						    update_rgb_reg, length);
	...
	for (i = 0; i < cmd_update_count; i++) {
		...
		if (info && info->mipi_address == mi_cfg->gamma_rgb_param[0] &&
			...
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[17] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 18];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[34] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 35];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[51] &&
			   info->length == 2) {
			for (j = 0; j < 2; j++) {
				update_rgb_reg2[j] =
					mi_cfg->gamma_rgb_param[j + 52];
			...
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[54] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 55];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[71] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 72];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[88] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 89];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[105] &&
			   info->length == 2) {
			for (j = 0; j < 2; j++) {
				update_rgb_reg2[j] =
					mi_cfg->gamma_rgb_param[j + 106];
				DISP_DEBUG("[%s] update_rgb_reg2[%d] = %d \n",
					   panel->type, j, update_rgb_reg2[j]);
			}
			update_rgb_reg_ptr2 = update_rgb_reg2;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[108] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 109];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[125] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 126];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[142] &&
			   info->length == length) {
			for (j = 0; j < length; j++) {
				update_rgb_reg[j] =
					mi_cfg->gamma_rgb_param[j + 143];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg[j]);
			}
			update_rgb_reg_ptr = update_rgb_reg;
			mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
			   ...
			   info->mipi_address == mi_cfg->gamma_rgb_param[159] &&
			   info->length == 2) {
			for (j = 0; j < 2; j++) {
				update_rgb_reg2[j] =
					mi_cfg->gamma_rgb_param[j + 160];
				DISP_DEBUG("[%s] update_rgb_reg[%d] = %d \n",
					   panel->type, j, update_rgb_reg2[j]);
			}
			update_rgb_reg_ptr2 = update_rgb_reg2;
     ...
     */
		for (i = 0; i < size / 2; i++) {
			...
			if (type ==
				...
				DISP_DEBUG(
					"gamma update：tmp[%d] * 500 / 420\n",
					tmp);
				tmp = tmp * 500 / 420;
			} else {
				DISP_DEBUG(
					"gamma update：tmp[%d] * 530 / 420\n",
					tmp);
				tmp = tmp * 530 / 420;
			}
		...
		DISP_DEBUG(
			"type_index = %d,bl_index = %d,coefficient_index = %d\n",
		...
		for (i = 0; i < size / 2; i++) {
			...
			if (panel->id_config.build_id == N2_PANEL_PA_P11_02) {
				...
				tmp = tmp *
				      lhbm_coefficient_N2_PANEL_PA_P11_02
					      [coefficient_index] /
				      420;
				...
				DISP_DEBUG(
					...
					tmp, coefficient_index);
				tmp = tmp *
				      lhbm_coefficient_N2_PANEL_PA_P2
					      [coefficient_index] /
				      420;
			}
					     ...
					     int flat_mode, int bl_lvl)
{
	...
	if (bl_lvl < 327) {
		if (panel->id_config.build_id >= N2_PANEL_PA_P11_02) {
			alpha_buf[0] =
				(aa_alpha_N2_PANEL_PA_P11_02[bl_lvl] >> 8) &
				0xFF;
			alpha_buf[1] = aa_alpha_N2_PANEL_PA_P11_02[bl_lvl] &
				       0xFF;
		} else {
			alpha_buf[0] = (aa_alpha_N2_PANEL_PA[bl_lvl] >> 8) &
				       0xFF;
	...
	if (mi_cfg->read_lhbm_gamma_success) {
		...
		if (flat_mode)
			memcpy(update_D0_reg_ptr,
			       &mi_cfg->lhbm_param[lhbm_data_index +
						   LHBMGIRONNITSOFFSET],
			       length);
		else
			memcpy(update_D0_reg_ptr,
			       &mi_cfg->lhbm_param[lhbm_data_index], length);

		if (is_cal_lhbm_param ||
		    panel->id_config.build_id >= N2_PANEL_PA_P11_02) {
			mi_dsi_panel_cal_lhbm_param(panel, type,
						    update_D0_reg_ptr, length,
						    bl_lvl, type_index);
		}
		DISP_DEBUG(
			"update lbhm data index %d, calculate data %d, length %d",
...
int mi_dsi_panel_set_round_corner_locked(struct dsi_panel *panel, bool enable)
{
	...
	if (panel->mi_cfg.ddic_round_corner_enabled) {
		if (enable)
			rc = dsi_panel_tx_cmd_set(
				panel, DSI_CMD_SET_MI_ROUND_CORNER_ON);
		else
			rc = dsi_panel_tx_cmd_set(
				panel, DSI_CMD_SET_MI_ROUND_CORNER_OFF);
	...
	if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
		...
		if (is_aod_and_panel_initialized(panel)) {
			...
			case DOZE_BRIGHTNESS_HBM:
				...
				DISP_DEBUG(
					"LOCAL_HBM_OFF_TO_0SIZE_LHBM in doze_hbm_dbv_level\n");
				break;
			case DOZE_BRIGHTNESS_LBM:
				update_bl_lvl = mi_cfg->doze_lbm_dbv_level;
				DISP_DEBUG(
					"LOCAL_HBM_OFF_TO_0SIZE_LHBM in doze_lbm_dbv_level\n");
				break;
			default:
				update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
				DISP_DEBUG(
				     ...
				     struct disp_feature_ctl *ctl)
{
	...
	case LOCAL_HBM_OFF_TO_NORMAL:
		...
		if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
			...
			rc = dsi_panel_tx_cmd_set(panel, lhbm_type);
				   ...
				   N3_PANEL_PA &&
			   mi_cfg->lhbm_gxzw &&
			   mi_cfg->feature_val[DISP_FEATURE_FP_STATUS] !=
				   AUTH_STOP) {
			...
			mi_cfg->dimming_state = STATE_DIM_RESTORE;
			mi_dsi_panel_update_last_bl_level(
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT:
		...
		if (is_aod_and_panel_initialized(panel)) {
			switch (mi_cfg->doze_brightness) {
			case DOZE_BRIGHTNESS_HBM:
				if (mi_cfg->last_no_zero_bl_level <
				...
				DISP_INFO(
					"LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT in doze_hbm_dbv_level\n");
				break;
			case DOZE_BRIGHTNESS_LBM:
				if (mi_cfg->last_no_zero_bl_level <
				...
				DISP_INFO(
					"LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT in doze_lbm_dbv_level\n");
				break;
			default:
				DISP_INFO(
					"LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT defaults\n");
				break;
			}
		} else {
			mi_dsi_update_51_mipi_cmd(panel, lhbm_type,
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT_RESTORE:
		...
		mi_dsi_update_backlight_in_aod(panel, true);
		mi_dsi_update_51_mipi_cmd(
			panel, DSI_CMD_SET_MI_LOCAL_HBM_OFF_TO_NORMAL,
			mi_cfg->last_bl_level);
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
			...
			if (mi_get_panel_id(mi_cfg->mi_panel_id) == N2_PANEL_PA)
				update_bl_lvl = mi_cfg->last_bl_level;
			else if (mi_get_panel_id(mi_cfg->mi_panel_id) ==
				 N3_PANEL_PA) {
				update_bl_lvl = mi_cfg->lhbm_hbm_mode_threshold;
				mi_dsi_update_51_mipi_cmd(
					panel,
			...
			if (is_aod_and_panel_initialized(panel)) {
				...
				default:
					update_bl_lvl =
						mi_cfg->doze_hbm_dbv_level;
					DISP_INFO(
						"LOCAL_HBM_NORMAL_WHITE_1000NIT use doze_hbm_dbv_level as "
						"defaults\n");
					break;
				}
			} else {
		...
		case N2_PANEL_PA:
			...
			break;
		case N3_PANEL_PA:
			mi_dsi_panel_update_lhbm_param_N3(
				panel, lhbm_type, update_bl_lvl, lhbm_is_0size);
			break;
		default:
	...
	case LOCAL_HBM_NORMAL_WHITE_110NIT:
		...
		if (is_aod_and_panel_initialized(panel)) {
			...
			default:
				update_bl_lvl = mi_cfg->doze_lbm_dbv_level;
				DISP_INFO(
					"LOCAL_HBM_NORMAL_WHITE_110NIT use doze_lbm_dbv_level as "
					"defaults\n");
				break;
			}
		}
	...
	case LOCAL_HBM_HLPM_WHITE_1000NIT:
		...
		if (is_aod_and_panel_initialized(panel)) {
			...
			default:
				update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
				DISP_INFO(
					"LOCAL_HBM_HLPM_WHITE_1000NIT use doze_hbm_dbv_level as "
					"defaults\n");
				break;
			}
		}
	...
	case LOCAL_HBM_HLPM_WHITE_110NIT:
		...
		if (is_aod_and_panel_initialized(panel)) {
			...
			default:
				update_bl_lvl = mi_cfg->doze_lbm_dbv_level;
				DISP_INFO(
					"LOCAL_HBM_HLPM_WHITE_1000NIT use doze_lbm_dbv_level as "
					"defaults\n");
				break;
			}
		}
						      ...
						      bool enable)
{
	...
	if (enable && mi_cfg->panel_state != PANEL_STATE_ON) {
		...
		case DOZE_BRIGHTNESS_HBM:
			...
			if (rc) {
				DISP_ERROR(
					"[%s] failed to send MI_DOZE_HBM_NOLP cmd, rc=%d\n",
					panel->type, rc);
			} else {
				panel->mi_cfg.panel_state = PANEL_STATE_ON;
		...
		case DOZE_BRIGHTNESS_LBM:
			...
			if (rc) {
				DISP_ERROR(
					"[%s] failed to send MI_DOZE_LBM_NOLP cmd, rc=%d\n",
					panel->type, rc);
			} else {
				panel->mi_cfg.panel_state = PANEL_STATE_ON;
				mi_cfg->aod_to_normal_statue = true;
				...
				struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_DOZE_BRIGHTNESS:
		...
		if (dsi_panel_initialized(panel) &&
			...
			if (ctl->feature_val == DOZE_BRIGHTNESS_HBM)
				rc = dsi_panel_tx_cmd_set(
					panel, DSI_CMD_SET_MI_DOZE_HBM);
			else
				rc = dsi_panel_tx_cmd_set(
					panel, DSI_CMD_SET_MI_DOZE_LBM);
		} else {
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) ==
			    N3_PANEL_PA) {
				mi_dsi_update_switch_cmd_N3(
	...
	case DISP_FEATURE_AOD_TO_NORMAL:
		rc = mi_dsi_panel_aod_to_normal_optimize_locked(
			panel, ctl->feature_val == FEATURE_ON);
		mi_cfg->feature_val[DISP_FEATURE_AOD_TO_NORMAL] =
			ctl->feature_val;
		break;
	...
	case DISP_FEATURE_DBI:
		if (mi_get_panel_id_by_dsi_panel(panel) == N2_PANEL_PA) {
			...
			mi_cfg->feature_val[DISP_FEATURE_DBI] =
				ctl->feature_val;
			if (panel->mi_cfg.last_bl_level < 0x84 &&
			    panel->mi_cfg.last_bl_level &&
			...
			mi_dsi_panel_set_dbi_by_temp_locked(panel,
							    ctl->feature_val);
			mi_cfg->feature_val[DISP_FEATURE_DBI] =
				ctl->feature_val;
		}
...
int mi_dsi_panel_ip_set_by_dbv(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (bl_lvl >= 0x160 && !mi_cfg->ip_state) {
		...
		if (rc) {
			DISP_ERROR(
				"(%s) send DSI_CMD_SET_MI_IP_ON failed! bl_lvl(%d),rc(%d)\n",
				panel->name, bl_lvl, rc);
			return rc;
		} else {
			mi_cfg->ip_state = true;
			DISP_INFO(
				"(%s) send DSI_CMD_SET_MI_IP_ON! bl_lvl(%d),rc(%d)\n",
				panel->name, bl_lvl, rc);
		}
		...
		if (rc) {
			DISP_ERROR(
				"(%s) send DSI_CMD_SET_MI_IP_OFF failed! bl_lvl(%d),rc(%d)\n",
				panel->name, bl_lvl, rc);
			return rc;
		} else {
			mi_cfg->ip_state = false;
			DISP_INFO(
				"(%s) send DSI_CMD_SET_MI_IP_OFF! bl_lvl(%d),rc(%d)\n",
				panel->name, bl_lvl, rc);
		}
	}
...
int mi_dsi_panel_vrr_set_by_dbv(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (bl_lvl >= 0x7FF) {
		...
		mi_dsi_panel_lgs_cie_by_temperature(&dbi_bwg_type, bl_lvl,
						    temp);
	} else if (0x4C > bl_lvl && bl_lvl >= 0x28 &&
		   panel->id_config.build_id > N2_PANEL_PA_P11_02) {
	...
	if ((mi_cfg->skip_source_type != skip_source_type) &&
		...
		if (rc) {
			DISP_ERROR(
				"(%s) send  cmds(%s) failed! bl_lvl(%d),rc(%d)\n",
				panel->name, cmd_set_prop_map[skip_source_type],
				bl_lvl, rc);
			return rc;
		}
	...
	if ((mi_cfg->dbi_bwg_type != dbi_bwg_type) &&
		...
		if (rc) {
			DISP_ERROR(
				"(%s) send  cmds(%s) failed! bl_lvl(%d),rc(%d)\n",
				panel->name, cmd_set_prop_map[dbi_bwg_type],
				bl_lvl, rc);
			return rc;
		}
