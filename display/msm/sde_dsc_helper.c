...
/*
 * Rate control - Min QP values for each ratio type in sde_dsc_ratio_type for
 * NT37801
 */
static char
	sde_dsc_rc_range_min_qp_nvt[DSC_RATIO_TYPE_MAX][DSC_NUM_BUF_RANGES] = {
		/* DSC v1.1 */
		{ 0, 0, 1, 1, 3, 3, 3, 3, 3, 3, 5, 5, 5, 7, 13 },
		{ 0, 4, 5, 5, 7, 7, 7, 7, 7, 7, 9, 9, 9, 11, 17 },
		{ 0, 4, 5, 6, 7, 7, 7, 7, 7, 7, 9, 9, 9, 11, 15 },
...
/*
 * Rate control - Max QP values for each ratio type in sde_dsc_ratio_type for
 * NT37801
 */
static char
	sde_dsc_rc_range_max_qp_nvt[DSC_RATIO_TYPE_MAX][DSC_NUM_BUF_RANGES] = {
		/* DSC v1.1 */
		{ 4, 4, 5, 6, 7, 7, 7, 8, 9, 10, 11, 12, 13, 13, 15 },
		{ 4, 8, 9, 10, 11, 11, 11, 12, 13, 14, 15, 16, 17, 17, 19 },
		{ 7, 8, 9, 10, 11, 11, 11, 12, 13, 13, 14, 14, 15, 15, 16 },
...
static struct sde_dsc_rc_init_params_lut {
	...
	/* DSC v1.1 SCR and DSC v1.2 RGB 444 */
	{ 11, 11, 6144, 512, 0, 0, 3,
	  12 }, /* DSC_V12_444_8BPC_8BPP or DSC_V11_SCR1_8BPC_8BPP */
	{ 15, 15, 6144, 512, 0, 0, 7,
	  16 }, /* DSC_V12_444_10BPC_8BPP or DSC_V11_SCR1_10BPC_8BPP */
	{ 15, 15, 5632, 410, 0, 0, 7,
	  16 }, /* DSC_V12_444_10BPC_10BPP or DSC_V11_SCR1_10BPC_10BPP */
	/* DSC v1.2 YUV422 */
...
static int _get_rc_table_index(struct drm_dsc_config *dsc, int scr_ver)
{
	...
	SDE_ERROR("unsupported DSC v%d.%dr%d, bpc:%d, bpp:%d, fmt:0x%x\n",
		  dsc->dsc_version_major, dsc->dsc_version_minor, scr_ver, bpc,
		  bpp, fmt);
	return -EINVAL;
}
				...
				u64 mi_panel_id)
{
	...
	if ((dsc->dsc_version_minor == 0x2) && (dsc->native_420))
		dsc->nsl_bpg_offset =
			(2048 * (DIV_ROUND_UP(dsc->second_line_bpg_offset,
					      (dsc->slice_height - 1))));
	...
	if (dsc->native_422)
		pre_num_extra_mux_bits = 4 * dsc->mux_word_size +
					 (4 * bpc + 4) + (3 * 4 * bpc) - 2;
	else if (dsc->native_420)
		pre_num_extra_mux_bits = 3 * dsc->mux_word_size +
					 (4 * bpc + 4) + (2 * 4 * bpc) - 2;
	else
		pre_num_extra_mux_bits =
			3 * (dsc->mux_word_size + (4 * bpc + 4) - 2);
	...
	final_scale =
		8 * dsc->rc_model_size / (dsc->rc_model_size - final_value);
	...
	dsc->scale_decrement_interval =
		groups_per_line / (dsc->initial_scale_value - 8);
					...
					int intf_width, bool widebus_en)
{
	...
	if (!dsc_info || !dsc_info->config.slice_width ||
		...
		SDE_ERROR("invalid input, intf_width=%d slice_width=%d\n",
			  intf_width,
			  dsc_info ? dsc_info->config.slice_width : -1);
		return -EINVAL;
	}
	...
	slice_per_intf = DIV_ROUND_UP(intf_width, dsc_info->config.slice_width);
   ...
   */
	...
	bytes_in_slice = DIV_ROUND_UP(dsc_info->config.slice_width * bpp, 8);
   ...
   */
	if (widebus_en)
		dsc_info->pclk_per_line = DIV_ROUND_UP(
			total_bytes_per_intf * 8,
			msm_get_src_bpc(dsc_info->chroma_format, bpc));
	else
		dsc_info->pclk_per_line =
			DIV_ROUND_UP(total_bytes_per_intf * 8, 24);
	...
	if (bytes_in_dsc_pair % 8) {
		...
		total_bytes_in_dsc_pair =
			bytes_in_dsc_pair + dsc_info->dsc_4hsmerge_padding;
		if (total_bytes_in_dsc_pair % 16)
			dsc_info->dsc_4hsmerge_alignment =
...
int sde_dsc_create_pps_buf_cmd(struct msm_display_dsc_info *dsc_info, char *buf,
			       int pps_id, u32 len)
{
	...
	memset(buf, 0, len);
	/* pps0 */
	*bp++ = (dsc->dsc_version_minor | dsc->dsc_version_major << 4);
	*bp++ = (pps_id & 0xff); /* pps1 */
	bp++; /* pps2, reserved */
