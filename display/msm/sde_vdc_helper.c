...
static u8 sde_vdc_mppf_bpc_r_y[VDC_PROFILE_MAX] = { 2, 1, 3, 2, 2, 1, 3,
						    2, 2, 4, 3, 2, 2 };

static u8 sde_vdc_mppf_bpc_g_cb[VDC_PROFILE_MAX] = { 2, 2, 3, 2, 2, 2, 2,
						     2, 1, 3, 2, 2, 2 };

static u8 sde_vdc_mppf_bpc_b_cr[VDC_PROFILE_MAX] = { 2, 1, 3, 2, 2, 1, 2,
						     2, 1, 3, 2, 2, 2 };

static u8 sde_vdc_mppf_bpc_y[VDC_PROFILE_MAX] = { 2, 2, 3, 2, 2, 2, 0,
						  0, 0, 0, 0, 0, 0 };

static u8 sde_vdc_mppf_bpc_co[VDC_PROFILE_MAX] = { 2, 1, 3, 2, 2, 1, 0,
						   0, 0, 0, 0, 0, 0 };

static u8 sde_vdc_mppf_bpc_cg[VDC_PROFILE_MAX] = { 2, 1, 3, 2, 2, 1, 0,
						   0, 0, 0, 0, 0, 0 };

static u8 sde_vdc_flat_qp_vf_fbls[VDC_PROFILE_MAX] = { 20, 24, 24, 24, 24,
						       24, 24, 24, 24, 8,
						       20, 18, 24 };

static u8 sde_vdc_flat_qp_vf_nfbls[VDC_PROFILE_MAX] = { 24, 28, 28, 28, 28,
							28, 28, 28, 28, 16,
							24, 20, 28 };

static u8 sde_vdc_flat_qp_sf_fbls[VDC_PROFILE_MAX] = { 24, 28, 28, 28, 28,
						       28, 28, 28, 28, 16,
						       24, 20, 28 };

static u8 sde_vdc_flat_qp_sf_nbls[VDC_PROFILE_MAX] = { 28, 40, 32, 28, 32,
						       28, 36, 36, 36, 16,
						       24, 24, 28 };

static u16 sde_vdc_flat_qp_lut[VDC_PROFILE_MAX][VDC_FLAT_QP_LUT_SIZE] = {
	{ 20, 20, 24, 24, 28, 32, 36, 40 }, { 24, 24, 28, 32, 36, 40, 40, 40 },
	{ 24, 24, 28, 32, 32, 36, 36, 36 }, { 20, 24, 28, 28, 32, 36, 40, 44 },
	{ 20, 24, 28, 32, 32, 36, 36, 40 }, { 24, 28, 32, 32, 36, 40, 40, 40 },
	{ 24, 28, 32, 34, 36, 38, 40, 40 }, { 24, 28, 32, 36, 40, 42, 44, 44 },
	{ 24, 28, 32, 36, 40, 42, 44, 44 }, { 0, 8, 10, 12, 14, 16, 18, 20 },
	{ 12, 16, 20, 20, 20, 24, 24, 28 }, { 16, 18, 20, 22, 24, 26, 28, 28 },
	{ 20, 22, 24, 26, 28, 28, 32, 32 },
};

static u16 sde_vdc_max_qp_lut[VDC_PROFILE_MAX][VDC_MAX_QP_LUT_SIZE] = {
	{ 28, 28, 32, 32, 36, 42, 42, 48 }, { 32, 32, 36, 40, 44, 48, 48, 52 },
	{ 32, 32, 36, 36, 36, 40, 44, 48 }, { 24, 28, 32, 32, 36, 40, 44, 48 },
	{ 28, 28, 32, 32, 36, 42, 42, 48 }, { 28, 32, 36, 40, 44, 44, 46, 52 },
	{ 32, 32, 36, 40, 40, 44, 48, 48 }, { 32, 32, 36, 40, 44, 48, 50, 52 },
	{ 32, 32, 36, 40, 44, 48, 50, 52 }, { 8, 12, 12, 16, 20, 24, 28, 28 },
	{ 18, 20, 22, 24, 28, 30, 32, 40 }, { 18, 20, 22, 24, 28, 30, 32, 40 },
...
static u16 sde_vdc_lbda_brate_lut[VDC_PROFILE_MAX][VDC_LBDA_BRATE_LUT_SIZE] = {
	{ 4, 6, 10, 16, 25, 40, 64, 102, 161, 256, 406, 645, 1024, 1625, 2580,
	  4095 },
	{ 8, 12, 18, 28, 42, 64, 97, 147, 223, 338, 512, 776, 1176, 1782, 2702,
	  4095 },
	{ 16, 23, 34, 48, 70, 102, 147, 213, 308, 446, 645, 933, 1351, 1955,
	  2829, 4095 },
	{ 8, 12, 18, 28, 42, 64, 97, 147, 223, 338, 512, 776, 1176, 1782, 2702,
	  4095 },
					...
					int intf_width)
{
	...
	if (!vdc_info->slice_width || !vdc_info->slice_height ||
	...
	vdc_info->slice_num_px = vdc_info->slice_width * vdc_info->slice_height;
	...
	vdc_info->tot_pad_bits =
		(vdc_info->avg_block_bits + vdc_info->per_chunk_pad_bits - 8);

	vdc_info->rc_stuffing_bits = ((vdc_info->tot_pad_bits + 8) / 9);
	vdc_info->slice_num_bits =
		(8 * vdc_info->chunk_size * vdc_info->slice_height);

	temp = (16 * vdc_info->chunk_size);
	vdc_info->chunk_adj_bits =
		temp - ((2 * vdc_info->slice_width * bpp_codec) >> 4);
	...
	vdc_info->rc_target_rate_threshold =
		(1 << (vdc_info->rc_tar_rate_scale - 1));
	...
	temp = div_u64(vdc_info->ramp_bits,
		       (vdc_info->ramp_blocks) ? vdc_info->ramp_blocks : 1);
	...
	if ((temp % SSM_MAX_SE_SIZE) == 0) {
		...
		diff = vdc_info->slice_num_bits -
		       vdc_info->num_extra_mux_bits_init;
		vdc_info->extra_crop_bits =
			(SSM_MAX_SE_SIZE - (diff % SSM_MAX_SE_SIZE));
	}

	vdc_info->num_extra_mux_bits =
		vdc_info->num_extra_mux_bits_init - vdc_info->extra_crop_bits;

	vdc_info->mppf_bits_comp_0 = 16 * vdc_info->mppf_bpc_r_y;
	vdc_info->mppf_bits_comp_1 =
		vdc_info->chroma_samples * vdc_info->mppf_bpc_g_cb;
	vdc_info->mppf_bits_comp_2 =
		vdc_info->chroma_samples * vdc_info->mppf_bpc_b_cr;
			      ...
			      int intf_width)
{
	...
	slice_per_intf = DIV_ROUND_UP(intf_width, vdc_info->slice_width);
   ...
   */
	...
	bpp = VDC_BPP(vdc_info->bits_per_pixel);
	bytes_in_slice = DIV_ROUND_UP(vdc_info->slice_width * bpp, 8);
				    ...
				    int traffic_mode)
{
	...
	vdc_info->input_ssm_out_latency =
		MAX_PIPELINE_LATENCY + (8 * vdc_info->max_ssm_delay);
	...
	temp = vdc_info->input_ssm_out_latency + vdc_info->obuf_latency;
	...
	vdc_info->base_hs_latency_pixels_min =
		(2 * vdc_info->base_hs_latency_min);
	...
	if (traffic_mode == VDC_TRAFFIC_BURST_MODE) {
		temp = vdc_info->output_rate_ratio_100 * vdc_info->slice_width;
	...
	if (traffic_mode == VDC_TRAFFIC_BURST_MODE) {
		if ((vdc_info->burst_accum_pixels + vdc_info->base_top_up) <
		    vdc_info->slice_width)
	...
	vdc_info->obuf_extra_burst =
		vdc_info->burst_initial_lines * vdc_info->chunk_size_bits;
	...
	temp = OUT_BUF_OF_MARGIN_TC_10 * vdc_info->chunk_size_bits;
	temp /= 10;
	temp /= SSM_MAX_SE_SIZE;
	vdc_info->obuf_margin_words = max(OUT_BUF_OF_MARGIN_OB, temp);
...
int sde_vdc_create_pps_buf_cmd(struct msm_display_vdc_info *vdc_info, char *buf,
			       int pps_id, u32 len)
{
	char *bp = buf;
	u32 i;
