...
static void __dump_packet(u8 *packet, const char *function, void *qinfo)
{
   ...
   */
	...
	d_vpr_t("%08x: %s: %pK: read_idx = %u, write_idx = %u\n", session_id,
		function, qinfo, q_hdr->qhdr_read_idx, q_hdr->qhdr_write_idx);

	for (c = 0; c * row_size < packet_size; ++c) {
		int bytes_to_read = ((c + 1) * row_size > packet_size) ?
					    packet_size % row_size :
					    row_size;
			 ...
			 bool *rx_req_is_set)
{
	...
	if (!packet_size_in_words ||
	    packet_size_in_words > qinfo->q_array.mem_size >> 2) {
	...
	write_ptr =
		(u32 *)((qinfo->q_array.align_virtual_addr) + (write_idx << 2));
			...
			u32 *pb_tx_req_is_set)
{
   ...
   */
	...
	read_ptr =
		(u32 *)((qinfo->q_array.align_virtual_addr) + (read_idx << 2));
	...
	if (((packet_size_in_words << 2) <= VIDC_IFACEQ_VAR_HUGE_PKT_SIZE) &&
		...
		if (new_read_idx < (qinfo->q_array.mem_size >> 2)) {
			...
			memcpy(packet + ((packet_size_in_words - new_read_idx)
					 << 2),
...
static int __iface_cmdq_write_relaxed(struct msm_vidc_core *core, void *pkt,
				      bool *requires_interrupt)
{
...
int venus_hfi_queue_init(struct msm_vidc_core *core)
{
   ...
   */
	...
	*((u32 *)core->sfr.align_virtual_addr) = core->sfr.mem_size;
	...
	rc = venus_hfi_iommu_map_registers(
		core, MSM_VIDC_QTIMER, MSM_VIDC_NON_SECURE, &core->qtimer_reg);
	if (rc)
		return rc;
   ...
   *     payload[19-20] : address and size of IPC_PROTOCOL4_CLIENT_VERSION
   * registers payload[21-22] : address and size of FENCE QTIMER registers
   */
