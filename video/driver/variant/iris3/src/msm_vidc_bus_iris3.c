...
u32 calculate_number_ubwctiles_kalama(u32 width, u32 height, u32 tile_w,
				      u32 tile_h)
{
			    ...
			    struct api_calculation_input codec_input)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		compression_factor->dpb_cf_y =
			dpbopb_ubwc30_cr_table_cratio_kalama[cr_index_entry]
							    [cr_index_y];
     ...
     */
		if (frame_width < 3840) {
			...
			compression_factor->ipb_cr =
				ipblossy_ubwc30_cr_table_cratio_kalama
					[cr_index_entry][cr_index_uni];
			compression_factor->ipb_cr_y =
				ipblossy_ubwc30_cr_table_cratio_kalama
					[cr_index_entry][cr_index_y];
		...
		compression_factor->dpb_cf_y =
			rpb_ubwc30_cr_table_cratio_kalama[cr_index_entry]
							 [cr_index_y];
	...
	struct api_calculation_bw_output *codec_output)
{
	...
	lcu_per_frame = calculate_number_lcus_kalama(frame_width, frame_height,
						     frame_lcu_size);
	...
	if (codec_input.codec == CODEC_AV1) {
		...
		av1_tile_numbers = av1_num_tiles_kalama[av1tile_index_entry]
						       [av1tile_complexity];
				 ...
				 struct api_calculation_bw_output *codec_output)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		rc = calculate_bandwidth_decoder_iris3(codec_input,
						       codec_output);
	} else if (codec_input.decoder_or_encoder == CODEC_ENCODER) {
		rc = calculate_bandwidth_encoder_iris3(codec_input,
						       codec_output);
	} else {
