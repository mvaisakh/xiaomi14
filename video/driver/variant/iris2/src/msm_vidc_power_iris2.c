...
u64 msm_vidc_calc_freq_iris2(struct msm_vidc_inst *inst, u32 data_size)
{
   ...
   */
     ...
     */
		...
		if (inst->codec == MSM_VIDC_VP9 &&
		    inst->capabilities[STAGE].value == MSM_VIDC_STAGE_2 &&
		    inst->capabilities[PIPE].value == 4 && bitrate > 90000000)
...
static u64 __calculate_decoder(struct vidc_bus_vote_data *d)
{
   ...
   */
	...
	if (d->codec == MSM_VIDC_HEVC || d->codec == MSM_VIDC_HEIC ||
   ...
   */
	if (num_vpp_pipes != 4)
		tnbr_per_lcu = lcu_size == 16 ? 64 : lcu_size == 32 ? 64 : 128;
	else
