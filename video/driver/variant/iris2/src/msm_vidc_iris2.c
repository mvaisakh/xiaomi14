...
static int __power_off_iris2_controller(struct msm_vidc_core *core)
{
   ...
   */
	...
	rc = __write_register(core, WRAPPER_DEBUG_BRIDGE_LPI_CONTROL_IRIS2,
			      0x7);
	if (rc)
		return rc;

	rc = __read_register_with_poll_timeout(
		core, WRAPPER_DEBUG_BRIDGE_LPI_STATUS_IRIS2, 0x7, 0x7, 200,
		2000);
	if (rc)
	...
	rc = __read_register_with_poll_timeout(
		core, WRAPPER_DEBUG_BRIDGE_LPI_STATUS_IRIS2, 0xffffffff, 0x0,
		200, 2000);
	if (rc)
...
static int __prepare_pc_iris2(struct msm_vidc_core *core)
{
	...
	rc = __read_register_with_poll_timeout(core, CTRL_STATUS_IRIS2,
					       ...
					       CTRL_STATUS_PC_READY_IRIS2, 250,
					       2500);
	if (rc) {
...
static int __raise_interrupt_iris2(struct msm_vidc_core *core)
{
	...
	rc = __write_register(core, CPU_IC_SOFTINT_IRIS2,
...
int msm_vidc_adjust_blur_type_iris2(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, BLUR_TYPES, BITRATE_MODE, &rc_type,
				      __func__) ||
	    msm_vidc_get_parent_value(inst, BLUR_TYPES, CONTENT_ADAPTIVE_CODING,
				      &cac, __func__) ||
	    msm_vidc_get_parent_value(inst, BLUR_TYPES, PIX_FMTS, &pix_fmts,
				      __func__) ||
