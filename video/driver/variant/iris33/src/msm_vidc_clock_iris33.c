...
static int initialize_encoder_complexity_table(void)
{
	...
	codec_encoder_gop_complexity_table_fp[CODEC_GOP_I3B4b1P]
					     [CODEC_ENCODER_GOP_Bb_ENTRY] =
						     70000;

	codec_encoder_gop_complexity_table_fp[CODEC_GOP_I3B4b1P]
					     [CODEC_ENCODER_GOP_P_ENTRY] =
						     10000;
	...
	codec_encoder_gop_complexity_table_fp[CODEC_GOP_I1B2b1P]
					     [CODEC_ENCODER_GOP_Bb_ENTRY] =
						     30000;
	...
	codec_encoder_gop_complexity_table_fp[CODEC_GOP_IbP]
					     [CODEC_ENCODER_GOP_Bb_ENTRY] =
						     10000;
	...
	codec_encoder_gop_complexity_table_fp[CODEC_GOP_IPP]
					     [CODEC_ENCODER_GOP_Bb_ENTRY] = 0;

	codec_encoder_gop_complexity_table_fp[CODEC_GOP_IPP]
					     [CODEC_ENCODER_GOP_P_ENTRY] = 1;
...
static int
calculate_vsp_min_freq(struct api_calculation_input codec_input,
		       struct api_calculation_freq_output *codec_output)
{
   ...
   * tables. As a consequence remove x1000 multipler as well.
   */
	u32 codec = codec_input.codec;
	/* UInt32 *bitratetable; */
	u32 pixle_count = codec_input.frame_width * codec_input.frame_height *
     ...
     *  8KUHD60fps with B frame
     *     - bitrate_entry = 0
     *     - Clock=NOMINAL for H264 & 2Stage H265. Because bitrate table entry
     * for TURBO is 0.
     *
     *  TODO : Reduce these conditions by removing the zero entries from Bitrate
     * table.
     */
		...
		if ((codec_input.codec == CODEC_H264) ||
			...
			vsp_hw_min_frequency =
				(frequency_table_pineapple[0][2] * 1000 +
				 (fw_sw_vsp_offset - 1));
			vsp_hw_min_frequency = DIV_ROUND_UP(
				vsp_hw_min_frequency, fw_sw_vsp_offset);
		} else {
			if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
				vsp_hw_min_frequency =
					vsp_hw_min_frequency +
					(bitrate_table_pineapple_2stage_fp[codec]
									  [0] *
						 fw_sw_vsp_offset -
					 1);
				vsp_hw_min_frequency = DIV_ROUND_UP(
				...
				vsp_hw_min_frequency =
					vsp_hw_min_frequency +
					(bitrate_table_pineapple_1stage_fp[codec]
									  [0] *
						 fw_sw_vsp_offset -
					 1);
				vsp_hw_min_frequency = DIV_ROUND_UP(
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			vsp_hw_min_frequency =
				vsp_hw_min_frequency +
				(bitrate_table_pineapple_2stage_fp
						 [codec][bitrate_entry] *
					 fw_sw_vsp_offset -
				 1);
			vsp_hw_min_frequency =
			...
			vsp_hw_min_frequency =
				vsp_hw_min_frequency +
				(bitrate_table_pineapple_1stage_fp
						 [codec][bitrate_entry] *
					 fw_sw_vsp_offset -
				 1);
			vsp_hw_min_frequency =
...
static u32 calculate_pipe_penalty(struct api_calculation_input codec_input)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		...
		if (codec_input.codec == CODEC_AV1) {
			pixel_count = codec_input.frame_width *
				      codec_input.frame_height;
			if (pixel_count <= 1920 * 1080)
				pipe_penalty_codec = pipe_penalty_pineapple
					[avid_commercial_content + 1][0];
			else if (pixel_count < 3840 * 2160)
				pipe_penalty_codec =
					(pipe_penalty_pineapple
						 [avid_commercial_content + 1]
						 [0] +
					 pipe_penalty_pineapple
						 [avid_commercial_content + 1]
						 [1]) /
				 ...
				 (pixel_count == 4096 * 2160) ||
				 (pixel_count == 4096 * 2304))
				pipe_penalty_codec = pipe_penalty_pineapple
					[avid_commercial_content + 1][1];
...
static int
calculate_vpp_min_freq(struct api_calculation_input codec_input,
		       struct api_calculation_freq_output *codec_output)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) { /* decoder */
		vpp_hw_min_frequency =
			((decoder_vpp_target_clk_per_mb_pineapple) *
				 (codec_mbspersession_pineaple) +
			 codec_input.pipe_num - 1) /
		...
		if (codec_input.pipe_num > 1) {
			pipe_penalty_codec =
				calculate_pipe_penalty(codec_input);
			vpp_hw_min_frequency =
				(vpp_hw_min_frequency * pipe_penalty_codec +
				 999) /
				1000;
		}
		...
		if (codec_input.codec == CODEC_AV1)
			decoder_vpp_fw_overhead =
				DECODER_VPP_FW_OVERHEAD_PINEAPPLE_AV1D;
		else
			decoder_vpp_fw_overhead =
				DECODER_VPP_FW_OVERHEAD_PINEAPPLE_NONAV1D;
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			decoder_vpp_fw_overhead = DIV_ROUND_UP(
			...
			if (codec_input.linear_opb == 1 &&
				...
				decoder_vpp_fw_overhead =
					1200 + decoder_vpp_fw_overhead - 1000;
       ...
       */
			...
			fmin_measured_fwoverhead =
				fmin +
				(((DECODER_VPPVSP1STAGE_FW_OVERHEAD_PINEAPPLE *
					   codec_input.frame_rate * 10 +
				   14) / 15 +
				  999) / 1000 +
				 999) / 1000;
		...
		tensilica_min_frequency =
			(DECODER_SW_OVERHEAD_PINEAPPLE * 10 + 14) / 15;
		tensilica_min_frequency =
			(tensilica_min_frequency + 999) / 1000;
		tensilica_min_frequency =
			tensilica_min_frequency * codec_input.frame_rate;
		tensilica_min_frequency =
			(tensilica_min_frequency + 999) / 1000;
		...
		/* VPP base cycle */
		lpmode_uhd_cycle_permb =
			(320 * codec_encoder_gop_complexity_table_fp
		...
		hqmode1080p_cycle_permb =
			(675 * codec_encoder_gop_complexity_table_fp
					 [codec_input.hierachical_layer]
					 [CODEC_ENCODER_GOP_FACTORY_ENTRY] +
			 99) /
			100;
		...
		encoder_vpp_target_clk_per_mb =
			(hq_mode) ? hqmode1080p_cycle_permb :
				    lpmode_uhd_cycle_permb;
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			encoder_vpp_fw_overhead = DIV_ROUND_UP(
				(ENCODER_VPP_FW_OVERHEAD_PINEAPPLE * 10 *
				 codec_input.frame_rate),
				15);
		...
		tensilica_min_frequency =
			(ENCODER_SW_OVERHEAD_PINEAPPLE * 10 + 14) / 15;
		tensilica_min_frequency =
			(tensilica_min_frequency + 999) / 1000;
...
int msm_vidc_calculate_frequency(
