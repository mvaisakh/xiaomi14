 ...
 */
u32 vpe_csc_custom_matrix_coeff[MAX_MATRIX_COEFFS] = { 440, 8140, 8098, 0,  460,
						       52,  0,	  34,	463 };

/* offset coefficients in s9 fixed point format */
u32 vpe_csc_custom_bias_coeff[MAX_BIAS_COEFFS] = { 53, 0, 4 };

/* clamping value for Y/U/V([min,max] for Y/U/V) */
u32 vpe_csc_custom_limit_coeff[MAX_LIMIT_COEFFS] = { 16, 235, 16, 240, 16, 240 };
			      ...
			      s32 adjusted_val, const char *func)
{
	...
	if (prev_value != inst->capabilities[cap_id].value) {
		i_vpr_h(inst,
			...
			func, cap_name(cap_id), prev_value,
			 ...
			 u32 check_parent, const char *func)
{
	...
	i_vpr_e(inst, "%s: missing parent %s for %s\n", func,
		cap_name(check_parent), cap_name(cap_id));
	return false;
}

int msm_vidc_get_parent_value(struct msm_vidc_inst *inst, u32 cap_id,
			      u32 parent, s32 *value, const char *func)
{
	int rc = 0;
			      ...
			      u32 *value)
{
	...
	case ENTROPY_MODE:
		...
		return 0;
	default:
		i_vpr_e(inst, "%s: mapping not specified for ctrl_id: %#x\n",
...
int msm_vidc_packetize_control(struct msm_vidc_inst *inst,
			       enum msm_vidc_inst_capability_type cap_id,
			       u32 payload_type, void *hfi_val,
			       u32 payload_size, const char *func)
{
	int rc = 0;
	u64 payload = 0;
	...
	if (payload_size > sizeof(u32)) {
		i_vpr_e(inst,
			"%s: payload size is more than u32 for cap[%d] %s\n",
	...
	i_vpr_h(inst, FMT_STRING_SET_CAP, cap_name(cap_id),
		inst->capabilities[cap_id].value, payload);

	rc = venus_hfi_session_property(inst, inst->capabilities[cap_id].hfi_id,
					HFI_HOST_FLAGS_NONE,
					msm_vidc_get_port_info(inst, cap_id),
					payload_type, hfi_val, payload_size);
	if (rc) {
		i_vpr_e(inst, "%s: failed to set cap[%d] %s to fw\n", func,
			cap_id, cap_name(cap_id));
		return rc;
	}
...
int msm_vidc_adjust_entropy_mode(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, ENTROPY_MODE, PROFILE, &profile,
				      __func__))
		return -EINVAL;
...
int msm_vidc_adjust_profile(void *instance, struct v4l2_ctrl *ctrl)
{
   ...
   */
	if (msm_vidc_get_parent_value(inst, PROFILE, PIX_FMTS, &pix_fmt,
				      __func__))
		return -EINVAL;
...
int msm_vidc_adjust_ltr_count(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, LTR_COUNT, BITRATE_MODE, &rc_type,
				      __func__))
		return -EINVAL;

	if ((rc_type != HFI_RC_OFF && rc_type != HFI_RC_CBR_CFR &&
	     rc_type != HFI_RC_CBR_VFR)) {
		adjusted_value = 0;
		i_vpr_h(inst, "%s: ltr count unsupported, rc_type: %#x\n",
			__func__, rc_type);
		goto exit;
	}

	if (is_valid_cap(inst, ALL_INTRA)) {
		if (msm_vidc_get_parent_value(inst, LTR_COUNT, ALL_INTRA,
					      &all_intra, __func__))
			return -EINVAL;
		if (all_intra) {
			adjusted_value = 0;
	...
	if (!msm_vidc_get_parent_value(inst, LTR_COUNT, PIX_FMTS, &pix_fmts,
				       __func__)) {
	...
	if (!msm_vidc_get_parent_value(inst, LTR_COUNT, ENH_LAYER_COUNT,
	    ...
	    !msm_vidc_get_parent_value(inst, LTR_COUNT, LAYER_TYPE, &layer_type,
				       __func__)) {
		if (layer_type == HFI_HIER_P_SLIDING_WINDOW) {
			SLIDING_WINDOW_REF_FRAMES(
				inst->codec,
		...
		i_vpr_h(inst, "%s: ltr count %d enh_layers %d layer_type %d\n",
			__func__, adjusted_value,
			inst->capabilities[ENH_LAYER_COUNT].value, layer_type);
	}
...
int msm_vidc_adjust_use_ltr(void *instance, struct v4l2_ctrl *ctrl)
{
   ...
   */
	...
	if (adjusted_value <= 0 || adjusted_value > ((1 << ltr_count) - 1)) {
		/*
...
int msm_vidc_adjust_delta_based_rc(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, TIME_DELTA_BASED_RC, BITRATE_MODE,
				      &rc_type, __func__))
		return -EINVAL;
	...
	msm_vidc_update_cap_value(inst, TIME_DELTA_BASED_RC, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_output_order(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (is_valid_cap(inst, THUMBNAIL_MODE)) {
		if (msm_vidc_get_parent_value(inst, OUTPUT_ORDER,
...
int msm_vidc_adjust_input_buf_host_max_count(void *instance,
...
int msm_vidc_adjust_output_buf_host_max_count(void *instance,
					      struct v4l2_ctrl *ctrl)
{
	struct msm_vidc_inst *inst = (struct msm_vidc_inst *)instance;
...
int msm_vidc_adjust_transform_8x8(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, TRANSFORM_8X8, PROFILE, &profile,
				      __func__))
		return -EINVAL;
	...
	msm_vidc_update_cap_value(inst, TRANSFORM_8X8, adjusted_value,
				  __func__);
					   ...
					   struct v4l2_ctrl *ctrl)
{
	...
	msm_vidc_update_cap_value(inst, CHROMA_QP_INDEX_OFFSET, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_slice_count(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	slice_mode = ctrl ? ctrl->val : inst->capabilities[SLICE_MODE].value;
	...
	if (msm_vidc_get_parent_value(inst, SLICE_MODE, BITRATE_MODE, &rc_type,
				      __func__) ||
	    msm_vidc_get_parent_value(inst, SLICE_MODE, ENH_LAYER_COUNT,
				      &enh_layer_count, __func__))
		return -EINVAL;
	...
	if (fps > MAX_SLICES_FRAME_RATE ||
		...
		i_vpr_h(inst, "%s: slice unsupported, fps: %u, rc_type: %#x\n",
			__func__, fps, rc_type);
		goto exit;
	}

	if (is_valid_cap(inst, ALL_INTRA)) {
		if (msm_vidc_get_parent_value(inst, SLICE_MODE, ALL_INTRA,
					      &all_intra, __func__))
			return -EINVAL;
	...
	max_width = (slice_mode == V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_MB) ?
			    MAX_MB_SLICE_WIDTH :
			    MAX_BYTES_SLICE_WIDTH;
	max_height = (slice_mode == V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_MB) ?
			     MAX_MB_SLICE_HEIGHT :
			     MAX_BYTES_SLICE_HEIGHT;
	min_width = (inst->codec == MSM_VIDC_HEVC) ? MIN_HEVC_SLICE_WIDTH :
						     MIN_AVC_SLICE_WIDTH;
   ...
   */
	...
	max_mbps =
		NUM_MBS_PER_SEC(max_height, max_width, MAX_SLICES_FRAME_RATE);
...
static int
msm_vidc_adjust_static_layer_count_and_type(struct msm_vidc_inst *inst,
					    s32 layer_count)
{
...
int msm_vidc_adjust_layer_count(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (!inst->bufq[OUTPUT_PORT].vb2q->streaming) {
		rc = msm_vidc_adjust_static_layer_count_and_type(
			inst, client_layer_count);
		if (rc)
			goto exit;
	} else {
...
int msm_vidc_adjust_gop_size(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, GOP_SIZE, ENH_LAYER_COUNT,
				      &enh_layer_count, __func__))
		return -EINVAL;
...
int msm_vidc_adjust_b_frame(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, B_FRAME, ENH_LAYER_COUNT,
				      &enh_layer_count, __func__))
		return -EINVAL;
...
int msm_vidc_adjust_bitrate(void *instance, struct v4l2_ctrl *ctrl)
{
     ...
     */
		adjusted_value = ctrl ? ctrl->val :
					inst->capabilities[BIT_RATE].value;
	...
	if (msm_vidc_get_parent_value(inst, BIT_RATE, ENH_LAYER_COUNT,
				      &enh_layer_count, __func__))
		return -EINVAL;
	...
	if (inst->capabilities[BIT_RATE].value > max_bitrate)
		msm_vidc_update_cap_value(inst, BIT_RATE, max_bitrate,
					  __func__);
   ...
   */
	if (inst->capabilities[ENH_LAYER_COUNT].max) {
		...
		if (cumulative_bitrate > max_bitrate) {
			...
			u32 decrement_in_percent =
				((cumulative_bitrate - max_bitrate) * 100) /
				max_bitrate;
			...
			for (i = 0; i <= enh_layer_count; i++) {
				...
				decrement_in_value =
					(cap_value * decrement_in_percent) /
					100;
				cumulative_bitrate +=
		...
		i_vpr_h(inst, "%s: update BIT_RATE with cumulative bitrate\n",
			__func__);
		msm_vidc_update_cap_value(inst, BIT_RATE, cumulative_bitrate,
					  __func__);
	}
...
int msm_vidc_adjust_layer_bitrate(void *instance, struct v4l2_ctrl *ctrl)
{
   ...
   */
	...
	if (!msm_vidc_check_all_layer_bitrate_set(inst)) {
		i_vpr_h(inst, "%s: client did not set all layer bitrates\n",
			__func__);
		return 0;
	}
   ...
   */
	...
	if ((cumulative_bitrate - old_br + new_br) > max_bitrate) {
		/* adjust new bitrate */
		exceeded_br =
			(cumulative_bitrate - old_br + new_br) - max_bitrate;
	...
	msm_vidc_update_cap_value(inst, BIT_RATE,
				  msm_vidc_get_cumulative_bitrate(inst),
				  __func__);
...
int msm_vidc_adjust_peak_bitrate(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, PEAK_BITRATE, BITRATE_MODE,
				      &rc_type, __func__))
		return -EINVAL;
	...
	if (msm_vidc_get_parent_value(inst, PEAK_BITRATE, BIT_RATE, &bitrate,
				      __func__))
		return -EINVAL;
				   ...
				   enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (msm_vidc_get_parent_value(inst, cap_id, PIX_FMTS, &pix_fmt,
				      __func__))
		return -EINVAL;
...
int msm_vidc_adjust_hevc_min_qp(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl)
		msm_vidc_update_cap_value(inst, MIN_FRAME_QP, ctrl->val,
					  __func__);
...
int msm_vidc_adjust_hevc_max_qp(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl)
		msm_vidc_update_cap_value(inst, MAX_FRAME_QP, ctrl->val,
					  __func__);
...
int msm_vidc_adjust_hevc_i_frame_qp(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl)
		msm_vidc_update_cap_value(inst, I_FRAME_QP, ctrl->val,
					  __func__);
...
int msm_vidc_adjust_hevc_p_frame_qp(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl)
		msm_vidc_update_cap_value(inst, P_FRAME_QP, ctrl->val,
					  __func__);
...
int msm_vidc_adjust_hevc_b_frame_qp(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl)
		msm_vidc_update_cap_value(inst, B_FRAME_QP, ctrl->val,
					  __func__);
...
int msm_vidc_adjust_blur_type(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, BLUR_TYPES, BITRATE_MODE, &rc_type,
				      __func__) ||
	...
	if (adjusted_value == MSM_VIDC_BLUR_EXTERNAL) {
		...
		if (is_scaling_enabled(inst) || min_quality ||
		    (rc_type != HFI_RC_VBR_CFR && rc_type != HFI_RC_CBR_CFR &&
		     rc_type != HFI_RC_CBR_VFR)) {
			adjusted_value = MSM_VIDC_BLUR_NONE;
			goto exit;
		}
...
int msm_vidc_adjust_all_intra(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, ALL_INTRA, GOP_SIZE, &gop_size,
				      __func__) ||
	    msm_vidc_get_parent_value(inst, ALL_INTRA, B_FRAME, &bframe,
				      __func__))
		return -EINVAL;
...
int msm_vidc_adjust_blur_resolution(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	msm_vidc_update_cap_value(inst, BLUR_RESOLUTION, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_brs(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, CONTENT_ADAPTIVE_CODING,
	    ...
	    msm_vidc_get_parent_value(inst, CONTENT_ADAPTIVE_CODING, LAYER_TYPE,
				      &layer_type, __func__))
		return -EINVAL;
...
adjust:
	msm_vidc_update_cap_value(inst, CONTENT_ADAPTIVE_CODING, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_bitrate_boost(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, BITRATE_BOOST, MIN_QUALITY,
				      &min_quality, __func__) ||
	    msm_vidc_get_parent_value(inst, BITRATE_BOOST, BITRATE_MODE,
				      &rc_type, __func__))
		return -EINVAL;
   ...
   */
	...
	bitrate = inst->capabilities[BIT_RATE].value;
	if (adjusted_value) {
		if ((bitrate + bitrate / (100 / adjusted_value)) >
		    max_bitrate) {
			i_vpr_h(inst,
...
adjust:
	msm_vidc_update_cap_value(inst, BITRATE_BOOST, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_min_quality(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val :
				inst->capabilities[MIN_QUALITY].value;
   ...
   */
	...
	if (msm_vidc_get_parent_value(inst, MIN_QUALITY, BITRATE_MODE, &rc_type,
				      __func__) ||
	    msm_vidc_get_parent_value(inst, MIN_QUALITY, ENH_LAYER_COUNT,
				      &enh_layer_count, __func__))
		return -EINVAL;
   ...
   */
	if (inst->codec == MSM_VIDC_HEVC) {
		if (msm_vidc_get_parent_value(inst, MIN_QUALITY, PIX_FMTS,
					      &pix_fmts, __func__))
			return -EINVAL;
	...
	if (is_valid_cap(inst, META_ROI_INFO)) {
		if (msm_vidc_get_parent_value(inst, MIN_QUALITY, META_ROI_INFO,
					      &roi_enable, __func__))
			return -EINVAL;
		if (is_meta_tx_inp_enabled(inst, META_ROI_INFO)) {
			i_vpr_h(inst,
	...
	if (enh_layer_count > 0 && inst->hfi_layer_type != HFI_HIER_B) {
		i_vpr_h(inst, "%s: min quality not supported for HP encoding\n",
			__func__);
		adjusted_value = 0;
		goto update_and_exit;
	}
...
int msm_vidc_adjust_preprocess(void *instance, struct v4l2_ctrl *ctrl)
{
   ...
   */
	if (is_valid_cap(inst, CONTENT_ADAPTIVE_CODING)) {
		if (msm_vidc_get_parent_value(inst, REQUEST_PREPROCESS,
					      CONTENT_ADAPTIVE_CODING, &brs,
					      __func__))
			return -EINVAL;
		if (brs == 0) {
...
update_preprocess:
	msm_vidc_update_cap_value(inst, REQUEST_PREPROCESS, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_enc_lowlatency_mode(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	msm_vidc_update_cap_value(inst, LOWLATENCY_MODE, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_dec_lowlatency_mode(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (is_valid_cap(inst, META_OUTBUF_FENCE)) {
		if (msm_vidc_get_parent_value(inst, LOWLATENCY_MODE,
					      META_OUTBUF_FENCE, &outbuf_fence,
					      __func__))
			return -EINVAL;
...
int msm_vidc_adjust_session_priority(void *instance, struct v4l2_ctrl *ctrl)
{
   ...
   */
	...
	adjusted_value = ctrl ? ctrl->val : inst->capabilities[PRIORITY].value;
...
int msm_vidc_adjust_roi_info(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val :
				inst->capabilities[META_ROI_INFO].value;

	if (msm_vidc_get_parent_value(inst, META_ROI_INFO, BITRATE_MODE,
				      &rc_type, __func__))
		return -EINVAL;

	if (msm_vidc_get_parent_value(inst, META_ROI_INFO, PIX_FMTS, &pix_fmt,
				      __func__))
		return -EINVAL;

	if ((rc_type != HFI_RC_VBR_CFR && rc_type != HFI_RC_CBR_CFR &&
	     rc_type != HFI_RC_CBR_VFR) ||
	    !is_8bit_colorformat(pix_fmt) || is_scaling_enabled(inst) ||
	    is_rotation_90_or_270(inst))
		adjusted_value = 0;

	msm_vidc_update_cap_value(inst, META_ROI_INFO, adjusted_value,
				  __func__);

	return 0;
}

int msm_vidc_adjust_dec_outbuf_fence_type(void *instance,
					  struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val : capability[OUTBUF_FENCE_TYPE].value;
	...
	msm_vidc_update_cap_value(inst, OUTBUF_FENCE_TYPE, adjusted_value,
				  __func__);
					       ...
					       struct v4l2_ctrl *ctrl)
{
	...
	msm_vidc_update_cap_value(inst, OUTBUF_FENCE_DIRECTION, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_eva_stats(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val :
				inst->capabilities[META_EVA_STATS].value;
	...
	msm_vidc_update_cap_value(inst, META_EVA_STATS, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_sei_mastering_disp(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value =
		ctrl ? ctrl->val :
		       inst->capabilities[META_SEI_MASTERING_DISP].value;
...
adjust:
	msm_vidc_update_cap_value(inst, META_SEI_MASTERING_DISP, adjusted_value,
				  __func__);
	return 0;
}
...
int msm_vidc_adjust_sei_cll(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val :
				inst->capabilities[META_SEI_CLL].value;

	if (msm_vidc_get_parent_value(inst, META_SEI_CLL, PROFILE, &profile,
				      __func__))
		return -EINVAL;
...
int msm_vidc_adjust_hdr10plus(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, META_HDR10PLUS, PROFILE, &profile,
				      __func__))
		return -EINVAL;
...
adjust:
	msm_vidc_update_cap_value(inst, META_HDR10PLUS, adjusted_value,
				  __func__);
	return 0;
}
...
int msm_vidc_adjust_transcoding_stats(void *instance, struct v4l2_ctrl *ctrl)
{
   ...
   */
	...
	width = f->fmt.pix_mp.width;
	height = f->fmt.pix_mp.height;
	if (res_is_greater_than(width, height, MAX_TRANSCODING_STATS_WIDTH,
				MAX_TRANSCODING_STATS_HEIGHT)) {
		i_vpr_h(inst, "%s: unsupported res, wxh %ux%u\n", __func__,
			width, height);
		adjusted_value = 0;
		goto exit;
				  ...
				  enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (msm_vidc_get_parent_value(inst, cap_id, BITRATE_MODE, &rc_type,
				      __func__))
		return -EINVAL;
...
int msm_vidc_set_cbr_related_properties(
	void *instance, enum msm_vidc_inst_capability_type cap_id)
{
	int rc = 0;
	struct msm_vidc_inst *inst = (struct msm_vidc_inst *)instance;
	...
	if (rc_type != HFI_RC_CBR_VFR && rc_type != HFI_RC_CBR_CFR)
		return 0;
				  ...
				  enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (!inst->capabilities[LTR_COUNT].value ||
		...
		i_vpr_h(inst, "%s: LTR_COUNT: %d %s: %d, cap %s is not set\n",
			__func__, inst->capabilities[LTR_COUNT].value,
			cap_name(cap_id), inst->capabilities[cap_id].value,
			cap_name(cap_id));
		return 0;
	}
			...
			enum msm_vidc_inst_capability_type cap_id)
{
   ...
   */
	i_frame_qp = max(inst->capabilities[I_FRAME_MIN_QP].value,
			 inst->capabilities[MIN_FRAME_QP].value) +
		     offset;
			...
			enum msm_vidc_inst_capability_type cap_id)
{
   ...
   */
	i_frame_qp = min(inst->capabilities[I_FRAME_MAX_QP].value,
			 inst->capabilities[MAX_FRAME_QP].value) +
		     offset;
...
int msm_vidc_set_chroma_qp_index_offset(
	void *instance, enum msm_vidc_inst_capability_type cap_id)
{
	int rc = 0;
	struct msm_vidc_inst *inst = (struct msm_vidc_inst *)instance;
	...
	if (slice_mode == V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_MB) {
		hfi_value =
			(inst->codec == MSM_VIDC_HEVC) ?
				((inst->capabilities[SLICE_MAX_MB].value + 3) /
				 4) :
			    ...
			    enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (!inst->capabilities[WITHOUT_STARTCODE].value) {
		...
		rc = msm_vidc_v4l2_to_hfi_enum(inst, NAL_LENGTH_FIELD,
					       &hfi_value);
		if (rc)
			return -EINVAL;
	}
				 ...
				 enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (msm_vidc_get_parent_value(inst, cap_id, BLUR_TYPES, &blur_type,
				      __func__))
		return -EINVAL;
	...
	if (blur_width > inst->crop.width || blur_height > inst->crop.height) {
		i_vpr_e(inst, "%s: blur wxh: %dx%d exceeds crop wxh: %dx%d\n",
			__func__, blur_width, blur_height, inst->crop.width,
			inst->crop.height);
		hfi_value = 0;
	}
...
static int msm_venc_set_csc_coeff(struct msm_vidc_inst *inst,
				  const char *prop_name, u32 hfi_id,
				  void *payload, u32 payload_size,
				  u32 row_count, u32 column_count)
{
	int rc = 0;

	i_vpr_h(inst, "set cap: name: %24s, hard coded %dx%d matrix array\n",
		prop_name, row_count, column_count);
	rc = venus_hfi_session_property(inst, hfi_id, HFI_HOST_FLAGS_NONE,
					HFI_PORT_BITSTREAM,
					HFI_PAYLOAD_S32_ARRAY, payload,
					payload_size);
	if (rc) {
		i_vpr_e(inst, "%s: failed to set %s to fw\n", __func__,
			prop_name);
	}
				   ...
				   enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (!inst->capabilities[cap_id].value ||
		...
		i_vpr_h(inst, "%s: ignored as custom martix %u, csc %u\n",
			__func__, inst->capabilities[cap_id].value,
			inst->capabilities[CSC].value);
		return 0;
	}
   ...
   */
	...
	rc = msm_venc_set_csc_coeff(inst, "CSC_BIAS", HFI_PROP_CSC_BIAS,
				    &csc_bias_payload[0],
	...
	rc = msm_venc_set_csc_coeff(inst, "CSC_LIMIT", HFI_PROP_CSC_LIMIT,
				    &csc_limit_payload[0],
				  ...
				  enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (!is_critical_priority_session(inst)) {
		i_vpr_h(inst,
			"%s: reserve duration allowed only for critical session\n",
			__func__);
		return 0;
	}
...
int msm_vidc_set_s32(void *instance, enum msm_vidc_inst_capability_type cap_id)
{
	int rc = 0;
	struct msm_vidc_inst *inst = (struct msm_vidc_inst *)instance;
	s32 hfi_value = 0;
		       ...
		       enum msm_vidc_inst_capability_type cap_id)
{
	...
	rc = msm_vidc_packetize_control(inst, cap_id, HFI_PAYLOAD_U32, &stage,
					sizeof(u32), __func__);
	if (rc)
		return rc;
...
int msm_vidc_set_pipe(void *instance, enum msm_vidc_inst_capability_type cap_id)
{
	...
	rc = msm_vidc_packetize_control(inst, cap_id, HFI_PAYLOAD_U32, &pipe,
					sizeof(u32), __func__);
	if (rc)
		return rc;
				   ...
				   enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (inst->capabilities[OUTBUF_FENCE_TYPE].value == MSM_VIDC_FENCE_NONE)
		return 0;
...
int msm_vidc_set_outbuf_fence_direction(
	void *instance, enum msm_vidc_inst_capability_type cap_id)
{
	int rc = 0;
	struct msm_vidc_inst *inst = (struct msm_vidc_inst *)instance;
