...
int msm_vidc_adjust_ir_period(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, IR_PERIOD, ALL_INTRA, &all_intra,
				      __func__) ||
	...
	if (inst->codec == MSM_VIDC_HEVC) {
		if (msm_vidc_get_parent_value(inst, IR_PERIOD, PIX_FMTS,
					      &pix_fmts, __func__))
			return -EINVAL;
...
int msm_vidc_adjust_dec_frame_rate(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val :
				inst->capabilities[FRAME_RATE].value;
...
int msm_vidc_adjust_dec_operating_rate(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	adjusted_value = ctrl ? ctrl->val :
				inst->capabilities[OPERATING_RATE].value;
	msm_vidc_update_cap_value(inst, OPERATING_RATE, adjusted_value,
				  __func__);
...
int msm_vidc_adjust_delivery_mode(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	msm_vidc_update_cap_value(inst, DELIVERY_MODE, adjusted_value,
				  __func__);
			   ...
			   enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (inst->capabilities[IR_TYPE].value ==
		...
		if (inst->bufq[OUTPUT_PORT].vb2q->streaming) {
			i_vpr_h(inst,
				"%s: dynamic random intra refresh not allowed\n",
				__func__);
			return 0;
		}
	...
	if (rc) {
		i_vpr_e(inst, "%s: failed to set ir period %d\n", __func__,
			inst->capabilities[IR_PERIOD].value);
		return rc;
	}
				   ...
				   enum msm_vidc_inst_capability_type cap_id)
{
	...
	if (primaries != V4L2_COLORSPACE_DEFAULT ||
		...
		primaries = v4l2_color_primaries_to_driver(inst, primaries,
							   __func__);
		matrix_coeff = v4l2_matrix_coeff_to_driver(inst, matrix_coeff,
							   __func__);
	...
	hfi_value = (matrix_coeff & 0xFF) | ((transfer_char << 8) & 0xFF00) |
...
int msm_vidc_adjust_csc(void *instance, struct v4l2_ctrl *ctrl)
{
	...
	if (msm_vidc_get_parent_value(inst, CSC, PIX_FMTS, &pix_fmt, __func__))
		return -EINVAL;
