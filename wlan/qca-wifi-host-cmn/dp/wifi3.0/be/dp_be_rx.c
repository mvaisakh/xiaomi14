...
static inline void dp_rx_update_flow_info(qdf_nbuf_t nbuf, uint8_t *rx_tlv_hdr)
{
   ...
   */
	...
	if (!hal_rx_msdu_flow_idx_invalid_be(rx_tlv_hdr) &&
	    (fse_metadata == DP_RX_FSE_FLOW_MATCH_SFE))
		return;

	qdf_nbuf_set_rx_flow_idx_valid(
		nbuf, !hal_rx_msdu_flow_idx_invalid_be(rx_tlv_hdr));
}
#else
static inline void dp_rx_update_flow_info(qdf_nbuf_t nbuf, uint8_t *rx_tlv_hdr)
{
...
static void dp_rx_wds_learn(struct dp_soc *soc, struct dp_vdev *vdev,
	...
	if (qdf_nbuf_is_rx_chfrag_start(nbuf) &&
		...
		if (qdf_unlikely(ta_txrx_peer->nawds_enabled &&
			...
			ta_base_peer = dp_get_primary_link_peer_by_id(
				soc, ta_txrx_peer->peer_id, DP_MOD_ID_RX);
		} else {
		...
		soc->cdp_soc.ol_ops->rx_wds_ext_peer_learn(
			soc->ctrl_psoc, ta_txrx_peer->peer_id,
			ta_txrx_peer->vdev->vdev_id, wds_ext_src_mac);
	}
}
#else
static inline void dp_wds_ext_peer_learn_be(struct dp_soc *soc,
...
more_data:
	...
	last_prefetched_hw_desc = dp_srng_dst_prefetch_32_byte_desc(
		hal_soc, hal_ring_hdl, num_pending);
	/*
   ...
   */
	while (qdf_likely(num_pending)) {
		...
		rx_desc =
			(struct dp_rx_desc *)hal_rx_get_reo_desc_va(ring_desc);
...
#endif
					dp_rx_nbuf_unmap(soc, rx_desc,
							 reo_ring_num);
				rx_desc->unmapped = 1;
				dp_rx_buffer_pool_nbuf_free(soc, rx_desc->nbuf,
							    rx_desc->pool_id);
				dp_rx_add_to_free_desc_list(
					&head[rx_desc->chip_id]
					     [rx_desc->pool_id],
					&tail[rx_desc->chip_id]
					     [rx_desc->pool_id],
					rx_desc);
			}
			continue;
     ...
     */
		...
		pkt_capture_offload = dp_rx_copy_desc_info_in_nbuf_cb(
			soc, ring_desc, rx_desc->nbuf, reo_ring_num);
       ...
       */
			if (is_prev_msdu_last) {
				...
				num_entries_avail = hal_srng_dst_num_valid(
					hal_soc, hal_ring_hdl, 1);
     ...
     */
		...
		if (!xsk_pool || !soc->osrtp_info.in_use ||
		    qdf_nbuf_is_rx_chfrag_cont(rx_desc->nbuf)) {
			if (rx_desc->xbuf)
			...
			if (rx_desc->xbuf &&
			    (qdf_xbuf_is_pool_equal(rx_desc->xbuf, xsk_pool))) {
				DP_RX_XBUF_LIST_APPEND(xbuf_head, xbuf_tail,
						       rx_desc->xbuf);
			} else {
				if (rx_desc->xbuf &&
				    (qdf_xbuf_is_pool_equal(rx_desc->xbuf,
							    xsk_pool) == false))
					dp_rx_buffers_xbuf_free(
						rx_desc->xbuf,
						rx_desc_pool->buf_size);

				rx_desc->xbuf = dp_rx_buffer_pool_xbuf_alloc(
					soc, rx_desc->nbuf);
				if (rx_desc->xbuf) {
					rx_desc->xbuf->nbuf = rx_desc->nbuf;
					rx_desc->xbuf->pool_id =
						rx_desc->pool_id;
					qdf_mem_copy(rx_desc->xbuf->xdp->data,
						     rx_desc->xbuf->orig_ndata,
						     rx_desc_pool->buf_size);
					DP_RX_XBUF_LIST_APPEND(xbuf_head,
							       xbuf_tail,
							       rx_desc->xbuf);
				} else
					DP_RX_PROCESS_NBUF(soc, nbuf_head,
							   nbuf_tail, ebuf_head,
							   ebuf_tail, rx_desc);
			}
		}
		...
		dp_rx_prefetch_hw_sw_nbuf_32_byte_desc(
			soc, hal_soc, num_pending, hal_ring_hdl,
			&last_prefetched_hw_desc, &last_prefetched_sw_desc);
	...
	for (chip_id = 0; chip_id < WLAN_MAX_MLO_CHIPS; chip_id++) {
       ...
       */
			...
			dp_rx_buffers_replenish_simple(
				replenish_soc, mac_id, dp_rxdma_srng,
	...
	while (xbuf) {
		xnext = xbuf->next;
		dp_rx_prefetch_nbuf_data_be(xbuf->nbuf,
					    xnext ? xnext->nbuf : NULL);
		...
		if (dp_rx_is_xbuf_list_ready(deliver_xlist_head, vdev,
					     txrx_peer, peer_id, vdev_id)) {
			dp_rx_deliver_osrtp_to_stack(soc, vdev, txrx_peer,
						     deliver_xlist_head,
						     deliver_xlist_tail,
						     &nbuf_list);
			deliver_xlist_head = NULL;
		...
		if (qdf_unlikely(!txrx_peer)) {
			txrx_peer = dp_rx_get_txrx_peer_and_vdev(
				soc, nbuf, peer_id, &txrx_ref_handle,
				pkt_capture_offload, &vdev, &rx_pdev, &dsf,
				&old_tid);
		...
		if (qdf_unlikely(!txrx_peer || !vdev)) {
			DP_RX_XBUF_PROCESS_NBUF(soc, nbuf_head, nbuf_tail,
						ebuf_head, ebuf_tail, xbuf);
			dp_rx_buffers_xbuf_free(xbuf, xbuf->pkt_len);
			xbuf = xnext;
			continue;
		}

		DP_RX_XBUF_LIST_APPEND(deliver_xlist_head, deliver_xlist_tail,
				       xbuf);
		xbuf = xnext;
	}
	...
	if (qdf_likely(deliver_xlist_head)) {
		if (qdf_likely(txrx_peer)) {
			...
			while (xbuf) {
				xnext = xbuf->next;
				xbuf->next = NULL;
				DP_RX_XBUF_PROCESS_NBUF(soc, nbuf_head,
							nbuf_tail, ebuf_head,
							ebuf_tail, xbuf);
				dp_rx_buffers_xbuf_free(xbuf, xbuf->pkt_len);
				xbuf = xnext;
   ...
   */
     ...
     */
		if (qdf_unlikely(qdf_nbuf_is_frag(nbuf))) {
			...
			is_mcbc = hal_rx_msdu_end_da_is_mcbc_get(soc->hal_soc,
								 rx_tlv_hdr);
			is_sa_vld = hal_rx_msdu_end_sa_is_valid_get(
				soc->hal_soc, rx_tlv_hdr);
			is_da_vld = hal_rx_msdu_end_da_is_valid_get(
				soc->hal_soc, rx_tlv_hdr);
			...
			if (qdf_nbuf_is_raw_frame(nbuf)) {
				...
				DP_PEER_PER_PKT_STATS_INC_PKT(txrx_peer, rx.raw,
							      1, msdu_len,
							      link_id);
			} else {
		...
		if (!dp_wds_rx_policy_check(rx_tlv_hdr, vdev, txrx_peer)) {
			...
			DP_PEER_PER_PKT_STATS_INC(
				txrx_peer, rx.policy_check_drop, 1, link_id);
     ...
     */
		if (qdf_likely(txrx_peer) &&
			...
			if (!is_eapol) {
				DP_PEER_PER_PKT_STATS_INC(
					txrx_peer, rx.peer_unauth_rx_pkt_drop,
					1, link_id);
				dp_rx_nbuf_free(nbuf);
				nbuf = next;
				continue;
       ...
       */
		...
		DP_PEER_TO_STACK_INCC_PKT(
			txrx_peer, 1, QDF_NBUF_CB_RX_PKT_LEN(nbuf), enh_flag);
		if (qdf_unlikely(txrx_peer->in_twt))
			DP_PEER_PER_PKT_STATS_INC_PKT(
				txrx_peer, rx.to_stack_twt, 1,
	...
	DP_RX_DELIVER_TO_STACK(soc, vdev, txrx_peer, peer_id,
			       pkt_capture_offload, deliver_list_head,
			       deliver_list_tail);
   ...
   */
	...
	if (dp_rx_enable_eol_data_check(soc) && rx_bufs_used) {
		if (quota) {
			num_pending = dp_rx_srng_get_num_pending(
				hal_soc, hal_ring_hdl, num_entries, &near_full);
			if (num_pending) {
 ...
 */
static QDF_STATUS dp_rx_desc_pool_init_be_cc(struct dp_soc *soc,
					     struct rx_desc_pool *rx_desc_pool,
					     uint32_t pool_id)
{
     ...
     */
		...
		rx_desc_elem->rx_desc.cookie = dp_cc_desc_id_generate(
			page_desc->ppt_index, avail_entry_index);
					     ...
					     uint32_t pool_id)
{
     ...
     */
		...
		rx_desc_pool->array[i].rx_desc.cookie = dp_cc_desc_id_generate(
			page_desc->ppt_index, avail_entry_index);
...
static void dp_rx_desc_pool_deinit_be_cc(struct dp_soc *soc,
					 struct rx_desc_pool *rx_desc_pool,
					 uint32_t pool_id)
{
	struct dp_spt_page_desc *page_desc;
				   ...
				   uint32_t pool_id)
{
	...
	if (rx_desc_pool->desc_type == QDF_DP_RX_DESC_BUF_TYPE) {
		...
		status = dp_rx_desc_pool_init_be_cc(soc, rx_desc_pool, pool_id);
	} else {
					       ...
					       struct dp_rx_desc **r_rx_desc)
{
	if (hal_rx_wbm_get_cookie_convert_done(ring_desc)) {
		...
		*r_rx_desc =
			(struct dp_rx_desc *)hal_rx_wbm_get_desc_va(ring_desc);
	} else {
		/* SW do cookie conversion */
		uint32_t cookie = HAL_RX_BUF_COOKIE_GET(ring_desc);

		*r_rx_desc = (struct dp_rx_desc *)dp_cc_desc_find(soc, cookie);
	}
...
QDF_STATUS dp_wbm_get_rx_desc_from_hal_desc_be(struct dp_soc *soc,
					       ...
					       struct dp_rx_desc **r_rx_desc)
{
	*r_rx_desc = (struct dp_rx_desc *)hal_rx_wbm_get_desc_va(ring_desc);
...
static inline void dp_rx_dummy_src_mac(struct dp_vdev *vdev, qdf_nbuf_t nbuf)
{
...
static inline bool dp_rx_check_ext_hybrid_mode(struct dp_soc *soc,
					       struct dp_vdev *vdev)
{
...
bool dp_rx_mlo_igmp_handler(struct dp_soc *soc, struct dp_vdev *vdev,
			    ...
			    uint8_t link_id)
{
	...
	struct cdp_tid_rx_stats *tid_stats =
		&peer->vdev->pdev->stats.tid_stats.tid_rx_wbm_stats[0][tid];
	...
	if (qdf_unlikely(vdev->multipass_en)) {
		if (dp_rx_multipass_process(peer, nbuf, tid) == false) {
			DP_PEER_PER_PKT_STATS_INC(
				peer, rx.multipass_rx_pkt_drop, 1, link_id);
			return false;
		}
	}
   ...
   */
	if (vdev->mcast_enhancement_en || peer->is_mld_peer ||
	    peer->nawds_enabled || !vdev->mlo_vdev ||
...
send_pkt:
	dp_rx_deliver_to_stack(be_vdev->vdev.pdev->soc, &be_vdev->vdev, peer,
			       nbuf, NULL);
	return true;
}
#else
 ...
 */
static inline bool dp_rx_intrabss_fwd_mlo_allow(struct dp_txrx_peer *ta_peer,
 ...
 */
static bool
dp_rx_intrabss_ucast_check_be(qdf_nbuf_t nbuf, struct dp_txrx_peer *ta_peer,
			      uint8_t *rx_tlv_hdr,
			      struct hal_rx_msdu_metadata *msdu_metadata,
			      struct dp_be_intrabss_params *params)
{
	uint8_t dest_chip_id, dest_chip_pmac_id;
	struct dp_vdev_be *be_vdev = dp_get_be_vdev_from_dp_vdev(ta_peer->vdev);
	struct dp_soc_be *be_soc = dp_get_be_soc_from_dp_soc(params->dest_soc);
	uint16_t da_peer_id;
	...
	hal_rx_tlv_get_dest_chip_pmac_id(rx_tlv_hdr, &dest_chip_id,
					 &dest_chip_pmac_id);

	params->dest_soc =
		dp_mlo_get_soc_ref_by_chip_id(be_soc->ml_ctxt, dest_chip_id);
	if (!params->dest_soc)
		return false;
			      ...
			      struct dp_be_intrabss_params *params)
{
   ...
   */
	if (dp_get_peer_isolation(ta_peer) || dp_get_peer_isolation(da_peer)) {
		ret = false;
		goto rel_da_peer;
	}
...
void dp_rx_word_mask_subscribe_be(struct dp_soc *soc, uint32_t *msg_word,
				  void *rx_filter)
{
	...
	*msg_word = 0;
	...
	HTT_RX_RING_SELECTION_CFG_RX_MPDU_START_WORD_MASK_SET(
		*msg_word, tlv_filter->rx_mpdu_start_wmask);

	/* word 15 */
	msg_word++;
	*msg_word = 0;
	HTT_RX_RING_SELECTION_CFG_RX_MSDU_END_WORD_MASK_SET(
		*msg_word, tlv_filter->rx_msdu_end_wmask);
}
#else
void dp_rx_word_mask_subscribe_be(struct dp_soc *soc, uint32_t *msg_word,
				  void *rx_filter)
...
static inline bool dp_rx_intrabss_mlo_mcbc_fwd(struct dp_soc *soc,
					       struct dp_vdev *vdev,
					       qdf_nbuf_t nbuf_copy)
{
	...
	mcast_primary_vdev =
		dp_mlo_get_mcast_primary_vdev(be_soc, be_vdev, DP_MOD_ID_RX);

	if (!mcast_primary_vdev)
		return false;

	nbuf_copy = dp_tx_send_exception(
		(struct cdp_soc_t *)mcast_primary_vdev->pdev->soc,
		mcast_primary_vdev->vdev_id, nbuf_copy, &tx_exc_metadata);

	if (nbuf_copy)
		qdf_nbuf_free(nbuf_copy);

	dp_vdev_unref_delete(mcast_primary_vdev->pdev->soc, mcast_primary_vdev,
			     DP_MOD_ID_RX);
	return true;
}
#else
...
bool dp_rx_intrabss_mcast_handler_be(struct dp_soc *soc,
				     struct dp_txrx_peer *ta_txrx_peer,
				     qdf_nbuf_t nbuf_copy,
				     struct cdp_tid_rx_stats *tid_stats,
				     uint8_t link_id)
{
	if (qdf_unlikely(ta_txrx_peer->vdev->nawds_enabled)) {
		...
		if (dp_tx_send_exception((struct cdp_soc_t *)soc,
					 ta_txrx_peer->vdev->vdev_id, nbuf_copy,
					 &tx_exc_metadata)) {
	...
	if (dp_rx_intrabss_mlo_mcbc_fwd(soc, ta_txrx_peer->vdev, nbuf_copy))
		return true;
			   ...
			   uint8_t link_id)
{
	...
	struct cdp_tid_rx_stats *tid_stats =
		&ta_peer->vdev->pdev->stats.tid_stats.tid_rx_stats[ring_id][tid];
	bool ret = false;
	struct dp_be_intrabss_params params;
	struct hal_rx_msdu_metadata msdu_metadata;
   ...
   */
	if (qdf_nbuf_is_da_mcbc(nbuf) && !ta_peer->bss_peer) {
		return dp_rx_intrabss_mcbc_fwd(soc, ta_peer, rx_tlv_hdr, nbuf,
					       tid_stats, link_id);
	}
			  ...
			  uint8_t *rx_tlv_hdr, uint8_t mac_id)
{
   ...
   * up
   */
	if (!dp_pdev->first_nbuf || (dp_pdev->invalid_peer_head_msdu &&
				     QDF_NBUF_CB_RX_NUM_ELEMENTS_IN_LIST(
					     dp_pdev->invalid_peer_head_msdu) >=
					     DP_MAX_INVALID_BUFFERS)) {
   ...
   */
	...
	DP_RX_LIST_APPEND(dp_pdev->invalid_peer_head_msdu,
			  dp_pdev->invalid_peer_tail_msdu, nbuf);

	return mpdu_done;
}

qdf_nbuf_t dp_rx_wbm_err_reap_desc_be(struct dp_intr *int_ctx,
				      struct dp_soc *soc,
				      hal_ring_handle_t hal_ring_hdl,
				      uint32_t quota, uint32_t *rx_bufs_used)
{
	hal_ring_desc_t ring_desc;
	hal_soc_handle_t hal_soc;
	...
	while (qdf_likely(quota)) {
		...
		if (dp_wbm_get_rx_desc_from_hal_desc_be(soc, ring_desc,
							&rx_desc)) {
		...
		if (!dp_rx_desc_check_magic(rx_desc)) {
			dp_rx_err_err("%pK: Invalid rx_desc %pK", soc, rx_desc);
			continue;
		}
     ...
     */
		wbm_err.info = dp_rx_wbm_err_copy_desc_info_in_nbuf(
			soc, ring_desc, nbuf, rx_desc->pool_id);
		/*
     ...
     */
		...
		if (qdf_unlikely(soc->wbm_release_desc_rx_sg_support &&
				 dp_rx_is_sg_formation_required(
					 &wbm_err.info_bit))) {
			/* SG is detected from continuation bit */
			msdu_continuation = dp_rx_wbm_err_msdu_continuation_get(
				soc, ring_desc, nbuf);
			    ...
			    bool is_reo_exception, uint8_t link_id)
{
   ...
   */
	...
	if (!txrx_peer && dp_rx_null_q_handle_invalid_peer_id_exception(
				  soc, pool_id, rx_tlv_hdr, nbuf))
		return QDF_STATUS_E_FAILURE;
	...
	if (!txrx_peer) {
		...
		if (qdf_unlikely(soc->wbm_release_desc_rx_sg_support)) {
			...
			mpdu_done = soc->arch_ops.dp_rx_chain_msdus(
				soc, nbuf, rx_tlv_hdr, pool_id);
			/* Trigger invalid peer handler wrapper */
			dp_rx_process_invalid_peer_wrapper(
				soc, pdev->invalid_peer_head_msdu, mpdu_done,
				pool_id);
		}
   ...
   */
	if (qdf_unlikely(
		    check_extap_multicast_loopback(vdev, eh->ether_shost))) {
		DP_PEER_PER_PKT_STATS_INC_PKT(txrx_peer, rx.mec_drop, 1,
					      qdf_nbuf_len(nbuf), link_id);
		goto drop_nbuf;
	}
     ...
     */
		...
		if (qdf_unlikely(hal_rx_msdu_end_da_is_mcbc_get(soc->hal_soc,
			...
			DP_PEER_UC_INCC_PKT(txrx_peer, 1, qdf_nbuf_len(nbuf),
					    enh_flag, link_id);
		}
		...
		if (qdf_unlikely(vdev->multipass_en)) {
			if (dp_rx_multipass_process(txrx_peer, nbuf, tid) ==
			    false) {
				DP_PEER_PER_PKT_STATS_INC(
					txrx_peer, rx.multipass_rx_pkt_drop, 1,
					link_id);
				goto drop_nbuf;
			}
		}
