...
#define DP_TCL_METADATA_TYPE_PEER_BASED HTT_TCL_METADATA_V2_TYPE_PEER_BASED
#define DP_TCL_METADATA_TYPE_VDEV_BASED HTT_TCL_METADATA_V2_TYPE_VDEV_BASED
#else
#define DP_TX_TCL_METADATA_PDEV_ID_SET(_var, _val) \
...
#define DP_TCL_METADATA_TYPE_PEER_BASED HTT_TCL_METADATA_TYPE_PEER_BASED
#define DP_TCL_METADATA_TYPE_VDEV_BASED HTT_TCL_METADATA_TYPE_VDEV_BASED
#endif
...
uint8_t sec_type_map[MAX_CDP_SEC_TYPE] = {
	HAL_TX_ENCRYPT_TYPE_NO_CIPHER,	   HAL_TX_ENCRYPT_TYPE_WEP_128,
	HAL_TX_ENCRYPT_TYPE_WEP_104,	   HAL_TX_ENCRYPT_TYPE_WEP_40,
	HAL_TX_ENCRYPT_TYPE_TKIP_WITH_MIC, HAL_TX_ENCRYPT_TYPE_TKIP_NO_MIC,
	HAL_TX_ENCRYPT_TYPE_AES_CCMP_128,  HAL_TX_ENCRYPT_TYPE_WAPI,
	HAL_TX_ENCRYPT_TYPE_AES_CCMP_256,  HAL_TX_ENCRYPT_TYPE_AES_GCMP_128,
	HAL_TX_ENCRYPT_TYPE_AES_GCMP_256,  HAL_TX_ENCRYPT_TYPE_WAPI_GCM_SM4
};
...
static inline void dp_tx_desc_history_add(struct dp_soc *soc, dma_addr_t paddr,
...
static inline void dp_tx_tso_seg_history_add(struct dp_soc *soc,
...
static inline void dp_tx_tso_history_add(struct dp_soc *soc,
				    ...
				    struct qdf_tso_num_seg_elem_t *num_seg_desc)
{
	...
	if (qdf_unlikely(!seg_desc)) {
		DP_TRACE(ERROR, "%s %d TSO desc is NULL!", __func__, __LINE__);
		qdf_assert(0);
	} else if (qdf_unlikely(!num_seg_desc)) {
		DP_TRACE(ERROR, "%s %d TSO num desc is NULL!", __func__,
			 __LINE__);
		qdf_assert(0);
	} else {
		bool is_last_seg;
		...
		is_last_seg = (num_seg_desc->num_seg.tso_cmn_num_seg == 1) ?
				      true :
				      false;
		qdf_nbuf_unmap_tso_segment(soc->osdev, seg_desc, is_last_seg);
	...
	if (qdf_unlikely(!tx_desc->msdu_ext_desc->tso_desc)) {
		...
		struct qdf_tso_num_seg_elem_t *tso_num_desc =
			(struct qdf_tso_num_seg_elem_t *)
				tx_desc->msdu_ext_desc->tso_num_desc;
		...
		dp_tx_tso_desc_free(soc, tx_desc->pool_id,
				    tx_desc->msdu_ext_desc->tso_desc);
...
static inline int dp_tx_release_ds_tx_desc(struct dp_soc *soc,
					   struct dp_tx_desc_s *tx_desc,
			...
			uint8_t desc_pool_id)
{
	...
	if (HAL_TX_COMP_RELEASE_SOURCE_TQM == tx_desc->buffer_src)
					  ...
					  struct dp_tx_msdu_info_s *msdu_info)
{
   ...
   */
	...
	if (vdev->mesh_vdev || msdu_info->is_tx_sniffer ||
	    HTT_TX_MSDU_EXT2_DESC_FLAG_VALID_KEY_FLAGS_GET(
				       ...
				       void *ext_desc)
{
   ...
   */
	...
	for (num_frag = 0; num_frag < tso_seg->num_frags; num_frag++) {
		...
		qdf_dmaaddr_to_32s(tso_seg->tso_frags[num_frag].paddr, &lo,
				   &hi);
 ...
 * Return: void
 */
static void dp_tx_free_tso_seg_list(struct dp_soc *soc,
				    ...
				    struct dp_tx_msdu_info_s *msdu_info)
{
	...
	while (free_seg) {
		...
		dp_tx_tso_desc_free(soc, msdu_info->tx_queue.desc_pool_id,
				    free_seg);
		free_seg = next_seg;
	}
}
 ...
 */
static void
dp_tx_free_tso_num_seg_list(struct dp_soc *soc,
			    ...
			    struct dp_tx_msdu_info_s *msdu_info)
{
	...
	while (free_num_seg) {
		...
		dp_tso_num_seg_free(soc, msdu_info->tx_queue.desc_pool_id,
				    free_num_seg);
 ...
 */
static void
dp_tx_unmap_tso_seg_list(struct dp_soc *soc,
			 struct qdf_tso_seg_elem_t *free_seg,
			 struct qdf_tso_num_seg_elem_t *num_seg_desc)
{
	struct qdf_tso_seg_elem_t *next_seg;
 ...
 */
static QDF_STATUS dp_tx_prepare_tso(struct dp_vdev *vdev, qdf_nbuf_t msdu,
				    struct dp_tx_msdu_info_s *msdu_info)
{
	struct qdf_tso_seg_elem_t *tso_seg;
	...
	tso_num_seg =
		dp_tso_num_seg_alloc(soc, msdu_info->tx_queue.desc_pool_id);
	...
	msdu_info->num_seg = qdf_nbuf_get_tso_info(soc->osdev, msdu, tso_info);

	TSO_DEBUG(" %s: msdu_info->num_seg: %d", __func__, msdu_info->num_seg);
	...
	dp_tx_tso_history_add(soc, msdu_info->u.tso_info, msdu, 0,
			      DP_TX_DESC_MAP);

	tso_info->curr_seg = tso_info->tso_seg_list;

	tso_info->msdu_stats_idx = dp_tso_get_stats_idx(pdev);
	dp_tso_packet_update(pdev, tso_info->msdu_stats_idx, msdu,
			     msdu_info->num_seg);
 ...
 * Return:
 */
static struct dp_tx_ext_desc_elem_s *
dp_tx_prepare_ext_desc(struct dp_vdev *vdev,
		       struct dp_tx_msdu_info_s *msdu_info,
		       uint8_t desc_pool_id)
{
	uint8_t i;
	...
	if (msdu_info->exception_fw && qdf_unlikely(vdev->mesh_vdev)) {
	...
	hal_tx_ext_desc_sync(&cached_ext_desc[0], msdu_ext_desc->vaddr);
...
static void dp_tx_trace_pkt(struct dp_soc *soc, qdf_nbuf_t skb,
			    uint16_t msdu_id, uint8_t vdev_id,
			    ...
			    enum QDF_OPMODE op_mode)
{
	...
	DPTRACE(qdf_dp_trace_ptr(
		skb, QDF_DP_TRACE_LI_DP_TX_PACKET_PTR_RECORD,
		QDF_TRACE_DEFAULT_PDEV_ID, qdf_nbuf_data_addr(skb),
		sizeof(qdf_nbuf_data(skb)), msdu_id, vdev_id, 0, op_mode));
...
dp_tx_get_traffic_end_indication_pkt(struct dp_vdev *vdev, qdf_nbuf_t nbuf)
{
	...
	if (qdf_unlikely(QDF_NBUF_CB_GET_PACKET_TYPE(nbuf) ==
		...
		if (!end_nbuf) {
			end_nbuf = qdf_nbuf_alloc(
				NULL, (htt_desc_size_aligned + end_nbuf_len),
 ...
 */
static inline void
dp_tx_send_traffic_end_indication_pkt(struct dp_vdev *vdev, qdf_nbuf_t end_nbuf,
				      struct dp_tx_msdu_info_s *msdu_info,
				      uint16_t peer_id)
{
	...
	nbuf = dp_tx_send_msdu_single(vdev, end_nbuf, &e_msdu_info, peer_id,
				      NULL);
	if (nbuf) {
 ...
 */
static bool dp_tx_traffic_end_indication_enq_ind_pkt(struct dp_soc *soc,
						     struct dp_tx_desc_s *desc,
						     qdf_nbuf_t nbuf)
{
	struct dp_vdev *vdev = NULL;

	if (qdf_unlikely((desc->flags & DP_TX_DESC_FLAG_TRAFFIC_END_IND) !=
			 0)) {
 ...
 */
static inline bool dp_tx_traffic_end_indication_is_enabled(struct dp_vdev *vdev)
{
			       ...
			       uint16_t peer_id, qdf_nbuf_t end_nbuf)
{
	...
	if (qdf_unlikely(end_nbuf))
		dp_tx_send_traffic_end_indication_pkt(vdev, end_nbuf, msdu_info,
						      peer_id);
	return nbuf;
}
#else
...
static inline bool dp_tx_traffic_end_indication_enq_ind_pkt(
	struct dp_soc *soc, struct dp_tx_desc_s *desc, qdf_nbuf_t nbuf)
{
	return false;
}
...
{
	if (soc->features.wds_ext_ast_override_enable && tx_exc_metadata &&
	    tx_exc_metadata->is_wds_extended)
		return true;
...
dp_tx_prepare_desc_single(struct dp_vdev *vdev, qdf_nbuf_t nbuf,
			  uint8_t desc_pool_id,
			  struct dp_tx_msdu_info_s *msdu_info,
			  struct cdp_tx_exception_metadata *tx_exc_metadata)
{
	...
	tx_desc->tx_encap_type =
		((tx_exc_metadata && (tx_exc_metadata->tx_encap_type !=
   ...
   */
	if (qdf_unlikely((msdu_info->exception_fw)) ||
	    ...
	    (tx_exc_metadata && tx_exc_metadata->is_tx_sniffer)) {
		align_pad = ((unsigned long)qdf_nbuf_data(nbuf)) & 0x7;

		if (qdf_unlikely(qdf_nbuf_headroom(nbuf) < align_pad)) {
			DP_STATS_INC(vdev, tx_i.dropped.headroom_insufficient,
				     1);
			goto failure;
		}
		...
		htt_hdr_size =
			dp_tx_prepare_htt_metadata(vdev, nbuf, msdu_info);
		if (htt_hdr_size == 0)
			goto failure;
		...
		dp_tx_traffic_end_indication_set_desc_flag(tx_desc, msdu_info);
		is_exception = 1;
 ...
 */
static struct dp_tx_desc_s *
dp_tx_prepare_desc(struct dp_vdev *vdev, qdf_nbuf_t nbuf,
		   struct dp_tx_msdu_info_s *msdu_info, uint8_t desc_pool_id)
{
	...
	dp_tx_tso_seg_history_add(soc, msdu_info->u.tso_info.curr_seg, nbuf,
				  tx_desc->id, DP_TX_DESC_COOKIE);
				    ...
				    struct dp_tx_msdu_info_s *msdu_info)
{
     ...
     */
		...
		if (QDF_STATUS_SUCCESS !=
		    qdf_nbuf_map_nbytes_single(vdev->osdev, curr_nbuf,
					       QDF_DMA_TO_DEVICE,
					       curr_nbuf->len)) {
 ...
 * Return: None
 */
static void dp_tx_raw_prepare_unset(struct dp_soc *soc, qdf_nbuf_t nbuf)
{
	qdf_nbuf_t cur_nbuf = nbuf;

	do {
		qdf_nbuf_unmap_nbytes_single(soc->osdev, cur_nbuf,
					     QDF_DMA_TO_DEVICE, cur_nbuf->len);
					       ...
					       qdf_nbuf_t nbuf)
{
	...
	do {
		...
		if (qdf_unlikely(((vdev_local)->tx_encap_type) ==
			...
			break;
		dp_vdev_peer_stats_update_protocol_cnt(
			(vdev_local), (nbuf_local), NULL, 1, 0);
	} while (0);
}
#endif

#ifdef WLAN_DP_FEATURE_SW_LATENCY_MGR
void dp_tx_update_stats(struct dp_soc *soc, struct dp_tx_desc_s *tx_desc,
			uint8_t ring_id)
{
...
int dp_tx_attempt_coalescing(struct dp_soc *soc, struct dp_vdev *vdev,
...
void dp_tx_ring_access_end(struct dp_soc *soc, hal_ring_handle_t hal_ring_hdl,
			   int coalesce)
{
	if (coalesce)
...
static inline void dp_flush_tcp_hp(struct dp_soc *soc, uint8_t ring_id)
{
	hal_ring_handle_t hal_ring_hdl = dp_tx_get_hal_ring_hdl(soc, ring_id);
...
static inline void dp_tx_check_and_flush_hp(struct dp_soc *soc,
					    QDF_STATUS status,
					    struct dp_tx_msdu_info_s *msdu_info)
{
	if (QDF_IS_STATUS_ERROR(status) && !msdu_info->skip_hp_update) {
		dp_flush_tcp_hp(soc, (msdu_info->tx_queue.ring_id &
				      DP_TX_QUEUE_MASK));
	}
}
#else
...
void dp_tx_ring_access_end_wrapper(struct dp_soc *soc,
				   hal_ring_handle_t hal_ring_hdl, int coalesce)
{
	int ret;
	...
	if (DP_FRAME_IS_LLC(ether_type) && DP_FRAME_IS_SNAP(llcHdr)) {
		...
		if (ether_type == htons(ETHERTYPE_VLAN)) {
			L3datap = hdr_ptr + sizeof(qdf_ethervlan_header_t) +
				  sizeof(*llcHdr);
			ether_type = (uint16_t)*(
				nbuf->data + 2 * QDF_MAC_ADDR_SIZE +
				sizeof(*llcHdr) + sizeof(qdf_net_vlanhdr_t));
		} else {
			L3datap = hdr_ptr + sizeof(qdf_ether_header_t) +
				  sizeof(*llcHdr);
		}
 ...
 * Return: None
 */
static void dp_tx_update_tdls_flags(struct dp_soc *soc, struct dp_vdev *vdev,
				    struct dp_tx_desc_s *tx_desc)
{
	if (vdev) {
...
static uint8_t dp_htt_tx_comp_get_status(struct dp_soc *soc, char *htt_desc)
{
	...
	case CDP_ARCH_TYPE_RH: {
		...
		tx_status =
			HTT_TX_MSDU_INFO_RELEASE_REASON_GET(*(msg_word + 3));
	} break;
	default:
					     ...
					     struct dp_tx_desc_s *tx_desc)
{
	...
	struct dp_vdev *vdev =
		dp_vdev_get_ref_by_id(soc, tx_desc->vdev_id, DP_MOD_ID_TDLS);
	...
	if (vdev->tx_non_std_data_callback.func) {
		...
		vdev->tx_non_std_data_callback.func(
			vdev->tx_non_std_data_callback.ctxt, nbuf, tx_status);
					       ...
					       qdf_nbuf_t nbuf)
{
	if (qdf_likely(!(tx_desc->flags & DP_TX_DESC_FLAG_TDLS_FRAME)))
		return qdf_nbuf_map_nbytes_single(vdev->osdev, nbuf,
...
static inline void
dp_non_std_htt_tx_comp_free_buff(struct dp_soc *soc,
				 struct dp_tx_desc_s *tx_desc)
{
}
...
static inline QDF_STATUS dp_tx_msdu_single_map(struct dp_vdev *vdev,
					       ...
					       qdf_nbuf_t nbuf)
{
	return qdf_nbuf_map_nbytes_single(vdev->osdev, nbuf, QDF_DMA_TO_DEVICE,
					  nbuf->len);
}
#endif

static inline qdf_dma_addr_t
dp_tx_nbuf_map_regular(struct dp_vdev *vdev, struct dp_tx_desc_s *tx_desc,
		       qdf_nbuf_t nbuf)
{
...
static inline void dp_tx_nbuf_unmap_regular(struct dp_soc *soc,
					    struct dp_tx_desc_s *desc)
{
	qdf_nbuf_unmap_nbytes_single_paddr(soc->osdev, desc->nbuf,
					   desc->dma_addr, QDF_DMA_TO_DEVICE,
					   desc->length);
}

#ifdef QCA_DP_TX_RMNET_OPTIMIZATION
static inline bool is_nbuf_frm_rmnet(qdf_nbuf_t nbuf,
				     struct dp_tx_msdu_info_s *msdu_info)
{
...
static inline qdf_dma_addr_t
dp_tx_rmnet_nbuf_map(struct dp_tx_msdu_info_s *msdu_info,
		     struct dp_tx_desc_s *tx_desc)
{
	qdf_dma_addr_t paddr;
	...
	qdf_nbuf_dma_clean_range(
		...
		(void *)(msdu_info->payload_addr + msdu_info->buf_len));
...
static inline qdf_dma_addr_t dp_tx_nbuf_map(struct dp_vdev *vdev,
					    struct dp_tx_desc_s *tx_desc,
					    qdf_nbuf_t nbuf)
{
#ifdef WLAN_FEATURE_OSRTP
...
static inline void dp_tx_nbuf_unmap(struct dp_soc *soc,
				    struct dp_tx_desc_s *desc)
{
...
static inline void dp_tx_enh_unmap(struct dp_soc *soc,
				   struct dp_tx_desc_s *desc)
{
...
int dp_tx_frame_is_drop(struct dp_vdev *vdev, uint8_t *srcmac, uint8_t *dstmac)
{
	...
	dst_ast_entry = dp_peer_ast_hash_find_by_pdevid(soc, dstmac,
							vdev->pdev->pdev_id);

	src_ast_entry = dp_peer_ast_hash_find_by_pdevid(soc, srcmac,
							vdev->pdev->pdev_id);
	if (dst_ast_entry && src_ast_entry) {
		if (dst_ast_entry->peer_id == src_ast_entry->peer_id)
			return 1;
	}
...
static inline void dp_tx_update_mcast_param(uint16_t peer_id,
...
static void tx_sw_drop_stats_inc(struct dp_pdev *pdev, qdf_nbuf_t nbuf,
		       ...
		       struct cdp_tx_exception_metadata *tx_exc_metadata)
{
	...
	if (qdf_unlikely(peer_id == DP_INVALID_PEER)) {
		...
		DP_TX_TCL_METADATA_PEER_ID_SET(htt_tcl_metadata, peer_id);
	...
	dp_tx_desc_history_add(soc, tx_desc->dma_addr, nbuf, tx_desc->id,
			       DP_TX_DESC_MAP);
	...
	status = soc->arch_ops.tx_hw_enqueue(soc, vdev, tx_desc,
					     htt_tcl_metadata, tx_exc_metadata,
					     msdu_info);

	if (status != QDF_STATUS_SUCCESS) {
		dp_tx_err_rl("Tx_hw_enqueue Fail tx_desc %pK queue %d", tx_desc,
			     tx_q->ring_id);
		...
		if (QDF_NBUF_CB_GET_PACKET_TYPE(nbuf) ==
			...
			dma_sync_single_range_for_cpu(soc->osdev->dev,
						      tx_desc->dma_addr, 0,
						      nbuf->len, DMA_TO_DEVICE);
		else
#endif
...
fail_return:
	dp_tx_get_tid(vdev, nbuf, msdu_info);
	tx_sw_drop_stats_inc(pdev, nbuf, drop_code);
	tid_stats = &pdev->stats.tid_stats.tid_tx_stats[tx_q->ring_id][tid];
 ...
 */
#ifdef FEATURE_WLAN_TDLS
static inline int dp_tdls_tx_comp_free_buff(struct dp_soc *soc,
					    struct dp_tx_desc_s *desc)
{
			       ...
			       bool delayed_free)
{
	...
	if (desc->msdu_ext_desc) {
		...
		if (hal_tx_ext_desc_get_tso_enable(
			...
			dp_tx_tso_unmap_segment(
				soc, desc->msdu_ext_desc->tso_desc,
				desc->msdu_ext_desc->tso_num_desc);
			goto nbuf_free;
		}
	...
	if (qdf_nbuf_is_osrtp_pkt(nbuf)) {
		dma_sync_single_range_for_cpu(soc->osdev->dev, desc->dma_addr,
					      0, desc->nbuf->len,
					      DMA_TO_DEVICE);
	} else
#endif
 ...
 * Return: None
 */
static inline void dp_tx_sg_unmap_buf(struct dp_soc *soc, qdf_nbuf_t nbuf,
				      struct dp_tx_msdu_info_s *msdu_info)
{
	uint32_t cur_idx;
	...
	for (cur_idx = 1; cur_idx < seg->frag_cnt; cur_idx++)
		qdf_mem_unmap_page(
			soc->osdev,
			(qdf_dma_addr_t)(seg->frags[cur_idx].paddr_lo |
					 ((uint64_t)seg->frags[cur_idx].paddr_hi)
						 << 32),
			seg->frags[cur_idx].len, QDF_DMA_TO_DEVICE);
}
				 ...
				 struct dp_tx_msdu_info_s *msdu_info)
{
   ...
   */
     ...
     */
		...
		if (!tx_desc) {
			if (msdu_info->frm_type == dp_tx_frm_me) {
				...
				dp_tx_me_free_buf(
					pdev,
					(void *)(msdu_info->u.sg_info.curr_seg
							 ->frags[0]
							 .vaddr));
         ...
         */
				qdf_nbuf_free(
					msdu_info->u.sg_info.curr_seg->nbuf);
				if (msdu_info->u.sg_info.curr_seg->next) {
					msdu_info->u.sg_info.curr_seg =
						msdu_info->u.sg_info.curr_seg
							->next;
					nbuf = msdu_info->u.sg_info.curr_seg
						       ->nbuf;
				}
				i++;
				continue;
			...
			if (msdu_info->frm_type == dp_tx_frm_tso) {
				dp_tx_tso_seg_history_add(
					soc, msdu_info->u.tso_info.curr_seg,
					nbuf, 0, DP_TX_DESC_UNMAP);
				dp_tx_tso_unmap_segment(
					soc, msdu_info->u.tso_info.curr_seg,
					msdu_info->u.tso_info.tso_num_seg_list);
		...
		if (msdu_info->frm_type == dp_tx_frm_me) {
			tx_desc->msdu_ext_desc->me_buffer =
				(struct dp_tx_me_buf_t *)
					msdu_info->u.sg_info.curr_seg->frags[0]
						.vaddr;
     ...
     */
		...
		dp_tx_update_mcast_param(DP_INVALID_PEER, &htt_tcl_metadata,
					 vdev, msdu_info);
		/*
     * Enqueue the Tx MSDU descriptor to HW for transmit
     */
		status = soc->arch_ops.tx_hw_enqueue(
			soc, vdev, tx_desc, htt_tcl_metadata, NULL, msdu_info);
				   ...
				   struct dp_tx_msdu_info_s *msdu_info)
{
	...
	if (QDF_STATUS_SUCCESS !=
	    qdf_nbuf_map_nbytes_single(vdev->osdev, nbuf, QDF_DMA_TO_DEVICE,
				       qdf_nbuf_headlen(nbuf))) {
	...
	for (cur_frag = 0; cur_frag < nr_frags; cur_frag++) {
		if (QDF_STATUS_SUCCESS !=
		    qdf_nbuf_frag_map(vdev->osdev, nbuf, 0, QDF_DMA_TO_DEVICE,
				      cur_frag)) {
		...
		seg_info->frags[cur_frag + 1].paddr_hi = ((uint64_t)paddr) >>
							 32;
...
map_err:
	...
	qdf_nbuf_mapped_paddr_set(
		nbuf, (qdf_dma_addr_t)(seg_info->frags[0].paddr_lo |
				       ((uint64_t)seg_info->frags[0].paddr_hi)
					       << 32));
	...
	for (i = 1; i <= cur_frag; i++) {
		qdf_mem_unmap_page(
			vdev->osdev,
			(qdf_dma_addr_t)(seg_info->frags[i].paddr_lo |
					 ((uint64_t)seg_info->frags[i].paddr_hi)
						 << 32),
			seg_info->frags[i].len, QDF_DMA_TO_DEVICE);
	}
	qdf_nbuf_free(nbuf);
	return NULL;
}
 ...
 */
static void dp_tx_add_tx_sniffer_meta_data(struct dp_vdev *vdev,
					   struct dp_tx_msdu_info_s *msdu_info,
					   uint16_t ppdu_cookie)
{
	...
	HTT_TX_MSDU_EXT2_DESC_FLAG_HOST_OPAQUE_VALID_SET(
		msdu_info->meta_data[5], 1);
	HTT_TX_MSDU_EXT2_DESC_HOST_OPAQUE_COOKIE_SET(msdu_info->meta_data[6],
						     ppdu_cookie);
 ...
 */
static qdf_nbuf_t
dp_tx_extract_mesh_meta_data(struct dp_vdev *vdev, qdf_nbuf_t nbuf,
			     struct dp_tx_msdu_info_s *msdu_info)
{
...
remove_meta_hdr:
	...
	dp_tx_info("Meta hdr %0x %0x %0x %0x %0x %0x"
		   " tid %d to_fw %d",
		   msdu_info->meta_data[0], msdu_info->meta_data[1],
		   msdu_info->meta_data[2], msdu_info->meta_data[3],
		   msdu_info->meta_data[4], msdu_info->meta_data[5],
...
{
	bool invalid_tid =
		(tx_exc->tid >= DP_MAX_TIDS && tx_exc->tid != HTT_INVALID_TID);
	...
	bool invalid_cookie =
		(tx_exc->is_tx_sniffer == 1 && tx_exc->ppdu_cookie == 0);
...
bool dp_tx_mcast_enhance(struct dp_vdev *vdev, qdf_nbuf_t nbuf)
{
	...
	if (DP_FRAME_IS_MULTICAST((eh)->ether_dhost) &&
		...
		DP_STATS_INC_PKT(vdev, tx_i.mcast_en.mcast_pkt, 1,
				 qdf_nbuf_len(nbuf));
		if (dp_tx_prepare_send_me(vdev, nbuf) == QDF_STATUS_SUCCESS) {
			return false;
		}
 ...
 * Return: QDF_STATUS
 */
static inline QDF_STATUS dp_tx_per_pkt_vdev_id_check(qdf_nbuf_t nbuf,
						     struct dp_vdev *vdev)
{
	struct dp_ast_entry *dst_ast_entry = NULL;
	...
	dst_ast_entry = dp_peer_ast_hash_find_by_vdevid(
		vdev->pdev->soc, eh->ether_dhost, vdev->vdev_id);
	...
	TAILQ_FOREACH(peer, &vdev->peer_list, peer_list_elem)
	{
       ...
       */
			...
			nbuf_clone = dp_tx_send_msdu_single(
				vdev, nbuf_clone, msdu_info, peer_id, NULL);
		     ...
		     struct cdp_tx_exception_metadata *tx_exc_metadata)
{
	...
	struct dp_tx_msdu_info_s msdu_info;
	struct dp_vdev *vdev =
		dp_vdev_get_ref_by_id(soc, vdev_id, DP_MOD_ID_TX_EXCEPTION);
	...
	dp_verbose_debug("skb " QDF_MAC_ADDR_FMT, QDF_MAC_ADDR_REF(nbuf->data));
   ...
   */
	...
	if (qdf_unlikely(qdf_nbuf_is_nonlinear(nbuf))) {
		...
		DP_STATS_INC_PKT(vdev, tx_i.sg.sg_pkt, 1, qdf_nbuf_len(nbuf));
   ...
   */
	...
	if (soc->arch_ops.dp_tx_mlo_mcast_send) {
		nbuf = soc->arch_ops.dp_tx_mlo_mcast_send(soc, vdev, nbuf,
							  tx_exc_metadata);
		if (!nbuf)
			goto fail;
	}
	...
	if (qdf_likely(tx_exc_metadata->is_intrabss_fwd)) {
       ...
       */
			dp_tx_nawds_handler(soc, vdev, &msdu_info, nbuf,
					    tx_exc_metadata->peer_id);
			DP_STATS_INC_PKT(vdev, tx_i.nawds_mcast, 1,
					 qdf_nbuf_len(nbuf));
		}
...
qdf_nbuf_t dp_tx_send_exception_vdev_id_check(
	struct cdp_soc_t *soc_hdl, uint8_t vdev_id, qdf_nbuf_t nbuf,
	struct cdp_tx_exception_metadata *tx_exc_metadata)
{
	struct dp_soc *soc = cdp_soc_t_to_dp_soc(soc_hdl);
	struct dp_vdev *vdev =
			   ...
			   qdf_nbuf_t nbuf)
{
	...
	if ((mhdr->flags & METAHDR_FLAG_INFO_UPDATED) && !no_enc_frame) {
		nbuf_clone = qdf_nbuf_clone(nbuf);
		if (!nbuf_clone) {
			QDF_TRACE(QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_ERROR,
...
static inline void dp_tx_prefetch_nbuf_data(qdf_nbuf_t nbuf)
{
	if (nbuf) {
		      ...
		      qdf_nbuf_t nbuf)
{
   ...
   */
	...
	if (qdf_unlikely(vdev->nawds_enabled)) {
		...
		DP_STATS_INC_PKT(vdev, tx_i.nawds_mcast, 1, qdf_nbuf_len(nbuf));
	}
...
send_single:
   ...
   */
	...
	nbuf = dp_tx_send_msdu_single_wrapper(vdev, nbuf, &msdu_info, peer_id,
					      end_nbuf);
	return nbuf;
...
qdf_nbuf_t dp_tx_send_vdev_id_check(struct cdp_soc_t *soc_hdl, uint8_t vdev_id,
				    qdf_nbuf_t nbuf)
{
	struct dp_soc *soc = cdp_soc_t_to_dp_soc(soc_hdl);
	struct dp_vdev *vdev = NULL;
...
#ifdef WLAN_MCAST_MLO
static bool dp_tx_reinject_mlo_hdl(struct dp_soc *soc, struct dp_vdev *vdev,
				   struct dp_tx_desc_s *tx_desc,
				   qdf_nbuf_t nbuf, uint8_t reinject_reason)
{
...
#else /* WLAN_MCAST_MLO */
static inline bool dp_tx_reinject_mlo_hdl(struct dp_soc *soc,
					  struct dp_vdev *vdev,
					  struct dp_tx_desc_s *tx_desc,
					  qdf_nbuf_t nbuf,
...
void dp_tx_reinject_handler(struct dp_soc *soc, struct dp_vdev *vdev,
			    ...
			    uint8_t reinject_reason)
{
	...
	struct ieee80211_frame_addr4 *wh =
		(struct ieee80211_frame_addr4 *)(qdf_nbuf_data(nbuf));
#endif
	struct dp_txrx_peer *txrx_peer;
	...
	if (qdf_unlikely(vdev->mesh_vdev)) {
		...
		TAILQ_FOREACH(peer, &vdev->peer_list, peer_list_elem)
		{
           ...
           */
			    ((txrx_peer->bss_peer && num_peers_3addr &&
			      is_mcast) ||
				...
				nbuf_copy = dp_tx_send_msdu_single(
					vdev, nbuf_copy, &msdu_info, peer_id,
					NULL);
...
void dp_tx_inspect_handler(struct dp_soc *soc, struct dp_vdev *vdev,
			   struct dp_tx_desc_s *tx_desc, uint8_t *status)
{
	QDF_TRACE(QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_INFO, "%s Tx inspect path",
		  __func__);
 ...
 */
static void
dp_tx_comp_fill_tx_completion_stats(struct dp_tx_desc_s *tx_desc,
...
static void dp_tx_update_peer_sawf_stats(struct dp_soc *soc,
					 ...
					 uint8_t tid)
{
	dp_sawf_tx_compl_update_peer_stats(soc, vdev, txrx_peer, tx_desc, ts,
					   tid);
}
...
static void dp_tx_compute_delay_avg(struct cdp_delay_tx_stats *tx_delay,
				    ...
				    uint32_t hw_delay)
{
	dp_peer_tid_delay_avg(tx_delay, nw_delay, sw_delay, hw_delay);
}
#else
static void dp_tx_update_peer_sawf_stats(struct dp_soc *soc,
					 struct dp_vdev *vdev,
...
static inline void dp_tx_compute_delay_avg(struct cdp_delay_tx_stats *tx_delay,
					   uint32_t nw_delay, uint32_t sw_delay,
					   uint32_t hw_delay)
{
}
				    ...
				    struct dp_vdev *vdev)
{
	...
	sw_enqueue_delay = (uint32_t)(timestamp_hw_enqueue - timestamp_ingress);
	fwhw_transmit_delay =
		(uint32_t)(current_timestamp - timestamp_hw_enqueue);
...
#else
static inline void dp_tx_update_peer_delay_stats(
	struct dp_txrx_peer *txrx_peer, struct dp_tx_desc_s *tx_desc,
	struct hal_tx_completion_status *ts, uint8_t ring_id)
{
}
#endif
					    ...
					    uint32_t avg_jitter)
{
	...
	if (jitter_diff < 0)
		avg_jitter = avg_jitter - (qdf_abs(jitter_diff) >>
					   DP_AVG_JITTER_WEIGHT_DENOM);
	else
		avg_jitter = avg_jitter + (qdf_abs(jitter_diff) >>
					   DP_AVG_JITTER_WEIGHT_DENOM);
					   ...
					   uint32_t avg_delay)
{
	...
	if (delay_diff < 0)
		avg_delay = avg_delay -
			    (qdf_abs(delay_diff) >> DP_AVG_DELAY_WEIGHT_DENOM);
	else
		avg_delay = avg_delay +
			    (qdf_abs(delay_diff) >> DP_AVG_DELAY_WEIGHT_DENOM);
 ...
 * Return: void
 */
static QDF_STATUS dp_tx_compute_cur_delay(struct dp_soc *soc,
					  struct dp_vdev *vdev,
					  struct hal_tx_completion_status *ts,
					  uint32_t *curr_delay,
					  struct dp_tx_desc_s *tx_desc)
				     ...
				     struct dp_tx_desc_s *tx_desc)
{
	...
	status = dp_tx_compute_cur_delay(soc, vdev, ts, &curr_delay, tx_desc);

	if (QDF_IS_STATUS_SUCCESS(status)) {
		avg_delay = jitter->tx_avg_delay;
		avg_jitter = jitter->tx_avg_jitter;
		prev_delay = jitter->tx_prev_delay;
		avg_jitter = dp_tx_jitter_get_avg_jitter(curr_delay, prev_delay,
							 avg_jitter);
					   ...
					   uint8_t ring_id)
{
   ...
   */
	...
	dp_tx_compute_tid_jitter(rx_tid, ts, txrx_peer->vdev, tx_desc);
}
#else
static void dp_tx_update_peer_jitter_stats(struct dp_txrx_peer *txrx_peer,
					   struct dp_tx_desc_s *tx_desc,
 ...
 * Return: none
 */
static inline void dp_update_tx_delay_stats(struct dp_vdev *vdev,
					    uint32_t delay, uint8_t tid,
			 ...
			 uint8_t tid, uint8_t ring_id)
{
	...
	if (dp_is_vdev_tx_delay_stats_enabled(vdev)) {
		fwhw_transmit_delay_us = qdf_ktime_to_us(qdf_ktime_real_get()) -
   ...
   */
	...
	sw_enqueue_delay = (uint32_t)(timestamp_hw_enqueue - timestamp_ingress);
	interframe_delay =
		(uint32_t)(timestamp_ingress - vdev->prev_tx_enq_tstamp);

	/*
   * Delay in software enqueue
   */
	dp_update_tx_delay_stats(vdev, sw_enqueue_delay, tid,
				 CDP_DELAY_STATS_SW_ENQ, ring_id, false);
   ...
   */
	dp_update_tx_delay_stats(vdev, interframe_delay, tid,
				 CDP_DELAY_STATS_TX_INTERFRAME, ring_id, false);
	vdev->prev_tx_enq_tstamp = timestamp_ingress;
}

#ifdef DISABLE_DP_STATS
static inline void dp_update_no_ack_stats(qdf_nbuf_t nbuf,
					  struct dp_txrx_peer *txrx_peer,
					  uint8_t link_id)
{
}
			     ...
			     struct dp_txrx_peer *txrx_peer, uint8_t link_id)
{
	...
	pkt_type = (pkt_type >= HAL_DOT11_MAX ?
			    DOT11_MAX :
	...
	if (MCS_INVALID_ARRAY_INDEX != dst_mcs_idx)
		DP_PEER_EXTD_STATS_INC(
			txrx_peer, tx.pkt_type[pkt_type].mcs_count[dst_mcs_idx],
			1, link_id);
	...
	DP_PEER_EXTD_STATS_UPD(txrx_peer, tx.last_ack_rssi, ts->ack_frame_rssi,
			       link_id);
	DP_PEER_EXTD_STATS_INC(
		txrx_peer, tx.wme_ac_type[TID_TO_WME_AC(ts->tid)], 1, link_id);
	...
	if (ts->first_msdu) {
		...
		if (!retry_threshold)
			return;
		DP_PEER_EXTD_STATS_INCC(
			txrx_peer, tx.mpdu_success_with_retries,
			qdf_do_div(ts->transmit_cnt, retry_threshold),
			ts->transmit_cnt > retry_threshold, link_id);
	}
}
#else
static inline void
...
static inline uint8_t dp_tx_get_link_id_from_ppdu_id(
	struct dp_soc *soc, struct hal_tx_completion_status *ts,
	struct dp_txrx_peer *txrx_peer, struct dp_vdev *vdev)
{
	...
	if (hw_link_id > DP_MAX_MLO_LINKS) {
		hw_link_id = 0;
		DP_PEER_PER_PKT_STATS_INC(txrx_peer, tx.inval_link_id_pkt_cnt,
					  1, hw_link_id);
	}
 ...
 * Return: None
 */
static inline void dp_tx_update_peer_stats(struct dp_tx_desc_s *tx_desc,
					   struct hal_tx_completion_status *ts,
					   struct dp_txrx_peer *txrx_peer,
					   uint8_t ring_id, uint8_t link_id)
{
	struct dp_pdev *pdev = txrx_peer->vdev->pdev;
	uint8_t tid = ts->tid;
	uint32_t length;
   ...
   */
	...
	DP_PEER_PER_PKT_STATS_INCC(txrx_peer, tx.failed_retry_count, 1,
				   ts->transmit_cnt > DP_RETRY_COUNT, link_id);
	...
	if (ts->status == HAL_TX_TQM_RR_REM_CMD_AGED) {
		...
		DP_PEER_PER_PKT_STATS_INC(
			txrx_peer, tx.dropped.fw_rem_queue_disable, 1, link_id);
	} else if (ts->status == HAL_TX_TQM_RR_REM_CMD_TILL_NONMATCHING) {
		DP_PEER_PER_PKT_STATS_INC(txrx_peer, tx.dropped.fw_rem_no_match,
					  1, link_id);
	} else if (ts->status == HAL_TX_TQM_RR_DROP_THRESHOLD) {
		DP_PEER_PER_PKT_STATS_INC(txrx_peer, tx.dropped.drop_threshold,
					  1, link_id);
	} else if (ts->status == HAL_TX_TQM_RR_LINK_DESC_UNAVAILABLE) {
		DP_PEER_PER_PKT_STATS_INC(
			txrx_peer, tx.dropped.drop_link_desc_na, 1, link_id);
	} else if (ts->status == HAL_TX_TQM_RR_DROP_OR_INVALID_MSDU) {
		DP_PEER_PER_PKT_STATS_INC(txrx_peer, tx.dropped.invalid_drop, 1,
					  link_id);
	} else if (ts->status == HAL_TX_TQM_RR_MULTICAST_DROP) {
		DP_PEER_PER_PKT_STATS_INC(txrx_peer, tx.dropped.mcast_vdev_drop,
					  1, link_id);
	} else {
 ...
 * Return: None
 */
static inline void dp_tx_flow_pool_lock(struct dp_soc *soc,
					struct dp_tx_desc_s *tx_desc)
{
 ...
 * Return: None
 */
static inline void dp_tx_flow_pool_unlock(struct dp_soc *soc,
					  struct dp_tx_desc_s *tx_desc)
{
	struct dp_tx_desc_pool_s *pool;
	uint8_t desc_pool_id;
...
static inline void dp_tx_sojourn_stats_process(struct dp_pdev *pdev,
					       struct dp_txrx_peer *txrx_peer,
					       uint8_t tid, uint64_t txdesc_ts,
					       uint32_t ppdu_id,
					       uint8_t link_id)
{
	uint64_t delta_ms;
	...
	if (qdf_unlikely(tid == HTT_INVALID_TID || tid >= CDP_DATA_TID_MAX))
		return;
	...
	primary_link_peer = dp_get_primary_link_peer_by_id(
		pdev->soc, txrx_peer->peer_id, DP_MOD_ID_TX_COMP);

	if (qdf_unlikely(!primary_link_peer))
		return;

	sojourn_stats =
		(struct cdp_tx_sojourn_stats *)qdf_nbuf_data(pdev->sojourn_buf);

	link_peer_soc = primary_link_peer->vdev->pdev->soc;
	sojourn_stats->cookie = (void *)dp_monitor_peer_get_peerstats_ctx(
		link_peer_soc, primary_link_peer);

	delta_ms = qdf_ktime_to_ms(qdf_ktime_real_get()) - txdesc_ts;
	qdf_ewma_tx_lag_add(&txrx_peer->stats[link_id]
				     .per_pkt_stats.tx.avg_sojourn_msdu[tid],
			    delta_ms);
	...
	dp_wdi_event_handler(WDI_EVENT_TX_SOJOURN_STAT, pdev->soc,
			     pdev->sojourn_buf, HTT_INVALID_PEER, WDI_NO_VAL,
			     pdev->pdev_id);
...
void dp_send_completion_to_pkt_capture(struct dp_soc *soc,
				       ...
				       struct hal_tx_completion_status *ts)
{
	dp_wdi_event_handler(WDI_EVENT_PKT_CAPTURE_TX_DATA, soc, desc,
			     ts->peer_id, WDI_NO_VAL, desc->pdev->pdev_id);
}
#endif

void dp_tx_comp_process_desc(struct dp_soc *soc, struct dp_tx_desc_s *desc,
			     struct hal_tx_completion_status *ts,
			     struct dp_txrx_peer *txrx_peer)
{
   ...
   */
	...
	if (!(desc->msdu_ext_desc)) {
		...
		if (QDF_STATUS_SUCCESS ==
		    dp_get_completion_indication_for_stack(
			    soc, desc->pdev, txrx_peer, ts, desc->nbuf,
			    time_latency)) {
			dp_send_completion_to_stack(soc, desc->pdev,
						    ts->peer_id, ts->ppdu_id,
						    desc->nbuf);
			return;
		}
	}
 ...
 * Return: none
 */
static inline void dp_tx_update_connectivity_stats(struct dp_soc *soc,
						   struct dp_vdev *vdev,
						   struct dp_tx_desc_s *tx_desc,
						   uint8_t status)
{
						   ...
						   uint8_t status)
{
	...
	if (!vdev || !vdev->osif_vdev || !vdev->stats_cb)
		return;
 ...
 */
static inline uint32_t dp_tx_adjust_enqueue_buffer_ts(uint32_t ack_ts,
						      uint32_t enqueue_ts,
...
dp_tx_compute_hw_delay_us(struct hal_tx_completion_status *ts,
			  uint32_t delta_tsf, uint32_t *delay_us)
{
	uint32_t buffer_ts;
	uint32_t delay;
   ...
   */
	...
	buffer_ts =
		dp_tx_adjust_enqueue_buffer_ts(ts->tsf, buffer_ts, delta_tsf);
	...
	if (qdf_unlikely(delay & 0x80000000)) {
		dp_err_rl("delay = 0x%x (-ve)\n"
			  ...
			  ts->buffer_timestamp, ts->buffer_timestamp, delta_tsf,
			  delta_tsf);
	...
	if (delay > 0x1000000) {
		dp_info_rl("----------------------\n"
			   ...
			   "buffer_timestamp = %u (0x%x)\n"
			   "delta_tsf = %u (0x%x)\n",
			   ts->release_src, ts->ppdu_id, ts->status, ts->tsf,
			   ts->tsf, ts->buffer_timestamp, ts->buffer_timestamp,
			   delta_tsf, delta_tsf);
		return QDF_STATUS_E_FAILURE;
	}
				      ...
				      struct hal_tx_completion_status *ts)
{
	...
	if (QDF_IS_STATUS_ERROR(
		    dp_tx_compute_hw_delay_us(ts, vdev->delta_tsf, &ul_delay)))
		return;
...
static inline void
dp_tx_update_uplink_delay(struct dp_soc *soc, struct dp_vdev *vdev,
			  struct hal_tx_completion_status *ts)
{
}
				  ...
				  uint8_t ring_id)
{
	...
	dp_tx_comp_debug("-------------------- \n"
			 ...
			 ts->pkt_type, ts->stbc, ts->ldpc, ts->sgi, ts->mcs,
			 ts->ofdma, ts->tones_in_ru, ts->tsf, ts->ppdu_id,
			 ts->transmit_cnt, ts->tid, ts->peer_id, ts->status,
			 ts->release_src);
	...
	if (qdf_nbuf_tx_notify_comp_get(nbuf))
		dp_tx_notify_completion(soc, vdev, tx_desc, nbuf, ts->status);
	...
	dp_tx_update_peer_sawf_stats(soc, vdev, txrx_peer, tx_desc, ts,
				     ts->tid);
...
out_log:
	DPTRACE(qdf_dp_trace_ptr(
		tx_desc->nbuf, QDF_DP_TRACE_LI_DP_FREE_PACKET_PTR_RECORD,
		QDF_TRACE_DEFAULT_PDEV_ID, qdf_nbuf_data_addr(nbuf),
		sizeof(qdf_nbuf_data(nbuf)), tx_desc->id, ts->status, dp_status,
		op_mode));
out:
	return;
}
...
static inline void dp_tx_prefetch_next_nbuf_data(struct dp_tx_desc_s *next)
{
...
static inline bool dp_tx_mcast_reinject_handler(struct dp_soc *soc,
						struct dp_tx_desc_s *desc)
{
	struct dp_vdev *vdev = NULL;
...
dp_tx_update_ppeds_tx_comp_stats(struct dp_soc *soc,
				 struct dp_txrx_peer *txrx_peer,
				 struct hal_tx_completion_status *ts,
				 struct dp_tx_desc_s *desc, uint8_t ring_id)
{
	uint8_t link_id = 0;
	struct dp_vdev *vdev = NULL;
	...
	if (qdf_likely(txrx_peer)) {
		if (!(desc->flags & DP_TX_DESC_FLAG_SIMPLE)) {
			...
			vdev = txrx_peer->vdev;
			link_id = dp_tx_get_link_id_from_ppdu_id(
				soc, ts, txrx_peer, vdev);
			if (link_id < 1 || link_id > DP_MAX_MLO_LINKS)
				link_id = 0;
			dp_tx_update_peer_stats(desc, ts, txrx_peer, ring_id,
						link_id);
		} else {
...
void dp_tx_comp_process_desc_list_fast(struct dp_soc *soc,
...
void dp_tx_comp_process_desc_list(struct dp_soc *soc,
...
static inline bool dp_tx_comp_loop_pkt_limit_hit(struct dp_soc *soc,
						 int num_reaped,
						 int max_reap_limit)
{
	bool limit_hit = false;

	limit_hit = (num_reaped >= max_reap_limit) ? true : false;
...
static inline int dp_srng_test_and_update_nf_params(struct dp_soc *soc,
...
more_data:
     ...
     * Tx completion indication, assert */
       ...
       */
			wbm_internal_error = hal_get_wbm_internal_error(
				hal_soc, tx_comp_hal_desc);

			if (wbm_internal_error) {
				dp_err_rl("Tx comp wbm_internal_error!!");
				DP_STATS_INC(
					soc,
					tx.wbm_internal_error[WBM_INT_ERROR_ALL],
					1);
		...
		soc->arch_ops.tx_comp_get_params_from_hal_desc(
     ...
     */
		if (qdf_unlikely(buffer_src == HAL_TX_COMP_RELEASE_SOURCE_FW)) {
			...
			hal_tx_comp_desc_sync(tx_comp_hal_desc, &tx_desc->comp,
					      1);
			soc->arch_ops.dp_tx_process_htt_completion(
				soc, tx_desc, htt_tx_status, ring_id);
		} else {
			tx_desc->tx_status =
...
next_desc:
     ...
     */
		...
		dp_tx_prefetch_hw_sw_nbuf_desc(soc, hal_soc, num_avail_for_reap,
					       hal_ring_hdl,
   ...
   */
	...
	if (dp_tx_comp_enable_eol_data_check(soc)) {
		...
		if (!force_break &&
		    hal_srng_dst_peek_sync_locked(soc->hal_soc, hal_ring_hdl)) {
			DP_STATS_INC(soc, tx.hp_oos2, 1);
			if (!hif_exec_should_yield(soc->hif_handle,
						   int_ctx->dp_intr_id))
				goto more_data;

			num_avail_for_reap = hal_srng_dst_num_valid_locked(
				soc->hal_soc, hal_ring_hdl, true);
			if (qdf_unlikely(num_entries && (num_avail_for_reap >=
							 num_entries >> 1))) {
			 ...
			 enum ol_tx_spec tx_spec, qdf_nbuf_t msdu_list)
{
	struct dp_soc *soc = cdp_soc_t_to_dp_soc(soc_hdl);
	struct dp_vdev *vdev =
		dp_vdev_get_ref_by_id(soc, vdev_id, DP_MOD_ID_TDLS);
...
QDF_STATUS dp_tx_vdev_attach(struct dp_vdev *vdev)
{
   ...
   */
	...
	DP_TX_TCL_METADATA_VDEV_ID_SET(vdev->htt_tcl_metadata, vdev->vdev_id);

	pdev_id = dp_get_target_pdev_id_for_host_pdev_id(vdev->pdev->soc,
							 vdev->pdev->pdev_id);
...
#ifdef WLAN_SUPPORT_PPEDS
static inline bool dp_is_tx_desc_flush_match(struct dp_pdev *pdev,
					     struct dp_vdev *vdev,
					     struct dp_tx_desc_s *tx_desc)
{
 ...
 */
static inline void dp_tx_desc_reset_vdev(struct dp_soc *soc,
					 struct dp_tx_desc_s *tx_desc,
					 uint8_t desc_pool_id)
{
		      ...
		      bool force_free)
{
	...
	for (i = 0; i < num_pool; i++) {
		...
		num_desc_t =
			dp_get_updated_tx_desc(soc->ctrl_psoc, i, num_desc);
		num_desc_per_page =
			tx_desc_pool->desc_pages.num_element_per_page;
		for (j = 0; j < num_desc_t; j++) {
			...
			if (dp_is_tx_desc_flush_match(pdev, vdev, tx_desc)) {
				if (force_free) {
					...
					dp_tx_desc_reset_vdev(soc, tx_desc, i);
				}
			}
		}
	}
					   ...
					   uint32_t num_desc)
{
	...
	for (i = 0; i < num_pool; i++) {
		if (dp_tx_desc_pool_alloc(soc, i, num_desc)) {
			QDF_TRACE(QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_ERROR,
				  FL("Tx Desc Pool alloc %d failed %pK"), i,
				  soc);
			goto fail;
		}
	}
					  ...
					  uint32_t num_desc)
{
	...
	for (i = 0; i < num_pool; i++) {
		if (dp_tx_desc_pool_init(soc, i, num_desc)) {
			QDF_TRACE(QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_ERROR,
				  FL("Tx Desc Pool init %d failed %pK"), i,
				  soc);
			return QDF_STATUS_E_NOMEM;
		}
	}
						...
						uint32_t num_desc)
{
	...
	if (dp_tx_tso_num_seg_pool_alloc(soc, num_pool, num_desc)) {
		dp_err("TSO Num of seg Pool alloc %d failed %pK", num_pool,
		       soc);
		return QDF_STATUS_E_FAILURE;
	}
	return QDF_STATUS_SUCCESS;
...
QDF_STATUS dp_soc_tx_desc_sw_pools_alloc(struct dp_soc *soc)
{
	...
	QDF_TRACE(QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_INFO,
		  "%s Tx Desc Alloc num_pool = %d, descs = %d", __func__,
		  num_pool, num_desc);
 ...
 */
static inline bool dp_tx_need_mcast_reinject(struct dp_vdev *vdev)
{
 ...
 */
static uint8_t dp_tx_need_multipass_process(struct dp_soc *soc,
					    struct dp_vdev *vdev,
			     ...
			     struct dp_tx_msdu_info_s *msdu_info)
{
	...
	if (HTT_TX_MSDU_EXT2_DESC_FLAG_VALID_KEY_FLAGS_GET(
		    msdu_info->meta_data[0]))
		return true;
