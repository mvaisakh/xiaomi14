...
static struct wlan_cfg_tcl_wbm_ring_num_map
	g_tcl_wbm_map_array[MAX_TCL_DATA_RINGS] = {
		{ .tcl_ring_num = 0,
		  .wbm_ring_num = 0,
		  .wbm_rbm_id = HAL_LI_WBM_SW0_BM_ID,
		  .for_ipa = 0 },
		/*
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS dp_rxdma_ring_sel_cfg_li(struct dp_soc *soc)
{
	int i;
	int mac_id;
...
static inline QDF_STATUS dp_srng_init_li(struct dp_soc *soc,
...
static inline void dp_deinit_txcomp_ring4(struct dp_soc *soc)
{
	if (soc) {
...
static inline QDF_STATUS dp_init_txcomp_ring4(struct dp_soc *soc)
{
	if (soc) {
		if (dp_srng_init(soc, &soc->tx_comp_ring[3], WBM2SW_RELEASE,
				 WBM2SW_TXCOMP_RING4_NUM, 0)) {
			dp_err("%pK: dp_srng_init failed for rx_rel_ring", soc);
...
static inline void dp_free_txcomp_ring4(struct dp_soc *soc)
{
	if (soc)
		dp_srng_free(soc, &soc->tx_comp_ring[3]);
}

static inline QDF_STATUS dp_alloc_txcomp_ring4(struct dp_soc *soc,
...
static void dp_tx_implicit_rbm_set_li(struct dp_soc *soc, uint8_t tx_ring_id,
				      uint8_t bm_id)
{
}
...
bool dp_rx_intrabss_handle_nawds_li(struct dp_soc *soc,
...
static void dp_rx_word_mask_subscribe_li(struct dp_soc *soc, uint32_t *msg_word,
					 void *rx_filter)
{
}
...
static bool dp_reo_remap_config_li(struct dp_soc *soc, uint32_t *remap0,
...
static void
dp_get_vdev_stats_for_unmap_peer_li(struct dp_vdev *vdev, struct dp_peer *peer,
				    struct cdp_vdev_stats **vdev_stats)
{
}

static struct dp_soc *dp_get_soc_by_chip_id_li(struct dp_soc *soc,
					       uint8_t chip_id)
{
	return soc;
}
...
void dp_initialize_arch_ops_li(struct dp_arch_ops *arch_ops)
{
	...
	arch_ops->dp_rx_desc_cookie_2_va = dp_rx_desc_cookie_2_va_li;
	arch_ops->dp_rx_intrabss_mcast_handler = dp_rx_intrabss_handle_nawds_li;
