 ...
 */
static QDF_STATUS
hif_ar6000_set_address_window_register(struct hif_sdio_dev *hif_device,
				       uint32_t register_addr, uint32_t addr)
{
	QDF_STATUS status;
	...
	status = hif_read_write(hif_device, register_addr, (char *)(&address),
				...
				uint32_t address, uint32_t *data)
{
	...
	status = hif_ar6000_set_address_window_register(
		hif_device, WINDOW_READ_ADDR_ADDRESS, address);

	if (status != QDF_STATUS_SUCCESS)
		return status;

	/* read the data */
	status = hif_read_write(hif_device, WINDOW_DATA_ADDRESS,
				(char *)&readvalue, sizeof(uint32_t),
				 ...
				 uint32_t address, uint32_t data)
{
	...
	/* set write data */
	status = hif_read_write(hif_device, WINDOW_DATA_ADDRESS,
				(char *)&write_value, sizeof(uint32_t),
	...
	return hif_ar6000_set_address_window_register(
		hif_device, WINDOW_WRITE_ADDR_ADDRESS, address);
}
			      ...
			      uint8_t *data, int nbytes)
{
	...
	for (i = 0; i < nbytes; i += 4) {
		tmp_data = data[i] | (data[i + 1] << 8) | (data[i + 2] << 16) |
			   (data[i + 3] << 24);
						...
						int regsel, uint32_t *regval)
{
	...
	status = hif_read_write(hif_device, CPU_DBG_SEL_ADDRESS,
				register_selection, 4, HIF_WR_SYNC_BYTE_FIX,
				NULL);
	...
	status = hif_read_write(hif_device, CPU_DBG_ADDRESS, (char *)vals,
