/*
 * Copyright (c) 2013-2014, 2016-2018, 2020 The Linux Foundation. All rights
 * reserved.
 *
...
static inline void set_target_reg_bits(struct hif_softc *scn, void __iomem *mem,
...
void priv_start_agc(struct hif_softc *scn)
{
	...
	set_target_reg_bits(scn, scn->mem, BB_gains_min_offsets,
			    AGC_HISTORY_DUMP_MASK, 0x0f);
}

static void priv_stop_agc(struct hif_softc *scn)
{
	set_target_reg_bits(scn, scn->mem, BB_gains_min_offsets,
			    AGC_HISTORY_DUMP_MASK, 0);
}
...
void priv_dump_chaninfo(struct hif_softc *scn)
{
	...
	chain_mask = get_target_reg_bits(scn, scn->mem, BB_multichain_enable,
	...
	if (chain0) {
		if (bw < 2) {
			...
			for (i = 0; i < len; i++) {
				tmp = hif_read32_mb(
					scn,
					scn->mem + BB_chn_tables_intf_data);
			...
			if (bw > 2) {
				/* bw == 3 for vht80 */
				hif_write32_mb(
					scn, scn->mem + BB_chn_tables_intf_addr,
					0x80003300);
				len = 61;
				for (i = 0; i < len; i++) {
					tmp = hif_read32_mb(
	...
	if (chain1) {
		if (bw < 2) {
			...
			for (i = 0; i < len; i++) {
				tmp = hif_read32_mb(
					scn,
					scn->mem + BB_chn1_tables_intf_data);
				qdf_debug("0x%x", (tmp >> 16) & 0x0000ffff);
				qdf_debug("0x%x", tmp & 0x0000ffff);
				if (i % 2 == 0)
					qdf_debug("\n");
...
void priv_dump_agc(struct hif_softc *scn)
{
	...
	if (chain0) {
		for (i = 0; i < len; i++) {
			hif_write32_mb(scn,
				       scn->mem + PHY_BB_CHN_TABLES_INTF_ADDR,
				       BB_chaninfo_tab_b0 + i * 4);
			val = hif_read32_mb(
				scn, scn->mem + PHY_BB_CHN_TABLES_INTF_DATA);
			qdf_debug("0x%x\t", val);
			if (i % 4 == 0)
				qdf_debug("\n");
		}
	...
	if (chain1) {
		for (i = 0; i < len; i++) {
			hif_write32_mb(scn,
				       scn->mem + PHY_BB_CHN1_TABLES_INTF_ADDR,
				       BB_chaninfo_tab_b0 + i * 4);
			val = hif_read32_mb(
				scn, scn->mem + PHY_BB_CHN1_TABLES_INTF_DATA);
			qdf_debug("0x%x\t", val);
			if (i % 4 == 0)
				qdf_debug("\n");
		}
