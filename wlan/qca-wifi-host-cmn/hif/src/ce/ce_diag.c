...
void hif_ce_dump_target_memory(struct hif_softc *scn, void *ramdump_base,
 ...
 */
#ifdef QCA_WIFI_3_0
#define TARG_CPU_SPACE_TO_CE_SPACE(sc, pci_addr, addr) (scn->mem_pa + addr)
#else
#define TARG_CPU_SPACE_TO_CE_SPACE(sc, pci_addr, addr)             \
	(((hif_read32_mb(sc, (pci_addr) + (SOC_CORE_BASE_ADDRESS | \
					   CORE_CTRL_ADDRESS)) &   \
	   0x7ff)                                                  \
	  << 21) |                                                 \
				      ...
				      unsigned int target_type)
{
	...
	if ((target_type == TARGET_TYPE_AR900B) ||
		...
		ce_phy_addr = TARG_CPU_SPACE_TO_CE_SPACE(sc, sc->mem, address);
	}
			     ...
			     uint8_t *data, int nbytes)
{
   ...
   */
	...
	if (address < boundary_addr) {
		...
		while ((nbytes >= 4) &&
		       (QDF_STATUS_SUCCESS ==
			(status = hif_diag_read_access(hif_ctx, address,
						       (uint32_t *)data)))) {
   ...
   */
     ...
     */
		...
		while (ce_completed_send_next(ce_diag, NULL, NULL, &buf,
					      &completed_nbytes, &id, NULL,
					      NULL, &toeplitz_hash_result) !=
		       QDF_STATUS_SUCCESS) {
			qdf_mdelay(1);
		...
		i = 0;
		while (ce_completed_recv_next(ce_diag, NULL, NULL, &buf,
					      &completed_nbytes, &id,
			      ...
			      uint32_t address, uint8_t *data, int nbytes)
{
   ...
   */
     ...
     */
		status = ce_send(ce_diag, NULL, (qdf_dma_addr_t)CE_data, nbytes,
				 transaction_id, 0, user_flags);
