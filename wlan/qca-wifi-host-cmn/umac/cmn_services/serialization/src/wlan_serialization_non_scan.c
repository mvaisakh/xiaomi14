...
bool wlan_serialization_is_non_scan_pending_queue_empty(
...
bool wlan_serialization_is_active_non_scan_cmd_allowed(
	struct wlan_serialization_command *cmd)
{
...
wlan_ser_add_non_scan_cmd(struct wlan_ser_pdev_obj *ser_pdev_obj,
   ...
   */
	if (pdev_status != vdev_status) {
		wlan_serialization_remove_cmd_from_vdev_queue(
			ser_pdev_obj, &pcmd_list, &cmd_list->cmd,
...
enum wlan_serialization_status
wlan_ser_move_non_scan_pending_to_active(struct wlan_ser_pdev_obj *ser_pdev_obj,
					 ...
					 bool blocking_cmd_removed)
{
	...
	qdf_status =
		wlan_serialization_peek_front(pending_queue, &pending_node);
	...
	while (qsize--) {
       ...
       */
			...
			if (qdf_atomic_test_bit(CMD_MARKED_FOR_MOVEMENT,
						&pending_cmd_list->cmd_in_use)) {
				break;
			}
		}
		...
		qdf_status = wlan_ser_remove_non_scan_cmd(
			ser_pdev_obj, &pending_cmd_list, &cmd_to_remove, false);

		wlan_ser_update_cmd_history(pdev_queue, &pending_cmd_list->cmd,
					    SER_PENDING_TO_ACTIVE, false,
					    false);
		...
		wlan_ser_update_cmd_history(pdev_queue, &active_cmd_list->cmd,
					    SER_PENDING_TO_ACTIVE, true, true);
			     ...
			     uint8_t is_active_cmd)
{
	...
	vdev_status = wlan_serialization_remove_cmd_from_vdev_queue(
   ...
   */
	...
	if (vdev_status != QDF_STATUS_SUCCESS) {
		...
		if (vdev_status != QDF_STATUS_E_PENDING)
			ser_debug(
				"Failed to remove type %d id %d from vdev queue",
				cmd->cmd_type, cmd->cmd_id);
		goto error;
	}

	pdev_status = wlan_serialization_remove_cmd_from_pdev_queue(
		ser_pdev_obj, pcmd_list, cmd, is_active_cmd);

	if (pdev_status != QDF_STATUS_SUCCESS) {
		ser_debug(
			"Failed to remove type %d id %d from pdev active/pending queue",
			cmd->cmd_type, cmd->cmd_id);
		goto error;
	}
...
enum wlan_serialization_cmd_status wlan_ser_cancel_non_scan_cmd(
	struct wlan_ser_pdev_obj *ser_pdev_obj, struct wlan_objmgr_pdev *pdev,
	struct wlan_objmgr_vdev *vdev, struct wlan_serialization_command *cmd,
	enum wlan_serialization_cmd_type cmd_type, uint8_t is_active_queue,
	enum wlan_ser_cmd_attr cmd_attr)
{
	...
	while (!wlan_serialization_list_empty(pdev_queue) && qsize--) {
		...
		if (cmd && !(wlan_serialization_match_cmd_id_type(
			     ...
			     wlan_serialization_match_cmd_vdev(
				     nnode, cmd->vdev, WLAN_SER_PDEV_NODE))) {
			pnode = nnode;
			continue;
		}

		if (vdev && !wlan_serialization_match_cmd_vdev(
				    nnode, vdev, WLAN_SER_PDEV_NODE)) {
			pnode = nnode;
			continue;
		}

		if (pdev && !wlan_serialization_match_cmd_pdev(
     ...
     */
		if (is_active_queue) {
			if (!psoc || !cmd_list) {
				ser_err("psoc:0x%pK, cmd_list:0x%pK", psoc,
					cmd_list);
				status = WLAN_SER_CMD_NOT_FOUND;
				break;
			}
       ...
       */
		...
		pdev_status = wlan_serialization_remove_node(
			pdev_queue, &cmd_list->pdev_node);

		ser_vdev_obj =
			wlan_serialization_get_vdev_obj(cmd_list->cmd.vdev);

		vdev_queue = wlan_serialization_get_list_from_vdev_queue(
			ser_vdev_obj, cmd_type, is_active_queue);

		vdev_status = wlan_serialization_remove_node(
			vdev_queue, &cmd_list->vdev_node);
		...
		qdf_status = wlan_serialization_insert_back(
			&pdev_q->cmd_pool_list, &cmd_list->pdev_node);
		...
		wlan_ser_update_cmd_history(pdev_q, &cmd_bkup, SER_CANCEL,
					    false, is_active_queue);
     ...
     */
		if (cmd_bkup.cmd_cb) {
			...
			ser_debug(
				"Cancel command: type %d id %d vdev %d and Release memory",
     ...
     */
		wlan_ser_move_non_scan_pending_to_active(ser_pdev_obj, NULL,
							 false);
	}
...
bool wlan_serialization_is_blocking_non_scan_cmd_waiting(
