...
static QDF_STATUS wlan_objmgr_vdev_object_status(struct wlan_objmgr_vdev *vdev)
{
	uint8_t id;
...
static QDF_STATUS wlan_objmgr_vdev_obj_free(struct wlan_objmgr_vdev *vdev)
{
	...
	if (wlan_objmgr_pdev_vdev_detach(pdev, vdev) == QDF_STATUS_E_FAILURE)
		return QDF_STATUS_E_FAILURE;

	/* Detach VDEV from PSOC VDEV's list */
	if (wlan_objmgr_psoc_vdev_detach(psoc, vdev) == QDF_STATUS_E_FAILURE)
		return QDF_STATUS_E_FAILURE;
...
wlan_objmgr_vdev_get_osif_priv(struct wlan_objmgr_vdev *vdev)
{
...
wlan_objmgr_vdev_obj_create(struct wlan_objmgr_pdev *pdev,
			    struct wlan_vdev_create_params *params)
{
	...
	if (wlan_objmgr_psoc_vdev_attach(psoc, vdev) != QDF_STATUS_SUCCESS) {
	...
	/* Attach vdev to PDEV */
	if (wlan_objmgr_pdev_vdev_attach(pdev, vdev) != QDF_STATUS_SUCCESS) {
	...
	if (obj_status == QDF_STATUS_SUCCESS) {
		...
		for (id = 0; id < WLAN_UMAC_MAX_COMPONENTS; id++) {
			...
			if (stat_handler) {
				stat_handler(vdev, arg, QDF_STATUS_SUCCESS);
			}
		}
		/*
	...
	wlan_minidump_log(vdev, sizeof(*vdev), psoc, WLAN_MD_OBJMGR_VDEV,
			  "wlan_objmgr_vdev");
...
static QDF_STATUS wlan_objmgr_vdev_obj_destroy(struct wlan_objmgr_vdev *vdev)
{
	...
	if (vdev->obj_state != WLAN_OBJ_STATE_LOGICALLY_DELETED) {
		obj_mgr_alert(
			"VDEV object delete is not invoked vdevid:%d objstate:%d",
	...
	if (!qdf_is_macaddr_zero(mld_addr)) {
		...
		if (cdp_mlo_dev_ctxt_detach(wlan_psoc_get_dp_handle(psoc),
		    ...
		    QDF_STATUS_SUCCESS) {
			obj_mgr_err(
				"Failed to detach DP vdev from DP MLO Dev ctxt");
			QDF_BUG(0);
			return QDF_STATUS_E_FAILURE;
		}
	}
	...
	for (id = WLAN_UMAC_COMP_ID_MAX - 1; id >= 0; id--) {
		...
		if (handler && (vdev->obj_status[id] == QDF_STATUS_SUCCESS ||
 ...
 */
QDF_STATUS wlan_objmgr_vdev_component_obj_attach(struct wlan_objmgr_vdev *vdev,
...
QDF_STATUS wlan_objmgr_vdev_component_obj_detach(struct wlan_objmgr_vdev *vdev,
						 enum wlan_umac_comp_id id,
						 void *comp_priv_obj)
{
	QDF_STATUS obj_status;
 ...
 */
QDF_STATUS wlan_objmgr_iterate_peerobj_list(struct wlan_objmgr_vdev *vdev,
				  ...
				  wlan_objmgr_ref_dbgid dbg_id)
{
	...
	peer = wlan_vdev_peer_list_peek_head(peer_list);
	while (peer) {
		peer_next = wlan_peer_get_next_peer_of_vdev(peer_list, peer);
		if (wlan_objmgr_peer_try_get_ref(peer, dbg_id) ==
		    QDF_STATUS_SUCCESS) {
...
static qdf_list_t *wlan_obj_vdev_populate_logically_del_peerlist(
	qdf_list_t *obj_list, struct wlan_objmgr_vdev_objmgr *vdev_obj,
	wlan_objmgr_ref_dbgid dbg_id)
{
...
qdf_list_t *
wlan_objmgr_vdev_get_log_del_peer_list(struct wlan_objmgr_vdev *vdev,
				       wlan_objmgr_ref_dbgid dbg_id)
{
	qdf_list_t *peer_list;
	...
	if (peer_list) {
		log_del_peer_list =
			wlan_obj_vdev_populate_logically_del_peerlist(
				peer_list, &vdev->vdev_objmgr, dbg_id);
	}
...
QDF_STATUS wlan_objmgr_trigger_vdev_comp_priv_object_creation(
	struct wlan_objmgr_vdev *vdev, enum wlan_umac_comp_id id)
{
	wlan_objmgr_vdev_create_handler handler;
	void *arg;
...
static QDF_STATUS
wlan_obj_vdev_peerlist_remove_peer(qdf_list_t *obj_list,
				   struct wlan_objmgr_peer *peer)
{
...
struct wlan_objmgr_peer *
wlan_objmgr_vdev_try_get_bsspeer(struct wlan_objmgr_vdev *vdev,
...
void *wlan_objmgr_vdev_get_comp_private_obj(struct wlan_objmgr_vdev *vdev,
					    enum wlan_umac_comp_id id)
{
	void *comp_priv_obj;
...
static inline void wlan_objmgr_vdev_ref_trace(struct wlan_objmgr_vdev *vdev,
					      wlan_objmgr_ref_dbgid id,
...
static inline void wlan_objmgr_vdev_deref_trace(struct wlan_objmgr_vdev *vdev,
						wlan_objmgr_ref_dbgid id,
						const char *func, int line)
{
	struct wlan_objmgr_trace *trace;
...
struct wlan_objmgr_vdev *wlan_vdev_get_next_active_vdev_of_pdev_debug(
	struct wlan_objmgr_pdev *pdev, qdf_list_t *vdev_list,
	struct wlan_objmgr_vdev *vdev, wlan_objmgr_ref_dbgid dbg_id,
	const char *func, int line)
{
	...
	while (qdf_list_peek_next(vdev_list, prev_node, &node) ==
		...
		vdev_next = qdf_container_of(node, struct wlan_objmgr_vdev,
					     vdev_node);
		if (wlan_objmgr_vdev_try_get_ref_debug(vdev_next, dbg_id, func,
						       line) ==
		    QDF_STATUS_SUCCESS) {
			wlan_pdev_obj_unlock(pdev);
			return vdev_next;
...
#else
struct wlan_objmgr_vdev *
wlan_pdev_vdev_list_peek_active_head(struct wlan_objmgr_pdev *pdev,
...
#ifdef WLAN_OBJMGR_REF_ID_TRACE
struct wlan_objmgr_vdev *
wlan_pdev_peek_active_first_vdev_debug(struct wlan_objmgr_pdev *pdev,
				       wlan_objmgr_ref_dbgid dbg_id,
				       const char *func, int line)
{
...
#else
struct wlan_objmgr_vdev *
wlan_pdev_peek_active_first_vdev(struct wlan_objmgr_pdev *pdev,
				 wlan_objmgr_ref_dbgid dbg_id)
{
	struct wlan_objmgr_pdev_objmgr *objmgr = &pdev->pdev_objmgr;
	qdf_list_t *vdev_list;
	...
	return wlan_pdev_vdev_list_peek_active_head(pdev, vdev_list, dbg_id);
}
#endif
...
bool wlan_vdev_mlme_is_tdls_vdev(struct wlan_objmgr_vdev *vdev)
{
	...
	is_tdls_vdev = wlan_vdev_mlme_feat_ext2_cap_get(
