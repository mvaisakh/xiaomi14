...
void wlan_cm_connect_resp_fill_mld_addr_from_cm_id(
...
#ifdef WLAN_FEATURE_11BE_MLO
void wlan_cm_connect_resp_fill_mld_addr_from_vdev_id(
	struct wlan_objmgr_psoc *psoc, uint8_t vdev_id,
	struct scan_cache_entry *entry, struct wlan_cm_connect_resp *rsp)
{
	struct wlan_objmgr_vdev *vdev;
...
QDF_STATUS wlan_cm_sta_set_chan_param(struct wlan_objmgr_vdev *vdev,
				      ...
				      uint16_t ori_punc, uint8_t ccfs0,
				      uint8_t ccfs1,
				      struct ch_params *chan_param)
{
	...
	if (ori_bw == CH_WIDTH_320MHZ) {
		...
		center_freq_320 =
			wlan_reg_chan_band_to_freq(pdev, ccfs1, band_mask);
	} else if (ori_bw == CH_WIDTH_40MHZ) {
		if (WLAN_REG_IS_24GHZ_CH_FREQ(ch_freq)) {
			band_mask = BIT(REG_BAND_2G);
			center_freq_40 = wlan_reg_chan_band_to_freq(pdev, ccfs0,
								    band_mask);
	...
	wlan_reg_extract_puncture_by_bw(ori_bw, ori_punc, ch_freq,
					center_freq_320, CH_WIDTH_20MHZ,
					&primary_puncture_bitmap);
	if (primary_puncture_bitmap) {
		mlme_err(
			"sta vdev %d freq %d RX bw %d puncture 0x%x primary chan is punctured",
			wlan_vdev_get_id(vdev), ch_freq, ori_bw, ori_punc);
		return QDF_STATUS_E_FAULT;
	}
	if (chan_param->ch_width != CH_WIDTH_320MHZ)
		center_freq_320 = 0;
	qdf_mem_zero(&chan_list, sizeof(chan_list));
	wlan_reg_fill_channel_list_for_pwrmode(pdev, ch_freq, sec_ch_2g_freq,
					       chan_param->ch_width,
...
QDF_STATUS wlan_cm_sta_update_bw_puncture(struct wlan_objmgr_vdev *vdev,
					  uint8_t *peer_mac, uint16_t ori_punc,
					  enum phy_ch_width ori_bw,
					  ...
					  enum phy_ch_width new_bw)
{
	...
	status = wlan_cm_sta_set_chan_param(vdev, des_chan->ch_freq, ori_bw,
					    ori_punc, ccfs0, ccfs1, &ch_param);
	...
	des_chan->ch_width = ch_param.ch_width;
	mlme_debug(
		"sta vdev %d freq %d bw %d puncture 0x%x ch_cfreq1 %d ch_cfreq2 %d",
		wlan_vdev_get_id(vdev), des_chan->ch_freq, des_chan->ch_width,
		des_chan->puncture_bitmap, des_chan->ch_cfreq1,
		des_chan->ch_cfreq2);
	QDF_SET_BITS(bw_puncture, 0, 8, des_chan->ch_width);
	QDF_SET_BITS(bw_puncture, 8, 16, des_chan->puncture_bitmap);
	return wlan_util_vdev_peer_set_param_send(
		vdev, peer_mac, WLAN_MLME_PEER_BW_PUNCTURE, bw_puncture);
}
#endif /* WLAN_FEATURE_11BE */

#ifdef WLAN_FEATURE_11BE_MLO
#ifdef WLAN_FEATURE_11BE_MLO_ADV_FEATURE
bool wlan_cm_check_mlo_roam_auth_status(struct wlan_objmgr_vdev *vdev)
{
