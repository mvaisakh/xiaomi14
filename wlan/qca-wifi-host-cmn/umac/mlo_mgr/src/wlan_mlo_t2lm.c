...
QDF_STATUS wlan_mlo_parse_t2lm_info(uint8_t *ie, struct wlan_t2lm_info *t2lm)
{
	...
	t2lm->direction = dir;
	t2lm->default_link_mapping = QDF_GET_BITS(
		t2lm_control, WLAN_T2LM_CONTROL_DEFAULT_LINK_MAPPING_IDX,
		WLAN_T2LM_CONTROL_DEFAULT_LINK_MAPPING_BITS);

	t2lm->mapping_switch_time_present = QDF_GET_BITS(
		t2lm_control, WLAN_T2LM_CONTROL_MAPPING_SWITCH_TIME_PRESENT_IDX,
		WLAN_T2LM_CONTROL_MAPPING_SWITCH_TIME_PRESENT_BITS);

	t2lm->expected_duration_present = QDF_GET_BITS(
		t2lm_control, WLAN_T2LM_CONTROL_EXPECTED_DURATION_PRESENT_IDX,
		WLAN_T2LM_CONTROL_EXPECTED_DURATION_PRESENT_BITS);

	t2lm->link_mapping_size = QDF_GET_BITS(
		t2lm_control, WLAN_T2LM_CONTROL_LINK_MAPPING_SIZE_IDX,
		WLAN_T2LM_CONTROL_LINK_MAPPING_SIZE_BITS);

	t2lm_debug(
		"direction:%d default_link_mapping:%d mapping_switch_time_present:%d "
		"expected_duration_present:%d link_mapping_size:%d",
		t2lm->direction, t2lm->default_link_mapping,
		t2lm->mapping_switch_time_present,
		t2lm->expected_duration_present, t2lm->link_mapping_size);
	...
	for (tid_num = 0; tid_num < T2LM_MAX_NUM_TIDS; tid_num++) {
		...
		if (!t2lm->link_mapping_size) {
			t2lm->ieee_link_map_tid[tid_num] = qdf_le16_to_cpu(
...
QDF_STATUS
wlan_mlo_parse_bcn_prbresp_t2lm_ie(struct wlan_t2lm_context *t2lm_ctx,
				   uint8_t *ie)
{
...
QDF_STATUS
wlan_mlo_parse_t2lm_ie(struct wlan_t2lm_onging_negotiation_info *t2lm,
		       uint8_t *ie, uint32_t frame_len)
{
	...
	for (dir = 0; dir < WLAN_T2LM_MAX_DIRECTION; dir++) {
		...
		if (frame_len < (ie_len_parsed + sizeof(struct ie_header))) {
			t2lm_err(
				"Frame length is lesser than parsed T2LM IE header length");
		...
		if (ext_ie_hdr->ie_id == WLAN_ELEMID_EXTN_ELEM &&
		    ext_ie_hdr->ie_extn_id == WLAN_EXTN_ELEMID_T2LM) {
			ie_len_parsed +=
				ext_ie_hdr->ie_len + sizeof(struct ie_header);
			if (frame_len < ie_len_parsed) {
				t2lm_err(
					"Frame length is lesser than parsed T2LM IE length");
				return QDF_STATUS_E_PROTO;
			}
			...
			if (!retval &&
				...
				qdf_mem_copy(
					&t2lm->t2lm_info[t2lm_info.direction],
					&t2lm_info,
					sizeof(struct wlan_t2lm_info));
			} else {
				t2lm_err("Failed to parse the T2LM IE");
	...
	if ((t2lm->t2lm_info[WLAN_T2LM_DL_DIRECTION].direction ==
		...
		t2lm_err(
			"Both DL/UL and BIDI T2LM IEs should not be present at the same time");
				   ...
				   struct wlan_objmgr_vdev *vdev)
{
	...
	QDF_SET_BITS(t2lm_control, WLAN_T2LM_CONTROL_LINK_MAPPING_SIZE_IDX,
		     WLAN_T2LM_CONTROL_LINK_MAPPING_SIZE_BITS,
     ...
     */
		*t2lm_control_field = (uint8_t)t2lm_control;
		...
		t2lm_rl_debug(
			"T2LM IE added, direction:%d link_mapping_presence_indicator:%x",
	...
	for (tid_num = 0; tid_num < T2LM_MAX_NUM_TIDS; tid_num++) {
		...
		if (!t2lm->link_mapping_size) {
			*(uint16_t *)link_mapping_of_tids =
			...
			t2lm_rl_debug(
				"link mapping of TID%d is %x", tid_num,
				htole16(t2lm->ieee_link_map_tid[tid_num]));
			link_mapping_of_tids += sizeof(uint16_t);
		} else {
			*(uint8_t *)link_mapping_of_tids =
   ...
   */
	...
	for (dir = 0; dir < WLAN_T2LM_MAX_DIRECTION; dir++) {
		if (t2lm->t2lm_info[dir].direction !=
		    WLAN_T2LM_INVALID_DIRECTION)
			frm = wlan_mlo_add_t2lm_info_ie(
				frm, &t2lm->t2lm_info[dir], vdev);
	}
...
static QDF_STATUS wlan_mlo_parse_t2lm_request_action_frame(
	struct wlan_t2lm_onging_negotiation_info *t2lm,
	struct wlan_action_frame *action_frm, uint32_t frame_len,
	enum wlan_t2lm_category category)
{
   ...
   */
	...
	return wlan_mlo_parse_t2lm_ie(t2lm, t2lm_action_frm + sizeof(uint8_t),
				      frame_len);
}
	...
	enum wlan_t2lm_category category)
{
   ...
   */
	...
	t2lm->dialog_token = *t2lm_action_frm;
	t2lm->t2lm_resp_type = qdf_le16_to_cpu(
		*(uint16_t *)(t2lm_action_frm + sizeof(uint8_t)));
	...
	enum wlan_t2lm_category category)
{
	...
	*(uint16_t *)frm = htole16(args->arg2);
	...
	t2lm_info("T2LM response frame: category:%d action:%d dialog_token:%d "
		  "status_code:%d",
...
uint8_t *wlan_mlo_add_t2lm_action_frame(uint8_t *frm,
					...
					enum wlan_t2lm_category category)
{
	switch (category) {
	case WLAN_T2LM_CATEGORY_REQUEST:
		return wlan_mlo_add_t2lm_request_action_frame(frm, args, buf,
							      category);
	case WLAN_T2LM_CATEGORY_RESPONSE:
		return wlan_mlo_add_t2lm_response_action_frame(frm, args, buf,
							       category);
	case WLAN_T2LM_CATEGORY_TEARDOWN:
		*frm++ = args->category;
		*frm++ = args->action;
...
static void wlan_mlo_t2lm_handle_mapping_switch_time_expiry(
	struct wlan_t2lm_context *t2lm_ctx, struct wlan_objmgr_vdev *vdev)
{
	...
	t2lm_debug(
		"Established mapping: disabled_link_bitmap:%x dir:%d default_map:%d "
		"MSTP:%d EDP:%d MST:%d ED:%d ieee_link_map:%x hw_link_map:%x",
		t2lm_ctx->established_t2lm.disabled_link_bitmap,
		t2lm->direction, t2lm->default_link_mapping,
		t2lm->mapping_switch_time_present,
		t2lm->expected_duration_present, t2lm->mapping_switch_time,
		t2lm->expected_duration, t2lm->ieee_link_map_tid[0],
		t2lm->hw_link_map_tid[0]);
	...
	struct mlo_vdev_host_tid_to_link_map_resp *event)
{
	...
	t2lm_debug("psoc_id:%d vdev_id:%d status:%d", wlan_psoc_get_id(psoc),
		...
		if (t2lm_ctx->upcoming_t2lm.t2lm.mapping_switch_time_present)
			vdev_mlme->proto.ap.mapping_switch_time =
				(event->mapping_switch_tsf &
				 WLAN_T2LM_MAPPING_SWITCH_TSF_BITS) >>
				10;
...
static QDF_STATUS
wlan_send_t2lm_info(struct wlan_objmgr_vdev *vdev, struct wlan_t2lm_info *t2lm,
		    ...
		    struct wlan_lmac_if_mlo_tx_ops *mlo_tx_ops)
{
	...
	for (i = 0; i < vdev_count; i++) {
		...
		status =
			mlo_tx_ops->send_tid_to_link_mapping(co_mld_vdev, t2lm);
		if (QDF_IS_STATUS_ERROR(status))
			t2lm_err("Failed to send T2LM command to FW");
		mlo_release_vdev_ref(co_mld_vdev);
	}
 ...
 */
static QDF_STATUS
wlan_get_vdev_t2lm_mapping_status(struct wlan_objmgr_vdev *vdev,
				  struct wlan_t2lm_info *t2lm)
{
	struct wlan_t2lm_context *t2lm_ctx;
	int i = 0;
 ...
 */
static QDF_STATUS
wlan_get_peer_t2lm_mapping_status(struct wlan_objmgr_peer *peer,
				  struct wlan_t2lm_info *t2lm)
{
					 ...
					 struct wlan_objmgr_peer *peer)
{
	...
	for (dir = 0; dir < WLAN_T2LM_MAX_DIRECTION; dir++) {
		t2lm_info = &ml_peer->t2lm_policy.t2lm_negotiated_info
				     .t2lm_info[dir];
		if (t2lm_info &&
		    t2lm_info->direction != WLAN_T2LM_INVALID_DIRECTION) {
			t2lm_debug("send peer-level mapping to FW for dir: %d",
				   dir);
...
#if defined(WLAN_FEATURE_11BE_MLO) && defined(WLAN_FEATURE_11BE_MLO_ADV_FEATURE)
void wlan_clear_peer_level_tid_to_link_mapping(struct wlan_objmgr_vdev *vdev)
{
	struct wlan_objmgr_peer *peer;
...
void wlan_mlo_t2lm_timer_expiry_handler(void *vdev)
{
   ...
   */
	if (t2lm_ctx->established_t2lm.t2lm.expected_duration_present) {
		...
		/* Notify the registered caller about the link update*/
		wlan_mlo_dev_t2lm_notify_link_update(
			vdev_ctx, &t2lm_ctx->established_t2lm.t2lm);
...
wlan_mlo_t2lm_timer_init(struct wlan_objmgr_vdev *vdev)
{
	...
	qdf_timer_init(NULL, &t2lm_timer->t2lm_timer,
		       wlan_mlo_t2lm_timer_expiry_handler, vdev,
...
QDF_STATUS
wlan_mlo_t2lm_timer_start(struct wlan_objmgr_vdev *vdev, uint32_t interval)
{
	struct wlan_t2lm_timer *t2lm_timer;
	struct wlan_t2lm_context *t2lm_ctx;
...
QDF_STATUS wlan_handle_t2lm_timer(struct wlan_objmgr_vdev *vdev)
{
	...
	if (t2lm_ctx->established_t2lm.t2lm.expected_duration_present) {
		...
		status = wlan_mlo_t2lm_timer_start(
			vdev, t2lm_ctx->upcoming_t2lm.t2lm.mapping_switch_time);
	}
 ...
 */
static QDF_STATUS
wlan_update_mapping_switch_time_expected_dur(struct wlan_objmgr_vdev *vdev,
					     ...
					     uint64_t tsf)
{
     ...
     */
		if (!qdf_mem_cmp(
			    t2lm_ctx->established_t2lm.t2lm.ieee_link_map_tid,
	...
	if (rx_t2lm->upcoming_t2lm.t2lm.mapping_switch_time_present) {
		...
		if (ms_time > tsf_bit25_10) {
			t2lm_ctx->upcoming_t2lm.t2lm.mapping_switch_time =
				(ms_time - tsf_bit25_10 -
				 (3 * WLAN_T2LM_MAPPING_SWITCH_TIME_DELAY));
		} else {
			t2lm_ctx->upcoming_t2lm.t2lm.mapping_switch_time =
				0xFFFF - (tsf_bit25_10 - ms_time) -
				(3 * WLAN_T2LM_MAPPING_SWITCH_TIME_DELAY);
		}
	}
					   ...
					   uint64_t tsf)
{
	...
	status = wlan_update_mapping_switch_time_expected_dur(vdev, rx_t2lm_ie,
							      tsf);
	if (QDF_IS_STATUS_ERROR(status))
		return status;
...
QDF_STATUS wlan_mlo_dev_t2lm_notify_link_update(struct wlan_objmgr_vdev *vdev,
						struct wlan_t2lm_info *t2lm)
{
	struct wlan_t2lm_context *t2lm_ctx;
	wlan_mlo_t2lm_link_update_handler handler;
				      ...
				      void *evt_params)
{
	...
	is_connected = wlan_get_connected_vdev_by_mld_addr(
		psoc, params->mld_addr.bytes, &vdev_id);
	if (!is_connected) {
