...
QDF_STATUS ucfg_afc_init(void)
{
	...
	status = wlan_objmgr_register_psoc_create_handler(
		WLAN_UMAC_COMP_AFC, wlan_afc_psoc_created_notification, NULL);
	...
	status = wlan_objmgr_register_psoc_destroy_handler(
		WLAN_UMAC_COMP_AFC, wlan_afc_psoc_destroyed_notification, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = wlan_objmgr_register_pdev_create_handler(
		WLAN_UMAC_COMP_AFC, wlan_afc_pdev_obj_create_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	status = wlan_objmgr_register_pdev_destroy_handler(
		WLAN_UMAC_COMP_AFC, wlan_afc_pdev_obj_destroy_handler, NULL);
	if (QDF_IS_STATUS_ERROR(status)) {
