...
static QDF_STATUS
target_if_vdev_mgr_register_event_handler(struct wlan_objmgr_psoc *psoc)
{
	return target_if_vdev_mgr_wmi_event_register(psoc);
}

static QDF_STATUS
target_if_vdev_mgr_unregister_event_handler(struct wlan_objmgr_psoc *psoc)
{
				   ...
				   enum wlan_vdev_mgr_tgt_if_rsp_bit set_bit)
{
	...
	if (qdf_atomic_test_and_set_bit(set_bit, &vdev_rsp->rsp_status)) {
		mlme_err(
			"PSOC_%d VDEV_%d: %s requested, waiting for %s response",
			wlan_psoc_get_id(psoc), vdev_rsp->vdev_id,
			string_from_rsp_bit(set_bit),
			string_from_rsp_bit(set_bit));
...
target_if_vdev_mlme_build_txbf_caps(struct wlan_objmgr_vdev *vdev)
{
   ...
   */
	...
	wlan_util_vdev_get_param(vdev, WLAN_MLME_CFG_BFEE_STS_CAP,
				 &bfee_sts_cap);
	wlan_util_vdev_get_param(vdev, WLAN_MLME_CFG_IMLICIT_BF, &implicit_bf);
...
static inline uint32_t target_if_vdev_mlme_id_2_wmi(uint32_t cfg_id)
{
	int wmi_id;
	...
	case WLAN_MLME_CFG_MAX_UNRESPONSIVE_INACTIVE_TIME:
		wmi_id = wmi_vdev_param_ap_keepalive_max_unresponsive_time_secs;
		break;
...
static QDF_STATUS
target_if_vdev_set_tx_rx_decap_type(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_set_param_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_create_send(struct wlan_objmgr_vdev *vdev,
			       struct vdev_create_params *param)
{
	...
	status = wmi_unified_vdev_create_send(wmi_handle, vap_addr, param);
...
static QDF_STATUS target_if_vdev_mgr_start_send(struct wlan_objmgr_vdev *vdev,
						struct vdev_start_params *param)
{
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		...
		target_if_vdev_start_link_handler(vdev, param->is_restart);
	}
	return status;
}

static QDF_STATUS
target_if_vdev_mgr_delete_response_send(struct wlan_objmgr_psoc *psoc,
					uint8_t vdev_id,
					struct wlan_lmac_if_mlme_rx_ops *rx_ops)
{
...
static QDF_STATUS target_if_vdev_mgr_del_rsp_post_cb(struct scheduler_msg *msg)
{
	struct wlan_objmgr_psoc *psoc;
	struct wlan_lmac_if_mlme_rx_ops *rx_ops;
	uint8_t vdev_id;
...
static void
target_if_vdev_mgr_delete_rsp_post_ctx(struct wlan_objmgr_psoc *psoc,
				       uint8_t vdev_id)
{
	...
	msg.bodyptr = psoc;
	msg.bodyval = vdev_id;
	msg.flush_callback = target_if_vdev_mgr_del_rsp_post_flush_cb;

	if (scheduler_post_message(
		    QDF_MODULE_ID_TARGET_IF, QDF_MODULE_ID_TARGET_IF,
		    QDF_MODULE_ID_TARGET_IF, &msg) == QDF_STATUS_SUCCESS)
		return;
...
static void
target_if_vdev_mgr_delete_rsp_handler(struct wlan_objmgr_psoc *psoc,
				      uint8_t vdev_id,
				      struct wlan_lmac_if_mlme_rx_ops *rx_ops)
{
...
static QDF_STATUS
target_if_vdev_mgr_delete_send(struct wlan_objmgr_vdev *vdev,
			       struct vdev_delete_params *param)
{
	...
	target_if_vdev_mgr_rsp_timer_start(psoc, vdev_rsp, DELETE_RESPONSE_BIT);
...
static QDF_STATUS target_if_vdev_mgr_stop_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS target_if_vdev_mgr_down_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS target_if_vdev_mgr_up_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_beacon_tmpl_send(struct wlan_objmgr_vdev *vdev,
...
QDF_STATUS
target_if_vdev_mgr_send_fd_tmpl(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_set_nac_rssi_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS target_if_vdev_mgr_set_neighbour_rx_cmd_send(
	struct wlan_objmgr_vdev *vdev, struct set_neighbour_rx_params *param,
	uint8_t *mac)
{
	QDF_STATUS status;
	struct wmi_unified *wmi_handle;
	...
	status = wmi_unified_vdev_set_neighbour_rx_cmd_send(wmi_handle, mac,
							    param);
...
static QDF_STATUS
target_if_vdev_mgr_sifs_trigger_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS target_if_vdev_mgr_config_ratemask_cmd_send(
	struct wlan_objmgr_vdev *vdev, struct config_ratemask_params *param)
{
	QDF_STATUS status;
	struct wmi_unified *wmi_handle;
	...
	status = wmi_unified_vdev_config_ratemask_cmd_send(wmi_handle, param);
	return status;
}

static QDF_STATUS
target_if_vdev_mgr_peer_flush_tids_send(struct wlan_objmgr_vdev *vdev,
...
static int32_t target_if_vdev_mgr_multi_vdev_restart_get_ref(
	...
	struct wlan_objmgr_vdev **vdev_list, bool *vdev_timer_started)
{
	struct wlan_objmgr_psoc *psoc;
	...
	for (vdev_idx = 0; vdev_idx < param->num_vdevs; vdev_idx++) {
		...
		vdev_rsp = rx_ops->psoc_get_vdev_response_timer_info(
			psoc, wlan_vdev_get_id(tvdev));
		if (!vdev_rsp) {
			wlan_objmgr_vdev_release_ref(tvdev,
						     WLAN_VDEV_TARGET_IF_ID);
			vdev_list[vdev_idx] = NULL;
			mlme_err("VDEV_%d PSOC_%d No vdev rsp timer", vdev_idx,
				 wlan_psoc_get_id(psoc));
			return last_vdev_idx;
		}
...
static void target_if_vdev_mgr_multi_vdev_restart_rel_ref(
	struct wlan_objmgr_pdev *pdev, struct wlan_objmgr_vdev **vdev_list,
	bool *vdev_timer_started, int32_t last_vdev_idx, QDF_STATUS status)
{
	struct wlan_objmgr_psoc *psoc;
	struct wlan_objmgr_vdev *tvdev;
	struct wlan_lmac_if_mlme_rx_ops *rx_ops;
	...
	for (vdev_idx = 0; vdev_idx <= last_vdev_idx; vdev_idx++) {
		...
		wlan_objmgr_vdev_release_ref(tvdev, WLAN_VDEV_TARGET_IF_ID);
	}
}
	...
	struct multiple_vdev_restart_params *param)
{
	...
	target_if_vdev_mgr_multi_vdev_restart_rel_ref(
		pdev, vdev_list, vdev_timer_started, last_vdev_idx, status);

	return status;
}

static QDF_STATUS target_if_vdev_mgr_multiple_vdev_set_param_cmd(
	struct wlan_objmgr_pdev *pdev, struct multiple_vdev_set_param *param)
{
	QDF_STATUS status = QDF_STATUS_E_FAILURE;
	struct wmi_unified *wmi_handle;
	struct wlan_objmgr_psoc *psoc;
	...
	status =
		wmi_unified_send_multiple_vdev_set_param_cmd(wmi_handle, param);

	return status;
}

static QDF_STATUS target_if_vdev_mgr_beacon_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_sta_ps_param_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_peer_delete_all_send(struct wlan_objmgr_vdev *vdev,
...
static QDF_STATUS
target_if_vdev_mgr_fils_enable_send(struct wlan_objmgr_vdev *vdev,
...
static void
target_if_vdev_register_tx_fils(struct wlan_lmac_if_mlme_tx_ops *mlme_tx_ops)
{
					...
					struct wlan_objmgr_vdev *vdev)
{
	...
	status = target_if_vdev_mgr_rsp_timer_start(
		psoc, vdev_rsp, UPDATE_MAC_ADDR_RESPONSE_BIT);
 ...
 */
static QDF_STATUS
target_if_vdev_peer_set_param_send(struct wlan_objmgr_vdev *vdev,
				   uint8_t *peer_mac_addr, uint32_t param_id,
				   uint32_t param_value)
{
	struct peer_set_params param;
...
target_if_vdev_mgr_register_tx_ops(struct wlan_lmac_if_tx_ops *tx_ops)
{
	...
	mlme_tx_ops->vdev_set_param_send = target_if_vdev_mgr_set_param_send;
	...
	mlme_tx_ops->psoc_vdev_rsp_timer_mod = target_if_vdev_mgr_rsp_timer_mod;
	...
	mlme_tx_ops->psoc_wake_lock_init = target_if_wake_lock_init;
	mlme_tx_ops->psoc_wake_lock_deinit = target_if_wake_lock_deinit;
