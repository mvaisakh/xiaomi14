				 ...
				 struct target_if_samp_msg_params *params)
{
	...
	ret = target_if_get_spectral_msg_type(spectral_mode, &msg_type);
	...
	if (!spectral->det_map[params->hw_detector_id]
		...
		spectral_info(
			"Detector Map not valid for det id = %d and spectral mode = %d",
	...
	spec_samp_msg = spectral->nl_cb.get_sbuff(spectral->pdev_obj, msg_type,
						  det_map->buf_type);
	if (!spec_samp_msg) {
		qdf_spin_unlock_bh(&spectral->session_det_map_lock);
	...
	for (dest_det_idx = 0; dest_det_idx < det_map->num_dest_det_info;
		...
		detector_info->timestamp_war_offset =
		...
		bin_pwr_data =
			&params->bin_pwr_data[map_det_info->src_start_bin_idx];
		pwr_count = detector_info->end_bin_idx -
			    detector_info->start_bin_idx + 1;
		num_edge_bins = lb_edge_bins->num_bins + rb_edge_bins->num_bins;
		...
		if (lb_edge_bins->num_bins > 0) {
			ret = target_if_spectral_copy_fft_bins(
				...
				&spec_samp_msg
					 ->bin_pwr[lb_edge_bins->start_bin_idx],
				lb_edge_bins->num_bins, &bytes_copied,
				pwr_format);
			...
			if (QDF_IS_STATUS_ERROR(ret)) {
				...
				spectral_err_rl(
					"Unable to copy left edge FFT bins");
				return QDF_STATUS_E_FAILURE;
			}
		...
		ret = target_if_spectral_copy_fft_bins(
			spectral, bin_pwr_data,
			&spec_samp_msg->bin_pwr[start_bin_index], pwr_count,
			&bytes_copied, pwr_format);

		if (QDF_IS_STATUS_ERROR(ret)) {
			qdf_spin_unlock_bh(&spectral->session_det_map_lock);
			spectral_err_rl(
				"Unable to copy in-band/out-band FFT bins");
			return QDF_STATUS_E_FAILURE;
		}
		/* Advance the fft bin pointer in the report */
		bin_pwr_data += bytes_copied;
		...
		if (rb_edge_bins->num_bins > 0) {
			ret = target_if_spectral_copy_fft_bins(
				...
				rb_edge_bins->num_bins, &bytes_copied,
				pwr_format);
			...
			if (QDF_IS_STATUS_ERROR(ret)) {
				qdf_spin_unlock_bh(
					&spectral->session_det_map_lock);
				spectral_err_rl(
					"Unable to copy right edge FFT bins");
				return QDF_STATUS_E_FAILURE;
			}
	...
	if (det_map->send_to_upper_layers) {
		...
		spec_samp_msg->operating_bw =
			spectral->nl_cb.convert_to_nl_ch_width(
		...
		if (spectral->send_phy_data(spectral->pdev_obj, msg_type) ==
		    0) {
...
void target_if_spectral_create_samp_msg(struct target_if_spectral *spectral,
					struct target_if_samp_msg_params *params)
{
	/*
   ...
   */
	...
	if (is_primaryseg_rx_inprog(spectral, params->smode)) {
		spec_samp_msg =
			(struct spectral_samp_msg *)spectral->nl_cb.get_sbuff(
				spectral->pdev_obj, msg_type,
		...
		samp_data->spectral_combined_rssi = (uint8_t)params->rssi;
		...
		samp_data->spectral_lower_max_index = params->max_lower_index;
		samp_data->spectral_upper_max_index = params->max_upper_index;
     ...
     */
		...
		samp_data->lb_edge_extrabins = spectral->lb_edge_extrabins;
		samp_data->rb_edge_extrabins = spectral->rb_edge_extrabins;
     ...
     */
		if (swar->fftbin_size_war ==
			...
			SPECTRAL_MESSAGE_COPY_CHAR_ARRAY(&samp_data->bin_pwr[0],
							 bin_pwr_data,
							 pwr_count);
		}
	...
	if (is_secondaryseg_rx_inprog(spectral, params->smode)) {
		...
		samp_data = &spec_samp_msg->samp_data;
		samp_data->spectral_rssi_sec80 = params->rssi_sec80;
		samp_data->noise_floor_sec80 = params->noise_floor_sec80;
		...
		samp_data->spectral_data_len_sec80 = params->datalen_sec80;
		samp_data->spectral_max_index_sec80 = params->max_index_sec80;
		samp_data->spectral_max_mag_sec80 = params->max_mag_sec80;
	...
	if (!is_ch_width_160_or_80p80(spectral->ch_width[params->smode]) ||
		...
		if (spectral->send_phy_data(spectral->pdev_obj, msg_type) == 0)
			spectral->spectral_sent_msg++;
		samp_msg_index++;
	}
	...
	if (spectral->spectral_gen == SPECTRAL_GEN3 &&
		...
		target_if_160mhz_delivery_state_change(
			spectral, params->smode, SPECTRAL_DETECTOR_ID_INVALID);
}
#endif
