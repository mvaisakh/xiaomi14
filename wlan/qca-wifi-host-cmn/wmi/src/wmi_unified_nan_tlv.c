...
static QDF_STATUS extract_nan_event_rsp_tlv(wmi_unified_t wmi_handle,
					    void *evt_buf,
   ...
   */
	...
	if (nan_rsp_event_hdr->data_len > event->num_data) {
		wmi_err("Provided NAN event length(%d) exceeding actual length(%d)!",
			nan_rsp_event_hdr->data_len, event->num_data);
					       ...
					       struct nan_disable_req *nan_msg)
{
   ...
   */
	...
	WMITLV_SET_HDR(&cmd->tlv_header, WMITLV_TAG_STRUC_wmi_nan_cmd_param,
				       ...
				       struct nan_msg_params *nan_msg)
{
   ...
   */
	...
	nan_data_len_aligned =
		roundup(nan_msg->request_data_len, sizeof(uint32_t));
	...
	WMI_NAN_SET_RANGING_INITIATOR_ROLE(
		cfg->flags, !!(nan_msg->rtt_cap & WMI_FW_NAN_RTT_INITR));
	WMI_NAN_SET_RANGING_RESPONDER_ROLE(
		cfg->flags, !!(nan_msg->rtt_cap & WMI_FW_NAN_RTT_RESPR));
...
static QDF_STATUS
nan_ndp_initiator_req_tlv(wmi_unified_t wmi_handle,
			  struct nan_datapath_initiator_req *ndp_req)
{
   ...
   */
	...
	len = sizeof(*cmd) + sizeof(*ch_tlv) + (5 * WMI_TLV_HDR_SIZE) +
	      ndp_cfg_len + ndp_app_info_len + pmk_len + passphrase_len +
	      service_name_len;
	...
	qdf_mem_copy(&tlv_ptr[WMI_TLV_HDR_SIZE], ndp_req->ndp_config.ndp_cfg,
		     cmd->ndp_cfg_len);
	tlv_ptr = tlv_ptr + WMI_TLV_HDR_SIZE + ndp_cfg_len;

	WMITLV_SET_HDR(tlv_ptr, WMITLV_TAG_ARRAY_BYTE, ndp_app_info_len);
	qdf_mem_copy(&tlv_ptr[WMI_TLV_HDR_SIZE], ndp_req->ndp_info.ndp_app_info,
	...
	if (ndp_req->is_ipv6_addr_present) {
		...
		WMITLV_SET_HDR(
			...
			WMITLV_GET_STRUCT_TLVLEN(wmi_ndp_transport_ip_param));
		tcp_ip_param->ipv6_addr_present = true;
		qdf_mem_copy(tcp_ip_param->ipv6_intf_addr, ndp_req->ipv6_addr,
	...
	wmi_debug(
		"vdev_id = %d, transaction_id: %d, service_instance_id: %d, ch: "
		"%d, ch_cfg: %d, csid: %d peer mac addr: mac_addr31to0: 0x%x, "
		"mac_addr47to32: 0x%x",
	...
	wmi_debug(
		"ndp_config len: %d ndp_app_info len: %d pmk len: %d pass phrase "
		"len: %d service name len: %d",
...
static QDF_STATUS
nan_ndp_responder_req_tlv(wmi_unified_t wmi_handle,
			  struct nan_datapath_responder_req *req)
{
	...
	wmi_debug(
		"vdev_id: %d, transaction_id: %d, ndp_rsp %d, ndp_instance_id: %d, "
		"ndp_app_info_len: %d",
		vdev_id, req->transaction_id, req->ndp_rsp,
   ...
   */
	...
	service_name_len = qdf_roundup(req->service_name.service_name_len, 4);

	/* allocated memory for fixed params as well as variable size data */
	len = sizeof(*cmd) + 5 * WMI_TLV_HDR_SIZE + ndp_cfg_len +
	      ndp_app_info_len + pmk_len + passphrase_len + service_name_len;
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_ndp_responder_req_fixed_param));
	cmd->vdev_id = vdev_id;
	...
	WMITLV_SET_HDR(tlv_ptr, WMITLV_TAG_ARRAY_BYTE, ndp_cfg_len);
	qdf_mem_copy(&tlv_ptr[WMI_TLV_HDR_SIZE], req->ndp_config.ndp_cfg,
		     cmd->ndp_cfg_len);

	tlv_ptr = tlv_ptr + WMI_TLV_HDR_SIZE + ndp_cfg_len;
	WMITLV_SET_HDR(tlv_ptr, WMITLV_TAG_ARRAY_BYTE, ndp_app_info_len);
	qdf_mem_copy(&tlv_ptr[WMI_TLV_HDR_SIZE], req->ndp_info.ndp_app_info,
	...
	tlv_ptr = tlv_ptr + WMI_TLV_HDR_SIZE + pmk_len;
	WMITLV_SET_HDR(tlv_ptr, WMITLV_TAG_ARRAY_BYTE, passphrase_len);
	qdf_mem_copy(&tlv_ptr[WMI_TLV_HDR_SIZE], req->passphrase.passphrase,
		     cmd->nan_passphrase_len);
	tlv_ptr = tlv_ptr + WMI_TLV_HDR_SIZE + passphrase_len;

	WMITLV_SET_HDR(tlv_ptr, WMITLV_TAG_ARRAY_BYTE, service_name_len);
	qdf_mem_copy(&tlv_ptr[WMI_TLV_HDR_SIZE], req->service_name.service_name,
		     cmd->nan_servicename_len);
	...
	if (req->is_ipv6_addr_present || req->is_port_present ||
		...
		qdf_mem_copy(tcp_ip_param->ipv6_intf_addr, req->ipv6_addr,
			     WMI_NDP_IPV6_INTF_ADDR_LEN);
				      ...
				      struct nan_datapath_end_req *req)
{
	...
	ndp_end_req_lst =
		(wmi_ndp_end_req *)((uint8_t *)&cmd[1] + WMI_TLV_HDR_SIZE);
			   ...
			   struct nan_datapath_host_event *evt)
{
	...
	evt->vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		wmi_handle->soc->wmi_psoc, fixed_params->vdev_id, WLAN_NAN_ID);
	if (!evt->vdev) {
...
static QDF_STATUS
extract_ndp_initiator_rsp_tlv(wmi_unified_t wmi_handle, uint8_t *data,
			      struct nan_datapath_initiator_rsp *rsp)
{
	...
	rsp->vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		wmi_handle->soc->wmi_psoc, fixed_params->vdev_id, WLAN_NAN_ID);
	if (!rsp->vdev) {
		wmi_err("vdev is null");
		return QDF_STATUS_E_INVAL;
...
static QDF_STATUS extract_nan_msg_tlv(uint8_t *data, struct nan_dump_msg *msg)
{
	...
	if (!fixed_params->msg_len || fixed_params->msg_len > MAX_NAN_MSG_LEN ||
...
static QDF_STATUS extract_ndp_ind_tlv(wmi_unified_t wmi_handle, uint8_t *data,
				      struct nan_datapath_indication_event *rsp)
{
	...
	if (fixed_params->service_id_len > event->num_service_id) {
		wmi_err("FW msg service id len %d more than TLV hdr %d",
			fixed_params->service_id_len, event->num_service_id);
		return QDF_STATUS_E_INVAL;
	}
	...
	total_array_len = fixed_params->ndp_cfg_len + sizeof(*fixed_params);
	...
	wmi_debug(
		"WMI_NDP_INDICATION_EVENTID(0x%X) received. vdev %d "
		"service_instance %d, ndp_instance %d, role %d, policy %d csid: "
		"%d, scid_len: %d, peer_addr: " QDF_MAC_ADDR_FMT
		", peer_disc_addr: " QDF_MAC_ADDR_FMT
		" ndp_cfg - %d bytes ndp_app_info - %d bytes ndp_csid_caps %d, "
		"ndp_gtk_required %d",
		...
		rsp->ndp_add_params.csid_cap, rsp->ndp_add_params.gtk);
...
static QDF_STATUS
extract_ndp_confirm_tlv(wmi_unified_t wmi_handle, uint8_t *data,
			struct nan_datapath_confirm_event *rsp)
{
	...
	size_t total_array_len;
	bool ndi_dbs =
		wmi_service_enabled(wmi_handle, wmi_service_ndi_dbs_support);

	event = (WMI_NDP_CONFIRM_EVENTID_param_tlvs *)data;
	fixed_params = (wmi_ndp_confirm_event_fixed_param *)event->fixed_param;
	wmi_debug(
		"WMI_NDP_CONFIRM_EVENTID(0x%X) received. vdev %d, ndp_instance %d, "
		"rsp_code %d, reason_code: %d, num_active_ndps_on_peer: %d num_ch: %d",
	...
	if (fixed_params->num_ndp_channels > event->num_ndp_channel_list ||
		...
		wmi_err("NDP Ch count %d greater than NDP Ch TLV len(%d) or NSS TLV len(%d)",
			fixed_params->num_ndp_channels,
			event->num_ndp_channel_list, event->num_nss_list);
		return QDF_STATUS_E_INVAL;
	}
	...
	for (i = 0; i < rsp->num_channels; i++) {
		...
		rsp->ch[i].ch_width =
			wmi_get_ch_width_from_phy_mode(wmi_handle, ch_mode);
		if (ndi_dbs) {
	...
	if (event->ndp_transport_ip_param &&
		...
		if (event->ndp_transport_ip_param->trans_proto_present) {
			...
			rsp->protocol = event->ndp_transport_ip_param
						->transport_protocol;
		}
	}
	wmi_debug(
		"IPv6 addr present: %d, addr: %pI6 port: %d present: %d protocol: "
		"%d present: %d",
...
static QDF_STATUS
extract_ndp_responder_rsp_tlv(wmi_unified_t wmi_handle, uint8_t *data,
			      struct nan_datapath_responder_rsp *rsp)
{
	...
	WMI_MAC_ADDR_TO_CHAR_ARRAY(&fixed_params->peer_ndi_mac_addr,
				   rsp->peer_mac_addr.bytes);
	wmi_debug(
		"WMI_NDP_RESPONDER_RSP_EVENTID(0x%X) received. vdev_id: %d, "
		"peer_mac_addr: " QDF_MAC_ADDR_FMT
		",transaction_id: %d, status_code %d, reason_code: %d, create_peer: %d",
		WMI_NDP_RESPONDER_RSP_EVENTID, fixed_params->vdev_id,
		QDF_MAC_ADDR_REF(rsp->peer_mac_addr.bytes), rsp->transaction_id,
...
static QDF_STATUS
extract_ndp_end_rsp_tlv(wmi_unified_t wmi_handle, uint8_t *data,
			struct nan_datapath_end_rsp_event *rsp)
{
	...
	wmi_debug("WMI_NDP_END_RSP_EVENTID(0x%X) received. transaction_id: %d, "
		  "rsp_status: %d, reason_code: %d",
...
static QDF_STATUS
extract_ndp_end_ind_tlv(wmi_unified_t wmi_handle, uint8_t *data,
			struct nan_datapath_end_indication_event **rsp)
{
	...
	if (event->num_ndp_end_indication_list >
	    ((UINT_MAX - sizeof(**rsp)) / sizeof((*rsp)->ndp_map[0]))) {
...
static QDF_STATUS
extract_ndp_sch_update_tlv(wmi_unified_t wmi_handle, uint8_t *data,
			   struct nan_datapath_sch_update_event *ind)
{
	...
	if (fixed_params->num_channels > event->num_ndl_channel_list ||
		...
		wmi_err("Channel count %d greater than NDP Ch list TLV len(%d) or NSS list "
			"TLV len(%d)",
			fixed_params->num_channels, event->num_ndl_channel_list,
			event->num_nss_list);
		return QDF_STATUS_E_INVAL;
	}
	...
	ind->vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		wmi_handle->soc->wmi_psoc, fixed_params->vdev_id, WLAN_NAN_ID);
	if (!ind->vdev) {
		wmi_err("vdev is null");
		return QDF_STATUS_E_INVAL;
	...
	for (i = 0; i < fixed_params->num_ndp_instances; i++)
		wmi_debug("instance_id[%d]: %d", i,
			  event->ndp_instance_list[i]);
