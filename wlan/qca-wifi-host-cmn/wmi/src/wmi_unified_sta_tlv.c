				...
				struct sta_keep_alive_params *params)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(WMI_STA_KEEPALIVE_CMD_fixed_param));
 ...
 */
static QDF_STATUS send_vdev_set_gtx_cfg_cmd_tlv(wmi_unified_t wmi_handle,
						uint32_t if_id,
						struct wmi_gtx_config *gtx_info)
{
	...
	wmi_debug(
		"Setting vdev%d GTX values:htmcs 0x%x, vhtmcs 0x%x, usermask 0x%x, \
		 gtxPERThreshold %d, gtxPERMargin %d, gtxTPCstep %d, gtxTPCMin %d, \
		 gtxBWMask 0x%x.",
		if_id, cmd->gtxRTMask[0], cmd->gtxRTMask[1], cmd->userGtxMask,
		cmd->gtxPERThreshold, cmd->gtxPERMargin, cmd->gtxTPCstep,
		cmd->gtxTPCMin, cmd->gtxBWMask);
 ...
 */
static QDF_STATUS
send_process_dhcp_ind_cmd_tlv(wmi_unified_t wmi_handle,
					      ...
					      wmi_mac_addr peer_macaddr)
{
	...
	qdf_mem_copy(&cmd->peer_macaddr, &peer_macaddr, sizeof(peer_macaddr));
					    ...
					    uint32_t value2)
{
	...
	case WMI_WLAN_PROFILE_TRIGGER_CMDID:
		...
		prof_trig_cmd = (wmi_wlan_profile_trigger_cmd_fixed_param *)
			wmi_buf_data(buf);
		WMITLV_SET_HDR(
	...
	case WMI_WLAN_PROFILE_GET_PROFILE_DATA_CMDID:
		...
		WMITLV_SET_HDR(
			...
			WMITLV_GET_STRUCT_TLVLEN(
				wmi_wlan_profile_get_prof_data_cmd_fixed_param));
		wmi_mtrace(WMI_WLAN_PROFILE_GET_PROFILE_DATA_CMDID, NO_SESSION,
			   0);
	...
	case WMI_WLAN_PROFILE_SET_HIST_INTVL_CMDID:
		...
		wmi_mtrace(WMI_WLAN_PROFILE_SET_HIST_INTVL_CMDID, NO_SESSION,
			   0);
		ret = wmi_unified_cmd_send(
			wmi_handle, buf, len,
	...
	case WMI_WLAN_PROFILE_ENABLE_PROFILE_ID_CMDID:
		...
		wmi_mtrace(WMI_WLAN_PROFILE_ENABLE_PROFILE_ID_CMDID, NO_SESSION,
			   0);
		ret = wmi_unified_cmd_send(
			wmi_handle, buf, len,
			WMI_WLAN_PROFILE_ENABLE_PROFILE_ID_CMDID);
		if (ret) {
			wmi_err("enable cmd Failed for id %d value %d", value1,
				value2);
			wmi_buf_free(buf);
			return ret;
		}
 ...
 * Return: QDF_STATUS_SUCCESS for success or error code
 */
static QDF_STATUS send_nat_keepalive_en_cmd_tlv(wmi_unified_t wmi_handle,
						uint8_t vdev_id)
{
...
static QDF_STATUS
send_wlm_latency_level_cmd_tlv(wmi_unified_t wmi_handle,
			       struct wlm_latency_level_param *params)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_wlm_config_cmd_fixed_param));
	cmd->vdev_id = params->vdev_id;
	...
	if (wmi_unified_cmd_send(wmi_handle, buf, len, WMI_WLM_CONFIG_CMDID)) {
 ...
 */
static QDF_STATUS send_set_tdls_offchan_mode_cmd_tlv(
	...
	struct tdls_channel_switch_params *chan_switch_params)
{
	...
	buf_ptr = (uint8_t *)wmi_buf_data(wmi_buf);
	cmd = (wmi_tdls_set_offchan_mode_cmd_fixed_param *)wmi_buf_data(
		wmi_buf);
	WMITLV_SET_HDR(
		&cmd->tlv_header,
	...
	cmd->offchan_bw_bitmap = tdls_get_wmi_offchannel_bw(
	...
	wmi_debug("vdev_id: %d, off channel mode: %d, off channel Num: %d, "
		  ...
		  cmd->vdev_id, cmd->offchan_mode, cmd->offchan_num,
		  cmd->offchan_freq, cmd->offchan_bw_bitmap,
		  cmd->is_peer_responder, cmd->offchan_oper_class);
				  ...
				  enum wmi_tdls_state tdls_state)
{
	...
	cmd->teardown_notification_ms = tdls_param->teardown_notification_ms;
	...
	wmi_debug("vdev %d tdls_state: %d, state: %d, "
		  ...
		  cmd->notification_interval_ms, cmd->tx_discovery_threshold,
		  cmd->tx_teardown_threshold, cmd->rssi_teardown_threshold,
		  cmd->rssi_delta, cmd->tdls_options,
		  cmd->tdls_peer_traffic_ind_window,
		  cmd->tdls_peer_traffic_response_timeout_ms,
		  cmd->tdls_puapsd_mask, cmd->tdls_puapsd_inactivity_time_ms,
		  cmd->tdls_puapsd_rx_frame_threshold,
				    ...
				    uint32_t *ch_mhz)
{
	...
	wmi_debug("vdev_id: %d, peermac: " QDF_MAC_ADDR_FMT ", "
		  ...
		  cmd->vdev_id, QDF_MAC_ADDR_REF(peer_state->peer_macaddr),
		  cmd->peer_macaddr.mac_addr31to0,
   ...
   */
	...
	wmi_debug(
		"peer_qos: 0x%x, buff_sta_support: %d, off_chan_support: %d, "
	...
	for (i = 0; i < in_peer_cap->peer_chanlen; ++i) {
		...
		if (in_chan_info->dfs_set) {
			...
			wmi_debug("chan_freq[%d] DFS[%d]",
				  in_chan_info->ch_freq, in_chan_info->dfs_set);
		}
						 ...
						 struct tdls_event_info *param)
{
	...
	WMI_MAC_ADDR_TO_CHAR_ARRAY(&evt->peer_macaddr, param->peermac.bytes);
	...
	wmi_debug("tdls event, peer: " QDF_MAC_ADDR_FMT
		  ", type: 0x%x, reason: %d, vdev: %d",
		  QDF_MAC_ADDR_REF(param->peermac.bytes), param->message_type,
...
void wmi_tdls_attach_tlv(struct wmi_unified *wmi_handle)
{
	...
	ops->send_update_fw_tdls_state_cmd = send_update_fw_tdls_state_cmd_tlv;
 ...
 */
static QDF_STATUS
send_process_set_ie_info_cmd_tlv(wmi_unified_t wmi_handle,
				 struct vdev_ie_info_param *ie_info)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_vdev_set_ie_cmd_fixed_param));
	...
	ret = wmi_unified_cmd_send(wmi_handle, buf, len, WMI_VDEV_SET_IE_CMDID);
	if (QDF_IS_STATUS_ERROR(ret)) {
 ...
 */
static QDF_STATUS
send_set_base_macaddr_indicate_cmd_tlv(wmi_unified_t wmi_handle,
				       uint8_t *custom_addr)
{
			    ...
			    struct reject_ap_params *reject_params)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_pdev_dsm_filter_fixed_param));
	...
	for (i = 0; i < num_of_reject_bssid; i++) {
		...
		WMI_CHAR_ARRAY_TO_MAC_ADDR(reject_list[i].bssid.bytes,
					   &chan_list->bssid);
		chan_list->bssid_type = wmi_get_wmi_reject_ap_type(
 ...
 */
static QDF_STATUS
send_sar_limit_cmd_tlv(wmi_unified_t wmi_handle,
		       struct sar_limit_cmd_params *sar_limit_params)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_sar_limits_cmd_fixed_param));
	...
	wmi_sar_rows_list =
		(wmi_sar_limit_cmd_row *)(buf_ptr + WMI_TLV_HDR_SIZE);
	...
	for (i = 0; i < sar_limit_params->num_limit_rows; i++) {
		...
		wmi_debug(
			"row %d, band_id = %d, chain_id = %d, mod_id = %d, limit_value = "
			"%d, validity_bitmap = %d",
			i, wmi_sar_rows_list->band_id,
			wmi_sar_rows_list->chain_id, wmi_sar_rows_list->mod_id,
...
static QDF_STATUS get_sar_limit_cmd_tlv(wmi_unified_t wmi_handle)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_sar_get_limits_cmd_fixed_param));
 ...
 * Return: 0 for success or error code
 */
static QDF_STATUS extract_sar2_result_event_tlv(void *handle, uint8_t *event,
						uint32_t len)
{
					      ...
					      struct sar_limit_event *event)
{
	...
	if (event->num_limit_rows > MAX_SAR_LIMIT_ROWS_SUPPORTED) {
		QDF_ASSERT(0);
		wmi_err("Num rows %d exceeds max of %d", event->num_limit_rows,
 ...
 */
static QDF_STATUS
send_set_del_pmkid_cache_cmd_tlv(wmi_unified_t wmi_handle,
				 struct wmi_unified_pmk_cache *pmk_info)
{
	...
	WMITLV_SET_HDR(buf_ptr, WMITLV_TAG_ARRAY_STRUC, sizeof(*pmksa));
	buf_ptr += WMI_TLV_HDR_SIZE;

	pmksa = (wmi_pmk_cache *)buf_ptr;
	WMITLV_SET_HDR(buf_ptr, WMITLV_TAG_STRUC_wmi_pmk_cache,
		       WMITLV_GET_STRUCT_TLVLEN(wmi_pmk_cache));
 ...
 */
static QDF_STATUS
send_aggr_qos_cmd_tlv(wmi_unified_t wmi_handle,
		      struct aggr_add_ts_param *aggr_qos_rsp_msg)
{
	...
	for (i = 0; i < WMI_QOS_NUM_AC_MAX; i++) {
       ...
       */
			...
			cmd->ac =
				WMI_TID_TO_AC(aggr_qos_rsp_msg->tspec[i]
						      .tsinfo.traffic.userPrio);
			cmd->medium_time_us =
				aggr_qos_rsp_msg->tspec[i].mediumTime * 32;
			cmd->downgrade_type = WMM_AC_DOWNGRADE_DEPRIO;
			wmi_debug(
				"Addts vdev:%d, ac:%d, mediumTime:%d downgrade_type:%d",
 ...
 */
static QDF_STATUS
send_process_add_periodic_tx_ptrn_cmd_tlv(wmi_unified_t wmi_handle,
					  ...
					  uint8_t vdev_id)
{
	...
	wmi_debug("Add ptrn id: %d vdev_id: %d", cmd->pattern_id, cmd->vdev_id);
 ...
 */
static QDF_STATUS
send_process_del_periodic_tx_ptrn_cmd_tlv(wmi_unified_t wmi_handle,
					  uint8_t vdev_id, uint8_t pattern_id)
{
	...
	cmd = (WMI_DEL_PROACTIVE_ARP_RSP_PATTERN_CMD_fixed_param *)wmi_buf_data(
		wmi_buf);
	WMITLV_SET_HDR(
		&cmd->tlv_header,
	...
	wmi_debug("Del ptrn id: %d vdev_id: %d", cmd->pattern_id, cmd->vdev_id);
 ...
 */
static QDF_STATUS
send_set_led_flashing_cmd_tlv(wmi_unified_t wmi_handle,
			      struct flashing_req_params *flashing)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_set_led_flashing_cmd_fixed_param));
...
static QDF_STATUS send_process_ch_avoid_update_cmd_tlv(wmi_unified_t wmi_handle)
{
	...
	WMITLV_SET_HDR(
		&ch_avoid_update_fp->tlv_header,
		WMITLV_TAG_STRUC_wmi_chan_avoid_update_cmd_param,
		WMITLV_GET_STRUCT_TLVLEN(wmi_chan_avoid_update_cmd_param));
 ...
 */
static QDF_STATUS
send_pdev_set_dual_mac_config_cmd_tlv(wmi_unified_t wmi_handle,
				      struct policy_mgr_dual_mac_config *msg)
{
	...
	wmi_debug("scan_config:%x fw_mode_config:%x", msg->scan_config,
		  msg->fw_mode_config);
 ...
 */
static QDF_STATUS send_adapt_dwelltime_params_cmd_tlv(
	wmi_unified_t wmi_handle,
 ...
 * Return: QDF_STATUS_SUCCESS on success and QDF failure reason code for failure
 */
static QDF_STATUS send_dbs_scan_sel_params_cmd_tlv(
	wmi_unified_t wmi_handle,
	struct wmi_dbs_scan_sel_params *dbs_scan_params)
{
	...
	WMITLV_SET_HDR(
		&dbs_scan_param->tlv_header,
		WMITLV_TAG_STRUC_wmi_scan_dbs_duty_cycle_fixed_param,
		WMITLV_GET_STRUCT_TLVLEN(wmi_scan_dbs_duty_cycle_fixed_param));
						 ...
						 struct set_arp_stats *req_buf)
{
	...
	buf_ptr = (uint8_t *)wmi_buf_data(buf);
	wmi_set_arp = (wmi_vdev_set_arp_stats_cmd_fixed_param *)buf_ptr;
   ...
   */
	if (req_buf->pkt_type_bitmap) {
		...
		WMITLV_SET_HDR(
			&wmi_set_connect_stats->tlv_header,
		...
		wmi_debug(
			"Connectivity Stats: pkt_type_bitmap %u tcp_src_port:%u "
			"tcp_dst_port %u icmp_ipv4 %u",
			wmi_set_connect_stats->pkt_type_bitmap,
	...
	wmi_debug("set arp stats flag=%d, vdev=%d", req_buf->flag,
		  req_buf->vdev_id);
	return QDF_STATUS_SUCCESS;
error:
	wmi_buf_free(buf);
						 ...
						 struct get_arp_stats *req_buf)
{
	...
	buf_ptr = (uint8_t *)wmi_buf_data(buf);
	get_arp_stats = (wmi_vdev_get_arp_stats_cmd_fixed_param *)buf_ptr;
					       ...
					       uint16_t *peer_id_list)
{
	...
	WMITLV_SET_HDR(buf_ptr, WMITLV_TAG_ARRAY_UINT32, peer_id_list_len);
...
void wmi_sta_attach_tlv(wmi_unified_t wmi_handle)
{
	...
	ops->send_dbs_scan_sel_params_cmd = send_dbs_scan_sel_params_cmd_tlv;
