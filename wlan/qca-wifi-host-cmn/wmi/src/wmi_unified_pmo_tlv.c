						    ...
						    bool enable)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(WMI_WOW_ADD_DEL_EVT_CMD_fixed_param));
	cmd->vdev_id = vdev_id;
	cmd->is_add = enable;
	qdf_mem_copy(&(cmd->event_bitmaps[0]), bitmap,
		     sizeof(uint32_t) * WMI_WOW_MAX_EVENT_BM_LEN);
 ...
 */
static QDF_STATUS
send_wow_patterns_to_fw_cmd_tlv(wmi_unified_t wmi_handle, uint8_t vdev_id,
				uint8_t ptrn_id, const uint8_t *ptrn,
				uint8_t ptrn_len, uint8_t ptrn_offset,
				const uint8_t *mask, uint8_t mask_len,
				bool user, uint8_t default_patterns)
{
	...
	len = sizeof(WMI_WOW_ADD_PATTERN_CMD_fixed_param) + WMI_TLV_HDR_SIZE +
	      1 * sizeof(WOW_BITMAP_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_IPV4_SYNC_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_IPV6_SYNC_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_MAGIC_PATTERN_CMD) + WMI_TLV_HDR_SIZE +
	...
	wmi_debug(
		"vdev: %d, ptrn id: %d, ptrn len: %d, ptrn offset: %d user %d",
 ...
 */
static void
fill_arp_offload_params_tlv(wmi_unified_t wmi_handle,
			    struct pmo_arp_offload_params *offload_req,
			    uint8_t **buf_ptr)
{
	int i;
...
static void fill_ns_offload_params_tlv(wmi_unified_t wmi_handle,
				       struct pmo_ns_offload_params *ns_req,
				       uint8_t **buf_ptr)
{
	int i;
	...
	*buf_ptr += WMI_TLV_HDR_SIZE;
     ...
     */
       ...
       */
			WMI_CHAR_ARRAY_TO_MAC_ADDR(ns_req->self_macaddr.bytes,
						   &ns_tuple->target_mac);
				   ...
				   uint8_t **buf_ptr)
{
	...
	num_ns_ext_tuples = ns_req->num_ns_offload_count - WMI_MAX_NS_OFFLOADS;
 ...
 * Return: QDF Status
 */
static QDF_STATUS send_enable_arp_ns_offload_cmd_tlv(
	wmi_unified_t wmi_handle,
	struct pmo_arp_offload_params *arp_offload_req,
	struct pmo_ns_offload_params *ns_offload_req, uint8_t vdev_id)
{
	int32_t res;
 ...
 */
static QDF_STATUS
send_add_clear_mcbc_filter_cmd_tlv(wmi_unified_t wmi_handle, uint8_t vdev_id,
				   ...
				   bool clearList)
{
	...
	wmi_debug(
		"Action:%d; vdev_id:%d; clearList:%d; MCBC MAC Addr: " QDF_MAC_ADDR_FMT,
...

{
	...
	buf_ptr = (uint8_t *)wmi_buf_data(buf);
	cmd = (WMI_SET_MULTIPLE_MCAST_FILTER_CMD_fixed_param *)wmi_buf_data(
		buf);
	qdf_mem_zero(cmd, sizeof(*cmd));
	...
	cmd->operation = ((filter_param->action == 0) ?
	...
	mac_addr_dst_ptr = (wmi_mac_addr *)(buf_ptr + WMI_TLV_HDR_SIZE);
...
static void fill_fils_tlv_params(WMI_GTK_OFFLOAD_CMD_fixed_param *cmd,
				 uint8_t vdev_id, struct pmo_gtk_req *params)
{
	uint8_t *buf_ptr;
	...
	WMITLV_SET_HDR(
		&ext_param->tlv_header,
		WMITLV_TAG_STRUC_wmi_gtk_offload_extended_tlv_param,
		WMITLV_GET_STRUCT_TLVLEN(wmi_gtk_offload_fils_tlv_param));
 ...
 */
static QDF_STATUS
send_igmp_offload_cmd_tlv(wmi_unified_t wmi_handle,
			  struct pmo_igmp_offload_req *pmo_igmp_req)
{
	...
	cmd->version_support_bitmask = pmo_igmp_req->version_support;
	...
	wmi_debug("VDEVID:%d, FLAG:x%x version support:%d", cmd->vdev_id,
		  cmd->enable, cmd->version_support_bitmask);
 ...
 * Return: QDF status
 */
static QDF_STATUS send_gtk_offload_cmd_tlv(wmi_unified_t wmi_handle,
					   uint8_t vdev_id,
					   ...
					   uint32_t gtk_offload_opcode)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(WMI_GTK_OFFLOAD_CMD_fixed_param));
	...
	wmi_debug("VDEVID: %d, GTK_FLAGS: x%x kek len %d", vdev_id, cmd->flags,
		  params->kek_len);
	/* send the wmi command */
	wmi_mtrace(WMI_GTK_OFFLOAD_CMDID, cmd->vdev_id, 0);
	if (wmi_unified_cmd_send(wmi_handle, buf, len, WMI_GTK_OFFLOAD_CMDID)) {
...
static QDF_STATUS send_process_gtk_offload_getinfo_cmd_tlv(
	wmi_unified_t wmi_handle, uint8_t vdev_id, uint64_t offload_req_opcode)
{
	int len;
	wmi_buf_t buf;
	WMI_GTK_OFFLOAD_CMD_fixed_param *cmd;
...
static QDF_STATUS
send_enable_enhance_multicast_offload_tlv(wmi_unified_t wmi_handle,
					  uint8_t vdev_id, bool action)
{
	...
	cmd = (wmi_config_enhanced_mcast_filter_cmd_fixed_param *)wmi_buf_data(
		buf);
 ...
 */
static QDF_STATUS
extract_gtk_rsp_event_tlv(wmi_unified_t wmi_handle, void *evt_buf,
			  ...
			  uint32_t len)
{
	...
	qdf_mem_copy(&gtk_rsp_param->replay_counter,
		     &fixed_param->replay_counter, GTK_REPLAY_COUNTER_BYTES);
						 ...
						 uint16_t rate_limit_interval)
{
	...
	len = sizeof(WMI_WOW_ADD_PATTERN_CMD_fixed_param) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_BITMAP_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_IPV4_SYNC_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_IPV6_SYNC_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_MAGIC_PATTERN_CMD) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(uint32_t) + WMI_TLV_HDR_SIZE + 1 * sizeof(uint32_t);
 ...
 */
static QDF_STATUS send_action_frame_patterns_cmd_tlv(
	wmi_unified_t wmi_handle,
 ...
 */
static QDF_STATUS
send_lphb_config_hbenable_cmd_tlv(wmi_unified_t wmi_handle,
				  wmi_hb_set_enable_cmd_fixed_param *params)
{
	...
	WMITLV_SET_HDR(
		&hb_enable_fp->tlv_header,
		WMITLV_TAG_STRUC_wmi_hb_set_enable_cmd_fixed_param,
		WMITLV_GET_STRUCT_TLVLEN(wmi_hb_set_enable_cmd_fixed_param));
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		wmi_err("cmd_send WMI_HB_SET_ENABLE returned Error %d", status);
		wmi_buf_free(buf);
	}
 ...
 * Return: QDF status
 */
static QDF_STATUS send_lphb_config_tcp_params_cmd_tlv(
	wmi_unified_t wmi_handle,
 ...
 * Return: QDF status
 */
static QDF_STATUS send_lphb_config_tcp_pkt_filter_cmd_tlv(
	wmi_unified_t wmi_handle,
 ...
 * Return: QDF status
 */
static QDF_STATUS send_lphb_config_udp_params_cmd_tlv(
	wmi_unified_t wmi_handle,
 ...
 * Return: QDF status
 */
static QDF_STATUS send_lphb_config_udp_pkt_filter_cmd_tlv(
	wmi_unified_t wmi_handle,
	wmi_hb_set_udp_pkt_filter_cmd_fixed_param *lphb_conf_req)
{
	...
	memcpy((void *)&hb_udp_filter_fp->filter,
	       (void *)&lphb_conf_req->filter, WMI_WLAN_HB_MAX_FILTER_SIZE);
...
void wmi_lphb_attach_tlv(struct wmi_unified *wmi_handle)
{
	...
	ops->send_lphb_config_hbenable_cmd = send_lphb_config_hbenable_cmd_tlv;
 ...
 */
static QDF_STATUS
send_enable_disable_packet_filter_cmd_tlv(wmi_unified_t wmi_handle,
 ...
 */
static QDF_STATUS
send_config_packet_filter_cmd_tlv(wmi_unified_t wmi_handle, uint8_t vdev_id,
				  ...
				  uint8_t filter_id, bool enable)
{
	...
	if (enable) {
		cmd->num_params =
			QDF_MIN(WMI_PACKET_FILTER_MAX_CMP_PER_PACKET_FILTER,
...
void wmi_packet_filtering_attach_tlv(struct wmi_unified *wmi_handle)
{
	...
	ops->send_config_packet_filter_cmd = send_config_packet_filter_cmd_tlv;
}
						  ...
						  uint8_t vdev_id)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(WMI_WOW_DEL_PATTERN_CMD_fixed_param));
	cmd->vdev_id = vdev_id;
	cmd->pattern_id = ptrn_id;
	cmd->pattern_type = WOW_BITMAP_PATTERN;
...
wmi_unified_cmd_send_chk(struct wmi_unified *wmi_handle, wmi_buf_t buf,
			 uint32_t buflen, uint32_t cmd_id)
{
	wmi_debug("Send WMI_WOW_HOSTWAKEUP_FROM_SLEEP_CMDID over QMI");
	return wmi_unified_cmd_send_over_qmi(wmi_handle, buf, buflen, cmd_id);
}
#else
static inline QDF_STATUS
wmi_unified_cmd_send_chk(struct wmi_unified *wmi_handle, wmi_buf_t buf,
						     ...
						     bool tx_pending_ind)
{
	...
	cmd = (wmi_wow_hostwakeup_from_sleep_cmd_fixed_param *)wmi_buf_data(
		buf);
						 ...
						 uint32_t cookie, uint32_t time)
{
	...
	len = sizeof(WMI_WOW_ADD_PATTERN_CMD_fixed_param) + WMI_TLV_HDR_SIZE +
	      ...
	      0 * sizeof(WOW_IPV6_SYNC_PATTERN_T) + WMI_TLV_HDR_SIZE +
	      0 * sizeof(WOW_MAGIC_PATTERN_CMD) + WMI_TLV_HDR_SIZE +
	      1 * sizeof(uint32_t) + WMI_TLV_HDR_SIZE + 1 * sizeof(uint32_t);
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(WMI_WOW_ADD_PATTERN_CMD_fixed_param));
	cmd->vdev_id = vdev_id;
	cmd->pattern_id = cookie, cmd->pattern_type = WOW_TIMER_PATTERN;
	buf_ptr += sizeof(WMI_WOW_ADD_PATTERN_CMD_fixed_param);
	...
	*((uint32_t *)buf_ptr) = 0;
	...
	wmi_debug("send wake timer pattern with time[%d] to fw vdev = %d", time,
		  vdev_id);
					      ...
					      struct ext_wow_params *params)
{
	...
	WMITLV_SET_HDR(
		...
		WMITLV_GET_STRUCT_TLVLEN(wmi_extwow_enable_cmd_fixed_param));

	cmd->vdev_id = params->vdev_id;
	cmd->type = params->type;
	cmd->wakeup_pin_num = params->wakeup_pin_num;

	wmi_debug("vdev_id %d type %d Wakeup_pin_num %x", cmd->vdev_id,
 ...
 */
static QDF_STATUS
send_set_app_type2_params_in_fw_cmd_tlv(wmi_unified_t wmi_handle,
					struct app_type2_params *appType2Params)
{
	...
	cmd = (wmi_extwow_set_app_type2_params_cmd_fixed_param *)wmi_buf_data(
		buf);
	...
	wmi_debug("vdev_id %d gateway_mac " QDF_MAC_ADDR_FMT " "
		  ...
		  cmd->rc4_key, cmd->rc4_key_len, cmd->ip_id, cmd->ip_device_ip,
		  cmd->ip_server_ip, cmd->tcp_src_port, cmd->tcp_dst_port,
		  cmd->tcp_seq, cmd->tcp_ack_seq, cmd->keepalive_init,
		  cmd->keepalive_min, cmd->keepalive_max, cmd->keepalive_inc,
 ...
 */
static QDF_STATUS
send_app_type1_params_in_fw_cmd_tlv(wmi_unified_t wmi_handle,
				    struct app_type1_params *app_type1_params)
{
	...
	cmd = (wmi_extwow_set_app_type1_params_cmd_fixed_param *)wmi_buf_data(
		buf);
				 ...
				 struct pmo_icmp_offload *pmo_icmp_req)
{
	...
	case pmo_ipv4_change_notify:
		status =
			send_icmp_ipv4_config_cmd_tlv(wmi_handle, pmo_icmp_req);
		break;
	case pmo_ipv6_change_notify:
		status =
			send_icmp_ipv6_config_cmd_tlv(wmi_handle, pmo_icmp_req);
		break;
	default:
...
void wmi_pmo_attach_tlv(wmi_unified_t wmi_handle)
{
	...
	ops->send_add_wow_wakeup_event_cmd = send_add_wow_wakeup_event_cmd_tlv;
