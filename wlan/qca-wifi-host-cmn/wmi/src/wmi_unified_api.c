					   ...
					   uint8_t pdev_id)
{
	if (wmi_handle->ops->send_pdev_set_rf_path_cmd)
		return wmi_handle->ops->send_pdev_set_rf_path_cmd(
			wmi_handle, rf_path_index, pdev_id);
...
QDF_STATUS wmi_unified_vdev_delete_send(wmi_unified_t wmi_handle, uint8_t if_id)
{
	if (wmi_handle->ops->send_vdev_delete_cmd)
		return wmi_handle->ops->send_vdev_delete_cmd(wmi_handle, if_id);
				       ...
				       struct vdev_nss_chains *user_cfg)
{
	if (wmi_handle->ops->send_vdev_nss_chain_params_cmd)
		return wmi_handle->ops->send_vdev_nss_chain_params_cmd(
			wmi_handle, vdev_id, user_cfg);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_unified_vdev_stop_send(wmi_unified_t wmi_handle, uint8_t vdev_id)
{
	if (wmi_handle->ops->send_vdev_stop_cmd)
		return wmi_handle->ops->send_vdev_stop_cmd(wmi_handle, vdev_id);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_unified_vdev_down_send(wmi_unified_t wmi_handle, uint8_t vdev_id)
{
				   ...
				   struct vdev_scan_nac_rssi_params *req)
{
	if (wmi_handle->ops->send_vdev_set_nac_rssi_cmd)
		return wmi_handle->ops->send_vdev_set_nac_rssi_cmd(wmi_handle,
								   req);
				 ...
				 struct peer_flush_params *param)
{
	if (wmi_handle->ops->send_peer_flush_tids_cmd)
		return wmi_handle->ops->send_peer_flush_tids_cmd(
			wmi_handle, peer_addr, param);
...
QDF_STATUS
wmi_unified_peer_delete_all_send(wmi_unified_t wmi_hdl,
...
QDF_STATUS wmi_unified_peer_rx_reorder_queue_remove_send(
	wmi_unified_t wmi_handle, struct rx_reorder_queue_remove_params *param)
{
...
QDF_STATUS
wmi_unified_green_ap_ll_ps_send(wmi_unified_t wmi_handle,
				struct green_ap_ll_ps_cmd_param *ll_ps_params)
{
	if (wmi_handle->ops->send_green_ap_ll_ps_cmd)
		return wmi_handle->ops->send_green_ap_ll_ps_cmd(wmi_handle,
								ll_ps_params);
	...
	struct wlan_green_ap_ll_ps_event_param *green_ap_ll_ps_event_param)
{
	if (wmi_handle->ops->extract_green_ap_ll_ps_param)
		return wmi_handle->ops->extract_green_ap_ll_ps_param(
			evt_buf, green_ap_ll_ps_event_param);
...
wmi_unified_pdev_utf_cmd_send(wmi_unified_t wmi_handle,
			      struct pdev_utf_params *param, uint8_t mac_id)
{
...
QDF_STATUS
wmi_unified_pdev_param_send(wmi_unified_t wmi_handle, struct pdev_params *param,
			    uint8_t mac_id)
{
...
QDF_STATUS
wmi_unified_multiple_pdev_param_send(
...
QDF_STATUS
wmi_unified_multiple_vdev_param_send(
	wmi_unified_t wmi_handle, struct set_multiple_pdev_vdev_param *params)
{
...
QDF_STATUS wmi_unified_resume_send(wmi_unified_t wmi_handle, uint8_t mac_id)
{
	if (wmi_handle->ops->send_resume_cmd)
		return wmi_handle->ops->send_resume_cmd(wmi_handle, mac_id);
...
wmi_unified_wow_add_wakeup_pattern_send(
	wmi_unified_t wmi_handle, struct wow_add_wakeup_pattern_params *param)
{
				      ...
				      struct ap_ps_params *param)
{
	if (wmi_handle->ops->send_set_ap_ps_param_cmd)
		return wmi_handle->ops->send_set_ap_ps_param_cmd(
			wmi_handle, peer_addr, param);
				   ...
				   uint8_t mac_id)
{
	if (wmi_handle->ops->send_packet_log_enable_cmd)
		return wmi_handle->ops->send_packet_log_enable_cmd(
			wmi_handle, PKTLOG_EVENT, mac_id);
			     ...
			     struct fils_discovery_tmpl_params *param)
{
	if (wmi_handle->ops->send_fd_tmpl_cmd)
		return wmi_handle->ops->send_fd_tmpl_cmd(wmi_handle, param);
			    ...
			    struct peer_assoc_params *param)
{
	if (wmi_handle->ops->send_peer_assoc_cmd)
		return wmi_handle->ops->send_peer_assoc_cmd(wmi_handle, param);
				...
				struct scan_req_params *param)
{
	if (wmi_handle->ops->send_scan_start_cmd)
		return wmi_handle->ops->send_scan_start_cmd(wmi_handle, param);
			       ...
			       struct scan_cancel_param *param)
{
	if (wmi_handle->ops->send_scan_stop_cmd)
		return wmi_handle->ops->send_scan_stop_cmd(wmi_handle, param);
				     ...
				     struct wmi_mgmt_params *param)
{
	if (wmi_handle->ops->send_mgmt_cmd)
		return wmi_handle->ops->send_mgmt_cmd(wmi_handle, param);
				  ...
				  int value)
{
	if (wmi_handle->ops->send_set_mimops_cmd)
		return wmi_handle->ops->send_set_mimops_cmd(wmi_handle, vdev_id,
							    value);
					...
					struct sta_uapsd_trig_params *param)
{
	if (wmi_handle->ops->send_set_sta_uapsd_auto_trig_cmd)
		return wmi_handle->ops->send_set_sta_uapsd_auto_trig_cmd(
			wmi_handle, param);
	...
	struct wmi_peer_rate_report_params *rate_report_params)
{
	if (wmi_handle->ops->send_peer_rate_report_cmd)
		return wmi_handle->ops->send_peer_rate_report_cmd(
			wmi_handle, rate_report_params);
	...
	struct wmi_host_wme_vparams wmm_vparams[WMI_MAX_NUM_AC])
{
	if (wmi_handle->ops->send_process_update_edca_param_cmd)
		return wmi_handle->ops->send_process_update_edca_param_cmd(
			wmi_handle, vdev_id, mu_edca_param, wmm_vparams);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS
wmi_unified_probe_rsp_tmpl_send_cmd(wmi_unified_t wmi_handle, uint8_t vdev_id,
				    struct wmi_probe_resp_params *probe_rsp_info)
{
	if (wmi_handle->ops->send_probe_rsp_tmpl_send_cmd)
		return wmi_handle->ops->send_probe_rsp_tmpl_send_cmd(
			wmi_handle, vdev_id, probe_rsp_info);
	...
	struct ipa_uc_offload_control_params *ipa_offload)
{
	...
	if (wmi_handle->ops->send_ipa_offload_control_cmd)
		return wmi_handle->ops->send_ipa_offload_control_cmd(
			wmi_handle, ipa_offload);
...
QDF_STATUS wmi_unified_pno_stop_cmd(wmi_unified_t wmi_handle, uint8_t vdev_id)
{
	if (wmi_handle->ops->send_pno_stop_cmd)
		return wmi_handle->ops->send_pno_stop_cmd(wmi_handle, vdev_id);
				     ...
				     struct pno_scan_req_params *pno)
{
	if (wmi_handle->ops->send_pno_start_cmd)
		return wmi_handle->ops->send_pno_start_cmd(wmi_handle, pno);
	...
	wmi_unified_t wmi_handle, const struct ll_stats_clear_params *clear_req)
{
	if (wmi_handle->ops->send_process_ll_stats_clear_cmd)
		return wmi_handle->ops->send_process_ll_stats_clear_cmd(
			wmi_handle, clear_req);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS
wmi_unified_process_ll_stats_set_cmd(wmi_unified_t wmi_handle,
				     const struct ll_stats_set_params *set_req)
{
	if (wmi_handle->ops->send_process_ll_stats_set_cmd)
		return wmi_handle->ops->send_process_ll_stats_set_cmd(
			wmi_handle, set_req);
...
QDF_STATUS
wmi_unified_process_ll_stats_get_cmd(wmi_unified_t wmi_handle,
...
QDF_STATUS wmi_unified_snr_cmd(wmi_unified_t wmi_handle, uint8_t vdev_id)
{
	if (wmi_handle->ops->send_snr_cmd)
		return wmi_handle->ops->send_snr_cmd(wmi_handle, vdev_id);
					     ...
					     bool dfs_phyerr_filter_offload)
{
	if (wmi_handle->ops->send_dfs_phyerr_filter_offload_en_cmd)
		return wmi_handle->ops->send_dfs_phyerr_filter_offload_en_cmd(
			wmi_handle, dfs_phyerr_filter_offload);
					   ...
					   uint8_t user_triggered)
{
	if (wmi_handle->ops->send_pktlog_wmi_send_cmd)
		return wmi_handle->ops->send_pktlog_wmi_send_cmd(
			wmi_handle, pktlog_event, cmd_id, user_triggered);
...
QDF_STATUS wmi_unified_process_dhcpserver_offload_cmd(
	wmi_unified_t wmi_handle, struct dhcp_offload_info_params *params)
{
	if (wmi_handle->ops->send_process_dhcpserver_offload_cmd)
		return wmi_handle->ops->send_process_dhcpserver_offload_cmd(
			wmi_handle, params);
...
QDF_STATUS wmi_unified_send_regdomain_info_to_fw_cmd(
	wmi_unified_t wmi_handle, uint32_t reg_dmn, uint16_t regdmn2G,
	uint16_t regdmn5G, uint8_t ctl2G, uint8_t ctl5G)
{
	if (wmi_handle->ops->send_regdomain_info_to_fw_cmd)
		return wmi_handle->ops->send_regdomain_info_to_fw_cmd(
			wmi_handle, reg_dmn, regdmn2G, regdmn5G, ctl2G, ctl5G);
...
wmi_unified_cfg_action_frm_tb_ppdu_cmd(
	wmi_unified_t wmi_handle, struct cfg_action_frm_tb_ppdu_param *cfg_info)
{
					...
					struct wmi_wifi_start_log *start_log)
{
	if (wmi_handle->ops->send_enable_specific_fw_logs_cmd)
		return wmi_handle->ops->send_enable_specific_fw_logs_cmd(
			wmi_handle, start_log);
...
wmi_unified_set_active_apf_mode_cmd(
	wmi_unified_t wmi, uint8_t vdev_id,
	...
	enum wmi_host_active_apf_mode mcast_bcast_mode)
{
	if (wmi->ops->send_set_active_apf_mode_cmd)
		return wmi->ops->send_set_active_apf_mode_cmd(
			wmi, vdev_id, ucast_mode, mcast_bcast_mode);
	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS
wmi_unified_send_apf_enable_cmd(wmi_unified_t wmi, uint32_t vdev_id,
				bool enable)
{
...
wmi_unified_send_apf_read_work_memory_cmd(
	wmi_unified_t wmi, struct wmi_apf_read_memory_params *read_params)
{
...
wmi_unified_encrypt_decrypt_send_cmd(
	void *wmi_hdl, struct disa_encrypt_decrypt_req_params *params)
{
...
QDF_STATUS
wmi_unified_set_vap_dscp_tid_map_cmd_send(wmi_unified_t wmi_handle,
...
QDF_STATUS wmi_unified_set_bridge_mac_addr_cmd_send(
	wmi_unified_t wmi_handle, struct set_bridge_mac_addr_params *param)
{
...
QDF_STATUS
wmi_unified_simulation_test_cmd_send(wmi_unified_t wmi_handle,
				     struct simulation_test_params *param)
{
	if (wmi_handle->ops->send_simulation_test_cmd)
		return wmi_handle->ops->send_simulation_test_cmd(wmi_handle,
								 param);
...
QDF_STATUS wmi_unified_vdev_spectral_enable_cmd_send(
	wmi_unified_t wmi_handle, struct vdev_spectral_enable_params *param)
{
...
QDF_STATUS wmi_unified_bss_chan_info_request_cmd_send(
	wmi_unified_t wmi_handle, struct bss_chan_info_request_params *param)
{
...
QDF_STATUS wmi_unified_thermal_mitigation_param_cmd_send(
	wmi_unified_t wmi_handle, struct thermal_mitigation_params *param)
{
...
QDF_STATUS wmi_extract_health_mon_event(wmi_unified_t wmi_handle, void *ev,
...
wmi_unified_vdev_set_qdepth_thresh_cmd_send(
	wmi_unified_t wmi_handle, struct set_qdepth_thresh_params *param)
{
...
QDF_STATUS
wmi_unified_pdev_set_regdomain_cmd_send(wmi_unified_t wmi_handle,
...
QDF_STATUS
wmi_unified_set_beacon_filter_cmd_send(wmi_unified_t wmi_handle,
...
QDF_STATUS wmi_unified_remove_beacon_filter_cmd_send(
	wmi_unified_t wmi_handle, struct remove_beacon_filter_params *param)
{
			    ...
			    struct peer_request_pn_param *pn_params)
{
	if (wmi_hdl->ops->send_pdev_get_pn_cmd)
		return wmi_hdl->ops->send_pdev_get_pn_cmd(wmi_hdl, pn_params);
			      ...
			      struct peer_request_rxpn_param *pn_params)
{
	if (wmi_hdl->ops->send_pdev_get_rxpn_cmd)
		return wmi_hdl->ops->send_pdev_get_rxpn_cmd(wmi_hdl, pn_params);
...
QDF_STATUS wmi_unified_addba_clearresponse_cmd_send(
	wmi_unified_t wmi_handle, uint8_t macaddr[QDF_MAC_ADDR_SIZE],
	struct addba_clearresponse_params *param)
{
	if (wmi_handle->ops->send_addba_clearresponse_cmd)
		return wmi_handle->ops->send_addba_clearresponse_cmd(
				...
				struct addba_send_params *param)
{
	if (wmi_handle->ops->send_addba_send_cmd)
		return wmi_handle->ops->send_addba_send_cmd(wmi_handle, macaddr,
							    param);
				...
				struct delba_send_params *param)
{
	if (wmi_handle->ops->send_delba_send_cmd)
		return wmi_handle->ops->send_delba_send_cmd(wmi_handle, macaddr,
							    param);
				       ...
				       struct addba_setresponse_params *param)
{
	if (wmi_handle->ops->send_addba_setresponse_cmd)
		return wmi_handle->ops->send_addba_setresponse_cmd(
			wmi_handle, macaddr, param);
				       ...
				       void *bitmap_buf)
{
	if (wmi_handle->ops->save_ext_service_bitmap) {
		return wmi_handle->ops->save_ext_service_bitmap(
			wmi_handle, evt_buf, bitmap_buf);
	}
	return QDF_STATUS_E_FAILURE;
}
...
bool wmi_service_enabled(wmi_unified_t wmi_handle, uint32_t service_id)
{
	if ((service_id < wmi_services_max) &&
		...
		if (wmi_handle->ops->is_service_enabled) {
			return wmi_handle->ops->is_service_enabled(
				wmi_handle, wmi_handle->services[service_id]);
		}
	} else {
				  ...
				  struct wmi_host_fw_ver *fw_ver)
{
	if (wmi_handle->ops->extract_fw_version)
		return wmi_handle->ops->extract_fw_version(wmi_handle, evt_buf,
							   fw_ver);
			...
			struct wlan_psoc_hal_reg_capability *hal_reg_cap)
{
	if (wmi_handle->ops->extract_hal_reg_cap)
		return wmi_handle->ops->extract_hal_reg_cap(wmi_handle, evt_buf,
							    hal_reg_cap);
...
uint32_t wmi_extract_num_mem_reqs_from_service_ready(wmi_unified_t wmi_handle,
						     void *evt_buf)
{
	...
	enum wmi_fw_mem_prio fw_prio, uint16_t idx)
{
	if (wmi_handle->ops->extract_host_mem_req)
		return wmi_handle->ops->extract_host_mem_req(
			wmi_handle, evt_buf, mem_reqs, num_active_peers,
...
QDF_STATUS wmi_ready_extract_mac_addr(wmi_unified_t wmi_handle, void *ev,
				      uint8_t *macaddr)
{
	if (wmi_handle->ops->ready_extract_mac_addr)
		return wmi_handle->ops->ready_extract_mac_addr(wmi_handle, ev,
							       macaddr);

	return QDF_STATUS_E_FAILURE;
}

wmi_host_mac_addr *wmi_ready_extract_mac_addr_list(wmi_unified_t wmi_handle,
						   void *ev,
						   uint8_t *num_mac_addr)
{
	if (wmi_handle->ops->ready_extract_mac_addr_list)
		return wmi_handle->ops->ready_extract_mac_addr_list(
			wmi_handle, ev, num_mac_addr);
...
QDF_STATUS
wmi_extract_ready_event_params(wmi_unified_t wmi_handle, void *evt_buf,
			       struct wmi_host_ready_ev_param *ev_param)
{
	if (wmi_handle->ops->extract_ready_event_params)
		return wmi_handle->ops->extract_ready_event_params(
			wmi_handle, evt_buf, ev_param);
				  ...
				  struct wmi_host_get_pn_event *param)
{
	if (wmi_hdl->ops->extract_get_pn_data)
		return wmi_hdl->ops->extract_get_pn_data(wmi_hdl, evt_buf,
							 param);
	return QDF_STATUS_E_FAILURE;
}
				    ...
				    struct wmi_host_get_rxpn_event *param)
{
	if (wmi_hdl->ops->extract_get_rxpn_data)
		return wmi_hdl->ops->extract_get_rxpn_data(wmi_hdl, evt_buf,
							   param);
	return QDF_STATUS_E_FAILURE;
}
...
wmi_extract_encrypt_decrypt_resp_params(
	...
	struct disa_encrypt_decrypt_resp_params *param)
{
	wmi_unified_t wmi_handle = (wmi_unified_t)wmi_hdl;
			   ...
			   struct mgmt_rx_event_params *hdr, uint8_t **bufp)
{
	if (wmi_handle->ops->extract_mgmt_rx_params)
		return wmi_handle->ops->extract_mgmt_rx_params(
			wmi_handle, evt_buf, hdr, bufp);
...
QDF_STATUS
wmi_unified_mgmt_rx_reo_filter_config_cmd(wmi_unified_t wmi_handle,
					  uint8_t pdev_id,
			    ...
			    struct frame_pn_params *pn_params)
{
	if (wmi_handle->ops->extract_frame_pn_params)
		return wmi_handle->ops->extract_frame_pn_params(
			wmi_handle, evt_buf, pn_params);
			     ...
			     struct frm_conn_ap *is_conn_ap_frm)
{
	if (wmi_handle->ops->extract_is_conn_ap_frame)
		return wmi_handle->ops->extract_is_conn_ap_frame(
			wmi_handle, evt_buf, is_conn_ap_frm);
			       ...
			       struct scan_event *param)
{
	if (wmi_handle->ops->extract_vdev_scan_ev_param)
		return wmi_handle->ops->extract_vdev_scan_ev_param(
			wmi_handle, evt_buf, param);
			       ...
			       struct scan_event *param)
{
	if (wmi_handle->ops->extract_nlo_match_ev_param)
		return wmi_handle->ops->extract_nlo_match_ev_param(
			wmi_handle, evt_buf, param);
...
QDF_STATUS
wmi_extract_mu_db_entry(wmi_unified_t wmi_handle, void *evt_buf, uint8_t idx,
			wmi_host_mu_db_entry *param)
{
				...
				uint32_t *gpio_num)
{
	if (wmi_handle->ops->extract_gpio_input_ev_param)
		return wmi_handle->ops->extract_gpio_input_ev_param(
			wmi_handle, evt_buf, gpio_num);
...
QDF_STATUS wmi_extract_peer_ratecode_list_ev(wmi_unified_t wmi_handle,
					     ...
					     wmi_sa_rate_cap *rate_cap)
{
	if (wmi_handle->ops->extract_peer_ratecode_list_ev)
		return wmi_handle->ops->extract_peer_ratecode_list_ev(
			wmi_handle, evt_buf, peer_mac, pdev_id, rate_cap);
			...
			wmi_host_phyerr_t *phyerr)
{
	if (wmi_handle->ops->extract_comb_phyerr)
		return wmi_handle->ops->extract_comb_phyerr(
			wmi_handle, evt_buf, datalen, buf_offset, phyerr);
			  ...
			  wmi_host_phyerr_t *phyerr)
{
	if (wmi_handle->ops->extract_single_phyerr)
		return wmi_handle->ops->extract_single_phyerr(
			wmi_handle, evt_buf, datalen, buf_offset, phyerr);
			     ...
			     uint16_t datalen, wmi_host_phyerr_t *phyerr)
{
	if (wmi_handle->ops->extract_composite_phyerr)
		return wmi_handle->ops->extract_composite_phyerr(
			wmi_handle, evt_buf, datalen, phyerr);
		      ...
		      wmi_unit_test_event *unit_test, uint32_t maxspace)
{
	if (wmi_handle->ops->extract_unit_test)
		return wmi_handle->ops->extract_unit_test(wmi_handle, evt_buf,
							  unit_test, maxspace);
			       ...
			       wmi_host_rtt_event_hdr *ev)
{
	if (wmi_handle->ops->extract_rtt_hdr)
		return wmi_handle->ops->extract_rtt_hdr(wmi_handle, evt_buf,
							ev);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_extract_bcnflt_stats(wmi_unified_t wmi_handle, void *evt_buf,
				    uint32_t index,
				    wmi_host_bcnflt_stats *bcnflt_stats)
{
	if (wmi_handle->ops->extract_bcnflt_stats)
		return wmi_handle->ops->extract_bcnflt_stats(
			wmi_handle, evt_buf, index, bcnflt_stats);
			      ...
			      uint16_t hdump_len)
{
	if (wmi_handle->ops->extract_rtt_ev)
		return wmi_handle->ops->extract_rtt_ev(wmi_handle, evt_buf, ev,
						       hdump, hdump_len);
...
QDF_STATUS
wmi_extract_peer_retry_stats(wmi_unified_t wmi_handle, void *evt_buf,
			     uint32_t index,
			     struct wmi_host_peer_retry_stats *peer_retry_stats)
{
	if (wmi_handle->ops->extract_peer_retry_stats)
		return wmi_handle->ops->extract_peer_retry_stats(
			wmi_handle, evt_buf, index, peer_retry_stats);
				...
				wmi_host_rtt_error_report_event *ev)
{
	if (wmi_handle->ops->extract_rtt_error_report_ev)
		return wmi_handle->ops->extract_rtt_error_report_ev(
			wmi_handle, evt_buf, ev);
...
QDF_STATUS
wmi_extract_chan_stats(wmi_unified_t wmi_handle, void *evt_buf, uint32_t index,
		       wmi_host_chan_stats *chan_stats)
{
	if (wmi_handle->ops->extract_chan_stats)
		return wmi_handle->ops->extract_chan_stats(wmi_handle, evt_buf,
							   index, chan_stats);
				     ...
				     uint32_t *pdev_id)
{
	if (wmi_handle->ops->extract_thermal_stats)
		return wmi_handle->ops->extract_thermal_stats(
			wmi_handle, evt_buf, temp, level, therm_throt_levels,
			tt_stats, pdev_id);
			...
			wmi_host_wlan_profile_ctx_t *profile_ctx)
{
	if (wmi_handle->ops->extract_profile_ctx)
		return wmi_handle->ops->extract_profile_ctx(wmi_handle, evt_buf,
							    profile_ctx);
				...
				uint32_t *dccount)
{
	if (wmi_handle->ops->extract_thermal_level_stats)
		return wmi_handle->ops->extract_thermal_level_stats(
			wmi_handle, evt_buf, idx, levelcount, dccount);
			 ...
			 wmi_host_wlan_profile_t *profile_data)
{
	if (wmi_handle->ops->extract_profile_data)
		return wmi_handle->ops->extract_profile_data(
			wmi_handle, evt_buf, idx, profile_data);
	...
	wmi_host_pdev_bss_chan_info_event *bss_chan_info)
{
	if (wmi_handle->ops->extract_bss_chan_info_event)
		return wmi_handle->ops->extract_bss_chan_info_event(
			wmi_handle, evt_buf, bss_chan_info);
			    ...
			    wmi_host_vdev_extd_stats *vdev_extd_stats)
{
	if (wmi_handle->ops->extract_vdev_extd_stats)
		return wmi_handle->ops->extract_vdev_extd_stats(
			wmi_handle, evt_buf, index, vdev_extd_stats);
	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS
wmi_extract_bcn_stats(wmi_unified_t wmi_handle, void *evt_buf, uint32_t index,
		      wmi_host_bcn_stats *vdev_bcn_stats)
{
	if (wmi_handle->ops->extract_bcn_stats)
		return wmi_handle->ops->extract_bcn_stats(
			wmi_handle, evt_buf, index, vdev_bcn_stats);
	return QDF_STATUS_E_FAILURE;
}
	...
	struct wmi_host_vdev_nac_rssi_event *vdev_nac_rssi_stats)
{
	if (wmi_handle->ops->extract_vdev_nac_rssi_stats)
		return wmi_handle->ops->extract_vdev_nac_rssi_stats(
			wmi_handle, evt_buf, vdev_nac_rssi_stats);
	...
	struct wmi_host_vdev_prb_fils_stats *vdev_prb_fils_stats)
{
	if (wmi_handle->ops->extract_vdev_prb_fils_stats)
		return wmi_handle->ops->extract_vdev_prb_fils_stats(
			wmi_handle, evt_buf, index, vdev_prb_fils_stats);
					  ...
					  struct wmi_power_dbg_params *param)
{
	if (wmi_handle->ops->send_power_dbg_cmd)
		return wmi_handle->ops->send_power_dbg_cmd(wmi_handle, param);
				       ...
				       struct btcoex_cfg_params *param)
{
	if (wmi_handle->ops->send_btcoex_duty_cycle_cmd)
		return wmi_handle->ops->send_btcoex_duty_cycle_cmd(wmi_handle,
								   param);
			       ...
			       struct esl_egid_params *param)
{
	if (wmi_handle->ops->send_egid_info_cmd) {
		return wmi_handle->ops->send_egid_info_cmd(wmi_handle, param);
	}
...
QDF_STATUS
wmi_extract_service_ready_ext(wmi_unified_t wmi_handle, uint8_t *evt_buf,
			      struct wlan_psoc_host_service_ext_param *param)
{
	if (wmi_handle->ops->extract_service_ready_ext)
		return wmi_handle->ops->extract_service_ready_ext(
			wmi_handle, evt_buf, param);
...
QDF_STATUS
wmi_extract_service_ready_ext2(struct wmi_unified *wmi_handle, uint8_t *evt_buf,
			       struct wlan_psoc_host_service_ext2_param *param)
{
	if (wmi_handle->ops->extract_service_ready_ext2)
		return wmi_handle->ops->extract_service_ready_ext2(
			wmi_handle, evt_buf, param);
...
QDF_STATUS
wmi_extract_dbs_or_sbs_cap_service_ready_ext2(wmi_unified_t wmi_handle,
					      uint8_t *evt_buf,
					      uint32_t *sbs_lower_band_end_freq)
{
	if (wmi_handle->ops->extract_dbs_or_sbs_service_ready_ext2)
		return wmi_handle->ops->extract_dbs_or_sbs_service_ready_ext2(
			wmi_handle, evt_buf, sbs_lower_band_end_freq);
	...
	struct wlan_psoc_host_service_ext_param *ext_param)
{
	if (wmi_handle->ops->extract_sar_cap_service_ready_ext)
		return wmi_handle->ops->extract_sar_cap_service_ready_ext(
			wmi_handle, evt_buf, ext_param);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_extract_hw_mode_cap_service_ready_ext(
	wmi_unified_t wmi_handle, uint8_t *evt_buf, uint8_t hw_mode_idx,
	struct wlan_psoc_host_hw_mode_caps *param)
{
	if (wmi_handle->ops->extract_hw_mode_cap_service_ready_ext)
		return wmi_handle->ops->extract_hw_mode_cap_service_ready_ext(
			wmi_handle, evt_buf, hw_mode_idx, param);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_extract_mac_phy_cap_service_ready_ext(
	wmi_unified_t wmi_handle, uint8_t *evt_buf, uint8_t hw_mode_id,
	uint8_t phy_id, struct wlan_psoc_host_mac_phy_caps *param)
{
	if (wmi_handle->ops->extract_mac_phy_cap_service_ready_ext)
		return wmi_handle->ops->extract_mac_phy_cap_service_ready_ext(
			wmi_handle, evt_buf, hw_mode_id, phy_id, param);
	...
	struct wlan_psoc_host_mac_phy_caps_ext2 *mac_phy_cap)
{
	if (wmi_handle->ops->extract_mac_phy_cap_service_ready_ext2)
		return wmi_handle->ops->extract_mac_phy_cap_service_ready_ext2(
			wmi_handle, evt_buf, hw_mode_id, phy_id, phy_idx,
			mac_phy_cap);
...
QDF_STATUS wmi_extract_reg_cap_service_ready_ext(
	wmi_unified_t wmi_handle, uint8_t *evt_buf, uint8_t phy_idx,
	struct wlan_psoc_host_hal_reg_capabilities_ext *param)
{
	if (wmi_handle->ops->extract_reg_cap_service_ready_ext)
		return wmi_handle->ops->extract_reg_cap_service_ready_ext(
			wmi_handle, evt_buf, phy_idx, param);
...
QDF_STATUS
wmi_extract_msdu_idx_qtype_map_service_ready_ext2(wmi_unified_t wmi_handle,
	...
	if (wmi_handle->ops->extract_msdu_idx_qtype_map_service_ready_ext2)
		return wmi_handle->ops
			->extract_msdu_idx_qtype_map_service_ready_ext2(
				wmi_handle, evt_buf, idx, msdu_qtype);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_extract_sw_cal_ver_ext2(wmi_unified_t wmi_handle, uint8_t *event,
...
QDF_STATUS
wmi_extract_spectral_scan_bw_caps(wmi_unified_t wmi_handle, void *event,
				      ...
				      struct wmi_host_pdev_utf_event *param)
{
	if (wmi_handle->ops->extract_pdev_utf_event)
		return wmi_handle->ops->extract_pdev_utf_event(wmi_handle,
							       evt_buf, param);
				       ...
				       struct wmi_host_pdev_qvit_event *param)
{
	if (wmi_handle->ops->extract_pdev_qvit_event)
		return wmi_handle->ops->extract_pdev_qvit_event(wmi_handle,
								evt_buf, param);
				 ...
				 struct coex_config_params *param)
{
	if (wmi_handle->ops->send_coex_config_cmd)
		return wmi_handle->ops->send_coex_config_cmd(wmi_handle, param);
				 ...
				 struct coex_dbam_config_params *param)
{
	if (wmi_handle->ops->send_dbam_config_cmd)
		return wmi_handle->ops->send_dbam_config_cmd(wmi_handle, param);
				      ...
				      struct vdev_adfs_ch_cfg_params *param)
{
	if (wmi_handle->ops->send_adfs_ch_cfg_cmd)
		return wmi_handle->ops->send_adfs_ch_cfg_cmd(wmi_handle, param);
	return QDF_STATUS_E_FAILURE;
}
					  ...
					  struct vdev_adfs_abort_params *param)
{
	if (wmi_handle->ops->send_adfs_ocac_abort_cmd)
		return wmi_handle->ops->send_adfs_ocac_abort_cmd(wmi_handle,
								 param);
	return QDF_STATUS_E_FAILURE;
}
#endif
...
QDF_STATUS wmi_extract_num_rf_characterization_entries(
	...
	uint32_t *num_rf_characterization_entries)
{
	if (wmi_hdl->ops->extract_num_rf_characterization_entries)
		return wmi_hdl->ops->extract_num_rf_characterization_entries(
			wmi_hdl, evt_buf, num_rf_characterization_entries);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_extract_rf_characterization_entries(
	wmi_unified_t wmi_hdl, uint8_t *evt_buf,
	uint32_t num_rf_characterization_entries,
		...
		*rf_characterization_entries)
{
	if (wmi_hdl->ops->extract_rf_characterization_entries)
		return wmi_hdl->ops->extract_rf_characterization_entries(
			wmi_hdl, evt_buf, num_rf_characterization_entries,
			rf_characterization_entries);
	...
	struct wlan_psoc_host_chainmask_table *chainmask_table)
{
	if (wmi_handle->ops->extract_chainmask_tables)
		return wmi_handle->ops->extract_chainmask_tables(
			wmi_handle, evt_buf, chainmask_table);
					    ...
					    struct set_country *param)
{
	if (wmi_handle->ops->send_set_country_cmd)
		return wmi_handle->ops->send_set_country_cmd(wmi_handle, param);
				...
				struct action_oui_request *req)
{
	if (wmi_handle->ops->send_action_oui_cmd)
		return wmi_handle->ops->send_action_oui_cmd(wmi_handle, req);
		      ...
		      struct wdsentry *wds_entry, u_int32_t idx)
{
	if (wmi_handle->ops->extract_wds_entry)
		return wmi_handle->ops->extract_wds_entry(wmi_handle, evt_buf,
							  wds_entry, idx);
	...
	struct wmi_obss_detection_cfg_param *obss_cfg_param)
{
	if (wmi_handle->ops->send_obss_detection_cfg_cmd)
		return wmi_handle->ops->send_obss_detection_cfg_cmd(
			wmi_handle, obss_cfg_param);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS
wmi_unified_extract_obss_detection_info(wmi_unified_t wmi_handle, uint8_t *data,
...
wmi_get_ch_width_from_phy_mode(wmi_unified_t wmi_handle,
			       ...
			       uint32_t *dfs_status_check)
{
	if (wmi_handle->ops->extract_dfs_status_from_fw)
		return wmi_handle->ops->extract_dfs_status_from_fw(
			wmi_handle, evt_buf, dfs_status_check);
...
QDF_STATUS wmi_extract_smartlog_ev(wmi_unified_t wmi_handle, void *evt_buf,
				   struct wmi_debug_fatal_events *ev)
{
	if (wmi_handle->ops->extract_smartlog_event)
		return wmi_handle->ops->extract_smartlog_event(wmi_handle,
							       evt_buf, ev);
...
uint8_t
wlan_crypto_cipher_to_wmi_cipher(enum wlan_crypto_cipher_type crypto_cipher)
{
...
enum cdp_sec_type
wlan_crypto_cipher_to_cdp_sec_type(enum wlan_crypto_cipher_type crypto_cipher)
{
	switch (crypto_cipher) {
	case WLAN_CRYPTO_CIPHER_NONE:
				    ...
				    struct wmi_roam_scan_stats_res **res_param)
{
	if (wmi->ops->extract_roam_scan_stats_res_evt)
		return wmi->ops->extract_roam_scan_stats_res_evt(
			wmi, evt_buf, vdev_id, res_param);
				      ...
				      uint32_t *vdev_id, uint32_t *tx_status)
{
	if (wmi_handle->ops->extract_offload_bcn_tx_status_evt)
		return wmi_handle->ops->extract_offload_bcn_tx_status_evt(
			wmi_handle, evt_buf, vdev_id, tx_status);
	...
	struct wmi_host_obss_spatial_reuse_set_param *obss_spatial_reuse_param)
{
	if (wmi_handle->ops->send_obss_spatial_reuse_set)
		return wmi_handle->ops->send_obss_spatial_reuse_set(
			wmi_handle, obss_spatial_reuse_param);
	...
	uint8_t pdev_id)
{
	if (wmi_handle->ops->send_self_srg_obss_color_enable_bitmap)
		return wmi_handle->ops->send_self_srg_obss_color_enable_bitmap(
			wmi_handle, bitmap_0, bitmap_1, pdev_id);
	...
	uint8_t pdev_id)
{
	if (wmi_handle->ops->send_self_srg_obss_bssid_enable_bitmap)
		return wmi_handle->ops->send_self_srg_obss_bssid_enable_bitmap(
			wmi_handle, bitmap_0, bitmap_1, pdev_id);
					      ...
					      uint32_t *host_pdev_id)
{
	if (wmi_handle->ops->convert_pdev_id_target_to_host) {
		*host_pdev_id = wmi_handle->ops->convert_pdev_id_target_to_host(
			wmi_handle, target_pdev_id);
		return QDF_STATUS_SUCCESS;
	}
...
QDF_STATUS
wmi_unified_extract_measreq_chan_info(wmi_unified_t wmi_handle,
				      uint32_t data_len, uint8_t *data,
				      struct rtt_channel_info *chinfo)
{
	if (wmi_handle->ops->extract_measreq_chan_info)
		return wmi_handle->ops->extract_measreq_chan_info(data_len,
								  data, chinfo);
					     ...
					     uint32_t vdev_id, uint32_t cmd_id)
{
	if (wmi_handle->ops->send_mws_coex_status_req_cmd)
		return wmi_handle->ops->send_mws_coex_status_req_cmd(
			wmi_handle, vdev_id, cmd_id);
			       ...
			       struct wmi_oem_response_param *oem_resp_param)
{
	if (wmi_hdl->ops->extract_oem_response_param)
		return wmi_hdl->ops->extract_oem_response_param(
			wmi_hdl, resp_buf, oem_resp_param);
				 ...
				 struct wifi_pos_pasn_peer_data *dst)
{
	if (wmi->ops->extract_pasn_peer_create_req_event)
		return wmi->ops->extract_pasn_peer_create_req_event(
			wmi, evt_buf, dst);
				 ...
				 struct wifi_pos_pasn_peer_data *dst)
{
	if (wmi->ops->extract_pasn_peer_delete_req_event)
		return wmi->ops->extract_pasn_peer_delete_req_event(
			wmi, evt_buf, dst);
...
QDF_STATUS wmi_unified_extract_hw_mode_resp(wmi_unified_t wmi, void *evt_buf,
					    uint32_t *cmd_status)
{
	if (wmi->ops->extract_hw_mode_resp_event)
		return wmi->ops->extract_hw_mode_resp_event(wmi, evt_buf,
							    cmd_status);

	return QDF_STATUS_E_FAILURE;
}

QDF_STATUS wmi_unified_extract_rf_path_resp(wmi_unified_t wmi, void *evt_buf,
					    uint32_t *cmd_status)
{
	if (wmi->ops->extract_rf_path_resp)
		return wmi->ops->extract_rf_path_resp(wmi, evt_buf, cmd_status);
				    ...
				    uint8_t chan_idx, uint8_t ap_idx)
{
	if (wmi->ops->extract_roam_scan_stats)
		return wmi->ops->extract_roam_scan_stats(wmi, evt_buf, dst, idx,
							 chan_idx, ap_idx);
						 ...
						 uint64_t lpass_ts)
{
	if (wmi_handle->ops->send_wlan_ts_qtime_cmd)
		return wmi_handle->ops->send_wlan_ts_qtime_cmd(
			wmi_handle, vdev_id, lpass_ts);
					 ...
					 struct ftm_time_sync_offset *param)
{
	if (wmi_handle->ops->extract_time_sync_ftm_offset_event)
		return wmi_handle->ops->extract_time_sync_ftm_offset_event(
			wmi_handle, evt_buf, param);
...
wmi_unified_send_injector_frame_config_cmd(
	wmi_unified_t wmi_handle, struct wmi_host_injector_frame_params *param)
{
					  ...
					  void *evt_buf, uint32_t *more_flag)
{
	if (wmi_handle->ops->extract_cp_stats_more_pending)
		return wmi_handle->ops->extract_cp_stats_more_pending(
			wmi_handle, evt_buf, more_flag);
					      ...
					      uint32_t *end_of_event_flag)
{
	if (wmi_handle->ops->extract_halphy_stats_end_of_event)
		return wmi_handle->ops->extract_halphy_stats_end_of_event(
			wmi_handle, evt_buf, end_of_event_flag);
					     ...
					     uint32_t *event_count_flag)
{
	if (wmi_handle->ops->extract_halphy_stats_event_count)
		return wmi_handle->ops->extract_halphy_stats_event_count(
			wmi_handle, evt_buf, event_count_flag);
...
QDF_STATUS
wmi_unified_send_set_tpc_power_cmd(wmi_unified_t wmi_handle, uint8_t vdev_id,
			 ...
			 struct reg_afc_resp_rx_ind_info *param)
{
	if (wmi_handle->ops->send_afc_cmd)
		return wmi_handle->ops->send_afc_cmd(wmi_handle, pdev_id,
						     param);
...
QDF_STATUS
wmi_extract_dpd_status_ev_param(wmi_unified_t wmi_handle, void *evt_buf,
...
QDF_STATUS
wmi_extract_halphy_cal_status_ev_param(
	wmi_unified_t wmi_handle, void *evt_buf,
...
QDF_STATUS
wmi_extract_halphy_cal_ev_param(wmi_unified_t wmi_handle, void *evt_buf,
...
QDF_STATUS
wmi_extract_quiet_offload_event(struct wmi_unified *wmi_handle, void *evt_buf,
				   ...
				   struct peer_ppe_ds_param *param)
{
	if (wmi_handle->ops->peer_ppe_ds_param_send)
		return wmi_handle->ops->peer_ppe_ds_param_send(wmi_handle,
							       param);
...
QDF_STATUS
wmi_extract_pktlog_decode_info_event(wmi_unified_t wmi_handle, void *evt_buf,
				     ...
				     uint32_t *pktlog_json_version)
{
	if (wmi_handle->ops->extract_pktlog_decode_info_event)
		return wmi_handle->ops->extract_pktlog_decode_info_event(
			wmi_handle, evt_buf, pdev_id, software_image, chip_info,
			pktlog_json_version);
	return QDF_STATUS_E_FAILURE;
}
...
QDF_STATUS
wmi_unified_update_edca_pifs_param(wmi_unified_t wmi_handle,
