...
#define WINDOW_CONFIGURATION_VALUE_6432                                     \
	((CE_WINDOW_ADDRESS_6432 << 6) | (UMAC_WINDOW_ADDRESS_6432 << 12) | \
	 WINDOW_ENABLE_BIT)
 ...
 */
static inline void
hal_rx_proc_phyrx_other_receive_info_tlv_6432(void *rx_tlv_hdr,
					      void *ppdu_info_hdl)
{
...
static inline void hal_rx_get_bb_info_6432(void *rx_tlv, void *ppdu_info_hdl)
{
...
static inline void hal_rx_get_rtt_info_6432(void *rx_tlv, void *ppdu_info_hdl)
{
	struct hal_rx_ppdu_info *ppdu_info = ppdu_info_hdl;
	...
	ppdu_info->cfr_info.rx_start_ts = HAL_RX_GET_64(
		rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS, RX_START_TS);

	ppdu_info->cfr_info.rtt_cfo_measurement = (int16_t)HAL_RX_GET_64(
		rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS,
		RTT_CFO_MEASUREMENT);

	ppdu_info->cfr_info.agc_gain_info0 = HAL_RX_GET_64(
		rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS, GAIN_CHAIN0);

	ppdu_info->cfr_info.agc_gain_info0 |=
		(((uint32_t)HAL_RX_GET_64(
			 rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS,
			 GAIN_CHAIN1))
		 << 16);

	ppdu_info->cfr_info.agc_gain_info1 = HAL_RX_GET_64(
		rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS, GAIN_CHAIN2);
	...
	ppdu_info->cfr_info.mcs_rate = HAL_RX_GET_64(
		rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS, RTT_MCS_RATE);

	ppdu_info->cfr_info.gi_type = HAL_RX_GET_64(
		rx_tlv, PHYRX_LOCATION_RX_LOCATION_INFO_DETAILS, RTT_GI_TYPE);
}
#endif
						   ...
						   uint8_t dbg_level)
{
	...
	QDF_TRACE(QDF_MODULE_ID_HAL, dbg_level,
		  ...
		  mpdu_info->pn_31_0, mpdu_info->pn_63_32, mpdu_info->pn_95_64);
	...
	QDF_TRACE(QDF_MODULE_ID_HAL, dbg_level,
		  ...
		  mpdu_info->ast_index, mpdu_info->sw_peer_id,
	...
	QDF_TRACE(QDF_MODULE_ID_HAL, dbg_level,
		  ...
		  mpdu_info->more_fragment_flag, mpdu_info->fr_ds,
		  mpdu_info->to_ds, mpdu_info->encrypted, mpdu_info->mpdu_retry,
 ...
 * Return: void
 */
static void hal_rx_dump_msdu_end_tlv_6432(void *msduend, uint8_t dbg_level)
{
	...
	QDF_TRACE(QDF_MODULE_ID_DP, dbg_level,
		  ...
		  "cce_metadata: %d "
		  "sa_sw_peer_id: %d ",
		  msdu_end->key_id_octet, msdu_end->tcp_udp_chksum,
		  ...
		  msdu_end->da_is_mcbc, msdu_end->tkip_mic_err,
						   ...
						   uint8_t dbg_level)
{
	...
	QDF_TRACE(QDF_MODULE_ID_HAL, dbg_level,
		  ...
		  mpdu_info->receive_queue_number,
		  mpdu_info->pre_delim_err_warning, mpdu_info->first_delim_err,
		  mpdu_info->reserved_2a, mpdu_info->pn_31_0,
		  mpdu_info->pn_63_32, mpdu_info->pn_95_64,
		  mpdu_info->pn_127_96, mpdu_info->epd_en,
		  mpdu_info->all_frames_shall_be_encrypted,
		  mpdu_info->encrypt_type,
		  mpdu_info->wep_key_width_for_variable_key,
		  mpdu_info->mesh_sta, mpdu_info->bssid_hit,
...
static void hal_rx_dump_msdu_end_tlv_6432(void *msduend, uint8_t dbg_level)
{
	...
	QDF_TRACE(QDF_MODULE_ID_DP, dbg_level,
		  ...
		  msdu_end->rule_indication_31_0, msdu_end->tcp_udp_chksum,
		  msdu_end->sa_idx_timeout, msdu_end->da_idx_timeout,
		  msdu_end->msdu_limit_error, msdu_end->flow_idx_timeout,
		  msdu_end->flow_idx_invalid, msdu_end->wifi_parser_error,
		  msdu_end->sa_is_valid, msdu_end->da_is_valid,
 ...
 *
 */
static void hal_reo_status_get_header_6432(hal_ring_desc_t ring_desc, int b,
					   void *h1)
{
...
static void *hal_rx_msdu0_buffer_addr_lsb_6432(void *link_desc_va)
{
	return (void *)HAL_RX_MSDU0_BUFFER_ADDR_LSB(link_desc_va);
}

static void *hal_rx_msdu_desc_info_ptr_get_6432(void *msdu0)
{
	return (void *)HAL_RX_MSDU_DESC_INFO_PTR_GET(msdu0);
}

static void *hal_ent_mpdu_desc_info_6432(void *ent_ring_desc)
{
	return (void *)HAL_ENT_MPDU_DESC_INFO(ent_ring_desc);
}

static void *hal_dst_mpdu_desc_info_6432(void *dst_ring_desc)
{
 ...
 * Return: void
 */
static void hal_reo_config_6432(struct hal_soc *soc, uint32_t reg_val,
...
static void hal_compute_reo_remap_ix2_ix3_6432(uint32_t *ring,
					       uint32_t num_rings,
					       uint32_t *remap1,
					       uint32_t *remap2)
{
 ...
 */
static void *hal_rx_flow_setup_fse_6432(uint8_t *rx_fst, uint32_t table_offset,
					uint8_t *rx_flow)
{
	...
	HAL_SET_FLD(fse, RX_FLOW_SEARCH_ENTRY, SERVICE_CODE) |= HAL_SET_FLD_SM(
		RX_FLOW_SEARCH_ENTRY, SERVICE_CODE, flow->service_code);
 ...
 * Return: None.
 */
static void hal_cmem_write_6432(hal_soc_handle_t hal_soc_hdl, uint32_t offset,
				uint32_t value)
{
...
static void hal_compute_reo_remap_ix0_6432(struct hal_soc *soc)
{
	uint32_t remap0;
	...
	HAL_REG_WRITE(
		soc,
		HWIO_REO_R0_DESTINATION_RING_CTRL_IX_0_ADDR(REO_REG_REG_BASE),
		remap0);
			       ...
			       int qref_reset)
{
	...
	reg_val = HAL_REG_READ(
		soc, HWIO_REO_R0_GENERAL_ENABLE_ADDR(REO_REG_REG_BASE));
...
hal_read_pmm_scratch_reg_6432(struct hal_soc *soc, uint32_t base_addr,
			      enum hal_scratch_reg_enum reg_enum)
{
	uint32_t val = 0;
					     ...
					     uint8_t mac_id, uint64_t *value)
{
	...
	offset_lo = hal_read_pmm_scratch_reg_6432(soc, PMM_REG_BASE_QCN6432,
						  enum_lo);

	offset_hi = hal_read_pmm_scratch_reg_6432(soc, PMM_REG_BASE_QCN6432,
						  enum_hi);
...
static void hal_hw_txrx_ops_attach_qcn6432(struct hal_soc *hal_soc)
{
	...
	hal_soc->ops->hal_reo_shared_qaddr_setup =
		hal_reo_shared_qaddr_setup_be;
	hal_soc->ops->hal_reo_shared_qaddr_init = hal_reo_shared_qaddr_init_be;
	hal_soc->ops->hal_reo_shared_qaddr_detach =
		hal_reo_shared_qaddr_detach_be;
	hal_soc->ops->hal_reo_shared_qaddr_write =
		hal_reo_shared_qaddr_write_be;
	hal_soc->ops->hal_reo_shared_qaddr_cache_clear =
		hal_reo_shared_qaddr_cache_clear_be;
#endif
	...
	hal_soc->ops->hal_txmon_get_word_mask = hal_txmon_get_word_mask_qcn6432;
#else
	...
	hal_soc->ops->hal_rx_msdu_end_wmask_get = hal_rx_msdu_end_wmask_get_be;
#endif
	hal_soc->ops->hal_get_tsf2_scratch_reg =
		hal_get_tsf2_scratch_reg_qcn6432;
	hal_soc->ops->hal_get_tqm_scratch_reg = hal_get_tqm_scratch_reg_qcn6432;
...
struct hal_hw_srng_config hw_srng_table_6432[] = {
    ...
    },
    {
         ...
         * type are supported
         */
        .reg_size = {HWIO_REO_R0_SW2REO1_RING_BASE_LSB_ADDR(0) -
...
#endif
    {
        ...
        .reg_start =
            {
                HWIO_REO_R0_REO2PPE_RING_BASE_LSB_ADDR(REO_REG_REG_BASE),
                HWIO_REO_R2_REO2PPE_RING_HP_ADDR(REO_REG_REG_BASE),
            },
        /* Single ring - provide ring size if multiple rings of this
         * type are supported
         */
        .reg_size = {},
        .max_size = HWIO_REO_R0_REO2PPE_RING_BASE_LSB_RING_BASE_ADDR_LSB_BMSK >>
    ...
    },
    {
        ...
        .reg_start =
            {
                HWIO_TCL_R0_PPE2TCL1_RING_BASE_LSB_ADDR(MAC_TCL_REG_REG_BASE),
                HWIO_TCL_R2_PPE2TCL1_RING_HP_ADDR(MAC_TCL_REG_REG_BASE),
            },
        .reg_size = {},
        .max_size = HWIO_TCL_R0_SW2TCL1_RING_BASE_MSB_RING_SIZE_BMSK >>
                    HWIO_TCL_R0_SW2TCL1_RING_BASE_MSB_RING_SIZE_SHFT,
 ...
 */
static inline void
hal_reo_config_reo2ppe_dest_info_6432(hal_soc_handle_t hal_soc_hdl)
{
