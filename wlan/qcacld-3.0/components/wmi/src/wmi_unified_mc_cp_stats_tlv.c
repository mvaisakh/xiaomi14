 ...
 */
static QDF_STATUS extract_per_chain_rssi_stats_tlv(
	...
	struct wmi_host_per_chain_rssi_stats *rssi_stats)
{
	...
	qdf_mem_copy(rssi_stats->rssi_avg_data, fw_rssi_stats->rssi_avg_data,
		     sizeof(fw_rssi_stats->rssi_avg_data));
	qdf_mem_copy(&rssi_stats->peer_macaddr, &fw_rssi_stats->peer_macaddr,
 ...
 */
static QDF_STATUS
extract_peer_adv_stats_tlv(wmi_unified_t wmi_handle, void *evt_buf,
 ...
 * Return: QDF_STATUS_SUCCESS for success or error code
 */
static QDF_STATUS extract_mib_stats_tlv(wmi_unified_t wmi_handle, void *evt_buf,
					struct mib_stats_metrics *mib_stats)
{
	...
	mib_stats->mib_counters.tx_frags = ev->tx_mpdu_grp_frag_cnt;
	mib_stats->mib_counters.group_tx_frames = ev->tx_msdu_grp_frm_cnt;
	mib_stats->mib_counters.failed_cnt = ev->tx_msdu_fail_cnt;
	mib_stats->mib_counters.rx_frags = ev->rx_mpdu_frag_cnt;
	mib_stats->mib_counters.group_rx_frames = ev->rx_msdu_grp_frm_cnt;
	mib_stats->mib_counters.fcs_error_cnt = ev->rx_mpdu_fcs_err;
	mib_stats->mib_counters.tx_frames = ev->tx_msdu_frm_cnt;
	mib_stats->mib_mac_statistics.retry_cnt = ev->tx_msdu_retry_cnt;
	mib_stats->mib_mac_statistics.frame_dup_cnt = ev->rx_frm_dup_cnt;
	mib_stats->mib_mac_statistics.rts_success_cnt = ev->tx_rts_success_cnt;
	mib_stats->mib_mac_statistics.rts_fail_cnt = ev->tx_rts_fail_cnt;

	mib_stats->mib_qos_counters.qos_tx_frag_cnt =
		ev->tx_Qos_mpdu_grp_frag_cnt;
	mib_stats->mib_qos_counters.qos_retry_cnt = ev->tx_Qos_msdu_retry_UP;
	mib_stats->mib_qos_counters.qos_failed_cnt = ev->tx_Qos_msdu_fail_UP;
	...
	mib_stats->mib_qos_counters.qos_mpdu_rx_cnt = ev->rx_Qos_mpdu_cnt;
	mib_stats->mib_qos_counters.qos_retries_rx_cnt =
		ev->rx_Qos_mpdu_retryBit_cnt;

	mib_stats->mib_rsna_stats.tkip_icv_err = ev->rsna_TKIP_icv_err_cnt;
	mib_stats->mib_rsna_stats.tkip_replays = ev->rsna_TKIP_replay_err_cnt;
	mib_stats->mib_rsna_stats.ccmp_decrypt_err =
		ev->rsna_CCMP_decrypt_err_cnt;
	mib_stats->mib_rsna_stats.ccmp_replays = ev->rsna_CCMP_replay_err_cnt;

	mib_stats->mib_counters_group3.tx_ampdu_cnt = ev->tx_ampdu_cnt;
	...
	mib_stats->mib_counters_group3.ampdu_rx_cnt = ev->rx_ampdu_cnt;
				     ...
				     struct peer_stats_request_params *param)
{
	...
	wmi_debug("PEER STATS REQ VDEV_ID:%d PEER:" QDF_MAC_ADDR_FMT
		  " TYPE:%d RESET:%d",
		  cmd->vdev_id, QDF_MAC_ADDR_REF(param->peer_mac_addr),
		  cmd->request_type, cmd->reset_after_request);
			     ...
			     wmi_host_stats_event *stats_param)
{
	...
	if (!param_buf->num_peer_stats_info ||
		...
		wmi_err_rl(
			"actual num of peers stats info: %d is less than provided peers: %d",
			param_buf->num_peer_stats_info, ev_param->num_peers);
...
static void dump_peer_stats_info(wmi_peer_stats_info *stats)
{
	...
	wmi_debug("tx_bytes %d %d tx_packets %d %d tx_retries %d tx_failed %d",
		  stats->tx_bytes.low_32, stats->tx_bytes.high_32,
		  stats->tx_packets.low_32, stats->tx_packets.high_32,
		  stats->tx_retries, stats->tx_failed);
	wmi_debug("rx_bytes %d %d rx_packets %d %d", stats->rx_bytes.low_32,
		  stats->rx_bytes.high_32, stats->rx_packets.low_32,
		  stats->rx_packets.high_32);
	wmi_debug(
		"tx_rate_code %x rx_rate_code %x tx_rate %x rx_rate %x peer_rssi "
		"%d tx_succeed %d",
 ...
 * Return: QDF_STATUS_SUCCESS for success or error code
 */
static QDF_STATUS extract_big_data_stats_tlv(wmi_unified_t wmi_handle,
					     void *evt_buf,
...
#ifdef WLAN_FEATURE_BIG_DATA_STATS
static void wmi_attach_big_data_stats_handler(struct wmi_ops *ops)
{
