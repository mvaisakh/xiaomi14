				  ...
				  struct pmo_ns_req *ns_req)
{
     ...
     */
		...
		request->self_ipv6_addr[i][13] = ns_req->ipv6_addr[i][13];
		request->self_ipv6_addr[i][14] = ns_req->ipv6_addr[i][14];
		request->self_ipv6_addr[i][15] = ns_req->ipv6_addr[i][15];
		request->slot_idx = i;
		qdf_mem_copy(&request->target_ipv6_addr[i],
			     &ns_req->ipv6_addr[i][0], QDF_IPV6_ADDR_SIZE);

		request->target_ipv6_addr_valid[i] = PMO_IPV6_ADDR_VALID;
...
static QDF_STATUS pmo_core_cache_ns_in_vdev_priv(struct pmo_ns_req *ns_req,
...
static QDF_STATUS
pmo_core_flush_ns_from_vdev_priv(struct wlan_objmgr_vdev *vdev)
{
	struct pmo_vdev_priv_obj *vdev_ctx;
...
static QDF_STATUS
pmo_core_do_enable_ns_offload(struct wlan_objmgr_vdev *vdev, uint8_t vdev_id,
...
static QDF_STATUS
pmo_core_do_disable_ns_offload(struct wlan_objmgr_vdev *vdev, uint8_t vdev_id,
			       enum pmo_offload_trigger trigger)
{
	QDF_STATUS status = QDF_STATUS_SUCCESS;
	struct pmo_psoc_priv_obj *psoc_ctx;
...
QDF_STATUS pmo_core_cache_ns_offload_req(struct pmo_ns_req *ns_req)
{
	...
	vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
		ns_req->psoc, ns_req->vdev_id, WLAN_PMO_ID);
	if (!vdev) {
		pmo_err("vdev is NULL");
		status = QDF_STATUS_E_INVAL;
		goto out;
...
QDF_STATUS
pmo_core_disable_ns_offload_in_fwr(struct wlan_objmgr_vdev *vdev,
				   enum pmo_offload_trigger trigger)
{
	QDF_STATUS status;
	uint8_t vdev_id;
