...
QDF_STATUS pmo_allocate_ctx(void)
{
	...
	gp_pmo_ctx = (struct wlan_pmo_ctx *)qdf_mem_malloc(sizeof(*gp_pmo_ctx));
	if (!gp_pmo_ctx)
...
{
	psoc_cfg->extwow_goto_suspend = cfg_get(psoc, CFG_EXTWOW_GOTO_SUSPEND);
					...
					struct pmo_psoc_cfg *psoc_cfg)
{
	...
	psoc_cfg->wow_pulse_interval_low = cfg_get(psoc, CFG_PMO_WOW_PULSE_LOW);
	psoc_cfg->wow_pulse_repeat_count =
		cfg_get(psoc, CFG_PMO_WOW_PULSE_REPEAT);
	psoc_cfg->wow_pulse_init_state = cfg_get(psoc, CFG_PMO_WOW_PULSE_INIT);
}
#else
static void wlan_pmo_wow_pulse_init_cfg(struct wlan_objmgr_psoc *psoc,
					struct pmo_psoc_cfg *psoc_cfg)
					   ...
					   struct pmo_psoc_cfg *psoc_cfg)
{
	...
	if (QDF_IS_STATUS_SUCCESS(
		    ucfg_fwol_get_is_rate_limit_enabled(psoc, &ra_enable)))
					  ...
					  struct pmo_psoc_cfg *psoc_cfg)
{
	...
	psoc_cfg->gpio_wakeup_pin = cfg_get(psoc, CFG_PMO_GPIO_WAKEUP_PIN);
	psoc_cfg->gpio_wakeup_mode = cfg_get(psoc, CFG_PMO_GPIO_WAKEUP_MODE);
}
#else
static void wlan_pmo_gpio_wakeup_init_cfg(struct wlan_objmgr_psoc *psoc,
					  struct pmo_psoc_cfg *psoc_cfg)
...
static void wlan_pmo_get_igmp_version_support_cfg(struct wlan_objmgr_psoc *psoc,
						  struct pmo_psoc_cfg *psoc_cfg)
{
...
static void wlan_pmo_get_igmp_offload_enable_cfg(struct wlan_objmgr_psoc *psoc,
						 struct pmo_psoc_cfg *psoc_cfg)
{
	psoc_cfg->igmp_offload_enable =
		cfg_get(psoc, CFG_PMO_ENABLE_IGMP_OFFLOAD);
}
#else
static void wlan_pmo_get_igmp_version_support_cfg(struct wlan_objmgr_psoc *psoc,
						  struct pmo_psoc_cfg *psoc_cfg)
...
static void wlan_pmo_get_icmp_offload_enable_cfg(struct wlan_objmgr_psoc *psoc,
						 struct pmo_psoc_cfg *psoc_cfg)
{
			      ...
			      struct pmo_psoc_cfg *psoc_cfg)
{
	...
	psoc_cfg->active_uc_apf_mode = cfg_get(psoc, CFG_ACTIVE_UC_APF_MODE);
	...
	psoc_cfg->enable_ssr_on_page_fault =
		cfg_get(psoc, CFG_ENABLE_SSR_ON_PAGEFAULT);
	psoc_cfg->max_pagefault_wakeups_for_ssr =
		cfg_get(psoc, CFG_MAX_PAGEFAULT_WAKEUPS_FOR_SSR);
	psoc_cfg->interval_for_pagefault_wakeup_counts =
		cfg_get(psoc, CFG_INTERVAL_FOR_PAGEFAULT_WAKEUP_COUNT);
...
QDF_STATUS
pmo_get_vdev_bss_peer_mac_addr(struct wlan_objmgr_vdev *vdev,
					 ...
					 enum QDF_OPMODE vdev_opmode)
{
	...
	if ((vdev_opmode == QDF_SAP_MODE || vdev_opmode == QDF_P2P_GO_MODE) &&
...
void pmo_core_psoc_set_hif_handle(struct wlan_objmgr_psoc *psoc, void *hif_hdl)
{
	struct pmo_psoc_priv_obj *psoc_ctx;
