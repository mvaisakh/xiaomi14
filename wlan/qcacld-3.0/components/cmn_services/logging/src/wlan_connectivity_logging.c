...
static void wlan_connectivity_logging_register_callbacks(
	struct wlan_cl_osif_cbks *osif_cbks, void *osif_cb_context)
{
...
{
	global_cl.head =
		qdf_mem_valloc(sizeof(*global_cl.head) * WLAN_MAX_LOG_RECORDS);
	if (!global_cl.head) {
		QDF_BUG(0);
		return;
	}
					   ...
					   uint8_t vdev_id)
{
   ...
   */
	for (i = 0; i < MAX_ROAM_CANDIDATE_AP; i++) {
		...
		if (qdf_is_macaddr_equal(bssid, (struct qdf_mac_addr *)mlme_priv
							->auth_log[i][0]
							.diag_cmn.bssid))
			break;
	}
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS wlan_add_sae_log_record_to_available_slot(
	...
	struct wlan_diag_packet_info *pkt_info)
{
	...
	bool is_entry_exist = wlan_is_log_record_present_for_bssid(
		psoc, (struct qdf_mac_addr *)pkt_info->diag_cmn.bssid, vdev_id);
	...
	for (i = 0; i < MAX_ROAM_CANDIDATE_AP; i++) {
		if (is_entry_exist &&
		    ...
		    qdf_is_macaddr_equal(
			    (struct qdf_mac_addr *)pkt_info->diag_cmn.bssid,
			    (struct qdf_mac_addr *)mlme_priv->auth_log[i][0]
				    .diag_cmn.bssid)) {
			/*
...
static void wlan_cache_connectivity_log(struct wlan_objmgr_psoc *psoc,
					uint8_t vdev_id,
					struct wlan_diag_packet_info *pkt_info)
{
	struct wlan_objmgr_vdev *vdev;
...
void wlan_connectivity_mgmt_event(struct wlan_objmgr_psoc *psoc,
				  struct wlan_frame_hdr *mac_hdr,
				  uint8_t vdev_id, uint16_t status_code,
				  enum qdf_dp_tx_rx_status tx_status,
				  int8_t peer_rssi, uint8_t auth_algo,
				  uint8_t auth_type, uint8_t auth_seq,
				  uint16_t aid, enum wlan_main_tag tag)
{
	...
	is_auth_frame_caching_required = wlan_psoc_nif_fw_ext2_cap_get(
