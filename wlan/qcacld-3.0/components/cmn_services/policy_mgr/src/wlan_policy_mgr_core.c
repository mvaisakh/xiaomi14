...
policy_mgr_get_context(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	pm_ctx = (struct policy_mgr_psoc_priv_obj *)
		wlan_objmgr_psoc_get_comp_private_obj(
			psoc, WLAN_UMAC_COMP_POLICY_MGR);
	return pm_ctx;
}

QDF_STATUS policy_mgr_get_updated_scan_config(struct wlan_objmgr_psoc *psoc,
					      ...
					      bool single_mac_scan_with_dfs)
{
	...
	*scan_config = pm_ctx->dual_mac_cfg.cur_scan_config;
	...
	WMI_DBS_CONC_SCAN_CFG_DBS_SCAN_SET(
		*scan_config, dbs_scan & WMI_DBS_CONC_SCAN_CFG_DBS_SCAN_GET(
						 conc_scan_config_bits));
	WMI_DBS_CONC_SCAN_CFG_AGILE_SCAN_SET(
		*scan_config,
		dbs_plus_agile_scan & WMI_DBS_CONC_SCAN_CFG_AGILE_SCAN_GET(
					      conc_scan_config_bits));
	WMI_DBS_CONC_SCAN_CFG_AGILE_DFS_SCAN_SET(
		*scan_config, single_mac_scan_with_dfs &
...
QDF_STATUS policy_mgr_get_updated_fw_mode_config(struct wlan_objmgr_psoc *psoc,
						 uint32_t *fw_mode_config,
						 bool dbs, bool agile_dfs)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
bool policy_mgr_is_dual_mac_disabled_in_ini(struct wlan_objmgr_psoc *psoc)
{
...
uint32_t policy_mgr_get_mcc_to_scc_switch_mode(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
int8_t policy_mgr_get_matching_hw_mode_index(
	struct wlan_objmgr_psoc *psoc, uint32_t mac0_tx_ss, uint32_t mac0_rx_ss,
	enum hw_mode_bandwidth mac0_bw, uint32_t mac1_tx_ss,
	uint32_t mac1_rx_ss, enum hw_mode_bandwidth mac1_bw,
	enum hw_mode_mac_band_cap mac0_band_cap, enum hw_mode_dbs_capab dbs,
	enum hw_mode_agile_dfs_capab dfs, enum hw_mode_sbs_capab sbs)
{
	uint32_t i;
...
int8_t policy_mgr_get_hw_mode_idx_from_dbs_hw_list(
	struct wlan_objmgr_psoc *psoc, enum hw_mode_ss_config mac0_ss,
	enum hw_mode_bandwidth mac0_bw, enum hw_mode_ss_config mac1_ss,
	enum hw_mode_bandwidth mac1_bw, enum hw_mode_mac_band_cap mac0_band_cap,
	enum hw_mode_dbs_capab dbs, enum hw_mode_agile_dfs_capab dfs,
	enum hw_mode_sbs_capab sbs)
{
	...
	policy_mgr_debug("MAC0: TxSS=%d, RxSS=%d, BW=%d band=%d", mac0_tx_ss,
			 mac0_rx_ss, mac0_bw, mac0_band_cap);
	policy_mgr_debug("MAC1: TxSS=%d, RxSS=%d, BW=%d", mac1_tx_ss,
			 mac1_rx_ss, mac1_bw);
	policy_mgr_debug("DBS=%d, Agile DFS=%d, SBS=%d", dbs, dfs, sbs);

	return policy_mgr_get_matching_hw_mode_index(
		psoc, mac0_tx_ss, mac0_rx_ss, mac0_bw, mac1_tx_ss, mac1_rx_ss,
		mac1_bw, mac0_band_cap, dbs, dfs, sbs);
}

QDF_STATUS
policy_mgr_get_hw_mode_from_idx(struct wlan_objmgr_psoc *psoc, uint32_t idx,
				struct policy_mgr_hw_mode_params *hw_mode)
{
	...
	if (hw_mode->dbs_cap) {
		...
		if (hw_mode->mac0_band_cap == WLAN_5G_CAPABILITY &&
		    mac0_min_ss && mac1_min_ss && mac0_min_ss > mac1_min_ss)
...
QDF_STATUS policy_mgr_get_old_and_new_hw_index(struct wlan_objmgr_psoc *psoc,
...
void policy_mgr_update_conc_list(
	struct wlan_objmgr_psoc *psoc, uint32_t conn_index,
	enum policy_mgr_con_mode mode, uint32_t ch_freq,
	enum hw_mode_bandwidth bw, uint8_t mac,
	enum policy_mgr_chain_mode chain_mask, uint32_t original_nss,
	uint32_t vdev_id, bool in_use, bool update_conn, uint16_t ch_flagext)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	bool mcc_mode;
	...
	*num_cxn_del = 0;
	...
	while (PM_CONC_CONNECTION_LIST_VALID_INDEX(conn_index)) {
       ...
       */
			...
			policy_mgr_decr_connection_count(
				psoc, info[found_index].vdev_id);
			policy_mgr_debug(
				"Stored %d (%d), deleted STA entry with vdev id %d, index %d",
	...
	if (!found_index) {
		...
		*num_cxn_del = found_index;
		policy_mgr_debug("Mode:%d number of conn %d temp del", mode,
				 *num_cxn_del);
	}
   ...
   */
	if (*num_cxn_del == 1) {
		*info = pm_conc_connection_list[conn_index];
		...
		/* Deleting the connection entry */
		policy_mgr_decr_connection_count(
			psoc, pm_conc_connection_list[conn_index].vdev_id);
	}
	...
	struct policy_mgr_conc_connection_info *info, uint8_t *num_cxn_del)
{
	...
	*num_cxn_del = 0;
	...
	while (PM_CONC_CONNECTION_LIST_VALID_INDEX(conn_index)) {
		...
		info[found_index] = pm_conc_connection_list[conn_index];
		policy_mgr_debug(
			"Stored %d (%d), deleted STA entry with vdev id %d, index %d ch %d",
			info[found_index].vdev_id, info[found_index].mode,
			info[found_index].vdev_id, conn_index, ch_freq);
		found_index++;
		conn_index++;
	}
	conn_index = 0;
 ...
 * Return: None
 */
void policy_mgr_restore_deleted_conn_info(
	...
	struct policy_mgr_conc_connection_info *info, uint8_t num_cxn_del)
{
	...
	if (conn_index + num_cxn_del > MAX_NUMBER_OF_CONC_CONNECTIONS) {
		qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
		policy_mgr_err(
			"Failed to restore the deleted information %d/%d, as it exceed max %d",
	...
	for (i = 0; i < num_cxn_del; i++)
		policy_mgr_debug(
			"Restored the deleleted conn info, vdev:%d, index:%d",
			info[i].vdev_id, conn_index++);
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
}

static bool policy_mgr_is_freq_range_5_6ghz(qdf_freq_t start_freq,
					    qdf_freq_t end_freq)
{
	/*
...
static bool policy_mgr_is_freq_range_2ghz(qdf_freq_t start_freq,
					  qdf_freq_t end_freq)
{
	/*
   * As Fw is sending the whole hardware range which include 4.9Ghz as
				   ...
				   struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	pm_ctx->hw_mode.cur_mac_freq_range[mac_id].low_2ghz_freq =
		QDF_MAX(freq->start_freq, wlan_reg_min_24ghz_chan_freq());
	pm_ctx->hw_mode.cur_mac_freq_range[mac_id].high_2ghz_freq =
		QDF_MIN(freq->end_freq, wlan_reg_max_24ghz_chan_freq());
}
				   ...
				   struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	...
	pm_ctx->hw_mode.cur_mac_freq_range[mac_id].low_5ghz_freq =
		QDF_MAX(freq->start_freq, wlan_reg_min_5ghz_chan_freq());
	pm_ctx->hw_mode.cur_mac_freq_range[mac_id].high_5ghz_freq =
		QDF_MIN(freq->end_freq, max_5g_freq);
}

void policy_mgr_fill_curr_mac_freq_by_hwmode(
	struct policy_mgr_psoc_priv_obj *pm_ctx, enum policy_mgr_mode mode_hw)
{
	uint8_t i;
...
static QDF_STATUS policy_mgr_fill_curr_freq_by_pdev_freq(
	int32_t num_mac_freq, struct policy_mgr_pdev_mac_freq_map *freq,
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	struct policy_mgr_hw_mode_params hw_mode)
{
	uint32_t mac_id, i;
	...
	for (i = 0; i < num_mac_freq; i++) {
		...
		policy_mgr_debug("mac_id %d start freq %d end_freq %d", mac_id,
				 freq[i].start_freq, freq[i].end_freq);

		if (policy_mgr_is_freq_range_2ghz(freq[i].start_freq,
						  freq[i].end_freq)) {
			policy_mgr_fill_curr_mac_2ghz_freq(mac_id, &freq[i],
							   pm_ctx);
			...
			policy_mgr_err(
				"Invalid different band freq range: mac_id %d start freq "
				"%d end_freq %d",
				mac_id, freq[i].start_freq, freq[i].end_freq);
			return QDF_STATUS_E_INVAL;
		}
	}
				...
				struct policy_mgr_hw_mode_params hw_mode)
{
	...
	if (num_mac_freq && freq) {
		status = policy_mgr_fill_curr_freq_by_pdev_freq(
			num_mac_freq, freq, pm_ctx, hw_mode);
 ...
 * Return: None
 */
void policy_mgr_update_hw_mode_conn_info(
	struct wlan_objmgr_psoc *psoc, uint32_t num_vdev_mac_entries,
	struct policy_mgr_vdev_mac_map *vdev_mac_map,
	struct policy_mgr_hw_mode_params hw_mode, uint32_t num_mac_freq,
...
void policy_mgr_pdev_set_hw_mode_cb(
	uint32_t status, uint32_t cfgd_hw_mode_index,
	uint32_t num_vdev_mac_entries,
	struct policy_mgr_vdev_mac_map *vdev_mac_map, uint8_t next_action,
	enum policy_mgr_conn_update_reason reason, uint32_t session_id,
	void *context, uint32_t request_id)
{
	...
	policy_mgr_set_hw_mode_change_in_progress(
		context, POLICY_MGR_HW_MODE_NOT_IN_PROGRESS);
	...
	if (reason != POLICY_MGR_UPDATE_REASON_NAN_DISCOVERY && !vdev_mac_map) {
	...
	policy_mgr_debug("MAC1: TxSS:%d, RxSS:%d, Bw:%d", hw_mode.mac1_tx_ss,
			 hw_mode.mac1_rx_ss, hw_mode.mac1_bw);
	policy_mgr_debug("DBS:%d, Agile DFS:%d, SBS:%d", hw_mode.dbs_cap,
...
next_action:
	if (PM_NOP != next_action && (status == SET_HW_MODE_STATUS_ALREADY ||
		...
		policy_mgr_next_actions(context, session_id, next_action,
					reason, request_id);
	else
...
static char *ml_sta_prefix(struct wlan_objmgr_psoc *psoc, uint32_t vdev_id)
{
...
static void policy_mgr_dump_ml_sta_conc(struct wlan_objmgr_psoc *psoc,
					uint8_t *num_mlo_sta)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	if (policy_mgr_is_mlo_in_mode_dbs(psoc, PM_STA_MODE, NULL, num_mlo_sta))
	...
	struct wlan_objmgr_psoc *psoc, char *cc_mode, uint32_t length)
{
	...
	case PM_STA_MODE:
		count = strlcat(
			cc_mode,
			ml_sta_prefix(psoc, pm_conc_connection_list[0].vdev_id),
			length);
		break;
	case PM_SAP_MODE:
	...
	struct wlan_objmgr_psoc *psoc, char *cc_mode, uint32_t length)
{
	...
	case PM_STA_MODE:
		...
		count += strlcat(
			cc_mode,
			ml_sta_prefix(psoc, pm_conc_connection_list[1].vdev_id),
			length);
		break;
	case PM_SAP_MODE:
		count = policy_mgr_dump_current_concurrency_one_connection(
	...
	struct wlan_objmgr_psoc *psoc, char *cc_mode, uint32_t length)
{
	...
	case PM_STA_MODE:
		...
		count += strlcat(
			cc_mode,
			ml_sta_prefix(psoc, pm_conc_connection_list[2].vdev_id),
			length);
		break;
	case PM_SAP_MODE:
		count = policy_mgr_dump_current_concurrency_two_connection(
...
#ifdef WLAN_FEATURE_11BE_MLO
void policy_mgr_dump_disabled_ml_links(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	...
	for (i = 0; i < MAX_NUMBER_OF_DISABLE_LINK; i++) {
		if (pm_disabled_ml_links[i].in_use) {
			len += qdf_scnprintf(
				buf + len, POLICY_MGR_MAX_CON_STRING_LEN - len,
 ...
 */
static uint32_t
policy_mgr_dump_current_concurrency_4_connection(struct wlan_objmgr_psoc *psoc,
						 char *cc_mode, uint32_t length)
{
	uint32_t count = 0;
	enum policy_mgr_con_mode mode;
	...
	case PM_STA_MODE:
		...
		count += strlcat(
			cc_mode,
			ml_sta_prefix(psoc, pm_conc_connection_list[3].vdev_id),
			length);
		break;
	case PM_SAP_MODE:
		count = policy_mgr_dump_current_concurrency_three_connection(
				      ...
				      uint32_t len)
{
	...
	count = policy_mgr_dump_current_concurrency_4_connection(pm_ctx->psoc,
								 cc_mode, len);
...
void policy_mgr_dump_current_concurrency(struct wlan_objmgr_psoc *psoc)
{
	...
	case 3:
		...
		if (pm_conc_connection_list[0].freq ==
			...
			qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
			strlcat(cc_mode, " SCC", len);
		} else if (policy_mgr_are_3_freq_on_same_mac(
				   psoc, pm_conc_connection_list[0].freq,
				   pm_conc_connection_list[1].freq,
				   pm_conc_connection_list[2].freq)) {
			qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
		...
		break;
	case 4:
		if (policy_mgr_handle_dump_4th_connection(
					   ...
					   uint8_t vdev_id, bool clear_pcl)
{
   ...
   */
	...
	policy_mgr_debug(
		"dual_sta_roam:%d, sta concurrency on different mac:%d, clear_pcl:%d",
		dual_sta_roam_enabled, sta_concurrency_is_with_different_mac,
		clear_pcl);
	...
	if (dual_sta_roam_enabled) {
       ...
       */
			policy_mgr_set_pcl(psoc, &msg, roam_enabled_vdev_id,
					   true);
			wlan_cm_roam_activate_pcl_per_vdev(
				psoc, roam_enabled_vdev_id, false);
		} else if (sta_concurrency_is_with_different_mac) {
			wlan_cm_roam_activate_pcl_per_vdev(
				psoc, roam_enabled_vdev_id, true);
		}
						...
						uint8_t vdev_id)
{
	...
	is_sbs_capable = policy_mgr_is_hw_sbs_capable(psoc);
   ...
   */
	...
	policy_mgr_debug(
		"connected STA vdev_id:%d, pcl_per_vdev:%d, dual_sta_roam_active:%d",
		vdev_id, is_pcl_per_vdev, dual_sta_roam_active);
...
QDF_STATUS policy_mgr_set_pcl(struct wlan_objmgr_psoc *psoc,
			      struct policy_mgr_pcl_list *msg, uint8_t vdev_id,
			      bool clear_vdev_pcl)
{
...
static uint32_t pm_get_vdev_id_of_first_conn_idx(struct wlan_objmgr_psoc *psoc)
{
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		if (pm_conc_connection_list[conn_index].in_use) {
			...
			policy_mgr_debug(
				"Use vdev_id:%d for opportunistic upgrade",
				vdev_id);
			break;
		}
	}
	...
	if (conn_index == MAX_NUMBER_OF_CONC_CONNECTIONS) {
		...
		policy_mgr_debug(
			"Use default vdev_id:%d for opportunistic upgrade",
			vdev_id);
	}
...
void pm_dbs_opportunistic_timer_handler(void *data)
{
	...
	policy_mgr_next_actions(psoc, session_id, action, reason,
 ...
 */
enum policy_mgr_con_mode policy_mgr_get_mode(uint8_t type, uint8_t subtype)
{
...
static bool is_preset_in_chlist(uint32_t chan_freq, const uint32_t *chlist,
				uint32_t chlist_len)
{
	uint32_t i;
...
static void get_sbs_chlist(struct wlan_objmgr_psoc *psoc, uint32_t *sbs_freqs,
			   uint32_t *sbs_num, uint32_t chan_freq,
...
static void get_rest_chlist(uint32_t *rest_freqs, uint32_t *rest_num,
...
static void get_sub_channels(struct wlan_objmgr_psoc *psoc, uint32_t *sbs_freqs,
			     uint32_t *sbs_num, uint32_t *scc_freqs,
			     uint32_t *scc_num, uint32_t *rest_freqs,
			     uint32_t *rest_num, const uint32_t *chlist_5g,
			     uint32_t chlist_5g_len, const uint32_t *chlist_6g,
			     uint32_t chlist_6g_len)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t i = 0;
	...
	while (PM_CONC_CONNECTION_LIST_VALID_INDEX(i)) {
		if (WLAN_REG_IS_6GHZ_CHAN_FREQ(
			...
			if (*scc_num < size_of_scc &&
			    !is_preset_in_chlist(pm_conc_connection_list[i].freq,
						 scc_freqs, *scc_num))
				scc_freqs[(*scc_num)++] =
					pm_conc_connection_list[i].freq;
		}
   ...
   */
	if (policy_mgr_is_hw_sbs_capable(psoc) &&
	    (*scc_num == 1 ||
	     (*scc_num == 2 &&
	      !policy_mgr_are_sbs_chan(psoc, scc_freqs[0], scc_freqs[1]))))
		get_sbs_chlist(psoc, sbs_freqs, sbs_num, scc_freqs[0], chlist1,
			       chlist1_len, chlist2, chlist2_len);
	else
		*sbs_num = 0;

	get_rest_chlist(rest_freqs, rest_num, scc_freqs, *scc_num, sbs_freqs,
			*sbs_num, chlist1, chlist1_len, chlist2, chlist2_len);
}
 ...
 * Return: None
 */
static void add_sbs_chlist_to_pcl(
	struct wlan_objmgr_psoc *psoc, uint32_t *pcl_freqs,
	uint8_t *pcl_weights, uint32_t pcl_sz, uint32_t *index,
	bool skip_6gh_channel, const uint32_t *chlist_5, uint8_t chlist_len_5,
	const uint32_t *chlist_6, uint8_t chlist_len_6,
	enum policy_mgr_pcl_channel_order order, bool *high_5_band_scc_present,
	bool *low_5_band_scc_present)
{
	...
	if (order == POLICY_MGR_PCL_ORDER_SCC_5G_LOW_5G_LOW) {
		...
		for (i = 0;
		     i < chlist_len_6 && *index < pcl_sz && !skip_6gh_channel;
		     i++) {
		...
		if (order ==
			...
			qdf_mutex_acquire(&pm_ctx->qdf_conc_list_lock);
			cl = pm_conc_connection_list;
			while (PM_CONC_CONNECTION_LIST_VALID_INDEX(
				conn_index)) {
...
static void add_chlist_to_pcl(struct wlan_objmgr_pdev *pdev,
			      ...
			      bool skip_6gh_channel)
{
	...
	for (i = 0; i < chlist_len && *index < pcl_sz; i++) {
		if (skip_6gh_channel && WLAN_REG_IS_6GHZ_CHAN_FREQ(chlist[i]))
			continue;
		pcl_freqs[*index] = chlist[i];
		pcl_weights[*index] = weight;
		(*index)++;
	}
	policy_mgr_debug("Add chlist len %d index %d", chlist_len, *index);
}
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS policy_mgr_get_connection_channels(
	struct wlan_objmgr_psoc *psoc, enum policy_mgr_con_mode mode,
	enum policy_mgr_pcl_channel_order order, bool skip_dfs_channel,
	enum policy_mgr_pcl_group_id group_id, uint32_t *pcl_freqs,
	uint8_t *pcl_weights, uint32_t pcl_sz, uint32_t *index)
{
	QDF_STATUS status = QDF_STATUS_SUCCESS;
	uint32_t conn_index = 0;
   ...
   */
	...
	if (POLICY_MGR_PCL_ORDER_NONE == order) {
		while (PM_CONC_CONNECTION_LIST_VALID_INDEX(conn_index)) {
			bool is_6ghz_ch =
				WLAN_REG_IS_6GHZ_CHAN_FREQ(cl[conn_index].freq);
			if (skip_dfs_channel &&
...
void policy_mgr_set_weight_of_disabled_inactive_channels_to_zero(
	struct wlan_objmgr_psoc *psoc, uint32_t *pcl_channels, uint32_t *len,
			 ...
			 const uint32_t *chlist_6g, uint8_t chlist_6g_len)
{
	...
	*index = idx;
	policy_mgr_debug(
		"Add 5g chlist len %d 6g chlist len %d len %d index %d order %d",
 ...
 * Return: None
 */
static void policy_mgr_add_24g_to_pcl(uint32_t *pcl_freqs, uint8_t *pcl_weights,
				      uint32_t pcl_sz, uint32_t *index,
				      uint32_t weight,
				      const uint32_t *chlist_24g,
				      uint8_t chlist_24g_len)
{
				       ...
				       uint32_t *len)
{
	...
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t *channel_list, *channel_list_24, *channel_list_5, *sbs_freqs,
		*channel_list_6, *scc_freqs, *rest_freqs;
	...
	*len = 0;
	...
	scc_freqs = qdf_mem_malloc(MAX_NUMBER_OF_CONC_CONNECTIONS *
				   sizeof(uint32_t));
   ...
   */
	...
	case PM_MCC_CH:
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_NONE, false,
			POLICY_MGR_PCL_GROUP_ID1_ID2, pcl_channels, pcl_weights,
			pcl_sz, len);
		status = QDF_STATUS_SUCCESS;
		break;
	case PM_SCC_CH_24G:
	...
	case PM_24G_MCC_CH:
		...
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_NONE, false,
			POLICY_MGR_PCL_GROUP_ID2_ID3, pcl_channels, pcl_weights,
			pcl_sz, len);
		status = QDF_STATUS_SUCCESS;
		break;
	case PM_5G_SCC_CH:
	case PM_5G_MCC_CH:
		...
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_NONE, false,
			POLICY_MGR_PCL_GROUP_ID3_ID4, pcl_channels, pcl_weights,
			pcl_sz, len);
		status = QDF_STATUS_SUCCESS;
		break;
	case PM_SCC_ON_24_SCC_ON_5:
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_24G_THEN_5G, false,
			POLICY_MGR_PCL_GROUP_ID1_ID2, pcl_channels, pcl_weights,
			pcl_sz, len);
		status = QDF_STATUS_SUCCESS;
		break;
	case PM_SCC_ON_5_SCC_ON_24:
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_5G_THEN_2G, false,
			POLICY_MGR_PCL_GROUP_ID1_ID2, pcl_channels, pcl_weights,
			pcl_sz, len);
		status = QDF_STATUS_SUCCESS;
		break;
	...
	case PM_SCC_ON_24_CH_24G:
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_2G, true,
			POLICY_MGR_PCL_GROUP_ID1_ID2, pcl_channels, pcl_weights,
			pcl_sz, len);
		policy_mgr_add_24g_to_pcl(pcl_channels, pcl_weights, pcl_sz,
					  len, WEIGHT_OF_GROUP3_PCL_CHANNELS,
	...
	case PM_SCC_ON_5_CH_5G:
		policy_mgr_get_connection_channels(
			psoc, mode, POLICY_MGR_PCL_ORDER_5G, false,
			POLICY_MGR_PCL_GROUP_ID1_ID2, pcl_channels, pcl_weights,
			pcl_sz, len);
		policy_mgr_add_5g_to_pcl(psoc, pcl_channels, pcl_weights,
					 pcl_sz, len,
	...
	case PM_24G_SCC_CH_SBS_CH:
		get_sub_channels(psoc, sbs_freqs, &sbs_num, scc_freqs, &scc_num,
				 rest_freqs, &rest_num, channel_list_5,
				 chan_index_5, channel_list_6, chan_index_6);
		add_chlist_to_pcl(pm_ctx->pdev, pcl_channels, pcl_weights,
				  pcl_sz, len, WEIGHT_OF_GROUP1_PCL_CHANNELS,
				  channel_list_24, chan_index_24, false);
		add_chlist_to_pcl(pm_ctx->pdev, pcl_channels, pcl_weights,
				  pcl_sz, len, WEIGHT_OF_GROUP2_PCL_CHANNELS,
				  scc_freqs, scc_num, skip_6ghz_channel);
		add_chlist_to_pcl(pm_ctx->pdev, pcl_channels, pcl_weights,
				  pcl_sz, len, WEIGHT_OF_GROUP3_PCL_CHANNELS,
				  sbs_freqs, sbs_num, skip_6ghz_channel);
		status = QDF_STATUS_SUCCESS;
		break;
	case PM_24G_SCC_CH_SBS_CH_5G:
		...
		add_chlist_to_pcl(pm_ctx->pdev, pcl_channels, pcl_weights,
				  ...
				  sbs_freqs, sbs_num, skip_6ghz_channel);
		add_chlist_to_pcl(pm_ctx->pdev, pcl_channels, pcl_weights,
				  pcl_sz, len, WEIGHT_OF_GROUP4_PCL_CHANNELS,
				  rest_freqs, rest_num, skip_6ghz_channel);
		status = QDF_STATUS_SUCCESS;
		break;
	...
	case PM_SCC_ON_5G_LOW_5G_LOW_PLUS_SHARED_2G:
		add_sbs_chlist_to_pcl(psoc, pcl_channels, pcl_weights, pcl_sz,
				      len, skip_6ghz_channel, channel_list_5,
				      chan_index_5, channel_list_6,
				      chan_index_6,
     ...
     */
		if ((!policy_mgr_2ghz_connection_present(pm_ctx) ||
			...
			add_chlist_to_pcl(pm_ctx->pdev, pcl_channels,
					  pcl_weights, pcl_sz, len,
					  WEIGHT_OF_GROUP3_PCL_CHANNELS,
					  channel_list_24, chan_index_24,
					  false);
		status = QDF_STATUS_SUCCESS;
	...
	case PM_SCC_ON_5G_HIGH_5G_HIGH_SCC_ON_5G_LOW_PLUS_SHARED_2G:
		add_sbs_chlist_to_pcl(
			...
			(pcl == PM_SCC_ON_5G_HIGH_5G_HIGH_PLUS_SHARED_2G) ?
				...
				POLICY_MGR_PCL_ORDER_SCC_5G_HIGH_5G_HIGH_SCC_5G_LOW,
			&high_5_band_scc_present, &low_5_band_scc_present);
		/*
     * If no 2.4 GHZ connection is present and if 2.4 GHZ is shared
	...
	policy_mgr_debug("pcl len %d and weight list sz %d", *len, pcl_sz);

	policy_mgr_set_weight_of_disabled_inactive_channels_to_zero(
		psoc, pcl_channels, len, pcl_weights, pcl_sz);
...
bool policy_mgr_disallow_mcc(struct wlan_objmgr_psoc *psoc, uint32_t ch_freq)
{
	uint32_t index = 0;
	bool match = false;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
 ...
 * Return: True/False
 */
static bool policy_mgr_allow_same_mac_diff_freq(struct wlan_objmgr_psoc *psoc,
						qdf_freq_t ch_freq)
{
	bool allow = true;
 ...
 * Return: True/False
 */
static bool policy_mgr_allow_same_mac_same_freq(struct wlan_objmgr_psoc *psoc,
						qdf_freq_t ch_freq,
						enum policy_mgr_con_mode mode)
{
	bool allow = true;
	...
	if (!policy_mgr_is_hw_dbs_capable(psoc) &&
	    policy_mgr_is_interband_mcc_supported(psoc)) {
		policy_mgr_rl_debug(
			"allow 2 intf SCC + new intf ch %d for legacy hw",
			ch_freq);
     ...
     */
		   ...
		   !policy_mgr_is_3rd_conn_on_same_band_allowed(psoc, mode,
								ch_freq)) {
		policy_mgr_rl_debug(
			"don't allow 3rd home channel on same MAC for sta+multi-AP");
		allow = false;
	}
...
bool policy_mgr_allow_new_home_channel(struct wlan_objmgr_psoc *psoc,
				       enum policy_mgr_con_mode mode,
				       qdf_freq_t ch_freq,
				       uint32_t num_connections, bool is_dfs_ch,
				       uint32_t ext_flags)
{
	bool status = true;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	if (num_connections == 3) {
		status = policy_mgr_allow_4th_new_freq(psoc, ch_freq, mode,
						       ext_flags);
	} else if (num_connections == 2) {
		/* No SCC or MCC combination is allowed with / on DFS channel */
		on_same_mac = policy_mgr_2_freq_always_on_same_mac(
			psoc, pm_conc_connection_list[0].freq,
			pm_conc_connection_list[1].freq);
		if (force_switch_without_dis && is_dfs_ch &&
		    ((pm_conc_connection_list[0].ch_flagext &
		     ...
		     (pm_conc_connection_list[1].ch_flagext &
		      (IEEE80211_CHAN_DFS | IEEE80211_CHAN_DFS_CFREQ2)))) {
			policy_mgr_rl_debug(
				"Existing DFS connection, new 3-port DFS connection is not allowed");
			status = false;
			...
			status = policy_mgr_allow_same_mac_same_freq(
				psoc, ch_freq, mode);
		}
		   ...
		   (pm_conc_connection_list[0].ch_flagext &
		    (IEEE80211_CHAN_DFS | IEEE80211_CHAN_DFS_CFREQ2))) {
		policy_mgr_rl_debug(
			"Existing DFS connection, new 2-port DFS connection is not allowed");
		status = false;
     ...
     */
		if ((pm_conc_connection_list[0].mode != PM_NAN_DISC_MODE) &&
		    (mode != PM_NAN_DISC_MODE))
			status = policy_mgr_2_freq_always_on_same_mac(
				psoc, ch_freq, pm_conc_connection_list[0].freq);
	}
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
...
policy_mgr_get_pref_force_scc_freq(struct wlan_objmgr_psoc *psoc,
				   uint8_t vdev_id, qdf_freq_t *intf_ch_freq,
				   qdf_freq_t sap_ch_freq, uint32_t acs_band,
				   bool allow_6ghz)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	enum policy_mgr_con_mode mode;
   ...
   */
     ...
     */
		if (dbs_ml_sta_present)
			same_mac = policy_mgr_2_freq_same_mac_in_dbs(
				pm_ctx, sap_ch_freq, pcl_freq);
		else if (sbs_ml_sta_present)
			same_mac = policy_mgr_2_freq_same_mac_in_sbs(
				pm_ctx, sap_ch_freq, pcl_freq);
		else
			same_mac = policy_mgr_2_freq_always_on_same_mac(
				psoc, sap_ch_freq, pcl_freq);
	...
	policy_mgr_debug(
		"2ghz_only %d allow_6ghz %d, ml sta SBS:%d DBS:%d, SCC: "
		"same_as_sap %d same_mac %d on_diff_mac %d, NON-SCC: "
		"same_as_sap %d same_mac %d on_diff_mac %d. intf_ch_freq %d",
		...
		non_scc_ch_freq_same_as_sap, non_scc_ch_freq_on_same_mac,
 ...
 * is found.
 */
static QDF_STATUS policy_mgr_handle_sta_sap_fav_channel(
	struct wlan_objmgr_psoc *psoc, struct policy_mgr_psoc_priv_obj *pm_ctx,
	uint8_t vdev_id, qdf_freq_t sap_ch_freq, qdf_freq_t *intf_ch_freq,
	uint32_t acs_band)
{
	...
	if (WLAN_REG_IS_24GHZ_CH_FREQ(*intf_ch_freq) ==
	    WLAN_REG_IS_24GHZ_CH_FREQ(sap_ch_freq))
		return policy_mgr_get_sap_mandatory_channel(
			psoc, sap_ch_freq, intf_ch_freq, vdev_id);
   ...
   */
	...
	if (WLAN_REG_IS_24GHZ_CH_FREQ(sap_ch_freq) && user_config_freq &&
...
void policy_mgr_check_scc_channel(struct wlan_objmgr_psoc *psoc,
				  ...
				  qdf_freq_t sap_ch_freq, uint8_t vdev_id,
				  uint8_t cc_mode)
{
	...
	if (pm_ctx->hdd_cbacks.wlan_get_sap_acs_band) {
		status = pm_ctx->hdd_cbacks.wlan_get_sap_acs_band(psoc, vdev_id,
								  &acs_band);
		if (QDF_IS_STATUS_SUCCESS(status))
	...
	if (sta_count && sta_count < 2 &&
		...
		status = policy_mgr_handle_sta_sap_fav_channel(
			psoc, pm_ctx, vdev_id, sap_ch_freq, intf_ch_freq,
			acs_band);
		if (QDF_IS_STATUS_SUCCESS(status))
			return;
				     ...
				     uint32_t request_id)
{
	...
	if (QDF_STATUS_SUCCESS != tx_status)
		policy_mgr_err("nss update failed(%d) for vdev %d", tx_status,
			       vdev_id);
   ...
   */
	...
	if (PM_NOP != next_action) {
		...
		if (reason == POLICY_MGR_UPDATE_REASON_STA_CONNECT ||
			...
			sme_debug(
				"Continue connect/reassoc on vdev %d request_id %x reason %d",
		...
		if (!QDF_IS_STATUS_SUCCESS(ret))
			policy_mgr_err(
				"ERROR: set opportunistic_update event failed");
	}

	return;
}

QDF_STATUS policy_mgr_nss_update(struct wlan_objmgr_psoc *psoc, uint8_t new_nss,
				 uint8_t next_action, enum policy_mgr_band band,
				 enum policy_mgr_conn_update_reason reason,
				 uint32_t original_vdev_id, uint32_t request_id)
{
	...
	for (index = 0; index < count; index++) {
		...
		qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
		conn_index =
			policy_mgr_get_connection_for_vdev_id(psoc, vdev_id);
		if (MAX_NUMBER_OF_CONC_CONNECTIONS == conn_index) {
		...
		if (original_nss == 2 &&
			...
			status = pm_ctx->sme_cbacks.sme_nss_update_request(
				...
				policy_mgr_nss_update_cb, next_action, psoc,
				reason, original_vdev_id, request_id);
			if (!QDF_IS_STATUS_SUCCESS(status)) {
				policy_mgr_err(
					"sme_nss_update_request() failed for vdev %d",
					vdev_id);
			}
		}
	}
				      ...
				      uint32_t session_id, uint32_t request_id)
{
	...
	if (policy_mgr_is_hw_dbs_capable(psoc) == false) {
		policy_mgr_rl_debug(
			"driver isn't dbs capable, no further action needed");
   ...
   */
	...
	status = policy_mgr_nss_update(psoc, new_nss, next_action,
				       downgrade_band, reason, session_id,
				       request_id);
	if (!QDF_IS_STATUS_SUCCESS(status))
		status = policy_mgr_next_actions(psoc, session_id, next_action,
						 reason, request_id);
...
policy_mgr_get_mode_by_vdev_id(struct wlan_objmgr_psoc *psoc, uint8_t vdev_id)
{
	enum policy_mgr_con_mode mode = PM_MAX_NUM_OF_MODE;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t conn_index;
 ...
 */
QDF_STATUS
policy_mgr_init_connection_update(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
...
policy_mgr_get_current_pref_hw_mode_dbs_2x2(struct wlan_objmgr_psoc *psoc)
{
	...
	policy_mgr_debug(
		"chan[0]:%d chan[1]:%d chan[2]:%d num_connections:%d dbs:%d",
		pm_conc_connection_list[0].freq,
		pm_conc_connection_list[1].freq,
...
enum policy_mgr_conc_next_action
policy_mgr_get_current_pref_hw_mode_dbs_1x1(struct wlan_objmgr_psoc *psoc)
{
	uint32_t num_connections;
	uint8_t band1, band2, band3;
	struct policy_mgr_hw_mode_params hw_mode;
...
enum policy_mgr_conc_next_action
policy_mgr_get_current_pref_hw_mode_dual_dbs(struct wlan_objmgr_psoc *psoc)
{
	enum policy_mgr_conc_next_action next_action;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
uint32_t
policy_mgr_get_sap_mandatory_chan_list_len(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
 ...
 */
static void
policy_mgr_add_sap_mandatory_6ghz_chan(struct wlan_objmgr_psoc *psoc)
{
	...
	for (i = 0; (i < len) && (i < NUM_CHANNELS) &&
		    (pm_ctx->sap_mandatory_channels_len < NUM_CHANNELS);
	     i++) {
					...
					uint32_t org_ch_freq)
{
	if (WLAN_REG_IS_5GHZ_CH_FREQ(org_ch_freq)) {
		...
		policy_mgr_add_sap_mandatory_chan(psoc, org_ch_freq);
	} else if (WLAN_REG_IS_6GHZ_CHAN_FREQ(org_ch_freq)) {
		policy_mgr_init_sap_mandatory_chan_by_band(
			psoc,
			BIT(REG_BAND_2G) | BIT(REG_BAND_5G) | BIT(REG_BAND_6G));
	} else {
		policy_mgr_init_sap_mandatory_chan_by_band(psoc,
							   BIT(REG_BAND_2G));
	}
}
