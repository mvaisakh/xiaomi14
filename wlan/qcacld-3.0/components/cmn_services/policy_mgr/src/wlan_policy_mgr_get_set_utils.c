 ...
 * Return: none
 */
static void policy_mgr_debug_alert(void)
{
	int module_id;
	int qdf_print_idx;
	...
	for (module_id = 0; module_id < QDF_MODULE_ID_MAX; module_id++)
		qdf_print_set_category_verbose(qdf_print_idx, module_id,
...
policy_mgr_set_multi_sap_allowed_on_same_band(
	struct wlan_objmgr_psoc *psoc, bool multi_sap_allowed_on_same_band)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
					     ...
					     uint8_t vdev_id)
{
	...
	if (!enable)
		policy_mgr_debug(
			"sta_sap_scc_on_dfs_chnl %d sta_on_2g %d sta_on_5g %d enable %d",
...
bool policy_mgr_get_dfs_master_dynamic_enabled(struct wlan_objmgr_psoc *psoc,
					       uint8_t vdev_id)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
bool policy_mgr_get_can_skip_radar_event(struct wlan_objmgr_psoc *psoc,
					 uint8_t vdev_id)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
QDF_STATUS
policy_mgr_set_dynamic_mcc_adaptive_sch(struct wlan_objmgr_psoc *psoc,
...
QDF_STATUS
policy_mgr_get_dynamic_mcc_adaptive_sch(struct wlan_objmgr_psoc *psoc,
					bool *dynamic_mcc_adaptive_sched)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
policy_mgr_map_wmi_channel_width_to_hw_mode_bw(wmi_channel_width width)
{
	switch (width) {
...
static void
policy_mgr_get_hw_mode_params(struct wlan_psoc_host_mac_phy_caps *caps,
			      struct policy_mgr_mac_ss_bw_info *info)
{
	...
	info->mac_tx_stream = policy_mgr_get_num_of_setbits_from_bitmask(
		QDF_MAX(caps->tx_chain_mask_2G, caps->tx_chain_mask_5G));
	info->mac_rx_stream = policy_mgr_get_num_of_setbits_from_bitmask(
		QDF_MAX(caps->rx_chain_mask_2G, caps->rx_chain_mask_5G));
	info->mac_bw = policy_mgr_map_wmi_channel_width_to_hw_mode_bw(
		QDF_MAX(caps->max_bw_supported_2G, caps->max_bw_supported_5G));
	...
	if (caps->supported_bands & WMI_HOST_WLAN_5G_CAPABILITY) {
		...
		max_5g_freq = caps->reg_cap_ext.high_5ghz_chan ?
				      QDF_MIN(caps->reg_cap_ext.high_5ghz_chan,
					      max_5g_freq) :
				      max_5g_freq;
		info->support_6ghz_band = max_5g_freq >
			      ...
			      uint64_t emlsr_mode)
{
	...
	POLICY_MGR_HW_MODE_MAC0_BANDWIDTH_SET(pm_ctx->hw_mode.hw_mode_list[pos],
	...
	POLICY_MGR_HW_MODE_MAC1_BANDWIDTH_SET(pm_ctx->hw_mode.hw_mode_list[pos],
					      mac1_ss_bw_info.mac_bw);
	POLICY_MGR_HW_MODE_DBS_MODE_SET(pm_ctx->hw_mode.hw_mode_list[pos],
					dbs_mode);
	POLICY_MGR_HW_MODE_AGILE_DFS_SET(pm_ctx->hw_mode.hw_mode_list[pos],
					 HW_MODE_AGILE_DFS_NONE);
	POLICY_MGR_HW_MODE_SBS_MODE_SET(pm_ctx->hw_mode.hw_mode_list[pos],
					sbs_mode);
	POLICY_MGR_HW_MODE_MAC0_BAND_SET(pm_ctx->hw_mode.hw_mode_list[pos],
					 mac0_ss_bw_info.mac_band_cap);
	POLICY_MGR_HW_MODE_ID_SET(pm_ctx->hw_mode.hw_mode_list[pos],
				  hw_mode_id);

	legacy_hwmode_lst = pm_ctx->hw_mode.hw_mode_list[pos];
	POLICY_MGR_HW_MODE_EMLSR_MODE_SET(pm_ctx->hw_mode.hw_mode_list[pos],
	...
	for (i = 0; i < pm_ctx->radio_comb_num; i++) {
		...
		if (*comb_num >= comb_max) {
			policy_mgr_err("out of buffer %d max %d",
				       pm_ctx->radio_comb_num, comb_max);
			return QDF_STATUS_E_FAILURE;
		}
		policy_mgr_debug(
			"radio %d: mode %d mac0 (0x%x, 0x%x), mac1 (0x%x 0x%x)",
			*comb_num, radio_comb->hw_mode,
			radio_comb->band_mask[0], radio_comb->antenna[0],
			radio_comb->band_mask[1], radio_comb->antenna[1]);
		qdf_mem_copy(&comb[*comb_num], radio_comb, sizeof(*radio_comb));
		(*comb_num)++;
	}
				      ...
				      struct radio_combination *radio)
{
	...
	policy_mgr_debug(
		"radio %d: mode %d mac0 (0x%x, 0x%x), mac1 (0x%x 0x%x)",
		pm_ctx->radio_comb_num, radio->hw_mode, radio->band_mask[0],
		radio->antenna[0], radio->band_mask[1], radio->antenna[1]);

	qdf_mem_copy(&pm_ctx->radio_combinations[pm_ctx->radio_comb_num], radio,
		     sizeof(*radio));
	pm_ctx->radio_comb_num++;
}

#define SET_RADIO(_radio, _mode, _mac0_band, _mac1_band, _mac0_antenna, \
		  _mac1_antenna)                                        \
	do {                                                            \
 ...
 * Return: void
 */
static void policy_mgr_update_radio_combination_matrix(
	struct wlan_objmgr_psoc *psoc,
	struct policy_mgr_mac_ss_bw_info mac0_ss_bw_info,
	struct policy_mgr_mac_ss_bw_info mac1_ss_bw_info, uint32_t dbs_mode,
	uint32_t sbs_mode)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	struct radio_combination radio;
	...
	if (!dbs_mode && !sbs_mode) {
		...
		if (mac0_ss_bw_info.mac_band_cap &
			...
			if (mac0_ss_bw_info.support_6ghz_band) {
				SET_RADIO(&radio, MODE_SMM, BIT(REG_BAND_6G), 0,
					  mac0_ss_bw_info.mac_tx_stream, 0);
				policy_mgr_add_radio_comb(pm_ctx, &radio);
			}
		}
...
policy_mgr_update_freq_info(struct policy_mgr_psoc_priv_obj *pm_ctx,
			    struct wlan_psoc_host_mac_phy_caps *mac_cap,
			    enum policy_mgr_mode mode, uint32_t phy_id)
{
	...
	for (phy_id = 0; phy_id < MAX_MAC; phy_id++) {
		...
		if (lower_sbs_freq_range[phy_id].low_2ghz_freq) {
			lower_sbs_freq_range[phy_id].low_5ghz_freq =
				policy_mgr_get_lowest_5ghz_freq_frm_range(
					ref_freq);
				  ...
				  enum policy_mgr_mode hwmode)
{
	...
	for (phy_id = 0; phy_id < MAX_MAC; phy_id++) {
		mac_range = &pm_ctx->hw_mode.freq_range_caps[hwmode][phy_id];
...
static void policy_mgr_update_mac_freq_info(
	struct wlan_objmgr_psoc *psoc, struct policy_mgr_psoc_priv_obj *pm_ctx,
	enum wmi_hw_mode_config_type hw_config_type, uint32_t phy_id,
	struct wlan_psoc_host_mac_phy_caps *mac_cap)
{
	if (phy_id >= MAX_MAC) {
		policy_mgr_err("mac more than two not supported: %d", phy_id);
		return;
	}

	policy_mgr_debug(
		"hw_mode_cfg: %d mac: %d band: 0x%x, SBS cutoff freq %d, 2Ghz: %d -> %d "
		"5Ghz: %d -> %d",
	...
	case WMI_HW_MODE_DBS_2G_5G:
		if (!policy_mgr_both_phy_range_updated(pm_ctx, MODE_DBS))
			policy_mgr_update_freq_info(pm_ctx, mac_cap, MODE_DBS,
						    phy_id);
		break;
	...
	case WMI_HW_MODE_EMLSR:
		...
		break;
	default:
		policy_mgr_err("HW mode not defined %d", hw_config_type);
		break;
	}
}

void policy_mgr_dump_curr_freq_range(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	uint32_t i;
...
void policy_mgr_dump_freq_range_per_mac(
	struct policy_mgr_freq_range *freq_range, enum policy_mgr_mode hw_mode)
{
	uint32_t i;
	...
	for (i = 0; i < MAX_MAC; i++)
		if (freq_range[i].low_2ghz_freq || freq_range[i].low_5ghz_freq)
			policymgr_nofl_debug(
				...
				policy_mgr_hw_mode_to_str(hw_mode), hw_mode, i,
...
void policy_mgr_dump_sbs_freq_range(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	...
	for (i = MODE_SMM; i < MODE_HW_MAX; i++) {
		if ((i == MODE_SBS) ||
		    (pm_ctx->hw_mode.sbs_lower_band_end_freq &&
		     (i == MODE_SBS_LOWER_SHARE ||
		      i == MODE_SBS_UPPER_SHARE))) {
			freq_range = pm_ctx->hw_mode.freq_range_caps[i];
			policy_mgr_dump_freq_range_per_mac(freq_range, i);
		}
	}
...
void policy_mgr_dump_freq_range(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
					  ...
					  struct target_psoc_info *tgt_hdl)
{
   ...
   */
	...
	for (i = 0; i < pm_ctx->num_dbs_hw_modes; i++) {
		...
		policy_mgr_update_mac_freq_info(psoc, pm_ctx, hw_config_type,
						tmp->phy_id, tmp);
		...
		if ((hw_config_type == WMI_HW_MODE_DBS) ||
			...
			policy_mgr_update_mac_freq_info(
				psoc, pm_ctx, hw_config_type, tmp->phy_id, tmp);
			if (hw_config_type == WMI_HW_MODE_DBS ||
			    hw_config_type == WMI_HW_MODE_DBS_OR_SBS)
		...
		policy_mgr_set_hw_mode_params(psoc, mac0_ss_bw_info,
					      mac1_ss_bw_info, i,
					      tmp->hw_mode_id, dbs_mode,
					      sbs_mode, emlsr_mode);
		/* Update radio combination info */
		policy_mgr_update_radio_combination_matrix(psoc,
							   mac0_ss_bw_info,
							   mac1_ss_bw_info,
							   dbs_mode, sbs_mode);
	}
...
qdf_freq_t policy_mgr_get_sbs_cut_off_freq(struct wlan_objmgr_psoc *psoc)
{
   ...
   */
	...
	second_mac_range = &pm_ctx->hw_mode.freq_range_caps[MODE_SBS][1];
...
static bool policy_mgr_2_freq_same_mac_in_freq_range(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	struct policy_mgr_freq_range *freq_range, qdf_freq_t freq_1,
	qdf_freq_t freq_2)
{
	uint8_t i;
...
bool policy_mgr_sbs_24_shared_with_high_5(
	struct policy_mgr_psoc_priv_obj *pm_ctx)
{
...
bool policy_mgr_sbs_24_shared_with_low_5(struct policy_mgr_psoc_priv_obj *pm_ctx)
{
	qdf_freq_t sbs_cut_off_freq;
	struct policy_mgr_freq_range freq_range;
	uint8_t i = 0;
...
bool policy_mgr_2_freq_same_mac_in_dbs(struct policy_mgr_psoc_priv_obj *pm_ctx,
				       ...
				       qdf_freq_t freq_1, qdf_freq_t freq_2)
{
	...
	if (policy_mgr_can_2ghz_share_low_high_5ghz_sbs(pm_ctx)) {
		...
		sbs_low_share =
			pm_ctx->hw_mode.freq_range_caps[MODE_SBS_LOWER_SHARE];
		if (policy_mgr_2_freq_same_mac_in_freq_range(
			    pm_ctx, sbs_low_share, freq_1, freq_2) ||
		    policy_mgr_2_freq_same_mac_in_freq_range(
			    pm_ctx, sbs_uppr_share, freq_1, freq_2))
			return true;
...
static bool policy_mgr_is_cur_freq_range_sbs(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	struct policy_mgr_freq_range *freq_range;
	uint8_t i;
					  ...
					  qdf_freq_t freq_1, qdf_freq_t freq_2)
{
	...
	if (is_dbs_mode_same_mac)
		is_sbs_mode_same_mac = policy_mgr_2_freq_same_mac_in_sbs(
			pm_ctx, freq_1, freq_2);
				       ...
				       qdf_freq_t freq_1, qdf_freq_t freq_2)
{
	...
	policy_mgr_rl_debug(
		"freq_1 %d freq_2 %d old_hw_mode_index=%d, new_hw_mode_index=%d",
	...
	if (hw_mode.dbs_cap || hw_mode.sbs_cap)
		return policy_mgr_2_freq_same_mac_in_freq_range(
			pm_ctx, freq_range, freq_1, freq_2);
...
static bool policy_mgr_3_freq_same_mac_in_freq_range(
	struct policy_mgr_psoc_priv_obj *pm_ctx,
	struct policy_mgr_freq_range *freq_range, qdf_freq_t freq_1,
	qdf_freq_t freq_2, qdf_freq_t freq_3)
{
	uint8_t i;
				  ...
				  qdf_freq_t freq_3)
{
	...
	if (pm_ctx->hw_mode.sbs_lower_band_end_freq) {
		...
		sbs_low_share =
			pm_ctx->hw_mode.freq_range_caps[MODE_SBS_LOWER_SHARE];
		if (policy_mgr_3_freq_same_mac_in_freq_range(
			    pm_ctx, sbs_low_share, freq_1, freq_2, freq_3) ||
		    policy_mgr_3_freq_same_mac_in_freq_range(
			    pm_ctx, sbs_uppr_share, freq_1, freq_2, freq_3))
			return true;
...
bool policy_mgr_3_freq_always_on_same_mac(struct wlan_objmgr_psoc *psoc,
					  qdf_freq_t freq_1, qdf_freq_t freq_2,
					  qdf_freq_t freq_3)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	freq_range = pm_ctx->hw_mode.freq_range_caps[MODE_DBS];
	is_dbs_mode_same_mac = policy_mgr_3_freq_same_mac_in_freq_range(
		pm_ctx, freq_range, freq_1, freq_2, freq_3);

	/* if DBS mode leading to same mac, check for SBS mode */
	if (is_dbs_mode_same_mac)
		is_sbs_mode_same_mac = policy_mgr_3_freq_same_mac_in_sbs(
			pm_ctx, freq_1, freq_2, freq_3);

	policy_mgr_rl_debug(
		"freq1 %d freq2 %d freq3 %d: Same mac:: DBS:%d SBS:%d", freq_1,
		freq_2, freq_3, is_dbs_mode_same_mac, is_sbs_mode_same_mac);
	/*
   * if in SBS and DBS mode, both is leading to freqs on same mac,
   * return true else return false.
   */
...
bool policy_mgr_are_3_freq_on_same_mac(struct wlan_objmgr_psoc *psoc,
				       qdf_freq_t freq_1, qdf_freq_t freq_2,
				       qdf_freq_t freq_3)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	/* current HW is DBS OR SBS check current DBS/SBS freq range */
	if (hw_mode.dbs_cap || hw_mode.sbs_cap)
		return policy_mgr_3_freq_same_mac_in_freq_range(
			pm_ctx, freq_range, freq_1, freq_2, freq_3);
	/*
   * If current HW mode is not DBS/SBS, check if in all supported mode
   * it they will be on same mac
...
static void policy_mgr_get_mac_freq_list(
	struct policy_mgr_freq_range *freq_range, uint8_t mac_id,
	uint8_t mac_freq_list[MAX_NUMBER_OF_CONC_CONNECTIONS],
	uint8_t mac_mode_list[MAX_NUMBER_OF_CONC_CONNECTIONS],
	uint8_t *mac_freq_num, qdf_freq_t freq_1,
	enum policy_mgr_con_mode mode_1, qdf_freq_t freq_2,
	enum policy_mgr_con_mode mode_2, qdf_freq_t freq_3,
	enum policy_mgr_con_mode mode_3, qdf_freq_t freq_4,
	enum policy_mgr_con_mode mode_4)
{
	uint8_t j = 0;
				...
				enum policy_mgr_mode hw_mode)
{
	...
	if (hw_mode == MODE_SBS_UPPER_SHARE || hw_mode == MODE_SBS_LOWER_SHARE)
...
static bool policy_mgr_mac_freq_list_allow(
	uint8_t mac_freq_list[MAX_NUMBER_OF_CONC_CONNECTIONS],
	uint8_t mac_mode_list[MAX_NUMBER_OF_CONC_CONNECTIONS],
...
static void policy_mgr_ml_sta_active_freq(struct wlan_objmgr_psoc *psoc,
					  qdf_freq_t ch_freq,
					  ...
					  qdf_freq_t *ml_sta_link1_freq)
{
	...
	policy_mgr_get_ml_sta_info_psoc(psoc, &num_ml_sta, &num_disabled_ml_sta,
...
bool policy_mgr_allow_4th_new_freq(struct wlan_objmgr_psoc *psoc,
				   qdf_freq_t ch_freq,
				   enum policy_mgr_con_mode mode,
				   uint32_t ext_flags)
{
	...
	policy_mgr_ml_sta_active_freq(psoc, ch_freq, mode, ext_flags,
				      &ml_sta_link0_freq, &ml_sta_link1_freq);
   ...
   */
     ...
     */
       ...
       */
			policy_mgr_get_mac_freq_list(
				freq_range, i, mac_freq_list, mac_mode_list,
				&mac_freq_num, conn[0].freq, conn[0].mode,
				conn[1].freq, conn[1].mode, conn[2].freq,
				conn[2].mode, ch_freq, mode);
     ...
     */
		if (i == MAX_MAC) {
			policy_mgr_rl_debug(
				"new freq %d mode %s is allowed in hw mode %s",
	...
	policy_mgr_debug(
		"the 4th new freq %d mode %s is not allowed in any hw mode",
		ch_freq, device_mode_to_string(mode));
				 ...
				 uint32_t *ev_wlan_dbs_hw_mode_list)
{
	...
	qdf_mem_copy(pm_ctx->hw_mode.hw_mode_list, ev_wlan_dbs_hw_mode_list,
...
void policy_mgr_dump_dbs_hw_mode(struct wlan_objmgr_psoc *psoc)
{
	...
	for (i = 0; i < pm_ctx->num_dbs_hw_modes; i++) {
		...
		policy_mgr_debug(
			"[%d]-MAC0: tx_ss:%d rx_ss:%d bw_idx:%d band_cap:%d", i,
		...
		policy_mgr_debug("[%d]-MAC1: tx_ss:%d rx_ss:%d bw_idx:%d", i,
				...
				uint32_t scan_config, uint32_t fw_config)
{
   ...
   */
	...
	WMI_DBS_FW_MODE_CFG_DBS_SET(pm_ctx->dual_mac_cfg.cur_fw_mode_config,
...
bool policy_mgr_find_if_fw_supports_dbs(struct wlan_objmgr_psoc *psoc)
{
	...
	dbs_support = wmi_service_enabled(
		wmi_handle, wmi_service_dual_band_simultaneous_support);
				       ...
				       bool *two_by_two_dbs)
{
	...
	*two_by_two_dbs = false;
	...
	policy_mgr_get_tx_rx_ss_from_config(HW_MODE_SS_1x1, &conf1_tx_ss,
					    &conf1_rx_ss);
	/* To check 2x2 capability */
	policy_mgr_get_tx_rx_ss_from_config(HW_MODE_SS_2x2, &conf2_tx_ss,
					    &conf2_rx_ss);
...
QDF_STATUS
policy_mgr_get_current_hw_mode(struct wlan_objmgr_psoc *psoc,
			       struct policy_mgr_hw_mode_params *hw_mode)
{
	QDF_STATUS status;
...
bool policy_mgr_is_2x2_5G_1x1_2G_dbs_capable(struct wlan_objmgr_psoc *psoc)
{
	return policy_mgr_is_2x2_1x1_dbs_capable(psoc) &&
	       (policy_mgr_get_hw_mode_idx_from_dbs_hw_list(
			psoc, HW_MODE_SS_2x2, HW_MODE_80_MHZ, HW_MODE_SS_1x1,
			HW_MODE_40_MHZ, HW_MODE_MAC_BAND_5G, HW_MODE_DBS,
			HW_MODE_AGILE_DFS_NONE, HW_MODE_SBS_NONE) >= 0);
}
...
bool policy_mgr_is_2x2_2G_1x1_5G_dbs_capable(struct wlan_objmgr_psoc *psoc)
{
	return policy_mgr_is_2x2_1x1_dbs_capable(psoc) &&
	       (policy_mgr_get_hw_mode_idx_from_dbs_hw_list(
			psoc, HW_MODE_SS_2x2, HW_MODE_40_MHZ, HW_MODE_SS_1x1,
			HW_MODE_40_MHZ, HW_MODE_MAC_BAND_2G, HW_MODE_DBS,
			HW_MODE_AGILE_DFS_NONE, HW_MODE_SBS_NONE) >= 0);
}
...
uint32_t policy_mgr_get_connection_count_with_mlo(struct wlan_objmgr_psoc *psoc)
{
	uint32_t conn_index, count = 0;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	enum policy_mgr_con_mode mode;
...
uint32_t
policy_mgr_mode_specific_connection_count(struct wlan_objmgr_psoc *psoc,
					  enum policy_mgr_con_mode mode,
					  uint32_t *list)
{
				       ...
				       uint32_t *list)
{
	...
	count += policy_mgr_mode_specific_connection_count(
		psoc, PM_LL_LT_SAP_MODE, list ? &list[count] : NULL);
	return count;
}
					     ...
					     uint32_t *list)
{
	...
	count += policy_mgr_mode_specific_connection_count(
		psoc, PM_P2P_GO_MODE, list ? &list[count] : NULL);
	return count;
}

QDF_STATUS
policy_mgr_check_conn_with_mode_and_vdev_id(struct wlan_objmgr_psoc *psoc,
					    enum policy_mgr_con_mode mode,
					    uint32_t vdev_id)
{
					 ...
					 uint8_t single_mac_scan_with_dbs_val)
{
	...
	status = policy_mgr_get_updated_scan_config(
	...
	status = policy_mgr_get_updated_fw_mode_config(
		psoc, &cfg.fw_mode_config, policy_mgr_get_dbs_config(psoc),
		policy_mgr_get_agile_dfs_config(psoc));
	if (status != QDF_STATUS_SUCCESS) {
		policy_mgr_err(
			"policy_mgr_get_updated_fw_mode_config failed %d",
			status);
		return;
	}
	...
	policy_mgr_debug("scan_config:%x fw_mode_config:%x", cfg.scan_config,
			 cfg.fw_mode_config);
					    ...
					    uint8_t dbs, uint8_t dfs)
{
	...
	status = policy_mgr_get_updated_scan_config(
		psoc, &cfg.scan_config, policy_mgr_get_dbs_scan_config(psoc),
	...
	status = policy_mgr_get_updated_fw_mode_config(
		psoc, &cfg.fw_mode_config, dbs, dfs);
	if (status != QDF_STATUS_SUCCESS) {
		policy_mgr_err(
			"policy_mgr_get_updated_fw_mode_config failed %d",
			status);
	...
	for (i = 0; i < MAX_NUMBER_OF_CONC_CONNECTIONS; i++) {
		...
		if (pm_conc_connection_list[i].freq != ch_freq &&
		    policy_mgr_are_2_freq_on_same_mac(
			    psoc, pm_conc_connection_list[i].freq, ch_freq)) {
			if (mcc_vdev_id)
				*mcc_vdev_id =
...
bool policy_mgr_is_mcc_on_any_sta_vdev(struct wlan_objmgr_psoc *psoc)
{
	...
	connection_count = policy_mgr_get_mode_specific_conn_info(
...
bool policy_mgr_current_concurrency_is_scc(struct wlan_objmgr_psoc *psoc)
{
	...
	case 2:
		if (pm_conc_connection_list[0].freq ==
		    ...
		    policy_mgr_are_2_freq_on_same_mac(
			    psoc, pm_conc_connection_list[0].freq,
			    pm_conc_connection_list[1].freq))
			is_scc = true;
		break;
	case 3:
     ...
     */
		if ((pm_conc_connection_list[0].freq !=
		    ...
		    (pm_conc_connection_list[1].freq !=
			     pm_conc_connection_list[2].freq &&
		     policy_mgr_are_2_freq_on_same_mac(
			     psoc, pm_conc_connection_list[1].freq,
			     pm_conc_connection_list[2].freq)))
			is_mcc = true;
		break;
	default:
...
bool policy_mgr_is_sap_p2pgo_on_dfs(struct wlan_objmgr_psoc *psoc)
{
	...
	count = policy_mgr_mode_specific_connection_count(psoc, PM_P2P_GO_MODE,
							  list);
	index = 0;
	while (index < count) {
		if (pm_conc_connection_list[list[index]].ch_flagext &
				     ...
				     enum QDF_OPMODE mode)
{
	...
	policy_mgr_debug(
		"concurrency_mode = 0x%x Number of open sessions for mode %d = %d",
 ...
 */
static bool policy_mgr_validate_conn_info(struct wlan_objmgr_psoc *psoc)
{
	...
	if (j != conn_num) {
		policy_mgr_err("active session/conn count mismatch %d %d", j,
			       conn_num);
		panic = true;
	}
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
...
policy_mgr_get_ml_sta_info(struct policy_mgr_psoc_priv_obj *pm_ctx,
			   uint8_t *num_ml_sta, uint8_t *num_disabled_ml_sta,
			   uint8_t *ml_vdev_lst, qdf_freq_t *ml_freq_lst,
			   uint8_t *num_non_ml, uint8_t *non_ml_vdev_lst,
...
void policy_mgr_get_ml_sta_info_psoc(
	struct wlan_objmgr_psoc *psoc, uint8_t *num_ml_sta,
	uint8_t *num_disabled_ml_sta, uint8_t *ml_vdev_lst,
	qdf_freq_t *ml_freq_lst, uint8_t *num_non_ml, uint8_t *non_ml_vdev_lst,
	qdf_freq_t *non_ml_freq_lst)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	return policy_mgr_get_ml_sta_info(pm_ctx, num_ml_sta,
					  num_disabled_ml_sta, ml_vdev_lst,
					  ml_freq_lst, num_non_ml,
					  non_ml_vdev_lst, non_ml_freq_lst);
}
				 ...
				 uint8_t vdev_id)
{
	...
	if (i < MAX_NUMBER_OF_DISABLE_LINK) {
		...
		qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
		policy_mgr_debug(
			"Disabled link already present vdev %d, pm_mode %d, update freq %d",
	...
	for (i = 0; i < MAX_NUMBER_OF_DISABLE_LINK; i++) {
		if (!pm_disabled_ml_links[i].in_use) {
			...
			pm_disabled_ml_links[i].freq = freq;
			policy_mgr_debug(
				"Disabled link added vdev id: %d freq: %d pm_mode %d",
	...
	if (i >= MAX_NUMBER_OF_DISABLE_LINK) {
		policy_mgr_err(
			"No empty entry found to disable link for vdev %d",
			vdev_id);
	...
	struct wlan_objmgr_psoc *psoc, uint8_t vdev_id)
{
   ...
   */
	if (QDF_IS_STATUS_SUCCESS(status))
		...
		policy_mgr_debug(
			"Connection tbl dont have vdev %d in STA mode, Add it in disabled tbl",
			vdev_id);
				       ...
				       bool peer_assoc)
{
	...
	if (ml_is_nlink_service_supported(psoc) && !peer_assoc) {
		ml_nlink_get_dynamic_inactive_links(
			psoc, vdev, &dynamic_inactive, &forced_inactive);
		link_id = wlan_vdev_get_link_id(vdev);
		if ((forced_inactive | dynamic_inactive) & (1 << link_id)) {
			policy_mgr_debug(
				"vdev %d linkid %d is forced inactived 0x%0x dyn 0x%x",
				wlan_vdev_get_id(vdev), link_id,
				forced_inactive, dynamic_inactive);
			return true;
		}
	}
...
bool policy_mgr_ml_link_vdev_need_to_be_disabled(struct wlan_objmgr_psoc *psoc,
						 struct wlan_objmgr_vdev *vdev,
						 bool peer_assoc)
{
	union conc_ext_flag conc_ext_flags;
	...
	if (policy_mgr_vdev_disabled_by_link_force(psoc, vdev, peer_assoc))
		return true;
   ...
   * not allowed
   */
	return !policy_mgr_is_concurrency_allowed(
		psoc, PM_STA_MODE, wlan_get_operation_chan_freq(vdev),
		HW_MODE_20_MHZ, conc_ext_flags.value, NULL);
}

static void policy_mgr_enable_disable_link_from_vdev_bitmask(
	struct wlan_objmgr_psoc *psoc, unsigned long enable_vdev_mask,
	unsigned long disable_vdev_mask, uint8_t start_vdev_id)
{
	uint8_t i;
...
policy_mgr_trigger_roam_on_link_removal(struct wlan_objmgr_vdev *vdev)
{
	...
	policy_mgr_get_ml_sta_info(pm_ctx, &num_ml_sta, &num_disabled_ml_sta,
				   ml_sta_vdev_lst, ml_freq_lst, NULL, NULL,
				   NULL);
	if (!num_ml_sta) {
	...
	if (num_ml_sta > MAX_NUMBER_OF_CONC_CONNECTIONS ||
	    num_disabled_ml_sta > MAX_NUMBER_OF_CONC_CONNECTIONS ||
	    num_ml_sta <= num_disabled_ml_sta) {
		policy_mgr_debug("unexpected ml sta num %d %d", num_ml_sta,
				 num_disabled_ml_sta);
		return;
	}
	num_active_ml_sta = num_ml_sta;
   ...
   */
	for (i = 0; i < num_ml_sta; i++) {
		...
		ml_vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
			pm_ctx->psoc, ml_sta_vdev_lst[i], WLAN_POLICY_MGR_ID);
		if (!ml_vdev) {
		...
		if (!wlan_cm_is_vdev_connected(ml_vdev)) {
			policy_mgr_debug(
				"ml sta vdev %d is not connected state",
				ml_sta_vdev_lst[i]);
		...
		if (assoc_vdev_id == WLAN_INVALID_VDEV_ID &&
		    !wlan_vdev_mlme_get_is_mlo_link(psoc, ml_sta_vdev_lst[i]))
	...
	if (ml_sta_is_not_connected) {
		policy_mgr_debug(
			"ml sta is non-connected state, don't trigger roam");
		return;
	}
	/* trigger roaming */
...
static void
policy_mgr_update_dynamic_inactive_bitmap(struct wlan_objmgr_psoc *psoc,
					  struct wlan_objmgr_vdev *vdev,
					  ...
					  struct mlo_link_set_active_resp *resp)
{
   ...
   */
	candidate_inactive_links = req->param.force_cmd.ieee_link_id_bitmap &
				   resp->inactive_linkid_bitmap;

	num = ml_nlink_convert_link_bitmap_to_ids(
		candidate_inactive_links, QDF_ARRAY_SIZE(link_ids), link_ids);
	if (num < req->param.force_cmd.link_num &&
		...
		num += ml_nlink_convert_link_bitmap_to_ids(
			candidate_inactive_links,
			QDF_ARRAY_SIZE(link_ids) - num, &link_ids[num]);
	}
	for (i = 0; i < num; i++) {
...
update:
	policy_mgr_debug(
		"inactive link num %d bitmap 0x%x force inactive 0x%x dyn "
		"links 0x%x num %d",
		req->param.force_cmd.link_num,
		req->param.force_cmd.ieee_link_id_bitmap,
		resp->inactive_linkid_bitmap, dyn_inactive_links, dyn_num);
	if (dyn_num < req->param.force_cmd.link_num)
		policy_mgr_debug(
			"unexpected selected dynamic inactive link num %d",
			dyn_num);
	ml_nlink_set_dynamic_inactive_links(psoc, vdev, dyn_inactive_links);
}

static void policy_mgr_handle_vdev_active_inactive_resp(
	...
	struct mlo_link_set_active_resp *resp)
{
   ...
   */
	ml_nlink_get_dynamic_inactive_links(
		psoc, vdev, &dynamic_inactive_bitmap, &forced_inactive_bitmap);
	resp->inactive_linkid_bitmap |= dynamic_inactive_bitmap;
	ml_nlink_convert_linkid_bitmap_to_vdev_bitmap(
		psoc, vdev, resp->inactive_linkid_bitmap, &assoc_bitmap,
		&resp->inactive_sz, resp->inactive, &vdev_id_num, vdev_ids);

	ml_nlink_convert_linkid_bitmap_to_vdev_bitmap(
		psoc, vdev, (~resp->inactive_linkid_bitmap) & assoc_bitmap,
		NULL, &resp->active_sz, resp->active, &vdev_id_num, vdev_ids);
				    ...
				    struct mlo_link_set_active_resp *resp)
{
	...
	ml_nlink_convert_vdev_bitmap_to_linkid_bitmap(
		psoc, vdev, req->param.num_vdev_bitmap, req->param.vdev_bitmap,
		&resp->active_linkid_bitmap, &assoc_bitmap);
	ml_nlink_set_curr_force_active_state(
					...
					struct mlo_link_set_active_resp *resp)
{
	...
	ml_nlink_set_curr_force_active_num_state(
		psoc, vdev, req->param.link_num[0].num_of_link, link_bitmap);
	/*
   ...
   */
	for (i = 0; i < resp->active_sz; i++)
		policy_mgr_enable_disable_link_from_vdev_bitmask(
			psoc, resp->active[i],
			resp->active[i] ^ req->param.vdev_bitmap[i], i * 32);
}

static void
policy_mgr_handle_force_inactive_num_resp(struct wlan_objmgr_psoc *psoc,
					  struct wlan_objmgr_vdev *vdev,
					  struct mlo_link_set_active_req *req,
					  struct mlo_link_set_active_resp *resp)
{
   ...
   */
	for (i = 0; i < resp->inactive_sz; i++)
		policy_mgr_enable_disable_link_from_vdev_bitmask(
			psoc, resp->inactive[i] ^ req->param.vdev_bitmap[i],
			resp->inactive[i], i * 32);
}

static void policy_mgr_handle_force_active_inactive_resp(
	struct wlan_objmgr_psoc *psoc, struct wlan_objmgr_vdev *vdev,
	struct mlo_link_set_active_req *req,
	struct mlo_link_set_active_resp *resp)
{
	...
	ml_nlink_convert_vdev_bitmap_to_linkid_bitmap(
		...
		req->param.inactive_vdev_bitmap, &resp->inactive_linkid_bitmap,
		&assoc_bitmap);
	ml_nlink_set_curr_force_inactive_state(
		psoc, vdev, resp->inactive_linkid_bitmap, LINK_ADD);
	ml_nlink_convert_vdev_bitmap_to_linkid_bitmap(
				...
				struct mlo_link_set_active_resp *resp)
{
	...
	ml_nlink_set_curr_force_inactive_state(psoc, vdev, link_bitmap,
					       LINK_CLR);
	ml_nlink_set_curr_force_active_state(psoc, vdev, link_bitmap, LINK_CLR);
	ml_nlink_set_curr_force_active_num_state(psoc, vdev, 0, 0);
	ml_nlink_set_curr_force_inactive_num_state(psoc, vdev, 0, 0);
...
static void policy_mgr_handle_link_enable_disable_resp(
	...
	struct mlo_link_set_active_resp *resp)
{
	...
	if (resp->status) {
		policy_mgr_err(
			"Set link status %d, for mode %d reason %d vdev bitmask 0x%x",
			resp->status, req->param.force_mode, req->param.reason,
			req->param.vdev_bitmap[0]);
		goto complete_evnt;
	}

	policy_mgr_debug(
		"Req mode %d reason %d, bitmask[0] = 0x%x, resp: active %d "
		"inactive %d, active[0] 0x%x inactive[0] 0x%x",
		req->param.force_mode, req->param.reason,
		req->param.vdev_bitmap[0], resp->active_sz, resp->inactive_sz,
...
complete_evnt:
	...
	if (ml_is_nlink_service_supported(psoc) && req && resp &&
	    !resp->status && req->param.control_flags.post_re_evaluate)
	...
	if (req && resp && !resp->status && status == QDF_STATUS_SUCCESS)
				    ...
				    enum QDF_OPMODE mode, uint8_t session_id)
{
   ...
   */
	...
	if (mode == QDF_NDI_MODE &&
		...
		pm_ctx->hdd_cbacks.wlan_hdd_indicate_active_ndp_cnt(
			psoc, session_id, pm_ctx->no_of_active_sessions[mode]);
	...
	policy_mgr_debug("No.# of active sessions for mode %d = %d", mode,
			 pm_ctx->no_of_active_sessions[mode]);
   ...
   */
	if ((policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
	    ...
	    (policy_mgr_mode_specific_connection_count(psoc, PM_SAP_MODE,
						       NULL) > 0) ||
	    (policy_mgr_mode_specific_connection_count(psoc, PM_P2P_CLIENT_MODE,
						       NULL) > 0) ||
	    (policy_mgr_mode_specific_connection_count(psoc, PM_P2P_GO_MODE,
						       NULL) > 0) ||
	    (policy_mgr_mode_specific_connection_count(psoc, PM_NDI_MODE,
						       NULL) > 0)) {
	...
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
	if (mode == QDF_SAP_MODE || mode == QDF_P2P_GO_MODE)
		ml_nlink_conn_change_notify(psoc, session_id,
					    ml_nlink_ap_started_evt, NULL);
}
...
static void policy_mgr_update_sta_scc_info_for_later_check(
	struct policy_mgr_psoc_priv_obj *pm_ctx, enum QDF_OPMODE mode,
	uint8_t vdev_id)
{
	uint32_t conn_index = 0;
					  ...
					  uint8_t session_id)
{
	...
	policy_mgr_update_sta_scc_info_for_later_check(pm_ctx, mode,
						       session_id);
	qdf_mutex_acquire(&pm_ctx->qdf_conc_list_lock);
	switch (mode) {
	case QDF_STA_MODE:
	...
	if ((policy_mgr_get_connection_count(psoc) == 0) ||
	    ((policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
	     ...
	     (policy_mgr_mode_specific_connection_count(
		      psoc, PM_P2P_CLIENT_MODE, NULL) == 0) &&
	     (policy_mgr_mode_specific_connection_count(psoc, PM_P2P_GO_MODE,
							NULL) == 0) &&
	     (policy_mgr_mode_specific_connection_count(psoc, PM_NDI_MODE,
							NULL) == 0))) {
   ...
   */
	...
	if (!pm_ctx->last_disconn_sta_freq) {
		if (policy_mgr_update_indoor_concurrency(
			    psoc, session_id, cur_freq,
					    ...
					    enum QDF_OPMODE op_mode)
{
	...
	if (op_mode == QDF_NAN_DISC_MODE) {
		...
		if (QDF_STATUS_SUCCESS != status) {
			policy_mgr_err(
				"can't find vdev_id %d in connection table",
				vdev_id);
			return status;
		}
	} else {
	...
	policy_mgr_update_conc_list(
		...
		conn_table_entry.mac_id, chain_mask, nss, vdev_id, true,
		update_conn, conn_table_entry.ch_flagext);
	policy_mgr_debug("Add at idx:%d vdev %d mac=%d", conn_index, vdev_id,
					    ...
					    uint32_t vdev_id)
{
	...
	if (!found) {
		policy_mgr_err(
			"can't find vdev_id %d in pm_conc_connection_list",
			vdev_id);
		qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
		return status;
	}
	...
	if (count == 1) {
		...
		*vdev_id = pm_conc_connection_list[list[index]].vdev_id;
		...
		for (index = 0; index < count; index++) {
			if (ch_freq_list)
				ch_freq_list[index] =
					pm_conc_connection_list[list[index]]
						.freq;
				      ...
				      uint32_t *ch_freq_list, uint8_t *vdev_id)
{
	...
	count += policy_mgr_get_mode_specific_conn_info(
		psoc, ch_freq_list ? &ch_freq_list[count] : NULL,
		vdev_id ? &vdev_id[count] : NULL, PM_LL_LT_SAP_MODE);
	return count;
}
					    ...
					    uint8_t *vdev_id)
{
	...
	count += policy_mgr_get_mode_specific_conn_info(
		psoc, ch_freq_list ? &ch_freq_list[count] : NULL,
		vdev_id ? &vdev_id[count] : NULL, PM_P2P_GO_MODE);
	return count;
}
					    ...
					    uint8_t *vdev_id_list)
{
	...
	*num_non_ml = 0;
	...
	sta_num = policy_mgr_get_mode_specific_conn_info(
		psoc, freq_list, vdev_id_list, PM_STA_MODE);
	if (!sta_num)
		return;

	for (i = 0; i < sta_num; i++) {
		temp_vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
			psoc, vdev_id_list[i], WLAN_POLICY_MGR_ID);
		if (!temp_vdev) {
			policy_mgr_err("invalid vdev for id %d",
				       vdev_id_list[i]);
			*num_ml = 0;
...
out:
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
	policy_mgr_debug(
		"Non-ML STA count %d, ML STA count %d, sta concurrency on "
		"different mac %d",
...
bool policy_mgr_max_concurrent_connections_reached(struct wlan_objmgr_psoc *psoc)
{
	uint8_t i = 0, j = 0;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
static bool policy_mgr_is_concurrency_allowed_4_port(
	...
	uint32_t ch_freq, struct policy_mgr_pcl_list pcl)
{
	uint32_t i;
	struct policy_mgr_psoc_priv_obj *pm_ctx = NULL;
	...
	sap_cnt = policy_mgr_mode_specific_connection_count(psoc, PM_SAP_MODE,
							    NULL);

	go_cnt = policy_mgr_mode_specific_connection_count(psoc, PM_P2P_GO_MODE,
							   NULL);
	if (sap_cnt || go_cnt) {
		pm_ctx = policy_mgr_get_context(psoc);
		if (!pm_ctx) {
		...
		if (!policy_mgr_is_force_scc(psoc)) {
			policy_mgr_err(
				"couldn't start 4th port for bad force scc cfg");
			return false;
		}
		...
		if (!policy_mgr_is_dbs_enable(psoc) ||
			...
			policy_mgr_err(
				"Couldn't start 4th port for bad cfg of dual mac, dfs "
				"scc, lte coex scc");
			return false;
		}
...
bool policy_mgr_is_6ghz_conc_mode_supported(struct wlan_objmgr_psoc *psoc,
					    enum policy_mgr_con_mode mode)
{
 ...
 */
static bool policy_mgr_is_6g_channel_allowed(struct wlan_objmgr_psoc *psoc,
					     enum policy_mgr_con_mode mode,
					     uint32_t ch_freq)
{
	uint32_t conn_index = 0;
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		if ((conn->mode == PM_SAP_MODE ||
		    ...
		    is_dfs &&
		    (ch_freq != conn->freq &&
		     !policy_mgr_are_sbs_chan(psoc, ch_freq, conn->freq))) {
			policy_mgr_rl_debug(
				"don't allow MCC if SAP/GO on DFS channel");
			qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
			return false;
		}
	}
					    ...
					    uint8_t sap_vdev_id)
{
	...
	if (pm_ctx->hdd_cbacks.wlan_get_sap_acs_band)
		pm_ctx->hdd_cbacks.wlan_get_sap_acs_band(psoc, sap_vdev_id,
							 &acs_band);
...
uint8_t policy_mgr_get_legacy_conn_info(struct wlan_objmgr_psoc *psoc,
					uint8_t *vdev_lst, qdf_freq_t *freq_lst,
					...
					uint8_t lst_sz)
{
     ...
     */
		...
		policy_mgr_debug("vdev %d freq %d mode %s pri %d", vdev_id,
				 freq_lst[j],
					   ...
					   uint32_t num_mlo_vdev)
{
	...
	for (vdev_idx = 0; vdev_idx < num_mlo_vdev; vdev_idx++) {
		...
		if (entry_idx >= MLO_LINK_NUM_SZ) {
			policy_mgr_err(
				"Invalid entry_idx %d num_mlo_vdev %d vdev %d",
...
static void policy_mgr_fill_ml_inactive_link_vdev_bitmap(
	struct mlo_link_set_active_req *req, uint8_t *mlo_inactive_vdev_lst,
	uint32_t num_mlo_inactive_vdev)
{
	...
	policy_mgr_debug("num_vdev_bitmap %d inactive_vdev_bitmap[0] = 0x%x, "
			 "inactive_vdev_bitmap[1] = 0x%x",
...
{
	return policy_mgr_handle_ml_sta_link_state_allowed(psoc, param->reason);
}

uint32_t policy_mgr_get_active_vdev_bitmap(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS policy_mgr_mlo_sta_set_link_by_linkid(
	struct wlan_objmgr_psoc *psoc, struct wlan_objmgr_vdev *vdev,
	enum mlo_link_force_reason reason, enum mlo_link_force_mode mode,
	uint8_t link_num, uint8_t num_mlo_vdev, uint8_t *mlo_vdev_lst,
	uint8_t num_mlo_inactive_vdev, uint8_t *mlo_inactive_vdev_lst)
{
	uint32_t link_bitmap = 0;
	...
	ml_nlink_convert_vdev_bitmap_to_linkid_bitmap(psoc, vdev,
						      MLO_VDEV_BITMAP_SZ,
						      vdev_bitmap, &link_bitmap,
						      &assoc_bitmap);

	ml_nlink_convert_vdev_bitmap_to_linkid_bitmap(
		psoc, vdev, MLO_VDEV_BITMAP_SZ, vdev_bitmap2, &link_bitmap2,
		&assoc_bitmap);
	...
	return policy_mgr_mlo_sta_set_nlink(psoc, vdev, reason, mode, link_num,
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS policy_mgr_mlo_sta_set_link_ext(
	struct wlan_objmgr_psoc *psoc, enum mlo_link_force_reason reason,
	enum mlo_link_force_mode mode, uint8_t num_mlo_vdev,
	uint8_t *mlo_vdev_lst, uint8_t num_mlo_inactive_vdev,
	uint8_t *mlo_inactive_vdev_lst)
{
   ...
   */
	...
	req->ctx.validate_set_mlo_link_cb = policy_mgr_validate_set_mlo_link_cb;
	req->ctx.cb_arg = req;
   ...
   */
	...
	if (ml_is_nlink_service_supported(psoc)) {
		status = policy_mgr_mlo_sta_set_link_by_linkid(
			psoc, vdev, reason, mode,
			req->param.link_num[0].num_of_link, num_mlo_vdev,
			mlo_vdev_lst, num_mlo_inactive_vdev,
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err(
			"vdev %d: Failed to set link mode %d num_mlo_vdev %d reason %d",
			wlan_vdev_get_id(vdev), mode, num_mlo_vdev, reason);
		qdf_mem_free(req);
		policy_mgr_set_link_in_progress(pm_ctx, false);
	}
...
policy_mgr_mlo_sta_set_nlink(struct wlan_objmgr_psoc *psoc,
			     struct wlan_objmgr_vdev *vdev,
			     enum mlo_link_force_reason reason,
			     enum mlo_link_force_mode mode, uint8_t link_num,
			     uint16_t link_bitmap, uint16_t link_bitmap2,
			     uint32_t link_control_flags)
{
	...
	policy_mgr_debug(
		"vdev %d: mode %d %s reason %d bitmap 0x%x 0x%x ctrl 0x%x",
		wlan_vdev_get_id(vdev), mode, force_mode_to_string(mode),
		reason, link_bitmap, link_bitmap2, link_control_flags);
	...
	if (link_control_flags & link_ctrl_f_overwrite_inactive_bitmap)
		req->param.control_flags.overwrite_force_inactive_bitmap = true;
	...
	status = wlan_vdev_get_bss_peer_mld_mac(
		vdev, &req->param.force_cmd.ap_mld_mac_addr);
	if (QDF_IS_STATUS_ERROR(status)) {
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err(
			"vdev %d: Failed to set link mode %d num_mlo_vdev %d reason %d",
			wlan_vdev_get_id(vdev), mode, link_num, reason);
		qdf_mem_free(req);
		policy_mgr_set_link_in_progress(pm_ctx, false);
	} else {
...
uint32_t policy_mgr_get_conc_ext_flags(struct wlan_objmgr_vdev *vdev,
				       bool force_mlo)
{
 ...
 *  Return: true if supports else false.
 */
static bool policy_mgr_allow_sta_concurrency(struct wlan_objmgr_psoc *psoc,
					     qdf_freq_t freq,
					     uint32_t ext_flags)
{
	uint32_t conn_index = 0;
...
next:
	...
	if (sta_cnt && !policy_mgr_allow_multiple_sta_connections(psoc)) {
				   ...
				   struct wlan_mlo_link_switch_req *req)
{
	...
	policy_mgr_debug("target link %d freq %d curr link %d vdev %d",
			 new_ieee_link_id, new_primary_freq, curr_ieee_link_id,
			 vdev_id);
	...
	status = ml_nlink_conn_change_notify(
		psoc, vdev_id, ml_nlink_link_switch_start_evt, &data);
	if (QDF_IS_STATUS_ERROR(status))
		return status;
	...
	policy_mgr_store_and_del_conn_info_by_vdev_id(psoc, vdev_id, info,
						      &num_del);
	conc_ext_flags.value = policy_mgr_get_conc_ext_flags(vdev, true);
	if (!policy_mgr_is_concurrency_allowed(psoc, PM_STA_MODE,
					       new_primary_freq, HW_MODE_20_MHZ,
					       conc_ext_flags.value, NULL)) {
		status = QDF_STATUS_E_INVAL;
		policy_mgr_debug(
			"target link %d freq %d not allowed by conc rule",
				   ...
				   uint8_t *mlo_vdev_lst, uint8_t *num_mlo)
{
	...
	if (mlo_idx < 2)
		is_sbs_link = false;
	if (is_sbs_link && !policy_mgr_are_sbs_chan(psoc, mlo_freq_list[0],
						    mlo_freq_list[1])) {
		policy_mgr_debug("Freq %d and %d are not SBS, set SBS false",
				 mlo_freq_list[0], mlo_freq_list[1]);
		is_sbs_link = false;
	}
				   ...
				   uint8_t *mlo_vdev_lst, uint8_t *num_mlo)
{
	...
	for (i = 0; i < mode_num; i++) {
		...
		if (wlan_vdev_mlme_is_mlo_vdev(temp_vdev)) {
			if (mlo_vdev_lst)
				mlo_vdev_lst[mlo_idx] = vdev_id_list[i];
			mlo_freq = wlan_get_operation_chan_freq(temp_vdev);
				     ...
				     uint8_t *mlo_vdev_lst, uint8_t *num_mlo)
{
	...
	mode_num = policy_mgr_get_mode_specific_conn_info(
		psoc, NULL, vdev_id_list, PM_STA_MODE);
					     ...
					     bool emlsr_sta_coming_up)
{
	...
	is_mlo_emlsr =
		policy_mgr_is_mlo_in_mode_emlsr(psoc, mlo_vdev_lst, &num_mlo);
...
static uint8_t policy_mgr_get_affected_links_for_sta_sta(
	struct wlan_objmgr_psoc *psoc, uint8_t num_ml, qdf_freq_t *freq_list,
	uint8_t *vdev_id_list, uint8_t *ml_vdev_lst, uint8_t *ml_idx,
	qdf_freq_t freq)
{
	uint8_t i = 0;
   ...
   */
	...
	i = 0;
	while (i < num_ml && !wlan_reg_is_24ghz_ch_freq(freq_list[ml_idx[i]]))
		i++;
...
policy_mgr_get_concurrent_num_links(struct wlan_objmgr_vdev *vdev,
				    ...
				    uint8_t *vdev_id_list, uint8_t *ml_vdev_lst,
				    uint8_t *ml_idx, qdf_freq_t freq)
{
	...
	if (i < num_ml) {
		...
		policy_mgr_debug(
			"vdev %d: ML vdev %d lead to SCC, STA freq %d ML freq %d, "
			"no need to disable link",
			wlan_vdev_get_id(vdev), vdev_id_list[ml_idx[i]], freq,
			freq_list[ml_idx[i]]);
		return 0;
	}

	return policy_mgr_get_affected_links_for_sta_sta(
		psoc, num_ml, freq_list, vdev_id_list, ml_vdev_lst, ml_idx,
		freq);
}

static void policy_mgr_ml_sta_concurrency_on_connect(
	struct wlan_objmgr_psoc *psoc, struct wlan_objmgr_vdev *vdev,
	uint8_t num_ml, uint8_t *ml_idx, uint8_t num_non_ml,
	uint8_t *non_ml_idx, qdf_freq_t *freq_list, uint8_t *vdev_id_list)
{
	qdf_freq_t freq = 0;
	...
	affected_links = policy_mgr_get_concurrent_num_links(
		vdev, num_ml, freq_list, vdev_id_list, ml_vdev_lst, ml_idx,
		freq);
	...
	policy_mgr_debug("affected link found: %u vdev_id: %u", affected_links,
			 ml_vdev_lst[0]);
   ...
   */
	if (affected_links < num_ml &&
		...
		if (mlo_is_sta_inactivity_allowed_with_quiet(
			    psoc, vdev_id_list, num_ml, ml_idx, affected_links,
			    ml_vdev_lst)) {
			mode = MLO_LINK_FORCE_MODE_INACTIVE;
		} else {
			policy_mgr_debug(
				"vdev %d: force inactivity is not allowed",
				ml_vdev_lst[0]);
			return;
		}
	}

	policy_mgr_mlo_sta_set_link(psoc, MLO_LINK_FORCE_REASON_CONNECT, mode,
				    affected_links, ml_vdev_lst);
}

static void policy_mgr_get_disabled_ml_sta_idx(struct wlan_objmgr_psoc *psoc,
					       uint8_t *ml_sta, uint8_t *ml_idx,
					       qdf_freq_t *freq_list,
					       uint8_t *vdev_id_list,
					       uint8_t next_idx)
{
					  ...
					  struct wlan_objmgr_vdev *vdev)
{
   ...
   */
	...
	policy_mgr_get_disabled_ml_sta_idx(psoc, &num_ml, ml_idx, freq_list,
					   vdev_id_list, next_idx);
   ...
   */
	...
	policy_mgr_ml_sta_concurrency_on_connect(psoc, vdev, num_ml, ml_idx,
						 num_non_ml, non_ml_idx,
						 freq_list, vdev_id_list);
	return QDF_STATUS_SUCCESS;
}

static bool policy_mgr_is_mode_p2p_sap(enum policy_mgr_con_mode mode)
{
	return (mode == PM_SAP_MODE) || (mode == PM_P2P_CLIENT_MODE) ||
	       (mode == PM_P2P_GO_MODE);
}

bool policy_mgr_is_vdev_high_tput_or_low_latency(struct wlan_objmgr_psoc *psoc,
						 uint8_t vdev_id)
{
	struct wlan_objmgr_vdev *vdev;
	bool is_vdev_ll_ht;
...
bool policy_mgr_check_2ghz_only_sap_affected_link(struct wlan_objmgr_psoc *psoc,
						  uint8_t sap_vdev_id,
 ...
 */
static uint8_t policy_mgr_get_affected_links_for_go_sap_cli(
	struct wlan_objmgr_psoc *psoc, uint8_t num_ml_sta, uint8_t *ml_vdev_lst,
	qdf_freq_t *ml_freq_lst, uint8_t num_p2p_sap, uint8_t *p2p_sap_vdev_lst,
	...
	while (i < num_ml_sta) {
       ...
       */
			if (policy_mgr_check_2ghz_only_sap_affected_link(
				    ...
				    p2p_sap_freq_lst[k], num_ml_sta,
				    ml_freq_lst)) {
				policy_mgr_debug(
					"2G only SAP vdev %d ch freq %d is not SCC with any MLO STA link",
					p2p_sap_vdev_lst[k],
					p2p_sap_freq_lst[k]);
				num_affected_links++;
				continue;
			...
			if (policy_mgr_are_2_freq_on_same_mac(
				    ...
				    p2p_sap_freq_lst[k])) {
				policy_mgr_debug(
					"ml sta vdev %d (freq %d) and p2p/sap vdev %d (freq %d) are MCC",
 ...
 * Return: void
 */
static void policy_mgr_get_ml_sta_and_p2p_cli_go_sap_info(
	struct policy_mgr_psoc_priv_obj *pm_ctx, uint8_t *num_ml_sta,
	uint8_t *num_disabled_ml_sta, uint8_t *ml_vdev_lst,
	qdf_freq_t *ml_freq_lst, uint8_t *num_p2p_sap,
	uint8_t *p2p_sap_vdev_lst, qdf_freq_t *p2p_sap_freq_lst)
{
	enum policy_mgr_con_mode mode;
	uint8_t vdev_id, conn_index;
	qdf_freq_t freq;
 ...
 */
bool policy_mgr_is_ml_sta_links_in_mcc(struct wlan_objmgr_psoc *psoc,
				       qdf_freq_t *ml_freq_lst,
				       uint8_t *ml_vdev_lst,
	...
	for (i = 0; i < num_ml_sta; i++) {
		...
		for (j = i + 1; j < num_ml_sta; j++) {
			if (ml_freq_lst[i] != ml_freq_lst[j] &&
				...
				if (ml_vdev_lst)
					policy_mgr_debug(
						"vdev %d and %d are in MCC with freq %d and freq %d",
						ml_vdev_lst[i], ml_vdev_lst[j],
						ml_freq_lst[i], ml_freq_lst[j]);
				if (ml_linkid_lst) {
					link_id_bitmap |= 1 << ml_linkid_lst[j];
					policy_mgr_debug(
						"link %d and %d are in MCC with freq %d and freq %d",
						ml_linkid_lst[i],
						ml_linkid_lst[j],
						ml_freq_lst[i], ml_freq_lst[j]);
 ...
 */
static QDF_STATUS policy_mgr_handle_mcc_ml_sta(struct wlan_objmgr_psoc *psoc,
					       struct wlan_objmgr_vdev *vdev)
{
	uint8_t num_ml_sta = 0, num_disabled_ml_sta = 0;
	uint8_t ml_sta_vdev_lst[MAX_NUMBER_OF_CONC_CONNECTIONS] = { 0 };
	...
	if (num_ml_sta < 2 || num_ml_sta > MAX_NUMBER_OF_CONC_CONNECTIONS ||
	    num_disabled_ml_sta) {
		policy_mgr_debug(
			"num_ml_sta invalid %d or link already disabled%d",
			num_ml_sta, num_disabled_ml_sta);
		return QDF_STATUS_E_FAILURE;
	}

	if (!policy_mgr_is_ml_sta_links_in_mcc(
		    psoc, ml_freq_lst, ml_sta_vdev_lst, NULL, num_ml_sta, NULL))
		return QDF_STATUS_E_FAILURE;
   ...
   */
	...
	policy_mgr_mlo_sta_set_link(psoc, MLO_LINK_FORCE_REASON_CONNECT,
				    MLO_LINK_FORCE_MODE_ACTIVE_NUM, num_ml_sta,
				    ml_sta_vdev_lst);
	return QDF_STATUS_SUCCESS;
}
 ...
 */
static bool policy_mgr_sta_ml_link_enable_allowed(struct wlan_objmgr_psoc *psoc,
					   ...
					   struct wlan_objmgr_vdev *vdev)
{
   ...
   */
	if (policy_mgr_is_mlo_in_mode_emlsr(psoc, NULL, NULL)) {
		policy_mgr_debug(
			"STA connected in eMLSR mode, don't enable/disable links");
		return;
	}
	...
	policy_mgr_get_ml_sta_and_p2p_cli_go_sap_info(
		pm_ctx, &num_ml_sta, &num_disabled_ml_sta, ml_sta_vdev_lst,
		ml_freq_lst, &num_p2p_sap, p2p_sap_vdev_lst, p2p_sap_freq_lst);
	...
	num_affected_link = policy_mgr_get_affected_links_for_go_sap_cli(
		psoc, num_ml_sta, ml_sta_vdev_lst, ml_freq_lst, num_p2p_sap,
		p2p_sap_vdev_lst, p2p_sap_freq_lst);
...
void policy_mgr_handle_ml_sta_links_on_vdev_up_csa(
	struct wlan_objmgr_psoc *psoc, enum QDF_OPMODE mode, uint8_t vdev_id)
{
	struct wlan_objmgr_vdev *vdev;
...
QDF_STATUS policy_mgr_wait_for_set_link_update(struct wlan_objmgr_psoc *psoc)
{
	...
	status = qdf_wait_for_event_completion(
		&pm_ctx->set_link_update_done_evt, SET_LINK_TIMEOUT);
						 ...
						 struct wlan_objmgr_vdev *vdev)
{
   ...
   */
	if (!num_disabled_ml_sta ||
		...
		if (num_disabled_ml_sta)
			policy_mgr_debug(
				"Not re-enabled due to disallowed concurrency");
		goto done;
	}

	policy_mgr_mlo_sta_set_link(psoc, MLO_LINK_FORCE_REASON_DISCONNECT,
				    MLO_LINK_FORCE_MODE_NO_FORCE, num_ml_sta,
				    ml_sta_vdev_lst);
	...
	if (num_p2p_sap)
		num_affected_link =
			policy_mgr_get_affected_links_for_go_sap_cli(
				psoc, num_ml_sta, ml_sta_vdev_lst, ml_freq_lst,
				num_p2p_sap, p2p_sap_vdev_lst,
				p2p_sap_freq_lst);

	if (num_affected_link)
		policy_mgr_debug(
			"vdev %d: Affected link present, dont reanabe ML link",
			vdev_id);
	else
		policy_mgr_mlo_sta_set_link(psoc,
					    MLO_LINK_FORCE_REASON_DISCONNECT,
 ...
 * Return: void
 */
static void policy_mgr_pick_link_vdev_from_inactive_list(
	struct wlan_objmgr_psoc *psoc, struct wlan_objmgr_vdev *vdev,
	...
	uint8_t *non_removed_vdev_id)
{
	...
	policy_mgr_store_and_del_conn_info_by_vdev_id(
		psoc, wlan_vdev_get_id(vdev), info, &num_del);
	...
	for (i = 0; i < inactive_vdev_num; i++) {
		...
		partner_vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
			psoc, inactive_vdev_lst[i], WLAN_POLICY_MGR_ID);
		if (!partner_vdev) {
		...
		*non_removed_vdev_id = inactive_vdev_lst[i];
		...
		if (policy_mgr_is_concurrency_allowed(
			    psoc, PM_STA_MODE, inactive_freq_lst[i],
			    HW_MODE_20_MHZ, conc_ext_flags.value, NULL)) {
...
void policy_mgr_handle_link_removal_on_vdev(struct wlan_objmgr_vdev *vdev)
{
	...
	wlan_set_vdev_link_removed_flag_by_vdev_id(psoc, vdev_id, true);
	status = policy_mgr_handle_ml_sta_link_state_allowed(
   ...
   */
	...
	policy_mgr_debug(
		"removal link vdev %d num_ml_sta %d num_disabled_ml_sta %d",
     ...
     */
		policy_mgr_mlo_sta_set_link(psoc,
					    ...
					    MLO_LINK_FORCE_MODE_INACTIVE, 1,
					    &vdev_id);
		return;
	}
	...
	if (num_active_ml_sta < num_ml_sta)
		policy_mgr_pick_link_vdev_from_inactive_list(
			...
			&ml_freq_lst[num_active_ml_sta], &picked_vdev_id,
     ...
     */
		policy_mgr_debug(
			"active parnter vdev %d, inactive removal vdev %d",
			picked_vdev_id, vdev_id);
		policy_mgr_mlo_sta_set_link_ext(
			psoc, MLO_LINK_FORCE_REASON_LINK_REMOVAL,
			MLO_LINK_FORCE_MODE_ACTIVE_INACTIVE, 1, &picked_vdev_id,
			1, &vdev_id);
		return;
	}
     ...
     */
		if (non_removal_link_vdev_id != WLAN_INVALID_VDEV_ID) {
			policy_mgr_debug(
				"trigger roaming, non_removal_link_vdev_id %d",
				non_removal_link_vdev_id);
			policy_mgr_trigger_roam_on_link_removal(vdev);
		}
		return;
   ...
   */
	policy_mgr_mlo_sta_set_link(psoc, MLO_LINK_FORCE_REASON_LINK_REMOVAL,
				    MLO_LINK_FORCE_MODE_INACTIVE, 1, &vdev_id);
}
						...
						qdf_freq_t sap_ch_freq)
{
	...
	if (num_ml_active_sta >= 2 && !same_freq_with_mlo_sta) {
		policy_mgr_debug(
			"SAP is not SCC with any of active MLO STA link, restart SAP");
				   ...
				   struct qdf_mac_addr active_link_addr[2])
{
   ...
   */
	if (active_vdev_cnt && inactive_vdev_cnt)
		policy_mgr_mlo_sta_set_link_ext(
			psoc, MLO_LINK_FORCE_REASON_CONNECT,
			MLO_LINK_FORCE_MODE_ACTIVE_INACTIVE, active_vdev_cnt,
			active_vdev_lst, inactive_vdev_cnt, inactive_vdev_lst);
					    ...
					    uint32_t *config_state_list)
{
	...
	policy_mgr_debug(
		"vdev: %d, active links: %d, ml count: %d, active count: "
		"%d, inactive count: %d",
   ...
   */
     ...
     */
		if (active_vdev_cnt && inactive_vdev_cnt) {
			status = policy_mgr_mlo_sta_set_link_ext(
				psoc, MLO_LINK_FORCE_REASON_CONNECT,
     ...
     */
		if (active_vdev_cnt &&
		    !policy_mgr_sta_ml_link_enable_allowed(
			    psoc, num_disabled_ml_sta, num_ml_sta, ml_freq_lst,
			    ml_sta_vdev_lst)) {
     ...
     */
		if (active_vdev_cnt && inactive_vdev_cnt)
			...
			status = policy_mgr_mlo_sta_set_link(
				psoc, MLO_LINK_FORCE_REASON_DISCONNECT,
				MLO_LINK_FORCE_MODE_ACTIVE, active_vdev_cnt,
				active_vdev_lst);
	}
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS policy_mgr_process_mlo_sta_dynamic_force_num_link(
	struct wlan_objmgr_psoc *psoc, enum mlo_link_force_reason reason,
	enum mlo_link_force_mode mode, uint8_t num_mlo_vdev,
	uint8_t *mlo_vdev_lst, uint8_t force_active_cnt)
{
	struct mlo_link_set_active_req *req;
	QDF_STATUS status;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
   ...
   */
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err(
			"vdev %d: Failed to set link mode %d num_mlo_vdev %d "
			"force_active_cnt: %d, reason %d",
			wlan_vdev_get_id(vdev), mode, num_mlo_vdev,
			force_active_cnt, reason);
		qdf_mem_free(req);
		policy_mgr_set_link_in_progress(pm_ctx, false);
	}
	wlan_objmgr_vdev_release_ref(vdev, WLAN_POLICY_MGR_ID);
...
set_link:
	status = policy_mgr_process_mlo_sta_dynamic_force_num_link(
		psoc, MLO_LINK_FORCE_REASON_CONNECT,
		MLO_LINK_FORCE_MODE_ACTIVE_NUM, num_ml_sta, ml_sta_vdev_lst,
		force_active_cnt);
	if (QDF_IS_STATUS_SUCCESS(status))
					 ...
					 uint8_t vdev_id)
{
   ...
   */
	status = policy_mgr_process_mlo_sta_dynamic_force_num_link(
		psoc, MLO_LINK_FORCE_REASON_CONNECT,
		MLO_LINK_FORCE_MODE_ACTIVE_NUM, num_link_to_no_force,
		ml_sta_vdev_lst, 0);
	if (QDF_IS_STATUS_SUCCESS(status))
...
static bool policy_mgr_is_third_conn_sta_p2p_p2p_valid(
	struct wlan_objmgr_psoc *psoc, enum policy_mgr_con_mode new_conn_mode)
{
	int num_sta, num_go, num_cli;

	num_sta = policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
							    NULL);

	num_go = policy_mgr_mode_specific_connection_count(psoc, PM_P2P_GO_MODE,
							   NULL);

	num_cli = policy_mgr_mode_specific_connection_count(
...
static bool
policy_mgr_is_sap_go_allowed_with_ll_sap(struct wlan_objmgr_psoc *psoc,
					 qdf_freq_t freq,
					 enum policy_mgr_con_mode mode)
{
	/**
				       ...
				       struct policy_mgr_pcl_list *pcl)
{
     ...
     */
		if (!policy_mgr_is_5g_channel_allowed(psoc, ch_freq, list,
						      PM_P2P_GO_MODE))
			return status;
		if (!policy_mgr_is_5g_channel_allowed(psoc, ch_freq, list,
						      PM_SAP_MODE))
			return status;
		if (!policy_mgr_is_6g_channel_allowed(psoc, mode, ch_freq))
			return status;
		...
		if ((mode == PM_SAP_MODE || mode == PM_P2P_GO_MODE) &&
			...
			if (is_dfs_ch)
				match = policy_mgr_disallow_mcc(psoc, ch_freq);
		}
		if (true == match) {
			policy_mgr_rl_debug(
				"No MCC, SAP/GO about to come up on DFS channel");
			return status;
		}
		if ((policy_mgr_is_hw_dbs_capable(psoc) != true) &&
			...
			if (WLAN_REG_IS_24GHZ_CH_FREQ(ch_freq)) {
				if (policy_mgr_is_sap_p2pgo_on_dfs(psoc)) {
					policy_mgr_rl_debug(
						"MCC not allowed: SAP/P2PGO on DFS");
					return status;
				}
			}
		}
	...
	if (!policy_mgr_allow_sap_go_concurrency(psoc, mode, ch_freq,
		...
		policy_mgr_rl_debug(
			"This concurrency combination is not allowed");
		return status;
	}
   ...
   */
	if (ch_freq && mode == PM_P2P_GO_MODE && num_connections &&
		...
		while (index < count) {
			if (WLAN_REG_IS_SAME_BAND_FREQS(
				    ...
				    pm_conc_connection_list[list[index]].freq)) {
				policy_mgr_rl_debug(
					"Don't allow P2P GO on same band");
				qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
				return status;
			}
			index++;
	...
	if (!policy_mgr_allow_wapi_concurrency(pm_ctx)) {
		policy_mgr_rl_debug(
			"Don't allow new conn when wapi security conn existing");
		return status;
	}
	...
	if (!policy_mgr_is_third_conn_sta_p2p_p2p_valid(psoc, mode)) {
		policy_mgr_err(
			"Don't allow third connection as GO or GC or STA with old fw");
		return status;
	}
				      ...
				      enum sap_csa_reason_code reason)
{
	...
	policy_mgr_debug(
		"check concurrency_csa vdev:%d ch %d bw %d, forced %d, reason %d",
		vdev_id, ch_freq, bw, forced, reason);

	status = policy_mgr_get_chan_by_session_id(psoc, vdev_id, &old_ch_freq);
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err("Failed to get channel for vdev:%d", vdev_id);
		return allow;
	}
   ...
   */
	...
	if (forced &&
	    (reason == CSA_REASON_UNSAFE_CHANNEL || reason == CSA_REASON_DCS))
...
uint32_t policy_mgr_get_concurrency_mode(struct wlan_objmgr_psoc *psoc)
{
	...
	policy_mgr_debug("concurrency_mode: 0x%x", pm_ctx->concurrency_mode);
 ...
 */
QDF_STATUS
policy_mgr_change_mcc_go_beacon_interval(struct wlan_objmgr_psoc *psoc,
					 uint8_t vdev_id,
	...
	if (QDF_SAP_MODE == dev_mode) {
		if (pm_ctx->sme_cbacks.sme_change_mcc_beacon_interval) {
			...
			if (status == QDF_STATUS_E_FAILURE) {
				policy_mgr_err(
					"Failed to update Beacon Params");
				return QDF_STATUS_E_FAILURE;
			}
		} else {
			policy_mgr_err(
				"sme_change_mcc_beacon_interval callback is NULL");
			return QDF_STATUS_E_FAILURE;
		}
	}
				     ...
				     uint8_t vdev_id)
{
	...
	case QDF_NDI_MODE:
		...
		break;
	default:
		policy_mgr_debug("Unsupported mode (%d)", device_mode);
	}
...
enum QDF_OPMODE
policy_mgr_get_qdf_mode_from_pm(enum policy_mgr_con_mode device_mode)
{
...
QDF_STATUS
policy_mgr_mode_specific_num_open_sessions(struct wlan_objmgr_psoc *psoc,
					   enum QDF_OPMODE mode,
...
QDF_STATUS
policy_mgr_mode_specific_num_active_sessions(struct wlan_objmgr_psoc *psoc,
					     enum QDF_OPMODE mode,
					     uint8_t *num_sessions)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
 ...
 */
bool policy_mgr_concurrent_open_sessions_running(struct wlan_objmgr_psoc *psoc)
{
	uint8_t i = 0;
	uint8_t j = 0;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
{
	return policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
							 NULL) > 1;
}
 ...
 */
bool policy_mgr_is_sta_active_connection_exists(struct wlan_objmgr_psoc *psoc)
{
	return (!policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
							   NULL)) ?
		       false :
...
uint32_t policy_mgr_get_dfs_beaconing_session_id(struct wlan_objmgr_psoc *psoc)
{
					      ...
					      qdf_freq_t *freq)
{
	...
	*freq = dfs_ch_frq;
	...
	if (policy_mgr_is_sta_present_on_dfs_channel(
		    psoc, &vdev_id, &dfs_sta_frq, &ch_sta_width) &&
   ...
   */
	...
	policy_mgr_debug(
		"scan skip 5g chan due to dfs ap(ch %d / ch_width %d) present",
		dfs_ch_frq, ch_width);

	return true;
}

static void policy_mgr_fill_trim_chan(struct wlan_objmgr_pdev *pdev,
				      void *object, void *arg)
{
	...
	policy_mgr_debug("vdev %d - peer count %d", wlan_vdev_get_id(vdev),
			 sap_peer_count);
	if (sap_peer_count <= 1)
		return;
...
policy_mgr_scan_trim_chnls_for_connected_ap(struct wlan_objmgr_pdev *pdev)
{
	...
	status =
		wlan_mlme_get_band_capability(psoc, &trim_info.band_capability);
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err("Could not get band capability");
		return TRIM_CHANNEL_LIST_NONE;
	}

	trim_info.sap_count = policy_mgr_mode_specific_connection_count(
				       ...
				       uint8_t *nss_2g, uint8_t *nss_5g)
{
	...
	if (pm_ctx->sme_cbacks.sme_get_nss_for_vdev) {
		pm_ctx->sme_cbacks.sme_get_nss_for_vdev(dev_mode, nss_2g,
							nss_5g);
...
void policy_mgr_dump_connection_status_info(struct wlan_objmgr_psoc *psoc)
{
	...
	for (i = 0; i < MAX_NUMBER_OF_CONC_CONNECTIONS; i++) {
		if (!pm_conc_connection_list[i].in_use)
			continue;
		policy_mgr_debug(
			"%d: use:%d vdev:%d mode:%d mac:%d freq:%d orig chainmask:%d orig "
			"nss:%d bw:%d, ch_flags %0X",
	...
	enum policy_mgr_band band)
{
	...
	for (i = 0; i < MAX_NUMBER_OF_CONC_CONNECTIONS; i++) {
		...
		case POLICY_MGR_BAND_24:
			if ((pm_conc_connection_list[i].vdev_id !=
			     session_id) &&
			    (pm_conc_connection_list[i].in_use) &&
...
enum phy_ch_width policy_mgr_get_bw_by_session_id(struct wlan_objmgr_psoc *psoc,
						  uint8_t session_id)
{
	uint32_t i;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
					    ...
					    uint32_t *list, uint8_t mac_id)
{
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		if (pm_conc_connection_list[conn_index].mac == mac_id &&
			...
			if (list)
				list[count] =
					pm_conc_connection_list[conn_index]
						.vdev_id;
			count++;
		}
	}
					  ...
					  uint32_t ch_freq, bool *ok)
{
	...
	cc_count = policy_mgr_get_mode_specific_conn_info(
		psoc, &op_ch_freq_list[cc_count], &vdev_id[cc_count],
		PM_SAP_MODE);
	...
	for (i = 0; i < pm_ctx->num_dbs_hw_modes; i++) {
		...
		if (dbs) {
			...
			if (final_max_rf_chains < max_rf_chains) {
				final_max_rf_chains = (max_rf_chains == 2) ?
							      HW_MODE_SS_2x2 :
							      HW_MODE_SS_1x1;

				nss_dbs->mac0_ss = (min_mac0_rf_chains == 2) ?
							   HW_MODE_SS_2x2 :
							   HW_MODE_SS_1x1;

				nss_dbs->mac1_ss = (min_mac1_rf_chains == 2) ?
							   HW_MODE_SS_2x2 :
							   HW_MODE_SS_1x1;
			}
		} else {
	...
	uint32_t channel_select_logic_conc)
{
	...
	*fw_mode_config = pm_ctx->dual_mac_cfg.cur_fw_mode_config;
	...
	WMI_DBS_FW_MODE_CFG_DBS_FOR_STA_PLUS_STA_SET(
		*fw_mode_config,
		PM_CHANNEL_SELECT_LOGIC_STA_STA_GET(channel_select_logic_conc));
	WMI_DBS_FW_MODE_CFG_DBS_FOR_STA_PLUS_P2P_SET(
		*fw_mode_config,
					   ...
					   uint8_t vdev_id, qdf_freq_t ch_freq)
{
	...
	sta_cnt = policy_mgr_mode_specific_connection_count(psoc, PM_STA_MODE,
							    NULL);
	gc_cnt = policy_mgr_mode_specific_connection_count(
		psoc, PM_P2P_CLIENT_MODE, NULL);
   ...
   */
	if ((wlan_reg_get_channel_state_for_pwrmode(pdev, ch_freq,
						    REG_CURRENT_PWR_MODE) ==
	     CHANNEL_STATE_DFS) &&
	    !sta_cnt && !gc_cnt && sta_sap_scc_on_dfs_chan &&
	    !policy_mgr_get_dfs_master_dynamic_enabled(psoc, vdev_id)) {
		policy_mgr_err(
			"SAP not allowed on DFS channel if no dfs master capability!!");
		return false;
	}
...
bool policy_mgr_is_sap_go_interface_allowed_on_indoor(
	struct wlan_objmgr_pdev *pdev, uint8_t vdev_id, qdf_freq_t ch_freq)
{
	struct wlan_objmgr_psoc *psoc;
   ...
   */
	...
	if (WLAN_REG_IS_6GHZ_CHAN_FREQ(ch_freq)) {
		policy_mgr_rl_debug(
			"SAP operation is not allowed on 6 GHz indoor channel");
		return false;
	}
	...
	if (mode == QDF_SAP_MODE) {
		if (is_scc &&
			...
			return true;
		policy_mgr_rl_debug(
			"SAP operation is not allowed on indoor channel");
		return false;
	}
	...
	if (mode == QDF_P2P_GO_MODE) {
		if (ucfg_p2p_get_indoor_ch_support(psoc) ||
			...
			return true;
		policy_mgr_rl_debug(
			"GO operation is not allowed on indoor channel");
		return false;
	}
...
bool policy_mgr_is_sta_sap_scc_allowed_on_dfs_chan(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
bool policy_mgr_is_multi_sap_allowed_on_same_band(struct wlan_objmgr_pdev *pdev,
						  enum policy_mgr_con_mode mode,
						  qdf_freq_t ch_freq)
{
	struct wlan_objmgr_psoc *psoc;
	...
	status = policy_mgr_get_multi_sap_allowed_on_same_band(
		psoc, &multi_sap_allowed_on_same_band);
	if (!QDF_IS_STATUS_SUCCESS(status)) {
	...
	if (!multi_sap_allowed_on_same_band) {
		...
		ap_cnt = policy_mgr_mode_specific_connection_count(
					 ...
					 uint32_t ch_freq, uint32_t vdev_id)
{
	...
	for (id = 0; id < MAX_NUMBER_OF_CONC_CONNECTIONS; id++) {
		...
		if (policy_mgr_dual_beacon_on_single_mac_scc_capable(psoc) &&
		    (ch_freq == con_freq)) {
			policy_mgr_debug(
				"SCC enabled, 2 AP on same channel, allow 2nd AP");
			return true;
		}
		if (!dbs) {
			policy_mgr_debug(
				"DBS unsupported, mcc and scc unsupported too, don't allow 2nd AP");
			return false;
		}
		...
		if (policy_mgr_are_2_freq_on_same_mac(psoc, ch_freq,
						      con_freq)) {
			policy_mgr_debug(
				"DBS supported, 2 SAP on same band, reject 2nd AP");
			return false;
		}
	}
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	if (wmi_service_enabled(
		...
		policy_mgr_debug(
			"Dual beaconing on same channel on single MAC supported");
		return true;
	}
	policy_mgr_debug(
		"Dual beaconing on same channel on single MAC is not supported");
	return false;
}
	...
	struct wlan_objmgr_psoc *psoc)
{
	...
	if (wmi_service_enabled(
		...
		policy_mgr_debug(
			"Dual beaconing on different channel on single MAC supported");
		return true;
	}
	policy_mgr_debug(
		"Dual beaconing on different channel on single MAC is not supported");
	return false;
}

bool policy_mgr_sta_sap_scc_on_lte_coex_chan(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
				     ...
				     enum conn_6ghz_flag ap_6ghz_capable)
{
	...
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
	policy_mgr_debug("vdev %d init conn_6ghz_flag %x new %x", vdev_id,
				    ...
				    enum conn_6ghz_flag ap_6ghz_capable)
{
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		if (conn_info->in_use &&
		    ...
		    policy_mgr_is_6ghz_conc_mode_supported(psoc,
							   conn_info->mode) &&
		    vdev_id == conn_info->vdev_id) {
			if (set)
				conn_info->conn_6ghz_flag |= ap_6ghz_capable;
	...
	qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
	policy_mgr_debug("vdev %d %s conn_6ghz_flag %x new %x", vdev_id,
			 set ? "set" : "clr", ap_6ghz_capable, conn_6ghz_flag);
}

bool policy_mgr_get_ap_6ghz_capable(struct wlan_objmgr_psoc *psoc,
				    uint8_t vdev_id, uint32_t *conn_flag)
{
	struct policy_mgr_conc_connection_info *conn_info;
	uint32_t conn_index;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
					    ...
					    uint32_t ch_freq)
{
	...
	sap_count = policy_mgr_mode_specific_connection_count(psoc, PM_SAP_MODE,
							      NULL);
	state = wlan_reg_get_channel_state_for_pwrmode(pm_ctx->pdev, ch_freq,
						       REG_CURRENT_PWR_MODE);

	policy_mgr_debug(
		"sta_sap_scc_on_dfs_chan %u, sap_count %u, ch freq %u, state %u",
...
bool policy_mgr_is_sta_sap_scc(struct wlan_objmgr_psoc *psoc, uint32_t sap_freq)
{
	uint32_t conn_index;
	bool is_scc = false;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		if (pm_conc_connection_list[conn_index].in_use &&
		    (pm_conc_connection_list[conn_index].mode == PM_STA_MODE ||
		     pm_conc_connection_list[conn_index].mode ==
			     PM_P2P_CLIENT_MODE) &&
		    (sap_freq == pm_conc_connection_list[conn_index].freq)) {
			is_scc = true;
			break;
		}
	}
					...
					tQDF_MCC_TO_SCC_SWITCH_MODE scc_mode)
{
   ...
   */
	if (num_mcc_conn > 1 && !num_scc_conn) {
		policy_mgr_debug(
			"sap vdev %d has chan %d diff with %d exsting conn",
	...
	for (i = 0; i < MAX_NUMBER_OF_CONC_CONNECTIONS; i++) {
		is_sta_p2p_cli = connection[i].in_use &&
		...
		if (connection[i].freq != freq &&
		    policy_mgr_are_2_freq_on_same_mac(psoc, freq,
						      connection[i].freq)) {
			policy_mgr_debug(
				"SAP:%d and STA:%d on same mac. Restart SAP ",
				freq, connection[i].freq);
			restart_required = true;
			break;
		}
		if (connection[i].freq == freq && !sta_sap_scc_on_dfs_chan &&
		    sap_on_dfs) {
     ...
     */
		...
		if (connection[i].freq != freq &&
			...
			policy_mgr_debug(
				"Move SAP from:%d to STA ch:%d  (sap start freq:%d)",
				freq, connection[i].freq, user_config_freq);
			restart_required = true;
	...
	if (!restart_required &&
	    policy_mgr_is_restart_sap_required_with_mlo_sta(psoc, vdev_id,
							    freq))
		restart_required = true;
...
uint8_t
policy_mgr_get_roam_enabled_sta_session_id(struct wlan_objmgr_psoc *psoc,
					   uint8_t vdev_id)
{
	uint32_t list[MAX_NUMBER_OF_CONC_CONNECTIONS];
	uint32_t index, count;
	...
	if (wlan_vdev_mlme_is_link_sta_vdev(vdev)) {
		...
		if (assoc_vdev && ucfg_cm_is_vdev_active(assoc_vdev)) {
			policy_mgr_debug(
				"replace link vdev %d with assoc vdev %d",
...
bool policy_mgr_is_sta_mon_concurrency(struct wlan_objmgr_psoc *psoc)
{
	...
	if (wlan_mlme_is_sta_mon_conc_supported(psoc)) {
		...
		if (conc_mode & QDF_STA_MASK && conc_mode & QDF_MONITOR_MASK) {
...
QDF_STATUS policy_mgr_check_mon_concurrency(struct wlan_objmgr_psoc *psoc)
{
	...
	if (policy_mgr_mode_specific_num_open_sessions(psoc, QDF_MONITOR_MODE,
						       &num_open_session) !=
	    QDF_STATUS_SUCCESS)
		return QDF_STATUS_E_INVAL;
	...
	if (num_open_session) {
		policy_mgr_err(
			"monitor mode already exists, only one is possible");
	...
	if (num_open_session) {
		policy_mgr_err(
			"cannot add monitor mode, due to SAP concurrency");
		return QDF_STATUS_E_INVAL;
	}
	...
	if (num_open_session) {
		policy_mgr_err(
			"cannot add monitor mode, due to P2P CLIENT concurrency");
		return QDF_STATUS_E_INVAL;
	}
	...
	if (num_open_session) {
		policy_mgr_err(
			"cannot add monitor mode, due to P2P GO concurrency");
		return QDF_STATUS_E_INVAL;
	}

	num_open_session = policy_mgr_mode_specific_connection_count(
		psoc, PM_NAN_DISC_MODE, NULL);

	if (num_open_session) {
		policy_mgr_err(
			"cannot add monitor mode, due to NAN concurrency");
		return QDF_STATUS_E_INVAL;
	}
...
bool policy_mgr_is_ap_ap_mcc_allow(struct wlan_objmgr_psoc *psoc,
				   struct wlan_objmgr_pdev *pdev,
				   struct wlan_objmgr_vdev *vdev,
				   uint32_t ch_freq, enum phy_ch_width ch_width,
				   uint8_t *con_vdev_id, uint32_t *con_freq)
{
	enum QDF_OPMODE mode;
	enum policy_mgr_con_mode con_mode;
	union conc_ext_flag conc_ext_flags;
	...
	cc_count = policy_mgr_get_mode_specific_conn_info(
		psoc, &op_freq[0], &vdev_id[0], PM_SAP_MODE);
	if (cc_count < MAX_NUMBER_OF_CONC_CONNECTIONS)
		cc_count = cc_count + policy_mgr_get_mode_specific_conn_info(
					      psoc, &op_freq[cc_count],
					      &vdev_id[cc_count],
					      PM_P2P_GO_MODE);
	if (!cc_count)
		return true;
   ...
   */
	...
	if (!policy_mgr_allow_concurrency(
		    psoc, con_mode, ch_freq, policy_mgr_get_bw(ch_width),
		    conc_ext_flags.value, wlan_vdev_get_id(vdev))) {
		policy_mgr_debug(
			"AP AP mcc not allowed, try to override 2nd SAP/GO chan");
		return false;
	}
...
bool policy_mgr_any_other_vdev_on_same_mac_as_freq(
	struct wlan_objmgr_psoc *psoc, uint32_t freq, uint8_t vdev_id)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t conn_index = 0;
	bool same_mac = false;
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		if (policy_mgr_are_2_freq_on_same_mac(
			    psoc, pm_conc_connection_list[conn_index].freq,
			    freq)) {
			same_mac = true;
			break;
		}
					      ...
					      enum ll_ap_type ap_type)
{
	...
	for (conn_idx = 0; conn_idx < MAX_NUMBER_OF_CONC_CONNECTIONS;
		...
		policy_mgr_debug(
			"LL SAP %d present with vdev_id %d and freq %d",
					    ...
					    enum policy_mgr_con_mode mode)
{
   ...
   */
	...
	if (policy_mgr_2_freq_always_on_same_mac(psoc, ll_sap_freq, freq)) {
		policy_mgr_debug(
			"Invalid LL-SAP concurrency for SBS/DBS hw, ll-sap freq "
			"%d, conc_freq %d, conc_mode %d",
...
bool policy_mgr_update_indoor_concurrency(struct wlan_objmgr_psoc *psoc,
					  uint8_t vdev_id, uint32_t discon_freq,
   ...
   */
	...
	case SWITCH_WITHOUT_CONCURRENCY:
		...
		if (wlan_reg_is_freq_indoor(pm_ctx->pdev, ch_freq))
			wlan_reg_modify_indoor_concurrency(
				pm_ctx->pdev, vdev_id, ch_freq, ch_width, true);
		break;
	case DISCONNECT_WITH_CONCURRENCY:
