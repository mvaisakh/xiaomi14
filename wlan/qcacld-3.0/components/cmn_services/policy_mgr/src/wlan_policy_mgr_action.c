...
void policy_mgr_hw_mode_transition_cb(
	uint32_t old_hw_mode_index, uint32_t new_hw_mode_index,
	uint32_t num_vdev_mac_entries,
	struct policy_mgr_vdev_mac_map *vdev_mac_map, uint32_t num_mac_freq,
	...
	struct wlan_objmgr_psoc *context)
{
	...
	policy_mgr_update_hw_mode_conn_info(context, num_vdev_mac_entries,
					    vdev_mac_map, hw_mode, num_mac_freq,
					    mac_freq_range);
...
QDF_STATUS
policy_mgr_check_n_start_opportunistic_timer(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	if (!pm_ctx) {
		policy_mgr_err(
			"PM ctx not valid. Oppurtunistic timer cannot start");
		return QDF_STATUS_E_FAILURE;
	}
	if (policy_mgr_need_opportunistic_upgrade(psoc, NULL)) {
		/* let's start the timer */
		qdf_mc_timer_stop(&pm_ctx->dbs_opportunistic_timer);
		status = qdf_mc_timer_start(&pm_ctx->dbs_opportunistic_timer,
					    DBS_OPPORTUNISTIC_TIME * 1000);
		if (!QDF_IS_STATUS_SUCCESS(status))
			policy_mgr_err(
				"Failed to start dbs opportunistic timer");
	}
	return status;
}

QDF_STATUS policy_mgr_pdev_set_hw_mode(
	struct wlan_objmgr_psoc *psoc, uint32_t session_id,
	enum hw_mode_ss_config mac0_ss, enum hw_mode_bandwidth mac0_bw,
	enum hw_mode_ss_config mac1_ss, enum hw_mode_bandwidth mac1_bw,
	...
	enum policy_mgr_conn_update_reason reason, uint8_t next_action,
	enum policy_mgr_conc_next_action action, uint32_t request_id)
{
   ...
   */
	if ((HW_MODE_SS_2x2 == mac0_ss) && (!pm_ctx->user_cfg.enable2x2)) {
		policy_mgr_debug(
			"2x2 is not allowed downgrading to 1x1 for mac0");
		mac0_ss = HW_MODE_SS_1x1;
	}
	if ((HW_MODE_SS_2x2 == mac1_ss) && (!pm_ctx->user_cfg.enable2x2)) {
		policy_mgr_debug(
			"2x2 is not allowed downgrading to 1x1 for mac1");
		mac1_ss = HW_MODE_SS_1x1;
	}

	hw_mode_index = policy_mgr_get_hw_mode_idx_from_dbs_hw_list(
		psoc, mac0_ss, mac0_bw, mac1_ss, mac1_bw, mac0_band_cap, dbs,
		dfs, sbs);
	if (hw_mode_index < 0) {
   ...
   */
	...
	policy_mgr_debug(
		"set hw mode to sme: hw_mode_index: %d session:%d reason:%d "
		"action %d request_id %d",
	...
	enum policy_mgr_conn_update_reason *reason)
{
	...
	for (conn_index = 0; conn_index < MAX_NUMBER_OF_CONC_CONNECTIONS;
	     conn_index++) {
		policy_mgr_debug(
			"index:%d mac:%d in_use:%d chan:%d org_nss:%d",
			conn_index, pm_conc_connection_list[conn_index].mac,
		...
		if ((pm_conc_connection_list[conn_index].mac == 0) &&
			...
			if (policy_mgr_is_hw_dbs_required_for_band(
				...
				qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
				policy_mgr_debug(
					"2X2 DBS capable with 2.4 GHZ connection");
				goto done;
			}
			if (POLICY_MGR_MAC0_AND_MAC1 == mac) {
				qdf_mutex_release(&pm_ctx->qdf_conc_list_lock);
					     ...
					     uint32_t vdev_id)
{
	...
	policy_mgr_update_conc_list(
		...
		conn_table_entry.mac_id, chain_mask, nss, vdev_id, true, true,
	...
	else if (policy_mgr_update_indoor_concurrency(
			 psoc, vdev_id, cur_freq, SWITCH_WITHOUT_CONCURRENCY))
		wlan_reg_recompute_current_chan_list(psoc, pm_ctx->pdev);
...
QDF_STATUS policy_mgr_update_and_wait_for_connection_update(
	struct wlan_objmgr_psoc *psoc, uint8_t session_id, uint32_t ch_freq,
	enum policy_mgr_conn_update_reason reason)
{
	QDF_STATUS status;

	policy_mgr_debug("session:%d ch_freq:%d reason:%d", session_id, ch_freq,
			 reason);
	...
	status = policy_mgr_current_connections_update(
		psoc, session_id, ch_freq, reason, POLICY_MGR_DEF_REQ_ID);
...
bool policy_mgr_is_dbs_allowed_for_concurrency(struct wlan_objmgr_psoc *psoc,
...
policy_mgr_pri_id_to_con_mode(enum policy_mgr_pri_id pri_id)
{
	switch (pri_id) {
	case PM_STA_PRI_ID:
		return PM_STA_MODE;
...
enum policy_mgr_conc_next_action policy_mgr_get_preferred_dbs_action_table(
	struct wlan_objmgr_psoc *psoc, uint32_t vdev_id, uint32_t ch_freq,
	enum policy_mgr_conn_update_reason reason)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
   ...
   */
	...
	if (PM_GET_VDEV_PRIORITY_ENABLED(pm_ctx->cfg.dbs_selection_plcy) == 1 &&
	...
	if (vdev_id != INVALID_VDEV_ID && ch_freq) {
		if (pm_ctx->hdd_cbacks.hdd_get_device_mode)
			new_conn_op_mode =
				pm_ctx->hdd_cbacks.hdd_get_device_mode(vdev_id);

		new_conn_mode = policy_mgr_qdf_opmode_to_pm_con_mode(
			psoc, new_conn_op_mode, vdev_id);
		if (new_conn_mode == PM_MAX_NUM_OF_MODE)
			policy_mgr_debug(
				"new vdev %d op_mode %d freq %d reason %d: not prioritized",
				vdev_id, new_conn_op_mode, ch_freq, reason);
		else
			policy_mgr_debug(
				"new vdev %d op_mode %d freq %d : reason %d",
				vdev_id, new_conn_op_mode, ch_freq, reason);
	}
	...
	while (vdev_pri_list) {
		...
		if (pri_conn_mode == PM_MAX_NUM_OF_MODE) {
			policy_mgr_debug(
				"vdev_pri_id %d prioritization not supported",
				vdev_pri_id);
			goto NEXT;
		}
     ...
     */
		...
		for (i = 0; i < vdev_count; i++) {
			policy_mgr_debug("[%d] vdev %d chan %d conn_mode %d", i,
					 vdev_list[i], ch_freq_list[i],
					 pri_conn_mode);
...
policy_mgr_get_third_conn_action_table(struct wlan_objmgr_psoc *psoc,
				       uint32_t vdev_id, uint32_t ch_freq,
				       enum policy_mgr_conn_update_reason reason)
{
	enum policy_mgr_conc_next_action preferred_action;
			   ...
			   enum policy_mgr_conc_next_action *next_action)
{
	...
	policy_mgr_debug("num_connections=%d freq=%d", num_connections,
			 ch_freq);
   ...
   */
	if (reason == POLICY_MGR_UPDATE_REASON_NAN_DISCOVERY)
		...
		new_conn_mode =
			pm_ctx->hdd_cbacks.hdd_get_device_mode(session_id);
   ...
   */
	if (policy_mgr_is_current_hwmode_dbs(psoc) &&
		...
		*next_action = PM_SINGLE_MAC;
		 ...
		 !policy_mgr_is_dbs_allowed_for_concurrency(psoc,
							    new_conn_mode))
		*next_action = PM_NOP;

	policy_mgr_debug(
		"idx2=%d idx3=%d next_action=%d, band=%d reason=%d session_id=%d",
		second_index, third_index, *next_action, band, reason,
		session_id);

	return QDF_STATUS_SUCCESS;
}

static bool policy_mgr_is_hw_mode_change_required(struct wlan_objmgr_psoc *psoc,
						  uint32_t ch_freq,
						  uint8_t vdev_id)
{
				    ...
				    qdf_list_t *scan_list, uint8_t vdev_id)
{
	...
	if (!policy_mgr_is_hw_dbs_2x2_capable(psoc) &&
		...
		policy_mgr_debug(
			"1x1 DBS with no existing connection, HW mode change not required");
		goto end;
	}
	...
	while (cur_node) {
		...
		if (policy_mgr_is_hw_mode_change_required(psoc, ch_freq,
							  vdev_id)) {
			policy_mgr_debug(
				"Scan list has BSS of freq %d hw mode required",
				ch_freq);
			break;
		}
				      ...
				      uint32_t connect_id)
{
	...
	status = policy_mgr_current_connections_update(
		psoc, vdev_id, ch_freq, POLICY_MGR_UPDATE_REASON_STA_CONNECT,
		connect_id);
				 ...
				 struct policy_mgr_hw_mode_params *hw_mode)
{
	if (policy_mgr_is_2x2_5G_1x1_2G_dbs_capable(psoc) ||
		...
		if (hw_mode->action_type == PM_DBS1 &&
		    ((action == PM_DBS1 || action == PM_DBS1_DOWNGRADE))) {
			policy_mgr_debug(
				"driver is already in DBS_5G_2x2_24G_1x1 (%d), no "
				"further action %d needed",
				hw_mode->action_type, action);
			return false;
		} else if (hw_mode->action_type == PM_DBS2 &&
			   ((action == PM_DBS2 ||
			...
			policy_mgr_debug(
				"driver is already in DBS_24G_2x2_5G_1x1 (%d), no "
				"further action %d needed",
				hw_mode->action_type, action);
			return false;
		}
			       ...
			       enum policy_mgr_conc_next_action action)
{
   ...
   */
	if (policy_mgr_is_2x2_5G_1x1_2G_dbs_capable(psoc) ||
		...
		policy_mgr_debug(
			"driver is already in %s mode, no further action needed",
 ...
 */
static QDF_STATUS
policy_mgr_validate_unsupported_action(struct wlan_objmgr_psoc *psoc,
				       enum policy_mgr_conc_next_action action)
{
	if (action == PM_SBS || action == PM_SBS_DOWNGRADE) {
		if (!policy_mgr_is_hw_sbs_capable(psoc)) {
				   ...
				   uint32_t request_id)
{
	...
	case PM_DBS:
		...
		status = policy_mgr_pdev_set_hw_mode(
			psoc, session_id, nss_dbs.mac0_ss, bw_dbs.mac0_bw,
			nss_dbs.mac1_ss, bw_dbs.mac1_bw, HW_MODE_MAC_BAND_NONE,
			HW_MODE_DBS, HW_MODE_AGILE_DFS_NONE, sbs_capab, reason,
			PM_NOP, PM_DBS, request_id);
		break;
     ...
     */
		status = policy_mgr_pdev_set_hw_mode(
			psoc, session_id, HW_MODE_SS_2x2, HW_MODE_80_MHZ,
			HW_MODE_SS_0x0, HW_MODE_BW_NONE, HW_MODE_MAC_BAND_NONE,
			HW_MODE_DBS_NONE, HW_MODE_AGILE_DFS_NONE,
			HW_MODE_SBS_NONE, reason, PM_UPGRADE,
			PM_SINGLE_MAC_UPGRADE, request_id);
		break;
	case PM_SINGLE_MAC:
		status = policy_mgr_pdev_set_hw_mode(
			psoc, session_id, HW_MODE_SS_2x2, HW_MODE_80_MHZ,
			HW_MODE_SS_0x0, HW_MODE_BW_NONE, HW_MODE_MAC_BAND_NONE,
			HW_MODE_DBS_NONE, HW_MODE_AGILE_DFS_NONE,
			HW_MODE_SBS_NONE, reason, PM_NOP, PM_SINGLE_MAC,
			request_id);
		break;
	case PM_DBS_UPGRADE:
		status = policy_mgr_pdev_set_hw_mode(
			psoc, session_id, HW_MODE_SS_2x2, HW_MODE_80_MHZ,
			HW_MODE_SS_2x2, HW_MODE_80_MHZ, HW_MODE_MAC_BAND_NONE,
			HW_MODE_DBS, HW_MODE_AGILE_DFS_NONE, HW_MODE_SBS_NONE,
			reason, PM_UPGRADE, PM_DBS_UPGRADE, request_id);
		break;
	...
	case PM_SBS:
		status = policy_mgr_pdev_set_hw_mode(
			psoc, session_id, HW_MODE_SS_1x1, HW_MODE_80_MHZ,
			HW_MODE_SS_1x1, HW_MODE_80_MHZ, HW_MODE_MAC_BAND_NONE,
			HW_MODE_DBS, HW_MODE_AGILE_DFS_NONE, HW_MODE_SBS,
			reason, PM_NOP, PM_SBS, request_id);
		break;
	case PM_DOWNGRADE:
		/*
	...
	case PM_DBS1_DOWNGRADE:
		if (policy_mgr_dbs1_dbs2_need_action(psoc, action, &hw_mode))
			status = policy_mgr_complete_action(
				psoc, POLICY_MGR_RX_NSS_1, PM_DBS1, reason,
				session_id, request_id);
		else
			status = QDF_STATUS_E_ALREADY;
		break;
	case PM_DBS2_DOWNGRADE:
		if (policy_mgr_dbs1_dbs2_need_action(psoc, action, &hw_mode))
			status = policy_mgr_complete_action(
				psoc, POLICY_MGR_RX_NSS_1, PM_DBS2, reason,
				session_id, request_id);
		else
			status = QDF_STATUS_E_ALREADY;
		break;
     ...
     */
		if (policy_mgr_dbs1_dbs2_need_action(psoc, action, &hw_mode)) {
			if (hw_mode.dbs_cap)
				...
				next_action = PM_NOP;
			status = policy_mgr_pdev_set_hw_mode(
				psoc, session_id, HW_MODE_SS_2x2,
				HW_MODE_80_MHZ, HW_MODE_SS_1x1, HW_MODE_40_MHZ,
				HW_MODE_MAC_BAND_5G, HW_MODE_DBS,
				HW_MODE_AGILE_DFS_NONE, HW_MODE_SBS_NONE,
				reason, next_action, PM_DBS1, request_id);
		} else {
			status = QDF_STATUS_E_ALREADY;
		}
		break;
     ...
     */
		if (policy_mgr_dbs1_dbs2_need_action(psoc, action, &hw_mode)) {
			...
			status = policy_mgr_pdev_set_hw_mode(
				psoc, session_id, HW_MODE_SS_2x2,
				HW_MODE_40_MHZ, HW_MODE_SS_1x1, HW_MODE_40_MHZ,
				HW_MODE_MAC_BAND_2G, HW_MODE_DBS,
				HW_MODE_AGILE_DFS_NONE, HW_MODE_SBS_NONE,
				reason, next_action, PM_DBS2, request_id);
		} else {
			status = QDF_STATUS_E_ALREADY;
		}
		break;
	case PM_UPGRADE_5G:
		status = policy_mgr_nss_update(psoc, POLICY_MGR_RX_NSS_2,
					       PM_NOP, POLICY_MGR_BAND_5,
					       reason, session_id, request_id);
		break;
	case PM_UPGRADE_2G:
		status = policy_mgr_nss_update(psoc, POLICY_MGR_RX_NSS_2,
					       PM_NOP, POLICY_MGR_BAND_24,
					       reason, session_id, request_id);
		break;
	default:
...
QDF_STATUS
policy_mgr_handle_conc_multiport(struct wlan_objmgr_psoc *psoc, uint8_t vdev_id,
				 uint32_t ch_freq,
				 enum policy_mgr_conn_update_reason reason,
				 uint32_t request_id)
{
...
policy_mgr_con_mode_by_vdev_id(struct wlan_objmgr_psoc *psoc, uint8_t vdev_id)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
bool policy_mgr_is_safe_channel(struct wlan_objmgr_psoc *psoc, uint32_t ch_freq)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	bool is_safe = true;
	uint8_t j;
	...
	for (j = 0; j < pm_ctx->unsafe_channel_count; j++) {
		if ((ch_freq == pm_ctx->unsafe_channel_list[j]) &&
			...
			is_safe = false;
			policy_mgr_warn(
				"Freq %d is not safe, restriction mask %lu",
				ch_freq, restriction_mask);
			break;
		}
	}
	...
	uint32_t *intf_ch_freq, bool is_acs_mode)
{
	...
	cc_count = policy_mgr_get_mode_specific_conn_info(
		psoc, &op_ch_freq_list[0], &vdev_id[0], PM_SAP_MODE);
	...
	sta_gc_present = policy_mgr_mode_specific_connection_count(
				 psoc, PM_STA_MODE, NULL) +
			 policy_mgr_mode_specific_connection_count(
				 psoc, PM_P2P_CLIENT_MODE, NULL);

	for (i = 0; i < cc_count; i++) {
		if (sap_vdev_id != INVALID_VDEV_ID && sap_vdev_id != vdev_id[i])
			continue;
		...
		if ((is_acs_mode ||
			...
			curr_sap_freq = op_ch_freq_list[i];
			policy_mgr_debug(
				"sta_sap_scc_on_lte_coex_chan %u unsafe sap_ch_freq %u",
				sta_sap_scc_on_lte_coex_chan, curr_sap_freq);
			break;
		}
     ...
     */
		if (pm_ctx->last_disconn_sta_freq == op_ch_freq_list[i] &&
		    !policy_mgr_is_sap_go_interface_allowed_on_indoor(
			    pm_ctx->pdev, sap_vdev_id, op_ch_freq_list[i])) {
			curr_sap_freq = op_ch_freq_list[i];
     ...
     */
		if (!sta_gc_present &&
		    !policy_mgr_is_sap_go_interface_allowed_on_indoor(
			    pm_ctx->pdev, sap_vdev_id, pcl_channels[i])) {
			policy_mgr_debug(
				"Do not allow SAP on indoor frequency, STA is absent");
			continue;
		}
	...
	*intf_ch_freq = new_sap_freq;
	policy_mgr_debug(
		"Standalone SAP(vdev_id %d) is not allowed on DFS/Unsafe "
		"channel, Move it to channel %u",
 ...
 */
static bool policy_mgr_is_nan_sap_unsafe_ch_scc_allowed(
	struct policy_mgr_psoc_priv_obj *pm_ctx, uint32_t ch_freq)
{
					     ...
					     uint32_t sap_freq)
{
	...
	policy_mgr_debug("Freq SAP: %d NAN: %d %d", sap_freq, nan_2g_freq,
			 nan_5g_freq);
	if (WLAN_REG_IS_SAME_BAND_FREQS(nan_2g_freq, sap_freq)) {
...
bool policy_mgr_nan_sap_pre_enable_conc_check(struct wlan_objmgr_psoc *psoc,
					      enum policy_mgr_con_mode mode,
					      uint32_t ch_freq)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	...
	if (mode == PM_NAN_DISC_MODE) {
		...
		if (ucfg_is_nan_conc_control_supported(pm_ctx->psoc) &&
			...
			if (!policy_mgr_is_force_scc(pm_ctx->psoc)) {
				policy_mgr_debug(
					"NAN and SAP are in different bands but SAP force SCC disabled");
				ucfg_nan_disable_concurrency(pm_ctx->psoc);
				return true;
			}
			...
			if ((WLAN_REG_IS_5GHZ_CH_FREQ(ch_freq) &&
			    ...
			    (WLAN_REG_IS_24GHZ_CH_FREQ(ch_freq) &&
			     !policy_mgr_is_nan_sap_unsafe_ch_scc_allowed(
				     pm_ctx, nan_2g_freq))) {
				policy_mgr_debug(
					"NAN+SAP unsafe ch SCC disabled");
				ucfg_nan_disable_concurrency(pm_ctx->psoc);
				return true;
			}
		}
...
policy_mgr_nan_sap_post_enable_conc_check(struct wlan_objmgr_psoc *psoc)
{
	...
	if (!ucfg_is_nan_dbs_supported(pm_ctx->psoc) ||
		...
		status = policy_mgr_change_sap_channel_with_csa(
			psoc, sap_info->vdev_id, nan_freq_2g,
			policy_mgr_get_ch_width(sap_info->bw), true);
		if (status == QDF_STATUS_SUCCESS)
		...
		policy_mgr_debug("Force SCC for NAN+SAP Ch freq: %d",
				 nan_freq_5g);
		status = policy_mgr_change_sap_channel_with_csa(
			psoc, sap_info->vdev_id, nan_freq_5g,
			policy_mgr_get_ch_width(sap_info->bw), true);
		if (status == QDF_STATUS_SUCCESS)
			status = QDF_STATUS_E_PENDING;
	}
...
void policy_mgr_nan_sap_post_disable_conc_check(struct wlan_objmgr_psoc *psoc)
{
	...
	user_config_freq =
		policy_mgr_get_user_config_sap_freq(psoc, sap_info->vdev_id);

	sap_freq = policy_mgr_get_nondfs_preferred_channel(
		psoc, PM_SAP_MODE, false, sap_info->vdev_id);
	...
	if (wlan_reg_is_enable_in_secondary_list_for_freq(pm_ctx->pdev,
		...
		channel_list = qdf_mem_malloc(
			sizeof(struct regulatory_channel) * NUM_CHANNELS);
		if (!channel_list)
			return;
		...
		for (chn_idx = 0; chn_idx < num_chan; chn_idx++) {
			if (wlan_reg_is_enable_in_secondary_list_for_freq(
			    ...
			    policy_mgr_is_safe_channel(
				    psoc, channel_list[chn_idx].center_freq)) {
				policy_mgr_debug(
					"Prefer user config band freq %d",
					channel_list[chn_idx].center_freq);
	...
	if (pm_ctx->hdd_cbacks.hdd_is_chan_switch_in_progress &&
	    pm_ctx->hdd_cbacks.hdd_is_chan_switch_in_progress()) {
		policy_mgr_debug(
			"wait as channel switch is already in progress");
		status = qdf_wait_single_event(
			&pm_ctx->channel_switch_complete_evt,
			CHANNEL_SWITCH_COMPLETE_TIMEOUT);
		if (QDF_IS_STATUS_ERROR(status))
			policy_mgr_err(
				"wait for event failed, still continue with channel switch");
	}
	...
	policy_mgr_change_sap_channel_with_csa(
		psoc, sap_info->vdev_id, sap_freq,
		policy_mgr_get_ch_width(sap_info->bw), true);
}
				  ...
				  uint8_t vdev_id)
{
   ...
   */
	...
	status = pm_ctx->hdd_cbacks.wlan_hdd_get_channel_for_sap_restart(
		psoc, vdev_id, &ch_freq);
	if (status == QDF_STATUS_SUCCESS)
...
policy_mgr_handle_sap_plus_go_force_scc(struct wlan_objmgr_psoc *psoc)
{
	...
	vdev_con_mode =
		policy_mgr_qdf_opmode_to_pm_con_mode(psoc, opmode, vdev_id);
	...
	policy_mgr_debug(
		"initiator vdev %d mode %d freq %d, existing vdev %d mode "
		"%d freq %d reason %d",
	...
	intf_ch_freq = 0;
	status = pm_ctx->hdd_cbacks.wlan_check_cc_intf_cb(
		psoc, existing_vdev_id, &intf_ch_freq);
	policy_mgr_debug("vdev %d freq %d intf %d status %d", existing_vdev_id,
			 existing_ch_freq, intf_ch_freq, status);
	if (QDF_IS_STATUS_ERROR(status))
		goto force_scc_done;
	...
	if (pm_ctx->hdd_cbacks.wlan_get_ap_prefer_conc_ch_params) {
		...
		if (QDF_IS_STATUS_ERROR(status))
			policy_mgr_debug(
				"no candidate valid bw for vdev %d intf %d",
				existing_vdev_id, intf_ch_freq);
	}

	status = policy_mgr_valid_sap_conc_channel_check(psoc, &intf_ch_freq,
							 existing_ch_freq,
							 existing_vdev_id,
							 &ch_params);
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err(
			"warning no candidate freq for vdev %d freq %d intf %d",
			existing_vdev_id, existing_ch_freq, intf_ch_freq);
		goto force_scc_done;
	}
	...
	status = policy_mgr_change_sap_channel_with_csa(
		psoc, existing_vdev_id, intf_ch_freq, ch_params.ch_width, true);
	if (QDF_IS_STATUS_ERROR(status)) {
		policy_mgr_err(
			"warning sap/go vdev %d freq %d intf %d csa failed",
			existing_vdev_id, existing_ch_freq, intf_ch_freq);
	}
				  ...
				  enum sap_csa_reason_code reason_code)
{
	...
	existing_vdev_id = policy_mgr_fetch_existing_con_info(
		psoc, vdev_id, chan_freq, &existing_vdev_mode, &con_freq,
		&ch_width);
	if (existing_vdev_id == WLAN_UMAC_VDEV_ID_MAX)
		return QDF_STATUS_SUCCESS;
	...
	policy_mgr_debug(
		"initiator vdev %d freq %d, existing vdev %d freq %d reason %d",
		vdev_id, chan_freq, existing_vdev_id, con_freq, reason_code);
 ...
 */
static bool policy_mgr_is_any_conn_in_transition(struct wlan_objmgr_psoc *psoc)
{
	struct policy_mgr_psoc_priv_obj *pm_ctx;
	uint32_t i;
	...
	struct policy_mgr_psoc_priv_obj *pm_ctx)
{
   ...
   */
	...
	mcc_to_scc_switch = policy_mgr_get_mcc_to_scc_switch_mode(pm_ctx->psoc);

	policy_mgr_debug(
		"Concurrent open sessions running: %d",
	...
	cc_count = policy_mgr_get_mode_specific_conn_info(
		pm_ctx->psoc, &op_ch_freq_list[cc_count], &vdev_id[cc_count],
		PM_SAP_MODE);
	...
	if (!cc_count) {
		policy_mgr_err(
			"Could not retrieve SAP/GO operating channel&vdevid");
		goto end;
	}
   ...
   */
	if (policy_mgr_is_any_conn_in_transition(pm_ctx->psoc)) {
		policy_mgr_debug(
			"defer sap conc check to a later time due to another "
			"sta/cli dicon/roam pending");
		goto end;
	}
	...
	if (policy_mgr_is_set_link_in_progress(pm_ctx->psoc)) {
		policy_mgr_debug(
			"defer sap conc check to a later time due to ml sta set "
			"link in progress");
		qdf_delayed_work_start(&pm_ctx->sta_ap_intf_check_work,
				       SAP_CONC_CHECK_DEFER_TIMEOUT_MS);
		goto end;
		...
		for (i = 0; i < cc_count; i++) {
			status = pm_ctx->hdd_cbacks
					 .wlan_hdd_get_channel_for_sap_restart(
						 pm_ctx->psoc, vdev_id[i],
						 &ch_freq);
			if (status == QDF_STATUS_SUCCESS) {
				policy_mgr_debug(
					"SAP vdev id %d restarts due to MCC->SCC switch, old "
					"ch freq :%d new ch freq: %d",
					vdev_id[i], op_ch_freq_list[i],
					ch_freq);
				break;
			}
		}
...
static bool policy_mgr_get_srd_enable_for_vdev(struct wlan_objmgr_psoc *psoc,
					       uint8_t vdev_id)
{
	struct wlan_objmgr_vdev *vdev;
					...
					struct ch_params *ch_params)
{
   ...
   */
	if (!ch_freq) {
		if (!policy_mgr_any_other_vdev_on_same_mac_as_freq(
			    psoc, sap_ch_freq, sap_vdev_id))
			return QDF_STATUS_SUCCESS;
	...
	is_dfs = wlan_mlme_check_chan_param_has_dfs(pm_ctx->pdev, ch_params,
						    ch_freq);
	...
	if (WLAN_REG_IS_5GHZ_CH_FREQ(ch_freq) && is_dfs &&
		...
		policymgr_nofl_debug(
			"sap not capable of DFS SCC on con ch_freq %d",
			ch_freq);
	} else if (wlan_reg_is_disable_for_pwrmode(pm_ctx->pdev, ch_freq,
						   REG_CURRENT_PWR_MODE)) {
		find_alternate = true;
		policymgr_nofl_debug(
			"sap not capable on disabled con ch_freq %d", ch_freq);
		...
		policymgr_nofl_debug("sap not capable unsafe con ch_freq %d",
		   ...
		   !WLAN_REG_IS_6GHZ_CHAN_FREQ(sap_ch_freq) && !is_6ghz_cap) {
		...
		policymgr_nofl_debug("sap not capable on SRD con ch_freq %d",
			   ...
			   pm_ctx->pdev, sap_vdev_id, ch_freq)) {
		policymgr_nofl_debug(
	...
	if (find_alternate) {
		if (policy_mgr_is_hw_dbs_capable(psoc)) {
			...
			if (con_mode == PM_SAP_MODE) {
				policymgr_nofl_debug(
					"MCC situation in non-dbs hw STA freq %d SAP freq %d not supported",
					*con_ch_freq, sap_ch_freq);
				return QDF_STATUS_E_FAILURE;
			} else {
				policymgr_nofl_debug(
					"MCC situation in non-dbs hw STA freq %d GO freq "
					"%d SCC not supported",
					*con_ch_freq, sap_ch_freq);
			}
		}
	...
	if (ch_freq != sap_ch_freq || old_ch_width != ch_params->ch_width) {
		*con_ch_freq = ch_freq;
		policymgr_nofl_debug(
			"sap conc result con freq %d bw %d org freq %d bw %d",
	...
	struct wlan_objmgr_psoc *psoc, bool is_acs_mode)
{
   ...
   */
	...
	policy_mgr_debug("MCC to SCC switch: %d chan: %d", mcc_to_scc_switch,
			 op_ch_freq_list[0]);
sap_restart:
	/*
   ...
   */
	if (restart_sap ||
		...
		if (!qdf_delayed_work_start(&pm_ctx->sta_ap_intf_check_work,
					    timeout_ms)) {
			policy_mgr_debug(
				"change interface request already queued");
			return;
		}
	}
}
				       ...
				       uint32_t ch_width, bool forced)
{
	...
	if (!policy_mgr_check_bw_with_unsafe_chan_freq(psoc, center_freq,
						       ch_width)) {
	...
	if (pm_ctx->hdd_cbacks.sap_restart_chan_switch_cb) {
		...
		status = pm_ctx->hdd_cbacks.sap_restart_chan_switch_cb(
			psoc, vdev_id, ch_freq, ch_width, forced);
	} else {
			...
			if (wlan_reg_is_dfs_for_freq(pdev, new_freq) ||
				...
				new_freq =
					policy_mgr_get_nondfs_preferred_channel(
						psoc, concur_mode, true,
						concur_vdev_id);
		} else {
			new_freq = wlan_reg_min_6ghz_chan_freq();
		}
	}
	policy_mgr_debug("Restart vdev: %u on freq: %u", concur_vdev_id,
			 new_freq);
					...
					uint8_t vdev_id)
{
	...
	if (!is_sap_go_moved_before_sta || !move_sap_go_first) {
		policy_mgr_debug("SAP / GO moved before STA: %u INI "
				 "g_move_sap_go_1st_on_dfs_sta_csa: %u",
					...
					uint32_t ch_width)
{
	...
	if (total_connection > 1) {
		...
		policy_mgr_change_sap_channel_with_csa(psoc, vdev_id, ch_freq,
						       ch_width, true);
	}
}
...
QDF_STATUS
policy_mgr_wait_chan_switch_complete_evt(struct wlan_objmgr_psoc *psoc)
{
	QDF_STATUS status;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
QDF_STATUS
policy_mgr_wait_for_connection_update(struct wlan_objmgr_psoc *psoc)
{
	QDF_STATUS status;
...
QDF_STATUS
policy_mgr_set_chan_switch_complete_evt(struct wlan_objmgr_psoc *psoc)
{
	QDF_STATUS status;
	struct policy_mgr_psoc_priv_obj *pm_ctx;
...
QDF_STATUS
policy_mgr_reset_chan_switch_complete_evt(struct wlan_objmgr_psoc *psoc)
{
	QDF_STATUS status;
	struct policy_mgr_psoc_priv_obj *policy_mgr_context;
...
QDF_STATUS policy_mgr_restart_opportunistic_timer(struct wlan_objmgr_psoc *psoc,
						  bool check_state)
{
	QDF_STATUS status = QDF_STATUS_E_FAILURE;
	struct policy_mgr_psoc_priv_obj *policy_mgr_ctx;
	...
	status = qdf_mc_timer_start(&policy_mgr_ctx->dbs_opportunistic_timer,
				    DBS_OPPORTUNISTIC_TIME * 1000);
...
QDF_STATUS
policy_mgr_set_hw_mode_on_channel_switch(struct wlan_objmgr_psoc *psoc,
					 uint8_t session_id)
{
	...
	if ((action != PM_DBS_DOWNGRADE) && (action != PM_SINGLE_MAC_UPGRADE) &&
	    (action != PM_DBS1_DOWNGRADE) && (action != PM_DBS2_DOWNGRADE)) {
   ...
   */
	...
	status = policy_mgr_next_actions(
		psoc, session_id, action,
...
QDF_STATUS policy_mgr_check_and_set_hw_mode_for_channel_switch(
	struct wlan_objmgr_psoc *psoc, uint8_t vdev_id, uint32_t ch_freq,
	enum policy_mgr_conn_update_reason reason)
{
	QDF_STATUS status;
   ...
   */
	...
	status = policy_mgr_get_next_action(psoc, vdev_id, ch_freq, reason,
					    &next_action);
	/* Restore the connection entry */
	if (num_cxn_del)
		policy_mgr_restore_deleted_conn_info(psoc, &info, num_cxn_del);
	...
	if (PM_NOP != next_action)
		status = policy_mgr_next_actions(psoc, vdev_id, next_action,
	...
	struct wlan_objmgr_psoc *psoc, uint32_t ch_freq)
{
	...
	dbs_required_2g = policy_mgr_is_hw_dbs_required_for_band(
		psoc, HW_MODE_MAC_BAND_2G);
	...
	struct wlan_objmgr_psoc *psoc, uint8_t id)
{
	...
	if (QDF_TIMER_STATE_RUNNING == pm_ctx->dbs_opportunistic_timer.state) {
		...
		if (action) {
			...
			if (status != QDF_STATUS_SUCCESS) {
				policy_mgr_err(
					"Failed in policy_mgr_next_actions");
				return;
			}
			status = qdf_wait_single_event(
...
policy_mgr_is_hw_mode_change_in_progress(struct wlan_objmgr_psoc *psoc)
{
...
enum policy_mgr_hw_mode_change
policy_mgr_get_hw_mode_change_from_hw_mode_index(struct wlan_objmgr_psoc *psoc,
...
bool policy_mgr_get_allowed_tdls_offchannel_freq(struct wlan_objmgr_psoc *psoc,
						 struct wlan_objmgr_vdev *vdev,
						 qdf_freq_t *ch_freq)
{
   ...
   */
	...
	default:
		policy_mgr_debug(
			"TDLS off channel not allowed on > 3 port conc");
		break;
	}
