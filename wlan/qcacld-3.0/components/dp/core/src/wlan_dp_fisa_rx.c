...
static void hex_dump_skb_data(qdf_nbuf_t nbuf, bool dump)
{
	...
	dp_fisa_debug(
		"%ps: skb: %pK skb->next:%pK frag_list %pK skb->data:%pK len "
		"%d data_len %d",
	...
	while (next_nbuf) {
		dp_fisa_debug(
			"%d nbuf:%pK nbuf->next:%pK nbuf->data:%pK len %d", i,
		      ...
		      uint8_t dbg_level)
{
	...
	flow_aggr_cont =
		hal_rx_get_fisa_flow_agg_continuation(hal_soc_hdl, buf);
	fisa_aggr_count = hal_rx_get_fisa_flow_agg_count(hal_soc_hdl, buf);
	fisa_timeout = hal_rx_get_fisa_timeout(hal_soc_hdl, buf);
	cumulat_l4_csum =
		hal_rx_get_fisa_cumulative_l4_checksum(hal_soc_hdl, buf);
	cumulat_ip_len = hal_rx_get_fisa_cumulative_ip_length(hal_soc_hdl, buf);

	dp_fisa_debug("flow_aggr_cont %d, fisa_timeout %d, fisa_aggr_count %d, "
		      "cumulat_l4_csum %d, cumulat_ip_len %d",
...
static void dp_fisa_record_pkt(struct dp_fisa_rx_sw_ft *fisa_flow,
			     ...
			     struct dp_rx_fst *rx_fst, qdf_nbuf_t nbuf)
{
	...
	l2_hdr_offset =
		hal_rx_msdu_end_l3_hdr_padding_get(dp_ctx->hal_soc, rx_tlv_hdr);
				 ...
				 qdf_nbuf_t nbuf, uint8_t *rx_tlv_hdr)
{
	...
	hal_rx_get_l3_l4_offsets(dp_ctx->hal_soc, rx_tlv_hdr, &ip_hdr_offset,
				 &tcp_hdr_offset);
	...
	dp_fisa_debug(
		"head_skb: %pK head_skb->next:%pK head_skb->data:%pK len %d data_len %d",
 ...
 */
static void *
dp_rx_fisa_setup_hw_fse(struct dp_rx_fst *fisa_hdl, uint32_t hashed_flow_idx,
 ...
 * Return: None
 */
static inline void __dp_rx_fisa_acquire_ft_lock(struct dp_rx_fst *fisa_hdl,
						uint8_t reo_id,
						const char *func)
{
 ...
 * Return: None
 */
static inline void __dp_rx_fisa_release_ft_lock(struct dp_rx_fst *fisa_hdl,
						uint8_t reo_id,
						const char *func)
{
	if (!fisa_hdl->flow_deletion_supported)
 ...
 * Return: None
 */
static inline void dp_rx_fisa_acquire_ft_lock(struct dp_rx_fst *fisa_hdl,
					      uint8_t reo_id)
{
	if (fisa_hdl->flow_deletion_supported)
		qdf_spin_lock_bh(&fisa_hdl->dp_rx_sw_ft_lock[reo_id]);
 ...
 * Return: None
 */
static inline void dp_rx_fisa_release_ft_lock(struct dp_rx_fst *fisa_hdl,
					      uint8_t reo_id)
{
	if (fisa_hdl->flow_deletion_supported)
		qdf_spin_unlock_bh(&fisa_hdl->dp_rx_sw_ft_lock[reo_id]);
			  ...
			  uint32_t flow_steer_info)
{
	...
	sw_ft_entry =
		&(((struct dp_fisa_rx_sw_ft *)fisa_hdl->base)[hashed_flow_idx]);
...
dp_fisa_rx_get_dp_intf_for_vdev(struct dp_vdev *vdev)
{
	struct wlan_dp_link *dp_link = (struct wlan_dp_link *)vdev->osif_vdev;
...
dp_rx_fisa_add_ft_entry(struct dp_vdev *vdev, struct dp_rx_fst *fisa_hdl,
			qdf_nbuf_t nbuf, uint8_t *rx_tlv_hdr,
			uint32_t flow_idx_hash, uint32_t reo_dest_indication)
{
	struct dp_fisa_rx_sw_ft *sw_ft_entry;
	uint32_t flow_hash;
	...
	do {
		...
		if (!sw_ft_entry->is_populated) {
			/* Add SW FT entry */
			dp_rx_fisa_update_sw_ft_entry(sw_ft_entry, flow_hash,
						      vdev, fisa_hdl->dp_ctx,
						      hashed_flow_idx);

			/* Add HW FT entry */
			sw_ft_entry->hw_fse = dp_rx_fisa_setup_hw_fse(
				fisa_hdl, hashed_flow_idx, &rx_flow_tuple_info,
				reo_dest_indication);
   ...
   */
	...
	if (skid_count > max_skid_length) {
		dp_fisa_debug(
			"Max skid length reached flow cannot be added, evict exiting flow");
		return NULL;
	}
   ...
   */
	if (is_fst_updated && fisa_hdl->fse_cache_flush_allow &&
 ...
 * Return: None
 */
static void dp_fisa_rx_delete_flow(struct dp_rx_fst *fisa_hdl,
				   ...
				   uint32_t hashed_flow_idx)
{
	...
	sw_ft_entry->cmem_offset = dp_rx_fisa_setup_cmem_fse(
		fisa_hdl, hashed_flow_idx, &elem->flow_tuple_info,
 ...
 * Return: Timestamp
 */
static uint32_t dp_fisa_rx_get_hw_ft_timestamp(struct dp_rx_fst *fisa_hdl,
					       uint32_t hashed_flow_idx)
{
...
void dp_fisa_rx_fst_update_work(void *arg)
{
	...
	while (qdf_list_peek_front(&fisa_hdl->fst_update_list, &node) ==
		...
		vdev = dp_vdev_get_ref_by_id(fisa_hdl->soc_hdl, elem->vdev_id,
					     DP_MOD_ID_RX);
		/*
 ...
 * Return: True/False
 */
static inline bool dp_fisa_rx_is_fst_work_queued(struct dp_rx_fst *fisa_hdl,
						 uint32_t flow_idx)
{
	struct dp_fisa_rx_fst_update_elem *elem;
 ...
 * Return: None
 */
static void *dp_fisa_rx_queue_fst_update_work(struct dp_rx_fst *fisa_hdl,
					      uint32_t flow_idx,
					      qdf_nbuf_t nbuf,
					      struct dp_vdev *vdev)
{
	...
	if (sw_ft_entry->is_populated &&
	    is_same_flow(&sw_ft_entry->rx_flow_tuple_info, &flow_tuple_info))
		return sw_ft_entry;
 ...
 */
static inline QDF_STATUS dp_rx_reo_dest_honor_check(struct dp_rx_fst *fisa_hdl,
						    qdf_nbuf_t nbuf,
						    uint32_t tlv_reo_dest_ind)
{
	uint8_t sw_exception = qdf_nbuf_get_rx_reo_dest_ind_or_sw_excpt(nbuf);
 ...
 */
static struct dp_fisa_rx_sw_ft *dp_rx_get_fisa_flow(struct dp_rx_fst *fisa_hdl,
						    struct dp_vdev *vdev,
						    qdf_nbuf_t nbuf)
{
	uint8_t *rx_tlv_hdr;
	...
	sw_ft_entry = dp_rx_fisa_add_ft_entry(vdev, fisa_hdl, nbuf, rx_tlv_hdr,
					      flow_idx_hash, tlv_reo_dest_ind);

print_and_return:
	dp_fisa_debug(
		"nbuf %pK fl_idx 0x%x fl_inv %d fl_timeout %d flow_id_toeplitz "
		"%x reo_dest_ind 0x%x",
 ...
 */
static inline int
get_transport_header_offset(struct dp_fisa_rx_sw_ft *fisa_flow,
		    ...
		    struct dp_fisa_rx_sw_ft *fisa_flow, qdf_nbuf_t nbuf)
{
	qdf_nbuf_t head_skb = fisa_flow->head_skb;
	uint8_t *rx_tlv_hdr = qdf_nbuf_data(nbuf);
	uint32_t l2_hdr_offset = hal_rx_msdu_end_l3_hdr_padding_get(
		fisa_hdl->dp_ctx->hal_soc, rx_tlv_hdr);
	...
	udp_hdr = (qdf_net_udphdr_t *)(qdf_nbuf_data(nbuf) +
				       get_transport_header_offset(
					       fisa_flow, l3_hdr_offset,
					       l4_hdr_offset));
   ...
   */
	...
	transport_payload_offset = get_transport_payload_offset(
		fisa_hdl, l3_hdr_offset, l4_hdr_offset);
	...
	fisa_flow->frags_cumulative_len += (udp_len - sizeof(qdf_net_udphdr_t));
...
get_new_vdev_ref:
	fisa_flow_head_skb_vdev = dp_vdev_get_ref_by_id(
		cdp_soc_t_to_dp_soc(cdp_soc), vdev_id, DP_MOD_ID_RX);
				   ...
				   struct dp_fisa_rx_sw_ft *fisa_flow)
{
	...
	fisa_flow_head_skb_vdev = dp_vdev_get_ref_by_id(
		...
		QDF_NBUF_CB_RX_VDEV_ID(fisa_flow->head_skb), DP_MOD_ID_RX);
	if (qdf_unlikely(!fisa_flow_head_skb_vdev ||
...
out:
	if (fisa_flow_head_skb_vdev)
		dp_vdev_unref_delete(cdp_soc_t_to_dp_soc(cdp_soc),
				     fisa_flow_head_skb_vdev, DP_MOD_ID_RX);
	return NULL;
}
#endif
 ...
 * Return: None
 */
static void dp_rx_fisa_flush_udp_flow(struct dp_vdev *vdev,
				      struct dp_fisa_rx_sw_ft *fisa_flow)
{
	qdf_nbuf_t head_skb = fisa_flow->head_skb;
	...
	if (qdf_nbuf_get_ext_list(head_skb)) {
		...
		dp_fisa_debug("cumu ip length host order 0x%x",
			      fisa_flow->adjusted_cumulative_ip_length);
		head_skb_iph =
			(qdf_net_iphdr_t *)(qdf_nbuf_data(head_skb) +
     ...
     */
		qdf_nbuf_set_data_len(
			head_skb, ((fisa_flow->adjusted_cumulative_ip_length) -
				   qdf_ntohs(head_skb_udp_hdr->udp_len)));

		qdf_nbuf_set_len(head_skb,
				 (qdf_nbuf_len(head_skb) +
				  qdf_nbuf_get_only_data_len(head_skb)));

		head_skb_iph->ip_len = qdf_htons(
			(fisa_flow->adjusted_cumulative_ip_length) + /* IP hdr len */
			fisa_flow->head_skb_l4_hdr_offset);
		pseudo = ~qdf_csum_tcpudp_magic(
			head_skb_iph->ip_saddr, head_skb_iph->ip_daddr,
			fisa_flow->adjusted_cumulative_ip_length,
			head_skb_iph->ip_proto, 0);

		head_skb_iph->ip_check = 0;
		head_skb_iph->ip_check =
			qdf_ip_fast_csum(head_skb_iph, head_skb_iph->ip_hl);
     ...
     */
		if (qdf_unlikely(
			    fisa_flow->frags_cumulative_len !=
 ...
 * Return: None
 */
static void dp_rx_fisa_flush_tcp_flow(struct dp_vdev *vdev,
				      struct dp_fisa_rx_sw_ft *fisa_flow)
{
	qdf_nbuf_t head_skb = fisa_flow->head_skb;
	qdf_net_iphdr_t *head_skb_iph;
	...
	head_skb_iph->ip_check =
		ip_fast_csum((u8 *)head_skb_iph, head_skb_iph->ip_hl);
 ...
 */
static bool dp_fisa_aggregation_should_stop(struct dp_fisa_rx_sw_ft *fisa_flow,
					    uint32_t hal_aggr_count,
					    uint16_t hal_cumulative_ip_len,
					    uint8_t *rx_tlv_hdr)
{
	uint32_t msdu_len = hal_rx_msdu_start_msdu_len_get(
		fisa_flow->dp_ctx->hal_soc, rx_tlv_hdr);
	uint32_t l3_hdr_offset, l4_hdr_offset, l2_l3_hdr_len;
	uint32_t cumulative_ip_len_delta =
				    ...
				    struct dp_fisa_rx_sw_ft *fisa_flow)
{
	...
	dp_fisa_debug(
		"nbuf: %pK nbuf->next:%pK nbuf->data:%pK len %d data_len %d",
		nbuf, qdf_nbuf_next(nbuf), qdf_nbuf_data(nbuf),
		qdf_nbuf_len(nbuf), qdf_nbuf_get_only_data_len(nbuf));
   ...
   */
     ...
     */
		if (cce_match) {
			...
			DP_STATS_INC(fisa_hdl, reo_mismatch.allow_cce_match, 1);
	...
	hal_cumulative_ip_len =
		hal_rx_get_fisa_cumulative_ip_length(hal_soc_hdl, rx_tlv_hdr);
	flow_aggr_cont =
		hal_rx_get_fisa_flow_agg_continuation(hal_soc_hdl, rx_tlv_hdr);
	hal_aggr_count =
		hal_rx_get_fisa_flow_agg_count(hal_soc_hdl, rx_tlv_hdr);
     ...
     */
		...
		if (qdf_unlikely(dp_fisa_aggregation_should_stop(
			    fisa_flow, hal_aggr_count, hal_cumulative_ip_len,
			    rx_tlv_hdr))) {
			qdf_assert(0);
   ...
   */
	...
	dp_fisa_debug(
		"hal cum_len 0x%x - napI_cumu_len 0x%x = flow_cum_len 0x%x cur_aggr %d",
		hal_cumulative_ip_len,
		fisa_flow->napi_flush_cumulative_ip_length,
		fisa_flow->adjusted_cumulative_ip_length, fisa_flow->cur_aggr);
...
static bool dp_is_nbuf_bypass_fisa(qdf_nbuf_t nbuf)
{
	...
	if (QDF_NBUF_CB_RX_TCP_PROTO(nbuf) || qdf_nbuf_is_exc_frame(nbuf) ||
	    qdf_nbuf_is_ipv4_dhcp_pkt(nbuf) || qdf_nbuf_is_da_mcbc(nbuf))
		return true;
 ...
 */
static QDF_STATUS dp_rx_fisa_flush_by_intf_ctx_id(struct wlan_dp_intf *dp_intf,
						  uint8_t rx_ctx_id)
{
	struct wlan_dp_psoc_context *dp_ctx = dp_get_context();
...
static bool dp_fisa_disallowed_for_vdev(struct dp_soc *soc,
					struct dp_vdev *vdev, uint8_t rx_ctx_id)
{
...
QDF_STATUS dp_fisa_rx(struct wlan_dp_psoc_context *dp_ctx, struct dp_vdev *vdev,
		      qdf_nbuf_t nbuf_list)
{
	...
	while (head_nbuf) {
		...
		if (qdf_atomic_read(&dp_ctx->skip_fisa_param.skip_fisa)) {
			if (!dp_ctx->skip_fisa_param
				     .fisa_force_flush[rx_ctx_id]) {
		...
		fisa_flow =
			dp_rx_get_fisa_flow(dp_fisa_rx_hdl, vdev, head_nbuf);
...
static void dp_rx_fisa_flush_flow_wrap(struct dp_fisa_rx_sw_ft *sw_ft)
{
   ...
   */
	...
	dp_rx_fisa_flush_flow(sw_ft->vdev, sw_ft);
	sw_ft->cur_aggr = 0;
}
