				      ...
				      qdf_nbuf_t nbuf, uint8_t status)
{
	return pkt_capture_process_mgmt_tx_data(
		pdev, params, nbuf, pkt_capture_mgmt_status_map(status));
}

void ucfg_pkt_capture_mgmt_tx(struct wlan_objmgr_pdev *pdev, qdf_nbuf_t nbuf,
			      uint16_t chan_freq, uint8_t preamble_type)
{
	pkt_capture_mgmt_tx(pdev, nbuf, chan_freq, preamble_type);
}

void ucfg_pkt_capture_mgmt_tx_completion(
	struct wlan_objmgr_pdev *pdev, uint32_t desc_id, uint32_t status,
	struct mgmt_offload_event_params *params)
{
	pkt_capture_mgmt_tx_completion(pdev, desc_id, status, params);
}

void ucfg_pkt_capture_rx_msdu_process(uint8_t *bssid, qdf_nbuf_t head_msdu,
				      uint8_t vdev_id, htt_pdev_handle pdev)
{
	pkt_capture_msdu_process_pkts(bssid, head_msdu, vdev_id, pdev, 0);
}
...
void ucfg_pkt_capture_offload_deliver_indication_handler(void *msg,
							 uint8_t vdev_id,
							 uint8_t *bssid,
							 htt_pdev_handle pdev)
{
	pkt_capture_offload_deliver_indication_handler(msg, vdev_id, bssid,
						       pdev);
}

struct htt_tx_data_hdr_information *
ucfg_pkt_capture_tx_get_txcomplete_data_hdr(uint32_t *msg_word, int num_msdus)
{
	return pkt_capture_tx_get_txcomplete_data_hdr(msg_word, num_msdus);
}

void ucfg_pkt_capture_tx_completion_process(
	uint8_t vdev_id, qdf_nbuf_t mon_buf_list,
	enum pkt_capture_data_process_type type, uint8_t tid, uint8_t status,
	bool pkt_format, uint8_t *bssid, htt_pdev_handle pdev,
	uint8_t tx_retry_cnt)
{
	pkt_capture_datapkt_process(vdev_id, mon_buf_list,
				    TXRX_PROCESS_TYPE_DATA_TX_COMPL, tid,
				    status, pkt_format, bssid, pdev,
				    tx_retry_cnt);
}
...
int ucfg_pkt_capture_register_wma_callbacks(
	struct wlan_objmgr_psoc *psoc, struct pkt_capture_callbacks *cb_obj)
{
