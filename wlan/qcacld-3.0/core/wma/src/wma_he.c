				 ...
				 uint8_t *host_ppet, int req_byte)
{
	...
	for (i = 0; i < nss; i++) {
		for (j = 1; j <= ru; j++) {
			...
			wma_nofl_debug("ppet16 (nss:%d ru:%d): %04x", i, j,
				       ppet16);
			wma_nofl_debug("ppet8 (nss:%d ru:%d): %04x", i, j,
				       ppet8);
				...
				struct wmi_host_ppe_threshold *ppet)
{
   ...
   */
	bits = HE_PPET_NSS_RU_LEN +
	       ((hdr->nss + 1) * ru_count) * (HE_PPET_SIZE * 2);

	req_byte = (bits / HE_BYTE_SIZE) + 1;
	wma_he_populate_ppet(ppet->ppet16_ppet8_ru3_ru0, hdr->nss + 1, ru_count,
			     he_ppet, req_byte);
}
			       ...
			       uint16_t *mcs_12_13_supp)
{
	...
	he_cap->he_dynamic_smps = WMI_HECAP_MAC_DYNSMPWRSAVE_GET(mac_cap[1]);
...
static void wma_derive_ext_he_cap(tDot11fIEhe_cap *he_cap,
				  tDot11fIEhe_cap *new_cap, bool is_5g_cap)
{
	...
	he_cap->twt_request =
		QDF_MIN(he_cap->twt_request, new_cap->twt_request);
	he_cap->twt_responder =
		QDF_MIN(he_cap->twt_responder, new_cap->twt_responder);
	he_cap->fragmentation =
		QDF_MIN(he_cap->fragmentation, new_cap->fragmentation);
	he_cap->max_num_frag_msdu_amsdu_exp =
		QDF_MIN(he_cap->max_num_frag_msdu_amsdu_exp,
			new_cap->max_num_frag_msdu_amsdu_exp);
	he_cap->min_frag_size =
		QDF_MIN(he_cap->min_frag_size, new_cap->min_frag_size);
	he_cap->trigger_frm_mac_pad = QDF_MIN(he_cap->trigger_frm_mac_pad,
					      new_cap->trigger_frm_mac_pad);
	he_cap->multi_tid_aggr_rx_supp =
		QDF_MIN(he_cap->multi_tid_aggr_rx_supp,
			new_cap->multi_tid_aggr_rx_supp);
	he_cap->he_link_adaptation = QDF_MIN(he_cap->he_link_adaptation,
					     new_cap->he_link_adaptation);
	he_cap->all_ack = QDF_MIN(he_cap->all_ack, new_cap->all_ack);
	he_cap->trigd_rsp_sched =
		QDF_MIN(he_cap->trigd_rsp_sched, new_cap->trigd_rsp_sched);
	he_cap->a_bsr = QDF_MIN(he_cap->a_bsr, new_cap->a_bsr);
	he_cap->broadcast_twt =
		QDF_MIN(he_cap->broadcast_twt, new_cap->broadcast_twt);
	he_cap->ba_32bit_bitmap =
		QDF_MIN(he_cap->ba_32bit_bitmap, new_cap->ba_32bit_bitmap);
	he_cap->mu_cascade = QDF_MIN(he_cap->mu_cascade, new_cap->mu_cascade);
	he_cap->ack_enabled_multitid = QDF_MIN(he_cap->ack_enabled_multitid,
					       new_cap->ack_enabled_multitid);
	he_cap->omi_a_ctrl = QDF_MIN(he_cap->omi_a_ctrl, new_cap->omi_a_ctrl);
	he_cap->ofdma_ra = QDF_MIN(he_cap->ofdma_ra, new_cap->ofdma_ra);
	he_cap->max_ampdu_len_exp_ext = QDF_MIN(he_cap->max_ampdu_len_exp_ext,
						new_cap->max_ampdu_len_exp_ext);
	he_cap->amsdu_frag = QDF_MIN(he_cap->amsdu_frag, new_cap->amsdu_frag);
	he_cap->flex_twt_sched =
		QDF_MIN(he_cap->flex_twt_sched, new_cap->flex_twt_sched);
	he_cap->rx_ctrl_frame =
		QDF_MIN(he_cap->rx_ctrl_frame, new_cap->rx_ctrl_frame);
	he_cap->bsrp_ampdu_aggr =
		QDF_MIN(he_cap->bsrp_ampdu_aggr, new_cap->bsrp_ampdu_aggr);
	...
	he_cap->ndp_feedback_supp =
		QDF_MIN(he_cap->ndp_feedback_supp, new_cap->ndp_feedback_supp);
	he_cap->ops_supp = QDF_MIN(he_cap->ops_supp, new_cap->ops_supp);
	he_cap->amsdu_in_ampdu =
		QDF_MIN(he_cap->amsdu_in_ampdu, new_cap->amsdu_in_ampdu);
	...
	he_cap->device_class =
		QDF_MIN(he_cap->device_class, new_cap->device_class);
	he_cap->ldpc_coding =
		QDF_MIN(he_cap->ldpc_coding, new_cap->ldpc_coding);
	he_cap->he_1x_ltf_800_gi_ppdu = QDF_MIN(he_cap->he_1x_ltf_800_gi_ppdu,
						new_cap->he_1x_ltf_800_gi_ppdu);
	he_cap->midamble_tx_rx_max_nsts =
		QDF_MIN(he_cap->midamble_tx_rx_max_nsts,
			new_cap->midamble_tx_rx_max_nsts);
	he_cap->he_4x_ltf_3200_gi_ndp = QDF_MIN(he_cap->he_4x_ltf_3200_gi_ndp,
						new_cap->he_4x_ltf_3200_gi_ndp);
	he_cap->tb_ppdu_tx_stbc_lt_80mhz =
		QDF_MIN(he_cap->tb_ppdu_tx_stbc_lt_80mhz,
			new_cap->tb_ppdu_tx_stbc_lt_80mhz);
	he_cap->rx_stbc_lt_80mhz =
		QDF_MIN(he_cap->rx_stbc_lt_80mhz, new_cap->rx_stbc_lt_80mhz);
	he_cap->doppler = QDF_MIN(he_cap->doppler, new_cap->doppler);
	he_cap->ul_mu = QDF_MIN(he_cap->ul_mu, new_cap->ul_mu);
	he_cap->dcm_enc_tx = QDF_MIN(he_cap->dcm_enc_tx, new_cap->dcm_enc_tx);
	he_cap->dcm_enc_rx = QDF_MIN(he_cap->dcm_enc_rx, new_cap->dcm_enc_rx);
	he_cap->ul_he_mu = QDF_MIN(he_cap->ul_he_mu, new_cap->ul_he_mu);
	he_cap->su_beamformer =
		QDF_MIN(he_cap->su_beamformer, new_cap->su_beamformer);
	he_cap->su_beamformee =
		QDF_MIN(he_cap->su_beamformee, new_cap->su_beamformee);
	he_cap->mu_beamformer =
		QDF_MIN(he_cap->mu_beamformer, new_cap->mu_beamformer);
	he_cap->bfee_sts_lt_80 =
		QDF_MIN(he_cap->bfee_sts_lt_80, new_cap->bfee_sts_lt_80);
	he_cap->bfee_sts_gt_80 =
		QDF_MIN(he_cap->bfee_sts_gt_80, new_cap->bfee_sts_gt_80);
	...
	he_cap->codebook_su =
		QDF_MIN(he_cap->codebook_su, new_cap->codebook_su);
	he_cap->codebook_mu =
		QDF_MIN(he_cap->codebook_mu, new_cap->codebook_mu);
	he_cap->beamforming_feedback = QDF_MIN(he_cap->beamforming_feedback,
					       new_cap->beamforming_feedback);
	he_cap->he_er_su_ppdu =
		QDF_MIN(he_cap->he_er_su_ppdu, new_cap->he_er_su_ppdu);
	he_cap->dl_mu_mimo_part_bw = QDF_MIN(he_cap->dl_mu_mimo_part_bw,
					     new_cap->dl_mu_mimo_part_bw);
	he_cap->ppet_present =
		QDF_MIN(he_cap->ppet_present, new_cap->ppet_present);
	he_cap->srp = QDF_MIN(he_cap->srp, new_cap->srp);
	he_cap->power_boost =
		QDF_MIN(he_cap->power_boost, new_cap->power_boost);
	he_cap->he_ltf_800_gi_4x =
		QDF_MIN(he_cap->he_ltf_800_gi_4x, new_cap->he_ltf_800_gi_4x);
	he_cap->er_he_ltf_800_gi_4x = QDF_MIN(he_cap->er_he_ltf_800_gi_4x,
	...
	he_cap->er_1x_he_ltf_gi =
		QDF_MIN(he_cap->er_1x_he_ltf_gi, new_cap->er_1x_he_ltf_gi);
	he_cap->midamble_tx_rx_1x_he_ltf =
		QDF_MIN(he_cap->midamble_tx_rx_1x_he_ltf,
			new_cap->midamble_tx_rx_1x_he_ltf);
	he_cap->reserved2 = QDF_MIN(he_cap->reserved2, new_cap->reserved2);
...
void wma_print_he_cap(tDot11fIEhe_cap *he_cap)
{
	...
	/* HE MAC capabilities */
	wma_debug(
		"HE Cap: HTC-HE 0x%01x, TWT: Reqstor 0x%01x Respder 0x%01x, Frag 0x%02x, "
		"Max MSDUs 0x%03x, Min frag 0x%02x, Trigg MAC pad duration 0x%02x",
	...
	wma_nofl_debug(
		" Multi-TID aggr RX 0x%03x, Link adapt 0x%02x, All ACK "
		"0x%01x, Trigg resp sched 0x%01x, A-Buff status rpt 0x%01x, "
		"BC TWT 0x%01x, 32bit BA 0x%01x",
		he_cap->multi_tid_aggr_rx_supp, he_cap->he_link_adaptation,
		he_cap->all_ack, he_cap->trigd_rsp_sched, he_cap->a_bsr,
		he_cap->broadcast_twt, he_cap->ba_32bit_bitmap);

	wma_nofl_debug(
		" MU Cascading 0x%01x, ACK enabled Multi-TID 0x%01x, OMI "
		"A-Cntrl 0x%01x, OFDMA RA 0x%01x, Max A-MPDU 0x%02x, A-MSDU "
	...
	wma_nofl_debug(
		" Rx Ctrl frame MBSS 0x%01x, BSRP A-MPDU Aggr 0x%01x, Quite Time Period "
		"0x%01x, A-BQR 0x%01x, SR Respder 0x%01x, ndp FB 0x%01x, ops supp 0x%01x",
		he_cap->rx_ctrl_frame, he_cap->bsrp_ampdu_aggr, he_cap->qtp,
		he_cap->a_bqr, he_cap->spatial_reuse_param_rspder,
		he_cap->ndp_feedback_supp, he_cap->ops_supp);

	wma_nofl_debug(
		" Amsdu in ampdu 0x%01x, Multi-TID aggr 0x%03x, sub ch sel tx 0x%01x, UL "
		"2x996tone RU 0x%01x, OM ctrl ULMU dis rx 0x%01x, DSMPS 0x%01x",
		he_cap->amsdu_in_ampdu, he_cap->multi_tid_aggr_tx_supp,
		he_cap->he_sub_ch_sel_tx_supp, he_cap->ul_2x996_tone_ru_supp,
	...
	wma_nofl_debug(
		" Punctured sounding 0x%01x, HTVHT Trigg frame Rx 0x%01x, Chan width "
		"0x%07x, Preamble punct Rx 0x%04x, device class 0x%01x, LDPC 0x%01x",
		he_cap->punctured_sounding_supp, he_cap->ht_vht_trg_frm_rx_supp,
		chan_width, he_cap->rx_pream_puncturing, he_cap->device_class,
		he_cap->ldpc_coding);

	wma_nofl_debug(
		" LTF and GI for HE PPDUs 0x%02x, Midamble Tx Rx MAX NSTS 0x%02x, LTF "
		"and GI for NDP 0x%02x, TB PPDU STBC Tx support LE 80MHz 0x%01x",
		he_cap->he_1x_ltf_800_gi_ppdu, he_cap->midamble_tx_rx_max_nsts,
		he_cap->he_4x_ltf_3200_gi_ndp,
		he_cap->tb_ppdu_tx_stbc_lt_80mhz);

	wma_nofl_debug(
		" STBC Rx support LE 80Mhz 0x%01x, Doppler 0x%02x, UL MU: 0x%02x, DCM "
		"encoding: Tx 0x%03x Rx 0x%03x, HE MU PPDU payload 0x%01x",
		he_cap->rx_stbc_lt_80mhz, he_cap->doppler, he_cap->ul_mu,
		he_cap->dcm_enc_tx, he_cap->dcm_enc_rx, he_cap->ul_he_mu);

	wma_nofl_debug(
		" SU: Bfer 0x%01x Bfee 0x%01x, MU Bfer 0x%01x, Bfee STS:  LE "
		"80Mhz 0x%03x GT 80Mhz 0x%03x, No. of sounding dim: LE 80Mhz "
		"0x%03x GT 80Mhz 0x%03x",
	...
	wma_nofl_debug(
		" Ng=16 FB: SU 0x%01x MU 0x%01x, Codebook size: SU 0x%01x MU 0x%01x, "
		"Bfing trigger w/ Trigger 0x%01x, ER SU PPDU payload 0x%01x",
	...
	wma_nofl_debug(
		" DL MUMIMO on partial BW: 0x%01x, PPET 0x%01x, SRP 0x%01x, "
		"Power boost 0x%01x, 4x HE LTF 0x%01x, Max NC 0x%01x, TB PPDU "
		"stbc Tx GT 80Mhz 0x%01x",
		he_cap->dl_mu_mimo_part_bw, he_cap->ppet_present, he_cap->srp,
		he_cap->power_boost, he_cap->he_ltf_800_gi_4x, he_cap->max_nc,
		he_cap->tb_ppdu_tx_stbc_gt_80mhz);

	wma_nofl_debug(
		" STBC Rx GT 80Mhz 0x%01x, er ltf 800 gt 4x 0x%01x, ppdu 20: 40Mhz 2G "
		"0x%01x 160_80p80Mhz 0x%01x, ppdu 80 160_80p80Mhz 0x%01x",
		he_cap->rx_stbc_gt_80mhz, he_cap->er_he_ltf_800_gi_4x,
		he_cap->he_ppdu_20_in_40Mhz_2G,
		he_cap->he_ppdu_20_in_160_80p80Mhz,
		he_cap->he_ppdu_80_in_160_80p80Mhz);

	wma_nofl_debug(
		" ER 1X LTF GI 0x%01x, midamble tx/rx 1x LTF 0x%01x, DCM max BW 0x%02x, "
		"LT 16 sigb ofdm sym 0x%01x, non-trig cqi fb 0x%01x",
		he_cap->er_1x_he_ltf_gi, he_cap->midamble_tx_rx_1x_he_ltf,
		he_cap->dcm_max_bw, he_cap->longer_than_16_he_sigb_ofdm_sym,
		he_cap->non_trig_cqi_feedback);

	wma_nofl_debug(
		" 1024 QAM LT 242 tone ru: Tx 0x%01x RX 0x%01x, RX full BW SU "
		"MU SIGB: Compress 0x%01x non compress 0x%01x",
		he_cap->tx_1024_qam_lt_242_tone_ru,
		he_cap->rx_1024_qam_lt_242_tone_ru,
		he_cap->rx_full_bw_su_he_mu_compress_sigb,
		he_cap->rx_full_bw_su_he_mu_non_cmpr_sigb);
	...
	wma_nofl_debug(
		" RX MCS: LE 80MHz 0x%x 160MHz 0x%x 80+80MHz 0x%x, TX MCS: LE 80MHz 0x%x "
		"160MHz 0x%x 80+80MHz 0x%x, NSS %d, RU index 0x%04x, num ppet %d",
		he_cap->rx_he_mcs_map_lt_80,
		*((uint16_t *)he_cap->rx_he_mcs_map_160),
		*((uint16_t *)he_cap->rx_he_mcs_map_80_80),
		he_cap->tx_he_mcs_map_lt_80,
		*((uint16_t *)he_cap->tx_he_mcs_map_160),
		*((uint16_t *)he_cap->tx_he_mcs_map_80_80), hdr->nss + 1,
		hdr->ru_idx_mask, he_cap->ppet.ppe_threshold.num_ppe_th);
...
void wma_print_he_ppet(void *he_ppet)
{
	...
	for (i = 0; i < numss; i++) {
		...
		for (j = 1; j <= ru_count; j++) {
			wma_nofl_debug(
				"\tNSS[%d],RU[%d]: PPET16: %02x PPET8: %02x", i,
				j,
				WMI_GET_PPET16(ppet->ppet16_ppet8_ru3_ru0, j,
					       i),
				WMI_GET_PPET8(ppet->ppet16_ppet8_ru3_ru0, j,
					      i));
		}
	}
}
...
{
	wma_debug(
		"HE PHY Caps: Chan width 0x%07x, Preamble punct Rx 0x%04x, dev "
		"class 0x%01x, LDPC 0x%01x, LTF and GI: HE PPDUs 0x%02x NDP 0x%02x",
		WMI_HECAP_PHY_CBW_GET(phy_cap),
		WMI_HECAP_PHY_PREAMBLEPUNCRX_GET(phy_cap),
		WMI_HECAP_PHY_COD_GET(phy_cap), WMI_HECAP_PHY_LDPC_GET(phy_cap),
		WMI_HECAP_PHY_LTFGIFORHE_GET(phy_cap),
		WMI_HECAP_PHY_LTFGIFORNDP_GET(phy_cap));

	wma_nofl_debug(
		" STBC Tx & Rx support LE 80Mhz 0x%02x, Doppler 0x%02x, UL MU: Full BW "
		"0x%01x Partial BW 0x%01x, DCM encoding: TX 0x%03x RX 0x%03x",
	...
	wma_nofl_debug(
		" HE MU PPDU payload 0x%01x, SU: Bfer 0x%01x Bfee 0x%01x, MU Bfer "
		"0x%01x, Bfee STS: LE 80Mhz 0x%03x GT 80Mhz 0x%03x",
	...
	wma_nofl_debug(
		" NSTS: LE 80Mhz 0x%03x GT 80Mhz 0x%03x, No of Sounding dim: LE 80Mhz "
		"0x%03x GT 80Mhz 0x%03x, Ng=16 FB: SU 0x%01x MU 0x%01x",
	...
	wma_nofl_debug(
		" Codebook size: SU 0x%01x MU 0x%01x, Beamforming trigger w/ Trigger "
		"0x%01x, HE ER SU PPDU 0x%01x, DL MUMIMO on partial BW 0x%01x",
		...
		WMI_HECAP_PHY_DLMUMIMOPARTIALBW_GET(phy_cap));
	wma_nofl_debug(
		" PPET 0x%01x, SRP based SR 0x%01x, Power boost 0x%01x, 4x HE LTF "
		"0x%01x, Max Nc 0x%03x, STBC Tx/Rx GT 80Mhz 0x%02x, ER 4x HE LTF 0x%01x",
...
{
	wma_debug(
		"HE MAC Caps: HTC-HE control 0x%01x, TWT Requestor 0x%01x, TWT Responder "
		"0x%01x, Fragmentation 0x%02x, Max no.of MSDUs 0x%03x, Min. frag 0x%02x",
	...
	wma_nofl_debug(
		" Trigger MAC pad dur 0x%02x, Multi-TID aggr Rx 0x%03x, Link adaptation "
		"0x%02x, All ACK 0x%01x, UL MU resp sched 0x%01x, A-Buff status 0x%01x",
		WMI_HECAP_MAC_TRIGPADDUR_GET(mac_cap),
		WMI_HECAP_MAC_MTID_RX_GET(mac_cap),
		WMI_HECAP_MAC_HELINK_ADPT_GET(mac_cap),
		WMI_HECAP_MAC_AACK_GET(mac_cap), WMI_HECAP_MAC_TRS_GET(mac_cap),
		WMI_HECAP_MAC_BSR_GET(mac_cap));
	wma_nofl_debug(
		" BC TWT 0x%01x, 32bit BA 0x%01x, MU Cascading 0x%01x, ACK enabled "
		"Multi-TID 0x%01x, OMI A-Control 0x%01x, OFDMA RA 0x%01x, Max A-MPDU "
		"0x%02x",
	...
	wma_nofl_debug(
		" A-MSDU Fragmentation: 0x%01x, Flex. TWT sched 0x%01x, Rx "
		"Ctrl frame to MBSS: 0x%01x",
...
{
	wma_nofl_debug(
		" BSRP A-MPDU Aggregation 0x%01x, Quite Time Period 0x%01x, A-BQR "
		"0x%01x, SR Responder 0x%01x, NDP Feedback 0x%01x, OPS 0x%01x",
		WMI_HECAP_MAC_BSRPAMPDU_GET(mac_cap),
		WMI_HECAP_MAC_QTP_GET(mac_cap), WMI_HECAP_MAC_ABQR_GET(mac_cap),
		WMI_HECAP_MAC_SRPRESP_GET(mac_cap),
		WMI_HECAP_MAC_NDPFDBKRPT_GET(mac_cap),
		WMI_HECAP_MAC_OPS_GET(mac_cap));
	wma_nofl_debug(
		" Multi-TID aggr Tx 0x%03x, Sub Ch selective Tx 0x%01x, UL "
		"2x996 tone RU 0x%01x, OM ctrl UL MU data disable Rx: 0x%01x",
				  ...
				  struct wma_tgt_cfg *tgt_cfg)
{
	...
	for (i = 0; i < total_mac_phy_cnt; i++) {
		qdf_mem_zero(&he_cap_mac, sizeof(tDot11fIEhe_cap));
		mac_cap = &mac_phy_cap[i];
		is_5g_cap = false;
		if (mac_cap->supported_bands & WLAN_2G_CAPABILITY) {
			wma_convert_he_cap(&he_cap_mac, mac_cap->he_cap_info_2G,
		...
		if (he_cap_mac.present) {
			wma_derive_ext_he_cap(&tmp_he_cap, &he_cap_mac,
					      is_5g_cap);
			wma_derive_ext_he_cap(&tgt_cfg->he_cap_2g, &he_cap_mac,
					      is_5g_cap);
		}

		qdf_mem_zero(&he_cap_mac, sizeof(tDot11fIEhe_cap));
		if (mac_cap->supported_bands & WLAN_5G_CAPABILITY) {
			wma_convert_he_cap(&he_cap_mac, mac_cap->he_cap_info_5G,
		...
		if (he_cap_mac.present) {
			wma_derive_ext_he_cap(&tmp_he_cap, &he_cap_mac,
					      is_5g_cap);
			wma_derive_ext_he_cap(&tgt_cfg->he_cap_5g, &he_cap_mac,
					      is_5g_cap);
		}
	}
...
{
	wma_debug(
		"bss_color %0x def_pe_dur %0x twt_req %0x txop_rts_thre %0x "
		"vht_oper %0x part_bss_color %0x, MBSSID AP %0x BSS color dis %0x",
			      ...
			      struct wmi_host_ppe_threshold *peer_ppet)
{
	...
	wma_debug("Rcvd nss=%d ru_idx_mask: %0x ru_count=%d", nss,
		  hdr->ru_idx_mask, ru);
   ...
   * +------------+-----------+-----+------------+-----------+-----+-----------+-----------+
   * | PPET16 for | PPET8 for | ... | PPET16 for | PPET8 for | ... | PET16 for |
   * PPET8 for | | NSS1, RU1  | NSS1, RU1 | ... | NSS1, RUm  | NSS1, RUm | ... |
   * NSSn, RUm | NSSn, RUm |
   * +------------+-----------+-----+------------+-----------+-----+-----------+-----------+
   *        3           3       ...       3           3        ...       3 3
   */
			      ...
			      tpAddStaParams params)
{
	...
	WMI_HECAP_PHY_HEPPDU20IN40MHZ2G_SET(phy_cap,
					    he_cap->he_ppdu_20_in_40Mhz_2G);
	WMI_HECAP_PHY_HEPPDU20IN160OR80P80MHZ_SET(
		phy_cap, he_cap->he_ppdu_20_in_160_80p80Mhz);
	WMI_HECAP_PHY_HEPPDU80IN160OR80P80MHZ_SET(
		phy_cap, he_cap->he_ppdu_80_in_160_80p80Mhz);
	WMI_HECAP_PHY_ERSU1X800NSECGI_SET(phy_cap, he_cap->er_1x_he_ltf_gi);
	WMI_HECAP_PHY_MIDAMBLETXRX2XAND1XHELTF_SET(
		phy_cap, he_cap->midamble_tx_rx_1x_he_ltf);
	WMI_HECAP_PHY_DCMMAXBW_SET(phy_cap, he_cap->dcm_max_bw);
	WMI_HECAP_PHY_LNG16SIGBSYMBSUPRT_SET(
		phy_cap, he_cap->longer_than_16_he_sigb_ofdm_sym);
	WMI_HECAP_PHY_NONTRIGCQIFEEDBK_SET(phy_cap,
					   he_cap->non_trig_cqi_feedback);
	WMI_HECAP_PHY_TX1024QAM242RUSUPRT_SET(
		phy_cap, he_cap->tx_1024_qam_lt_242_tone_ru);
	WMI_HECAP_PHY_RX1024QAM242RUSUPRT_SET(
		phy_cap, he_cap->rx_1024_qam_lt_242_tone_ru);
	WMI_HECAP_PHY_RXFULBWSUWCMPRSSIGB_SET(
		phy_cap, he_cap->rx_full_bw_su_he_mu_compress_sigb);
	WMI_HECAP_PHY_RXFULBWSUWNONCMPRSSIGB_SET(
		phy_cap, he_cap->rx_full_bw_su_he_mu_non_cmpr_sigb);
	...
	if (params->he_mcs_12_13_map) {
		peer->peer_he_tx_mcs_set[0] |=
			(params->he_mcs_12_13_map << WMA_MCS_12_13_MAP_L80) &
			    ...
			    struct bss_params *add_bss)
{
	...
	wma_debug(
		"HE_STA_OBSSPD: PD_MIN: %d PD_MAX: %d SEC_CH_ED: %d TX_PWR: %d",
...
void wma_set_he_txbf_cfg(struct mac_context *mac, uint8_t vdev_id)
{
	wma_set_he_txbf_params(
		vdev_id, mac->mlme_cfg->he_caps.dot11_he_cap.su_beamformer,
		mac->mlme_cfg->he_caps.dot11_he_cap.su_beamformee,
		mac->mlme_cfg->he_caps.dot11_he_cap.mu_beamformer);
}

void wma_set_he_txbf_params(uint8_t vdev_id, bool su_bfer, bool su_bfee,
			    bool mu_bfer)
{
	uint32_t hemu_mode;
	QDF_STATUS status;
...
QDF_STATUS wma_get_he_capabilities(struct he_capability *he_cap)
{
	...
	qdf_mem_copy(he_cap->phy_cap, &wma_handle->he_cap.phy_cap,
...
QDF_STATUS wma_get_hemu_mode(uint32_t *hemumode, struct mac_context *mac)
{
	...
	*hemumode |= ((mac->mlme_cfg->he_caps.dot11_he_cap.su_beamformer
		       << HE_SUBFER) |
		      (mac->mlme_cfg->he_caps.dot11_he_cap.su_beamformee
		       << HE_SUBFEE) |
		      (mac->mlme_cfg->he_caps.dot11_he_cap.mu_beamformer
...
void wma_allow_suspend_after_obss_color_change(struct wlan_objmgr_vdev *vdev)
{
	...
	qdf_runtime_pm_allow_suspend(&mlme_priv->bss_color_change_runtime_lock);
