				...
				uint32_t *phy_cap)
{
	...
	eht_cap->eht_trs_support = WMI_EHTCAP_MAC_TRS_SUPPORT_GET(mac_cap);
	eht_cap->txop_return_support_txop_share_m2 =
		WMI_EHTCAP_MAC_TXOP_RETURN_SUPP_IN_SHARINGMODE2_GET(mac_cap);
	eht_cap->two_bqrs_support = WMI_EHTCAP_MAC_TWO_BQRS_SUPP_GET(mac_cap);
	...
	eht_cap->partial_bw_dl_mu_mimo =
		WMI_EHTCAP_PHY_PARTBWDLMUMIMO_GET(phy_cap);
	...
	eht_cap->triggered_mu_bf_full_bw_fb_and_dl_mumimo =
		WMI_EHTCAP_PHY_20MHZ_ONLY_TRIGGER_MUBF_FULL_BW_FB_AND_DLMUMIMO_GET(
			phy_cap);
...
static void wma_update_eht_20mhz_only_mcs(uint32_t *mcs_2g_20,
					  tDot11fIEeht_cap *eht_cap)
{
...
static void wma_update_eht_le_80mhz_mcs(uint32_t *mcs_le_80,
					tDot11fIEeht_cap *eht_cap)
{
...
static void wma_update_eht_160mhz_mcs(uint32_t *mcs_160mhz,
				      tDot11fIEeht_cap *eht_cap)
{
...
static void wma_update_eht_320mhz_mcs(uint32_t *mcs_320mhz,
				      tDot11fIEeht_cap *eht_cap)
{
...
void wma_print_eht_cap(tDot11fIEeht_cap *eht_cap)
{
	...
	wma_debug(
		"EHT Caps: EPCS PA 0x%01x OM ctl 0x%01x Triggered TXOP Sharing "
		"mode1:0x%01x mode2:0x%01x, Restricted TWT 0x%01x SCS Traffic Desc "
		"0x%01x Max MPDU 0x%01x Max A-MPDU exponent ext: 0x%01x",
		eht_cap->epcs_pri_access, eht_cap->eht_om_ctl,
		eht_cap->triggered_txop_sharing_mode1,
		eht_cap->triggered_txop_sharing_mode2, eht_cap->restricted_twt,
		eht_cap->scs_traffic_desc, eht_cap->max_mpdu_len,
		eht_cap->max_a_mpdu_len_exponent_ext);
	wma_nofl_debug(
		" TRS supp 0x%01x TXOP return support in TXOP M2 0x%01x Two BQRs supp "
		"0x%01x EHT link adaptation supp 0x%01x 320MHz 6GHz 0x%01x 242-tone RU "
		"WT 20 MHz 0x%01x NDP_4x EHT-LTF 3.2 us GI 0x%01x",
		eht_cap->eht_trs_support,
		eht_cap->txop_return_support_txop_share_m2,
		eht_cap->two_bqrs_support, eht_cap->eht_link_adaptation_support,
		eht_cap->support_320mhz_6ghz, eht_cap->ru_242tone_wt_20mhz,
		eht_cap->ndp_4x_eht_ltf_3dot2_us_gi);
	wma_nofl_debug(
		" Partial BW UL MU-MIMO: 0x%01x, SU: Bfer 0x%01x Bfee 0x%01x, Bfee SS: "
		"LE 80Mhz 0x%03x  160Mhz 0x%03x 320Mhz 0x%03x, No. of Sounding Dim LE "
		"80Mhz 0x%03x  160Mhz 0x%03x 320Mhz 0x%03x ",
		eht_cap->partial_bw_mu_mimo, eht_cap->su_beamformer,
		eht_cap->su_beamformee, eht_cap->bfee_ss_le_80mhz,
		eht_cap->bfee_ss_160mhz, eht_cap->bfee_ss_320mhz,
		eht_cap->num_sounding_dim_le_80mhz,
		eht_cap->num_sounding_dim_160mhz,
		eht_cap->num_sounding_dim_320mhz);
	wma_nofl_debug(
		" Ng 16: SU Feedback 0x%01x, MU Feedback 0x%01x Codebook: 4 2 SU: "
		"0x%01x, 7 5 MU: 0x%01x, Trig SU Bfing fb 0x%01x, MU Bfing partial BW "
		"0x%01x Trig CQI FB 0x%01x, Part BW DL MU-MIMO: 0x%01x",
		...
		eht_cap->partial_bw_dl_mu_mimo);
	wma_nofl_debug(
		" PSR-Based SR 0x%01x, Power Boost Factor 0x%01x, MU PPDU With 4x "
		"EHT-LTF 0.8 us GI 0x%01x Max Nc: 0x%04x, Non-Trig CQI FB 0x%01x, "
		"1024-QAM 4096-QAM < 242-tone RU: TX 0x%01x RX 0x%01x",
		eht_cap->psr_based_sr, eht_cap->power_boost_factor,
		eht_cap->eht_mu_ppdu_4x_ltf_0_8_us_gi, eht_cap->max_nc,
		eht_cap->non_trig_cqi_feedback,
		eht_cap->tx_1024_4096_qam_lt_242_tone_ru,
		eht_cap->rx_1024_4096_qam_lt_242_tone_ru);
	wma_nofl_debug(
		" PPE Thresholds 0x%01x, Common Nominal Pkt Padding 0x%02x, "
		"Max No. Sup EHT-LTFs 0x%05x, MCS 15 0x%04x, DUP 6 GHz "
		"0x%01x, 20 MHz STA RX NDP With Wider BW 0x%01x",
		eht_cap->ppet_present, eht_cap->common_nominal_pkt_padding,
		eht_cap->max_num_eht_ltf, eht_cap->mcs_15,
		eht_cap->eht_dup_6ghz, eht_cap->op_sta_rx_ndp_wider_bw_20mhz);
	wma_nofl_debug(
		" Non-OFDMA ULMU: LE 80MHz 0x%01x 160MHz 0x%01x 320MHz 0x%01x, MUBfmer: "
		"LE 80MHz 0x%01x 160MHz 0x%01x 320MHz 0x%01x, TB sound FBRL 0x%01x, WBDL "
		"OFDMA Rx: 1024QAM 0x%01x 4096QAM 0x%01x",
		eht_cap->non_ofdma_ul_mu_mimo_le_80mhz,
		eht_cap->non_ofdma_ul_mu_mimo_160mhz,
		eht_cap->non_ofdma_ul_mu_mimo_320mhz,
		eht_cap->mu_bformer_le_80mhz, eht_cap->mu_bformer_160mhz,
		eht_cap->mu_bformer_320mhz, eht_cap->tb_sounding_feedback_rl,
		eht_cap->rx_1k_qam_in_wider_bw_dl_ofdma,
		eht_cap->rx_4k_qam_in_wider_bw_dl_ofdma);
	wma_nofl_debug(
		" 20 MHz-Only: Limited Cap 0x%01x Triggered MU Bfing Full BW "
		"FB, DL MU-MIMO 0x%01x M-RU Support 0x%01x, EHT MCS: 20MHz::: "
		"0-7: RX: 0x%x TX: 0x%x, 8-9: RX: 0x%x TX: 0x%x",
		eht_cap->limited_cap_support_20mhz,
		eht_cap->triggered_mu_bf_full_bw_fb_and_dl_mumimo,
		eht_cap->mru_support_20mhz,
		eht_cap->bw_20_rx_max_nss_for_mcs_0_to_7,
		...
		eht_cap->bw_20_tx_max_nss_for_mcs_8_and_9);
	wma_nofl_debug(
		" 20MHz::: 10-11: RX: 0x%x TX: 0x%x, 12-13: RX: 0x%x TX: "
		"0x%x, 80Mhz LE::: 0-9: RX: 0x%x TX: 0x%x, 10-11: RX: 0x%x "
		...
		eht_cap->bw_le_80_tx_max_nss_for_mcs_12_and_13);
	wma_nofl_debug(
		" 160Mhz::: 0-9: RX: 0x%x TX: 0x%x, 10-11: RX: 0x%x TX: 0x%x, "
		"12-13: RX: 0x%x TX: 0x%x, 320Mhz::: 0-9: RX: 0x%x TX: 0x%x, "
		"10-11: RX: 0x%x TX: 0x%x, 12-13: RX: 0x%x TX: 0x%x",
		eht_cap->bw_160_rx_max_nss_for_mcs_0_to_9,
		eht_cap->bw_160_tx_max_nss_for_mcs_0_to_9,
		eht_cap->bw_160_rx_max_nss_for_mcs_10_and_11,
		eht_cap->bw_160_tx_max_nss_for_mcs_10_and_11,
...
{
	wma_debug(
		"EHT PHY Cap: 320 MHz In 6 GHz 0x%01x, 242-tone RU In BW Wider "
		"Than 20 MHz 0x%01x, NDP With 4x EHT-LTF And 3.2 us GI 0x%01x, "
		"Partial BW UL MU-MIMO 0x%01x",
		WMI_EHTCAP_PHY_320MHZIN6GHZ_GET(phy_cap),
		WMI_EHTCAP_PHY_242TONERUBWLT20MHZ_GET(phy_cap),
		WMI_EHTCAP_PHY_NDP4XEHTLTFAND320NSGI_GET(phy_cap),
		WMI_EHTCAP_PHY_PARTIALBWULMU_GET(phy_cap));
	wma_nofl_debug(
		" SU: Bfmer 0x%01x Bfmee 0x%01x, Bfmee SS: LE 80MHz 0x%03x "
		"160MHz 0x%03x 320MHz 0x%03x, No. of Sounding Dim: LE 80MHz "
		...
		WMI_EHTCAP_PHY_NUMSOUND320MHZ_GET(phy_cap));
	wma_nofl_debug(
		" Ng 16 FB: SU 0x%01x MU 0x%01x, Codebook Size: 42 SU FB 0x%01x 75 MU "
		"FB: 0x%01x, Trigg SU Bfming FB 0x%01x, MU Bfming Partial BW FB 0x%01x",
		...
		WMI_EHTCAP_PHY_TRIGMUBFPARTBWFB_GET(phy_cap));
	wma_nofl_debug(
		" Triggered CQI FB 0x%01x, Partial BW DL MU-MIMO 0x%01x, PSR-Based SR "
		"0x%01x, Power Boost Factor 0x%01x, MU PPDU 4x EHT-LTF 0.8 us GI 0x%01x",
		...
		WMI_EHTCAP_PHY_4XEHTLTFAND800NSGI_GET(phy_cap));
	wma_nofl_debug(
		" Max Nc 0x%04x, Non-Triggered CQI FB 0x%01x, 1024-QAM "
		"4096-QAM < 242-tone RU: TX 0x%01x RX 0x%01x, PPE Thresholds "
		"0x%01x, Common Nominal Packet Padding 0x%02x",
		...
		WMI_EHTCAP_PHY_CMNNOMPKTPAD_GET(phy_cap));
	wma_nofl_debug(
		" Max No. Supp LTFs 0x%05x, MCS 15 0x%04x, EHT DUP 6 GHz "
		"0x%01x, 20MHz STA RX NDP Wider BW 0x%01x, Non-OFDMA UL "
		"MU-MIMO: LE 80MHz 0x%01x 160 MHz 0x%01x 320Mhz 0x%01x",
		...
		WMI_EHTCAP_PHY_NONOFDMAULMUMIMO320MHZ_GET(phy_cap));
	wma_nofl_debug(
		" MUBfmer: LE 80MHz 0x%01x 160MHz 0x%01x 320Mhz 0x%01x, TB sound FBRL "
		"0x%01x, WBW DLOFDMA Rx: 1024QAM 0x%01x 4096QAM 0x%01x, 20MHz: Lim Cap "
		"0x%01x Trig MUBfing BWFB DLMU 0x%01x M-RU 0x%01x",
		WMI_EHTCAP_PHY_MUBFMRLT80MHZ_GET(phy_cap),
...
{
	wma_debug(
		"EHT MAC Cap: EPCS Priority Access: 0x%01x OM Control: 0x%01x, "
		"Trig TXOP Sharing: mode1 0x%01x mode2 0x%01x, Restricted TWT "
		"0x%01x SCS Traffic Desc 0x%01x",
	...
	wma_nofl_debug(
		" Max MPDU len 0x%01x, Max A-MPDU Len Exponent Ext 0x%01x EHT "
		"TRS 0x%01x, OP In TXOP Sharing Mode2 0x%01x, Two BQRs "
		"0x%01x, EHT Link Adaptation 0x%01x",
			       ...
			       tpAddStaParams params)
{
	...
	WMI_EHTCAP_MAC_TRIGTXOPMODE2_SET(mac_cap,
					 eht_cap->triggered_txop_sharing_mode2);
	WMI_EHTCAP_MAC_RESTRICTTWT_SET(mac_cap, eht_cap->restricted_twt);
	WMI_EHTCAP_MAC_SCSTRAFFICDESC_SET(mac_cap, eht_cap->scs_traffic_desc);
	WMI_EHTCAP_MAC_MAXMPDULEN_SET(mac_cap, eht_cap->max_mpdu_len);
	WMI_EHTCAP_MAC_MAXAMPDULEN_EXP_SET(
		mac_cap, eht_cap->max_a_mpdu_len_exponent_ext);
	WMI_EHTCAP_MAC_TRS_SUPPORT_SET(mac_cap, eht_cap->eht_trs_support);
	WMI_EHTCAP_MAC_TXOP_RETURN_SUPP_IN_SHARINGMODE2_SET(
		mac_cap, eht_cap->txop_return_support_txop_share_m2);
	WMI_EHTCAP_MAC_TWO_BQRS_SUPP_SET(mac_cap, eht_cap->two_bqrs_support);
	WMI_EHTCAP_MAC_EHT_LINK_ADAPTATION_SUPP_SET(
		mac_cap, eht_cap->eht_link_adaptation_support);
	...
	WMI_EHTCAP_PHY_NUMSOUNDLT80MHZ_SET(phy_cap,
					   eht_cap->num_sounding_dim_le_80mhz);
	...
	WMI_EHTCAP_PHY_TBSUNDFBRATELIMIT_SET(phy_cap,
					     eht_cap->tb_sounding_feedback_rl);
	WMI_EHTCAP_PHY_RX1024QAMWIDERBWDLOFDMA_SET(
		phy_cap, eht_cap->rx_1k_qam_in_wider_bw_dl_ofdma);
	WMI_EHTCAP_PHY_RX4096QAMWIDERBWDLOFDMA_SET(
		phy_cap, eht_cap->rx_4k_qam_in_wider_bw_dl_ofdma);
	WMI_EHTCAP_PHY_20MHZ_ONLY_CAPS_SET(phy_cap,
					   eht_cap->limited_cap_support_20mhz);
	WMI_EHTCAP_PHY_20MHZ_ONLY_TRIGGER_MUBF_FULL_BW_FB_AND_DLMUMIMO_SET(
		phy_cap, eht_cap->triggered_mu_bf_full_bw_fb_and_dl_mumimo);
   ...
   */
	...
	case CH_WIDTH_320MHZ:
		...
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 0,
			4, rates->bw_320_rx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 0,
			4, rates->bw_320_tx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 4,
			4, rates->bw_320_rx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 4,
			4, rates->bw_320_tx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 8,
			4, rates->bw_320_rx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 8,
			4, rates->bw_320_tx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 12,
			4, rates->bw_320_rx_max_nss_for_mcs_12_and_13);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX2], 12,
			4, rates->bw_320_tx_max_nss_for_mcs_12_and_13);
		fallthrough;
	case CH_WIDTH_160MHZ:
		peer->peer_eht_mcs_count++;
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 0,
			4, rates->bw_160_rx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 0,
			4, rates->bw_160_tx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 4,
			4, rates->bw_160_rx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 4,
			4, rates->bw_160_tx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 8,
			4, rates->bw_160_rx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 8,
			4, rates->bw_160_rx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 12,
			4, rates->bw_160_rx_max_nss_for_mcs_12_and_13);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX1], 12,
			4, rates->bw_160_tx_max_nss_for_mcs_12_and_13);
		fallthrough;
	case CH_WIDTH_80MHZ:
	case CH_WIDTH_40MHZ:
		peer->peer_eht_mcs_count++;
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 0,
			4, rates->bw_le_80_rx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 0,
			4, rates->bw_le_80_tx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 4,
			4, rates->bw_le_80_rx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 4,
			4, rates->bw_le_80_tx_max_nss_for_mcs_0_to_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 8,
			4, rates->bw_le_80_rx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 8,
			4, rates->bw_le_80_tx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 12,
			4, rates->bw_le_80_rx_max_nss_for_mcs_12_and_13);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 12,
			4, rates->bw_le_80_rx_max_nss_for_mcs_12_and_13);
		break;
	case CH_WIDTH_20MHZ:
		peer->peer_eht_mcs_count++;
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 0,
			4, rates->bw_20_rx_max_nss_for_mcs_0_to_7);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 0,
			4, rates->bw_20_tx_max_nss_for_mcs_0_to_7);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 4,
			4, rates->bw_20_rx_max_nss_for_mcs_8_and_9);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 4,
			4, rates->bw_20_tx_max_nss_for_mcs_8_and_9);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 8,
			4, rates->bw_20_rx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 8,
			4, rates->bw_20_tx_max_nss_for_mcs_10_and_11);
		QDF_SET_BITS(
			peer->peer_eht_rx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 12,
			4, rates->bw_20_rx_max_nss_for_mcs_12_and_13);
		QDF_SET_BITS(
			peer->peer_eht_tx_mcs_set[EHTCAP_TXRX_MCS_NSS_IDX0], 12,
			4, rates->bw_20_tx_max_nss_for_mcs_12_and_13);
		break;
	default:
		break;
	}
...
QDF_STATUS wma_get_eht_capabilities(struct eht_capability *eht_cap)
{
	...
	qdf_mem_copy(eht_cap->phy_cap, &wma_handle->eht_cap.phy_cap,
...
void wma_set_eht_txbf_params(uint8_t vdev_id, bool su_bfer, bool su_bfee,
			     bool mu_bfer)
{
...
uint16_t wma_match_eht_rate(uint16_t raw_rate, enum tx_rate_info rate_flags,
			    uint8_t *nss, uint8_t *dcm,
			    enum txrate_gi *guard_interval,
			    enum tx_rate_info *mcs_rate_flag, uint8_t *p_index)
{
	uint8_t index;
	...
	for (index = 0; index < QDF_ARRAY_SIZE(eht_mcs_nss1); index++) {
		...
		for (dcm_index = 0; dcm_index < dcm_index_max; dcm_index++) {
			if (rate_flags & TX_RATE_EHT320) {
				nss1_rate = &eht_mcs_nss1[index]
						     .supported_eht320_rate
							     [dcm_index][0];
				nss2_rate = &eht_mcs_nss2[index]
						     .supported_eht320_rate
							     [dcm_index][0];
			...
			if (rate_flags & (TX_RATE_EHT80 | TX_RATE_EHT160)) {
				nss1_rate =
					&eht_mcs_nss1[index]
						 .supported_eht80_rate[dcm_index]
								      [0];
				nss2_rate =
					&eht_mcs_nss2[index]
						 .supported_eht80_rate[dcm_index]
			...
			if (rate_flags &
				...
				nss1_rate =
					&eht_mcs_nss1[index]
						 .supported_eht40_rate[dcm_index]
								      [0];
				nss2_rate =
					&eht_mcs_nss2[index]
						 .supported_eht40_rate[dcm_index]
			...
			if (rate_flags & (TX_RATE_EHT80 | TX_RATE_EHT40 |
				...
				nss1_rate =
					&eht_mcs_nss1[index]
						 .supported_eht20_rate[dcm_index]
								      [0];
				nss2_rate =
					&eht_mcs_nss2[index]
						 .supported_eht20_rate[dcm_index]
