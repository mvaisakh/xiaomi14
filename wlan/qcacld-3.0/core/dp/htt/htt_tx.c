...
#define HTT_TX_DESC_FRAG_FIELD_UPDATE(frag_filed_ptr, frag_desc_addr)     \
	do {                                                              \
		*frag_filed_ptr = qdf_get_lower_32_bits(frag_desc_addr);  \
		frag_filed_ptr++;                                         \
		/* frags_desc_ptr.hi */                                   \
		*frag_filed_ptr = qdf_get_upper_32_bits(frag_desc_addr) & \
				  0x1F;                                   \
	} while (0)
#else
#define HTT_TX_DESC_FRAG_FIELD_UPDATE(frag_filed_ptr, frag_desc_addr)    \
...
static void htt_tx_desc_get_size(struct htt_pdev_t *pdev)
{
     ...
     */
		pdev->frag_descs.size =
			(ol_cfg_netbuf_frags_max(pdev->ctrl_pdev) + 1) * 8 + 4;
	}
}
					  ...
					  struct htt_tx_msdu_desc_t *desc_v_ptr)
{
	...
	frag_desc_addr =
		(dma_page->page_p_addr + offset * pdev->frag_descs.size);
				   ...
				   uint16_t desc_pool_elems)
{
	...
	qdf_mem_multi_pages_alloc(
		pdev->osdev, &pdev->frag_descs.desc_pages,
 ...
 * Return: None
 */
int htt_tx_frag_alloc(htt_pdev_handle pdev, u_int16_t index,
		      qdf_dma_addr_t *frag_paddr, void **frag_ptr)
{
   ...
   */
	...
	frag_page_index =
		index / pdev->frag_descs.desc_pages.num_element_per_page;
	frag_elem_index =
		index % pdev->frag_descs.desc_pages.num_element_per_page;
	...
	*frag_paddr =
		dma_page->page_p_addr + frag_elem_index * pdev->frag_descs.size;
	return 0;
}
#else
...
int htt_tx_attach(struct htt_pdev_t *pdev, int desc_pool_elems)
{
   ...
   */
	...
	qdf_mem_multi_pages_alloc(
		pdev->osdev, &pdev->tx_descs.desc_pages, pdev->tx_descs.size,
		pdev->tx_descs.pool_elems,
		qdf_get_dma_mem_context((&pdev->tx_descs), memctx), true);
...
free_htt_desc:
	qdf_mem_multi_pages_free(
		pdev->osdev, &pdev->tx_descs.desc_pages,
		qdf_get_dma_mem_context((&pdev->tx_descs), memctx), true);
out_fail:
	return -ENOBUFS;
}
 ...
 * Return: None
 */
void htt_tx_desc_frags_table_set(htt_pdev_handle pdev, void *desc,
				 qdf_dma_addr_t paddr,
				 qdf_dma_addr_t frag_desc_paddr, int reset)
{
...
int htt_tx_credit_update(struct htt_pdev_t *pdev)
{
	...
	credit_delta =
		QDF_MIN(qdf_atomic_read(&pdev->htt_tx_credit.target_delta),
			qdf_atomic_read(&pdev->htt_tx_credit.bus_delta));
	if (credit_delta) {
		qdf_atomic_add(-credit_delta,
			       &pdev->htt_tx_credit.target_delta);
		qdf_atomic_add(-credit_delta, &pdev->htt_tx_credit.bus_delta);
	}
	return credit_delta;
}
 ...
 */
static inline qdf_dma_addr_t htt_tx_get_paddr(htt_pdev_handle pdev,
					      char *target_vaddr)
{
	return 0;
}
...
static void htt_tx_set_frag_desc_addr(struct htt_pdev_t *pdev,
				      struct htt_tx_msdu_desc_t *htt_tx_desc,
				      uint16_t index)
{
...
void htt_tx_desc_frags_table_set(htt_pdev_handle pdev, void *htt_tx_desc,
				 qdf_dma_addr_t paddr,
				 qdf_dma_addr_t frag_desc_paddr, int reset)
{
	uint32_t *fragmentation_descr_field_ptr;
...
static qdf_dma_addr_t htt_tx_get_paddr(htt_pdev_handle pdev, char *target_vaddr)
{
	uint16_t i;
...
void htt_tx_sched(htt_pdev_handle pdev)
{
	...
	while (msdu) {
       ...
       */
		...
		not_accepted = htc_send_data_pkt(pdev->htc_pdev, msdu,
...
int htt_tx_send_std(htt_pdev_handle pdev, qdf_nbuf_t msdu, uint16_t msdu_id)
{
	...
	if (htc_send_data_pkt(pdev->htc_pdev, msdu, pdev->htc_tx_endpoint,
			      download_len)) {
		HTT_TX_NBUF_QUEUE_ADD(pdev, msdu);
	}
...
qdf_nbuf_t htt_tx_send_batch(htt_pdev_handle pdev, qdf_nbuf_t head_msdu,
			     int num_msdus)
{
...
int htt_tx_send_nonstd(htt_pdev_handle pdev, qdf_nbuf_t msdu, uint16_t msdu_id,
...
static inline HTC_ENDPOINT_ID htt_tx_htt2_get_ep_id(htt_pdev_handle pdev,
						    qdf_nbuf_t msdu)
{
	/*
...
static inline int htt_tx_send_base(htt_pdev_handle pdev, qdf_nbuf_t msdu,
				   uint16_t msdu_id, int download_len,
				   uint8_t more_data)
{
   ...
   */
	htt_host_tx_desc =
		(struct htt_host_tx_desc_t *)qdf_nbuf_get_frag_vaddr(msdu, 0);
	...
	SET_HTC_PACKET_INFO_TX(&pkt->htc_pkt, pdev->tx_send_complete_part2,
			       (unsigned char *)htt_host_tx_desc,
			       download_len - HTC_HDR_LENGTH, ep_id,
...
#if HTT_PADDR64
	qdf_debug("  frag desc addr.lo = %#x", htt_tx_desc->frags_desc_ptr.lo);
	qdf_debug("  frag desc addr.hi = %#x", htt_tx_desc->frags_desc_ptr.hi);
#else /* ! HTT_PADDR64 */
					  ...
					  unsigned int uc_tx_partition_base)
{
	...
	for (tx_buffer_count = 0; tx_buffer_count < (uc_tx_buf_cnt - 1);
	     tx_buffer_count++) {
		shared_tx_buffer =
			qdf_mem_shared_mem_alloc(pdev->osdev, uc_tx_buf_sz);
		...
		buffer_paddr = qdf_mem_get_dma_addr(
			pdev->osdev, &shared_tx_buffer->mem_info);
		...
		*header_ptr = HTT_IPA_UC_OFFLOAD_TX_HEADER_DEFAULT;
		...
		/* PKT ID */
		*header_ptr |=
			((uint16_t)uc_tx_partition_base + tx_buffer_count)
			<< 16;
		...
		*header_ptr = qdf_get_lower_32_bits(
			buffer_paddr + IPA_UC_TX_BUF_FRAG_DESC_OFFSET);
		header_ptr++;
...
pwr2:
   ...
   * size.
   */
	tx_buffer_count_pwr2 =
		qdf_rounddown_pow_of_two(tx_buffer_count + 1) - 1;
	if (tx_buffer_count > tx_buffer_count_pwr2) {
		QDF_TRACE(
			QDF_MODULE_ID_HTT, QDF_TRACE_LEVEL_INFO,
 ...
 */
int htt_tx_ipa_uc_attach(struct htt_pdev_t *pdev, unsigned int uc_tx_buf_sz,
			 unsigned int uc_tx_buf_cnt,
			 unsigned int uc_tx_partition_base)
{
	int return_code = 0;
	...
	tx_comp_ring_size =
		qdf_get_pwr2(uc_tx_buf_cnt) * sizeof(target_paddr_t);
	pdev->ipa_uc_tx_rsc.tx_comp_ring =
		qdf_mem_shared_mem_alloc(pdev->osdev, tx_comp_ring_size);
	...
	pdev->ipa_uc_tx_rsc.tx_buf_pool_strg = qdf_mem_malloc(
		uc_tx_buf_cnt * sizeof(*pdev->ipa_uc_tx_rsc.tx_buf_pool_strg));
...
free_tx_comp_base:
	qdf_mem_shared_mem_free(pdev->osdev, pdev->ipa_uc_tx_rsc.tx_comp_ring);
free_tx_ce_idx:
	qdf_mem_shared_mem_free(pdev->osdev, pdev->ipa_uc_tx_rsc.tx_ce_idx);
...
void htt_tx_desc_fill_tso_info(htt_pdev_handle pdev, void *desc,
			       struct qdf_tso_info_t *tso_info)
{
	...
	msdu_ext_desc->tso_flags = tso_seg->seg.tso_flags;

	/* First 24 bytes (6*4) contain the TSO flags */
	TSO_DEBUG("%s seq# %u l2 len %d, ip len %d", __func__,
		  tso_seg->seg.tso_flags.tcp_seq_num,
		  tso_seg->seg.tso_flags.l2_len, tso_seg->seg.tso_flags.ip_len);
	TSO_DEBUG("%s flags 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", __func__, *word,
	...
	for (i = 0; i < tso_seg->seg.num_frags; i++) {
		...
		qdf_dmaaddr_to_32s(tso_seg->seg.tso_frags[i].paddr, &lo, &hi);
		...
		*word = (tso_seg->seg.tso_frags[i].length << 16) | hi;
		...
		TSO_DEBUG("%s frag[%d] ptr_low 0x%x ptr_hi 0x%x len %u",
			  __func__, i, msdu_ext_desc->frags[i].u.frag32.ptr_low,
 ...
 */
static inline int htt_get_ext_tid(enum extension_header_type type,
 ...
 */
static inline int htt_get_channel_freq(enum extension_header_type type,
				       void *ext_header_data)
{
	if (type == OCB_MODE_EXT_HEADER && ext_header_data)
...
static void
htt_fill_ocb_ext_header(qdf_nbuf_t msdu,
 ...
 */
static void
htt_fill_wisa_ext_header(qdf_nbuf_t msdu,
			 struct htt_tx_msdu_desc_ext_t *local_desc_ext,
			 enum extension_header_type type, void *ext_header_data)
{
 ...
 * Return: none
 */
static void htt_push_ext_header(qdf_nbuf_t msdu,
				struct htt_tx_msdu_desc_ext_t *local_desc_ext,
				...
				void *ext_header_data)
{
	...
	case OCB_MODE_EXT_HEADER:
		htt_fill_ocb_ext_header(msdu, local_desc_ext, type,
					ext_header_data);
		break;
	case WISA_MODE_EXT_HEADER_6MBPS:
	case WISA_MODE_EXT_HEADER_24MBPS:
		htt_fill_wisa_ext_header(msdu, local_desc_ext, type,
					 ext_header_data);
		break;
	default:
...
QDF_STATUS
htt_tx_desc_init(htt_pdev_handle pdev, void *htt_tx_desc,
		 qdf_dma_addr_t htt_tx_desc_paddr, uint16_t msdu_id,
		 qdf_nbuf_t msdu, struct htt_msdu_info_t *msdu_info,
		 struct qdf_tso_info_t *tso_info, void *ext_header_data,
		 enum extension_header_type type)
{
	...
	struct htt_host_tx_desc_t *htt_host_tx_desc =
		(struct htt_host_tx_desc_t *)(((char *)htt_tx_desc) -
					      HTT_TX_DESC_VADDR_OFFSET);
   ...
   */
	...
	HTT_TX_DESC_EXT_TID_SET(
		local_word0, htt_get_ext_tid(type, ext_header_data, msdu_info));
	...
	if (pdev->cfg.is_high_latency)
		HTT_TX_DESC_TX_COMP_SET(local_word0,
					msdu_info->action.tx_comp_req);
	HTT_TX_DESC_NO_ENCRYPT_SET(local_word0,
				   msdu_info->action.do_encrypt ? 0 : 1);
	...
	*word0 = local_word0;
	...
	if (tso_info->is_tso) {
		...
		TSO_DEBUG("%s setting HTT TX DESC Len = %d", __func__,
			  total_len);
	} else {
   ...
   */
	if (desc_ext_required) {
		...
		htt_push_ext_header(msdu, &local_desc_ext, type,
				    ext_header_data);
	}
   ...
   */
	...
	if (QDF_NBUF_CB_PADDR(msdu) == 0) {
		dir = QDF_NBUF_CB_TX_DMA_BI_MAP(msdu) ? QDF_DMA_BIDIRECTIONAL :
							QDF_DMA_TO_DEVICE;
...
void htt_tx_group_credit_process(struct htt_pdev_t *pdev, u_int32_t *msg_word)
{
	...
	while (more_group_present) {
		...
		group_credit_abs = HTT_TXQ_GROUP_CREDIT_COUNT_GET(
			*(msg_word + 1 + group_offset));
		group_credit_sign =
			HTT_TXQ_GROUP_SIGN_GET(*(msg_word + 1 + group_offset)) ?
				-1 :
				1;
		group_credit = group_credit_sign * group_credit_abs;
		group_abs =
			HTT_TXQ_GROUP_ABS_GET(*(msg_word + 1 + group_offset));

		vdev_id_mask = HTT_TXQ_GROUP_VDEV_ID_MASK_GET(
			*(msg_word + 2 + group_offset));
		ac_mask = HTT_TXQ_GROUP_AC_MASK_GET(
			*(msg_word + 2 + group_offset));
		...
		more_group_present =
			HTT_TXQ_GROUP_EXT_GET(*(msg_word + 1 + group_offset));
