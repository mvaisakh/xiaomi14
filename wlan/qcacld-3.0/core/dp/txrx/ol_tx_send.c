 ...
 * messages.
 */
static inline void ol_tx_target_credit_decr_int(struct ol_txrx_pdev_t *pdev,
						int delta)
{
	qdf_atomic_add(-1 * delta, &pdev->target_tx_credit);
}

static inline void ol_tx_target_credit_incr_int(struct ol_txrx_pdev_t *pdev,
						int delta)
{
...
static inline uint16_t ol_tx_send_base(struct ol_txrx_pdev_t *pdev,
...
void ol_tx_send(struct ol_txrx_pdev_t *pdev, struct ol_tx_desc_t *tx_desc,
		qdf_nbuf_t msdu, uint8_t vdev_id)
{
	int msdu_credit_consumed;
	uint16_t id;
	int failed;
	...
	DPTRACE(qdf_dp_trace_ptr(msdu, QDF_DP_TRACE_TXRX_PACKET_PTR_RECORD,
				 ...
				 vdev_id, 0, tx_desc->vdev->qdf_opmode));
...
void ol_tx_send_batch(struct ol_txrx_pdev_t *pdev, qdf_nbuf_t head_msdu,
		      int num_msdus)
{
	qdf_nbuf_t rejected;
...
void ol_tx_send_nonstd(struct ol_txrx_pdev_t *pdev,
		       ...
		       enum htt_pkt_type pkt_type)
{
	...
	if (failed) {
		ol_txrx_err("Error: freeing tx frame after htt_tx failed");
		ol_tx_target_credit_incr_int(pdev, msdu_credit_consumed);
		ol_tx_desc_frame_free_nonstd(pdev, tx_desc, 1 /* had error */);
	}
}

static inline bool ol_tx_download_done_base(struct ol_txrx_pdev_t *pdev,
					    A_STATUS status, qdf_nbuf_t msdu,
					    uint16_t msdu_id)
{
	struct ol_tx_desc_t *tx_desc;
   ...
   */
	if (tx_desc->pkt_type >= OL_TXRX_MGMT_TYPE_BASE) {
		ol_txrx_mgmt_tx_cb download_cb = pdev->tx_mgmt_cb.download_cb;
		if (download_cb) {
			download_cb(pdev->tx_mgmt_cb.ctxt, tx_desc->netbuf,
				    status != A_OK);
		}
	}
...
void ol_tx_download_done_ll(void *pdev, A_STATUS status, qdf_nbuf_t msdu,
			    uint16_t msdu_id)
{
...
void ol_tx_download_done_hl_retain(void *txrx_pdev, A_STATUS status,
...
void ol_tx_download_done_hl_free(void *txrx_pdev, A_STATUS status,
				 qdf_nbuf_t msdu, uint16_t msdu_id)
{
	struct ol_txrx_pdev_t *pdev = txrx_pdev;
	struct ol_tx_desc_t *tx_desc;
	...
	DPTRACE(qdf_dp_trace_ptr(msdu, QDF_DP_TRACE_FREE_PACKET_PTR_RECORD,
				 QDF_TRACE_DEFAULT_PDEV_ID,
				 qdf_nbuf_data_addr(msdu),
				 sizeof(qdf_nbuf_data(msdu)), tx_desc->id,
				 dp_status, 0, tx_desc->vdev->qdf_opmode));
...
void ol_tx_target_credit_update(struct ol_txrx_pdev_t *pdev, int credit_delta)
{
	TX_CREDIT_DEBUG_PRINT(
		...
		qdf_atomic_read(&pdev->target_tx_credit), credit_delta,
		qdf_atomic_read(&pdev->target_tx_credit) + credit_delta);
...
static void ol_tx_delay_compute(struct ol_txrx_pdev_t *pdev,
				enum htt_tx_status status, uint16_t *desc_ids,
				int num_msdus);

#else
static inline void ol_tx_delay_compute(struct ol_txrx_pdev_t *pdev,
...
int ol_tx_deduct_one_credit(struct ol_txrx_pdev_t *pdev)
{
	...
	if (!pdev->cfg.default_tx_comp_req) {
		...
		DPTRACE(qdf_dp_trace_credit_record(
			QDF_TX_HTT_MSG, QDF_CREDIT_DEC, 1,
			qdf_atomic_read(&pdev->target_tx_credit),
...
#define ol_tx_msdu_complete_single(_pdev, _tx_desc, _netbuf, _lcl_freelist,   \
				   _tx_desc_last)                             \
	do {                                                                  \
		...
		if (qdf_unlikely(!lcl_freelist)) {                            \
			(_tx_desc_last) =                                     \
				(union ol_tx_desc_list_elem_t *)(_tx_desc);   \
		}                                                             \
...
#define ol_tx_msdu_complete(_pdev, _tx_desc, _tx_descs, _netbuf,           \
			    _lcl_freelist, _tx_desc_last, _status,         \
			    is_tx_desc_freed)                              \
	{                                                                  \
		is_tx_desc_freed = 0;                                      \
		ol_tx_msdu_complete_single((_pdev), (_tx_desc), (_netbuf), \
...
#define ol_tx_msdu_complete(_pdev, _tx_desc, _tx_descs, _netbuf,            \
	...
	{                                                                   \
		is_tx_desc_freed = 0;                                       \
		ol_tx_msdu_complete_batch((_pdev), (_tx_desc), (_tx_descs), \
					  (_status))                        \
	}
#else /* !QCA_TX_STD_PATH_ONLY */
#define ol_tx_msdu_complete(_pdev, _tx_desc, _tx_descs, _netbuf,           \
...
void ol_tx_discard_target_frms(ol_txrx_pdev_handle pdev)
{
     ...
     */
		if (qdf_atomic_read(&tx_desc->ref_cnt)) {
			ol_txrx_dbg("Warning: freeing tx desc %d", tx_desc->id);
	...
	if (num_disarded)
		QDF_TRACE(
			...
			"Warning: freed %d tx descs for which no tx completion rcvd from "
			"the target",
			num_disarded);
}
#endif
 ...
 * Return: None
 */
static inline void ol_tx_flow_pool_lock(struct ol_tx_desc_t *tx_desc)
{
 ...
 * Return: None
 */
static inline void ol_tx_flow_pool_unlock(struct ol_tx_desc_t *tx_desc)
{
	struct ol_tx_flow_pool_t *pool;
...
static void ol_tx_pkt_capture_tx_completion_process(
	ol_txrx_pdev_handle pdev, struct ol_tx_desc_t *tx_desc,
	struct htt_tx_data_hdr_information *payload_hdr, uint8_t tid,
	uint8_t status)
{
	qdf_nbuf_t netbuf;
	int nbuf_len;
   ...
   */
	...
	if (tx_desc->pkt_type == OL_TX_FRM_TSO) {
		...
		qdf_mem_copy(qdf_nbuf_data(netbuf),
			     qdf_nbuf_data(tx_desc->netbuf), nbuf_len);
	}
	...
	ucfg_pkt_capture_tx_completion_process(tx_desc->vdev_id, netbuf,
					       pkt_type, tid, status,
					       TXRX_PKTCAPTURE_PKT_FORMAT_8023,
					       bssid, pdev->htt_pdev,
					       payload_hdr->tx_retry_cnt);
}
#else
static void ol_tx_pkt_capture_tx_completion_process(
	ol_txrx_pdev_handle pdev, struct ol_tx_desc_t *tx_desc,
...
ol_tx_get_txtstamps(u_int32_t *msg_word_header, u_int32_t **msg_word_payload,
		    int num_msdus)
{
	...
	*msg_word_payload += offset_dwords;
	txtstamp_list =
		(struct htt_tx_compl_ind_append_tx_tstamp *)(*msg_word_payload);
	return txtstamp_list;
}

static inline struct htt_tx_compl_ind_append_tx_tsf64 *
ol_tx_get_txtstamp64s(u_int32_t *msg_word_header, u_int32_t **msg_word_payload,
		      int num_msdus)
{
	...
	*msg_word_payload = *msg_word_payload + offset_dwords;
	txtstamp64_list =
		(struct htt_tx_compl_ind_append_tx_tsf64 *)(*msg_word_payload);

	return txtstamp64_list;
}

static inline void ol_tx_timestamp(ol_txrx_pdev_handle pdev,
				   enum htt_tx_status status, qdf_nbuf_t netbuf,
				   u_int64_t ts)
{
	if (!netbuf)
		return;
				    ...
				    qdf_nbuf_t netbuf, uint8_t status)
{
	...
	if (!tx_desc->vdev || !tx_desc->vdev->osif_dev) {
		ol_tx_flow_pool_unlock(tx_desc);
		return;
	}
   ...
   */
	...
	if (pkt_type_bitmap) {
		if (status != htt_tx_status_download_fail)
			stats_rx(netbuf, osif_dev, PKT_TYPE_TX_HOST_FW_SENT,
				 &pkt_type);
		if (status == htt_tx_status_ok)
			stats_rx(netbuf, osif_dev, PKT_TYPE_TX_ACK_CNT,
				 &pkt_type);
	}
}
		  ...
		  enum htt_tx_status status, enum qdf_pkt_type pkt_type)
{
	...
	if (tx_desc->pkt_type != OL_TX_FRM_TSO) {
		...
		if (packetdump_cb) {
			...
			packetdump_cb((void *)soc, pdev->id, tx_desc->vdev_id,
 ...
 */
void ol_tx_completion_handler(ol_txrx_pdev_handle pdev, int num_msdus,
			      enum htt_tx_status status, void *msg)
{
	...
	if (status == htt_tx_status_ok || status == htt_tx_status_discard ||
	...
	if ((ucfg_pkt_capture_get_mode((void *)soc->psoc) ==
		...
		pkt_capture_txcomp_hdr_list =
			ucfg_pkt_capture_tx_get_txcomplete_data_hdr(msg_word,
								    num_msdus);
	...
	for (i = 0; i < num_msdus; i++) {
		...
		if (txtstamp64_list) {
			tx_tsf64 = (u_int64_t)txtstamp64_list[i].tx_tsf64_high
					   << 32 |
		...
		if (pkt_capture_txcomp_hdr_list) {
			ol_tx_pkt_capture_tx_completion_process(
				pdev, tx_desc, &pkt_capture_txcomp_hdr_list[i],
				tid, status);
		}
		...
		ol_tx_update_connectivity_stats(tx_desc, netbuf, status);
...
void ol_tx_desc_update_group_credit(ol_txrx_pdev_handle pdev,
				    u_int16_t tx_desc_id, int credit,
				    u_int8_t absolute,
				    enum htt_tx_status status)
{
	uint8_t i, is_member;
	...
	for (i = 0; i < OL_TX_MAX_TXQ_GROUPS; i++) {
		vdev_id_mask = OL_TXQ_GROUP_VDEV_ID_MASK_GET(
			pdev->txq_grps[i].membership);
		is_member = OL_TXQ_GROUP_VDEV_ID_BIT_MASK_GET(vdev_id_mask,
							      tx_desc->vdev_id);
		if (is_member) {
			ol_txrx_update_group_credit(&pdev->txq_grps[i], credit,
						    absolute);
			break;
		}
	}
...
void ol_tx_update_group_credit_stats(ol_txrx_pdev_handle pdev)
{
	...
	if (pdev->grp_stats.last_valid_index >
	    (OL_TX_GROUP_STATS_LOG_SIZE - 1)) {
...
void ol_tx_dump_group_credit_stats(ol_txrx_pdev_handle pdev)
{
	...
	for (i = 0; i < OL_TX_GROUP_STATS_LOG_SIZE; i++) {
		...
		for (j = 0; j < OL_TX_MAX_TXQ_GROUPS; j++) {
			qdf_spin_lock_bh(&pdev->grp_stat_spinlock);
			curr_credit =
				pdev->grp_stats.stats[curr_index].grp[j].credit;
			if (!is_break)
				old_credit = pdev->grp_stats.stats[old_index]
						     .grp[j]
						     .credit;

			mem_vdevs = pdev->grp_stats.stats[curr_index]
					    .grp[j]
					    .member_vdevs;
			qdf_spin_unlock_bh(&pdev->grp_stat_spinlock);
			...
			if (!is_break)
				txrx_nofl_info("%4d: %5d: %6d %6d %8x",
					       curr_index, j, curr_credit,
					       (curr_credit - old_credit),
					       mem_vdevs);
			else
				txrx_nofl_info("%4d: %5d: %6d %6s %8x",
					       curr_index, j, curr_credit, "NA",
					       mem_vdevs);
		}
 ...
 */
void ol_tx_single_completion_handler(ol_txrx_pdev_handle pdev,
				     enum htt_tx_status status,
				     uint16_t tx_desc_id)
{
	struct ol_tx_desc_t *tx_desc;
 ...
 */
void ol_tx_inspect_handler(ol_txrx_pdev_handle pdev, int num_msdus,
			   void *tx_desc_id_iterator)
{
	...
	for (i = 0; i < num_msdus; i++) {
		...
		vdev_id = HTT_TX_DESC_VDEV_ID_GET(
			*((uint32_t *)(tx_desc->htt_tx_desc)));
		TAILQ_FOREACH(vdev, &pdev->vdev_list, vdev_list_elem)
		{
       ...
       */
			ol_tx_msdu_complete(pdev, tx_desc, tx_descs, netbuf,
					    ...
					    htt_tx_status_ok, is_tx_desc_freed);
#ifdef QCA_SUPPORT_TXDESC_SANITY_CHECKS
			if (!is_tx_desc_freed) {
				tx_desc->pkt_type = ol_tx_frm_freed;
#ifdef QCA_COMPUTE_TX_DELAY
	...
	TX_CREDIT_DEBUG_PRINT(
		" <HTT> Increase HTT credit %d + %d = %d..\n",
		qdf_atomic_read(&pdev->target_tx_credit), num_msdus,
		qdf_atomic_read(&pdev->target_tx_credit) + num_msdus);
 ...
 * Return: None
 */
void ol_tx_set_compute_interval(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
				uint32_t interval)
{
 ...
 */
void ol_tx_packet_count(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
...
void ol_tx_delay(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
		 ...
		 int category)
{
	...
	if (data->avgs.transmit_num > 0) {
		avg_delay_ticks = ol_tx_delay_avg(data->avgs.transmit_sum_ticks,
	...
	if (data->avgs.queue_num > 0) {
		avg_delay_ticks = ol_tx_delay_avg(data->avgs.queue_sum_ticks,
						  data->avgs.queue_num);
...
void ol_tx_delay_hist(struct cdp_soc_t *soc_hdl, uint8_t pdev_id,
...
static uint8_t ol_tx_delay_tid_from_l3_hdr(struct ol_txrx_pdev_t *pdev,
					   qdf_nbuf_t msdu,
					   struct ol_tx_desc_t *tx_desc)
{
	uint16_t ethertype;
	...
	if (is_mgmt) {
		return (is_mcast) ? OL_TX_NUM_TIDS + OL_TX_VDEV_DEFAULT_MGMT :
				    HTT_TX_EXT_TID_MGMT;
	}
	if (is_mcast)
	...
	if (pdev->frame_format == wlan_frm_fmt_802_3) {
		...
		ethertype = (enet_hdr->ethertype[0] << 8) |
			    enet_hdr->ethertype[1];
		if (!IS_ETHERTYPE(ethertype)) {
			struct llc_snap_hdr_t *llc_hdr;

			llc_hdr =
				(struct llc_snap_hdr_t *)(qdf_nbuf_data(msdu) +
							  l2_hdr_size);
			l2_hdr_size += sizeof(struct llc_snap_hdr_t);
			ethertype = (llc_hdr->ethertype[0] << 8) |
				    llc_hdr->ethertype[1];
		}
	} else {
		struct llc_snap_hdr_t *llc_hdr;
...
static inline int ol_tx_delay_hist_bin(struct ol_txrx_pdev_t *pdev,
