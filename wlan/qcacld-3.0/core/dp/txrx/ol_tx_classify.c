...
static void ol_tx_classify_htt2_frm(struct ol_txrx_vdev_t *vdev,
				    qdf_nbuf_t tx_nbuf,
				    struct ol_txrx_msdu_info_t *tx_msdu_info)
{
   ...
   */
	if ((htt->info.l2_hdr_type == htt_pkt_type_ethernet) &&
	    ...
	    htt->info.is_unicast && (htt->info.ethertype == ETHERTYPE_IPV4)) {
...
ol_if_tx_bad_peer_txq_overflow(struct ol_txrx_pdev_t *pdev,
 ...
 */
static inline A_UINT8 ol_tx_tid_by_ipv4(A_UINT8 *pkt)
{
	A_UINT8 ipPri, tid;
...
static inline A_UINT8 ol_tx_tid_by_ipv6(A_UINT8 *pkt)
{
	return (ipv6_traffic_class((struct ipv6_hdr_t *)pkt) >> 5) & 0x7;
}

static inline void
ol_tx_set_ether_type(A_UINT8 *datap, struct ol_txrx_msdu_info_t *tx_msdu_info)
{
	...
	if (tx_msdu_info->htt.info.l2_hdr_type == htt_pkt_type_raw) {
		...
		if ((wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK) ==
			...
			llc = (struct llc_snap_hdr_t *)(datap +
...
static inline A_UINT8
ol_tx_tid_by_raw_type(A_UINT8 *datap, struct ol_txrx_msdu_info_t *tx_msdu_info)
{
...
static A_UINT8 ol_tx_tid(struct ol_txrx_pdev_t *pdev, qdf_nbuf_t tx_nbuf,
			 struct ol_txrx_msdu_info_t *tx_msdu_info)
{
	...
	if (pdev->frame_format == wlan_frm_fmt_raw) {
		...
		tx_msdu_info->htt.info.l3_hdr_offset =
			sizeof(struct ieee80211_frame);
		llc = (struct llc_snap_hdr_t *)(datap + tx_msdu_info->htt.info
								.l3_hdr_offset);
		tx_msdu_info->htt.info.ethertype = (llc->ethertype[0] << 8) |
						   llc->ethertype[1];
		/*
...
static inline struct ol_txrx_peer_t *
ol_tx_tdls_peer_find(struct ol_txrx_pdev_t *pdev, struct ol_txrx_vdev_t *vdev,
		     uint8_t *dest_addr, uint8_t *peer_id)
{
	...
	struct ol_txrx_peer_t *(
		*find_peer)(struct ol_txrx_pdev_t *pdev, uint8_t *peer_mac_addr,
			    int mac_addr_is_aligned, u8 check_valid,
			    enum peer_debug_id_type dbg_id) =
		ol_txrx_peer_find_hash_find_get_ref;

	if (vdev->hlTdlsFlag) {
		peer = find_peer(pdev, vdev->hl_tdls_ap_mac_addr.raw, 0, 1,
				 id_type);
   ...
   */
	if (vdev->last_real_peer &&
	    !qdf_mem_cmp(vdev->last_real_peer->mac_addr.raw, dest_addr,
			 QDF_MAC_ADDR_SIZE)) {
		...
		*peer_id = vdev->last_real_peer->local_id;
		...
		if (vdev->last_real_peer &&
		    !qdf_mem_cmp(vdev->hl_tdls_ap_mac_addr.raw, zero_mac_addr,
				 QDF_MAC_ADDR_SIZE)) {
       ...
       */
			...
			*peer_id = vdev->last_real_peer->local_id;
			peer = vdev->last_real_peer;
              ...
              * STA has TDLS link
              */
			peer = find_peer(pdev, vdev->hl_tdls_ap_mac_addr.raw, 0,
					 1, id_type);
...
ol_tx_classify(struct ol_txrx_vdev_t *vdev, struct ol_tx_desc_t *tx_desc,
	       qdf_nbuf_t tx_nbuf, struct ol_txrx_msdu_info_t *tx_msdu_info)
{
	...
	if ((IEEE80211_IS_MULTICAST(dest_addr)) ||
       ...
       */
			...
			if (!peer) {
				QDF_TRACE(
					...
					"Error: STA %pK (" QDF_MAC_ADDR_FMT
					") trying to send bcast DA tx data frame w/o association\n",
					vdev,
					QDF_MAC_ADDR_REF(vdev->mac_addr.raw));
				return NULL; /* error */
			} else if ((peer->security[OL_TXRX_PEER_SECURITY_MULTICAST]
					    .sec_type != htt_sec_type_wapi) &&
       ...
       */
			peer = ol_txrx_peer_find_hash_find_get_ref(
				pdev, vdev->mac_addr.raw, 0, 1,
				PEER_DEBUG_ID_OL_INTERNAL);
			if (!peer) {
				QDF_TRACE(
					QDF_MODULE_ID_TXRX,
					QDF_TRACE_LEVEL_ERROR,
					"Error: vdev %pK (" QDF_MAC_ADDR_FMT
     ...
     */
       ...
       */
			peer = ol_tx_tdls_peer_find(pdev, vdev, dest_addr,
						    &peer_id);
		} else {
			peer = ol_txrx_peer_find_hash_find_get_ref(
				pdev, dest_addr, 0, 1,
       ...
       */
		...
		} else if ((peer->security[OL_TXRX_PEER_SECURITY_UNICAST]
				    .sec_type != htt_sec_type_wapi) &&
			   (qdf_nbuf_is_ipv4_pkt(tx_nbuf) == true)) {
			if (QDF_NBUF_CB_PACKET_TYPE_DHCP ==
			    QDF_NBUF_CB_GET_PACKET_TYPE(tx_nbuf))
     ...
     */
		if (tx_msdu_info->htt.info.peer_id == HTT_INVALID_PEER_ID) {
			if (peer) {
				ol_txrx_info(
					"remove the peer for invalid peer_id %pK",
					peer);
   ...
   */
	...
	if (IEEE80211_IS_MULTICAST(dest_addr) &&
	    vdev->opmode != wlan_op_mode_sta && tx_msdu_info->peer != NULL) {
...
struct ol_tx_frms_queue_t *
ol_tx_classify_mgmt(struct ol_txrx_vdev_t *vdev, struct ol_tx_desc_t *tx_desc,
		    qdf_nbuf_t tx_nbuf,
		    struct ol_txrx_msdu_info_t *tx_msdu_info)
{
	struct ol_txrx_pdev_t *pdev = vdev->pdev;
   ...
   * indicate an error.
   */
	OL_TX_CLASSIFY_MGMT_EXTENSION(vdev, tx_desc, tx_nbuf, tx_msdu_info,
				      txq);
...
QDF_STATUS
ol_tx_classify_extension(struct ol_txrx_vdev_t *vdev,
			 struct ol_tx_desc_t *tx_desc, qdf_nbuf_t tx_msdu,
			 struct ol_txrx_msdu_info_t *msdu_info)
{
   ...
   */
	msdu_info->htt.action.do_encrypt =
		(!peer) ? 0 :
		(peer->security[which_key].sec_type == htt_sec_type_none) ?
			  0 :
			  msdu_info->htt.action.do_encrypt;
	/*
...
QDF_STATUS
ol_tx_classify_mgmt_extension(struct ol_txrx_vdev_t *vdev,
			      struct ol_tx_desc_t *tx_desc, qdf_nbuf_t tx_msdu,
			      struct ol_txrx_msdu_info_t *msdu_info)
{
