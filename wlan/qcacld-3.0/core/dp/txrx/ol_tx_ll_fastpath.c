...
ol_tx_prepare_ll_fast(struct ol_txrx_pdev_t *pdev, ol_txrx_vdev_handle vdev,
		      qdf_nbuf_t msdu, uint32_t *pkt_download_len,
		      uint32_t ep_id, struct ol_txrx_msdu_info_t *msdu_info)
{
   ...
   */
	...
	if (msdu_info->tso_info.is_tso) {
		...
		for (i = 1; i < num_frags; i++) {
			...
			qdf_debug(
				"htt_fdesc=%pK frag=%d frag_paddr=0x%0llx len=%zu",
				tx_desc->htt_frag_desc, i - 1, frag_paddr,
				frag_len);
   ...
   */
	...
	if (msdu_info->tso_info.curr_seg)
		*pkt_download_len = ol_tx_tso_adjust_pkt_dnld_len(
			msdu, msdu_info, *pkt_download_len);
         ...
         */
				...
				if (msdu_info.tso_info.curr_seg) {
					msdu_info.tso_info.curr_seg
						->sent_to_target = 1;
					next_seg = msdu_info.tso_info.curr_seg
							   ->next;
					/*
           ...
           */
				...
				if ((ce_send_fast(pdev->ce_tx_hdl, msdu, ep_id,
						  pkt_download_len) == 0)) {
           ...
           */
				...
				if (msdu_info.tso_info.is_tso) {
					TXRX_STATS_TSO_INC_SEG(
						vdev->pdev, tso_msdu_stats_idx);
					TXRX_STATS_TSO_INC_SEG_IDX(
						vdev->pdev, tso_msdu_stats_idx);
				}
			} else {
				/*
...
qdf_nbuf_t ol_tx_ll_fast(ol_txrx_vdev_handle vdev, qdf_nbuf_t msdu_list)
{
   ...
   */
	while (msdu) {
		...
		tx_desc = ol_tx_prepare_ll_fast(
			pdev, vdev, msdu, &pkt_download_len, ep_id, &msdu_info);

		TXRX_STATS_MSDU_INCR(pdev, tx.from_stack, msdu);

		if (qdf_likely(tx_desc)) {
			DPTRACE(qdf_dp_trace_ptr(
				msdu, QDF_DP_TRACE_TXRX_FAST_PACKET_PTR_RECORD,
