...
static ssize_t __hdd_wlan_dp_mem_stats_show(char *buf)
{
	...
	len += scnprintf(
		...
		qdf_mem_dp_tx_skb_cnt_read(), qdf_mem_dp_tx_skb_max_cnt_read(),
		qdf_mem_dp_rx_skb_cnt_read(), qdf_mem_dp_rx_skb_max_cnt_read(),
		qdf_mem_tx_desc_cnt_read(), qdf_mem_tx_desc_max_read());
	return len;
}
