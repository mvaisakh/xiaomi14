 ...
 * Return: None
 */
void lim_send_reassoc_req_with_ft_ies_mgmt_frame(
	struct mac_context *mac_ctx, tLimMlmReassocReq *mlm_reassoc_req,
	struct pe_session *pe_session)
{
	...
	if (add_ie_len && pe_session->is_ext_caps_present) {
		...
		pe_debug(
			"No addn IE or peer doesn't support addnIE for Assoc Req");
		extr_ext_flag = false;
	}
   ...
   */
	...
	if (pe_session->lim11hEnable && pe_session->spectrumMgtEnabled) {
	...
	if (!pe_session->is11Rconnection) {
		...
		if (!wps_ie) {
			populate_dot11f_rsn_opaque(
				mac_ctx, &(pe_session->pLimReAssocReq->rsnIE),
				&frm->RSNOpaque);
			populate_dot11f_wpa_opaque(
				mac_ctx, &(pe_session->pLimReAssocReq->rsnIE),
				&frm->WPAOpaque);
		}
#ifdef FEATURE_WLAN_ESE
		if (mlme_priv->connect_info.cckm_ie_len) {
			populate_dot11f_ese_cckm_opaque(
				mac_ctx, &mlme_priv->connect_info,
				&frm->ESECckmOpaque);
		}
#endif
	}
   ...
   */
	...
	status =
		dot11f_get_packed_re_assoc_request_size(mac_ctx, frm, &payload);
	...
	status = dot11f_pack_re_assoc_request(mac_ctx, frm,
	...
	pe_debug("*** Sending Re-Assoc Request length: %d %d to", bytes,
		 payload);
	...
	if (pe_session->is11Rconnection &&
		...
		for (i = 0; i < ft_ies_length; i++) {
			*body = mlme_priv->connect_info.ft_info.reassoc_ft_ie[i];
			body++;
		}
	...
	if ((pe_session->ftPEContext.pFTPreAuthReq) &&
	    (!wlan_reg_is_24ghz_ch_freq(pe_session->ftPEContext.pFTPreAuthReq
				     ...
				     struct pe_session *pe_session)
{
   ...
   */
	...
	if (!wpsIe) {
		...
		populate_dot11f_wpa_opaque(mac,
					   ...
					   &frm->WPAOpaque);
#if defined(FEATURE_WLAN_WAPI)
		populate_dot11f_wapi_opaque(
			mac, &(pe_session->pLimReAssocReq->rsnIE),
			&frm->WAPIOpaque);
	...
	if (fWmeEnabled) {
		populate_dot11f_wmm_info_station_per_session(
			mac, pe_session, &frm->WMMInfoStation);

		if (fWsmEnabled)
			populate_dot11f_wmm_caps(&frm->WMMCaps);
	}

	if (pe_session->htCapability && mac->lim.htCapabilityPresentInBeacon) {
	...
	if (lim_is_session_he_capable(pe_session)) {
		pe_debug("Populate HE IEs");
		populate_dot11f_he_caps(mac, pe_session, &frm->he_cap);
   ...
   */
	...
	if (frm->ExtCap.present && pe_session->is_ext_caps_present) {
		...
		if (pe_session->beacon &&
			...
			p_ext_cap = (uint8_t *)wlan_get_ie_ptr_from_eid(
				DOT11F_EID_EXTCAP, bcn_ie, bcn_ie_len);
			lim_update_extcap_struct(mac, p_ext_cap, &bcn_ext_cap);
			lim_merge_extcap_struct(&frm->ExtCap, &bcn_ext_cap,
						false);
		}
		/*
   ...
   */
	...
	nStatus = dot11f_get_packed_re_assoc_request_size(mac, frm, &nPayload);
	...
	nStatus = dot11f_pack_re_assoc_request(
		mac, frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload, &nPayload);
	if (DOT11F_FAILED(nStatus)) {
	...
	pe_debug("*** Sending Re-Association Request length: %d"
		 "to",
		 nBytes);
	...
	lim_diag_event_report(mac, WLAN_PE_DIAG_REASSOC_START_EVENT, pe_session,
			      QDF_STATUS_SUCCESS, QDF_STATUS_SUCCESS);
	...
	qdf_status = wma_tx_frame(mac, pPacket,
				 ...
				 struct scan_event *event, void *arg)
{
	...
	case SIR_SCAN_EVENT_START_FAILED:
		if ((mac_ctx->lim.req_id | PREAUTH_REQUESTOR_ID) ==
		    event->requester)
			lim_preauth_scan_event_handler(mac_ctx, event_type,
						       event->vdev_id,
						       event->scan_id);
		break;
