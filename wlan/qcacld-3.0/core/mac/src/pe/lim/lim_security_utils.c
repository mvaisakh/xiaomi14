 ...
 */
uint8_t lim_is_auth_algo_supported(struct mac_context *mac,
...
void lim_add_pre_auth_node(struct mac_context *mac,
			       ...
			       tpLimPreAuthNode pAuthNode)
{
	...
	MTRACE(mac_trace(mac, TRACE_CODE_TIMER_DEACTIVATE, NO_SESSION,
...
void lim_delete_pre_auth_node(struct mac_context *mac, tSirMacAddr macAddr)
{
	...
	if (!qdf_mem_cmp((uint8_t *)macAddr, (uint8_t *)&pTempNode->peerMacAddr,
			 sizeof(tSirMacAddr))) {
		...
		pe_debug("fRelease data for %d peer " QDF_MAC_ADDR_FMT,
			 pTempNode->authNodeIdx, QDF_MAC_ADDR_REF(macAddr));
	...
	while (pTempNode) {
		if (!qdf_mem_cmp((uint8_t *)macAddr,
			...
			pe_debug(
				"subsequent node to delete, Release data entry: %pK id %d "
...
void lim_restore_from_auth_state(struct mac_context *mac,
				 ...
				 struct pe_session *pe_session)
{
   ...
   */
	...
	if (tx_timer_running(
		...
		lim_deactivate_and_change_timer(mac, eLIM_AUTH_RETRY_TIMER);
	/* 'Change' timer for future activations */
	if (tx_timer_running(&mac->lim.lim_timers.gLimAuthFailureTimer))
		lim_deactivate_and_change_timer(mac, eLIM_AUTH_FAIL_TIMER);
...
void lim_encrypt_auth_frame(struct mac_context *mac, uint8_t keyId,
			    uint8_t *pKey, uint8_t *pPlainText,
			    uint8_t *pEncrBody, uint32_t keyLength)
{
   ...
   */
	...
	qdf_mem_copy(pPlainText + frame_len, icv, SIR_MAC_WEP_ICV_LENGTH);

	/* Run RC4 on plain text with the seed */
	lim_rc4(pEncrBody + SIR_MAC_WEP_IV_LENGTH, (uint8_t *)pPlainText, seed,
		keyLength, frame_len + SIR_MAC_WEP_ICV_LENGTH);
...
void lim_rc4(uint8_t *pDest, uint8_t *pSrc, uint8_t *seed, uint32_t keyLength,
	     uint16_t frameLen)
{
	typedef struct {
		uint8_t i, j;
...
uint8_t lim_decrypt_auth_frame(struct mac_context *mac, uint8_t *pKey,
			       uint8_t *pEncrBody, uint8_t *pPlainBody,
			       uint32_t keyLength, uint16_t frameLen)
{
	uint8_t seed[LIM_SEED_LENGTH], icv[SIR_MAC_WEP_ICV_LENGTH];
	int i;
