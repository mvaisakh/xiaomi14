 ...
 *
 */
static void lim_add_mgmt_seq_num(struct mac_context *mac,
 ...
 */
QDF_STATUS
lim_send_probe_req_mgmt_frame(struct mac_context *mac_ctx, tSirMacSSid *ssid,
			      tSirMacAddr bssid, qdf_freq_t chan_freq,
			      tSirMacAddr self_macaddr, uint32_t dot11mode,
			      ...
			      uint8_t *additional_ie)
{
   ...
   */
     ...
     */
		...
		populate_dot11f_supp_rates(mac_ctx, channel, &pr->SuppRates,
					   pesession);
   ...
   */
	...
	populate_dot11f_he_6ghz_cap(mac_ctx, pesession, &pr->he_6ghz_band_cap);
	...
	if (addn_ielen && additional_ie) {
		...
		if (QDF_STATUS_SUCCESS != sir_status) {
			...
			struct s_ext_cap *p_ext_cap =
				(struct s_ext_cap *)extracted_ext_cap.bytes;
		...
		qcn_ie = wlan_get_vendor_ie_ptr_from_oui(
   ...
   */
	...
	if (DOT11F_FAILED(status)) {
		...
		pe_warn("There were warnings while calculating the packed size for a Probe "
			"Request (0x%08x)",
			status);
	}
	...
	status = dot11f_pack_probe_request(
		mac_ctx, pr, frame + sizeof(tSirMacMgmtHdr), payload, &payload);
	if (DOT11F_FAILED(status)) {
		pe_err("Failed to pack a Probe Request (0x%08x)", status);
		cds_packet_free((void *)packet);
		return QDF_STATUS_E_FAILURE; /* allocated! */
	} else if (DOT11F_WARNED(status)) {
		pe_warn("There were warnings while packing a Probe Request (0x%08x)",
			status);
	}
	...
	if (lim_is_session_eht_capable(pesession)) {
		...
		qdf_status = lim_strip_eht_cap_ie(
			mac_ctx, frame + sizeof(tSirMacMgmtHdr), &ie_buf_size,
			eht_cap_ie);
		...
		qdf_mem_copy(frame + sizeof(tSirMacMgmtHdr) + ie_buf_size,
...
skip_eht_ie_update:
	...
	pe_nofl_debug("Probe req TX: vdev %d seq num %d to " QDF_MAC_ADDR_FMT
		      " len %d",
		      vdev_id, mac_ctx->mgmtSeqNum, QDF_MAC_ADDR_REF(bssid),
   ...
   */
	...
	qdf_status = wma_tx_frame(mac_ctx, packet,
				  ...
				  lim_tx_complete, frame, txflag, vdev_id, 0,
				  RATEID_DEFAULT, 0);
	if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
...
void lim_send_probe_rsp_mgmt_frame(struct mac_context *mac_ctx,
				   tSirMacAddr peer_macaddr, tpAniSSID ssid,
				   ...
				   uint8_t preq_p2pie)
{
   ...
   */
	if (LIM_IS_AP_ROLE(pe_session)) {
		frm->BeaconInterval.interval = mac_ctx->sch.beacon_interval;
	} else {
	...
	if (LIM_IS_AP_ROLE(pe_session)) {
		if (pe_session->wps_state != SAP_WPS_DISABLED)
			populate_dot11f_probe_res_wpsi_es(
				mac_ctx, &frm->WscProbeRes, pe_session);
	} else {
		...
		if (mac_ctx->lim.wscIeInfo.probeRespWscEnrollmentState ==
			...
			populate_dot11f_wsc_registrar_info_in_probe_res(
	...
	if (pe_session->vhtCapability) {
		...
		populate_dot11f_tx_power_env(
			mac_ctx, &frm->transmit_power_env[0],
			pe_session->ch_width, pe_session->curr_op_freq,
	...
	if (lim_is_session_he_capable(pe_session)) {
		...
		populate_dot11f_he_caps(mac_ctx, pe_session, &frm->he_cap);
		populate_dot11f_he_operation(mac_ctx, pe_session, &frm->he_op);
	...
	if (pe_session->pLimStartBssReq) {
		populate_dot11f_wpa(mac_ctx,
				    ...
				    &frm->WPA);
		populate_dot11f_rsn_opaque(
			mac_ctx, &(pe_session->pLimStartBssReq->rsnIE),
			&frm->RSNOpaque);
	}
   ...
   */
	addn_ie_present = (pe_session->add_ie_params.probeRespDataLen != 0);
	...
	if (addn_ie_present) {
		...
		if (QDF_STATUS_SUCCESS !=
		    lim_get_addn_ie_for_probe_resp(mac_ctx, add_ie,
						   &addn_ie_len, preq_p2pie)) {
			pe_err("Unable to get addn_ie");
			goto err_ret;
		}

		sir_status = lim_strip_extcap_update_struct(
			mac_ctx, add_ie, &addn_ie_len, &extracted_ext_cap);
		if (QDF_STATUS_SUCCESS != sir_status) {
		...
		if (p2p_ie) {
			...
			noalen = lim_get_noa_attr_stream(mac_ctx, noa_stream,
							 pe_session);
			if (noalen) {
				if ((p2p_ie[1] + noalen) >
   ...
   */
	if (extracted_ext_cap_flag)
		lim_merge_extcap_struct(&frm->ExtCap, &extracted_ext_cap, true);
	...
	if (DOT11F_FAILED(status)) {
		pe_err("Probe Response size error (0x%08x)", status);
		/* We'll fall back on the worst case scenario: */
		payload = sizeof(tDot11fProbeResponse);
	} else if (DOT11F_WARNED(status)) {
		pe_warn("Probe Response size warning (0x%08x)", status);
	}
	...
	status = dot11f_pack_probe_response(mac_ctx, frm,
					    frame + sizeof(tSirMacMgmtHdr),
					    payload, &payload);
	if (DOT11F_FAILED(status)) {
		pe_err("Probe Response pack failure (0x%08x)", status);
		goto err_ret;
	} else if (DOT11F_WARNED(status)) {
	...
	qdf_status = wma_tx_frame(mac_ctx, packet, (uint16_t)bytes,
				  TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7,
				  lim_tx_complete, frame, tx_flag, vdev_id, 0,
				  RATEID_DEFAULT, 0);
...
void lim_send_addts_req_action_frame(struct mac_context *mac,
				     ...
				     struct pe_session *pe_session)
{
	...
	if (!pAddTS->wmeTspecPresent) {
		...
		nStatus = dot11f_get_packed_add_ts_request_size(mac, &AddTSReq,
								&nPayload);
		if (DOT11F_FAILED(nStatus)) {
			...
			pe_warn("There were warnings while calculating the packed size for an "
				"Add TS Request (0x%08x)",
				nStatus);
		}
	} else {
		...
		nStatus = dot11f_get_packed_wmm_add_ts_request_size(
			mac, &WMMAddTSReq, &nPayload);
		if (DOT11F_FAILED(nStatus)) {
			pe_err("Failed to calculate the packed size for a WMM Add TS Request "
			       "(0x%08x)",
			       nStatus);
			/* We'll fall back on the worst case scenario: */
			nPayload = sizeof(tDot11fAddTSRequest);
		} else if (DOT11F_WARNED(nStatus)) {
			pe_warn("There were warnings while calculating the packed size for a WMM "
				"Add TS Request (0x%08x)",
				nStatus);
		}
	}
	...
	if (!pAddTS->wmeTspecPresent) {
		nStatus = dot11f_pack_add_ts_request(
			mac, &AddTSReq, pFrame + sizeof(tSirMacMgmtHdr),
			nPayload, &nPayload);
		if (DOT11F_FAILED(nStatus)) {
		...
		nStatus = dot11f_pack_wmm_add_ts_request(
			mac, &WMMAddTSReq, pFrame + sizeof(tSirMacMgmtHdr),
			nPayload, &nPayload);
		if (DOT11F_FAILED(nStatus)) {
	...
	lim_diag_mgmt_tx_event_report(mac, pMacHdr, pe_session,
				      QDF_STATUS_SUCCESS, QDF_STATUS_SUCCESS);

	/* Queue Addts Response frame in high priority WQ */
	qdf_status = wma_tx_frame(mac, pPacket, (uint16_t)nBytes,
				  TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7,
				  lim_tx_complete, pFrame, txFlag, smeSessionId,
				  0, RATEID_DEFAULT, 0);
	MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
			 pe_session->peSessionId, qdf_status));

	if (!QDF_IS_STATUS_SUCCESS(qdf_status))
		pe_err("Could not send an Add TS Request (%X", qdf_status);
				    ...
				    struct pe_session *pe_session)
{
	...
	if (DOT11F_FAILED(bytes)) {
		...
		pe_warn("There were warnings while calculating the packed size for MSCS "
			"Request (0x%08x)",
			bytes);
	}
	...
	bytes = dot11f_pack_mscs_request_action_frame(
		mac, &mscs_req_frame, frame + sizeof(tSirMacMgmtHdr), payload,
		&payload);
	if (DOT11F_FAILED(bytes)) {
		pe_err("Failed to pack an mscs Request "
		       "(0x%08x)",
		       bytes);
		cds_packet_free((void *)packet);
		return; /* allocated! */
	...
	pe_debug("mscs req TX: vdev id: %d to " QDF_MAC_ADDR_FMT
		 " seq num[%d], frame subtype:%d ",
	...
	qdf_status = wma_tx_frameWithTxComplete(
		mac, packet, (uint16_t)(sizeof(tSirMacMgmtHdr) + payload),
		TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7, lim_tx_complete, frame,
		lim_mscs_req_tx_complete_cnf, HAL_USE_PEER_STA_REQUESTED_MASK,
 ...
 * Return: This returns QDF_STATUS
 */
static QDF_STATUS lim_assoc_rsp_tx_complete(void *context, qdf_nbuf_t buf,
					    uint32_t tx_complete, void *params)
{
	...
	session_entry =
		pe_find_session_by_bssid(mac_ctx, mac_hdr->sa, &session_id);
	if (!session_entry) {
		pe_err("session entry is NULL");
		goto end;
	}

	sta_ds = dph_lookup_hash_entry(mac_ctx, (uint8_t *)mac_hdr->da, &aid,
	...
	if (lim_is_mlo_conn(session_entry, sta_ds) &&
	    QDF_IS_STATUS_ERROR(lim_mlo_assoc_ind_upper_layer(
		    mac_ctx, session_entry, &assoc_req->mlo_info))) {
	...
	if (!lim_fill_lim_assoc_ind_params(lim_assoc_ind, mac_ctx, sta_ds,
					   session_entry)) {
	...
	lim_fill_sme_assoc_ind_params(mac_ctx, lim_assoc_ind, sme_assoc_ind,
				      session_entry, true);
...
QDF_STATUS lim_strip_eht_cap_ie(struct mac_context *mac_ctx, uint8_t *frame_ies,
				uint16_t *ie_buf_size, uint8_t *eht_cap_ie)
{
	return lim_strip_ie(mac_ctx, frame_ies, ie_buf_size,
			    WLAN_ELEMID_EXTN_ELEM, ONE_BYTE, EHT_CAP_OUI_TYPE,
			    EHT_CAP_OUI_SIZE, eht_cap_ie, WLAN_MAX_IE_LEN);
}

QDF_STATUS lim_strip_eht_op_ie(struct mac_context *mac_ctx, uint8_t *frame_ies,
			       uint16_t *ie_buf_size, uint8_t *eht_op_ie)
{
	return lim_strip_ie(mac_ctx, frame_ies, ie_buf_size,
			    WLAN_ELEMID_EXTN_ELEM, ONE_BYTE, EHT_OP_OUI_TYPE,
			    EHT_OP_OUI_SIZE, eht_op_ie, WLAN_MAX_IE_LEN);
}
#endif

void lim_send_assoc_rsp_mgmt_frame(struct mac_context *mac_ctx,
				   uint16_t status_code, uint16_t aid,
				   tSirMacAddr peer_addr, uint8_t subtype,
				   tpDphHashNode sta,
				   struct pe_session *pe_session,
				   bool tx_complete)
{
	...
	if (sta && lim_is_mlo_conn(pe_session, sta) &&
		...
		pe_err("Do not send assoc rsp in mlo partner peer " QDF_MAC_ADDR_FMT,
   ...
   */
	...
	if (!sta) {
		...
		populate_dot11f_ext_supp_rates(
			mac_ctx, POPULATE_DOT11F_RATES_OPERATIONAL,
			&frm.ExtSuppRates, pe_session);
	} else {
	...
	if (LIM_IS_AP_ROLE(pe_session) && sta &&
		...
		assoc_req =
			(tpSirAssocReq)pe_session->parsedAssocReq[sta->assocId];
		/*
     ...
     */
		if (assoc_req && assoc_req->addIEPresent)
			populate_dot11_assoc_res_p2p_ie(
				mac_ctx, &frm.P2PAssocRes, assoc_req);
	}
	...
	if (sta) {
		if (eHAL_SET == qos_mode) {
			if (sta->lleEnabled) {
				lle_mode = 1;
				populate_dot11f_edca_param_set(
					mac_ctx, &frm.EDCAParamSet, pe_session);
			}
		}
       ...
       */
		...
		if (sta->mlmStaContext.vhtCapability &&
			...
			populate_dot11f_vht_operation(mac_ctx, pe_session,
						      &frm.VHTOperation);
			is_vht = true;
		} else if (sta->mlmStaContext.force_1x1 && frm.HTCaps.present) {
			/*
       ...
       */
		...
		if (pe_session->vhtCapability && pe_session->vendor_vht_sap &&
		    (assoc_req) && assoc_req->vendor_vht_ie.VHTCaps.present) {
		...
		if (status_code == STATUS_ASSOC_REJECTED_TEMPORARILY) {
			...
			retry_int = mac_ctx->mlme_cfg->gen
					    .pmf_sa_query_retry_interval;
   ...
   */
	...
	if (sta && lim_is_sta_eht_capable(sta) &&
		...
		mlo_ie_len = lim_send_assoc_rsp_mgmt_frame_mlo(
			mac_ctx, pe_session, sta, &frm);
	}
	...
	lim_populate_mac_header(mac_ctx, frame, SIR_MAC_MGMT_FRAME,
				...
				peer_addr, pe_session->self_mac_addr);
	mac_hdr = (tpSirMacMgmtHdr)frame;
	...
	status = dot11f_pack_assoc_response(mac_ctx, &frm,
					    ...
					    payload, &payload);
	if (DOT11F_FAILED(status)) {
		pe_err("Association Response pack failure(0x%08x)", status);
		cds_packet_free((void *)packet);
		goto error;
	} else if (DOT11F_WARNED(status)) {
		pe_warn("Association Response pack warning (0x%08x)", status);
	}
	...
	if (lim_is_session_eht_capable(pe_session)) {
		...
		bytes = bytes - payload;
		payload =
			ie_buf_size + WLAN_ASSOC_RSP_IES_OFFSET + eht_op_ie_len;
		bytes = bytes + payload;
	...
	pe_nofl_debug(
		"Assoc rsp TX: vdev %d subtype %d to " QDF_MAC_ADDR_FMT
		" seq num %d status %d aid %d addn_ie_len %d ht %d vht %d "
		"vendor vht %d he %d eht %d",
		pe_session->vdev_id, subtype, QDF_MAC_ADDR_REF(mac_hdr->da),
	...
	lim_diag_mgmt_tx_event_report(mac_ctx, mac_hdr, pe_session,
				      QDF_STATUS_SUCCESS, status_code);
	/* Queue Association Response frame in high priority WQ */
	if (tx_complete)
		qdf_status = wma_tx_frameWithTxComplete(
			mac_ctx, packet, (uint16_t)bytes, TXRX_FRM_802_11_MGMT,
			ANI_TXDIR_TODS, 7, lim_tx_complete, frame,
			lim_assoc_rsp_tx_complete, tx_flag, sme_session, false,
			0, RATEID_DEFAULT, 0, 0);
	else
		qdf_status = wma_tx_frame(mac_ctx, packet, (uint16_t)bytes,
	...
	/* Pkt will be freed up by the callback */
	if (!QDF_IS_STATUS_SUCCESS(qdf_status))
		pe_err("Could not Send Re/AssocRsp, retCode=%X", qdf_status);
				     ...
				     struct pe_session *pe_session)
{
	...
	if (!wmmTspecPresent) {
		...
		if (DOT11F_FAILED(nStatus)) {
			...
			pe_warn("There were warnings while calculating the packed size for a Del "
				"TS (0x%08x)",
				nStatus);
		}
	} else {
		...
		nStatus = dot11f_get_packed_wmm_del_ts_size(mac, &WMMDelTS,
							    &nPayload);
		if (DOT11F_FAILED(nStatus)) {
	...
	if (!wmmTspecPresent) {
		...
		nStatus = dot11f_pack_wmm_del_ts(
	...
	if (pe_session && pe_session->opmode == QDF_STA_MODE) {
		...
		if (tag == WLAN_AUTH_REQ) {
			...
			wlan_connectivity_mgmt_event(
				mac->psoc, mac_hdr, params->vdev_id, status,
				qdf_tx_complete, mac->lim.bss_rssi, algo, type,
				seq, 0, WLAN_AUTH_REQ);
			return;
		}
		...
		wlan_connectivity_mgmt_event(mac->psoc, mac_hdr,
					     params->vdev_id, reason_code,
...
static QDF_STATUS lim_assoc_tx_complete_cnf(void *context, qdf_nbuf_t buf,
					    uint32_t tx_complete, void *params)
{
	...
	lim_diag_event_report(mac_ctx, WLAN_PE_DIAG_ASSOC_ACK_EVENT, NULL,
			      assoc_ack_status, reason_code);
	return QDF_STATUS_SUCCESS;
}
...
#else
static inline QDF_STATUS lim_fill_adaptive_11r_ie(struct pe_session *pe_session,
						  uint8_t **ie_buf,
						  uint8_t *ie_len)
{
	return QDF_STATUS_SUCCESS;
 ...
 * Return: Void
 */
void lim_send_assoc_req_mgmt_frame(struct mac_context *mac_ctx,
				   ...
				   struct pe_session *pe_session)
{
	...
	if (add_ie_len && pe_session->is_ext_caps_present) {
		qdf_mem_zero((uint8_t *)&extr_ext_cap, sizeof(tDot11fIEExtCap));
		sir_status = lim_strip_extcap_update_struct(
		...
		if (QDF_STATUS_SUCCESS != sir_status) {
			...
			struct s_ext_cap *p_ext_cap =
				(struct s_ext_cap *)extr_ext_cap.bytes;
   ...
   */
	...
	if (pe_session->lim11hEnable &&
		...
		populate_dot11f_power_caps(mac_ctx, &frm->PowerCaps, LIM_ASSOC,
					   pe_session);
   ...
   */
	...
	if (wme_enabled) {
		populate_dot11f_wmm_info_station_per_session(
			mac_ctx, pe_session, &frm->WMMInfoStation);
   ...
   */
	...
	if (pe_session->vhtCapability &&
		...
		if (pe_session->gLimOperatingMode.present &&
			...
			populate_dot11f_operating_mode(
   ...
   */
	...
	if (frm->ExtCap.present && pe_session->is_ext_caps_present) {
		...
		if (pe_session->beacon &&
			...
			p_ext_cap = (uint8_t *)wlan_get_ie_ptr_from_eid(
				DOT11F_EID_EXTCAP, bcn_ie, bcn_ie_len);
	...
	if (QDF_STATUS_SUCCESS !=
	    lim_strip_supp_op_class_update_struct(mac_ctx, add_ie, &add_ie_len,
						  &frm->SuppOperatingClasses))
		pe_debug(
			"Unable to Stripoff supp op classes IE from Assoc Req");
	...
	if (frag_ie_present && add_ie_len &&
		...
		qdf_status = lim_strip_ie(
			mac_ctx, add_ie, &add_ie_len, WLAN_ELEMID_FRAGMENT,
			ONE_BYTE, NULL, 0, &fils_hlp_ie[fils_hlp_ie_len],
	...
	if (add_ie_len &&
		...
		qdf_status = lim_strip_ie(mac_ctx, add_ie, &add_ie_len,
					  WLAN_ELEMID_RSNXE, ONE_BYTE, NULL, 0,
					  rsnx_ie, WLAN_MAX_IE_LEN);
		if (QDF_IS_STATUS_ERROR(qdf_status)) {
   ...
   */
	if (add_ie_len && wlan_get_vendor_ie_ptr_from_oui(SIR_MAC_MBO_OUI,
		...
		qdf_status = lim_strip_ie(mac_ctx, add_ie, &add_ie_len,
					  ...
					  SIR_MAC_MBO_OUI, SIR_MAC_MBO_OUI_SIZE,
		...
		if (!is_open_auth) {
			...
			if (!bss_mfp_capable) {
				pe_debug(
					"Peer doesn't support PMF, Don't add MBO IE");
				qdf_mem_free(mbo_ie);
				mbo_ie = NULL;
				mbo_ie_len = 0;
			}
   ...
   */
	if (add_ie_len) {
		ret = dot11f_unpack_assoc_request(mac_ctx, add_ie, add_ie_len,
						  frm, true);
	...
	if (rsnx_ie && rsnx_ie_len) {
		qdf_mem_copy(frame + sizeof(tSirMacMgmtHdr) + payload, rsnx_ie,
			     rsnx_ie_len);
	...
	qdf_mem_copy(frame + sizeof(tSirMacMgmtHdr) + payload, vendor_ies,
		     vendor_ie_len);
	payload = payload + vendor_ie_len;

	/* Copy the MBO IE to the end of the frame */
	qdf_mem_copy(frame + sizeof(tSirMacMgmtHdr) + payload, mbo_ie,
		     mbo_ie_len);
   ...
   * assoc request frame
   */
	qdf_mem_copy(frame + sizeof(tSirMacMgmtHdr) + payload, adaptive_11r_ie,
		     adaptive_11r_ie_len);
	...
	if (lim_is_fils_connection(pe_session)) {
		qdf_status = aead_encrypt_assoc_req(mac_ctx, pe_session, frame,
						    &payload);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			cds_packet_free((void *)packet);
			goto end;
	...
	pe_debug(
		"extr_ext_flag %d mbo ie len %d is open auth %d stripped vendor len %d "
		"he with tkip %d ht %d vht %d opmode %d vendor vht %d he %d eht %d",
		extr_ext_flag, mbo_ie_len, is_open_auth, current_len,
		pe_session->he_with_wep_tkip, frm->HTCaps.present,
		frm->VHTCaps.present, frm->OperatingMode.present,
		frm->vendor_vht_ie.present, frm->he_cap.present,
		frm->eht_cap.present);
	...
	qdf_status = wma_tx_frameWithTxComplete(
		mac_ctx, packet, (uint16_t)(sizeof(tSirMacMgmtHdr) + payload),
		TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7, lim_tx_complete, frame,
		lim_assoc_tx_complete_cnf, tx_flag, vdev_id, false, 0, min_rid,
		peer_rssi, 0);
	MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
			 pe_session->peSessionId, qdf_status));

	if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
		pe_err("Failed to send Association Request (%X)!", qdf_status);
		mac_ctx->assoc_ack_status = LIM_TX_FAILED;
		lim_diag_event_report(mac_ctx, WLAN_PE_DIAG_ASSOC_ACK_EVENT,
				      pe_session, SENT_FAIL,
 ...
 */
static uint8_t *lim_get_addba_rsp_ptr(uint8_t *ie, uint32_t ie_len)
{
 ...
 * Return: This returns QDF_STATUS
 */
static QDF_STATUS lim_addba_rsp_tx_complete_cnf(void *context, qdf_nbuf_t buf,
						uint32_t tx_complete,
						void *params)
{
	struct mac_context *mac_ctx = (struct mac_context *)context;
	...
	addba_rsp_ptr = lim_get_addba_rsp_ptr(data + sizeof(*mac_hdr),
	...
	mac_hdr = (tSirMacMgmtHdr *)data;
	rsp_len = sizeof(rsp);
	status = dot11f_unpack_addba_rsp(mac_ctx, addba_rsp_ptr, rsp_len, &rsp,
					 false);

	if (DOT11F_FAILED(status)) {
		pe_err("Failed to unpack and parse (0x%08x, %d bytes)", status,
		       rsp_len);
		goto error;
	}
 ...
 * Return: This returns QDF_STATUS
 */
static QDF_STATUS lim_auth_tx_complete_cnf(void *context, qdf_nbuf_t buf,
					   uint32_t tx_complete, void *params)
{
	struct mac_context *mac_ctx = (struct mac_context *)context;
	...
	lim_diag_event_report(mac_ctx, WLAN_PE_DIAG_AUTH_ACK_EVENT, NULL,
			      auth_ack_status, reason_code);
					      ...
					      tSirMacAddr peer_addr)
{
	...
	if (wlan_vdev_mlme_is_mlo_vdev(session->vdev)) {
		...
		if (LIM_IS_STA_ROLE(session)) {
			...
			auth_node =
				lim_search_pre_auth_list(mac_ctx, peer_addr);
			if (!auth_node) {
...
lim_calculate_auth_mlo_ie_len(struct mac_context *mac_ctx,
			      struct pe_session *session, tSirMacAddr peer_addr)
{
	return 0;
}
#endif
 ...
 * Return: void
 */
void lim_send_auth_mgmt_frame(struct mac_context *mac_ctx,
			      tpSirMacAuthFrameBody auth_frame,
			      tSirMacAddr peer_addr, uint8_t wep_challenge_len,
			      struct pe_session *session)
{
	uint8_t *frame, *body;
     ...
     */
		...
		status = lim_create_fils_auth_data(mac_ctx, auth_frame, session,
						   &frame_len);
		...
		if (session->lim_join_req && session->is11Rconnection &&
...
alloc_packet:
	...
	if (LIM_IS_AP_ROLE(session))
		qdf_mem_copy((uint8_t *)mac_hdr->bssId,
			     (uint8_t *)session->bssId, sizeof(tSirMacAddr));
	...
	if (wep_challenge_len) {
		...
		*((uint16_t *)(body)) = sir_swap_u16if_needed(
		...
		if ((auth_frame->authAlgoNumber == eSIR_FT_AUTH) &&
			...
			if (ft_ies_length > 0) {
				...
				QDF_TRACE_HEX_DUMP(QDF_MODULE_ID_PE,
						   ...
						   (uint8_t *)body,
						   ft_ies_length);
			} else if (session->ftPEContext.pFTPreAuthReq
					   ->pbssDescription) {
				/* MDID attr is 54 */
	...
	pe_nofl_info("Auth TX: vdev %d seq %d seq num %d status %d WEP %d "
		     "to " QDF_MAC_ADDR_FMT,
	...
	peer_rssi = mac_ctx->lim.bss_rssi;
	lim_diag_mgmt_tx_event_report(mac_ctx, mac_hdr, session,
				      QDF_STATUS_SUCCESS, QDF_STATUS_SUCCESS);

	if (session->ftPEContext.pFTPreAuthReq)
		ch_freq_tx_frame = session->ftPEContext.pFTPreAuthReq
					   ->pre_auth_channel_freq;

	qdf_status = wma_tx_frameWithTxComplete(
		mac_ctx, packet, (uint16_t)frame_len, TXRX_FRM_802_11_MGMT,
		ANI_TXDIR_TODS, 7, lim_tx_complete, frame,
		lim_auth_tx_complete_cnf, tx_flag, vdev_id, false,
...
static void lim_delete_deauth_all_pending_sta(struct mac_context *mac_ctx,
					      struct pe_session *session)
{
	int i = 0;
...
QDF_STATUS lim_send_deauth_cnf(struct mac_context *mac_ctx, uint8_t vdev_id)
{
	...
	if (deauth_req) {
		...
		sta_ds = dph_lookup_hash_entry(
			mac_ctx, deauth_req->peer_macaddr.bytes, &aid,
			&session_entry->dph.dphHashTable);
		if (!sta_ds) {
			deauth_cnf.resultCode = eSIR_SME_INVALID_PARAMETERS;
			goto end;
		...
		if ((session_entry->limSystemRole == eLIM_STA_ROLE) &&
			...
			pe_debug("FT Preauth (%pK,%d) Deauth rc %d src = %d",
				 session_entry, session_entry->peSessionId,
		...
		if (!session_entry ||
		    (session_entry->opmode != QDF_SAP_MODE &&
	...
	qdf_copy_macaddr(&deauth_cnf.peer_macaddr, &deauth_req->peer_macaddr);
	...
	lim_post_sme_message(mac_ctx, LIM_MLM_DEAUTH_CNF,
			     (uint32_t *)&deauth_cnf);
	return QDF_STATUS_SUCCESS;
}
	...
	if (disassoc_req) {
		if (tx_timer_running(
			...
			lim_deactivate_and_change_timer(
				mac_ctx, eLIM_DISASSOC_ACK_TIMER);
...
QDF_STATUS lim_disassoc_tx_complete_cnf(void *context, uint32_t tx_success,
					void *params)
{
...
QDF_STATUS lim_deauth_tx_complete_cnf(void *context, uint32_t tx_success,
				      void *params)
{
	struct mac_context *mac_ctx = (struct mac_context *)context;
	struct wmi_mgmt_params *mgmt_params = (struct wmi_mgmt_params *)params;
						     ...
						     void *params)
{
	...
	pe_debug("tx_complete = %s tx_success = %d",
		 (tx_success == WMI_MGMT_TX_COMP_TYPE_COMPLETE_OK) ? "success" :
								     "fail",
		 tx_success);
	...
	if (!deauth_req && mac_hdr) {
		...
		status_code = lim_ap_delete_sta_upon_deauth_tx(
			mac_ctx, session, (uint8_t *)mac_hdr->da);
	}
	...
	if (tx_success != WMI_MGMT_TX_COMP_TYPE_COMPLETE_OK && session &&
		...
		lim_send_deauth_mgmt_frame(
			mac_ctx, session->deauth_retry.reason_code,
			session->deauth_retry.peer_macaddr.bytes, session,
			true);
		session->deauth_retry.retry_cnt--;
		return QDF_STATUS_SUCCESS;
	}
 ...
 * Return: None
 */
static void lim_append_ies_to_frame(uint8_t *frame, uint32_t *frame_len,
				    struct element_info *ie)
{
	if (!ie || !ie->len || !ie->ptr)
		return;
...
void lim_send_disassoc_mgmt_frame(struct mac_context *mac, uint16_t nReason,
				  tSirMacAddr peer,
				  struct pe_session *pe_session,
				  bool waitForAck)
{
   ...
   */
	...
	if (DOT11F_FAILED(nStatus)) {
		...
		pe_warn("There were warnings while calculating the packed size for a "
			"Disassociation (0x%08x)",
			nStatus);
	}
	...
	nStatus = dot11f_pack_disassociation(mac, &frm,
					     pFrame + sizeof(tSirMacMgmtHdr),
					     nPayload, &nPayload);
	if (DOT11F_FAILED(nStatus)) {
		pe_err("Failed to pack a Disassociation (0x%08x)", nStatus);
		cds_packet_free((void *)pPacket);
		if (waitForAck)
			lim_send_disassoc_cnf(mac);
		return;
	...
	pe_nofl_info("Disassoc TX: vdev %d seq %d reason %u and waitForAck %d "
		     "to " QDF_MAC_ADDR_FMT " From " QDF_MAC_ADDR_FMT,
	...
	if (waitForAck) {
		...
		qdf_status = wma_tx_frameWithTxComplete(
			mac, pPacket, (uint16_t)nBytes, TXRX_FRM_802_11_MGMT,
			ANI_TXDIR_TODS, 7, lim_tx_complete, pFrame,
			lim_disassoc_tx_complete_cnf_handler, txFlag,
			smeSessionId, false, 0, RATEID_DEFAULT, 0, 0);
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
				 pe_session->peSessionId, qdf_status));
		...
		lim_diag_mgmt_tx_event_report(mac, pMacHdr, pe_session,
					      ...
					      QDF_STATUS_SUCCESS);
		wlan_connectivity_mgmt_event(
			mac->psoc, (struct wlan_frame_hdr *)pMacHdr,
			pe_session->vdev_id, nReason, QDF_TX_RX_STATUS_OK,
			mac->lim.bss_rssi, 0, 0, 0, 0, WLAN_DISASSOC_TX);
...
void lim_send_deauth_mgmt_frame(struct mac_context *mac, uint16_t nReason,
				...
				bool waitForAck)
{
   ...
   */
	...
	nStatus = dot11f_pack_de_auth(mac, &frm,
				      pFrame + sizeof(tSirMacMgmtHdr), nPayload,
				      &nPayload);
	if (DOT11F_FAILED(nStatus)) {
		pe_err("Failed to pack a DeAuthentication (0x%08x)", nStatus);
		cds_packet_free((void *)pPacket);
		if (waitForAck)
			lim_send_deauth_cnf(mac, pe_session->vdev_id);
		return;
	...
	pe_nofl_rl_info("Deauth TX: vdev %d seq_num %d reason %u waitForAck %d "
			"to " QDF_MAC_ADDR_FMT " from " QDF_MAC_ADDR_FMT,
	...
	if (waitForAck) {
		...
		if (pe_session->opmode == QDF_STA_MODE &&
			...
			qdf_mem_copy(
				pe_session->deauth_retry.peer_macaddr.bytes,
		...
		qdf_status = wma_tx_frameWithTxComplete(
			mac, pPacket, (uint16_t)nBytes, TXRX_FRM_802_11_MGMT,
			ANI_TXDIR_TODS, 7, lim_tx_complete, pFrame,
			lim_deauth_tx_complete_cnf_handler, txFlag,
			smeSessionId, false, 0, RATEID_DEFAULT, 0, 0);
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
				 pe_session->peSessionId, qdf_status));
       ...
       */
		...
		if (tx_timer_change_context(
			...
			return;
				   ...
				   &mac->lim.lim_timers.gLimDeauthAckTimer, val,
				   0) != TX_SUCCESS) {
		...
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_MGMT,
				 pe_session->peSessionId, pMacHdr->fc.subType));
#ifdef FEATURE_WLAN_TDLS
		if ((sta) && (STA_ENTRY_TDLS_PEER == sta->staType)) {
			/* Queue Disassociation frame in high priority WQ */
			lim_diag_mgmt_tx_event_report(mac, pMacHdr, pe_session,
						      QDF_STATUS_SUCCESS,
						      QDF_STATUS_SUCCESS);
			qdf_status = wma_tx_frame(
				mac, pPacket, (uint16_t)nBytes,
				TXRX_FRM_802_11_MGMT, ANI_TXDIR_IBSS, 7,
				lim_tx_complete, pFrame, txFlag, smeSessionId,
				0, RATEID_DEFAULT, 0);
		} else {
#endif
			...
			wlan_connectivity_mgmt_event(
				...
				QDF_TX_RX_STATUS_OK, mac->lim.bss_rssi, 0, 0, 0,
				0, WLAN_DEAUTH_TX);
			   ...
			   tSirMacAddr peer, struct pe_session *pe_session)
{
	...
	case SIR_MAC_BASIC_MEASUREMENT_TYPE:
		nSirStatus = populate_dot11f_measurement_report0(
			mac, pMeasReqFrame, &frm.MeasurementReport);
		break;
	case SIR_MAC_CCA_MEASUREMENT_TYPE:
		nSirStatus = populate_dot11f_measurement_report1(
			mac, pMeasReqFrame, &frm.MeasurementReport);
		break;
	case SIR_MAC_RPI_MEASUREMENT_TYPE:
		nSirStatus = populate_dot11f_measurement_report2(
			mac, pMeasReqFrame, &frm.MeasurementReport);
		break;
	default:
	...
	nStatus =
		dot11f_get_packed_measurement_report_size(mac, &frm, &nPayload);
	if (DOT11F_FAILED(nStatus)) {
		...
		pe_warn("There were warnings while calculating the packed size for a "
			"Measurement Report (0x%08x)",
			nStatus);
	}

	nBytes = nPayload + sizeof(tSirMacMgmtHdr);

	qdf_status = cds_packet_alloc(mac->hdd_handle, TXRX_FRM_802_11_MGMT,
	...
	nStatus = dot11f_pack_measurement_report(
		mac, &frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload,
		&nPayload);
	if (DOT11F_FAILED(nStatus)) {
		pe_err("Failed to pack a Measurement Report (0x%08x)", nStatus);
	...
	MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
			 ...
			 qdf_status));
	if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
		pe_err("Failed to send a Measurement Report (%X)!", qdf_status);
		/* Pkt will be freed up by the callback */
			  ...
			  tSirMacAddr peer, struct pe_session *pe_session)
{
	...
	if (DOT11F_FAILED(nStatus)) {
		pe_err("Failed to calculate the packed size for a TPC Report (0x%08x)",
		       nStatus);
		/* We'll fall back on the worst case scenario: */
		nPayload = sizeof(tDot11fTPCReport);
	} else if (DOT11F_WARNED(nStatus)) {
		pe_warn("There were warnings while calculating the packed size for a TPC "
			"Report (0x%08x)",
			nStatus);
	}
	...
	nStatus = dot11f_pack_tpc_report(mac, &frm,
					 pFrame + sizeof(tSirMacMgmtHdr),
					 nPayload, &nPayload);
	if (DOT11F_FAILED(nStatus)) {
		pe_err("Failed to pack a TPC Report (0x%08x)", nStatus);
		cds_packet_free(mac->hdd_handle, TXRX_FRM_802_11_MGMT,
				(void *)pFrame, (void *)pPacket);
		return QDF_STATUS_E_FAILURE; /* allocated! */
	} else if (DOT11F_WARNED(nStatus)) {
		...
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
				 ...
				 qdf_status));
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			pe_err("Failed to send a TPC Report (%X)!", qdf_status);
			/* Pkt will be freed up by the callback */
			return QDF_STATUS_E_FAILURE; /* just allocated... */
		}
   ...
   */
					   ...
					   struct pe_session *pe_session)
	{
		...
		nStatus = dot11f_get_packed_channel_switch_size(mac, &frm,
								&nPayload);
		if (DOT11F_FAILED(nStatus)) {
			...
			pe_warn("There were warnings while calculating the packed size for a "
				"Channel Switch (0x%08x)",
				nStatus);
		}

		nBytes = nPayload + sizeof(tSirMacMgmtHdr);

		qdf_status = cds_packet_alloc(
			(uint16_t)nBytes, (void **)&pFrame, (void **)&pPacket);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			pe_err("Failed to allocate %d bytes for a TPC Report",
			       nBytes);
			return QDF_STATUS_E_FAILURE;
		}
		/* Paranoia: */
		...
		nStatus = dot11f_pack_channel_switch(
			mac, &frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload,
			&nPayload);
		if (DOT11F_FAILED(nStatus)) {
   ...
   */
	...
	lim_send_extended_chan_switch_action_frame(
		struct mac_context * mac_ctx, tSirMacAddr peer, uint8_t mode,
		uint8_t new_op_class, uint8_t new_channel, uint8_t count,
		struct pe_session *session_entry)
	{
		...
		status = dot11f_get_packed_ext_channel_switch_action_frame_size(
			mac_ctx, &frm, &n_payload);
		if (DOT11F_FAILED(status)) {
			pe_err("Failed to get packed size for Channel Switch 0x%08x",
			       status);
			/* We'll fall back on the worst case scenario*/
			n_payload =
				sizeof(tDot11fext_channel_switch_action_frame);
		} else if (DOT11F_WARNED(status)) {
			pe_warn("There were warnings while calculating the packed size for a Ext "
				"Channel Switch (0x%08x)",
				status);
		}
		...
		status = dot11f_pack_ext_channel_switch_action_frame(
		...
		pe_debug("ECSA frame to :" QDF_MAC_ADDR_FMT
			 " count %d mode %d chan %d op class %d",
		...
		qdf_status = wma_tx_frame(mac_ctx, packet, (uint16_t)num_bytes,
					  TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS,
					  7, lim_tx_complete, frame, txFlag,
	...
	/**
   * lim_oper_chan_change_confirm_tx_complete_cnf()- Confirmation for
   * oper_chan_change_confirm sent over the air
   *
   * @context: pointer to global mac
   * @buf: buffer
   * @tx_complete : Sent status
   ...
   */
	...
	static QDF_STATUS lim_oper_chan_change_confirm_tx_complete_cnf(
		void *context, qdf_nbuf_t buf, uint32_t tx_complete,
		void *params)
	{
   ...
   */
		...
		struct pe_session * session_entry)
	{
		...
		qdf_mem_copy(frm.p2p_action_oui.oui_data, SIR_MAC_P2P_OUI,
		...
		if (session_entry->htCapability) {
			...
			populate_dot11f_ht_caps(mac_ctx, session_entry,
						&frm.HTCaps);
		}

		if (session_entry->vhtCapability) {
			pe_debug("Populate VHT Caps in Assoc Request");
			populate_dot11f_vht_caps(mac_ctx, session_entry,
						 &frm.VHTCaps);
			populate_dot11f_operating_mode(
				mac_ctx, &frm.OperatingMode, session_entry);
		}

		status = dot11f_get_packed_p2p_oper_chan_change_confirmSize(
			mac_ctx, &frm, &n_payload);
		if (DOT11F_FAILED(status)) {
		...
		status = dot11f_pack_p2p_oper_chan_change_confirm(
			mac_ctx, &frm, frame + sizeof(tSirMacMgmtHdr),
			n_payload, &n_payload);
		if (DOT11F_FAILED(status)) {
		...
		pe_debug(
			"Send frame on channel freq %d to mac " QDF_MAC_ADDR_FMT,
			session_entry->curr_op_freq, QDF_MAC_ADDR_REF(peer));
		...
		qdf_status = wma_tx_frameWithTxComplete(
			mac_ctx, packet, (uint16_t)num_bytes,
			TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7,
			lim_tx_complete, frame,
			lim_oper_chan_change_confirm_tx_complete_cnf, tx_flag,
			vdev_id, false, 0, RATEID_DEFAULT, 0, 0);
   ...
   */
	...
	lim_send_neighbor_report_request_frame(
		...
		tpSirMacNeighborReportReq pNeighborReq, tSirMacAddr peer,
		struct pe_session * pe_session)
	{
		...
		if (!pe_session) {
			pe_err("(!psession) in Request to send Neighbor Report request action "
			       "frame");
			return QDF_STATUS_E_FAILURE;
		}
		smeSessionId = pe_session->smeSessionId;
		...
		if (pNeighborReq->ssid_present) {
			populate_dot11f_ssid(mac, &pNeighborReq->ssid,
					     &frm.SSID);
		}

		nStatus = dot11f_get_packed_neighbor_report_request_size(
			mac, &frm, &nPayload);
		if (DOT11F_FAILED(nStatus)) {
			pe_err("Failed to calculate the packed size for a Neighbor Report "
			       "Request(0x%08x)",
			       nStatus);
			/* We'll fall back on the worst case scenario: */
			...
			pe_warn("There were warnings while calculating the packed size for a "
				"Neighbor Report Request(0x%08x)",
				nStatus);
		}
		...
		nStatus = dot11f_pack_neighbor_report_request(
			mac, &frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload,
			&nPayload);
		...
		pe_debug(
			"Sending a Neighbor Report Request to " QDF_MAC_ADDR_FMT,
...
returnAfterError:
					  ...
					  struct pe_session * pe_session)
	{
		...
		/* Even though TPC report an IE, it is represented using fixed fields since
     * it is positioned */
		/* in the middle of other fixed fields in the link report frame(IEEE Std.
     * 802.11k section7.4.6.4 */
		/* and frame parser always expects IEs to come after all fixed fields. It is
     * easier to handle */
		...
		nStatus = dot11f_get_packed_link_measurement_report_size(
			mac, &frm, &nPayload);
		if (DOT11F_FAILED(nStatus)) {
			pe_err("Failed to calculate the packed size for a Link Report (0x%08x)",
			       nStatus);
			/* We'll fall back on the worst case scenario: */
		...
		if (DOT11F_FAILED(nStatus)) {
			pe_err("Failed to pack an Link Report (0x%08x)",
			       nStatus);
		...
		pe_warn("RRM: Sending Link Report to " QDF_MAC_ADDR_FMT
			" on vdev[%d]",
...
returnAfterError:
   ...
   * @vdev_id: vdev Id
   */
	static void lim_beacon_report_response_event(
		uint8_t token, uint8_t num_rpt, uint8_t vdev_id)
	{
		...
		wlan_diag_event.diag_cmn.timestamp_us =
			qdf_get_time_of_the_day_us();
		wlan_diag_event.diag_cmn.ktime_us =
			qdf_ktime_to_us(qdf_ktime_get());
...
#else
static void lim_beacon_report_response_event(uint8_t token, uint8_t num_rpt,
					     uint8_t vdev_id)
{
}
#endif
	...
	lim_send_radio_measure_report_action_frame(
		struct mac_context * mac, uint8_t dialog_token,
		uint8_t num_report, bool is_last_frame,
		tpSirMacRadioMeasureReport pRRMReport, tSirMacAddr peer,
		struct pe_session *pe_session)
	{
		QDF_STATUS status_code = QDF_STATUS_SUCCESS;
		uint8_t *pFrame;
     ...
     */
		...
		for (i = 0; i < frm->num_MeasurementReport; i++) {
			...
			frm->MeasurementReport[i].late =
				0; /* IEEE 802.11k section 7.3.22. (always zero in rrm) */
         ...
         */
				...
				populate_dot11f_beacon_report(
					...
					&pRRMReport[i].report.beaconReport,
					is_last_report);
		...
		nStatus = dot11f_get_packed_radio_measurement_report_size(
			mac, frm, &nPayload);
		if (DOT11F_FAILED(nStatus)) {
			pe_nofl_err(
			...
			pe_warn("Warnings while calculating the size for Radio Measure Report "
				"(0x%08x)",
				nStatus);
		}
		...
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			pe_nofl_err(
				"TX: [802.11 RRM] Allocation of %d bytes failed for RM"
				"Report",
				nBytes);
			qdf_mem_free(frm);
			return QDF_STATUS_E_FAILURE;
		...
		/* Now, we're ready to "pack" the frames */
		nStatus = dot11f_pack_radio_measurement_report(
			mac, frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload,
			&nPayload);
		...
		if (frm->MeasurementReport[0].type == SIR_MAC_RRM_BEACON_TYPE) {
			lim_beacon_report_response_event(
				frm->MeasurementReport[0].token, num_report,
				wlan_vdev_get_id(pe_session->vdev));
		}

		pe_nofl_info(
			"TX: %s seq_no:%d dialog_token:%d no. of APs:%d "
			"is_last_rpt:%d num_report: %d peer:" QDF_MAC_ADDR_FMT,
...
returnAfterError:
	...
	bool lim_is_self_and_peer_ocv_capable(struct mac_context * mac,
					      uint8_t *peer,
					      struct pe_session *pe_session)
	{
	...
	void lim_fill_oci_params(struct mac_context * mac,
				 struct pe_session * session,
				 tDot11fIEoci * oci, uint8_t *peer,
				 uint16_t *tx_chan_width)
	{
		tpDphHashNode sta_ds;
		uint16_t aid;
		uint8_t ch_offset;
		...
		oci->op_class = lim_op_class_from_bandwidth(
			mac, session->curr_op_freq, session->ch_width,
			ch_offset);
		...
		if (WLAN_REG_IS_24GHZ_CH_FREQ(session->curr_op_freq)) {
			if (*tx_chan_width > 20 &&
				...
				*tx_chan_width = 20;
				pe_nofl_debug(
					"peer no support ht40 in 2g, %d :" QDF_MAC_ADDR_FMT,
					sta_ds->ch_width,
					QDF_MAC_ADDR_REF(peer));
			}
		}
   ...
   *
   * \param peer    The Mac address of the station to which this action frame is
   * addressed
   *
   ...
   */
	...
	QDF_STATUS lim_send_sa_query_request_frame(
		struct mac_context * mac, uint8_t *transId, tSirMacAddr peer,
		struct pe_session *pe_session)
	{
       ...
       action: 1 --> SA Query Response action frame */
		...
		if (lim_is_self_and_peer_ocv_capable(mac, peer, pe_session))
			lim_fill_oci_params(mac, pe_session, &frm.oci, NULL,
					    NULL);

		nStatus = dot11f_get_packed_sa_query_req_size(mac, &frm,
							      &nPayload);
		if (DOT11F_FAILED(nStatus)) {
			pe_err("Failed to calculate the packed size for an SA Query Request "
			       "(0x%08x)",
			       nStatus);
			/* We'll fall back on the worst case scenario: */
			nPayload = sizeof(tDot11fSaQueryReq);
		} else if (DOT11F_WARNED(nStatus)) {
			pe_warn("There were warnings while calculating the packed size for an SA "
				"Query Request (0x%08x)",
				nStatus);
		}

		nBytes = nPayload + sizeof(tSirMacMgmtHdr);
		qdf_status = cds_packet_alloc(nBytes, (void **)&pFrame,
					      (void **)&pPacket);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
		...
		nStatus = dot11f_pack_sa_query_req(
			mac, &frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload,
			&nPayload);
		...
		pe_debug("Sending an SA Query Request from " QDF_MAC_ADDR_FMT
			 " to " QDF_MAC_ADDR_FMT,
			 QDF_MAC_ADDR_REF(pe_session->self_mac_addr),
			 QDF_MAC_ADDR_REF(peer));
...
returnAfterError:
   ...
   */
	...
	QDF_STATUS lim_send_sa_query_response_frame(
		struct mac_context * mac, uint8_t *transId, tSirMacAddr peer,
		struct pe_session *pe_session)
	{
       ...
       SA query request transId */
		...
		nStatus = dot11f_get_packed_sa_query_rsp_size(mac, &frm,
							      &nPayload);
		if (DOT11F_FAILED(nStatus)) {
			pe_err("Failed to calculate the packed size for a SA Query Response "
			       "(0x%08x)",
			       nStatus);
			/* We'll fall back on the worst case scenario: */
		...
		nStatus = dot11f_pack_sa_query_rsp(
			mac, &frm, pFrame + sizeof(tSirMacMgmtHdr), nPayload,
			&nPayload);
		...
		pe_debug(
			"Sending SA Query Response to " QDF_MAC_ADDR_FMT
			" op_class %d prim_ch_num %d freq_seg_1_ch_num %d oci_present %d",
...
#else
static inline bool lim_tdls_peer_support_he(tpDphHashNode sta_ds)
{
	return false;
}
#endif
...
#ifdef WLAN_FEATURE_11BE_MLO
	static void lim_prepare_tdls_with_mlo(struct pe_session * session,
...
static void lim_prepare_tdls_with_mlo(struct pe_session *session,
				      tSirMacAddr peer_mac,
				      tSirMacAddr self_mac, uint16_t *action)
{
	sir_copy_mac_addr(self_mac, session->self_mac_addr);
	*action = 0;
}
	...
	QDF_STATUS lim_send_addba_response_frame(
		struct mac_context * mac_ctx, tSirMacAddr peer_mac,
		uint16_t tid, struct pe_session *session,
		uint8_t addba_extn_present, uint8_t amsdu_support,
		uint8_t is_wep, uint16_t calc_buff_size)
	{
		...
		if ((LIM_IS_STA_ROLE(session) &&
		    ...
		    mac_ctx->reject_addba_req) {
			frm.Status.status = STATUS_REQUEST_DECLINED;
			pe_err("refused addba req for rx_aggregation_size: %d "
			       "mac_ctx->reject_addba_req: %d",
			       qos_aggr->rx_aggregation_size,
			       mac_ctx->reject_addba_req);
		} else if (LIM_IS_STA_ROLE(session) &&
			   !mac_ctx->usr_cfg_ba_buff_size) {
			frm_buff_size = QDF_MIN(qos_aggr->rx_aggregation_size,
						calc_buff_size);
			...
			if (buff_size && frm_buff_size > buff_size) {
				pe_debug(
					"buff size: %d larger than peer's capability: %d",
     ...
     */
		cdp_tid_update_ba_win_size(soc, peer_mac, vdev_id, tid,
					   frm_buff_size);
			...
			if (lim_is_session_he_capable(session)) {
				...
				if (he_frag != 0) {
					frm.addba_extn_element.no_fragmentation =
						0;
					frm.addba_extn_element
						.he_frag_operation = he_frag;
				}
			}
		}
     ...
     */
		...
		pe_debug("Sending a ADDBA Response from " QDF_MAC_ADDR_FMT
			 " to " QDF_MAC_ADDR_FMT,
			 QDF_MAC_ADDR_REF(self_mac),
			 QDF_MAC_ADDR_REF(peer_mac));
		pe_debug(
			"tid %d dialog_token %d status %d buff_size %d amsdu_supp %d",
			tid, frm.DialogToken.token, frm.Status.status,
			frm.addba_param_set.buff_size,
			frm.addba_param_set.amsdu_supp);
		pe_debug(
			"addba_extn %d he_capable %d no_frag %d he_frag %d, exted buff size %d",
			addba_extn_present, lim_is_session_he_capable(session),
			frm.addba_extn_element.no_fragmentation,
			frm.addba_extn_element.he_frag_operation,
			frm.addba_extn_element.extd_buff_size);

		status = dot11f_get_packed_addba_rsp_size(mac_ctx, &frm,
							  &payload_size);
		if (DOT11F_FAILED(status)) {
     ...
     */
		...
		qdf_status = wma_tx_frameWithTxComplete(
			...
			TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7, NULL,
			frame_ptr, lim_addba_rsp_tx_complete_cnf, tx_flag,
			vdev_id, false, 0, RATEID_DEFAULT, 0, action);
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
				 session->peSessionId, qdf_status));
		if (QDF_STATUS_SUCCESS != qdf_status) {
			pe_err("wma_tx_frame FAILED! Status [%d]", qdf_status);
...
error_addba_rsp:
				       ...
				       struct wlan_action_frame_args *args)
	{
		...
		pe_debug(
			"Sending a EPCS negotiation Response from " QDF_MAC_ADDR_FMT
			" to " QDF_MAC_ADDR_FMT,
		...
		status = dot11f_get_packed_epcs_neg_rspSize(mac_ctx, &frm,
							    &payload_size);
		if (DOT11F_FAILED(status)) {
			pe_err("Failed to calculate packed size for a EPCS negotiation Response "
			       "(0x%08x).",
			       status);
			/* We'll fall back on the worst case scenario: */
			payload_size = sizeof(tDot11fepcs_neg_rsp);
		} else if (DOT11F_WARNED(status)) {
			pe_warn("There were warnings while calculating packed size for a EPCS "
				"negotiation Response (0x%08x).",
				status);
		}
		...
		status = dot11f_pack_epcs_neg_rsp(
			mac_ctx, &frm, frame_ptr + sizeof(tSirMacMgmtHdr),
			payload_size, &payload_size);
		...
		qdf_status = wma_tx_frame(mac_ctx, pkt_ptr, (uint16_t)num_bytes,
					  TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS,
					  7, lim_tx_complete, frame_ptr,
					  tx_flag, vdev_id, 0, RATEID_DEFAULT,
					  0);
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
				 session->peSessionId, qdf_status));
		if (QDF_STATUS_SUCCESS != qdf_status) {
...
error_epcs_rsp:
				       ...
				       struct wlan_action_frame_args *args)
	{
		...
		session = pe_find_session_by_vdev_id(mac_ctx, vdev_id);
		if (!session) {
			pe_debug("session not found for given vdev_id %d ",
				 vdev_id);
			return QDF_STATUS_E_INVAL;
		}
		...
		pe_debug(
			"Sending a EPCS negotiation Request from " QDF_MAC_ADDR_FMT
			" to " QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(session->self_mac_addr),
			QDF_MAC_ADDR_REF(peer_mac));
		...
		status = dot11f_get_packed_epcs_neg_reqSize(mac_ctx, &frm,
							    &payload_size);
		if (DOT11F_FAILED(status)) {
			pe_err("Failed to calculate packed size for a EPCS negotiation Request "
			       "(0x%08x).",
			       status);
			/* We'll fall back on the worst case scenario: */
		...
		status = dot11f_pack_epcs_neg_req(
			mac_ctx, &frm, frame_ptr + sizeof(tSirMacMgmtHdr),
			payload_size, &payload_size);
...
error_epcs_req:
					    ...
					    struct wlan_action_frame_args *args)
	{
		...
		pe_debug("Sending a EPCS tear down from " QDF_MAC_ADDR_FMT
			 " to " QDF_MAC_ADDR_FMT,
			 QDF_MAC_ADDR_REF(session->self_mac_addr),
			 QDF_MAC_ADDR_REF(peer_mac));
		...
		status = dot11f_pack_epcs_teardown(
			mac_ctx, &frm, frame_ptr + sizeof(tSirMacMgmtHdr),
			payload_size, &payload_size);
		if (DOT11F_FAILED(status)) {
...
error_epcs_td:
				       ...
				       enum wlan_t2lm_resp_frm_type status_code)
	{
		...
		pe_debug(
			"Sending a T2LM negotiation Response from " QDF_MAC_ADDR_FMT
			" to " QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(session->self_mac_addr),
			QDF_MAC_ADDR_REF(peer_mac));
		pe_debug("Dialog token %d status %d", frm.DialogToken.token,
		...
		status = dot11f_get_packed_t2lm_neg_rspSize(mac_ctx, &frm,
							    &payload_size);
		if (DOT11F_FAILED(status)) {
			pe_err("Failed to calculate packed size for a T2LM negotiation Response "
			       "(0x%08x).",
			       status);
			/* We'll fall back on the worst case scenario: */
			payload_size = sizeof(tDot11ft2lm_neg_rsp);
		} else if (DOT11F_WARNED(status)) {
			pe_warn("There were warnings while calculating packed size for a T2LM "
				"negotiation Response (0x%08x).",
				status);
		}
		...
		status = dot11f_pack_t2lm_neg_rsp(
			mac_ctx, &frm, frame_ptr + sizeof(tSirMacMgmtHdr),
			payload_size, &payload_size);
...
error_t2lm_rsp:
	...
	lim_send_t2lm_action_req_frame(
		struct wlan_objmgr_vdev * vdev, uint8_t *peer_mac,
		struct wlan_action_frame_args *args,
		...
		uint8_t token)
	{
		...
		ie_buf = qdf_mem_malloc(sizeof(uint8_t) *
					T2LM_IE_ACTION_FRAME_MAX_LEN);
		...
		t2lm_ie = (struct wlan_ie_tid_to_link_mapping *)&frm.t2lm_ie[0]
				  .data;
		t2lm_frame =
			wlan_mlo_add_t2lm_ie((uint8_t *)ie_buf, t2lm_neg, vdev);
		if (!t2lm_frame) {
		...
		pe_debug(
			"Sending a T2LM negotiation Request from " QDF_MAC_ADDR_FMT
			" to " QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(session->self_mac_addr),
			QDF_MAC_ADDR_REF(peer_mac));
		pe_debug("Dialog token %d", frm.DialogToken.token);
		...
		status = dot11f_get_packed_t2lm_neg_reqSize(mac_ctx, &frm,
							    &payload_size);
		if (DOT11F_FAILED(status)) {
			pe_err("Failed to calculate packed size for a T2LM negotiation Request "
			       "(0x%08x).",
			       status);
			/* We'll fall back on the worst case scenario: */
		...
		status = dot11f_pack_t2lm_neg_req(
			mac_ctx, &frm, frame_ptr + sizeof(tSirMacMgmtHdr),
			payload_size, &payload_size);
...
error_t2lm_req:
   ...
   */
						    ...
						    void *params)
	{
		...
		status = dot11f_unpack_delba_req(
			mac_ctx, (uint8_t *)data + sizeof(*mac_hdr), frame_len,
			&frm, false);
		if (DOT11F_FAILED(status)) {
		...
		pe_debug("delba ota done vdev %d " QDF_MAC_ADDR_FMT
			 " tid %d desc_id %d status %d",
			 mgmt_params->vdev_id, QDF_MAC_ADDR_REF(mac_hdr->da),
			 frm.delba_param_set.tid, mgmt_params->desc_id,
			 tx_complete);
...
error:
					       ...
					       uint8_t tid, uint8_t reason_code)
	{
		...
		pe_debug("send delba vdev %d " QDF_MAC_ADDR_FMT
			 " tid %d reason %d",
			 vdev_id, QDF_MAC_ADDR_REF(peer_macaddr), tid,
			 reason_code);
		qdf_mem_zero((uint8_t *)&frm, sizeof(frm));
		frm.Category.category = ACTION_CATEGORY_BACK;
		...
		status = dot11f_get_packed_delba_req_size(mac_ctx, &frm,
							  &payload_size);
		if (DOT11F_FAILED(status)) {
     ...
     */
		...
		status = dot11f_pack_delba_req(
			mac_ctx, &frm, frame_ptr + sizeof(tSirMacMgmtHdr),
			payload_size, &payload_size);
		if (DOT11F_FAILED(status)) {
			pe_err("Failed to pack a DELBA (0x%08x)", status);
			qdf_status = QDF_STATUS_E_FAILURE;
			goto error_delba;
		} else if (DOT11F_WARNED(status)) {
		...
		qdf_status = wma_tx_frameWithTxComplete(
			mac_ctx, pkt_ptr, (uint16_t)num_bytes,
			TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7, NULL,
			frame_ptr, lim_delba_tx_complete_cnf, tx_flag, vdev_id,
			false, 0, RATEID_DEFAULT, 0, 0);
		if (qdf_status != QDF_STATUS_SUCCESS) {
			pe_err("delba wma_tx_frame FAILED! Status [%d]",
			       qdf_status);
			return qdf_status;
		} else {
			return QDF_STATUS_SUCCESS;
...
#define MAC_AUTH_FRAME_STATUS_CODE_OFFSET 4
   ...
   */
	static void lim_tx_mgmt_frame(struct mac_context * mac_ctx,
				      uint8_t vdev_id, uint32_t msg_len,
				      void *packet, uint8_t *frame)
	{
		...
		qdf_mtrace(QDF_MODULE_ID_PE, QDF_MODULE_ID_WMA,
			   TRACE_CODE_TX_MGMT, session_id, 0);
		...
		qdf_status = wma_tx_frameWithTxComplete(
			mac_ctx, packet, (uint16_t)msg_len,
			TXRX_FRM_802_11_MGMT, ANI_TXDIR_TODS, 7,
			lim_tx_complete, frame, lim_auth_tx_complete_cnf, 0,
			vdev_id, false, channel_freq, min_rid, 0, 0);
		MTRACE(qdf_trace(QDF_MODULE_ID_PE, TRACE_CODE_TX_COMPLETE,
		...
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			...
			lim_diag_event_report(mac_ctx,
					      ...
					      session, auth_ack_status,
					      QDF_STATUS_E_FAILURE);
			/* Pkt will be freed up by the callback */
		}
	}

	static void lim_handle_sae_auth_retry(struct mac_context * mac_ctx,
					      uint8_t vdev_id, uint8_t *frame,
					      uint32_t frame_len)
	{
		struct pe_session *session;
		...
		if (session->limMlmState == eLIM_MLM_WT_SAE_AUTH_STATE)
			wlan_mlme_get_sae_auth_retry_count(mac_ctx->psoc,
							   &retry_count);
		else
		...
		pe_debug("SAE auth frame queued vdev_id %d seq_num %d", vdev_id,
			 mac_ctx->mgmtSeqNum + 1);
...
#ifdef WLAN_FEATURE_11BE_MLO
	static QDF_STATUS lim_update_mld_to_link_address(
		struct mac_context * mac_ctx, struct wlan_objmgr_vdev * vdev,
		tpSirMacMgmtHdr mac_hdr)
	{
		...
		self_link_addr =
			(struct qdf_mac_addr *)wlan_vdev_mlme_get_linkaddr(
				vdev);

		switch (opmode) {
		case QDF_SAP_MODE:
			pre_auth_node = lim_search_pre_auth_list_by_mld_addr(
				mac_ctx, mac_hdr->da);
			if (!pre_auth_node) {
				/**
         ...
         */
				pre_auth_node = lim_search_pre_auth_list(
					mac_ctx, mac_hdr->da);
				if (!pre_auth_node) {
		...
		case QDF_STA_MODE:
			if (!wlan_cm_is_vdev_roaming(vdev)) {
				status = wlan_vdev_get_bss_peer_mac(
					vdev, &peer_link_addr);
				if (QDF_IS_STATUS_ERROR(status))
					return status;
			} else {
				peer_roaming_link_addr =
					wlan_cm_roaming_get_peer_link_addr(
						vdev);
		...
		qdf_mem_copy(mac_hdr->sa, self_link_addr->bytes,
			     QDF_MAC_ADDR_SIZE);
						 ...
						 tpSirMacMgmtHdr mac_hdr)
{
	...
	void lim_send_frame(struct mac_context * mac_ctx, uint8_t vdev_id,
			    uint8_t *buf, uint16_t buf_len)
	{
		...
		pe_debug("sending fc->type: %d fc->subType: %d", fc->type,
			 fc->subType);

		vdev = wlan_objmgr_get_vdev_by_id_from_psoc(
			mac_ctx->psoc, vdev_id, WLAN_LEGACY_MAC_ID);
		if (!vdev)
			return;
     ...
     */
		...
		if (QDF_IS_STATUS_ERROR(status)) {
			pe_err("SAE address conversion failure with status:%d",
			       status);
			return;
		}
	...
	void lim_send_mgmt_frame_tx(struct mac_context * mac_ctx,
				    struct scheduler_msg * msg)
	{
		struct sir_mgmt_msg *mb_msg =
			(struct sir_mgmt_msg *)msg->bodyptr;
		uint32_t msg_len;
		...
		if (fc->subType == SIR_MAC_MGMT_AUTH) {
			...
			if (auth_algo == eSIR_FT_AUTH) {
				...
				sta_pre_auth_ctx = lim_search_pre_auth_list(
					mac_ctx,
					((tpSirMacMgmtHdr)(mb_msg->data))->da);
				pe_debug(
					"FT Auth TX to " QDF_MAC_ADDR_FMT,
					QDF_MAC_ADDR_REF(
						((tpSirMacMgmtHdr)(mb_msg->data))
							->da));
				if (sta_pre_auth_ctx) {
