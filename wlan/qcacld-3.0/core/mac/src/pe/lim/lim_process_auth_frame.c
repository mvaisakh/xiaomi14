 ...
 * - AUTH3 and AUTH4 must have challenge text IE, that is,'type' field has been
 *set to WLAN_ELEMID_CHALLENGE by parser
 * -
 *
 ***ASSUMPTIONS:
 *
...
static QDF_STATUS lim_get_wep_key_sap(struct pe_session *pe_session,
				      ...
				      uint8_t key_id, uint8_t *default_key,
				      qdf_size_t *key_len)
{
	return mlme_get_wep_key(pe_session->vdev, wep_params,
				(MLME_WEP_DEFAULT_KEY_1 + key_id), default_key,
				key_len);
}

static void lim_process_auth_shared_system_algo(
	...
	tSirMacAuthFrameBody *auth_frame, struct pe_session *pe_session)
{
	...
	if (!cfg_privacy_opt_imp) {
		pe_err("rx Auth frame for unsupported auth algorithm %d " QDF_MAC_ADDR_FMT,
     ...
     */
		...
		auth_frame->authStatusCode = STATUS_NOT_SUPPORTED_AUTH_ALG;
		...
		if (tx_timer_change_context(&auth_node->timer,
			...
			pe_warn("Unable to chg context auth response timer for "
     ...
     */
		...
		qdf_mem_zero(auth_node->challengeText,
			     SIR_MAC_SAP_AUTH_CHALLENGE_LENGTH);
		if (!qdf_mem_cmp(challenge_txt_arr, auth_node->challengeText,
				 SIR_MAC_SAP_AUTH_CHALLENGE_LENGTH)) {
		...
		qdf_mem_copy(auth_node->challengeText, challenge_txt_arr,
...
static void lim_process_auth_open_system_algo(
	struct mac_context *mac_ctx, tpSirMacMgmtHdr mac_hdr,
	tSirMacAuthFrameBody *rx_auth_frm_body,
	tSirMacAuthFrameBody *auth_frame, struct pe_session *pe_session)
{
				       ...
				       struct qdf_mac_addr *mld_addr)
{
	...
	vdev = wlan_objmgr_get_vdev_by_macaddr_from_pdev(
		mac_ctx->pdev, mac_hdr->sa, WLAN_LEGACY_MAC_ID);
	if (vdev) {
	...
	vdev = wlan_objmgr_get_vdev_by_macaddr_from_pdev(
		mac_ctx->pdev, mld_addr->bytes, WLAN_LEGACY_MAC_ID);
	if (vdev) {
		wlan_objmgr_vdev_release_ref(vdev, WLAN_LEGACY_MAC_ID);
		return QDF_STATUS_E_ALREADY;
	}
...
void lim_sae_auth_cleanup_retry(struct mac_context *mac_ctx, uint8_t vdev_id)
{
	struct pe_session *pe_session;
				       ...
				       uint8_t *rx_pkt_info)
{
	...
	if (rxbody_ptr[SAE_AUTH_SEQ_OFFSET] == qbody_ptr[SAE_AUTH_SEQ_OFFSET])
		return true;
...
static QDF_STATUS lim_skip_sae_fixed_field(uint8_t *body_ptr,
					   uint32_t frame_len, uint8_t **ie_ptr,
					   qdf_size_t *ie_len)
{
				    ...
				    struct qdf_mac_addr *peer_mld)
{
	...
	status =
		lim_skip_sae_fixed_field(body_ptr, frame_len, &ie_ptr, &ie_len);
				       ...
				       struct pe_session *pe_session)
{
     ...
     */
		...
		if (!pre_auth_node ||
			...
			if (pre_auth_node) {
				pe_debug(
					"Delete existing preauth node for SAE peer in state: "
					"%u " QDF_MAC_ADDR_FMT,
       ...
       */
			...
			status = lim_validate_mac_address_in_auth_frame(
				mac_ctx, mac_hdr, &peer_mld);
			if (QDF_IS_STATUS_ERROR(status)) {
				pe_debug(
					"Drop SAE auth, duplicate entity found");
				return;
			}
       ...
       */
			status = lim_update_link_to_mld_address(
				mac_ctx, pe_session->vdev, mac_hdr);
			if (QDF_IS_STATUS_ERROR(status)) {
				pe_debug(
					"SAE address conversion failure with status:%d",
					status);
				return;
			}
		}
	...
	if (LIM_IS_STA_ROLE(pe_session)) {
		...
		if (frame_len >= (SAE_AUTH_STATUS_CODE_OFFSET + 2)) {
			sae_auth_seq = *(uint16_t *)(body_ptr +
		...
		wlan_connectivity_mgmt_event(
			mac_ctx->psoc, (struct wlan_frame_hdr *)mac_hdr,
	...
	lim_send_sme_mgmt_frame_ind(
		mac_ctx, mac_hdr->fc.subType, (uint8_t *)mac_hdr,
		frame_len + sizeof(tSirMacMgmtHdr), pe_session->vdev_id,
				      ...
				      struct pe_session *pe_session)
{
	...
	if (LIM_IS_AP_ROLE(pe_session)) {
		...
		/* Extract pre-auth context for the STA, if any. */
		sta_pre_auth_ctx =
			lim_search_pre_auth_list(mac_ctx, mac_hdr->sa);
		if (sta_pre_auth_ctx) {
			pe_debug("STA Auth ctx have ininted");
			/* Pre-auth context exists for the STA */
			if (sta_pre_auth_ctx->mlmState ==
		...
		/* Create entry for this STA in pre-auth list */
		sta_pre_auth_ctx = lim_acquire_free_pre_auth_node(
			mac_ctx, &mac_ctx->lim.gLimPreAuthTimerTable);
		if (!sta_pre_auth_ctx) {
		...
		lim_send_sme_mgmt_frame_ind(
			...
			pe_session->smeSessionId, WMA_GET_RX_FREQ(rx_pkt_info),
			WMA_GET_RX_RSSI_NORMALIZED(rx_pkt_info), rx_flags);
	}
}

static uint8_t lim_get_pasn_peer_vdev_id(struct mac_context *mac,
					 uint8_t *bssid)
{
	struct wlan_objmgr_peer *peer;
	uint8_t vdev_id;

	peer = wlan_objmgr_get_peer_by_mac(mac->psoc, bssid, WLAN_MGMT_RX_ID);
	if (!peer) {
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS lim_process_pasn_auth_frame(struct mac_context *mac_ctx,
					      uint8_t vdev_id,
					      uint8_t *rx_pkt_info)
{
	tpSirMacMgmtHdr mac_hdr;
	...
	lim_send_sme_mgmt_frame_ind(mac_ctx, mac_hdr->fc.subType,
				    (uint8_t *)mac_hdr,
				    frame_len + sizeof(tSirMacMgmtHdr), vdev_id,
				    WMA_GET_RX_FREQ(rx_pkt_info),
					 ...
					 struct pe_session *pe_session)
{
	...
	/* AuthFrame 1 */
	sta_ds_ptr = dph_lookup_hash_entry(mac_ctx, mac_hdr->sa, &associd,
	...
	if (sta_ds_ptr) {
		...
		if (pMlmDisassocReq &&
		    (!qdf_mem_cmp(
			    (uint8_t *)mac_hdr->sa,
			    (uint8_t *)&pMlmDisassocReq->peer_macaddr.bytes,
			    QDF_MAC_ADDR_SIZE))) {
			pe_debug(
				"TODO:Ack for disassoc frame is pending Issue delsta "
		...
		if (pMlmDeauthReq &&
		    (!qdf_mem_cmp((uint8_t *)mac_hdr->sa,
				  (uint8_t *)&pMlmDeauthReq->peer_macaddr.bytes,
				  QDF_MAC_ADDR_SIZE))) {
			pe_debug(
				"TODO:Ack for deauth frame is pending Issue delsta "
				"for " QDF_MAC_ADDR_FMT,
				QDF_MAC_ADDR_REF(
     ...
     */
		for (associd = 0; associd < pe_session->dph.dphHashTable.size;
		     associd++) {
			sta_ds_ptr = dph_get_hash_entry(
				mac_ctx, associd,
				&pe_session->dph.dphHashTable);
			if (!sta_ds_ptr)
				continue;
			if (sta_ds_ptr->valid &&
			    (!qdf_mem_cmp((uint8_t *)&sta_ds_ptr->staAddr,
					  (uint8_t *)&(mac_hdr->sa),
		...
		if (sta_ds_ptr && !sta_ds_ptr->rmfEnabled) {
			pe_debug(
				"lim Del Sta Ctx associd: %d sta mac" QDF_MAC_ADDR_FMT,
				associd, QDF_MAC_ADDR_REF(sta_ds_ptr->staAddr));
			lim_send_deauth_mgmt_frame(
				mac_ctx, REASON_UNSPEC_FAILURE,
				(uint8_t *)auth_node->peerMacAddr, pe_session,
				false);
			lim_trigger_sta_deletion(mac_ctx, sta_ds_ptr,
						 pe_session);
			return;
	...
	if (lim_is_auth_algo_supported(
		    mac_ctx, (tAniAuthType)rx_auth_frm_body->authAlgoNumber,
		    pe_session)) {
		struct qdf_mac_addr *mld_addr = &rx_auth_frm_body->peer_mld;

		status = lim_validate_mac_address_in_auth_frame(
			mac_ctx, mac_hdr, mld_addr);
		if (QDF_IS_STATUS_ERROR(status)) {
		...
		case eSIR_SHARED_KEY:
			...
			break;
		default:
			pe_err("rx Auth frm for unsupported auth algo %d " QDF_MAC_ADDR_FMT,
			       rx_auth_frm_body->authAlgoNumber,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
       ...
       */
		...
		pe_err("received Authentication frame for unsupported auth algorithm "
		       "%d " QDF_MAC_ADDR_FMT,
		       rx_auth_frm_body->authAlgoNumber,
		       QDF_MAC_ADDR_REF(mac_hdr->sa));
...
static void lim_process_auth_frame_type2(struct mac_context *mac_ctx,
					 ...
					 tSirMacAuthFrameBody *rx_auth_frm_body,
					 tSirMacAuthFrameBody *auth_frame,
					 uint8_t *plainbody, uint8_t *body_ptr,
					 uint16_t frame_len,
					 struct pe_session *pe_session)
{
     ...
     */
       ...
       */
			pe_debug(
				"rx Auth frm2 from peer in state: %d addr " QDF_MAC_ADDR_FMT,
				pe_session->limMlmState,
				QDF_MAC_ADDR_REF(mac_hdr->sa));
		}
		return;
     ...
     */
		...
		pe_warn("received Auth frame2 from unexpected peer" QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(mac_hdr->sa));
		return;
	}
	...
	if (rx_auth_frm_body->authStatusCode == STATUS_NOT_SUPPORTED_AUTH_ALG) {
		/*
     ...
     */
       ...
       */
			...
			pe_warn("rx Auth frame2 for unexpected auth algo %d" QDF_MAC_ADDR_FMT,
				rx_auth_frm_body->authAlgoNumber,
				QDF_MAC_ADDR_REF(mac_hdr->sa));
			return;
		}
     ...
     */
		pe_err("rx Auth frame from peer with failure code %d " QDF_MAC_ADDR_FMT,
	...
	if (rx_auth_frm_body->authAlgoNumber == eSIR_OPEN_SYSTEM) {
       ...
       */
			...
			pe_err("rx Auth frm from peer for unsupported auth algo "
			       "%d " QDF_MAC_ADDR_FMT,
			       rx_auth_frm_body->authAlgoNumber,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
		...
		if (LIM_IS_AP_ROLE(pe_session)) {
			qdf_status = lim_get_wep_key_sap(pe_session, wep_params,
							 key_id, defaultkey,
							 &val);
		} else {
			qdf_status = mlme_get_wep_key(
				pe_session->vdev, wep_params,
				(MLME_WEP_DEFAULT_KEY_1 + key_id), defaultkey,
				&val);
			if (QDF_IS_STATUS_ERROR(qdf_status)) {
				...
				lim_restore_from_auth_state(
					...
					REASON_UNSPEC_FAILURE, pe_session);
				return;
			}
		}
		key_length = val;
		...
		((tpSirMacAuthFrameBody)plainbody)
			->authTransactionSeqNumber = sir_swap_u16if_needed(
			(uint16_t)(rx_auth_frm_body->authTransactionSeqNumber +
				   1));
		...
		qdf_mem_copy((uint8_t *)((tpSirMacAuthFrameBody)plainbody)
				     ->challengeText,
		...
		if (!encr_auth_frame)
			return;
		lim_encrypt_auth_frame(mac_ctx, key_id, defaultkey, plainbody,
					 ...
					 struct pe_session *pe_session)
{
	...
	if (rx_auth_frm_body->authAlgoNumber != eSIR_SHARED_KEY) {
		pe_err("rx Auth frame3 from peer with auth algo number %d " QDF_MAC_ADDR_FMT,
		       rx_auth_frm_body->authAlgoNumber,
		       QDF_MAC_ADDR_REF(mac_hdr->sa));
		/*
     ...
     */
		if (!mac_hdr->fc.wep) {
			pe_err("received Auth frame3 from peer with no WEP bit "
			       "set " QDF_MAC_ADDR_FMT,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
		...
		auth_node = lim_search_pre_auth_list(mac_ctx, mac_hdr->sa);
		if (!auth_node) {
			pe_warn("received AuthFrame3 from peer that has no preauth "
				"context " QDF_MAC_ADDR_FMT,
				QDF_MAC_ADDR_REF(mac_hdr->sa));
			/*
       ...
       */
		...
		if (auth_node->mlmState == eLIM_MLM_AUTH_RSP_TIMEOUT_STATE) {
			pe_warn("auth response timer timedout for peer " QDF_MAC_ADDR_FMT,
				QDF_MAC_ADDR_REF(mac_hdr->sa));
			/*
       ...
       */
			...
			auth_frame->authTransactionSeqNumber =
				SIR_MAC_AUTH_FRAME_4;
			auth_frame->authStatusCode = STATUS_AUTH_TIMEOUT;
       ...
       */
			pe_err("rx Auth frm3 from peer with status code %d " QDF_MAC_ADDR_FMT,
			       rx_auth_frm_body->authStatusCode,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
			return;
		}
					 ...
					 struct pe_session *pe_session)
{
     ...
     * Log error and ignore the frame.
     */
		pe_warn("received unexpected Auth frame4 from peer in state %d, "
			"addr " QDF_MAC_ADDR_FMT,
			pe_session->limMlmState, QDF_MAC_ADDR_REF(mac_hdr->sa));
		return;
	}
     ...
     * failure to SME.
     */
		pe_err("received Auth frame4 from peer with invalid auth algo "
		       "%d" QDF_MAC_ADDR_FMT,
		       rx_auth_frm_body->authAlgoNumber,
		       QDF_MAC_ADDR_REF(mac_hdr->sa));
		return;
     ...
     */
		...
		pe_err("received Authentication frame from peer with invalid auth seq "
		       "number %d " QDF_MAC_ADDR_FMT,
     ...
     */
		...
		pe_debug("Alloc new data: %pK peer " QDF_MAC_ADDR_FMT,
			 auth_node, QDF_MAC_ADDR_REF(mac_hdr->sa));
		qdf_mem_copy((uint8_t *)auth_node->peerMacAddr,
			     mac_ctx->lim.gpLimMlmAuthReq->peerMacAddr,
			     sizeof(tSirMacAddr));
		auth_node->fTimerStarted = 0;
 ...
 * Return: None
 */
void lim_process_auth_frame(struct mac_context *mac_ctx, uint8_t *rx_pkt_info,
			    struct pe_session *pe_session)
{
	...
	pe_nofl_rl_info(
		"Auth RX: vdev %d sys role %d lim_state %d from " QDF_MAC_ADDR_FMT
		" rssi %d auth_alg %d seq %d",
		pe_session->vdev_id, GET_LIM_SYSTEM_ROLE(pe_session),
		pe_session->limMlmState, QDF_MAC_ADDR_REF(mac_hdr->sa),
		WMA_GET_RX_RSSI_NORMALIZED(rx_pkt_info), auth_alg,
		curr_seq_num);
   ...
   */
     ...
     */
		...
		if (!cfg_privacy_opt_imp) {
			pe_err("received Authentication frame3 from peer that while privacy "
			       "option is turned OFF " QDF_MAC_ADDR_FMT,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
			/*
     ...
     */
		if (!auth_node) {
			pe_err("rx Auth frame with no preauth ctx with WEP bit "
			       "set " QDF_MAC_ADDR_FMT,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
			/*
       ...
       */
		...
		lim_deactivate_and_change_per_sta_id_timer(
			mac_ctx, eLIM_AUTH_RSP_TIMER, auth_node->authNodeIdx);
		...
		if ((auth_node->mlmState != eLIM_MLM_WT_AUTH_FRAME3_STATE) &&
			...
			pe_err("received Authentication frame from peer that is in state "
			       "%d " QDF_MAC_ADDR_FMT,
			       auth_node->mlmState,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
			/*
       ...
       */
		...
		key_length = val;
		decrypt_result = lim_decrypt_auth_frame(
			mac_ctx, defaultkey, body_ptr, plainbody, key_length,
			(uint16_t)(frame_len - SIR_MAC_WEP_IV_LENGTH));
		if (decrypt_result == LIM_DECRYPT_ICV_FAIL) {
			pe_err("received Authentication frame from peer that failed "
			       "decryption: " QDF_MAC_ADDR_FMT,
			       QDF_MAC_ADDR_REF(mac_hdr->sa));
			/* ICV failure */
			lim_delete_pre_auth_node(mac_ctx, mac_hdr->sa);
			auth_frame->authAlgoNumber = eSIR_SHARED_KEY;
		...
		if ((sir_convert_auth_frame2_struct(
			     mac_ctx, plainbody, frame_len - 8,
			     rx_auth_frame) != QDF_STATUS_SUCCESS) ||
		...
		lim_process_ft_auth_frame(mac_ctx, rx_pkt_info, pe_session);
		goto free;
	} else if ((sir_convert_auth_frame2_struct(mac_ctx, body_ptr, frame_len,
						   rx_auth_frame) !=
		    QDF_STATUS_SUCCESS) ||
		   (!is_auth_valid(mac_ctx, rx_auth_frame, pe_session))) {
		pe_err("failed to convert Auth Frame to structure or Auth is not valid");
   ...
   */
	...
	wlan_connectivity_mgmt_event(
		mac_ctx->psoc, (struct wlan_frame_hdr *)mac_hdr,
		pe_session->vdev_id, rx_auth_frm_body->authStatusCode, 0,
		WMA_GET_RX_RSSI_NORMALIZED(rx_pkt_info), auth_alg, 0,
		rx_auth_frm_body->authTransactionSeqNumber, 0, WLAN_AUTH_RESP);
	switch (rx_auth_frm_body->authTransactionSeqNumber) {
	case SIR_MAC_AUTH_FRAME_1:
		lim_process_auth_frame_type1(mac_ctx, mac_hdr, rx_auth_frm_body,
					     rx_pkt_info, curr_seq_num,
					     auth_frame, pe_session);
		break;
	case SIR_MAC_AUTH_FRAME_2:
		lim_process_auth_frame_type2(mac_ctx, mac_hdr, rx_auth_frm_body,
					     auth_frame, plainbody, body_ptr,
					     frame_len, pe_session);
		break;
	case SIR_MAC_AUTH_FRAME_3:
		lim_process_auth_frame_type3(mac_ctx, mac_hdr, rx_auth_frm_body,
					     auth_frame, pe_session);
		break;
	case SIR_MAC_AUTH_FRAME_4:
		lim_process_auth_frame_type4(mac_ctx, mac_hdr, rx_auth_frm_body,
					     pe_session);
		break;
	default:
 ...
 */
static bool lim_process_sae_preauth_frame(struct mac_context *mac,
					  uint8_t *rx_pkt)
{
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		pe_err("dropping the auth frame for vdev id: %d and BSSID " QDF_MAC_ADDR_FMT
		       ", SAE address conversion failure",
		       vdev_id, QDF_MAC_ADDR_REF(dot11_hdr->bssId));
		return false;
	}

	wlan_connectivity_mgmt_event(
		mac->psoc, (struct wlan_frame_hdr *)dot11_hdr, vdev_id,
		sae_status_code, 0, WMA_GET_RX_RSSI_NORMALIZED(rx_pkt),
		auth_alg, sae_auth_seq, sae_auth_seq, 0, WLAN_AUTH_RESP);

	lim_send_sme_mgmt_frame_ind(mac, dot11_hdr->fc.subType,
				    (uint8_t *)dot11_hdr,
				    frm_len + sizeof(tSirMacMgmtHdr),
				    SME_SESSION_ID_ANY, WMA_GET_RX_FREQ(rx_pkt),
					     ...
					     uint8_t *pBd, void *body)
{
	...
	pe_debug("Auth RX: BSSID " QDF_MAC_ADDR_FMT
		 " RSSI:%d auth_alg:%d seq:%d",
		 QDF_MAC_ADDR_REF(mac_hdr->bssId),
		 (uint)abs((int8_t)WMA_GET_RX_RSSI_NORMALIZED(pBd)), auth_alg,
		 curr_seq_num);
   ...
   */
	...
	if (auth_alg == eSIR_AUTH_TYPE_PASN) {
       ...
       */
			vdev_id = wlan_nan_get_vdev_id_from_bssid(
				mac->pdev, mac_hdr->bssId, WLAN_MGMT_RX_ID);
			if (vdev_id == WLAN_UMAC_VDEV_ID_MAX) {
	...
	if (qdf_mem_cmp(pe_session->ftPEContext.pFTPreAuthReq->preAuthbssId,
	...
	pe_info("Pre-Auth RX: type: %d trans_seqnum: %d status: %d %d "
	...
	lim_handle_ft_pre_auth_rsp(mac, ret_status, pBody, frameLen,
				   pe_session);
	qdf_mem_free(rx_auth_frame);
