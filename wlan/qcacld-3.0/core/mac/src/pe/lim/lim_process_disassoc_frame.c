 ...
 * @return None
 */
void lim_process_disassoc_frame(struct mac_context *mac, uint8_t *pRxPacketInfo,
				struct pe_session *pe_session)
{
   ...
   * protected */
	...
	pe_nofl_rl_info(
		"Disassoc RX: vdev %d from " QDF_MAC_ADDR_FMT
	...
	wlan_connectivity_mgmt_event(mac->psoc, (struct wlan_frame_hdr *)pHdr,
				     ...
				     frame_rssi, 0, 0, 0, 0, WLAN_DISASSOC_RX);
   ...
   */
	...
	if (lim_check_disassoc_deauth_ack_pending(mac, (uint8_t *)pHdr->sa)) {
		pe_err("Ignore the DisAssoc received, while waiting for ack of "
		       "disassoc/deauth");
     ...
     */
		...
		/** If the Disassoc is received from the new AP to which we tried to
     * ReAssociate Drop ReAssoc and Restore the Previous context( current
     * connected AP).
     */
	...
	if (LIM_IS_AP_ROLE(pe_session)) {
		...
		default:
			...
			pe_warn("received Disassoc frame with invalid reasonCode: %d "
		...
		case REASON_POOR_RSSI_CONDITIONS:
			...
			if (pe_session->gLimChannelSwitch.state !=
				...
				pe_err("Ignoring disassoc frame due to upcoming channel switch, "
				       "from " QDF_MAC_ADDR_FMT,
		...
		pe_err("received Disassoc frame with invalid reasonCode: %d in role:"
		       "%d in sme state: %d from " QDF_MAC_ADDR_FMT,
		       reasonCode, GET_LIM_SYSTEM_ROLE(pe_session),
		       pe_session->limSmeState, QDF_MAC_ADDR_REF(pHdr->sa));
     ...
     */
		...
		pe_err("received Disassoc frame from peer that is in state: %X, "
		       "addr " QDF_MAC_ADDR_FMT,
		       sta->mlmStaContext.mlmState, QDF_MAC_ADDR_REF(pHdr->sa));
	...
	lim_perform_disassoc(mac, frame_rssi, reasonCode, pe_session, pHdr->sa);
   ...
   */
	if ((LIM_IS_STA_ROLE(pe_session)) &&
	    ((sta_ds->mlmStaContext.mlmState ==
	     ...
	     (sta_ds->mlmStaContext.mlmState == eLIM_MLM_IDLE_STATE)) &&
			  ...
			  tSirMacAddr addr)
{
     ...
     */
		pe_debug(
			"received Disassoc from AP while waiting for Reassoc Rsp");
