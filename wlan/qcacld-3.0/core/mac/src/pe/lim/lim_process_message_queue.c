...
restore_auth_state:
			lim_restore_from_auth_state(mac, eSIR_SME_SUCCESS,
						    STATUS_SUCCESS, session);
		} else {
				   ...
				   struct sir_sae_msg *sae_msg)
{
   ...
   */
	sta_pre_auth_ctx =
		lim_search_pre_auth_list(mac, sae_msg->peer_mac_addr);
	...
	if (sae_msg->sae_status != STATUS_SUCCESS) {
		pe_debug("SAE authentication failed for " QDF_MAC_ADDR_FMT
			 " status: %u",
			 QDF_MAC_ADDR_REF(sae_msg->peer_mac_addr),
			 sae_msg->sae_status);
     ...
     */
		...
		assoc_ind_sent = lim_send_assoc_ind_to_sme(
			mac, session, assoc_req->sub_type, assoc_req->sa,
			assoc_req->assoc_req, ANI_AKM_TYPE_SAE,
			assoc_req->pmf_connection, &assoc_req_copied,
...
void lim_process_sae_msg(struct mac_context *mac, struct sir_sae_msg *body)
{
	...
	if (session->opmode != QDF_STA_MODE &&
		...
		pe_err("SAE:Not supported in this mode %d", session->opmode);
		return;
	}

	pe_debug(
		"SAE:status %d limMlmState %d opmode %d peer: " QDF_MAC_ADDR_FMT,
		sae_msg->sae_status, session->limMlmState, session->opmode,
		QDF_MAC_ADDR_REF(sae_msg->peer_mac_addr));
						    ...
						    uint32_t *msg_buf)
{
	...
	pe_session =
		pe_find_session_by_vdev_id(mac_ctx, set_ie_params->vdev_id);
	if (lim_update_ext_cap_ie(mac_ctx, (uint8_t *)set_ie_params->ie_data,
	...
	wma_ie_params->data =
		(uint8_t *)(wma_ie_params) + sizeof(*wma_ie_params);
	...
	if (!GET_LIM_PROCESS_DEFD_MESGS(mac_ctx)) {
       ...
       */
         ...
         * ADD TS response received over the air */
		    ...
		    (mgmt_pkt_defer)) {
			pe_debug(
				"Defer the current message %s , gLimProcessDefdMsgs is false "
				"and system is not in scan/learn mode",
...
static void __lim_pno_match_fwd_bcn_probepsp(struct mac_context *pmac,
					     uint8_t *rx_pkt_info,
					     tSirProbeRespBeacon *frame,
					     uint32_t ie_len, uint32_t msg_type)
{
   ...
   * the upper layer, hence num_results is set to 1 */
	...
	for (i = 0; i < result->num_results; i++) {
		...
		result->ap[i].channel = wlan_reg_freq_to_chan(
			pmac->pdev, WMA_GET_RX_FREQ(rx_pkt_info));
		...
		qdf_mem_copy((uint8_t *)&result->ap[i].bssid,
			     (uint8_t *)hdr->bssId, sizeof(tSirMacAddr));
		/* Copy IE fields */
...
static void __lim_ext_scan_forward_bcn_probe_rsp(struct mac_context *pmac,
						 uint8_t *rx_pkt_info,
						 tSirProbeRespBeacon *frame,
						 uint32_t ie_len,
						 uint32_t msg_type)
{
	...
	result->ap.capability = lim_get_u16((uint8_t *)&frame->capabilityInfo);
	result->ap.channel =
		wlan_reg_freq_to_chan(pmac->pdev, WMA_GET_RX_FREQ(rx_pkt_info));
	...
	qdf_mem_copy((uint8_t *)&result->ap.bssid.bytes, (uint8_t *)hdr->bssId,
		     QDF_MAC_ADDR_SIZE);
	/* Copy IE fields */
...
static void __lim_process_ext_scan_beacon_probe_rsp(struct mac_context *pmac,
						    uint8_t *rx_pkt_info,
						    uint8_t sub_type)
{
	tSirProbeRespBeacon *frame;
	...
	if (sub_type == SIR_MAC_MGMT_BEACON) {
		...
		status = sir_convert_beacon_frame2_struct(
			pmac, (uint8_t *)rx_pkt_info, frame);
	} else if (sub_type == SIR_MAC_MGMT_PROBE_RSP) {
		pe_debug("Probe Rsp due to ExtScan/epno");
		body = WMA_GET_RX_MPDU_DATA(rx_pkt_info);
		status = sir_convert_probe_frame2_struct(pmac, body, frm_len,
							 frame);
	} else {
		qdf_mem_free(frame);
		return;
 ...
 *    handled by lim_handle_frames_in_scan_state before __lim_handle_beacon call
 * is invoked. during scanning, when any session is active, but beacon/Pr does
 * not belong to that session, pe_session will be null. handled by
 * lim_handle_frames_in_scan_state before __lim_handle_beacon call is invoked.
 * during scanning, when any session is active, and beacon/Pr belongs to one of
 * the session, pe_session will not be null. handled by
 * lim_handle_frames_in_scan_state before __lim_handle_beacon call is invoked.
 * Not scanning, no session:
 *    there should not be any beacon coming, if coming, should be dropped.
 * Not Scanning,
 */
static void __lim_handle_beacon(struct mac_context *mac,
				      ...
				      cds_pkt_t *rx_pkt)
{
	...
	rx_pkt->pkt_meta.mpdu_data_len =
		rx_pkt->pkt_meta.mpdu_len - rx_pkt->pkt_meta.mpdu_hdr_len;
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS lim_allocate_and_get_bcn(struct mac_context *mac_ctx,
					   cds_pkt_t **pkt,
					   uint8_t **rx_pkt_info,
					   ...
					   struct scan_cache_entry *scan_entry)
{
	...
	if (sir_convert_beacon_frame2_struct(mac_ctx, (uint8_t *)rx_pkt_info_l,
...
uint32_t lim_defer_msg(struct mac_context *mac, struct scheduler_msg *pMsg)
{
	...
	if (retCode == TX_SUCCESS) {
		...
		MTRACE(mac_trace_msg_rx(mac, NO_SESSION,
					LIM_TRACE_MAKE_RXMSG(pMsg->type,
							     LIM_MSG_DROPPED)));
	}
					       ...
					       struct pe_session *session_entry)
{
	...
	if (IEEE80211_IS_MULTICAST(mac_hdr->addr2)) {
		pe_debug(
			"Ignoring A2 Invalid Packet received for MC/BC: " QDF_MAC_ADDR_FMT,
			QDF_MAC_ADDR_REF(mac_hdr->addr2));
		return;
	}
	pe_debug("type=0x%x, subtype=0x%x", mac_hdr->fc.type,
		 mac_hdr->fc.subType);
 ...
 */
static bool lim_check_mgmt_registered_frames(struct mac_context *mac_ctx,
					     uint8_t *buff_desc,
					     struct pe_session *session_entry)
{
	tSirMacFrameCtl fc;
		...
		if (cfg_p2p_is_roam_config_disabled(mac_ctx->psoc) &&
			...
			if (frm_len >= sizeof(*action_hdr) && action_hdr &&
				...
				if (category == ACTION_CATEGORY_WNM &&
					...
					pe_debug(
						"p2p session active drop BTM frame");
					return match;
				}
			}
		}
     ...
     */
		...
		lim_send_sme_mgmt_frame_ind(
			...
			vdev_id, WMA_GET_RX_FREQ(buff_desc),
 ...
 *         it not loggable
 */
static bool lim_is_mgmt_frame_loggable(uint8_t type, uint8_t subtype)
{
	if (type != SIR_MAC_MGMT_FRAME)
		return false;
...
static void lim_handle80211_frames(struct mac_context *mac,
				   ...
				   uint8_t *pDeferMsg)
{
	...
	*pDeferMsg = false;
	...
	is_hw_sbs_capable = policy_mgr_is_hw_sbs_capable(mac->psoc);
	if (WLAN_REG_IS_5GHZ_CH_FREQ(frequency) &&
		...
		if (pe_session && (QDF_SAP_MODE == pe_session->opmode)) {
	...
	pe_debug("ProtVersion %d, Type %d, Subtype %d rateIndex=%d", fc.protVer,
		 fc.type, fc.subType, WMA_GET_RX_MAC_RATE_IDX(pRxPacketInfo));
	...
	if (fc.type == SIR_MAC_MGMT_FRAME) {
		if ((mac->mlme_cfg->gen.debug_packet_log &
			...
			QDF_TRACE_HEX_DUMP(
				QDF_MODULE_ID_PE, QDF_TRACE_LEVEL_DEBUG, pHdr,
	...
	if (WMA_IS_EXTSCAN_SCAN_SRC(pRxPacketInfo) ||
		...
		if (fc.subType == SIR_MAC_MGMT_BEACON ||
		    fc.subType == SIR_MAC_MGMT_PROBE_RSP) {
			__lim_process_ext_scan_beacon_probe_rsp(
				mac, pRxPacketInfo, fc.subType);
		} else {
			pe_err("Wrong frameType %d, Subtype %d for %d", fc.type,
			       fc.subType, WMA_GET_SCAN_SRC(pRxPacketInfo));
		}
		goto end;
	}
#endif
	...
	if (!pe_session) {
		if (fc.subType == SIR_MAC_MGMT_AUTH) {
			...
			if (lim_process_auth_frame_no_session(
				    mac, pRxPacketInfo, limMsg->bodyptr) ==
			    QDF_STATUS_SUCCESS) {
				goto end;
			}
		}
		...
		if ((fc.subType != SIR_MAC_MGMT_PROBE_RSP) &&
		    ...
		    (fc.subType != SIR_MAC_MGMT_PROBE_REQ) &&
		    (fc.subType != SIR_MAC_MGMT_ACTION)) {
			pe_session = pe_find_session_by_peer_sta(mac, pHdr->sa,
								 &sessionId);
			if (!pe_session) {
				pe_debug(
					"session does not exist for bssId: " QDF_MAC_ADDR_FMT,
		...
		if (fc.subType == SIR_MAC_MGMT_ACTION) {
			pe_session = pe_find_session_by_bssid(mac, pHdr->da,
							      &sessionId);
		}
	}
	...
	if (fc.protVer !=
	    SIR_MAC_PROTOCOL_VERSION) { /* Received Frame with non-zero
                                                   Protocol Version */
	...
	case SIR_MAC_MGMT_FRAME: {
		...
		case SIR_MAC_MGMT_ASSOC_REQ:
			...
			if (LIM_IS_AP_ROLE(pe_session))
				lim_process_assoc_req_frame(mac, pRxPacketInfo,
							    LIM_ASSOC,
							    pe_session);
			else {
				pe_err("unexpected message received %X",
				       limMsg->type);
				lim_print_msg_name(mac, LOGE, limMsg->type);
			}
			break;
		...
		case SIR_MAC_MGMT_PROBE_RSP:
			if (pe_session)
				lim_process_probe_rsp_frame(mac, pRxPacketInfo,
							    pe_session);
			break;
		...
		case SIR_MAC_MGMT_AUTH:
			lim_process_auth_frame(mac, pRxPacketInfo, pe_session);
			break;
		...
		case SIR_MAC_MGMT_ACTION:
			if (!pe_session)
				...
				if (mac->mlme_cfg->gen.debug_packet_log &
					...
					pe_debug(
						"RX MGMT - Type %hu, SubType %hu, seq num[%d]",
						fc.type, fc.subType,
						((pHdr->seqControl.seqNumHi
						  << HIGH_SEQ_NUM_OFFSET) |
						 pHdr->seqControl.seqNumLo));
				...
				if (WMA_GET_RX_UNKNOWN_UCAST(pRxPacketInfo))
	...
	if (lim_is_mgmt_frame_loggable(fc.type, fc.subType))
		lim_diag_mgmt_rx_event_report(mac, pHdr, pe_session,
					      QDF_STATUS_SUCCESS,
					      QDF_STATUS_SUCCESS);
end:
	lim_pkt_free(mac, TXRX_FRM_802_11_MGMT, pRxPacketInfo,
...
void lim_process_abort_scan_ind(struct mac_context *mac_ctx, uint8_t vdev_id,
				uint32_t scan_id, uint32_t scan_requestor_id)
{
	...
	pe_debug("scan_id %d, scan_requestor_id 0x%x", scan_id,
		 scan_requestor_id);
					  ...
					  struct scheduler_msg *msg)
{
	...
	session = pe_find_session_by_bssid(
		mac_ctx, ht40_scanind->mac_addr.bytes, &session_id);
	if (!session) {
		pe_err("OBSS Scan not started: session id is NULL");
		return;
	}
	pe_debug(
		"OBSS Scan Req: vdev %d (pe session %d) htSupportedChannelWidthSet %d",
				 ...
				 struct scheduler_msg *msg)
{
   ...
   */
	if ((SIR_SME_MODULE_ID ==
		...
		MTRACE(mac_trace(mac_ctx, TRACE_CODE_RX_SME_MSG, NO_SESSION,
				 msg->type));
	} else {
		/*
     ...
     */
		...
		qdf_mem_copy((uint8_t *)&new_msg, (uint8_t *)msg,
			     sizeof(struct scheduler_msg));
		body_ptr = (cds_pkt_t *)new_msg.bodyptr;
		cds_pkt_get_packet_length(body_ptr, &pkt_len);

		qdf_status = wma_ds_peek_rx_packet_info(
			body_ptr, (void **)&new_msg.bodyptr);
	...
	case SIR_HAL_P2P_NOA_ATTR_IND:
		...
		pe_debug("Received message Noa_ATTR %x", msg->type);
		for (i = 0; i < mac_ctx->lim.maxBssId; i++) {
			...
			if ((session_entry) && (session_entry->valid) &&
				...
				pe_debug(
					"uNoa1IntervalCnt=%d uNoa1Duration=%d uNoa1Interval=%d "
					"uNoa1StartTime=%d",
     ...
     */
		if (!msg->bodyptr)
			...
			lim_handle_heart_beat_timeout_for_session(
	...
	case SIR_LIM_ADDR2_MISS_IND:
		pe_err(FL("Addr2 mismatch interrupt received %X"), msg->type);
	...
	case WMA_DEAUTH_TX_COMP:
		lim_deauth_tx_complete_cnf(mac_ctx, msg->bodyval, msg->bodyptr);
		break;
#ifdef FEATURE_AP_MCC_CH_AVOIDANCE
  ...
  **/
	...
	if (size > 0) {
		while ((readMsg = lim_read_deferred_msg_q(mac)) != NULL) {
			qdf_mem_copy((uint8_t *)&limMsg, (uint8_t *)readMsg,
				     sizeof(struct scheduler_msg));
			size--;
 ...
 * Return:  None.
 */
void lim_message_processor(struct mac_context *mac_ctx,
			   struct scheduler_msg *msg)
{
				     ...
				     struct pe_session *pe_session)
{
	...
	mac->lim.gHTMaxAmsduLength = (uint8_t)ht_cap_info->maximal_amsdu_size;
	...
	mac->lim.gHTMaxRxAMpduFactor =
		(uint8_t)mac->mlme_cfg->ht_caps.ampdu_params.max_rx_ampdu_factor;
	...
	mac->lim.gHTOperMode =
		(tSirMacHTOperatingMode)
			mac->mlme_cfg->ht_caps.info_field_2.op_mode;

	mac->lim.gHTPCOActive =
		(uint8_t)mac->mlme_cfg->ht_caps.info_field_3.pco_active;
	mac->lim.gHTPCOPhase =
		(uint8_t)mac->mlme_cfg->ht_caps.info_field_3.pco_phase;
	mac->lim.gHTSecondaryBeacon =
		(uint8_t)mac->mlme_cfg->ht_caps.info_field_3.secondary_beacon;
	mac->lim.gHTDualCTSProtection =
		(uint8_t)mac->mlme_cfg->ht_caps.info_field_3.dual_cts_protection;
	mac->lim.gHTSTBCBasicMCS =
		(uint8_t)mac->mlme_cfg->ht_caps.info_field_3.basic_stbc_mcs;

	/* The lim globals for channelwidth and secondary chnl have been removed and
   * should not be used during no session; instead direct cfg is read and used
   * when no session for transmission of mgmt frames (same as old); For now, we
   * might come here during init and join with pe_session = NULL; in that case
   * just fill the globals which exist Sessionized entries values will be filled
   * in join or add bss req. The ones which are missed in join are filled below
   */
	if (pe_session) {
	...
	for (i = 0; i < mac_ctx->lim.maxBssId; i++) {
		if (mac_ctx->lim.gpSession[i].valid) {
			...
			pe_debug("SME: Curr %s,Prev %s,MLM: Curr %s,Prev %s",
				 ...
				 lim_sme_state_str(mac_ctx->lim.gpSession[i]
							   .limPrevSmeState),
				 lim_mlm_state_str(
					 mac_ctx->lim.gpSession[i].limMlmState),
				 lim_mlm_state_str(mac_ctx->lim.gpSession[i]
							   .limPrevMlmState));
		}
	}
#endif
