				   ...
				   struct pe_session *session_entry)
{
	...
	if (assoc_rsp->HTCaps.present) {
		sta_ds->htGreenfield = (uint8_t)ht_caps->greenField;
				...
				tSchBeaconStruct *beacon)
{
	...
	if (lim_populate_peer_rate_set(mac_ctx, &sta_ds->supportedRates,
				       assoc_rsp->HTCaps.supportedMCSSet, false,
				       session_entry, vht_caps, he_cap, eht_cap,
				       sta_ds,
				       bss_desc) != QDF_STATUS_SUCCESS) {
		pe_err("could not get rateset and extended rate set");
		return;
	}
	sta_ds->vhtSupportedRxNss = ((sta_ds->supportedRates.vhtTxMCSMap &
	...
	if (qos_mode) {
		if (assoc_rsp->edcaPresent) {
			...
			qdf_mem_copy(&sta_ds->qos.peer_edca_params,
				     &assoc_rsp->edca, sizeof(assoc_rsp->edca));

			status = sch_beacon_edca_process(
				mac_ctx, &assoc_rsp->edca, session_entry);
	...
	if (session_entry->limWmeEnabled && assoc_rsp->wmeEdcaPresent) {
		...
		qdf_mem_copy(&sta_ds->qos.peer_edca_params, &assoc_rsp->edca,
     ...
     */
		pe_debug(
			"OMN IE is present in the assoc rsp, update NSS/Ch width");
	}
   ...
   */
	...
	for (cnt = 0; cnt < assoc_rsp->num_tspecs; cnt++) {
		if (upToAc(assoc_rsp->TSPECInfo[cnt].user_priority) ==
		    QCA_WLAN_AC_VO) {
			tsm_ctx->tid = assoc_rsp->TSPECInfo[cnt].user_priority;
			qdf_mem_copy(&tsm_ctx->tsmInfo, &assoc_rsp->tsmIE,
				     sizeof(struct ese_tsm_ie));
			lim_send_sme_tsm_ie_ind(mac_ctx, session_entry,
						assoc_rsp->tsmIE.tsid,
				     ...
				     tpDphHashNode sta_ds)
{
	...
	mlme_set_tdls_chan_switch_prohibited(
		session_entry->vdev, ext_cap->tdls_chan_swit_prohibited);
	;
	pe_debug("ExtCap: tdls_prohibited: %d tdls_chan_swit_prohibited: %d",
		 ext_cap->tdls_prohibited, ext_cap->tdls_chan_swit_prohibited);
#endif
...
static void lim_stop_reassoc_retry_timer(struct mac_context *mac_ctx)
{
	...
	if ((mac_ctx->lim.pe_session) &&
	    (NULL != mac_ctx->lim.pe_session->pLimMlmReassocRetryReq)) {
		qdf_mem_free(mac_ctx->lim.pe_session->pLimMlmReassocRetryReq);
...
static QDF_STATUS lim_handle_pmfcomeback_timer(struct pe_session *session_entry,
					       tpSirAssocRsp assoc_rsp)
{
   ...
   */
	...
	pe_debug(
		"ASSOC res with eSIR_MAC_TRY_AGAIN_LATER recvd.Starting timer to "
		"wait timeout: %d",
		timeout_value);
...
#else
static inline void lim_set_r0kh(tpSirAssocRsp assoc_rsp,
 ...
 */
static bool lim_get_iot_aggr_sz(struct mac_context *mac, uint8_t *ie_ptr,
				uint32_t ie_len, uint32_t *amsdu_sz,
				uint32_t *ampdu_sz)
{
 ...
 * Return: None
 */
static void lim_update_iot_aggr_sz(struct mac_context *mac_ctx, uint8_t *ie_ptr,
				   uint32_t ie_len,
				   struct pe_session *session_entry)
{
	int ret;
	...
	iot_hit = lim_get_iot_aggr_sz(mac_ctx, ie_ptr, ie_len, &amsdu_sz,
				      &ampdu_sz);
	if (!iot_hit)
		return;
 ...
 * Return: None.
 */
static inline void lim_update_mcs_rate_set(struct wlan_objmgr_vdev *vdev,
 ...
 * Return: None.
 */
static QDF_STATUS lim_update_sta_vdev_punc(struct wlan_objmgr_psoc *psoc,
					   uint8_t vdev_id,
					   tpSirAssocRsp assoc_resp)
{
	...
	if (ori_bw == CH_WIDTH_320MHZ) {
		...
		center_freq_320 = wlan_reg_chan_band_to_freq(
			wlan_vdev_get_pdev(vdev), assoc_resp->eht_op.ccfs1,
			band_mask);
	} else {
		center_freq_320 = 0;
	}
	wlan_reg_extract_puncture_by_bw(ori_bw, ori_puncture_bitmap,
					des_chan->ch_freq, center_freq_320,
					CH_WIDTH_20MHZ,
					&primary_puncture_bitmap);
	if (primary_puncture_bitmap) {
		pe_err("sta vdev %d freq %d assoc rsp bw %d puncture 0x%x primary chan is "
		       "punctured",
	...
	pe_debug(
		"sta vdev %d freq %d assoc rsp bw %d puncture 0x%x 320M center "
		"frequency %d intersect bw %d puncture 0x%x",
		vdev_id, des_chan->ch_freq, ori_bw, ori_puncture_bitmap,
		center_freq_320, des_chan->ch_width, des_chan->puncture_bitmap);
	wlan_objmgr_vdev_release_ref(vdev, WLAN_LEGACY_MAC_ID);
 ...
 * Return: None.
 */
static void lim_update_vdev_rate_set(struct wlan_objmgr_psoc *psoc,
				     uint8_t vdev_id, tpSirAssocRsp assoc_resp)
{
	struct wlan_objmgr_vdev *vdev;
	...
	if (assoc_resp->extendedRatesPresent &&
		...
		mlme_set_ext_opr_rate(vdev, assoc_resp->extendedRates.rate,
				      assoc_resp->extendedRates.numRates);
	else
...
static void lim_process_assoc_rsp_t2lm(struct pe_session *session,
				       tpSirAssocRsp assoc_rsp)
{
	struct wlan_objmgr_vdev *vdev;
...
#else
static inline void lim_process_assoc_rsp_t2lm(struct pe_session *session,
					      tpSirAssocRsp assoc_rsp)
{
}
#endif
 ...
 * Return: None
 */
void lim_process_assoc_rsp_frame(struct mac_context *mac_ctx,
				 uint8_t *rx_pkt_info, uint32_t frame_body_len,
				 uint8_t subtype,
				 struct pe_session *session_entry)
{
	uint8_t *body, *ie;
	...
	pe_nofl_rl_info(
		"Assoc rsp RX: subtype %d vdev %d sys role %d lim state %d "
		"rssi %d from " QDF_MAC_ADDR_FMT,
		subtype, session_entry->vdev_id,
		GET_LIM_SYSTEM_ROLE(session_entry), session_entry->limMlmState,
		rssi, QDF_MAC_ADDR_REF(hdr->sa));
	...
	if (((subtype == LIM_ASSOC) &&
		...
		pe_debug(
			"Received Re/Assoc rsp in unexpected state: %d on session: %d",
	...
	if (subtype == LIM_ASSOC) {
		if (qdf_mem_cmp(hdr->sa, current_bssid, sizeof(tSirMacAddr))) {
			/*
       ...
       */
		...
		if (qdf_mem_cmp(hdr->sa, session_entry->limReAssocbssId,
				sizeof(tSirMacAddr))) {
			/*
	...
	if (sir_convert_assoc_resp_frame2_struct(mac_ctx, session_entry, body,
						 frame_body_len, assoc_rsp) ==
	    QDF_STATUS_E_FAILURE) {
		qdf_mem_free(assoc_rsp);
		pe_err("Parse error Assoc resp subtype: %d"
		       "length: %d",
	...
	if (lim_is_session_eht_capable(session_entry)) {
		...
		status = lim_strip_and_decode_eht_op(
			body + ies_offset, frame_body_len - ies_offset,
			&assoc_rsp->eht_op, assoc_rsp->VHTOperation,
			assoc_rsp->he_op, assoc_rsp->HTInfo);
		...
		status = lim_strip_and_decode_eht_cap(
			body + ies_offset, frame_body_len - ies_offset,
			&assoc_rsp->eht_cap, assoc_rsp->he_cap,
	...
	if (!assoc_rsp->suppRatesPresent) {
		...
		qdf_mem_copy(&assoc_rsp->supportedRates,
			     &session_entry->rateSet, sizeof(tSirMacRateSet));
	}
	...
	if (assoc_rsp->status_code == STATUS_DENIED_POOR_CHANNEL_CONDITIONS &&
		...
		if (!assoc_rsp->rssi_assoc_rej.retry_delay)
			ap_info.expected_rssi =
				assoc_rsp->rssi_assoc_rej.delta_rssi +
				WMA_GET_RX_RSSI_NORMALIZED(rx_pkt_info) +
				wlan_dlm_get_rssi_denylist_threshold(
					mac_ctx->pdev);
		else
			ap_info.expected_rssi =
				assoc_rsp->rssi_assoc_rej.delta_rssi +
   ...
   */
	if (!lim_verify_fils_params_assoc_rsp(mac_ctx, session_entry, assoc_rsp,
					      &assoc_cnf)) {
	...
	if (assoc_rsp->QosMapSet.present)
		qdf_mem_copy(&session_entry->QosMapSet, &assoc_rsp->QosMapSet,
	...
	if (lim_is_session_he_capable(session_entry)) {
		lim_set_twt_peer_capabilities(
			mac_ctx, (struct qdf_mac_addr *)current_bssid,
			&assoc_rsp->he_cap, &assoc_rsp->he_op);
	...
	if (subtype != LIM_REASSOC) {
		...
		wlan_connectivity_mgmt_event(mac_ctx->psoc,
					     ...
					     session_entry->vdev_id,
					     assoc_rsp->status_code, 0, rssi, 0,
					     0, 0, aid, WLAN_ASSOC_RSP);
	}

	ap_nss = lim_get_nss_supported_by_ap(
		&assoc_rsp->VHTCaps, &assoc_rsp->HTCaps, &assoc_rsp->he_cap);
     ...
     */
		...
		lim_update_vdev_rate_set(
			mac_ctx->psoc, session_entry->smeSessionId, assoc_rsp);
		...
		if ((session_entry->limMlmState ==
			...
			if (!lim_is_roam_synch_in_progress(mac_ctx->psoc,
				...
				lim_objmgr_update_emlsr_caps(
					mac_ctx->psoc, session_entry->vdev_id,
					assoc_rsp);
	...
	lim_objmgr_update_vdev_nss(mac_ctx->psoc, session_entry->smeSessionId,
				   session_entry->nss);
...
assocReject:
	if ((subtype == LIM_ASSOC) ||
	    ((subtype == LIM_REASSOC) && (session_entry->limMlmState ==
					  eLIM_MLM_WT_FT_REASSOC_RSP_STATE))) {
		pe_err("Assoc Rejected by the peer mlmestate: %d sessionid: %d Reason: %d "
