 ...
 * @return None
 */
void rrm_cache_mgmt_tx_power(struct mac_context *mac, int8_t txPower,
				    ...
				    struct scheduler_msg *limMsgQ)
{
	...
	if (qdf_is_macaddr_broadcast(&pMaxTxParams->bssId)) {
		...
		pe_session = pe_find_session_by_bssid(
			mac, pMaxTxParams->bssId.bytes, &sessionId);
		if (!pe_session) {
				     ...
				     struct pe_session *pe_session)
{
   ...
   */
     ...
     */
		...
		mlme_obj->reg_tpc_obj.reg_max[0] = pe_session->def_max_tx_pwr;
		mlme_obj->reg_tpc_obj.ap_constraint_power =
			pLinkReq->MaxTxPower.maxTxPower;
				     ...
				     struct pe_session *pe_session)
{
	...
	for (i = 0; i < pNeighborRep->num_NeighborReport; i++) {
		pSmeNeighborRpt->sNeighborBssDescription[i].length = sizeof(
			tSirNeighborBssDescription); /*+ any optional ies */
		qdf_mem_copy(pSmeNeighborRpt->sNeighborBssDescription[i].bssId,
			     pNeighborRep->NeighborReport[i].bssid,
			     sizeof(tSirMacAddr));
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fApPreauthReachable =
			pNeighborRep->NeighborReport[i].APReachability;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fSameSecurityMode =
			pNeighborRep->NeighborReport[i].Security;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fSameAuthenticator =
			pNeighborRep->NeighborReport[i].KeyScope;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fCapSpectrumMeasurement =
			pNeighborRep->NeighborReport[i].SpecMgmtCap;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fCapQos =
			pNeighborRep->NeighborReport[i].QosCap;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fCapApsd =
			pNeighborRep->NeighborReport[i].apsd;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fCapRadioMeasurement =
			pNeighborRep->NeighborReport[i].rrm;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fCapDelayedBlockAck =
			pNeighborRep->NeighborReport[i].DelayedBA;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fCapImmediateBlockAck =
			pNeighborRep->NeighborReport[i].ImmBA;
		pSmeNeighborRpt->sNeighborBssDescription[i]
			.bssidInfo.rrmInfo.fMobilityDomain =
				...
				tpSirNeighborReportReqInd pNeighborReq)
{
	...
	pe_session =
		pe_find_session_by_bssid(mac, pNeighborReq->bssId, &sessionId);
	if (!pe_session) {
	...
	status = lim_send_neighbor_report_request_frame(
		mac, &NeighborReportReq, pNeighborReq->bssId, pe_session);
 ...
 * @vdev_id: vdev Id
 */
static void wlan_diag_log_beacon_rpt_req_event(uint8_t token, uint8_t mode,
...
rrm_process_beacon_report_req(struct mac_context *mac, tpRRMReq pCurrentReq,
			      ...
			      struct pe_session *pe_session)
{
     ...
     * repetitions. */
		/* Beacon reporting should not be included in request if number of
     * repetitons is zero. */
		/* IEEE Std 802.11k-2008 Table 7-29g and section 11.10.8.1 */

		pe_nofl_err(
			"RX: [802.11 BCN_RPT] Dropping req: Reporting condition "
			"included is not zero");
		return eRRM_INCAPABLE;
	}

	/* The logic here is to check the measurement duration passed in the beacon
     request. Following are the cases handled. Case 1: If measurement duration
     received in the beacon request is greater than the max measurement duration
     advertised in the RRM capabilities(Assoc Req), and Duration Mandatory bit
     is set to 1, REFUSE the beacon request Case 2: If measurement duration
     received in the beacon request is greater than the max measurement duration
     advertised in the RRM capabilities(Assoc Req), and Duration Mandatory bit
     is set to 0, perform measurement for the duration advertised in the RRM
     capabilities
   ...
   */
	...
	pe_nofl_info(
		"RX: [802.11 BCN_RPT] seq:%d SSID:" QDF_SSID_FMT
		" BSSID:" QDF_MAC_ADDR_FMT
		" Token:%d op_class:%d ch:%d meas_mode:%d meas_duration:%d max_dur: %d "
		"sign: %d max_meas_dur: %d",
		...
		pBeaconReq->measurement_request.Beacon.meas_mode, measDuration,
	...
	wlan_diag_log_beacon_rpt_req_event(
		...
		pBeaconReq->measurement_request.Beacon.regClass,
		pBeaconReq->measurement_request.Beacon.channel, req_mode,
		measDuration, wlan_vdev_get_id(pe_session->vdev));

	if (measDuration == 0 &&
	    pBeaconReq->measurement_request.Beacon.meas_mode !=
		    eSIR_BEACON_TABLE) {
		pe_nofl_err(
			"RX: [802.11 BCN_RPT] Invalid measurement duration");
		return eRRM_REFUSED;
	}
	...
	if (maxMeasduration < measDuration) {
		if (pBeaconReq->durationMandatory) {
			pe_nofl_err(
				"RX: [802.11 BCN_RPT] Dropping the req: duration mandatory & "
				"maxduration > measduration");
			return eRRM_REFUSED;
		} else
	...
	pCurrentReq->request.Beacon.reportingDetail =
		pBeaconReq->measurement_request.Beacon.BcnReportingDetail
			...
			pBeaconReq->measurement_request.Beacon
				.BcnReportingDetail.reportingDetail :
			BEACON_REPORTING_DETAIL_ALL_FF_IE;

	if (pBeaconReq->measurement_request.Beacon.last_beacon_report_indication
		    .present) {
	...
	if (pBeaconReq->measurement_request.Beacon.RequestedInfo.present) {
		if (!pBeaconReq->measurement_request.Beacon.RequestedInfo
			     .num_requested_eids) {
			pe_debug(
				"RX: [802.11 BCN_RPT]: Requested num of EID is 0");
			return eRRM_FAILURE;
		}
		pCurrentReq->request.Beacon.reqIes.pElementIds = qdf_mem_malloc(
			sizeof(uint8_t) *
	...
	qdf_mem_copy(psbrr->bssId, pe_session->bssId, sizeof(tSirMacAddr));
	...
	psbrr->randomizationInterval = SYS_TU_TO_MS(
		pBeaconReq->measurement_request.Beacon.randomization);
	...
	if (psbrr->channel_info.chan_num &&
		...
		psbrr->channel_info.chan_freq =
			wlan_reg_country_chan_opclass_to_freq(
				mac->pdev, country,
				psbrr->channel_info.chan_num,
				psbrr->channel_info.reg_class, false);
	...
	sme_debug(
		"opclass %d, ch %d freq %d AP's country code %c%c 0x%x index:%d",
		psbrr->channel_info.reg_class, psbrr->channel_info.chan_num,
		psbrr->channel_info.chan_freq, country[0], country[1],
		country[2], psbrr->measurement_idx);
	...
	for (idx_rpt = 0; idx_rpt < num_rpt; idx_rpt++) {
		ie_ap_chan_rpt = &pBeaconReq->measurement_request.Beacon
					  .APChannelReport[idx_rpt];
		for (tmp_idx = 0; tmp_idx < ie_ap_chan_rpt->num_channelList;
			...
			if (ch_ctr >=
			    QDF_ARRAY_SIZE(psbrr->channel_list.chan_freq_lst))
				break;
		}
	...
	for (idx_rpt = 0; idx_rpt < ch_ctr; idx_rpt++) {
		buf_cons =
			qdf_snprint(tmp_buf, buf_left, "%d ",
 ...
 *	   in pIes.
 */
static uint16_t rrm_fill_beacon_ies(struct mac_context *mac, uint8_t *pIes,
				    uint8_t *pNumIes, uint8_t pIesMaxSize,
				    uint8_t *eids, uint8_t numEids,
				    ...
				    struct bss_description *bss_desc)
{
	...
	if (start_offset > BcnNumIes) {
		pe_err("Invalid start offset %d Bcn IE len %d", start_offset,
		       total_ies_len);
		return 0;
	}
   ...
   */
	...
	while (BcnNumIes >= 2) {
		...
		pe_debug("EID = %d, len = %d total = %d", *pBcnIes,
			 *(pBcnIes + 1), len);

		if (BcnNumIes < len || len <= 2) {
			pe_err("RRM: Invalid IE len:%d exp_len:%d", len,
			       BcnNumIes);
			break;
		}
	...
	pe_debug("Total length of Ies added = %d rem_len %d", *pNumIes,
		 rem_len);
   ...
   */
	if (beacon_xmit_ind->numBssDesc || curr_req->sendEmptyBcnRpt ||
		...
		session_entry = pe_find_session_by_bssid(
			mac_ctx, beacon_xmit_ind->bssId, &session_id);
		if (!session_entry) {
			pe_err("TX: [802.11 BCN_RPT] Session does not exist for "
			       "bssId:" QDF_MAC_ADDR_FMT "",
		...
		for (i = 0; i < MAX_BEACON_REPORTS &&
			    bss_desc_count < beacon_xmit_ind->numBssDesc;
		     i++) {
       ...
       */
			pe_debug("TX: [802.11 BCN_RPT] report %d bss %d", i,
				 bss_desc_count);
			bss_desc =
				beacon_xmit_ind->pBssDescription[bss_desc_count];
       ...
       */
			...
			if (bss_desc) {
				beacon_report->channel = wlan_reg_freq_to_chan(
					mac_ctx->pdev, bss_desc->chan_freq);
			...
			pe_debug(
				"TX: [802.11 BCN_RPT] reporting detail requested %d",
			...
			default:
				...
				rem_len = rrm_fill_beacon_ies(
					...
					(uint8_t *)&beacon_report->Ies[0],
					(uint8_t *)&beacon_report->numIes,
					BEACON_REPORT_MAX_IES, NULL, 0, offset,
					bss_desc);
				break;
			}
       ...
       */
			if (rem_len && curr_req->request.Beacon
					       .last_beacon_report_indication) {
				offset = GET_IE_LEN_IN_BSS(bss_desc->length) -
					 rem_len;
				pe_debug(
					"TX: [802.11 BCN_RPT] offset %d ie_len %lu rem_len %d frag_id %d",
					offset,
				...
				bss_desc_count++;
				pe_debug(
					"TX: [802.11 BCN_RPT] No remaining IEs");
			}
		...
		pe_debug(
			"TX: [802.11 BCN_RPT] Total reports filled %d, last bcn_rpt ind:%d",
			i,
			curr_req->request.Beacon.last_beacon_report_indication);
		...
		for (j = 0; j < num_frames; j++) {
			...
			lim_send_radio_measure_report_action_frame(
				mac_ctx, curr_req->dialog_token,
				num_reports_in_frame, is_last_measurement_frame,
				&report[report_index], beacon_xmit_ind->bssId,
				session_entry);
 ...
 */
static QDF_STATUS
rrm_process_beacon_req(struct mac_context *mac_ctx, tSirMacAddr peer,
		       struct pe_session *session_entry,
		       ...
		       uint8_t *num_report, int index)
{
	...
	if (index >= MAX_MEASUREMENT_REQUEST) {
		status = rrm_reject_req(
			&report, rrm_req, num_report, index,
	...
	curr_req->token = rrm_req->MeasurementRequest[index].measurement_token;
	curr_req->sendEmptyBcnRpt = true;
	curr_req->measurement_idx = index;
	mac_ctx->rrm.rrmPEContext.pCurrentReq[index] = curr_req;
	mac_ctx->rrm.rrmPEContext.num_active_request++;
	pe_debug("Processing Bcn Report req %d num_active_req:%d", index,
		 mac_ctx->rrm.rrmPEContext.num_active_request);
	rrm_status = rrm_process_beacon_report_req(
		mac_ctx, curr_req, &rrm_req->MeasurementRequest[index],
		session_entry);
 ...
 * Return: QDF_STATUS
 */
static QDF_STATUS update_rrm_report(struct mac_context *mac_ctx,
			  ...
			  uint8_t measurement_type)
{
     ...
     * any subsequent requests.
     */
		*radiomes_report = qdf_mem_malloc(
			sizeof(*report) *
			(rrm_req->num_MeasurementRequest - index));
		if (!*radiomes_report)
			return QDF_STATUS_E_NOMEM;
		...
		pe_debug(
			"rrm beacon refused of %d report, index: %d in beacon table",
			*num_report, index);
	}
				      ...
				      struct pe_session *session_entry)
{
	...
	if (!rrm_req->num_MeasurementRequest) {
		...
		if (!report)
			return QDF_STATUS_E_NOMEM;
		pe_err("RX: [802.11 RRM] No requestIes in the measurement request, sending "
		       "incapable report");
		report->incapable = 1;
		num_report = 1;
		lim_send_radio_measure_report_action_frame(
			mac_ctx, rrm_req->DialogToken.token, num_report, true,
   ...
   */
	...
	for (i = 0; i < rrm_req->num_MeasurementRequest; i++) {
		...
		case SIR_MAC_RRM_FINE_TIME_MEAS_TYPE:
			pe_debug(
				"RRM with type: %d sent to userspace",
				rrm_req->MeasurementRequest[i].measurement_type);
			break;
		default:
			/* Send a report with incapabale bit set. */
...
void rrm_cleanup(struct mac_context *mac, uint8_t idx)
{
	...
	pe_debug("Beacon report cleanup idx:%d, num_active_request:%d", idx,
		 mac->rrm.rrmPEContext.num_active_request);
