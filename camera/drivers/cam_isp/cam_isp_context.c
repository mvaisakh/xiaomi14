...
#define INC_HEAD(head, max_entries, ret) \
	div_u64_rem(atomic64_add_return(1, head), max_entries, (ret))

static int cam_isp_context_dump_requests(void *data, void *pf_args);
...
static int
__cam_isp_ctx_check_deferred_buf_done(struct cam_isp_context *ctx_isp,
				      struct cam_isp_hw_done_event_data *done,
				      uint32_t bubble_state);

// xiaomi add
static int __cam_isp_notify_mqs_event(uint32_t error_type, uint32_t error_code,
				      uint64_t error_request_id,
				      struct cam_context *ctx);
// xiaomi add

static const char *__cam_isp_evt_val_to_type(uint32_t evt_id)
{
	switch (evt_id) {
...
static void __cam_isp_ctx_update_event_record(struct cam_isp_context *ctx_isp,
					      ...
					      struct cam_ctx_request *req)
{
	...
	ctx_isp->dbg_monitors.event_record[event][iterator].timestamp =
		cur_time;
}
...
static void *cam_isp_ctx_user_dump_events(void *dump_struct, uint8_t *addr_ptr)
{
	uint64_t *addr;
...
static int __cam_isp_ctx_print_event_record(struct cam_isp_context *ctx_isp)
{
	...
	for (i = 0; i < CAM_ISP_CTX_EVENT_MAX; i++) {
		state_head = atomic64_read(
			&ctx_isp->dbg_monitors.event_record_head[i]);
		...
		for (j = 0; j < num_entries; j++) {
			...
			len += scnprintf(buf + len,
					 CAM_ISP_CONTEXT_DBG_BUF_LEN - len,
					 "%llu[%lld:%06lld] ", record->req_id,
					 ts.tv_sec, ts.tv_nsec / NSEC_PER_USEC);
			index = (index + 1) %
				CAM_ISP_CTX_EVENT_RECORD_MAX_ENTRIES;
		}
		if (len)
			CAM_INFO(CAM_ISP, "Ctx:%d %s: %s", ctx->ctx_id,
				 __cam_isp_evt_val_to_type(i), buf);
	}
	return rc;
}

static int
__cam_isp_ctx_dump_event_record(struct cam_isp_context *ctx_isp,
				struct cam_common_hw_dump_args *dump_args)
{
	...
	if (!dump_args || !ctx_isp) {
		CAM_ERR(CAM_ISP, "Invalid args %pK %pK", dump_args, ctx_isp);
		return -EINVAL;
	}
	...
	for (i = 0; i < CAM_ISP_CTX_EVENT_MAX; i++) {
		...
		for (j = 0; j < num_entries; j++) {
			...
			rc = cam_common_user_dump_helper(
				dump_args, cam_isp_ctx_user_dump_events, record,
				sizeof(uint64_t),
				"ISP_EVT_%s:", __cam_isp_evt_val_to_type(i));
			if (rc) {
				CAM_ERR(CAM_ISP,
					...
					unsigned long *bytes_updated)
{
	...
	*bytes_updated += bytes_required;
	...
	if (req_isp->num_fence_map_out) {
		...
		req_md->map_out =
			(struct cam_hw_fence_map_entry *)((uint8_t *)start_addr +
							  *bytes_updated);
	...
	if (packet && packet->num_io_configs) {
		bytes_required =
			packet->num_io_configs * sizeof(struct cam_buf_io_cfg);
		if (start_addr + *bytes_updated + bytes_required > end_addr)
			return;

		io_cfg =
			(struct cam_buf_io_cfg *)((uint32_t *)&packet->payload +
						  packet->io_configs_offset /
							  4);
		req_md->io_cfg =
			(struct cam_buf_io_cfg *)((uint8_t *)start_addr +
						  *bytes_updated);
...
static int __cam_isp_ctx_minidump_cb(void *priv, void *args)
{
	...
	for (i = 0; i < CAM_ISP_CTX_EVENT_MAX; i++) {
		memcpy(md->event_record[i],
		       ctx_isp->dbg_monitors.event_record[i],
	...
	if (!list_empty(&ctx->active_req_list)) {
		md->active_list =
			(struct cam_isp_ctx_req_mini_dump *)(start_addr +
							     total_bytes);
		list_for_each_entry_safe(req, req_temp, &ctx->active_req_list,
					 list) {
			bytes_updated = 0;
			__cam_isp_ctx_req_mini_dump(
				req,
	...
	if (!list_empty(&ctx->wait_req_list)) {
		md->wait_list =
			(struct cam_isp_ctx_req_mini_dump *)(start_addr +
							     total_bytes);
		list_for_each_entry_safe(req, req_temp, &ctx->wait_req_list,
					 list) {
			bytes_updated = 0;
			__cam_isp_ctx_req_mini_dump(
				req, (uint8_t *)&md->wait_list[md->wait_cnt++],
				end_addr, &bytes_updated);
			total_bytes += bytes_updated;
			if ((start_addr + total_bytes >= end_addr))
				goto end;
	...
	if (!list_empty(&ctx->pending_req_list)) {
		md->pending_list =
			(struct cam_isp_ctx_req_mini_dump *)(start_addr +
							     total_bytes);
		list_for_each_entry_safe(req, req_temp, &ctx->pending_req_list,
					 list) {
			bytes_updated = 0;
			__cam_isp_ctx_req_mini_dump(
				req,
	...
	enum cam_isp_state_change_trigger trigger_type, uint64_t req_id)
{
	...
	ctx_isp->dbg_monitors.state_monitor[iterator].frame_id =
		ctx_isp->frame_id;
	ctx_isp->dbg_monitors.state_monitor[iterator].trigger = trigger_type;
	ctx_isp->dbg_monitors.state_monitor[iterator].req_id = req_id;
...
static int
__cam_isp_ctx_update_frame_timing_record(enum cam_isp_hw_event_type hw_evt,
					 struct cam_isp_context *ctx_isp)
{
	uint32_t index = 0;
	...
	if (hw_evt == CAM_ISP_HW_EVENT_SOF)
		...
		div_u64_rem(atomic64_read(
				    &ctx_isp->dbg_monitors.frame_monitor_head),
			    CAM_ISP_CTX_MAX_FRAME_RECORDS, &index);

	switch (hw_evt) {
	case CAM_ISP_HW_EVENT_SOF:
		CAM_GET_BOOT_TIMESTAMP(
			ctx_isp->dbg_monitors.frame_monitor[index].sof_ts);
		break;
	case CAM_ISP_HW_EVENT_EOF:
		CAM_GET_BOOT_TIMESTAMP(
			ctx_isp->dbg_monitors.frame_monitor[index].eof_ts);
		break;
	case CAM_ISP_HW_EVENT_EPOCH:
		CAM_GET_BOOT_TIMESTAMP(
			ctx_isp->dbg_monitors.frame_monitor[index].epoch_ts);
		break;
	case CAM_ISP_HW_SECONDARY_EVENT:
		CAM_GET_BOOT_TIMESTAMP(
			ctx_isp->dbg_monitors.frame_monitor[index]
				.secondary_sof_ts);
		break;
	default:
		break;
...
static void
__cam_isp_ctx_dump_frame_timing_record(struct cam_isp_context *ctx_isp)
{
	int i = 0;
	...
	for (i = 0; i < num_entries; i++) {
		CAM_INFO(
			CAM_ISP,
			"Index: %u SOF_TS: %llu.%llu EPOCH_TS: %llu.%llu EOF_TS: %llu.%llu "
			"SEC_SOF: %llu.%llu",
			index,
			...
			ctx_isp->dbg_monitors.frame_monitor[index]
				...
				NSEC_PER_USEC,
			ctx_isp->dbg_monitors.frame_monitor[index]
				.secondary_sof_ts.tv_sec,
			ctx_isp->dbg_monitors.frame_monitor[index]
					.secondary_sof_ts.tv_nsec /
				NSEC_PER_USEC);
...
__cam_isp_ctx_dump_state_monitor_array(struct cam_isp_context *ctx_isp)
{
	...
	for (i = 0; i < num_entries; i++) {
		time64_to_tm(ctx_isp->dbg_monitors.state_monitor[index]
				     .evt_time_stamp.tv_sec,
			     0, &ts);
		CAM_ERR(CAM_ISP,
			"Idx[%d] time[%d-%d %d:%d:%d.%lld]:Substate[%s] Frame[%lld] "
			"Req[%llu] evt[%s]",
			index, ts.tm_mon + 1, ts.tm_mday, ts.tm_hour, ts.tm_min,
			ts.tm_sec,
			ctx_isp->dbg_monitors.state_monitor[index]
					.evt_time_stamp.tv_nsec /
				1000000,
...
static void *cam_isp_ctx_user_dump_state_monitor_array_info(void *dump_struct,
							    uint8_t *addr_ptr)
{
...
static int cam_isp_context_info_dump(void *context, enum cam_context_dump_id id)
{
	...
	default:
		CAM_DBG(CAM_ISP,
			"DUMP id not valid %u, ctx_idx: %u, link: 0x%x", id,
...
static const char *__cam_isp_ctx_crm_trigger_point_to_string(int trigger_point)
{
...
static int __cam_isp_ctx_notify_trigger_util(int trigger_type,
					     struct cam_isp_context *ctx_isp)
{
	int rc = -EINVAL;
	struct cam_context *ctx = ctx_isp->base;
	...
	if (!(ctx_isp->subscribe_event & trigger_type)) {
		CAM_DBG(CAM_ISP,
			"%s trigger point not subscribed for in mask: %u in ctx: %u on "
			"link: 0x%x last_bufdone: %lld",
	...
	if (atomic_read(&ctx_isp->internal_recovery_set)) {
		CAM_DBG(CAM_ISP,
			"Internal recovery in progress skip notifying %s trigger point in "
			"ctx: %u on link: 0x%x",
			__cam_isp_ctx_crm_trigger_point_to_string(trigger_type),
			ctx->ctx_id, ctx->link_hdl);
		return 0;
	...
	if (rc)
		CAM_ERR_RATE_LIMIT(
			CAM_ISP,
			"Failed to notify CRM %s on frame: %llu ctx: %u link: "
			"0x%x last_buf_done_req: %lld rc: %d",
			__cam_isp_ctx_crm_trigger_point_to_string(trigger_type),
			ctx_isp->frame_id, ctx->ctx_id, ctx->link_hdl,
...
static int __cam_isp_ctx_notify_v4l2_error_event(uint32_t error_type,
						 uint32_t error_code,
						 uint64_t error_request_id,
						 struct cam_context *ctx)
{
	...
	CAM_DBG(CAM_ISP,
		"v4l2 error event [type: %u code: %u] for req: %llu in ctx: %u on "
		"link: 0x%x notified successfully",
		error_type, error_code, error_request_id, ctx->ctx_id,
		ctx->link_hdl);
	...
	if (rc)
		CAM_ERR(CAM_ISP,
			"Notifying v4l2 error [type: %u code: %u] failed for req id:%llu "
			"in ctx %u on link: 0x%x",
			error_type, error_code, error_request_id, ctx->ctx_id,
			ctx->link_hdl);
...
static int __cam_isp_ctx_notify_error_util(uint32_t trigger_type,
					   ...
					   struct cam_isp_context *ctx_isp)
{
	...
	if (error == CRM_KMD_ERR_BUBBLE)
		CAM_WARN(
			CAM_ISP,
			"Notify CRM about bubble req: %llu frame: %llu in ctx: %u on "
			"link: 0x%x",
			req_id, ctx_isp->frame_id, ctx->ctx_id, ctx->link_hdl);
	else
		CAM_ERR(CAM_ISP,
			"Notify CRM about fatal error: %u req: %llu frame: %llu in ctx: %u "
			"on link: 0x%x",
			error, req_id, ctx_isp->frame_id, ctx->ctx_id,
			ctx->link_hdl);
...
static int __cam_isp_ctx_trigger_reg_dump(enum cam_hw_mgr_command cmd,
					  struct cam_context *ctx)
{
	int rc = 0;
	struct cam_hw_cmd_args hw_cmd_args;
	...
	CAM_DBG(CAM_ISP,
		"Reg dump type: %u successful in ctx: %u on link: 0x%x", cmd,
		ctx->ctx_id, ctx->link_hdl);
...
static int __cam_isp_ctx_pause_crm_timer(struct cam_context *ctx)
{
	int rc = -EINVAL;
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Failed to pause sof timer in ctx: %u on link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		goto end;
	}

	CAM_DBG(CAM_ISP,
		"Notify CRM to pause timer for ctx: %u link: 0x%x success",
		ctx->ctx_id, ctx->link_hdl);
...
static int cam_isp_ctx_dump_req(struct cam_isp_ctx_req *req_isp,
				uintptr_t cpu_addr, size_t buf_len,
				size_t *offset, bool dump_to_buff)
{
	int i, rc = 0;
	size_t len = 0;
	uint32_t *buf_addr;
						  ...
						  bool lock)
{
	...
	ctx_isp = (struct cam_isp_context *)ctx->ctx_priv;
	__cam_isp_ctx_update_event_record(ctx_isp, CAM_ISP_CTX_EVENT_SUBMIT,
					  req);
	if (lock)
					      ...
					      struct cam_ctx_request *req)
{
	...
	if (list_empty(&ctx->pending_req_list)) {
		list_add_tail(&req->list, &ctx->pending_req_list);
		CAM_DBG(CAM_ISP,
			"INIT packet added req id= %d, ctx_idx: %u, link: 0x%x",
	...
	if (req_isp_old->hw_update_data.packet_opcode_type ==
		...
		if (!rc) {
			...
			list_splice_init(&req->buf_tracker,
					 &req_old->buf_tracker);
		...
		CAM_WARN(
			CAM_ISP,
			"Received Update pkt before INIT pkt. req_id= %lld, ctx_idx: %u, "
			"link: 0x%x",
			req->request_id, ctx->ctx_id, ctx->link_hdl);
		rc = -EINVAL;
	}
...
static char *
__cam_isp_ife_sfe_resource_handle_id_to_type(uint32_t resource_handle)
{
...
static const char *
__cam_isp_tfe_resource_handle_id_to_type(uint32_t resource_handle)
{
	switch (resource_handle) {
	/* TFE output ports */
...
static const char *
__cam_isp_resource_handle_id_to_type(uint32_t device_type,
				     uint32_t resource_handle)
{
	switch (device_type) {
	case CAM_IFE_DEVICE_TYPE:
	case CAM_TFE_MC_DEVICE_TYPE:
		return __cam_isp_ife_sfe_resource_handle_id_to_type(
			resource_handle);
	case CAM_TFE_DEVICE_TYPE:
		return __cam_isp_tfe_resource_handle_id_to_type(
			resource_handle);
	default:
...
static int __cam_isp_ctx_get_hw_timestamp(struct cam_context *ctx,
					  uint64_t *prev_ts, uint64_t *curr_ts,
					  uint64_t *boot_ts)
{
	struct cam_hw_cmd_args hw_cmd_args;
	struct cam_isp_hw_cmd_args isp_hw_cmd_args;
						...
						uint64_t *last_cdm_done_req)
{
	...
	*last_cdm_done_req = isp_hw_cmd_args.u.last_cdm_done;
	...
	CAM_DBG(CAM_ISP,
		...
		last_cdm_done_req, ctx->ctx_id, ctx->link_hdl, ts.tm_mon + 1,
					       ...
					       uint64_t request_id)
{
	...
	/**
   * If the last received SOF was for frame A and we have missed the SOF for
   * frame B, then we need to find out if the hardware is at frame B or C.
   *   +-----+-----+-----+
   *   |  A  |  B  |  C  |
   *   +-----+-----+-----+
   */
	...
	if (a == prev_ts) {
		...
		b = curr_ts;
		CAM_DBG(CAM_ISP,
			"ctx:%u link:0x%x recover time(last:0x%llx,curr:0x%llx)req:%llu",
		...
		CAM_DBG(CAM_ISP,
			"ctx:%u link:0x%x recover "
			"time(last:0x%llx,prev:0x%llx,curr:0x%llx)req:%llu",
			ctx->ctx_id, ctx->link_hdl, a, b, c, request_id);
	} else {
		/* Hardware is at frame A (which we supposedly missed) */
		CAM_ERR_RATE_LIMIT(
			CAM_ISP,
			"ctx:%u link: 0x%x erroneous call to SOF recovery(last:0x%llx, "
			"prev:0x%llx, curr:0x%llx) req: %llu",
			ctx->ctx_id, ctx->link_hdl, a, prev_ts, curr_ts,
			request_id);
		return 0;
	}
...
static void
__cam_isp_ctx_send_unified_timestamp(struct cam_isp_context *ctx_isp,
				     uint64_t request_id)
{
	struct cam_req_mgr_message req_msg;
	...
	req_msg.u.frame_msg_v2.timestamps[CAM_REQ_BOOT_TIMESTAMP] =
	...
	CAM_DBG(CAM_ISP,
		...
		boot time stamp:0x%llx",
		ctx_isp->base->link_hdl, request_id, ctx_isp->frame_id,
		ctx_isp->sof_timestamp_val, ctx_isp->base->ctx_id,
		ctx_isp->boot_timestamp);

	if (cam_req_mgr_notify_message(&req_msg,
				       V4L_EVENT_CAM_REQ_MGR_SOF_UNIFIED_TS,
				       V4L_EVENT_CAM_REQ_MGR_EVENT))
		CAM_ERR(CAM_ISP,
	...
	uint64_t request_id, uint32_t sof_event_status)
{
	...
	timestamp = mul_u64_u32_div(timestamp, CAM_IFE_QTIMER_MUL_FACTOR,
					     ...
					     uint32_t sof_event_status)
{
	...
	if (request_id == 0 &&
		...
		CAM_WARN_RATE_LIMIT(
			CAM_ISP,
			"Missed SOF Recovery for invalid req, Skip notificaiton to userspace "
			"Ctx: %u link: 0x%x frame_id %u",
	...
	if (frame_interval_para > 1) {
		...
		CAM_DBG(MI_PERF,
			"ERROR, the frame interval num must greater than 1");
	}
	/*xiaomi added detect framerate end*/

	if ((ctx_isp->v4l2_event_sub_ids &
	     (1 << V4L_EVENT_CAM_REQ_MGR_SOF_UNIFIED_TS)) &&
	...
	CAM_DBG(CAM_ISP,
		...
		"ctx_idx: %u, link: 0x%x",
		request_id, ctx_isp->frame_id, ctx_isp->sof_timestamp_val,
		sof_event_status, ctx->ctx_id, ctx->link_hdl);

	if (cam_req_mgr_notify_message(&req_msg, V4L_EVENT_CAM_REQ_MGR_SOF,
				       V4L_EVENT_CAM_REQ_MGR_EVENT))
		CAM_ERR(CAM_ISP,
			"Error in notifying the sof time for req id:%lld, ctx_idx: %u, "
			"link: 0x%x",
...
static void
__cam_isp_ctx_handle_buf_done_fail_log(struct cam_isp_context *ctx_isp,
				       uint64_t request_id,
				       struct cam_isp_ctx_req *req_isp)
{
	int i;
	...
	CAM_WARN_RATE_LIMIT(
		CAM_ISP,
		"Prev Req[%lld] : num_out=%d, num_acked=%d, bubble : "
		"report=%d, detected=%d",
	...
	for (i = 0; i < req_isp->num_fence_map_out; i++) {
		if (req_isp->fence_map_out[i].sync_id != -1) {
			...
			trace_cam_log_event("Buf_done Congestion", handle_type,
					    request_id,
...
static int
__cam_isp_context_try_internal_recovery(struct cam_isp_context *ctx_isp)
{
	int rc = 0;
	struct cam_context *ctx = ctx_isp->base;
	struct cam_ctx_request *req;
   ...
   */
	if (!list_empty(&ctx->wait_req_list)) {
		...
		if (req->request_id == ctx_isp->recovery_req_id) {
			rc = __cam_isp_ctx_notify_error_util(
				CAM_TRIGGER_POINT_SOF, CRM_KMD_ERR_BUBBLE,
				ctx_isp->recovery_req_id, ctx_isp);
			if (rc) {
				/* Unable to do bubble recovery reset back to normal */
				CAM_WARN(
					CAM_ISP,
					"Unable to perform internal recovery [bubble reporting "
					"failed] for req: %llu in ctx: %u on link: 0x%x",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl);
				__cam_isp_context_reset_internal_recovery_params(
					ctx_isp);
			...
			CAM_INFO(
				...
				ctx_isp->recovery_req_id, ctx->ctx_id,
				ctx->link_hdl);
			goto end;
		}
	}
...
static int
__cam_isp_ctx_handle_buf_done_for_req_list(struct cam_isp_context *ctx_isp,
					   struct cam_ctx_request *req)
{
	int rc = 0, i;
	...
	if (req_isp->bubble_detected && req_isp->bubble_report) {
		...
		if (buf_done_req_id <= ctx->last_flush_req) {
			...
			list_add_tail(&req->list, &ctx->free_req_list);
			CAM_DBG(CAM_REQ,
				"Move active request %lld to free list(cnt = %d) [flushed], ctx "
				"%u, link: 0x%x",
			...
			list_add(&req->list, &ctx->pending_req_list);
			CAM_DBG(CAM_REQ,
				"Move active request %lld to pending list(cnt = %d) [bubble "
				"recovery], ctx %u, link: 0x%x",
     ...
     */
		...
		CAM_DBG(CAM_REQ,
			"Move active request %lld to free list(cnt = %d) [all fences "
			"done], ctx %u link: 0x%x",
	...
	__cam_isp_ctx_update_event_record(ctx_isp, CAM_ISP_CTX_EVENT_BUFDONE,
					  req);
	return rc;
}
	...
	struct cam_isp_hw_done_event_data *done_next_req)
{
	...
	CAM_DBG(CAM_ISP,
		...
		bubble_state, req_isp->bubble_detected, ctx->ctx_id,
		ctx->link_hdl);
     ...
     */
		CAM_WARN(CAM_ISP, "BUF_DONE for res %s not found in Req %lld ",
	...
	for (i = 0; i < comp_grp->num_res; i++) {
       ...
       */
		...
		if (req_isp->fence_map_out[j].sync_id == -1) {
			...
			CAM_WARN(
				CAM_ISP,
				"Duplicate BUF_DONE for req %lld : i=%d, j=%d, res=%s, ctx %u "
				"link: 0x%x",
				req->request_id, i, j, handle_type, ctx->ctx_id,
				ctx->link_hdl);

			trace_cam_log_event("Duplicate BufDone", handle_type,
					    req->request_id, ctx->ctx_id);
			continue;
		}
		...
		if (cam_presil_mode_enabled()) {
			rc = cam_presil_retrieve_buffers_from_packet(
				req_isp->hw_update_data.packet,
				ctx->img_iommu_hdl,
				req_isp->fence_map_out[j].resource_handle);
			if (rc) {
				CAM_ERR(CAM_ISP,
					"Failed to retrieve image buffers req_id:%d ctx_id:%u link: "
					"0x%x bubble detected:%d rc:%d",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl, req_isp->bubble_detected,
					rc);
				return rc;
			}
		}
       ...
       */
		...
		CAM_DBG(CAM_ISP,
			"req %lld, reset sync id 0x%x ctx %u link: 0x%x",
...
static int __cam_isp_handle_deferred_buf_done(struct cam_isp_context *ctx_isp,
					      struct cam_ctx_request *req,
					      ...
					      uint32_t event_cause)
{
	...
	CAM_DBG(CAM_ISP,
		"ctx[%u] link[0x%x] : Req %llu : Handling %d deferred buf_dones "
		"num_acked=%d, bubble_handling=%d",
		ctx->ctx_id, ctx->link_hdl, req->request_id,
		req_isp->num_deferred_acks, req_isp->num_acked,
		bubble_handling);
	...
	for (i = 0; i < req_isp->num_deferred_acks; i++) {
		...
		CAM_DBG(CAM_ISP,
			"ctx[%u] link[0x%x] : Sync with status=%d, event_cause=%d: req "
			"%lld res 0x%x sync_id 0x%x",
		...
		if (req_isp->fence_map_out[j].sync_id == -1) {
			CAM_WARN(
				CAM_ISP,
				"ctx[%u] link[0x%x] :  Deferred buf_done already signalled, "
				"req_id=%llu, j=%d, res=0x%x",
		...
		if (!bubble_handling) {
			CAM_WARN(
				CAM_ISP,
				"Unexpected Buf done for res=0x%x on ctx[%u] link[0x%x] for Req "
				"%llu, status=%d, possible bh delays",
				req_isp->fence_map_out[j].resource_handle,
				ctx->ctx_id, ctx->link_hdl, req->request_id,
				status);
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"ctx[%u] link[0x%x] : Sync signal for Req %llu, sync_id %d "
					"status=%d failed with rc = %d",
					ctx->ctx_id, ctx->link_hdl,
					req->request_id,
					req_isp->fence_map_out[j].sync_id,
					status, rc);
	...
	CAM_DBG(CAM_ISP,
		"ctx[%u] link[0x%x] : Req %llu : Handled %d deferred buf_dones "
		"num_acked=%d, num_fence_map_out=%d",
		ctx->ctx_id, ctx->link_hdl, req->request_id,
		req_isp->num_deferred_acks, req_isp->num_acked,
		req_isp->num_fence_map_out);
	...
	struct cam_isp_context *ctx_isp, struct cam_ctx_request *req)
{
	...
	if (req_isp->num_deferred_acks)
		rc = __cam_isp_handle_deferred_buf_done(
			ctx_isp, req, req_isp->bubble_report,
			CAM_SYNC_STATE_SIGNALED_ERROR,
			CAM_SYNC_ISP_EVENT_BUBBLE);
...
static int __cam_isp_ctx_handle_buf_done_for_request_verify_addr(
	struct cam_isp_context *ctx_isp, struct cam_ctx_request *req,
	struct cam_isp_hw_done_event_data *done, uint32_t bubble_state,
	bool verify_consumed_addr, bool defer_buf_done)
{
	int rc = 0;
	int i, j, k, def_idx;
	bool not_found = false;
	...
	if (i == req_isp->num_fence_map_out) {
		for (j = 0; j < comp_grp->num_res; j++) {
			...
			if ((k == req_isp->num_fence_map_out) &&
				...
				cmp_addr =
					cam_smmu_is_expanded_memory() ?
						CAM_36BIT_INTF_GET_IOVA_BASE(
							req_isp->fence_map_out[k]
								.image_buf_addr
									[0]) :
						req_isp->fence_map_out[k]
							.image_buf_addr[0];
	...
	for (i = 0; i < comp_grp->num_res; i++) {
       ...
       */
		...
		if (cam_presil_mode_enabled()) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					"Failed to retrieve image buffers req_id:%d ctx_id:%u "
					"link[0x%x] bubble detected:%d rc:%d",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl, req_isp->bubble_detected,
					rc);
				return rc;
		...
		if (defer_buf_done) {
			...
			for (k = 0; k < req_isp->num_deferred_acks; k++) {
				...
				if (def_idx == j) {
					CAM_WARN(
						CAM_ISP,
						"duplicate deferred ack for ctx[%u] link[0x%x] req %lld res "
						"0x%x sync_id 0x%x",
						ctx->ctx_id, ctx->link_hdl,
						req->request_id,
			...
			if (req_isp->num_deferred_acks ==
				...
				CAM_WARN(
					CAM_ISP,
					"WARNING: req_id %lld num_deferred_acks %d > map_out %d, "
					"ctx_idx:%u link[0x%x]",
					req->request_id,
			...
			if (deferred_indx >= CAM_ISP_CTX_RES_MAX) {
				CAM_WARN(
					CAM_ISP,
					"ctx: %u req: %llu number of Deferred buf done %u is more than %u",
       ...
       */
			...
			req_isp->num_deferred_acks++;
			CAM_DBG(CAM_ISP,
				"ctx[%u] link[0x%x]:Deferred buf done for %llu with bubble state "
				"%d recovery %d",
				ctx->ctx_id, ctx->link_hdl, req->request_id,
				bubble_state, req_isp->bubble_report);
			CAM_DBG(CAM_ISP,
				"ctx[%u] link[0x%x]:Deferred "
				"info:num_acks=%d,fence_map_index=%d,resource_handle=0x%x,sync_id=%d",
				ctx->ctx_id, ctx->link_hdl,
				req_isp->num_deferred_acks, j,
				req_isp->fence_map_out[j].resource_handle,
				req_isp->fence_map_out[j].sync_id);
			continue;
       ...
       */
			req_isp->num_acked++;
			CAM_DBG(CAM_ISP,
				"buf done with bubble state %d recovery %d for req %lld, "
				"ctx_idx:%u link[0x%x]",
				bubble_state, req_isp->bubble_report,
				req->request_id, ctx->ctx_id, ctx->link_hdl);
			...
			if (req_isp->num_acked == req_isp->num_fence_map_out) {
				rc = __cam_isp_ctx_handle_buf_done_for_req_list(
					ctx_isp, req);
				if (rc)
					CAM_ERR(CAM_ISP,
						"Error in buf done for req = %llu with rc = %d, ctx_idx:%u "
						"link[0x%x]",
						req->request_id, rc,
						ctx->ctx_id, ctx->link_hdl);
				return rc;
...
check_deferred:
	if ((unhandled_done.resource_handle > 0) && (!defer_buf_done))
		__cam_isp_ctx_check_deferred_buf_done(ctx_isp, &unhandled_done,
						      bubble_state);
...
static int
__cam_isp_ctx_handle_buf_done(struct cam_isp_context *ctx_isp,
			      struct cam_isp_hw_done_event_data *done,
			      uint32_t bubble_state)
{
	int rc = 0;
	...
	if (done_next_req.resource_handle) {
       ...
       */
			...
			CAM_WARN(
				CAM_ISP,
				"Unhandled bufdone resources for req %lld,trying next request "
				"%lld,ctx:%u link[0x%x]",
			...
			if (unhandled_res.resource_handle == 0)
				CAM_INFO(
					CAM_ISP,
					"BUF Done event handed for next request %lld, ctx_idx:%u "
					"link[0x%x]",
					next_req->request_id, ctx->ctx_id,
					ctx->link_hdl);
			else
				CAM_ERR(CAM_ISP,
...
static void
__cam_isp_ctx_buf_done_match_req(struct cam_ctx_request *req,
				 struct cam_isp_hw_done_event_data *done,
				 bool *irq_delay_detected)
{
	...
	CAM_DBG(CAM_ISP, "irq_delay_detected %d", *irq_delay_detected);
}
	...
	struct cam_ctx_request *deferred_req)
{
	...
	curr_active_req = list_first_entry(&ctx->active_req_list,
					   struct cam_ctx_request, list);
	curr_active_isp_req =
		(struct cam_isp_ctx_req *)curr_active_req->req_priv;
	deferred_isp_req = (struct cam_isp_ctx_req *)deferred_req->req_priv;

	/* Check from newly updated deferred acks */
	for (i = deferred_ack_start_idx;
	     i < deferred_isp_req->num_deferred_acks; i++) {
		deferred_map_idx =
			deferred_isp_req->deferred_fence_map_index[i];

		for (j = 0; j < curr_active_isp_req->num_fence_map_out; j++) {
			/* resource needs to match */
			if (curr_active_isp_req->fence_map_out[j]
				    .resource_handle !=
			    deferred_isp_req->fence_map_out[deferred_map_idx]
				    .resource_handle)
				continue;
			...
			CAM_WARN(
				CAM_ISP,
				"Processing delayed buf done req: %llu bubble_detected: %s res: "
				"0x%x fd: 0x%x, ctx: %u link: 0x%x [deferred req: %llu last "
				"applied: %llu]",
				curr_active_req->request_id,
				CAM_BOOL_TO_YESNO(
					curr_active_isp_req->bubble_detected),
				curr_active_isp_req->fence_map_out[j]
					.resource_handle,
				curr_active_isp_req->fence_map_out[j].sync_id,
				ctx->ctx_id, ctx->link_hdl,
				deferred_req->request_id,
				      ...
				      uint32_t bubble_state)
{
	...
	if (!list_empty(&ctx->wait_req_list)) {
		...
		if (ctx_isp->last_applied_req_id !=
		    ctx_isp->last_bufdone_err_apply_req_id) {
			CAM_DBG(CAM_ISP,
				"Trying to find buf done with req in wait list, req %llu last apply "
				"id:%lld last err id:%lld curr_num_deferred: %u, ctx: %u link: 0x%x",
				req->request_id, ctx_isp->last_applied_req_id,
				ctx_isp->last_bufdone_err_apply_req_id,
				curr_num_deferred, ctx->ctx_id, ctx->link_hdl);
			ctx_isp->last_bufdone_err_apply_req_id =
				ctx_isp->last_applied_req_id;
		}
     ...
     */
		...
		if (ctx_isp->last_applied_req_id !=
		    ctx_isp->last_bufdone_err_apply_req_id) {
			CAM_DBG(CAM_ISP,
				"Trying to find buf done with req in pending list, req %llu last "
				"apply id:%lld last err id:%lld curr_num_deferred: %u, ctx: %u "
				"link: 0x%x",
				req->request_id, ctx_isp->last_applied_req_id,
				ctx_isp->last_bufdone_err_apply_req_id,
				curr_num_deferred, ctx->ctx_id, ctx->link_hdl);
	...
	if (!req_in_pending_wait_list &&
	    (ctx_isp->last_applied_req_id !=
	     ctx_isp->last_bufdone_err_apply_req_id)) {
		CAM_DBG(CAM_ISP,
			"Bufdone without active request bubble_state=%d "
			"last_applied_req_id:%lld,ctx:%u link:0x%x",
			bubble_state, ctx_isp->last_applied_req_id, ctx->ctx_id,
			ctx->link_hdl);
		ctx_isp->last_bufdone_err_apply_req_id =
			ctx_isp->last_applied_req_id;
	}
	...
	struct cam_isp_hw_done_event_data *done, uint32_t bubble_state)
{
	...
	if (list_empty(&ctx->active_req_list)) {
		return __cam_isp_ctx_check_deferred_buf_done(ctx_isp, done,
							     bubble_state);
	}
	...
	if (ctx_isp->active_req_cnt > 1) {
		...
		if (next_req->request_id != req->request_id)
			...
			CAM_WARN(
				CAM_ISP,
				"Req %lld only active request, spurious buf_done rxd, ctx: %u "
				"link: 0x%x",
				req->request_id, ctx->ctx_id, ctx->link_hdl);
	}
   ...
   */
	rc = __cam_isp_ctx_handle_buf_done_for_request_verify_addr(
		ctx_isp, req, done, bubble_state, !irq_delay_detected, false);
   ...
   */
	if (!rc && irq_delay_detected)
		rc = __cam_isp_ctx_handle_buf_done_for_request_verify_addr(
			ctx_isp, next_req, done, bubble_state, true, false);
	...
	struct cam_isp_hw_done_event_data *done, uint32_t bubble_state)
{
	...
	if (ctx_isp->support_consumed_addr)
		rc = __cam_isp_ctx_handle_buf_done_verify_addr(ctx_isp, done,
							       bubble_state);
	else
		rc = __cam_isp_ctx_handle_buf_done(ctx_isp, done, bubble_state);

	return rc;
}

static int __cam_isp_ctx_apply_pending_req(void *priv, void *data)
{
	int rc = 0;
   ...
   */
	...
	if (rc) {
		...
		CAM_DBG(CAM_ISP,
			...
			CAM_ISP_CTX_ACTIVATED_APPLIED,
			ctx_isp->last_applied_req_id, ctx->ctx_id,
			ctx->link_hdl);
...
static int __cam_isp_ctx_schedule_apply_req(struct cam_isp_context *ctx_isp)
{
	int rc = 0;
...
static int
__cam_isp_ctx_offline_epoch_in_activated_state(struct cam_isp_context *ctx_isp,
					       void *evt_data)
{
	struct cam_context *ctx = ctx_isp->base;
	struct cam_ctx_request *req, *req_temp;
   ...
   */
	...
	__cam_isp_ctx_update_state_monitor_array(
		ctx_isp, CAM_ISP_STATE_CHANGE_TRIGGER_EPOCH, request_id);
					    ...
					    void *evt_data)
{
	if (ctx_isp->frame_id == 1)
		...
		CAM_WARN_RATE_LIMIT(
			CAM_ISP,
			"ctx:%u Unexpected regupdate in activated Substate[%s] for "
			"frame_id:%lld",
			ctx_isp->base->ctx_id,
			__cam_isp_ctx_substate_val_to_type(
				ctx_isp->substate_activated),
					    ...
					    void *evt_data)
{
	...
	if (atomic_read(&ctx_isp->process_bubble)) {
		...
		if (ctx_isp->last_sof_timestamp == ctx_isp->sof_timestamp_val) {
			CAM_DBG(CAM_ISP,
				"Tasklet delay detected! Bubble frame check skipped, "
				"sof_timestamp: %lld, ctx %u, link: 0x%x",
				ctx_isp->sof_timestamp_val, ctx->ctx_id,
				ctx->link_hdl);
			goto notify_only;
		}
		...
		if (ctx_isp->bubble_frame_cnt >= 1 &&
			...
			if (last_cdm_done_req >= req->request_id) {
				CAM_DBG(CAM_ISP,
					"invalid sof event data CDM cb detected for req: %lld, possible "
					"buf_done delay, waiting for buf_done, ctx %u, link: 0x%x",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl);
				ctx_isp->bubble_frame_cnt = 0;
			} else {
				CAM_DBG(CAM_ISP,
					"CDM callback not happened for req: %lld, possible CDM stuck "
					"or workqueue delay, ctx %u, link: 0x%x",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl);
				req_isp->num_acked = 0;
				req_isp->num_deferred_acks = 0;
				...
				ctx_isp->active_req_cnt--;
				CAM_DBG(CAM_REQ,
					"Move active req: %lld to pending list(cnt = %d) [bubble "
					"re-apply], ctx %u link: 0x%x",
					req->request_id,
			...
			CAM_DBG(CAM_ISP,
				"Waiting on bufdone for bubble req: %lld, since frame_cnt = "
				"%lld, ctx %u link: 0x%x",
		...
		list_for_each_entry(req, &ctx->active_req_list, list) {
			...
			if ((!req_isp->bubble_detected) &&
				...
				__cam_isp_ctx_update_event_record(
					ctx_isp, CAM_ISP_CTX_EVENT_EPOCH, req);
				break;
			}
		}
					    ...
					    void *evt_data)
{
	...
	if (rc)
		CAM_ERR(CAM_ISP,
			...
			ctx->ctx_id, ctx->link_hdl);
	__cam_isp_ctx_update_state_monitor_array(
		ctx_isp, CAM_ISP_STATE_CHANGE_TRIGGER_CDM_DONE,
		last_cdm_done_req);
						...
						void *evt_data)
{
   ...
   */
	...
	CAM_DBG(CAM_ISP,
		"frame id: %lld time stamp:0x%llx, ctx %u request %llu, link: 0x%x",
		ctx_isp->frame_id, ctx_isp->sof_timestamp_val, ctx->ctx_id,
		request_id, ctx->link_hdl);
					  ...
					  void *evt_data)
{
     ...
     */
		CAM_ERR(CAM_ISP, "Ctx:%u link: 0x%x No wait request",
			ctx->ctx_id, ctx->link_hdl);
		ctx_isp->substate_activated = CAM_ISP_CTX_ACTIVATED_SOF;
		...
		__cam_isp_ctx_send_sof_timestamp(ctx_isp, request_id,
						 CAM_REQ_MGR_SOF_EVENT_SUCCESS);
		__cam_isp_ctx_update_event_record(
			ctx_isp, CAM_ISP_CTX_EVENT_EPOCH, NULL);
		goto end;
	}
	...
	CAM_INFO_RATE_LIMIT(
		CAM_ISP, "ctx:%u link: 0x%x Report Bubble flag %d req id:%lld",
		ctx->ctx_id, ctx->link_hdl, req_isp->bubble_report,
		req->request_id);

	if (req_isp->bubble_report) {
		__cam_isp_ctx_notify_error_util(CAM_TRIGGER_POINT_SOF,
						CRM_KMD_ERR_BUBBLE,
   ...
   * other invalid req.
   */
	__cam_isp_ctx_update_event_record(ctx_isp, CAM_ISP_CTX_EVENT_EPOCH,
					  req);
   ...
   */
	...
	__cam_isp_ctx_send_sof_timestamp(ctx_isp, request_id, sof_event_status);

	cam_req_mgr_debug_delay_detect();
	trace_cam_delay_detect("ISP", "bubble epoch_in_applied",
			       req->request_id, ctx->ctx_id, ctx->link_hdl,
			       ctx->session_hdl, CAM_DEFAULT_VALUE);
end:
	if (request_id == 0) {
				      ...
				      void *evt_data)
{
   ...
   */
	...
	// xiaomi add
	__cam_isp_notify_mqs_event(V4L_EVENT_CAM_MQS_ISP,
				   V4L_EVENT_CAM_MQS_BUBBLE, req->request_id,
				   ctx);
	// xiaomi add
...
static void __cam_isp_get_notification_evt_params(uint32_t hw_error,
						  uint32_t *fence_evt_cause,
...
static int
__cam_isp_ctx_validate_for_req_reapply_util(struct cam_isp_context *ctx_isp)
{
	int rc = 0;
	...
	if (!list_empty(&ctx->active_req_list)) {
       ...
       */
			...
			if (!__cam_isp_ctx_request_can_reapply(req_isp)) {
				CAM_WARN(
					CAM_ISP,
					"Req: %llu in ctx:%u on link: 0x%x fence has partially "
					"signaled, cannot do recovery",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl);
				rc = -EINVAL;
				goto end;
...
static int
__cam_isp_ctx_handle_recovery_req_util(struct cam_isp_context *ctx_isp)
{
	int rc = 0;
	struct cam_context *ctx = ctx_isp->base;
...
static int
__cam_isp_ctx_trigger_error_req_reapply(uint32_t err_type,
					struct cam_isp_context *ctx_isp)
{
	int rc = 0;
	struct cam_context *ctx = ctx_isp->base;
				      ...
				      void *evt_data)
{
	...
	if (error_event_data->try_internal_recovery) {
		rc = __cam_isp_ctx_trigger_error_req_reapply(
			error_event_data->error_type, ctx_isp);
		if (!rc)
			goto exit;
	}
   ...
   */
	if (error == CRM_KMD_ERR_FATAL)
		__cam_isp_ctx_notify_v4l2_error_event(
			recovery_type, req_mgr_err_code, error_request_id, ctx);

	ctx_isp->substate_activated = CAM_ISP_CTX_ACTIVATED_HW_ERROR;
	CAM_DBG(CAM_ISP, "Handling error done on ctx: %u, link: 0x%x",
		ctx->ctx_id, ctx->link_hdl);
...
static void
__cam_isp_ctx_notify_aeb_error_for_sec_event(struct cam_isp_context *ctx_isp)
{
	struct cam_context *ctx = ctx_isp->base;
	...
	if ((++ctx_isp->aeb_error_cnt) <= CAM_ISP_CONTEXT_AEB_ERROR_CNT_MAX) {
		CAM_WARN(
			CAM_ISP,
			"AEB slave RDI's current request's SOF seen after next req is applied "
			"for ctx: %u on link: 0x%x last_applied_req: %llu err_cnt: %u",
			ctx->ctx_id, ctx->link_hdl,
			ctx_isp->last_applied_req_id, ctx_isp->aeb_error_cnt);
		return;
	}

	CAM_ERR(CAM_ISP,
		"Fatal - AEB slave RDI's current request's SOF seen after next req "
		"is applied, EPOCH height need to be re-configured for ctx: %u on "
		"link: 0x%x err_cnt: %u",
	...
	__cam_isp_ctx_notify_v4l2_error_event(
		...
		ctx_isp->last_applied_req_id, ctx);
...
static int
__cam_isp_ctx_trigger_internal_recovery(bool sync_frame_drop,
					struct cam_isp_context *ctx_isp)
{
	int rc = 0;
	...
	if (!do_recovery) {
		CAM_WARN(
			CAM_ISP,
			"No request to perform recovery - ctx: %u on link: 0x%x "
			"last_applied: %lld last_buf_done: %lld",
			ctx->ctx_id, ctx->link_hdl,
			ctx_isp->last_applied_req_id,
			ctx_isp->req_info.last_bufdone_req_id);
		goto end;
	...
	if (!req) {
		...
		if (req->request_id != ctx_isp->last_applied_req_id)
			CAM_WARN(
				CAM_ISP,
				"Top of wait list req: %llu does not match with last applied: "
				"%llu in ctx: %u on link: 0x%x",
				req->request_id, ctx_isp->last_applied_req_id,
				ctx->ctx_id, ctx->link_hdl);
	}
   ...
   */
	...
	if (ctx_isp->active_req_cnt) {
		...
		CAM_WARN(
			CAM_ISP,
			"Active req cnt: %u wait for all buf dones before kicking in "
			"recovery on req: %lld ctx: %u on link: 0x%x",
			ctx_isp->active_req_cnt, ctx_isp->recovery_req_id,
			ctx->ctx_id, ctx->link_hdl);
	} else {
		rc = __cam_isp_ctx_notify_error_util(CAM_TRIGGER_POINT_SOF,
						     CRM_KMD_ERR_BUBBLE,
						     ctx_isp->recovery_req_id,
						     ctx_isp);
		if (rc) {
			/* Unable to do bubble recovery reset back to normal */
			CAM_WARN(
				CAM_ISP,
				"Unable to perform internal recovery [bubble reporting failed] "
				"for req: %llu in ctx: %u on link: 0x%x",
				ctx_isp->recovery_req_id, ctx->ctx_id,
				ctx->link_hdl);
			__cam_isp_context_reset_internal_recovery_params(
				ctx_isp);
...
static int
__cam_isp_ctx_handle_secondary_events(struct cam_isp_context *ctx_isp,
				      void *evt_data)
{
	int rc = 0;
	...
	if (!ctx_isp->aeb_enabled) {
		CAM_WARN(
			CAM_ISP,
			"Recovery not supported for non-AEB ctx: %u on link: 0x%x reject "
			"sec evt: %u",
	...
	if (atomic_read(&ctx_isp->internal_recovery_set)) {
		CAM_WARN(
			CAM_ISP,
			"Internal recovery in progress in ctx: %u on link: 0x%x reject "
			"sec evt: %u",
			ctx->ctx_id, ctx->link_hdl, sec_evt_data->evt_type);
		goto end;
	}
   ...
   */
	...
	case CAM_ISP_HW_SEC_EVENT_SOF:
		__cam_isp_ctx_update_state_monitor_array(
			ctx_isp, CAM_ISP_STATE_CHANGE_TRIGGER_SEC_EVT_SOF,
			ctx_isp->last_applied_req_id);

		__cam_isp_ctx_update_frame_timing_record(
			CAM_ISP_HW_SECONDARY_EVENT, ctx_isp);
	...
	case CAM_ISP_HW_SEC_EVENT_EPOCH:
		__cam_isp_ctx_update_state_monitor_array(
			ctx_isp, CAM_ISP_STATE_CHANGE_TRIGGER_SEC_EVT_EPOCH,
			ctx_isp->last_applied_req_id);
     ...
     */
		if ((ctx_isp->substate_activated ==
			...
			CAM_WARN(
				CAM_ISP,
				"Programming delay input frame dropped ctx: %u on link: 0x%x "
				"last_applied_req: %llu, kicking in internal recovery....",
				ctx->ctx_id, ctx->link_hdl,
				ctx_isp->last_applied_req_id);
		}
		break;
	case CAM_ISP_HW_SEC_EVENT_OUT_OF_SYNC_FRAME_DROP:
		__cam_isp_ctx_update_state_monitor_array(
			ctx_isp, CAM_ISP_STATE_CHANGE_TRIGGER_FRAME_DROP,
			ctx_isp->last_applied_req_id);

		/* Avoid recovery loop if frame is dropped at stream on */
		if (!ctx_isp->frame_id) {
			CAM_ERR(CAM_ISP,
				"Sensor sync [vc mismatch] frame dropped at stream on ctx: %u "
				"link: 0x%x frame_id: %u last_applied_req: %lld",
				ctx->ctx_id, ctx->link_hdl, ctx_isp->frame_id,
				ctx_isp->last_applied_req_id);
			rc = -EPERM;
			break;
		...
		if (!(ctx_isp->out_of_sync_cnt++) &&
			...
			CAM_WARN(
				CAM_ISP,
				"Sensor sync [vc mismatch] frame dropped ctx: %u on link: 0x%x "
				"last_applied_req: %llu last_recovered_req: %llu "
				"out_of_sync_cnt: %u, recovery maybe in progress...",
				ctx->ctx_id, ctx->link_hdl,
				ctx_isp->last_applied_req_id,
				ctx_isp->recovery_req_id,
		...
		CAM_WARN(
			CAM_ISP,
			"Sensor sync [vc mismatch] frame dropped ctx: %u on link: 0x%x "
			"last_applied_req: %llu last_recovered_req: %llu out_of_sync_cnt: "
			"%u, kicking in internal recovery....",
			ctx->ctx_id, ctx->link_hdl,
			ctx_isp->last_applied_req_id, ctx_isp->recovery_req_id,
			ctx_isp->out_of_sync_cnt);
		break;
	default:
		break;
	...
	if (recover && !(isp_ctx_debug.disable_internal_recovery_mask &
			 CAM_ISP_CTX_DISABLE_RECOVERY_AEB))
		rc = __cam_isp_ctx_trigger_internal_recovery(sync_frame_drop,
							     ctx_isp);
	...
	enum cam_isp_ctx_activated_substate next_state)
{
	...
	if (apply->re_apply)
		if (apply->request_id <= ctx_isp->last_applied_req_id) {
			CAM_INFO_RATE_LIMIT(
				...
				ctx->ctx_id, ctx->link_hdl, apply->request_id);
			return 0;
		}
   ...
   */
	...
	if (ctx_isp->active_req_cnt >= 2) {
		CAM_WARN_RATE_LIMIT(
			CAM_ISP,
			"Reject apply request (id %lld) due to congestion(cnt "
			"= %d) ctx %u, link: 0x%x",
			req->request_id, ctx_isp->active_req_cnt, ctx->ctx_id,
			ctx->link_hdl);
		...
		if (!list_empty(&ctx->active_req_list))
			...
			CAM_ERR_RATE_LIMIT(
				CAM_ISP,
				"WARNING: should not happen (cnt = %d) but "
				"active_list empty, ctx %u, link: 0x%x",
				ctx_isp->active_req_cnt, ctx->ctx_id,
				ctx->link_hdl);
		spin_unlock_bh(&ctx->lock);
   ...
   */
	...
	if (!rc) {
		...
		CAM_ERR_RATE_LIMIT(
			CAM_ISP,
			"ctx_id:%u link: 0x%x,Can not apply (req %lld) the "
			"configuration, rc %d",
...
static void
__cam_isp_ctx_find_mup_for_default_settings(int64_t req_id,
					    struct cam_context *ctx,
					    struct cam_ctx_request **switch_req)
{
	...
	list_for_each_entry_safe(req, temp_req, &ctx->pending_req_list, list) {
	...
	struct cam_context *ctx, struct cam_req_mgr_apply_request *apply)
{
	...
	if (isp_ctx->mode_switch_en && isp_ctx->handle_mswitch) {
		if ((apply->last_applied_max_pd_req > 0) &&
		    (atomic_dec_and_test(
			    &isp_ctx->mswitch_default_apply_delay_ref_cnt))) {
		...
		if (req) {
			...
			cfg.cdm_reset_before_apply =
				req_isp->cdm_reset_before_apply;

			rc = ctx->hw_mgr_intf->hw_config(
	...
	if (isp_ctx->use_default_apply) {
		...
		hw_cmd_args.cmd_type = CAM_HW_MGR_CMD_INTERNAL;
		isp_hw_cmd_args.cmd_type = CAM_ISP_HW_MGR_CMD_PROG_DEFAULT_CFG;
...
static void *cam_isp_ctx_user_dump_req_list(void *dump_struct,
					    uint8_t *addr_ptr)
{
...
static void *cam_isp_ctx_user_dump_active_requests(void *dump_struct,
						   uint8_t *addr_ptr)
{
	uint64_t *addr;
	struct cam_ctx_request *req;
...
static int
__cam_isp_ctx_dump_req_info(struct cam_context *ctx,
			    struct cam_ctx_request *req,
			    struct cam_common_hw_dump_args *dump_args)
{
	int i, rc = 0;
	...
	if (!req || !ctx || !dump_args) {
		CAM_ERR(CAM_ISP, "Invalid parameters %pK %pK %pK", req, ctx,
			dump_args);
		return -EINVAL;
	}
	req_isp = (struct cam_isp_ctx_req *)req->req_priv;
	...
	if (dump_args->buf_len <= dump_args->offset) {
		CAM_WARN(
			...
			dump_args->buf_len, dump_args->offset, ctx->ctx_id,
			ctx->link_hdl);
		return -ENOSPC;
	}
	...
	min_len = sizeof(struct cam_isp_context_dump_header) +
		  (CAM_ISP_CTX_DUMP_REQUEST_NUM_WORDS *
		   req_isp->num_fence_map_out * sizeof(uint64_t));
	...
	rc = cam_common_user_dump_helper(dump_args,
					 cam_isp_ctx_user_dump_req_list,
					 &ctx->pending_req_list,
					 sizeof(uint64_t),
	...
	rc = cam_common_user_dump_helper(dump_args,
					 cam_isp_ctx_user_dump_req_list,
					 &ctx->wait_req_list, sizeof(uint64_t),
					 "ISP_OUT_FENCE_APPLIED_REQUESTS:");
	if (rc) {
		CAM_ERR(CAM_ISP,
			"CAM_ISP_CONTEXT: Applied request dump failed, rc:%d, ctx:%u, "
			"link:0x%x",
			rc, ctx->ctx_id, ctx->link_hdl);
		return rc;
	}
...
static void *cam_isp_ctx_user_dump_timer(void *dump_struct, uint8_t *addr_ptr)
{
	struct cam_ctx_request *req = NULL;
	struct cam_isp_ctx_req *req_isp = NULL;
	uint64_t *addr;
...
static void *cam_isp_ctx_user_dump_stream_info(void *dump_struct,
					       uint8_t *addr_ptr)
{
	struct cam_context *ctx = NULL;
	int32_t *addr;
...
static int
__cam_isp_ctx_dump_in_top_state(struct cam_context *ctx,
...
hw_dump:
	...
	req_isp = (struct cam_isp_ctx_req *)req->req_priv;
	cur_time = ktime_get();
	diff = ktime_us_delta(req_isp->event_timestamp[CAM_ISP_CTX_EVENT_APPLY],
			      cur_time);
	...
	/* Dump time info */
	rc = cam_common_user_dump_helper(&dump_args,
					 cam_isp_ctx_user_dump_timer, req,
					 sizeof(uint64_t),
					 "ISP_CTX_DUMP.%c:", req_type);
	if (rc) {
		CAM_ERR(CAM_ISP,
	...
	if (ctx->hw_mgr_intf->hw_dump) {
		...
		rc = cam_common_user_dump_helper(
			&dump_args, cam_isp_ctx_user_dump_stream_info, ctx,
	...
	if (ctx->hw_mgr_intf->hw_dump) {
		...
		rc = ctx->hw_mgr_intf->hw_dump(ctx->hw_mgr_intf->hw_mgr_priv,
					       &ife_dump_args);
	...
	struct cam_context *ctx, struct cam_req_mgr_flush_request *flush_req)
{
	CAM_INFO(
		CAM_ISP,
		"Flush (type %d) in flushed state req id %lld ctx_id:%u link: 0x%x",
	...
	struct cam_context *ctx, struct cam_req_mgr_flush_request *flush_req)
{
	...
	CAM_DBG(CAM_ISP, "Flush pending list, ctx_idx: %u, link: 0x%x",
		ctx->ctx_id, ctx->link_hdl);
	spin_lock_bh(&ctx->lock);
	...
	if (flush_req->type == CAM_REQ_MGR_FLUSH_TYPE_ALL) {
		...
		__cam_isp_ctx_trigger_reg_dump(CAM_HW_MGR_CMD_REG_DUMP_ON_FLUSH,
					       ctx);
					   ...
					   void *evt_data)
{
	...
	if (atomic_read(&ctx_isp->process_bubble)) {
		...
		if (ctx_isp->last_sof_timestamp == ctx_isp->sof_timestamp_val) {
			CAM_DBG(CAM_ISP,
				"Tasklet delay detected! Bubble frame: %lld check skipped, "
				"sof_timestamp: %lld, ctx_id: %u, link: 0x%x",
		...
		if (req_isp->bubble_detected) {
			...
			if (last_cdm_done_req >= req->request_id) {
				CAM_DBG(CAM_ISP,
					"CDM callback detected for req: %lld, possible buf_done delay, "
					"waiting for buf_done, ctx_id: %u, link: 0x%x",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl);
				if (req_isp->num_fence_map_out ==
				    req_isp->num_deferred_acks) {
				...
				CAM_WARN(
					CAM_ISP,
					"CDM callback not happened for req: %lld, possible CDM stuck "
					"or workqueue delay, ctx_id: %u, link: 0x%x",
					req->request_id, ctx->ctx_id,
					ctx->link_hdl);
				req_isp->num_acked = 0;
				req_isp->num_deferred_acks = 0;
...
static int
__cam_isp_ctx_rdi_only_reg_upd_in_bubble_state(struct cam_isp_context *ctx_isp,
					       void *evt_data)
{
	struct cam_ctx_request *req = NULL;
	struct cam_context *ctx = ctx_isp->base;
...
static struct cam_ctx_ops
    cam_isp_ctx_rdi_only_activated_state_machine[CAM_ISP_CTX_ACTIVATED_MAX] = {
        /* SOF */
        {
            .ioctl_ops = {},
            .crm_ops =
						...
						struct cam_flush_dev_cmd *cmd)
{
	...
	flush_req.type = (cmd->flush_type == CAM_FLUSH_TYPE_ALL) ?
				 CAM_REQ_MGR_FLUSH_TYPE_ALL :
				       ...
				       struct cam_release_dev_cmd *cmd)
{
	...
	memset(&ctx_isp->evt_inject_params, 0,
						 ...
						 struct cam_config_dev_cmd *cmd)
{
	...
	memset(req_isp->fence_map_out, 0,
	...
	for (i = 0; i < req_isp->num_fence_map_out; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can't get ref for fence %d, ctx_idx: %u, link: 0x%x",
	...
	CAM_DBG(CAM_ISP,
		"packet req-id:%lld, opcode:%d, num_entry:%d, num_fence_out: %d, "
		"num_fence_in: %d, ctx_idx: %u, link: 0x%x",
		packet->header.request_id,
		req_isp->hw_update_data.packet_opcode_type, req_isp->num_cfg,
		req_isp->num_fence_map_out, req_isp->num_fence_map_in,
		ctx->ctx_id, ctx->link_hdl);
		...
		if ((ctx_isp->offline_context) || (ctx_isp->vfps_aux_context)) {
			...
			if (rc) {
				if (rc == -EBADR)
					CAM_INFO(
						CAM_ISP,
						"Add req failed: req id=%llu, it has been flushed on link "
						"0x%x ctx %u",
...
static int
__cam_isp_ctx_allocate_mem_hw_entries(struct cam_context *ctx,
				      struct cam_hw_acquire_args *param)
{
	...
	CAM_DBG(CAM_ISP,
		"Allocate max_entries: 0x%x max_res: 0x%x is_sfe_en: %d, ctx: %u, "
		"link: 0x%x",
		max_hw_upd_entries, max_res,
		(param->op_flags & CAM_IFE_CTX_SFE_EN), ctx->ctx_id,
		ctx->link_hdl);

	ctx->hw_update_entry = kcalloc(CAM_ISP_CTX_REQ_MAX,
				       sizeof(struct cam_hw_update_entry *),
				       GFP_KERNEL);

	if (!ctx->hw_update_entry) {
		CAM_ERR(CAM_CTXT, "%s[%u] no memory, link: 0x%x", ctx->dev_name,
			ctx->ctx_id, ctx->link_hdl);
		return -ENOMEM;
	}

	for (i = 0; i < CAM_ISP_CTX_REQ_MAX; i++) {
		ctx->hw_update_entry[i] =
			kcalloc(ctx->max_hw_update_entries,
	...
	ctx->in_map_entries = kcalloc(CAM_ISP_CTX_REQ_MAX,
				      sizeof(struct cam_hw_fence_map_entry *),
				      GFP_KERNEL);

	if (!ctx->in_map_entries) {
		CAM_ERR(CAM_CTXT,
			"%s[%u] no memory for in_map_entries, link: 0x%x",
	...
	ctx->out_map_entries = kcalloc(CAM_ISP_CTX_REQ_MAX,
				       sizeof(struct cam_hw_fence_map_entry *),
				       GFP_KERNEL);

	if (!ctx->out_map_entries) {
		CAM_ERR(CAM_CTXT,
			"%s[%u] no memory for out_map_entries, link: 0x%x",
			ctx->dev_name, ctx->ctx_id, ctx->link_hdl);
		rc = -ENOMEM;
		goto end;
	}
	...
	for (i = 0; i < CAM_ISP_CTX_REQ_MAX; i++) {
		ctx->out_map_entries[i] = kcalloc(
			ctx->max_out_map_entries,
			sizeof(struct cam_hw_fence_map_entry), GFP_KERNEL);

		if (!ctx->out_map_entries[i]) {
			CAM_ERR(CAM_CTXT,
				"%s[%u] no memory for out_map_entries: %u, link: 0x%x",
				ctx->dev_name, ctx->ctx_id, i, ctx->link_hdl);
			rc = -ENOMEM;
			goto end;
		}
				       ...
				       struct cam_acquire_dev_cmd *cmd)
{
	...
	CAM_DBG(CAM_ISP,
		"session_hdl 0x%x, num_resources %d, hdl type %d, res %lld, ctx_idx: "
		"%u, link: 0x%x",
		cmd->session_handle, cmd->num_resources, cmd->handle_type,
		cmd->resource_hdl, ctx->ctx_id, ctx->link_hdl);
	...
	if (rc != 0) {
		CAM_ERR(CAM_ISP,
			"Acquire device failed, ctx_idx: %u, link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		goto free_res;
	}
...
static int __cam_isp_ctx_acquire_hw_v1(struct cam_context *ctx, void *args)
{
	...
	if (cmd->handle_type != 1) {
		CAM_ERR(CAM_ISP,
			"Only user pointer is supported, ctx %u link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		rc = -EINVAL;
		goto end;
	}
	...
	if (cmd->data_size < sizeof(*acquire_hw_info)) {
		CAM_ERR(CAM_ISP,
			"data_size is not a valid value, ctx %u link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		goto end;
	}
	...
	CAM_INFO(
		CAM_ISP,
		"Acquire success:session_hdl 0x%xs ctx_type %d ctx %u link: 0x%x "
		"hw_mgr_ctx: %u",
		ctx->session_hdl, isp_hw_cmd_args.u.ctx_type, ctx->ctx_id,
		ctx->link_hdl, param.hw_mgr_ctx_id);
...
static int __cam_isp_ctx_acquire_hw_v2(struct cam_context *ctx, void *args)
{
	...
	if (!ctx->hw_mgr_intf) {
		CAM_ERR(CAM_ISP,
			"HW interface is not ready, ctx_id %u link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		rc = -EFAULT;
		goto end;
	}
	...
	if (cmd->handle_type != 1) {
		CAM_ERR(CAM_ISP,
			"Only user pointer is supported, ctx_id %u link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		rc = -EINVAL;
		goto end;
	}
	...
	if (cmd->data_size < sizeof(*acquire_hw_info)) {
		CAM_ERR(CAM_ISP,
			"data_size is not a valid value, ctx_id %u link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		goto end;
	}
   ...
   */
	ctx_isp->custom_enabled = (param.op_flags & CAM_IFE_CTX_CUSTOM_EN);
	ctx_isp->use_frame_header_ts =
		(param.op_flags & CAM_IFE_CTX_FRAME_HEADER_EN);
	...
	if (param.op_flags & CAM_IFE_CTX_SFE_EN) {
		ctx_isp->sfe_bus_comp_grp = kcalloc(
			CAM_SFE_BUS_COMP_NUM_MAX,
			sizeof(struct cam_isp_context_comp_record), GFP_KERNEL);
		if (!ctx_isp->sfe_bus_comp_grp) {
			CAM_ERR(CAM_CTXT,
				"%s[%d] no memory for sfe_bus_comp_grp",
				ctx->dev_name, ctx->ctx_id);
			rc = -ENOMEM;
			goto end;
		}
	...
	if (ctx_isp->offline_context || ctx_isp->vfps_aux_context) {
		rc = cam_req_mgr_workq_create(
			"ife_apply_req", 20, &ctx_isp->workq,
	...
	snprintf(ctx->ctx_id_string, sizeof(ctx->ctx_id_string),
		 "%s_ctx[%d]_hwmgrctx[%d]_hwidx[0x%x]", ctx->dev_name,
		 ctx->ctx_id, ctx->hw_mgr_ctx_id, ctx_isp->hw_idx);
						...
						struct cam_config_dev_cmd *cmd)
{
	...
	CAM_DBG(CAM_ISP, "next state %d, ctx %u link: 0x%x", ctx->state,
		ctx->ctx_id, ctx->link_hdl);
	return rc;
}
					       ...
					       struct cam_config_dev_cmd *cmd)
{
	...
	CAM_DBG(CAM_ISP,
		...
		CAM_BOOL_TO_YESNO(ctx_isp->vfps_aux_context),
		ctx_isp->resume_hw_in_flushed, ctx->ctx_id, ctx->link_hdl);
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Failed to resume HW rc: %d, ctx_id %u link: 0x%x", rc,
			ctx->ctx_id, ctx->link_hdl);
		goto end;
	}
	...
	CAM_DBG(CAM_ISP, "next state %d sub_state:%d ctx_id %u link: 0x%x",
			       ...
			       struct cam_req_mgr_core_dev_link_setup *link)
{
	...
	if (!link) {
		CAM_ERR(CAM_ISP,
			"setup link info is null: %pK ctx: %u link: 0x%x", link,
			ctx->ctx_id, ctx->link_hdl);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "Enter.........ctx: %u, link: 0x%x", ctx->ctx_id,
		ctx->link_hdl);

	ctx->link_hdl = link->link_hdl;
	ctx->ctx_crm_intf = link->crm_cb;
	ctx_isp->subscribe_event = CAM_TRIGGER_POINT_SOF |
...
static inline void
__cam_isp_context_reset_ctx_params(struct cam_isp_context *ctx_isp)
{
	atomic_set(&ctx_isp->process_bubble, 0);
	atomic_set(&ctx_isp->rxd_epoch, 0);
	atomic_set(&ctx_isp->internal_recovery_set, 0);
					    ...
					    struct cam_start_stop_dev_cmd *cmd)
{
	...
	if (list_empty(&ctx->pending_req_list)) {
		/* should never happen */
		CAM_ERR(CAM_ISP,
			"Start device with empty configuration, ctx: %u, link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		rc = -EFAULT;
		goto end;
	} else {
	...
	if (!ctx_isp->hw_ctx) {
		CAM_ERR(CAM_ISP,
			"Wrong hw context pointer.ctx_idx: %u, link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		rc = -EFAULT;
		goto end;
	}
   ...
   */
	...
	if (ctx_isp->offline_context && !req_isp->num_fence_map_out) {
		...
		atomic_set(&ctx_isp->rxd_epoch, 1);
		CAM_DBG(CAM_REQ,
			"Move pending req: %lld to free list(cnt: %d) offline ctx %u link: "
			"0x%x",
			req->request_id, ctx_isp->active_req_cnt, ctx->ctx_id,
			ctx->link_hdl);
		...
		ctx_isp->active_req_cnt++;
		CAM_DBG(CAM_REQ,
			"Move pending req: %lld to active list(cnt: %d) ctx %u link: 0x%x "
			"offline %d",
			req->request_id, ctx_isp->active_req_cnt, ctx->ctx_id,
			ctx->link_hdl, ctx_isp->offline_context);
	}
   ...
   */
	...
	if (rc) {
		...
		CAM_ERR(CAM_ISP, "Start HW failed, ctx %u link: 0x%x",
			ctx->ctx_id, ctx->link_hdl);
		ctx->state = CAM_CTX_READY;
		if ((rc == -ETIMEDOUT) &&
	...
	CAM_DBG(CAM_ISP, "start device success ctx %u link: 0x%x", ctx->ctx_id,
		ctx->link_hdl);
	...
	struct cam_context *ctx, struct cam_start_stop_dev_cmd *stop_cmd)
{
	...
	if (!stop_cmd) {
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"Unlink failed rc=%d, ctx %u link: 0x%x", rc,
				ctx->ctx_id, ctx->link_hdl);
	}
	return rc;
}
				       ...
				       struct cam_release_dev_cmd *cmd)
{
	...
	if (rc)
		CAM_ERR(CAM_ISP,
			"Release device failed rc=%d ctx %u link: 0x%x", rc,
			ctx->ctx_id, ctx->link_hdl);
...
static int __cam_isp_ctx_reset_and_recover(bool skip_resume,
					   struct cam_context *ctx)
{
	int rc = 0;
	struct cam_isp_context *ctx_isp =
	...
	if (ctx_isp->active_req_cnt) {
		...
		CAM_WARN(
			CAM_ISP,
			"Active list not empty: %u in ctx: %u on link: 0x%x, retry "
			"recovery for req: %lld after buf_done",
	...
	if (ctx->state != CAM_CTX_ACTIVATED) {
		spin_unlock_bh(&ctx->lock);
		CAM_ERR(CAM_ISP,
			"In wrong state %d, for recovery ctx: %u in link: 0x%x recovery "
			"req: %lld",
			ctx->state, ctx->ctx_id, ctx->link_hdl,
			ctx_isp->recovery_req_id);
		rc = -EINVAL;
	...
	if (list_empty(&ctx->pending_req_list)) {
		...
		spin_unlock_bh(&ctx->lock);
		CAM_ERR(CAM_ISP,
			"Failed to reset and recover last_applied_req: %llu in ctx: %u on "
			"link: 0x%x",
			ctx_isp->last_applied_req_id, ctx->ctx_id,
			ctx->link_hdl);
		rc = -EFAULT;
	...
	CAM_INFO(
		CAM_ISP,
		"Trigger Halt, Reset & Resume for req: %llu ctx: %u in state: %d "
		"link: 0x%x",
	...
	CAM_DBG(CAM_ISP, "Stop HW success ctx: %u link: 0x%x", ctx->ctx_id,
		ctx->link_hdl);

	/* API provides provision to stream off and not resume as well in case of
   * fatal errors */
	if (skip_resume) {
		atomic_set(&ctx_isp->internal_recovery_set, 0);
		CAM_INFO(
			CAM_ISP,
			"Halting streaming off IFE/SFE ctx: %u last_applied_req: %lld "
			"[recovery_req: %lld] on link: 0x%x",
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Failed to resume HW rc: %d ctx: %u link: 0x%x", rc,
			ctx->ctx_id, ctx->link_hdl);
		goto end;
	}
	...
	__cam_isp_ctx_notify_v4l2_error_event(
		CAM_REQ_MGR_WARN_TYPE_KMD_RECOVERY, 0, req->request_id, ctx);

	CAM_INFO(CAM_ISP,
		 "Internal Start HW success ctx %u on link: 0x%x for req: %llu",
		 ctx->ctx_id, ctx->link_hdl, req->request_id);
...
static bool
__cam_isp_ctx_try_internal_recovery_for_bubble(int64_t error_req_id,
					       struct cam_context *ctx)
{
	int rc;
	struct cam_isp_context *ctx_isp =
	...
	if (ctx_isp->last_applied_req_id < error_req_id) {
		CAM_WARN(
			CAM_ISP,
			"Skip trying for internal recovery last applied: %lld error_req: "
			"%lld for ctx: %u on link: 0x%x",
			ctx_isp->last_applied_req_id, error_req_id, ctx->ctx_id,
			ctx->link_hdl);
		return false;
	}
	...
	if (__cam_isp_ctx_validate_for_req_reapply_util(ctx_isp)) {
		CAM_WARN(
			CAM_ISP,
			"Internal recovery not possible for ctx: %u on link: 0x%x req: "
			"%lld [last_applied: %lld]",
			ctx->ctx_id, ctx->link_hdl, error_req_id,
			ctx_isp->last_applied_req_id);
		return false;
	}
	...
	if (rc) {
		CAM_WARN(
			CAM_ISP,
			"Internal recovery failed in ctx: %u on link: 0x%x req: %lld "
			"[last_applied: %lld]",
			ctx->ctx_id, ctx->link_hdl, error_req_id,
			ctx_isp->last_applied_req_id);
		atomic_set(&ctx_isp->internal_recovery_set, 0);
	...
	CAM_DBG(CAM_ISP,
		"Internal recovery done in ctx: %u on link: 0x%x req: %lld "
		"[last_applied: %lld]",
		ctx->ctx_id, ctx->link_hdl, error_req_id,
		ctx_isp->last_applied_req_id);
	...
	struct cam_context *ctx, struct cam_req_mgr_core_dev_link_setup *unlink)
{
	...
	if (rc)
		CAM_WARN(CAM_ISP,
			 "Stop device failed rc=%d, ctx: %u link: 0x%x", rc,
			 ctx->ctx_id, ctx->link_hdl);

	rc = __cam_isp_ctx_unlink_in_ready(ctx, unlink);
	if (rc)
		CAM_ERR(CAM_ISP, "Unlink failed rc=%d, ctx: %u link: 0x%x", rc,
			ctx->ctx_id, ctx->link_hdl);
				   ...
				   struct cam_req_mgr_apply_request *apply)
{
	...
	trace_cam_apply_req("ISP", ctx->ctx_id, apply->request_id,
			    apply->link_hdl);
	CAM_DBG(CAM_ISP,
		"Enter: apply req in Substate[%s] request_id:%lld, ctx: %u link: 0x%x",
		__cam_isp_ctx_substate_val_to_type(ctx_isp->substate_activated),
		apply->request_id, ctx->ctx_id, ctx->link_hdl);
...
static int
__cam_isp_ctx_apply_default_settings(struct cam_context *ctx,
				     struct cam_req_mgr_apply_request *apply)
{
	int rc = 0;
	struct cam_ctx_ops *ctx_ops = NULL;
   ...
   */
	if ((ctx_isp->use_default_apply) ||
		...
		ctx_ops =
			&ctx_isp->substate_machine[ctx_isp->substate_activated];
...
void __cam_isp_ctx_notify_cpas(struct cam_context *ctx, uint32_t evt_id)
{
	...
	snprintf(ctx_evt_id_string, sizeof(ctx_evt_id_string),
		 "%s_frame[%llu]_%s", ctx->ctx_id_string, ctx_isp->frame_id,
		 cam_isp_hw_evt_type_to_string(evt_id));
	CAM_DBG(CAM_ISP, "Substate[%s] ctx: %s frame: %llu evt: %s req: %llu",
		__cam_isp_ctx_substate_val_to_type(ctx_isp->substate_activated),
		ctx->ctx_id_string, ctx_isp->frame_id,
		cam_isp_hw_evt_type_to_string(evt_id), request_id);
						 ...
						 void *evt_data)
{
	...
	CAM_DBG(CAM_ISP,
		...
		ctx->state,
		__cam_isp_ctx_substate_val_to_type(ctx_isp->substate_activated),
		evt_id, ctx->ctx_id, ctx->link_hdl);
...
static int cam_isp_context_validate_event_notify_injection(
	struct cam_context *ctx, struct cam_hw_inject_evt_param *evt_params)
{
	int rc = 0;
	uint32_t evt_type;
	uint64_t req_id;
	...
	case V4L_EVENT_CAM_REQ_MGR_ERROR: {
		...
		default:
			CAM_ERR(CAM_ISP,
				"Invalid error type: %u for error event injection err type: %u "
				"req id: %llu ctx id: %u link: 0x%x dev hdl: %d",
				err_evt_params->err_type,
				err_evt_params->err_code, req_id, ctx->ctx_id,
				ctx->link_hdl, ctx->dev_hdl);
			return -EINVAL;
		}

		CAM_INFO(
			CAM_ISP,
			"Inject ERR evt: err code: %u err type: %u req id: %llu ctx id: "
			"%u link: 0x%x dev hdl: %d",
	...
	case V4L_EVENT_CAM_REQ_MGR_PF_ERROR: {
		...
		rc = cam_smmu_is_cb_non_fatal_fault_en(ctx->img_iommu_hdl,
						       &non_fatal_en);
		if (rc) {
			CAM_ERR(CAM_ISP,
		...
		if (!non_fatal_en) {
			CAM_ERR(CAM_ISP,
				"Fail to inject pagefault event notif. Pagefault fatal for "
				"ISP,ctx:%u link:0x%x",
				ctx->ctx_id, ctx->link_hdl);
			return -EINVAL;
		}
	...
	default:
		CAM_ERR(CAM_ISP,
			"Event notification type not supported: %u, ctx: %u link: 0x%x",
...
static int cam_isp_context_inject_evt(void *context, void *evt_args)
{
	...
	if (!ctx || !evt_args) {
		CAM_ERR(CAM_ISP, "Invalid params ctx %s event args %s",
	...
	if (evt_params->inject_id == CAM_COMMON_EVT_INJECT_NOTIFY_EVENT_TYPE) {
		rc = cam_isp_context_validate_event_notify_injection(
			ctx, evt_params);
		if (rc) {
			CAM_ERR(CAM_ISP,
...
/* top state machine */
static struct cam_ctx_ops cam_isp_ctx_top_state_machine[CAM_CTX_STATE_MAX] = {
    /* Uninit */
    {
        .ioctl_ops = {},
        .crm_ops = {},
        ...
        .crm_ops =
            {
                ...
                .notify_frame_skip = __cam_isp_ctx_apply_default_settings,
                .flush_req = __cam_isp_ctx_flush_req_in_top_state,
                .process_evt = __cam_isp_ctx_process_evt,
                .dump_req = __cam_isp_ctx_dump_in_top_state,
            },
...
static int cam_isp_context_hw_recovery(void *priv, void *data)
{
	...
	if (ctx->hw_mgr_intf->hw_recovery)
		rc = ctx->hw_mgr_intf->hw_recovery(
			ctx->hw_mgr_intf->hw_mgr_priv, data);
	else
		CAM_ERR(CAM_ISP,
...
static void cam_isp_context_find_faulted_context(
	struct cam_context *ctx, struct list_head *req_list,
	struct cam_hw_dump_pf_args *pf_args, bool *found)
{
	struct cam_ctx_request *req = NULL;
	...
	*found = false;
	list_for_each_entry_safe(req, req_temp, req_list, list) {
		...
		rc = cam_context_dump_pf_info_to_hw(ctx, pf_args,
						    &req->pf_data);
		if (rc)
			CAM_ERR(CAM_ISP,
     ...
     */
		if (pf_args->pf_context_info.mem_type !=
...
static int cam_isp_context_dump_requests(void *data, void *args)
{
	...
	struct cam_hw_dump_pf_args *pf_args =
		(struct cam_hw_dump_pf_args *)args;
	int rc = 0;
	bool found;

	if (!ctx || !pf_args) {
		CAM_ERR(CAM_ISP, "Invalid ctx %pK or pf args %pK", ctx,
			pf_args);
		return -EINVAL;
	}
	...
	CAM_INFO(
		...
		ctx->ctx_id, ctx->link_hdl, ctx->state);
	cam_isp_context_find_faulted_context(ctx, &ctx->wait_req_list, pf_args,
					     &found);
	if (found)
		goto end;
   ...
   */
	...
	if (pf_args->pf_context_info.resource_type) {
		...
		CAM_INFO(
			CAM_ISP,
			"Page fault on resource:%s (0x%x) ctx id:%u link: 0x%x frame id:%d "
			"reported id:%lld applied id:%lld",
			__cam_isp_resource_handle_id_to_type(
				ctx_isp->isp_device_type,
				pf_args->pf_context_info.resource_type),
			pf_args->pf_context_info.resource_type, ctx->ctx_id,
			ctx->link_hdl, ctx_isp->frame_id,
...
int cam_isp_context_init(struct cam_isp_context *ctx,
			 ...
			 uint32_t isp_device_type, int img_iommu_hdl)
{
	int rc = -1;
	int i;
	...
	rc = cam_context_init(ctx_base, isp_dev_name, CAM_ISP, ctx_id,
			      ...
			      CAM_ISP_CTX_REQ_MAX, img_iommu_hdl);
	if (rc) {
		CAM_ERR(CAM_ISP,
...
void cam_isp_detect_framerate(struct cam_isp_context *ctx, uint interval)
{
	...
	if ((ctx->base->exlink != ctx->base->link_hdl) ||
		...
		case 0: {
			timespan = (ctx->sof_timestamp_val -
				    ctx->base->dbg_timestamp) /
				   1000000;
			frame_rate = ctx->base->batchsize *
				     (1000000 *
				      (ctx->frame_id - ctx->base->dbg_frame)) /
				     timespan;
			CAM_DBG(MI_PERF,
				"link hdl 0x%x frame number %d, Time Span(ms): %d Frame "
				"Rate(fps): %d.%03d ctx %d",
				ctx->base->link_hdl, ctx->frame_id, timespan,
				 ...
				 struct cam_packet *cpkt)
{
	...
	cmd_desc = (struct cam_cmd_buf_desc *)((uint8_t *)cpkt->payload +
					       cpkt->cmd_buf_offset);

	for (i = 0; i < cpkt->num_cmd_buf; i++) {
		if (cmd_desc[i].meta_data ==
		    CAM_ISP_PACKET_META_GENERIC_BLOB_COMMON) {
			...
			while (len_read < cmd_desc[i].length) {
				blob_type =
					((*blob_ptr) &
					 CAM_GENERIC_BLOB_CMDBUFFER_TYPE_MASK) >>
					CAM_GENERIC_BLOB_CMDBUFFER_TYPE_SHIFT;
				blob_size =
					((*blob_ptr) &
					 CAM_GENERIC_BLOB_CMDBUFFER_SIZE_MASK) >>
				...
				if (blob_type ==
				    CAM_ISP_GENERIC_BLOB_TYPE_HFR_CONFIG) {
					hfr_config =
						(struct cam_isp_resource_hfr_config
							 *)(uint8_t *)(blob_ptr +
								       1);
					ctx->batchsize =
						hfr_config->port_hfr_config[10]
							.subsample_period +
						1;
					break;
				}
				      ...
				      struct cam_context *ctx)
{
	...
	CAM_INFO(
		CAM_ISP,
		"[DMQS] MQS event [type: %u code: %u] for req: %llu in ctx: %u on "
		"link: 0x%x notified",
		error_type, error_code, error_request_id, ctx->ctx_id,
		ctx->link_hdl);

	rc = cam_req_mgr_notify_message(&req_msg, V4L_EVENT_CAM_MQS_ISP,
					V4L_EVENT_CAM_MQS_EVENT);
	if (rc)
		CAM_ERR(CAM_ISP,
			"[DMQS] Notifying MQS event error [type: %u code: %u] failed for "
			"req id:%llu in ctx %u on link: 0x%x",
