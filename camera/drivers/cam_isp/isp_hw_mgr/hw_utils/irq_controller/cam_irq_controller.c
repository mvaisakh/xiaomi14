 ...
 * @test_sub_val:           Value to write to IRQ MASK register to receive test
 * IRQ
 ...
 * @dependent_read_mask:    Mask to check if any dependent controllers' read
 * needs to triggered from independent controller
 * @dirty_clear:            Flag to identify if clear register contains non-zero
 * value
 */
struct cam_irq_register_obj {
	uint32_t index;
 ...
 *                          to take effect
 * @global_clear_bitmask:   Bitmask needed to be used in Global IRQ command
 * register for Clear IRQ cmd to take effect
 * @global_set_bitmask:     Bitmask needed to be used in Global IRQ command
 * register for Set IRQ cmd to take effect
 ...
 * @is_dependent:           Flag to indicate is this controller is dependent on
 * another controller
 * @dependent_controller:   Array of controllers that depend on this controller
 * @delayed_global_clear:   Flag to indicate if this controller issues global
 * clear after dependent controllers are handled
 * @lock:                   Lock to be used by controller, Use mutex lock in
 * presil mode, and spinlock in regular case
 */
struct cam_irq_controller {
...
#ifdef CONFIG_CAM_PRESIL
static inline void
cam_irq_controller_lock_init(struct cam_irq_controller *controller)
{
	mutex_init(&controller->lock);
}

static inline unsigned long
cam_irq_controller_lock_irqsave(struct cam_irq_controller *controller)
{
...
static inline void
cam_irq_controller_unlock_irqrestore(struct cam_irq_controller *controller,
				     unsigned long flags)
{
...
static inline void
cam_irq_controller_lock(struct cam_irq_controller *controller)
{
	mutex_lock(&controller->lock);
}

static inline void
cam_irq_controller_unlock(struct cam_irq_controller *controller)
{
	mutex_unlock(&controller->lock);
}
#else
...
int cam_irq_controller_unregister_dependent(void *primary_controller,
					    void *secondary_controller)
{
	struct cam_irq_controller *ctrl_primary, *ctrl_secondary;
	int i, dep_idx;

	if (!primary_controller || !secondary_controller) {
		CAM_ERR(CAM_IRQ_CTRL, "invalid args: %pK, %pK",
			primary_controller, secondary_controller);
		return -EINVAL;
	}
	...
	for (i = 0; i < ctrl_primary->num_registers; i++)
		ctrl_primary->irq_register_arr[i].dependent_read_mask[dep_idx] =
			0;
...
int cam_irq_controller_register_dependent(void *primary_controller,
					  void *secondary_controller,
					  uint32_t *mask)
{
	struct cam_irq_controller *ctrl_primary, *ctrl_secondary;
   ...
   */
	...
	CAM_DBG(CAM_IRQ_CTRL, "successfully registered %s as dependent of %s",
		ctrl_secondary->name, ctrl_primary->name);
	return 0;
}

static inline void
cam_irq_controller_clear_irq(struct cam_irq_controller *controller,
			    ...
			    void **irq_controller)
{
	...
	*irq_controller = NULL;
	...
	controller->global_irq_cmd_offset =
		register_info->global_irq_cmd_offset;
...
static inline void
__cam_irq_controller_disable_irq(struct cam_irq_controller *controller,
				 struct cam_irq_evt_handler *evt_handler)
{
	struct cam_irq_register_obj *irq_register;
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		cam_io_w_mb(irq_register->aggr_mask,
			    controller->mem_base +
...
static inline void
__cam_irq_controller_disable_irq_evt(struct cam_irq_controller *controller,
				     struct cam_irq_evt_handler *evt_handler)
{
	struct cam_irq_register_obj *irq_register;
	uint32_t *update_mask;
...
static inline void
__cam_irq_controller_enable_irq(struct cam_irq_controller *controller,
				struct cam_irq_evt_handler *evt_handler)
{
	struct cam_irq_register_obj *irq_register;
	uint32_t *update_mask;
...
int cam_irq_controller_subscribe_irq(
	...
	struct cam_irq_bh_api *irq_bh_api, enum cam_irq_event_group evt_grp)
{
	struct cam_irq_controller *controller = irq_controller;
	struct cam_irq_evt_handler *evt_handler = NULL;
	int i;
...
static inline int cam_irq_controller_find_event_handle(
	struct cam_irq_controller *controller, uint32_t handle,
	struct cam_irq_evt_handler **found_evt_handler)
{
...
static void
__cam_irq_controller_th_processing(struct cam_irq_controller *controller,
				   struct list_head *th_list_head, int evt_grp)
{
	struct cam_irq_evt_handler *evt_handler = NULL;
	...
	list_for_each_entry_safe(evt_handler, evt_handler_tmp, th_list_head,
				 th_list_node) {
		is_irq_match = cam_irq_controller_match_bit_mask(
...
void cam_irq_controller_disable_all(void *priv)
{
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		cam_io_w_mb(0x0, controller->mem_base +
					 irq_register->mask_reg_offset);
		cam_io_w_mb(controller->clear_all_bitmask,
			    controller->mem_base +
				    irq_register->clear_reg_offset);
	}
...
static void
__cam_irq_controller_read_registers(struct cam_irq_controller *controller)
{
	struct cam_irq_register_obj *irq_register;
	int i;
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		controller->irq_status_arr[i] = cam_io_r(
			controller->mem_base + irq_register->status_reg_offset);

		CAM_DBG(CAM_IRQ_CTRL, "(%s) Read irq status%d (0x%x) = 0x%x",
			controller->name, i,
			controller->irq_register_arr[i].status_reg_offset,
			controller->irq_status_arr[i]);
...
static void __cam_irq_controller_sanitize_clear_registers(
	struct cam_irq_controller *controller)
{
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		CAM_DBG(CAM_IRQ_CTRL,
			"(%s) Write irq clear%d (0x%x) = 0x%x (dirty=%s)",
			controller->name, i,
			controller->irq_register_arr[i].clear_reg_offset, 0x0,
			CAM_BOOL_TO_YESNO(irq_register->dirty_clear));
...
static void
cam_irq_controller_read_registers(struct cam_irq_controller *controller)
{
	struct cam_irq_register_obj *irq_register;
...
static void cam_irq_controller_process_th(struct cam_irq_controller *controller,
					  int evt_grp)
{
	struct cam_irq_register_obj *irq_register;
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		for (j = 0; j < CAM_IRQ_PRIORITY_MAX; j++) {
			...
			CAM_DBG(CAM_IRQ_CTRL,
				"reg:%d priority:%d need_th_processing = %d", i,
				j, need_th_processing[j]);
		}
	}
...
static int
cam_irq_controller_test_irq_line_top_half(uint32_t evt_id,
					  struct cam_irq_th_payload *th_payload)
{
...
int cam_irq_controller_test_irq_line(void *irq_controller, const char *fmt, ...)
{
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		if (irq_reg->set_reg_offset != 0 &&
		    irq_reg->test_set_val != 0 && irq_reg->test_sub_val)
			can_test = true;
	...
	if (controller->global_irq_cmd_offset == 0 ||
	...
	if (!mask) {
		CAM_ERR(CAM_IRQ_CTRL,
			"%s: cannot allocate mask array of length %d",
	...
	if (!test_priv) {
		CAM_ERR(CAM_IRQ_CTRL, "%s: cannot allocate test-priv",
			controller->name);
		rc = -ENOMEM;
		goto kfree_exit;
	}
	...
	handle = cam_irq_controller_subscribe_irq(
		controller, CAM_IRQ_PRIORITY_0, mask, test_priv,
		cam_irq_controller_test_irq_line_top_half, NULL, NULL, NULL,
		CAM_IRQ_EVT_GROUP_0);
	if (handle < 0) {
		CAM_ERR(CAM_IRQ_CTRL,
			"%s: failed to subscribe to test irq line",
			controller->name);
		rc = -EINVAL;
		goto kfree_exit;
	}
	...
	for (i = 0; i < controller->num_registers; i++) {
		...
		if (irq_reg->test_set_val && irq_reg->set_reg_offset) {
			cam_io_w_mb(irq_reg->test_set_val,
				    controller->mem_base +
					    irq_reg->set_reg_offset);
			CAM_DBG(CAM_IRQ_CTRL, "%s[%d] offset:0x%08x val:0x%08x",
				controller->name, i, irq_reg->set_reg_offset,
				irq_reg->test_set_val);
		}
	}

	cam_io_w_mb(controller->global_set_bitmask,
		    controller->mem_base + controller->global_irq_cmd_offset);
	CAM_DBG(CAM_IRQ_CTRL, "%s[SET-CMD] addr:0x%08x value:0x%08x",
		controller->name,
		controller->mem_base + controller->global_irq_cmd_offset,
		controller->global_set_bitmask);
	...
	if (!cam_common_wait_for_completion_timeout(
		...
		CAM_ERR(CAM_IRQ_CTRL, "%s: IRQ line verification timed out",
			controller->name);
		rc = -ETIMEDOUT;
		goto unsub_exit;
	}

	CAM_DBG(CAM_IRQ_CTRL, "%s: verified IRQ line successfully",
		controller->name);
