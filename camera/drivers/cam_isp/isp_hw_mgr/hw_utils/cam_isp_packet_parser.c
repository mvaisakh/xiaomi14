...
static void cam_isp_add_update_entry(enum cam_isp_cdm_bl_type cdm_bl_type,
				     ...
				     uint32_t update_size)
{
	...
	CAM_DBG(CAM_ISP,
		...
		num_ent - 1, curr_update_entry->handle, curr_update_entry->len,
		curr_update_entry->offset);
}

static void
cam_isp_combine_update_entry(struct cam_hw_prepare_update_args *prepare,
			     struct cam_kmd_buf_info *kmd_buf_info,
			     uint32_t update_size)
{
	/* Validation is done from the caller */
	...
	CAM_DBG(CAM_ISP,
		...
		num_ent - 1, prev_update_entry->handle, prev_update_entry->len,
		prev_update_entry->offset);

	kmd_buf_info->used_bytes += update_size;
	kmd_buf_info->offset += update_size;
}

void cam_isp_update_hw_entry(enum cam_isp_cdm_bl_type cdm_bl_type,
			     struct cam_hw_prepare_update_args *prepare,
			     struct cam_kmd_buf_info *kmd_buf_info,
			     uint32_t update_size, bool combine)
{
	if (combine)
		cam_isp_combine_update_entry(prepare, kmd_buf_info,
					     update_size);
	else
		cam_isp_add_update_entry(cdm_bl_type, prepare, kmd_buf_info,
					 update_size);
}
...
static int cam_isp_update_dual_config(
	...
	struct cam_isp_hw_mgr_res *res_list_isp_out, uint8_t *out_map,
	uint32_t out_base, uint32_t out_max)
{
	int rc = -EINVAL;
	...
	for (i = 0; i < dual_config->num_ports; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (res->res_id < out_base || res->res_id >= out_max)
				continue;
...
int cam_isp_add_cmd_buf_update(struct cam_isp_resource_node *res,
			       struct cam_hw_intf *hw_intf, uint32_t cmd_type,
			       uint32_t hw_cmd_type, uint32_t *cmd_buf_addr,
			       ...
			       void *cmd_update_data, uint32_t *bytes_used)
{
	...
	rc = hw_intf->hw_ops.process_cmd(
		hw_intf->hw_priv, cmd_update.cmd_type, &cmd_update,
		sizeof(struct cam_isp_hw_get_cmd_update));
...
int cam_isp_add_command_buffers(struct cam_hw_prepare_update_args *prepare,
				...
				uint8_t *out_map, uint32_t out_base,
				uint32_t out_max)
{
	int rc = 0;
   ...
   */
	...
	for (i = 0; i < prepare->packet->num_cmd_buf; i++) {
		...
		case CAM_ISP_PACKET_META_DUAL_CONFIG:
			rc = cam_isp_update_dual_config(
				&cmd_desc[i], split_id, base_idx,
				res_list_isp_out, out_map, out_base, out_max);
			if (rc)
				return rc;
			break;
...
int cam_sfe_add_command_buffers(struct cam_hw_prepare_update_args *prepare,
				struct cam_kmd_buf_info *kmd_buf_info,
				struct cam_isp_ctx_base_info *base_info,
				cam_packet_generic_blob_handler blob_handler_cb,
				...
				uint32_t out_max)
{
   ...
   */
	...
	for (i = 0; i < prepare->packet->num_cmd_buf; i++) {
		...
		case CAM_ISP_SFE_PACKET_META_DUAL_CONFIG:
			rc = cam_isp_update_dual_config(
				&cmd_desc[i], split_id, base_info->idx,
	...
	uint32_t res_type, uint32_t out_base)
{
	...
	if ((res_id_out) <
	    ((out_base & 0xFF) + sfe_res_info->num_active_fe_rdis)) {
		CAM_DBG(CAM_ISP,
...
static int
cam_isp_io_buf_get_entries_util(struct cam_isp_io_buf_info *buf_info,
				...
				struct cam_isp_hw_mgr_res **hw_mgr_res)
{
	...
	CAM_DBG(CAM_REQ,
		...
		io_cfg->resource_type, io_cfg->fence, io_cfg->direction,
		io_cfg->format);
	...
	if (io_cfg->direction == CAM_BUF_OUTPUT) {
		...
		if ((buf_info->base->hw_type == CAM_ISP_HW_TYPE_SFE) &&
			...
			cam_isp_validate_for_sfe_scratch(
				&buf_info->scratch_check_cfg
					 ->sfe_scratch_res_info,
			...
			cam_isp_validate_for_ife_scratch(
				&buf_info->scratch_check_cfg
					 ->ife_scratch_res_info,
				io_cfg->resource_type);
		}

		*hw_mgr_res =
			&buf_info->res_list_isp_out[buf_info->out_map[res_id]];
		if ((*hw_mgr_res)->res_type == CAM_ISP_RESOURCE_UNINT) {
	...
	if (buf_info->fill_fence) {
		if (io_cfg->direction == CAM_BUF_OUTPUT &&
			...
			map_entries =
				&buf_info->prepare->in_map_entries[num_entries];
			buf_info->prepare->num_in_map_entries++;

		} else {
			CAM_ERR(CAM_ISP,
				"dir: %d max_ln:%d max_out: %u in: %u out %u",
				io_cfg->direction,
				buf_info->prepare->max_in_map_entries,
				buf_info->prepare->max_out_map_entries,
				buf_info->prepare->num_in_map_entries,
...
static int cam_isp_add_io_buffers_util(struct cam_isp_io_buf_info *buf_info,
				       struct cam_buf_io_cfg *io_cfg,
				       struct cam_isp_resource_node *res)
{
	...
	if (res->res_id != io_cfg->resource_type) {
		CAM_ERR(CAM_ISP, "err res id:%d io res id:%d", res->res_id,
			io_cfg->resource_type);
		return -EINVAL;
	}
	...
	for (plane_id = 0; plane_id < CAM_PACKET_MAX_PLANES; plane_id++) {
		...
		rc = res->hw_intf->hw_ops.process_cmd(
			res->hw_intf->hw_priv, secure_mode_cmd, &secure_mode,
			sizeof(struct cam_isp_hw_get_cmd_update));
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Get secure mode failed cmd_type %d res_id %d",
				secure_mode_cmd, res->res_id);
			return -EINVAL;
		}

		is_buf_secure =
			cam_mem_is_secure_buf(io_cfg->mem_handle[plane_id]);
		...
		rc = cam_mem_get_io_buf(
			io_cfg->mem_handle[plane_id], mmu_hdl,
			&io_addr[plane_id], &size, NULL,
			(!plane_id && (io_cfg->direction == CAM_BUF_OUTPUT)) ?
				buf_info->prepare->buf_tracker :
				NULL);
		if (rc) {
		...
		CAM_DBG(CAM_ISP,
			...
			io_cfg->mem_handle[plane_id]);

		CAM_DBG(CAM_ISP, "mmu_hdl=0x%x, size=%d, end=0x%x", mmu_hdl,
			(int)size, io_addr[plane_id] + size);
	}
	...
	rc = res->hw_intf->hw_ops.process_cmd(
		res->hw_intf->hw_priv, bus_update_cmd, &update_buf,
		sizeof(struct cam_isp_hw_get_cmd_update));
	...
	if (bus_port_update.fh_enabled) {
		...
		CAM_DBG(CAM_ISP, "Frame header enabled for res: 0x%x iova: %pK",
			buf_info->frame_hdr->frame_header_res_id,
			bus_port_update.frame_header);
	}
	...
	if (io_cfg->direction == CAM_BUF_OUTPUT) {
		if (buf_info->fill_fence) {
			num_entries =
				buf_info->prepare->num_out_map_entries - 1;
			out_map_entry =
				&buf_info->prepare->out_map_entries[num_entries];
			if (!out_map_entry) {
			...
			new_head_entry = list_first_entry_or_null(
				buf_info->prepare->buf_tracker,
				struct cam_smmu_buffer_tracker, list);
			if (new_head_entry &&
			    old_head_entry != new_head_entry) {
				out_map_entry->buffer_tracker = new_head_entry;
				CAM_DBG(CAM_ISP,
					"[SMMU_BT] Tracking io_buf, buf_handle: 0x%x, fd: 0x%x, res_id: %d",
					io_cfg->mem_handle[0],
					out_map_entry->buffer_tracker->ion_fd,
					res->res_id);
			}
		}
	}
...
static int cam_isp_add_io_buffers_mc(uint64_t *mc_io_cfg,
				     struct cam_isp_io_buf_info *io_info,
				     uint8_t num_ports, uint32_t ctxt_id)
{
	uint32_t bytes_used;
	uint32_t kmd_buf_remain_size;
	...
	rc = cam_isp_add_cmd_buf_update(
		NULL, io_info->hw_intf, CAM_ISP_HW_CMD_MC_CTXT_SEL,
		CAM_ISP_HW_CMD_MC_CTXT_SEL, (void *)cmd_buf_addr,
		kmd_buf_remain_size, (void *)(&ctxt_id), &bytes_used);
	...
	for (i = 0; i < num_ports; i++) {
		rc = cam_isp_io_buf_get_entries_util(
			io_info,
			(struct cam_buf_io_cfg *)
				mc_io_cfg[(max_out * ctxt_id) + i],
			&hw_mgr_res);
		...
		rc = cam_isp_add_io_buffers_util(
			io_info,
			(struct cam_buf_io_cfg *)
				mc_io_cfg[(max_out * ctxt_id) + i],
			res);
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"ctxt[%d] io_cfg[%d] add buf failed rc %d",
				ctxt_id, i, rc);
			return rc;
		}
	}
...
int cam_isp_add_io_buffers(struct cam_isp_io_buf_info *io_info)
{
	...
	io_cfg = (struct cam_buf_io_cfg
			  *)((uint8_t *)&io_info->prepare->packet->payload +
	...
	if (major_version == 3) {
		mc_cfg = vzalloc(sizeof(uint64_t) * CAM_ISP_MULTI_CTXT_MAX *
				 (max_out_res));
		if (!mc_cfg) {
			CAM_ERR(CAM_ISP,
	...
	for (i = 0; i < io_info->prepare->packet->num_io_configs; i++) {
		if (major_version == 3) {
			...
			mc_cfg[(max_out_res * ctxt_id) + num_ports[ctxt_id]] =
				(uint64_t)&io_cfg[i];
			num_ports[ctxt_id]++;
		} else {
			rc = cam_isp_io_buf_get_entries_util(
				io_info, &io_cfg[i], &hw_mgr_res);
			if (rc == -ENOMSG) {
				rc = 0;
				continue;
			} else if (rc) {
				CAM_ERR(CAM_ISP, "io_cfg[%d] failed rc %d", i,
					rc);
				return rc;
			}
			...
			rc = cam_isp_add_io_buffers_util(io_info, &io_cfg[i],
							 res);
			if (rc) {
				CAM_ERR(CAM_ISP,
					"io_cfg[%d] add buf failed rc %d", i,
					rc);
				return rc;
			}
		}
	...
	if (major_version == 3) {
		for (i = 0; i < CAM_ISP_MULTI_CTXT_MAX; i++) {
			if (!num_ports[i])
				continue;
			rc = cam_isp_add_io_buffers_mc(mc_cfg, io_info,
						       num_ports[i], i);
			if (rc) {
				CAM_ERR(CAM_ISP,
					"MC context[%u] failed for base[%d]", i,
					io_info->base->idx);
				goto err;
			}
		}
	...
	CAM_DBG(CAM_ISP, "io_cfg_used_bytes %d, fill_fence %d", bytes_updated,
		io_info->fill_fence);
     ...
     */
		...
		cam_isp_update_hw_entry(CAM_ISP_IOCFG_BL, io_info->prepare,
					io_info->kmd_buf_info, bytes_updated,
					false);
	}
...
int cam_isp_add_reg_update(struct cam_hw_prepare_update_args *prepare,
			   struct list_head *res_list_isp_src,
			   uint32_t base_idx,
			   struct cam_kmd_buf_info *kmd_buf_info, bool combine)
{
	int rc = -EINVAL;
	struct cam_isp_resource_node *res;
	struct cam_isp_hw_mgr_res *hw_mgr_res;
...
int cam_isp_add_go_cmd(struct cam_hw_prepare_update_args *prepare,
		       struct cam_isp_resource_node *res,
		       struct cam_kmd_buf_info *kmd_buf_info, bool combine)
{
	int rc;
	...
	reg_update_size = 0;
	if (kmd_buf_info->size > (kmd_buf_info->used_bytes + reg_update_size)) {
		kmd_buf_remain_size =
			kmd_buf_info->size -
			(kmd_buf_info->used_bytes + reg_update_size);
	} else {
	...
	rc = res->process_cmd(res->res_priv, CAM_ISP_HW_CMD_FE_TRIGGER_CMD,
			      &get_regup,
			      sizeof(struct cam_isp_hw_get_cmd_update));
	if (rc)
		return rc;

	CAM_DBG(CAM_ISP, "GO_CMD added for RD res %d hw_id %d", res->res_type,
		res->hw_intf->hw_idx);
	reg_update_size += get_regup.cmd.used_bytes;
...
int cam_isp_add_comp_wait(struct cam_hw_prepare_update_args *prepare,
			  struct list_head *res_list_isp_src, uint32_t base_idx,
			  struct cam_kmd_buf_info *kmd_buf_info)
{
	int rc = -EINVAL;
	struct cam_isp_hw_mgr_res *hw_mgr_res;
	...
	if (kmd_buf_info->size > (kmd_buf_info->used_bytes + add_wait_size)) {
		kmd_buf_remain_size =
			kmd_buf_info->size -
			(kmd_buf_info->used_bytes + add_wait_size);
	} else {
		CAM_ERR(CAM_ISP, "no free mem %d %d %d", base_idx,
			kmd_buf_info->size,
			kmd_buf_info->used_bytes + add_wait_size);
		rc = -EINVAL;
		return rc;
	...
	rc = hw_intf->hw_ops.process_cmd(
		hw_intf->hw_priv, CAM_ISP_HW_CMD_ADD_WAIT, &add_wait,
		sizeof(struct cam_isp_hw_get_cmd_update));
...
int cam_isp_add_wait_trigger(struct cam_hw_prepare_update_args *prepare,
			     struct list_head *res_list_isp_src,
			     uint32_t base_idx,
			     struct cam_kmd_buf_info *kmd_buf_info,
			     bool trigger_cdm_en)
{
	...
	rc = hw_intf->hw_ops.process_cmd(
		hw_intf->hw_priv, CAM_ISP_HW_CMD_ADD_WAIT_TRIGGER, &add_trigger,
		sizeof(struct cam_isp_hw_get_cmd_update));
...
int cam_isp_add_csid_reg_update(struct cam_hw_prepare_update_args *prepare,
				struct cam_kmd_buf_info *kmd_buf_info,
				void *args, bool combine)
{
	int rc;
	struct cam_isp_resource_node *res;
	...
	if (kmd_buf_info->size <=
	    (kmd_buf_info->used_bytes + reg_update_size)) {
		CAM_ERR(CAM_ISP, "no free mem %u %u %u", kmd_buf_info->size,
			kmd_buf_info->used_bytes + reg_update_size);
		return -EINVAL;
	}
	...
	rc = res->hw_intf->hw_ops.process_cmd(
		res->hw_intf->hw_priv, CAM_ISP_HW_CMD_GET_REG_UPDATE, rup_args,
		sizeof(struct cam_isp_csid_reg_update_args));
	if (rc)
		return rc;

	CAM_DBG(CAM_ISP, "Reg update added for res %d hw_id %d", res->res_id,
		res->hw_intf->hw_idx);
...
int cam_isp_add_csid_offline_cmd(struct cam_hw_prepare_update_args *prepare,
				 struct cam_isp_resource_node *res,
				 struct cam_kmd_buf_info *kmd_buf_info,
				 bool combine)
{
	...
	go_cmd_size = 0;
	if (kmd_buf_info->size > (kmd_buf_info->used_bytes + go_cmd_size)) {
		kmd_buf_remain_size = kmd_buf_info->size -
	...
	rc = res->hw_intf->hw_ops.process_cmd(res->hw_intf->hw_priv,
					      ...
					      &go_args, sizeof(go_args));
	if (rc)
		return rc;

	CAM_DBG(CAM_ISP, "offline cmd update added for CSID: %u res: %d",
	...
	if (go_cmd_size)
		cam_isp_update_hw_entry(CAM_ISP_COMMON_CFG_BL, prepare,
					kmd_buf_info, go_cmd_size, combine);
	return 0;
}

int cam_isp_get_cmd_buf_count(struct cam_hw_prepare_update_args *prepare,
