...
static int cam_vfe_get_dt_properties(struct cam_hw_soc_info *soc_info)
{
	...
	if (num_ubwc_cfg < 0 || num_ubwc_cfg > UBWC_STATIC_CONFIG_MAX) {
		CAM_ERR(CAM_ISP, "wrong num_ubwc_cfg: %d", num_ubwc_cfg);
		rc = num_ubwc_cfg;
		goto pid;
	}
			       ...
			       irq_handler_t vfe_irq_handler, void *irq_data)
{
	...
	if (!soc_private->is_ife_lite) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP, "Failed to query DRV enable rc:%d",
				rc);
			goto free_soc_private;
		}
	}
			   ...
			   const char *clk_name)
{
	...
	if (!strcmp(clk_name, CAM_VFE_DSP_CLK_NAME)) {
		if (soc_private->dsp_clk_index == -1) {
			CAM_ERR(CAM_ISP, "DSP clock not supported for vfe: %d",
				soc_info->index);
			rc = -EPERM;
			goto end;
		}
			    ...
			    const char *clk_name)
{
	...
	if (!strcmp(clk_name, CAM_VFE_DSP_CLK_NAME)) {
		...
		rc = cam_soc_util_clk_disable(soc_info, CAM_CLK_SW_CLIENT_IDX,
					      true, soc_private->dsp_clk_index);
		if (rc)
			CAM_ERR(CAM_ISP, "Error disable dsp clk failed rc=%d",
				rc);
	}
