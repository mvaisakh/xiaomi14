...
struct cam_vfe_top_ver4_priv {
	...
	uint32_t sof_cnt;
	struct cam_vfe_top_ver4_perf_counter_cfg
		perf_counters[CAM_VFE_PERF_CNT_MAX];
...
static inline int cam_vfe_top_ver4_get_hw_ctxt_from_irq_status(
	struct cam_vfe_mux_ver4_data *vfe_priv, uint32_t irq_status)
{
	int i;
...
static int
cam_vfe_top_ver4_get_path_port_map(struct cam_vfe_top_ver4_priv *top_priv,
				   void *cmd_args, uint32_t arg_size)
{
	struct cam_isp_hw_path_port_map *arg = cmd_args;
	struct cam_vfe_top_ver4_hw_info *hw_info =
		top_priv->common_data.hw_info;
	int i;
...
static int
cam_vfe_top_ver4_pdaf_lcr_config(struct cam_vfe_top_ver4_priv *top_priv,
				 void *cmd_args, uint32_t arg_size)
{
	struct cam_vfe_top_ver4_hw_info *hw_info;
	...
	if (!cdm_args->res) {
		CAM_ERR(CAM_ISP, "VFE:%u Error, Invalid res",
	...
	if (!hw_info->num_pdaf_lcr_res || !hw_info->pdaf_lcr_res_mask) {
		CAM_DBG(CAM_ISP, "VFE:%u PDAF LCR is not supported",
			top_priv->top_common.hw_idx);
		return 0;
	}
	...
	if (!cdm_util_ops) {
		CAM_ERR(CAM_ISP, "VFE:%u Invalid CDM ops",
			top_priv->top_common.hw_idx);
		return -EINVAL;
	}

	for (i = 0; i < hw_info->num_pdaf_lcr_res; i++)
		if (cfg_args->ife_src_res_id ==
	...
	if ((size * 4) > cdm_args->cmd.size) {
		CAM_ERR(CAM_ISP,
			...
			top_priv->top_common.hw_idx, cdm_args->cmd.size,
			(size * 4));
		return -EINVAL;
	}

	if (cfg_args->is_init) {
		reg_val_pair[reg_val_idx++] =
					 ...
					 void *cmd_args, uint32_t arg_size)
{
	...
	if (!soc_private) {
		CAM_ERR(CAM_ISP, "VFE:%u soc_private is null",
			top_priv->top_common.hw_idx);
		return -EINVAL;
	}
	...
	mem_base = CAM_SOC_GET_REG_MAP_CAM_BASE(top_priv->top_common.soc_info,
						VFE_CORE_BASE_IDX);
...
static int cam_vfe_top_fs_update(struct cam_vfe_top_ver4_priv *top_priv,
				 void *cmd_args, uint32_t arg_size)
{
	...
	struct cam_vfe_top_ver4_debug_reg_info (*status_list)[][8])
{
	...
	for (i = 0; i < num_reg; i++) {
		...
		for (j = 0; j < 8; j++) {
			...
			CAM_INFO_BUF(CAM_ISP, log_buf, 1024, &len,
				     "%s [I:%u V:%u R:%u]",
	...
	struct cam_vfe_mux_ver4_data *vfe_priv)
{
	...
	val = cam_io_r(base + common_data->common_reg->pdaf_violation_status),
	...
	for (i = 0; i < common_data->hw_info->num_pdaf_violation_errors; i++) {
		if (common_data->hw_info->pdaf_violation_desc[i].bitmask &
		    val) {
			CAM_ERR(CAM_ISP, "%s",
				common_data->hw_info->pdaf_violation_desc[i]
					.desc);
		}
	}
}
	...
	struct cam_vfe_top_ver4_priv *top_priv)
{
	...
	soc_info = top_priv->top_common.soc_info;
	base = soc_info->reg_map[VFE_CORE_BASE_IDX].mem_base;
	val = cam_io_r(base + common_data->common_reg->ipp_violation_status),

	CAM_INFO(CAM_ISP, "VFE[%u] IPP Violation status 0x%x", soc_info->index,
		 val);
...
static void
cam_vfe_top_ver4_print_top_irq_error(struct cam_vfe_mux_ver4_data *vfe_priv,
				     uint32_t irq_status)
{
	uint32_t i = 0;
	struct cam_vfe_top_ver4_priv *top_priv;
...
static void
cam_vfe_top_dump_perf_counters(const char *event, const char *res_name,
			       struct cam_vfe_top_ver4_priv *top_priv)
{
	int i;
	void __iomem *mem_base;
	...
	for (i = 0; i < top_priv->common_data.common_reg->num_perf_counters;
	     i++) {
		if (top_priv->perf_counters[i].dump_counter) {
			CAM_INFO(
				CAM_ISP,
				"VFE [%u] on %s %s counter: %d pixel_cnt: %d line_cnt: %d stall_cnt: "
				"%d always_cnt: %d status: 0x%x",
				top_priv->common_data.hw_intf->hw_idx, res_name,
				event, (i + 1),
				cam_io_r_mb(mem_base +
					    common_reg->perf_count_reg[i]
						    .perf_pix_count),
				cam_io_r_mb(mem_base +
					    common_reg->perf_count_reg[i]
						    .perf_line_count),
...
static void
cam_vfe_top_ver4_print_debug_reg_status(struct cam_vfe_top_ver4_priv *top_priv)
{
	struct cam_vfe_top_ver4_reg_offset_common *common_reg;
	uint32_t val = 0;
	uint32_t num_reg = 0;
	...
	while (i < num_reg) {
		for (j = 0; j < 4 && i < num_reg; j++, i++) {
			val = cam_io_r(base + common_reg->top_debug[i]);
			reg_val[i] = val;
			CAM_INFO_BUF(CAM_ISP, log_buf, CAM_VFE_LEN_LOG_BUF,
				     &len, "VFE[%u] status %2d : 0x%08x",
				     soc_info->index, i, val);
		}
		CAM_INFO(CAM_ISP, "VFE[%u]: Top Debug Status: %s",
			 soc_info->index, log_buf);
		len = 0;
	}
...
int cam_vfe_top_ver4_dump_timestamps(struct cam_vfe_top_ver4_priv *top_priv,
				     int res_id)
{
	uint32_t i;
	...
	for (i = 0; i < top_priv->top_common.num_mux; i++) {
		...
		if (!res || !res->res_priv) {
			CAM_ERR_RATE_LIMIT(
				CAM_ISP, "VFE[%u] Invalid Resource",
	...
	CAM_INFO(
		CAM_ISP,
		"VFE[%u] CAMIF Error timestamp:[%lld.%09lld] SOF timestamp:[%lld.%09lld] "
		"EPOCH timestamp:[%lld.%09lld] EOF timestamp:[%lld.%09lld]",
		vfe_priv->hw_intf->hw_idx, vfe_priv->error_ts.tv_sec,
		vfe_priv->error_ts.tv_nsec, vfe_priv->sof_ts.tv_sec,
		vfe_priv->sof_ts.tv_nsec, vfe_priv->epoch_ts.tv_sec,
		vfe_priv->epoch_ts.tv_nsec, vfe_priv->eof_ts.tv_sec,
	...
	struct cam_vfe_top_ver4_priv *top_priv, void *cmd_args)
{
	...
	bus_overflow_status =
		cam_io_r(soc_info->reg_map[VFE_CORE_BASE_IDX].mem_base +
	...
	if (soc_private->is_ife_lite)
		...
		CAM_ERR(CAM_ISP,
			"VFE[%u] sof_cnt:%d src_clk sw_client:%lu hw_client:[%lu %lu] "
			"overflow:%s violation:%s",
			soc_info->index, top_priv->sof_cnt,
			soc_info->applied_src_clk_rates.sw_client,
			soc_info->applied_src_clk_rates
				.hw_client[soc_info->index]
				.high,
			soc_info->applied_src_clk_rates
				.hw_client[soc_info->index]
				.low,
...
static int cam_vfe_core_config_control(struct cam_vfe_top_ver4_priv *top_priv,
				       void *cmd_args, uint32_t arg_size)
{
...
static int cam_vfe_init_config_update(void *cmd_args, uint32_t arg_size)
{
	...
	mux_data->epoch_factor = init_cfg->init_config->epoch_cfg.epoch_factor;

	CAM_DBG(CAM_ISP,
		"VFE:%u Init Update for res_name: %s epoch_factor: %u%%",
		rsrc_node->hw_intf->hw_idx, rsrc_node->res_name,
...
static int
cam_vfe_top_ver4_mux_get_reg_update(struct cam_vfe_top_ver4_priv *top_priv,
				    void *cmd_args, uint32_t arg_size)
{
...
static int cam_vfe_top_ver4_get_data(struct cam_vfe_top_ver4_priv *top_priv,
				     void *cmd_args, uint32_t arg_size)
{
	struct cam_isp_resource_node *res = cmd_args;

	if (res->process_cmd)
		return res->process_cmd(res, CAM_ISP_HW_CMD_CAMIF_DATA,
					cmd_args, arg_size);

	return -EINVAL;
}

int cam_vfe_top_ver4_get_hw_caps(void *device_priv, void *args,
				 uint32_t arg_size)
{
	...
	if (!device_priv || !args) {
		CAM_ERR(CAM_ISP, "Invalid arguments device_priv:%p, args:%p",
			device_priv, args);
		return -EINVAL;
	}
	...
	soc_priv = (struct cam_vfe_soc_private *)
	...
	vfe_cap_info->minor = ((vfe_top_prv->top_common.hw_version) >> 16) &
			      0x0fff;
...
int cam_vfe_top_ver4_init_hw(void *device_priv, void *init_hw_args,
			     uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u hw-version:0x%x", top_priv->top_common.hw_idx,
		top_priv->top_common.hw_version);

	return 0;
}

int cam_vfe_top_ver4_reset(void *device_priv, void *reset_core_args,
			   uint32_t arg_size)
{
...
int cam_vfe_top_acquire_resource(struct cam_isp_resource_node *vfe_full_res,
				 void *acquire_param)
{
	...
	res_data = (struct cam_vfe_mux_ver4_data *)vfe_full_res->res_priv;
	...
	res_data->horizontal_bin = acquire_data->vfe_in.in_port->horizontal_bin;
...
config_done:
	CAM_DBG(CAM_ISP,
		...
		vfe_full_res->hw_intf->hw_idx, vfe_full_res->res_id,
		vfe_full_res->res_name, res_data->dsp_mode, res_data->is_dual,
		res_data->dual_hw_idx);

	return rc;
}

int cam_vfe_top_ver4_reserve(void *device_priv, void *reserve_args,
			     uint32_t arg_size)
{
	struct cam_vfe_top_ver4_priv *top_priv;
			...
			if (top_priv->top_common.mux_rsrc[i].res_state !=
				...
				if (acquire_args->res_id !=
					...
					CAM_ERR(CAM_ISP,
						...
						top_priv->common_data.hw_intf
							->hw_idx);
					rc = -EINVAL;
					break;
				}

				if (!(vfe_priv->hw_ctxt_mask &
					...
					CAM_ERR(CAM_ISP,
						"VFE:%d Duplicate hw ctxt mask: 0x%x for camif "
						"curr_mask_val: 0x%x",
						top_priv->common_data.hw_intf
							->hw_idx,
						acquire_args->hw_ctxt_mask,
...
int cam_vfe_top_ver4_release(void *device_priv, void *release_args,
			     uint32_t arg_size)
{
...
int cam_vfe_top_ver4_start(void *device_priv, void *start_args,
			   uint32_t arg_size)
{
	struct cam_vfe_top_ver4_priv *top_priv;
	struct cam_isp_resource_node *mux_res;
	...
	if (hw_info->hw_state == CAM_HW_STATE_POWER_UP) {
		...
		/* Perf counter config */
		for (i = 0;
		     i < top_priv->common_data.common_reg->num_perf_counters;
		     i++) {
		...
		CAM_ERR(CAM_ISP, "VFE:%u HW not powered up",
...
int cam_vfe_top_ver4_stop(void *device_priv, void *stop_args, uint32_t arg_size)
{
	struct cam_vfe_top_ver4_priv *top_priv;
	struct cam_isp_resource_node *mux_res;
	struct cam_hw_soc_info *soc_info = NULL;
		...
		for (i = 0; i < top_priv->top_common.num_mux; i++) {
			if (top_priv->top_common.mux_rsrc[i].res_id ==
				...
				if (!top_priv->top_common.skip_data_rst_on_stop)
					top_priv->top_common.req_clk_rate[i] =
						0;
...
int cam_vfe_top_ver4_read(void *device_priv, void *read_args, uint32_t arg_size)
{
	return -EPERM;
}

int cam_vfe_top_ver4_write(void *device_priv, void *write_args,
			   uint32_t arg_size)
{
	return -EPERM;
}
				 ...
				 void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_GET_CHANGE_BASE:
		rc = cam_vfe_top_ver4_mux_get_base(top_priv, cmd_args,
						   arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_CAMIF_DATA:
		rc = cam_vfe_top_ver4_get_data(top_priv, cmd_args, arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_APPLY_CLK_BW_UPDATE:
		rc = cam_vfe_top_apply_clk_bw_update(&top_priv->top_common,
						     cmd_args, arg_size);
		break;
	...
	case CAM_ISP_HW_CMD_IFE_DEBUG_CFG: {
		...
		top_priv->enable_ife_frame_irqs =
			debug_cfg->enable_ife_frame_irqs;
	} break;
	case CAM_ISP_HW_CMD_GET_SET_PRIM_SOF_TS_ADDR: {
		...
		if (sof_addr_args->get_addr) {
			...
			rc = cam_vfe_top_ver4_set_primary_sof_timer_reg_addr(
				top_priv, sof_addr_args);
	} break;
	default:
		rc = -EINVAL;
...
static int
cam_vfe_get_evt_payload(struct cam_vfe_mux_ver4_data *vfe_priv,
...
static int
cam_vfe_top_put_evt_payload(struct cam_vfe_mux_ver4_data *vfe_priv,
			    struct cam_vfe_top_irq_evt_payload **evt_payload)
{
	unsigned long flags;
	...
	if (*evt_payload == NULL) {
		CAM_ERR(CAM_ISP, "VFE:%u No payload to put",
				       ...
				       struct cam_irq_th_payload *th_payload)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u IRQ status_0: 0x%X status_1: 0x%X",
	...
	if (th_payload->evt_status_arr[vfe_priv->common_reg
		...
		if (vfe_priv->top_priv->sof_ts_reg_addr.curr0_ts_addr &&
			...
			evt_payload->ts.sof_ts =
				cam_io_r_mb(vfe_priv->top_priv->sof_ts_reg_addr
						    .curr1_ts_addr);
			evt_payload->ts.sof_ts =
				(evt_payload->ts.sof_ts << 32) |
				cam_io_r_mb(vfe_priv->top_priv->sof_ts_reg_addr
						    .curr0_ts_addr);
		}
		...
		trace_cam_log_event(
			"SOF", "TOP_HALF",
			th_payload->evt_status_arr
				[vfe_priv->common_reg->frame_timing_irq_reg_idx],
			vfe_res->hw_intf->hw_idx);
	}
...
static enum cam_vfe_top_ver4_fsm_state
cam_vfe_top_ver4_fsm_next_state(struct cam_isp_resource_node *res,
				enum cam_vfe_top_ver4_fsm_state state)
{
	switch (state) {
	case VFE_TOP_VER4_FSM_SOF:
		return (res->is_rdi_primary_res) ? VFE_TOP_VER4_FSM_EOF :
						   VFE_TOP_VER4_FSM_EPOCH;
...
static const char *
cam_vfe_top_ver4_fsm_state_to_string(enum cam_vfe_top_ver4_fsm_state state)
{
	switch (state) {
	case VFE_TOP_VER4_FSM_SOF:
		return "SOF";
...
typedef int (*cam_vfe_handle_frame_irq_t)(
	struct cam_vfe_mux_ver4_data *vfe_priv,
			      ...
			      struct cam_isp_hw_event_info *evt_info)
{
	if ((vfe_priv->enable_sof_irq_debug) &&
		...
		uint32_t frm_irq_status =
			payload->irq_reg_val[vfe_priv->common_reg
						     ->frame_timing_irq_reg_idx];
		int hw_ctxt_id = -1;
			      ...
			      struct cam_isp_hw_event_info *evt_info)
{
	...
	if (vfe_priv->reg_data->is_mc_path)
		hw_ctxt_id = cam_vfe_top_ver4_get_hw_ctxt_from_irq_status(
			vfe_priv, frm_irq_status);
...
static int
__cam_vfe_handle_frame_timing_irqs(struct cam_isp_resource_node *vfe_res,
				   bool event,
				   ...
				   struct cam_isp_hw_event_info *evt_info)
{
	...
	if (!event) {
		...
		if (!(vfe_priv->top_priv->enable_ife_frame_irqs) &&
		    vfe_priv->event_cb)
			vfe_priv->event_cb(vfe_priv->priv, event_type,
					   evt_info);
	}
	vfe_priv->fsm_state =
		cam_vfe_top_ver4_fsm_next_state(vfe_res, vfe_priv->fsm_state);

	return 0;
}

static int
cam_vfe_handle_frame_timing_irqs(struct cam_isp_resource_node *vfe_res,
				 uint32_t irq_status,
				 struct cam_vfe_top_irq_evt_payload *payload,
				 struct cam_isp_hw_event_info *evt_info)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u SOF:%s EPOCH:%s EOF:%s",
		vfe_priv->hw_intf->hw_idx, CAM_BOOL_TO_YESNO(sof),
		CAM_BOOL_TO_YESNO(epoch), CAM_BOOL_TO_YESNO(eof));

	i = (sof ? 1 : 0) + (epoch ? 1 : 0) + (eof ? 1 : 0);
	j = i;

	if (i == vfe_priv->n_frame_irqs)
		CAM_WARN_RATE_LIMIT(CAM_ISP, "VFE:%u top-half delay",
				    vfe_priv->hw_intf->hw_idx);
	...
	while (i > 0) {
		...
		CAM_DBG_PR2(CAM_ISP, "VFE:%u enter state:%s (%d/%d)",
			    vfe_priv->hw_intf->hw_idx,
			    cam_vfe_top_ver4_fsm_state_to_string(
				    vfe_priv->fsm_state),
			    i, j);
		...
		__cam_vfe_handle_frame_timing_irqs(vfe_res, event, event_type,
						   handle_irq_fn, payload,
						   evt_info);

		CAM_DBG_PR2(CAM_ISP, "VFE:%u exit state:%s (%d/%d)",
			    vfe_priv->hw_intf->hw_idx,
			    cam_vfe_top_ver4_fsm_state_to_string(
				    vfe_priv->fsm_state),
			    i, j);
					  ...
					  void *evt_payload_priv)
{
	...
	if (irq_status[vfe_priv->common_reg->frame_timing_irq_reg_idx] &
	    frame_timing_mask) {
		ret = cam_vfe_handle_frame_timing_irqs(
			vfe_res,
			irq_status[vfe_priv->common_reg
					   ->frame_timing_irq_reg_idx] &
				frame_timing_mask,
			payload, &evt_info);
	}
	...
	if (irq_status[CAM_IFE_IRQ_CAMIF_REG_STATUS0] &
		...
		cam_vfe_top_ver4_print_top_irq_error(
			vfe_priv, irq_status[CAM_IFE_IRQ_CAMIF_REG_STATUS0]);
	...
	if (irq_status[vfe_priv->common_reg->frame_timing_irq_reg_idx] &
	    vfe_priv->reg_data->eof_irq_mask)
		cam_vfe_top_dump_perf_counters("EOF", vfe_res->res_name,
					       vfe_priv->top_priv);

	if (irq_status[vfe_priv->common_reg->frame_timing_irq_reg_idx] &
	    vfe_priv->reg_data->sof_irq_mask)
		cam_vfe_top_dump_perf_counters("SOF", vfe_res->res_name,
					       vfe_priv->top_priv);
...
static int cam_vfe_ver4_err_irq_top_half(uint32_t evt_id,
					 struct cam_irq_th_payload *th_payload)
{
	int32_t rc = 0;
	int i;
   ...
   */
	if ((th_payload->evt_status_arr[0] &
	     vfe_priv->reg_data->error_irq_mask)) {
		CAM_ERR(CAM_ISP, "VFE:%u Err IRQ status_0: 0x%X",
			vfe_res->hw_intf->hw_idx,
			th_payload->evt_status_arr[0]);
		CAM_ERR(CAM_ISP, "Stopping further IRQ processing from VFE:%u",
			vfe_res->hw_intf->hw_idx);
		cam_irq_controller_disable_all(vfe_priv->vfe_irq_controller);
		error_flag = true;
	}
	...
	cam_isp_hw_get_timestamp(&evt_payload->ts);
	if (error_flag) {
		vfe_priv->error_ts.tv_sec = evt_payload->ts.mono_time.tv_sec;
		vfe_priv->error_ts.tv_nsec = evt_payload->ts.mono_time.tv_nsec;
	}
...
static int cam_vfe_resource_start(struct cam_isp_resource_node *vfe_res)
{
	...
	CAM_DBG(CAM_ISP,
		"VFE:%u height [0x%x : 0x%x] vbi_val: 0x%x epoch_factor: %u%% "
		"epoch_line_cfg: 0x%x",
		vfe_res->hw_intf->hw_idx, rsrc_data->first_line,
		rsrc_data->last_line, rsrc_data->vbi_value, epoch_factor, val);
   ...
   */
	...
	if (!rsrc_data->frame_irq_handle) {
		rsrc_data->frame_irq_handle = cam_irq_controller_subscribe_irq(
			...
			irq_mask, vfe_res, vfe_res->top_half_handler,
			vfe_res->bottom_half_handler, vfe_res->tasklet_info,
...
skip_frame_irq_subscribe:
	...
	if (!rsrc_data->irq_err_handle) {
		rsrc_data->irq_err_handle = cam_irq_controller_subscribe_irq(
			...
			err_irq_mask, vfe_res, cam_vfe_ver4_err_irq_top_half,
			vfe_res->bottom_half_handler, vfe_res->tasklet_info,
			&tasklet_bh_api, CAM_IRQ_EVT_GROUP_0);
	...
	CAM_DBG(CAM_ISP, "VFE:%u Res: %s Start Done", vfe_res->hw_intf->hw_idx,
		vfe_res->res_name);

	return rc;
}

static int cam_vfe_resource_stop(struct cam_isp_resource_node *vfe_res)
{
	struct cam_vfe_mux_ver4_data *vfe_priv;
	struct cam_vfe_top_ver4_priv *top_priv;
	int rc = 0;
...
skip_core_decfg:
	...
	if (vfe_priv->frame_irq_handle) {
		cam_irq_controller_unsubscribe_irq(vfe_priv->vfe_irq_controller,
	...
	CAM_DBG(CAM_ISP, "VFE:%u Res: %s Stopped", vfe_res->hw_intf->hw_idx,
		vfe_res->res_name);

	return rc;
}

static int cam_vfe_resource_init(struct cam_isp_resource_node *vfe_res,
				 void *init_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u Res: %s Init Done", vfe_res->hw_intf->hw_idx,
		vfe_res->res_name);

	return rc;
}

static int cam_vfe_resource_deinit(struct cam_isp_resource_node *vfe_res,
				   void *deinit_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u Res: %s DeInit Done", vfe_res->hw_intf->hw_idx,
		vfe_res->res_name);
	return rc;
}

int cam_vfe_res_mux_init(struct cam_vfe_top_ver4_priv *top_priv,
			 struct cam_hw_intf *hw_intf,
			 struct cam_hw_soc_info *soc_info, void *vfe_hw_info,
			 struct cam_isp_resource_node *vfe_res,
			 void *vfe_irq_controller)
{
	struct cam_vfe_mux_ver4_data *vfe_priv = NULL;
	...
	vfe_priv = kzalloc(sizeof(struct cam_vfe_mux_ver4_data), GFP_KERNEL);
	if (!vfe_priv)
		return -ENOMEM;
...
int cam_vfe_res_mux_deinit(struct cam_isp_resource_node *vfe_res)
{
	struct cam_vfe_mux_ver4_data *vfe_priv;
	int i = 0;

	if (!vfe_res) {
		CAM_ERR(CAM_ISP, "Error, VFE Node Resource is NULL %pK",
			vfe_res);
		return -ENODEV;
	}
	...
	if (!vfe_priv) {
		CAM_ERR(CAM_ISP, "VFE:%u vfe_priv is NULL %pK",
			vfe_res->hw_intf->hw_idx, vfe_priv);
		return -ENODEV;
	}
			  ...
			  struct cam_vfe_top **vfe_top_ptr)
{
	...
	if (!vfe_top) {
		CAM_DBG(CAM_ISP, "VFE:%u Error, Failed to alloc for vfe_top",
			hw_intf->hw_idx);
		rc = -ENOMEM;
		goto end;
	}

	top_priv = kzalloc(sizeof(struct cam_vfe_top_ver4_priv), GFP_KERNEL);
	if (!top_priv) {
		CAM_DBG(CAM_ISP,
	...
	for (i = 0, j = 0;
	     i < top_priv->top_common.num_mux && j < hw_info->num_rdi; i++) {
		...
		if (hw_info->mux_type[i] == CAM_VFE_CAMIF_VER_4_0) {
			...
			rc = cam_vfe_res_mux_init(
				top_priv, hw_intf, soc_info,
		...
		} else if (hw_info->mux_type[i] == CAM_VFE_RDI_VER_1_0) {
	...
	*vfe_top_ptr = vfe_top;
	...
	if (top_priv->common_data.common_reg->num_perf_counters >
	...
	for (--i; i >= 0; i--) {
		if (hw_info->mux_type[i] == CAM_VFE_CAMIF_VER_4_0) {
			if (cam_vfe_res_mux_deinit(
				    &top_priv->top_common.mux_rsrc[i]))
				CAM_ERR(CAM_ISP, "VFE:%u Camif Deinit failed",
					hw_intf->hw_idx);
		} else {
			if (cam_vfe_res_mux_deinit(
				    &top_priv->top_common.mux_rsrc[i]))
