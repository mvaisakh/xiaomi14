	...
	struct cam_isp_resource_node *camif_lite_res, void *acquire_param)
{
	...
	CAM_DBG(CAM_ISP, "Acquired VFE:%d CAMIF LITE:%d %s sync_mode=%d",
		...
		camif_lite_res->res_name, camif_lite_data->sync_mode);
	return 0;
}
				       ...
				       struct cam_irq_th_payload *th_payload)
{
	...
	CAM_DBG(CAM_ISP,
		"VFE:%d CAMIF LITE:%d %s IRQ status_0: 0x%X status_1: 0x%X status_2: "
		"0x%X",
		camif_lite_node->hw_intf->hw_idx, camif_lite_node->res_id,
		camif_lite_node->res_name, th_payload->evt_status_arr[0],
						     ...
						     void *evt_payload_priv)
{
	...
	if (irq_status[CAM_IFE_IRQ_CAMIF_REG_STATUS1] &
		...
		camif_lite_priv->sof_ts.tv_sec = payload->ts.mono_time.tv_sec;
		camif_lite_priv->sof_ts.tv_nsec = payload->ts.mono_time.tv_nsec;
	...
	if (irq_status[CAM_IFE_IRQ_CAMIF_REG_STATUS1] &
		...
		CAM_DBG(CAM_ISP, "VFE:%d CAMIF LITE:%d %s Received EPOCH",
			...
			camif_lite_node->res_name);
		ret = CAM_VFE_IRQ_STATUS_SUCCESS;
		camif_lite_priv->epoch_ts.tv_sec = payload->ts.mono_time.tv_sec;
	...
	if (irq_status[CAM_IFE_IRQ_CAMIF_REG_STATUS1] &
		...
		CAM_DBG(CAM_ISP, "VFE:%d CAMIF LITE:%d %s Received EOF",
			...
			camif_lite_node->res_name);
		ret = CAM_VFE_IRQ_STATUS_SUCCESS;
		camif_lite_priv->eof_ts.tv_sec = payload->ts.mono_time.tv_sec;
		camif_lite_priv->eof_ts.tv_nsec = payload->ts.mono_time.tv_nsec;
	...
	if (status_0) {
		...
		CAM_INFO(CAM_ISP, "ERROR timestamp:[%lld.%09lld]",
			 camif_lite_priv->error_ts.tv_sec,
			 camif_lite_priv->error_ts.tv_nsec);
		...
		if (camif_lite_node->is_rdi_primary_res)
			CAM_INFO(
				CAM_ISP,
				"SOF timestamp:[%lld.%09lld] EPOCH timestamp:[%lld.%09lld] EOF "
				"timestamp:[%lld.%09lld]",
