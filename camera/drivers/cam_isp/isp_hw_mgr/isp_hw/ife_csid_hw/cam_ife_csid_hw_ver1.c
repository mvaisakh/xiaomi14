...
static const struct cam_ife_csid_irq_desc ver1_path_irq_desc[] = {
	...
	{
		.desc = "PATH_OVERFLOW_RECOVERY: Back pressure/output fifo ovrfl",
	},
	{
...
static int cam_ife_csid_ver1_set_debug(struct cam_ife_csid_ver1_hw *csid_hw,
				       uint32_t debug_val)
{
	int bit_pos = 0;
	uint32_t val;
...
int cam_ife_csid_ver1_get_hw_caps(void *hw_priv, void *get_hw_cap_args,
				  uint32_t arg_size)
{
	int rc = 0;
...
static int cam_ife_csid_ver1_prepare_reset(struct cam_ife_csid_ver1_hw *csid_hw)
{
...
static int cam_ife_csid_ver1_hw_reset(struct cam_ife_csid_ver1_hw *csid_hw)
{
	int rc = 0;
	...
	if (rem_jiffies == 0) {
		...
		if (val & 0x1) {
			...
			CAM_DBG(CAM_ISP, "CSID:%d HW reset completed %d",
				csid_hw->hw_intf->hw_idx, rem_jiffies);
			goto end;
		}
...
static int cam_ife_csid_ver1_sw_reset(struct cam_ife_csid_ver1_hw *csid_hw)
{
	int rc = 0;
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
   ...
   */
	...
	spin_unlock_irqrestore(&csid_hw->hw_info->hw_lock, flags);
	CAM_DBG(CAM_ISP, "CSID[%d] top reset start", csid_hw->hw_intf->hw_idx);
...
static int cam_ife_csid_ver1_global_reset(struct cam_ife_csid_ver1_hw *csid_hw)
{
	int rc = 0;
...
static int cam_ife_csid_path_reset(struct cam_ife_csid_ver1_hw *csid_hw,
...
int cam_ife_csid_ver1_reset(void *hw_priv, void *reset_args, uint32_t arg_size)
{
	struct cam_hw_info *hw_info;
	struct cam_ife_csid_ver1_hw *csid_hw;
	struct cam_csid_reset_cfg_args *reset;
	...
	CAM_DBG(CAM_ISP, "CSID[%d] reset type :%d", csid_hw->hw_intf->hw_idx,
		reset->reset_type);
...
static int
cam_ife_csid_ver1_deinit_rdi_path(struct cam_ife_csid_ver1_hw *csid_hw,
				  struct cam_isp_resource_node *res)
{
	...
	if (path_reg->overflow_ctrl_en)
		cam_io_w_mb(0, mem_base + path_reg->err_recovery_cfg0_addr);
...
static int
cam_ife_csid_ver1_deinit_udi_path(struct cam_ife_csid_ver1_hw *csid_hw,
				  struct cam_isp_resource_node *res)
{
	const struct cam_ife_csid_ver1_path_reg_info *path_reg;
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
...
static int
cam_ife_csid_ver1_deinit_pxl_path(struct cam_ife_csid_ver1_hw *csid_hw,
				  struct cam_isp_resource_node *res)
{
	int rc = 0;
	void __iomem *mem_base;
...
static int cam_ife_csid_ver1_stop_pxl_path(struct cam_ife_csid_ver1_hw *csid_hw,
					   struct cam_isp_resource_node *res,
					   enum cam_ife_csid_halt_cmd stop_cmd)
{
	...
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base + path_reg->irq_mask_addr);
...
static int cam_ife_csid_ver1_stop_rdi_path(struct cam_ife_csid_ver1_hw *csid_hw,
					   struct cam_isp_resource_node *res,
					   enum cam_ife_csid_halt_cmd stop_cmd)
{
	const struct cam_ife_csid_ver1_path_reg_info *path_reg;
...
static int cam_ife_csid_ver1_stop_udi_path(struct cam_ife_csid_ver1_hw *csid_hw,
					   struct cam_isp_resource_node *res,
					   enum cam_ife_csid_halt_cmd stop_cmd)
{
	const struct cam_ife_csid_ver1_path_reg_info *path_reg;
...
static int
cam_ife_csid_hw_ver1_path_cfg(struct cam_ife_csid_ver1_hw *csid_hw,
			      struct cam_ife_csid_ver1_path_cfg *path_cfg,
...
static int
cam_ife_csid_ver1_rx_capture_config(struct cam_ife_csid_ver1_hw *csid_hw)
{
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
...
static int
cam_ife_csid_ver1_tpg_config(struct cam_ife_csid_ver1_hw *csid_hw,
			     struct cam_csid_hw_reserve_resource_args *reserve)
{
	...
	default:
		CAM_ERR(CAM_ISP, "CSID[%d] invalid input format %d",
			csid_hw->hw_intf->hw_idx, reserve->in_port->format);
		return -EINVAL;
	}
...
static int cam_ife_csid_ver1_tpg_start(struct cam_ife_csid_ver1_hw *csid_hw)
{
	int rc = 0;
	uint32_t val = 0;
	uint32_t i;
	...
	/*Enable the TPG */
	CAM_DBG(CAM_ISP, "CSID:%d start CSID TPG", csid_hw->hw_intf->hw_idx);
	...
	for (i = 0; i < 16; i++) {
		val = cam_io_r_mb(soc_info->reg_map[0].mem_base + base + i * 4);
		CAM_DBG(CAM_ISP, "reg 0x%x = 0x%x", (base + i * 4), val);
	}
	...
	if (csid_hw->tpg_cfg.usage_type) {
		rc = cam_ife_csid_enable_ife_force_clock_on(
			soc_info, csid_reg->tpg_reg->cpas_ife_reg_offset);
		if (rc)
			return rc;
	}
...
static int cam_ife_csid_ver1_tpg_stop(struct cam_ife_csid_ver1_hw *csid_hw)
{
	...
	/* disable the TPG */
	CAM_DBG(CAM_ISP, "CSID:%d stop CSID TPG", csid_hw->hw_intf->hw_idx);

	/* Disable the IFE force clock on for dual isp case */
	if (csid_hw->tpg_cfg.usage_type) {
		rc = cam_ife_csid_disable_ife_force_clock_on(
			soc_info, csid_reg->tpg_reg->cpas_ife_reg_offset);
		CAM_DBG(CAM_ISP, "Dual isp case: Disable IFE force clk. rc %d",
			rc);
	}
...
static int cam_ife_csid_ver1_init_tpg_hw(struct cam_ife_csid_ver1_hw *csid_hw)
{
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	val = (tpg_reg->num_frames << tpg_reg->num_frame_shift) |
	      csid_hw->tpg_cfg.vc;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->vc_cfg0_addr);
	...
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + tpg_reg->vc_cfg1_addr);

	cam_io_w_mb(tpg_reg->lfsr_seed,
		    soc_info->reg_map[0].mem_base + tpg_reg->lfsr_seed_addr);

	val = csid_hw->tpg_cfg.width << tpg_reg->width_shift |
	      csid_hw->tpg_cfg.height;
	cam_io_w_mb(val,
		    soc_info->reg_map[0].mem_base + tpg_reg->dt_n_cfg_0_addr);

	cam_io_w_mb(csid_hw->tpg_cfg.dt,
		    soc_info->reg_map[0].mem_base +
...
static int
cam_ife_csid_hw_ver1_rx_cfg(struct cam_ife_csid_ver1_hw *csid_hw,
			    struct cam_csid_hw_reserve_resource_args *reserve)
{
	...
	csid_hw->rx_cfg.epd_supported = reserve->in_port->epd_supported;
	...
	CAM_DBG(CAM_ISP,
		...
		reserve->in_port->res_type, csid_hw->counters.csi2_reserve_cnt);
	...
	struct cam_csid_hw_reserve_resource_args *reserve, uint32_t cid)
{
	...
	cam_ife_csid_hw_ver1_path_cfg(csid_hw, path_cfg, reserve, cid);

	if (reserve->res_type == CAM_ISP_IFE_IN_RES_TPG) {
		rc = cam_ife_csid_ver1_tpg_config(csid_hw, reserve);

		if (rc)
			CAM_ERR(CAM_ISP, "CSID[%d] Res_id %d tpg config fail",
				csid_hw->hw_intf->hw_idx, reserve->res_id);
	}
...
static int
cam_ife_csid_check_cust_node(struct cam_csid_hw_reserve_resource_args *reserve,
			     struct cam_ife_csid_ver1_hw *csid_hw)
{
	struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	if (reserve->in_port->usage_type == CAM_ISP_RES_USAGE_DUAL) {
		CAM_ERR(CAM_ISP, "Dual IFE is not supported for cust_node %u",
	...
	if (!(csid_reg->csid_cust_node_map[csid_hw->hw_intf->hw_idx] &
		...
		CAM_ERR(CAM_ISP, "Invalid CSID:%u cust_node %u",
			csid_hw->hw_intf->hw_idx, reserve->in_port->cust_node);
		return -EINVAL;
	}
...
static bool
cam_ife_csid_ver1_is_width_valid_by_fuse(struct cam_ife_csid_ver1_hw *csid_hw,
					 uint32_t width)
{
	struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	if (width > csid_reg->fused_max_width[fuse_val]) {
		CAM_ERR(CAM_ISP,
			"CSID[%u] Resolution not supported required_width: %d "
			"max_supported_width: %d",
			csid_hw->hw_intf->hw_idx, width,
			csid_reg->fused_max_width[fuse_val]);
		return false;
	}
...
static bool
cam_ife_csid_ver1_is_width_valid_by_dt(struct cam_ife_csid_ver1_hw *csid_hw,
				       uint32_t width)
{
	struct cam_csid_soc_private *soc_private = NULL;
	...
	struct cam_ife_csid_ver1_hw *csid_hw)
{
	...
	/* check in port args */
	rc = cam_ife_csid_check_in_port_args(reserve, csid_hw->hw_intf->hw_idx);
	if (rc)
		goto err;
...
int cam_ife_csid_ver1_reserve(void *hw_priv, void *reserve_args,
			      uint32_t arg_size)
{
	struct cam_ife_csid_ver1_hw *csid_hw;
	struct cam_hw_info *hw_info;
	...
	rc = cam_ife_csid_hw_ver1_hw_cfg(csid_hw, path_cfg, reserve, cid);
	res->res_state = CAM_ISP_RESOURCE_STATE_RESERVED;
	...
	CAM_DBG(CAM_ISP,
		"CSID %d Resource[id:%d name:%s] state %d cid %d rc %d",
		csid_hw->hw_intf->hw_idx, reserve->res_id, res->res_name,
		res->res_state, cid, rc);

	return 0;
}

int cam_ife_csid_ver1_release(void *hw_priv, void *release_args,
			      uint32_t arg_size)
{
	struct cam_ife_csid_ver1_hw *csid_hw;
	struct cam_hw_info *hw_info;
	...
	if (csid_hw->counters.csi2_reserve_cnt)
		csid_hw->counters.csi2_reserve_cnt--;
	if (!csid_hw->counters.csi2_reserve_cnt) {
		memset(&csid_hw->rx_cfg, 0, sizeof(struct cam_ife_csid_rx_cfg));
...
static int
cam_ife_csid_ver1_start_rdi_path(struct cam_ife_csid_ver1_hw *csid_hw,
				 struct cam_isp_resource_node *res)
{
	int rc = 0;
	struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	CAM_DBG(CAM_ISP, "CSID:%d start: %s", csid_hw->hw_intf->hw_idx,
		res->res_name);

	val = path_reg->fatal_err_mask | path_reg->non_fatal_err_mask |
	      csid_hw->debug_info.path_mask | IFE_CSID_VER1_PATH_INFO_RST_DONE;
...
static int
cam_ife_csid_ver1_start_udi_path(struct cam_ife_csid_ver1_hw *csid_hw,
				 struct cam_isp_resource_node *res)
{
	int rc = 0;
	struct cam_ife_csid_ver1_reg_info *csid_reg;
...
static int
cam_ife_csid_ver1_start_pix_path(struct cam_ife_csid_ver1_hw *csid_hw,
				 struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
     ...
     */
		...
		if (path_cfg->sync_mode == CAM_ISP_HW_SYNC_NONE) {
			...
			ipp_path_cfg =
				(struct cam_ife_csid_ver1_path_cfg *)csid_hw
					->path_res[CAM_IFE_PIX_PATH_RES_IPP]
					.res_priv;
   ...
   */
	...
	CAM_DBG(CAM_ISP, "CSID:%d Resource[id:%d name:%s]ctrl val: 0x%x",
		csid_hw->hw_intf->hw_idx, res->res_id, res->res_name, val);
...
static int cam_ife_csid_ver1_enable_csi2(struct cam_ife_csid_ver1_hw *csid_hw)
{
	...
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + csi2_reg->cfg0_addr);
	...
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base + csi2_reg->cfg1_addr);
	rc = cam_ife_csid_ver1_hw_reset(csid_hw);
   ...
   */
	...
	cam_io_w_mb(val,
		    soc_info->reg_map[0].mem_base + csi2_reg->irq_mask_addr);
...
static int
cam_ife_csid_ver1_init_config_rdi_path(struct cam_ife_csid_ver1_hw *csid_hw,
				       struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	rc = cam_ife_csid_get_format_rdi(path_cfg->in_format,
					 ...
					 path_reg->mipi_pack_supported, false);
	if (rc)
		return rc;
...
static int
cam_ife_csid_ver1_init_config_udi_path(struct cam_ife_csid_ver1_hw *csid_hw,
				       struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
...
static int
cam_ife_csid_ver1_init_config_pxl_path(struct cam_ife_csid_ver1_hw *csid_hw,
				       struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	rc = cam_ife_csid_get_format_ipp_ppp(path_cfg->in_format, &path_format);
   ...
   */
	...
	CAM_DBG(CAM_ISP, "CSID[%u] cfg0_addr val %x", csid_hw->hw_intf->hw_idx,
		val);
...
int cam_ife_csid_ver1_init_hw(void *hw_priv, void *init_args, uint32_t arg_size)
{
	...
	case CAM_IFE_PIX_PATH_RES_IPP:
	case CAM_IFE_PIX_PATH_RES_PPP:
		rc = cam_ife_csid_ver1_init_config_pxl_path(csid_hw, res);
		break;
	case CAM_IFE_PIX_PATH_RES_RDI_0:
	case CAM_IFE_PIX_PATH_RES_RDI_1:
	case CAM_IFE_PIX_PATH_RES_RDI_2:
	...
	case CAM_IFE_PIX_PATH_RES_RDI_4:
		rc = cam_ife_csid_ver1_init_config_rdi_path(csid_hw, res);
		break;
	case CAM_IFE_PIX_PATH_RES_UDI_0:
	case CAM_IFE_PIX_PATH_RES_UDI_1:
	case CAM_IFE_PIX_PATH_RES_UDI_2:
		rc = cam_ife_csid_ver1_init_config_udi_path(csid_hw, res);
		break;
	default:
...
static int cam_ife_csid_ver1_disable_csi2(struct cam_ife_csid_ver1_hw *csid_hw)
{
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
	...
	CAM_DBG(CAM_ISP, "CSID:%d Disable csi2 rx", csid_hw->hw_intf->hw_idx);

	return 0;
}

static int cam_ife_csid_ver1_disable_hw(struct cam_ife_csid_ver1_hw *csid_hw)
{
	const struct cam_ife_csid_ver1_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
	int rc = 0;
...
int cam_ife_csid_ver1_deinit_hw(void *hw_priv, void *deinit_args,
				uint32_t arg_size)
{
	struct cam_ife_csid_ver1_hw *csid_hw = NULL;
	struct cam_isp_resource_node *res;
...
int cam_ife_csid_ver1_start(void *hw_priv, void *args, uint32_t arg_size)
{
	struct cam_ife_csid_ver1_hw *csid_hw = NULL;
	struct cam_isp_resource_node *res;
	struct cam_hw_info *hw_info;
	...
	CAM_DBG(CAM_ISP, "CSID:%d num_res %u", csid_hw->hw_intf->hw_idx,
		start_args->num_res);
	...
	for (i = 0; i < start_args->num_res; i++) {
		...
		CAM_DBG(CAM_ISP, "CSID:%d res_type :%d res: %s",
			csid_hw->hw_intf->hw_idx, res->res_type, res->res_name);
...
static int cam_ife_csid_poll_stop_status(struct cam_ife_csid_ver1_hw *csid_hw,
					 uint32_t res_mask)
{
	int rc = 0, id;
	...
	for (; res_id < CAM_IFE_PIX_PATH_RES_MAX; res_id++, res_mask >>= 1) {
		...
		if (res_id == CAM_IFE_PIX_PATH_RES_IPP) {
			status_addr = csid_reg->ipp_reg->status_addr;
		} else if (res_id == CAM_IFE_PIX_PATH_RES_PPP) {
			status_addr = csid_reg->ppp_reg->status_addr;
			...
			status_addr = csid_reg->rdi_reg[res_id]->status_addr;
			...
			status_addr = csid_reg->udi_reg[id]->status_addr;
		} else {
		...
		rc = cam_common_read_poll_timeout(
			soc_info->reg_map[0].mem_base + status_addr,
			CAM_IFE_CSID_TIMEOUT_SLEEP_US,
			CAM_IFE_CSID_TIMEOUT_ALL_US, 0x1, 0x1, &val);
...
static int
cam_ife_csid_change_pxl_halt_mode(struct cam_ife_csid_ver1_hw *csid_hw,
...
int cam_ife_csid_halt(struct cam_ife_csid_ver1_hw *csid_hw, void *halt_args)
{
	struct cam_isp_resource_node *res;
	struct cam_ife_csid_hw_halt_args *csid_halt;
	int rc = 0;
...
int cam_ife_csid_ver1_stop(void *hw_priv, void *stop_args, uint32_t arg_size)
{
	struct cam_ife_csid_ver1_hw *csid_hw = NULL;
	struct cam_isp_resource_node *res;
	struct cam_hw_info *hw_info;
	...
	for (i = 0; i < csid_stop->num_res; i++) {
		...
		case CAM_IFE_PIX_PATH_RES_IPP:
		case CAM_IFE_PIX_PATH_RES_PPP:
			rc = cam_ife_csid_ver1_stop_pxl_path(
		...
		case CAM_IFE_PIX_PATH_RES_RDI_3:
		case CAM_IFE_PIX_PATH_RES_RDI_4:
			rc = cam_ife_csid_ver1_stop_rdi_path(
				csid_hw, res, csid_stop->stop_cmd);
			break;
		case CAM_IFE_PIX_PATH_RES_UDI_0:
		case CAM_IFE_PIX_PATH_RES_UDI_1:
		case CAM_IFE_PIX_PATH_RES_UDI_2:
			rc = cam_ife_csid_ver1_stop_udi_path(
				csid_hw, res, csid_stop->stop_cmd);
			break;
		default:
...
int cam_ife_csid_ver1_read(void *hw_priv, void *read_args, uint32_t arg_size)
{
...
int cam_ife_csid_ver1_write(void *hw_priv, void *write_args, uint32_t arg_size)
{
	CAM_ERR(CAM_ISP, "CSID: un supported");
	return -EINVAL;
}

static int cam_ife_csid_ver1_sof_irq_debug(struct cam_ife_csid_ver1_hw *csid_hw,
					   void *cmd_args)
{
	int i = 0;
	uint32_t val = 0;
	...
	CAM_INFO(CAM_ISP, "Notify CSIPHY: %d", csid_hw->rx_cfg.phy_sel);
...
static int cam_ife_csid_ver1_print_hbi_vbi(struct cam_ife_csid_ver1_hw *csid_hw,
					   struct cam_isp_resource_node *res)
{
	...
	CAM_INFO_RATE_LIMIT(CAM_ISP,
			    ...
			    csid_hw->hw_intf->hw_idx, res->res_id,
			    res->res_name, hbi, vbi);

	return 0;
}

static int
cam_ife_csid_ver1_get_time_stamp(struct cam_ife_csid_ver1_hw *csid_hw,
				 void *cmd_args)
{
	struct cam_isp_resource_node *res = NULL;
	...
	case CAM_IFE_PIX_PATH_RES_RDI_4:
		curr_0_sof_addr =
			csid_reg->rdi_reg[res->res_id]->timestamp_curr0_sof_addr;
		curr_1_sof_addr =
			csid_reg->rdi_reg[res->res_id]->timestamp_curr1_sof_addr;
		break;
	default:
	...
	time_hi = cam_io_r_mb(soc_info->reg_map[0].mem_base + curr_1_sof_addr);
	time_lo = cam_io_r_mb(soc_info->reg_map[0].mem_base + curr_0_sof_addr);
	timestamp_args->time_stamp_val = (time_hi << 32) | time_lo;

	timestamp_args->time_stamp_val = mul_u64_u32_div(
		timestamp_args->time_stamp_val, CAM_IFE_CSID_QTIMER_MUL_FACTOR,
	...
	CAM_DBG(CAM_ISP, "timestamp:%lld", timestamp_args->boot_timestamp);
...
static int
cam_ife_csid_ver1_set_csid_clock(struct cam_ife_csid_ver1_hw *csid_hw,
				 void *cmd_args)
{
...
int cam_ife_csid_ver1_set_csid_qcfa(struct cam_ife_csid_ver1_hw *csid_hw,
				    void *cmd_args)
{
	...
	if (!csid_hw || !cmd_args) {
		CAM_ERR(CAM_ISP, "Invalid param %pK %pK", csid_hw, cmd_args);
		return -EINVAL;
	}
	...
	if (!res) {
		CAM_ERR(CAM_ISP, "CSID[%u] NULL res", csid_hw->hw_intf->hw_idx);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "CSID %u QCFA binning %d", csid_hw->hw_intf->hw_idx,
		path_cfg->qcfa_bin);

	return 0;
}

static int cam_ife_csid_ver1_dump_hw(struct cam_ife_csid_ver1_hw *csid_hw,
				     void *cmd_args)
{
	int i;
	uint8_t *dst;
	...
	if (!dump_args->cpu_addr || !dump_args->buf_len) {
		CAM_ERR(CAM_ISP, "Invalid params %pK %zu",
	...
	min_len = soc_info->reg_map[0].size +
		  sizeof(struct cam_isp_hw_dump_header) + sizeof(uint32_t);
	...
	*addr = soc_info->index;
	...
	for (i = 0; i < num_reg; i++) {
		...
		addr[1] = cam_io_r(soc_info->reg_map[0].mem_base + (i * 4));
		addr += 2;
	}
	hdr->size = hdr->word_size * (addr - start);
	dump_args->offset += hdr->size + sizeof(struct cam_isp_hw_dump_header);
	CAM_DBG(CAM_ISP, "offset %zu", dump_args->offset);
	return 0;
}

static int cam_ife_csid_log_acquire_data(struct cam_ife_csid_ver1_hw *csid_hw,
					 void *cmd_args)
{
	struct cam_isp_resource_node *res = NULL;
	struct cam_hw_soc_info *soc_info;
	...
	CAM_INFO(
		CAM_ISP,
		"CSID:%d res id:%d type:%d state:%d in f:%d out f:%d st pix:%d end "
		"pix:%d st line:%d end line:%d h bin:%d qcfa bin:%d",
		csid_hw->hw_intf->hw_idx, res->res_id, res->res_type,
	...
	if (res->res_id >= CAM_IFE_PIX_PATH_RES_RDI_0 &&
		...
		CAM_INFO(CAM_ISP,
			 ...
			 csid_hw->hw_intf->hw_idx, res->res_id, byte_cnt_ping,
			 byte_cnt_pong);
	}
...
static int
cam_ife_csid_ver1_dump_csid_clock(struct cam_ife_csid_ver1_hw *csid_hw,
				  void *cmd_args)
{
	if (!csid_hw)
		return -EINVAL;

	CAM_INFO(CAM_ISP, "CSID:%d clock rate %llu", csid_hw->hw_intf->hw_idx,
		 csid_hw->clk_rate);

	return 0;
}

static int cam_ife_csid_ver1_process_cmd(void *hw_priv, uint32_t cmd_type,
					 void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_CSID_QCFA_SUPPORTED:
		rc = cam_ife_csid_ver1_set_csid_qcfa(csid_hw, cmd_args);
		break;
...
static int
cam_ife_csid_ver1_handle_rx_debug_event(struct cam_ife_csid_ver1_hw *csid_hw,
					uint32_t bit_pos)
{
	struct cam_hw_soc_info *soc_info;
	struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	case IFE_CSID_VER1_RX_LONG_PKT_CAPTURED:
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP, "Csid :%d Long pkt ECC: %d",
				    csid_hw->hw_intf->hw_idx, val);
		...
		CAM_INFO_RATE_LIMIT(
			...
			csid_hw->hw_intf->hw_idx, val & csi2_reg->calc_crc_mask,
	...
	case IFE_CSID_VER1_RX_SHORT_PKT_CAPTURED:
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP, "Csid :%d Short pkt ECC: %d",
				    csid_hw->hw_intf->hw_idx, val);
		break;
	...
	case IFE_CSID_VER1_RX_UNMAPPED_VC_DT:
		...
		CAM_ERR_RATE_LIMIT(
			CAM_ISP,
			"CSID:%d UNMAPPED_VC_DT: VC:%d DT:%d WC:%d not mapped "
			"to any csid paths",
		...
		break;
	default:
		CAM_INFO_RATE_LIMIT(CAM_ISP, "CSID[%d] RX_IRQ: %s",
				    csid_hw->hw_intf->hw_idx,
...
static int cam_ife_csid_ver1_handle_event_err(
	struct cam_ife_csid_ver1_hw *csid_hw,
	struct cam_ife_csid_ver1_evt_payload *evt_payload, uint32_t err_type)
{
	...
	CAM_DBG(CAM_ISP, "CSID[%d] Error type %d", csid_hw->hw_intf->hw_idx,
		err_type);

	rc = csid_hw->event_cb(csid_hw->token, CAM_ISP_HW_EVENT_ERROR,
			       (void *)&event_info);
	...
	struct list_head *payload_list)
{
	...
	*evt_payload = list_first_entry(
		payload_list, struct cam_ife_csid_ver1_evt_payload, list);
	...
	struct cam_ife_csid_ver1_evt_payload *evt_payload)
{
	...
	if (irq_status) {
		...
		if (irq_status & IFE_CSID_VER1_RX_LANE0_FIFO_OVERFLOW) {
			...
			CAM_ERR_BUF(
				...
				&len,
				"RX_ERROR_LANE0_FIFO_OVERFLOW: Skew/Less Data on lanes/ Slow "
				"csid clock:%luHz\n",
		...
		if (irq_status & IFE_CSID_VER1_RX_LANE1_FIFO_OVERFLOW) {
			...
			CAM_ERR_BUF(
				CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				&len,
				"RX_ERROR_LANE1_FIFO_OVERFLOW: Skew/Less Data on lanes/ Slow "
				"csid clock:%luHz\n",
				soc_info->applied_src_clk_rates.sw_client);
		}
		...
		if (irq_status & IFE_CSID_VER1_RX_LANE2_FIFO_OVERFLOW) {
			...
			CAM_ERR_BUF(
				CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				&len,
				"RX_ERROR_LANE2_FIFO_OVERFLOW: Skew/Less Data on lanes/ Slow "
				"csid clock:%luHz\n",
				soc_info->applied_src_clk_rates.sw_client);
		}
		...
		if (irq_status & IFE_CSID_VER1_RX_LANE3_FIFO_OVERFLOW) {
			...
			CAM_ERR_BUF(
				CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				&len,
				"RX_ERROR_LANE3_FIFO_OVERFLOW: Skew/Less Data on lanes/ Slow "
				"csid clock:%luHz\n",
				soc_info->applied_src_clk_rates.sw_client);
		}
		...
		if (irq_status & IFE_CSID_VER1_RX_TG_FIFO_OVERFLOW) {
			event_type |=
				CAM_ISP_HW_ERROR_CSID_OUTPUT_FIFO_OVERFLOW;
			CAM_ERR_BUF(
				CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				&len,
		...
		if (irq_status & IFE_CSID_VER1_RX_CPHY_EOT_RECEPTION) {
			...
			CAM_ERR_BUF(
				CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN,
				&len,
				"CPHY_EOT_RECEPTION: No EOT on lane/s, is_EPD: %d, PHY_Type: "
				"%s(%u)\n",
	...
	if ((!csid_hw->flags.reset_awaited) &&
		...
		cam_ife_csid_ver1_handle_event_err(csid_hw, evt_payload,
						   event_type);
...
static int cam_ife_csid_ver1_path_bottom_half_handler(
	const struct cam_ife_csid_ver1_path_reg_info *path_reg,
	struct cam_ife_csid_ver1_evt_payload *evt_payload,
	struct cam_ife_csid_ver1_hw *csid_hw, uint32_t index)
{
	...
	if (!csid_hw || !evt_payload) {
		CAM_ERR(CAM_ISP, "Invalid Param csid_hw %pK evt_payload %pK",
			csid_hw, evt_payload);
		return 0;
	}
	...
	if (len)
		CAM_ERR_RATE_LIMIT(CAM_ISP, "CSID[%u] %s: %s",
...
static int cam_ife_csid_ver1_bottom_half_handler(void *handler_priv,
						 void *evt_payload_priv)
{
	...
	if (evt_payload->irq_status[CAM_IFE_CSID_IRQ_REG_RX])
		cam_ife_csid_ver1_rx_bottom_half_handler(csid_hw, evt_payload);
...
static int cam_ife_csid_ver1_rx_top_half(uint32_t *irq_status,
					 struct cam_ife_csid_ver1_hw *csid_hw,
					 uint32_t *need_bh_sched)
{
	...
	cam_io_w_mb(status,
		    soc_info->reg_map[0].mem_base + csi2_reg->irq_clear_addr);
	...
	if (status & BIT(csi2_reg->rst_done_shift_val)) {
		CAM_DBG(CAM_ISP, "CSID[%d] rx_reset done",
			csid_hw->hw_intf->hw_idx);
		complete(&csid_hw->irq_complete[CAM_IFE_CSID_IRQ_REG_RX]);
		return IRQ_HANDLED;
	}
...
static int cam_ife_csid_ver1_path_top_half(uint32_t *irq_status,
					   struct cam_ife_csid_ver1_hw *csid_hw,
					   uint32_t *need_bh, uint32_t index)
{
	struct cam_ife_csid_ver1_path_reg_info *path_reg = NULL;
	struct cam_ife_csid_ver1_reg_info *csid_reg;
	struct cam_hw_soc_info *soc_info;
	...
	cam_io_w_mb(status,
		    soc_info->reg_map[0].mem_base + path_reg->irq_clear_addr);
	...
	if ((status & IFE_CSID_VER1_PATH_ERROR_PIX_COUNT) ||
		...
		CAM_ERR(CAM_ISP,
			...
			"[format_measure0: 0x%x]",
			((val1 >>
			  csid_reg->cmn_reg->format_measure_height_shift_val) &
			 csid_reg->cmn_reg->format_measure_height_mask_val),
			val1 & csid_reg->cmn_reg->format_measure_width_mask_val,
			((val >>
			  csid_reg->cmn_reg->format_measure_height_shift_val) &
			 csid_reg->cmn_reg->format_measure_height_mask_val),
			val & csid_reg->cmn_reg->format_measure_width_mask_val,
			val);
	}
...
static irqreturn_t cam_ife_csid_irq(int irq_num, void *data)
{
	...
	for (i = CAM_IFE_CSID_IRQ_REG_RX; i < CAM_IFE_CSID_IRQ_REG_MAX; i++) {
		...
		case CAM_IFE_CSID_IRQ_REG_RX:
			cam_ife_csid_ver1_rx_top_half(status, csid_hw,
						      &need_rx_bh);
			break;
		case CAM_IFE_CSID_IRQ_REG_IPP:
		case CAM_IFE_CSID_IRQ_REG_PPP:
		...
		case CAM_IFE_CSID_IRQ_REG_UDI_1:
		case CAM_IFE_CSID_IRQ_REG_UDI_2:
			cam_ife_csid_ver1_path_top_half(status, csid_hw,
							&need_path_bh, i);
			break;
		default:
			break;
	...
	tasklet_bh_api.bottom_half_enqueue_func(
		csid_hw->tasklet, bh_cmd, csid_hw, evt_payload,
...
static int cam_ife_ver1_hw_alloc_res(struct cam_isp_resource_node *res,
				     uint32_t res_type,
...
static int
cam_ife_csid_ver1_hw_init_path_res(struct cam_ife_csid_ver1_hw *ife_csid_hw)
{
	int rc = 0;
	int i;
	struct cam_ife_csid_ver1_reg_info *csid_reg;
	...
	if (csid_reg->cmn_reg->num_pix) {
		...
		rc = cam_ife_ver1_hw_alloc_res(res, CAM_ISP_RESOURCE_PIX_PATH,
					       ife_csid_hw->hw_intf,
	...
	if (csid_reg->cmn_reg->num_ppp) {
		...
		scnprintf(res->res_name, CAM_ISP_RES_NAME_LEN, "PPP");
	}
			      ...
			      bool is_custom)
{
	...
	rc = cam_ife_csid_init_soc_resources(&ife_csid_hw->hw_info->soc_info,
					     cam_ife_csid_irq, NULL,
					     ife_csid_hw, is_custom);
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "CSID:%d Failed to init_soc", hw_intf->hw_idx);
		return rc;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "CSID[%d] Probe Init failed", hw_intf->hw_idx);
		return rc;
	}
