...
const uint8_t *cam_ife_csid_irq_reg_tag[CAM_IFE_CSID_IRQ_REG_MAX] = {
	"TOP",	"RX",  "RDI0", "RDI1", "RDI2", "RDI3",
	"RDI4", "IPP", "PPP",  "UDI0", "UDI1", "UDI2",
};

static int
cam_ife_csid_get_cid(struct cam_ife_csid_cid_data *cid_data,
...
int cam_ife_csid_is_pix_res_format_supported(uint32_t in_format)
{
	int rc = -EINVAL;
...
int cam_ife_csid_get_format_rdi(uint32_t in_format, uint32_t out_format,
				struct cam_ife_csid_path_format *path_format,
				bool mipi_pack_supported, bool mipi_unpacked)
{
	int rc = 0;
	...
	CAM_DBG(CAM_ISP,
		"in %u out %u plain_fmt %u packing %u decode %u bpp %u unpack %u "
		"pack supported %u",
		in_format, out_format, path_format->plain_fmt,
		path_format->packing_fmt, path_format->decode_fmt,
		path_format->bits_per_pxl, mipi_unpacked, mipi_pack_supported);
	return rc;

err:
	CAM_ERR(CAM_ISP, "Unsupported format pair in %d out %d", in_format,
		out_format);
	return rc;
}

int cam_ife_csid_get_format_ipp_ppp(
	uint32_t in_format, struct cam_ife_csid_path_format *path_format)
{
	int rc = 0;
	...
	CAM_DBG(CAM_ISP, "decode_fmt:%d plain_fmt:%d", path_format->decode_fmt,
			       ...
			       bool is_custom)
{
	...
	if (core_info->sw_version == CAM_IFE_CSID_VER_1_0) {
		rc = cam_ife_csid_hw_ver1_init(hw_intf, core_info, is_custom);
	} else if (core_info->sw_version == CAM_IFE_CSID_VER_2_0) {
		rc = cam_ife_csid_hw_ver2_init(hw_intf, core_info, is_custom);
	}
			   ...
			   struct cam_ife_csid_core_info *core_info)
{
	...
	if (core_info->sw_version == CAM_IFE_CSID_VER_1_0)
		...
		rc = cam_ife_csid_hw_ver2_deinit(hw_intf->hw_priv);
...
int cam_ife_csid_is_vc_full_width(struct cam_ife_csid_cid_data *cid_data)
{
	...
	for (i = 0; i < CAM_IFE_CSID_CID_MAX; i++) {
		...
		for (j = 0; j < p_cid->num_vc_dt; j++) {
			if (p_cid->vc_dt[j].valid && p_cid->vc_dt[j].vc > 3) {
				rc = 1;
				goto end;
			}
		}
...
int cam_ife_csid_cid_reserve(struct cam_ife_csid_cid_data *cid_data,
			     uint32_t *cid_value, uint32_t hw_idx,
			     struct cam_csid_hw_reserve_resource_args *reserve)
{
	int i, j, rc = 0;
	...
	if (i == CAM_IFE_CSID_CID_MAX) {
		for (j = 0; j < reserve->in_port->num_valid_vc_dt; j++)
			CAM_ERR(CAM_ISP, "CSID[%d] reserve fail vc[%d] dt[%d]",
...
int cam_ife_csid_cid_release(struct cam_ife_csid_cid_data *cid_data,
			     uint32_t hw_idx, uint32_t cid)
{
	int i;

	if (!cid_data->cid_cnt) {
		CAM_WARN(CAM_ISP, "CSID[%d] unbalanced cid:%d release", hw_idx,
			 cid);
		return 0;
	}
...
int cam_ife_csid_check_in_port_args(
	struct cam_csid_hw_reserve_resource_args *reserve, uint32_t hw_idx)
{
	if (reserve->in_port->res_type >= CAM_ISP_IFE_IN_RES_MAX) {
		CAM_ERR(CAM_ISP, "CSID:%d  Invalid phy sel %d", hw_idx,
	...
	if (reserve->in_port->lane_type >= CAM_ISP_LANE_TYPE_MAX &&
		...
		CAM_ERR(CAM_ISP, "CSID:%d  Invalid lane type %d", hw_idx,
	...
	if ((reserve->in_port->lane_type == CAM_ISP_LANE_TYPE_DPHY &&
	    ...
	    reserve->in_port->res_type != CAM_ISP_IFE_IN_RES_TPG) {
		CAM_ERR(CAM_ISP, "CSID:%d Invalid lane num %d", hw_idx,
	...
	if ((reserve->res_id == CAM_IFE_PIX_PATH_RES_IPP ||
		...
		CAM_ERR(CAM_ISP, "CSID %d, res_id %d, unsupported format %d",
			hw_idx, reserve->res_id,
			reserve->in_port
				->format[CAM_IFE_CSID_MULTI_VC_DT_GRP_0]);
		return -EINVAL;
	}
...
int cam_ife_csid_get_rt_irq_idx(uint32_t irq_reg, uint32_t num_ipp,
				uint32_t num_ppp, uint32_t num_rdi)
{
	...
	case CAM_IFE_CSID_IRQ_REG_IPP:
		rt_irq_reg_idx = CAM_IFE_CSID_IRQ_REG_RX + num_rdi + 1;
		break;
	case CAM_IFE_CSID_IRQ_REG_PPP:
		rt_irq_reg_idx =
			CAM_IFE_CSID_IRQ_REG_RX + num_rdi + num_ipp + 1;
		break;
	...
	case CAM_IFE_CSID_IRQ_REG_UDI_2:
		rt_irq_reg_idx = CAM_IFE_CSID_IRQ_REG_RX + num_rdi + num_ipp +
				 num_ppp +
...
const char *
cam_ife_csid_reset_type_to_string(enum cam_ife_csid_reset_type reset_type)
{
	switch (reset_type) {
...
int cam_ife_csid_get_base(struct cam_hw_soc_info *soc_info, uint32_t base_id,
			  void *cmd_args, size_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "core %d mem_base 0x%x, cdm_id:%u", soc_info->index,
		mem_base, cdm_args->cdm_id);
