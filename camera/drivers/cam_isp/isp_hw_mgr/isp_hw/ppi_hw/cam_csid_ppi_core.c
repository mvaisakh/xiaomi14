...
static int cam_csid_ppi_reset(struct cam_csid_ppi_hw *ppi_hw)
{
	...
	cam_io_w_mb(0,
		    soc_info->reg_map[0].mem_base + ppi_reg->ppi_irq_mask_addr);
	cam_io_w_mb(PPI_RST_CONTROL,
		    soc_info->reg_map[0].mem_base + ppi_reg->ppi_irq_set_addr);
	cam_io_w_mb(PPI_RST_CONTROL,
		    soc_info->reg_map[0].mem_base + ppi_reg->ppi_rst_cmd_addr);
	cam_io_w_mb(PPI_IRQ_CMD_SET,
		    soc_info->reg_map[0].mem_base + ppi_reg->ppi_irq_cmd_addr);
...
static int cam_csid_ppi_enable_hw(struct cam_csid_ppi_hw *ppi_hw)
{
	...
	CAM_DBG(CAM_ISP, "PPI:%d PPI HW version: 0x%x", ppi_hw->hw_intf->hw_idx,
		val);
...
clk_disable:
	for (--i; i >= 0; i--)
		cam_soc_util_clk_disable(soc_info, CAM_CLK_SW_CLIENT_IDX, false,
					 i);
	ppi_hw->hw_info->open_count--;
	return rc;
}
...
static int cam_csid_ppi_disable_hw(struct cam_csid_ppi_hw *ppi_hw)
{
	...
	CAM_DBG(CAM_ISP, "PPI:%d De-init PPI HW", ppi_hw->hw_intf->hw_idx);
			       ...
			       uint32_t ppi_idx)
{
	...
	CAM_DBG(CAM_ISP, "type %d index %d", csid_ppi_hw->hw_intf->hw_type,
		ppi_idx);
