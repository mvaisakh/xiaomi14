...
static void cam_cpas_dump_monitor_array(struct cam_hw_info *cpas_hw);
static int cam_cpas_log_vote(struct cam_hw_info *cpas_hw, bool ddr_only);

static struct cam_cpas_subpart_info
	g_cam_cpas_camera_subpart_info = { .num_bits = 8,
					   /*
     * Below fuse indexing is based on software fuse definition which is in SMEM
     * and provided by XBL team.
     */
					   .hw_bitmap_mask = {
						   { CAM_CPAS_CAM_FUSE,
						     BIT(0) },
						   ...
						   { CAM_CPAS_SFE_FUSE,
						     BIT(2) },
						   { CAM_CPAS_CUSTOM_FUSE,
						     BIT(0) },
					   } };

static void cam_cpas_process_drv_bw_overrides(
	struct cam_cpas_bus_client *bus_client, uint64_t *high_ab,
	uint64_t *high_ib, uint64_t *low_ab, uint64_t *low_ib,
	const struct cam_cpas_debug_settings *cpas_settings)
{
	...
	if (strnstr(bus_client->common_data.name, "cam_ife_0_drv", name_len)) {
	...
	CAM_INFO(
		CAM_CPAS,
		"Overriding mnoc bw for: %s with [AB IB] high: [%llu %llu], low: "
		"[%llu %llu], curr high: [%llu %llu], curr low: [%llu %llu]",
		bus_client->common_data.name, *high_ab, *high_ib, *low_ab,
		*low_ib, curr_ab_high, curr_ib_high, curr_ab_low, curr_ib_low);
}
	...
	const struct cam_cpas_debug_settings *cpas_settings)
{
	...
	CAM_INFO(
		CAM_CPAS,
		"Overriding mnoc bw for: %s with ab: %llu, ib: %llu, curr_ab: %llu, "
		"curr_ib: %llu",
			   ...
			   struct cam_cpas_reg *reg_info)
{
	...
	CAM_INFO(CAM_CPAS, "Base[%d] Offset[0x%08x] Value[0x%08x]", reg_base,
			     ...
			     struct cam_cpas_reg *reg_info)
{
	...
	CAM_DBG(CAM_CPAS, "Base[%d]:[0x%08x] Offset[0x%08x] Value[0x%08x]",
		reg_base, soc_info->reg_map[reg_base_index].mem_base,
		reg_info->offset, value);
...
static int
cam_cpas_util_vote_drv_bus_client_bw(struct cam_cpas_bus_client *bus_client,
				     struct cam_cpas_axi_bw_info *curr_vote,
				     struct cam_cpas_axi_bw_info *applied_vote)
{
	int rc = 0;
	...
	if ((curr_vote->drv_vote.high.ab || curr_vote->drv_vote.high.ib ||
		...
		cam_cpas_process_drv_bw_overrides(bus_client,
						  &curr_vote->drv_vote.high.ab,
						  &curr_vote->drv_vote.high.ib,
						  &curr_vote->drv_vote.low.ab,
	...
	rc = cam_soc_bus_client_update_bw(bus_client->soc_bus_client,
					  curr_vote->drv_vote.high.ab,
					  curr_vote->drv_vote.high.ib,
					      ...
					      int enable)
{
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		...
		rc = cam_cpas_util_vote_hlos_bus_client_bw(
			&cpas_core->axi_port[i].bus_client, ab_bw, ib_bw, false,
			&applied_ab_bw, &applied_ib_bw);
		if (rc) {
			CAM_ERR(CAM_CPAS,
...
static int cam_cpas_hw_dump_camnoc_buff_fill_info(struct cam_hw_info *cpas_hw,
						  uint32_t client_handle)
{
	...
	for (camnoc_idx = 0; camnoc_idx < cpas_core->num_valid_camnoc;
	     camnoc_idx++) {
		log_buf[0] = '\0';
		len = 0;
		...
		for (i = 0; i < camnoc_info->specific_size; i++) {
			...
			rc = cam_cpas_hw_reg_read(
				cpas_hw, client_handle, camnoc_info->reg_base,
				camnoc_info->specific[i].maxwr_low.offset, true,
				&val);
			if (rc)
				break;

			len += scnprintf((log_buf + len),
					 (CAM_CPAS_LOG_BUF_LEN - len),
					 " %s:[%d %d]",
...
static void cam_cpas_print_smart_qos_priority(struct cam_hw_info *cpas_hw)
{
	struct cam_cpas *cpas_core = (struct cam_cpas *)cpas_hw->core_info;
	struct cam_hw_soc_info *soc_info = &cpas_hw->soc_info;
	...
	for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius; i++) {
		...
		len += scnprintf((log_buf + len), (CAM_CPAS_LOG_BUF_LEN - len),
				 ...
				 niu_node->node_name, val_high, val_low);
	}
...
static bool cam_cpas_is_new_rt_bw_lower(const struct cam_hw_info *cpas_hw)
{
	struct cam_cpas *cpas_core = (struct cam_cpas *)cpas_hw->core_info;
	int i;
...
static void cam_cpas_reset_niu_priorities(struct cam_hw_info *cpas_hw)
{
	struct cam_cpas_private_soc *soc_private =
		(struct cam_cpas_private_soc *)cpas_hw->soc_info.soc_private;
	uint8_t i;
	...
	for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius; i++) {
		soc_private->smart_qos_info->rt_wr_niu_node[i]
			->applied_priority_low = 0x0;
		soc_private->smart_qos_info->rt_wr_niu_node[i]
			->applied_priority_high = 0x0;
	}
}

static bool cam_cpas_calculate_smart_qos(struct cam_hw_info *cpas_hw)
{
	struct cam_cpas *cpas_core = (struct cam_cpas *)cpas_hw->core_info;
	struct cam_cpas_private_soc *soc_private =
		(struct cam_cpas_private_soc *)cpas_hw->soc_info.soc_private;
	...
	uint64_t bw_per_kb, total_camnoc_bw, max_bw_per_kb = 0, remainder,
					     ramp_val;
	...
	for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius; i++) {
		...
		bw_per_kb = niu_node->bw_info[CAM_CPAS_PORT_HLOS_DRV]
				    .hlos_vote.camnoc;

		if (soc_private->enable_cam_clk_drv)
			bw_per_kb += niu_node->bw_info[CAM_CPAS_PORT_DRV_0]
					     .drv_vote.high.camnoc +
				     niu_node->bw_info[CAM_CPAS_PORT_DRV_0]
					     .drv_vote.low.camnoc +
				     niu_node->bw_info[CAM_CPAS_PORT_DRV_1]
					     .drv_vote.high.camnoc +
				     niu_node->bw_info[CAM_CPAS_PORT_DRV_1]
					     .drv_vote.low.camnoc +
				     niu_node->bw_info[CAM_CPAS_PORT_DRV_2]
					     .drv_vote.high.camnoc +
				     niu_node->bw_info[CAM_CPAS_PORT_DRV_2]
					     .drv_vote.low.camnoc;
		...
		CAM_DBG(CAM_PERF,
			"NIU[%d][%s]camnoc_bw %llu, niu_size %u, init_bw_per_kb %lld, "
			"remainder %lld, max_bw_per_kb %lld, total_bw_per_kb %lld",
			i, niu_node->node_name, total_camnoc_bw,
			niu_node->niu_size, bw_per_kb, remainder, max_bw_per_kb,
			total_bw_per_kb);
	}
	...
	for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius; i++) {
		...
		if ((bw_per_kb * CAM_CPAS_MAX_STRESS_INDICATOR) >
			...
			clamp_threshold = soc_private->smart_qos_info
						  ->moststressed_clamp_th;
			CAM_DBG(CAM_PERF, "Current niu clamp_threshold=%d",
				clamp_threshold);
		} else {
			ramp_val = soc_private->smart_qos_info
					   ->bw_ratio_scale_factor *
				   bw_per_kb;
			ramp_val =
				ramp_val * (soc_private->smart_qos_info
						    ->leaststressed_clamp_th -
					    soc_private->smart_qos_info
						    ->moststressed_clamp_th);
       ...
       */
		...
		for (pos = 15; pos >= clamp_threshold; pos--) {
			...
			CAM_DBG(CAM_PERF, "pos=%d, val=0x%x, priority=0x%llx",
				pos, val, priority);
		}

		for (pos = clamp_threshold - 1; pos >= 0; pos--) {
			if (pos == 0) {
				val = soc_private->smart_qos_info
					      ->rt_wr_priority_min;
			} else {
         ...
         */
				...
				CAM_DBG(CAM_PERF,
					"pos=%d, bw_per_kb=%lld, pos*bw_per_kb=%lld, ramp_val=%lld, "
					"remainder=%lld, max_bw_per_kb=%lld",
					pos, bw_per_kb, pos * bw_per_kb,
					ramp_val, remainder, max_bw_per_kb);
				...
				val += soc_private->smart_qos_info
					       ->rt_wr_priority_min;
				val = min(val, soc_private->smart_qos_info
						       ->rt_wr_priority_max);
			}
		...
		niu_node->curr_priority_high =
			(uint32_t)((priority >> 32) & 0xFFFFFFFF);

		if ((niu_node->curr_priority_low !=
		     niu_node->applied_priority_low) ||
		    (niu_node->curr_priority_high !=
		     niu_node->applied_priority_high))
			needs_update = true;

		CAM_DBG(CAM_PERF,
			"Node[%d][%s]Priority applied high 0x%x low 0x%x, new high 0x%x "
			"low 0x%x, needs_update %d",
			i, niu_node->node_name, niu_node->applied_priority_high,
			niu_node->applied_priority_low,
			niu_node->curr_priority_high,
			niu_node->curr_priority_low, needs_update);
	}

	if (cpas_core->smart_qos_dump && needs_update) {
		uint64_t total_camnoc;
		for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius;
		     i++) {
			niu_node =
				soc_private->smart_qos_info->rt_wr_niu_node[i];

			total_camnoc = niu_node->bw_info[CAM_CPAS_PORT_HLOS_DRV]
					       .hlos_vote.camnoc;
			...
			CAM_INFO(
				CAM_PERF,
				"Node[%d][%s] camnoc_bw=%lld, niu_size=%d, offset high 0x%x, low "
				"0x%x, Priority new high 0x%x low 0x%x, applied high 0x%x low 0x%x",
				i, niu_node->node_name, total_camnoc,
				niu_node->niu_size,
				niu_node->pri_lut_high_offset,
				niu_node->pri_lut_low_offset,
				niu_node->curr_priority_high,
				niu_node->curr_priority_low,
				niu_node->applied_priority_high,
				niu_node->applied_priority_low);
		}
	}
...
static int cam_cpas_apply_smart_qos(struct cam_hw_info *cpas_hw)
{
	struct cam_cpas *cpas_core = (struct cam_cpas *)cpas_hw->core_info;
	struct cam_hw_soc_info *soc_info = &cpas_hw->soc_info;
	struct cam_cpas_private_soc *soc_private =
	...
	for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius; i++) {
		...
		if (niu_node->curr_priority_high !=
		    niu_node->applied_priority_high) {
...
static int
cam_cpas_util_camnoc_drv_idx_to_cesta_hw_client_idx(int camnoc_drv_idx)
{
	int hw_client = -1;
...
static int cam_cpas_util_set_camnoc_axi_drv_clk_rate(
	struct cam_hw_soc_info *soc_info,
	...
	int cesta_drv_idx)
{
	...
	for (i = 0; i < CAM_CPAS_MAX_TREE_NODES; i++) {
		...
		if (req_drv_high_camnoc_bw <
			...
			req_drv_high_camnoc_bw =
				(tree_node->bw_info[cesta_drv_idx]
					 .drv_vote.high.camnoc *
				 tree_node->bus_width_factor);
	...
	intermediate_drv_high_result =
		req_drv_high_camnoc_bw * soc_private->camnoc_axi_clk_bw_margin;
	intermediate_drv_low_result =
		req_drv_low_camnoc_bw * soc_private->camnoc_axi_clk_bw_margin;
	...
	/*
   * Since all low votes are considered as part of high votes as well, add low
   * camnoc bw to final requested high camnoc bw value.
   */
	...
	if (cpas_core->streamon_clients) {
		...
		/*
     * cesta_drv_idx is based on enum we set in dtsi properties which is +1 of
     * actual corresponding hw client index
     */
		hw_client_idx =
			cam_cpas_util_camnoc_drv_idx_to_cesta_hw_client_idx(
				cesta_drv_idx);
		if (hw_client_idx == -1) {
			CAM_ERR(CAM_CPAS,
				"Invalid hw client idx %d, cesta_drv_idx %d",
		...
		if (debug_drv)
			CAM_INFO(
				CAM_PERF,
				"Setting camnoc axi cesta clk rate[BW Clk] : High [%llu %lld] "
				"Low [%llu %lld] cesta/hw_client_idx:[%d][%d]",
				req_drv_high_camnoc_bw, drv_high_clk_rate,
				req_drv_low_camnoc_bw, drv_low_clk_rate,
				cesta_drv_idx, hw_client_idx);
		else
			CAM_DBG(CAM_PERF,
				"Setting camnoc axi cesta clk rate[BW Clk] : High [%llu %lld] "
				"Low [%llu %lld] cesta/hw_client_idx:[%d][%d]",
		...
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Failed in setting camnoc cesta clk rates[high low]:[%ld %ld] "
				"client_idx:%d rc:%d",
		...
		cpas_core->applied_camnoc_axi_rate.hw_client[hw_client_idx]
			.high = drv_high_clk_rate;
		...
		rc = cam_soc_util_cesta_channel_switch(hw_client_idx,
						       "cpas_update");
		if (rc) {
			CAM_ERR(CAM_CPAS,
...
static int cam_cpas_util_set_camnoc_axi_hlos_clk_rate(
	struct cam_hw_soc_info *soc_info,
	struct cam_cpas_private_soc *soc_private, struct cam_cpas *cpas_core)
{
	struct cam_cpas_tree_node *tree_node = NULL;
	...
	for (i = 0; i < CAM_CPAS_MAX_TREE_NODES; i++) {
		...
		if (req_hlos_camnoc_bw <
		    (tree_node->bw_info[CAM_CPAS_PORT_HLOS_DRV].hlos_vote.camnoc *
		     tree_node->bus_width_factor)) {
			req_hlos_camnoc_bw =
				(tree_node->bw_info[CAM_CPAS_PORT_HLOS_DRV]
					 .hlos_vote.camnoc *
				 tree_node->bus_width_factor);
		}
	}

	intermediate_hlos_result =
		req_hlos_camnoc_bw * soc_private->camnoc_axi_clk_bw_margin;
	...
	CAM_DBG(CAM_PERF,
		"Setting camnoc axi HLOS clk rate[BW Clk] : [%llu %lld]",
   ...
   */
	if (cpas_core->streamon_clients) {
		rc = cam_soc_util_set_src_clk_rate(
			soc_info, CAM_CLK_SW_CLIENT_IDX, hlos_clk_rate, 0);
		if (rc)
			CAM_ERR(CAM_CPAS,
						 ...
						 int cesta_drv_idx)
{
	...
	if (cesta_drv_idx == CAM_CPAS_PORT_HLOS_DRV) {
		rc = cam_cpas_util_set_camnoc_axi_hlos_clk_rate(
			soc_info, soc_private, cpas_core);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Failed in setting hlos clk rate rc: %d", rc);
			goto end;
		}
	} else {
		rc = cam_cpas_util_set_camnoc_axi_drv_clk_rate(
			soc_info, soc_private, cpas_core, cesta_drv_idx);
		if (rc) {
			CAM_ERR(CAM_CPAS,
	...
	for (i = 0; i < axi_vote->num_paths; i++) {
         ...
         */
				...
				if (!cons_entry_found) {
					...
					axi_path->vote_level =
						axi_vote->axi_path[i].vote_level;
					con_axi_vote->num_paths++;
				}
				break;
			}
		...
		if (!path_found) {
			CAM_ERR(CAM_CPAS,
				"Client [%s][%d] i=%d num_paths=%d Consolidated path not found "
				"for path=%d, transac=%d",
				cpas_client->data.identifier,
				cpas_client->data.cell_index, i,
				axi_vote->num_paths, path_data_type,
				transac_type);
			return -EINVAL;
		}
	}
...
static int cam_cpas_update_axi_vote_bw(
	struct cam_hw_info *cpas_hw, struct cam_cpas_tree_node *cpas_tree_node,
	int ddr_drv_idx, int cesta_drv_idx, bool *mnoc_axi_port_updated,
	bool *camnoc_axi_port_updated)
{
	...
	if ((axi_port_idx < 0) || (axi_port_idx >= CAM_CPAS_MAX_AXI_PORTS)) {
		CAM_ERR(CAM_CPAS, "Invalid axi_port_idx: %d drv_idx: %d",
			axi_port_idx, ddr_drv_idx);
		return -EINVAL;
	}

	memcpy(&cpas_core->axi_port[axi_port_idx].curr_bw,
	       &cpas_tree_node->bw_info[ddr_drv_idx],
	       sizeof(struct cam_cpas_axi_bw_info));
	...
	if (ddr_drv_idx > CAM_CPAS_PORT_HLOS_DRV) {
		cpas_core->axi_port[axi_port_idx].curr_bw.drv_vote.high.ab +=
			cpas_core->axi_port[axi_port_idx]
				.curr_bw.drv_vote.low.ab;
		cpas_core->axi_port[axi_port_idx].curr_bw.drv_vote.high.ib +=
			cpas_core->axi_port[axi_port_idx]
				.curr_bw.drv_vote.low.ib;
	}
	...
	if (cesta_drv_idx > CAM_CPAS_PORT_HLOS_DRV)
		cpas_core
			->camnoc_axi_port[cpas_tree_node->axi_port_idx_arr
						  [CAM_CPAS_PORT_HLOS_DRV]]
			.curr_bw.hlos_vote.camnoc =
			cpas_tree_node->bw_info[cesta_drv_idx]
				.drv_vote.high.camnoc +
			cpas_tree_node->bw_info[cesta_drv_idx]
				.drv_vote.low.camnoc;
	else
				       ...
				       bool *camnoc_axi_port_updated)
{
   ...
   */
	...
	for (i = 0; i < cpas_core->num_camnoc_axi_ports; i++) {
		...
		rc = cam_cpas_util_vote_hlos_bus_client_bw(
			&camnoc_axi_port->bus_client, 0, camnoc_bw, true,
			&applied_ab, &applied_ib);
	...
	struct cam_axi_vote *axi_vote, uint32_t apply_type)
{
	...
	struct cam_cpas_axi_bw_info curr_mnoc_old = { 0 }, par_mnoc_old = { 0 },
				    curr_camnoc_old = { 0 },
				    par_camnoc_old = { 0 },
				    curr_port_bw = { 0 },
				    applied_port_bw = { 0 };
     ...
     */
		for (i = 0; i < cpas_core->num_axi_ports; i++) {
			if (cpas_core->axi_port[i]
				    .bus_client.common_data.is_drv_port)
				continue;
	...
	for (i = 0; i < con_axi_vote->num_paths; i++) {
		...
		} else if (!soc_private->enable_cam_clk_drv ||
			   cpas_core->force_cesta_sw_client) {
			cesta_drv_idx = CAM_CPAS_PORT_HLOS_DRV;
		}

		if ((ddr_drv_idx < 0) || (ddr_drv_idx > CAM_CPAS_PORT_DRV_2) ||
		    (cesta_drv_idx < 0) ||
		    (cesta_drv_idx > CAM_CPAS_PORT_DRV_2)) {
			CAM_ERR(CAM_CPAS,
       ...
       */
		...
		memcpy(&curr_camnoc_old,
		       &curr_tree_node->bw_info[cesta_drv_idx],
		       sizeof(struct cam_cpas_axi_bw_info));
		memcpy(&curr_mnoc_old, &curr_tree_node->bw_info[ddr_drv_idx],
		       sizeof(struct cam_cpas_axi_bw_info));
		cam_cpas_dump_tree_vote_info(cpas_hw, curr_tree_node,
					     "Level0 before update",
					     ddr_drv_idx, cesta_drv_idx);

		/* Check and update camnoc bw first */
		if (con_axi_vote->axi_path[i].vote_level ==
			...
			if (cesta_drv_idx > CAM_CPAS_PORT_HLOS_DRV) {
				...
				curr_tree_node->bw_info[cesta_drv_idx]
					...
					con_axi_vote->axi_path[i].camnoc_bw;
				curr_tree_node->bw_info[cesta_drv_idx]
					.drv_vote.high.camnoc = 0;
			} else {
				if (curr_tree_node->bw_info[cesta_drv_idx]
					    .hlos_vote.camnoc ==
				    con_axi_vote->axi_path[i].camnoc_bw) {
					camnoc_unchanged = true;
					goto update_l0_mnoc;
...
update_l0_mnoc:
		...
		if (con_axi_vote->axi_path[i].vote_level ==
			...
			if ((apply_type != CAM_CPAS_APPLY_TYPE_STOP) &&
			    camnoc_unchanged &&
			    (curr_tree_node->bw_info[ddr_drv_idx]
				     .drv_vote.high.ab ==
		...
		cam_cpas_dump_tree_vote_info(cpas_hw, curr_tree_node,
					     "Level0 after update", ddr_drv_idx,
					     cesta_drv_idx);

		while (curr_tree_node->parent_node) {
			par_tree_node = curr_tree_node->parent_node;
			memcpy(&par_camnoc_old,
			       &par_tree_node->bw_info[cesta_drv_idx],
			       sizeof(struct cam_cpas_axi_bw_info));
			memcpy(&par_mnoc_old,
			       &par_tree_node->bw_info[ddr_drv_idx],
			       sizeof(struct cam_cpas_axi_bw_info));
			cam_cpas_dump_tree_vote_info(cpas_hw, par_tree_node,
       ...
       */
			if (cesta_drv_idx > CAM_CPAS_PORT_HLOS_DRV) {
				par_tree_node->bw_info[cesta_drv_idx]
					.drv_vote.high.camnoc -=
					(curr_camnoc_old.drv_vote.high.camnoc /
					 merge_type_factor);
				par_tree_node->bw_info[cesta_drv_idx]
					.drv_vote.low.camnoc -=
					(curr_camnoc_old.drv_vote.low.camnoc /
					 merge_type_factor);
				...
				par_tree_node->bw_info[cesta_drv_idx]
					.hlos_vote.camnoc -=
					(curr_camnoc_old.hlos_vote.camnoc /
					 merge_type_factor);
       ...
       */
			...
			if (!par_tree_node->parent_node) {
				rc = cam_cpas_update_axi_vote_bw(
					cpas_hw, par_tree_node, ddr_drv_idx,
					cesta_drv_idx, mnoc_axi_port_updated,
			...
			memcpy(&curr_camnoc_old, &par_camnoc_old,
			       sizeof(struct cam_cpas_axi_bw_info));
			memcpy(&curr_mnoc_old, &par_mnoc_old,
			       sizeof(struct cam_cpas_axi_bw_info));
		}
	}
	...
	if (soc_private->enable_smart_qos) {
		...
		if (rt_bw_updated) {
			...
			if (apply_smart_qos &&
			    cam_cpas_is_new_rt_bw_lower(cpas_hw)) {
				/*
...
vote_start_clients:
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		...
		memcpy(&curr_port_bw, &mnoc_axi_port->curr_bw,
		       sizeof(struct cam_cpas_axi_bw_info));
		...
		if (mnoc_axi_port->bus_client.common_data.is_drv_port) {
			...
			rc = cam_cpas_util_vote_drv_bus_client_bw(
				&mnoc_axi_port->bus_client, &curr_port_bw,
				&applied_port_bw);
			if (rc) {
				CAM_ERR(CAM_CPAS,
			...
			if ((apply_type == CAM_CPAS_APPLY_TYPE_START) &&
				...
				rc = cam_cpas_start_drv_for_dev(
					mnoc_axi_port->cam_rsc_dev);
				if (rc) {
					CAM_ERR(CAM_CPAS,
						"Port[%s] failed in DRV start rc:%d",
						mnoc_axi_port->axi_port_name,
						rc);
					goto unlock_tree;
				}
				...
				rc = cam_cpas_stop_drv_for_dev(
					mnoc_axi_port->cam_rsc_dev);
				if (rc) {
					CAM_ERR(CAM_CPAS,
						"Port[%s] failed in DRV stop rc:%d",
						mnoc_axi_port->axi_port_name,
						rc);
					goto unlock_tree;
				}
			...
			rc = cam_cpas_util_vote_hlos_bus_client_bw(
				...
				curr_port_bw.hlos_vote.ab,
				curr_port_bw.hlos_vote.ib, false,
				&applied_port_bw.hlos_vote.ab,
				&applied_port_bw.hlos_vote.ib);
			if (rc) {
				CAM_ERR(CAM_CPAS,
					"Failed in mnoc vote for %s rc=%d",
					mnoc_axi_port->axi_port_name, rc);
	...
	if (soc_private->control_camnoc_axi_clk) {
		...
		rc = cam_cpas_camnoc_set_bw_vote(cpas_hw,
						 camnoc_axi_port_updated);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Failed in setting camnoc bw vote rc=%d", rc);
			goto unlock_tree;
		}
	}
...
static int cam_cpas_util_apply_default_axi_vote(struct cam_hw_info *cpas_hw,
						bool enable)
{
	struct cam_cpas *cpas_core = (struct cam_cpas *)cpas_hw->core_info;
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		...
		rc = cam_cpas_util_vote_hlos_bus_client_bw(
			...
			&axi_port->applied_bw.hlos_vote.ab,
...
static int cam_cpas_hw_start(void *hw_priv, void *start_args, uint32_t arg_size)
{
   ...
   */
	...
	if (cpas_core->streamon_clients == 0) {
		...
		if (debug_drv)
			CAM_INFO(CAM_CPAS, "DRV enable[DDR CLK]:[%s %s]",
				 ...
				 CAM_BOOL_TO_YESNO(
					 soc_private->enable_cam_clk_drv));
		...
		if (count > 0)
			CAM_DBG(CAM_CPAS, "Regulators already enabled %d",
				count);
	...
	/*
   * Need to apply axi vote after we enable clocks, since we need certain clocks
   * enabled for drv channel switch
   */
	rc = cam_cpas_util_apply_client_axi_vote(
		cpas_hw, cpas_client, &axi_vote, CAM_CPAS_APPLY_TYPE_START);
	if (rc)
		goto remove_ahb_vote;
...
static int
cam_cpas_util_validate_stop_bw(struct cam_cpas_private_soc *soc_private,
			       struct cam_cpas *cpas_core)
{
	int i;
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		if (soc_private->enable_cam_ddr_drv &&
			...
			if ((cpas_core->axi_port[i]
				     .applied_bw.drv_vote.high.ab) ||
			    (cpas_core->axi_port[i]
				     .applied_bw.drv_vote.high.ib) ||
				...
				CAM_ERR(CAM_CPAS,
					...
					cpas_core->axi_port[i]
						.applied_bw.drv_vote.high.ib,
					cpas_core->axi_port[i]
						.applied_bw.drv_vote.low.ab,
					cpas_core->axi_port[i]
						.applied_bw.drv_vote.low.ib);
				return -EINVAL;
			}
		} else {
			if (cpas_core->axi_port[i]
			...
			if ((cpas_core->axi_port[i].applied_bw.hlos_vote.ab) ||
				...
				CAM_ERR(CAM_CPAS,
					...
					cpas_core->axi_port[i].axi_port_name,
					cpas_core->axi_port[i]
						.applied_bw.hlos_vote.ab,
					cpas_core->axi_port[i]
						.applied_bw.hlos_vote.ib);
				return -EINVAL;
			}
		}
	}
...
static int cam_cpas_hw_stop(void *hw_priv, void *stop_args, uint32_t arg_size)
{
	...
	rc = cam_cpas_util_apply_client_axi_vote(
		cpas_hw, cpas_client, &axi_vote, CAM_CPAS_APPLY_TYPE_STOP);
	if (rc)
		goto done;
	...
	if (cpas_core->streamon_clients == 0) {
		...
		result = wait_event_timeout(cpas_core->soc_access_count_wq,
					    _check_soc_access_count(cpas_core),
					    HZ);
		if (result == 0) {
		...
		if (count > 0)
			CAM_WARN(
				CAM_CPAS,
				"Client=[%d][%s][%d] qchannel shut down while top gdsc is still "
				"on %d",
				client_indx, cpas_client->data.identifier,
		...
		if (rc)
			CAM_ERR(CAM_CPAS,
				"Failed in power off default vote rc: %d", rc);

		rc = cam_cpas_util_validate_stop_bw(soc_private, cpas_core);
		if (rc)
			CAM_ERR(CAM_CPAS, "Invalid applied bw at stop rc: %d",
				rc);
...
static int cam_cpas_log_vote(struct cam_hw_info *cpas_hw, bool ddr_only)
{
	...
	if ((cpas_core->streamon_clients > 0) &&
	    soc_private->enable_smart_qos && !ddr_only)
		cam_cpas_print_smart_qos_priority(cpas_hw);
   ...
   */
     ...
     */
		for (i = 0; i < rpmh_info[CAM_RPMH_NUMBER_OF_BCMS]; i++) {
			if ((!cpas_core->full_state_dump) &&
			    (i != ddr_bcm_index) && (i != mnoc_bcm_index))
				continue;

			offset_fe =
				rpmh_info[CAM_RPMH_BCM_FE_OFFSET] + (i * 0x4);
			offset_be =
				rpmh_info[CAM_RPMH_BCM_BE_OFFSET] + (i * 0x4);
	...
	if ((cpas_core->streamon_clients > 0) &&
		...
		uint32_t cesta_vcd_curr_perfol_offset,
		...
		for (i = 0; i < num_vcds; i++) {
			...
			cesta_vcd_curr_perfol_offset =
				vcd_curr_lvl_base + (vcd_base_inc * vcd_idx);
...
skip_vcd_dump:
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		if (cpas_core->axi_port[i].bus_client.common_data.is_drv_port) {
			CAM_INFO(
				CAM_PERF,
				"[%s] DRV applied: high [%llu %llu] low[%llu %llu] new: high "
				"[%llu %llu] low [%llu %llu]",
				cpas_core->axi_port[i].axi_port_name,
				cpas_core->axi_port[i]
					.applied_bw.drv_vote.high.ab,
			...
			CAM_INFO(
				CAM_PERF,
				"Port %s HLOS applied [%llu %llu] new [%llu %llu]",
				cpas_core->axi_port[i].axi_port_name,
	...
	if (soc_private->control_camnoc_axi_clk) {
		...
		if (soc_private->enable_cam_clk_drv)
			CAM_INFO(
				CAM_CPAS,
				"applied camnoc axi clk hw_client[high low] cesta_idx0:[%lld "
				"%lld] cesta_idx1:[%lld %lld] cesta_idx2:[%lld %lld]",
				cpas_core->applied_camnoc_axi_rate.hw_client[0]
					.high,
				cpas_core->applied_camnoc_axi_rate.hw_client[0]
					.low,
				cpas_core->applied_camnoc_axi_rate.hw_client[1]
					.high,
				cpas_core->applied_camnoc_axi_rate.hw_client[1]
					.low,
				cpas_core->applied_camnoc_axi_rate.hw_client[2]
					.high,
				cpas_core->applied_camnoc_axi_rate.hw_client[2]
					.low);
	} else {
		for (i = 0; i < cpas_core->num_camnoc_axi_ports; i++) {
			CAM_INFO(
				CAM_CPAS,
				"[%s] ab_bw[%lld] ib_bw[%lld] additional_bw[%lld] "
				"applied_ab[%lld] applied_ib[%lld]",
				cpas_core->camnoc_axi_port[i].axi_port_name,
				cpas_core->camnoc_axi_port[i]
					.curr_bw.hlos_vote.ab,
				cpas_core->camnoc_axi_port[i]
					.curr_bw.hlos_vote.ib,
				cpas_core->camnoc_axi_port[i].additional_bw,
				cpas_core->camnoc_axi_port[i]
					.applied_bw.hlos_vote.ab,
				cpas_core->camnoc_axi_port[i]
					  ...
					  int32_t identifier_value)
{
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		entry->axi_info[i].axi_port_name =
			cpas_core->axi_port[i].axi_port_name;
		memcpy(&entry->axi_info[i].curr_bw,
		       &cpas_core->axi_port[i].curr_bw,
		       sizeof(struct cam_cpas_axi_bw_info));

		/* camnoc bw value not applicable for mnoc ports */
		entry->axi_info[i].camnoc_bw = 0;
		memcpy(&entry->axi_info[i].applied_bw,
		       &cpas_core->axi_port[i].applied_bw,
		       sizeof(struct cam_cpas_axi_bw_info));
		entry->axi_info[i].is_drv_started =
		...
		if (atomic_inc_not_zero(&cpas_core->soc_access_count)) {
			for (i = 0; i < num_vcds; i++) {
				...
				cesta_vcd_curr_perfol_val = cam_io_r_mb(
					cesta_base +
					cesta_vcd_curr_perfol_offset);
				entry->vcd_reg_debug_info
					.vcd_curr_lvl_debug_info[i]
					.index = cesta_info->vcd_info[i].index;
				entry->vcd_reg_debug_info
					.vcd_curr_lvl_debug_info[i]
	...
	for (camnoc_idx = 0; camnoc_idx < cpas_core->num_valid_camnoc;
		...
		camnoc_info = cpas_core->camnoc_info[camnoc_idx];
		camnoc_reg_idx =
			cpas_core->regbase_index[camnoc_info->reg_base];
	...
	if (soc_private->enable_smart_qos) {
		...
		for (i = 0; i < soc_private->smart_qos_info->num_rt_wr_nius;
			...
			entry->rt_wr_niu_pri_lut_high[i] = cam_io_r_mb(
				soc_info->reg_map[camnoc_reg_idx].mem_base +
				niu_node->pri_lut_high_offset);

			entry->rt_wr_niu_pri_lut_low[i] = cam_io_r_mb(
				soc_info->reg_map[camnoc_reg_idx].mem_base +
				niu_node->pri_lut_low_offset);
		}
	}
...
static void cam_cpas_dump_monitor_array(struct cam_hw_info *cpas_hw)
{
	...
	if (state_head == -1) {
		...
		div_u64_rem(state_head + 1, CAM_CPAS_MONITOR_MAX_ENTRIES,
			    &oldest_entry);
	}
	...
	for (i = 0; i < num_entries; i++) {
		...
		CAM_CONVERT_TIMESTAMP_FORMAT(entry->timestamp, hrs, min, sec,
					     ms);
		log_buf[0] = '\0';

		CAM_INFO(
			CAM_CPAS,
			"**** %llu:%llu:%llu.%llu : Index[%d] Identifier[%s][%d] "
			"camnoc=sw : %ld, hw clients [%ld %ld][%ld %ld][%ld %ld], ahb=%d",
			hrs, min, sec, ms, index, entry->identifier_string,
		...
		for (j = 0; j < cpas_core->num_axi_ports; j++) {
			if ((entry->axi_info[j].applied_bw.vote_type ==
				...
				CAM_INFO(
					CAM_CPAS,
					"BW [%s] : HLOS ab=%lld, ib=%lld, DRV high_ab=%lld, "
					"high_ib=%lld, low_ab=%lld, low_ib=%lld",
					entry->axi_info[j].axi_port_name,
					entry->axi_info[j]
						.applied_bw.hlos_vote.ab,
					entry->axi_info[j]
		...
		if (cpas_core->regbase_index[CAM_CPAS_REG_RPMH] != -1) {
			CAM_INFO(
				...
				entry->fe_ddr, entry->fe_mnoc, entry->be_ddr,
				entry->be_mnoc, entry->be_shub);
		}

		if (cpas_core->regbase_index[CAM_CPAS_REG_CESTA] != -1) {
			uint32_t vcd_base_inc =
				cesta_info->cesta_reg_info->vcd_currol
					.vcd_base_inc;
			uint32_t vcd_curr_lvl_base =
				cesta_info->cesta_reg_info->vcd_currol
					.reg_offset;
			...
			for (k = 0; k < num_vcds; k++) {
				vcd_index = entry->vcd_reg_debug_info
						    .vcd_curr_lvl_debug_info[k]
						    .index;
				reg_offset = vcd_curr_lvl_base +
					     (vcd_base_inc * vcd_index);
				CAM_INFO(
					CAM_CPAS,
					"VCD[index=%d, type=%d, name=%s] [offset=0x%x, value=0x%x]",
					vcd_index, cesta_info->vcd_info[k].type,
					cesta_info->vcd_info[k].clk, reg_offset,
					entry->vcd_reg_debug_info
						.vcd_curr_lvl_debug_info[k]
						.reg_value);
			}
		...
		for (camnoc_idx = 0; camnoc_idx < cpas_core->num_valid_camnoc;
			...
			for (j = 0; j < entry->num_camnoc_lvl_regs[camnoc_idx];
			     j++) {
				len += scnprintf(
					(log_buf + len),
					(CAM_CPAS_LOG_BUF_LEN - len),
					" %s:[%d %d]",
					entry->camnoc_port_name[camnoc_idx][j],
					(entry->camnoc_fill_level[camnoc_idx][j] &
					 0x7FF),
					(entry->camnoc_fill_level[camnoc_idx][j] &
					 0x7F0000) >>
						16);
		...
		if (soc_private->enable_smart_qos) {
			len = 0;
			for (j = 0;
			     j < soc_private->smart_qos_info->num_rt_wr_nius;
			     j++) {
				struct cam_cpas_tree_node *niu_node =
					soc_private->smart_qos_info
						->rt_wr_niu_node[j];
			...
			CAM_INFO(CAM_CPAS, "SmartQoS [Node: Pri_lut] %s",
				 log_buf);
		}
...
static void *cam_cpas_user_dump_state_monitor_array_info(void *dump_struct,
							 uint8_t *addr_ptr)
{
	uint64_t *addr;
	struct cam_common_hw_dump_header *hdr;
	struct cam_cpas_monitor *monitor =
		(struct cam_cpas_monitor *)dump_struct;
	...
	*addr++ = monitor->be_shub;
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		...
		if (axi_info->applied_bw.vote_type == CAM_CPAS_VOTE_TYPE_DRV) {
			scnprintf(hdr->tag, CAM_COMMON_HW_DUMP_TAG_MAX_LEN,
				  "%s.%s.%s:", axi_info->axi_port_name, "DRV",
			...
			*addr++ = axi_info->applied_bw.drv_vote.low.ib;
			...
			scnprintf(hdr->tag, CAM_COMMON_HW_DUMP_TAG_MAX_LEN,
				  "%s.%s.%s:", axi_info->axi_port_name, "HLOS",
				  CAM_BOOL_TO_YESNO(axi_info->is_drv_started));
			addr = (uint64_t
					*)(dst +
					   sizeof(struct cam_common_hw_dump_header));
	...
	for (i = 0; i < num_vcds; i++) {
		...
		*addr++ =
			vcd_reg_debug_info->vcd_curr_lvl_debug_info[i].reg_value;
	}
 ...
 */
static int
cam_cpas_dump_state_monitor_array_info(struct cam_hw_info *cpas_hw,
				       struct cam_req_mgr_dump_info *dump_info)
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		for (j = 0; j < cpas_core->num_axi_ports; j++) {
			if (entry->axi_info[j].applied_bw.vote_type ==
				...
				min_len +=
					sizeof(struct cam_common_hw_dump_header) +
					CAM_CPAS_DUMP_NUM_WORDS_VOTE_TYEP_DRV *
						sizeof(uint64_t);
			} else {
				min_len +=
					sizeof(struct cam_common_hw_dump_header) +
					CAM_CPAS_DUMP_NUM_WORDS_VOTE_TYEP_HLOS *
						sizeof(uint64_t);
			}
		}
		...
		for (j = 0; j < soc_private->smart_qos_info->num_rt_wr_nius;
		     j++)
			min_len += sizeof(struct cam_common_hw_dump_header) +
				   CAM_CPAS_DUMP_NUM_WORDS_RT_WR_NIUS *
					   sizeof(uint64_t);

		for (j = 0; j < CAM_CPAS_MAX_CESTA_VCD_NUM; j++)
			min_len += CAM_CPAS_DUMP_NUM_WORDS_VCD_CURR_LVL *
				   sizeof(uint64_t);
	...
	for (i = 0; i < num_entries; i++) {
		rc = cam_common_user_dump_helper(
			&dump_args, cam_cpas_user_dump_state_monitor_array_info,
			&cpas_core->monitor_entries[index], sizeof(uint64_t),
			"CPAS_MONITOR.%d.%s:", index,
...
static int cam_cpas_activate_cache(struct cam_hw_info *cpas_hw,
...
static int cam_cpas_deactivate_cache(struct cam_hw_info *cpas_hw,
				     struct cam_sys_cache_info *cache_info)
{
	int rc = 0;
...
static int cam_cpas_configure_staling_cache(
	struct cam_hw_info *cpas_hw, struct cam_sys_cache_info *cache_info,
	struct cam_sys_cache_local_info *sys_cache_info)
{
	...
	if (!rc) {
		...
		CAM_ERR(CAM_CPAS,
			"llcc staling feature is not supported cache:%s",
			cache_info->name);
	} else if (rc) {
	...
	mutex_unlock(&cpas_hw->hw_mutex);
	CAM_DBG(CAM_CPAS,
		"llcc notif cache name:%s staling_distance %d cache mode :%d cache "
		"op_type :%s",
...
static int
cam_cpas_notif_stalling_inc_cache(struct cam_hw_info *cpas_hw,
				  struct cam_sys_cache_info *cache_info)
{
	int rc = 0;
...
static inline int
cam_cpas_validate_cache_type(uint32_t num_caches,
...
static int cam_cpas_get_slice_id(struct cam_hw_info *cpas_hw,
				 enum cam_sys_cache_config_types type)
{
	struct cam_cpas_private_soc *soc_private =
		(struct cam_cpas_private_soc *)cpas_hw->soc_info.soc_private;
...
static int cam_cpas_activate_cache_slice(struct cam_hw_info *cpas_hw,
					 enum cam_sys_cache_config_types type)
{
	struct cam_cpas_private_soc *soc_private =
		(struct cam_cpas_private_soc *)cpas_hw->soc_info.soc_private;
	...
	for (i = 0; i < num_caches; i++) {
		if (type == soc_private->llcc_info[i].type)
			rc = cam_cpas_activate_cache(
				cpas_hw, &soc_private->llcc_info[i]);
	}
...
static int cam_cpas_deactivate_cache_slice(struct cam_hw_info *cpas_hw,
					   enum cam_sys_cache_config_types type)
{
	struct cam_cpas_private_soc *soc_private =
		(struct cam_cpas_private_soc *)cpas_hw->soc_info.soc_private;
	...
	struct cam_sys_cache_local_info sys_cache_info)
{
	...
	for (i = 0; i < num_caches; i++) {
		if (sys_cache_info.type == soc_private->llcc_info[i].type) {
			rc = cam_cpas_configure_staling_cache(
				cpas_hw, &soc_private->llcc_info[i],
				&sys_cache_info);
			if (rc) {
				CAM_ERR(CAM_CPAS,
					"llc sys cache type %d config failed, rc: %d",
...
static int
cam_cpas_notif_stalling_inc_cache_slice(struct cam_hw_info *cpas_hw,
					enum cam_sys_cache_config_types type)
{
	struct cam_cpas_private_soc *soc_private =
		(struct cam_cpas_private_soc *)cpas_hw->soc_info.soc_private;
	...
	for (i = 0; i < num_caches; i++) {
		if (type == soc_private->llcc_info[i].type)
			rc = cam_cpas_notif_stalling_inc_cache(
				cpas_hw, &soc_private->llcc_info[i]);
	}
						   ...
						   bool set_port)
{
	...
	if ((csid_idx < 0) || (sfe_idx < 0)) {
		CAM_ERR(CAM_CPAS, "Invalid core info csid:%d sfe:%d", csid_idx,
			sfe_idx);
		return -EINVAL;
	}
	...
	for (i = 0; i < CAM_CPAS_PATH_DATA_MAX; i++) {
		for (j = 0; j < CAM_CPAS_TRANSACTION_MAX; j++) {
			tree_node = cpas_core->cpas_client[client_indx]
					    ->tree_node[i][j];
			if (!tree_node)
				continue;

			if (set_port)
				tree_node->drv_voting_idx =
					CAM_CPAS_PORT_DRV_0 + csid_idx;
			else
				tree_node->drv_voting_idx =
	...
	if (enable) {
		for (i = 0; i < domain_id_clks->number_clks; i++) {
			rc = cam_soc_util_clk_enable(
				&cpas_hw->soc_info, CAM_CLK_SW_CLIENT_IDX, true,
				domain_id_clks->clk_idx[i], 0);
			if (rc) {
				CAM_ERR(CAM_CPAS,
...
clean_up:
	for (--i; i >= 0; i--)
		cam_soc_util_clk_disable(&cpas_hw->soc_info,
...
static int cam_cpas_hw_csid_process_resume(struct cam_hw_info *cpas_hw,
					   uint32_t csid_idx)
{
	int i, rc = 0;
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		...
		rc = cam_cpas_util_vote_drv_bus_client_bw(
			...
			&cpas_core->axi_port[i].applied_bw);
		if (rc) {
			CAM_ERR(CAM_CPAS, "Failed in BW update on resume rc:%d",
				rc);
			goto end;
		}
		...
		rc = cam_cpas_drv_channel_switch_for_dev(
			cpas_core->axi_port[i].cam_rsc_dev);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				   ...
				   void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_CPAS_HW_CMD_CONFIGURE_STALING_LLC: {
		...
		sys_cache_info = *((struct cam_sys_cache_local_info *)cmd_args);
		rc = cam_cpas_configure_staling_cache_slice(hw_priv,
							    sys_cache_info);
	} break;
	...
	case CAM_CPAS_HW_CMD_CSID_INPUT_CORE_INFO_UPDATE: {
		...
		if (sizeof(struct cam_cpas_hw_cmd_csid_input_core_info_update) !=
		    arg_size) {
			CAM_ERR(CAM_CPAS, "cmd_type %d, size mismatch %d",
				cmd_type, arg_size);
			break;
		...
		core_info_update =
			(struct cam_cpas_hw_cmd_csid_input_core_info_update *)
				cmd_args;
		rc = cam_cpas_hw_csid_input_core_info_update(
			hw_priv, core_info_update->csid_idx,
...
static int cam_cpas_util_create_debugfs(struct cam_cpas *cpas_core)
{
	...
	debugfs_create_bool("full_state_dump", 0644, cpas_core->dentry,
			    &cpas_core->full_state_dump);

	debugfs_create_bool("smart_qos_dump", 0644, cpas_core->dentry,
			    &cpas_core->smart_qos_dump);

	debugfs_create_bool("force_hlos_drv", 0644, cpas_core->dentry,
			    &cpas_core->force_hlos_drv);

	debugfs_create_bool("force_cesta_sw_client", 0644, cpas_core->dentry,
			    &cpas_core->force_cesta_sw_client);
...
static ssize_t cam_cpas_sysfs_get_subparts_info(struct kobject *kobj,
						...
						char *buf)
{
   ...
   * num_ifes: 0x3, 0x3 (If all IFEs are available)/0x2 (If 1 IFE is
   * unavailable) num_ife_lites: 0x2, 0x2 num_sfes: 0x3, 0x3 (If all SFEs are
   * available)/0x2 (If 1 SFE is unavailable) num_custom: 0x0, 0x0
   */
...
static struct kobj_attribute cam_subparts_info_attribute =
	__ATTR(subparts_info, 0444, cam_cpas_sysfs_get_subparts_info, NULL);
...
static void cam_cpas_remove_sysfs(struct cam_hw_info *cpas_hw)
{
	...
	sysfs_remove_file(soc_private->sysfs_info.kobj,
			  &cam_subparts_info_attribute.attr);
...
static int cam_cpas_create_sysfs(struct cam_hw_info *cpas_hw)
{
	...
	rc = sysfs_create_file(&kobj_camera->base_kobj,
			       &cam_subparts_info_attribute.attr);
	if (rc) {
		CAM_ERR(CAM_CPAS, "failed to create subparts_info file, rc: %d",
			rc);
		goto end;
	}
		      ...
		      struct cam_hw_intf **hw_intf)
{
	...
	rc = cam_get_subpart_info(&soc_private->part_info,
				  CAM_CPAS_CAMERA_INSTANCES);
	if (rc) {
