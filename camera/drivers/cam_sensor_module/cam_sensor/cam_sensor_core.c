...
static int cam_sensor_notify_v4l2_error_event(struct cam_sensor_ctrl_t *s_ctrl,
					      uint32_t error_type,
					      uint32_t error_code)
{
	int rc = 0;
...
static int cam_sensor_update_req_mgr(struct cam_sensor_ctrl_t *s_ctrl,
	...
	if (s_ctrl->bridge_intf.crm_cb && s_ctrl->bridge_intf.crm_cb->add_req) {
		...
		if (rc) {
			if (rc == -EBADR)
				CAM_INFO(
					CAM_SENSOR,
					"Adding request: %llu failed with request manager rc: %d, it "
					"has been flushed",
				      ...
				      struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_INFO(
		CAM_SENSOR,
		"Sensor[%s-%d] Feature: 0x%x updated for request id: %lu, res "
		"index: %u, width: 0x%x, height: 0x%x, capability: %s, fps: %u",
...
static int32_t cam_sensor_pkt_parse(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
	int32_t rc = 0;
	...
	case CAM_SENSOR_PACKET_OPCODE_SENSOR_BUBBLE_UPDATE: {
		...
		i2c_reg_settings =
			&i2c_data->bubble_update[csl_packet->header.request_id %
						 MAX_PER_FRAME_ARRAY];
		CAM_DBG(CAM_SENSOR,
			"Received bubble update packet: %lld req: %lld",
			csl_packet->header.request_id % MAX_PER_FRAME_ARRAY,
			csl_packet->header.request_id);
		break;
	}
	...
	case CAM_SENSOR_PACKET_OPCODE_SENSOR_WRITE: {
		...
		rc = cam_sensor_apply_settings(
			s_ctrl, 0, CAM_SENSOR_PACKET_OPCODE_SENSOR_WRITE);
	...
	case CAM_SENSOR_PACKET_GENERIC_BLOB:
		if ((csl_packet->header.op_code & 0xFFFFFF) !=
			...
			rc = -EINVAL;
			CAM_ERR(CAM_SENSOR,
				"Wrong packet opcode sent with blob: %u",
				csl_packet->header.op_code & 0xFFFFFF);
			goto end;
		}
     ...
     */
		...
		rc = cam_packet_util_process_generic_cmd_buffer(
			cmd_desc, cam_sensor_generic_blob_handler, s_ctrl);
		if (rc)
   ...
   */
	if ((!s_ctrl->is_res_info_updated) &&
	    (csl_packet->header.request_id != 0)) {
		/*
     * Update the last updated req at two places.
     * 1# Got generic blob: The req id can be zero for the initial res info
     * updating 2# Copy previous res info: The req id can't be zero, in case
     * some queue info are override by slot0.
     */
		s_ctrl->last_updated_req = csl_packet->header.request_id;
		s_ctrl->sensor_res[s_ctrl->last_updated_req %
				   MAX_PER_FRAME_ARRAY] =
			s_ctrl->sensor_res[prev_updated_req %
					   MAX_PER_FRAME_ARRAY];
	}
...
int32_t cam_sensor_update_slave_info(void *probe_info, uint32_t cmd,
				     struct cam_sensor_ctrl_t *s_ctrl,
				     uint8_t probe_ver)
{
	int32_t rc = 0;
	...
	if (probe_ver == CAM_SENSOR_PACKET_OPCODE_SENSOR_PROBE) {
		...
		s_ctrl->pipeline_delay = sensor_probe_info->reserved;
		...
		s_ctrl->pipeline_delay = (sensor_probe_info_v2->pipeline_delay &
	...
	CAM_DBG(CAM_SENSOR,
		"%s Sensor Addr: 0x%x sensor_id: 0x%x sensor_mask: 0x%x "
		"sensor_pipeline_delay:0x%x",
		s_ctrl->sensor_name,
		s_ctrl->sensordata->slave_info.sensor_id_reg_addr,
		s_ctrl->sensordata->slave_info.sensor_id,
		s_ctrl->sensordata->slave_info.sensor_id_mask,
...
int32_t cam_handle_cmd_buffers_for_probe(void *cmd_buf,
					 ...
					 size_t remain_len, uint32_t probe_ver,
			   ...
			   struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	for (i = 0; i < pkt->num_cmd_buf; i++) {
		...
		rc = cam_handle_cmd_buffers_for_probe(ptr, s_ctrl, i, cmd,
						      cmd_desc[i].length,
						      remain_len, probe_ver,
						      &cmd_desc[i]);
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_DBG(CAM_SENSOR,
		"%s read id: 0x%x expected id 0x%x:", s_ctrl->sensor_name,
...
int cam_sensor_stream_off(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensor_state != CAM_SENSOR_START) {
		rc = -EINVAL;
		CAM_WARN(CAM_SENSOR, "Not in right state to stop %s state: %d",
	...
	CAM_INFO(CAM_SENSOR,
		 "%llu:%llu:%llu.%llu CAM_STOP_DEV Success for %s "
		 "sensor_id:0x%x,sensor_slave_addr:0x%x",
		 hrs, min, sec, ms, s_ctrl->sensor_name,
		 s_ctrl->sensordata->slave_info.sensor_id,
...
int32_t cam_sensor_driver_cmd(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		if (s_ctrl->i2c_data.reg_bank_lock_settings.is_settings_valid) {
			...
			rc = delete_request(
				&(s_ctrl->i2c_data.reg_bank_lock_settings));
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR,
     ...
     */
		...
		if (rc < 0) {
			CAM_WARN(CAM_SENSOR,
				 "Create sensor debugfs failed rc: %d", rc);
			rc = 0;
		}
	...
	case CAM_ACQUIRE_DEV: {
		...
		if (s_ctrl->bridge_intf.device_hdl != -1) {
			CAM_ERR(CAM_SENSOR, "%s Device is already acquired",
				s_ctrl->sensor_name);
			rc = -EINVAL;
			goto release_mutex;
		}
		...
		CAM_DBG(CAM_SENSOR, "%s Device Handle: %d", s_ctrl->sensor_name,
			sensor_acq_dev.device_handle);
		...
		lock_power_sync_mutex(s_ctrl->io_master_info.cci_client,
				      s_ctrl->cci_i2c_master); // xiaomi add
		rc = cam_sensor_power_up(s_ctrl);
		unlock_power_sync_mutex(s_ctrl->io_master_info.cci_client,
					s_ctrl->cci_i2c_master); // xiaomi add
	...
	case CAM_START_DEV: {
		...
		CAM_INFO(
			CAM_SENSOR,
			"%llu:%llu:%llu.%llu CAM_START_DEV Success for %s "
			"sensor_id:0x%x,sensor_slave_addr:0x%x num_batched_frames:%d",
			hrs, min, sec, ms, s_ctrl->sensor_name,
			s_ctrl->sensordata->slave_info.sensor_id,
			s_ctrl->sensordata->slave_info.sensor_slave_addr,
	...
	case CAM_STOP_DEV: {
		if (s_ctrl->stream_off_after_eof) {
			...
			CAM_DBG(CAM_SENSOR,
				"Ignore stop dev cmd for VFPS feature");
			goto release_mutex;
		}
	...
	case CAM_CONFIG_DEV: {
		...
		if (s_ctrl->i2c_data.init_settings.is_settings_valid &&
			...
			CAM_DBG(MI_PERF, "%s start initial settings",
				s_ctrl->sensor_name);
			/* xiaomi add end */
			pkt_opcode =
				CAM_SENSOR_PACKET_OPCODE_SENSOR_INITIAL_CONFIG;
         ...
         */
			...
			CAM_DBG(MI_PERF,
				"%s end initial settings, occupy time is: %ld ms",
		...
		if (s_ctrl->i2c_data.config_settings.is_settings_valid &&
			...
			/* xiaomi add begin */
			CAM_GET_TIMESTAMP(ts1);
			CAM_DBG(MI_PERF, "%s start config settings",
				s_ctrl->sensor_name);
			/* xiaomi add end */
			rc = cam_sensor_apply_settings(
				s_ctrl, 0,
			...
			CAM_GET_TIMESTAMP_DIFF_IN_MICRO(ts1, ts2, microsec);
			CAM_DBG(MI_PERF,
				"%s end config settings, occupy time is: %ld ms",
				s_ctrl->sensor_name, microsec / 1000);
			/* xiaomi add end */
		...
		if (s_ctrl->i2c_data.read_settings.is_settings_valid) {
			...
			rc = delete_request(&s_ctrl->i2c_data.read_settings);
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR,
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->aon_camera_id != NOT_AON_CAM) {
		CAM_INFO(CAM_SENSOR,
			 ...
			 s_ctrl->sensordata->subdev_id[SUB_MODULE_CSIPHY]);
		rc = cam_sensor_util_aon_ops(
			true, s_ctrl->sensordata->subdev_id[SUB_MODULE_CSIPHY]);
		if (rc) {
			CAM_ERR(CAM_SENSOR,
	...
	if (rc < 0) {
		...
		// add by xiaomi
		cam_hw_notify_v4l2_error_event(CAM_SENSOR_NAME, (void *)s_ctrl,
					       V4L_EVENT_HW_ISSUE_POWER_ERROR,
					       HW_ISSUE_ERROR_TYPE_SENSOR_POWER,
					       power_info->fail_type);
		// end
		return rc;
	}
...
int cam_sensor_power_down(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->aon_camera_id != NOT_AON_CAM) {
		CAM_INFO(CAM_SENSOR, "Setup for AON FW with csiphy index: %d",
			 s_ctrl->sensordata->subdev_id[SUB_MODULE_CSIPHY]);
		rc = cam_sensor_util_aon_ops(
			false,
			s_ctrl->sensordata->subdev_id[SUB_MODULE_CSIPHY]);
	...
	if (s_ctrl->bob_pwm_switch) {
		...
		if (rc) {
			CAM_WARN(CAM_SENSOR, "%s BoB PWM setup failed rc: %d",
				 s_ctrl->sensor_name, rc);
			rc = 0;
		}
	}
	...
	if (req_id == 0) {
			...
			list_for_each_entry(i2c_list, &(i2c_set->list_head),
				...
				case CAM_SENSOR_I2C_WRITE_SEQ: {
					for (j = 0;
						...
						if (0x3010 ==
							...
							CAM_WARN(
								CAM_SENSOR,
								"_DBG_ sensorname[%s] [%04d] [CDBG] 0x%04X 0x%04X "
								"0x%02X reqid %d",
								s_ctrl->sensor_name,
								j,
								i2c_list->i2c_settings
				...
				if (rc < 0) {
					...
					cam_hw_notify_v4l2_error_event(
						CAM_SENSOR_NAME, (void *)s_ctrl,
						V4L_EVENT_HW_ISSUE_CCI_ERROR,
						HW_ISSUE_ERROR_TYPE_SENSOR_CCI,
						cam_hw_get_cci_ops(i2c_list));
					// end
					return rc;
				}
			}
       ...
       */
		...
		if (i2c_set[offset].is_settings_valid == 1 &&
		    i2c_set[offset].request_id == req_id) {
			list_for_each_entry(
				i2c_list, &(i2c_set[offset].list_head), list) {
				/* xiaomi add I2C trace begin */
				switch (i2c_list->op_code) {
				case CAM_SENSOR_I2C_WRITE_RANDOM:
				case CAM_SENSOR_I2C_WRITE_BURST:
		...
		CAM_DBG(CAM_REQ,
			"Sensor[%d] updating last_applied [req id: %lld last_applied: "
			"%lld] with opcode:%d",
			s_ctrl->soc_info.index, req_id,
			s_ctrl->last_applied_req, opcode);

		/* Change the logic dynamically */
		for (i = 0; i < MAX_PER_FRAME_ARRAY; i++) {
			if ((req_id >= i2c_set[i].request_id) &&
			    (top < i2c_set[i].request_id) &&
			    (i2c_set[i].is_settings_valid == 1)) {
				del_req_id = top;
				top = i2c_set[i].request_id;
			}
		}
		...
		for (i = 0; i < MAX_PER_FRAME_ARRAY; i++) {
			if ((del_req_id > i2c_set[i].request_id) &&
			    (i2c_set[i].is_settings_valid == 1)) {
				i2c_set[i].request_id = 0;
				rc = delete_request(&(i2c_set[i]));
				if (rc < 0)
					CAM_ERR(CAM_SENSOR,
...
int32_t cam_sensor_apply_request(struct cam_req_mgr_apply_request *apply)
{
	...
	if ((apply->recovery) && (apply->request_id > 0)) {
		...
		last_applied_idx =
			s_ctrl->last_applied_req % MAX_PER_FRAME_ARRAY;
     ...
     */
		...
		if ((s_ctrl->sensor_res[curr_idx].res_index !=
			...
			CAM_INFO(
				CAM_REQ,
				"Sensor[%d] update req id: %lld [last_applied: %lld] with opcode:%d "
				"recovery: %d last_applied_res_idx: %u current_res_idx: %u",
	...
	trace_cam_apply_req("Sensor", s_ctrl->soc_info.index, apply->request_id,
			    apply->link_hdl);

	mutex_lock(&(s_ctrl->cam_sensor_mutex));
	rc = cam_sensor_apply_settings(s_ctrl, apply->request_id, opcode);
	mutex_unlock(&(s_ctrl->cam_sensor_mutex));
	return rc;
}
