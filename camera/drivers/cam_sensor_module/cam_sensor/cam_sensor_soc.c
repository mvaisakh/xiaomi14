...
static int32_t cam_sensor_driver_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "Failed in Initialize Bus params, rc %d",
			rc);
		goto FREE_SENSOR_DATA;
	}
	...
	if (s_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		CAM_DBG(CAM_SENSOR, "cci-master %d", s_ctrl->cci_i2c_master);
	...
	if (of_property_read_u32(of_node, "aon-camera-id",
				 &s_ctrl->aon_camera_id)) {
		CAM_DBG(CAM_SENSOR, "cell_idx: %d is not used for AON usecase",
			soc_info->index);
...
int32_t cam_sensor_parse_dt(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	init_power_sync_mutex(s_ctrl->io_master_info.cci_client,
