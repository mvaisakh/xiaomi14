				    ...
				    struct tpg_global_config_t *configs)
{
	...
	cam_io_w_mb(val, soc_info->reg_map[1].mem_base +
				 tpg_reg->top_mux_reg_offset);
	CAM_INFO(CAM_ISP, "TPG:%d Set top Mux: 0x%x", hw->hw_idx, val);

	val = ((4 - 1) << tpg_reg->tpg_num_active_lines_shift) |
			...
			struct tpg_stream_config_t *stream)
{
	...
	val = ((num_dts - 1) << tpg_reg->tpg_num_dts_shift_val) | stream->vc;
...
static int tpg_hw_v_1_0_reset(struct tpg_hw *hw, void *data)
{
	...
	cam_io_w_mb(val, soc_info->reg_map[1].mem_base +
				 tpg_reg->top_mux_reg_offset);
	CAM_INFO(CAM_TPG, "TPG:%d Reset Top Mux: 0x%x", hw->hw_idx, val);

	return 0;
}

int tpg_hw_v_1_0_process_cmd(struct tpg_hw *hw, uint32_t cmd, void *arg)
{
	if (hw == NULL) {
		CAM_ERR(CAM_TPG, "invalid argument");
		return -EINVAL;
	...
	case TPG_CONFIG_VC: {
		...
		configure_vc(hw, vc_config->vc_slot, vc_config->num_dts,
			     vc_config->stream);
	} break;
	case TPG_CONFIG_DT: {
		...
		configure_dt(hw, dt_config->vc_slot, dt_config->dt_slot,
			     dt_config->stream);
	} break;
