				    ...
				    struct tpg_global_config_t *configs)
{
	...
	cam_io_w_mb(1,
		    soc_info->reg_map[0].mem_base + tpg_reg->tpg_top_irq_mask);

	val = ((num_vcs - 1) << (tpg_reg->tpg_num_active_vcs_shift) |
			   ...
			   struct tpg_stream_config_v3_t *stream)
{
	...
	if (stream->cfa_info_exist != 0) {
		val |= ((stream->cfa_info.pattern_height - 1)
			<< tpg_reg->tpg_size_y_shift);
		val |= ((stream->cfa_info.pattern_width - 1)
			<< tpg_reg->tpg_size_x_shift);
	...
	val = ((0 << tpg_reg->tpg_num_frames_shift_val) |
	       ((num_dts - 1) << tpg_reg->tpg_num_dts_shift_val) | stream->vc);
	...
	if (hw->hw_info->shdr_overlap == 1) {
		cam_io_w_mb(hw->hw_info->shdr_overlap
				    << tpg_reg->tpg_overlap_shdr_en_shift,
...
static int tpg_hw_v_1_3_reset(struct tpg_hw *hw, void *data)
{
	struct cam_hw_soc_info *soc_info = NULL;
	uint32_t val;
	struct cam_tpg_ver_1_3_reg_offset *tpg_reg = NULL;
	...
	cam_io_w_mb(1,
		    soc_info->reg_map[0].mem_base + tpg_reg->tpg_top_irq_clear);

	cam_io_w_mb(1,
		    soc_info->reg_map[0].mem_base + tpg_reg->tpg_top_irq_cmd);
...
int tpg_hw_v_1_3_process_cmd(struct tpg_hw *hw, uint32_t cmd, void *arg)
{
	int rc = 0;
	if (hw == NULL) {
...
int tpg_hw_v_1_3_dump_status(struct tpg_hw *hw, void *data)
{
	...
	CAM_INFO(CAM_TPG, "TPG[%d] TPG HW status: 0x%x started", hw->hw_idx,
		 val);
	val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
			  tpg_reg->tpg_top_irq_status);
	CAM_INFO(CAM_TPG, "TPG[%d] TPG HW irq status: 0x%x started", hw->hw_idx,
		 val);
