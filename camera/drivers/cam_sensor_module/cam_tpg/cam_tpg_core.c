...
int cam_tpg_publish_dev_info(struct cam_req_mgr_device_info *info)
{
	int rc = 0;
	...
	tpg_dev = (struct cam_tpg_device *)cam_get_device_priv(info->dev_hdl);
...
int cam_tpg_setup_link(struct cam_req_mgr_core_dev_link_setup *link)
{
	struct cam_tpg_device *tpg_dev = NULL;
	...
	tpg_dev = (struct cam_tpg_device *)cam_get_device_priv(link->dev_hdl);
	if (!tpg_dev) {
		CAM_ERR(CAM_TPG, "Device data is NULL");
		return -EINVAL;
	}
	...
	if (link->link_enable) {
		...
		CAM_DBG(CAM_TPG, "TPG[%d] CRM enable link done",
			tpg_dev->soc_info.index);
	} else {
		tpg_dev->crm_intf.link_hdl = -1;
		tpg_dev->crm_intf.crm_cb = NULL;
		CAM_DBG(CAM_TPG, "TPG[%d] CRM disable link done",
			tpg_dev->soc_info.index);
	}
...
static int cam_tpg_notify_frame_skip(struct cam_req_mgr_apply_request *apply)
{
	struct cam_tpg_device *tpg_dev = NULL;
	...
	CAM_DBG(CAM_TPG, "TPG[%d] Got Skip frame from crm for request %lld",
		tpg_dev->soc_info.index, apply->request_id);
	return 0;
}

static int cam_tpg_apply_req(struct cam_req_mgr_apply_request *apply)
{
	struct cam_tpg_device *tpg_dev = NULL;
	...
	CAM_DBG(CAM_TPG, "TPG[%d] Got Apply request from crm %lld",
		tpg_dev->soc_info.index, apply->request_id);
	mutex_lock(&tpg_dev->mutex);
	tpg_hw_apply(&tpg_dev->tpg_hw, apply->request_id);
	mutex_unlock(&tpg_dev->mutex);

	return 0;
}

static int cam_tpg_flush_req(struct cam_req_mgr_flush_request *flush)
{
	CAM_DBG(CAM_TPG, "Got Flush request from crm");
	return 0;
}

static int cam_tpg_process_crm_evt(struct cam_req_mgr_link_evt_data *event)
{
	struct cam_tpg_device *tpg_dev = NULL;
	if (!event) {
	...
	case CAM_REQ_MGR_LINK_EVT_SOF_FREEZE:
		...
		break;
	default:
		CAM_DBG(CAM_TPG, "Got crm event notification: %d",
			event->evt_type);
		break;
	}
	return 0;
...
static int cam_tpg_dump_req(struct cam_req_mgr_dump_info *dump_info)
{
...
int tpg_crm_intf_init(struct cam_tpg_device *tpg_dev)
{
	...
	tpg_dev->crm_intf.ops.notify_frame_skip = cam_tpg_notify_frame_skip;
...
static int __cam_tpg_handle_query_cap(struct cam_tpg_device *tpg_dev,
...
static int __cam_tpg_handle_acquire_dev(struct cam_tpg_device *tpg_dev,
					struct cam_tpg_acquire_dev *acquire)
{
	...
	acquire->device_handle = cam_create_device_hdl(&crm_intf_params);
	tpg_dev->crm_intf.device_hdl = acquire->device_handle;
	tpg_dev->crm_intf.session_hdl = acquire->session_handle;
	rc = tpg_hw_acquire(&tpg_dev->tpg_hw,
			    (struct tpg_hw_acquire_args *)NULL);
	if (rc) {
		CAM_ERR(CAM_TPG, "TPG[%d] hw acquire failed",
			tpg_dev->soc_info.index);
		...
		CAM_INFO(CAM_TPG, "TPG[%d] Acquire Device done",
			 tpg_dev->soc_info.index);
	}
exit:
	return rc;
...
static int __cam_tpg_handle_release_dev(struct cam_tpg_device *tpg_dev,
					struct cam_release_dev_cmd *release)
{
	...
	if (rc) {
		...
		tpg_dev->crm_intf.device_hdl = -1;
		tpg_dev->crm_intf.session_hdl = -1;
		CAM_INFO(CAM_TPG, "TPG[%d] Release Done.",
			 tpg_dev->soc_info.index);
		tpg_dev->state = CAM_TPG_STATE_INIT;
	}
...
static int __cam_tpg_handle_start_dev(struct cam_tpg_device *tpg_dev,
				      struct cam_start_stop_dev_cmd *start)
{
	...
	if (rc) {
		CAM_ERR(CAM_TPG, "TPG[%d] START_DEV failed",
			tpg_dev->soc_info.index);
	} else {
		tpg_dev->state = CAM_TPG_STATE_START;
		CAM_INFO(CAM_TPG, "TPG[%d] START_DEV done.",
			 tpg_dev->soc_info.index);
	}
...
static int __cam_tpg_handle_stop_dev(struct cam_tpg_device *tpg_dev,
				     struct cam_start_stop_dev_cmd *stop)
{
	...
	rc = tpg_hw_stop(&tpg_dev->tpg_hw);
	if (rc) {
		CAM_ERR(CAM_TPG, "TPG[%d] STOP_DEV failed",
			tpg_dev->soc_info.index);
	} else {
		tpg_dev->state = CAM_TPG_STATE_ACQUIRE;
		CAM_INFO(CAM_TPG, "TPG[%d] STOP_DEV done.",
			 tpg_dev->soc_info.index);
	}
...
static int cam_tpg_validate_cmd_descriptor(struct cam_cmd_buf_desc *cmd_desc,
					   uint32_t *cmd_type,
					   uintptr_t *cmd_addr)
{
	int rc = 0;
	...
	rc = cam_mem_get_cpu_buf(cmd_desc->mem_handle, &generic_ptr,
				 &len_of_buff);
	if (rc < 0) {
		CAM_ERR(CAM_TPG, "Failed to get cmd buf Mem address : %d", rc);
		return rc;
	}
	...
	case TPG_CMD_TYPE_ILLUMINATION_CONFIG: {
		if (cmd_header->size !=
		    sizeof(struct tpg_illumination_control)) {
			CAM_ERR(CAM_TPG,
...
static int cam_tpg_cmd_buf_parse(struct cam_tpg_device *tpg_dev,
				 struct cam_packet *packet)
{
	...
	rc = tpg_hw_request_set_opcode(req, packet->header.op_code & 0xFF);
	if (rc < 0)
		goto free_request;

	CAM_DBG(CAM_TPG, "Queue TPG requests for request id %lld",
		req->request_id);
	...
	for (i = 0; i < packet->num_cmd_buf; i++) {
		...
		cmd_desc = (struct cam_cmd_buf_desc
				    *)((uint32_t *)&packet->payload +
				       ...
				       (i *
					(sizeof(struct cam_cmd_buf_desc) / 4)));

		rc = cam_tpg_validate_cmd_descriptor(cmd_desc, &cmd_type,
						     &cmd_addr);
		if (rc < 0) {
			kfree(req);
			goto end;
		...
		case TPG_CMD_TYPE_GLOBAL_CONFIG:
			...
			memcpy(&req->global_config,
			       ...
			       sizeof(struct tpg_global_config_t));
			tpg_dev->tpg_hw.tpg_clock =
				req->global_config.tpg_clock;
			break;
		case TPG_CMD_TYPE_STREAM_CONFIG: {
			if (cmd_header->cmd_version == 3) {
				rc = tpg_hw_add_stream_v3(
					&tpg_dev->tpg_hw, req,
		...
		default:
			CAM_ERR(CAM_TPG, "TPG[%d] invalid command %d",
				tpg_dev->soc_info.index, cmd_type);
			rc = -EINVAL;
			goto free_request;
			break;
		}
...
static int cam_tpg_packet_parse(struct cam_tpg_device *tpg_dev,
				struct cam_config_dev_cmd *config)
{
	...
	CAM_DBG(CAM_TPG,
		...
		tpg_dev->soc_info.index, (csl_packet->header.op_code & 0xFF),
		csl_packet->num_cmd_buf, csl_packet->num_io_configs,
	...
	case CAM_TPG_PACKET_OPCODE_UPDATE: {
		...
		CAM_DBG(CAM_TPG, "TPG[%d] packet request id : %llu",
			tpg_dev->soc_info.index, csl_packet->header.request_id);
...
static int __cam_tpg_handle_config_dev(struct cam_tpg_device *tpg_dev,
				       struct cam_config_dev_cmd *config)
{
	int rc = 0;
...
static int validate_ioctl_params(struct cam_tpg_device *tpg_dev,
				 struct cam_control *cmd)
{
	...
	if (cmd->handle_type != CAM_HANDLE_USER_POINTER) {
		CAM_ERR(CAM_TPG, "TPG[%d] Invalid handle type: %d",
			tpg_dev->soc_info.index, cmd->handle_type);
		rc = -EINVAL;
	}
	CAM_DBG(CAM_TPG, "TPG[%d] Opcode: %d", tpg_dev->soc_info.index,
		cmd->op_code);
exit:
	return rc;
}

int cam_tpg_core_cfg(struct cam_tpg_device *tpg_dev, void *arg)
{
	int rc = 0;
	...
	case CAM_ACQUIRE_DEV: {
		...
		if (rc) {
			CAM_ERR(CAM_TPG,
				"TPG[%d] acquire device failed(rc = %d)",
				tpg_dev->soc_info.index, rc);
			break;
		}
