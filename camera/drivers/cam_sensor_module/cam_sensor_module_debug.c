...
#define USAGE_STRING                                                 \
	"Read format: r/R, reg_addr(hex), addr_type, "               \
...
struct camera_io_master *cam_sensor_module_get_io_master(int device_type,
							 int instance_number,
							 bool *is_on)
{
	...
	*is_on = false;
	...
	case CAM_SENSOR_ACTUATOR:
		io_master = instance_number >= devices.num_actuator ?
				    NULL :
				    &devices.actuator[instance_number]
					     ->io_master_info;
		if (io_master)
			*is_on = devices.actuator[instance_number]
							 ->cam_act_state ==
						 CAM_ACTUATOR_CONFIG ?
					 true :
					 false;
	...
	case CAM_SENSOR_EEPROM:
		io_master =
			instance_number >= devices.num_eeprom ?
				NULL :
				&devices.eeprom[instance_number]->io_master_info;
		if (io_master)
			*is_on = devices.eeprom[instance_number]
							 ->cam_eeprom_state ==
						 CAM_EEPROM_CONFIG ?
					 true :
					 false;
	...
	case CAM_SENSOR_FLASH:
		io_master =
			instance_number >= devices.num_flash ?
				NULL :
	...
	case CAM_SENSOR_OIS:
		io_master =
			instance_number >= devices.num_ois ?
				NULL :
	...
	case CAM_SENSOR_DEVICE:
		io_master =
			instance_number >= devices.num_sensor ?
				NULL :
				&devices.sensor[instance_number]->io_master_info;
		if (io_master)
			*is_on =
				devices.sensor[instance_number]->sensor_state >=
						CAM_SENSOR_ACQUIRE ?
					true :
					false;
		break;
	default:
		CAM_WARN(CAM_SENSOR, "Unrecognized sensor device type: %d",
			 device_type);
		break;
	}
...
static int cam_sensor_module_parse_line(
	const char *p_line, struct cam_sensor_i2c_reg_setting *reg_list,
	struct camera_io_master **io_master, bool *is_read, bool *power_state)
{
	...
	if (p_line[0] == 'r' || p_line[0] == 'R') {
		*is_read = true;
		...
		rc = sscanf(p_line + 2, "%x,%d,%d,%d,%d", &reg_array->reg_addr,
		...
		*is_read = false;
		...
		rc = sscanf(p_line + 2, "%x,%x,%u,%d,%d,%d,%d",
			    &reg_array->reg_addr, &reg_array->reg_data,
			    &reg_array->delay, &reg_list->addr_type,
			    &reg_list->data_type, &device_type,
			    &instance_number);
...
static void cam_sensor_get_device_status(int offset)
{
	...
	for (i = 0; i < devices.num_actuator; i++) {
		snprintf(
			...
			"Device: %s, \t %u, state: %d, ID(device_type, instance_number): "
			"%d, %d\n",
			devices.actuator[i]->device_name,
			devices.actuator[i]->soc_info.index,
			devices.actuator[i]->cam_act_state, CAM_SENSOR_ACTUATOR,
			i);
	...
	for (i = 0; i < devices.num_eeprom; i++) {
		snprintf(line_buffer, LINE_BUFFER_SIZE,
			 "Device: %s, \t\t %u, state: %d, ID(device_type, "
			 "instance_number): %d, %d\n",
			 devices.eeprom[i]->device_name,
			 devices.eeprom[i]->soc_info.index,
			 devices.eeprom[i]->cam_eeprom_state, CAM_SENSOR_EEPROM,
			 i);
		strlcat(start, line_buffer, RW_BUFFER_SIZE - offset);
	}
	memset(line_buffer, '\0', LINE_BUFFER_SIZE);
	for (i = 0; i < devices.num_flash; i++) {
		snprintf(line_buffer, LINE_BUFFER_SIZE,
			 "Device: %s, \t\t %u, state: %d, ID(device_type, "
			 "instance_number): %d, %d\n",
			 devices.flash[i]->device_name,
			 devices.flash[i]->soc_info.index,
	...
	for (i = 0; i < devices.num_ois; i++) {
		snprintf(line_buffer, LINE_BUFFER_SIZE,
			 "Device: %s, \t\t %u, state: %d, ID(device_type, "
			 "instance_number): %d, %d\n",
			 devices.ois[i]->device_name,
			 devices.ois[i]->soc_info.index,
	...
	for (i = 0; i < devices.num_sensor; i++) {
		snprintf(line_buffer, LINE_BUFFER_SIZE,
			 "Device: %s, \t\t %u, state: %d, ID(device_type, "
			 "instance_number): %d, %d\n",
			 devices.sensor[i]->sensor_name,
			 devices.sensor[i]->soc_info.index,
			...
			size_t t_size_t, loff_t *t_loff_t)
{
	...
	count = simple_read_from_buffer(t_char, t_size_t, t_loff_t, out_buffer,
					RW_BUFFER_SIZE);
			 ...
			 size_t t_size_t, loff_t *t_loff_t)
{
	...
	rc = cam_sensor_module_parse_line(in_buffer, &read_write, &io_master,
					  &is_read, &power_state);
	...
	if (!rc) {
		...
		if (!is_read) {
			...
			rc = camera_io_dev_read(
				io_master, read_write.reg_setting->reg_addr,
				&read_write.reg_setting->reg_data,
				read_write.addr_type, read_write.data_type,
				false);
			if (!rc)
				snprintf(line_buffer, LINE_BUFFER_SIZE,
					 "Read data: 0x%X\n",
...
int cam_sensor_module_debug_register(void)
{
	...
	debugfs_create_file("i2c-rw", 0644, debugfs_root, NULL,
			    &i2c_operations);
