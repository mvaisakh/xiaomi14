...
static inline void cam_qup_i2c_txdata_fill(struct camera_io_master *dev_client,
					   unsigned char *txdata, int length,
					   struct i2c_msg *msgs, int curr_mindx)
{
...
cam_qup_i2c_write_optimized(struct camera_io_master *client,
			    struct cam_sensor_i2c_reg_setting *write_setting,
			    struct i2c_msg *msgs, unsigned char *buf)
{
	int32_t rc = 0;
	uint8_t len = 0;
	...
	while (i < write_setting->size) {
			...
			if (i < write_setting->size) {
				...
				isLookAhead =
					((reg_setting_previous->reg_addr + 1) ==
						 /* xiaomi add "&& data_type + len <=
                              CAM_MAX_NUM_QUPI2C_PAYLOAD_BYTES" start*/
						 reg_setting->reg_addr &&
					 data_type + len <=
						 CAM_MAX_NUM_QUPI2C_PAYLOAD_BYTES) ?
						true :
						false;
				/* xiaomi add "&& data_type + len <= CAM_MAX_NUM_QUPI2C_PAYLOAD_BYTES"
         * end*/
			} else {
				break;
			}
		...
		CAM_DBG(CAM_SENSOR, "offset: %d len: %d curr_mindx: %d", offset,
			len, curr_mindx);
		cam_qup_i2c_txdata_fill(client, &buf[offset - len], len, msgs,
					curr_mindx);
			...
			struct cam_sensor_i2c_reg_setting *write_setting)
{
	...
	buf = kzalloc(write_setting->size * I2C_REG_MAX_BUF_SIZE,
		      GFP_KERNEL | GFP_DMA);
	if (!buf) {
	...
	i2c_msg_size =
		cam_qup_i2c_write_optimized(client, write_setting, msgs, buf);
	if (i2c_msg_size < 0) {
		rc = i2c_msg_size;
		goto deallocate_buffer;
	}
