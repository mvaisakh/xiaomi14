...
static int cam_ois_power_up(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->io_master_info.master_type == I3C_MASTER)
		i3c_probe_completion =
			cam_ois_get_i3c_completion(o_ctrl->soc_info.index);
			       ...
			       enum cam_endianness_type endianness)
{
	...
	if (endianness == CAM_ENDIANNESS_BIG)
		qtime_ns = swap_high_word_and_low_word(
			(uint16_t *)(&qtime_ns),
	...
	list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
		if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_SEQ) {
			...
			if (size * (uint32_t)(i2c_list->i2c_settings.data_type) !=
			    8) {
			...
			case CAMERA_SENSOR_I2C_TYPE_BYTE:
				for (i = 0; i < size; i++) {
					CAM_DBG(CAM_OIS,
						"time: reg_data[%d]: 0x%x", i,
						(qtime_ns & 0xFF));
					i2c_list->i2c_settings.reg_setting[i]
						.reg_data = (qtime_ns & 0xFF);
					qtime_ns >>= 8;
				}
			...
			for (i = 0; i < size; i++) {
				...
				if (o_ctrl->opcode.is_littleendian_op) {
					...
					if (I2C_COMPARE_MATCH == rc) {
						CAM_DBG(CAM_OIS,
							"i2c poll addr 0x%x data 0x%x success",
							i2c_list->i2c_settings
								.reg_setting[i]
								.reg_addr,
							i2c_list->i2c_settings
...
// xiaomi add begin
static int32_t cam_ois_i2c_modes_util(struct camera_io_master *io_master_info,
				      struct i2c_settings_list *i2c_list)
{
	int32_t rc = 0;
	uint32_t i, size;
	...
	if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
		...
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] write addr: %x  data: %x delay %d",
		...
		if (rc < 0) {
			CAM_ERR(MI_PARKLENS,
				"[OISParklensLog] Failed in Applying i2c wrt settings");
			return rc;
		}
	} else if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_SEQ) {
		...
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] write continuous addr: %x  data: %x delay %d",
			i2c_list->i2c_settings.reg_setting->reg_addr,
			i2c_list->i2c_settings.reg_setting->reg_data,
			i2c_list->i2c_settings.delay);
		...
		if (rc < 0) {
			CAM_ERR(MI_PARKLENS,
				"[OISParklensLog] Failed to seq write I2C settings: %d",
				rc);
			return rc;
		}
	} else if (i2c_list->op_code == CAM_SENSOR_I2C_POLL) {
		...
		for (i = 0; i < size; i++) {
			...
			CAM_DBG(MI_PARKLENS,
				"[OISParklensLog] poll  addr: %x  data: %x delay %d",
				i2c_list->i2c_settings.reg_setting[i].reg_addr,
				i2c_list->i2c_settings.reg_setting[i].reg_data,
				i2c_list->i2c_settings.reg_setting[i].delay);
			...
			if (rc < 0) {
				CAM_ERR(MI_PARKLENS,
					"[OISParklensLog] i2c poll apply setting Fail");
				return rc;
			}
		}
	}
...
static int32_t ois_parklens_thread_func(void *arg)
{
	...
	if (!arg) {
		...
		CAM_ERR(MI_PARKLENS,
			"[OISParklensLog] ois_parklens_thread_func arg is NULL");
	} else {
		o_ctrl = (struct cam_ois_ctrl_t *)arg;
		parklens_ctrl = &(o_ctrl->parklens_ctrl);
		soc_private = (struct cam_ois_soc_private *)
				      o_ctrl->soc_info.soc_private;
		i2c_set = (struct i2c_settings_array *)(&(
			o_ctrl->i2c_parklens_data));
		power_info = &soc_private->power_info;
	}
	...
	while (ois_parklens_time_with_ms > 0) {
		...
		if (parklens_opcode >= EXIT_PARKLENS_WITH_POWERDOWN) {
			if (parklens_opcode ==
				...
				CAM_DBG(MI_PARKLENS,
					"[OISParklensLog] exit parklens thread no need power off");
				parklens_atomic_set(
					&(parklens_ctrl->exit_result),
					PARKLENS_EXIT_WITHOUT_POWERDOWN);
				goto exit_without_powerdown;
			} else if (parklens_opcode ==
				...
				CAM_WARN(
					MI_PARKLENS,
					"[OISParklensLog] Invalid opcode for parklens %d",
					parklens_opcode);
				continue;
			}
		}
		...
		if (i2c_set == NULL) {
			CAM_ERR(MI_PARKLENS,
				"[OISParklensLog] parklens Invalid parklens settings");
			parklens_atomic_set(&(parklens_ctrl->exit_result),
					    PARKLENS_EXIT_WITHOUT_PARKLENS);
			goto exit_with_powerdown;
		} else if (i2c_set->is_settings_valid != 1) {
			CAM_ERR(MI_PARKLENS,
				"[OISParklensLog] parklens settings is not valid");
			parklens_atomic_set(&(parklens_ctrl->exit_result),
					    PARKLENS_EXIT_WITHOUT_PARKLENS);
			goto exit_with_powerdown;
		} else {
			if (!delaytimes) {
				...
				if ((i2c_list_last != NULL) &&
				    list_entry_is_head(i2c_list,
						       ...
						       list)) {
					CAM_DBG(MI_PARKLENS,
						"[OISParklensLog] parklens settings execute done");
					parklens_atomic_set(
						&(parklens_ctrl->exit_result),
						PARKLENS_EXIT_WITH_POWERDOWN);
					goto exit_with_powerdown;
					...
					for (i = 0;
					     i < (i2c_list->i2c_settings.size);
					     i++) {
						CAM_DBG(MI_PARKLENS,
							"[OISParklensLog] Address 0x%x data 0x%x delay time %d ms",
							i2c_list->i2c_settings
								.reg_setting[i]
								.reg_addr,
							i2c_list->i2c_settings
					...
					if (rc < 0) {
						CAM_ERR(MI_PARKLENS,
							"[OISParklensLog] parklens step failed, %d",
							rc);
						parklens_atomic_set(
							&(parklens_ctrl
								  ->exit_result),
						...
						CAM_DBG(MI_PARKLENS,
							"[OISParklensLog] ois_parklens_thread_func arg "
							"time(ms)/state %d/%d",
							delaytimes,
							parklens_ctrl
								->parklens_state);
				...
				CAM_DBG(MI_PARKLENS,
					"[OISParklensLog] delay time %d ms",
					delaytimes);
			}
		}
		...
		if (0 == ois_parklens_time_with_ms) {
			CAM_INFO(
				MI_PARKLENS,
				"[OISParklensLog] ois parklens time exceed! power down");
			parklens_atomic_set(&(parklens_ctrl->exit_result),
					    PARKLENS_EXIT_WITH_POWERDOWN);
		}
	}
...
exit_with_powerdown:
	CAM_DBG(MI_PARKLENS,
		"[OISParklensLog] parklens thread exit with power down");
	CAM_DBG(MI_PARKLENS,
		"[OISParklensLog] parklens thread remaining time(ms)/result %d/%d",
		ois_parklens_time_with_ms,
		parklens_atomic_read(&(parklens_ctrl->exit_result)));
	lock_power_sync_mutex(o_ctrl->io_master_info.cci_client,
			      o_ctrl->cci_i2c_master);
	rc = cam_ois_power_down(o_ctrl);
	unlock_power_sync_mutex(o_ctrl->io_master_info.cci_client,
				o_ctrl->cci_i2c_master);
	if (rc < 0) {
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens power down failed rc: %d",
			rc);
		parklens_atomic_set(&(parklens_ctrl->exit_result),
				    PARKLENS_EXIT_WITH_POWERDOWN_FAILED);
	} else
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens thread  power down success");
...
exit_without_powerdown:
	if (parklens_atomic_read(&(parklens_ctrl->exit_result)) <=
	    PARKLENS_EXIT_WITHOUT_POWERDOWN) {
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens thread exit without power down");
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens thread exit time(ms)/result %d/%d",
			ois_parklens_time_with_ms,
			parklens_atomic_read(&(parklens_ctrl->exit_result)));
	}
	...
	if (rc < 0)
		CAM_ERR(MI_PARKLENS,
			"[OISParklensLog] delete parklens request failed rc: %d",
			rc);
					...
					uint32_t *cmd_buf, size_t len)
{
	...
	if (o_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		o_ctrl->io_master_info.cci_client->sid = ois_info->slave_addr >>
							 1;
	...
	if ((true == o_ctrl->opcode.fw_mem_store) && (!fw_info)) {
		fw_info = kzalloc(sizeof(struct fw_ctl) * MAX_CAM_COUNT,
				  GFP_KERNEL);
		if (!fw_info)
			return -ENOMEM;
		...
		for (j = 0; j < MAX_CAM_COUNT; j++)
			for (i = 0; i < MAX_FW_COUNT; i++) {
				fw_info[j].fw_name[i] =
					kzalloc(sizeof(char) * OIS_NAME_LEN,
						GFP_KERNEL);
...
static const struct firmware *cam_fw_mem_find(struct cam_ois_ctrl_t *o_ctrl,
					      const char *fw_name)
{
	int32_t rc = 0, i;
	uint32_t index;
	...
	for (i = 0; i < MAX_CAM_COUNT; ++i) {
		if (fw_info[i].fw_count) {
			...
			rc = cam_common_util_get_string_index(
				string, fw_info[i].fw_count, fw_name, &index);

			if ((rc == 0) && (index < MAX_FW_COUNT)) {
				CAM_DBG(CAM_OIS, "find fw %s %p", fw_name,
					fw_info[i].fw[index]);
...
static int cam_fw_mem_store(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	const char fw_name_exten[MAX_FW_COUNT][OIS_NAME_LEN] = {
		{ "%s.coeff" }, { "%s.prog" }, { "%s.mem" }, { "%s.pher" }
	};
	...
	for (i = 0, j = 0; i < MAX_CAM_COUNT; ++i) {
		if (!fw_info[i].fw_count) {
			CAM_INFO(
				CAM_OIS,
				"begin store fw_index,i=%d rc=%d, %d , so store it",
				i, rc, MAX_FW_COUNT);

			for (fw_num = 0; fw_num < MAX_FW_COUNT; fw_num++, j++) {
				snprintf(fw_info[i].fw_name[j], OIS_NAME_LEN,
					 fw_name_exten[fw_num],
					 o_ctrl->ois_name);
				fw_name = fw_info[i].fw_name[j];
				/* Load FW */
				rc = request_firmware(
					(const struct firmware **)&fw_info[i]
						.fw[j],
					fw_name, dev);
				if (rc) {
					CAM_INFO(CAM_OIS, "Failed to locate %s",
						 fw_name_exten[fw_num]);
					--j;
				}
			}
	...
	while (byte_cnt < size) {
		...
		case CAMERA_SENSOR_CMD_TYPE_I2C_RNDM_WR: {
			...
			rc = cam_sensor_handle_random_write(
				cam_cmd_i2c_random_wr, reg_settings,
				&cmd_length_in_bytes, &j, &list);
			if (rc < 0) {
				CAM_ERR(CAM_OIS, "Failed in random write %d",
					rc);
				goto end;
			}
		...
		case CAMERA_SENSOR_CMD_TYPE_I2C_CONT_WR: {
			...
			rc = cam_sensor_handle_continuous_write(
				cam_cmd_i2c_continuous_wr, reg_settings,
				&cmd_length_in_bytes, &j, &list);
			if (rc < 0) {
				CAM_ERR(CAM_OIS,
		...
		case CAMERA_SENSOR_CMD_TYPE_WAIT: {
			if (op_code == CAMERA_SENSOR_WAIT_OP_HW_UCND ||
				...
				rc = cam_sensor_handle_delay(
					...
					reg_settings, j, &byte_cnt, list);
				if (rc < 0) {
					CAM_ERR(CAM_OIS, "delay hdl failed: %d",
						rc);
					goto end;
				}
				...
				if (rc < 0) {
					CAM_ERR(CAM_OIS,
						"parsing POLL fail: %d", rc);
					goto end;
				}
			} else {
				CAM_ERR(CAM_OIS, "Wrong Wait Command: %d",
					op_code);
				rc = -EINVAL;
				goto end;
			}
		...
		case CAMERA_SENSOR_CMD_TYPE_I2C_RNDM_RD: {
			...
			rc = cam_sensor_handle_random_read(i2c_random_rd,
							   reg_settings,
							   &cmd_length_in_bytes,
							   &j, &list, NULL);
			if (rc < 0) {
				CAM_ERR(CAM_OIS, "Failed in random read %d",
					rc);
				goto end;
			}
		...
		case CAMERA_SENSOR_CMD_TYPE_I2C_CONT_RD: {
			uint16_t cmd_length_in_bytes = 0;
			struct cam_cmd_i2c_continuous_rd *i2c_continuous_rd =
			...
			rc = cam_sensor_handle_continuous_read(
				i2c_continuous_rd, reg_settings,
				&cmd_length_in_bytes, &j, &list, NULL);
			if (rc < 0) {
				CAM_ERR(CAM_OIS, "Failed in continuous read %d",
					rc);
				goto end;
			}
				      ...
				      uint32_t *cmd_buf, size_t len)
{
	...
	CAM_DBG(CAM_OIS, "endianness %d, fw_count %d", ois_fw_info->endianness,
		ois_fw_info->fw_count);

	if (ois_fw_info->fw_count <= MAX_OIS_FW_COUNT) {
		memcpy(&o_ctrl->fw_info, ois_fw_info,
		       sizeof(struct cam_cmd_ois_fw_info));
		pSettingData =
			(uint8_t *)cmd_buf + sizeof(struct cam_cmd_ois_fw_info);

		if ((ois_fw_info->param_mask & CAM_OIS_FW_VERSION_CHECK_MASK) ==
		    0x1) {
		...
		if ((version_size != 0) &&
		    (o_ctrl->i2c_fw_version_data.is_settings_valid == 0)) {
			reg_settings = &o_ctrl->i2c_fw_version_data;
			reg_settings->is_settings_valid = 1;
			rc = cam_ois_parse_fw_setting(
				pSettingData, version_size, reg_settings);
			if (rc) {
				CAM_ERR(CAM_OIS,
		...
		for (count = 0; count < ois_fw_info->fw_count * 2; count++) {
			...
			if ((count & 0x1) == 0) {
				...
				size = ois_fw_info->fw_param[idx]
					       .fw_finalize_size;
				reg_settings =
					&o_ctrl->i2c_fw_finalize_data[idx];
			...
			if (size != 0) {
				reg_settings->is_settings_valid = 1;
				rc = cam_ois_parse_fw_setting(
					pSettingData, size, reg_settings);
			}
...
static int cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (true == o_ctrl->opcode.fw_mem_store) {
		...
		if (NULL == fw) {
			...
			if (rc) {
				CAM_ERR(CAM_OIS, "Failed to locate %s",
					fw_name_prog);
				return rc;
			}
			is_need_reload = true;
	...
	if (!vaddr) {
		CAM_ERR(CAM_OIS, "Failed in allocating i2c_array: fw_size: %u",
			fw_size);
		if ((false == is_need_reload) &&
		    (true == o_ctrl->opcode.fw_mem_store)) {
	...
	i2c_reg_setting.reg_setting =
		(struct cam_sensor_i2c_reg_array *)(vaddr);
	...
	for (i = 0, ptr = (uint8_t *)fw->data, j = 0; i < total_bytes;) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"OIS FW(prog) size(%d) download failed. %d",
				total_bytes, rc);
	...
	for (i = 0, ptr = (uint8_t *)fw->data, j = 0; i < total_bytes;) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"OIS FW(coeff) size(%d) download failed rc: %d",
				total_bytes, rc);
			goto release_firmware;
		}
	}
	...
	for (i = 0, ptr = (uint8_t *)fw->data, j = 0; i < total_bytes;) {
		...
		if (rc < 0)
			CAM_ERR(CAM_OIS,
				"OIS FW(mem) size(%d) download failed rc: %d",
				total_bytes, rc);
	}
			...
			uint8_t i2c_operation)
{
	...
	uint16_t len_per_write =
		fw_param->fw_len_per_write / fw_param->fw_data_type;

	vaddr = vmalloc(
		(sizeof(struct cam_sensor_i2c_reg_array) * len_per_write));
	if (!vaddr) {
		CAM_ERR(CAM_OIS, "Failed in allocating i2c_array: size: %u",
		...
		for (cnt = 0; cnt < len_per_write; cnt++, ptr += data_type) {
			...
			if (endianness == CAM_ENDIANNESS_BIG) {
				setting.reg_setting[cnt].reg_data =
					(uint32_t)swap_high_byte_and_low_byte(
						ptr, data_type);
				/* Little */
...
static int cam_ois_fw_download_v2(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->i2c_fw_version_data.is_settings_valid == 1) {
		...
		if ((rc == -EAGAIN) &&
			...
			rc = cam_ois_apply_settings(
				o_ctrl, &o_ctrl->i2c_fw_version_data);
		}
		...
		if (rc == I2C_COMPARE_MATCH) {
			...
			CAM_WARN(CAM_OIS, "OIS FW version check failed,rc=%d",
				 rc);
		}
	}
	...
	for (count = 0; count < o_ctrl->fw_info.fw_count; count++) {
		...
		if (0 == rc && NULL != fw &&
			...
			if (o_ctrl->i2c_fw_init_data[count].is_settings_valid ==
			    1) {
				rc = cam_ois_apply_settings(
					o_ctrl,
			...
			write_ois_fw(ptr,
				     (o_ctrl->fw_info.endianness &
				      OIS_ENDIANNESS_MASK_FW),
				     fw_param, o_ctrl->io_master_info,
				     cont_wr_flag);

			/* fw finalize */
			CAM_DBG(CAM_OIS, "fw finalize");
			if (o_ctrl->i2c_fw_finalize_data[count]
				    .is_settings_valid == 1) {
				rc = cam_ois_apply_settings(
					o_ctrl,
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	// xiaomi add begin
	CAM_DBG(CAM_OIS,
		"[CRM-OIS] op_code: %d, cmd buf: %d last_flush_req %llu, request_id %llu",
		(csl_packet->header.op_code & 0xFFFFFF),
		csl_packet->num_cmd_buf, o_ctrl->last_flush_req,
	...
	case CAM_OIS_PACKET_OPCODE_INIT:
		...
		for (i = 0; i < csl_packet->num_cmd_buf; i++) {
			...
			CAM_DBG(CAM_OIS, "cmm_hdr->cmd_type: %d",
				cmm_hdr->cmd_type);
			switch (cmm_hdr->cmd_type) {
			...
			case CAMERA_SENSOR_CMD_TYPE_PWR_DOWN:
				...
				if (PARKLENS_INVALID !=
				    parklens_atomic_read(
					    &(o_ctrl->parklens_ctrl
						      .parklens_state))) {
					ois_parklens_thread_stop(
						o_ctrl,
						EXIT_PARKLENS_WITHOUT_POWERDOWN);
					parklens_power_down =
					...
					if (parklens_power_down == false) {
						CAM_INFO(
							MI_PARKLENS,
							"[OISParklensLog] May be need reset OIS");
					}
					ois_deinit_parklens_info(o_ctrl);
					CAM_DBG(MI_PARKLENS,
						"[OISParklensLog] stop parklens thread, powerdown:%d",
						parklens_power_down);
				} else {
					CAM_DBG(MI_PARKLENS,
						"[OISParklensLog] parklens thread is invalid, powerdown:%d",
						parklens_power_down);
				}

				if (parklens_power_down == true) {
					CAM_DBG(MI_PARKLENS,
						"[OISParklensLog] need power up again");
					rc = cam_sensor_update_power_settings(
						cmd_buf, total_cmd_buf_in_bytes,
						power_info, remain_len);
					CAM_DBG(MI_PARKLENS,
						"[OISParklensLog] power up again successed");
					if (rc) {
						CAM_ERR(MI_PARKLENS,
							"[OISParklensLog] Failed:parse power settings: %d",
							rc);
						goto releasemem_end;
					}
				} else {
					CAM_INFO(
						MI_PARKLENS,
						"[OISParklensLog] no need repower up again");
				}
				// xiaomi modify end
				break;
			...
			default:
				if (o_ctrl->i2c_init_data.is_settings_valid ==
				...
				} else if (o_ctrl->i2c_fwinit_data
						   .is_settings_valid == 0) {
					CAM_DBG(CAM_OIS,
					...
					if (rc < 0) {
						CAM_DBG(CAM_OIS,
							"fw init parsing failed: %d",
							rc);
					}
				} else if (o_ctrl->i2c_postinit_data
						   .is_settings_valid == 0) {
					CAM_DBG(CAM_OIS,
						"received postinit settings");
					i2c_reg_settings =
		...
		if (o_ctrl->cam_ois_state != CAM_OIS_CONFIG) {
			...
			if (parklens_power_down == true) {
				CAM_DBG(MI_PARKLENS,
					"[OISParklensLog] OIS Power up start");
				rc = cam_ois_power_up(o_ctrl);
				if (rc) {
					CAM_ERR(MI_PARKLENS,
						"[OISParklensLog] OIS Power up failed");
					goto releasepacket_end;
				}
			}
			// xiaomi modify end
		...
		if (o_ctrl->opcode.customized_ois_flag) {
			for (j = 0;
			     j < sizeof(pflash_ois) /
					 sizeof(struct flash_ois_function);
			     j++) {
				if (ps[j].flag ==
				    o_ctrl->opcode.customized_ois_flag) {
					config_flag++;
					...
					if (rc) {
						CAM_ERR(CAM_OIS,
							"Failed OIS Customer Pkt Download");
						goto pwr_dwn;
					}
					o_ctrl->cam_ois_state = CAM_OIS_CONFIG;
					CAM_INFO(CAM_OIS,
						 "%s use flash_ois_function %d",
						 o_ctrl->ois_name, j);
					break;
				}
			}
			if (1 != config_flag) {
				CAM_ERR(CAM_OIS,
					"ERROR! need  pkt function, flag %d",
					config_flag);
			}
			/* xiaomi add end*/
		} else {
			if (o_ctrl->ois_fw_flag) {
				CAM_DBG(CAM_OIS, "fw_count: %d",
					o_ctrl->fw_info.fw_count);
				if (o_ctrl->fw_info.fw_count != 0) {
					/* xiaomi add begin */
					CAM_GET_TIMESTAMP(ts1);
					CAM_DBG(MI_PERF,
						"%s start firmware download",
						o_ctrl->ois_name);
					/* xiaomi add end */
					...
					/* xiaomi add begin */
					CAM_GET_TIMESTAMP(ts2);
					CAM_GET_TIMESTAMP_DIFF_IN_MICRO(
						ts1, ts2, microsec);
					CAM_DBG(MI_PERF,
						"%s end firmware download, occupy time is: %ld ms",
						o_ctrl->ois_name,
						microsec / 1000);
					/* xiaomi add end */
				} else {
					if (o_ctrl->i2c_fwinit_data
						    .is_settings_valid == 1) {
						rc = cam_ois_apply_settings(
							o_ctrl,
							&o_ctrl->i2c_fwinit_data);
			...
			for (i = 0; i < MAX_OIS_FW_COUNT; i++) {
				if (o_ctrl->i2c_fw_init_data[i]
					    .is_settings_valid == 1) {
					rc = delete_request(
						&o_ctrl->i2c_fw_init_data[i]);
					if (rc < 0) {
						CAM_WARN(
							CAM_OIS,
				...
				if (o_ctrl->i2c_fw_finalize_data[i]
					    .is_settings_valid == 1) {
					rc = delete_request(
						&o_ctrl->i2c_fw_finalize_data[i]);
					if (rc < 0) {
						CAM_WARN(
							CAM_OIS,
			...
			if (o_ctrl->i2c_postinit_data.is_settings_valid == 1) {
				rc = cam_ois_apply_settings(
					o_ctrl, &o_ctrl->i2c_postinit_data);
				if ((rc == -EAGAIN) &&
				    (o_ctrl->io_master_info.master_type ==
				     CCI_MASTER)) {
					CAM_WARN(
			...
			if (rc < 0) {
				CAM_WARN(CAM_OIS,
					 "Fail deleting postinit data: rc: %d",
					 rc);
				rc = 0;
			}
			// xiaomi add end
		...
		if (rc < 0) {
			CAM_WARN(CAM_OIS, "Fail deleting fwinit data: rc: %d",
				 rc);
			rc = 0;
		}
	...
	case CAM_OIS_PACKET_OPCODE_READ: {
		...
		if (o_ctrl->cam_ois_state < CAM_OIS_CONFIG) {
			rc = -EINVAL;
			CAM_WARN(CAM_OIS, "Not in right state to read OIS: %d",
				 o_ctrl->cam_ois_state);
			goto releasepacket_end;
		}
		...
		rc = cam_sensor_i2c_read_write_ois_data(
			&i2c_read_settings, &o_ctrl->io_master_info);
		// xiaomi change end
		if (rc < 0) {
		...
		if (csl_packet->num_io_configs > 1) {
			rc = cam_sensor_util_write_qtimer_to_io_buffer(
				qtime_ns, &io_cfg[1]);
			if (rc < 0) {
				CAM_ERR(CAM_OIS, "write qtimer failed rc: %d",
					rc);
				delete_request(&i2c_read_settings);
				goto releasepacket_end;
			}
	...
	case CAM_OIS_PACKET_OPCODE_WRITE_TIME: {
		CAM_DBG(CAM_OIS, "CAM_OIS_PACKET_OPCODE_WRITE_TIME");
		if (o_ctrl->cam_ois_state < CAM_OIS_CONFIG) {
			rc = -EINVAL;
			CAM_ERR(CAM_OIS,
		...
		if (o_ctrl->fw_info.fw_count > 0) {
			uint8_t ois_endianness =
				(o_ctrl->fw_info.endianness &
				 OIS_ENDIANNESS_MASK_INPUTPARAM) >>
				4;
	...
	case CAM_OIS_PACKET_OPCODE_OIS_PARKLENS: {
		int32_t exit_result = parklens_atomic_read(
			&(o_ctrl->parklens_ctrl.exit_result));

		parklens_state = parklens_atomic_read(
			&(o_ctrl->parklens_ctrl.parklens_state));
		CAM_INFO(
			MI_PARKLENS,
			"[OISParklensLog] Received parklens buffer parklens_state %d",
			parklens_state);
		if ((parklens_state != PARKLENS_INVALID) ||
			...
			CAM_WARN(
				MI_PARKLENS,
				"[OISParklensLog] Not in right state to do parklens: %d/%d, "
				"exit result %d release power control right",
		...
		if (rc < 0) {
			CAM_ERR(MI_PARKLENS,
				"[OISParklensLog] Failed:parse parklens settings: %d",
				rc);
	...
	case CAM_OIS_PACKET_OPCODE_OIS_CHANGE_PWM:
		...
		CAM_DBG(CAM_OIS,
			"[CRM-OIS] request_id %llu, i2c_data %p &i2c_data->per_frame %p",
			csl_packet->header.request_id, &(o_ctrl->i2c_data),
			&i2c_data->per_frame);
		...
		rc = cam_sensor_i2c_command_parser(&o_ctrl->io_master_info,
						   ...
						   1, NULL);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "[CRM-OIS] CRM OIS parsing failed: %d",
				rc);
			goto releasepacket_end;
		}
...
void cam_ois_shutdown(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->cam_ois_state >= CAM_OIS_CONFIG) {
		if (false == ois_parklens_power_down(o_ctrl)) {
			...
			CAM_INFO(MI_PARKLENS,
				 "[OISParklensLog] parklens isn't powerdown");
		} else {
			CAM_INFO(
				MI_PARKLENS,
				"[OISParklensLog] parklens has been powerdown");
		}
	} else {
		CAM_INFO(
			MI_PARKLENS,
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_RELEASE_DEV: {
		...
		CAM_INFO(
			MI_PARKLENS,
			"[OISParklensLog] CAM_RELEASE_DEV is_parklens_power_down status %d/%d",
			parklens_power_down, o_ctrl->cam_ois_state);
		if (o_ctrl->cam_ois_state == CAM_OIS_CONFIG) {
			if (parklens_power_down == false) {
		...
		for (i = 0; i < MAX_OIS_FW_COUNT; i++) {
			if (o_ctrl->i2c_fw_init_data[i].is_settings_valid ==
			    1) {
				rc = delete_request(
					&o_ctrl->i2c_fw_init_data[i]);
				if (rc < 0) {
			...
			if (o_ctrl->i2c_fw_finalize_data[i].is_settings_valid ==
			    1) {
				rc = delete_request(
					&o_ctrl->i2c_fw_finalize_data[i]);
				if (rc < 0) {
...
int32_t init_ois_parklens_info(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (!o_ctrl) {
		CAM_ERR(MI_PARKLENS, "[OISParklensLog] failed: o_ctrl %pK",
			o_ctrl);
		return -EINVAL;
	}
	...
	CAM_DBG(MI_PARKLENS, "[OISParklensLog] init parklens: %s %s",
		o_ctrl->device_name, o_ctrl->ois_name);

	parklens_atomic_set(&(parklens_ctrl->parklens_opcode),
			    ENTER_PARKLENS_WITH_POWERDOWN);
	parklens_atomic_set(&(parklens_ctrl->exit_result), PARKLENS_ENTER);
	parklens_atomic_set(&(parklens_ctrl->parklens_state), PARKLENS_INVALID);
...
int32_t ois_deinit_parklens_info(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_DBG(MI_PARKLENS, "[OISParklensLog] deinit parklens: %s",
		o_ctrl->device_name);
	...
	if (PARKLENS_STOP !=
		...
		CAM_ERR(MI_PARKLENS,
			"[OISParklensLog] deinit parklens in wrong state");
		return -EINVAL;
	}
...
bool ois_parklens_power_down(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	case PARKLENS_RUNNING: {
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens is running, no need powerdown");
		is_power_down = true;
	} break;
	...
	case PARKLENS_STOP: {
		if (parklens_atomic_read(&(parklens_ctrl->exit_result)) <=
		    PARKLENS_EXIT_WITHOUT_POWERDOWN) {
			CAM_DBG(MI_PARKLENS,
				"[OISParklensLog] parklens stop without power down");
			is_power_down = false;
		} else {
			is_power_down = true;
		}
	...
	case PARKLENS_INVALID: {
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens is not created, power down");
		is_power_down = false;
	} break;

	default:
		CAM_ERR(MI_PARKLENS,
			"[OISParklensLog] Invalid parklens_state %d",
			parklens_state);
	}
...
int32_t ois_parklens_thread_trigger(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (rc < 0) {
		CAM_ERR(MI_PARKLENS,
			"[OISParklensLog] failed to init parklens info %d", rc);
		return rc;
	}
	parklens_ctrl = &(o_ctrl->parklens_ctrl);
	...
	parklens_ctrl->parklens_thread = parklens_thread_run(
		ois_parklens_thread_func, o_ctrl, "parklens-thread");

	if (!(parklens_ctrl->parklens_thread)) {
		CAM_ERR(MI_PARKLENS,
			"[OISParklensLog] parklens_thread create failed");
		rc = -ENOMEM;
		goto deinit_parklens;
	} else {
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] %s trigger parklens thread",
			o_ctrl->ois_name);
		...
		if (rc > PARKLENS_ENTER) {
			CAM_DBG(MI_PARKLENS,
				"[OISParklensLog] parklens thread execute failed %d",
				rc);
			goto clear_thread;
		} else
...
int32_t ois_parklens_thread_stop(struct cam_ois_ctrl_t *o_ctrl,
				 enum parklens_opcodes opcode)
{
	...
	if (parklens_state != PARKLENS_RUNNING) {
		CAM_DBG(MI_PARKLENS,
			"[OISParklensLog] parklens thread is in other state: %d",
			parklens_state);
		return -EINVAL;
	} else {
		if (exit_result != PARKLENS_ENTER) {
			CAM_DBG(MI_PARKLENS,
				"[OISParklensLog] parklens thread is already end: %d",
				exit_result);
		} else {
			if ((opcode == EXIT_PARKLENS_WITH_POWERDOWN) ||
			    (opcode == EXIT_PARKLENS_WITHOUT_POWERDOWN)) {
				parklens_atomic_set(
					&(parklens_ctrl->parklens_opcode),
					opcode);
			} else {
				CAM_DBG(MI_PARKLENS,
					"[OISParklensLog] Invalid stop opcode, stop "
					"parklens with power down");
				parklens_atomic_set(
					&(parklens_ctrl->parklens_opcode),
					EXIT_PARKLENS_WITH_POWERDOWN);
			}
			CAM_DBG(MI_PARKLENS,
				"[OISParklensLog] wake up parklens thread to stop it");
		}
		...
		if (exit_result) {
			CAM_INFO(
				MI_PARKLENS,
				"[OISParklensLog] parklens thread exit status: %d",
				exit_result);
		} else
			CAM_DBG(MI_PARKLENS,
				"[OISParklensLog] parklens thread exit failed ! ! !");
	}
...
void ois_fw_version_set(uint64_t fw_version, char *name)
{
	...
	CAM_INFO(MI_DEBUG, "index:%d cach version:%s, name:%s version:0x%x", i,
		 (rc == true) ? "Y" : "N", name, fw_version);
}

static int cam_ois_fw_set(const char *kmessage, const struct kernel_param *kp)
{
	return 0;
}

static int cam_ois_fw_get(char *buffer, const struct kernel_param *kp)
{
	uint16_t buff_max_size = CAM_OIS_FW_VERSION_MAX_LENGTH;
	uint16_t ret = 0, i;

	for (i = 0; (ois_firmware[i].is_valid == true && i < MAX_OIS_DEV);
	     i++) {
...
static const struct kernel_param_ops cam_ois_fw_ops = { .set = cam_ois_fw_set,
							.get = cam_ois_fw_get };
 ...
 * Returns success or failure
 */
int cam_ois_update_req_mgr(struct cam_ois_ctrl_t *o_ctrl,
			   struct cam_packet *csl_packet)
{
	...
	CAM_DBG(CAM_OIS,
		"[CRM-OIS] Adding request: %llu link_hdl %d, dev_hdl %d",
		csl_packet->header.request_id, add_req.link_hdl,
		add_req.dev_hdl);
	if (o_ctrl->bridge_intf.crm_cb && o_ctrl->bridge_intf.crm_cb->add_req) {
		rc = o_ctrl->bridge_intf.crm_cb->add_req(&add_req);
		if (rc) {
			if (rc == -EBADR)
				CAM_INFO(
					CAM_OIS,
					"[CRM-OIS] Adding request: %llu failed: rc: %d, it has been "
					"flushed",
...
int32_t cam_ois_flush_request(struct cam_req_mgr_flush_request *flush_req)
{
	...
	o_ctrl = (struct cam_ois_ctrl_t *)cam_get_device_priv(
		flush_req->dev_hdl);
	if (!o_ctrl) {
		CAM_ERR(CAM_OIS, "[CRM-OIS] Device data is NULL");
		return -EINVAL;
...
int32_t cam_ois_apply_request(struct cam_req_mgr_apply_request *apply)
{
	...
	o_ctrl = (struct cam_ois_ctrl_t *)cam_get_device_priv(apply->dev_hdl);
	...
	trace_cam_apply_req("OIS", o_ctrl->soc_info.index, apply->request_id,
			    apply->link_hdl);
	...
	if ((apply->request_id ==
		...
		rc = cam_ois_apply_settings(
			o_ctrl, &o_ctrl->i2c_data.per_frame[request_id]);
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
