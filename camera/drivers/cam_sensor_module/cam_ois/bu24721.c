...
int bu24721_ois_pkt_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	struct bu24721_ois_i2c_info_t i2c_info = { FLASH_I2C_ADDR,
						   I2C_FAST_PLUS_MODE };
	...
	cam_cci_i2c_poll(ois_cci_client, F024_SETTING.reg_addr,
			 F024_SETTING.reg_data, F024_SETTING.data_mask,
	...
	CAM_INFO(CAM_OIS,
		 "[BU24721]  read addr 0x%04x  version 0x%04x, flag %d",
		 F01C_SETTING.reg_data, reg_data,
	...
	if (bu24721_ois_enable_read_eeprom == ENABLE_READ_EEPROM) {
		...
		if (rc) {
			CAM_ERR(CAM_OIS,
				"[BU24721] failed: to update i2c info rc %d",
				rc);
		}
		cam_camera_cci_i2c_read_seq(ois_cci_client, CALI_ADDR, data_tmp,
					    CAMERA_SENSOR_I2C_TYPE_WORD,
					    CAMERA_SENSOR_I2C_TYPE_BYTE,
					    CALI_SIZE_BYTE);
		...
		for (i = 0; i < CALI_SIZE_BYTE;) {
			CAM_DBG(CAM_OIS,
				"[BU24721]addr:0x%04x data: 0x%02x 0x%02x 0x%02x 0x%02x",
				CALI_ADDR + i, data_tmp[i], data_tmp[i + 1],
				data_tmp[i + 2], data_tmp[i + 3]);
			i += 4;
		}
	...
	if (F01C_SETTING.reg_data != reg_data) {
		...
		if (o_ctrl->i2c_init_data.is_settings_valid == 1) {
			...
			if (rc == -EAGAIN) {
				...
				rc = bu24721_ois_apply_settings(
					o_ctrl, &o_ctrl->i2c_init_data);
			}
			if (rc) {
				CAM_ERR(CAM_OIS,
					"[bu24721] Cannot apply i2c_init_data data %d flag %d",
					rc, o_ctrl->opcode.customized_ois_flag);
				//	goto pwr_dwn;
			} else {
				CAM_DBG(CAM_OIS,
					"[bu24721] OIS program Flash settings success");
			}
		}
		...
		if (rc) {
			CAM_ERR(CAM_OIS, "[bu24721] Failed to locate %s",
				fw_name_prog);
			return rc;
		}
		...
		if (!vaddr) {
			CAM_ERR(CAM_OIS,
				"[bu24721] Failed in allocating i2c_array: fw_size: %u",
				fw_size);
			release_firmware(fw);
			return -ENOMEM;
		}
		CAM_DBG(CAM_OIS, "[bu24721] %s: fw_size: %u", fw_name_prog,
			fw_size);
		i2c_reg_setting.reg_setting =
			(struct cam_sensor_i2c_reg_array *)(vaddr);

		for (cnt = 0, i = 0, ptr = (uint8_t *)fw->data;
		     4 * cnt < total_bytes && 4 * i < total_bytes;
		     cnt++, ptr += 4, i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"[bu24721] OIS FW(prog) size(%d) download failed. %d",
				total_bytes, rc);
		...
		CAM_DBG(CAM_OIS, "[bu24721] %s: fw_size: %u", fw_name_coeff,
			fw_size);
		i2c_reg_setting.reg_setting =
			(struct cam_sensor_i2c_reg_array *)(vaddr);
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"[BU24721] OIS FW(coeff) size(%d) download failed rc: %d",
				total_bytes, rc);
			goto release_firmware;
		}
		vfree(vaddr);
		...
		CAM_DBG(CAM_OIS, "[bu24721] %s: fw_size: %u", fw_name_mem,
			fw_size);
		i2c_reg_setting.reg_setting =
			(struct cam_sensor_i2c_reg_array *)(vaddr);
		...
		if (rc < 0)
			CAM_ERR(CAM_OIS,
				"[BU24721] OIS FW(mem) size(%d) download failed rc: %d",
				total_bytes, rc);
...
release_firmware:
		...
		cam_cci_i2c_poll(ois_cci_client, F024_SETTING.reg_addr,
				 ...
				 F024_SETTING.delay);
		// read version
		cam_cci_i2c_read(ois_cci_client, F01C_SETTING.reg_addr,
				 &reg_data, CAMERA_SENSOR_I2C_TYPE_WORD,
				 CAMERA_SENSOR_I2C_TYPE_DWORD, TRUE);
		cam_cci_i2c_poll(ois_cci_client, F024_SETTING.reg_addr,
				 F024_SETTING.reg_data, F024_SETTING.data_mask,
				 CAMERA_SENSOR_I2C_TYPE_BYTE,
		...
		CAM_INFO(
			CAM_OIS,
			"[BU24721] update end, read addr 0x%04x  version 0x%04x, flag %d",
			F01C_SETTING.reg_data, reg_data,
			o_ctrl->opcode.customized_ois_flag);
	}
	...
	if (o_ctrl->is_ois_calib) {
		...
		if ((rc == -EAGAIN) &&
			...
			CAM_WARN(
				CAM_OIS,
				"[bu24721] CCI HW is restting: Reapplying calib settings");
			usleep_range(1000, 1010);
			rc = bu24721_ois_apply_calib_settings(
				o_ctrl, &o_ctrl->i2c_calib_data);
		}
		if (rc) {
			CAM_ERR(CAM_OIS,
				"[bu24721] Cannot apply calib data %d flag %d",
				rc, o_ctrl->opcode.customized_ois_flag);
			//	goto pwr_dwn;
		} else {
			CAM_DBG(CAM_OIS,
				"[bu24721] apply calib data settings success");
		}
	}
	...
	CAM_DBG(CAM_OIS, "[BU24721] default gyrogain x=0x%x y=0x%x",
		gyro_Gain_H, gyro_Gain_L);
	...
	list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
		...
		case CAM_SENSOR_I2C_WRITE_SEQ: {
			for (j = 0; j < i2c_list->i2c_settings.size; j++) {
				CAM_DBG(CAM_OIS,
					"[BU24721] ois name %s, request id %d, j=[%d], reg_addr 0x%x, 0x%x",
		...
		case CAM_SENSOR_I2C_READ_SEQ: {
			for (j = 0; j < i2c_list->i2c_settings.size; j++) {
				CAM_DBG(CAM_OIS,
					"[BU24721] ois name  %s, request id %d, j=[%d], reg_addr 0x%x, "
					"0x%x",
					o_ctrl->ois_name, i2c_set->request_id,
					j,
					i2c_list->i2c_settings.reg_setting[j]
				    ...
				    struct bu24721_ois_i2c_info_t *i2c_info)
{
	...
	if (o_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		CAM_DBG(CAM_OIS, "[BU24721] Slave addr: 0x%x",
			i2c_info->slave_addr);
				     ...
				     struct i2c_settings_array *i2c_set)
{
	...
	CAM_DBG(CAM_OIS, "[BU24721] reg_data x=0x%x y=0x%x",
		F09D_SETTING[0].reg_data = buff[0],
		F09D_SETTING[0].reg_data = buff[1]);
