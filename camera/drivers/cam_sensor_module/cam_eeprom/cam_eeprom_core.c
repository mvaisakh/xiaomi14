			       ...
			       struct cam_sensor_power_ctrl_t *power_info)
{
	...
	if (e_ctrl->io_master_info.master_type == I3C_MASTER)
		i3c_probe_completion =
			cam_eeprom_get_i3c_completion(e_ctrl->soc_info.index);
	...
	if (rc) {
		...
		// add by xiaomi
		cam_hw_notify_v4l2_error_event(CAM_EEPROM_NAME, (void *)e_ctrl,
					       V4L_EVENT_HW_ISSUE_POWER_ERROR,
					       HW_ISSUE_ERROR_TYPE_EEPROM_POWER,
...
static int32_t cam_eeprom_pkt_parse(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	...
	case CAM_EEPROM_PACKET_OPCODE_INIT:
		...
		if (rc)
			CAM_WARN(CAM_EEPROM,
				 "failed to get calibration data rc %d", rc);
