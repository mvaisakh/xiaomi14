...
struct cam_smmu_nested_region_info {
	...
	struct cam_smmu_subregion_info subregions[CAM_SMMU_SUBREGION_MAX];
...
struct cam_smmu_multi_region_info {
	int32_t num_regions;
	struct cam_smmu_nested_region_info
		nested_regions[CAM_SMMU_MULTI_REGION_MAX];
};
...
cam_smmu_find_mapping_by_ion_index(int idx, int ion_fd,
...
static int cam_smmu_map_buffer_and_add_to_list(
	int idx, int ion_fd, bool dis_delayed_unmap,
	enum dma_data_direction dma_dir, dma_addr_t *paddr_ptr, size_t *len_ptr,
	enum cam_smmu_region_id region_id, bool is_internal,
	struct dma_buf *dmabuf, struct kref **ref_count);
...
static void
cam_smmu_update_monitor_array(struct cam_context_bank_info *cb_info,
			      bool is_map,
...
static void cam_smmu_dump_monitor_array(struct cam_context_bank_info *cb_info)
{
	int i = 0;
	...
	if (state_head == -1) {
		...
		div_u64_rem(state_head + 1, CAM_SMMU_MONITOR_MAX_ENTRIES,
			    &oldest_entry);
	}
	...
	for (i = 0; i < num_entries; i++) {
		CAM_CONVERT_TIMESTAMP_FORMAT(
			cb_info->monitor_entries[index].timestamp, hrs, min,
			sec, ms);

		CAM_INFO(
			CAM_SMMU,
			"**** %llu:%llu:%llu.%llu : Index[%d] [%s] : ion_fd=%d i_ino=%lu "
			"start=0x%llx end=0x%llx len=%zu region=%d",
			hrs, min, sec, ms, index,
			cb_info->monitor_entries[index].is_map ? "MAP" :
								 "UNMAP",
...
static struct cam_smmu_subregion_info *
cam_smmu_find_subregion(enum cam_smmu_subregion_id subregion_id,
...
static int
cam_smmu_validate_nested_region_idx(struct cam_context_bank_info *cb_info,
				    int32_t *nested_reg_idx, int32_t region_id)
{
	...
	case CAM_SMMU_REGION_FWUNCACHED:
		if ((*nested_reg_idx) >
		    (cb_info->fwuncached_region.num_regions - 1))
			goto err;
	...
	case CAM_SMMU_REGION_DEVICE:
		if ((*nested_reg_idx) >
		    (cb_info->device_region.num_regions - 1))
			*nested_reg_idx = 0;
...
err:
	CAM_ERR(CAM_SMMU,
		...
		nested_reg_idx, cb_info->shared_info.num_regions, region_id,
		cb_info->name[0]);
	return -EINVAL;
}

static inline int
cam_smmu_get_multiregion_client_dev_idx(struct cam_context_bank_info *cb_info,
					int32_t multi_client_device_idx,
					int32_t region_id, int32_t *region_idx)
{
	if (cb_info->num_multi_regions && multi_client_device_idx) {
		*region_idx = multi_client_device_idx;
		return cam_smmu_validate_nested_region_idx(cb_info, region_idx,
							   region_id);
	}
...
static void cam_smmu_page_fault_work(struct work_struct *work)
{
	...
	if (payload->iova)
		buf_info = cam_smmu_find_closest_mapping(
			idx, (void *)payload->iova, &in_map);
...
static void cam_smmu_dump_cb_info(int idx)
{
	...
	if (cb_info->shared_support) {
		for (j = 0; j < cb_info->shared_info.num_regions; j++) {
			nested_reg_info =
				&cb_info->shared_info.nested_regions[j];
	...
	if (iommu_cb_set.debug_cfg.cb_dump_enable) {
		list_for_each_entry_safe(
			...
			i++;
			CAM_CONVERT_TIMESTAMP_FORMAT(mapping->ts, hrs, min, sec,
						     ms);
			CAM_ERR(CAM_SMMU,
				"%llu:%llu:%llu:%llu: %u ion_fd=%d i_ino=%lu start=0x%lx "
				"end=0x%lx len=%lu region=%d",
				hrs, min, sec, ms, i, mapping->ion_fd,
				mapping->i_ino, mapping->paddr,
...
static void cam_smmu_print_user_list(int idx)
{
	...
	list_for_each_entry(mapping, &iommu_cb_set.cb_info[idx].smmu_buf_list,
		...
		CAM_ERR(CAM_SMMU,
			...
			mapping->ion_fd, mapping->i_ino, mapping->paddr,
			mapping->len, mapping->region_id);
	}
}
...
static void cam_smmu_print_table(void)
{
	...
	for (i = 0; i < iommu_cb_set.cb_num; i++) {
		for (j = 0; j < iommu_cb_set.cb_info[i].num_shared_hdl; j++) {
			CAM_ERR(CAM_SMMU,
				"i= %d, handle= %d, name_addr=%pK name %s", i,
					      ...
					      bool *in_map_region)
{
	...
	*in_map_region = false;
	...
	if (closest_mapping) {
		...
		end_addr = (unsigned long)closest_mapping->paddr +
			   closest_mapping->len;
		if (start_addr <= current_addr && current_addr < end_addr)
			*in_map_region = true;
		CAM_INFO(
			CAM_SMMU,
			"Faulting addr 0x%lx closest map fd %d i_ino %lu %llu-%llu "
			"0x%lx-0x%lx buf=%pK",
			current_addr, closest_mapping->ion_fd,
			closest_mapping->i_ino, mapping->len,
			closest_mapping->len,
...
void cam_smmu_set_client_page_fault_handler(
	int handle, void (*handler_cb)(struct cam_smmu_pf_info *pf_info),
	void *token)
{
	int idx, i = 0;
   ...
   */
	if (iommu_cb_set.cb_info[idx].non_fatal_faults_en) {
		if (iommu_cb_set.debug_cfg.fatal_pf_mask & BIT(idx))
			CAM_TRIGGER_PANIC(
				"SMMU context fault from soc: %s[cb_idx: %u]",
...
int cam_smmu_is_cb_non_fatal_fault_en(int smmu_hdl, bool *non_fatal_en)
{
	...
	if (idx < 0 || idx >= iommu_cb_set.cb_num) {
		CAM_ERR(CAM_SMMU, "Invalid handle or idx. idx: %d, hdl: 0x%x",
			idx, smmu_hdl);
		return -EINVAL;
	}
...
static inline void
cam_smmu_update_multiregion_dev_id(struct cam_context_bank_info *cb_info,
				   char *name, int *hdl)
{
	int k;
	...
	for (k = 0; k < cb_info->num_multi_regions; k++) {
		if (!strcmp(cb_info->multi_region_clients[k], name)) {
			*hdl |= ((k + 1) << MULTI_CLIENT_REGION_SHIFT);
			CAM_DBG(CAM_SMMU,
				"%s got shared multi region handle 0x%x", name,
				*hdl);
		}
	}
}
			...
			if (iommu_cb_set.cb_info[i].handle == HANDLE_INIT) {
				...
				*hdl = handle;
				...
				if (iommu_cb_set.cb_info[i].is_mul_client) {
					iommu_cb_set.cb_info[i].device_count++;
					*hdl = iommu_cb_set.cb_info[i].handle;
					if (iommu_cb_set.cb_info[i]
						    .num_multi_regions) {
						cam_smmu_update_multiregion_dev_id(
							&iommu_cb_set.cb_info[i],
							name, hdl);
					}
					mutex_unlock(
						&iommu_cb_set.cb_info[i].lock);
...
static struct cam_dma_buff_info *
cam_smmu_find_mapping_by_ion_index(int idx, int ion_fd, struct dma_buf *dmabuf)
{
	struct cam_dma_buff_info *mapping;
	unsigned long i_ino;
	...
	list_for_each_entry(mapping, &iommu_cb_set.cb_info[idx].smmu_buf_list,
		...
		if ((mapping->ion_fd == ion_fd) && (mapping->i_ino == i_ino)) {
			CAM_DBG(CAM_SMMU, "find ion_fd %d i_ino %lu", ion_fd,
				i_ino);
			return mapping;
		}
	}
	...
	CAM_ERR(CAM_SMMU,
		"Error: Cannot find entry by index %d, fd %d i_ino %lu", idx,
		ion_fd, i_ino);
	...
	list_for_each_entry_safe(mapping_info, temp,
		...
		if (ret < 0) {
			...
			CAM_ERR(CAM_SMMU,
				...
				mapping_info->paddr, mapping_info->ion_fd,
				mapping_info->i_ino);
			/*
...
static int cam_smmu_alloc_iova(size_t size, int32_t smmu_hdl,
			       unsigned long *iova)
{
	...
	vaddr = gen_pool_alloc(
		iommu_cb_set.cb_info[idx].shared_mem_pool[shared_mem_pool_idx],
		size);
...
static int cam_smmu_free_iova(unsigned long iova, size_t size, int32_t smmu_hdl)
{
			    ...
			    uintptr_t *cpuva, size_t *len)
{
   ...
   */
	rc = iommu_map(domain, firmware_start, (phys_addr_t)icp_fw.fw_hdl,
		       firmware_len, IOMMU_READ | IOMMU_WRITE | IOMMU_PRIV);
...
static int cam_smmu_retrieve_region_info(
	struct cam_context_bank_info *cb_info, uint32_t region_id,
	uint32_t subregion_id, int32_t nested_reg_idx, bool *is_supported,
	int32_t **ref_cnt, unsigned long **mapping_mask, bool **is_allocated,
	struct cam_smmu_region_info **region_info)
{
	int rc = 0;
	...
	case CAM_SMMU_REGION_QDSS: {
		...
		*is_allocated = &qdss_region->nested_regions[nested_reg_idx]
					 .is_allocated;
		*region_info =
			&qdss_region->nested_regions[nested_reg_idx].region_info;
		*ref_cnt = &qdss_region->nested_regions[nested_reg_idx]
				    .mapped_refcnt;
		*mapping_mask = &qdss_region->nested_regions[nested_reg_idx]
					 .mapped_client_mask;
	} break;
	case CAM_SMMU_REGION_DEVICE: {
		struct cam_smmu_multi_region_info *device_region;
		...
		*is_supported = cb_info->device_region_support;
		...
		if (device_region->nested_regions[nested_reg_idx]
			    .subregion_support) {
			subregion = cam_smmu_find_subregion(
				subregion_id,
				device_region->nested_regions[nested_reg_idx]
					.subregions);
	...
	case CAM_SMMU_REGION_FWUNCACHED: {
		...
		if (fw_uncached->nested_regions[nested_reg_idx]
			    .subregion_support) {
			subregion = cam_smmu_find_subregion(
				subregion_id,
				fw_uncached->nested_regions[nested_reg_idx]
					.subregions);
			if (IS_ERR_OR_NULL(subregion)) {
				CAM_ERR(CAM_SMMU,
					"Failed to find subregion: %d in region: %d cb: %s",
			...
			*region_info =
				&fw_uncached->nested_regions[nested_reg_idx]
					 .region_info;
			*ref_cnt = &fw_uncached->nested_regions[nested_reg_idx]
					    .mapped_refcnt;
			*mapping_mask =
				&fw_uncached->nested_regions[nested_reg_idx]
					 .mapped_client_mask;
...
int cam_smmu_map_phy_mem_region(int32_t smmu_hdl, uint32_t region_id,
				uint32_t subregion_id, dma_addr_t *iova,
				size_t *len)
{
	...
	if ((*is_allocated)) {
		...
		(*ref_cnt)++;
		CAM_DBG(CAM_SMMU,
			"Trying to allocate region: %u [subregion: %u] on cb: %s for "
			"different clients: 0x%x ref_cnt: %u",
			region_id, subregion_id, cb_info->name[0],
			*map_client_mask, *ref_cnt);
	...
	CAM_DBG(CAM_SMMU,
		"mapping region= %u [subregion = %u] for va = 0x%x len = %zu phy = "
		"0x%x prot=0x%x",
		region_id, subregion_id, region_start, region_len,
		region_info->phy_addr, prot);
   ...
   * being mapped to this CB must be CACHED
   */
	rc = iommu_map(domain, region_start, region_info->phy_addr, region_len,
		       prot);
	if (rc) {
		CAM_ERR(CAM_SMMU,
...
int cam_smmu_unmap_phy_mem_region(int32_t smmu_hdl, uint32_t region_id,
				  uint32_t subregion_id)
{
	...
	cb_info = &iommu_cb_set.cb_info[idx];
	rc = cam_smmu_get_multiregion_client_dev_idx(
		cb_info, multi_client_device_idx, region_id, &nested_reg_idx);
	if (rc)
		goto end;
	...
	if ((*ref_cnt) > 0) {
		CAM_DBG(CAM_SMMU,
			"Mapping for region: %u on cb: %s still in use refcnt: %d "
			"mapping_mask: 0x%x",
			region_id, cb_info->name[0], *ref_cnt,
			*map_client_mask);
		goto unlock_and_end;
	}
				...
				size_t *discard_iova_len)
{
	...
	mutex_lock(&iommu_cb_set.cb_info[idx].lock);
	io_region = &iommu_cb_set.cb_info[idx]
			     .io_info.nested_regions[nested_reg_idx];

	*iova = io_region->region_info.iova_start;
	*len = io_region->region_info.iova_len;
	*discard_iova_start = io_region->region_info.discard_iova_start;
	*discard_iova_len = io_region->region_info.discard_iova_len;
			     ...
			     struct cam_smmu_region_info *region_info)
{
	...
	case CAM_SMMU_REGION_SHARED: {
		...
		nested_reg_info =
			&cb->shared_info.nested_regions[nested_reg_idx];
	...
	case CAM_SMMU_REGION_FWUNCACHED: {
		...
		nested_reg_info =
			&cb->fwuncached_region.nested_regions[nested_reg_idx];
	...
	case CAM_SMMU_REGION_DEVICE: {
		...
		if (!cb->device_region_support) {
			CAM_WARN(CAM_SMMU,
				 "device memory region not supported");
			mutex_unlock(&iommu_cb_set.cb_info[idx].lock);
			return -ENODEV;
		}

		nested_reg_info =
			&cb->device_region.nested_regions[nested_reg_idx];
				...
				dma_addr_t *iova, size_t *request_len)
{
	...
	cb_info = &iommu_cb_set.cb_info[idx];
	rc = cam_smmu_get_multiregion_client_dev_idx(
		cb_info, multi_client_device_idx, region, &nested_reg_idx);
	if (rc)
		return rc;
	...
	if (region == CAM_SMMU_REGION_SECHEAP) {
		...
		nested_reg_info = &cb_info->fwuncached_region
					   .nested_regions[nested_reg_idx];

		if (nested_reg_info->subregion_support) {
			/* For FW uncached, subregion generic is to be used */
			subregion_info = cam_smmu_find_subregion(
	...
	CAM_DBG(CAM_SMMU, "Map region=%d iova=0x%x len=0x%x", region,
		region_info->iova_start, region_info->iova_len);

	buf_info->buf = buf;
	buf_info->attach = dma_buf_attach(buf_info->buf, cb_info->dev);
	...
	size = iommu_map_sg(cb_info->domain, region_info->iova_start,
			    buf_info->table->sgl, buf_info->table->orig_nents,
			    prot);
...
err_unmap_sg:
	dma_buf_unmap_attachment(buf_info->attach, buf_info->table,
				 DMA_BIDIRECTIONAL);
err_detach:
				...
				int32_t smmu_hdl)
{
	...
	cb_info = &iommu_cb_set.cb_info[idx];
	if (cam_smmu_get_multiregion_client_dev_idx(
		    cb_info, multi_client_device_idx, region, &nested_reg_idx))
		return -EINVAL;
	...
	size = iommu_unmap(cb_info->domain, region_info->iova_start,
...
void cam_smmu_buffer_tracker_putref(struct list_head *track_list)
{
	...
	list_for_each_entry_safe(buffer_tracker, temp, track_list, list) {
		if (refcount_dec_and_test(&buffer_tracker->ref_count->refcount))
			CAM_ERR(CAM_SMMU,
				"[SMMU_BT] Unexpected - buffer reference [fd: 0x%x ino: 0x%x cb: "
				"%s] zeroed prior to unmap invocation",
				buffer_tracker->ion_fd, buffer_tracker->i_ino,
				buffer_tracker->cb_name);
		else
			CAM_DBG(CAM_SMMU,
				"[SMMU_BT] kref_count after put, [fd: 0x%x ino: 0x%x cb: %s], "
				"count: %d",
				buffer_tracker->ion_fd, buffer_tracker->i_ino,
				buffer_tracker->cb_name,
					...
					struct cam_dma_buff_info **mapping_info)
{
	...
	*mapping_info = kzalloc(sizeof(struct cam_dma_buff_info), GFP_KERNEL);
	...
	CAM_DBG(CAM_SMMU,
		"idx=%d, dma_buf=%pK, dev=%pOFfp, paddr=0x%llx, len=%zu", idx,
		buf, iommu_cb_set.cb_info[idx].dev->of_node, *paddr_ptr,
		*len_ptr);
...
static int cam_smmu_map_buffer_and_add_to_list(
	...
	enum dma_data_direction dma_dir, dma_addr_t *paddr_ptr, size_t *len_ptr,
	enum cam_smmu_region_id region_id, bool is_internal,
	struct dma_buf *buf, struct kref **ref_count)
{
	int rc = -1;
	...
	*ref_count = &mapping_info->ref_count;
	...
	CAM_DBG(CAM_SMMU, "fd %d i_ino %lu dmabuf %pK", ion_fd,
		mapping_info->i_ino, buf);
	...
	enum cam_smmu_region_id region_id)
{
	...
	CAM_DBG(CAM_SMMU, "fd %d i_ino %lu dmabuf %pK", mapping_info->ion_fd,
		mapping_info->i_ino, buf);
					...
					int idx)
{
	...
	if (iommu_cb_set.debug_cfg.map_profile_enable) {
		...
		trace_cam_log_event("SMMUUnmapProfile",
				    "size and time in micro", mapping_info->len,
				    microsec);
	}
...
static int
cam_smmu_util_get_free_map_entry(struct cam_smmu_buffer_tracker **entry)
{
	spin_lock_bh(&iommu_cb_set.s_lock);
...
int cam_smmu_add_buf_to_track_list(int ion_fd, unsigned long inode,
				   struct kref **ref_count,
				   struct list_head *buf_tracker, int idx)
{
	int rc = 0;
	...
	CAM_DBG(CAM_SMMU,
		...
		buf->ion_fd, buf->i_ino, kref_read(buf->ref_count),
		buf->cb_name);
...
cam_smmu_check_fd_in_list(int idx, int ion_fd, struct dma_buf *dmabuf,
			  dma_addr_t *paddr_ptr, size_t *len_ptr,
			  struct timespec64 **ts_mapping, unsigned long *inode,
			  struct kref **ref_count)
{
	struct cam_dma_buff_info *mapping;
	unsigned long i_ino;
...
static enum cam_smmu_buf_state
cam_smmu_user_reuse_fd_in_list(int idx, int ion_fd, struct dma_buf *dmabuf,
			       dma_addr_t *paddr_ptr, size_t *len_ptr,
...
static enum cam_smmu_buf_state
cam_smmu_check_secure_fd_in_list(int idx, int ion_fd, struct dma_buf *dmabuf,
				 dma_addr_t *paddr_ptr, size_t *len_ptr,
				 struct kref **ref_count)
{
	struct cam_sec_buff_info *mapping;
...
static enum cam_smmu_buf_state cam_smmu_validate_secure_fd_in_list(
	int idx, int ion_fd, struct dma_buf *dmabuf, dma_addr_t *paddr_ptr,
	size_t *len_ptr, unsigned long *inode, struct kref **ref_count)
{
	struct cam_sec_buff_info *mapping;
	unsigned long i_ino;
...
static int cam_smmu_map_stage2_buffer_and_add_to_list(
	...
	dma_addr_t *paddr_ptr, size_t *len_ptr, struct dma_buf *dmabuf,
	struct kref **ref_count)
{
	int rc = 0;
	struct dma_buf_attachment *attach = NULL;
   ...
   */
	...
	*ref_count = &mapping_info->ref_count;
	...
	CAM_DBG(CAM_SMMU,
		...
		iommu_cb_set.cb_info[idx].dev->of_node, *paddr_ptr, *len_ptr);
...
int cam_smmu_map_stage2_iova(int handle, int ion_fd, struct dma_buf *dmabuf,
			     ...
			     size_t *len_ptr, struct kref **ref_count)
{
	int idx, rc;
	enum dma_data_direction dma_dir;
	...
	*len_ptr = (size_t)0;
	...
	buf_state = cam_smmu_check_secure_fd_in_list(
		idx, ion_fd, dmabuf, paddr_ptr, len_ptr, ref_count);
	...
	rc = cam_smmu_map_stage2_buffer_and_add_to_list(
		idx, ion_fd, dma_dir, paddr_ptr, len_ptr, dmabuf, ref_count);
	if (rc < 0) {
		CAM_ERR(CAM_SMMU,
			       ...
			       bool force_unmap)
{
	...
	mapping_info =
		cam_smmu_find_mapping_by_sec_buf_idx(idx, ion_fd, dma_buf);
	if (!mapping_info) {
	...
	if (mapping_info->map_count > 0) {
		CAM_DBG(CAM_SMMU, "idx: %d fd = %d map_count: %d", idx, ion_fd,
			mapping_info->map_count);
		rc = 0;
		goto put_addr_end;
	}
	...
	if (!force_unmap && kref_read(&mapping_info->ref_count) > 1) {
		CAM_ERR(CAM_SMMU,
			"[SMMU_BT] Error: can't unmap buffer as it's still active, idx: "
			"%d, cb: %s, fd: 0x%x, ino: 0x%x, ref_count: %d",
...
int cam_smmu_map_user_iova(int handle, int ion_fd, struct dma_buf *dmabuf,
			   bool dis_delayed_unmap, enum cam_smmu_map_dir dir,
			   dma_addr_t *paddr_ptr, size_t *len_ptr,
			   enum cam_smmu_region_id region_id, bool is_internal,
			   struct kref **ref_count)
{
	...
	buf_state = cam_smmu_user_reuse_fd_in_list(
		idx, ion_fd, dmabuf, paddr_ptr, len_ptr, &ts, ref_count);
	if (buf_state == CAM_SMMU_BUFF_EXIST) {
		...
		CAM_ERR(CAM_SMMU,
			"fd=%d already in list [%llu:%llu:%lu:%llu] cb=%s idx=%d handle=%d "
			"len=%llu,give same addr back",
			ion_fd, hrs, min, sec, ms,
			iommu_cb_set.cb_info[idx].name[0], idx, handle,
			*len_ptr);
		rc = 0;
		goto get_addr_end;
	}
	...
	if (rc < 0) {
		CAM_ERR(CAM_SMMU,
			...
			iommu_cb_set.cb_info[idx].name[0], idx, ion_fd,
			region_id, rc);
		cam_smmu_dump_cb_info(idx);
	}
...
int cam_smmu_get_iova(int handle, int ion_fd, struct dma_buf *dma_buf,
		      dma_addr_t *paddr_ptr, size_t *len_ptr,
		      struct list_head *buf_tracker, struct kref **ref_count)
{
	int idx, rc = 0;
	struct timespec64 *ts = NULL;
	enum cam_smmu_buf_state buf_state;
	...
	*len_ptr = (size_t)0;
	...
	if (buf_tracker)
		rc = cam_smmu_add_buf_to_track_list(ion_fd, i_ino, ref_count,
						    buf_tracker, idx);
			     ...
			     struct kref **ref_count)
{
	...
	*len_ptr = (size_t)0;
	...
	buf_state = cam_smmu_validate_secure_fd_in_list(
		idx, ion_fd, dma_buf, paddr_ptr, len_ptr, &i_ino, ref_count);
...
int cam_smmu_unmap_user_iova(int handle, int ion_fd, struct dma_buf *dma_buf,
			     enum cam_smmu_region_id region_id,
			     bool force_unmap)
{
	int idx, rc;
	...
	if (cb->shared_support) {
		for (i = 0; i < cb->shared_info.num_regions; i++) {
			...
			rc = gen_pool_add(cb->shared_mem_pool[i],
					  cb->shared_info.nested_regions[i]
						  .region_info.iova_start,
					  cb->shared_info.nested_regions[i]
						  .region_info.iova_len,
					  -1);
			...
			CAM_DBG(CAM_SMMU,
				...
				cb->name[0],
				(unsigned long)cb->shared_info.nested_regions[i]
					.region_info.iova_start,
				cb->shared_info.nested_regions[i]
					.region_info.iova_len);
		}
...
static int cam_smmu_get_iova_info_util(struct device_node **child_node,
				       dma_addr_t *region_start,
				       size_t *region_len, uint32_t *region_id)
{
	...
	if (iommu_cb_set.is_expanded_memory) {
		rc = of_property_read_u64(*child_node, "iova-region-start",
					  &start);
		if (rc < 0) {
	...
	struct cam_context_bank_info *cb)
{
	...
	for_each_available_child_of_node(child_node, sub_node) {
		subregions =
			&nested_regions
				 ->subregions[nested_regions->num_subregions];

		rc = of_property_read_string(sub_node, "iova-region-name",
					     &subregion_name);
		if (rc < 0) {
		...
		case CAM_SMMU_SUBREGION_SYNX_HWMUTEX:
			...
			rc = of_property_read_u32(
				sub_node, "phy-addr",
				(uint32_t *)&subregions->subregion_info
					.phy_addr);
			if (rc < 0) {
		...
		case CAM_SMMU_SUBREGION_GLOBAL_CNTR:
			...
			break;
		default:
			CAM_ERR(CAM_SMMU, "Unsupported subregion_id: %d",
				subregion_id);
			rc = -EINVAL;
			goto err;
		}
		...
		CAM_DBG(CAM_SMMU,
			...
			cb->name[0], subregion_id,
			subregions->subregion_info.iova_start,
			subregions->subregion_info.iova_len,
			subregions->subregion_info.phy_addr,
			nested_regions->num_subregions);
	}
...
static int
cam_smmu_validate_discard_iova_region(struct cam_context_bank_info *cb,
				      struct cam_smmu_region_info *region_info)
{
	/* Make sure Discard region is properly specified */
	if ((cb->discard_iova_start != region_info->discard_iova_start) ||
	    (cb->discard_iova_len != region_info->discard_iova_len)) {
		CAM_ERR(CAM_SMMU,
		...
		if ((cb->discard_iova_start <= region_info->iova_start) ||
		    (cb->discard_iova_start >=
		     region_info->iova_start + region_info->iova_len) ||
		    (cb->discard_iova_start + cb->discard_iova_len >=
		     region_info->iova_start + region_info->iova_len)) {
			CAM_ERR(CAM_SMMU,
				"[%s] : Incorrect Discard region specified [0x%x 0x%x] in [0x%x "
				"0x%x]",
				cb->name[0], cb->discard_iova_start,
				cb->discard_iova_start + cb->discard_iova_len,
				region_info->iova_start,
				region_info->iova_start +
					region_info->iova_len);
					    ...
					    struct cam_context_bank_info *cb)
{
   ...
   */
	...
	for_each_available_child_of_node(mem_map_node, child_node) {
		...
		rc = cam_smmu_get_iova_info_util(&child_node, &region_start,
						 &region_len, &region_id);
		if (rc) {
			of_node_put(mem_map_node);
			return rc;
		}
		...
		case CAM_SMMU_REGION_FIRMWARE:
			...
			break;
		case CAM_SMMU_REGION_SHARED: {
			int32_t num_shared_regions =
				cb->shared_info.num_regions;
			struct cam_smmu_nested_region_info *nested_reg_info;

			if (num_shared_regions >= CAM_SMMU_MULTI_REGION_MAX) {
				CAM_ERR(CAM_SMMU,
					"Exceeding max supported number of regions max: %u current: %u "
					"in cb: %s for region: %d",
					CAM_SMMU_MULTI_REGION_MAX,
					num_shared_regions, cb->name[0],
					region_id);
				rc = -EINVAL;
				goto end;
			}
			...
			if (nested_reg_info->subregion_support) {
				rc = cam_smmu_get_subregions_memory_info(
					&child_node, nested_reg_info, cb);
				if (rc)
					goto end;
			}
		...
		case CAM_SMMU_REGION_IO: {
			...
			if (num_io_regions >= CAM_SMMU_MULTI_REGION_MAX) {
				CAM_ERR(CAM_SMMU,
					...
					"in cb: %s for region: %d",
					CAM_SMMU_MULTI_REGION_MAX,
					num_io_regions, cb->name[0], region_id);
				rc = -EINVAL;
				goto end;
			}

			nested_reg_info =
				&cb->io_info.nested_regions[num_io_regions];
			nested_reg_info->subregion_support =
				of_property_read_bool(child_node,
						      "subregion_support");
		...
		case CAM_SMMU_REGION_SECHEAP:
			...
			break;
		case CAM_SMMU_REGION_FWUNCACHED: {
			int32_t num_fwuncached_regions =
				cb->fwuncached_region.num_regions;
			struct cam_smmu_nested_region_info *nested_reg_info;

			if (num_fwuncached_regions >=
			    CAM_SMMU_MULTI_REGION_MAX) {
				CAM_ERR(CAM_SMMU,
					"Exceeding max supported number of regions max: %u current: %u "
					"in cb: %s for region: %d",
					CAM_SMMU_MULTI_REGION_MAX,
					num_fwuncached_regions, cb->name[0],
					region_id);
				rc = -EINVAL;
				goto end;
			}
			...
			nested_reg_info =
				&cb->fwuncached_region
					 .nested_regions[num_fwuncached_regions];
			nested_reg_info->subregion_support =
				of_property_read_bool(child_node,
						      "subregion_support");
		...
		case CAM_SMMU_REGION_QDSS: {
			...
			rc = of_property_read_u32(
				child_node, "qdss-phy-addr",
				(uint32_t *)&nested_reg_info->region_info
					.phy_addr);
			if (rc) {
				CAM_ERR(CAM_SMMU,
		...
		case CAM_SMMU_REGION_DEVICE: {
			...
			nested_reg_info =
				&cb->device_region
					 .nested_regions[num_device_regions];
			nested_reg_info->subregion_support =
				of_property_read_bool(child_node,
						      "subregion_support");
	...
	if (cb->io_support) {
		...
		for (i = 0; i < cb->io_info.num_regions; i++) {
			rc = cam_smmu_validate_discard_iova_region(
				cb, &cb->io_info.nested_regions[i].region_info);
			if (rc)
				goto end;
		}
	}
...
static void
cam_smmu_check_for_fault_properties(const char *fault_property,
				    struct cam_context_bank_info *cb)
{
					   ...
					   enum cam_iommu_type type)
{
	...
	cb->num_shared_hdl =
		of_property_count_strings(dev->of_node, "cam-smmu-label");

	if (cb->num_shared_hdl > CAM_SMMU_SHARED_HDL_MAX) {
	...
	for (i = 0; i < cb->num_shared_hdl; i++) {
		rc = of_property_read_string_index(
			dev->of_node, "cam-smmu-label", i, &cb->name[i]);
		if (rc < 0) {
			CAM_ERR(CAM_SMMU,
	...
	cb->num_multi_regions = of_property_count_strings(
		dev->of_node, "multiple-same-region-clients");
	...
	for (j = 0; j < cb->num_multi_regions; j++) {
		...
		rc = of_property_read_string_index(
			dev->of_node, "multiple-same-region-clients", j,
			&cb->multi_region_clients[j]);
		if (rc < 0) {
			CAM_ERR(CAM_SMMU,
				"Error: failed to read label from sub device");
			goto cb_init_fail;
	...
	dma_coherent_hint =
		of_property_read_bool(dev->of_node, "dma-coherent-hint-cached");
	...
	if (cb->io_support && cb->domain) {
		...
		num_fault_props = of_property_count_strings(
			dev->of_node, "qcom,iommu-faults");
		if (num_fault_props > 0) {
			const char *fault_property = NULL;

			for (i = 0; i < num_fault_props; i++) {
				rc = of_property_read_string_index(
					dev->of_node, "qcom,iommu-faults", i,
					&fault_property);
				if (!rc)
...
static void
cam_smmu_mini_dump_entries(struct cam_smmu_mini_dump_cb_info *target,
			   struct cam_context_bank_info *src)
{
	...
	for (i = 0; i < num_entries; i++) {
		memcpy(&target->mapping[index], &src->monitor_entries[index],
					   ...
					   void *priv_data)
{
	...
	if (!dst || len < sizeof(*md)) {
		CAM_ERR(CAM_SMMU, "Invalid params dst: %pk len:%lu", dst, len);
		return 0;
	}

	md = (struct cam_smmu_mini_dump_info *)dst;
	md->cb_num = 0;
	md->cb = (struct cam_smmu_mini_dump_cb_info *)((uint8_t *)dst +
						       sizeof(*md));
	...
	for (i = 0; i < iommu_cb_set.cb_num; i++) {
		...
		cb_md->fwuncached_region_support =
			cb->fwuncached_region_support;
...
static int cam_smmu_get_fatal_pf_mask(void *data, u64 *val)
{
	*val = iommu_cb_set.debug_cfg.fatal_pf_mask;
	CAM_DBG(CAM_SMMU, "Get fatal page fault value: 0x%llx", *val);
	return 0;
}

DEFINE_DEBUGFS_ATTRIBUTE(cam_smmu_fatal_pf_mask, cam_smmu_get_fatal_pf_mask,
			 cam_smmu_set_fatal_pf_mask, "%16llu");
...
static int cam_smmu_create_debug_fs(void)
{
	...
	debugfs_create_bool("cb_dump_enable", 0644,
			    iommu_cb_set.debug_cfg.dentry,
			    &iommu_cb_set.debug_cfg.cb_dump_enable);
	debugfs_create_bool("map_profile_enable", 0644,
			    iommu_cb_set.debug_cfg.dentry,
			    &iommu_cb_set.debug_cfg.map_profile_enable);
	debugfs_create_file("fatal_pf_mask", 0644,
			    iommu_cb_set.debug_cfg.dentry, NULL,
			    &cam_smmu_fatal_pf_mask);
	debugfs_create_bool("disable_buf_tracking", 0644,
			    iommu_cb_set.debug_cfg.dentry,
			    &iommu_cb_set.debug_cfg.disable_buf_tracking);
...
int cam_smmu_driver_init(struct cam_csf_version *csf_ver, int32_t *num_cbs)
{
	...
	*num_cbs = iommu_cb_set.cb_num;
	...
	iommu_cb_set.is_track_buf_disabled =
		iommu_cb_set.debug_cfg.disable_buf_tracking;

	if (!iommu_cb_set.is_track_buf_disabled) {
		buf_tracking_pool = kcalloc(
			CAM_SMMU_BUF_TRACKING_POOL,
				   ...
				   struct device *master_dev, void *data)
{
	...
	iommu_cb_set.need_shared_buffer_padding = of_property_read_bool(
		dev->of_node, "need_shared_buffer_padding");
	iommu_cb_set.is_expanded_memory =
		of_property_read_bool(dev->of_node, "expanded_memory");

	cam_common_register_mini_dump_cb(cam_smmu_mini_dump_cb, "cam_smmu",
					 NULL);
