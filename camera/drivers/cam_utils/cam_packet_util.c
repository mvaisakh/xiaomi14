				    ...
				    uint64_t packet_handle, uint32_t offset)
{
	...
	if (rc) {
		CAM_ERR(CAM_UTIL,
			"Failed to get packet address from handle: 0x%llx rc: %d",
			packet_handle, rc);
		*packet = NULL;
		return rc;
	}
...
void cam_packet_util_dump_patch_info(struct cam_packet *packet,
				     int32_t iommu_hdl, int32_t sec_iommu_hdl,
				     struct cam_hw_dump_pf_args *pf_args)
{
	...
	CAM_INFO(CAM_UTIL, "Total num of patches : %d", packet->num_patches);
	...
	for (i = 0; i < packet->num_patches; i++) {
		...
		if (pf_args &&
			...
			pf_context_info->delta = CAM_SMMU_GET_IOVA_DELTA(
				pf_args->pf_smmu_info->iova, iova_addr);
			pf_context_info->req_id = packet->header.request_id;
			pf_context_info->ctx_found = true;

			CAM_ERR(CAM_UTIL,
				"Found PF at patch: %d src buf hdl: 0x%llx", i,
		...
		CAM_INFO(
			CAM_UTIL,
			"i = %d src_buf 0x%llx src_hdl 0x%x src_buf_with_offset 0x%llx "
			"src_size 0x%llx src_flags: %x dst %p dst_offset %u dst_hdl 0x%x "
			"value 0x%x",
			i, iova_addr, patch_desc[i].src_buf_hdl,
			(iova_addr + patch_desc[i].src_offset), src_buf_size,
			flags, dst_cpu_addr, patch_desc[i].dst_offset,
...
static int
cam_packet_util_get_patch_iova(struct cam_patch_unique_src_buf_tbl *tbl,
			       int32_t hdl, uint32_t buf_hdl, dma_addr_t *iova,
			       size_t *buf_size, uint32_t *flags,
			       struct list_head *mapped_io_list)
{
	int idx = 0;
	int rc = 0;
	size_t src_buf_size;
	...
	for (idx = 0; idx < CAM_UNIQUE_SRC_HDL_MAX; idx++) {
		...
		CAM_DBG(CAM_UTIL,
			"Index: %d is filled with differnt src_hdl: 0x%x", idx,
			buf_hdl);
	}
	...
	if (!is_found) {
		CAM_DBG(CAM_UTIL, "src_hdl 0x%x not found in table entries",
			buf_hdl);
		rc = cam_mem_get_io_buf(buf_hdl, hdl, &iova_addr, &src_buf_size,
					flags, mapped_io_list);
		if (rc < 0) {
			CAM_ERR(CAM_UTIL,
				    ...
				    bool exp_mem)
{
	...
	uint32_t flags = 0;
	int32_t hdl;
	struct cam_patch_unique_src_buf_tbl tbl[CAM_UNIQUE_SRC_HDL_MAX];

	memset(tbl, 0,
	       CAM_UNIQUE_SRC_HDL_MAX *
	...
	for (i = 0; i < packet->num_patches; i++) {
		...
		if ((size_t)patch_desc[i].src_offset >= src_buf_size) {
			CAM_ERR(CAM_UTIL,
				"Invalid src buf patch offset: patch:src_offset: 0x%x, "
				"src_buf_size: %zu",
		...
		if (exp_mem && cam_smmu_is_expanded_memory()) {
			if ((flags & CAM_MEM_FLAG_HW_SHARED_ACCESS) ||
				...
				*dst_cpu_addr =
					CAM_36BIT_INTF_GET_IOVA_BASE(temp);
			}
		} else {
			*dst_cpu_addr = temp;
		}
		...
		CAM_DBG(CAM_UTIL,
			"patch is done for dst %pK with base iova 0x%lx final iova 0x%lx "
			"patched value 0x%x, shared=%s, cmd=%s, HwAndCDM %s",
			dst_cpu_addr, iova_addr, temp, *dst_cpu_addr,
			CAM_BOOL_TO_YESNO(flags &
					  CAM_MEM_FLAG_HW_SHARED_ACCESS),
			CAM_BOOL_TO_YESNO(flags & CAM_MEM_FLAG_CMD_BUF_TYPE),
			CAM_BOOL_TO_YESNO(flags &
					  CAM_MEM_FLAG_HW_AND_CDM_OR_SHARED));
		cam_mem_put_cpu_buf((int32_t)patch_desc[i].dst_buf_hdl);
	}
...
void cam_packet_util_dump_io_bufs(struct cam_packet *packet, int32_t iommu_hdl,
				  int32_t sec_mmu_hdl,
				  struct cam_hw_dump_pf_args *pf_args,
				  bool res_id_support)
{
	...
	for (i = 0; i < packet->num_io_configs; i++) {
		if (res_id_support &&
		    io_cfg[i].resource_type != pf_context_info->resource_type)
			continue;
		...
		for (j = 0; j < CAM_PACKET_MAX_PLANES; j++) {
			...
			if (GET_FD_FROM_HANDLE(io_cfg[i].mem_handle[j]) ==
			    buf_fd) {
				pf_context_info->mem_type =
					CAM_FAULT_IO_CFG_BUF;
				pf_context_info->buf_hdl =
					io_cfg[i].mem_handle[j];
				...
				CAM_INFO(
					...
					io_cfg[i].resource_type,
					io_cfg[i].mem_handle[j], buf_fd, j,
					pf_context_info->delta);
			}
			CAM_INFO(
				CAM_UTIL,
				"pln %d w %d h %d s %u size %zu addr 0x%llx end_addr 0x%llx "
				"offset %u memh 0x%x",
				j, io_cfg[i].planes[j].width,
				io_cfg[i].planes[j].height,
				io_cfg[i].planes[j].plane_stride, src_buf_size,
				iova_addr, iova_addr + src_buf_size,
...
int cam_presil_retrieve_buffers_from_packet(struct cam_packet *packet,
					    int iommu_hdl, int out_res_id)
{
	int rc = 0, i, j;
	...
	if (!packet || (iommu_hdl < 0)) {
		CAM_ERR(CAM_PRESIL, "Invalid params packet %pK iommu_hdl: %d",
			packet, iommu_hdl);
		return -EINVAL;
	}
	...
	for (i = 0; i < packet->num_io_configs; i++) {
		...
		for (j = 0; j < CAM_PACKET_MAX_PLANES; j++) {
			...
			rc = cam_mem_get_io_buf(io_cfg[i].mem_handle[j],
						iommu_hdl, &io_addr[j], &size,
						NULL, NULL);
			if (rc) {
				CAM_ERR(CAM_PRESIL, "no io addr for plane%d",
					j);
				rc = -ENOMEM;
				return rc;
			}
			...
			CAM_INFO(
				...
				io_cfg[i].mem_handle[j], io_addr[j],
				io_cfg[i].offsets[j], io_cfg[i].resource_type);
			cam_mem_mgr_retrieve_buffer_from_presil(
				io_cfg[i].mem_handle[j], size,
...
int cam_presil_send_buffers_from_packet(struct cam_packet *packet,
					int img_iommu_hdl, int cdm_iommu_hdl)
{
	struct cam_buf_io_cfg *io_cfg = NULL;
	...
	for (i = 0; i < packet->num_io_configs; i++) {
		...
		for (j = 0; j < CAM_PACKET_MAX_PLANES; j++) {
			...
			CAM_DBG(CAM_PRESIL, "Adding IO CFG buffer:%d",
				io_cfg[i].mem_handle[j]);
			cam_presil_add_unique_buf_hdl_to_list(
				io_cfg[i].mem_handle[j], unique_img_buffers,
				&num_img_handles, CAM_PRESIL_UNIQUE_HDL_MAX);
		}
	}

	for (i = 0; i < num_img_handles; i++) {
		CAM_DBG(CAM_PRESIL, "Sending Image buffer i:%d mem_handle:%d",
			i, unique_img_buffers[i]);
...
send_cmd_buffers:
	...
	for (i = 0; i < packet->num_cmd_buf; i++) {
		CAM_DBG(CAM_PRESIL, "Adding CMD buffer:%d",
			cmd_desc[i].mem_handle);
		cam_presil_add_unique_buf_hdl_to_list(
			cmd_desc[i].mem_handle, unique_cmd_buffers,
			&num_cmd_handles, CAM_PRESIL_UNIQUE_HDL_MAX);
	}

	/* Adding Patch src buffer handles to list */
	patch_desc = (struct cam_patch_desc *)((uint8_t *)&packet->payload +
					       packet->patch_offset);
	for (i = 0; i < packet->num_patches; i++) {
		CAM_DBG(CAM_PRESIL, "Adding Patch src buffer:%d",
			patch_desc[i].src_buf_hdl);
		cam_presil_add_unique_buf_hdl_to_list(
			patch_desc[i].src_buf_hdl, unique_cmd_buffers,
			&num_cmd_handles, CAM_PRESIL_UNIQUE_HDL_MAX);
	}

	for (i = 0; i < num_cmd_handles; i++) {
		CAM_DBG(CAM_PRESIL, "Sending Command buffer i:%d mem_handle:%d",
			i, unique_cmd_buffers[i]);
