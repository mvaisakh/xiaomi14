				   ...
				   struct cam_cre_device_core_info *core_info)
{
	...
	CAM_DBG(CAM_CRE, "soc_info = %x core_info = %x", soc_info, core_info);
	...
	core_info->hw_version =
		cam_io_r_mb(core_info->cre_hw_info->cre_top_base +
	...
	cre_hw_100.bus_rd_reg_offset->base =
		core_info->cre_hw_info->cre_bus_rd_base;
	cre_hw_100.bus_wr_reg_offset->base =
		core_info->cre_hw_info->cre_bus_wr_base;
...
static int cam_cre_component_bind(struct device *dev, struct device *master_dev,
				  void *data)
{
	...
	snprintf(cre_dev_name, sizeof(cre_dev_name), "cre%1u",
		 cre_dev_intf->hw_idx);
	...
	CAM_DBG(CAM_CRE, "type %d index %d", cre_dev_intf->hw_type,
		cre_dev_intf->hw_idx);

	if (cre_dev_intf->hw_idx < CRE_DEV_MAX)
		cam_cre_dev_list[cre_dev_intf->hw_idx].hw_intf = cre_dev_intf;

	platform_set_drvdata(pdev, cre_dev_intf);

	cre_dev->core_info =
		kzalloc(sizeof(struct cam_cre_device_core_info), GFP_KERNEL);
	...
	rc = cam_cre_register_cpas(&cre_dev->soc_info, core_info,
				   cre_dev_intf->hw_idx);
	if (rc < 0)
	...
	cpas_vote.axi_vote.axi_path[0].transac_type = CAM_AXI_TRANSACTION_WRITE;
	...
	cam_cre_process_cmd(cre_dev, CRE_HW_PROBE, &cre_probe,
			    sizeof(cre_probe));
