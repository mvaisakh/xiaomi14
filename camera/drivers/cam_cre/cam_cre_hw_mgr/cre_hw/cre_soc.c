			       ...
			       irq_handler_t cre_irq_handler, void *data)
{
	...
	rc = cam_soc_util_request_platform_resource(soc_info, cre_irq_handler,
						    &(irq_data[0]));
	if (rc)
...
int cam_cre_update_clk_rate(struct cam_hw_soc_info *soc_info, uint32_t clk_rate)
{
	int32_t src_clk_idx;
	...
	CAM_DBG(CAM_CRE, "clk_rate = %u src_clk_index = %d", clk_rate,
		src_clk_idx);
