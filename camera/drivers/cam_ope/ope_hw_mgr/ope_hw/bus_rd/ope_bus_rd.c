...
static int cam_ope_bus_rd_in_port_idx(uint32_t input_port_id)
{
	...
	for (i = 0; i < OPE_IN_RES_MAX; i++)
		if (bus_rd->in_port_to_rm[i].input_port_id == input_port_id)
			return i;
...
static int cam_ope_bus_rd_combo_idx(uint32_t format)
{
	...
	CAM_DBG(CAM_OPE, "Input format = %u rc = %d", format, rc);
	return rc;
}

static int cam_ope_bus_is_rm_enabled(struct cam_ope_request *ope_request,
				     uint32_t batch_idx, uint32_t rm_id)
{
	int i, k;
	int32_t combo_idx;
	...
	for (i = 0; i < ope_request->num_io_bufs[batch_idx]; i++) {
		...
		for (k = 0; k < io_buf->num_planes; k++) {
			if (rm_id == in_port_to_rm->rm_port_id[combo_idx][k])
				return true;
		}
	}
...
static uint32_t *cam_ope_bus_rd_update(struct ope_hw *ope_hw_info,
				       int32_t ctx_id, uint32_t *kmd_buf,
				       ...
				       int32_t *num_stripes)
{
	...
	CAM_DBG(CAM_OPE, "req_idx = %d req_id = %lld KMDbuf %x offset %d",
		req_idx, ope_request->request_id, kmd_buf,
	...
	CAM_DBG(CAM_OPE,
		...
		req_idx, ope_request->request_id, kmd_buf,
		prepare->kmd_buf_offset, io_buf->resource_type);
	CAM_DBG(CAM_OPE, "batch:%d iobuf:%d direction:%d", batch_idx, io_idx,
		io_buf->direction);
	io_port_cdm = &bus_rd_ctx->io_port_cdm_batch.io_port_cdm[batch_idx];
	in_port_to_rm = &bus_rd->in_port_to_rm[io_buf->resource_type - 1];
	combo_idx = cam_ope_bus_rd_combo_idx(io_buf->format);
	if (combo_idx < 0) {
		CAM_ERR(CAM_OPE, "Invalid combo_idx");
		return NULL;
	...
	for (k = 0; k < io_buf->num_planes; k++) {
		for (l = 0; l < io_buf->num_stripes[k]; l++) {
			*num_stripes = io_buf->num_stripes[k];
			...
			temp_reg[count++] = ctx_data->ope_acquire.secure_mode;
			...
			/* Buffer size */
			temp_reg[count++] =
				rd_reg->offset + rd_reg_client->img_cfg;
			temp = 0;
			...
			/* stride */
			temp_reg[count++] =
				rd_reg->offset + rd_reg_client->stride;
       ...
       */
			...
			temp_reg[count++] =
				rd_reg->offset + rd_reg_client->unpack_cfg;
			temp = 0;
			...
			CAM_DBG(CAM_OPE, "kmd_buf:%x offset:%d", kmd_buf,
				prepare->kmd_buf_offset);
					...
					uint32_t *kmd_buf, uint32_t num_stripes)
{
	...
	for (l = 0; l < num_stripes; l++) {
		...
		temp_reg[count++] = rd_reg->offset + rd_reg_client->core_cfg;
		temp_reg[count++] = 0;
		...
		CAM_DBG(CAM_OPE, "RD cmd bufs = %d",
			io_port_cdm->num_s_cmd_bufs[l]);
		CAM_DBG(CAM_OPE, "stripe %d off:%d len:%d", l,
			io_port_cdm->s_cdm_info[l][idx].offset,
			io_port_cdm->s_cdm_info[l][idx].len);
		count = 0;
	}
...
static int cam_ope_bus_rd_prepare(struct ope_hw *ope_hw_info, int32_t ctx_id,
				  void *data)
{
	int rc = 0;
	int i, j;
	...
	kmd_buf = (uint32_t *)ope_request->ope_kmd_buf.cpu_addr +
		  prepare->kmd_buf_offset;
	CAM_DBG(CAM_OPE, "req_idx = %d req_id = %lld", req_idx,
		ope_request->request_id);
	CAM_DBG(CAM_OPE, "KMD buf and offset = %x %d", kmd_buf,
		prepare->kmd_buf_offset);
	bus_rd_ctx = bus_rd->bus_rd_ctx[ctx_id];
	io_port_cdm_batch = &bus_rd_ctx->io_port_cdm_batch;
	...
	for (i = 0; i < ope_request->num_batch; i++) {
		for (j = 0; j < ope_request->num_io_bufs[i]; j++) {
			...
			CAM_DBG(CAM_OPE, "batch:%d iobuf:%d direction:%d", i, j,
				io_buf->direction);
			io_port_cdm =
			...
			kmd_buf = cam_ope_bus_rd_update(ope_hw_info, ctx_id,
							kmd_buf, i, j, prepare,
							&num_stripes);
			if (!kmd_buf) {
				rc = -EINVAL;
	...
	for (i = 0; i < ope_request->num_batch; i++) {
		for (j = 0; j < rd_reg_val->num_clients; j++) {
			is_rm_enabled =
				cam_ope_bus_is_rm_enabled(ope_request, i, j);
			if (is_rm_enabled < 0) {
				rc = -EINVAL;
				goto end;
			}
			...
			kmd_buf = cam_ope_bus_rm_disable(ope_hw_info, ctx_id,
							 prepare, i, j, kmd_buf,
							 num_stripes);
			if (!kmd_buf) {
				rc = -EINVAL;
	...
	/* Go command */
	count = 0;
	temp_reg[count++] = rd_reg->offset + rd_reg->input_if_cmd;
	temp = 0;
	temp |= rd_reg_val->go_cmd;
	temp_reg[count++] = temp;

	header_size = cdm_ops->cdm_get_cmd_header_size(CAM_CDM_CMD_REG_RANDOM);
	for (i = 0; i < ope_request->num_batch; i++) {
		io_port_cdm = &bus_rd_ctx->io_port_cdm_batch.io_port_cdm[i];
		io_port_cdm->go_cmd_addr = kmd_buf;
		io_port_cdm->go_cmd_len = sizeof(temp) * (count + header_size);
		io_port_cdm->go_cmd_offset = prepare->kmd_buf_offset;
	}
	next_buff_addr =
		cdm_ops->cdm_write_regrandom(kmd_buf, count / 2, temp_reg);
	if (next_buff_addr > kmd_buf)
		prepare->kmd_buf_offset +=
			((count + header_size) * sizeof(temp));
	kmd_buf = next_buff_addr;
	CAM_DBG(CAM_OPE, "kmd_buf:%x,offset:%d", kmd_buf,
		prepare->kmd_buf_offset);
	CAM_DBG(CAM_OPE, "t_reg:%xcount: %d size:%d", temp_reg, count,
		header_size);
	prepare->rd_cdm_batch = &bus_rd_ctx->io_port_cdm_batch;
...
static int cam_ope_bus_rd_release(struct ope_hw *ope_hw_info, int32_t ctx_id,
				  void *data)
{
	int rc = 0;
...
static int cam_ope_bus_rd_acquire(struct ope_hw *ope_hw_info, int32_t ctx_id,
				  void *data)
{
	int rc = 0, i;
	...
	if (ctx_id < 0 || !data || !ope_hw_info || ctx_id >= OPE_CTX_MAX) {
		CAM_ERR(CAM_OPE, "Invalid data: %d %x %x", ctx_id, data,
			ope_hw_info);
		return -EINVAL;
	}
	...
	for (i = 0; i < in_acquire->num_in_res; i++) {
		...
		combo_idx =
			cam_ope_bus_rd_combo_idx(in_acquire->in_res[i].format);
		if (combo_idx < 0) {
...
static int cam_ope_bus_rd_init(struct ope_hw *ope_hw_info, int32_t ctx_id,
			       void *data)
{
	int rc = 0;
	struct cam_ope_bus_rd_reg_val *bus_rd_reg_val;
	...
	rc = cam_common_wait_for_completion_timeout(&bus_rd->reset_complete,
...
static int cam_ope_bus_rd_probe(struct ope_hw *ope_hw_info, int32_t ctx_id,
				void *data)
{
	...
	for (i = 0; i < OPE_IN_RES_MAX; i++) {
		...
		for (j = 0; j < in_port_to_rm->num_combos; j++) {
			CAM_DBG(CAM_OPE, "combo idx = %d num_rms = %d", j,
				in_port_to_rm->num_rm[j]);
...
static int cam_ope_bus_rd_isr(struct ope_hw *ope_hw_info, int32_t ctx_id,
			      void *data)
{
	int rc = 0;
	uint32_t irq_status;
	...
	cam_io_w_mb(irq_status, bus_rd_reg->base + bus_rd_reg->irq_clear);
...
int cam_ope_bus_rd_process(struct ope_hw *ope_hw_info, int32_t ctx_id,
