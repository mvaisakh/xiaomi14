...
void mmrm_vm_fe_recv(struct mmrm_vm_driver_data *mmrm_vm, void *data,
		     size_t size)
{
...
int mmrm_vm_fe_load_clk_rsrc(struct mmrm_vm_driver_data *mmrm_vm)
{
	...
	fe_data->clk_src_set.clk_src_tbl =
...
mmrm_vm_fe_clk_src_get(struct mmrm_client_desc *desc)
{
...
int mmrm_vm_fe_init_lookup_table(struct mmrm_vm_driver_data *mmrm_vm)
{
	...
	fe_data->client_tbl = devm_kzalloc(&pdev->dev,
					   fe_data->clk_src_set.count *
						   sizeof(struct mmrm_client),
					   GFP_KERNEL);
...
int mmrm_vm_fe_clk_print_info(struct mmrm_vm_fe_clk_src_set *clk_src_set,
			      char *buf, int max_len)
{
	...
	for (c = 0; c < num_clk_src; c++, pclk_src++) {
		len = scnprintf(buf, left_spaces, "%d\t%d\t%d\n",
				pclk_src->client_domain, pclk_src->client_id,
				pclk_src->num_hw_blocks);
		left_spaces -= len;
		buf += len;
	}
...
struct mmrm_client *mmrm_vm_fe_get_client(u32 client_id)
{
	...
	for (i = 0, ptr = fe_data->client_tbl; i < fe_data->clk_src_set.count;
	     i++, ptr++) {
