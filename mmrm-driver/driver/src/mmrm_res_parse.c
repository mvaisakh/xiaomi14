...
static int mmrm_find_key_value(struct mmrm_platform_data *pdata,
			       const char *key)
{
	int i = 0;
...
static int
mmrm_read_clk_pltfrm_rsrc_frm_drv_data(struct mmrm_driver_data *ddata)
{
	...
	cres->scheme = mmrm_find_key_value(pdata, "qcom,mmrm_clk_mgr_scheme");
	d_mpr_h("%s: configured mmrm scheme %d\n", __func__, cres->scheme);
	cres->throttle_clients_data_length =
		pdata->throttle_clk_clients_data_length;
...
static void
mmrm_free_rail_corner_table(struct mmrm_clk_platform_resources *cres)
{
...
static int
mmrm_load_mm_rail_corner_table(struct mmrm_clk_platform_resources *cres)
{
	...
	num_corners =
		of_property_count_strings(pdev->dev.of_node, "mm-rail-corners");
	if (num_corners <= 0) {
	...
	corners->corner_tbl = devm_kzalloc(
		&pdev->dev, sizeof(*corners->corner_tbl) * num_corners,
		GFP_KERNEL);
	...
	for (c = 0; c < num_corners; c++) {
		...
		of_property_read_u32_index(pdev->dev.of_node,
					   "scaling-fact-dyn", c,
					   &ci->scaling_factor_dyn);
		of_property_read_u32_index(pdev->dev.of_node,
					   "scaling-fact-leak", c,
					   &ci->scaling_factor_leak);
	}
	...
	for (c = 0; c < num_corners; c++) {
		...
		d_mpr_h("%s: corner_name:%s volt_factor: %d sc_dyn: %d sc_leak: %d\n",
...
static void
mmrm_free_nom_clk_src_table(struct mmrm_clk_platform_resources *cres)
{
	cres->nom_clk_set.clk_src_tbl = NULL;
	cres->nom_clk_set.count = 0;
}

static int mmrm_load_nom_clk_src_table(struct mmrm_clk_platform_resources *cres)
{
	...
	rc = of_property_read_u32(pdev->dev.of_node, "mmrm-peak-threshold",
				  &cres->peak_threshold);
	if (rc < 0) {
	...
	clk_srcs->clk_src_tbl =
		devm_kzalloc(&pdev->dev, size_clk_src, GFP_KERNEL);
...
static int
mmrm_read_clk_pltfrm_rsrc_frm_dt(struct mmrm_clk_platform_resources *cres)
{
	int rc = 0;
				 ...
				 struct mmrm_driver_data *drv_data)
{
	...
	if (pdev->dev.of_node) {
		...
		if (rc) {
			d_mpr_e("%s: failed to read clk platform res from driver\n",
				__func__);
			goto exit;
		}
