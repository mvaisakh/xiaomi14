...
static int
ib_parse_type7_set_draw_state(struct kgsl_device *device, unsigned int *ptr,
			      struct kgsl_process_private *process,
			      struct adreno_ib_object_list *ib_obj_list);
 ...
 */
static struct adreno_ib_object *
adreno_ib_check_overlap(uint64_t gpuaddr, uint64_t size, int type,
			struct adreno_ib_object_list *ib_obj_list)
{
 ...
 */
static int adreno_ib_add(struct kgsl_process_private *process, uint64_t gpuaddr,
			 int type, struct adreno_ib_object_list *ib_obj_list)
{
	uint64_t size;
	struct adreno_ib_object *ib_obj;
	...
	if (ib_obj) {
		...
		adreno_ib_init_ib_obj(
			gpuaddr, size, type, entry,
				 ...
				 struct adreno_ib_object_list *ib_obj_list)
{
	...
	if (3 == block && 1 == type) {
		...
		if (!kgsl_gpuaddr_in_memdesc(&ent->memdesc, gpuaddr, size)) {
				 ...
				 struct ib_parser_variables *ib_parse_vars)
{
	...
	ret = adreno_ib_add(process, pkt[1], SNAPSHOT_GPU_OBJECT_GENERIC,
			    ib_obj_list);
	if (ret)
		return ret;

	/* visiblity stream size buffer (fixed size 8 dwords) */
	ret = adreno_ib_add(process, pkt[2], SNAPSHOT_GPU_OBJECT_GENERIC,
			    ib_obj_list);
				...
				struct ib_parser_variables *ib_parse_vars)
{
	...
	for (i = ADRENO_CP_ADDR_VSC_PIPE_DATA_ADDRESS_0;
		...
		if (ib_parse_vars->cp_addr_regs[i]) {
			ret = adreno_ib_add(
				process, ib_parse_vars->cp_addr_regs[i] & mask,
				SNAPSHOT_GPU_OBJECT_GENERIC, ib_obj_list);
			if (ret)
				return ret;
	...
	for (i = ADRENO_CP_ADDR_VFD_FETCH_INSTR_1_0; i <= vfd_end; i++) {
		if (ib_parse_vars->cp_addr_regs[i]) {
	...
	if (ib_parse_vars->cp_addr_regs[ADRENO_CP_ADDR_VSC_SIZE_ADDRESS]) {
		...
		if (ret)
			return ret;
		ib_parse_vars->cp_addr_regs[ADRENO_CP_ADDR_VSC_SIZE_ADDRESS] =
			0;
	}
	mask = 0xFFFFFFE0;
			      ...
			      struct ib_parser_variables *ib_parse_vars)
{
	...
	case CP_DRAW_INDX:
		if (type3_pkt_size(pkt[0]) > 3) {
			ret = adreno_ib_add(process, pkt[4],
					    SNAPSHOT_GPU_OBJECT_GENERIC,
					    ib_obj_list);
		}
		break;
	case CP_DRAW_INDX_OFFSET:
		if (type3_pkt_size(pkt[0]) == 6) {
			ret = adreno_ib_add(process, pkt[5],
					    SNAPSHOT_GPU_OBJECT_GENERIC,
					    ib_obj_list);
		}
		break;
	...
	case CP_DRAW_AUTO:
		if (type3_pkt_size(pkt[0]) == 6) {
			ret = adreno_ib_add(process, pkt[3],
					    SNAPSHOT_GPU_OBJECT_GENERIC,
					    ib_obj_list);
			if (ret)
				break;
   ...
   */
	ret = ib_add_type0_entries(device, process, ib_obj_list, ib_parse_vars);
	if (ret)
		return ret;
	...
	for (i = 0; i < NUM_SET_DRAW_GROUPS; i++) {
		...
		ret = adreno_ib_find_objs(
			...
			ib_parse_vars->set_draw_groups[i].cmd_stream_dwords, 0,
			SNAPSHOT_GPU_OBJECT_DRAW, ib_obj_list, 2);
		if (ret)
			break;
	}
	return ret;
			  ...
			  struct ib_parser_variables *ib_parse_vars)
{
	...
	for (i = 0; i < size; i++, offset++) {
		/* Visiblity stream buffer */
		if (offset >= adreno_cp_parser_getreg(
				      adreno_dev,
				      ADRENO_CP_ADDR_VSC_PIPE_DATA_ADDRESS_0) &&
		    offset <= adreno_cp_parser_getreg(
				      adreno_dev,
     ...
     */
		if (flags & 0x1 || !flags) {
			...
			if (cmd_stream_dwords)
				ret = adreno_ib_find_objs(
					device, process, cmd_stream_addr,
					cmd_stream_dwords, 0,
		...
		if (flags & 0x8) {
			...
			ret = adreno_ib_find_objs(device, process, gpuaddr,
						  (ptr[i] & 0x0000FFFF), 0,
				   ...
				   struct ib_parser_variables *ib_parse_vars)
{
   ...
   */
     ...
     */
		...
		/* load immediate */
		if (flags & 0x8) {
			ret = adreno_ib_find_objs(device, process, ptr[i + 1],
						  (ptr[i] & 0x0000FFFF), 0,
						  SNAPSHOT_GPU_OBJECT_IB,
						  ib_obj_list, 2);
			if (ret)
				 ...
				 struct adreno_ib_object_list **out_ib_obj_list)
{
	...
	*out_ib_obj_list = NULL;
	...
	ib_obj_list->obj_list =
		vmalloc(MAX_IB_OBJS * sizeof(struct adreno_ib_object));
