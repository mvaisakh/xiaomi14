   ...
   */
	...
	if (!ret)
		gmu_core_regwrite(KGSL_DEVICE(adreno_dev),
				  A6XX_GMU_HOST2GMU_INTR_SET, 0x1);
...
int a6xx_hfi_init(struct adreno_device *adreno_dev)
{
	...
	if (IS_ERR_OR_NULL(hfi->hfi_mem)) {
		hfi->hfi_mem = reserve_gmu_kernel_block(
...
int a6xx_hfi_send_set_value(struct adreno_device *adreno_dev, u32 type,
				 ...
				 struct pending_cmd *ret_cmd)
{
	...
	case F2H_MSG_DEBUG: /* No Reply */
		...
		break;
	default: /* No Reply */
		dev_err(&gmu->pdev->dev, "HFI V1 request %d not supported\n",
			MSG_HDR_GET_ID(rcvd[0]));
		break;
	}
}

int a6xx_hfi_process_queue(struct a6xx_gmu_device *gmu, uint32_t queue_idx,
			   struct pending_cmd *ret_cmd)
{
...
static int a6xx_hfi_verify_fw_version(struct adreno_device *adreno_dev)
{
	...
	result = a6xx_hfi_get_fw_version(adreno_dev,
					 GMU_VERSION(major, minor, 0), &ver);
	if (result) {
