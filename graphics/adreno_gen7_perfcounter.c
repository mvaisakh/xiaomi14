 ...
 */
static u64
gen7_counter_read_norestore(struct adreno_device *adreno_dev,
...
static int
gen7_hwsched_counter_enable(struct adreno_device *adreno_dev,
			    const struct adreno_perfcount_group *group,
			    u32 counter, u32 countable)
{
	...
	return gen7_hwsched_counter_inline_enable(adreno_dev, group, counter,
						  countable);
}
...
static int
gen7_counter_gbif_pwr_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static int
gen7_counter_alwayson_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	return 0;
...
static u64
gen7_counter_alwayson_read(struct adreno_device *adreno_dev,
			   const struct adreno_perfcount_group *group,
			   unsigned int counter)
{
	struct adreno_perfcount_register *reg = &group->regs[counter];
...
static int
gen7_counter_gmu_xoclk_enable(struct adreno_device *adreno_dev,
			      const struct adreno_perfcount_group *group,
			      unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static int
gen7_counter_gmu_gmuclk_enable(struct adreno_device *adreno_dev,
			       const struct adreno_perfcount_group *group,
			       unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
   ...
   */
	gen7_write_gmu_counter_enable(device, reg, 16 + (counter * 8),
				      countable);
...
static int
gen7_counter_gmu_perf_enable(struct adreno_device *adreno_dev,
			     const struct adreno_perfcount_group *group,
			     unsigned int counter, unsigned int countable)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
#define GEN7_PERFCOUNTER_GROUP_FLAGS(core, offset, name, flags, enable, read) \
	[KGSL_PERFCOUNTER_GROUP_##                                            \
		offset] = { core##_perfcounters_##name,                       \
...
#define GEN7_PERFCOUNTER_GROUP(offset, name, enable, read)                     \
	GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, offset, name,                       \
				     ADRENO_PERFCOUNTER_GROUP_RESTORE, enable, \
				     read)

#define GEN7_REGULAR_PERFCOUNTER_GROUP(offset, name)              \
	GEN7_PERFCOUNTER_GROUP(offset, name, gen7_counter_enable, \
			       gen7_counter_read)
...
#define GEN7_BV_REGULAR_PERFCOUNTER_GROUP(offset, name)              \
	GEN7_BV_PERFCOUNTER_GROUP(offset, name, gen7_counter_enable, \
				  gen7_counter_read)

static const struct adreno_perfcount_group
	gen7_hwsched_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		GEN7_REGULAR_PERFCOUNTER_GROUP(CP, cp),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, RBBM, rbbm, 0,
					     gen7_counter_enable,
					     gen7_counter_read),
		GEN7_REGULAR_PERFCOUNTER_GROUP(PC, pc),
		GEN7_PERFCOUNTER_GROUP(VFD, vfd, gen7_hwsched_counter_enable,
				       gen7_counter_read),
		GEN7_PERFCOUNTER_GROUP(HLSQ, hlsq, gen7_counter_br_enable,
				       gen7_counter_read),
		GEN7_REGULAR_PERFCOUNTER_GROUP(VPC, vpc),
		GEN7_REGULAR_PERFCOUNTER_GROUP(CCU, ccu),
		GEN7_REGULAR_PERFCOUNTER_GROUP(CMP, cmp),
		GEN7_PERFCOUNTER_GROUP(TSE, tse, gen7_counter_br_enable,
				       gen7_counter_read),
		GEN7_PERFCOUNTER_GROUP(RAS, ras, gen7_counter_br_enable,
				       gen7_counter_read),
		GEN7_PERFCOUNTER_GROUP(LRZ, lrz, gen7_counter_br_enable,
				       gen7_counter_read),
		GEN7_REGULAR_PERFCOUNTER_GROUP(UCHE, gen7_2_0_uche),
		GEN7_PERFCOUNTER_GROUP(TP, tp, gen7_hwsched_counter_enable,
				       gen7_counter_read),
		GEN7_PERFCOUNTER_GROUP(SP, sp, gen7_hwsched_counter_enable,
				       gen7_counter_read),
		GEN7_REGULAR_PERFCOUNTER_GROUP(RB, rb),
		GEN7_REGULAR_PERFCOUNTER_GROUP(VSC, vsc),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, VBIF, gbif, 0,
					     gen7_counter_gbif_enable,
					     gen7_counter_read_norestore),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, VBIF_PWR, gbif_pwr,
					     ADRENO_PERFCOUNTER_GROUP_FIXED,
					     gen7_counter_gbif_pwr_enable,
					     gen7_counter_read_norestore),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, ALWAYSON, alwayson,
					     ADRENO_PERFCOUNTER_GROUP_FIXED,
					     gen7_counter_alwayson_enable,
					     gen7_counter_alwayson_read),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, GMU_XOCLK, gmu_xoclk, 0,
					     gen7_counter_gmu_xoclk_enable,
					     gen7_counter_read_norestore),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, GMU_GMUCLK, gmu_gmuclk, 0,
					     gen7_counter_gmu_gmuclk_enable,
					     gen7_counter_read_norestore),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, GMU_PERF, gmu_perf, 0,
					     gen7_counter_gmu_perf_enable,
					     gen7_counter_read_norestore),
		...
		GEN7_BV_PERFCOUNTER_GROUP(TSE, tse, gen7_counter_bv_enable,
					  gen7_counter_read),
		GEN7_BV_PERFCOUNTER_GROUP(RAS, ras, gen7_counter_bv_enable,
					  gen7_counter_read),
		GEN7_BV_PERFCOUNTER_GROUP(LRZ, lrz, gen7_counter_bv_enable,
					  gen7_counter_read),
		GEN7_BV_PERFCOUNTER_GROUP(HLSQ, hlsq, gen7_counter_bv_enable,
					  gen7_counter_read),
	};

static const struct adreno_perfcount_group
	gen7_perfcounter_groups[KGSL_PERFCOUNTER_GROUP_MAX] = {
		GEN7_REGULAR_PERFCOUNTER_GROUP(CP, cp),
		GEN7_PERFCOUNTER_GROUP_FLAGS(gen7, RBBM, rbbm, 0,
					     gen7_counter_enable,
					     gen7_counter_read),
