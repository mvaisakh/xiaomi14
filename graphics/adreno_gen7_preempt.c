...
#define PREEMPT_SMMU_RECORD(_field) offsetof(struct gen7_cp_smmu_info, _field)
			 ...
			 bool atomic)
{
     ...
     */
		if (rb->skip_inline_wptr) {
			ret = gen7_fenced_write(
				adreno_dev, GEN7_CP_RB_WPTR, rb->wptr,
...
static void _power_collapse_set(struct adreno_device *adreno_dev, bool val)
{
	...
	gmu_core_regwrite(device, GEN7_GMU_PWR_COL_PREEMPT_KEEPALIVE,
			  (val ? 1 : 0));
}
...
static void _gen7_preemption_done(struct adreno_device *adreno_dev)
{
   ...
   */
	...
	if (status & 0x1) {
		dev_err(device->dev,
			"Preemption not complete: status=%X cur=%d R/W=%X/%X next=%d "
			"R/W=%X/%X\n",
...
static void _gen7_preemption_fault(struct adreno_device *adreno_dev)
{
   ...
   */
	...
	dev_err(device->dev,
		...
		adreno_dev->cur_rb->id, adreno_get_rptr(adreno_dev->cur_rb),
		adreno_dev->cur_rb->wptr, adreno_dev->next_rb->id,
		adreno_get_rptr(adreno_dev->next_rb),
		adreno_dev->next_rb->wptr);
...
static void _gen7_preemption_worker(struct work_struct *work)
{
	struct adreno_preemption *preempt =
		container_of(work, struct adreno_preemption, work);
	struct adreno_device *adreno_dev =
		container_of(preempt, struct adreno_device, preempt);
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
gen7_next_ringbuffer(struct adreno_device *adreno_dev)
{
...
void gen7_preemption_trigger(struct adreno_device *adreno_dev, bool atomic)
{
	...
	if (!adreno_move_preempt_state(adreno_dev, ADRENO_PREEMPT_NONE,
   ...
   */
	...
	kgsl_sharedmem_writel(next->preemption_desc, PREEMPT_RECORD(wptr),
			      next->wptr);
   ...
   */
	...
	if (gen7_fenced_write(
		    adreno_dev, GEN7_CP_CONTEXT_SWITCH_NON_PRIV_RESTORE_ADDR_LO,
		    lower_32_bits(gpuaddr), FENCE_STATUS_WRITEDROPPED1_MASK))
		goto err;

	if (gen7_fenced_write(
		    adreno_dev, GEN7_CP_CONTEXT_SWITCH_NON_PRIV_RESTORE_ADDR_HI,
		    upper_32_bits(gpuaddr), FENCE_STATUS_WRITEDROPPED1_MASK))
		goto err;
...
void gen7_preemption_callback(struct adreno_device *adreno_dev, int bit)
{
	...
	if (!adreno_move_preempt_state(adreno_dev, ADRENO_PREEMPT_TRIGGERED,
...
void gen7_preemption_prepare_postamble(struct adreno_device *adreno_dev)
{
	...
	/*
   * First 28 dwords of the device scratch buffer are used to store shadow rb
   * data. Reserve 15 dwords in the device scratch buffer from
   * SCRATCH_POSTAMBLE_OFFSET for KMD postamble pm4 packets. This should be in
   * *device->scratch* so that userspace cannot access it.
   */
	postamble = device->scratch->hostptr + SCRATCH_POSTAMBLE_OFFSET;

	/*
   * Reserve 4 dwords in the scratch buffer for dynamic QOS control feature. To
   * ensure QOS value is updated for first preemption, send it during bootup
   */
	if (gen7_core->qos_value) {
		postamble[count++] = cp_type7_packet(CP_MEM_TO_REG, 3);
		postamble[count++] = GEN7_RBBM_GBIF_CLIENT_QOS_CNTL;
		postamble[count++] = lower_32_bits(
			PREEMPT_SCRATCH_ADDR(adreno_dev, QOS_VALUE_IDX));
		postamble[count++] = upper_32_bits(
			PREEMPT_SCRATCH_ADDR(adreno_dev, QOS_VALUE_IDX));
	}
		...
		*cmds++ = upper_32_bits(gpuaddr);
		...
		if (adreno_dev->preempt.postamble_len) {
			u64 kmd_postamble_addr =
				SCRATCH_POSTAMBLE_ADDR(KGSL_DEVICE(adreno_dev));
...
u32 gen7_preemption_post_ibsubmit(struct adreno_device *adreno_dev, u32 *cmds)
{
	u32 index = 0;
	...
	if (adreno_dev->cur_rb) {
		u64 dest = PREEMPT_SCRATCH_ADDR(adreno_dev,
...
void gen7_preemption_start(struct adreno_device *adreno_dev)
{
	...
	if (kgsl_mmu_is_perprocess(&device->mmu)) {
		...
		kgsl_sharedmem_writeq(iommu->smmu_info,
				      PREEMPT_SMMU_RECORD(ttbr0),
				      MMU_DEFAULT_TTBR0(device));
	...
	FOR_EACH_RINGBUFFER(adreno_dev, rb, i)
	{
		kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(rptr),
				      0);
		kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(wptr),
				      0);
				    ...
				    struct adreno_ringbuffer *rb)
{
	...
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(magic),
			      GEN7_CP_CTXRECORD_MAGIC_REF);
	kgsl_sharedmem_writel(rb->preemption_desc, PREEMPT_RECORD(cntl),
			      GEN7_CP_RB_CNTL_DEFAULT);
	kgsl_sharedmem_writeq(rb->preemption_desc, PREEMPT_RECORD(rptr_addr),
			      SCRATCH_RB_GPU_ADDR(KGSL_DEVICE(adreno_dev),
						  rb->id, rptr));
	kgsl_sharedmem_writeq(rb->preemption_desc, PREEMPT_RECORD(rbase),
			      rb->buffer_desc->gpuaddr);
	kgsl_sharedmem_writeq(rb->preemption_desc, PREEMPT_RECORD(bv_rptr_addr),
			      SCRATCH_RB_GPU_ADDR(KGSL_DEVICE(adreno_dev),
						  rb->id, bv_rptr));
}
					   ...
					   struct adreno_ringbuffer *rb)
{
	...
	ret = adreno_allocate_global(device, &rb->perfcounter_save_restore_desc,
				     GEN7_CP_PERFCOUNTER_SAVE_RESTORE_SIZE, 0,
...
int gen7_preemption_init(struct adreno_device *adreno_dev)
{
	...
	ret = adreno_allocate_global(device, &preempt->scratch, PAGE_SIZE, 0, 0,
				     flags, "preempt_scratch");
	if (ret)
		goto done;

	/* Allocate mem for storing preemption smmu record */
	if (kgsl_mmu_is_perprocess(&device->mmu)) {
		ret = adreno_allocate_global(device, &iommu->smmu_info,
					     PAGE_SIZE, 0,
...
int gen7_preemption_context_init(struct kgsl_context *context)
{
   ...
   */
	context->user_ctxt_record = gpumem_alloc_entry(
		context->dev_priv, GEN7_CP_CTXRECORD_USER_RESTORE_SIZE, flags);
