		   ...
		   struct dma_buf *dmabuf)
{
   ...
   */
	...
	ret = of_property_read_u32(client_dev->of_node, "qti,smmu-proxy-cb-id",
				   &req->cb_id);
	if (ret) {
		dev_err(client_dev,
			"%s: Err reading 'qti,smmu-proxy-cb-id' rc: %d\n",
			__func__, ret);
		goto free_buf;
	}
   ...
   */
	...
	if (ret) {
		pr_err_ratelimited(
			"%s: Failed to set vmperm destructor, rc: %d\n",
			__func__, ret);
		goto free_buf;
	}
	...
	/*
   * We set the number of entries to one here, as we only allow the mapping to
   * go through on the TVM if the sg_table returned by dma_buf_map_attachment
   * has one entry.
   */
	proxy_iova->nents = 1;
				 ...
				 unsigned long arg)
{
	...
	case QTI_SMMU_PROXY_GET_VERSION_IOCTL: {
		struct csf_version *csf_version = &ioctl_arg.csf_version;
...
static int sender_probe_handler(struct platform_device *pdev)
{
	...
	if (csf_version.arch_ver == 2 && csf_version.max_ver == 0) {
		...
		ret = qti_smmu_proxy_register_callbacks(smmu_proxy_map,
							smmu_proxy_unmap_nop);
	} else {
		pr_err("%s: Invalid CSF version: %d.%d\n", __func__,
		       csf_version.arch_ver, csf_version.max_ver);
		goto free_msgq;
	}
