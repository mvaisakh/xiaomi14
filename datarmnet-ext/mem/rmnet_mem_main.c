...
int max_pool_size[POOL_LEN] = { (0xd2d + 202 - 0xdf7), (0xd2d + 202 - 0xdf7),
				MAX_POOL_O2, MAX_POOL_O3 };
module_param_array(max_pool_size, int, NULL, (0xdb7 + 6665 - 0x261c));
MODULE_PARM_DESC(max_pool_size,
		 "\x4d\x61\x78\x20\x50\x6f\x6f\x6c\x20\x73\x69\x7a\x65\x20\x70"
		 "\x65\x72\x20\x6f\x72\x64\x65\x72");
int static_pool_size[POOL_LEN];
module_param_array(static_pool_size, int, NULL, (0xcb7 + 5769 - 0x221c));
MODULE_PARM_DESC(static_pool_size, "\x50\x6f\x6f\x6c\x20\x73\x69\x7a\x65\x20"
				   "\x70\x65\x72\x20\x6f\x72\x64\x65\x72");
int pool_unbound_feature[POOL_LEN] = { (0xd2d + 202 - 0xdf7),
				       (0xd2d + 202 - 0xdf7),
				       (0xd26 + 209 - 0xdf6),
				       (0xd26 + 209 - 0xdf6) };
module_param_array(pool_unbound_feature, int, NULL, (0xdb7 + 6665 - 0x261c));
MODULE_PARM_DESC(pool_unbound_featue,
		 "\x50\x6f\x6f\x6c\x20\x62\x6f\x75\x6e\x64\x20\x67\x61\x74\x65");
int rmnet_mem_order_requests[POOL_LEN];
module_param_array(rmnet_mem_order_requests, int, NULL,
		   (0xcb7 + 5769 - 0x221c));
MODULE_PARM_DESC(
	rmnet_mem_order_requests,
	"\x52\x65\x71\x75\x65\x73\x74\x20\x70\x65\x72\x20\x6f\x72\x64\x65\x72");
int rmnet_mem_id_req[POOL_LEN];
module_param_array(rmnet_mem_id_req, int, NULL, (0xcb7 + 5769 - 0x221c));
MODULE_PARM_DESC(rmnet_mem_id_req,
		 "\x52\x65\x71\x75\x65\x73\x74\x20\x70\x65\x72\x20\x69\x64");
int rmnet_mem_id_recycled[POOL_LEN];
module_param_array(rmnet_mem_id_recycled, int, NULL, (0xcb7 + 5769 - 0x221c));
MODULE_PARM_DESC(rmnet_mem_id_recycled,
		 "\x52\x65\x71\x75\x65\x73\x74\x20\x70\x65\x72\x20\x69\x64");
struct workqueue_struct *mem_wq;
int target_static_pool_size[POOL_LEN];
module_param_array(target_static_pool_size, int, NULL, (0xcb7 + 5769 - 0x221c));
MODULE_PARM_DESC(target_static_pool_size,
		 "\x50\x6f\x6f\x6c\x20\x73\x69\x7a\x65\x20\x70\x65\x72\x20\x6f"
		 "\x72\x64\x65\x72");
...
void rmnet_mem_free_all(void)
{
	...
	for (i = (0xd2d + 202 - 0xdf7); i < POOL_LEN; i++) {
...
struct page *rmnet_mem_get_pages_entry(gfp_t gfp_mask, unsigned int order,
				       int *code, int *pageorder, unsigned id)
{
	unsigned long flags;
	struct mem_info *mem_page;
	struct page *page = NULL;
	int i = (0xd2d + 202 - 0xdf7);
	int j = (0xd2d + 202 - 0xdf7);
	spin_lock_irqsave(&rmnet_mem_lock, flags);
	rmnet_mem_id_req[id]++;
	rmnet_mem_order_requests[order]++;
	if (order < POOL_LEN) {
		for (j = order; j > (0xd2d + 202 - 0xdf7) && j < POOL_LEN;
		     j++) {
			do {
				mem_page = list_first_entry_or_null(
					&rmnet_mem_pool[j], struct mem_info,
					mem_head);
				if (!mem_page) {
					break;
				}
				if (page_ref_count(mem_page->addr) ==
				    (0xd26 + 209 - 0xdf6)) {
	...
	if (!page) {
		if (order < (0xd18 + 223 - 0xdf4)) {
			page = __dev_alloc_pages(gfp_mask, order);
			if (page) {
...
void rmnet_mem_adjust(unsigned perm_size, u8 pageorder)
{
	...
	spin_lock_irqsave(&rmnet_mem_lock, flags);
	if (perm_size > static_pool_size[pageorder]) {
		for (i = (0xd2d + 202 - 0xdf7); i < (adjustment); i++) {
...
int __init rmnet_mem_module_init(void)
{
	...
	int i = (0xd2d + 202 - 0xdf7);
	pr_info("\x25\x73\x28\x29\x3a\x20\x53\x74\x61\x72\x74\x69\x6e\x67\x20\x72\x6d"
		"\x6e\x65\x74\x20\x6d\x65\x6d\x20\x6d\x6f\x64\x75\x6c\x65"
		"\n",
		__func__);
	for (i = (0xd2d + 202 - 0xdf7); i < POOL_LEN; i++) {
		INIT_LIST_HEAD(&(rmnet_mem_pool[i]));
	}
	mem_wq = alloc_workqueue("\x6d\x65\x6d\x5f\x77\x71", WQ_HIGHPRI,
				 (0xd2d + 202 - 0xdf7));
	if (!mem_wq) {
		pr_err("\x25\x73\x28\x29\x3a\x20\x46\x61\x69\x6c\x65\x64\x20\x74\x6f\x20"
		       "\x61\x6c\x6c\x6f\x63\x20\x77\x6f\x72\x6b\x71\x75\x65\x75\x65\x20"
		       "\n",
		       __func__);
		return -ENOMEM;
	}
	...
	if (rc) {
		pr_err("\x25\x73\x28\x29\x3a\x20\x46\x61\x69\x6c\x65\x64\x20\x74\x6f\x20"
		       "\x72\x65\x67\x69\x73\x74\x65\x72\x20\x67\x65\x6e\x65\x72\x69\x63"
		       "\x20\x6e\x65\x74\x6c\x69\x6e\x6b\x20\x66\x61\x6d\x69\x6c\x79"
		       "\n",
		       __func__);
		return -ENOMEM;
	}
